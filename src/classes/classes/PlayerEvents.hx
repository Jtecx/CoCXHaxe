...
class PlayerEvents extends BaseContent implements TimeAwareInterface
{
    ...
    private var LunaFullMoonScene : Bool = false;
    ...
    private function hourlyHunger() : Bool
    {
        var needNext : Bool = false;
        var hungerActive : Bool = false;
        if (flags[kFLAGS.HUNGER_ENABLED] > 0 || player.hasPerk(PerkLib.EndlessHunger))
        {
            hungerActive = true;
        }
        if (hungerActive)
        {
            ...
            else if (player.hasPerk(PerkLib.DeadMetabolism))
            {
                hungerActive = false;
            }
        }
        if (hungerActive)
        {
            ...
            if (player.hasPerk(PerkLib.Survivalist))
            {
                multiplier -= 0.2;
            }
            if (player.hasPerk(PerkLib.Survivalist2))
            {
                multiplier -= 0.2;
            }
            if (player.hasPerk(PerkLib.Survivalist3))
            {
                multiplier -= 0.2;
            }
            if (player.hasPerk(PerkLib.Metabolization))
            {
                multiplier -= 0.2;
            }
            if (player.hasPerk(PerkLib.ImprovedMetabolization))
            {
                multiplier -= 0.2;
            }
            if (player.hasPerk(PerkLib.GreaterMetabolization))
            {
                multiplier -= 0.2;
            }
            if (player.hasPerk(PerkLib.HighlyVenomousDiet) && player.tailVenom < player.maxVenom())
            {
                if (player.maxHunger() > 1600)
                {
                    multiplier += 0.25;
                }
                else if (player.maxHunger() > 800)
                {
                    multiplier += 0.25;
                }
                else if (player.maxHunger() > 400)
                {
                    multiplier += 0.25;
                }
                else if (player.maxHunger() > 200)
                {
                    multiplier += 0.25;
                }
                else if (player.maxHunger() > 100)
                {
                    multiplier += 0.25;
                }
                else
            ...
            if (player.hasPerk(PerkLib.ManticoreCumAddict))
            {
                multiplier *= 2;
            }
            if (player.hasPerk(PerkLib.HydraRegeneration))
            {
                multiplier *= 2;
            }
            if (player.hasPerk(PerkLib.AxillaryVenomGlands) && player.tailVenom < player.maxVenom())
            {
                multiplier *= 2;
            }
            ...
            if (player.hasStatusEffect(StatusEffects.FastingPill))
            {
                player.hunger += 1;
            }
            if (player.internalChimeraRating() >= 1)
            {
                player.hunger -= (0.5 * player.internalChimeraRating());
            }
            if (player.hunger > 80)
            {
                player.hunger -= (0.5 * multiplier);
            }  //If satiated, depletes at 2 points per hour.  
            if (player.hunger > 50)
            {
                player.hunger -= (0.5 * multiplier);
            }
            ...
            if (player.buttPregnancyType == PregnancyStore.PREGNANCY_GOO_STUFFED)
            {
                player.hunger = player.maxHunger();
            }  //After Valeria x Goo Girl, you'll never get hungry until you "birth" the goo-girl.  
            if (player.hunger < 50 && player.hasPerk(PerkLib.MagicMetabolism))
            {
                var manaDrain : Float = 0;
            ...
            if (player.hunger <= 0)
            {
                ...
                {
                    campUniqueScene.droppingToZeroSatietyAsGargoyle();
                }
                ...
                else
                {
                    ...
                    else if ((model.time.hours + 2) % 4 == 0)
                    
                    //Lose thickness 2x as fast.{
            ...
            if (player.hunger < 25)
            {
                if (player.hunger > 0)
                {
                    flags[kFLAGS.ACHIEVEMENT_PROGRESS_FASTING]++;
                }
                else
                {
                    flags[kFLAGS.ACHIEVEMENT_PROGRESS_FASTING] = 0;
                }
            }
            else
            {
    ...
    private function hourlyTimersProc() : Bool
    {
        var needNext : Bool = false;
        
        if (player.cumMultiplier > 19999)
        {
            player.cumMultiplier = 19999;
        }
        if (player.ballSize > 400)
        {
            player.ballSize = 400;
        }
        ...
        if (flags[kFLAGS.SOCK_COUNTER] > 0)
        {
            ...
            if (flags[kFLAGS.SOCK_COUNTER] < 0)
            {
                flags[kFLAGS.SOCK_COUNTER] = 0;
            }
            if (flags[kFLAGS.SOCK_COUNTER] > 24)
            {
                flags[kFLAGS.SOCK_COUNTER] = 24;
            }
        }
        if (flags[kFLAGS.HUNGER_ENABLED] <= 0 || (flags[kFLAGS.HUNGER_ENABLED] > 0 && player.hunger >= 10))
        
        //If you're starving, your cum won't build up over time.{
            
            player.hoursSinceCum++;
            //Super cumbuilding activate!
            if (player.hasPerk(PerkLib.MaraesGiftProfractory))
            {
                player.hoursSinceCum += 2;
            }
        ...
        if (!player.hasPerk(PerkLib.WellAdjusted))
        {
            ...
            {
                dynStats("lus", player.libStat.totalCore * 0.01, "scale", false);
            }
        }
        //Well adjusted perk
        else
        {
            ...
            if (player.hasPerk(PerkLib.Lusty))
            {
                dynStats("lus", player.libStat.totalCore * 0.005, "scale", false);
            }
        }
        if (player.perkv1(IMutationsLib.HumanMetabolismIM) >= 1)
        {
            dynStats("lus", -(Math.round(player.maxLust() * 0.01 * player.perkv1(IMutationsLib.HumanMetabolismIM))));
        }
        if (player.perkv1(IMutationsLib.HumanMetabolismIM) >= 3)
        {
            EngineCore.changeFatigue(-(Math.round(player.maxFatigue() * 0.01)));
        }
        if (player.perkv1(IMutationsLib.HumanDigestiveTractIM) >= 3)
        {
            dynStats("lus", -(Math.round(player.maxLust() * 0.01)));
        }
        //Jewelry effect
        ...
        {
            flags[kFLAGS.EVANGELINE_WENT_OUT_FOR_THE_ITEMS]--;
        }
        //cumOmeter dropping down
        if (flags[kFLAGS.SEXUAL_FLUIDS_LEVEL] > 0)
        
        /*if (flags[kFLAGS.SEXUAL_FLUIDS_LEVEL] == 1) */{
            flags[kFLAGS.SEXUAL_FLUIDS_LEVEL]--;
        }
        //Corruption check for achievement.
        if (player.cor >= 80)
        {
            if (flags[kFLAGS.ACHIEVEMENT_PROGRESS_SCHIZOPHRENIA] == 0 || flags[kFLAGS.ACHIEVEMENT_PROGRESS_SCHIZOPHRENIA] == 2)
            {
                flags[kFLAGS.ACHIEVEMENT_PROGRESS_SCHIZOPHRENIA]++;
            }
        }
        if (player.cor <= 20)
        {
            if (flags[kFLAGS.ACHIEVEMENT_PROGRESS_SCHIZOPHRENIA] == 1 || flags[kFLAGS.ACHIEVEMENT_PROGRESS_SCHIZOPHRENIA] == 3)
            {
                flags[kFLAGS.ACHIEVEMENT_PROGRESS_SCHIZOPHRENIA]++;
            }
        }
        if (player.cor >= 100)
        {
            if (flags[kFLAGS.ACHIEVEMENT_PROGRESS_CLEAN_SLATE] == 0)
            {
                flags[kFLAGS.ACHIEVEMENT_PROGRESS_CLEAN_SLATE]++;
            }
        }
        if (player.cor <= 0)
        {
            if (flags[kFLAGS.ACHIEVEMENT_PROGRESS_CLEAN_SLATE] == 1)
            {
                flags[kFLAGS.ACHIEVEMENT_PROGRESS_CLEAN_SLATE]++;
            }
        }
        ...
        if (SceneLib.valeria.valeriaFluidsEnabled())
        {
            ...
            if (flags[kFLAGS.VALERIA_FLUIDS] > 100)
            {
        ...
        {
            if (player.statusEffectv1(StatusEffects.CampSparingNpcsTimers1) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 1, -1);
            }
            if (player.statusEffectv2(StatusEffects.CampSparingNpcsTimers1) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 2, -1);
            }
            if (player.statusEffectv3(StatusEffects.CampSparingNpcsTimers1) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 3, -1);
            }
            if (player.statusEffectv4(StatusEffects.CampSparingNpcsTimers1) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 4, -1);
            }
        ...
        {
            if (player.statusEffectv1(StatusEffects.CampSparingNpcsTimers2) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers2, 1, -1);
            }
            if (player.statusEffectv2(StatusEffects.CampSparingNpcsTimers2) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers2, 2, -1);
            }
            if (player.statusEffectv3(StatusEffects.CampSparingNpcsTimers2) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers2, 3, -1);
            }
            if (player.statusEffectv4(StatusEffects.CampSparingNpcsTimers2) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers2, 4, -1);
            }
        ...
        {
            if (player.statusEffectv1(StatusEffects.CampSparingNpcsTimers3) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers3, 1, -1);
            }
            if (player.statusEffectv2(StatusEffects.CampSparingNpcsTimers3) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers3, 2, -1);
            }
            if (player.statusEffectv3(StatusEffects.CampSparingNpcsTimers3) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers3, 3, -1);
            }
            if (player.statusEffectv4(StatusEffects.CampSparingNpcsTimers3) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers3, 4, -1);
            }
        ...
        {
            if (player.statusEffectv1(StatusEffects.CampSparingNpcsTimers4) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 1, -1);
            }
            if (player.statusEffectv2(StatusEffects.CampSparingNpcsTimers4) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 2, -1);
            }
            if (player.statusEffectv3(StatusEffects.CampSparingNpcsTimers4) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 3, -1);
            }
            if (player.statusEffectv4(StatusEffects.CampSparingNpcsTimers4) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 4, -1);
            }
        ...
        {
            if (player.statusEffectv1(StatusEffects.CampSparingNpcsTimers5) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers5, 1, -1);
            }
            if (player.statusEffectv2(StatusEffects.CampSparingNpcsTimers5) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers5, 2, -1);
            }
            if (player.statusEffectv3(StatusEffects.CampSparingNpcsTimers5) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers5, 3, -1);
            }
            if (player.statusEffectv4(StatusEffects.CampSparingNpcsTimers5) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers5, 4, -1);
            }
        ...
        {
            if (player.statusEffectv1(StatusEffects.CampSparingNpcsTimers6) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers6, 1, -1);
            }
            if (player.statusEffectv2(StatusEffects.CampSparingNpcsTimers6) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers6, 2, -1);
            }
            if (player.statusEffectv3(StatusEffects.CampSparingNpcsTimers6) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers6, 3, -1);
            }
            if (player.statusEffectv4(StatusEffects.CampSparingNpcsTimers6) > 0)
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers6, 4, -1);
            }
        ...
        if (player.isInAyoArmor() && player.buff("Ayo Armor").isPresent())
        {
            if (flags[kFLAGS.SOULFORCE_STORED_IN_AYO_ARMOR] > 0)
            {
                ...
                if (player.hasPerk(PerkLib.ConductionSoulforceCharge) && player.soulforce >= drainAmt)
                {
                    EngineCore.SoulforceChange(-drainAmt);
                }
                else
                {
                    flags[kFLAGS.SOULFORCE_STORED_IN_AYO_ARMOR] -= drainAmt;
                }
            }
            if (flags[kFLAGS.SOULFORCE_STORED_IN_AYO_ARMOR] <= 0)
            {
        ...
        if (player.hasStatusEffect(StatusEffects.KonstantinArmorPolishing))
        {
            ...
            {
                player.removeStatusEffect(StatusEffects.KonstantinArmorPolishing);
            }
            else
            {
                player.addStatusValue(StatusEffects.KonstantinArmorPolishing, 1, -1);
            }
        }
        if (player.hasStatusEffect(StatusEffects.KonstantinWeaponSharpening))
        {
            ...
            {
                player.removeStatusEffect(StatusEffects.KonstantinWeaponSharpening);
            }
            else
            {
                player.addStatusValue(StatusEffects.KonstantinWeaponSharpening, 1, -1);
            }
        }
        if (player.hasStatusEffect(StatusEffects.AlchemicalThunderBuff))
        {
            player.removeStatusEffect(StatusEffects.AlchemicalThunderBuff);
        }
        //Sidonie checks
        if (flags[kFLAGS.SIDONIE_RECOLLECTION] > 0)
        {
            flags[kFLAGS.SIDONIE_RECOLLECTION]--;
        }
        if (flags[kFLAGS.LUNA_FOLLOWER] >= 4 && !player.hasStatusEffect(StatusEffects.LunaOff))
        {
            if (flags[kFLAGS.LUNA_JEALOUSY] < 400)
            {
                flags[kFLAGS.LUNA_JEALOUSY]++;
            }
            if ((flags[kFLAGS.LUNA_FOLLOWER] % 2 == 0 && flags[kFLAGS.LUNA_FOLLOWER] > 4) && flags[kFLAGS.LUNA_JEALOUSY] < 100)
            {
                flags[kFLAGS.LUNA_FOLLOWER]--;
            }
            if ((flags[kFLAGS.LUNA_FOLLOWER] % 2 == 1) && flags[kFLAGS.LUNA_JEALOUSY] >= 100 && (CoC.instance.model.time.hours > 6 && CoC.instance.model.time.hours < 23))
            {
                SceneLib.lunaFollower.warrningAboutJelously();
            }
        }
        //Zenji loneliness
        ...
        if (player.statusEffectv3(StatusEffects.TelAdreTripxi) > 0)
        {
            player.addStatusValue(StatusEffects.TelAdreTripxi, 3, -1);
        }
        ...
        {
            if (player.statusEffectv1(StatusEffects.DinahGift) <= 0)
            {
                player.removeStatusEffect(StatusEffects.DinahGift);
            }
            else
            {
                player.addStatusValue(StatusEffects.DinahGift, 1, -1);
            }
        }
        //Spooders pregnancy
        if (DriderTown.BelisaPregnancy > 1)
        {
            DriderTown.BelisaPregnancy -= 1;
        }
        if (DriderTown.BelisaKidsEggsHatching > 1)
        {
            DriderTown.BelisaKidsEggsHatching -= 1;
        }
        if (DriderTown.BelisaKidsEggsHatching1 > 1)
        {
            DriderTown.BelisaKidsEggsHatching1 -= 1;
        }
        if (DriderTown.BelisaKidsEggsHatching2 > 1)
        {
            DriderTown.BelisaKidsEggsHatching2 -= 1;
        }
        if (DriderTown.LilyKidsPCPregnancy > 1)
        {
            DriderTown.LilyKidsPCPregnancy -= 1;
        }
        if (DriderTown.LilyKidsPCEggsHatching > 1)
        {
            DriderTown.LilyKidsPCEggsHatching -= 1;
        }
        if (DriderTown.LilyKidsPCEggsHatching1 > 1)
        {
            DriderTown.LilyKidsPCEggsHatching1 -= 1;
        }
        if (DriderTown.LilyKidsPCEggsHatching2 > 1)
        {
            DriderTown.LilyKidsPCEggsHatching2 -= 1;
        }
        if (DriderTown.TyrantiaPregnancy > 1)
        {
            DriderTown.TyrantiaPregnancy -= 1;
        }
        if (DriderTown.TyrantiaKidsEggsHatching > 1)
        {
            DriderTown.TyrantiaKidsEggsHatching -= 1;
        }
        if (DriderTown.TyrantiaKidsEggsHatching1 > 1)
        {
            DriderTown.TyrantiaKidsEggsHatching1 -= 1;
        }
        if (DriderTown.TyrantiaKidsEggsHatching2 > 1)
        {
            DriderTown.TyrantiaKidsEggsHatching2 -= 1;
        }
        //Gardening
        if (Garden.GardenSlot01Time > 0)
        {
            Garden.GardenSlot01Time -= 1;
        }
        if (Garden.GardenSlot02Time > 0)
        {
            Garden.GardenSlot02Time -= 1;
        }
        if (Garden.GardenSlot03Time > 0)
        {
            Garden.GardenSlot03Time -= 1;
        }
        if (Garden.GardenSlot04Time > 0)
        {
            Garden.GardenSlot04Time -= 1;
        }
        if (Garden.GardenSlot05Time > 0)
        {
            Garden.GardenSlot05Time -= 1;
        }
        if (Garden.GardenSlot06Time > 0)
        {
            Garden.GardenSlot06Time -= 1;
        }
        if (Garden.GardenSlot07Time > 0)
        {
            Garden.GardenSlot07Time -= 1;
        }
        if (Garden.GardenSlot08Time > 0)
        {
            Garden.GardenSlot08Time -= 1;
        }
        if (Garden.GardenSlot09Time > 0)
        {
            Garden.GardenSlot09Time -= 1;
        }
        if (Garden.GardenSlot10Time > 0)
        {
            Garden.GardenSlot10Time -= 1;
        }
        if (Garden.GardenSlot11Time > 0)
        {
            Garden.GardenSlot11Time -= 1;
        }
        if (Garden.GardenSlot12Time > 0)
        {
            Garden.GardenSlot12Time -= 1;
        }
        //Ruined Town
        if (RuinedTownRebuilt.AmilyAngerCooldown > 0)
        {
            RuinedTownRebuilt.AmilyAngerCooldown -= 1;
        }
        ...
        //Balls{
            ...
            if (player.ballSize > lim && player.hasBalls())
            {
                tempSpeedPenalty += Math.round((player.ballSize - lim) / 2);
            }
            //Breasts
            lim = (player.isTaur()) ? BreastCup.I : BreastCup.G;
            if (player.hasBreasts() && player.biggestTitSize() > lim)
            {
                tempSpeedPenalty += ((player.biggestTitSize() - lim) / 2);
            }
            //Cocks
            lim = (player.isTaur()) ? 72 : 24;
            if (player.biggestCockArea() > lim)
            {
                tempSpeedPenalty += ((player.biggestCockArea() - lim) / 6);
            }
            //Min-cap
        ...
        if (player.hasPerk(PerkLib.MummyLord) && player.racialScore(Races.ANUBIS) < 15 && player.cor < (50 - player.corruptionTolerance))
        {
            ...
            {
                var mummies : Float = player.perkv1(PerkLib.MummyLord);
                var dura : Float = player.perkv2(PerkLib.MummyLord);
        ...
        if (player.hasPerk(PerkLib.Familiar) && player.racialScore(Races.VAMPIRE) < 20 && player.racialScore(Races.DRACULA) < 22)
        {
            ...
            if (flags[kFLAGS.PLAYER_COMPANION_1] == "" + flags[kFLAGS.GHOULISH_VAMPIRE_SERVANT_NAME] + "")
            {
                flags[kFLAGS.PLAYER_COMPANION_1] = "";
            }
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "" + flags[kFLAGS.GHOULISH_VAMPIRE_SERVANT_NAME] + "")
            {
                flags[kFLAGS.PLAYER_COMPANION_2] = "";
            }
            if (flags[kFLAGS.PLAYER_COMPANION_3] == "" + flags[kFLAGS.GHOULISH_VAMPIRE_SERVANT_NAME] + "")
            {
        ...
        if (player.hasStatusEffect(StatusEffects.WormPlugged) && flags[kFLAGS.PLAYER_PREGGO_WITH_WORMS] == 0)
        ...
        //Update worm drippy-cooch{
            ...
            if (player.hasVagina())
            {
                if (rand(5) == 0)
                {
                    ...
                    {
                        player.fertility -= (1 + Math.round(player.fertility / 4));
                    }
                    player.addStatusValue(StatusEffects.WormPlugged, 1, -1);  //Lower chances  
                    if (player.statusEffectv1(StatusEffects.WormPlugged) <= 0)
        ...
        if (player.hasStatusEffect(StatusEffects.Dysfunction))
        {
            ...
            {
                player.addStatusValue(StatusEffects.Dysfunction, 1, -1);
            }
        }
        ...
        if (player.pregnancy2Incubation <= 0 && player.pregnancy2Type == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS)
        ...
        if (player.biggestLactation() > 0 && !player.hasStatusEffect(StatusEffects.Feeder) && !player.hasPerk(PerkLib.MilkMaid) && !player.isPregnant())
        {
            ...
            else
            {
                ...
                {
                    var before : Float = player.biggestLactation();
                    player.boostLactation(-0.5 * player.breastRows.length / 24);  //decrease by 0.5 each day  
                    var after : Float = player.biggestLactation();
        ...
        if (player.hasStatusEffect(StatusEffects.CuntStretched))
        ...
        needNext = (needNext) ? needNext : player.gainPerk(PerkLib.ImprovedVenomGland, flags[kFLAGS.VENOM_TIMES_USED] >= 50, "You feel wonderfully healthy. After using your venom so many times your body finally got acclimated to the presence of your venom gland allowing for increased capacity and production.");
        needNext = (needNext) ? needNext : player.gainPerk(PerkLib.ImprovedVenomGlandEx, flags[kFLAGS.VENOM_TIMES_USED] >= 125, "You feel wonderfully healthy. After using your venom so many times your venom gland development reached it next stage. Allowing for increased capacity, production and lowering usage of venom.");
        needNext = (needNext) ? needNext : player.gainPerk(PerkLib.ImprovedVenomGlandSu, flags[kFLAGS.VENOM_TIMES_USED] >= 375, "You feel wonderfully healthy. After using your venom so many times your venom gland started to produce more potent venom. Allowing for increased capacity, production and increased effects of venom.");
        
        needNext = (needNext) ? needNext : player.gainPerk(PerkLib.GeneticMemory, flags[kFLAGS.TIMES_TRANSFORMED] >= 25, "Your body behave weirdly as if all the transformation, which you have undergone started to make it unsure about what it truly is. Sometime you even try to move limbs that are no longer there. Suddenly you realise that no mather how many time you change your body remembers it. Your body developed genetic memory!");
        needNext = (needNext) ? needNext : player.gainPerk(PerkLib.TransformationResistance, flags[kFLAGS.TIMES_TRANSFORMED] >= 100, "You feel a strange tingling sensation. It seems as if you've finally adapted to the transformative properties of the food in Mareth and your body has finally built up enough resistance! You suspect that you can still transform but at somewhat diminished rate.");
        needNext = (needNext) ? needNext : player.gainPerk(PerkLib.TransformationAcclimation, flags[kFLAGS.TIMES_TRANSFORMED] >= 200, "Due to you're continued consumption of transformative items, you have discovered a way to draw out more of the transformative properties of the food in Mareth.");
        if (player.hasPerk(PerkLib.DominantAlpha))
        {
            player.DominantAlphaBonus();
        }
        if (player.inHeat)
        
        //Heats v1 is bonus fertility, v2 is bonus libido, v3 is hours till it's gone{
            
            if (player.statusEffectv3(StatusEffects.Heat) <= 1 || player.vaginas.length == 0)
            ...
            {
                player.addStatusValue(StatusEffects.Heat, 3, -1);
            }
        }
        ...
        //Rut v1 is bonus cum, v2 is bonus libido, v3 is hours till it's gone{
            ...
            if (player.statusEffectv3(StatusEffects.Rut) <= 1 || player.cockTotal() == 0)
            ...
            {
                player.addStatusValue(StatusEffects.Rut, 3, -1);
            }
        }
        if (player.statusEffectv1(StatusEffects.BathedInHotSpring) >= 1)
        {
            ...
            {
                player.addStatusValue(StatusEffects.BathedInHotSpring, 1, -1);
            }
        }
        if (player.statusEffectv1(StatusEffects.ShiraOfTheEastFoodBuff1) >= 1)
        {
            ...
            {
                player.addStatusValue(StatusEffects.ShiraOfTheEastFoodBuff1, 1, -1);
            }
        }
        if (player.statusEffectv1(StatusEffects.RaijuLightningStatus) >= 1)
        {
            ...
            {
                player.addStatusValue(StatusEffects.RaijuLightningStatus, 1, -1);
            }
        }
        if (player.statusEffectv1(StatusEffects.SoulGemCrafting) >= 1)
        {
            player.addStatusValue(StatusEffects.SoulGemCrafting, 1, -1);
        }
        ...
        if (player.hasStatusEffect(StatusEffects.Bammed1))
        {
            if (player.statusEffectv3(StatusEffects.Bammed1) > 0 && (player.statusEffectv1(StatusEffects.Bammed1) == 1 || player.statusEffectv1(StatusEffects.Bammed1) == 2))
            {
                ...
                {
                    if (player.statusEffectv4(StatusEffects.Bammed1) <= 0 && player.statusEffectv2(StatusEffects.Bammed1) < 3)
                    {
                        player.removeStatusEffect(StatusEffects.Bammed1);
                    }
            ...
            if (player.statusEffectv4(StatusEffects.Bammed1) > 0 && (player.statusEffectv2(StatusEffects.Bammed1) == 1 || player.statusEffectv2(StatusEffects.Bammed1) == 2))
            {
                ...
                {
                    if (player.statusEffectv3(StatusEffects.Bammed1) <= 0 && player.statusEffectv1(StatusEffects.Bammed1) < 3)
                    {
                        player.removeStatusEffect(StatusEffects.Bammed1);
                    }
        ...
        if (player.hasStatusEffect(StatusEffects.Bammed2))
        {
            if (player.statusEffectv3(StatusEffects.Bammed2) > 0 && (player.statusEffectv1(StatusEffects.Bammed2) == 1 || player.statusEffectv1(StatusEffects.Bammed2) == 2))
            {
                ...
                {
                    if (player.statusEffectv4(StatusEffects.Bammed2) <= 0 && player.statusEffectv2(StatusEffects.Bammed2) < 3)
                    {
                        player.removeStatusEffect(StatusEffects.Bammed2);
                    }
            ...
            if (player.statusEffectv4(StatusEffects.Bammed2) > 0 && (player.statusEffectv2(StatusEffects.Bammed2) == 1 || player.statusEffectv2(StatusEffects.Bammed2) == 2))
            {
                ...
                {
                    if (player.statusEffectv3(StatusEffects.Bammed2) <= 0 && player.statusEffectv1(StatusEffects.Bammed2) < 3)
                    {
                        player.removeStatusEffect(StatusEffects.Bammed2);
                    }
        ...
        {
            if (player.statusEffectv1(StatusEffects.CombatWounds) > 0.01)
            {
                player.addStatusValue(StatusEffects.CombatWounds, 1, -0.01);
            }
            else
            {
        ...
        if (player.statusEffectv2(StatusEffects.Kelt) > 0)
        {
            player.addStatusValue(StatusEffects.Kelt, 2, -0.15);
        ...
        if (player.hasStatusEffect(StatusEffects.Fullness))
        {
            ...
            if (player.statusEffectv1(StatusEffects.Fullness) <= 0)
            {
                player.removeStatusEffect(StatusEffects.Fullness);
            }
        }
        ...
        {
            //flags[kFLAGS.MINOCUM_WARNING_COOLDOWN] prevents PC getting two of the same notices overniteif (flags[kFLAGS.MINOCUM_WARNING_COOLDOWN] > 0)
            {
                flags[kFLAGS.MINOCUM_WARNING_COOLDOWN]--;
            }
        }
        if (player.statusEffectv1(StatusEffects.Airweed) >= 1)
        {
            ...
            {
                player.addStatusValue(StatusEffects.Airweed, 1, -1);
            }
        }
        ...
        if (player.flags[kFLAGS.LUSTSTICK_RESISTANCE] >= 50 && !player.hasPerk(PerkLib.LuststickAdapted))
        ...
        if (player.hasPerk(PerkLib.MaraesGiftButtslut) && player.ass.analWetness < 2)
        ...
        if (flags[kFLAGS.BIKINI_ARMOR_BONUS] > 0)
        {
            if (player.armorName == "lusty maiden's armor")
            {
                ...
                {
                    flags[kFLAGS.BIKINI_ARMOR_BONUS]--;
                }  //Adjust for inflation  
                if (flags[kFLAGS.BIKINI_ARMOR_BONUS] < 0)
                {
                    flags[kFLAGS.BIKINI_ARMOR_BONUS] = 0;
                }  //Keep in bounds.  
                if (flags[kFLAGS.BIKINI_ARMOR_BONUS] > 8)
                {
                    flags[kFLAGS.BIKINI_ARMOR_BONUS] = 8;
                }
            }
            else
            {
        ...
        if (flags[kFLAGS.GATS_ANGEL_TIME_TO_FIND_KEY] > 0 && flags[kFLAGS.GATS_ANGEL_TIME_TO_FIND_KEY] < 500)
        {
            flags[kFLAGS.GATS_ANGEL_TIME_TO_FIND_KEY]++;
        }
        if (player.statStore.hasBuff("Crossed Holy Band") && player.cor > 0)
        {
            player.cor -= 1;
        }
        
        //Recharge venom/web pool
        if (player.tailType == Tail.BEE_ABDOMEN || player.tailType == Tail.SPIDER_ADBOMEN || player.tailType == Tail.SCORPION || player.tailType == Tail.MANTICORE_PUSSYTAIL || player.faceType == Face.SNAKE_FANGS || player.faceType == Face.SPIDER_FANGS || player.lowerBody == LowerBody.HYDRA || player.lowerBody == LowerBody.ATLACH_NACHA || player.lowerBody == LowerBody.SANDWORM || player.hasPerk(PerkLib.ImprovedVenomGland) || player.perkv1(IMutationsLib.VenomGlandsIM) >= 2 || player.hasPerk(PerkLib.VenomousDiet) || player.hasPerk(PerkLib.HighlyVenomousDiet) || player.hasPerk(PerkLib.AxillaryVenomGlands) || player.hasPerk(PerkLib.VenomousAdiposeTissue) || (player.hasKeyItem("Sky Poison Pearl") >= 0))
        
        //Spider, Bee, Scorpion, Manticore, Naga and Altach Nacha Venom Recharge{
            
            var venomHRecharge : Float = 0;
        ...
        if (flags[kFLAGS.AYANE_CURE_COOLDOWN] > 0)
        {
            --flags[kFLAGS.AYANE_CURE_COOLDOWN];
        }
        if (flags[kFLAGS.NADIA_CURE_COOLDOWN] > 0)
        {
            --flags[kFLAGS.NADIA_CURE_COOLDOWN];
        }
        //Luna nursing reset
        ...
        //Cow bell
        if (player.necklace == necklaces.COWBELL && player.isRaceCached(Races.COW) && !player.statStore.hasBuff("Cow bell"))
        {
        ...
        if (!player.isRaceCached(Races.COW) && player.statStore.hasBuff("Cow bell"))
        {
            player.statStore.removeBuffs("Cow bell");
        }
        //Jiangshi cursed tag
        if (player.headJewelry == headjewelries.JIANGCT && player.isRaceCached(Races.JIANGSHI) && !player.statStore.hasBuff("Jiangshi Curse Tag"))
        {
            player.statStore.replaceBuffObject({
                        str.mult : 0.2,
                        tou.mult : 0.2,
        ...
        if (player.hasStatusEffect(StatusEffects.TFWarning))
        {
            ...
            if (player.getStatusValue(StatusEffects.TFWarning, 3) <= 0)
            {
        ...
        {
            var vthirst : VampireThirstEffect = try cast(player.statusEffectByType(StatusEffects.VampireThirst), VampireThirstEffect) catch(e:Dynamic) null;
            if (vthirst != null)
            {
                if (vthirst.value2 > 0)
                {
                ...
                if (vthirst.value2 <= 0)
                {
                    if (player.perkv1(IMutationsLib.VampiricBloodstreamIM) >= 4)
                    {
                        vthirst.value2 = 3;
                    }
                    else if (player.perkv1(IMutationsLib.VampiricBloodstreamIM) >= 2)
                    {
                        vthirst.value2 = 2;
                    }
                    else
            ...
            {
                var protection : Float = 0;
                if (flags[kFLAGS.PC_GOBLIN_DAUGHTERS] <= 25)
                {
                    protection = 25;
                }
                else if (flags[kFLAGS.PC_GOBLIN_DAUGHTERS] <= 50)
                {
                    protection = 50;
                }
                else if (flags[kFLAGS.PC_GOBLIN_DAUGHTERS] <= 75)
                {
                    protection = 75;
                }
                else
                {
                    protection += flags[kFLAGS.PC_GOBLIN_DAUGHTERS];
                }
                var nails : Float = 0;
                if (protection > rand(100))
                {
                    nails += 5 + rand(11);
                }
                if (protection > rand(100))
                ...
                var mechanism : Float = 0;
                if (protection > rand(100))
                {
                    mechanism += 1;
                }
                var energycore : Float = 0;
                if (protection > rand(100))
                {
                    energycore += 1;
                }
                if (player.hasStatusEffect(StatusEffects.PCDaughtersWorkshopSpareParts))
                {
                    if (nails > 0)
                    {
                        if (player.statusEffectv1(StatusEffects.PCDaughtersWorkshopSpareParts) + nails > 300)
                        {
                            nails = 300 - player.statusEffectv1(StatusEffects.PCDaughtersWorkshopSpareParts);
                        }
                        player.addStatusValue(StatusEffects.PCDaughtersWorkshopSpareParts, 1, nails);
                    }
                    if (metalpieces > 0)
                    {
                        if (player.statusEffectv2(StatusEffects.PCDaughtersWorkshopSpareParts) + metalpieces > 15)
                        {
                            metalpieces = 15 - player.statusEffectv2(StatusEffects.PCDaughtersWorkshopSpareParts);
                        }
                        player.addStatusValue(StatusEffects.PCDaughtersWorkshopSpareParts, 2, metalpieces);
                    }
                    if (mechanism > 0 && player.statusEffectv3(StatusEffects.PCDaughtersWorkshopSpareParts) + mechanism < 4)
                    {
                        player.addStatusValue(StatusEffects.PCDaughtersWorkshopSpareParts, 3, mechanism);
                    }
                    if (energycore > 0 && player.statusEffectv4(StatusEffects.PCDaughtersWorkshopSpareParts) + energycore < 4)
                    {
                        player.addStatusValue(StatusEffects.PCDaughtersWorkshopSpareParts, 4, energycore);
                    }
                }
                else
                {
        ...
        //Once per day{
            ...
            if (player.statusEffectv3(StatusEffects.MetRathazul) > 0 && !player.hasStatusEffect(StatusEffects.CampRathazul))
            {
            ...
            if (CoC.instance.model.time.days % 2 == 0 && flags[kFLAGS.KAIJU_BAD_END_COUNTER] > 0)
            {
                ...
                if (flags[kFLAGS.KAIJU_BAD_END_COUNTER] < 0)
                {
                    flags[kFLAGS.KAIJU_BAD_END_COUNTER] = 0;
                }
            }
            flags[kFLAGS.GILDED_JERKED] = 0;
            flags[kFLAGS.FED_SCYLLA_TODAY] = 0;
            flags[kFLAGS.NOT_HELPED_ARIAN_TODAY] = 0;
            if (flags[kFLAGS.RUBI_PROSTITUTION] > 0)
            {
                flags[kFLAGS.RUBI_PROFIT] += 2 + rand(4);
            }
            flags[kFLAGS.BENOIT_TALKED_TODAY] = 0;
            SceneLib.bazaar.benoit.updateBenoitInventory();
            flags[kFLAGS.ROGAR_FUCKED_TODAY] = 0;
            if (flags[kFLAGS.LUSTSTICK_RESISTANCE] > 0)
            {
                flags[kFLAGS.LUSTSTICK_RESISTANCE]--;
            ...
            //Dominika fellatrix countdown{
                ...
                if (flags[kFLAGS.DOMINIKA_MAGIC_COOLDOWN] < 0)
                {
                    flags[kFLAGS.DOMINIKA_MAGIC_COOLDOWN] = 0;
                }
            }
            ...
            //Loppe denial counter{
                ...
                if (flags[kFLAGS.LOPPE_DENIAL_COUNTER] < 0)
                {
                    flags[kFLAGS.LOPPE_DENIAL_COUNTER] = 0;
                }
            }
            ...
            //Countdown to next faerie orgy{
                ...
                if (flags[kFLAGS.WEEKLY_FAIRY_ORGY_COUNTDOWN] < 0)
                {
                    flags[kFLAGS.WEEKLY_FAIRY_ORGY_COUNTDOWN] = 0;
                }
            }
            if (CoC.instance.model.time.days % 7 == 0)
            {
            ...
            if (player.hasStatusEffect(StatusEffects.Contraceptives))
            ...
            // Herbal contraceptives countdown{
                ...
                if (player.statusEffectv1(StatusEffects.Contraceptives) == 1)
                {
                    ...
                    if (player.statusEffectv1(StatusEffects.Contraceptives) < 0)
                    {
                        player.removeStatusEffect(StatusEffects.Contraceptives);
                    }
                }
            }
            if (player.statusEffectv1(StatusEffects.SharkGirl) > 0)
            {
            ...
            {
                var _sw7_ = (flags[kFLAGS.INCREASED_HAIR_GROWTH_SERUM_TIMES_APPLIED]);                
            ...
            if (flags[kFLAGS.HAIR_GROWTH_STOPPED_BECAUSE_LIZARD] == 0)
            {
                ...
                if (player.beardLength > 0 && player.beardLength < 12)
                {
                    EventParser.growBeard(0.02);
                }
            }
            //Clear dragon breath cooldowns!
            if (player.hasStatusEffect(StatusEffects.DragonBreathCooldown) && !player.perkv1(IMutationsLib.DrakeLungsIM) >= 3)
            {
                player.removeStatusEffect(StatusEffects.DragonBreathCooldown);
            }
            if (player.hasStatusEffect(StatusEffects.DragonDarknessBreathCooldown) && !player.perkv1(IMutationsLib.DrakeLungsIM) >= 1)
            {
                player.removeStatusEffect(StatusEffects.DragonDarknessBreathCooldown);
            }
            if (player.hasStatusEffect(StatusEffects.DragonFireBreathCooldown) && !player.perkv1(IMutationsLib.DrakeLungsIM) >= 1)
            {
                player.removeStatusEffect(StatusEffects.DragonFireBreathCooldown);
            }
            if (player.hasStatusEffect(StatusEffects.DragonIceBreathCooldown) && !player.perkv1(IMutationsLib.DrakeLungsIM) >= 1)
            {
                player.removeStatusEffect(StatusEffects.DragonIceBreathCooldown);
            }
            if (player.hasStatusEffect(StatusEffects.DragonLightningBreathCooldown) && !player.perkv1(IMutationsLib.DrakeLungsIM) >= 1)
            {
                player.removeStatusEffect(StatusEffects.DragonLightningBreathCooldown);
            }
            if (player.hasStatusEffect(StatusEffects.DragonPoisonBreathCooldown) && !player.perkv1(IMutationsLib.DrakeLungsIM) >= 1)
            {
                player.removeStatusEffect(StatusEffects.DragonPoisonBreathCooldown);
            }
            if (player.hasStatusEffect(StatusEffects.DragonWaterBreathCooldown) && !player.perkv1(IMutationsLib.DrakeLungsIM) >= 1)
            {
                player.removeStatusEffect(StatusEffects.DragonWaterBreathCooldown);
            }
            if (player.hasStatusEffect(StatusEffects.DragonFaerieBreathCooldown) && !player.perkv1(IMutationsLib.DrakeLungsIM) >= 1)
            {
                player.removeStatusEffect(StatusEffects.DragonFaerieBreathCooldown);
            }
            if (player.hasStatusEffect(StatusEffects.DragonPoisonousSapBreathCooldown) && !player.perkv1(IMutationsLib.DrakeLungsIM) >= 1)
            {
                player.removeStatusEffect(StatusEffects.DragonPoisonousSapBreathCooldown);
            }
            if (player.hasStatusEffect(StatusEffects.DragonRoyalBreathCooldown) && !player.perkv1(IMutationsLib.DrakeLungsIM) >= 1)
            {
            ...
            if (flags[kFLAGS.ALRAUNE_GROWING] > 0 && flags[kFLAGS.ALRAUNE_GROWING] < 15)
            {
                flags[kFLAGS.ALRAUNE_GROWING]++;
            }
            ...
            if (player.isWoodElf())
            {
                if (SceneLib.woodElves.ConvertNewSisters())
                {
                    needNext = true;
                }
            }
            ...
            {
                if (flags[kFLAGS.SAND_WITCH_LOOT_TAKEN] == 1)
                {
                    flags[kFLAGS.SAND_WITCH_LOOT_TAKEN] = 0;
                }
                else
                {
                    flags[kFLAGS.SAND_WITCH_LOOT_TAKEN] -= 2;
                }
            }
            ...
            {
                if (player.statusEffectv1(StatusEffects.TGStorage) < 5)
                {
                    player.changeStatusValue(StatusEffects.TGStorage, 1, 5);
                }
                if (player.statusEffectv2(StatusEffects.TGStorage) < 5)
                {
                    player.changeStatusValue(StatusEffects.TGStorage, 2, 5);
                }
                if (player.statusEffectv3(StatusEffects.TGStorage) < 5)
                {
                    player.changeStatusValue(StatusEffects.TGStorage, 3, 5);
                }
                if (player.statusEffectv4(StatusEffects.TGStorage) < 1)
                {
                    player.changeStatusValue(StatusEffects.TGStorage, 4, 1);
                }
            }
            //Countdown to 5th floor boss rematch
            if (flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] > 12 && flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] < 13)
            {
                flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] -= 0.1;
            }
            //Clear sidewinder cooldown
            if (player.hasStatusEffect(StatusEffects.CooldownSideWinder))
            {
                player.removeStatusEffect(StatusEffects.CooldownSideWinder);
            }
            //Energy Dependent
            if (player.hasPerk(PerkLib.EnergyDependent))
            {
                var intBuff : Float = player.buff("Energy Vampire").getValueOfStatBuff("int.mult");
                var speBuff : Float = player.buff("Energy Vampire").getValueOfStatBuff("spe.mult");
                if (intBuff > -0.9)
                {
                    player.buff("Energy Vampire").addStats({
                                        int.mult : -0.05
            ...
            if (flags[kFLAGS.CAMP_UPGRADES_FISHERY] > 0 && camp.FisheryDailyProduction() > 0)
            {
                flags[kFLAGS.FISHES_STORED_AT_FISHERY] += camp.FisheryDailyProduction();
            }
            ...
            if (player.hasStatusEffect(StatusEffects.MitziDaughtersBarrels))
            {
                if (rand(3) == 0)
                {
                    player.addStatusValue(StatusEffects.MitziDaughtersBarrels, 1, 5);
                }
                else if (rand(2) == 0)
                {
                    player.addStatusValue(StatusEffects.MitziDaughtersBarrels, 2, 5);
                }
                else
                {
                    player.addStatusValue(StatusEffects.MitziDaughtersBarrels, 3, 5);
                }
            }
            ...
            if (player.hasPerk(PerkLib.Metamorph) && player.perkv1(PerkLib.Metamorph) < 18)
            {
                player.addPerkValue(PerkLib.Metamorph, 1, 1);
            }
            if (player.hasPerk(PerkLib.MetamorphEx) && player.perkv1(PerkLib.MetamorphEx) < 10)
            {
                player.addPerkValue(PerkLib.MetamorphEx, 1, 1);
            }
            ...
            if (flags[kFLAGS.CHI_CHI_LVL_UP] < 5 && flags[kFLAGS.CHI_CHI_DAILY_TRAINING] < 1)
            {
                ...
                {
                    flags[kFLAGS.CHI_CHI_LVL_UP] = 2;
                }
                else
                {
                    flags[kFLAGS.CHI_CHI_LVL_UP]++;
                }
            }
            ...
            if ((flags[kFLAGS.PLAYER_COMPANION_1] == "Tyrantia" || flags[kFLAGS.PLAYER_COMPANION_2] == "Tyrantia" || flags[kFLAGS.PLAYER_COMPANION_3] == "Tyrantia") && TyrantiaFollower.TyrantiaAffectionMeter < 100)
            {
                SceneLib.tyrantia.tyraniaAffection(2);
            }
            ...
            if (flags[kFLAGS.EXCELLIA_RECRUITED] > 2 && flags[kFLAGS.EXCELLIA_RECRUITED] < 30)
            {
                flags[kFLAGS.EXCELLIA_RECRUITED]++;
            }
            //Alvina timer
            if (SceneLib.dungeons.checkFactoryClear() && flags[kFLAGS.ALVINA_FOLLOWER] < 8)
            {
                flags[kFLAGS.ALVINA_FOLLOWER]++;
            }
            //Siegweird
            if (player.statusEffectv1(StatusEffects.SiegweirdTraining2) == 1 && flags[kFLAGS.SIEGWEIRD_FOLLOWER] < 10)
            {
                flags[kFLAGS.SIEGWEIRD_FOLLOWER]++;
            }
            if (player.statusEffectv1(StatusEffects.SiegweirdSoup) == 1)
            {
            ...
            {
                player.removeStatusEffect(StatusEffects.CampLunaMishaps1);
            }
            if (player.hasStatusEffect(StatusEffects.CampLunaMishaps2))
            {
                player.removeStatusEffect(StatusEffects.CampLunaMishaps2);
            }
            if (player.hasStatusEffect(StatusEffects.CampLunaMishaps3))
            {
                player.removeStatusEffect(StatusEffects.CampLunaMishaps3);
            }
            //Full moon
            ...
            {
                var ngMult : Float = (player.newGamePlusMod() + 1);
                var changeV : Float = 0;
                var textA : String = "";
                ...
                var _sw8_ = (flags[kFLAGS.LUNA_MOON_CYCLE]);                
                ...
                {
                    player.statStore.replaceBuffObject({
                                str.mult : changeV * 0.1 * ngMult,
                                tou.mult : changeV * 0.06 * ngMult,
                                spe.mult : changeV * 0.04 * ngMult,
                                minlustx : changeV * 0.01
                            }, "Lycanthropy", {
                                text : "Lycanthropy"
                            });
                ...
                {
                    player.statStore.replaceBuffObject({
                                str.mult : changeV * 0.01 * ngMult,
                                tou.mult : changeV * 0.1 * ngMult,
                                spe.mult : changeV * 0.04 * ngMult,
                                minlustx : changeV * 0.005
                            }, "Vulpesthropy", {
                                text : "Vulpesthropy"
                            });
                ...
                if (player.hasPerk(PerkLib.Selachimorphanthropy))
                {
                    player.statStore.replaceBuffObject({
                                str.mult : changeV * 0.1 * ngMult,
                                tou.mult : changeV * 0.05 * ngMult,
                                spe.mult : changeV * 0.05 * ngMult,
                                minlustx : changeV * 0.01
                            }, "Selachimorphanthropy", {
                                text : "Selachimorphanthropy"
                            });
                    player.setPerkValue(PerkLib.Selachimorphanthropy, 1, changeV);
                }
                needNext = true;
            }
            if (player.hasStatusEffect(StatusEffects.MitziIzmaDaughters))
            {
                player.removeStatusEffect(StatusEffects.MitziIzmaDaughters);
            }
            if (flags[kFLAGS.SAMIRAH_HYPNOSIS] == 4 || flags[kFLAGS.SAMIRAH_HYPNOSIS] == 2)
            {
            ...
            if (player.hasStatusEffect(StatusEffects.AdventureGuildQuests1))
            {
                ...
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuests1, 1, -3);
                }
                if (player.statusEffectv2(StatusEffects.AdventureGuildQuests1) > 6)
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuests1, 2, -3);
                }
                if (player.statusEffectv3(StatusEffects.AdventureGuildQuests1) > 6)
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuests1, 3, -3);
                }
            }
            if (player.hasStatusEffect(StatusEffects.AdventureGuildQuests2))
            {
                ...
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuests2, 1, -3);
                }
                if (player.statusEffectv2(StatusEffects.AdventureGuildQuests2) > 6)
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuests2, 2, -3);
                }
            }
            if (player.hasStatusEffect(StatusEffects.AdventureGuildQuests3))
            {
                if (player.statusEffectv1(StatusEffects.AdventureGuildQuests3) > 6)
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuests3, 1, -3);
                }
                if (player.statusEffectv2(StatusEffects.AdventureGuildQuests3) > 6)
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuests3, 2, -3);
                }
            }
            if (player.hasStatusEffect(StatusEffects.AdventureGuildQuests4))
            {
                ...
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuests4, 1, -3);
                }
                if (player.statusEffectv2(StatusEffects.AdventureGuildQuests4) > 4)
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuests4, 2, -3);
                }
            }
            if (flags[kFLAGS.AURORA_LVL] > 0.3 && flags[kFLAGS.AURORA_LVL] < 0.7)
            {
                flags[kFLAGS.AURORA_LVL] += 0.05;
            }
            //Zenji training conuters
            if (player.hasStatusEffect(StatusEffects.ZenjiTrainingsCounters1))
            {
                if (player.statusEffectv1(StatusEffects.ZenjiTrainingsCounters1) > 0)
                {
                    player.addStatusValue(StatusEffects.ZenjiTrainingsCounters1, 1, -player.statusEffectv1(StatusEffects.ZenjiTrainingsCounters1));
                }
                if (player.statusEffectv2(StatusEffects.ZenjiTrainingsCounters1) > 0)
                {
                    player.addStatusValue(StatusEffects.ZenjiTrainingsCounters1, 2, -player.statusEffectv2(StatusEffects.ZenjiTrainingsCounters1));
                }
                if (player.statusEffectv3(StatusEffects.ZenjiTrainingsCounters1) > 0)
                {
                    player.addStatusValue(StatusEffects.ZenjiTrainingsCounters1, 3, -player.statusEffectv3(StatusEffects.ZenjiTrainingsCounters1));
                }
            }
            if (player.hasStatusEffect(StatusEffects.ZenjiTrainingsCounters2))
            {
                if (player.statusEffectv1(StatusEffects.ZenjiTrainingsCounters2) > 0)
                {
                    player.addStatusValue(StatusEffects.ZenjiTrainingsCounters2, 1, -player.statusEffectv1(StatusEffects.ZenjiTrainingsCounters2));
                }
                if (player.statusEffectv2(StatusEffects.ZenjiTrainingsCounters2) > 0)
                {
                    player.addStatusValue(StatusEffects.ZenjiTrainingsCounters2, 2, -player.statusEffectv2(StatusEffects.ZenjiTrainingsCounters2));
                }
            }
            if (ZenjiScenes.isLover())
            {
                if (player.statusEffectv2(StatusEffects.ZenjiPreparationsList) < 20)
                {
                    player.addStatusValue(StatusEffects.ZenjiPreparationsList, 2, 1);
                }
                if (player.statusEffectv1(StatusEffects.ZenjiModificationsList) > 0)
                {
                    player.addStatusValue(StatusEffects.ZenjiModificationsList, 1, -1);
                }
                if (player.statusEffectv3(StatusEffects.ZenjiZList) > 0)
                {
                    var deduce : Float = player.statusEffectv3(StatusEffects.ZenjiZList);
                    player.addStatusValue(StatusEffects.ZenjiZList, 3, -deduce);
                }
                ZenjiScenes.ZenjiFood = false;
            }
            //Neisa counter to payment
            if (flags[kFLAGS.NEISA_FOLLOWER] >= 7 && flags[kFLAGS.NEISA_FOLLOWER] < 17)
            {
                flags[kFLAGS.NEISA_FOLLOWER]++;
            }
            //Reset of event for Coronation quest at Bazzar / Bathing timer in camp
            if (flags[kFLAGS.THE_TRENCH_ENTERED] == 13.5)
            {
                flags[kFLAGS.THE_TRENCH_ENTERED] = 13;
            }
            if (flags[kFLAGS.THE_TRENCH_ENTERED] > 14)
            {
                flags[kFLAGS.GRAYDA_BATHING]++;
            }
            if (flags[kFLAGS.THE_TRENCH_ENTERED] > 15)
            {
            ...
            if (player.hasPerk(PerkLib.EasterBunnyBalls) && player.balls >= 2)
            {
                ...
                {
                    var changeLib : Float = (player.ballSize * 5 / 100) + 1;  //Exemple (1*5/100)+1= 1.05 wich is the modifier to libido  
            ...
            if (player.hasStatusEffect(StatusEffects.GraydaRandomnCampEvents))
            {
                player.removeStatusEffect(StatusEffects.GraydaRandomnCampEvents);
            }
            if (player.hasStatusEffect(StatusEffects.ZenjiArian))
            {
                if (player.statusEffectv1(StatusEffects.ZenjiArian) <= 0)
                {
                    player.removeStatusEffect(StatusEffects.ZenjiArian);
                }
                else
                {
                    player.addStatusValue(StatusEffects.ZenjiArian, 1, -1);
                }
            }
            //Reset clone creation tracker
            if (camp.gcc(true) && camp.gcc() == 0)
            {
            ...
            if (player.weaponRange == weaponsrange.SAGITTB)
            {
                ...
                if (player.statusEffectv1(StatusEffects.TookSagittariusBanefulGreatBow) > 0 && player.cor < 70)
                {
                    player.cor += 2;
                }
                if (!player.statStore.hasBuff("Sagittarius Focus"))
                {
                    if (player.statusEffectv1(StatusEffects.TookSagittariusBanefulGreatBow) < 10)
                    {
                        player.addStatusValue(StatusEffects.TookSagittariusBanefulGreatBow, 1, 1);
                    }
                    player.buff("Sagittarius Curse").remove();
                    var curseInt : Float = 0.1 * player.statusEffectv1(StatusEffects.TookSagittariusBanefulGreatBow);
                    if (curseInt <= 0)
                    {
                        curseInt = 0.01;
                    }
                    curseInt *= player.inte;
                    player.buff("Sagittarius Curse").addStats({
                                int : -curseInt,
                                sens : Math.round(Math.sqrt(curseInt))
                            });
                }
                else
                {
            ...
            if (player.hasStatusEffect(StatusEffects.MeetXuviel) && player.statusEffectv2(StatusEffects.MeetXuviel) > 0)
            {
                player.addStatusValue(StatusEffects.MeetXuviel, 2, -1);
            }
            if (player.hasStatusEffect(StatusEffects.MeetXuviel2) && player.statusEffectv1(StatusEffects.MeetXuviel2) > 0)
            {
                player.addStatusValue(StatusEffects.MeetXuviel2, 1, -1);
            }
            if (player.hasStatusEffect(StatusEffects.FontOfCorruption) && player.statusEffectv1(StatusEffects.FontOfCorruption) > 0)
            {
                if (player.statusEffectv1(StatusEffects.FontOfCorruption) <= 0)
                {
                    player.removeStatusEffect(StatusEffects.FontOfCorruption);
                }
                else
                {
                    player.addStatusValue(StatusEffects.FontOfCorruption, 1, -1);
                }
            }
        }
    ...
    private function hourlyCheckEvents() : Bool
    {
        var needNext : Bool = false;
        //Soul Sense
        ...
        //
        for (clone/* AS3HX WARNING could not determine type for var: clone exp: EField(EIdent(Soulforce),clones) type: null */ in Soulforce.clones)
        {
            if (player.hasStatusEffect(clone))
            {
                ...
                {
                    for (i in 0...Soulforce.daos.length)
                    {
        ...
        if (player.hasPerk(PerkLib.ElementalBloodline) && player.hasPerk(PerkLib.ElementalBody))
        {
            ...
            if (dao != null && player.statusEffectv2(dao[1]) < SceneLib.soulforce.highestLayerOfDaoComprehension())
            {
        ...
        if (InCollection("Kiha", flags[kFLAGS.PLAYER_COMPANION_0], flags[kFLAGS.PLAYER_COMPANION_2], flags[kFLAGS.PLAYER_COMPANION_3]))
        {
            ...
            if (flags[kFLAGS.PLAYER_COMPANION_0] == "Kiha")
            {
        ...
        if ((player.hasPerk(PerkLib.MummyLord) && player.perkv2(PerkLib.MummyLord) > 0) || (player.hasPerk(PerkLib.MummyLordDormant) && player.perkv2(PerkLib.MummyLordDormant) > 0))
        {
            ...
            {
                player.addPerkValue(PerkLib.MummyLord, 2, -1);
            }
            else
            {
                player.addPerkValue(PerkLib.MummyLordDormant, 2, -1);
            }
        }
        //Heaven Tribulations
        if (player.hasStatusEffect(StatusEffects.TribulationCountdown))
        {
            ...
            {
                player.addStatusValue(StatusEffects.TribulationCountdown, 1, -1);
            }
        }
        ...
        else if (flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] >= 2 && CoC.instance.model.time.hours % 13 == 0 && flags[kFLAGS.MINOCUM_WARNING_COOLDOWN] == 0)
        ...
        //Getting chrismas tree{
            ...
            {
                if (player.statusEffectv2(StatusEffects.ZenjiZList) == 0)
                {
                    SceneLib.zenjiScene.loverZenjiSleepWithMorning();
                }
                if (player.statusEffectv2(StatusEffects.ZenjiZList) == 1)
                {
                    SceneLib.zenjiScene.loverZenjiSleepWithGetPenetratedMorning();
                }
                if (player.statusEffectv2(StatusEffects.ZenjiZList) == 2)
                {
                    SceneLib.zenjiScene.loverZenjiSleepWithCatchAnalMorning();
                }
                if (player.statusEffectv2(StatusEffects.ZenjiZList) == 3)
                {
                    SceneLib.zenjiScene.loverZenjiSleepWithNoSexMorning();
                }
                needNext = true;
            }
    ...
    private function hourlyCheckRacialPerks() : Bool
    {
        var needNext : Bool = false;
        // RaceTf itemeffect
        ...
        //Easter bunny cumming its eggs out{
            ...
            player.ballSize = 1;
            var changeLib1 : Float = (player.ballSize * 5 / 100) + 1;  //Exemple (1*5/100)+1= 1.05 wich is the modifier to libido  
        ...
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.HydraRegeneration, player.lowerBody == LowerBody.HYDRA, "", "You accidentally cut yourself but to your stupor the wound does not close as fast as it should. Guess you are no longer a hydra enough to benefit from superior regeneration.");
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.HydraAcidBreath, player.lowerBody == LowerBody.HYDRA, "", "As your lead hydra head vanishes so do your ability to belch acid.");
        //Troll Regeneration
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.TrollRegeneration, player.isAnyRaceCached(Races.TROLL, Races.GLACIAL_TROLL), "", "You accidentally cut yourself but to your stupor the wound does not close as fast as it should. I appears your lack of troll blood is no longer enough to benefit from superior regeneration.");
        ...
        //Argument 1 is weither pc is in heat stage or not 1 means active{
            ...
            if (player.isRaceCached(Races.SALAMANDER))
            {
                outputText(" salamander");
            }
            else if (player.isRaceCached(Races.KITSHOO))
            {
                outputText(" kitsumori");
            }
            else if (player.isRaceCached(Races.HELLCAT))
            {
                outputText(" hellcat");
            }
            else
        ...
        if (player.hasPerk(PerkLib.DarkSlimeCore))
        
        //Lose DARK slime core perk{
        ...
        if (player.hasKeyItem("Ruby Orb") >= 0)
        
        //Regain DARK slime core{
        ...
        //Slime craving stuff{
            ...
            else
            {
                ...
                {
                    player.addStatusValue(StatusEffects.SlimeCraving, 1, 1);
                }
                if (player.statusEffectv1(StatusEffects.SlimeCraving) >= delay)
                {
                    ...
                    if (player.spe > 1)
                    {
                        player.addStatusValue(StatusEffects.SlimeCraving, 3, 0.1);
                    ...
                    player.addStatusValue(StatusEffects.SlimeCraving, 2, 0.1);
                }
            }
        }
        if (player.hasStatusEffect(StatusEffects.SlimeCravingFeed))
        ...
        //Hydra heads
        if (player.lowerBody != LowerBody.HYDRA && player.hasStatusEffect(StatusEffects.HydraTailsPlayer))
        {
            player.removeStatusEffect(StatusEffects.HydraTailsPlayer);
        }
        //Greed Perk
        ...
        if (!player.isRaceCached(Races.YUKIONNA) && player.hasPerk(PerkLib.IcyFlesh))
        {
            ...
            if (player.hasPerk(PerkLib.ColdAffinity) && !player.isRaceCached(Races.YETI))
            {
                player.removePerk(PerkLib.ColdAffinity);
            }
        ...
        //Blood Affinity
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.BloodAffinity, player.isAnyRaceCached(Races.VAMPIRE, Races.WERESPIDER, Races.DRACULA), "Your ability to hone the power of the red, flowing essence heightens. Hemophilia is a delightful chance for you to revel in the hemorrhage of freshly spilt blood.", "The sight of blood no longer fuels you with the same rousing sensations you once had. Any affinity you carried for the crimson fluid is now lost.");
        //Dark Affinity
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.DarknessAffinity, player.isAnyRaceCached(Races.VAMPIRE, Races.APOPHIS, Races.ANUBIS, Races.DRACULA) || player.hasPerk(PerkLib.Nightshade), "You are at home while shrouded in darkness, seeing clearly within the shadows as if it were broad daylight.", "The inky black darkness grows hostile toward you once more as your affinity for it is lost.");
        //Compelling Aria
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.HarpySong, player.isAnyRaceCached(Races.SIREN, Races.HARPY, Races.PHOENIX, Races.THUNDERBIRD) || player.hasMutation(IMutationsLib.HarpyHollowBonesIM), "Your voice sound like magicaly entrancing music to your ears now, it would seem you have gained the infamous magicaly compeling voices common to harpies.", "Your voice no longer carries the magical power it used to and thus you are no longer able to use your compelling aria.");
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.MelkieSong, player.tongue.type == Tongue.MELKIE || player.hasMutation(IMutationsLib.MelkieLungIM), "Your words are notes, your sentence a melody. Your voice is like music to your ears and you realise it is because your body became closer to that of a Melkie adapting even your tongue and voice. Well you could always go sit on a rock and sing in case some sailor came by.", "Your voice no longer carries the magical power it used to and thus you are no longer able to use your compelling aria.")  //Alraune perks  ;
        
        
        
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.AlrauneNectar, player.isAlraune(), "Your flower now produces a delicious nectar the scent of wich is sure to draw in partners.", "As you no longer have a pitcher to produce nectar your scent has became more like that of an ordinary human.", function() : Void
                        {
        ...
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.PlantKnowledge, player.isAlraune(), "It would seem you aquired additionnal skills in herbalism thanks to behing a plant yourself.", "No longer a plant you lost some of your innate floral knowledge.");
        //Kamaitachi perks
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.CursedWound, player.arms.type == Arms.KAMAITACHI, "Your kamaitachi blades shines with a sharp, deadly glow. Those will clearly leaves deadly wounds.", "Lacking a pair of kamaitachi scythe you can no longer inflict your trademark cursed kamaitachi wounds.");
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.NaturalHerbalism, player.isRaceCached(Races.KAMAITACHI), "Great knowledges flows through your mind as you become more Kamaitachi like. It dawns on you that you have aquired a natural affinity for medicine and herbalism, something your species is famous for, heck you can identify every single plant near your camp by name and species now.", "No longer a Kamaitachi, you seem to have lost your knack for herbs and medicines.");
        //Aquatic Affinity
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.AquaticAffinity, (InCollection(player.lowerBody, LowerBody.ORCA, LowerBody.SEA_DRAGON) && InCollection(player.arms.type, Arms.ORCA, Arms.SEA_DRAGON) && player.tailType == Tail.ORCA && InCollection(player.ears.type, Ears.ORCA, Ears.ORCA)) || player.isScylla() || player.isKraken() || InCollection(player.tailType, Tail.ARIGEAN_GREEN, Tail.ARIGEAN_RED, Tail.ARIGEAN_YELLOW, Tail.ARIGEAN_PRINCESS), 
        ...
        //Salamander bullshit
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.LustingWarrior, player.isRaceCached(Races.SALAMANDER, 2) && player.hasStatusEffect(StatusEffects.Overheat), 
        ...
        if (player.hasStatusEffect(StatusEffects.PostfluidIntakeRegeneration))
        {
            player.removeStatusEffect(StatusEffects.PostfluidIntakeRegeneration);
        }
        /*
        ...
        //}
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.BigHandAndFeet, player.lowerBody == LowerBody.YETI && player.arms.type == Arms.YETI, "Geeze, with those oversized hands and feet you should just drop your weapon and punch things out cold. Why do you even need a weapon for anyway?", "Your limbs are now back to their normal size, it might be smart to use a weapon over your bare hands and feet for fighting.");
        //Gazer perks
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.TrueSeeing, player.eyes.type == Eyes.MONOEYE || player.hasMutation(IMutationsLib.GazerEyesIM), "As part of acquiring an all seeing eye your sight has improved greatly. It would seem you now possess the innate ability to dismiss illusions and falsehood, seeing the world as it truly is.", "Your sight is not what it used to be. No longer having a cyclopean vision you have lost the power of true sight.");
        ...
        {
            if (player.hasStatusEffect(StatusEffects.GazerEyeStalksPlayer))
            {
                player.removeStatusEffect(StatusEffects.GazerEyeStalksPlayer);
            }
        ...
        if (player.isRaceCached(Races.DRAGONNE))
        {
            if (!player.hasPerk(PerkLib.LionHeart))
            {
                ...
                {
                    player.createPerk(PerkLib.DragonRegalBreath, 0, 0, 0, 0);
                }
                player.createPerk(PerkLib.LionHeart, 0, 0, 0, 0);
                needNext = true;
            }
        }
        else if (player.hasPerk(PerkLib.LionHeart))
        {
        ...
        //Equine Strength
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.EquineStrength, player.isAnyRaceCached(Races.HORSE, Races.CENTAUR, Races.LAQUINE), "You are enough of a horse-morph or centaur and have gained your special abilities!", "You are no longer enough of a horse-morph or centaur and have lost your special abilities!");
        //Laquine Might
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.LaquineMight, player.isRaceCached(Races.LAQUINE), "Woa maybe you’ve eaten too many raw carrots you feel your body ripple with power. It might also be you turning into a full Laquine doing this.", "As your body becomes less Laquine like you feel your energy is down both in libido and strength.");
        //Titan Might
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.TitanicSize, player.tallness >= 80 && (player.isRaceCached(Races.SCYLLA, 2) || player.isAnyRaceCached(Races.HYDRA, Races.FROSTWYRM, Races.SANDWORM)), "Whoa, you've grown so big its a sheer miracle you don't damage the landscape while moving. That said, your size now contributes to your strength as well.", 
        ...
        else if (player.hasPerk(PerkLib.GiantMight))
        {
        ...
        if (player.vehiclesName == "Goblin Mech Alpha" && (player.isAnyRaceCached(Races.ELF, Races.WOODELF) || player.tallness > 48 || player.tailType != Tail.NONE || player.hasPhysicalWings()))
        
        //Elf OR Taller than 4 ft or having wings/tail{
            
            if (player.isAnyRaceCached(Races.ELF, Races.WOODELF))
            {
            ...
            if (player.hasKeyItem("Upgraded Armor plating 2.0") >= 0)
            {
                player.HP /= 1.35;
            }
            if (player.hasKeyItem("Upgraded Armor plating 3.0") >= 0)
            {
                player.HP /= 1.5;
            }
            if (player.hasKeyItem("Upgraded Armor plating 4.0") >= 0)
            {
                player.HP /= 1.7;
            }
            if (player.hasKeyItem("Upgraded Armor plating 5.0") >= 0)
            {
                player.HP /= 1.9;
            }
            if (player.hasKeyItem("Upgraded Armor plating 6.0") >= 0)
            {
                player.HP /= 2.1;
            }
        ...
        if (player.vehiclesName == "Goblin Mech Prime" && (player.isAnyRaceCached(Races.ELF, Races.WOODELF) || player.tallness > 48 || player.tailType != Tail.NONE || player.hasPhysicalWings()))
        ...
        if (player.vehiclesName == "Giant Slayer Mech" && (player.isAnyRaceCached(Races.ELF, Races.WOODELF) || player.tallness > 66 || player.hasPhysicalWings()))
        
        //Elf OR Taller than 5'5" ft or having large wings{
            ...
            if (player.hasKeyItem("Upgraded Armor plating 1.0") >= 0 || player.hasKeyItem("Upgraded Leather Insulation 1.0") >= 0)
            {
                ...
                if (player.hasKeyItem("Upgraded Leather Insulation 1.0") >= 0)
                {
                    RHP += 0.25;
                }
                if (player.hasKeyItem("Upgraded Leather Insulation 2.0") >= 0)
                {
                    RHP += 0.5;
                }
                if (player.hasKeyItem("Upgraded Leather Insulation 3.0") >= 0)
                {
                    RHP += 0.75;
                }
                player.HP /= RHP;
        ...
        if (player.vehiclesName == "Howling Banshee Mech" && player.tallness < 84)
        {
            ...
            if (player.hasKeyItem("Upgraded Armor plating 1.0") >= 0)
            {
                var oldMax : Float = player.maxOverHP();
        ...
        //ABOBA
        needNext = (needNext) ? needNext : player.gainPerk(PerkLib.BasiliskResistance, flags[kFLAGS.BASILISK_RESISTANCE_TRACKER] >= 100, ((player.perkv1(IMutationsLib.GorgonEyesIM) >= 1)) ? "You notice that you feel a bit stiff and your skin is a bit harder.  Something clicks in your mind as you finally unlock the potential to protect yourself from the goddamn basilisks! " : "You notice that you feel a bit stiff and your skin is a bit harder.  Something clicks in your mind as you finally unlock the potential to protect yourself from the goddamn basilisks!");
        needNext = (needNext) ? needNext : player.losePerk(PerkLib.EnlightenedNinetails, player.perkv4(PerkLib.EnlightenedNinetails) == 0 && (player.tailType != Tail.FOX || player.tailCount < 9), "Without your tails, the magic power they once granted withers and dies, vanishing completely.");
        needNext = (needNext) ? needNext : player.losePerk(PerkLib.CorruptedNinetails, player.perkv4(PerkLib.CorruptedNinetails) == 0 && (player.tailType != Tail.FOX || player.tailCount < 9), "Without your tails, the magic power they once granted withers and dies, vanishing completely.");
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.NinetailsKitsuneOfBalance, player.hasPerk(PerkLib.EnlightenedNinetails) && player.perkv4(PerkLib.EnlightenedNinetails) > 0 && player.hasPerk(PerkLib.CorruptedNinetails) && player.perkv4(PerkLib.CorruptedNinetails) > 0 && player.tailType == Tail.FOX && player.tailCount == 9, 
        ...
        needNext = (needNext) ? needNext : player.losePerk(PerkLib.Diapause, !player.hasVagina(), "With the loss of your womb, you lose your kangaroo-like diapause ability.");
        //Small Frame perk
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.SmallFrame, player.isAnyRaceCached(Races.IMP, Races.MOUSE, Races.DEVIL, Races.AZAZEL) && player.effectiveTallness <= 60, "Due to your small build you have become harder to hit.", "Due to being too tall you no longer qualify.");
        //Small Caster perk
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.SmallCaster, player.isAnyRaceCached(Races.IMP, Races.DEVIL, Races.AZAZEL) && player.effectiveTallness <= 60, "Your magic becomes more concentrated in your smaller body.", "Your racial alignment does not meet the conditions and as such, the high concentration of mana in your body disperses.");
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.Immortality, player.isRaceCached(Races.AZAZEL), "You gain a sense of invulnerability as you are now an Azazel.", "You lose your sense of invulnerability as you are no longer an Azazel.");
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.Purifier, player.isRaceCached(Races.AZAZEL), "You gain a sense of conviction as you are now an Azazel.", "You lose your sense of conviction as you are no longer an Azazel.");
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.QueenOfTheFairies, player.isRaceCached(Races.FAIRY, 2), "You sense a change in yourself, a faint calling from the forrest. " + PerkLib.QueenOfTheFairies.desc());
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.WhatIsReality, player.isRaceCached(Races.CHESHIRE), "You gain a sence of invulnerability as you are now an Cheshire.", "You lose your sense of invulnerability as you are no longer an Cheshire.");
        needNext = (needNext) ? needNext : player.gainOrLosePerk(PerkLib.VorpalClaw, player.isRaceCached(Races.CHESHIRE));
        //Gain venomancy if utherly toxic
        needNext = (needNext) ? needNext : player.gainPerk(PerkLib.Venomancy, player.isRaceCached(Races.APOPHIS) && player.cor > 50, "Your inborn toxicity has reached such a peak that even your spells are now charged with venom.");
        needNext = (needNext) ? needNext : player.losePerk(PerkLib.Venomancy, !player.isRaceCached(Races.APOPHIS) && player.cor < 89, "Your mystical powers over poison and toxins have waned.");
        ...
        if (player.isPregnant() && (player.isHarpy() || player.isGoblinoid() || player.isAlraune() || player.isSandWorm() || (player.perkv1(IMutationsLib.FiendishOvariesIM) >= 4 && (player.pregnancyType == PregnancyStore.PREGNANCY_IMP || player.pregnancy2Type == PregnancyStore.PREGNANCY_IMP))) && !player.statStore.hasBuff("Pregomania"))
        {
            ...
            player.statStore.addBuffObject({
                        ...
                        text : "Your motherly instincs gives you increased resiliance and resolve."
                    });
        ...
        //Wendigo stuff
        if (!player.blockingBodyTransformations() && player.hasStatusEffect(StatusEffects.WendigoPsychosis) && !player.hasPerk(PerkLib.EndlessHunger) && ((flags[kFLAGS.HUNGER_ENABLED] > 0 && player.hunger <= 0) || (flags[kFLAGS.HUNGER_ENABLED] <= 0 && player.lust >= player.maxOverLust())))
        {
        ...
        if (player.cor >= 50 && player.weaponRange == weaponsrange.SAGITTB && !player.hasPerk(PerkLib.SagittariusAuraOfDominance))
        {
            ...
            if (player.statusEffectv3(StatusEffects.Kelt) >= 5)
            {
                outputText(" You recall that centaur Kelt had something similar going on about him.");
            }
        ...
        {
            var hellcatparts : Float = 3;
            if (player.tailType != Tail.BURNING && player.tailType != Tail.TWINKASHA)
            {
                hellcatparts -= 1;
            }
            ...
            if (player.eyes.type != Eyes.INFERNAL)
            {
                hellcatparts -= 1;
            }
            if (hellcatparts < 3)
            {
                ...
                if (flags[kFLAGS.WITCHES_SABBATH] > 1)
                {
                    flags[kFLAGS.WITCHES_SABBATH] = 1;
                }
                needNext = true;
            }
        }
        ...
        if (player.hasPerk(PerkLib.AvatorOfCorruption) && player.cor > 10 && player.horns.type != Horns.BICORN)
        {
            ...
            if (!player.perkv1(IMutationsLib.EclipticMindIM) >= 1)
            {
        ...
        //Turn to unicorn
        var unicornColorPalette : Array<Dynamic> = ["white", "pure white"];
        var unicornHairPalette : Array<Dynamic> = ["platinum blonde", "silver", "white", "pure white"];
        if ((player.horns.type == Horns.BICORN || player.horns.type == Horns.UNICORN) && player.cor < 11 && !player.hasPerk(PerkLib.AvatorOfPurity))
        {
            ...
            if (player.hasPerk(PerkLib.AuraOfCorruption))
            {
                player.removePerk(PerkLib.AuraOfCorruption);
            }
            player.cor = 0;
        ...
        //All the hourly bee cock checks except the 'seek out the bee girl' check. That's in timeChangeLarge{
            ...
            if (player.cocks[0].cockLength < 25 || player.cocks[0].cockThickness < 4)
            {
                ...
                {
                    outputText((player.cocks[0].cockLength < 25) ? "longer again" : "wider again");
                }
            ...
            //Reduced to 5 with 'Well Adjusted' perk.
            else
            {
                dynStats("lust", 10);
            }  //Always gain 10 lust each hour  
            needNext = true;
        }
        //tail absorb section
        ...
        if (player.hasStatusEffect(StatusEffects.Uniball) && player.ballSize > 1 && player.hasBalls())
        ...
        //Harpy
        if (LowerBody.canLayEggs(player) && player.hasPerk(PerkLib.HarpyWomb))
        ...
        if (player.hasPerk(PerkLib.SpiderOvipositor) || (player.hasPerk(PerkLib.BeeOvipositor) && !player.hasPerk(PerkLib.TransformationImmunityBeeHandmaiden)) || player.hasPerk(PerkLib.MantisOvipositor) || player.hasPerk(PerkLib.AntOvipositor))
        
        //Spider, Bee and, Mantis and Ant ovipositor updates{
            
            if (transformations.RemoveOvipositor.isPossible())
            ...
            else
            {
                ...
                //Stage 1 egg message{
                    ...
                    else
                    {
                        ...
                        if (!player.hasVagina())
                        {
                            outputText("  Wait, how would you even go about that?");
                        }
                    }
                    outputText("  <b>You have enough eggs to lay!</b>\n");
                    needNext = true;
                }
                else if (prevEggs < 20 && player.eggs() >= 20)
                ...
                //Stage 2 egg message{
                    ...
                    if (player.hasPerk(PerkLib.SpiderOvipositor))
                    {
                        ...
                        if (!player.hasVagina())
                        {
                            outputText("  Wait, that's not right...");
                        }
                    }
                    else
                    {
                ...
                else if (prevEggs < 40 && player.eggs() >= 40)
        ...
        if (player.hasPerk(PerkLib.Oviposition) || player.hasPerk(PerkLib.BunnyEggs))
        
        //Oviposition perk for lizard and bunny folks{
            
            if ((player.racialScore(Races.NAGA) + player.racialScore(Races.LIZARD) < 3) && player.hasPerk(PerkLib.Oviposition) && !player.hasPerk(PerkLib.BasiliskWomb))
            ...
            else if (player.racialScore(Races.BUNNY) < 3 && player.hasPerk(PerkLib.BunnyEggs))
            ...
            else if (!player.isPregnant() && player.hasVagina() && CoC.instance.model.time.hours == 1)
            
            //Otherwise pregger check, once every morning{
                
                if ((player.totalFertility() > 50 && CoC.instance.model.time.days % 15 == 0) || CoC.instance.model.time.days % 30 == 0)
        ...
        if (player.isRaceCached(Races.WEREWOLF) && player.hasPerk(PerkLib.LycanthropyDormant))
        {
            ...
            var ngMWW : Float = (player.newGamePlusMod() + 1);
            var bonusStats1 : Float = 0;
            ...
            player.statStore.replaceBuffObject({
                        str.mult : bonusStats1 * 0.1 * ngMWW,
                        tou.mult : bonusStats1 * 0.06 * ngMWW,
                        spe.mult : bonusStats1 * 0.04 * ngMWW,
                        minlustx : bonusStats1 * 0.01
                    }, "Lycanthropy", {
                        text : "Lycanthropy"
                    });
        ...
        if (player.isRaceCached(Races.WERESHARK) && player.hasPerk(PerkLib.SelachimorphanthropyDormant))
        {
            ...
            var ngMWS : Float = (player.newGamePlusMod() + 1);
            var bonusStats2 : Float = 0;
            if (flags[kFLAGS.LUNA_MOON_CYCLE] == 3 || flags[kFLAGS.LUNA_MOON_CYCLE] == 5)
            {
                bonusStats2 += 10;
            }
            ...
            player.createStatusEffect(StatusEffects.HumanForm, 1, 0, 0, 0);
            player.statStore.replaceBuffObject({
                        str.mult : bonusStats2 * 0.1 * ngMWS,
                        tou.mult : bonusStats2 * 0.05 * ngMWS,
                        spe.mult : bonusStats2 * 0.05 * ngMWS,
                        minlustx : bonusStats2 * 0.01
                    }, "Selachimorphanthropy", {
                        text : "Selachimorphanthropy"
                    });
        ...
        if (player.hasPerk(PerkLib.BimboBody) || player.hasStatusEffect(StatusEffects.BimboChampagne))
        ...
        //Bro checks{
            ...
            //(Dick rebiggening){
                ...
                if (player.hasPerk(PerkLib.BroBrains))
                {
                    outputText("Goddamn, that thing is almost as tough as you!  ");
                }
        ...
        if ((player.shieldName == "Tome of Imp" || player.shieldName == "cursed Tome of Imp") && !player.blockingBodyTransformations())
        {
            ...
            else if (player.statusEffectv1(StatusEffects.SlowTFingItems1) <= 0)
            {
                player.removeStatusEffect(StatusEffects.SlowTFingItems1);
            }
            else
            {
                player.addStatusValue(StatusEffects.SlowTFingItems1, 1, -1);
            }
        }
        if (player.weaponRange == weaponsrange.SAGITTB && !player.blockingBodyTransformations())
        {
            ...
            else if (player.statusEffectv1(StatusEffects.SlowTFingItems2) <= 0)
            {
                player.removeStatusEffect(StatusEffects.SlowTFingItems2);
            }
            else
            {
                player.addStatusValue(StatusEffects.SlowTFingItems2, 1, -1);
            }
        }
        if (flags[kFLAGS.ERLKING_CANE_ATTACK_COUNTER] > 0)
        {
            flags[kFLAGS.ERLKING_CANE_ATTACK_COUNTER] -= 1;
        }
        //Wrap it up
        return needNext;
    }
    ...
    public function timeChangeLarge() : Bool
    {
        if (!ingnam.inIngnam)
        {
            ...
            if (camp.IsSleeping && Holidays.isChristmas() && date.fullYear > flags[kFLAGS.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE])
            ...
            if (checkedTurkey++ == 0 && (rand(5) == 0 && (CoC.instance.model.time.hours == 18 || CoC.instance.model.time.hours == 19)) && (date.fullYear > flags[kFLAGS.TURKEY_FUCK_YEAR_DONE] || flags[kFLAGS.MORE_TURKEY] > 0) && Holidays.isThanksgiving() && player.gender > 0 && flags[kFLAGS.IN_INGNAM] == null)
            {
                SceneLib.holidays.datTurkeyRumpMeeting();  //TURKEY SURPRISE  
                return true;
            }
            if (checkedHellhound++ == 0 && rand(2) == 0 && !player.isRaceCached(Races.CERBERUS) && player.isHellHound() && player.hasPerk(PerkLib.Hellfire) && camp.IsSleeping && player.hasKeyItem("Marae's Lethicite") >= 0 && player.keyItemvX("Marae's Lethicite", 1) > 0 && !player.blockingBodyTransformations())
            {
        ...
        if (checkedDream++ == 0 && camp.IsSleeping && camp.CanDream)
        
        //You can only have one dream each night (NEEDS TO BE FIXED){
            
            camp.CanDream = false;
            if (player.gender > 0 && CoC.instance.model.time.days == 10)
            ...
            if (player.hasCock() && player.hasPerk(PerkLib.BeeOvipositor) && (player.eggs() >= 20 && rand(6) == 0))
            ...
            //Bee dreams proc{
                ...
                if (player.cumQ() >= 1000)
                {
                    outputText("  It's completely soaked your bedroll, too... you won't be sleeping on this again until you wash it out.  Grumbling, you roll the soggy, white-stained fabric up and stow it.");
                }
            ...
            if (player.hasCock() && player.hasPerk(PerkLib.MantisOvipositor) && (player.eggs() >= 20 && rand(6) == 0))
            ...
            //Mantis dreams proc{
                ...
                if (player.hasVagina())
                {
                    outputText(Appearance.vaginaDescript(player, 0) + " and ");
                }
                ...
                if (player.hasVagina())
                {
                    outputText(" while you bend and force your own into her cunt");
                }
                outputText(".  It's not long until you feel ");
                if (player.hasVagina())
                {
                    outputText("her pussy clenching around you as you orgasm explosively inside, followed by ");
                }
                ...
                if (player.cumQ() > 1000)
                {
                    outputText("It's all over your bedroll, too...");
                }
            ...
            if (player.hasCock() && player.hasPerk(PerkLib.SpiderOvipositor) && (player.eggs() >= 20 && rand(6) == 0))
            ...
            var _sw9_ = (flags[kFLAGS.CERAPH_OWNED_DICKS] + flags[kFLAGS.CERAPH_OWNED_PUSSIES] + flags[kFLAGS.CERAPH_OWNED_TITS]);            

            switch (_sw9_)
            {
                case 0:ceraph = 0;  //If you've given her no body parts then Ceraph will not cause any dreams  
                case 1:ceraph = 10;  //Once every 10 days if 1, once every 7 days if 2, once every 5 days if 3  
                case 2:ceraph = 7;
                case 3:ceraph = 5;
                case 4:ceraph = 4;
                default:ceraph = 3;
            ...
            if (flags[kFLAGS.DOMINIKA_FOLLOWUP] > 0 && flags[kFLAGS.DOMINIKA_FOLLOWUP] < 4 && flags[kFLAGS.DOMINIKA_COVENANT] < 3)
            ...
            if (player.lib > (player.libStat.max * 0.5) || player.lust > (player.maxLust() * 0.4))
        ...
        if (player.statusEffectv1(StatusEffects.SlimeCraving) >= 18 && player.str <= 1)
