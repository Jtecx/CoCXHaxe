 ...
 */
class VaginaTransformations extends MutationsHelper
{
    /*
    *	 ██       ██  █████   ██████  ██ ███   ██  █████
    * 	  ██     ██  ██   ██ ██       ██ ████  ██ ██   ██
    ...
    */
    ...
    //noinspection FallThroughInSwitchStatementJS	// Fallthrough is intended for retroactively unlocking in Metamorph after getting GeneticMemory
    {
        ...
        var _sw5_ = (player.vaginas.length);        
        ...
        switch (_sw5_)
        {
            case 2, 1:
                ...
                }Metamorph.unlockMetamorphEx(VaginaCountMem.getMemory(VaginaCountMem.VAGINA1));
            default:
        }
    }
    ...
    private function vaginaIndex(index : Int, type : Int) : Int
    {
        if (index < 0)
        {
            for (index in 0...player.vaginas.length)
            {
                if (player.vaginas[index].type != type)
                {
                    return index;
                }
            }
        }
        if (index >= player.vaginas.length)
        {
            index = 0;
        }
        return index;
    ...
    private function isPresentVagina(index : Int, type : Int) : Bool
    {
        ...
        function(doOutput : Bool) : Void
        {
            ...
            if (player.cockTotal() == 1 && !player.hasVagina())
            {
                ...
                if (flags[kFLAGS.HYPER_HAPPY] == null)
                {
                    if (player.hasBalls())
                    {
                        desc += "  At the same time, your [balls] fall victim to the same sensation; eagerly swallowed whole by your crotch.";
                    }
            ...
            else if (flags[kFLAGS.HYPER_HAPPY] == null)
            {
                player.killCocks(1);
            }
            if (doOutput)
    ...
    public function GrowVaginaGenericText() : String
    {
        ...
        if (!player.hasVagina())
        {
            if (player.hasBalls())
            {
                desc += "An itch starts behind your [balls], but before you can reach under to scratch it, the discomfort fades. A moment later a warm, wet feeling brushes your [sack], and curious about the sensation, you lift up your balls to reveal your new vagina. ";
            }
            else if (player.hasCock())
            {
                desc += "An itch starts on your groin, just below your [cocks]. You pull the manhood aside to give you a better view, and you're able to watch as your skin splits to give you a new vagina, complete with a tiny clit. ";
            }
            else
            {
                desc += "An itch starts on your groin and fades before you can take action. Curious about the intermittent sensation, you peek under your [armor] to discover your brand new vagina, complete with pussy lips and a tiny clit. ";
            }
            player.createVagina();
        ...
        else
        {
            ...
            if (player.hasBalls())
            {
                desc += "You feel behind your [balls], but before you can reach under to scratch it, the discomfort fades. A moment later a warm, wet feeling brushes your [sack], and curious about the sensation, you lift up your balls to reveal your new vagina. ";
            }
            else if (player.hasCock())
            {
                desc += "An itch starts on your groin, just below your [cocks]. You pull the manhood aside to give you a better view, and you're able to watch as your skin splits to give you a new vagina, complete with a tiny clit. ";
    ...
    public function VaginaSandTrap(index : Int = 0) : Transformation
    {
        ...
        function(doOutput : Bool) : Void
        {
            ...
            if (player.vaginas.length > vagina)
            {
                ...
                if (player.wetness() >= 3)
                {
                    desc += "  Your natural lubrication makes it gleam invitingly.";
                }
                //(Corruption <50:
                if (player.cor < 50)
                {
                    desc += "  After a few cautious touches you decide it doesn't feel any different- it does certainly look odd, though.";
                }
                else
                {
                    desc += "  After a few cautious touches you decide it doesn't feel any different - the sheer bizarreness of it is a big turn on though, and you feel it beginning to shine with anticipation at the thought of using it.";
                }
    ...
    public function VaginaCentipede(index : Int = 0) : Transformation
    {
        ...
        function(doOutput : Bool) : Void
        {
            ...
            if (player.vaginas.length > vagina)
            {
                ...
                {
                    desc += "  It would seem your vagina has been stimulated to produce venom.";
                }
