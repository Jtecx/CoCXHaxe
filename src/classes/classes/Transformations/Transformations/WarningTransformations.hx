...
class WarningTransformations extends MutationsHelper
{
    //======================================================================================================================
    //                                                                                                                    ==
    //   ██       ██  █████  ██████  ███    ██ ██ ███    ██  ██████  ███████                                              ==
    ...
    public static inline var BAD_END_COOLDOWN : Int = 6;
    
    public var CaninePepperWarning(default, never) : PossibleEffect = new SimpleEffect("Transformation Warning for Kanga Fruit", 
        // apply
        function(doOutput : Bool) : Void
        {
            ...
            var isBadEnd : Bool = player.getStatusValue(StatusEffects.TFWarning, 2) >= 3;
            if (isBadEnd)
            {
                if (rand(2) == 0)
                {
    ...
    public var KangaFruitWarning(default, never) : PossibleEffect = new SimpleEffect("Transformation Warning for Kanga Fruit", 
        // apply
        function(doOutput : Bool) : Void
        {
            var desc : String = "";
            ...
            player.buff("KangaFruit").addStats({
                                        int.mult : -0.1
                                    }).withText("Kanga Fruit").forHours(BAD_END_COOLDOWN);
            
            var isBadEnd : Bool = player.getStatusValue(StatusEffects.TFWarning, 2) >= 10;
            if (isBadEnd)
            {
