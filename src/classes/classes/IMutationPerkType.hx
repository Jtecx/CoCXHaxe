...
class IMutationPerkType extends PerkType
{
    public var maxLvl(get, never) : Int;
    public var slot(get, never) : String;
    public var trueMutation(get, set) : Bool;
    public var mName(get, never) : String;
    public static var runValidIMutates(get, never) : String;

    /**
		 * For deprecated/not implemented mutations
		 */
    public static inline var SLOT_NONE : String = "";
    public static inline var SLOT_HEART : String = "Heart";
    public static inline var SLOT_MUSCLE : String = "Muscle";
    public static inline var SLOT_MOUTH : String = "Mouth";
    public static inline var SLOT_ADRENALS : String = "Adrenals";
    public static inline var SLOT_BLOODSTREAM : String = "Bloodstream";
    public static inline var SLOT_FAT : String = "FaT";
    public static inline var SLOT_LUNGS : String = "Lungs";
    public static inline var SLOT_METABOLISM : String = "Metabolism";
    public static inline var SLOT_OVARIES : String = "Ovaries";
    public static inline var SLOT_TESTICLES : String = "Testicles";
    public static inline var SLOT_EYES : String = "Eyes";
    public static inline var SLOT_BONE : String = "Bone";
    public static inline var SLOT_NERVSYS : String = "Nerv/Sys";
    public static inline var SLOT_THYROID : String = "Thyroid";
    public static inline var SLOT_PARATHYROID : String = "PThyroid";
    public static inline var SLOT_ADAPTATIONS : String = "Adaptations";
    ...
    /**
		 ...
		 * }
		 */
    public static var Slots : Dynamic = Utils.createMapFromPairs([
                [SLOT_HEART, {
                    name : "Heart"
                }], 
                [SLOT_MUSCLE, {
    ...
    public static var SlotList : Array<Dynamic> = [
        SLOT_HEART, 
        SLOT_MUSCLE, 
        SLOT_MOUTH, 
        SLOT_ADRENALS, 
    ...
    /**
		 ...
		 */
    public static var MutationsBySlot : Dynamic = { };
    
    private var _maxLvl : Int;
    private var _slot : String;
    private var _pBuffs : Dynamic;
    private var _trueVariant : Bool;
    private static var _IMvalid : Dynamic = { };
    private static var _IMNotvalid : Dynamic = { };
    
    
    public function new(id : String, name : String, slot : String, maxLvl : Int, trueVariant : Bool = false)
    
    //GDI probably pre-initialization issue again{
        
        if (_IMvalid.exists(id))
        {
            name += "_errorIM";
            Reflect.setField(_IMNotvalid, id, name);
        }
        else
        {
            Reflect.setField(_IMvalid, id, name);
        }
        super(id, name, name, name, false);
        this._maxLvl = maxLvl;
        this._slot = slot;
        this._trueVariant = trueVariant;
        (if (Reflect.field(MutationsBySlot, slot) == null) {
                    Reflect.setField(MutationsBySlot, slot, []);
                }).push(this);
    }
    
    private function get_maxLvl() : Int
    {
        return _maxLvl;
    }
    
    private function get_slot() : String
    {
        return _slot;
    }
    ...
    public function buffsForTier(pTier : Int, target : Creature) : Dynamic
    {
        return _pBuffs;
    }
    
    private function get_trueMutation() : Bool
    {
        return _trueVariant;
    }
    private function set_trueMutation(isTrue : Bool) : Bool
    {
        _trueVariant = isTrue;
        return isTrue;
    }
    ...
    {
        var tempObj : Dynamic = buffsForTier(pTier, player);
        var res : String = "";
        ...
        res += StatUtils.explainBuff(key, Reflect.field(tempObj, Std.string(key)));
        if (res != "")
        {
            res = "\nBuffs: " + res;
        }
    ...
    public function pBuffs(target : Creature = null) : Dynamic
    {
        var target2 : Creature = ((target == null)) ? player : target;
        return buffsForTier(currentTier(this, target2), target2);
    }
    
    public function trueMutationBuffs(statStr : String, pTier : Int, racePoint : Int) : Dynamic
    //Probably wanna pass "xxx.bonus"
    {
        
        var pBuffs : Dynamic = { };
        Reflect.setField(pBuffs, statStr, ((30 + pTier * 15) * racePoint));
        return pBuffs;
    }
    ...
    public function currentTier(mutate : IMutationPerkType, target : Creature) : Int
    {
        ...
        //If param returns a missing result, is likely because player doesn't have it so cannot query.{
    ...
    /**
		 ...
		 */
    public function acquireMutation(target : Creature, nextFunc : Dynamic, pTier : Int = 1) : Void
    {
        ...
        try
        {
            ...
            //No Perk, set to 1 or pTier.{
                
                target.createPerk(mutations, pTier, 0, 0, 0);
                if (target == player)
                {
                    if (mutations.trueMutation)
                    {
                        EngineCore.outputText(mutations.evolveText());
                    }
            ...
            //Perk exists, but pTier is > 1.{
                
                target.setPerkValue(mutations, 1, pTier);
            }
            //Perk is incremented by 1.
            else
            {
        ...
        {
            nextFunc();
        }  //"none" is a much more explicit statement... though I suppose it can be replaced to a `false` instead....  
        
        //Sets up the buff for the perk.
        function setBuffs() : Void
        {
            var stname : String = "perk_" + mutations.id;
            var pBuff : Dynamic = mutations.pBuffs(target);
    ...
    public function updateDynamicPerkBuffs(target : Creature) : Void
    {
        ...
        var pBuff : Dynamic = this.pBuffs(target);
        if (target.statStore.hasBuff(stname))
        {
            target.statStore.removeBuffs(stname);
        }
    ...
    private function get_mName() : String
    {
        return "";
    }
    
    
    private static function get_runValidIMutates() : String
    {
        if (_IMNotvalid != { })
        {
            for (badIM in Reflect.fields(_IMNotvalid))
            {
