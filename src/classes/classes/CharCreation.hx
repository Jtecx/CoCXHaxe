...
class CharCreation extends BaseContent
{
    
    public var MAX_TOLERANCE_LEVEL(default, never) : Int = 10;
    public var MAX_MORALSHIFTER_LEVEL(default, never) : Int = 10;
    public var MAX_DESIRES_LEVEL(default, never) : Int = 35;
    public var MAX_ENDURANCE_LEVEL(default, never) : Int = 35;
    public var MAX_HARDINESS_LEVEL(default, never) : Int = 35;
    public var MAX_SOULPURITY_LEVEL(default, never) : Int = 35;
    public var MAX_INNERPOWER_LEVEL(default, never) : Int = 35;
    public var MAX_FURY_LEVEL(default, never) : Int = 35;
    public var MAX_KILLINGINTENT_LEVEL(default, never) : Int = 30;
    public var MAX_BLOODLUST_LEVEL(default, never) : Int = 30;
    public var MAX_MYSTICALITY_LEVEL(default, never) : Int = 30;
    public var MAX_SPIRITUALENLIGHTENMENT_LEVEL(default, never) : Int = 30;
    public var MAX_WISDOM_LEVEL(default, never) : Int = 50;
    public var MAX_TRANSHUMANISM_LEVEL(default, never) : Int = 35;
    public var MAX_TRANSHUMANISM_STR_LEVEL(default, never) : Int = 35;
    public var MAX_TRANSHUMANISM_TOU_LEVEL(default, never) : Int = 35;
    public var MAX_TRANSHUMANISM_SPE_LEVEL(default, never) : Int = 35;
    public var MAX_TRANSHUMANISM_INT_LEVEL(default, never) : Int = 35;
    public var MAX_TRANSHUMANISM_WIS_LEVEL(default, never) : Int = 35;
    public var MAX_TRANSHUMANISM_LIB_LEVEL(default, never) : Int = 35;
    public var MAX_TRANSHUMANISM_SEN_LEVEL(default, never) : Int = 35;
    public var MAX_FORTUNE_LEVEL(default, never) : Int = 50;
    public var MAX_VIRILITY_LEVEL(default, never) : Int = 10;
    public var MAX_FERTILITY_LEVEL(default, never) : Int = 10;
    public var MAX_MILK_FAUCET_LEVEL(default, never) : Int = 25;
    public var MAX_CUM_HOSE_LEVEL(default, never) : Int = 25;
    public var MAX_BOTTOMLESS_HUNGER_LEVEL(default, never) : Int = 20;
    public var MAX_EFFICIENT_SOUL_CONSUMPTION_LEVEL(default, never) : Int = 19;
    public var MAX_THE_DARK_SOUL(default, never) : Int = 10;
    ...
    public function newGameGo() : Void
    {
        ...
        var showSpecialNames : Bool = true;  // achievements[kACHIEVEMENTS.STORY_FINALBOSS] > 0;  
        ...
        var preList : Array<Dynamic> = [];
        for (t in 0...specialCharacters.customs.length)
        {
            preList.push({
                        label : specialCharacters.customs[t][0],
                        data : specialCharacters.customs[t]
                    });
        }
        ...
        player.nosePShort = "";
        player.nosePLong = "";
        for (cock/* AS3HX WARNING could not determine type for var: cock exp: EField(EIdent(player),cocks) type: null */ in player.cocks)
        {
            cock.pierced = 0;
            cock.pShortDesc = "";
            cock.pLongDesc = "";
        }
        for (vagina/* AS3HX WARNING could not determine type for var: vagina exp: EField(EIdent(player),vaginas) type: null */ in player.vaginas)
        {
        ...
        {
            var hadOldCock : Bool = player.hasCock();
            var hadOldVagina : Bool = player.hasVagina();
            //Clear cocks
            while (player.cocks.length > 0)
            {
                player.removeCock(0, 1);
            ...
            while (player.biggestTitSize() > 14)
            {
                player.shrinkTits(true);
            }
        }
        //Clear Statuses
        var statusTemp : Array<Dynamic> = [];
        for (i in 0...player.statusEffects.length)
        {
            if (isSpecialStatus(player.statusEffects[i]))
            {
        ...
        //Clear perks
        var ascendPerkTemp : Array<Dynamic> = [];
        for (i in 0...player.perks.length)
        {
        ...
        {
            for (i in 0...ascendPerkTemp.length)
            {
        ...
        //Clear key items
        var keyItemTemp : Array<Dynamic> = [];
        for (i in 0...player.keyItems.length)
        {
        ...
        if (player.hasKeyItem("Ascension") >= 0)
        {
            ...
            {
                Reflect.setField(newFlags, Std.string(flag), Reflect.field(flags, Std.string(flag)));
            }
        }
        ...
        {
            aSPPMX += player.perkv1(PerkLib.AscensionSkyPoisonPearlMasteryStageX);
        }
        if (flags[kFLAGS.SPIRIT_STONES] > (100 * (1 + player.newGamePlusMod()) * aSPPMX))
        {
            flags[kFLAGS.SPIRIT_STONES] = (100 * (1 + player.newGamePlusMod()) * aSPPMX);
        }
        //Time reset
        model.time.days = 0;
        model.time.hours = 0;
    ...
    private function chooseName() : Void
    {
        ...
        //Dirty checking as the NG+ flag is incremented after reincarnating.{
    ...
    private function selectName(selectedItem : Dynamic) : Void
    {
    ...
    {
        for (i in 0...specialCharacters.customs.length)
        {
            if (specialCharacters.customs[i][0] == arg && specialCharacters.customs[i][2] == null)
            {
                return specialCharacters.customs[i][1];
            }
        }
        return specialName(arg);
    }
    ...
    private function setEyesColor(choice : String) : Void
    //And choose hair
    {
    ...
    private function genericStyleCustomizeMenu() : Void
    {
        ...
        if (player.hasCock())
        {
            addButton(5, "Cock Size", menuCockLength);
        }
    ...
    private function chooseBeardLength(choiceLength : Float = 0) : Void
    {
    ...
    {
        mainView.nameBox.visible = false;
        if (as3hx.Compat.parseInt(mainView.nameBox.text) < 48)
        {
            clearOutput();
        ...
        if (as3hx.Compat.parseInt(mainView.nameBox.text) > 96)
        {
            clearOutput();
        ...
        player.tallness = as3hx.Compat.parseInt(mainView.nameBox.text);
        mainView.nameBox.maxChars = 16;
        mainView.nameBox.restrict = null;
        clearOutput();
    ...
    private function chooseCockLength(length : Float) : Void
    {
    ...
    private function menuBreastSize() : Void
    {
        ...
        {
            addButton(1, "A-cup", chooseBreastSize, 1);
        }
        if (player.femininity >= 40)
        {
            addButton(2, "B-cup", chooseBreastSize, 2);
        }
        if (player.femininity >= 50)
        {
            addButton(3, "C-cup", chooseBreastSize, 3);
        }
        if (player.femininity >= 60)
        {
            addButton(4, "D-cup", chooseBreastSize, 4);
        }
        if (player.femininity >= 70)
        {
            addButton(5, "DD-cup", chooseBreastSize, 5);
        }
        addButton(14, "Back", genericStyleCustomizeMenu);
    }
    ...
    //-- STARTER PERKS
    //-----------------
    private function chooseEndowment(clear : Bool) : Void
    {
        if (clear)
        {
            clearOutput();
    ...
    private function setEndowmentStrength() : Void
    {
        ...
        if (!player.hasPerk(PerkLib.Strong))
        {
    ...
    private function setEndowmentToughness() : Void
    {
        player.tone += 5;
        player.thickness += 5;
        if (!player.hasPerk(PerkLib.Tough))
        {
    ...
    {
        player.thickness -= 5;
        if (!player.hasPerk(PerkLib.Smart))
        {
    ...
    {
        player.thickness -= 5;
        if (!player.hasPerk(PerkLib.Wise))
        {
            player.createPerk(PerkLib.Wise, 1, 0, 0, 0);
        }
        chooseBloodlineorNot();
    }
    ...
    private function setEndowmentLibido() : Void
    {
        ...
        {
            player.createPerk(PerkLib.Lusty, 1, 0, 0, 0);
        }
        chooseBloodlineorNot();
    }
    ...
    {
        if (!player.hasPerk(PerkLib.Sensitive))
        {
            player.createPerk(PerkLib.Sensitive, 1, 0, 0, 0);
        }
        chooseBloodlineorNot();
    }
    ...
    {
        player.cor += 5;
        if (!player.hasPerk(PerkLib.Pervert))
        {
            player.createPerk(PerkLib.Pervert, 1, 0, 0, 0);
        }
        chooseBloodlineorNot();
    }
    ...
    private function setEndowmentBigCock() : Void
    {
        ...
        if (!player.hasPerk(PerkLib.BigCock))
        {
    ...
    private function setEndowmentBigBreasts() : Void
    {
        ...
        if (!player.hasPerk(PerkLib.BigTits))
        {
    ...
    private function setEndowmentBigClit() : Void
    {
        player.femininity -= 10;
        player.clitLength = 1;
        if (!player.hasPerk(PerkLib.BigClit))
        {
    ...
    private function setEndowmentFertile() : Void
    {
        ...
        player.fertility += 50;
        player.hips.type += 3;
        if (!player.hasPerk(PerkLib.Fertile))
        {
    ...
    private function setEndowmentWetVagina() : Void
    {
        ...
        if (!player.hasPerk(PerkLib.WetPussy))
        {
    ...
    public function chooseBloodline1() : Void
    {
        ...
        menu();
        if (!player.hasPerk(PerkLib.BloodlineDragon))
        {
            addButton(0, "Dragon", confirmBloodline1, PerkLib.DragonsDescendant).hint("(+2 to dragon score)");
        }
        else
        {
            addButtonDisabled(0, "Dragon", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.BloodlineElf))
        {
            addButton(1, "Elf", confirmBloodline1, PerkLib.ElfsDescendant).hint("(+2 to elf score)");
        }
        else
        {
            addButtonDisabled(1, "Elf", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.BloodlineGoblin))
        {
            addButton(2, "Goblin", confirmBloodline1, PerkLib.GoblinsDescendant).hint("(+2 to goblin score)");
        }
        else
        {
            addButtonDisabled(2, "Goblin", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.BloodlineKitsune))
        {
            addButton(3, "Kitsune", confirmBloodline1, PerkLib.KitsunesDescendant).hint("(+2 to kitsune score)");
        }
        else
        {
            addButtonDisabled(3, "Kitsune", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.BloodlineRaiju))
        {
            addButton(4, "Raiju", confirmBloodline1, PerkLib.RaijusDescendant).hint("(+2 to raiju score)");
        }
        else
        {
            addButtonDisabled(4, "Raiju", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.BloodlineMinotaur))
        {
            addButton(5, "Minotaur", confirmBloodline1, PerkLib.MinotaursDescendant).hint("(+2 to minotaur score)");
        }
        else
        {
            addButtonDisabled(5, "Minotaur", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.BloodlineOni))
        {
            addButton(6, "Oni", confirmBloodline1, PerkLib.OnisDescendant).hint("(+2 to oni score)");
        }
        else
        {
            addButtonDisabled(6, "Oni", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.BloodlineOrc))
        {
            addButton(7, "Orc", confirmBloodline1, PerkLib.OrcsDescendant).hint("(+2 to orc score)");
        }
        else
        {
            addButtonDisabled(7, "Orc", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.BloodlineVampire))
        {
            addButton(8, "Vampire", confirmBloodline1, PerkLib.VampiresDescendant).hint("(+2 to vampire score)");
        }
        else
        {
            addButtonDisabled(8, "Vampire", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.BloodlineMelkie))
        {
            addButton(9, "Melkie", confirmBloodline1, PerkLib.MelkiesDescendant).hint("(+2 to melkie score)");
        }
        else
        {
            addButtonDisabled(9, "Melkie", "You already have this bloodline!");
        }
        addButton(12, "-2-", chooseBloodline2);
    ...
    {
        menu();
        if (!player.hasPerk(PerkLib.BloodlineHydra))
        {
            addButton(0, "Hydra", confirmBloodline2, PerkLib.HydrasDescendant).hint("(+2 to hydra score)");
        }
        else
        {
            addButtonDisabled(0, "Hydra", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.BloodlineSalamander))
        {
            addButton(1, "Salamander", confirmBloodline2, PerkLib.SalamandersDescendant).hint("(+2 to salamander score)");
        }
        else
        {
            addButtonDisabled(1, "Salamander", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.BloodlineWerefox))
        {
            addButton(2, "Werefox", confirmBloodline2, PerkLib.WerefoxsDescendant).hint("(+2 to werefox score)");
        }
        else
        {
            addButtonDisabled(2, "Werefox", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.BloodlineWerewolf))
        {
            addButton(3, "Werewolf", confirmBloodline2, PerkLib.WerewolfsDescendant).hint("(+2 to werewolf score)");
        }
        else
        {
            addButtonDisabled(3, "Werewolf", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.BloodlineWereshark))
        {
            addButton(4, "Wereshark", confirmBloodline2, PerkLib.WeresharksDescendant).hint("(+2 to wereshark score)");
        }
        else
        {
            addButtonDisabled(4, "Wereshark", "You already have this bloodline!");
        }
        //any future wereraces would go here
        if (!player.hasPerk(PerkLib.BloodlineRatatoskr))
        {
            addButton(5, "Ratatoskr", confirmBloodline2, PerkLib.RatatoskrsDescendant).hint("(+2 to ratatoskr score)");
        }
        else
        {
            addButtonDisabled(5, "Ratatoskr", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.BloodlineKraken))
        {
            addButton(6, "Kraken", confirmBloodline2, PerkLib.KrakensDescendant).hint("(+2 to scylla/kraken score)");
        }
        else
        {
            addButtonDisabled(6, "Kraken", "You already have this bloodline!");
        }
    ...
    public function chooseBloodline3() : Void
    {
        ...
        {
            if (flags[kFLAGS.HUMAN_BLOODLINE] >= 1)
            {
                addButton(0, "CommonH", confirmBloodline3, PerkLib.CommonHumansDescendant).hint("(+2 to human score)");
            }
            else
            {
                addButtonDisabled(0, "CommonH", "You not meet req. for this bloodline: 10+ human score and 4+ different human internal mutations.");
            }
        }
        else
        {
            addButtonDisabled(0, "CommonH", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.NobleHumanBloodline))
        {
            if (flags[kFLAGS.HUMAN_BLOODLINE] >= 2)
            {
                addButton(1, "NobleH", confirmBloodline3, PerkLib.NobleHumansDescendant).hint("(+4 to human score)");
            }
            else
            {
                addButtonDisabled(1, "NobleH", "You not meet req. for this bloodline: 20+ human score and 8+ different human internal mutations.");
            }
        }
        else
        {
            addButtonDisabled(1, "NobleH", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.RoyalHumanBloodline))
        {
            if (flags[kFLAGS.HUMAN_BLOODLINE] >= 3)
            {
                addButton(2, "RoyalH", confirmBloodline3, PerkLib.RoyalHumansDescendant).hint("(+6 to human score)");
            }
            else
            {
                addButtonDisabled(2, "RoyalH", "You not meet req. for this bloodline: 40+ human score and 12+ different human internal mutations.");
            }
        }
        else
        {
            addButtonDisabled(2, "RoyalH", "You already have this bloodline!");
        }
        if (!player.hasPerk(PerkLib.PrimarchBloodline))
        {
            if (flags[kFLAGS.HUMAN_BLOODLINE] >= 4)
            {
                addButton(3, "PrimarchB", confirmBloodline3, PerkLib.PrimarchsDescendant).hint("(+10 to human score)");
            }
            else
            {
                addButtonDisabled(3, "PrimarchB", "You not meet req. for this bloodline: 50+ human score and 14+ different human internal mutations.");
            }
        }
        else
        {
            addButtonDisabled(3, "PrimarchB", "You already have this bloodline!");
        }
        addButton(12, "-1-", chooseBloodline1);
    ...
    public function chooseHistory() : Void
    {
        ...
        menu();
        if (!player.hasPerk(PerkLib.PastLifeAlchemist))
        {
            addButton(0, "Alchemy", confirmHistory1, PerkLib.HistoryAlchemist);
        }
        else
        {
            addButtonDisabled(0, "Alchemy", "You already have this History as one of your Past Lives!");
        }
        if (!player.hasPerk(PerkLib.Soulless))
        {
            if (!player.hasPerk(PerkLib.PastLifeCultivator))
            {
                addButton(1, "Cultivator", confirmHistory1, PerkLib.HistoryCultivator);
            }
            else
            {
                addButtonDisabled(1, "Cultivator", "You already have this History as one of your Past Lives!");
            }
        }
        if (!player.hasPerk(PerkLib.PastLifeFeral))
        {
            addButton(2, "Feral", confirmHistory1, PerkLib.HistoryFeral);
        }
        else
        {
            addButtonDisabled(2, "Feral", "You already have this History as one of your Past Lives!");
        }
        if (!player.hasPerk(PerkLib.PastLifeFighter))
        {
            addButton(3, "Fighting", confirmHistory1, PerkLib.HistoryFighter);
        }
        else
        {
            addButtonDisabled(3, "Fighting", "You already have this History as one of your Past Lives!");
        }
        if (!player.hasPerk(PerkLib.PastLifeFortune))
        {
            addButton(4, "Fortune", confirmHistory1, PerkLib.HistoryFortune);
        }
        else
        {
            addButtonDisabled(4, "Fortune", "You already have this History as one of your Past Lives!");
        }
        if (!player.hasPerk(PerkLib.PastLifeHealer))
        {
            addButton(5, "Healing", confirmHistory1, PerkLib.HistoryHealer);
        }
        else
        {
            addButtonDisabled(5, "Healing", "You already have this History as one of your Past Lives!");
        }
        if (!player.hasPerk(PerkLib.PastLifeReligious))
        {
            addButton(6, "Religion", confirmHistory1, PerkLib.HistoryReligious);
        }
        else
        {
            addButtonDisabled(6, "Religion", "You already have this History as one of your Past Lives!");
        }
    ...
    {
        menu();
        if (!player.hasPerk(PerkLib.PastLifeScholar))
        {
            addButton(0, "Schooling", confirmHistory2, PerkLib.HistoryScholar);
        }
        else
        {
            addButtonDisabled(0, "Schooling", "You already have this History as one of your Past Lives!");
        }
        if (!player.hasPerk(PerkLib.PastLifeScout))
        {
            addButton(1, "Scout", confirmHistory2, PerkLib.HistoryScout);
        }
        else
        {
            addButtonDisabled(1, "Scout", "You already have this History as one of your Past Lives!");
        }
        if (!player.hasPerk(PerkLib.PastLifeSlacker))
        {
            addButton(2, "Slacking", confirmHistory2, PerkLib.HistorySlacker);
        }
        else
        {
            addButtonDisabled(2, "Slacking", "You already have this History as one of your Past Lives!");
        }
        if (!player.hasPerk(PerkLib.PastLifeSlut))
        {
            addButton(3, "Slutting", confirmHistory2, PerkLib.HistorySlut);
        }
        else
        {
            addButtonDisabled(3, "Slutting", "You already have this History as one of your Past Lives!");
        }
        if (!player.hasPerk(PerkLib.PastLifeSmith))
        {
            addButton(4, "Smithing", confirmHistory2, PerkLib.HistorySmith);
        }
        else
        {
            addButtonDisabled(4, "Smithing", "You already have this History as one of your Past Lives!");
        }
        if (!player.hasPerk(PerkLib.PastLifeTactician))
        {
            addButton(5, "Tactician", confirmHistory2, PerkLib.HistoryTactician);
        }
        else
        {
            addButtonDisabled(5, "Tactician", "You already have this History as one of your Past Lives!");
        }
        if (!player.hasPerk(PerkLib.PastLifeThief))
        {
            addButton(6, "Thief", confirmHistory2, PerkLib.HistoryThief);
        }
        else
        {
            addButtonDisabled(6, "Thief", "You already have this History as one of your Past Lives!");
        }
        if (!player.hasPerk(PerkLib.PastLifeWhore))
        {
            addButton(7, "Whoring", confirmHistory2, PerkLib.HistoryWhore);
        }
        else
        {
            addButtonDisabled(7, "Whoring", "You already have this History as one of your Past Lives!");
        }
    ...
    {
        if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] < 11)
        {
            player.perkPoints += (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] + 1);
        }
        else
        {
            player.perkPoints += 12;
    ...
    public function arrival() : Void
    {
        ...
        {
            outputText("and your body reacts with a sense of growing warmth focusing in your groin, your manhood hardening for no apparent reason.");
        }
    ...
    private function chooseGameModes() : Void
    {
        ...
        {
            addButton(3, "Hardcore (On)", toggleForHardcoreOn);
        }
        if (flags[kFLAGS.HARDCORE_MODE] != 0)
        {
            addButton(4, "Hardcore (Off)", toggleForHardcoreOff);
        }
        if (flags[kFLAGS.HUNGER_ENABLED] != 0)
        {
            addButton(5, "Hunger (Off)", toggleForHungerOff);
        }
        if (flags[kFLAGS.HUNGER_ENABLED] != 0.5)
        {
            addButton(6, "Hunger (On)", toggleForHungerOn);
        }
        if (flags[kFLAGS.HUNGER_ENABLED] != 1)
        {
            addButton(7, "Realistic (On)", toggleForRealisticOn);
        }
        //8+
        //9-
        //10+
    ...
    private function chooseGameModes1() : Void
    {
        ...
        if (flags[kFLAGS.PRIMARY_DIFFICULTY] != 0)
        {
            addButton(0, "PrimaryDiff(0)", toggleForPrimaryDiffultyModifier, 0);
        }
        if (flags[kFLAGS.PRIMARY_DIFFICULTY] != 1)
        {
            addButton(1, "PrimaryDiff(1)", toggleForPrimaryDiffultyModifier, 1);
        }
        if (flags[kFLAGS.PRIMARY_DIFFICULTY] != 2)
        {
            addButton(2, "PrimaryDiff(2)", toggleForPrimaryDiffultyModifier, 2);
        }
        if (flags[kFLAGS.PRIMARY_DIFFICULTY] != 3)
        {
            addButton(3, "PrimaryDiff(3)", toggleForPrimaryDiffultyModifier, 3);
        }
        if (flags[kFLAGS.PRIMARY_DIFFICULTY] != 4)
        {
            addButton(4, "PrimaryDiff(4)", toggleForPrimaryDiffultyModifier, 4);
        }
    ...
    private function chooseGameModes2() : Void
    {
        ...
        if (flags[kFLAGS.SECONDARY_STATS_SCALING] != 0)
        {
            addButton(0, "SecondaryStat(0)", toggleForSecondaryStatsModifier, 0);
        }
        if (flags[kFLAGS.SECONDARY_STATS_SCALING] != 1)
        {
            addButton(1, "SecondaryStat(1)", toggleForSecondaryStatsModifier, 1);
        }
        if (flags[kFLAGS.SECONDARY_STATS_SCALING] != 2)
        {
            addButton(2, "SecondaryStat(2)", toggleForSecondaryStatsModifier, 2);
        }
        if (flags[kFLAGS.SECONDARY_STATS_SCALING] != 3)
        {
            addButton(3, "SecondaryStat(3)", toggleForSecondaryStatsModifier, 3);
        }
        if (flags[kFLAGS.SECONDARY_STATS_SCALING] != 4)
        {
            addButton(4, "SecondaryStat(4)", toggleForSecondaryStatsModifier, 4);
        }
        if (flags[kFLAGS.SECONDARY_STATS_SCALING] != 5)
        {
            addButton(5, "SecondaryStat(5)", toggleForSecondaryStatsModifier, 5);
        }
        if (flags[kFLAGS.SECONDARY_STATS_SCALING] != 6)
        {
            addButton(6, "SecondaryStat(6)", toggleForSecondaryStatsModifier, 6);
        }
        addButton(14, "Back", chooseGameModes);
    }
    private function chooseGameModes3() : Void
    {
        ...
        if (flags[kFLAGS.BOSS_CHAMPION_ELITE_SCALING] != 0)
        {
            addButton(0, "Normal", toggleForBossesModifier, 0);
        }
        if (flags[kFLAGS.BOSS_CHAMPION_ELITE_SCALING] != 1)
        {
            addButton(1, "Fantasy", toggleForBossesModifier, 1);
        }
        if (flags[kFLAGS.BOSS_CHAMPION_ELITE_SCALING] != 2)
        {
            addButton(2, "Torment", toggleForBossesModifier, 2);
        }
        if (flags[kFLAGS.BOSS_CHAMPION_ELITE_SCALING] != 3)
        {
            addButton(3, "Infernium", toggleForBossesModifier, 3);
        }
        if (flags[kFLAGS.BOSS_CHAMPION_ELITE_SCALING] != 4)
        {
            addButton(4, "Hell", toggleForBossesModifier, 4);
        }
        addButton(14, "Back", chooseGameModes);
    }
    ...
    private function chooseTimescale() : Void
    {
        ...
        var setTimescale : Int->Void = function(val : Int) : Void
        {
    ...
    private function startTheGame() : Void
    {
        ...
        statScreenRefresh();
        if (player.hasPerk(PerkLib.PastLifeCultivator) || player.hasPerk(PerkLib.PastLifeFighter) || player.hasPerk(PerkLib.PastLifeScout) || player.hasPerk(PerkLib.PastLifeScholar) || player.hasPerk(PerkLib.PastLifeSmith) || player.hasPerk(PerkLib.PastLifeTactician) || player.hasPerk(PerkLib.PastLifeWhore))
        {
            chooseToPlayHalf();
        }
        else
    ...
    {
        if (player.femininity >= 55)
        {
            player.setUnderBottom(undergarments.C_PANTY, false);
        }
        else
        {
            player.setUnderBottom(undergarments.C_LOIN, false);
        }
        if (player.biggestTitSize() >= 2)
        {
            player.setUnderTop(undergarments.C_BRA, false);
        }
        else
        {
            player.setUnderTop(undergarments.C_SHIRT, false);
        }
        ...
        if (player.hasPerk(PerkLib.HistoryFeral) || (player.hasPerk(PerkLib.PastLifeFeral) && player.hasKeyItem("PerksOverJobs") < 0))
        {
            player.createPerk(PerkLib.JobBeastWarrior, 0, 0, 0, 0);
        }
        if (player.hasPerk(PerkLib.HistoryFighter) || (player.hasPerk(PerkLib.PastLifeFighter) && player.hasKeyItem("PerksOverJobs") < 0))
        {
            player.createPerk(PerkLib.JobWarrior, 0, 0, 0, 0);
        }
        if (player.hasPerk(PerkLib.HistoryScout) || (player.hasPerk(PerkLib.PastLifeScout) && player.hasKeyItem("PerksOverJobs") < 0))
        {
            player.createPerk(PerkLib.JobRanger, 0, 0, 0, 0);
        }
        if (player.hasPerk(PerkLib.HistoryScholar) || (player.hasPerk(PerkLib.PastLifeScholar) && player.hasKeyItem("PerksOverJobs") < 0))
        {
            player.createPerk(PerkLib.JobSorcerer, 0, 0, 0, 0);
        }
        if (player.hasPerk(PerkLib.HistorySmith) || (player.hasPerk(PerkLib.PastLifeSmith) && player.hasKeyItem("PerksOverJobs") < 0))
        {
            player.createPerk(PerkLib.JobGuardian, 0, 0, 0, 0);
        }
        if (player.hasPerk(PerkLib.HistoryTactician) || (player.hasPerk(PerkLib.PastLifeTactician) && player.hasKeyItem("PerksOverJobs") < 0))
        {
            player.createPerk(PerkLib.JobLeader, 0, 0, 0, 0);
        }
        if (player.hasPerk(PerkLib.HistoryThief) || (player.hasPerk(PerkLib.PastLifeThief) && player.hasKeyItem("PerksOverJobs") < 0))
        {
            player.createPerk(PerkLib.JobRogue, 0, 0, 0, 0);
        }
        if (player.hasPerk(PerkLib.HistoryWhore) || (player.hasPerk(PerkLib.PastLifeWhore) && player.hasKeyItem("PerksOverJobs") < 0))
        {
        ...
        if (player.hasPerk(PerkLib.HistoryHealer))
        {
            player.perkPoints += 1;
        }
        if (player.hasPerk(PerkLib.HistoryReligious))
        {
            player.perkPoints += 1;
        }
        if (player.hasPerk(PerkLib.HistorySlacker))
        {
            player.perkPoints += 1;
        }
        if (player.hasPerk(PerkLib.HistorySlut))
        {
            player.perkPoints += 1;
        }
        if (player.hasPerk(PerkLib.PastLifeFeral) && player.hasKeyItem("PerksOverJobs") >= 0)
        {
            player.perkPoints += 1;
        }
        if (player.hasPerk(PerkLib.PastLifeFighter) && player.hasKeyItem("PerksOverJobs") >= 0)
        {
            player.perkPoints += 1;
        }
        if (player.hasPerk(PerkLib.PastLifeScout) && player.hasKeyItem("PerksOverJobs") >= 0)
        {
            player.perkPoints += 1;
        }
        if (player.hasPerk(PerkLib.PastLifeScholar) && player.hasKeyItem("PerksOverJobs") >= 0)
        {
            player.perkPoints += 1;
        }
        if (player.hasPerk(PerkLib.PastLifeSmith) && player.hasKeyItem("PerksOverJobs") >= 0)
        {
            player.perkPoints += 1;
        }
        if (player.hasPerk(PerkLib.PastLifeTactician) && player.hasKeyItem("PerksOverJobs") >= 0)
        {
            player.perkPoints += 1;
        }
        if (player.hasPerk(PerkLib.PastLifeThief) && player.hasKeyItem("PerksOverJobs") >= 0)
        {
            player.perkPoints += 1;
        }
        if (player.hasPerk(PerkLib.PastLifeWhore) && player.hasKeyItem("PerksOverJobs") >= 0)
        {
            player.perkPoints += 1;
        }
        if (player.hasPerk(PerkLib.PastLifeFortune))
        ...
        if (player.hasKeyItem("PerksOverJobs") >= 0)
        {
            player.removeKeyItem("PerksOverJobs");
        }
        ...
        if (player.hasPerk(PerkLib.Soulless))
        {
            addButton(1, "Skip Time", arrivalPartZero);
        }
        else
        {
    ...
    //-- ASCENSION
    //-----------------
    private var migration : Bool = false;
    ...
    public function ascensionMenuChoice() : Void
    {
        ...
        if (player.ascensionPerkPoints >= 50)
        {
            addButton(3, "Maybe?", ascensionMenuChoiceMaybe).hint("Ascend without increasing difficulty.");
        }
        else
        {
            addButtonDisabled(3, "Maybe?", "50 ascension points required.");
        }
    }
    public function darkAscensionMenuChoice() : Void
    {
        outputText("\n\nWould you like to ascend without increasing the New Game+ cycle (difficulty) or not?");
        if (player.perkv4(PerkLib.Soulless) < 1)
        {
            player.addPerkValue(PerkLib.Soulless, 4, 1);
        }
        menu();
    ...
    public function ascensionMenu() : Void
    {
        ...
        {
            addButton(7, "Past Life", historyTopastlife).hint("Spend Ascension Points to change current possessed History perk into Past Life perk (5 points).", "Perk Selection");
        }
        else
        {
            addButtonDisabled(7, "Past Life", "You don't have enough Ascension Perk Points to use this option.");
        }
        if (player.hasPerk(PerkLib.AscensionCruelChimerasThesis) && flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 3)
        {
            if (player.ascensionPerkPoints >= 10)
            {
                addButton(8, "Bloodline", bloodlineACQ).hint("Spend Ascension Points to change current possessed Descendant perk into Bloodline perk (10 points).", "Perk Selection");
            }
            else
            {
                addButtonDisabled(8, "Bloodline", "You don't have enough Ascension Perk Points to use this option.");
            }
        }
        else
        {
            addButtonDisabled(8, "???", "You don't have Ascended enough times or/and have required ascension perk to use this option.");
        }
        addButton(10, "Rename", renamePrompt).hint("Change your name at no charge.");
        if (!migration)
        {
            addButton(14, "Reincarnate", reincarnatePrompt).hint("Reincarnate and start an entirely new adventure.");
        }
        else
        {
            addButton(14, "Return", returnFromUpdateAscension).hint("Go back to your camp after updating your Ascension perks. (Only available during updates that refund points like this)");
        }
    }
    ...
    private function genMemPatch() : Void
    //Cause for some fuckall rason the status gets wiped on ascending.
    {
        ...
        if (player.hasPerk(PerkLib.AscensionTrancendentalGeneticMemoryStageX))
        {
            ...
            for (k in Reflect.fields(Metamorph.PermanentMemoryStorage))
            {
                permedMetamorphCount++;
            }
            var tier : Int = as3hx.Compat.parseInt(player.perkv1(PerkLib.AscensionTrancendentalGeneticMemoryStageX) + 1);
    ...
    private function ascensionPerkMenu() : Void
    {
        ...
        menu();
        var limitReached : String = "Limit Reached";
        addButton(0, "Killing Intent", ascensionPerkSelection, PerkLib.AscensionKillingIntent, MAX_KILLINGINTENT_LEVEL, null, PerkLib.AscensionKillingIntent.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionKillingIntent) + " / " + MAX_KILLINGINTENT_LEVEL).disableIf(player.perkv1(PerkLib.AscensionKillingIntent) >= MAX_KILLINGINTENT_LEVEL, limitReached);
        addButton(1, "Bloodlust", ascensionPerkSelection, PerkLib.AscensionBloodlust, MAX_BLOODLUST_LEVEL, null, PerkLib.AscensionBloodlust.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionBloodlust) + " / " + MAX_BLOODLUST_LEVEL).disableIf(player.perkv1(PerkLib.AscensionBloodlust) >= MAX_BLOODLUST_LEVEL, limitReached);
        addButton(2, "Mysticality", ascensionPerkSelection, PerkLib.AscensionMysticality, MAX_MYSTICALITY_LEVEL, null, PerkLib.AscensionMysticality.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionMysticality) + " / " + MAX_MYSTICALITY_LEVEL).disableIf(player.perkv1(PerkLib.AscensionMysticality) >= MAX_MYSTICALITY_LEVEL, limitReached);
        if (player.hasPerk(PerkLib.Soulless))
        {
            addButtonDisabled(3, "S.Enlight.", "Without soul you can't buy or improve this ascension perk.");
        }
        else
        {
            addButton(3, "S.Enlight.", ascensionPerkSelection, PerkLib.AscensionSpiritualEnlightenment, MAX_SPIRITUALENLIGHTENMENT_LEVEL, null, PerkLib.AscensionSpiritualEnlightenment.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionSpiritualEnlightenment) + " / " + MAX_SPIRITUALENLIGHTENMENT_LEVEL).disableIf(player.perkv1(PerkLib.AscensionSpiritualEnlightenment) >= MAX_SPIRITUALENLIGHTENMENT_LEVEL, limitReached);
        }
        addButton(4, "Tolerance", ascensionPerkSelection, PerkLib.AscensionTolerance, MAX_TOLERANCE_LEVEL, null, PerkLib.AscensionTolerance.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionTolerance) + " / " + MAX_TOLERANCE_LEVEL).disableIf(player.perkv1(PerkLib.AscensionTolerance) >= MAX_TOLERANCE_LEVEL, limitReached);
        addButton(5, "Fertility", ascensionPerkSelection, PerkLib.AscensionFertility, MAX_FERTILITY_LEVEL, null, PerkLib.AscensionFertility.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionFertility) + " / " + MAX_FERTILITY_LEVEL).disableIf(player.perkv1(PerkLib.AscensionFertility) >= MAX_FERTILITY_LEVEL, limitReached);
        addButton(6, "Virility", ascensionPerkSelection, PerkLib.AscensionVirility, MAX_VIRILITY_LEVEL, null, PerkLib.AscensionVirility.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionVirility) + " / " + MAX_VIRILITY_LEVEL).disableIf(player.perkv1(PerkLib.AscensionVirility) >= MAX_VIRILITY_LEVEL, limitReached);
        addButton(7, "Wisdom", ascensionPerkSelection, PerkLib.AscensionWisdom, MAX_WISDOM_LEVEL, null, PerkLib.AscensionWisdom.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionWisdom) + " / " + maxRank).disableIf(player.perkv1(PerkLib.AscensionWisdom) >= maxRank, limitReached);
        addButton(8, "Milk Faucet", ascensionPerkSelection, PerkLib.AscensionMilkFaucet, MAX_MILK_FAUCET_LEVEL, null, PerkLib.AscensionMilkFaucet.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionMilkFaucet) + " / " + MAX_MILK_FAUCET_LEVEL).disableIf(player.perkv1(PerkLib.AscensionMilkFaucet) >= MAX_MILK_FAUCET_LEVEL, limitReached);
        addButton(9, "Cum Hose", ascensionPerkSelection, PerkLib.AscensionCumHose, MAX_CUM_HOSE_LEVEL, null, PerkLib.AscensionCumHose.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionCumHose) + " / " + MAX_CUM_HOSE_LEVEL).disableIf(player.perkv1(PerkLib.AscensionCumHose) >= MAX_CUM_HOSE_LEVEL, limitReached);
        addButton(10, "Fortune", ascensionPerkSelection, PerkLib.AscensionFortune, MAX_FORTUNE_LEVEL, null, PerkLib.AscensionFortune.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionFortune) + " / " + maxRank).disableIf(player.perkv1(PerkLib.AscensionFortune) >= maxRank, limitReached);
        addButton(11, "Moral Shifter", ascensionPerkSelection, PerkLib.AscensionMoralShifter, MAX_MORALSHIFTER_LEVEL, null, PerkLib.AscensionMoralShifter.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionMoralShifter) + " / " + MAX_MORALSHIFTER_LEVEL).disableIf(player.perkv1(PerkLib.AscensionMoralShifter) >= MAX_MORALSHIFTER_LEVEL, limitReached);
        if (player.hasPerk(PerkLib.Soulless))
        {
    ...
    private function ascensionPerkMenu2() : Void
    {
        ...
        menu();
        var limitReached : String = "Limit Reached";
        addButton(0, "Desires", ascensionPerkSelection2, PerkLib.AscensionDesires, MAX_DESIRES_LEVEL, null, PerkLib.AscensionDesires.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionDesires) + " / " + MAX_DESIRES_LEVEL).disableIf(player.perkv1(PerkLib.AscensionDesires) >= MAX_DESIRES_LEVEL, limitReached);
        addButton(1, "Endurance", ascensionPerkSelection2, PerkLib.AscensionEndurance, MAX_ENDURANCE_LEVEL, null, PerkLib.AscensionEndurance.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionEndurance) + " / " + MAX_ENDURANCE_LEVEL).disableIf(player.perkv1(PerkLib.AscensionEndurance) >= MAX_ENDURANCE_LEVEL, limitReached);
        addButton(2, "Hardiness", ascensionPerkSelection2, PerkLib.AscensionHardiness, MAX_HARDINESS_LEVEL, null, PerkLib.AscensionHardiness.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionHardiness) + " / " + MAX_HARDINESS_LEVEL).disableIf(player.perkv1(PerkLib.AscensionHardiness) >= MAX_HARDINESS_LEVEL, limitReached);
        if (player.hasPerk(PerkLib.Soulless))
        {
            addButtonDisabled(3, "Soul Purity", "Without soul you can't buy or improve this ascension perk.");
        }
        else
        {
            addButton(3, "Soul Purity", ascensionPerkSelection2, PerkLib.AscensionSoulPurity, MAX_SOULPURITY_LEVEL, null, PerkLib.AscensionSoulPurity.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionSoulPurity) + " / " + MAX_SOULPURITY_LEVEL).disableIf(player.perkv1(PerkLib.AscensionSoulPurity) >= MAX_SOULPURITY_LEVEL, limitReached);
        }
        addButton(4, "Inner Power", ascensionPerkSelection2, PerkLib.AscensionInnerPower, MAX_INNERPOWER_LEVEL, null, PerkLib.AscensionInnerPower.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionInnerPower) + " / " + MAX_INNERPOWER_LEVEL).disableIf(player.perkv1(PerkLib.AscensionInnerPower) >= MAX_INNERPOWER_LEVEL, limitReached);
        addButton(5, "Fury", ascensionPerkSelection2, PerkLib.AscensionFury, MAX_FURY_LEVEL, null, PerkLib.AscensionFury.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionFury) + " / " + MAX_FURY_LEVEL).disableIf(player.perkv1(PerkLib.AscensionFury) >= MAX_FURY_LEVEL, limitReached);
        addButton(6, "Transhuman.", ascensionPerkSelection2, PerkLib.AscensionTranshumanism, MAX_TRANSHUMANISM_LEVEL, null, PerkLib.AscensionTranshumanism.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionTranshumanism) + " / " + MAX_TRANSHUMANISM_LEVEL).disableIf(player.perkv1(PerkLib.AscensionTranshumanism) >= MAX_TRANSHUMANISM_LEVEL, limitReached);
        addButton(7, "T-Human.ST", ascensionPerkSelection2, PerkLib.AscensionTranshumanismStr, MAX_TRANSHUMANISM_STR_LEVEL, null, PerkLib.AscensionTranshumanismStr.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionTranshumanismStr) + " / " + MAX_TRANSHUMANISM_STR_LEVEL).disableIf(player.perkv1(PerkLib.AscensionTranshumanismStr) >= MAX_TRANSHUMANISM_STR_LEVEL, limitReached);
        addButton(8, "T-Human.TO", ascensionPerkSelection2, PerkLib.AscensionTranshumanismTou, MAX_TRANSHUMANISM_TOU_LEVEL, null, PerkLib.AscensionTranshumanismTou.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionTranshumanismTou) + " / " + MAX_TRANSHUMANISM_TOU_LEVEL).disableIf(player.perkv1(PerkLib.AscensionTranshumanismTou) >= MAX_TRANSHUMANISM_TOU_LEVEL, limitReached);
        addButton(9, "T-Human.SP", ascensionPerkSelection2, PerkLib.AscensionTranshumanismSpe, MAX_TRANSHUMANISM_SPE_LEVEL, null, PerkLib.AscensionTranshumanismSpe.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionTranshumanismSpe) + " / " + MAX_TRANSHUMANISM_SPE_LEVEL).disableIf(player.perkv1(PerkLib.AscensionTranshumanismSpe) >= MAX_TRANSHUMANISM_SPE_LEVEL, limitReached);
        addButton(10, "T-Human.IN", ascensionPerkSelection2, PerkLib.AscensionTranshumanismInt, MAX_TRANSHUMANISM_INT_LEVEL, null, PerkLib.AscensionTranshumanismInt.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionTranshumanismInt) + " / " + MAX_TRANSHUMANISM_INT_LEVEL).disableIf(player.perkv1(PerkLib.AscensionTranshumanismInt) >= MAX_TRANSHUMANISM_INT_LEVEL, limitReached);
        addButton(11, "T-Human.WI", ascensionPerkSelection2, PerkLib.AscensionTranshumanismWis, MAX_TRANSHUMANISM_WIS_LEVEL, null, PerkLib.AscensionTranshumanismWis.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionTranshumanismWis) + " / " + MAX_TRANSHUMANISM_WIS_LEVEL).disableIf(player.perkv1(PerkLib.AscensionTranshumanismWis) >= MAX_TRANSHUMANISM_WIS_LEVEL, limitReached);
        addButton(12, "T-Human.Li", ascensionPerkSelection2, PerkLib.AscensionTranshumanismLib, MAX_TRANSHUMANISM_LIB_LEVEL, null, PerkLib.AscensionTranshumanismLib.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionTranshumanismLib) + " / " + MAX_TRANSHUMANISM_LIB_LEVEL).disableIf(player.perkv1(PerkLib.AscensionTranshumanismLib) >= MAX_TRANSHUMANISM_LIB_LEVEL, limitReached);
        addButton(13, "T-Human.SE", ascensionPerkSelection2, PerkLib.AscensionTranshumanismSen, MAX_TRANSHUMANISM_SEN_LEVEL, null, PerkLib.AscensionTranshumanismSen.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.AscensionTranshumanismSen) + " / " + MAX_TRANSHUMANISM_SEN_LEVEL).disableIf(player.perkv1(PerkLib.AscensionTranshumanismSen) >= MAX_TRANSHUMANISM_SEN_LEVEL, limitReached);
        if (player.hasPerk(PerkLib.Soulless))
        {
            addButton(14, "Back", darkAscensionMenu);
        }
    ...
    private function maxRankValue() : Float
    {
        var maxV : Float = 0;
        if (!player.hasPerk(PerkLib.AscensionMenuChoiceMaybe))
        {
            maxV += 5;
        }
        if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 1)
        {
            maxV += 5;
        }
        ...
        if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 5)
        {
            maxV += 5;
        }
        if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 6)
        {
            maxV += 5;
        }
        if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 7)
        {
            maxV += 5;
        }
        if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 8)
        {
            maxV += 5;
        }
        if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 9)
        {
            maxV += 5;
        }
        return maxV;
    }
    private function maxRankValue1() : Float
    {
        var maxV : Float = 0;
        if (!player.hasPerk(PerkLib.AscensionMenuChoiceMaybe))
        {
    ...
    private function ascensionPerkSelection(perk : Dynamic = null, maxRank : Int = 10) : Void
    {
        clearOutput();
        maxRank = Math.min(maxRankValue(), maxRank);
        outputText("Perk Effect: " + perk.longDesc);
        outputText("\nCurrent level: " + player.perkv1(perk) + " / " + maxRank + "");
        if (player.perkv1(perk) >= maxRank)
        {
            outputText(" <b>(Maximum)</b>");
        }
        var cost : Int = as3hx.Compat.parseInt(player.perkv1(perk) + 1);
        if (cost > 5)
        {
            if (cost > 15)
            {
        ...
        outputText("\n\nAscension Perk Points: " + player.ascensionPerkPoints);
        menu();
        if (player.ascensionPerkPoints >= cost && player.perkv1(perk) < maxRank)
        {
            addButton(0, "Add 1 level", addAscensionPerk, perk, maxRank);
        }
        if (player.ascensionPerkPoints >= cost && player.perkv1(perk) == maxRank)
        {
            addButtonDisabled(0, "Add 1 level", "You've reached max rank for this perk at current tier of ascension. To unlock higher ranks you need to Ascend again.");
        }
        addButton(4, "Back", ascensionPerkMenu);
    }
    private function addAscensionPerk(perk : Dynamic = null, maxRank : Int = 10) : Void
    {
        var cost : Int = as3hx.Compat.parseInt(player.perkv1(perk) + 1);
        if (cost > 5)
        {
        ...
        {
            player.addPerkValue(perk, 1, 1);
        }
        else
        {
            player.createPerk(perk, 1, 0, 0, 0);
        }
        ascensionPerkSelection(perk, maxRank);
    }
    private function ascensionPerkSelection2(perk : Dynamic = null, maxRank : Int = 10) : Void
    {
        clearOutput();
        maxRank = Math.min(maxRankValue(), maxRank);
        outputText("Perk Effect: " + perk.longDesc);
        ...
        if (player.ascensionPerkPoints >= cost && player.perkv1(perk) < maxRank)
        {
            addButton(0, "Add 1 level", addAscensionPerk2, perk, maxRank);
        }
        if (player.ascensionPerkPoints >= cost && player.perkv1(perk) == maxRank)
        {
            addButtonDisabled(0, "Add 1 level", "You've reached max rank for this perk at current tier of ascension. To unlock higher ranks you need to Ascend again.");
    ...
    private function addAscensionPerk2(perk : Dynamic = null, maxRank : Int = 10) : Void
    {
        var cost : Int = as3hx.Compat.parseInt(player.perkv1(perk) + 1);
        if (cost > 5)
        {
    ...
    private function darkAscensionSpecificPerkSelection(page : Int = 1) : Void
    {
        ...
        if (page == 1)
        {
            addButton(0, "The Dark Soul", darkAscensionPerkSelection2, PerkLib.DarkAscensionTheDarkSoul, MAX_THE_DARK_SOUL, null, PerkLib.DarkAscensionTheDarkSoul.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.DarkAscensionTheDarkSoul) + " / " + MAX_THE_DARK_SOUL).disableIf(player.perkv1(PerkLib.DarkAscensionTheDarkSoul) >= MAX_THE_DARK_SOUL, limitReached);
            addButton(1, "BottomlessHunger", darkAscensionPerkSelection, PerkLib.DarkAscensionBottomlessHunger, MAX_BOTTOMLESS_HUNGER_LEVEL, null, PerkLib.DarkAscensionBottomlessHunger.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.DarkAscensionBottomlessHunger) + " / " + MAX_BOTTOMLESS_HUNGER_LEVEL).disableIf(player.perkv1(PerkLib.DarkAscensionBottomlessHunger) >= MAX_BOTTOMLESS_HUNGER_LEVEL, limitReached);
            addButton(2, "Eff.SoulCons", darkAscensionPerkSelection, PerkLib.DarkAscensionEfficientSoulConsumption, MAX_EFFICIENT_SOUL_CONSUMPTION_LEVEL, null, PerkLib.DarkAscensionEfficientSoulConsumption.longDesc + "\n\nCurrent level: " + player.perkv1(PerkLib.DarkAscensionEfficientSoulConsumption) + " / " + MAX_EFFICIENT_SOUL_CONSUMPTION_LEVEL).disableIf(player.perkv1(PerkLib.DarkAscensionEfficientSoulConsumption) >= MAX_EFFICIENT_SOUL_CONSUMPTION_LEVEL, limitReached);
        }
        addButton(14, "Back", darkAscensionMenu);
    }
    private function darkAscensionPerkSelection(perk : Dynamic = null, maxRank : Int = 10) : Void
    {
        clearOutput();
        maxRank = Math.min(maxRankValue(), maxRank);
        outputText("Perk Effect: " + perk.longDesc);
        ...
        if (player.ascensionPerkPoints >= cost && player.perkv1(perk) < maxRank)
        {
            addButton(0, "Add 1 level", addDarkAscensionPerk, perk, maxRank);
        }
        if (player.ascensionPerkPoints >= cost && player.perkv1(perk) == maxRank)
        {
            addButtonDisabled(0, "Add 1 level", "You've reached max rank for this perk at current tier of dark ascension. To unlock higher ranks you need to Ascend again.");
        }
        addButton(4, "Back", darkAscensionSpecificPerkSelection);
    }
    private function addDarkAscensionPerk(perk : Dynamic = null, maxRank : Int = 10) : Void
    {
        var cost : Int = as3hx.Compat.parseInt(player.perkv1(perk) + 1);
        if (cost > 5)
        {
    ...
    private function darkAscensionPerkSelection2(perk : Dynamic = null, maxRank : Int = 10) : Void
    {
        clearOutput();
        ...
        if (player.ascensionPerkPoints >= cost && player.perkv1(perk) < maxRank)
        {
            addButton(0, "Add 1 level", addDarkAscensionPerk2, perk, maxRank);
        }
        if (player.ascensionPerkPoints >= cost && player.perkv1(perk) == maxRank)
        {
            addButtonDisabled(0, "Add 1 level", "You've reached max rank for this perk at current tier of dark ascension. To unlock higher ranks you need to Ascend again.");
    ...
    private function addDarkAscensionPerk2(perk : Dynamic = null, maxRank : Int = 10) : Void
    {
        var cost : Int = as3hx.Compat.parseInt(player.perkv1(perk) + 1);
        if (cost > 5)
        {
    ...
    private function rarePerks1() : Void
    {
        ...
        {
            if (player.ascensionPerkPoints >= 75 && !player.hasPerk(PerkLib.AscensionBloodlineHeritage))
            {
                addButton(btn, "BloodHeritage", perkBloodlineHeritage).hint("Perk giving you an additional 1 perk point, 1 super perk point and 5 stat points at the start of the game (scaling with current NG tier, for super perk points amount is reduced by 3). Also would increase any bloodline perk bonus by 1.\n\nCost: 75 points");
            }
            else if (player.ascensionPerkPoints < 75)
            {
                button(btn).disable("You do not have enough ascension perk points!");
            }
            else
            {
                addButtonDisabled(btn, "BloodHeritage", "You already bought Bloodline Heritage perk.");
            }
        }
        else if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 1 && !player.hasPerk(PerkLib.AscensionHerosLegacy))
        {
            addButtonDisabled(btn, "BloodHeritage", "You need to buy Hero's Legacy perk first.");
        }
        else
        {
            addButtonDisabled(btn, "BloodHeritage", "You need ascend more times to buy this perk.");
        }
        btn++;
        ...
        {
            if (player.ascensionPerkPoints >= 20 && !player.hasPerk(PerkLib.AscensionCruelChimerasThesis))
            {
                addButton(btn, "C Chimera's T", perkCruelChimerasThesis).hint("Perk allowing you to receive race bonuses for one point less. (still req. min 8 race points to work).\n\nCost: 20 points");
            }
            else if (player.ascensionPerkPoints < 20)
            {
                addButtonDisabled(btn, "C Chimera's T", "You do not have enough ascension perk points!");
            }
            else
            {
                addButtonDisabled(btn, "C Chimera's T", "You already bought Cruel Chimera's Thesis perk.");
            }
        }
        else if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 1 && !player.hasPerk(PerkLib.AscensionHybridTheory))
        {
            addButtonDisabled(btn, "C Chimera's T", "You need to buy Hybrid Theory perk first.");
        }
        else
        {
            addButtonDisabled(btn, "C Chimera's T", "You need ascend more times to buy this perk.");
        }
        btn++;
        if (player.ascensionPerkPoints >= 5 && !player.hasPerk(PerkLib.AscensionHerosHeritage))
        {
            addButton(btn, "HeroHeritage", perkHerosHeritage).hint("Perk giving you an additional 3 perk points and 15 stat points at the start of the game (scaling with current NG tier).\n\nCost: 5 points");
        }
        else if (player.ascensionPerkPoints < 5 && !player.hasPerk(PerkLib.AscensionHerosHeritage))
        {
            addButtonDisabled(btn, "HeroHeritage", "You do not have enough ascension perk points!");
        }
        else
        {
            addButtonDisabled(btn, "HeroHeritage", "You already bought Hero's Heritage perk.");
        }
        btn++;
        if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 1 && player.hasPerk(PerkLib.AscensionHerosHeritage))
        {
            if (player.ascensionPerkPoints >= 10 && !player.hasPerk(PerkLib.AscensionHerosLineage))
            {
                addButton(btn, "HeroLineage", perkHerosLineage).hint("Perk giving you an additional 2 perk points and 10 stat points at the start of the game (scaling with current NG tier).\n\nCost: 10 points");
            }
            else if (player.ascensionPerkPoints < 10)
            {
                button(btn).disable("You do not have enough ascension perk points!");
            }
            else
            {
                addButtonDisabled(btn, "HeroLineage", "You already bought Hero's Lineage perk.");
            }
        }
        else if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 1 && !player.hasPerk(PerkLib.AscensionHerosHeritage))
        {
            addButtonDisabled(btn, "HeroLineage", "You need to buy Hero's Heritage perk first.");
        }
        else
        {
            addButtonDisabled(btn, "HeroLineage", "You need ascend more times to buy this perk.");
        }
        btn++;
        if (player.ascensionPerkPoints >= 10 && !player.hasPerk(PerkLib.AscensionHybridTheory))
        {
            addButton(btn, "HybridTheory", perkHybridTheory).hint("Perk allowing you to receive race bonuses for one point less. (still req. min 4 race points to work).\n\nCost: 10 points");
        }
        else if (player.ascensionPerkPoints < 10 && !player.hasPerk(PerkLib.AscensionHybridTheory))
        {
            addButtonDisabled(btn, "HybridTheory", "You do not have enough ascension perk points!");
        }
        else
        {
            addButtonDisabled(btn, "HybridTheory", "You already bought Hybrid Theory perk.");
        }
        btn++;
        ...
        {
            if (player.ascensionPerkPoints >= 25 && !player.hasPerk(PerkLib.AscensionHerosLegacy))
            {
                addButton(btn, "HeroLegacy", perkHerosLegacy).hint("Perk giving you an additional 1 perk point, 1 super perk point and 5 stat points at the start of the game (scaling with current NG tier, for super perk points amount is reduced by 2).\n\nCost: 25 points");
            }
            else if (player.ascensionPerkPoints < 25)
            {
                button(btn).disable("You do not have enough ascension perk points!");
            }
            else
            {
                addButtonDisabled(btn, "HeroLegacy", "You already bought Hero's Legacy perk.");
            }
        }
        else if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 2 && !player.hasPerk(PerkLib.AscensionHerosLineage))
        {
            addButtonDisabled(btn, "HeroLegacy", "You need to buy Hero's Lineage perk first.");
        }
        else
        {
            addButtonDisabled(btn, "HeroLegacy", "You need ascend more times to buy this perk.");
        }
        btn++;
        addButton(14, "Back", ascensionMenu);
    }
    
    private function whichNewGameAreYouOn() : Float
    {
        var wNGAYO : Float = flags[kFLAGS.NEW_GAME_PLUS_LEVEL];
        if (!player.hasPerk(PerkLib.AscensionMenuChoiceMaybe))
        {
            wNGAYO += 1;
        }
    ...
    {
        var NGPL : Array<Dynamic> = [1, 3, 5, 7];
        var pCost : Int = 20;
        if (tier > 5)
        {
            addButtonDisabled(btn, "A.O.M. Rank " + Std.string(tier - 1), "You have the highest tier already.");
        }
        else if (whichNewGameAreYouOn() < NGPL[tier - 1])
        {
            addButtonDisabled(btn, "A.O.M. Rank " + Std.string(tier), "You need to ascend a few more times.");
        }
        else if (player.internalChimeraScore() < 10 * tier)
        {
            addButtonDisabled(btn, "A.O.M. Rank " + Std.string(tier), "You do not have a high enough chimera score.");
        }
        else if (player.ascensionPerkPoints < pCost * tier)
        {
            addButtonDisabled(btn, "A.O.M. Rank " + Std.string(tier), "You do not have enough points.");
        }
        else
        {
            addButton(btn, "A.O.M. Tier " + Std.string(tier), perkRPConfirm, tier, PerkLib.AscensionAdditionalOrganMutationX, pCost, "Acquire A.O.M. Prestige Rank " + Std.string(tier));
        }
    }
    ...
    private function perkBPCheck(tier : Int, btn : Int) : Void
    {
        ...
        if (tier > 6)
        {
            addButtonDisabled(btn, "B.Prestige. Rank " + Std.string(tier - 1), "You have the highest tier already.");
        }
        else if (whichNewGameAreYouOn() < tier)
        {
            addButtonDisabled(btn, "B.Prestige. Rank " + Std.string(tier), "You need to ascend once more.");
        }
        else if (player.ascensionPerkPoints < pCost * tier)
        {
            addButtonDisabled(btn, "B.Prestige. Rank " + Std.string(tier), "You do not have enough points.");
        }
        else
        {
            addButton(btn, "B. Prestige Rank" + Std.string(tier), perkRPConfirm, tier, PerkLib.AscensionBuildingPrestigeX, pCost, "Acquire Building Prestige Rank " + Std.string(tier));
        }
    }
    ...
    private function perkAdvancedTrainingCheck(tier : Int, btn : Int) : Void
    {
        ...
        if (tier > 5)
        {
            addButtonDisabled(btn, "Adv. Training Rank " + Std.string(tier - 1), "You have the highest tier already.");
        }
        else if (whichNewGameAreYouOn() < tier)
        {
            addButtonDisabled(btn, "Adv. Training  Rank " + Std.string(tier), "You need to ascend once more.");
        }
        else if (player.ascensionPerkPoints < pCost * tier)
        {
            addButtonDisabled(btn, "Adv. Training  Rank " + Std.string(tier), "You do not have enough points.");
        }
        else
        {
            addButton(btn, "Adv. Training  Rank" + Std.string(tier), perkRPConfirm, tier, PerkLib.AscensionAdvTrainingX, pCost, "Acquire Advanced Training Rank " + Std.string(tier));
        }
    }
    ...
    private function perkOneRaceToRuleThemAllCheck(tier : Int, btn : Int) : Void
    {
        ...
        if (tier > 5)
        {
            addButtonDisabled(btn, "ORTRTA Rank " + Std.string(tier - 1), "You have the highest tier already.");
        }
        else if (whichNewGameAreYouOn() < tier)
        {
            addButtonDisabled(btn, "ORTRTA Rank " + Std.string(tier), "You need to ascend once more.");
        }
        else if (player.ascensionPerkPoints < pCost * tier)
        {
            addButtonDisabled(btn, "ORTRTA Rank " + Std.string(tier), "You do not have enough points.");
        }
        else
        {
            addButton(btn, "ORTRTA Rank" + Std.string(tier), perkRPConfirm, tier, PerkLib.AscensionOneRaceToRuleThemAllX, pCost, 
                    "Acquire One Race To Rule Them All Rank " + Std.string(tier) + ".\n\nIncreases the number of stat points earned per level, and racial skill power.\n" + "Cost: " + tier * pCost
            );
        }
    }
    ...
    private function perkHerosBirthrightCheck(tier : Int, btn : Int) : Void
    {
        ...
        if (tier > 6)
        {
            addButtonDisabled(btn, "HBirthright R " + Std.string(tier - 1), "You have the highest tier already.");
        }
        else if (whichNewGameAreYouOn() < tier)
        {
            addButtonDisabled(btn, "HBirthright R " + Std.string(tier), "You need to ascend once more.");
        }
        else if (player.ascensionPerkPoints < pCost * tier)
        {
            addButtonDisabled(btn, "HBirthright R " + Std.string(tier), "You do not have enough points.");
        }
        else
        {
            addButton(btn, "HBirthright R " + Std.string(tier), perkRPConfirm, tier, PerkLib.AscensionHerosBirthrightRankX, pCost, 
                    "Acquire Hero's Birthright Rank " + Std.string(tier) + ".\n\nReduces the level needed to equip legendary items by 9.\n" + "Cost: " + tier * pCost
            );
        }
    }
    ...
    private function perkRPConfirm(tier : Int, perk : PerkType, pCost : Int, RPP : Int = 1) : Void
    {
        ...
        if (tier == 1)
        {
            player.createPerk(perk, 1, 0, 0, 1);
        }
        else
        {
            player.setPerkValue(perk, 1, player.perkv1(perk) + 1);
        }
        clearOutput();
    ...
    private function rarePerks2() : Void
    {
        ...
        btn++;
        if (player.ascensionPerkPoints >= 5 && !player.hasPerk(PerkLib.AscensionUnderdog))
        {
            addButton(btn, "Underdog", perkUnderdog).hint("Perk allowing you to double base exp gains for fighting enemies above PC level, increasing max lvl diff when bonus is in effect will still increase from 20 to 40 above current PC lvl.\n\nCost: 5 points");
        }
        // And... to live up to underdog role PC will 'accidentally' find few places to further power-up.
        else if (player.ascensionPerkPoints < 5 && !player.hasPerk(PerkLib.AscensionUnderdog))
        {
            addButtonDisabled(btn, "Underdog", "You do not have enough ascension perk points!");
        }
        else
        {
            addButtonDisabled(btn, "Underdog", "You already bought Underdog perk.");
        }
        btn++;
        if (player.ascensionPerkPoints >= 5 && !player.hasPerk(PerkLib.AscensionUnlockedPotential))
        {
            addButton(btn, "UnlockPotent", perkUnlockedPotential).hint("Perk allowing you to have increased passive gains of max hp, lust and fatigue at each lvl-up.\n\nCost: 5 points");
        }
        else if (player.ascensionPerkPoints < 5 && !player.hasPerk(PerkLib.AscensionUnlockedPotential))
        {
            addButtonDisabled(btn, "UnlockPotent", "You do not have enough ascension perk points!");
        }
        else
        {
            addButtonDisabled(btn, "UnlockPotent", "You already bought Unlocked Potential perk.");
        }
        btn++;
        if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 1 && player.hasPerk(PerkLib.AscensionUnlockedPotential))
        {
            if (player.ascensionPerkPoints >= 5 && !player.hasPerk(PerkLib.AscensionUnlockedPotential2ndStage))
            {
                addButton(btn, "U.Potent2nd", perkUnlockedPotential2ndStage).hint("Perk allowing you to have increased passive gains of max wrath, mana and soulforce at each lvl-up.\n\nCost: 5 points");
            }
            else if (player.ascensionPerkPoints < 5)
            {
                addButtonDisabled(btn, "U.Potent2nd", "You do not have enough ascension perk points!");
            }
            else
            {
                addButtonDisabled(btn, "U.Potent2nd", "You already bought Unlocked Potential (2nd Stage) perk.");
            }
        }
        else if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 1 && !player.hasPerk(PerkLib.AscensionUnlockedPotential))
        {
            addButtonDisabled(btn, "U.Potent2nd", "You need to buy Unlocked Potential perk first.");
        }
        else
        {
            addButtonDisabled(btn, "U.Potent2nd", "You need ascend more times to buy this perk.");
        }
        btn++;
        if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 2 && player.hasPerk(PerkLib.AscensionUnlockedPotential2ndStage))
        {
            if (player.ascensionPerkPoints >= 10 && !player.hasPerk(PerkLib.AscensionUnlockedPotential3rdStage))
            {
                addButton(btn, "U.Potent3rd", perkUnlockedPotential3rdStage).hint("Perk allowing you to have increased passive gains of max hp, lust and fatigue at each lvl-up.\n\nCost: 10 points");
            }
            else if (player.ascensionPerkPoints < 10)
            {
                addButtonDisabled(btn, "U.Potent3rd", "You do not have enough ascension perk points!");
            }
            else
            {
                addButtonDisabled(btn, "U.Potent3rd", "You already bought Unlocked Potential (3rd Stage) perk.");
            }
        }
        else if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 2 && !player.hasPerk(PerkLib.AscensionUnlockedPotential3rdStage))
        {
            addButtonDisabled(btn, "U.Potent3rd", "You need to buy Unlocked Potential (2nd Stage) perk first.");
        }
        else
        {
            addButtonDisabled(btn, "U.Potent3rd", "You need ascend more times to buy this perk.");
        }
        btn++;
        if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 3 && player.hasPerk(PerkLib.AscensionUnlockedPotential3rdStage))
        {
            if (player.ascensionPerkPoints >= 10 && !player.hasPerk(PerkLib.AscensionUnlockedPotential4thStage))
            {
                addButton(btn, "U.Potent4th", perkUnlockedPotential4thStage).hint("Perk allowing you to have increased passive gains of max wrath, mana and soulforce at each lvl-up.\n\nCost: 10 points");
            }
            else if (player.ascensionPerkPoints < 10)
            {
                addButtonDisabled(btn, "U.Potent4th", "You do not have enough ascension perk points!");
            }
            else
            {
                addButtonDisabled(btn, "U.Potent4th", "You already bought Unlocked Potential (4th Stage) perk.");
            }
        }
        else if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 3 && !player.hasPerk(PerkLib.AscensionUnlockedPotential4thStage))
        {
            addButtonDisabled(btn, "U.Potent4th", "You need to buy Unlocked Potential (3rd Stage) perk first.");
        }
        else
        {
            addButtonDisabled(btn, "U.Potent4th", "You need ascend more times to buy this perk.");
        }
        btn++;
        if (!player.hasPerk(PerkLib.Soulless))
        {
            if (player.ascensionPerkPoints >= 21 && !player.hasPerk(PerkLib.AscensionSkyPoisonPearlMasteryStageX))
            {
                addButton(btn, "SPPearlMst1", perkSkyPoisonPearlMasteryStage1).hint("Perk allowing you to have increased venom recharge, max venom, poison resistance, amount of carried over spirit stones and unlock next sections of sky poison pearl 6 levels earlier.\n\nCost: 21 points");
            }
            else if (player.ascensionPerkPoints < 21 && !player.hasPerk(PerkLib.AscensionSkyPoisonPearlMasteryStageX))
            {
                addButtonDisabled(btn, "SPPearlMst1", "You do not have enough ascension perk points!");
            }
            else
            {
                addButtonDisabled(btn, "SPPearlMst1", "You already bought Sky Poison Pearl Mastery 1 perk.");
            }
            btn++;
            ...
            {
                if (player.ascensionPerkPoints >= 57 && player.perkv1(PerkLib.AscensionSkyPoisonPearlMasteryStageX) == 1)
                {
                    addButton(btn, "SPPearlMst2", perkSkyPoisonPearlMasteryStage2).hint("Perk allowing you to have increased venom recharge, max venom, poison resistance, amount of carried over spirit stones and unlock next sections of sky poison pearl 12 levels earlier.\n\nCost: 57 points");
                }
                else if (player.ascensionPerkPoints < 57)
                {
                    addButtonDisabled(btn, "SPPearlMst2", "You do not have enough ascension perk points!");
                }
                else if (player.perkv1(PerkLib.AscensionSkyPoisonPearlMasteryStageX) > 1)
                {
                    addButtonDisabled(btn, "SPPearlMst2", "You already bought Sky Poison Pearl Mastery 2 perk.");
                }
            }
            else
            {
                addButtonDisabled(btn, "SPPearlMst2", "You need to buy Sky Poison Pearl Mastery 1 perk first.");
            }
            btn++;
            ...
            {
                if (player.ascensionPerkPoints >= 93 && player.perkv1(PerkLib.AscensionSkyPoisonPearlMasteryStageX) == 2)
                {
                    addButton(btn, "SPPearlMst3", perkSkyPoisonPearlMasteryStage3).hint("Perk allowing you to have increased venom recharge, max venom, poison resistance, amount of carried over spirit stones and unlock next sections of sky poison pearl 18 levels earlier.\n\nCost: 93 points");
                }
                else if (player.ascensionPerkPoints < 93)
                {
                    addButtonDisabled(btn, "SPPearlMst3", "You do not have enough ascension perk points!");
                }
                else if (player.perkv1(PerkLib.AscensionSkyPoisonPearlMasteryStageX) > 2)
                {
                    addButtonDisabled(btn, "SPPearlMst3", "You already bought Sky Poison Pearl Mastery 3 perk.");
                }
            }
            else
            {
                addButtonDisabled(btn, "SPPearlMst3", "You need to buy Sky Poison Pearl Mastery 2 perk first.");
            }
            btn++;
            ...
            {
                if (player.ascensionPerkPoints >= 129 && player.perkv1(PerkLib.AscensionSkyPoisonPearlMasteryStageX) == 3)
                {
                    addButton(btn, "SPPearlMst4", perkSkyPoisonPearlMasteryStage4).hint("Perk allowing you to have increased venom recharge, max venom, poison resistance, amount of carried over spirit stones and unlock next sections of sky poison pearl 24 levels earlier.\n\nCost: 129 points");
                }
                else if (player.ascensionPerkPoints < 129)
                {
                    addButtonDisabled(btn, "SPPearlMst4", "You do not have enough ascension perk points!");
                }
                else if (player.perkv1(PerkLib.AscensionSkyPoisonPearlMasteryStageX) > 3)
                {
                    addButtonDisabled(btn, "SPPearlMst4", "You already bought Sky Poison Pearl Mastery 4 perk.");
                }
            }
            else
            {
                addButtonDisabled(btn, "SPPearlMst4", "You need to buy Sky Poison Pearl Mastery 3 perk first.");
            }
            btn++;
            ...
            {
                if (player.ascensionPerkPoints >= 165 && player.perkv1(PerkLib.AscensionSkyPoisonPearlMasteryStageX) == 4)
                {
                    addButton(btn, "SPPearlMst5", perkSkyPoisonPearlMasteryStage5).hint("Perk allowing you to have increased venom recharge, max venom, poison resistance, amount of carried over spirit stones and unlock next sections of sky poison pearl 30 levels earlier.\n\nCost: 129 points");
                }
                else if (player.ascensionPerkPoints < 165)
                {
                    addButtonDisabled(btn, "SPPearlMst5", "You do not have enough ascension perk points!");
                }
                else if (player.perkv1(PerkLib.AscensionSkyPoisonPearlMasteryStageX) > 4)
                {
                    addButtonDisabled(btn, "SPPearlMst5", "You already bought Sky Poison Pearl Mastery 5 perk.");
                }
            }
            else
            {
                addButtonDisabled(btn, "SPPearlMst5", "You need to buy Sky Poison Pearl Mastery 4 perk first.");
            }
            btn++;
            ...
            {
                if (player.ascensionPerkPoints >= 201 && player.perkv1(PerkLib.AscensionSkyPoisonPearlMasteryStageX) == 5)
                {
                    addButton(btn, "SPPearlMst6", perkSkyPoisonPearlMasteryStage6).hint("Perk allowing you to have increased venom recharge, max venom, poison resistance, amount of carried over spirit stones and unlock next sections of sky poison pearl 36 levels earlier.\n\nCost: 129 points");
                }
                else if (player.ascensionPerkPoints < 201)
                {
                    addButtonDisabled(btn, "SPPearlMst6", "You do not have enough ascension perk points!");
                }
                else if (player.perkv1(PerkLib.AscensionSkyPoisonPearlMasteryStageX) > 5)
                {
                    addButtonDisabled(btn, "SPPearlMst5", "You already bought Sky Poison Pearl Mastery 6 perk.");
                }
            }
            else
            {
                addButtonDisabled(btn, "SPPearlMst6", "You need to buy Sky Poison Pearl Mastery 5 perk first.");
            }
            btn++;
        }
    ...
    private function perkMetamorphAscCheck(btn : Int) : Void
    {
        if (!player.hasPerk(PerkLib.AscensionNaturalMetamorph))
        {
            ...
            {
                addButton(btn, "Nat.MetaMph", curry(perkRPConfirm, 1, PerkLib.AscensionNaturalMetamorph, 30, 2)).hint("Gain free perks: Genetic Memory, Metamorph, MetamorphEx at the start of the game\n\n"
                        + "Cost: 30 points.");
            }
        }
        else
        {
            ...
            if (!player.hasPerk(PerkLib.AscensionTrancendentalGeneticMemoryStageX))
            {
                ...
                {
                    addButton(btn, "Gen. Memory", curry(perkRPConfirm, 1, PerkLib.AscensionTrancendentalGeneticMemoryStageX, pCost, 2)).hint("Unlocks an ascension menu to make some metamorphs permanent and have them unlocked in your next runs.\n\n"
                            + "Cost: "
                            + pCost * 1
                            + " points.");
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    addButton(btn, "Gen. Memory", curry(perkRPConfirm, tier, PerkLib.AscensionTrancendentalGeneticMemoryStageX, pCost, 2)).hint("Increases the maximum number of permed metamorphs."
                            + "Cost: "
                            + pCost * tier
                            + " points.");
                }
    ...
    private function acensionPermSpellMenu(page : Int = 1) : Void
    {
        ...
        if (page == 1)
        {
            if (player.hasStatusEffect(StatusEffects.KnowsPolarMidnight) && player.statusEffectv4(StatusEffects.KnowsPolarMidnight) != 9000)
            {
                addButton(0, "Polar Midnight", permanentizeSpell, StatusEffects.KnowsPolarMidnight, 1);
            }
            else if (player.hasStatusEffect(StatusEffects.KnowsPolarMidnight) && player.statusEffectv4(StatusEffects.KnowsPolarMidnight) == 9000)
            {
                addButtonDisabled(0, "Polar Midnight", "Polar Midnight spell is already permanent.");
            }
            else
            {
                addButtonDisabled(0, "???", "You haven't learnt this spell yet!");
            }
            if (player.hasStatusEffect(StatusEffects.KnowsMeteorShower) && player.statusEffectv4(StatusEffects.KnowsMeteorShower) != 9000)
            {
                addButton(1, "Meteor Shower", permanentizeSpell, StatusEffects.KnowsMeteorShower, 1);
            }
            else if (player.hasStatusEffect(StatusEffects.KnowsMeteorShower) && player.statusEffectv4(StatusEffects.KnowsMeteorShower) == 9000)
            {
                addButtonDisabled(1, "Meteor Shower", "Meteor Shower spell is already permanent.");
            }
            else
            {
                addButtonDisabled(1, "???", "You haven't learnt this spell yet!");
            }
        }
        if (player.hasPerk(PerkLib.Soulless))
        {
    ...
    private function ascensionPermeryMenu(page : Int = 1) : Void
    {
        ...
        if (page == 1)
        {
            if (player.hasPerk(PerkLib.CorruptedKitsune) && player.perkv4(PerkLib.CorruptedKitsune) < 1 && player.hasPerk(PerkLib.CorruptedNinetails) && player.perkv4(PerkLib.CorruptedNinetails) < 1)
            {
                addButton(0, "CorruptedK", permanentizePerkCorruptedKitsuneAndNinetails);
            }
            else if (player.hasPerk(PerkLib.CorruptedKitsune) && player.perkv4(PerkLib.CorruptedKitsune) > 0 && player.hasPerk(PerkLib.CorruptedNinetails) && player.perkv4(PerkLib.CorruptedNinetails) > 0)
            {
                addButtonDisabled(0, "CorruptedK", "Corrupted Kitsune and Corrupted Ninetails perks are already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(0, "CorruptedK", "Corrupted Kitsune and Corrupted Ninetails");
            }
            if (player.hasPerk(PerkLib.DarkCharm) && player.perkv4(PerkLib.DarkCharm) < 1)
            {
                addButton(1, "DarkCharm", permanentizePerk1, PerkLib.DarkCharm);
            }
            else if (player.hasPerk(PerkLib.DarkCharm) && player.perkv4(PerkLib.DarkCharm) > 0)
            {
                addButtonDisabled(1, "DarkCharm", "Dark Charm perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(1, "DarkCharm", "Dark Charm");
            }
            if (player.hasPerk(PerkLib.DragonDarknessBreath) && player.perkv4(PerkLib.DragonDarknessBreath) < 1)
            {
                addButton(2, "D.DarknessB.", permanentizePerk1, PerkLib.DragonDarknessBreath);
            }
            else if (player.hasPerk(PerkLib.DragonDarknessBreath) && player.perkv4(PerkLib.DragonDarknessBreath) > 0)
            {
                addButtonDisabled(2, "D.DarknessB.", "Dragon Darkness Breath perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(2, "D.DarknessB.", "Dragon Darkness Breath");
            }
            //3 DragonEarthBreath
            if (player.hasPerk(PerkLib.DragonFaerieBreath) && player.perkv4(PerkLib.DragonFaerieBreath) < 1)
            {
                addButton(3, "D.FaerieB.", permanentizePerk1, PerkLib.DragonFaerieBreath);
            }
            else if (player.hasPerk(PerkLib.DragonFaerieBreath) && player.perkv4(PerkLib.DragonFaerieBreath) > 0)
            {
                addButtonDisabled(3, "D.FaerieB.", "Dragon Faerie Breath perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(3, "D.FaerieB.", "Dragon Faerie Breath");
            }
            addButton(4, "Next", ascensionPermeryMenu, page + 1);
            if (player.hasPerk(PerkLib.DragonFireBreath) && player.perkv4(PerkLib.DragonFireBreath) < 1)
            {
                addButton(5, "D.FireB.", permanentizePerk1, PerkLib.DragonFireBreath);
            }
            else if (player.hasPerk(PerkLib.DragonFireBreath) && player.perkv4(PerkLib.DragonFireBreath) > 0)
            {
                addButtonDisabled(5, "D.FireB.", "Dragon Fire Breath perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(5, "D.FireB.", "Dragon Fire Breath");
            }
            if (player.hasPerk(PerkLib.DragonIceBreath) && player.perkv4(PerkLib.DragonIceBreath) < 1)
            {
                addButton(6, "D.IceB.", permanentizePerk1, PerkLib.DragonIceBreath);
            }
            else if (player.hasPerk(PerkLib.DragonIceBreath) && player.perkv4(PerkLib.DragonIceBreath) > 0)
            {
                addButtonDisabled(6, "D.IceB.", "Dragon Ice Breath perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(6, "D.IceB.", "Dragon Ice Breath");
            }
            if (player.hasPerk(PerkLib.DragonLightningBreath) && player.perkv4(PerkLib.DragonLightningBreath) < 1)
            {
                addButton(7, "D.LightningB.", permanentizePerk1, PerkLib.DragonLightningBreath);
            }
            else if (player.hasPerk(PerkLib.DragonLightningBreath) && player.perkv4(PerkLib.DragonLightningBreath) > 0)
            {
                addButtonDisabled(7, "D.LightningB.", "Dragon Lightning Breath perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(7, "D.LightningB.", "Dragon Lightning Breath");
            }
            if (player.hasPerk(PerkLib.DragonLustPoisonBreath) && player.perkv4(PerkLib.DragonLustPoisonBreath) < 1)
            {
                addButton(8, "D.LustPoisonB.", permanentizePerk1, PerkLib.DragonLustPoisonBreath);
            }
            else if (player.hasPerk(PerkLib.DragonLustPoisonBreath) && player.perkv4(PerkLib.DragonLustPoisonBreath) > 0)
            {
                addButtonDisabled(8, "D.LustPoisonB.", "Dragon Lust Poison Breath perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(8, "D.LustPoisonB.", "Dragon Lust Poison Breath");
            }
            ...
            if (player.hasPerk(PerkLib.DragonPoisonousSapBreath) && player.perkv4(PerkLib.DragonPoisonousSapBreath) < 1)
            {
                addButton(10, "D.PoisonSapB.", permanentizePerk1, PerkLib.DragonPoisonousSapBreath);
            }
            else if (player.hasPerk(PerkLib.DragonPoisonousSapBreath) && player.perkv4(PerkLib.DragonPoisonousSapBreath) > 0)
            {
                addButtonDisabled(10, "D.PoisonSapB.", "Dragon Poisonous Sap Breath perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(10, "D.PoisonSapB.", "Dragon Poisonous Sap Breath");
            }
            if (player.hasPerk(PerkLib.DragonRegalBreath) && player.perkv4(PerkLib.DragonRegalBreath) < 1)
            {
                addButton(11, "D.RegalB.", permanentizePerk1, PerkLib.DragonRegalBreath);
            }
            else if (player.hasPerk(PerkLib.DragonRegalBreath) && player.perkv4(PerkLib.DragonRegalBreath) > 0)
            {
                addButtonDisabled(11, "D.RegalB.", "Dragon Regal Breath perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(11, "D.RoyalB.", "Dragon Royal Breath");
            }
            if (player.hasPerk(PerkLib.DragonWaterBreath) && player.perkv4(PerkLib.DragonWaterBreath) < 1)
            {
                addButton(12, "D.WaterB.", permanentizePerk1, PerkLib.DragonWaterBreath);
            }
            else if (player.hasPerk(PerkLib.DragonWaterBreath) && player.perkv4(PerkLib.DragonWaterBreath) > 0)
            {
                addButtonDisabled(12, "D.WaterB.", "Dragon Water Breath perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(12, "D.WaterB.", "Dragon Water Breath");
            }
            if (player.hasPerk(PerkLib.EnlightenedKitsune) && player.perkv4(PerkLib.EnlightenedKitsune) < 1 && player.hasPerk(PerkLib.EnlightenedNinetails) && player.perkv4(PerkLib.EnlightenedNinetails) < 1)
            {
                addButton(13, "EnlightenedK", permanentizePerkEnlightenedKitsuneAndNinetails);
            }
            else if (player.hasPerk(PerkLib.EnlightenedKitsune) && player.perkv4(PerkLib.EnlightenedKitsune) > 0 && player.hasPerk(PerkLib.EnlightenedNinetails) && player.perkv4(PerkLib.EnlightenedNinetails) > 0)
            {
                addButtonDisabled(13, "EnlightenedK", "Enlightened Kitsune and Enlightened Ninetails perks are already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(13, "EnlightenedK", "Enlightened Kitsune and Enlightened Ninetails");
            }
        }
        if (page == 2)
        {
            if (player.hasPerk(PerkLib.FerasBoonAlpha) && player.perkv4(PerkLib.FerasBoonAlpha) < 1)
            {
                addButton(0, "FerasB.A.", permanentizePerk2, PerkLib.FerasBoonAlpha);
            }
            else if (player.hasPerk(PerkLib.FerasBoonAlpha) && player.perkv4(PerkLib.FerasBoonAlpha) > 0)
            {
                addButtonDisabled(0, "FerasB.A.", "Feras Boon Alpha perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(0, "FerasB.A.", "Feras Boon Alpha");
            }
            if (player.hasPerk(PerkLib.FerasBoonBreedingBitch) && player.perkv4(PerkLib.FerasBoonBreedingBitch) < 1)
            {
                addButton(1, "FerasB.B.B.", permanentizePerk2, PerkLib.FerasBoonBreedingBitch);
            }
            else if (player.hasPerk(PerkLib.FerasBoonBreedingBitch) && player.perkv4(PerkLib.FerasBoonBreedingBitch) > 0)
            {
                addButtonDisabled(1, "FerasB.B.B.", "Feras Boon Breeding Bitch perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(1, "FerasB.B.B.", "Feras Boon Breeding Bitch");
            }
            if (player.hasPerk(PerkLib.FerasBoonMilkingTwat) && player.perkv4(PerkLib.FerasBoonMilkingTwat) < 1)
            {
                addButton(2, "FerasB.M.T.", permanentizePerk2, PerkLib.FerasBoonMilkingTwat);
            }
            else if (player.hasPerk(PerkLib.FerasBoonMilkingTwat) && player.perkv4(PerkLib.FerasBoonMilkingTwat) > 0)
            {
                addButtonDisabled(2, "FerasB.M.T.", "Feras Boon Milking Twat perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(2, "FerasB.M.T.", "Feras Boon Milking Twat");
            }
            if (player.hasPerk(PerkLib.FerasBoonSeeder) && player.perkv4(PerkLib.FerasBoonSeeder) < 1)
            {
                addButton(3, "FerasB.S.", permanentizePerk2, PerkLib.FerasBoonSeeder);
            }
            else if (player.hasPerk(PerkLib.FerasBoonSeeder) && player.perkv4(PerkLib.FerasBoonSeeder) > 0)
            {
                addButtonDisabled(3, "FerasB.S.", "Feras Boon Seeder perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(3, "FerasB.S.", "Feras Boon Seeder");
            }
            addButton(4, "Next", ascensionPermeryMenu, page + 1);
            if (player.hasPerk(PerkLib.FerasBoonWideOpen) && player.perkv4(PerkLib.FerasBoonWideOpen) < 1)
            {
                addButton(5, "FerasB.W.O.", permanentizePerk2, PerkLib.FerasBoonWideOpen);
            }
            else if (player.hasPerk(PerkLib.FerasBoonWideOpen) && player.perkv4(PerkLib.FerasBoonWideOpen) > 0)
            {
                addButtonDisabled(5, "FerasB.W.O.", "Feras Boon Wide Open perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(5, "FerasB.W.O.", "Feras Boon Wide Open");
            }
            if (player.hasPerk(PerkLib.FireLord) && player.perkv4(PerkLib.FireLord) < 1)
            {
                addButton(6, "FireLord", permanentizePerk2, PerkLib.FireLord);
            }
            else if (player.hasPerk(PerkLib.FireLord) && player.perkv4(PerkLib.FireLord) > 0)
            {
                addButtonDisabled(6, "FireLord", "FireLord perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(6, "FireLord", "FireLord");
            }
            if (player.hasPerk(PerkLib.Flexibility) && player.perkv4(PerkLib.Flexibility) < 1)
            {
                addButton(7, "Flexibility", permanentizePerk2, PerkLib.Flexibility);
            }
            else if (player.hasPerk(PerkLib.Flexibility) && player.perkv4(PerkLib.Flexibility) > 0)
            {
                addButtonDisabled(7, "Flexibility", "Flexibility perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(7, "Flexibility", "Flexibility");
            }
            if (player.hasPerk(PerkLib.Hellfire) && player.perkv4(PerkLib.Hellfire) < 1)
            {
                addButton(8, "Hellfire", permanentizePerk2, PerkLib.Hellfire);
            }
            else if (player.hasPerk(PerkLib.Hellfire) && player.perkv4(PerkLib.Hellfire) > 0)
            {
                addButtonDisabled(8, "Hellfire", "Hellfire perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(8, "Hellfire", "Hellfire");
            }
            addButton(9, "Previous", ascensionPermeryMenu, page - 1);
            if (player.hasPerk(PerkLib.InkSpray) && player.perkv4(PerkLib.InkSpray) < 1)
            {
                addButton(10, "InkSpray", permanentizePerk2, PerkLib.InkSpray);
            }
            else if (player.hasPerk(PerkLib.InkSpray) && player.perkv4(PerkLib.InkSpray) > 0)
            {
                addButtonDisabled(10, "InkSpray", "Ink Spray perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(10, "InkSpray", "Ink Spray");
            }
            if (player.hasPerk(PerkLib.LizanRegeneration) && player.perkv4(PerkLib.LizanRegeneration) < 1)
            {
                addButton(11, "LizanRegen", permanentizePerk2, PerkLib.LizanRegeneration);
            }
            else if (player.hasPerk(PerkLib.LizanRegeneration) && player.perkv4(PerkLib.LizanRegeneration) > 0)
            {
                addButtonDisabled(11, "LizanRegen", "Lizan Regeneration perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(11, "LizanRegen", "Lizan Regeneration");
            }
            if (player.hasPerk(PerkLib.Lustzerker) && player.perkv4(PerkLib.Lustzerker) < 1)
            {
                addButton(12, "Lustzerker", permanentizePerk2, PerkLib.Lustzerker);
            }
            else if (player.hasPerk(PerkLib.Lustzerker) && player.perkv4(PerkLib.Lustzerker) > 0)
            {
                addButtonDisabled(12, "Lustzerker", "Lustzerker perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(12, "Lustzerker", "Lustzerker");
            }
            if (player.hasPerk(PerkLib.MagicalFertility) && player.perkv4(PerkLib.MagicalFertility) < 1)
            {
                addButton(13, "MagicalF.", permanentizePerk2, PerkLib.MagicalFertility);
            }
            else if (player.hasPerk(PerkLib.MagicalFertility) && player.perkv4(PerkLib.MagicalFertility) > 0)
            {
                addButtonDisabled(13, "MagicalF.", "Magical Fertility perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(13, "MagicalF.", "Magical Fertility");
            }
        }
        if (page == 3)
        {
            if (player.hasPerk(PerkLib.MagicalVirility) && player.perkv4(PerkLib.MagicalVirility) < 1)
            {
                addButton(0, "MagicalV.", permanentizePerk3, PerkLib.MagicalVirility);
            }
            else if (player.hasPerk(PerkLib.MagicalVirility) && player.perkv4(PerkLib.MagicalVirility) > 0)
            {
                addButtonDisabled(0, "MagicalV.", "Magical Virility perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(0, "MagicalV.", "Magical Virility");
            }
            if (player.hasPerk(PerkLib.MaraesGiftButtslut) && player.perkv4(PerkLib.MaraesGiftButtslut) < 1)
            {
                addButton(1, "MaraesGiftB.", permanentizePerk3, PerkLib.MaraesGiftButtslut);
            }
            else if (player.hasPerk(PerkLib.MaraesGiftButtslut) && player.perkv4(PerkLib.MaraesGiftButtslut) > 0)
            {
                addButtonDisabled(1, "MaraesGiftB.", "Maraes Gift Buttslut perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(1, "MaraesGiftB.", "Maraes Gift Buttslut");
            }
            if (player.hasPerk(PerkLib.MaraesGiftFertility) && player.perkv4(PerkLib.MaraesGiftFertility) < 1)
            {
                addButton(2, "MaraesGiftF.", permanentizePerk3, PerkLib.MaraesGiftFertility);
            }
            else if (player.hasPerk(PerkLib.MaraesGiftFertility) && player.perkv4(PerkLib.MaraesGiftFertility) > 0)
            {
                addButtonDisabled(2, "MaraesGiftF.", "Maraes Gift Fertility perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(2, "MaraesGiftF.", "Maraes Gift Fertility");
            }
            if (player.hasPerk(PerkLib.MaraesGiftProfractory) && player.perkv4(PerkLib.MaraesGiftProfractory) < 1)
            {
                addButton(3, "MaraesGiftP.", permanentizePerk3, PerkLib.MaraesGiftProfractory);
            }
            else if (player.hasPerk(PerkLib.MaraesGiftProfractory) && player.perkv4(PerkLib.MaraesGiftProfractory) > 0)
            {
                addButtonDisabled(3, "MaraesGiftP.", "Maraes Gift Profractory perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(3, "MaraesGiftP.", "Maraes Gift Profractory");
            }
            addButton(4, "Next", ascensionPermeryMenu, page + 1);
            if (player.hasPerk(PerkLib.MaraesGiftStud) && player.perkv4(PerkLib.MaraesGiftStud) < 1)
            {
                addButton(5, "MaraesGiftS.", permanentizePerk3, PerkLib.MaraesGiftStud);
            }
            else if (player.hasPerk(PerkLib.MaraesGiftStud) && player.perkv4(PerkLib.MaraesGiftStud) > 0)
            {
                addButtonDisabled(5, "MaraesGiftS.", "Maraes Gift Stud perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(5, "MaraesGiftS.", "Maraes Gift Stud");
            }
            if (player.hasPerk(PerkLib.MilkMaid) && player.perkv4(PerkLib.MilkMaid) < 1)
            {
                addButton(6, "MilkMaid", permanentizePerk3, PerkLib.MilkMaid);
            }
            else if (player.hasPerk(PerkLib.MilkMaid) && player.perkv4(PerkLib.MilkMaid) > 0)
            {
                addButtonDisabled(6, "MilkMaid", "MilkMaid perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(6, "MilkMaid", "MilkMaid");
            }
            if (player.hasPerk(PerkLib.Misdirection) && player.perkv4(PerkLib.Misdirection) < 1)
            {
                addButton(7, "Misdirection", permanentizePerk3, PerkLib.Misdirection);
            }
            else if (player.hasPerk(PerkLib.Misdirection) && player.perkv4(PerkLib.Misdirection) > 0)
            {
                addButtonDisabled(7, "Misdirection", "Misdirection perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(7, "Misdirection", "Misdirection");
            }
            if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] >= 2)
            {
                if (player.hasPerk(PerkLib.NinetailsKitsuneOfBalance) && player.perkv4(PerkLib.NinetailsKitsuneOfBalance) < 1)
                {
                    addButton(8, "9TKitsOfB", permanentizePerk3, PerkLib.NinetailsKitsuneOfBalance);
                }
                else if (player.hasPerk(PerkLib.NinetailsKitsuneOfBalance) && player.perkv4(PerkLib.NinetailsKitsuneOfBalance) > 0)
                {
                    addButtonDisabled(8, "9TKitsOfB", "9T Kitsune Of Balance perk is already made permanent and will carry over in all subsequent ascensions.");
                }
            }
            else
            {
                addButtonDisabled(8, "9TKitsOfB", "9T Kitsune Of Balance");
            }
            addButton(9, "Previous", ascensionPermeryMenu, page - 1);
            if (player.hasPerk(PerkLib.OneTrackMind) && player.perkv4(PerkLib.OneTrackMind) < 1)
            {
                addButton(10, "OneTrackMind", permanentizePerk3, PerkLib.OneTrackMind);
            }
            else if (player.hasPerk(PerkLib.OneTrackMind) && player.perkv4(PerkLib.OneTrackMind) > 0)
            {
                addButtonDisabled(10, "OneTrackMind", "One Track Mind perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(10, "OneTrackMind", "One Track Mind");
            }
            if (player.hasPerk(PerkLib.PureAndLoving) && player.perkv4(PerkLib.PureAndLoving) < 1)
            {
                addButton(11, "Pure&Loving", permanentizePerk3, PerkLib.PureAndLoving);
            }
            else if (player.hasPerk(PerkLib.PureAndLoving) && player.perkv4(PerkLib.PureAndLoving) > 0)
            {
                addButtonDisabled(11, "Pure&Loving", "Pure And Loving perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(11, "Pure&Loving", "Pure And Loving");
            }
            if (player.hasPerk(PerkLib.PurityBlessing) && player.perkv4(PerkLib.PurityBlessing) < 1)
            {
                addButton(12, "PurityBlessing", permanentizePerk3, PerkLib.PurityBlessing);
            }
            else if (player.hasPerk(PerkLib.PurityBlessing) && player.perkv4(PerkLib.PurityBlessing) > 0)
            {
                addButtonDisabled(12, "PurityBlessing", "Purity Blessing perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(12, "PurityBlessing", "Purity Blessing");
            }
        }
        if (page == 4)
        {
            if (player.hasPerk(PerkLib.RapierTraining) && player.perkv4(PerkLib.RapierTraining) < 1)
            {
                addButton(0, "RapierTrain", permanentizePerk4, PerkLib.RapierTraining);
            }
            else if (player.hasPerk(PerkLib.RapierTraining) && player.perkv4(PerkLib.RapierTraining) > 0)
            {
                addButtonDisabled(0, "RapierTrain", "Rapier Training perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(0, "RapierTrain", "Rapier Training");
            }
            if (player.hasPerk(PerkLib.SensualLover) && player.perkv4(PerkLib.SensualLover) < 1)
            {
                addButton(1, "SensualLover", permanentizePerk4, PerkLib.SensualLover);
            }
            else if (player.hasPerk(PerkLib.SensualLover) && player.perkv4(PerkLib.SensualLover) > 0)
            {
                addButtonDisabled(1, "SensualLover", "Sensual Lover perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(1, "SensualLover", "Sensual Lover");
            }
            if (player.hasPerk(PerkLib.TechOverdrive) && player.perkv4(PerkLib.TechOverdrive) < 1)
            {
                addButton(2, "TechOverdrive", permanentizePerk4, PerkLib.TechOverdrive);
            }
            else if (player.hasPerk(PerkLib.TechOverdrive) && player.perkv4(PerkLib.TechOverdrive) > 0)
            {
                addButtonDisabled(2, "TechOverdrive", "Tech Overdrive perk is already made permanent and will carry over in all subsequent ascensions.");
            }
            else
            {
                addButtonDisabled(2, "TechOverdrive", "Tech Overdrive");
            }
            //3
            //addButton(4, "Next", ascensionPermeryMenu, page + 1);
            //5
    ...
    //Not enough points or perk already permed? Cancel.
    {
        ...
        if (player.statusEffectv4(statusEffect) == 9000)
        {
            return;
        }
        //Deduct points
    ...
    //Not enough points or perk already permed? Cancel.
    {
        ...
        if (player.perkv4(PerkLib.EnlightenedKitsune) > 0)
        {
            return;
        }
    ...
    //Not enough points or perk already permed? Cancel.
    {
        ...
        if (player.perkv4(PerkLib.CorruptedKitsune) > 0)
        {
            return;
        }
    ...
    public function ascensionMetamorphPermMenu(currentPage : Int = 0) : Void
    {
        ...
        if (lastPage > 0)
        {
            if (currentPage > 0)
            {
                addButton(12, "Prev Page", ascensionMetamorphPermMenu, currentPage - 1);
            }
            else
            {
            ...
            {
                addButton(13, "Next Page", ascensionMetamorphPermMenu, currentPage + 1);
            }
            else
            {
    ...
    // Hides Taur variants of pre-existing TFs
    {
        ...
        var memArray : Array<Dynamic> = LowerBodyMem.Memories.filter(function(element : Dynamic, index : Int, array : Array<Dynamic>) : Bool
                {
    ...
    private function openPaginatedMetamorphMenu(title : String, thisMenu : Dynamic, currentPage : Int, memArray : Array<Dynamic>) : Void
    {
        clearOutput();
        ...
        for (genMem in pageMems)
        {
            ...
            var unlocked : Bool = Metamorph.GeneticMemoryStorage[genMem.id];
            var enoughPP : Bool = player.ascensionPerkPoints >= (genMem.permCost || 5);
            var obtained : Bool = Metamorph.PermanentMemoryStorage[genMem.id];
            var hasRequirements : Bool = ((genMem.permReq) ? Metamorph.PermanentMemoryStorage[genMem.permReq] || false : true);
            
            if (unlocked && hasRequirements && enoughPP && !obtained)
            {
                addButton(currentButton, buttonStr, permanentizeMetamorph, menuInfo, genMem).hint("Cost: " + (genMem.permCost || 5) + " Ascension Perk Points" + ((genMem.permInfo) ? "\n\n" + genMem.permInfo : ""));
            }
            else if (unlocked && !hasRequirements && !obtained)
            {
                addButtonDisabled(currentButton, buttonStr, "Cost: " + (genMem.permCost || 5) + " Ascension Perk Points\n\n<b>Requirement</b>: Permanentize " + genMem.permReq);
            }
            else if (unlocked && hasRequirements && !enoughPP && !obtained)
            {
                addButtonDisabled(currentButton, buttonStr, "Cost: " + (genMem.permCost || 5) + " Ascension Perk Points (You don't have enough Ascension Perk Points to permanentize this metamorphosis!)");
            }
            else if (obtained)
            {
                addButtonDisabled(currentButton, buttonStr, "You already permanentized this metamorphosis!");
            }
            else if (!unlocked)
            {
                addButtonDisabled(currentButton, buttonStr, "You can't permanentize a metamorphosis you didn't unlock!");
            }
            currentButton++;
        }
        ...
        if (lastPage > 0)
        {
            if (currentPage > 0)
            {
                addButton(12, "Prev Page", thisMenu, currentPage - 1);
            }
            else
            {
                addButtonDisabled(12, "Prev Page");
            ...
            if (currentPage < lastPage)
            {
                addButton(13, "Next Page", thisMenu, currentPage + 1);
            }
            else
            {
                addButtonDisabled(13, "Next Page");
    ...
    private function permanentizeMetamorph(menuInfo : Dynamic, genMem : Dynamic) : Void
    {
    ...
    private function reincarnatePrompt() : Void
    {
        ...
        if (player.hasPerk(PerkLib.AscensionMenuChoiceMaybe))
        {
            addButton(1, "Yes", reincarnate002).hint("Reincarnate");
        }
        else
        {
            addButton(1, "Yes", reincarnate001).hint("Reincarnate");
        }
    ...
    private function reincarnate002() : Void
    {
        ...
        //In case ng0, otherwise, player is treated as new char.{
    ...
    private function reincarnate() : Void
    {
        ...
        if (player.humanScore() >= 10 && player.howManyDifferentHumanIMYouGot() >= 4 && flags[kFLAGS.HUMAN_BLOODLINE] == 0)
        {
            flags[kFLAGS.HUMAN_BLOODLINE] = 1;
        }
        if (player.humanScore() >= 20 && player.howManyDifferentHumanIMYouGot() >= 8 && flags[kFLAGS.HUMAN_BLOODLINE] == 1)
        {
            flags[kFLAGS.HUMAN_BLOODLINE] = 2;
        }
        if (player.humanScore() >= 40 && player.howManyDifferentHumanIMYouGot() >= 12 && flags[kFLAGS.HUMAN_BLOODLINE] == 2)
        {
            flags[kFLAGS.HUMAN_BLOODLINE] = 3;
        }
        if (player.humanScore() >= 50 && player.howManyDifferentHumanIMYouGot() >= 14 && flags[kFLAGS.HUMAN_BLOODLINE] == 3)
        {
            flags[kFLAGS.HUMAN_BLOODLINE] = 4;
        }  //>= 77 - >= 14  
        customPlayerProfile = null;
        newGameGo();
    ...
    private function removeLevelPerks() : Void
    {
        ...
        for (i in 0...player.perks.length)
        {
            if (isAscensionPerk(player.perks[i], true))
            {
                ascendPerkTemp.push(player.perks[i]);
            }
        }
    ...
    private function isAscensionPerk(perk : PerkClass, respec : Bool = false) : Bool
    {
        return perk.ptype.keepOnAscension(respec) || (perk.value4 > 0);
    }
    
    private function isSpecialKeyItem(keyName : Dynamic = null) : Bool
    //tylko sky poison pearl zostawić tutaj (only leave sky, poison, pearl here)
    {
        
        return (keyName == "Ascension" || keyName == "Sky Poison Pearl" || keyName == "Dimensional Pocket" || keyName == "Nieve's Tear");
    }
    
    private function isSpecialStatus(statusEffects : StatusEffectClass, statusEffect : Dynamic = null) : Bool
    {
        return (statusEffect == StatusEffects.KnowsWereBeast || statusEffects.value4 == 9000);
    ...
    public static function hasAscensionSpell(spellCat : Int) : Bool
    {
        var spellsToCheck : Array<Dynamic>;
        switch (spellCat)
        {
        ...
        if (spellsToCheck != null)
        {
            return spellsToCheck.some(function(statusEffect : StatusEffectType) : Bool
                    {
