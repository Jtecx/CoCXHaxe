...
class SaveUpdater extends NPCAwareContent
{
    ...
    //Dungeon counter
    {
        ...
        var dungeonsList : Array<Dynamic> = [
        ...
        if (flags[kFLAGS.THIEFS_KILLED] > 0)
        {
            totalKillCount += flags[kFLAGS.THIEFS_KILLED];
        }
        if (flags[kFLAGS.IMPS_KILLED] > 0)
        {
            totalKillCount += flags[kFLAGS.IMPS_KILLED];
        }
        if (flags[kFLAGS.GOBLINS_KILLED] > 0)
        {
            totalKillCount += flags[kFLAGS.GOBLINS_KILLED];
        }
        if (flags[kFLAGS.HELLHOUNDS_KILLED] > 0)
        {
            totalKillCount += flags[kFLAGS.HELLHOUNDS_KILLED];
        }
        if (flags[kFLAGS.MINOTAURS_KILLED] > 0)
        {
            totalKillCount += flags[kFLAGS.MINOTAURS_KILLED];
        }
        if (flags[kFLAGS.TRUE_DEMONS_KILLED] > 0)
        {
            totalKillCount += flags[kFLAGS.TRUE_DEMONS_KILLED];
        }
        //Check how many NPCs got bad-ended.
        var badEndList : Array<Dynamic> = [
        ...
        if (AetherTwinsFollowers.AetherTwinsTalkMenu > 0)
        {
            EvolutionsCount++;
        }
        
        var dedickList : Array<Dynamic> = [
        ...
        var achievementList : Array<Dynamic> = [
        //Story
        ...
        SceneLib.forest.timesExploredOuter() > 0 && SceneLib.forest.timesExploredInner() > 0 && SceneLib.forest.timesExploredDeepwoods() > 0 && SceneLib.lake.timesExplored() > 0 && SceneLib.desert.timesExploredOuter() > 0 && SceneLib.mountain.timesExploredMid() > 0 && SceneLib.plains.timesExplored() > 0 && SceneLib.swamp.timesExplored() > 0 && SceneLib.blightridge.timesExplored() > 0 && SceneLib.battlefiledouter.timesExplored() > 0 && SceneLib.caves.timesExplored() > 0 && SceneLib.highMountains.timesExplored() > 0 && SceneLib.bog.timesExplored() > 0 && SceneLib.tundra.timesExplored() > 0 && SceneLib.glacialRift.timesExplored() > 0 && SceneLib.ashlands.timesExplored() > 0 && SceneLib.volcanicCrag.timesExplored() > 0
    ], 
        ...
        {
            if (Reflect.field(ach, Std.string(2)) != null)
            {
                awardAchievement(Reflect.field(ach, Std.string(0)), Reflect.field(ach, Std.string(1)));
            }
        }
        ...
        //Shadow
        if (achStat.achievementsShadowEarned >= 1)
        {
            awardAchievement("Shadow Initiate", kACHIEVEMENTS.SHADOW_INITIATE);
        }
        if (achStat.achievementsShadowEarned >= 10)
        {
            awardAchievement("Shadow Squire", kACHIEVEMENTS.SHADOW_SQUIRE);
        }
        if (achStat.achievementsShadowEarned >= 25)
        {
            awardAchievement("Shadow Knight", kACHIEVEMENTS.SHADOW_KNIGHT);
        }
        if (achStat.achievementsShadowEarned >= 45)
        {
            awardAchievement("Shadow Paladin", kACHIEVEMENTS.SHADOW_PALADIN);
        }
        if (achStat.achievementsShadowEarned >= 70)
        {
            awardAchievement("Shadow General", kACHIEVEMENTS.SHADOW_GENERAL);
        }
        //Epic
        if (achStat.achievementsTotalCurrentlyUnlocked >= 10)
        {
            awardAchievement("Achievementception", kACHIEVEMENTS.EPIC_ACHIEVEMENTCEPTION);
        }
        if (achStat.achievementsTotalCurrentlyUnlocked >= 30)
        {
            awardAchievement("Achievement within Achievement", kACHIEVEMENTS.EPIC_ACHIEVEMENT_WITHIN_ACHIEVEMENT);
        }
        if (achStat.achievementsTotalCurrentlyUnlocked >= 60)
        {
            awardAchievement("Achievements - Going Deeper (1st layer)", kACHIEVEMENTS.EPIC_ACHIEVEMENTS_GOING_DEEPER_1L);
        }
        if (achStat.achievementsTotalCurrentlyUnlocked >= 100)
        {
            awardAchievement("Achievements - Going Deeper (2nd layer)", kACHIEVEMENTS.EPIC_ACHIEVEMENTS_GOING_DEEPER_2L);
        }
        if (achStat.achievementsTotalCurrentlyUnlocked >= 300)
        {
            awardAchievement("Achievements - Going Deeper (3rd layer)", kACHIEVEMENTS.EPIC_ACHIEVEMENTS_GOING_DEEPER_3L);
        }
        if (achStat.achievementsTotalCurrentlyUnlocked >= 600)
        {
            awardAchievement("Achievements Limbo", kACHIEVEMENTS.EPIC_ACHIEVEMENTS_LIMBO);
        }
    }
    ...
    //Marae
    {
        ...
        if (SceneLib.dungeons.checkPhoenixTowerClear() && flags[kFLAGS.CLEARED_HEL_TOWER] < 2)
        {
    ...
    public function onUnknownPerk(player : Player, savedata : Dynamic, id : String, value1 : Float, value2 : Float, value3 : Float, value4 : Float) : Void
    {
        CoC_Settings.error("Unknown perk id: " + id);
    }
    public function onUnknownStatusEffect(player : Player, savedata : Dynamic, id : String, value1 : Float, value2 : Float, value3 : Float, value4 : Float) : Void
    {
        switch (id)
        {
    ...
    public function onUnknownInventoryItem(player : Player, savedata : Dynamic, itemId : String, savedShortName : String, quantity : Float, storageSlot : ItemSlotClass) : Void
    {
        storageSlot.emptySlot();
    }
    public function onUnknownEquipmentItem(player : Player, savedata : Dynamic, slot : Int, savedId : String, savedName : String, defaultValue : Equipable) : Bool
    {
	 ...
	 */
    public function postLoadSaveObject(player : Player, savedata : Dynamic) : Void
    {
        if (!(savedata.ss && savedata.ss["Exploration"]))
        {
            ...
            SceneLib.exploration.counters.forestInner = as3hx.Compat.parseInt(savedata.exploredForest / 2);
            SceneLib.exploration.counters.forestOuter = as3hx.Compat.parseInt(savedata.exploredForest / 2);
            SceneLib.exploration.counters.desertOuter = savedata.exploredDesert;
            SceneLib.exploration.counters.explore = savedata.explored;
        }
        for (fn in updateQueue)
        {
            fn();
        }
        updateQueue = [];
    ...
    public function promptSaveUpdate() : Void
    {
        ...
        var initialVersion : Float = flags[kFLAGS.MOD_SAVE_VERSION];
        ...
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 5)
        {
            ...
            if (player.armorName == "revealing fur loincloths" || player.armorName == "comfortable underclothes" || player.weaponName == "dragon-shell shield")
            {
                ...
                if (player.armorName == "revealing fur loincloths")
                {
                    inventory.takeItem(player.setArmor(ArmorLib.COMFORTABLE_UNDERCLOTHES), promptSaveUpdate);
                }
                if (player.weaponName == "dragon-shell shield")
                {
                    inventory.takeItem(player.setWeapon(WeaponLib.FISTS), promptSaveUpdate);
                }
                doNext(camp.doCamp);
                return;
            }
        ...
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 6)
        {
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] == 6)
            {
                ...
                flags[kFLAGS.CORRUPTED_GLADES_DESTROYED] = 0;
            }
        ...
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 11)
        {
            ...
            if (flags[kFLAGS.SOULFORCE_USED_FOR_BREAKTHROUGH] < 0)
            {
                flags[kFLAGS.SOULFORCE_USED_FOR_BREAKTHROUGH] = 0;
            }
        ...
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 12)
        {
            ...
            {
                refund += as3hx.Compat.parseInt(player.perkv1(PerkLib.AscensionTolerance) - 10);
        ...
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 18)
        {
            ...
            if (flags[kFLAGS.KITSUNE_SHRINE_UNLOCKED] > 0 && flags[kFLAGS.AYANE_FOLLOWER] < 0)
            {
                flags[kFLAGS.AYANE_FOLLOWER] = 0;
            }
            if (flags[kFLAGS.GOTTEN_INQUISITOR_ARMOR] > 0)
            {
        ...
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 21)
        {
            ...
            if (flags[kFLAGS.STAT_GAIN_MODE] == CoC.STAT_GAIN_CLASSIC)
            {
                ...
                {
                    player.statPoints += (5 * player.level);
                }
            }
            if (player.level > 6)
            {
            ...
            if (flags[kFLAGS.SOUL_ARENA_FINISHED_GAUNLETS] > 0)
            {
                ...
                {
                    player.createStatusEffect(StatusEffects.SoulArenaGaunlets1, 2, 0, 0, 0);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.SoulArenaGaunlets1, 2, 2, 0, 0);
                }
        ...
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 22)
        {
            ...
            {
                player.addStatusValue(StatusEffects.TranscendentalGeneticMemory, 1, 5);
            }
            if (player.hasPerk(PerkLib.AscensionTranscendentalGeneticMemoryStage2))
            {
                player.addStatusValue(StatusEffects.TranscendentalGeneticMemory, 1, 10);
            }
            if (player.hasPerk(PerkLib.AscensionTranscendentalGeneticMemoryStage3))
            {
                player.addStatusValue(StatusEffects.TranscendentalGeneticMemory, 1, 15);
            }
            ...
            {
                player.addStatusValue(StatusEffects.TranscendentalGeneticMemory, 1, 25);
            }
        ...
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 25)
        {
            ...
            if (flags[kFLAGS.CURSE_OF_THE_JIANGSHI] > 1)
            {
                ...
                if (player.hasPerk(PerkLib.SuperStrength))
                {
                    player.removePerk(PerkLib.SuperStrength);
                }
                if (player.hasPerk(PerkLib.PoisonNails))
                {
                ...
                if (player.hasPerk(PerkLib.LifeLeech))
                {
                    player.removePerk(PerkLib.LifeLeech);
                }
                if (player.hasPerk(PerkLib.Undeath))
                {
                ...
                if (player.statStore.hasBuff("Energy Vampire"))
                {
            ...
            if (player.hasStatusEffect(StatusEffects.AdventureGuildQuests1))
            {
                ...
                if (player.statusEffectv1(StatusEffects.AdventureGuildQuests1) == 2)
                {
                ...
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuestsCounter1, 1, 2);
                }
                if (player.statusEffectv1(StatusEffects.AdventureGuildQuests1) == 7)
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuestsCounter1, 1, 3);
                }
                if (player.statusEffectv2(StatusEffects.AdventureGuildQuests1) == 2)
                {
                ...
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuestsCounter1, 2, 2);
                }
                if (player.statusEffectv2(StatusEffects.AdventureGuildQuests1) == 7)
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuestsCounter1, 2, 3);
                }
                if (player.statusEffectv3(StatusEffects.AdventureGuildQuests1) == 2)
                {
                ...
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuestsCounter1, 3, 2);
                }
                if (player.statusEffectv3(StatusEffects.AdventureGuildQuests1) == 7)
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuestsCounter1, 3, 3);
                }
            }
            if (player.hasStatusEffect(StatusEffects.AdventureGuildQuests2))
            {
                ...
                if (player.statusEffectv1(StatusEffects.AdventureGuildQuests2) == 2)
                {
                ...
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuestsCounter2, 1, 2);
                }
                if (player.statusEffectv1(StatusEffects.AdventureGuildQuests2) == 7)
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuestsCounter2, 1, 3);
                }
                if (player.statusEffectv2(StatusEffects.AdventureGuildQuests2) == 2)
                {
                ...
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuestsCounter2, 2, 2);
                }
                if (player.statusEffectv2(StatusEffects.AdventureGuildQuests2) == 7)
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuestsCounter2, 2, 3);
                }
            }
            if (player.hasStatusEffect(StatusEffects.AdventureGuildQuests4))
            {
                ...
                if (player.statusEffectv1(StatusEffects.AdventureGuildQuests4) == 5)
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuestsCounter4, 1, 2);
                }
                ...
                if (player.statusEffectv2(StatusEffects.AdventureGuildQuests4) == 5)
                {
                    player.addStatusValue(StatusEffects.AdventureGuildQuestsCounter4, 2, 2);
                }
            }
            doNext(camp.doCamp);
            return;
        ...
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 27)
        {
            ...
            if (player.blockingBodyTransformations())
            {
                ...
                {
                    player.removePerk(PerkLib.FutaForm);
                }
        ...
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 28)
        {
            ...
            if (flags[kFLAGS.STAT_GAIN_MODE] == CoC.STAT_GAIN_CLASSIC)
            {
                if (player.level > 6)
                {
                    player.statPoints += ((5 * player.level) + 60);
                }
                else
                {
                    player.statPoints += (10 * player.level);
                }
            }
            ...
            {
                var refund1 : Int = 0;
                refund1 += as3hx.Compat.parseInt(player.perkv1(PerkLib.AscensionWisdom) - 50);
                player.setPerkValue(PerkLib.AscensionWisdom, 1, 50);
                player.ascensionPerkPoints += refund1;
            }
            var SphereMastery : Float = 10;
        ...
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 29)
        {
            ...
            if (!player.hasStatusEffect(StatusEffects.ZenjiZList))
            {
            ...
            player.statStore.addBuffObject({
                        ...
                        text : "Ezekiel Blessing"
                    });
            if (flags[kFLAGS.DIANA_FOLLOWER] >= 6 && flags[kFLAGS.DIANA_LVL_UP] < 8)
            {
        ...
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 30)
        {
            ...
            if (flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 1 && flags[kFLAGS.IZMA_BROFIED] == 1)
            {
                flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] = 0;
            }
            if (flags[kFLAGS.MARAE_ISLAND] < 1 && flags[kFLAGS.MET_MARAE] == 1)
            {
            ...
            if (player.hasStatusEffect(StatusEffects.TelAdreTripxi))
            {
                ...
                {
                    player.removeStatusEffect(StatusEffects.TelAdreTripxiGuns1);
                }
                if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns2))
                {
                    player.removeStatusEffect(StatusEffects.TelAdreTripxiGuns2);
                }
                if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns3))
                {
                    player.removeStatusEffect(StatusEffects.TelAdreTripxiGuns3);
                }
                if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns4))
                {
                    player.removeStatusEffect(StatusEffects.TelAdreTripxiGuns4);
                }
                if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns5))
                {
                    player.removeStatusEffect(StatusEffects.TelAdreTripxiGuns5);
                }
                if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns6))
                {
                    player.removeStatusEffect(StatusEffects.TelAdreTripxiGuns6);
                }
            }
            doNext(camp.doCamp);
            return;
        ...
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 32)
        {
            ...
            if (flags[kFLAGS.GAME_DIFFICULTY] < 2 && flags[kFLAGS.HARDCORE_MODE] == 1)
            {
            ...
            if (player.hasPerk(PerkLib.GargoylePure) || player.hasPerk(PerkLib.GargoyleCorrupted))
            {
                player.createPerk(PerkLib.StrengthOfStone, 0, 0, 0, 0);
            }
            if (player.weapon == weapons.AETHERD && flags[kFLAGS.AETHER_DEXTER_TWIN_AT_CAMP] == 1)
            {
                flags[kFLAGS.AETHER_DEXTER_TWIN_AT_CAMP] = 2;
            }
            if (player.shield == shields.AETHERS && flags[kFLAGS.AETHER_SINISTER_TWIN_AT_CAMP] == 1)
            {
        ...
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 33)
        {
            ...
            if (flags[kFLAGS.TOUGHNESS_SCALING] != 0)
            {
                flags[kFLAGS.TOUGHNESS_SCALING] = 0;
            }
            doNext(camp.doCamp);
            return;
        }
        if (flags[kFLAGS.MOD_SAVE_VERSION] == 34)
        {
            ...
            if (player.hasKeyItem("Backpack") >= 0)
            {
                player.gems += (150 * player.keyItemvX("Backpack", 1));
            }
            ...
            if (flags[kFLAGS.SOUL_CULTIVATION] == 1 || flags[kFLAGS.SOUL_CULTIVATION] == 2)
            {
                flags[kFLAGS.SOUL_CULTIVATION] = 0;
            }
            if (flags[kFLAGS.SOUL_CULTIVATION] >= 3)
            {
            ...
            {
                jiangshiBuggedItemsCleanUpCrew();
            }  //LAST THING TO DO IN THIS SAVE UPDATE  
            doNext(camp.doCamp);
            return;
        }
        if (as3hx.Compat.parseInt(flags[kFLAGS.MOD_SAVE_VERSION]) == 35)
        
        //now using float to store versions!{
            ...
            //If something needs a fix, abort - not ready to update the version yet!{
                ...
                if (consumables.E3PEARL.fixSave())
                {
                    return;
                }
                if (consumables.E5PEARL.fixSave())
                {
                    return;
                }
                if (consumables.E7PEARL.fixSave())
                {
                    return;
                }
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 35.008)
            
            //Sapphire affection adjusted. Need some fixing.{
                
                if (flags[kFLAGS.WEDDINGS_UNLOCKED] == 1)
                
                //checked sex before, now transformed to a wedding check.{
                ...
                {
                    flags[kFLAGS.SAPPHIRE_AFFECTION] *= 10;
                }  //should work.  
            ...
            {
                if (player.lowerBody == LowerBody.GOO && player.arms.type == Arms.GOO && player.rearBody.type == RearBody.METAMORPHIC_GOO && player.skinAdj != "slimy")
                {
            ...
            {
                if (flags[kFLAGS.NEISA_FOLLOWER] >= 10 && flags[kFLAGS.NEISA_FOLLOWER] < 17)
                {
            ...
            {
                if (flags[kFLAGS.IZUMI_MET] > 0 && !TyrantiaFollower.TyraniaAndIzumi && TyrantiaFollower.TyrantiaFollowerStage > 0)
                {
                    TyrantiaFollower.TyraniaAndIzumi = true;
                }
            ...
            {
                if (AdventurerGuild.lootBag.SlotCaps[3] >= 5)
                {
                    AdventurerGuild.lootBag.SlotCaps[4] = 10;
                }
        ...
        if (as3hx.Compat.parseInt(flags[kFLAGS.MOD_SAVE_VERSION]) == 36)
        {
            clearOutput();
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.001)
            {
                if (player.hasPerk(PerkLib.AscensionTranscendentalGeneticMemoryStage1))
                {
                    ...
                    {
                        player.setPerkValue(PerkLib.AscensionTrancendentalGeneticMemoryStageX, 1, 1);
                    }
                    else
                    {
                        player.createPerk(PerkLib.AscensionTrancendentalGeneticMemoryStageX, 1, 0, 0, 1);
                    }
                    player.removePerk(PerkLib.AscensionTranscendentalGeneticMemoryStage1);
                }
                if (player.hasPerk(PerkLib.AscensionTranscendentalGeneticMemoryStage2))
                {
                    if (player.hasPerk(PerkLib.AscensionTrancendentalGeneticMemoryStageX))
                    {
                        player.setPerkValue(PerkLib.AscensionTrancendentalGeneticMemoryStageX, 1, 2);
                    }
                    else
                    {
                        player.createPerk(PerkLib.AscensionTrancendentalGeneticMemoryStageX, 2, 0, 0, 1);
                    }
                    player.removePerk(PerkLib.AscensionTranscendentalGeneticMemoryStage2);
                }
                if (player.hasPerk(PerkLib.AscensionTranscendentalGeneticMemoryStage3))
                {
                    if (player.hasPerk(PerkLib.AscensionTrancendentalGeneticMemoryStageX))
                    {
                        player.setPerkValue(PerkLib.AscensionTrancendentalGeneticMemoryStageX, 1, 3);
                    }
                    else
                    {
                        player.createPerk(PerkLib.AscensionTrancendentalGeneticMemoryStageX, 3, 0, 0, 1);
                    }
                    player.removePerk(PerkLib.AscensionTranscendentalGeneticMemoryStage3);
                }
                if (player.hasPerk(PerkLib.AscensionTranscendentalGeneticMemoryStage4))
                {
                    if (player.hasPerk(PerkLib.AscensionTrancendentalGeneticMemoryStageX))
                    {
                        player.setPerkValue(PerkLib.AscensionTrancendentalGeneticMemoryStageX, 1, 4);
                    }
                    else
                    {
                        player.createPerk(PerkLib.AscensionTrancendentalGeneticMemoryStageX, 4, 0, 0, 1);
                    }
                    player.removePerk(PerkLib.AscensionTranscendentalGeneticMemoryStage4);
                }
                if (player.hasPerk(PerkLib.AscensionTranscendentalGeneticMemoryStage5))
                {
                    if (player.hasPerk(PerkLib.AscensionTrancendentalGeneticMemoryStageX))
                    {
                        player.setPerkValue(PerkLib.AscensionTrancendentalGeneticMemoryStageX, 1, 5);
                    }
                    else
                    {
                        player.createPerk(PerkLib.AscensionTrancendentalGeneticMemoryStageX, 5, 0, 0, 1);
                    }
                    player.removePerk(PerkLib.AscensionTranscendentalGeneticMemoryStage5);
                }
                if (player.hasPerk(PerkLib.AscensionTranscendentalGeneticMemoryStage6))
                {
                    if (player.hasPerk(PerkLib.AscensionTrancendentalGeneticMemoryStageX))
                    {
                        player.setPerkValue(PerkLib.AscensionTrancendentalGeneticMemoryStageX, 1, 6);
                    }
                    else
                    {
                        player.createPerk(PerkLib.AscensionTrancendentalGeneticMemoryStageX, 6, 0, 0, 1);
                    }
                    player.removePerk(PerkLib.AscensionTranscendentalGeneticMemoryStage6);
                }
            ...
            {
                if (player.hasPerk(PerkLib.AscensionNaturalMetamorph) && !player.hasPerk(PerkLib.MetamorphEx))
                {
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.007)
            ...
            {
                if (flags[kFLAGS.APEX_SELECTED_RACE] >= 18)
                {
                    flags[kFLAGS.APEX_SELECTED_RACE] += 1;
                }
                flags[kFLAGS.MOD_SAVE_VERSION] = 36.008;
            }
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.009)
            {
                if (player.furColor == "lilac and white striped")
                {
                    player.furColor = "lilac and white";
                }
                if (player.hairColor == "lilac and white striped")
                {
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.011)
            {
                ...
                if (flags[kFLAGS.MINERVA_PURE_TALKED_CORRUPTION] == 1)
                {
                    flags[kFLAGS.SEX_MACHINE_STATUS] = -1;
                }  //sex machine disabled using its status now  
            ...
            {
                if (player.statStore.hasBuff("Drained"))
                {
                    player.statStore.removeBuffs("Drained");
                }
                if (flags[kFLAGS.MALI_BLADE_STATUS] == 1)
                
                //old status = 'MALI BLADE GIVEN'{
            ...
            // Reorder SPP (Old slot unlock order: 56-69, 0-55, 70-97; new is 0-97){
                ...
                var spp : Array<Dynamic> = inventory.pearlStorageDirectGet();
                var n : Int = 0;
                var sz : Int = inventory.pearlStorageSize();
                var nl : Int = 0;
                if (sz > 0 && sz < 70)
                {
                    for (i in 56...70)
            ...
            // convert old buff tags to new ("item_"+itemid){
                ...
                var ItemBuffsRename : Array<Dynamic> = [
                ...
                for (pair in ItemBuffsRename)
                
                // buffs are re-created on save load with afterEquip fn{
                    
                    player.buff(Reflect.field(pair, Std.string(0))).remove();
                }
            ...
            // these buffs are recreated on save load{
                ...
                var ItemBuffsRemove : Array<Dynamic> = [
                ...
                for (tag in ItemBuffsRemove)
                {
                    for (buff/* AS3HX WARNING could not determine type for var: buff exp: ECall(EField(ECall(EField(EIdent(player),buff),[EIdent(tag)]),findAllBuffObjects),[]) type: null */ in player.buff(tag).findAllBuffObjects())
                    {
                        if (buff.save)
                        {
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.023)
            {
                ...
                if (flags[kFLAGS.MET_MARAE_CORRUPTED] == 2)
                {
            ...
            // Split existing core stats into core + train{
                ...
                var primaryStats : Array<Dynamic> = [player.strStat, player.touStat, player.speStat, player.intStat, player.wisStat, player.libStat];
                var oldCoreTotal : Int = 0;
                var oldCoreStats : Array<Dynamic> = [0, 0, 0, 0, 0, 0];
                outputText("\n\nStat rework! Training is separated from level-up, <b>but benefits less from multipliers</b>.\nOld core stat values:");
                for (i in 0...primaryStats.length)
                {
                ...
                var statPointsPerLevel : Int = as3hx.Compat.parseInt(5 + (player.perkv1(PerkLib.AscensionAdvTrainingX)));
                var statPoints : Int = as3hx.Compat.parseInt(player.level * statPointsPerLevel);
                if (player.level <= 6)
                {
                    statPoints += as3hx.Compat.parseInt(player.level * statPointsPerLevel);
                }
                else
                {
                    statPoints += as3hx.Compat.parseInt(6 * statPointsPerLevel);
                }
                statPoints -= player.statPoints;
                statPoints -= as3hx.Compat.parseInt(JourneyToTheEast.AhriStatsToPerksConvertCounter * 5);
                statPoints += as3hx.Compat.parseInt(JourneyToTheEast.EvelynnPerksToStatsConvertCounter * 5);
                if (flags[kFLAGS.FACTORY_OMNIBUS_DEFEATED] == 2)
                {
                    statPoints += 10;
                }
                if (flags[kFLAGS.DEFEATED_ZETAZ] > 1)
                {
                    statPoints += 20;
                }
                ...
                if (flags[kFLAGS.DISCOVERED_WITCH_DUNGEON] == 2)
                {
                    statPoints += 5;
                }
                if (flags[kFLAGS.CLEARED_HEL_TOWER] == 2)
                {
                    statPoints += 10;
                }
                if (flags[kFLAGS.DISCOVERED_BEE_HIVE_DUNGEON] == 3)
                {
                    statPoints += 10;
                }
                if (flags[kFLAGS.HIDDEN_CAVE_LOLI_BAT_GOLEMS] == 6)
                {
                    statPoints += 5;
                }
                if (flags[kFLAGS.HIDDEN_CAVE_BOSSES] == 3)
                ...
                if (flags[kFLAGS.DEN_OF_DESIRE_QUEST] == 2)
                {
                    statPoints += 15;
                }
                if (player.hasStatusEffect(StatusEffects.RiverDungeonFloorRewards))
                {
                    statPoints += as3hx.Compat.parseInt(5 * player.statusEffectv1(StatusEffects.RiverDungeonFloorRewards));
                }
                if (flags[kFLAGS.EBON_LABYRINTH] >= 50)
                {
                    statPoints += 5;
                }
                statPoints += as3hx.Compat.parseInt(as3hx.Compat.parseInt(flags[kFLAGS.EBON_LABYRINTH] / 150) * 5);
                var totalTrainPoints : Int = as3hx.Compat.parseInt(oldCoreTotal - statPoints);
                ...
                for (i in 0...primaryStats.length)
                {
                    stat = primaryStats[i];
                    // ratio
                    var x : Float = (oldCoreStats[i] - 15) / (oldCoreTotal - 6 * 15);
                    // don't train over max or old value
                    x = Math.min(Math.min(stat.train.max, 15 + as3hx.Compat.parseInt(x * (totalTrainPoints - 6 * 15))), oldCoreStats[i]);
                    remainingTrainPoints -= as3hx.Compat.parseInt(x);
                    stat.train.value = x;
                }
                // Leftover points
                ...
                // Count trainable stats (not maxed, below old value) and split reamining points evenly{
                    ...
                    for (i in 0...primaryStats.length)
                    {
                        ...
                        if (stat.train.value < stat.train.max && stat.train.value < oldCoreStats[i])
                        {
                            x = as3hx.Compat.parseInt(remainingTrainPoints / n + 0.999);  // round up  
                            // don't train over max or old value
                            x = Math.min(Math.min(x, stat.train.max - stat.train.value), oldCoreStats[i] - stat.train.value);
                            if (x > 0)
                            {
                                stat.train.value += x;
                                remainingTrainPoints -= as3hx.Compat.parseInt(x);
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.027)
            {
                if (player.hasStatusEffect(StatusEffects.Exgartuan))
                {
                    ...
                    {
                        player.changeStatusValue(StatusEffects.Exgartuan, 3, 0);
                    }
                    else
                    {
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.028)
            {
                ...
                {
                    flags[kFLAGS.KNOWS_SHEILA_NAME] = 0;
                }
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.029)
            {
                ...
                if (flags[kFLAGS.MARRIAGE_FLAG] != null)
                {
                    sceneHunter.marry(flags[kFLAGS.MARRIAGE_FLAG]);
                }  //will add the current spouse to the flag  
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.031)
            {
                ...
                if (HelSpawnScene.incestEnabled() && flags[kFLAGS.HAD_FIRST_HELSPAWN_TALK] != null)
                
                //printed only for "our" people in debug version{
            ...
            // reclaiming prison flags just in case{
                ...
                for (prisonFlag in kFLAGS.UNKNOWN_FLAG_NUMBER_02141...kFLAGS.NADIA_FOLLOWER + 1)
                {
                    flags[prisonFlag] = 0;
                }
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.036)
            {
                ...
                if (player.statusEffectv1(StatusEffects.Kindra) >= 150)
                {
                    player.changeStatusValue(StatusEffects.Kindra, 1, 150);
                }
            ...
            // clones retroactive fix{
                ...
                var cloneMax : Int = as3hx.Compat.parseInt(Soulforce.clones.length - 1);
                while (cloneMax >= 0)
                {
                    if (player.hasStatusEffect(Soulforce.clones[cloneMax]))
                    {
                        for (cloneNum in 0...cloneMax)
                        {
            ...
            {
                if (flags[kFLAGS.LOPPE_KIDS_LIMIT] == 0)
                {
                    flags[kFLAGS.LOPPE_KIDS_LIMIT] = 8;
                }
            ...
            {
                if (player.isRace(Races.CERBERUS))
                {
                ...
                if (player.hasPerk(PerkLib.TransformationImmunityAtlach))
                {
                    ...
                    if (player.isRace(Races.ATLACH_NACHA))
                    {
                ...
                if (player.hasPerk(PerkLib.TransformationImmunityFairy))
                {
            ...
            {
                if (player.hasPerk(PerkLib.HellfireCoat) && !player.hasPerk(PerkLib.TransformationImmunity2))
                {
                    player.createPerk(PerkLib.TransformationImmunity2, 3, 0, 0, 0);
                }
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.045)
            {
                ...
                if (flags[kFLAGS.DEFEATED_ZETAZ] == 2)
                {
                    player.superPerkPoints++;
                }
                if (flags[kFLAGS.DEMON_LABORATORY_DISCOVERED] == 2)
                {
                    player.superPerkPoints++;
                }
                if (flags[kFLAGS.LETHICE_DEFEATED] == 2)
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.049)
            {
                if (player.hasKeyItem("Pocket Watch") >= 0)
                {
                    ...
                    if (player.hasPerk(PerkLib.ElementalConjurerMindAndBodyResolveEx))
                    {
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.050)
            {
                ...
                {
                    var acv : Float = 0;
                    if (player.hasPerk(PerkLib.ElementalContractRank4) || (player.hasPerk(PerkLib.DaoOfTheElements)))
                    {
                        acv += 1;
                    }
                    if (player.hasPerk(PerkLib.ElementalContractRank8) || (player.hasPerk(PerkLib.DaoOfTheElements) && player.perkv1(PerkLib.DaoOfTheElements) == 2))
                    {
                        acv += 1;
                    }
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.051)
            {
                ...
                {
                    var capacityIncrease : Float = player.statusEffectv2(StatusEffects.ElementalEnergyConduits) * 0.5;
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.054)
            {
                ...
                {
                    var costback1 : Float = (flags[kFLAGS.IMPROVED_PERMANENT_GOLEMS_BAG] - SceneLib.campMakeWinions.maxPermanentImprovedStoneGolemsBagSize());
                ...
                {
                    var costback2 : Float = (flags[kFLAGS.PERMANENT_STEEL_GOLEMS_BAG] - SceneLib.campMakeWinions.maxPermanentSteelGolemsBagSize());
                ...
                {
                    var costback3 : Float = (flags[kFLAGS.IMPROVED_PERMANENT_STEEL_GOLEMS_BAG] - SceneLib.campMakeWinions.maxPermanentImprovedSteelGolemsBagSize());
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.057)
            {
                ...
                if (player.eggs() >= 40)
                {
                    player.buff("EggFever").setStat("minlust", 0.2).withText("Egg Fever");
                }
                else if (player.eggs() >= 20)
                {
                    player.buff("EggFever").setStat("minlust", 0.1).withText("Egg Fever");
                }
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.060)
            {
                ...
                var pigmentStock : Dynamic = Crafting.pigmentStock;
                for (s/* AS3HX WARNING could not determine type for var: s exp: ECall(EIdent(keys),[EIdent(pigmentStock)]) type: null */ in keys(pigmentStock))
                {
                    if (s.indexOf(" and ") > 0)
                    {
                        var c1 : String = s.substring(0, s.indexOf(" and "));
                        var c2 : String = s.substring(s.indexOf(" and ") + 5);
                        SceneLib.crafting.addPigment(c1, Reflect.field(pigmentStock, Std.string(s)));
                        SceneLib.crafting.addPigment(c2, Reflect.field(pigmentStock, Std.string(s)));
                        Reflect.deleteField(pigmentStock, Std.string(s));
                    }
                }
                // Fix "x and y" pigments in knowledge
                var ingredientKnowledge : Dynamic = Crafting.ingredientKnowledge;
                for (item/* AS3HX WARNING could not determine type for var: item exp: ECall(EIdent(keys),[EIdent(ingredientKnowledge)]) type: null */ in keys(ingredientKnowledge))
                {
                    var pigmentKnowledge : Dynamic = Reflect.field(Reflect.field(ingredientKnowledge, Std.string(item)), Std.string(AlchemyLib.RT_PIGMENT));
                    if (pigmentKnowledge == null)
                    {
                        continue;
                    }
                    for (s/* AS3HX WARNING could not determine type for var: s exp: ECall(EIdent(keys),[EIdent(pigmentKnowledge)]) type: null */ in keys(pigmentKnowledge))
                    {
                        if (s.indexOf(" and ") > 0)
                        {
                            c1 = s.substring(0, s.indexOf(" and "));
                            c2 = s.substring(s.indexOf(" and ") + 5);
                            Reflect.setField(pigmentKnowledge, Std.string(c1), true);
                            Reflect.setField(pigmentKnowledge, Std.string(c2), true);
                            Reflect.deleteField(pigmentKnowledge, Std.string(s));
                        }
                    }
                }
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.070)
            {
                ...
                if (player.hasPerk(PerkLib.Lycanthropy))
                {
                    var ngM : Float = (player.newGamePlusMod() + 1);
                    ...
                    player.statStore.replaceBuffObject({
                                str.mult : bonusStats2 * ngM,
                                tou.mult : bonusStats2 * 0.6 * ngM,
                                spe.mult : bonusStats2 * 0.4 * ngM,
            ...
            {
                if (flags[kFLAGS.TAMED_06_NAME] != 0)
                {
                    flags[kFLAGS.TAMED_06_NAME] = 0;
                }
                if (flags[kFLAGS.TAMED_07_NAME] != 0)
                {
                    flags[kFLAGS.TAMED_07_NAME] = 0;
                }
                if (flags[kFLAGS.TAMED_02_NAME] != 0)
                {
                    flags[kFLAGS.TAMED_02_NAME] = 0;
                }
                if (flags[kFLAGS.TAMED_03_NAME] != 0)
                {
                    flags[kFLAGS.TAMED_03_NAME] = 0;
                }
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.09)
            {
                ...
                {
                    inventory.takeItem(weapons.D_FLAIL, promptSaveUpdate);
                }
                flags[kFLAGS.MOD_SAVE_VERSION] = 36.09;
            }
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.10)
            {
                if (SceneLib.exploration.counters.explore > 0 && flags[kFLAGS.ALVINA_FOLLOWER] < 1)
                {
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.11)
            {
                ...
                {
                    var a1 : Float = player.statusEffectv4(StatusEffects.PCClone);
                    var a2 : Float = player.statusEffectv3(StatusEffects.PCClone);
                    if (a2 > 0)
                    {
                        player.addStatusValue(StatusEffects.PCClone, 3, -a2);
                    }
            ...
            {
                if (player.hasPerk(PerkLib.TransformationImmunity2) && player.perkv1(PerkLib.TransformationImmunity2) == 1 && player.hairColor != "midnight purple")
                {
                ...
                if (player.hasPerk(PerkLib.TransformationImmunity2) && player.hasPerk(PerkLib.Immortality))
                {
                    ...
                    if (player.hairColor != "immaculate white")
                    {
                        player.hairColor = "immaculate white";
                    }
                }
                if (player.hasPerk(PerkLib.TransformationImmunity2) && player.perkv1(PerkLib.TransformationImmunity2) == 3 && player.hairColor != "midnight black")
                {
                    player.hairColor = "midnight black";
                }
                if (player.hasPerk(PerkLib.TransformationImmunity) && !InCollection(player.hairColor1, GargoyleRace.GargoyleHairColors))
                {
                    transformations.HairChangeColor(GargoyleRace.GargoyleHairColors).applyEffect(false);
                }
                ...
                if (player.hasPerk(PerkLib.TransformationImmunity2) && player.perkv1(PerkLib.TransformationImmunity2) == 2 && player.hairColor != "purple")
                {
                ...
                if (player.hasPerk(PerkLib.BlessingOfTheAncestorTree) && player.hairColor != "golden blonde")
                {
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.16)
            {
                ...
                var currentWeakness : Float = libStat.valueOfBuff("Weakened");
                if (currentWeakness > 0)
                {
                    libStat.removeBuff("Weakened");
                }
                flags[kFLAGS.MOD_SAVE_VERSION] = 36.16;
            }
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.17)
            {
                if (player.statStore.hasBuff("Feeding Euphoria"))
                {
                    player.buff("Feeding Euphoria").remove();
                }
                if (player.statStore.hasBuff("Milking Euphoria"))
                {
                    player.buff("Milking Euphoria").remove();
                }
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.18)
            {
                ...
                {
                    player.addStatusValue(StatusEffects.BlessedItemAtTheLake, 1, 1);
                }
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.36)
            {
                ...
                {
                    if (JourneyToTheEast.AhriStatsToPerksConvertCounter == 1 || JourneyToTheEast.AhriStatsToPerksConvertCounter == 3 || JourneyToTheEast.AhriStatsToPerksConvertCounter == 5)
                    {
                        JourneyToTheEast.AhriStatsToSuperPerksConvertCounter += 1;
                    }
                    if (JourneyToTheEast.AhriStatsToPerksConvertCounter > 1)
                    {
                        if (JourneyToTheEast.AhriStatsToPerksConvertCounter > 3)
                        {
                            player.superPerkPoints += 2;
                        }
                        else
                        {
                            player.superPerkPoints += 1;
                ...
                if (JourneyToTheEast.EvelynnPerksToStatsConvertCounter > 0)
                {
                    JourneyToTheEast.EvelynnCoreLimitBreakerCounter += JourneyToTheEast.EvelynnPerksToStatsConvertCounter;
                }
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.42)
            {
                ...
                if (player.tailType == Tail.ARIGEAN_RED)
                {
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.47)
            {
                ...
                if (player.racialScore(Races.DRACULA) >= 22 && !player.hasStatusEffect(StatusEffects.Familiar))
                {
                    player.createStatusEffect(StatusEffects.Familiar, 0, 0, 0, 0);
                }
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.48)
            {
                ...
                if (player.tailType == Tail.ARIGEAN_PRINCESS && !player.hasPerk(PerkLib.TransformationImmunity2))
                {
            ...
            {
                if (player.racialScore(Races.DRACULA) >= 22 && !player.hasPerk(PerkLib.SoulDrinker))
                {
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.51)
            {
                ...
                if (player.hasMutation(IMutationsLib.DraconicBonesIM) || player.hasMutation(IMutationsLib.DraconicHeartIM) || player.hasMutation(IMutationsLib.DraconicLungIM))
                {
                    ...
                    if (player.hasMutation(IMutationsLib.DraconicLungIM))
                    {
                        ...
                        {
                            if (player.perkv1(IMutationsLib.DraconicLungIM) > player.perkv1(IMutationsLib.DrakeLungsIM))
                            {
                                player.addPerkValue(IMutationsLib.DrakeLungsIM, 1, player.perkv1(IMutationsLib.DraconicLungIM) - player.perkv1(IMutationsLib.DrakeLungsIM));
                            }
                        }
                        else
                        {
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.54)
            {
                ...
                if (!player.hasStatusEffect(StatusEffects.TempleOfTheDivineTracker2) && player.hasStatusEffect(StatusEffects.TempleOfTheDivineTracker))
                {
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.55)
            {
                ...
                if (player.hasPerk(PerkLib.Soulless))
                {
                    ...
                    if (flags[kFLAGS.AMILY_FOLLOWER] == 1)
                    {
                        ...
                        if (player.hasStatusEffect(StatusEffects.CombatFollowerAmily))
                        {
            ...
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.57)
            {
                ...
                if (Garden.PotionsBagSlot01Potion == "Poultrice")
                {
                    Garden.PotionsBagSlot01Potion = "Poultice";
                }
            }
            if (flags[kFLAGS.MOD_SAVE_VERSION] < 36.58)
            {
                ...
                if (flags[kFLAGS.TOOK_EMBER_EGG] > 0)
                {
    ...
    {
        var dildoId : Int = -1;
        var counter : Float = player.keyItems.length;
        ...
        while (counter > 0)
        {
            ...
            if (player.keyItems[counter].keyName == "Deluxe Dildo")
            {
                if (dildoId == -1)
                {
                    dildoId = as3hx.Compat.parseInt(counter);
                }
                else
                {
                    player.keyItems.splice(counter, 1);
                }
            }
        }
    }
