...
class PlayerAppearance extends BaseContent
{
    ...
    private function inchesOrCentimetres(args : Array<Dynamic> = null) : String
    {
        return Measurements.inchesOrCentimetres.apply(null, args);
    }
    private function shortSuffix(args : Array<Dynamic> = null) : String
    {
        return Measurements.shortSuffix.apply(null, args);
    }
    private function init() : Void
    {
    }
    public function appearance(debug : Bool = false) : Void
    //Determine race type:
    {
        ...
        {
            outputText("<font size=\"36\" face=\"Georgia\"><u>Body Parts Editor</u></font>\n");
        }
        ...
        if (!player.hasPerk(PerkLib.ElementalBody))
        {
            ...
            var wingsDescription : String = describeWings();
            outputText((wingsDescription != null) ? "[pg]" + wingsDescription : "");
            var rearBodyDescription : String = describeRearBody();
            outputText((rearBodyDescription != null) ? "[pg]" + rearBodyDescription : "");
            var tailDescription : String = describeTail();
            outputText((tailDescription != null) ? " " + tailDescription : "");
        }
        outputText("[pg]" + describeBreasts());
        var crotchDesc : String = describeCrotch();
        outputText((crotchDesc != null) ? " " + crotchDesc : "");
        if (player.cocks.length > 1)
        {
            outputText("[pg]" + describeCocks());
            outputText("[pg]" + describeBalls());
        }
        else if (player.hasCock() || player.hasBalls())
        {
            outputText("[pg]" + Utils.mergeSentences([describeCocks(), describeBalls()]));
        }
        var pussyDesc : String = describePussies();
        outputText((pussyDesc != null) ? "[pg]" + pussyDesc : "");
        var noSexString : String = ((player.cockTotal() == 0 && player.vaginas.length == 0) ? "You have a curious lack of any sexual endowments." : "");
        if (player.ass || noSexString != null)
        {
            outputText("[pg]" + Utils.mergeSentences([noSexString, describeAsshole()]));
        }
        var piercingsDesc : String = describePiercings();
        outputText((piercingsDesc != null) ? "[pg]" + piercingsDesc : "");
        var specialCasesDescription : String = describeSpecialCases();
        outputText((specialCasesDescription != null) ? "[pg]" + specialCasesDescription : "");
        var pregnancyDescription : String = describePregnancy();
        outputText((pregnancyDescription != null) ? "[pg]" + pregnancyDescription : "");
        // EQUIPMENT
        ...
        {
            return;
        }  // don't print buttons if opened from debug menu  
        
        menu();
        addButton(0, "Next", playerMenu);
        if (player.hasPerk(PerkLib.RacialParagon))
        {
            addButton(1, "Set Race.", ApexRaceSetting);
        }
        ...
        {
            addButton(4, "Were/Human", HumanFormDisplay);
        }
    ...
    public function WeaponDisplay() : Void
    {
        ...
        if (flags[kFLAGS.WEAPON_DISPLAY_FLAG] == 0)
        {
            outputText("[pg]Display Style: <b>Melee</b>");
        }
        if (flags[kFLAGS.WEAPON_DISPLAY_FLAG] == 1)
        {
            outputText("[pg]Display Style: <b>Ranged</b>");
        }
    ...
    public function BoobDisplay() : Void
    {
        ...
        if (flags[kFLAGS.BOOB_DISPLAY_FLAG] == 0)
        {
            outputText("[pg]Display Style: <b>Yes</b>");
        }
        if (flags[kFLAGS.BOOB_DISPLAY_FLAG] == 1)
        {
            outputText("[pg]Display Style: <b>No</b>");
        }
        mainView.hideAllMenuButtons();
        menu();
    ...
    public function WeaponDisplaySwitch(Display : Float) : Void
    {
        flags[kFLAGS.WEAPON_DISPLAY_FLAG] = Display;
        WeaponDisplay();
    }
    
    public function BoobDisplaySwitch(Display : Float) : Void
    {
    ...
    public function ApexRaceSetting() : Void
    {
        ...
        var races : Array<Dynamic> = [];
        for (x/* AS3HX WARNING could not determine type for var: x exp: EField(EIdent(Races),AllRacesByName) type: null */ in Races.AllRacesByName)
        {
            races.push({
                        label : x.name,
                        race : x
        ...
        // fill the races
        CoC.instance.showComboBox(races, "Select race", function(item : Dynamic) : Void
                // item is selected
                {
    ...
    public function ApexRaceDisplayTextUpdate() : Void
    {
        ...
        {
            selectedracetext = player.racialParagonSelectedRace().name;
        }
    ...
    public function describeGear() : String
    {
        ...
        {
            if (player.headjewelryPerk == "Helmet")
            {
                gearDesc += " On your head is " + player.headjewelryName + ".";
            }
            else
            {
                gearDesc += " In your hair is " + player.headjewelryName + ".";
            }
        }
        ...
        if (player.hasKeyItem("Gleipnir Collar") >= 0)
        {
            if (player.necklaceName != "nothing")
            {
                gearDesc += " Benath it is ";
            }
            else
            {
                gearDesc += " On your neck is ";
            }
        ...
        if (player.jewelryName != "nothing")
        {
            ...
            if (player.jewelryName3 != "nothing")
            {
                gearDesc += " and on another finger is a " + player.jewelryName3 + "";
            }
            if (player.jewelryName2 != "nothing")
            {
                gearDesc += ", on one of your left hand fingers is a " + player.jewelryName2 + "";
            }
            if (player.jewelryName4 != "nothing")
            {
                gearDesc += " and on another finger is a " + player.jewelryName4 + "";
            }
            gearDesc += ".";
        }
        gearDesc += "</b>";
    ...
    public function describeMoney() : String
    {
        ...
        if (player.gems == 0)
        {
            moneyDesc += "<b>Your money-purse is devoid of any currency.</b>";
        }
        if (player.gems > 1)
        {
            moneyDesc += "<b>You have " + addComma(Math.floor(player.gems)) + " shining gems, collected in your travels.</b>";
        }
        if (player.gems == 1)
        {
            moneyDesc += "<b>You have " + addComma(Math.floor(player.gems)) + " shining gem, collected in your travels.</b>";
        }
    ...
    // story.display("race");
    {
        ...
        else
        {
            ...
            var genderlessRace : Array<Dynamic> = ["half cow-morph", "half cow-girl", "cow-girl", "cow-girl", "cow-morph", "minotaur", "half-minotaur", "alraune", "liliraune", "half unicorn", "unicorn", "unicornkin", "half alicorn", "alicorn", "alicornkin", "true alicorn", "half bicorn", "bicorn", "bicornkin", "half nightmare", "nightmare", "nightmarekin", "true nightmare"];
            if (!(Lambda.indexOf(genderlessRace, pcrace) >= 0))
            {
        ...
        if (player.isBiped() || player.isNaga())
        {
            if (player.thickness > 70)
            {
                ...
                if (player.hips.type < 6)
                {
                    ...
                    {
                        lowerBodyDesc += " that blend into your pillar-like waist, and";
                    }
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (player.hips.type >= 20)
                    {
                        ...
                        {
                            lowerBodyDesc += "vacant groin plenty of room";
                        }
                        lowerBodyDesc += ", and";
                    }
                }
    ...
    public function describePregnancy() : String
    {
        ...
        {
            if (player.isPregnant() && Reflect.field(womb, "type") == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS)
            {
                pregnancyDesc += "<b>";
                //Compute size
                temp = player.statusEffectv3(StatusEffects.Eggs) + player.statusEffectv2(StatusEffects.Eggs) * 10;
                if (Reflect.field(womb, "incubation") <= sceneHunter.adjustPregEventTimer(50, Reflect.field(womb, "type")) && Reflect.field(womb, "incubation") > sceneHunter.adjustPregEventTimer(20, Reflect.field(womb, "type")))
                {
                ...
                if (Reflect.field(womb, "incubation") <= sceneHunter.adjustPregEventTimer(20, Reflect.field(womb, "type")))
                {
                    pregnancyDesc += "Your swollen pregnant belly is as large as a ";
                    if (temp < 10)
                    {
            ...
            else if (player.buttPregnancyType == PregnancyStore.PREGNANCY_SATYR && (!player.isPregnant() || player.buttPregnancyIncubation < Reflect.field(womb, "incubation")))
            {
                ...
                else
                {
                    ...
                    else if (InCollection(Reflect.field(womb, "type"), PregnancyStore.PREGNANCY_IZMA, PregnancyStore.PREGNANCY_MOUSE, PregnancyStore.PREGNANCY_AMILY, PregnancyStore.PREGNANCY_EMBER, PregnancyStore.PREGNANCY_BENOIT, PregnancyStore.PREGNANCY_COTTON, PregnancyStore.PREGNANCY_URTA, PregnancyStore.PREGNANCY_BEHEMOTH, PregnancyStore.PREGNANCY_ZENJI) || (Reflect.field(womb, "type") == PregnancyStore.PREGNANCY_JOJO && (JojoScene.monk <= 0 || flags[kFLAGS.JOJO_BIMBO_STATE] == 3)))
                    {
            ...
            else if (player.isButtPregnant())
            {
                ...
                else if (player.buttPregnancyType != PregnancyStore.PREGNANCY_GOO_STUFFED)
                {
                    pregnancyDesc += "<b>You're stuffed so full with eggs that your belly looks obscenely distended, huge and weighted with the gargantuan eggs crowding your gut. They make your gait a waddle and your gravid tummy wobble obscenely.</b>";
                }
            }
            //URTA PREG
            else if ((Reflect.field(womb, "type") == PregnancyStore.PREGNANCY_URTA))
            {
                if (Reflect.field(womb, "incubation") <= sceneHunter.adjustPregEventTimer(432, Reflect.field(womb, "type")) && Reflect.field(womb, "incubation") > sceneHunter.adjustPregEventTimer(360, Reflect.field(womb, "type")))
                {
                    pregnancyDesc += "<b>Your belly is larger than it used to be.</b>\n";
                }
                else if (Reflect.field(womb, "incubation") > sceneHunter.adjustPregEventTimer(288, Reflect.field(womb, "type")))
                {
                    pregnancyDesc += "<b>Your belly is more noticeably distended. You're pretty sure it's Urta's.</b>";
                }
                else if (Reflect.field(womb, "incubation") > sceneHunter.adjustPregEventTimer(216, Reflect.field(womb, "type")))
                {
                    pregnancyDesc += "<b>The unmistakable bulge of pregnancy is visible in your tummy, and the baby within is kicking nowadays.</b>";
                }
                else if (Reflect.field(womb, "incubation") > sceneHunter.adjustPregEventTimer(144, Reflect.field(womb, "type")))
                {
                ...
                else if (Reflect.field(womb, "incubation") > sceneHunter.adjustPregEventTimer(72, Reflect.field(womb, "type")))
                {
                ...
                else if (Reflect.field(womb, "incubation") > sceneHunter.adjustPregEventTimer(48, Reflect.field(womb, "type")))
                {
                    pregnancyDesc += "<b>Your stomach is painfully distended by your pregnancy, making it difficult to walk normally.</b>";
                }
                else
            ...
            else if (Reflect.field(womb, "type") == PregnancyStore.PREGNANCY_FAERIE)
            ...
            //Belly size remains constant throughout the pregnancy{
                ...
                else if (Reflect.field(womb, "incubation") <= sceneHunter.adjustPregEventTimer(140, Reflect.field(womb, "type")))
                {
                    pregnancyDesc += "It feels like it’s full of thick syrup or jelly.</b>";
                }
                else
                {
                    pregnancyDesc += "It still feels like there’s a solid ball inside your womb.</b>";
                }
            }
            else if (Reflect.field(womb, "type") == PregnancyStore.PREGNANCY_HARPY_HATCHING)
            {
                pregnancyDesc += "You eagerly await the hatching of your harpy egg at camp. You feel it should hatch ";
                if (7 - SophieFollowerScene.HarpyEggDay > 0)
                {
                    pregnancyDesc += "in " + (7 - SophieFollowerScene.HarpyEggDay) + " days.";
                }
                else
                {
                    pregnancyDesc += "tomorrow.";
                }
            }
            else if (Reflect.field(womb, "incubation") <= sceneHunter.adjustPregEventTimer(336, Reflect.field(womb, "type")) && Reflect.field(womb, "incubation") > sceneHunter.adjustPregEventTimer(280, Reflect.field(womb, "type")))
            {
                pregnancyDesc += "<b>Your belly is larger than it used to be.</b>";
            }
            else if (Reflect.field(womb, "incubation") > sceneHunter.adjustPregEventTimer(216, Reflect.field(womb, "type")))
            ...
            else if (Reflect.field(womb, "incubation") > sceneHunter.adjustPregEventTimer(180, Reflect.field(womb, "type")))
            {
                pregnancyDesc += "<b>The unmistakable bulge of pregnancy is visible in your tummy.</b>";
            }
            else if (Reflect.field(womb, "incubation") > sceneHunter.adjustPregEventTimer(120, Reflect.field(womb, "type")))
            {
            ...
            else
            {
                ...
                else if (Reflect.field(womb, "type") == PregnancyStore.PREGNANCY_HARPY_EGGS)
                {
    ...
    public function describeBreasts() : String
    {
        ...
        if (player.breastRows.length == 1)
        {
            ...
            breastsDesc += Measurements.shortSuffix(as3hx.Compat.parseInt(player.nippleLength * 10) / 10) + " ";
    ...
    public function describeCocks() : String
    {
        ...
        if (player.hasCock())
        {
            ...
            {
                cockDesc += " Every now and again slimy worms coated in spunk slip partway out of your " + player.multiCockDescriptLight() + ", tasting the air like tongues of snakes";
            }
        }
    ...
    {
        var cockDesc : String = "";
        if (cock_index >= player.cocks.length)
        {
            return cockDesc;
        }
        if (rando == 0)
        ...
        {
            cockDesc += " in diameter.";
        }
        cockDesc += " You estimate its area to be about " + Math.round(player.cockArea(cock_index)) + ".";
        // What flavor of cock do you have?
        var _sw4_ = (player.cocks[cock_index].cockType);        

        switch (_sw4_)
        {
            case CockTypesEnum.HORSE:cockDesc += " It's mottled black and brown in a very animalistic pattern. The 'head' of its shaft flares proudly, just like a horse's.";
            case CockTypesEnum.KIRIN:cockDesc += " It's a glowing pale blue in color. The 'head' of its shaft flares proudly, just like a horse's.";
            case CockTypesEnum.DOG:cockDesc += " It is shiny, pointed, and covered in veins, just like a large dog's cock.";
            case CockTypesEnum.WOLF:cockDesc += " It is shiny red, pointed, and covered in veins, just like a large wolf's cock.";
            case CockTypesEnum.RAIJU:cockDesc += " It's a deep, glowing electric blue in color, pointed, and covered in veins, just like a large raiju's cock.";
            case CockTypesEnum.FOX:cockDesc += " It is shiny, pointed, and covered in veins, just like a large fox's cock.";
            case CockTypesEnum.DEMON:cockDesc += " The crown is ringed with a circle of rubbery protrusions that grow larger as you get more aroused. The entire thing is shiny and covered with tiny, sensitive nodules that leave no doubt about its demonic origins.";
            case CockTypesEnum.TENTACLE:cockDesc += " The entirety of its green surface is covered in perspiring beads of slick moisture. It frequently shifts and moves of its own volition, the slightly oversized and mushroom-like head shifting in coloration to purplish-red whenever you become aroused.";
            case CockTypesEnum.STAMEN:cockDesc += " It is dark green, tampered, and crowned by several colorful balls near the tip that secrete pollen when aroused.";
            case CockTypesEnum.CAT:cockDesc += " It ends in a single point, much like a spike, and is covered in small, fleshy barbs. The barbs are larger at the base and shrink in size as they get closer to the tip. Each of the spines is soft and flexible, and shouldn't be painful for any of your partners.";
            case CockTypesEnum.LIZARD:cockDesc += " It's a deep, iridescent purple in color. Unlike a human penis, the shaft is not smooth, and is instead patterned with multiple bulbous bumps.";
            case CockTypesEnum.CAVE_WYRM:cockDesc += " It's a deep, glowing neon blue in color. Unlike a human penis, the shaft is not smooth, and is instead patterned with multiple bulbous bumps. It also regularly drips the acidic substance that serves as your cum. In the event you need to fertilise more fragile partner you can slightly alter the composition of your cum to prevent disolving their flesh.";
            case CockTypesEnum.CANCER:cockDesc += " It constantly drips and produce bubbles.";
            case CockTypesEnum.ANEMONE:cockDesc += " The crown is surrounded by tiny tentacles with a venomous, aphrodisiac payload. At its base a number of similar, longer tentacles have formed, guaranteeing that pleasure will be forced upon your partners.";
            case CockTypesEnum.KANGAROO:cockDesc += " It usually lies coiled inside a sheath, but undulates gently and tapers to a point when erect, somewhat like a taproot.";
            case CockTypesEnum.DRAGON:cockDesc += " With its tapered tip, there are few holes you wouldn't be able to get into. It has a strange, knot-like bulb at its base, but doesn't usually flare during arousal as a dog's knot would.";
            case CockTypesEnum.BEE:cockDesc += " It's a long, smooth black shaft that's rigid to the touch. Its base is ringed with a layer of " + shortSuffix(4) + " long soft bee hair. The tip has a much finer layer of short yellow hairs. The tip is very sensitive, and it hurts constantly if you don’t have bee honey on it.";
            case CockTypesEnum.PIG:cockDesc += " It's bright pinkish red, ending in a prominent corkscrew shape at the tip.";
            case CockTypesEnum.AVIAN:cockDesc += " It's a red, tapered cock that ends in a tip. It rests nicely in a sheath.";
            case CockTypesEnum.RHINO:cockDesc += " It's a smooth, tough pink colored and takes on a long and narrow shape with an oval shaped bulge along the center.";
            case CockTypesEnum.ECHIDNA:cockDesc += " It is quite a sight to behold, coming well-equipped with four heads.";
            case CockTypesEnum.RED_PANDA:cockDesc += " It lies protected in a soft, fuzzy sheath.";
            case CockTypesEnum.OOMUKADE:cockDesc += " It constantly drips with venom.";
            case CockTypesEnum.USHI_ONI:cockDesc += " It's starfish tipped shaft.";
            case CockTypesEnum.MINDBREAKER:cockDesc += " Hidden in a slit along with the rest of your male equipment, it constantly fills with your thick green slimy cum, to the point that your cum constantly leaks out of your genital slit, leaving a sticky trail of green \"slime\" in your wake.";
            default:
        }
        // Knot?
    ...
    public function describeBalls() : String
    {
        ...
        var ballsSizeDesc : String = " about " + num2Text(Math.round(player.ballSize)) + " " + ((Math.round(player.ballSize) == 1) ? "inch" : "inches") + " across";
        ...
        if (player.hasBalls())
        {
            if (player.hasStatusEffect(StatusEffects.Uniball))
            {
                ...
                {
                    ballsDesc += "Your [sack] clings tightly to your groin, holding [balls]," + ballsSizeDesc + ", snugly against you.";
                }
                else if (player.skinType == Skin.GOO)
                {
                    ballsDesc += "Your [sack] clings tightly to your groin, dripping and holding [balls]," + ballsSizeDesc + ", snugly against you.";
                }
            }
            else
            {
                ...
                {
                    ballsDesc += "A fuzzy [sack] filled with [ballsarticle]," + ballsSizeDesc + " each, swings low" + swingsWhere;
                }
                else if (player.hasCoatOfType(Skin.CHITIN))
                {
                    ballsDesc += "A chitin [sack] hugs your [balls]," + ballsSizeDesc + " each, tightly against your body.";
                }
                else if (player.isScaleCovered())
                {
                    ballsDesc += "A scaly [sack] hugs your [balls]," + ballsSizeDesc + " each, tightly against your body.";
                }
                else if (player.skinType == Skin.STONE)
                {
                    ballsDesc += "A stone-solid sack with [ballsarticle]," + ballsSizeDesc + " each, swings heavily" + swingsWhere;
                }
                else if (player.skinType == Skin.GOO)
                {
                    ballsDesc += "An oozing, semi-solid sack with [ballsarticle]," + ballsSizeDesc + " each, swings heavily" + swingsWhere;
                }
                else
                {
                    ballsDesc += "A [sack] with [ballsarticle]," + ballsSizeDesc + " each, swings heavily" + swingsWhere;
                }
            }
            ...
            if (player.hasStatusEffect(StatusEffects.Infested) && player.statusEffectv1(StatusEffects.Infested) == 5)
            {
                ballsDesc += " Across its surface, random lumps move rapidly around, further proof of your infested sack, and it's parasitic inhabitants that both boost and increase your cum production.";
            }
        }
        else if (player.hasStatusEffect(StatusEffects.Infested) && player.statusEffectv1(StatusEffects.Infested) == 5)
        {
            ballsDesc += "Deep within your prostate you feel the worms moving, constantly rubbing against your trigger button, keeping you aroused and constantly churning up cum to be spewed out at a moment's notice.";
        }
    ...
    public function describePussies() : String
    {
        ...
        if (player.vaginas.length > 0)
        {
            ...
            for (i in 0...player.vaginas.length)
            {
    ...
    public function describePussy(i : Int = 0) : String
    {
        ...
        if (player.vaginas.length == 1)
        {
            ...
            {
                pussyDesc += "You have two sets of vaginal lips, the first";
            }
            else
            {
        ...
        switch (vagType)
        {  //missing black sand trap, maybe melkie.  
            ...
            case VaginaClass.NAGA:
                ...
                if (player.isNaga())
                {
                    pussyDesc += " as your hot spot is now way further inside your deep canal, requiring quite a longer member to reach";
                }
        ...
        if (clitExists == 0)
        {
            pussyDesc += Measurements.shortSuffix(as3hx.Compat.parseInt(player.clitLength * 10) / 10) + " clit";
        }
        //Hymen check.
        ...
        else if ((player.lib >= 50 || player.lust >= 50) && (player.lib < 80 && player.lust < 80))
        ...
        else if (player.lib > 80 || player.lust > 80)
    ...
    public var new_ui : Bool = true;
    public function set_ui_version(bool : Bool, auto_run : Bool = false) : Void
    {
        new_ui = bool;
        if (auto_run)
        {
    ...
    private function race_clicked_display(race : Race = null, sortby : Int = 0) : Void
    {
        ...
        outputText(Utils.capitalizeFirstLetter((rtier != null) ? rtier.nameFor(body) : race.name) + "[/font]</b> racial scores (and bonuses to stats if applicable):\n");
        ...
        if (debug && race.debugFormNames().length > 0)
        {
            ...
            for (tfname/* AS3HX WARNING could not determine type for var: tfname exp: ECall(EField(EIdent(race),debugFormNames),[]) type: null */ in race.debugFormNames())
            {
                outputText(" ");
    ...
    private function RacialScores_new(clickedRace : Race = null, sortBy : Int = 0) : Void
    {
        ...
        var list : Array<Dynamic> = Races.AllRacesByName;
        var scrollPos : Int = 0;
        if (sortBy == 1)
        {
        ...
        var dummy : Void->Void = function() : Void
        {
        }
        
        var local_button : String->String->String->String->CoCButton = function(text : String = "Label", text_color : String = "black", hint : String = "", corner_text : String = "") : CoCButton
        {
            ...
            var local_button : CoCButton = new CoCButton({
                                                        square : true
                                                    }).show(text, dummy).hint(hint).disableIf(true).color(tColor).cornerLabel(corner_text)  //.disableIf(race==clickedRace)  ;
        ...
        for (i in 0...list.length)
        {
            ...
            // skip "old" races
            if (race.requirements.length == 0 || race.tiers.length == 0)
            {
                continue;
            }
            ...
            groupgrid.addElement(new CoCButton({
                                                        square : true
                                                    }).show(race.name, curry(race_clicked_display, race, sortBy)).hint(s).color(tColor).cornerLabel("" + rscore)  //.disableIf(race==clickedRace)  
            );
        ...
        mainView.linkHandler = function(event : String) : Void
                {
                    var parts : Array<Dynamic> = event.split(",");
                    var _sw5_ = (parts[0]);                    

                    switch (_sw5_)
                    {
                        case "race":
                            var clickedRace2 : Race = Race.byId(as3hx.Compat.parseInt(parts[1]));
                            if (clickedRace2 == clickedRace)
                            {
                                clickedRace2 = null;
                            }
                            RacialScores(clickedRace2, sortBy);
                        case "sort":
                            flags[kFLAGS.RACESORT_SETTING] = as3hx.Compat.parseInt(parts[1]);
                            RacialScores(clickedRace, as3hx.Compat.parseInt(parts[1]));
                        case "tf":
                            clearOutput();
                            Race.byId(as3hx.Compat.parseInt(parts[1])).takeForm(player, parts[2]);
        ...
        if (new_ui)
        {
            if (score > 0)
            {
                local_button("INTERNAL CHIMERICAL DISPOSITION", (player.hasPerk(PerkLib.RacialParagon)) ? "green" : "blue", (player.hasPerk(PerkLib.RacialParagon)) ? "- no buffs because of the Racial Paragon perk" : " (" + player.internalChimeraRating() + ") " + score + "\n(+" + (5 * score) + "% to Str racial multi / Tou / Spe / Int / Wis / Lib", score);
            }
        }
        else if (score >= 1)
        {
            ...
            if (player.hasPerk(PerkLib.RacialParagon))
            {
                outputText(" - no buffs because of the Racial Paragon perk");
            }
            else
            {
                outputText(" (+" + (5 * score) + "% to Str racial multi / Tou / Spe / Int / Wis / Lib");
            }
            outputText("[/font]");
        }
        else
        ...
        else if (player.hasCoatOfType(Skin.SCALES))
        {
            factor += 1;
        }
        ...
        if (scrollPos != 0)
        {
            mainView.scrollBar.value = scrollPos;
        }
        addButton(1, "Exit", playerMenu);
    ...
    private function RacialScores_old(clickedRace : Race = null, sortBy : Int = 0) : Void
    {
        ...
        for (i in 0...list.length)
        {
            ...
            printLink(Utils.capitalizeFirstLetter((rtier != null) ? rtier.nameFor(body) : race.name), "race," + race.id);
            outputText(": " + rscore);
            outputText("[/font]");
            if (rtier != null)
            {
        ...
        if (scrollPos != 0)
        {
            mainView.mainText.scrollV = scrollPos;
        }
        addButton(0, "Next", playerMenu);
    ...
    public function GenderForcedSetting() : Void
    {
        ...
        menu();
        addButton(0, "Next", playerMenu);
        if (flags[kFLAGS.MALE_OR_FEMALE] == 0)
        {
            addButtonDisabled(1, "Auto", "It's currently used setting option.");
        }
        else
        {
            addButton(1, "Auto", GenderForcedSettingAuto);
        }
        if (flags[kFLAGS.MALE_OR_FEMALE] == 1)
        {
            addButtonDisabled(2, "Male", "It's currently used setting option.");
        }
        else
        {
            addButton(2, "Male", GenderForcedSettingMale);
        }
        if (flags[kFLAGS.MALE_OR_FEMALE] == 2)
        {
            addButtonDisabled(3, "Female", "It's currently used setting option.");
        }
        else
        {
            addButton(3, "Female", GenderForcedSettingFemale);
        }
    }
    ...
    {
        var gargoyleMaterials : Array<Dynamic> = ["marble", "alabaster"];
    ...
    private function campActionsReflect() : Void
    {
        ...
        if (player.race() != player.startingRace)
        {
            outputText(" While you were formerly a " + player.startingRace + " you now are a " + player.race() + " for better or for worse and have decided to live this new life to its fullest.");
        }
    ...
    public function sockDescript(index : Int) : String
    {
        ...
        {
            cockSockDesc += "<b>Yo, this is an error.</b>";
        }
        return cockSockDesc;
    }
}
