...
class PerkManager
{
    ...
    private var arrayCache : Array<Dynamic>;
     ...
     */
    public function createPerk(ptype : PerkType, value1 : Float = 0, value2 : Float = 0, value3 : Float = 0, value4 : Float = 0) : Bool
     ...
     * @return {Boolean} True if perk is added. False if perk already exists or fails to add.
     */
    public function add(perk : PerkClass) : Bool
     ...
     */
    public function remove(ptype : PerkType) : Bool
    {
        if (this.has(ptype))
        {
            ;
     ...
     * @return
     */
    public function has(ptype : PerkType) : Bool
    {
        return this.perks[ptype.id] != null;
    }
    ...
    public function asArray() : Array<Dynamic>
    {
        ...
        {
            this.arrayCache = [];
            for (perk/* AS3HX WARNING could not determine type for var: perk exp: EField(EIdent(this),perks) type: null */ in this.perks)
            {
                this.arrayCache.push(perk);
            }
            this.arrayCache.sortOn("perkName");
        }
     ...
     */
    public function getPerkValue(ptype : PerkType, valueIdx : Float = 1) : Float
    {
     ...
     */
    public function addPerkValue(ptype : PerkType, valueIdx : Float = 1, bonus : Float = 0) : Void
    {
     ...
     */
    public function setPerkValue(ptype : PerkType, valueIdx : Float = 1, newNum : Float = 0) : Void
