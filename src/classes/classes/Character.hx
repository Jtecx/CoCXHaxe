...
class Character extends Creature
{
    ...
    public var pregnancyTypeText(get, never) : String;
    public var pregnancyIncubation(get, never) : Int;
    public var pregnancy2Type(get, never) : Int;
    public var pregnancy2TypeText(get, never) : String;
    public var pregnancy2Incubation(get, never) : Int;
    public var buttPregnancyType(get, never) : Int;
    public var buttPregnancyTypeText(get, never) : String;
    ...
    //Key items
    public var keyItems : Array<Dynamic>;
    ...
    public function modThickness(goal : Float, strength : Int = 1) : String
    {
        if (goal == thickness)
        {
            return "";
    ...
    {
        return facePart.hasBeard();
    }
    public function beard() : String
    {
        return facePart.beard();
    }
    public function hasMuzzle() : Bool
    {
        return facePart.hasMuzzle();
    }
    public function hasBeak() : Bool
    {
        return facePart.hasBeak();
    }
    public function face() : String
    {
        return facePart.describe();
    }
    public function faceDesc() : String
    {
        return facePart.describeMF();
    }
    public function faceDescArticle() : String
    {
        return facePart.describeMF(true);
    }
    ...
    {
        return _pregnancyType > 0 || _pregnancy2Type > 0;
    }
    public function canGetPregnant() : Bool
    {
        return (vaginas.length > 0 && _pregnancyType == 0) || (vaginas.length > 1 && _pregnancy2Type == 0);
    }
    public function hasVisiblePregnancy() : Bool
    {
        return ((pregnancyIncubation > 0 && pregnancyIncubation <= CoC.instance.gameSettings.sceneHunter_inst.adjustPregEventTimer(180, pregnancyType)) || (pregnancy2Incubation > 0 && pregnancy2Incubation <= CoC.instance.gameSettings.sceneHunter_inst.adjustPregEventTimer(180, pregnancy2Type)));
    }
    public function hasVeryVisiblePregnancy() : Bool
    {
        return ((pregnancyIncubation > 0 && pregnancyIncubation <= CoC.instance.gameSettings.sceneHunter_inst.adjustPregEventTimer(100, pregnancyType)) || (pregnancy2Incubation > 0 && pregnancy2Incubation <= CoC.instance.gameSettings.sceneHunter_inst.adjustPregEventTimer(100, pregnancyType)));
    }
    public function hasNonVisiblePregnancy() : Bool
    {
        return ((pregnancyIncubation > CoC.instance.gameSettings.sceneHunter_inst.adjustPregEventTimer(180, pregnancyType)) && (pregnancy2Incubation > CoC.instance.gameSettings.sceneHunter_inst.adjustPregEventTimer(180, pregnancy2Type))) || ((pregnancyIncubation > CoC.instance.gameSettings.sceneHunter_inst.adjustPregEventTimer(180, pregnancyType)) && (pregnancy2Incubation == 0)) || ((pregnancyIncubation == 0) && (pregnancy2Incubation > CoC.instance.gameSettings.sceneHunter_inst.adjustPregEventTimer(180, pregnancy2Type)));
    }
    ...
    public function knockUpForce(type : Int = 0, incubation : Int = 0, womb : Int = 0, bypassSH : Int = 0) : Void
    {
        ...
        {
            _pregnancyType = type;
            _pregnancyIncubation = ((type == 0) ? 0 : incubation);
        }
        else
        {
            _pregnancy2Type = type;
            _pregnancy2Incubation = ((type == 0) ? 0 : incubation);
        }
    }
    ...
    public function buttKnockUpForce(type : Int = 0, incubation : Int = 0, bypassSH : Int = 0) : Void
    {
        ...
        _buttPregnancyIncubation = ((type == 0) ? 0 : incubation);
    }
    ...
    public function pregnancyAdvance() : Bool
    {
        ...
        if (_pregnancy2Incubation > 0)
        {
            _pregnancy2Incubation--;
        }
        if (_pregnancy2Incubation < 0)
        {
            _pregnancy2Incubation = 0;
        }
    ...
    {
        for (i in 0...keyItems.length)
        {
            if (keyItems[i].keyName == keyName)
            {
                return keyItems[i];
            }
        }
    ...
    //Create a keyItem
    public function createKeyItem(keyName : String, value1 : Float, value2 : Float, value3 : Float, value4 : Float) : Void
    {
        ...
        //trace("New Key Item Belongs at the beginning! " + keyName);{
            ...
            as3hx.Compat.arraySplice(keyItems, 0, 0, [newKeyItem]);
            arrayed = true;
            keySlot = 0;
        }
        ...
        //trace("New Key Item using alphabetizer! " + keyName);{
            ...
            while (counter > 0 && !arrayed)
            {
                ...
                if (Reflect.field(keyItems, Std.string(counter)).keyName > keyName)
                ...
                //If the earlier slot is earlier than new key && a real spot{
                    ...
                    //If the earlier slot is earlier slot in!{
                        ...
                        {
                            arrayed = true;
                            as3hx.Compat.arraySplice(keyItems, counter, 0, [newKeyItem]);
                            keySlot = counter;
                        }
                    }
        ...
        Reflect.setField(keyItems, Std.string(keySlot), keyName).keyName;
        Reflect.setField(keyItems, Std.string(keySlot), value1).value1;
        Reflect.setField(keyItems, Std.string(keySlot), value2).value2;
        Reflect.setField(keyItems, Std.string(keySlot), value3).value3;
        Reflect.setField(keyItems, Std.string(keySlot), value4).value4;
    }
    ...
    public function removeKeyItem(itemName : String) : Void
    {
        ...
        {
            counter--;
            if (Reflect.field(keyItems, Std.string(counter)).keyName == itemName)
            {
    ...
    public function addKeyValue(keyItemName : String, statusValueNum : Float = 1, newNum : Float = 0) : Void
    {
        var counter : Float = keyItems.length;
        //Various Errors preventing action
        if (keyItems.length <= 0)
        ...
        while (counter > 0)
        {
            ...
            if (Reflect.field(keyItems, Std.string(counter)).keyName == keyItemName)
            {
                ...
                if (statusValueNum == 1)
                {
                    Reflect.field(keyItems, Std.string(counter)).value1 += newNum;
                }
                if (statusValueNum == 2)
                {
                    Reflect.field(keyItems, Std.string(counter)).value2 += newNum;
                }
                if (statusValueNum == 3)
                {
                    Reflect.field(keyItems, Std.string(counter)).value3 += newNum;
                }
                if (statusValueNum == 4)
                {
                    Reflect.field(keyItems, Std.string(counter)).value4 += newNum;
                }
                return;
            }
        }
    ...
    public function keyItemvX(keyItemName : String, keyValue : Int) : Float
    {
        if (keyItems.length <= 0)
        
        //If there's nothing, then it shouldn't be considered an error to report. Code handles it fine.{
    ...
    public function hasKeyItem(keyName : String) : Float
    {
        var counter : Float = keyItems.length;
        //Various Errors preventing action
        if (keyItems.length <= 0)
    ...
    override public function minHP() : Float
    {
        var min : Float = 0;
        ...
        {
            min -= maxHP() * 0.05;
            if (perkv1(IMutationsLib.HumanAdrenalGlandsIM) >= 2)
            {
                min -= maxHP() * 0.05;
            }
            if (perkv1(IMutationsLib.HumanAdrenalGlandsIM) >= 3)
            {
                min -= maxHP() * 0.15;
            }
        ...
        if (hasPerk(PerkLib.TooAngryToDie))
        {
            min -= maxWrath();
        }
        ...
        {
            var limit : Float = perkv1(PerkLib.SPSurvivalTrainingX) * 10;
            var bonus : Float = Math.round((level - 1) / 3);
            if (bonus > limit)
            {
                bonus = limit;
            }
    ...
    override public function maxFatigue() : Float
    {
        var max : Float = maxFatigueBaseStat.value;
        var multimax : Float = maxFatigueMultStat.value;
        max += maxFatiguePerSpeStat.value * spe;
        if (hasPerk(PerkLib.EromancyBeginner))
        {
            max += Math.round(lib);
        }
        if (hasPerk(PerkLib.EromancyExpert))
        {
            max += Math.round(lib * 2);
        }
        if (hasPerk(PerkLib.EromancyMaster))
        {
            max += Math.round(lib * 2);
        }
        ...
        {
            max += (150 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL]));
        }
        ...
        {
            max += (250 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL]));
        }
        if (hasPerk(PerkLib.EclassHeavenTribulationSurvivor))
        {
            max += (300 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL]));
        }
        if (hasPerk(PerkLib.AscensionEndurance))
        {
            max += perkv1(PerkLib.AscensionEndurance) * 30;
        }
        ...
        {
            max += 6 * maxFatiguePerLevelStat.value;
        }
    ...
    override public function maxOverFatigue() : Float
    {
        var max1 : Float = maxFatigue();
        var max2 : Float = 1;
        if (hasPerk(PerkLib.HiddenJobSwordImmortal))
        {
            max2 += 0.05;
        }
        if (hasPerk(PerkLib.SwordIntentAura))
        {
            max2 += 0.05;
        }
        if (hasPerk(PerkLib.SwordImmortalFirstForm))
        {
            max2 += 0.05;
        }
        ...
        if (perkv1(IMutationsLib.HumanBloodstreamIM) >= 4)
        {
            max2 += 0.05;
        }
        ...
        if (perkv1(IMutationsLib.HumanMusculatureIM) >= 4)
        {
            max2 += 0.1;
        }
        max1 *= max2;  //~140%  
    ...
    override public function maxSoulforce() : Float
    {
        var max : Float = maxSfBaseStat.value;
        var multimax : Float = maxSfMultStat.value;
        var ratioW : Float = 1;
        if (perkv1(IMutationsLib.WhiteFacedOneBirthrightIM) >= 3)
        {
            ratioW += 1;
        }
        if (perkv1(IMutationsLib.WhiteFacedOneBirthrightIM) >= 4)
        {
            ratioW += 1;
        }
        ...
        {
            max += 30 * flags[kFLAGS.SOUL_CULTIVATION];
        }  //Soul Apprentice  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 4)
        {
            max += 15 * (flags[kFLAGS.SOUL_CULTIVATION] - 3);
        }  //Soul Personage  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 7)
        {
            max += 15 * (flags[kFLAGS.SOUL_CULTIVATION] - 6);
        }  //Soul Warrior  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 10)
        {
            max += 30 * (flags[kFLAGS.SOUL_CULTIVATION] - 9);
        }  //Soul Sprite  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 13)
        {
            max += 30 * (flags[kFLAGS.SOUL_CULTIVATION] - 12);
        }  //Soul Scholar  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 16)
        {
            max += 30 * (flags[kFLAGS.SOUL_CULTIVATION] - 15);
        }  //Soul Grandmaster  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 19)
        {
            max += 60 * (flags[kFLAGS.SOUL_CULTIVATION] - 18);
        }  //Soul Elder  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 23)
        {
            max += 60 * (flags[kFLAGS.SOUL_CULTIVATION] - 22);
        }  //Soul Exalt  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 27)
        {
            max += 60 * (flags[kFLAGS.SOUL_CULTIVATION] - 26);
        }  //Soul Overlord  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 31)
        {
            max += 90 * (flags[kFLAGS.SOUL_CULTIVATION] - 30);
        }  //Soul Tyrant  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 35)
        {
            max += 90 * (flags[kFLAGS.SOUL_CULTIVATION] - 34);
        }  //Soul King  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 39)
        {
            max += 90 * (flags[kFLAGS.SOUL_CULTIVATION] - 38);
        }  //Soul Emperor  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 43)
        {
            max += 150 * (flags[kFLAGS.SOUL_CULTIVATION] - 42);
        }  //Soul Ancestor  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 48)
        {
            max += 150 * (flags[kFLAGS.SOUL_CULTIVATION] - 47);
        }  //Soul Sage  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 53)
        {
            max += 150 * (flags[kFLAGS.SOUL_CULTIVATION] - 52);
        }  //Soul Sovereign  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 58)
        {
            max += 210 * (flags[kFLAGS.SOUL_CULTIVATION] - 57);
        }  //Soul Saint  
        ...
        if (hasPerk(PerkLib.FlyingSwordPath))
        {
            max += (50 * (1 + perkv1(PerkLib.FlyingSwordPath)));
        }
        if (hasPerk(PerkLib.DemonicLethicite))
        {
            max += Math.round(lib);
        }
        if (hasPerk(PerkLib.Metamorph))
        {
            max += (50 * (1 + perkv1(PerkLib.Metamorph)));
        }
        if (hasPerk(PerkLib.MetamorphEx))
        {
            max += (50 + (100 * perkv1(PerkLib.MetamorphEx)));
        }
        ...
        if (jewelryEffectId2 == JewelryLib.MODIFIER_SF)
        {
            max += jewelryEffectMagnitude2;
        }  //+100  
        if (jewelryEffectId3 == JewelryLib.MODIFIER_SF)
        {
            max += jewelryEffectMagnitude3;
        }  //+100  
        if (jewelryEffectId4 == JewelryLib.MODIFIER_SF)
        {
            max += jewelryEffectMagnitude4;
        }  //+100  
        if (necklaceEffectId == NecklaceLib.MODIFIER_SF)
        {
            max += necklaceEffectMagnitude;
        }  //+100	 necklaceName == "soulmetal necklace"  
        ...
        {
            max += 6 * ratioW * maxSfPerLevelStat.value;
        }
        ...
        if (hasPerk(PerkLib.HistoryCultivator) || hasPerk(PerkLib.PastLifeCultivator))
        {
            multimax += 0.1;
        }
        if (hasPerk(PerkLib.JobSoulCultivator))
        
        //8005-9005 soulforce na razie przed liczeniem mnożnika jest{
            ...
            {
                multimax += (flags[kFLAGS.SOULFORCE_GAINED_FROM_CULTIVATING]) * 0.01;
            }
        }
        if (hasPerk(PerkLib.CovenantOfTheSpirits))
        {
        ...
        max *= multimax;
        max = Math.round(max);
        if (hasPerk(PerkLib.Soulless) || max < 0)
        {
            max = 0;
        }
        if (max > 1499999)
    ...
    override public function maxOverSoulforce() : Float
    {
        var max1 : Float = maxSoulforce();
        var max2 : Float = 1;
        if (hasPerk(PerkLib.HiddenJobSwordImmortal))
        {
            max2 += 0.05;
        ...
        if (perkv1(IMutationsLib.HumanSmartsIM) >= 4)
        {
            max2 += 0.05;
        }
        max1 *= max2;  //~130%  
    ...
    override public function maxDemonicEnergy() : Float
    {
        var max : Float = 50;
        max += level * 10;
        if (hasPerk(PerkLib.DarkAscensionBottomlessHunger))
        {
            max *= (1 + (0.05 * perkv1(PerkLib.DarkAscensionBottomlessHunger)));
        }
        max = Math.round(max);
        return max;
    }
    
    override public function maxWrath() : Float
    {
        var max : Float = maxWrathBaseStat.value;
        var multimax : Float = maxWrathMultStat.value;
        if (hasPerk(PerkLib.AscensionFury))
        {
            max += perkv1(PerkLib.AscensionFury) * 100;
        }
        if (jewelryEffectId == JewelryLib.MODIFIER_WR)
        {
            max += jewelryEffectMagnitude;
        }  //+75 to +175  
        if (jewelryEffectId2 == JewelryLib.MODIFIER_WR)
        {
            max += jewelryEffectMagnitude2;
        }  //+75 to +175  
        if (jewelryEffectId3 == JewelryLib.MODIFIER_WR)
        {
            max += jewelryEffectMagnitude3;
        }  //+75 to +175  
        if (jewelryEffectId4 == JewelryLib.MODIFIER_WR)
        {
            max += jewelryEffectMagnitude4;
        }  //+75 to +175  
        if (jewelryName == "Undefeated King's Signet" || jewelryName3 == "Undefeated King's Signet")
        {
            max += 150;
        }
        ...
        {
            max += 6 * maxWrathPerLevelStat.value;
        }
        //~194,455
        if (game.player.isRace(Races.ORC))
        {
            multimax += 0.2;
        }
    ...
    override public function maxOverWrath() : Float
    {
        var max1 : Float = maxWrath();
        var max2 : Float = 1;
        if (hasPerk(PerkLib.HiddenJobAsura))
        {
            max2 += 0.1;
        }
        if (hasPerk(PerkLib.AbsoluteStrength))
        {
            max2 += 0.1;
        }
        if (hasPerk(PerkLib.AsuraStrength))
        {
            max2 += 0.1;
        }
        if (hasPerk(PerkLib.ICastAsuraFist))
        {
            max2 += 0.1;
        }
        if (hasPerk(PerkLib.LikeAnAsuraBoss))
        {
            max2 += 0.1;
        }
        if (hasPerk(PerkLib.AsuraToughness))
        {
            max2 += 0.1;
        }
        if (hasPerk(PerkLib.ItsZerkingTime))
        {
            max2 += 0.1;
        }
        //
        if (hasPerk(PerkLib.AsuraSpeed))
        {
            max2 += 0.1;
        }
        if (hasPerk(PerkLib.HiddenJobSwordImmortal))
    ...
    public function maxSafeWrathMagicalAbilities() : Float
    {
        var max1 : Float = 0;
        if (hasPerk(PerkLib.ICastAsuraFist))
        {
        ...
        var max2 : Float = 0.75;
        if (flags[kFLAGS.PRIMARY_DIFFICULTY] < 2 || necklaceName == "Wrathless")
        {
            max2 += 0.25;
        }
        max1 *= max2;
    ...
    public function maxSafeWrathSpellcasting() : Float
    {
        var max1 : Float = 0;
        if (hasPerk(PerkLib.ICastAsuraFist))
        {
        ...
        if (hasPerk(PerkLib.MagesWrath))
        {
            max2 += 0.05;
        }
        if (hasPerk(PerkLib.MagesWrathEx))
        {
            max2 += 0.05;
        }
        if (hasPerk(PerkLib.WarMageNovice))
        {
            max2 += 0.05;
        }
        if (hasPerk(PerkLib.WarMageApprentice))
        {
            max2 += 0.05;
        }
        if (hasPerk(PerkLib.WarMageAdept))
        {
            max2 += 0.05;
        }
        if (hasPerk(PerkLib.WarMageExpert))
        {
            max2 += 0.05;
        }
        if (hasPerk(PerkLib.WarMageMaster))
        {
            max2 += 0.05;
        }
        max1 *= max2;
    ...
    override public function maxMana() : Float
    {
        var max : Float = maxManaBaseStat.value;
        var multimax : Float = maxManaMultStat.value;
        max += maxManaPerIntStat.value * inte;
        max += maxManaPerWisStat.value * wis;
        if (hasPerk(PerkLib.Archmage) && inte >= 100)
        {
            max += 180;
        }
        if (hasPerk(PerkLib.ArchmageEx) && inte >= 100)
        {
            max += 600;
        }
        if (hasPerk(PerkLib.Channeling) && inte >= 60)
        {
            max += 90;
        }
        if (hasPerk(PerkLib.GrandArchmage) && inte >= 125)
        {
            max += 225;
        }
        if (hasPerk(PerkLib.GrandArchmage2ndCircle) && inte >= 150)
        {
            max += 270;
        }
        if (hasPerk(PerkLib.GrandArchmage3rdCircle) && inte >= 175)
        {
            max += 315;
        }
        if (hasPerk(PerkLib.GrandGreyArchmage) && inte >= 225)
        {
            max += 600;
        }
        if (hasPerk(PerkLib.GrandGreyArchmage2ndCircle) && inte >= 275)
        {
            max += 900;
        }
        if (hasPerk(PerkLib.GrandMage) && inte >= 75)
        {
            max += 135;
        }
        if (hasPerk(PerkLib.GreyArchmage) && inte >= 175)
        {
            max += 450;
        }
        if (hasPerk(PerkLib.GreyMage) && inte >= 125)
        {
            max += 300;
        }
        if (hasPerk(PerkLib.GreyMageApprentice) && inte >= 75)
        {
            max += 150;
        }
        if (hasPerk(PerkLib.Mage) && inte >= 50)
        {
            max += 90;
        }
        ...
        if (hasPerk(PerkLib.SpellpowerGrey) && inte >= 50)
        {
            max += 75;
        }
        if (hasPerk(PerkLib.SpellpowerHealing) && wis >= 50)
        {
            max += 90;
        }
        if (hasPerk(PerkLib.EromancyBeginner))
        ...
        if (hasPerk(PerkLib.EromancyMaster))
        {
            max += Math.round(inte * 6);
        }
        if (hasPerk(PerkLib.HistoryScholar) || hasPerk(PerkLib.PastLifeScholar))
        {
            multimax += 0.1;
        }
        ...
        {
            multimax += 0.05;
            if (hasPerk(PerkLib.ArcaneRegenerationMajor) && inte >= 75)
            {
                multimax += 0.1;
            }
            if (hasPerk(PerkLib.ArcaneRegenerationEpic) && inte >= 100)
            {
                multimax += 0.15;
            }
            if (hasPerk(PerkLib.ArcaneRegenerationLegendary) && inte >= 125)
            {
                multimax += 0.2;
            }
            if (hasPerk(PerkLib.ArcaneRegenerationMythical) && inte >= 150)
            {
                multimax += 0.25;
            }
        }
        ...
        if (hasPerk(PerkLib.AscensionInnerPower))
        {
            max += perkv1(PerkLib.AscensionInnerPower) * 120;
        }
        if (jewelryEffectId == JewelryLib.MODIFIER_MP)
        {
            max += jewelryEffectMagnitude;
        }
        if (jewelryEffectId2 == JewelryLib.MODIFIER_MP)
        {
            max += jewelryEffectMagnitude2;
        }
        if (jewelryEffectId3 == JewelryLib.MODIFIER_MP)
        {
            max += jewelryEffectMagnitude3;
        }
        if (jewelryEffectId4 == JewelryLib.MODIFIER_MP)
        {
            max += jewelryEffectMagnitude4;
        }
        ...
        {
            max += 6 * maxManaPerLevelStat.value;
        }
        if (hasPerk(PerkLib.CovenantOfTheSpirits))
        {
            max *= 2;
    ...
    override public function maxOverMana() : Float
    {
        var max1 : Float = maxMana();
        var max2 : Float = 1;
        if (hasPerk(PerkLib.PrestigeJobGreySage))
        {
            max2 += 0.1;
        }
        if (hasPerk(PerkLib.Equilibrium))
        {
            max2 += 0.1;
        }
        if (hasPerk(PerkLib.GreySageIntelligence))
        {
            max2 += 0.1;
        }
        if (hasPerk(PerkLib.HyperCasting))
        {
            max2 += 0.1;
        }
        if (hasPerk(PerkLib.MunchkinAtWork))
    ...
    public function maxVenom() : Float
    {
        var maxven : Float = 0;
        var multimaxven : Float = 1;
        if (CoC.instance.transformations.FaceSnakeFangs.isPresent())
        {
            maxven += 200;
        }
        if (CoC.instance.transformations.FaceSpiderFangs.isPresent())
        {
            maxven += 200;
        }
        if (CoC.instance.transformations.FaceJabberwocky.isPresent())
        {
            maxven += 200;
        }
        if (game.player.tailType == Tail.BEE_ABDOMEN)
        {
            maxven += 300;
        }
        if (game.player.tailType == Tail.SPIDER_ADBOMEN)
        {
            maxven += 300;
        }
        if (game.player.tailType == Tail.SCORPION)
        {
            maxven += 300;
        }
        if (game.player.tailType == Tail.MANTICORE_PUSSYTAIL)
        {
            maxven += 400;
        }
        if (game.player.lowerBody == LowerBody.HYDRA)
        {
            maxven += 400;
        }
        if (game.player.lowerBody == LowerBody.SANDWORM)
        {
            maxven += 400;
        }
        if (game.player.lowerBody == LowerBody.ATLACH_NACHA)
        {
            maxven += 1200;
        }
        if (hasPerk(PerkLib.ImprovedVenomGland))
        {
            maxven += 100;
        }
        ...
        if (perkv1(IMutationsLib.ArachnidBookLungIM) > 0)
        {
            multimaxven += perkv1(IMutationsLib.ArachnidBookLungIM);
        }
    ...
    public function maxHunger() : Float
    {
        ...
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 1)
        {
            max += 20 * flags[kFLAGS.SOUL_CULTIVATION];
        }  //Soul Apprentice  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 4)
        {
            max += 1 * (flags[kFLAGS.SOUL_CULTIVATION] - 3);
        }  //Soul Personage  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 7)
        {
            max += 1 * (flags[kFLAGS.SOUL_CULTIVATION] - 6);
        }  //Soul Warrior  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 10)
        {
            max += 2 * (flags[kFLAGS.SOUL_CULTIVATION] - 9);
        }  //Soul Sprite  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 13)
        {
            max += 2 * (flags[kFLAGS.SOUL_CULTIVATION] - 12);
        }  //Soul Scholar  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 16)
        {
            max += 2 * (flags[kFLAGS.SOUL_CULTIVATION] - 15);
        }  //Soul Grandmaster  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 19)
        {
            max += 4 * (flags[kFLAGS.SOUL_CULTIVATION] - 18);
        }  //Soul Elder  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 23)
        {
            max += 4 * (flags[kFLAGS.SOUL_CULTIVATION] - 22);
        }  //Soul Exalt  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 27)
        {
            max += 4 * (flags[kFLAGS.SOUL_CULTIVATION] - 26);
        }  //Soul Overlord  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 31)
        {
            max += 6 * (flags[kFLAGS.SOUL_CULTIVATION] - 30);
        }  //Soul Tyrant  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 35)
        {
            max += 6 * (flags[kFLAGS.SOUL_CULTIVATION] - 34);
        }  //Soul King  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 39)
        {
            max += 6 * (flags[kFLAGS.SOUL_CULTIVATION] - 38);
        }  //Soul Emperor  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 43)
        {
            max += 8 * (flags[kFLAGS.SOUL_CULTIVATION] - 42);
        }  //Soul Ancestor  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 48)
        {
            max += 8 * (flags[kFLAGS.SOUL_CULTIVATION] - 47);
        }  //Soul Sage  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 53)
        {
            max += 8 * (flags[kFLAGS.SOUL_CULTIVATION] - 52);
        }  //Soul Sovereign  
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 58)
        {
            max += 10 * (flags[kFLAGS.SOUL_CULTIVATION] - 57);
        }  //Soul Saint  
        ...
        if (game.player.isRace(Races.TROLL))
        {
            max += 50;
        }
        if (game.player.isRace(Races.GLACIAL_TROLL))
        {
            max += 75;
        }
        ...
        if (perkv1(IMutationsLib.HumanFatIM) >= 3 && game.player.racialScore(Races.HUMAN) > 17)
        {
            max += 50;
        }
        if (perkv1(IMutationsLib.HumanFatIM) >= 4 && game.player.racialScore(Races.HUMAN) > 17)
        {
            max += 100;
        }
