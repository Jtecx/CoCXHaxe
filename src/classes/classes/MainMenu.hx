...
class MainMenu extends BaseContent
{
    ...
    {
        CoC.instance.stage.focus = (try cast(mainView, MainView) catch(e:Dynamic) null).mainText;
        ...
        if (resume != null)
        {
            addButton(0, "Resume", resume).hint("Get back to gameplay?");
        }
        else
        {
            addButtonDisabled(0, "Resume", "Please start or load a game first.");
        }
        if (resume != null)
        {
            addButton(1, "Achievements", achievements.achievementsScreen).hint("View all achievements you have unlocked so far.");
        }
        else
        {
            addButtonDisabled(1, "Achievements", "Please start or load a game first.");
        }
        ...
        if (CoC_Settings.debugBuild)
        {
            addButton(8, "Debug Info", CoC.instance.debugInfoMenu.debugPane).hint("View debug information.");
        }
    }
    ...
    // NO FUCKING DECENT MULTI-LINE STRING LITERALS BECAUSE FUCKING STUPID
    {
        ...
        if (flags[kFLAGS.SHOW_SPRITES_FLAG] != null)
        {
            outputText("\n\n<b>Sprites disabled.</b>");
        }
        if (flags[kFLAGS.EASY_MODE_ENABLE_FLAG] != null)
        {
