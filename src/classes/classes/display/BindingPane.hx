...
{
    public var functions(get, set) : Array<Dynamic>;
    ...
    private var _functions : Array<Dynamic>;
    private var _newFuncs : Array<Dynamic>;
    ...
    private function UpdateContentObjects() : Void
    {
        for (i in 0..._functions.length)
        {
            var currLabel : BindDisplay = try cast(_content.getElementByName(_functions[i].Name), BindDisplay) catch(e:Dynamic) null;
    ...
    {
        var keyDescriptions : FastXML = describeType(Keyboard);
        var keyNames : FastXMLList = keyDescriptions.descendants("constant").att.name;
        
        _keyDict = new Dictionary();
        
        for (i in 0...keyNames.length())
        {
            _keyDict[Reflect.setField(Keyboard, Std.string(keyNames.get(i)), humanize(Std.string(keyNames.get(i))))];
        }
    ...
    {
        if (name.indexOf("NUMBER_") == 0)
        {
            name = name.substring("NUMBER_".length);
        }
        return name;
    }
    ...
    private function set_functions(funcs : Array<Dynamic>) : Array<Dynamic>
    {
        _functions = funcs;
        return funcs;
    }
    
    private function get_functions() : Array<Dynamic>
    {
        return _functions;
    }
    private static var BindingPane_static_initializer = {
