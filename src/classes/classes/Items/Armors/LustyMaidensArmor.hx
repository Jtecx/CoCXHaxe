...
@:final class LustyMaidensArmor extends Armor
{
    ...
    override private function get_def() : Float
    {
        ...
        {
            return 15 + game.flags[kFLAGS.BIKINI_ARMOR_BONUS];
        }
    ...
    override public function afterEquip(doOutput : Bool, slot : Int) : Void
    {
        if (game.player.hasVirginVagina())
        {
            Reflect.setField(_buffs, "teasedmg", 10 + game.flags[kFLAGS.BIKINI_ARMOR_BONUS]);
        }
        else
        {
            Reflect.setField(_buffs, "teasedmg", 6 + game.flags[kFLAGS.BIKINI_ARMOR_BONUS]);
        }
        super.afterEquip(doOutput, slot);
    }
    
    public static function canUseStatic(doOutput : Bool) : Bool
    {
        if (!doOutput)
        
        // This section should be in sync with text below{
            
            if (CoC.instance.player.biggestTitSize() < BreastCup.D)
            {
                return false;
            }
            if (CoC.instance.player.hasCock() && !CoC.instance.player.hasSheath())
            {
                return false;
            }
            if (CoC.instance.player.hasBalls())
            {
                return false;
            }
        ...
        if (CoC.instance.player.biggestTitSize() < BreastCup.A)
        ...
        if ((CoC.instance.player.hasCock() && !CoC.instance.player.hasSheath()) || CoC.instance.player.hasBalls())
        {
            ...
            {
                EngineCore.outputText("or your " + CoC.instance.player.multiCockDescriptLight());
            }
            else
            {
        ...
        if (CoC.instance.player.wetness() >= 3)
        {
    ...
    public static function lustyMaidenPaizuri(player : Player = null, monster : Monster = null) : Void
    {
        ...
        {
            EngineCore.outputText(", bouncing with each beat of [monster his] heart, thick beads of pre dribbling from [monster his] tip as you bat [monster his] hands away before [monster he] can waste the load [monster he]'s saved up for you.");
        }
        ...
        if (monster.hasBalls())
        {
            EngineCore.outputText(" and fondle [monster his] balls one-handed, squeezing the virile orbs to try and coax more of [monster his] dirty, perverted thoughts to distill into salty seed");
        }
        else if (monster.short == "anemone")
        {
            EngineCore.outputText(" and stroke [monster his] taint, even brushing over the featureless spot where an asshole would be, if she had one, to try and coax more of [monster his] dirty, perverted thoughts to distill into salty seed");
        }
        else
        {
            EngineCore.outputText(" and stroke [monster his] taint, even brushing close to [monster his] asshole to try and coax more of [monster his] dirty, perverted thoughts to distill into salty seed");
        }
        ...
        if (monster.hasBalls())
        {
            EngineCore.outputText(", [monster his] balls pumping and bouncing in [monster his] sack");
        }
        ...
        if (CoC.instance.flags[kFLAGS.BIKINI_ARMOR_BONUS] > 10)
        {
            CoC.instance.flags[kFLAGS.BIKINI_ARMOR_BONUS] = 10;
        }
        player.sexReward("cum");
        CoC.instance.player.dynStats("sen", 2);
        if (player.hasVirginVagina())
        {
            CoC.instance.player.dynStats("cor", -1);
        }
        if (Std.is(monster, Minotaur) || Std.is(monster, MinotaurLord) || Std.is(monster, MinotaurMob))
        {
            player.minoCumAddiction(3);
        }
        if (Std.is(monster, Ceraph))
        {
            CoC.instance.flags[kFLAGS.TIMES_BEATEN_SERAPH_WITHOUT_LOSING]++;
        }
    ...
    public static function addTitfuckButton(btn : Int, player : Player = null, monster : Monster = null) : CoCButton
    {
        ...
        if (Std.is(player.armor, LustyMaidensArmor) || Std.is(player.armor, SuccubusArmor))
        {
        ...
        {
            return EngineCore.addButtonDisabled(btn, "B.Titfuck", "Req. to have Lusty Maiden's or Succubus' armor equipped.");
        }
    }
}
