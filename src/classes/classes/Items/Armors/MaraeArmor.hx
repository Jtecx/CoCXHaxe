...
class MaraeArmor extends Armor
{
    ...
    override public function equipText() : Void
    {
        ...
        if (!CoC.instance.player.isTaur())
        {
            ...
            {
                outputText(CoC.instance.player.multiCockDescriptLight());
            }
            if (CoC.instance.player.hasCock() && CoC.instance.player.hasVagina())
            {
                outputText(" and ");
            }
            ...
            if (CoC.instance.player.hasCock())
            {
                ...
                if (CoC.instance.player.biggestCockArea() >= 100)
                {
                    ...
                    else if (CoC.instance.player.cor >= 33 && CoC.instance.player.cor < 66)
                    {
                        outputText("You blush a bit, not sure how you feel. ");
                    }
                    else if (CoC.instance.player.cor >= 66 || game.flags[kFLAGS.PC_FETISH] > 0)
                    {
