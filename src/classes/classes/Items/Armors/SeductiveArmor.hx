...
@:final class SeductiveArmor extends Armor
{
    ...
    override public function equipText() : Void
    {
        if (!SceneLib.ceraphFollowerScene.ceraphIsFollower() && !game.player.hasStatusEffect(StatusEffects.CeraphOff))
        {
            ...
            if (game.player.nipplesPierced > 0)
            {
                outputText("You're left to wonder - where did the old piercings go?\n\n");
            }
    ...
    override public function beforeEquip(doOutput : Bool, slot : Int) : Equipable
    {
        ...
        return ArmorLib.COMFORTABLE_UNDERCLOTHES;
    }
}
