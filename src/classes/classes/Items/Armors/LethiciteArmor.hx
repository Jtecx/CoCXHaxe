...
class LethiciteArmor extends Armor
{
    public function new()
    {
    ...
    {
        return 20 + as3hx.Compat.parseInt(game.player.cor / 10);
    }
    ...
    override public function equipText() : Void
    {
        ...
        {
            outputText("You hesitate at how the armor will expose your groin but you proceed to put it on anyway. ");
        }
        if (game.player.cor >= 33 && CoC.instance.player.cor < 66)
        {
            outputText("You are not sure about the crotch-exposing armor. ");
        }
        if (game.player.cor >= 66)
        {
            outputText("You are eager to show off once you get yourself suited up. ");
        }
        //Put on breastplate
        ...
        if (game.player.biggestLactation() >= 4)
        {
            outputText("A bit of milk gradually flows over your breastplate. ");
        }
        //Put on leggings
        if (game.player.isBiped())
        {
            ...
            if (game.player.hasCock() && game.player.lowerGarment.isNothing)
            {
                outputText("Your " + game.player.multiCockDescriptLight() + " hang" + ((game.player.cocks.length == 1) ? "s" : "") + " freely. ");
            }
            if (game.player.cor < 33)
            ...
            //Low corruption{
                ...
                {
                    outputText("Good thing you have your " + game.player.lowerGarment + " on!");
                }
                else
                {
                    outputText("You blush with embarrassment. ");
                }
            }
            ...
            //Medium corruption{
                ...
                if (!game.player.lowerGarment.isNothing)
                {
                    outputText("You are unsure about whether you should keep your " + game.player.lowerGarment + " on or not.");
                }
                else
                {
                    outputText("You are unsure how you feel about your crotch being exposed to the world.");
                }
            }
            else if (game.player.cor >= 66)
            
            //High corruption{
                
                if (!game.player.lowerGarment.isNothing)
                {
                    outputText("You ponder over taking off your undergarments.");
                }
                else
                {
                    outputText("You delight in having your nether regions open to the world.");
                }
            }
