...
class OvipositionElixir extends Consumable
{
    ...
    override public function useItem() : Bool
    {
        ...
        if (game.player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS || game.player.pregnancy2Type == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS)
        ...
        if (!changeOccurred && game.player.pregnancyIncubation > CoC.instance.gameSettings.sceneHunter_inst.adjustPregEventTimer(20, player.pregnancyType) && game.player.pregnancyType != PregnancyStore.PREGNANCY_BUNNY)
        ...
        //If no changes, speed up pregnancy.{
            ...
            var newIncubation : Int = as3hx.Compat.parseInt(game.player.pregnancyIncubation - as3hx.Compat.parseInt(game.player.pregnancyIncubation * 0.3 + 10));
            if (newIncubation < 2)
            {
                newIncubation = 2;
            }
        ...
        if (!changeOccurred && game.player.pregnancy2Incubation > 20 && game.player.pregnancy2Type != PregnancyStore.PREGNANCY_BUNNY)
        
        //If no changes, speed up pregnancy.{
            
            outputText("\n\nYou gasp as your pregnancy suddenly leaps forwards, your belly bulging outward a few inches as it gets closer to time for birthing.");
            var newIncubation2 : Int = as3hx.Compat.parseInt(game.player.pregnancy2Incubation - as3hx.Compat.parseInt(game.player.pregnancy2Incubation * 0.3 + 10));
            if (newIncubation2 < 2)
            {
                newIncubation2 = 2;
            }
