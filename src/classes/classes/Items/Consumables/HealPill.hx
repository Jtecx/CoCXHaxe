...
/**
	 ...
	 * Retro UTG stuff!
	 */
class HealPill extends Consumable
{
    public function new()
    {
    ...
    {
        var rand : Int = as3hx.Compat.parseInt(Math.random() * 100);
        outputText("You pop the small pill into your mouth and swallow. ");
        
        if (player.HP < player.maxOverHP())
        {
            if (player.hasPerk(PerkLib.GoblinoidBlood) && player.perkv1(IMutationsLib.NaturalPunchingBagIM) >= 3)
            {
                EngineCore.HPChange(Math.round((50 + player.tou) * 2 * player.postConsumptionMlt()), true);
            }
            else if ((player.hasPerk(PerkLib.GoblinoidBlood) && player.perkv1(IMutationsLib.NaturalPunchingBagIM) >= 2) || player.perkv1(IMutationsLib.NaturalPunchingBagIM) >= 3)
            {
                EngineCore.HPChange(Math.round((50 + player.tou) * 1.5 * player.postConsumptionMlt()), true);
            }
            else
            {
                EngineCore.HPChange((50 + player.tou) * player.postConsumptionMlt(), true);
            }
