...
class Centaurinum extends Consumable
{
    public function new()
    {
    ...
    public function centaurTFEffects(sagittariusBow : Bool = false) : Void
    {
        var changes : Float = 0;
        ...
        if (changes < changeLimit && rand(2) == 0 && player.basetallness < 84)
        {
            ...
            if (temp < 5)
            {
                outputText("[pg]You shift uncomfortably as you realize you feel off balance.  Gazing down, you realize you have grown SLIGHTLY taller.");
            }
            if (temp >= 5 && temp < 7)
            {
        ...
        //If cocks that aren't horsified!{
            ...
            else
            {
                ...
                if (temp2 > 2)
                {
                    outputText("Your [cock " + (temp + 1) + "] tightens painfully, inches of taut horse-flesh pouring out from your sheath as it grows longer.  Thick animal-pre forms at the flared tip, drawn out from the pleasure of the change.");
                }
                if (temp2 > 1 && temp2 <= 2)
                {
            ...
            if (rand(2) == 0 && changes < changeLimit && player.horseCocks() > 0)
            {
                ...
                else if (player.cor + player.lib <= 175)
                {
