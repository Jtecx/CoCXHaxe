...
class MaraFruit extends Consumable
{
    public function new()
    {
    ...
    override public function useItem() : Bool
    {
        ...
        if (player.gender == 2 || (player.gender == 3 && player.mf("m", "f") == "f"))
        {
            ...
            if (player.vaginas.length > 0)
            {
                ...
                if (player.vaginas[0].vaginalWetness == VaginaClass.WETNESS_SLAVERING)
                {
                    ...
                    {
                        outputText("Your [vagina] gushes fluids down your leg as you spontaneously orgasm.");
                    }
                    else
                    {
                        outputText("Your [vagina]s gush fluids down your legs as you spontaneously orgasm, leaving a thick puddle of pussy-juice on the ground.  It is rapidly absorbed by the earth.");
                    }
                    player.orgasm();
                }
                if (player.vaginas[0].vaginalWetness == VaginaClass.WETNESS_DROOLING)
                {
                    ...
                    if (player.vaginas.length == 1)
                    {
                        outputText("You cannot resist as your hands dive into your [vagina].  ");
                    }
                    if (player.vaginas.length > 1)
                    {
                        outputText("You cannot resist plunging your hands inside your [vagina]s.  ");
                    }
                ...
                if (player.vaginas[0].vaginalWetness == VaginaClass.WETNESS_NORMAL)
                {
                    ...
                    if (player.vaginas.length == 1)
                    {
                        outputText("You realize afterwards that your [vagina] felt much wetter than normal.");
                    }
                    else
                    {
                        outputText("You realize afterwards that your [vagina] were much wetter than normal.");
                    }
        ...
        if (player.isRace(Races.PLANT, 4, false) && player.wings.type == Wings.PLANT && player.hasStatusEffect(StatusEffects.AlrauneFlower) && changes < changeLimit && rand(2) == 0)
        {
            ...
            if (player.tailType != 0)
            {
                ...
                {
                    outputText(" Your insectile abdomen recedes");
                }
                else if (player.tailType > 0 && player.tailCount > 1)
                {
                    outputText(" Your tails recede");
                }
                else
                {
                    outputText(" Your tail recedes");
                }
