...
class BeeHoney extends Consumable
{
    private var PURE_HONEY_VALUE(default, never) : Int = 40;
    private var SPECIAL_HONEY_VALUE(default, never) : Int = 20;
    
    public function new(pure : Bool, special : Bool)
    {
    ...
    {
        if (value == SPECIAL_HONEY_VALUE && SceneLib.exgartuan.dickPresent())
    ...
    override public function useItem() : Bool
    {
        ...
        var pure : Bool = (value == PURE_HONEY_VALUE);
        var special : Bool = (value == SPECIAL_HONEY_VALUE);
        var changes : Float = 0;
        ...
        if ((pure || special) && (player.pregnancyType == PregnancyStore.PREGNANCY_FAERIE || player.pregnancy2Type == PregnancyStore.PREGNANCY_FAERIE))
        
        //Pure or special honey can reduce the corruption of a phouka baby{
            
            if (CoC.instance.flags[kFLAGS.PREGNANCY_CORRUPTION] > 1)
            ...
            else if (CoC.instance.flags[kFLAGS.PREGNANCY_CORRUPTION] < 1)
        ...
        if (changes < changeLimit && player.bRows() > 2 && Utils.rand(3) == 0 && CoC.instance.flags[kFLAGS.HYPER_HAPPY] == null)
        {
            changes++;
        ...
        //Bee Legs
        if (changes < changeLimit && player.lowerBody != LowerBody.BEE && player.lowerBody != LowerBody.GARGOYLE && Utils.rand(4) == 0)
        {
            outputText("\n\n");
        ...
        //All the special honey effects occur after any normal bee transformations (if the player wasn't a full bee morph){
            
            //Cock growth multiplier.
            var mult : Int = 1.0;
            if (player.cocks[0].cArea() >= 140)
            {
                mult -= 0.2;
            }
            if (player.cocks[0].cArea() >= 180)
            {
                mult -= 0.2;
            }
            if (player.cocks[0].cArea() >= 220)
            {
                mult -= 0.2;
            }
            if (player.cocks[0].cArea() >= 260)
            {
                mult -= 0.2;
            }
            if (player.cocks[0].cArea() >= 300)
            {
                mult -= 0.1;
            }
            if (player.cocks[0].cArea() >= 400)
            {
                mult -= 0.1;
            }  //Cock stops growing at that point.  
            //Begin TF
            if (!player.hasCock())
            {
