...
@:final class GroPlus extends Consumable
{
    ...
    override public function useItem() : Bool
    {
        ...
        if (player.hasCock())
        {
            EngineCore.addButton(0, "Cock", growPlusCock);
        }
        if (player.hasBalls())
        {
            EngineCore.addButton(1, "Balls", pickDoses, growPlusBalls);
        }
        EngineCore.addButton(2, "Breasts", growPlusBreasts);
        EngineCore.addButton(3, "Nipples", pickDoses, growPlusNipples);
        if (player.hasVagina())
        {
            EngineCore.addButton(4, "Clit", pickDoses, growPlusClit);
        }
    ...
    private function sharedEnd(dose : Int, sexual : Bool = false) : Void
    {
        ...
        {
            player.consumeItem(game.consumables.GROPLUS, dose - 1);
        }  //eat up more gros  
    ...
    private function growPlusBreasts() : Void
    {
        ...
        var inject : Int->Int->Void = function(row1 : Int, dose : Int) : Void
        {
            var d : Int = dose;
            clearOutput();
            ...
            while (d-- > 0)
            {
                player.growTits(Utils.rand(5) + 1, row1, d == 1, 4);
            }
            sharedEnd(dose);
        }
    }
    ...
    private function growPlusCock() : Void
    {
        ...
        var inject : Int->String->Int->Void = function(dick : Int, part : String, dose : Int) : Void
        {
            clearOutput();
    ...
    {
        var cnt : Int = as3hx.Compat.parseInt(player.itemCount(game.consumables.GROPLUS) + 1);
        if (cnt == 0)
        {
            fun(1);
        }
