...
class Enigmanium extends Consumable
{
    public function new()
    {
    ...
    override public function useItem() : Bool
    {
        var choice : Int;
        var changes : Float = 0;
        var changeLimit : Float = player.additionalTransformationChances;
        var temp2 : Float = 0;
        var temp3 : Float = 0;
        ...
        if (rand(3) == 0 && changes < changeLimit && player.MutagenBonus("str", 1))
        {
            if (rand(2) == 0)
            {
                outputText("[pg]Your muscles feel taut, like a coiled spring, and a bit more on edge.");
            }
            else
            {
                outputText("[pg]You arch your back as your muscles clench painfully.  The cramp passes swiftly, leaving you feeling like you've gotten a bit stronger.");
            }
            changes++;
        }
        ...
        {
            var intensified : Bool = player.inHeat;
        ...
        //If cocks that aren't Catsified!{
            ...
            if ((player.catCocks() + player.demonCocks()) < player.cockTotal() && rand(3) == 0)
            {
                ...
                if (j != -1)
                {
                    ...
                    if (player.cocks[j].cockThickness <= 2)
                    {
                        player.thickenCock(j, 1);
                    }
            ...
            else
            {
                ...
                //Grow smallest cock!{
                    ...
                    if (temp2 > 2)
                    {
                        outputText("Your [cock " + (temp + 1) + "] tightens painfully, inches of taut flesh pouring out from your sheath as it grows longer.  Thick animal-pre forms at the flared tip, drawn out from the pleasure of the change.");
                    }
                    if (temp2 > 1 && temp2 <= 2)
                    {
                        outputText("Aching pressure builds within your sheath, suddenly releasing as an inch or more of extra dick flesh spills out.  A dollop of pre beads on the head of your enlarged [cock " + (temp + 1) + "] from the pleasure of the growth.");
                    }
                    if (temp2 <= 1)
                    {
                        outputText("A slight pressure builds and releases as your [cock " + (temp + 1) + "] pushes a bit further out of your sheath.");
                    }
                    changes++;
                }
            }
            ...
            if (rand(2) == 0 && changes < changeLimit && player.catCocks() > 0)
            {
                ...
                if (temp0 != -1)
                {
                    ...
                    {
                        outputText(" Your [cock " + (temp0 + 1) + "] is perfect for fucking about anything that is a cat or a taur.  You imagine the feel of plowing an equine pussy deeply, bottoming out and unloading sticky jets of horse-jizz into its fertile womb.  Your hand strokes your cat cock of its own accord, musky pre dripping from the spiked tip with each stroke.  Your mind wanders to the thought of you with a harem of pregnant cat or centaurs.");
                    }
                    else if (player.cor + player.lib >= 100)
                    {
                        outputText(" to be a rutting stud.  You ache to find a catgirl in heat to breed with.  Longing to spend your evenings plunging a [cock " + (temp0 + 1) + "] deep into their passages, dumping load after load of your thick animal-cum into them.  You'd be happy just fucking cunt morning, noon, and night.");
                    }
                    else if (player.cor + player.lib >= 60)
                    {
                        outputText(" to be this way... You breath the powerful animalistic scent and fantasize about fucking catgirls and centaurs night and day until their bellies slosh with your cum.");
                    }
                    else
                    {
                        outputText(" to have such a splendid tool.  You idly daydream about cunts and pussies, your [cock " + (temp0 + 1) + "] plowing them relentlessly, stuffing them pregnant with cum.");
                    }
            ...
            if (rand(2) == 0 && changes < changeLimit && player.ballSize <= 3)
            {
                ...
                else
                {
                    ...
                    {
                        outputText("[pg]A flash of warmth passes through you and a sudden weight develops in your groin.  You pause to examine the changes and your roving fingers discover your [balls] have grown larger than a human's.");
                    }
                    else
                    {
                        outputText("[pg]A sudden onset of heat envelops your groin, focusing on your [sack].  Walking becomes difficult as you discover your [balls] have enlarged again.");
                    }
        ...
        //sphinx color skin
        var SphinxSkinColor : Array<Dynamic> = ["dark", "tan"];
        ...
        if (player.tailType != Tail.CAT && player.ears.type == Ears.LION && rand(4) == 0 && changes < changeLimit)
        {
            ...
            {
                outputText("[pg]You pause and tilt your head... something feels different.  Ah, that's what it is; you turn around and look down at your tail as it starts to change shape, narrowing and sprouting glossy fur. <b>You now have a cat tail.</b>");
            }
        ...
        if (changes < changeLimit)
        {
            if (rand(2) == 0)
            {
                outputText(player.modThickness(5, 2));
            }
            if (rand(2) == 0)
            {
                outputText(player.modTone(76, 2));
            }
            else
            {
