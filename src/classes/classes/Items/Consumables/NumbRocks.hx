...
/**
	 ...
	 */
class NumbRocks extends Consumable
{
    ...
    override public function useItem() : Bool
    {
        ...
        if (player.hasVagina() && player.vaginas[0].vaginalLooseness > VaginaClass.LOOSENESS_TIGHT)
        {
            choices.push(3);
        }
        ...
        //add one debuff to the pool
        else
        {
            choices.push(0);
        }  //no effects  
        
        var _sw0_ = (choices[rand(choices.length)]);        
        ...
        switch (_sw0_)
        {
            ...
            case 4:
                ...
                if (player.ass.analLooseness == AssClass.LOOSENESS_VIRGIN)
                {
                    outputText("You marvel at the odd feeling - <b>your [asshole] is so tight as if it was never penetrated!</b>!");
                }
                else
                {
                    outputText("Touching it experimentally, you try to slip a finger inside and find out that <b>your [asshole] is tighter than before!</b>!");
                }
            case 5:
            ...
            case 7:
                ...
                if (!player.hasCoat())
                {
                    outputText("your [skin base]");
                }
                else
                {
                    outputText("the skin under your [skin coat]");
                }
