...
class GreyBook extends Consumable
{
    ...
    public function greySpellbook() : Void
    {
        ...
        if (player.hasPerk(PerkLib.GreyMagic) || player.hasPerk(PerkLib.PrestigeJobGreySage))
        {
            ...
            {
                EngineCore.addButton(2, "M.Shie", greySpellbook1, 1).hint("Learn Mana Shield spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(2, "M.Shie", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 55 && !player.hasStatusEffect(StatusEffects.KnowsWaterBall))
            {
                EngineCore.addButton(3, "W.Ball", greySpellbook1, 2).hint("Learn Water Ball spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(3, "W.Ball", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 55 && !player.hasStatusEffect(StatusEffects.KnowsWindBullet))
            {
                EngineCore.addButton(4, "W.Bull", greySpellbook1, 3).hint("Learn Wind Bullet spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(4, "W.Bull", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 55 && !player.hasStatusEffect(StatusEffects.KnowsStalagmite))
            {
                EngineCore.addButton(5, "Stalag", greySpellbook1, 4).hint("Learn Stalagmite spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(5, "Stalag", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 55 && !player.hasStatusEffect(StatusEffects.KnowsAcidSpray))
            {
                EngineCore.addButton(6, "A.Spra", greySpellbook1, 5).hint("Learn Acid Spray spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(6, "A.Spra", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 60 && !player.hasStatusEffect(StatusEffects.KnowsClearMind))
            {
                EngineCore.addButton(7, "C.Mind", greySpellbook1, 6).hint("Learn Clear Mind spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(7, "C.Mind", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 65 && !player.hasStatusEffect(StatusEffects.KnowsWaterSphere))
            {
                EngineCore.addButton(8, "W.Sphe", greySpellbook1, 7).hint("Learn Water Sphere spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(8, "W.Sphe", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 65 && !player.hasStatusEffect(StatusEffects.KnowsWindBlast))
            {
                EngineCore.addButton(9, "W.Blas", greySpellbook1, 8).hint("Learn Wind Blast spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(9, "W.Blas", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 65 && !player.hasStatusEffect(StatusEffects.KnowsShatterstone))
            {
                EngineCore.addButton(10, "Shatte", greySpellbook1, 9).hint("Learn Shatterstone spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(10, "Shatte", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 65 && !player.hasStatusEffect(StatusEffects.KnowsCorrosiveWave))
            {
                EngineCore.addButton(11, "C.Wave", greySpellbook1, 10).hint("Learn Corrosive Wave spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(11, "C.Wave", "You not have have enough int or already know this spell.");
            }
    ...
    public function greySpellbookPage2() : Void
    {
        ...
        {
            EngineCore.addButton(1, "H.Acid", greySpellbook1, 11).hint("Learn Hydro Acid spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(1, "H.Acid", "You not have have enough int or already know this spell.");
        }
        if (player.inte >= 80 && !player.hasStatusEffect(StatusEffects.KnowsEnergyDrain))
        {
            EngineCore.addButton(2, "E.Drai", greySpellbook1, 12).hint("Learn Energy Drain spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(2, "E.Drai", "You not have have enough int or already know this spell.");
        }
        if (player.inte >= 85 && !player.hasStatusEffect(StatusEffects.KnowsAcidRain))
        {
            EngineCore.addButton(3, "A.Rain", greySpellbook1, 13).hint("Learn Acid Rain spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(3, "A.Rain", "You not have have enough int or already know this spell.");
        }
        if (player.inte >= 90 && !player.hasStatusEffect(StatusEffects.KnowsRestore))
        {
            EngineCore.addButton(4, "Restor", greySpellbook1, 14).hint("Learn Restore spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(4, "Restor", "You not have have enough int or already know this spell.");
        }
        if (player.inte >= 100 && !player.hasStatusEffect(StatusEffects.KnowsBalanceOfLife))
        {
            EngineCore.addButton(5, "B.Life", greySpellbook1, 15).hint("Learn Balance of Life spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(5, "B.Life", "You not have have enough int or already know this spell.");
        }
    }
    ...
    public function greySpellbook1(spell : Float) : Void
    {
        clearOutput();
