...
class BlackBook extends Consumable
{
    ...
    public function blackSpellbook() : Void
    {
        ...
        {
            EngineCore.addButton(2, "I.Spik", blackSpellbook1, 1).hint("Learn Ice Spike spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(2, "I.Spik", "You not have have enough int or already know this spell.");
        }
        if (player.inte >= 25 && !player.hasStatusEffect(StatusEffects.KnowsDarknessShard))
        {
            EngineCore.addButton(3, "D.Shar", blackSpellbook1, 2).hint("Learn Darkness Shard spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(3, "D.Shar", "You not have have enough int or already know this spell.");
        }
        if (player.inte >= 30 && !player.hasStatusEffect(StatusEffects.KnowsArouse))
        {
            EngineCore.addButton(4, "Arouse", blackSpellbook1, 3).hint("Learn Arouse spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(4, "Arouse", "You not have have enough int or already know this spell.");
        }
        if (player.inte >= 35 && !player.hasStatusEffect(StatusEffects.KnowsRegenerate))
        {
            EngineCore.addButton(5, "Regene", blackSpellbook1, 4).hint("Learn Regenerate spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(5, "Regene", "You not have have enough int or already know this spell.");
        }
        if (player.inte >= 40 && !player.hasStatusEffect(StatusEffects.KnowsMight))
        {
            EngineCore.addButton(6, "Might", blackSpellbook1, 5).hint("Learn Might spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(6, "Might", "You not have have enough int or already know this spell.");
        }
        if (player.inte >= 45 && !player.hasStatusEffect(StatusEffects.KnowsBlink))
        {
            EngineCore.addButton(7, "Blink", blackSpellbook1, 6).hint("Learn Blink spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(7, "Blink", "You not have have enough int or already know this spell.");
        }
        if (player.inte >= 50 && !player.hasStatusEffect(StatusEffects.KnowsArcticGale))
        {
            EngineCore.addButton(8, "A.Gale", blackSpellbook1, 7).hint("Learn Arctic Gale spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(8, "A.Gale", "You not have have enough int or already know this spell.");
        }
        if (player.inte >= 55 && !player.hasStatusEffect(StatusEffects.KnowsDuskWave))
        {
            EngineCore.addButton(9, "D.Wave", blackSpellbook1, 8).hint("Learn Dusk Wave spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(9, "D.Wave", "You not have have enough int or already know this spell.");
        }
        if (player.inte >= 60 && !player.hasStatusEffect(StatusEffects.KnowsWaveOfEcstasy))
        {
            EngineCore.addButton(10, "W.Ecst", blackSpellbook1, 9).hint("Learn Wave of Ecstasy spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(10, "W.Ecst", "You not have have enough int or already know this spell.");
        }
        if (player.inte >= 100 && !player.hasStatusEffect(StatusEffects.KnowsIceRain))
        {
            EngineCore.addButton(11, "I.Rain", blackSpellbook1, 10).hint("Learn Ice Rain spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(11, "I.Rain", "You not have have enough int or already know this spell.");
        }
        if (player.inte >= 110 && !player.hasStatusEffect(StatusEffects.KnowsNosferatu))
        {
            EngineCore.addButton(12, "Nosfer", blackSpellbook1, 11).hint("Learn Nosferatu spell.");
        }
        else
        {
            EngineCore.addButtonDisabled(12, "Nosfer", "You not have have enough int or already know this spell.");
        }
    ...
    public function blackSpellbook2() : Void
    {
        ...
        if (player.hasPerk(PerkLib.PrestigeJobNecromancer) || player.hasPerk(PerkLib.PrestigeJobGreySage))
        {
            ...
            {
                EngineCore.addButton(1, "B.Spir", blackSpellbook1, 12).hint("Learn Bone spirit spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(1, "B.Spir", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 105 && !player.hasStatusEffect(StatusEffects.KnowsBoneArmor))
            {
                EngineCore.addButton(2, "B.Armo", blackSpellbook1, 13).hint("Learn Bone armor spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(2, "B.Armo", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 110 && !player.hasStatusEffect(StatusEffects.KnowsCorpseExplosion))
            {
                EngineCore.addButton(3, "C.Expl", blackSpellbook1, 14).hint("Learn Corpse Explosion spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(3, "C.Expl", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 115 && !player.hasStatusEffect(StatusEffects.KnowsBloodShower))
            {
                EngineCore.addButton(4, "B.Show", blackSpellbook1, 15).hint("Learn Blood Shower spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(4, "B.Show", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 120 && !player.hasStatusEffect(StatusEffects.KnowsBoneshatter))
            {
                EngineCore.addButton(5, "Bonesh", blackSpellbook1, 16).hint("Learn Boneshatter spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(5, "Bonesh", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 125 && !player.hasStatusEffect(StatusEffects.KnowsNecroticRot))
            {
                EngineCore.addButton(6, "N.Rot", blackSpellbook1, 17).hint("Learn Necrotic Rot spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(6, "N.Rot", "You not have have enough int or already know this spell.");
            }
        }
        if (player.hasPerk(PerkLib.HexKnowledge) || player.hasPerk(PerkLib.PrestigeJobGreySage))
        {
            ...
            {
                EngineCore.addButton(7, "Lifeta", blackSpellbook1, 18).hint("Learn Lifetap spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(7, "Lifeta", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 160 && !player.hasStatusEffect(StatusEffects.KnowsLifeSiphon))
            {
                EngineCore.addButton(8, "L.Siph", blackSpellbook1, 19).hint("Learn Life siphon spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(8, "L.Siph", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 170 && !player.hasStatusEffect(StatusEffects.KnowsConsumingDarkness))
            {
                EngineCore.addButton(9, "C.Dark", blackSpellbook1, 20).hint("Learn Consuming darkness spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(9, "C.Dark", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 180 && !player.hasStatusEffect(StatusEffects.KnowsCurseOfDesire))
            {
                EngineCore.addButton(10, "C.Desi", blackSpellbook1, 21).hint("Learn Curse of Desire spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(10, "C.Desi", "You not have have enough int or already know this spell.");
            }
            if (player.inte >= 190 && !player.hasStatusEffect(StatusEffects.KnowsCurseOfWeeping))
            {
                EngineCore.addButton(11, "C.Weep", blackSpellbook1, 22).hint("Learn Curse of Weeping spell.");
            }
            else
            {
                EngineCore.addButtonDisabled(11, "C.Weep", "You not have have enough int or already know this spell.");
            }
        }
    }
    ...
    public function blackSpellbook1(spell : Float) : Void
    {
        clearOutput();
