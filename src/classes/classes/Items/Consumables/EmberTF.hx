...
class EmberTF extends BaseContent
{
    ...
    public function dragonTFeffects(drakesHeart : Bool = false) : Void
    {
        var changes : Int = 0;
        var changeLimit : Int = as3hx.Compat.parseInt(2 + player.additionalTransformationChances);
        //Temporary storage
        var temp : Float = 0;
	...
	*/
        ...
        if (player.racialScore(Races.DRAGON, false) >= 4)
        {
            if (changes < changeLimit && !player.hasPerk(PerkLib.DragonFireBreath))
            {
                ...
                if (SceneLib.emberScene.emberAffection() >= 75 && !drakesHeart)
                {
        ...
        if (changes < changeLimit && rand(2) == 0 && player.basetallness < 132)
        {
            ...
            if (player.basetallness >= 120)
            {
                temp = Math.floor(temp / 3.5);
            }
            if (player.basetallness >= 96 && player.basetallness < 120)
            {
        ...
        var canReactMale : Bool = player.hasCock() && (drakesHeart || SceneLib.emberScene.hasVagina());
        var canReactFemale : Bool = player.hasVagina() && (drakesHeart || SceneLib.emberScene.hasCock());
        if (player.racialScore(Races.DRAGON, false) >= 4 && rand(3) == 0 && (canReactMale || canReactFemale))
        {
            ...
            if (canReactMale && (rand(2) == 0 || !canReactFemale))
