...
class PhoukaWhiskey extends Consumable
{
    ...
    //This function provides a single common test that can be used both by this class and the PhoukaScene class
    {
        ...
        if (!player.isPregnant())
        {
            ...
            {
                return 0;
            }  //No baby. Simplest, most common case  
        ...
        //Double pregnancy
        var wombBabyLikesAlcohol : Bool = ((player.pregnancyType == PregnancyStore.PREGNANCY_SATYR) || (player.pregnancyType == PregnancyStore.PREGNANCY_FAERIE) || (player.pregnancy2Type == PregnancyStore.PREGNANCY_SATYR) || (player.pregnancy2Type == PregnancyStore.PREGNANCY_FAERIE));
        var colonBabyLikesAlcohol : Bool = (player.buttPregnancyType == PregnancyStore.PREGNANCY_SATYR);
    ...
    //This function provides a single common test that can be used both by this class and the PhoukaScene class
    {
        ...
        {
            if (game.flags[kFLAGS.PREGNANCY_CORRUPTION] == 0)
            {
                return 2;
            }
            if (game.flags[kFLAGS.PREGNANCY_CORRUPTION] < 0)
    ...
    public function phoukaWhiskeyAddStatus(player : Player) : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.PhoukaWhiskeyAffect))
        {
            ...
            {
                player.addStatusValue(StatusEffects.PhoukaWhiskeyAffect, 1, 1);
    ...
    public function phoukaWhiskeyExpires(player : Player) : Void
    {
        ...
        var libidoChange : Int = as3hx.Compat.parseInt((libidoSensCombined - sensChange) / 256);
        var intChange : Int = intSpeedCombined & 255;
        var speedChange : Int = as3hx.Compat.parseInt((intSpeedCombined - intChange) / 256);
