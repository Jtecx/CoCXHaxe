...
class AbstractEquinum extends Consumable
{
    
    
    public function new(type : Float, id : String, shortName : String, longName : String, value : Float, description : String)
    {
    ...
    override public function useItem() : Bool
    {
        ...
        if (type == 0 && player.isFurCovered() && player.faceType == Face.HORSE && player.tailType == Tail.HORSE && (player.lowerBody != LowerBody.HOOFED) && !player.hasPerk(PerkLib.TransformationResistance))
        {
            ...
            else
            {
                ...
                else if (player.getStatusValue(StatusEffects.TFWarning, 2) == 1)
                {
        ...
        //If cocks that aren't horsified!{
            ...
            if (rand(2) == 0 && changes < changeLimit && player.horseCocks() > 0)
            {
                ...
                {
                    outputText(" to be a rutting stud.  You ache to find a mare or centaur to breed with.  Longing to spend your evenings plunging a " + Appearance.cockNoun(CockTypesEnum.HORSE) + " deep into their musky passages, dumping load after load of your thick animal-cum into them.  You'd be happy just fucking horsecunts morning, noon, and night.  Maybe somewhere there is a farm needing a breeder..");
                }
        ...
        if (!player.isFurCovered() && !player.isTaur() && !player.isGargoyle() && changes < changeLimit && rand(4) == 0 && player.tailType == Tail.HORSE)
        {
            ...
            if (type == 0)
            {
                CoC.instance.transformations.SkinFur(Skin.COVERAGE_COMPLETE, {
                                    colors : HorseRace.FurColors
                                }).applyEffect();
            }
            else
            {
                CoC.instance.transformations.SkinFur(Skin.COVERAGE_COMPLETE, {
                                    colors : ["platinum blonde", "silver", "white", "pure white"]
                                }).applyEffect();
            }
            changes++;
        }
        ...
        if ((type == 1 || type == 2) && changes < changeLimit && rand(3) == 0 && player.ears.type == Ears.HORSE && player.tailType == Tail.HORSE)
        {
            ...
            //Get bigger if player has horns{
                ...
                if (player.horns.type == Horns.UNICORN)
                {
                    ...
                    {
                        outputText("It is the size of tiny nub.");
                    }
                    ...
                    {
                        outputText("It look like the horn on a grown unicorn, big enough and dangerous enough to do some damage.");
                    }
                    ...
                    {
                        outputText("It is large, pointed and spiraling horn.");
                    }
                    changes++;
                }
                //If no horns yet..
