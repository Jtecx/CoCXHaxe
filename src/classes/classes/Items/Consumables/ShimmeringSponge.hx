 ...
 * @author Svalkash
 * */
class ShimmeringSponge extends Consumable
{
    public function new()
    {
    ...
    override private function get_description() : String
    {
        ...
        if (EngineCore.silly())
        {
            desc += "You swear, every so often, you can hear faint laughter, like \"<i>Dalhahahaha!</i>\" Inside your head. You get the vague feeling that this sponge should have pants.";
        }
    ...
    override public function useItem() : Bool
    {
        ...
        EngineCore.menu();
        EngineCore.addButton(0, "Ass", useAss).disableIf(player.ass.analWetness == AssClass.WETNESS_DRY, "It's completely dry!");
        if (player.hasVagina())
        {
            EngineCore.addButton(1, "Pussy", usePussy).disableIf(player.vaginas[0].vaginalWetness == VaginaClass.WETNESS_DRY, "It's completely dry!");
        }
        if (player.hasBalls())
        {
            EngineCore.addButton(2, "Balls", useBalls).disableIf(player.cumMultiplier == 1, "Your production is at its minimum.\n<b>If it's still too high, please report this on Discord and I'll make it better.\n\n-Svalkash</b>");
        }
        if (player.hasBreasts())
        {
            EngineCore.addButton(3, "Breasts", useBreasts).disableIf(player.biggestLactation() == 0, "You don't lactate at all!");
        }
        ...
        function useAss() : Void
        {
            ...
            if (player.ass.analWetness == AssClass.WETNESS_DRY)
            {
                outputText("<b>Your ass is no longer lubricated.</b>");
            }
            else
            {
                outputText("<b>Your ass feels drier than before.</b>");
            }
            SceneLib.inventory.itemGoNext();
        };
        
        var usePussy : Void->Void = function() : Void
        {
            ...
            {
                outputText("<b>Your pussy is completely dry!</b>\n");
            }
            SceneLib.inventory.itemGoNext();
        }
        
        var useBalls : Void->Void = function() : Void
        {
            ...
            if (player.cumMultiplier >= 4)
            {
                player.cumMultiplier *= 0.75;
            }
            else
            {
                player.cumMultiplier -= 1;
            }
            if (player.cumMultiplier < 1)
            {
                player.cumMultiplier = 1;
            }
            if (player.cumMultiplier == 1)
        ...
        var useBreasts : Void->Void = function() : Void
        {
