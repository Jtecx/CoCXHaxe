...
class BimboLiqueur extends Consumable
{
    ...
    {
        if (!player.blockingBodyTransformations() || !game.player.hasPerk(PerkLib.FutaForm))
        {
            return true;
        }
    ...
    override public function useItem() : Bool
    {
        if (game.player.hasPerk(PerkLib.BroBody))
        {
            ...
            if (game.player.biggestTitSize() < 7)
            {
                ...
                {
                    outputText("  You feel a tingling inside your breasts.  ");
                }
            ...
            if (!game.player.hasVagina())
            {
                ...
                {
                    outputText("  As if realizing the necessity of womanly proportions to attract the hard cocks your body now craves, your waist pinches slightly inward and your hips and butt swell.  You can't help but run a hand across your newly-feminized pelvis, admiring it.");
                }
                CoC.instance.transformations.VaginaHuman().applyEffect(false);
                if (game.player.hips.type < 12)
                {
                    game.player.hips.type = 12;
                }
                if (game.player.butt.type < 12)
                {
            ...
            if (game.player.femininity < 80)
            {
                game.player.femininity = 80;
            }
        ...
        else
        {
            ...
            if (game.player.hairColor != "platinum blonde")
            {
                ...
                {
                    outputText("as your hair grows thicker and heavier.");
                }
            ...
            if (!game.player.hasVagina())
            {
                ...
                {
                    outputText("Wait!? Wet? You wish you could touch yourself between the " + game.player.legs() + ", but you can tell from the fluid running down your hind-legs just how soaked your new vagina is.");
                }
                else
                {
                    outputText("Wait!?  Wet?  You touch yourself between the " + game.player.legs() + " and groan when your fingers sink into a sloppy, wet cunt.");
                }
            }
            else if (game.player.isTaur())
            {
            ...
            if (game.player.tone > 0)
            {
                ...
                if (game.player.str >= 30)
                {
                    ...
                    {
                        game.player.dynStats("str", -10);
                    }
                    if (game.player.str >= 70)
                    {
                        game.player.dynStats("str", -10);
                    }
