 ...
 * @author Kitteh6660
 */
class GoldenRind extends Consumable
{
    ...
    override public function useItem() : Bool
    {
        ...
        if (rand(3) == 0 && changes < changeLimit && player.horns.type == Horns.ANTLERS && player.horns.count < 34)
        {
            ...
            if (player.horns.count < 20 && rand(2) == 0)
            {
                player.horns.count += (1 + rand(4));
            }
            player.horns.count++;
            outputText("  After counting the number of points you have on your antlers, <b>you have " + player.horns.count + " points.</b>");
            if (player.horns.count >= 34)
            {
                outputText("<b>  It seems that your antlers can't get any more pointier.</b>");
            }
            changes++;
        }
        //Gain fur
        ...
        if (rand(3) == 0 && changes < changeLimit && player.ears.type == Ears.DEER && player.tailType == Tail.DEER && player.isFurCovered() && (player.lowerBody != LowerBody.DEERTAUR && player.lowerBody != LowerBody.CLOVEN_HOOFED))
        {
            ...
            if (player.isTaur())
            {
                player.lowerBody = LowerBody.DEERTAUR;
            }
            else
            {
                CoC.instance.transformations.LowerBodyClovenHoofed(2).applyEffect(false);
            }
            changes++;
        }
        //Change arms to deer
        ...
        if (rand(3) == 0 && player.thickness > 20)
        {
            ...
            {
                player.modThickness(20, 2 + rand(3));
            }
            else
            {
                player.modThickness(20, 2);
            }
        }
