...
class AsumaKirin extends Consumable
{
    public function new()
    {
    ...
    override public function useItem() : Bool
    {
        ...
        if ((player.gender == 1 || player.gender == 3) && rand(3) == 0 && changes < changeLimit)
        {
            ...
            else
            {
                ...
                if (temp2 > 2)
                {
                    outputText("Your " + player.cockDescript(temp) + " tightens painfully, inches of taut horse-flesh pouring out from your sheath as it grows longer.  Thick animal-pre forms at the flared tip, drawn out from the pleasure of the change.");
                }
                if (temp2 > 1 && temp2 <= 2)
                {
                    outputText("Aching pressure builds within your sheath, suddenly releasing as an inch or more of extra dick flesh spills out.  A dollop of pre beads on the head of your enlarged " + player.cockDescript(temp) + " from the pleasure of the growth.");
                }
                if (temp2 <= 1)
                {
                    outputText("A slight pressure builds and releases as your " + player.cockDescript(temp) + " pushes a bit further out of your sheath.");
                }
                changes++;
            }
            ...
            if (rand(2) == 0 && changes < changeLimit && player.kirinCocks() > 0)
            {
                ...
                if (player.cor + player.lib < 60)
                {
                    outputText(" to have such a splendid tool.  You idly daydream about cunts and pussies, your " + Appearance.cockNoun(CockTypesEnum.HORSE) + " plowing them relentlessly, stuffing them pregnant with cum");
                }
                if (player.cor + player.lib >= 60 && player.cor + player.lib < 100)
                {
                ...
                if (player.cor + player.lib >= 100 && player.cor + player.lib <= 175)
                {
                    outputText(" to be a rutting stud.  You ache to find a mare or centaur to breed with.  Longing to spend your evenings plunging a " + Appearance.cockNoun(CockTypesEnum.KIRIN) + " deep into their musky passages, dumping load after load of your thick animal-cum into them.  You'd be happy just fucking horsecunts morning, noon, and night.  Maybe somewhere there is a farm needing a breeder..");
                }
                if (player.cor + player.lib > 175)
                {
                    ...
                    if (!player.isTaur())
                    {
                        outputText(" Your hand strokes your horsecock of its own accord, musky pre dripping from the flared tip with each stroke.");
                    }
                    else
                    {
                        outputText(" Your flared tip begins dripping large amount of plasma as arousal overtake your member.");
                    }
        ...
        if (!player.hasPartialCoat(Skin.DRAGON_SCALES) && !player.isGargoyle() && changes < changeLimit && rand(3) == 0)
        {
            ...
            if (!InCollection(player.coatColor, KirinRace.KirinScaleColors))
            {
                scaleColor = randomChoice(KirinRace.KirinScaleColors);
            }
        ...
        //Get bigger if player has horns{
            ...
            if (player.horns.type == Horns.KIRIN)
            {
                ...
                {
                    outputText("It is the size of a tiny nub.");
                }
                if (player.horns.count >= 3 && player.horns.count < 6)
                {
                    outputText("It is similar to what you would see on a young unicorn.");
                }
                if (player.horns.count >= 6 && player.horns.count < 12)
                {
                    outputText("It look like the horn on a grown kirin, big enough and dangerous enough to do some damage.");
                }
                if (player.horns.count >= 12 && player.horns.count < 20)
                {
                    outputText("It is large and wicked looking.");
                }
                if (player.horns.count >= 20)
                {
                    outputText("It is a large, pointed and spiraling horn.");
                }
                changes++;
            }
            //If no horns yet..
        ...
        {
            outputText("[pg]");
            if (CoC.instance.transformations.EyesKirinColors.isPossible())
            {
                CoC.instance.transformations.EyesKirinColors.applyEffect();
            }
            if (CoC.instance.transformations.EyesDraconic.isPossible())
            {
