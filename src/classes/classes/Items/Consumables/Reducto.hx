...
@:final class Reducto extends Consumable
{
    ...
    override public function useItem() : Bool
    {
        ...
        if (player.hasCock())
        {
            EngineCore.addButton(0, "Cock", reductoCock).disableIf(player.longestCockLength() <= 2 && player.thickestCockThickness() <= 0.5, "It can't shrink further!");
        }
        if (player.hasBalls())
        {
            EngineCore.addButton(1, "Balls", pickDoses, reductoBalls).disableIf(player.ballSize == 1, "They can't be any smaller!");
        }
        if (player.biggestTitSize() > 0)
        {
            EngineCore.addButton(2, "Breasts", reductoBreasts);
        }
        EngineCore.addButton(3, "Nipples", pickDoses, reductoNipples).disableIf(player.nippleLength <= 0.25, "Minimum size reached.");
        if (player.hasVagina())
        {
            EngineCore.addButton(4, "Clit", pickDoses, reductoClit).disableIf(player.clitLength <= 0.25, "Minimum size reached.");
        }
        EngineCore.addButton(5, "Butt", pickDoses, reductoButt).disableIf(player.butt.type <= 1, "Not thick anymore.");
        EngineCore.addButton(6, "Hips", pickDoses, reductoHips).disableIf(player.hips.type <= 2, "Already thin.");
        if (player.horns.count > 2)
        {
            EngineCore.addButton(7, "Horns", shrinkHorns);
        }
        EngineCore.addButton(14, "Never mind", reductoCancel);
        return true;
    }
    
    private function sharedEnd(dose : Int, sexual : Bool = false) : Void
    {
        player.dynStats("lus", -10 * dose, "scale", false);
        if (sexual)
        {
            game.player.dynStats("sen", -2 * dose);
        }
        if (dose > 1 && !CoC.instance.debug)
        {
            player.consumeItem(game.consumables.REDUCTO, dose - 1);
        }  //eat up more reds  
    ...
    private function reductoBalls(dose : Int) : Void
    {
        ...
        if (game.player.ballSize < 1)
        {
            game.player.ballSize = 1;
        }
    ...
    private function reductoBreasts() : Void
    {
        ...
        var shrink : Int->Int->Void = function(row1 : Int, dose : Int) : Void
        {
            var d : Int = dose;
            var randProcs : Int = 0;
            clearOutput();
    ...
    private function reductoButt(dose : Int) : Void
    {
        ...
        while (d-- > 0)
        {
            if (game.player.butt.type >= 15)
            {
                game.player.butt.type -= (3 + as3hx.Compat.parseInt(game.player.butt.type / 3));
            }
            else if (game.player.butt.type >= 10)
            {
        ...
        if (game.player.butt.type < 1)
        {
            game.player.butt.type = 1;
        }
        sharedEnd(dose);
    }
    ...
    private function reductoClit(dose : Int) : Void
    {
        ...
        if (game.player.clitLength < 0.25)
        {
            game.player.clitLength = 0.25;
        }
        //Set clitlength down to 2 digits in length
        game.player.clitLength = as3hx.Compat.parseInt(game.player.clitLength * 100) / 100;
    ...
    private function reductoCock() : Void
    {
        ...
        var shrink : Int->String->Int->Void = function(dick : Int, part : String, dose : Int) : Void
        {
            var d : Int = dose;
            clearOutput();
    ...
    private function reductoHips(dose : Int) : Void
    {
        ...
        while (d-- > 0)
        {
            if (game.player.hips.type >= 15)
            {
                game.player.hips.type -= (3 + as3hx.Compat.parseInt(game.player.hips.type / 3));
            }
            else if (game.player.hips.type >= 10)
            {
        ...
        if (game.player.hips.type < 1)
        {
            game.player.hips.type = 1;
        }
        sharedEnd(dose);
    }
    ...
    {
        var cnt : Int = as3hx.Compat.parseInt(player.itemCount(game.consumables.REDUCTO) + 1);
        if (cnt == 0)
        {
            fun(1);
        }
