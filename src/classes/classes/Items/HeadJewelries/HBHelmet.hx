...
class HBHelmet extends HeadJewelry
{
    ...
    override public function canEquip(doOutput : Bool, slot : Int) : Bool
    {
        ...
        if (doOutput)
        {
            outputText("You try to put helmet on but it clearly wasn't designed for someone your size (less than 7 feet are we?). Frustrated, you put it back in the bag.  ");
        }
        return false;
    }
}
