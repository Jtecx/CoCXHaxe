...
class LactoBlasters extends WeaponRange
{
    ...
    {
        if (game.player.hasPerk(PerkLib.DualWield) && game.player.str >= 100 && game.player.biggestTitSize() > 4)
        {
        ...
        if (doOutput)
        {
            ...
            {
                outputText("You have yet to master the art of dual wielding! ");
            }
            if (game.player.str < 100)
            {
                outputText("This setup is to heavy for you to wield! ");
            }
            if (game.player.biggestTitSize() < 4)
            {
    ...
    {
        var lactoatk : Float = (1 + (game.player.lactationQ() / 100));
        lactoatk = FnHelpers.FN.logScale(lactoatk, LactoblastersABC, 10);
        return lactoatk;
    }
    
    private static var LactoblastersABC : Dynamic = FnHelpers.FN.buildLogScaleABC(10, 100, 1000, 10, 100);
}
