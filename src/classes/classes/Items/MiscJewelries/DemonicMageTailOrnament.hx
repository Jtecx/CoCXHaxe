...
class DemonicMageTailOrnament extends MiscJewelry
{
    ...
    {
        if (game.player.tailType == Tail.DEMONIC)
        {
            return true;
        }
        if (doOutput)
        {
            ...
            if (game.player.tailType == Tail.NONE)
            {
                outputText(", let alone a tail at all");
            }
