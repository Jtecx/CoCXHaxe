...
class Nexus extends Weapon implements TimeAwareInterface
{
    ...
    {
        var multadd : Float = 0.6;
        var checkedNeutralityBonus : Float = 4.4;
        if (game.player.cor > 50)
        {
            checkedNeutralityBonus = 4.4 - ((game.player.cor - 50) * 0.088);
        }
        if (game.player.cor < 50)
        {
            checkedNeutralityBonus = 4.4 - ((50 - game.player.cor) * 0.088);
        }
