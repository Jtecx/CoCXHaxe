...
/**
	 ...
	 * @author ...
	 */
class CharSpecial extends BaseContent
{
    ...
    public var customs(default, never) : Array<Dynamic> = [
        //
    ...
    private function customZavos() : Void
    {
        ...
        {
            player.createKeyItem("Backpack", 4, 0, 0, 0);
        }
    ...
    private function customKitteh6660() : Void
    //Not yet implemented.
    {
    ...
    // Herm kitsune-dragon with 3 tentacle cocks. And tentacle hair. And tentacle (well, draconic, but sort of) tongue.
    {
        ...
        if (player.armor.isNothing || player.armor == armors.C_CLOTH)
        {
            player.setArmor(armors.LTHRROB);
        }  // you like concealing clothes, your body is your masterpiece, but your extra benefits are more fun when not expected... ok, you are a bit shy of your tentacles  
        if (player.weapon.isNothing)
        {
            player.setWeapon(weapons.SUCWHIP);
        }  // have succubus whip, to get corruption quickly when desired  
        ...
        if (player.hasKeyItem("Backpack") < 0)
        {
            player.createKeyItem("Backpack", 6, 0, 0, 0);
        }
        if (player.hasKeyItem("Camp - Chest") < 0)
        {
            player.createKeyItem("Camp - Chest", 0, 0, 0, 0);for (i in 0...6)
            {
                inventory.createStorage();
            }
        }
        ...
        {
            player.createKeyItem("Camp - Murky Chest", 0, 0, 0, 0);for (i in 0...4)
            {
                inventory.createStorage();
            }
        }
        ...
        {
            player.createKeyItem("Camp - Ornate Chest", 0, 0, 0, 0);for (i in 0...4)
            {
                inventory.createStorage();
            }
        }
        ...
        if (!player.hasPerk(PerkLib.PastLifeReligious) && !player.hasPerk(PerkLib.HistoryReligious))
        {
            player.createPerk(PerkLib.HistoryReligious, 0, 0, 0, 0);
        }  // abandoned religion after obtaining nine tails and being disappointed in achieved enlightment  
        if (!player.hasPerk(PerkLib.PastLifeFighter) && !player.hasPerk(PerkLib.HistoryAlchemist))
        {
            player.createPerk(PerkLib.HistoryAlchemist, 0, 0, 0, 0);
        }  // and resorted to your hobby - alchemy  
        if (!player.hasPerk(PerkLib.TransformationResistance))
        {
            player.createPerk(PerkLib.TransformationResistance, 0, 0, 0, 0);
        }  // tf resistance and alchemist are actually mutually nullifying each other - this is flavor mostly  
        if (!player.hasPerk(PerkLib.PastLifeHealer) && !player.hasPerk(PerkLib.HistoryHealer))
        {
            player.createPerk(PerkLib.HistoryHealer, 0, 0, 0, 0);
        }  // with religious and alchemical knowlege you are skilled healer  
        if (!player.hasPerk(PerkLib.Medicine))
        {
            player.createPerk(PerkLib.Medicine, 0, 0, 0, 0);
        }  // able to treat wounds and poisoning alike  
        if (!player.hasPerk(PerkLib.AscensionMoralShifter))
        {
            player.createPerk(PerkLib.AscensionMoralShifter, 5, 0, 0, 0);
        }  // your kitsune's trickster nature makes you susceptable to corruption  
        if (!player.hasPerk(PerkLib.AscensionTolerance))
        {
            player.createPerk(PerkLib.AscensionTolerance, 10, 0, 0, 0);
        }  // but in the same time your enlightment keeps you from really turning to demon, so corruption level does not really affect you much  
        if (!player.hasPerk(PerkLib.Fast))
        {
            player.createPerk(PerkLib.Fast, 1, 0, 0, 0);
        }  // gaining speed is pain in ass... this one is not for history flavor  
        if (!player.hasPerk(PerkLib.Smart))
        {
            player.createPerk(PerkLib.Smart, 1, 0, 0, 0);
        }  // int is easy to get, just for history flavor  
        if (!player.hasPerk(PerkLib.Lusty))
        {
            player.createPerk(PerkLib.Lusty, 1, 0, 0, 0);
        }  // have a lust problem  
        if (!player.hasPerk(PerkLib.HotBlooded))
        {
            player.createPerk(PerkLib.HotBlooded, 0, 0, 0, 0);
        }  // even with your willpower and religious training you sometimes struggling to restrain your impulse  
        if (!player.hasPerk(PerkLib.Pervert))
        {
            player.createPerk(PerkLib.Pervert, 0.25, 0, 0, 0);
        }  // you always ready for something VERY lewd  
        if (!player.hasPerk(PerkLib.Masochist))
        {
            player.createPerk(PerkLib.Masochist, 0, 0, 0, 0);
        }  // with your knowlege of healing and innatural body it is easy for you to enjoy things which would be really painful for others  
        if (!player.hasPerk(PerkLib.Sadist))
        {
            player.createPerk(PerkLib.Sadist, 0, 0, 0, 0);
        }  // and you are always ready to return favor  
        if (!player.hasPerk(PerkLib.SensualLover))
        {
            player.createPerk(PerkLib.SensualLover, 0, 0, 0, 0);
        }  // still you tend to care about mutual enjoyment - there are difference between extreme entertainment and torture, and you are mischievous, not evil  
        if (!player.hasPerk(PerkLib.SpellcastingAffinity))
        {
            player.createPerk(PerkLib.SpellcastingAffinity, 80, 0, 0, 0);
        }  // very strong magic affinity, can even be effective as mage without robe  
        if (!player.hasPerk(PerkLib.Channeling))
        {
            player.createPerk(PerkLib.Channeling, 0, 0, 0, 0);
        }  // despite strong magic affinity wasn't actually much interested in combat magic before, so only can use fox fire as offensive spell  
        if (!player.hasPerk(PerkLib.Spellpower))
        {
            player.createPerk(PerkLib.Spellpower, 0, 0, 0, 0);
        }
        ...
        }  // some of your experiments had nice returns  
        ...
        }  // and some brought much more problems than benefits, but you wasn't able to reverse all of them. well, this is occupational hazard for alchemist  
        // you have no idea how you got this... heritage, maybe?
        if (debug && !player.hasPerk(PerkLib.FerasBoonAlpha))
        {
            player.createPerk(PerkLib.FerasBoonAlpha, 0, 0, 0, 0);
        }
        if (debug && !player.hasPerk(PerkLib.FerasBoonBreedingBitch))
        {
        ...
        if (player.itemSlot1.isEmpty())
        {
            player.itemSlot1.setItemAndQty(consumables.L_PNKEG, 1);
        }  // if you want to get rid of tentacles, but still maintain dragon one you'll have to find Ceraph  
        if (player.itemSlot2.isEmpty())
        {
            player.itemSlot2.setItemAndQty(consumables.L_BLUEG, 1);
        }
        if (debug && player.itemSlot3.isEmpty())
        {
            player.itemSlot3.setItemAndQty(consumables.REDUCTO, 5);
        }
        if (debug && player.itemSlot4.isEmpty())
        {
            player.itemSlot4.setItemAndQty(consumables.GROPLUS, 5);
        }
        ...
        if (debug)
        {
            player.createStatusEffect(StatusEffects.TelAdre, 1, 1, 0, 0);flags[kFLAGS.BAZAAR_ENTERED] = 1;
        }  // small straightforward cheating  
    ...
    // This one is sort of challenge character. It is supposed to be mage, at least at start.
    {
        ...
        do
        {
            ...
            {
                player.breastRows[row].breastRating = player.breastRows[row - 1].breastRating - rand(2);
            }  // lower rows are same size or one size smaller than upper  
            row++;
        }
        while ((row < 4 && rand(2) == 0 && player.breastRows[0].breastRating > 1));  // if last row was flat do not add more  
        
        var tent : Bool = rand(5) == 0;  // 20% chance to have tentacle cluster  
        var cocks : Float = rand(5) + ((tent) ? 4 : 2);  // 2-6 mixed cocks  
        var i : Int;
        for (i in 0...cocks)
        {
            player.createCock();
        ...
        else
        {
            ...
            player.faceType = rand(20) + 1;
        }  // since it is not a enum, it is impossible to make it auto-adjust...  
        
        if (player.faceType == Face.SPIDER_FANGS && rand(2) == 0)
        {
            player.eyes.type = Eyes.SPIDER;
        }
        // 50% to have spider eyes with spider fangs
        else if (rand(20) == 0)
        
        // 5% for inhuman eyes otherwise{
        ...
        // if human face, 50% to have human ears{
        ...
        else
        {
            ...
            player.ears.type = rand(14) + 1;
        }  // since it is not a enum, it is impossible to make it auto-adjust...  
        
        
        if (rand(2) != 0)
        
        // 50% to have human lower body{
        ...
        var horns : Float = rand(100);  // 70% no horns, 30% to random  
        if (horns < 70)
        {
            player.horns.type = Horns.NONE;
        }
        ...
        var wings : Float = rand(4);  // always have wings to fly... small boon to make up for lack of fighting power  
        if (wings == 0)
        {
            player.wings.type = Wings.BAT_LIKE_LARGE;
        }
        ...
        var arms : Float = rand(100);  // if have harpy wings 33% chance to have harpy hands, otherwise 5% to have spider hands  
        ...
        {
            player.hairType = randomChoice(Hair.FEATHER, Hair.GOO, Hair.ANEMONE);
        }
    ...
    // Feral Goblin Wolf-Girl Alchemist
    {
        ...
        // have lots of different traits
        if (!player.hasPerk(PerkLib.PastLifeFighter) && !player.hasPerk(PerkLib.HistoryFighter))
        {
            player.createPerk(PerkLib.HistoryFighter, 0, 0, 0, 0);
        }  // abandoned religion after obtaining nine tails and being disappointed in achieved enlightment  
        if (!player.hasPerk(PerkLib.PastLifeAlchemist) && !player.hasPerk(PerkLib.HistoryAlchemist))
        {
            player.createPerk(PerkLib.HistoryAlchemist, 0, 0, 0, 0);
        }  // and resorted to your hobby - alchemy  
        if (!player.hasPerk(PerkLib.PastLifeHealer) && !player.hasPerk(PerkLib.HistoryHealer))
        ...
        if (!player.hasPerk(PerkLib.PastLifeFeral) && !player.hasPerk(PerkLib.HistoryFeral))
        {
        ...
        {
            player.createPerk(PerkLib.Sensitive, 0, 0, 0, 0);
        }
        if (!player.hasPerk(PerkLib.Channeling))
        {
            player.createPerk(PerkLib.Channeling, 0, 0, 0, 0);
        ...
        {
            player.createPerk(PerkLib.Battlemage, 0, 0, 0, 0);
        }
        if (!player.hasPerk(PerkLib.Spellsword))
        {
            player.createPerk(PerkLib.Spellsword, 0, 0, 0, 0);
        }
        ...
        if (!player.hasPerk(PerkLib.BouncyBody))
        {
        ...
        }  // Prone to Excessive Transformations  
        ...
        if (player.itemSlot1.isEmpty())
        {
            player.itemSlot1.setItemAndQty(weapons.H_GAUNT, 1);
        }
        if (debug)
        {
            player.perkPoints += 1000;
        }
        if (debug)
        {
