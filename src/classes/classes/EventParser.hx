...
class EventParser
{
    public static var doCamp(never, set) : Function;

    private static var _timeAwareClassList : Array<TimeAwareInterface> = new Array<TimeAwareInterface>();
    ...
    private static var _campSet : Bool = false;
    ...
    public static function playerMenu() : Void
    {
        ...
        {
            if (DungeonAbstractContent.inRoomedDungeonResume != null)
            {
    ...
    public static var badEnded : Bool = false;  //set to 'true' to track bad ended state in save-load menu  
    
    public static function gameOver(clear : Bool = false) : Void
    //Leaves text on screen unless clear is set to true
    {
        ...
        {
            badEnded = true;
            var textChoices : Array<Dynamic> = ["GAME OVER", 
            ...
            if (CoC.instance.player.hasStatusEffect(StatusEffects.PCClone) && CoC.instance.player.statusEffectv3(StatusEffects.PCClone) >= 1)
            {
                EngineCore.addButton(2, "Rebirth", SceneLib.camp.rebirthFromBadEnd).hint("You can move your nascent soul into your body clone achieving rebirth.\n\n<b>UNSAFE: Some bad-ends are bugged and may set flags/statuses/debuffs/transformations, hence breaking the game state. Use at your own risk; if possible, load a save.</b>");
            }
            ...
            if (CoC.instance.flags[kFLAGS.EASY_MODE_ENABLE_FLAG] == 1 || CoC.instance.debug)
            {
                EngineCore.addButton(4, "Debug Cheat", debugCheatBadEnd).hint("<b>UNSAFE: Some bad-ends are bugged and may set flags/statuses/debuffs/transformations, hence breaking the game state. Use at your own risk; if possible, load a save.</b>");
            }
        ...
        {
            if (CoC.instance.flags[kFLAGS.NEISA_FOLLOWER] == 3)
            {
                CoC.instance.flags[kFLAGS.PLAYER_COMPANION_1] = "";
            }
            if (CoC.instance.player.hasStatusEffect(StatusEffects.ThereCouldBeOnlyOne))
            {
                CoC.instance.player.removeStatusEffect(StatusEffects.ThereCouldBeOnlyOne);
            }
    ...
    private static function gameOverMenuOverride() : Void
    //Game over event; override whatever the fuck has been done to the UI up to this point to force display of the data and new game buttons
    {
    ...
    public static function goNext(needNext : Bool) : Bool
    {
        Utils.Begin("eventParser", "goNext");
        var x : Bool = goNextWrapped(needNext);
        Utils.End("eventParser", "goNext");
        return x;
    }
    
    private static function goNextWrapped(needNext : Bool) : Bool
    {
        ...
        //Finish calling timeChangeLarge before advancing the hour again{
            ...
            while (timeAwareLargeLastEntry < _timeAwareClassList.length)
            {
                var item : TimeAwareInterface = _timeAwareClassList[timeAwareLargeLastEntry];
                var classname : String = Type.getClassName(item);
        ...
        var time : Float = CoC.instance.timeQ;
        ...
        // Advance minutes, but only 1 hour at a time{
            ...
            if (player.statStore.recentlyRemovedTags["IzumiSmoke"] != null)
            {
            ...
            if (player.statStore.recentlyRemovedTags["AndysSmoke"] != null)
            {
            ...
            if (player.statStore.recentlyRemovedTags["DrunkenPowerEmpower"] != null)
            {
                EngineCore.outputText("\nYou sober up, loosing the benefits of your oni drunken rampage.\n");
            }
            if (player.statStore.recentlyRemovedTags["AelfwineEmpower"] != null)
            {
                EngineCore.outputText("\nYou sober up, loosing the benefits of Aelfwine.\n");
            }
            if (player.statStore.recentlyRemovedTags["Hangover"] != null)
            {
                EngineCore.outputText("\nYour head finally clears as your hangover wears off. Drinking with the shemale lizard was definitely a bad idea.\n");
            }
            if (player.statStore.recentlyRemovedTags["Feeding Euphoria"] != null)
            {
            ...
            {
                SceneLib.combat.soulforceregeneration1(false);
            }
            if (player.hasPerk(PerkLib.JobSorcerer) || player.hasPerk(PerkLib.JobElementalConjurer))
            {
                SceneLib.combat.manaregeneration1(false);
            }
            ...
            for (tac in 0..._timeAwareClassList.length)
            {
                item = _timeAwareClassList[tac];
                classname = Type.getClassName(item);
            ...
            if (CoC.instance.model.time.hours > 23)
            {
                ...
                player.HP = HPPercent * player.maxHP();
                if (player.statStore.recentlyRemovedTags["KitsuneShrine"] != null)
                {
                ...
                if (player.statStore.recentlyRemovedTags["WellFed"] != null)
                {
                    EngineCore.outputText("\nYou begin feeling hungry again as the satisfaction of your last cooked meal has all but disapeared.\n");
                }
                if (player.statStore.recentlyRemovedTags["TaothBlessing"] != null || player.statStore.recentlyRemovedTags["FenrirBlessing"] != null)
                {
            ...
            {
                if (CoC.instance.flags[kFLAGS.LETHICE_DEFEATED] <= 0)
                {
            ...
            timeAwareLargeLastEntry = 0;
            while (timeAwareLargeLastEntry < _timeAwareClassList.length && !DungeonAbstractContent.inDungeon)
            {
        ...
        if (Std.is(player.armor, LustyMaidensArmor) || Std.is(player.armor, SuccubusArmor) || Std.is(player.armor, BattleMaidenArmor))
        ...
        if (((player.weapon.isSingleLarge() && player.weapon != CoC.instance.weapons.AETHERD && !player.hasPerk(PerkLib.GigantGrip) && !player.hasPerk(PerkLib.AntyDexterity)) || (player.weapon.isDualMedium() && player.weapon != CoC.instance.weapons.AETHERD && !player.hasPerk(PerkLib.AntyDexterity)) || (player.weapon.isDualLarge() && player.weapon != CoC.instance.weapons.AETHERD) || player.weapon == CoC.instance.weapons.DAISHO) && !player.shield.isNothing)
        {
        ...
        {
            var counter : Float = player.cockTotal() - 1;
            while (counter >= 0)
            {
    ...
    //Egg loot!
    {
        ...
        if (CoC.instance.flags[kFLAGS.FEMOIT_EGGS] > 0)
        {
            CoC.instance.flags[kFLAGS.FEMOIT_INCUBATION]--;
    ...
    private static function pregnancyProgress() : Int
    {
        ...
        {
            if (flags[kFLAGS.DIAPAUSE_FLUID_STORE] <= 0 || !player.isPregnant() && !player.isButtPregnant())
            
            //no pregnancy, I guess?{
    ...
    //IMP GANGBAAAAANGA
    {
        ...
        var player : Player = CoC.instance.player;
        var chance : Float = player.statusEffectv1(StatusEffects.BirthedImps) * 2;
        if (chance > 7)
        {
            chance = 7;
        }
        ...
        {
            chance /= 1 + (flags[kFLAGS.CAMP_WALL_PROGRESS] / 100);
        }
        ...
        {
            chance *= 1 - (flags[kFLAGS.CAMP_WALL_SKULLS] / 100);
        }
        if (CoC.instance.model.time.hours == 2)
        {
            if (flags[kFLAGS.KID_A_GOBLIN_NIGHT_HAPPENED] == null && flags[kFLAGS.ANEMONE_KID] > 0 && player.hasCock() && flags[kFLAGS.ANEMONE_WATCH] > 0 && flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS] >= 40)
            {
            ...
            else if (chance > Utils.rand(100) && !player.hasStatusEffect(StatusEffects.DefenseCanopy))
            {
                if (player.gender > 0 && (!player.hasStatusEffect(StatusEffects.JojoNightWatch) || !player.hasStatusEffect(StatusEffects.PureCampJojo)) && (flags[kFLAGS.HEL_GUARDING] == 0 || !SceneLib.helFollower.followerHel()) && !player.hasStatusEffect(StatusEffects.HeliaOff) && flags[kFLAGS.ANEMONE_WATCH] == 0 && (flags[kFLAGS.HOLLI_DEFENSE_ON] == 0 || flags[kFLAGS.FUCK_FLOWER_KILLED] > 0) && (flags[kFLAGS.KIHA_CAMP_WATCH] == 0 || !SceneLib.kihaFollower.followerKiha()) && EtnaDaughterScene.EtnaDaughterGuardingCamp != 2 && SceneLib.midokaScene.MidokaGuardingCamp != 2 &&
                    (!player.hasStatusEffect(StatusEffects.Familiar) && player.statusEffectv1(StatusEffects.Familiar) == 0) && !(SceneLib.camp.sleepInCabin() && (player.inte / 5 >= Utils.rand(15) || player.lust < 0.8 * player.maxLust() || CoC.instance.gameSettings.sceneHunter_inst.other)) && flags[kFLAGS.IN_INGNAM] == null || flags[kFLAGS.CAMP_UPGRADES_MAGIC_WARD] == 2)
                {
                ...
                else if (SceneLib.camp.sleepInCabin() && player.inte / 5 >= Utils.rand(15) && player.lust < 0.8 * player.maxLust())
                
                //lust condition: horny - less smart!{
    ...
    public static function cheatTime(time : Float, needNext : Bool = false) : Void
    //Advance minutes
    {
        
        var minutesToPass : Float = (time - Math.floor(time)) * 60;
        ...
        if (CoC.instance.model.time.minutes > 59)
        {
            CoC.instance.timeQ++;
            CoC.instance.model.time.minutes -= 60;
            if (!EngineCore.buttonIsVisible(0))
            {
                goNext(needNext);
            }
        }
     ...
     * @param time
     */
    public static function advanceMinutes(time : Float) : Void
    // Ex. minutes = 45, time = 20, overflow = 6
    {
        
        var overflow : Float = (CoC.instance.model.time.minutes + time) - 59;
        if (overflow > 0)
        {
        ...
        if (CoC.instance.player.hasPerk(PerkLib.JobSoulCultivator))
        {
            SceneLib.combat.soulforceregeneration(time);
        }
        if (CoC.instance.player.hasPerk(PerkLib.JobSorcerer) || CoC.instance.player.hasPerk(PerkLib.JobElementalConjurer))
        {
            SceneLib.combat.manaregeneration(time);
        }
    ...
    public static function growHair(amount : Float = .1) : Bool
    //Grow hair!
    {
        
        var player : Player = CoC.instance.player;
        var currentLength : Float = player.hairLength;
    ...
    public static function growBeard(amount : Float = .1) : Bool
    //Grow beard!
    {
        
        var player : Player = CoC.instance.player;
        var tempBeard : Float = player.beardLength;
    ...
    {
        var i : Int = Lambda.indexOf(_timeAwareClassList, toRemove);
        if (i >= 0)
        {
    ...
    private static function set_doCamp(value : Function) : Function
    {
        if (_campSet)
        {
            throw new Error("Multiple Docamp Inits");
        }
        _doCamp = value;
        _campSet = true;
        return value;
