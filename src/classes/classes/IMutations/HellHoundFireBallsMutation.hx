...
class HellHoundFireBallsMutation extends IMutationPerkType
{
    ...
    override public function pReqs(pCheck : Int = -1) : Void
    {
        try
        {
            ...
            if (pTier == 0)
            {
                this.requireBallsMutationSlot().requirePerk(PerkLib.Hellfire).requireCustomFunction(function(player : Player) : Bool
                        {
            ...
            else
            {
                ...
                this.requireLevel(pLvl).requireCustomFunction(function(player : Player) : Bool
                        {
    ...
    {
        var pBuffs : Dynamic = { };
        Reflect.setField(pBuffs, "lib.mult", 0.05 * currentTier(this, player));
        return pBuffs;
    }
