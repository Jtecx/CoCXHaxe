...
class HarpyHollowBonesMutation extends IMutationPerkType
{
    ...
    override public function pReqs(pCheck : Int = -1) : Void
    {
        try
        {
            ...
            if (pTier == 0)
            {
                this.requireBonesAndMarrowMutationSlot().requireAnyRace(Races.HARPY, Races.SIREN, Races.THUNDERBIRD, Races.PHOENIX, Races.COUATL, Races.AVIAN, Races.GRYPHON, Races.PEACOCK);
            }
            else
            {
    ...
    override public function buffsForTier(pTier : Int, target : Creature) : Dynamic
    {
        ...
        if (pTier == 3)
        {
            Reflect.setField(pBuffs, "spe.mult", 0.9);
