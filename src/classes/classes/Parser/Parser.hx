...
class Parser
{
    public static var mainParserDebug : Bool = false;
    public static var lookupParserDebug : Bool = false;
    public static var conditionalDebug : Bool = false;
    public static var printCcntentDebug : Bool = false;
    public static var printConditionalEvalDebug : Bool = false;
    public static var printIntermediateParseStateDebug : Bool = false;
    public static var logErrors : Bool = true;
    ...
    private static function convertSingleArg(arg : String) : String
    {
        ...
        var capitalize : Bool = isUpperCase(arg.charAt(0));
        
        var argLower : String;
        argLower = arg.toLowerCase();
        if (Lambda.has(ParserTags.singleArgConverters, argLower))
        ...
        else
        {
            ...
            var descriptorArray : Array<Dynamic> = arg.split(".");
            ...
            // Completely bad tag{
                ...
                {
                    if (lookupParserDebug || logErrors)
                    {
            ...
            if (obj.exists("getDescription") && arg.indexOf(".") > 0)
            {
    ...
    private static function convertDoubleArg(inputArg : String) : String
    {
        ...
        var argTemp : Array<Dynamic> = inputArg.split(" ");
        if (argTemp.length != 2)
        {
            if (logErrors)
            {
        ...
        var aspect : Dynamic = argTemp[1];
        var subjectLower : String = argTemp[0].toLowerCase();
        var aspectLower : Dynamic = argTemp[1].toLowerCase();
        ...
        var capitalize : Bool = isUpperCase(subject.charAt(0)) || isUpperCase(aspect.charAt(0));
        
        
        // Only perform lookup in twoWordNumericTagsLookup if aspect can be cast to a valid number
        
        if ((Lambda.has(ParserTags.twoWordNumericTagsLookup, subjectLower)) && !Math.isNaN(as3hx.Compat.parseFloat(aspect)))
        {
            aspectLower = as3hx.Compat.parseFloat(aspectLower);
        ...
        if (Lambda.has(ParserTags.twoWordTagsLookup, subjectLower))
        {
            if (Lambda.has(ParserTags.twoWordTagsLookup[subjectLower], aspectLower))
            {
                if (lookupParserDebug)
                {
                    trace("WARNING: Found corresponding anonymous function");
        ...
        var descriptorArray : Array<Dynamic> = subject.split(".");
        ...
        if (thing.exists("getDescription") && subject.indexOf(".") > 0)
        {
            if (argTemp.length > 1)
            {
        ...
        var aspectLookup : Dynamic = getObjectFromString(CoC.instance, aspect);
        
        if (thing != null)
        {
            if (Std.is(thing, Function))
            {
                if (lookupParserDebug)
                {
                    trace("WARNING: Found corresponding function in owner class");
            ...
            {
                var indice : Float = as3hx.Compat.parseFloat(aspectLower);
                if (Math.isNaN(indice))
                {
                    if (logErrors)
                    {
                ...
                {
                    return Reflect.field(thing, Std.string(indice));
                }
            }
            else if (Std.is(thing, Dynamic))
            {
                if (thing.exists(aspectLookup))
                {
                    return Reflect.field(thing, Std.string(aspectLookup));
                }
                else if (thing.exists(aspect))
                {
                    return Reflect.field(thing, Std.string(aspect));
                }
                else
                {
                    if (logErrors)
     ...
     * @return
     */
    private static function convertConditionalArgumentFromStr(arg : String) : Dynamic
    // convert the string contents of a conditional argument into a meaningful variable.
    {
        
        var argLower : Dynamic = arg.toLowerCase();
        var argResult : Dynamic = -1;
        ...
        if (!Math.isNaN(as3hx.Compat.parseFloat(arg)))
        {
            if (printConditionalEvalDebug)
            {
                trace("WARNING: Converted to float. Number = ", as3hx.Compat.parseFloat(arg));
            }
            return as3hx.Compat.parseFloat(arg);
        }
        if (Lambda.has(ParserTags.conditionalOptions, argLower))
        {
            if (printConditionalEvalDebug)
            {
                trace("WARNING: Found corresponding anonymous function");
        ...
        var obj : Dynamic = getObjectFromString(CoC.instance, arg);
        
        if (printConditionalEvalDebug)
        {
            trace("WARNING: Looked up ", arg, " in ", CoC.instance, "Result was:", obj);
        }
        if (obj != null)
        {
            if (printConditionalEvalDebug)
            ...
            if (Std.is(obj, Function))
            {
                ...
                argResult = as3hx.Compat.parseFloat(obj());
                return argResult;
            }
            else
            {
        ...
        {
            if (printConditionalEvalDebug || logErrors)
            {
     ...
     * @return
     */
    private static function evalConditionalStatementStr(textCond : String) : Bool
    {
        var isExp : as3hx.Compat.Regex = new as3hx.Compat.Regex('([\\w.]+)\\s?(==|=|!=|<|>|<=|>=)\\s?([\\w.]+)', "");
        var expressionResult : Dynamic = isExp.exec(textCond);
        if (expressionResult == null)
        {
            var condArg : Dynamic = convertConditionalArgumentFromStr(textCond);
            if (condArg != null)
            {
                if (printConditionalEvalDebug)
                {
        ...
        var condArgStr1 : String = Reflect.field(expressionResult, Std.string(1));
        var operator : String = Reflect.field(expressionResult, Std.string(2));
        var condArgStr2 : String = Reflect.field(expressionResult, Std.string(3));
        
        var retVal : Bool = false;
        
        var condArg1 : Dynamic = convertConditionalArgumentFromStr(condArgStr1);
        var condArg2 : Dynamic = convertConditionalArgumentFromStr(condArgStr2);
        
        //Perform check
        switch (operator)
        {
            case "=":retVal = (condArg1 == condArg2);
            case ">":retVal = (condArg1 > condArg2);
            case "==":retVal = (condArg1 == condArg2);
            case "<":retVal = (condArg1 < condArg2);
            case ">=":retVal = (condArg1 >= condArg2);
            case "<=":retVal = (condArg1 <= condArg2);
            case "!=":retVal = (condArg1 != condArg2);
            default:retVal = (condArg1 != condArg2);
        }
     ...
     */
    private static function splitConditionalResult(textCtnt : String) : Array<Dynamic>
    // Splits the conditional section of an if-statemnt in to two results:
    {
                ...
                // conditional split is only valid in this context if we're not in a nested bracket.{
                    
                    {
                        ...
                        else
                        {
                            ...
                            sectionStart = as3hx.Compat.parseInt(i + 1);
                            section += 1;
                        }
                    }
                }
    ...
    // NOTE: enclosing brackets are *not* included in the actual textCtnt string passed into this function
    {
        ...
        var ret : Array<Dynamic> = ["", "", ""];  // first string is conditional, second string is the output  
        
        var i : Float = 0;
        var parenthesisCount : Float = 0;
        
        //var ifText;
        var conditional : Dynamic;
        var output : Dynamic;
        
        var condStart : Float = textCtnt.indexOf("(");
        
        if (condStart == -1)
        
        // If we don't have any open parenthesis{
                ...
                // We've found the matching closing bracket for the opening bracket at textCtnt[condStart]{
                    
                    {
                        ...
                        if (conditionalDebug)
                        {
                            ...
                            trace("WARNING: -0--------------------------------------------------");
                            trace("WARNING: Content Item 1 = ", Reflect.field(output, Std.string(0)));
                            trace("WARNING: -1--------------------------------------------------");
                            trace("WARNING: Item 2 = ", Reflect.field(output, Std.string(1)));
                            trace("WARNING: -2--------------------------------------------------");
                        }
                        
                        if (conditional != null)
                        {
                            return recParser(Reflect.field(output, Std.string(0)), depth);
                        }
                        else
                        {
                            return recParser(Reflect.field(output, Std.string(1)), depth);
                        }
                    }
                }
                i += 1;
    ...
    private static function getObjectFromString(localThis : Dynamic, inStr : String) : Dynamic
    {
        if (Lambda.has(localThis, inStr))
        {
            if (lookupParserDebug)
            {
                trace("WARNING: item: ", inStr, " in: ", localThis);
            }
            return Reflect.field(localThis, inStr);
        }
        ...
        // *should* be > -1, but if the string starts with a dot, it can't be a valid reference to a nested class anyways.{
            
            {
                ...
                if (lookupParserDebug)
                {
                    ...
                    trace("WARNING: dereferenced = ", Reflect.field(localThis, localReference));
                }
                ...
                if (Lambda.has(localThis, localReference))
                {
                    if (lookupParserDebug)
                    {
                        trace("WARNING: have localReference:", Reflect.field(localThis, localReference));
                    }
                    return getObjectFromString(Reflect.field(localThis, localReference), itemName);
                }
                else
                {
                    return null;
    ...
    private static function isIfStatement(textCtnt : String) : Bool
    {
    ...
    private static function parseNonIfStatement(textCtnt : String, depth : Int) : String
    {
        ...
        var singleWordTagRegExp : as3hx.Compat.Regex = new as3hx.Compat.Regex('^[\\/\\d\\w-_.]+$', "");
        var doubleWordTagRegExp : as3hx.Compat.Regex = new as3hx.Compat.Regex('^[\\/\\d\\w-_.]+\\s[\\w\\d-_.]+$', "");
     ...
     */
    private static function recParser(textCtnt : String, depth : Float) : String
    {
        if (mainParserDebug)
        {
            trace("WARNING: Recursion call", depth, "---------------------------------------------+++++++++++++++++++++");
        }
        if (printIntermediateParseStateDebug)
        {
        ...
        else
        {
            ...
            for (i in lastBracket...textCtnt.length)
            {
                ...
                // We've found the matching closing bracket for the opening bracket at textCtnt[lastBracket]{
                    
                    {
                        ...
                        }  // incorrectly interpreted as a multi-word tag (and shit would asplode and shit)    // Parsing the trailing string if it doesn't have brackets could lead to it being    // if not, we need to just return the string as-is.    // Only parse the trailing string if it has brackets in it.    // I haven't implemented yet    // then, eval their contents, in case they're an if-statement or other control-flow thing    // First parse into the text in the brackets (to resolve any nested brackets)  
                        ...
                        if (postfixTmp.indexOf("[") != -1)
                        {
                            ...
                            retStr += recParser(postfixTmp, depth);
                        }
                        else
                        {
                            if (printCcntentDebug)
     ...
     */
    public static function recursiveParser(contents : String, prettyQuotes : Bool = true) : String
    {
        if (mainParserDebug)
        {
        ...
        contents = new as3hx.Compat.Regex('\\\\n', "g").replace(contents, "\n");
        ...
        ret = new as3hx.Compat.Regex('\\\\]', "g").replace(ret, "]");
        ret = new as3hx.Compat.Regex('\\\\\\[', "g").replace(ret, "[");
        
        // And repeated spaces
        ret = new as3hx.Compat.Regex(' {2}', "g").replace(ret, " ");
    ...
    {
        inStr =new as3hx.Compat.Regex('(\\w)'(\\w)', "g").replace(inStr, "$1\u2019$2").replace(new as3hx.Compat.Regex('(^|[\\r\\n 	.!,?])"([a-zA-Z<>.!,?])', "g"), "$1\u201c$2").replace(new as3hx.Compat.Regex('([a-zA-Z<>.!,?])"([\\r\\n 	.!,?]|$)', "g"), "$1\u201d$2").replace(new as3hx.Compat.Regex('--', "g"), "\u2014")  // Closing doubles    // Opening doubles    // Apostrophes  ;  // em-dashes  
        return inStr;
    }
    ...
    // ---------------------------------------------------------------------------------------------------------------------------------------
    // ---------------------------------------------------------------------------------------------------------------------------------------
    public static function isUpperCase(char : String) : Bool
    {
        if (!Math.isNaN(as3hx.Compat.parseFloat(char)))
        {
            return false;
        }
