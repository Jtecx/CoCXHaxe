...
class SceneHunter extends BaseContent
{
    public var uniHerms(get, never) : Bool;
    public var dickSelect(get, never) : Bool;
    public var lossSelect(get, never) : Bool;
    public var printChecks(get, never) : Bool;
    public var polygamy(get, never) : Bool;
    public var shortPreg(get, never) : Bool;
    public var other(get, never) : Bool;
    public var mockFights(get, never) : Bool;
    ...
    public function settingsPage() : Void
    {
        ...
        outputText("\n\n<b>Universal Herms:</b> ");
        if (flags[kFLAGS.SCENEHUNTER_UNI_HERMS] != null)
        {
        ...
        outputText("\n\n<b>Dick Select:</b> ");
        if (flags[kFLAGS.SCENEHUNTER_DICK_SELECT] != null)
        {
            outputText("<b>[font-green]ENABLED[/font]</b>");
        ...
        outputText("\n\n<b>Loss Select:</b> ");
        if (flags[kFLAGS.SCENEHUNTER_LOSS_SELECT] != null)
        {
            outputText("<b>[font-green]ENABLED[/font]</b>");
        ...
        outputText("\n\n<b>Mock Fights:</b> ");
        if (flags[kFLAGS.SCENEHUNTER_MOCK_FIGHTS] != null)
        {
            outputText("<b>[font-green]ENABLED[/font]</b>");
        ...
        outputText("\n\n<b>Short Preg:</b> ");
        if (flags[kFLAGS.SCENEHUNTER_SHORT_PREG] != null)
        {
            outputText("<b>[font-green]ENABLED[/font]</b>");
        ...
        outputText("\n\n<b>Other changes:</b> ");
        if (flags[kFLAGS.SCENEHUNTER_OTHER] != null)
        {
            outputText("<b>[font-green]ENABLED[/font]</b>");
        }
        else
        ...
        addButton(9, "Polygamy", togglePolygamy).disableIf(!player, "Requires a loaded save. Set to 0 at the start of the game.");
        outputText("\n\n<b>Polygamy:</b> ");
        if (polygamy)
        {
            outputText("<b>[font-green]ENABLED[/font]</b>");
    ...
    private function get_uniHerms() : Bool
    {
     ...
     */
    public function selectGender(dickF : Function, vagF : Function, assA : Dynamic = null, hermF : Function = null,
            dickPriority : Int = 1, dickActive : Bool = true, dickDisabledMsg : String = "") : Void
    //decomposing ass
    {
        
        var assText : String = ((Std.is(assA, Array))) ? Reflect.field(assA, Std.string(0)) : "Ass";
        var assF : Function = ((Std.is(assA, Function))) ? try cast(assA, Function) catch(e:Dynamic) null : 
        ((Std.is(assA, Array))) ? Reflect.field(assA, Std.string(1)) : 
        null;
        //booleans
        var dickB : Bool = dickF != null && player.hasCock();
        var vagB : Bool = vagF != null && player.hasVagina();
        var assB : Bool = assF != null;
        var hermB : Bool = hermF != null && player.hasCock() && player.hasVagina();
        //Auto-calls. No auto call when dick is just inactive — player should know!
        if (!(dickB && dickActive) && !vagB && !assB && !(hermB && dickActive))
        ...
        if (hermB && (!dickB && !vagB && !assB || !uniHerms) && dickActive)
        ...
        if (dickB && (!hermB && !vagB && !assB || !uniHerms && (!player.hasVagina() || dickPriority == 1 || dickPriority == 0 && rand(2) == 0)) && dickActive)
        
        //!uniherms - if priority{
        ...
        if (hermF != null)
        {
            if (player.hasCock() && player.hasVagina())
            {
                ...
                //checking the size here too... though not sure if it should work this way{
    ...
    private function get_dickSelect() : Bool
    {
     ...
     */
    public function selectFitNofit(fitF : Function, nofitF : Function, maxSize : Float, compareBy : String = "area") : Void
    //Auto-calls
    {
    ...
    public function callFitNofit(fun : Function, max : Float, compareBy : String = "area") : Void
     ...
     */
    public function selectBigSmall(bigF : Function, bigMin : Float, mediumF : Function, smallMax : Float = -1, smallF : Function = null, compareBy : String = "area", totalMax : Float = -1) : Void
    {
        var smallProvided : Bool = smallMax >= 0 && smallF != null;
        //Auto-calls
        if (!dickSelect)
        {
            ...
            else if (player.findCock(1, (smallProvided) ? smallMax : -1, bigMin, compareBy) >= 0)
            {
                ...
                if (smallProvided)
                {
                    print("Passed? Another size check, but alt scene available for dicks smaller than " + smallMax + " " + compareBy);
                }
                mediumF();
            }
            else
        ...
        //medium cocks{
            ...
            if (player.findCock(1, smallMax, bigMin, compareBy) >= 0)
            ...
            //small cocks
            if (player.findCock(1, -1, smallMax, compareBy) >= 0)
        ...
        else
        {
            ...
            if (player.findCock(1, -1, bigMin, compareBy) >= 0)
    ...
    //Calls the 'fun' function, finding the biggest cock index in selected limits
    public function callBigSmall(fun : Function, bigMin : Float, smallMax : Float = -1, compareBy : String = "area", totalMax : Float = -1) : Void
    //Invalid calls may be created, but must NEVER be called.
    {
     ...
     */
    public function selectSingleMulti(singleF : Function, multiF : Function, compareBy : String = "area", maxSize : Float = -1, maxSize2 : Float = -2) : Void
    {
        if (maxSize2 == -2)
        {
            maxSize2 = maxSize;
        ...
        var multiEn : Bool = player.findCockNotIn([first], 1, -1, maxSize2, compareBy) >= 0;  //try to find the second one  
        //Auto-calls
        if (!dickSelect)
        {
            if (!multiEn)
        ...
        addButton(1, "Multiple", restoreText, beforeText, multiF).disableIf(!multiEn, "Not enough dicks." + ((maxSize2 >= 0) ? "Requires another cock fitting " + maxSize2 + " " + compareBy : ""));
        _passCheck = false;
    }
    ...
    private function get_lossSelect() : Bool
    {
     ...
     */
    public function selectLossMenu(options : Array<Dynamic>, msg : String = "") : Void
    {
        var choices : Array<Dynamic> = [];  //enabled functions  
        //init when enabled
        if (lossSelect)
        {
            outputText(msg);
        ...
        {
            var arr : Array<Dynamic> = try cast(options[i], Array</*AS3HX WARNING no type*/>) catch(e:Dynamic) null;
            if (arr == null)
            {
                throw new Error("selectLossMenu called with non-array arguments!");
            }
            //button
            if (arr.length == 3)
            {
                if (lossSelect)
                {
                    if (arr[2] == null)
                    {
                        addButtonDisabled(arr[0], arr[1], "");
                    }
                    else
                    {
                        addButton.apply(this, arr);
                    }
                }
                else if (arr[2] != null)
                {
            ...
            else if (arr.length >= 5 && arr.length <= 6)
            {
                ...
                {
                    print("Loss scene (random) check failed: " + arr[3]);
                }
            }
            else
            {
                throw new Error("selectLossMenu - argument length mismatch!");
            }
        }
    ...
    private function get_printChecks() : Bool
    {
    ...
    //flag bits
    public static var POLYGAMY_ENABLED : Int = 1 << 0;  //enabled flag  
    public static var POLYGAMY_CHICHI : Int = 1 << 1;  //saves who you were married to  
    public static var POLYGAMY_ETNA : Int = 1 << 2;
    public static var POLYGAMY_ZENJI : Int = 1 << 2;
    public static var POLYGAMY_ALVINA : Int = 1 << 3;
    public static var POLYGAMY_KIHA : Int = 1 << 4;
    public static var polyBits : Dynamic = {
    ...
    private function get_polygamy() : Bool
    {
        return cast(flags[kFLAGS.SCENEHUNTER_POLYGAMY] & POLYGAMY_ENABLED, Bool);
    }
    
    //Checks if the player can marry someone.
    public function canMarry() : Bool
    {
        return cast(!flags[kFLAGS.MARRIAGE_FLAG] || polygamy, Bool);
    }
    ...
    public function marry(name : String) : Void
    {
        if (!polygamy)
        {
            flags[kFLAGS.MARRIAGE_FLAG] = name;
        }
        if (!polyBits.exists(name))
        {
            CoC_Settings.error("Married " + name + " without registering them in SceneHunter.");
        }
        else
        {
            flags[kFLAGS.SCENEHUNTER_POLYGAMY] = flags[kFLAGS.SCENEHUNTER_POLYGAMY] | Reflect.field(polyBits, name);
        }
    }
    
    public function married(name : String) : Bool
    {
        if (!polygamy)
        {
            return flags[kFLAGS.MARRIAGE_FLAG] == name;
        }
        else if (polyBits.exists(name))
        {
            return cast(flags[kFLAGS.SCENEHUNTER_POLYGAMY] & Reflect.field(polyBits, name), Bool);
        }
        else
        {
    ...
    {
        var name : String;
        if (flags[kFLAGS.SCENEHUNTER_POLYGAMY] != null)
        {
            var pcnt : Int = 0;
            ...
            menu();
            for (name in Reflect.fields(polyBits))
            {
                if (cast(flags[kFLAGS.SCENEHUNTER_POLYGAMY] & Reflect.field(polyBits, name), Bool))
                {
            ...
            if (pcnt == 1)
            {
                disablePoly(button(0).labelText);
            }
            //the button should contain spouse's name
            else if (pcnt == 0)
            {
                disablePoly("");
            }
        ...
        else
        {
            ...
            if ((flags[kFLAGS.SCENEHUNTER_POLYGAMY] & POLYGAMY_CHICHI) != 0 && flags[kFLAGS.MARRIAGE_FLAG] != "Chi Chi")
            {
                flags[kFLAGS.CHI_CHI_FOLLOWER] = 6;
            }
            if ((flags[kFLAGS.SCENEHUNTER_POLYGAMY] & POLYGAMY_ETNA) != 0 && flags[kFLAGS.MARRIAGE_FLAG] != "Etna")
            {
                flags[kFLAGS.ETNA_FOLLOWER] = 4;
            }
            if ((flags[kFLAGS.SCENEHUNTER_POLYGAMY] & POLYGAMY_ZENJI) != 0 && flags[kFLAGS.MARRIAGE_FLAG] != "Zenji")
            {
                flags[kFLAGS.ZENJI_PROGRESS] = 12;
            }
            if ((flags[kFLAGS.SCENEHUNTER_POLYGAMY] & POLYGAMY_ALVINA) != 0 && flags[kFLAGS.MARRIAGE_FLAG] != "Alvina")
            {
                flags[kFLAGS.ALVINA_FOLLOWER] = 21;
            }
            if ((flags[kFLAGS.SCENEHUNTER_POLYGAMY] & POLYGAMY_KIHA) != 0 && flags[kFLAGS.MARRIAGE_FLAG] != "Kiha")
            {
                KihaFollower.ProposalStatus = 6;
            }
            //Enable Polygamy
            flags[kFLAGS.MARRIAGE_FLAG] = "POLYGAMY";  //just to break anything that doesn't support it  
            flags[kFLAGS.SCENEHUNTER_POLYGAMY] = flags[kFLAGS.SCENEHUNTER_POLYGAMY] ^ POLYGAMY_ENABLED;
            settingsPage();
        }
        
        var disablePoly : String->Void = function(singleName : String) : Void
        {
            if (singleName != null)
            {
                var singleBit : Int = Reflect.field(polyBits, singleName);
                //revert some flags to pre-marriage state
                if ((flags[kFLAGS.SCENEHUNTER_POLYGAMY] & POLYGAMY_CHICHI) != 0 && singleBit != POLYGAMY_CHICHI)
                {
                    flags[kFLAGS.CHI_CHI_FOLLOWER] = 3;
                }
                if ((flags[kFLAGS.SCENEHUNTER_POLYGAMY] & POLYGAMY_ETNA) != 0 && singleBit != POLYGAMY_ETNA)
                {
                    flags[kFLAGS.ETNA_FOLLOWER] = 2;
                }
                if ((flags[kFLAGS.SCENEHUNTER_POLYGAMY] & POLYGAMY_ZENJI) != 0 && singleBit != POLYGAMY_ZENJI)
                {
                    flags[kFLAGS.ZENJI_PROGRESS] = 11;
                }
                if ((flags[kFLAGS.SCENEHUNTER_POLYGAMY] & POLYGAMY_ALVINA) != 0 && singleBit != POLYGAMY_ALVINA)
                {
                ...
                if ((flags[kFLAGS.SCENEHUNTER_POLYGAMY] & POLYGAMY_KIHA) != 0 && singleBit != POLYGAMY_ALVINA)
                {
    ...
    private function get_shortPreg() : Bool
    {
        return flags[kFLAGS.SCENEHUNTER_SHORT_PREG];
    }
    
    // returns shortened value if the flag is enabled, unchanged otherwise
    public function shortPregTimer(oldTimer : Float) : Int
    {
        if (!shortPreg || oldTimer < 2)
        {
            return as3hx.Compat.parseInt(oldTimer);
        }
        var num : Float;
        if (oldTimer <= 96)
        {
        ...
        else if (oldTimer <= 240)
        {
            num = 12 + (oldTimer - 96) / 24;
        }
        else if (oldTimer <= 480)
        {
            num = 18 + (oldTimer - 240) / 40;
        }
        else
        {
            num = 24 + (oldTimer - 480) / 70;
        }
        if (num < 1)
        {
            num = 1;
    ...
    public function adjustPregEventTimer(oldTimer : Float, pregType : Int) : Int
    {
        return adjustPregEventTimerNum(oldTimer, PregnancyStore.getIncubation(pregType));
    }
    
    public function adjustPregEventTimerNum(oldTimer : Float, oldIncub : Int) : Int
    {
        if (!shortPreg || oldTimer < 2)
        {
            return as3hx.Compat.parseInt(oldTimer);
        ...
        if (oldIncub < 0)
        {
            return shortPregTimer(oldTimer);
        }  // can't find => at least fix it to the new scale  
        var newTimer : Float = Math.round(oldTimer / oldIncub * shortPregTimer(oldIncub));
        if (newTimer <= 0)
        {
            return 1;
        }
    ...
    private function get_other() : Bool
    {
    ...
    private var _passCheck : Bool = false;
     ...
     */
    public function selectPureCor(pureButton : Array<Dynamic>, corButton : Array<Dynamic>, maxForPure : Int, minForCor : Int = -1, tolerance : Bool = true) : Void
    ...
    */
    public var _recalling : Bool = false;  //set to true when a scene is recalled.  
    public var _mocking : Bool = false;  //set to true when a scene is recalled.  
    
    private function get_mockFights() : Bool
    {
    ...
    public function recallScenes() : Void
    {
        ...
        {
            addButton(4, "Nightmare", SceneLib.forest.nightmareScene.nightmareVictory).hint("Demonic bicorn fucks you into a pile of mess."
                    + (!(CelessScene.instance.questFinishedNightmare) ? "\n\n<b>Brought here by SceneHunter:Other.</b>" : ""));
        }
        //Venus cock scenes
        if (flags[kFLAGS.FACTORY_SHUTDOWN] == 2 && flags[kFLAGS.KAIJU_COCK] == 1)
        {
            addButton(5, "VenusCock", SceneLib.boat.kaiju.kaijuGrowsWangus).hint("Venus discovers her new cock.");
        }
        if (flags[kFLAGS.TIMES_EGGED_IN_EXGARTUAN] > 0 && player.hasCock())
        {
            addButton(6, "EggExgartuan", SceneLib.masturbation.eggExgartuan).hint("Your small egg-fight with your dick-demon.");
        }
        // Turkey holiday encounter
        if (flags[kFLAGS.TURKEY_FUCK_YEAR_DONE] > 0)
        {
            addButton(7, "Turkey-1", SceneLib.holidays.datTurkeyRumpMeeting).hint("The weird turkey encounter");
        }
        ...
        {
            addButton(13, "UrtaQuest", recallScenes_quest);
        }
    ...
    private function recallScenes_places() : Void
    {
        ...
        if (flags[kFLAGS.OWCA_SACRIFICE_DISABLED] != null)
        {
            addButton(6, "OwcaDemons", SceneLib.owca.loseOrSubmitToVapula).hint("Why not submit to lusty demons again, huh?");
        }
        if (flags[kFLAGS.OWCA_UNLOCKED] == -1 || sceneHunter.other && flags[kFLAGS.OWCA_SACRIFICE_DISABLED] != null)
        {
    ...
    {
        menu();
        if (player.hasStatusEffect(StatusEffects.KeltOff) || sceneHunter.other && flags[kFLAGS.KELT_BREAK_LEVEL] >= 4)
        
        //allowing for Kelly in 'Other' because I want!{
        ...
        {
            addButton(6, "MarbleMeet", SceneLib.marbleScene.encounterMarbleInitially).hint("First meeting. Should it go nice... or not?");
        }
        if (player.statusEffectv1(StatusEffects.FuckedMarble) > 0)
        {
            addButton(7, "MarblSexFarm", SceneLib.marbleScene.standardSex).hint("Marble invites you to her bed.");
        }
        if (player.statusEffectv2(StatusEffects.FuckedMarble) > 0)
        {
            addButton(8, "MilkySex", SceneLib.marbleScene.marbleMilkSex).hint("Something hot after milking. Why not?");
        }
        if (player.statusEffectv2(StatusEffects.FuckedMarble) > 0)
        {
            addButton(9, "MilkySex", SceneLib.marbleScene.marbleMilkSex).hint("Something hot after milking. Why not?");
        }
    ...
    private function recallScenes_telAdre() : Void
    {
        ...
        if (flags[kFLAGS.BROOKE_MEDIUM_SCENE] != null && (player.hasVagina() || player.cockThatFits(SceneLib.telAdre.brooke.brookeCapacity()) >= 0))
        {
        ...
        {
            addButton(3, "CottonFirst", SceneLib.telAdre.cotton.cottonShowerFunTimes).hint("First shower with Cotton.");
        }
        //4
        if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 0)
        {
            addButton(5, "UrtaFirst", SceneLib.urta.urtaFirstEncounter).hint("Your first awkward encounter with Urta.");
        }
        if (flags[kFLAGS.URTA_PC_LOVE_COUNTER] == 1)
        {
            addButton(6, "UrtaFriendSex", SceneLib.urta.goBackToUrtasForLuvinz, true).hint("Friendly sex with Urta before confession.");
        }
        if (flags[kFLAGS.URTA_X_RAPHAEL_HAPPENED] != null)
        {
            addButton(7, "Urta x Raph", SceneLib.urta.urtaAndRaphaelSurprise).hint("Drunk Urta and Raphael surprise.");
        }
        if (SceneLib.telAdre.sexMachine.totalUses() > 0)
        {
            addButton(8, "Sex Machine", SceneLib.telAdre.sexMachine.exploreShowers).hint("Your first time with the 'unique' gym machine");
        }
        addButton(14, "Back", recallScenes_places);
    }
    ...
    private function recallScenes_NPCs() : Void
    {
        ...
        //Phylla demon fuck
        if (flags[kFLAGS.ANT_COLONY_KEPT_HIDDEN] != null || flags[kFLAGS.PHYLLA_SAVED] != null && sceneHunter.other)
        {
            ...
            {
                addButtonDisabled(2, "PhyllaCart", "You must be corrupted and not genderless to recall this.");
            }
        }
        //Phylla keks
        if (flags[kFLAGS.ANT_WAIFU] != null || flags[kFLAGS.PHYLLA_STAY_HOME] != null)
        {
    ...
    private function recallScenes_NPCs_2() : Void
    {
        ...
        if (flags[kFLAGS.KIHA_AND_HEL_WHOOPIE] != null && player.hasCock())
        {
        ...
        {
            addButton(9, "PatchRape", SceneLib.patchouliScene.patchouliRapeHim).hint("Take your revenge on the deceiving cat.");
        }
        if (flags[kFLAGS.RAPHAEL_MET] != null)
        {
            addButton(10, "Raph-Meeting", SceneLib.raphael.meetRaphael).hint("Your first encounter.");
        }
        if (player.hasPerk(PerkLib.RapierTraining) && player.hasVagina())
        {
            addButton(11, "Raph-Fencing", SceneLib.raphael.RaphaelPicnicSkill).hint("The last test of your skill.");
        }
        if (player.hasPerk(PerkLib.Misdirection) && player.hasVagina())
        {
            addButton(12, "Raph-Thievin", SceneLib.raphael.RaphaelPicnicChooseThieving).hint("Your last discussion about thievery.");
        }
        if (flags[kFLAGS.SHEILA_XP] >= 4 || sceneHunter.other && flags[kFLAGS.SHEILA_DEMON] != null && flags[kFLAGS.SHEILA_CITE] == 1)
        {
            addButton(13, "SheilaEvening", SceneLib.sheilaScene.sheilaXPThreeSexyTime).hint("The unusual evening encounter with Sheila.");
        }
        addButton(14, "Back", recallScenes);
    }
    ...
    private function recallScenes_NPCs_3() : Void
    {
        ...
        {
            addButton(0, "ExgartuDrama", SceneLib.shouldraFollower.exgartuMonAndShouldraShowdown).hint("The finish of Shouldra's quarrel with Exgartuan, your dick-demon.");
        }
        ...
        {
            addButton(6, "ZenjiMarry", SceneLib.zenjiScene.ZenjiMarriageSceneCinco).hint("Zenji marriage sex.");
        }
        if (SceneLib.kihaFollower.isMarried())
        {
            addButton(7, "KihaMarry", SceneLib.kihaFollower.KihaWeddingVictory).hint("Kiha marriage sex.");
            addButton(8, "KihaMLose", SceneLib.kihaFollower.KihaWeddingDefeat).hint("Kiha marriage fight - defeat!");
        }
        addButton(14, "Back", recallScenes);
    }
    ...
    {
        menu();
        if (flags[kFLAGS.SANDWITCH_MOB_DEFEATED] != null)
        {
        ...
        if (flags[kFLAGS.FACTORY_FOUND] != null)
        {
            addButton(1, "Factory", recallScenes_factory);
        }
        if (flags[kFLAGS.DISCOVERED_DUNGEON_2_ZETAZ] > 0)
        {
            addButton(2, "Deep Cave", recallScenes_deepCave);
        }
        if (flags[kFLAGS.D3_DISCOVERED] > 0)
        {
            addButton(3, "Stronghold", recallScenes_d3);
        }
        if (flags[kFLAGS.D3_DISCOVERED] > 0)
        {
            addButton(3, "Stronghold", recallScenes_d3);
        }
        if (flags[kFLAGS.HEL_HARPY_QUEEN_DEFEATED] != null)
        {
            addButton(4, "PhoenixTower", recallScenes_phoenixTower);
        }
        if (DemonLab.MainAreaComplete >= 3)
        {
            addButton(5, "Lab Succubus", SceneLib.dungeons.demonLab.succubusDecision);
        }
        addButton(14, "Back", recallScenes);
    }
    ...
    {
        menu();
        if (flags[kFLAGS.FACTORY_SUCCUBUS_DEFEATED] != null)
        {
        ...
        if (flags[kFLAGS.FACTORY_INCUBUS_DEFEATED] != null)
        {
    ...
    private function recallScenes_deepCave() : Void
    {
        menu();
        //Doppel
        if (flags[kFLAGS.ZETAZ_IMP_HORDE_DEFEATED] != null)
        {
        ...
        if (flags[kFLAGS.DEFEATED_ZETAZ] != null)
        {
        ...
        if (flags[kFLAGS.INVESTIGATED_VALA_AFTER_ZETAZ_DEFEATED] != null)
        {
    ...
    private var urtaSwapped : Bool = false;
    ...
    //Run Urta quest init sequence to swap her with PC
    {
        ...
        var campEnd : Void->Void = function() : Void
        {
