...
class Soulforce extends BaseContent
{
    ...
    public function accessSoulforceMenu() : Void
    {
        ...
        {
            if (player.hasPerk(PerkLib.HclassHeavenTribulationSurvivor) && player.perkv1(PerkLib.Dantain) == 0)
            {
                player.addPerkValue(PerkLib.Dantain, 1, 1);
            }
            if (player.hasPerk(PerkLib.GclassHeavenTribulationSurvivor) && player.perkv1(PerkLib.Dantain) == 1)
            {
                player.addPerkValue(PerkLib.Dantain, 1, 1);
            }
            if (player.hasPerk(PerkLib.FclassHeavenTribulationSurvivor) && player.perkv1(PerkLib.Dantain) == 2)
            {
                player.addPerkValue(PerkLib.Dantain, 1, 1);
            }
            outputText("<b>Dantain:</b> ");
            if (player.perkv1(PerkLib.Dantain) == 3)
            {
                outputText("Nascent Soul");
            }
            else if (player.perkv1(PerkLib.Dantain) == 2)
            {
                outputText("Core Formation");
            }
            else if (player.perkv1(PerkLib.Dantain) == 1)
            {
                outputText("Foundation Establishment");
            }
            else
            {
                outputText("Qi Condensation");
            }
            outputText("\n");
        }
		...
		*/outputText("<b>Uses of soulforce per day (for 4 first option beside cultivate):</b> " + flags[kFLAGS.DAILY_SOULFORCE_USE_LIMIT] + " / " + dailySoulforceUsesLimit + "\n");
        ...
        {
            addButtonDisabled(0, "Meditations", "You're unable to recover soulforce by meditating.");
        }
        else
        {
            addButton(0, "Meditations", SoulforceRegeneration).hint("Spend some time on restoring some of the used soulforce.");
        }
        if (player.hasPerk(PerkLib.HclassHeavenTribulationSurvivor))
        {
            addButton(1, "Contemplate", DaoContemplations).hint("Dao Contemplations");
        }
        else
        {
            addButtonDisabled(1, "???", "Req.  successfully surviving 1st Tribulation.");
        }
        //button 2 - ?
        if (flags[kFLAGS.DAILY_SOULFORCE_USE_LIMIT] < dailySoulforceUsesLimit)
        {
            ...
            addButton(9, "Mana", ManaAndSoulforce).hint("Convert some soulforce into mana or vice versa.");
        }
        else
        {
        ...
        {
            addButton(6, "Imprinting", ImprintingSF).hint("Imprint your SF to combine or seperate sets of flying swords.");
        }
        else
        {
            addButtonDisabled(6, "???", "Req. Flying Sword Path perk.");
        }
        if (player.hasPerk(PerkLib.SoulSense))
        {
            addButton(7, "Soul Sense", SoulSense).hint("Use your soul sense to trigger specific encounters.");
        }
        //używanie divine sense aby znaleść określone event encounters: Tamani (lvl 6+), Tamani daugthers (lvl 6+), Kitsune mansion (lvl 12+), Izumi (lvl 18/24+), itp.
        else
        {
            addButtonDisabled(7, "???", "Req. Soul Sense perk.");
        }
        //button 8 - ?
        ...
        {
            addButtonDisabled(10, "???", "Req. Metamorph.");
        }
        //button 11 - ?
        if (player.hasKeyItem("Cultivation Manual: My Dao Sticks are better than Yours") >= 0 || player.hasKeyItem("Cultivation Manual: Body like a Coke Fiend") >= 0 || player.hasKeyItem("Cultivation Manual: Heart-shaped Eyed She-Devil") >= 0)
        {
            addButton(12, "Sub-paths", SubPaths).hint("Contemplate mysteries on your choosen sub-path(s).");
        }
    ...
    public function SoulCultivationLvL() : Void
    {
        ...
        if (flags[kFLAGS.SOUL_CULTIVATION] >= 1)
        {
            ...
            "Exalt", "Overlord", "Tyrant", "King", "Emperor", "Ancestor", "Sage"
        ][player.perkv2(PerkLib.JobSoulCultivator)];
        }
        else
        {
            cultStanding = "Mortal";
    ...
    {
        var hoursOpt : Array<Dynamic> = [1, 2, 4, 6, 8, 10, 12, 14, 16];
        menu();
        var btn : Int = 0;
        for (hours in hoursOpt)
        {
            var timeS : String = (hours == 1) ? "1 hour" : "" + hours + " hours";
            addButton(btn++, timeS, meditate, hours).hint("Meditate for " + timeS + " (Allow to recover " + meditationPredict(hours) + " soulforce).");
        }
        addButton(14, "Back", accessSoulforceMenu);
    }
    private function sfTrainingItems() : Int
    {
        var itemConds : Array<Dynamic> = [
    ...
    //Types: 0 - primary, 1 - secondary, 2 - tribulation
    {
        ...
        {
            var stage : Array<Dynamic> = stages[flags[kFLAGS.SOUL_CULTIVATION]];
            var level : Int = as3hx.Compat.parseInt(flags[kFLAGS.SOUL_CULTIVATION] * 3);
            ...
            if (stage[0] != 1)
            {
                outputText("Wisdom: " + stage[3]);
            }
            //Primary
            if (stage[0] == 0)
            {
                addButton(0, stage[2], contPrimary, stage[1], stage[4]).disableIf(player.soulforce < player.maxSoulforce(), "Not enough soulforce, required: full");
            }
            //Secondary
            else if (stage[0] == 1)
            {
                addButton(0, stage[2], contSecondary, stage[1]).disableIf(player.soulforce < Math.round(player.maxSoulforce() * 0.3), "Not enough soulforce, required: " + Math.round(player.maxSoulforce() * 0.3));
            }
            //Tribulation
            else
            {
                addButton(0, stage[2], tribulationsPrompt).disableIf(player.hasStatusEffect(StatusEffects.TribulationCountdown), "It's already coming.");
            }
            //Requirements
            if (stage[0] != 1)
            {
                button(0).disableIf(player.wis < stage[3], "Your wisdom is too low, required: " + stage[3]);
            }
            button(0).disableIf(player.level < level, "Your level is too low, required: " + level);
            //unique cases
            if (button(0).enabled)
            {
                var _sw40_ = (flags[kFLAGS.SOUL_CULTIVATION]);                
        ...
        {
            outputText("<b>MAXIMUM STAGE REACHED</b>");
        }
        addButton(14, "Back", accessSoulforceMenu);
    }
    ...
    public function SubPaths() : Void
    {
        ...
        {
            addButton(0, "Daoist", daoistSubPath).hint("Contemplate mysteries from 'My Dao Sticks are better than Yours' daoist cultivation manual.");
        }
        if (player.hasKeyItem("Cultivation Manual: Body like a Coke Fiend") >= 0)
        {
            addButton(5, "BodyCult", bodycultivationSubPath).hint("Contemplate mysteries from 'Body like a Coke Fiend' body cultivation manual.");
        }
    ...
    public function daoistSubPath() : Void
    {
        ...
        {
            addButton(i, stages[i][0], daoistSubPathChosen, stages[i][1], stages[i][2]).disableIf(!player.hasItem(stages[i][2]), "Requires " + (try cast(stages[i][2], ItemType) catch(e:Dynamic) null).longName).disableIf(!player.hasPerk(stages[i][3]), "Requires perk: " + (try cast(stages[i][3], PerkType) catch(e:Dynamic) null).name()).disableIf(i != 0 && !player.hasPerk(stages[i - 1][1]), "You need to have achieved the previous stage first.").disableIf(player.hasPerk(stages[i][1]), "You have already reached this stage.");
        }
...
*/
    public function bodycultivationSubPath() : Void
    {
        ...
        for (i in 0...stages.length)
        {
            addButton(i, stages[i][0], bodycultivationSubPathChosen, stages[i][1], stages[i][2]).disableIf(!player.hasItem(stages[i][2], 1) || !player.hasItem(useables.BTSOLUTION, 1), 
                                            "Requires 1 bottle of " + (try cast(stages[i][2], ItemType) catch(e:Dynamic) null).longName + " and 1 vial of " + useables.BTSOLUTION.longName
                        ).disableIf(!player.hasPerk(stages[i][3]), "Requires perk: " + (try cast(stages[i][3], PerkType) catch(e:Dynamic) null).name()).disableIf(i != 0 && !player.hasPerk(stages[i - 1][1]), "You need to have achieved the previous stage first.").disableIf(player.hasPerk(stages[i][1]), "You have already reached this stage.");
        }
        addButton(14, "Back", SubPaths);
    }
    ...
    public function tribulationsPrompt() : Void
    {
        ...
        if (player.level >= 126 && !player.hasPerk(PerkLib.SoulAncestor))
        {
            outputText("Emperor");
        }
        else if (player.level >= 102 && !player.hasPerk(PerkLib.SoulKing))
        {
            outputText("Tyrant");
        }
        else if (player.level >= 78 && !player.hasPerk(PerkLib.SoulOverlord))
        {
            outputText("Exalt");
        }
        else if (player.level >= 54 && !player.hasPerk(PerkLib.SoulElder))
        {
    ...
    public static var daos : Array<Dynamic> = [
    ...
    public static var clones : Array<Dynamic> = [
    ...
    public static inline var clonelevelcost : Int = 9;
    
    public function DaoContemplations() : Void
    {
        clearOutput();
        outputText("Which Dao would you try to comprehend?\n\n");
        for (i in 0...daos.length)
        {
            var dao : Array<Dynamic> = daos[i];
            if (player.hasStatusEffect(dao[1]))
            {
                outputText(dao[0] + ": Level - " + player.statusEffectv2(dao[1]) + ", Progress - " + player.statusEffectv1(dao[1]) + "\n");
            }
            addButton(i, dao[0], daoContemplationsEffect, dao[1], dao[0]).disableIf(player.statusEffectv2(dao[1]) == highestLayerOfDaoComprehension(), 
    ...
    public function daoContemplationsEffect(statusEffect : StatusEffectType, daoname : String, clone : Bool = false, elementalBody : Bool = false) : Void
    {
        ...
        else
        {
            ...
            {
                case "Fire":
                    if (player.hasAnyPerk(PerkLib.FireAffinity, PerkLib.FireShadowAffinity, PerkLib.AffinityIgnis) && !player.hasPerk(PerkLib.CovenantOfTheSpirits))
                    {
                        dao += as3hx.Compat.parseInt(1 + rand(3));
                    }
                case "Ice":
                    if (player.hasAnyPerk(PerkLib.ColdAffinity, PerkLib.ColdMastery))
                    {
                        dao += as3hx.Compat.parseInt(1 + rand(3));
                    }
                case "Lightning":
                ...
                case "Darkness":
                    if (player.hasAnyPerk(PerkLib.DarknessAffinity, PerkLib.FireShadowAffinity))
                    {
                        dao += as3hx.Compat.parseInt(1 + rand(3));
                    }
                case "Poison":
                    if (player.hasPerk(PerkLib.PoisonAffinity))
                    {
                        dao += as3hx.Compat.parseInt(1 + rand(3));
                    }
                case "Wind":
                    ...
                    {
                        dao += as3hx.Compat.parseInt(1 + rand(3));
                        if (player.hasPerk(PerkLib.CovenantOfTheSpirits))
                        {
                            dao += as3hx.Compat.parseInt(1 + rand(9));
                        }
                    }
                case "Blood":
                    if (player.hasAnyPerk(PerkLib.BloodAffinity, PerkLib.BloodMastery, PerkLib.WayOfTheBlood))
                    {
                        dao += as3hx.Compat.parseInt(1 + rand(3));
                    }
                case "Water":
        ...
        if (dao > 0)
        {
            ...
            if (player.hasStatusEffect(statusEffect))
            {
                ...
                var thres : Array<Dynamic> = [20, 40, 60, 100, 140, 180, 220, 260, 300, 400, 500, 600];
                ...
                if (curLevel < thres.length)
                {
                    if (player.statusEffectv1(statusEffect) >= thres[curLevel])
                    {
                        ...
                        if (clone)
                        {
                            outputText("Due to your clone contemplations your");
                        }
                        else
                        {
                            outputText("Your");
            ...
            {
                player.createStatusEffect(statusEffect, dao, 0, 0, 0);
            }
        }
        else
        {
            outputText("After the session ends, you did not manage to progress in your comprehension.\n\n");
        }
        if (!clone && !elementalBody)
        {
    ...
    public function highestLayerOfDaoComprehension() : Float
    {
        var hLrODC : Float = 3;
    ...
    public function sfRegenRacialMult() : Float
    {
        var kitsuneBonus : Array<Dynamic> = [0.4, 0.8, 1.3, 2.0];
        var nekoBonus : Array<Dynamic> = [0.2, 0.4];
        var kitshooBonus : Array<Dynamic> = [0.3, 0.4, 0.5];
        var mult : Float = 1.0;
        if (player.isRace(Races.KITSUNE))
        {
            mult += kitsuneBonus[player.racialTier(Races.KITSUNE) - 1];
        }
        if (player.isRace(Races.NEKOMATA))
        {
            mult += nekoBonus[player.racialTier(Races.NEKOMATA) - 1];
        }
        if (player.isRace(Races.KITSHOO))
        {
            mult += kitshooBonus[player.racialTier(Races.KITSHOO) - 1];
        }
    ...
    private function sfRegen(hours : Int) : Int
    {
        ...
        amount *= as3hx.Compat.parseInt(hours * 2 - 1);  //time mult  
        amount *= sfRegenRacialMult();
        return as3hx.Compat.parseInt(amount);
    }
    
    //Predict the soulforce change after the meditation
    public function meditationPredict(hours : Int) : Int
    {
        var maxChange : Int = sfRegen(hours)
        + Math.round(SceneLib.combat.soulforceregeneration2() * 2 * SceneLib.combat.soulforceRecoveryMultiplier() * hours)  //regen from meditation itself  ;  //from time spent  
    ...
    private function meditate(hours : Int) : Void
    {
        ...
        if (player.isGargoyle() && player.hasPerk(PerkLib.GargoylePure))
        {
            player.refillGargoyleHunger(20 * hours);
        }
    ...
    {
        var maxForItems : Float = 0;  //razem może mieć max 2330%  
        if (player.weaponName == "training soul axe")
        {
            maxForItems += 80;
        }
        if (player.weaponRangeName == "training soul crossbow")
        {
            maxForItems += 50;
        }
        if (player.shieldName == "training soul buckler")
        {
            maxForItems += 60;
        }
        if (player.armorName == "training soul armor")
        {
            maxForItems += 240;
        }
        if (player.upperGarmentName == "soul training shirt")
        {
            maxForItems += 200;
        }
        if (player.lowerGarmentName == "soul training panties")
        {
            maxForItems += 200;
        }
        if (player.headjewelryName == "training soul hairpin")
        {
            maxForItems += 200;
        }
        if (player.necklaceName == "training soul necklace")
        {
            maxForItems += 300;
        }
        if (player.jewelryName == "training soul ring")
        {
            maxForItems += 100;
        }
        if (player.jewelryName2 == "training soul ring")
        {
            maxForItems += 100;
        }
        if (player.jewelryName3 == "training soul ring")
        {
            maxForItems += 100;
        }
        if (player.jewelryName4 == "training soul ring")
        {
            maxForItems += 100;
        }
        if (player.weaponFlyingSwordsName == "training soul flying sword")
        {
            maxForItems += 500;
        }
        return maxForItems;
    ...
    {
        var itemCnt : Float = sfTrainingItems();
        if (itemCnt > 10)
        {
            itemCnt *= (1.0 + itemCnt * 0.1);
        }
        else if (itemCnt > 5)
        {
            itemCnt *= (1.0 + itemCnt * 0.05);
        }
        var limit : Int = cultivationBonusMaxSF_limit();
        if (flags[kFLAGS.SOULFORCE_GAINED_FROM_CULTIVATING] < limit)
        {
            flags[kFLAGS.SOULFORCE_GAINED_FROM_CULTIVATING] += Math.min(as3hx.Compat.parseInt(itemCnt * hours), limit - flags[kFLAGS.SOULFORCE_GAINED_FROM_CULTIVATING]);
        }
    }
    ...
    public function SelfSustain() : Void
    {
        ...
        var maxH : Float = player.maxHunger();
        menu();
        if (player.soulforce >= Math.round(maxH * 0.5))
        {
            addButton(0, "-10-", SelfSustain0, Math.round(maxH * 0.1)).hint("Spend soulforce to decrease hunger by 10%.");
        }
        else
        {
            addButtonDisabled(0, "-10-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= maxH)
        {
            addButton(1, "-20-", SelfSustain0, Math.round(maxH * 0.2)).hint("Spend soulforce to decrease hunger by 20%.");
        }
        else
        {
            addButtonDisabled(1, "-20-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxH * 1.5))
        {
            addButton(2, "-30-", SelfSustain0, Math.round(maxH * 0.3)).hint("Spend soulforce to decrease hunger by 30%.");
        }
        else
        {
            addButtonDisabled(2, "-30-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxH * 2))
        {
            addButton(3, "-40-", SelfSustain0, Math.round(maxH * 0.4)).hint("Spend soulforce to decrease hunger by 40%.");
        }
        else
        {
            addButtonDisabled(3, "-40-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxH * 2.5))
        {
            addButton(4, "-50-", SelfSustain0, Math.round(maxH * 0.5)).hint("Spend soulforce to decrease hunger by 50%.");
        }
        else
        {
            addButtonDisabled(4, "-50-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxH * 3))
        {
            addButton(5, "-60-", SelfSustain0, Math.round(maxH * 0.6)).hint("Spend soulforce to decrease hunger by 60%.");
        }
        else
        {
            addButtonDisabled(5, "-60-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxH * 3.5))
        {
            addButton(6, "-70-", SelfSustain0, Math.round(maxH * 0.7)).hint("Spend soulforce to decrease hunger by 70%.");
        }
        else
        {
            addButtonDisabled(6, "-70-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxH * 4))
        {
            addButton(7, "-80-", SelfSustain0, Math.round(maxH * 0.8)).hint("Spend soulforce to decrease hunger by 80%.");
        }
        else
        {
            addButtonDisabled(7, "-80-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxH * 4.5))
        {
            addButton(8, "-90-", SelfSustain0, Math.round(maxH * 0.93)).hint("Spend soulforce to decrease hunger by 90%.");
        }
        else
        {
            addButtonDisabled(8, "-90-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxH * 5))
        {
            addButton(9, "-100-", SelfSustain0, maxH).hint("Spend soulforce to decrease hunger by 00%.");
        }
        else
        {
            addButtonDisabled(9, "-100-", "Your current soulforce is too low.");
        }
        addButton(14, "Back", accessSoulforceMenu);
    }
    public function SelfSustain0(ratio : Float) : Void
    {
        clearOutput();
        ...
        if (player.isGargoyle() && player.hasPerk(PerkLib.GargoylePure))
        {
            player.refillGargoyleHunger(ratio);
        }
        statScreenRefresh();
    ...
    public function RepresLust() : Void
    {
        ...
        outputText("So what amount of your soulforce do you want to use?");
        var maxL : Float = player.maxLust();
        menu();
        if (player.soulforce >= Math.round(maxL * 0.2))
        {
            addButton(0, "-10-", RepresLust0, Math.round(maxL * 0.1)).hint("Spend soulforce to decrease lust by 10%.");
        }
        else
        {
            addButtonDisabled(0, "-10-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxL * 0.4))
        {
            addButton(1, "-20-", RepresLust0, Math.round(maxL * 0.2)).hint("Spend soulforce to decrease lust by 20%.");
        }
        else
        {
            addButtonDisabled(1, "-20-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxL * 0.6))
        {
            addButton(2, "-30-", RepresLust0, Math.round(maxL * 0.3)).hint("Spend soulforce to decrease lust by 30%.");
        }
        else
        {
            addButtonDisabled(2, "-30-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxL * 0.8))
        {
            addButton(3, "-40-", RepresLust0, Math.round(maxL * 0.4)).hint("Spend soulforce to decrease lust by 40%.");
        }
        else
        {
            addButtonDisabled(3, "-40-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= maxL)
        {
            addButton(4, "-50-", RepresLust0, Math.round(maxL * 0.5)).hint("Spend soulforce to decrease lust by 50%.");
        }
        else
        {
            addButtonDisabled(4, "-50-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxL * 1.2))
        {
            addButton(5, "-60-", RepresLust0, Math.round(maxL * 0.6)).hint("Spend soulforce to decrease lust by 60%.");
        }
        else
        {
            addButtonDisabled(5, "-60-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxL * 1.4))
        {
            addButton(6, "-70-", RepresLust0, Math.round(maxL * 0.7)).hint("Spend soulforce to decrease lust by 70%.");
        }
        else
        {
            addButtonDisabled(6, "-70-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxL * 1.6))
        {
            addButton(7, "-80-", RepresLust0, Math.round(maxL * 0.8)).hint("Spend soulforce to decrease lust by 80%.");
        }
        else
        {
            addButtonDisabled(7, "-80-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxL * 1.8))
        {
            addButton(8, "-90-", RepresLust0, Math.round(maxL * 0.9)).hint("Spend soulforce to decrease lust by 90%.");
        }
        else
        {
            addButtonDisabled(8, "-90-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxL * 2))
        {
            addButton(9, "-100-", RepresLust0, maxL).hint("Spend soulforce to decrease lust by 100%.");
        }
        else
        {
            addButtonDisabled(9, "-100-", "Your current soulforce is too low.");
    ...
    public function RepresLust0(ratio : Float) : Void
    {
        clearOutput();
    ...
    public function ManaAndSoulforce() : Void
    {
        ...
        var maxS : Float = player.maxSoulforce();
        var maxM : Float = player.maxMana();
        menu();
        ...
        {
            addButtonDisabled(0, "100", "Your current soulforce is too low.");
        }
        if (player.soulforce >= 500)
        {
            addButton(1, "500", convertSoulforce, 500).hint("Convert 500 soulforce to 500 mana.");
        }
        else
        {
            addButtonDisabled(1, "500", "Your current soulforce is too low.");
        }
        if (player.soulforce >= 1000)
        {
            addButton(2, "1000", convertSoulforce, 1000).hint("Convert 1,000 soulforce to 1,000 mana.");
        }
        else
        {
            addButtonDisabled(2, "1000", "Your current soulforce is too low.");
        }
        if (player.soulforce >= 5000)
        {
            addButton(3, "5000", convertSoulforce, 5000).hint("Convert 5,000 soulforce to 5,000 mana.");
        }
        else
        {
            addButtonDisabled(3, "5000", "Your current soulforce is too low.");
        }
        if (player.soulforce >= 10000)
        {
            addButton(4, "10000", convertSoulforce, 10000).hint("Convert 10,000 soulforce to 10,000 mana.");
        }
        else
        {
            addButtonDisabled(4, "10000", "Your current soulforce is too low.");
        }
        if (player.mana >= 200)
        {
            addButton(5, "200", convertMana, 200).hint("Convert 200 mana to 100 soulforce.");
        }
        else
        {
            addButtonDisabled(5, "200", "Your current mana is too low.");
        }
        if (player.mana >= 1000)
        {
            addButton(6, "1000", convertMana, 1000).hint("Convert 1,000 mana to 500 soulforce.");
        }
        else
        {
            addButtonDisabled(6, "1000", "Your current mana is too low.");
        }
        if (player.mana >= 2000)
        {
            addButton(7, "2000", convertMana, 2000).hint("Convert 2,000 mana to 1,000 soulforce.");
        }
        else
        {
            addButtonDisabled(7, "200", "Your current mana is too low.");
        }
        if (player.mana >= 10000)
        {
            addButton(8, "10000", convertMana, 10000).hint("Convert 10,000 mana to 5,000 soulforce.");
        }
        else
        {
            addButtonDisabled(8, "200", "Your current mana is too low.");
        }
        if (player.mana >= 20000)
        {
            addButton(9, "20000", convertMana, 20000).hint("Convert 20,000 mana to 10,000 soulforce.");
        }
        else
        {
            addButtonDisabled(9, "200", "Your current mana is too low.");
        }
        if (player.soulforce >= Math.round(maxM * 0.25))
        {
            addButton(10, "-25-", convertSoulforce, Math.round(maxM * 0.25)).hint("Convert " + Math.round(maxM * 0.25) + " soulforce to " + Math.round(maxM * 0.25) + " mana.");
        }
        else
        {
            addButtonDisabled(10, "-25-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxM * 0.25))
        {
            addButton(11, "-100-", convertSoulforce, maxM).hint("Convert " + maxM + " soulforce to " + maxM + " mana.");
        }
        else
        {
            addButtonDisabled(11, "-100-", "Your current soulforce is too low.");
        }
        if (player.soulforce >= Math.round(maxS * 0.5))
        {
            addButton(12, "-25-", convertMana, Math.round(maxS * 0.5)).hint("Convert " + Math.round(maxS * 0.5) + " mana to " + Math.round(maxS * 0.25) + " soulforce.");
        }
        else
        {
            addButtonDisabled(12, "-25-", "Your current mana is too low.");
        }
        if (player.soulforce >= (maxS * 2))
        {
            addButton(13, "-100-", convertMana, maxS * 2).hint("Convert " + (maxS * 2) + " mana to " + maxS + " soulforce.");
        }
        else
        {
            addButtonDisabled(13, "-100-", "Your current mana is too low.");
        }
        addButton(14, "Back", accessSoulforceMenu);
    }
    ...
    {
        var cost : Int = as3hx.Compat.parseInt(Math.abs(change) * 100);
        if (change > 0)
        {
            cost *= 0.5;
        }
    ...
    public function ImprintingSF() : Void
    {
        ...
        menu();
        var btn : Int = 0;
        if (player.itemCount(weaponsflyingswords.W_HALFM) > 1)
        {
            addButton(btn++, "W.HalfM2", ImprintingSFCombine, weaponsflyingswords.W_HALFM, weaponsflyingswords.W_HALFM2, 120).disableIf(player.soulforce < 120, "Req. 120 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.W_HALFM2) > 0)
        {
            addButton(btn++, "W.HalfM", ImprintingSFSeparate1, weaponsflyingswords.W_HALFM2, weaponsflyingswords.W_HALFM, 120).disableIf(player.soulforce < 120, "Req. 120 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.B_HALFM) > 1)
        {
            addButton(btn++, "B.HalfM2", ImprintingSFCombine, weaponsflyingswords.B_HALFM, weaponsflyingswords.B_HALFM2, 120).disableIf(player.soulforce < 120, "Req. 120 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.B_HALFM2) > 0)
        {
            addButton(btn++, "B.HalfM", ImprintingSFSeparate1, weaponsflyingswords.B_HALFM2, weaponsflyingswords.B_HALFM, 120).disableIf(player.soulforce < 120, "Req. 120 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.S_HALFM) > 1)
        {
            addButton(btn++, "S.HalfM2", ImprintingSFCombine, weaponsflyingswords.S_HALFM, weaponsflyingswords.S_HALFM2, 120).disableIf(player.soulforce < 120, "Req. 120 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.S_HALFM2) > 0)
        {
            addButton(btn++, "S.HalfM", ImprintingSFSeparate1, weaponsflyingswords.S_HALFM2, weaponsflyingswords.S_HALFM, 120).disableIf(player.soulforce < 120, "Req. 120 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.E_HALFM) > 1)
        {
            addButton(btn++, "E.HalfM2", ImprintingSFCombine, weaponsflyingswords.E_HALFM, weaponsflyingswords.E_HALFM2, 120).disableIf(player.soulforce < 120, "Req. 120 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.E_HALFM2) > 0)
        {
            addButton(btn++, "E.HalfM", ImprintingSFSeparate1, weaponsflyingswords.E_HALFM2, weaponsflyingswords.E_HALFM, 120).disableIf(player.soulforce < 120, "Req. 120 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.MOONLGT) > 1)
        {
            addButton(btn++, "MoonLgt2", ImprintingSFCombine, weaponsflyingswords.MOONLGT, weaponsflyingswords.MOONLGT2, 160).disableIf(player.soulforce < 160, "Req. 160 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.MOONLGT2) > 0)
        {
            addButton(btn++, "W.MoonLgt", ImprintingSFSeparate1, weaponsflyingswords.MOONLGT2, weaponsflyingswords.MOONLGT, 160).disableIf(player.soulforce < 160, "Req. 160 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.W_HALFM2) > 1)
        {
            addButton(btn++, "W.HalfM4", ImprintingSFCombine, weaponsflyingswords.W_HALFM2, weaponsflyingswords.W_HALFM3, 240).disableIf(player.soulforce < 240, "Req. 240 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.W_HALFM3) > 0)
        {
            addButton(btn++, "W.HalfM2", ImprintingSFSeparate1, weaponsflyingswords.W_HALFM3, weaponsflyingswords.W_HALFM2, 240).disableIf(player.soulforce < 240, "Req. 240 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.B_HALFM2) > 1)
        {
            addButton(btn++, "B.HalfM4", ImprintingSFCombine, weaponsflyingswords.B_HALFM2, weaponsflyingswords.B_HALFM3, 240).disableIf(player.soulforce < 240, "Req. 240 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.B_HALFM3) > 0)
        {
            addButton(btn++, "B.HalfM2", ImprintingSFSeparate1, weaponsflyingswords.B_HALFM3, weaponsflyingswords.B_HALFM2, 240).disableIf(player.soulforce < 240, "Req. 240 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.S_HALFM2) > 1)
        {
            addButton(btn++, "S.HalfM4", ImprintingSFCombine, weaponsflyingswords.S_HALFM2, weaponsflyingswords.S_HALFM3, 240).disableIf(player.soulforce < 240, "Req. 240 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.S_HALFM3) > 0)
        {
            addButton(btn++, "S.HalfM2", ImprintingSFSeparate1, weaponsflyingswords.S_HALFM3, weaponsflyingswords.S_HALFM2, 240).disableIf(player.soulforce < 240, "Req. 240 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.E_HALFM2) > 1)
        {
            addButton(btn++, "E.HalfM4", ImprintingSFCombine, weaponsflyingswords.E_HALFM2, weaponsflyingswords.E_HALFM3, 240).disableIf(player.soulforce < 240, "Req. 240 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.E_HALFM3) > 0)
        {
            addButton(btn++, "E.HalfM2", ImprintingSFSeparate1, weaponsflyingswords.E_HALFM3, weaponsflyingswords.E_HALFM2, 240).disableIf(player.soulforce < 240, "Req. 240 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.A_HALFM) > 1)
        {
            addButton(btn++, "A.HalfM2", ImprintingSFCombine, weaponsflyingswords.A_HALFM, weaponsflyingswords.A_HALFM2, 300).disableIf(player.soulforce < 300, "Req. 300 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.A_HALFM2) > 0)
        {
            addButton(btn++, "A.HalfM", ImprintingSFSeparate1, weaponsflyingswords.A_HALFM2, weaponsflyingswords.A_HALFM, 300).disableIf(player.soulforce < 300, "Req. 300 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.O_HALFM) > 1)
        {
            addButton(btn++, "O.HalfM2", ImprintingSFCombine, weaponsflyingswords.O_HALFM, weaponsflyingswords.O_HALFM2, 300).disableIf(player.soulforce < 300, "Req. 300 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.O_HALFM2) > 0)
        {
            addButton(btn++, "O.HalfM", ImprintingSFSeparate1, weaponsflyingswords.O_HALFM2, weaponsflyingswords.O_HALFM, 300).disableIf(player.soulforce < 300, "Req. 300 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.MOONLGT2) > 1)
        {
            addButton(btn++, "MoonLgt4", ImprintingSFCombine, weaponsflyingswords.MOONLGT2, weaponsflyingswords.MOONLGT3, 320).disableIf(player.soulforce < 320, "Req. 320 soulforce.");
        }
        if (player.itemCount(weaponsflyingswords.MOONLGT3) > 0)
        {
            addButton(btn++, "MoonLgt2", ImprintingSFSeparate1, weaponsflyingswords.MOONLGT3, weaponsflyingswords.MOONLGT2, 320).disableIf(player.soulforce < 320, "Req. 320 soulforce.");
        }
        addButton(14, "Back", accessSoulforceMenu);
    }
    public function ImprintingSFCombine(flyingsword1 : FlyingSwords, flyingsword2 : FlyingSwords, soulforceCost : Float) : Void
    {
        clearOutput();
    ...
    public function ImprintingSFSeparate1(flyingsword1 : FlyingSwords, flyingsword2 : FlyingSwords, soulforceCost : Float) : Void
    {
        clearOutput();
    ...
    public function theUnknown(page : Int = 1) : Void
    {
        if (player.soulforce >= 100)
        {
            ...
            if (page == 1)
            {
                if (BelisaFollower.BelisaInGame && BelisaFollower.BelisaFollowerStage < 3 && BelisaFollower.BelisaEncounternum >= 1 && !player.hasStatusEffect(StatusEffects.SpoodersOff))
                {
                    addButton(0, "???", belisatest).hint("Shy Spooder");
                }
                if (!LilyFollower.LilyFollowerState && flags[kFLAGS.LILY_LVL_UP] > 0 && !player.hasStatusEffect(StatusEffects.SpoodersOff))
                {
                    addButton(1, "???", lilytest).hint("Lewd Spooder");
                }
                if (TyrantiaFollower.TyrantiaFollowerStage > 0 && TyrantiaFollower.TyrantiaFollowerStage < 4 && !TyrantiaFollower.TyraniaIsRemovedFromThewGame && !player.hasStatusEffect(StatusEffects.SpoodersOff))
                {
                    addButton(2, "???", FightTyrantia).hint("Scary Spooder");
                }
                if (flags[kFLAGS.IZMA_ENCOUNTER_COUNTER] > 0 && (flags[kFLAGS.IZMA_WORMS_SCARED] == 0 || !player.hasStatusEffect(StatusEffects.Infested)) && flags[kFLAGS.IZMA_FOLLOWER_STATUS] <= 0)
                {
                    addButton(3, "???", tigerSharkGal).hint("Tigershark Gal?");
                }
                if (flags[kFLAGS.NADIA_LVL_UP] > 0 && flags[kFLAGS.NADIA_FOLLOWER] < 6 && player.statusEffectv4(StatusEffects.CampSparingNpcsTimers2) < 1 && !player.hasStatusEffect(StatusEffects.NadiaOff))
                {
                    addButton(4, "???", shyHealer).hint("Shy Healer");
                }
                if (flags[kFLAGS.ISABELLA_AFFECTION] > 0 && flags[kFLAGS.ISABELLA_PLAINS_DISABLED] == 0)
                {
                    addButton(5, "???", germanCow).hint("German Cow");
                }
                if (player.isNaga() && flags[kFLAGS.SAMIRAH_FOLLOWER] <= 9)
                {
                    addButton(6, "???", sneakOnThePlane).hint("F**king ??? on the Plane.");
                }
                if (!SceneLib.kihaFollower.followerKiha() && flags[kFLAGS.KIHA_TALK_STAGE] > 0)
                {
                    addButton(7, "???", quasiDragoness).hint("Quasi-dragoness.");
                }
                if (flags[kFLAGS.ZENJI_PROGRESS] != -1 && flags[kFLAGS.ZENJI_PROGRESS] > 0 && (flags[kFLAGS.ZENJI_PROGRESS] < 8 || flags[kFLAGS.ZENJI_PROGRESS] == 10))
                {
                    addButton(8, "???", theySeeHimTrollinTheyHatin).hint("They see him trollin' They hatin'");
                }
            ...
            if (page == 2)
            {
                if (flags[kFLAGS.ETNA_AFFECTION] >= 2 && flags[kFLAGS.ETNA_FOLLOWER] < 1)
                {
                    addButton(0, "???", hornyCore).hint("Horny-core.");
                }
                if (flags[kFLAGS.ELECTRA_AFFECTION] >= 2 && flags[kFLAGS.ELECTRA_FOLLOWER] < 2)
                {
                    addButton(1, "???", lightningRod).hint("Lightning Rod.");
                }
                if (SceneLib.helScene.helSexualAmbushCondition())
                {
                    addButton(9, "???", analLover).hint("Anal-lover.");
                }
                addButton(10, "???", dragON);
    ...
    {
        clearOutput();
        if (TyrantiaFollower.TyrantiaFollowerStage > 2)
        {
            SceneLib.tyrantia.repeatEncounterBattlefield();
        }
        else if (TyrantiaFollower.TyrantiaAffectionMeter > 40 && TyrantiaFollower.TyrantiaFollowerStage > 1)
        {
            SceneLib.tyrantia.encounterBattlefieldAfter40Affection();
        }
        else if (TyrantiaFollower.TyrantiaFollowerStage > 0)
        {
            SceneLib.tyrantia.repeatEncounterBattlefield();
        }
        else
        {
            SceneLib.tyrantia.firstEncounter();
        }
    }
    ...
    {
        if ((flags[kFLAGS.NADIA_FOLLOWER] < 3 || flags[kFLAGS.NADIA_FOLLOWER] == 5) && flags[kFLAGS.NADIA_LVL_UP] >= 8)
        {
            SceneLib.nadiaScene.postNameEnc();
        }
        else
        {
            SceneLib.nadiaScene.repeatEnc();
        }
    }
    ...
    public function lightningRod() : Void
    {
        ...
        {
            if (flags[kFLAGS.ELECTRA_FOLLOWER] == 1)
            {
                SceneLib.electraScene.ElectraRecruitingAgain();
            }
            else
            {
                SceneLib.electraScene.ElectraRecruiting();
            }
        }
        else
        {
        ...
        if (flags[kFLAGS.ZENJI_PROGRESS] >= 4)
        {
            if (flags[kFLAGS.ZENJI_PROGRESS] == 6)
            {
                if (flags[kFLAGS.ZENJI_PERSPECTIVE_ON_PLAYER] == 100)
                {
                    ...
                    {
                        SceneLib.zenjiScene.followerZenjiRepeatOffer();
                    }
                    else
                    {
                        SceneLib.zenjiScene.followerZenjiFirstTimeOffer();
                    }
                }
                else if (flags[kFLAGS.ZENJI_PERSPECTIVE_ON_PLAYER] == 0)
                {
                    if (flags[kFLAGS.ZENJI_PROGRESS] == 10)
                    {
                        SceneLib.zenjiScene.loverZenjiRepeatOffer();
                    }
                    else
                    {
                        SceneLib.zenjiScene.loverZenjiFirstTimeOffer();
                    }
                }
                else
                {
                    SceneLib.zenjiScene.part2TrollEncounterRepeat();
                }
            }
            else
            {
                SceneLib.zenjiScene.part2TrollEncounterFirst();
            }
        }
        else if (flags[kFLAGS.ZENJI_PROGRESS] > 0 && flags[kFLAGS.ZENJI_PROGRESS] < 4)
        {
            SceneLib.zenjiScene.part1TrollEncounterRepeat();
        }
    }
    ...
    public function accessDemonicEnergyMenu() : Void
    {
        ...
        if (player.hasPerk(PerkLib.Metamorph))
        {
            ...
            {
                addButton(10, "Metamorph", SceneLib.metamorph.openMetamorph).hint("Use your mana to mold your body.");
            }
        }
        else
        {
    ...
    public function demonicEnergyStrengthenBody() : Void
    {
        ...
        if (player.hasPerk(PerkLib.StrengthenBody))
        {
            player.addPerkValue(PerkLib.StrengthenBody, 1, 1);
        }
        else
        {
            player.createPerk(PerkLib.StrengthenBody, 1, 0, 0, 0);
        }
    ...
    public function demonicEnergyStrengthenMagic() : Void
    {
        ...
        if (player.hasPerk(PerkLib.StrengthenMagic))
        {
            player.addPerkValue(PerkLib.StrengthenMagic, 1, 1);
        }
        else
        {
            player.createPerk(PerkLib.StrengthenMagic, 1, 0, 0, 0);
        }
        doNext(accessDemonicEnergyMenu);
    }
    public function demonicEnergyCorruptElementImprove(daoType : String = "") : Void
    {
        ...
        switch (daoType)
        {
            case "Fire":
                ...
                {
                    player.addStatusValue(StatusEffects.DaoOfFire, 2, 1);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.DaoOfFire, 0, 1, 0, 0);
                }
            case "Ice":
                if (player.hasStatusEffect(StatusEffects.DaoOfIce))
                {
                    player.addStatusValue(StatusEffects.DaoOfIce, 2, 1);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.DaoOfIce, 0, 1, 0, 0);
                }
            case "Lightning":
                if (player.hasStatusEffect(StatusEffects.DaoOfLightning))
                {
                    player.addStatusValue(StatusEffects.DaoOfLightning, 2, 1);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.DaoOfLightning, 0, 1, 0, 0);
                }
            case "Darkness":
                if (player.hasStatusEffect(StatusEffects.DaoOfDarkness))
                {
                    player.addStatusValue(StatusEffects.DaoOfDarkness, 2, 1);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.DaoOfDarkness, 0, 1, 0, 0);
                }
            case "Poison":
                if (player.hasStatusEffect(StatusEffects.DaoOfPoison))
                {
                    player.addStatusValue(StatusEffects.DaoOfPoison, 2, 1);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.DaoOfPoison, 0, 1, 0, 0);
                }
            case "Wind":
                if (player.hasStatusEffect(StatusEffects.DaoOfWind))
                {
                    player.addStatusValue(StatusEffects.DaoOfWind, 2, 1);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.DaoOfWind, 0, 1, 0, 0);
                }
            case "Blood":
                if (player.hasStatusEffect(StatusEffects.DaoOfBlood))
                {
                    player.addStatusValue(StatusEffects.DaoOfBlood, 2, 1);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.DaoOfBlood, 0, 1, 0, 0);
                }
            case "Water":
                if (player.hasStatusEffect(StatusEffects.DaoOfWater))
                {
                    player.addStatusValue(StatusEffects.DaoOfWater, 2, 1);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.DaoOfWater, 0, 1, 0, 0);
                }
            case "Earth":
                if (player.hasStatusEffect(StatusEffects.DaoOfEarth))
                {
                    player.addStatusValue(StatusEffects.DaoOfEarth, 2, 1);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.DaoOfEarth, 0, 1, 0, 0);
                }
            case "Acid":
                if (player.hasStatusEffect(StatusEffects.DaoOfAcid))
                {
                    player.addStatusValue(StatusEffects.DaoOfAcid, 2, 1);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.DaoOfAcid, 0, 1, 0, 0);
                }
        }
        doNext(demonicEnergyCorruptElement);
    }
    
    private function deCost(mod : Float) : Float
    {
        var costPercent : Float = 100;
        if (player.hasPerk(PerkLib.DarkAscensionEfficientSoulConsumption))
        {
            costPercent -= (5 * player.perkv1(PerkLib.DarkAscensionEfficientSoulConsumption));
        }
        if (costPercent < 5)
        {
            costPercent = 5;
