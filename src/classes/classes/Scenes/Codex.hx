...
/**
	 ...
	 * @author Kitteh6660
	 */
class Codex extends BaseContent
{
    ...
    public function unlockEntry(flag : Int) : Bool
    {
        for (entries/* AS3HX WARNING could not determine type for var: entries exp: EIdent(pages) type: null */ in pages)
        {
            for (entry/* AS3HX WARNING could not determine type for var: entry exp: EIdent(entries) type: null */ in entries)
            {
                if (entry[0] == flag)
                {
                    if (flags[flag] != null)
                    {
                        return false;
                    }  //return if already unlocked. Not earlier - still want to check that flag is legal.  
    ...
    public function checkUnlocked() : Float
    {
        var num : Int = 0;
        for (entries/* AS3HX WARNING could not determine type for var: entries exp: EIdent(pages) type: null */ in pages)
        {
    ...
    public function checkAchievements() : Void
    {
        ...
        if (unlocked >= 20)
        {
            awardAchievement("Scholar (1)", kACHIEVEMENTS.GENERAL_SCHOLAR);
        }
        if (unlocked >= 30)
        {
            awardAchievement("Scholar (2)", kACHIEVEMENTS.GENERAL_SCHOLAR_2);
        }
        if (unlocked >= 40)
        {
            awardAchievement("Scholar (3)", kACHIEVEMENTS.GENERAL_SCHOLAR_3);
        }
    }
    ...
    public function accessCodexMenu() : Void
    {
        ...
        if (player.hasSpells())
        {
            unlockEntry(kFLAGS.CODEX_ENTRY_MAGIC);
        ...
        var btn : Int = 0;
        for (page in Reflect.fields(pages))
        {
    ...
    private function openPage(page : String) : Void
    {
        ...
        for (i in 0...Reflect.field(pages, page).length)
        {
            var entry : Array<Dynamic> = Reflect.field(Reflect.field(pages, page), Std.string(i));  //Pointer, not a copy!  
            buttons.add((currentEntry == i) ? "(" + entry[1] + ")" : entry[1], curry(entryWrapper, i, entry[2])).disableIf(!Reflect.field(flags, Std.string(entry[0])), null, null, "???");
        }
    ...
    private function codexEntryTyrant() : Void
    {
        ...
        outputText("Today we were given the driders from the village near Gargant river: Noteworthy among them was a muscular, large specimen named Tyrantia. In addition to her larger than normal muscle mass and high density, her mind refuses to break like the others, depriving us of her lethicite. It is...rather vexing...But this could be an opportunity in disguise.\n"
                ...
                + "\n");
        if (flags[kFLAGS.CODEX_ENTRY_TYRANT] == 2)
        {
            outputText("Project Screwed Earth has grown over a foot and a half since we last checked in with her. Worrisome, however, is the appearance of one of Mareth’s self-proclaimed ‘champions’. We believe it’s the sacrificial cow from Ignam this year, the same one that destroyed the factory. Project Fucked Earth has not told them of their true reason for existence.\n"
                    + "\n");
        }
