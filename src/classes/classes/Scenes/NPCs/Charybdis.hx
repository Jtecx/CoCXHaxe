...
class Charybdis extends Monster
{
    private var coldFury : Float = 0;
    
    private function charybdisAttack1() : Void
    {
        var hits : Float = 1 + rand(8);
        if (hits > 1)
        {
            outputText("The daggers gleams as they slices into your flesh.");
        }
        else
        {
            outputText("The dagger gleams as it slices into your flesh.");
        }
        var hibD : Float = hits;
        while (hibD-- > 0)
        {
    ...
    public function charybdisConstrict() : Void
    {
        ...
        {
            outputText("You hit the ground hard, narrowly dodging as Charybdis flies past. You roll to your feet, turning around to face him. The Scylla’s eyes narrow, and he exhales deeply, splaying his knives for the next attack.");
        }
        else
        {
    ...
    private function charybdisRejuvenationRhythm() : Void
    {
        ...
        var temp : Float = Math.round(maxHP() * 0.15);
    ...
    {
        if (HPRatio() < .2 && coldFury == 2)
        {
            charybdisColdFury2();
        }
        else if (HPRatio() < .4 && coldFury == 1)
        {
            charybdisColdFury2();
        }
        else if (HPRatio() < .7 && coldFury == 0)
        {
            charybdisColdFury2();
        }
    }
    private function charybdisColdFury2() : Void
    {
        ...
        lustVuln -= 0.05;
        var speSta : Float = speStat.core.value;
        speSta *= 1.25;
        speSta = Math.round(speSta);
        speStat.core.value += speSta;
        var weaponAtt : Float = weaponAttack;
        weaponAtt *= 1.25;
    ...
    override private function get_long() : String
    {
        ...
        {
            str += "Charybdis stands, bobbing to and fro on two tentacles. His purple eyes stare at you nervously, and his gills flare. Evidently this battle’s starting to anger the Scylla.";
        }
        else if (this.HP >= this.maxHP() * 0.4)
        {
            str += "Red eyes stare at you from inside slightly sunken sockets. The knives on his tentacles are levelled towards you, and his barrel chest heaves. Charybdis has almost completely stopped moving except to avoid a blow or deal one in turn. Be careful.";
        }
        else
        {
            str += "His body is slumping, but the glow in his red eyes have only intensified. His guard seems to have loosened, but his tendrils and arms have gotten noticeably faster. Keep on your guard.";
        }
        return str;
    }
