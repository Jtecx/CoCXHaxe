...
class SamirahScene extends NPCAwareContent implements TimeAwareInterface
{
    ...
    public function samirahAffection(changes : Float = 0) : Float
    {
        flags[kFLAGS.SAMIRAH_AFFECTION] += changes;
        if (flags[kFLAGS.SAMIRAH_AFFECTION] > 100)
        {
            flags[kFLAGS.SAMIRAH_AFFECTION] = 100;
        }
    ...
    public function desertEncounters() : Void
    {
        ...
        else if (flags[kFLAGS.SAMIRAH_FOLLOWER] == 9)
        {
    ...
    public function samirahNoCamp() : Void
    {
        ...
        {
            flags[kFLAGS.SAMIRAH_FOLLOWER] = 9;
        }
        endEncounter();
    }
    ...
    public function samirahMainCampMenu2() : Void
    {
        ...
        {
            outputText("She's coiled trying to preserve her heat as the last rays of sunlight slowly disappears");
        }
        else
        {
            outputText("She’s as her usual resting on some warm ground and sunbathing. She opens her eyelids as you approach");
        }
        outputText(".\n\n\"<i>Good ");
        if (model.time.hours >= 19)
        {
        ...
        {
            outputText((model.time.hours <= 11) ? "morning" : "afternoon");
        }
        ...
        {
            addButton(4, "Sleep With", samirahSleepToggle);
        }
        else
        {
            addButton(4, "Sleep Alone", samirahSleepToggle);
        }
        if (flags[kFLAGS.SAMIRAH_HYPNOSIS] > 0)
        {
            if (player.isNaga())
            {
                addButton(6, "Hypnosis", samirahHypnosisTraining);
            }
            else
            {
                addButtonDisabled(6, "Hypnosis", "You need a naga body to train into hypnosis...");
            }
        }
    ...
    public function samirahMainTalkMenu() : Void
    {
        ...
        if (flags[kFLAGS.SAMIRAH_CLOTHES] < 1)
        {
            addButton(5, "Clothes", samirahTalkClothes);
        }
        addButton(14, "Back", samirahMainCampMenu2);
    }
    ...
    public function sexMenu() : Void
    {
        ...
        addButton(12, "Twin Tail Fuck", samirahTwinTailFuck).disableIf(!player.isNaga() && player.tailType != Tail.DRACONIC && player.tailType != Tail.LIZARD && player.tailType != Tail.SALAMANDER && player.tailType != Tail.CAVE_WYRM, "Requires a long (naga, dragon, lizard, salamander, cave wyrm) tail!").disableIf(!player.hasVagina(), "Req. a vagina!");
    }
    ...
    public function nagaDefaultSexScene() : Void
    {
        ...
        var nofitF : Void->Void = function() : Void
        {
            ...
            if (player.cumQ() > 250)
            {
                outputText("Her cheeks bulge with the volume and she struggles to swallow as much of it as she can. ");
            }
            if (player.cumQ() > 1000)
            {
                outputText("Her cheeks bulge out with the sheer volume of your cum and she is forced to release you from her mouth to avoid drowning in your seed. ");
            }
        ...
        {
            var x1 : Int = as3hx.Compat.parseInt(player.cockThatFits(35) + 1);
            ...
            if (player.cockTotal() == 1)
            {
                outputText("Gently she starts to stroke the length of your [cock " + x1 + "] with one hand while circling the tip with the other. ");
            }
            if (player.cockTotal() == 2)
            {
                outputText("She takes one in each of her hands and gently strokes them up and down. Every few seconds she stops at the tip to rub her palms over them. ");
            }
            if (player.cockTotal() >= 3)
            {
                outputText("She takes one in each hand, stroking them slowly and making sure to pay attention to the tip. Every so often she switches to a different dick to make sure that each and every one of your throbbing cocks has some love given to them. ");
            }
            ...
            if (player.cockTotal() == 1)
            {
                outputText("She carefully lines it up with your member and starts to tease the tip before gently inserting the first few inches. ");
            }
            if (player.cockTotal() == 2)
            {
                outputText("She carefully lines it up with both of your members and starts to tease their tips before gently inserting the first few inches. ");
            }
            if (player.cockTotal() >= 3)
            {
                outputText("She carefully lines it up with one of your members and starts to tease the tip before gently inserting the first few inches. ");
            }
            ...
            if (player.cumQ() > 250)
            {
                outputText("You quickly fill her with your seed to the point where she overflows, leaving her pussy dripping with semen afterwards.");
            }
            else if (player.cumQ() > 1000)
            {
                outputText("Her stomach quickly swells from the sheer volume of seed pumped into her. The sperm that her womb is unable to hold starts to gush out from her stuffed cunt.");
            }
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            {
                outputText("carefully nibbling at it so as to not break the skin. ");
            }
            else
            {
                outputText("nibbling gently at it. ");
            }
