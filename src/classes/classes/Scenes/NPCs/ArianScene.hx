...
*/
class ArianScene extends NPCAwareContent implements SaveableState implements TimeAwareInterface
{
    public static var ArianSonsNum : Float;
    public static var ArianDaughtersNum : Float;
    public static var ArianHermKidsNum : Float;
    public static var ArianTotalKidsNum : Float;
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            ArianSonsNum = Reflect.field(o, "ArianSonsNum");
            ArianDaughtersNum = Reflect.field(o, "ArianDaughtersNum");
            ArianHermKidsNum = Reflect.field(o, "ArianHermKidsNum");
            ArianTotalKidsNum = Reflect.field(o, "ArianTotalKidsNum");
        }
    ...
    //Get out if already pregged.
    {
        ...
        var score : Float = Math.min(player.cumQ() / 100, 5);
    ...
    {
        if (flags[kFLAGS.ARIAN_EGG_COUNTER] > 0)
        {
            flags[kFLAGS.ARIAN_EGG_COUNTER]++;
        }
        if (model.time.hours > 23)
        {
            if (arianScene.arianFollower() && flags[kFLAGS.ARIAN_VAGINA] > 0)
            {
                flags[kFLAGS.ARIAN_EGG_EVENT]++;
            }
    ...
    private function arianCockSize() : Float
    {
        if (flags[kFLAGS.ARIAN_COCK_SIZE] < 0 || flags[kFLAGS.ARIAN_COCK_SIZE] > 3)
        {
            return 0;
        }
        else if (flags[kFLAGS.ARIAN_COCK_SIZE] == 1)
        {
            return 9;
        }
    ...
    public static inline var arianCapacity : Float = 50;
    
    override public function arianFollower() : Bool
    {
    ...
    public function arianHealth(arg : Float = 0) : Float
    {
        if (arg != 0)
        {
            ...
            {
                flags[kFLAGS.ARIAN_HEALTH] = 100;
            }
            else if (flags[kFLAGS.ARIAN_HEALTH] < 0)
            {
                flags[kFLAGS.ARIAN_HEALTH] = 0;
            }
        }
    ...
    //[=Don't Help=]
    private function dontHelpArianWhenYouMeetHim(never : Bool = false) : Void
    {
        clearOutput();
    ...
    public function visitThePark() : Void
    {
        ...
        else if (flags[kFLAGS.ARIAN_PARK] == 3)
        {
            ...
            if (player.cor > 66)
            {
                outputText(", whilst privately thinking to yourself how useless that looks - no wonder they can't fight the demons if this is the best they're capable of");
            }
            outputText(".");
            ...
            if (player.cor >= 66)
            {
                outputText(", keeping your real reasons for bothering with him to yourself");
            }
            outputText(".");
    ...
    //First Visit
    public function visitAriansHouse(back : Bool = false) : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_arian);
        if (flags[kFLAGS.ARIAN_HEALTH] < 29 || flags[kFLAGS.ARIAN_VIRGIN] == 1 && !back)
        {
            arianHealth(1);
        }
        if (arianFollower())
        ...
        else
        {
            ...
            if (flags[kFLAGS.ARIAN_PARK] == 4)
            {
                ...
                {
                    outputText("seat your tauric body on the floor");
                }
            ...
            else
            {
                ...
                else if (flags[kFLAGS.ARIAN_HEALTH] < 75)
                {
                    ...
                    if (!player.isTaur())
                    {
                        outputText("take the offered chair");
                    }
                    else
                    {
                        outputText("the offered chair");
                    }
                    outputText(".");
                ...
                else
                {
                    ...
                    if (!player.isTaur())
                    {
                        outputText(" and offers you the chair nearby");
                    }
    ...
    private function arianHomeMenu(back : Bool = false) : Void
    {
        sceneHunter.print("Arian's health: " + arianHealth() + " out of 100.");
        menu();
        if (!back && flags[kFLAGS.ARIAN_S_DIALOGUE] == 0)
        {
            addButton(0, "Next", arianStoryDialogue1);
        }
        else if (!back && flags[kFLAGS.ARIAN_S_DIALOGUE] == 1 && arianHealth() >= 20)
        {
            addButton(0, "Next", arianStoryDialogue2);
        }
        else if (!back && flags[kFLAGS.ARIAN_S_DIALOGUE] == 2 && arianHealth() >= 30)
        {
            addButton(0, "Next", arianDialogue3);
        }
        else if (!back && flags[kFLAGS.ARIAN_S_DIALOGUE] == 3 && arianHealth() >= 50)
        {
            addButton(0, "Next", arianImbue);
        }
        else if (!back && flags[kFLAGS.ARIAN_S_DIALOGUE] == 4 && arianHealth() >= 75)
        {
            addButton(0, "Next", arianPlot4);
        }
        else if (!back && flags[kFLAGS.ARIAN_S_DIALOGUE] == 5 && arianHealth() >= 100)
        {
            addButton(0, "Next", arianPlot5);
        }
        //If no story dialogue
        else
        {
            addButton(0, "Talk", talkToArianChoices);
            if (flags[kFLAGS.ARIAN_S_DIALOGUE] >= 2)
            {
                addButton(1, "Sex", arianSexMenu).disableIf(player.lust < 33, "Not aroused enough!");
            }
            if (flags[kFLAGS.ARIAN_S_DIALOGUE] >= 1)
            {
                addButton(3, "Give Item", giveArianAnItem);
            }
            if (player.hasKeyItem("Arian's Talisman") >= 0 || player.hasKeyItem("Arian's Charged Talisman") >= 0)
            {
                addButton(2, "Talisman", imbueTalisman);
            }
            if (flags[kFLAGS.ARIAN_S_DIALOGUE] >= 5)
            {
                addButton(4, "Treat Corr.", treatCorruption);
            }
            if (arianFollower())
            {
                addButton(5, "Appearance", arianAppearance);
            }
            addButton(6, "Cursed Item", removeCursedItem).hint("Ask Arian to remove your cursed item. Costs 500 gems. ").disableIf(arianHealth() < 75, "Arian is too weak").disableIf(player.gems < 500, "Ask Arian to remove your cursed item. Costs 500 gems (Can't afford).").disableIf(player.equippedKnownCursedItems().length == 0, "Arian can remove cursed items, but you're not wearing any.").disableIf(flags[kFLAGS.ARIAN_S_DIALOGUE] < 3, "???", "???");
            if (player.weaponRange == weaponsrange.SAGITTB)
            {
                addButton(7, "Cursed Bow", removeCursedItem2).hint("Ask Arian to remove your cursed bow. Costs 500 gems. ").disableIf(arianHealth() < 75, "Arian is too weak").disableIf(player.gems < 500, "Ask Arian to remove your cursed bow. Costs 500 gems (Can't afford).").disableIf(flags[kFLAGS.ARIAN_S_DIALOGUE] < 3, "???", "???");
            }
            if (model.time.hours >= 17 && arianFollower())
            {
                addButton(8, "Sleep With", sleepWithArian, true);
            }
            if (flags[kFLAGS.SLEEP_WITH] == "Arian")
            {
                addButton(8, "NoSleepWith", dontSleepWithArian);
            }
            if (!arianFollower())
            {
    ...
    private function removeCursedItem() : Void
    {
        ...
        var slot : Int = player.slotOfEquippedItem(try cast(item, Equipable) catch(e:Dynamic) null);
        if (slot == -1)
        {
    ...
    private function hermsLikeMaleArian() : Void
    {
        ...
        menu();
        if (player.hasCock() && player.cockThatFits(arianCapacity) >= 0)
        {
    ...
    private function arianStoryDialogue1() : Void
    {
        ...
        //PC has met Marae:
        if (flags[kFLAGS.MET_MARAE] >= 1)
        {
            outputText("Marae herself told you they showed up about, what, 20-30 years ago?");
        }
        else
        {
            outputText("you'd guess a long while ago given the general mess they seem to have made of the world.");
        }
    ...
    private function arianStoryPry() : Void
    {
        ...
        if (player.cor < 40)
        {
            outputText("; [arian ey]'ll sleep better with the burden off [arian eir] conscience");
        }
        outputText(".");
    ...
    private function arianPlot5() : Void
    {
        ...
        {
            outputText("  There are other people living with you already, so what's one more?  Right?");
        }
        outputText("</i>\"");
    ...
    private function talkToArianChoices() : Void
    {
        ...
        {
            addButton(0, "Sexy Talk", arianSexingTalk);
        }
        if (flags[kFLAGS.ARIAN_S_DIALOGUE] >= 3)
        {
            addButton(1, "Teach Magic", arianMagicLessons);
        }
        if (arianFollower())
        {
            addButton(2, "Scales", arianScalesTalk);
        }
        if (!arianFollower() && flags[kFLAGS.ARIAN_S_DIALOGUE] >= 6)
        {
            addButton(4, "Invite2Camp", inviteArianToCamp);
        }
        if (TyrantiaFollower.TyrantiaFollowerStage == 5)
        {
            addButton(5, "Tyrantia", TyrantiaEggQuestArian);
        }
        if (flags[kFLAGS.ARIAN_VIRGIN] == 0 && flags[kFLAGS.ARIAN_S_DIALOGUE] < 3)
        {
    ...
    private function arianMagicLessons() : Void
    {
        ...
        if (player.inte < 25)
        {
            ...
            if (!arianFollower())
            {
                outputText("make your way back to your camp.");
            }
            else
            {
                outputText("return to your duties.");
            }
        }
        ...
        else if (player.inte < 50 || !player.hasStatusEffect(StatusEffects.KnowsCharge))
        {
            ...
            if (!arianFollower())
            {
                outputText(", making your way back to camp.");
            }
            else
            {
                outputText(", exiting [arian eir] tent and going about your business.");
            }
        ...
        else
        {
            ...
            if (!arianFollower())
            {
                outputText("begin the trek home.");
            }
            else
            {
                outputText("exit [arian eir] tent.");
            }
    ...
    private function arianSexingTalk() : Void
    {
        ...
        {
            //(else if (ArianAnalXP <= 100))if (flags[kFLAGS.ARIAN_ANAL_XP] <= 100)
            {
    ...
    private function sexTalkFinish(newl : Bool = false) : Void
    {
        if (newl)
        {
            clearOutput();
    ...
    private function takeYerLizardHomePartII() : Void
    {
        ...
        {
            outputText("cabin.  \"<i>That's a nice cabin you've got here,</i>\" [arian Ey] says.  You thank [arian em] for the appreciation.");
        }
        else
        {
            outputText("tent.");
        ...
        if (camp.followersCount() > 1)
        {
            outputText(" and your other companions");
        }
        outputText(".");
        ...
        {
            outputText("In fact, you jest, you just might bring your sleeping roll in here and sleep with the lizan instead of in your own " + ((CoC.instance.flags[kFLAGS.CAMP_BUILT_CABIN] == 1) ? "cabin" : "tent") + "Arian averts [arian eir] eyes and then smiles at you nervously.  \"<i>T-there's no need for you to bring your sleeping bag... </i>");
        }
        else
        {
            outputText("In fact, you jest, you just might have to forgo your own bed and sleep with the lizan instead of in your own cabin. Arian averts [arian eir] eyes and then smiles at you nervously. \"<i>W-well... </i>");
        }
    ...
    private function arianSexMenu(output : Bool = true) : Void
    {
        if (output)
        {
            clearOutput();
    ...
    private function giveArianAnal() : Void
    {
        ...
        if (flags[kFLAGS.ARIAN_ANAL_XP] >= 100)
        {
            flags[kFLAGS.ARIAN_ANAL_XP] = 100;
        }
        ...
        if (flags[kFLAGS.ARIAN_ANAL_XP] < 33)
        {
            ...
            if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
            {
                ...
                if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
                {
                    outputText("pair of reptilian pricks");
                }
                else
                {
                    outputText("reptilian prick");
                }
            }
            ...
            if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
            {
                outputText("spewing forth large gobs of cum of [arian eir] own " + ((flags[kFLAGS.ARIAN_VAGINA] > 0) ? "and " : ""));
            }
            if (flags[kFLAGS.ARIAN_VAGINA] > 0)
            {
                outputText("wetting both your and [arian eir] legs with a healthy serving of lizan pussy juice");
            }
        ...
        {
            //Loose, sensitive enough to make Arian scream in pleasure. (It's like a tight vagina, feels like one too, minor pain)if (flags[kFLAGS.ARIAN_ANAL_XP] < 66)
            {
                ...
                if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
                {
                    ...
                    if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
                    {
                        outputText(" pair of dripping cocks");
                    }
                    else
                    {
                ...
                if (flags[kFLAGS.ARIAN_VAGINA] > 0)
                {
                    outputText(" a sopping wet pussy");
                }
                outputText(" greet");
                if ((flags[kFLAGS.ARIAN_VAGINA] == 0 && flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0) || (flags[kFLAGS.ARIAN_VAGINA] == 1 && flags[kFLAGS.ARIAN_COCK_SIZE] == 0))
                {
                    outputText("s");
                }
                ...
                if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
                {
                    if (flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0)
                    {
                        outputText("cock spews its load");
                    }
                    else
                    {
                        outputText("twin cocks spew their loads");
                    }
                    outputText(" on the bedsheets");
                    if (flags[kFLAGS.ARIAN_VAGINA] > 0)
                    {
                        outputText(" and [arian eir] ");
                    }
                }
                if (flags[kFLAGS.ARIAN_VAGINA] > 0)
                {
                    outputText("contracting pussy paints your lower body with lizan femcum");
                }
                ...
                if (player.balls == 0)
                {
                    outputText("base of your spine");
                }
                else
                {
                    outputText("depths of your balls");
                }
            ...
            else
            {
                //Very Loose, sensitive enough to make Arian cum just from insertion. (Feels better than anything else. Yep, Arian really becomes a buttslut at this point){
                    ...
                    if (flags[kFLAGS.ARIAN_VAGINA] > 0)
                    {
                        outputText("wet gash is positively leaking lizan juices");
                    }
                    if (flags[kFLAGS.ARIAN_VAGINA] > 0 && flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
                    {
                        outputText(" and [arian eir] ");
                    }
                    if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
                    {
                        outputText("erect cock is already leaking pre like a sieve");
                    }
                    ...
                    if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
                    {
                        outputText("his cock throbs and shoots rope after rope of cum onto the bedsheets");
                    }
                    ...
                    if (flags[kFLAGS.ARIAN_VAGINA] > 0)
                    {
                        outputText("pussy squirting juices against your [legs] with each shuddering impact of your [hips]");
                    }
                    ...
                    if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
                    {
                        outputText("  [arian Eir] throbbing cock flexes and shoots one last rope of lizan-cum against your belly.");
                    }
                    ...
                    if (flags[kFLAGS.ARIAN_ASS_CHAT] == 0)
                    {
                        ...
                        if (!arianFollower())
                        {
                            outputText("back to Tel'Adre's streets, and from there back to camp.");
                        }
                        else
                        {
                            outputText("out of [arian eir] tent.");
                        }
    ...
    private function getBlownByArian() : Void
    {
        ...
        if (flags[kFLAGS.ARIAN_HAS_BLOWN] == 0)
        {
            ...
            {
                outputText("  You notice Arian's exposed shaft slowly hardening at your invitation.");
            }
        ...
        if (player.cumQ() >= 1500)
        {
            outputText(", in fact, you're amazed [arian ey] didn't pop with how much [arian ey] drank");
        }
        ...
        if (player.cumQ() >= 1000)
        {
            outputText(" which, considering the fact [arian ey] looks like a mother ready to birth a toddler from the size of [arian eir] belly, isn't as easy as you'd think");
        }
        ...
        if (player.cumQ() >= 250)
        {
            outputText(", [arian eir] gut audibly sloshing as the cum inside is churned by the impact");
        }
        outputText(".");
        ...
        if (!arianFollower())
        {
            outputText(" back to camp, stopping only to notify Laika and Boon that Arian is sleeping.");
        }
        else
        {
            outputText(" out of Arian's tent.");
        }
    ...
    private function penetrateArian() : Void
    {
        ...
        if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
        {
            outputText(arianMF("maleherm.", "dickgirl."));
        }
        else
        {
            outputText("girl.");
        ...
        else if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
        {
            ...
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            {
                outputText("s peeking out of their");
            }
            else
            {
                outputText(" peeking out of its");
            }
        ...
        if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
        {
            ...
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            {
                outputText("s twitch as they spurt ");
            }
            else
            {
                outputText(" twitches as it spurts ");
            }
        ...
        if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
        {
            ...
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            {
                outputText("twin cocks throb");
            }
            else
            {
                outputText("cock throbs");
            }
        ...
        if (player.hasVagina())
        {
            outputText(" A wash of fluids spills down your body from your cunt as it spasms in release and y");
        }
        else
        {
            outputText("Y");
    ...
    private function getButtWreckedByArian() : Void
    {
        ...
        else
        {
            ...
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            {
                outputText("s peek out of their");
            }
            else
            {
                outputText(" peeks out of its");
            }
        ...
        if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
        {
            outputText("  [arian Eir] other shaft twiches and sprays your " + ((player.tailType == Tail.NONE) ? "back" : "tail") + ".");
        }
        ...
        if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
        {
            outputText("stroking [arian eir] other cock and sliding it between your ass cheeks, ");
        }
        ...
        if (flags[kFLAGS.ARIAN_ANAL_XP] > 66)
        {
            outputText("eager ");
        }
        ...
        {
            outputText(" practically jump back to full mast, even surprising you with Arian's eagerness.");
        }
        ...
        if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
        {
            outputText("sliding [arian eir] neglected prick through your ass-cheeks, ");
        }
        ...
        if (player.hasVagina())
        {
            outputText("cunt dripping with arousal");
        }
        if (player.hasCock() && player.hasVagina())
        {
            outputText(" and ");
        ...
        //Would look weird without Genderless text.{
            outputText("null body shiverring with pleasure");
        }
        ...
        if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
        {
            outputText(", as well as your butt");
        }
        outputText(", as Arian climaxes");
        if (flags[kFLAGS.ARIAN_VAGINA] > 0)
        {
            outputText(", [arian eir] own pussy clenching and drizzling juices on the matted sheets beneath the two of you.");
        }
        else
        {
            outputText(".");
        ...
        if (player.hasCock())
        {
            outputText("  [EachCock] spasms and shoots rope after rope of cum, painting Arian's [arian chest] white, as well as the groaning lizan's face.");
        }
        if (player.hasVagina())
        {
            outputText("  Your [pussy], though unattended, joins in the perverted display, leaking copious amount of fluids on top of the prone lizan.");
        }
    ...
    private function suckAriansDick() : Void
    {
        ...
        if (flags[kFLAGS.ARIAN_COCK_SIZE] > 3)
        {
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            {
                outputText("exposed cocks hardening to full mast");
            }
            else
            {
                outputText("exposed cock hardening to full mast");
            }
        }
        else
        {
            ...
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            {
                outputText("s emerging from their");
            }
            else
            {
                outputText(" emerging from its");
            }
        ...
        if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
        {
            outputText("twin shafts tremble and throb against your hand; pre quickly forming on the tips only to slowly slide down the bulbous surface of Arian's lizan pricks");
        }
        else
        {
            outputText("shaft trembles and throbs against your hand; pre quickly forming on the tip only to slowly slide down the bulbous surface of Arian's lizan prick");
        }
        ...
        if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
        {
            outputText(", oblivious to the second cock spewing cum all over your [chest] in your eagerness");
        }
        outputText(".  ");
        ...
        if (flags[kFLAGS.ARIAN_VAGINA] > 0)
        {
            outputText(" and tickle [arian eir] cunt with a finger");
        }
    ...
    {
        clearOutput();
        if (flags[kFLAGS.ARIAN_DOUBLE_COCK] != null)
        {
    ...
    private function getFuckedScene(dp : Bool = false) : Void
    {
        clearOutput();
        flags[kFLAGS.ARIAN_VIRGIN] += 1;
        arianHealth(3);
        ...
        if (flags[kFLAGS.ARIAN_COCK_SIZE] < 3)
        {
            outputText("slit, and think of what it's hiding in there");
        }
        else
        {
            outputText("exposed cock");
        ...
        if (player.isBiped())
        {
            outputText("spread your [legs] and ");
        }
        ...
        if (dp)
        {
            outputText(" twin shafts immediately fill");
        }
        else
        {
            outputText(" shaft immediately fills");
        }
        ...
        {
            outputText(", looping your tail around [arian eir] waist");
        }
        outputText(".");
        ...
        else
        {
            ...
            if (player.hasLongTail())
            {
                outputText(", further encouraging [arian em] to do so, by looping your tail around [arian eir] waist");
            }
        ...
        if (dp)
        {
            outputText("twin bulbous shafts feel");
        }
        else
        {
            outputText("bulbous shaft feels");
        }
    ...
    private function arianDocking() : Void
    {
        ...
        if (flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0)
        {
            outputText(" brushing up against [arian eir] own lizan pecker");
        }
        else
        {
            outputText("sandwiched between [arian eir] two lizard dicks");
        }
        ...
        if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
        {
            outputText("pair of cummy masts emerge");
        }
        else
        {
            outputText("cummy mast emerges");
        }
    ...
    private function giveArianAnItem() : Void
    {
        ...
        {
            addButton(0, consumables.P_DRAFT.shortName, giveIncubusDraftToArian).disableIf(!player.hasItem(consumables.P_DRAFT), "You don't have any.");
            addButton(1, consumables.P_S_MLK.shortName, succubiMilkForArian).disableIf(!player.hasItem(consumables.P_S_MLK), "You don't have any.");
            addButton(2, consumables.LACTAID.shortName, giveArianLactaid).disableIf(!player.hasItem(consumables.LACTAID), "You don't have any.");
            addButton(3, consumables.REDUCTO.shortName, giveArianReducto).disableIf(!player.hasItem(consumables.REDUCTO), "You don't have any.");
            addButton(4, consumables.REPTLUM.shortName, giveArianReptilum).disableIf(!player.hasItem(consumables.REPTLUM), "You don't have any.");
            if (arianFollower() && flags[kFLAGS.ARIAN_SCALES] != 0)
            {
                if (flags[kFLAGS.ARIAN_SCALES] != 1)
                {
                    addButton(5, useables.VIALCLE.shortName, giveArianScalesDyeVial1).disableIf(!player.hasItem(useables.VIALCLE), "You don't have any.");
                }
                if (flags[kFLAGS.ARIAN_SCALES] != 2)
                {
                    addButton(6, useables.VIALTUR.shortName, giveArianScalesDyeVial2).disableIf(!player.hasItem(useables.VIALTUR), "You don't have any.");
                }
                if (flags[kFLAGS.ARIAN_SCALES] != 3)
                {
                    addButton(7, useables.VIALPIN.shortName, giveArianScalesDyeVial3).disableIf(!player.hasItem(useables.VIALPIN), "You don't have any.");
                }
                if (flags[kFLAGS.ARIAN_SCALES] != 4)
                {
                    addButton(8, useables.VIALRAI.shortName, giveArianScalesDyeVial4).disableIf(!player.hasItem(useables.VIALRAI), "You don't have any.");
                }
            }
        }
        if (flags[kFLAGS.ARIAN_HEALTH] < 100)
        {
            addButton(10, "Vital Tinct", arianVitalityTincture).disableIf(!player.hasItem(consumables.VITAL_T), "You don't have any.");
        }
    ...
    private function giveIncubusDraftToArian() : Void
    {
        ...
        if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
        {
            outputText("make [arian eir] cock bigger");
        }
        else
        {
            outputText("have a touch of [arian eir] old malehood back");
        }
        outputText(".");
        ...
        else if (flags[kFLAGS.ARIAN_COCK_SIZE] < 3)
        {
            ...
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0)
            {
                outputText("shaft emerges from its hiding place.  It throbs and grows, settling in its");
            }
            else
            {
                outputText("pair of shafts emerge from their hiding place.  They throb and grow, settling in their");
            }
            ...
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            {
                outputText("s back into their");
            }
            else
            {
                outputText(" back into its");
            }
            outputText(" slit.");
            ...
            else
            {
                ...
                if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
                {
                    outputText("they're so big I can't even manage to hide them,</i>\" Arian pants as the tip of [arian eir] cocks hang");
                }
                else
                {
        ...
        else if (flags[kFLAGS.ARIAN_BREASTS] > 1 && (flags[kFLAGS.HYPER_HAPPY] == null))
        {
        ...
        else if (flags[kFLAGS.ARIAN_VAGINA] > 0 && (flags[kFLAGS.HYPER_HAPPY] == null))
        {
        ...
        else
        {
            ...
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            {
                outputText("s of [arian eir] exposed shafts are");
            }
            else
            {
                outputText(" of [arian eir] exposed shaft is");
            }
    ...
    private function succubiMilkForArian() : Void
    {
        ...
        if (flags[kFLAGS.ARIAN_VAGINA] == 0)
        {
            ...
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            {
                outputText("s harden fully out of their");
            }
            else
            {
                outputText(" hardens fully out of its");
            }
        ...
        //Flat chest grows{
            ...
            if (flags[kFLAGS.ARIAN_COCK_SIZE] >= 3)
            {
                outputText(" and [arian eir] exposed shaft begins to grow hard");
            }
            outputText(".");
        }
        else if (flags[kFLAGS.ARIAN_BREASTS] < 3 && flags[kFLAGS.ARIAN_BREASTS] > 0)
        {
            ...
            else
            {
                ...
                if (flags[kFLAGS.ARIAN_COCK_SIZE] == 3)
                {
                    outputText(" and you see [arian eir] exposed shaft slowly rising to point at you");
                }
                outputText(".");
        ...
        else if (flags[kFLAGS.ARIAN_COCK_SIZE] > 1 && (flags[kFLAGS.HYPER_HAPPY] == null))
        {
        ...
        else if (flags[kFLAGS.ARIAN_COCK_SIZE] == 1 && flags[kFLAGS.ARIAN_DOUBLE_COCK] >= 1 && (flags[kFLAGS.HYPER_HAPPY] == null))
        {
    ...
    private function giveArianLactaid() : Void
    {
        ...
        else
        {
            ...
            if (flags[kFLAGS.ARIAN_COCK_SIZE] == 3)
            {
                outputText(", [arian eir] exposed cock growing hard as [arian ey] flushes with arousal");
            }
    ...
    private function giveArianReducto() : Void
    {
        ...
        if (flags[kFLAGS.ARIAN_BREASTS] > 0)
        {
            addButton(0, "Breasts", useReductoOnAriansBreasts);
        }
        if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
        {
            addButton(1, "Cock", useReductoOnArianCocks);
        }
    ...
    private function useReductoOnArianCocks() : Void
    {
        ...
        if (flags[kFLAGS.ARIAN_COCK_SIZE] == 3)
        {
            ...
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            {
                outputText("pair of exposed lizard cocks");
            }
            else
            {
                outputText("exposed lizard cock");
            }
            ...
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            {
                outputText("s slowly shrink");
            }
            else
            {
                outputText(" slowly shrinks");
            }
        ...
        else if (flags[kFLAGS.ARIAN_COCK_SIZE] == 2)
        {
            ...
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            {
                outputText("s return to their");
            }
            else
            {
                outputText(" returns to its");
            }
    ...
    private function useReductoOnAriansAsshole() : Void
    {
        ...
        else if (flags[kFLAGS.ARIAN_ANAL_XP] <= 100)
        {
            ...
            if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
            {
                ...
                if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
                {
                    outputText("s suddenly jolting from their sheathe");
                }
                else
                {
                    outputText(" suddenly jolting from its sheathe");
                }
                if (flags[kFLAGS.ARIAN_VAGINA] > 0)
                {
                    outputText(" and ");
    ...
    private function giveArianReptilum() : Void
    {
        ...
        {
            outputText("genderless crotch");
        }
        outputText(".  \"<i>Hmm, that looks much tastier than candy actually.</i>\"  [arian Ey] licks your ");
        if (player.hasCock())
        {
            outputText("cock from base to tip");
        }
        else if (player.hasVagina())
        {
            outputText("pussy all the way from the back to the tip of your [clit]");
        }
        else
        {
            outputText("your sweaty crotch");
        }
        ...
        if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
        {
            ...
            {
                outputText("perky little breasts");
            }
            else if (flags[kFLAGS.ARIAN_BREASTS] == 2)
            {
                outputText("soft breasts");
            }
            else
            {
                outputText("pillowy breasts");
            }
            if (flags[kFLAGS.ARIAN_VAGINA] > 0)
            {
                outputText(" while [arian eir] vagina does");
            }
        }
        else
        {
            outputText("[arian eir] vagina doing");
        }
        if (flags[kFLAGS.ARIAN_VAGINA] > 0)
        {
            outputText(" its best to soak [arian eir] bed with lizan femcum");
        }
        outputText(".");
    ...
    private function giveArianScalesDyeVial() : Void
    {
        ...
        if (flags[kFLAGS.ARIAN_VAGINA] == 1)
        {
            ...
            if (flags[kFLAGS.ARIAN_BREASTS] > 0 && flags[kFLAGS.ARIAN_COCK_SIZE] == 0)
            {
                outputText("the outline of her pussy and her nipples");
            }
            if (flags[kFLAGS.ARIAN_BREASTS] == 0 && flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
            {
                outputText("the outline of his pussy");
            }
            if (flags[kFLAGS.ARIAN_BREASTS] > 0 && flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
            {
                outputText("her nipples, as well as the outlines of her pussy and slit");
            }
        }
        else
        {
            outputText(", except for a bit of naked skin in the outline of his slit");
        }
        ...
        if (flags[kFLAGS.ARIAN_SCALES] == 2)
        {
            outputText("turquoise");
        }
        if (flags[kFLAGS.ARIAN_SCALES] == 3)
        {
            outputText("purplish-pink");
        }
        if (flags[kFLAGS.ARIAN_SCALES] == 4)
        {
            outputText("rainbow-colored");
        }
        outputText(" liquid.\n\n");
    ...
    private function treatCorruption() : Void
    {
        ...
        else
        {
            ...
            if (player.cor == 0)
            {
                outputText(" - in fact, you're quite sure that Arian has purified you entirely");
            }
    ...
    private function imbueTalisman() : Void
    {
        ...
        {
            outputText("place a spell in the enchanted talisman [arian ey] created for you");
        }
        else
        {
            outputText("change the spell in the talisman [arian ey] created for you");
        }
	...
	*/
        ...
        {
            addButton(0, "Shielding", arianSpellPlace, "Shielding Spell");
        }
        if (player.hasItem(consumables.GOB_ALE, 2) && player.hasItem(consumables.S_GOSSR))
        {
            addButton(1, "Immolation", arianSpellPlace, "Immolation Spell");
        }
        if (player.hasItem(consumables.ICICLE_, 2) && player.hasItem(consumables.NUMBROX))
        {
            addButton(2, "Ice Prison", arianSpellPlace, "Ice Prison Spell");
        }
        addButton(14, "Back", arianHomeMenu);
    }
...
*/
    ...
    //Sleep With Arian
    public function sleepWithArian(newl : Bool = false) : Void
    {
        if (newl)
        {
            clearOutput();
        ...
        if (flags[kFLAGS.ARIAN_VAGINA] > 0)
        {
            outputText("You can tell that [arian ey] is aroused by the little show, despite [arian eir] body doing a good job of hiding it.  ");
        }
    ...
    private function listenToMediumAnalXPArian() : Void
    {
        ...
        if (player.cor >= 66)
        {
            outputText(", despite the silly lizan being deserving of it");
        }
        outputText(".");
        if (player.hasCock())
        {
    ...
    public function wakeUpAfterArianSleep() : Void
    {
        ...
        if (player.hasCock())
        {
            ...
            else
            {
                ...
                if (!player.hasVagina())
                {
                    outputText("Did [arian ey] always know [arian ey] wanted a man to top [arian em], or is this development new to [arian em]?");
                }
                else
                {
                    outputText("Does [arian ey] really get off on having a girl with a dick of her own fuck [arian em] like a girl, hmm?");
                }
        ...
        else
        {
            ...
            else if (flags[kFLAGS.ARIAN_ANAL_XP] < 66)
            {
                ...
                if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
                {
                    ...
                    {
                        outputText(", poking ");
                        if (flags[kFLAGS.ARIAN_DOUBLE_COCK] == 1)
                        {
                            outputText("its tip");
                        }
                        else
                        {
                            outputText("their tips");
                        }
                ...
                if (flags[kFLAGS.ARIAN_VAGINA] > 0)
                {
                    outputText("moist pussy begins forming a stream of juices that gently runs its course through the inside of Arian's thighs, evaporating before hitting the floor due to the lizan's currently elevated body heat.");
                }
    ...
    public function arianLaysEggs() : Void
    {
        ...
        var itype : ItemType;
        if (flags[kFLAGS.ARIAN_EGG_COLOR] == "brown")
        {
            itype = consumables.L_BRNEG;
        }
        else if (flags[kFLAGS.ARIAN_EGG_COLOR] == "purple")
        {
            itype = consumables.L_PRPEG;
        }
        else if (flags[kFLAGS.ARIAN_EGG_COLOR] == "blue")
        {
            itype = consumables.L_BLUEG;
        }
        else if (flags[kFLAGS.ARIAN_EGG_COLOR] == "pink")
        {
            itype = consumables.L_PNKEG;
        }
        else if (flags[kFLAGS.ARIAN_EGG_COLOR] == "white")
        {
            itype = consumables.L_WHTEG;
        }
        else if (flags[kFLAGS.ARIAN_EGG_COLOR] == "rubbery black")
        {
            itype = consumables.L_BLKEG;
        }
    ...
    //As usual, nothing we write is centaur compatible.
    {
        ...
        if (flags[kFLAGS.TIMES_ARIAN_DILDOED] == 0)
        {
            ...
            {
                outputText("bought from Giacomo");
            }
            else
            {
                outputText("got from Tamani");
            }
    ...
    private function arianAppearance() : Void
    {
        ...
        if (flags[kFLAGS.ARIAN_VAGINA] == 1 && flags[kFLAGS.ARIAN_COCK_SIZE] == 0 && flags[kFLAGS.ARIAN_BREASTS] > 0)
        {
            outputText("2in");
        }
        else
        ...
        if (flags[kFLAGS.ARIAN_BREASTS] > 0 && flags[kFLAGS.ARIAN_VAGINA] == 1)
        {
            outputText("soft, feminine");
        }
        if (flags[kFLAGS.ARIAN_BREASTS] == 0 && flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
        {
        ...
        if (flags[kFLAGS.ARIAN_SCALES] <= 1)
        {
            outputText("entirely in white scales, probably a harmless remnant of [arian em] time of sickness");
        }
        if (flags[kFLAGS.ARIAN_SCALES] == 2)
        {
            outputText("in bright turquoise scales, turning a lighter shade of the same color upon reaching to [arian eir] neck, while [arian eir] inner neck has creamy-gold colored scales");
        }
        if (flags[kFLAGS.ARIAN_SCALES] == 3)
        {
            outputText("in lovely pink scales, turning a into a pinkish-white upon reaching to [arian eir] neck. Purplish-pink accents surround [arian eir] eyes and upper head");
        }
        if (flags[kFLAGS.ARIAN_SCALES] == 4)
        {
            outputText("in scales, the ones on [arian eir] head a soft, almost pinkish ruby-red, softly turning to orange-golden at [arian eir] neck. [arian Eir] inner neck has a creamy-golden coloration");
        }
        ...
        if (flags[kFLAGS.ARIAN_BREASTS] == 0)
        {
            outputText("manly pecs");
        }
        else
        {
            outputText("" + arianChestAdjective() + "breasts, each one with a 0.5 inch nipple protruding from them");
        }
        outputText(".\n\n");
        outputText("Arian’s body is thoroughly covered in scales, the ");
        if (flags[kFLAGS.ARIAN_SCALES] <= 1)
        {
            outputText("white of [arian eir] head remaining on most of the lizan’s body, except for a few brown markings on [arian eir] back and tail. [arian Eir] chest and underbelly is covered too by white scales, though a bit more greyish");
        }
        if (flags[kFLAGS.ARIAN_SCALES] == 2)
        {
            outputText("bright turquoise of [arian eir] head remaining on the lizan’s upper arms and legs, as well as on [arian eir] back and tail. [arian Eir] lower arms and legs, and [arian eir] sides have a lighter shade of turquoise. Light-golden scales cover Arian’s chest, underbelly and crotch");
        }
        if (flags[kFLAGS.ARIAN_SCALES] == 3)
        {
            outputText("purple-pink of [arian eir] head remaining on the lizan’s upper arms, legs, as well as on [arian eir] back and tail. [arian Eir] lower arms and legs, and [arian eir] sides have a lilac coloration. Pink-white scales cover Arian’s chest, underbelly and crotch");
        }
        if (flags[kFLAGS.ARIAN_SCALES] == 4)
        {
            outputText("ruby-red and orange of [arian eir] head turning into golden upon reaching the lizan’s shoulders. The same golden scales cover [arian eir] upper arms. [arian Eir] chest and underbelly have a lighter, creamy-golden set of scales. On [arian eir] back, sides and lower arms, the scales turn into a chartreuse green, later into a light blue at the lizan’s butt and legs, to end with a violet coloration at the end of the tail");
        }
        outputText(".\n\n");
        outputText("Arian’s hips are ");
        if (flags[kFLAGS.ARIAN_VAGINA] == 0 && flags[kFLAGS.ARIAN_BREASTS] == 0)
        {
            outputText("nice and well-formed, but nevertheless, boyish and average-looking");
        }
        else
        {
            outputText("soft and rounded, albeit not too thick");
        }
        outputText(". [arian Eir] butt has a rounded shape with a firm appearance that ");
        if (flags[kFLAGS.ARIAN_VAGINA] == 0 && flags[kFLAGS.ARIAN_BREASTS] == 0)
        {
            outputText("is soft, at least,");
        }
        else
        {
            outputText("has a delightful softness, just");
        }
        ...
        if (flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
        {
            ...
            if (flags[kFLAGS.ARIAN_COCK_SIZE] == 1)
            {
                outputText("6\" long and 1.5\" thick");
            }
            if (flags[kFLAGS.ARIAN_COCK_SIZE] == 2)
            {
                outputText("10\" long and 2\" thick");
            }
            if (flags[kFLAGS.ARIAN_COCK_SIZE] == 3)
            {
                outputText("14\" long and 3\" thick");
            }
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] == 0)
            {
                outputText(" penis emerges when aroused. It’s ");
            }
            if (flags[kFLAGS.ARIAN_DOUBLE_COCK] > 0)
            {
                outputText(" pair of penises emerge when aroused. They’re ");
            }
            ...
            if (flags[kFLAGS.ARIAN_VAGINA] == 1)
            {
                outputText("Under [arian eir] slit [arian ey]");
            }
        }
        if (flags[kFLAGS.ARIAN_VAGINA] == 1)
        {
