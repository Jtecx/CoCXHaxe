...
class Grayda extends Monster
{
    ...
    private function graydaHeadbutt() : Void
    {
        ...
        damage *= 3;
        player.takePhysDamage(damage, true);
        if (rand(5) == 0 && !player.hasPerk(PerkLib.Resolute))
        {
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        ...
        {
            SceneLib.graydaScene.graydaEncounterLoss2();
        }
        else
        {
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (flags[kFLAGS.THE_TRENCH_ENTERED] == 7)
        {
            SceneLib.graydaScene.graydaEncounterWin2();
        }
        else
        {
    ...
    public function new()
    {
        ...
        if (flags[kFLAGS.THE_TRENCH_ENTERED] == 7)
        {
            this.long = "Your enemy is Grayda, a 6’3 Arigean Countess. And although this is just a friendly spar, she is not to be underestimated in the slightest.";
        }
        else
        {
        ...
        this.drop = new WeightedDrop().add(consumables.SFILLET, 1).add(consumables.C_STEAK, 1).add(consumables.C_JERKY, 1).add(consumables.SIINGOT, 3).add(consumables.L_B_BAR, 3).add(consumables.EAINGOT, 3);
