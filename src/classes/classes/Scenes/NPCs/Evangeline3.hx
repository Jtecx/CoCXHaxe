...
class Evangeline3 extends Monster
{
	...
	*/public function spellCostChargeWeaponMightChargeArmorBlink() : Float
    {
        var cost : Float = 160;
        if (flags[kFLAGS.EVANGELINE_SPELLS_CASTED] >= 10)
        {
    ...
    override private function performCombatAction() : Void
    {
        if (flags[kFLAGS.EVANGELINE_LVL_UP] >= 12)
        {
            ...
            else
            {
                ...
                if (choice1 == 1)
                {
                    if (HPRatio() < .75 && (fatigue < (maxFatigue() - spellCostHeal())))
                    {
                        HealSpell();
                    }
                    ...
                    else if ((this.lust < maxLust() * 0.75) && rand(2) == 0 && (fatigue < (maxFatigue() - spellCostWhitefire())))
                    {
                        WhiteFireSpell();
                    }
