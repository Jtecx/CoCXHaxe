...
class Holli extends Monster
{
    ...
    override public function postPlayerAbility(ability : CombatAbility, display : Bool = true) : Void
    {
        if (ability.hasTag(CombatAbility.TAG_DAMAGING) && ability.hasTag(CombatAbility.TAG_FIRE))
        {
            ...
            {
                lightHolliOnFireMagically(display);
            }
        }
    }
    ...
    public function escapeFailWithHolli() : Void
    {
        ...
        if (player.canFly())
        {
            outputText("beat your wings and ");
        }
    ...
    public function lightHolliOnFireMagically(display : Bool = true) : Void
    {
        if (hasStatusEffect(StatusEffects.JojoIsAssisting))
        {
            if (!hasStatusEffect(StatusEffects.HolliBurning))
    ...
    private function fuckinJamanjiFlowerDarts() : Void
    {
        ...
        {
            outputText("  Though the demon herself is blinded, the fresh eye on the flower seems more than capable of aiming for her!");
        }
    ...
    private function holliConstrictAttack() : Void
    {
        ...
        else
        {
            ...
            var damage : Int = as3hx.Compat.parseInt(10 + rand(5));
            damage = player.takePhysDamage(damage, true);
    ...
    public function waitForHolliConstrict(newScreen : Bool = true) : Void
    {
        if (newScreen)
        {
            clearOutput();
    ...
    override private function performCombatAction() : Void
    {
        ...
        else if (rand(4) == 0 && !player.hasStatusEffect(StatusEffects.HolliConstrict))
        {
