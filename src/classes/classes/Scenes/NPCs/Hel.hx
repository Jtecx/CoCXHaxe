...
class Hel extends Monster
{
    ...
    private function helAttack() : Void
    {
        ...
        {
            damage = as3hx.Compat.parseInt((str + weaponAttack) - rand(player.tou / 2) - player.armorDef / 2);
            //No damage
            if (damage <= 0)
            {
                damage = 0;
    ...
    private function helAttack2() : Void
    {
        ...
        {
            outputText("The salamander rushes at you, knocking aside your defensive feint and sliding in past your guard.  She lashes out at your feet with her tail, and you can feel the heated wake of the fiery appendage on your ensuing fall toward the now-smouldering grass. ");
        }
        if (damage > 0)
        {
    ...
    //FAIL
    {
        ...
        else
        {
            ...
            var lust : Float = 20 + rand(10) + player.effectiveSensitivity() / 10 + rand(player.lib / 20);
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (player.hasStatusEffect(StatusEffects.RiverDungeonA))
        {
            SceneLib.dungeons.riverdungeon.defeatedByMistSalamander();
        }
        else if (pcCameWorms)
        {
    ...
    public function new()
    {
        ...
        if (player.hasStatusEffect(StatusEffects.RiverDungeonA))
        {
            ...
            this.drop = new ChainedDrop().add(weapons.SCIMITR, 0.3).add(consumables.SALAMFW, 0.7).add(useables.PCSHARD, 1);
        }
        else
        {
