...
class UrtaHeatRut extends NPCAwareContent
{
    private function urtaQuestDone() : Bool
    {
    ...
    private function approachSoberUrtaHeatRutProc() : Void
    {
        clearOutput();
        urta.urtaSprite();
        ...
        if (player.inHeat && player.inRut && player.hasCock() && player.hasVagina())
        {
            outputText(" Males, females and herms alike stare at you with a mixture of puzzlement, longing and hostility, as if unsure whether they want to fight with you, fuck you senseless or be fucked senseless by you.");
        }
        else if (player.inRut && player.hasCock())
        {
            outputText(" Males stare at you with a hint of challenge in their eyes, whilst females eye you attentively. Herms don't seem sure whether they want to fight you or fuck you.");
        }
        //(Player in heat
        else if (player.inHeat && player.hasVagina())
        {
            outputText(" Males, and individuals who look like females at first, until you spot the telltale bulge of at least one cock in garments or hanging freely between their legs, stare at you with obvious desire as you go, more than a few erections starting to form in your wake.");
        }
        ...
        if (urtaLove())
        {
            outputText(" she opens her arms to embrace you, and smiles.");
        }
        else
        {
            outputText(" she smiles when she sees you, but then gives the air a sniff and immediately seems to gain more color to the skin under her fur.  \"<i>Uh, I, I see.</i>\"  You sit down next to her, but even though she isn't even suggesting anything yet, you already find it hard to control yourself.");
        }
        
        if (player.inRut && player.hasCock())
        {
            outputText("  Urta is right next to you, and she has already proven herself a good lay");
        }
        else
        {
            outputText("  You keep reminding yourself what Urta is packing between her legs and how much precious semen it can give, and can't help but want to try that now");
        }
        ...
        if (player.inHeat && player.hasVagina())
        {
            outputText("  You... smell good, so I at least hope you're not here to tease me.");
        }
        outputText("</i>\"  She leans closer to you, and ");
        if (player.inRut && player.hasCock())
        {
            outputText("your eyes are quickly drawn to her full breasts as she whispers something to you. You're too hypnotized to care.");
        }
        else
        {
            outputText("you lose yourself in your imagination about having Urta take you raw and pump you full of sperm.  There's a faint voice you hear, and you realize it's Urta's as you snap back to reality.");
        }
        ...
        if (urtaLove())
        {
            outputText("your adorable, hot vixen");
        }
        else
        {
            outputText("someone you know and trust");
        }
        outputText(", right?</i>\"");
        ...
        if (player.inRut && player.hasCock())
        {
            outputText(" focus is on the hips of the fox-morph");
        }
        else
        {
            outputText(" focus on catching a glimpse of that amazing cock, and you keep imagining what she can do with it");
        }
        outputText(".  Your aggressive hormonal state demands that you ");
        if (player.inRut && player.inHeat && player.hasCock() && player.hasVagina())
        {
            outputText(" put a baby in her, then let her put a baby in you");
        }
        else if (player.inRut && player.hasCock())
        {
            outputText(" put a baby in her so she can push it out of those beautiful hips");
        }
        else
        {
            outputText(" put that beautiful big cock into your needy [vagina]");
        }
        //[If Urta's fertility quest wasnât done]
        if (!urtaQuestDone())
        {
            outputText(", even if something keeps reminding you that's impossible.");
        }
        else
        {
            outputText(".");
        ...
        if (urtaLove())
        {
            outputText("  I'm really thankful you managed to control yourself and see me with this.  You know that if you need me, I'm always ready to help, love.</i>\"");
        }
        else
        {
            outputText("  I'm really flattered you thought of me when in such a state!</i>\"");
        }
        ...
        else if (player.inRut && player.hasCock())
        {
            ...
            if (player.hasVagina())
            {
                outputText("  Your [cocks] and [vagina] are filling your clothes with mixed juices");
            }
            else
            {
                outputText("  Your [cock biggest] begins to seep precum into your clothes");
            }
            ...
            if (player.hasVagina())
            {
                outputText(" and your pussy dripping and eager");
            }
            outputText(".");
            menu();
    ...
    @:allow(classes.scenes.nPCs)
    private function approachDrunkenUrta() : Void
    {
        clearOutput();
        urta.urtaSprite();
        outputText("As you walk over to the table where your foxy herm-lover sits, you think you see eyes following you, your presence attracting an unusual amount of attention.");
    ...
    private function notInterestedInUburDrunkUrtaRuts() : Void
    {
        ...
        {
            outputText("Your nose does seem to pick up that Urta was and still is getting unusually aroused behind you.  ");
        }
        if (player.inRut && player.hasCock())
        {
            outputText("[EachCock] twitches in protest at you leaving a willing lay behind.  ");
        }
        ...
        {
            outputText("You instinctively taste the air, and Urta suddenly seems utterly delicious. ");
        }
        //Rut:
        if (player.inRut && player.hasCock())
        {
            outputText("Your [cocks] hardens in your pants as you still force yourself to leave.");
        }
        //[Medium lust increase]
        else
        {
            outputText("Your condition does seem kind of protesting against leaving such a willing lay behind, but it isn't time to be indulging it.");
        }
    ...
    private function interestedInUburDrunkurtaRuts() : Void
    {
        ...
        if (player.inRut && player.inHeat && player.hasCock() && player.hasVagina())
        {
            ...
            if (!urtaLove())
            {
                outputText("under her skirt");
            }
            else
            {
                outputText("through her quickly-soaked dress");
            }
    ...
    private function drunkUrtaIntroPartDuex(chosenSex : Int = 1, newl : Bool = true) : Void
    {
        if (newl)
        {
            clearOutput();
        ...
        if (player.hasBalls() && chosenSex == 1)
        {
            outputText("your [balls], rolling them in her hand and squeezing a bit too roughly.");
        }
        else if (player.hasVagina())
        {
            ...
            if (chosenSex == 1)
            {
                outputText("the very same state [eachCock] seems to be in so often recently, ");
            }
            outputText("erect and throbbing.");
        }
        else
        {
            outputText("your [hips], stroking along them towards [eachCock] in an agonizing display of teasing.");
        }
        ...
        if (chosenSex == 1)
        {
            outputText(", and [eachCock] already gets hard in your [armor].");
        }
        else
        {
            outputText(", and you feel the inside of your [armor] rapidly slickened by your [vagina]'s excitement.");
        }
        ...
        if (player.inRut && player.inHeat && player.hasCock() && player.hasVagina())
        {
            outputText("and she proceeds to maul her breasts, pre-cum seeping copiously from her cock and her snatch drooling over her balls and onto the floor.");
        }
        else if (chosenSex == 1)
        {
            outputText("but she doesn't seem to mind it for the moment.  Rather, the fur between her thighs appears to be rapidly soaking.");
        }
        else
        {
            outputText("which she seems more than intent to push into you, although you do see a bit of female lubricant sliding down her balls.");
        }
        ...
        if (chosenSex == 1)
        {
            addButton(0, "Next", sceneHunter.selectFitNofit, drunkUrtaRidesARutPCsCock, sateRutWithDrunkUrtaWithHugeDick, urta.urtaCapacity());
        }
        else
        {
            addButton(0, "Next", drunkenUrtaFucksPCInHeat);
        }
    }
    ...
    private function watchDrunkRuturtaJerkoff() : Void
    {
        ...
        if (!urtaLove())
        {
            outputText("pulls off her skirt");
        }
        else
        {
            outputText("roughly tears away her dress from the waist down");
        }
    ...
    private function keepTeasingDatHornyFawkes() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText("lewdly cupping and fondling your [balls], ");
        }
        ...
        {
            outputText("what passes for knees on your [legs]");
        }
        else
        {
            outputText("knees.");
    ...
    //If she's Lover Urta, her vaginal capacity increases to cock area 66 (or at least it's what we planned).
    {
        ...
        {
            outputText(" by shoving your [balls] out of the way");
        }
        ...
        {
            outputText("  Your hungry gash eagerly welcomes her, Urta easily sliding herself home.");
        }
        else if (player.vaginalCapacity() > 60)
        {
            outputText("  You feel it stretch you in the most pleasuring of ways, filling you with warm, throbbing flesh as she pushes it deeper and deeper.");
        }
        else
        {
            outputText("  You wince in pain at being stretched so much, but you grow wetter and wetter as Urta slowly pushes herself inside your depths, your [vagina] slowly getting wetter and warmer, accepting the intruder.  Pleasure starts to override pain and you let out a gasp.");
        }
        ...
        {
            outputText("Urta's erection seems completely at home in your depths, completely embraced by the wet walls of your throbbing [vagina].");
        }
        else
        {
            outputText("You moan again, feeling Urta's member stretch, harden and throb, blood running through it.  You feel complete, and your heat is ignited further by the sensation of something so wonderful and big filling you.");
        }
        ...
        {
            outputText("clitoris, ");
            if (player.clitLength < 2.5)
            {
                outputText("rubbing the hard, stimulated sensitive spot between her fingers");
            }
            else
            {
                outputText("running her fingers over your entire big clit before stroking it like she'd do with a cock");
            }
        }
        ...
        if (player.hasCock())
        {
            outputText(" Cum rushes out of [eachCock] and onto the soft fur of Urta's abdomen, then starts leaking out slowly in time with her thrusts.");
        }
        outputText(" Your cunt clamps down tighter onto Urta as you orgasm,");
        if (player.wetness() >= 4)
        {
            outputText(" pussy juice shooting out around her member, coating Urta's fur and soaking her balls and the base of her cock.");
        }
        else
        {
            outputText(" love juices flowing liberally out of you, giving the skin of Urta's cock a visible shine and spreading onto her balls.");
        }
        ...
        if (player.wetness() >= 4)
        {
            outputText("shooting out all over the place");
        }
        else
        {
            outputText("running down her member and the crack of your [butt], coating the two of you in the proof of your passion");
        }
        outputText(".");
        ...
        if (player.hasCock())
        {
            outputText("your own member responding alike to Urta's, shooting out semen, ");
        }
        outputText("your [vagina] ");
        if (player.wetness() >= 4)
        {
            outputText("once again setting off to coat Urta in your juices.");
        }
        else
        {
            outputText("squeezing on her, trying to milk her of her seed.");
        }
        ...
        if (!urtaLove())
        {
            outputText("You're still not satisfied?</i>\"");
        }
        else
        {
            outputText("You want to keep going, darling?  I want to make you satisfied, so I'll spare all the time I can!</i>\"");
        }
        ...
        if (player.hasCock())
        {
            outputText("[eachCock] shoots its seed onto the floor of the alley, helplessly");
        }
        else
        {
            outputText("your [vagina] squeezes and tugs on her shooting member, leaking its own juices all over her member, the intensity of the orgasm shaking you right into your core");
        }
        outputText(".");
        ...
        if (!urtaLove())
        {
            outputText("If only I had you by hand every time I need to blow a load.");
        }
        else
        {
            outputText("Your pussy is absolutely amazing, sweetheart: I hope I've helped you settle down?  Is your heat sated for the moment?");
        }
        outputText("</i>\"");
        ...
        if (!urtaLove())
        {
            outputText("Sure thing, I'm here to help,</i>\" she responds with a wink.");
        }
        else
        {
            outputText("I'm here if you need me, love.  Whatever makes you happy.  You've helped me with my own problems so much already, so I owe you at least one.  Not to mention, making love to you feels absolutely amazing!</i>\"");
        }
    ...
    private function sateRutWithSoberUrtaButHuegDicked() : Void
    {
        ...
        if (!urtaLove())
        {
            ...
            if (player.cumQ() <= 50)
            {
                outputText("spattering spooge onto Urta's face");
            }
            else if (player.cumQ() <= 250)
            {
                outputText("splashing her face and tits with your cum");
            }
            else
            {
                outputText("soaking the shocked fox-morph in your jism");
            }
            outputText("; though you still feel the urge to breed fertile females, your lust is momentarily sated.  Urta wipes ");
            if (player.cumQ() >= 500)
            {
                outputText("halfheartedly ");
            }
        ...
        else
        {
            ...
            if (player.cumQ() <= 50)
            {
                outputText("She swallows it all without complaint, licking her lips clean of any stray droplets once you remove it from her mouth.");
            }
            else if (player.cumQ() <= 250)
            {
                outputText("Her cheeks bulge at the amount you pump into her, but she's able to swallow with an admirable amount of control, gasping for breath.");
            }
            else
            {
                outputText("Rivulets of cum burst from between her lips, but she stubbornly sucks and swallows as much as she can, her stomach starting to bulge from the sudden influx of spooge.  When you stop, she is drooling cum and gasping for breath, but she meets your eyes with pride, even as she gives a wet belch.");
            }
    ...
    private function sateRutWithSoberUrta() : Void
    {
        ...
        if (urtaLove())
        {
            outputText("  \"<i>You're the only one who has ever accepted every part of me,</i>\" she tells you, one hand brushing affectionately against your cheek.");
        }
        ...
        if (player.hasVagina())
        {
            outputText(", trigging a mutual explosion from your [vagina]");
        }
        ...
        if (urtaLove())
        {
            outputText("blows you a kiss with a flirty wink of her eye and then ");
        }
        ...
        if (urtaLove())
        {
            outputText("  Her hips deliberately sway in a mesmerizing fashion, tail flicking from side to side to give you a tantalizing grip of her still-dripping snatch and the now-flaccid prick underneath.");
        }
        ...
        else
        {
            ...
            if (!urtaLove())
            {
                outputText("\"<i>And you had better learn to control yourself, " + player.mf("mister", "missy") + ".");
            }
            else
            {
                outputText("\"<i>But you should learn to control yourself lover-" + player.mf("boy", "girl") + ".");
            }
    ...
    private function drunkenUrtaFucksPCInHeat() : Void
    {
        ...
        if (!urtaQuestDone())
        {
            outputText("  You know that she can't give you babies... but, honestly, at this moment, you couldn't care less. Her wonderful stallion cock is there and ready and it's all for you.");
        }
        else
        {
            outputText("  You dimly recall that Urta could be virile, but she's right there, her huge mare-cock just dripping to plunge into your ripe [vagina] and knock you up.  You don't care anymore; you want her babies!");
        }
        ...
        if (player.clitLength < 2.5)
        {
            outputText("playing with your [clit]");
        }
        else
        {
            outputText("lapping around your huge pleasure-button, slurping it into her mouth where she suckles from it");
        }
        outputText(", and making you thrash and scream until you think you're going to cum from the pussy-eating alone.  ");
        if (player.wetness() >= 4)
        {
            outputText("As if to emphasize this, your squelching, slurping pussy suddenly sprays fem-cum all over Urta's face, making her pull herself free with a shocked shake of her head.");
        }
        else
        {
            outputText("As if sensing your impending orgasm, Urta removes her head from your [vagina], giving you a cocky grin and shaking her head.");
        }
        ...
        if (player.hasCock())
        {
            outputText("nor even your " + multiCockDescriptLight());
        }
    ...
    private function sateRutWithDrunkUrtaWithHugeDick() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText("against your own [balls]");
        }
        else if (player.hasVagina())
        {
            outputText("against your [clit]");
        }
        ...
        {
            outputText("compete with minotaurs or something?");
        }
        else
        {
            outputText("set a world record or something?");
        }
        outputText("</i>\"\n\n");
        ...
        if (player.cumQ() <= 50)
        {
            outputText("You cum all over your [chest] as Urta blows thick ropes of vixen-spunk all over your cock and upper body, a single shot landing on your lips.");
        }
    ...
    private function drunkUrtaRidesARutPCsCock() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText("feel your balls churning");
        }
        else
        {
            outputText("recognize the tingling sensation from your " + cockDescript(x) + " to your spine");
        }
        ...
        if (player.cumQ() <= 50)
        {
            outputText("you unload your thick spunk into her pussy");
        }
        else if (player.cumQ() <= 250)
        {
            outputText("you erupt inside her, filling her with wave after wave of warm, thick semen");
        }
        else
        {
            outputText("you blow massive surges of spunk inside her continuously.  The fox's stomach expands, rivulets of semen running down onto the table as she continues her ride on top of you");
        }
        ...
        if (player.hasKnot(x))
        {
            outputText("You slowly feel the blood flow to your erect penis, and the knot at the base of it seems to start to expand");
        }
        else
        {
            outputText("You don't mind staying like this for much, much longer, and the hermaphrodite on top of you seems to share that point of view");
        }
        ...
        if (player.hasKnot(x))
        {
            outputText("knot surge with more blood and your erection twitch, ensuring that you're completely trapped inside her until you're empty.");
        }
        else
        {
            outputText("tip expand as you're getting ready to blow another load.");
        }
        ...
        else if (player.cumQ() > 1500)
        {
            ...
            else if (player.hasVagina() && player.clitLength >= 3.5)
            {
                outputText("cock-like clit");
            }
            else if (player.hasVagina())
            {
            ...
            {
                outputText("inside of your thighs");
            }
        ...
        if (player.hasBalls())
        {
            outputText("  Your [balls] don't seem to be completely empty yet");
        }
        else
        {
            outputText("  It doesn't appear to be the end of what your body can produce");
        }
        ...
        {
            outputText(" the inside of your thighs");
        }
        else if (player.hasBalls())
        {
        ...
        if (!urtaLove())
        {
            outputText("intense, unadultered lust");
        }
        else
        {
            outputText("possessiveness and a hungry, but somehow warm expression");
        }
        outputText(".");
        ...
        if (urtaLove())
        {
            outputText("  She suddenly seems a bit shy and surprised, before saying, \"<i>Make sure to remind me I said that later....</i>\"");
        }
        ...
        if (!urtaLove())
        {
            outputText("Show me you're " + player.mf("the man", "the breeder") + " here!");
        }
        else
        {
            outputText("You don't want to make me feel like I'm abusing you, do you?  I know you want this, so pound me!</i>\"");
        }
        ...
        if (urtaLove())
        {
            outputText("  You can rest later!  I'll even give you a massage, so keep giving it to me!");
        }
        outputText("</i>\"");
        ...
        if (player.cumQ() <= 200)
        {
            outputText("  The table already seems like it'll be stained white by Urta's efforts alone, and your own fluid doesn't help the situation.");
        }
        else if (player.cumQ() < 1000)
        {
            outputText("  The two of you are already fucking in a small pool of your mixed semen sauce, making naughty sounds heard all over the bar, and her vagina, dripping with your fluids, only makes the pool seem bigger.");
        }
        else if (player.cumQ() >= 2000)
        {
            outputText("  Your mixed sperm is covering the table and the floor, and woe to anyone who will have to clean this place up.  Despite the ridiculous amount of fluids around, she seems set on the idea that the two of you are barely mid-way through the act.");
        }
        ...
        if (player.cumQ() >= 1000)
        {
            outputText("  Well, maybe you can, but let's make some twins!");
        }
        ...
        if (player.tallness <= 93)
        {
            outputText("making you lick, kiss and suck on her flared, wide tip");
        }
        else
        {
            outputText("staring deep into your eyes as her erection smacks you against your [chest]");
        }
        outputText(".");
        if (!urtaLove())
        {
            outputText("  She grins at your lack of resistance.");
        }
        else
        {
            outputText("  \"<i>You're mine, right?  Right now, you're mine and mine alone.  My wonderful virile breeder, all " + player.mf("his", "her") + " seed just for me.</i>\"");
        }
        ...
        if (urtaLove())
        {
            outputText("  Just a few more loads, okay?  You don't mind, lover?  I'm practically addicted, so let me just fucking overdose!");
        }
        outputText("</i>\"");
        ...
        if (player.hasBalls())
        {
            outputText(", your [balls] frantically gathering any cum that is left in them to prepare for another surge of shots into the hermaphrodite's eager nethers");
        }
        ...
        if (player.cor > 35 || flags[kFLAGS.PC_FETISH] > 0)
        {
            outputText("You tell her it was actually a great experience to have such passionate sex publicly.  She blushes at that suggestion.");
        }
        ...
        if (!urtaQuestDone())
        {
            ...
            if (urtaLove())
            {
                outputText("  If only I could... uh, I guess I should just enjoy what I can.");
            }
            outputText("</i>\"");
        }
        //[If Urta is fertile]
        ...
        if (!urtaLove())
        {
            outputText("winks at you as you get ready to leave.  I hope I'll get to play with you again soon.</i>\"");
        }
        else
        {
