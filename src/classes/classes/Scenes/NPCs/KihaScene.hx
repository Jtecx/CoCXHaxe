...
class KihaScene extends NPCAwareContent
{
    public var kihaTerrirotyEncounters(get, never) : GroupEncounter;
    ...
    private function meetKihaAndFight() : Void
    {
        clearOutput();
    ...
    private function get_kihaTerrirotyEncounters() : GroupEncounter
    {
    ...
    public function kihaExplore(clearScreen : Bool = true) : Void
    {
        if (clearScreen)
        {
            clearOutput();
    ...
    @:allow(classes.scenes.nPCs)
    private function kihaVictoryIntroduction() : Void
    {
        ...
        if (monster.lust >= monster.maxOverLust())
        {
            outputText("\"<i>You... can't make me like it!,</i>\" she shouts, struggling with all her might to keep her hands away from her genitals.  Somehow, she seems to be managing.");
        }
        //(HP)
        else
        {
            outputText("\"<i>You... you... haven't... beaten me,</i>\" she mutters, even though it's quite clear that you have.");
        }
        ...
        else
        {
            ...
            addButton(1, "Use Tail", kihaVictoryPomfTail).hint("Use her tail as a dildo.").disableIf(!player.hasVagina(), "Req. a vagina!");
            addButton(2, "FuckHerPussy", victoryDickKiha).hint("Penetrate Kiha vaginally.").disableIf(player.cockThatFits(monster.vaginalCapacity()) < 0, "Req. a cock fitting " + monster.vaginalCapacity() + " area!");
            addButton(3, "FuckHerAss", analRapuzulaKiha).hint("Penetrate Kiha anally.").disableIf(player.cockThatFits(monster.analCapacity()) < 0, "Req. a cock fitting " + monster.analCapacity() + " area!");
        }
        if (!mocking)
        {
            if (player.cor <= 40 + player.corruptionTolerance || sceneHunter.other && player.cor < 66 + player.corruptionTolerance)
    ...
    @:allow(classes.scenes.nPCs)
    private function kihaLossIntro() : Void
    {
        var useMenu : Bool = sceneHunter.uniHerms && sceneHunter.lossSelect;
        clearOutput();
        ...
        if (!(mocking || monster.lust >= 50 || sceneHunter.other && player.lib >= 80 || player.biggestLactation() >= 2))
        {
            ...
            {
                outputText("You give up on fighting, too horny to keep fighting.  Kiha strikes the ground with her axe and snorts out a puff of smoke.  \"<i>What a shameless slut!  You're lusting after me even in the heat of battle, like a common imp!  You aren't worthy to lick between my foot-claws!</i>\"\n\n");
            }
            else
            {
                outputText("You collapse, too wounded to keep fighting.  Kiha strikes the ground with her axe and snorts out a puff of smoke.  \"<i>What a wimp!  I've barely started fighting and you're already beaten!</i>\"\n\n");
            }
        ...
        else if (player.biggestLactation() >= 2 && (!useMenu || player.isGenderless()))
        {
    ...
    private function kihaMilkTitHumiliation() : Void
    {
        ...
        if (player.lust >= player.maxOverLust())
        {
            outputText("You slide to the ground, too turned on to fight and openly touching yourself.");
        }
        else
        {
            outputText("You slide to the ground, too defeated to continue to fight.");
        }
        ...
        if (player.hasCock())
        {
            outputText("not noticing the stiffening in your loins");
        }
        else if (player.hasVagina())
        {
            outputText("not noticing the scent of your growing need");
        }
        else
        {
            outputText("not noticing the flush of your skin");
        }
        ...
        if (player.cor < 33)
        {
            outputText("How far you've fallen.");
        }
        else if (player.cor < 66)
        {
            outputText("How confusing this all is.  Why can't things be simple like they were back home?");
        }
        else
        {
            outputText("Damn, that was hot.");
        }
    ...
    private function kihaRapesMen() : Void
    {
        ...
        sharedRapeIntro();
        if (!(player.isNaga() || player.tailType == Tail.LIZARD))
        {
            sceneHunter.print("You could have more fun if you had a naga/lizard tail.");
        }
        ...
        if (player.isNaga() || player.tailType == Tail.LIZARD)
        {
            outputText("\"<i>Heh, if you had half as much dexterity as you do impudence, I'd let you do this for me.  Fat chance of that,</i>\" she remarks off-handedly.  ");
        }
        ...
        if (player.hasBalls())
        {
            outputText(", resting the heel of her foot against the swell of your [balls] and applying just enough pressure to feel the cum churning within");
        }
        ...
        if (player.isNaga() || player.tailType == Tail.LIZARD)
        {
            outputText(" while your tail curls around to find your own asshole.  The grip of the dragon's nethers so intense, you barely feel the pressure of your own tail, pushing in easily to soothingly stroke your overtaxed prostate");
        }
        ...
        if (player.hasBalls())
        {
            outputText(", your [balls] practically leaping in over-burdened joy as their heavy load rushes through your flesh");
        }
        ...
        if (player.cockTotal() > 1)
        {
            ...
            {
                outputText("  Your unoccupied shafts spray ropey strands of cum all around you, jerking and straining under the weight of the dragoness.");
            }
            else
            {
                outputText("  Your unoccupied shaft sprays ropey strands of cum all around you, jerking and straining under the weight of the dragoness.");
            }
        }
    ...
    private function kihaRapesHerms() : Void
    {
        ...
        {
            outputText("Sliding backward slightly");
        }
        //LEGS:
        else
        {
            outputText("Hooking a leg under one of yours");
        }
        ...
        {
            sceneHunter.print("You really missed something not having any incubi draft...");
        }
    ...
    private function tsundereMasturbationChristmasCarol() : Void
    {
        ...
        if (player.gender > 0)
        {
            outputText("aroused genitals");
        }
        else
        {
            outputText("distinct lack of genitals");
        }
        outputText(".\n\n");
        
        //If Multi/Herm
        if (player.cockTotal() > 1 || player.gender == 3)
        {
            outputText("\"<i>W-what are you going to do, you freak of nature?  If you make me play with those, I swear... I swear I'll hit you!</i>\"");
        }
        //If Cock
        else if (player.hasCock())
        {
            outputText("\"<i>G-get that away from me!  As if I'd want anything to do with your hard, juicy cock!  Idiot!</i>\"");
        }
        //If Vagina
        else if (player.hasVagina())
        {
            outputText("\"<i>What do you think you're going to do with that pathetic, dripping pussy, huh?  I swear, if you make me l-lick that thing...</i>\"");
        }
        //If Genderless
        else
        {
            outputText("\"<i>H-huh?  Why the hell are you coming closer?  How can you possibly do anything when you're as smooth as a board down there, you loser!</i>\"");
        }
        
        outputText("  Today, however, you don't have any plans to fuck her directly");
        if (player.gender == 0)
        {
            outputText(", and seeing as you don't have anything to fuck her with, it's not like you could");
        }
        ...
        {
            outputText("- though you're hidden behind her tallness");
        }
        else
        {
            outputText("- though you're much taller than her");
        }
        ...
        if (player.hasCock())
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("a small bead of pre-cum forming from the tip and slathering her back with your juices.");
            }
            else
            {
                outputText("small beads of pre-cum forming from the tips and slathering her back with your juices.");
            }
        }
        else
        {
            outputText(nippleDescript(0) + "s against the hard ridges of her back, your tongue softly licking her smooth skin.");
        }
    ...
    private function kihaVictoryPomfTail() : Void
    {
        ...
        {
            outputText("She looks away and blushes as she says, \"<i>It's not like I'd find such tiny breasts attractive!</i>\"  Her mannerisms seem to indicate that she does.");
        }
        else if (player.biggestTitSize() <= 12)
        {
            outputText("She sighs and says, \"<i>As if I'd be impressed by udders like those!</i>\"");
        }
        else
        {
            outputText("She laughs and taunts, \"<i>How do you even walk with ridiculous hooters like those?  Go on, moo, cow!</i>\"");
        }
        ...
        if (player.wetness() <= 3)
        {
            outputText("grows wet");
        }
        else if (player.wetness() <= 4)
        {
            outputText("quickly grows slimy");
        }
        else
        {
            outputText("immediately becomes soaked from your constant drippings");
        }
        outputText(".\n\n");
    ...
    private function victoryDickKiha() : Void
    {
        ...
        if (y < 0)
        {
            sceneHunter.print("Maybe use another one for her ass?");
        }
        ...
        if (monster.lust >= monster.maxOverLust())
        {
            outputText("Just because you've aroused my body doesn't mean I'll enjoy this!");
        }
        else
        {
            outputText("Don't think I'll enjoy this!");
        }
        outputText("</i>\"\n\n");
        ...
        if (monster.lust >= monster.maxOverLust())
        {
            outputText(", rubbing it back and forth against her folds without thinking about it");
        }
        ...
        if (player.lust > 60)
        {
            outputText("only makes " + sMultiCockDesc() + " harder.");
        }
        else
        {
            outputText("quickly makes " + sMultiCockDesc() + " hard");
        }
        ...
        function multiF() : Void
        {
            ...
            if (player.isNaga() || player.tailType == Tail.LIZARD || z >= 0)
            {
                ...
                if (z >= 0)
                {
                    outputText(", plunging the head of your cock in and out of her throat.");
                }
                outputText("\n\n");
            }
            else
            {
                sceneHunter.print("Check failed: Naga/Lizard tail OR tentacock!");
            }
            ...
            if (player.isNaga() || player.tailType == Tail.LIZARD || z >= 0)
            {
                outputText(" gasping mouth,");
            }
            outputText(" and tight anus with waves of white.");
            if (player.cumQ() >= 500)
            {
                outputText("  Her belly distends before long, stuffed by the heavy loads of cream you push out.");
            }
            if (player.cockTotal() > 2)
            {
        ...
        var singleF : Void->Void = function() : Void
        {
            ...
            if (monster.lust >= monster.maxOverLust())
            {
                outputText("more aroused despite her feigned indifference");
            }
            else
            {
                outputText("very aroused in spite of her desire to fight you");
            }
            ...
            if (player.cumQ() >= 500)
            {
                outputText("A chubby bulge appears on her taut, defined belly, the only visible sign of your copious cum production.  ");
            }
            if (player.cumQ() >= 1000)
            {
                outputText("Even after that, thick gouts of the stuff spray out through her cock-stuffed lips, forming a puddle beneath your foe.  ");
            }
            outputText("You pull out");
            if (player.cor < 33)
            {
                outputText(", clean up, and get dressed, leaving Kiha behind to consider the effects of her arrogance.");
            }
            else if (player.cor < 66)
            {
                outputText(", wipe your cock off on her face, and get dressed, leaving Kiha to consider being a little humbler.");
            }
            else
            {
                outputText(", smear your dribbling cock all over her face, make her lick it clean, and then, get dressed.  Maybe the bitch will consider a little humility when addressing the real owner of this territory - you.");
            }
    ...
    private function kihaRapesLittleGirlsISawItOnTheNews() : Void
    {
        ...
        {
            outputText("  You are grateful for your forked tongue, which allows you to cover more of her pussy at once.");
        }
        else if (player.hasLongTongue())
        {
            outputText("  Your inhumanly long, prehensile tongue is a blessing here; you can reach into her deepest recesses, slurp your way into every nook and cranny, and generally provide a tongue-fuck that no human ever could.");
        }
        outputText("\n\n");
        ...
        if (player.wetness() >= 5)
        {
            outputText("  Your juices spray into her face like cum from a cock, your quantity of lubricants unable to resist cascading from the pressure of your orgasm.");
        }
    ...
    private function rapeKihaWithWORDS() : Void
    {
        ...
        else if (flags[kFLAGS.KIHA_TALK_STAGE] == 1)
        {
            ...
            if (silly() && flags[kFLAGS.LOTTIE_TIMES_ENCOUNTERED] > 0 && telAdre.lottie.lottieMorale() <= 33)
            {
                outputText("  Is that what it feels like to be Lottie?");
            }
        }
    ...
    private function analRapuzulaKiha() : Void
    {
        ...
        if (y < 0)
        {
            sceneHunter.print("Maybe you could use another fitting dick?");
        }
        ...
        {
            outputText("You quickly shed your [armor], letting your [cocks] out to breathe in the warm, swampy air.  Her red eyes widen slightly, but she quickly regains her composure, suddenly all confidence despite her obvious loss.  \"<i>Oh? What do you think you're gonna do with THAT, huh?  Just ravish me like some kind of sex-crazed demon? Is that your genius plan?</i>\"\n\n");
        }
        else
        {
        ...
        if (player.hasBalls())
        {
            outputText(" and threatening to crush your " + ballsDescriptLight());
        }
        ...
        if (player.cor < 33)
        {
            outputText("You move your hands from her two mounds and down her sides, resting just above her hips. Hoping that this will work, you begin to tickle the stubborn girl.  ");
        }
        else if (player.cor < 66)
        {
            outputText("Since she seems to be concentrating on your hands, you frantically try to think of a solution that doesn't involve them.  An idea forms and, deciding it's now or never, you lean forward and nibble on the tip of her ear.  ");
        }
        else
        {
            outputText("Since she seems to be concentrating on your hands at the moment, your mind races trying to find a solution that doesn't involve them.  An idea forms and, deciding it's now or never, you lean forward and slowly slide your tongue across her cheek.  ");
        }
        ...
        function multiF() : Void
        {
            ...
            {
                outputText(" another of your dicks");
            }
            ...
            if (player.cockTotal() >= 3)
            {
                if (player.cockTotal() == 3)
                {
                    outputText("; your remaining cock tarnishes the ground with your cum");
                }
                else
                {
                    outputText("; your remaining cocks tarnish the ground with your cum");
                }
            }
            else if (player.hasVagina())
            {
                outputText("; your " + vaginaDescript() + " pulses with pleasure, soaking your thighs with girl-cum as your cocks unload");
            }
            outputText(".\n\n");
        ...
        var singleF : Void->Void = function() : Void
        {
            ...
            if (player.hasVagina())
            {
                outputText("Your " + vaginaDescript() + " pulses with pleasure, but it's an empty orgasm, leaving you wishing you had something to fill that aching void.  Still, you cannot help but marvel at the superior feeling of cumming with a multitude of sexual organs.  ");
            }
    ...
    public function inquisitorRobesDiscovery() : Void
    {
        ...
        if (flags[kFLAGS.GOTTEN_INQUISITOR_ARMOR] == 1)
        {
            addButton(2, "Insight", insightTomeIsCoolTome).hint("Use Insight key word.");
        }
        else if (player.inte >= 100)
        {
            addButton(2, "Inspect", inspectInquisitorTomb).hint("Perhaps you should inspect the area before you make your decision?");
        }
        else
        {
            addButtonDisabled(2, "???");
    ...
    public function carnalityArmorIsCoolShitToo() : Void
    {
        ...
        {
            outputText(", which strikes you as unusual given your flat chest");
        }
    ...
    public function insightTomeIsCoolTome() : Void
    {
        ...
        {
            outputText(" Beyond that, the book pulses with magic, something you could certainly make use of. Power from... blood? Some sort of blood magic.");
        }
        outputText("\n\n");
    ...
    public function mishapsLunaKiha() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.CampLunaMishaps1, 3, 1);
        }
        else
        {
            player.createStatusEffect(StatusEffects.CampLunaMishaps1, 0, 0, 1, 0);
        }
