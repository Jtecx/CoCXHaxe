...
class HelSpawnScene extends NPCAwareContent
{
    ...
    public static function incestEnabled() : Bool
    {
        return CoC_Settings.debugBuild;
    }
    ...
    override public function helspawnFollower() : Bool
    {
        return flags[kFLAGS.HELSPAWN_AGE] == 3;
    }
    
    override public function helPregnant() : Bool
    {
    ...
    private function heliasAppearanceScreen() : Void
    {
        clearOutput();
        helScene.helSprite();
        ...
        if (flags[kFLAGS.HEL_BONUS_POINTS] < 150)
        {
            outputText("bright orange glow");
        }
        else
        {
            outputText("bright white glow");
        }
        ...
        //if Isabella is cool:
        if (flags[kFLAGS.HEL_ISABELLA_THREESOME_ENABLED] >= 1)
        {
            outputText("; she’s got her blue bandanna wrapped around her head, mostly obscured by her fiery hair");
        }
        ...
        {
            var _sw17_ = (helScene.pregnancy.event);            
            ...
            switch (_sw17_)
            {
                case 1, 2:
                    ...
                    {case 1:  //She's pregnant, but no special text yet  
                        break;
                    }outputText("  Hel's just starting to show a little bulge of pregnancy.");
                ...
                default:  //She's not pregnant, check if she's ready for it  
                    if (flags[kFLAGS.HEL_BONUS_POINTS] >= 150)
                    {
                        outputText("  Hel’s long, reptilian tail is currently burning white-hot, signaling her body’s ready for motherhood.");
                    }
            }
        }
    ...
    private function helSpawnsAppearanceScreen() : Void
    {
        ...
        if (flags[kFLAGS.HELSPAWN_DADDY] == 1)
        {
            outputText("chitin-black");
        }
        else
        {
            outputText("bright-red");
        }
        ...
        {
            outputText("a scale bikini, just like her mother’s");
        }
        else
        {
            outputText("a short skirt, thigh-high boots, and a sky-blue blouse, in stark contrast to her mother’s sluttier attire");
        }
        if (flags[kFLAGS.HEL_ISABELLA_THREESOME_ENABLED] >= 1)
        {
            outputText(", with a brightly-colored scarf wrapped around her neck");
        }
        ...
        {
            outputText("puppy-dog");
        }
        else
        {
            outputText("reptilian");
        }
    ...
    public function heliaBonusPointsAward() : Void
    {
        ...
        {
            outputText("You cock an eyebrow, saying that you're already alone, but she shakes her head, saying she needs somewhere private, away from prying eyes.  ");
        }
        ...
        {
            outputText(" and your [chest] and your [vagina]");
        }
        if (!player.hasCock() && player.hasVagina())
        {
            outputText(" [vagina] and [chest]");
        }
    ...
    private function heliaLoveFollowup() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText("even another woman, it seems, ");
        }
        ...
        //If HelLove = No:
        if (flags[kFLAGS.HEL_LOVE] == -1)
        {
            outputText("Even if you don't love me, you'd love our child, right?  You can fuck me full of kids, or even... even get someone else to, I don't care.  I just </i>need<i> it, [name].  More than anything else in the whole wide world.");
        }
        else
        {
            ...
            if (player.hasCock())
            {
                outputText("You'll fuck me full of kids, right?  Please?  Give me your seed, [name].  Give it all to me until I'm fucking gravid with your salamander babies.");
            }
            else
            {
                outputText("Even if you can't fuck me full of babies... you could grow a dick!  There's plenty of things in this god-forsaken world that do that. O-or if you don't want to, I bet I can get one of the fox-girls, or a minotaur, or something.  You know I can find a dick somewhere, just say the word.");
            }
        }
        outputText("  So what do you say, [name]?  Let's have a kid!</i>\"");
        menu();
        addButton(0, "Have A Kid", haveAKid).disableIf(player.cockThatFits(helFollower.helCapacity()) < 0, 
                                "Unfortunately, you're too big to squeeze inside Helia to do the business yourself.  You might need to shrink down some."
                ).disableIf(!player.hasCock(), "And how exactly are you going to do this without a dick?!").disableIf(player.isGargoyle(), "You're made of stone. You can't impregnate <b>anyone</b> anymore.");
    ...
    {
        var x1 : Int = as3hx.Compat.parseInt(player.cockThatFits(helFollower.helCapacity()) + 1);
        clearOutput();
        helScene.helSprite();
        if (!recalling)
        {
        ...
        {
            outputText("the base of your prick");
        }
        ...
        {
            outputText("supports her, pressing down on your [chest]");
        }
        else
        {
            outputText("gropes one of your tits, kneading your tender breasts under you squirm underneath her, unable to hold in your pleasured little gasps");
        }
        ...
        {
            outputText("We're going to be mothers!");
        }
        else
        {
            outputText("You're going to be a father... and me a mother!");
        }
    ...
    private function HaveAHellKidPartII() : Void
    {
        ...
        if (flags[kFLAGS.HEL_LOVE] == 1)
        {
            outputText(", and I'm so very, very glad you love me back.");
        }
        else
        {
            outputText(", even if you can't say the same.  And that's alright; it doesn't change my heart, and it never will. I love you");
        }
        outputText(".</i>\"");
    ...
    private function getAnotherDad() : Void
    {
        ...
        if (!player.hasCock())
        {
            ...
            {
                outputText("There's Miko and Mai from the bar.  Mai's said she wanted a kid, but can't take care of one... she'd probably be willing to fuck one into me!  If that's not alright, then... lemme think.  ");
            }
        ...
        if (player.statusEffectv1(StatusEffects.TelAdre) >= 1)
        {
            addButton(0, "Mai", maiWouldBeTheBestInseminator);
        }
        //[Spiderboy]
        ...
        {
            addButton(2, "I Will", haveAKid);
        }
        else if (!player.hasCock())
        {
            addButton(2, "I Will", growingDicks4Hel);
        }
    ...
    private function noKidsHel() : Void
    {
        ...
        if (player.hasCock() && player.cockThatFits(helFollower.helCapacity()) >= 0)
        {
            outputText("You couldn't if you wanted to anyway - you're too big to fit.  ");
        }
    ...
    private function sureHelGimmeSpidahBoyDetails() : Void
    {
        ...
        if (followerKiha())
        {
            outputText(", toward hotwings' old place");
        }
        ...
        {
            outputText(", though still nothing compared to you, lover mine");
        }
    ...
    private function youWantABoy() : Void
    {
        ...
        if (player.hasCock() && !player.hasVagina())
        {
            outputText("If I were a guy, I'd want a big strong son to hang out with, too.  Take him fishing, teach him how to fight the way you do... you'll make a great dad, my love.  I'm sure you will.");
        }
        else
        {
            outputText("Would be nice to have a man around here, you know?  I miss hanging around the boys back home, watching 'em strut like peacocks for every passing girl.");
        }
    ...
    private function youWantAGirl() : Void
    {
        ...
        {
            outputText("We're going to be a gaggle of tittering girls before you know it, though.  Salamanders grow up so fast, [name]...  I just hope she doesn't turn out like me, you know?  I don't know if I could stomach seeing my little girl becoming a wanton slut like her mom.  I might get jealous, after all...");
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function heliaTalkSeven() : Void
    {
        clearOutput();
        helScene.helSprite();
        ...
        if (flags[kFLAGS.HELSPAWN_DADDY] != 0)
        {
            outputText(", even if he's not your get, I can hope, you know?  If he spends enough time around you, maybe he'll take more after you than me");
        }
        outputText(".</i>\"");
        menu();
    ...
    private function encouragePregalia() : Void
    {
        ...
        if (flags[kFLAGS.HEL_LOVE] == 1)
        {
            outputText("You love Hel");
        }
        else
        {
            outputText("Hel's your friend");
        }
    ...
    private function helsLifestyle() : Void
    {
        ...
        if (flags[kFLAGS.HEL_LOVE] == 1)
        {
            outputText("and I hope that's part of why you love me");
        }
        else
        {
            outputText("even though I know that's probably part of why you can't love me back");
        }
    ...
    private function applyHelspawnName() : Void
    {
        ...
        else if (mainView.nameBox.text == "" || mainView.nameBox.text == "Hellgirl" || mainView.nameBox.text == "Kid" || mainView.nameBox.text == "Phoenix" || mainView.nameBox.text == "Savin" || mainView.nameBox.text == "Helia" || mainView.nameBox.text == "Mini-doofus")
        {
            clearOutput();
            ...
            else if (mainView.nameBox.text == "Hellgirl")
            {
                outputText("\"<i>Hey, that's copyright infringement.  Probably.</i>\"");
            }
            else if (mainView.nameBox.text == "Kid")
            {
                outputText("\"<i>Wow, what are you, five?</i>\"");
            }
            else if (mainView.nameBox.text == "Phoenix")
            {
                outputText("\"<i>Oh hell no.  You're not naming my little girl after one of those... things!  Sorry Kiri, but still!</i>\"");
            }
            else if (mainView.nameBox.text == "Savin")
            {
                outputText("\"<i>That's a boy's name, dumbass.</i>\"");
            }
            else if (mainView.nameBox.text == "Helia")
            {
                outputText("\"<i>My favorite name!  Except it's kinda taken, love.  Don't want things to get too confusing around here, do you?</i>\"");
            }
            else if (mainView.nameBox.text == "Mini-doofus")
            {
                outputText("\"<i>Oh yeah, Kiha'll get a laugh out of that.  You ass.</i>\"");
            }
        ...
        //Easter Egg Names:
        clearOutput();
        if (flags[kFLAGS.HELSPAWN_NAME] == "Helspawn")
        {
            outputText("\"<i>That's what I was gonna call her!  Except not at all.  God dammit, seriously, [name]?</i>\"\n\n");
        }
        if (flags[kFLAGS.HELSPAWN_NAME] == "Jayne")
        {
            outputText("\"<i>That sounds like a hero's name... I like it.  A name that demands respect!</i>\"\n\n");
        }
        if (flags[kFLAGS.HELSPAWN_NAME] == "Hesper")
        {
            outputText("\"<i>Ah, the Evening Star.  She'll be the star of my life, alright...</i>\"\n\n");
        }
        if (flags[kFLAGS.HELSPAWN_NAME] == "Kiri")
        {
            outputText("\"<i>Aw, that's great, lover.  Sis'll shit her pants when she hears her niece is named after her!  Gonna get confusing, though...</i>\"\n\n");
        }
        if (flags[kFLAGS.HELSPAWN_NAME] == "Mai")
        {
            outputText("\"<i>Aw, that's great, lover.  Mai'll be honored, I know it.  And it is a great name...</i>\"\n\n");
        }
        if (flags[kFLAGS.HELSPAWN_NAME] == "Tanis")
        {
            outputText("\"<i>I don't... I don't know if I want her to have that name, lover.  It's got a lot of baggage, but... but it does my mother an honor.  More than I could ever give her.</i>\"\n\n");
        }
        //Syn:
        if (flags[kFLAGS.HELSPAWN_NAME] == "Syn")
        {
            outputText("\"Awesome.  I was hoping you'd choose that one, lover mine.</i>\"\n\n");
        }
        if (flags[kFLAGS.HELSPAWN_NAME] == "Chara")
        {
            outputText("\"<i>Awesome.  You liked my ideas, after all!</i>\"\n\n");
        }
    ...
    //HelspawnChaste and HelspawnSlutty are the new Variabls for Helspawn, indicating the ways you can morph her personality, whichever is higher at the end of the Teenage years indicates whether she gets to be a proud, chaste warrior girl fit for Paladinhood or a slutty, filthy whore of a berzerker like mom.
    {
        ...
        helScene.pregnancy.knockUpForce();
    }
    
    //Hel Talk 8 (Only while HelSpawn is still a baby)
    @:allow(classes.scenes.nPCs)
    private function heliaTalkEight() : Void
    {
        clearOutput();
    ...
    @:allow(classes.scenes.nPCs)
    private function playWithYourKid() : Void
    {
        clearOutput();
        helScene.helSprite();
    ...
    public function helSpawnGraduation() : Void
    {
        ...
        if (flags[kFLAGS.HELSPAWN_DADDY] == 2)
        {
            outputText("And the puppy ears, now more wolf-like, are a dead giveaway.  Utterly unique to her.  ");
        }
    ...
    public function helspawnDiscoversBooze() : Void
    {
        ...
        if (player.isRace(Races.CAT, 1, false))
        {
            outputText("a cat");
        }
        else
        {
            outputText("the cat you are");
        }
    ...
    public function helSpawnChoosesAFightingStyle() : Void
    {
        ...
        {
            outputText("apparently having cleared out most of your other followers for the moment, ");
        }
    ...
    private function snipermanders() : Void
    {
        ...
        {
            outputText(", even with your own lizard legs");
        }
        ...
        {
            outputText("  You hear your old alchemist friend suddenly putting up a ruckus as the sounds of breaking glass echo throughout camp.  " + flags[kFLAGS.HELSPAWN_NAME] + " looks up at you nervously, but you ruffle her fiery hair and tell her to try again. Rath is probably just fine.");
        }
        //else if Valeria/Latexy is at camp:
        else if (flags[kFLAGS.VALERIA_AT_CAMP] == 1)
        {
            outputText("  You hear a sudden yelp of pain from across camp. Valeria slithers up to you with an irritated look on her gooey face, pointing an accusing finger at an arrow sticking out of her tit.  \"<i>Dammit, [name], I'm a googirl, not a pin cushion!</i>\"  You wave her off, and tell " + flags[kFLAGS.HELSPAWN_NAME] + " to try again.");
        }
        else if (latexGooFollower())
        {
            outputText("  You hear a sudden yelp of pain from across camp.  " + flags[kFLAGS.GOO_NAME] + " slithers up to you with an irritated look on her gooey face, pointing an accusing finger at an arrow sticking out of her tit.  \"<i>Dammit, [name], I'm a googirl, not a pin cushion!</i>\"  You wave her off, and tell " + flags[kFLAGS.HELSPAWN_NAME] + " to try again.");
        }
    ...
    public function helspawnIsASlut() : Void
    {
        ...
        {
            outputText("clutch your [weapon]");
        }
        else
        {
            outputText("ball your fists");
        }
        ...
        if (player.cor >= 50)
        {
            outputText("  And by mistake you clearly mean old enough to get her ass fucked in by a cute little spider trap... you contemplate jumping in on it, but with a sigh figure it's " + flags[kFLAGS.HELSPAWN_NAME] + "'s catch.  Let her enjoy it.");
        }
        menu();
    ...
    private function helSpawnStopFucking() : Void
    {
        ...
        //If No Hel Love:
        if (flags[kFLAGS.HEL_LOVE] != 0)
        {
            outputText("  You don't want to be a whore like your mother, do you?  The girl that everyone's had a ride with.");
        }
        //if HelLove:
        if (flags[kFLAGS.HEL_LOVE] == 1)
        {
            outputText("  I'm not going to a raise a whore, " + flags[kFLAGS.HELSPAWN_NAME] + ".  Your mother's made mistakes, sure, but...</i>\" you sigh.  \"<i>");
        }
    ...
    public function helspawnsMainMenu() : Void
    {
        ...
        {
            outputText("grabbing you off your [feet] and clutching you in a tight hug");
        }
        else
        {
            outputText("throwing her arms around you and hugging you tightly");
        }
        ...
        //[Spar]
        addButton(2, "Spar", sparHelspawn).disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, "You need a good sparring ring for that.");
        //[Sex]; bipeds only, lust must be high, incest must be flagged active, and player must have a dick.
        if (incestEnabled())
        {
            if (flags[kFLAGS.HELSPAWN_INCEST] != null)
            {
                addButton(3, "Sex", sexHelspawn).hint("Oh, what is it? Probably some unique button for debug game versions!").disableIf(player.lust < 33, "Not aroused enough.").disableIf(!player.isBiped(), "You have to be biped, sorry.").disableIf(!player.hasCock(), "Req. a cock.");
            }
            else
            {
                addButtonDisabled(3, "???", "You should talk to her first.");
            }
        }
        //[Appearance]
    ...
    private function sexHelspawn() : Void
    {
        ...
        var playerCameBeforeRiding : Bool = false;  //for tracking during sex  
        sexStart();
        
        var sexStart : Void->Void = function() : Void
        //INTRO TEXT
        {
        ...
        var sexOralHelspawn : Void->Void = function() : Void
        {
            clearOutput();
            //ORAL SEX TEXT
            ...
            if (player.balls > 0)
            {
                ...
                if (player.balls == 1)
                {
                    outputText("ball, pulling it up as she plants a wet kiss on your orb. She buries her face in your sac, basking in the scent of your masculine genitalia. Soft wet lips gently tug at your ball, swirling your cum factory around in her mouth and making you grind your sac into her lips.");
                }
                else
                {
                    outputText("balls, pulling them up as she plants a wet kiss on each orb. She buries her face in your sac, basking in the scent of your masculine genitalia. Soft wet lips gently tug at your balls, swirling your cum factories around in her mouth and making you grind your sac into her lips. \n \n");
                }
            ...
            {
                sceneHunter.print("Check failed: cocks up to 3.");
            }
        ...
        var sexDoggyHelspawn : Void->Void = function() : Void
        {
            clearOutput();
            //DOGGY STYLE TEXT
            ...
            var singleF : Void->Void = function() : Void
            {
                ...
                {
                    sceneHunter.print("Get a bigger cock~");
                }
            ...
            var multiF : Void->Void = function() : Void
            {
                ...
                if (player.cocks[y].cockLength <= 8)
                {
                    outputText("You push forward, penetrating " + flags[kFLAGS.HELSPAWN_NAME] + "’s vaginal opening as your " + player.cockDescript() + " pushes deeper into her anal canal. She squeezes both of your embedded cocks with her internal muscles and looks back at you, eyes bright with desire. She wants you so bad her lip quivers as you effortlessly sink into her wet pussy.");
                }
                else if (player.cocks[y].cockLength > 8 && player.cocks[y].cockLength <= 12)
                {
                    outputText("You push forward, penetrating " + flags[kFLAGS.HELSPAWN_NAME] + "’s vaginal walls as you push deeper into her anal canal. She gasps, looking back at you with " + eyeColor() + " eyes glazed over with lust as you sink into her dripping pussy. \n \n");
                }
                else if (player.cocks[y].cockLength > 12)
                {
                    outputText("You push forward, stretching " + flags[kFLAGS.HELSPAWN_NAME] + "’s vaginal walls apart with your " + player.cockDescript() + ". She trembles as your " + player.cockDescript() + " tunnels into her. She wiggles her bottom, the size of your cock combining with the dual sensation to make her twitch. You simply hold yourself inside of her until you feel she’s able to take the pounding you’re about to give her. \n \n");
                }
        ...
        var sexAnalHelspawn : Void->Void = function() : Void
        {
            clearOutput();
            ...
            if (player.cocks[x].cockType == CockTypesEnum.DOG)
            {
                outputText("Even lost in passion as you are you’re careful to not shove your " + player.cockDescript() + " in to the knot. Merely banging the sensitive bundle of flesh against her rectal entrance is enough... for now. \n \n");
            }
        ...
        var sexRidingHelspawn : Void->Void = function() : Void
        {
            clearOutput();
            ...
            var sharedEnd : Void->Void = function() : Void
            {
                if (player.cumQ() >= 400)
                {
                    outputText("As your climax begins to diminish " + flags[kFLAGS.HELSPAWN_NAME] + " rises, and a cascade of your hot cream comes pouring out, joining the already immense pool of spooge around your [feet] as her cum-stuffed belly deflates.");
                }
                else
                {
                    outputText("As your climax begins to diminish " + flags[kFLAGS.HELSPAWN_NAME] + " rises, and a hot mixture of your cum and her juices runs down her scaly thighs in rivulets.");
                }
                player.sexReward("vaginalFluids", "Dick");
                doNext(postSexHelspawn);
            }
        ...
        var postSexHelspawn : Void->Void = function() : Void
        {
            clearOutput();
            
            if (player.cumQ() >= 400)
            {
                outputText("Both slick with sweat, cum and lady juices, the two of you collapse on the ground, ignoring the fact that you're lying in a pool of each other's fluids. You're both breathing as though you've just run a marathon. ");
            }
            else
            {
                outputText("You both collapse on the ground, breathing like you’ve just run a marathon. ");
            }
    ...
    //{Note: Talk 1 displays first and only once, followed by a random selection from the other scenes. Most of Helspawn's talk scenes involve other characters from camp, since after all, it takes a village!}
    {
        ...
        if (flags[kFLAGS.HAD_FIRST_HELSPAWN_TALK] == 0)
        {
            ...
            if (flags[kFLAGS.HELSPAWN_PERSONALITY] >= 50)
            {
                outputText("  Even if she wants to be just like mom, she shouldn't be bringing monsters home.  Some of them may be dangerous, and she's not a warrior quite like you and Helia yet.");
            }
            else
            {
                outputText("  She should know better than to bring monsters back.  She's not her mother; lust doesn't have to dominate her life like that.");
            }
            ...
            if (flags[kFLAGS.HELSPAWN_DADDY] == 0)
            {
                outputText("Of course, as her father, you could always say it's your duty to keep her happy...");
            }
            else
            {
                outputText("Of course, as her adoptive father, you could always say that you love her in a different, more intimate way.");
            }
        ...
        {
            //{Kiha must be at camp}if (temp <= 0 && followerKiha())
            {
            ...
            {
                //{Needs Rath at camp]if (temp <= 1 && player.hasStatusEffect(StatusEffects.CampRathazul))
                {
                    ...
                    {
                        outputText("Little Amily's solution was makeshift at best, and the children are beyond my reach to study.  Still.  ");
                    }
                ...
                {
                    //{Bath Slut w/ DD or HHH must be at camp}if (temp <= 2 && milkSlave() && flags[kFLAGS.MILK_SIZE] > 0)
                    {
                    ...
                    {
                        //{Isabella must be at camp}if (temp <= 3 && isabellaFollower())
                        {
                            ...
                            {
                                outputText(" and you yourself have suffered as well");
                            }
    ...
    private function helSpawnMilkHerNow() : Void
    {
        ...
        if (flags[kFLAGS.MILK_SIZE] == 2)
        {
            outputText(", already working to unfasten her shift");
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function loseSparringToDaughter() : Void
    //if Sluttymander:
    {
        ...
        if (flags[kFLAGS.HELSPAWN_PERSONALITY] >= 50)
        {
            ...
            {
                outputText("succumbing to your own lusts");
            }
            else
            {
                outputText("unable to withstand her unending hail of attacks");
            }
            ...
            if (player.weaponName != "fists")
            {
                outputText("weapon tumbling out of hand");
            }
            else
            {
                outputText("unable to even clench your fists anymore");
            }
        ...
        else
        {
            ...
            if (player.HP < 1)
            {
                outputText("recover from that ass kicking");
            }
            else
            {
                outputText("dunk your head");
            }
            outputText(".");
        }
        cleanupAfterCombat();
    ...
    @:allow(classes.scenes.nPCs)
    private function beatUpYourDaughter() : Void
    {
        clearOutput();
        ...
        if (flags[kFLAGS.HELSPAWN_PERSONALITY] >= 50 && monster.lust >= monster.maxOverLust())
        {
            ...
            //If No Incest:
            if (flags[kFLAGS.HELSPAWN_INCEST] == 0)
            {
                outputText("  You wash your hands of the defeated slut and head back to camp, leaving her to work through her tension herself.");
            }
            else
            {
        ...
        else
        {
            ...
            {
                outputText(", rubbing her many bruises");
            }
            else
            {
                outputText(", blushing brightly after your slutty display");
            }
    ...
    public function spiderBrosGift() : Void
    {
        ...
        if (isabellaFollower())
        {
            outputText(", or aunt Isabella might have knitted me a new one, I don't know");
        }
        outputText(".</i>\"");
    ...
    public function hakonAndKiriComeVisit() : Void
    {
        ...
        if (camp.companionsCount() > 1)
        {
            outputText(" and everyone else's too, much to their chagrin");
        }
        ...
        outputText("\n\n\"<i>Sure!  ");
        if (flags[kFLAGS.HELSPAWN_WEAPON] == "scimitar and shield")
        {
            outputText(championRef() + "'s been teaching me how to fight.");
        }
        else
        {
            outputText("Mom's been teaching me how to fight.");
        }
