...
class ShouldraFollower extends NPCAwareContent
{
    ...
    override public function followerShouldra() : Bool
    {
        return flags[kFLAGS.SHOULDRA_FOLLOWER_STATE] == 1;
    }
    private function shouldraSleeping(changes : Int = 0, reset : Bool = false) : Bool
    //Adjust sleep timer
    {
        ...
        if (flags[kFLAGS.SHOULDRA_SLEEP_TIMER] > 48)
        {
            flags[kFLAGS.SHOULDRA_SLEEP_TIMER] = 48;
        }
        //Is she sleeping?
    ...
    private function initialShouldersRecruitment() : Void
    {
        clearOutput();
    ...
    private function shouldraAndValaGetBigger() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_ghostGirl2);
        if (flags[kFLAGS.SHOULDRA_MET_VALA] < 1)
        {
            flags[kFLAGS.SHOULDRA_MET_VALA] = 1;
        }
    ...
    private function pureValaAndShouldraMacroMax() : Void
    {
        ...
        if (player.gender > 0)
        {
            ...
            if (player.hasCock())
            {
                outputText(", becoming firmly erect.  You happily piston your hips to grind your [cocks] along the pink-glossed walls, oozing your fluids in delight.");
            }
            else
            {
                outputText(", growing wet, though in this slick fluid, your contribution is a mere drop in the ocean.");
            }
        }
        ...
        if (player.hasCock())
        {
            ...
            if (player.cumQ() >= 1000)
            {
                outputText("  Somehow, you pour out almost as much spunk as Vala's orgasm, turning the inside of her honeypot into an off-white mess.");
            }
        }
        else if (player.hasVagina())
        {
    ...
    public function shouldraMeetsCorruptVala() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("  Your cock slips past the folds, pressed up against your stomach as the underside is massaged with the gentle tightness within.");
        }
    ...
    public function sandWitchGetsGhostly() : Void
    {
        ...
        else
        {
            ...
            {
                outputText("\"<i>You know, maybe I should give you a couple of those chest-mounted treats.  They're pretty fun,</i>\" Shouldra teases, tweaking your [nipples].  ");
            }
    ...
    private function shouldraFappinTimes() : Void
    {
        ...
        else if (sceneHunter.uniHerms)
        {
            ...
            menu();
            addButton(0, "Dick", maleMasturbationProper).disableIf(!player.hasCock(), "You don't have any.");
            addButton(1, "Pussy", girlyMasturbationWithGhosts).disableIf(!player.hasVagina(), "You don't have any.");
            addButton(2, "GetLicked", shouldraTongueLicksPCs).disableIf(!player.hasVagina(), "You don't have a pussy to lick.");
        ...
        else if (player.hasCock() && (!player.hasVagina() || rand(2) == 0))
        {
    ...
    private function genderlessShouldraMasturbation() : Void
    {
        ...
        if (player.isGenderless())
        {
            ...
            else
            {
                ...
                if (player.cor < 50)
                {
                    outputText("There isn't an answer or outcome to that question that you're in favor of, honestly.");
                }
                else
                {
                    outputText("You sure as hell wouldn't have bothered asking if you weren't.");
                }
            }
        }
        else
        ...
        if (flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0 && flags[kFLAGS.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0)
        {
            outputText("Within lies a rather rustic yet well-built chest.");
        }
        else if (flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && flags[kFLAGS.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] == 0)
        {
            outputText("The moment the light reveals what lies within, your [butthole] instinctively clenches shut.  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\" It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
        }
        else
        {
            outputText("Inside rests the ghost's familiar treasure trove of dildos and other toys you have yet to discover.");
        }
        ...
        if (player.isGenderless())
        {
            outputText("featureless groin.  \"<i>Let me take care of this, Champ,</i>\" she exclaims");
        }
        else
        {
            outputText("groin.  You can't decide whether to laugh or cry at the sight as she[if (hasCock = true)  substitutes your [cock] in place of her nose] [if (isHerm = true)  and she][if (hasVagina = true)  shuffles your [vagina] for a moment, using it as her mouth].  \"<i>I had to get dressed up for the occasion,</i>\" she jests");
        }
        ...
        {
            outputText("  Just what else did she pack in there since last you peered at its contents back at the ruins?");
        }
        ...
        if (player.ass.analLooseness < 3)
        {
            ...
            if (player.cor < 50)
            {
                outputText("..  This only worries you further.");
            }
        ...
        else
        {
            ...
            if (player.cor < 50)
            {
                outputText(", which only serves to confuse you");
            }
            outputText(".");
        }
        ...
        if (flags[kFLAGS.ZETSUKO_MET] > 0)
        {
            outputText("\"<i>Your run-in with a particular kitsune inspired me,</i>\" Shouldra teases");
        }
        else
        {
            outputText("Shouldra is too caught up in her machination to give you any hint as to what is going on, leaving you confused");
        }
        ...
        if (player.isGenderless())
        {
            outputText("Having come as close to cumming as your featureless form can manage, your entire body goes limp.");
        }
        else
        {
            outputText("The final delivery throws you over the deep-end, [if (hasCock = true) your [cock] spraying cum everywhere] [if (isHerm = true)  while ][if (hasVagina = true) your [vagina] sprays femspunk, mixing in with the otherworldly drool already coating your [legs]].");
        }
        ...
        if (flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || flags[kFLAGS.GENDERLESS_MASTURBATION_WITH_GHOST_COUNT] > 0)
        {
            outputText("You recognize it instantly as she pulls it free from her chest: the nearly foot-long floppy horse cock she impaled you on before.");
        }
        //{if SHOULDRA_GENDERLESS_FUCK_COUNT = 0}
        else if (flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0)
        {
            outputText("She holds her catch up clear for you to see; to your horror, you have clutched onto an enormous horse cock replica.  All 13 inches flop restlessly in your grip, seemingly taunting you with the foreboding future that lays before you.");
        }
    ...
    private function maleMasturbationProper() : Void
    {
        ...
        if (player.biggestCockArea() < 200)
        {
            outputText("  You stagger a bit at the unaccustomed heft of your junk altering your center of gravity, but you know that will wear off soon enough.");
        }
    ...
    private function shouldraTongueLicksPCs() : Void
    {
        ...
        //if SHOULDRA_GENDERLESS_FUCK_COUNT > 0 || event occurrence > 0
        if (flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 || flags[kFLAGS.SHOULDRA_TONGUE_LICKS_TIMES] == 0)
        {
            outputText("  The consistency of your forgetfulness while traveling this path leads you to believe that your ethereal inhabitant would rather you not come back here on your own.");
        }
        ...
        else if (flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] > 0 && flags[kFLAGS.SHOULDRA_TONGUE_LICKS_TIMES] == 0)
        {
            outputText("  \"<i>You didn't think I'd leave my most treasured possessions behind for some goblin to run into, did you?</i>\"  It's the ghost girl's chest of dildos, as well-kept as the last time you laid worried eyes on it.");
        }
        else
        {
            outputText("  Inside rests the ghost's familiar treasure trove of dildos and other toys you have yet to discover.");
        ...
        addButton(1, "Flexible", flexible).disableIf(!player.hasPerk(PerkLib.Flexibility), "Req. 'Flexibility' perk.");
        addButton(2, "LongTongue", longTong).disableIf(!player.hasLongTongue(), "...Did you read the button name?");
        ...
        var needMagic : Void->Void = function() : Void
        {
    ...
    private function shouldraYappin() : Void
    {
        ...
        //unlocked with Dream 1{
            ...
            //Clear out that talk happened and start a countdown till next dream sequence.{
                ...
                spriteSelect(SpriteDb.s_ghostGirl2);
                if (flags[kFLAGS.SHOULDRA_PLOT_LEVEL] < 4)
                {
                    flags[kFLAGS.SHOULDRA_PLOT_COUNTDOWN] = 7;
                }
                ...
                {
                    outputText("  Shouldra bumps you to the side a little as she makes her next point, \"<i>Even with your hoaky quasi-possession power, you don't really know what it's like to actually possess someone.</i>\" You look at her funny, unaware that you weren't really getting the full effect.  Not that you care!");
                }
                ...
                if (player.hasPerk(PerkLib.Incorporeality))
                {
                    outputText(" - and I mean <b>really</b> possess someone -");
                }
            ...
            //Clear out that talk happened and start a countdown till next dream sequence.{
                ...
                {
                    outputText("  A fleshy, penis-shaped bulge begins to swell out of your loins; looking up at the ghost, you realized she hasn't uttered a single word!");
                }
                ...
                if (player.gender == 0)
                {
                    outputText("burgeoning dickflesh");
                }
                ...
                if (player.gender == 0)
                {
                    outputText("Your now erect dick is leaking out pre at an alarming rate, which your possessed hands greedly lap up and distribute all along your length.  ");
                }
            ...
            //Clear out that talk happened and start a countdown till next dream sequence.{
                ...
                if (player.hasCock())
                {
                    outputText("</i>\"  Shouldra eyes your [cock] gingerly, acting as if she can stare right through your outfit at it.  \"<i>");
                }
                else
                {
                    outputText("  ");
    ...
    public function shouldraFollowerScreen() : Void
    {
        ...
        if (!shouldraSleeping())
        {
            outputText("You pulse a desire for interaction with Shouldra, summoning her head to form from your shoulder. \"<i>What's on your mind, Champ? I could've found out on my own, but I was... preoccupied.</i>\"");
        }
        ...
        addButton(1, "Sex", shouldraSexMenu).disableIf(player.lust < 33, "You're not aroused enough!");
        if (sceneHunter.mockFights)
        {
            addButton(3, "Mock Fight", mockShouldra);
        }
    ...
    private function shouldraSexMenu() : Void
    {
        ...
        //variant of genderless masturbation{
            
            addButton(1, "AnalMasturb", genderlessShouldraMasturbation).hint("Shouldra could also let you have a little anal fun, if you'd like.");
        }
        addButton(2, "VisitVala", shouldraAndValaGetBigger).hint("Vala might interest Shouldra.  You could go introduce them...").disableIf(model.time.hours < 12 || model.time.hours > 21, "Vala is not available right now.").disableIf(!flags[kFLAGS.FREED_VALA], "Maybe found someone else who might be interesting to her?", "???");
    ...
    private function shouldraGroPlus() : Void
    {
        ...
        if (player.hasBalls())
        {
            balls = groBallsBiggaGHOSTYSTYLE;
        }
        if (player.hasCock())
        {
            cock = shouldraCockBloating101;
        }
        if (player.hasVagina())
        {
            clit = shouldraGrowsYoClit;
        }
    ...
    private function groBallsBiggaGHOSTYSTYLE() : Void
    {
        ...
        {
            outputText("  You second guess your decision when your swinging cum barrels seem to impede walking.  Shouldra reassures you that the enhanced libido and baby batter supply is worth the wider gait.");
        }
    ...
    private function shouldraGivesYaSomeFukkinTeats() : Void
    {
        ...
        {
            if (transformations.NipplesFuckable.isPossible())
            {
                temp2++;
            }
    ...
    private function shouldraReductoMenu() : Void
    {
        ...
        {
            nipples = shrinkDemNipplzForYoGhost;
        }
        if (player.biggestTitSize() >= 1)
        {
            breasts = shouldraReductosYourTits;
        }
        if (player.butt.type >= 2)
        {
            butt = shrinkDatBootyForYoGhost;
        }
        if (player.hasBalls() && player.ballSize > 1)
        {
            balls = shouldraReductosYourBallsUpInsideYa;
        }
        if (player.hasCock() && player.longestCockLength() > 4)
        {
            cock = shouldraMakesCocksDisappear;
        }
        if (player.hasVagina() && player.clitLength > .25)
        {
            clit = clittyVanishingActShouldra;
        }
    ...
    private function clittyVanishingActShouldra() : Void
    {
        ...
        player.clitLength = as3hx.Compat.parseInt(player.clitLength * 100) / 100;
    ...
    private function shouldraMakesCocksDisappear() : Void
    {
        ...
        else
        {
            ...
            for (ii in 0...player.cocks.length)
            {
    ...
    private function shrinkDatBootyForYoGhost() : Void
    {
        ...
        {
            player.butt.type -= 3 + as3hx.Compat.parseInt(player.butt.type / 3);
    ...
    public function shouldersWarnings() : Bool
    //Requires hour counter to count time since last Shouldra scenes
    {
        ...
        {
            //7 days (168 hours) since last Shouldra sceneif (flags[kFLAGS.SHOULDRA_SLEEP_TIMER] == -168)
            {
                spriteSelect(SpriteDb.s_ghostGirl2);
                ...
                if (rand(3) == 0)
                {
                    ...
                    if (player.gender == 0)
                    {
                        outputText("libido seems to be going haywire");
                    }
    ...
    {
        var dick : Bool = player.hasCock() && (!player.hasVagina() || rand(2) == 0);
        ...
        if (player.biggestTitSize() >= 1)
        {
            outputText(", circling their entire lengths around your [fullChest]");
        }
        ...
        if (dick)
        {
            outputText("magically enlarged member");
        }
        else
        {
            outputText("magically gaped snatch");
        }
        ...
        if (dick)
        {
            outputText("flailing cock");
        }
        else
        {
            outputText("moist vagina");
        }
        outputText(".");
        //if cock
        if (dick)
    ...
    public function shouldraWakesUpOrPokesPCsForShitsAndGigglesIdunnoHowLongCanIMakeThisFunctionNameQuestionMark() : Void
    {
        ...
        if (player.hasPerk(PerkLib.MaraesGiftStud) || player.hasPerk(PerkLib.MaraesGiftFertility) || player.hasPerk(PerkLib.MaraesGiftProfractory) || player.hasPerk(PerkLib.MaraesGiftButtslut))
        {
        ...
        switch (select)
        {
            ...
            case 1:
                ...
                if (subSelect == 0)
                {
                    outputText("Your [cock] tingles, and your [armor] suddenly feels much tighter.  A little giggle from Shouldra reveals her intention.  Though adamant about making you as big and virile as possible, Shouldra does realize it <b>is</b> your body, and your meat shrinks at least three inches to its former glory.");
                }
                else if (subSelect == 1)
                {
                    outputText("An uncomfortable stirring in your nether regions has you stop to undo your [armor] and figure out what's the matter.  Your [cock] is wagging enthusiastically.  Shouldra's face quickly sprouts from your shoulder, \"<i>Oh look, he's happy to see you!</i>\" You pay her an irritated glance which she returns with a content snicker.  With your cock under control, you redo your [armor] and continue on your way.");
                }
                else if (subSelect == 2)
                {
                    outputText("You're distracted as you notice your [cock] snaking its way down your [leg], quickly growing larger.  You make your displeasure obvious, causing it to recoil back to its original size.  Seems as though your ghostly friend is getting restless.");
                }
                else if (subSelect == 3)
                {
                    outputText("You come to a halt as you suddenly lose control of your arms, forcing you to watch your hands dart down your [armor] to fiercely grab at your [cock].  Though you let out a growl for her to stop, it takes Shouldra a minute to return your arms to you.  She must be getting restless.");
                }
                else if (subSelect == 4 && player.horseCocks() == 0)
                {
                    outputText("There's a sudden twisting pain in your [cock], forcing you to undo your [armor] and see what's wrong.  To your dismay, a large equine dong is staring back at you, leaking profusely.  \"<i>Just testing out some new transformations, Champ,</i>\" Shouldra snickers.  You yell out again as your dick quickly turns back to normal.");
                }
                else
                {
                    outputText("One of your hands begins playing with your [cock], no doubt spurred on by your paranormal passenger.  \"<i>You know, Champ, I could make you some easy gems.  Just find some know-nothing and introduce them to The Amazing Penis, capable of astounding feats like responding to any vocal coital commands!</i>\"  Playing the part of a sideshow attraction is quite the tangent to your typical daily activities.  Probably best not to think about it.");
                }
            case 2:
                //PC with BIG COCK
                subSelect = rand(5);
            ...
            case 3:
                ...
                if (subSelect == 0)
                {
                    outputText("Your [balls] are getting warm.  They're getting... really hot.  \"<i>Don't mind me, Champ. Just... checking something out.</i>\"  You decide the best course of action would probably be not to pry any further into whatever machination the ghost girl is working on.");
                }
                else if (subSelect == 1)
                {
                    outputText("A tightness in your [armor] reveals to you your swelling [balls].  Shouldra continues to try and convince you of the importance of a virile package, but you remind her as to who's in control of this ship.  Regretfully conceding, the ghost girl returns your sac to its original proportions.");
                }
                else if (subSelect == 2)
                {
                    outputText("You suddenly fall [if (isBiped = true) to your knees][if (isBiped = false) to the ground] as your [balls] begin to thrash and vibrate wildly!  \"<i>Interesting. That was way more effective than I would have guessed.  I'll have to make a note of that,</i>\" your paranormal passenger says, ending the spell.  With a mild case of confusion, you're left to your thoughts as you go about your business.");
                }
                else if (subSelect == 3 && player.hasCock())
                {
                    outputText("You feel your [balls] quickly swell then tighten out of nowhere, forcing some of their payload out your [cock].  Suddenly your hand darts down your [armor] and delivers the errant cum directly to your mouth!  \"<i>Had some time on my hands, so I decided to try out some new recipes.  What do you think?</i>\" Shouldra asks.  Tastes... citrus-y?  That your loins are being used as an experimental factory is... disconcerting.");
                }
                else
                {
                    outputText("\"<i>Champ... you weren't thinking of impregnating anyone too soon, were you?</i>\"  You ask Shouldra what's wrong, afraid of the answer you'll hear.  The odd sensations you've felt in your [balls] are probably related.  \"<i>Well, let's just say that your sperm may be little wild cards at the moment.  I'll... get that sorted out shortly.</i>\"  On that note, a rough churning starts up down below.  Your baby batter isn't safe with this ghost around, it seems.");
                }
            //PC with boobs
            case 4:
                subSelect = rand(5);
            ...
            case 5:
                ...
                {
                    outputText("You almost don't realize your hand creeping its way into your [armor], trying to sneak a quick fingering. Looks like Shouldra is getting antsy.");
                }
                else if (subSelect == 3)
                {
                    outputText("What the... ?  Your [vagina] has started gushing fluids everywhere, absolutely soaking your [legs]!  \"<i>Don't mind me, Champ.  I'm just testing to see how far I can push this. Part of something I have in mind for later...</i>\"  No good can come of this...");
                }
                else
                {
                    outputText("You can sense Shouldra laughing to herself over something.  Though it's usually a poor idea, you decide to ask and see what's on her mind.  \"<i>I was just scheming.  Would you get too upset if I spooked your next fuck?</i>\"  As she asks, you feel your [vagina] clenching hard, giving you a clear idea of what the trickster has in mind.  There's no way that-  On second thought, maybe it could be kind of fun?");
                }
            ...
            case 6:
                ...
                if (subSelect == 0)
                {
                    outputText("While minding your business, you realize faint whispering is coming from your [cock].  Shouldra and Exgartuan are scheming something, probably something you wouldn't be a fan of.  Every so often your wang lets out a chuckle.  Maybe getting these two together wasn't in your best interest.");
                }
                else if (subSelect == 1)
                {
                    outputText("Your [cock] is wiggling around inside your [armor], making you incredibly uncomfortable.  You go to undo your outfit when the demonic dong plows its way out anyway, scaring you half to death.  Shouldra and Exgartuan just laugh, too content with one another to pay you much mind.  You never expected them to get along THIS well.");
                }
                else if (subSelect == 2)
                {
                    outputText("\"<i>So what other sorts of things can you concoct?</i>\" Exgartuan asks, shuffling around in your armor.  You're incredibly confused by the sudden question.\n\n\"<i>Honestly, that put me in the mood to make the mother nature our bitch,</i>\" you respond... Shouldra responds.  The spirit is gabbing through your mouth to your possessed pecker.  The two prattle on like this for some time.");
                }
                else if (subSelect == 3)
                {
                    outputText("Your hands slip inside your [armor], exploring your [cock].  Your fingers caress each little nub and nodule they come across.  \"<i>That's the ticket,</i>\" Exgartuan gleefully responds, getting rigid in the tight confines of your outfit.  Looks like your ghostly partner is out to make everyone happy.");
                }
                else if (subSelect == 4)
                {
                    outputText("\"<i>Why don't you help out during battles, babe?</i>\" Exgartuan asks rather bluntly.\n\n\"<i>Hey, I've offered to help out with Champ's more seductive poses like you do...</i>\"\n\nIt isn't like you have much choice in the matter as far as Exgartuan is concerned!  The two continue their discussion, belittling your apparently meager combat performance.  Maybe your possessors could actually help deal some real damage for a change!");
                }
                else
                {
                    outputText("Your [armor] has vanished!  You're butt-naked!  It's pretty obvious who's at fau-\n\n\"<i>Really, [name], you don't even need it,</i>\" Exgartuan says, surprising you.\n\nShouldra laughs, \"<i>Right, Champ?  Two sexually-charged tyrants dwelling within you and you insist on sneaking around like common rabble?  Why not let it all hang out?</i>\"\n\nYour hands caress your [cock], exciting the demonic member into attention.  You remind the couple that your outfit isn't <b>just</b> for protection, citing many other-  Oh, your [armor] is back.  Guess they don't want to hear you prattle.");
                }
            //General flavor text
            case 7:
                subSelect = rand(6);
                if (subSelect == 0)
                {
                    outputText("\"<i>Yoooo, Champ, are we gonna fuck something, or am I just gonna stagnate here?</i>\" Shouldra moans, managing to steal control of an arm long enough to give your [butt] a honk.  You sigh.");
                }
                else if (subSelect == 1)
                {
                    outputText("Out of nowhere, a colossal yawn bursts forth from your craw.  \"<i>Come on, Champ. I'm dying to try out some new stuff I've cooked up.</i>\" What is this sense of dread consuming your being?");
                }
                else if (subSelect == 2)
                {
                    outputText("It's been pretty quiet recently.  No doubt Shouldra is up to something -- she always is.  Honestly, you've forgotten what it's like to not having your body passing for a bloated something or other.");
                }
                else if (subSelect == 3)
                {
                    outputText("As you are going about your business, you swear you make out muttering emanating from somewhere on your person. It sounds like the ghost girl is reciting some magic, but you don't feel any different. This only worries you even more.");
                }
                else if (subSelect == 4)
                {
                    outputText("\"<i>Do foot fetishes still apply if it's your own foot?</i>\" Shouldra asks.  For a moment you wonder how such a feat would even be possible.  But only for a brief, forgetful moment...");
                }
                else if (subSelect == 5)
                {
                    outputText("A familiar voice works its way from the back of your mind, \"<i>Hey, Champ.  You think there's any way to make armpits sexy?  What about knees or noses?</i>\"  This line of questioning could lead to some very troubling consequences...");
                }
                else
                {
                    outputText("\"<i>I like what you did with those racks,</i>\" Shouldra says unexpectedly, her voice losing its mischievous edge for a moment.  \"<i>I'm glad you kept them.</i>\"  You both sit in silence for several moments as you consider her mood shift to tenderness, but the moment passes.  Shouldra chuckles softly.  \"<i>Just thought I'd mention.</i>\"");
                }
            ...
            case 17:
                outputText("\"<i>");
                if (flags[kFLAGS.IZMA_CHILDREN_SHARKGIRLS] + flags[kFLAGS.IZMA_CHILDREN_TIGERSHARKS] == 1)
                {
                    outputText("That shark daughter of yours is");
                }
                else
                {
                    outputText("Those shark daughters of yours are");
                }
    ...
    private function shouldraAndWormsYoureGonnaHaveABadTime() : Void
    {
        ...
        if (player.cocks[0].cockLength < 48)
        {
            outputText(" and cuts right to the chase, ballooning your [cock] into a rigid, wide, four-foot tall slab of meat");
        }
    ...
    private function kickOutShouldra4YoWormyBuddies() : Void
    {
        ...
        if (player.cocks[0].cockLength < 48)
        {
            outputText("  With that little ordeal dealt with, you now are left to decide how best to manage the time until your cock returns to its normal size.");
        }
    ...
    //unlocks after Argue 3
    {
        ...
        if (silly())
        {
            outputText("  It's a man's right to choose what's done with his body.  Not anyone else's!");
        }
    ...
    private function keepShouldraAndKickOutExgartuan() : Void
    {
        ...
        {
            doNext(keepShouldraPartIIExgartumonsUndeatH);
        }
    }
    ...
    private function keepAllTheGhosts() : Void
    {
        ...
        if (player.cumQ() >= 1000)
        {
            outputText(", trails of your cum pouring out from them");
        }
        outputText(".");
    ...
    public function shouldraBakeryIntro() : Void
    {
        ...
        //first time:
        if (flags[kFLAGS.SHOULDRA_BAKERY_TIMES] == 0)
        {
            outputText("You smirk, wondering how exactly a ghost manages to work up a hunger.  Your skepticism is met by an intense stabbing pain in your stomach, forcing you to buckle slightly from the cramping.  It lasts only a moment before it vanishes, however.  \"<i>You feel that, smartass?</i>\" The ghost girl sneers, \"<i>That's what decades of fasting will do to you.");
        }
        else
        {
            outputText("You point out that she's already had something at the bakery.  \"<i>Yeah, I did.  And now I'm hungry again.  Deal with it.");
        }
    ...
    private function feedShouldraACake() : Void
    {
        ...
        if (flags[kFLAGS.SHOULDRA_BAKERY_TIMES] > 0)
        {
            outputText("Well, nothing <i>permanent</i>, anyway.  ");
        }
    ...
    private function shouldraAndMaddieSittingInATree() : Void
    {
        ...
        if (player.tone >= 60)
        {
            outputText("well-toned");
        }
        else if (player.thickness < 50)
        {
            outputText("trim");
        }
        else
        {
            outputText("curvaceous");
        }
        outputText(" figure...");
    ...
    private function goAlongWIthShouldrasEatingSpree() : Void
    {
        ...
        if (flags[kFLAGS.SHOULDRA_BAKERY_TIMES] > 0)
        {
            outputText("\"Man, you're not sure you want that kind of impact on your figure again...  \"");
        }
        ...
        {
            outputText(", though not completely unwelcome");
        }
        ...
        if (flags[kFLAGS.PC_FETISH] > 0)
        {
            ...
            if (player.hasVagina())
            {
                outputText("trembling nether-lips");
            }
            if (player.gender == 0)
            {
                outputText("groin");
        ...
        if (flags[kFLAGS.PC_FETISH] > 0)
        {
            outputText(", even with that kink of yours");
        }
        outputText(", Champ.  Here's something extra anyway, just in case.</i>\" One ghostly incantation later, and ");
        if (player.femininity <= 50)
        {
            outputText("a buttoned shirt and a pair of belted jeans");
        }
        else
        {
            outputText("a tight-fitting tank top and skirt");
        }
        ...
        var dickF : Void->Void = function() : Void
        {
            ...
            if (silly())
            {
                outputText("  This is a <i>fucking good eclair</i>.");
            }
            ...
            if (player.cocks[0].cockLength < 8 || player.cocks[0].cockLength > 25)
            {
                outputText("\"<i>First though, let's change it to something more manageable.</i>\" Arcane sounds float up towards you, causing your shaft to shift and morph until your throbbing rod is around 2 feet long and dripping pre.  \"<i>Mmm, much better!</i>\"  ");
            }
