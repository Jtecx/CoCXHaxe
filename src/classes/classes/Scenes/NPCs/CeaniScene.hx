...
class CeaniScene extends NPCAwareContent
{
    
    public function ceaniAffection(changes : Float = 0) : Float
    {
        flags[kFLAGS.CEANI_AFFECTION] += changes;
        if (flags[kFLAGS.CEANI_AFFECTION] > 100)
        {
            flags[kFLAGS.CEANI_AFFECTION] = 100;
        }
    ...
    public function basicarcherytraining2() : Void
    {
        ...
        else if (flags[kFLAGS.CEANI_ARCHERY_TRAINING] == 2 || flags[kFLAGS.CEANI_ARCHERY_TRAINING] == 3)
        {
            ...
            if (flags[kFLAGS.CEANI_ARCHERY_TRAINING] == 2)
            {
                flags[kFLAGS.CEANI_ARCHERY_TRAINING] = 3;
            }
    ...
    private function ceaniHungry(ocean : Bool) : Void
    {
        ...
        if (player.hasItem(consumables.FISHFIL) && player.hasItem(consumables.FREFISH))
        {
            ...
            {
                player.destroyItems(consumables.FREFISH, 1);
            }
        }
        else
        {
            ...
            if (player.hasItem(consumables.FREFISH))
            {
                player.destroyItems(consumables.FREFISH, 1);
            }
        }
        ...
        addButton(1, "Date", (ocean) ? oceanInteractionsDate : beachInteractionsDate).disableIf(!player.hasVagina() && player.findCock(1, 15, 36, "length") < 0, 
                "Req. a cock with length between 15 and 36 inches or a vagina."
        );
        if (flags[kFLAGS.CEANI_AFFECTION] == 100)
        {
            addButton(2, "Come2Camp", come2campCeani);
        }
    }
    ...
    public function ceaniCampMainMenu2() : Void
    {
        ...
        if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] >= 2 && (flags[kFLAGS.CEANI_DAILY_TRAINING] == 0 || flags[kFLAGS.CEANI_DAILY_TRAINING] == 1))
        {
            addButton(3, "Spar", sparringWithCeani);
        }
        if (flags[kFLAGS.CEANI_DAILY_TRAINING] < 2)
        {
            addButton(4, "Fishing Contest", fishingContestWithCeani);
        }
    ...
    public function beachInteractionsDate() : Void
    {
        ...
        {
            addButton(0, "Underwater", beachInteractionsDateUnderwater0);
        }
    ...
    public function beachInteractionsDateUnderwater2() : Void
    {
        if (player.hasPerk(PerkLib.AquaticAffinity))
        {
            ...
            {
                outputText("however she reaches the finishing line before you cheerfully swimming in circle up and down.\n\n");
            }
        }
        else
        {
            outputText("While you can breathe underwater, you don’t have the body to swim quite as fast as she, so you simply cling to her back as she swims to various locations showing you what the ocean is like. \n\n");
        }
        ...
        {
            outputText(". You were too busy pondering these details to notice as Ceani slowly removed your [armor]");
        }
        if (!player.lowerGarment.isNothing)
        {
            outputText(" and undergarment");
        }
        ...
        //additional check{
            ...
            menu();
            addButton(0, "Male", underwaterDateMaleVer).disableIf(player.findCock(1, 15, 36, "length") < 0, 
        ...
        if (player.gender == 1)
        {
            underwaterDateMaleVer();
        }
        if (player.gender == 2)
        {
            underwaterDateFemaleVer();
        }
    }
    ...
    public function underwaterDateMaleVer() : Void
    {
        ...
        if (player.cocks[x].cockLength >= 22)
        {
            outputText("Ceani looks to be lost in the pleasure of your [cock], her spasming pussy desperately trying to milk your [cock] as it hits her cervix on every trust. She really must not get a decent fuck this often and you almost feel bad for her. ");
        }
        ...
        if (player.gender == 3)
        {
            outputText("Perhaps you should try a man underwater one day and see it for yourself.");
        }
        else
        {
            outputText("But again, you don’t have a pussy so I guess it’s not something you could understand.");
        }
        outputText("</i>\"\n\n");
        ...
        if (player.cocks[x].cockLength < 22)
        {
            outputText(" Still something tells you if your cock was bigger Ceani might like it more. Your dick felt like a small key trying to fit an oversized lock.");
        }
        outputText("\n\n");
    ...
    public function underwaterDateFemaleVer() : Void
    {
        ...
        if (player.necklace == necklaces.MCPNECK)
        {
            outputText("or perhaps because of the necklace ");
        }
    ...
    public function beachInteractionsDateOnTheBeach2() : Void
    {
        ...
        {
            outputText("\"<i>Say... uh... I know I’m a big girl but.. is it ok if we hold hands?</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Say... uh... would you mind if we held hands?</i>\"\n\n");
        }
        ...
        if (!player.isNaked())
        {
            outputText("Ceani isn’t staying idle either slowly taking care of your some of your equipment. ");
        }
        ...
        if (flags[kFLAGS.CEANI_AFFECTION] < 21)
        {
            if (player.tallness < 96)
            {
                outputText("\"<i>I have been eyeing you since the trainings but I wasn’t sure you were into... girls above a certain size... furthermore, I was to shy to ask.</i>\"\n\n");
            }
            else
            {
                outputText("\"<i>I have been eyeing you since the trainings but I wasn’t sure you were interested...furthermore I was too shy to ask you out.</i>\"\n\n");
            }
        }
        else
        {
        ...
        if (!player.lowerGarment.isNothing)
        {
            outputText(", as her tail finish getting rid of your lower clothing, ");
        }
        ...
        if (player.gender == 3)
        {
            ...
            menu();
            addButton(0, "Male", beachDateMaleVer).disableIf(player.findCock(1, 15, 36, "length") < 0, 
                    "Req. a cock with length between 15 and 36 inches."
            );
    ...
    public function beachDateMaleVer() : Void
    {
        ...
        if (player.cumQ() >= 2000)
        {
            outputText(", the sand below you darkening from the sheer amount of fluids you both produced");
        }
    ...
    public function come2campCeani() : Void
    {
        ...
        if (camp.FisheryWorkersCount() > 0)
        {
            ...
            {
                outputText("" + flags[kFLAGS.FOLLOWER_AT_FISHERY_1] + "");
            }
            if (camp.FisheryWorkersCount() > 1)
            {
                if (flags[kFLAGS.FOLLOWER_AT_FISHERY_2] != "")
                {
                    outputText(", " + flags[kFLAGS.FOLLOWER_AT_FISHERY_2] + "");
                }
                if (flags[kFLAGS.FOLLOWER_AT_FISHERY_3] != "")
                {
                    outputText(", " + flags[kFLAGS.FOLLOWER_AT_FISHERY_3] + "");
                }
            }
    ...
    public function fishingContestWithCeani() : Void
    {
        ...
        if (player.isRace(Races.ORCA, 1, false))
        {
            outputText(" although Ceani definitively eaten most of the fishes on her own");
        }
        ...
        if (flags[kFLAGS.CEANI_DAILY_TRAINING] < 1)
        {
            flags[kFLAGS.CEANI_DAILY_TRAINING] = 2;
        }
        if (flags[kFLAGS.CEANI_DAILY_TRAINING] == 1)
        {
            flags[kFLAGS.CEANI_DAILY_TRAINING] = 3;
        }
        endEncounter();
    }
    ...
    public function sparringWithCeaniWon() : Void
    {
        ...
        {
            if (flags[kFLAGS.CEANI_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.CEANI_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.CEANI_DEFEATS_COUNTER] = 1;
            }
