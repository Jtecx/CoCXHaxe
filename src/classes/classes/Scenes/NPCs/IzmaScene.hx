...
class IzmaScene extends NPCAwareContent implements TimeAwareInterface
{
    public var izmaCapacity(get, never) : Float;
    public var izmaNofitMsg(get, never) : String;
    ...
    private var voluntary : Bool = false;  //set before dom/submit scenes to determine if they started from a fight  
    ...
    public function timeChange() : Bool
    {
        ...
        {
            flags[kFLAGS.IZMA_TIME_TILL_NEW_BOOK_AVAILABLE]--;
        ...
        if (model.time.hours > 23 && flags[kFLAGS.GOT_STOOTH_FROM_IZMA_TODAY] > 0)
        {
            flags[kFLAGS.GOT_STOOTH_FROM_IZMA_TODAY] = 0;
        }
        return false;
    }
    ...
    override public function izmaFollower() : Bool
    {
    ...
    private function get_izmaCapacity() : Float
    {
        return 65;
    }
    private function get_izmaNofitMsg() : String
    {
        return "Req. a dick fitting " + izmaCapacity + " area";
    }
    
    public function totalIzmaChildren() : Int
    {
        return as3hx.Compat.parseInt(flags[kFLAGS.IZMA_CHILDREN_SHARKGIRLS] + flags[kFLAGS.IZMA_CHILDREN_TIGERSHARKS]);
    }
    ...
    public function meetIzmaAtLake() : Void
    {
        ...
        else if (flags[kFLAGS.IZMA_ENCOUNTER_COUNTER] == 1)
        {
            ...
            {
                outputText("outside the safety of Tel'Adre");
            }
            else
            {
                outputText("since you arrived");
            }
        ...
        {
            //[After 3 encounters] (Izmacounter = 4)if (flags[kFLAGS.IZMA_ENCOUNTER_COUNTER] == 4)
            {
            ...
            else
            {
                ...
                {
                    //[After 5 consecutive wins against Izma] (encountered with Izmafight >= 5)if (flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] >= 6 && flags[kFLAGS.IZMA_FOLLOWER_STATUS] == 0)
                    {
    ...
    private function readSharkCuntManual() : Void
    {
        ...
        if (flags[kFLAGS.CAMP_BUILT_CABIN] > 0 && flags[kFLAGS.CAMP_CABIN_FURNITURE_BOOKSHELF] > 0 && player.hasKeyItem("Izma's Book - Combat Manual") < 0)
        {
            addButton(2, "Buy", buyBookCombatManual);
        }
    }
    ...
    private function readSharkCuntManual2() : Void
    {
        ...
        if (flags[kFLAGS.IZMA_FOLLOWER_STATUS] != 1)
        {
            outputText("head back to your camp.");
        }
        else
        {
            outputText("leave the shark-girl to her books.");
        }
        //(Izmacounter +1)
        flags[kFLAGS.IZMA_ENCOUNTER_COUNTER]++;
        endEncounter();
    ...
    private function sharkEdgingGuideLOL() : Void
    {
        ...
        if (flags[kFLAGS.CAMP_BUILT_CABIN] > 0 && flags[kFLAGS.CAMP_CABIN_FURNITURE_BOOKSHELF] > 0 && player.hasKeyItem("Izma's Book - Etiquette Guide") < 0)
        {
            addButton(2, "Buy", buyBookEtiquetteGuide);
        }
    }
    ...
    private function sharkgirlPronz() : Void
    {
        ...
        if (flags[kFLAGS.CAMP_BUILT_CABIN] > 0 && flags[kFLAGS.CAMP_CABIN_FURNITURE_BOOKSHELF] > 0 && player.hasKeyItem("Izma's Book - Porn") < 0)
        {
            addButton(2, "Buy", buyBookPorn);
        }
    }
    ...
    private function talkToASharkCoochie() : Void
    {
        ...
        if (flags[kFLAGS.IZMA_TALKED_AT_LAKE] == 0)
        {
            ...
            {
                outputText("You scratch your chin in thought, feeling that this matter warrants further investigation.");
            }
            //(If the player has done the Demon Factory)
            else
            {
                outputText("You smile, and detail just what that factory is and what you went on to do in this factory.");
            }
        }
    ...
    private function fightSharkCunt() : Void
    {
        ...
        {
            outputText("\"<i>All right, show me just what a Champion can do!</i>\" she says, entering a fighting stance.");
        }
        else if (flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] > 0 && flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] <= 2)
        {
            outputText("Izma's eyes narrow at you, and she assumes a fighting stance.  \"<i>You won't get so lucky this time.</i>\"");
        }
        else if (flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] > 2)
        {
            outputText("Izma seems uncertain with herself as she prepares for battle. \"<i>Go a little easier on me this time... please?</i>\"");
        }
        else if (flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] < 0 && flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] >= -2)
        {
            outputText("\"<i>Hm, really?  Well, maybe you'll get lucky this time,</i>\" she mocks, gesturing at you to strike first.");
        }
        else
        {
            outputText("Izma laughs slightly and shakes her head.  \"<i>If you insist.  At least TRY this time, will ya?</i>\"");
        }
    ...
    private function defeatIzma() : Void
    {
        voluntary = false;
        izmaLakeDominate();
    }
    ...
    //[Loss dialogue]
    @:allow(classes.scenes.nPCs)
    private function IzmaWins() : Void
    {
        clearOutput();
        ...
        else if (flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] <= -5)
        {
            finalIzmaSubmission();
        }
        else
    ...
    {
        dynStats("sen", 2);
        if (flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] >= -1)
        {
            outputText("  You realize what you're thinking and shudder, forcing the submission-tinged desires down. Where did they come from, anyway?");
        }
        else if (flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] >= -2)
        {
            outputText("  Though you manage to force them away, the dreams of submitting to Izma are starting to haunt you, their power and seductive allure growing. But, still, you can control them.");
        }
        else if (flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] >= -3)
        {
            outputText("  You hum idly to yourself and enjoy the mental visions for a while, then, with some reluctance, you push them aside. Still, you're sure you can bring them out again when you want to enjoy them.");
        }
        else if (flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] >= -4)
        {
            outputText("  You have only the vaguest thought that maybe you shouldn't be thinking about Izma in that way, but it's so tempting to just immerse yourself in the sexiness of having a hot herm shark-girl dominate you so thoroughly. After all, it's not as if there's any harm in doing so, is there?");
        }
        else
        {
            outputText("  You embrace the dreams fully, desperate to cling to them as long as you can. It's getting so hard to care about your former mission anymore; why fight the demons when you can just give it up and surrender yourself to Izma? Yes... such a strong, powerful, worthy alpha she is; Izma is all you need. Let her take control of your life, why don't you?");
        }
    ...
    private function finalIzmaSubmission() : Void
    {
        ...
        if (player.hasCock() || player.hasVagina())
        {
            ...
            if (player.hasVagina())
            {
                outputText("your " + vaginaDescript(0) + " moisten considerably");
            }
        }
        outputText(".\n\n");
        ...
        if (player.hasVagina() || player.hasCock())
        {
            ...
            if (player.hasVagina())
            {
                outputText("your " + vaginaDescript(0) + " starts squirting fluids liberally");
            }
        }
        outputText(".\n\n");
        ...
        if (!player.isRace(Races.SHARK, 1, false))
        {
            outputText("  \"<i>But we'll have to make some changes to your body, of course...</i>\"");
        }
        outputText("\n\n");
    ...
    private function nonFightIzmaSmexPAINUS() : Void
    {
        ...
        {
            outputText("Izma seems surprised to see your genitalia is similar to hers.  \"<i>Huh. Thought that every land-dweller usually bumbled into a dick transformation around here.</i>\"");
        }
        //[Horse cock in slot 0]
        else if (player.cocks[0].cockType == CockTypesEnum.HORSE)
        {
            outputText("Seeing your [cock] causes Izma to lick her lips slowly. \"<i>Well... Equinum is pretty popular, isn't it?</i>\"");
        }
        //[Tentacle]
        else if (player.cocks[0].cockType == CockTypesEnum.TENTACLE)
        {
            outputText("A puzzled look plays across Izma's face as she stares at your [cock].  \"<i>Wow... I've read a lot about the corrupt plants.  Didn't think people could wind up like that.</i>\"");
        }
        //[Demon Dick]
        else if (player.cocks[0].cockType == CockTypesEnum.DEMON)
        {
            outputText("Izma looks shocked as she lays eyes on your perverted pecker, having never seen anything quite like it before. \"<i>Um... that looks... awkward. Like having a truncheon between your legs.</i>\"");
        }
        //[cat dork]
        else if (player.cocks[0].cockType == CockTypesEnum.CAT)
        {
            outputText("She glances at your barbed pecker and winces.  \"<i>You ate the fruit around here, didn't you?  One of my friends did that, too.</i>\"");
        }
        //[lizardick]
        else if (player.cocks[0].cockType == CockTypesEnum.LIZARD)
        {
            outputText("Her eyes goggle as you reveal a bulbous purple pecker.  \"<i>Oh wow.  Do you have a green one too?</i>\"");
        }
        //[Captain Nemo]
        else if (player.cocks[0].cockType == CockTypesEnum.ANEMONE)
        {
            outputText("She grimaces as you unveil a [cock].  \"<i>God, not one of those.  You wind up hornier after cumming than you were before you started touching it.</i>\"");
        }
        //[Kangaroo Jack]
        else if (player.cocks[0].cockType == CockTypesEnum.KANGAROO)
        {
            outputText("She looks on blankly as you reveal your sheath, then gives a start as your [cock] slides out of it after a few strokes.  \"<i>Woah... it's so... thin and pointy.</i>\"");
        }
        //[Dog cock]
        else if (player.hasKnot(0))
        {
            outputText("Izma stares intently at your [cock], as if trying to make up her mind about it. \"<i>Well, it is a rather cute look. Just be careful with the knot, will ya?</i>\"");
        }
        //weird new shit.
        else
        {
            outputText("Her eyes bug out as you reveal " + sMultiCockDesc() + ".  \"<i>Woah, jeez, look at that one... oh man, and that one's... daaamn.</i>\"");
        }
        outputText("\n\n");
        ...
        if (player.mf("m", "f") == "m" && player.biggestTitSize() >= 2)
        {
            ...
            {
                outputText("  Catching sight of some milk oozing from your nipples makes Izma step back slightly.  \"<i>You're lactating too?!  Man, there are way too many things that are weird about this...</i>\"  Still, you doubt Izma is as turned off as she acts, what with the raging erection between her legs.");
            }
            outputText("\n\n");
        }
        ...
        {
            outputText("\"<i>Well, actually...</i>\" you begin, flexing your cock-tentacle.  She cuts you off with a glare.  ");
        }
        //(other slot 0)
        else
        {
            outputText("You go to protest further, but stop when she gives you a glare.  ");
        }
        ...
        if (player.cumQ() >= 500)
        {
            outputText("You continue to shoot off stroke after stroke long after her mouth has filled up and she's pulled away.  \"<i>For the love of Marae, [name], I can't swallow all that!</i>\"  ");
        }
    ...
    private function nonFightIzmaSmexCUNTPUSSYSNATCHQUIM() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(" and " + multiCockDescriptLight());
        }
        ...
        if (player.hasCock())
        {
            outputText(" and " + sMultiCockDesc() + " shooting a weak load across her chest");
        }
    ...
    private function acceptIzmaAsYourBitch() : Void
    {
        ...
        //[Marble is at camp when Izma arrives]{
            ...
            {
                outputText("You couldn't break your addiction, and now you'll die without a steady supply of her milk.");
            }
            //(If the player broke their addiction)
            else
            {
                outputText("Thankfully, though you were able to break her milk's hold, and you realized you love Marble despite all that transpired between you, letting her move in with you.");
            }
            outputText("\n\n");
            ...
            if (player.hasPerk(PerkLib.MarblesMilk))
            {
                outputText("turned you into some weak drug-addict");
            }
            //(Broke addiction)
            else
            {
                outputText("drugged you into some twisted relationship");
            }
    ...
    //[=Sex=]
    private function izmaLakeTurnedDownCampSex(fromCamp : Bool = false) : Void
    {
        spriteSelect(SpriteDb.s_izma);
        clearOutput();
        if (fromCamp)
        {
            ...
            if (player.spe < 90)
            {
                outputText(" Although she manages to do that after all, she regrets it the moment she sees it as you punish your disobedient lover by throwing her in the cold water. Laughing, she sputters and tries to pull you there too, ");
            }
            else
            {
                outputText("She can't outspeed you. After you arrive there first, you tease your slow inferior and push the exhausted lover into the water while she can't resist, still panting.");
            }
            if (player.str < 90)
            {
                outputText(" making you fall on top of her with a loud splash. Feeling her already hard cock, you grin, kissing her wet face.  ");
            }
            else
            {
                outputText(" receiving nothing but a teasing slap across her face. When she crawls back from the water, you can see her already rock-hard dick.  ");
            }
        }
        else
        {
    ...
    private function izmaLakeDominate() : Void
    {
        ...
        {
            outputText("Izma falls back into the sand, her " + ((monster.HP <= monster.minHP()) ? "injuries" : "lust") + " preventing her from fighting on.  She growls at you in annoyance, \"<i>Fine.  You win... this time.</i>\"\n\n");
        }
        else if (flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] < 3)
        {
            outputText("Having incapacitated Izma through " + ((monster.HP <= monster.minHP()) ? "physical" : "sexual") + " prowess, you stand over the defeated tigershark.  \"<i>Okay, okay!  You win!  Geez... let's get on with it, I feel like I'm gonna go crazy.</i>\"  She begins removing her clothing.\n\n");
        }
        else
        {
            outputText("Izma falls into the sand in an exaggerated fashion.  \"<i>Oh no!  I seem to have lost!  Please don't ravish me again!</i>\" she proclaims loudly as she undresses, her bad acting almost making you laugh your ass off.\n\n");
        }
        menu();
        ...
        if (!voluntary)
        {
            addButton(4, "Leave", leaveIzmaVictoryTease);
        }
        ...
        function dickF() : Void
        {
            var x : Float = player.cockThatFits(izmaCapacity);
            if (x < 0)
            {
                x = 0;
            }
            ...
            //[(If Izmafight = 3+)
            if (!voluntary && flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] >= 3)
            {
                outputText("  Hell, maybe it was made for you, given just how eager Izma seems to be whenever she sees you.  It's like she loses to you on purpose.");
            }
            outputText("\n\n");
        ...
        function vagF() : Void
        {
            ...
            if (player.isTaur())
            {
                outputText("your weight pinning her to the sand and preventing her from taking control.");
            }
            ///(non-taur and height > 4')
            else if (player.tallness > 48)
            {
                outputText("and pin her hands above her head to stop her from trying to change the position. Her heaving breasts rub against your " + chestDesc() + " as you ride her in this posture.  She needs to know who's in charge here, after all.");
            }
            //(non-taur and height <= 4')
            else
            {
                outputText("but small as you are, you can't stop her as she reaches up and grabs onto your [butt], then begins bouncing you like a goblin cocksleeve.");
            }
            outputText("\n\n");
            ...
            if (!player.isTaur())
            {
                outputText("  To reward your eager partner for her efforts you reach back with one free hand to massage and grope at her testes.  Izma bites her lip and starts growling loudly, pushing her hips up as far as possible, eager to cum for you.  You decide to return the favor, mashing your pussy down with increasing speed.");
            }
            outputText("\n\n");
        ...
        var assF : Void->Void = function() : Void
        {
            if (!voluntary)
            {
    ...
    private function izmaLakeDominateContinueVanilla(vanilla : Bool = true) : Void
    {
        spriteSelect(SpriteDb.s_izma);
        var cockIndex : Float = player.cockThatFits(izmaCapacity);
        if (cockIndex < 0)
        {
            cockIndex = 0;
        }
        ...
        if (player.gender == 1)
        {
            ...
            if (player.cumQ() >= 500)
            {
                ...
                if (player.cumQ() >= 1500)
                {
                    outputText(".  Eventually it can swell no more and each new squirt forces cum out from her stuffed pussy, trickling past her asshole");
                }
                outputText(".");
            }
    ...
    private function izmaCumsAfterRadarStuffHere(denied : Bool) : Void
    {
        var cockIndex : Float = player.cockThatFits(izmaCapacity);
        if (cockIndex < 0)
        {
        ...
        if (!denied)
        {
            outputText("The double stimulation pushes Izma past her limit very quickly, and she starts shooting thick jets of spunk into the air, which begin to rain down on her face and breasts.  Her vaginal walls clamp down on your " + Appearance.cockNoun(player.cocks[cockIndex].cockType) + " almost painfully as the orgasm wracks her female genitalia too.\n\n");
        }
        //[Remove her hands]
        else
        {
            outputText("Without another word, or another moment of tormenting impediment, Izma howls with unbridled fury, releasing her hot, sticky load into the air and onto her forehead.  Convulsing with utter euphoria as she finally blows her wad, the walls of her vagina clamp down hard in concert with her feelers, trying ever so valiantly to push you over the edge.  Unfortunately, (for Izma anyway), the unconscious efforts of her body fail to bring you to release. Remarking to Izma that she came so soon, you coo down to her that you are pleased with her ability to speedily heed your commands.\n\n");
        }
    ...
    private function submitToLakeIzma() : Void
    {
        ...
        if (voluntary)
        {
            ...
            {
                outputText("your tail waving enticingly, ");
            }
        ...
        else
        {
            if (flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] >= 0)
            {
                outputText("Izma chuckles slightly as she prowls around your defeated form.  \"<i>Well, as far as things around here stand, you made for a decent fight.  Still no match for me, though.</i>\"");
            }
            else if (flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] >= -2)
            {
                outputText("\"<i>Ya know, just because we're friends doesn't mean you need to hold back... you were holding back, right?</i>\" Izma asks, placing her hands on her hips.");
            }
            else
            {
                outputText("Izma sighs and shakes her head at you, letting a foot rest on your stomach  \"<i>You're doing this on purpose, aren't you?  Hm, fine.  If you love my cock so much, I think you'd make for a decent mate...</i>\"");
            }
            if (player.HP < 1)
            {
        ...
        {
            outputText("\"<i>All right; time to hold up your end,</i>\" she notes.  The double entendre is lost on your fuddled mind.  Her tone is conversational, but her grin is wicked, and she is slipping out of her grass skirt as hastily as she can, given the care she needs to take it off without damaging it.  After all, skirts that can so easily conceal the iron-hard foot-and-a-quarter-long erection she is sporting require considerable skill to make, especially if they must also conceal two pairs of baseball-sized nuts, so swollen and heavy with cum that you think you can hear them slosh softly from where you have fallen.  \"<i>We agreed to do it shark style; you lost, so that means I'm in charge.  Get undressed and show me that cute little " + ((vag != 0) ? "cunt" : "pucker") + " of yours!</i>\"  ");
            //[(If Izmafight is -1 or -2)
            if (flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] >= -2)
            {
                outputText("With some reluctance, but driven by your promise, you remove pieces of your [armor] until you stand naked before the hungrily ogling tigershark.");
            }
            else
            {
                outputText("You are already undressed before Izma has finished speaking.  The tigershark is clearly surprised, and, to be honest, a part of you is surprised too... but it's drowned out by the need to give yourself over to Izma's lusts.");
            }
            outputText("\n\n");
            
            outputText("Your 'cute little " + ((vag != 0) ? "cunt" : "pucker") + " clenches tight in " + ((player.HP < 1) ? "fear" : "anticipation") + " at the thought of something that huge forcing its way into it... but you did make a promise and you are " + ((player.HP < 1) ? "too weak to resist" : "feeling very horny") + ", so you comply.  You remove the last bits of your [armor], and position yourself on the ground, your [butt] facing towards Izma.  You can hear her stalking across the sand toward you, but it still sends tingles up your spine when her hands - now free of her metal gauntlets - fall onto your [ass].\n\n");
        ...
        function assF() : Void
        {
            ...
            if (player.analCapacity() < 26)
            {
                ...
                if (!voluntary && flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] <= -4)
                {
                    outputText("  You find yourself pushing back to speed up the process, desperate for Izma to fill you again.");
                }
            }
            else if (player.analCapacity() < 60)
            {
                ...
                if (!voluntary)
                {
                    ...
                    if (flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] <= -4)
                    {
                        outputText("  You find yourself wondering that as well...");
                    }
                }
                else
                {
                    outputText("</i>\" she exclaims, pleasured by the perfect fit.");
                }
            }
            else
            {
            ...
            if (player.hasCock())
            {
                outputText(", and your own male organ is hard and throbbing from the stimulation");
            }
            outputText(".\n\n");
            ...
            if (!voluntary && flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] <= -4)
            {
                outputText("  If this is how the sharks do it, you could really get used to it...");
            }
            outputText("\n\n");
            ...
            if (player.hasCock())
            {
                ...
                if (player.cumQ() < 25)
                {
                    outputText("a trickle");
                }
                else if (player.cumQ() < 150)
                {
                    outputText("several squirts");
                }
                else
                {
                    outputText("a steady stream");
                }
            ...
            {
                outputText("your own muscles spasming from the immense pleasure.");
            }
        ...
        function vagF() : Void
        {
            ...
            if (player.vaginalCapacity() < 26)
            {
                ...
                {
                    outputText("First time, huh?");
                }
                else
                {
                    outputText("That's tight!");
                }
            ...
            if (voluntary)
            {
                ...
                {
                    outputText("Hauling out a plant, she gingerly removes a leaf and offers it to you.  \"<i>Could you take this please?  It's an anti-pregnancy herb.  I'd be happy to have babies with you someday... but I want to earn your acknowledgment as a mate first.</i>\"  Smiling a bit at how old-fashioned she seems, you take the leaf out of her hands and eat it.");
                }
            }
            else
            {
                ...
                if (!player.isPregnant() || player.hasNonVisiblePregnancy())
                {
                    outputText("  From her oak chest she fetches an odd leaf, which she pushes past your lips.  \"<i>Birth control herbs.  No way I'm giving kids to someone who's not my mate,</i>\" Izma says, making sure you swallow the plant before diving into the water, most likely to clean herself off.\n\n");
                }
        ...
        var hermF : Void->Void = function() : Void
        {
            if (!voluntary)
            {
                ...
                if (player.lust < 40)
                {
                    outputText("limp cock and barely-engorged cunt");
                }
                else if (player.lust < 70)
                {
                    outputText("turgid erection and moist cunt");
                }
                else
                {
                    outputText("throbbing erection and slavering cunt.");
                }
                outputText("\n\n");
            }
            ...
            {
                outputText("Izma manages to supress a snort of laughter at the sight of your cock.  \"<i>Um... wow?  It's simply... heh, huge...</i>\"");
            }
            //(10-19 inches)
            else if (player.cocks[x].cockLength <= 19)
            {
                outputText("\"<i>Not bad, I'm actually impressed,</i>\" Izma says, nodding slightly in approval.");
            }
            //(20+ inches)
            else
            {
                outputText("Izma gives a low whistle at the sight of your [cock].  \"<i>Now THAT'S a cock.  Looks like you've got a third leg down there!</i>");
            }
            ...
            if (!voluntary && flags[kFLAGS.IZMA_TIMES_FOUGHT_AND_WON] <= -4)
            {
                outputText("  You're not even really ashamed to admit that such is the case anymore.");
            }
            outputText("\n\n");
            ...
            if (voluntary)
            {
                ...
                {
                    outputText("She gives you a decidedly unchaste kiss before she helps you up.  You dress yourself and head back to camp feeling very sated.");
                }
                endEncounter();
            }
            else
    ...
    public function izmaFollowerMenu() : Void
    {
        ...
        //Izma pops 'em out!{
            ...
            switch (_sw19_)
            {
                ...
                case 6:outputText("Izma's certain the baby will come soon.  You're inclined to believe her; she's huge now.  She still wears no clothing, but there's definitely a practical reason for it");
                    ...
                    {
                        outputText("- you don't think her skirt would be able to cope with how often she gets erect now, her huge cock rubbing along");
                    }
                    else
                    {
                        outputText(", her juices staining");
                    }
                    ...
                    if (flags[kFLAGS.IZMA_NO_COCK] == 0)
                    {
                        outputText("the friction of her cock against her bulge, you think it has more to do with avoiding being splattered in sexual juices all day long.");
                    }
                    else
                    {
                        outputText("the friction of her thighs as she moves, you think it has more to do with avoiding being splattered in sexual juices all day long.");
                    }
                default:
            }
        }
        ...
        addButton(2, "Books", IzmaCampBooks).hint("Engage in a book-reading session with Izma.").disableIf(flags[kFLAGS.FOLLOWER_AT_FARM_IZMA], "This can only be done at your camp.");
        addButton(3, "Children", izmaKidsPlaytime).hint("Watch the children of Izma play together.").disableIf(flags[kFLAGS.FOLLOWER_AT_FARM_IZMA], "This can only be done at your camp.").disableIf(totalIzmaChildren() == 0, "Perhaps if you havd a child with Izma.", "???");
        addButton(4, "Tooth", gatASharkTooth).hint("Ask Izma for the tigershark tooth.");
        addButton(5, "Sex", izmaSexMenu).hint("Do some romp with the tigershark!").disableIf(player.lust < 33, "You are not horny enough to consider that.");
        addButton(6, "Lake", izmaLakeTurnedDownCampSex, true).hint("Go to the lake together to fuck the old-fashioned way!").disableIf(player.lust < 33, "You are not horny enough to consider that.");
        if (flags[kFLAGS.IZMA_BROFIED] != -1 || sceneHunter.other)
        {
            if (flags[kFLAGS.IZMA_NO_COCK] == 0)
            {
                addButton(7, "Remove Dick", removeIzmasPenis).hint("Get Izma to remove her dick for you. Why would you do that? You monster.");
            }
            else
            {
                addButton(7, "Go Herm", izmaDickToggle).hint("Tell Izma to regrow her dick. She would be quite grateful if you choose to do so.");
            }
        }
        else
        {
            addButtonDisabled(7, "Remove Dick", "You cannot remove Izma's penis as she has Futa Form perk."
                    + "\n\n<b>But, if you enabled something in SceneHunter...</b>");
        }
        addButton(8, (flags[kFLAGS.IZMA_PREGNANCY_ENABLED] == 1) ? "NoKidsPlease" : "Have Kids?", childToggle).hint((flags[kFLAGS.IZMA_PREGNANCY_ENABLED] == 1) ? 
        ...
        //never brofied or debroed{
            
            addButton(9, "Brotize", izmaelScene.brotizeIzma).disableIf(!player.hasItem(consumables.BROBREW), "You'd need a can of Bro Brew for that.").disableIf(flags[kFLAGS.IZMA_BROFIED] == -1 && !sceneHunter.other, 
                    "You can't do that again.\n\n<b>Unless you enable something in SceneHunter.</b>"
            );
        }
        if (flags[kFLAGS.FARM_CORRUPTION_STARTED] == 1)
        {
            if (flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 0 && !pregnancy.isPregnant && flags[kFLAGS.FOLLOWER_AT_FISHERY_1] != "Izma" && flags[kFLAGS.FOLLOWER_AT_FISHERY_2] != "Izma" && flags[kFLAGS.FOLLOWER_AT_FISHERY_3] != "Izma")
            {
                addButton(10, "Farm Work", sendToFarm);
            }
            if (flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] != 0)
            {
        ...
        {
            if (camp.FisheryWorkersCount() < camp.FisheryMaxWorkersCount() && flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 0)
            {
                addButton(11, "Fishery Work", sendToFishery);
            }
            if (flags[kFLAGS.FOLLOWER_AT_FISHERY_1] == "Izma" || flags[kFLAGS.FOLLOWER_AT_FISHERY_2] == "Izma" || flags[kFLAGS.FOLLOWER_AT_FISHERY_3] == "Izma")
            {
                addButton(11, "Stop Work", backFromFishery);
            }
        }
        else
        {
            addButtonDisabled(11, "???", "You need to have functional fishery first.");
        }
        addButton(12, flags[kFLAGS.GOO_NAME], izmaLatexySubmenu).disableIf(flags[kFLAGS.IZMA_NO_COCK], "She needs a cock for this!").disableIf(!latexGirl.latexGooFollower() || flags[kFLAGS.TIMES_IZMA_DOMMED_LATEXY] == 0, "", "???");
        if (valeria.valeriaFluidsEnabled())
        {
            addButton(13, "Valeria", izmaValeriaSubmenu).disableIf(flags[kFLAGS.IZMA_NO_COCK], "She needs a cock for this!");
        }
        addButton(14, "Back", (flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] != null) ? SceneLib.farm.farmCorruption.rootScene : camp.campLoversMenu);
    }
    ...
    private function sendToFishery() : Void
    {
        ...
        {
            flags[kFLAGS.FOLLOWER_AT_FISHERY_1] = "Izma";
        }
        else if (flags[kFLAGS.FOLLOWER_AT_FISHERY_2] == "")
        {
            flags[kFLAGS.FOLLOWER_AT_FISHERY_2] = "Izma";
        }
        else if (flags[kFLAGS.FOLLOWER_AT_FISHERY_3] == "")
        {
            flags[kFLAGS.FOLLOWER_AT_FISHERY_3] = "Izma";
        }
    ...
    private function backFromFishery() : Void
    {
        ...
        if (flags[kFLAGS.FOLLOWER_AT_FISHERY_1] == "Izma")
        {
            flags[kFLAGS.FOLLOWER_AT_FISHERY_1] = "";
        }
        else if (flags[kFLAGS.FOLLOWER_AT_FISHERY_2] == "Izma")
        {
            flags[kFLAGS.FOLLOWER_AT_FISHERY_2] = "";
        }
        else if (flags[kFLAGS.FOLLOWER_AT_FISHERY_3] == "Izma")
        {
            flags[kFLAGS.FOLLOWER_AT_FISHERY_3] = "";
        }
        doNext(izmaFollowerMenu);
    }
    ...
    private function izmaPearance() : Void
    {
        ...
        if (flags[kFLAGS.IZMA_GLOVES_TAKEN] == 0)
        {
            outputText(" and wielding iron gauntlets as a weapon.");
        }
        else
        {
            outputText(".");
    ...
    private function izmaSexMenu() : Void
    {
        ...
        menu();
        addButton(0, "Dom Vagina", fuckIzmasPussyDominate).hint("Dominate Izma and pound her pussy!", "Dominate Vagina").disableIf(player.cockThatFits(izmaCapacity) < 0, izmaNofitMsg);
        addButton(5, "Get Mounted", followerIzmaMountsPC).hint("Order Izma to mount your dick with her pussy.").disableIf(player.cockThatFits(izmaCapacity) < 0, izmaNofitMsg);
        addButton(10, "Izma's Vagina", followerIzmaTakesItInVagoo).hint("Have some play with the tigershark's pussy and fuck her.").disableIf(player.cockThatFits(izmaCapacity) < 0, izmaNofitMsg);
        addButton(1, "DomWithAss", radarIzmaAnalDominant).hint("Be dominant and take her dick anally.", "Dominate with Ass").disableIf(flags[kFLAGS.IZMA_NO_COCK], "She needs a cock for this!");
        addButton(6, "Anal 'Catch'", followerIzmaTakesItInPooper).hint("It's Izma's turn to be alpha! Let her dom your ass with her penis.").disableIf(flags[kFLAGS.IZMA_NO_COCK], "She needs a cock for this!");
        addButton(11, "RideIzmaCock", inCampRideIzmasDickDongTheWitchIsDead).hint("Ride Izma's member vaginally.", "Ride Izma's Cock").disableIf(!player.hasVagina(), "You need a vagina for this!").disableIf(flags[kFLAGS.IZMA_NO_COCK], "She needs a cock for this!");
        addButton(2, "Sixtynine", followerIzmaTakesIt69).disableIf(player.isGenderless(), "Not for genderless!");
        addButton(7, "FuckHerAss", radarFucksIzmasAss).disableIf(player.cockThatFits(izmaCapacity) < 0, izmaNofitMsg);
        addButton(12, "TentacleAnal", radarTentacleAnal).disableIf(player.countCocksWithType(CockTypesEnum.TENTACLE, 48) == 0, "You need long (4 feet at least) tentacles for that!");
        addButton(14, "Back", izmaFollowerMenu);
    }
    //[Izma mounts the PC]
    private function followerIzmaMountsPC(lastHalf : Bool = false) : Void
    {
        spriteSelect(SpriteDb.s_izma);
        clearOutput();
        var x : Float = player.cockThatFits(izmaCapacity);
        ...
        if (!lastHalf)
        {
            ...
            if (flags[kFLAGS.IZMA_NO_COCK] == 0)
            {
                outputText(" instead of her cock");
            }
            ...
            if (flags[kFLAGS.IZMA_NO_COCK] == 0)
            {
                outputText("  Your stomach starts to feel wet, and as you look down you realize her pre is starting to soak into your " + player.skinFurScales() + ".\n\n");
            }
            else
            {
                outputText("  Your stomach starts to feel wet, and as you look down you realize her feminine lubricant is starting to soak into your " + player.skinFurScales() + ".\n\n");
            }
            ...
            {
                outputText("  Still, the sight of her curvaceous cleavage bouncing gives you an idea.  You reach up to cup your strong lover's hard bosom, enjoying the sight of her slightly-upturned nipples pointing up at the sky.  You flick them gently as she rides you, sometimes squeezing and tugging encouragingly.\n\n");
            }
        }
        if (flags[kFLAGS.IZMA_NO_COCK] == 0)
        {
        ...
        if (player.cumQ() < 500)
        {
            outputText("You can feel Izma's womb fill with your cum, and she shudders from the sheer pleasure.");
        }
        //(High cum production)
        else
        {
            outputText("Izma squeals loudly as you cum inside her, almost having a second orgasm as her belly is bloated and distended by your inhuman amount of cum.");
        }
    ...
    private function followerIzmaTakesItInPooper() : Void
    {
        ...
        {
            outputText("pulls you up off the ground in a bear hug to kiss you fiercely");
        }
        //[else]
        else
        {
            outputText("pulls you in to her body and kisses you fiercely");
        }
        ...
        {
            outputText("against your lower body");
        }
        ...
        if (player.hasCock())
        {
            ...
            if (player.hasBalls())
            {
                outputText(" against your own " + ballsDescriptLight());
            }
            //(if no balls and vag)
            else if (player.hasVagina())
            {
                outputText(" against your already-dripping " + vaginaDescript(0));
            }
            //(else)
            else
            {
                outputText(" against your perineum");
            }
            ...
            {
                outputText("\"<i>Hmm...</i>\" she sighs, as she looks at her glans sticking out past the tip of your dick.  \"<i>It's nice to know who's in charge.</i>\"");
            }
            //[dick0 >14 but <16]
            else if (player.cocks[0].cockLength < 16)
            {
                outputText("\"<i>It's nice that we keep things fair around here,</i>\" murmurs Izma, smiling at your similarly-sized cocks.");
            }
            //[If dick0 >16]
            else
            {
                outputText("\"<i>Oh fuck,</i>\" she says, her eyes filling with lust as she sees your cock extending well past her own.  She tilts forward and rubs the base of her cock hard against the base of your " + Appearance.cockNoun(player.cocks[0].cockType) + ", jerking both of you off together.  You moan helplessly when she stops, and she sighs, \"<i>It feels so good to be alpha over a powerful cock like yours.</i>\"");
            }
            outputText("\n\n");
        }
        ...
        if (player.isBiped())
        {
            outputText(" behind your knees to immobilize you");
        }
        else
        {
            outputText(" around your [legs] to immobilize you");
        }
        ...
        if (player.hasCock())
        {
            outputText("  You try to reach for your cock and get off, but her hands keep your wrists securely pinned to the ground.");
        }
        outputText("\n\n");
        ...
        if (!player.hasCock())
        {
            outputText("The feeling of her cock throbbing finally puts you over the edge and you orgasm, your tight pucker helping milk the last of her cum into you.\n\n");
        }
        //(if player has penis)
        else
        {
            ...
            if (player.cocks[x].cockLength < 14)
            {
                ...
                if (player.cockTotal() > 1)
                {
                    outputText(" first, then your other " + cockDescript(1) + ", synchronizing her movements");
                }
                ...
                if (player.cumQ() >= 1500)
                {
                    outputText(" over and over again until you're a sticky mess lying in a pool of your own semen.");
                }
                //[(big cum)
                else if (player.cumQ() >= 500)
                {
                    outputText(" until cum pours off of your body and pools on the ground.");
                }
                else
                {
                    outputText(".");
            ...
            else if (player.cocks[x].cockLength < 16)
            {
                ...
                if (player.cumQ() < 300)
                {
                    outputText("leaving her looking spattered but pleased");
                }
                else if (player.cumQ() < 500)
                {
                    outputText("leaving her looking wet but impressed");
                }
                else
                {
                    outputText("showering her in so much cum that she leans down to lick it off of her breasts");
                }
                outputText(".\n\n");
            }
            ...
            else
            {
                ...
                if (player.cockTotal() > 1)
                {
                    outputText(" and onto your combined bodies");
                }
                outputText(".\n\n");
                ...
                if (player.cumQ() < 1000)
                {
                    outputText("She swallows dutifully and, when your cock has stopped throbbing between her breasts, takes her mouth off of you, licks her lips, and smiles, showing so many teeth that your cock shrinks a little bit faster than normal.\n\n");
                }
                //[<5 L]
                else if (player.cumQ() < 3000)
                {
                    outputText("She swallows dutifully at first, and when you keep pumping her with cum, gives up on swallowing. Instead, she grabs you by the hips and, by leaning down and pulling you up, gets your cock down her throat so you can deliver your seed directly.  It looks difficult but feels amazing, and when you're done and she pulls her head back, she looks very proud of herself.\n\n");
                }
                //[>5 L]
                else
                {
        ...
        if (!player.hasCock())
        {
            outputText("After a few minutes, she pulls her shrinking cock out of you and gives you a deep kiss. \"<i>Thanks for that,</i>\" she says, \"<i>...my Alpha.</i>\"  She jogs off to the river to clean off.  After a minute you follow her, with her cum dripping out of your well-used anus.");
        }
        if (player.hasCock())
        {
    ...
    private function followerIzmaTakesItInVagoo() : Void
    {
        ...
        if (flags[kFLAGS.IZMA_NO_COCK] == 0)
        {
            outputText(", gently lifting her meaty quads up so");
        }
        else
        {
            outputText(", gently parting her muscled thighs so");
        }
        ...
        if (flags[kFLAGS.IZMA_NO_COCK] == 0)
        {
            outputText("the other holding her maleness out of the way, teasing her cock with her thumb and flexing her fingers to roll her four balls around in the palm of her hand.");
        }
        else
        {
            outputText("the other slowly spreading the folds of her vulva, teasing her clit with her thumb and flexing her interior muscles to make the pink opening wink at you.");
        }
        ...
        if (flags[kFLAGS.IZMA_NO_COCK] == 0)
        {
            outputText(", while Izma herself reaches around to jerk herself off.\n\n");
        }
        else
        {
            outputText(", while Izma herself reaches up to massage her heavy breasts.\n\n");
        }
        ...
        if (flags[kFLAGS.IZMA_NO_COCK] == 0)
        {
            outputText(", and gains speed as she jerks her meat pole");
        }
        outputText(".\n\n");
        ...
        {
            outputText(", a small explosion of fem-cum splattering your belly, and several subsequent squirts dripping down your thighs.  ");
        }
        ...
        if (player.cumQ() < 750)
        {
            outputText("You pump straight into her womb, enough to make a trickle of your baby batter dribble from her freshly used cunt when you pull out.");
        }
        //(High cum production)
        else
        {
            outputText("Your load is so vast that it bloats and distends her belly, almost giving her a second orgasm from the sensations.");
        }
        outputText("\n\n");
        ...
        {
            outputText("licking you face affectionately.  \"<i>Come fill me whenever you need to.</i>\"  You grin back at Izma knowingly.\n\n");
        }
        ...
        {
            outputText("She giggles and runs her tongue over your lips, her tongue lingering for a few moments before she gives you another long kiss and lets you go.");
        }
        //PREGGO CHANCES
        izmaPreg();
        player.sexReward("vaginalFluids", "Dick");
    ...
    private function wheresItGoing(denied : Bool = false) : Void
    {
        spriteSelect(SpriteDb.s_izma);
        ...
        if (!denied)
        {
            outputText("jerks off ");
        }
        else
        {
            outputText("moans fiercely ");
        }
    ...
    private function RadarIzmaCumInYourChest() : Void
    {
        ...
        {
            outputText("in between your " + chestDesc() + " and coats ");
        }
        outputText("against your chest");
        if (player.biggestTitSize() < 1)
        {
            outputText(", coating it");
        }
        ...
        if (player.cumQ() < 750)
        {
            outputText("liberally lubricating her fuckhole with steamy cum and causing your beta to cry out in blissful pleasure.");
        }
        //(High cum production)
        else
        {
            outputText("flooding her love canal with so much sperm that a slew of wet, sloppy noises ring out from her pussy.  Izma's expression betrays a sense of utter disbelief as you stuff her full of your spooge; which begins to flow out of her at a steady rate as each contraction of your dick's muscles fires more and more into her.");
        }
    ...
    private function IzmaSelfFacialWheeRadar() : Void
    {
        ...
        if (player.cumQ() < 750)
        {
            outputText("liberally lubricates her fuckhole with steamy cum and causes your luscious beta to cry out in blissful pleasure.");
        }
        //(High cum production)
        else
        {
            outputText("floods her love canal with so much sperm that a slew of wet, sloppy noises ring out from her pussy.  Izma's expression betrays a sense of utter disbelief as you stuff her full of your spunk, some of which begins to flow out of her at a steady rate, each contraction of your dick's muscles firing more and more into her.");
        }
    ...
    private function followerIzmaTakesIt69() : Void
    {
        ...
        if (player.hasCock())
        {
            ...
            if (player.hasVagina())
            {
                outputText(" while pushing two fingers into your " + vaginaDescript(0));
            }
        }
        //(Female)
        else if (player.hasVagina())
        ...
        if (flags[kFLAGS.IZMA_NO_COCK] == 0)
        {
            outputText("You wrap your lips around Izma's own massive erection, sucking and gagging on her almost-two-foot cock as she jerks and twists her hips around.  Her muffled moans seem to assure you that you're doing a great job, and Izma responds by increasing the speed of her head-bobbing.  Your vision is a little obscured by Izma's quartet of balls repeatedly bumping against your forehead and the bridge of your nose, but if anything this serves to arouse you further.\n\n");
        }
        else
        {
            outputText("You wrap your lips around Izma's puffy, inhuman lower lips, sucking and swallowing her fragrant fem-juices as she jerks and twists her hips around.  Her muffled moans seem to assure you that you're doing a great job, and Izma responds by increasing the speed of her head-bobbing.  Your vision is a little obscured by Izma's sinuous tail, but if anything this serves to arouse you further.\n\n");
        }
        ...
        {
            outputText("to push your " + clitDescript());
        }
        outputText(" as far into her mouth as you can.\n\n");
        
        if (player.hasCock())
        {
            outputText("Eventually, the two of you manage to bring each other to a powerful simultaneous orgasm, Izma's hot cum shoots down your eager throat, and you ");
        }
        else
        {
            outputText("Eventually, the two of you manage to bring each other to a powerful simultaneous orgasm, Izma's slippery cum drenching your face, and you ");
        }
        if (player.hasCock())
        {
        ...
        {
            outputText("drool a sticky mess over Izma's cheeks.");
        }
    ...
    private function talkWivIzma() : Void
    {
        ...
        else if (flags[kFLAGS.IZMA_TALK_LEVEL] == 3)
        {
            ...
            if (flags[kFLAGS.IZMA_NO_COCK] == 1)
            {
                outputText("she used to have a ");
            }
            else
            {
                outputText("she's got a ");
            }
            outputText("perfectly functional set of male genitalia.\n\n");
            
            if (flags[kFLAGS.IZMA_NO_COCK] == 0)
            {
                outputText("\"<i>Oh, nah.  Pfft, as if that's a challenge,</i>\" she says.  It's interesting to see how Izma, as a hermaphrodite, thinks of herself when it comes to the most basic of gender roles.  She scratches her chin in thought.  \"<i>Well, sexually, I'm more accustomed to my penis, since it's a bit easier to dominate with and the lake life seems to fixate on it.  But for you, I'll use whichever you want.</i>\"\n\n");
            }
            else
            {
        ...
        else if (flags[kFLAGS.IZMA_TALK_LEVEL] == 4)
        {
            ...
            if (flags[kFLAGS.IZMA_NO_COCK] == 0)
            {
                outputText("..  and patiently ignoring the fact you can feel her cock starting to stiffen against your thigh.");
            }
    ...
    private function chooseToFlirtWithIzma() : Void
    {
        ...
        if (flags[kFLAGS.IZMA_NO_COCK] == 0)
        {
            outputText("her huge cock barely able to poke past her baby-filled belly, ");
        }
        ...
        if (flags[kFLAGS.IZMA_NO_COCK] == 0)
        {
            outputText("  But she glances between her legs and sees her massive cock poking out of her skirt.");
        }
        else
        {
            outputText("  But she glances down to behold her rock-hard nipples jutting out.");
        }
        ...
        if (flags[kFLAGS.IZMA_NO_COCK] == 0)
        {
            ...
            {
                outputText("  \"<i>Oh... you'd be so big and curvy, so sexy,</i>\" she purrs.  You advance towards her.  Leaning in, you mention you're willing to try and make a baby, right here and now.  \"<i>I... oh, wow.  We could certainly try,</i>\" she replies, grabbing your neck and pulling you in for a kiss.");
            }
        }
        izmaSexMenu();
    }
    ...
    private function IzmaPoopsBabies() : Void
    {
        ...
        if (flags[kFLAGS.IZMA_NO_COCK] == 0)
        {
            outputText("Divvying your time between her raging erection, quartet of sizable balls and dilating cunt while she strains under you is an awkward experience, but you persevere; when Izma stops hissing and gasping in pain and starts instead moaning lightly, it seems more worthwhile.\n\n");
        }
        else
        {
            outputText("Divvying your time between her dilating cunt and engorged clit while she strains under you is an awkward experience, but you persevere.  When Izma stops hissing and gasping in pain and starts instead moaning lightly, it seems more worthwhile.\n\n");
        }
    ...
    public function pcPopsOutASharkTot(womb : Int = 0) : Void
    {
        ...
        else
        {
            ...
            if (rand(100) <= 59)
            {
                ...
                {
                    flags[kFLAGS.IZMA_CHILDREN_SHARKGIRLS] += 2;
                }
                else
                {
                    flags[kFLAGS.IZMA_CHILDREN_SHARKGIRLS]++;
            ...
            else
            {
                ...
                if (player.hasMutation(IMutationsLib.GoblinOvariesIM))
                {
                    flags[kFLAGS.IZMA_CHILDREN_TIGERSHARKS] += 2;
                }
                else
                {
                    flags[kFLAGS.IZMA_CHILDREN_TIGERSHARKS]++;
            ...
            {
                outputText("  She then starts on your third breast, and while she slows down at the fourth breast, she still finishes it off.");
            }
            //(If player has six breasts:
            if (player.breastRows.length > 2)
            {
                outputText("  She seems to almost be struggling to finish off your third pair, but she manages to suck all six of your breasts dry.");
            }
    ...
    public function izmaKidsPlaytime() : Void
    {
        ...
        if (flags[kFLAGS.IZMA_CHILDREN_SHARKGIRLS] > 1)
        {
        ...
        var choice : Float = choices[rand(choices.length)];
        ...
        {
            outputText("There are ");
        }
        if (flags[kFLAGS.IZMA_CHILDREN_SHARKGIRLS] > 0)
        {
            if (flags[kFLAGS.IZMA_CHILDREN_SHARKGIRLS] == 1)
            {
                outputText("one shark-girl");
            }
            else
            {
                outputText(num2Text(flags[kFLAGS.IZMA_CHILDREN_SHARKGIRLS]) + " shark-girls");
            }
            if (flags[kFLAGS.IZMA_CHILDREN_TIGERSHARKS] > 0)
            {
                outputText(" and ");
        ...
        if (flags[kFLAGS.IZMA_CHILDREN_TIGERSHARKS] > 0)
        {
            ...
            {
                outputText("one tigershark");
            }
            else
            {
                outputText(num2Text(flags[kFLAGS.IZMA_CHILDREN_TIGERSHARKS]) + " tigersharks");
            }
        }
        ...
        //outputText("You see your daughter has caught a goblin who managed to find her way to this nook.  You look away before your tigershark daughter does something inappropriate.  ");{
            ...
            if (totalIzmaChildren() > 2)
            {
                outputText("  Your other kids are gathered around, watching with amusement and a little envy as one of your tigershark offspring plows an excited goblin.  \"<i>Oh, yeah, baby, now you folks know how to live!</i>\" she cheers.");
            }
            if (flags[kFLAGS.IZMA_CHILDREN_TIGERSHARKS] > 2)
            {
                outputText("  \"<i>Don't worry; you'll get a chance to use a beautiful big cock on me, too - just wait your turn!</i>\" she laughs looking around the circle.  Your other daughters look like they can't wait.");
            }
        ...
        if (choice == 4)
        {
            ...
            {
                outputText("one of your daughters");
            }
            ...
            if (totalIzmaChildren() == 1)
            {
                outputText("  She immediately goes to sink her fangs into the still-flopping creature's skull, but Izma coughs pointedly.  Looking chastened, your daughter sits on a nearby rock and waits for the catch to die and be still, then carefully pulls off the sharp fins before dining.");
            }
            else if (totalIzmaChildren() == 2)
            {
        ...
        if (choice == 5)
        {
            ...
            if (totalIzmaChildren() == 1)
            {
                outputText("child perks up to see Izma and comes running over to her, throwing herself into her arms");
            }
            else
            {
                outputText("children perk up to see Izma and come running over to her, throwing themselves into her arms");
            }
            ...
            if (totalIzmaChildren() == 1)
            {
                outputText("cries out in joy at the prospect of storytime and eagerly seats herself in front of her, listening as she starts to read.");
            }
            else
            {
                outputText("cry out in joy at the prospect of storytime and eagerly seat themselves in front of her, listening as she starts to read.");
            }
    ...
    private function inCampRideIzmasDickDongTheWitchIsDead() : Void
    {
        ...
        if (player.wetness() >= 5)
        {
            outputText("  Not that this stops your nether-lips from drooling your lubricant all over Izma's crotch.");
        }
        outputText("\n\n");
        ...
        if (player.cor < 66)
        {
            outputText("you've teased her enough");
        }
        else
        {
            outputText("you've delayed your pleasure long enough");
        }
        outputText(", you sink down onto her, stifling a yelp as she fills your depths.");
        
        //(Tight/Virgin vagina)
        if (player.vaginalCapacity() < 20 || player.vaginas[0].virgin)
        {
            outputText("  Holy shit, that hurts like a motherfucker! It's as if you're trying to stuff a baguette into your vag!");
        }
        //(Loose)
        else if (player.vaginalCapacity() < 50)
        {
            outputText("  You groan and shudder slightly as you slide down the generous 15 inches of dick, the slight pain being drowned out by the sensation of being filled as full as you could possibly hold.");
        }
        //(Gaping)
        else
        {
            outputText("  You shudder and lick your lips. Izma may be big but you've had bigger, and you fit her cock like a glove.");
        }
        player.cuntChange(30, true, true, false);
        ...
        if (player.cor < 33)
        {
            outputText("Feeling merciful, you tell her that you're almost there... just a little more.  ");
        }
    ...
    private function fuckIzmasPussyDominate() : Void
    {
        ...
        if (flags[kFLAGS.IZMA_NO_COCK] == 0)
        {
            outputText("their sexual lubricants");
        }
        else
        {
            outputText("its sexual lubricant");
        }
        ...
        if (player.isNaga() || player.isTaur() || player.isGoo())
        {
            outputText("With her tail now against your shoulder blade and jutting up in the air, your path to her fuckhole is now clear.  ");
        }
        //(If human or Harpy legs are present:
        else
        {
            outputText("Lifting your right leg over her rump, you give yourself the superior position needed to ravage her cunt.  ");
        }
        ...
        if ((player.tentacleCocks() > 1 || (player.tentacleCocks() == 1 && player.cocks[x].cockType != CockTypesEnum.TENTACLE)) || (player.stamenCocks() > 1 || (player.stamenCocks() == 1 && player.cocks[x].cockType != CockTypesEnum.STAMEN)))
        {
    ...
    //[Spanking and orgasm]
    private function radarIzmaSpanking(newSentence : Bool = true) : Void
    {
        if (newSentence)
        {
            clearOutput();
    ...
    private function radarIzmaGasm() : Void
    {
        ...
        if (player.cumQ() <= 750)
        {
            outputText("shooting your hot load into Izma's fuckhole, enveloping the walls of her pussy with your seed.");
        }
        //(Cum production High:
        else
        {
            outputText("unleashing a cascade of steamy jizz into Izma's well fucked hole.  Your volume is so large that it causes a back draft of semen to rush out of her within mere seconds.");
        }
    ...
    private function radarIzmaAnalDominant() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText("Your [clit] flushes with");
        }
        else if (player.hasCock())
        {
            outputText("[EachCock] bobs in");
        }
        else
        {
            outputText("You flush with");
        }
        ...
        {
            outputText("painfully rending the tight shape of your anus on it, causing both of you to moan in pain as you envelop more and more of her dick inside of you.  Izma slams into you forcefully as she unleashes a huge gasp, face utterly flooded with ecstasy as she stares into the sky behind closed eyes.");
        }
        //(If loose to gaping:
        else
        {
            outputText("slipping past the fleshy \"gates\" of your anus, requiring little stretching or working of your " + assholeDescript() + ".  You're worried that this might not work the way you want it to, given how easy it was to take Izma; a series of wild, erratic thrashings against your ass as the penile intruder tunnels deeper seems to dispel that notion.");
        }
        ...
        if (player.isGoo())
        {
            outputText("; taking her firm ass into your slippery body's grip, you begin to force Izma to gyrate against you, driving the shark-morph mad with pleasure");
        }
        ...
        if (player.hasVagina())
        {
            outputText("feverish hands as you fingerfuck your " + vaginaDescript() + ".");
        }
        //(If PC has balls:
        else if (player.hasBalls())
        {
            outputText("swinging [balls] with each gyration of your hips.");
        }
        //(If PC is genderless:
        else
        {
            outputText("thighs, thoroughly \"tenderizing\" them with every thunderous impact.");
        }
    ...
    private function radarIzmaAnalDomResultTuckIn() : Void
    {
        ...
        if (player.str < 70)
        {
            ...
            {
                outputText("you try and get her to her bedroll, but you are simply too exhausted.  Izma tells you not to worry about her and go get some rest; that she'll be okay in the grass.  You groggily agree as you set her down, telling her she better be ready for more sex if you want it; something she approves of as she hums and drifts off to sleep.");
            }
        }
        else
        {
    ...
    private var tdJerking : Bool = false;
    private var tdBlowing : Bool = false;
    
    //[Fuck Izma's ass (Dominant)]
    private function radarFucksIzmasAss() : Void
    {
        var x : Float = player.cockThatFits(izmaCapacity);
        var y : Float = player.cockThatFits2(izmaCapacity);
        clearOutput();
        if (y < 0)
        {
            sceneHunter.print("You could use another one!");
        }
        ...
        if (y >= 0)
        {
            outputText(" as you gently place your other dick against her labia");
        }
        outputText(".");
        ...
        if (y >= 0)
        {
            outputText("cocks against the unyielding ends of her anus and pussy, unleashing a weak whimper of bliss from under you.");
        }
        else
        {
            outputText("cock against the unyielding end of her anus, unleashing a weak whimper of bliss from under you.");
        }
    ...
    private function radarTentacleAnal() : Void
    {
        ...
        if (tenta < 5)
        {
            outputText("You need MORE tentacles.");
        }
        //Intro:
        outputText("Your eyes are unavoidably drawn to her well toned ass; the potential feel of her tight asshole against your " + cockDescript(x) + " is one you cannot pass up now.  Placing an authoritative hand on your beta's shoulder, you physically command her to get down on the ground... something the tigershark does all too willingly, flopping down to the ground on her stomach and waving her tail in your face; almost as if to taunt you and jokingly convey that \"<i>you can't have this.</i>\"  Her intent on what she wants you to do clear, you forcefully reach out and grab that smugly shaking tail of hers and pull hard, dragging her prone body along the ground to the base of your feet as you yank her crescent shaped appendage to your face.  Izma groans briefly from the friction burn and then giggles at the rough treatment.  Oh... she needs a good fucking in that asshole of hers, you think to yourself, and grin as you lower your body down to her rump.  Flush with desire, Izma glances over her shoulder and stares intently, eagerly awaiting the moment that your " + cockDescript(x) + " lights her body up like the night sky.");
        ...
        if (tenta >= 4)
        {
            ...
            if (tenta == 4)
            {
                outputText("a cock that needs");
            }
            else
            {
                outputText("cocks that need");
            }
    ...
    private function analFuckRadarEnding() : Void
    {
        ...
        {
            outputText("releases a maleficiently muffled moan");
        }
        ...
        else
        {
            ...
            else if (player.cumQ() < 10000)
            {
                ...
                if (tenta >= 3)
                {
                    outputText(", her smooth shark skin is also covered its white sticky coating");
                }
                outputText(".");
                if (tdBlowing)
                {
                    outputText(" Her face is drenched pretty well too, no place is left dry. While she tries to remove some of your seed from her eyes, you can clearly see just how good her mouth is filled.");
                }
                if (tdJerking)
                {
                    outputText(" But probably the messiest part of her body right now is her huge foot-long dick: already limp, it's wallowing in a pool of your mixed cum while traces of your orgasms are dripping from her groin.");
                }
            ...
            else
            {
                ...
                if (choice == 1)
                {
                    outputText("Staring daggers at you, the old rodent slowly wipes your wayward jizz from his face, turning to view his now ruined experiment and sighing.  Without a single word, he heads off to the lake to rinse himself clean.");
                }
                else if (choice == 2)
                {
                    outputText("Lost in meditation, Jojo doesn't seem to realize that he now has cum stuck in his fur until a few moments have passed.  Looking downward at himself as he wakes, he seems to ask himself if what clings to him is what he thinks it is.  In disgust, he wades off to the lake to wash it out of his coat, shaking his head at you as he trudges along.");
                }
                else if (choice == 3)
                {
                    outputText("A loud yell of anger rises across the camp as Marble suddenly shoots up into a panicked brushing of her body.  \"<i>DAMNIT SWEETIE!  DAMNIT!</i>\"  She howls as she hurries off to the river to get your semen off of her; no doubt disgusted at the fact that Izma's fluids are mixed in with yours and are all over her.");
                }
                else if (choice == 4)
                {
                    outputText("Staring back at you is a goo slathered mouse that can only be Amily.  Gazing at you behind her " + ((flags[kFLAGS.AMILY_NOT_FURRY] != null) ? "face" : "furry muzzle") + " she grits her teeth, looking like she wants to tear your head off.  All you can do is try and smile in a way that can diffuse the situation.  She doesn't buy it, but luckily she decides not to make a scene.");
                }
                else if (choice == 5)
                {
                    outputText("Almost like a child during a treasure hunt, Amily is busy running around and finding up all of the cum she can, greedily lapping it up as she goes.  Either she is unaware of the fact your cum has the taste and scent of Izma about it, or she doesn't care.  Either way, the " + ((flags[kFLAGS.AMILY_NOT_FURRY] != null) ? "mouse-girl" : "mouse-morph") + " seems content with 'cleaning' up after you.");
                }
                else if (choice == 6)
                {
                    outputText("Without having to even utter 'SLUT!', Jojo is pouncing on the globs of cum that have rained down on the camp site, hungrily lapping his master's sperm up.  \"<i>Good slut...</i>\" you whisper, giggling to yourself.");
                }
                else
                {
                    outputText("Your cum stained a lot of ground around you, you can see its ropes on the grass and stones. Good thing nobody got hit by it...");
                }
    ...
    private function backJizzShot() : Void
    {
        ...
        {
            outputText("two shafts ");
        }
        else
        {
            outputText("three shafts");
        }
        ...
        if (player.cumQ() < 500)
        {
            outputText("firing thin streams of semen from [eachCock], lathering Izma's luscious red body and making it glisten in the sun.");
        }
        //(Cum volume high:
        else if (player.cumQ() <= 1000)
        {
            outputText("streaming forth like fierce rain down a tree, repeatedly striking Izma across the face and breasts as your orgasm continues unabated for several moments.");
        }
        //(Cum volume very high:
        else
        {
    ...
    private function izmaDomsLatexyPartI() : Void
    {
        ...
        if (latexGirl.gooObedience() < 50)
        {
            outputText("  A look of annoyance flashes across her face as soon as she says it, like she didn't really want to say that so openly.");
        }
        else if (latexGirl.gooObedience() <= 80)
        {
            outputText("  A conflicted look flashes across her face as soon as the words slip out, but there's no taking them back.");
        }
        else
        {
            outputText("  A hungry look flashes in her eyes as soon as the words leave her mouth, and she licks her lips hungrily.");
        }
    ...
    private function izmaLatexySubmenu() : Void
    {
        ...
        menu();
        if (flags[kFLAGS.IZMA_X_LATEXY_DISABLED] == 1)
        {
            addButton(0, "KeepHerFed", izmaLatexyToggle);
        }
        else
        {
    ...
    private function izmaValeriaSubmenu() : Void
    {
        ...
        menu();
        if (flags[kFLAGS.IZMA_FEEDING_VALERIA] == 0)
        {
            addButton(0, "KeepHerFed", izmaValeriaToggle);
        }
        else
        {
            addButton(0, "No Feeding", izmaValeriaToggle);
        }
        addButton(4, "Back", izmaSexMenu);
    }
    //[No Feeding]
    ...
    public function mishapsLunaIzma() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.CampLunaMishaps1, 4, 1);
        }
        else
        {
            player.createStatusEffect(StatusEffects.CampLunaMishaps1, 0, 0, 0, 1);
        }
