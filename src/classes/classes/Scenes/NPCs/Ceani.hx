...
class Ceani extends Monster
{
    public function moveBleedingBite() : Void
    {
        ...
        if (!player.immuneToBleed())
        {
            ...
            {
                player.addStatusValue(StatusEffects.IzmaBleed, 1, bleeddura);
            }
            else
            {
                player.createStatusEffect(StatusEffects.IzmaBleed, SceneLib.combat.debuffsOrDoTDuration(bleeddura), 0, 0, 0);
            }
        }
    }
    ...
    public function moveAwesomeBlow() : Void
    {
        ...
        if (flags[kFLAGS.CEANI_LVL_UP] >= 4)
        {
            damage += eBaseStrengthDamage() * 0.3;
        }
        if (flags[kFLAGS.CEANI_LVL_UP] >= 8)
        {
            damage += eBaseStrengthDamage() * 0.3;
    ...
    public function new()
    {
        ...
        this.lustVuln = .8;
        this.gems = 50;
        this.drop = new ChainedDrop().add(consumables.ORCASUN, 0.7);
