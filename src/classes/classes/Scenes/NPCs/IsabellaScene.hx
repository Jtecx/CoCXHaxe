...
class IsabellaScene extends NPCAwareContent implements TimeAwareInterface
{
    //Isabella Flags:
    ...
    public var isabellaOffspringData : Array<Dynamic> = [];
    
    public static inline var OFFSPRING_HUMAN_BOYS : Int = 1;
    public static inline var OFFSPRING_HUMAN_GIRLS : Int = 2;
    public static inline var OFFSPRING_HUMAN_HERMS : Int = 3;
    public static inline var OFFSPRING_COWGIRLS : Int = 4;
    public static inline var OFFSPRING_COWFUTAS : Int = 5;
    ...
    public function timeChange() : Bool
    {
        ...
        //Isabella is angry at the player{
            ...
            if (flags[kFLAGS.ISABELLA_ANGRY_AT_PC_COUNTER] > 300)
            {
                flags[kFLAGS.ISABELLA_ANGRY_AT_PC_COUNTER] = 300;
            }
            if (flags[kFLAGS.ISABELLA_ANGRY_AT_PC_COUNTER] < 0)
            {
        ...
        if (flags[kFLAGS.ISABELLA_MILK_COOLDOWN] > 0)
        {
            ...
            if (flags[kFLAGS.ISABELLA_MILK_COOLDOWN] < 0)
            {
                flags[kFLAGS.ISABELLA_MILK_COOLDOWN] = 0;
            }
        }
        ...
        {
            if (flags[kFLAGS.FOUND_ISABELLA_AT_FARM_TODAY] == 1)
            {
                flags[kFLAGS.FOUND_ISABELLA_AT_FARM_TODAY] = 0;
            }
    ...
    {
        return as3hx.Compat.parseInt(isabellaOffspringData.length / 2);
    }
    ...
    //1: Human boys
    {
        ...
        var count : Int = 0;
        var i : Int = 0;
        while (i < isabellaOffspringData.length)
        {
            if (isabellaOffspringData[i] == type)
            {
                count++;
            }
            i += 2;
    ...
    public function isabellaGreeting() : Void
    {
        ...
        if (flags[kFLAGS.ISABELLA_MET] == 0)
        {
            ...
            {
                outputText("She's like Marble - she has a human face with horns and cow-like ears.");
            }
            else
            {
                outputText("She has a human face, but the top of her head is also adorned with a pair of stubby, bovine horns and floppy cow-ears.");
            }
            ...
            {
                if (flags[kFLAGS.ISABELLA_MET_SHORT_PC] < 0)
                {
                ...
                if (player.hasCock())
                {
                    ...
                    {
                        outputText("  The cow's eyes close, disappointment visible on her face when she sees the sheer size of your bulge.");
                    }
                }
        ...
        else if (flags[kFLAGS.ISABELLA_OKAY_WITH_TALL_FOLKS] > 0 && flags[kFLAGS.ISABELLA_MET_SHORT_PC] == 0 && player.tallness <= 78)
        {
            ...
            if (player.hasCock())
            {
                ...
                else
                {
                    ...
                    if (player.cocks[player.shortestCockIndex()].cockLength < 20)
                    {
                        outputText("large");
                    }
                    else
        ...
        else if (flags[kFLAGS.ISABELLA_MET_SHORT_PC] > 0 && flags[kFLAGS.ISABELLA_OKAY_WITH_TALL_FOLKS] == 0 && player.tallness > 78)
        {
            ...
            if (player.hasCock())
            {
                ...
                if (player.cocks[player.shortestCockIndex()].cockLength >= 9)
                {
                    outputText("before sighing wistfully.");
                }
                else
                {
        ...
        else
        {
            //Camp Meeting – Standard Repeat{
                ...
                if (player.hasCock())
                {
                    ...
                    if (player.cocks[player.shortestCockIndex()].cockLength >= 9)
                    {
                        outputText(", sighing wistfully. Seems like it's too big for her.");
                    }
                    else
                    {
    ...
    public function tryToTalkDownAngryCow() : Void
    {
        ...
        else
        {
            ...
            {
                outputText("You toss aside your [weapon] and");
            }
            else
            {
                outputText("You");
    ...
    public function nomOnMommaIzzysTits() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_isabella);
        var x : Float = player.cockThatFits(20);
        ...
        if (player.tallness > 78 || player.isTaur())
        {
            ...
            if (player.hasCock())
            {
                ...
                {
                    outputText("hard, barely constraining " + sMultiCockDesc() + ".");
                }
                else
                {
                    outputText("visibly from " + sMultiCockDesc() + ".");
                }
                outputText("  ");
            }
            if (player.hasVagina())
            ...
            if (isabellaAccent())
            {
                outputText("\"<i>Come closer, I do not bite.  Ve both know how very thirsty you are. Izabella vill sate you,</i>\" as she pulls the tightly stretched silk over the curve of her deliciously bronzed mounds.\n\n");
            }
            else
            {
                outputText("\"<i>Come closer, I don't bite.  We both know how thirsty you are.  Isabella will sate you,</i>\" as she pulls the tightly stretched silk over the curve of her deliciously bronzed mounds.\n\n");
            }
            ...
            {
                outputText("your horns");
            }
            else
            {
                outputText("the back of your head");
            }
            ...
            if (player.hasCock())
            {
                outputText("  " + SMultiCockDesc() + " twitches and drips from the intensity of the kiss, but you had completely forgotten about your sexual needs while you were drinking.");
            }
            else if (player.hasVagina())
            {
                outputText("  Your " + vaginaDescript(0) + " aches with need and desire from the intensity of the kiss, but you had completely forgotten it during the feeding.");
            }
            ...
            {
                outputText(" and stroke your wings");
            }
            ...
            if (player.hasCock() && x >= 0)
            {
                ...
                {
                    outputText("  S-she fucked you in your sleep?  That explains how good your dreams were.  On one hand you feel a little violated, but on the other you have to wonder how long this woman has held her camp against the demons with nothing to please her.");
                }
                else
                {
                    outputText("  It looks like she fucked you in your sleep again.  You wish you wouldn't fall asleep so soundly after drinking her milk so that you could contribute to the sex, but you wake so COMPLETELY SATED in every way that you know it's going to be hard to ever turn her down.");
                }
            ...
            else
            {
                ...
                if (isabellaAccent())
                {
                    outputText("\"<i>Did you enjoy mein snack?  I think ve both needed ze drink, no?</i>\" You nod, stand up stretch, feeling energized and awake.");
                }
                else
                {
                    outputText("\"<i>Did you enjoy the snack?  I think we both needed the drink, no?</i>\" You nod, stand up stretch, feeling energized and awake.");
                }
                //(-65 fatigue)
                fatigue(-65);
            }
        ...
        else
        {
            ...
            if (isabellaAccent())
            {
                outputText("\"<i>Vell, you are in luck then!  I have ze most delicious milk you'll find anywhere.  Come here little one, and Isabella vill give you all you need.</i>\"\n\n");
            }
            else
            {
                outputText("\"<i>Well, you're in luck then!  I have the most delicious milk you'll find anywhere.  Come here little one, and Isabella will give you all you need.</i>\"\n\n");
            }
            ...
            if (isabellaAccent())
            {
                ...
                if (!isabellaFollower())
                {
                    outputText(" on the vay back!");
                }
                else
                {
                    outputText(" vhile you are aus und über!");
                }
            ...
            else
            {
                ...
                if (!isabellaFollower())
                {
                    outputText(" on the way back!");
                }
                else
                {
                    outputText(" while you're out and about!");
                }
            ...
            if (player.hasCock() && x >= 0)
            {
                ...
                else
                {
                    ...
                    else if (model.time.hours < 4)
                    {
                        outputText("afternoon");
                    }
                    else
                    {
                        outputText("evening");
        ...
        if (flags[kFLAGS.ISABELLA_MILKED_YET] > 0)
        {
        ...
        if (rand(2) == 0)
        {
            outputText(player.modThickness(75, 4));
        }
        if (rand(2) == 0 && player.hasVagina())
        {
    ...
    {
        spriteSelect(SpriteDb.s_isabella);
        var x : Float = player.smallestCockIndex();
        clearOutput();
        outputText("You ");
        if (player.cor < 33)
        {
            outputText("blush hard and tell Isabella that she can lick if she wants to.");
        }
        else if (player.cor < 66)
        {
            outputText("blush and tell Isabella that she can definitely give you a lick.");
        }
        else
        {
            outputText("pose lewdly and trace a finger over your bulge as you inform Isabella just how happy you'd be to feel her tongue on your " + cockDescript(x) + ".");
        }
        ...
        if (isabellaAccent())
        {
            outputText("You like, yes?  Mmmmhmm, Izabella knows.  I can see it on your face.  You aren't one of those perverts, are you?  I think you might be, but ve vill have to see, yes?  If you are one of them you'll be squirting all over Isabella's tongue in no time.  I dearly hope you'll prove me wrong.");
        }
        else
        {
            outputText("You like, yes?  Mmmmhmm, Izabella knows.  I can see it on your face.  You aren't one of those perverts, are you?  I think you might be, but we will have to see, yes?  If you're one of them you'll be squirting all over my tongue in no time.  I dearly hope you'll prove me wrong.");
        }
        ...
        if (player.cor < 50)
        {
            outputText("before blushing, ashamed by your wanton behavior but not really wanting it to end.");
        }
        else
        {
            outputText("before sighing blissfully, absorbed in the feel of her tongue, never wanting it to end.");
        }
        ...
        sceneHunter.print("Lust check: 50%")  //(Low sensitivity success!)  ;
        ...
        if (player.lust < 0.5 * player.maxLust())
        {
            ...
            if (isabellaAccent())
            {
                outputText("Isabella grunts and pulls back, pulling her horns from your pleasure-weakened fingers and panting heavily.  She groans, \"<i>Nein, I cannot believe it!  Such a small, hard little cock and I couldn't make it spurt, not even vith mein special techniques!</i>\"  She looks up at you with her flushed, breathy face and coos, \"<i>You are NOT a pervert after all.  Not a " + player.mf("boy", "maid") + ", but a " + player.mf("man", "woman") + " with a beautiful, succulent little cock for Isabella to suck.  How lucky I am!</i>\"\n\n");
            }
            else
            {
                outputText("Isabella grunts and pulls back, pulling her horns from your pleasure-weakened fingers and panting heavily.  She groans, \"<i>No, I can't believe it!  Such a small, hard little cock and I couldn't make it spurt, not even with my special techniques!</i>\"  She looks up at you with her flushed, breathy face and coos, \"<i>You aren't a pervert after all.  Not a " + player.mf("boy", "maid") + ", but a " + player.mf("man", "woman") + " with a beautiful, succulent little cock for Isabella to suck.  How lucky I am!</i>\"\n\n");
            }
            ...
            if (player.hasBalls())
            {
                outputText("She cups your [balls] and begins caressing the twitching orbs, giving them gentle squeezes each time the pleasure forces them to involuntarily contract towards your groin.");
            }
            else if (player.hasVagina())
            {
                ...
                {
                    outputText("gentle squeezes");
                }
                else
                {
                    outputText("firm pumps");
                }
                outputText(".");
            }
            else
            ...
            {
                outputText("eruptions");
            }
            outputText(" of jism splatter from your spasming cum-tube.  You ");
            if (player.cumQ() < 250)
            {
                outputText("empty the last of your load all over her tongue and pull out.");
            }
            else if (player.cumQ() < 500)
            {
                outputText("empty the last of your load into her belly and leak all over her waiting tongue as you pull out.");
            }
            else if (player.cumQ() < 1500)
            {
                outputText("empty huge batches of spunk into her belly until it's gurgling and full, and as you pull out you dribble enough to completely soak her tongue.");
            }
            else
            {
                outputText("empty enough cum inside the cow-girl for it to fill her belly and back up her throat.  By the time you pull out, she's got runners of sperm leaking from both sides of her mouth and dripping onto her tits, staining her dusky skin white.");
            }
            outputText("\n\n");
            ...
            isabellaFollowerScene.isabellaAffection(12);
        }
        ...
        else
        {
            ...
            if (player.hasSheath())
            {
                outputText("around your sheath");
            }
            else if (player.hasBalls())
            {
                outputText("above your balls");
            }
            else
            {
            ...
            if (player.hasBalls())
            {
                outputText("her hand stroking and gently squeezing at your [balls] as if she could milk a bigger load from them that way.");
            }
            else if (player.cockTotal() > 1)
            {
                outputText("her hand stroking and squeezing your neglected " + cockDescript(1) + " almost as an afterthought.");
            }
            else
            {
                outputText("her hand stroking in an effort to milk your load from you.");
            }
            outputText("\n\n");
            ...
            isabellaFollowerScene.isabellaAffection(7);
        }
    ...
    public function volunteerToSlurpCowCunt() : Void
    {
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>You mean to lick me, down zere?</i>\"  Once again, Isabella preempts your words, though this time she does it with an uplifted skirt-hem and slowly-spreading thighs.  The shadow of the garment conceals the treasure inside, but a faint, feminine smell hits your nose, assuring you that she's looking forward to it as much as you.\n\n");
        }
        else
        {
            outputText("\"<i>You mean to lick me there?</i>\"  Once again, Isabella preempts your words, though this time she does it with an uplifted skirt-hem and slowly-spreading thighs.  The shadow of the garment conceals the treasure inside, but a faint, feminine smell hits your nose, assuring you that she's looking forward to it as much as you.\n\n");
        }
        ...
        if (player.cor < 33)
        {
            outputText("Hesitantly, you start to lean closer and closer until you're mere inches from the cow-girl's slick box.  The air practically fogs with her anticipation.  A strong, impatient hand grabs you by the back of the head and pushes you forward, burying your nose and lips into her wet, squelching lips.");
        }
        else if (player.cor < 66)
        {
            outputText("Eagerly, you start to lean further and further forward until you're less than an inch from the cow-girl's slick folds, and the air fogs with lusty anticipation.  Before you can dive in, her hand grips the back of your head and FORCES you inside her.  Her greedy lips swallow your nose and lips with one wet squelch.");
        }
        else
        {
            outputText("Unabashedly, you dive right into the cow-girl's lust-slicked snatch.  Her greedy lips swallow your nose and lips with one wet squelch, but the cow-girl doesn't seem quite satisfied until her hand is on the back of your head, mashing your [face] roughly over her sex.");
        }
        ...
        if (player.cor < 33)
        {
            outputText("You blush at her words and hope she's just talking dirty.");
        }
        else if (player.cor < 66)
        {
            outputText("You blush at her words and wonder what it would be like if she wasn't talking dirty.");
        }
        else
        {
            outputText("Despite knowing she's probably just getting into the moment, you see the appeal of the idea, but perhaps it would be hotter if the roles were reversed?");
        }
        ...
        if (player.hasCock())
        {
            ...
            else
            {
                ...
                if (isabellaAccent())
                {
                    outputText("vould like to return ze favor.</i>\"");
                }
                else
                {
                    outputText("would like to return the favor.</i>\"");
                }
        ...
        else
        {
            ...
            if (!isabellaAccent())
            {
                outputText("Seeing the ardent desire your sexual service has so visibly inspired in your body - in your slick, ready cunt and erect nipples - the cow-girl smiles slightly, and asks, \"<i>Perhaps you would like me to return the favor?  It seems only fair...</i>\"");
            }
            else
            {
                outputText("Seeing the ardent desire your sexual service has so visibly inspired in your body - in your slick, ready cunt and erect nipples - the cow-girl smiles slightly, and asks, \"<i>Perhaps you vould like me to return ze favor?  It seems only fair...</i>\"");
            }
            //[Leave] [Get Cowlicked]
    ...
    public function IsabellaWinsAndSpanks() : Void
    {
        ...
        if (player.tallness <= 78)
        {
            if (player.HP < 1)
            {
                outputText("You collapse at Isabella's feet, nearly senseless from all the damage you've taken.");
            }
            else
            {
                outputText("You collapse at Isabella's feet, masturbating pathetically as she glares down at you.");
            }
            ...
            if (player.hasPerk(PerkLib.Masochist))
            {
                ...
                if (player.hasCock())
                {
                    outputText("squirt pre-cum ");
                }
                else if (player.hasVagina())
                {
                    outputText("slick your thighs ");
                }
                else
                {
                    outputText("tremble ");
            ...
            if (isabellaAccent())
            {
                outputText("Isabella grunts, \"<i>Look at you, acting like one of ze demons!  Now Izabella vill have to beat ze corruption out of you!</i>\"  You groan ");
            }
            else
            {
                outputText("Isabella grunts, \"<i>Look at you, acting like one of the demons!  Now I will have to beat the corruption out of you!</i>\"  You groan ");
            }
            if (player.hasPerk(PerkLib.Masochist))
            {
                outputText("excitedly, wondering just how many more smacks you'll get to take");
            }
            else
            {
                outputText("piteously");
            }
            ...
            if (player.hasPerk(PerkLib.Masochist))
            {
                outputText(" while you swoon and moan, wiggling your rump");
            }
            ...
            if (!player.hasPerk(PerkLib.Masochist))
            {
                outputText("draws a scream from your lips, pushing you past the bounds of consciousness.  Isabella hefts your limp form like a wet noodle and grasses sway behind you as you're carried off.");
            }
            else
            {
                ...
                if (player.hasCock())
                {
                    ...
                    if (player.cumQ() >= 50)
                    {
                        outputText(", moistening the fur");
                    }
                    if (player.cumQ() >= 250)
                    {
                        outputText(" and dripping down to her hooves");
                    }
                    if (player.cumQ() >= 1000)
                    {
                        outputText(" until you've created a puddle underneath her");
                    }
                    outputText(".");
                }
                if (player.hasVagina())
                {
                    ...
                    {
                        outputText("squirts, splashing girl-cum over the cow's ankle and hoof");
                    }
                    outputText(".");
                }
            ...
            if (isabellaAccent())
            {
                outputText("\"<i>Zere zere little one, just lie zere and drink.  Ve'll forget about all that earlier nastiness.  I'd feel bad leaving you out here all alone to be brutalized and raped by some monster!</i>\"\n\n ");
            }
            else
            {
                outputText("\"<i>There there little one, just lie there and drink.  we'll forget about all that earlier nastiness.  I'd feel bad leaving you out here all alone to be brutalized and raped by some monster!</i>\"\n\n ");
            }
        ...
        else
        {
            ...
            if (player.lust >= player.maxOverLust())
            {
                outputText("You start trying to masturbate yourself, but the angry cow will have none of it.  ");
            }
            outputText("SMACK!  A powerful impact slams into your [ass], making you gasp out in pain");
            ...
            {
                outputText("Isabella grunts, \"<i>Look at you, acting like one of thee demons!  Now I will have to beat the corruption out of you!</i>\"  You groan ");
            }
            if (!player.hasPerk(PerkLib.Masochist))
            {
                outputText("piteously");
            ...
            if (player.hasPerk(PerkLib.Masochist))
            {
                outputText(" draws a scream from your lips, pushing you past the bounds of consciousness.  Isabella hefts your limp form heavily, dragging you through the dirt as your eyes close.");
            }
            else
            {
                outputText(" pushes you past your limit.");
                if (player.hasCock())
                {
                    ...
                    if (player.cumQ() > 100)
                    {
                        outputText(", moistening your hair");
                    }
                    if (player.cumQ() > 250)
                    {
                        outputText(" and dripping down to the ground");
                    }
                    if (player.cumQ() > 1000)
                    {
                        outputText(" until you've created a puddle around yourself");
                    }
                    outputText(".");
                }
                if (player.hasVagina())
                {
                    ...
                    {
                        outputText("squirts, splashing girl-cum over the dirt");
                    }
                    outputText(".");
                }
            ...
            if (isabellaAccent())
            {
                outputText("\"<i>Zere zere big " + player.mf("boy", "girl") + ", just lie zere and drink.  Ve'll forget about all that earlier nastiness.  I'd feel bad leaving you out here all alone – you'd probably turn into one of ze monsters!</i>\"\n\n");
            }
            else
            {
                outputText("\"<i>There there big " + player.mf("boy", "girl") + ", just lie there and drink.  we'll forget about all that earlier nastiness.  I'd feel bad leaving you out here all alone – you'd probably turn into one of the monsters!</i>\"\n\n");
            }
        ...
        {
            sceneHunter.print("Check failed: you need a short cock!");
        }
        cleanupAfterCombat();
    }
    ...
    public function isabellaRapesYouWithHerAss(x : Int) : Void
    {
        ...
        if (player.lust >= player.maxOverLust())
        {
            ...
            if (isabellaAccent())
            {
                outputText("<i>Oooh, so eager to be taught a lesson, ja?  Very vell, Izabella vill give you your punishment!</i>\"\n\n");
            }
            else
            {
                outputText("<i>Oooh, so eager to be taught a lesson, huh?  Very vell, I'll give you your punishment!</i>\"\n\n");
            }
        }
        else
        {
            ...
            if (isabellaAccent())
            {
                outputText("Already you vant your lesson, ja?  Very vell, Izabella can punish the naughty little [boy].</i>\"\n\n");
            }
            else
            {
                outputText("Already want your lesson, huh?  Very vell, I can punish you, you naughty little [boy].</i>\"\n\n");
            }
        }
        ...
        if (player.cockTotal() > 1)
        {
            if (player.cockTotal() > 2)
            {
                ...
                if (isabellaAccent())
                {
                    outputText("vould you vant so many penises?</i>\"");
                }
                else
                {
                    outputText("would you want so many penises?</i>\"");
                }
            }
            else
            {
                ...
                if (isabellaAccent())
                {
                    outputText("ja?  Vhy are you so eager to be punished?</i>\"");
                }
                else
                {
                    outputText("huh?  Why are you so eager to be punished?</i>\"");
                }
            }
        }
        outputText("\n\n");
        ...
        if (isabellaAccent())
        {
            outputText("Nein!  You are being punished!");
        }
        else
        {
            outputText("No! You're being punished!");
        }
        ...
        if (isabellaAccent())
        {
            outputText("I vant you to apologize for being so wicked and nasty from the bottom of your heart.  Until you do, I'll keep squeezing and rubbing with mein heiny!");
        }
        else
        {
            outputText("I want you to apologize for being so wicked and nasty - from the bottom of your heart! Until you do, I'll keep squeezing and rubbing!");
        }
        ...
        if (player.hasBalls())
        {
            outputText(" or the tightening of your " + sackDescript() + " as it pulls your " + ballsDescriptLight());
        }
        ...
        if (isabellaAccent())
        {
            outputText("No, zat is not sorrow.  You are sorry you can't cum, not sorry for being such a beast.  You must convince me!");
        }
        else
        {
            outputText("No, that's not sorrow. You're sorry that you can't cum, not for being such a beast.");
        }
        ...
        if (player.cockArea(x) < 38)
        {
            ...
            if (player.cockTotal() > 1)
            {
                outputText(", smushing against your other dick");
            }
            if (player.cockTotal() > 2)
            {
                outputText("s");
            ...
            if (player.hasBalls())
            {
                outputText("Involuntary muscles contract, tugging your " + sackDescript() + " up against your crotch while your [balls] visibly contract, loading your body full of liquid lust that's about to explode.");
            }
            else
            {
                outputText("Involuntary muscles contract, clenching inside you as your prostate loads your urethra with liquid lust that's about to explode.");
            }
            ...
            if (player.cumQ() >= 1000)
            {
                ...
                if (player.cumQ() >= 1500)
                {
                    outputText(", but your " + cockDescript(x) + " won't let up.  She cries and moans, jism pouring out around your shaft");
                }
                if (player.cumQ() >= 2500)
                {
                    outputText(", yet you manage to keep going, drooling thick spunk until even the ground is soaked with it");
                }
            }
            outputText(".");
            if (player.cockTotal() > 1)
        ...
        else
        {
            ...
            if (player.cockTotal() > 1)
            {
                if (player.cockTotal())
                {
                    outputText("Even though they're being ignored, each time her cheeks crush against your other cocks, they squeeze out a few spurts of pre-cum.");
                }
                else
                {
                    outputText("Even though they're being ignored, each time her cheeks crush against your other cock, it squeezes out a spurt of pre-cum.");
                }
            }
            outputText("\n\n");
            ...
            if (player.hasBalls())
            {
                outputText("Your " + sackDescript() + " pulls tight against your body, each of your [balls] quivering and pushing its load through you.");
            }
            else
            {
                outputText("Your body seems to pull tight, like a violin string, and you feel your organs quivering and working to push your load through you.");
            }
            ...
            if (player.cumQ() >= 1000)
            {
                outputText("  Ropes of it drip from Isabella's crimson locks, plastering her hair to her neck and dripping onto her shirt.  All around you the dirt has turned to a slimy, soupy mud, nearly white in color from your copious leavings.");
            }
            if (player.cumQ() >= 2000)
            {
                outputText("  The spooge completely soaks you both, surprising even you with its volume and quantity.");
            }
            outputText("\n\n");
    ...
    public function IsabellaPostSpankFeedSex() : Void
    {
        ...
        if (isabellaAccent())
        {
            outputText(" \"<i>Das is good [boy]!  Don't move!  Iz impolite to interrupt your elder's pleasure, and your tiny cock is so small and unique.  You vill lie there until momma has had her fill, ja?</i>\" To emphasize her point she puts a hand ");
        }
        else
        {
            outputText(" \"<i>There's a good [boy]!  Don't move!  Its impolite to interrupt your elder's pleasure, and your tiny cock is so small and unique.  You will lie there until momma has had her fill, yes?</i>\" To emphasize her point she puts a hand ");
        }
        if (player.biggestTitSize() < 1)
        {
            outputText("on your chest");
        }
        else
        {
            outputText("in between your " + allBreastsDescript());
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>MooooOOOOOooooooh jaaaaaaaaaaa!</i>\"");
        }
        else
        {
            outputText("\"<i>MooooOOOOOooooooh yeeeeeeesssss!</i>\"");
        }
        ...
        if (player.cumQ() < 50)
        {
            outputText("spurting into Isabella's hungry, constricting snatch.");
        }
        else if (player.cumQ() < 250)
        {
            outputText("spurting into Isabella's suddenly-tight cunny with such thick streams that drops of it run from her lips.");
        }
        else if (player.cumQ() < 1000)
        {
            outputText("bursting into Isabella's constricting cunny and soaking every inch of her passage with your copious spooge.");
        }
        else if (player.cumQ() < 2000)
        {
            outputText("bursting into Isabella's constricting cunt, filling her womb, and leaving her belly with a little bit of a spunk-paunch.");
        }
        else
        {
            outputText("exploding into Isabella's constricting cunt in huge waves.  You feel her passage fill around you, then her womb, and then the next pump bulges her belly, giving her a spunk-paunch.  She moans as each successive deposit of seed fills her until her belly is pregnant with spooge, and her nether-lips are glazed white and dripping.");
        }
        if (player.cumQ() >= 10000)
        {
            outputText("So much leaks out that the mud lightens and thickens, taking on a cum-like viscosity.");
        }
        outputText("\n\n");
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Das vas a very good [boy]!  I hope I taught you some manners.  Maybe come visit me some time, but be polite for me or I'll have to give you another spanking!</i>\"");
        }
        else
        {
            outputText("\"<i>You're a very good [boy]! I hope I taught you some manners. Maybe you should come and visit me some time, but be polite for me or I'll have to give you another spanking!</i>\"");
        }
    ...
    public function defeatIsabella() : Void
    {
        ...
        {
            if (flags[kFLAGS.ISABELLA_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.ISABELLA_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.ISABELLA_DEFEATS_COUNTER] = 1;
            }
            if (flags[kFLAGS.ISABELLA_LVL_UP] < 16 && flags[kFLAGS.ISABELLA_DEFEATS_COUNTER] >= flags[kFLAGS.ISABELLA_LVL_UP] + 6)
            {
                var vAdd : Int = as3hx.Compat.parseInt(player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * flags[kFLAGS.ISABELLA_DEFEATS_COUNTER]);
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers1))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 2, vAdd);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers1, 0, vAdd, 0, 0);
                }
        ...
        if (monster.statusEffectv1(StatusEffects.Sparring) == 2)
        {
            ...
            {
                outputText("damage-dazed");
            }
            else
            {
                outputText("arousal-addled");
            }
            ...
            if (monster.HP <= monster.minHP())
            {
                outputText("recuperate.");
            }
            else
            {
                outputText("take care of her needs (or be taken care of).");
            }
            cleanupAfterCombat();
            return;
        }
        ...
        if (monster.HP < 1)
        {
            if (isabellaAccent())
            {
                outputText("Vhy must you be so like them?  You act like a demon!  Stay away from mein milk!");
            }
            else
            {
                outputText("Why must you be so like them? You act just like a demon! Stay away from my milk!");
            }
        }
        else if (isabellaAccent())
        {
            outputText("I didn't vant this!  ...but, please, I have so much milk... drink my moOOOO-ilk!");
        }
        else
        {
            outputText("I didn't want this! Please, I have so much milk... dink my moOOO-ilk!");
        }
        outputText("</i>\"  ");
        if (monster.HP <= monster.minHP())
        {
            outputText("She acts so haughty, but she can't hide how her nipples are tenting her sheer top.");
        }
        else
        {
            outputText("She acts like all she needs is a milking, but you can smell the dampness she's trickling from 'down under'.");
        }
        if (player.lust < 33)
        {
        ...
        menu();
        addButton(0, "Lactation69", victoryLactation69).disableIf(player.biggestLactation() < 1, "You don't lactate enough!");
        addButton(1, "Buttsex", PCVictoryOnIsabellaButtsex).disableIf(player.cockThatFits(monster.analCapacity()) < 0, "Req. a cock fitting " + monster.analCapacity() + " capacity.");
        ...
        addButton(3, "Vaginal", vaginalProdNPokeIsabella).disableIf(player.cockThatFits(monster.vaginalCapacity()) < 0, "Req. a cock fitting " + monster.vaginalCapacity() + " capacity.");
        addButton(4, "Big Titfuck", tooBigVictoryTittyFuckingFuntimesWithMilk).disableIf(!player.hasCock() || player.biggestCockArea() < 70, "Req. a cock with area larger than 70.");
        addButton(5, "Small Titfuck", tinyVictoryTittyFuckingFuntimesWithMilk).disableIf(!player.hasCock() || player.shortestCockLength() >= 9, "Req. a cock shorter than 9 inches.");
    ...
    public function victoryLactation69() : Void
    {
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Moo-milk?  Vell, it does sound kind of ni...</i>\"");
        }
        else
        {
            outputText("\"<i>Moo-milk? Well it does sound kind of ni...</i>\"");
        }
        ...
        if (player.hasCock())
        {
            outputText("squeeze " + sMultiCockDesc() + " gently, feeling the heavy, turgid mass pulsating with each beat of your heart.");
        }
        else if (player.hasVagina())
        {
            outputText("rub your fingers over the puffy lips of your pussy, feeling your vulva spread to give the juice-lubed digits access.");
        }
        else
        {
            outputText("rub your fingers up your taint to your clenched pucker, circling the clenching asshole with a digit before you bury it inside.");
        }
        outputText("\n\n");
        ...
        if (player.breastRows.length > 1)
        {
            outputText(", while your other " + breastDescript(1) + " plasters her crimson hair into her scalp");
        }
        ...
        if (player.hasVagina())
        {
            outputText("  Of course, you're pumping at your own quim nearly as hard, but the messy cow-girl's efforts still manage to drown you out.");
        }
        else if (player.hasCock())
        {
            outputText("  Of course, you're pumping at " + sMultiCockDesc() + " just as hard, but the sounds of your fapping are easily drowned out by the enthusiastic cow-girl.");
        }
        else
        {
            outputText("  Of course, you're fingering your " + assholeDescript() + " just as hard, but that muscular hole barely makes a sound as you violate it.");
        }
        ...
        {
            outputText("  Isabella thrusts her tongue at your " + nippleDescript(0) + ", burrowing it deep into the unusual passage to get at the last of your liquid bounty and inadvertently intensifying your pleasure.  Your arm gives out and you drop on top of her, burying her in tit while hers serve as your pillows.");
        }
        //(Cock & Puss)
        if (player.gender == 3)
        {
            if (player.wetness() == 5)
            {
                outputText("  The juices that splatter from your " + vaginaDescript() + " almost seems an afterthought, but when you cum, you cum <i>allll</i> over.");
            }
            else if (player.wetness() > 3)
            {
                outputText("  The juices that drip from your " + vaginaDescript() + " almost seems an afterthought, but when you cum, you cum all over.");
            }
            else
            {
                outputText("  The clenching of your " + vaginaDescript() + " almost seems an afterthought, but when you cum you cum all over.");
            }
        }
        //(Cock)
        if (player.hasCock())
        {
            ...
            if (player.hasBalls())
            {
                outputText(", your [balls] pulling tight against your groin");
            }
            ...
            if (player.cumQ() >= 1000)
            {
                outputText("  You squirt and drip for ages, making a nice big puddle around both of you.");
            }
        }
        //(Puss only)
        if (player.gender == 2)
        {
            ...
            if (player.wetness() >= 4)
            {
                outputText("Juice squirts from your hole, splattering onto the dirt.");
            }
            else if (player.wetness() >= 2)
            {
                outputText("Girlcum drips from the hole, absorbing into the dirt.");
            }
            else
            {
                outputText("Girlcum soaks your lips, filling the air with female lust.");
            }
        ...
        if (player.gender == 0)
        {
            outputText("  The constant fingering of your " + assholeDescript() + " finally comes to fruition, sending bolts of pleasure from your anal passage.  The star clenches tight around your digits, contracting involuntarily around the invader.");
        }
        outputText("\n\n");
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Zat vas nice... a good way to cool down angry hearts.  Let's... do zis next time ve fight!</i>\"");
        }
        else
        {
            outputText("\"<i>That was nice... a good way to cool down angry hearts. Let's... do this next time we fight!</i>\"");
        }
    ...
    public function PCVictoryOnIsabellaButtsex() : Void
    {
        ...
        var y : Float = player.cockThatFits2(monster.vaginalCapacity());
        clearOutput();
        if (y < 0)
        {
        ...
        if (monster.HP <= monster.minHP())
        {
            outputText("She struggles to comply, heaving her hefty body until she's wobbling on all fours, nearly falling into the dirt.");
        }
        else
        {
            outputText("She struggles to comply, pulling her hands away from her erogenous zones as she wobbles onto shaky hands and knees.");
        }
        ...
        if (monster.HP <= monster.minHP())
        {
            outputText(", forgetting her wounds between the sight of your naked body and her lewd, compromised position");
        }
        else
        {
            outputText(" lustily, begging to be penetrated with her body while her voice tries to make love to your ears");
        }
        if (isabellaAccent())
        {
            outputText(".  \"<i>Vould you... put it inside me?  Please, I'm so sorry I didn't let you ravish me.  Take me, violate me vith your spear of lust!</i>\"");
        }
        else
        {
            outputText(".  \"<i>Would you... put it inside me? Please, I'm so sorry I didn't let you ravish me. Take me, violate me with your spear of lust!</i>\"");
        }
        ...
        if (player.cockTotal() > 1)
        {
            ...
            if (y >= 0)
            {
                outputText(", and your " + cockDescript(y) + " may as well go into that sloppy cow-cooch");
            }
            outputText(".  ");
        }
        ...
        if (y >= 0)
        {
            outputText(" and other " + player.cockHead(y) + " pressing against her holes");
        }
        else
        {
            outputText(" pressing against her dark pucker");
        }
        ...
        if (y >= 0)
        {
            outputText(" while her pussy sheaths your other member in dick-melting warmth");
        }
        outputText(".\n\n");
        ...
        if (y >= 0)
        {
            outputText("contracts around your " + cockDescript(y) + " and squeezes a flow of heated pussy-juice over the penis, cocooning it with enough warm heat and wetness to make you feel as if you're melting.");
        }
        else
        {
            outputText("spews out a few strands of girl-juice that cling to your [legs], hanging and stretching with every sticky slap against her body.");
        }
        ...
        if (y < 0)
        {
            outputText("  Though you could never impregnate such an orifice, your instinct to fill her tightness with male essence has taken over, and you won't be satisfied until her asshole is leaking your seed.");
        }
        else
        {
            outputText("  Though you know that at least half your load will be wasted, your instinct to fill her has taken over, and you won't be satisfied until every single one of the cow-cunt's vacancies are leaking sperm.");
        }
        ...
        if (y >= 0)
        {
            outputText("Your " + cockDescript(y) + " emulates its brother, shooting ropes of semen into the cow-girl's drippy pussy.  ");
        }
        ...
        if (player.cumQ() >= 750)
        {
            ...
            {
                outputText("  White goo sprays out from her flooded backside, forced out by the pressure of your incessant jizz-jets.  By the time you finish, her belly looks pregnant with your goo, but her blissful, cross-eyed expression shows just how little she minds.");
            }
        }
        outputText("\n\n");
    ...
    public function victoryAgainstIzzzzzySixtyNine(x : Int) : Void
    {
        ...
        {
            outputText("plumpest, juiciest set of pussy lips you've ever had the chance to lick.");
        }
        else
        {
            outputText("plumpest, juiciest vulva you've seen on a pussy since you met Edryn.");
        }
        ...
        if (x < 0)
        {
            outputText("nuzzle your " + vaginaDescript(0) + ", smearing your vulva with her lips even she spreads the folds to access your " + clitDescript());
        }
        else if (player.cockArea(x))
        {
            outputText("swallow your " + cockDescript(x) + " whole, plying her smooth, spit-covered tongue over every inch of it");
        }
        else
        {
            outputText("lick your " + cockDescript(x) + ", fondling it with her long, smooth tongue in ways that no human could");
        }
        outputText(".\n\n");
        ...
        if (x < 0)
        {
            ...
            {
                outputText("struggling with the cock-like appendage");
            }
            else if (player.clitLength >= 2)
            {
                outputText("suckling the large button with slow, measured inhalations");
            }
            else
            {
                outputText("devouring the tiny bud");
            }
            outputText(" even as the smooth skin of her tongue inadvertently caresses it.  You writhe against her with your juices ");
            if (player.wetness() < 3)
            {
                outputText("coating her tongue");
            }
            else if (player.wetness() < 4)
            {
                outputText("sloppily leaking everywhere");
            }
            else
            {
                outputText("squirting each time she hits a particularly sensitive spot");
            }
        ...
        else if (player.cockArea(x) < 38)
        {
            ...
            if (player.cockTotal() > 1)
            {
                ...
                if (player.cockTotal() > 2)
                {
                    outputText("s as they smear her cheeks, focusing on this one, perfectly-matched specimen.");
                }
                else
                {
                    outputText(" as it smears her cheeks, focusing on this one, perfectly-matched specimen.");
                }
            }
        }
        //(WANGS – no fit)
        else
        {
            ...
            {
                outputText(", getting stuck at the medial ring");
            }
            else if (player.cocks[x].cockType == CockTypesEnum.DEMON)
            {
                outputText(", slowed by all the nubs");
            }
            else if (player.cocks[x].cockType == CockTypesEnum.CAT)
            {
                outputText(", slowed by all the spines");
            }
            else if (player.cocks[x].cockType == CockTypesEnum.TENTACLE)
            {
                outputText(", stopping under the mushroom-like head");
            }
            else if (player.cocks[x].cockType == CockTypesEnum.ANEMONE)
            {
                outputText(", giggling from the 'stings' your anemone-like prick gave her");
            }
            else if (player.hasKnot(x))
            {
                outputText(", struggling with the knot");
            }
            else
            {
                outputText(", rubbing her mouth against your urethral bulge");
            }
        ...
        if (player.hasMuzzle())
        {
            outputText("It gives you an idea, and you shift and close your mouth.  A moment later you plunge down, burying your muzzle deep inside the sloppy cunt.  It stretches pleasantly around you, and you feel moans vibrate through your crotch.  ");
        }
        else
        {
            outputText("Her clit bumps your nose, and you decide it's time to subdue Isabella once and for all.  You suck the bulb into your mouth and raise one hand.  Then, with a simultaneous blur of motion, you plunge your fist into her open gash and attack her clit with a frenzied series of licks.  You feel moans vibrate through your groin in response.  ");
        }
        ...
        else if (player.cockArea(x) < 38)
        {
            ...
            if (player.cumQ() >= 250)
            {
                ...
                if (player.cumQ() >= 500)
                {
                    outputText(", but you keep cumming, pumping more and more spooge inside her oral cavity with every eruption");
                }
                if (player.cumQ() >= 1000)
                {
                    outputText(".  She gives up after her belly fills and pulls back to let you blow the rest on her face");
                }
                if (player.cumQ() >= 1500)
                {
                    outputText(".  Her tan visage is coated in a generous helping of spunk when you finish");
                }
            }
            else
            {
                outputText("your entire load");
            }
            outputText(".");
        }
        //(Dick + No fit)
        else
        {
            ...
            if (player.cumQ() > 250)
            {
                ...
                if (player.cumQ() > 500)
                {
                    if (player.cumQ() > 1000)
                    {
                        outputText(", and somehow, your orgasm drags on, creating a lake of spunky mud for Isabella to lie in");
                    }
                    else
                    {
                        outputText(".  Her top clings to her, revealing every curve of her chest and the many tips of her exotic nipples");
                    }
                }
                outputText(".");
            }
        ...
        if (x >= 0)
        {
            outputText("twitching as your member disentangles itself from her tongue.");
        }
        else
        {
            outputText("twitching weakly as your clit catches her tongue one last time.");
        }
    ...
    public function tooBigVictoryTittyFuckingFuntimesWithMilk() : Void
    {
        ...
        if (player.lust >= (player.maxLust() * 0.7))
        {
            outputText("pulsate with your raging lust");
        }
        else
        {
            outputText("slowly fill with blood from your growing lust");
        }
        ...
        {
            outputText(" since there's no way you'll be able to cram it in her cunt or asshole.\n\n");
        }
        else
        {
            outputText(".\n\n");
        ...
        if (isabellaAccent())
        {
            outputText("The cow-girl whines, \"<i>B-but I don't vant it in mein milkers!  Your thing... it is big and gross and nasty, like a demon's!</i>\"\n\n");
        }
        else
        {
            outputText("The cow-girl whines, \"<i>B-but I don't want that!  Your cock... it's big and gross... and nasty, like a demon's!</i>\"\n\n");
        }
        
        outputText("\"<i>");
        if (player.cor < 33)
        {
            outputText("I'm sorry, but you're too hot to resist, and it's better I do this than try to force it in a hole that could never accept it,");
        }
        else if (player.cor < 66)
        {
            ...
            if (monster.lust >= monster.maxOverLust())
            {
                outputText("You're so drippy that you want this anyway,");
            }
            else
            {
                outputText("You lost and you've got to deal with the consequences,");
            }
        }
        else
        {
            outputText("Tough shit,");
        }
        ...
        if (player.cor < 33)
        {
            outputText("You take care to remove it without damaging the garment, as if respect for property will make up for slaking your lusts on her unwilling body.");
        }
        else if (player.cor < 66)
        {
            outputText("You remove the garment without much care for its condition, not damaging it too much as you wrench it free.");
        }
        else
        {
            outputText("You tear off the offending garment without a single care for the cow-girl's property.");
        }
        ...
        if (player.cocks[x].cockLength < 50)
        {
            outputText("burying the top few inches of your length into her mouth");
        }
        else
        {
            outputText("pushing your cock across her puckered lips and past her forehead");
        }
        ...
        if (player.cocks[x].cockLength < 50)
        {
            outputText("to her mouth");
        }
        else
        {
            outputText("to smother her face in heavy dick-flesh");
        }
        ...
        if (player.hasBalls())
        {
            outputText("  Your balls pull tight to your loins, accompanied by a burst of warmth");
        }
        else
        {
            outputText("  A bubble of warmth grows in your loins");
        }
        ...
        if (player.cocks[x].cockLength < 50)
        {
            outputText("leans forward to swallow more of your cock just before");
        }
        else
        {
            outputText("leans back to swallow your " + player.cockHead(x) + " and lick it, just before");
        }
        outputText(" it explodes in her mouth.  ");
        if (player.cumQ() < 25)
        {
            outputText("A single gulp demolishes your meager output.");
        }
        else if (player.cumQ() < 150)
        {
            outputText("A slight change in the shape of her cheeks is the only reaction you get to the size of your load.  A noticeable swallow empties her maw.");
        }
        else if (player.cumQ() < 250)
        {
            outputText("A runnel of cum escapes from the bovine beauty's mouth while her distended cheeks slowly narrow. She audible gulps down the heavy load.");
        }
        else
        {
            outputText("A torrent of seed runs down her chin, but the bovine beauty gulps and swallows, trying her best to keep up.");
        }
    ...
    {
        var x : Float = player.cockThatFits(monster.vaginalCapacity());
        if (x < 0)
        {
            x = 0;
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>But vhy not?  Mein milk is so gooood.</i>\"");
        }
        else
        {
            outputText("\"<i>But why not? My milk is so goood.</i>\"");
        }
        ...
        if (monster.HP <= monster.minHP())
        {
            outputText("slowly-engorging");
        }
        else
        {
            outputText("juice-slicked");
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Nein! I... I never vanted to have to fight you!  And I definitely didn't vant to vind up on my keister vith my legs spread while a handsome " + player.mf("man", "woman") + " ogles my vagina!</i>\" exclaims the cow-girl.\n\n");
        }
        else
        {
            outputText("\"<i>No! I... I never wanted to have to fight you!  And I definitely didn't want to wind up on my butt with my legs spread while a handsome " + player.mf("man", "woman") + " ogles my vagina!</i>\" exclaims the cow-girl.\n\n");
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Fine!  I-I vant it in my pussy!  Fuck me!  Fuck mein cunt...</i>\"");
        }
        else
        {
            outputText("\"<i>Fine!  I-I want it in my pussy!  Fuck me!  Fuck my cunt...</i>\"");
        }
        ...
        if (isabellaAccent())
        {
            ...
            {
                outputText("Oh! Ze cock! It is too big for meeee!  I... am being stretched so mooo-uch!");
            }
            else if (player.cockArea(x) > 13)
            {
                outputText("Oh, such a nice cock you have!  It is rubbing mein pussy so good!");
            }
            else
            {
                outputText("Oh, what a surprise!  Ze cock, it is nice and comfortable in my pussy.  I love cute little dicks ze best!");
            }
        ...
        else
        {
            if (player.cockArea(x) > monster.vaginalCapacity())
            {
                outputText("Oh! The cock! It's too big for me!  It's... stretching me so mooo-uch!");
            }
            else if (player.cockArea(x) > 13)
            {
                outputText("Oh, such a nice cock you have!  It's rubbing my pussy so well!");
            }
            else
            {
                outputText("Oh, what a surprise!  Your cock is nice and comfortable in my pussy.  I love cute little dicks the best!");
            }
        ...
        if (player.cockTotal() > 1)
        {
            ...
            if (player.cockTotal() > 2)
            {
                outputText("another " + oMultiCockDesc());
            }
            else
            {
        ...
        if (player.tou > 80)
        {
            outputText("that they'd damage a lesser person");
        }
        else if (player.tou > 50)
        {
            outputText("that it actually hurts");
        }
        else
        {
            outputText("that you worry she'll crush your pelvis");
        }
        ...
        if (player.cumQ() >= 1000)
        {
            if (player.cumQ() < 1500)
            {
                outputText("  Isabella's belly pudges out from all the semen in her womb by the time you finish; visible proof of your fertility.");
            }
            else if (player.cumQ() < 2500)
            {
                outputText("  Isabella's belly distends massively by the time you finish, showing visible proof of your obscene fertility.");
            }
            else
            {
                outputText("  Isabella's massively distended belly and cum-squirting cunt are all the proof of your fertility you would ever need.");
            }
            if (player.cumQ() >= 1500)
            {
                outputText("  The huge puddle you leave behind is a nice bonus though.");
            }
        }
    ...
    public function tinyVictoryTittyFuckingFuntimesWithMilk() : Void
    {
        ...
        if (player.cockTotal() > 1)
        {
            outputText(", singling the smallest one out for some reason");
        }
        if (isabellaAccent())
        {
            outputText(".  \"<i>Is zat vant you vanted from me?  Oooh baby, you could have just asked!  Mamma Isabella loves sucking on cute little penises like yours, particularly ones like zat little guy, right zere.</i>\"\n\n");
        }
        else
        {
            outputText(".  \"<i>Is that what you wanted from me?  Oooh baby, you could have just asked!  Momma Isabella loves sucking on cute little dicks like yours, particularly ones like that little guy, right there.</i>\"\n\n");
        }
        ...
        if (player.cor < 33)
        {
            outputText("bringing a shameful blush to your cheeks");
        }
        else if (player.cor < 66)
        {
            outputText("bringing a tiny blush to your cheeks");
        }
        else
        {
            outputText("but you aren't really bothered by it in the slightest");
        }
        ...
        if (monster.lust >= monster.maxOverLust())
        {
            outputText("her own needs.");
        }
        else
        {
            outputText("her wounds.");
        }
        outputText("\n\n");
        ...
        if (isabellaAccent())
        {
            outputText(" \"<i>Does your cute little cock like zis?  Ooooh, I zink it does.  It vants to squirt into my cleavage like a good little dick, ja?</i>\"");
        }
        else
        {
            outputText(" \"<i>Does your cute little cock like this?  Ooooh, I think it does.  It wants to squirt into my cleavage like a good little dick doesn't it?</i>\"");
        }
        ...
        if (isabellaAccent())
        {
            outputText(" \"<i>Are you going to cum for me soon?  I vant zat tiny cock to spurt all its meek little cum over my breasts until it's limp.  Isn't zat vat you want?</i>\"");
        }
        else
        {
            outputText(" \"<i>Are you going to cum for me soon?  I want that tiny cock to spurt all of your meek little load of cum over my breasts until it's limp.  Isn't that what you want?</i>\"");
        }
        outputText(" Her sultry teases arouse you past the tipping point, and you feel your cock begin to release ");
        if (player.cumQ() < 50)
        {
            outputText("its pathetic little load");
        }
        else if (player.cumQ() < 250)
        {
            outputText("a nice gooey load");
        }
        else if (player.cumQ() < 500)
        {
            outputText("its hefty, tit-drenching load");
        }
        else
        {
            outputText("its massive, body-soaking load");
        }
        outputText(" into Isabella's cleavage.");
        if (player.cumQ() >= 1000)
        {
            outputText("  It pumps and pumps, blotting out her chest and face, turning her expression to one of ecstatic shock.");
        }
        if (isabellaAccent())
        {
            outputText(" \"<i>Oh my!  Such an obedient little cock!  Zat's right; cover me vis your seed, my adorable little weenie!</i>\"");
        }
        else
        {
            outputText(" \"<i>Oh my!  Such an obedient little cock!  That's right; cover me with your seed, my adorable little dick!</i>\"");
        }
        ...
        if (isabellaAccent())
        {
            outputText("  \"<i>Come back soon!  You have ze tastiest little dick and I vant to play with it more, ja?</i>\"");
        }
        else
        {
            outputText("  \"<i>Come back soon!  You have the tastiest little dick and I want to play with it more!</i>\"");
        }
        //Reset anger
