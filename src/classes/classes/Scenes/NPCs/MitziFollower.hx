...
class MitziFollower extends NPCAwareContent implements TimeAwareInterface
{
    ...
    public function MitziCampMainMenu() : Void
    {
        ...
        switch (_sw28_)
        {
            case 2:if (flags[kFLAGS.MITZI_DAUGHTERS] < 4)
                ...
                {
                    outputText("You spot Mitzi lounging in her usual shaded spot with a few of her daughters. Looking closer, her stomach is notably rounder and her breasts appear fuller. You can tell she's definitely pregnant again. She glances over at you with a knowing smirk, caressing her belly. Your daughters notice you as well and give a friendly wave to you before going back to their discussion.\n\n");
                }
            case 3, 4:outputText("Mitzi's belly has grown larger. It's clear that she's carrying more than one child. She spends most of the day lounging in the shade and munching on some fresh fruits. Her top struggles to contain her milk filled jugs.\n\n");
            case 5, 6:outputText("Mitzi lazes by the stream. Her belly looks obscenely swollen with her children. She doesn't wear a top seeing as it can no longer support her hefty milk dribbling breasts. You're sure it won't be too much longer before she gives birth. She shoots you a sly look, slicking out her long tongue and swiping it over her large nipple. She laps at the milk slowly pouring out before slurping it back into her mouth.\n\n");
            default:
        }
        MitziCampMainMenu2();
    }
    public function MitziCampMainMenu2() : Void
    {
        ...
        if (player.hasPerk(PerkLib.BasicLeadership))
        {
            ...
            {
                addButton(5, "Assist me", mitziHenchmanOption);
            }
            else
            {
                addButtonDisabled(5, "Assist me", "She doesn't have enough daughters (6+) to accompany you.");
            }
        }
        else
        {
            addButtonDisabled(5, "Assist me", "You need to have at least Basic Leadership to form a team with Mitzi.");
        }
    ...
    public function mitziCamp() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.CampRathazul) || EvangelineFollower.EvangelineFollowerStage >= 1)
        {
            outputText("\"<i>Gotta admit those alchemists really know their stuff. I wonder if they can help me with a few potions I wanted to whip up…</i>\"\n\n");
        }
        if (flags[kFLAGS.KONSTANTIN_FOLLOWER] >= 2 && !player.hasStatusEffect(StatusEffects.KonstantinOff))
        {
            outputText("\"<i>You have a bear of a hunk over there. The way he works that hammer and anvil. Those rippling muscles… Mmm… He could be helpful indeed…</i>\"\n\n");
        }
        if (isabellaFollower())
        {
        ...
        {
            outputText("\"<i>I'm surprised she hasn’t eaten you yet [name]. I've seen a group of those shark girls tear a Minotaur to shreds before. You must really have her under control.</i>\"\n\n");
        }
        if (player.hasStatusEffect(StatusEffects.CampMarble))
        {
            outputText("\"<i>Hmph, that Marble chick almost took my fucking head off when I was still living in the forest with that big ass hammer of hers. I haven't forgotten that…</i>\"\n\n");
        }
        if (player.hasStatusEffect(StatusEffects.PureCampJojo) && flags[kFLAGS.JOJO_BIMBO_STATE] != 3)
        {
            outputText("\"<i>You have a really cute little mouse boy there. Can't say that I'm into that meditation thing he keeps encouraging me to do.</i>\"\n\n");
        }
        ...
        {
            outputText("\"<i>I have no idea what that weird shock weasel girl is honestly. Though I can see making her a pretty good battery…</i>\"\n\n");
        }
        ...
        {
            outputText("\"<i>Oh geez, I can’t remember the last time I saw a troll! Beefy all over, packed with muscle, just the way I like it… I just want him to help me with my needs…</i>\"\n\n");
        }
        if (ZenjiScenes.isLover())
        {
            outputText("\"<i>Oh geez, I can’t remember the last time I saw a troll! Beefy all over, packed with muscle, just the way I like it… I just want him to help me with my needs… But he’s so hard to talk to.</i>\"\n\n");
        }
        //Aenair
    ...
    public function mitziHenchmanOption() : Void
    {
        ...
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "Mitzi" || flags[kFLAGS.PLAYER_COMPANION_3] == "Mitzi")
            {
                addButtonDisabled(0, "Team (1)", "You already have Mitzi accompany you.");
            }
            else
            {
                addButton(0, "Team (1)", mitziHenchmanOption2, 1).hint("Ask Mitzi to join you in adventures outside camp.");
            }
        }
        else if (flags[kFLAGS.PLAYER_COMPANION_1] == "Mitzi")
        {
            addButton(5, "Team (1)", mitziHenchmanOption2, 21).hint("Ask Mitzi to stay in camp.");
        }
        else
        {
            addButtonDisabled(5, "Team (1)", "You already have other henchman accompany you as first party member. Ask him/her to stay at camp before you talk with Mitzi about accompaning you as first party member.");
        }
        if (player.hasPerk(PerkLib.IntermediateLeadership))
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "")
            {
                if (flags[kFLAGS.PLAYER_COMPANION_1] == "Mitzi" || flags[kFLAGS.PLAYER_COMPANION_3] == "Mitzi")
                {
                    addButtonDisabled(1, "Team (2)", "You already have Mitzi accompany you.");
                }
                else
                {
                    addButton(1, "Team (2)", mitziHenchmanOption2, 2).hint("Ask Mitzi to join you in adventures outside camp.");
                }
            }
            else if (flags[kFLAGS.PLAYER_COMPANION_2] == "Mitzi")
            {
                addButton(6, "Team (2)", mitziHenchmanOption2, 22).hint("Ask Mitzi to stay in camp.");
            }
            else
            {
                addButtonDisabled(6, "Team (2)", "You already have other henchman accompany you as second party member. Ask him/her to stay at camp before you talk with Mitzi about accompaning you as second party member.");
            }
        }
        else
        {
    ...
    public function mitziHenchmanOption2(slot : Float = 1) : Void
    {
        clearOutput();
        if (slot < 21)
        {
            ...
            var strMitzi : Float = 20;
            var intMitzi : Float = 40;
            var libMitzi : Float = 50;
            ...
            if (slot == 2)
            {
                flags[kFLAGS.PLAYER_COMPANION_2] = "Mitzi";
            }
            if (slot == 1)
            {
    ...
    public function mitziSexMenu() : Void
    {
        ...
        menu();
        addButton(0, "Tit Job", mitziSexTitJob).disableIf(!player.hasCock(), "Req. a cock.");
        addButton(1, "Fuck Her", mitziSexFuckHer).disableIf(!player.hasCock(), "Req. a cock.");
        addButton(2, "Jog Fuck", mitziSexJogFuck).disableIf(player.str < 50, "You're too weak to carry her.").disableIf(!player.isBiped() || player.tallness < 60, "You need to be biped and pretty tall to try that.").disableIf(!player.hasCock(), "Req. a cock.");
        addButton(3, "Big Dick Sex", mitziSexBigDickSex).disableIf(!player.hasStatusEffect(StatusEffects.MitziPregnant), "You need to wait until Mitzi gives a birth.").disableIf(player.findCock(1, 20, -1, "length") < 0, "You need 20+ inches cock for this.");
        addButton(4, "Eaten Out", mitziSexEatenOut).disableIf(!player.hasVagina(), "Req. a pussy.");
        addButton(14, "Back", MitziCampMainMenu);
    }
    ...
    public function mitziSexTitJob() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.MitziPregnant))
        {
            outputText("Your hands wander up to her long milk leaking nipples, pinching them between your fingers and squirting out thin streams of goblin cream. ");
        }
        ...
        if (player.hasStatusEffect(StatusEffects.MitziPregnant))
        {
            outputText("Milk pours from her engorged nipples, adding to the widening puddle beneath you two. ");
        }
    ...
    public function mitziSexFuckHer() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.MitziPregnant))
        {
            outputText(" Milk dribbles from her purple teats, coating her front in goblin cream.");
        }
        ...
        if (player.hasStatusEffect(StatusEffects.MitziPregnant))
        {
            outputText("Unfortunately most of it comes flooding back out with her already being pregnant. ");
        }
    ...
    public function MitziDaughtersCampMainMenu(first : Bool = true) : Void
    {
        clearOutput();
        if (first)
        {
        ...
        {
            addButton(1, "Barrels", MitziDaughtersBarrels);
        }
    ...
    {
        menu();
        if (player.statusEffectv1(StatusEffects.MitziDaughtersBarrels) > 0)
        {
            addButton(0, "Goblin Ale", MitziDaughtersBarrelsGoblinAle);
        }
        else
        {
            addButtonDisabled(0, "Goblin Ale", "There is no Goblin Ale in barrels.");
        }
        if (player.statusEffectv2(StatusEffects.MitziDaughtersBarrels) > 0)
        {
            addButton(1, "Lust Draft", MitziDaughtersBarrelsLustDraft);
        }
        else
        {
            addButtonDisabled(1, "Lust Draft", "There is no Lust Draft in barrels.");
        }
        if (player.statusEffectv3(StatusEffects.MitziDaughtersBarrels) > 0)
        {
            addButton(2, "Ovi Elixir", MitziDaughtersBarrelsOviElixir);
        }
        else
        {
            addButtonDisabled(2, "Ovi Elixir", "There is no Ovi Elixir in barrels.");
        }
