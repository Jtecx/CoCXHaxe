...
class HolliScene extends NPCAwareContent
{
    ...
    public function treeMenu(output : Bool = true) : Void
    {
        if (flags[kFLAGS.FOUGHT_HOLLI] > 0)
        {
            ...
            {
                addButton(2, "Call Jojo", callDatJojo);
            }
            addButton(3, "Back", inventory.inventoryMenu);
        }
        else if (flags[kFLAGS.FUCK_FLOWER_LEVEL] == 1)
        {
            flowerStage1Menu(output);
        }
        else if (flags[kFLAGS.FUCK_FLOWER_LEVEL] == 2)
        {
            flowerStage2Menu(output);
        }
        else if (flags[kFLAGS.FUCK_FLOWER_LEVEL] == 3)
        {
            flowerStage3Menu(output);
        }
        else
        {
            holliCampMenu(output);
        }
    }
    
    public function flowerStage1Menu(output : Bool = true) : Void
    {
        if (output)
        {
            clearOutput();
    ...
    public function flowerStage2Menu(output : Bool = true) : Void
    {
        if (output)
        {
            clearOutput();
        ...
        menu();
        addButton(6, "Fuck It", fuckFuckingFuckFlowerP2).disableIf(player.lust < 33, "Not aroused enough!").disableIf(player.cockThatFits(100) < 0, "Req. a cock fitting 100 capacity.");
        addButton(7, "Ride Stamen", rideDatFuckingFukkFlowerP2).disableIf(player.lust < 33, "Not aroused enough!").disableIf(!player.hasVagina(), "Req. a vagina.");
        if (!recalling)
        {
    ...
    public function flowerStage3Menu(output : Bool = true) : Void
    {
        if (output)
        {
            clearOutput();
        ...
        menu();
        addButton(5, "Fuck Flower", fuckTheFlower).disableIf(player.lust < 33, "Not aroused enough!").disableIf(!player.hasCock(), "Req. a cock.");
        addButton(6, "Ride Tentacle", rideTheWalrusP3).disableIf(player.lust < 33, "Not aroused enough!").disableIf(!player.hasVagina(), "Req. a vagina.");
    ...
    public function holliCampMenu(output : Bool = true) : Void
    {
        if (output)
        {
            clearOutput();
        ...
        if (output)
        {
            ...
            {
                outputText("\n\n\"<i>Welcome back, [name].  ");
                if (flags[kFLAGS.TIMES_FUCKED_FLOWER] > 0 && rand(3) == 0)
                {
                    outputText("Did you want to fertilize me again?  My flower is ready and waiting...");
                }
                else if (flags[kFLAGS.TIMES_RIDDEN_FLOWER] > 0 && rand(3) == 0)
                {
                    outputText("Did you want to use my tentacles again?  They've been restless without you...");
                }
                else if (rand(2) == 0)
                {
                    outputText("Did you come for a drink?  I've built up lots of nectar for you.  Surely, a sip would be refreshing for you...");
                }
                else
                {
                    outputText("How may I serve you?  Come, give me a command so that I can fulfill it for you.");
                }
                outputText("</i>\"");
            }
            else
            {
                ...
                if (flags[kFLAGS.TIMES_FUCKED_FLOWER] > 0 && rand(3) == 0)
                {
                    outputText("Did you want to fertilize me again?  Gardening is such exhausting work, but so rewarding...");
                }
                else if (flags[kFLAGS.TIMES_RIDDEN_FLOWER] > 0 && rand(2) == 0)
                {
                    outputText("Did you come back to play again?  My tentacles are so restless.");
                }
                else if (rand(2) == 0)
                {
                    outputText("Did you come for a drink?  I've built up lots of nectar for you.  Surely a hungry adventurer like yourself needs a sip, right?");
                }
                else
                {
                    outputText("Do you want to play?  Come on, taste Mother Marae's finest creation.");
                }
                outputText("</i>\"");
            }
            ...
            {
                if (flags[kFLAGS.HOLLI_FRUIT] == 1)
                {
                    outputText("\n\nA large, purple fruit hangs from Holli's branches, twisting softly in the breeze.  It looks succulent and ripe.");
                }
                else
                {
                    outputText("\n\n" + cast((flags[kFLAGS.HOLLI_FRUIT]), Num2Text) + " large, purple fruits hang from Holli's branches, twisting softly in the breeze.  They look succulent and ripe.");
                }
                //Optional Addition:
                if (flags[kFLAGS.HOLLI_FRUIT_EXPLAINED] > 0)
                {
                    outputText("  A very small, strange part of you actually feels proud to have made something like that with the tree-woman.");
                }
            }
        }
        sceneHunter.print("\nTo make her submissive, make her guard the camp. To reset it, get humiliated by her when asking to stop guarding.");
        menu();
        addButton(0, "FuckHolli", (flags[kFLAGS.HOLLI_SUBMISSIVE] == 0) ? fuckHolliInZeFlowerPuss : holliGetsDickDommed).disableIf(player.lust < 33, "Not aroused enough!").disableIf(!player.hasCock(), "Req. a cock!");
        addButton(1, "RideTentacles", (flags[kFLAGS.HOLLI_SUBMISSIVE] == 0) ? level4RideHollisTentacruels : vaginalDomHollisTentacruels).disableIf(player.lust < 33, "Not aroused enough!").disableIf(!player.hasVagina(), "Req. a vagina!");
        if (flags[kFLAGS.HOLLI_SUBMISSIVE] == 1)
        {
            addButton(2, "TentacleBone", fullOnTentacleTasticGangBangForHolli).disableIf(player.lust < 33, "Not aroused enough!").disableIf(player.countCocksWithType(CockTypesEnum.TENTACLE) >= 10, "Req. 10 tentacle cocks!");
        }
        addButton(3, "TentacleDuel", alrauneExtraSceneWithHollicyntiaTentacleDuel).hint("There's no way you won't end up getting pluged in all of your holes from this.").disableIf(player.lust < 33, "Not aroused enough!").disableIf(!player.isAlraune(), "Req. to be an alraune!");
        addButton(5, "Drink Sap", haveAMapleSyrupSnack);
        addButton(6, "Eat A Fruit", eatHolliFruit).disableIf(flags[kFLAGS.HOLLI_FRUIT] == 0);
        addButton(7, "Mara Fruit", getMaraFruit).disableIf(flags[kFLAGS.DAILY_MARA_FRUIT_COUNTER] > 0);
        if (flags[kFLAGS.HOLLI_SUBMISSIVE] == 0)
        {
            if (flags[kFLAGS.HOLLI_DEFENSE_ON] == 1)
            {
                addButton(8, "Stop Guarding", askHolliToWatch);
            }
            else
            {
                addButton(8, "Guard Camp", askHolliToWatch);
            }
        }
        else
        {
        ...
        if (flags[kFLAGS.FOLLOWER_AT_FARM_HOLLI] == 0 && flags[kFLAGS.FARM_CORRUPTION_STARTED] == 1)
        {
            addButton(9, "Farm Help", helpWithFarm);
        }
    ...
    public function alrauneExtraSceneWithHollicyntiaTentacleDuel() : Void
    {
        ...
        {
            sceneHunter.print("Check failed: Liliraune!");
        }
        ...
        if (player.isLiliraune())
        {
            outputText("Your twin responds right away clearly upset \"<i>As if! Twice the girl, twice the pleasure! You’re just jealous that I get to literally enjoy having an orgy partner all day long! If both of us worked you up you would not even last a minute before melting in your tree little sis!</i>\"\n\n");
        }
        else
        {
    ...
    private function fertilizeHolli(cock : Bool = true) : Void
    ...
    public function getASprout() : Void
    {
        ...
        {
            outputText("  She even promised to plant a tree in your camp...");
        }
    ...
    public function fuckPlantGrowsToLevel2() : Void
    {
        ...
        if (player.gender == 0)
        {
            outputText("  Sucks to be it!  You don't waste time with stupid shit, like genitals.");
        }
        outputText("  Is this Marae's idea of a gift?");
        if (player.gender > 0)
        {
            outputText("  Well, you could give it a whirl... just being around it seems to slowly turn you on.");
        }
    ...
    private function fuckFuckingFuckFlowerP2() : Void
    {
        ...
        if (player.cor < 33)
        {
            outputText(" and you timidly look around, hoping you haven't attracted an audience.");
        }
        else if (player.cor < 66)
        {
            outputText(" and you worriedly glance around, hoping yet terrified that someone will walk in on it.");
        }
        else
        {
            outputText(" and you glance around, hoping someone will walk in on the sexy show you're making.");
        }
        ...
        if (player.hasSheath())
        {
            outputText(" inside your violated sheath");
        }
        ...
        if (player.hasBalls())
        {
            outputText("Your [balls] gurgle, a surprisingly happy sound, and cling tight to your loins.  ");
        }
        ...
        if (player.cumQ() > 500)
        {
            outputText("  You pour forth your prodigious essence without pause or fail, and the plant immediately begins to drink it down, the stem bulging with rounded balls of sperm that it carries underground.");
        }
        if (player.cumQ() > 1500)
        {
            outputText("  Soon, the meager stem cannot swallow it all.  Spunk froths and squirts out around the base of your tool for a moment, and then a veritable tide of the stuff gushes out to soak  you.");
        }
        ...
        if (!recalling)
        {
            ...
            if (flags[kFLAGS.FUCK_FLOWER_GROWTH_COUNTER] < 1000)
            {
                flags[kFLAGS.FUCK_FLOWER_GROWTH_COUNTER] += 4;
            }
            fatigue(5);
            doNext(camp.returnToCampUseOneHour);
        }
    ...
    private function rideDatFuckingFukkFlowerP2() : Void
    {
        ...
        {
            outputText("  Your heavy buzzer is so big that the flexible plant-cock has plenty of room to curl around it, and if you could see it, your clit would look almost like a candy-cane (if one set of stripes was jerking off the cane beneath).");
        }
        if (player.cockTotal() > 0)
        {
            outputText("  [EachCock] soon has its own partner twisting around it, a pussy-slicked vine to jerk and twine about its girth.");
        }
        outputText("\n\nWith nothing to occupy your roving hands, you let them slide up your " + player.skinFurScales() + " to your ");
        if (player.bRows() > 1)
        {
            outputText("lowest row of tits, cupping them before groping to your top row of breasts");
        }
        else
        {
            outputText("[chest]");
        ...
        if (flags[kFLAGS.PC_FETISH] == 0 && player.cor + player.lib < 150)
        {
            outputText("embarrassment");
        }
        else
        {
            outputText("lust");
        ...
        if (player.hasCock())
        {
            outputText("  [EachCock] explodes in release, pumping heavy flows of seed into the petals, even as y");
        }
        else
        {
            outputText("Y");
    ...
    public function flowerGrowsToP3() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_FUCKED_FLOWER] + flags[kFLAGS.TIMES_RIDDEN_FLOWER] > 0)
        {
            outputText("  The fucking probably only helped it to grow faster.");
        }
    ...
    private function fuckTheFlower() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText("  Wetness builds in your [vagina] as well, but you're focusing on your maleness just now.  Your neglected twat will have to wait its turn.");
        }
        ...
        {
            outputText(", sliding down the trunk as easily if the entire inside was a giant pussy just waiting to get fucked");
        }
        outputText(".");
        ...
        if (player.hasBalls())
        {
            outputText(" with your gurgling balls slapping against the flower's edge");
        }
        ...
        if (player.cockTotal() > 1)
        {
            outputText(", as your multiple endowments allow you to drizzle the purple bloom's exterior with alabaster cream");
        }
        outputText(".");
        if (player.cumQ() > 500)
        {
            outputText("  Pearly ropes big enough to fill a flask lurch out from your cum-slit, propelled by your quivering orgasmic muscles deep into the trunk, and you swear you can hear the pitch of your sexual pumping rising in tone, changed by the flooded vessel.");
        }
        if (player.cumQ() > 1500)
        {
            outputText("  A wave of your thick gunk rolls out from under your pumping cock - you've filled the tree beyond even its capacity, and the thick puddle forming beneath your [feet] is all the proof you need of your virility.");
        }
        if (player.hasVagina())
        {
        ...
        if (!recalling)
        {
            ...
            if (flags[kFLAGS.FUCK_FLOWER_GROWTH_COUNTER] < 1000)
            {
                flags[kFLAGS.FUCK_FLOWER_GROWTH_COUNTER] += 5;
            }
    ...
    private function drinkThePlantGirlsSap() : Void
    {
        ...
        if (silly())
        {
            outputText(", tree hugger that you are");
        }
    ...
    private function torchP3Tree() : Void
    {
        ...
        {
            outputText("Closing your eyes to focus, you gather your energies, and unleash your white, supernatural flames upon the thing.");
        }
        //(bromancefire)
        else if (player.hasPerk(PerkLib.FireLord))
        {
            outputText("Sucking in your breath, you inflate your chest and stir the embers of the jaguar demon's gift.  Dredging up an enormous pine-green fireball, you exhale and launch it at the tree.");
        }
        else
        {
            outputText("Sucking in your breath, you inflate your chest and stir the embers of your fiery gift.  Dredging up an enormous fireball, you exhale and launch it at the tree.");
        }
    ...
    public function treePhaseFourGo() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_FUCKED_FLOWER] + flags[kFLAGS.TIMES_RIDDEN_FLOWER] > 0)
        {
            outputText("Did you enjoy feeding me?  You should have seen the expression on your face when you were cumming in me.  Your eyes were rolling back and your mouth was just hanging open.  Do you know what it made me want to do?");
        }
        else
        {
            outputText("Did you enjoy watching me grow?  You should have fed me.  It was so much harder to grow without proper nutrients.  Do you know what I thought about during all the time I spent waiting?  Do you know what it made me want to do?");
        }
        outputText("</i>\"");
    ...
    public function JojoTransformAndRollOut() : Void
    {
        ...
        if (player.cor < 25)
        {
            outputText("Not willing to lie to your friend, you also admit that she seems to have changed after your actions in the factory - that, though you were trying to render the tanks permanently inoperable, the spillage you created pushed the cthonic spirit over the edge.");
        }
        else
        {
            outputText("Though, you do leave out the precise, needless details of what caused the change.");
        }
        ...
        if (flags[kFLAGS.TIMES_FUCKED_FLOWER] + flags[kFLAGS.TIMES_RIDDEN_FLOWER] == 0)
        {
            outputText("To think that I must now consider Marae herself my enemy.  Forgive me, but this place is not safe for us anymore if it has attracted the attention of such a creature.  For now, I will return to the forest.  I know that you believe your mission keeps you here, but I urge you to stay away from that thing and watch the portal from an adequate distance.");
        }
        //(totally deflowered that flower)
        else
        {
            outputText("And even knowing that this could be the case, you still chose to fornicate with it?  [name]... I cannot say that I am not disappointed.  This place is not safe for me anymore.  For now, I will return to the forest.");
        }
        outputText("</i>\"");
        ...
        if (player.level < 20)
        {
            outputText("  Nonetheless, if you find yourself in a position where you must confront this demon or perish, call upon me.  I will render what aid I can, though I prefer it not be in vain.");
        }
        else
        {
            outputText("  Though, a mortal of your stature and resources may be able to destroy her and escape the reprisal.  I would counsel against direct confrontation until you are sure of victory, but if you must fight, then call upon me and you will have what aid I can give.");
        }
    ...
    private function fuckHolliInZeFlowerPuss() : Void
    {
        ...
        if (player.cockTotal() > 1)
        {
            if (player.cockTotal() == 2)
            {
                outputText("  Your other penis splays");
            }
            else
            {
                outputText("The rest of your penises splay");
            }
        ...
        if (player.hasBalls())
        {
            outputText("cradle your [balls], gently massaging your sack");
        }
        else
        {
        ...
        {
            outputText("leans down and kisses the tip, opening her glossy green puckers to slurp it down, sucking just hard enough to make your " + player.cockHead(x) + " tingle with erotic warmth.");
        }
        else
        {
            outputText("kisses up and down your huge, towering shaft, slobbering her glossy green puckers all over your urethral bulge, making it even more of a wet mess.");
        }
        ...
        if (player.cockArea(x) >= 100)
        {
            outputText("the air, where it hangs for a moment before splattering down atop the plant-girl's leaf-colored hair and face, forming a gossamer shroud.");
        }
        else
        {
            outputText("her mouth, which works noisily to swallow the hot load even as you inject it into her.  After a few swallows, she leans back and lets you shoot it up into the air, so that it can fall down over her face and hair, into a gossamer shroud.");
        }
        //highish cum:
        if (player.cumQ() >= 500)
        {
            outputText("  You keep blasting heavy ropes of seed until her body and trunk are painted with goo, and then you cum some more, smattering spunk until she looks more like a gooey waterfall than a fey creature.");
        }
        if (player.cumQ() >= 1500)
        {
            outputText("  Before you know it, there's a lake around the two of you and a small river running downhill away from camp.");
        }
        ...
        if (player.cockTotal() > 1)
        {
            outputText("s, massaging the bundle of cocks with uncommon ferver");
        }
        else
        {
            outputText(", massing your tool with uncommon fervor");
        }
    ...
    private function haveAMapleSyrupSnack() : Void
    {
        ...
        if (flags[kFLAGS.HOLLI_SUBMISSIVE] == 0)
        {
            outputText("knowingly, Holli asks, \"<i>Come for a little pick-me-up?  Well, go on; drink deeply of my nectar, </i>champion.\"  The last word is inundated with malicious, mocking tones.");
        }
        else
        {
            outputText("eagerly, Holli asks, \"<i>Care for a little pick-me-up?  They're getting very engorged, and I'm sure it would be a refreshing snack for you.</i>\"");
        }
    ...
    private function level4RideHollisTentacruels() : Void
    {
        ...
        if (silly())
        {
            outputText("  She proceeds to sing about Jack Sparrow's awesome adventures for a solid ten minutes, then goes on as if nothing had happened.");
        }
        ...
        {
            outputText("droplets of milk dripping from your nipples.  When did you start lactating?");
        }
        else
        {
            outputText("steady streams of milk erupting from your nipples.  When did you start lactating this hard?");
        }
        ...
        if (!player.isBiped())
        {
            outputText("you'd be walking bowlegged for a week, if you had legs.");
        }
        else
        {
            outputText("you'll be walking bowlegged for a week!");
        }
    ...
    private function eatHolliFruit() : Void
    {
        ...
        if (flags[kFLAGS.HOLLI_FRUIT] == 1)
        {
            outputText("the fruit");
        }
        else
        {
            outputText("one of the fruits");
        }
        outputText(".  ");
        if (flags[kFLAGS.HOLLI_FRUIT_EXPLAINED] == 0)
        {
    ...
    private function askHolliToWatch() : Void
    {
        ...
        if (flags[kFLAGS.HOLLI_DEFENSE_ON] == 0)
        {
            ...
            if (flags[kFLAGS.THREATENED_HOLLI] > 0)
            {
                outputText("<b>  You could always just slap the cunt.</b>");
            }
            ...
            if (flags[kFLAGS.THREATENED_HOLLI] == 0)
            {
                addButton(1, "Threaten", threatenHolli);
            }
            else
            {
                addButton(1, "Slap Her", slapDatHo);
            }
    ...
    private function domUpSomeHolli() : Void
    {
        ...
        var domPowah : Float = player.level;
    ...
    private function askBrokenHolliToGuard() : Void
    {
        ...
        if (flags[kFLAGS.HOLLI_DEFENSE_ON] == 1)
        {
            outputText("keeping watch");
        }
        else
        {
            outputText("sleeping, instead");
        }
        outputText(".</b></i>\"");
        ...
        if (flags[kFLAGS.HOLLI_DEFENSE_ON] == 1)
        {
            addButton(1, "Don't Guard", toggleBrokenHolliGuard);
        }
        else
        {
            addButton(0, "Guard", toggleBrokenHolliGuard);
        }
        addButton(4, "Back", holliCampMenu);
    }
    ...
    private function holliGetsDickDommed() : Void
    {
        ...
        {
            outputText(", jaw stretching obscenely");
        }
        outputText(".");
        ...
        {
            outputText(", swallowed as deep as Holli's distended neck can handle.");
        }
        ...
        if (player.hasBalls())
        {
            outputText("[sack] feel taut and ready to burst");
        }
        else
        {
            outputText("body feel strung tight like an over-tuned guitar");
        }
        ...
        if (player.biggestCockArea() > 80)
        {
            outputText("  It bends slightly as the immense girth is forced inside her, aiming down, deep inside the trunk.");
        }
        ...
        if (player.cockTotal() > 1)
        {
            ...
            if (player.cumQ() >= 500)
            {
                outputText("  The constant sprays turn them white in little time, dripping from the peaks of her nipples along with her sticky sap.");
            }
        }
        if (player.cumQ() > 1000)
        {
            outputText("  A trickle of sperm washes out of the demoness's entrance, somehow escaping the heavy suction.");
        }
        if (player.cumQ() > 2000)
        {
            outputText("  A moment later, that trickle turns into a river.");
        }
        if (player.cumQ() > 3000)
        {
            outputText("  You swear that her trunk must be absolutely stuffed with spunk.");
        }
        if (player.cumQ() > 5000)
        {
            outputText("  A spooge-spout sprays out as the pressure goes through the roof, confirming your suspicions in the messiest way possible.");
        }
    ...
    private function fullOnTentacleTasticGangBangForHolli() : Void
    {
        ...
        if (silly())
        {
            outputText(", inflatable-arm, flailing tube-cocks");
        }
        ...
        if (player.hasBalls())
        {
            outputText("  Your [balls] draw up tight to your groin and squeeze, rhythmically discharging their heavy loads.");
        }
    ...
    private function vaginalDomHollisTentacruels() : Void
    {
        ...
        if (!player.hasFuckableNipples() && player.lactationQ() < 50)
        {
            outputText(" and pay special attention to your [nipples], tugging them in time with every sluggish thrust in your slit.");
        }
        else if (player.lactationQ() >= 50)
        {
            ...
            else if (player.lactationQ() < 800)
            {
                outputText("sprays");
            }
            else if (player.lactationQ() < 2000)
            {
                outputText("outpouring");
            }
            else
            {
                outputText("thick fountains");
            }
            outputText(" of milk that escape with every tug, timed to match the sluggish thrusts at your slit.");
        }
        else
        {
            outputText(" and sink your fingers into your [nipples], fucking them to the sluggish tempo of the thrusting tentacle.");
        }
        ...
        {
            outputText(" for a moment before wrapping around and jerking on it");
        }
        outputText(".");
        ...
        if (player.wetness() < 4)
        {
            outputText("onto the ground");
        }
        else
        {
            outputText("into the leavings of your squirting orgasm");
        }
    ...
    private function threatenHolli() : Void
    {
        ...
        if (player.level < 30)
        {
            outputText("Mighty as you are, Marae is likely still beyond your power.  Perhaps you should just ignore this demon for the time being.");
        }
        else
        {
            outputText("Marae is likely beyond your power yet, but... you feel like she might not pressure you too hard if you can make a suitably credible threat.");
        }
    ...
    private function slapDatHo() : Void
    {
        ...
        if (player.hasKeyItem("Jojo's Talisman") >= 0)
        {
            addButton(7, "Call Jojo", callDatJojo);
        }
    ...
    private function defeatHolli() : Void
    {
        clearOutput();
        flags[kFLAGS.FUCK_FLOWER_KILLED] = 1;
        //Win without Jojo -Z
    ...
    //>Lose
    @:allow(classes.scenes.nPCs)
    private function enjoyYourBadEndBIYAAAATCH() : Void
    {
        clearOutput();
        ...
        function dickF() : Void
        {
            ...
            if (player.cocks[player.biggestCockIndex()].cockThickness > 6)
            {
                outputText(", stretching her jaw wide open in a way that makes you wince,");
            }
            ...
            {
                outputText("her neck ripples and extends grotesquely, and her head begins to migrate down all " + num2Text(Math.round(player.biggestCockLength())) + " of your prick with alarming speed, pushing the stem inside along with it.  ");
            }
            else
            {
                outputText("she sinks her head down to the base of your prick in one quick thrust, bringing the stem with it.  ");
            }
            ...
            if (player.hasBalls())
            {
                outputText(", and your [balls] draw close to your body to unload");
            }
            ...
            if (player.hasBalls())
            {
                outputText("migrating the other direction into your [balls], entangling and infiltrating them, filling them with the same warmth, then pressing out against your sack.  ");
            }
            else
            {
                outputText("exploring the depths of your body and curling back against the surface of your [skin].  ");
            }
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            if (player.isTaur())
            {
                outputText(" and turning you around");
            }
            ...
            {
                outputText("slips between your thighs and ");
            }
            ...
            if (player.hasCock())
            {
                outputText(" and a glob of precum from [eachCock]");
            }
            outputText(".");
            ...
            if (player.hasCock())
            {
                outputText("  [OneCock] drools precum down its length and onto the flower, triggering another spasm and another gush from your prick.  Holli laughs as you continue to self-stimulate with your uncontrollable fluids.");
            }
            ...
            if (player.biggestTitSize() > 1)
            {
                outputText("wrapping around and under your breasts to keep them permanently jutting out in a lurid display");
            }
            else
            {
                outputText("tracing a lattice of slender, criss-crossing ivy over your chest that hangs just far enough over your [nipples] that a stiff breeze will drag it into them");
            }
            ...
            if (player.biggestTitSize() > 1)
            {
                outputText("your breasts cupped and pressed together by vines, ");
            }
            if (!player.isNaga() && !player.isGoo())
            {
                outputText("your thighs spread wide open, ");
            }
        ...
        //branch of nothing-having
        {
            ...
            {
                outputText("Betraying your depravity, the slick crack opens wide and your greedy, wet asshole practically welcomes the demon's entire hand, sucking it up and making you shiver with repressed delight.  \"<i>Oh, mother!  It seems as though this is not a new lesson to you, is it?  My little whore, will you beg and moan as I fill you?</i>\"");
            }
            else if (player.analCapacity() < 30)
            {
                outputText("With some difficulty, she forces it into your pucker.  \"<i>I think we'll just use this as our inroads, instead.  It may be slow-going, but the journey is half the fun.</i>\"");
            }
            else
            {
                outputText("Despite your clenching, your loose hole proves no impediment to the demon's exploration.  \"<i>Ah, how roomy you are!  I think I'll move right in.</i>\"");
            }
            ...
            if (player.ass.analWetness > 0)
            {
                outputText("Try as you might, you can't see far enough down your back to determine what the demon has done.  The question answers itself, though, when she presses a shaft almost as thick as her leg into your [butt], nosing it into your wet, receptive asshole.  Despite yourself, you shiver in anticipation at being filled so completely.  ");
            }
            else if (player.analCapacity() < 30)
            {
                outputText("Your jaw hits your collarbone as Holli angles the florid head of a long, narrow penis into your field of view.  Judging by how far it comes up her chest, it's almost a foot and half long, and it's drooling sticky sap all over your back.  With a twisted smile, the demon pushes it along your spine until it reaches your [butt], then shoves it toward the hole.  ");
            }
            else
            {
                outputText("The demon presses up to your back, brushing her nipples against your skin and shoving her new growth between your thighs.  Looking down, you can see a fat, green shaft sticking from your crotch, glistening and dripping with sticky slime.  With another moan, Holli pulls it back from between your legs, then pushes apart your asscheeks and plunges it into your [asshole].  ");
            }
            outputText("The sap oozing from her member coats your passage, ");
            if (player.ass.analWetness > 0)
            {
                outputText("mingling with your own lubrication and ceasing her thrusts.  Holli's tempo picks up quickly, and soon you can feel her twitch inside you as she nears her climax.");
            }
            ...
            //[(buttslut or loose)
            if (player.ass.analWetness > 0 || player.analCapacity() > 30)
            {
                outputText("spilling a wave of her deposited fluids down your [legs].  ");
            }
            else
            {
                outputText("and your once-tight pucker reflexively squeezes shut as best it can, trapping all but a few trickles of Holli's deposit inside you.  ");
            }
            ...
            if (player.biggestTitSize() < 1)
            {
                outputText("folded atop your chest");
            }
            else
            {
                outputText("crooked at your sides with your hands resting atop your [chest]");
            }
            ...
            if (player.biggestTitSize() >= 1)
            {
                outputText(", your breasts exposed from the sides,");
            }
    ...
    private function holliPlaysWithPenisesBadEnd() : Void
    {
        ...
        if (player.biggestTitSize() < 1)
        {
            outputText("masculine shape, it bears");
        }
        else
        {
            outputText("feminine shape, it nonetheless bears");
        }
        ...
        if (player.smallestCockArea() > 36)
        {
            outputText("  As the dryad speaks, the silent figure's [cock smallest] shudders and begins to shrink, dwindling to a size that would fit " + player.mf("him", "her") + " prospective lover.");
        }
        ...
        {
            outputText("spraying the girl's stomach and tits with waste seed from [his] unused manhood and ");
        }
    ...
    private function girlsGetANiceManToBadEnd() : Void
    {
        ...
        if (player.biggestTitSize() > 1)
        {
            outputText("[chest] cupped by vines and held out toward the young man, ");
        }
        else
        {
            outputText("chest crisscrossed with a mesh of fine vines, ");
        }
        outputText("its legs elevated and spread apart to display a ");
        if (player.hasBalls() && player.ballSize > 10 && player.gender == 3)
        {
            outputText(multiCockDescriptLight() + " with [balls] swinging below.");
        }
        //(if H and ballsize < 10)
        else if (player.hasBalls() && player.ballSize <= 10 && player.gender == 3)
        {
            outputText(multiCockDescriptLight() + " and [balls] that hang slightly over a [vagina].");
        }
        //(if H and noballs)
        else if (player.gender == 3)
        {
            outputText(multiCockDescriptLight() + " and a [vagina].");
        }
        else
        {
            outputText("a wet, drooling cunt that practically begs to be filled.");
        }
        ...
        if (player.ballSize > 10 && player.hasBalls())
        {
            outputText("lifting the hefty testicles to reveal a wet, eager pussy.");
        }
        else
        {
            outputText("spreading the labia and exposing the eager, clenching hole inside.");
        }
        ...
        {
            outputText("leans down and kisses the [woman/hermaphrodite]'s smooth chest, causing her to arch her back in pleasure.  ");
        }
        if (player.hasCock())
        {
            outputText("With a nervous glance downward, he slides one hand to her [cock smallest].  \"<i>It's not gay if it's a chick's,</i>\" he mumbles to himself, and then begins to stroke haltingly, playing his finger along the urethra and drawing a squirt of precum from his shivering partner.  ");
        }
        else
        {
            ...
            {
                outputText("he smears it over the little button and then begins to trace up and down the underside with a finger.  ");
            }
            else
            {
                outputText("he coats the swollen organ with it and begins to jerk it off valiantly, like a cock.  ");
            }
        ...
        if (player.hasCock())
        {
            ...
            if (player.hasBalls())
            {
                outputText("her balls draw closer to her body and ");
            }
        ...
        if (player.hasCock())
        {
            ...
            if (player.biggestCockArea() < 20)
            {
                ...
                if (player.cumQ() >= 500)
                {
                    outputText("So much so that by the time she finishes her climax, both her partner's legs and her trunk are covered in a sheen of milky white seed.  ");
                }
            }
            else if (player.biggestCockArea() < 80)
            {
                ...
                if (player.cumQ() >= 500)
                {
                    outputText("The waves of seed don't stop coming, pouring into her and swelling her belly out until its round bulge pushes the cock away from her mouth and the remaining squirts fall on her [chest].  ");
                }
            }
            ...
            else
            {
                ...
                if (player.cumQ() >= 1000)
                {
                    outputText("The overproductive hermaphrodite's body pushes out so much seed that the entire canopy soon turns white and drips constantly as though it were in a tropical rainforest.  ");
                }
            }
        }
        //(vagina and no cock)
    ...
    private function holliAndGenderlessSittingInATree() : Void
    {
        ...
        if (!player.hasCoat())
        {
            outputText("[skin base.color]-colored");
        }
        else
        {
            outputText("covered in [skin.coat]");
        }
        ...
        {
            outputText("a quiver of arousal shakes its scales");
        }
        else if (player.isFurCovered())
        {
            outputText("its fur begins to prick up with arousal");
        }
        else
        {
            outputText("a blush of lust colors its [skin]");
        }
        outputText("...");
