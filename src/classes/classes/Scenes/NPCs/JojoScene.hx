...
class JojoScene extends NPCAwareContent implements TimeAwareInterface
{
    
    public var pregnancy : PregnancyStore;
    public static inline var JOJO_NOT_MET : Int = 0;
    public static inline var JOJO_MET : Int = 1;
    public static inline var JOJO_CORRUPT_1 : Int = 2;
    public static inline var JOJO_CORRUPT_2 : Int = 3;
    public static inline var JOJO_CORRUPT_3 : Int = 4;
    public static inline var JOJO_CORRUPT_FULL : Int = 5;
    // Jojo denied sex first time
    public static var JOJO_NOSEX_1 : Int = -1;
    // Jojo denied sex at high affection
    public static var JOJO_NOSEX_2 : Int = -2;
    // Jojo agreed to sex
    public static var JOJO_SEXED : Int = -3;
    
    
    public static var monk : Float = JOJO_NOT_MET;
    ...
    public function timeChange() : Bool
    {
        ...
        if (flags[kFLAGS.JOJO_COCK_MILKING_COOLDOWN] > 0)
        {
            flags[kFLAGS.JOJO_COCK_MILKING_COOLDOWN]--;
        }
        if (player.hasStatusEffect(StatusEffects.NoJojo))
        {
            player.removeStatusEffect(StatusEffects.NoJojo);
        }
        ...
        {
            if (joyScene.joyPregnancyUpdate())
            {
                return true;
            }
        }
    ...
    public function jojoSprite() : Void
    {
        if (flags[kFLAGS.JOJO_BIMBO_STATE] != 3)
        {
            if (tentacleJojo())
            {
                spriteSelect(SpriteDb.s_jojoTentacle);
            }
            else
            {
    ...
    private function assholeOrDP() : String
    {
        if (player.hasVagina())
        {
            return (vaginaDescript(0) + " and " + assholeDescript());
        }
    ...
    private function vaginaOrDicksOrCrotch() : String
    {
        ...
        if (player.gender == 1)
        {
            return multiCockDescriptLight();
        }
        if (player.gender == 2)
        {
            return vaginaDescript(0);
        }
        if (player.gender == 3)
        {
            return (multiCockDescriptLight() + " and " + vaginaDescript(0));
        }
        return "FUKK: ERROR";
    }
    ...
    public function tentacleJojo() : Bool
    {
        return player.hasStatusEffect(StatusEffects.TentacleJojo);
    }
    override public function campCorruptJojo() : Bool
    {
    ...
    public function corruptCampJojo() : Void
    {
        ...
        else
        {
            ...
            if (tentacleJojo())
            {
                outputText(" A number of tentacles vaugley resembling cocks have sprouted from his back and groin. They sway restlessly around him, oozing thick, fragrant pre from their tips.");
            }
            ...
            if (SceneLib.farm.farmCorruption.hasTattoo("jojo"))
            {
                ...
                {
                    if (SceneLib.farm.farmCorruption.numTattoos("jojo") > 1)
                    {
                        outputText("He has the following tattoos emblazoned across his body:\n");
                    }
                    else
                    {
                        outputText("He has ");
                    ...
                    if (flags[kFLAGS.JOJO_TATTOO_COLLARBONE] != 0)
                    {
                        outputText(flags[kFLAGS.JOJO_TATTOO_COLLARBONE] + "\n");
                    }
                    if (flags[kFLAGS.JOJO_TATTOO_SHOULDERS] != 0)
                    {
                        outputText(flags[kFLAGS.JOJO_TATTOO_SHOULDERS] + "\n");
                    }
                    if (flags[kFLAGS.JOJO_TATTOO_LOWERBACK] != 0)
                    {
                        outputText(flags[kFLAGS.JOJO_TATTOO_LOWERBACK] + "\n");
                    }
                    if (flags[kFLAGS.JOJO_TATTOO_BUTT] != 0)
                    {
                        outputText(flags[kFLAGS.JOJO_TATTOO_BUTT] + "\n");
                    }
                    outputText("\n");
                }
            }
        ...
        menu();
        addButton(0, "Sex", corruptJojoSexMenu).disableIf(player.isGenderless(), "Not for you, genderless!").disableIf(player.lust < 33, "Not aroused enough!");
        addButton(1, "TentacleSex", useTentacleJojo).disableIf(player.lust < 33, "Not aroused enough!").disableIf(!tentacleJojo(), "He has to undergo some <i>changes</i> first...", "???");
        addButton(2, "Milk Him", (flags[kFLAGS.JOJO_TIMES_MILKED] == 0) ? milkJojoFirst : repeatMilkJojo).disableIf(flags[kFLAGS.JOJO_COCK_MILKING_COOLDOWN] > 0, 
        ...
        addButton(3, "TentacleMilk", repeatMilkJojo, true).disableIf(flags[kFLAGS.JOJO_COCK_MILKING_COOLDOWN] > 0, 
                                "Jojo is still drained from his last visit to the milkers - you should wait a few hours before taking him back."
                ).disableIf(!tentacleJojo(), "He has to undergo some <i>changes</i> first...", "???").disableIf(player.hasKeyItem("Cock Milker - Installed At Whitney's Farm") < 0, 
                "You lack some equipment on Whitney's farm...", "???"
        );
        addButton(4, "Lay Eggs", beeEggsInCorruptJojo).disableIf(!player.canOvipositBee(), "Req. a bee ovipositor");
        addButton(5, "Hair Care", jojoPaysForPerms).hint("Use your mouse-slut to pay for your hair-care at Lynette's.").disableIf(!player.hasStatusEffect(StatusEffects.HairdresserMeeting), 
                "You need to find some place to take care of your hair first.", "???"
        );
        if (flags[kFLAGS.FARM_CORRUPTION_STARTED] == 1)
        {
            if (flags[kFLAGS.FOLLOWER_AT_FARM_JOJO] != null)
            {
                addButton(10, "Go Camp", backToCamp);
                if (flags[kFLAGS.FOLLOWER_PRODUCTION_JOJO] != null)
                {
                    addButton(11, "Stop Harvest", stopHarvestJojoDraft);
                }
                else
                {
                    addButton(11, "Harvest Draft", harvestJojoDraft);
                }
            }
            else
            {
        ...
        addButton(14, "Back", (flags[kFLAGS.FOLLOWER_AT_FARM_JOJO] != null) ? SceneLib.farm.farmCorruption.rootScene : camp.campSlavesMenu);
    }
    ...
    private function harvestJojoDraft() : Void
    {
        ...
        {
            flags[kFLAGS.FOLLOWER_PRODUCTION_JOJO] = 1;
        }
    ...
    private function sendToFarm() : Void
    {
        ...
        if (tentacleJojo())
        {
            outputText("-come-tentacle-beast");
        }
    ...
    public function jojoMutationOfferYes() : Void
    {
        ...
        sceneHunter.selectGender(dickF, vagF, null, null)  //[Male]  ;
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            {
                outputText(" while your tits cover his head, nearly suffocating him");
            }
            else
            {
                outputText(" while he suckles one of your breasts hungrily, sucking down your milk");
            }
            ...
            {
                outputText("(Yet somehow your virginity remains intact).  ");
            }
    ...
    private function mutateJojo() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText("caressing your aching twat ");
        }
    ...
    public function useTentacleJojo() : Void
    {
        jojoSprite();
        player.slimeFeed();
        var nippleSucking : Bool = false;
        var mouthFucking : Bool = false;
        var titFucking : Bool = false;
        var cumPuddle : Bool = false;
        var milkPuddle : Bool = false;
        clearOutput();
        ...
        if (player.hasVagina())
        {
            outputText("You feel it split in half, dividing in order to take you from both ends.  ");
        }
        ...
        if (player.cockTotal() > 0)
        {
            ...
            //(1dick+balls ={
                ...
                if (player.hasBalls())
                {
                    ...
                    {
                        outputText("laboriously stretching around");
                    }
                    else if (player.biggestCockArea() >= 20)
                    {
                        outputText("engulfing");
                    }
                    else
                    {
                        outputText("easily swallowing");
                    }
                ...
                else
                {
                    ...
                    if (!player.hasVagina())
                    {
                        outputText("rubbing back and forth along your taint and smearing you with their slime.");
                    }
                    else
                    {
                        ...
                        {
                            outputText("  You nearly cum on the spot from the clitoral stimulation; it's like you have a super-sensitive cock being licked by a thousand tongues.");
                        }
                    }
                    outputText("\n\n");
                }
            ...
            //balls){
                ...
                else
                {
                    ...
                    {
                        outputText("to rub itself along your taint, massaging the sensitive skin with its slimy lubricants.");
                    }
                    outputText("\n\n");
                }
            }
            ...
            if (player.cockTotal() >= 3)
            {
                ...
                if (player.biggestCockArea() > 60)
                {
                    outputText("laboriously working to swallow");
                }
                else if (player.biggestCockArea() >= 20)
                {
                    outputText("engulfing");
                ...
                if (player.cockTotal() > 3)
                {
                    outputText("  The remaining " + Appearance.cockNoun(CockTypesEnum.HUMAN) + "'s pulse and throb with jealous desire, until one of the tentacles slips off and captures it.  They take turns, slipping off and capturing each of your offerings until you wish they'd just keep sucking forever.");
                }
                outputText("\n\n");
            }
        }
        ...
        if (player.cockTotal() == 0 && player.hasVagina())
        {
            ...
            {
                outputText("  The sensation was unreal, and you find yourself wishing your clit was bigger so you could feel even more of the tendrils pleasure-hole.");
            }
            outputText("\n\n");
        }
        ...
        else
        {
            ...
            if (player.hasVagina())
            {
                ...
                {
                    outputText(" practically falling into your oversized cum-receptacles");
                }
                else if (player.vaginas[0].vaginalWetness >= VaginaClass.WETNESS_SLICK)
                {
                    outputText(" sliding inside easily thanks to your copious lubrication");
                }
                else
                {
                    outputText("forcing their way inside your " + vaginaDescript(0) + " and " + assholeDescript());
                }
            }
            //Just butt
            else
            {
                ...
                {
                    outputText(" practically falling into your oversized cum-receptacle");
                }
                else if (player.ass.analWetness >= 3)
                {
                    outputText(" sliding inside easily thanks to your copious lubrication");
                }
                else
                {
                    outputText("forcing its way inside your " + assholeDescript());
                }
            }
        }
        ...
        if (player.cockTotal() > 0)
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("  The tentacle on your [cocks] seems appreciative, and begins sucking to swallow down all your pre.");
            }
            if (player.cockTotal() > 1)
            {
                outputText("  The tentacles on your [cocks] seem appreciative, and begin sucking to swallow down all your pre.");
            }
        }
        if (player.hasVagina())
        {
            outputText("  Your " + vaginaDescript(0) + " spasms around the invader, clenching uncontrollably as it's plowed by the thick vine-prick.");
        }
        outputText("\n\n");
        ...
        if (player.biggestLactation() >= 2)
        {
            ...
            if (player.cor < 80)
            {
                outputText("being such a wanton slut");
            }
            else
            {
                outputText("giving it up too easily");
            }
            ...
            if (player.isRace(Races.COW, 1, false))
            {
                outputText("You find yourself mooing in contentment as your fuck-slave works to milk you.  ");
            }
        ...
        else if (player.biggestTitSize() < 4 && rand(2) == 0)
        {
            ...
            {
                outputText("A squirt of milk escapes and is quickly sucked down by the hungry tentacles.  ");
            }
        ...
        if (player.hasVagina())
        {
            ...
            {
                outputText("even more pregnant");
            }
            else
            {
                outputText("mildly pregnant");
            }
            ...
            {
                outputText(", and soaks him with a wave of puss juice.  ");
            }
            else if (player.vaginas[0].vaginalWetness >= VaginaClass.WETNESS_SLICK)
            {
                outputText(", as your drooling cunt forms a slick puddle.  ");
            }
            else
            {
                outputText(".  ");
                ...
                if (player.cumQ() >= 100)
                {
                    ...
                    if (player.cumQ() >= 300)
                    {
                        ...
                        if (player.cumQ() >= 600)
                        {
                            ...
                            if (player.cumQ() >= 800)
                            {
                                outputText(" but you don't stop cumming until it lies in a deep puddle of jizz");
                            }
                            outputText(".");
                            cumPuddle = true;
                        }
                ...
                if (player.cumQ() >= 100)
                {
                    ...
                    if (player.cumQ() >= 300)
                    {
                        ...
                        if (player.cumQ() >= 600)
                        {
                            ...
                            if (player.cumQ() >= 800)
                            {
                                outputText(" but you don't stop cumming until they lie in a deep puddle of jizz");
                            }
                            outputText(".");
                            cumPuddle = true;
                        }
        ...
        if (player.biggestLactation() >= 2 && nippleSucking)
        {
            ...
            {
                outputText("  Somehow you know they won't get any bigger from his rough treatment.");
            }
        ...
        if (titFucking)
        {
            ...
            if (player.cor >= 80 && player.lib >= 70)
            {
                outputText("You lick your lips and swallow as much of it as possible, wallowing in the mouse-cream.  ");
            }
            if (player.hairLength > 0)
            {
                outputText("He keeps unloading it until you feel it in your " + hairDescript());
            }
            else
            {
                outputText("He keeps unloading it until you feel it on your head");
            }
        ...
        if (mouthFucking)
        {
            ...
            if (player.canGetPregnant() && player.buttPregnancyIncubation == 0)
            {
                outputText("an eight month pregnant woman");
            }
            else
            {
                outputText("a brood-mother immobilized by her growing offspring");
            }
        ...
        if (cumPuddle)
        {
            if (milkPuddle)
            {
                outputText("in the puddle of cum and milk");
            }
            else
            {
                outputText("in the puddle of spunk");
            }
        }
        else if (milkPuddle)
        {
            outputText("in the puddle of milk");
        }
        else
        {
            outputText("on the ground");
        }
    ...
    //Jojo milk payments
    private function jojoMilkPay(tentacle : Bool = false) : Void
    {
        jojoSprite();
        ...
        if (cumQ > 100)
        {
            payout = 2 + as3hx.Compat.parseInt(cumQ / 200) * 2;
        }
        ...
        if (payout > 0)
        {
            ...
            if (payout == 1)
            {
                outputText(cast((payout), Num2Text) + " gem rolls out into a collection plate.  Whitney really put a lot of work into this!  You pocket the gem and g");
            }
            else
            {
                outputText(cast((payout), Num2Text) + " gems roll out into a collection plate.  Whitney really put a lot of work into this!  You pocket the gems and g");
            }
        }
        else
        {
    ...
    public function jojoCumQ() : Float
    {
        var cumQ : Float;
        cumQ = 400;
        if (flags[kFLAGS.JOJO_TIMES_MILKED] < 4)
        {
            cumQ += flags[kFLAGS.JOJO_TIMES_MILKED] * 200;
        }
        else
        {
            cumQ += flags[kFLAGS.JOJO_TIMES_MILKED] * 100;
        }
        if (tentacleJojo())
        {
            cumQ += 500 + flags[kFLAGS.JOJO_TIMES_MILKED] * 100;
        }
        if (flags[kFLAGS.JOJO_BLOWJOB_XP] < 10)
        {
            cumQ += flags[kFLAGS.JOJO_BLOWJOB_XP] * 50;
        }
        else
        {
            cumQ += 500;
        }
        if (flags[kFLAGS.JOJO_SEX_COUNTER] < 10)
        {
            cumQ += flags[kFLAGS.JOJO_SEX_COUNTER] * 25;
        }
        else
        {
            cumQ += 250;
    ...
    {
        jojoSprite();
        var x : Float = player.cockThatFits(40);
        clearOutput();
        if (!recalling)
        {
            outputText("<b>New scene is unlocked in 'Recall' menu!</b>\n\n");
        ...
        if (flags[kFLAGS.TIMES_COCK_MILKED] > 0)
        {
            outputText(", even though it reeks of the spunk you've spilled into the milker's mechanical embrace");
        }
        ...
        if (player.hasCock())
        {
            outputText("It starts to turn your way before it stops and rises, seeming to sniff at the rodent directly above.");
        }
        else
        {
            outputText("It slowly starts to rise, seeming to sniff at the rodent above as it orients itself.");
        }
        ...
        //(Cock - too big!)
        {
            ...
            if (x < 0)
            {
                ...
                if (player.lust < 50)
                {
                    outputText("it stiffen in your hand");
                }
                else
                {
                    outputText("it's rigidity with your hand");
                }
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            {
                outputText("Your other hand grabs your penis-sized 'button' and begins to jack it, the action sending tremors of sensation through your " + hipDescript() + ".  ");
            }
        ...
        var glessF : Void->Void = function() : Void
        {
            if (flags[kFLAGS.TIMES_COCK_MILKED] > 0)
            {
                outputText("You know from experience that machine usually takes an hour before it will allow release");
        ...
        var sharedEnd : Bool->Void = function(cock : Bool = false) : Void
        {
            ...
            if (cock)
            {
                outputText(", squeezing your " + cockDescript(x) + " with agonizingly pleasurable pressure");
            }
            ...
            if (!cock)
            {
                outputText("  His balls tremble and shrink in your mouth, pumping his prodigious semen out, wasting it in exchange for a pittance of gems.");
            }
            outputText("\n\n");
            ...
            //(Too big dick:{
                ...
                {
                    outputText("You release your cum all over Jojo's back, matting his white fur with cum.  Each successive splatter of spunk makes the mouse look even more like a semen-drenched whore.  It's a good look for him.");
                }
                else
                {
                    outputText("You pull out and release your cum all over Jojo's back, matting his white fur with cum.  Each successive splatter of spunk makes the mouse look even more like a semen-drenched whore.  It's a good look for him.");
                }
            }
            else if (player.hasVagina())
            {
                outputText("You climax and clamp down on your fingers, squeezing your hand as your own cries of pleasure mix with your cock-toy's milked-out moans.");
            }
            else
            {
                outputText("You sigh wistfully as Jojo ejaculates his load, wishing you could feel such exquisite sensations.  Maybe you can find something to give you some proper sexual organs soon?");
            }
            ...
            {
                outputText("A loud 'ding' chimes and a panel displays 0.864 Ls.  Ten gems roll out into a collection plate.  Whitney really put a lot of work into this!  You pocket the gems and g");
            }
    ...
    private function repeatMilkJojo(tentacle : Bool = false) : Void
    {
        jojoSprite();
        clearOutput();
        ...
        if (!tentacle || !tentacleJojo())
        {
            ...
            if (jojoCumQ() < 1000)
            {
                outputText("has already dripped everywhere, making a mess of the floor.");
            }
            else if (jojoCumQ() < 4000)
            {
                outputText("is drizzling a steady flow of man-slime, puddling on the floor.");
            }
            else
            {
                outputText("is already pouring out a river of pre-cum, turning the floor into a slippery morass.");
            }
            ...
            {
                outputText(", even though to do so you're forced to stimulate your " + assholeDescript());
            }
        ...
        else
        {
            ...
            {
                outputText("An exhibitionist thrill shivers through you at the thought of being watched while engaged in such a carnal act.  ");
            }
    ...
    private function jojoPaysForPerms() : Void
    {
        ...
        {
            outputText("and rolls them back and forth in her fingers, expertly pleasuring Jojo.");
        }
    ...
    private function amilyTeachingJojoBJ() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(" Do you surprise them both by allowing Amily to demonstrate on you?");
        }
        else
        {
            outputText(" Do you want to watch them fuck?");
        }
    ...
    private function BJRidesGETYOUONE() : Void
    {
        ...
        {
            puss = stuffAmilysMouthWithPostBJCUM;
        }
    ...
    private function fillAmilysMouth() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText(" leans down to desperately suckle at your [sack], subserviently worshipping your bloated testes and their liquid bliss one after another; tonguing, sucking, and moaning into the spunk - bloated mass of your ambrosial scrotum.");
        }
        else
        {
            outputText(" leans down to slurp needily at the bottom of your cock, licking it with languid tongue-strokes each time Amily gives him an opening.");
        }
    ...
    private function amilyAndJojoFuck() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText(" It isn't until you notice how wet your crotch is that you come to the realization that you've masturbated yourself through a few orgasms as well.");
        }
        else
        {
            outputText(" It isn't until you notice how clearheaded you are that you come to the relaization that you've masturbated while watching the pair.");
        }
    ...
    public function jojoFollowerMeditate(doClear : Bool = true) : Void
    {
        jojoSprite();
        if (doClear)
        {
    ...
    public function defeatedJojo(hpVictory : Bool) : Void
    {
        jojoSprite();
    ...
    private function postCombatRape() : Void
    {
        ...
        {
            outputText("teach the uppity monk a lesson...\n\n");
        }
        menu();
        addButton(0, "Next", jojoRape);
    }
    ...
    public function jojoRape() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.EverRapedJojo, 1, 1);
        }
        switch (monk)
        {
            case JOJO_MET:
    ...
    public function jojosSecondRape() : Void
    {
        ...
        var dickF : Void->Void = function() : Void
        {
            ...
            {
                outputText("It makes little difference to your [cock], he still feels tight as a virgin's cunt to you.  ");
            }
            ...
            if (player.cockTotal() > 1)
            {
                outputText("  Your remaining equipment showers him with jizz, more than you ever thought you could produce.");
            }
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            {
                outputText("Your " + vaginaDescript(0) + " drenches him with your squirting girl-cum, mixed with his own seed.");
            }
            if (player.cockTotal() == 1)
            {
                outputText("Jizz sprays onto his chest from your [cock].  ");
            }
            if (player.cockTotal() == 2)
            {
                outputText("A hail of jizz splatters over Jojo from your [cocks].  ");
            }
            if (player.cockTotal() == 3)
            {
                outputText("A multitude of thick cum-streams splatter over Jojo from head to waist as your [cocks] hose him down.  ");
            }
    ...
    public function jojosThirdRape() : Void
    {
        ...
        var dickF : Void->Void = function() : Void
        {
            ...
            {
                outputText("presenting your [cocks] to him.  ");
            }
            ...
            if (player.cockTotal() == 1)
            {
                outputText("to ram your [cock] deep into his maw.  You roughly grab his ears, facefucking him hard, his tongue working hard to please.  ");
            }
            if (player.cockTotal() > 1)
            {
                outputText("to cram two cocks deep into his maw, making his jaw stretch obscenely.  You roughly grab his ears, facefucking him hard, his tongue working hard to please you.  ");
            }
            ...
            if (player.cockTotal() == 1)
            {
                outputText("Your [cock] clenches tightly, erupting ");
            }
            if (player.cockTotal() > 1)
            {
                outputText("Your twin dicks clench tightly, erupting ");
            }
        ...
        var vagF : Void->Void = function() : Void
        {
            if (player.isBiped())
            {
                outputText("You spread your legs and crook your finger");
            }
            else
            {
                outputText("You crook your finger");
            }
            ...
            if (player.biggestLactation() >= 1.5 && player.biggestTitSize() > 2 && player.mostBreastsPerRow() >= 2 && player.breastRows.length >= 1)
            {
                outputText("Every sensual peak within you is mirrored with small spurts of milk from your nipples.  It eventually trickles down to Jojo's tongue, spurring his efforts on.  ");
            }
            ...
            {
                outputText("your jiggling goo ");
            }
            else
            {
                outputText("your thighs ");
            }
            outputText("around his head and quiver with passion, ");
            if (player.averageVaginalWetness() <= 1)
            {
                outputText("squeezing him tightly into your " + vaginaDescript(0) + ".");
            }
            if (player.averageVaginalWetness() > 1 && player.averageVaginalWetness() <= 3)
            {
                outputText("creaming his tongue liberally with girlcum.");
            }
            if (player.averageVaginalWetness() == 4)
            {
                outputText("creaming all over him with your slobbering pussy.");
            }
            if (player.averageVaginalWetness() == 5)
            {
                outputText("splattering him with girlcum from your " + vaginaDescript(0) + ".");
            }
            outputText("\n\nYou pull the cunt-dazed mouse away reluctantly, and bend over,");
            if (player.isBiped())
            {
                outputText(" spreading your legs and");
            }
            outputText(" shaking your rump.  ");
            if (player.hasLongTail() > 0)
            {
                outputText("Your tail swishes back and forth teasingly.  ");
            }
            ...
            if (player.isBiped())
            {
                outputText(" between your thighs");
            }
            else
            {
        ...
        var hermF : Void->Void = function() : Void
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("You sigh as your [cock] droops over his head.  ");
            }
            if (player.cockTotal() > 1)
            {
                outputText("You sigh as your [cocks] pile atop his head.  ");
            }
            if (player.biggestTitSize() >= 2)
            {
            ...
            if (player.averageVaginalWetness() > 1 && player.averageVaginalWetness() < 4)
            {
                outputText("creaming his tongue liberally with girlcum.");
            }
            if (player.averageVaginalWetness() == 4)
            ...
            if (player.averageVaginalWetness() >= 5)
            {
                outputText("splattering him with you girlcum from your " + vaginaDescript(0) + ".");
            }
            if (player.cockTotal() > 0)
            {
                outputText("  Thick runners of your pre dribble down his neck, sticking to his fur.  ");
            }
            outputText("\n\nYou pull the cunt-dazed mouse away reluctantly, and bend over,");
            if (player.isBiped())
            {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("Your [cock] trembles in orgasm, squirting your load into the thick forest loam.  ");
            }
            if (player.cockTotal() > 1)
            {
                outputText("Your [cocks] tremble in orgasm, squirting their hot loads all over the thick forest loam.  ");
            }
            outputText("\n\n");
            outputText("You let the drained mouse pop free, reveling in the sensation of cum pouring from ");
            if (player.isBiped())
        ...
        var sharedEnd : Bool->Void = function(vag : Bool) : Void
        {
    ...
    public function jojosFourthRape() : Void
    {
        ...
        sceneHunter.selectGender(dickF, vagF, null, null, -1)  //Male Version  ;
        ...
        function dickF() : Void
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("You touch his head softly and guide him to your [cock], smiling as he licks his lips with anticipation.  You gasp at the feeling of his soft lips and wet tongue as he pleasures you, your knees going weak from his efforts.  Jojo cups your balls and slips a fuzzy finger into your ass.  He labors to take as much of you as possible into his mouth, panting warmly and wetly around your [cock].  You twitch in pleasure as he alternates pressure on your prostate with smooth strokes of his slick tongue.  ");
            }
            else
            {
                outputText("You touch his head softly and guide him to your [cocks], smiling as he licks his lips in anticipation.  You gasp at the feelings of his soft lips and wet tongue on your heads as he pleasures you, your knees going weak from his efforts.  Jojo cups your balls and slips a fuzzy finger into your ass.  He labors to take in two shafts, struggling to swallow them as deep as possible, panting wetly around you.  You twitch with pleasure as he alternates pressure on your prostate with smooth strokes of his slick tongue.  ");
            }
            //Too thick for him
            if (player.averageCockThickness() > 3 || (player.dogCocks() > 0 && player.averageCockThickness() > 3))
            {
                outputText("Jojo is forced to back off of your [cock] from time to time to come up for air, barely opening his jaw wide enough to take your girth back inside his mouth.  ");
            }
            //Too long for him
            if (player.averageCockLength() > 10 || (player.horseCocks() > 0 && player.averageCockLength() > 10))
            {
                outputText("He struggles not to gag on your [cock]'s length, opening his throat as far as he can.  ");
            }
            outputText("You rock back and forth on his face as he expertly keeps you on the edge of orgasm.  ");
            if (player.cumQ() > 25)
            {
                outputText("Your constant stream of heavy pre practically chokes the poor mouse as he edges you mercilessly, his own equipment drizzling in sympathetic lust.  ");
            }
            ...
            if (player.cumQ() < 25)
            {
                outputText("You buck against him as you orgasm, your small squirts of cum eagerly devoured by the slutty mouse.");
            }
            if (player.cumQ() >= 25 && player.cumQ() < 250)
            {
                outputText("You buck against him as you orgasm, the slutty mouse's throat squeezing against you as he works to swallow your seed.");
            }
            if (player.cumQ() >= 250 && player.cumQ() < 500)
            {
                outputText("You spasm against him as you orgasm, the pleasure erupting into the slut-mouse's throat as he tries to swallow it all.  Excess cum dribbles from the corners of his mouth as you fully spend yourself.");
            }
            if (player.cumQ() >= 500)
            {
        ...
        //Oral
        {
            ...
            if (player.isBiped())
            {
                outputText(" brushes against your calf");
            }
            else
            {
                outputText(" brushes against your [leg]");
            }
            outputText(".  You get a devilish idea, ");
            if (player.lowerBody == LowerBody.GOO)
            {
                outputText("and push his [monster cockshort] in between folds of slime, sandwiching it in a mass of slippery, gooey tightness.  Holding his shoulder for balance, you slowly squeeze him, gently milking out small dribbles of pre.  He redoubles his efforts, burying his nose into your " + vaginaDescript(0) + ", tongue swirling over your folds and around your " + clitDescript() + ".  For every effort on his part you step up your own, squeezing and stroking him with your goo, doing your best to impart a fetish for being masturbated with someone's lower body on his slowly warping mind.  You feel a hot wetness in your slime, and it grows slicker and wetter every second. Jojo's cum drips out of you, glazing your jello-like form white.  ");
            }
            else
            {
                outputText("and push his [monster cockshort] with your [foot], sandwiching it under one [foot] and on top of the other.  Holding his shoulder for balance, you slowly squeeze him, gently milking out small dribbles of pre.  He redoubles his efforts, burying his nose into your " + vaginaDescript(0) + ", tongue swirling over your folds and around your " + clitDescript() + ".  For every effort on his part you step up your own, squeezing and stroking him with your [feet], doing your best to impart a [foot] fetish on his slowly warping mind.  You feel a hot wetness on your [feet], and they grow slicker and wetter every second. Jojo's cum drips out from between them, glazing them white.  ");
            }
            if (player.averageVaginalWetness() <= 1)
            {
                outputText("You clamp down on his muzzle as you writhe in orgasm.  ");
            }
            if (player.averageVaginalWetness() > 1 && player.averageVaginalWetness() <= 3)
            {
                outputText("Your legs trap his muzzle in your " + vaginaDescript(0) + " as orgasm wracks your body.  ");
            }
            if (player.averageVaginalWetness() == 4)
            {
                outputText("Your legs trap his muzzle in your " + vaginaDescript(0) + ", slicking his muzzle with girlcum as you spasm with bliss.  ");
            }
            if (player.averageVaginalWetness() == 5)
            {
                outputText("Your legs squeeze him against your " + vaginaDescript(0) + ", girlcum erupting over his face and soaking him as you bliss out with orgasm  ");
            }
            if (player.cockTotal() == 1)
            {
                outputText("Your [cock] drizzles cum in his hair, some dripping off by his ear.  ");
            }
            if (player.cockTotal() > 1)
            {
                outputText("Your [cock]s drizzle him with cum, covering his head with thick white streams of your jizz.  ");
            }
            ...
            if (player.averageVaginalLooseness() == 0)
            {
                outputText("He is painfully large, so much so that you feel more pain than pleasure.  ");
            }
            if (player.averageVaginalLooseness() == 1)
            {
                outputText("He stretches you around him like a latex glove, pulling your " + vaginaDescript(0) + " taught with his [monster cockshort], the sensation riding a razor's edge between pleasure and pain.  ");
            }
            if (player.averageVaginalLooseness() == 2)
            {
                outputText("His [monster cockshort] stuffs you completely, filling your " + vaginaDescript(0) + " to capacity.  ");
            }
            if (player.averageVaginalLooseness() == 3)
            {
                outputText("His [monster cockshort] fits you perfectly, burying deep inside your folds.  ");
            }
            if (player.averageVaginalLooseness() == 4)
            {
                outputText("You easily accomadate his member into your " + vaginaDescript(0) + ".  ");
            }
            if (player.averageVaginalLooseness() == 5)
            {
                outputText("His [monster cockshort] slips inside your " + vaginaDescript(0) + " with little resistance, easily sinking in to the hilt.  You muse to yourself, \"<i>If only he were thicker...</i>\"  ");
            }
            ...
            //Single Cock{
                ...
                //Horsefun!{
                    ...
                    {
                        outputText("Your [cock] feels a building pressure, the whole thing pulsating wildly with each of your heartbeats, most noticably the tip, which flares out wildly.  Powerful contractions wrack your sheath and [cock] as pre practically fountains from it.  ");
                    }
                    //DogFun!
                    if (player.dogCocks() == 1)
                    {
                        outputText("Your [cock] feels an intense pressure, and begins bulging out obscenely above your sheath.  The knot thickens gratuitiously, filling as it pulses with need.  Cum drips from your pointed tip as it continues to bulge wider, filling you with unbearable pressure.  ");
                    }
                    //Else
                    if (player.normalCocks() == 1)
                    {
                        outputText("Your [cock] twitches, muscle contractions slowly working their way up from the base.  ");
                    }
                    //CUMSPLOISION
                    if (player.cumQ() < 25)
                    {
                    ...
                    if (player.cumQ() >= 25 && player.cumQ() < 250)
                    {
                        if (player.horseCocks() == 1)
                        {
                            outputText(" Your [cock] flares wildly as musky horse-cum erupts from it, splattering over Jojo.  ");
                        }
                        if (player.dogCocks() == 1)
                        {
                            outputText("The terrible pressure in your [cock] finally relents, in the form of a fountain of doggie-cum, spraying out from your [cock] in a steady stream that seems to last and last.  ");
                        }
                        if (player.cocks[0].cockType == CockTypesEnum.HUMAN || player.cocks[0].cockType.Index > 2)
                        {
                    ...
                    if (player.cumQ() >= 250)
                    {
                        if (player.horseCocks() == 1)
                        {
                            outputText("Your [cock] ripples and bulges with animalistic vigor, horse-cum splattering everywhere with each contraction.  The musky animal-jizz never seems to stop pouring from your equine organ, soaking the mouse from the waist up.  ");
                        }
                        if (player.dogCocks() == 1)
                        {
                            outputText("Your [cock] suddenly releases the pressure, a constant stream of doggie-cum spouting from your [cock] like some kind of cum-hose.  It seems to go on endlessly, covering the mouse from the waist up with thick ribbons of doggie-spooge as your knot slowly shrinks to normal.  ");
                        }
                        if (player.cocks[0].cockType == CockTypesEnum.HUMAN || player.cocks[0].cockType.Index > 2)
                        {
                            outputText("Your body tenses and cums a thick eruption far beyond what a normal human could produce.  Jojo is splattered with the stuff.  ");
            ...
            if (player.biggestLactation() >= 1 && player.biggestTitSize() > 3)
            {
                ...
                {
                    outputText("Milk sprays from your " + breastDescript(0) + " in tiny streams, triggered by your orgasms.");
                }
                if (player.biggestTitSize() * player.biggestLactation() * player.breastRows.length < 20 &&
                    player.biggestTitSize() * player.biggestLactation() * player.breastRows.length >= 8)
                {
                    outputText("Milk erupts from your " + breastDescript(0) + ", spraying out over the mouse, squirting out the contractions of each shuddering orgasm.");
                }
                if (player.biggestTitSize() * player.biggestLactation() * player.breastRows.length < 35 &&
                    player.biggestTitSize() * player.biggestLactation() * player.breastRows.length >= 20)
                {
                    outputText("Milk erupts from your " + breastDescript(0) + ", spraying in pulsing blasts, soaking the mouse.  Each nerve-wracking orgasm seems to wring more and more milk from you, until it puddles around.");
                }
                if (player.biggestTitSize() * player.biggestLactation() * player.breastRows.length >= 35)
                {
                    outputText("Milk fountains from your " + breastDescript(0) + ", soaking the mouse with a continuous river of cream.  For every blob of cum you feel pushing into your over-filled uterus, another torrent of milk sprays out.  As your mind-bending orgasms drag on, a small lake of milk forms around you.");
                }
            }
            if (!recalling)
            {
        ...
        else
        {
            ...
            if (player.lust >= player.maxOverLust() || recalling)
            {
                ...
                if (player.gender >= 2)
                {
                    ...
                    if (player.cockTotal() > 1)
                    {
                        outputText("Your [cock] splatters the ground with cum repeatedly, until both your genders are raw and sore.  ");
                    }
                    else
                    {
                        outputText("Your " + vaginaDescript(0) + " cums on him many more times it until it is sore and tender, dripping with spunk.  ");
                    }
                    ...
                    if (!recalling)
                    {
                        ...
                        {
                            player.knockUp(PregnancyStore.PREGNANCY_MOUSE, PregnancyStore.INCUBATION_MOUSE + 82, 101);
                        }  //Jojo's kids take longer for some reason  
                        player.sexReward("cum", "Vaginal");
                    }
                }
    ...
    private function corruptJojoSexMenu() : Void
    {
        ...
        addButton(5, (player.biggestLactation() > 1) ? "Suckle" : "Breasts", corruptJojoBreasts).disableIf(player.biggestTitSize() < 2, "You need boobs for that!");
        addButton(1, "Gentle BJ", corruptJojoBJGentle).disableIf(!player.hasCock(), "Req. a cock!");
        addButton(6, "Cruel BJ", corruptJojoBJCruel).disableIf(!player.hasCock(), "Req. a cock!");
        addButton(2, "Gentle Anal", corruptJojoAnalGentle).disableIf(!player.hasCock(), "Req. a cock!");
        addButton(7, "Cruel Anal", corruptJojoAnalCruel).disableIf(!player.hasCock(), "Req. a cock!");
        addButton(12, "Whisper", whisperJojobait).disableIf(player.hasPerk(PerkLib.Whispered), "You need to submit to some jaguar-like demon for it.").disableIf(!player.hasCock(), "Req. a cock!");
        addButton(3, "Gentle Vaginal", corruptJojoVaginalGentle).disableIf(!player.hasVagina(), "Req. a vagina!");
        addButton(8, "Vag. Smother", corruptJojoVaginalSmother).disableIf(!player.hasVagina(), "Req. a vagina!");
        addButton(13, "Anal Smother", corruptJojoAnalSmother).disableIf(!player.hasVagina(), "Req. a vagina!");
    ...
    private function corruptJojoBreasts() : Void
    //Should only be available to players with biggestBreastSize > 2
    {
        ...
        //For suckling the scene is the same regardless of player's gender{
            ...
            if (player.breastRows.length > 1)
            {
                outputText("and then all the others, ");
            }  //Extra boob coverage  
    ...
    private function beeEggsInCorruptJojo() : Void
    {
        ...
        if (!player.hasCock())
        {
            if (flags[kFLAGS.TIMES_EGGED_JOJO] == 0)
            {
                outputText("Jojo looks confused at this; he knows you don't have a cock!  ");
            }
            else
            {
                outputText("Jojo looks confused at first, and then nods in understanding, remembering the last time you laid in him.  ");
            }
        }
        ...
        if (player.isNaga())
        {
            outputText("slither around him");
        }
        else if (player.isGoo())
        {
            outputText("ooze over him");
        }
        else if (player.isTaur())
        {
            outputText("straddle him");
        }
        else
        {
            outputText("climb atop him");
        }
        ...
        if (player.eggs() > 30)
        {
            outputText("  His belly is slightly rounded by the time you finish, with small, egg-shaped bumps visible through his fur.");
        }
    ...
    private function rapeButton(btn : Int, camp : Bool) : CoCButton
    {
        return addButton(btn, "Rape", (camp) ? jojoAtCampRape : jojoRape).hint("Rape the poor monk." + ((player.cor < 50) ? "  Why would you do that?" : "")).disableIf(player.lust < 33, "Not horny enough.").disableIf(player.cor < 33, "You're not corrupted enough to betray your friend.").disableIf(player.isGenderless(), "You're genderless...").disableIf(flags[kFLAGS.DISABLED_JOJO_RAPE], "You can just ask him nicely.");
    }
    ...
    public function corruptJojoEncounter() : Void
    {
        ...
        if (monk == 2)
        {
            outputText("his mousey visage twisted into a ferocious snarl.  \"YOU!\" he screams, launching himself towards you, claws extended.");
        }
        if (monk == 3)
        {
            outputText("unsteady on his feet, but looking for a fight!");
        }
        if (monk == 4)
        {
            outputText("visibly tenting his robes, but intent on fighting you.");
        }
        if (monk == 5)
        {
            outputText("panting and nude, his fur rustling in the breeze, a twitching behemoth of a cock pulsing between his legs.");
        }
    ...
    public function jojoCamp() : Void
    {
        ...
        if (player.cor > 10 && flags[kFLAGS.JOJO_LAST_MEDITATION] != model.time.days)
    ...
    //Normal Follower Choices
    {
        ...
        if (flags[kFLAGS.UNLOCKED_JOJO_TRAINING] == 1)
        {
            addButton(2, "Train", apparantlyJojoDOESlift).hint("Join him in a training session.");
        }
        ...
        if (player.hasStatusEffect(StatusEffects.Infested))
        {
            addButton(5, "Purge", wormRemoval).hint("Request him to purge the worms from your body.");
        }
    ...
    public function talkMenu() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_TALKED_WITH_JOJO] >= 4)
        {
            addButton(4, "You", jojoTalkYourOrigin).hint("Tell him about Ingnam and your history.");
        }
        if (flags[kFLAGS.FACTORY_SHUTDOWN] > 0)
        {
            addButton(5, "Factory", jojoTalkFactory).hint("Tell him about how you've shut down the factory.");
        }
        if (flags[kFLAGS.SAND_WITCHES_COWED] == 1 || flags[kFLAGS.SAND_WITCHES_FRIENDLY] == 1 || flags[kFLAGS.SAND_MOTHER_DEFEATED] == 1)
        {
            addButton(6, "SandCave", jojoTalkSandCave).hint("Tell him about your encounter in the Sand Cave in the desert.");
        }
        if (flags[kFLAGS.UNLOCKED_JOJO_TRAINING] == 0 && flags[kFLAGS.TIMES_TALKED_WITH_JOJO] >= 4)
        {
            addButton(7, "Training", apparantlyJojoDOESlift).hint("Ask him if he's willing to train you.");
        }
        if (flags[kFLAGS.MINERVA_PURIFICATION_JOJO_TALKED] == 1 && flags[kFLAGS.MINERVA_PURIFICATION_PROGRESS] < 10)
        {
            addButton(8, "Purification", SceneLib.mountain.minervaScene.minervaPurification.purificationByJojoPart1).hint("Ask him if he can exorcise the demonic parasite infesting Minerva.");
        }
        //Sex button
        if (player.cor <= (10 + player.corruptionTolerance) && player.lust >= 33)
        {
            ...
            if (flags[kFLAGS.TIMES_TALKED_WITH_JOJO] < 4)
            {
                addButtonDisabled(9, "Sex?", "You should socialize with Jojo a bit more.");
            }
        }
        if (player.cor <= 10 && player.lust >= 33 && monk == -1)
        {
            addButtonDisabled(9, "Sex?", "You need to spend more time with Jojo. \n\nTalk sessions: " + flags[kFLAGS.TIMES_TALKED_WITH_JOJO] + "/6 \nTraining sessions: " + flags[kFLAGS.TIMES_TRAINED_WITH_JOJO] + "/10 \nMeditation sessions: " + player.statusEffectv1(StatusEffects.JojoMeditationCount) + "/10 \nYou must be pure enough and have sufficient lust as well.");
        }
        if (player.cor <= 10 && player.lust >= 33 && flags[kFLAGS.TIMES_TALKED_WITH_JOJO] >= 6 && flags[kFLAGS.TIMES_TRAINED_WITH_JOJO] >= 10 && player.statusEffectv1(StatusEffects.JojoMeditationCount) >= 10 && monk > -3)
        {
            addButton(9, "Sex?", offerSexFirstTimeHighAffection).hint("You've spent quite the time with Jojo, maybe you can offer him if he's willing to have sex with you?");
        }  //Will unlock consensual sex scenes.  
        if (monk <= -3)
        {
            removeButton(9);
        }
        if (TyrantiaFollower.TyrantiaFollowerStage == 5)
        {
            addButton(10, "Tyrantia", TyrantiaEggQuestJoJo);
        }
    ...
    public function jojoTalkSandCave() : Void
    {
        ...
        {
            // FRIENDLY is the overall victory flag (theres a separate flag used to indicate you dun wanna be attacked by sandbitches in the desert anymore)if (flags[kFLAGS.SAND_WITCHES_FRIENDLY] == 1 && flags[kFLAGS.SAND_MOTHER_DEFEATED] == 1)
            {
        ...
        if (flags[kFLAGS.MET_MILK_SLAVE] == 1)
        {
            ...
            else if (Std.is(flags[kFLAGS.MILK_NAME], String))
            {
    ...
    public function apparantlyJojoDOESlift() : Void
    {
        ...
        if (!player.hasPerk(PerkLib.ControlledBreath) && !player.hasPerk(PerkLib.CleansingPalm) && !player.hasPerk(PerkLib.Enlightened))
        {
            ...
            if (flags[kFLAGS.TIMES_TRAINED_WITH_JOJO] == 5)
            {
                ...
                {
                    outputText(" rises and falls");
                }
                else
                {
                    outputText(" rise and fall");
                }
        ...
        else
        {
            
            {
                ...
                var enlightenedBlurbs : Array<Dynamic> = [];
                ...
                {
                    enlightenedBlurbs.push("You can hear Ember cleaning" + emberScene.emberMF("his", "her") + "scales.");
                }
                if (player.hasStatusEffect(StatusEffects.CampRathazul))
                {
                    enlightenedBlurbs.push("You can hear Rathazul experimenting with surprisingly nimble fingers.");
                }
                if (sophieFollower() && !player.hasStatusEffect(StatusEffects.SophieOff))
                {
                ...
                if (flags[kFLAGS.IZMA_FOLLOWER_STATUS] > 0)
                {
                    ...
                    {
                        enlightenedBlurbs.push("You can hear Izmael doing push-ups to stay fit.");
                    }
                    else
                    {
                        enlightenedBlurbs.push("You can hear Izma flipping through the pages of a book.");
                    }
                }
    ...
    public function wormRemoval() : Void
    {
        ...
        if (player.HP > as3hx.Compat.parseInt(player.maxHP() * .5))
        {
            player.HP = as3hx.Compat.parseInt(player.maxHP() * .5);
        }
    ...
    public function jojoFreaksOutSoulless() : Void
    {
        ...
        var saveMeFromMyWickedness : Void->Void = function() : Void
        {
    ...
    public function jojoForestPurifyMeFromDevilry() : Void
    {
        ...
        var availableArtifacts : Array<Dynamic> = player.allPureLegendaryItems();
        var button : Int = 0;
        menu();
        for (item in availableArtifacts)
        {
        ...
        var jojoDevilPurificationScene : ItemType->Void = function(item : ItemType) : Void
        {
    ...
    public function offerSexFirstTimeHighAffection() : Void
    {
        ...
        if (player.inte >= 60 && player.cor <= (10 + player.corruptionTolerance))
        {
            addButton(2, "Confront", confrontChastity);
        }
    }
    ...
    public function confrontChastity() : Void
    {
        ...
        if (silly())
        {
            outputText("<b> Jojo can only learn four moves. Delete a move to learn a new one? Yes. 1... 2... 3... Poof! Jojo has forgotten Chastity and learned Sex!</b>");
        }
    ...
    {
        var capacity : Int = 40;
        if (flags[kFLAGS.JOJO_ANAL_XP] < 10)
        {
            capacity += as3hx.Compat.parseInt(flags[kFLAGS.JOJO_ANAL_XP] * 3);
        }
        else
        {
            capacity += 30;
        ...
        if (flags[kFLAGS.JOJO_SEX_COUNTER] < 3)
        {
            outputText("Jojo looks into your eyes and says, \"<i>Only if you're going to be gentle.</i>\"");
        }
        else
        {
            outputText("Jojo looks into your eyes and says, \"<i>Yes, young one. Let's go.</i>\"");
        }
        ...
        menu();
        addButton(0, "Anal Pitch", anallyFuckTheMouseButtSlut).hint("Fuck the monk mouse-morph's butt.").disableIf(player.cockThatFits(capacity) < 0, "Req. a cock fitting " + capacity + " capacity.");
        addButton(1, "Anal Catch", getAnallyFuckedByMouseYouSlut).hint("Have Jojo penetrate you anally.");
        addButton(2, "Vaginal Catch", getVagFuckedByMouse).hint("Have Jojo penetrate you vaginally.").disableIf(!player.hasVagina(), "Req. a vagina.");
    ...
    private function anallyFuckTheMouseButtSlut() : Void
    {
        ...
        jojoSprite();
        clearOutput();
        if (flags[kFLAGS.JOJO_ANAL_XP] == 0)
        {
            outputText("You finally make up your mind; you want to stuff your cock into that tight ass of his. " + player.clothedOrNaked("You remove your [armor] and ") + "Jojo hesitantly strips out of his robe, revealing his naked form.");
        }
        else if (flags[kFLAGS.JOJO_ANAL_XP] == 1)
        {
            outputText("You decide that you want to stuff your cock into that tight ass of his again. " + player.clothedOrNaked("You remove your [armor] and ") + "Jojo hesitantly strips out of his robe, revealing his naked form.");
        }
        else if (flags[kFLAGS.JOJO_ANAL_XP] >= 2)
        {
            outputText("You decide that you want to stuff your cock into that tight ass of his again. " + player.clothedOrNaked("You remove your [armor] and ") + "Jojo without hesitation strips out of his robe, revealing his naked form.");
        }
        ...
        //Intro{
            ...
            if (flags[kFLAGS.JOJO_ANAL_XP] < 1)
            {
                outputText("He’s too tight to stick your " + player.cockDescript(x) + " in right off; he needs stretching. You warn Jojo that you're going to start by fingering him, and he shakes with trepidation as he nods. You slick your hand up with some spit and then go for it. ");
            }
            else
            {
                outputText("You warn Jojo you're going to test him with your hand first. \"<i>If you have to,</i>\" he says, closing his eyes. You slick your fingers up with some saliva, and then gently probe Jojo’s pucker.");
            }
            ...
            if (flags[kFLAGS.JOJO_ANAL_XP] < 1)
            {
                outputText("You take your time about it, since he's worried enough, but eventually you think you might be able to get your cock in.");
            }
            else
            {
                outputText("He relaxes much faster than he did last time. You should be good.");
            }
        ...
        //The End
        if (flags[kFLAGS.JOJO_SEX_COUNTER] >= 4)
        {
            outputText("He plants a kiss on your lips.");
        }
    ...
    {
        var isVirgin : Bool = (player.looseness(false) == 0);
        jojoSprite();
        clearOutput();
        ...
        if (isVirgin)
        {
            outputText(" \"<i>That's really tight; I'm going to enjoy your tightness while it lasts,</i>\" Jojo remarks. ");
        }
        else if (player.looseness(false) < 2)
        {
            outputText(" \"<i>That's tight,</i>\" Jojo says. ");
        }
        else
        {
            outputText(" \"<i>That's nice,</i>\" Jojo remarks. ");
        }
        ...
        if (player.hasCock())
        {
            outputText("[EachCock] twitches with pleasure, soaking the ground beneath you with a small lake of cum. ");
        }
        if (player.hasVagina())
        {
            outputText("Your [pussy] spontaneously " + ((player.averageVaginalWetness() >= 4) ? "sprays" : "leaks") + " juices all over the ground. ");
        }
        if (player.gender == 0)
        {
            outputText("Your body rocks with ecstasy. ");
        }
    ...
    private function suckJojosCock() : Void
    {
        jojoSprite();
        clearOutput();
        if (flags[kFLAGS.JOJO_BLOWJOB_XP] <= 0)
        {
            outputText("You tell Jojo that you feel like it’s his turn to be pleasured. " + player.clothedOrNaked("You remove your [armor] and ") + "Jojo hesitantly drops his pants, revealing his sheathed cock.");
        }
        else
        {
        ...
        if (jojoCumQ() < 2500)
        {
            player.refillHunger(jojoCumQ() / 25);
        }
        else
        {
    ...
    public function mishapsLunaJojo() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.CampLunaMishaps1, 2, 1);
        }
        else
        {
            player.createStatusEffect(StatusEffects.CampLunaMishaps1, 0, 1, 0, 0);
        }
