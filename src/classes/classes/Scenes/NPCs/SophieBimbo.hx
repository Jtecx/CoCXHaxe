...
class SophieBimbo extends NPCAwareContent
{
    private var pregnancy(get, never) : PregnancyStore;
    public var cockFits(get, never) : Bool;
    public var nofitMsg(get, never) : String;
    public var eggTypes(get, never) : Array<Dynamic>;

    private function get_pregnancy() : PregnancyStore
    {
        return SceneLib.sophieScene.pregnancy;
    ...
    private function sophieSprite() : Void
    {
        if (flags[kFLAGS.SOPHIE_BIMBO_ACCEPTED] > 0 && flags[kFLAGS.SOPHIE_DISABLED] == 0)
        {
            spriteSelect(SpriteDb.s_sophieBimbo);
        }
        else
        {
            spriteSelect(SpriteDb.s_sophie);
        }
    }
    
    override public function bimboSophie() : Bool
    {
        return (flags[kFLAGS.SOPHIE_BIMBO_ACCEPTED] > 0 && flags[kFLAGS.SOPHIE_DISABLED] == 0 && flags[kFLAGS.SOPHIE_DEBIMBOED] == 0);
    }
    
    public function sophieIsInSeason() : Bool
    {
        return (flags[kFLAGS.SOPHIE_HEAT_COUNTER] > 551 && !pregnancy.isPregnant);
    }
    
    @:allow(classes.scenes.nPCs)
    private function sophiePregChance() : Void
    {
    ...
    public function sophieCapacity() : Float
    {
        return 179;
    }
    private function get_cockFits() : Bool
    {
    ...
    @:allow(classes.scenes.nPCs)
    private function bimbotizeMeCaptainSophie() : Void
    {
        sophieSprite();
        clearOutput();
        ...
        {
            outputText("touch herself");
        }
        else
        {
            outputText("squirm in the dirt around her nest");
        }
    ...
    private function acceptBimboSophie() : Void
    {
        ...
        {
            pregnancy.knockUpForce(pregnancy.type, as3hx.Compat.parseInt(pregnancy.incubation * 1.75));
        }
        ...
        if (player.canFly())
        {
            outputText(" and unable to bring your wings to bear with arms pinning them to your back");
        }
        outputText(".\n\n");
        ...
        else if (flags[kFLAGS.AMILY_FOLLOWER] > 0 && flags[kFLAGS.AMILY_BLOCK_COUNTDOWN_BECAUSE_CORRUPTED_JOJO] == 0)
        {
            ...
            {
                outputText("flushes hotly and denies the bimbo's request with a terse 'no'.");
            }
            else
            {
                outputText("flushes hotly and wiggles her hips Sophie's way.  The slutty, corrupted mouse and Sophie will clearly be helping to sate each other's needs in your absence.");
            }
        }
        ...
        else if (SceneLib.isabellaFollowerScene.isabellaFollower())
        {
            ...
            if (isabellaAccent())
            {
                outputText("\"<i> Nein!  But you can drink mein milk from time to time.</i>\"");
            }
            else
            {
                outputText("\"<i>No! But you can still drink my milk from time to time.</i>\"");
            }
    ...
    public function sophieCampLines() : Void
    {
        ...
        if (choice == 0)
        {
            outputText("Sophie is sitting by herself, applying yet another layer of glittering lip gloss to her puffy bimbo lips.");
        }
        else if (choice == 1)
        {
            outputText("Sophie is sitting by herself, idly diddling her twat and noisily moaning.  She keeps looking your way, hoping you'll approach.");
        }
        else if (choice == 2)
        {
            outputText("The beach-blond harpy is preening herself with a vapid, brainless look plastered on her face.  Her puffy, bee-stung lips are slightly parted and lacquered with a heavy layer of golden gloss.");
        }
        else if (choice == 3)
        {
            outputText("Your personal bimbo, Sophie, is playing with her breasts, touching and caressing the heavy orbs in between sighs of pleasure.  She keeps glancing your way, hoping you'll join her.  She's insatiable!");
        }
        else
        {
            outputText("Sophie is sitting by herself, posing a duckface and taking a selfie on her FenPhone. Wait, where did she get that?");
        }
        outputText("\n\n");
    }
    //Approach Texts
    public function approachBimboSophieInCamp(output : Bool = true) : Void
    {
        sophieSprite();
        if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 0)
        {
            if (output)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (player.hasCock())
                        {
                            outputText(" cock-pillows slurping around your member.");
                        }
                        else if (player.hasVagina())
                        {
                            outputText(", fuckable pussy-pillows crushing against your quim.");
                        }
                        else
                        {
                            outputText(", fuckable pillows locking you in their embrace.");
                        }
                        if (player.cor < 33)
                        {
                            outputText("  You find yourself nodding and blushing heavily as you drink in the bimbo's body.");
                        }
                        else if (player.cor < 66)
                        {
                            outputText("  You find yourself nodding and giving a lewd grin as you drink in the bimbo's body.");
                        }
                        else
                        {
                            outputText("  You nod vigorously, giving the harpy a lecherous leer as you take in her body.");
                        }
        ...
        menu();
        addButton(0, "Sex", bimboSophieSexMenu).disableIf(player.lust < 33, "You aren't turned on enough right now to make use of Sophie's 'services'.");
        if (flags[kFLAGS.RATHAZUL_DEBIMBO_OFFERED] != null)
        {
            if (output)
            {
            ...
            addButton(4, "Debimbo", sophieFollowerScene.unbimboSophie).disableIf(!player.hasItem(consumables.DEBIMBO), "You don't have any.");
        }
        if (flags[kFLAGS.SOPHIE_CAMP_EGG_COUNTDOWN] > 0 && output)
        {
        ...
        if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] != null)
        {
            addButton(1, "Go Camp", backToCamp);
            if (flags[kFLAGS.FOLLOWER_PRODUCTION_SOPHIE] != null)
            {
            ...
            {
                addButton(2, "Harvest Eggs", harvestEggs);
            }
            addButton(14, "Back", SceneLib.farm.farmCorruption.rootScene);
        }
        else
        {
            if (flags[kFLAGS.FARM_CORRUPTION_STARTED] != null)
            {
                addButton(1, "Farm Work", sendToFarm).disableIf(pregnancy.isPregnant, "She's pregnant!");
            }
    ...
    private function sophieBimboAppearance() : Void
    {
        ...
        if (flags[kFLAGS.SOPHIE_BIMBO_ACCEPTED] > 0)
        {
            outputText("so immense they would drag her to the ground if she tried to fly, a sexy, lingering aftereffect of the draft you gave her.  The wobbly bimbo-tits look to be about F-cups.");
        }
        else
        {
            outputText("unusually large.  The buxom harpy does seem slightly encumbered by them, but still capable of flight.  Her breasts look to be about DD-cups, nice and big enough for a soft squeeze or rough tit-fuck.");
        }
        ...
        if (SceneLib.farm.farmCorruption.hasTattoo("sophie"))
        {
            ...
            {
                if (SceneLib.farm.farmCorruption.numTattoos("sophie") > 1)
                {
                ...
                if (flags[kFLAGS.SOPHIE_TATTOO_COLLARBONE] != 0)
                {
                    outputText(flags[kFLAGS.SOPHIE_TATTOO_COLLARBONE] + "\n");
                }
                if (flags[kFLAGS.SOPHIE_TATTOO_SHOULDERS] != 0)
                {
                    outputText(flags[kFLAGS.SOPHIE_TATTOO_SHOULDERS] + "\n");
                }
                if (flags[kFLAGS.SOPHIE_TATTOO_LOWERBACK] != 0)
                {
                    outputText(flags[kFLAGS.SOPHIE_TATTOO_LOWERBACK] + "\n");
                }
                if (flags[kFLAGS.SOPHIE_TATTOO_BUTT] != 0)
                {
                    outputText(flags[kFLAGS.SOPHIE_TATTOO_BUTT] + "\n");
                }
            }
        }
    ...
    public var eggColors : Array<Dynamic> = [
        "Black(Ltx)", 
        "Blue(-Vg)", 
        "Brown(Ass)", 
        "Pink(-Dk)", 
    ...
    private var _eggTypes : Array<Dynamic>;
    
    private function get_eggTypes() : Array<Dynamic>
    {
        if (_eggTypes == null)
        {
            _eggTypes = [
    ...
    private function stopHarvest() : Void
    {
        ...
        {
            outputText("\"<i>I only wish I was good with numbers and things,</i>\" Sophie sighs wistfully, picking at her bellybutton. \"<i>Then I’d know when to give you my eggs and, and when to give them to the doggie. Whatever you say anyways, babe!</i>\"");
        }
    ...
    private function bimboSophieSexMenu() : Void
    {
        ...
        menu();
        addButton(0, "Anal", buttFuckBimboSophie).disableIf(!cockFits, nofitMsg);
        addButton(1, "Nice Fuck", sophieFenCraftedSex).disableIf(!cockFits, nofitMsg);
        addButton(2, "Rough Fuck", rideDatSophieVag).disableIf(!cockFits, nofitMsg);
        addButton(3, "Impregnate", sophieImpregnationSex).disableIf(!sophieIsInSeason(), "Only available when she's in season.").disableIf(!cockFits, nofitMsg);
        addButton(4, "Big Impreg", giantCocksImpregnateSophie).hint("You can always try to impregnate her with your biggest member...").disableIf(!sophieIsInSeason(), "Only available when she's in season.").disableIf(player.findCock(1, sophieCapacity(), -1) < 0, 
                "Req. a cock with area larger than " + sophieCapacity() + "."
        );
        addButton(5, "Get Licked", bimboSophieLicksRugs).disableIf(!player.hasVagina(), "Req. a vagina.");
        addButton(7, "Titty-Fuck", fuckBimboSophiesTits).disableIf(!player.hasCock(), "Req. a cock.").disableIf(pregnancy.event >= 2, "Not available when she's pregnant.");
        addButton(8, "TitFuckPreg", sophiePreggoTitJobs).disableIf(pregnancy.event < 2, "She's not pregnant - or it isn't visible.").disableIf(!player.hasCock(), "Req. a cock.");
        addButton(10, "Ovi Elixir", sophieEggApocalypse).disableIf(!player.hasItem(consumables.OVIELIX) && !inventory.hasItemInStorage(consumables.OVIELIX), "You don't have any.").disableIf(pregnancy.isPregnant, "She's pregnant already.").disableIf(player.isGenderless(), "Not for you, genderless.").disableIf(flags[kFLAGS.TIMES_SOPHIE_HAS_DRUNK_OVI_ELIXIR] == 0, 
                "Nothing will happen if you just accidentally forget an Ovi Elixir in your camp storage... right?"
        );
        if (flags[kFLAGS.TOLD_SOPHIE_TO_IZMA] == null)
        {
            addButton(11, "Visit Izma", tellSophieToVisitIzma).hint("You could tell Sophie to visit Izma again.  She'd probably wind up tied AGAIN.").disableIf(flags[kFLAGS.TIMES_SOPHIE_AND_IZMA_FUCKED] == 0, "Just wait.", "???").disableIf(flags[kFLAGS.IZMA_NO_COCK], "Izma doesn't possess a cock. This button will never be available, you can hide it somewhere.", "???").disableIf(!SceneLib.izmaScene.izmaFollower(), "You don't have any sharks at your camp. Why would you?", "???");
        }
        if (!(flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] > 0 && flags[kFLAGS.SOPHIE_FAMILY_INCEST] == 0))
        
        //hide if disabled{
            
            addButton(12, "DaughterFuck", sophieFollowerScene.sophieIncest).disableIf(!cockFits, nofitMsg).disableIf(flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] == 0, "You need to have any adult kids with Sophie. She'll talk to you.\n\n"
    ...
    public function bimboSophieLustStickSurprise() : Void
    {
        ...
        else if (select == 2)
        {
            ...
            else
            {
                ...
                if (player.hasVagina())
                {
                    outputText("vulva, sucking at your clitoral hood.  Those gold, slobbery pussy-lickers look good on your groin, and you find yourself releasing your wetness from her oral attentions.  It might be time to fuck her soon...");
                }
                else
                {
                    outputText("smooth groin, eventually licking at your [asshole].  Those gold, slobbery lickers look good pressed against you.  It might be time to fuck her soon...");
                }
            }
            sophieScene.luststickApplication(5);
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function daughterCheckup() : Void
    {
        clearOutput();
        ...
        else if (flags[kFLAGS.SOPHIE_DAUGHTER_MATURITY_COUNTER] > 0)
        {
            ...
            if (bimboSophie())
            {
                outputText("boisterous bimbo ");
            }
            else
            {
                outputText("matronly harpy ");
            }
            ...
            if (bimboSophie())
            {
                outputText("live-in bimbo ");
            }
            else
            {
                outputText("favorite bird-woman ");
            }
        ...
        if (bimboSophie())
        {
            addButton(4, "Back", approachBimboSophieInCamp);
        }
        else
        {
            addButton(4, "Back", sophieFollowerScene.followerSophieMainScreen);
        }
    }
    ...
    private function bimboSophieLicksRugs() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText(", lifting your heavy sack out of the way");
        }
        ...
        if (player.cor < 33)
        {
            outputText("You nod nervously, blushing hotly at the thought of your bimbo-slut's fat lips kissing and lapping at your slit.");
        }
        else if (player.cor < 66)
        {
            outputText("You nod and smile, blushing a bit in anticipation of feeling your bimbo-slut's fat, pussy-licking lips against your slit.");
        }
        else
        {
            outputText("You confidently smile and scoot your " + hipDescript() + " forward, anticipating the feeling of your bimbo-bitch's fat, pussy-licking lips on your slit.");
        }
        ...
        if (player.clitLength < 3)
        {
            outputText("hums on your clit and doubles the speed of her licking while you watch her finger-fucking her sloppy cunt.");
        }
        else
        {
            outputText("pumps on your clit with her hand and doubles the speed of her lashing tongue while you watch her finger-fucking her sloppy cunt.");
        }
        ...
        if (!player.hasCock())
        {
            outputText("  It's a good thing you don't have a penis!");
        }
        else
        {
            outputText("You groan as your groin burns with desire, filling your [cocks] with blood.  Images of yourself relentlessly fucking your bimbo bride pregnant over and over again fill your mind.  You shudder and blush, trying to resist the artificial needs pounding through your body, but you realize your body is ready.");
        }
        outputText("\n\n");
        ...
        if (player.hasCock())
        {
            outputText(", or thick, juicy dick");
        }
    ...
    private function bimboSophieSixtyNineAfterCunnilingus() : Void
    {
        ...
        function vagF() : Void
        {
            ...
            {
                outputText("Her pregnant belly rests against your middle, untouched by the rough fall.  ");
            }
            ...
            if (player.clitLength >= 3)
            {
                outputText(", even pausing to jack it off like a penis");
            }
            ...
            if (pregnancy.event >= 2)
            {
                outputText(", egg-swollen belly smushing between you");
            }
        ...
        {
            var x : Float = player.cockThatFits(sophieCapacity());
            if (x < 0)
            {
            ...
            if (pregnancy.event >= 2)
            {
                outputText("Her gravid belly bumps up against your own, somehow untouched by the rough-and-tumble fall.  ");
            }
            outputText("How did she do that?  Your questions fade away as you look up and spot your target - the cum-slicked delta of Sophie's cunt, so moist it seems to fog the air, beckoning you to plunge your tongue inside and taste it.\n\n");
            ...
            if (player.cumQ() >= 500)
            {
                ...
                if (player.cumQ() >= 1000)
                {
                    outputText("  After a moment of this, the full-bellied harpy pulls back and lets you paint the remainder over her face, hair, and mountainous breasts.");
                }
            }
            outputText("\n\n");
            ...
            if (pregnancy.event >= 2)
            {
                outputText(" and the way her egg-stuffed womb jiggles");
            }
    ...
    private function buttFuckBimboSophie() : Void
    {
        ...
        {
            outputText("Those cocks look");
        }
        else
        {
            outputText("That cock looks");
        }
        ...
        if (pregnancy.event >= 2)
        {
            outputText("  Just like her breasts, her gravid tummy sways a little as well, plush with the weight of the egg you've given her.");
        }
        ...
        if (player.hasBalls())
        {
            outputText("over your " + ballsDescriptLight());
        }
        else
        {
            outputText("across your taint");
        }
        ...
        if (player.hasBalls())
        {
            outputText("Your balls seize up at Sophie's request, and you swear you can actually hear the fluid churning and sloshing of your cum before it burns a path of warmth through your body.");
        }
        else
        {
            outputText("Your insides seem to seize up at Sophie's request, and you swear you can actually feel the fluid churning a path of warmth through your body.");
        }
        ...
        if (player.cumQ() >= 500)
        {
            ...
            if (player.cumQ() >= 1000)
            {
                outputText(", filling her belly with spooge");
            }
            outputText(".");
        }
    ...
    private function fuckBimboSophiesTits() : Void
    {
        ...
        if (pregnancy.event >= 2)
        {
            outputText("Her swollen tummy pushes against your [legs] as she assumes the position, the weighty egg-distorted lump pressing against you.  ");
        }
        ...
        if (player.cocks[x].cockThickness < 8)
        {
            outputText("completely surround yourself");
        }
        else
        {
            outputText("surround as much of yourself as possible");
        }
        ...
        if (player.hasBalls())
        {
            outputText("all over your balls whenever she needs a break, and you feel your [balls] growing heavier by the moment.");
        }
        else
        {
            outputText("all over your taint and asshole, and you feel your body churning, building up more seed for your moronic mate.");
        }
        ...
        if (pregnancy.event >= 2)
        {
            outputText("her swollen midriff is glazed, ");
        }
    ...
    public function sophieEggApocalypse() : Void
    {
        ...
        menu();
        addButton(0, "Daddy", beBimboSophiesSugarDaddy).disableIf(!player.hasCock(), "Req. a cock!");
        addButton(1, "Mommy", beBimboSophiesSugarMommy).disableIf(!player.hasVagina(), "Req. a vagina!");
    ...
    private function beBimboSophiesSugarDaddy() : Void
    {
        ...
        else if (inventory.hasItemInStorage(consumables.OVIELIX))
        {
            inventory.consumeItemInStorage(consumables.OVIELIX);
        }
    ...
    private function rideDatSophieVag() : Void
    {
        ...
        if (pregnancy.event >= 2)
        {
            outputText("  Her pregnancy-imbued middle doesn't really help, either.");
        }
        ...
        var beNice : Void->Void = function() : Void
        {
            ...
            if (player.hasBalls())
            {
                outputText("your [balls] bouncing and slapping against what parts of Sophie's bubble butt they can reach, ");
            }
    ...
    //{if (cocks = 1) and (cumNormal = true)}
    {
        ...
        if (player.cockTotal() == 1)
        {
            ...
            else
            {
                ...
                if (pregnancy.event >= 2)
                {
                    outputText(" (or at least all over her channel, since her womb is bound up with your egg)");
                }
        ...
        else if (player.cumQ() < 1500)
        {
            ...
            {
                outputText(", along with your third length, spittling and spraying onto her belly.");
            }
            else if (player.cockTotal() > 3)
            {
                outputText(", along with the rest of your cocks, your entire mass of lengths getting your seed all over her.");
            }
            if (player.cockTotal() == 2)
            {
                outputText(".");
        ...
        if (player.hasVagina())
        {
            ...
            {
                outputText("dripping and drooling ");
            }
        ...
        if (pregnancy.event >= 2)
        {
            outputText("resting your head on her gravid tummy and smiling to yourself.");
        }
        else
        {
            outputText("hugging her tight as she tries her best to show her appreciation.");
        }
        if (sophieIsInSeason())
        {
            sophiePregChance();
    ...
    private function sophieImpregnationSex() : Void
    {
        ...
        if (player.cockTotal() == 1)
        {
            outputText("It wobbles");
        }
        else
        {
            outputText("They wobble");
        }
        outputText(", ");
        if (player.cockTotal() > 1)
        {
        ...
        if (player.cockTotal() > 1)
        {
            outputText("one of your lengths");
        }
        else
        {
            outputText("your length");
        }
        ...
        if (player.cockTotal() > 1)
        {
            outputText(", the rest of your lipstick-smeared shafts slipping and sliding between the soft, lube-slicked thighs");
        }
        ...
        if (player.cockTotal() > 1)
        {
            ...
            if (player.cockTotal() > 2)
            {
                outputText("s between her thighs are");
            }
            else
            {
                outputText(" between her thighs is");
            }
        ...
        if (player.hasBalls())
        {
            outputText(", and your [balls] begin to rhythmically pulse in your [sack]");
        }
        else
        {
            outputText(", something inside you begins to clench rhythmically");
        }
        outputText(".  Sophie's pussy stretches around your increasing thickness, dripping its amber honey");
        if (player.cockTotal() > 1)
        {
            outputText(" over your extra, thigh slapping cock");
        }
        if (player.cockTotal() > 2)
        {
            outputText("s");
        ...
        if (player.cockTotal() > 1)
        {
            outputText("  Ropes of creamy spunk drizzle over the ground and the underside of Sophie's tits, but the lust-drunk harpy doesn't seem to mind any more than you do.");
        }
        outputText("  Her hungry twat drinks down the seed with gusto, drawing it out of you even as it milks you for more.");
        if (player.cumQ() >= 500)
        {
            outputText("  You give her exactly what she wants, drowning her pussy with jism.");
        }
        if (player.cumQ() >= 1000)
        {
            outputText("  Alabaster spooge oozes out of her lips after you stuff her womb full, dripping down your [legs] and her own feathery thighs.");
        }
        if (player.cumQ() >= 2000)
        {
            outputText("  The ground quickly becomes a salty quagmire that Sophie's hands are sinking into.");
        }
        if (player.cumQ() >= 5000)
        {
            outputText("  Soon, those droplets become thick gouts, as your immense virility asserts itself.  You hold her there, letting her belly balloon underneath you as you forcefully inseminate her, flooding her with hot seed.");
        }
        ...
        if (y >= 0)
        {
            outputText("  A sinful tightness clenches your " + cockDescript(y) + " at the same time.  You accidently started double penetrating her!  The sloppy vaginal heat around your second member is enough to make you forget that it was even an accident.");
        }
        ...
        if (player.cumQ() >= 1000)
        {
            outputText(" and sperm-oozing pussy");
        }
        outputText(".");
        ...
        if (y >= 0)
        {
            outputText("and squeezing pussy ");
        }
        ...
        {
            outputText("nipping her ear");
        }
        else if (player.tallness >= 60)
        {
            outputText("nipping at her shoulder");
        }
        else
        {
            outputText("nipping at her side");
        }
        ...
        if (y < 0)
        {
            outputText("spraying across your [legs]");
        }
        else
        {
            outputText("running down your " + cockDescript(y) + " in miniature rivers");
        }
        ...
        if (y >= 0)
        {
            ...
            if (player.cumQ() >= 1000)
            {
                outputText(" and her belly even more distended");
            }
            outputText(".  ");
        }
        ...
        if (player.cumQ() >= 500)
        {
            ...
            if (player.cumQ() < 1000)
            {
                outputText("giving her a");
            }
            else
            {
                outputText("adding to her visible");
            }
    ...
    private function giantCocksImpregnateSophie() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText("in those plump balls of yours");
        }
        else
        {
            outputText("inside you");
        }
        ...
        if (player.cumQ() >= 500)
        {
            outputText("  In no time, Sophie's belly begins to form into a bump, and the pressure leaves more and more of your seed to be wasted on the bimbo's thighs.");
        }
        if (player.cumQ() >= 1000)
        {
            outputText("  Your intense ejaculations quickly balloon that petite bump into a rounded dome.  Torrents of spunk run out of her around your tip, fountaining slightly with each fresh dose.  You simply can't pack any more cum into her, and the excess puddles around her plush bottom.");
        }
    ...
    private function yesSophieSleepWith() : Void
    {
        ...
        var _sw30_ = (pregnancy.event);        

        switch (_sw30_)
        {
            case 1:outputText("burgeoning");
            case 2:outputText("swollen");
            case 3, 4:outputText("huge, gravid");
    ...
    private function moveSophieInRepeat() : Void
    {
        ...
        if (!pregnancy.isPregnant)
        {
            outputText(" even though she's not pregnant anymore");
        }
    ...
    public function sophieFenCraftedSex(morning : Bool = false) : Void
    {
        var x : Int = player.cockThatFits(sophieCapacity());
        if (x < 0)
        {
        ...
        if (player.hasBalls())
        {
            outputText("  Sophie's immense backside slides over your [balls] each time she bottoms out, her tail feathers tickling at the bottom of your [sack].");
        }
        ...
        if (pregnancy.isPregnant)
        {
            ...
            if (player.cumQ() >= 500)
            {
                outputText("  You cum so hard that torrents of spooge wash out of her slit, her womb obstructed by the egg you made with her.");
            }
        }
        if (player.cumQ() >= 2000)
        {
            outputText("  The oozing spunk soaks you thoroughly, and Sophie's thighs become matted with the gallons of gunk that squirts out of her lips.");
        }
    ...
    //Alternate Intro/Outro for consensual fucking when it isn't morning?
    public function fuckYoPregnantHarpyWaifu(morning : Bool = false) : Void
    {
        var x : Int = player.cockThatFits(sophieCapacity());
        //{Morning Fuck}
        if (morning)
        ...
        else
        {
            ...
            if (player.lust > 50)
            {
                outputText("feeling its firmness.  \"<i>Wow, you're so hard already, [name]!</i>\" she comments.");
            }
            else
            {
                outputText("slowly stroking it until it begins to pulse to life, thickening in her hand.");
            }
        ...
        if (player.hasBalls())
        {
            outputText("[balls], where they hang in thick drops before falling to the ground,");
        }
        else
        {
            outputText("taint and [legs]");
        }
        ...
        if (player.cumQ() >= 500)
        {
            ...
            if (player.cumQ() < 1000)
            {
                outputText("soak your blankets");
            }
            else if (player.cumQ() < 2000)
            {
                outputText("soak your blankets and the ground below");
            }
            else if (player.cumQ() < 5000)
            {
                outputText("soak your blankets and form a giant puddle below, leaving you feeling like you're on a waterbed made of spunk.");
            }
            else
            {
                outputText("soak your blankets and create a lake the two of you sink partway into, surrounded by muddy, cum-flavored mud.");
            }
        }
        outputText("\n\nYou sag back as the last few drops leak out around you.  Sophie rolls off to the side, panting heavily.  She titters, \"<i>");
        if (morning)
        {
            outputText("What... a... way to... start a... morning!");
        }
        else
        {
            outputText("Ooohh... you're... gonna be... a great... daddy...");
        }
        ...
        if (player.cumQ() >= 1000)
        {
            outputText(" in the hardening mud");
        }
        outputText(", swaying her hips sensually as seed runs from her cunt in thick little rivers.  You're wet, sticky, and sated");
        if (morning)
        {
            outputText(" when you go wash up to get dressed.  It's going to be a good day.");
        }
        else
        {
            outputText(" when you go to get dressed.  You'll have to make sure to wash clean at some point, but for now you enjoy the sweet scent of your pet bimbo's cum.");
        }
        player.sexReward("vaginalFluids", "Dick");
        dynStats("sen", -2);
        if (morning)
    ...
    private function sophiePreggoTitJobs() : Void
    {
        ...
        {
            outputText("the underside of your immense erection");
        }
        else
        {
            outputText("your " + player.cockHead(x) + "");
        }
        ...
        if (player.cocks[x].cockLength > 30)
        {
            outputText("presses against");
        }
        else
        {
            outputText("slips into");
        }
        ...
        if (player.cocks[x].cockLength > 30)
        {
            outputText("  Sophie pulls back long enough to gasp, \"<i>You're so long, I can't even take the tip!</i>\"");
        }
    ...
    private function goHogWildOnSophieBewbs() : Void
    {
        ...
        if (player.cocks[x].cockLength <= 30)
        {
            outputText(player.cockHead(x) + " pushes into through her startled cock-pillows, muffling her more effectively than any gag.  She immediately sucks on it, a happy look in her eyes.  The bimbo seems to want nothing more than to lie there and polish your rod.");
        }
        else
        {
            outputText(" immense rod smacks across her face, too long to try to jam down her throat.  It completely covers her face, smothering her features with rigid, veiny dickflesh.  She immediately begins to lavish the underside with licks and long wet kisses, aided by her plus-sized bimbo lips.  When her eyes peek out from under your " + cockDescript(x) + ", they seem placid and happy.  It seems the slut wants nothing more than to lie there and service your cock.");
        }
        ...
        if (player.cocks[x].cockLength <= 30)
        {
            outputText(" right into Sophie's mouth");
        }
        else
        {
            outputText(" above Sophie's head");
        }
        ...
        if (player.hasBalls())
        {
            outputText(", cupping your [balls] with a hand and massaging them to aid your ejaculation");
        }
        ...
        if (player.hasKnot(x))
        {
            outputText("  Your knot inflates to a ludicrous degree, squeezed down on from all sides by slick, gold-painted tits, happy to be engulfed by warm flesh.");
        }
        ...
        if (player.cumQ() > 1000)
        {
            outputText("  Again and again, you continue to paint her white.  Your orgasm seems almost without end as you ejaculate.  Sophie's face is soaked with it, and her breasts have become glazed mountain-peaks.");
        }
        if (player.cumQ() > 2500)
        {
            outputText("  By the time you finish, the stuff is puddled under her.  The best your harpy pet can manage is a spermy smile as she languishes in your seed.");
        }
    ...
    private function letSophieMilkYoDick() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText("Your [balls] drag across her pregnant belly as she goes to work, but you don't mind in the slightest.  There's something erotic about having them bumping up against the fruit of your union.  ");
        }
        ...
        if (player.cocks[x].cockLength <= 30)
        {
            outputText(", ready to catch the cum");
        }
        else
        {
            outputText(", assisting the cum");
        }
        ...
        if (player.cocks[x].cockLength <= 30)
        {
            ...
            if (player.cumQ() < 250)
            {
                outputText(" swallows it without complaint, her eyes twinkling mirthfully as she suckles down your small load.  Surprisingly, you dump a few more pulses into her, each weaker than the last.  The bird-woman drinks it with ease, never looking away from you.");
            }
            else if (player.cumQ() < 500)
            {
                outputText("'s cheeks bulge from the size of your load, but she manages to get it down with a big gulp.  Surprisingly, you dump a few more jets of thick cream into her waiting gullet, each weaker than the last but big enough to make it hard for her to swallow.  She never looks away from you as she takes it.");
            }
            else if (player.cumQ() < 1000)
            {
                outputText("'s cheeks bulge cartoonishly at your immense deposit, cream escaping from the corners of her mouth as her poor mouth floods.  Her throat bobs ludicrously, trying to keep up with the flow, but when you follow up that virile blast with several smaller, still potent deposits, she gives up and lets the jizz run down her lip and face, dripping onto her tits.  Through it all, her eyes never leaves yours.");
            }
            else
            {
                ...
                if (silly())
                {
                    outputText("repel cum of that magnitude");
                }
                else
                {
                    outputText("handle that much cum");
                }
            ...
            if (player.cumQ() < 1000)
            {
                outputText("with a happy smile");
            }
            else
            {
                outputText("coughing and sputtering");
            }
        ...
        else
        {
            ...
            if (player.cumQ() < 250)
            {
                outputText(" giggles happily when the cum jets into the air and down atop her head, moaning against your dick while it seeps through her platinum feathers.  Throughout it all, she licks and sucks on your shaft, milking out every drop and never looking away.");
            }
            else if (player.cumQ() < 800)
            {
                outputText(" moans when your heavy deposits splatter into her hair, dripping through the downy blond feathers in a messy web.  Throughout it all, she sucks and licks at you, polishing your " + cockDescript(x) + " until she's sure every single drop of jizz has been expelled onto her curvy frame.");
            }
            else
            {
                ...
                if (player.cumQ() >= 2000)
                {
                    ...
                    if (player.cumQ() >= 3000)
                    {
                        ...
                        {
                            outputText("big puddle");
                        }
                        else if (player.cumQ() < 10000)
                        {
                            outputText("small lake");
                        }
                        else
                        {
                            outputText("obscene lake");
                        }
    ...
    public function sophieBirthsEgg() : Void
    {
        ...
        if (bimboSophie())
        {
            outputText("Come here, [name]!  I... whooooaaaa... I wanna like, hold your hand when the egg comes out!");
        }
        else
        {
            outputText("Come here, [name]!  I... whoooaaaa...  I need something to hold on to while this comes out!");
        }
        outputText("</i>\"");
        
        if (player.cor < 33)
        {
            outputText("  You sit down next to her, and figuring this must be normal for her, you hold her hand with one arm and grope at her free breast with the other.");
        }
        else if (player.cor < 66)
        {
            outputText("  You sit down next to her with a sigh.  Well, at least you can grope her tit with your other hand!");
        }
        else
        {
            outputText("  You plop down next to her, but ignore her proffered hand for one of those juicy melons on her chest.  Grinning, you can't help but think that this is a far more tender thing to hold anyway.");
        }
        ...
        {
            outputText("  Can you handle having another harpy living in your camp?\n");
        }
        else
        {
            outputText("  Just how many harpies can your camp hold?\n");
        }
    ...
    public function sophiesEggHatches() : Void
    {
        ...
        //First Harpy:{
            ...
            if (flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] == 0)
            {
                ...
                if (flags[kFLAGS.SOPHIE_BIMBO_ACCEPTED] > 0)
                {
                    outputText(", though her lips are a little larger than you'd think was normal - maybe she inherited it from her mom?");
                }
                else
                {
                    outputText(".");
            ...
            {
                //Second Harpy:  //-Badonkadonk to 9000.  Blond feathers that grow very long. (violet eyes)  if (flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] == 1)
                {
                ...
                {
                    //-Not that curvy, but has human feet and purple feathers. (violet eyes)if (flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] == 2)
                    {
                        ...
                        if (flags[kFLAGS.SOPHIE_BIMBO_ACCEPTED] > 0)
                        {
                            flags[kFLAGS.DAUGHTER_THREE_BIMBO] = 1;
                        }
                    }
                    //Fourth Harpy:
                    else
                    {
                        //-Curvy like Sophie, but with well-developed wings and extra-soft feathers (WINGJOBS).  Pink plumage.  (blue eyes){
                            ...
                            if (flags[kFLAGS.SOPHIE_BIMBO_ACCEPTED] > 0)
                            {
                                outputText(" like her mother used to have before her bimbofication");
                            }
                            ...
                            if (flags[kFLAGS.SOPHIE_BIMBO_ACCEPTED] > 0)
                            {
                                flags[kFLAGS.DAUGHTER_FOUR_BIMBO] = 1;
                            }
                        }
                    }
                }
            ...
            if (bimboSophie())
            {
                outputText("  Before the girl can try to stand she is swept up off her feet by her ecstatic mother.  \"<i>Oohhhhh, just look at you!  My baby girl is sooo cute!  Soooo totally fluffy and like, suuuuper soft!</i>\"");
            }
            else
            {
                outputText("  Before the girl can try to stand she is swept up off her feet by her ecstatic mother.  \"<i>Oohhhhh, you're so cute!  My baby girl is sooo cute!  Oh, the boys will be all over you when you grow up, I just know it!</i>\"");
            }
        }
        ...
        else
        {
            
            {
                ...
                else if (flags[kFLAGS.SOPHIE_BIMBO_ACCEPTED] > 0)
                {
                    outputText("blonde");
                }
                else
                ...
                if (bimboSophie())
                {
                    outputText("\"<i>Oohhhhh, just look at you!  My baby girl is sooo cute!  Soooo totally fluffy and like, suuuuper soft!</i>\"");
                }
                else
                {
                    outputText("\"<i>Oohhhhh, you're so cute!  My baby girl is sooo cute! Oh, the boys will be all over you when you grow up, I just know it!</i>\"");
                }
            }
        }
        ...
        if (bimboSophie())
        {
            outputText("\"<i>Oh dearie, there there, like, calm down, sweetie.  Don't worry, Momma Sophie's here.</i>\"  The matronly harpy gently hugs the squirming fluff-ball into her big, soft breasts, her hands gently stroking her fussy daughter's feathery hair.");
        }
        else
        {
            outputText("\"<i>There there, calm down, baby.  Momma Sophie's here...  I'm here.</i>\"  The matronly harpy gently hugs the squirming fluff-ball into her big, soft breasts, her hands gently stroking her fussy daughter's feathery hair.");
        }
    ...
    public function sophieKidMaturation() : Void
    {
        ...
        //First Harpy:{
            ...
            //-Pink plumage, big tits and (sensitive) lips, normal harpy hips. (blue eyes)
            if (flags[kFLAGS.DAUGHTER_ONE_BIMBO] == 1)
            {
        ...
        //Second Harpy:{
            ...
            if (flags[kFLAGS.DAUGHTER_TWO_BIMBO] == 1)
            {
        ...
        //Third Harpy:{
            ...
            if (flags[kFLAGS.DAUGHTER_THREE_BIMBO] == 1)
            {
        ...
        //Fourth Harpy:{
            ...
            //-Curvy like Sophie, but with well-developed wings and extra-soft feathers (WINGJOBS).  Pink plumage.  (blue eyes)
            if (flags[kFLAGS.DAUGHTER_FOUR_BIMBO] == 1)
            {
        ...
        {
            outputText("bombshell");
        }
        outputText(", just like her mother.");
        if (bimboSophie())
        {
            outputText("  From the inquisitive look on her face and spark of intelligence in her eyes, it seems that she hasn't lost her mind to her inner bimbo.");
        }
    ...
    private function yesToSophieIncest() : Void
    {
        ...
        if (bimboSophie())
        {
            outputText("personal bimbo ");
        }
        else
        {
            outputText("lusty matron ");
        }
    ...
    private function watchIzmaSophie() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(", from me OR my Alpha");
        }
        ...
        if (player.hasCock())
        {
            outputText("stroke yourself through bliss");
        }
        else
        {
            outputText("finger yourself through bliss");
        }
        ...
        if (player.hasCock())
        {
            if (player.cumQ() >= 1000)
            {
                outputText(", leaving you in a puddle of your own slimy mess");
            }
        }
