...
class MagnoliaFollower extends NPCAwareContent
{
    ...
    public function treeMenu() : Void
    {
        ...
        else if (flags[kFLAGS.CHRISTMAS_TREE_LEVEL] == 3)
        {
            ...
            menu();
            if (player.fatigue + 50 < player.maxOverFatigue())
            {
                addButton(1, "Water It", waterIt);
            }
            if (player.HP > 150 && player.soulforce >= 50 && player.hasPerk(PerkLib.SoulApprentice))
            {
                addButton(2, "Use SFIB", useSouforceInfusedBlood);
            }
        ...
        else if (flags[kFLAGS.CHRISTMAS_TREE_LEVEL] >= 6 && flags[kFLAGS.CHRISTMAS_TREE_LEVEL] < 9)
        {
            if (flags[kFLAGS.CHRISTMAS_TREE_LEVEL] == 7 && player.hasKeyItem("Decorations") >= 0)
            {
                ...
                {
                    outputText(" Some of the camp residents come over to see what you're doing.");
                }
                if (player.hasStatusEffect(StatusEffects.PureCampJojo) && flags[kFLAGS.JOJO_BIMBO_STATE] != 3)
                {
                    outputText("\"<i>Oh? Are you decorating the tree? I would like to assist too if you don't mind.</i>\" Says Jojo.\n\n");
                }
                if (amilyScene.amilyFollower() && !amilyScene.amilyCorrupt())
                {
                    outputText("\"<i>[name] there's no way you're decorating that alone. Let me lend a hand.</i>\" Comments Amily.\n\n");
                }
                if (flags[kFLAGS.ETNA_FOLLOWER] > 0 && !player.hasStatusEffect(StatusEffects.EtnaOff))
                {
                    outputText("\"<i>Oooo, that looks like fun! Maybe I can fly up and decorate the harder to reach areas.</i>\" Etna suggests.\n\n");
                }
                if (DivaScene.instance.isCompanion())
                {
                    outputText("\"<i>Tis the season already? I suppose I could lend my services.</i>\" Diva speaks with overdramatic voice.\n\n");
                }
                if (flags[kFLAGS.IZMA_FOLLOWER_STATUS] == 1 && flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 0)
                {
                    outputText("\"<i>You're decorating the tree? Can I help too?</i>\" Asks Izma.\n\n");
                }
                if (flags[kFLAGS.LUNA_FOLLOWER] >= 4 && !player.hasStatusEffect(StatusEffects.LunaOff))
                {
                    outputText("\"<i>Oh, is it time to decorate the tree? I will gladly lend my hand if you need it " + player.mf("Master", "Mistress") + ".</i>\" Asks Luna.\n\n");
                }
                if (emberScene.followerEmber())
                {
                    outputText("\"<i>I still don't see the point in putting those on a tree…</i>\" Ember questions.\n\n");
                }
                if (player.hasStatusEffect(StatusEffects.CampMarble) && flags[kFLAGS.FOLLOWER_AT_FARM_MARBLE] == 0)
                {
                    outputText("\"<i>I can't remember the last time I did something like this. Let me help too!</i>\" Marble says.\n\n");
                }
                if (arianScene.arianFollower())
                {
                    outputText("\"<i>What's this [name]? You're putting ornaments on the tree?</i>\" Arian asks, puzzled.\n\n");
                }
                if (followerKiha())
                {
                    outputText("\"<i>Geez can you go any slower [name]? Bet I can get this done faster than you can blink!</i>\" Kiha boasts.\n\n");
                }
                if (flags[kFLAGS.CHI_CHI_FOLLOWER] > 2 && flags[kFLAGS.CHI_CHI_FOLLOWER] != 5 && !player.hasStatusEffect(StatusEffects.ChiChiOff))
                {
                    outputText("\"<i>So this is what you've been up to. Well, I can't deny that this looks interesting. Let me help too.</i>\" Chi Chi offers.\n\n");
                }
                if (flags[kFLAGS.SAMIRAH_FOLLOWER] > 9)
                {
                    outputText("\"<i>Oh? I've never since something like this before. Is it normal to put decorations on a tree like this?</i>\" Samirah asks while looking at the tree.\n\n");
                }
    ...
    public function plantGrowsToLevel4() : Void
    {
        ...
        if (camp.companionsCount() > 0)
        {
            ...
            {
                outputText("\"<i>Gods [name]... Did you do this? I'm surprised something can even grow here. Especially so fast and so large.</i>\"\n\n");
            }
    ...
    private function waterIt() : Void
    {
        ...
        fatigue(50);
        if (flags[kFLAGS.CHRISTMAS_TREE_GROWTH_COUNTER] < 1000)
        {
            flags[kFLAGS.CHRISTMAS_TREE_GROWTH_COUNTER]++;
        }
    ...
    private function useSouforceInfusedBlood() : Void
    {
        ...
        if (flags[kFLAGS.CHRISTMAS_TREE_GROWTH_COUNTER] < 1000)
        {
            flags[kFLAGS.CHRISTMAS_TREE_GROWTH_COUNTER] += 4;
        }
        doNext(camp.returnToCampUseOneHour);
    }
}
