...
class Rathazul extends NPCAwareContent implements TimeAwareInterface
{
    private var philters(get, never) : Bool;
    private var reductos(get, never) : Bool;
    private var dyes(get, never) : Bool;
    ...
    public function timeChange() : Bool
    {
        if (flags[kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN] > 1)
        {
            ...
            if (flags[kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN] < 1)
            {
                flags[kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN] = 1;
            }
            if (flags[kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN] > 300)
            {
        ...
        if (flags[kFLAGS.RATHAZUL_CAMP_INTERACTION_COUNTDOWN] > 0)
        {
            ...
            if (flags[kFLAGS.RATHAZUL_CAMP_INTERACTION_COUNTDOWN] < 0)
            {
                flags[kFLAGS.RATHAZUL_CAMP_INTERACTION_COUNTDOWN] = 0;
            }
        }
        return false;
    }
    ...
    public function returnToRathazulMenu() : Void
    {
        ...
        {
            encounterRathazul(false);
        }
    }
    
    public function encounterRathazul(meet : Bool = true) : Void
    {
    ...
    private function get_philters() : Bool
    {
        return player.gems >= 100;
    }
    private function get_reductos() : Bool
    {
    ...
    private function get_dyes() : Bool
    {
        return player.gems >= 50;
    }
    private function rathazulWorkOffer() : Bool
    {
        spriteSelect(SpriteDb.s_rathazul);
        var totalOffers : Int = 0;
        var spoken : Bool = false;
        var debimbo : Bool = false;
        var mindup : Bool = false;
        ...
        if (dyes)
        {
            ...
            {
                outputText("I should be able to make exotic-colored dyes if you're interested.  ");
            }
        ...
        if (reductos)
        {
            ...
            if (flags[kFLAGS.AMILY_MET_RATHAZUL] >= 2)
            {
                outputText("50");
            }
            else
        ...
        if (player.hasStatusEffect(StatusEffects.CampRathazul))
        {
            ...
            else if (flags[kFLAGS.RATHAZUL_DEBIMBO_OFFERED] > 0)
            {
                ...
                else if (!player.hasItem(consumables.SMART_T, 5))
                {
                    outputText("  You should probably find some if you want that...");
                }
                else
                {
                    outputText("  You need more gems to afford that, though.");
                }
                outputText("\n\n");
            }
            ...
            if (flags[kFLAGS.SAMIRAH_FOLLOWER] == 6)
            {
                rathazulReptaTonguePotionOffer();
            }
            if (flags[kFLAGS.ELECTRA_FOLLOWER] == 2 && !player.hasStatusEffect(StatusEffects.ElectraOff) && player.hasItem(useables.RPLASMA, 1))
            {
                rathazulAlchemicalThunderOffer();
            }
            ...
            else if (flags[kFLAGS.RATHAZUL_MINDUP_OFFERED] > 0)
            {
                ...
                else if (!player.hasItem(consumables.INCOINS, 5))
                {
                    outputText("  You should probably find some if you want that...");
                }
                else
        ...
        if (totalOffers > 0)
        {
            ...
            {
                if (dyes && player.hasItem(consumables.REPTLUM, 1))
                {
                    addButton(3, "Make Dye", makeDyes).hint("Ask him to make a special dye for you. (Only dyes here are for Arian) \n\nCost: 50 Gems.");
                }
                else
                {
                    addButtonDisabled(3, "Make Dye", "You need 50+ gems and 1 vial of Reptilium.");
                }
            }
            if (player.hasItem(consumables.BEEHONY))
            {
                addButton(4, consumables.PURHONY.shortName, rathazulMakesPureHoney).hint("Ask him to distill a vial of bee honey into a pure honey. \n\nCost: 25 Gems \nNeeds 1 vial of Bee Honey");
            }
            if (debimbo)
            {
                addButton(5, "Debimbo", makeADeBimboDraft).hint("Ask Rathazul to make a debimbofying potion for you. \n\nCost: 250 Gems \nNeeds 5 Scholar Teas.");
            }
            if (mindup)
            {
                addButton(6, "MindUp", makeAMindUpDraft).hint("Ask Rathazul to make a sanity restoring potion for you. \n\nCost: 250 Gems \nNeeds 5 incenses of Insight.");
            }
            if (flags[kFLAGS.ELECTRA_FOLLOWER] > 2 && !player.hasStatusEffect(StatusEffects.ElectraOff))
            {
                if (player.hasItem(useables.RPLASMA, 2) && player.hasItem(consumables.L_DRAFT, 1))
                {
                    addButton(7, "Alch.Thun.", makeAlchemicalThunder).hint("Ask him to help Mitzi. \n\nNeeds two raiju plasmas and one lust draft");
                }
                else
                {
                    addButtonDisabled(7, "Alch.Thun.", "Need to gather two raiju plasmas and one lust draft for this.");
                }
            }
            /*
		 ...
		 */
            ...
            {
                addButton(11, "Lethicite", lethiciteDefense).hint("Ask him if he can make use of that lethicite you've obtained from Marae.");
            }
            if (silly() && player.hasStatusEffect(StatusEffects.CampRathazul))
            {
                addButton(12, "Flirt", getThatRatAss).hint("Try to score with Rathazul.");
            }
    ...
    private function askForTools() : Void
    {
        ...
        else if (player.inte >= 25)
        {
            ...
            menu();
            addButton(0, "Buy", curry(getAlembic, cost)).hint("Buy alembic for " + cost + " gems.").disableIf(player.gems < cost, "You need " + cost + " gems");
            addButton(1, "Cancel", rathazulWorkOffer);
        }
        else
        {
            outputText("<i>(todo writeme: int < 25, can't buy)</i> ");
            if (player.inte < 16 && silly())
            {
                outputText("You have been fascinated with alchemy ever since you saw mixing blue and yellow paint into green one. But the village alchemists never allowed you to perform such sciences. Envious of your talent, they kept making excuses about <i>'keeping braindead brutes away from fragile equipment'</i>. You enthusiastically tell this story to Rathazul, expecting understanding from a fellow alchemist. ");
            }
    ...
    private function getAlembic(price : Int) : Void
    {
        ...
        {
            outputText("For a full set of alchemical tools, you need a furnace.");
        }
    ...
    private function oneTimeOptions() : Void
    {
        ...
        {
            if (player.hasItem(consumables.SHARK_T) && player.hasItem(consumables.PPHILTR) && player.hasItem(consumables.VITAL_T))
            {
                addButton(0, "C.CurePotion", RathazulMakesToothCursePotion).hint("Ask him to make cure curse potion. \n\nNeeds shark tooth, purity philter and vitality tincture");
            }
            else
            {
                addButtonDisabled(0, "C.CurePotion", "Ask him to make curse cure potion. \n\nNeeds shark tooth, purity philter and vitality tincture");
            }
        }
        if (TyrantiaFollower.TyrantiaFollowerStage == 5)
        {
            addButton(1, "Tyrantia", TyrantiaEggQuestRathazul);
        }
        if (flags[kFLAGS.SAMIRAH_FOLLOWER] == 7)
        {
            if (player.hasItem(consumables.HUMMUS_, 1) && player.hasItem(consumables.REPTLUM, 1) && player.hasItem(consumables.OVIELIX, 1))
            {
                addButton(5, "ReptaTongue P", makeReptaTonguePotion).hint("Ask him to make Repta-Tongue Potion. \n\nNeeds 1 Hummus, 1 Reptilium and 1 Ovi Elixir");
            }
            else
            {
                addButtonDisabled(5, "ReptaTongue P", "Need to gather 1 Hummus, 1 Reptilium and 1 Ovi Elixir for this potion.");
            }
        }
        ...
        {
            if (player.hasItem(consumables.SMART_T, 5) && player.hasItem(consumables.VITAL_T, 5) && player.hasItem(consumables.S_WATER, 1) && player.hasItem(consumables.PURHONY, 1))
            {
                addButton(7, "Mitzi", cureMitzi).hint("Ask him to help Mitzi. \n\nNeeds five scholar teas, five vitality tinctures, one bottle of pure spring water, and one vial of pure honey");
            }
            else
            {
                addButtonDisabled(7, "Mitzi", "Need to gather five scholar teas, five vitality tinctures, one bottle of pure spring water, and one vial of pure honey for this.");
            }
        }
    ...
    private function rathazulShopMenu() : Void
    {
        ...
        if (player.gems >= 12)
        {
            addButton(7, consumables.H_PILL.shortName, buyPuritySomething1, consumables.H_PILL);
        }
        else
        {
            addButtonDisabled(7, consumables.H_PILL.shortName, "You can't afford to buy this.\n\n12 gems required.");
        }
        if (player.level >= 24)
        {
            if (player.gems >= 30)
            {
                addButton(8, consumables.MH_PILL.shortName, buyPuritySomething2, consumables.MH_PILL);
            }
            else
            {
                addButtonDisabled(8, consumables.MH_PILL.shortName, "You can't afford to buy this.\n\n30 gems required.");
            }
        }
        else
        {
            addButtonDisabled(8, "???", "Required to be lvl 24+ for this.");
        }
        if (player.level >= 42)
        {
            if (player.gems >= 85)
            {
                addButton(9, consumables.BH_PILL.shortName, buyPuritySomething3, consumables.BH_PILL);
            }
            else
            {
                addButtonDisabled(9, consumables.BH_PILL.shortName, "You can't afford to buy this.\n\n85 gems required.");
            }
        }
        else
        {
            addButtonDisabled(9, "???", "Required to be lvl 42+ for this.");
        }
        if (player.statusEffectv2(StatusEffects.MetRathazul) >= 5)
        {
            addButton(10, "ProLactaid", rathazulMakesMilkPotion).hint("Ask him to brew a special lactation potion. \n\nCost: 250 Gems \nNeeds 5 Lactaids and 2 Purified LaBovas.");
        }
        if (player.statusEffectv2(StatusEffects.MetRathazul) >= 5)
        {
            addButton(11, "Scorpinum", rathazulMakesScorpioPotion).hint("Ask him to brew a special potion that could aid in gaining scorpion tail. \n\nCost: 100 Gems \nNeeds 2 vials of Bee Honey and 2 vials of Snake Oil.");
        }
    ...
    private function makeDyes() : Void
    {
        ...
        {
            if (player.hasItem(consumables.WHITEDY, 1))
            {
                addButton(0, "White", makeDye1);
            }
            else
            {
                addButtonDisabled(0, "White", "You not have white dye.");
            }
            if (player.hasItem(consumables.BLUEDYE, 1))
            {
                addButton(1, "Blue", makeDye2);
            }
            else
            {
                addButtonDisabled(1, "Blue", "You not have blue dye.");
            }
            if (player.hasItem(consumables.PINKDYE, 1))
            {
                addButton(2, "Pink", makeDye3);
            }
            else
            {
                addButtonDisabled(2, "Pink", "You not have pink dye.");
            }
            if (player.hasItem(consumables.RAINDYE, 1))
            {
                addButton(3, "Rainbow", makeDye4);
            }
            else
            {
                addButtonDisabled(3, "Rainbow", "You not have rainbow dye.");
            }
        }
    ...
    private function rathazulAprilFoolPart2() : Void
    {
        ...
        {
            player.createStatusEffect(StatusEffects.RathazulAprilFool, date.fullYear, 0, 0, 0);
        }
        doNext(returnToRathazulMenu);
    }
