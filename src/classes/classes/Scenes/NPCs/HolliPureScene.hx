...
class HolliPureScene extends NPCAwareContent
{
    ...
    public function treeMenu() : Void
    {
        ...
        else if (flags[kFLAGS.FLOWER_LEVEL] == 4)
        {
            ...
            {
                outputText("How may I help you?  Tell me your wish so that I can try fulfill it.");
            }
            outputText("</i>\"");
            ...
            {
                addButton(0, "Fuck Holli", holliGetsDickDommed);
            }
            if (player.tentacleCocks() >= 10 && player.lust >= 33)
            {
                addButton(1, "TentacleBone", fullOnTentacleTasticGangBangForHolli);
            }
            ...
            {
                addButton(2, "Ride Holli", vaginalDomHollisTentacruels);
            }
            if (player.isAlraune())
            {
                addButton(3, "TentacleDuel", SceneLib.holliScene.alrauneExtraSceneWithHollicyntiaTentacleDuel);
            }
            else
            {
                addButtonDisabled(2, "TentacleDuel", "This scene requires to be an alraune of some kind.");
            }
            ...
            {
                addButtonDisabled(6, "Eat A Fruit");
            }
            if (flags[kFLAGS.DAILY_MARA_FRUIT_COUNTER] < 1)
            {
                addButton(7, "Mara Fruit", getMaraFruit);
            }
            else
            {
                addButtonDisabled(7, "Mara Fruit");
            }
    ...
    public function plantGrowsToLevel2() : Void
    {
        ...
        if (player.gender == 0)
        {
            outputText("  Sucks to be it!  You don't waste time with stupid shit.");
        }
        outputText("  Is this really Marae's gift?");
        if (player.gender > 0)
        {
            outputText("  Well, you could make use of this build up arousal on someone... just being around it seems to slowly turn you on.");
        }
    ...
    private function waterIt() : Void
    {
        ...
        if (flags[kFLAGS.FLOWER_LEVEL] == 3)
        {
            outputText("tree");
        }
        else
    ...
    public function flowerGrowsToP3() : Void
    {
        ...
        {
            outputText("  The gardening and using souldforce infused blood probably only helped it to grow faster.");
        }
    ...
    public function treePhaseFourGo() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_FUCKED_FLOWER] + flags[kFLAGS.TIMES_RIDDEN_FLOWER] > 0)
        {
            outputText("Did you enjoy watering me?");
        }
        else
        {
            outputText("Did you enjoy watching me grow?");
        }
        outputText("</i>\"");
        ...
        if (flags[kFLAGS.TIMES_FUCKED_FLOWER] + flags[kFLAGS.TIMES_RIDDEN_FLOWER] > 0)
        {
            outputText("  \"<i>It was so much easier to grow with this nutrients");
        }
        else
        {
            outputText("  \"<i>You should have watered me.  It was so much harder to grow without proper nutrients");
        }
    ...
    private function askHolliToGuard() : Void
    {
        ...
        if (flags[kFLAGS.HOLLI_DEFENSE_ON] == 1)
        {
            addButton(1, "Don't Guard", toggleHolliGuard);
        }
        else
        {
            addButton(0, "Guard", toggleHolliGuard);
        }
