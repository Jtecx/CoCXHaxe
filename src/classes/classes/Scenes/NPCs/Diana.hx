...
class Diana extends Monster
{
    private function spellCostMultiplier() : Float
    {
        var multiplier : Float = 1;
        if (flags[kFLAGS.DIANA_SPELLS_CASTED] >= 10)
        {
            multiplier -= 0.1;
        }
        if (flags[kFLAGS.DIANA_SPELLS_CASTED] >= 30)
        {
            multiplier -= 0.1;
        }
        if (flags[kFLAGS.DIANA_SPELLS_CASTED] >= 70)
        {
            multiplier -= 0.1;
        }
        if (flags[kFLAGS.DIANA_SPELLS_CASTED] >= 150)
        {
            multiplier -= 0.1;
        }
        if (flags[kFLAGS.DIANA_SPELLS_CASTED] >= 310)
        {
            multiplier -= 0.1;
        }
        return multiplier;
    }
    public function spellCostMagicBolt() : Float
    {
        var cost : Float = 40;
        cost *= spellCostMultiplier();
        return cost;
    }
    public function spellCostMagicBoltsBarrage() : Float
    {
        var cost : Float = 200;
        cost *= spellCostMultiplier();
        return cost;
    }
    public function spellCostHeal() : Float
    {
        var cost : Float = 30;
        cost *= spellCostMultiplier();
        if (hasPerk(PerkLib.NaturalHealingMinor))
        {
            cost -= 3;
        }
        if (hasPerk(PerkLib.NaturalHealingMajor))
        {
            cost -= 4.5;
        }
        if (hasPerk(PerkLib.NaturalHealingEpic))
        {
            cost -= 5;
        }
        if (hasPerk(PerkLib.NaturalHealingLegendary))
        {
            cost -= 6.5;
        }
        if (hasPerk(PerkLib.WisenedHealer))
        {
            cost *= 2;
        }
        return cost;
    }
    public function spellCostRegenerate() : Float
    {
        var cost : Float = 50;
        cost *= spellCostMultiplier();
        if (hasPerk(PerkLib.NaturalHealingMinor))
    ...
    public function soulskillCostManyBirds() : Float
    {
        var cost : Float = 10;
    ...
    public function soulskillCostHailOfBlades1() : Float
    {
        var cost : Float = 50;
        if (hasPerk(PerkLib.DaoistApprenticeStage))
        {
    ...
    public function HealMod() : Float
    {
        var mod1 : Float = 1;
        if (hasPerk(PerkLib.SpellpowerHealing))
        {
            mod1 += .2;
        }
        if (hasPerk(PerkLib.NaturalHealingMinor))
    ...
    public function SpellMod() : Float
    {
        var mod2 : Float = 1;
        if (hasPerk(PerkLib.Spellpower))
        {
            mod2 += .1;
        }
        if (hasPerk(PerkLib.Channeling))
        {
            mod2 += .2;
        }
    ...
    public function SoulskillMod() : Float
    {
        var mod3 : Float = 1;
        if (hasPerk(PerkLib.DaoistApprenticeStage))
        {
    ...
    public function usingARC() : Void
    {
        ...
        else if (flags[kFLAGS.DIANA_LVL_UP] >= 12)
        {
            mana += 1440;
        }
    ...
    public function usingManyBirdsSoulskill() : Void
    {
        ...
        var damage : Float = wisdomscalingbonus();
        if (damage < 10)
        {
            damage = 10;
        }
    ...
    private function BladesD() : Void
    {
        ...
        if (this.wis <= 200)
        {
            critChance += as3hx.Compat.parseInt(this.wis / 10);
        }
        if (this.wis > 200)
        {
            critChance += 20;
    ...
    public function usingMagicBoltsBarrageSpell() : Void
    {
        ...
        var damage : Float = inteligencescalingbonus() * SpellMod() * 1.2;
        if (damage < 10)
        {
            damage = 10;
        }
        if (weaponAttack < 51)
        {
            damage *= (1 + (weaponAttack * 0.04));
        }
        else if (weaponAttack >= 51 && weaponAttack < 101)
        {
            damage *= (3 + ((weaponAttack - 50) * 0.035));
        }
        else if (weaponAttack >= 101 && weaponAttack < 151)
        {
            damage *= (4.75 + ((weaponAttack - 100) * 0.03));
        }
        else if (weaponAttack >= 151 && weaponAttack < 201)
        {
            damage *= (6.25 + ((weaponAttack - 150) * 0.025));
        }
        else
        {
            damage *= (7.5 + ((weaponAttack - 200) * 0.02));
        }
        player.takeMagicDamage(damage, true);
        player.takeMagicDamage(damage, true);
        player.takeMagicDamage(damage, true);
    ...
    public function usingHealSpell() : Void
    {
        ...
        else if (flags[kFLAGS.DIANA_LVL_UP] >= 2 && flags[kFLAGS.DIANA_LVL_UP] < 8)
        {
            outputText("Unicorn");
        }
        else
    ...
    override private function performCombatAction() : Void
    {
        ...
        {
            if (HPRatio() > .9 || soulforce < 100)
            {
                removeStatusEffect(StatusEffects.MonsterVPT);
            }
            else
            {
                soulforce -= 100;
        ...
        if (HPRatio() < .2 && (mana >= spellCostHeal()) && !hasStatusEffect(StatusEffects.AbilityCooldown1))
        {
            usingHealSpell();
        }
        else if (flags[kFLAGS.DIANA_LVL_UP] >= 12 && flags[kFLAGS.DIANA_LVL_UP] < 16)
        {
            var choice4 : Float = rand(6);
            if (choice4 < 3)
            {
                if ((soulforce >= soulskillCostHailOfBlades1()) && rand(3) == 0)
                {
                    usingHailOfBlades1Soulskill();
                }
                else if ((soulforce >= soulskillCostManyBirds()) && rand(2) == 0)
                {
                    usingManyBirdsSoulskill();
                }
                else
                {
                    eAttack();
            ...
            {
                if (HPRatio() < .5 && !hasStatusEffect(StatusEffects.AbilityCooldown1) && (mana >= spellCostHeal()))
                {
                    usingHealSpell();
                }
                else if (HPRatio() < .5 && rand(3) == 0 && !hasStatusEffect(StatusEffects.MonsterRegen2) && (mana >= spellCostRegenerate()))
                {
                    usingRegenerateSpell();
                }
                else if (rand(3) == 0 && (mana >= spellCostMagicBoltsBarrage()))
                {
                    usingMagicBoltsBarrageSpell();
                }
                else if (rand(2) == 0 && (mana >= spellCostMagicBolt()))
                {
                    usingMagicBoltSpell();
                }
                else if (rand(2) == 0 && (mana < (this.maxMana() - 1500)))
                {
                    usingARC();
                }
                else
        ...
        {
            var choice3 : Float = rand(6);
            if (choice3 < 3)
            {
                if ((soulforce >= soulskillCostManyBirds()) && rand(2) == 0)
                {
                    usingManyBirdsSoulskill();
                }
                else
            ...
            {
                if (HPRatio() < .6 && !hasStatusEffect(StatusEffects.AbilityCooldown1) && (mana >= spellCostHeal()))
                {
                    usingHealSpell();
                }
                else if (HPRatio() < .6 && rand(3) == 0 && !hasStatusEffect(StatusEffects.MonsterRegen2) && (mana >= spellCostRegenerate()))
                {
                    usingRegenerateSpell();
                }
                else if (rand(2) == 0 && (mana >= spellCostMagicBolt()))
                ...
                else if (rand(2) == 0 && (mana < (this.maxMana() - 300)))
                {
                    usingARC();
                }
                else
        ...
        else if (flags[kFLAGS.DIANA_LVL_UP] >= 2 && flags[kFLAGS.DIANA_LVL_UP] < 5)
        {
            ...
            if (choice2 > 2 && choice2 < 5)
            {
                ...
                else if (rand(3) == 0 && (mana < (this.maxMana() - 80)))
                {
                    usingARC();
                }
                else
        ...
        else
        {
            ...
            if (choice1 > 2 && choice1 < 5)
            {
                ...
                else if (rand(4) == 0 && (mana < (this.maxMana() - 40)))
                {
                    usingARC();
                }
                else
    ...
    public function new()
    {
        ...
        if (flags[kFLAGS.DIANA_LVL_UP] < 2 && flags[kFLAGS.DIANA_FOLLOWER] < 3)
        {
            ...
            this.hairLength = 12;
            this.gems = rand(5) + 5;
            this.drop = new ChainedDrop().add(weapons.W_STAFF, 1 / 10).add(consumables.H_PILL, 1 / 5).add(consumables.VDARCON, 1 / 5).add(consumables.BAGOCA1, 1 / 5).add(consumables.EQUINUM, 1 / 2);
        }
        if (flags[kFLAGS.DIANA_LVL_UP] >= 0 && flags[kFLAGS.DIANA_FOLLOWER] == 3)
        {
            ...
            this.hairLength = 16;
            this.gems = rand(5) + 10;
            this.drop = new ChainedDrop().add(weapons.W_STAFF, 1 / 10).add(consumables.H_PILL, 1 / 5).add(consumables.D_ARCON, 1 / 5).add(consumables.BAGOCA1, 1 / 5).add(consumables.EQUINUM, 1 / 2);
        }
        if ((flags[kFLAGS.DIANA_LVL_UP] >= 2 && flags[kFLAGS.DIANA_LVL_UP] < 8) && flags[kFLAGS.DIANA_FOLLOWER] < 3)
        {
            ...
            this.hairLength = 16;
            this.gems = rand(5) + 10;
            this.drop = new ChainedDrop().add(weapons.W_STAFF, 1 / 10).add(consumables.MH_PILL, 1 / 5).add(consumables.D_ARCON, 1 / 5).add(consumables.BAGOCA1, 1 / 5).add(consumables.UNICORN, 1 / 2);
        }
        if (flags[kFLAGS.DIANA_LVL_UP] >= 2 && flags[kFLAGS.DIANA_FOLLOWER] == 3)
        {
            this.a = "the ";
            ...
            this.hairLength = 26;
            this.gems = rand(5) + 10;
            this.drop = new ChainedDrop().add(weapons.W_STAFF, 1 / 10).add(consumables.MH_PILL, 1 / 5).add(consumables.AREGCON, 1 / 5).add(consumables.BAGOCA2, 1 / 5).add(consumables.UNICORN, 1 / 2);
        }
        ...
        {
            this.gems = rand(6) + 14;
            this.drop = new ChainedDrop().add(weapons.W_STAFF, 1 / 10).add(consumables.MH_PILL, 1 / 5).add(consumables.AREGCON, 1 / 5).add(consumables.BAGOCA3, 1 / 5).add(consumables.BAGOCA2, 1 / 4).add(consumables.ALICORN, 1 / 2);
        }
        if (flags[kFLAGS.DIANA_LVL_UP] >= 12 && flags[kFLAGS.DIANA_LVL_UP] < 17)
        {
            this.gems = rand(7) + 18;
            this.drop = new ChainedDrop().add(consumables.BH_PILL, 1 / 15).add(consumables.BAGOCA4, 1 / 15).add(weapons.W_STAFF, 1 / 10).add(consumables.MH_PILL, 1 / 5).add(consumables.AREGCON, 1 / 5).add(consumables.BAGOCA3, 1 / 5).add(consumables.BAGOCA2, 1 / 4).add(consumables.ALICORN, 1 / 2);
        }
        // this.plural = false;
