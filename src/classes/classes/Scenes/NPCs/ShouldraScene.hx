...
class ShouldraScene extends NPCAwareContent implements TimeAwareInterface
{
    ...
    public function timeChange() : Bool
    {
        ...
        if (flags[kFLAGS.SHOULDRA_MAGIC_COOLDOWN] >= 1)
        {
            flags[kFLAGS.SHOULDRA_MAGIC_COOLDOWN]--;
        }
        if (shouldraFollower.followerShouldra())
        {
            if (SceneLib.exgartuan.dickPresent() && rand(10) == 0)
            {
            ...
            if (shouldraFollower.shouldersWarnings())
            {
                needNext = true;
            }
            ...
            if (flags[kFLAGS.SHOULDRA_PLOT_COUNTDOWN] > 0 && CoC.instance.model.time.hours == 3)
            {
    ...
    public function timeChangeLarge() : Bool
    {
        ...
        //Ghostgirl madness
        if (flags[kFLAGS.SHOULDRA_MAIDEN_COUNTDOWN] > 0)
        {
            ...
            {
                flags[kFLAGS.SHOULDRA_MAIDEN_COUNTDOWN] = 0;
            }
            else
            {
                flags[kFLAGS.SHOULDRA_MAIDEN_COUNTDOWN]--;
                if (flags[kFLAGS.SHOULDRA_MAIDEN_COUNTDOWN] == 0)
                {
                    paladinModeFollowup();
                    return true;
    ...
    private function shouldraGreeting() : Void
    {
        spriteSelect(SpriteDb.s_ghostGirl);
        clearOutput();
        if (silly() && flags[kFLAGS.SHOULDRA_MAIDEN_TIMES_MET] == 0)
        ...
        if (flags[kFLAGS.TIMES_MET_SHOULDRA] < 3)
        {
            ...
            //([if first time]
            if (flags[kFLAGS.TIMES_MET_SHOULDRA] == 0)
            {
                outputText("\"<i>You're the asshole that took my racks, aren't you?  I was hoard-...er, I was keeping an eye on those!</i>\" she accuses, eyes narrowing angrily.  ");
            }
        ...
        else
        {
            ...
            if (flags[kFLAGS.TIMES_BEATEN_SHOULDRA] >= 3)
            {
                ...
                if (flags[kFLAGS.TIMES_BEATEN_SHOULDRA] == 3)
                {
                    outputText("You can't say you saw that coming.  It seems she has offered to forego the fight in lieu of sexings.  ");
                }
    ...
    //Ghost Girl Defeated
    @:allow(classes.scenes.nPCs)
    private function defeatDannyPhantom() : Void
    {
        ...
        else
        {
            ...
            if (flags[kFLAGS.TIMES_POSSESSED_BY_SHOULDRA] < 1)
            {
                outputText("Strangely, the action seems to have little to no effect.");
            }
            else
            {
                outputText("Of course, the motion has no effect on her libido, due to her ghostly state.");
            }
    ...
    private function noSlimingSlimer() : Void
    {
        ...
        //([if first time]
        if (flags[kFLAGS.TIMES_POSSESSED_BY_SHOULDRA] == 0)
        {
            outputText("not feeling up to heed the strange girl's whims.");
        }
        else
        {
            outputText("mystified at the thought of letting a ghost possess your body.");
        }
        ...
        if (flags[kFLAGS.TIMES_POSSESSED_BY_SHOULDRA] == 0)
        {
            outputText("girl (what was that about her 'taking over' a goblin?)");
        }
        else
        {
            outputText("spirit");
    ...
    private function littlePigLittlePigLetMeIn() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_POSSESSED_BY_SHOULDRA] == 0)
        {
            outputText("you can afford being a little adventurous with the obviously desperate girl");
        }
        else
        {
            outputText("having a spiritual tag-along might be a bit of fun");
        }
        ...
        {
            outputText(" into your arms for a tight hug");
        }
        else
        {
            outputText(" up into your arms for a tight hug");
        }
        ...
        if (flags[kFLAGS.TIMES_POSSESSED_BY_SHOULDRA] == 0)
        {
            outputText("Wait... WHAT!?  Did you just let some sort of... g-... g-... g-... G-... G-... GHOOOOOST possess you?");
        }
        ...
        menu();
        addButton(0, "Sex Here", curry(sceneHunter.selectGender, 
                        penisSexMerged, 
                        curry(ghostGinaSexings, true), 
                        [(player.isGenderless()) ? "???" : "Wait, no!", curry(genderlessGhostBuster, true)]
            ));
        addButton(1, "Lake", nowOnVickiLake).hint("You could take her to the lake to find someone to play with...").disableIf(player.isGenderless(), "Not for genderless!").disableIf(!SceneLib.boat.isDiscovered()
                || !flags[kFLAGS.CODEX_ENTRY_SHARKGIRLS] && !flags[kFLAGS.CODEX_ENTRY_GOOGIRLS], 
                "You don't know any other place to show her... yet?", "???"
        );
        addButton(2, "Exgartuan", penisGartuanGhostSmex, true).disableIf(!exgartuan.dickAwake(), "You don't have Exgartuan or he isn't awake!");
        addButton(3, "Xenora", ginaBoobgartuanShouldra, true).disableIf(!exgartuan.boobsAwake(), "You don't have Xenora or she isn't awake!");
    }
    //Lake Victory Scenes
    private function nowOnVickiLake() : Void
    {
        ...
        menu();
        addButton(0, "Shark", sharkbustersVictory).disableIf(!flags[kFLAGS.CODEX_ENTRY_SHARKGIRLS], "You've yet to find this one yourself", "???");
        addButton(1, "Goo/Ooze", ghostGooGurlzDuckfaces).disableIf(!flags[kFLAGS.CODEX_ENTRY_GOOGIRLS], "You've yet to find this one yourself", "???");
    }
    ...
    private function sharkbustersVictory() : Void
    {
        ...
        {
            outputText(", [allbreasts] squishing and squashing against her own barely-clad bosom");
        }
        ...
        outputText("She muses over her choices for several moments, snapping her fingers when she appears to find a good one. \"<i>Oh, this will be fun,</i>\" she purrs, then begins casting the archaic spell. A short way into the chant, however, the ghost-girl's voice wavers, and she falls to a knee. You figure the shark girl decided her mind was not completely taken yet, but that never stops the ghost, whose voice hardens as she forces the rest of the words out of her mouth, ending the last word with an uncomfortable groan. As soon as her concentration returns to her, she mentally cows the shark girl back into submission, then stands, brushes herself off, and shrugs at you. You return the gesture, then move forward to check the results of the spell.");
        //(If hermaphrodite:
        ...
        {
            doNext(sharkyEctoginas);
        }
        else if (player.hasCock())
        {
            doNext(ghostIzmaPenis);
        }
    }
    ...
    private function sharkyEctoginas() : Void
    {
        ...
        //[if repeat]
        if (flags[kFLAGS.TIMED_SHARKGINAS] > 0)
        {
            outputText(" again...");
        }
        ...
        if (flags[kFLAGS.TIMED_SHARKGINAS] == 0)
        {
            outputText("You begin to ask what she's talking about, but the words die in your mouth when");
        }
        else
        {
            outputText("You can only shake your head resignedly as");
        }
        ...
        {
            outputText(" by the biggest object you've experienced since Tamani's deluxe dildo");
        }
        ...
        if (player.biggestTitSize() >= 1)
        {
            outputText("and caking your cleavage with the excess lubrication");
        }
    ...
    private function ghostIzmaPenis() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText(" and cradling your [balls] with almost reverent gentleness, running her coarse shark-skinned fingers across your sack as gently as she can manage");
        }
        outputText(".\n\n");
        ...
        //[if first time]
        if (flags[kFLAGS.TIMES_SHARKPENISED] == 0)
        {
            outputText("whose expression reveals she's as clueless as you are.");
        }
        else
        {
            outputText("who only smiles as she realizes her 'mistake' in her spellcasting once again.");
        }
        outputText("  \"<i>That wasn't nearly what the spell should have been,</i>\" the girl muses, licking her lips ");
        if (flags[kFLAGS.TIMES_SHARKPENISED] == 0)
        {
            outputText("absently as she contemplates what could have caused the lengthening of your loins");
        }
        else
        {
            outputText("in anticipation for the experience you both are about to undergo, yet again");
        }
        ...
        if (flags[kFLAGS.TIMES_SHARKPENISED] == 0)
        {
            outputText("as she deduces the nature of the spell. \"<i>I think... oh my... my host's saliva may have expansive qualities on... sexual aspects of the body,</i>\" she explains. \"<i>So, if I were to do this...</i>\"");
        }
        else
        {
            outputText("\"<i>Would you look at that, my magic spittle still works! How about I just...</i>\"");
        }
        outputText("\n\n");
        ...
        if (player.hasBalls())
        {
            outputText(" and softly churning sack");
        }
        ...
        if (player.hasBalls())
        {
            outputText("  Your enchanted testes give an audible groan as you reach climax.");
        }
        else
        {
            outputText("  You feel something stirring in your groin, and you squirm as the tingling intensifies.");
        }
    ...
    private function ghostGooGurlzDuckfaces() : Void
    {
        ...
        if (player.hasCock() && !player.hasVagina())
        {
            outputText(" presenting your [cocks] to the spectral slime.");
        }
        else if (player.hasVagina())
        {
            outputText(" slipping a hand down to your " + clitDescript() + " eagerly.");
        }
        //[if herm]
        else
        {
            outputText(" both of your sexes throbbing in excitement.");
        }
        ...
        if (player.isHerm() && xor(flags[kFLAGS.SLIMEGINAED] == 0, flags[kFLAGS.SHOULDRA_SLIME_PENOR_TIMES] == 0))
        {
            outputText("\n\n<b>Maybe if you try both of your genders first, Shouldra will come up with something new?</b>");
            sceneHunter.selectGender(slimeyPenorsAhoy, slimeGinaFlation, null, null, 0);
        }
        else
        {
            sceneHunter.selectGender(slimeyPenorsAhoy, slimeGinaFlation, null, slimeGhostGalForHerms, 0);
        }
    }
    ...
    private function slimeGinaFlation() : Void
    {
        ...
        if (player.cor < 50)
        {
            outputText("you beg her to stop before this gets out of hand.");
        }
        else
        {
            outputText("you feel the constant stimulation of your carnal canal weakening your resolve until you're almost encouraging her.");
        }
        outputText("\n\n");
        ...
        if (player.biggestTitSize() < 1)
        {
            outputText("Your frame is jostled about as the slime under you shrinks.");
        }
        else
        {
            outputText("Your frame is jostled about as the slime under you shrinks, your goo-filled gut pushing your [allbreasts] up into your overburdened jaw.");
        }
        outputText("\n\n");
        ...
        if (player.isNaga())
        {
            outputText("snake-like lower body");
        }
        else if (player.isGoo())
        {
            outputText("undulating, gooey mass");
        }
        else if (player.tailType == Tail.NONE)
        {
            outputText("arms and " + player.legs());
        }
        else
        {
            outputText("arms, [legs], and tail");
        }
        ...
        if (player.biggestTitSize() < 1)
        {
            outputText("see your " + nippleDescript(0) + " wiggling sporadically.");
        }
        else
        {
            outputText("see your [allbreasts] trembling, " + nippleDescript(0) + " twitching eagerly.");
        }
    ...
    private function getEctoplasm() : Void
    {
    ...
    private function slimeyPenorsAhoy() : Void
    {
        ...
        if (player.biggestTitSize() < 1)
        {
            outputText(" while squishing her small boobs into your own " + allBreastsDescript());
        }
        ...
        if (flags[kFLAGS.SHOULDRA_SLIME_PENOR_TIMES] > 0)
        {
            outputText("You can't shake the feeling that she's used this particular spell before. What an unoriginal ghost.  ");
        }
        ...
        if (player.hasBalls())
        {
            outputText("and teasing your quaking testes ");
        }
        ...
        if (player.isNaga())
        {
            outputText("slapping against your tail");
        }
        else if (player.isGoo())
        {
            outputText("smooshing against your goo");
        }
        else
        {
            outputText("swiftly bruising your thighs");
        }
        ...
        if (player.hasBalls())
        {
            outputText("  The near-vacant-minded girl rewards you with two slime-tentacles to fondle your scrotum.  You hiss in a sharp breath at the sudden stimulation, but swiftly come to welcome it.");
        }
        ...
        if (player.hasBalls())
        {
            outputText("the heat spreads to your testicles, promptly puffing them up with arcane virility");
        }
        else
        {
            outputText("you feel a discomfort beginning in your prostate, as if the organ is beginning to grow with arcane virility");
        }
        ...
        if (player.hasBalls())
        {
            outputText("Your [legs] are forced apart by your burgeoning cum-sack");
        }
        else
        {
            outputText("You nearly go insane with the pressure upon your sex");
        }
        ...
        if (player.hasBalls())
        {
            outputText("balls to churn");
        }
        else
        {
            outputText("body to prepare itself for release");
        }
        ...
        if (player.hasBalls())
        {
            outputText("waddle over to where the mewling ghost girl is twitching on the ground; your now-shrinking testicles hindering your gait.");
        }
        else
        {
            outputText("limp over to where the mewling ghost girl is twitching on the ground; your now-shrinking prostate hinders your gait.");
        }
    ...
    //Genderless Scene
    private function genderlessGhostBuster(win : Bool) : Void
    {
        spriteSelect(SpriteDb.s_ghostGirl2);
        if (win)
        {
            ...
            {
                outputText("Excited to help relieve the girl of her accumulating lust, you allow her control of your arms. She eagerly cups a hand against your crotch to feel... nothing. Confused, she quickly wriggles out of your [armor] and goes in for another grope, feeling nothing but skin. \"<i>Son of a...please don't tell me you don't have any sexual bits I can play with!</i>\" she huffs, and you chuckle sheepishly, not knowing what to tell her.");
            }
            else
            {
                outputText("Having changed your mind, you cross your arms and tell Shouldra she's not getting off this time. You don't want it, and the will of the host should be above hers... After a second of confusion, she finally responds. \"<i>W-what? Are you kidding me?! I'm already inside. And what is this about will?</i>\" You calmly repeat everything including the last parts, not without a little smirk, knowing just how much she will be pissed.");
            }
        }
        ...
        {
            outputText("She tries to get herself off using your genitals, but you do your best to prevent her from it.");
        }
        outputText(" She groans and falls back onto your [butt], muttering a string of curse words under your breath. \"<i>That's it!</i>\" she fumes. \"<i>I'm going to get off, whether you want me to or not!</i>\" She resumes cursing and wiggling your hips for a bit longer, then stops suddenly, a smirk playing across your face. \"<i>Whether you want me to or not...</i>\"\n\n");
        
        outputText("She hops to your feet and makes for the decrepit hut you first saw her in.  ");
        if (flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0)
        {
            outputText("You raise an eyebrow, genuinely curious as to how she'll figure out how to relieve her desire.  ");
        }
        ...
        if (flags[kFLAGS.SHOULDRA_GENDERLESS_FUCK_COUNT] == 0)
        {
            outputText("Curious as to its contents, you glance inside - it's a chest of dildos, of every shape and size. Before you can ask why the hell a ghost would carry a CHEST of dildos, ");
        }
        else
        {
            outputText("You recall distinctly the contents of that chest, and are not surprised ");
        }
        ...
        if (player.biggestTitSize() < 1)
        {
            outputText("two new additions bud on your chest.");
        }
        else if (player.biggestTitSize() <= 8)
        {
            outputText("your [allbreasts] begin to bounce all around.");
        }
        else
        {
            outputText("your [allbreasts] swell slightly to make room for a sudden increase in milk production.");
        }
        outputText("As the ghost continues darting around the room, ");
        //[if flat to E-cup]
        if (player.biggestTitSize() <= 8)
        {
            outputText("your new EE-cup breasts settle");
        }
        else
        {
            outputText("the lactation continues, and the ghost girl moves back to the table and settles down a bit");
        }
        ...
        if (player.isNaga())
        {
            outputText("against your naga-crotch");
        }
        else if (player.isGoo())
        {
            outputText("into your cushiony goo");
        }
        else
        {
        ...
        if (player.isGenderless())
        {
            outputText(", seemingly forgetting about your lack of genitalia,");
        }
        outputText(" and grudgingly goes for the next penetrable orifice; your " + assholeDescript() + ". With blatant disregard for safety, the vibrators still buzzing happily in your increasingly warm nipples, she slams the horsecock against the ground, waits for the wiggling to somewhat cease, then lines your butt up with the flared tip. Her crazed laughter mingles with your horrified scream as she sits on it, driving it at least four inches inside in its initial penetration");
        //[if anus is not gaping]
        if (player.ass.analLooseness <= 4)
        {
            outputText(" and stretching your butt obscenely");
        }
        outputText(".");
        ...
        if (player.biggestTitSize() < 1)
        {
            outputText("your tits, along with the elongated nipples, receding back into a flat chest.");
        }
        else if (player.biggestTitSize() <= 8)
        {
            outputText("your boobflesh shrinking back down into its normal size, nipples included.");
        }
        else
        {
            outputText("the nipples shortening once again, and the lactation finally stabilizing.");
        }
    ...
    private function penisSexMerged() : Void
    {
        ...
        if (flags[kFLAGS.SHOULDRA_PENIS_DEFEAT_TIMES] == 0)
        {
        ...
        {
            outputText("She chuckles softly as she recalls the times you've shared, and she hungrily goes for your maleness once more, though you mentally throw her off of it. ");
        }
        ...
        if (player.inte < 50)
        {
            outputText("It's not very hard for her to break down your fortifications, and you soon feel yourself being pushed aside by her vast mental capacity.");
        }
        else if (player.inte < 90)
        {
            outputText("You begin to gain an edge, but the ghost girl is craftier than you expected, and she begins to pulse mental intrusions at you, scrambling your thoughts just long enough for her to overtake you and push you to the side.  \"<i>You are one hundred years too early to challenge me,</i>\" she chides. \"<i>My intellect has long surpassed most any living creature on this plane.</i>\"");
        }
        if (player.inte < 90)
        {
            sceneHunter.print("Check failed: intelligence.");
            sceneHunter.selectGender(
                    curry(ourDadTaughtUsNotToBeAshamedOfOurDicks, true), null, null, 
                    curry(loseToShouldraAsHerm, true)
            );
        }
        else
        {
            outputText("But thanks to a strong will of your own, you still can overpower her again. Will you do it, or do you want to let her play with your body?");
            menu();
            addButton(0, "Crush!", crushHer);
            addButton(1, "Let Her", curry(sceneHunter.selectGender, 
                            curry(ourDadTaughtUsNotToBeAshamedOfOurDicks, true), null, null, 
                            curry(loseToShouldraAsHerm, true)
                ));
        }
        
        var crushHer : Void->Void = function() : Void
        {
            ...
            if (player.cockTotal() > 1)
            {
                ...
                {
                    outputText("the rest of " + multiCockDescriptLight());
                }
                outputText(", the mentally dominated girl quickly clutches ");
                if (player.cockTotal() == 2)
                {
                    outputText("your other shaft and pumps it vigorously.");
                }
                else if (player.cockTotal() == 3)
                {
                    outputText("your other two shafts and pumps them vigorously.");
                }
                else
                {
                    outputText("two other shafts and pumps them vigorously.");
                }
            }
            outputText("\n\n");
            ...
            if (player.cockTotal() == 2)
            {
                outputText("Taking her hand away from your other dick, the ");
            }
            else if (player.cockTotal() >= 3)
            {
                outputText("Taking her hands away from your other dicks, the ");
            }
            else
            {
                outputText("The ");
            ...
            {
                outputText("near the base of your shaft");
            }
            outputText(".  ");
            if (player.balls == 0)
            {
                outputText("The first effects of the spell make themselves known in a big way when an overladen cumsack forms and drops between your legs.  ");
            }
    ...
    private function ghostBJFinisher(inside : Bool) : Void
    {
        spriteSelect(SpriteDb.s_ghostGirl2);
        var x : Float = player.smallestCockIndex();
        ...
        if (inside)
        {
            outputText("excitedly convulsing");
        }
        else
        {
            outputText("practically lust-paralyzed");
        }
    ...
    private function penisGartuanGhostSmex(win : Bool) : Void
    {
        spriteSelect(SpriteDb.s_ghostGirl2);
        var x : Float = player.biggestCockIndex();
        if (win)
        {
            clearOutput();
            outputText("You feel an almost overwhelming surge of anxious arousal as the ghost girl regards your excessively large " + cockDescript(x) + ". You decide to play along and place your hand over the bulge in your [armor], enjoying her gleeful titter as your member twitches and hardens, just a tad. You begin to strip, but are");
        }
        else
        {
            outputText("Your entire body shivers in delight as the ghost girl regards your excessively large " + cockDescript(x) + ". She places your hand over the bulge in your [armor] and titters in glee as your member twitches and hardens, just a tad. She strips eagerly, but is");
        }
        outputText(" halted by a familiar booming voice coming from the half-erect member. \"<i>WHO THE HELL IS THIS BITCH?</i>\" a perplexed Exgartuan yells grumpily. \"<i>Living in peoples' bodies is MY THING!</i>\"\n\n");
        ...
        if (player.hasVagina())
        {
            outputText(" like this bitch here");
        }
        outputText("?</i>\"\n\n");
        
        outputText("A blush (somehow) coming over her [color] cheeks, the ghost girl nods her head meekly. She obviously doesn't know what to expect from the volatile creature. She suddenly gives a little cry, and her head is sucked back into your body. You can vaguely feel her presence shift, and then, as if pulled, moving... into your penis?  You feel full control returning to you, and you place a hand under your prodigious pecker and hoist it slightly. You jump as the thing bucks in your grasp. \"<i>ALRIGHT, BITCH, LET'S GET TO SPIRIT SEX,</i>\" Exgartuan exclaims, and to your surprise, your spiritual newcomer groans expectantly.\n\n");
        ...
        if (flags[kFLAGS.SHOULDRA_EXGARTUAN_SPIRIT_SEX_COUNT] > 0)
        {
            outputText("once again ");
        }
        ...
        if (flags[kFLAGS.SHOULDRA_EXGARTUAN_SPIRIT_SEX_COUNT] == 0)
        {
            outputText("\"<i>Spirit sex... I can't say I've ever tried that,</i>\" she admits. \"<i>Damn if it isn't good.</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Damn,</i>\" she sighs, \"<i>that is still a great ride, I'd say...</i>\"\n\n");
        }
    ...
    //Vagina Scene
    private function ghostGinaSexings(win : Bool) : Void
    {
        spriteSelect(SpriteDb.s_ghostGirl2);
        if (win)
        {
        ...
        if (flags[kFLAGS.SHOULDRA_VAGINAL_POSSESSIONS] == 0)
        {
            outputText("She 'hmm's several times inside your head, but you can feel a surge of elation as she apparently reaches a solution. \"<i>Of course!</i>\" she cries excitedly.  \"<i>I actually have never done this before!</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Oh well, I suppose we can always just do that again,</i>\" she muses. \"<i>It worked well enough the first time.</i>\"\n\n");
        }
    ...
    //Loss Introduction
    @:allow(classes.scenes.nPCs)
    private function loseToShouldra() : Void
    {
        spriteSelect(SpriteDb.s_ghostGirl2);
        clearOutput();
        ...
        {
            outputText("Each pulse pushes her a bit further onto your hardening manhood");
        }
        else if (player.hasVagina())
        {
            outputText("You can't deny the heat slowly welling up in your loins, your love-tunnel getting more lubricated by the moment");
        }
        else
        {
            outputText("You can't deny the heat slowly starting to gather in your bare loins");
        }
        ...
        if (player.hasCock() && (!player.hasVagina() || rand(2) == 0))
        {
            outputText("quivering member");
        }
        else if (player.hasVagina())
        {
            outputText("sopping gash");
        }
        else
        {
            outputText("sensitive crotch-flesh");
        }
        ...
        {
            sceneHunter.selectLossMenu([  //full menu  
                    [0, "Dick", curry(ourDadTaughtUsNotToBeAshamedOfOurDicks, false), 
                    "Req. a cock.", player.hasCock(), 
                    "Let the girl play with your male parts!"
            ], 
                    [5, "Exgart-Dick", curry(penisGartuanGhostSmex, false), 
                    "Req. a cock and Exgartuan in it.", player.hasCock() && SceneLib.exgartuan.dickPresent(), 
                    ...
                    "The ghost-girl loves to have fun and make dicks big... but what if you give her THIS one to play?"
            ], 
                    [1, "Vagina", curry(ghostGinaSexings, false), 
        ...
        //No uniherms - only one scene, lol{
            
            if (player.hasStatusEffect(StatusEffects.Infested))
            {
                loseToShouldraWithWorms();
            }
            else if (SceneLib.exgartuan.dickAwake())
            {
                penisGartuanGhostSmex(false);
            }
            ...
            {
                ourDadTaughtUsNotToBeAshamedOfOurDicks(false);
            }
        }
        else if (player.gender == 2)
        {
            ghostGinaSexings(false);
        }
        else if (player.gender == 3)
        {
            ...
            {
                loseToShouldraAsHerm(false);
            }
        }
        else
        {
            genderlessGhostBuster(false);
        }
    }
    ...
    //Penis Scene
    //[If you have a penis]
    private function ourDadTaughtUsNotToBeAshamedOfOurDicks(win : Bool) : Void
    {
        spriteSelect(SpriteDb.s_ghostGirl2);
        var x : Float = player.biggestCockIndex();
        if (win)
        {
            outputText("\n\nTaking complete control of your body as if it was never actually yours, she removes your [armor] and throws them carelessly to the ground.");
        }
        else
        {
            if (flags[kFLAGS.SHOULDRA_PENIS_DEFEAT_TIMES] == 0)
            {
                outputText("She abruptly gasps in excitement, ");
            }
            else
            {
                outputText("She 'ooo's softly as she recalls your anatomy, ");
            }
            outputText("and you feel your hands moving unbidden towards your crotch. You snap the sneaky extremities back to your sides, knowing exactly what she wants to do. The ghost girl soon proves herself the stronger in the contest of wills, however, and eventually, your will breaks and she takes complete control once again. She forces your body to stand, removing your [armor] and throwing them carelessly to the ground.");
        }
        outputText(" Grabbing up your " + cockDescript(x) + " in both hands and fondling it awkwardly, she strikes you as obviously inexperienced with the male anatomy, despite her levels of intelligence. She continues to grope your lengthening member, admitting, \"<i>Even with the rare male bodies I have possessed, I rarely got to play with their...</i>\"\n\n");
        ...
        if (player.hasBalls())
        {
            outputText("your testicles churn with fertility and ");
        }
    ...
    private function loseEnding() : Void
    {
    ...
    //Hermaphrodite Scene
    private function loseToShouldraAsHerm(win : Bool) : Void
    {
        spriteSelect(SpriteDb.s_ghostGirl2);
        var x : Float = player.biggestCockIndex();
        if (win)
        ...
        if (player.hasBalls())
        {
            outputText("Stopping her double-handed stroke technique, she slips your hand under your obtrusive [balls] and gently pushes them aside to get a better look at your previously unnoticed female sex.  ");
        }
    ...
    private function shouldraGiantCockLoss() : Void
    {
        ...
        //[if first time]
        if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0)
        {
            outputText("bemused ");
        }
        else
        ...
        if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0)
        {
            outputText("searches her body for any signs of change, and when she finds none, begins pouting.");
        }
        else
        {
            outputText("resists the urge to smash her tits together, waiting with as much patience as she can muster.");
        }
        outputText("  By this time, the ghost girl has finished with the spell, ");
        //[if first time]
        if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0)
        {
            outputText("and instead of offering an explanation, she merely ");
        }
        else
        {
            outputText("and, to the green girl's obvious delight, ");
        }
        outputText("reaches over and kneads the goblin's ");
        //([if first time]
        if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0)
        {
            outputText("small, firm B-cups");
        }
        else if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1)
        {
            outputText("perky, sizeable C-cups");
        }
        else if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2)
        {
            outputText("well-formed, jiggly D-cups");
        }
        else if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3)
        {
            outputText("big and squishy E-cups");
        }
        else if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4)
        {
            outputText("oversized, heaving F-cups");
        }
        else
        {
            outputText("gigantic and openly-bared G-cups");
        }
        outputText(". The goblin ");
        //[if first time]
        if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0)
        {
            outputText("instantly stops her protests, suddenly too stimulated by the massage to worry about any magical favoritism, ");
        }
        else
        {
            outputText("cries out in ecstasy at the still-incredible sensation, ");
        }
        //[if first time]
        if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0)
        {
            outputText("although that doesn't stop the second goblin to begin voicing her own dissent");
        }
        else
        {
            outputText("and you can't help but notice the excited blush running across the second goblin's face");
        }
        outputText(". The ghost girl pinches a quickly-stiffening nipple and pulls it out, ");
        if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0)
        {
            outputText("and... pulls the breast along with it?");
        }
        else
        {
            outputText("and pulls the breast along with it.");
        ...
        if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0)
        {
            outputText("as the effects of the magic are revealed");
        }
        else
        {
            outputText("despite seeing the magic before, always enthralled by the process");
        }
        outputText("; it has turned the goblin's breasts into an almost putty-like substance");
        if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0)
        {
            outputText(", evidently!");
        }
        else
        {
            outputText(" once again");
        ...
        if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0)
        {
            outputText("gasps in amazement, ");
        }
        else
        {
            outputText("can't restrain her urges any longer, ");
        }
        outputText("swiftly reaching down to roughly grab at her own ");
        //[if first time]
        if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0)
        {
            outputText("petite boobs");
        }
        else if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1)
        {
            outputText("modest bosom");
        }
        else if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2)
        {
            outputText("sizable chest");
        }
        else if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3)
        {
            outputText("large endowments");
        }
        else if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4)
        {
            outputText("huge tits");
        }
        else
        {
            outputText("titanic melons");
        }
        ...
        if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 0)
        {
            outputText("DD-cup (and C-cup, on the other side)");
        }
        else if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 1)
        {
            outputText("EE-cup (and D-cup, on the other side)");
        }
        else if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 2)
        {
            outputText("FF-cup (and E-cup, on the other side)");
        }
        else if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 3)
        {
            outputText("GG-cup (and F-cup, on the other side)");
        }
        else if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] == 4)
        {
            outputText("HH-cup (and G-cup, on the other side)");
        }
        else
        {
            outputText("II-cup (and H-cup, on the other side)");
        }
        ...
        if (flags[kFLAGS.SHOULDRA_USES_YOUR_GIANT_COCK_COUNT] <= 4)
        {
            outputText("they have kept at least a cup size of flesh from their experience");
        }
        else
        {
            outputText("their breasts seem to have reached their crescendo, keeping the same (admittedly incredibly stacked) measurements");
        }
    ...
    private function superCoolPaladinSexStuffDudeGasmJizzMonkies() : Void
    {
        ...
        flags[kFLAGS.SHOULDRA_MAIDEN_TIMES_MET]++;
    ...
    public function paladinModeFollowup() : Void
    {
        ...
        if (model.time.hours < 6 || model.time.hours >= 23)
        {
    ...
    private function ginaBoobgartuanShouldra(win : Bool) : Void
    {
        spriteSelect(SpriteDb.s_ghostGirl2);
        if (win)
        {
        ...
        outputText("\n\n\"<i>Perfect,</i>\" Xenora purrs in satisfaction, pinching and probing her newfound folds.  The spiritual slave's mouth flies open, tongue lolling from one side as she squirms under the demoness' teasing touch.  With only a little chuckle as a warning, Xenora pulls the lips as far apart as possible, baring her dark recesses and eliciting a shrill scream of surprise from the ghost girl.  Before she can even recover, the huge-breasted demon cups one of her " + breastDescript(0) + "s and leans forward, slipping the " + nippleDescript(0) + " and a good amount of her boob in with hardly an effort.  A grunt from the kneeling girl and a groan from the demoness is all it takes.  The ghost girl seems unable to even cry out as her already-rotund belly is pushed up to make way for Xenora's boob.  Completely under the influence of the corrupt boob-demon, you mimic the motion, boob compressing into thin air.  A fierce blush paints your features as this awkward scene plays out; you can only hope no one's spying on you.");
        outputText("\n\nGood thing she's a ghost, you figure as you watch the girl's poor stomach grow to absolutely ridiculous size.  Even with the enhanced box, the pressure of her walls pressing down against Xenora's breasts - and yours by extension, you lament while watching the stream of milk splatter against the ground - sends a hearty spray of lactate into the ghost girl's depths.  Cackling devlishly, the demoness takes a hold of the too-big tummy, pressing into it and forcing a gargle from the stuffed specter.");
        outputText("\n\nThe ghost girl tries her hardest to grab her big belly, but she can't even reach all the way around... she can hardly reach halfway!  Xenora, cackling excitedly, draws her boob halfway back before leaning back in, squishing the boobflesh against the wraith's loins and guiding it back inside.  The bizarre boobsex starts off slow, then gains speed as lubrication from the ghost girl's plentiful fluids and Xenora's own lactate builds.");
        outputText("\n\nA duet of beautiful screams echoes around the huge fleshy chamber as the girls fall into rhythm, the ghost girl's heavy lurching complementing the encumbered-yet-lithe manipulations of the demon.  Soon, both girls are panting heavily, Xenora squeezing and stroking her other unattended breast.  With the gusto of one rapidly approaching climax, Xenora rams with all of your strength into the ghost girl's depths instants before her 'floodgates' open.");
        outputText("\n\nWhat used to be an exercise ball swiftly grows to even more exaggerated proportions as the combined milk load of her belly and womb swish against each other.  The prone spirit coughs once, then passes out with a content hum.  Xenora withdraws from the slowly-shrinking vagina with a little pop, rising to her feet with a smooth movement and dropping a hand between her legs.  It's only then, when you feel the sheer amount of fluid soaking your thighs (as you're forced to mimic Xenora's movements, of course), do you realize; you just came from lactation alone!");
        ...
        outputText("\n\nThough exhausted and a little sore from Xenora's treatment of your body, you keep the presence of mind to lean down -- pushing your " + breastDescript(0) + " out of the way -- and scoop up a little ectoplasm from her still-gaping box.  You make a point to ignore Xenora's triumphant chortle as you leave the ghost girl to her sleep and walk back to camp.  What a day.");
        player.sexReward("vaginalFluids", "Vaginal");
        if (!win)
        {
            dynStats("sen", 1);
...
Scene is selectable along with original two*/
    private function slimeGhostGalForHerms() : Void
    {
        ...
        //{event occurrence == 0}
        if (flags[kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 0)
        {
            outputText("I've got this thing all figured out.");
        }
        //event occurrence =1
        else if (flags[kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 1)
        {
            outputText("I've got this thing all figured out.  For real this time.");
        }
        //event occurrence >=2
        else if (flags[kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 2)
        {
            outputText("I've got this thing all figured out.  For real this time.  Promise.");
        }
        //event occurrence >=3
        else
        {
            outputText("Well... don't worry TOO much.");
        }
        outputText("</i>\"");
        ...
        //event occurrence == 0
        if (flags[kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] <= 2)
        {
            outputText("Maybe she really does have a firm grip on the reigns of this beast.");
        }
        //event occurrence >=3
        else
        {
            outputText("Regardless, you've been through this dog and pony show enough times to know that something is bound to go wrong anyway.");
        }
        outputText("\n\nA moist hand on your shoulder belays your skepticism.  A mischievous grin dissolves your apprehension.  A rather large mitt cupping your crotch reinforces your ardor.  \"<i>You didn't take me to the lake just so we could frolic in the water, right?</i>\"  The not-so-subtle arsenal of hints has breached the last of your defenses, forcing your surrender.  Accepting your unspoken peace treaty, the ghost girl breaks into her indecipherable tongue of wizardry.  Despite every syllable slipping past your comprehension, a tinge of familiarity is laced through them.  The tingling in your still-cupped crotch confirms your suspicions.  Your [cock] has widened substantially.  You try and respond with a patronizing smirk, but the supposedly uninventive spector isn't through.  \"<i>I had to refamiliarize myself with some of my favorite work,</i>\" she says while waving her amorphous extremity away from your groin and towards her own.  \"<i>How else would I perfect the set?</i>\"");
        ...
        //if cockLength(0) > 24
        if (player.cocks[0].cockLength > 24)
        {
            outputText("However... it isn't exactly up to your immense scale.  ");
        }
        ...
        //vaginalLooseness == 0
        if (player.looseness() == 0)
        {
            outputText("tight");
        }
        //vaginalLooseness == 1
        else if (player.looseness() == 1)
        {
            outputText("forgiving");
        }
        else if (player.looseness() == 2)
        {
            outputText("loose");
        }
        else if (player.looseness() == 3)
        {
            outputText("gaping");
        }
        else
        {
            outputText("cavernous");
        }
        outputText(", velvety scabbard.  Hands too busy latched onto one another's hips, ");
        if (player.biggestTitSize() > 0)
        {
            outputText("both you and the spooky slime press your tits together, your swaying thrusts only enhancing the mammary meltdown.");
        }
        else
        {
            outputText("the spooky slime presses her tits hard into your flat chest, your gyrations teasing them to no end.");
        }
        ...
        if (flags[kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 0)
        {
            outputText("You couldn't spank it even to save your life, as we've learned today.</i>\"");
        }
        else
        {
            outputText("Evidently, you haven't put in any meaningful practice since last time,</i>\" the specter chides you while making an exaggerated jerking-off motion.");
        }
        if (flags[kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] == 1)
        {
            outputText("  \"<i>I was afraid you'd have... given up the ghost.</i>\"");
        }
        else if (flags[kFLAGS.GHOST_GIRL_SLIME_X_SHOULDRA_COUNTER] > 1)
        {
            outputText("  \"<i>But I'm more than willing to do it again next time with a conclusion like that!</i>\"");
        }
