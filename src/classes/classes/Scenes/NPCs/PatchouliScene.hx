...
class PatchouliScene extends NPCAwareContent
{
	...
	6/7 - Bimbofied
	*/
    public static inline var MET : Int = 1;
    public static inline var OFFERTAKEN : Int = 2;
    public static inline var BADENDED : Int = 3;
    public static inline var FORGIVEN : Int = 4;
    public static inline var TIEDINCAMP : Int = 5;
    public static inline var BIMBO : Int = 6;
    public static inline var MATE : Int = 7;
    
    private var _allVisited : Bool = false;
    
    private function visitedAllAreas() : Bool
    {
        if (_allVisited)
        {
            return _allVisited;
    ...
    public function meetThePatchouli() : Void
    {
        ...
        clearOutput();
        var follower : Dynamic = flags[kFLAGS.PATCHOULI_FOLLOWER];
        if (follower < 1)
        {
        ...
        else
        {
            ...
            if (silly())
            {
                addButton(3, "Finish Him", patchouliKillHim, true);
            }
        }
        
        var patchouliForgiveHim : Void->Void = function() : Void
        {
            spriteSelect(SpriteDb.s_patchouli_male);
            clearOutput();
        ...
        var patchouliKillHim : Bool->Void = function(silly : Bool = false) : Void
        {
            spriteSelect(SpriteDb.s_patchouli_male);
            clearOutput();
            if (silly)
    ...
    public function patchouliRapeHim() : Void
    {
        ...
        var dickF : Void->Void = function() : Void
        {
            ...
            {
                sceneHunter.print("need a bigger dick");
            }
        ...
        else
        {
            ...
            switch (area)
            {
                ...
                case 7:
                    ...
                    {
                        SceneLib.glacialRift.SubZeroConditionsTick();
                    }
                ...
                case 8:
                    ...
                    {
                        SceneLib.volcanicCrag.ConstantHeatConditionsTick();
                    }
    ...
    private function patchouleTalkMenu() : Void
    {
        ...
        var tiedUp : Bool = flags[kFLAGS.PATCHOULI_FOLLOWER] == TIEDINCAMP;
        outputText(" should answer.\n\n");
        outputText("\"<i>Aw nya, wasting your time on such trifles");
        if (flags[kFLAGS.PATCHOULI_FOLLOWER] >= 6)
        {
            outputText(" when we could be having sex");
        }
    ...
    private function patchouleTalkMenuWhoIsSHe() : Void
    {
        ...
        if (tiedUp)
        {
            outputText(". It’s like that rope and tree, why do you think I didn’t go elsewhere yet? It's because I don’t have a cheese on me to break free. I'm not bound by the same rules and reality, yet I’m bound all the same. You and I come from different worlds, [name]");
        }
        ...
        if (tiedUp)
        {
            outputText("Of course, you would have to untie him first and that ain't happening anytime soon.\n\n");
        }
        doNext(patchouleTalkMenu);
        cheatTime(1 / 4);
    }
    ...
    {
        menu();
        addButton(0, "Girl On Girl", patchouleGirlOnGirl).disableIf(!player.hasVagina(), "Req. a vagina!");
        addButton(1, "TakeVaginal", patchouleTakeVaginal).disableIf(flags[kFLAGS.PATCHOULI_COCK] <= 2, "She doesn't have a cock!").disableIf(!player.hasVagina(), "Req. a vagina!");
        addButton(2, "Vaginal", patchouleVaginal).disableIf(!player.hasCock(), "Req. a cock!");
        addButton(3, "Anal", patchouleAnal).disableIf(!player.hasCock(), "Req. a cock!");
        addButton(14, "Back", patchouleMainCampMenu);
    }
    ...
    private function patchouleGirlOnGirl() : Void
    {
        ...
        if (!player.isNaked())
        {
            outputText(" removing your [armor]");
        }
    ...
    private function patchouleGiveItem() : Void
    {
        ...
        var itemButton : ItemType->ButtonData = function(item : ItemType) : ButtonData
        {
        ...
        itemButton(consumables.W_FRUIT).hint("Makes her cock feline").disableIf(flags[kFLAGS.PATCHOULI_COCK] <= 0, "She doesn't have a cock").disableIf(flags[kFLAGS.PATCHOULI_COCK_TYPE] == "feline", "Her cock is already feline.");
        ...
        var giveItem : ItemType->Void = function(item : ItemType) : Void
        {
            clearOutput();
            ...
            switch (item)
            {
                ...
                case consumables.REDUCTO:
                    ...
                    if (flags[kFLAGS.PATCHOULI_COCK] > 6)
                    {
                        addButton(0, "Cock", applyReducto);
                    }
                    if (flags[kFLAGS.PATCHOULI_CUP_SIZE] > 5)
                    {
                        addButton(1, "Boobs", applyReducto, false);
                    }
            }
            
            var applyReducto : Bool->Void = function(cock : Bool = true) : Void
            {
                if (cock)
                {
                    flags[kFLAGS.PATCHOULI_COCK] -= 2;
        ...
        var giveHairDye : Void->Void = function() : Void
        {
            var buttons : ButtonDataList = new ButtonDataList();
            var current : String = flags[kFLAGS.PATCHOULI_HAIR_COLOR];
            var dyes : Array<HairDye> = [
            ...
            var applyDye : HairDye->String->Void = function(dye : HairDye, colour : String) : Void
            {
                clearOutput();
