...
class ValeriaScene extends NPCAwareContent implements TimeAwareInterface
{
    ...
    public function valeriaFluidsEnabled() : Bool
    {
    ...
    public function valeriaFollower() : Void
    {
        ...
        addButton(1, "Spar", valeriaSpar).hint("Do a quick battle with Valeria!").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, "You need a good sparring ring for that.");
        if (sceneHunter.mockFights)
        {
            addButton(6, "Mock Fight", mockValeria);
        }
        addButton(2, "Sex", followersValeriaSex).hint("Initiate sexy time with the armor-goo.").disableIf(player.lust < 33, "Not aroused enough.");
    ...
    public function feedValeria(amnt : Float) : Void
    {
    ...
    private function pcWinsValeriaSpar() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_valeria);
    ...
    public function pcWinsValeriaSparDefeat(offCamp : Bool = false) : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_valeria);
        if (!offCamp)
        {
            ...
            if (player.HP < 1)
            {
                outputText("in too much pain");
            }
            else
            {
                outputText("too turned on");
            }
    ...
    private function followersValeriaSex(display : Bool = true) : Void
    {
        spriteSelect(SpriteDb.s_valeria);
        if (display)
        {
            ...
            if (player.gender > 0)
            {
                outputText("she's interested in a little fluid exchange");
            }
            else
            {
                outputText("she wouldn't mind helping you get some release, despite your genderlessness");
            }
            outputText(".");
            outputText("\n\n\"<i>Hmm... I suppose that could be arranged. What did you have in mind, partner?</i>\"");
        }
        menu();
        addButton(0, "PenetrateHer", penetrateValeria).hint("Fuck the goo girl with your penis!").disableIf(!player.hasCock(), "Req. a cock.");
        ...
        if (player.gender > 0)
        {
            addButton(3, "GetDominated", valeriaSexDominated).hint("Submit to the armor-goo and have her take charge.");
        }
        if (flags[kFLAGS.VELARIA_FUTA] == 1)
        {
            addButton(4, "Lose Dick", valeriaDickToggle).hint("Ask her to hide that gooey cock of hers.");
        }
        else
        {
            addButton(4, "Grow Dick", valeriaDickToggle).hint("Ask her to grow a gooey cock.");
        }
    ...
    public function valeriaGetFucked() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText("stroking the slick lips of your [vagina], ask Valeria to fuck you.");
        }
        else
        {
            outputText("posing seductively, ask Valeria to fuck you.");
        }
        outputText("\n\nShe grins.  \"<i>You just lie back and let me take care of everything, partner.</i>\"  You do as she asks, flopping onto your back ");
        if (player.isBiped())
        {
            outputText("and spreading your legs ");
        }
        outputText("as Valeria ");
        if (flags[kFLAGS.VELARIA_FUTA] == 0)
        {
            outputText("forms a thick, gooey cock");
        }
        else
        {
            outputText("grabs her thick, gooey cock");
        }
        ...
        {
            outputText("bumping over your [chest] and stopping for a brief moment to tickle your sensitive tits with her goopy bottom before ");
        }
        outputText("coming to a stop over your crotch.  She pours herself into your lap, ");
        //[if PC has cock: \"<i>
        if (player.hasCock())
        {
            outputText("her gooey ass devouring your [cock] as she slides down your belly");
        }
        ...
        if (player.hasVagina())
        {
            outputText(" letting bits of her goo-body slither into your wet, waiting [vagina], ever so slightly peeling your walls apart");
        }
        //If PC has no gender
        if (player.gender == 0)
        {
            outputText("poking the shaft of her cock gingerly, teasingly against your [asshole]");
        }
        outputText(".");
        ...
        if (player.hasCock())
        {
            outputText(", as she rhythmically flexes her ass cheeks, shifting her fluidity around your shaft within her with a feeling like lotion-covered hands gently stroking you");
        }
        outputText(".");
    ...
    public function gooFlation(clearText : Bool = true) : Void
    {
        spriteSelect(SpriteDb.s_valeria);
        if (clearText)
        {
    ...
    public function valeriaSexDominated(offCamp : Bool = false) : Void
    {
        spriteSelect(SpriteDb.s_valeria);
        clearOutput();
        ...
        function vagF() : Void
        {
            ...
            if (player.biggestTitSize() > 1)
            {
                ...
                {
                    outputText("  To her delight, a spray of warm milk jets out of your sore nipples, milky white mixing into blue goo like oil in water.  \"<i>Mmm, tasty!</i>\" she teases, massaging more and more from you.");
                }
            }
            ...
            if (player.vaginalCapacity() > 80 || player.analCapacity() > 80)
            {
                outputText(" and your own receptibility");
            }
    ...
    private function talkWithValeria() : Void
    {
        ...
        if (player.gender > 0)
        {
            addButton(0, "Flirt", flirtWithValeria).hint("Flirt with Valeria and initiate sex with her.");
        }
    ...
    private function flirtWithValeria() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("brushing along the length of your rapidly-hardening cock");
        }
        else
        {
            outputText("brushing a wet, warm knuckle across your [clit]");
        }
        outputText(".");
    ...
    private function acceptValeriasNeeds() : Void
    {
        ...
        {
            outputText(" after all, you have the same ones");
        }
    ...
    public function valeriaAndGooThreeStuff() : Void
    {
        ...
        if (player.hasCock())
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("it as it distends");
            }
            else
            {
                outputText("them as they distend");
            }
        ...
        outputText("\n\nThe bluish");
        if (monster.bodyColor != "blue")
        {
            outputText(" " + (try cast(monster, GooGirl) catch(e:Dynamic) null).gooColor6());
        }
        ...
        if (flags[kFLAGS.TIMES_VALERIA_GOO_THREESOMED] == 0)
        {
            outputText("demand that she release you");
        }
        else
        {
            outputText("playfully comment that she really ought to release you");
        }
    ...
    private function valeriaGooRapeII() : Void
    {
        ...
        if (player.biggestTitSize() >= 3)
        {
            outputText("the valley of your cleavage");
        }
        else
        {
        ...
        {
            outputText("You hold tight, keeping up your faux resistance until she yanks your cheeks open and twists her gooey reaming-tool into a sphincter-penetrating drill bit.");
        }
        ...
        if (player.hasFuckableNipples() && player.biggestTitSize() >= 5)
        {
            ...
            if (player.hasVagina())
            {
                outputText(" as your snatch");
            }
            else
            {
                outputText(" as the randiest slut's snatch");
            }
        ...
        if (player.hasCock())
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("this little guy");
            }
            else
            {
                outputText("these little guys");
            }
            outputText(", did you? ");
            if (player.biggestCockArea() >= 50)
            {
                outputText("...Maybe big guy is a bit more appropriate. Whatever. ");
            }
        ...
        if (flags[kFLAGS.TIMES_VALERIA_GOO_THREESOMED] > 0)
        {
            outputText("You grin in eager anticipation, waiting to feel the flood of fluid that you know is coming. It won't take long.... You feel it in your butt first.");
        }
        else
        {
            outputText("At first, you don't realize what it is. You're floating in a bit of a sexual daze by now and barely cogent of what's going on, but it does feel quite nice, whatever it is. The spreading heat in your posterior is what finally clues you in to what's happening.");
        }
        ...
        if (player.hasCock())
        {
            ...
            if (player.cockTotal() > 1)
            {
                outputText("They start");
            }
            else
            {
                outputText("It starts");
            }
            ...
            if (player.balls == 0)
            {
                outputText("something expanding as you cum, filling up with spunk or slime or some mixture of the two");
            }
            else
            {
                outputText("your [balls] expanding as you cum, filling up with spunk or slime or some mixture of the two, weighing down your sack with ever-increasing heaviness. Tender licks of tongue roll across the surface as you fill beyond completely");
            }
            outputText(".");
        }
        //Butt fillinggasm
        ...
        {
            outputText(", pausing to handle your hefty, swelling tits");
        }
        ...
        if (player.hasVagina())
        {
            outputText(", so gravid from both holes that a real mother would be shamed by your obscene \"pregnancy.\"");
        }
        else
        {
            outputText(".");
        ...
        if (player.hasVagina() && !player.isPregnant())
        {
            ...
            player.knockUp(PregnancyStore.PREGNANCY_GOO_STUFFED, 500, 1, 1);
        }
        if (player.hasCock())
        {
            if (player.hasBalls())
            {
                player.changeStatusValue(StatusEffects.GooStuffed, 2, 2);
            }
            else
            {
                player.changeStatusValue(StatusEffects.GooStuffed, 2, 1);
            }
        }
        if (player.biggestTitSize() >= 5 && player.hasFuckableNipples())
        {
            player.changeStatusValue(StatusEffects.GooStuffed, 4, 1);
        }
        cleanupAfterCombat();
    }
    ...
    public function birthOutDatGooSlut() : Void
    {
        ...
        {
            outputText(" Your tits are even wobbling around wildly, shaking and jiggling obscenely inside your [armor] in a way that makes you your [nipples] more than a little leaky.");
        }
        ...
        if (player.statusEffectv4(StatusEffects.GooStuffed) > 0)
        {
            ...
            if (!player.hasFuckableNipples())
            {
                outputText("Stiffening up");
            }
            else
            {
                outputText("Opening up");
            }
            ...
            if (player.bRows() > 1)
            {
                outputText(" and the next row down");
            }
            if (player.bRows() > 2)
            {
                outputText(", and so on");
            }
        ...
        if (player.hasCock() && player.statusEffectv2(StatusEffects.GooStuffed) > 0)
        {
            ...
            if (player.hasBalls())
            {
                outputText("ball-clenching");
            }
            else
            {
                outputText("body-clenching");
            }
            ...
            if (player.statusEffectv2(StatusEffects.GooStuffed) >= 2)
            {
                outputText("  Your balls keep their enhanced sized. Odd.");
            }
        }
