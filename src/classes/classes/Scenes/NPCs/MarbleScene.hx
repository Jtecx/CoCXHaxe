...
class MarbleScene extends NPCAwareContent implements TimeAwareInterface
{
    ...
    public function timeChange() : Bool
    {
        ...
        //Marble stuff pt I{
            ...
            if (flags[kFLAGS.MARBLE_PURIFICATION_STAGE] >= 5)
            {
                ...
                if (flags[kFLAGS.MARBLE_TIME_SINCE_NURSED_IN_HOURS] > 1000)
                {
                    flags[kFLAGS.MARBLE_TIME_SINCE_NURSED_IN_HOURS] = 1000;
                }
            }
            //Reset Marble corruption warning
            if (flags[kFLAGS.MARBLE_WARNED_ABOUT_CORRUPTION] == 1 && player.cor < 50)
            {
                flags[kFLAGS.MARBLE_WARNED_ABOUT_CORRUPTION] = 0;
            }
            //Lactation whoopie!
            if (flags[kFLAGS.MARBLE_TIME_SINCE_NURSED_IN_HOURS] < 100)
            {
                flags[kFLAGS.MARBLE_TIME_SINCE_NURSED_IN_HOURS]++;
            }
            //Increment Marble's Lust
            if (flags[kFLAGS.MARBLE_LUST] < -100)
            {
                flags[kFLAGS.MARBLE_LUST] = -100;
            }
            if (rand(2) == 0)
            {
                flags[kFLAGS.MARBLE_LUST] += 1;
            }
            if (player.statusEffectv4(StatusEffects.Marble) > 50)
            {
                flags[kFLAGS.MARBLE_LUST] += .3;
            }
            if (player.statusEffectv4(StatusEffects.Marble) > 70)
            {
                flags[kFLAGS.MARBLE_LUST] += .3;
            }
        ...
        if (player.hasStatusEffect(StatusEffects.MarbleItemCooldown))
        {
            ...
            if (player.statusEffectv1(StatusEffects.MarbleItemCooldown) < 1)
            {
                player.removeStatusEffect(StatusEffects.MarbleItemCooldown);
            }
        }
        ...
        //Decrement time remaining by 1{
            ...
            if (player.statusEffectv1(StatusEffects.MarblesMilk) <= 0)
            {
                ...
                if (player.statusEffectv2(StatusEffects.Marble) <= 10)
                {
                ...
                //[addiction is 11-30]
                else if (player.statusEffectv2(StatusEffects.Marble) <= 30)
                {
                ...
                else if (player.statusEffectv2(StatusEffects.Marble) <= 50)
                {
                ...
                //If player is under bottled milk effects{
                    ...
                    else
                    {
                        ...
                        if (player.statusEffectv2(StatusEffects.Marble) <= 90)
                        {
        ...
        if (player.statusEffectv3(StatusEffects.Marble) <= 0)
        {
            if (player.statusEffectv2(StatusEffects.Marble) > 60)
            {
                player.changeStatusValue(StatusEffects.Marble, 2, 60);
            }
        }
        ...
        if (player.hasStatusEffect(StatusEffects.BottledMilk))
        {
            ...
            if (player.statusEffectv1(StatusEffects.BottledMilk) <= 0)
            {
                player.removeStatusEffect(StatusEffects.BottledMilk);
            }
        }
        if (model.time.hours > 23)
        {
            flags[kFLAGS.MARBLE_PLAYED_WITH_KIDS_TODAY] = 0;
            if (player.hasStatusEffect(StatusEffects.Marble) && player.statusEffectv2(StatusEffects.Marble) > 0)
            {
                marbleStatusChange(0, -1);
            }
        }
        return needNext;
    }
    ...
    override public function marbleFollower() : Bool
    {
        return player.hasStatusEffect(StatusEffects.CampMarble);
    }
    
    public function marbleAtCamp() : Bool
    {
        if (marbleFollower())
        {
    ...
    public function encounterMarbleInitially() : Void
    {
        ...
        {
            sceneHunter.print("Check failed: you're too tall.");
        }
        ...
        addButton(4, "Leave", turnOffMarbleForever).hint((recalling) ? "<b>RECALL: Shortcut to the second meeting.</b>" : "She certainly will not be pleased.... but you still will be able to save it.");
    }
    ...
    private function turnOffMarbleForever() : Void
    {
        ...
        {
            doNext(marbleWarningStateMeeting);
        }
    }
    ...
    public function marbleWarningStateMeeting() : Void
    {
        ...
        if (recalling)
        {
            outputText("<b>After some time...</b>\n\n");
        }
    ...
    private function apologizetoWalkingTitsIMEANMARBLE() : Void
    {
        ...
        if (player.inte < 15)
        {
            outputText("Eventually you find Marble stopped, looking towards you with her hands in the air saying, \"<i>You caught me!  Come here.</i>\"  She beckons you towards her chest, and you don't make her wait.");
        }
        else if (player.inte < 40)
        {
            outputText("Eventually you find Marble stopped and waiting for you.  She puts her hands in the air and says, \"<i>You caught me!</i>\"  It's fairly clear she's given herself up, but when she folds her hands in front of her chest and presses her breasts together, then tells you to come over, you aren't complaining.");
        }
        else
        {
            outputText("It isn't too hard to figure out that Marble isn't really trying, and you easily catch her off guard on one of her double backs.  She doesn't even notice you until you peek out from between the stalks next to her, reaching out and getting a handful of her backside.  \"<i>Clever " + player.mf("boy", "girl") + "...</i>\" she says.");
        }
    ...
    private function marbleFightWin() : Void
    {
        marbleSprite();
        clearOutput();
        //Win by hp
        ...
        {
            outputText("Marble falls to the ground defeated; she looks up at you helplessly, wondering what you're going to do next.  ");
        }
        //win by lust
        else
        {
            outputText("Marble collapses and looks at you longingly, pulling up her skirt with a look of desperation in her eyes.  ");
        }
        ...
        menu();
        addButton(1, "Feed Her", forceFeedMarble).disableIf(!player.hasPerk(PerkLib.Feeder) || player.lactationQ() < 200, 
    ...
    @:allow(classes.scenes.nPCs)
    private function marbleFightLose() : Void
    {
        marbleSprite();
        clearOutput();
        //lose by hp
        if (player.HP < 1)
        {
            outputText("After a few too many blows to the head, you topple over to the ground.  ");
        }
        //lose by lust
        else
        {
            outputText("Overcome by desire, you fall to your knees, and start masturbating furiously.  Disgusted with you, Marble hits you upside the head once more, knocking you over.  ");
        }
    ...
    private function forceFeedMarble() : Void
    {
        ...
        {
            outputText("You bet this cow-girl loves to get milked and nursed on just like you, but how often does she get to taste the creamy sweetness of milk?  Having her suck on your own leaky tits would be doing her a favor, right?  You approach the defeated cow-girl; her eyes are still locked onto you, wondering what you're going to do next.  Well, not that you can do much with this crowd watching you...\n\n");
        }
    ...
    private function marblePicksYouUpInitially() : Void
    {
        ...
        {
            outputText("gently scratches behind your ears.  ");
        }
        else
        {
            outputText("lightly caresses your head. ");
        }
    ...
    private function suckleMarble() : Void
    {
        ...
        if (player.tallness < 60)
        {
            outputText("Realizing you might have trouble reaching her breasts, you grab one of the chairs from the table. ");
        }
    ...
    public function encounterMarbleExploring() : Void
    {
        ...
        if (player.tallness < 60)
        {
            outputText("Marble gives her customary greeting of hugging you to her breast before telling you that she is off to get milked at the barn. ");
        }
        //[player hight >= 5 feet]
        else
        {
            outputText("You exchange a quick greeting before Marble tells you that she is off to get milked at the barn. ");
        }
        //[affection <30]
        if (player.statusEffectv1(StatusEffects.Marble) < 30)
        {
    ...
    private function drinkMarbleMilk() : Void
    {
        ...
        if (player.ears.type > Ears.HUMAN)
        {
            outputText(" and occasionally scratching behind your ears");
        }
        outputText(".  ");
    ...
    public function encounterMarbleExploring2() : Void
    {
        ...
        if (player.tallness < 60)
        {
            outputText("As you step into her room, she eagerly rushes over and hugs you to her breast. \"<i>You're as cute as ever, sweetie!</i>\" ");
        }
        ...
        {
            outputText("While you talk mostly about unimportant things, there is some discussion about the world and the dangers within.");
        }
        //[addiction >30]
    ...
    public function helpMarble1() : Void
    {
        ...
        if (player.str < 20)
        {
            outputText("Unfortunately, the crates are quite heavy and you end up having to stick with small ones to keep up with Marble's pace. She doesn't appear to mind, just enjoying having someone to talk to while she works, even if it doesn't save her many trips.\n\n");
        }
        //[player str >=20, <50]
        if (player.str >= 20 && player.str < 50)
        {
            outputText("You try your best, but for every crate you carry, Marble caries three. She doesn't mind though, since you'll end up saving her a quarter of the trips she would have had to make.\n\n");
        }
        //[player str >=50, <80]
        if (player.str >= 50 && player.str < 80)
        {
            outputText("You put your back into the job and manage to match Marble in her efforts. She is really impressed with your strength, and together you can cut the number of trips needed in half.\n\n");
        }
        //[player str >=80]
        if (player.str >= 80)
        {
            outputText("Marble may be strong, but you are stronger. She is amazed as you manage to take even more crates at a time than she can, only held back by the number you can balance. Thanks to your efforts, the chore only takes a third of the number of trips it normally would have taken.\n\n");
        }
        ...
        marbleStatusChange(as3hx.Compat.parseInt(player.str / 10), 0);
    ...
    public function helpMarble2() : Void
    {
        ...
        if (player.spe < 20)
        {
            outputText("Even though Marble often stops to munch on a weed, she is still able to get more weeds than you do. Despite her size, she can move surprisingly fast. Regardless, she enjoys simply having you there while she works, and you get to enjoy the view.\n\n");
        }
        //[player spd >=20, <50]
        if (player.spe >= 20 && player.spe < 50)
        {
            outputText("You put in a good effort at cleaning out the weeds, and Marble often gives you a good look at her rear when she finds a tasty looking weed.\n\n");
        }
        //[player spd >=50, <80]
        if (player.spe >= 50 && player.spe < 80)
        {
            outputText("Moving quickly through the fields, you surprise Marble with your speed so much that she jokingly pouts that you're getting to all the tasty weeds before she has a chance to eat them. You still end up getting a few good views of her ass.\n\n");
        }
        //[player spd >=80]
        if (player.spe >= 80)
        {
            outputText("Weeding the field is a breeze for you, going fast enough that you're able to bring weeds to Marble faster than she can eat them. In the end, you do almost all the work yourself. She does reward you with a good view for your efforts.\n\n");
        }
        //(increase player spd)
        dynStats("spe", 1.5);
        //(increase affection by one tenth the player's spd)
        marbleStatusChange(as3hx.Compat.parseInt(player.spe / 10), 0);
        //(increase player lust)
        dynStats("lus", 10, "scale", false);
        if (afterMarbleHelp())
        {
    ...
    private function afterMarbleHelp() : Bool
    {
        marbleSprite();
    ...
    private function marbleAddiction(newPage : Bool) : Void
    {
        marbleSprite();
        //[start a new page]
        if (newPage)
    ...
    public function addictedEncounterHappy(clearS : Bool = true) : Void
    {
        marbleSprite();
        if (clearS)
        {
    ...
    private function playerAgreesToDrinkMarbleMilkBottled() : Void
    {
        ...
        {
            player.createStatusEffect(StatusEffects.BottledMilk, 8, 0, 0, 0);
        }
        player.slimeFeed();
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    public function encounterMarbleAshamedAddiction(clearS : Bool = true) : Void
    {
        if (clearS)
        {
            clearOutput();
    ...
    private function AshamedAddictionComfort() : Void
    {
        ...
        if (player.tallness < 60)
        {
            outputText("She pulls you into her chest and you feel relieved to see the Marble you know is still in there. You feel warm drops of water fall on your head and look up to find Marble crying fresh tears, but this time with a big smile on her face.\n\n");
        }
        //[player height greater than or equal to 5 feet]
        else
        {
            outputText("You hear her breath start to come in short breaths and look at her face to find Marble crying fresh tears, but this time with a big smile on her face.\n\n");
        }
    ...
    private function refuseMarblesMilkAddiction() : Void
    {
        ...
        if (player.tallness < 60)
        {
            outputText("hugs you to her chest, ");
        }
        else
        {
            outputText("gives you a big hug, ");
        }
    ...
    //--- First Conversation ---
    {
        ...
        var _sw26_ = (flags[kFLAGS.MURBLE_FARM_TALK_LEVELS]);        
        ...
        switch (_sw26_)
        {
            ...
            case 0, 1:
                ...
                {case 0:
                        ...
                        if (!player.isRace(Races.HUMAN, 1, false))
                        {
                            ...
                            if (player.tallness < 60)
                            {
                                outputText("  Her eyes light up. \"<i>Are they all as cute as you?</i>\"");
                            }
                            //[if PC is taller then 6'6" feet]
                            else if (player.tallness > 78)
                            {
                                outputText("  \"<i>Are most of you this tall?</i>\"");
                            }
                        }
                        ...
                        //[if PC is a dogmorph]
                        else if (player.isRace(Races.DOG, 1, false))
                        {
                            outputText("  \"<i>I've seen lots of dog-morphs before, are you from Barkersvile?</i>\"");
                        }
                        //[if PC is a centaur]
                        else if (player.isRace(Races.CENTAUR, 1, false))
                        {
                            outputText("  \"<i>I've seen a few centaurs before, but they don't seem to have regular homes.  They're nomads, wandering the plains.  Are you the same?</i>\"");
                        }
                        //[if PC is not human, cow-girl/cowboi, dogmorph, or centaur]
                        else
                        {
                            outputText("  \"<i>It's very rare that we get a [race] here.  Are you from around these parts?</i>\"");
                        }
                ...
                {
                    outputText("You chuckle and say that you hardly think of them with all the other fun things to think about.  Then you pause and say that it is in the moments that you do think about them that keep you from forgetting who you once were.");
                }
            ...
            case 3:
                ...
                if (player.tallness < 60)
                {
                    outputText("  \"<i>Sorry, sorry, it's just that you're so cute!  I can't picture you as some great hero leaving behind home and family for a mission.</i>\"  She wipes a few tears from her face.");
                }
                //[if PC is over 7 feet tall]
                else if (player.tallness > 84)
                {
                    outputText("  \"<i>I'm sure a giant like you was the perfect choice for this mission.</i>\"");
                }
                ...
                if (player.statusEffectv1(StatusEffects.Marble) >= 60)
                {
                    outputText("  \"<i>Even though the start of the trip didn't turn out all that well, I think I found what I was looking for.</i>\"");
                }
            ...
            case 4:
                ...
                if (player.statusEffectv3(StatusEffects.Marble) == 2 || player.hasPerk(PerkLib.MarbleResistant))
                {
                ...
                else if (player.statusEffectv3(StatusEffects.Marble) == 1 || player.hasPerk(PerkLib.MarblesMilk))
                {
            ...
            case 5:
                ...
                if (player.hasPerk(PerkLib.HistorySlut) || player.hasPerk(PerkLib.HistoryWhore) || player.hasPerk(PerkLib.PastLifeSlut) || player.hasPerk(PerkLib.PastLifeWhore))
                {
                    outputText("Well, you certainly gave an arousing description.  I think I can share something in return.");
                }
                else if (player.cor < 33)
                {
                    outputText("Sweetie, after you told me such a nice story, of course I will.");
                }
                else if (player.cor < 66)
                {
                    outputText("Sweetie, I can tell that you care a great deal about what happened, even if you try to hide it. Of course, I'll share.");
                }
                else
                {
                    outputText("Well, you certainly gave an arousing description.  I think I can share something in return.");
            ...
            default:
                ...
                if (player.str < 90)
                {
                    outputText("  In fact, I don't think there's anyone out there who is stronger than me.");
                }
                else
                {
                    outputText("  You're probably the only other person I've ever met who is stronger than me.");
                }
                outputText("</i>\"");
                ...
                if (!player.hasStatusEffect(StatusEffects.CampMarble))
                {
                    outputText(", except that mission of yours that's so important to you");
                }
                outputText(".</i>\"");
                ...
                if (!player.hasStatusEffect(StatusEffects.CampMarble))
                {
                    ...
                    if (player.statusEffectv3(StatusEffects.Marble) == 2)
                    {
                        outputText("  I really just wanted someone who loved me and loved nursing from my breast.  I had no idea what that would do to you.");
                    }
                    //[if addiction quest is active, and Marble is happy with her milk]
                    else if (player.statusEffectv3(StatusEffects.Marble) == 1)
                    {
                        outputText("  But I think that I've found the one who will change that.  Right sweetie?");
                    }
                    //[if addiction quest has not yet triggered]
                    else
                    {
                        outputText("  Though, I'm not giving up hope yet.");
                    }
                    outputText("</i>\"");
                }
                else
                {
                    outputText("  Until I met you, I was actually really lonely on the inside.</i>\"");
                }
    ...
    public function postAddictionFarmHelpings() : Void
    {
        ...
        if (player.hasPerk(PerkLib.MarbleResistant))
        {
            outputText("She assures you that you can't get addicted to it again if you don't drink her milk directly from her breasts.");
        }
    ...
    public function postAddictionCampMornings(extra : Bool = true) : Void
    {
        marbleSprite();
        //(if player is completely addicted, do this event at the start of every day)
        ...
        if (!player.hasItem(consumables.M__MILK, 5))
        {
            ...
            if (player.hasPerk(PerkLib.MarbleResistant))
            {
                outputText("She assures you that you'll be fine as long as you don't drink directly from her breasts.");
            }
    ...
    public function standardSex() : Void
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (player.biggestTitSize() >= 2)
                {
                    outputText("running one of your hands down them before continuing.");
                }
                //[player has small or no breasts]
                else
                {
                    outputText("running one of your hands over them before continuing on.");
                }
    ...
    public function marbleSex2Continued(genders : Int) : Void
    {
        ...
        if (genders == 1)
        {
            ...
            {
                outputText("You easily slide all the way into her depths before beginning your hungry thrusts.  ");
            }
            //[player is more than 8 inches in length]
            else
            {
                outputText("She is unable to take all of you, but you're hardly discouraged as you begin your hungry thrusts.  ");
            }
            outputText("She gasps slightly and her arms clamp down on you, ");
            //[player is between 4 and 5 feet in height]
            if (player.tallness < 60 && player.tallness >= 48)
            {
                outputText("keeping your head tightly locked between her breasts.  ");
            }
            //[player is not between 4 and 5 feet in height]
            else
            {
                outputText("keeping your body tightly locked against her.  ");
            }
            ...
            if (player.cocks[0].cockLength < 8 && player.hasKnot(0))
            {
                outputText(", and with a pop, your knot slips inside of her");
            }
        ...
        else
        {
            ...
            if (player.tallness < 60 && player.tallness >= 48)
            {
                outputText("squeezes your head into her breasts.  ");
            }
            //[player is not between 4 and 5 feet in height]
            else
            {
                outputText("pulls you securely against her.  ");
            }
            ...
            if (player.cockTotal() > 0)
            {
                outputText(".  Your [cocks] erupts while clamped between your bodies");
            }
    ...
    public function marbleMilkSex() : Void
    {
        ...
        //[player has only human or animal dicks]
        {
            ...
            {
                outputText("\"<i>So, which one is your favorite?</i>\"  Marble asks you while taking measure of your [cocks].  Before you have a chance to answer, Marble grabs a hold of your central [cock] and says \"<i>I'll bet it's this one.</i>\"  ");
            }
            ...
            if (player.cocks[0].cockLength < 8)
            {
                outputText("every bit of your [cock] is deep inside.  ");
            }
            // //[dick(0) is more than 8 inches long]
            else
            {
                outputText("Marble bottoms out at eight inches and you can go no further.  ");
            }
        ...
        //[player has sizable breast, C or bigger]
        {
            ...
            {
                outputText(" with each of your " + allBreastsDescript());
            }
            else
            {
                outputText(" with each of your " + breastDescript(0));
            }
            ...
            if (player.hasFuckableNipples())
            {
                ...
                {
                    outputText("  The whole while, Marble continues to gulp down your milk.");
                }
            }
            //[if player is lactating]
            else if (player.biggestLactation() > 1)
            {
                outputText("Her suckling soon brings a stream of milk to her lips that she gulps down eagerly.  You give a contented sigh, but are soon brought out of your revelry as her sucking becomes more stimulating and intense.");
            }
            //[player is neither lactating or has nipplecunts]
            if (player.biggestLactation() <= 1 && !player.hasFuckableNipples())
            {
                outputText("Of course, since you aren't lactating, nothing but your " + nippleDescript(0) + " reaches her lips.  Marble doesn't seem to be put off by this and still gives you a thoroughly enjoyable experience.");
            }
            outputText("\n\n");
    ...
    public function marbleAddictionSex() : Void
    {
        ...
        {
            outputText("Marble bends over and kisses you deeply on the lips, a kiss that you eagerly return.  She lifts you up into the air, her lips still locked on yours, and holds you tightly against her body. A full minute passes before she lowers you down so your bodies can be joined.\n\n");
        }
        //[player is between 5.5 feet and 6.5 feet in height]
        else if (player.tallness < 78)
        {
            outputText("Marble lowers her head and locks her lips with yours, a show of passion that you eagerly return.  The two of you stay like that for a while, time seeming to standing still.  Then Marble breaks the kiss and looks deeply into your eyes as a sly grin spreads across her face.\n\n");
        }
        //[player is over 6.5 feet in height]
        else
        {
            outputText("Marble grabs a hold of your head and pulls your mouth to hers for a passionate kiss.  You are only too eager to comply and join in.  Even without her vice grip on your head, it feels like you couldn't possibly leave her wonderful mouth. But you eventually push back and look into her eyes intensely.\n\n");
        }
        sceneHunter.selectGender(dickF, vagF);
        ...
        function dickF() : Void
        {
            ...
            if (player.tallness >= 48 && player.tallness <= 60)
            {
                outputText(", the breasts your head is clamped between,");
            }
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            if (player.tallness >= 48 && player.tallness <= 60)
            {
                outputText(" and the breasts your head is clamped between");
            }
    ...
    public function marbleStatusChange(affection : Float, addiction : Float, isAddicted : Float = -1) : Void
    {
        ...
        if (isAddicted != -1)
        {
            player.changeStatusValue(StatusEffects.Marble, 3, isAddicted);
        }
    ...
    private function applyMarblesMilk() : Void
    {
        ...
        if (player.statusEffectv1(StatusEffects.MarblesMilk) > 36)
        {
            player.changeStatusValue(StatusEffects.MarblesMilk, 1, 36);
        }
    ...
    public function interactWithMarbleAtCamp() : Void
    {
        marbleSprite();
        clearOutput();
        if (flags[kFLAGS.MARBLE_PURIFICATION_STAGE] != 2)
        {
            outputText("How will you interact with Marble?");
        }
        else
        {
        ...
        addButton(2, "Present", marbleGathered).disableIf(!player.hasStatusEffect(StatusEffects.MarbleHasItem), "Marble doesn't have anything for you yet.");
        addButton(3, "Give Item", giveItem);
        addButton(4, "Get Milk", gotMilk);
        addButton(5, "Release", marbleCampSexNew).hint("Get with marble for a quick cuddle and some sex.").disableIf(player.lust < 33, "Not aroused enough!");
        addButton(6, "Playtime", marbleKidsPlaytime).disableIf(flags[kFLAGS.MARBLE_KIDS] == 0, "You and Marble don't have kids yet.", "???");
        ...
        {
            addButton(9, "Sleep With", marbleSleepToggle);
        }
        else
        {
            addButton(9, "Sleep Alone", marbleSleepToggle);
        }
    ...
    private function marbleTalkOverhaul() : Void
    {
        ...
        {
            addButtonDisabled(2, "???", "Visit her later... maybe when she's horny enough and not milked for some time?");
        }
    ...
    private function marbleKidsPlaytime() : Void
    {
        ...
        if (flags[kFLAGS.KID_SITTER] == 2 && flags[kFLAGS.ANEMONE_KID] > 0)
        {
            select = 6;
        }
        //Bovine Saviour (Z)
        ...
        if (select == 2)
        {
            ...
            if (player.cor < 66)
            {
                outputText("  You do make sure to skip over the items of a more explicit nature throughout.");
            }
        ...
        if (select == 4)
        {
            ...
            if (flags[kFLAGS.MARBLE_KIDS] == 2)
            {
                outputText("both");
            }
            else
            {
                outputText("about half of your children");
            }
        ...
        if (select == 5)
        {
            ...
            if (player.tallness < 60)
            {
                outputText("grabs you and immediately lifts you up to her chest, giving you a big hug.");
            }
            //[Height above 5 feet]
            else
            {
                outputText("embraces you strongly, pressing her warm body against yours.");
            }
        ...
        if (flags[kFLAGS.MARBLE_PLAYED_WITH_KIDS_TODAY] > 0)
        {
            dynStats("lus", -10, "scale", false);
        }
        else
        {
            dynStats("lib", -1, "lus", -10, "cor", -1);
        }
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    private function definitelyBreakUpWithWithMarble() : Void
    {
        ...
        if (flags[kFLAGS.MARBLE_KIDS] == 1)
        {
            outputText(" and her child");
        }
        else if (flags[kFLAGS.MARBLE_KIDS] > 1)
        {
            outputText(" and her children");
        }
	...
	otherwise leave it blank*/
        ...
        {
            outputText("You have no new stories to share with Marble, so you chat for a bit about inconsequential things.");
        }
        ...
        //check if the player is far too weak to go to the factory{
            ...
            if (player.level < 3)
            {
                outputText("\"<i>I think we should help out Marae and shut down that factory she mentioned was in the mountains, but I don't think you're ready to go into the Mountains yet. They can be brutal - get a little more practice, and make sure you've got a good weapon.</i>\"");
            }
            else
            {
                outputText("\"<i>I think we should help out Marae and shut down that factory she mentioned was in the mountains. I have no idea what will happen in there, though, so make sure you're as ready as you can be before you go.</i>\"\n\n");
            }
        }
        ...
        {
            outputText("\"<i>Sweetie, you've finished the main story of the game!</i>\" she says to you winking. \"<i>Why not explore? There's a lot in here that isn't a part of the main story.  You could also try changing your form and see how that changes each encounter.  Just don't get caught or lose your head, ok sweetie?</i>\"");
        }
    ...
    private function marbleInfo() : Void
    {
        ...
        else if (flags[kFLAGS.MARBLE_PURIFIED] == 0)
        {
            outputText("So long as you don't drink milk from Marble's breasts again, you don't have to worry about getting addicted.  ");
        }
        else
        {
            outputText("Now that she's been purified, you can drink as much of her milk as you like without fear.  ");
        }
    ...
    public function giveItem() : Void
    {
        ...
        else if (player.hasPerk(PerkLib.MarblesMilk))
        {
            addItemButton(0, consumables.LACTAID, giveMarbleLactaid);
        }
    ...
    public function giveMarbleLactaid() : Void
    {
        ...
        if (player.tallness < 60)
        {
            outputText("lifts you up to her chest before saying breathlessly saying \"<i>Drink now!</i>\"  You waste no time obliging her.\n\n");
        }
        else if (player.tallness > 72)
        {
            outputText("breathlessly says\"<i>Drink now!</i>\"  You waste no time and descend upon her breasts.\n\n");
        }
        else
        {
            outputText("breathlessly says, \"<i>Drink now!</i>\"  You quickly move forward to get a drink.\n\n");
        }
...
*/
    private function marbleCampSexNew() : Void
    {
        ...
        if (!player.isNaga())
        {
            ...
            menu();
            addButton(0, "Feeding (M)", feedingF, true).disableIf(!player.hasCock(), "Req. a cock.").disableIf(!player.hasPerk(PerkLib.MarblesMilk), "You're not addicted to do that!");
            addButton(0, "Feeding (F)", feedingF, false).disableIf(!player.hasVagina(), "Req. a vagina.").disableIf(!player.hasPerk(PerkLib.MarblesMilk), "You're not addicted to do that!");
            addButton(2, "Oral (M)", oralF, true).disableIf(!player.hasCock(), "Req. a cock.");
            addButton(3, "Oral (F)", oralF, false).disableIf(!player.hasVagina(), "Req. a vagina.");
            addButton(4, "Breasts", breastsF);
            
            //=====================================
            //Feeding
            function feedingF(cock : Bool) : Void
            {
                clearOutput();
                ...
                if (player.tallness < 66)
                {
                    outputText("right breast.");
                }
                else
                {
                    outputText("left breast.");
                }
                if (player.tallness > 90)
                {
                    outputText("  You do need to bend over in order to access her breast as well as allow her access to your groin, but while slightly uncomfortable, your plan is worth the effort.");
                }
                ...
                else
                {
                    ...
                    if (player.wetness() < 2)
                    {
                        outputText("\"<i>Oh, sweetie, this just won't do!  It's like you're not looking forward to this at all!</i>\"");
                    }
                    else if (player.wetness() < 4)
                    {
                        outputText("\"<i>It seems you're quite in need of something down here, sweetie...</i>\"");
                    }
                    else if (player.wetness() < 5)
                    {
                        outputText("\"<i>My, my, sweetie; this is so wet... are you sure we should stimulate it more?</i>\"");
                    }
                    else
                    {
                        outputText("\"<i>Oh my, sweetie; it seems like you need to be toweled off...</i>\"");
                    }
                }
                ...
                if (cock)
                {
                    ...
                    if (flags[kFLAGS.MARBLE_DICK_LENGTH] == 0)
                    {
                        outputText("  Her thighs seem to be rubbing together, and the place you're lying on feels a tiny bit wet, so you assume it isn't only you who's getting more and more excited by this.");
                    }
                    else
                    {
                        outputText("  Her thighs are gently rubbing beneath you, and you feel something hard poking you in the back.  You smile around the nipple you have in your mouth.");
                    }
                    if (player.hasBalls())
                    {
                        outputText("  As her hand reaches the base of your cock, she stops stroking it to take a moment and gently roll your [balls] with her fingers.  \"<i>Are these ready to shoot out for me soon, sweetie?</i>\"");
                    }
                    if (player.hasVagina())
                    {
                        outputText("  From the base of your cock, her hand moves even lower, gently rubbing your " + clitDescript() + ".  \"<i>This seems to also require some attention...  Maybe later, sweetie, if you do a good job nursing from me.  But right now, I want to see you shoot off,</i>\" she says, as her hand moves back to your [cocks].");
                    }
                    ...
                    if (player.cumQ() < 500)
                    {
                        outputText("  You cover her hand as she keeps jerking you off, squeezing out tiny little droplets once you're done ejaculating.");
                    }
                    else if (player.cumQ() < 1000)
                    {
                        outputText("  Your load is as large as always, and ropes of cum fly into the air, covering her arm and your abdomen... and you swear one string even lands of her right breast along the way.  \"<i>That's it, sweetie, shoot all you have.</i>\"");
                    }
                    else
                    {
                        outputText("  You pretty much cover yourself, Marble, and the surrounding ground with your enormous white load of cum.  She seems numbed from the sheer output you're capable of.  \"<i>Uh... sweetie...</i>\"");
                    }
                    outputText("\n\n\"<i>Well... that was fun.</i>\"  Marble slowly pulls you away from her breast");
                    if (player.cumQ() >= 1000)
                    {
                        outputText(", then wipes the nipple clean on the other");
                    }
                ...
                else
                {
                    ...
                    if (player.wetness() >= 4)
                    {
                        outputText("  Messy as it is, Marble never stops, wanting to make the orgasm as intense as possible.");
                    }
                    //([Squirter]
                    if (player.wetness() > 4)
                    {
                        outputText("  Soon, you're shooting your femcum all over her arm, and she giggles at the intensity and visibility of your orgasm.");
                    }
            ...
            function oralF(cock : Bool) : Void
            {
                ...
                if (cock)
                {
                    outputText("a tent growing in your " + player.armorName);
                }
                else
                {
                    outputText("a moistness growing under your " + player.armorName);
                }
                ...
                if (!cock)
                {
                    outputText("probing her tongue in your " + vaginaDescript(0) + ", licking you in the most appropriate places");
                }
                else
                {
                    outputText("wrapping her tongue around your rod of dickflesh, squeezing and pumping beads of pre-cum up your shaft");
                }
                ...
                if (!cock)
                {
                    outputText("poking your cunt");
                }
                else
                {
                    outputText("suckling the juices out of your urethra and literally absorbing the drops of pre-cum as they spurt out of the tip of your junk.");
                }
                //[if multicock]
                if (cock && player.cockTotal() > 1)
                {
                    outputText("  She alternatively sucks and jerks every cock you have, making sure there isn't a single meat-tower that isn't being taken care of.");
                }
                if (player.hasBalls())
                {
                    outputText("  Her hands softly rub your [balls], making them churn and swell in pure arousal.");
                }
                ...
                if (!cock)
                {
                    outputText("tongue-fuck");
                }
                else
                {
                    outputText("deepthroat");
                }
                ...
                if (cock)
                {
                    ...
                    if (player.cockTotal() > 1)
                    {
                        outputText(" while she strokes another");
                    }
                ...
                {
                    outputText("  Her flexible, yet firm tongue really feels like a dong and it keeps provoking you with its prolonged digging. With a mighty thrust, you shove her against your " + vaginaDescript() + ", forcing the entirety of her oral muscle inside.");
                }
                ...
                if (cock)
                {
                    ...
                    if (player.cumQ() >= 500)
                    {
                        outputText("  Your [cock] keeps spouting more seed inside her, efficiently distending her belly");
                    }
                    //[if cum production is massive]until she looks ready to give birth.[/]
                    if (player.cumQ() > 1500)
                    {
                        outputText(" until she looks ready to give birth.");
                    }
            ...
            function breastsF() : Void
            {
                ...
                //Select one based on overall cum production{
                    ...
                    if (player.cumQ() < 25)
                    {
                        outputText("  In moments, a small amount of your cum is covering Marble's breasts.");
                    }
                    else if (player.cumQ() < 250)
                    {
                        outputText("  After a few moments, a fair amount of your cum is covering Marble's breasts and face.");
                    }
                    else if (player.cumQ() < 500)
                    {
                        outputText("  You keep cumming and cumming, until eventually Marble is completely drenched in your cum.");
                    }
                    else
                    {
                        outputText("  Your seemingly endless orgasm eventually results in the two of you becoming completely drenched in your cum, as well as a large part of the ground around you.");
                    }
                }
                ...
                //Select one based on vagina wetness, similarly to the cum production, you should do the logic for this part{
                    ...
                    {
                        outputText("  As you recover, you see that a small amount of your girly fluids has leaked onto Marble's breasts.");
                    }
                    else if (player.vaginas[0].vaginalWetness <= VaginaClass.WETNESS_SLICK)
                    {
                        outputText("  As you recover, you see that Marble has been covered fairly liberally with your girly fluids.");
                    }
                    else if (player.vaginas[0].vaginalWetness < 5)
                    {
                        outputText("  As you recover, you see that Marble is decently drenched in your girly fluids.");
                    }
                    else
                    {
                        outputText("  It takes you a minute to recover, but upon doing so, you blush and realize just how soaked with fluid you've become.");
                    }
                }
                //finally some philosophy on what an orgasm would feel like for someone who is genderless
                if (player.gender == 0)
                {
                    outputText("  It's not quite the same as when you had genitals, but it is undeniably an orgasm.  While it is essentially a pointless orgasm, it's also almost pure in a way.  There is not a feeling inside you but the explosion of pleasure.");
                }
        ...
        else
        {
            ...
            if (player.lib >= 50)
            {
                outputText("  Quietly, you reach down and hook your fingers into the hem of her clothing, allowing her to pull it half off with her next shift and expose her pussy");
            }
            if (flags[kFLAGS.MARBLE_DICK_LENGTH] > 0)
            {
                outputText(" and " + marbleCock());
            }
            ...
            if (player.gender > 0)
            {
                ...
                if (player.hasCock())
                {
                    outputText("grabs your shaft");
                }
                else
                {
                    outputText("penetrates your pussy");
                }
                ...
                if (player.hasCock())
                {
                    outputText("  [EachCock] unleashes a load of semen right into her mouth, which she swallows with pink-faced, wanton gusto.");
                }
                if (player.hasVagina())
                {
                    outputText("  Your [pussy] runs and squirts as you moan, drooling your juices into the little valley where Marble's breasts press against your scales.");
                }
            }
            else
            {
    ...
    private function marbleNips() : String
    {
        ...
        if (player.statusEffectv2(StatusEffects.MarbleSpecials) == 0)
        {
            player.changeStatusValue(StatusEffects.MarbleSpecials, 2, 1);
        }
        if (player.statusEffectv2(StatusEffects.MarbleSpecials) == 1)
        {
            return "nipples";
        }
        if (player.statusEffectv2(StatusEffects.MarbleSpecials) == 4)
        {
            return "quad-nipples";
        }
    ...
    private function marbleCock() : String
    {
        ...
        if (as3hx.Compat.parseInt(Math.random() * 3) == 0)
        {
            if (flags[kFLAGS.MARBLE_DICK_LENGTH] < 3)
            {
                descript = "squat ";
            }
            if (flags[kFLAGS.MARBLE_DICK_LENGTH] >= 3 && flags[kFLAGS.MARBLE_DICK_LENGTH] < 5)
            {
                descript = "short ";
            }
            if (flags[kFLAGS.MARBLE_DICK_LENGTH] >= 5 && flags[kFLAGS.MARBLE_DICK_LENGTH] < 7)
            {
                descript = "average ";
            }
            if (flags[kFLAGS.MARBLE_DICK_LENGTH] >= 7 && flags[kFLAGS.MARBLE_DICK_LENGTH] < 9)
            {
                descript = "long ";
            }
            if (flags[kFLAGS.MARBLE_DICK_LENGTH] >= 10 && flags[kFLAGS.MARBLE_DICK_LENGTH] < 13)
            {
                descript = "huge ";
            }
            if (flags[kFLAGS.MARBLE_DICK_LENGTH] >= 13 && flags[kFLAGS.MARBLE_DICK_LENGTH] < 18)
            {
                descript = "massive ";
            }
            if (flags[kFLAGS.MARBLE_DICK_LENGTH] >= 18 && flags[kFLAGS.MARBLE_DICK_LENGTH] < 30)
            {
                descript = "enormous ";
            }
            if (flags[kFLAGS.MARBLE_DICK_LENGTH] >= 30)
            {
                descript = "towering ";
            }
        }
        ...
        //narrow, thin, ample, broad, distended, voluminous{
            ...
            if (flags[kFLAGS.MARBLE_DICK_THICKNESS] <= .75)
            {
                descript += "narrow ";
            }
            if (flags[kFLAGS.MARBLE_DICK_THICKNESS] > 1 && flags[kFLAGS.MARBLE_DICK_THICKNESS] <= 1.4)
            {
                descript += "ample ";
            }
            if (flags[kFLAGS.MARBLE_DICK_THICKNESS] > 1.4 && flags[kFLAGS.MARBLE_DICK_THICKNESS] <= 2)
            {
                descript += "broad ";
            }
            if (flags[kFLAGS.MARBLE_DICK_THICKNESS] > 2 && flags[kFLAGS.MARBLE_DICK_THICKNESS] <= 3.5)
            {
                descript += "fat ";
            }
            if (flags[kFLAGS.MARBLE_DICK_THICKNESS] > 3.5)
            {
                descript += "distended ";
            }
        }
        rando = as3hx.Compat.parseInt(Math.random() * 10);
        if (rando >= 0 && rando <= 4)
        {
            descript += "cock";
        }
    ...
    //Marble Rape scene
    private function rapeMarble(room : Bool = false) : Void
    {
        marbleSprite();
        ...
        var raped : Bool = false;  //records whether or not the player was successful in raping Marble  
        clearOutput();
        if (room)
        {
        ...
        {
            outputText("You decide that rather than helping her, you are going to roughly manhandle her breasts and rape her.");
        }
        ...
        {
            sceneHunter.selectGender(dickF, vagF, ["Masturb.Her", mastF]);
        }
        ...
        var mastF : Void->Void = function() : Void
        {
            if (player.isGenderless())
            {
                outputText("Your lack of genitals makes it difficult to actually rape Marble, but there are other things you can do.  ");
            }
        ...
        var goodEnd : Void->Void = function() : Void
        {
            if (room)
            {
    ...
    //Normal shitz
    {
        ...
        //Gives birth at 28 days{
            ...
            else
            {
                ...
                if (flags[kFLAGS.MARBLE_DICK_TYPE] > 0)
                {
                    outputText(", but you notice that she does not have a cock of any kind. It seems that trait isn't passed on");
                }
    ...
    public function marbleNightSleepFlavor() : Bool
    {
        marbleSprite();
        ...
        switch (pregnancy.eventTriggered())
        {
            case 2, 3, 4:
                ...
                {case 3:outputText("<b>Marble's belly has gotten a fair bit bigger; she may be pregnant.</b>\n\n");  //8 days in  
                        break;
                }outputText("<b>Marble's belly has gotten bigger; she is obviously pregnant.");  //12 days in  
            ...
            case 5, 6:
                ...
                {case 5:outputText("<b>Marble's pregnancy has advanced further still, though the structure of her body keeps it from slowing her down.</b>\n\n");  //18 days in  
                        break;
                }outputText("<b>Marble is probably getting close to giving birth, as her belly has gotten very large.</b>\n\n");
        }
        ...
        else if (flags[kFLAGS.MARBLE_LUST] < 50 || player.gender == 0)
        {
            ...
            if (flags[kFLAGS.MARBLE_LUST] < 0)
            {
                outputText("She makes a point of turning to face away from you when you climb into your bedroll.  Looks like she's still annoyed at you.");
            }
            else if (flags[kFLAGS.MARBLE_LUST] < 20)
            {
                outputText("She lies next to you contentedly when you climb into your bedroll.");
            }
            else if (flags[kFLAGS.MARBLE_LUST] < 30)
            {
                outputText("Marble gives a soft sigh when you climb into your bedroll.");
            }
            else if (flags[kFLAGS.MARBLE_LUST] < 40)
            {
                outputText("Marble fidgets around a bit when you climb into your bedroll.");
            }
            else
            {
                outputText("Marble seems rather worked up when you climb into your bedroll.");
            }
            if (player.lust < 33 && flags[kFLAGS.MARBLE_LUST] < 20)
            {
                outputText("  After a few moments of getting comfortable, you drift off to sleep.");
            }
            else if (player.lust < 33)
            {
                ...
                {
                    outputText("  However, you're not aroused enough to consider sex.");
                }
            }
            else
            {
        ...
        else
        {
            //Replaces laying down texts when Marble is at or over 50 lust{
                ...
                if (player.lust < 33)
                {
                    ...
                    addButton(2, "Tailjob", giveMarbleTailjobRelease).disableIf(!player.isNaga(), "Requires a naga tail.").disableIf(flags[kFLAGS.MARBLE_DICK_TYPE] == 0, "Maybe if Marble had a dick?", "???");
                }
                else
                {
                    ...
                    addButton(3, "Tailjob", giveMarbleTailjobRelease).disableIf(!player.isNaga(), "Requires a naga tail.").disableIf(flags[kFLAGS.MARBLE_DICK_TYPE] == 0, "Maybe if Marble had a dick?", "???");
                }
                return true;
            }
        }
    ...
    private function pcPregWithMarblesKids() : Bool
    {
    ...
    private function marblePregWithPCKids() : Bool
    {
    ...
    private function marbleCuddlin() : Void
    {
        ...
        //if (PC is 6.5 feet or less tall){
            ...
            {
                outputText("pulls your head on top of her chest.  She idly strokes your " + hairDescript() + " and says, \"<i>Sweetie, how about I be your pillow for the night?</i>\"  You happily agree, and soon you drift off to the sound of her steady breathing while you lay between her sizable breasts.");
            }
            else
            {
                outputText("wraps her arms around you, gently pulling you against her.  \"<i>Sweetie, do you mind if we stay like this for a while?</i>\"  There is no way you could refuse, and before long, the two of you drift off to sleep in each other's arms.");
            }
        }
        //Preg-cuddles
        else
    ...
    //SEKCS
    private function marbleNightSexIntro(clear : Bool = true) : Void
    {
        if (clear)
        {
            clearOutput();
        ...
        var x : Float = player.cockThatFits(marbleCuntCapacity());
        if (x < 0)
        {
            x = 0;
        }
    ...
    private function marbleNightSexDudes() : Void
    {
        ...
        else
        {
            
            {
                ...
                if (player.biggestTitSize() >= 2)
                {
                    outputText("You pull your hands back up and begin to run them over your " + biggestBreastSizeDescript() + ", toying and teasing your nipples as your " + cockDescript(x) + " continues to pound away at Marble's confines.  Marble soon has her hands on the breasts you vacated, clearly enjoying teasing herself just as much as you are.  The two of you cry out in pleasure from the stimulation; you can feel your orgasm fast approaching.\n\n");
                }
                else
                {
    ...
    private function marblePreggoChance(preggerMult : Float) : Void
    {
        var preggerOdds : Float = 10;
        //Count cum quantity
        ...
        if (player.hasPerk(PerkLib.FertilityPlus))
        {
            preggerOdds += 5;
        }
        if (player.hasPerk(PerkLib.FertilityMinus) && player.lib < 25)
        {
            preggerOdds -= 5;
        }
    ...
    public function marbleOvulatesLikeMadDawg() : Void
    {
        ...
        else if (pregnancy.isPregnant)
        {
            ...
            var newIncubation : Int = as3hx.Compat.parseInt(pregnancy.incubation - (40 + rand(30)));
            if (newIncubation <= 0)
            {
                newIncubation = 1;
            }
...
*/
    ...
    public function marbleCuntCapacity() : Float
    {
        var size : Float = 36;
    ...
    private function marbleAppearance() : Void
    {
        ...
        else if (flags[kFLAGS.MARBLE_BOVA_LEVEL] == 1)
        {
            outputText("6 foot 8 ");
        }
        else
        ...
        if (flags[kFLAGS.MARBLE_BOVA_LEVEL] <= 1)
        {
            outputText("She has a fairly normal soft human face, with light-colored skin.  ");
        }
        else
        {
            outputText("She has a soft face that is a mix of bovine and human features, with thick brown fur covering her body.  ");
        }
        ...
        {
            outputText("Two small horns grow from her forehead, similar in size and appearance to those on a young female bovine.  ");
        }
        else if (player.statusEffectv4(StatusEffects.Marble) <= 50)
        {
            outputText("Two medium-sized horns grow from her forehead, similar in size and appearance to those on a female bovine.  ");
        }
        else
        {
            outputText("Two fairly large horns grow from her forehead, similar in appearance to those on a female bovine.  ");
        }
        ...
        if (flags[kFLAGS.MARBLE_BOVA_LEVEL] <= 1)
        {
            outputText(", human until about half-way down her thigh.  The lower portion of her legs is covered in thick dark brown fur and ends in a pair of bestial hooves.\n\n");
        }
        else
        {
            outputText(" that are oddly jointed and end in a pair of bestial hooves.\n\n");
        }
        
        if (player.statusEffectv4(StatusEffects.Marble) <= 15)
        {
            outputText("She has two pillowy breasts, each supporting a 0.4-inch lactating " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + " bra.\n\n");
        }
        else if (player.statusEffectv4(StatusEffects.Marble) <= 25)
        {
            outputText("She has two large breasts, each supporting a 0.6-inch lactating " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + ".\n\n");
        }
        else if (player.statusEffectv4(StatusEffects.Marble) <= 35)
        {
            outputText("She has two basketball-sized breasts, each supporting a 0.8-inch milk-seeping " + marbleNip() + ".  She could easily fill a " + marbleBreastSize() + " bra.\n\n");
        }
        else
        {
            outputText("She has two basketball-sized breasts, each supporting a 1-inch milk-seeping " + marbleNip() + ".  She could easily fill an " + marbleBreastSize() + " bra.\n\n");
        }
        ...
        switch (_sw27_)
        {
            case 2, 3, 4, 5, 6:
                ...
                {case 5:outputText("Her belly is very swollen; she is very pregnant.\n\n");  //18 to 24 days in  
                        break;
                }outputText("Her belly is extremely swollen and occasionally quivers when whatever she is pregnant with moves around.\n\n");
        }
        ...
        if (flags[kFLAGS.MARBLE_DICK_TYPE] > 0)
        {
            outputText("She has grown a " + marbleCock() + " since you brought her to camp. It is " + num2Text(as3hx.Compat.parseInt(flags[kFLAGS.MARBLE_DICK_LENGTH])) + " inches long and " + num2Text(as3hx.Compat.parseInt(flags[kFLAGS.MARBLE_DICK_THICKNESS])) + " inches thick.\n\n");
        }
        
        if (player.statusEffectv4(StatusEffects.Marble) <= 50)
        {
            outputText("She has a pussy, with a 0.5 inch clit.\n\n");
        }
        else if (player.statusEffectv4(StatusEffects.Marble) <= 75)
        {
            outputText("She has a cunt, with a 0.6 inch clit.  You can see moisture gleaming from it.\n\n");
        }
        else
        {
            outputText("She has a fuck-hole, with a 0.7 inch clit.  Moisture gleams in her cunt, its lips slightly parted.\n\n");
        }
        //since I assume Marble may become ass fucked in the future, Boolean Marble.analVirgin:
        if (player.statusEffectv1(StatusEffects.MarbleSpecials) == 0)
        {
            outputText("She has one virgin bum-hole, placed between her squeezable butt-cheeks where, you presume, it belongs.");
        }
        else
        {
            outputText("She has one bum-hole, placed between her squeezable butt-cheeks where, you presume, it belongs.");
        }
        doNext(interactWithMarbleAtCamp);
    }
    ...
    private function giveMarbleTailjobRelease() : Void
    {
        ...
        if (player.cor > 70)
        {
            ...
            {
                outputText("  Your tongue runs along the length of the end of your tail, tasting both Marble's feminine secretions and her semen.  She gives you a smoldering gaze as you lick her juices up.  You grin at her.");
            }
            //(Demon Tongue)
            if (player.hasLongTongue())
            {
                outputText("  You decide to put on a show for Marble, moving your tail as you drop out a large piece of your inhumanly long tongue, licking up her secretions sensuously while staring at her.  She quickly blushes under your gaze.");
            }
        ...
        {
            sceneHunter.print("Check failed: need higher corruption.. and maybe long tongue?");
        }
        //teh end
    ...
    private function milkMarble() : Void
    {
        ...
        addButton(1, "Fuck Her", milkMarbleFuckDatCowPussy).disableIf(!player.isBiped() && !player.isNaga() && !player.isGoo(), "Possible only with bipedal, naga or goo lower body.").disableIf(player.cockThatFits(marbleCuntCapacity()) < 0, "Req. a cock fitting " + marbleCuntCapacity() + " area.");
        addButton(2, "Take Her", milkMarbleTakeHerDick).disableIf(!player.isBiped() && !player.isGoo(), "Possible only with bipedal or goo lower body.").disableIf(!player.hasVagina(), "Req. a vagina.").disableIf(flags[kFLAGS.MARBLE_DICK_TYPE] == 0, "Maybe if Marble had a dick?", "???");
    ...
    private function milkMarbleCunnilingling() : Void
    {
        ...
        if (player.isBiped())
        {
            outputText("get down on your knees, and slowly crawl forward");
        }
        else
        {
            outputText("lower your body close to the ground, and slowly inch forward");
        }
        ...
        if (flags[kFLAGS.MARBLE_DICK_TYPE] > 0)
        {
            outputText("With the first flick across its pink flesh, Marble grabs ahold of her " + marbleCock() + ", and tightly squeezes it, slowly pushing her hand down it.");
        }
        else
        {
            outputText("With the first flick across its pink flesh, Marble grabs onto an arm of her chair, gripping it tightly.");
        }
        ...
        if (flags[kFLAGS.MARBLE_DICK_TYPE] > 0)
        {
            outputText("Of course, there is also a spurt of white which sprays out over her lap and your head.  ");
        }
        ...
        var pLust : Int = as3hx.Compat.parseInt(20 + ((player.lib + player.cor) / 10));
        dynStats("lus", pLust);
        dynStats("sen", 1);
    ...
    private function milkMarbleFuckDatCowPussy() : Void
    {
        ...
        {
            outputText("[pg]\"<i>Hmm, yes, I think that will work.  Come closer so I can make a more thorough inspection.</i>\"");
        }
        ...
        function addictF() : Void
        {
            ...
            dynStats("lib", .3);
            if (player.cor < player.statusEffectv4(StatusEffects.Marble))
            {
                dynStats("cor", 1);
            }
            dynStats("tou", .1);
        ...
        var corruptF : Void->Void = function() : Void
        {
        ...
        var pureF : Void->Void = function() : Void
        {
    ...
    private function milkMarbleTakeHerDick() : Void
    {
        ...
        if (player.tallness < 56)
        {
            outputText(", grab a box,");
        }
        ...
        if (player.vaginalCapacity() < flags[kFLAGS.MARBLE_DICK_LENGTH] * flags[kFLAGS.MARBLE_DICK_THICKNESS])
        {
            outputText("With each new bit of length inserted, she waits a few moments for you to adjust to her size, letting you fill out to accommodate her.  ");
        }
        ...
        if (player.butt.type <= 4 && !player.isGoo())
        {
            outputText("taut");
        }
        else
    ...
    private function milkMarbleOnTheBar() : Void
    {
        ...
        if (player.cockThatFits(marbleCuntCapacity()) > 0)
        {
            outputText("[pg]You idly consider grabbing and pulling on her tail, so you could have a chance to take charge, though Marble would probably try and take control in that case for sure...");
        }
        ...
        addButton(2, "Pull Tail", milkMarbleBarPullTail).disableIf(player.cockThatFits(marbleCuntCapacity()) < 0, "Req. a cock fitting " + marbleCuntCapacity() + " area.");
    ...
    {
        var fits : Bool = player.cockArea(x) < marbleCuntCapacity();
        var y : Int;
        clearOutput();
        if (player.cockArea(x) < marbleCuntCapacity())
        {
            ...
            if (y < 0)
            {
                sceneHunter.print("You could use a second one!");
            }
        }
        ...
        if (fits)
        {
            ...
            if (y >= 0)
            {
                outputText("  You find your " + cockDescript(y) + " slides up comfortably in between her cheeks, and squeeze them together a bit to give it a nice tight valley of soft flesh to run through.");
            }
        ...
        {
            var pLust : Int = as3hx.Compat.parseInt(10 + player.lib / 10);
            dynStats("lus", pLust);
    ...
    private function milkMarbleBarPullTail() : Void
    {
        ...
        if (flags[kFLAGS.MARBLE_DICK_TYPE] > 0)
        {
            outputText("  She grabs her erect member, and rubs it against your cheek a few times, spreading around some of her pre-cum.");
        }
        ...
        if (flags[kFLAGS.MARBLE_DICK_TYPE] == 0 && rand(2) == 0)
        {
            ...
            {
                outputText("You take full advantage of your inhumanly long tongue to fill her up, running across her passage all the way to the entrance to her womb and then back to her lower lips.");
            }
            else
            {
                outputText("You send your tongue as far into her passage as it will go and wriggle around her lower lips, trying to please your aggressive bovine lover to the best of your abilities.");
            }
        }
        else
        {
        ...
        if (silly())
        {
            outputText("  Want to brush your teeth with my tail next time?");
        }
    ...
    private function milkMarbleLeaveAfterBar() : Void
    {
        ...
        var pLust : Int = as3hx.Compat.parseInt(5 + player.lib / 20);
        dynStats("lus", pLust);
        flags[kFLAGS.MARBLE_LUST] += 5;
        doNext(camp.returnToCampUseTwoHours);
    }
