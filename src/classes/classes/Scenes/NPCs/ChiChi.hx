...
class ChiChi extends Monster
{
    ...
    public function PentaStrike() : Void
    {
        ...
        {
            outputText(" Thankfully your training with her helped you learn how to counter most of these attacks and you manage to weaken her normally overwhelming blows.");
        }
        outputText("\n\n");
    }
    ...
    override private function performCombatAction() : Void
    {
        ...
        {
            if ((this.HPRatio() < 0.5 || this.lust > this.maxLust() * 0.5) && !hasStatusEffect(StatusEffects.DefendMonsterVer))
            {
                DefensiveStance();
            }
            else
        ...
        else if (hasStatusEffect(StatusEffects.AbilityChanneled))
        {
            SoulBlast();
        }
        else if (flags[kFLAGS.CHI_CHI_FOLLOWER] == 3)
        {
            var choice1 : Float = rand(5);
            if (choice1 < 3)
            {
                PentaStrike();
            }
    ...
    {
        if (flags[kFLAGS.CHI_CHI_SAM_TRAINING] == 3)
        {
            chichiScene.WonSparringFight();
        }
        else if (flags[kFLAGS.CHI_CHI_SAM_TRAINING] == 2)
        {
            chichiScene.WonSecondFight();
        }
        else
        {
            chichiScene.WonFirstFight();
        }
    }
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (flags[kFLAGS.CHI_CHI_SAM_TRAINING] == 3)
        {
            chichiScene.LostSparringFight();
        }
        else if (flags[kFLAGS.CHI_CHI_SAM_TRAINING] == 2)
        {
            chichiScene.LostSecondFight();
        }
        else
        {
    ...
    override private function get_long() : String
    {
        ...
        if (hasStatusEffect(StatusEffects.DefendMonsterVer))
        {
            str += " Chi Chi appears to be in great pain but keeps fighting on with little regards for her wounds. It would be smart to surrender so she doesn’t go so far as to kill herself for an arena match!</b>";
        }
        return str;
    }
    ...
    public function new()
    {
        ...
        {
            this.drop = new ChainedDrop().add(consumables.FIERYS_, 0.2);
        }
        //this.arms.type = LION;
        //this.lowerBody = LION;
