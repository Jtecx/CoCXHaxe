...
class Vapula extends NPCAwareContent implements TimeAwareInterface
{
    public function new()
    {
        super();
    ...
    public function timeChange() : Bool
    {
        if (model.time.hours > 23)
        {
            if (vapulaSlave() && flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 0)
            {
                ...
                {
                    flags[kFLAGS.VAPULA_DAYS_SINCE_FED]++;
                }
                else
                {
            ...
            if (flags[kFLAGS.VAPULA_FOLLOWER] == .5 || flags[kFLAGS.VAPULA_FOLLOWER] == 1.5)
            {
                flags[kFLAGS.VAPULA_FOLLOWER]++;
            }
            flags[kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS]++;
        }
        if (vapulaSlave() && player.hasKeyItem("Demonic Strap-On") < 0 && player.gender == 2 && flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 0 && flags[kFLAGS.IN_INGNAM] == null)
        {
    ...
    override public function vapulaSlave() : Bool
    {
    ...
    public function vapulaSlaveFlavorText() : Void
    {
        ...
        if (choice == 0)
        {
            outputText("Vapula is currently resting on a very rough bed of leaves she's gathered herself.  She sleeps quietly, her hand absent-mindedly laid on her vagina and the other one resting on her breasts.  A thin trickle of her juice is leaking out of her crotch.");
        }
        if (choice == 1)
        {
            outputText("Your succubus slut, Vaplua, is furiously stroking herself on her bed of leaves, her purple body shining in sweat.  She moans raggedly, often looking up to you with lust-consumed eyes.");
        }
        if (choice == 2)
        {
            if (flags[kFLAGS.VAPULA_HAREM_FUCK] == 0)
            ...
            {
                outputText("Vapula is currently absent, probably getting screwed by some of the imps surrounding your camp in order to get her semen rations.  You're sure she would come back instantly if you ever needed to give her YOUR cum instead.");
            }
        }
        if (choice == 3)
        {
            outputText("Vapula is busy drinking a vial of milk that looks very much like the kind you'd usually find on imps.  Her purple cheeks seem to blush darker when she sees you; she awkwardly gulps the rest of the bottle, hiccups, and giggles softly, eyeing you with a coy smile and a lust-filled gaze.  The potion seems to have made her even more aroused than usual.");
        }
        if (choice == 4)
        {
        ...
        if (choice == 5)
        {
            if (flags[kFLAGS.VAPULA_HAREM_FUCK] == 0 || !(sophieBimbo.bimboSophie()))
            {
                choice = 6;
            }
            ...
            {
                outputText("Vapula is fingering herself while eating out Sophie, your harpy slut.  The dumb avian matron keeps moaning like she's in heat while the succubus furiously assaults her lush fuck-hole; both of their pair of wings flap in rhythm as they feel their pleasure rising in waves.");
            }
        }
        if (choice == 6)
        {
            if (flags[kFLAGS.VAPULA_HAREM_FUCK] == 0 || flags[kFLAGS.IZMA_NO_COCK] > 0 || !izmaFollower())
            {
                choice = 7;
            }
        ...
        //if C. Jojo and harem fucking is on){
            ...
            if (flags[kFLAGS.VAPULA_HAREM_FUCK] == 0 || !jojoScene.campCorruptJojo())
            {
                choice = 8;
            }
            else
        ...
        if (choice == 8)
        {
            if (flags[kFLAGS.VAPULA_HAREM_FUCK] == 0 || !(amilyScene.amilyFollower() && amilyScene.amilyCorrupt()))
            {
                choice = 9;
            }
            ...
            {
                outputText("Vapula is busy caressing your corrupted mousette, Amily while softly kissing her breasts.  Her fingers are buried in your fuck-toy's pussy, and Amily is moaning whorishly and doesn't even bother to stop when she looks at you, her helpless eyes betraying her lack of control over her own lust.");
            }
        }
        if (choice == 9)
        {
            ...
            {
                outputText("Vapula is having a very active conversation with Rathazul, the alchemist.  They seem to be discussing the chemical properties of demonic mixtures and the various taints that could occur from their ingestion.  The succubus is holding a bottle of a purplish white fluid that appears to be her own milk, probably waiting for an analysis.");
            }
        }
        if (choice == 10)
        {
            outputText("Vapula is resting on her stomach, a buttplug shoved into her ass.  She doesn't seem to notice you watching her butt jiggle under the effect of her unconscious anal contractions.");
        }
    }
    ...
    public function mouseWaifuFreakout(amily : Bool = false, jojo : Bool = false) : Void
    {
        clearOutput();
        if (amily)
        {
        ...
        if (jojo)
        {
            outputText((flags[kFLAGS.JOJO_BIMBO_STATE] != 3) ? "Jojo " : "Joy ");
        }
        outputText("walk");
        if (!(amily && jojo))
        {
        ...
        //[if libido >=50]{
            ...
            if (amily)
            {
                ...
                {
                    player.knockUpForce(PregnancyStore.PREGNANCY_MOUSE, player.pregnancyIncubation, 1);
                }
            ...
            //[(if PC corr > 70)
            if (player.cor > 70 && !(jojo && flags[kFLAGS.JOJO_BIMBO_STATE] == 3))
            {
            ...
            outputText("\n\n");
            if (jojo && flags[kFLAGS.JOJO_BIMBO_STATE] != 3)
            {
                outputText("Jojo");
            }
            if (amily && (jojo && flags[kFLAGS.JOJO_BIMBO_STATE] != 3))
    ...
    public function callSlaveVapula(output : Bool = true) : Void
    {
        if (output)
        {
            clearOutput();
        ...
        addButton(2, "Feed", feedVapulaACupOfJizz).disableIf(player.lust < 33, "You aren't suitably aroused to feed Vapula right now.").disableIf(!player.hasCock(), "You can't feed her with your cock when you don't have any!");
        addButton(3, "Feed(Dildo)", chixFeedVapulaBlehblehIVantToZuckYourSpooo).disableIf(player.lust < 33, "You aren't suitably aroused to feed Vapula right now.").disableIf(!player.hasVagina(), "You still need a vagina to use the strap-on.").disableIf(!player.hasKeyItem("Demonic Strap-On"), "You're not sure if it's possible to feed her without using a dick.\n\n"
                + ((player.isHerm()) ? "But it's incredibly annoying that she doesn't pay much attention to your pussy.. Perhaps you should ask her about it?" : "Not that you really need it at the moment."), "???");
        addButton(5, "Spank", spankVapulaLikeABoss).disableIf(player.isTaur() || player.isDrider(), "Not for taurs or driders.").disableIf(!flags[kFLAGS.VAPULA_EARNED_A_SPANK], "She has to earn it first.");
        if (flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] != null)
        {
            addButton(6, "Go Camp", backToCamp);
            if (flags[kFLAGS.FOLLOWER_PRODUCTION_VAPULA] != null)
            {
                addButton(7, "Stop Harvest", stopHarvest);
            }
            else
            {
                addButton(7, "Harvest Milk", harvestMilk);
            }
            addButton(14, "Back", SceneLib.farm.farmCorruption.rootScene);
        }
        else
        {
            addButton(4, "Threesome", vapulaThreesomeMenu).disableIf(player.lust < 33, "You aren't much interested in any of the threesomes Vapula seems poised to suggest.").disableIf(!player.hasCock() && !(player.hasVagina() && player.hasKeyItem("Demonic Strap-On") >= 0), 
                    "You don't have anything to fuck Vapula with. Maybe try growing a dick?"
            );
            if (flags[kFLAGS.FARM_CORRUPTION_STARTED] != null)
            {
                addButton(6, "Farm Work", sendToFarm);
            }
    ...
    private function harvestMilk() : Void
    {
        ...
        {
            outputText("\"<i>It’ll sure as hell beat whatever disgusting thing the bitch would have me doing instead. Why don’t you make her build a concentration machine? Lethice has loads; makes one dose of cream go so much further.</i>\"");
        }
        else
        {
            outputText("\"<i>As you insist, [master]. It’ll sure as hell beat working.</i>\"");
        }
    ...
    private function fapulaFapfapfapAppearance() : Void
    {
        ...
        if (SceneLib.farm.farmCorruption.hasTattoo("vapula"))
        {
            ...
            {
                if (SceneLib.farm.farmCorruption.numTattoos("vapula") > 1)
                {
                ...
                if (flags[kFLAGS.VAPULA_TATTOO_COLLARBONE] != 0)
                {
                    outputText(flags[kFLAGS.VAPULA_TATTOO_COLLARBONE] + "\n");
                }
                if (flags[kFLAGS.VAPULA_TATTOO_SHOULDERS] != 0)
                {
                    outputText(flags[kFLAGS.VAPULA_TATTOO_SHOULDERS] + "\n");
                }
                if (flags[kFLAGS.VAPULA_TATTOO_LOWERBACK] != 0)
                {
                    outputText(flags[kFLAGS.VAPULA_TATTOO_LOWERBACK] + "\n");
                }
                if (flags[kFLAGS.VAPULA_TATTOO_BUTT] != 0)
                {
                    outputText(flags[kFLAGS.VAPULA_TATTOO_BUTT] + "\n");
                }
            }
        }
    ...
    private function talkToVapulaForSomeReason() : Void
    {
        ...
        {
            addButtonDisabled(1, "??", "You have to try some suspicious potion first... maybe Giacomo could help?");
        }
    ...
    private function feedVapulaACupOfJizz() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText("Your [balls] churn");
        }
        else
        {
            outputText("Your [cock] twitches");
        }
        ...
        if (player.cumQ() >= 1500)
        {
            outputText(" until she looks six months pregnant");
        }
    ...
    private function vapulaThreesomeMenu() : Void
    {
        ...
        addButton(0, "Amily", vapulaAndAmilyThreesome).disableIf(!player.hasCock(), "You need a <b>REAL</b> cock for that.").disableIf(amilyScene.amilyFollower() && amilyScene.amilyCorrupt(), 
                "You need a corrupted mouse for this.", "???"
        );
        addButton(1, "Jojo", vapulaJojoThreesomes).disableIf(!jojoScene.campCorruptJojo() || flags[kFLAGS.JOJO_BIMBO_STATE] == 3, "You need a corrupted mouse with a dick.", "???");
        addButton(2, "Izma", vapulaAndIzmaThreeSome).disableIf(flags[kFLAGS.IZMA_NO_COCK], "Izma must have a cock.").disableIf(!izmaFollower(), "No sharks at your camp... yet?", "???");
        addButton(3, "Sophie", vapulaSophieThreesomeSelect).disableIf(!sophieBimbo.bimboSophie(), 
                "If you had some bimbo harpy, you could have some fun with her.", "???"
        );
        addButton(4, "Ceraph", vapulaCeraphThreesome).disableIf(!player.hasCock(), "You need a <b>REAL</b> cock for that.").disableIf(!ceraphFollowerScene.ceraphIsFollower() && !player.hasStatusEffect(StatusEffects.CeraphOff), 
    ...
    private function vapulaCeraphThreesomePleaseBoth() : Void
    {
        ...
        if (player.cumQ() >= 1500)
        {
            outputText(" and swelling her belly until she looks ready to give birth to a full horde of imps");
        }
        outputText(".");
        outputText("\n\nYou feel your ");
        if (player.hasBalls())
        {
            outputText(ballsDescriptLight() + " twitch and your " + cockDescript(x) + " pulsate as they unload their");
        }
        else
        {
            outputText("your " + cockDescript(x) + " twitch and pulsate as it unloads its");
        }
        ...
        if (player.hasBalls())
        {
            outputText(ballsDescriptLight() + " hitting her butt-hole with a faint thud");
        }
        else
        {
            outputText("thudding up against her cushy butt-cheeks");
        }
        ...
        {
            outputText("  Sadly, your extra junk does not have any hole left to fill.  It just hangs there, rubbing against Ceraph's backside and teasing her all the more.");
        }
        ...
        if (player.cockTotal() > 1)
        {
            outputText("  She expertly jerks each of your rods in turn, gently polishing them as they grow back into an erect state.");
        }
        ...
        if (player.hasBalls())
        {
            outputText(" as the girls keep fondling your " + ballsDescriptLight());
        }
    ...
    private function vapulaAndCeraphButtfuckTrainYeehaw() : Void
    {
        ...
        if (silly())
        {
            outputText("screaming at the top of your lungs: \"<i>BUTT-FUCK! BUTT-FUCK! BUTT-FUCK!</i>\", soon joined by your horny anal partners");
        }
        else
        {
            outputText("groaning and panting as you ravage Ceraph's pucker with your junk");
        }
        outputText(".\n\n");
        ...
        if (player.hasBalls())
        {
            outputText("  As you relentlessly smash your groin against Ceraph's plump rump, your [balls] repeatedly hit her quivering vagina; they are soon utterly coated in fem-spunk.");
        }
        ...
        if (player.cumQ() >= 1500)
        {
            outputText("  She automatically clenches her anus in order to keep as much of your baby-batter inside as possible, but to no avail; torrents of white goo start spurting out of her rectum, splattering her legs and your groin.");
        }
    ...
    private function vapulaSophieThreeSome() : Void
    {
        ...
        if (player.cockArea(x) > 75)
        {
            outputText("you've filled the entirety of her colon");
        }
        else
        {
            outputText("your cock is already buried to the hilt, its mammoth girth bulging between her broad hips");
        }
        ...
        if (player.hasBalls())
        {
            outputText("balls and massages your ");
        }
        outputText(buttDescript() + ".");
        //[if third cock]
        if (player.cockTotal() > 2)
        {
            outputText("  Using her extra flexibility, she bends over and start jerking your third pole, the sweet handjob adding to your pure cocktail of pleasure.");
        }
        ...
        if (player.cockTotal() < 3)
        {
            outputText("hastily pull out and ");
        }
        outputText("wave ");
        if (player.cockTotal() >= 3)
        {
    ...
    private function vapulaAndAmilyThreesome() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText(": her drooling pussy needs to be prepared because you're going to take her next");
        }
        ...
        {
            outputText(" and her fur starts shrinking, letting her rosy skin shine below");
        }
        ...
        if (player.hasBalls())
        {
            ...
            if (player.cumQ() >= 1500)
            {
                outputText("  Sticky globs of white goo are shot from the tip of your rod, effectively stuffing Vapula so full of your spooge that some of it drips out and rains down on Amily's face, sopping her with fluids. The succubus quivers in your arms, giving in to her lust before squirting a powerful spray of fem-spooge, splattering Amily even more.");
            }
    ...
    public function vapulaAssistsCeruleanSuccubus() : Void
    {
        ...
        if (player.cockTotal() > 1)
        {
            outputText("  She suddenly seems to have an idea: leaning down to reach your second cock, she starts licking it, accompanying the savage thrusting of the blue demoness as she slurps your extra junk, giving you even more pleasure.");
        }
        else
        {
            outputText("  The blue demoness leans down and starts licking Vapula's moist pussy, intertwining her flexible tongue with yours and pleasuring the purple slut even more.");
        }
        ...
        if (player.cockTotal() > 1)
        {
            ...
            if (player.cockTotal() > 2)
            {
                outputText("and your blue lover's mouth ");
            }
            outputText("likewise.");
        }
        ...
        {
            outputText(", enclosing your head in her cleavage");
        }
    ...
    private function vapulaJojoThreesomes() : Void
    {
        ...
        addButton(1, "ButtfuckTrain", jojoButtFuckTrain).disableIf(!player.hasCock(), "You need a <b>REAL</b> cock for that.");
    }
    ...
    private function NTRSomeJojos() : Void
    {
        ...
        function dickF() : Void
        {
            ...
            if (player.cockTotal() > 1)
            {
                ...
                if (player.cockTotal() > 2)
                {
                    outputText("  Since the horny slut has a demonic vaginal capacity, you might as well fit more dick in it: in no time your third dick joins its colleague in this infernal vaginal assault.");
                }
                outputText("  ");
            }
            else
            ...
            if (jojoScene.tentacleJojo())
            {
                outputText("his tentacles start flying toward his ass, ");
            }
            outputText("his-");
            outputText("\n\n\"<i>No.</i>\"");
            ...
            if (player.cockTotal() > 1)
            {
                outputText("  Your extra junk shoved down her love-tunnel also explodes in a milky torrent, blasting her fuck-hole full of spooge too.");
            }
        ...
        var strapF : Void->Void = function() : Void
        {
            clearOutput();
    ...
    private function jojoButtFuckTrain() : Void
    {
        ...
        if (tentaCnt < 3)
        {
            sceneHunter.print("You could use more tentacle dicks... just ensure they are long enough.");
        }
        //select tentas first
        var tentas : Array<Dynamic> = [];
        for (i in 0...3)
        {
        ...
        if (mainCock < 0)
        {
            mainCock = player.findCockNotIn(tentas, -1, -1, -1);
        }  //smallest then  
        ...
        if (player.cockArea(mainCock) > 100)
        {
            outputText("  You can actually feel his internal organs being bumped against your meat as your " + cockDescript(mainCock) + " stretches his colon beyond its normal size and thrusts in and out like a charging mammoth.");
        }
        ...
        if (jojoScene.tentacleJojo())
        {
            ...
            if (player.hasVagina())
            {
                outputText("  Last but not least, a soft cock-knock at the entrance of your " + vaginaDescript() + " signals an incoming intrusion.  If your mouth weren't so full of cock you would sigh in relief as your aching pussy is thoroughly penetrated.");
            }
        }
        //tentacock part
        if (tentas[0] >= 0)
        ...
        if (tentas[0] >= 0)
        {
            outputText(", as well as Vapula's love-tunnel");
        }
        if (tentas[1] >= 0)
        {
        ...
        if (jojoScene.tentacleJojo())
        {
            outputText("; his tentacles absolutely drench your every hole with greenish mousey-cum");
        }
    ...
    private function vapulaAndIzma(girls : Bool = true) : Void
    {
        clearOutput();
        var x : Int = player.cockThatFits(100);
        if (x < 0)
    ...
    private function chixFeedVapulaBlehblehIVantToZuckYourSpooo() : Void
    {
        ...
        addButton(1, "Tease", teaseVapula).disableIf(flags[kFLAGS.VAPULA_TEASE_COUNT] > flags[kFLAGS.VAPULA_DAYS_SINCE_FED], 
    ...
    private function teaseVapula() : Void
    {
        ...
        {
            //Requires: Vapula not fucking harem, teased the day beforeif (flags[kFLAGS.VAPULA_TEASE_COUNT] == 1)
            {
                ...
                {
                    outputText("\"<i>I would worship your feet, [master].  I would clean your hooves with my tongue to a shine, tease out every scrap of dirt from every crevice.  And I would thank you for the privilege.</i>\"");
                }
                //[Naga:
                ...
                else if (player.lowerBody == LowerBody.DOG || player.lowerBody == LowerBody.CAT || player.lowerBody == LowerBody.LIZARD || player.lowerBody == LowerBody.KANGAROO || player.lowerBody == LowerBody.FOX)
                {
                ...
                //[Insect:
                else if (player.lowerBody == LowerBody.BEE || player.lowerBody == LowerBody.CHITINOUS_SPIDER_LEGS)
                {
            ...
            else
            {
                ...
                if (player.isNaga())
                {
                    outputText("you slide your tail under and over her trunk, wrapping your coils around her and drawing her face into your stuffed crotch.");
                }
                //Other:
                else if (player.isTaur() || player.isGoo() || player.isDrider())
                {
                    outputText("you sit the monstrous half of your body down comfortably and draw her face into your stuffed cotch.");
                }
                else
                {
                    outputText("you open your [legs] and wrap them around her head, drawing her face into your stuffed crotch.");
                }
    ...
    public function vapulaForceFeeds() : Void
    {
        ...
        else if (player.hasCock())
        {
            ...
            {
                outputText("  Your " + cockDescript(x) + " twitches");
            }
            ...
            if (player.cumQ() >= 1500)
            {
                outputText(" until she looks 6 months pregnant.  At last, the cum-flow spilling through your urethra starts to ebb and your " + cockDescript(x) + " stops throbbing; only then does she remove your junk from her mouth with a loud POP.  Your tool appears to be clean of any spooge: your cock-slut did a very good job.  Satisfied, you pat her head with your cock and let her digest her lavish meal.");
            }
            player.sexReward("saliva", "Dick");
            dynStats("cor", 1);
            flags[kFLAGS.VAPULA_DAYS_SINCE_FED] = 0;
    ...
    private function spankVapulaLikeABoss() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(" and a healthy surge of blood to [eachCock]");
        }
        else if (player.hasVagina())
        {
            outputText(" and a teltale moistness to your nethers");
        }
        outputText(".");
        ...
        {
            outputText(", drawing the eyes of your other companions.");
        }
        else if (camp.companionsCount() == 2)
        {
            outputText(", drawing the attention of your other companion.");
        }
        else
        {
            outputText(", echoing off the sentinel-like rocks that seem to ring your encampment.");
        }
        ...
        {
            outputText("  Dishing out all this suffering is getting you a little hot under the collar, and you squirm a bit in place in anticipation of the pain to come.");
        }
