...
class KidAScene extends BaseContent implements TimeAwareInterface
{
    public function new()
    {
        super();
    ...
    public function timeChange() : Bool
    {
        ...
        //weapon check to avoid errors in scene{
            ...
            else if (flags[kFLAGS.KID_SITTER] == 2)
            {
    ...
    public function kidAXP(diff : Float = 0) : Float
    {
        if (diff == 0)
        {
        ...
        if (flags[kFLAGS.KID_A_XP] < 0)
        {
            flags[kFLAGS.KID_A_XP] = 0;
        }
        if (flags[kFLAGS.KID_A_XP] > 100)
        {
            flags[kFLAGS.KID_A_XP] = 100;
        }
        return flags[kFLAGS.KID_A_XP];
    }
    ...
    public function anemoneKidBirthPtII() : Void
    {
        ...
        {
            outputText(" even as the face angles up to maintain eye contact");
        }
    ...
    public function approachAnemoneBarrel() : Void
    {
        ...
        {
            outputText("Kid A is away right now, helping season some of Marble's children.  If you wanted to leave or take an item, she would discover it when she got back.  ");
        }
        else if (kidAXP() < 25)
        {
            outputText("Kid A sinks below the rim, peering nervously at you.  ");
        }
        else if (kidAXP() < 75)
        {
            outputText("Kid A pokes her head out and smiles at you.  ");
        }
        else
        {
            outputText("Kid A leans her head and shoulders out of the barrel and puts her elbows on the rim, staring unabashedly at you as she rests her chin on her hands.  ");
        }
        ...
        if (flags[kFLAGS.KID_SITTER] <= 1)
        {
            if (flags[kFLAGS.ANEMONE_WEAPON_ID] != 0 && player.fatigue <= (player.maxOverFatigue() - 25))
            {
                addButton(3, "Tutor", tutorAnemoneKid);
            }
            else if (player.fatigue > (player.maxOverFatigue() - 25))
            {
                outputText("\n\nYou're too tired to tutor Kid A.");
            }
            addButton(4, "Watch", anemoneWatchToggle);
            addButton(8, "Evict", evictANemone);
            addButton(2, "Sex", kidASex, false).disableIf(player.lust < 33, "You aren't aroused enough to have sex with her right now.").disableIf(!sexPossible(), "You need a vagina or a fitting (60) dick to have sex with her.").disableIf(kidAXP() < 40, "Kid A isn't self-confident enough to have sex with right now...  Perhaps if you could tutor her with a weapon she seems to agree with?").disableIf(!flags[kFLAGS.HAD_KID_A_DREAM] || flags[kFLAGS.ANEMONE_KID] < 2, "", "???");
        }
    ...
    private function getAnemoneItem() : Void
    {
        ...
        {
            outputText("  Holy... how did she drag this thing home!?");
        }
        outputText("\n\n");
    ...
    private function giveAnemoneWeapon(page : Int = 1) : Void
    {
        ...
        var giveableToAnemone : ItemType->Bool = function(item : ItemType) : Bool
        {
            return item == consumables.W__BOOK || item == consumables.B__BOOK || item == consumables.W_STICK || Std.is(item, Weapon);
        }
        menu();
        ...
        if (page == 1)
        {
            ...
            {
                addButton(13, "Next", giveAnemoneWeapon, page + 1);
            }
        }
        if (page == 2)
        {
    ...
    private function anemoneWatchToggle() : Void
    {
        ...
        else if (flags[kFLAGS.ANEMONE_WEAPON_ID] == 0)
        {
    ...
    private function tutorAnemoneKid() : Void
    {
        ...
        {
            outputText("taking up an attentive stance with " + ItemType.lookupItem(flags[kFLAGS.ANEMONE_WEAPON_ID]).longName + " in her hands.");
        }
        ...
        if (InCollection(flags[kFLAGS.ANEMONE_WEAPON_ID], weapons.PIPE.id, weapons.MACE.id, weapons.W_STAFF.id, weapons.E_STAFF.id, weapons.L_STAFF.id))
        {
            ...
            else if (flags[kFLAGS.ANEMONE_WEAPON_ID] == weapons.MACE.id)
            {
                outputText("mace");
            }
            else
            ...
            if (flags[kFLAGS.ANEMONE_WEAPON_ID] != weapons.MACE.id)
            {
                outputText(" is still enough to bruise you a bit.");
            }
            else
            {
                outputText(" manages to bruise you a lot.");
            }
            HPChange(-5, false);
            if (flags[kFLAGS.ANEMONE_WEAPON_ID] == weapons.MACE.id)
            {
        ...
        {
            //[Riding Crop]if (flags[kFLAGS.ANEMONE_WEAPON_ID] == weapons.RIDINGC.id)
            {
                ...
                flags[kFLAGS.ANEMONE_WEAPON_ID] == weapons.SUCWHIP.id)
            {
                ...
                if (flags[kFLAGS.ANEMONE_WEAPON_ID] == weapons.SUCWHIP.id)
                {
                    outputText("  The demonic enchantment chooses then to activate, and her color deepens as her lust peaks, as does your own.");
                }
        ...
        if (player.HP <= player.minHP())
        {
            ...
            if (flags[kFLAGS.ANEMONE_WEAPON_ID] != weapons.S_GAUNT.id &&
                flags[kFLAGS.ANEMONE_WEAPON_ID] != weapons.H_GAUNT.id)
            {
                outputText("the thump of something hitting the ground and ");
            }
        ...
        if (flags[kFLAGS.HAD_KID_A_DREAM] > 0 && kidAXP() >= 40 && player.lust >= player.maxOverLust())
        {
            ...
            {
                //if KidXP >= 40 and lust > 99 after tutor and PC has only huge dicks of area >= 60 or hasn't got shitif (kidAXP() >= 40 && player.lust >= player.maxOverLust())
                {
                    ...
                    if (player.hasCock())
                    {
                        outputText("something too enormous");
                    }
                    else
                    {
                        outputText("nothing useful");
                    }
    ...
    //
    private function sexPossible() : Bool
    {
        return player.cockThatFits(60) >= 0 || player.hasVagina();
    }
    
    private function kidASex(cont : Bool = true) : Void
    {
        if (!cont)
        {
            clearOutput();
        ...
        {
            sceneHunter.selectGender(sexFellatio, sexVaginal, null, null, 1, 
    ...
    private function sexFellatio() : Void
    {
        ...
        if (y >= 0 || player.hasBalls() || player.hasVagina())
        {
            ...
            //[(if cock or sack)]
            if (y >= 0 || player.hasBalls())
            {
                outputText(", pulling it free from your armor as well");
            }
            ...
            if (y >= 0)
            {
                ...
                if (player.cocks[y].cockLength >= 36)
                {
                    outputText("towers over her head");
                }
                else
                {
                    outputText("quivers and twitches into her venomous locks");
                }
            ...
            {
                outputText("  One hand cups below your " + ballsDescript() + " and lifts them up to her chin, where the other, formerly tracing gentle circles along your shaft, grabs a tentacle from her head and begins to tickle.");
            }
            //[(vag)
            else if (player.hasVagina())
            {
                ...
                {
                    outputText("working it free as well and allowing her to tease and stroke it, cock-like, with her hands and hair.");
                }
                else
                {
                    outputText("in order to give the sensitive button their attention.");
                }
        ...
        if (player.hasBalls())
        {
            outputText("your balls tighten up");
        }
        else
        {
            outputText("your body tenses");
        }
        ...
        if (player.cumQ() < 1000)
        {
            outputText("then gets to her feet and returns to her barrel to digest the meal.");
        }
        else
        {
            outputText("but can barely move from the spot thanks to the incredible distension of her belly.  She manages to drag herself over to the barrel with a great shuffling of arms and legs but cannot by any means get into it, and settles for slumping beside it to occasionally sprinkle water on herself while she deflates.");
        }
    ...
    private function sexVaginal() : Void
    {
        ...
        {
            outputText("  You can barely feel her little shaft in your stretched cunt, but the chemical stimulation from the tentacles stroking your insides goes a long way toward making up for that.");
        }
        ...
        if (player.clitLength < 6)
        {
            outputText("  The little button gets rubbed by her probing hand even as she resumes thrusting, accelerating your imminent peaking.");
        }
        else if (player.clitLength < 36)
        {
            outputText("  The monstrous chick-stick bobs in the air as she pounds you harder; the breeze alone would be enough to arouse you further, but the anemone grabs it and begins jacking the nerve-laden stalk off like a dick, which sends your back into spasms.");
        }
        else
        {
            outputText("  The incredible length of your clitoris is such that the pace of the fuck slows to a crawl as the anemone puzzles out what to do with it; finally falling back on her racial instinct, she pops it into her mouth and begins to caress the tip gently with her teeth and tongue as her short hair reaches around to tickle the shaft.  Your body flops weakly, too wracked by the sensation from your clit to allow conscious control.");
        }
    ...
    public function kidADreams() : Void
    {
        ...
        else if (player.hasCock() && (!player.hasVagina() || player.femininity < 50))
        {
            dickF();
        }
        else
        ...
        function dickF() : Void
        {
            ...
            if (!player.isTaur())
            {
                outputText("soft hand strokes your face and a ");
            }
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            if (player.cor >= 66)
            {
                outputText(" and, with a sigh, you grab her head and force it down to the mess, compelling her to lick it up.");
            }
            else
            {
                outputText(" and neither of you says a word as she backs away slowly on her knees.");
            }
    ...
    public function kidAWatchesSharks() : Void
    {
        ...
        if (kidAXP() < 40 || flags[kFLAGS.ANEMONE_WEAPON_ID] == 0)
        {
        ...
        else if (kidAXP() < 40 && (flags[kFLAGS.ANEMONE_WEAPON_ID] == consumables.B__BOOK.id || flags[kFLAGS.ANEMONE_WEAPON_ID] == consumables.W__BOOK.id))
        {
            ...
            if (flags[kFLAGS.ANEMONE_WEAPON_ID] == consumables.W__BOOK.id)
            {
                outputText("blinds them with a sudden flash of white light");
            }
            else
            {
                outputText("turns deep blue as the various shark-girls clap their thighs together and look away from one another in embarrassment");
            }
