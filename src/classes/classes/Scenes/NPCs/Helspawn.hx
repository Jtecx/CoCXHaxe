...
class Helspawn extends Monster
{
    ...
    override private function performCombatAction() : Void
    {
        ...
        {
            choices[choices.length] = calledShot;
        }
        //Zerker ability
        if (weaponAttack < 50 || flags[kFLAGS.HELSPAWN_WEAPON] == "scimitar")
        {
            choices[choices.length] = helSpawnBerserk;
        }  //Shield Bash (Shieldmander Only)  
        if (flags[kFLAGS.HELSPAWN_WEAPON] == "scimitar and shield")
        {
            choices[choices.length] = helSpawnShieldBash;
        }
        //Tease (Sluttymander Only)
        if (flags[kFLAGS.HELSPAWN_PERSONALITY] >= 50)
        {
            choices[choices.length] = sluttyMander;
        }
        //Focus (Chastemander Only)
        //Self-healing & lust restoration
        if (flags[kFLAGS.HELSPAWN_PERSONALITY] < 50)
        {
            choices[choices.length] = helSpawnFocus;
        }
    ...
    //if Bowmander
    {
        ...
        if (flags[kFLAGS.HELSPAWN_WEAPON] == "bow")
        {
            outputText(flags[kFLAGS.HELSPAWN_NAME] + " leaps back out of your reach and nocks a pair of blunted arrows, drawing them back together and loosing them at once!\n");
        }
        else
        {
            outputText(flags[kFLAGS.HELSPAWN_NAME] + " lunges at you, scimitar cleaving through the air toward your throat!\n");
        }
    ...
    private function calledShot() : Void
    {
        ...
        if (damage <= 0 || (rand(2) == 0 && (player.getEvasionRoll())))
        {
            outputText("\nYou avoid the hit!");
        }
        else
        {
            ...
            var sec : CalledShotDebuff = try cast(player.createOrFindStatusEffect(StatusEffects.CalledShot), CalledShotDebuff) catch(e:Dynamic) null;
            sec.increase();
    ...
    {
        clearOutput();
        var damage : Float = as3hx.Compat.parseInt((str) - rand(player.tou) - player.armorDef);
        // Stuns a bitch
    ...
    // Light physical, armor piercing (fire, bitch). Random chance to get this on top of any other attack
    {
        ...
        var damage : Float = as3hx.Compat.parseInt((str) - rand(player.tou));
    ...
    // Medium Lust damage
    {
        ...
        else
        {
            ...
            var lustDelta : Float = player.lustVuln * (10 + player.lib / 10);
