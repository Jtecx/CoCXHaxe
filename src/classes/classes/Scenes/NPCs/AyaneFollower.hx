...
class AyaneFollower extends NPCAwareContent implements TimeAwareInterface
{
    ...
    {
        return as3hx.Compat.parseInt(flags[kFLAGS.AYANE_CHILDREN_MALES] + flags[kFLAGS.AYANE_CHILDREN_FEMALES] + flags[kFLAGS.AYANE_CHILDREN_HERMS]);
    }
    ...
    public function ayaneCampMenu() : Void
    {
        ...
        {
            addButton(5, "Team", ayaneHenchmanOption);
        }
        else
        {
        ...
        {
            addButtonDisabled(6, "Cure C.", "You don't have any curses to cure.");
        }
        if (BelisaFollower.BelisaQuestOn && !BelisaFollower.BelisaQuestComp)
        {
            addButton(13, "ToothacheQ", BelisaAyaneTalk);
        }
    ...
    {
        var atCamp : Bool = (flags[kFLAGS.AYANE_FOLLOWER] == 2);
        clearOutput();
        if (atCamp)
        {
    ...
    private function sellItem(item : Useable, cost : Int = -1, buy : Bool = false) : Void
    {
        var priceRate : Float = 1;
        if (cost > 0)
        {
        ...
        switch (item)
        {
            case armors.WKIMONO, armors.RKIMONO, armors.BLKIMONO, armors.BKIMONO, armors.PKIMONO:
                descString = "\"<i>To look the part, you will have to dress the part. This magical clothing is made for a kitsune, and to be honest I think <b>" + Std.string(priceToShow) + " gems</b> is somewhat cheap for an enchanted garment like this.</i>\"";
            case armors.ARCBANG:
                descString = "\"<i>To look the part, you will have to dress the part. This is magical clothing made for a kitsune, and to be honest I think <b>" + Std.string(priceToShow) + " gems</b> gems is somewhat cheap for it.</i>\"";
            case shields.SPI_FOC:
                descString = "\"<i>This little icon is a very powerful spellcasting tool. It helps empower a kitsune’s magic. I don't get the use of shields; it’s so pointless. I can sell you one for <b>" + Std.string(priceToShow) + " gems</b>.</i>\"";
            case headjewelries.FOXHAIR:
                descString = "\"<i>This might appear to be just an accessory, but I personally blessed it in the name of Taoth. Should you wear it, this hairpin is likely to improve your ability to focus soul magic. This item wasn’t easy to make, which is why I can’t sell it to you for less than <b>" + Std.string(priceToShow) + " gems</b>.</i>\"";
            case consumables.FOXJEWL:
                descString = "\"<i>Don’t worry, these jewels are not actually that precious. One could say it’s concentrated kitsune energy crystallized into a gem. It’s not much, but it will help you grow your powers. I can sell you one for <b>" + Std.string(priceToShow) + " gems</b>.</i>\"";
            ...
            case consumables.SMART_T:
                descString = "\"<i>Kitsune's wits are their primary weapon. Since you weren't born one of us, you will need to learn true trickery. Drinking this tea can help you sharpen your dull human wits. I think <b>" + Std.string(priceToShow) + " gems</b> is not too steep a price for the gift of intelligence.</i>\"";
            case consumables.VIXEN_T:
                descString = "\"<i>Honing your tongue and sexual knowledge is a valiant goal as a kitsune. We kitsune are naturally born with a talent for sex and innuendo, but since you weren't born as one of us, you will need this tea to master it. I think <b>" + Std.string(priceToShow) + " gems</b> is a good deal to learn how to truly be lascivious.</i>\"";
	...
	if (player.hasCock() && player.hasVagina()) addButton(0, "Herm Worship", ayaneHermWorship);
        ...
        if (player.hasCock())
        {
            addButton(0, "Cock Worship", ayaneCockWorship);
        }
        else
        {
            addButtonDisabled(0, "Cock Worship", "You need a cock for this");
        }
        if (player.hasVagina())
        {
            addButton(1, "Vaginal Worship", ayaneVaginalWorship);
        }
        else
        {
            addButtonDisabled(1, "Vaginal Worship", "You need a vagina for this");
        }
    ...
    public function ayaneHenchmanOption() : Void
    {
        ...
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "Ayane" || flags[kFLAGS.PLAYER_COMPANION_3] == "Ayane")
            {
                addButtonDisabled(0, "Team (1)", "You already have Ayane accompany you.");
            }
            else
            {
                addButton(0, "Team (1)", ayaneHenchmanOption2, 1).hint("Ask Ayane to join you in adventures outside camp.");
            }
        }
        else if (flags[kFLAGS.PLAYER_COMPANION_1] == "Ayane")
        {
            addButton(5, "Team (1)", ayaneHenchmanOption2, 21).hint("Ask Ayane to stay in camp.");
        }
        else
        {
            addButtonDisabled(5, "Team (1)", "You already have other henchman accompany you as first party member. Ask him/her to stay at camp before you talk with Ayane about accompaning you as first party member.");
        }
        if (player.hasPerk(PerkLib.IntermediateLeadership))
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "")
            {
                if (flags[kFLAGS.PLAYER_COMPANION_1] == "Ayane" || flags[kFLAGS.PLAYER_COMPANION_3] == "Ayane")
                {
                    addButtonDisabled(1, "Team (2)", "You already have Ayane accompany you.");
                }
                else
                {
                    addButton(1, "Team (2)", ayaneHenchmanOption2, 2).hint("Ask Ayane to join you in adventures outside camp.");
                }
            }
            else if (flags[kFLAGS.PLAYER_COMPANION_2] == "Ayane")
            {
                addButton(6, "Team (2)", ayaneHenchmanOption2, 22).hint("Ask Ayane to stay in camp.");
            }
            else
            {
                addButtonDisabled(6, "Team (2)", "You already have other henchman accompany you as second party member. Ask him/her to stay at camp before you talk with Ayane about accompaning you as second party member.");
            }
        }
        else
        {
    ...
    public function ayaneHenchmanOption2(slot : Float = 1) : Void
    {
        clearOutput();
        if (slot < 21)
        {
            ...
            var intAyane : Float = 220;
            var wisAyane : Float = 110;
            var spellsoulskillpowerAyane : Float = 1.5;  /*
		if (flags[kFLAGS.AURORA_LVL] >= 1) {
            ...
            if (slot == 2)
            {
                flags[kFLAGS.PLAYER_COMPANION_2] = "Ayane";
            }
            if (slot == 1)
            {
                flags[kFLAGS.PLAYER_COMPANION_1] = "Ayane";
            }
        }
        else
        {
    ...
    public function ayaneGivesBirth() : Void
    {
        ...
        if (roll < 10)
        {
            flags[kFLAGS.AYANE_CHILDREN_MALES]++;
        }
        else if (roll < 70)
        {
            flags[kFLAGS.AYANE_CHILDREN_FEMALES]++;
        }
        else
        {
            flags[kFLAGS.AYANE_CHILDREN_HERMS]++;
        }
    }
    public function ayanePregUpdate() : Bool
    {
    ...
    {
        return (flags[kFLAGS.KITSUNE_SHRINE_UNLOCKED] != null) ? known : unknown;
    }
    ...
    public function randomEncounter() : Void
    {
        ...
        if (flags[kFLAGS.AYANE_FOUGHT] != null)
        {
    ...
    public function followFlame() : Void
    {
        ...
        outputText("You fellow it into complete darkness and eventually run into a wide area. This is getting creepier by the second. Just as you are about to run, fire lit everywhere around you into the form of pale ghostly flames, a mocking woman's voice commenting.\n"
                ...
                + ayaName("Ayane, turns out she’s away from the shrine today and was looking for someone to feed from, guess that is you", "a tan skinned woman in a white kimono which could almost pass for human if not for her fox-like pupils, animal ears and 7 swishing fox tails")
                + ". She’s pointing an exotic staff at you smirking mischievously before fading in the ambient darkness."
    ...
    public function fightWin() : Void
    {
        ...
        outputText("You smirk in satisfaction as you finally hit [themonster] hard enough to force her to the ground. You grab her by the tail and force her to look at you. You’re going to do terrible things to her.\n"
                ...
                + "\n"
                + "“<i>Hey, don’t hold it against me, but I have places to be, so see you later. "
                + ayaName("Ayane", "By the way, my name is Ayane and if you're ever interested I hold a shop at the kitsune shrine. Do pay me a visit, I could always trade you some services and wares. ")
                + "This said, you seriously should just let me win next time, it will be way more pleasant.</i>” \n"
                + "\n"
    ...
    public function loseMale() : Void
    {
        ...
        outputText("“<i>Ah finally giving up?</i>” the kitsune says as she leans over your slumped form, grinning coyly. “<i>If you were going to give up so easily, you could have said so in the beginning and saved yourself the trouble! I was worried someone else would get to you first but it looks like I have you all to myself for a while.</i>” \n"
                ...
                + "\n"
                + "\"<i>Fighting for so long must've been making all that delicious soul force of yours slosh about."
                + ((player.hasBalls()) ? " I can practically hear the bubbling sound of the energy in churning your balls." : "")
                + " Don't worry I'm going to relieve you of most of it.</i>\" She licks her lips hungrily before unravelling her kimono as four of her tails wraps around your arms and limb to hold you in place. \n"
                + "\n"
        ...
        if (player.hasBalls())
        {
            outputText("The reaction is as extreme as it is pleasurable as the flames directly lick at your balls causing them to suddenly inflate like giant balloons, your eyes going white as you spontaneously cum enough shpongle to bathe both you and her in semen! You shoot jet after jet directly in the vixen pussy yet she never seems to inflate! Just where is all of your cum going. Just as your balls finally shrinks back to their normal size you mercifully begin to pass out from exhaustion.");
        }
        else
        {
