...
class EvangelineFollower extends NPCAwareContent implements SaveableState
{
    public static var EvangelinePeepTalkOnInternalMutations : Float;
    public static var EvangelineGemsPurse : Float;
    public static var EvangelineTalks : Float;
    public static var EvangelineAffectionMeter : Float;
    public static var EvangelineFollowerStage : Float;
    public static var EvangelineVirginity : Bool;
    public static var EvangelineSex1 : Bool;
    public static var EvangelineCuringArigeanInfection : Float;
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            EvangelinePeepTalkOnInternalMutations = Reflect.field(o, "EvangelinePeepTalkOnInternalMutations");
            EvangelineGemsPurse = Reflect.field(o, "EvangelineGemsPurse");
            EvangelineTalks = Reflect.field(o, "EvangelineTalks");
            EvangelineAffectionMeter = Reflect.field(o, "EvangelineAffectionMeter");
            EvangelineFollowerStage = Reflect.field(o, "EvangelineFollowerStage");
            EvangelineVirginity = valueOr(Reflect.field(o, "EvangelineVirginity"), false);
            EvangelineSex1 = valueOr(Reflect.field(o, "EvangelineSex1"), false);
            EvangelineCuringArigeanInfection = valueOr(Reflect.field(o, "EvangelineCuringArigeanInfection"), 0);
        }
    ...
    public function evangelineAffection(changes : Float = 0) : Float
    {
        ...
        if (flags[kFLAGS.EVANGELINE_LVL_UP] < 5 && EvangelineAffectionMeter > AffCap)
        {
            EvangelineAffectionMeter = AffCap;
        }
        else if (EvangelineAffectionMeter < 0)
        {
            EvangelineAffectionMeter = 0;
        }
    ...
    public function meetEvangeline() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.ArigeanInfected) && player.statusEffectv3(StatusEffects.ArigeanInfected) == 0)
        {
            curingArigean1();
        }
        if (!player.hasStatusEffect(StatusEffects.ArigeanInfected) && player.statusEffectv3(StatusEffects.ArigeanInfected) == 0 && player.tailType == Tail.ARIGEAN_GREEN)
        {
            curingArigean2();
        }
        if (!player.hasStatusEffect(StatusEffects.ArigeanInfected) && player.statusEffectv3(StatusEffects.ArigeanInfected) == 0 && player.tailType == Tail.ARIGEAN_RED)
        {
            curingArigean3();
        }
        if (!player.hasStatusEffect(StatusEffects.ArigeanInfected) && player.statusEffectv3(StatusEffects.ArigeanInfected) == 0 && player.tailType == Tail.ARIGEAN_YELLOW)
        {
            curingArigean4();
        }
        ...
        {
            addButton(2, "Sex", evangelineSexMenu).hint("Have some sex with the demonic chimera girl.");
        }  //godess  
        if (EvangelineAffectionMeter >= 5)
        {
            addButton(3, "Spar", evangelineSparMenu).hint("Get into a quick battle with Evangeline!").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, "You need a good sparring ring for that.");
            if (flags[kFLAGS.EVANGELINE_LVL_UP] >= 1)
            {
                addButton(4, "Give Gems", LvLUp).hint("Give Evangeline some gems to cover her expenses on getting stronger.");
            }
            else
            {
                addButtonDisabled(4, "Give Gems", "Req. sparring with Evangeline at least once.");
            }
        ...
        if (flags[kFLAGS.EVANGELINE_LVL_UP] >= 5)
        {
            addButton(9, "Experiments", Experiments).hint("Check on what experiments Evangeline can work on.");
        }
        //menu do eksperymentow alchemicznych jak tworzenie eksperymentalnych TF lub innych specialnych tworow evangeline typu specjalny bimbo liq lub tonik/coskolwiek nazwane wzmacniajace postacie do sparingu w obozie
        else
        {
            addButtonDisabled(9, "???", "Req. Evangeline been lvl 16+.");
        }
        if (player.hasStatusEffect(StatusEffects.ArigeanInfected) || player.tailType == Tail.ARIGEAN_GREEN || player.tailType == Tail.ARIGEAN_RED || player.tailType == Tail.ARIGEAN_YELLOW || player.tailType == Tail.ARIGEAN_PRINCESS)
        {
            addButton(10, "Arigean I.", curingArigeanMain);
        }
        else
        {
            addButtonDisabled(10, "???", "Req. to be infected by Arigean.");
        }
        if (player.hasPerk(PerkLib.WendigoCurse))
        {
            if (player.perkv1(PerkLib.WendigoCurse) > 0)
            {
                ...
                {
                    addButton(11, "Wendigo", curingWendigo);
                }
                else
                {
                    addButtonDisabled(11, "Wendigo", "Req. five pure peaches and five purity philters to fix your 'issue'.");
                }
            }
            else
            {
        ...
        {
            addButtonDisabled(11, "???", "Req. to be cursed by Wendigo.");
        }
        if (flags[kFLAGS.CURSE_OF_THE_JIANGSHI] == 2 || flags[kFLAGS.CURSE_OF_THE_JIANGSHI] == 3)
        {
            if (flags[kFLAGS.CURSE_OF_THE_JIANGSHI] == 2)
            {
                addButton(12, "Jiangshi", curingJiangshi);
            }
            else if (player.hasItem(consumables.VITAL_T, 5) && player.hasItem(consumables.PPHILTR, 5))
            {
                addButton(12, "Jiangshi", curingJiangshi);
            }
            else
            {
                addButtonDisabled(12, "Jiangshi", "Req. five vitality tinctures and five purity philters to fix your 'issue'.");
            }
        }
        else
        {
            addButtonDisabled(12, "???", "Req. to be Jiangshi.");
        }
        if (player.hasKeyItem("Soul Gem Research") >= 0)
        {
            if (player.statusEffectv1(StatusEffects.SoulGemCrafting) == 0)
            {
                addButton(13, "Soul Gem", receivingCraftedSoulGem).hint("Pick up crafted Soul Gem.");
            }
            if (!player.hasStatusEffect(StatusEffects.SoulGemCrafting))
            {
                addButton(13, "Soul Gem", craftingSoulGem).hint("Ask Evangeline for crafting Soul Gem.");
            }
        }
        else
        {
            addButtonDisabled(13, "???", "Req. to acquire Soul Gem Research materials first.");
        }
    ...
    private function evangelineAppearance() : Void
    {
        ...
        {
            outputText("eight feet");
        }
        else
        {
            outputText("seven and a half feet");
        }
        outputText(" tall.\n\n");
        ...
        if (flags[kFLAGS.EVANGELINE_LVL_UP] >= 10)
        {
            outputText("hair are ass-length along with breats that could easily fill a F-cup bra, expansive ass and fertile hips.\n\n");
        }
        else if (flags[kFLAGS.EVANGELINE_LVL_UP] >= 5)
        {
            outputText("hair are ass-length along with breats that could easily fill a E-cup bra, jiggly ass and curvy hips is quite a change that resulted from her drinking bimbo liquer personaly modified by her.\n\n");
        }
        else
        {
            outputText("hair short along with breasts not bigger than A cup, almost non-existent butt and boyish hips cause her to look quite tomboyish.\n\n");
        }
        //outputText(".\n\n");
        outputText("She's wearing ");
        if (flags[kFLAGS.EVANGELINE_LVL_UP] >= 8)
        {
            outputText("a skimpy chain bikini that barely qualifies as armor. The chain is made from links much finer and lighter than normal, so fine that it feels almost silken under fingertips. A simple seal in the g-string-like undergarment states, \"Virgins only.\"");
        }
        else if (flags[kFLAGS.EVANGELINE_LVL_UP] >= 3)
        {
            outputText("a suit of steel 'armor' which have two round disks that barely cover her nipples, a tight chainmail bikini, and circular butt-plates.");
        }
        else
        {
            outputText("a simple peasant’s robe that’s torn and frayed in a few places.");
        }
    ...
    private function evangelineTalkMenu() : Void
    {
        ...
        if (EvangelineTalks >= 1)
        {
            addButton(1, "Demons", TalkDemons).hint("Ask about demons in general.");
        }
        else
        {
            addButtonDisabled(1, "???", "???");
        }
        if (EvangelineTalks >= 2)
        {
            addButton(2, "Past Life", TalkPastLife1).hint("Talk about her past before meeting you.");
        }
        else
        {
            addButtonDisabled(2, "???", "???");
        ...
        {
            if (EvangelineAffectionMeter >= 30)
            {
                addButton(3, "Father", TalkYourFather).hint("Talk about her father.");
            }
            else
            {
                addButtonDisabled(3, "???", "Req. 30%+ affection");
            }
        }
        else
        {
    ...
    private function TalkYourEyes() : Void
    {
        ...
        if (EvangelineTalks >= 1)
        {
            outputText("\"<i>So you once again want talk about it? Let it be,</i>\"");
        }
        else
        {
            outputText("\"<i>I was kinda expecting that you would ask about them eventually,</i>\"");
        }
    ...
    private function TalkDemons() : Void
    {
        ...
        if (EvangelineTalks >= 2)
        {
            outputText("but one or two thing are still new to you");
        }
        else
        {
            outputText("but a few things you’re hearing for the first time");
        }
    ...
    private function TalkPastLife1() : Void
    {
        ...
        if (EvangelineTalks >= 3)
        {
            outputText("once again fails");
        }
        else
        {
            outputText("turns out to be futile");
        }
    ...
    private function TalkYourFather() : Void
    {
        ...
        if (EvangelineTalks >= 4)
        {
            outputText("\"<i>Hahahaha, so you want hear that stories about me and my father again? Well, not like I got so 'much more important' things to do now.</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Hmmm. My father...</i>\" Evangeline muses to herself when she hears your reason to finding her today. \"<i>Well I've been living pretty good here for some time, and you seem to be not so untrustworthy like many others... fine.</i>\"\n\n");
        }
        ...
        {
            flags[kFLAGS.DILAPIDATED_SHRINE_UNLOCKED] = 1;
        }
        evangelineAffection(1);
        if (EvangelineTalks == 3)
        {
...
*/  
    ...
    private function evangelineSparMenu() : Void
    {
        ...
        {
            outputText("You suggest a sparring session.  Evangeline after a moment of hesistation nod in agreement.");
        }
        menu();
        addButton(0, "Light", LightSpar);
        if (EvangelineAffectionMeter >= 50)
        {
            addButton(1, "Hard", HardSpar);
        }
        addButton(14, "Back", meetEvangeline);
    }
    ...
    private function LightSpar() : Void
    {
        ...
        if (flags[kFLAGS.EVANGELINE_LVL_UP] >= 8)
        {
            outputText("lusty maiden's armor");
        }
        if (flags[kFLAGS.EVANGELINE_LVL_UP] >= 3 && flags[kFLAGS.EVANGELINE_LVL_UP] < 8)
        {
            outputText("practically indecent steel armor");
        }
        if (flags[kFLAGS.EVANGELINE_LVL_UP] < 3)
        {
            outputText("rags");
        ...
        {
            flags[kFLAGS.EVANGELINE_LVL_UP] = 1;
        }
    ...
    private function ingredientsMenu() : Void
    {
        ...
        addButton(0, "D.E-Ichor", MakeDEIchor).hint("Ask Evangeline to prepare a diluted version of E-Ichor").disableIf(!player.hasItem(useables.E_ICHOR), "++\n\n<b>You don't have any E-Ichor!</b>").disableIf(emptySlot < 0, "++\n\n<b>You don't have free space in your inventory!</b>");
        addButton(14, "Back", meetEvangeline);
    }
    ...
    private function LvLUp() : Void
    {
        ...
        if (player.gems > 1)
        {
            addButton(0, "1 gem", Give1Gem).hint("Give Evangeline 1 gem.");
        }
        if (player.gems > 10)
        {
            addButton(1, "10 gems", Give10Gems).hint("Give Evangeline 10 gems.");
        }
        if (player.gems > 100)
        {
            addButton(2, "100 gems", Give100Gems).hint("Give Evangeline 100 gems.");
        }
        if (flags[kFLAGS.EVANGELINE_LVL_UP] == 1 && EvangelineGemsPurse >= 150)
        {
            addButton(5, "Just Do It!", JustDoIt).hint("JUST DO IT!");
        }
        if (flags[kFLAGS.EVANGELINE_LVL_UP] == 2 && EvangelineGemsPurse >= 450)
        {
            addButton(5, "Just Do It!", JustDoIt).hint("JUST DO IT!");
        }
        if (flags[kFLAGS.EVANGELINE_LVL_UP] == 3 && EvangelineGemsPurse >= 800)
        {
            addButton(5, "Just Do It!", JustDoIt).hint("JUST DO IT!");
        }
        if (flags[kFLAGS.EVANGELINE_LVL_UP] == 4 && EvangelineGemsPurse >= 800)
        {
            ...
            {
                addButtonDisabled(5, "Just Do It???", "IF you had Rathazul in camp you could JUST DO IT!");
            }
        }
        if (flags[kFLAGS.EVANGELINE_LVL_UP] == 5 && EvangelineGemsPurse >= 400)
        {
            addButton(5, "Just Do It!", JustDoIt).hint("JUST DO IT!");
        }
        ...
        if (flags[kFLAGS.EVANGELINE_LVL_UP] >= 5)
        {
            addButton(13, "Experiments", Experiments).hint("Check on what experiments Evangeline can work on.");
        }
        addButton(14, "Back", meetEvangeline);
    ...
    private function curingArigeanMain() : Void
    {
        ...
        {
            addButtonIfTrue(0, "Arigean I.", curingArigeanYes, "Req. 750 gems.", player.gems >= 750);
        }
        else
        {
    ...
    private function curingArigeanYes() : Void
    {
        ...
        if (player.tailType == Tail.ARIGEAN_GREEN)
        {
            ...
            if (EvangelineCuringArigeanInfection == 0)
            {
                EvangelineCuringArigeanInfection += 1;
            }
    ...
    private function curingArigean2a() : Void
    {
        ...
        if (flags[kFLAGS.THE_TRENCH_ENTERED] >= 1)
        {
    ...
    private function curingJiangshi() : Void
    {
        ...
        if (flags[kFLAGS.CURSE_OF_THE_JIANGSHI] == 3)
        {
            ...
            if (player.shield.isNothing && flags[kFLAGS.PLAYER_DISARMED_SHIELD_ID] != 0)
            {
                ...
                {
                    player.setShield(try cast(ItemType.lookupItem(flags[kFLAGS.PLAYER_DISARMED_SHIELD_ID]), Shield) catch(e:Dynamic) null);
                }
                flags[kFLAGS.PLAYER_DISARMED_SHIELD_ID] = 0;
            }
            if (player.armor == armors.TRADITC && flags[kFLAGS.PLAYER_DISARMED_ARMOR_ID] != 0)
            {
                player.setArmor(try cast(ItemType.lookupItem(flags[kFLAGS.PLAYER_DISARMED_ARMOR_ID]), Armor) catch(e:Dynamic) null);
            ...
            {
                player.setUnderBottom(try cast(ItemType.lookupItem(flags[kFLAGS.PLAYER_DISARMED_UNDERWEAR_BOTTOM_ID]), Undergarment) catch(e:Dynamic) null);
            ...
            {
                player.setUnderTop(try cast(ItemType.lookupItem(flags[kFLAGS.PLAYER_DISARMED_UNDERWEAR_UPPER_ID]), Undergarment) catch(e:Dynamic) null);
            ...
            {
                player.setHeadJewelry(try cast(ItemType.lookupItem(flags[kFLAGS.PLAYER_DISARMED_HEAD_ACCESORY_ID]), HeadJewelry) catch(e:Dynamic) null);
    ...
    private function Experiments() : Void
    {
        ...
        {
            addButtonDisabled(0, "BL/BB Plus", "Progress require sacrif... test subjects. Like feral imps...");
        }
        else
        {
            addButton(0, "BL/BB Plus", ExperimentsBLBBPlus).hint("Bimbo Liquer Plus / Bro Brew Plus");
        }
    ...
    private function ExperimentsBLBBPlus() : Void
    {
        ...
        if (flags[kFLAGS.GALIA_AFFECTION] == 1)
        {
            ...
            menu();
            if (EvangelineGemsPurse >= 500)
            {
                addButton(1, "Imp Subject", ExperimentsBLBBPlusImpSubject).hint("Find out what happens to another creature.");
            }
            else
            {
                addButtonDisabled(1, "Imp Subject", "Req. Evangeline having over 500 gems in her purse.");
            }
            if (player.hasItem(consumables.BIMBOLQ, 1) || player.hasItem(consumables.BROBREW, 1))
            {
                addButton(2, "Yourself", ExperimentsBLBBPlusYourself).hint("See what’s in store for yourself.");
            }
            else
            {
                addButtonDisabled(2, "Yourself", "You need something to transform yourself with!");
            }
        ...
        else
        {
            ...
            if (player.hasItem(consumables.BIMBOLQ, 1))
            {
                ...
                {
                    addButtonDisabled(1, "Bimbo", "Can't have Bro Body/Futa Form.");
                }
                else
                {
                    addButton(1, "Bimbo", ExperimentsBLBBPlusBimbo).hint("Ask Evangeline if she can make a new sample of that special drink.");
                }
            }
            if (player.hasItem(consumables.BROBREW, 1))
            {
                if (player.hasPerk(PerkLib.BimboBody) || player.hasPerk(PerkLib.FutaForm))
                {
                    addButtonDisabled(2, "Bro", "Can't have Bimbo Body/Futa Form.");
                }
                else
                {
                    addButton(2, "Bro", ExperimentsBLBBPlusBro).hint("Ask Evangeline if she can make a new sample of that special drink.");
                }
            }
    ...
    private function ExperimentsBLBBPlusYourself() : Void
    {
        ...
        if (player.hasItem(consumables.BIMBOLQ, 1))
        {
            addButton(1, "Bimbo", ExperimentsBLBBPlusYourselfBimbo).disableIf(player.hasPerk(PerkLib.BroBody) || player.hasPerk(PerkLib.FutaForm), "Can't have Bro Body/Futa Form.");
        }
        if (player.hasItem(consumables.BROBREW, 1))
        {
            addButton(2, "Bro", ExperimentsBLBBPlusYourselfBro).disableIf(player.hasPerk(PerkLib.BimboBody) || player.hasPerk(PerkLib.FutaForm), "Can't have Bimbo Body/Futa Form.");
        }
    ...
    private function ExperimentsBLBBPlusYourselfBroGetDirty() : Void
    {
        ...
        {
            outputText("She gags slightly as she holds onto you for dear life, \"<i>Gosh… so… much… cum… how do you..?</i>\" ");
        }
        else if (player.cumQ() >= 3000)
        {
            outputText("Her stomach distends slightly from the sheer volume of your massive load. ");
        }
        ...
        if (!EvangelineVirginity)
        {
            EvangelineVirginity = true;
        }
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    private function InternalMutations() : Void
    {
        ...
        var IMutationsYN : Bool->Void = function(yn : Bool = true) : Void
        {
            if (yn)
            {
        ...
        var IMutationsGemsOrMutagen : Int->Void = function(costType : Int = 0) : Void
        {
    ...
    private function IMutationsSelector(page : Int = 0) : Void
    {
        ...
        var mutationsAssistant : Array<Dynamic>->Int->Void = function(pArray : Array<Dynamic>, menuButton : Int = 0) : Void
        {
            var bd : ButtonDataList = new ButtonDataList();
            var bdFunc : Dynamic;
            var bdDesc : String;
            var target : Dynamic = player;
            for (mutations in pArray)
            {
                bdFunc = null;
                mutations.pReqs();
                ...
                var mutagenBypass : Bool = (GoM == 2 && (((!mutations.available(target))) ? player.hasItem(useables.E_ICHOR, 2) : true) && (!player.blockingBodyTransformations()) && ((target.hasMutation(mutations)) ? true : player.maxCurrentMutationsInSlot(mutations.slot)));
                if ((flags[kFLAGS.EVA_MUTATIONS_BYPASS] != null || mutations.available(target) || mutagenBypass) && mutations.maxLvl > target.perkv1(mutations))
                
                //last bit retains the blocking max mutation level.{
        ...
        var costTaker : Bool->Void = function(mutagenBypassDoubledCost : Bool) : Void
        {
            if (GoM == 1)
            {
                player.gems -= 500;
            ...
            {
                player.destroyItems(useables.E_ICHOR, ((mutagenBypassDoubledCost)) ? 2 : 1);
            }
            menu();
            clearOutput();
