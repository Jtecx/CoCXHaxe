...
class Raphael extends NPCAwareContent implements TimeAwareInterface
{
    public var dressTimerDisabled(get, never) : Bool;
    ...
    private function get_dressTimerDisabled() : Bool
    {
    ...
    public function timeChange() : Bool
    {
        ...
        if (flags[kFLAGS.RAPHAEL_DRESS_TIMER] > 1 && player.gems >= 5)
        {
            flags[kFLAGS.RAPHAEL_DRESS_TIMER]--;
        }
        if (flags[kFLAGS.RAPHEAL_COUNTDOWN_TIMER] > 1 && player.gems >= 5)
        {
            flags[kFLAGS.RAPHEAL_COUNTDOWN_TIMER]--;
        }
        //Fix 'hangs' - PC is at the bottom of the dress countdown
        if (flags[kFLAGS.RAPHAEL_DRESS_TIMER] == 1 && flags[kFLAGS.RAPHEAL_COUNTDOWN_TIMER] == 0 && RaphaelLikes())
        {
            flags[kFLAGS.RAPHAEL_DRESS_TIMER] = 4;
        }
        return false;
    }
    ...
    {
        if (checkedRussetRogue++ == 0 && model.time.hours == 6 && flags[kFLAGS.RAPHEAL_COUNTDOWN_TIMER] >= 0 && player.hasKeyItem("Camp - Chest") >= 0 && player.gems >= 5 && player.statusEffectv1(StatusEffects.TelAdre) >= 1 && flags[kFLAGS.IN_INGNAM] == null)
        {
    ...
    override public function RaphaelLikes() : Bool
    ...
    private function meetRaphaelPtII() : Void
    {
        ...
        {
            sceneHunter.print("The only thing you need to fuck Raphael is a vagina. Just don't disable Low Standards or things will blow up.");
        }
        else
        {
            sceneHunter.print("Okay, for Raphael to like you, you need to have: bipedal body, tits C-E, medium-sized hips, height 4-6 feet, a vagina, no cocks or balls... or just enable Low Standards and let his writer go cry in their pillow.");
        }
        if (!RaphaelLikes())
        {
            sceneHunter.print("You'd better reload if you still want him. There's no second chance.");
        }
        //Requirements: .
        ...
        addButton(3, "Rape", rapeRaphael).disableIf(player.lust < 33, "Tsk. Not aroused enough.").disableIf(!player.hasCock(), "Req. a cock!").disableIf(player.cor < 60 - player.corruptionTolerance, 
    ...
    private function RaphaelFirstMeetingSLAP() : Void
    {
        ...
        if (RaphaelLikes())
        {
            outputText("\"<i>You might look like one, but verily, you are not a lady.  I would advise you to gain manners, but sadly, a hag cannot be taught female grace with any greater aptitude than a pig can be taught to dine with silverware. You will remain at best, a very curvy mangirl.</i>\"\n\n");
        }
        //else outputText("\"<i></i>\"");
    ...
    private function RaphaelFirstMeetingSWOON() : Void
    {
        ...
        if (!recalling)
        {
            ...
            player.trainStat("spe", 3, player.trainStatCap("spe", 100));
            doNext(playerMenu);
        }
        else
        {
    ...
    private function rapeRaphael() : Void
    {
        ...
        /*you CAN'T do all-way through with 1-inch dick :) */if (player.findCockWithType(CockTypesEnum.TENTACLE, 1, 48, -1, "length") >= 0)
        {
    ...
    //{Encounter two}
    {
        ...
        else
        {
            ...
            {
                outputText("\"<i>Your female curves... replaced with such... udders!</i>\" He looks at your bosom. \"<i>No woman could be elegant with such monstrosities up front!</i>\"\n\n");
            }
            //({If player has below C cup breasts}
            if (player.biggestTitSize() < 3)
            {
                outputText("\"<i>Your female curves... gone!</i>\" He looks at your bosom. \"<i>It's hard to tell you apart from a little girl!</i>\"\n\n");
            }
            //({If player has grown less than girly hips}
            if (player.hips.type < 6)
            {
                outputText("\"<i>What happened to that fine hourglass shaped figure? Those comely hips?</i>\"\n\n");
            }
            //({If player has gotten a massive butt}
            if (player.butt.type >= 13)
            {
                outputText("\"<i>Oh... my... Marae! [name], look at your butt. It is so big! You look one of those cat guy's girlfriends. Who understands those cat guys? You look like a total prostitute. I mean, your butt. It's just so big. I can't believe it's just so round and so out there! Gross!</i>\"  The fox shakes his head and breaks it down. \"<i>I hate big butts! So vulgar.</i>\"\n\n");
            }
            //({If female player has gotten bigger than 6 feet}
            if (player.tallness > 72)
            {
                outputText("Raphael rolls his eyes across your giant body and looks intimidated. \"<i>I can forget about remaining unseen when I take someone of your size somewhere!</i>\"\n\n");
            }
            //({If female player has gotten smaller than 4 feet}
            if (player.tallness < 48)
            {
                outputText("Raphael squints like he has trouble seeing you from there, because of your dimunitive size. \"<i>I've dated goblins once... didn't work out.</i>\"\n\n");
            }
            //({If player has grown ANY cock and balls}
            if (player.hasBalls() || player.cockTotal() > 0)
            {
                outputText("\"<i>What is that bulge below the tight outfit I gave you?</i>\" The fox inspects your groin. \"<i>No, never mind. I don't want to know.</i>\"\n\n");
            }
            //(For now:
            //({If player has lost all gender}
            if (player.gender == 0)
            {
                outputText("\"<i>There's... something different about you today. Your smell, it has changed.</i>\"\n\n");
            }
    ...
    private function RaphaelChooseFrisk() : Void
    {
        ...
        else if ((player.inte < 36 && player.spe < 36) && player.cor < 36)
        {
            ...
            if (player.cor <= 15)
            {
                outputText("You have the stronge urge to grab hold of the oddly enlarged bulge between his legs and retrieve the necklace from within his pants, but the lewdness of the gesture keeps you from it.\n\n");
            }
            //{If player corruption is at or higher than 15}
            else
            {
                outputText("With no intention to relent, you grab him by the oddly enlarged bulge in his tight leather pants and squeeze softly, discovering how several of his jewels are harder than others. He hid your gem amongst his own!\n\n");
            }
    ...
    public function RaphaelPicnicSkill() : Void
    {
        ...
        else if (!recalling && player.spe < 39 || flags[kFLAGS.RAPHAEL_RAPIER_TRANING] == 1)
        {
            ...
            outputText("When he picks you back up, he also appears to have picked up his rapier.  Within the hold of both your hands, a firm hilt is now felt and your swirls are accompanied by the sound of swishing metal. When he leads you off into a mutual sideward step across the marsh, it looks as though both of you are following the rapier's tip as a united entity.\n\n");
    ...
    private function fenceOfferChangeToDiscuss() : Void
    {
        ...
        cast((false), RaphaelPicnicChooseThieving);
    }
    ...
    private function finishTraining(fucked : Bool = true) : Void
    {
        if (recalling)
        {
            recallWakeUp();
    ...
    public function RaphaelPicnicChooseThieving(newl : Bool = true) : Void
    {
        if (newl)
        {
            clearOutput();
    ...
    private function thieveryEnding() : Void
    {
        ...
        if (flags[kFLAGS.RAPHAEL_RAPIER_TRANING] != 4)
        {
            outputText("You try to reach for the magnificent ornament hanging off his hips, but as the fox continues to move, you lack the nimbleness to remove it.\n\n");
        }
        //({When player has also reached the apex of fencing lessons
        else
        {
            outputText("You reach and fondle the magnificent ornament hanging off his hips. With a few nimble flicks of your fingers, you manage to get it off.\n\n");
        }
    ...
    private function RaphaelThieverySmex() : Void
    {
        ...
        outputText("It is how you spend the rest of that morning, filled a thousand times over and constantly driven past the edge of orgasmic bliss by the master fencer's trained thrusts. His civilized smile has long since given way to the mean smirk of a sexual victor, driving his prey to the edge of madness.");
    ...
    private function quiksilverFawkesEndGame() : Void
    {
        ...
        if (flags[kFLAGS.RAPHAEL_FUCKED] == 1)
        {
    ...
    private function QuiksilverFawkesPtII() : Void
    {
        ...
        if (player.armorName == "red, high-society bodysuit")
        {
            outputText("  Suddenly, his eyes and ears perk.  <i>\"Isn't that the stolen clothing of the widow Camembert that she's wearing? You know, before she mysteriously dropped the charges after a nightly visit from the fox? Should we confiscate it anyway?\"</i>");
        }
        outputText("\n\n");
    ...
    private function coverForRaphael() : Void
    {
        ...
        if (player.cor < 30)
        {
            outputText("  You slowly shake your head, tortured by the trusting twinkle in the wolf's soft yellow eyes.");
        }
        outputText("\n\n");
...
*/
    ...
    private function kissRaphaelFirstTimeOrphanage() : Void
    {
        ...
        outputText("\n\nThere's a brief moment of vulnerability, as he finds an opening into your pants.  As sure as a master thief would find his way into the vault of a city, he quickly opens it.  The thorns at your back do bite while you hang limply between trunk and fox with your privates exposed midair, but you are soon relieved of your awkward neglect when Raphael tilts his hips and drives deep a much larger implement.  Finally the embrace is whole again as you can feel his pelvis thump in against your netherlips. His bone-hard length has already breached you.");
    ...
    private function cunnilingusWithRaphael() : Void
    {
        ...
        if (player.wetness() < 3)
        {
            outputText("your cunt fluttering");
        }
        //({Medium wetness multiplier}
        else if (player.wetness() < 4)
        {
            outputText("sending out a small spurt of femcum landing on your belly");
        }
        //({High wetness multiplier}
        else
        {
            outputText("gushing a torrent of femcum into the air");
        }
