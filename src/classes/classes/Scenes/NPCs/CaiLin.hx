...
class CaiLin extends Monster
{
    override private function performCombatAction() : Void
    {
        ...
        if (choice == 4)
        {
            ...
            else if (player.hasStatusEffect(StatusEffects.AbilityCooldown1))
            {
                if (rand(2) == 0)
                {
                    medusaPoisonBiteAttack();
    ...
    public function TailWhip() : Void
    {
        ...
        else
        {
            ...
            if (player.armorDef < 100)
            {
                damage += 100 - player.armorDef;
            }
            damage += rand(30);
    ...
    {
        var cost : Float = 40;
        if (flags[kFLAGS.CAILIN_SPELLS_CASTED] >= 10)
        {
            cost -= 4;
        }
        if (flags[kFLAGS.CAILIN_SPELLS_CASTED] >= 30)
        {
            cost -= 4;
        }
        if (flags[kFLAGS.CAILIN_SPELLS_CASTED] >= 70)
        {
            cost -= 4;
        }
        if (flags[kFLAGS.CAILIN_SPELLS_CASTED] >= 150)
        {
            cost -= 4;
        }
        if (flags[kFLAGS.CAILIN_SPELLS_CASTED] >= 310)
        {
            cost -= 4;
        }
        return cost;
    ...
    {
        if (fatigue < (maxFatigue() - spellCostWhitefire()))
        {
            WhiteFireSpell();
        }
        //później jak dodam ice, lighting spells rozbudować wybór zakleć
        else
        {
            eAttack();
        }
    ...
    public function new()
    {
        ...
        this.fatigue = 0;
        this.gems = 0;
        this.drop = new WeightedDrop().add(null, 1).add(consumables.REPTLUM, 5).add(consumables.GORGOIL, 4);
