...
class JinxFollower extends NPCAwareContent
{
    ...
    public function bazaarEncounters() : Void
    {
        ...
        if (flags[kFLAGS.JINX_LVL_UP] >= 0.8)
        {
            addButton(4, "You", bazaarEncountersYouRepeat).hint("That a weird item to sell... but you all there behind [name] have very weird minds.");
        }
        else
        {
            addButton(4, "You", bazaarEncountersYou).hint("That a weird item to sell... but you all there behind [name] have very weird minds.");
        }
    ...
    private function buyItemMinoBomber() : Void
    {
        ...
        menu();
        if (flags[kFLAGS.SPIRIT_STONES] < 250)
        {
            addButtonDisabled(1, "Buy", "You do not have enough spirit stones to buy this.");
        }
        else
        {
            addButton(1, "Buy", buyItemMinoBomberYes);
        }
    ...
    {
        clearOutput();
        var cost : Int = as3hx.Compat.parseInt(520 / 4);
        ...
        else if (CampStatsAndResources.EnergyCoreResc >= 200)
        {
            addButtonDisabled(1, "Buy", "You can't store any more of this type of items.");
        }
        else
        {
            addButton(1, "Buy", curry(buyItemEnergyCoreYes, cost));
        }
        addButton(3, "Don't Buy", bazaarEncounters);
    }
    private function buyItemEnergyCoreYes(cost : Float) : Void
    {
    ...
    private function buyItemGolemCore() : Void
    {
        ...
        menu();
        if (flags[kFLAGS.SPIRIT_STONES] < 1)
        {
            addButtonDisabled(1, "Buy", "You do not have spirit stone to buy this.");
        }
        else
        {
            addButton(1, "Buy", buyItemGolemCoreYes);
        }
        addButton(3, "Don't Buy", bazaarEncounters);
    }
    ...
    private function buyItemMetalPlates(amt : Float) : Void
    {
        clearOutput();
        var cost : Int = as3hx.Compat.parseInt((60 * amt) / 4);
        outputText("\"<i>That will be... perhaps... " + cost + " spirit stones, yes definitely! It's how valuable they are. Show me da stones, baby!</i>\"\n\n");
        menu();
        if (flags[kFLAGS.SPIRIT_STONES] < cost)
        {
        ...
        else if (CampStatsAndResources.MetalPieces >= (201 - amt))
        {
            addButtonDisabled(1, "Buy", "You can't store any more of this type of items.");
        }
        else
        {
            addButton(1, "Buy", curry(buyItemMetalPlatesYes, cost, amt));
        }
        addButton(3, "Don't Buy", bazaarEncounters);
    }
    private function buyItemMetalPlatesYes(cost : Float, amt : Float) : Void
    {
        flags[kFLAGS.SPIRIT_STONES] -= cost;
        statScreenRefresh();
        outputText("She counts the stones carefully, putting them away before handing your purchase over.\n\n");
