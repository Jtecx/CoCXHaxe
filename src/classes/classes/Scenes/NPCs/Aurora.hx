...
class Aurora extends Monster
{
    public function soulskillCostTripleTrust() : Float
    {
        var cost : Float = 30;
        if (this.level >= 24)
        {
    ...
    public function soulskillCostSextupleTrust() : Float
    {
        var cost : Float = 70;
        if (this.level >= 24)
        {
    ...
    public function soulskillCostNonupleTrust() : Float
    {
        var cost : Float = 150;
        if (this.level >= 24)
        {
    ...
    public function tripletrust() : Void
    {
        ...
        {
            outputText(" You slide underneath the thrust!");
        }
        else
        {
    ...
    private function trustD() : Void
    {
        ...
        if (weaponAttack < 51)
        {
            td *= (1 + (weaponAttack * 0.04));
        }
        else if (weaponAttack >= 51 && weaponAttack < 101)
        {
            td *= (3 + ((weaponAttack - 50) * 0.035));
        }
        else if (weaponAttack >= 101 && weaponAttack < 151)
        {
            td *= (4.75 + ((weaponAttack - 100) * 0.03));
        }
        else if (weaponAttack >= 151 && weaponAttack < 201)
        {
            td *= (6.25 + ((weaponAttack - 150) * 0.025));
        }
        else
        {
            td *= (7.5 + ((weaponAttack - 200) * 0.02));
        }
        var crit : Bool = false;
        var critChance : Int = 5;
        if (this.inte <= 200)
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (flags[kFLAGS.AURORA_LVL] >= 9)
        {
            ...
            if (choice4 == 2)
            {
                if (this.soulforce >= soulskillCostNonupleTrust() && !hasStatusEffect(StatusEffects.AbilityCooldown3))
                {
                    nonupletrust();
                }
                else if (this.soulforce >= soulskillCostSextupleTrust() && !hasStatusEffect(StatusEffects.AbilityCooldown2))
                {
                    sextupletrust();
                }
                else if (this.soulforce >= soulskillCostTripleTrust())
                {
                    tripletrust();
                }
                else
        ...
        if (flags[kFLAGS.AURORA_LVL] >= 5 && flags[kFLAGS.AURORA_LVL] < 7)
        {
            ...
            if (choice2 == 2)
            {
                if (this.soulforce >= soulskillCostSextupleTrust() && !hasStatusEffect(StatusEffects.AbilityCooldown2))
                {
                    sextupletrust();
                }
                else if (this.soulforce >= soulskillCostTripleTrust())
        ...
        else
        {
            ...
            if (choice0 == 2)
            {
                if (this.soulforce >= soulskillCostTripleTrust())
                {
                    tripletrust();
                }
                else
    ...
    public function new()
    {
        ...
        if (flags[kFLAGS.AURORA_LVL] >= 5 && flags[kFLAGS.AURORA_LVL] < 8)
        {
            this.lustVuln = .4;
        }
        if (flags[kFLAGS.AURORA_LVL] >= 8 && flags[kFLAGS.AURORA_LVL] < 13)
        {
            this.lustVuln = .3;
        }
        if (flags[kFLAGS.AURORA_LVL] >= 13 && flags[kFLAGS.AURORA_LVL] < 18)
        {
            this.lustVuln = .2;
        }
        if (flags[kFLAGS.AURORA_LVL] >= 18)
        {
            this.lustVuln = .1;
        }
        this.a = "";
