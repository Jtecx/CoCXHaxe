...
class Valeria extends GooGirl
{
    public function gooArmorAI() : Void
    {
        if (rand(2) == 0 && !player.hasStatusEffect(StatusEffects.GooArmorSilence))
        {
            gooSilenceAttack();
        }
        else if (rand(3) > 0)
        {
            gooArmorAttackPhysical();
        }
        else
        {
            gooArmorAttackTwoGooConsume();
        }
    }
    ...
    public function gooArmorAttackPhysical() : Void
    {
        ...
        else
        {
            ...
            var damage : Float = Math.round((str + weaponAttack) - rand(player.tou) - player.armorDef);
            if (damage <= 0)
            {
                damage = 1;
            }
    ...
    {
        if (hasStatusEffect(StatusEffects.Spar) || CoC.instance.gameSettings.sceneHunter_inst._mocking)
        {
            SceneLib.valeria.pcWinsValeriaSpar();
        }
        else
        {
            SceneLib.dungeons.heltower.beatUpGooArmor();
        }
    }
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        ...
        {
            SceneLib.dungeons.heltower.gooArmorBeatsUpPC();
        }
    }
