...
class Iridesian extends Monster
{
    ...
    public function soulskillCostGrandioseHailofMoonBlades() : Float
    {
        var cost : Float = 800;
        if (hasPerk(PerkLib.DaoistApprenticeStage))
        {
    ...
    public function soulskillCostEnergyProjection() : Float
    {
        var cost : Float = 20;
        if (hasPerk(PerkLib.DaoistApprenticeStage))
        {
    ...
    public function castEnergyProjectionIridesian() : Void
    {
        ...
        var damage : Float = eBaseIntelligenceDamage();
    ...
    {
        var damage : Float = eBaseIntelligenceDamage() * 0.15;
        damage += eBaseWisdomDamage() * 0.15;
        damage = Math.round(damage);
        var element : Float = rand(5);
        if (element == 0)
        {
    ...
    private function AuraOfPurityIridesian() : Void
    {
        ...
        }  //30% more damage against very high corruption.  
        var crit : Bool = false;
    ...
    override private function performCombatAction() : Void
    {
        if (player.cor > 20)
        {
            AuraOfPurityIridesian();
        }
        if (!hasStatusEffect(StatusEffects.TranscendentSoulField) && soulforce >= 20)
        {
        ...
        else
        {
            ...
            if (choice == 1)
            {
                if (soulforce >= soulskillCostHailofBlades())
                {
                    castHailOfBladesSoulskillIridesian();
                }
                else
                {
                    IridesianOmnicast();
            ...
            if (choice == 2)
            {
                ...
                {
                    castGrandioseHailOfBladesSoulskillIridesian();
                }
                else
                {
                    IridesianOmnicast();
            ...
            if (choice == 3)
            {
                if (soulforce >= soulskillCostGrandioseHailofMoonBlades() && !hasStatusEffect(StatusEffects.AbilityCooldown3))
                {
                    castGrandioseHailOfMoonBladesSoulskillIridesian();
                }
                else
                {
                    IridesianOmnicast();
            ...
            if (choice == 4)
            {
                if (soulforce >= soulskillCostEnergyProjection())
                {
                    castEnergyProjectionIridesian();
                }
                else
                {
                    IridesianOmnicast();
    ...
    override private function get_long() : String
    {
        ...
        {
            str += "You are fighting an Oculicorn. This powerful creature look like a human save for the hooves, horse tail, the mono all encompassing eye in his head and six eye mounted stalks expending from his back. Medium long single horn adorn his forehead ocassionaly glowing with mild white light.";
        }
        if (hasStatusEffect(StatusEffects.TranscendentSoulField))
        {
    ...
    public function new()
    {
        ...
        this.lustVuln = .75;
        this.lust = 30;
        this.drop = new WeightedDrop().add(useables.SRESIDUE, 1).add(consumables.ME_DROP, 1);
