...
class UrtaPregs extends NPCAwareContent
{
...
*/
    ...
    {
        return SceneLib.urta.pregnancy;
    }
    ...
    {
        return as3hx.Compat.parseInt(flags[kFLAGS.URTA_KIDS_MALES] + flags[kFLAGS.URTA_KIDS_FEMALES] + flags[kFLAGS.URTA_KIDS_HERMS]);
    }
    ...
    private function urtaPregAppearance() : Void
    {
        var _sw34_ = (pregnancy.event);        

        switch (_sw34_)
        {
            case 1, 2:outputText("Urta is sitting at her usual table, looking more clear-eyed and sober than usual.  She looks extremely happy, tail wagging gently behind her, and she gives you a thrilled look when she sees you, smiling blissfully and patting her belly in a meaningful gesture.");
            case 3, 4, 5, 6, 7:outputText("Urta has moved from her usual table to a more private stall, apparently for the comfort of its padded seats.  Her bulging belly makes it obvious she's pregnant - and the wide grin that seems fixed to her face makes it obvious she's ecstatic about it.");
            case 8, 9:outputText("Urta's pregnancy has caused her to outgrow the stall; she's moved herself to an entirely new position in a quiet, out of the way part of the bar.  Looking at the sheer size of her, she's doubtlessly going to pop any day now.");
        }
    }
    
    //In Urta Menu
    @:allow(classes.scenes.nPCs)
    private function urtaPreggoApproached() : Void
    {
        clearOutput();
        menu();
        var _sw35_ = (pregnancy.event);        
    ...
    private function IAintGotTimeForUrtaBirfs() : Void
    {
        ...
        if (flags[kFLAGS.URTA_LATESTBORN_GENDER] == 1)
        {
            flags[kFLAGS.URTA_KIDS_MALES]++;
        }
        else if (flags[kFLAGS.URTA_LATESTBORN_GENDER] == 2)
        {
    ...
    public function getUrtaLetter() : Void
    {
        ...
        {
            outputText("hermaphrodite daughter");
        }
        outputText(", though.\n");
    ...
    //[=Go=]
    private function goVisitUrtaBirfs(withUrta : Bool = false) : Void
    {
        clearOutput();
        pregnancy.knockUpForce();  //Clear Pregnancy  
        flags[kFLAGS.URTA_PREGNANT_DELIVERY_SCENE] = 0;
        ...
        if (!withUrta)
        {
            outputText("The trip to the clinic where Urta is giving birth to your child is a blur; all that matters is getting there as fast as possible.  The centaur abandons you at the hospital doors, and you make your own way inside, approaching the counter inside.");
        }
        ...
        var gender : Int = as3hx.Compat.parseInt(rand(3) + 1);
        ...
        {
            outputText("boy...?  Well... isn't that unusual, looks like you actually gave birth to a healthy herm!");
        }
        outputText("</i>\"");
        ...
        if (gender == 1 || gender == 3)
        {
            ...
            else
            {
                ...
                if (gender == 1)
                {
                    outputText("boy when he grows up.");
                }
                else
                {
                    outputText("girl when she grows up.");
                }
                flags[kFLAGS.URTA_LATESTBORN_COCKTYPE] = 1;
            }
        }
        ...
        if (urtaKids() == 0)
        {
            outputText("overwhelmed by the fact she's a mother at long last");
        }
        else
        {
            outputText("still as delighted as ever to have a new addition to your growing brood");
        }
        outputText(".");
    ...
    private function wakeUpWithUrtaAfterStaying() : Void
    {
        ...
        if (flags[kFLAGS.URTA_KIDS_MALES] == 1)
        {
            outputText("son");
        }
        else
        ...
        if (player.cor < 50)
        {
            outputText("whimpering moan");
        }
        else
        {
            outputText("gasping shudder");
        }
        ...
        //1st time:
        if (flags[kFLAGS.URTA_TIMES_BIRTHED] == 1)
        {
            outputText("You look at her questioningly; she's breast-feeding, so she shouldn't be drinking alcohol.  \"<i>Oh, don't worry; even I don't drink booze in the morning.  It's a little pick-me-up of my own invention; totally non-alcoholic.  It's mostly fruit juice with a few spices thrown in.</i>\"  Urta laughs.  ");
        }
        ...
        if (urtaKids() == 2)
        {
            outputText("  The other kid is staying at Edryn's, so there's no risk of you two needing to answer... complicated questions.");
        }
        if (urtaKids() > 2)
        {
            outputText("  The other kids are staying at Edryn's, so there's no risk of you two needing to answer... complicated questions.");
        }
    ...
    public function urtaPregooUpdates(womb : Dynamic) : Bool
    {
        if (Reflect.field(womb, "incubation") == sceneHunter.adjustPregEventTimer(504, Reflect.field(womb, "type")))
        {
            outputText("\n<b>You're feeling a bit nauseated.  Your mind floats to Urta and you wonder if maybe this means her seed took...</b>\n");
            return true;
        }
        else if (Reflect.field(womb, "incubation") == sceneHunter.adjustPregEventTimer(432, Reflect.field(womb, "type")))
        {
        ...
        //3:
        else if (Reflect.field(womb, "incubation") == sceneHunter.adjustPregEventTimer(360, Reflect.field(womb, "type")))
        {
        ...
        else if (Reflect.field(womb, "incubation") == sceneHunter.adjustPregEventTimer(288, Reflect.field(womb, "type")))
        {
            ...
            {
                outputText("sounding somewhat jealous that it's you carrying and not her.");
            }
            else
            {
                outputText("sounding a little smug as she caresses her own gravid tummy.");
            }
        ...
        else if (Reflect.field(womb, "incubation") == sceneHunter.adjustPregEventTimer(216, Reflect.field(womb, "type")))
        {
            ...
            if (urtaKids() == 0)
            {
                outputText(" Oh, [name], we really are going to be parents...");
            }
    ...
    public function urtaIsAPregnantCopScene() : Void
    {
        ...
        {
            outputText("  Edryn's pregnant belly isn't helping her, either.");
        }
    ...
    public function urtaSpecialDeliveries() : Void
    {
        ...
        if (SceneLib.telAdre.edryn.pregnancy.isPregnant)
        {
            outputText(" and her pregnant belly jiggling from the motions");
        }
        ...
        if (SceneLib.telAdre.edryn.pregnancy.isPregnant)
        {
            outputText("as quickly as a pregnant mare can, anyway,");
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function talkToUrtaAboutHerHouse() : Void
    {
        clearOutput();
        ...
        if (player.cor < 50)
        {
            outputText(" despite it having some emotional value to you.");
        }
    ...
    private function yesUrtaHouseSex() : Void
    {
        ...
        if (player.lib > 50)
        {
            outputText("  Sometimes you almost give up on waiting to get to her apartment and instead strip and do her right then and there... but it wouldn't bode well for the Watch captain to get a fine for public indecency.");
        }
    ...
    private function preggoUrtaSmexOrSomething() : Void
    {
        ...
        var _sw36_ = (pregnancy.event);        

        switch (_sw36_)
        {
            case 7:outputText("dripping wetly onto the floor");
            case 8, 9:outputText("slopping into the filling bucket");
    ...
    private function massagePregnantUrtasBelly() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_MASSAGED_URTA_BELLY] == 0)
        {
            outputText("  She looks at you and her newly cream-adorned belly contemplatively.  \"<i>I never knew my sweet tooth would bring me to this...</i>\"");
        }
        ...
        {
            outputText(", a task that is made far easier due to your tongue's ability to stretch to a surprising length");
        }
    ...
    private function urtaMassageClimax() : Void
    {
        ...
        if (player.cor < 33)
        {
            outputText("  You walk up to her and carefully remove her condom, tying it up with a knot to prevent any of her seed from escaping, then dispose of it.");
        }
        //Medium Cor:
        else if (player.cor < 66)
        {
            outputText("Well, whatever, she's probably too sleepy to do anything about it either way.");
        }
        //High Cor:
        else
        {
            outputText("You walk up to her with a smirk and remove her condom, then pour down its contents over her belly, chuckling to yourself.  Now that's a great cream topping for your lovely fox-herm.");
        }
    ...
    private function urtaBoobWorshipScene() : Void
    {
        ...
        if (player.faceType == Face.SHARK_TEETH || player.faceType == Face.SNAKE_FANGS || player.faceType == Face.SPIDER_FANGS)
        {
            outputText(", always minding your sharp teeth");
        }
        ...
        if (player.hasCock())
        {
            outputText(", tasting a bit of yourself,");
        }
    ...
    private function nurseFromMommaUrtasBooBees() : Void
    {
        ...
        if (player.tongue.type > Tongue.HUMAN)
        {
            outputText("  Every few licks you coil your tongue around one of her nipples and gently pull at it, milking a thin strand of white goodness straight on your tongue, much to the fox-herm's delight.");
        }
    ...
    private function milkPregnantUrta() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("Urta promptly stops stroking your dick, though whether it's because she wants to be helpful or because she's annoyed you've stopped is anyone's guess.  ");
        }
    ...
    private function urtaPregCravings() : Void
    {
        ...
        {
            outputText(", which automatically causes milk to trickle forth");
        }
        outputText(".  \"<i>Nope, not it either...");
        if (player.lactationQ() >= 200)
        {
            outputText(" sweet, though.");
        }
        ...
        {
            outputText("The rest of your [cocks] are as erect as your [cock biggest] dribbling pre along their lengths as they slap Urta during her frenzied bobs.  ");
        }
    ...
    private function getBlownByPregnantUrta() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText("your balls churn and ");
        }
        ...
        if (player.cockTotal() > 1)
        {
            ...
            if (player.cockTotal() > 2)
            {
                outputText("s as they join");
            }
            else
            {
                outputText(" as it joins");
            }
    ...
    private function holdOnYouAintGettingYerCumYetYouNaughtyFox() : Void
    {
        ...
        if (player.cockTotal() > 1)
        {
            ...
            if (player.cockTotal() > 2)
            {
                outputText("s bulging ominously as they too prepare to deposit their loads on the willing fox-herm.");
            }
            else
            {
                outputText(" bulging ominously as it too prepares to deposit its load on the willing fox-herm.");
            }
        }
        ...
        if (player.cumQ() < 250)
        {
            ...
            if (player.cockTotal() > 1)
            {
                ...
                if (player.cockTotal() == 2)
                {
                    outputText(" convulses");
                }
                else
                {
                    outputText("s convulse");
                }
        ...
        else if (player.cumQ() < 1000)
        {
            ...
            if (player.cockTotal() > 1)
            {
                ...
                if (player.cockTotal() == 2)
                {
                    outputText(" contributes");
                }
                else
                {
                    outputText("s contribute");
                }
                outputText(" by making a complete mess of her as ");
                if (player.cockTotal() > 2)
                {
                    outputText("they randomly spatter");
                }
                else
                {
                    outputText("it randomly spatters");
                }
        ...
        else
        {
            ...
            if (player.cockTotal() > 1)
            {
                ...
                if (player.cockTotal() > 2)
                {
                    outputText("s shoot their loads");
                }
                else
                {
                    outputText(" shoots its load");
                }
            ...
            {
                outputText(", just above your knot,");
            }
    ...
    private function weirdUrtaCravingsConclusion() : Void
    {
        ...
        if (player.cumQ() < 1000)
        {
            outputText("completely clean.");
        }
        else
        {
            outputText("lacking the extra bit of paunch you gave her earlier.");
        }
    ...
    private function urtaRaepsJoo() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("Every time her swollen tip rubs against your prostate you feel droplets of pre escape your dilating cum-hole.  ");
        }
        if (player.hasVagina())
        {
            outputText("Each slap of her balls against your [vagina] sends electric waves of pleasure that expand throughout your body.  ");
        }
        ...
        if (player.cockTotal() > 0)
        {
            if (player.cockTotal() > 1)
            {
                outputText("  [EachCock] tenses in unison");
            }
            else
            {
                outputText("  [EachCock] throbs, tensing in preparation");
            }
            //(Low Cum Amount)
            if (player.cumQ() < 250)
            {
                outputText(" and you unload on the floor, spilling rope after rope of cum, forming a small puddle where you stand.  An undeniable testament to your enjoyment of the kinkiness of the act, however small.");
            }
            //(Medium Cum Amount)
            else if (player.cumQ() < 1000)
            {
                outputText(" and you deliver your payload right there on the floor.  Jet after jet splash against it, and you have no choice but to continue moaning and spilling your cum as Urta remains firmly lodged in your depths, no longer able to move now that you're squeezing her shaft so deliciously tight.  By the time you're done, the puddle you've formed is big enough that you can see your own face on the reflection, enraptured in pleasure and enjoyment of Urta's equine meatstick.");
            }
            //(High Cum Amount)
            else
            {
        ...
        if (player.hasVagina())
        {
            ...
            if (player.wetness() >= 4)
            {
                outputText(" to join your previous discharge on");
            }
            else
            {
                outputText(" and down towards");
            }
        ...
        if (player.gender == 0)
        {
            ...
            {
                outputText(" It even secretes its own wetness to slide lovingly down the fox-herm's balls and drip on the floor, an undeniable proof of your enjoyment.");
            }
        }
    ...
    private function partTwoOfUrtaBoning() : Void
    {
        ...
        if (player.cockTotal() > 1)
        {
            ...
            if (!player.isTaur())
            {
                outputText("  So close that you can almost feel its heat against your [cockHead].");
            }
    ...
    private function acceptUrtaRimJobbies() : Void
    {
        ...
        if (player.cockTotal() > 1)
        {
            outputText("  Especially not if your [cocks] have anything to say about it, throbbing at the mere feeling of Urta's wet tongue.");
        }
        else if (player.cockTotal() == 1)
        {
            outputText("  Especially not if your [cock] has anything to say about it, throbbing at the mere feeling of Urta's wet tongue.");
        }
        else if (player.hasVagina())
        {
            outputText("  Especially not if your [pussy] has any say in the matter, winking at Urta, hoping for a licking of its own.");
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function visitKidsFirstTime() : Void
    {
        clearOutput();
        ...
        if (urtaKids() == 1)
        {
            outputText("our child");
        }
        else
        {
            outputText("our babies");
        }
        ...
        if (flags[kFLAGS.URTA_TIME_SINCE_LAST_CAME] == 0)
        {
            outputText(", erection already diminishing as she forgets about her horniness in her parental pride");
        }
    ...
    private function pickKidSceneHere() : Void
    {
        ...
        {
            //Lianna can cook, its hard to make a mess when you have someone who knows what they're doing around. Then again Lianna is a mad scientist, so...if (model.time.hours >= 12 && model.time.hours <= 14)
            {
                hugAttackScene();
            }
            ...
            {
                //Randomly picked when selecting Kids option in Urta's house.if (rand(2) == 0)
                {
                    takeTheKidsForWalk();
                }
                //Bathtime
                ...
                {
                    //Randomly picked by selecting the Kids option in Urta's House.  //So far this is the ONLY option to use that button, so...  bathtime();
                }
            }
        }
    }
    ...
    private function newBabyComing() : Void
    {
        ...
        {
            outputText(num2Text(urtaKids()) + " kids");
        }
        ...
        if (urtaKids() < 8)
        {
            ...
            else if (flags[kFLAGS.URTA_FIRSTBORN_GENDER] >= 2)
            {
                outputText("her");
            }
            else
            ...
            if (urtaKids() == 1)
            {
                outputText("a small fluffy bundle is");
            }
            else
            {
                outputText("several small fluffy bundles are");
            }
            outputText(" doing ");
            if (urtaKids() == 1)
            {
    ...
    private function giveTheKidsABath() : Void
    {
        ...
        if (flags[kFLAGS.URTA_KIDS_MALES] > 0)
        {
            ...
            if (flags[kFLAGS.URTA_FIRSTBORN_GENDER] == 1)
            {
                ...
                if (flags[kFLAGS.URTA_KIDS_MALES] == 1)
                {
                    outputText("the only one,");
                }
                else
                {
                    outputText("among them,");
                }
                outputText(" sporting a");
                if (flags[kFLAGS.URTA_FIRSTBORN_COCKTYPE] == 2)
                {
                    outputText(" vulpine penis");
                }
                else
                {
                    outputText("n equine phallus, similar to what Urta has");
                }
                outputText(".");
            }
            if (flags[kFLAGS.URTA_LATESTBORN_GENDER] == 1 && urtaKids() > 1)
            {
                ...
                if (flags[kFLAGS.URTA_LATESTBORN_COCKTYPE] == 2)
                {
                    outputText(" vulpine penis");
                }
                else
                {
                    outputText("n equine phallus, just like his mom's");
                }
                outputText(".");
            }
            ...
            if (flags[kFLAGS.URTA_KIDS_MALES] > 1)
            {
                outputText("strong fighters like she is.");
            }
            else
            {
                outputText("a strong fighter like she is.");
            }
        }
        ...
        if (flags[kFLAGS.URTA_KIDS_HERMS] > 0)
        {
            ...
            if (flags[kFLAGS.URTA_KIDS_HERMS] > 1)
            {
                outputText("s");
            }
            outputText(".");
            if (flags[kFLAGS.URTA_FIRSTBORN_GENDER] == 3)
            {
                ...
                {
                    outputText("n equine phallus, a peculiarity she inherited from her mother");
                }
                outputText(".");
            }
            if (flags[kFLAGS.URTA_LATESTBORN_GENDER] == 3 && urtaKids() > 1)
            {
                ...
                {
                    outputText("n equine phallus.  No doubt such a characteristic was passed from her mother");
                }
                outputText(".");
            }
            outputText("  You have a feeling ");
        ...
        if (flags[kFLAGS.URTA_KIDS_FEMALES] > 0)
        {
            ...
            if (flags[kFLAGS.URTA_KIDS_FEMALES] > 1)
            {
                outputText("s");
            }
            outputText(".");
        ...
        if (flags[kFLAGS.URTA_LATESTBORN_GENDER] >= 2)
        {
            outputText("She");
        }
        else
        ...
        else
        {
            ...
            if (urtaKids() > 2)
            {
                outputText("One of your other children");
            }
            else
            {
                outputText("Your other child");
            }
    ...
    private function takeTheKidsForWalk() : Void
    {
        ...
        if (urtaKids() > 1)
        {
            outputText("  \"<i>Yeah, can we " + player.mf("daddy", "mommy") + "?!</i>\"");
        }
        outputText("  Your child");
        if (urtaKids() > 1)
        {
            outputText("ren plead");
        }
        else
        {
            outputText(" pleads");
        ...
        if (urtaKids() > 1)
        {
            outputText("a chorus of excited cheers sounds out.");
        }
        else
        {
            outputText("an excited cheer sounds out.");
        }
        outputText("  It's hard not to want to live up to that title with such ");
        if (urtaKids() == 1)
        {
            outputText("a happy face");
        }
        else
        {
            outputText("happy faces");
        }
        ...
        if (urtaKids() > 1)
        {
            outputText("they fan out and curiously inspect");
        }
        else
        {
            outputText(urtaKidsText("he", "she", "") + " runs off to curiously inspect");
        }
        ...
        if (urtaKids() == 1)
        {
            outputText("buried in cone");
        }
        else
        {
            outputText("s buried in their cones");
        }
	...
	(Back to camp/TA as appropriate.)
        ...
        if (flags[kFLAGS.DISCUSSED_LUBE_SPRAY] > 0)
        {
            addButton(3, "Get Spray", getSpray);
        }
    ...
    private function talkToLiana(output : Bool = true) : Void
    {
        if (output)
        {
            clearOutput();
        ...
        if (flags[kFLAGS.LIANNA_HAVESTED_MALES] + flags[kFLAGS.LIANNA_HAVESTED_LADIES] > 0)
        {
            addButton(2, "Her Spray", talkAboutLiannasLubeSpray);
        }
        if (flags[kFLAGS.DISCUSSED_LUBE_SPRAY] > 0)
        {
            addButton(3, "Side Effect", lubeSpraySideEffects);
        }
        addButton(14, "Back", visitTheHouse);
    }
    ...
    private function talkToLianaAboutHerResearch() : Void
    {
        ...
        if (player.cor >= 50)
        {
            outputText("however half-heartedly it may be, ");
        }
    ...
    private function getSpray() : Void
    {
        ...
        outputText("You ask the budding alchemist if she would be willing to distill some of her spray for you.\n"
                + "\n");
        if (flags[kFLAGS.LUBESPRAY_RECEIVED_TODAY] != null && !debug)
        {
    ...
    private function sexWithLianna() : Void
    {
        ...
        if (player.cockTotal() > 1)
        {
            outputText("  Your lower garments suddenly feel much tighter.");
        }
        ...
        if (player.hasVagina())
        {
            ...
            {
                addButton(0, "HarvestGirly", getEggsHarvested);
            }
            else
            {
    ...
    private function acceptARandomGenderFromASkunk() : Void
    {
        ...
        if (player.isTaur())
        {
            outputText("  Or at least hold on to something, this might sting a little and I don't need you kicking down anything in here.");
        }
        outputText("</i>\"");
    ...
    private function getEggsHarvested() : Void
    {
        ...
        {
            outputText("remembrance, knowing what's coming next");
        }
        ...
        if (player.vaginalCapacity() >= 50)
        {
            outputText("  \"<i>Hmm, I might not have had to lube it up after all...</i>\"  The skunkette remarks.");
        }
        ...
        {
            outputText("; your loose pussy doesn't hurt, either");
        }
    ...
    private function liannaHandjobbies() : Void
    {
        ...
        if (player.cockTotal() > 1)
        {
            outputText("  \"<i>However I think milking just one is good enough.</i>\"");
        }
        ...
        if (player.hasBalls())
        {
            outputText("occasionally stopping to caress your [balls], ");
        }
