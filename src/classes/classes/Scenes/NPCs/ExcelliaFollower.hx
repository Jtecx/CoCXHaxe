...
class ExcelliaFollower extends NPCAwareContent implements TimeAwareInterface
{
    ...
    public function humanCow(strHuman : String, strCow : String) : String
    {
        ...
        //follower{
            ...
            return (flags[kFLAGS.EXCELLIA_LA_BOVA] == 0 && flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] == 1) ? strHuman : strCow;
        }
        //any bova stuff = not pure human
        else
        {
            return (flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] == 3) ? strHuman : strCow;
        }
    }
    ...
    public function girlRacial() : String
    {
        ...
        //follower{
            ...
            return (flags[kFLAGS.EXCELLIA_LA_BOVA] == 0 && flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] == 1) ? "girl" : "cowgirl";
        }
        //any bova stuff = not pure human
        else
        {
            return (flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] == 3) ? "demon-morph" : "cowgirl";
        }
    }
    ...
    public function ExcelliaPathChoice2() : Void
    {
        ...
        {
            addButtonDisabled(3, "Make Slave", "Not for genderless.");
        }
    }
    ...
    public function ExcelliaCampMainMenuFixHer() : Void
    {
        ...
        var _sw15_ = (pregnancy.event);        

        switch (_sw15_)
        {
            case 2:outputText("You notice that Excellia seems to have a small swell in her belly. She has been eating more frequently.\n\n");
            case 3, 4:outputText("The swell in Excellia's belly has grown out into a noticeable bump. You get the distinct feeling that she might be pregnant.\n\n");
    ...
    public function ExcelliaCampMainMenuFixHer2() : Void
    {
        ...
        if (fixed())
        {
            ...
            {
                addButton(5, "Team", excelliaHenchmanOption);
            }
            else
            {
            ...
            {
                addButton(6, "Sleep With", ExcelliaCampFixHerSleepToggle);
            }
            else
            {
                addButton(6, "Sleep Alone", ExcelliaCampFixHerSleepToggle);
            }
        }
        else
        {
    ...
    public function ExcelliaCampFixHerAppearance() : Void
    {
        ...
        {
            outputText("has a bevy of pleasantly defined muscles");
        }
        else if (flags[kFLAGS.EXCELLIA_MAN_UP_BEER] > 0)
        {
            outputText("has some visible musculature");
        }
        else
        {
            outputText("is very soft and plump with no visible muscles");
        }
        outputText(". A strange tattoo marks her stomach.\n\n");
        if (flags[kFLAGS.EXCELLIA_LA_BOVA] > 0)
        {
            outputText("Her arms are " + ((flags[kFLAGS.EXCELLIA_LA_BOVA] == 2) ? "almost human in shape with thick hoof like fingernails" : "human in shape and structure") + ". Her oddly jointed bovine legs are covered in red fur. ");
        }
        else
        {
            outputText("Her arms and legs are both human in shape and structure. ");
        }
    ...
    public function ExcelliaCampFixHerGetMilk() : Void
    {
        ...
        if (fixed())
        {
            outputText("\"<i>Oh, [name]... I will gladly share my milk with you. I could use a little relief myself honestly.</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Of course my " + player.mf("Lord", "Lady") + "! It would be an honor to quench your thirst.</i>\"\n\n");
        }
        if (flags[kFLAGS.EXCELLIA_RECRUITED] < 32)
        {
            flags[kFLAGS.EXCELLIA_RECRUITED]++;
    ...
    public function ExcelliaCampFixHerGetMilkDrink() : Void
    {
        ...
        if (fixed())
        {
            outputText("\"<i>Phew… W-Wow [name]. That was amazing. I do hope that you got your fill!</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Oooo… T-Thank you for making use of me my " + player.mf("Lord", "Lady") + ". I hope it was to your liking.</i>\"\n\n");
        }
    ...
    public function ExcelliaCampFixHerGiveItems() : Void
    {
        ...
        if (fixed())
        {
            outputText("\"<i>Hm? You want to give me something [name]?</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Oh? Does my " + player.mf("Lord", "Lady") + " have something for me?</i>\"\n\n");
        }
        menu();
        
        if (flags[kFLAGS.EXCELLIA_MAN_UP_BEER] < 4)
        {
            addButton(0, consumables.MANUP_B.shortName, ExcelliaCampFixHerGiveItemsManUpBeer).disableIf(!player.hasItem(consumables.MANUP_B), "You don't have any!");
        }
        if (flags[kFLAGS.EXCELLIA_LA_BOVA] == 0 && flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] < 1)
        {
            addButton(1, consumables.HUMMUS_.shortName, ExcelliaCampFixHerGiveItemsHummus).disableIf(!player.hasItem(consumables.HUMMUS_), "You don't have any!");
        }
        if (flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] == 0 && flags[kFLAGS.EXCELLIA_LA_BOVA] < 2)
        {
            addButton(2, consumables.P_LBOVA.shortName, ExcelliaCampFixHerGiveItemsPureLaBova).disableIf(!player.hasItem(consumables.P_LBOVA), "You don't have any!");
        }
        if (flags[kFLAGS.EXCELLIA_RECRUITED] < 33)
        {
            addButton(2, consumables.DEBIMBO.shortName, ExcelliaCampFixHerGiveItemsDeBimbo).hint("Up to 3 doses. Giving all 3 will full cure her without need to wait.").disableIf(!player.hasItem(consumables.DEBIMBO), "You don't have any!");
        }
        addButton(14, "Back", ExcelliaCampMainMenuFixHer);
    }
    ...
    public function ExcelliaCampFixHerGiveItemsPureLaBova() : Void
    {
        ...
        if (flags[kFLAGS.EXCELLIA_LA_BOVA] == 1)
        {
            outputText("Excellia takes the purified la bova then gulps it down. She shudders when it has an immediate effect on her. She yelps and clutches her face as it reshapes into that of a cow. Her fingernails become harder and thicker. Patches of red fur grow on her body slowly filling in until she's covered from head to hooves in red fur.\n\n");
        }
        else
        {
            outputText("Excellia takes the purified la bova then gulps it down. She shudders when it has an immediate effect on her. She groans when her legs reshape into bovine hooves covered in smooth red fur.\n\n");
        }
    ...
    public function ExcelliaCampFixHerSexFuckHer() : Void
    {
        ...
        if (fixed())
        {
            outputText("\"<i>A-Ah please [name]... I want you…</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>P-Please my " + player.mf("Lord", "Lady") + "... I can’t wait any longer!</i>\"\n\n");
        }
        ...
        function maleF() : Void
        {
            ...
            if (fixed())
            {
                outputText("\"<i>Mmm yes, [name]... I’m all yours...</i>\"\n\n");
            }
            else
            {
                outputText("\"<i>Oooo my " + player.mf("Lord", "Lady") + " please fuck me! I-I can take it!</i>\"\n\n");
            }
        ...
        var femF : Void->Void = function() : Void
        {
            ...
            if (fixed())
            {
                outputText("\"<i>[name] please! K-ah-Keep going!</i>\"\n\n");
            }
            else
            {
                outputText("\"<i>A-Ah yes! Right there m-my Lady!</i>\"\n\n");
            }
        ...
        var sharedEnd : Void->Void = function() : Void
        {
            if (fixed())
            {
                outputText("\"<i>Oh [name]... That was just amazing… Oooo, I’m still tingling!</i>\"\n\n");
            }
            else
            {
                outputText("\"<i>A-Ah thanking you my " + player.mf("Lord", "Lady") + "... You truly are amazing...</i>\"\n\n");
            }
    ...
    public function ExcelliaCampFixHerSexOralFun() : Void
    {
        ...
        if (fixed())
        {
            outputText("\"<i>Of course you can have a little of this, [name]. That is if you'll let me have a little of you too.</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Oh my " + player.mf("Lord", "Lady") + ". You are always welcome to have a taste of me. Can I have a taste of you too?</i>\"\n\n");
        }
        ...
        if (fixed())
        {
            outputText("\"<i>Ah… That tongue of yours is devious. I hope you use it on me again VERY soon [name].</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Thank you my " + player.mf("Lord", "Lady") + "! I love it when you do that to me!</i>\"\n\n");
        }
    ...
    public function ExcelliaCampFixHerSexSuckleFuck() : Void
    {
        ...
        if (fixed())
        {
            outputText("\"<i>I'm so sorry [name]! It just felt so good and I just couldn't control myself!</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Oh my apologies my " + player.mf("Lord", "Lady") + "! I might've gotten a bit carried away...</i>\"\n\n");
        }
    ...
    public function excelliaHenchmanOption() : Void
    {
        ...
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "Excellia" || flags[kFLAGS.PLAYER_COMPANION_3] == "Excellia")
            {
                addButtonDisabled(0, "Team (1)", "You already have Excellia accompany you.");
            }
            else
            {
                addButton(0, "Team (1)", excelliaHenchmanOption2, 1).hint("Ask Excellia to join you in adventures outside camp.");
            }
        }
        else if (flags[kFLAGS.PLAYER_COMPANION_1] == "Excellia")
        {
            addButton(5, "Team (1)", excelliaHenchmanOption2, 21).hint("Ask Excellia to stay in camp.");
        }
        else
        {
            addButtonDisabled(5, "Team (1)", "You already have other henchman accompany you as first party member. Ask him/her to stay at camp before you talk with Excellia about accompaning you as first party member.");
        }
        if (player.hasPerk(PerkLib.IntermediateLeadership))
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "")
            {
                if (flags[kFLAGS.PLAYER_COMPANION_1] == "Excellia" || flags[kFLAGS.PLAYER_COMPANION_3] == "Excellia")
                {
                    addButtonDisabled(1, "Team (2)", "You already have Excellia accompany you.");
                }
                else
                {
                    addButton(1, "Team (2)", excelliaHenchmanOption2, 2).hint("Ask Excellia to join you in adventures outside camp.");
                }
            }
            else if (flags[kFLAGS.PLAYER_COMPANION_2] == "Excellia")
            {
                addButton(6, "Team (2)", excelliaHenchmanOption2, 22).hint("Ask Excellia to stay in camp.");
            }
            else
            {
                addButtonDisabled(6, "Team (2)", "You already have other henchman accompany you as second party member. Ask him/her to stay at camp before you talk with Excellia about accompaning you as second party member.");
            }
        }
        else
        {
    ...
    public function excelliaHenchmanOption2(slot : Float = 1) : Void
    {
        clearOutput();
        if (slot < 21)
        {
            ...
            var strExcellia : Float = 290;  //tyle i miałby mino king an tym samym levelu ^^  
            var unarmedExcellia : Float = 90;  //j.w.  
            ...
            unarmedExcellia += (1 + as3hx.Compat.parseInt(unarmedExcellia / 5)) * player.newGamePlusMod();
            player.createStatusEffect(StatusEffects.CombatFollowerExcellia, strExcellia, unarmedExcellia, 0, 0);
            if (slot == 2)
            {
                flags[kFLAGS.PLAYER_COMPANION_2] = "Excellia";
            }
            if (slot == 1)
            {
    ...
    public function ExcelliaCampMakeSlaveAppearance() : Void
    {
        ...
        {
            outputText("'s noticeable very thick with a muffin top and plush body");
        }
        else if (flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] > 0)
        {
            outputText(" appears thick with a noticeable muffin top");
        }
        else
        {
            outputText(" is very soft and plump with no visible muscles");
    ...
    public function ExcelliaCampMakeSlaveGiveItems() : Void
    {
        ...
        if (flags[kFLAGS.EXCELLIA_LA_BOVA] < 2)
        {
            addButton(0, consumables.PROBOVA.shortName, ExcelliaCampMakeSlaveGiveItemsProBova).disableIf(!player.hasItem(consumables.PROBOVA), "You don't have any!");
        }
        if (flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] < 3)
        {
            addButton(1, consumables.SUCMILK.shortName, ExcelliaCampMakeSlaveGiveItemsSuccubusMilk).disableIf(!player.hasItem(consumables.SUCMILK), "You don't have any!");
        }
        addButton(14, "Back", ExcelliaCampMainMenuMakeSlave);
    }
    public function ExcelliaCampMakeSlaveGiveItemsProBova() : Void
    {
        ...
        if (flags[kFLAGS.EXCELLIA_LA_BOVA] == 1)
        {
            outputText("You force Excellia to take the tainted la bova, forcing her to gulp it down. She [exc moo]s and shudders when it has an immediate effect on her. She yelps and clutches her face as it reshapes into that of a cow. Her fingernails become harder and thicker. Patches of red fur grow on her body slowly filling in until she's covered from head to hooves in red fur.\n\n");
        }
        else
        {
            outputText("You force Excellia to take the tainted la bova, making her gulp it down. She [exc moo]s and shudders when it has an immediate effect on her. She groans when her legs reshape into bovine hooves covered in smooth red fur. Her nipples grow out and multiply, forming into four long teats.\n\n");
        }
        flags[kFLAGS.EXCELLIA_LA_BOVA]++;
        doNext(camp.returnToCampUseOneHour);
    }
    public function ExcelliaCampMakeSlaveGiveItemsSuccubusMilk() : Void
    {
        ...
        if (flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] == 2)
        {
            outputText("Excellia eagerly opens her mouth as you pour the demonic milk into her mouth. She nearly squeals with glee as it has an immediate effect on her. Her cow like horns twist and reshape into more demonic looking ones. Tiny bat wings grow out from between her shoulder. A long demonic tongue snakes out of her mouth. She moans loudly as she orgasms, streams of her milk erupts from her breasts and femcum dribbles down between her legs. She pants heavily with lust in her eyes.\n\n");
        }
        else if (flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] == 1)
        {
            outputText("Excellia opens her mouth as you pour the demonic milk into her mouth. She lets out a moan as it has an immediate effect on her. Her breasts swell up a cup size, becoming H cups. Her tail thrashes behind her, reshaping into a spaded demon tail. She moos loudly as she orgasms, a spray of femcum pooling under her.\n\n");
        }
        else
        {
            outputText("Excellia opens her mouth as you pour the demonic milk into her mouth. She lets out a moan as it has an immediate effect on her. Her breasts swell up a cup size, becoming G cups. Her pussy moistens and drools down her thighs.\n\n");
        }
        flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK]++;
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    public function ExcelliaCampMakeSlaveSexFuckHer() : Void
    {
        ...
        function maleF() : Void
        {
            ...
            {
                outputText("You grab her hips then pull back until only the heads of your dicks are inside her.");
            }
            else
            {
                outputText("You grab her hips then pull back until only the head of your dick is inside her.");
            }
    ...
    {
        return as3hx.Compat.parseInt(flags[kFLAGS.EXCELLIA_FEMALE_KIDS] + flags[kFLAGS.EXCELLIA_MALE_KIDS] + flags[kFLAGS.EXCELLIA_FEMALE_COW_KIDS] + flags[kFLAGS.EXCELLIA_MALE_COW_KIDS]);
    }
    ...
    //spriteSelect(SpriteDb.s_izma);
    {
        ...
        if (flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] == 1)
        {
            if (player.statusEffectv1(StatusEffects.ExcelliaChildbirth) == 1)
            {
                flags[kFLAGS.EXCELLIA_MALE_KIDS]++;
            }
            else
            {
                flags[kFLAGS.EXCELLIA_FEMALE_KIDS]++;
            }
        }
        else if (player.statusEffectv1(StatusEffects.ExcelliaChildbirth) == 1)
        {
            flags[kFLAGS.EXCELLIA_MALE_COW_KIDS]++;
        }
        else
        {
            flags[kFLAGS.EXCELLIA_FEMALE_COW_KIDS]++;
        }
