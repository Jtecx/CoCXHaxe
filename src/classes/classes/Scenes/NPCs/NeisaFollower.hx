...
class NeisaFollower extends NPCAwareContent implements TimeAwareInterface
{
    ...
    public function neisaAffection(changes : Float = 0) : Float
    {
        flags[kFLAGS.NEISA_AFFECTION] += changes;
        if (flags[kFLAGS.NEISA_AFFECTION] > 50)
        {
            flags[kFLAGS.NEISA_AFFECTION] = 50;
        }
        if (flags[kFLAGS.NEISA_AFFECTION] > 150 && flags[kFLAGS.NEISA_FOLLOWER] >= 18)
        {
            flags[kFLAGS.NEISA_AFFECTION] = 150;
        }
    ...
    //spriteSelect(SpriteDb.s_isabella);
    {
        ...
        else if (flags[kFLAGS.SPIRIT_STONES] > 6)
        {
            outputText("You pay the spirit stone you owe Neisa as she comes over to collect.\n\n");
    ...
    public function neisaCampMenu() : Void
    {
        ...
        {
            if (flags[kFLAGS.PLAYER_COMPANION_1] == "Neisa" || flags[kFLAGS.PLAYER_COMPANION_2] == "Neisa")
            {
                addButtonDisabled(1, "Spar", "You can't fight against her as long she's in your team.");
            }
            else
            {
                addButton(1, "Spar", neisaSpar).hint("Do a quick battle with Neisa!");
            }
        }
        ...
        {
            addButton(5, "Team", neisaHenchmanOption);
        }
        else
        {
            addButtonDisabled(5, "Team", "You need to have at least Basic Leadership to form a team.");
        }
        if (flags[kFLAGS.NEISA_FOLLOWER] >= 14)
        {
            addButton(13, "Paycheck", neisaMorningPaycheckCall2).hint("Pay Neisa due payment (make sure to not lack spirit stones for it)");
        }
    ...
    {
        if (flags[kFLAGS.NEISA_DEFEATS_COUNTER] >= 1)
        {
            flags[kFLAGS.NEISA_DEFEATS_COUNTER]++;
        }
        else
        {
            flags[kFLAGS.NEISA_DEFEATS_COUNTER] = 1;
        }
        if (flags[kFLAGS.NEISA_DEFEATS_COUNTER] == 1 && flags[kFLAGS.NEISA_LVL_UP] == 1)
        {
            ...
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction));
            }
            else
            {
                player.createStatusEffect(StatusEffects.CampSparingNpcsTimers4, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction));
            }
        ...
        if (flags[kFLAGS.NEISA_DEFEATS_COUNTER] == 2 && flags[kFLAGS.NEISA_LVL_UP] == 2)
        {
            if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers4))
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 2);
            }
            else
            {
                player.createStatusEffect(StatusEffects.CampSparingNpcsTimers4, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 2);
            }
            flags[kFLAGS.NEISA_DEFEATS_COUNTER] = 0;
        ...
        if (flags[kFLAGS.NEISA_DEFEATS_COUNTER] == 3 && flags[kFLAGS.NEISA_LVL_UP] == 3)
        {
            if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers4))
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 3);
            }
            else
            {
                player.createStatusEffect(StatusEffects.CampSparingNpcsTimers4, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 3);
            }
            flags[kFLAGS.NEISA_DEFEATS_COUNTER] = 0;
        ...
        if (flags[kFLAGS.NEISA_DEFEATS_COUNTER] == 4 && flags[kFLAGS.NEISA_LVL_UP] == 4)
        {
            if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers4))
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 4);
            }
            else
            {
                player.createStatusEffect(StatusEffects.CampSparingNpcsTimers4, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 4);
            }
            flags[kFLAGS.NEISA_DEFEATS_COUNTER] = 0;
        ...
        if (flags[kFLAGS.NEISA_DEFEATS_COUNTER] == 5 && flags[kFLAGS.NEISA_LVL_UP] == 5)
        {
            if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers4))
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 5);
            }
            else
            {
                player.createStatusEffect(StatusEffects.CampSparingNpcsTimers4, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 5);
            }
            flags[kFLAGS.NEISA_DEFEATS_COUNTER] = 0;
        ...
        if (flags[kFLAGS.NEISA_DEFEATS_COUNTER] == 6 && flags[kFLAGS.NEISA_LVL_UP] == 6)
        {
            if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers4))
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 6);
            }
            else
            {
                player.createStatusEffect(StatusEffects.CampSparingNpcsTimers4, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 6);
            }
            flags[kFLAGS.NEISA_DEFEATS_COUNTER] = 0;
        ...
        if (flags[kFLAGS.NEISA_DEFEATS_COUNTER] == 7 && flags[kFLAGS.NEISA_LVL_UP] == 7)
        {
            if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers4))
            {
                player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 7);
            }
            else
            {
                player.createStatusEffect(StatusEffects.CampSparingNpcsTimers4, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 7);
            }
            flags[kFLAGS.NEISA_DEFEATS_COUNTER] = 0;
    ...
    public function neisaHenchmanOption() : Void
    {
        ...
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "Neisa" || flags[kFLAGS.PLAYER_COMPANION_3] == "Neisa")
            {
                addButtonDisabled(0, "Team (1)", "You already have Neisa accompany you.");
            }
            else
            {
                addButton(0, "Team (1)", neisaHenchmanOption2, 1).hint("Ask Neisa to join you in adventures outside camp.");
            }
        }
        else if (flags[kFLAGS.PLAYER_COMPANION_1] == "Neisa")
        {
            addButton(5, "Team (1)", neisaHenchmanOption2, 21).hint("Ask Neisa to stay in camp.");
        }
        else
        {
            addButtonDisabled(5, "Team (1)", "You already have other henchman accompany you as first party member. Ask him/her to stay at camp before you talk with Neisa about accompaning you as first party member.");
        }
        if (player.hasPerk(PerkLib.IntermediateLeadership))
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "")
            {
                if (flags[kFLAGS.PLAYER_COMPANION_1] == "Neisa" || flags[kFLAGS.PLAYER_COMPANION_3] == "Neisa")
                {
                    addButtonDisabled(1, "Team (2)", "You already have Neisa accompany you.");
                }
                else
                {
                    addButton(1, "Team (2)", neisaHenchmanOption2, 2).hint("Ask Neisa to join you in adventures outside camp.");
                }
            }
            else if (flags[kFLAGS.PLAYER_COMPANION_2] == "Neisa")
            {
                addButton(6, "Team (2)", neisaHenchmanOption2, 22).hint("Ask Neisa to stay in camp.");
            }
            else
            {
                addButtonDisabled(6, "Team (2)", "You already have other henchman accompany you as second party member. Ask him/her to stay at camp before you talk with Neisa about accompaning you as second party member.");
            }
        }
        else
        {
    ...
    public function neisaHenchmanOption2(slot : Float = 1) : Void
    {
        clearOutput();
        if (slot < 21)
        {
            ...
            if (slot == 2)
            {
                flags[kFLAGS.PLAYER_COMPANION_2] = "Neisa";
            }
            if (slot == 1)
            {
