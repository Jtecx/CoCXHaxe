...
class Ted extends Monster
{
    private function tedSpecialAttackOne() : Void
    {
        ...
        {
            damage = 0;
            outputText("You easily deflect and block the damage from dragon boy wide swing.");
        }
        else
        {
            outputText(((flags[kFLAGS.TED_LVL_UP] >= 3) ? "Ted" : "Dragon-boy") + " easily hits you with a wide, difficult to avoid swing.  ");
        }  //Ted  
        if (damage > 0)
        {
    ...
    private function tedSpecialAttackTwo() : Void
    {
        ...
        {
            damage = 0;
            outputText("You somehow manage to deflect and block dragon-boy massive overhead swing.");
        }
        if (damage > 0)
        {
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        ...
        {
            SceneLib.tedScene.defeatedTedPostHiddenCave();
        }
        else
        {
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (flags[kFLAGS.TED_LVL_UP] >= 1)
        {
            SceneLib.tedScene.lostToTedPostHiddenCave();
        }
        else
        {
    ...
    public function new()
    {
        ...
        if (flags[kFLAGS.TED_LVL_UP] >= 6)
        {
            this.drop = new WeightedDrop(consumables.BAGOCA3, 1);
        }
        else if (flags[kFLAGS.TED_LVL_UP] >= 3 && flags[kFLAGS.TED_LVL_UP] < 6)
        {
