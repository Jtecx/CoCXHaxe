...
class PhyllaScene extends BaseContent implements TimeAwareInterface
{
    ...
    public function timeChange() : Bool
    {
        ...
        if (flags[kFLAGS.PHYLLA_EGG_LAYING] > 0 && rand(5) == 0 && flags[kFLAGS.ANT_KIDS] < 5000)
        {
        ...
        //The pregnancyStore doesn't handle Phylla's ant eggs because they are continuous. The regular egg production is all handled here.{
            ...
            {
                flags[kFLAGS.DAYS_PHYLLA_HAS_SPENT_BIRTHING]++;
            }
            if (flags[kFLAGS.PHYLLA_GEMS_HUNTED_TODAY] > 0)
            {
                flags[kFLAGS.PHYLLA_GEMS_HUNTED_TODAY] = 0;
            }
            if (phyllaWaifu())
            {
                flags[kFLAGS.DAYS_PHYLLA_IN_CAMP]++;
            }
        }
        return false;
    }
    ...
    public function phyllaWaifu() : Bool
    {
    ...
    public function phyllaSprite(nude : Bool = false) : Void
    {
        ...
        {
            spriteSelect(SpriteDb.s_phylla_preg);
        }
    }
    ...
    public function introductionToPhyllaFollower() : Void
    {
        ...
        else
        {
            ...
            //If <10 days in camp:
            if (flags[kFLAGS.DAYS_PHYLLA_IN_CAMP] < 10)
            {
                outputText("\n\nSo far the map of the caves doesn't look very impressive.");
            }
            //If <30 days in camp:
            else if (flags[kFLAGS.DAYS_PHYLLA_IN_CAMP] < 30)
            {
        ...
        if (flags[kFLAGS.PHYLLA_EGG_LAYING] == 0)
        {
            addButton(2, "Lay Eggs", phyllaLaysEggsToggle);
        }
        else
        {
            addButton(2, "No Eggs", phyllaLaysEggsToggle);
        }
        if (flags[kFLAGS.ANT_KIDS] > 0)
        {
            addButton(3, "Children", phyllasKidsChildren);
        }
    ...
    {
        var egg : Bool = flags[kFLAGS.PHYLLA_EGG_LAYING] > 0 && flags[kFLAGS.ANT_KIDS] >= 10;
        menu();
        //Dick
        addButton(0, "Get BJ", phyllaBeeeJays).disableIf(!player.hasCock(), "Req. a cock!");
        addButton(1, "FuckHer", dickPhylla).disableIf(egg, "Phylla should not be laying eggs at the moment.").disableIf(!player.hasCock(), "Req. a cock!");
        addButton(2, "FuckHer(Egg)", dudesFuckEggLayingBitches).disableIf(!egg, "Phylla should be laying eggs and have at least 10 children.").disableIf(!player.hasCock(), "Req. a cock!");
        addButton(3, "Orgy(Male)", orgyWithDatColonyCorruptDudes).disableIf(flags[kFLAGS.ANT_KIDS] < 10, "You need more children for this.").disableIf(!player.hasCock(), "Req. a cock!").disableIf(player.cor < 75 - player.corruptionTolerance, "You're not corrupted enough!", "???");
        //Pussy
        addButton(6, "Lesbi", lesbianFisting).disableIf(egg, "Phylla should not be laying eggs at the moment.").disableIf(!player.hasVagina(), "Req. a vagina!");
        addButton(7, "Lesbi(Egg)", birfingSexWithAntsForDasLadies).disableIf(!egg, "Phylla should be laying eggs and have at least 10 children.").disableIf(!player.hasVagina(), "Req. a vagina!");
        addButton(8, "Orgy(Female)", antColonyOrgy4Ladies).disableIf(flags[kFLAGS.ANT_KIDS] < 10, "You need more children for this.").disableIf(!player.hasVagina(), "Req. a vagina!").disableIf(player.cor < 75 - player.corruptionTolerance, "You're not corrupted enough!", "???");
        //Driderimpregnation scene for Phylla
        addButton(10, "Oviposit", eggDatBitch).disableIf(!player.canOvipositSpider(), "Req. a spider ovipositor!");
    ...
    private function talkAboutAntHistory() : Void
    {
        ...
        var boring : Void->Void = function() : Void
        {
        ...
        var berate : Void->Void = function() : Void
        {
    ...
    private function talkAboutAntMatingAndRituals() : Void
    {
        ...
        if (flags[kFLAGS.PHYLLA_TIMES_DRIDER_EGG_LAYED] > 0)
        {
        ...
        addButton(1, "Embarrass", embarrass).disableIf(player.cor < 33 - player.corruptionTolerance, "Not corrupted enough.");
        ...
        var embarrass : Void->Void = function() : Void
        {
            dynStats("cor", 1);
            ...
            menu();
            addButton(0, "Push It!", pushIt).disableIf(player.cor < 66 - player.corruptionTolerance, "Not corrupted enough.");
            addButton(1, "Stop", stopPushing);
            
            var pushIt : Void->Void = function() : Void
            {
                ...
                if (flags[kFLAGS.PHYLLA_EGG_LAYING] > 0)
                {
                    outputText(", sending jets of milk shooting out onto the floor");
                }
            ...
            var stopPushing : Void->Void = function() : Void
            {
    ...
    private function phyllasLifePastAndFuture() : Void
    {
        ...
        function izmaTalk() : Void
        {
            ...
            var retort : Void->Void = function() : Void
            {
                ...
                menu();
                addButton(0, "HowDareShe?!", howDareShe).disableIf(player.cor < 50 - player.corruptionTolerance, "Not corrupted enough.");
                ...
                var probeHer : Void->Void = function() : Void
                {
        ...
        var normalTalk : Void->Void = function() : Void
        {
    ...
    private function phyllaBeeeJays() : Void
    {
        ...
        //Dick size less than 60 inches:
        {
            ...
            if (player.cockArea(x) < 60)
            {
                ...
                {
                    outputText(" \"tall\" and proud, rock hard and ready for action.");
                }
                else
                {
                    outputText(" tall and proud, rock hard and ready for action.");
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (flags[kFLAGS.PHYLLA_BLOWJOBS] == 2)
                    {
                        outputText("last time");
                    }
                    else
                    {
                        outputText("our first time");
                    }
                    outputText(".  I'll help to free ");
                    if (player.cockTotal() == 1)
                    {
                        outputText("that monster");
                    }
                    else
                    {
                        outputText("those monsters");
                    }
                    ...
                    if (player.cocks[x].cockLength < 16)
                    {
                        outputText("base");
                    }
                    else
            ...
            addButton(1, "ForceHer", corruptPhyllaEndings).disableIf(player.cor < 66 - player.corruptionTolerance, "Not corrupted enough!");
        }
    }
    ...
    private function purePhyllaBJOver() : Void
    {
        ...
        if (player.cockTotal() > 1)
        {
            ...
            {
                outputText("the rest of your dicks");
            }
            outputText("; if she would be \"<i>so kind.</i>\" With gusto, her four hands shoot up and take your ");
            if (player.cockTotal() == 2)
            {
                outputText("cocks into their firm grips");
            }
            else
            {
                outputText("cock into their firm grips");
            }
    ...
    private function nopeNotOnMouthOrWhateverFuckThisNoise() : Void
    {
        ...
        if (!player.canAutoFellate())
        {
            outputText(" or anything like it would feel like.");
        }
        else
        {
            outputText(" feels like with Phylla's mind linked to your own.");
        }
    ...
    //Pure BJ Ending
    private function pureBJEnding(linked : Bool = true) : Void
    {
        ...
        if (flags[kFLAGS.TIMES_LINKED_BJ_SUCK] > 0)
        {
            outputText("  You can feel her relying less and less on the link with every subsequent time; there's no doubt in your mind that she will no longer \"<i>cheat</i>\" her way into making you cum before you know it.");
        }
        var x : Int = player.smallestCockIndex();
        ...
        if (player.cumQ() > 1000)
        {
            outputText("  Though she might have a mouthful, you're not finished yet!  You continue to unload the remainder of your hot load on her breasts and stomach.");
        }
        if (player.cockTotal() > 1)
        ...
        {
            outputText(" and congratulate her on her first time.");
        }
    ...
    private function corruptPhyllaEndings() : Void
    {
        ...
        if (flags[kFLAGS.PHYLLA_BLOWJOBS] == 1)
        {
            outputText("achieved the desired effect.");
        }
        //Every other time:
        else
        {
            outputText("improved since the last time.");
        }
    ...
    //shitty implementation, but sounds logical
    {
        ...
        if (cockVag < 0)
        {
            cockVag = player.smallestCockIndex();
        }
        if (player.cockTotal() >= 2)
        ...
        //3-4 times:
        if (flags[kFLAGS.PHYLLA_FUCKS] >= 3)
        {
            outputText(" she asks playfully, knowing full well that that's what you came here for.");
        }
        //(First time)
        else if (flags[kFLAGS.PHYLLA_FUCKS] == 0)
        {
            outputText(" You nod quickly.");
        }
        //(Subsequent fucks)
        if (flags[kFLAGS.PHYLLA_FUCKS] > 0)
        {
    ...
    private function lesbianFisting() : Void
    {
        ...
        if (player.hasVirginVagina() || player.vaginalCapacity() <= 10)
        {
            outputText(" tries to fit itself into you, only managing to insert her fingers into your pussy.  Despite the \"<i>setback,</i>\" Phylla");
        }
    ...
    private function dudesFuckEggLayingBitches() : Void
    {
        ...
        addButton(1, "DontCare", birthDickSex2, true).disableIf(player.cor < 66 - player.corruptionTolerance, "You're not an asshole.. are you?");
    }
    private function birthDickSex2(corrupt : Bool = false) : Void
    //If Corruption is less than 50:
    {
        ...
        if (player.hasBalls())
        {
            outputText(" your balls slap against the smoothness of her pelvis.");
        }
        else
        {
            outputText(" your knees grind along the edge of her makeshift throne.");
        }
    ...
    private function birfingSexWithAntsForDasLadies() : Void
    {
        ...
        if (player.isNaga())
        {
            outputText("  You slither almost whorishly with a slow, measured wriggle,");
        }
        else
        {
            outputText("  You walk almost whorishly with a slow, sensual gait,");
        }
        ...
        addButton(1, "Use Her", birthVagSex2, true).disableIf(player.cor < 66 - player.corruptionTolerance, "You're not corrupted enough to use a pregnant ant-girl.");
    }
    
    private function birthVagSex2(corrupt : Bool = false) : Void
    //If Corruption is less than 75:
    {
        ...
        if (!corrupt)
        {
            ...
            else if (player.statusEffectv1(StatusEffects.Birthed) > 0)
            {
        ...
        if (!corrupt)
        {
            outputText("  You probe her mind and are almost taken aback.\"<i>NO!  Just please keep going!</i>\"  You hear her voice ring out in your head.");
        }
        ...
        if (!corrupt)
        {
            ...
            if (!player.armor.hasBuff("teasedmg"))
            {
    ...
    private function antColonyOrgy4Ladies() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_CORRUPT_FEMALE_ANT_ORGY] == 1)
        {
            ...
            if (player.hasCock())
            {
                outputText(" and [eachCock] harden");
            }
        ...
        if (player.isBiped())
        {
            outputText("  Setting one foot on his shoulder, you present your [vagina] to him.");
        }
        else
        {
            outputText(" Now at the perfect height, you present your [vagina] to him.");
        }
        ...
        if (player.isBiped())
        {
            outputText(" from between your legs.");
        }
        else
        {
            outputText(" from your cunt.");
        }
    ...
    private function phyllaLaysEggsToggle() : Void
    {
        ...
        if (flags[kFLAGS.PHYLLA_EGG_LAYING] == 0)
        {
            ...
            if (flags[kFLAGS.TALKED_WITH_PHYLLA_ABOUT_HISTORY] > 0)
            {
                outputText("  You tell her you want a colony like those she spoke of in the times before the corruption.");
            }
    ...
    private function lickThatAntButt() : Void
    {
        ...
        if (flags[kFLAGS.ANT_KIDS] < 5000)
        {
            flags[kFLAGS.ANT_KIDS] += 5;
        }
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    private function dontLickAntButt() : Void
    {
        ...
        if (flags[kFLAGS.ANT_KIDS] > 1)
        {
            outputText("; the children can tend to the eggs and I can really use the rest");
        }
    ...
    private function phyllaLaysSomeDriderEggs() : Void
    {
        ...
        addButton(1, "Watch", watch).disableIf(player.cor < 60 - player.corruptionTolerance, "You're not THAT corrupted.");
        ...
        function helpHer() : Void
        {
            ...
            if (flags[kFLAGS.PHYLLA_EGG_LAYING] > 0)
            {
                outputText("both eggs are just fine on their own");
            }
            else
            {
                outputText("the egg is just fine on its own");
            }
        ...
        var watch : Void->Void = function() : Void
        {
    ...
    private function driderDoublePhllaMount() : Void
    {
        ...
        {
            pregnancy.knockUpForce(PregnancyStore.PREGNANCY_DRIDER_EGGS, 8 * 24);
    ...
    private function phyllasKidsChildren() : Void
    {
        ...
        if (flags[kFLAGS.DAYS_PHYLLA_HAS_SPENT_BIRTHING] < 10)
        {
        ...
        else if (flags[kFLAGS.DAYS_PHYLLA_HAS_SPENT_BIRTHING] < 40)
        {
        ...
        if (flags[kFLAGS.PHYLLA_TIMES_DRIDER_EGG_LAYED] > 10)
        {
    ...
    private function phyllaDigsForGems() : Void
    {
        clearOutput();
        var kidsMod : Int = 0;
        if (flags[kFLAGS.ANT_KIDS] > 10)
        {
            kidsMod++;
        }
        if (flags[kFLAGS.ANT_KIDS] > 50)
        {
            kidsMod++;
        }
        if (flags[kFLAGS.ANT_KIDS] > 150)
        {
            kidsMod++;
        }
        if (flags[kFLAGS.ANT_KIDS] > 300)
        {
            kidsMod++;
        }
        if (flags[kFLAGS.ANT_KIDS] > 600)
        {
            kidsMod++;
        }
        ...
        {
            var gems : Int = as3hx.Compat.parseInt(10 + rand(10) + kidsMod * 2);
            player.gems += gems;
            statScreenRefresh();
            //If Phylla IS NOT Laying Eggs
            if (flags[kFLAGS.PHYLLA_EGG_LAYING] == 0)
    ...
    private function phyllaStones() : Void
    {
        ...
        if (flags[kFLAGS.ANT_KIDS] > 1250)
        {
            kidsMod2++;
        }
        if (flags[kFLAGS.ANT_KIDS] > 1500)
        {
            kidsMod2++;
        }
        if (flags[kFLAGS.ANT_KIDS] > 1750)
        {
            kidsMod2++;
        }
        if (flags[kFLAGS.ANT_KIDS] > 2000)
        ...
        if (flags[kFLAGS.ANT_KIDS] > 2500)
        {
            kidsMod2++;
        }
        if (flags[kFLAGS.ANT_KIDS] > 3000)
        {
            kidsMod2++;
        }
        if (flags[kFLAGS.ANT_KIDS] > 3500)
        {
            kidsMod2++;
        }
        if (flags[kFLAGS.ANT_KIDS] > 4000)
        {
            kidsMod2++;
        }
        var stones : Int = as3hx.Compat.parseInt(3 + rand(7) + (kidsMod2 * 5));
        flags[kFLAGS.ACHIEVEMENT_PROGRESS_ANTWORKS] += stones;
        if (flags[kFLAGS.ACHIEVEMENT_PROGRESS_ANTWORKS] >= 200)
        {
            awardAchievement("AntWorks", kACHIEVEMENTS.GENERAL_ANTWORKS);
        }
        if (flags[kFLAGS.ACHIEVEMENT_PROGRESS_ANTWORKS] >= 5000)
        {
            awardAchievement("Ant-Kids and Phylla", kACHIEVEMENTS.GENERAL_ANT_KIDS_AND_PHYLLA);
        }
    ...
    private function eggDatBitch() : Void
    {
        ...
        {
            outputText("You give Phylla a knowing look then glance back at your swollen spider abdomen.  Playfully you thrust your ovipositor at her.  Phylla giggles a little, knowing what you want to do to her.");
        }
    ...
    public function ovipositionScene() : Void
    {
        ...
        else
        {
            ...
            if (player.cor < 75)
            {
                outputText("  Little does poor Phylla know, you're going to just use her as a baby maker to ease the tension of your own sacks.");
            }
        }
