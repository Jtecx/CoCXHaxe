...
class Kindra extends Monster
{
    public var kindraScene : KindraFollower = new KindraFollower();
    
    public function bowShooting() : Float
    {
        var cost : Float = 15;
        if (flags[kFLAGS.KINDRA_ARROWS_SHOT] >= 60)
        {
            cost -= 5;
        }
        if (flags[kFLAGS.KINDRA_ARROWS_SHOT] >= 240)
        {
            cost -= 5;
        }
        return cost;
    }
    public function soulskillMulti() : Float
    {
        var multi : Float = 1;
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (flags[kFLAGS.KINDRA_LVL_UP] >= 15)
        {
            ...
            else if (rand(3) == 0 && fatigue >= 240)
            {
                KindraDrinkSheepMilk();
            }
            else if (rand(2) == 0 && soulforce >= 10)
            {
                KindraManyBirdsSoulskill();
            }
            else
            {
                eAttack();
        ...
        {
            if (fatigue < (maxFatigue() - (bowShooting() * 5)))
            {
                KindraFireBow05();
            }
            else if (rand(3) == 0 && fatigue >= 60)
        ...
        {
            if (fatigue < (maxFatigue() - (bowShooting() * 4)))
            {
                KindraFireBow04();
            }
            else if (rand(3) == 0 && fatigue >= 60)
        ...
        {
            if (fatigue < (maxFatigue() - (bowShooting() * 3)))
            {
                KindraFireBow03();
            }
            else if (rand(3) == 0 && fatigue >= 60)
        ...
        {
            if (fatigue < (maxFatigue() - (bowShooting() * 2)))
            {
                KindraFireBow02();
            }
            else if (rand(3) == 0 && fatigue >= 60)
        ...
        {
            if (fatigue < (maxFatigue() - bowShooting()))
            {
                KindraFireBow01();
            }
            else if (rand(3) == 0 && fatigue >= 60)
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        ...
        {
            kindraScene.wonFightWithKindra();
        }
        else if (flags[kFLAGS.KINDRA_AFFECTION] >= 6)
        {
            kindraScene.thirdWonFightWithKindra();
        }
        else if (flags[kFLAGS.KINDRA_AFFECTION] >= 2 && flags[kFLAGS.KINDRA_AFFECTION] < 6)
        {
            kindraScene.secondWonFightWithKindra();
        }
        else
        {
            kindraScene.firstWonFightWithKindra();
        }
    }
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (flags[kFLAGS.KINDRA_FOLLOWER] == 1)
        {
            kindraScene.lostFightWithKindra();
        }
        else
        {
            kindraScene.lostFightToKindra();
        }
    }
    ...
    public function new()
    {
        ...
        if (flags[kFLAGS.KINDRA_LVL_UP] >= 6)
        {
            this.drop = new ChainedDrop().add(consumables.BAGOCA3, 0.2).add(consumables.MG_SFRP, 0.3).add(consumables.SHEEPMK, 0.6);
        }
        else
        {
            this.drop = new ChainedDrop().add(consumables.BAGOCA2, 0.2).add(consumables.LG_SFRP, 0.3).add(consumables.SHEEPMK, 0.6);
        }
