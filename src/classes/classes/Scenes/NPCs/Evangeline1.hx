...
class Evangeline1 extends Monster
{
    /*
    ...
    override private function performCombatAction() : Void
    {
        if (flags[kFLAGS.EVANGELINE_LVL_UP] == 4)
        {
            ...
            if (choice4 == 1)
            {
                if (this.lust > 50)
                {
                    ...
                    else if (!statStore.hasBuff("EvangelineMight") && (mana >= spellCostMight()))
                    {
                        MightSpell();
                    }
                    else if (!hasStatusEffect(StatusEffects.Blink) && (mana >= spellCostBlink()))
                    {
                        BlinkSpell();
                    }
                    else if (HPRatio() < .75 && (mana >= spellCostHeal()))
                    {
                        HealSpell();
                    }
                    else if ((this.lust < maxLust() * 0.75) && rand(2) == 0 && (mana >= spellCostBlind()) && !player.hasStatusEffect(StatusEffects.Blind))
                    {
                        BlindSpell();
                    }
                    else if (rand(2) == 0 && (mana >= spellCostArouse()))
                    {
                        ArouseSpell();
                    }
                    else if ((this.lust < maxLust() * 0.75) && (mana >= spellCostWhitefire()))
                    {
                        WhiteFireSpell();
                    }
                    else
        ...
        else if (flags[kFLAGS.EVANGELINE_LVL_UP] == 2)
        {
            ...
            if (choice1 == 1)
            {
                if (this.lust > 50)
                {
                    if (HPRatio() < .75 && (mana >= spellCostHeal()))
                    {
                        HealSpell();
                    }
                    else if (rand(3) == 0 && (mana >= spellCostArouse()))
                    {
                        ArouseSpell();
                    }
                    else if ((this.lust < maxLust() * 0.75) && (mana >= -spellCostWhitefire()))
