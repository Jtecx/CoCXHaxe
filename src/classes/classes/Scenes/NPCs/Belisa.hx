...
class Belisa extends Monster
{
    private function spellCostWhitefire() : Float
    {
        var cost : Float = 30;
        return cost;
    }
    private function spellCostHeal() : Float
    {
        var cost : Float = 30;
    ...
    private function belisaBasicAttack() : Void
    {
        ...
        if (!player.immuneToBleed())
        {
            ...
            {
                player.createStatusEffect(StatusEffects.Hemorrhage, SceneLib.combat.debuffsOrDoTDuration(2 + rand(2)), bleedP, 0, 0);
            }
        }
    }
    ...
    private function belisaWhitefire() : Void
    {
        ...
        if (player.cor < 15)
        {
            ...
            if (BelisaFollower.BelisaEncounternum == 2)
            {
                BelisaFollower.BelisaEncounternum = 3;
            }
        }
        else
        {
            ...
            var damage : Int = as3hx.Compat.parseInt(4 * inteligencescalingbonus() * SpellMod());
    ...
    {
        if (BelisaFollower.BelisaEncounternum == 3 && Combat.playerWaitsOrDefends())
        {
            SceneLib.belisa.postFightOptionsWhitefireWait();
        }
        ...
        switch (choice0)
        {
            ...
            case 1:
                belisaWebAttack();
            case 2:
                if (mana >= spellCostWhitefire() && !hasStatusEffect(StatusEffects.AbilityCooldown1))
                {
                    belisaWhitefire();
                }
                else
            ...
            case 3:
                if (HPRatio() < .8 && (mana >= spellCostHeal()) && !hasStatusEffect(StatusEffects.AbilityCooldown2))
                {
                    belisaHeal();
                }
                else
    ...
    override private function get_long() : String
    {
        ...
        if (BelisaFollower.BelisaQuestComp)
        {
            str += " Her fangs glint dangerously in the light, and she smiles despite her situation. She seems eager to bite.";
        }
        else
        {
            str += " The one fang of hers you can see is dripping a clear venom, but her sunhat covers most of her face, hiding her eyes from you.";
        }
        return str;
    }
    ...
    public function new()
    {
        ...
        this.drop = new WeightedDrop().add(consumables.B_GOSSR, 1).add(useables.T_SSILK, 3).add(null, 5);
        ...
        if (flags[kFLAGS.BELISA_LVL_UP] >= 8)
        {
            this.createPerk(PerkLib.PeerlessSpirituality, 0, 0, 0, 0);
        }
        checkMonster();
    }
}
