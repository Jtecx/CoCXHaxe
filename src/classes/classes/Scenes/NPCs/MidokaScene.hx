...
class MidokaScene extends NPCAwareContent implements TimeAwareInterface implements SaveableState
{
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            MidokaName = Reflect.field(o, "MidokaName");
            MidokaAge = Reflect.field(o, "MidokaAge");
            GrowUpTime = Reflect.field(o, "GrowUpTime");
            MidokaGuardingCamp = Reflect.field(o, "MidokaGuardingCamp");
        }
        else
        {
            resetState();
    ...
    public function midokaFollower() : Bool
    {
        return MidokaAge > 0;
    }
    ...
    //Easter Egg Names Hel WILL NOT ALLOW:
    {
        ...
        else if (mainView.nameBox.text == "")
        {
            ...
            {
                outputText("“<i>Please give her a name [name].</i>”");
            }
        ...
        MidokaAge = 1;
        GrowUpTime = as3hx.Compat.parseInt(7 * 24);
    ...
    public function midokaMainMenu() : Void
    {
        ...
        else
        {
            ...
            if (MidokaAge == 1)
            {
                outputText("coos at you, waving her arms at you to pick her up.");
            }
            else if (MidokaAge == 2)
            {
                outputText("sits up, crawlnig to the edge of her basket and waving her arms at you to pick her up.");
            }
            else if (MidokaAge == 3)
            {
                outputText("stands up, gripping the edge of her basket with one tiny hand and waving the other at you to pick her up.");
            }
            else if (MidokaAge == 4)
            {
                outputText("calls to you, “<i>" + championRef() + "!</i>”, clambering out of her basket and running over to you.");
            }
            else if (MidokaAge == 5)
            {
                outputText("waves to you, “<i> Hi " + championRef() + ", did you come to watch me train today?</i>”");
            }
            else if (MidokaAge == 6)
            {
                outputText("waves to you, “<i> Hi " + championRef() + ", did you come to train with me today?</i>” while giving you a light jab to the shoulder");
            }
            if (MidokaGuardingCamp == 2)
            {
                outputText("She is currently guarding the camp at night.");
            }
        }
        ...
        //[Hug]
        //[Talk]
        addButton(0, "Talk", talkToMidoka).disableIf(MidokaAge < 4, "She is too young to hold a conversation yet");
        addButton(1, "Guard Camp", toggleMidokaGuardingCamp).disableIf(MidokaGuardingCamp > 0, "She cannot guard your camp yet, maybe talk to her first.").disableIf(MidokaAge < 5, "She is too young and inexperienced to guard your camp yet");
        if (player.hasPerk(PerkLib.BasicLeadership))
        {
            if (flags[kFLAGS.PLAYER_COMPANION_1] == "")
            {
                addButton(12, "Team", midokaHenchmanOption).hint("Ask Midoka to join you in adventures outside camp.").disableIf(MidokaAge < 5, "She is too young and inexperienced to fight as your companion yet");
            }
            else if (flags[kFLAGS.PLAYER_COMPANION_1] == "Midoka")
            {
                addButton(12, "Team", midokaHenchmanOption).hint("Ask Midoka to stay in camp.");
            }
            else
            {
                addButtonDisabled(12, "Team", "You already have other henchman accompany you. Ask him/her to stay at camp before you talk with Etna about accompaning you.");
            }
        }
    ...
    public function midokaHenchmanOption() : Void  //TODO Please elaborate on her Combat texts in \Scenes\Combat\CombatFollowerActions  
    {
        ...
        if (flags[kFLAGS.PLAYER_COMPANION_1] == "")
        {
            ...
            var strMidoka : Float = 85;
            var meleeAtkMidoka : Float = 28;
            strMidoka *= (1 + (0.2 * player.newGamePlusMod()));
            strMidoka = Math.round(strMidoka);
            meleeAtkMidoka += (1 + as3hx.Compat.parseInt(meleeAtkMidoka / 5)) * player.newGamePlusMod();
