...
class SidonieFollower extends NPCAwareContent
{
    private function anyHermInCamp() : Bool
    {
    ...
    public function wonGnollPackFight() : Void
    {
        ...
        {
            outputText(" and how it’s now slowly returning to normal, and other trivial stuff");
        }
    ...
    public function commingToCamp() : Void
    {
        ...
        {
            outputText(" and meet the rest of the camp inhabitants");
        }
        ...
        if (camp.companionsCount() >= 2)
        {
            ...
            if (sophieFollower() && flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 0 && !player.hasStatusEffect(StatusEffects.SophieOff))
            {
                ...
                {
                    outputText("\"<i>That blondie ditz is really annoying, you know,</i>\" the horse-girl says, pointing at the bimbofied harpy, \"<i>I was still unpacking my things and she jumped from nowhere asking if she could suck my dick. Anyways, I guess she is special to you, so I’ll do my best to be polite with her.</i>\"\n\n");
                }
            }
            if (flags[kFLAGS.IZMA_FOLLOWER_STATUS] == 1 && flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 0)
            {
                outputText("\"<i>Near the stream, I caught sight of a shark fin, and look at my surprise when I found that we were living with one of those tigersharks, that girl Izma. She was not a bloodthirsty or lust crazed as average shark girls, that’s for sure.</i>\"\n\n");
            }
            if (isabellaFollower() && flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 0)
            {
                outputText("\"<i>I also met that cow-looking girl, Izzy. It’s nice to know that I’m not the only tough looking gal at camp, huh? We even had the chance to have a little strength contest.</i>\"\n\n");
            }
        }
    ...
    public function mainSidonieMenu() : Void
    {
        ...
        //Kiha interactions{
            ...
            if (!KihaFollower.SidonieInteractionHappened)
            
            //first time - with return to camp{
            ...
            {
                kihaFollower.sidonieInteraction(true);
            }
        }
        else
        {
    ...
    public function SidonieTalkThePlains() : Void
    {
        ...
        {
            outputText(" The redhead girl on your camp reminds me a bit of her, but since she hasn’t come yet to hit on me, I doubt that they’re the same person. Still gives off the ‘haughty slut’ vibe.");
        }
        ...
        {
            outputText("You’re sure that the Bazaar is quite the place. And everything from the Black Cock has the fame of being quite delicious.");
        }
        else
        {
            outputText("You’ve still to visit that place, but from all you’ve heard, seems like it’s worth the effort.");
        }
    ...
    public function SidonieTheCamp() : Void
    {
        ...
        {
            outputText("\"<i>I see that you follow the ‘less is more’ idea.</i>\" Sidonie jokes. \"<i>That said, this place seems a bit far from the rest of Mareth, while, at the same time is recognizable enough to be easily found if you know what you’re looking.</i>\"\n\n");
        }
        if (flags[kFLAGS.CAMP_WALL_PROGRESS] >= 100)
        {
            ...
            {
                outputText("Still, those skulls send a clear message to that corrupted scum.");
            }
            outputText("</i>\"\n\n");
        }
        if (flags[kFLAGS.CAMP_CABIN_PROGRESS] >= 0)
        {
            ...
            {
                outputText("\"<i>Now I see where all the wood that you requested from me before went. It’s good to know that you have given it a good use. When I checked it seems solid and well-built, and I bet that sleeping in a proper bed beats having to sleep on the ground, by far.</i>\"\n\n");
            }
            else
            {
                outputText("\"<i>You must really like that bedroll of yours, by what I’ve seen. Not judging, of course. I suppose that it must be quite cozy. I’m only worried about that you could be a bit...exposed, in here.</i>\"\n\n");
            }
        }
        if (flags[kFLAGS.CAMP_UPGRADES_HOT_SPRINGS] >= 4)
        {
            ...
            if (anyHermInCamp())
            {
                outputText(", and since some of us girls have extra ‘equipment’ too, I’m quite comfortable with them,");
            }
            if (player.hasCock())
            {
        ...
        if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] >= 2)
        {
            ...
            if ((isabellaFollower() && flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 0) || followerKiha() || emberScene.followerEmber() || player.hasStatusEffect(StatusEffects.PureCampJojo) && flags[kFLAGS.JOJO_BIMBO_STATE] != 3)
            {
                outputText(" Though some of our pals are really hard to crack.");
            }
            outputText("</i>\"\n\n");
        }
        doNext(SidonieTalkMenu);
    ...
    private function object() : String
    {
        ...
        if (temp == 0)
        {
            buffer += "white-striped bark";
        }
        else if (temp == 1)
        {
            buffer += "a walnut-like appearance";
        }
        else if (temp == 2)
        {
            buffer += "a lemon-smelling bark";
        }
        else
        {
            buffer += "mahogany color on them";
        }
        return buffer;
    }
    public function SidonieRecollection() : Void
    {
        ...
        else
        {
            ...
            if (player.str < 50)
            {
                outputText("require a bit more time for it");
            }
            else
            {
                outputText("chop the wood without any particular issue");
            }
            ...
            if (player.hasStatusEffect(StatusEffects.ResourceNode1) && player.statusEffectv1(StatusEffects.ResourceNode1) < 4)
            {
                player.addStatusValue(StatusEffects.ResourceNode1, 1, 1);
            }
            else
            {
                player.createStatusEffect(StatusEffects.ResourceNode1, 0, 0, 0, 0);
            }
            flags[kFLAGS.SIDONIE_RECOLLECTION] = 46;
            if ((CampStatsAndResources.WoodResc + 300) < SceneLib.campUpgrades.checkMaterialsCapWood())
            {
    ...
    public function SidonieSexMenu() : Void
    {
        ...
        if (flags[kFLAGS.SIDONIE_SEX_MENU] == 1)
        {
            ...
            {
                outputText("" + player.skinFurScales() + " ");
            }
        ...
        else
        {
            ...
            {
                outputText(" and now she’s selling a lot of pieces to the He Xin Dao folks,");
            }
            if (flags[kFLAGS.FACTORY_SHUTDOWN] == 1)
            {
                outputText(" or how the sky, slowly but surely, is acquiring a more blueish color");
            }
            ...
            if (player.gender == 3)
            {
                outputText("with your [cock] getting harder and your [vagina] getting wetter as time goes");
            }
            if (player.gender == 1)
            {
                outputText("[cock] getting harder");
            }
            if (player.gender == 2)
            {
                outputText("[vagina] getting wetter");
            }
    ...
    public function SidonieSexGetFucked() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("increasingly hard [cock]");
        }
        else if (player.hasVagina() && !player.hasCock())
        {
            outputText("crotch, idly teasingly your clit");
        }
        outputText(".");
        if (player.hasBalls())
        {
            outputText(" From time to time she grabs your [sack], playing teasingly with the soft " + player.skinFurScales() + " covering your ballsack.");
        }
        outputText("\n\nBefore you realize it, you’re really worked up after all those ministrations, ");
        if (player.gender == 3)
        {
            outputText("your [cock] achingly hard and your pussy wet and needy");
        }
        if (player.gender == 1)
        {
            outputText("your [cock] achingly hard ");
        }
        if (player.gender == 2)
        {
            outputText("your pussy wet and needy ");
        }
        ...
        {
            outputText("[tail] enticingly");
        }
        else
        {
            outputText("backside enticingly to her");
        }
        ...
        if (player.gender == 3)
        {
            outputText("[cock] is hard and leaking precum, your body taken by lust under the dicking that you’re getting and [cunt] is wet and needy for the dicking that isn’t getting");
        }
        if (player.gender == 1)
        {
            outputText("[cock] is hard and leaking precum, your body taken by lust under the dicking that you’re getting");
        }
        if (player.gender == 2)
        {
            outputText("[cunt] is wet and needy for the dicking that isn’t getting");
        }
        ...
        {
            outputText("One of her hands puts away your [wings] near your shoulder, so she can see your panting face while she pounds you.");
        }
        ...
        if (player.hasCock())
        {
            outputText(", your own [cock] erupting below you at the pleasure of having Sidonie cumming inside you");
        }
    ...
    public function SidonieSexBlowHer() : Void
    {
        ...
        if (player.hasBeak())
        {
            outputText("You slide your tongue playfully over Sidonie’s ");
        }
        else
        {
            outputText("You plant a kiss on Sidonie’s flare, ");
        }
        ...
        {
            outputText("tousles your hair");
        }
        else
        {
            outputText("caresses your head with the other");
        }
        ...
        if (player.gender == 1)
        {
            outputText("[cock] aches");
        }
        if (player.gender == 2)
        {
            outputText("[vagina] aches");
        }
        if (player.gender == 3)
        {
            outputText("[cock] and [vagina] ache");
        }
        ...
        if (player.gender == 1)
        {
            outputText("cock erupts in a hot, cummy mess.");
        }
        if (player.gender == 2)
        {
            outputText("pussy wets itself with girlcum.");
        }
        if (player.gender == 3)
        {
            outputText("cock erupts in a hot, cummy mess, while you pussy wets itself with girlcum.");
        }
    ...
    public function SidonieSex69() : Void
    {
        ...
        if (player.cumQ() > 1500)
        {
            outputText("assure her that she’ll get surprised");
        }
        else if (player.cumQ() > 200)
        {
            outputText("tell her while not a cum cascade, you produce quite enough");
        }
        else
        {
            outputText("timidly answer that you’re not exactly a fountain, but you won’t disappoint her");
        }
        ...
        {
            outputText("Given how big she is compared to you, you can’t help but feel a little intimidated.");
        }
        else
        {
            outputText("Being as big as you, you have no trouble accommodating her atop you.");
        }
        ...
        if (player.hasBalls())
        {
            outputText(" and kneading your balls with one of her free hands");
        }
        ...
        if (player.cumQ() > 1500)
        {
            outputText("with a bit of difficulty");
        }
        else
        {
            outputText("without issue");
        }
