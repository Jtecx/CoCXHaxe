...
class SiegweirdFollower extends NPCAwareContent
{
    ...
    public function siegweirdFirstEncounterPostFight() : Void
    {
        ...
        {
            siegweirdFirstEncounterPostFightAnotherFight();
        }
        else
        {
            siegweirdFirstEncounterPostFightJoin();
        }
    }
    ...
    public function siegweirdMainCampMenu() : Void
    {
        ...
        {
            addButtonDisabled(5, "Soup", "You already ate today bowl of Siegweird’s soup.");
        }
        else
        {
            addButton(5, "Soup", siegweirdCampSoup);
        }
        if (!soupMaxLevel())
        {
            addButton(9, "Add an ingredient", addIngredientMenu);
        }
        if (player.hasStatusEffect(StatusEffects.SiegweirdTraining))
        {
            addButtonDisabled(10, "Study", "You already completed basic Study.");
        }
        else
        {
            addButton(10, "Study", siegweirdCampStudy);
        }
    ...
    public function soupMaxLevel() : Bool
    {
        return player.statusEffectv1(StatusEffects.SiegweirdSoup1) > 0 && player.statusEffectv1(StatusEffects.SiegweirdSoup2) > 0 && player.statusEffectv1(StatusEffects.SiegweirdSoup3) > 0 && player.statusEffectv1(StatusEffects.SiegweirdSoup4) > 0 && player.statusEffectv1(StatusEffects.SiegweirdSoup5) > 0 && player.statusEffectv2(StatusEffects.SiegweirdSoup1) > 0 && player.statusEffectv2(StatusEffects.SiegweirdSoup2) > 0 && player.statusEffectv2(StatusEffects.SiegweirdSoup3) > 0 && player.statusEffectv2(StatusEffects.SiegweirdSoup4) > 0 && player.statusEffectv2(StatusEffects.SiegweirdSoup5) > 0 && player.statusEffectv3(StatusEffects.SiegweirdSoup1) > 0;
    }
    ...
    public function soupRecovery() : Float
    {
        var recoveryV : Float = 0.25;
        ...
        if (player.statusEffectv1(StatusEffects.SiegweirdSoup1) > 0)
        {
            recoveryV += 0.05;
        }
        if (player.statusEffectv1(StatusEffects.SiegweirdSoup2) > 0)
        {
            recoveryV += 0.05;
        }
        if (player.statusEffectv1(StatusEffects.SiegweirdSoup3) > 0)
        {
            recoveryV += 0.05;
        }
        if (player.statusEffectv1(StatusEffects.SiegweirdSoup4) > 0)
        {
            recoveryV += 0.05;
        }
        if (player.statusEffectv1(StatusEffects.SiegweirdSoup5) > 0)
        {
            recoveryV += 0.05;
        }
        if (player.statusEffectv2(StatusEffects.SiegweirdSoup1) > 0)
        {
            recoveryV += 0.05;
        }
        if (player.statusEffectv2(StatusEffects.SiegweirdSoup2) > 0)
        {
            recoveryV += 0.05;
        }
        if (player.statusEffectv2(StatusEffects.SiegweirdSoup3) > 0)
        {
            recoveryV += 0.05;
        }
        if (player.statusEffectv2(StatusEffects.SiegweirdSoup4) > 0)
        {
            recoveryV += 0.05;
        }
        if (player.statusEffectv2(StatusEffects.SiegweirdSoup5) > 0)
        {
            recoveryV += 0.05;
        }
        if (player.statusEffectv3(StatusEffects.SiegweirdSoup1) > 0)
        {
            recoveryV += 0.05;
        }
        return recoveryV;
    ...
    public function siegweirdCampSoup() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.SiegweirdSoup))
        {
            ...
            var recoveryV : Float = soupRecovery();
    ...
    public function siegweirdAdvancedStudy_0() : Void
    {
        ...
        else
        {
            ...
            {
                outputText("But be wary that it won't work unless she's still pure. And since the lake where she lived is tainted too... the chance of that is low.");
            }
            else
            {
                ...
                {
                    outputText("She's still alive... but to finish her purification, you must shut down the factory. There are no other options.");
                }
                else if (flags[kFLAGS.MARAE_QUEST_COMPLETE] == 1)
                {
                    outputText("You have disabled the factory? Excellent! And that pearl you mentioned will be an excellent replacement for the bark!");
                }
                else if (flags[kFLAGS.MET_MARAE_CORRUPTED] > 0)
                {
                    outputText("So... she's fallen to it too. Forget about it then. Even if you manage to obtain her bark, it will probably be tainted too... Unless you've got some <b>before</b> she'd become corrupted. But I don't think it's possible...");
                }
            }
    ...
    public function siegweirdAdvancedStudy_1(usePearl : Bool) : Void
    {
        clearOutput();
        if (usePearl)
        {
