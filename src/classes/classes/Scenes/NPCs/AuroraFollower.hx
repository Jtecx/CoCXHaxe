...
class AuroraFollower extends NPCAwareContent
{
    ...
    public function auroraCampMenu() : Void
    {
        ...
        //talk
        addButton(4, "Spar", auroraSparsWithPC).hint("Ask Aurora for a mock battle.").disableIf(flags[kFLAGS.PLAYER_COMPANION_1] == "Aurora" || flags[kFLAGS.PLAYER_COMPANION_2] == "Aurora", "You can't fight against her as long she's in your team.").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, "You need a good sparring ring for that.");
        if (player.hasPerk(PerkLib.BasicLeadership))
        {
            addButton(5, "Team", auroraHenchmanOption);
        }
        else
        {
        ...
        {
            if (camp.FisheryWorkersCount() < camp.FisheryMaxWorkersCount() && flags[kFLAGS.PLAYER_COMPANION_1] != "Aurora" && flags[kFLAGS.PLAYER_COMPANION_2] != "Aurora")
            {
                addButton(6, "Fishery Work", sendToFishery);
            }
            if (flags[kFLAGS.FOLLOWER_AT_FISHERY_1] == "Aurora" || flags[kFLAGS.FOLLOWER_AT_FISHERY_2] == "Aurora" || flags[kFLAGS.FOLLOWER_AT_FISHERY_3] == "Aurora")
            {
                addButton(6, "Stop Work", backFromFishery);
            }
        }
        else
        {
            addButtonDisabled(6, "???", "You need to have functional fishery first.");
        }
    ...
    public function AuroraLostSparring() : Void
    {
        ...
        {
            if (flags[kFLAGS.AURORA_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.AURORA_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.AURORA_DEFEATS_COUNTER] = 1;
            }
            if (flags[kFLAGS.AURORA_DEFEATS_COUNTER] == 1 && flags[kFLAGS.AURORA_LVL] == 1)
            {
                ...
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 2, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction));
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers4, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction), 0, 0);
                }
            ...
            if (flags[kFLAGS.AURORA_DEFEATS_COUNTER] == 2 && flags[kFLAGS.AURORA_LVL] == 2)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers4))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 2, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 2);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers4, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 2, 0, 0);
                }
                flags[kFLAGS.AURORA_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.AURORA_DEFEATS_COUNTER] == 3 && flags[kFLAGS.AURORA_LVL] == 3)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers4))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 2, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 3);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers4, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 3, 0, 0);
                }
                flags[kFLAGS.AURORA_DEFEATS_COUNTER] = 0;
    ...
    public function auroraHenchmanOption() : Void
    {
        ...
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "Aurora" || flags[kFLAGS.PLAYER_COMPANION_3] == "Aurora")
            {
                addButtonDisabled(0, "Team (1)", "You already have Aurora accompany you.");
            }
            else
            {
                addButton(0, "Team (1)", auroraHenchmanOption2, 1).hint("Ask Aurora to join you in adventures outside camp.");
            }
        }
        else if (flags[kFLAGS.PLAYER_COMPANION_1] == "Aurora")
        {
            addButton(5, "Team (1)", auroraHenchmanOption2, 21).hint("Ask Aurora to stay in camp.");
        }
        else
        {
            addButtonDisabled(5, "Team (1)", "You already have other henchman accompany you as first party member. Ask him/her to stay at camp before you talk with Aurora about accompaning you as first party member.");
        }
        if (player.hasPerk(PerkLib.IntermediateLeadership))
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "")
            {
                if (flags[kFLAGS.PLAYER_COMPANION_1] == "Aurora" || flags[kFLAGS.PLAYER_COMPANION_3] == "Aurora")
                {
                    addButtonDisabled(1, "Team (2)", "You already have Aurora accompany you.");
                }
                else
                {
                    addButton(1, "Team (2)", auroraHenchmanOption2, 2).hint("Ask Aurora to join you in adventures outside camp.");
                }
            }
            else if (flags[kFLAGS.PLAYER_COMPANION_2] == "Aurora")
            {
                addButton(6, "Team (2)", auroraHenchmanOption2, 22).hint("Ask Aurora to stay in camp.");
            }
            else
            {
                addButtonDisabled(6, "Team (2)", "You already have other henchman accompany you as second party member. Ask him/her to stay at camp before you talk with Aurora about accompaning you as second party member.");
            }
        }
        else
        {
    ...
    public function auroraHenchmanOption2(slot : Float = 1) : Void
    {
        clearOutput();
        if (slot < 21)
        {
            ...
            var strAurora : Float = 10;
            var touAurora : Float = 300;
            var meleeAtkAurora : Float = 5;
            ...
            meleeAtkAurora += (1 + as3hx.Compat.parseInt(meleeAtkAurora / 5)) * player.newGamePlusMod();
            player.createStatusEffect(StatusEffects.CombatFollowerAurora, strAurora, touAurora, meleeAtkAurora, 0);
            if (slot == 2)
            {
                flags[kFLAGS.PLAYER_COMPANION_2] = "Aurora";
            }
            if (slot == 1)
            {
    ...
    private function sendToFishery() : Void
    {
        ...
        {
            flags[kFLAGS.FOLLOWER_AT_FISHERY_1] = "Aurora";
        }
        else if (flags[kFLAGS.FOLLOWER_AT_FISHERY_2] == "")
        {
            flags[kFLAGS.FOLLOWER_AT_FISHERY_2] = "Aurora";
        }
        else if (flags[kFLAGS.FOLLOWER_AT_FISHERY_3] == "")
        {
            flags[kFLAGS.FOLLOWER_AT_FISHERY_3] = "Aurora";
        }
        doNext(auroraCampMenu);
    }
    ...
    private function backFromFishery() : Void
    {
        ...
        if (flags[kFLAGS.FOLLOWER_AT_FISHERY_1] == "Aurora")
        {
            flags[kFLAGS.FOLLOWER_AT_FISHERY_1] = "";
        }
        else if (flags[kFLAGS.FOLLOWER_AT_FISHERY_2] == "Aurora")
        {
            flags[kFLAGS.FOLLOWER_AT_FISHERY_2] = "";
        }
        else if (flags[kFLAGS.FOLLOWER_AT_FISHERY_3] == "Aurora")
        {
