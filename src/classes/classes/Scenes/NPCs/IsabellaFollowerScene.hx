...
class IsabellaFollowerScene extends NPCAwareContent
{
    ...
    {
        if (flags[kFLAGS.ISABELLA_POTENCY_STATE] == 1 && !isabellaScene.pregnancy.isPregnant)
    ...
    @:allow(classes.scenes.nPCs)
    private function isabellaMoovesInGreeting() : Void
    {
        ...
        if (flags[kFLAGS.ISABELLA_TIMES_OFFERED_FOLLOWER] == 0)
        {
            ...
            if (player.cor < 33)
            {
                outputText("smile, and admit that you do enjoy the time you spend with her.");
            }
            else if (player.cor < 66)
            {
                outputText("grin and joke that she probably missed you more.");
            }
            else
            {
                outputText("smirk and wonder if you missed her or her magnificent body.");
            }
            ...
            {
                outputText("crush around your head, smothering you in heavy, milk-filled breast flesh");
            }
            else
            {
                outputText("squeeze against your " + chestDesc() + ", the four hard nipples capping each milk-flooded breast pressing on your own.");
            }
    ...
    public function callForFollowerIsabella() : Void
    {
        ...
        if (flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 0)
        {
            ...
            {
                outputText("picks you up in her arms and embraces you with a crushing hug, nearly burying you in her boobflesh before she lets you go.");
            }
            else
            {
                outputText("gives you a crushing hug, smashing her tits flat against your body.");
            }
            outputText("  She says, \"<i>");
            if (isabellaAccent())
            {
                outputText("Hi [name]!  Vat do you need from Izabella?");
            }
            else
            {
                outputText("Hiya [name]!  Anything I can help you with?");
            }
            outputText("</i>\"");
            if (player.hasCock())
            {
        ...
        addButton(2, "Sex", campIzzySexMenu).hint("Have some sex with the cow-girl.").disableIf(player.lust < 33, "You aren't aroused enough to initiate sex.");
        addButton(3, "Spar", isabellaSparMenu).hint("Get into a quick battle with Isabella!").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, "You need a better ring for that.");
        if (flags[kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT] < 100)
        {
            addButton(4, "Accent Coach", isabellasAccentCoaching).hint("Teach Isabella to talk in normal accent.");
        }
        else
        {
            addButton(4, "Accent Uncoach", isabellaAccentUncoaching).hint("Let Isabella talk the way she wants. This will pretty much throw away all the coaching progress you've made.");
        }
        if (flags[kFLAGS.ISABELLA_MILKED_YET] < 0)
        {
            addButton(5, "Get Milk", getMilk).hint("Get a bottle of Isabella's milk.");
        }
        if (player.hasItem(consumables.PROBOVA))
        {
            outputText("\n\n<b>Isabella would probably drink a bottle of Pro Bova if you gave it to her.</b>");
        }
        addButton(6, "GiveProBova", isabellaBurps).hint("Give a bottle of Pro Bova to Isabella?", "Give Pro Bova").disableIf(player.isGenderless(), "Not for genderless!").disableIf(!player.hasItem(consumables.PROBOVA), "You don't have any");
        if (flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 0 && flags[kFLAGS.FARM_CORRUPTION_STARTED] == 1)
        {
    ...
    private function isabellasAppearance() : Void
    {
        ...
        {
            var _sw18_ = (isabellaScene.pregnancy.event);            
            ...
            switch (_sw18_)
            {
                case 1:
                    ...
                    if (player.cumQ() >= 500)
                    {
                        outputText("and your potent babymaking skills, ");
                    }
                ...
                case 2:
                    ...
                    if (player.lib >= 60)
                    {
                        outputText("Every once in a while when the wind blows just right you get a pleasing view of her well lubricated womanhood between her legs. ");
                    }
    ...
    {
        menu();
        addButton(0, (flags[kFLAGS.ISABELLA_BLOWJOBS_DISABLED] != null) ? "Plz BJs" : "No BJs", toggleIsabellasMorningWoodChopping).disableIf(!player.hasCock(), "No cocks - no blowjobs!");
        addButton(5, "Get Sucked", isabellaScene.izzyGivesSmallWangsFreeOral).hint("You could have her suck you off now, since she seems quite taken with the size of your penis.").disableIf(!player.hasCock() || player.shortestCockLength() >= 9, "Requires a dick shorter than 9 inches.");
        addButton(10, "Service Her", isabellaScene.volunteerToSlurpCowCunt).hint("There's always the opportunity to climb under her skirt and service her as well.");
        addButton(1, "Fuck Her", fuckIsabella).disableIf(player.cockThatFits(164) < 0, "Req. a cock fitting 164 area.");
        addButton(6, "Hotdog", curry(sceneHunter.callFitNofit, repeatGermanBratwurstInCamp, 38)).disableIf(!player.hasCock(), "Req. a cock.");
        addButton(11, "TentacleSex", tentacleBoneFollowerIzzy).hint("You could take advantage of your numerous tentacles to show her a truly grand time.").disableIf(player.countCocksWithType(CockTypesEnum.TENTACLE, 24, -1, "length") < 3, 
                "Requires at least 3 tentacles longer than 24 inches"
        );
        addButton(2, "Drink Milk", isabellaScene.nomOnMommaIzzysTits).hint("You could get a drink of Isabella's delicious milk, straight from the tap.");
	...
	based on inte)*/
        ...
        var result : Float = player.inte / 5 + rand(9);
        var accentChange : Float = 0;
        //(Success/Fail texts)
        if (result < 12)
        {
            switch (rand(3))
        ...
        else if (result < 14)
        {
            if (rand(2) == 0)
            {
                outputText("You spend an hour going over Isabella's language and diction with her.  She seems to make a little progress with it, which makes her happy, but you feel that in the grand scheme of things, she could do better.");
            }
            else
            {
                outputText("You go over Isabella's word choice with her and try to correct her whenever she uses the wrong word or slips into her native tongue.  She seems to make a modicum of progress, yet the downward cast of her eyes shows just how much better she feels she could have done.");
            }
        ...
        else
        {
            if (rand(2) == 0)
            {
                outputText("You sit down with Isabella to work on her language once again.  She gets quite involved in the lesson, and the cow-girl makes large strides toward lessening her accent.");
            }
            else
            {
                outputText("You go over Isabella's word choice with her in an effort to reduce her accent.  In short order, the cow-girl is looking at you with rapt, attentive eyes, hanging on to your every word as you help her disguise her tongue to match those she's surrounded by.");
            }
        ...
        if (flags[kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT] > 100)
        {
            flags[kFLAGS.ISABELLA_ACCENT_TRAINING_PERCENT] = 100;
        }
    ...
    public function isabellaMorningWakeupCall() : Void
    {
        ...
        var x : Float = player.shortestCockIndex();
        outputText("\n");
        ...
        {
            outputText("Isabella is fellating you!?");
        }
        else
        {
            outputText("Isabella is giving you one of her traditional good-morning blowjobs.");
        }
        ...
        if (player.hasBalls())
        {
            outputText(ballsDescriptLight() + " against her chin, the cum-bloated orbs bouncing and shuddering with your " + cockDescript(x) + " as release nears.");
        }
        else
        {
        ...
        if (player.hasBalls())
        {
            outputText("  Your balls clench tightly as they release your seed");
        }
        else
        {
            outputText("  You clench tightly and release your seed");
        }
        ...
        if (player.cockTotal() > 1)
        {
            ...
            {
                outputText("the one penis she's left untouched");
            }
        ...
        if (player.cumQ() < 250)
        {
            outputText("  She slurps it all down with a smile, giving you a happy wink as her throat swallows the last of your salty load.");
        }
        else if (player.cumQ() < 500)
        {
            outputText("  She swallows hard, but even with her talents, Isabella can't quite get it all down.  The cum-dribbling red-head gives you a wink as you finish, gathering the spooge with her fingers to swallow down the excess.");
        }
        else
        {
            ...
            if (player.cumQ() >= 2000)
            {
                outputText("  Even that fails when Isabella's body becomes too semen-stuffed to take any more, and the cow-girl pulls you out of her mouth to let you spray the rest on her face.  Her tongue strokes you like a hand throughout the entire thing, encouraging you to make as big of a mess as possible.");
            }
        ...
        if (flags[kFLAGS.ISABELLA_MORNING_FELLATIO_COUNT] == 1)
        {
            if (isabellaAccent())
            {
                outputText("Mmm, your penis has such tasty treats for me.  Keep zat cock small enough for mein mouth and I vill be happy to tend to it every morning, ja?");
            }
            else
            {
                outputText("Mmm, your penis has such tasty treats for me.  Keep that cock small enough for my mouth and I'll be happy to tend to it every morning, okay?");
            }
        }
        else if (player.cumQ() >= 500)
        {
            if (isabellaAccent())
            {
                outputText("Yum!  You cum so hard for momma Izabella, don't you?</i>\"  The sated cow-girl gives your " + cockDescript(x) + " an affectionate squeeze before murmuring, \"<i>So yummy...");
            }
            else
            {
                outputText("Yum!  You cum so hard for momma Isabella, don't you?</i>\"  The sated cow-girl gives your " + cockDescript(x) + " an affectionate squeeze before murmuring, \"<i>So yummy...");
            }
        }
        else if (isabellaAccent())
        {
            outputText("I hope you did not mind me getting zat snack...");
        }
        else
        {
            outputText("I hope you didn't mind me getting that snack...");
        }
        outputText("</i>\"\n\n");
    ...
    private function toggleIsabellasMorningWoodChopping() : Void
    {
        ...
        if (flags[kFLAGS.ISABELLA_BLOWJOBS_DISABLED] == 0)
        {
            ...
            if (isabellaAccent())
            {
                outputText("Just let Isabella know if you change your mind, ja?</i>\"");
            }
            else
            {
                outputText("Just let me know if you change your mind, all right?</i>\"");
            }
        ...
        else
        {
            ...
            if (player.cor < 33)
            {
                outputText("blush and ");
            }
            else if (player.cor >= 66)
            {
                outputText("smirk and ");
            }
            ...
            if (player.cocks[player.shortestCockIndex()].cockLength > 9)
            {
                if (isabellaAccent())
                {
                    outputText("I vill, but only once you shrink zat monster down.  I cannot handle somezing like zat!");
                }
                else
                {
                    outputText("I will, but only once you shrink that monster down.  I can't handle something like that!");
                }
            }
            else if (isabellaAccent())
            {
                outputText("Of course; ve vouldn't vant you to be out zere battling demons unsated.  Who knows vat vould happen?");
            }
            else
            {
                outputText("Of course; we wouldn't want you to be out there battling demons unsated.  Who knows what would happen?");
            }
            outputText("</i>\"");
    ...
    private function repeatGermanBratwurstInCamp(x : Int) : Void
    {
        ...
        if (flags[kFLAGS.ISABELLA_TIME_SINCE_LAST_HOTDOGGING] < 5 && flags[kFLAGS.ISABELLA_TIME_SINCE_LAST_HOTDOGGING] > 0)
        {
            if (isabellaAccent())
            {
                outputText("mumbles, \"<i>Already?  Did I not just take care of you?</i>\"  The blushing cow-girl teases, \"<i>Are you really zat horny, or do you just like mein heiny that much?");
            }
            else
            {
                outputText("mumbles, \"<i>Already?  Didn't I just take care of you?</i>\"  The blushing cow-girl teases, \"<i>Are you really that horny, or do you just like my heiny that much?");
            }
        }
        ...
        //If too big, use the biggest dick for hotdoggin!{
            ...
            if (isabellaAccent())
            {
                outputText("gasps, \"<i>Oh yes, I see.  It looks so pent up... a shame it is too big to get ze full treatment in mein caboose.  You vill still cum for mein cheeks though, ja?");
            }
            else
            {
                outputText("gasps, \"<i>Oh yes, I see.  It looks so pent up... a shame it's too big for you to get the full treatment in my caboose.  You'll still cum for my cheeks though, right?");
            }
        }
        else if (isabellaAccent())
        {
            outputText("gleefully muses, \"<i>Oooh, I see.  Of course I vill help you, [name]!</i>\"  She traces a finger around " + oMultiCockDesc() + "' outline and continues, \"<i>It vould be mein pleasure to squeeze it all out into mein caboose.  Zat is what you vant, nein?");
        }
        else
        {
            outputText("gleefully muses, \"<i>Oooh, I see.  Of course I'll help you, [name]!</i>\"  She traces a finger around " + oMultiCockDesc() + "' outline and continues, \"<i>It would be my pleasure to squeeze it all out into my caboose.  That is what you want, no?");
        }
        outputText("</i>\"\n\n");
        
        if (player.cor < 33)
        {
            outputText("Coloring red, you sheepishly answer in the affirmative.");
        }
        else if (player.cor < 66)
        {
            outputText("Not seeing a reason to hide it, you admit as much.");
        }
        else
        {
            outputText("With a knowing twinkle in your eye, you agree with the busty woman's intuition, rocking your hips enticingly.  Her eyes glue to your manhood for a moment, before she remembers herself and meets your gaze.");
        }
        ...
        if (player.cor < 33)
        {
            outputText("feel the weight of shame settling over you");
        }
        else if (player.cor < 66)
        {
            outputText("feel mildly ashamed");
        }
        else
        {
            outputText("color at getting caught staring");
        }
        ...
        if (isabellaAccent())
        {
            outputText("Isabella crooks a finger at you and commands, \"<i>Come, sit down, but lean back so Isabella has plenty of room to vork at milking zis naughty ");
        }
        else
        {
            outputText("Isabella crooks a finger at you and commands, \"<i>Come, sit down, but lean back so I have plenty of room to work at milking this naughty ");
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>So wet,</i>\" ");
        }
        else
        {
            outputText("\"<i>So vet,</i>\" ");
        }
        outputText("she moans before resuming her shaking, ");
        if (isabellaAccent())
        {
            outputText("\"<i>So hot... Am I making you hot, [name]?  Is your cock twitching for me yet?  Aching and leaking and dribbling a mess all over itself, ja?</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>So hot... Am I making you hot, [name]?  Is your cock twitching for me yet?  Aching and leaking and dribbling a mess all over itself?</i>\"\n\n");
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>I said not yet.  Do you vant me to help you cum or not?</i>\"");
        }
        else
        {
            outputText("\"<i>I said not yet.  Do you want me to help you cum or not?</i>\"");
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Good " + player.mf("boy", "girl") + ".  Relax, Momma Isabella vill take care of you.</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Good " + player.mf("boy", "girl") + ".  Relax, Momma Isabella will take care of you.</i>\"\n\n");
        }
        ...
        if (player.cockTotal() == 2)
        {
            outputText("The other penis is shoved to the side, forgotten about as Isabella selects the lucky fuck-stick.  ");
        }
        else if (player.cockTotal() > 2)
        {
            outputText("The other penises are shoved aside, forgotten about as Isabella selects the lucky fuck-stick.  ");
        }
        ...
        if (player.cockArea(x) <= 38)
        {
            outputText(", though with your dick completely devoured by her big butt-cheeks, you may as well be.");
        }
        else
        {
            outputText(", as the mast rising from the top of her crack indicates.");
        }
        ...
        if (player.cockArea(x) <= 38)
        {
            if (isabellaAccent())
            {
                outputText("\"<i>Oh my, does mein behind feel so good zat your cute, delectable cock vill squirt all over me before you even get to feel ze best part?</i>\" teases Isabella as she flexes one side and then the other, forcing your " + cockDescript(x) + " to bend in response to the two dueling masses of her booty cheeks.  She keeps teasing you as she caresses your member with her hefty butt.  \"<i>You have such a naughty look on your face, drooling on yourself from just a little bit of play with Momma Isabella.  No, don't stop, I want to watch the ecstasy spread through you while I work your nice, hot cock.  My asshole wants to kiss it... Can I kiss your dick with my asshole?</i>\"");
            }
            else
            {
                outputText("\"<i>Oh my, does my ass-job feel so good that your cute, delectable cock is gonna squirt all over me before you even get to feel the best part?</i>\" teases Isabella as she flexes one side and then the other, forcing your " + cockDescript(x) + " to bend in response to the two dueling masses of her booty cheeks.  She keeps teasing you as she caresses your member with her hefty butt.  \"<i>You have such a naughty look on your face, drooling on yourself from just a little bit of play with Momma Isabella.  No, don't stop, I want to watch the ecstasy spread through you while I work your nice, hot cock.  My asshole wants to kiss it... Can I kiss your dick with my asshole?</i>\"");
            }
            //(if libido <=15:
            ...
            if (isabellaAccent())
            {
                outputText("Isabella roughly lifts up and tuts, \"<i>No, zat vill not do.</i>\"  ");
            }
            else
            {
                outputText("Isabella roughly lifts up and tuts, \"<i>No, that will not do.</i>\"  ");
            }
            ...
            if (player.cumQ() >= 1000)
            {
                ...
                if (player.cumQ() >= 2000)
                {
                    outputText(", eventually growing to a pregnant-looking curve");
                }
        ...
        else
        {
            if (isabellaAccent())
            {
                outputText("\"<i>Oh my, does mein ass feel zat good on ze huge, pervy thing you call a dick?</i>\" teases Isabella as she flexes one side then the other, forcing your " + cockDescript(x) + " to bend in response to the two dueling masses of her booty cheeks.  She keeps talking, \"<i>You have such a naughty look on your face, drooling all over yourself like a dog.  Is that what having a big dick does to you?  Does it make you so horny and weak-willed that you'll waste your cum all over my ass?</i>\"  Isabella stops her back-and-forth flexing in order to begin bouncing up and down.  Her soft, flexing buns caress your " + cockDescript(x) + " from base ");
            }
            else
            {
                outputText("\"<i>Oh my, does my ass-job feel that good on that huge, pervy thing you call a dick?</i>\" teases Isabella as she flexes one side then the other, forcing your " + cockDescript(x) + " to bend in response to the two dueling masses of her booty cheeks.  She keeps talking, \"<i>You have such a naughty look on your face, drooling all over yourself like a dog.  Is that what having a big dick does to you?  Does it make you so horny and weak-willed that you'll waste your cum all over my ass?</i>\"  Isabella stops her back-and-forth flexing in order to begin bouncing up and down.  Her soft, flexing buns caress your " + cockDescript(x) + " from base ");
            }
            if (player.cocks[x].cockLength > 48)
            {
                outputText("to as high as she can reach on the towering shaft's length");
            }
            else
            {
                outputText("to " + player.cockHead(x));
            }
            ...
            if (player.cumQ() <= 200)
            {
                outputText("  Her ass gets a nice glaze from your over-sized cock, though the cow-girl stands up just before you finish, letting the last spurt weakly drip on your belly.");
            }
            else if (player.cumQ() <= 500)
            {
                outputText("  Her ass is completely glazed by your over-sized cock's heavy cum-flow in short order, though she stands up off of you to let you spurt the last few drizzles on your own belly.");
            }
            else
            {
    ...
    private function tentacleBoneFollowerIzzy() : Void
    {
        ...
        if (x[10] < 0)
        {
            sceneHunter.print("Wow. You can use up to TEN dicks here!");
        }
        ...
        if (isabellaAccent())
        {
            outputText("Oh, mein Gott!  Zey... zey are... so hug- vait, vat, vat are you doing?");
        }
        else
        {
            outputText("My God!  They... they are... so hug- wait, what are you doing?");
        }
        outputText("</i>\"\n\n");
        
        if (isabellaAccent())
        {
            outputText("Without a word, you adroitly guide your squirming bunch of tentacle peckers toward her body, slowly encircling her. In no time your " + cockDescript(x[1]) + " is already wrapping her supple waist, quickly followed by your " + cockDescript(x[2]) + "; using the combined strength of your inhumanly powerful pair of dick muscles, you lift her off the ground.  \"<i>Oh... you're a kinky boy, aren't you? Well, if that's what you want...</i>\"");
        }
        else
        {
        ...
        if (player.canFly() && player.str >= 90)
        {
            ...
            if (isabellaAccent())
            {
                outputText("But... but vait!  I'm going to fall- I'm going to...</i>\"\n\n");
            }
            else
            {
                outputText("But... but wait!  I'm going to fall- I'm going to...</i>\"\n\n");
            }
        ...
        {
            sceneHunter.print("Check failed: high strength and wings!");
        }
        ...
        if (x[7] >= 0)
        {
            ...
            if (x[8] >= 0)
            {
                outputText("  It is soon joined by your " + cockDescript(x[8]) + ", each prick taking care of a nipple, teasing the holes with shallow penetrations by her hard nubs, the pre-cum battling against the flow of milk in the opposite direction and stretching your urethra with a sensation that makes you shiver wildly.");
            }
            outputText("  You keep man-, or dick-, handling her tits while the rest of your peckers viciously pound her; your ");
            if (x[8] >= 0)
            {
                outputText("pair of boob-fondling pricks almost tremble in rage, unable to find any hole left to fill. This only makes them tit-fuck her harder and faster.");
            }
            else
            {
                outputText("boob-fondling prick almost trembles in rage, unable to find any hole left to fill. This only makes it tit-fuck her harder and faster.");
            }
            outputText("\n\n");
        }
        //[if 9th or 10th cock]
        if (x[9] >= 0)
        {
            ...
            if (x[10] >= 0)
            {
                outputText("s are left with nothing to stuff, writhing in frustration.  You think it would be fair to put them to use: you wrap one around her wrists, binding her hands and immobilizing her to ease the penetration of the others, then run the tip across the unused parts of her body, and simply enjoy the caress of her delicate skin against your " + cockDescript(x[9]) + ", feeling her shiver under your fluid tentacle motion.  Your remaining " + cockDescript(x[10]) + " wraps around her face, squeezing her cheeks and pointing its tip toward her eyes, in order to make sure her every sense is saturated with cock.");
            }
            else
            {
                outputText(" is left with nothing to stuff, writhing in frustration.  You think it would be fair to put it to use: you wrap it around her wrists, binding her hands and immobilizing her to ease the penetration of the others, then run the tip across the unused parts of her body, and simply enjoy the caress of her delicate skin against your " + cockDescript(x[9]) + ", feeling her shiver under your fluid tentacle motion.");
            }
            outputText("\n\n");
        }
        ...
        if (player.canFly() && player.str >= 90)
        {
            outputText("you descend, setting your [feet] on the soil again but still holding Isabella herself in the air; ");
        }
        ...
        if (x[5] >= 0)
        {
            ...
            if (player.cumQ() >= 1500)
            {
                outputText(" until she looks 8 months pregnant");
            }
        }
        if (x[7] >= 0)
        {
            outputText("; drowning her nipples with your spunk");
        }
        //[if 10th cock]
        if (x[10] >= 0)
        {
            outputText(", and cuming on her face as you stare at her");
        }
        outputText(".  Her luscious eyes are ");
        if (x[10] < 0)
        {
            outputText("eventually");
        }
        else
        {
    ...
    public function milktasticLacticLactation() : Void
    {
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Ohh, mein milkers...</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Ohh, my milkers...</i>\"\n\n");
        }
    ...
    private function izzyMilkingMeinMilkersMya() : Void
    {
        ...
        if (isabellaAccent())
        {
            outputText("Ahh, [name]... it has been sooo long since I have been properly milked, mein poor breasts are as full as zey have ever been.  I have been trying to release ze pressure myself, but zere are just not enuff hours in ze day vis everything else I must do, und mein nipples get quite sore, ja?");
        }
        else
        {
            outputText("Ahh, [name]... it's been sooo long since I've been properly milked, my poor breasts are fuller than they've ever been.  I have been trying to release the pressure myself, but there are just not enough hours in the day with everything else I must do, and my nipples get quite sore.");
        }
        outputText("</i>\"\n\n");
        ...
        if (isabellaAccent())
        {
            outputText("Do you zink you could help me vis mein milkers?  Zere must be more zan a week's vorth stored up!</i>\"\n\n");
        }
        else
        {
            outputText("Do you think you could help me with my milkers?  There must be more than a week's worth stored up!</i>\"\n\n");
        }
        ...
        if (isabellaAccent())
        {
            outputText("Zen -sniff- vat");
        }
        else
        {
            outputText("Then -sniff- what");
        }
        ...
        if (player.cor < 33)
        {
            outputText("unbearable, but you quickly");
        }
        else
        {
            outputText("kind of arousing, actually, but at length you");
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Ohh, [name], I had forgotten all about zose milkers vith all ze verk around camp!  Yes, let us go zere immediately!</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Ohh, [name], I had forgotten all about those milkers with all the work around camp!  Yes, let us go there immediately!</i>\"\n\n");
        }
    ...
    private function izzyMilkingMeinMilkersMya2() : Void
    {
        ...
        {
            outputText("with creaking effort, initially calibrated to your slender form as it was");
        }
        else
        {
            outputText("startlingly quickly, being already set to your much weightier bulk");
        }
        outputText(".  The process begins in earnest and Isabella groans as the suction on her finger-chafed nipples begins.  ");
        if (isabellaAccent())
        {
            outputText("\"<i>Oooh, mein udders... zey feel as zough zey vill pop already!</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Oooh, my udders... they feel as though they will pop already!</i>\"\n\n");
        }
        ...
        if (isabellaAccent())
        {
            outputText("The warrioress nods emphatically.  \"<i>Ja, it vould help sooo much vith how busy I am now!  Is zis okay, [name]?  For me to visit once a day und relieve the pressure on mein milkers?</i>\"\n\n");
        }
        else
        {
            outputText("The warrioress nods emphatically.  \"<i>Yes, it would help sooo much with how busy I am now!  Is this okay, [name]?  For me to visit once a day and relieve the pressure on my milkers?</i>\"\n\n");
        }
        
        outputText("You ");
        if (player.cor > 50)
        {
            outputText("briefly consider forbidding her outright in order to see that exquisite look of pained pleasure on her face again, but you don't think she'd comply.");
        }
        else
        {
            outputText("don't really see a problem with it as long as she has some left to slake your thirst, but that's the question, isn't it?");
        }
    ...
    private function AllowIzzyMilkerUse() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("(You have a hunch that you might be able to catch her using the milkers at the farm if you 'explore' there, provided she hasn't been recently milked.)\n\n");
        }
    ...
    private function noMilkingMilky() : Void
    {
        ...
        if (player.cor > 50)
        {
            outputText("roughly grab");
        }
        else
        {
            outputText("gently caress");
        }
    ...
    private function getMilk() : Void
    {
        ...
        if (flags[kFLAGS.ISABELLA_MILK_COOLDOWN] > 0)
        {
            ...
            {
                outputText("'s out of bottled milk right now, and that you should check back later.");
            }
            else
            {
                outputText("'s still sore from last time, and that you should wait till later.");
            }
            doNext(callForFollowerIsabella);
            return;
        }
        ...
        if (flags[kFLAGS.ISABELLA_MILKED_YET] == -1)
        {
            if (isabellaAccent())
            {
                outputText("Isabella nods, and fetches you one of the spares from her latest milking session at the farm.  \"<i>Here you go!  Less zan twenty-four hours old, ja?</i>\"");
            }
            else
            {
                outputText("Isabella nods, and fetches you one of the spares from her latest milking session at the farm.  \"<i>Here you go!  Less than twenty-four hours old, okay?</i>\"\n\n");
            }
        }
        else
        {
            ...
            if (isabellaAccent())
            {
                outputText("Ja, zat feels great... hurry and let out mein milk...</i>\"");
            }
            else
            {
                outputText("Ooh, that feels great... hurry and let out my milk...</i>\"");
            }
    ...
    private function isabellaSparMenu() : Void
    {
        ...
        if (flags[kFLAGS.ISABELLA_SPARRING_INTRO] == 0)
        {
            ...
            if (isabellaAccent())
            {
                outputText("Zis vill be good for both of us, ya!?</i>\"  The beaming cow-girl taps a finger to her chin and suggests, \"<i>Ve should set up out on ze edges of the plains, so ve don't damage zis camp.</i>\"");
            }
            else
            {
                outputText("This ought to be good for both of us!</i>\"  The beaming cow-girl taps a finger to her chin and suggests, \"<i>Why don't we set up a faux camp on the edge of the plains, so we don't damage this one.  Okay?</i>\"");
            }
        ...
        {
            outputText("You suggest a sparring session.  ");
        }
        if (isabellaAccent())
        {
            outputText("Isabella asks, \"<i>Vhat stakes should ve use?  Vould you rather we spar light or fight like ze creatures of zis world?</i>\"");
        }
        else
        {
            outputText("Isabella asks, \"<i>How should we fight?  Would you rather we spar light or fight like the creatures of this world?</i>\"");
        }
    ...
    private function sparring(type : Int = 1) : Void
    {
        ...
        if (type == 1)
        {
            outputText("  The knowledge that you're playing for keeps makes your heart beat faster.");
        }
    ...
    private function isabellaBurps() : Void
    {
        ...
        if (flags[kFLAGS.ISABELLA_PROBOVA_BURP_COUNT] == 0)
        {
            if (isabellaAccent())
            {
                outputText("\"<i>Vhat is zat, dear?</i>\"");
            }
            else
            {
                outputText("\"<i>What do you have there?</i>\"");
            }
            ...
            if (isabellaAccent())
            {
                outputText("\"<i>Vell... nuzzing irreversible can happen, ja?</i>\"");
            }
            else
            {
                outputText("\"<i>Well... there's nothing we can't change back, right?</i>\"");
            }
            outputText("\n\n");
            ...
            if (isabellaAccent())
            {
                outputText("Bottoms up, in zat case!");
            }
            else
            {
                outputText("Bottoms up, in that case!");
            }
    ...
    private function runAwayFromIzzyBurps() : Void
    {
        ...
        if (flags[kFLAGS.ISABELLA_PROBOVA_BURP_COUNT] == 0)
        {
            ...
            if (isabellaAccent())
            {
                outputText("Vat did you give me?  Zat was zimply embarrassing!");
            }
            else
            {
                outputText("What did you give me? That was simply embarrassing!");
            }
    ...
    private function getIzzyBurped() : Void
    {
        ...
        if (flags[kFLAGS.ISABELLA_PROBOVA_BURP_COUNT] == 1)
        {
            ...
            //[if no hooves present already]{
                ...
                if (player.isTaur())
                {
                    outputText("  Confused, you quickly inspect yourself before your gaze settles on your hooves – or should you say, your SINGLE PAIR of hooves.  Triggered by what you surmise are the Pro Bova's fumes, your centaur body has vanished, leaving you with a single pair of cow-like hooves.");
                }
                else
                {
                    outputText("  Confused, you quickly inspect yourself before your gaze settles on your [feet] – or should you say, former [feet].  Triggered by what you surmise are the Pro Bova's fumes, your [feet] have quickly and painlessly refashioned themselves into cloven hooves – not unlike those Isabella is currently sporting.");
                }
                changed = true;
            }
            ...
            if (player.horns.count <= 0 || player.horns.type != Horns.COW_MINOTAUR)
            {
                ...
                {
                    outputText("  Your existing horns quiver, then reform into those of a " + player.mf("bull", "cow"));
                }
                else
                {
                    outputText("  Two nubs sprout from your forehead, swiftly thickening and growing into a large pair of curved bovine horns");
                }
            ...
            if (player.tailType != Tail.COW)
            {
                ...
                {
                    outputText("  The tip of your tail reshapes into a burst of fur, the entire length changing to match.");
                }
                else
                {
                    outputText("  A tail creeps out of your [armor], perking up and growing rapidly.  A flare of fur bursts from the tip, the rest of the length changing to match.");
                }
                changed = true;
            }
            ...
            if (player.hasCock() && player.longestCockLength() < 18)
            {
                ...
                if (player.balls < 0)
                {
                    outputText("  Two testicles fall into a scrotum near the base of your enlarged meat, already swelling into baseball-sized balls o' fun.");
                }
                else if (player.ballSize < 4)
                {
                    outputText("  Your testicles engorge in the transformative powers of the cow-girl's violent reaction to the Pro Bova, growing to baseball-sized in short order.");
                }
                changed = true;
            }
            ...
            if (changed && player.hasVagina() && player.isRace(Races.COW, 1, false))
            {
                ...
                if (isabellaAccent())
                {
                    outputText("mein money now, ja?");
                }
                else
                {
                    outputText("my money now, darling!");
                }
                ...
                if (isabellaAccent())
                {
                    if (player.tallness < 87)
                    {
                        outputText("Now, if only ve could make you a bit taller, ve vould be twins!");
                    }
                    else if (player.tallness < 95)
                    {
                        outputText("Und look at zat, you're as grown up as me!");
                    }
                    else
                    {
                        outputText("Zough, of course, you do have me beat in ze height department!");
                    }
                }
                else if (player.tallness < 87)
                {
                    outputText("Now, if only we could make you a bit taller, we would be twins!");
                }
                else if (player.tallness < 95)
                {
                    outputText("And look at that, you're as grown up as me!");
                }
                else
                {
                    outputText("Though, of course, you do have me beat in the height department!");
                }
                outputText("</i>\"\n\n");
            }
            ...
            else if (changed && player.hasCock() && player.isRace(Races.COW, 1, false))
            {
                ...
                if (isabellaAccent())
                {
                    outputText("Nein nein nein nein, zis von't do at all!");
                }
                else
                {
                    outputText("No, no! This is awful!");
                }
                ...
                if (player.longestCockLength() < 18)
                {
                    if (isabellaAccent())
                    {
                        outputText("  \"<i>Zis did not have to be any bigger!  Vhat a tragedy! Your poor little wiener!</i>\"");
                    }
                    else
                    {
                        outputText("  \"<i>This did not have to be any bigger!  What a tragedy! Your poor little wiener!</i>\"");
                    }
                    outputText("  You raise an eyebrow at her selective worry, indicating just how cow-like you've become.  ");
                    if (isabellaAccent())
                    {
                        outputText("\"<i>...Yah, zat is important too, but mein poor pussy...</i>\"");
                    }
                    else
                    {
                        outputText("\"<i>I suppose that's important too, but what about my poor girl-parts?</i>\"");
                    }
                }
                outputText("\n\n");
            }
            ...
            else
            {
                ...
                if (isabellaAccent())
                {
                    outputText("mein... gas... did not harm you any, ja?  Zat is good, right?</i>\"  You manage a weak smile and nod, more than a little relieved everything turned out well in the end.\n\n");
                }
                else
                {
                    outputText("my... gas... didn't harm you any, did it?  That's good, right?</i>\"  You manage a weak smile and nod, more than a little relieved everything turned out well in the end.\n\n");
                }
            }
            ...
            if (changed)
            {
                player.createStatusEffect(StatusEffects.BurpChanged, 0, 0, 0, 0);
            }
        }
        ...
        else
        {
            ...
            if (player.lowerBody != LowerBody.HOOFED || player.horns.count <= 0 || player.horns.type != Horns.COW_MINOTAUR || player.ears.type != Ears.COW || player.tailType != Tail.COW || (player.hasCock() && player.longestCockLength() < 18))
            {
                ...
                if ((!player.hasCock() || rand(2) == 0) && player.hasVagina())
                {
                    outputText("Isabella beaming with eagerness at your sloshing and swelling bosom.\n\n");
                }
                else if (player.hasCock())
                {
                    outputText("Isabella not able to keep a little frustration from her face as your dick grows to enviable size.\n\n");
                }
            }
            //if all requirements already filled (already a busty cow-girl/well-endowed cowboy)
            else
            {
                outputText("Predictably, her sweet-smelling fumes do nothing but further exacerbate your sex-drive, and from the look in her eyes, she's looking forward to it.\n\n");
            }
    ...
    @:allow(classes.scenes.nPCs)
    private function receiveAllTheCowTOngues() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_isabella);
        ...
        {
            outputText("you are nearly absorbed by the great big mounds of Isabella's titflesh, sliding between her massive, milky mammaries.");
        }
        else
        {
            outputText("Isabella nuzzles at your neck, your massive frame nearly crushing her into the chair.");
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>I hope you are not zinking about cumming yet, mein friend,</i>\" she chides, kneading your titflesh a little more gently. \"<i>Ve have not even gotten zu ze pussy-licking yet, after all...</i>\"");
        }
        else
        {
            outputText("\"<i>I hope you're not thinking about cumming yet, [name],</i>\" she chides, kneading your titflesh a little more gently. \"<i>We haven't even gotten to the good part yet...</i>\"");
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Time fur ze main course,</i>\"");
        }
        else
        {
            outputText("\"<i>Time for the main course,</i>\"");
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Ve vill have to teach you to be mindful vhere you leak!</i>\"");
        }
        else
        {
            outputText("\"<i>Looks like I'll have to teach you to watch where you leak!</i>\"");
        }
        ...
        if (player.clitLength < 1)
        {
            outputText(", your clit grinding into her nose,");
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Vell, how vas zhat, [name]?</i>\"");
        }
        else
        {
            outputText("\"<i>Well, what did you think?</i>\"");
        }
        ...
        if (isabellaAccent())
        {
            outputText("  A bit confused, she nonetheless leaves and returns with a drink, setting it beside you before leaving you to your rest.");
        }
    ...
    private function fuckIsabella() : Void
    {
        ...
        {
            x = player.cockThatFits(164);
        }
        var y : Int = as3hx.Compat.parseInt(x + 1);
        ...
        if (player.cocks[x].cockLength < 9)
        {
            outputText("pulls your small [cockFit " + y + "] out of your armor, preparing to suck you off as normal.  You grab her shoulders to stop her, more interested in... giving back, this time.");
        }
        else
        {
            ...
            if (isabellaAccent())
            {
                outputText("\"<i>I don't know about zhat... I'm more partial to ze cute little cocks... Zey fit me so vell...</i>\"");
            }
            else
            {
                outputText("\"<i>I don't know about that... You know I like 'em cute and tiny. They just fit me so well...</i>\"");
            }
        }
        ...
        if (player.cocks[x].cockLength < 9)
        {
            ...
            if (player.tallness < 72)
            {
                outputText(" to tower over your diminutive frame");
            }
            outputText(".  ");
            if (isabellaAccent())
            {
                outputText("\"<i>Your tiny little cock ist zo cute, [name], I just couldn't zay no...</i>\"");
            }
            else
            {
                outputText("\"<i>You've got just the cutest little cock, [name]... how could I say no?</i>\"");
            }
        }
        ...
        else
        {
            ...
            if (isabellaAccent())
            {
                outputText("\"<i>I zuppose I could do vith some relief of mein own...  Very vell, [name].  Ve vill have zex, you und I.</i>\"");
            }
            else
            {
                outputText("\"<i>I guess I could do with some relief of my own...  Okay, [name], let's do it.</i>\"");
            }
        }
        ...
        {
            outputText("utterly engulfing your length");
        }
        else
        {
            outputText("surrounding a fair amount of your massive shaft");
        }
        ...
        {
            outputText("Ordinarily tight, Isabella is practically soaked with lube, making the effort to cram your cock into her an easy task; soon your hips are pressed against her raised ass, her thick thighs wrapped around your body.  ");
        }
        ...
        if (player.cumQ() <= 250)
        {
            outputText("until you're utterly spent");
        }
        else if (player.cumQ() <= 1000)
        {
            outputText("until cum spurts out of her passage and stains her thighs");
        }
        else
        {
            outputText("until her belly begins to bloat with the sheer amount of cream you're stuffing into her");
        }
        outputText(".\n\n");
    ...
    private function fuckIsabellaInTheBarn() : Void
    {
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Ach, [name]?  Vhat do you think you are doing?  Sneaking up on people like zhat...</i>\"");
        }
        else
        {
            outputText("\"<i>Oh, [name]?  What do you think you're doing, huh?  Sneaking up on people like that...</i>\"");
        }
        ...
        if (isabellaAccent())
        {
            outputText("  \"<i>I vhant it...  please, [name], do not tease me so...</i>\"");
        }
        else
        {
            outputText("  \"<i>Come on, [name], give it to me already...  enough with the teasing...</i>\"");
        }
    ...
    private function isabellaBarnFuckPartII() : Void
    {
        ...
        if (player.cocks[player.smallestCockIndex()].cockLength < 9)
        {
            ...
            if (isabellaAccent())
            {
                outputText("\"<i>Mmm, zuch a nice fit,</i>\"");
            }
            else
            {
                outputText("\"<i>Mmm, that's a nice fit,</i>\"  Isabella sighs, her vaginal muscles gently caressing your shaft.");
            }
        }
        //{If medium cock}
        else if (player.smallestCockArea() < 40)
        {
            ...
            if (isabellaAccent())
            {
                outputText("\"<i>Ach, so big</i>\"");
            }
            else
            {
                outputText("\"<i>It's so big,</i>\"");
            }
        ...
        else
        {
            ...
            if (isabellaAccent())
            {
                outputText("It iz sehr too big, zo huge?</i>\"");
            }
            else
            {
                outputText("God it's so huge, I... it's tearing me apart!</i>\"");
            }
        }
        ...
        if (player.smallestCockArea() <= 164)
        {
            outputText("in up to the hilt, smearing her wildly contracting walls with thick smears of leaking pre");
        }
        else
        {
            outputText("in until your [cock smallest] batters into her womb, thick globs of pre smearing her seed-catcher");
        }
        ...
        if (isabellaAccent())
        {
            outputText("[name], zhat was...  ah, [name]");
        }
        else
        {
            outputText("Oh, wow, [name], that was...  that was incredible!");
        }
        outputText("</i>\" Isabella gasps between ragged breaths.  \"<i>");
        if (isabellaAccent())
        {
            outputText("You really know how to please a voman, nein?");
        }
        else
        {
            outputText("You really know how to show a girl a good time, don't you?");
        }
        outputText("</i>\"");
    ...
    private function isabellaTalkMenu() : Void
    {
        ...
        if (debug)
        {
            ...
            {
                var i : Int = 0;
                while (i < isabellaScene.isabellaOffspringData.length)
                {
        ...
        if (flags[kFLAGS.ISABELLA_COUNTDOWN_TO_CONTRACEPTIONS] == -1)
        {
            addButton(1, "Contraception", toggleIsabellaContraceptives).hint((flags[kFLAGS.ISABELLA_POTENCY_STATE] == 1) ? "Tell Isabella to start using the contraceptive herbs. This should prevent Isabella from getting pregnancy although it does not cancel any existing pregnancies." : "Tell Isabella to stop using the contraceptive herbs. This will allow you to get her pregnant.");
        }
        if ((isabellaScene.totalIsabellaChildren() > 0 || isabellaScene.pregnancy.isPregnant) && flags[kFLAGS.ISABELLA_COWMOTHER] < 1)
        {
            addButton(2, "Cowify", isabellaCowifyPrompt).hint("Ask Isabella if she can turn into something special.");
        }
    ...
    {
        clearOutput();
        if (flags[kFLAGS.ISABELLA_POTENCY_STATE] == 1)
        ...
        //Not on contraceptives{
            ...
            {
                outputText(" Besides, we have " + num2Text(isabellaScene.totalIsabellaChildren()) + " small " + ((isabellaScene.totalIsabellaChildren() == 1) ? "baby" : "babies") + " to look after already.");
            }
        ...
        else
        {
            ...
            if (isabellaScene.totalIsabellaChildren() > 0)
            {
                outputText("\n\nShe grins at you wickedly. \"<i>" + cast((isabellaScene.totalIsabellaChildren()), Num2Text) + " small " + ((isabellaScene.totalIsabellaChildren() == 1) ? "baby" : "babies") + " was not enough, hmm? You are wanting more?</i>\"");
            }
    ...
    public function isabellaGivesBirth() : Void
    {
        ...
        {
            if (babyGenderChooser < 45)
            {
                babyGender = 1;
            }
            //It's a BOY!
            else if (babyGenderChooser < 90)
            {
                babyGender = 2;
            }
            //It's a GIRL!
        ...
        else if (babyGenderChooser < 10)
        {
            babyGender = 1;
        }
        //It's a BOY! (Human)
        else if (babyGenderChooser < 80)
        {
            babyGender = 2;
        }
        //It's a GIRL! (Cowgirl)
        ...
        else
        {
            ...
            //Perfectly normal human baby, always male.{
            ...
            else
            {
                ...
                if (babyGender == 3)
                {
                    outputText("One thing she does have that her mother lacks, however, is a juvenile yet very masculine appendage swinging between her legs.  The shape of it kind of reminds you of a minotaur’s distinctive masculinity, but you quietly confirm that she also has an infantile vagina there as well; she’s a herm.");
                }
            }
        ...
        if (flags[kFLAGS.ISABELLA_COWMOTHER] >= 1 && babyGender >= 2)
        {
            offspringType += 2;
        }
