...
class Evangeline2 extends Monster
{
    
    public function spellCostChargeWeapon() : Float
    {
        var cost : Float = 30;
        if (flags[kFLAGS.EVANGELINE_SPELLS_CASTED] >= 10)
        {
            cost -= 3;
        }
        if (flags[kFLAGS.EVANGELINE_SPELLS_CASTED] >= 30)
        {
            cost -= 3;
        }
        if (flags[kFLAGS.EVANGELINE_SPELLS_CASTED] >= 70)
        {
            cost -= 3;
        }
        if (flags[kFLAGS.EVANGELINE_SPELLS_CASTED] >= 150)
        {
            cost -= 3;
        }
        if (flags[kFLAGS.EVANGELINE_SPELLS_CASTED] >= 310)
        {
            cost -= 3;
        }
        return cost;
    }
    public function spellCostChargeArmor() : Float
    {
        var cost : Float = 40;
        if (flags[kFLAGS.EVANGELINE_SPELLS_CASTED] >= 10)
        {
    ...
    public function spellCostBlind() : Float
    {
        var cost : Float = 10;
        if (flags[kFLAGS.EVANGELINE_SPELLS_CASTED] >= 10)
        {
    ...
    public function spellCostWhitefire() : Float
    {
        var cost : Float = 40;
        if (flags[kFLAGS.EVANGELINE_SPELLS_CASTED] >= 10)
        {
    ...
    public function spellCostArouse() : Float
    {
        var cost : Float = 20;
        if (flags[kFLAGS.EVANGELINE_SPELLS_CASTED] >= 10)
        {
    ...
    public function spellCostMight() : Float
    {
        var cost : Float = 50;
        if (flags[kFLAGS.EVANGELINE_SPELLS_CASTED] >= 10)
        {
    ...
    public function spellCostBlink() : Float
    {
        var cost : Float = 40;
        if (flags[kFLAGS.EVANGELINE_SPELLS_CASTED] >= 10)
        {
    ...
    public function spellCostChargeWeaponMight() : Float
    {
        var cost : Float = 80;
        if (flags[kFLAGS.EVANGELINE_SPELLS_CASTED] >= 10)
        {
    ...
    {
        var damage : Int = as3hx.Compat.parseInt(inte / 3 + rand(inte / 2));
        if (inte >= 21 && inte < 41)
        {
            damage += as3hx.Compat.parseInt(inte / 2 + rand((inte * 3) / 4));
        }
        if (inte >= 41 && inte < 61)
        {
            damage += as3hx.Compat.parseInt((inte * 2) / 3 + rand(inte));
        }
        if (inte >= 61 && inte < 81)
        {
            damage += as3hx.Compat.parseInt((inte * 5) / 6 + rand(inte * 1.25));
        }
        if (inte >= 81 && inte < 101)
        {
            damage += as3hx.Compat.parseInt(inte + rand(inte * 1.5));
        }
        if (inte >= 101 && inte < 151)
        {
            damage += as3hx.Compat.parseInt((inte * 1.25) + rand(inte * 1.75));
        }
        if (inte >= 151 && inte < 201)
        {
            damage += as3hx.Compat.parseInt((inte * 1.5) + rand(inte * 2));
        }
        if (inte >= 201)
        {
            damage += as3hx.Compat.parseInt((inte * 1.75) + rand(inte * 2.25));
        }
        damage *= SpellMod();
    ...
    public function ArouseSpell() : Void
    {
        ...
        var lustDmg : Float = player.lustVuln * (inte / 5 * SpellMod() + (player.lib / 6) + (player.effectiveSensitivity() / 6));
        if (player.lust < (player.maxLust() * 0.3))
        {
            outputText("You squirms as the magic affects you.  ");
        }
        if (player.lust >= (player.maxLust() * 0.3) && player.lust < (player.maxLust() * 0.6))
        {
            outputText("You staggers, suddenly weak and having trouble focusing on staying upright.  ");
        }
        if (player.lust >= (player.maxLust() * 0.6))
        {
            outputText("Your eyes glaze over with desire for a moment.  ");
        }
    ...
    {
        var temp : Int = as3hx.Compat.parseInt(inte / 3 + rand(inte / 2));
        if (inte >= 21 && inte < 41)
        {
            temp += as3hx.Compat.parseInt(inte / 2 + rand((inte * 3) / 4));
        }
        if (inte >= 41 && inte < 61)
        {
            temp += as3hx.Compat.parseInt((inte * 2) / 3 + rand(inte));
        }
        if (inte >= 61 && inte < 81)
        {
            temp += as3hx.Compat.parseInt((inte * 5) / 6 + rand(inte * 1.25));
        }
        if (inte >= 81 && inte < 101)
        {
            temp += as3hx.Compat.parseInt(inte + rand(inte * 1.5));
        }
        if (inte >= 101)
        {
            temp += as3hx.Compat.parseInt((inte * 1.25) + rand(inte * 1.75));
        }
        temp *= SpellMod();
    ...
    public function MightSpell() : Void
    {
        ...
        if (hasPerk(PerkLib.JobEnchanter))
        {
            this.statStore.addBuffObject({
                        str : +(5 + (inte / 10)) * 1.2 * SpellMod(),
                        tou : -(5 + (inte / 10)) * 1.2 * SpellMod()
                    }, "EvangelineMight", { });
        }
        else
        {
            this.statStore.addBuffObject({
                        str : +(5 + (inte / 10)) * SpellMod(),
                        tou : -(5 + (inte / 10)) * SpellMod()
                    }, "EvangelineMight", { });
        }
    ...
    override private function performCombatAction() : Void
    {
        if (flags[kFLAGS.EVANGELINE_LVL_UP] == 10 || flags[kFLAGS.EVANGELINE_LVL_UP] == 11)
        {
            ...
            else
            {
                ...
                if (choice3 == 1)
                {
                    if ((this.lust < maxLust() * 0.75) && !hasStatusEffect(StatusEffects.ChargeArmor))
                    {
                        ChargeArmorSpell();
                    }
                    else if (!hasStatusEffect(StatusEffects.Blink) && (fatigue < (maxFatigue() - spellCostBlink())))
                    {
                        BlinkSpell();
                    }
                    else if (HPRatio() < .75 && (fatigue < (maxFatigue() - spellCostHeal())))
                    {
                        HealSpell();
                    }
                    else if ((this.lust < maxLust() * 0.75) && rand(2) == 0 && (fatigue < (maxFatigue() - spellCostBlind())) && !player.hasStatusEffect(StatusEffects.Blind))
                    {
                        BlindSpell();
                    }
                    else if (rand(2) == 0 && (fatigue < (maxFatigue() - spellCostArouse())))
                    {
                        ArouseSpell();
                    }
                    else if ((this.lust < maxLust() * 0.75) && (fatigue < (maxFatigue() - spellCostWhitefire())))
                    {
                        WhiteFireSpell();
                    }
                    else
        ...
        //var choice2:Number = rand(3);{
            ...
            if (choice2 == 1)
            {
                if ((this.lust < maxLust() * 0.75) && !hasStatusEffect(StatusEffects.ChargeWeapon))
                {
                    ChargeWeaponSpell();
                }
                else if (!statStore.hasBuff("EvangelineMight") && (fatigue < (maxFatigue() - spellCostMight())))
                {
                    MightSpell();
                }
                else if ((this.lust < maxLust() * 0.75) && !hasStatusEffect(StatusEffects.ChargeArmor))
                {
                    ChargeArmorSpell();
                }
                else if (!hasStatusEffect(StatusEffects.Blink) && (fatigue < (maxFatigue() - spellCostBlink())))
        ...
        //var choice1:Number = rand(3);{
            ...
            if (choice1 == 1)
            {
                ...
                else if ((this.lust < maxLust() * 0.75) && !hasStatusEffect(StatusEffects.ChargeWeapon))
                {
                    ChargeWeaponSpell();
                }
                else if ((this.lust < maxLust() * 0.75) && !hasStatusEffect(StatusEffects.ChargeArmor))
    ...
    public function new()
    {
        ...
        //na etapie tym zrobić event z atakiem na obóz i ktoś z atakujących poznaje Evangeline jako demonice - po tym jest scena z Eve ujawniającą swoje demoniczne korzenie a także jak sie jej udawało ukrywać swoją korupcje przed mieszkańcami obozu i PC ^^{
