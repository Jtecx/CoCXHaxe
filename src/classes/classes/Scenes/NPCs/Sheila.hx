...
class Sheila extends Monster
{
    ...
    private function sheilaFlyingKick() : Void
    {
        ...
        else
        {
            ...
            damage = as3hx.Compat.parseInt((str + 100 + weaponAttack) - rand(player.tou) - player.armorDef);
            if (damage < 1)
            {
                damage = 2;
            }
    ...
    private function suspiciousGlint() : Void
    {
        ...
        else
        {
            ...
            if (!player.hasCock())
            {
                outputText("burying her drooling tail inside you until it squirts");
            }
            else
            {
                outputText("riding your dick to the hilt");
            }
            ...
            {
                var counter : Int = as3hx.Compat.parseInt(40 + rand(5));
    ...
    private function tittyMonsterAttack() : Void
    {
        ...
        {
            outputText("  But with nothing there for her to work with, it's a lot like being teased by a dressmaker's mannequin.");
        }
    ...
    private function lickEmAndStickEm() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("[oneCock] fucks [sheilaname] to the base while her tail snakes around and penetrates your [vagOrAss]");
        }
        else
        {
            outputText("you take [sheilaname] from behind by plunging her spaded tail into your [vagina] as she lies face-down on the ground with her ass pushed in the air");
        }
        outputText(".");
        //[(int check passed)
    ...
    //Count up till give up!
    {
        ...
        if (!hasStatusEffect(StatusEffects.Counter))
        {
            createStatusEffect(StatusEffects.Counter, 0, 0, 0, 0);
        }
    ...
    {
        super();
        var sheilaDemon : Bool = game.flags[kFLAGS.SHEILA_DEMON] == 1;
        this.a = "";
        ...
        var lust : Float = 30;
        var lustVuln : Float = 0.4;
        var bonusHP : Float = 500;
        if (sheilaDemon)
        ...
        {
            this.drop = new ChainedDrop(consumables.KANGAFT).add(consumables.SUCMILK, 1 / 3).add(consumables.INCUBID, 1 / 2);
        }
