...
class GraydaRetainer extends NPCAwareContent
{
    ...
    public function graydaAffection(changes : Float = 0) : Float
    {
        flags[kFLAGS.GRAYDA_AFFECTION] += changes;
        if (flags[kFLAGS.GRAYDA_AFFECTION] > 100)
        {
            flags[kFLAGS.GRAYDA_AFFECTION] = 100;
        }
    ...
    public function graydaEncounter() : Void
    {
        ...
        clearOutput();
        if (player.eyes.type == Eyes.ARIGEAN && player.tailType == Tail.ARIGEAN_GREEN)
        {
            SceneLib.theTrench.graydaEncounterTheTrench();
        }
        else
        {
    ...
    public function graydaEncounterWin() : Void
    {
        ...
        if (player.gender == 3)
        {
            ...
            {
                graydaEncounterWinFemale();
            }
            else
            {
                graydaEncounterWinMale();
            }
        }
        else
        {
    ...
    public function graydaMainWhenCalled() : Void
    {
        ...
        {
            addButton(7, "Sleep With", graydaSleepToggle).hint("Spend your nights with Grayda.");
        }
        else
        {
            addButton(7, "Sleep Alone", graydaSleepToggle).hint("Stop sleeping with Grayda.");
        }
    ...
    public function mishapsLunaGrayda() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.CampLunaMishaps3, 2, 1);
        }
        else
        {
            player.createStatusEffect(StatusEffects.CampLunaMishaps3, 0, 1, 0, 0);
        }
    ...
    private function graydaMainTalkHer() : Void
    {
        ...
        {
            outputText("After all, you were nothing but a sacrifice to the elders for their own personal gain, and worst of all is that they are continuing on unpunished. How long until someone you held dear is thrown through and defiled by the land?");
        }
        else
        {
            outputText("This reaction seems to somewhat concern you, nothing bad exactly happened. In fact, you are your village’s champion, so why does this upset you?");
        }
    ...
    private function graydaMainTalkOthersAtCamp() : Void
    {
        ...
        {
            outputText("\"<i>That maid, Luna, seems to be a good maid but she has a few issues, such as her lust towards you, keep a close eye on her.</i>\"\n\n");
        }
        if (flags[kFLAGS.LUNA_FOLLOWER] >= 7 && !player.hasStatusEffect(StatusEffects.LunaOff))
        {
            outputText("\"<i>That maid, Luna, seems to be a good maid but she has a few issues, such as her lust towards you, and her...condition...thank you for not firing her by the way my princess, I feel as if you did...nothing good would have come from it.</i>\"\n\n");
        }
        if (player.hasStatusEffect(StatusEffects.CampRathazul))
        {
            outputText("\"<i>The old one seems quite friendly, but I wish he would stop trying to collect ‘samples’ from me. It's getting quite irritating. Please let me know if he starts bugging you, my Princess.</i>\"\n\n");
        }
        if (EvangelineFollower.EvangelineFollowerStage >= 1)
        {
            outputText("\"<i>That rather tall human with the cross-shaped pupils seems almost afraid of us, did you do something, my princess? Or is she just always weary?</i>\"\n\n");
        }
        if (flags[kFLAGS.KONSTANTIN_FOLLOWER] >= 2 && !player.hasStatusEffect(StatusEffects.KonstantinOff))
        {
            outputText("\"<i>The smith seems surprisingly polite for how large he is, hopefully, he doesn’t get the wrong idea if I were to ask to sample his wares, or put off by our… strange diets</i>\"\n\n");
        }
        if (player.hasStatusEffect(StatusEffects.PureCampJojo) && flags[kFLAGS.JOJO_BIMBO_STATE] != 3)
        {
            outputText("\"<i>I'm sorry my Princess, but I can’t help but see that little monk as prey. Surely I'm not the only one thinking he would taste well with a bit of seasoning. However, other than that I've noticed he seems quite cautious around me, I hope I haven’t scared him too badly.</i>\"\n\n");
        }
        if (DivaScene.instance.isCompanion())
        {
            outputText("\"<i>Is… that a vampire? I thought they were nothing but a myth, although it is nice to meet another who is irritated by the harsh sun here.</i>\"\n\n");
        }
        if (flags[kFLAGS.ALVINA_FOLLOWER] > 12 && flags[kFLAGS.ALVINA_FOLLOWER] < 20 && !SceneLib.alvinaFollower.AlvinaPurified)
        {
            outputText("\"<i>My Princess there seems to be a witch near our camp who reeks of corruption, and when I tried confronting her she shooed me away saying you had allowed her to be here. I would be cautious of her Princess, she seems to be the kind to have her own interests in mind.</i>\"\n\n");
        }
        if (flags[kFLAGS.SIEGWEIRD_FOLLOWER] > 3)
        {
            outputText("\"<i>That paladin doesn’t seem to be the brightest, but he certainly is dedicated towards his cause. If he wasn’t so hostile towards me then I could imagine we would get along quite well. That soup also smells lovely.</i>\"\n\n");
        }
    ...
    private function graydaMainTalkTheCamp() : Void
    {
        ...
        {
            outputText("\"<i>It’s nice, there’s at least some shelter so our skins don’t burn. But shouldn't we try to continue to expand our defense a bit given we are currently out in the open?</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>It’s… something, but don’t you think you should build some shelter, my princess? I'm not sure how you handle constantly being in this harsh sunlight.</i>\"\n\n");
        }
        graydaAffection(2.5);
        doNext(graydaMainTalk);
        advanceMinutes(5);
