...
class LunaFollower extends NPCAwareContent implements SaveableState implements TimeAwareInterface
{
    public var mutations : MutationsHelper = new MutationsHelper();
    public static var Nursed : Bool;
    public static var NursedCooldown : Int;
    public static var Sated : Bool;
    public static var SatedCooldown : Int;
    public static var WerewolfPackMember : Int;
    public static var HellhoundPackMember : Int;
    public static var LunaSonsNum : Float;
    public static var LunaDaughtersNum : Float;
    public static var LunaHermKidsNum : Float;
    public static var LunaTotalKidsNum : Float;
    
    public static var mooning : Bool = false;  //no need to save it  
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            Nursed = Reflect.field(o, "LunaNursed");
            Sated = Reflect.field(o, "LunaSated");
            NursedCooldown = valueOr(Reflect.field(o, "LunaNursedCooldown"), 0);
            SatedCooldown = valueOr(Reflect.field(o, "LunaSatedCooldown"), 0);
            WerewolfPackMember = valueOr(Reflect.field(o, "LunaWerewolfPackMember"), 0);
            HellhoundPackMember = valueOr(Reflect.field(o, "HellhoundPackMember"), 0);
            LunaDaughtersNum = Reflect.field(o, "LunaDaughtersNum");
            LunaSonsNum = Reflect.field(o, "LunaSonsNum");
            LunaHermKidsNum = Reflect.field(o, "LunaHermKidsNum");
            LunaTotalKidsNum = Reflect.field(o, "LunaTotalKidsNum");
        }
        else
        {
            resetState();
    ...
    public function lunaAffection(changes : Float = 0) : Float
    {
        flags[kFLAGS.LUNA_AFFECTION] += changes;
        if (flags[kFLAGS.LUNA_AFFECTION] > 100)
        {
            flags[kFLAGS.LUNA_AFFECTION] = 100;
        }
        return flags[kFLAGS.LUNA_AFFECTION];
    }
    public function lunaJealousy(changes : Float = 0) : Float
    {
        flags[kFLAGS.LUNA_JEALOUSY] += changes;
        if (flags[kFLAGS.LUNA_JEALOUSY] > 400)
        {
            flags[kFLAGS.LUNA_JEALOUSY] = 400;
        }
        if (flags[kFLAGS.LUNA_JEALOUSY] < 0)
        {
    ...
    //Get out if already pregged.
    {
        ...
        {
            outputText("Luna isn't pregnant");
        }
        if (preg)
        {
    ...
    public function mainLunaMenu() : Void
    {
        ...
        else
        {
            outputText("You call out to Luna, who immediately stops what she's doing and trots over to you as quickly as her professional dignity permits. She straightens her skirt and addresses you with a polite bow.\n\n" +
                    "\"<i>Good </i>");
            if (model.time.hours >= 6 && model.time.hours <= 11)
            {
                outputText("<i>morning</i>");
            }
            else if (model.time.hours <= 18)
            {
                outputText("<i>afternoon</i>");
            }
            else
            {
                outputText("<i>evening</i>");
            }
            ...
            addButton(2, "Spar", sparLuna).disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, "You need a good sparring ring for that.").disableIf(flags[kFLAGS.LUNA_FOLLOWER] <= 10, "How can you attack your cute maid, you monster?!");
            if (flags[kFLAGS.LUNA_FOLLOWER] > 4)
            {
                addButton(3, "Meal", mealLuna);
                addButton(4, "Nurse", nurseLuna).disableIf(player.HP >= player.maxOverHP() && !player.statStore.hasBuff("Weakened") && !player.statStore.hasBuff("Drained"), 
            ...
            if (flags[kFLAGS.LUNA_AFFECTION] >= 50)
            {
                ...
                {
                    addButton(5, "Sleep With", lunaSleepToggle);
                }
                else
                {
                    addButton(5, "Sleep Alone", lunaSleepToggle);
                }
            }
            else
            {
                addButtonDisabled(5, "Sleep With", "Requires higher affection.");
            }
            addButton(6, "Sex", sexMenuMain).disableIf(player.lust < 33, "Not aroused enough.").disableIf(flags[kFLAGS.LUNA_FOLLOWER] <= 10, "Maybe ask her to sleep with you?", "???");
            if (flags[kFLAGS.LUNA_FOLLOWER] == 9 || flags[kFLAGS.LUNA_FOLLOWER] == 10)
            {
                addButton(7, "Unchain", lunaChainToggle).hint("Unchain Luna, if you dare.");
            }
            if (flags[kFLAGS.LUNA_FOLLOWER] == 7 || flags[kFLAGS.LUNA_FOLLOWER] == 8)
            {
                addButton(7, "Chain", lunaChainToggle).hint("Chain Luna before it's too late.");
            }
    ...
    public function LunaAppearance() : Void
    {
        ...
        if (flags[kFLAGS.LUNA_FOLLOWER] < 7)
        {
            ...
            {
                outputText(", though you could swear you see her making a different expression sometimes when you visit your other companion");
            }
    ...
    public function talkMenuLuna() : Void
    {
        ...
        if (flags[kFLAGS.LUNA_FOLLOWER] > 6)
        {
            ...
            {
                addButtonDisabled(4, "Lycanthropy", "Your body can't be transformed.");
            }
            else
            {
                addButton(4, "Lycanthropy", talkMenuLunaLycanthropy).hint("Yeah, humans <b>are</b> pretty rare, huh? Find out more about Luna's condition.");
            }
        }
        else
        {
            addButtonDisabled(4, "???", "You need to know her better for this.");
        }
        if (player.statusEffectv1(StatusEffects.LunaWasCaugh) >= 3)
        {
            addButton(7, "Accuse", talkMenuLunaStopJealousy).hint("You know it was her, and it needs to stop now.");
        }
        if ((flags[kFLAGS.LUNA_FOLLOWER] == 13 || flags[kFLAGS.LUNA_FOLLOWER] == 14) && !player.hasPerk(PerkLib.Lycanthropy) && !player.hasPerk(PerkLib.LycanthropyDormant))
        {
            addButton(9, "Bite Me", talkMenuBiteMe).hint("This is a terrible idea!");
        }
    ...
    public function talkMenuLunaHer() : Void
    {
        ...
        if (flags[kFLAGS.LUNA_FOLLOWER] > 6)
        {
            outputText("You could swear you see the faintest flash of green in her eyes as she finishes, but perhaps it was your imagination?\n\n");
        }
        if (player.cor >= 60)
        {
            outputText("Of course, it's those tantalizing glimpses of her true, corrupted nature that make her so cute, you think to yourself happily... definitely happily, not nervously at all.\n\n");
        }
        lunaJealousy(-100);
        lunaAffection(2);
        doNext(talkMenuLuna);
    ...
    public function talkMenuLunaWhatCanSheDo() : Void
    {
        ...
        if (flags[kFLAGS.LUNA_FOLLOWER] > 6)
        {
            outputText("She glances down slightly, her face flushing a bit. \"<i>Even your most... intimate needs... i-if it's your wish, I would never turn you down, [Master]...</i>\"\n\n");
        }
        outputText("You can think of absolutely no potential problems with this.\n\n");
        if (flags[kFLAGS.LUNA_FOLLOWER] < 5)
        {
            flags[kFLAGS.LUNA_FOLLOWER] = 5;
        }
        lunaJealousy(-100);
        doNext(talkMenuLuna);
        advanceMinutes(15);
    ...
    public function talkMenuLunaHuman() : Void
    {
        ...
        if (flags[kFLAGS.LUNA_FOLLOWER] > 6)
        {
            ...
            {
                outputText("She narrows her eyes slightly. \"<i>You don't have any <b>pesky</b> spouse to get between us and make you... inhibited.</i>\"\n\n");
            }
    ...
    public function talkMenuLunaCampThoughts() : Void
    {
        ...
        {
            outputText("\"<i>While I can’t say your home is as... grand as those of my prior employers, it is truly charming in its rustic way. Perhaps one day [Master] will consider expanding it? I could greatly expand my services to you with a larger home.</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>While I would never impose upon [Master], I will admit it would be far easier to clean if there were a proper dwelling here. I would greatly enjoy the wider scope for my services a cabin would provide.</i>\"\n\n");
        }
        lunaJealousy(-100);
        lunaAffection(2);
        doNext(talkMenuLuna);
    ...
    public function talkMenuLunaStopJealousy() : Void
    {
        ...
        if (flags[kFLAGS.LUNA_FOLLOWER] > 6)
        {
            outputText("<i>Though my love for you is unconditional,</i> ");
        }
    ...
    public function talkMenuLunaLycanthropy() : Void
    {
        ...
        {
            flags[kFLAGS.LUNA_FOLLOWER] = 13;
        }
        else
        {
            flags[kFLAGS.LUNA_FOLLOWER] = 14;
        }
        lunaJealousy(-100);
        lunaAffection(2);
        doNext(talkMenuLuna);
    ...
    public function talkMenuBiteMeYes() : Void
    {
        ...
        if (player.hasCock() && player.wolfCocks() < 1)
        {
            ...
            for (indexI in 0...player.cocks.length)
            {
        ...
        if (player.hasPerk(PerkLib.LycanthropyDormant))
        {
            player.removePerk(PerkLib.LycanthropyDormant);
        }
        player.statStore.replaceBuffObject({
                    str.mult : bonusStats * 0.1 * ngM,
                    tou.mult : bonusStats * 0.06 * ngM,
                    spe.mult : bonusStats * 0.04 * ngM,
                    minlustx : bonusStats * 0.01
                }, "Lycanthropy", {
                    text : "Lycanthropy"
                });
    ...
    public function sparLunaWon() : Void
    {
        ...
        {
            if (flags[kFLAGS.LUNA_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.LUNA_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.LUNA_DEFEATS_COUNTER] = 1;
            }
            if (flags[kFLAGS.LUNA_LVL_UP] < 15 && flags[kFLAGS.LUNA_DEFEATS_COUNTER] >= flags[kFLAGS.LUNA_LVL_UP] + 2)
            {
                ...
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers3, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * flags[kFLAGS.LUNA_DEFEATS_COUNTER], 0, 0, 0);
                }
    ...
    public function mealLuna() : Void
    {
        ...
        else
        {
            ...
            player.buff("WellFed").setStats({
                                        str.mult : 0.05,
                                        tou.mult : 0.05,
                                        spe.mult : 0.05
                                    }).forDays(1).withText("Well Fed");
    ...
    public function nurseLuna() : Void
    {
        ...
        {
            if (player.statusEffectv1(StatusEffects.CombatWounds) > 0.05)
            {
                player.addStatusValue(StatusEffects.CombatWounds, 1, -0.05);
            }
            else
            {
        ...
        if (flags[kFLAGS.LUNA_FOLLOWER] > 10)
        {
            ...
            {
                addButtonDisabled(0, "Yes", "Your lust is too low for that.");
            }
    ...
    private function lunaChainToggle() : Void
    {
        ...
        if (flags[kFLAGS.LUNA_FOLLOWER] == 9 || flags[kFLAGS.LUNA_FOLLOWER] == 10)
        {
            ...
            if (flags[kFLAGS.LUNA_FOLLOWER] == 9)
            {
                flags[kFLAGS.LUNA_FOLLOWER] = 7;
            }
            if (flags[kFLAGS.LUNA_FOLLOWER] == 10)
            {
                flags[kFLAGS.LUNA_FOLLOWER] = 8;
            }
        }
        else
        {
            ...
            if (flags[kFLAGS.LUNA_FOLLOWER] == 8)
            {
                flags[kFLAGS.LUNA_FOLLOWER] = 10;
            }
        }
        menu();
        addButton(0, "Next", mainLunaMenu);
    ...
    public function warrningAboutJelously() : Void
    {
        spriteSelect(SpriteDb.s_luna_maid);
        clearOutput();
        if (flags[kFLAGS.LUNA_FOLLOWER] >= 5 && flags[kFLAGS.LUNA_FOLLOWER] % 2 == 1)
        {
    ...
    public function fullMoonEvent(PCIsAwake : Bool = false, PCWalkedToLunaAtmaxJealousy : Bool = false) : Void
    {
        spriteSelect(SpriteDb.s_Luna_Mooning);
        clearOutput();
        flags[kFLAGS.LUNA_JEALOUSY] = 0;
        if (flags[kFLAGS.LUNA_FOLLOWER] > 6)
        {
            ...
            else
            {
                ...
                if (PCIsAwake)
                {
                    outputText("\"<i>We can do this two ways, [Master]: the hard way, where you resist and I take you by force until I'm happy, or the easy way, where you fuck me raw in a loving, consensual way. What is your pleasure, [Master]?</i>\"\n\n");
                }
                else
                {
                    outputText(player.mf("Master", "Mistress") + ", there's no need for words. You know what I both want, so let's get wild. I've been waiting so patiently for this, so eagerly, and you've denied me so long.</i>\"\n\n");
                }
            }
        }
        ...
        if (flags[kFLAGS.LUNA_FOLLOWER] > 6)
        {
            addButton(0, "Accept", fullMoonEventAccept2);
        }
        else
        {
            addButton(0, "Accept", fullMoonEventAccept);
        }
        if (player.racialScore(Races.DOG, false) > 5 || player.racialScore(Races.WOLF, false) > 5)
        {
            addButtonDisabled(1, "Resist", "The smell of a female wolf in heat drives your canine instincts into a frenzy. You must have her here and now!");
        }
        else
        {
            addButton(1, "Resist", fullMoonEventResist);
        }
    }
    public function fullMoonEventAccept() : Void
    {
        ...
        if (!player.isNaked())
        {
            outputText(" stand and take off your own clothes, and");
        }
        ...
        if (flags[kFLAGS.LUNA_FOLLOWER] == 5 || flags[kFLAGS.LUNA_FOLLOWER] == 7 || flags[kFLAGS.LUNA_FOLLOWER] == 9)
        {
            flags[kFLAGS.LUNA_FOLLOWER] = 11;
        }
        if (flags[kFLAGS.LUNA_FOLLOWER] == 6 || flags[kFLAGS.LUNA_FOLLOWER] == 8 || flags[kFLAGS.LUNA_FOLLOWER] == 10)
        {
            flags[kFLAGS.LUNA_FOLLOWER] = 12;
        }
        mooning = true;
    ...
    public function fullMoonEventAccept2() : Void
    {
        ...
        if (!player.isNaked())
        {
            outputText(" take off your clothes and");
        }
    ...
    public function sexMenuMain() : Void
    {
        ...
        menu();
        addButton(0, "Headpat", sexMenuHeadpat).hint("You know it's lewd, you know it's wrong, but you just can't help it.").disableIf(player.cor < 15 - player.corruptionTolerance, "That's lewd! How could you even think of such a thing??");
        addButton(1, "Human", sexMenuDomesticService).hint("Get some intimate domestic service from your cute maid Luna.");
        addButton(2, "Werewolf", sexMenuDoggyTreats).hint("Take a walk on the wild side and give your doggy a treat.").disableIf(player.lust < 33, "Bringing the beast out in Luna when you're not up for sex is too terrible of an idea even for you. Please reconsider.").disableIf(player.gender == 0, "You're going to need some special equipment to play with this pup.");
        addButton(3, "Fox&Hound", sexMenuSandwichWithAyane).hint("Some group action could be fun, and you bet Ayane would be up for it too.").disableIf(player.lust < 33, "Not aroused enough.").disableIf(flags[kFLAGS.LUNA_AFFECTION] < 100, "Some group action could be fun if you were sure Luna would do it, but you don't want to push your luck. You know how jealous she gets...").disableIf(flags[kFLAGS.AYANE_FOLLOWER] < 2, "Get a certain other fluffy-tailed follower to join your camp and maybe you can have some group fun.", "???").disableIf(!player.hasCock(), "Req. a cock, first of all.");
        addButton(14, "Back", mainLunaMenu);
    }
    ...
    public function sexMenuDomesticService() : Void
    {
        ...
        menu();
        addButton(1, "Vaginal", sexMenuVaginalIntro).hint("Have Luna soothe and pleasure you with her (currently, at least) normal human pussy; sometimes simple is best, and your cute maid can still be the best even when she isn't the beast.").disableIf(player.isGenderless(), "You need to have genitals if you want Luna to use her own on them!");
        addButton(2, "SprPolish", sexMenuSpearPolishing).hint("She may not have scales or a tail, but you still have a spear and she's still a maid; you're pretty sure you can make this work somehow.").disableIf(!player.hasCock(), "Your maid can't polish a spear you don't have!");
        addButton(2, "SprPolish", sexMenuSpearPolishing).hint("She may not have scales or a tail, but you still have a spear and she's still a maid; you're pretty sure you can make this work somehow.").disableIf(!player.hasCock(), "Your maid can't polish a spear you don't have!")  //.disableIf(player.lust < 33, "Your spear isn't feeling hard enough for a good polishing right now. Come back when you're in a more in ... stabby mood.");  ;
        addButton(3, "Titfuck", sexMenuBoobjob).hint("Luna will give you a nice cock massage with her pert C-cups if you ask politely. She'll still do it if you ask rudely, but you're feeling nice today.").disableIf(!player.hasCock(), "You're sure Luna would love to wrap her nice, hand-sized tits around your cock, if only you had one.")  //.disableIf(player.lust < 33, Luna's breasts look very nice under her dress and apron, but you're not turned on enough for anything but aesthetic appreciation just now.");  ;
    ...
    public function sexMenuVaginalIntro() : Void
    {
        ...
        if (player.cor < 50)
        {
            outputText("you begin to thank her for the care and affection she alwyas shows you,");
        }
        else
        {
            outputText("you are about to tell her to strip for you,");
        }
    ...
    public function sexMenuVaginal(werewolf : Bool = false) : Void
    {
        clearOutput();
    ...
    public function sexMenuSpearPolishing() : Void
    {
        ...
        if (!player.isNaked())
        {
            ...
            {
                outputText(", particularly your [breasts], you note - not that you have any objection to her skilled, teasing caresses");
            }
        ...
        if (player.hasBalls())
        {
            outputText(", then down to your full, aching " + ((player.balls == 2) ? "pair" : "quartet") + ", lavishing your tender orbs with her soft, wet lips and tongue until they are dripping with your mixed fluids");
        }
        if (player.hasBalls() && player.hasVagina())
        {
            outputText("; then, after that, with her hands taking over for her mouth on your twitching manhood, she moves her oral worship even further");
        }
        if (player.balls == 0 && player.hasVagina())
        {
            outputText(", and past that");
        }
        if (player.hasVagina())
        {
            outputText("to tease your engorged, sensitive clitoris and gently pry open your lower lips, teasing and kissing and nibbling until you are squirming in alarmed ecstasy");
        }
        ...
        {
            outputText("Ever too much the devoted servant to leave your [cock biggest2] lonely and unattended, she frees her hands from their gentle root massage and uses them on it, pumping up and down in time with her oral movements on your other one.");
        }
        if (player.cocks[x].cockLength >= 7)
        {
            outputText(" She soon reaches a pace, hilting you and then pulling up, your swollen tip feeling the back of her throat and then the tight seal of her lips, one after the other, over and over until you are moaning loudly in sticky, melty bliss.");
        }
        ...
        if (player.cocks.length > 1)
        {
            outputText(" while your other cock" + ((player.cocks.length == 2) ? "" : "s") + " explodes messily, painting the top of her head and your own torso with spurt after spurt of white, sticky cum");
        }
        outputText(".");
        if (player.cumQ() >= 2000)
        {
            ...
            if (player.cocks.length > 1)
            {
                outputText("your free [cock biggest2] has painted both of you and much of the surrounding area with the fruit of your orgasm, while ");
            }
        ...
        else if (player.cumQ() >= 1000)
        {
            ...
            if (player.cocks.length > 1)
            {
                outputText("your free [cock biggest2] has covered both of you in sticky, hot cum, and ");
            }
    ...
    public function sexMenuBoobjob() : Void
    {
        ...
        if (!player.isNaked())
        {
            outputText(" requests that you remove your clothing, which you do eagerly. As you do so she");
        }
        ...
        if (player.cumQ() <= 500)
        {
            outputText(" ropes of white, sticky cum that cling to her face and hair as she laughs playfully, gently milking you for all you have with her breasts until you are spent and panting.\n\n");
        }
        else if (player.cumQ() <= 1000)
        {
            outputText(" thick spurts of thick cum that paint her entire face white and change her playful laughing into sputters; by the time she has finished gently but firmly milking you dry with her breasts it is dripping from her chin and eyelashes and onto your still heaving chest as you pant and moan in spent pleasure.\n\n");
        }
        else if (player.cumQ() <= 2000)
        {
            outputText(" a near-constant spray of cum shooting fast and hard from your spasming member, painting her face, hair, chest, into a white, sticky mess. She barely has time to laugh playfully before her face is too covered in your seed to breathe properly, and even as she coughs and sputters you keep coming while she dutifully milks you dry with her breasts, now freshly lubricated along with everything else. Finally your enormous orgasm ceases, and your body goes slack with relaxed pleasure.\n\n");
        }
        else
        {
            outputText(" a deluge of cum; almost immediately you and she are drenched in seed, your spasming cock spewing a roiling, foamy geyser almost as if she had shaken a beer barrel and pulled out the tap. Screaming in your crazed pleasure you cum, and cum, and cum, until you are laying in a pool of your issue and Luna is completely drenched from head to toe, coughing and sputtering as she tries to catch her breath. Even so, she dutifully milks you with her breasts until nothing more than a trickle remains and your taut muscles come unstrung, leaving you utterly spent and unable to move.\n\n");
        }
    ...
    public function sexMenuDoggyTreats() : Void
    {
        ...
        if (!player.isNaked())
        {
            outputText(" expose your own lower half and");
        }
        if (player.hasCock())
        {
        ...
        if (player.hasVagina())
        {
            outputText(" fingering your increasingly wet pussy");
        }
        ...
        if (player.hasVagina())
        {
            outputText("your pussy is dripping down your thighs in anticipation, ");
        }
        ...
        menu();
        addButton(0, "Dominate", sexMenuDominateHer).hint("Take your bitch as rough as you want until you've had enough. It'll do her good.");
        addButton(1, "Surprise", sexMenuDoggyTreats2).hint("You're feeling a little saucy, and Luna's asshole looks too tempting to pass up.").disableIf(!player.hasCock(), "As tempting as Luna's ass looks, you're going to need a cock to take advantage of it.");
        addButton(2, "RoughFuck", roughFuck).hint("Grab her and use it however you want.");
    ...
    public function sexMenuDoggyTreats2() : Void
    {
        ...
        if (player.cocks.length > 1)
        {
            outputText(" lower member with her fleshy, round anus while your upper one presses its way between her soft, fluffy cheeks under her tail.");
        }
        else
        {
            outputText(" member with her fleshy, round anus.");
        }
        ...
        if (player.cocks.length > 1)
        {
            outputText("s with both her clenching, shuddering cheeks and rhythmically squeezing colon.\n\n");
        }
        else
        {
            outputText(" with her rhythmic squeezing.\n\n");
        }
        ...
        if (player.cumQ() <= 500)
        {
            ...
            if (player.cocks.length > 1)
            {
                outputText(" Meanwhile, your other cock fires jets of cum onto her tail and back, leaving sticky white trails along her gray fur that glisten as she writhes under you.");
            }
        }
        else if (player.cumQ() <= 1000)
        {
            ...
            if (player.cocks.length > 1)
            {
                outputText(" Meanwhile, your other cock fires messily onto her tail and back, painting it with jagged streaks and splotches of glistening white against her gray fur while she thrashes in the throes of her own ecstasy.");
            }
        }
        else if (player.cumQ() <= 2000)
        {
        ...
        else
        {
            ...
            if (player.cocks.length > 1)
            {
                outputText("Meanwhile, your other cock imitates a perverted fountain, framed and held upright by her sculpted cheeks. Cum geysers up nearly six feet in the air to fall back down in messy splatters all over her, you, and the surrounding several feet, until you are both sitting in a pool of cum.");
            }
        }
        ...
        if (player.cumQ() <= 1000)
        {
            outputText(" a dribble of your seed from her gaping, distended ring.");
        }
        else
        {
            outputText("a spout of cum from her gaping, distended ring as her insides resume their proper shape and size, forcing it out in pulsing spurts." +
        ...
        if (!player.isNaked())
        {
            outputText(", and you see your equipment neatly folded and piled a few feet away");
        }
    ...
    public function sexMenuSandwichWithAyane() : Void
    {
        ...
        if (player.cocks.length == 1)
        {
            outputText("You pump your cock in and out of the duo. Sometime sliding in either hole before coming back in between. ");
        }
        if (player.cocks.length == 2 || player.cocks.length == 3)
        {
            outputText("Why fuck only one girl when you can fuck both! Having more than one penis has its advantages and you gladly insert yourself into their two well aligned pussies, fucking Ayane and Luna at the same time. ");
        }
        if (player.cocks.length >= 4)
        {
            outputText("Why fuck only one girl when you can fuck both! Having more than one penis has its advantages and you gladly insert yourself into their two well aligned pussies, fucking Ayane and Luna at the same time. That said, you got enough cocks to fill their ass too, and gladly insert yourself in their waiting holes. Luna howls with delight as you break past her pucker. ");
        }
        ...
        if (player.hasBalls())
        {
            outputText("balls which only cause them to churn with a massive supply of cum; this is going to be one hell of a mess!");
        }
        else
        {
            outputText("sensitive skin.");
        }
    ...
    public function sexMenuDominateHer() : Void
    {
        ...
        if (player.hasPerk(PerkLib.Lycanthropy))
        {
            if (!player.hasStatusEffect(StatusEffects.HumanForm))
            {
                player.createStatusEffect(StatusEffects.HumanForm, 0, 0, 0, 0);
            }
            else
            {
        ...
        function dickF() : Void
        {
            if (player.cocks.length > 1)
            {
                ...
                if (player.hasKnot())
                {
                    outputText("<i>knot me... please... I need it... I need to feel you filling me up...</i>\"");
                }
                else
                {
                    outputText("<i>cum inside me... please... I need it... I need to feel you filling me up...</i>\"");
                }
                ...
                {
                    outputText("<i> cum inside</i>");
                }
                outputText("<i> your horny, needy bitch, [Master]... </i>");
                if (player.hasKnot())
                {
                    outputText("<i>lock me and </i>");
                }
            ...
            else
            {
                ...
                if (player.hasKnot())
                {
                    outputText("<i>knot me... please... I need it... I need to feel you filling me, locking me, claiming me...</i>\"\n\n");
                }
                else
                {
                    outputText("<i>cum inside me... please... I need it... I need to feel your hot stuff filling me...</i>\"\n\n");
                }
                outputText("You grin maliciously and suddenly even out your pace, causing your orgasm to plateau momentarily. Luna whines as she realizes you're teasing her, but you tell her that she can't tell you what to do; a horny bitch doesn't get");
                if (player.hasKnot())
                {
                ...
                if (player.cumQ() <= 500)
                {
                    outputText(" Your cock begins spurting thick ropes of cum into Luna's spasming pussy. She gives a ragged cry of relief, as if you had doused a fire inside her with your fluids; then, your passion spent, ");
                }
                else if (player.cumQ() <= 1000)
                {
                ...
                {
                    outputText(" Your cock begin spraying enormous amounts of cum into Luna's greedy hole; almost immediately her spasming tunnel and womb have taken all they can take and begin resisting further intrusion. But she has clamped down too tightly on you to allow full relief of the pressure, and as you continue to cum inside her belly swells until she looks almost pregnant. Despite the obvious discomfort of being filled past capacity Luna gives a ragged cry of relief, as if your inhuman ejaculation had put out a fire that was raging inside her; then, your passion spent, ");
                }
                else
                {
                ...
                {
                    outputText(" You pull yourself free of her with a wet 'squelch', and");
                }
                if (player.cumQ() <= 1000)
                {
                    outputText(" no more than a single trickle of cum leaves her pussy afterward; the breeding instincts you seem to have awoken with your rough treatment are still exerting influence over her body as it seeks to hold your seed inside her to ensure a pregnancy.\n\n");
                }
                else
                {
                    outputText(" her pussy spurts out a little bit of your cum every few seconds as it strains to contain everything you pumped into her, the breeding instincts you seem to have awoken not quite able to handle your unnatural volume. Still, until her body relaxes a bit it seems Luna will be looking a little puffy as her instincts attempt to ensure pregnancy in spite of her.\n\n");
                }
            }
            //if (!mooning)
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            {
                outputText(", while her own hips buck upward and massage yours every time you give her a new jolt of pleasure with your lips and tongue");
            }
        ...
        var sharedEnd : Void->Void = function() : Void
        {
            ...
            if (!mooning)
            {
                ...
                if (!player.isNaked())
                {
                    outputText(" Your equipment is neatly folded up nearby.");
                }
    ...
    {
        var woof : Bool = player.isRace(Races.DOG) || player.isRace(Races.WOLF) || player.isRace(Races.WEREWOLF);
        spriteSelect(SpriteDb.s_Luna_Mooning);
        clearOutput();
        ...
        function dickF() : Void
        {
            ...
            if (player.hasKnot())
            {
                outputText(", knotting her just fine");
            }
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            if (player.hasCock())
            {
                outputText(" as your cock explodes, covering her chest up to her belly in semen" + ((woof) ? " while you howl to the moon, savoring your mind-blowing orgasm" : "") + "");
            }
    ...
    public function LunaPregComfort() : Void
    {
        ...
        outputText("\"<i>...Thank you…[Player].</i>\" She looks you in the eyes, sheer adoration in her gaze. \"<i>...I love you.</i>\" You smile, saying that you like it when she uses your name. After everything, she deserves to be treated as an equal. \n\n");
