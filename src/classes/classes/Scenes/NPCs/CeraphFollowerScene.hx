...
class CeraphFollowerScene extends NPCAwareContent
{
    public function new()
    {
        super();
    ...
    override public function ceraphIsFollower() : Bool
    {
        return flags[kFLAGS.CERAPH_FOLLOWER_PIERCING] > 0 || flags[kFLAGS.CERAPH_FOLLOWER_CARRY] > 0;
    }
    
    public function ceraphFollowerEncounter(forceCeraph : Bool = false) : Void
    {
        if (forceCeraph)
        {
    ...
    //[Actually Ceraph] -
    public function ceraphFollowerAppearance(output : Bool = true) : Void
    {
        if (output)
        {
            clearOutput();
        ...
        if (output)
        {
            ...
            {
                outputText("  Her demonic prick forms a ridge in the tight fabric, its head peeking ever-so-slightly over the top");
            }
            else
            {
                outputText("  Thankfully, she seems to have removed or hidden her demonic cock somehow, so as not to disrupt her fragile outfit with unfeminine bulges");
            }
        ...
        {
            addButton(6, "GetFetish", CeraphHandsOutNewFetishesLikePervCandy);
        }
        if (flags[kFLAGS.PC_FETISH] > 0)
        {
            addButton(7, "RemoveFetish", unfetishifyYourselfWithFollowerCeraph);
        }
        addButton(8, (flags[kFLAGS.CERAPH_HIDING_DICK] != null) ? "Go Herm" : "Go Female", cawkTawgle);
        if (flags[kFLAGS.FOLLOWER_AT_FARM_CERAPH] == 0 && flags[kFLAGS.FARM_CORRUPTION_STARTED] == 1)
        {
            addButton(10, "Farm Work", helpWithFarm);
        }
    ...
    private function followerCeraphRoleplay() : Void
    {
        ...
        addButton(2, "Be A Pet", sumissivenessToCeraphFollower).disableIf(player.isGenderless(), "Not for genderless!");
        addButtonDisabled(5, "???");
        addButtonDisabled(6, "???");
        addButtonDisabled(7, "???");
        if (flags[kFLAGS.DOMINIKA_MET] > 0)
        {
            addButton(5, "Dominika", cerminika).disableIf(!player.hasCock(), "Req. a cock!");
        }
        if (player.hasStatusEffect(StatusEffects.Marble))
        {
            addButton(6, "Marble Play", sweetieNOOOO).disableIf(!player.cockThatFits(70) < 0, "Req. a cock fitting 70 area!");
        }
        if (flags[kFLAGS.TIMES_FUCKED_URTA] > 0)
        {
            addButton(7, "Urta Play", ceraphUrtaRoleplay).disableIf(!player.isGenderless(), "Not for genderless!");
        }
        addButton(14, "Back", ceraphFollowerAppearance);
    }
    ...
    public function submissiveCeraphOffer() : Void
    {
        ...
        if (monster.HP < 1)
        {
            outputText("drops on the ground in front of you, completely beaten.");
        }
        else
        {
            outputText("drops down and starts masturbating, practically fisting her drooling pussy while she pumps her demonic dick with reckless abandon.");
        }
        outputText("  She ");
        if (monster.HP < 1)
        {
            outputText("looks up at you and asks");
        }
        else
        {
            outputText("manages to stop fapping long enough to look up at you and ask");
        }
        ...
        if (player.cor < 33)
        {
            outputText("a nervous chuckle");
        }
        else if (player.cor < 66)
        {
            outputText("a surprised laugh");
        }
        else
        {
            outputText("a bemused smirk");
        }
    ...
    private function collarCeraph() : Void
    {
        ...
        if (player.cor < 33)
        {
            outputText("that you never expected making a demon into your slave would factor into your quest.  On one hand it seems wrong, but... she's a demon.  The fewer you have opposing you, the easier it will be to end their threat completely.");
        }
        else if (player.cor < 66)
        {
            outputText("that taking a demon as a slave would've been abhorrent to you when you started this journey.  Now, it's just a means to a very pleasurable end.");
        }
        else
        {
    ...
    private function getCeraphFollowerPiercing() : Void
    {
        ...
        if (player.cor < 40)
        {
            outputText("a little hesitation.");
        }
        else
        {
            outputText("a smirk, secure in your knowledge of her defeat.");
        }
    ...
    private function CeraphHandsOutNewFetishesLikePervCandy() : Void
    {
        ...
        if (flags[kFLAGS.PC_FETISH] == 0)
        {
            ...
            {
                outputText("pulls a pair of gleaming, green piercings from a pouch.  \"<i>Oh, don't worry [Master]; you're going to love this so much.  These piercings are special, and they'll give you exactly what you want.</i>\"");
            }
            else
            {
                outputText("places her hands upon your pierced ears.  She whispers softly, \"<i>Don't worry [Master], I can easily use the piercings you already have.  It won't hurt.</i>\"");
            }
            outputText("\n\n");
            //(NOT PIERCED)
            if (player.earsPierced == 0)
            {
                outputText("The demon places a hand on your forehead and rubs your temples.  Numbness spreads through your body with every touch, until you can barely feel a thing.  She snaps an earring into your left ear, and dizziness washes over you.  A moment later she's piercing the other side, and the now-familiar vertigo that accompanies it seems to come and go quicker than before.  ");
            }
            //(PIERCED)
            else
            {
                outputText("The demon rubs your ears in her hands, numbing them slightly.  A gradual buzz builds behind your eyes, accompanied by a wave of dizziness.  You blink and try to shake your head, but as numb as you are, it's quite difficult.  After a few moments, the odd sensations pass, returning normal feeling to your ears and [face], much to your relief.  ");
            }
            ...
            {
                outputText("cock bounce in time with your fluttering heartbeats");
            }
            else if (player.hasVagina())
            {
                outputText("vagina get wetter and wetter");
            }
            else
            {
                outputText("parted lips and confused expression");
            }
        ...
        else if (flags[kFLAGS.PC_FETISH] == 2)
        {
            ...
            {
                outputText("your body aches for release.");
            }
            ...
            if (player.cockTotal() > 0)
            {
                ...
                {
                    outputText(", the old piercing clattering to the ground as it slides out, ");
                }
            ...
            else
            {
                ...
                {
                    outputText(" and discarding your old jewelry like a piece of garbage");
                }
    ...
    private function goThroughWithCeraphUnfetishification() : Void
    {
        ...
        if (player.cor < 33)
        {
            outputText("It seems there's still a spark of Ceraph's fire under all her submission.");
        }
        else if (player.cor < 66)
        {
            outputText("You sigh and wonder if you should punish her for giving you such lip.");
        }
        else
        {
            outputText("You slap her across the face for her temerity.");
        }
    ...
    {
        var x : Float = player.cockThatFits(115);
        if (x < 0)
        {
            x = 0;
        }
        var y : Float = player.cockThatFits2(115);
        clearOutput();
        spriteSelect(SpriteDb.s_ceraphClothed);
        if (y < 0)
        {
            sceneHunter.print("Check failed: 2 dicks fitting 115 area.");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText(" and half-erect, throbbing cock");
        }
        outputText(".  ");
        if (player.cor < 33)
        {
            outputText("She gets down on all fours, then lays her cheek in the dirt.  Her arms cross behind her back, suddenly bound there by the abrupt appearance of those same panties, tied in a knot.  Supported by only her knees, her tits, shoulders, and her face, she asks, \"<i>Grab me and fuck me [Master], please grind your slave's face in the dirt!</i>\"\n\n");
        }
        else if (player.cor < 66)
        {
            outputText("She gets down on all fours, then lays her cheek in the dirt.  Her arms cross behind her back, and you pin them there for her, smiling as her panties appear as if by magic, tied in a binding knot around her wrists.  With only her knees and upper body supporting her, Ceraph begs, \"<i>Be rough with me.</i>\"\n\n");
        }
        else
        {
            ...
            if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
            {
                outputText("and her dick releasing a squirt of pre-cum ");
            }
        ...
        if (player.cor < 33)
        {
            outputText("Well, that wasn't quite what you would have thought of doing, but you've got a good idea what she wants, and you may as well go along with it.  The idea of taking her in such a rough, hard way stirs something primal within you, and you have an easy time slipping into a meaner, rougher persona.");
        }
        else if (player.cor < 66)
        {
            outputText("Well, with a request like that, you'll have no problem fucking her rough and hard.  The situation evokes something primal within you, and the inviting delta between Ceraph's thighs practically beckons you to ravage it.");
        }
        else
        {
            outputText("With an invitation like that, there's no way you'll decline.  Looking over her, a predatory thrill runs through you, urging you to utterly violate her.");
        }
        outputText("  You grab Ceraph's thighs");
        if (player.str < 60)
        {
            outputText(" and heave, lifting her up off the ground and forcing her to straddle you.");
        }
        else
        {
            outputText(" and easily lift her, forcing her to straddle you.");
        }
        outputText("  Dragging the demon back, you bring her slutty, sodden puss up to your " + cockDescript(x));
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText(", ignoring the rope of dangling pre-cum that hangs from her bumpy prick.");
        }
        else
        {
            outputText(", feeling the heat washing off her mons and onto your " + player.cockHead(x) + ".");
        }
        ...
        if (player.cockArea(x) > 150)
        {
            outputText("distorting her body around the sheer bulk of your massive member.\n\n");
        }
        else
        {
            outputText("immersing yourself in the decadent wetness.\n\n");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("Her cock dangles towards the ground, dripping thick ropes of pre-spunk as readily as her pussy.  ");
        }
        ...
        if (y >= 0)
        {
            ...
            if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
            {
                outputText("She squeals and sprays a thick rope of jizz from her bloated cock.  It seems a little prostate pressure was all it took to put her maleness over the edge.  ");
            }
            ...
            if (player.cockTotal() >= 3)
            {
                ...
                if (player.cockTotal() > 3)
                {
                    outputText("es have nothing to do but slide across her cheeks, dripping pre-cum all over her smooth skin.");
                }
                else
                {
                    outputText(" has nothing to do but slide across her cheeks, dripping pre-cum all over her smooth skin.");
                }
            }
            outputText("\n\n");
        }
        ...
        else
        {
            ...
            if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
            {
                outputText(" while her cock spews ropes of pre-cum into the dirt");
            }
        ...
        if (y >= 0)
        {
            outputText("Her asshole likewise gleefully devours the seed from your " + cockDescript(y) + ", flexing wildly from the fluid injection.  ");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText(", and her cock starts pumping more demonic spunk into the mud");
        }
        ...
        if (player.cumQ() >= 700)
        {
            outputText("bulging belly as she purrs, ");
        }
        else
        {
            outputText("purring, ");
    ...
    private function followerCeraphTongueFucking() : Void
    {
        ...
        if (player.wetness() >= 5)
        {
            outputText(", even though her cheeks are bulging and her throat struggles to devour all of the fountaining girl-spunk.");
        }
        else if (player.wetness() >= 4)
        {
            outputText(", even though her cheeks are slightly bulged and she's gulping it down.");
        }
        else if (player.wetness() >= 3)
        {
            outputText(", even though she has to gulp it down from time to time.");
        }
        else if (player.wetness() >= 2)
        {
            outputText(", even though you produce enough for her to gulp.");
        }
        else
        {
            outputText(", even though your pussy doesn't gush like most of the corrupted creatures in this realm.");
        }
    ...
    private function ceraphTentacleGrape() : Void
    {
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("  You can vaguely see Ceraph's hard, demonic-dick underneath all the waving tentacles.  She must have taken your command to keep her dick out for your use quite literally, even though there's little chance you'll get to put it anywhere.");
        }
        outputText("\n\n");
        ...
        if (player.cockTotal() == 1)
        {
            outputText("all of your girth");
        }
        else
        {
            outputText("in all of your members simultaneously");
        }
        ...
        if (player.cumQ() >= 800)
        {
            outputText("bulges wide from the sheer size, sucking");
        }
        else
        {
            outputText("sucks");
    ...
    public function catgirlEncounter() : Void
    {
        ...
        if (!player.hasPerk(PerkLib.Evade) && player.spe / 5 + rand(20) < 22)
        {
            outputText("flies into you, knocking you to the ground.  After a moment, you find yourself faced with a pair of overeager cat-morphs grinning down at you.");
        }
        else
        {
            outputText("just barely misses you and crashes into the ground behind you.  After a moment, two bodies disentangle themselves from the impact site.  Once they stand up, you can see that a pair of overeager cat morphs have arrived in your camp.");
        }
        outputText("\n\n");
    ...
    private function fuckCeraphsCatgirls() : Void
    {
        ...
        {
            outputText("  When some of your milk seeps out, she leans forward and latches onto a nipple eagerly, alternating between sucking on the tip and licking the drops off of it.");
        }
        else
        {
            ...
            if (rand(5) == 0)
            {
                outputText(" but some of the fur tickles your nose a bit <i>too</i> deeply; you deliver a sudden sneeze into her bosom, causing it to heave and jiggle.");
            }
            else
            {
                outputText("vibrating it wildly back and forth.");
            }
        }
        ...
        if (player.cockTotal() == 1)
        {
            outputText("sliding her hand between the furry tits, down the base of your " + cockDescript(x) + ".");
        }
        else
        {
            outputText("grabbing the lonely, neglected dickflesh left outside the warm embrace of her sister's breasts and shoving them into her own while caressing the tip with her tongue.");
        }
        outputText("\n\n");
        ...
        if (player.cumQ() >= 1000)
        {
            outputText("unleashes a torrent of ejaculate, coating the two girls liberally; each yowls in happiness, slurping at the nearby cock-head and swallowing mightily.");
        }
        else if (player.cumQ() >= 300)
        {
            outputText("lets out a generous load of 'cream' which each girl sloppily gulps down, lapping at the jism and its source roughly.");
        }
        else
        {
            outputText("squeezes out a few immodest squirts of semen, which the girls push and fight over, each vying to be the one to gulp down the next stroke.");
        }
        outputText("\n\n");
        ...
        if (player.cumQ() >= 1000)
        {
            outputText(" clean the remnants off of their fur eagerly, mollified by the sheer amount into actually helping lick each other clean with their tongues, and");
        }
    ...
    private function ceraphUrtaRoleplayDrunk2() : Void
    {
        ...
        if (player.isGoo() || player.isNaga())
        {
            outputText("wriggle to a wider stance");
        }
        else
        {
            outputText("spread your legs");
        }
        ...
        {
            outputText("  After I knock you up, try explaining the fox tails on your kids to those other bitches.");
        }
    ...
    private function ceraphUrtaRoleplaySober() : Void
    {
        ...
        {
            outputText("  Unable to keep her hands at her sides, but unwilling to stand between your mouth and her slit, the fox-girl takes hold of your horns, pulling your face tightly against her mound, her chest tight with a barely audible squeak of delight. Stroking the tip of your tongue at the curtain of her sex, you allow her the barest trace of penetration before drawing back and placing a wet kiss on her clit. Enough foreplay.");
        }
    ...
    private function ceraphUrtaRoleplaySober2() : Void
    {
        ...
        {
            outputText("  Even your tremendous size is no impediment to blessing the girl with your passion- every inch of her body gives way as you sink into her beyond the limits you would normally expect, as if her body were perfectly tailored to yours.");
        }
        outputText("\n\n");
    ...
    private function ceraphLackeyCorruption() : Void
    {
        ...
        if (!player.hasCock())
        {
            ...
            if (player.hasVagina())
            {
                outputText("your " + clitDescript() + " swells");
            }
            else
            {
                outputText("a small bump forms on your mons");
            }
        ...
        var x : Float;
        var demon : Bool = false;
        x = player.biggestCockIndex();
        if (player.hasCock())
        {
            if (player.cocks[x].cockType == CockTypesEnum.DEMON)
            {
                demon = true;
            }
        }
        ...
        if (!demon && !player.hasCock())
        {
            outputText(", then the temporary phallus shrinks and disappears back into your crotch");
        }
        if (!demon)
        {
            outputText(".\n\n");
    ...
    private function sweetieNOOOO() : Void
    {
        ...
        {
            outputText("similar to the one you got from the factory.");
        }
        else
        {
            outputText("not too different from the ones you've seen in Ingnam, although modified for human use, it seems.");
        }
        ...
        //([if first time]
        if (flags[kFLAGS.CERAPH_RP_MARBLE_COUNT] == 0)
        {
            outputText("Seeing your confused stare, she simply answers with, \"<i>Ah, do be patient... sweetie,</i>\" and goes back to her work.");
        }
        else
        {
            outputText("You simply chuckle knowingly at the apparent size disparity of the garment.");
        }
        ...
        if (flags[kFLAGS.MARBLE_DICK_LENGTH] > 0 && flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("Her nubbly member presumably shifts its color from royal purple to a lighter, brownish tone as well, inflating to the familiar seven-inch measurement; at least, so you'd judge by the sudden bulge in the overalls.  ");
        }
        ...
        if (flags[kFLAGS.CERAPH_RP_MARBLE_DISABLED] == 1)
        {
            doNext(postUdderChoice);
        }
        else
        {
    ...
    //[in a pig's eye, pal]
    private function noUdderPlz(perm : Bool = false) : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_marble);
        if (perm)
        {
            flags[kFLAGS.CERAPH_RP_MARBLE_DISABLED] = 1;
        }
    ...
    private function postUdderChoice(newl : Bool = false) : Void
    {
        spriteSelect(SpriteDb.s_marble);
        if (newl)
        {
        ...
        //([if udder]
        if (flags[kFLAGS.CERAPH_UDDERS_DISABLED] == 1)
        {
            outputText(", making sure to prep four extra tubes for her udder");
        }
        ...
        if (flags[kFLAGS.CERAPH_UDDERS_DISABLED] == 1)
        {
            outputText("  The four others quickly follow, the udder-cups sucking onto the nubs like hungry children.");
        }
        outputText("\n\n");
        ...
        if (player.hasBalls())
        {
            outputText("your [balls] tightening");
        }
        else
        {
            outputText("a readiness in your body");
        }
        outputText(", and you wind up for one last thrust into her depths.  ");
        var cum : Float = player.cumQ();
        if (cum < 50)
        {
            outputText("Her ass has no problem in taking your ejaculatory package, lapping it up like it was nothing.");
        }
        else if (cum < 250)
        {
            outputText("She groans a bit from the amount of seed you pump into her butt, but she doesn't seem overly concerned by it.");
        }
        else if (cum < 750)
        {
            outputText("Her belly actually bulges out a bit more as you shoot your sperm into her perfect butthole, but her only response is an approving grunt.");
        }
        else if (cum < 2000)
        {
            outputText("Normal girls might not be able to take this much cum at one time, but 'Marble' simply bucks her hips happily as her belly is stretched as least as large as her boobs to make room for the load.");
        }
        else
        {
            outputText("Her entire body writhes in ecstatic bliss as your gigantic load pumps into her, wave after wave of baby-batter inflating her gut so full of cum.  By the time you're done, her navel is an outtie; what's more, it's actually scraping the barn floor, and if her arms were free, you don't doubt she'd be rubbing happily at it.");
        }
        if (cum >= 10000)
        {
            outputText("  Marble's body convulses so hard it actually dislodges the funnel, dragging the tube free of her mouth.  \"<i>SWEETIE!</i>\" she screams, grinding herself against you as her stomach burgeons forward, filled with enough seed to propagate an entire city of goblins for months.  \"<i>MORE!</i>\"  You're able to give her just that, bloating her billowing belly obscenely, stretching the creamy skin to its near-breaking point with your virility.  Even if you wanted to pull out, you couldn't, not with her anal walls clamped onto your [cock] like this.  Her belly actually gains enough mass to nearly fill up the stall, pushing her tits up into her chin.");
        }
        if (player.cockTotal() == 2)
        {
            outputText("  Your other cock sprays ");
        }
        else if (player.cockTotal() > 2)
        {
            outputText("  Your other cocks spray ");
        }
        if (player.cockTotal() > 1)
        {
            outputText("a ");
            ...
            if (cum >= 250 && cum < 1000)
            {
                outputText(", spattering the entire area");
            }
            else if (cum >= 1000 && cum < 3000)
            {
                outputText(", drenching the area in white");
            }
            else if (cum >= 3000)
            {
                outputText(", coating the area in a thick sheet of alabaster");
            }
            outputText(".");
        }
        outputText("\n\n");
        ...
        if (cum >= 2000 && player.cor >= 66)
        {
            outputText(" and chuckle at the drizzle of semen that issues from her butt and mouth");
        }
    ...
    private function cerminika() : Void
    {
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText(" while her cock shrinks to what looks approximately like six inches of meat, an outright tiny penis for Mareth");
        }
        outputText(".\n\n");
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("  Her dick presses up into your crotch.");
        }
        ...
        {
            outputText("She casually and playfully gives the gentlest squeeze to your " + chestDesc() + ".  ");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText(", and her prick has remained firm and hard");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("The very base of her prick teases the veins of your own shaft, but she seems focused on the pleasure of her cunt for now.  ");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("  The degree that her cock abruptly shoots straight up is perhaps the most amusing part of the act.");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText(", and the heat of her prick bouncing into your lap");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("  A single bead of pre-cum forms slowly at the tip of her prick, growing larger until it finally slips down into the curve of her head, sliding down her shaft and into your lap.");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("Her prick bounces between her belly and yours as you take her.  ");
        }
        ...
        {
            outputText(" and slap at your own chest");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("A light spurt of pre-cum from her prick warms the flesh where your hips meet.  ");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("  You can practically hear her rock-hard cock kicking up the ground.");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("  Her prick has become engorged and infernal, covered with abnormal bumps and veins. It practically spits up pre-cum, gurgling with a greatly increased supply.");
        }
        ...
        if (player.cumQ() >= 500)
        {
            outputText("  You dump loads in her as though she's an addict who hasn't fed for days, bloating and humiliating her with the sheer volume of spunk that now inflates her belly.  Not as though the demon whore has much humility left to take.");
        }
        else
        {
            outputText("  She shudders as your [cock] fills her guts with warmth, hips seizing as she rides out her assgasm.");
        }
        outputText("  Her cunt nearly waterfalls her juices, squirting obscenely as she rides out the pleasure.");
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("  The demonic prick placed between her legs stops bouncing only because of how rock-hard it is as it spews her own filthy fucksludge across her body, splattering onto her tits and across her face.  The volume of off-white cream it spews out takes even longer to finish than you do, painting her obscenely.");
        }
        outputText("\n\n");
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("  Her own filthy jizz has already been licked clean by her hungry mouth, so as not to dirty your handsome prick.");
        }
        ...
        if (player.biggestCockArea() >= 75)
        {
            outputText("I doubt her ass could actually take you so well, but... I wanted my [master] to enjoy a perfect asshole.  Sue me.  ");
        }
    ...
    private function fuckZetsukosTonguepussy() : Void
    {
        ...
        {
            outputText("  You wonder just how she plans to fit you inside, but there's not much you can do about it at this point.  You'll just have to trust that she knows what she's doing, crazy or not.");
        }
        ...
        if (player.hasBalls())
        {
            outputText("slithering across your [balls], ");
        }
        else if (player.hasVagina())
        {
            outputText("gently licking the outer edges of your labia, ");
        }
    ...
    private function getTonguedByZetsuko() : Void
    {
        ...
        if (x >= 0)
        {
            outputText("wrapping itself around your " + cockDescript(x) + " and squeezing momentarily before ");
        }
        ...
        if (x >= 0)
        {
            ...
            if (player.cumQ() <= 100)
            {
                outputText("spurting a healthy spray of hot cum all over her face and chest.");
            }
            else if (player.cumQ() <= 500)
            {
                outputText("distending visibly as it begins to pump out thick streams of spunk all over her waiting face and tits.");
            }
            else
            {
                outputText("unleashing a heavy downpouring of cum all over her face and tits that soaks through her straitjacket and mats her hair down with your thick load.");
            }
        ...
        if (player.wetness() >= 4)
        {
            outputText(" and overflowing to dribble down her bountiful ass cheeks and stomach");
        }
    ...
    private function giveFollowerBodyBits() : Void
    {
        ...
        if (player.hasCock() || player.hasVagina() || player.hasBreasts())
        {
            if (player.hasCock())
            {
                outputText("  I could take your cock.");
            }
            if (player.hasVagina())
            {
                outputText("  I could remove your snatch.  Don't worry, once it's off of you, anything I put in it won't wind up in your womb.");
            }
            if (player.hasBreasts())
            {
                outputText("  I could even make off with your tits if you like.");
            }
        }
        else
        {
            outputText("  Sadly, you don't have anything that would be worth taking right now.  A true shame, my [Master].");
        }
        outputText("</i>\"");
        if (player.gender > 0 || player.biggestTitSize() > 0)
        {
            ...
            if (player.hasCock())
            {
                ...
                if (player.cockTotal() > 1)
                {
                    smallestWang = createCallBackFunction(ceraphFollowerCockTaking, true);
                }
            }
            if (player.hasVagina())
            {
            ...
            if (player.biggestTitSize() >= 1)
            {
                ...
                if (player.bRows() > 1)
                {
                    breasts2 = createCallBackFunction(ceraphFollowerTitTaking, 1);
                }
                if (player.bRows() > 2)
                {
                    breasts3 = createCallBackFunction(ceraphFollowerTitTaking, 2);
                }
            }
    ...
    private function ceraphFollowerCockTaking(smallest : Bool = false) : Void
    {
        spriteSelect(SpriteDb.s_ceraphClothed);
        clearOutput();
        ...
        if (player.cocks[x].cockThickness >= 4)
        {
            outputText(", compressing your massive girth into the rigid shaft");
        }
    ...
    private function ceraphFollowerTitTaking(rowNum : Int = 0) : Void
    {
        ...
        if (player.breastRows[x].fuckable)
        {
            flags[kFLAGS.CERAPH_STOLEN_BODYPART] = 4;
        }
        else if (player.lactationQ() >= 100)
        {
            flags[kFLAGS.CERAPH_STOLEN_BODYPART] = 5;
        }
        else
        {
            flags[kFLAGS.CERAPH_STOLEN_BODYPART] = 3;
        }
        ...
        //If only 1 row below{
            ...
            if (x >= player.breastRows.length - 2)
            {
                outputText("your torso has shifted and your " + breastDescript(x + 1) + " have moved up to fill the void.");
            }
            //Multiple below
            else
            {
                outputText("your torso has shifted and your other breasts have moved up to fill the void.");
            }
        ...
        {
            outputText("<b>ERROR!  ERROR!  Please contact Fenoxo and reload your game if you don't want your save messed up.</b>");
        }
        outputText("\n\n");
        
        outputText("You gasp when Ceraph ");
        if (flags[kFLAGS.CERAPH_STOLEN_BODYPART] == 3)
        {
            outputText("rolls the nipples in her hands");
        }
        else if (flags[kFLAGS.CERAPH_STOLEN_BODYPART] == 4)
        {
            outputText("pushes her fingertips inside the leaky nipple-cunts");
        }
        else
        {
            outputText("squeezes out a squirt of milk");
        }
    ...
    private function stuffSomeNippleCunts() : Void
    {
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText(" and proud demon-dick");
        }
        ...
        if (player.totalNipples() > 2)
        {
            outputText("  She moves from pussy to pussy with ease, exploring every single one of your myriad holes, a gleeful smile on her face.");
        }
        if (player.lactationQ() > 0)
        {
            outputText("  A squirt of breastmilk nearly takes your slave in the eye, your milk letting down uncontrollably from the tit-puckering finger-fuck.");
        }
        if (player.hasCock())
        {
            outputText("  [EachCock] is so hard by this point that it's smearing its pre-cum all over Ceraph's belly, but she doesn't mind.");
        }
        if (player.hasVagina())
        {
            outputText("  As for your [vagina], you've grown more than wet enough for a hard dicking, but honestly, right now, all you want is your [nipples] to be perfectly and completely fucked.");
        }
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("  The fetishistic slut's sloppy cock is dribbling on your hip, neglected and aroused.  Poor thing.");
        }
        ...
        if (player.bRows() > 1)
        {
            ...
            if (player.bRows() == 2)
            {
                outputText("another row of female flesh begins");
            }
            else
            {
                outputText("more rows of female flesh begin");
            }
        ...
        {
            outputText("  She strokes them, one by one, and they split down the middle, dividing into a multitude of penises, a perfect match to your quad-nipplecunts.");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText(", her cock spouting its own ivory wetness onto your belly in sympathetic release.");
        }
        else
        {
            outputText(", her pussy dripping weakly in sympathetic release.");
        }
    ...
    private function portalFuckWithFollowerCeraph() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_CERAPH_PORTAL_FUCKED] == 0)
        {
            ...
            if (flags[kFLAGS.CERAPH_HIDING_DICK] == 1)
            {
                outputText("  Of course, you don't seem to like that appendage, so my ring won't get much use today, will it?");
            }
            outputText("</i>\"");
            if (flags[kFLAGS.CERAPH_HIDING_DICK] == 1)
            {
                outputText("  Ceraph twirls one of the rings into her palm, where it promptly vanishes in a puff of purple smoke.");
            }
        ...
        else
        {
            ...
            if (flags[kFLAGS.CERAPH_HIDING_DICK] == 1)
            {
                outputText("  She sadly teleports one of them away, knowing you've commanded her to stay female for you.");
            }
        ...
        if (flags[kFLAGS.TIMES_CERAPH_PORTAL_FUCKED] == 0)
        {
            outputText("  Well, your dick doesn't look any different, aside from the gilden adornment stuck near the bottom.  Now, if it were hilt-deep in the demon's cunt, that would be something!");
        }
        else
        {
            outputText("  Well, no sense wasting time!  You look down at Ceraph's moist box and imagine your dick spearing through her folds.");
        }
    ...
    private function layEggsInSlaveCeraph() : Void
    {
        ...
        if (!player.isTaur())
        {
            outputText("absently rub the sensitive bulge in your abdomen as you ");
        }
        ...
        if (player.hasCock())
        {
            outputText("  Ceraph glances at the bulge in your [armor] quickly before she demurely lowers her eyes.  A wicked smile plays across her beautiful face as she responds, \"<i>Yes, [master].</i>\"");
        }
        else
        {
            outputText("  Ceraph frowns as she glances towards your crotch.  She is disciplined enough to answer you quickly, however, with a sharp \"<i>Yes, [master].</i>\"");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("  The whole garment creaks and strains under the pressure of her hardening member.");
        }
        ...
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("; unable to keep it penned up, her demonic cock spills out the side, waving back and forth obscenely as you saw the offending garment into her crotch");
        }
        outputText(".");
        ...
        if (!player.isTaur())
        {
            outputText(", and your fingers tremble slightly as you trace its segmented length, leaving a trail of lubricating mucus from tip to base");
        }
        ...
        if (player.gender > 0)
        {
            outputText("Your orgasm spatters your degraded demon's rear, adding to the sopping mess your ovipositor is leaving.  ");
        }
    ...
    private function sumissivenessToCeraphFollower() : Void
    {
        ...
        {
            outputText(", what with the beads of moisture forming between your legs");
        }
        else
        {
            outputText(".  Your hardness is easily visible to anyone who knows where to look, and the demoness definitely knows where to look");
        }
        outputText(".");
        ...
        {
            outputText("your rigid clit");
        }
        outputText(" with ticklishly-soft touches");
        if (player.hasBalls())
        {
            outputText(" before gently squeezing your [sack]");
        }
        outputText(", savoring your state.  You groan out loud at the sudden sensation, and a bead of ");
        if (player.hasCock())
        {
            outputText("pre-cum has begun to form on your [cockHead biggest]");
        }
        else
        {
            outputText("girl-cum has begun to form at your quivering entrance");
        }
    ...
    //[Either Once]
    private function barkToCeraphOnce(dog : Bool = true) : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_ceraphClothed);
        outputText("\"<i>");
        if (dog)
        {
            outputText("Arf!</i>\" you bark");
        }
        else
        {
            outputText("Meowwwww!</i>\" you mew");
        }
        outputText(", loud and almost proudly");
        if (player.tailType > Tail.NONE)
        {
            outputText(", wagging your tail excitedly");
        }
        outputText(".");
        outputText("\n\nCeraph casually pulls her panties a bit lower to ");
        if (flags[kFLAGS.CERAPH_HIDING_DICK] == 0)
        {
            outputText("ease the pressure on her tightly-contained phallus");
        }
        else
        {
            outputText("allow a penis to grow, fully-formed from where her clit was");
        }
        ...
        else if (player.analCapacity() < 20)
        {
            outputText("Even though you're pretty tight, the cunt-slick tool slides into your rectum with ease, the semi-soft nodules massaging your anal ring as it adjusts to the heated impalement.");
        }
        else if (player.ass.analLooseness >= 4)
        {
            outputText("The cunt-slick tool has no problem wiggling inside your well-used and abused asshole, hilted in a split second, the hundreds of nubs gently caressing your anal ring like a lover as they nestle inside you.");
        }
        else
        {
            outputText("The cunt-slick tool slides right on in thanks to your capacitative anus, the gentle nubs each massaging your snug ring as they slide on through.");
        }
        ...
        if (player.hasCock())
        {
            outputText("your twitching maleness");
        }
        else if (player.hasVagina())
        {
            outputText("to your achingly empty pussy");
        }
        if (player.cockTotal() > 1)
        {
            outputText("es");
        ...
        {
            outputText("lady spunk");
        }
        outputText(".  It dangles heavily enough to ");
        if (player.hasCock())
        {
            outputText("make your [cock biggest] bob below");
        }
        else
        {
            outputText("tantalize and tease your clit");
        }
        ...
        if (player.hasCock())
        {
            outputText("rams straight into your inner, cock-milk reserve");
        }
        else
        {
            outputText("rubs against your quivering vaginal walls, stimulating you from the inside out");
        }
        ...
        if (player.hasCock())
        {
            outputText("Dick-milk");
        }
        else
        {
            outputText("Love-juice");
        }
        ...
        {
            outputText("your slightly parted twat");
        }
        ...
        if (player.hasCock())
        {
            outputText("inner reservoir is squeezed");
        }
        else
        {
            outputText("cunt walls are stimulated through your anal walls");
        }
        ...
        if (player.hasCock())
        {
            outputText("Ironically, [eachCock] has begun to lose some of its stiffness, and you grow limper and limper as more of your cum leaks from you");
        }
        else
        {
            outputText("Strangely, the more femcum that leaks from your winking, tender twat, the more your body tingles and shivers with barely-constrained delight");
        }
        outputText(".");
        ...
        if (player.hasCock())
        {
            outputText("the 'pressure'");
        }
        else
        {
            outputText("a need within you");
        }
        ...
        if (player.hasCock())
        {
            if (player.cockTotal() == 1)
            {
                outputText(", even with a soft cock");
            }
            else
            {
                outputText(", even with soft cocks");
            }
        }
        ...
        if (dog)
        {
            outputText("bark in pleasure like the cum-slut doggy that you are");
        }
        else
        {
            outputText("mewl in delight like the cock-loving kitty that you are");
        }
        ...
        if (player.hasCock())
        {
            outputText("prostate is getting pushed up against each time");
        }
        else
        {
            outputText(", depressingly empty cunt walls are getting teased with every thrust");
        }
        outputText(".");
        ...
        if (player.hasCock())
        {
            outputText("prostate is");
        }
        else
        {
            outputText("vaginal walls are");
        }
        ...
        if (player.hasCock())
        {
            outputText("straight onto your sore prostate");
        }
        else
        {
            outputText("putting more delicious pressure on your sensitive walls");
        }
        ...
        {
            outputText("your pussy lets go of the last of its sticky, slippery snatch secretions");
        }
        outputText(".");
        if (player.cumQ() >= 700)
        {
            outputText("  It floods out everywhere.  You're just so productive that your hands and [legs] sink partway into the mess.");
        }
        
        outputText("\n\nYour Mistress eventually finishes and pulls out, offering a ");
        if (dog)
        {
            outputText("tasty bone-shaped biscuit");
        }
        else
        {
            outputText("fishy-smelling treat");
        }
    ...
    //[Either Twice]
    private function barkOrMeowTwiceToCeraph(dog : Bool = true) : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_ceraphClothed);
        outputText("\"<i>");
        if (dog)
        {
            outputText("Arf! Arf!</i>\" you bark");
        }
        else
        {
            outputText("Meow!  Meowwwww!</i>\" you mew");
        }
        ...
        //{Normal Holli}{
            ...
            else
            {
                ...
                if (flags[kFLAGS.HOLLI_SUBMISSIVE] != null)
                {
            ...
            {
                outputText("your rebellious clitoris, which has the unintended side effect of making it and your drooling cuntlips puff up, redder than ever before");
            }
            outputText(".  ");
            if (player.hasCock())
            {
                outputText("It bounces against your tummy");
            }
            else
            {
                outputText("Your vaginal muscles clench and contract");
            }
        ...
        //Both{
            ...
            else
            {
                ...
                if (!jojo)
                {
                    outputText("cum-slut is");
                }
                else
                {
                    outputText("I guess I'm");
                }
            ...
            if (amilyFollower() && flags[kFLAGS.AMILY_FOLLOWER] == 2 && campCorruptJojo())
            {
                outputText("look around uncertainly before darting back to their usual places, knowing you'll give the command if you want them to take care of your desires.");
            }
            else
            {
                outputText("looks around uncertainly before darting back to the usual places, knowing you'll give the command if you want a slave to take care of your desires.");
            }
        }
        //Generic
        else
        {
            ...
            if (player.hasCock())
            {
                outputText("erect maleness");
            }
            else
            {
                outputText("quivering quim");
            }
            if (player.cockTotal() > 1)
            {
                outputText("es");
            ...
            if (player.hasCock())
            {
                outputText("cum this nice, I'd be milking [him] dry every time [he] could get it up");
            }
            else
            {
                outputText("a scent like this, I'd be fucking [him] every chance I could get");
            }
            ...
            if (player.hasCock())
            {
                outputText("pre-cum rockets");
            }
            else
            {
                outputText("fem-juice spurts");
            }
            ...
            if (player.hasCock())
            {
                outputText("pre-ejaculate");
            }
            else
            {
                outputText("lady jizz");
            }
            ...
            if (player.cockTotal() > 1)
            {
                outputText(" feel like they");
            }
            else
            {
                outputText(" feels like it");
            }
            outputText(" could go off at any second.  The journey is murder for your poor, ");
            if (player.hasCock())
            {
                outputText("over-inflated maleness");
            }
            else
            {
                outputText("puffy, aching cleft");
            }
            if (player.cockTotal() > 1)
            {
                outputText("es");
            ...
            {
                outputText("tipping over the edge");
            }
