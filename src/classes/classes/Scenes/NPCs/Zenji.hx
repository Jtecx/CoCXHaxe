...
class Zenji extends Monster
{
    private function zenjiSpearThrust() : Void
    {
        ...
        if (player.speedDodge(this) > 0)
        {
            ...
            {
                outputText("you manage to dodge his oncoming attack");
            }
            else
            {
                outputText("he misses as you dodge his oncoming attack");
            }
        }
        else
        {
            ...
            damage = as3hx.Compat.parseInt(((str + 60 + weaponAttack) * boost) - Math.random() * (player.tou) - player.armorDef);
    ...
    private function zenjiFeint() : Void
    {
        ...
        if (player.speedDodge(this) > 0)
        {
            if (flags[kFLAGS.ZENJI_PROGRESS] == -1)
            {
                outputText("but you narrowly manage to avoid his tusks");
            }
            else
            {
                outputText(" but you manage to avoid his tusks");
            }
        }
        else
        {
            ...
            damage = as3hx.Compat.parseInt(((str + 50 + weaponAttack) * boost) - Math.random() * (player.tou) - player.armorDef);
            if (flags[kFLAGS.ZENJI_PROGRESS] == -1)
            {
                outputText(", he strikes you for ");
            }
            else
            {
                outputText(". He strikes you for ");
    ...
    private function zenjiEnrage() : Void
    {
        ...
        damage = as3hx.Compat.parseInt(((str + 100 + weaponAttack) * boost) - Math.random() * (player.tou / 2) - player.armorDef);
        if (this.short == "Zenji")
        {
            outputText("Zenji yells with primal wrath, he's fully intent on decimating");
        }
        else if (flags[kFLAGS.ZENJI_PROGRESS] == -1)
        {
            outputText("The troll yells with primal wrath, he's fully intent on killing");
        }
        else
        {
            outputText("The troll yells with primal wrath, he's fully intent on decimating");
        }
    ...
    private function zenjiRegenerate() : Void
    {
        ...
        if (this.short == "Zenji")
        {
            outputText("Zenji crouches down for a moment and clutches at his wounds, \"<i>Dis is noting!</i>\" He shouts, \"<i>Ya gotta try harda dan dat!</i>\" And before your eyes, his wounds close as he heals for " + heal + ".");
        }
        else
        {
            outputText("The troll crouches down for a moment and clutches at his wounds, \"<i>YOU ARE NOTHING!</i>\" He shouts, \"<i>I WILL NOT FALL!</i>\" And before your eyes his wounds close as he heals for " + heal + ".");
        }
        this.HP += heal;
    }
    private function zenjiPiercingBlow() : Void
    {
        ...
        if (player.speedDodge(this) > 0)
        {
            outputText(". You dodge the incoming spear, but you notice " + ((this.short == "Zenji") ? "Zenji" : "him") + " charging at you, he rams into your midriff, knocking the wind out of you. You struggle to regain your composure and are having trouble concentrating on spells for a moment.");
        }
        else
        {
            var damage : Float = 0;
    ...
    override private function performCombatAction() : Void
    {
        if (flags[kFLAGS.ZENJI_PROGRESS] == 8 || flags[kFLAGS.ZENJI_PROGRESS] == 9)
        {
            if (this.level > 49)
            {
                if ((this.lust > this.maxLust() * .8) || (this.HP < this.maxHP() * .3))
                {
                    zenjiEnrage();
                }
                else if (this.HP < this.maxHP() * .4 && mana > maxMana() * 0.2)
                {
                    zenjiRegenerate();
                }
                else
                {
                    var choice5 : Float = rand(4);
                    if (choice5 < 2)
                    {
                        zenjiSpearThrust();
                    }
            ...
            else if (this.lust > this.maxLust() * .8)
            {
                zenjiEnrage();
            }
            else
            {
                var choice3 : Float = rand(3);
                if (choice3 < 2)
                {
                    zenjiSpearThrust();
                }
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        ...
        {
            SceneLib.zenjiScene.followerZenjiSparZenjiDefeated(hpVictory);
        }
        else if (flags[kFLAGS.ZENJI_PROGRESS] > 5)
        {
            SceneLib.zenjiScene.part2TrollEncounterRepeatFightZenjiDefeated(hpVictory);
        }
        else if (flags[kFLAGS.ZENJI_PROGRESS] == 5)
        {
            SceneLib.zenjiScene.part2TrollEncounterFirstFightZenjiDefeated(hpVictory);
        }
        else if (flags[kFLAGS.ZENJI_PROGRESS] == -1)
        {
            SceneLib.zenjiScene.part1TrollEncounterFightTOTHEDEATHZenjiDefeated(hpVictory);
        }
        else
        {
            SceneLib.zenjiScene.part1TrollEncounterFightZenjiDefeated(hpVictory);
        }
    }
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (flags[kFLAGS.ZENJI_PROGRESS] == 8)
        {
            SceneLib.zenjiScene.followerZenjiSparPCDefeated();
        }
        else if (flags[kFLAGS.ZENJI_PROGRESS] > 5)
        {
            SceneLib.zenjiScene.part2TrollEncounterRepeatFightPCDefeated();
        }
        else if (flags[kFLAGS.ZENJI_PROGRESS] == 5)
        {
            SceneLib.zenjiScene.part2TrollEncounterFirstFightPCDefeated();
        }
        else if (flags[kFLAGS.ZENJI_PROGRESS] == -1)
        {
            SceneLib.zenjiScene.part1TrollEncounterFightTOTHEDEATHPCDefeated();
        }
        else
        {
            SceneLib.zenjiScene.part1TrollEncounterFightPCDefeated();
        }
    }
    ...
    public function new()
    {
        ...
        if ((flags[kFLAGS.ZENJI_PROGRESS] == 8 || flags[kFLAGS.ZENJI_PROGRESS] == 9) && this.level >= 40)
        {
