...
class Urta extends NPCAwareContent implements TimeAwareInterface
{
    public var nofitMsg(get, never) : String;
...
*/
    ...
    public function timeChange() : Bool
    {
        ...
        if (flags[kFLAGS.NEED_URTA_LETTER] == 1 && model.time.hours == 6)
        {
            urtaPregs.getUrtaLetter();
        }  //Urta Letters  
        ...
        {
            flags[kFLAGS.URTA_EGG_FORCE_EVENT]--;
        ...
        if (flags[kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN] > 1)
        {
            ...
            if (flags[kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN] < 1)
            {
                flags[kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN] = 1;
            }
        }
    ...
    public function urtaSprite() : Void
    {
        if (urtaDrunk())
        {
            spriteSelect(SpriteDb.s_urtaDrunk);
        }
        else
        {
    ...
    public function urtaCapacity() : Float
    {
        var bonus : Int = as3hx.Compat.parseInt(flags[kFLAGS.TIMES_RUT_FUCKED_URTAS_CUNT] * 5);
        if (bonus > 40)
        {
            bonus = 40;
        }
        if (flags[kFLAGS.URTA_TENTACLE_GAPED] > 0)
        {
            return 500;
        }
        return 60 + bonus;
    ...
    public function cockFits() : Bool
    {
        return player.cockThatFits(urtaCapacity()) >= 0;
    }
    
    private function get_nofitMsg() : String
    {
        return "Req. a cock fitting " + urtaCapacity() + " area.";
    }
    
    override public function urtaLove(love : Float = 0) : Bool
    {
        ...
        //Queue up Amily madness{
            ...
            if (love > 0 && flags[kFLAGS.AMILY_VISITING_URTA] == 0 && amilyScene.amilyFollower() && flags[kFLAGS.AMILY_FOLLOWER] == 1)
            {
                flags[kFLAGS.AMILY_NEED_TO_FREAK_ABOUT_URTA] = 1;
            }
            return true;
        }
        else
    ...
    public function hoursUntilHorny(hours : Float, add : Bool = false) : Float
    //reset days counter
    {
    ...
    public function urtaAvailableForSex() : Bool
    {
        return urtaFuckbuddy() && telAdre.scylla.action != Scylla.SCYLLA_ACTION_FUCKING_URTA && flags[kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN] == 0;
    }
    
    public function urtaFuckbuddy() : Bool
    //Returns true if Urta is either the player's fuckbuddy or lover
    {
        
        if (SceneLib.urtaQuest.urtaBusy() || flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] == -1)
        {
            return false;
        }
        if (urtaJustFriends())
    ...
    public function urtaJustFriends() : Bool
    {
        return flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] == 0 && flags[kFLAGS.URTA_PC_LOVE_COUNTER] == -1;
    }
    
    public function urtaAtBar() : Bool
    ...
    public function urtaDrunk() : Bool
    //Preg = no drinking!
    {
        ...
        var _sw33_ = (flags[kFLAGS.URTA_DRINK_TOOGLE]);        
    ...
    private function urtaOpenAboutEdryn() : Bool
    ...
    public function urtaBarDescript() : Bool
    {
        outputText("\n\n");
        //Urta PISSED
        if (flags[kFLAGS.URTA_ANGRY_AT_PC_COUNTDOWN] > 1)
        ...
        if (pregnancy.isPregnant)
        {
            ...
            else
            {
                ...
                //Egg Level 1:
                if (flags[kFLAGS.URTA_EGGS] < 20)
                {
                    outputText("Urta is sitting quietly at her usual seat, sipping a mug of some non-alcoholic beer.  Though her clothes are still in place, you can just make out the bulging midriff stretching them taut, a consequence of all the eggs in her womb.");
                }
                else if (flags[kFLAGS.URTA_EGGS] < 40)
                {
                    outputText("Urta is sitting rather uncomfortably at her usual seat.  Her swollen stomach is so large it keeps her from wearing her usual outfit, instead forcing her into a mini-shirt and a skirt bottom, allowing her midriff to be bare.  She occasionally runs a hand over the expanse, as if she can't believe it's actually there.");
                }
                //Egg Level 3:
                else
                {
                    outputText("Urta's bloated belly forces her to sit side-saddle at her usual table, the lumpy, egg-stuffed mass ballooning out in front of her.  Forced into a midriff-baring ensemble, she frequently touches it, occasionally seeming to trace an egg through the taut skin.");
                }
            }
            return true;
        }
        ...
        if (flags[kFLAGS.URTA_PC_LOVE_COUNTER] == 1)
        {
            ...
            {
                outputText("Urta is sitting at her usual table, sipping a glass of wine and wearing a form-fitting evening gown of shimmering black.  She looks up at you, happiness filling her eyes when she notices you entering the bar.");
            }
            //(DRUNK)
            else
            {
                outputText("Urta is sitting at her usual table wearing a shimmering black dress.  She's pretty clearly sloshed judging by her bleary gaze as she looks up at you.  The front of her dress visibly tents, and she waves you over with a lecherous grin.");
            }
            return true;
        }
        ...
        if (flags[kFLAGS.TIMES_FUCKED_URTA] > 0)
        {
            ...
            if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] < 11)
            {
                outputText("Thankfully she still has her other addition trapped under her tail, for now.");
            }
            else
            {
                outputText("Her half-hard horse-cock is plainly visible to anyone who spares her a glance, barely contained by a pair of tightly woven fishnet stockings.  She's looking at you and wiggling a finger your way...");
            }
            return true;
        }
        CoC_Settings.error("");
    ...
    public function urtaBarApproach() : Void
    {
        ...
        if (flags[kFLAGS.URTA_TALKED_ABOUT_SCYLLA] == 0 && flags[kFLAGS.URTA_SCYLLA_TIMES_CAUGHT] > 0)
        {
            ...
            if (urtaLove())
            {
                outputText("with a worried expression");
            }
            else if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] >= 12)
            {
                outputText("anxiously");
            }
            else
            {
                outputText("nervously");
            }
        ...
        if (telAdre.katherineEmployment.canTalkToUrta())
        ...
        if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] >= 5 && ((player.inRut && player.hasCock()) || (player.inHeat && player.hasVagina())))
        {
            if (urtaDrunk())
            {
                urtaHeatRut.approachDrunkenUrta();
            }
            else
            {
                urtaHeatRut.approachSoberUrtaHeatRutProc();
            }
            return;
        }
        ...
        //Sworn off drinking, up till noon, or up to 8 if 'drink more'{
            ...
            else
            {
                ...
                if (player.canOviposit() && flags[kFLAGS.URTA_TIMES_EGG_PREGGED] == 0)
                {
        ...
        else
        {
            ...
            if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] < 11)
            {
                outputText("moments before you hear her horse-cock thump into the bottom of the table.  ");
            }
            else
            {
                outputText("and pets her already-growing horse-cock through her fishnet stockings.  ");
            }
            if (rand(2) == 0)
            {
            ...
            {
                outputText("She asks, \"<i>Would you like to go back to my place and help me play with my 'little' friend again?  Or maybe you could climb under the table and give me some relief?</i>\"");
            }
        }
        soberSexMenu();
    }
    ...
    {
        var preg : Bool = pregnancy.type == PregnancyStore.PREGNANCY_PLAYER;
        var luv : Bool = flags[kFLAGS.URTA_PC_LOVE_COUNTER] == 1;
        ...
        if (flags[kFLAGS.URTA_CUM_NO_CUM_DAYS] >= 5)
        {
            if (luv)
            {
                addButton(1, "Suck Off", slurpFawkesCocksForFunAndInflation);
            }
            else
            {
                addButton(1, "Hidden BJ", slurpFawkesCocksForFunAndInflation);
            }
        ...
        else
        {
            if (luv)
            {
                addButton(1, "Suck Off", blowUrtaUnderTheTableLuv);
            }
            else
            {
                addButton(1, "Hidden BJ", blowUrtaUnderTable);
            }
        ...
        if (luv)
        {
            ...
            if (!preg)
            {
                addButton(3, "Vixen & Cream", vixenAndCreamPart1).hint("Try something special! \n\nNOTE: This is very long! Don't select this if you have ADHD.").disableIf(player.isTaur(), "Not for centaurs.").disableIf(!player.hasCock(), "Req. a cock.");
            }
        }
    ...
    public function barDrunkMenu() : Void
    {
        ...
        addButton(2, "Lay Eggs", giveTheFoxSomeEggs).disableIf(!player.canOviposit(), "Req. an ovipositor.");
        addButton(3, "Spank Her", spankUrta).disableIf(!cockFits() && player.hasKeyItem("Deluxe Dildo") < 0, 
        ...
        if (flags[kFLAGS.URTA_X_RAPHAEL_HAPPENED] == 0)
        {
            ...
            {
                addButtonDisabled(8, "???", "Maybe something else would be possible if some gentleman liked and trusted you?");
            }
        }
    ...
    private function drinkUrtasBoozeLetHer() : Void
    {
        ...
        {
            outputText(biggestBreastSizeDescript());
        }
        else
        {
        ...
        {
            outputText(" and " + multiCockDescript());
        }
        ...
        {
            outputText("suitably lubricated and leaking pre");
        }
        else
        {
            outputText("suitably lubricated");
        }
        outputText(".\n\n");
        ...
        if (player.hasVagina())
        {
            outputText("against your cervix as the tip presses firmly against it");
        }
        else
        {
            outputText("into your rectum");
        }
        outputText(", unloading a thick batch of cream directly into your ");
        if (player.hasVagina())
        {
            outputText("waiting womb");
        }
        else
        {
            outputText("abused asshole");
        }
        outputText(".");
        if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < sceneHunter.adjustPregEventTimer(250, player.pregnancyType))
        {
            outputText("  Whatever's growing inside you is probably bathing in the stuff now.");
        }
        else if (player.hasVagina())
        {
            outputText("  You briefly wonder if she's capable of knocking you up, and what the baby would look like.");
        }
        else
        {
            outputText("  Your backside fills with warmth as her spunk seeps deep inside you.");
        }
        ...
        if (player.cockTotal() > 0)
        {
            ...
            if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < sceneHunter.adjustPregEventTimer(100, player.pregnancyType))
            {
                outputText("pregnancy swollen ");
            }
            outputText("belly, making you squish with each brutal thrust of your oversized lover's rod.");
        }
        else if (player.hasVagina())
        {
            outputText("Your pussy dribbles and clenches, leaking Urta's whiteness as she claims it for her own.");
        }
        else
        {
            outputText("Your " + assholeDescript() + " clamps down tightly, spasming in pleasure around the invader.");
        }
        if (player.cockTotal() > 0)
        {
            outputText("  As you cum, more and more whiteness dribbles out until a puddle forms underneath you, showing Urta just how much you enjoyed the rough, drunken fuck.\n\n");
        }
        else
        {
            outputText("\n\n");
        ...
        if (player.cockTotal() > 0)
        {
            outputText("stick it in me");
        }
        else
        {
            outputText("come back for an even bigger load");
        }
        outputText(".</i>\"\n\n");
    ...
    private function dualUrtaMasturbation() : Void
    {
        ...
        if (player.cor < 60)
        {
            outputText("You blush furiously at exposing yourself in such a way");
        }
        else
        {
            outputText("Smiling seductively, you revel in exposing yourself so suddenly");
        }
        ...
        {
            outputText("bare groin and " + assholeDescript());
        }
        outputText(".  ");
        if (player.cockTotal() > 0)
        {
            outputText("The situation gives you all the spark you need to rise to your full size, ");
            if (player.biggestCockArea() < 40)
            {
                outputText("even if Urta's member dwarfs you.");
            }
            else if (player.biggestCockArea() < 70)
            {
                outputText("showing Urta you're just as big.");
            }
            else
            {
                outputText("proving to Urta just who has the bigger burden.");
            }
            outputText("  ");
        }
        if (player.hasVagina())
        {
            ...
            {
                outputText("A trickle of wetness escapes your nether-lips, starting to puddle underneath you.");
            }
            else
            {
                outputText("Quickly engorging, your lower lips begin to part around your " + clitDescript() + ".");
            }
            outputText("  ");
        }
        ...
        if (player.cockTotal() == 1)
        {
            outputText("Your hands caress your [cock] feverishly, tweaking every ounce of pleasure from the bloated rod.  ");
        }
        if (player.cockTotal() > 1)
        {
            outputText("Your hands caress your " + multiCockDescript() + " in turn, dancing back and forth from rod to rod as you attempt to play with every available pleasure outlet.  ");
        }
        if (player.hasVagina())
        {
            outputText("Delving deeply between your legs, you press forward into the source of your feminine pleasures, touching and rubbing, caressing your swollen button as you get closer and closer to orgasm.\n\n");
        }
        if (player.gender == 0)
        {
            outputText("Delving down between your legs, rubbing along your taint before you slowly penetrate your " + assholeDescript() + ", pleasuring yourself in the only way possible.\n\n");
        }
        ...
        if (player.cockTotal() > 0)
        {
            ...
            {
                outputText("Jism pours out onto the alley floor as your body releases the copious amount of cum it can produce.  It puddles before you, running off the stone into the grass.  ");
            }
        ...
        if (player.hasVagina())
        {
            ...
            {
                outputText(" and gushing everywhere");
            }
            outputText(".  ");
        }
        else
        {
            outputText("Your sphincter clenches down tightly around your invading digit as you get off, and the rapt attention of your audience certainly helps.  ");
        }
        outputText("Sighing happily, you give your nipple a tweak as Urta finally finishes cumming, ");
        if (player.cumQ() >= 1000)
        {
            outputText("adding to the massive pool on the ground after ");
        }
        ...
        if (player.cumQ() >= 1000)
        {
            outputText("tip-toes through the spunk");
        }
        else
        {
            outputText("gets her balance and heads for the bar's backdoor");
        }
    ...
    //[FUCK HER]
    public function urtaFuckHer(afterBefriending : Bool = false) : Void
    {
        ...
        if (player.hasVagina())
        {
            sceneHunter.selectGender(dickCaller, vagAssF);
        }
        else
        {
            sceneHunter.selectGender(dickCaller, null, vagAssF);
        }
        ...
        function vagAssF() : Void
        {
            ...
            if (player.tailType == Tail.HORSE || player.tailType == Tail.DOG)
            {
                outputText("swishing your tail across her taut nipples");
            }
            else
            {
                outputText("making her wait");
            }
        ...
        var dickF : Int->Void = function(x : Int) : Void
        {
            ...
            if (y >= 0 && player.cockTotal() > 2)
            {
                outputText("a pair of your [cocks] against her pussy and asshole.  ");
            }
            else if (y >= 0)
            {
                outputText("your [cocks] against her pussy and asshole.  ");
            }
            else
            {
                outputText("your [cock " + (x + 1) + "] against her pussy.  ");
            }
            //DOES IT FITZ
            if (player.cockArea(x) < urtaCapacity())
            {
                if (y < 0)
                {
                    sceneHunter.print("You could use a second fitting one...");
                }
                ...
                if (y >= 0)
                {
                    outputText("Her backdoor was rougher going – the tightly muscled entrance held you back for a moment until Urta relaxed completely, but now the tighter hole is nearly as easy to penetrate as her pussy.  ");
                }
                ...
                if (y >= 0)
                {
                    outputText(", " + cockDescript(x) + ", and " + cockDescript(y) + ".");
                }
                else
                {
                    outputText(" and " + cockDescript(x) + ".");
                }
                ...
                if (y >= 0)
                {
                    outputText("  Your " + cockDescript(y) + " feels like it's about to explode inside her asshole when her sphincter clamps down tightly, acting like the world's tightest cock-ring.");
                }
                outputText("\n\n");
                ...
                if (y >= 0)
                {
                    outputText("and tightly clenched ass seem");
                }
                else
                {
                    outputText("seems");
                ...
                if (player.cumQ() < 250)
                {
                    ...
                    if (y >= 0)
                    {
                        outputText(" and slowly-closing pucker");
                    }
                    outputText(".");
                }
                else if (player.cumQ() < 500)
                ...
                else
                {
                    ...
                    if (y >= 0)
                    {
                        outputText("both her holes.");
                    }
                    else
                    {
                        outputText("her abused hole.");
                    }
                }
            ...
            else
            {
                ...
                if (player.cumQ() < 250)
                {
                    outputText("You join in, though your own contribution seems meager by comparison.");
                }
                else if (player.cumQ() < 500)
                {
                    outputText("You join in, and manage to match her spurt for spurt, until she's lying in a thick puddle of the stuff.");
                }
                else
                {
                    outputText("You join in, and make her orgasm look like a trickle compared to the flood of spunk you unleash onto her.  By the time the pair of you finish, she's soaked from head to toe, and lying in a nicely sized puddle of the stuff.");
                }
    ...
    //[Under Table BJ]
    @:allow(classes.scenes.nPCs)
    private function blowUrtaUnderTable() : Void
    {
        urtaSprite();
        if (!recalling)
        {
        ...
        if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11)
        {
            outputText(" and pulling down her fishnet to release the monster trapped inside it.  It snaps up immediately, bobbing before your face as it fills with Urta's booze-fueled lust.");
        }
        else
        {
            outputText(" and pushing up her tight leather skirt, smiling at the sight of her trembling balls and juicy pussy.  Her tail unfurls around the equine member and it snaps up immediately, hanging before your face.");
        }
        ...
        if (player.cor < 70)
        {
            outputText("do your best to suppress your gag reflex and shove forwards, ramming her cock as deeply into your throat as you can.");
        }
        else
        {
            outputText("shove forwards, ramming her massive cock so far down your throat you can feel it dribbling pre-cum into your belly.");
        }
        ...
        {
            outputText("dragging between your " + allBreastsDescript());
        }
        else
        {
            outputText("dragging across your chest");
        }
    ...
    //[Public Buttfucking]
    @:allow(classes.scenes.nPCs)
    private function takeUrtaInTheButtPublically() : Void
    {
        urtaSprite();
        clearOutput();
        ...
        if (player.cor < 60 && flags[kFLAGS.PC_FETISH] == 0)
        {
            outputText("You can't believe you're doing it, but you undo the lower half of your [armor].  ");
        }
        else
        {
            outputText("You happily remove the lower half of your [armor], more than a little excited at the prospect of putting on a show for the crowd.  ");
        }
        outputText("Urta casually flips up her skirt and ");
        if (flags[kFLAGS.TIMES_FUCKED_URTA] < 11)
        {
            outputText("unwinds her tail from around the beast");
        }
        else
        {
            outputText("fishes the beast out of her fishnets");
        }
        ...
        if (player.hasVagina())
        {
            ...
            {
                outputText("grow to its full size");
            }
            outputText(".  ");
            if (player.vaginas[0].vaginalWetness < VaginaClass.WETNESS_SLICK)
            {
                outputText("Moisture beads on your lips, signaling just how turned on this total violation is making you.");
            }
            else if (player.vaginas[0].vaginalWetness < VaginaClass.WETNESS_SLAVERING)
            {
                outputText("Moisture begins dripping onto Urta's thighs, signaling just how turned on this total violation is making you.");
            }
            else
            {
                outputText("Moisture puddles on the seat under Urta as your juices flood her thighs, revealing just how turned on this violation is making you.");
            }
            outputText("  ");
        }
        //(NIPPLES:
        outputText("Your " + nippleDescript(0) + "s tent up under your " + player.armorName);
        if (player.hasFuckableNipples() && player.biggestLactation() >= 1)
        {
            outputText(", leaking milky lubricants and aching to be penetrated.");
        }
        else if (player.hasFuckableNipples())
        {
            outputText(", aching to be penetrated.");
        }
        else if (player.biggestLactation() >= 1.5 && player.biggestLactation() < 2.5)
        {
            outputText(", leaking thin streams of milk that rapidly become visible to everyone around you.");
        }
        else if (player.biggestLactation() >= 2.5)
        {
            outputText(", spurting thick streams of milk that soak your [armor] and are visible to everyone in the bar.");
        }
        else
        {
            outputText(".");
        ...
        if (tooBig)
        {
            outputText("you find yourself giggling drunkenly, barely feeling any pain and arching your back like a slut for the crowd.");
        }
        else if (player.cor < 50 && flags[kFLAGS.PC_FETISH] == 0)
        {
            outputText("you find yourself whimpering, regretting the decision to climb on top of her, but aroused in spite of it.");
        }
        else
        {
            outputText("you find yourself sliding your hands under your [armor] and pinching your " + nippleDescript(0) + "s, putting on a show for the crowd.");
        }
        ...
        if (player.biggestTitSize() >= 1)
        {
            ...
            {
                outputText("dripping lubricants");
            }
            else if (player.biggestLactation() > 1)
            {
                outputText("spurting milk");
            }
            else
            {
                outputText("rubbing your tight nipples against your " + player.armorName);
            }
            outputText(".");
        }
        if (player.cockTotal() > 0)
    ...
    //[PUBLIC FACIAL]
    @:allow(classes.scenes.nPCs)
    private function getAPublicFacialFromUrta() : Void
    {
        urtaSprite();
        player.slimeFeed();
        flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY]++;
        ...
        if (player.cor < 33)
        {
            outputText("and it makes you blush crimson");
        }
        else if (player.cor < 66)
        {
            outputText("and it makes you uncomfortable but horny");
        }
        else
        {
            outputText("and it makes you randy as hell");
        }
        outputText(".\n\n");
        ...
        if (player.cor < 75)
        {
            outputText("doing your best to play your part in the scene.");
        }
        else
        {
            outputText("hoping to encourage her to absolutely soak you with cum.");
        }
        outputText("\n\nUrta looks down with a surprisingly shy expression and asks, \"<i>Could you... lick the tip please?  For me?</i>\"\n\n");
        
        if (player.cor < 50)
        {
            outputText("Hesitantly, you extend your tongue and lick the underside of her flat-tip.  ");
        }
        else
        {
            outputText("Eagerly, you lick around the outside edge of her flared tip, sticking your tongue out as far as you can to lick the underside once you finish.  ");
        }
        ...
        if (player.cor > 50)
        {
            outputText("You hope the show will be arousing enough to get them off on you as well, and with renewed determination you turn back to Urta and give her balls a squeeze.");
        }
        else
        {
            outputText("You shudder in shame, and hope that the crowd will disperse. It's humiliating enough waiting to have Urta soak you with cum.");
        }
        ...
        if (player.cor > 60)
        {
            outputText(", being sure to lick your fingers clean afterwards");
        }
        outputText(".\n\n");
        ...
        else
        {
            ...
            if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11)
            {
                outputText("stuffs herself back into her fishnet");
            }
            else
            {
                outputText("curls her tail back around her member, concealing it from view");
            }
            outputText(".");
    ...
    public function goBackToUrtasForLuvinz(forceFriend : Bool = false) : Void
    {
        urtaSprite();
        clearOutput();
        ...
        if (flags[kFLAGS.URTA_PC_LOVE_COUNTER] == 1 && !forceFriend)
        {
            ...
            if (flags[kFLAGS.URTA_PETPLAY_DONE] == 0 && urtaLove())
            {
                outputText("  <b>A new, shiny leather collar lies half-concealed under a cushion, as though its owner haphazardly tried to bury it in a rush.</b>");
            }
            ...
            if (player.cockTotal() > 0)
            {
                outputText("In her hand is another sheath, ready to gird your own equipment in a fashion similar to her own.  ");
            }
        ...
        if (player.lib > 50 || player.cor > 50)
        {
            outputText(" and are actually turned on by the thought of walking through the city with such a 'big' lover hanging off your arm");
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function goBackToUrtasForLuvinzII() : Void
    {
        ...
        if (flags[kFLAGS.PC_SEEN_URTA_SEX_TOYS] == 0)
        {
            ...
            {
                outputText("  You recognize some of the toys as Giacomo's wares, though there are a fair number that seem to be custom made, and others that are clearly living creatures.");
            }
        ...
        if (player.hasVagina() && player.cockTotal() > 0)
        {
            outputText("hard and wet as you've ever been");
        }
        else if (player.cockTotal() > 0)
        {
            outputText("hard as you've ever been");
        }
        else if (player.hasVagina())
        {
            outputText("wet as you've ever been");
        }
        else
        {
            outputText("ready as you've ever been to take such a monster");
        }
        outputText(".\n\n");
        ...
        menu();
        addButton(0, "Fuck Vagina", dudeFuckingUrtasCooch).disableIf(!player.hasCock(), "Req. a cock.");
        addButton(1, "Ride(Vaginal)", rideUrtaTenderFemale).disableIf(!player.hasVagina(), "Req. a vagina.");
        addButton(2, "Ride(Anal)", tenderTakeItUpTheAssFromUrta);
        if (!recalling)
        
        //no recalls - less trouble for me.{
            
            addButton(3, "No Condoms", condomlessUrtaInHouseSmex);
            addButton(4, "Goo (Weird)", urtaGooTesticleVoreRuinedOrgasms).disableIf(!player.isGoo() || player.skinType != Skin.GOO, "Req. goo skin and body", "???");
        }
        else
        {
            addButton(5, "Hidden BJ", blowUrtaUnderTable).hint("Suck Urta's dick until she cums! \n\nOr... wait a bit longer and see what happens?");
        }
    }
    ...
    private function rideUrtaTenderFemale() : Void
    {
        ...
        outputText("You nod ");
        if (player.cor < 50 || player.lib < 50)
        {
            outputText("sheepishly ");
        }
        else
        {
            outputText("eagerly ");
        ...
        {
            outputText("growing moistness in your loins");
        }
        else if (player.vaginas[0].vaginalWetness < VaginaClass.WETNESS_DROOLING)
        {
            outputText("sopping wetness in your loins");
        }
        else
        {
            outputText("flood of moisture leaking from your crotch");
        }
        ...
        if (player.cockTotal() > 0)
        {
            if (player.cockTotal() == 1)
            {
                outputText("  Your [cocks] is raging hard and trembling, but ignored for now.");
            }
            else
            {
                outputText("  Each of your [cocks] is raging hard and trembling, but ignored for now.");
            }
        }
        ...
        if (player.vaginalCapacity() < 60)
        {
            outputText("and you bottom out there, unable to take in any more of her equine shaft.");
        }
        else
        {
            outputText("and you keep going, sliding down along the marvelous member until your cunt-lips are rubbing her sheath against your [clit].");
        }
        outputText("\n\n");
        ...
        if (player.biggestTitSize() >= 5)
        {
            ...
            if (!player.hasFuckableNipples())
            {
                outputText("rubbing over her black-skinned buds.");
            }
            else
            {
                outputText("parting as her prominent black nipples slip inside your own.");
            }
            ...
            if (player.cockTotal() > 0)
            {
                if (player.cocks[0].cockLength > 15)
                {
                    if (player.cockTotal() == 1)
                    {
                        outputText("  Your [cocks] pushes up between the jiggling breasts, slicking them with your pre-cum as it begins to get tit-fucked by both of your trembling forms.");
                    }
                    else
                    {
                        outputText("  Each of your [cocks] pushes up between the jiggling breasts, slicking them with your pre-cum as they begin to get tit-fucked by both of your trembling forms.");
                    }
                }
            }
            outputText("\n\n");
        ...
        if (player.cockTotal() > 0)
        {
            if (player.cockTotal() == 1)
            {
                ...
                if (player.cocks[0].cockLength > 15 && player.biggestTitSize() >= 5)
                {
                    outputText("jiggling tits");
                }
                else
                {
                    outputText("bellies");
        ...
        {
            outputText("</i>\"<i>You're so good to me [name].  I wish my little pony here was fertile enough for me to put a bun in your oven, if you wanted of course.</i>\"");
        }
    ...
    {
        urtaSprite();
        var x : Float = player.cockThatFits(urtaCapacity());
        if (x < 0)
        {
            x = 0;
        }
        var y : Float = player.cockThatFits2(urtaCapacity());
        if (player.cockArea(x) > urtaCapacity())
        {
        ...
        var cocks : Float = player.cockTotal();
        if (!recalling)
        {
            flags[kFLAGS.PC_SEEN_URTA_SEX_TOYS]++;
            flags[kFLAGS.TIMES_FUCKED_URTA]++;
        ...
        if (y < 0)
        {
            sceneHunter.print("You could use another fitting one!");
        }
        ...
        if (cocks > 1 && y >= 0)
        {
            outputText("Your secondary " + cockDescript(y) + " wobbles directly behind her back-door and Urta's eyes widen a bit when she realizes you're going to double-penetrate her.  ");
        }
        if (cocks > 2)
        {
            outputText("The remainder of your [cocks] fan out, ready to rub themselves against her butt-cheeks once you push inside.  ");
        }
        ...
        if (cocks > 1 && y >= 0)
        {
            outputText("Your second " + Appearance.cockNoun(player.cocks[y].cockType) + " soon bumps against her pucker and strains against it, pushing ever-so-slowly into her tight hole as she gradually relaxes before you.  ");
        }
        ...
        if (player.hasBalls())
        {
            outputText("Your balls loudly smack into her squelching-wet cunt and the soaked backside of HER balls");
        }
        else
        {
            outputText("Your crotch slaps loudly against her squelching-wet cunt and backside");
        }
        ...
        if (cocks > 1 && y >= 0)
        {
            outputText("The intensity and pressure in her back-door is even worse, and your " + cockDescript(y) + " is nearly in pain from her asshole's tight, pulsating grip.  ");
        }
        ...
        if (cocks > 1 && y >= 0)
        {
            outputText("The " + cockDescript(y) + " you've crammed in her ass does not disappoint either, matching its brother squirt for squirt as it spends your seed with reckless abandon.  ");
        }
        if (cocks > 2)
        {
            outputText("Her butt and tail are covered with the stuff as each of your remaining [cocks] splatters her with spooge, covering her fur in white sludge.  ");
        }
        ...
        if (player.cumQ() >= 750)
        {
            outputText("  Her body is quickly bloated thanks to your body's incredible fertility, but she seems able to take it just fine.");
        }
        outputText("\n\n");
        ...
        if (player.biggestTitSize() >= 1)
        {
            outputText("your own " + breastDescript(0));
        }
        else
        {
    ...
    private function tenderTakeItUpTheAssFromUrta() : Void
    {
        ...
        if (player.analCapacity() >= 60)
        {
            outputText("You sigh and begin sliding down it, your previously-stretched backdoor easily accepting the gigantic intruder as it fills you.");
        }
        else
        {
            ...
            if (player.ass.analLooseness < 5)
            {
                ...
                {
                    outputText("<b>  Urta has taken your anal virginity.</b>");
                }
                else
                {
                    outputText("<b>  Your ass is stretched.</b>");
                }
                if (!recalling)
                {
        ...
        //Butt Stretched used to determine how long since last enlargement{
            ...
            if (!player.hasStatusEffect(StatusEffects.ButtStretched))
            {
                player.createStatusEffect(StatusEffects.ButtStretched, 0, 0, 0, 0);
            }
        ...
        if (cocks > 0)
        {
            ...
            if (player.cocks[0].cockLength >= 15)
            {
                ...
                if (player.cocks[0].cockLength >= 20)
                {
                    outputText(", even taking time to lick and suck on the head");
                }
                outputText(".");
        ...
        if (cocks == 0)
        {
            outputText("Strange pleasure spreads from your sensitive opening, and you know that somehow you're going to cum from this.  ");
        }
        else
        {
            outputText("Powerful pleasure spreads through you as it bumps and rubs your prostate. You know you'll cum from this, and soon.  ");
        }
        ...
        if (cocks > 0)
        {
            if (player.cocks[0].cockLength < 15)
            {
                outputText("Your orgasm erupts out onto her belly, splattering her fur with your sticky cum.  ");
            }
            else if (player.cocks[0].cockLength < 20)
            {
                outputText("Your orgasm erupts all over her tits, slicking them with your sticky white cum.  ");
            }
            else
            {
                outputText("Your orgasm erupts into her muzzle, filling her mouth with your sticky, white cum.  ");
            }
            outputText("  Urta groans underneath you and begins caressing and squeezing you, milking you of every drop.");
            if (player.cumQ() >= 500)
            {
                outputText("  Urta gulps down as much as she can but it begins leaking around her muzzle, and she eventually lets it pop free, giving up on swallowing it entirely.");
            }
            if (player.cumQ() >= 1000 && player.cumQ() < 1500)
            {
                outputText("  Your output easily soaks her, and she gathers it up and slops it over her nipples, pinching and pulling on them.");
            }
            if (player.cumQ() >= 1500)
            {
                outputText("  Your output soaks her and the bed with seed, leaving the two of you in a warm gooey puddle in the very center of the bed.");
            }
            outputText("\n\n");
        }
        ...
        if (player.cumQ() > 750)
        {
            outputText("  You really are a messy, " + player.mf("boy", "girl") + " aren't you?");
        }
        outputText("</i>\"\n\n");
    ...
    private function TellUrtaDAHTRUUUUF() : Void
    {
        ...
        if (flags[kFLAGS.URTA_PC_LOVE_COUNTER] == 1)
        {
            outputText("I love you for it");
        }
        else
        {
            outputText("I respect you for it");
        }
        outputText(".");
        if (!urtaOpenAboutEdryn())
        {
            ...
            if (flags[kFLAGS.URTA_PC_LOVE_COUNTER] == 1)
            {
                outputText("been together");
            }
            else
            {
                outputText("gotten intimate");
            }
    ...
    private function lietoUrtaAboutMarble() : Void
    {
        ...
        {
            outputText(" and saw the kid");
        }
        if (flags[kFLAGS.MARBLE_NURSERY_CONSTRUCTION] > 1)
        {
            outputText(" and saw the kids");
        }
    ...
    public function eatUrtaOutNomNomPussy() : Void
    {
        ...
        {
            outputText("hoping no one saw you disappear");
        }
    ...
    public function blowUrtaUnderTheTableLuv() : Void
    {
        ...
        {
            outputText("shove forwards, ramming her massive cock so far down your throat that you can feel it dribbling pre-cum into your belly.");
        }
        outputText("  Urta grunts, doing her best to muffle the noise as it escapes her lips, but you can feel her twitch and thicken inside you, beginning to drip with proof of her complete and total submission to your mouth.\n\n");
        
        outputText("You can hear chuckles from the bar's other occupants as they realize what's going on, and you blush, ");
        if (player.cor < 50 && flags[kFLAGS.PC_FETISH] <= 0)
        {
            outputText("wishing there was a tablecloth to hide your activities from prying eyes");
        }
        else
        {
            outputText("wishing you had even more eyes watching your show");
        }
    ...
    private function rideUrtasCoochLoveyDovey() : Void
    {
        ...
        if (player.cockTotal() > 0)
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("a latex sheath");
            }
            else
            {
                outputText("some latex sheaths");
            }
        ...
        if (player.vaginas[0].vaginalWetness < VaginaClass.WETNESS_WET)
        {
            outputText("growing moist");
        }
        else if (player.vaginas[0].vaginalWetness < VaginaClass.WETNESS_DROOLING)
        {
            outputText("growing wetter and wetter as you lubricate the tip of her condom");
        }
        else
        {
            outputText("soaking her condom-covered cock with your copious cunt-secretions");
        }
        ...
        if (player.vaginalCapacity() < 80)
        {
            outputText("until there is no more room for cum and it's squirting out around the base of her cock, soaking Urta and the bed with fragrant horse-cum.");
        }
        else
        {
            outputText("filling your body with a large bubble of hot horse-cum.");
        }
        ...
        if (player.cockTotal() == 1)
        {
            ...
            if (player.cumQ() >= 250)
            {
                if (player.cumQ() < 500)
                {
                    outputText("  The balloon inflates slightly, drooping down as your maleness empties its reserves.  ");
                }
                else if (player.cumQ() < 1000)
                {
                    outputText("  The balloon swells larger and larger as you pump it full, eventually wobbling around on Urta's belly as you finish filling it.");
                }
                else
                {
                    ...
                    if (player.cumQ() > 1500)
                    {
                        outputText("  Your potent seed pops the condom, splattering Urta with your seed as it explosively fails.");
                    }
                }
            }
        }
        else
        {
            ...
            if (player.cumQ() >= 250)
            {
                if (player.cumQ() < 500)
                {
                    outputText("  The balloons inflate slightly, drooping down as your malenesses empty their reserves.  ");
                }
                else if (player.cumQ() < 1000)
                {
                    outputText("  The balloons swell larger and larger as you pump them full, eventually wobbling around on Urta's belly as you finish filling them.");
                }
                else
                {
                    ...
                    if (player.cumQ() > 1500)
                    {
                        outputText("  Your potent seed pops the condoms, splattering Urta with your seed as they explosively fail.");
                    }
                }
            }
        }
    ...
    private function rideUrtaInButtAtHomeLove() : Void
    {
        ...
        if (player.hasFuckableNipples())
        {
            outputText("even slipping a finger inside their slippery depths, ");
        }
        ...
        else
        {
            ...
            if (player.hasVagina())
            {
                ...
                if (player.vaginas[0].vaginalWetness > VaginaClass.WETNESS_WET && player.vaginas[0].vaginalWetness < VaginaClass.WETNESS_SLAVERING)
                {
                    outputText("  Fluids soak Urta's crotch with lust with feminine drool, though the fox barely seems to notice.");
                }
                else if (player.vaginas[0].vaginalWetness == VaginaClass.WETNESS_SLAVERING)
                {
                    outputText("  An explosion of feminine fluids spatters Urta's crotch and bed, completely soaking her groin in your need, but the distracted fox barely notices.");
                }
            }
            if (player.cockTotal() > 0)
            {
                ...
                if (player.cumQ() >= 750 && player.cumQ() < 1250)
                {
                    outputText("  By the time you're done the balloon is the size of a basketball and looking dangerously close to bursting.");
                }
                else if (player.cumQ() >= 1250)
                {
                    ...
                    if (player.cumQ() >= 1700)
                    {
                        outputText("  You fill it fuller and fuller, until it's bigger than you thought possible.  All at once the material gives out, soaking Urta from the tits down in cum, while your [cock] continues to pump even more onto her.  So much for not making a mess!");
                    }
                }
            }
            outputText("\n\n");
        ...
        if (player.cumQ() >= 1700)
        {
            outputText("Urta sighs at the mess and laughs, \"<i>I guess we got a mess anyways, huh?</i>\"");
        }
        else
        {
            outputText("Urta sighs contentedly and laughs, \"<i>I guess I don't have much of a mess to clean for once!</i>\"");
        }
    ...
    private function oralFiestyUberExplosionUrta() : Void
    {
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            if (player.vaginas[0].vaginalWetness < VaginaClass.WETNESS_DROOLING)
            {
                outputText("equally wet");
            }
            else
            {
                outputText("sopping wet");
            }
            ...
            {
                outputText("grab hold of her cock and begin jerking her off into your mouth.  An immediate and copious gush of pre drips onto your tongue, showing you just how much the hermaphroditic fox loves the feeling.\n\n");
            }
            ...
            if (player.vaginas[0].vaginalWetness > VaginaClass.WETNESS_DROOLING)
            {
                outputText("Fluids rush from your folds, splattering her face with sex fluid.  The fox opens wide, gulping it down like someone in a desert who has just found an oasis.");
            }
            else
            {
                outputText("Wetness slowly coats her muzzle as your canal squeezes and wriggles with involuntary pleasure-spasms.  Urta never lets up, sucking and licking your " + clitDescript() + " in a rhythm that matches your own orgasmic contractions.");
            }
            if (player.cockTotal() > 0)
            {
                outputText("  ");
                ...
                if (player.cumQ() < 50)
                {
                    outputText("sputtering spunk onto Urta's neck.");
                }
                else if (player.cumQ() < 250)
                {
                    outputText("soaking Urta's neck and chest with your spooge.");
                }
                else if (player.cumQ() < 700)
                {
                    outputText("showering Urta's neck and body with spooge until her breasts are painted white and the bed has a small puddle soaking into it.");
                }
                else if (player.cumQ() < 1500)
                {
                    outputText("soaking Urta with a torrent of seed that drenches her from the neck down and totally soaks the bed.");
                }
                else
                {
                    ...
                    if (player.cumQ() > 10000)
                    {
                        outputText("  The puddle thickens as you cum until it's nearly a foot deep.");
                    }
                }
            }
            outputText("\n\n");
            ...
            if (player.cor > 50)
            {
                outputText("pours down your throat, bulging it into an obscene imprint of the horse-cock that's lodged inside.  Your eyes cross from pleasure and you slump down a bit, shoving your nose into Urta's sheath and allowing her to dump her sperm directly into your belly.  Gurgling noisily, your belly slowly expands outwards by about two inches, making you feel bloated and full.");
            }
            else
            {
                outputText("erupts into your mouth, instantly filling it with thick fox-cream.  You convulsively swallow in an attempt to keep up with it, but are quickly overwhelmed by the backed up semen-flood.  It drips from your mouth and forces open your throat, flowing directly into your belly.  Some even begins dripping out from your nose as you sway with a mixture of discomfort and pleasure atop your partner.  A few trembling squirts later you feel and look a little bloated, and your [face] is thoroughly covered in foxy-horse-jizz.");
            }
            outputText("\n\n");
        ...
        var dickF : Void->Void = function() : Void
        {
            var x : Float = player.cockThatFits(100);
            //(DUDEZILLAZ)
            ...
            if (player.hasBalls())
            {
                outputText(ballsDescript() + ".  Slithering over and around them, she bathes your sack in her spit before moving up to your " + cockDescript(x) + ".");
            }
            else
            {
            ...
            if (player.cumQ() >= 500)
            {
                ...
                if (player.cumQ() >= 1000)
                {
                    outputText("It begins dripping from her nostrils as your explosive load overwhelms her ability to swallow spunk.  ");
                }
            }
            if (player.cumQ() >= 750 && player.cumQ() < 1500)
            {
                outputText("Her belly rounds out a little from all the cum as your orgasm begins to wind down.  ");
            }
            if (player.cumQ() >= 1500)
            {
                outputText("Her belly rounds out a little before she pulls back and you spurt all over her, soaking her face and head with more spooge than she could ever handle.  ");
            }
            ...
            if (player.cor < 60)
            {
                outputText("in your mouth, forcing you to compulsively swallow if you don't want to drown in it.");
            }
            else
            {
                outputText("down your throat, directly injecting it into your belly.");
            }
    ...
    @:allow(classes.scenes.nPCs)
    private function urtaHomeLuvLuvinsMenu() : Void
    {
        clearOutput();
        urtaSprite();
        ...
        menu();
        addButton(0, "Ride Vag", rideUrtasCoochLoveyDovey).disableIf(!player.hasVagina(), "Req. a vagina.");
        addButton(1, "Ride Ass", rideUrtaInButtAtHomeLove);
        addButton(2, "69", oralFiestyUberExplosionUrta).disableIf(!player.hasCock(), "Req. a cock.");
        addButton(3, "Vag Fuck", urtasCoochiNeedsFuckedLove).disableIf(!player.hasCock(), "Req. a cock.");
        addButton(4, "FuckAndLick", lickOutUrtaAtHome).disableIf(!player.hasCock(), "Req. a cock.");
        addButton(5, "No Condoms", condomlessUrtaInHouseSmex).disableIf(player.isGenderless(), "Not for genderless.");
        addButton(6, "Goo (Weird)", urtaGooTesticleVoreRuinedOrgasms).disableIf(!player.isGoo() || player.skinType != Skin.GOO, "Req. goo skin and body", "???");
        if (flags[kFLAGS.URTA_PETPLAY_DONE] >= 0)
        {
            addButton(7, "Collar", urtaPetPlayDeletedForeverBecauseThirdProovedMeWrongAboutDice).disableIf(player.isGenderless(), "Not for genderless.");
        }
        if ((flags[kFLAGS.KATHERINE_TRAINING] & KatherineEmployment.KBIT_TRAINING_URTA_HELP) != 0)
        {
    ...
    private function urtasCoochiNeedsFuckedLove() : Void
    {
        ...
        if (player.hasSheath())
        {
            outputText("sensitive skin of your sheath rubbing her exterior pussy-lips");
        }
        else
        {
            outputText("wetness of her pussy rubbing against your groin");
        }
        ...
        if (player.cocks[x].cockType == CockTypesEnum.DOG)
        {
            ...
            {
                outputText("Her belly puffs up slightly by the time you finish, thanks to your body's copious cum production.  ");
            }
            else if (player.cumQ() < 1000)
            {
                outputText("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive " + Appearance.cockNoun(CockTypesEnum.DOG) + " stops spewing inside her.  ");
            }
            else
            {
                ...
                if (player.cumQ() < 3000)
                {
                    outputText("By the time you finish, she looks VERY pregnant and is moaning with a mixture of pleasure and discomfort.  ");
                }
                else
                {
                    ...
                    if (player.cumQ() > 5000)
                    {
                        outputText("Not content with such an easy release, your " + cockDescript(x) + " unloads the last of its cream until the bed and floor are both covered under inches of your canine cum.  ");
                    }
                }
            }
            ...
            if (player.cumQ() >= 2000)
            {
                outputText("Spunk rushes out in a torrent, splattering everywhere.  ");
            }
        ...
        else if (player.cocks[x].cockType == CockTypesEnum.HORSE)
        {
            ...
            {
                outputText("prods against Urta's cervix.");
            }
            else
            {
                outputText("seats itself deep inside Urta's passage.");
            }
            ...
            {
                outputText("Her belly slowly rises as more and more cum slides into her womb, filling it.  Runners of the stuff leak down her thighs as it escapes around your bestial cock.  ");
            }
            else if (player.cumQ() < 1000)
            {
                outputText("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your over-productive " + player.cockDescript(x) + " has finished spewing your seed inside her.  ");
            }
            else
            {
                ...
                if (player.cumQ() < 3000)
                {
                    outputText("By the time you've finished, she looks heavily pregnant and cum soaks the bed.  ");
                }
                else
                {
                    outputText("Your seemingly endless orgasm pumps more and more into her until she looks ready to give birth, and  you just keep going.  She moans in pleasure and discomfort as each blast of your horse-cum forces thick runners to squirt from between her thighs.  ");
                }
                if (player.cumQ() > 5000)
                {
                    outputText("Not content with such an easy release, your " + player.cockDescript(x) + " unloads the last of its cream until the bed and floor are covered in several inches of thick equine cum.  ");
                }
            }
        ...
        else
        {
            ...
            {
                outputText("Her belly swells larger and larger as you continue to pump your seemingly endless seed into her.  Urta groans, looking a few months pregnant by the time your overproductive " + player.cockDescript(x) + " stops spewing inside her.  ");
            }
            else
            {
                ...
                {
                    outputText("Your seemingly never-ending orgasm pumps her up until she looks on the verge of giving birth, and you just keep going.  Jism sprays out from her dripping cunt as you release a torrent of semen, splattering you, her, and the bed.  ");
                }
                if (player.cumQ() > 5000)
                {
                    outputText("Not content with such an easy release, your [cock] unloads the last of its cream until the bed and floor are both covered under inches of your gooey cum.  ");
                }
            }
    ...
    public function scyllaAndUrtaSittingInATree() : Void
    {
        ...
        else
        {
            ...
            if (player.cor < 40)
            {
                outputText("corrupted ruffians");
            }
            else
            {
                outputText("demon agents");
            }
            ...
            if (urtaLove())
            {
                outputText("weakly protesting");
            }
            else if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11)
            {
                outputText("giving you a lewd look");
            }
            else
            {
                outputText("blushing hard");
            }
            outputText(" as she cradles Scylla's head in her hands and pushes down.  She slurs, \"<i>");
            if (urtaLove())
            {
                outputText("I'm... ahhhh... s-sorry love, but we both know she's been suuuu - oh yeah, keep licking - sucking you off before, and the hungry slut just couldn't take her eyes off my cock.  Why didn't you tell me ho - OH YES!  RIGHT THERE! - h-how good she was?");
            }
            else if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] >= 11)
            {
                outputText("Ohhh, you showed up a little late... mmmhmm yes, now lick... Oh, why didn't you tell me ho - OH YES!  RIGHT THERE! - h-how good of a cock-sucker this lewd-bodied bitch was?");
            }
            else
            {
                outputText("H-hi [name]... I didn't think I'd get to see you to- Oh yeah, like that!  Mmmm... OH!  I guess since we've been intimate before you can watch...");
            }
            outputText("</i>\"\n\n");
    ...
    private function urtaAndScyllaBoningLikeBitchesSober() : Void
    {
        ...
        if (silly())
        {
            outputText("For some reason, it makes you think of mythical balloonicorns.  ");
        }
    ...
    private function fuckUrtaWhileScyllaSucksSober() : Void
    {
        ...
        {
            outputText(", clearly bulging her stomach with your sizeable cock");
        }
        ...
        if (player.hasBalls())
        {
            outputText("the front of your [sack]");
        }
        else
        {
            outputText("your [legs]");
        }
        ...
        if (silly())
        {
            outputText("a cock and a sharp place.");
        }
        else
        {
            outputText("your lusty bodies.");
        }
        ...
        if (player.cockTotal() > 1)
        {
            outputText("  The unbound portion of your package bobs in the air before coming to rest in Urta's ass-crack.  A second later, the vulpine butt is coated with sticky white icing.");
        }
        if (player.lactationQ() > 50)
        {
            outputText("  Your sensitive nipples let down, releasing their own heady flows to compliment your climactic pleasure, spraying into the gray fur relentlessly.");
        }
        ...
        if (player.cumQ() >= 500)
        {
            ...
            if (player.cumQ() >= 2000)
            {
                outputText(", rounding more and more, a taut cum-tank that jiggles with your thick seed.");
            }
            else if (player.cumQ() >= 1200)
            {
                outputText(", rounding out into a swollen cum-bank that jiggles under the weight of your seed.");
            }
            else if (player.cumQ() >= 750)
            {
                outputText(", rounding out into a gravid bump that jiggles with liquid weight.");
            }
            else
            {
                outputText(", rounding out into a nicely-sized bump.");
            }
        ...
        else if (player.tallness >= 48)
        {
            outputText("narrow middle");
        }
        else
        {
            outputText("curvy hips");
        }
        ...
        if (urtaLove())
        {
            outputText("  You give your lover a wet kiss and a fondle before you go, snickering when it makes her even longer.");
        }
        else
        {
            outputText("  You give her a quick hug and depart, smiling when you realize the brief touch made her even longer.");
        }
        knockUpUrtaChance();
        //-2 sens, -100 lust
        player.sexReward("vaginalFluids", "Dick");
    ...
    private function dockWithUrtaUnderScyllasSoberSupervision() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText("  Your [sack] swings free, bouncing off the back of [oneCock] when it thickens precipitously.");
        }
        ...
        if (!player.isBiped() && !player.isDrider())
        {
            outputText("slips along");
        }
        else
        {
            outputText("slides through");
        }
        ...
        if (silly())
        {
            outputText(".  She even mentions something about how you can be her big daddy");
        }
        ...
        if (player.hasBalls())
        {
            outputText("  Your [sack] stretches tight, totally  filled by the burgeoning testes concealed within.");
        }
        ...
        if (player.hasBalls())
        {
            outputText("  You shift to let your bloated ball-sack breathe a little better as well.");
        }
        outputText("\n\nUrta has the sense to stop and step back in alarm.  ");
        if (flags[kFLAGS.URTA_SCYLLA_BIG_DICK_TIMES_DONE] > 0)
        {
            outputText("Just like last time, she adopts a panicked expression and lunges for the door, but she moves slowly and a gooey drop of pre-cum hangs from her flare as she moves.");
        }
        else
        {
            outputText("With a panicked expression on her face, she lunges for the door, her hard cock flopping awkwardly.");
        }
        ...
        if (player.hasBalls())
        {
            outputText("  Your [balls] quiver a little when the rushing seed reaches them, and you swear you hear them slosh with the horse-spunk pouring into their own reservoirs.");
        }
        else
        {
            outputText("  Your innards gurgle slightly as the rushing seed travels through them, and you swear you can feel your prostate stretching to accommodate being suddenly stuffed with horse-spunk.");
        }
        ...
        if (player.hasBalls())
        {
            outputText(" while your enormous balls gurgle and begin to empty");
        }
    ...
    private function dockWithUrtaUnderScyllasSoberSupervision2() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText("  Your balls definitely got a little bigger...");
        }
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    private function watchTwoHotBitchesAndJerkIt() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText(" and exposing your " + vaginaDescript(0));
        }
        ...
        if (player.hasVagina())
        {
            outputText(" and touch your " + vaginaDescript(0));
        }
        ...
        if (player.cumQ() >= 250)
        {
            outputText("A particularly coherent blast of seed splatters over Urta's tits, glazing her gray fur and black nipples with a thick layer of dripping spunk.  ");
        }
        if (player.cumQ() >= 450)
        {
            outputText("Another huge gout takes Scylla straight in the face, completely soaking her with yet more salty mess that drools onto the curve of her pendulous, black-clad tits.  ");
        }
        if (player.cumQ() >= 700)
        {
            outputText("A few more big blasts of the stuff fire off, spotting the pair with fat, sticky blotches.  ");
        }
        if (player.cumQ() >= 1000)
        {
        ...
        if (flags[kFLAGS.URTA_SCYLLA_TIMES_CAUGHT] == 1)
        {
            outputText("\"<i>We need to do thish again!</i>\"");
        }
        else
        {
            outputText("\"<i>You're the besht " + player.mf("man", "woman") + " a girl could ashk for.</i>\"");
        }
        ...
        if (urtaLove())
        {
            outputText("  You give your lover's back a wistful grin and muse over how good her taste in women is as you leave.");
        }
        player.sexReward("vaginalFluids", "Dick");
        //+luv
        urtaLove(2);
    ...
    private function makeUrtaSitOnYourLapWithScylla() : Void
    {
        ...
        if (player.isNaga())
        {
            outputText(", snaking your tail around her legs");
        }
        else if (player.lowerBody == LowerBody.GOO)
        {
            outputText(", enveloping her legs with your goo");
        }
        else
        {
            outputText(", straddling your [legs] around her hips");
        }
        ...
        if (y > -1)
        {
            outputText("Your " + cockDescript(y) + " even manages to get in line with her drooling, black-lipped pussy.  ");
        }
        if ((player.cockTotal() > 1 && y == -1) || player.cockTotal() > 2)
        {
            outputText("You allow your remaining, unused dickflesh to flop against her back, leaking pre-cum into her fur.  ");
        }
        ...
        if (y != -1)
        {
            outputText(" and squeezing you tightly with her sopping cunt");
        }
        outputText(".\n\n");
        ...
        if (x != -1 && y != -1)
        {
            outputText("Oooh, and in both holes?  You naughty " + player.mf("boy", "girl") + ", you!");
        }
        else if (urtaLove())
        {
            outputText("Oooh yeah, fill me love!");
        }
        else if (rand(2) == 0)
        {
            outputText("Oooh, you know how to treat a lady!");
        }
        else
        {
            outputText("MMmm, that's nice!");
        }
        ...
        if (urtaLove() || player.cor <= 40)
        {
            outputText("kiss her full on the mouth");
        }
        else
        {
            outputText("bite her neck");
        }
        ...
        if (y != -1)
        {
            outputText("double penetration");
        }
        else
        {
            outputText("anal fuck");
        }
        ...
        if (y > -1)
        {
            outputText("  Meanwhile, your other " + Appearance.cockNoun(player.cocks[y].cockType) + " utterly fills her womb, stuffing it so completely that the fox's belly rounds out.");
        }
        if ((player.cockTotal() > 1 && y == -1) || player.cockTotal() > 2)
        {
            outputText("  A wave of the stuff splatters her back, glazing her from the shoulder-blades to the crack of her ass and nearly catching you in the chin. Maybe there is such a thing as too many cocks?");
        }
        ...
        if (urtaLove())
        {
            outputText("a contended smile on her face, and gives you a long, slow kiss that practically radiates her love for you.");
        }
        else if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] < 11)
        {
            outputText("a nervous smile on her face, but gives you a quick peck all the same.");
        }
        else
        {
            outputText("a satisfied smile on her face, and crushes her lips against yours in a passionate kiss.");
        }
        ...
        if (flags[kFLAGS.URTA_SCYLLA_TIMES_CAUGHT] <= 1)
        {
            outputText("pay for the door and ");
        }
    ...
    private function tellUrtaNoMoreScylla() : Void
    {
        ...
        if (urtaLove())
        {
            outputText("Just please, don't doubt my love for you.  ");
        }
    ...
    private function tellUrtaMoreScyllaIsFine() : Void
    {
        ...
        if (urtaLove())
        {
            outputText("Just remember that YOU'RE the one I love, not her.  ");
        }
        else
        {
            outputText("Me and her, we've been partners in the guard for a long time, but the sex is just mechanical, to sate us.  You don't have to worry about her stealing me away.  ");
        }
    ...
    private function tellUrtaToBeADrunkenHussy() : Void
    {
        ...
        if (player.cor < 33)
        {
            outputText("blush a little bit");
        }
        else if (player.cor < 66)
        {
            outputText("smile nervously");
        }
        else
        {
            outputText("grin widely");
        }
    ...
    public function chastityBeltFun(cameFromSexMenu : Bool = true) : Void
    ...
    private function QBsTalkExpack() : Void
    {
        ...
        if (urtaLove())
        {
            outputText("When you approach Urta, she smiles to see you.  However, there is no rapid swivelling in her seat to avoid banging her horse-cock against the table; evidently she's not in the mood for some sex right now.  You sit down beside her, and she happily loops her arm around you, planting a tender kiss on your lips, before almost apologetically stating, \"<i>Hey there, lover.  Want to talk for a change?</i>\" with a twinkle of good-natured mirth in her eyes.");
        }
        else
        {
            outputText("When you approach Urta, you are almost surprised at the lack of an audible 'thump'.  Seeing your confusion, the herm fox quips, \"<i>What's with the look?</i>\" while raising an eyebrow.  When you indicate that you had expected to hear her demonstrating her excitement to see you, she blushes and looks embarrassed.  \"<i>Well... you do a pretty good job of letting my little pony blow off some steam, ya know, [name]?</i>\"");
        }
    ...
    private function urtaDiscussesSelf() : Void
    {
        ...
        {
            outputText("\"<i>Well, I don't think there's that much to tell,</i>\" she says with a smile.  She idly blows a bang out of her face and exhales, \"<i>Well, lover, I'm an open book for you, what do you want to know?</i>\"");
        }
    ...
    private function urtaFertilityDiscussion() : Void
    {
        ...
        if (flags[kFLAGS.URTA_FERTILE] == 1)
        {
            outputText("  Should I stay fertile?");
        }
        else
        {
            outputText("  How about I cut loose and go fertile?  Can you imagine me with a nice, round pregnant belly and a cock virile enough to get you pregnant just from looking at it?");
        }
        if (flags[kFLAGS.URTA_PREG_EVERYBODY] == 1)
        {
            outputText("  Or did you come to ask me not to be fertile and virile around anyone else?");
        }
        else
        {
            ...
            {
                outputText("  After all, you already had a turn!  Fair's fair!");
            }
        }
    ...
    private function urtaDiscussesAlcholism() : Void
    {
        ...
        else
        {
            ...
            else if (flags[kFLAGS.URTA_DRINK_TOOGLE] == 0)
            {
    ...
    private function urtaDiscussAlcoholNoChange() : Void
    {
        ...
        //{bonus wuv points}
        if (flags[kFLAGS.DISCUSSED_URTA_ALCOHOLISM] == 0)
        {
            urtaLove(1);
        }
        flags[kFLAGS.DISCUSSED_URTA_ALCOHOLISM] = 1;
    ...
    private function urtaDiscussesEdrynRomance() : Void
    {
        ...
        if (!urtaLove())
        {
            if (!urtaOpenAboutEdryn())
            {
                ...
                if (player.hasVagina())
                {
                    outputText("Until you, she was the only woman who didn't freak out and insist it was handjobs or nothing when she saw how big I was.  ");
                }
            ...
            else
            {
                ...
                if (player.cor < 66)
                {
                    outputText("her manner is playful");
                }
                else
                {
                    outputText("her expression sobers when you make no response");
                }
                outputText(".");
        ...
        else
        {
            ...
            else
            {
                ...
                if (player.statusEffectv1(StatusEffects.Edryn) >= 5)
                {
                    outputText("  \"<i>Though I have to admit I was still a little jealous when Edryn told me you were having sex with her too.  Still, I guess that's just another thing we have in common, yeah?  A mutual taste in sexy ladies.</i>\"  She laughs, and mockingly shapes her hands through the air as if caressing Edryn's sizable boobs.");
                }
                ...
                if (flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] > 0)
                {
                    if (flags[kFLAGS.URTA_QUEST_STATUS] == 1)
                    {
                        ...
                        {
                            outputText("  \"<i>...I still can't believe that you actually get her pregnant, though.  It's just not fair that Edryn's got little hooves running around, and after everything I went through, I don't have kids of my own... yet.</i>\" She gives you a slightly predatory smile along with the last word.");
                        }
                        else
                        {
                            outputText("  \"<i>...I still can't believe that you actually got her pregnant, though.  I guess I'll have to keep you busy in the sack if we're going to keep up, huh?</i>\" She gives you a slightly predatory smile along with the last word.");
                        }
                    }
                    else
                    {
                        outputText("  \"<i>...I still can't believe that you actually got her pregnant, though. It's just not fair that Edryn's got little hooves running around, and you and I can't have any.</i>\"  Urta sighs.");
                    }
                }
    ...
    @:allow(classes.scenes.nPCs)
    private function spankUrta() : Void
    {
        clearOutput();
        ...
        if (cockFits())
        {
            var y : Float = player.cockThatFits(urtaCapacity()) + 1;
            ...
            if (player.cumQ() < 250)
            {
                outputText("you've spent your load");
            }
            else if (player.cumQ() < 1000)
            {
                outputText("filled her utterly");
            }
            else
            {
                outputText("streams of cum leak out of her ass, pooling at your [feet]");
            }
    ...
    public function giveTheFoxSomeEggs() : Void
    {
        ...
        {
            outputText("as much fun as it might be, ");
        }
        ...
        if (player.isTaur())
        {
            outputText("positioning your centaur, ");
        }
        ...
        {
            outputText("spider-goo");
        }
        else
        {
            outputText("insect-goo");
        }
        ...
        {
            outputText("mixed sexual fluids");
        }
        outputText(".");
        ...
        if (player.gender > 0)
        {
            outputText(" just as her back is from your own release");
        }
        outputText(".");
        ...
        if (player.eggs() < 20)
        {
            outputText("  Urta's stomach starts to swell as your clutch of eggs fills her once-empty womb, stretching into a noticeable bulge, though not one that she couldn't hide if she wanted.");
        }
        //[Egg Level 2:
        else if (player.eggs() < 40)
        {
            outputText("  As the eggs keep on coming, Urta's womb swells and bulges, crammed full of goo and eggs, growing larger and larger until Urta could easily pass for an expectant mother, ready to give birth any day now.");
        }
        //[Egg Level 3:
        else
        {
            outputText("  You stuff Urta with eggs until you're wondering how many either of you can hold. Her belly just keeps getting rounder and rounder even as your eggs keep coming; soon, not only does she look like a woman pregnant with multiples, her skin is visibly stretched over the eggs, giving it a distinctively lumpy look rather than the round, smooth bulge she might have if your load was smaller.");
        }
        ...
        if (player.cor < 50)
        {
            ...
            {
                outputText("even if your eggs weren't fertile, ");
            }
        ...
        if (flags[kFLAGS.URTA_EGG_FORCE_EVENT] == 0)
        {
            flags[kFLAGS.URTA_EGG_FORCE_EVENT] = 48;
        }
        player.dumpEggs();
    ...
    public function urtaChewsOutPC(newScreen : Bool = true) : Void
    {
        if (newScreen)
        {
            clearOutput();
        ...
        if (player.cor > 50)
        {
            outputText(", especially given this may be the closest she'll ever get to experiencing pregnancy");
        }
        outputText("?");
        ...
        //[Giant Bee:
        else if (player.hasPerk(PerkLib.BeeOvipositor))
        {
    ...
    private function repeatUrtaEgging() : Void
    {
        ...
        if (!urtaLove())
        {
            outputText("blatantly hanging out of her stockings and drooling pre-cum everywhere");
        }
        else
        {
            outputText("vulgarly bulging her dress and rendering it sopping wet from her arousal");
        }
    ...
    private function condomlessUrtaInHouseSmex() : Void
    {
        ...
        menu();
        addButton(0, "Peg Her Ass", pegUrtasAssMessy).disableIf(!cockFits(), "<b>You're too big to peg her ass and make her bust a nut all over herself.</b>\n\n" + nofitMsg).disableIf(!player.hasCock(), "Req. a cock.");
        addButton(1, "Fill Me Up", fillMeUpPleaseUrta).disableIf(!player.hasVagina(), "Req. a vagina.");
        addButton(2, "TailFun", nagaOnUrtaMess).disableIf(player.isGenderless(), "Not for gendereless.").disableIf(!player.isNaga(), "Requires a naga tail.");
    ...
    private function pegUrtasAssMessy() : Void
    {
        ...
        if (urtaDrunk())
        {
            outputText("giggles drunkenly, \"<i>You're sho dirty!</i>\" while spreading those fishnet-clad legs further.");
        }
        else if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] < 11)
        {
            outputText("softly begs, \"<i>Be gentle, please,</i>\" while spreading those fishnet-clad legs a little further apart.");
        }
        else
        {
            outputText("eagerly coos, \"<i>Feeling a little dirty, eh, lover?  Go on baby, you can fuck my ass anytime.</i>\"  She spreads her fishnet-clad legs so wide they almost form an unbroken, horizontal line from padded toe to padded toe, tying her soft, poofy tail around a leg to keep it out of the way.");
        }
        ...
        if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 15)
        {
            outputText("casually");
        }
        else if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 11)
        {
            outputText("confidently");
        }
        else if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 4)
        {
            outputText("a little nervously");
        }
        else
        {
            outputText("with a shaky hand");
        }
        ...
        if (player.cockArea(x) > urtaCapacity())
        {
            outputText("The aching pressure around your " + cockDescript(x) + " doubles, and you realize she simply can't take any more.  Her anus is stretched wide, dilated to the extreme to accommodate this much of you.  There's little to do but plow her with what you can until she's fountaining white.");
        }
        else if (player.hasKnot(x))
        {
            outputText("The pleasure seems to double when your knot butts up against her furry rump, but her asshole isn't ready for that just yet.  This bitch needs a little more loving before you knot her.");
        }
        else
        {
        ...
        if (player.cockArea(x) > urtaCapacity())
        {
            outputText("most of you ");
        }
        else
        {
            outputText("your entire length ");
        }
        ...
        if (player.hasBalls())
        {
            outputText(" while your [balls] slap against her fuzzy buttcheeks.");
        }
        else
        {
            outputText(" while you slap your groin against her fuzzy buttcheeks.");
        }
        ...
        if (player.hasKnot(x))
        {
            outputText("  You barely notice when your knot pops through the ebony ring, locking you inside her.  All you know is that while your strokes have gotten shorter, the pleasure has gotten even greater.");
        }
        ...
        if (player.hasKnot(x))
        {
            outputText(" only makes your knot bigger and harder.");
        }
        else
        {
            ...
            if (player.cockTotal() > 1)
            {
                outputText("  Good thing you have more.");
            }
        }
        ...
        if (player.hasKnot(x))
        {
            outputText(" with a lewd pop, freeing your knot");
        }
        ...
        if (urtaDrunk())
        {
            outputText(" and of course, alcohol");
        }
        ...
        if (player.cumQ() < 250)
        {
            outputText(", swallowing with a smile and a wink.");
        }
        else
        {
            ...
            if (player.cumQ() > 750)
            {
                outputText("  You go on to wash her face with your own white sauce.");
            }
            if (player.cumQ() >= 1200)
            {
                outputText("  Your orgasm goes on and on, coating just about everything that Urta came on with yet another layer.");
            }
            if (player.cumQ() > 3000)
            {
                ...
                if (player.cumQ() > 10000)
                {
                    outputText("first few feet of the ");
                }
        ...
        if (urtaLove())
        {
            outputText("and nuzzles against your cheek, just holding you for a few moments, lover to lover.");
        }
        else
        {
            outputText("and sheepishly pulls back.");
        }
    ...
    private function UrtaGetsPeggedCleanUp() : Void
    {
        ...
        if (player.cor >= 50)
        {
            outputText("You start to duck out, but the disapproving glare the fox gives you makes it clear that you won't be getting any more messy fun unless you stick around to clean it up this time.  Damnit.  ");
        }
    ...
    private function fillMeUpPleaseUrta() : Void
    {
        ...
        {
            sceneHunter.print("That would be more fun if you were a virgin.");
        }
        ...
        if (player.wetness() > 3)
        {
            outputText("  You're so utterly soaked thanks to this place that a steady stream of vaginal lubricant drizzles over the meaty tool below.");
        }
        if (player.hasCock())
        {
            outputText("  Hard and erect, [eachCock] stands tall and proud, but ignored.  You have other plans.");
        }
        ...
        if (player.wetness() > 3)
        {
            outputText(", though it's a drop in the bucket next to your copious moisture.  Hell, even now, rivers of your girl-goo are sliding down that spasming length, wreathing it in lady-spunk");
        }
        else
        {
            outputText(", deliciously staining your slit");
        }
        outputText(".");
        //{Virgin PCs}
        if (player.vaginas[0].virgin)
        {
            ...
            if (player.hasCock())
            {
                outputText("  Drool begins to leak from [eachCock], slicking the underside with pre-cum as the pleasure affects your maleness.");
            }
        ...
        if (flags[kFLAGS.PC_FETISH] > 0)
        {
            outputText("  An exhibitionist thrill runs through you at that, making you even wetter.");
        }
        ...
        if (player.hasCock())
        {
            outputText("  You don't even notice [eachCock] dripping on Urta's belly.");
        }
        //HERMS:
        if (player.hasCock())
        {
            ...
            if (player.cockTotal() > 1)
            {
                outputText(", followed in short order by the untouched portion of your " + multiCockDescriptLight());
            }
            ...
            if (player.cumQ() > 400)
            {
                ...
                if (player.cumQ() > 1000)
                {
                    if (player.cumQ() < 2000)
                    {
                        outputText(" and her bed");
                    }
                    else
                    {
                        outputText(", her bed, and damn near everything else");
                    }
                }
        ...
        if (player.thickness < 50 && (!player.isPregnant() || player.hasNonVisiblePregnancy()))
        {
            outputText(" but the outline on your abdomen as well");
        }
        else
        {
            outputText(" but your [vagina] as well");
        }
        ...
        {
            outputText(" and your [chest] rest heavily upon your cum-stuffed form");
        }
        outputText(".");
        if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < sceneHunter.adjustPregEventTimer(150, player.pregnancyType))
        {
            outputText("  You may have been pregnant before, but you're beyond pregnant now... with cum AND child.");
        }
        //{Goo:
        if (player.isGoo())
        {
            outputText("  Successive eruptions make you bigger, but your flexible cunt soon grows so bloated that the onrushing injections mostly squirt back out, painting Urta, the bed, and your [legs] white with cum.");
        }
        else
        {
            outputText("  Successive eruptions do little to bloat you further - your body is stretched to capacity, so the onrushing injections only squirt back out through your entrance, painting Urta, the bed, and your [legs] white with cum.");
        }
    ...
    private function lippleBondUrtaScylla() : Void
    {
        ...
        if (player.cockTotal() > 1)
        {
            outputText("  Without much warning, you suddenly feel Urta's hand move away from your own palm, the sensation of your [cock smallest] being wrapped by her fingers also assaulting you as she squeezes and tugs at your second cock affectionately.");
        }
        //[if (cocks > 2)
        if (player.cockTotal() > 2)
        {
            outputText("  The barrage of sensations is intensified when Scylla also gets a bit greedy, grabbing another tool and starting to jerk it off as well.");
        }
        ...
        if (player.cockTotal() > 1)
        {
            outputText(" or more delicately and less hungrily with a feminine hand");
        }
        outputText(".");
        ...
        if (player.sens < 29)
        {
            outputText("  You can only tell so from her expression and from Scylla moaning and gasping sharply, but it appears Urta's cock is really just about ready to shoot off cum.");
        }
        //if (sensitivity > 29)
        else
        {
            outputText("  Scylla's titflesh slightly shifts from what is apparently the expansion of Urta's cock flare and the twitching of her massive shaft.  You're sensitive enough to notice.");
        }
        //[if (sensitivity > 44)
        if (player.sens >= 44)
        {
            outputText("  In fact, you can clearly feel the pressure inside Scylla's chest increasing as Urta's huge cock expands and twitches inside her next to your own, each such twitch sending a shiver of pleasure through both you and the nun.");
        }
        ...
        if (player.cockTotal() > 1)
        {
            outputText("as well as the hands of both of them working your cocks ");
        }
        ...
        {
            outputText("  You're not making the problem any easier, in fact, your knot is halfway stuck inside her breast, surged and filled to full size right after your orgasm.");
        }
        ...
        if (player.hasKnot(player.biggestCockIndex()))
        {
            outputText(", your knot sufficiently deflated now");
        }
    ...
    private function nagaOnUrtaMess() : Void
    {
        ...
        if (player.cockTotal() == 0)
        {
            outputText("  Urta runs her fingers over and gently pushes into your now revealed female slit. \"<i>My lover's sweet, sweet pussy. The best in the whole world.</i>\"");
        }
        //if (cocks = 1)
        else if (player.cockTotal() == 1)
        {
            outputText("  \"<i>And here is my favorite cock in the whole world... Probably including mine,</i>\" she giggles and winks at you.");
        }
        //if (cocks = 2)
        else if (player.cockTotal() == 2)
        {
            outputText("  \"<i>Two delicious cocks, rather than just one...  You're trying to tell me you're the man here, aren't you, lover?</i>\" she giggles and winks at you.");
        }
        else if (player.cockTotal() == 3)
        {
            outputText("  \"<i>You want to have a spare one just in case or something?</i>\" she giggles and winks at you.");
        }
        //[if (cocks >= 4)]
        else
        {
            outputText("  \"<i>Interesting.  It looks like a bouquet.  A beautiful bouquet of cocks just for me,</i>\" she giggles and winks at you.");
        }
        ...
        if (player.hasVagina())
        {
            ...
            if (player.wetness() >= 4)
            {
                outputText("  After a few convulsions, your femcum starts to shoot out of your pussy quite violently, adding to the mess your lover is making.");
            }
        }
        //if (cocks > 0)
        if (player.cockTotal() > 0)
        {
            ...
            {
                outputText("  Her eagerness does end up giving her a swollen tummy, but she's loving and hungry for you until the end, swallowing as much as she can, as quickly as she can.");
            }
            //(if [cumExtreme = True]
            else if (player.cumQ() >= 2500)
            {
                outputText("  Of course, your excessive load proves too much for the poor vixen to handle, and soon she is not only inflated, but painted in your cum as well, the mess the two of you are making definitely nothing to be scoffed at.");
            }
        }
    ...
    private function lickOutUrtaAtHome() : Void
    {
        ...
        if (urtaDrunk())
        {
            outputText("slurs, \"<i>You want to whaaat?");
        }
        else
        {
            outputText("asks, \"<i>You want to do what?!");
        }
        ...
        if (player.cockArea(x) >= urtaCapacity() / 2)
        {
            outputText("It fits you like a lubricated, stretchy little glove, ensuring every movement will caress her folds.");
        }
        else
        {
            outputText("It's nice and roomy, allowing you full control over how you want to press on her inner folds.");
        }
        ...
        {
            outputText("   Stretching ludicrously, her cunt dilates with surprising ease.  The elastic lips of her pussy cling oh-so-tightly to your girth as they're spread beyond reason.  Her hips even spread with only a modest bit of pressure and resistance as her passage is distorted, the shape of your " + cockDescript(x) + " visible through her stretched belly as it pushes towards her tits.  You can even make out some of the larger veins.");
        }
        ...
        if (player.cor < 66)
        {
            outputText("You're wonderful");
        }
        else
        {
            outputText("You're the most amazing fuck");
        }
        outputText("</i>\"");
        ...
        if (player.cumQ() >= 500)
        {
            outputText("  Spermy juices run out of her pussy in alabaster streams thick enough to obscure the black-lips they're travelling over.");
        }
        if (player.cumQ() >= 1500)
        {
            outputText("  The fox gives a surprised grin when her belly rounds slightly, though she seems concerned at how it makes her cum-filled prize wobble.");
        }
        if (player.cumQ() >= 3000)
        {
            outputText("  Your virile pecker just won't stop, and by the time you finish ejaculating, Urta is pregnant with sperm, her belly round with its belly-button protruding.");
        }
        ...
        if (player.hasKnot(x))
        {
            outputText(" in an attempt to withdraw.  Her entrance dilates slowly as your full, dog-like knot is pressed against her from inside, the pulsating, reddish flesh glistening with the mixed sexual lubricants.  You tug and tug, rocking your [hips] slowly to loosen her up, and eventually, you manage to pop out, drawing a moan of mixed anguish and pleasure from Urta's maw.");
        }
        else
        {
            outputText(" and withdraw.");
        }
        if (player.cumQ() >= 1500)
        {
            outputText("   Cum runs out of her packed womb in a wave, spreading all over the sheets down to her knees.  She shakes slightly as the cum runs out of her, rubbing her full middle protectively.");
        }
        else
        {
            outputText("  Cum drools out of her still-gaping slit in steady trickles, puddling on the sheets below.  She groans happily.  Clearly the fox is proud to have such a productive lover.");
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function slurpFawkesCocksForFunAndInflation() : Void
    {
        clearOutput();
        ...
        if (player.pregnancyIncubation < sceneHunter.adjustPregEventTimer(100, player.pregnancyType) && player.pregnancyIncubation > 0)
        {
            outputText(" than it already was");
        }
    ...
    public function urtaAndRaphaelSurprise() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(" and [eachCock] experience a twinge of excitement");
        }
    ...
    private function raphaelAndUrtaPartTwo() : Void
    {
        ...
        {
            outputText(", grinding through your [butt]'s ass-cleavage without an ounce of hesitation");
        }
        outputText(".");
        ...
        {
            outputText("lift your tails and ");
        }
        ...
        {
            outputText(", slowly stretching your tight sphincter with constant pressure.  You're not really loose enough back there for this to be entirely comfortable, but the incredible fullness her length provides has a way of muting the pain of stretching.");
        }
        else if (player.analCapacity() < 60)
        {
            outputText(", slowly sliding into your sphincter.  You groan into your other lover's mouth as your anus is claimed by the thick horse-pole, and you do your best to relax around the butthole-stretching shaft.");
        }
        else
        {
            outputText(", easily slipping inside your well-practiced anus.  You groan into your other lover's mouth as the thick cock claims your ready asshole, and you're glad your butthole was so well prepared to handle having a hot meat pole injected into it.");
        }
        if (!recalling)
        {
            player.buttChange(60, true, true, false);
    ...
    private function urtaGooTesticleVoreRuinedOrgasms() : Void
    {
        ...
        if (!silly())
        {
            outputText("a steel rod");
        }
        else
        {
            outputText("Marethian algebra");
        }
        outputText(".");
    ...
    private function urtasRuinedOrgasmsFromGooPartII() : Void
    {
        ...
        var edrynAvailable : Bool = !SceneLib.telAdre.edryn.pregnancy.isPregnant;
        if (edrynAvailable)
        {
            outputText("set of hoofsteps");
        }
        else
        {
            outputText("pair of footsteps");
        }
        ...
        if (edrynAvailable)
        {
            outputText("recognize as Edryn's");
        }
        else if (flags[kFLAGS.MET_OLIVIA] == 0)
        {
            outputText("recognize as Olivia's");
        }
        else
        {
            outputText("don't recognize");
        }
        outputText(" shouts.");
        ...
        if (edrynAvailable)
        {
            outputText("All right, but I'm still charging you for this one");
        }
        else
        {
            outputText("Uhm, yes Captain, whatever you need");
        }
        ...
        if (edrynAvailable)
        {
            outputText("Edryn moan in pleasure");
        }
        else
        {
            outputText("Olivia shout with surprise");
        }
        ...
        if (edrynAvailable)
        {
            outputText("All you need to do is push, right?");
        }
        else
        {
            outputText("Let's just sit you back, and then all you need to do is push.");
        }
        outputText("</i>\"  Your new host grunts in discomfort");
        if (edrynAvailable)
        {
            outputText(", but obliges, sitting back on something, perhaps a bed.  You feel yourself sloshing around oddly as she lies on her back");
        }
        ...
        if (edrynAvailable)
        {
            outputText("Edryn clutching Urta's hand");
        }
        else
        {
            outputText("Olivia lying back on the bed, clutching Urta's hand");
        }
        ...
        {
            outputText("a newly dressed Olivia");
        }
    ...
    private function wearZeCollar() : Void
    {
        ...
        {
            //Second Timeif (flags[kFLAGS.URTA_PETPLAY_DONE] == 1)
            {
        ...
        if (player.tailType == Tail.NONE)
        {
            ...
            if (player.tailType == Tail.NONE && player.ass.analWetness > 0)
            {
                outputText("  Despite your embarrassment, or perhaps because of it, you find your asshole getting wet with desire, practically salivating in anticipation of that thick plug.");
            }
    ...
    private function doggyStyle() : Void
    {
        ...
        {
            outputText("plugged asshole");
        }
        outputText(", and between your thighs, ");
        if (player.hasCock())
        {
            outputText("grasping your [cock biggest]");
        }
        else
        {
            outputText("her fingers slipping ever-so-slightly into the folds of your pussy");
        }
        outputText(".");
        ...
        if (player.hasCock())
        {
            outputText("Her thumb rubs against the sensitive underside of your cock");
        }
        else
        {
            outputText("One finger slips into you, all the way down to the knuckle, and swivels around to caress your g-spot with expert accuracy");
        }
        ...
        if (player.hasCock())
        {
            outputText("[cock biggest] does the same");
        }
        else
        {
            outputText("[vagina] grows slicker");
        }
        ...
        if (player.hasCock())
        {
            outputText("She wraps her fingers around your shaft, slowly pumping up and down, taking a moment to tease your [cockHead biggest] every few strokes.");
        }
        else
        {
            outputText("She buries two fingers deep inside your cunt, followed by a third, each one twisting around to rub at your g-spot softly.");
        }
        ...
        if (player.hasCock())
        {
            outputText("jerks you off");
        }
        else
        {
            outputText("rubs you from the inside out");
        }
        ...
        if (player.hasCock())
        {
            outputText("pre-cum dribbles from your dick like a leaky faucet");
        }
        else
        {
            outputText("your folds grow even more damp, constantly drizzling fluids that run down your ass crack, pooling on the floor like a lewd lake");
        }
        if (player.hasCock())
        {
            outputText(", and to top it all off, you find your [leg] inexplicably kicking in mid-air");
        }
        outputText(".");
        ...
        if (player.tailType == Tail.NONE)
        {
            outputText(", while she grasps the plug in your ass firmly and pulls. It comes out easily with a wet <b>pop</b>, making your ass feel suddenly empty.  You guess it won't be empty for long, however");
        }
        ...
        if (player.biggestTitSize() <= 5)
        {
            outputText("resting on your folded arms");
        }
        else
        {
            outputText("resting on your enormous boobs");
        }
        ...
        {
            outputText("painfully erect member");
        }
        else
        {
            outputText("slit");
        ...
        if (player.hasCock())
        {
            outputText("[cock] slaps against your belly roughly with every passing second, smearing thick globules of precum which drip down your belly, tickling your flesh like little balls of distilled pleasure");
        }
        if (player.gender == 0)
        {
            outputText(", while your");
        ...
        if (player.hasVagina())
        {
            outputText(" unused pussy aches to be filled or touched, its lips puffy and red, sensitive to every hard slap of your foxy lover's balls");
        }
        outputText(".");
        ...
        if (player.hasCock())
        {
            ...
            if (player.cumQ() >= 500)
            {
                outputText("  It continues to jerk on its own, unloading its massive load until you're laying in a puddle of your own semen.");
            }
            if (player.hasVagina())
            {
                outputText("  It then travels along to your ");
            }
        }
        if (player.hasVagina())
        {
            outputText("[vagina], which, despite being devoid of cock, clamps down anyway, gushing with femcum, drenching your Owner's balls and legs in sticky, clear fluids.");
        }
    ...
    private function getAPetReward() : Void
    {
        ...
        if (silly())
        {
            outputText("  In her other hand is what looks to be a jar of peanut butter.  What use could that have?");
        }
        ...
        if (silly())
        {
            outputText(", though you wonder if perhaps that's what the peanut butter is for");
        }
        ...
        if (player.biggestTitSize() < 1)
        {
            outputText("onto your chest");
        }
        else
        {
            outputText("between your breasts");
        }
        ...
        if (player.hasCock())
        {
            outputText("rapidly hardening shaft.");
        }
        else
        {
            outputText("moistening slit.");
        }
        ...
        if (silly())
        {
            outputText(", while she opens the jar of peanut butter and smears a dollop across the flat head.  Is... is that supposed to be enticement?  She really is treating you like a real dog");
        }
        ...
        if (silly())
        {
            outputText(", licking all of the peanut butter off the tip");
        }
        ...
        if (silly())
        {
            outputText("  Your owner uses her fingers to smear another large dollop of peanut butter along her own shaft, giving you an almost sadistic smirk.");
        }
        ...
        if (silly())
        {
            outputText(", the peanut butter now long forgotten");
        }
        outputText(", stretching your jaw wide.  ");
        if (player.hasCock())
        {
            outputText("Your own cock remains almost painfully rock hard, bobbing awkwardly beneath you, dripping precum onto the floor and bowl.");
        }
        if (player.hasVagina())
        {
            if (player.gender == 3)
            {
                outputText("  Not to mention");
            }
            else if (player.hasVagina())
            {
                outputText("  Meanwhile");
            }
        ...
        if (player.hasCock())
        {
            outputText("jerking off your cock between two toes");
        }
        else
        {
            outputText("rubbing your little love button like mad with one soft toe");
        }
        ...
        if (player.hasCock())
        {
            outputText("The cock between your legs, meanwhile, stiffens, briefly becoming harder than you've ever felt before, before blasting the floor with jet after jet of silky white cum.");
        }
        else
        {
            outputText("Meanwhile, your vagina clamps on nothing, achingly, painfully empty.  Juices spray out onto Urta's foot and down your thighs, soaking the floor beneath you in yet more slippery femcum.");
        }
    ...
    private function urtaTakesPCOnWalkies() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("[eachCock] grows hard beneath you");
        }
        if (player.gender == 3)
        {
            outputText(" and ");
        ...
        if (player.hasVagina())
        {
            outputText("your [vagina] moistens, ready to be on display");
        }
        outputText(".");
        ...
        if (player.biggestTitSize() >= 5)
        {
            outputText("the breasts hanging from your chest and ");
        }
        outputText("your openly displayed rear end.  ");
        if (player.hasCock())
        {
            outputText("Pre-cum drizzles from the end of your painfully erect cock");
        }
        else
        {
            outputText("Sweet juices flow from your cunt, drizzling down onto the ground");
        }
    ...
    private function vixenAndCreamPart1(teased : Bool = false) : Void
    {
        clearOutput();
        ...
        if (teased)
        {
            outputText("You want to have a few drinks");
        }
        else
        {
            outputText("You're here to enjoy a few drinks and chat");
        }
        ...
        if (urtaPregs.urtaKids() > 0)
        {
            ...
            if (flags[kFLAGS.URTA_TIMES_BIRTHED] > 0 && flags[kFLAGS.URTA_TIMES_PC_BIRTHED] > 0)
            {
                outputText(" and ");
            }
    ...
    private function vixenAndCreamPart2() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText("all the while kissing down your slickened shaft.");
        }
        else
        {
            outputText("all the while pointedly squeezing your shaft.");
        }
        if (player.hasBalls())
        {
    ...
    private function vixenAndCreamPart3() : Void
    {
        ...
        if (player.cumQ() >= 250)
        {
            outputText("Taking advantage of your post-orgasmic paralysis");
        }
        else
        {
            outputText("Seizing her chance");
        }
    ...
    private function vixenAndCreamPartFinal() : Void
    {
        ...
        if (player.cumQ() < 250)
        {
            outputText("You passionately kiss each other, and then you pull out your [cock]. As you do, a flood of your spunk comes pouring out of her parted black lips, and drools down between her furry buttocks.");
        }
        else
        {
            outputText("You passionately kiss each other, and then you pull out your [cock]. As you do, a flood of your spunk comes pouring out of her parted black lips, and stream down her furry buttocks. Urta lets out a bittersweet sigh as you pull out, and dreamily looks up at you. Her delicious cream is pooled on her flat equine glans like a cock cup.");
        }
        //End of dynamic text
