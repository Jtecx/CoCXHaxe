...
class Sophie extends Harpy
{
    ...
    private function sophieKissAttack() : Void
    {
        ...
        {
            outputText("The drugged lip-gloss is starting to get to you!\n");
        }
        else if (player.lust < 0.8 * player.maxLust())
        {
            outputText("Her curvy thighs look so inviting.  You barely stop yourself before you climb in between them!\n");
        }
        else if (player.lust < 0.9 * player.maxLust())
        {
            outputText("A trickle of pre-cum leaks from [eachcock].  Sophie coos, \"<i>Why don't you give in and let mommy Sophie drain out all that juicy cum?</i>\"\n");
        }
        else if (player.lust < 1.0 * player.maxLust())
        {
            outputText(player.SMultiCockDesc() + " twitches and bounces in time with your heartbeat, practically pulling you towards Sophie's gaping, pink-linked snatch.\n");
        }
        else
        {
            outputText("So horny.  You need to copulate - no, fuck - right NOW.  Your hand touches your [cock] and you swoon, pumping your hips lewdly as you submit.\n");
        }
    }
    ...
    private function sophieHarpyBoatsPC() : Void
    {
        ...
        if (rand(2) == 0)
        {
            outputText("  She jiggles them around you pleasantly and coos, \"<i>Don't fight it baby.  Just let your body do what comes naturally.</i>\"\n");
        }
        else
        {
            outputText("  She runs her long fingernails through your hair as she whispers, \"<i>Why fight it?  I'll make you feel so good.  Just relax and play with momma Sophie's tits.</i>\"\n");
        }
    ...
    private function talonsSophie() : Void
    {
        ...
        damage = as3hx.Compat.parseInt((str + weaponAttack) - player.armorDef);
        if (damage < 0)
        {
            damage = 0;
        }
    ...
    private function batterAttackSophie() : Void
    {
        ...
        //Determine damage - str modified by enemy toughness!
        damage = as3hx.Compat.parseInt((str) - player.armorDef);
        if (damage < 0)
        {
            damage = 0;
        }
    ...
    public function new()
    {
        ...
        this.drop = new ChainedDrop().add(armors.W_ROBES, 1 / 10).elseDrop(consumables.GLDSEED);
        if (player.hasPerk(PerkLib.LuststickAdapted))
        {
            (try cast(this.drop, ChainedDrop) catch(e:Dynamic) null).add(consumables.LUSTSTK, 1 / 3);
        }
