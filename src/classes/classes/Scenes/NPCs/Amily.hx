...
class Amily extends Monster
{
    ...
    {
        if (!hasStatusEffect(StatusEffects.Concentration) && rand(4) == 0)
        {
            amilyConcentration();
        }
        else if (rand(3) == 0)
    ...
    public function amilyAttack() : Void
    {
        ...
        else if (damage <= 0)
        {
            ...
            {
                outputText("Your [armor] absorb and deflect every " + weaponVerb + " from " + a + short + ".");
            }
            else
            {
    ...
    {
        var dodged : Float = player.getEvasionRoll() + player.getEvasionRoll();
        var damage : Float = 0;
        //return to combat menu when finished
        doNext(EventParser.playerMenu);
        //Get hit!
        ...
        //Determine damage - str modified by enemy toughness!{
            ...
            else
            {
                if (dodged > 0)
                {
                    outputText("Amily dashes at you and quickly slashes you twice; you manage to avoid the first blow, but the second one hits home, cutting you");
                }
                else
                {
                    outputText("Amily dashes at you and slashes at you twice in the time it would take most to throw a single blow");
                }
                outputText("! ");
                damage = player.takePhysDamage(damage, true);
            }
        ...
        {
            outputText("Amily dashes at you and quickly slashes you twice, but you quickly sidestep her first blow and jump back to avoid any follow-ups.");
        }
    }
    ...
    private function amilyDartGo() : Void
    {
        ...
        else
        {
            ...
            //Set status
            var ase : AmilyVenomDebuff = try cast(player.createOrFindStatusEffect(StatusEffects.AmilyVenom), AmilyVenomDebuff) catch(e:Dynamic) null;
            ase.increase();
        }
    }
