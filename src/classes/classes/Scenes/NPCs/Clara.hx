...
class Clara extends Monster
{
    ...
    private function claraDrugAttack() : Void
    {
        var temp2 : Float = rand(2);
        var color : String = "";
        if (temp2 == 0)
        {
            color = "red";
        ...
        else
        {
            ...
            if (color == "red")
            {
                ...
                {
                    player.createStatusEffect(StatusEffects.TemporaryHeat, 0, 5, 0, 0);
                }
            }
            //Increase fatigue
            if (color == "black")
    ...
    private function claraTeaseAttack() : Void
    //[cocked PCs only]
    {
        
        if (rand(3) == 0)
        {
            outputText("Clara hesitates, then lifts up her dress and shows you her womanhood.  Then she slowly utters, \"<i>You know, I’m still a virgin.  You’d be the first thing to ever enter inside this hole, something that Marble never could have offered you.</i>\"  What would it be like, you wonder for a moment, before catching yourself and trying to focus back on the fight.");
        }
        else if (rand(2) == 0)
        {
            outputText("Clara seems to relax for a moment and bounces her breasts in her hands.  \"<i>Come on, you know how good it is to drink cow-girl milk, just give up!</i>\" she coos.  Despite yourself, you can’t help but remember what it was like, and find yourself becoming aroused.");
        }
        else
        {
            outputText("Instead of attacking, Clara runs her hands up and down her body, emphasizing all the curves it has.  \"<i>You were made to be the milk slave of this, stop fighting it!</i>\" she says almost exasperated.  Even so, you find your gaze lingering on those curves against your will.");
        }
        outputText("\n");
    ...
    override private function performCombatAction() : Void
    {
        ...
        {
            var actions : Array<Dynamic> = [eAttack, claraDrugAttack, claraTeaseAttack, claraCastsBlind];
            var action : Int = rand(actions.length);
            trace("ACTION SELECTED: " + action);
            actions[action]();
        }
        if (!player.hasStatusEffect(StatusEffects.ClaraCombatRounds))
        {
            player.createStatusEffect(StatusEffects.ClaraCombatRounds, 1, 0, 0, 0);
        }
        else
        {
            player.addStatusValue(StatusEffects.ClaraCombatRounds, 1, 1);
        }
        
        //Bonus damage if not in camp
        if (HP > 0 && lust < maxLust() && !player.hasStatusEffect(StatusEffects.ClaraFoughtInCamp))
        {
    ...
    //PC wins via turn count
    {
        ...
        else
        {
            ...
            if (HP <= 0)
            {
                outputText("The pissed off cowgirl finally collapses to the ground.  She tries to stand up again, but finds that she can’t.  \"<i>Noo!</i>\" she cries out in frustration, \"<i>You were the perfect slave!  We were meant to be toogether!</i>\"\n\n");
            }
            //PC wins via lust
            else
            {
                outputText("The fury and anger finally give out to the overwhelming lust that you’ve help Clara feel.  She can’t fight anymore, and falls onto her backside.  She starts feeling herself up, and desperately asks you to fuck her.\n\n");
            }
        }
