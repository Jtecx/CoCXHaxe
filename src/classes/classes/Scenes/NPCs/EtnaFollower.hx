...
class EtnaFollower extends NPCAwareContent implements TimeAwareInterface implements SaveableState
{
    
    public static var EtnaHunting : Bool;
    public static var EtnaInfidelity : Int;  //0 = not yet happened, 1 = pc accept, 2 = pc reject, 3 = pc feeds her  
    public static var EtnaFertile : Bool;
    public static var EtnaJumpedToday : Bool;
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            EtnaHunting = Reflect.field(o, "EtnaHunting");
            EtnaInfidelity = Reflect.field(o, "EtnaInfidelity");
            EtnaFertile = Reflect.field(o, "EtnaFertile");
            EtnaJumpedToday = Reflect.field(o, "EtnaJumpedToday");
        }
        else
        {
            resetState();
    ...
    public function timeChange() : Bool
    {
        ...
        if (pregnancy.isPregnant && etnaPregUpdate())
        {
            needNext = true;
        }
    ...
    public function etnaAffection(changes : Float = 0) : Float
    {
        flags[kFLAGS.ETNA_AFFECTION] += changes;
        if (flags[kFLAGS.ETNA_AFFECTION] > 100)
        {
            flags[kFLAGS.ETNA_AFFECTION] = 100;
        }
    ...
    public function repeatEnc() : Void
    {
        ...
        if (player.isRace(Races.MANTICORE, 1, false))
        {
            ...
            if (player.hasStatusEffect(StatusEffects.WildManticore))
            {
    ...
    public function etnaRapesPlayer2() : Void
    {
        ...
        function dickF() : Void
        {
            ...
            if (player.hasStatusEffect(StatusEffects.WildManticore))
            {
                player.removeStatusEffect(StatusEffects.WildManticore);
            }
        ...
        var vagF : Void->Void = function() : Void
        {
            if (flags[kFLAGS.ETNA_FEMALE_WIN_PUSSYTAIL_PLAY] < 2)
            {
                ...
                if (silly())
                {
                    outputText(" Well again the crazy god who created this dimension sure had a futa furry fetish.");
                }
                outputText("\n\n");
            ...
            if (flags[kFLAGS.ETNA_FEMALE_WIN_PUSSYTAIL_PLAY] < 1)
            {
                flags[kFLAGS.ETNA_FEMALE_WIN_PUSSYTAIL_PLAY] = 2;
            }
            if (flags[kFLAGS.ETNA_FEMALE_WIN_PUSSYTAIL_PLAY] == 1)
            {
                flags[kFLAGS.ETNA_FEMALE_WIN_PUSSYTAIL_PLAY] = 3;
            }
            if (player.hasStatusEffect(StatusEffects.WildManticore))
            {
                player.removeStatusEffect(StatusEffects.WildManticore);
    ...
    public function etnaRapeIntro() : Void
    {
        ...
        {
            outputText("too horny to fight back");
        }
    ...
    public function etnaRapeYesF2() : Void
    {
        ...
        if (flags[kFLAGS.ETNA_FEMALE_WIN_PUSSYTAIL_PLAY] < 1)
        {
            outputText("Her tail tip suddenly opens to reveal what looks like a dripping pussy and to drive her even crazier you take it upon yourself to insert your entire fist inside. ");
        }
        else
        {
        ...
        if (flags[kFLAGS.ETNA_FEMALE_WIN_PUSSYTAIL_PLAY] < 1)
        {
            flags[kFLAGS.ETNA_FEMALE_WIN_PUSSYTAIL_PLAY] = 1;
        }
        if (flags[kFLAGS.ETNA_FEMALE_WIN_PUSSYTAIL_PLAY] == 2)
        {
            flags[kFLAGS.ETNA_FEMALE_WIN_PUSSYTAIL_PLAY] = 3;
    ...
    public function EtnaFillHerUp() : Void
    {
        ...
        {
            sceneHunter.print("More for Liliraunes!");
        }
        if (flags[kFLAGS.ETNA_TALKED_ABOUT_HER] >= 1 && !player.hasStatusEffect(StatusEffects.WildManticore))
        {
            outputText("Etna");
        ...
        if (flags[kFLAGS.ETNA_TALKED_ABOUT_HER] >= 1 && !player.hasStatusEffect(StatusEffects.WildManticore))
        {
            outputText("\"<i>Hey Etna</i>");
        }
        else
        {
            outputText("\"<i>Hey kitty</i>");
        }
        ...
        if (player.isAlraune())
        {
            outputText(", so aren't you, in theory, bathing in a giant sexual organ? The thought only arouses you all the more");
        }
        ...
        if (player.isLiliraune())
        {
            outputText("s groping each other boobs with both hands firmly and nectar runs down your tits,");
        }
    ...
    public function etnaRape3rdWin() : Void
    {
        ...
        if (flags[kFLAGS.ETNA_TALKED_ABOUT_HER] < 1)
        {
            flags[kFLAGS.ETNA_TALKED_ABOUT_HER] = 1;
        }
        menu();
        if (player.lust >= 33 && player.hasCock())
        {
            addButton(0, "Yes (M)", etnaRapeYesM);
        }
        if (player.lust >= 33 && player.hasVagina())
        {
            addButton(1, "Yes (F)", etnaRapeYesF);
        }
    ...
    public function etnaReady2Come2Camp() : Void
    {
        ...
        if (EtnaInfidelity == 2)
        {
            addButton(0, "Yes", etnaRapesYesCome2Camp).disableIf(player.cumQ() < 2000, "You dont have enough cum to satisfy her.");
        }
        else
        {
    ...
    private function etnaRapesYesCome2Camp() : Void
    {
        ...
        else if (player.hasVagina())
        {
            ...
            if (flags[kFLAGS.ETNA_FEMALE_WIN_PUSSYTAIL_PLAY] < 1)
            {
                flags[kFLAGS.ETNA_FEMALE_WIN_PUSSYTAIL_PLAY]++;
            }
        }
        etnaAffection(30);
        player.sexReward("vaginalFluids");
    ...
    private function etnaCome2Camp() : Void
    {
        ...
        {
            outputText("Rathazul almost drops a vial of purple liquid on the ground as he notices the manticore. \"<i>Is this what I think it is? Manticore venom is extremely potent. I could use it in alchemical concoctions, nice find! Just make sure to keep her in check...</i>\"\n\n");
        }
    ...
    public function etnaRapeYandere() : Void
    {
        ...
        if (EtnaInfidelity == 2)
        {
            outputText(" and even going so far as to confess your undying love for me");
        }
        ...
        if (player.hasCock())
        {
            outputText("as she prepares to feast");
        }
        else
        {
            outputText("at the mere thought of finally having sex with you");
        }
    ...
    public function etnaRapeYandereBadEnd() : Void
    {
        ...
        {
            outputText("Marble comes looking for you before long, but, instead of freeing you, she decides to help the manticore hold you there for your own good. ");
        }
    ...
    public function etnaRapeIntro2() : Void
    {
        ...
        {
            if (flags[kFLAGS.ETNA_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.ETNA_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.ETNA_DEFEATS_COUNTER] = 1;
            }
            if (flags[kFLAGS.ETNA_DEFEATS_COUNTER] == 5 && flags[kFLAGS.ETNA_LVL_UP] < 1)
            {
                ...
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 5);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers1, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 5);
                }
            ...
            if (flags[kFLAGS.ETNA_DEFEATS_COUNTER] == 6 && flags[kFLAGS.ETNA_LVL_UP] == 1)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers1))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 6);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers1, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 6);
                }
                flags[kFLAGS.ETNA_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.ETNA_DEFEATS_COUNTER] == 7 && flags[kFLAGS.ETNA_LVL_UP] == 2)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers1))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 7);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers1, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 7);
                }
                flags[kFLAGS.ETNA_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.ETNA_DEFEATS_COUNTER] == 8 && flags[kFLAGS.ETNA_LVL_UP] == 3)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers1))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 8);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers1, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 8);
                }
                flags[kFLAGS.ETNA_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.ETNA_DEFEATS_COUNTER] == 9 && flags[kFLAGS.ETNA_LVL_UP] == 4)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers1))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 9);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers1, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 9);
                }
                flags[kFLAGS.ETNA_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.ETNA_DEFEATS_COUNTER] == 10 && flags[kFLAGS.ETNA_LVL_UP] == 5)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers1))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 10);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers1, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 10);
                }
                flags[kFLAGS.ETNA_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.ETNA_DEFEATS_COUNTER] == 11 && flags[kFLAGS.ETNA_LVL_UP] == 6)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers1))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 11);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers1, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 11);
                }
                flags[kFLAGS.ETNA_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.ETNA_DEFEATS_COUNTER] == 12 && flags[kFLAGS.ETNA_LVL_UP] == 7)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers1))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 12);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers1, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 12);
                }
                flags[kFLAGS.ETNA_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.ETNA_DEFEATS_COUNTER] == 13 && flags[kFLAGS.ETNA_LVL_UP] == 8)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers1))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 13);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers1, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 13);
                }
                flags[kFLAGS.ETNA_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.ETNA_DEFEATS_COUNTER] == 14 && flags[kFLAGS.ETNA_LVL_UP] == 9)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers1))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 14);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers1, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 14);
                }
                flags[kFLAGS.ETNA_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.ETNA_DEFEATS_COUNTER] == 15 && flags[kFLAGS.ETNA_LVL_UP] == 10)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers1))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers1, 4, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 15);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers1, 0, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 15);
                }
                flags[kFLAGS.ETNA_DEFEATS_COUNTER] = 0;
    ...
    public function etnaCampMenu() : Void
    {
        ...
        if (flags[kFLAGS.ETNA_DAILY_VENOM_VIAL] > 0)
        {
            addButtonDisabled(3, "Req. Venom", "You already asked her for a vial today.");
        }
        else
        {
            addButton(3, "Req. Venom", etnaDailyVenomVial).hint("Ask Etna for a vial of her venom.");
        }
        addButton(4, "Spar", etnaSparsWithPC).hint("Ask Etna for a mock battle with sex for the winner.").disableIf(flags[kFLAGS.PLAYER_COMPANION_1] == "Etna" || flags[kFLAGS.PLAYER_COMPANION_2] == "Etna", "You can't fight against her as long she's in your team.").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, "You need a good sparring ring for that.");
        if (player.hasPerk(PerkLib.BasicLeadership))
        {
            addButton(5, "Team", etnaHenchmanOption);
        }
        else
        {
        ...
        {
            addButton(6, "Marriage", etnaMarriage).disableIf(player.isGenderless(), "Come on, you're genderless!").disableIf(flags[kFLAGS.MICHIKO_TALK_MARRIAGE] == 0, "You don't even know if it's possible in Mareth. Try talking to random people in random river towns... maybe you'll find out?");
        }
        if (flags[kFLAGS.SLEEP_WITH] != "Etna")
        {
            addButton(7, "Sleep With", etnaSleepToggle).hint("Spend your nights with Etna.");
        }
        else
        {
            addButton(7, "Sleep Alone", etnaSleepToggle).hint("Stop sleeping with Etna.");
        }
    ...
    public function etnaMarriage() : Void
    {
        ...
        else if (!SceneLib.templeofdivine.canMarry())
        {
            if (!player.hasItem(jewelries.ENDGRNG, 1))
            {
                outputText("realize however that to properly propose to her, you would need an engagement ring, preferably a ruby one also ");
            }
    ...
    public function etnaMarriageYes2() : Void
    {
        ...
        if (camp.loversCount() > 0 || camp.followersCount() > 0)
        {
            outputText(". All of your friends came to attend");
        }
        if (camp.loversCount() > 0)
        {
            outputText(" which is not so surprising considering the strength of your relationship with them");
        }
        outputText(".\n\n");
    ...
    public function etnaHenchmanOption() : Void
    {
        ...
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "Etna" || flags[kFLAGS.PLAYER_COMPANION_3] == "Etna")
            {
                addButtonDisabled(0, "Team (1)", "You already have Etna accompany you.");
            }
            else
            {
                addButton(0, "Team (1)", etnaHenchmanOption2, 1).hint("Ask Etna to join you in adventures outside camp.");
            }
        }
        else if (flags[kFLAGS.PLAYER_COMPANION_1] == "Etna")
        {
            addButton(5, "Team (1)", etnaHenchmanOption2, 21).hint("Ask Etna to stay in camp.");
        }
        else
        {
            addButtonDisabled(5, "Team (1)", "You already have other henchman accompany you as first party member. Ask him/her to stay at camp before you talk with Etna about accompaning you as first party member.");
        }
        if (player.hasPerk(PerkLib.IntermediateLeadership))
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "")
            {
                if (flags[kFLAGS.PLAYER_COMPANION_1] == "Etna" || flags[kFLAGS.PLAYER_COMPANION_3] == "Etna")
                {
                    addButtonDisabled(1, "Team (2)", "You already have Etna accompany you.");
                }
                else
                {
                    addButton(1, "Team (2)", etnaHenchmanOption2, 2).hint("Ask Etna to join you in adventures outside camp.");
                }
            }
            else if (flags[kFLAGS.PLAYER_COMPANION_2] == "Etna")
            {
                addButton(6, "Team (2)", etnaHenchmanOption2, 22).hint("Ask Etna to stay in camp.");
            }
            else
            {
                addButtonDisabled(6, "Team (2)", "You already have other henchman accompany you as second party member. Ask him/her to stay at camp before you talk with Etna about accompaning you as second party member.");
            }
        }
        else
        {
    ...
    public function etnaHenchmanOption2(slot : Float = 1) : Void
    {
        clearOutput();
        if (slot < 21)
        {
            ...
            var strEtna : Float = 100;
            var libEtna : Float = 170;
            ...
            if (slot == 2)
            {
                flags[kFLAGS.PLAYER_COMPANION_2] = "Etna";
            }
            if (slot == 1)
            {
    ...
    public function etnaSexMenu() : Void
    {
        ...
        addButton(6, "TailExplor", tailExploration).hint("Play with her tail using your goo body!", "Tail Exploration").disableIf(!player.isGoo(), "Req. goo body.");
        addButton(14, "Back", etnaCampMenu);
    }
    ...
    public function etnaPussyOpera2() : Void
    {
        ...
        player.sexReward("vaginalFluids", "Dick");
        if (!pregnancy.isPregnant && EtnaFertile && !followerEtnaKid())
        {
            etnaKnockupAttempt();
        }
        if (flags[kFLAGS.ETNA_TALKED_ABOUT_HER] < 3)
        {
            flags[kFLAGS.ETNA_TALKED_ABOUT_HER] = 3;
        }
    }
    ...
    public function etnaShareDrink() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText(" the [pussy] between your legs wets the ground as a new cargo of cum fills your tail cavity");
        }
    ...
    public function mishapsLunaEtna() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.CampLunaMishaps2, 1, 1);
        }
        else
        {
            player.createStatusEffect(StatusEffects.CampLunaMishaps2, 1, 0, 0, 0);
        }
    ...
    public function sleepWithEtna() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("She gently unwinds and plugs her tail to [onecock] penis making you gasp in pleasure as she slowly begins to massage you. Well, you won’t have to be afraid of any wet dream soiling the bed with Etna around. ");
        }
    ...
    public function etnaInfidelityEncounter() : Void
    {
        ...
        var acceptInfidelity : Void->Void = function() : Void
        {
            clearOutput();
        ...
        var breakOffInfidelity : Void->Void = function() : Void
        {
            clearOutput();
        ...
        var satisfyHerInfidelity : Void->Void = function() : Void
        {
            clearOutput();
    ...
    private function etnaJumpsPCinCamp() : Void
    {
        ...
        addButton(8, "TailExplor", tailExploration).hint("Play with her tail using your goo body!", "Tail Exploration").disableIf(!player.isGoo(), "Req. goo body.");
    }
    ...
    private function etnaPregUpdate() : Bool
    {
    ...
    private function etnaPregApproach() : Bool
    {
        var _sw14_ = (pregnancy.event);        
