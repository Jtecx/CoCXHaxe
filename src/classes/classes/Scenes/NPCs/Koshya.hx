...
class Koshya extends Monster
{
    ...
    {
        if (!hasStatusEffect(StatusEffects.ATranscendentSoulField) && soulforce >= 20)
        {
            createStatusEffect(StatusEffects.ATranscendentSoulField, 20, 20, 0, 0);
        }
    ...
    override private function get_long() : String
    {
        ...
        if (hasStatusEffect(StatusEffects.TrueFormAngel))
        {
            str += "You're fighting pale blue skinned women with snow white medium long hair. Using her six pairs of snow-white wings she constantly stays airborne. On wings or her body you can see many closed eyeballs. Runic bracelets on her hands looks like they been partialy cracked. Around her rotate twin halo with more eyes on them. A powerful pure aura emanates from her.";
        }
        else
        {
            str += "You're fighting pale blue skinned women with snow white medium long hair. Using her snow-white wings she constantly stays airborne. On her hands you can see pair of runic bracelets. Above her head you see halo.";
        }
        if (hasStatusEffect(StatusEffects.ATranscendentSoulField))
        {
    ...
    public function new()
    {
        ...
        this.drop = new WeightedDrop().add(null, 2).add(useables.SRESIDUE, 1);
