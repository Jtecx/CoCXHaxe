...
class EmberScene extends NPCAwareContent implements TimeAwareInterface
{
    ...
    public function timeChange() : Bool
    {
        ...
        if (player.statusEffectv1(StatusEffects.EmberFuckCooldown) > 0)
        {
            ...
            if (player.statusEffectv1(StatusEffects.EmberFuckCooldown) < 1)
            {
                player.removeStatusEffect(StatusEffects.EmberFuckCooldown);
            }
        }
        //Ember napping
        if (player.hasStatusEffect(StatusEffects.EmberNapping))
        {
            ...
            if (player.statusEffectv1(StatusEffects.EmberNapping) <= 0)
            {
                player.removeStatusEffect(StatusEffects.EmberNapping);
            }
        }
        ...
        if (model.time.hours > 23)
        {
            ...
            {
                flags[kFLAGS.EMBER_BITCHES_ABOUT_PREGNANT_PC] = 0;
            }
    ...
    public function timeChangeLarge() : Bool
    {
        ...
        //Ember get's a whiff of fuckscent and knocks up PC!{
            ...
            if (hasCock() && player.hasVagina() && player.inHeat && player.canGetPregnant() && rand(10) == 0 || hasVagina() && player.hasCock() && player.inRut && !pregnancy.isPregnant && rand(10) == 0)
            {
    ...
    public function emberAffection(changes : Float = 0) : Float
    {
        flags[kFLAGS.EMBER_AFFECTION] += changes;
        if (flags[kFLAGS.EMBER_AFFECTION] > 100)
        {
            flags[kFLAGS.EMBER_AFFECTION] = 100;
        }
        else if (flags[kFLAGS.EMBER_AFFECTION] < 0)
        {
            flags[kFLAGS.EMBER_AFFECTION] = 0;
        }
        return flags[kFLAGS.EMBER_AFFECTION];
    }
    
    private function emberCorruption(changes : Float = 0) : Float
    {
        flags[kFLAGS.EMBER_COR] += changes;
        if (flags[kFLAGS.EMBER_COR] > 100)
        {
            flags[kFLAGS.EMBER_COR] = 100;
        }
        else if (flags[kFLAGS.EMBER_COR] < -100)
        {
            flags[kFLAGS.EMBER_COR] = -100;
        }
    ...
    override public function followerEmber() : Bool
    {
    ...
    {
        return as3hx.Compat.parseInt(flags[kFLAGS.EMBER_CHILDREN_MALES] + flags[kFLAGS.EMBER_CHILDREN_FEMALES] + flags[kFLAGS.EMBER_CHILDREN_HERMS]);
    }
    
    private function internalDick() : Bool
    {
    ...
    public function emberCampMenu() : Void
    {
        ...
        {
            outputText("Ember sighs as you approach, and doesn't even look you in the eye before speaking.  \"<i>What do you want?</i>\"");
        }
        //Moderate Affection:
        else if (emberAffection() <= 75)
        {
            outputText("Ember fidgets as [ember eir] tail starts to sway from side to side, [ember ey] looks at you and asks, \"<i>What is it?</i>\"");
        }
        else
        {
            outputText("Ember's eyes light up as you close in on [ember em], and [ember ey] smiles nervously.  \"<i>Y-Yes?</i>\"");
        }
        //OPTIONS HERE
        menu();
        ...
        if (flags[kFLAGS.EMBER_MILK] > 0 || (player.hasItem(consumables.LACTAID, 1) && flags[kFLAGS.EMBER_AFFECTION] >= 75))
        {
            addButton(3, "Drink Milk", getMilkFromEmber).hint("Ask Ember if [ember ey]'s willing to let you drink [ember eir] milk." + ((flags[kFLAGS.EMBER_MILK] > 0) ? "" : "\n\nThis will cost you 1 Lactaid each time you want to suckle milk.") + "");
        }
        if ((flags[kFLAGS.EMBER_OVIPOSITION] > 0 || (player.hasItem(consumables.OVIELIX, 1) && flags[kFLAGS.EMBER_AFFECTION] >= 75)) && hasVagina() && !pregnancy.isPregnant)
        {
            addButton(4, "Get Egg", emberIsAnEggFactory).hint("Ask Ember if [ember ey]'s willing to lay an unfertilized egg for you." + ((flags[kFLAGS.EMBER_OVIPOSITION] > 0) ? "" : "\n\nThis will cost you 1 Ovi Elixir each time you want [ember em] to lay an unfertilized egg.") + "");
        }
        addButton(5, "Sex", emberSexMenu).hint("Get into a sex session with Ember.").disableIf(player.lust < 33, "Not aroused enough!");
        addButton(6, "Spar", decideToSparEmbra).hint("Do a quick battle with Ember!").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, "You need a good sparring ring for that.");
        if (model.time.hours >= 21 || model.time.hours < 5)
        {
            if (flags[kFLAGS.EMBER_AFFECTION] < 75)
            {
                addButton(7, "Sleep With?", sleepWithEmber).hint("Try to spend the night with Ember.");
            }
            else
            {
                addButton(7, "Sleep With", sleepWithEmber).hint("Spend the night with Ember.");
            }
        }
        if (flags[kFLAGS.SLEEP_WITH] == "Ember")
        {
            addButton(8, "NoSleepWith", noSleepWith);
        }
    ...
    private function emberSexMenu(output : Bool = true) : Void
    {
        if (output)
        {
            clearOutput();
            ...
            if (hasCock())
            {
                ...
                if (flags[kFLAGS.EMBER_INTERNAL_DICK] == 1 || flags[kFLAGS.EMBER_ROUNDFACE] == 0)
                {
                    outputText("is poking out of [ember eir] slit");
                }
                else
                {
                    outputText("is starting to swell with blood");
                }
                outputText(".  ");
            }
            if (hasVagina())
            {
                outputText("You can see tiny rivulets of moisture starting to run down Ember's inner thighs as they rub together, barely hiding her precious treasure from your hungry eyes.  ");
            }
        ...
        if (hasCock())
        {
            ...
            addButton(11, "Get Penetrated", getPenetratedByEmberLastSexSceneWoooo).hint("Ask Ember if [ember ey]'s willing to penetrate you vaginally with that cock of [ember eirs].").disableIf(!player.hasVagina(), "Req. a vagina!");
        }
        if (hasVagina())
        {
            addButton(2, "Eat Her Out", slurpDraggieCunnies).hint("Get a taste of Ember vagina!");
            addButton(7, "Penetrate Her", penetrateEmbrah).hint("Penetrate Ember vaginally!").disableIf(!player.hasCock(), "Req. a cock!");
        }
        addButton(0, "Get Blown", stickDickInKnifeDrawer).hint("Ask Ember if [ember ey]'s willing to suck you off.").disableIf(!player.hasCock(), "Req. a cock!");
        addButton(5, "Pitch Anal", stickItInEmbersButt).hint("Penetrate Ember anally with your cock.").disableIf(!player.hasCock(), "Req. a cock!");
        addButton(10, "Get Eaten Out", getEatenOutByEmbra).hint("Ask Ember if [ember ey]'s willing to get a taste of your vagina.").disableIf(!player.hasVagina(), "Req. a vagina!");
        addButton(3, "LustyFuck", highAffectionEmberLustFuck).disableIf(!player.hasCock() || !(player.hasItem(consumables.L_DRAFT) || player.lib >= 50 || player.minLust() >= 40), "Req. a cock AND high minimum lust / libido / lust draft!").disableIf(emberAffection() < 95, "You don't know her good enough.", "???");
    ...
    private function takeEmbersEggHomeInADoggieBag() : Void
    {
        ...
        {
            outputText("  Between the egg's surprising lightness, and your own size and wide arms, you can easily carry the egg.");
        }
    ...
    //Iz Ember an egg?
    {
        ...
        else
        {
            ...
            {
                outputText("Ember doesn't seem to be around [ember eir] excavated den... [ember ey]'s probably training somewhere and will be back soon.\n\n");
            }
            else
            {
                ...
                if (choice == 0)
                {
                    outputText("Ember is lying in front of [ember eir] excavated den, watching the camp and resting.  Every once in a while [ember eir] eyes dart in your direction, before [ember ey] quickly looks away.\n\n");
                }
                else if (choice == 1)
                {
                    outputText("Ember doesn't seem to be around [ember eir] excavated den... it doesn't take long before Ember lands in front of it and then takes off again, apparently occupied with darting around in the sky for fun.\n\n");
                }
                else
                {
                    outputText("Ember appears to be fighting to stay awake; sometimes [ember ey] falls into a doze, but quickly snaps back to consciousness.\n\n");
                }
            }
        }
    }
    ...
    {
        var genderColor : Array<Dynamic> = ["white", "blue", "pink", "lavender"];
        outputText("\nThe egg has a " + genderColor[flags[kFLAGS.EMBER_GENDER]] + " tone");
        if (flags[kFLAGS.EMBER_HAIR] > 0)
        {
            outputText(((flags[kFLAGS.EMBER_HAIR] == 1) ? " with a little" : " covered in a dense") + " tiger-stripe pattern from the hair extension serum");
        }
        if (flags[kFLAGS.EMBER_OVIPOSITION] == 1)
        {
            outputText(", small green splotches from the ovielixer");
        }
        if (flags[kFLAGS.EMBER_MILK] == 1)
        {
            outputText(", and a yellow, veiny pattern from the lactaid");
        }
        outputText(".");
    }
    ...
    private function eggMenu() : Void
    {
        ...
        {
            addButton(0, "Masturbate", masturbateOntoAnEgg).disableIf(player.lust < 33, "Not aroused enough!");
        }
        addButton(1, "Blood", giveEmberBludSausages);
        var btn : Int = 5;
        for (itemFun in itemFuns)
        {
            addButton(btn++, Reflect.field(itemFun, Std.string(0)).shortName, Reflect.field(itemFun, Std.string(1))).disableIf(!player.hasItem(Reflect.field(itemFun, Std.string(0))), "You don't have any!");
        }
    ...
    private function dontEggFap() : Void
    {
        ...
        if (itemGood)
        {
            outputText("it doesn't react to some, while others make its flashes quicken.  These you set aside.");
        }
        else
        {
            outputText("it doesn't react to anything you have at the moment.");
    ...
    private function useIncubusDraftOnEmber(purified : Bool = false) : Void
    {
        clearOutput();
        if (purified)
        {
    ...
    private function useSuccubiMilkOnEmber(purified : Bool = false) : Void
    {
        clearOutput();
        if (purified)
        {
    ...
    private function masturbateOntoAnEgg() : Void
    {
        ...
        if (player.gender == 1)
        {
            ...
            {
                outputText("pumping vigorously.");
            }
            else
            {
                outputText("wrangling all your shafts together into one awkward bouquet of male organs and furiously stroking and squeezing them as best you can manage");
            }
        ...
        else
        {
            ...
            {
                outputText("as many of your cocks as you can manage to grab without falling off");
            }
    ...
    private function hatchZeMuzzles() : Void
    {
        ...
        //Male Anthro (Z){
            ...
            if (flags[kFLAGS.EMBER_GENDER] == 1)
            {
                ...
                if (flags[kFLAGS.EMBER_MILK] > 0)
                {
                    outputText(", drooling nipples notwithstanding");
                }
                outputText(".");
            }
            //Female Anthro (Z)
        ...
        else
        {
            ...
            if (flags[kFLAGS.EMBER_GENDER] == 1)
            {
                ...
                if (flags[kFLAGS.EMBER_MILK] > 0 || flags[kFLAGS.EMBER_HAIR] > 0)
                {
                    outputText(", complete with ");
                }
                if (flags[kFLAGS.EMBER_MILK] > 0)
                {
                ...
                if (flags[kFLAGS.EMBER_HAIR] > 0)
                {
                    outputText("long, feminine locks of hair");
                }
                outputText(", but his maleness is undeniable.  Especially when you spot ");
                if (flags[kFLAGS.EMBER_INTERNAL_DICK] > 0)
                {
                    outputText("the slit in his pelvis that disgorges a foot and a half-long inhuman penis");
                }
                else
                {
                    outputText("a foot and a half long human penis that sways between his legs");
                }
            ...
            else
            {
                ...
                if (flags[kFLAGS.EMBER_INTERNAL_DICK] == 0)
                {
                    outputText("huge, human prick");
                }
                else
                {
                    outputText("huge inhuman cock hanging from some kind of internal sheath");
                }
    ...
    private function embersAppearance() : Void
    {
        ...
        if (flags[kFLAGS.EMBER_ROUNDFACE] == 0)
        {
            ...
            if (flags[kFLAGS.EMBER_MILK] > 0 || hasVagina())
            {
                outputText("F-cup soft, pillowy breasts");
            }
            else
            {
                outputText("flat manly pecs");
            }
            outputText(" covered in fine scales excepting [ember eir] areolas; 0.5 inch nipples protrude from the center of the ");
            if (flags[kFLAGS.EMBER_MILK] > 0 || hasVagina())
            {
                outputText("generous mounds");
            }
            else
            {
                outputText("masculine pectorals");
            }
            outputText(".");
        ...
        else
        {
            ...
            if (flags[kFLAGS.EMBER_HAIR] < 1)
            {
                outputText("is quite short, giving [ember em] that definitive " + emberMF("masculine", "tomboy") + " look.");
            }
            else
            {
                outputText("drops down to [ember eir] shoulders, giving [ember em] the look of the " + emberMF("handsome", "beautiful") + " warriors from your village legends.");
            }
            
            outputText("\n\n[ember Eir] chest is also human in appearance and houses a pair of ");
            if (flags[kFLAGS.EMBER_MILK] > 0 || hasVagina())
            {
                outputText("F-cup breasts that support 0.5 inch nipples and hang heavily; you idly wonder if [ember ey]'ll develop lower back problems as she spends more time in Mareth");
            }
            else
            {
                outputText("manly pectorals with 0.5 inch nipples");
            }
            ...
            if (hasCock())
            {
                if (flags[kFLAGS.EMBER_INTERNAL_DICK] == 0)
                {
                    outputText("  [ember Ey] sports a flaccid penis and a pair of apple-sized balls that sit dangerously exposed to the elements, let alone to naked blades or a heavy blow.  Yet [ember ey] doesn't seem concerned about that in the least, almost daring anyone to focus on them.  While [ember ey] isn't aroused right now, Ember's penis can reach a length of approximately 16 inches, and it looks to be about 2 inches thick.");
                }
                else
                {
                    outputText("  [ember Ey] sports what looks like a protective slit of some sort, protecting [ember eir] dick from the elements as well as stray blows.  You can't see it right now; but you remember it to be about 16 inches long and 2 inches thick.");
                }
            }
            //Ember has a pussy:
            if (hasVagina())
            {
                outputText("  The inviting lips of a human-looking pussy purse between her legs; some moisture seems to have gathered on her labia, giving it a slick look that just begs for attention.");
            }
    ...
    //Checks for special scenes go here!
    {
        ...
        if (flags[kFLAGS.EMBER_AFFECTION] >= 100 && flags[kFLAGS.EGG_BROKEN] < 1)
        {
            addButton(3, "Eggshell", askEmberForEggshell).hint("Ask Ember for the eggshell. After all, you should have deserved the eggshell by now.");
        }
    }
    ...
    private function talkToEmberAboutDragonzzz() : Void
    {
        ...
        else
        {
            ...
            {
                outputText(", a blush creeping onto [ember eir] cheeks");
            }
            outputText(".");
    ...
    private function discussExplorationWithEmber() : Void
    {
        ...
        //Mountain{
            ...
            if (subChoice == 0)
            {
                outputText("imps. Only a few imps, and they seemed very nervous. I'm guessing that whatever naturally lives there is something that they really don't want to get involved with.");
            }
            else if (subChoice == 1)
            {
                outputText("two goblins in the foothills, arguing with each other. One was saying that the other shouldn't go up into the hills, as apparently the minotaurs that live there are too much for an untrained girl like her to take without being split open. The second goblin just laughed at her, called her a 'wimp' and told her that she's going to get herself 'some juicy bull-cock and tasty mino-spooge'. Ugh, disgusting little creatures.");
            }
            else if (subChoice == 2)
            {
                outputText("a pair of muscle-bound bull-men beating on each other with their bare fists. They spent over an hour smashing each other into a bloody pulp, and then the winner promptly started fucking the loser up the ass.  I had seen more than enough by that point, so I left.");
            }
            else if (subChoice == 3)
            {
                outputText("this... creature... that looked kind of like a human woman, but with a big dick where her clit should be.  She was walking around stark naked, 'cept for a bunch of piercings, and leading this bull-man along like a pet by a chain attached to a ring anchored into his cockhead.");
            }
            else if (subChoice == 4)
            {
                outputText("a couple of goblins sharpening scissors on some rocks outside of a cave with a door on it. Weird. Wonder what they could be doing in there?");
            }
            else if (!player.hasStatusEffect(StatusEffects.WormsOff) && rand(2) == 0)
            {
                outputText("a horrible swarm of slimy white worms, clumped together into a mockery of a human form and squelching along.  It managed to latch onto this two-headed dog-creature and... ugh!  The worms started forcing their way into both of its cocks!  I've never seen anything so disgusting!");
            }
            else if (subChoice == 5)
            {
                outputText("this two-headed dog-morph loping around; it spotted an imp, dropped to all fours, then gave chase. Managed to catch the ugly little demon, whereupon it ass-raped it, then ate it.");
            }
        }
        else if (choice == 1)
        ...
        //Forest{
            ...
            if (subChoice == 0)
            {
                outputText("a whole tribe of imps, just lounging around in a glade, jerking themselves off or squabbling over food.  Nasty little things, but easily dispatched.");
            }
            else if (subChoice == 1)
            {
                outputText("a  goblin with a huge pregnant belly, laughing to herself and swilling down that ale they brew, slopping it all over herself.  Little hedonists.");
            }
            else if (subChoice == 2)
            {
                outputText("this strange bee-woman creature... she made this, this music that started messing with my head.  I spat a tongue of flames at her and she flew away in fright, luckily.");
            }
            ...
            else if (subChoice == 3 && flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS] > 0)
            {
                ...
                if (hasVagina())
                {
                    outputText("  You should have more pride than to let some brainless cunt like that have her way with you!");
                }
            }
            //(If player has not yet impregnated Tamani)
            else if (subChoice == 3 && !SceneLib.forest.tamaniScene.pregnancy.isPregnant)
            {
                outputText("one goblin being teased by a bunch of pregnant goblins for not being pregnant yet.  She just spat back that she wanted a 'better catch' to be her baby-maker than a mere imp and wandered off.");
            }
            //(If Jojo isn't in the camp & not corrupt)
            else if (rand(2) == 0 && monk <= 1 && !player.hasStatusEffect(StatusEffects.PureCampJojo))
            {
                outputText("this mouse-morph monk, sitting in a glade and meditating. A goblin tried to proposition him; he just gave her a lecture and sent her running away in tears.  When an imp tried to attack him, he crushed its skull with a staff he had.  Not bad moves for such a weedy little thing...");
            }
            else
            {
                outputText("one glade I touched down in to catch myself a nice brace of plump coneys, when all of a sudden this... this thing made out of flailing vines and fruit attacks me.  It went up in a puff of smoke once I torched it, of course.");
            }
        }
        else if (choice == 2)
        ...
        //Lake{
            ...
            else
            {
                ...
                if (rand(3) == 0 && player.statusEffectv1(StatusEffects.MetWhitney) <= 1)
                {
                    outputText(" a big farm on the shoreline.  I saw some sort of cow-woman tending to fields of peppers, and a dog-woman herding cattle. They looked pretty fat and tasty, too... What?  I meant the cattle, stupid!  And I wouldn't have taken them - it's no fun snatching up livestock.  I prefer to chase down a deer or a wild auroch or something like that.");
                }
                //(if Fetish Cult is encounterable)
                else if (rand(3) == 0 && player.hasStatusEffect(StatusEffects.FetishOn))
                {
                    outputText("a pair of humans, arguing about sex.  They both wanted it, but the guy said he wanted to do the 'naughty schoolboy and female teacher' routine, while the girl wanted to do the 'noblewoman and love-servant' routine.  Weird; their clothes kept changing back and forth as they argued.");
                }
                //(If Beautiful Sword has not been acquired)
                else if (rand(3) == 0 && !player.hasStatusEffect(StatusEffects.TookBlessedSword) && !player.hasStatusEffect(StatusEffects.BSwordBroken))
                {
                    outputText("a sword jammed into a tree.  Weird; what kind of idiot would stick a weapon there like that?  And what kind of weakling wouldn't be able to take it out?");
                }
                else if (rand(3) == 0 && !player.hasStatusEffect(StatusEffects.CampRathazul))
                {
                    outputText("a smelly rat-man moping around while some weird equipment bubbled and boiled.  I think maybe he was an alchemist.");
                }
                else
                {
                    outputText("a great blob of green goo, sliding along and minding its own business.  I could swear it looked up at me once, and grew a penis... that can't be right, though.");
                }
            }
        }
        else if (choice == 3)
        ...
        //Desert{
            ...
            if (subChoice == 0)
            {
                outputText("a woman with four big breasts, squeezing milk out of her tits and into the sand.  I didn't know breasts could hold that much milk!");
            }
            else if (subChoice == 1)
            {
                outputText("a whole tribe of demons, lounging around an oasis.  Would have been too much bother to kick the crap out of them, so I left them alone - well, alright, I did buzz them to make them scatter like scared sheep for fun.");
            }
            //(if player hasn't solved Marcus & Lucia's argument)
            else if (rand(2) == 0 && !player.hasStatusEffect(StatusEffects.WandererDemon) && !player.hasStatusEffect(StatusEffects.WandererHuman))
            {
                outputText("a human with balls so big he had to carry them in a wheelbarrow, trundling through the wasteland with a succubus.  They were arguing about whether or not he should become an incubus.");
            }
            else
            {
                ...
                if (player.isNaga())
                {
                    outputText("She looked a lot like you.");
                }
                else
                {
                    outputText("I've never seen anything like her before.");
                }
            }
        }
        outputText("</i>\"");
    ...
    private function talkToEmberAboutYourself() : Void
    {
        ...
        if (player.str < 50)
        {
            outputText("  \"<i>You could use more training; you look pretty weak...</i>\" Ember says, critically.");
        }
        //(If PC Str < 90)
        else if (player.str < 90)
        {
        ...
        if (player.tou < 50)
        {
            outputText("\"<i>I don't know how you could hope to survive out there.  You look like the wind could blow you away.</i>\"");
        }
        //(If PC Tou < 90)
        ...
        if (player.inte < 50)
        {
            outputText("\"<i>You're still very naive... anyone or anything could trick you at any time...</i>\"  Ember frowns.");
        }
        //(If PC Int < 90)
        ...
        if (player.spe < 50)
        {
            outputText("You try to block the pebble, but you're not quick enough.  It hits your belly, not hard enough to hurt.  \"<i>You need to work on your reactions; anything could surprise you out there and you'd be helpless.</i>\"");
        }
        //(If PC Spd < 90)
    ...
    private function manEmberBitchesAboutPCPregnancy() : Void
    {
        ...
        if (emberAffection() <= 25)
        {
            outputText("The two of you talk about nothing in particular.  It's light, airy and pointless.  When you finish up, though, you realize something odd; Ember was doing [ember eir] best all throughout the conversation to avoid looking at your pregnant belly - almost as if [ember ey] were upset by it?");
        }
        //(Moderate Affection)
        else if (emberAffection() < 75)
        {
            outputText("Ember stares coldly at your gravid midriff as icebreaker after icebreaker from you falls flat.  You ask what's wrong.  \"<i>Nothing.  Nothing is wrong,</i>\" Ember states, flatly.");
        }
        //(High Affection)
        else
        {
            outputText("The expression Ember gives you is very cold.  \"<i>What is the meaning of this!?</i>\" [ember ey] says, motioning towards your bloated belly.  \"<i>You're supposed to help <b>me</b> breed more dragons; not slut around with trash!</i>\"  Ember blows out an indignant puff of smoke and walks away.");
        }
    }
    ...
    private function emberTalksToPCAboutPCDragoNPregnancy() : Void
    {
        ...
        if (player.lib < 40)
        {
            outputText("start at this revelation, then ");
        }
    ...
    private function emberIsPregnantFirstTimeTalkScene() : Void
    {
        ...
        if (player.cor >= 66)
        {
            outputText("  You almost choke on those cheesy words; still, she'll never put out for you if you don't butter her up.");
        }
        ...
        if (player.cor >= 66)
        {
            outputText("  Is she seriously buying this nonsense?  Your smile would be miles wide if you let your lip curl even a bit, so you keep a straight face with due dilligence.");
        }
        ...
        if (player.cor < 30)
        {
            outputText("  Given her attitude and nature, you know how lucky you are to have someone like her want to be your mate.");
        }
    ...
    public function emberBitchesAtYouAboutLustiness() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText("your " + vaginaDescript() + " slick and wet with moisture, ready to fuck");
        }
    ...
    public function emberGetOverFreakingOutAboutMinoJizz() : Void
    {
        ...
        else
        {
            ...
            if (hasCock())
            {
                outputText(", especially for sucking cock,");
            }
    ...
    private function bloodForTheBloodGod() : Void
    {
        ...
        else if (emberAffection() < 75)
        {
            ...
            if (player.cor < 50)
            {
                outputText(", expressing your gratitude; gently, you kiss [ember eir] fingers, then the cut, letting its cool, iron-tinted tang roll across your lips and tongue.  You carefully lick, trying to avoid causing pain as you drink that which [ember ey] has so generously offered you.");
            }
            else
            {
                outputText(", quickly drawing it to your mouth and forming a seal around the cut with your lips, anxious to let not a drop escape.");
            }
    ...
    private function emberIsAnEggFactory() : Void
    {
        ...
        {
            outputText(" if you give her an oviposition elixir");
        }
        outputText(".");
        //(Low Affection)
        if (emberAffection() <= 25)
        {
            ...
            if (player.cor >= 40)
            {
                outputText("or sell it for quick cash ");
            }
            ...
            if (hasCock())
            {
                ...
                if (internalDick())
                {
                    outputText("dragon-dick jutting from its slit,");
                }
                else
                {
                    outputText("veiny, bulbous penis fully");
                }
                outputText(" erect");
            }
            ...
            if (hasCock())
            {
                outputText(" and her throbbing cock is soft enough to hide");
            }
        ...
        else if (emberAffection() < 75)
        {
            ...
            if (flags[kFLAGS.EMBER_ROUNDFACE] == 1)
            {
                outputText(" and her cheeks grow red with embarrassment");
            }
        ...
        else
        {
            ...
            if (hasCock())
            {
                outputText(", and her cock points you accusingly");
            }
            outputText(".");
            ...
            {
                outputText(" and out... and out...");
            }
            outputText(" and then lean forward to give her a great, wet, sloppy lick, straight up the center of her pussy");
            //(E.Herm:
            if (flags[kFLAGS.EMBER_GENDER] == 3)
            {
                outputText(" not stopping until you have slurped your way to the very tip of her cock");
            }
            ...
            if (player.tongue.type > Tongue.HUMAN)
            {
                outputText("which is quite far indeed, ");
            }
            ...
            if (flags[kFLAGS.EMBER_GENDER] == 3)
            {
                outputText(" and dribbling down her painfully stiff dick");
            }
            ...
            if (player.tongue.type > Tongue.HUMAN)
            {
                outputText("  Right at this time, you feel something round and smooth on the tip of your tongue, gently spreading Ember's walls.  Realizing that this can only be her egg, you start trying to worm your long, sinuous tongue between it and her innermost walls, hoping to coax it out of her.");
            }
            ...
            if (flags[kFLAGS.EMBER_GENDER] == 3)
            {
                outputText("  Her cock throbs and pumps out long streams of cum to paint the ground around you two; marking it as your special place.");
            }
            if (player.tongue.type > Tongue.HUMAN)
            {
    ...
    private function dontWatchEmberLayEgg() : Void
    {
        ...
        if (flags[kFLAGS.EMBER_GENDER] == 3)
        {
            outputText(".. it even drips with some kind of off-white fluid.");
        }
    ...
    private function watchMediumAffectionEmberEggLay() : Void
    {
        ...
        if (flags[kFLAGS.EMBER_GENDER] == 3)
        {
            outputText(", cock poking straight out");
        }
        if (internalDick())
        {
            outputText(" of its hiding place");
        }
        ...
        if (player.lib >= 50)
        {
            outputText(" she doesn't ask you to, beaming a salacious grin");
        }
        outputText(".");
        ...
        if (player.gender > 0)
        {
            ...
            if (player.hasVagina())
            {
                outputText("wetness gathering in your own pussy");
            }
        ...
        if (flags[kFLAGS.EMBER_GENDER] == 3)
        {
            outputText(" and caressing her throbbing cock");
        }
        outputText(".");
        outputText("\n\nWith a deep moan, Ember shakes and orgasms.");
        if (flags[kFLAGS.EMBER_GENDER] == 3)
        {
            outputText("  Ropes of jism spew out of her cock, arching into the air to hit the ground nearby.");
        }
        ...
        if (flags[kFLAGS.EMBER_GENDER] == 3)
        {
            outputText(" and her cock hanging out");
        }
    ...
    private function getMilkFromEmber() : Void
    {
        ...
        if (emberAffection() <= 25)
        {
            ...
            //noEmberkids:
            if (flags[kFLAGS.EMBER_EGGS] == 0 && emberChildren() == 0)
            {
                outputText(emberMF("he", "she") + " has no offspring to feed them with");
            }
            //(1+ EmberEggs:
            else if (flags[kFLAGS.EMBER_EGGS] > 0)
            {
                outputText(emberMF("his", "her") + " children haven't hatched yet");
            }
            else
            {
                outputText(emberMF("his", "her") + " children don't drink nearly enough to properly empty [ember em]), so is [ember ey] just going to suffer with [ember eir] breasts so full and aching?");
            }
        ...
        else if (emberAffection() < 75)
        {
            if (flags[kFLAGS.EMBER_MILK] <= 0)
            {
                ...
                player.destroyItems(consumables.LACTAID, 1);
            }
        ...
        else
        {
            ...
            if (hasCock())
            {
                ...
                {
                    outputText("hardening");
                }
            }
            else
            {
                outputText("small beads of moisture gathering on her honeypot");
            }
            ...
            if (hasCock())
            {
                ...
                if (internalDick())
                {
                    outputText(" strangely ridged, almost scaly");
                }
                outputText(" surface");
            }
            else
            {
                outputText("her gently-dripping cunt, sliding in between the lips to stroke the wet interior");
            }
            outputText(".");
            ...
            if (hasCock())
            {
                outputText("stroke [ember eir] shaft");
            }
            else
            {
                outputText("probe gently into her slick depths");
            }
    ...
    public function beatEmberSpar() : Void
    {
        ...
        {
            if (flags[kFLAGS.EMBER_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.EMBER_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.EMBER_DEFEATS_COUNTER] = 1;
            }
            if (flags[kFLAGS.EMBER_LVL_UP] < 13 && flags[kFLAGS.EMBER_DEFEATS_COUNTER] >= flags[kFLAGS.EMBER_LVL_UP] + 4)
            {
                var addToV1 : Float = player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * flags[kFLAGS.EMBER_DEFEATS_COUNTER];
    ...
    private function catchAnal() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText(" your " + clitDescript() + " drips with every passing moment.");
        }
        ...
        if (player.ass.analLooseness == 0 || player.analCapacity() < 25)
        {
            outputText(", leaving both of you wincing as the sensitive head of [ember eir] cock struggles to make room for its insertion.  \"<i>I can't believe you really want me to put it here... you're so tight it almost hurts,</i>\" [ember ey] whimpers.");
        }
        ...
        if (player.hasCock())
        {
            outputText("  You can feel the dragon's member rubbing against your prostate, making " + sMultiCockDesc() + " jump to painful erectness, bubble pre-cum, and drool it down the shaft to pool on the ground.");
        }
        //(PC has cunt:
        if (player.hasVagina())
        {
            ...
            if (player.hasBalls())
            {
                outputText("over your [balls]");
            }
            outputText(" to puddle underneath you");
            if (player.hasCock())
            {
                outputText(", mingling with your pre to form an ever-growing pool of mixed sexual fluids.");
            }
        }
        ...
        if (player.hasBalls())
        {
            outputText("  [ember Eir] own balls swing into yours, and every time they connect a shrill wave of pleasure flows through you.");
        }
        ...
        if (silly())
        {
            outputText("ASSUMING DIRECT CONTROL.");
        }
        else
        {
            outputText("time to take control.");
        }
        ...
        if (player.hasVagina())
        {
            outputText("your cunt gushing feminine lubricant to glaze Ember's belly");
        }
        ...
        if (player.hasCock())
        {
            ...
            if (player.cumQ() < 250)
            {
                outputText("spurting gouts of cum onto Ember's upper torso");
            }
            else if (player.cumQ() < 1000)
            {
                outputText("raining spunk across the supine dragon");
            }
            else
            {
                outputText("cascading your man-cream across your blissed-out lover until [ember ey] is completely glazed in a sperm coating");
            }
        }
        outputText(".");
        ...
        if (player.gender > 0)
        {
            outputText(" and rubbing the results of your own orgasm against you");
        }
    ...
    private function suckEmberCock() : Void
    {
        ...
        if (flags[kFLAGS.EMBER_GENDER] == 1)
        {
            if (internalDick())
            {
                outputText("his single genital slit");
            }
            else
            {
                outputText("his genitals");
            }
        }
        else if (internalDick())
        {
            outputText("her twin genital slits");
        }
        else
        {
            outputText("her genitals");
        }
        ...
        if (player.hasCock())
        {
            outputText("  [EachCock] hardens, spearing aimlessly into the ground in your arousal.");
        }
        if (player.hasVagina())
        {
            outputText("  Your [vagina] starts to seep with feminine juices, your [clit] hardening in anticipation as your excitement dribbles down onto the thirsty ground.");
        }
        ...
        {
            outputText("soft-skinned");
        }
        outputText(" lower belly");
        if (pregnancy.event > 3)
        {
            outputText(", though the gravid orb resists you");
        }
        ...
        if (flags[kFLAGS.EMBER_GENDER] == 3 && (internalDick()))
        {
            ...
            if (player.tongue.type > Tongue.HUMAN)
            {
                outputText("  Your inhuman length slithers deeper and deeper inside, and you realize you can feel two rounded objects; [ember eir] balls! You're actually touching the testicles that are normally locked away inside [ember eir] body, except when [ember ey] reaches [ember eir] most aroused states...");
            }
        }
        ...
        if (flags[kFLAGS.EMBER_GENDER] == 3)
        {
            outputText(", your tongue wriggling around in [ember eir] slit");
        }
        ...
        {
            outputText("increasingly leery of [ember eir] batty behavior");
        }
    ...
    private function stickDickInKnifeDrawer() : Void
    {
        ...
        if (!player.isNaga())
        {
            outputText("seat yourself on a conveniently lichen-covered boulder.");
        }
        else
        {
            outputText("coil up on your serpentine tail and start peeling off the clothes on your lower half, exposing [eachCock] to the air and awaiting Ember's response.");
        }
        ...
        {
            outputText("  [ember Ey] takes your [cock " + y + "] in hand and begins gently stroking it,");
        }
        ...
        if (flags[kFLAGS.EMBER_ROUNDFACE] == 0)
        {
            outputText(", the scales on [ember eir] palms producing an indescribable feeling as they slide across your sensitive flesh,");
        }
        ...
        if (hasCock())
        {
            outputText("  [ember Eir] shaft is already hard as rock, and sometimes you can feel it poke your [feet], smearing small beads of Ember's barely contained pleasure on your " + player.skinFurScales() + ".");
        }
        if (hasVagina())
        {
            outputText("  Looking behind her, you see a small pool of her feminine juices has formed; pheromones fly through the air and hit you like a wave, spurring you on, making you feed Ember more of your delicious pre.");
        }
        ...
        if (flags[kFLAGS.EMBER_HAIR] == 0 && flags[kFLAGS.EMBER_ROUNDFACE] == 0)
        {
            outputText("scaly pate");
        }
        else
        {
            outputText("hair");
        ...
        if (player.cumQ() < 250)
        {
            ...
            {
                outputText("the thumb of her wing");
            }
        ...
        if (player.cor < 75)
        {
            outputText(" and chuckle; you know [ember ey] loves you, really.");
        }
        else
        {
            outputText(", folding your arms; [ember eir] dementia is getting worse...");
        }
    ...
    private function stickItInEmbersButt() : Void
    {
        ...
        var x : Float = player.cockThatFits(emberAnalCapacity());
        if (x < 0)
        {
        ...
        if (player.cor < 50)
        {
            outputText("promise the dragon you understand, and insist you wouldn't dream of doing otherwise; you want this to be good for [ember em], not to cause [ember em] pain.");
        }
        else
        {
            outputText("nod.");
        ...
        if (flags[kFLAGS.EMBER_ROUNDFACE] == 0)
        {
            outputText(", panting with lust");
        }
        else
        {
            outputText(", a blush adorning [ember eir] face");
        }
        ...
        if (hasCock())
        {
            outputText("Droplets of pre slide along [ember eir] own shaft and down [ember eir] balls to gather at your belly.  ");
        }
        if (hasVagina())
        {
            outputText("Her slick pussy drips rhythmically, wetting your lower body with slick dragon juice.  ");
        }
        ...
        {
            outputText("the base of your spine");
        }
        ...
        if (hasCock())
        {
            outputText(", jet upon jet of spunk arcing over [ember eir] head to paint the ground below");
        }
        if (hasVagina())
        {
            outputText("; a veritable gush of juices spews forth from her forgotten love-hole to splash against your lower body and over herself");
        }
    ...
    private function slurpDraggieCunnies() : Void
    {
        ...
        if (emberAffection() <= 25)
        {
            ...
            {
                outputText("on my affection meter");
            }
        ...
        else if (emberAffection() < 75)
        {
            ...
            if (hasCock())
            {
                outputText(" and slowly hardening shaft");
            }
            outputText(".");
        }
        //(High Affection)
        ...
        if (hasCock())
        {
            outputText("  With Ember's cock jutting out above your head, you can't resist sliding your tongue along the underside of her shaft, just for an extra teasing motion.");
        }
        ...
        if (flags[kFLAGS.EMBER_ROUNDFACE] > 0)
        {
            outputText("blushing all the while and ");
        }
        ...
        if (hasCock())
        {
            outputText("  You think droplets of pre are starting to slide down her girl-cock and drip onto your head... but that's part and parcel of loving a herm, so you pay it no attention.");
        }
        ...
        if (hasCock())
        {
            outputText(", heedless of the steady rain of pre dribbling into your " + hairDescript() + ".  You're confident she'll crack soon...");
        }
        ...
        if (hasCock())
        {
            outputText("  Her throbbing shaft, not intending to be left out of the fun, records the event by marking the tree bark with white dragon spooge.");
        }
        ...
        if (silly())
        {
            outputText("  My meter's not that full!");
        }
        outputText("</i>\"");
        ...
        if (player.hasVagina())
        {
            outputText("moist cunt");
        }
        if (player.gender == 0)
        {
            outputText("empty crotch");
        }
    ...
    private function getEatenOutByEmbra() : Void
    {
        ...
        if (!player.isNaga())
        {
            outputText(" with your [legs] spread wide");
        }
        ...
        if (player.hasCock())
        {
            outputText(", as well as the musk emanating from your drooling cock");
        }
        ...
        if (player.hasCock())
        {
            outputText(", and pre-cum beginning to bubble out of " + sMultiCockDesc());
        }
        ...
        if (player.wetness() >= 4)
        {
            outputText("spattering");
        }
        else
        {
            outputText("drenching");
        }
        outputText(" Ember's face with your feminine honey");
        if (player.hasCock())
        {
            outputText(" even as " + sMultiCockDesc() + " belches cum all over the preoccupied dragon");
        }
        outputText(".");
        ...
        if (player.wetness() >= 4)
        {
            outputText("  However, your squirting spatters make this an impossible task.  Ember doesn't seem to care, even as [ember eir] face is completely covered in femcum.");
        }
    ...
    private function penetrateEmbrah() : Void
    {
        ...
        else
        {
            ...
            if (flags[kFLAGS.EMBER_GENDER] == 3)
            {
                if (internalDick())
                {
                    outputText(" and her cock peek out of its protective slit");
                }
                else
                {
                    outputText(" and her cock begin hardening");
                }
            }
        ...
        if (flags[kFLAGS.EMBER_PUSSY_FUCK_COUNT] == 0)
        {
            ...
            if (player.cor > 75)
            {
                outputText("  As if bitches like her could qualify as maidens in any but the most literal sense.");
            }
            ...
            if (player.cor <= 75)
            {
                outputText("cute... but very troublesome");
            }
            else
            {
                outputText("irritating as hell");
            }
        ...
        if (player.cor < 50)
        {
            outputText("hasten to undress yourself for the impatient dragoness, quickly pulling");
        }
        else
        {
            outputText("languidly denude yourself, making sure to exaggerate every movement as you pull");
        }
        ...
        if (player.cockThatFits(emberVaginalCapacity()) == -1)
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("It doesn't ");
            }
            else
            {
                outputText("None of them ");
            }
            outputText("fit!</i>\"");
            ...
            if (player.cockTotal() == 1)
            {
                outputText("that... that... monster inside me,");
            }
            else
            {
                outputText("any of those... those... monsters inside me,");
            }
        ...
        if (hasCock())
        {
            outputText("; her own erect cock spurts a small jet of pre");
        }
        outputText(".");
        ...
        if (hasCock())
        {
            outputText(", and her cock, while engorged and swollen, hasn't poured forth yet");
        }
        outputText(".");
        ...
        if (flags[kFLAGS.EMBER_GENDER] == 3)
        {
            outputText("; her cock jumps and unloads on the ground below, marking this as the spot of your lovemaking");
        }
    ...
    private function getPenetratedByEmberLastSexSceneWoooo() : Void
    {
        ...
        else
        {
            ...
            {
                outputText("to attention");
            }
            else
            {
                outputText("out of its protective slit");
            }
        ...
        {
            outputText("your own inhumanly sinuous muscle ");
        }
        ...
        if (player.hasCock())
        {
            outputText("  As if [ember eir] hands caressing your cock weren't evidence of that.");
        }
        ...
        if (player.hasCock())
        {
            ...
            {
                outputText("completely cover");
            }
            else
            {
                outputText("utterly soak");
            }
        ...
        if (player.hasPerk(PerkLib.HistoryWhore) || player.hasPerk(PerkLib.HistorySlut) || player.hasPerk(PerkLib.PastLifeWhore) || player.hasPerk(PerkLib.PastLifeSlut))
        {
            outputText("  Using the skills you've honed, you make the muscles in your vagina ripple and wrinkle, teasing the cock caught inside you in a way few women can.");
        }
    ...
    private function getPenetratedByEmberLastSexSceneWooooPartII() : Void
    {
        ...
        if (internalDick())
        {
            outputText("protective slit");
        }
        else
        {
            outputText("limp cock and balls");
        }
        ...
        if (emberAffection() < 75)
        {
            outputText(emberMF("his", "her") + " cock was unlike anything else in this world?");
        }
        else
        {
            outputText(emberMF("he", "she") + " could handle two or even three of you at the same time?");
        }
    ...
    public function emberRapesYourHeatness() : Void
    {
        ...
        {
            outputText("[eachCock] burning to be seeding ripe, ready wombs");
        }
        outputText(".");
        ...
        {
            outputText("tightly pressing her drooling, puffy netherlips on your thighs.");
        }
    ...
    private function fuckOffEmberIWantANap() : Void
    {
        ...
        if (hasVagina())
        {
            outputText("creaming wet vagina");
        }
        ...
        emberAffection(-10);
        if (flags[kFLAGS.EMBER_AFFECTION] < 75 && flags[kFLAGS.SLEEP_WITH] == "Ember")
        {
            flags[kFLAGS.SLEEP_WITH] = "";
        }
    ...
    private function getKnockedUpByEmbrahBroBaby() : Void
    {
        ...
        {
            outputText("his flat, muscular chest");
        }
        else
        {
            outputText(emberMF("his", "her") + " soft, squeezable boobs");
        }
        ...
        if (flags[kFLAGS.EMBER_OVIPOSITION] == 0)
        {
            outputText("little dragons");
        }
        else
        {
            outputText("dragon eggs");
        }
        ...
        if (player.hasCock())
        {
            outputText(", your neglected [cock] drooling pre-cum that is being smeared all across your belly and [ember eirs] with every move you make");
        }
        ...
        if (flags[kFLAGS.EMBER_OVIPOSITION] == 0)
        {
            outputText("baby dragon");
        }
        else
        {
            outputText("dragon egg");
        }
        ...
        {
            player.knockUp(PregnancyStore.PREGNANCY_EMBER, PregnancyStore.INCUBATION_EMBER, 0);
        }  //Will always impregnate unless contraceptives are in use  
    ...
    private function breedEmberPregnantAsIfThereWasAnyOtherKindOfBreeding() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText("  She then reaches down lower to rub at your cum filled orbs.");
        }
        if (player.hasVagina())
        {
            outputText("  The tip of her tail gently teases your slick [vagina] with tiny strokes.");
        }
        ...
        if (hasCock())
        {
            outputText(", her own cock springing up and throbbing with the electric contact.");
        }
        else
        {
            outputText(".");
        ...
        if (flags[kFLAGS.EMBER_ROUNDFACE] == 0)
        {
            outputText("scale-covered ");
        }
        outputText("orbs gently squish ");
        if (flags[kFLAGS.EMBER_MILK] > 0)
        {
            outputText("and leak milk ");
        }
        ...
        if (flags[kFLAGS.EMBER_MILK] > 0)
        {
            outputText("the slowly trickling rivulets of milk along with ");
        }
        ...
        if (hasCock())
        {
            outputText("her cock spurts a few jets of cum onto your [chest]; ");
        }
        ...
        if (hasCock())
        {
            outputText("  Ember's cock wets the side of the " + camp.homeDesc() + " with continuous jets of cum, only adding to the already overpowering smell of sex as she empties herself on top of you.");
        }
        ...
        if (hasCock())
        {
            outputText("her spent cock throb; ");
        }
        ...
        if (hasCock())
        {
            outputText("  Little gobs of cum spill from her cock to slide down her shaft and gather on your belly button.");
        }
        ...
        if (player.hasVagina())
        {
            outputText("your neglected cunt splashing fluid down your [legs] and ");
        }
        ...
        if (player.cumQ() < 250)
        {
            outputText("  Though you usually don't cum that much, Ember's contracting walls literally suck the cum out of you; forcing you to give up more than you usually would... something you're quite glad to do!");
        }
        //(Medium Cum Amount)
        else if (player.cumQ() < 1000)
        {
            outputText("  Your generous helping of cum works its way towards her awaiting womb.  In fact, due to the delicious massage your shaft is receiving from Ember's contracting, almost sucking vaginal walls, you find yourself pumping out even more cum that you would usually have, quickly filling her to the brim and beyond.  It's not until Ember's belly is slightly distended that you stop.");
        }
        //(High Cum Amount)
        else
        {
            outputText("  You dump a huge load of jism into Ember's awaiting belly, but her contracting walls seem to milk you for ever more!  Ember is filled to the brim, her belly already slightly distended, but you still continue to fill her with your seed.  Ember's tightly sealed pussy lips means the cum has nowhere to go, so instead it gathers in her womb, distending her belly until she looks at least a few months pregnant; and by the look of pleasure on Ember's panting face, you don't think she minds it.");
        }
        
        outputText("\n\nWith a gasp and a sigh of utter relief, you slump into Ember's arms, collapsing onto her ");
        if (hasCock())
        {
            outputText("cum-slick ");
        }
        outputText("belly and laying there to regain your strength, ");
        if (hasCock())
        {
            outputText("dismissive of the spent cock lying sandwiched between you as you start ");
        }
    ...
    private function emberBreedingAfterMathWatchOutForRadioactiveFallout(emberPregged : Bool = true) : Void
    {
        clearOutput();
        ...
        if (flags[kFLAGS.EMBER_ROUNDFACE] == 0)
        {
            outputText(" are probably blushing under [ember eir] scales.");
        }
        else
        {
            outputText("blush.");
        ...
        if (!emberPregged)
        {
            outputText("touch your belly");
        }
        else
        {
            outputText("point at Ember's belly");
        }
        ...
        if (emberPregged)
        {
            outputText("pumped into me...");
        }
        else
        {
            outputText("had me pump into you...");
        }
        ...
        if (!emberPregged)
        {
            outputText("  You touch your belly with a smirk.  Very certain they took indeed...");
        }
        doNext(playerMenu);
    }
    
    private function emberPregUpdate() : Bool
    ...
    //Ember Gives Live Birth
    {
        ...
        if (flags[kFLAGS.EMBER_OVIPOSITION] == 0)
        {
            ...
            if (hasCock())
            {
                if (flags[kFLAGS.EMBER_ROUNDFACE] == 0 && flags[kFLAGS.EMBER_INTERNAL_DICK] == 0)
                {
                    outputText("  Her human-like cock dangles heavily in front of her pussy, partially erect from the muscular spasms.");
                }
                else
                {
                    outputText("  Her draconic cock has been pushed from its internal sheathe by the pressure, but it's too painful for her to be erect.");
                }
            }
            ...
            if (flags[kFLAGS.EMBER_ROUNDFACE] > 0)
            {
                outputText("head crowning");
            }
            else
            {
                outputText("tip of the muzzle jutting out");
            }
            ...
            if (flags[kFLAGS.EMBER_ROUNDFACE] == 0)
            {
                outputText("n anthro dragon");
            }
            else
            {
                outputText(" hybrid of human and dragon");
            }
            ...
            if (emberChildren() > 1)
            {
                outputText("  Your other draconic offspring quietly peek into the lair, having vacated the place when Ember went into labor.  They smile at the sight of their new sibling, gently slipping in to curl up beside the mother and new child.");
            }
            outputText("  With a contented expression on your face, you leave Ember and your ");
            if (emberChildren() <= 1)
            {
                outputText("new child");
            }
            else
            {
                outputText("newly expanded draconic brood");
            }
        ...
        else
        {
            ...
            if (hasCock())
            {
                if (flags[kFLAGS.EMBER_ROUNDFACE] > 0 && flags[kFLAGS.EMBER_INTERNAL_DICK] == 0)
                {
                    outputText("  Her human cock dangles heavily in front of her pussy, partially erect from the muscular spasms.");
                }
                else
                {
                    outputText("  Her draconic cock has been pushed from its internal sheathe by the pressure, but it's too painful for her to be erect.");
            ...
            if (flags[kFLAGS.EMBER_EGGS] > 0)
            {
                ...
                if (flags[kFLAGS.EMBER_EGGS] == 1)
                {
                    outputText("other");
                }
                else
                {
                    outputText(num2Text(flags[kFLAGS.EMBER_EGGS]) + " others");
                }
            }
    ...
    public function giveBirthToEmberKids(womb : Int = 0) : Void
    {
        ...
        //40% chance of boy, 40% chance of girl and 20% chance of both{
            ...
            if (player.hasBalls())
            {
                outputText("slowly pushing your [balls]");
            }
            else if (player.hasCock())
            {
                outputText("slowly pushing your " + multiCockDescriptLight());
            }
            if (player.hasBalls() || player.hasCock())
            {
                outputText(" out of [ember eir] way, ");
            }
            ...
            if (roll < 40)
            {
                ...
                {
                    flags[kFLAGS.EMBER_CHILDREN_MALES] += 2;
                }
                else
                {
                    flags[kFLAGS.EMBER_CHILDREN_MALES]++;
            ...
            else if (roll < 80)
            {
                if (player.hasMutation(IMutationsLib.GoblinOvariesIM))
                {
                    flags[kFLAGS.EMBER_CHILDREN_FEMALES] += 2;
                }
                else
                {
                    flags[kFLAGS.EMBER_CHILDREN_FEMALES]++;
            ...
            else if (player.hasMutation(IMutationsLib.GoblinOvariesIM))
            {
                flags[kFLAGS.EMBER_CHILDREN_HERMS] += 2;
            }
            else
            {
                flags[kFLAGS.EMBER_CHILDREN_HERMS]++;
    ...
    private function highAffectionEmberLustFuck() : Void
    {
        ...
        if (hasCock())
        {
            ...
            if (internalDick())
            {
                outputText("poking out of [ember eir] protective slit");
            }
            else
            {
                outputText("growing erect");
            }
        }
        if (flags[kFLAGS.EMBER_GENDER] == 3)
        {
        ...
        if (hasVagina())
        {
            outputText(" her pussy beginning to drip in excitement");
        }
        outputText(".");
        ...
        if (player.cockTotal() == 1)
        {
            outputText("a wonderful " + cockDescript(0));
        }
        else
        {
            outputText(" wonderful cocks");
        }
        ...
        if (hasCock())
        {
            outputText("stiffness of [ember eir] cock");
        }
        if (flags[kFLAGS.EMBER_GENDER] == 3)
        {
            outputText(" and ");
        ...
        if (hasVagina())
        {
            outputText("the slickness of her pussy");
        }
        ...
        else if (player.cockTotal() == 1)
        {
            ...
            if (flags[kFLAGS.EMBER_ROUNDFACE] == 0)
            {
                outputText("she-dragon");
            }
            else
            {
                outputText("dragon-girl");
            }
        ...
        else
        {
            ...
            if (flags[kFLAGS.EMBER_GENDER] == 3)
            {
                outputText(", not to mention the stiffness of her dick");
            }
            ...
            if (flags[kFLAGS.EMBER_GENDER] == 3)
            {
                outputText(", cock dribbling precum onto her belly");
            }
            outputText(".");
        ...
        if (hasVagina() && player.cockTotal() > 1)
        {
            outputText(" and ass");
        }
        ...
        if (hasVagina())
        {
            ...
            if (player.cockTotal() > 1)
            {
                outputText(" and tight ass");
            }
        }
        else
        {
            outputText(" tight ass");
        }
        ...
        if (flags[kFLAGS.EMBER_GENDER] >= 1 && player.cockTotal() > 1)
        {
            outputText("s");
        }
        ...
        if (flags[kFLAGS.EMBER_GENDER] == 1)
        {
            outputText("ass squeezes");
        }
        else if (player.cockTotal() == 1)
        {
            outputText("pussy squeezes");
        }
        else
        {
            outputText("holes squeeze");
        }
        outputText(" you, milking your ");
        if (flags[kFLAGS.EMBER_GENDER] == 1)
        {
            outputText("shaft as his cock spurts dragon-seed onto the leaves below him, filling the air with the scent of spunk and matting them into a steaming morass.");
        }
        ...
        else
        {
            if (player.cockTotal() == 1)
            {
                outputText("shaft as her cunt spasms wetly, drenching your lap with fresh femcum");
            }
            else
            {
                outputText("twin shafts as her cunt spasms wetly, drenching your lap with fresh femcum");
            }
            if (flags[kFLAGS.EMBER_GENDER] == 3)
            {
                outputText(" and her cock spurts dragon seed onto the leaves below her, filling the air with the scent of spunk and matting them into a steaming morass");
            }
            outputText(".");
        }
        ...
        if (player.lib >= 50 || player.minLust() >= 40)
        {
            ...
            if (hasVagina())
            {
                outputText("her cunt starting to drip");
            }
            if (flags[kFLAGS.EMBER_GENDER] == 3)
            {
                outputText(" and ");
            ...
            if (hasCock())
            {
                outputText(emberMF("his", "her") + " cock growing hard again");
            }
        ...
        else
        {
            ...
            if (hasVagina())
            {
                outputText("her cunt starts to drip with moisture");
            }
            if (flags[kFLAGS.EMBER_GENDER] == 3)
            {
                outputText(" and ");
            ...
            if (flags[kFLAGS.EMBER_GENDER] == 3 || flags[kFLAGS.EMBER_GENDER] == 1)
            {
                outputText(emberMF("his", "her") + " cock begins to grow erect again");
            }
        ...
        if (player.cockTotal() > 1 && hasVagina())
        {
            outputText("s");
        }
        ...
        if (hasVagina())
        {
            outputText(", moisture drenching the cock buried in her cunt as it slobbers greedily across your burning flesh.");
        }
        if (hasCock())
        {
            outputText("  [ember Eir] cock throbs and pulsates, ripples of arousal giving way to cumvein-bulging jets that shoot from [ember eir] prick to splatter wetly against the wall.");
        }
        ...
        if (hasVagina())
        {
            outputText("her cunt gushing female ejaculate, almost soaking you with the cascade of feminine fluids and puddling wetly against your [feet]");
        }
        outputText(".");
    ...
    private function noStayingForCuddlesPostLustFuck() : Void
    {
        ...
        if (player.cor < 50)
        {
            outputText("thank her for being so understanding");
        }
        else
        {
            outputText("grunt an acknowledgement");
        }
    ...
    //Frotting:
    private function frottingWithEmber(clear : Bool = true) : Void
    {
        ...
        if (player.gender == 1 && flags[kFLAGS.EMBER_GENDER] == 1)
        {
        ...
        if (hasVagina())
        {
            outputText("  Her moist dragon pussy is not helping the mess in the least.  You can smell her femcum pooling along with her pre.");
        }
        ...
        if (hasVagina())
        {
            outputText("  Her other hand frigs her pussy as she comes down from her orgasm, gushing female juices and leaking some of the cum you've pumped into her earlier.");
        }
    ...
    //Penetrate:
    private function penetrateWithEmber(clear : Bool = true) : Void
    {
        if (clear)
        {
            clearOutput();
        ...
        if (flags[kFLAGS.EMBER_MILK] > 0)
        {
            outputText(", milk seeping across your fingers,");
        }
        ...
        {
            outputText("the base of your tail, tugging lightly on it and stroking it for a moment, then her hands move on to ");
        }
        ...
        {
            outputText("stopping momentarily to stroke along your " + player.wings.desc + " wings before continuing up and ");
        }
        ...
        if (flags[kFLAGS.EMBER_MILK] > 0)
        {
            outputText("Drink my milk, I've been saving it up specially for you,");
        }
        else
        {
            outputText("I may not have any milk in my breasts right now, but maybe if you keep suckling I'll be able to make you some,");
        }
        ...
        if (flags[kFLAGS.EMBER_MILK] > 0)
        {
            outputText("  You savor the resultant gush of sweet dragon's milk as it squirts obediently down your throat.");
        }
        ...
        if (hasCock())
        {
            outputText("  A small dollop of pre escapes Ember's dragon-dick at the pleasure of the movement.");
        }
        ...
        if (flags[kFLAGS.EMBER_MILK] > 0)
        {
            outputText("allowing milk to spill into your mouth and gulp down sweet load after load, ");
        }
        ...
        if (flags[kFLAGS.EMBER_MILK] > 0)
        {
            outputText(", not even caring that she's tasting her own milk as you finish gulping it down");
        }
        ...
        if (flags[kFLAGS.EMBER_GENDER] == 3)
        {
            outputText("  Her draconic cock throbs all the way through your orgasm, shooting blanks a few times before spurting a couple ropes of pre onto her belly.");
        }
    ...
    private function emberJizzbangbangEnding() : Void
    {
        ...
        if (hasCock())
        {
            ...
            {
                outputText("monster-" + emberMF("boy", "girl"));
            }
    ...
    //Set timeQ
    {
        ...
        {
            if (chooser == 0 && player.HP < player.maxHP() * 0.8)
        ...
        {
            chooser = rand(3);
            if (flags[kFLAGS.SLEEP_WITH] != "Ember" && flags[kFLAGS.BENOIT_CLOCK_BOUGHT] > 0)
            {
                outputText("\n\nYou enter your cabin to take your alarm clock with you before you depart to Ember's den.");
            }
            if (chooser == 0 && flags[kFLAGS.EMBER_MILK] > 0 && flags[kFLAGS.TIMES_SLEPT_WITH_EMBER] > 0)
            ...
            else if (chooser == 1 && ((player.lib + (player.cor / 2)) >= 50 || player.lust >= 70) && flags[kFLAGS.TIMES_SLEPT_WITH_EMBER] > 0)
            ...
            //Lusty tease{
                ...
                if (player.lib >= 66 || player.lust >= 90)
                {
                    outputText("and maybe you'll get lucky and [ember ey]'ll want to have a little fun, too?");
                }
            ...
            else if (chooser == 2 && rand(2) == 0 && flags[kFLAGS.TIMES_SLEPT_WITH_EMBER] > 0)
            
            //Bathtime, very LOOOOOOOOOOONG{
            ...
            else if (flags[kFLAGS.TIMES_SLEPT_WITH_EMBER] <= 0)
    ...
    public function mishapsLunaEmber() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.CampLunaMishaps2, 2, 1);
        }
        else
        {
            player.createStatusEffect(StatusEffects.CampLunaMishaps2, 0, 1, 0, 0);
        }
