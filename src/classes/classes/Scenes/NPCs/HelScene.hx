...
class HelScene extends NPCAwareContent implements TimeAwareInterface
{
    ...
    public function timeChange() : Bool
    {
        ...
        {
            if (flags[kFLAGS.HELSPAWN_GROWUP_COUNTER] > 0)
            {
                flags[kFLAGS.HELSPAWN_GROWUP_COUNTER]++;
            }
            if (flags[kFLAGS.HEL_RAPED_TODAY] == 1)
            {
            ...
            {
                --flags[kFLAGS.HELSPAWN_DAD_EVENT];
            }
        }
        if (model.time.hours == 3 && followerHel() && flags[kFLAGS.SLEEP_WITH] == "Helia" && rand(10) == 0 && flags[kFLAGS.IN_INGNAM] == null)
        {
    ...
    //Helia's morning surprise!
    {
        ...
        if (model.time.hours == 8 && helFollower.followerHel() && flags[kFLAGS.HEL_NTR_TRACKER] == 1 && flags[kFLAGS.IN_INGNAM] == null)
        {
        ...
        if (flags[kFLAGS.HEL_FOLLOWER_LEVEL] == 1 && flags[kFLAGS.HEL_HARPY_QUEEN_DEFEATED] > 0 && helFollower.helAffection() >= 100 &&
            flags[kFLAGS.HELIA_FOLLOWER_DISABLED] == 0 && model.time.hours == 2 && flags[kFLAGS.IN_INGNAM] == null)
        {
            helFollower.heliaFollowerIntro();
            return true;
        }
        if (flags[kFLAGS.HEL_FOLLOWER_LEVEL] == -1 && model.time.hours == 6 && flags[kFLAGS.IN_INGNAM] == null)
        {
        ...
        if (checkedHeliaIsabellaThreesome++ == 0 && flags[kFLAGS.HEL_FUCKBUDDY] == 1 && SceneLib.isabellaFollowerScene.isabellaFollower() && model.time.hours == 2 && model.time.days % 11 == 0 && flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 0 && flags[kFLAGS.IN_INGNAM] == null)
        {
    ...
    {
        if (flags[kFLAGS.HEL_ISABELLA_THREESOME_ENABLED] >= 1 && flags[kFLAGS.HEL_BONUS_POINTS] < 150)
        {
            spriteSelect(SpriteDb.s_hel_sprite_BB);
        }
        //bandanna, no pale flame
        else if (flags[kFLAGS.HEL_ISABELLA_THREESOME_ENABLED] == 0 && flags[kFLAGS.HEL_BONUS_POINTS] >= 150)
        {
            spriteSelect(SpriteDb.s_hel_sprite_PF);
        }
        //no bandanna, pale flame
        else if (flags[kFLAGS.HEL_ISABELLA_THREESOME_ENABLED] >= 1 && flags[kFLAGS.HEL_BONUS_POINTS] >= 150)
        {
            spriteSelect(SpriteDb.s_hel_sprite_BB_PF);
        }
        //bandanna, pale flame
        else
        {
    ...
    private function loseToSalamander() : Void
    {
        helSprite();
        clearOutput();
        //(If HP loss)
        ...
        function dickF() : Void
        {
            ...
            if (player.gender == 3)
            {
                outputText("revealing your " + vaginaDescript(0) + " and ");
            }
            ...
            if (player.gender == 3)
            {
                outputText(" and your vag begins to squirt girlcum onto her scaled thighs");
            }
            ...
            sharedEnd();
        }  //else if(player.hasVagina()) {    //Player Loss – Rape – Female  ;
        ...
        function vagF() : Void
        {
            ...
            if (player.gender == 3)
            {
                outputText("\"<i>Though not exactly a woman, are we?</i>\" she laughs, giving your [cock] a quick stroke.  ");
            }
            ...
            if (player.hasCock())
            {
                outputText(", your cock shooting a massive rope of spunk onto her face as your vag cums from the tailfucking");
            }
            outputText(".\n\n");
            ...
            sharedEnd();
        }  //else {    //Player Loss – Rape – Genderless & Male >helFollower.helCapacity() cockarea  ;
        ...
        //(Genderless)
        {
            ...
            if (player.gender == 0)
            {
                outputText("\"<i>Hmm, what do we have here,</i>\" the salamander asks, yanking your [armor] off to reveal your genderless crotch.  \"<i>Well, that's... Different.  Well, your loss, I suppose...</i>\" she trails off, stepping up and pushing her cunt into your face.  \"<i>Now lick, and maybe I'll think up a way to get you off, too</i>\".\n\n");
            }
            //(Cock(s) too big to fit)
            else if (player.hasCock() && player.cockThatFits(helFollower.helCapacity()) < 0)
            {
                outputText("\"<i>Hot damn.  That's impressive, but even I'm not tough enough to handle that.  But maybe I can still get you off by... other means,</i>\" she says with a wink.\n\n");
            }
            else
            {
                outputText("\"<i>Hot damn.  That's impressive, but even I'm not tough enough to handle that.  But maybe I can still get you off by... other means,</i>\" she says with a wink.\n\n");
    ...
    public function beatUpHel() : Void
    {
        ...
        {
            outputText("Panting heavily, knees shaking, she collapses onto the ground, heavily leaning upon her sword.");
        }
        outputText("\n\n");
        if (player.cor > 85 - player.corruptionTolerance)
        {
            helDefeatedCorrupt();
        }
        else
    ...
    private function helDefeatedCorrupt() : Void
    {
        ...
        {
            addButton(0, "Rape Ass", rapingHelsAssMeansYourCorruptCauseAnalIsEvil).disableIf(player.cockThatFits(helFollower.helAnalCapacity()) < 0, helFollower.helAnalNofitMsg);
    ...
    private function helFuckMenu(isAmbush : Bool = false) : Void
    //ha-ha, very funny...
    {
        ...
        if (player.lust < 33 && !isAmbush)
        
        //let people be fucked if it's an ambush.{
        ...
        if (cockA == cockV)
        {
            cockA = player.cockThatFits2(helFollower.helAnalCapacity());
        }  //find another one; fails if <2 cocks  
        addButton(10, "DoublePen", dpHel).disableIf(cockV < 0 || cockA < 0, helFollower.helNofitMsg + "; " + helFollower.helAnalNofitMsg);
        ...
        addButton(11, "Possess", helPossessionShitPoopCock).disableIf(!player.hasPerk(PerkLib.Incorporeality), "Req. 'Incorporeality' perk from the ghost race!");
        addButton(2, "Tail Wank", helTailWank).disableIf(!player.hasCock(), "Req. a cock!");
        addButton(7, "GetTailPegged", helTailPegging);
        addButton(3, "CoilFuck", nagaCoilsUpHel).disableIf(!player.isNaga() || cockV < 0, "Req. to have Naga lower body; " + helFollower.helNofitMsg);
        addButton(8, "TailsInButts", nagaCoilsUpAnalNaga).disableIf(!player.isNaga(), "Req. to have Naga lower body!");
        addButton(4, "Mount Her", mountHel).disableIf(!player.isTaur() || cockV < 0, "Req. to have Taur lower body; " + helFollower.helNofitMsg);
        addButton(9, "Hanging69", helVaginaTaur69).disableIf(!player.isTaur() || !player.hasVagina(), "Req. to have Taur lower body and a vagina!");
    ...
    private function declineHelSexings() : Void
    {
        ...
        {
            outputText("\"<i>Aw, hell.  You're no fun,</i>\" she pouts, picking up her sword and sheathing it.  \"<i>Well, at least take this,</i>\" she adds, tossing a few gems your way.  You thank her for the gems as she starts walking over the hill towards wherever it is she's going.  ");
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function fuckHelsVag() : Void
    {
        helSprite();
        clearOutput();
        outputText(images.showImage("helia-vagfuck"));
        var x : Float = player.cockThatFits(helFollower.helCapacity());
        ...
        if (player.hasVagina())
        {
            outputText(" while releasing her breasts to finger your " + vaginaDescript(0));
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function fuckHelsAss() : Void
    {
        helSprite();
        clearOutput();
        outputText(images.showImage("helia-buttfuck"));
        var x : Float = player.cockThatFits(helFollower.helAnalCapacity());
    ...
    @:allow(classes.scenes.nPCs)
    private function helBlowsYou() : Void
    {
        helSprite();
        ...
        {
            outputText(", making her deepthroat you");
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function dpHel() : Void
    {
        helSprite();
        var x : Float = player.cockThatFits(helFollower.helCapacity());
        var y : Float = player.cockThatFits2(helFollower.helCapacity());
        clearOutput();
        ...
        if (flags[kFLAGS.TIMES_HELIA_DOUBLE_DONGED] == 0)
        {
            outputText("You're my first multi-cock.  Damn, never thought it'd be like THIS!");
        }
        else
        {
            outputText("You're amazing!  I don't think I'll ever get used to sex like THIS!");
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function getLickedByHel() : Void
    {
        helSprite();
        clearOutput();
        ...
        if (player.hasCock())
        {
            ...
            if (player.cumQ() >= 500)
            {
                outputText(", bathing her in streams of jism");
            }
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function helTailWank() : Void
    {
        helSprite();
        clearOutput();
    ...
    @:allow(classes.scenes.nPCs)
    private function helTailPegging(loss : Bool = false) : Void
    {
        helSprite();
        clearOutput();
    ...
    private function nagaCoilsUpHel() : Void
    {
        ...
        var x : Float = player.cockThatFits(helFollower.helCapacity());
        var y : Float = player.cockThatFits2(helFollower.helAnalCapacity());
        ...
        if (y >= 0)
        {
            outputText(" and her ass up with your " + cockDescript(y));
        }
    ...
    private function mountHel() : Void
    {
        ...
        outputText("You're a little nervous about outright breaking her, but she asked for it.  You trot up and swing your front hooves over her shoulders, resting part of your weight on her as your " + cockDescript(x) + " presses urgently against her firm ass cheeks.  She gasps and groans under your weight and at the prodding of your cock, so desperately seeking a hole to fill.  Finally, you slip between her legs and enter her cunt, ramming your full " + num2Text(as3hx.Compat.parseInt(player.cocks[x].cockLength)) + " inches into her sweltering depths.  You gasp and shudder at the intense heat of her inner walls, but your lover can only cry with exaltation as your cock fills her to the hilt.\n\n");
        ...
        if (player.cumQ() >= 500)
        {
            outputText(" until it begins to pour out around your cock, pooling between her spread legs");
        }
        outputText(".\n\n");
        ...
        if (player.cor < 50)
        {
            outputText("Sadly, your duties as Champion prevent you from accepting her offer, even if you wanted to.  You tell her as much, and she sighs heavily.");
        }
        else
        {
            outputText("You can't imagine committing to just one of this world's many eager pussies when so many more will go unfilled, though you fob her off with a more polite excuse.");
        }
    ...
    private function helVaginaTaur69() : Void
    {
        ...
        if (player.hasCock())
        {
            ...
            if (player.shortestCockLength() <= 24)
            {
                outputText(" , until you're buried to the hilt");
            }
        ...
        if (player.hasCock())
        {
            outputText(" from around your encapsulated cock");
        }
        ...
        if (player.hasCock())
        {
            outputText(" and swelling member");
        }
        outputText(".\n\n");
        ...
        if (player.hasCock())
        {
            outputText(" and filling her throat with jism");
        }
        outputText(".\n\n");
    ...
    private function rapingHelsAssMeansYourCorruptCauseAnalIsEvil() : Void
    {
        ...
        if (y >= 0)
        {
            outputText(", and your " + cockDescript(y) + " into her waiting vag, doubling the assault on the helpless berserker");
        }
    ...
    private function receiveCorruptRimjobsFromHel() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText("  Her tongue begins to zero in on your " + vaginaDescript(0) + ", but you give her a quick penalty swat on the tits.  You've got other plans!");
        }
        outputText("\n\n");
    ...
    //===========================================================================
    private var minoTalkBerserk : Bool = false;
    private var minoTalkAttack : Bool = false;
    ...
    public function helMinotaurThreesome() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("I've got another hole needs filling!</i>\"");
        }
        else
        {
            outputText("There's enough 'taur to go around, so come on!</i>\"");
        }
        
        ///Player's Options:
        //Male/Herm – [Fuck her Ass] [Mino Lick] [Leave]
        menu();
        addButton(0, "FuckHerAss", fuckHerAss).disableIf(player.cockThatFits(helFollower.helAnalCapacity()) < 0, helFollower.helAnalNofitMsg);
    ...
    private function helMinoThreeSomeLickItsDick() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(" while " + sMultiCockDesc() + " ejects its load into the mess on the 'taur's chest");
        }
        outputText(".\n\n");
    ...
    private function berserkMode() : Void
    {
        ...
        if (flags[kFLAGS.HEL_ISABELLA_THREESOME_ENABLED] == 0)
        {
            outputText(", that fat cow Isabella");
        }
    ...
    private function helChatMenu() : Void
    {
        ...
        menu();
        if (flags[kFLAGS.HEL_TALKED_ABOUT_HER] == null)
        {
        ...
        if (flags[kFLAGS.HEL_FUCKBUDDY] == null && !minoTalkAttack)
        {
            addButton(2, "YouAttackMe", askHelAboutAttackingYou);
        }
        ...
        if (flags[kFLAGS.HEL_TALKED_ABOUT_HER] != null)
        {
            addButton(4, "Leave", leaveHelAfterMinoThreeSomeChat);
        }
        else
        {
            addButton(4, "Bug Out", bugOutAfterHelMinoThreesome);
        }
    }
    ...
    public function followrIzzyxSallyThreesomePretext() : Void
    {
        ...
        if (isabellaAccent())
        {
            outputText("Good, now ve can take ze fight to zis uncouth barbarian!</i>\"\n\n");
        }
        else
        {
            outputText("Good, now we can take the fight to this uncouth barbarian!</i>\"\n\n");
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>I know ze little beech, [name].</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>I know the little bitch, [name].</i>\"\n\n");
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Do not listen to ze little liar, [name].  I found it in ze hands of ze gnolls, and most certainly did not steal it.</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Do not listen to the little liar, [name].  I found it in the hands of the gnolls, and most certainly did not steal it.</i>\"\n\n");
        }
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>So,</i>\" Isabella finally says, shifting her gaze from you to the salamander. \"<i>You two are... lovers, ja?  And here I vas about to thrash you!</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>So,</i>\" Isabella finally says, shifting her gaze from you to the salamander. \"<i>You two are... lovers, huh?  And here I was about to thrash you!</i>\"\n\n");
        }
    ...
    public function isabellaXHelThreeSomeCampStart() : Void
    {
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Perhaps you vould like a drink as vell, no?</i>\" Isabella offers, patting the chocolate-colored tit that Hel is not actively suckling from.\n\n");
        }
        else
        {
            outputText("\"<i>Perhaps you would like a drink as well, no?</i>\" Isabella offers, patting the chocolate-colored tit that Hel is not actively suckling from.\n\n");
        }
    ...
    private function nomOnIzzyTitWithSallyMancer() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(", and your [armor] is tenting visibly, displaying your excitement as you take her soft, delightfully warm and full breast in hand");
        }
        else
        {
            outputText(", and your nipples stiffen with excitement as you take her soft, delightfully warm and full breast in hand");
        }
        outputText(".\n\n");
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>I vant you.  I need you.  Both of you.  Here and now.</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>I want you.  I need you.  Both of you.  Here and now.</i>\"\n\n");
        }
        ...
        if (player.hasCock() && player.hasVagina())
        {
            ...
            {
                outputText("<b>You're too big to fuck them with your man-bits...</b>");
            }
        ...
        {
            outputText("Unfortunately, you don't have much to contribute...");
        }
    ...
    private function noThreesomeSexWithSallyAndIssyLastMinute() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText(" and balls bluer than the lake");
        }
        outputText(".");
        if (model.time.hours < 6)
        {
    ...
    private function stuffIzzyAndSalamanderWithDicks() : Void
    {
        ...
        if (y >= 0)
        {
            outputText("Meanwhile, your " + cockDescript(y) + " slides easily into Isabella's pussy, making the cow-girl gasp at the sudden and unexpected penetration.  ");
        }
        ...
        if (y >= 0)
        {
            outputText(", with your " + cockDescript(y) + " similarly coming to a rest inside Isabella's depths; you're able to feel the cow's rapid heartbeat hammering through her, causing her already tight cunt to contract rhythmically around your girth");
        }
        outputText(".\n\n");
        ...
        if (y == -1)
        {
            ...
            if (player.cocks[x].cockLength > 9)
            {
                if (isabellaAccent())
                {
                    outputText("\"<i>I normally do not play vith such... large... zings, but for you, I vill make an exception.  Give me your cock!</i>\"\n\n");
                }
                else
                {
                    outputText("\"<i>I normally don't play with such... clumsy... things, but for you, I'll make an exception.  Give me your cock!</i>\"\n\n");
                }
            }
        ...
        else if (z == -1 || zz == -1)
        {
            ...
            if (z > -1)
            {
                outputText(" with your " + cockDescript(z) + " even now spearing her tight ass");
            }
            outputText(".\n\n");
            ...
            if (isabellaAccent())
            {
                outputText("\"<i>Oh, ja!</i>\" Isabella says, now grinning too.  You're starting to worry, but between the two large women pinning you down and the intense pleasure of having all your cocks ridden at once, you're virtually helpless against whatever's coming.  \"<i>Oh, ja,</i>\" Isabella repeats, \"<i>And I think our Champion needs a bath!</i>\"\n\n");
            }
            else
            {
                outputText("\"<i>Oh, yes!</i>\" Isabella says, now grinning too.  You're starting to worry, but between the two large women pinning you down and the intense pleasure of having all your cocks ridden at once, you're virtually helpless against whatever's coming.  \"<i>Oh, yes,</i>\" Isabella repeats, \"<i>And I think our Champion needs a bath!</i>\"\n\n");
            }
    ...
    private function izzySallyThreeSomeFollowup() : Void
    {
        ...
        if (isabellaAccent())
        {
            outputText("\"<i>Mmm, ja,</i>\" Isabella says with a long yawn.  \"<i>Ve must indeed.  But for now, ve must rest.</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Mmm, yes,</i>\" Isabella says with a long yawn.  \"<i>We must indeed.  But for now, we must rest.</i>\"\n\n");
        }
    ...
    private function izzySallyThreeSomeVagoozlaz() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(" and Hel jerking your [cock] off");
        }
        ...
        if (player.hasCock())
        {
            outputText(" and salamander double-team");
        }
        ...
        if (player.hasCock())
        {
            outputText(", wrapping her warm tail around your [cock] lovingly");
        }
        outputText(".\n\n");
    ...
    private function heliasFoxyFourSomeFluffs() : Void
    {
        ...
        {
            doNext(foxyFluffGirlsFuckSex);
        }
        else
        {
            doNext(foxyFluffsFoursomeAsMale);
        }
    }
    
    //Foursome Scene -- As Male
    @:allow(classes.scenes.nPCs)
    private function foxyFluffsFoursomeAsMale() : Void
    {
        helSprite();
        clearOutput();
    ...
    @:allow(classes.scenes.nPCs)
    private function foxyFluffGirlsFuckSex() : Void
    {
        helSprite();
        clearOutput();
    ...
    private function telHelToGetOffTheMInoCock() : Void
    {
        ...
        {
            outputText("you know firsthand");
        }
        else
        {
            outputText("you've heard");
        }
        ...
        {
            outputText(", just like how Cowgirls have addictive breastmilk");
        }
        outputText(".");
    ...
    private function satisfyHelSoSheStopsMinoFucking() : Void
    {
        ...
        if (player.lib >= 50)
        {
            ...
            if (player.hasCock())
            {
                outputText("[cock] stiffening, ready to go again");
            }
            if (player.gender == 3)
            {
                outputText(" and your ");
            ...
            if (player.hasVagina())
            {
                outputText("[vagina] already soaked with your lustful secretions");
            }
            if (player.gender == 0)
            {
                outputText("ass ready to be fucked again");
            }
    ...
    public function helSexualAmbushCondition() : Bool
    {
        return flags[kFLAGS.PC_PROMISED_HEL_MONOGAMY_FUCKS] == 1 && flags[kFLAGS.HEL_RAPED_TODAY] == 0 && player.gender > 0 && !followerHel() && !player.hasStatusEffect(StatusEffects.HeliaOff);
    }
    //Hel Sexual Ambush
