...
class Camilla extends Monster
{
    public function doubleSlash() : Void
    {
        ...
        else
        {
            ...
            //Get hit
            var damage : Int = as3hx.Compat.parseInt(str + weaponAttack + rand(80));
            player.takePhysDamage(damage, true);
            //Get hit again
            damage = as3hx.Compat.parseInt(str + weaponAttack + rand(80));
            player.takePhysDamage(damage, true);
        }
    }
    ...
    public function phoenixFireBreathA() : Void
    {
        ...
        else
        {
            ...
            {
                var damage : Int = as3hx.Compat.parseInt(str + weaponAttack + 300 + rand(250));
    ...
    public function lustBang() : Void
    {
        ...
        else
        {
            ...
            if (player.hasCock())
            {
                outputText("shaft quickly engorges inside your armour, already leaking precum down your leg");
            }
            if (player.hasCock() && player.hasVagina())
            {
                outputText("whilst your");
            }
            if (player.hasVagina())
            {
                outputText("thighs are suddenly soaked by a torrent of girlcum as your body reacts to the potent chemicals");
            }
            outputText(".");
            var lustDmg : Float = (30 + rand(30));
    ...
    public function new()
    {
        ...
        this.drop = new WeightedDrop().add(useables.EBONBLO, 1).add(weapons.SCIMITR, 2).add(consumables.NOCELIQ, 10);
