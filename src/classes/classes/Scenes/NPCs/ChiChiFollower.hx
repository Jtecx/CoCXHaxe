...
class ChiChiFollower extends NPCAwareContent implements TimeAwareInterface implements SaveableState
{
    public var ChiChiCorruption : Int;  //scale from 0-10	//9+ come to camp  
    public var ChiChiFertile : Bool;
    public var ChiChiKickedOut : Bool;
    public var ChiChiDefurred : Bool;
    public var kihaInteractionHappened : Bool;
    public var jojoInteractionHappened : Bool;
    public var joyInteractionHappened : Bool;
    public var ayaneInteractionHappened : Bool;
    public var ceaniInteractionHappened : Bool;
    public var lunaInteractionHappened : Bool;
    public var bimboPatchInteractionHappened : Bool;
    public var amilyInteractionHappened : Bool;
    public var heliaInteractionHappened : Bool;
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            ChiChiCorruption = Reflect.field(o, "ChiChiCorruption");
            ChiChiFertile = Reflect.field(o, "ChiChiFertile");
            ChiChiKickedOut = Reflect.field(o, "ChiChiKickedOut");
            ChiChiDefurred = Reflect.field(o, "ChiChiDefurred");
            kihaInteractionHappened = Reflect.field(o, "kihaInteractionHappened");
            jojoInteractionHappened = Reflect.field(o, "jojoInteractionHappened");
            joyInteractionHappened = Reflect.field(o, "joyInteractionHappened");
            ayaneInteractionHappened = Reflect.field(o, "ayaneInteractionHappened");
            ceaniInteractionHappened = Reflect.field(o, "ceaniInteractionHappened");
            lunaInteractionHappened = Reflect.field(o, "lunaInteractionHappened");
            bimboPatchInteractionHappened = Reflect.field(o, "bimboPatchInteractionHappened");
            amilyInteractionHappened = Reflect.field(o, "amilyInteractionHappened");
            heliaInteractionHappened = Reflect.field(o, "heliaInteractionHappened");
        }
        else
        {
            resetState();
    ...
    public function EnterOfTheChiChi() : Void
    {
        ...
        {
            outputText("Chi Chi, as it’s obviously her name, adopts a battle stance. Her fists, tail and legs suddenly light aflame. You blink in surprise as the heat washes over you. Something tells you this is going to hurt.");
        }
    ...
    public function WonSecondFightYes() : Void
    {
        ...
        if (player.hasCock())
        {
            ...
            if (!player.isNaked())
            {
                outputText(" slowly remove your [armor] to");
            }
    ...
    public function WonSparringFight() : Void
    {
        ...
        {
            outputText("the remaining greater demons");
        }
        ...
        {
            if (flags[kFLAGS.CHI_CHI_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.CHI_CHI_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.CHI_CHI_DEFEATS_COUNTER] = 1;
            }
            if (flags[kFLAGS.CHI_CHI_DEFEATS_COUNTER] == 9 && flags[kFLAGS.CHI_CHI_LVL_UP] == 5)
            {
                ...
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers2, 2, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 9);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers2, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 9, 0, 0);
                }
            ...
            if (flags[kFLAGS.CHI_CHI_DEFEATS_COUNTER] == 10 && flags[kFLAGS.CHI_CHI_LVL_UP] == 6)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers2))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers2, 2, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 10);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers2, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 10, 0, 0);
                }
                flags[kFLAGS.CHI_CHI_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.CHI_CHI_DEFEATS_COUNTER] == 11 && flags[kFLAGS.CHI_CHI_LVL_UP] == 7)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers2))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers2, 2, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 11);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers2, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 11, 0, 0);
                }
                flags[kFLAGS.CHI_CHI_DEFEATS_COUNTER] = 0;
    ...
    public function SoulskilsManualsShop() : Void
    {
        ...
        {
            outputText(" but I would need a few spirit stones first");
        }
        outputText(".</i>\"\n\n");
        menu();
    ...
    public function SoulskilsManualsShopIceFist() : Void
    {
        ...
        {
            addButtonDisabled(0, "Yes", "You already learned how to use Ice Fist.");
        }
        else
        {
            addButton(0, "Yes", SoulskilsManualsShopIceFist2);
        }
    ...
    public function SoulskilsManualsShopIceFist2() : Void
    {
        ...
        if (flags[kFLAGS.CHI_CHI_FOLLOWER] >= 6)
        {
        ...
        if (!player.hasStatusEffect(StatusEffects.KnowsIceFist))
        {
            player.createStatusEffect(StatusEffects.KnowsIceFist, 0, 0, 0, 0);
        }
    ...
    public function SoulskilsManualsShopFirePunch() : Void
    {
        ...
        {
            addButtonDisabled(0, "Yes", "You already learned how to use Fire Punch.");
        }
        else
        {
    ...
    public function SoulskilsManualsShopFirePunch2() : Void
    {
        ...
        if (!player.hasStatusEffect(StatusEffects.KnowsFirePunch))
        {
            player.createStatusEffect(StatusEffects.KnowsFirePunch, 0, 0, 0, 0);
        }
        flags[kFLAGS.SPIRIT_STONES] -= 5;
        doNext(camp.returnToCampUseFourHours);
    }
    ...
    public function SoulskilsManualsShopHurricaneDance() : Void
    {
        ...
        {
            addButtonDisabled(0, "Yes", "You already learned how to use Hurricane Dance.");
        }
        else
        {
            addButton(0, "Yes", SoulskilsManualsShopHurricaneDance2);
        }
        addButton(1, "No", SoulskilsManualsShop);
    }
    public function SoulskilsManualsShopHurricaneDance2() : Void
    {
        ...
        if (!player.hasStatusEffect(StatusEffects.KnowsHurricaneDance))
        {
            player.createStatusEffect(StatusEffects.KnowsHurricaneDance, 0, 0, 0, 0);
        }
        flags[kFLAGS.SPIRIT_STONES] -= 5;
        doNext(camp.returnToCampUseFourHours);
    }
    ...
    public function SoulskilsManualsShopEarthStance() : Void
    {
        ...
        {
            addButtonDisabled(0, "Yes", "You already learned how to use Earth Stance");
        }
        else
        {
            addButton(0, "Yes", SoulskilsManualsShopEarthStance2);
        }
        addButton(1, "No", SoulskilsManualsShop);
    }
    public function SoulskilsManualsShopEarthStance2() : Void
    {
        ...
        if (!player.hasStatusEffect(StatusEffects.KnowsEarthStance))
        {
            player.createStatusEffect(StatusEffects.KnowsEarthStance, 0, 0, 0, 0);
        }
        flags[kFLAGS.SPIRIT_STONES] -= 5;
        doNext(camp.returnToCampUseFourHours);
    }
    ...
    public function SoulskilsManualsShopPunishingKick() : Void
    {
        ...
        {
            addButtonDisabled(0, "Yes", "You already learned how to use Punishing Kick.");
        }
        else
        {
    ...
    public function SoulskilsManualsShopPunishingKick2() : Void
    {
        ...
        if (!player.hasStatusEffect(StatusEffects.KnowsPunishingKick))
        {
            player.createStatusEffect(StatusEffects.KnowsPunishingKick, 0, 0, 0, 0);
        }
        flags[kFLAGS.SPIRIT_STONES] -= 5;
        doNext(camp.returnToCampUseFourHours);
    }
    ...
    public function ChiChiCampMainMenu() : Void
    {
        spriteSelect(SpriteDb.s_chichi);
        clearOutput();
        if (rand(4) == 0 && !kihaInteractionHappened && SceneLib.kihaFollower.followerKiha())
        {
            kihaInteraction();
        }
        else if (rand(4) == 0 && !jojoInteractionHappened && player.hasStatusEffect(StatusEffects.PureCampJojo) && flags[kFLAGS.JOJO_BIMBO_STATE] != 3)
        {
            jojoInteraction();
        }
        else if (rand(4) == 0 && !joyInteractionHappened && flags[kFLAGS.JOJO_BIMBO_STATE] == 3)
        {
            joyInteraction();
        }
        else if (rand(4) == 0 && !ayaneInteractionHappened && flags[kFLAGS.AYANE_FOLLOWER] >= 2)
        {
            ayaneInteraction();
        }
        ...
        else if (rand(4) == 0 && !lunaInteractionHappened && flags[kFLAGS.LUNA_FOLLOWER] >= 4 && !player.hasStatusEffect(StatusEffects.LunaOff))
        {
            lunaInteraction();
        }
        else if (rand(4) == 0 && !bimboPatchInteractionHappened && flags[kFLAGS.PATCHOULI_FOLLOWER] >= 6)
        {
            bimboPatchInteraction();
        }
        ...
        if (flags[kFLAGS.CHI_CHI_FOLLOWER] < 4)
        {
            outputText("\"<i>Oh how are you doing, [name]? I just finished a new set of kicks and punches. Did you want us to train together or did you have something else in mind?</i>\" She’s arm crossed waiting on your reply.");
        }
        else
        {
            outputText("\"<i>So glad to see you [name], you are a sunshine in my life, you know that. So? What did you visit your lovely wife for today?</i>\"");
        }
        if (SceneLib.midokaScene.MidokaAge > 0 && SceneLib.midokaScene.MidokaAge < 4)
        {
            outputText("\n\n[midokaname] is sleeping in a basket not so far from Chi Chi, such a sweet child.");
        }
        
        var _sw10_ = (pregnancy.event);        
        ...
        switch (_sw10_)
        {
            ...
            case 3, 4:outputText("[pg]Chi-Chi's belly is starting to protrude a little. She's unquestionably pregnant.\n\n");
            case 5, 6:outputText("[pg]Chi-Chi's belly has gotten very big.\n\n");
        ...
        addButton(2, "Shop", SoulskilsManualsShop);
        addButton(3, "Spar", chichiSparring).disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, "You need a good sparring ring for that.");
    ...
    public function chichiAppearance() : Void
    {
        ...
        {
            outputText(" She has grown twin embery horns up her forehead.");
        }
        if (flags[kFLAGS.CHI_CHI_FOLLOWER] >= 6)
        {
            outputText(" That said Chi Chi tends to soften around you thanks to the feelings you both share. She wears the ruby ring you gave her at all time, symbol and reminder of your love.");
        }
    ...
    public function chichiTalksFormerStudents() : Void
    {
        ...
        if (flags[kFLAGS.CHI_CHI_FOLLOWER] < 4)
        {
            outputText(" Her hand is bleeding but she pays it no attention, the wounds slowly closing.");
        }
        ...
        if (flags[kFLAGS.CHI_CHI_FOLLOWER] < 4)
        {
            ...
            if (player.cor > 33)
            {
                outputText(" If she wasn’t such a blockhead, you might even show her in for a three-way.");
            }
    ...
    public function chichiSparring() : Void
    {
        ...
        if (flags[kFLAGS.CHI_CHI_FOLLOWER] < 4)
        {
            outputText(" now that she has recovered a little");
        }
    ...
    public function ChiChiDrunkSex() : Void
    {
        ...
        if (flags[kFLAGS.CHI_CHI_FOLLOWER] >= 1 && flags[kFLAGS.CHI_CHI_AFFECTION] >= 20)
        {
            outputText("Chi Chi");
        }
        else if (flags[kFLAGS.CHI_CHI_FOLLOWER] >= 1 && flags[kFLAGS.CHI_CHI_AFFECTION] < 20)
        {
            outputText("Chi Chi the waitress from the exotic food restaurant");
        }
        else
        {
            outputText("a blazing mouse girl");
        }
        ...
        if (flags[kFLAGS.CHI_CHI_FOLLOWER] > 2)
        {
            outputText("\"<i>Gaaaaah, [name], why do you hang around all those girls. Worse why do you fuck with them and not me?! Its like you are a " + player.mf("go-go boy", "cheap whore") + " selling your body to everyone and thish drives me mad! Yeaaa, I’m going to prove them all I’m the top shlut!</i>\"");
        }
        else if (flags[kFLAGS.CHI_CHI_FOLLOWER] == 1)
        {
            outputText("\"<i>Hey... you’re that so-called champ who lost in the arena?! Well I like you. I like you so much I’d want you as my " + player.mf("boy", "girl") + "friend... but waaaah I’m way too shy to tell you that right! Well don’t go tell [name] I like " + player.mf("him", "her") + " got it? Yeah, who cares about that! Barman, a round for [name] the best person I've ever met, woooo. Now let’s have sex!</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Hey you... yesh you, the tall person over there! Think you can fight?! Well I’m pretty sure nobody in this village hash the guts to anyway. If you show up in the arena, I'sh gonna beat the crap outta you... \"</i> She grins drunkenly, grabbing at your crotch with one hand. \" <i>Now that I think of it...Fightin' and fuckin'... I can beat anyone in both! I'sh Bet you're the kind who fucks every demon you meet. Well not tonight, letsh bang!</i>\"");
        }
    ...
    private function drunksex(selected : Bool) : Void
    {
        clearOutput();
        if (!selected)
        {
        ...
        else
        {
            ...
            function dickF() : Void
            {
                ...
                if (flags[kFLAGS.CHI_CHI_FOLLOWER] < 1)
                {
                    flags[kFLAGS.CHI_CHI_FOLLOWER] = 1;
                }
                doNext(camp.returnToCampUseOneHour);
            };
            function vagF() : Void
    ...
    public function chichiSex() : Void
    {
        ...
        if (flags[kFLAGS.CHI_CHI_FOLLOWER] == 3)
        {
            ...
            if (flags[kFLAGS.IZUMI_TIMES_GRABBED_THE_HORN] > 0)
            {
                outputText("Happily, you have already defeated one... and grabbed her horn!");
            }
            else if (flags[kFLAGS.IZUMI_MET] == 1)
            {
                outputText("Well, not you need to just kick Izumi's ass in any way.");
            }
            else
            {
                outputText("Now, where could you find one...? Perhaps, somewhere at the top of a mountain?");
            }
            menu();
            addButton(0, "Wedding", chichiSex1).disableIf(flags[kFLAGS.IZUMI_TIMES_GRABBED_THE_HORN] == 0, "Need to beat some oni.").disableIf(!sceneHunter.canMarry(), "You have already married someone else.");
        ...
        else
        {
            ...
            {
                addButton(0, "Until Dawn", chichiSexUntilDawn);
            }
            else
            {
                addButtonDisabled(0, "Until dawn", "It's too early for this. Wait till 7 pm.");
            }
            if (player.hasCock())
            {
                addButton(1, "Gentle Fuck", chichiSexGentleFuck);
            }
            else
            {
                addButtonDisabled(1, "Gentle fuck", "Get a dick, bro.");
            }
            if (player.hasVagina())
            {
                addButton(2, "HinezumiYuri", chichiSexHinezumiYuri);
            }
            else
            {
                addButtonDisabled(2, "HinezumiYuri", "Be a pussy or have one, sis.");
            }
            if (ChiChiCorruption >= 9)
            {
                addButton(3, "Corrupt", corruptSexRouter);
            }
    ...
    public function chichiSex1() : Void
    {
        ...
        else
        {
            ...
            if (!SceneLib.templeofdivine.canMarry())
            {
                ...
                {
                    outputText("also realize to your annoyance that not only are you missing a wedding ring but the two of you will need a holy place to recite your vows, one with a still working altar of Marae and active priestess. Come to think of it her favorite color was red maybe a ruby ring would do.");
                }
                else
                {
    ...
    public function chichiSex4() : Void
    {
        ...
        if ((camp.followersCount() + camp.loversCount()) > 0)
        {
            outputText(". All of your friends actually came to attend");
        }
        if (camp.loversCount() > 0)
        {
            outputText(" which is not so surprising considering some of them are also in your harem");
        }
    ...
    public function chichiSexUntilDawn() : Void
    {
        ...
        if (player.hasCock())
        {
            ...
            if (ChiChiCorruption > 90)
            {
                outputText("“<i>I don’t care about the rest of Mareth right now. The world could be on fire I would still be fucking with you, the here and now is all that matter. So let’s consume ourselves in this blaze of pleasure we started.</i>”[pg]");
            }
            else
            {
    ...
    public function chichiSexGentleFuck() : Void
    {
        ...
        if (ChiChiCorruption > 90)
        {
            outputText("“<i>Go fuck some other demon into submission [name]. I will be keeping that bed warm for you when you come back ready for another round.</i>”[pg]");
        }
        else
        {
    ...
    public function mishapsLunaChiChi() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.CampLunaMishaps2, 4, 1);
        }
        else
        {
            player.createStatusEffect(StatusEffects.CampLunaMishaps2, 0, 0, 0, 1);
        }
    ...
    public function corruptionOverflowing() : Void
    {
        ...
        if (flags[kFLAGS.CHI_CHI_FOLLOWER] <= 3)
        {
            outputText(" Regardless I ended up with no place to go and remembered you had a camp in the wasteland so I'm moving in.");
        }
        outputText("</i>[pg]");
    ...
    public function newLoverComesToCamp() : Void
    //TODO check for new lovers
    {
    ...
    public function talkAboutFur() : Void
    {
        ...
        var defur : Void->Void = function() : Void
        {
        ...
        var likeIt : Void->Void = function() : Void
        {
    ...
    //Get out if already pregged.
    {
        ...
        var score : Float = Math.min(player.cumQ() / 500, 5);
    ...
    public function chichiGivesBirth() : Void
    {
        ...
        if (flags[kFLAGS.CHI_CHI_FOLLOWER] < 6)
        {
            outputText("You can’t help but be amazed at the absence of a spiky reply or aggressiveness in her tone. This is likely how she is once she stops pretending she’s not in deep love with you. Makes you ponder what marrying her would be like. ");
        }
    ...
    private function chichiPregUpdate() : Bool
    {
        switch (pregnancy.eventTriggered())
        {
            case 2:if (flags[kFLAGS.CHI_CHI_FOLLOWER] < 4)
                {
                    unplannedPregnancy();
                }
                else
