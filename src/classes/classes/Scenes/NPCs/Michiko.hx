...
class Michiko extends NPCAwareContent
{
    ...
    public function firstMeetingMichiko() : Void
    {
        ...
        {
            outputText("Intrigued, Jojo snoops up to check on your new scholarly guest.\n\n");
        }
        if (player.hasStatusEffect(StatusEffects.CampRathazul))
        {
            outputText("Rathazul raises an eyebrow as the squirrel girl walks around and sets up her installations, though he swiftly goes back to his experiments.\n\n");
        }
    ...
    public function campMichikoMainMenu() : Void
    {
        ...
        addButton(2, "Identify", identifyMenu).hint("Identify items").disableIf(player.gems < 100, "You can't afford identification (100 gems)").disableIf(getAllUnidentifiedItems().length == 0, "You don't carry unidentified items");
        addButton(14, "Back", camp.campFollowers);
    }
    public function getAllUnidentifiedItems() : Array<Dynamic>
    {
        return player.itemSlots.filter(varargify(function(slot : ItemSlotClass) : Bool
                        {
                            return slot.unlocked && slot.quantity > 0 && Std.is(slot.itype, IDynamicItem) && !(try cast(slot.itype, IDynamicItem) catch(e:Dynamic) null).identified;
                        }));
    }
    public function identifyMenu() : Void
    {
        ...
        var items : Array<Dynamic> = getAllUnidentifiedItems();
        if (items.length > 1)
        {
            buttons.add("All (" + (items.length * 100) + ")", identifyAll, "Identify all items (" + (items.length * 100) + " gems)").disableIf(player.gems < items.length * 100, "You don't have " + (items.length * 100) + " gems");
        }
        for (slot in items)
        {
    ...
    public function identifyItem(item : ItemSlotClass) : Void
    {
        ...
        var newItem : ItemType = (try cast(item.itype, IDynamicItem) catch(e:Dynamic) null).identifiedCopy();
    ...
    public function identifyAll() : Void
    {
        ...
        {
            outputText("\n");
            var item : ItemType = (try cast(slot.itype, IDynamicItem) catch(e:Dynamic) null).identifiedCopy();
    ...
    public function campMichikoTalkMarriage() : Void
    {
        ...
        if (flags[kFLAGS.MICHIKO_TALK_MARRIAGE] == 0)
        {
