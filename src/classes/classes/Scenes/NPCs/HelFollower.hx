...
class HelFollower extends NPCAwareContent
{
    public var helNofitMsg(get, never) : String;
    public var helAnalNofitMsg(get, never) : String;
    ...
    public function helCapacity() : Float
    {
        return 85;
    }
    private function get_helNofitMsg() : String
    {
        return "Req. a cock fitting " + helCapacity() + " area";
    }
    public function helAnalCapacity() : Float
    {
        var anal : Int = 85;
        ...
        if (flags[kFLAGS.HELIA_ANAL_TRAINING] >= 2)
        {
            anal += 300;
        }
        return anal;
    }
    private function get_helAnalNofitMsg() : String
    {
        return "Req. a cock fitting " + helAnalCapacity() + " area";
    }
    public function helAffection(diff : Float = 0) : Float
    {
        if (flags[kFLAGS.HEL_AFFECTION_FOLLOWER] > 70 && flags[kFLAGS.HEL_HARPY_QUEEN_DEFEATED] == 0)
        {
            flags[kFLAGS.HEL_AFFECTION_FOLLOWER] = 70;
        }
        if (flags[kFLAGS.HEL_AFFECTION_FOLLOWER] < 100 || (flags[kFLAGS.HEL_BONUS_POINTS] == 0 && diff < 0))
        {
            ...
            if (flags[kFLAGS.HEL_AFFECTION_FOLLOWER] >= 100)
            {
                flags[kFLAGS.HEL_AFFECTION_FOLLOWER] = 100;
            }
            if (flags[kFLAGS.HEL_AFFECTION_FOLLOWER] < 0)
            {
                flags[kFLAGS.HEL_AFFECTION_FOLLOWER] = 0;
            }
        }
        else if (followerHel() && !player.hasStatusEffect(StatusEffects.HeliaOff))
        {
            ...
            if (diff > 0)
            {
                ...
                {
                    flags[kFLAGS.HEL_BONUS_POINTS] = 150;
                }
                else if (diff < 0)
                {
                    if (flags[kFLAGS.HEL_BONUS_POINTS] < 0)
                    {
                        flags[kFLAGS.HEL_BONUS_POINTS] = 0;
                    }
                }
            }
        }
    ...
    private function iCantLetFireButtsRapeMyCampsButt() : Void
    {
        ...
        if (flags[kFLAGS.HEL_FOXY_FOURSOME_WARNED] > 0 || flags[kFLAGS.HEL_EDRYN_OFFER] > 0)
        {
            outputText("like usual");
        }
    ...
    public function helFollowersIntro() : Void
    {
        ...
        else if (isabellaFollower() && flags[kFLAGS.HEL_INTROS_LEVEL] < 2)
        {
            ...
            else
            {
                ...
                if (player.cor < 50)
                {
                    outputText("fall silent under their considering gazes.");
                }
                //(corruption = high enough to call bitches out when they're hypocrites)
                else
                {
                    outputText("glower balefully at them, as if to challenge either to invite your opinion of their own sexual 'résumé' with an ill-considered comment.");
                }
                ...
                if (isabellaAccent())
                {
                    outputText("<i>\"You are... velcome,\"</i> Isabella says before collecting some of the scattered belongings from the ground.  You continue your tour, now that the girls are...  not going to murder each other in the middle of the night, at least.");
                }
                else
                {
                    outputText("<i>\"You're... welcome,\"</i> Isabella says before collecting some of the scattered belongings from the ground.  You continue your tour, now that the girls are...  not going to murder each other in the middle of the night, at least.");
                }
    ...
    public function heliaFollowerMenu(display : Bool = true) : Void
    {
        if (display)
        {
            clearOutput();
        ...
        if (flags[kFLAGS.HEL_FOLLOWER_LEVEL] == 2)
        {
            if (flags[kFLAGS.HELIA_ANAL_TRAINING_OFFERED] == 0 && display)
            {
                ...
                {
                    sceneHunter.print("Check failed: your biggest dick must be bigger than her ass can take.");
                }
            }
            if (display)
            {
                outputText("You call your salamander lover over, and in a few moments Hel walks your way, hips and tail swaying gaily as she moves.  She wraps an arm around your shoulders, pressing her soft, warm body against yours, and grins.  <i>\"Heyya, lover mine.  You need anything?\"</i>");
            }
            menu();
            ...
            addButton(1, "Sex", heliaRoughSex).hint("Initiate sex with Helia.").disableIf(player.lust < 33, "Not aroused enough!");
            addButton(2, "Threesomes", heliaThreesomes).hint("Invite someone for threesomes activity with Helia!").disableIf(player.lust < 33, "Not aroused enough!");
            addButton(4, "Talk", heliaOptions).hint("Discuss with Helia about various topics.");
            addButton(5, "Spar", sparWithHeliaFirebuttsAreHot).hint("Do some quick fight sessions!").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, "Req. a proper sparring ring.").disableIf(helScene.pregnancy.isPregnant, "She's pregnant, what are you thinking about?");
            if (sceneHunter.mockFights)
            {
                addButton(10, "Mock Fight", helMockFight).disableIf(helScene.pregnancy.isPregnant, "She's pregnant, what are you thinking about?");
            }
            addButton(6, "Box", boxWithInCampHel).hint("Box with Helia and train your strength and toughness.").disableIf(helScene.pregnancy.isPregnant, "She's pregnant, what are you thinking about?");
            if ((flags[kFLAGS.HEL_LOVE] == 1 || flags[kFLAGS.HEL_LOVE] == -1) && !helPregnant() && flags[kFLAGS.HELSPAWN_AGE] == 0)
            {
                addButton(7, "Have A Kid", helSpawnScene.haveAKid).hint("Get Helia pregnant and start a family with her.").disableIf(player.cockThatFits(helCapacity()) < 0, helNofitMsg).disableIf(player.lust < 33, "Not aroused enough!");
            }
        ...
        else if (flags[kFLAGS.HEL_FOLLOWER_LEVEL] == 1)
        {
            if (flags[kFLAGS.HEL_HARPY_QUEEN_DEFEATED] == 1)
            {
                if (display)
                {
                    outputText("(You've reached the culmination of Helia's current storyline.  Stay tuned for more!)");
                }
            }
            else
            {
                if (display)
                {
                    outputText("You approach Hel as she's pacing around camp.  She's clad in her normal field attire: a simple scale bikini top and leather thong which supports her scimitar's scabbard.  Her cloak is loosely thrown over her shoulders, giving her a slight measure of protection from the mountain's harsh environs.");
                }
                if (display)
                {
    ...
    private function heliaOptions() : Void
    {
        ...
        {
            addButton(1, "Cuddle", hugASmokeyTail).hint("Cuddle with Helia and sleep with her.");
        }
        else
        {
            if (flags[kFLAGS.SLEEP_WITH] == "Helia")
            {
                addButton(1, "NoSleepWith", dontSleepWithHelia).hint("Stop sleeping with Helia, for now.", "No Sleep With");
            }
        ...
        {
            addButton(3, "Anal Train", heliaGapeSceneChoices).disableIf(player.biggestCockArea() <= helAnalCapacity(), "Req. a bigger dick.").disableIf(!player.hasItem(consumables.GOB_ALE, 1), "Requires a bottle of Goblin Ale.");
        }
        ...
        if (flags[kFLAGS.HELSPAWN_AGE] == 1)
        {
            addButton(7, flags[kFLAGS.HELSPAWN_NAME], helSpawnScene.playWithYourKid).hint("Spend some time with your salamander child.");
        }
        if (flags[kFLAGS.HEL_GUARDING] == 0)
        {
            addButton(8, "GuardCamp", helGuardToggle).hint("Request her to guard the camp every night.", "Guard Camp");
        }
        else
        {
            addButton(8, "NoGuarding", helGuardToggle).hint("Request her to stop guarding the camp.", "No Guarding");
        }
    ...
    private function sparWithHeliaFirebuttsAreHot() : Void
    {
        ...
        if (mocking)
        {
            outputText("  Maybe she'll even like to fight and fuck with you just like you did it before, in the wild?");
        }
    ...
    private function loseToSparringHeliaLikeAButtRapedChump() : Void
    {
        clearOutput();
        //If HP loss)
        if (player.HP < 1)
        {
            outputText("You collapse on the ground, overwhelmed by pain and exhaustion caused by the berserker's onslaught.");
        }
        else
        {
            outputText("Your arousal is too great, and your mind can no longer focus on anything but a desperate need for release.  Lust robs you of your will, and with buckling knees you collapse.");
        }
    ...
    //PC Whips Hel's Ass
    @:allow(classes.scenes.nPCs)
    private function PCBeatsUpSalamanderSparring() : Void
    {
        clearOutput();
        //(If HP loss)
        if (monster.HP <= monster.minHP())
        {
            outputText("Unable to withstand your onslaught, the salamander collapses to a knee, barely supporting her weight on her sword.");
        }
        else
        {
            outputText("Panting heavily, knees snaking, she collapses, heavily leaning upon her sword.");
        }
        
        outputText("\n\n<i>\"Come on, [name],\"</i> Hel groans, swaying unsteadily.  ");
        if (monster.HP <= monster.minHP())
        {
            outputText("<i>\"Kick my ass a little harder, why don't you?\"</i>");
        }
        else
        {
            outputText("<i>\"Turn me on till I can't think straight, why don't you?\"</i>");
        }
    ...
    private function talkToHel() : Void
    {
        ...
        if (flags[kFLAGS.FOLLOWER_HEL_TALKS] == 0)
        {
            ...
            if (player.tou < 50)
            {
                outputText("You cough and gag, eyes misting as the pure-grain booze burns your throat worse than hot coals.  Hel laughs riotously, slapping you on the back hard enough to cause booze to snort out through your nose.");
            }
            else
            {
                outputText("You knock back the stein easily, your super-human endurance keeping you from choking on the powerful brew.  Impressed, Hel gives you a high-five and throws her head back in a laugh.");
            }
        ...
        else if (flags[kFLAGS.FOLLOWER_HEL_TALKS] == 1)
        {
            ...
            if (shouldraFollower.followerShouldra() && !player.hasStatusEffect(StatusEffects.ShouldraOff))
            {
        ...
        else if (flags[kFLAGS.FOLLOWER_HEL_TALKS] == 4)
        {
            ...
            {
                outputText("fizzy, non-alcoholic beer ");
            }
            ...
            if (!helSpawnScene.helPregnant())
            {
                outputText(", her eyes crossing a little.  Looks like she was dipping in before you got here...");
            }
            else
            {
                outputText(".");
            ...
            if (!helSpawnScene.helPregnant())
            {
                ...
                {
                    outputText("  From across camp, you hear a certain dusky dragoness shout <i>\"HEY! That's MY trick, firebutt!\"</i>");
                }
                ...
                if (kihaFollower.followerKiha())
                {
                    outputText("further pissing off Kiha, who throws a rock at Hel, and ");
                }
        ...
        else if (flags[kFLAGS.FOLLOWER_HEL_TALKS] == 5 && player.gems >= 1 && isabellaFollower() && kihaFollower.followerKiha())
        {
            ...
            if (!isabellaAccent())
            {
                outputText(", obviously only somewhat interested");
            }
            outputText(".");
            ...
            //[Successful]{
                ...
                gems = as3hx.Compat.parseInt(40 + rand(40));
            }
            //[Fail 1]
            else if (rand(3) == 0)
            {
                ...
                gems = as3hx.Compat.parseInt(-5 - rand(10));
            }
            //[Fail 2]
            else if (rand(2) == 0)
            {
    ...
    //Rough Sex
    //Into Text
    private function heliaRoughSex(output : Bool = true) : Void
    {
        if (output)
        {
            clearOutput();
        ...
        sceneHunter.print("Centaur and naga scenes are different from 'fuckbuddy' Hel.")  //check cocks  ;
        
        var cockV : Int = player.cockThatFits(helCapacity());
        var cockA : Int = player.cockThatFits(helAnalCapacity());
        menu();
        addButton(0, "FuckHerVag", helScene.fuckHelsVag).hint("Penetrate her vaginally.").disableIf(cockV < 0, helNofitMsg);
        addButton(5, "FuckHerAss", helScene.fuckHelsAss).hint("Penetrate her anally.").disableIf(cockA < 0, helAnalNofitMsg);
        if (cockA == cockV)
        {
            cockA = player.cockThatFits2(helAnalCapacity());
        }  //find another one; fails if <2 cocks  
        addButton(10, "DoublePen", helScene.dpHel).hint("Fill both of her holes with your cocks.").disableIf(cockV < 0 || cockA < 0, helNofitMsg + "; " + helAnalNofitMsg);
        addButton(1, "Get Blown", helScene.helBlowsYou).hint("Have her suck you off..").disableIf(!player.hasCock(), "Req. a cock!");
        addButton(6, "Get Licked", helScene.getLickedByHel).hint("Have her lick your vagina.").disableIf(!player.hasVagina(), "Req. a vagina!");
        addButton(11, "Possess", heliaCampPossession).disableIf(!player.hasPerk(PerkLib.Incorporeality) || flags[kFLAGS.IZMA_NO_COCK], 
        ...
        addButton(2, "Tail Wank", helScene.helTailWank).hint("Have her jerk you off using her tail.").disableIf(!player.hasCock(), "Req. a cock!");
        addButton(7, "GetTailPegged", helScene.helTailPegging).hint("Have her peg your ass.");
        addButton(12, "Tentacles", heliaFollowerTentafuck).disableIf(player.countCocksWithType(CockTypesEnum.TENTACLE) < 7, "Req. at least 7 tentacle cocks!");
        addButton(3, "NagaCoilM", inCampHelNagaLuv).disableIf(!player.isNaga() || cockV < 0, 
                "Req. to have Naga lower body; " + helNofitMsg
        );
        addButton(8, "NagaCoilF", nagaCoilForHelCampWithGirls).disableIf(!player.isNaga() || !player.hasVagina(), "Req. to have Naga lower body and a vagina!");
        addButtonDisabled(13, "o====8", "Old sex scenes are still available from the 'Mock Fight'!\n\n-SceneHunter");
        addButton(4, "Mount Her", centaurMountsCampHel).disableIf(!player.isTaur() || cockV < 0, "Req. to have Taur lower body; " + helNofitMsg);
        addButton(9, "CentaurToy", femtaurPlusCampHel).disableIf(!player.isTaur() || !player.hasVagina() || player.hasKeyItem("Centaur Pole") < 0, 
    ...
    private function centaurMountsCampHel() : Void
    {
        ...
        {
            outputText(" but your body is too big, and you must content yourself to endure Hel's ministrations");
        }
        else
        {
            outputText(" but all you can do is take hold of your [chest] and tease and tweak your [nipples], getting what pleasure you can from your sensitive teats");
        }
        outputText(".");
        ...
        if (player.cockArea(x) > helCapacity())
        {
            outputText("she can cram in, the rest of your length twitching between her legs");
        }
        else
        {
            outputText("you have to offer her");
        }
    ...
    private function heliaFollowerTentafuck() : Void
    {
        ...
        {
            outputText("wriggling tentacles");
        }
    ...
    private function muddyLizardFeet() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText("her heel coming to caress your taut sack");
        }
        else if (player.hasVagina())
        {
            outputText("her heel brushing against the nub of your [clit]");
        }
        else
        {
            outputText("her heel going so low as to brush your taint, making your [asshole] clench in surprise before she moves back up your length");
        }
        ...
        if (player.biggestCockArea() > helCapacity())
        {
            outputText("you feel your [cockHead biggest] against the lips of her cervix, as fully into her as you can go");
        }
        else
        {
            outputText("your groin meets Helia's, your [cock] wholly enveloped within her");
        }
    ...
    private function takeABath() : Void
    {
        ...
        else
        {
            ...
            if ((Std.is(player.armor, GooArmor) || flags[kFLAGS.VALERIA_AT_CAMP] == 1) && player.lust >= 33)
            {
    ...
    public function heliaThreesomes() : Void
    {
        ...
        menu();
        addButton(0, "Kiha", campHelAndKihaThreeSome).disableIf(player.isGenderless(), "Not for genderless!").disableIf(!followerKiha(), "Why not lay a dragon?", "???");
        addButton(1, "Vapula", heliaAndVapula).disableIf(player.cockThatFits(helCapacity()) < 0, helNofitMsg).disableIf(!vapulaSlave(), "Find some succubus for your pleasure.", "???");
        addButton(2, "Valeria", helAndValeriaCampThreesomes).disableIf(!(Std.is(player.armor, GooArmor) || flags[kFLAGS.VALERIA_AT_CAMP] == 1), "Slime armor? What a bullshit.", "???");
        addButton(3, "Sophie", helAndSluttyHarpy).disableIf(!bimboSophie(), "What about bimbo birds?", "???");
        addButton(14, "Back", heliaFollowerMenu);
    }
    ...
    private function heliaAndVapula() : Void
    {
        ...
        if (player.cumQ() >= 750)
        {
            outputText(" until her belly swells and starts spurting ropes of jism back out of her love-tunnel");
        }
    ...
    private function campHelAndKihaThreeSome() : Void
    {
        ...
        if (player.gender == 1)
        {
            addButton(0, "Next", dudeHeliaAndKihaThreeSome);
        }
        else if (player.gender == 2)
        {
            addButton(0, "Next", girlsThreesomeHelAndKiha);
        }
        else
        {
    ...
    private function girlsThreesomeHelAndKiha() : Void
    {
        ...
        {
            outputText("  A shudder rocks you as the first trickles of milk seep from your teats, a small stream soon flowing out of you into the twin lizards' mouths.  Their cheeks bulge between swallows, but both coo happily as they suckle, their forked tongues lapping up every drop you have to offer.");
        }
        //{If PC has a dick:}
        if (player.hasCock())
        {
            outputText("  Even as the girls worship your [chest], you feel a sudden pressure on [oneCock].  Hel's tail deviates from your cunt for a moment, instead wrapping lovingly around your prick.  A groan escapes your lips as her hot leathery tail coils around your manhood before looping back to tease your clitty, ensuring that your dick won't be left out of the fun.  You smile and stroke her hair, eager to get to the main course.");
        }
    ...
    private function dudeHeliaAndKihaThreeSome() : Void
    {
        ...
        {
            outputText("the rest of your shaft, her own tongue coiling around your tool");
        }
        else
        {
            outputText("your second erection, taking it in both hands and giving it long, loving strokes before slurping up the [cockHead 1]");
        }
        outputText(".");
    ...
    private function helXValeriaFemalePC() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText(images.showImage("helia-follower-threesome-valeria-female"));
        }
        else
        {
            outputText(images.showImage("helia-follower-threesome-valeria-male"));
        }
        ...
        var partArray : Array<Dynamic> = [];
        partArray.push("[butt]");
        if (player.hasCock())
        {
            partArray.push("your [cock]");
        }
        if (player.hasVagina())
        {
            partArray.push("and the slick slit of your cunt");
        }
    ...
    private function heliaAnalTrainingPrompt() : Void
    {
        ...
        if (player.tallness < 60)
        {
            outputText("her bosom towering over you");
        }
        else if (player.tallness < 72)
        {
            outputText("towering over you");
        }
        else if (player.tallness < 90)
        {
            outputText("meeting you eye to eye");
        }
        else
        {
            outputText("looking up at you confidently");
        }
    ...
    public function giveHeliaAnalTraining() : Void
    {
        ...
        if (player.cockTotal() > 1)
        {
            outputText("selecting the biggest and heaviest to stroke as you near her");
        }
        else
        {
            outputText("grabbing it and stroking it as you near her");
        }
        ...
        if (player.hasBalls())
        {
            outputText("She tilts forward to press her chin into your [sack] and begins to lather your [balls] with frothy, warm, salamander spit. One hand hefts a weighty nut as she works, caressing it as if it would somehow coax the cum out of it. ");
        }
        ...
        if (player.hasBalls())
        {
            outputText(", hard enough to lift your nuts in their [sack]");
        }
        outputText(", and you erupt, cumming a steamy hot load of spunk straight onto Helia’s face, head, and back.");
        if (player.cumQ() < 250)
        {
            outputText(" You run out before long, but it’s still enough to make her a white painted mess. She licks the excess from her upper lip and shudders happily.");
        }
        else if (player.cumQ() < 2000)
        {
            outputText(" You spurt rope after rope onto her, actually drenching her hair with alabaster moisture. Her back and face look equally smothered with spooge when you finish. She’s happy to wipe the goop out of her eyes and shovel it into her mouth, swallowing it down with a satisfied smile.");
        }
        else
        {
            ...
            if (player.cumQ() >= 3000)
            {
                outputText(", but you keep cumming, cascading fresh waves onto her until there’s not a single part of her that isn’t stained and dripping with your salty outflow");
            }
    ...
    {
        if (flags[kFLAGS.HELIA_ANAL_TRAINING] == 0)
        {
            giveHeliaAnalTraining();
        }
        else if (flags[kFLAGS.HELIA_ANAL_TRAINING] == 1)
        {
    ...
    private function yesMakeHeliaAGapeSlut() : Void
    {
        ...
        if (player.cumQ() < 1000)
        {
            outputText(", ignoring the way your sheer volume of spunk leaks from the corners of her mouth and nose, her belly gradually rounding with cum in addition to thick dildo");
        }
        outputText(". The slutty salamander eventually pulls it off and ");
        if (player.cumQ() < 2000)
        {
            outputText("squeezes a last few dollops onto her face, letting them stain her with your scent.");
        }
        else
        {
            outputText("allows you to hose her down with your ongoing waves of salty jism, soaking her in your sexual scent.");
        }
    ...
    private function helPartyMenu() : Void
    {
        ...
        menu();
        //heliaDrinks
        if (flags[kFLAGS.HELIA_BDAY_DRINKS] == 0)
        {
            addButton(0, "Drinks", heliaDrinks);
        }
        //heliaHakonAndKiri
        if (flags[kFLAGS.HELIA_BDAY_HAKON_AND_KIRI] == 0)
        {
            addButton(1, "Hakon 'n Kiri", heliaHakonAndKiri);
        }
        //heliaPhoenixes
        if (flags[kFLAGS.HELIA_BDAY_PHOENIXES] == 0)
        {
            addButton(2, "Phoenixes", heliaPhoenixes);
        }
        //heliaAndFoxTwins
        if (flags[kFLAGS.HELIA_BDAY_FOX_TWINS] == 0)
        {
            addButton(3, "Fox Twins", heliaAndFoxTwins);
        }
        //leaveWithoutSex
        ...
        if (flags[kFLAGS.HELIA_BDAY_FOX_TWINS] > 0)
        {
            addButton(5, "Leave w/Girls", leaveWithGirls);
        }
    }
    ...
    private function goWithHelia() : Void
    {
        ...
        if (player.tallness < 84)
        {
            outputText("sweeping you off your feet");
        }
        else
        {
            outputText("hanging off you");
        }
        ...
        //If PC did UrtaQuest:
        outputText("\n\n");
        if (flags[kFLAGS.URTA_MET_HEL] == 1)
        {
            outputText("From the quick look she and the salamander exchange, you get the feeling they've met before. Weird. Still, ");
        }
        ...
        if (player.tallness <= 72)
        {
            outputText(", her head resting on your shoulder");
        }
        ...
        if (flags[kFLAGS.HARPY_QUEEN_EXECUTED] > 0)
        {
            outputText("who forgave us for the queen's death");
        }
        else
        {
            outputText("who wanted to be friends still, even after the Tower");
        }
        ...
        if (flags[kFLAGS.HARPY_QUEEN_EXECUTED] > 0)
        {
            outputText("finally killing that bitch of a queen");
        }
        else
        {
            outputText("setting the queen right, but leaving her alive for when it's finally time to bring the fight to Lethice");
        }
        ...
        {
            outputText("a certain waitress fairy having... supplied the icing herself");
        }
        else
        {
            outputText("a certain minotaur chef having supplied the icing himself");
        }
        ...
        if (flags[kFLAGS.HEL_FOXY_FOURSOME_WARNED] == 1)
        {
            outputText("a pair of lovely fox-eared herms");
        }
        else
        {
            outputText("Miko and Mai");
        }
    ...
    private function heliaPhoenixes() : Void
    {
        ...
        {
            outputText(", then to the fiery dragoness and the ultra-busty cowgirl talking in the corner");
        }
    ...
    private function leaveWithGirls() : Void
    {
        ...
        {
            doNext(helScene.foxyFluffGirlsFuckSex);
        }
        else
        {
            doNext(helScene.foxyFluffsFoursomeAsMale);
        }
    }
    ...
    private function helAndSluttyHarpyMale() : Void
    {
        ...
        {
            outputText(" no longer be hilted inside");
        }
        else
        {
            outputText(" slip out of");
        }
        ...
        if (player.cocks.length == 1)
        {
            ...
            {
                outputText(" Your unused [vagina] cums with your cock in a show of sympathetic orgasm, coating your thighs with femcum.");
            }
        }
        // 2-3 cocks
        else if (player.cocks.length <= 3)
        {
            ...
            else
            {
                ...
                if (player.cumQ() <= 1000)
                {
                    ...
                    if (player.cumQ() >= 5000)
                    {
                        outputText(", pooling around your knees");
                    }
                }
            }
            outputText(".");
            ...
            {
                outputText(" Your unused cock splatters the two with hot, white jizz.");
            }
            if (player.vaginas.length > 0)
            {
                outputText(" Your unused [vagina] cums with your cocks in a show of sympathetic orgasm, coating your thighs with femcum.");
            }
        }
        // > 4 cocks
        else
        {
            
            {
                ...
                else if (flags[kFLAGS.HELIA_ANAL_TRAINING] >= 2)
                {
                    outputText(" gaped");
                }
                ...
                {
                    outputText(" Your extra cocks spray pearly cum all over the nearby ground.");
                }
                if (player.vaginas.length > 0)
                {
                    outputText(" Your unused [vagina] cums with your cocks in sympathetic orgasm, coating your thighs with femcum.");
                }
            }
        }
        // Merge
    ...
    public function mishapsLunaHel() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.CampLunaMishaps2, 3, 1);
        }
        else
        {
            player.createStatusEffect(StatusEffects.CampLunaMishaps2, 0, 0, 1, 0);
        }
