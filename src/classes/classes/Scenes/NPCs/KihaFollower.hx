...
class KihaFollower extends NPCAwareContent implements TimeAwareInterface implements SaveableState
{
    ...
    public static var FlameSpreaderBossKilled : Bool;
    ...
    public static var SidonieSeen : Bool;
    ...
    //
    public static var TalkedAfterLethice : Bool;
    public static var CanVisitKids : Bool;
    public static var EmberInteractionHappened : Bool;
    public static var SidonieInteractionHappened : Bool;
    public static var BelisaInteractionHappened : Bool;
    public static var ElectraInteractionHappened : Bool;
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            DergKidnapped = Reflect.field(o, "DergKidnapped");
            KihaTownState = Reflect.field(o, "KihaTownState");
            FlameSpreaderKillCount = Reflect.field(o, "FlameSpreaderKillCount");
            FlameSpreaderBossKilled = Reflect.field(o, "FlameSpreaderBossKilled");
            ProposalStatus = Reflect.field(o, "ProposalStatus");
            TyrantiaRelationshipStatus = Reflect.field(o, "TyrantiaRelationshipStatus");
            SidonieSeen = Reflect.field(o, "SidonieSeen");
            BelisaRelationshipStatus = Reflect.field(o, "BelisaRelationshipStatus");
            TalkedAfterLethice = (Lambda.has(o, "TalkedAfterLethice")) ? Reflect.field(o, "TalkedAfterLethice") : false;
            CanVisitKids = (Lambda.has(o, "CanVisitKids")) ? Reflect.field(o, "CanVisitKids") : false;
            EmberInteractionHappened = (Lambda.has(o, "EmberInteractionHappened")) ? Reflect.field(o, "EmberInteractionHappened") : false;
            SidonieInteractionHappened = (Lambda.has(o, "SidonieInteractionHappened")) ? Reflect.field(o, "SidonieInteractionHappened") : false;
            BelisaInteractionHappened = (Lambda.has(o, "BelisaInteractionHappened")) ? Reflect.field(o, "BelisaInteractionHappened") : false;
            ElectraInteractionHappened = (Lambda.has(o, "ElectraInteractionHappened")) ? Reflect.field(o, "ElectraInteractionHappened") : false;
        }
        else
        {
            resetState();
    ...
    public function timeChange() : Bool
    {
        ...
        if (pregnancy.isPregnant && kihaPregUpdate())
        {
            needNext = true;
        }
        if (flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] > 1)
        {
            if (flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] != 144)
            {
                flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER]--;
            }
            if (flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] == 240 && flags[kFLAGS.IN_INGNAM] == null)
            {
                kihaBreastfeedingTime();
                needNext = true;
            }
            else if (flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] == 72 && flags[kFLAGS.IN_INGNAM] == null)
            {
                kihaTrainsHerKids();
                needNext = true;
            }
        }
        else if (flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] == 1 && flags[kFLAGS.IN_INGNAM] == null)
        {
    ...
    public function kihaSprite(nude : Bool = false) : Void
    {
        if (!pregnancy.isPregnant)
        {
            if (nude)
            {
                spriteSelect(SpriteDb.s_kiha_nude);
            }
            else
            {
        ...
        {
            spriteSelect(SpriteDb.s_kiha_nude_preg);
        }
        else
        {
    ...
    override public function followerKiha() : Bool
    {
        return flags[kFLAGS.KIHA_FOLLOWER] > 0 && flags[kFLAGS.KIHA_CORRUPTION_BITCH] != 1;
    }
    
    private function kihaAffection(changes : Float = 0) : Bool
    {
        if (flags[kFLAGS.KIHA_AFFECTION_LEVEL] == 2)
        {
            flags[kFLAGS.KIHA_AFFECTION] += changes;
        }
        if (flags[kFLAGS.KIHA_AFFECTION] < 0)
        {
            flags[kFLAGS.KIHA_AFFECTION] = 0;
        }
        if (flags[kFLAGS.KIHA_AFFECTION] > 100)
        {
    ...
    private function canKihaGetPregnant() : Bool
    {
    ...
    //Call off the pregnancy!
    {
        ...
        chance += as3hx.Compat.parseInt(player.virilityQ() * 100);
        chance -= as3hx.Compat.parseInt(totalKihaChildren() / 5);
        if (chance < 10)
        {
            chance = 10;
        }
    ...
    {
        return as3hx.Compat.parseInt(flags[kFLAGS.KIHA_CHILDREN_BOYS] + flags[kFLAGS.KIHA_CHILDREN_GIRLS] + flags[kFLAGS.KIHA_CHILDREN_HERMS]);
    }
    
    //Introduction
    @:allow(classes.scenes.nPCs)
    private function kihaSpiderEventIntro() : Void
    {
        clearOutput();
        ...
        if (player.cor < 66)
        {
            outputText("You try to calm the dragoness down, explaining that you aren't one of Lethice's minions, that what you told her the first time around was true - that you're here to end the demon threat.  Kiha doesn't seem to be buying it, though… \n\n");
        }
        else
        {
            outputText("Irate, you stare her down, and with a dangerous gentleness, remind her that you're here to crush the demons.  Your fist tightens as, not unexpectedly, the erratic woman appears to hear none of it.\n\n");
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function loseKihaPreSpiderFight() : Void
    {
        clearOutput();
    ...
    @:allow(classes.scenes.nPCs)
    private function playerBeatsUpKihaPreSpiderFight() : Void
    {
        clearOutput();
    ...
    public function beatSpiderMob() : Void
    {
        ...
        if (player.cor < 50)
        {
            outputText("squeeze her shoulder and ");
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function kihaFriendlyGreeting(output : Bool = true) : Void
    {
        if (output)
        {
            clearOutput();
        ...
        menu();
        if (flags[kFLAGS.KIHA_TALK_STAGE] < 6)
        {
            addButton(0, "Talk", talkToFriendlyKiha).hint("Talk to Kiha. She might not like it but you might make some progress.");
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function winSparWithKiha() : Void
    {
        clearOutput();
        if (!followerKiha())
        {
    ...
    @:allow(classes.scenes.nPCs)
    private function sparWithFriendlyKihaLose() : Void
    {
        clearOutput();
        if (!followerKiha())
        {
    ...
    private function lieThere() : Void
    {
        ...
        outputText("[pg]\"<i>You BITCH!</i>\"  Kiha screams, flicking mud off her " + ((flags[kFLAGS.KIHA_UNDERGARMENTS] != null) ? "nearly-" : "") + "nude body.  \"<i>How dare you?  How DARE YOU throw ME in the MUD!?  I'll fucking teach you!</i>\"");
        ...
        if (!player.isGoo())
        {
            outputText("<i>meatsack</i>");
        }
        else
        {
            outputText("<i>goosack</i>");
        }
    ...
    private function jumpDaBitches() : Void
    {
        ...
        if (player.isBiped())
        {
            outputText("stride over");
        }
        else if (player.isNaga())
        {
            outputText("slither over");
        }
        else
        {
            outputText("move toward her");
        }
        ...
        {
            outputText("bends down her throat");
        }
        else if (player.cocks[0].cockLength > 6)
        {
            outputText("hits the back of her throat");
        }
        else
        {
            outputText("rests right on the tip of her tongue");
        }
        ...
        {
            outputText("[pg]Deciding to use Hel's mouth a bit more, you grab your [cock 2] out of your armor and press its head against Hel's lips.  She starts to protest, but just like you did with Kiha, as soon as she opens her mouth to complain you ram your secondary cock in, burying its length in her face.  She gags and gasps, but you just grab both girls by the hair and start to fellate yourself with them, ramming their faces down your cocks until you're a spit-slicked mess, until both girls have become completely compliant, simply allowing you to use them.");
        }
        ...
        if (player.biggestCockArea() < 36)
        {
            outputText("up to the hilt");
        }
        else
        {
            outputText("until she simply can't take any more of you, her anus already stretched beyond its capacity");
        }
        ...
        if (player.cumQ() >= 1000)
        {
            outputText(" filling her so utterly that your cum squirts back out her asshole around your cock");
        }
    ...
    private function lovinHugKiha() : Void
    {
        ...
        if (player.tallness >= 95)
        {
            outputText("pulling it down");
        }
        else if (player.tallness <= 60)
        {
            outputText("tilting it up");
        }
        else
        {
            outputText("tilting it slightly");
        }
        ...
        if (player.hasCock())
        {
            outputText("[pg][EachCock] springs free, smacking into Kiha's thigh as it fully engorges.  She gives it a gentle pump before commenting, \"<i>Acceptable, I suppose.</i>\"");
        }
        else if (player.hasVagina())
        {
            outputText("[pg]Your [vag] tingles as it's exposed to the air.  The lusty dragon-maid dips a digit inside your depths before commenting, \"<i>Not bad, I guess.</i>\"");
        }
        else
        {
            outputText("[pg]Your smooth groin and [asshole] tingle as they're exposed to the open air.  Kiha gently caresses your smooth skin all the way down your taint before commenting, \"<i>Not ideal, but I can make do.</i>\"");
        }
        ...
        if (player.canFly())
        {
            outputText("You could fly yourself, but you hang on for now, enjoying the embrace.");
        }
        else
        {
            outputText("You've no choice but to hang on to her for dear life as she lifts off.");
        }
    ...
    private function lovingHugDickings() : Void
    {
        ...
        {
            outputText("How do you walk with this thing swinging around everywhere, bludgeoning into everyone you meet?");
        }
        else if (player.biggestCockArea() > 9)
        {
            outputText("How do you walk with this thing sticking out all the time, or was that just for me?");
        }
        else
        {
            outputText("How can you enjoy sex with something that small?  I don't know if I'll even feel it.");
        }
        ...
        if (player.cocks[player.biggestCockIndex()].cockLength >= 36)
        {
            outputText(", even though you bump her repeatedly in the nose with your moist tip");
        }
        ...
        outputText("[pg]");
        if (player.isRace(Races.HORSE, 1, false) || player.isRace(Races.DOG, 1, false) || player.isRace(Races.CAT, 1, false))
        {
            outputText("Growling");
        }
        else
        ...
        {
            outputText("hilted inside her soaked love-tunnel,");
        }
        else
        {
            outputText("as far in as your prodigious size with allow,");
        }
        ...
        if (player.cumQ() >= 500)
        {
            outputText("  Jet after jet of your creamy batter soaks the reptilian womb in spunk, the proof of your virility.");
        }
        if (player.cumQ() >= 1000)
        {
            ...
            if (player.cumQ() <= 2000)
            {
                outputText("fill Kiha's bed");
            }
            else
            {
                outputText("turn Kiha's abode into your spermy swimming pool");
            }
            outputText(".");
        }
    ...
    private function lovingHugsForRetards() : Void
    {
        ...
        {
            outputText("in mixed pain and pleasure");
        }
        else
        {
        ...
        {
            outputText("you're sure you'll gape for a little after this");
        }
        else
        {
            outputText("you're sure you'll gape even wider for a little bit after this");
        }
    ...
    private function warmLoverKihaIntro(output : Bool = true) : Void
    {
        menu();
        ...
        addButton(3, "Sex", kihaSexMenu).hint("Initiate sex session with Kiha.").disableIf(player.lust < 33, "Not aroused enough!");
        addButton(4, "Spar", sparWithKiha).hint("Do some quick battle with Kiha!").disableIf(followerKiha() && flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, 
        ...
        if (followerKiha())
        {
            if (output)
            {
                if (flags[kFLAGS.LETHICE_DEFEATED] != null && !TalkedAfterLethice)
                {
                    KihaAfterLethice();
                    return;
                }
                ...
                else if (model.time.days % 15 == 0 && flags[kFLAGS.KIHA_PREGNANCY_POTENTIAL] == 0)
                {
                    kihasFirstClutch();
                }
                else if (TyrantiaFollower.isLover() && rand(10) == 0)
                {
                    tyrantiaInteraction(false);
                }
                else if (flags[kFLAGS.SIDONIE_FOLLOWER] == 1 && SidonieInteractionHappened && rand(9) == 0)
                {
                    sidonieInteraction(false);
                }
                else if (BelisaFollower.BelisaInCamp && BelisaInteractionHappened && rand(8) == 0)
                {
                    belisaInteraction(false);
                }
                else if (flags[kFLAGS.ELECTRA_FOLLOWER] > 1 && !player.hasStatusEffect(StatusEffects.ElectraOff) && ElectraInteractionHappened && rand(8) == 0)
                {
                    electraInteraction(false);
                }
                else
                {
            ...
            {
                addButton(5, "Team", kihaHenchmanOption).hint("Ask Kiha to join you in adventures outside camp.");
            }
            else
            {
            ...
            {
                addButton(6, "Stop Guard", guardMyCampKiha).hint("Request her to stop guarding.");
            }
            else
            {
                addButton(6, "Guard Camp", guardMyCampKiha).hint("Request her to guard your camp.");
            }
            if (CorruptedGlade.canBeDestroyed())
            {
                if (flags[kFLAGS.KIHA_DESTROYING_CORRUPTED_GLADES] == 0)
                {
                    addButton(7, "Destroy Glades", kihaDestroyGladesToggle).hint("Request Kiha to destroy any corrupted glades she can find.");
                }
                else
                {
                    addButton(7, "Stop Destroying", kihaDestroyGladesToggle).hint("Request Kiha to stop destroying the corrupted glades.");
                }
            }
            addButton(8, "Dominate", dominateKiha).hint("Try to turn the tables and dominate her instead! Will she agree, or… ?").disableIf(player.isGenderless(), "Not for genderless!");
            if (sceneHunter.mockFights)
            {
                addButton(9, "Mock-Rape", mockRapeKiha).hint("Do things as in the old days and challenge her for a fight! The loser gets fucked!").disableIf(player.isGenderless(), "Not for genderless!");
            }  //avoiding weirdness in defeat scene  
            addButton(10, "Talk", TalkWithKiha).hint("Talk to Kiha… but would she want to talk?");
            if (CanVisitKids)
            {
                addButton(11, "Kids", visitKids);
            }
            if (flags[kFLAGS.KIHA_UNDERGARMENTS] == 0)
            {
                addButton(12, "Give Underwear", giveKihaUndergarmentsPrompt).hint("Give Kiha something to wear to conceal her nether regions?").disableIf(!player.hasItem(undergarments.SSPANTY) && !player.hasItem(undergarments.SS_LOIN), 
    ...
    private function hangOutWithKiha() : Void
    {
        ...
        function walk() : Void
        {
            ...
            if (!followerKiha())
            {
                outputText("through the dense overgrowth of the swamp");
            }
            else
            {
                outputText("back to her old haunt in the swamp");
            }
            outputText(".");
            ...
            if (!followerKiha())
            {
                outputText("you're back at Kiha's little islet, waving to the dragoness as she flies back home and you head to camp");
            }
            else
            {
                outputText("you're walking arm in arm again, heading home");
            }
            outputText(".");
            endEncounter();
            kihaAffection(5);
        ...
        function dinner() : Void
        {
            ...
            if (!player.canFly())
            {
                outputText("  She slips her arms around you and takes off, flying through the roof-entrance to her lair");
            }
            else
            {
                outputText("  Extending a hand to you, the two of you fly up to and through the roof entrance to her lair");
            }
            outputText(".");
        ...
        var sky : Void->Void = function() : Void
        {
            clearOutput();
            ...
            if (!followerKiha())
            {
                ...
                {
                    outputText("particularly cloudy day above you, with great pink and purple clouds drifting across the demonically-tainted horizon");
                }
                else
                {
                    outputText("stars are out, a thousand little pin-pricks in the heavens");
                }
                outputText(".");
            }
            else
            {
                ...
                if (model.time.hours < 21)
                {
                    outputText("day around your camp - thick pink and purple clouds are rolling lazily over the wasteland, shimmering behind the great maw of the portal");
                }
                else
                {
                    outputText("night in the wastelands.  The stars are out, as they always are at night in your part of Mareth, a thousand of little pin-pricks in the heavens");
                }
                outputText(".");
            }
    ...
    private function blechKihaYourCooking() : Void
    {
        ...
        if (player.canFly())
        {
            outputText("fly off.  Scowling, Kiha takes to the air too, landing behind you outside");
        }
        else
        {
            outputText("demand to be taken outside.  Grudgingly, Kiha walks over, grabs you by the waist, and hauls you outside");
        }
    ...
    private function kihaSexMenu(allowBack : Bool = true) : Void
    {
        menu();
        addButton(0, "Anal", savinTheAnalForKiha).disableIf(player.cockThatFits(94) < 0, "You're too big to fuck her ass."
                + "\n\n<b>Req. a dick fitting 94 area.</b>");
        addButton(1, "Get HJ", kihaPlaysWithBigassCocksFemDomAhoy).disableIf(!player.hasCock() || player.biggestCockArea() < 150, 
        ...
        if (followerKiha())
        {
            addButton(2, "FuckVag", fuckKihasVagInCamp).disableIf(player.cockThatFits(67) < 0, "Kiha's vagina is too small and tight for you to take."
            ...
            addButton(3, "LustyFuck", boneTheShitOutofKihaHolesWithHorsecock).disableIf(player.findCock(1, 40, 200) < 0 || player.minLust() < 50 && player.lib < 80 && !player.hasItem(consumables.L_DRAFT), lustyReject);
            addButton(4, "TentacleFuck", fuckKihaWithATentacle).disableIf(player.countCocksWithType(CockTypesEnum.TENTACLE, 18, -1, "length") == 0, 
                    "<b>Req. at least one tentacle cock longer than 18 inches.</b>"
            );
            addButton(6, "Give I.Draft", giveKihaIncubusDraft).hint("You could slip her an incubi draft and let her fuck your ass with it.").disableIf(!player.hasItem(consumables.INCUBID) && !player.hasItem(consumables.P_DRAFT), 
            ...
            addButton(7, "GroPlusTits", ghostboobiesKiha).hint((player.hasPerk(PerkLib.Incorporeality)) ? 
                            "You could try and pump her boobs a bit with Gro+, and if she decides against it, possess her and do it anyway!" : "You could see if she'd let you pump her boobs with Gro+.").disableIf(!player.hasItem(consumables.GROPLUS), "If you had some Gro+, you could ask her about making her breasts bigger.");
        }
        ...
        if (allowBack)
        {
            addButton(14, "Back", kihaScene.encounterKiha);
        }
        else
        {
    ...
    private function boneTheShitOutofKihaHolesWithHorsecock() : Void
    {
        ...
        kihaSprite(true);
        var c : Float = player.cumQ();
        var x : Float = player.findCockWithType(CockTypesEnum.HORSE, 1, 40, 200);
        if (x < 0)
        {
        ...
        {
            outputText("half-turgid");
        }
        else
        {
            outputText("always semi-hard");
        }
        outputText(" length of your ");
        if (player.cocks[x].cockType == CockTypesEnum.HORSE)
        {
            outputText("fat horse-cock");
        }
        else
        {
        ...
        if (player.isTaur())
        {
            outputText(" beneath your equine body");
        }
        ...
        if (player.isTaur())
        {
            outputText(", crawling under your horse body");
        }
        outputText(", the better to examine your ");
        if (player.cocks[x].cockType == CockTypesEnum.HORSE)
        {
            outputText("bestial equine dong");
        }
        else
        {
            outputText("[cock " + x1 + "]");
        ...
        if (player.hasSheath())
        {
            outputText("She pumps you just above the sheath");
        }
        else
        {
            outputText("She pumps your shaft");
        }
        outputText(" with one hand, using slow, even strokes that tickle every sensitive area.  ");
        if (player.cocks[x].cockType == CockTypesEnum.HORSE)
        {
            outputText("Her other caresses the skin of your sheath, fingers slipping in between the folded outer skin and your dick's concealed, untouched flesh.  ");
        }
        else if (player.hasBalls())
        {
            outputText("Her other fondles your [balls], steadily increasing your desire for release.  ");
        }
        else
        {
            outputText("Her other fondles your taint, steadily increasing your desire for release.  ");
        }
        ...
        if (player.cocks[x].cockType == CockTypesEnum.HORSE || player.isTaur())
        {
            outputText("whinny ");
        }
        else
        ...
        if (horse)
        {
            outputText("you catch yourself before you start braying like an animal");
        }
        else
        {
            outputText("you catch yourself before it manages to escape your lips");
        }
        ...
        if (player.cocks[x].cockType == CockTypesEnum.HORSE)
        {
            outputText("her free hand eagerly caressing you inside your own sheath with inhuman warmth");
        }
        else if (player.hasBalls())
        {
            outputText("her tongue working as much of your shaft as possible with inhuman warmth.");
        }
        else
        {
            outputText("her free hand eagerly caressing your [balls] with inhuman warmth.");
        }
        ...
        if (player.cocks[x].cockType == CockTypesEnum.HORSE)
        {
            outputText("big, pony-like prick.");
        }
        else
        {
            outputText("[cock " + x1 + "].");
        ...
        if (player.hasBalls())
        {
            outputText("  Your [balls] smear and shudder against her chin, so tight, hot, and ready to explode.");
        }
        outputText("[pg]Kiha ");
        if (player.hasBalls())
        {
            outputText("squeezes your [sack]");
        }
        else if (player.hasVagina())
        {
            outputText("fondles your [vag]");
        }
        else
        {
            outputText("gives her tongue an encouraging twist");
        }
        ...
        if (c <= 250)
        {
            outputText("She manages to guzzle it all down without too much difficulty.");
        }
        else if (c <= 1000)
        {
            outputText("Her eyes water, and trickles of cum run from her nose, but she still manages to gulp most of it down.");
        }
        else
        {
            ...
            if (player.cocks[x].cockType.Index > 0 && player.cocks[x].cockType.Index < 10)
            {
                outputText("animal ");
            }
        ...
        if (horse)
        {
            if (player.isTaur())
            {
                outputText("clambers out from beneath you, pulling herself onto a nearby rock, putting herself into position for your horse-body to mount her. She spreads her legs wide, her slick vagina releasing a tantalizing musk that begs for a cock to fill it");
            }
        }
        else
        {
            outputText("leans back against a rock and spreads her legs, exposing you to a tantalizing, cock-filling scent");
        }
        ...
        if (player.cocks[x].cockType.Index > 0)
        {
            if (player.isTaur())
            {
                outputText("throwing your hooves onto the rock, looming over Kiha");
            }
            else
            {
                outputText("fondling your bestial tool with one hand");
            }
        }
        else
        {
        ...
        if (horse)
        {
            outputText("\"<i>Fuck, am I… rutting?!</i>\"  ");
        }
        else
        {
            outputText("\"<i>Fuck, why am I… ?!</i>\"  ");
        }
        ...
        if (player.isTaur())
        {
            outputText("punches your underside");
        }
        else
        {
            outputText("tries to push you back");
        }
        ...
        if (player.cocks[x].cockType == CockTypesEnum.HORSE)
        {
            outputText("wide head of your stallion-shaft");
        }
        else
        {
            outputText(player.cockHead(x) + " of your " + cockDescript(x));
        }
        ...
        if (player.cocks[x].cockType == CockTypesEnum.HORSE)
        {
            outputText("well-lubed horse-dick");
        }
        else
        {
            outputText(cockDescript(x));
        ...
        if (player.cocks[x].cockType == CockTypesEnum.HORSE)
        {
            outputText("flat-tipped horsecock");
        }
        else
        {
            outputText(cockDescript(x));
        ...
        {
            outputText(" to plow your favorite dragon-hole");
        }
        ...
        if (player.balls)
        {
            outputText("  You slap your [balls] into in her ass hard as you thrust, smearing them all over her sweaty ass and tail.");
        }
        else
        {
            outputText("  You slap into her hard as you thrust, feeling her tail thrashing beneath your [butt].");
        }
        ...
        if (c <= 250)
        {
            outputText(", stuffed up with hot cum");
        }
        else if (c <= 1500)
        {
            outputText(", her belly inflating to pregnant proportions");
        }
        else if (c <= 3000)
        {
            outputText(", her belly inflating to pregnant equine proportions");
        }
        else
        {
            outputText(", her belly distending hugely, grossly even, utterly filled with cum until it's squirting from her pussy in a river of white");
        }
        outputText(".");
        ...
        if (player.isTaur())
        {
            outputText(" until the shaft bumps your belly");
        }
        outputText(".  You've just started to breed this bitch!");
        outputText("[pg]Kiha's ");
        if (c > 250)
        {
            outputText("cum-gravid ");
        }
        else
        {
            outputText("spunk-stuffed ");
        }
        ...
        if (player.isTaur())
        {
            outputText("you push her onto her hands and knees atop the rock and re-mount her, and ");
        }
        ...
        if (player.cocks[x].cockType == CockTypesEnum.HORSE)
        {
            outputText("The unique shape of your cockhead makes such penetration difficult, but with animalistic stubbornness, you keep at it.");
        }
        else
        {
            outputText("The sheer size of your cockhead makes such penetration difficult, but with animalistic stubbornness, you keep at it.");
        }
        ...
        if (player.isTaur())
        {
            outputText("[pg]You reach down and grab Kiha's tail, providing ");
        }
        else
        {
            outputText("[pg]The dragon's limp tail provides ");
        }
        ...
        if (!player.isTaur())
        {
            outputText("You run a hand over her rump, stopping only to give it familiar slaps as x1");
        }
        else
        {
            outputText("Y");
        ...
        if (player.hasSheath())
        {
            outputText("to your sensitive sheath");
        }
        else
        {
            outputText("to your sensitive loins");
        }
        ...
        if (horse)
        {
            outputText("the unquestionable authority of a herd alpha.");
        }
        else
        {
            outputText("an unquestionable air of authority.");
        }
        ...
        if (c >= 3000)
        {
            outputText("[pg]A torrent of cum runs from her overfilled butthole and cunt.  It seems there just isn't enough room inside her to hold it all, and with two plus-sized loads in her bottom holes, most of it has to escape.  Even with that, she still looks heavily pregnant, a perfect tribute to your unholy virility.");
        }
        else if (c >= 1000)
        {
            outputText("[pg]A wash of cum runs from her overfilled butthole and cunt.  Even with that, she still looks heavily pregnant, a perfect tribute to your potent virility.");
        }
        else
        {
            outputText("[pg]Trickles of cum run from her butthole and well-ravished cunt, a potent reminder of your virility and the dominance you have over your females.");
        }
        ...
        if (flags[kFLAGS.KIHA_HORSECOCK_FUCKED] == 0)
        {
            ...
            {
                outputText("  Exgartuan suggests, \"<i>She probably loved it if she had as much fun as I did.</i>\"");
            }
        ...
        {
            outputText("[pg]It happened again!  You shudder and stroke yourself, the unmistakable desire to do it again already cementing itself in your brain.  Well, at least Kiha doesn't mind too much.");
        }
        if (player.minLust() < 50 && player.lib < 80)
        {
    ...
    @:allow(classes.scenes.nPCs)
    private function kihaReactsToHorseDicking() : Void
    {
    ...
    private function kihaPlaysWithBigassCocksFemDomAhoy() : Void
    {
        ...
        {
            outputText(" for a moment before climbing higher.  It quickly blocks your view of your lover.");
        }
        else
        {
            outputText(" for a moment before thickening and twitching against her.");
        }
        ...
        if (player.cocks[x].cockLength > 48)
        {
            outputText("bigger than");
        }
        else
        {
            outputText("the size of");
        }
        ...
        if (player.cocks[x].cockType == CockTypesEnum.HORSE)
        {
            outputText(", particularly your medial ring and the sensitive edge of your flare.");
        }
        else if (player.cocks[x].cockType == CockTypesEnum.DEMON)
        {
            outputText(", particularly the hundreds of demonic nodules that ring your shaft.");
        }
        else if (player.cocks[x].cockType == CockTypesEnum.ANEMONE)
        {
            outputText(", particularly the groups of aphrodisiac laced tentacles under your crown.");
        }
        else if (player.cocks[x].cockType == CockTypesEnum.TENTACLE)
        {
            outputText(", particularly the underside of your tentacle's mushroom-like tip.");
        }
        else if (player.cocks[x].cockType == CockTypesEnum.CAT)
        {
            outputText(", particularly the hundreds of rubbery spines covering your feline maleness.");
        }
        else if (player.hasKnot(x))
        {
            outputText(", particularly your massive, bulging canid knot.");
        }
        else
        {
            outputText(".");
        ...
        if (player.cocks[x].cockLength > 48)
        {
            outputText("pulls your cockhead down and ");
        }
        ...
        else
        {
            ...
            if (player.hasSheath())
            {
                outputText(", just above the sheath, ");
            }
            else
            {
                outputText(" ");
        ...
        if (player.hasBalls())
        {
            outputText("Your balls clench and relax in time with the waves of pleasure, and y");
        }
        else
        {
            outputText("Y");
        ...
        if (player.cumQ() >= 1000)
        {
            outputText(", effectively bukkaking her from the sheer volume of your ejaculate");
        }
        ...
        if (player.cumQ() >= 2000)
        {
            outputText("Wave after wave of spunk rains down over you, forming a gratuitous puddle and utterly, completely sliming you both.  ");
        }
        ...
        if (flags[kFLAGS.IZMA_CHILDREN_TIGERSHARKS] > 0)
        {
            if (player.cor < 50)
            {
                outputText("  You hope she doesn't get jumped upby your tiger-shark children.");
            }
            else
            {
                outputText("  You wonder what would happen if she got jumped by your tiger-shark children.");
            }
        }
        ...
        if (monk >= 5 && !player.hasStatusEffect(StatusEffects.NoJojo) && flags[kFLAGS.JOJO_DEAD_OR_GONE] == 0)
        {
            ...
            {
                outputText(", ignoring the sounds of Jojo feverishly masturbating in the woods");
            }
            else
            {
                outputText(", ignoring the sound of Jojo vigorously fucking himself with all his tentacles in the trees");
            }
        }
        outputText(".");
        if (sophieBimbo.bimboSophie())
        {
            outputText("  Sophie greets you when you return, though she seems crestfallen once she smells the sex on you and realizes how thoroughly you've been sated.");
        }
        if (amilyScene.amilyCorrupt())
        {
            outputText("  Amily begs, \"<i>May I help to service you next time, [master]?</i>\"");
        }
        player.sexReward("no", "Dick");
        dynStats("sen", -1);
        endEncounter();
    ...
    private function kihaGirlGirlSex() : Void
    {
        ...
        if (player.clitLength >= 10)
        {
            outputText(", gasping when it grows to a ridiculous size.");
        }
        else if (player.clitLength >= 3)
        {
            outputText(", humming appreciably when it grows to cock-like length.");
        }
        else
        {
            outputText(", humming happily when it stiffens against her.");
        }
    ...
    private function savinTheAnalForKiha() : Void
    {
        ...
        kihaSprite(true);
        var x : Float = player.cockThatFits(94);
        if (x < 0)
        {
            x = 0;
        }
        ...
        {
            outputText("the small of her back ");
        }
        ...
        {
            outputText("; you pull her cheeks apart just long enough to slip your prick between them, letting Kiha hotdog you");
        }
        ...
        if (player.isNaga())
        {
            outputText("along your long naga tail");
        }
        else
        {
        ...
        {
            outputText(" as she palms your " + biggestBreastSizeDescript());
        }
        ...
        {
            outputText("bottoms out on your cock");
        }
        else
        {
            outputText("rests her cheeks on your crotch");
        }
        outputText(".  Both of you are gasping and panting from the slow descent, Kiha ");
        if (flags[kFLAGS.TIMES_KIHA_ANALED] >= 5)
        {
            outputText("growing pleased by");
        }
        else
        {
            outputText("unused to anal penetration");
        }
    ...
    private function fuckKihasVagInCamp() : Void
    {
        ...
        if (silly())
        {
            outputText("She asks in a high-pitched whine, \"<i>Did I do thaaat?</i>\"  Steve Urkel has nothing on her!");
        }
        else
        {
            outputText("She nervously stutters, \"<i>Is that… because of me?</i>\"");
        }
        ...
        {
            outputText("leave long, red scratches on your back.");
        }
        else
        {
            outputText("dislodge the occasional old scale from your hide.");
        }
        ...
        if (player.cockTotal() > 1)
        {
            outputText("[pg]Kiha's belly is quickly glazed white with goo, and her ");
        }
        else
        {
            outputText("[pg]Kiha's ");
        ...
        if (player.cumQ() >= 500)
        {
            outputText("  Such a meager display is only the start.  You fire rope after rope of cum, enough to slightly distend the dragoness's belly with your seed.");
        }
        if (player.cumQ() >= 1500)
        {
            outputText("  Jizz pumps and pumps, squirting from her filled pussy almost as fast as you inject, yet you still manage to make her look nearly pregnant by the time you finish.");
        }
        if (player.cumQ() >= 1000)
        {
            outputText("  The drippings form a nice big puddle under Kiha.");
        }
    ...
    private function giveKihaIncubusDraft() : Void
    {
        ...
        if (silly())
        {
            outputText("  This is your game, you remind her: you control the horizontal and the vertical.");
        }
    ...
    private function fuckKihaWithATentacle() : Void
    {
        ...
        if (x1[4] <= 0)
        {
            sceneHunter.print("You could use more.");
        }
        ...
        if (silly())
        {
            outputText("  You whisper slowly, \"<i>Shhhh… no tears now.  Only dreams… and tentacles.</i>\"");
        }
        ...
        if (player.countCocksWithType(CockTypesEnum.TENTACLE) > 1)
        {
            outputText("those monstrous things");
        }
        else
        {
            outputText("that monstrous thing");
        }
        ...
        if (x1[2] > 0)
        {
            ...
            if (player.cockArea(x1[2] - 1) < 24)
            {
                outputText(" and your rod buried to the hilt within her");
            }
        ...
        if (player.hasBalls())
        {
            outputText("churn and quiver");
        }
        else
        {
            outputText("churns and quivers");
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function kihaOffersToMoveIn() : Void
    {
        clearOutput();
        kihaSprite();
    ...
    @:allow(classes.scenes.nPCs)
    private function kihaBitchesOutCorruptPCs() : Void
    {
        clearOutput();
        kihaSprite();
        if (!followerKiha())
        {
            ...
            if (flags[kFLAGS.KIHA_AFFECTION_LEVEL] == 1)
            {
                outputText("friend");
            }
            else
    ...
    //[Leave]
    @:allow(classes.scenes.nPCs)
    private function leaveKihaAfterCorruptionBitch() : Void
    {
        clearOutput();
    ...
    @:allow(classes.scenes.nPCs)
    private function kihaUnbitchesUncorruptedFolks() : Void
    {
        clearOutput();
        kihaSprite();
    ...
    private function kihaCampAppearance() : Void
    {
        ...
        if (flags[kFLAGS.KIHA_UNDERGARMENTS] > 0)
        {
            outputText((flags[kFLAGS.KIHA_UNDERGARMENTS] == 1) ? "panties" : "loincloth");
        }
        else
        {
            outputText("thighs");
        ...
        {
            outputText("[pg]");
            if (pregnancy.incubation > sceneHunter.adjustPregEventTimerNum(288, PregnancyStore.INCUBATION_KIHA))
            {
                outputText("<b>Her belly appears to be a bit swollen.</b>");
            }
            else if (pregnancy.incubation > sceneHunter.adjustPregEventTimerNum(240, PregnancyStore.INCUBATION_KIHA) && pregnancy.incubation <= sceneHunter.adjustPregEventTimerNum(288, PregnancyStore.INCUBATION_KIHA))
            {
                outputText("<b>Her belly is comparable to being six months into pregnancy.</b>");
            }
            else if (pregnancy.incubation > sceneHunter.adjustPregEventTimerNum(192, PregnancyStore.INCUBATION_KIHA) && pregnancy.incubation <= sceneHunter.adjustPregEventTimerNum(240, PregnancyStore.INCUBATION_KIHA))
            {
                outputText("<b>Her belly is comparable to being eight months into pregnancy.</b>");
            }
            else if (pregnancy.incubation > sceneHunter.adjustPregEventTimerNum(144, PregnancyStore.INCUBATION_KIHA) && pregnancy.incubation <= sceneHunter.adjustPregEventTimerNum(192, PregnancyStore.INCUBATION_KIHA))
            {
                outputText("<b>Her belly is comparable to being nine months into pregnancy. It wouldn't take very long until she eventually lays a clutch of eggs.</b>");
            }
            else if (pregnancy.incubation > sceneHunter.adjustPregEventTimerNum(72, PregnancyStore.INCUBATION_KIHA) && pregnancy.incubation <= sceneHunter.adjustPregEventTimerNum(144, PregnancyStore.INCUBATION_KIHA))
            {
                outputText("<b>Her belly is even bigger than the average belly size at end of a typical human pregnancy.</b>");
            }
            else if (pregnancy.incubation <= sceneHunter.adjustPregEventTimerNum(72, PregnancyStore.INCUBATION_KIHA))
            {
                outputText("<b>It's impossible to not notice her pregnancy. The size of her belly has taken its toll on her. She is about to lay a clutch of eggs soon.</b>");
            }
        }
    ...
    //[Fight for position]
    private function fightForDominanceWithDragonCunnies(mock : Bool = false) : Void
    {
        clearOutput();
        ...
        if (mock)
        {
            outputText("Kiha flies up and slams down into the mossy earth with enough force to spray chunks of dirt everywhere.  She stands and growls, \"<i>Just because you beat me before doesn't mean you've defeated me.  I'll NEVER BE DEFEATED!</i>\"\n\n");
        }
        else
        {
    ...
    //[PC loses the fight]
    @:allow(classes.scenes.nPCs)
    private function pcLosesDomFight() : Void
    {
        clearOutput();
    ...
    //[PC wins the fight]
    @:allow(classes.scenes.nPCs)
    private function pcWinsDomFight() : Void
    {
        clearOutput();
        lvlUpCheckup();
        outputText(images.showImage("kiha-dom-win"));
        kihaSprite(true);
        var x : Float = player.cockThatFits(67);
        if (x < 0)
        {
        ...
        if (player.hasCock())
        {
            outputText("pull your quickly hardening " + multiCockDescriptLight());
        }
        else if (player.hasVagina())
        {
            outputText("release your lathered up " + vaginaDescript(0));
        }
        ...
        if (flags[kFLAGS.KIHA_CHOKED_OUT_PC] > 0)
        {
        ...
        var select : Float = choices[rand(choices.length)];
        ...
        if (select == 0 || !player.hasVagina())
        {
            ...
            if (rand(2) == 0)
            {
                outputText("[pg]\"<i>Ha!  The Champion still has some fight in [him]!  Keep me here if you can, wimp!  I WILL pin you down and cum all over your face!  You'll lap it all up like a good little " + player.mf("boy", "girl") + " to bo- UGH!</i>\"");
            }
            //Kiha response one (requires cock):
            else
            {
                outputText("[pg]\"<i>Ha!  The Champion still has some fight in [him]!  Keep me here if you can, wimp!  I WILL pin you down and make you beg to plant your seed in my womb!  Just wa-OOhhOhhh… </i>\"");
            }
        }
        ...
        if (player.gender != 2 && player.balls == 0)
        {
            outputText("[pg]You're not sure if that's an insult or a challenge to step it up.");
        }
        ...
        if (select == 0 || !player.hasVagina())
        {
            outputText(" tug along her lengths and pull her head back towards you, soliciting a pained scream from Kiha as her roots convey the agony of the act to her.  \"<i>Is that all you've GOT?! Give me a break!</i>\"");
        }
        //PC is fucking Kiha via scissoring:
        else
        {
            outputText(" twist of your fingers along her clitoral hood, causing Kiha to screech in surprised pain and pleasure.  \"<i>Is that all you've GOT?! Give me a break!</i>\"");
        }
        ...
        else
        {
            ...
            if (!player.hasCock())
            {
                outputText("lapping viciously at your cunt.");
            }
            else
            {
                outputText("ramming your cock down her craw.");
            }
        }
        ...
        if (select <= 1)
        {
            outputText("not have your neck's flesh ripped up in the process");
        }
        else
        {
            outputText("not have a mishap with your genitalia");
        }
        ...
        else if (select == 1)
        {
            ...
            if (player.hasLongTongue() || player.tongue.type == Tongue.SNAKE)
            {
                ...
                {
                    outputText("taking to her pointed mammary like a boa to prey as you lather up each teat separately.  The hot-tempered dragon girl squirms, completely at your mercy while you have your fun.");
                }
                else if (player.tongue.type == Tongue.SNAKE)
                {
                    outputText("stimulating the soft, nubby flesh with your forked tongue, hissing for effect as you do it.  Imagine the look on her face if you worked that little bugger over her clit; she'd go berserk!  But she hasn't earned that yet; what you want to hear is her begging for a good dicking first.");
                }
            }
    ...
    private function lvlUpCheckup() : Void
    {
        if (flags[kFLAGS.SPARRABLE_NPCS_TRAINING] == 2)
        {
            ...
            {
                if (flags[kFLAGS.KIHA_DEFEATS_COUNTER] >= 1)
                {
                    flags[kFLAGS.KIHA_DEFEATS_COUNTER]++;
                }
                else
                {
                    flags[kFLAGS.KIHA_DEFEATS_COUNTER] = 1;
                }
            }
            if (flags[kFLAGS.KIHA_LVL_UP] < 13 && flags[kFLAGS.KIHA_DEFEATS_COUNTER] >= flags[kFLAGS.KIHA_LVL_UP] + 4)
            {
                var addToV3 : Float = player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * flags[kFLAGS.KIHA_DEFEATS_COUNTER];
    ...
    private function giveKihaUndergarmentsPrompt() : Void
    {
        ...
        menu();
        if (player.hasItem(undergarments.SSPANTY))
        {
            addButton(0, undergarments.SSPANTY.shortName, giveKihaUndergarments, 1);
        }
        if (player.hasItem(undergarments.SS_LOIN))
        {
            addButton(1, undergarments.SS_LOIN.shortName, giveKihaUndergarments, 2);
        }
    ...
    private function kihaPregUpdate() : Bool
    {
    ...
    //Determines how many eggs!
    {
        ...
        var eggCounter : Int = as3hx.Compat.parseInt((rand(5) + 1) * 2);
        eggCounter += as3hx.Compat.parseInt(player.virilityQ() / 10);
        if (eggCounter > 10)
        {
            eggCounter = 10;
        }
        ...
        if (flags[kFLAGS.KIHA_UNDERGARMENTS] > 0)
        {
            outputText("You remove her spider-silk " + ((flags[kFLAGS.KIHA_UNDERGARMENTS] == 1) ? "panties" : "loincloth") + ", exposing her moist vagina.");
        }
        ...
        var childList : Array<Dynamic> = [];
        for (i in 0...eggCounter)
        {
    ...
    public function kihaTellsChildrenStory() : Void
    {
        ...
        {
            outputText("Kiha gestures for the child to sit on her lap. ");
        }
        else
        {
            outputText("Kiha gestures for the children to sit in front of her, making sure you are visible to them. ");
        }
        //Story time!
    ...
    {
        var availableGenders : Array<Dynamic> = [];
    ...
    private function kihaChildGraduationTime() : Void
    {
        ...
        if (flags[kFLAGS.KIHA_UNDERGARMENTS] > 0)
        {
            outputText("like Kiha");
        }
        else
        {
            outputText("unlike Kiha, who is naked");
        }
    ...
    private function KihaTalkKids() : Void
    {
        ...
        {
            outputText("leech off you, put them up in your cabin");
        }
        else
        {
            outputText("raise them without a roof over their heads");
        }
    ...
    public function KihaProposal() : Void
    {
        ...
        else
        {
            ...
            if (player.hasCock())
            {
                outputText("Your [cock] hardens, pressing against Kiha’s thigh, ");
            }
            if (player.hasVagina())
            {
                outputText(((player.hasCock()) ? "y" : "Y") + "ou begin to drool from your lower lips, ");
            }
    ...
    private function TalkWithKiha() : Void
    {
        ...
        if (totalKihaChildren() > 0)
        {
            addButton(1, "Kids", KihaTalkKids);
        }  //TODO  
        if (KihaTownState == 1 && ProposalStatus < 1)
        {
        ...
        if (ProposalStatus == 1)
        {
            addButton(5, "Propose", KihaProposal).disableIf(!sceneHunter.canMarry(), "You have already married someone else.");
        }
        if (ProposalStatus == 3)
        {
            addButton(5, "WeddingReady", KihaWedding2).disableIf(!SceneLib.templeofdivine.canMarry(), "You need to repair the temple first!").disableIf(!sceneHunter.canMarry(), "You have already married someone else.");
        }
        if (ProposalStatus == 4)
        {
            addButton(5, "Propose", KihaProposal).disableIf(!sceneHunter.canMarry(), "You have already married someone else.");
    ...
    private function visitKids() : Void
    {
        ...
        outputText("Kiha gives you a small smile as you ask about them. She takes your hand, squeezing it hard, and the two of you walk to Kiha’s little cave. \n"
                ...
                + "Inside, "
                + totalKihaChildren()
                + " young dragonoids mill about. Mareth’s quick growth matured their bodies, but evidently not their minds, entirely. A few spurts of fire roar from your children’s mouths as they rush over, spreading their wings and tackling you. \n"
                + "\n"
    ...
    {
        menu();
        addButton(0, "Ember", KihaTalkEmber).disableIf(!emberScene.followerEmber(), "", "???");
        addButton(1, "Tyrantia", KihaTalkTyrantia).disableIf(!TyrantiaFollower.isLover(), "", "???");
        addButton(2, "Diva", KihaTalkDiva).disableIf(!DivaScene.instance.isCompanion(), "", "???");
    ...
    {
        emberStart();
        var emberStart : Void->Void = function() : Void
        {
            ...
            {
                outputText("“<i>If you were a real dragon, you’d be laughed out of the nest.</i>” Ember scowls. “<i>That or be used as breeding stock for a third-rate runt.</i>” \n"
                        + "\n"
        ...
        var explain : Void->Void = function() : Void
        {
            clearOutput();
        ...
        var kihaSide : Void->Void = function() : Void
        {
            clearOutput();
        ...
        var emberSide : Void->Void = function() : Void
        {
            clearOutput();
        ...
        var ladies : Void->Void = function() : Void
        {
            clearOutput();
    ...
    public function tyrantiaInteraction(fromTyrantia : Bool) : Void
    {
        ...
        if (fromTyrantia)
        {
            outputText("You say that you came to check up on the resident giantess, and Kiha nods, rubbing her wing. “<i>I’ll just be down by the water.</i>”");
        }
        else
        {
            outputText("You tell Kiha that you came to see her, and she nods. “<i>Alright. What do you want, doofus?</i>” Tyrantia excuses herself, heading down to the river to wash up.");
        }
    }
    ...
    //No buttons or clears
    public function sidonieInteraction(fromSidonie : Bool) : Void
    {
        ...
        if (fromSidonie)
        {
            outputText("You tell Sidonie that you wanted to speak to her. She nods, and the two of you leave Kiha by the fire. “<i>What can I do for you?</i>” Sidonie asks.");
        }
        else
        {
            outputText("You tell them that you wanted to see your dragoness. Sidonie nods. “<i>I’ll finish up the cooking, Kiha. Come on back when you’re done.</i>” She gives you a wink. “<i>Just let me know if you’ll be gone for too long.</i>” Kiha blushes at that, and you walk back to her section of camp.");
        }
    }
    
    public function belisaInteractionFirst(fromBelisa : Bool) : Void
    {
    ...
    public function belisaInteraction(fromBelisa : Bool) : Void
    {
        if (fromBelisa)
        {
            outputText("Deciding to pay your shy drider a visit, you head down to her silk house by the river. As you near it, you see a familiar face.");
        }
        else
        {
            outputText("As you head over towards Kiha’s part of camp, you find a small wooden sign, with 'Gone Fishing' scratched into it. You head down to the river, near Belisa’s house.");
        }
        ...
        if (fromBelisa)
        {
            outputText("Before you can say anything, you hear a small snore from underneath the tree. You turn, facing the noise, but to your surprise, it’s your dragoness lover. Kiha’s sound asleep, rod slack in her hands. Her snout rests on her soft DDs, and her taloned legs twitch occasionally. \n"
                    + "\n"
        ...
        {
            outputText("You sit down beside Kiha, gently taking the fishing rod from her slack hand. You bring the line in, and Belisa chuckles, putting a soft hand over her mouth. You give your Drider a once-over, noting her dripping legs and bare breasts. Belisa notices your interest, blushing bright red. She excuses herself, wrapping a towel around herself and going back home. \n"
                    + "\n"
    ...
    public function electraInteractionFirst(fromElectra : Bool) : Void
    {
    ...
    //no clears or buttons
    public function electraInteraction(fromElectra : Bool) : Void
    {
        ...
        if (fromElectra)
        {
            outputText("You tell Electra that you came to see her, and she grins, bobbing up and down excitedly. Kiha nods respectfully, leaving without a word, and Electra tilts her head slightly, static filling the air. “<i>What’s up, [name]?</i>” ");
        }
        else
        {
            outputText("You ask Kiha if she has a moment, and she nods. Electra leaves, and your fiery dragoness smiles slightly. ");
        }
    }
    ...
    public function kihaHenchmanOption() : Void
    {
        ...
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "Kiha" || flags[kFLAGS.PLAYER_COMPANION_3] == "Kiha")
            {
                addButtonDisabled(0, "Team (1)", "You already have Kiha accompany you.");
            }
            else
            {
                addButton(0, "Team (1)", kihaHenchmanOption2, 1).hint("Ask Kiha to join you in adventures outside camp.");
            }
        }
        else if (flags[kFLAGS.PLAYER_COMPANION_1] == "Kiha")
        {
            addButton(5, "Team (1)", kihaHenchmanOption2, 21).hint("Ask Kiha to stay in camp.");
        }
        else
        {
            addButtonDisabled(5, "Team (1)", "You already have other henchman accompany you as first party member. Ask him/her to stay at camp before you talk with Kiha about accompaning you as first party member.");
        }
        if (player.hasPerk(PerkLib.IntermediateLeadership))
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "")
            {
                if (flags[kFLAGS.PLAYER_COMPANION_1] == "Kiha" || flags[kFLAGS.PLAYER_COMPANION_3] == "Kiha")
                {
                    addButtonDisabled(1, "Team (2)", "You already have Kiha accompany you.");
                }
                else
                {
                    addButton(1, "Team (2)", kihaHenchmanOption2, 2).hint("Ask Kiha to join you in adventures outside camp.");
                }
            }
            else if (flags[kFLAGS.PLAYER_COMPANION_2] == "Kiha")
            {
                addButton(6, "Team (2)", kihaHenchmanOption2, 22).hint("Ask Kiha to stay in camp.");
            }
            else
            {
                addButtonDisabled(6, "Team (2)", "You already have other henchman accompany you as second party member. Ask him/her to stay at camp before you talk with Kiha about accompaning you as second party member.");
            }
        }
        else
        {
    ...
    public function kihaHenchmanOption2(slot : Float = 1) : Void
    {
        clearOutput();
        if (slot < 21)
        {
            ...
            var strKiha : Float = 85;
            var meleeAtkKiha : Float = 28;
            ...
            meleeAtkKiha += (1 + as3hx.Compat.parseInt(meleeAtkKiha / 5)) * player.newGamePlusMod();
            player.createStatusEffect(StatusEffects.CombatFollowerKiha, strKiha, meleeAtkKiha, 0, 0);
            if (slot == 2)
            {
                flags[kFLAGS.PLAYER_COMPANION_2] = "Kiha";
            }
            if (slot == 1)
            {
