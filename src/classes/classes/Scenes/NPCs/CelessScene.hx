 ...
 */
class CelessScene extends XXCNPC implements TimeAwareInterface
{
    public static var instance(get, never) : CelessScene;
    public var armorFound(get, never) : Bool;
    public var isCorrupt(get, never) : Bool;
    public var isAdult(get, never) : Bool;
    public var questFinishedUnicorn(get, never) : Bool;
    public var questFinishedNightmare(get, never) : Bool;
    private var myLocals(get, never) : Dynamic;

    private static var _ageIsAdult : Int = -1;
    private static var _ageShouldDoBirth : Int = -2;
    private static var _ageCanMeetNightmare : Int = -3;
    private static var _ageDidPregnancy : Int = -4;
    private static var _ageIsDeadOrRemoved : Int = -5;
    
    private static inline var _finishedUnicorn : Int = 1;
    private static var _finishedNightmare : Int = -1;
    
    private static var _instance : CelessScene;
    
    private static function get_instance() : CelessScene
    {
        if (_instance == null)
        {
            new CelessScene();
    ...
    public static function canMeetUnicorn() : Bool
    {
        return (
    ...
    public static function canGetArmour() : Bool
    {
        return instance.isCompanion() && !instance.armorFound;
    }
    
    private static function playerIsVirgin() : Bool
    {
    ...
    public static function canMeetNightmare() : Bool
    {
    ...
    private var _armorFound : Bool = false;
    
    private function get_armorFound() : Bool
    {
        return _armorFound;
    }
    
    private function get_isCorrupt() : Bool
    {
        return _corruption > 50 || player.perkv1(IMutationsLib.FiendishOvariesIM) >= 2;
    }
    
    private function get_isAdult() : Bool
    {
    ...
    private function get_questFinishedUnicorn() : Bool
    {
        return _questFinished == 1;
    }
    private function get_questFinishedNightmare() : Bool
    {
        return _questFinished == -1;
    }
    
    //Used for save update
    public function fixQuestFinished() : Bool
    {
        if (!canMeetNightmare() && !canMeetUnicorn() && _age != 0)
        
        //can't meet anyone, but the quest is progressed.{
    ...
    override public function save(saveto : Dynamic) : Void
    {
        saveto.celess = {
                    myClass : Type.getClassName(Type.getClass(this)),
                    age : _age,
    ...
    override public function load(loadfrom : Dynamic) : Void
    {
        if (loadfrom == null || loadfrom.celess == null)
        {
            unload();
        }
        else
    ...
    override public function checkCampEvent() : Bool
    {
    ...
    override public function isCompanion(type : Int = -1) : Bool
    {
    ...
    override public function campInteraction() : Void
    {
        ...
        var incestMenu : Void->Void = function() : Void
        {
            if (isCorrupt)
            {
                clearOutput();
    ...
    {
        var locals : Dynamic = myLocals;
        Reflect.setField(locals, "$dangerousPlants", player.hasKeyItem("Dangerous Plants") >= 0);
        clearOutput();
        if (isCorrupt)
        {
    ...
    private function pureIncest() : Void
    {
        ...
        if (player.cor > 20)
        {
            outputText("Just as you're about to dismiss the idea completely, " + _name + " surprises you.</if>\n\n");
        }
    ...
    private function nameScene() : Void
    {
        ...
        if (player.inte > 50)
        {
            outputText("Maybe it’s due to how time and space distort here? ");
        }
        ...
        if (player.inte > 50)
        {
            outputText("Does make sense, otherwise they would cause some nasty internal damage during birth. ");
        }
    ...
    public static function itemImproveMenu(dialogue : Int = 0, corrupt : Bool = false  /*for EL*/  ,  /*for EL*/  ) : Void
    //dialogue
    {
        ...
        for (i in 0...improvableItems.length)
        {
            if (improvableItems[i][selectfrom] != null)
            {
                ...
                selectMenu.add(item.id, curry(improveItemDialogue, item, from, dialogue, corrupt)).disableIf(!player.hasItem(from), "You need " + from.longName + " as a base to create this item.").disableIf(player.keyItemvX("Radiant shard", 1) < 3, "You need at least three radiant shards in order to create this item.").disableIf(player.gems < 20000, "You need at least 20 000 gems in order to create this item.");
            }
        }
    ...
    public static function improveItemDialogue(item : ItemType, from : ItemType, dialogue : Int, corrupt : Bool) : Void
    {
        clearOutput();
        //Celess herself
        if (dialogue == 0)
    ...
    public function celessUnicornIntro() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.CanMeetNightmare))
        {
            ...
            if (player.hasKeyItem("Nightmare Horns") >= 0)
            {
                addButton(1, "Show proof", celessUnicornIntro2);
            }
            else
            {
                addButtonDisabled(1, "Show proof", "Req. to have Nightmare Horns.");
            }
            return;
        }
        celessUnicornIntro1();
    }
    private function celessUnicornIntro1(stage1 : Int = 0, wasMale : Bool = false) : Void
    {
        spriteSelect(SpriteDb.s_celessWhite);
        switch (stage1)
        {
            case 0:
                ...
                if (playerIsVirgin())
                {
                    ...
                    menu();
                    addButton(0, "Okay", celessUnicornIntro1, ((player.isMale() || player.isGenderless())) ? 2 : 3).disableIf(player.isMale() && player.blockingBodyTransformations(), "Due to your unusual body, you have a feeling this won't work at you.").disableIf(player.isGargoyle(), "You are a gargoyle... you can't carry <b>anyone's</b> essence.");
                    ...
                    {
                        addButtonDisabled(1, "Fuck Her", "Req. a cock.");
                    }
            ...
            case 3:
                ...
                if (player.hasKeyItem("Nightmare Horns") >= 0)
                {
                    player.removeKeyItem("Nightmare Horns");
                }
    ...
    public function celessUnicornIntro2(stage2 : Int = 0, wasMale : Bool = false) : Void
    {
        spriteSelect(SpriteDb.s_celessWhite);
        switch (stage2)
        {
            case 0:
                ...
                menu();
                addButton(0, "Okay", celessUnicornIntro2, ((player.isMale() || player.isGenderless())) ? 2 : 3).disableIf(player.isMale() && player.blockingBodyTransformations(), "Due to your unusual body, you have a feeling this won't work at you.").disableIf(player.isGargoyle(), "You are a gargoyle... you can't carry <b>anyone's</b> essence.");
                var warningStr : String = "You have a strange feeling that you can miss something very important by doing this.\n\n<b>You can still get Celess this way by enabling 'Other' in SceneHunter settings.</b>";
                var hintStr : String = "\n\n<b>You can still get Celess this way by enabling 'Other' in SceneHunter settings.</b>";
                var noteStr : String = "<b>SH: Now this works as alternative way to Celess.</b>";
                addButton(1, "Fuck Her", celessUnicornIntro2, 4).hint((sceneHunter.other) ? noteStr : warningStr + hintStr).disableIf(!player.hasCock(), "Req. a cock.");
            ...
            case 1:
                ...
                if (silly())
                {
                    outputText(", followed by singing <i>\"nope, nope, nope, fuck this shit, I’m out!\" </i>, ");
                }
    ...
    private function celessGuardOkayMale() : Void
    {
        ...
        if (!player.isNaked())
        {
            ...
            if (player.inte > 70)
            {
                outputText("Is she seriously using spatial magic of all things just to avoid the bother of undressing you?");
            }
        }
        outputText("<i>\"A unicorn's ‘horns’ can pierce through anything, just relax, it will be easier for you.\"</i> ");
        if (silly() && player.inte > 70)
        {
            outputText("Part of you is mildly surprised, and grateful, no Demon has realized this potential application of spatial magic, otherwise they would have pushed to develop it until they learnt how to open portals to other realms by ramming a dick against space-time until they fucked it into submission.");
        }
    ...
    private function celessGuardOkayFemale(wasMale : Bool = false) : Void
    {
        clearOutput();
    ...
    private function celessGuardFuckHer() : Void
    {
        ...
        {
            outputText(" beside Edryn's of course");
        }
        ...
        if (!recalling)
        {
            ...
            {
                flags[kFLAGS.HACK_CELESS_INCUBATION] = PregnancyStore.INCUBATION_CELESS / 2;
            }
        }
        else
        {
    ...
    private function growUpScene() : Void
    {
        ...
        else
        {
            ...
            if (silly())
            {
                outputText(", kinda obvious, what with her outright telling you less than two seconds ago,");
            }
        ...
        if (isCorrupt)
        {
            ...
            {
                outputText("impaling yourself on her alluring cock.");
            }
            else if (player.hasStatusEffect(StatusEffects.Rut))
            {
                outputText("moving behind her and plowing her alluring pussy with your painfully erect " + player.cockDescript() + ".");
            }
    ...
    private function incestGetFucked() : Void
    {
        ...
        {
            outputText("Checking on your privates, you’re not surprised to discover you did, indeed, keep your virginity in spite of the crazy ride the young centauress gave you. " +
    ...
    private function incestFuckCorrupt() : Void
    {
        ...
        if (player.hasKeyItem("Fake Mare") >= 0 || player.hasKeyItem("Centaur Pole") >= 0)
        {
            outputText(" or the toys you brought back from Whitney’s farm");
        }
    ...
    private function doHeatOrRut() : Void
    {
        ...
        if (!player.inHeat && player.goIntoHeat(false))
        
        //This would put the PC into a 20 day heat/rut tho...{
            
            outputText("\nYou are now in Heat!");
        }
        else if (!player.inRut && player.goIntoRut(false))
        
        //This is based on the original.xml source.{
    ...
    private function get_myLocals() : Dynamic
    {
        return {
            __DOLLAR__name : _name,
            __DOLLAR__isCorrupt : isCorrupt,
            __DOLLAR__isAdult : isAdult
        };
    }
