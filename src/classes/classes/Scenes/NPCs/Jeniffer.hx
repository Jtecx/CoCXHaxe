...
class Jeniffer extends Monster
{
    public function Specials2() : Void
    {
        ...
        var damage : Float = Math.round(((str + weaponAttack) * 3) - rand(player.tou + player.armorDef));
        //Dodge
    ...
    override private function performCombatAction() : Void
    {
        if (flags[kFLAGS.JENIFFER_LVL_UP] >= 1 && flags[kFLAGS.JENIFFER_LVL_UP] < 4)
        {
            ...
            if (choice1 == 1)
            {
                if (!player.hasStatusEffect(StatusEffects.IzmaBleed) && rand(2) == 0)
                {
                    Specials1();
                }
                else
    ...
    public function new()
    {
        ...
        if (flags[kFLAGS.JENIFFER_LVL_UP] >= 4 && flags[kFLAGS.JENIFFER_LVL_UP] < 7)
        
        //lub później niż 7 upgrade{
