...
class Etna extends Monster
{
    ...
    public function moveClawCombo() : Void
    {
        ...
        else if (flags[kFLAGS.ETNA_LVL_UP] >= 4)
        {
    ...
    public function TailSpike() : Void
    {
        ...
        if (rand(100) < (this.spe - player.spe) / 2)
        {
            ...
            {
                var tailspikedmg : Float = Math.round(this.str / 16);
                var lustdmg : Float = Math.round(this.lib / 6);
    ...
    public function moveBoobCrash() : Void
    {
        if (game.flags[kFLAGS.ETNA_TALKED_ABOUT_HER] >= 1)
        {
            outputText("Etna flies down");
        }
        else
        {
            outputText("The flying manticore dives");
        }
        ...
        var boobcrashdmg : Float = Math.round(this.str / 8);
        var lustdmg : Float = Math.round(this.lib / 3);
        player.takeLustDamage(lustdmg, true);
    ...
    {
        var bitedmg : Float = Math.round(this.str / 25);
        if (game.flags[kFLAGS.ETNA_TALKED_ABOUT_HER] >= 1)
        {
            outputText("Etna");
        }
        ...
        if (hasStatusEffect(StatusEffects.ConstrictedScylla))
        {
    ...
    {
        if (flags[kFLAGS.ETNA_FOLLOWER] >= 2 && EtnaFollower.EtnaInfidelity != 2)
        {
            etnaScene.etnaRapeIntro2();
        }
        else if (flags[kFLAGS.ETNA_AFFECTION] > 75)
        {
            etnaScene.etnaReady2Come2Camp();
        }
        else if (flags[kFLAGS.ETNA_TALKED_ABOUT_HER] < 1 && flags[kFLAGS.ETNA_AFFECTION] > 15)
        {
            etnaScene.etnaRape3rdWin();
        }
        else
        {
    ...
    override private function get_long() : String
    {
        ...
        if (game.flags[kFLAGS.ETNA_TALKED_ABOUT_HER] >= 1)
        {
            ...
            {
                str += "Etna is circling you in the air readying a salvo of spike to throw at you.";
            }
            else
            {
                str += "Etna is currently circling you looking for an opening to strike. She’s actually good on the eye and you would give her more attention if not for the fact she’s trying to beat you down and rape you.";
            }
        }
        else if (hasStatusEffect(StatusEffects.Flying))
        {
            str += "The manticore is circling you in the air readying a salvo of spike to throw at you.";
        }
        else
        {
            str += "The manticore is currently circling you looking for an opening to strike. She’s actually good on the eye and you would give her more attention if not for the fact she’s trying to beat you down and rape you.";
        }
        return str;
    }
    ...
    public function new()
    {
        ...
        this.tailRecharge = 0;
        if (flags[kFLAGS.ETNA_FOLLOWER] > 1 || flags[kFLAGS.ETNA_TALKED_ABOUT_HER] > 1)
        {
