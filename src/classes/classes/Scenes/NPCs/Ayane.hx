...
class Ayane extends Kitsune
{
    ...
    private function foxfireStorm() : Void
    {
        ...
        var damage : Int = as3hx.Compat.parseInt((this.inte + this.wis) * 3 + rand(50));
        ...
        for (i in 0...7)
        {
            ...
            {
                outputText("  You avoid the foxfire barely feeling it's tingling against your skin!");
            }
            else
            {
    ...
    private function illusion() : Void
    {
        if (!player.hasPerk(PerkLib.TrueSeeing) && !player.hasPerk(PerkLib.SixthSense))
        {
            ...
            statStore.addBuff("spe", 40, "KitsuneIllusion", {
    ...
    //Ayane periodically appears and disappears around the player. At random at the start of the round Ayane may be hidden or visible. If she’s hidden all attack but tease will fail
    {
        ...
        {
            this.removeStatusEffect(StatusEffects.MonsterInvisible);
        }
        if (kissCooldown > 0)
        {
            --kissCooldown;
        ...
        //pick move
        var moves : Array<Dynamic> = [foxFireAttack, foxfireStorm, drainingKiss];
        if (HP < maxOverHP())
        {
            if (kissCooldown == 0)
            {
                moves.push(drainingKiss);
            }
            if (healCooldown == 0)
            {
                moves.push(heal);
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        ...
        {
            SceneLib.ayaneFollower.fightLose();
        }
    }
