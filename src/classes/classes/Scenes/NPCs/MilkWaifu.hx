...
class MilkWaifu extends NPCAwareContent
{
    ...
    override public function milkSlave() : Bool
    {
        return Std.is(flags[kFLAGS.MILK_NAME], String);
    }
    ...
    public function nameZeMilkBath() : Void
    {
        ...
        else if (mainView.nameBox.text == "" || Std.is(mainView.nameBox.text, Float))
        {
            clearOutput();
        ...
        if (mainView.nameBox.text == "Bath Slut")
        {
            outputText("Fuck it, Bath Slut it is.  At least she won't get confused.");
        }
        else if (mainView.nameBox.text == "Biscuit")
        {
            outputText("Fuck it, you may as well butter her buns!");
        }
    ...
    public function milkyMenu() : Void
    {
        ...
        if (flags[kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL] == 0)
        {
            ...
            {
                outputText("lying face-down on her massive rack, her plump little ass sticking up in the air for all to see.  Seeing you approach, she brightens up and shifts her titanic bust around to face you, all bright-eyes and smiles.  \"<i>Milk time?</i>\"");
            }
            else if (flags[kFLAGS.MILK_SIZE] == 1)
            {
                outputText("arms crossed under her hefty, milky bust to support their still-sizable weight.  She smiles as you approach, able to stand up under her own power to give you a hug, milky tits pressed tight against you.  \"<i>I-is it milk time?</i>\" she asks, cupping her tits for you.");
            }
            else
            {
                outputText("her long, tanned legs dangling into the tub.  She jumps up when you approach, and though still a bit unsteady with such easy movements, she's quick to leap into your arms, nuzzling into your neck and pressing her firm, perky DD-cup breasts against you, little trickles of milk staining your chest through the little shirt she's wearing.  \"<i>[name],</i>\" she purrs happily.");
            }
        }
        else if (flags[kFLAGS.MILK_SIZE] == 0)
        {
        ...
        if (flags[kFLAGS.MILK_SIZE] > 0)
        {
            addButton(1, "Milk Time!", nyanCatMilkTime);
        }
        if (flags[kFLAGS.MILK_SIZE] < 2)
        {
            addButton(2, "Milk Bath", milkBathTime);
        }
        if (flags[kFLAGS.MILK_SIZE] < 2 && player.hasKeyItem("Super Reducto") >= 0)
        {
            addButton(3, "Reducto", superReductoUsage);
        }
        if (flags[kFLAGS.MILK_SIZE] > 0 && player.lust >= 33 && player.hasCock())
        {
            addButton(4, "Titfuck", titFuckDatMilkSlut);
        }
        
        if (flags[kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL] == 0 && flags[kFLAGS.MILK_SIZE] > 0 && flags[kFLAGS.FARM_CORRUPTION_STARTED] == 1)
        {
    ...
    private function milkWriteFuAppearance() : Void
    {
        ...
        if (flags[kFLAGS.MILK_SIZE] >= 2)
        {
            outputText("Over her supple body, she wears a simple cloth shirt which cuts off just above her knees, hiding her newly-gained modesty, with a pair of silk painties and a simple bra beneath it.  ");
        }
        ...
        if (SceneLib.farm.farmCorruption.hasTattoo("milky"))
        {
            ...
            {
                if (SceneLib.farm.farmCorruption.numTattoos("milky") > 1)
                {
                ...
                if (flags[kFLAGS.MILKY_TATTOO_COLLARBONE] != 0)
                {
                    outputText(flags[kFLAGS.MILKY_TATTOO_COLLARBONE] + "\n");
                }
                if (flags[kFLAGS.MILKY_TATTOO_SHOULDERS] != 0)
                {
                    outputText(flags[kFLAGS.MILKY_TATTOO_SHOULDERS] + "\n");
                }
                if (flags[kFLAGS.MILKY_TATTOO_LOWERBACK] != 0)
                {
                    outputText(flags[kFLAGS.MILKY_TATTOO_LOWERBACK] + "\n");
                }
                if (flags[kFLAGS.MILKY_TATTOO_BUTT] != 0)
                {
                    outputText(flags[kFLAGS.MILKY_TATTOO_BUTT] + "\n");
                }
            }
        }
    ...
    private function superReductoUsage() : Void
    {
        ...
        else
        {
            ...
            if (isabellaFollowerScene.isabellaFollower())
            {
                ...
                {
                    outputText(" and Marble");
                }
                outputText(" pause");
            }
            else if (player.hasStatusEffect(StatusEffects.CampMarble))
            {
                outputText(", enough to give Marble pause");
            }
    ...
    private function milkBathTime() : Void
    {
        ...
        if (flags[kFLAGS.MILK_SIZE] == 0)
        {
            outputText("teat-like");
        }
        else
        {
            outputText("prominent");
        }
        ...
        {
            outputText(multiCockDescriptLight() + " and [vagina]");
        }
        else
        {
            outputText("[butt]");
        ...
        if (flags[kFLAGS.MILK_SIZE] == 0)
        {
            outputText("starting to shift her breasts' bulk toward a comfortable spot for a nap");
        }
        else
        {
            outputText("struggling to her feet, legs shaky with the afterglow of a mighty boobgasm");
        }
        outputText(".");
        if (player.lust >= 33)
        {
            outputText("  You could probably masturbate in the tub if you wanted to, or maybe pull the dusky milk-maid in for company.");
        }
        ...
        {
            addButton(0, "DrinkNFap", drinkAndFapturbate);
        }
        var count : Int = 0;
        if (sophieFollowerScene.sophieFollower() && !player.hasStatusEffect(StatusEffects.SophieOff))
        {
            count++;
        }
        ...
        if (helScene.followerHel() && !player.hasStatusEffect(StatusEffects.HeliaOff))
        {
            count++;
        }
    ...
    private function communalBath() : Void
    {
        ...
        if (isabellaFollowerScene.isabellaFollower())
        {
            ...
            {
                outputText("Vhat, ist Isabella's milk not gut enough fur you, [name].  Still, I could be using with a bath.");
            }
            else
            {
                outputText("What, is my milk not good enough for you, [name]?  Still, I could use a bath.");
            }
            outputText("</i>\"");
        }
        //If PC has Sophie:
        ...
        if (flags[kFLAGS.IZMA_FOLLOWER_STATUS] == 1)
        {
            var el : Bool = flags[kFLAGS.IZMA_BROFIED] > 0;
        ...
        if (kihaFollower.followerKiha())
        {
            ...
            if (silly() && player.str > 80 && player.spe > 80)
            {
                outputText("  Without warning Kiha, you grab her. \"<i>HEY, what are you doing, doofus?</i>\" she yells as you finally pull her into the milky bath. \"<i>What have you done, idiot!? Fine, I'll bathe. Are you happy? Baka!</i>\" she grumbles.");
            }
        }
    ...
    private function pullInZeMilkGirl() : Void
    {
        ...
        if (player.hasCock())
        {
            ...
            if (player.hasBalls())
            {
                outputText(", her other hand cupping your [balls], rolling the " + num2Text(player.balls) + " orbs in her palm with delightful dexterity");
            }
        ...
        if (player.hasVagina())
        {
            ...
            if (player.biggestTitSize() >= 1)
            {
                ...
                {
                    outputText("  A spurt of milk escapes your own full jugs, joining the pool-full of your friend's.  She gasps with delight, quickly nuzzling herself into your back and going to work.  She milks you just as she would herself, letting the hefty flow of your motherly fluids pour into the pool, odd trickles smearing down your chest, staining your chest as white as her own.");
                }
            }
        }
        ...
        if (player.hasCock())
        {
            addButton(0, "Fuck Her", fuckTheMilkWaifu);
        }
        if (player.hasVagina() && player.biggestTitSize() >= 3)
        {
            addButton(1, "LesboFuck", beARugMunchingMilkDyke);
        }
    }
    ...
    private function fuckTheMilkWaifu() : Void
    {
        ...
        {
            outputText("pushing your few inches into her until your hips join, her nice and loose cunt easily taking your length");
        }
    ...
    private function beARugMunchingMilkDyke() : Void
    {
        ...
        if (player.lactationQ() >= 200)
        {
            outputText(", milking you with skill beyond anything you've ever experienced before; and why not, when her entire existence revolved around that self-same skill?");
        }
        else
        {
            outputText(".");
    ...
    private function drinkAndFapturbate() : Void
    {
        ...
        if (flags[kFLAGS.MILK_SIZE] == 0)
        {
            outputText("a dinner plate");
        }
        else
        {
            outputText("an open palm");
        }
        ...
        if (flags[kFLAGS.MILK_SIZE] == 0)
        {
            outputText("drags her gigantic tits over the puddly, milk-slicked floor");
        }
        else
        {
            outputText("stumbles away, tenderly cupping her bright-red teats");
        }
    ...
    private function nyanCatMilkTime() : Void
    {
        ...
        if (flags[kFLAGS.MILK_SIZE] == 1)
        {
            ...
            if (!isabellaFollowerScene.isabellaFollower() || !sophieFollowerScene.sophieFollower() || player.hasStatusEffect(StatusEffects.SophieOff))
            {
        ...
        else
        {
            ...
            if (player.cockThatFits(50) >= 0 && player.hasCock())
            {
                outputText("  Then again, maybe you could hike her panties down and give her a good fucking to send her over the edge.  In her state, she certainly won't mind!");
            }
            menu();
            ...
            if (player.cockThatFits(50) >= 0 && player.hasCock())
            {
                addButton(2, "Fuck", fuckDatMilkSlat);
            }
        }
    }
    ...
    private function titFuckDatMilkSlut() : Void
    {
        ...
        if (flags[kFLAGS.MILK_SIZE] == 2)
        {
            outputText("pull off her shirt and bra");
        }
        else
        {
            outputText("sink your fingers into her udder-like teats");
        }
        ...
        if (!player.isNaga())
        {
            outputText("straddling her shoulders and shoving");
        }
        else
        {
            outputText("wrapping yourself around her to shove");
        }
        ...
        if (player.hasBalls())
        {
            outputText(", shivering as her tongue darts out around your cock to flick your nads");
        }
        outputText(".");
