...
class KindraFollower extends NPCAwareContent
{
    //implements TimeAwareInterface {
    
    public function kindraAffection(changes : Float = 0) : Float
    {
        flags[kFLAGS.KINDRA_AFFECTION] += changes;
        if (flags[kFLAGS.KINDRA_AFFECTION] > 100)
        {
            flags[kFLAGS.KINDRA_AFFECTION] = 100;
        }
    ...
    private function bowSkill(diff : Float) : Float
    {
        ...
        if (player.statusEffectv1(StatusEffects.Kindra) >= 95 && flags[kFLAGS.KINDRA_ADV_ARCHERY] < 3)
        {
            player.changeStatusValue(StatusEffects.Kindra, 1, 95);
        }
        if (player.statusEffectv1(StatusEffects.Kindra) >= 140 && flags[kFLAGS.KINDRA_ADV_ARCHERY] < 5)
        {
            player.changeStatusValue(StatusEffects.Kindra, 1, 140);
        }
    ...
    public function wonFightWithKindra() : Void
    {
        ...
        {
            if (flags[kFLAGS.KINDRA_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.KINDRA_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.KINDRA_DEFEATS_COUNTER] = 1;
            }
            if (flags[kFLAGS.KINDRA_LVL_UP] < 15 && flags[kFLAGS.KINDRA_DEFEATS_COUNTER] >= flags[kFLAGS.KINDRA_LVL_UP] + 2)
            {
                var addToV1 : Float = player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * flags[kFLAGS.KINDRA_DEFEATS_COUNTER];
    ...
    public function meet2Kindra() : Void
    {
        ...
        if (flags[kFLAGS.KINDRA_ADV_ARCHERY] < 6)
        {
            addButton(2, "Training", trainingArcheryWithKindra).hint("Train your archery skills with her.");
        }
        else
        {
            addButtonDisabled(2, "Training", "You finished advanced archery training.");
        }
        ...
        if (flags[kFLAGS.KINDRA_DAILY_TRAINING] < 1)
        {
            if (player.statusEffectv1(StatusEffects.Kindra) >= 140 && flags[kFLAGS.KINDRA_ADV_ARCHERY] >= 5 && player.hasStatusEffect(StatusEffects.KnowsBarrage))
            {
                if (player.isArtifactBow())
                {
                    ...
                    if (!player.hasStatusEffect(StatusEffects.KnowsSidewinder))
                    {
                        player.createStatusEffect(StatusEffects.KnowsSidewinder, 0, 0, 0, 0);
                    }
                    bowSkill(10);
            ...
            else if (player.statusEffectv1(StatusEffects.Kindra) >= 100 && player.hasStatusEffect(StatusEffects.KnowsBarrage))
            {
                ...
                {
                    flags[kFLAGS.KINDRA_ADV_ARCHERY] = 5;
                }
                bowSkill(10);
                flags[kFLAGS.KINDRA_DAILY_TRAINING] = 1;
                doNext(camp.returnToCampUseFourHours);
            ...
            else if (player.statusEffectv1(StatusEffects.Kindra) >= 50)
            {
                ...
                if (player.statusEffectv1(StatusEffects.Kindra) >= 90 && player.spe >= 150 && player.inte >= 120)
                {
                    ...
                    if (!player.hasStatusEffect(StatusEffects.KnowsBarrage))
                    {
                        player.createStatusEffect(StatusEffects.KnowsBarrage, 0, 0, 0, 0);
                    }
                    if (flags[kFLAGS.KINDRA_ADV_ARCHERY] <= 2)
                    {
            ...
            else if (player.statusEffectv1(StatusEffects.Kelt) >= 100 && player.statusEffectv1(StatusEffects.Kindra) < 50)
            {
                ...
                if (player.statusEffectv1(StatusEffects.Kindra) < 10)
                {
                    player.createStatusEffect(StatusEffects.Kindra, 10, 0, 0, 0);
                }
                else
                {
                    bowSkill(10);
