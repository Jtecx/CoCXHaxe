...
class LilyFollower extends NPCAwareContent implements SaveableState
{
    ...
    public static var LilyTalked : Float;
    public static var LilyFollowerState : Bool;
    public static var LilyExhibitionAble : Bool;
    public static var Lily3SomeSidonie : Bool;
    public static var Lily3SomeIzma : Bool;
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            LilyHairColor = Reflect.field(o, "LilyHairColor");
            LilySkinTone = Reflect.field(o, "LilySkinTone");
            LilyAffectionMeter = Reflect.field(o, "LilyAffectionMeter");
            LilySubmissivenessMeter = Reflect.field(o, "LilySubmissivenessMeter");
            LilyTalked = Reflect.field(o, "LilyTalked");
            LilyFollowerState = Reflect.field(o, "LilyFollowerState");
            LilyExhibitionAble = Reflect.field(o, "LilyExhibitionAble");
            Lily3SomeSidonie = Reflect.field(o, "Lily3SomeSidonie");
            Lily3SomeIzma = Reflect.field(o, "Lily3SomeIzma");
        }
    ...
    public function lilyAffection(changes : Float = 0) : Float
    {
        LilyAffectionMeter += as3hx.Compat.parseInt(changes);
        if (LilyAffectionMeter > 100)
        {
            LilyAffectionMeter = 100;
        }
        else if (LilyAffectionMeter < 0)
        {
            LilyAffectionMeter = 0;
        }
    ...
    public function lilySubmissiveness(changes : Float = 0) : Float
    {
        LilySubmissivenessMeter += as3hx.Compat.parseInt(changes);
        if (LilySubmissivenessMeter > 100)
        {
            LilySubmissivenessMeter = 100;
        }
        else if (LilySubmissivenessMeter < 0)
        {
            LilySubmissivenessMeter = 0;
        }
    ...
    public function LilyAfterBattle() : Void
    {
        ...
        else if (LilySubmissivenessMeter <= 40)
        {
        ...
        else if (LilySubmissivenessMeter <= 60)
        {
        ...
        else if (LilySubmissivenessMeter <= 80)
        {
        ...
        addButton(2, "Sex", LilySex).disableIf(player.lust < 33, "Your lust is too low.").disableIf(player.isGenderless(), "Not for genderless.").disableIf(LilyAffectionMeter < 20, "Req. 20%+ affection.", "???");
        addButton(3, "Rape", LilyRape).hint("Fuck the drider-girl senseless. " + ((LilySubmissivenessMeter == 0) ? "<b>Maybe that's exactly what she needs?</b>" : "You are sure she liked the first time...")).disableIf(player.lust < 33, "Your lust is too low.").disableIf(player.isGenderless(), "Not for genderless.");
        addButton(5, "DomHome", LilySubComeCamp).disableIf(LilySubmissivenessMeter < 80, "Req. 80%+ submissiveness.", "???");
        addButton(6, "Come2Camp", LilyBringCamp).disableIf(LilyAffectionMeter < 50, "Req. 50%+ affection.", "???");
    }
    ...
    public function LilyTalk() : Void
    {
        ...
        if (LilyTalked > 0)
        {
            outputText("She stands, looking almost disappointed. \"<i>Oh, is that all you wanted? Then why…? She shakes her head. “Very well, what is it?</i>\"");
        }
        else
        {
        ...
        {
            addButton(0, "Back", LilyCampFollower, true);
        }
        else
        {
            addButton(0, "Leave", cleanupAfterCombat);
        ...
        if (LilyTalked > 1)
        {
            addButton(2, "Driders", LilyTalkDriders);
        }
        else
        {
            addButtonDisabled(2, "Driders", "Req. to talk about 'Herself' first.");
        }
        if (LilyTalked > 2)
        {
            addButton(3, "D.Society", LilyTalkDriderSociety);
        }
        else
        {
            addButtonDisabled(3, "D.Society", "Req. to talk about 'Driders' first.");
        }
    ...
    public function LilySubComeCamp() : Void
    {
        ...
        addButton(1, "Tyrantia", LilyTyrantia3some1st).disableIf(player.isGenderless(), "Not for genderless ones.").disableIf(TyrantiaFollower.TyrantiaFollowerStage < 4, "Maybe there's an alternative?", "???");
    }
    ...
    public function LilyTyrantia3Fuck() : Void
    {
        ...
        if (!LilyFollowerState || recalling)
        {
    ...
    public function LilySex() : Void
    {
        ...
        addButton(2, "Bondage", LilyBondage).disableIf(LilySubmissivenessMeter < 40, "Req. 40%+ submissiveness.");
        addButton(3, "3Somes", Lily3Somes).disableIf(!LilyFollowerState, "Not yet!", "???");
        addButton(4, "Exhibition", LilyExhibition).disableIf(LilySubmissivenessMeter < 100, "Maybe if she's a full sub?", "???");
    }
    ...
    public function LilyBondage() : Void
    {
        ...
        addButton(1, "Cabin", LilyBondageCabin).disableIf(!LilyFollowerState, "Not yet!", "???");
        addButton(4, "Exhibition", LilyExhibition).disableIf(LilySubmissivenessMeter < 100, "Maybe if she's a full sub?", "???").disableIf(!LilyFollowerState, "Not yet!", "???");
    }
    ...
    public function Lily3Somes() : Void
    {
        ...
        addButton(1, "Tyrantia", LilyTyrantia3some).disableIf(TyrantiaFollower.TyrantiaFollowerStage < 4, "Need another spider in camp for this one.", "???");
        //addButton(2, "Izma",LilyIzma3Some);
        //addButton(3, "Kiha",LilyKiha3Some);
        if (LilyFollowerState)
        {
            addButton(14, "Back", LilyFollowerSex);
        }
        else
        {
            addButton(14, "Back", LilySex);
        }
    }
    ...
    public function LilyExhibition() : Void
    {
        ...
        {
            outputText("Ember sticks " + (((flags[kFLAGS.EMBER_GENDER] == 2 || flags[kFLAGS.EMBER_GENDER] == 3)) ? "her" : "his") + " head out of " + (((flags[kFLAGS.EMBER_GENDER] == 2 || flags[kFLAGS.EMBER_GENDER] == 3)) ? "her" : "his") + " lair, seeing you establish your dominance. She rolls " + (((flags[kFLAGS.EMBER_GENDER] == 2 || flags[kFLAGS.EMBER_GENDER] == 3)) ? "her" : "his") + " eyes at the crass display, taking off.\n\n");
        }
        ...
        {
            outputText("Etna perks up at the lewd display, scampering over and rubbing her pussy-tail on her thigh. <i>\"Nyaaa, how fun!\"</i>\n\n");
        }
        ...
        if (izmaFollower() && flags[kFLAGS.IZMA_NO_COCK] == 0 && flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 0)
        {
            outputText("Your shark-lover, Izma" + ((flags[kFLAGS.IZMA_BROFIED] == 1) ? "el" : "") + " surfaces, seeing your display. Already rock-hard, " + ((flags[kFLAGS.IZMA_BROFIED] == 1) ? "" : "s") + "he approaches, a grin on h" + ((flags[kFLAGS.IZMA_BROFIED] == 1) ? "is" : "er") + " face. <i>\"Having some fun, my Alpha?\"</i>\n\n");
        }
        if (flags[kFLAGS.SIDONIE_FOLLOWER] == 1)
        {
            outputText("Sidonie sees the tied up Drider, waggling her eyebrows at you. You notice that she already has a tent in her pants, and she walks over, eyeing up Lily. <i>\"Interesting. Caught some fresh meat for us, [name]?\"</i>\n\n");
        }
        menu();
        //addButton(1, "Etna", LilyExEtna);
        addButton(2, "Electra/Luna", LilyExElectra).disableIf(!player.hasCock(), "Req. a cock!").disableIf(flags[kFLAGS.ELECTRA_FOLLOWER] <= 1 || player.hasStatusEffect(StatusEffects.ElectraOff) || flags[kFLAGS.LUNA_FOLLOWER] < 4 || player.hasStatusEffect(StatusEffects.LunaOff), 
                "You don't have someone for this one... yet?", "???"
        );
        //addButton(3, "Izma/el", LilyExIzma);
        addButton(4, "Joy", LilyExJoy).disableIf(flags[kFLAGS.JOY_COCK_SIZE] >= 1, "Joy can't have a dick!", "???").disableIf(flags[kFLAGS.JOJO_BIMBO_STATE] != 3, "You don't have someone for this one... yet?", "???");
    ...
    public function LilyReward() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("[cock]. She begins expertly jerking you off, the pressure on your previously unstimulated shaft" + ((player.cockTotal() > 1) ? "s" : "") + " almost enough to get you off again.");
        }
        else
        {
            outputText("[breasts]. She takes a nipple in two fingers, rolling the nub around and pulling on it.");
        }
    ...
    public function LilyCampFollower(back : Bool = false) : Void
    {
        clearOutput();
        if (!back)
        {
        ...
        addButton(1, "Talk", LilyTalk);
        addButton(2, "Sex", LilyFollowerSex).disableIf(player.lust < 33, "Your lust is too low.").disableIf(player.isGenderless(), "Not for genderless.");
        if (LilyTalked > 4)
        {
            addButton(3, "Learning", LearningWithLily).hint("Learning with Lily.");
        }
        else
        {
            addButton(3, "House", LearningWithLily);
        }
        addButton(4, "Spar", LilySpar).disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, "You need a good sparring ring for that.");
    ...
    public function LearningWithLily() : Void
    {
        ...
        else
        {
            ...
            {
                addButton(1, "Yes", LilyFollowerSex);
            }
            else
            {
                addButtonDisabled(1, "Yes", "Your lust is too low or you're genderless. (or both?)");
            }
    ...
    public function LearningWithLilyYes(book : Float) : Void
    {
        ...
        if (player.statStore.hasBuff("Learning with Lily"))
        {
            player.statStore.removeBuffs("Learning with Lily");
        }
        switch (book)
        {
            case 1:
                player.buff("Learning with Lily").withText("Learning with Lily").setStats({
                                    spe.mult : 0.4
                                }).forHours(192);
            case 2:
                player.buff("Learning with Lily").withText("Learning with Lily").setStats({
            ...
            case 5:
                if (player.lust < Math.round(player.maxLust() * 0.7))
                {
                    player.lust = Math.round(player.maxLust() * 0.7);
                }
            default:
    ...
    {
        if (flags[kFLAGS.LILY_DEFEATS_COUNTER] >= 1)
        {
            flags[kFLAGS.LILY_DEFEATS_COUNTER]++;
        }
        else
        {
            flags[kFLAGS.LILY_DEFEATS_COUNTER] = 1;
        }
        if (flags[kFLAGS.LILY_LVL_UP] < 9 && flags[kFLAGS.LILY_DEFEATS_COUNTER] >= flags[kFLAGS.LILY_LVL_UP] + 3)
        {
            var addToV2 : Float = player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * flags[kFLAGS.LILY_DEFEATS_COUNTER];
