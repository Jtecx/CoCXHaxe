...
class SheilaScene extends NPCAwareContent implements TimeAwareInterface
{
    ...
    {
        return (flags[kFLAGS.KNOWS_SHEILA_NAME] != null) ? "Harriet" : "Sheila";
    }
    
    private function sheilaPreg(reducedChance : Bool = false) : Void
    ...
    public function sheilaCorruption(arg : Float = 0) : Float
    //No upper limit - grow these boobs yooo
    {
        
        if (flags[kFLAGS.SHEILA_CORRUPTION] + arg < 0)
        {
            flags[kFLAGS.SHEILA_CORRUPTION] = 0;
        }
        else
        {
            flags[kFLAGS.SHEILA_CORRUPTION] += arg;
        }
    ...
    private function sheilaCup() : String
    {
    ...
    public function sheilaEncounterRouter() : Void
    {
        ...
        //PREG SHIT{
            ...
            else if (flags[kFLAGS.SHEILA_XP] == 2 || (flags[kFLAGS.SHEILA_XP] == 3 && model.time.hours != 20))
            {
                sheilaGettingFamiliar();
            }
            //XP3: Sexy Time (sheila xp = 3 AND time = 20:00 and demon sheila = 0):
            else if (flags[kFLAGS.SHEILA_XP] == 3 && model.time.hours == 20)
            {
                sheilaXPThreeSexyTime();
            }
            //XP4: Fancy meeting you here (sheila xp >= 4 and demon sheila = 0):
            else if (flags[kFLAGS.SHEILA_XP] == 4)
            {
    ...
    private function sheila1ndEncLookCloserPtII() : Void
    {
        ...
        if (player.weaponName == "Scarred Blade")
        {
            outputText("As the blade feels her gaze, it vibrates eagerly toward her, sliding partway out of the oiled scabbard; she stares into the exposed metal transfixed, as if looking into the diary of a past life.  Abruptly, she shakes her head and turns away, and the saber calms down.  ");
        }
        ...
        if (player.lib >= 50)
        {
            outputText("  Noting you still checking out her body, she smirks and says, \"<i>Enjoying your lookie, then?  Up here, cobber,</i>\" causing you to snap your head up sheepishly.");
        }
        outputText("  You nod at the strange woman and introduce yourself.");
        if (player.short != "Sheila")
        {
            outputText("  \"<i>[name], eh?  My mates just call me Sheila.  Bunch of jokers, the bastards.</i>\"");
        }
        else
        {
            outputText("  \"<i>You too, eh?  Sheila's what my mates call me for a nickname.  Don't suppose you got yours from a bunch of adolescent-minded wankers, though.</i>\"");
        }
        ...
        //[(high demon score)
        if (player.isRace(Races.DEMON, 1, false) || player.isRace(Races.IMP, 1, false))
        {
            outputText("  I mean, take a squizz at yourself.  It's like you're not even trying to hide it; I've seen imps that look more like normal people.");
        }
    ...
    private function sheilaIsSorrySheRapedYou() : Void
    {
        ...
        {
            addButton(2, "CastArouse", sheilaPologyCastArouse);
        }
        else
        {
            sceneHunter.print("If you knew Arouse spell, you could try something... special.");
        }
    ...
    private function sheilaReallyMad() : Void
    {
        ...
        if (flags[kFLAGS.SHEILA_CORRUPTION] >= 90)
        {
            outputText("It's so hard to remember myself anymore, and the taint you've put in my body goes crazy when you touch me.  Do you want me to get on my hands and knees?</i>\"  The woman lowers herself to the ground.  \"<i>I beg you.  ");
        }
        outputText("Please, please leave me alone.");
        if (flags[kFLAGS.SHEILA_XP] == -3)
        {
            outputText("  I know you... you're not as bad as the rest of the monsters out here, so... please.");
        }
        outputText("</i>\"");
        //[Stand][Leave]
        menu();
    ...
    private function sheilaReconcileKay2() : Void
    {
        ...
        if (player.lib < 50)
        {
            outputText("the beckoning hunks of meat");
        }
        else
        {
            outputText("her rising and falling breasts under the pretext of watching her work");
        }
        ...
        //[(pc is kangaroo)
        if (player.isRace(Races.KANGAROO, 1, false))
        {
            outputText(" - except you, I suppose");
        }
        ...
        if (player.lib >= 50)
        {
            outputText(" like yourself - I saw you perving -");
        }
    ...
    private function sheilaFriendlyHerPeoples() : Void
    {
        ...
        {
            outputText("  She looks absently at your crotch.");
        }
        ...
        else
        {
            ...
            if (player.hasCock())
            {
                outputText(", and you raise the kids at home?  Or put them with a nanny");
            }
            outputText("?</i>\"");
            ...
            if (player.hasCock())
            {
                outputText(", and I'll turn the kids over to");
            }
            outputText(" - no, I mean, ");
            if (!player.hasCock())
            {
                outputText("if I decide");
            }
            else
            {
                outputText("I'm not ready for");
            }
    ...
    private function shielaXPThreeSexyTimePostSexStayII() : Void
    {
        ...
        function dickFitF() : Void
        {
            ...
            if (player.isTaur())
            {
                outputText(" and you playfully roll");
            }
            ...
            if (!player.isTaur())
            {
                outputText(" just above one " + nippleDescript(0));
            }
            else
            {
                outputText(" on the thigh");
            }
            ...
            if (!player.isTaur())
            {
                outputText("Your hands squeeze her hips almost of their own volition, helping her slam down with force, and her head rolls back on her neck.  ");
            }
            outputText("Her ");
            //[(sheila corruption < 30)]
            if (flags[kFLAGS.SHEILA_CORRUPTION] < 30)
            {
                outputText("little breasts rise and fall with increasing frequency atop her ribcage as her pace picks up and her breaths come short and shallow.");
            }
            else if (flags[kFLAGS.SHEILA_CORRUPTION] > 60)
            {
                outputText("huge tits jiggle magnificently as she rocks, nearly slapping her in the face as she plunges down and then falling back into place with waves of rippling flesh.");
            }
            else
            {
                outputText("breasts quiver invitingly as she pumps your cock, but before you can think of grabbing one yourself, both of her hands are squeezing and caressing them, drawing sighs from the woman as she toys with her erect nipples.");
            }
            ...
            {
                outputText("  Her pussy clamps down around you, the rippling muscles so tight that she wouldn't even be able to pump if it weren't for the hot river of juice you feel around your tool.  \"<i>A-ah,</i>\" [sheilaname] groans, \"<i>you thrust too!  Rail me silly!</i>\"  She tries to push against you with her hands, to return herself to an upright position, but can barely pull her chest apart from you - her nipples just narrowly recede from the surface of your " + player.skinFurScales() + " as her efforts peak.");
            }
            //(else if magically equal but < 90)
            else
            {
                outputText("  Her pussy squirts and sucks alternately, trying to find purchase on your cock even as her pumping hips deny it; the sensation drives you wild.  \"<i>[name], your old fella is amazing...</i>\"");
            }
            ...
            {
                outputText("your dick twitches inside her");
            }
            //(multicox)
            else
            {
                outputText("she spies the first bead of pearly semen exiting");
            }
            if (player.cockTotal() == 2)
            {
                outputText(" your lonely, unused prick");
            }
            if (player.cockTotal() > 2)
            {
                outputText(" each of your lonely, unused pricks");
            }
            outputText(".");
            ...
            if (player.cockTotal() > 1)
            {
                outputText(", to be joined shortly by a messy external eruption that covers her thighs and belly in blotches of gooey white");
            }
            outputText(".");
            //[(big skeet)
            if (player.cumQ() >= 1000)
            {
                outputText("  It rolls out of you in waves, packing [sheilaname]'s cunt until she fills to capacity and the excess is forced out, sliming your own crotch.");
            }
            ...
            var normalEnd : Void->Void = function() : Void
            {
        ...
        function vagF() : Void
        {
            ...
            if (player.isTaur())
            {
                outputText("You turn around and straddle a leg, squatting to bring your [vagina] atop her crotch.  \"<i>Push up,</i>\" you order, and [sheilaname] obeys eagerly, twisting her body to connect her excited pussy with yours.");
            }
            //(else if naga)
            else if (player.isNaga())
            {
                outputText("You slide it under her and lift gently; [sheilaname] plays along with a disquieted expression, rolling over and raising her pussy into the air, and you press your own against her butt as your tail slides between her legs.  \"<i>See, you just need to be more... flexible,</i>\" you quip.");
            }
            //(else if not horse and not naga)
            else
            {
                outputText("You straddle her legs with your own, pressing her pussy into yours; [sheilaname] gasps as the hot flesh connects and you tentatively rock your hips, dragging your lower lips across.");
            }
            ...
            {
                outputText(" even though they're small and humble");
            }
            //(else sheila corruption > 60)
            else if (sheilaCorruption() > 60)
            {
                outputText("; I worry that they're getting too mountainous");
            }
            ...
            if (player.isNaga())
            {
                outputText(" tail, drawing it up the hot opening and coiling it so you can return the warm, lube-slicked length for another pass even as you hump her butt.");
            }
            else
            {
                outputText(" cunt, grinding her like a sex toy in search of your own climax.");
            }
            ...
            if (player.cor >= 90 || player.cor > sheilaCorruption())
            {
                ...
                if (player.isNaga())
                {
                    outputText("  Her breasts swell in your hands as she sucks in her breath sharply, feeling your pussy drool its juice onto her ass.");
                }
            }
            ...
            else if (player.cor < sheilaCorruption())
            {
                ...
                if (player.isNaga())
                {
                    outputText("; it feels like her breasts are pulling away from you as she tries to push her ass further into your quivering quim");
                }
                outputText(".");
            }
            else
            ...
            if (player.hasCock())
            {
                ...
                if (player.cockTotal() == 1)
                {
                    outputText("prick quakes and spurts");
                }
                else
                {
                    outputText("pricks quake and spurt");
                }
        ...
        function fingerF() : Void
        {
            ...
            if (player.hasCock())
            {
                outputText(", stroking your oversized tool despite her dismissal");
            }
            ...
            if (player.hasCock())
            {
                ...
                if (!player.isTaur())
                {
                    outputText("falls into your lap");
                }
                //(horse)
                else
                {
                    outputText("drifts between your legs");
                }
            ...
            if (player.cor >= 90 || player.cor > sheilaCorruption())
            {
                outputText("  Her vagina heats up, squeezing your fingers so hard that you nearly can't pull out to rub her again, and you have a clear view of her breasts swelling up - arguable proof of your prowess, considering what she told you earlier.  Her people must be terrible at faking orgasm.");
            }
            //(pc corr < sheila corruption)
            else if (player.cor < sheilaCorruption())
            {
                outputText("  Her vagina gushes with clear fluid, filling the bowl made by your hand with warmth, and you think you can see her breasts tightening up slightly as her back arches, though it's hard to be sure with how she's shaking.");
            }
            else
            {
                outputText("  Her pussy quivers around you, and you draw the fingers out and pop them into [sheilaname]'s mouth.  The woman sucks on them greedily, and you raise an eyebrow at her.  She blushes again, but it's evidently not the first time she's done the same thing!");
            }
            if (!recalling)
            {
                if (player.hasCock())
                ...
                {
                    dynStats("lus", player.lib / 4, "scale", false);
                }
            }
            ...
            //[(cocks)
            {
                ...
                if (player.hasCock())
                {
                    ...
                    if (player.cumQ() > 1000)
                    {
                        outputText("  Her mouth fills quickly, and she writhes in your grip, managing to turn her head.  \"<i>Oh god, no way!</i>\" she coughs, spitting up enough semen to coat her chin as your ongoing orgasm spatters against her cheek.");
                    }
                    //[(mega skeet)
                    if (player.cumQ() >= 2000)
                    {
                        outputText("  Not satisfied with that alone, you turn her head the other way for your finish, ejaculating on the rest of her face and completing her gooey, white beard as she squirms.");
                    }
                    //[(multicock)
                    if (player.cockTotal() > 1)
                    {
                        outputText("  The other strokes not coming from this cock do their part to decorate your lover as well, turning her hair into a slimy mess.");
                    }
                ...
                if (player.hasCock())
                {
                    outputText(", her face still covered in globby white smears");
                }
                outputText(".  \"<i>Fuck...</i>\" she groans.  \"<i>You're a jerk, [name].");
                //[(cock.length > 0)
                if (player.hasCock())
                {
                    outputText("  I can't believe you called me a goblin, you arsehole.");
                }
                outputText("</i>\"");
                ...
                if (player.hasCock())
                {
                    outputText("can't believe how much she acts like one sometimes... but you ");
                }
                ...
                if (player.hasCock())
                {
                    outputText(", even after [sheilaname] finishes wiping her face clean and stares at you");
                }
    ...
    private function sheilaXPThreeSexyTimeGuardDuty() : Void
    {
        ...
        if (player.level >= 8)
        {
            outputText(" - which, you suppose, must include imps, even though they're so far beneath you, they might as well be underground");
        }
        ...
        else
        {
            ...
            if (player.hasCock())
            {
                outputText("shift your pelvis forward");
            }
            //(cock.length == 0)
            else
            {
                outputText("stick out your tongue playfully");
            }
    ...
    private function fuckBuddySheilaMeetingJustHangOut() : Void
    {
        ...
        {
            outputText("scoot your own forward");
        }
        else
        {
            outputText("fold your legs under you");
        }
    ...
    private function fuckBuddySheilaMeetingChat() : Void
    {
        ...
        if (flags[kFLAGS.SHEILA_JOEYS] > 0)
        {
            addButton(4, "Kids", fuckBuddySheilaMeetingChatKids);
        }
    }
    ...
    private function fuckBuddySheilaMeetingChatOldLoves() : Void
    {
        ...
        else
        {
            ...
            if (player.cor >= 60)
            {
                outputText(" - though it's probably meant to be a defensive gesture, all it does is push her tits together under her white shirt and frame them in light-brown skin");
            }
            ...
            if (player.cor < 30)
            {
                outputText("; you lay a hand on her shoulder reassuringly");
            }
    ...
    private function fuckBuddySheilaMeetingChatKids() : Void
    {
        ...
        else
        {
            ...
            if (player.lib < 50)
            {
                ...
                if (player.cor < 40)
                {
                    outputText("  You pat her shoulder carefully and suggest, \"<i>I could watch them for a while.</i>\"");
                }
    ...
    //[XP4 - Sex]
    private function sheilaXP4Sex(display : Bool = false) : Void
    {
        if (display)
        {
            clearOutput();
        ...
        menu();
        addButton(0, "69WithWang", curry(sceneHunter.callFitNofit, consensualSheila69, 32)).disableIf(!player.hasCock(), "Req. a cock!");
        addButton(1, "69WithCooch", consensualSheila69, -1).disableIf(!player.hasVagina(), "Req. a vagina!");
        addButton(2, "Fuck Her", consentacleVagSexForKangarooSlutBitches).disableIf(player.cockThatFits(32) < 0, "Req. a cock fitting 32 area.");
        //da scene
    ...
    {
        var cock : Bool = x >= 0;
        var fits : Bool = cock && player.cockArea(x) < 32;
        var x1 : Int = as3hx.Compat.parseInt(x + 1);
        clearOutput();
        ...
        else
        {
            ...
            if (player.clitLength > 8)
            {
                outputText(" and bring your " + clitDescript() + " to its erect state.  \"<i>... And quite a stem,</i>\" your lover concludes.  \"<i>You sure that's not a dick?</i>\"");
            }
            else
            {
                outputText(", producing a slight sigh and a wet finger.  \"<i>And I'm the eager one, right?</i>\" your lover asks, wryly - though her finger is just as wet.");
            }
        }
        ...
        if (cock && !fits)
        {
            outputText("  \"<i>[name], I'm telling you it won't fit!</i>\"");
        }
        else
        {
            outputText("  \"<i>Not talking, then?  Please yourself.</i>\"");
        }
        outputText("  Her commentary cuts off as you ");
        if (!player.isTaur())
        {
            outputText("turn around and hang your ");
        }
        else
        {
            outputText("straddle her and squat down to dangle your ");
        }
        if (cock)
        {
            outputText("pre-seeping shaft");
        }
        else
        {
            outputText("wet pussy");
        }
        ...
        if (cock)
        {
            ...
            if (fits)
            {
                outputText(" and guiding it to bend comfortably against her chest, drooling a trail of pre-cum as she slides it into position");
            }
            outputText(".");
        }
        //(vag)
        ...
        {
            outputText("returning the favor and plunging into your " + vaginaDescript(0) + " with her hand.");
        }
        ...
        if (!player.isTaur())
        {
            outputText("flattening your back");
        }
        else
        {
            outputText("squatting lower");
        }
        outputText(" so your ");
        if (cock)
        {
            ...
            if (fits)
            {
                outputText(" into her warm, waiting mouth");
            }
            else
            {
                outputText("; she lets go of it to grab her breasts instead, trying to push them around the sensitive shaft as she runs her tongue up the underside");
            }
            outputText(".");
        }
        else
        {
            ...
            if (player.clitLength > 8)
            {
                outputText("nips and sucks at the tip of your clit, rolling it with her tongue");
            }
            else
            {
                outputText("parts your labia with her tongue, stroking them gently and tickling your little clitoris");
            }
            outputText(".");
        }
        ...
        if (!player.isTaur())
        {
            outputText("lean down and bury your own tongue all the way inside [sheilaname], forcing her labia open with your mouth");
        }
        else
        {
            outputText("reach as far as you can to bring both hands into play, stroking and pulling [sheilaname]'s labia apart with one as you tease her exposed clitoris or slip inside her vagina with the other");
        }
        ...
        if (!cock)
        {
            ...
            if (player.clitLength > 8)
            {
                outputText(" and stroking your " + clitDescript() + " rapidly with one hand");
            }
            outputText(".");
        }
        //(smallcox)
        ...
        {
            outputText("taking you into her mouth once again and licking away a neglected drop of pre-cum.");
        }
        else
        {
            outputText("stroking your cock with one hand as she pushes an erect nipple against its side to trace a line.");
        }
        ...
        if (!player.isTaur())
        {
            ...
            if (cock && !fits)
            {
                outputText("shaking, teasing nipples and heaving chest");
            }
            else
            {
                outputText("mouth, humming in delight");
            }
            outputText(".");
        }
        //(horse)
        ...
        else if (player.cor < flags[kFLAGS.SHEILA_CORRUPTION])
        {
            ...
            if (!player.isTaur())
            {
                outputText("mouth, filling you with her taste.");
            }
            else
            {
                outputText("hands, completely soaking them in her orgasm.");
            }
        ...
        if (cock && !fits)
        {
            outputText("pressing her lips and tongue to the base of your shaft as she masturbates it vigorously.");
        }
        ///(small cox)
        else if (!cock)
        {
            outputText("penetrating you deeply with her tongue as her hands reach up to stroke your clit furiously.");
        }
        else
        {
            outputText("sinking your [cockhead " + x1 + "] into the warm recesses of her throat where her muscles caress and coax it.");
        }
        ...
        if (!cock)
        {
            ...
            if (player.wetness() <= 2)
            {
                outputText("trickle out, and are quickly sucked up by her greedy mouth");
            }
            else if (player.wetness() <= 4)
            {
                outputText("gush freely, leaving her face slick");
            }
            else
            {
                outputText("overflow, soaking her hair even as she shuts her eyes tight against the cataract");
            }
        ...
        else
        {
            ...
            if (fits)
            {
                ...
                if (player.cumQ() < 250)
                {
                    outputText("; you withdraw from her, allowing her throat to force your ejaculate back up and over her lips and cheeks with a splutter.");
                }
                //(big cum)
                else if (player.cumQ() < 2000)
                {
                    outputText(", but your relentless orgasm forces it down again, dumping more jism into her throat.  You can feel her hands clawing at your [butt] as you relieve your buildup.");
                }
                //[(mega cum)
                else
                {
                    outputText(".  Soon, her visibly-rounding stomach fills and the tip of your dick is once again washed in backflow; the next spurts force the cum all the way up, and she begins drooling the slimy semen-saliva blend from her mouth and nose, hands all the more frantic in her alarm.");
                }
            }
            //(else doesn't fit)
            else
            {
                ...
                if (player.cumQ() > 1000)
                {
                    outputText("  A wellspring of seed, your body continues producing until the fuzz on her thighs and tail is completely matted down with the whiteness drooling between you.");
                }
                //(multicocks)
                if (player.cockTotal() > 1)
                {
        ...
        if (fits)
        {
            outputText(", pulling free of her throat.  She immediately turns over as well and forces herself to her hands and knees, then gags and coughs up a large amount of your jism.  When she's dumped enough to clear her passageway, she collapses forward, smearing the mess onto her chest as she spends a few minutes to catch her breath.  Once she's got it, she turns back over and sits up, strands of your slime linking her breasts to the grass.");
        }
        //(else not smallcox)
        else
        {
            outputText(".  She sits up, wiping sweat and stray fluids away from her eyes.");
        }
        if (!recalling)
        {
            player.sexReward("vaginalFluids", (cock) ? "Dick" : "Vaginal");
        }
        ...
        //(else if no corruption warning)
        {
            ...
            else
            {
                ...
                if (fits)
                {
                    outputText(", smearing your dangling hand into the mess on her tits");
                }
    ...
    private function consentacleVagSexForKangarooSlutBitches() : Void
    {
        ...
        {
            outputText("squeezing and fondling");
        }
        else
        {
            outputText("plunging two fingers into");
        }
        ...
        //[(if wangaroo)
        if (player.cocks[player.cockThatFits(32)].cockType == CockTypesEnum.KANGAROO)
        {
            outputText("tapered, fleshy penis.  \"<i>Wow... you never see those.</i>\"");
        }
        else
        {
        ...
        if (!player.isTaur())
        {
            outputText("Taking hold of your " + cockDescript(x));
        }
        //(centard)
        else
        {
            outputText("Attempting to maneuver your " + cockDescript(x) + " into position");
        }
        outputText(", you rub the head along her dripping snatch, smearing her fluid over it.  ");
        //[(non-cent)
        if (!player.isTaur())
        {
            outputText("Gripping her fuzzy thighs next, you lift her bottom and line");
        }
        //(hoers)
        else
        {
            outputText("She lifts her bottom and lines");
        }
        ...
        if (sheilaCorruption() <= 30)
        {
            outputText("her perky breasts rock back and forth");
        }
        else
        {
            outputText("her jiggling breasts flatten into perfect domes and undulate on her chest");
        }
        ...
        if (player.hasBalls() || player.hasVagina())
        {
            ...
            {
                outputText("Your cunt twitches and drools a bit of fluid on the furry intruder.  ");
            }
        }
        outputText("You fuck harder, stuffing [sheilaname] with such force that her ");
        if (sheilaCorruption() < 30)
        {
            outputText("perky, erect nipples trace long arcs against your skin");
        }
        else
        {
            outputText("jiggly tits nearly slap into her face");
        }
        ...
        if (player.cor >= 90 || player.cor > flags[kFLAGS.SHEILA_CORRUPTION])
        {
            ...
            if (player.cockTotal() > 1)
            {
                ...
                if (player.cockTotal() == 2)
                {
                    outputText("dick erupts");
                }
                else
                {
                    outputText("dicks erupt");
                }
        ...
        else
        {
            ...
            if (player.cockTotal() > 1)
            {
                outputText(" or onto her thighs and belly");
            }
            outputText(".");
        }
        ...
        if (!player.hasKnot(x))
        {
            ...
            if (player.cumQ() > 500)
            {
                outputText(", breasts, and face");
            }
            outputText(".");
            //[(Huge skeet)
            if (player.cumQ() >= 1500)
            {
                outputText("  More and more globs of semen shoot from the end of " + sMultiCockDesc() + ", pasting [sheilaname] to the dry grass as you paint her body white.");
            }
        }
        //(dog cock)
        else
        {
            ...
            if (player.cumQ() >= 1000)
            {
                ...
                if (player.cumQ() >= 3000)
                {
                    outputText(", until it's stretched as far as possible and each successive jet forces out squirts of frothy jism from the tight seal her pussy makes around the knot");
                }
                outputText(". ");
            }
            //[(multi)
            if (player.cockTotal() > 2)
            {
                outputText("Your other cocks erupt as well, soaking her crotch and matting down the fur on her thighs with whiteness. ");
            }
        }
        ...
        if (player.hasKnot(x))
        {
            outputText(" and you finally manage to pull out of her");
        }
        ...
        var normalEnd : Void->Void = function() : Void
        {
            ...
            else
            {
                ...
                if (player.cumQ() < 500)
                {
                    outputText("as she strokes the " + player.skinFurScales() + " of your chest");
                }
                else
                {
                    outputText("as she wipes off as much jizz as she can");
                }
...
let me know if this is totally heinous or whatever*/
    ...
    private function sheilaMutualMasturbation() : Void
    {
        ...
        if (player.hasCock())
        {
            if ((player.isTaur() && player.tallness * (5 / 6) < player.cocks[player.longestCock()].cockLength) || player.hasCock())
            {
                choices[choices.length] = "cock";
            }
        }
        if (player.hasVagina() && !player.isTaur())
        {
        ...
        if (player.gender == 0 || choices.length == 0)
        {
        ...
        if (player.isTaur())
        {
            outputText(", folding your legs under you,");
        }
        ...
        {
            outputText(" sliding your hand to your [vagina], parting your lips with two fingers");
        }
        //(gross-ass nipcunts)
        else if (target == "nipplecunt")
        {
            outputText("plunging your fingers into one of your awkward, distended nipples");
        }
        //(Unsexed non-centaur only)
        else if (target == "ass" && !player.isTaur())
        {
            outputText("licking a finger and teasing your asshole with shallow strokes, gently circling the ring");
        }
        //(centaur who hasn't got shit)
        else
        {
            outputText("sucking a single digit salaciously, fingerfucking your mouth and coating it with saliva");
        }
        ...
        if (target == "cock")
        {
            outputText("begin stroking your shaft, drooling pre-cum onto her skin and ");
        }
        //(non-horse vag)
        else if (target == "cunt")
        {
            outputText("demonstrate by slipping your fingers into your own [vagina] and ");
        }
        ...
        if (target == "cock")
        {
            outputText("You return your hand to your [cock], pumping vigorously to catch up to your lover, spurred onward by the smooth skin she presses into your lips.");
        }
        //(else if nonhorse vag)
        else if (target == "cunt")
        {
            outputText("You slide your hand between your [legs] again, intent on recovering the march [sheilaname]'s stolen on you, and begin rubbing your [clit] vigorously while sighing into her breast.");
        }
        //(else if RNG disgusting nipplecunts)
        else if (target == "nipplecunt")
        {
            outputText("Your hand returns to your chest, slipping inside the sensitive flesh of your nipple, and you groan into [sheilaname]'s as a trickle of moisture falls from your chest.");
        }
        //(else if non-centaur asshole)
        else if (!player.isTaur())
        {
            outputText("Your hand reaches back to your [asshole], neglected for too long, and a finger spears your pucker, stroking the nerves inside as you hurry to bring your arousal in line with [sheilaname]'s.");
        }
        ...
        else
        {
            ...
            if (silly())
            {
                outputText(", even just a paperclip");
            }
            outputText(".");
        }
        ...
        {
            outputText(", and yours joins her, slipping into her entrance alongside");
        }
        ...
        if (target == "cock")
        {
            if (player.cockTotal() == 1)
            {
                outputText("prick begins");
            }
            else
            {
                outputText("pricks begin");
            }
        ...
        {
            outputText("fingers are pulled into her vagina by her muscles as her orgasm begins.");
        }
        //[( pc corr >= 90 or > sheila corruption)
        if (player.cor >= 90 || player.cor > flags[kFLAGS.SHEILA_CORRUPTION])
        {
        ...
        else
        {
            ...
            {
                outputText("  Emptying your reserves, you allow yourself to fall into her arms, resting your head on her chest.");
            }
        }
        if (!recalling)
        {
        ...
        var normalEnd : Void->Void = function() : Void
        {
            ...
            if (target == "cock")
            {
                outputText("there, staring at the load you left in her bellybutton");
            }
            else
            {
                outputText("together");
            ...
            if (player.gender == 0 && silly())
            {
                outputText("  I love you, my Ken doll.");
            }
            outputText("</i>\"");
            //[(if sheila xp = 3)
            if (flags[kFLAGS.SHEILA_XP] == 3)
    ...
    @:allow(classes.scenes.nPCs)
    private function sheilaGotWhomped() : Void
    {
        clearOutput();
        ...
        {
            outputText("completely spread-eagled from her injuries");
        }
        ///(lust)
        else
        {
            outputText("rubbing her thighs together and smearing around the moisture from her pussy");
        }
        ...
        {
            outputText("  I'll forgive you for everything else, but please, not this!");
        }
        outputText("</i>\" she implores.");
        //[(sheila corruption >= 90)
        if (sheilaCorruption() >= 90)
        {
            outputText("  \"<i>I-if you touch me again... god, I can't even think straight anymore!  Don't do this!</i>\"");
        }
        if (player.lust >= 33)
        {
            menu();
            addButton(0, "Fuck Pussy", rapeSheilasCooter).disableIf(player.cockThatFits(48) < 0, "Req. a cock fitting 48 area.");
            addButton(1, "Forced BJ", sceneHunter.callFitNofit, forcedSheilaOral, 8, "length").disableIf(player.isTaur(), "Not for taurs anymore. Sorry.").disableIf(!player.hasCock(), "Req. a cock.");
            addButton(2, "Forced Lick", forcedSheilaOral, -1).disableIf(!player.hasVagina(), "Req. a vagina.");
            addButton(3, "Use Dildo", sheilaGetsRapedWithADildont).disableIf(player.hasKeyItem("Deluxe Dildo") < 0, "Requires Deluxe Dildo.");
    ...
    private function rapeSheilasCooter() : Void
    {
        ...
        if (player.isTaur())
        {
            outputText(" by planting your forehooves just above her hips");
        }
        ...
        if (monster.HP <= monster.minHP())
        {
            outputText("sliding it between her labia and producing a damp spot, then ");
        }
        ...
        function teaseHer() : Void
        {
            ...
            if (player.cumQ() >= 500)
            {
                outputText("  Soon you've filled her as fully as the shallow penetration will allow, and the panties begin to press back against your cock, oozing onto you whatever semen the porous dam can't contain.");
            }
            //(mega skeet)
            else if (player.cumQ() >= 1500)
            {
                outputText("  It sags and bows around the tip of your spear, pulsing with each new stroke as the hemlines leak jizz onto her legs and butt.");
            }
            ...
            if (player.cumQ() >= 500)
            {
                outputText("So much cum fills her pussy that it slips out, once again restoring the opacity, though not the dryness, of her panties.  ");
            }
        ...
        var beRough : Void->Void = function() : Void
        {
            ...
            if (player.cockArea(x) > 32)
            {
                outputText(" and the pain of taking a cock slightly too large for her");
            }
            ...
            {
                outputText(" jiggles her breasts and");
            }
            ...
            if (!player.isTaur())
            {
                outputText(", leaning down to kiss her long neck and collarbone");
            }
            ...
            if (!player.isTaur())
            {
                outputText(", transferring yours to her breast while brushing her torn shirt to the side");
            }
            outputText("; without struggling further, she slowly lowers it");
            //[(nothors)
            if (!player.isTaur())
            {
                outputText(" to rest atop your own");
            }
            outputText(", confirming your suspicion.  Her eyes close and her chin raises resignedly");
            //[(nothors)
            if (!player.isTaur())
            {
                outputText(" as you triumphantly squeeze the flesh in your hand and kiss her under the jaw");
            }
            outputText(".");
            ...
            if (!player.isTaur())
            {
                outputText("place a hand along her face, tuning it toward you, and kiss her on the lips.");
            }
            else
            {
                outputText("bend down, lowering your torso until you can caress her cheek.");
            }
            ...
            if (!player.isTaur())
            {
                outputText(" and squeeze her breast");
            }
            ...
            if (!player.isTaur())
            {
                outputText(" and she presses your hand uncomfortably");
            }
            ...
            if (player.cor >= 90 || sheilaCorruption() < player.cor)
            {
                ...
                if (!player.isTaur())
                {
                    outputText("; you can almost feel her breast heating up and swelling with blood under your hand, as the nipple pokes into your palm");
                }
                outputText(".");
            }
            //(PC corr < sheila corruption)
            else if (player.cor < sheilaCorruption())
            {
                ...
                if (!player.isTaur())
                {
                    outputText("  She squeezes your hand so hard that you swear you can feel her breast compressing under it as she screams her pleasure.");
                }
            }
            //(else equal but < 90)
            else
            ...
            if (player.cumQ() >= 500)
            {
                outputText("  Overabundant as always, your semen quickly fills her to capacity and begins to squirt out around your shaft, staining her thighs.");
            }
            //[(multicock)
            if (player.cockTotal() > 1)
            {
                ...
                if (player.cockTotal() == 2)
                {
                    outputText("dick spits");
                }
                else
                {
                    outputText("dicks spit");
                }
    ...
    private function forcedSheilaOral(x : Int) : Void
    {
        ...
        if (dick)
        {
            sceneHunter.print("Real differences in scene the scene appear when your dick is longer than 8 inches or thicker than 6 inches.");
        }
        ...
        {
            outputText("soft, pillowy");
        }
        else
        {
            outputText("small, perky");
        }
        ...
        {
            outputText("little round things that trace gentle curves under her loose shirt.");
        }
        //(else sheila corruption < 50)
        else if (sheilaCorruption() < 50)
        {
            outputText("healthy mounds that fill out her shirt.");
        }
        else
        {
            outputText("swollen globes that bulge over her v-neck and strain at the seams of her shirt.");
        }
        ...
        if (dick)
        {
            outputText(SMultiCockDesc() + " flops free, half-erect and swelling steadily with blood as you appraise the woman's body.");
        }
        //(vag present)
        else
        {
            outputText("Your [vagina] drools slightly, labia parting and [clit] poking up; you imagine what you want to do with it.");
        }
        ...
        if (dick)
        {
            ...
            if (player.cocks[x].cockLength <= 8)
            {
                ...
                if (player.hasBalls())
                {
                    outputText(" and your [balls] tighten up");
                }
            ...
            else
            {
                ...
                if (player.cocks[x].cockThickness <= 6)
                {
                    ...
                    if (player.hasBalls())
                    {
                        outputText(" and your [balls] begin to rise");
                    }
                ...
                else
                {
                    ...
                    if (player.hasBalls())
                    {
                        outputText(" and your [balls] draw closer to your body");
                    }
            ...
            if (player.cor >= 70 && player.statusEffectv1(StatusEffects.Infested) == 5)
            {
                ...
                if (player.hasBalls())
                {
                    outputText("packing your [balls] and driving up your lust in their zeal to multiply");
                }
                else
                {
                    outputText("deep inside your body, trailing sensation along the nerves in your most intimate places and ramping up your production");
                }
                ...
                if (player.cumQ() >= 2500)
                {
                    outputText("  The volume of semen and slimy worms you pack her with is so great that it strains her stomach; you can actually see dozens of the annelids slithering around in a tight ball under the taut skin of her swollen midriff.");
                }
            ...
            else
            {
                ...
                if (player.cumQ() >= 1000)
                {
                    outputText(", and soon you can feel it lapping at the end of your shaft");
                }
                outputText(".");
                //[(mega skeet)
                if (player.cumQ() >= 3000)
                {
                    outputText("  Your body's not done with that, though, and keeps pushing out cum until it spurts around your prick and out [sheilaname]'s nose in little squirts.");
                }
                ...
                if (player.cumQ() >= 1000)
                {
                    outputText(", though it's so wet with seed now that you can see them almost as well through it");
                }
        ...
        else
        {
            ...
            if (sheilaCorruption() >= 30)
            {
                outputText("; her breasts jiggle pleasingly as they settle on her chest, filling you with the urge to play with them");
            }
            outputText(".  Straddling her, you lower your [vagOrAss] to her face.  \"<i>Lick,</i>\" you order, rubbing your [butt] on [sheilaname]'s nose");
            //[(not horse)
            if (!player.isTaur())
            {
                outputText(" as you fondle her tits");
            }
            outputText(".");
            ...
            if ((player.hasVagina() && player.vaginalCapacity() >= 100) || (!player.hasVagina() && player.analCapacity() >= 100))
            {
                outputText("  She keeps hitting the same spot again, too - she's not even bothering to adjust her angle and touch the rest of your stretched opening.");
            }
            ...
            {
                outputText("  I know a nice boy in the mountains who keeps dogs, and I'm sure he'd love to have a womb like yours for their seed.  By this time tomorrow, you could have a cock in both holes, filling you with hellhound babies since you can't please a person.");
            }
            outputText("</i>\"");
            ...
            if (!player.isTaur())
            {
                outputText("pinch her nipple for emphasis, then grind your hips across her face, mauling her nose with your ass.  \"<i>I could fuck a tree and it wouldn't be as wooden as you in the sack");
            }
            //[(PC has found corrupt glade)
            if (SceneLib.forest.timesExploredInner() + SceneLib.forest.timesExploredOuter() >= 40)
            {
                outputText(" - in fact, there are quite a few I've seen who look like better lovers than you.  Maybe I should carry you to the forest and tie you to a nice pussy-shaped giant flower to give you lessons");
            }
            outputText(".  ");
            ...
            {
                outputText("; I personally know fifteen inches of fat, orange dick and four heavy balls that'd love to get in at your other end while I ride your face");
            }
            outputText(".</i>\"");
            ...
            if (!player.isTaur())
            {
                outputText("  Even squeezing her breasts viciously can't compel her to reinsert it, though you amuse yourself trying.");
            }
    ...
    private function sheilaGetsRapedWithADildont() : Void
    {
        ...
        {
            outputText("The kiss is wet and indecent as she shudders through her anal orgasm; her lips don't stay in one place, instead tracking drool all over your face.");
        }
        ...
        if (player.hasCock())
        {
            outputText("  \"<i>Perhaps next time you'd like to try a real cock in there?</i>\"  Even exhausted and messed up on goblin drugs, she has the presence of mind to shake her head at that.  You laugh.");
        }
    ...
    @:allow(classes.scenes.nPCs)
    private function getBeatUpBySheila() : Void
    {
        clearOutput();
        ...
        if (monster.lust < 75 && !(sceneHunter.other && player.lib >= 80))
        {
            ...
            if (player.HP < 1)
            {
                outputText("colorful spots");
            }
            else
            {
                outputText("fantasies of her, stark naked but for her hat and waiting with her legs open");
            }
    ...
    private function sheilaCowgirlRapesYou() : Void
    {
        ...
        if (player.isTaur())
        {
            outputText("; your hooves stick awkwardly out to one side");
        }
        ...
        if (player.isTaur())
        {
            outputText("pulls your legs apart");
        }
        else
        {
            outputText("unfastens and strips off your [armor]");
        }
        ...
        function nofitF() : Void
        {
            ...
            if (player.isTaur())
            {
                outputText(", or at least as flat as you can get it");
            }
            outputText(", then sits on your stomach");
            if (player.isTaur())
            {
                outputText(", sliding her long legs between yours");
            }
            ...
            if (player.isTaur())
            {
                outputText("forelegs fold over her torso affectionately");
            }
            else
            {
                outputText("hands search out her breasts, caressing and squeezing");
            }
            ...
            if (!player.isTaur())
            {
                outputText("and suck on your fingers");
            }
            ...
            {
                outputText("Her orgasm wets your prick, coating it with fluid as she comes.  [sheilaname] wraps herself tighter in your limbs, squeezing her breasts against her chest, and you think you can feel them get a little smaller as your head fuzzes over with her excess emotion.");
            }
            else
            {
                outputText("Her hips lock up when she comes, leaving you to try and finish yourself as she heats up around your shaft.");
            }
            ...
            if (player.sens >= 50)
            {
                ...
                if (player.cumQ() >= 2000)
                {
                    outputText("  The giggles and the strokes stop long before your orgasm, though; eventually [sheilaname] just lies there, dumbfounded, as you deposit layer after layer of white lattice on her skin.  \"<i>Bloody hell,</i>\" she mumbles vaguely, either stone-sober or drunk from the sight of it.");
                }
        ...
        var fitF : Void->Void = function() : Void
        {
            ...
            else
            {
                ...
                if (player.isTaur())
                {
                    outputText(", pushing your legs apart");
                }
                ...
                if (!player.isTaur())
                {
                    outputText(", and your hands go to her waist, fairly pushing her up and slamming her back down in your eagerness");
                }
                ...
                if (player.cor >= 90 || player.cor > sheilaCorruption())
                {
                    outputText("You can feel her cum-slick inner walls pulling at you as she begins to orgasm, and your mind clears as you blow your load inside her, wondering when she'll likewise return to sense.");
                }
                //(else PC corr < sheila corruption)
                else if (player.cor < sheilaCorruption())
                {
                    outputText("Her pussy gushes with her orgasm, pushing out your semen and soaking her inner lips even as she continues riding your cock, and your mind clouds as you think of little except sneaking as much of your cum inside her as possible.");
                }
                //else equal but < 90)
                else
                {
                    outputText("Even as she comes mindlessly something makes you hold in your own moans, wondering how long it will take her to notice her pussy being filled with your egg-hunting sperm.");
                }
                //[(small cum)]
                if (player.cumQ() <= 250)
                {
    ...
    private function sheilaForcedOralGeneric() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("her hand slips around your [cock]");
        }
        else if (player.hasVagina())
        {
            outputText("her fingers slip between your labia");
        }
        else
        {
            outputText("she rubs your bare crotch for a few seconds");
        }
        ...
        if (player.str < 61)
        {
            outputText("; her weight bears you slowly to the ground");
        }
        ...
        {
            outputText(" and flushing as your stingers release their payloads into her palm");
        }
        outputText(".  You obediently stick out your tongue");
        if (player.cor > 60)
        {
            outputText(", motivated more by whimsy than the meager, almost playful pain,");
        }
        ...
        if (player.cor >= 90 || player.cor > sheilaCorruption())
        {
            ...
            if (player.tongue.type > Tongue.HUMAN)
            {
                outputText("  As tired as you are, you simply go along with it, allowing her rippling walls to slowly draw your tip deeper inside until it bumps against her cervix; she shivers and kisses the top of your head as you groggily run your tongue over the entrance to her womb.");
            }
            else
            {
                outputText("  However, it doesn't get very far; you lazily flop it around inside her as she works through her arousal.");
            }
        ...
        else if (player.cor < sheilaCorruption())
        {
            ...
            if (player.hasCock())
            {
                outputText("face-fucking her and blowing a load into her mouth, then holding her jaw shut until she swallows it unhappily.");
            }
            else if (player.hasVagina())
            {
                outputText("smearing your gash on her face until you come and drown her in your squirting girl-jizz.");
            }
            else
            {
                outputText("forcing her to tongue your asshole until you come, writhing and pressing her head into the grass with your [butt].");
            }
        }
        else
        {
            outputText("and she pets your head affectionately as her walls squeeze your tongue.  \"<i>Oh, mate...</i>\" she whispers lovingly, completely forgetting the struggle that led to your tryst.");
        }
        if (player.tongue.type > Tongue.HUMAN)
        {
            fatigue(15);
        ...
        var normalEnd : Void->Void = function() : Void
        {
            ...
            if (player.str < 61)
            {
                outputText("climbing off and kneeling next to you.");
            }
            else
            {
                outputText("letting herself down your body until she can put her long legs on the ground once more.");
            }
    ...
    private function normalSheilaPregNotifNumberOne() : Void
    {
        ...
        if (player.cor < 50)
        {
            outputText("  A hand placed around her shoulder seems to steady her a bit.");
        }
    ...
    private function normalSheilaPregNotifREPEATEDED() : Void
    {
        ...
        if (rand(10) < 7)
        {
            outputText("healthy.  It's eating a lot... frankly, it's a little hellraiser with a strong grip, and it cries well, too.</i>\"");
        }
        else
        {
            outputText("weak... we thought we were going to lose it for a bit.  It wouldn't drink from the nurse unless I was there, so I had to stay up with it all night myself.  Feeling a bit stuffed.</i>\"");
        }
        ...
        if (player.cor < 40)
        {
            outputText(", and you give it a squeeze");
        }
        ...
        {
            flags[kFLAGS.SHEILA_CORRUPTION] = 80;
        }
        menu();
        //[Help(requires >80 speed, centaur >= 5', naga, or big wings and archery)][Walk With Her][Let Her Go]
        addButton(0, "Help", normalSheilaPregNotifREPEATEDEDHelpABitchOut).disableIf(!(player.spe > 80 && player.hasPerk(PerkLib.Runner) || player.isTaur() || player.isNaga() || player.canFly && player.isBowTypeWeapon()), 
    ...
    private function normalSheilaPregNotifREPEATEDEDLetHerGo() : Void
    {
        ...
        if (player.cor < 40)
        {
            outputText(", imploring her to eat the rest and keep up her strength");
        }
    ...
    private function normalSheilaPregNotifREPEATEDEDHelpABitchOut() : Void
    {
        ...
        {
            //less QQ, more pew pewif (player.canFly() && player.isBowTypeWeapon())
            {
            ...
            else if (player.spe >= 80 && player.hasPerk(PerkLib.Runner))
            {
                ...
                if (player.lib < 50)
                {
                    outputText("you pinch her ear, provoking an annoyed flick from it.");
                }
                else
                {
                    outputText("you slap her on the ass, grabbing a handful and taking her mind off her own misery.");
                }
    ...
    private function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDWELP() : Void
    {
        ...
        if (player.cor < 50)
        {
            outputText("  You set a hand on [sheilaname]'s shoulder to steady her a bit as the tears start to fall.");
        }
    ...
    private function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE() : Void
    {
        ...
        if (player.isGoo())
        {
            ...
            if (player.lib >= 40)
            {
                outputText("  The leer you display is so prurient that she seems on the verge of reconsidering, but you compose yourself and");
            }
            else
            {
                outputText("  You");
        ...
        else if (player.isNaga())
        {
            ...
            if (player.lib >= 40)
            {
                outputText(" - but tighten the one sliding across her breasts just slightly.  [sheilaname]'s neck turns red and her ears twitch as you rub her nipples, and you plant a kiss on her nape");
            }
        ...
        else if (player.isTaur() && player.tallness >= 60)
        {
            ...
            if (player.sens >= 40)
            {
                outputText("... or would be, if you weren't distracted by the nipples poking into you and the thighs squeezing your flank");
            }
            outputText(".");
            //(apply sens-based lust damage if sens >=40)
            if (player.sens >= 40)
            {
                dynStats("lus", player.sens / 4, "scale", false);
            }
        }
        ...
        if (!player.isTaur())
        {
            outputText("  She grudgingly repositions, allowing you to rest against her.");
        }
        explorer.stopExploring();
        model.time.days++;
        model.time.hours = 2;
    ...
    private function normalSheilaPregNotifREPEATEDEDHelpABitchOutANDSTAYDERE2() : Void
    {
        ...
        if (player.isGoo())
        {
            outputText("; she's already dressed and appears to have just been outside");
        }
        outputText(".");
        ...
        if (player.lust < 30)
        {
            outputText("  You give her a hand, then help her load up.  She gives you a peck on the cheek before she departs.  \"<i>Thanks, love.</i>\"");
        }
        if (player.lust >= 30)
        {
    ...
    private function normalSheilaPregNotifREPEATEDEDHelpABitchOutTOCAMP() : Void
    {
        ...
        if (player.inte >= 60)
        {
            outputText("\"<i>The baby will be fine,</i>\" you venture, hazarding a guess.  \"<i>It's what the nurses are there for.</i>\"");
        }
        else
        {
            outputText("You slip an arm around her, and ask if she's okay.");
        }
        outputText("  She nods mutely and pulls the food out of the fire.");
        //[(if Jojo present)
        if (player.hasStatusEffect(StatusEffects.JojoNightWatch) || (player.hasStatusEffect(StatusEffects.PureCampJojo) && flags[kFLAGS.JOJO_BIMBO_STATE] != 3))
        {
            outputText("  She extends one bird to you and another to Jojo.  He raises his hand and shakes his head, but politely.");
        }
        else if (flags[kFLAGS.ANEMONE_WATCH] > 0)
        {
            outputText("  [sheilaname] offers you one of the birds, then extends another to your odd tenant.  The anemone's mouth tightens and she shakes her head vigorously, blushing a deep blue.");
        }
        ...
        //[(if no watch)
        if (!player.hasStatusEffect(StatusEffects.JojoNightWatch) || !player.hasStatusEffect(StatusEffects.PureCampJojo) || flags[kFLAGS.ANEMONE_WATCH] == 0)
        {
    ...
    private function sheilaGoesDemon() : Void
    {
        ...
        if (flags[kFLAGS.SHEILA_XP] < -2)
        {
            outputText("always so bloody eager to fuck me!");
        }
        else
        {
            outputText("me best mate!");
        }
        ...
        if (player.tongue.type > Tongue.HUMAN)
        {
            outputText("extending your oral muscle until it's all the way to the back of her pussy.  ");
        }
        else
        {
            outputText("but only manage to scrape her entrance; she shudders and seems satisfied with that, judging by what she says next.  ");
        }
    ...
    private function demonSheilaEncounter() : Void
    {
        ...
        if (player.lust >= 33)
        {
            addButton(1, "LetHerFuck", sheilaLetHerFuckYou);
        }
    ...
    private function demonSheilaResist() : Void
    {
        ...
        if (sheilaCorruption() < 30)
        {
            outputText(" thrusts her black spade forward and smears some of the constantly-oozing fluid from its tip down the length with her hand");
        }
        else
        {
            outputText(" kneads her breasts seductively, pinching her nipples");
        }
        outputText(".  Your posture softens a bit as she reaches out to you");
        if (sheilaCorruption() < 30)
        {
            outputText(", tilting her palm to dribble a handful of strong-scented dark oils");
        }
    ...
    private function demonSheilaTalk() : Void
    {
        ...
        if (!player.isTaur())
        {
            outputText("takes your arm in hers, then slips the other hand into your armor");
        }
        //[(horse)
        else
        {
        ...
        {
            outputText("smooth skin");
        }
        //[(lib <20)
        if (player.lib < 20)
        {
            outputText(", giggling as your expression petrifies");
        }
    ...
    private function askDemonSheilaAboutDahChange() : Void
    {
        ...
        if (flags[kFLAGS.SHEILA_CITE] >= 1)
        {
            ...
            if (player.hasCock())
            {
                outputText(", and you feel a monument of your own being erected");
            }
    ...
    private function askDemonSheilaAboutHerColony() : Void
    {
        ...
        if (player.cor >= 75)
        {
            outputText("harvested that day, you sigh wistfully");
        }
        else
        {
            outputText("stolen that day, you wince");
        }
        ...
        if (player.hasCock())
        {
            outputText("she ties you down and rides you until she's so full of your spunk that she can push it back out just by pressing on her stomach");
        }
        else
        {
            outputText("she binds your hands and sits on you, teasing your helpless body with her oozing, throbbing spade until she comes all over you, spraying dark slimy fluid from it");
        }
        outputText(".  You ");
        if (player.cor < 50)
        {
            outputText("break eye contact, blushing");
        }
        else
        {
            outputText("stare into her shining eyes, getting hotter and hotter as the vision plays out to the very end");
        }
        outputText(".");
    ...
    private function talkToDemonSheilaAboutWhatNow() : Void
    {
        ...
        if (player.cor < 50)
        {
            ...
            {
                outputText("the dick I grow for you");
            }
        ...
        if (player.hasCock())
        {
            outputText("give them to me");
        }
        else
        {
            outputText("have them for me");
        }
        ...
        if (flags[kFLAGS.AMILY_OFFER_ACCEPTED] > 0)
        {
            outputText(" save one.  Actually, the two are a bit reminiscent of each other..");
        }
        ...
        if (player.cor < 50)
        {
            outputText("; even though you hide it, I've seen it lurking");
        }
    ...
    private function demonSheilaTalkAnotherQuestion() : Void
    {
        ...
        if (player.lust < player.maxOverLust())
        {
            ...
            if (player.hasCock())
            {
                outputText("slipping more of her finger and thumb around your penis");
            }
            else if (player.hasVagina())
            {
                outputText("parting your lips more");
            }
            else
            {
                outputText("rubbing your bare groin");
            }
    ...
    private function demonImpChildren() : Void
    {
        ...
        if (sheilaCorruption() < 40)
        {
            outputText("her perky nipples rubbing into your arm as she snuggles up");
        }
        else
        {
            outputText("her enormous, pendulous breasts practically parting around you to wrap you up alongside her arms");
        }
        outputText(".");
        ...
        if (!player.hasCock())
        {
            outputText("  Or, had...</i>\" she finishes with a frown as she reaches your bare groin.  \"<i>Where'd you leave that magnificent old fella?  I was lookin' forward to another go.</i>\"  She pulls her hand out, then leans toward you with a pout.  \"<i>I'm so horny, too...</i>\"");
        }
        //(if cock)
        else
        {
            ...
            if (sheilaCorruption() <= 80)
            {
                outputText("with her fingers to make sure the invitation is all the way open");
            }
            //(else)
            else
            {
                outputText("with her split tail while she cradles her massive tits to make sure you see the open invitation");
            }
    ...
    private function pregDemonSheilaOtherSex() : Void
    {
        ...
        if (player.hasCock())
        {
            ...
            if (sheilaCorruption() >= 80)
            {
                outputText("heaves onto her feet, heavy breasts swaying");
            }
            else
            {
                outputText("nimbly hops to her clawed feet");
            }
    ...
    @:allow(classes.scenes.nPCs)
    private function loseToSheila(consensual : Bool = false) : Void
    {
        clearOutput();
    ...
    private function loseToNormalSheilaAndGetRidden() : Void
    {
        ...
        if (player.isTaur())
        {
            outputText(" and you're forced to put your forelegs down for support as you come to rest on your haunches");
        }
        else
        {
            outputText(", spreading your legs.  The demon unfastens your [armor] and strips it from your bottoms easily");
        }
        ...
        if (player.cockTotal() > 1)
        {
            outputText("\"<i>All of these look ripper, but... I want this one today,</i>\" she says, selecting your [cockFit 56].  ");
        }
        ...
        if (player.isTaur())
        {
            outputText(", draping your forelegs over her shoulders,");
        }
        outputText(" presses her " + sheilaCup() + " breasts");
        if (sheilaCorruption() < 100)
        {
            outputText(" against you, dragging her nipples against yours.  ");
        }
        else
        {
            outputText(" against you so forcefully that they part around your chest.  ");
        }
        ...
        if (player.hasCock())
        {
            outputText(" and slipping the unused remainder of your manhood between her thighs");
        }
        ...
        if (player.hasBalls())
        {
            ...
            if (!player.isTaur())
            {
                outputText("  [sheilaname] takes your face in her hands and presses it into her breasts as she reassures you.");
            }
            ...
            if (player.cumQ() < 250)
            {
                outputText("ten minutes and two");
            }
            //(big skeet)
            else if (player.cumQ() < 1000)
            {
                outputText("thirty minutes and four");
            }
            else
            {
                outputText("fifty minutes and eight");
            }
        ...
        else
        {
            ...
            if (player.isTaur())
            {
                outputText("You force her downward with your forelegs to");
            }
            else
            {
                outputText("You grab her hips and");
            }
            ...
            if (player.cumQ() >= 1000)
            {
                outputText("; she quickly reaches capacity and pulls off, jerking your messy, slimy [cockFit 56] and letting you squirt the last of your strokes all over her face and chest");
            }
            if (player.cumQ() >= 6000)
            {
                outputText(".  So much is left that she's practically underwater by the time you're finished; she giggles and bubbles form in the oozing liquid covering her lips");
            }
            outputText(".");
    ...
    private function tailSpadeSoundingForFuckHugeDongsWithDemonSheila() : Void
    {
        ...
        if (player.cockTotal() > 1)
        {
            outputText("; unguided, the ");
        }
        if (player.cockTotal() == 2)
        {
            outputText("other twitches and sprays");
        }
        else if (player.cockTotal() > 2)
        {
            outputText("others twitch and spray");
        }
        if (player.cockTotal() > 1)
        {
            outputText(" all over, coating her back, you, the grass, and anything ");
        }
        if (player.cockTotal() > 2)
        {
            outputText("else they happen to point at");
        }
        if (player.cockTotal() == 2)
        {
            outputText("else it happens to point at");
        }
    ...
    private function clitSwellingDemonSheilaClitSoundingAnal() : Void
    {
        ...
        if (!player.isTaur())
        {
            outputText("[sheilaname] places her hand in the small of your back and leans in, drawing closer to your face even as she pulls your waist inward.");
        }
        else
        {
            outputText("[sheilaname] sets her hand atop your withers, pressing down gently as she looks into your eyes.");
        }
        ...
        if (!player.isTaur())
        {
            outputText("turns around, then straddles your groin and looks over her shoulder.");
        }
        //(Equus ferus caballus)
        else
        {
            outputText("circles you until she reaches your rump, then turns around and kneels down herself, looking at you over her shoulder.");
        }
        ...
        if (player.hasCock())
        {
            ...
            {
                outputText("the underside of your [cock biggest], before");
            }
        }
        ...
        {
            outputText("lewd little");
        }
        else
        {
            outputText("grotesque");
        }
        ...
        if (player.clitLength > 6)
        {
            outputText(", taking it several inches deep");
        }
        ...
        {
            outputText("Though you're by now accustomed to the sight of your clitoris, more like a cock than a dainty feminine bump, this is something else altogether!  The huge thing poking into her tail is almost twice the thickness of the slim growth you remember and bulges with blood vessels; as you watch it, transfixed, a trickle of [sheilaname]'s oily, dark secretion slides down it, then is absorbed into some flesh near your groin, adding another layer of width with a pulse!");
        }
        ...
        if (player.clitLength <= 6)
        {
            ...
            if (!player.isTaur())
            {
                outputText("Taking your face into her hand, she closes your jaw and then leans in to plant a kiss on your chin.");
            }
            else
            {
                outputText("Her eyes glitter with mirth.");
            }
            ...
            if (player.isTaur())
            {
                outputText(" through the grass");
            }
            else if (player.biggestTitSize() >= 1)
            {
                outputText(" between your " + allBreastsDescript());
            }
            else
            {
                outputText(" along your flat chest");
            }
        ...
        else
        {
            ...
            if (player.isTaur())
            {
                outputText(" and rolls onto her back");
            }
            ...
            {
                outputText("all of it is in her bowels and her hot cunt rests against yours");
            }
            else
            {
                outputText("she's taken as much as she can");
            }
            ...
            if (player.clitLength <= 12)
            {
                outputText("Her hips shift, dragging her lips across yours and stirring your clit through her asshole even as it squeezes you; your pussy twitches at the stimulation.");
            }
            else
            {
                ...
                {
                    outputText("  \"<i>What's this?</i>\" [sheilaname] teases.  \"<i>Why, your virtue is still intact, little girl!  I wouldn't want anything to happen to it!</i>\"  She pulls her tails out a bit, contenting herself with simply teasing your lips and driving you mad for penetration.");
                }
            ...
            if (!player.isNaga())
            {
                outputText("your thighs, spreading your [legs] wide and then hooking them over top to touch ");
            }
            ...
            if (player.clitLength > 12)
            {
                outputText("pulls her black tail free, dripping with your juice, then ");
            }
            ...
            if (player.clitLength <= 12)
            {
                outputText("pussy; her smooth, small labia greedily lap at the fluid when they touch, brushing against yours and drawing further twitches and squirts.");
            }
            else
            {
                outputText("teasing tails; they shift from stroking your labia to holding them open, allowing your pussy to squirt as it pleases.");
            }
            ...
            if (player.clitLength <= 12)
            {
                ...
                if (player.hasVirginVagina())
                {
                    outputText("  <b>You've lost your virginity!</b>");
                }
            ...
            if (!player.isTaur())
            {
                outputText("her tits are covered by grass stains from her frenetic pumping.");
            }
            else
            {
                outputText("craning around shows her that her asscheeks are completely green and wet with torn grass from slamming into you so violently and repeatedly.");
            }
    ...
    @:allow(classes.scenes.nPCs)
    private function beatUpDemonSheila(output : Bool = true) : Void
    {
        if (output)
        {
            clearOutput();
            ...
            if (monster.HP <= monster.minHP())
            {
                outputText("and her wounds begin to close with a soft lilac glow.  \"<i>So fuckin' strong.  I don't suppose you've got any sperm for me, do you?  I'd love to have some of your kids...</i>\"  She looks at you beseechingly, gently fingering herself.");
            }
            //(lust)
            else
            {
                outputText("frigging herself vigorously.  \"<i>Do with me what you will, lover; those were the terms.  Won't you take my body to satisfy yourself, you brute?</i>\"  The audacious woman sticks her tongue out, as if to goad you into treating her roughly.");
            }
        }
        //[(lust < 30)]
        ...
        menu();
        addButton(0, "Missionary", missionaryForThePurposesOfCreatingImpsWithSheila).disableIf(player.cockThatFits(56) < 0, "Req. a cock fitting 56 area.");
        addButton(1, "AnalHateFuck", analHateFuck).disableIf(player.cockThatFits(56) < 0, "Req. a cock fitting 56 area.");
        addButton(2, "UseHerThighs", bigDickAndThighs).disableIf(player.findCock(1, 56, -1) < 0, "Req. a cock with area larger than 56.");
        addButton(3, "Clit-Fuck", winAgainstDemoNSheilaForVaginas).disableIf(!player.hasVagina(), "Req. a vagina.");
    }
    ...
    private function missionaryForThePurposesOfCreatingImpsWithSheila() : Void
    {
        ...
        if (player.cor < 40)
        {
            outputText("Your ardor subsides a bit at her performance, but seeing your look of contrition, ");
        }
        else
        {
            outputText("Your scowl hardens at her shameless playacting, and ");
        }
        ...
        if (sheilaCorruption() >= 40)
        {
            outputText("and her soft breasts jiggle ");
        }
        ...
        if (silly())
        {
            outputText("  \"<i>Make me feel like I'm fifteen years old again and playin' with the dog-morphs in the field!</i>\"  Heavens to Betsy!");
        }
        else
        {
            outputText("  \"<i>This is what you're here for!  Don't spill a single drop!</i>\"");
        }
        ...
        if (player.isTaur())
        {
            outputText(" as you slump forward");
        }
        ...
        {
            outputText("swells and squirts");
        }
        outputText(", pouring your semen into [sheilaname]'s ravenous pussy");
        if (player.cockTotal() > 1)
        {
            outputText(" and coating her crotch with a glaze of white");
        }
        outputText("; her eyes glow and it briefly feels as though her body is sucking the ejaculate right out of you");
        if (player.cumQ() >= 500)
        {
            outputText(", until she fills up and the rest of your orgasm is drooled out at its own lazy pace");
        }
        outputText(".  She kisses you");
        if (player.isTaur())
        {
            outputText("r forelegs");
        }
        outputText(" lovingly and deeply");
        if (player.cockTotal() > 1 || player.cumQ() >= 500)
        {
            outputText(" as the spare seed dribbles from her crotch");
        }
        outputText(", then breaks away and looks up into your face.");
        
        outputText("\n\n\"<i>[name], you ");
        if (player.cumQ() >= 500 || player.cockTotal() > 1)
        {
            outputText("spilled some.  I can't have your baby if you're fucking the grass.  And you ");
        }
        ...
        else
        {
            ...
            if (player.hasVagina())
            {
                outputText(" as her imp spawn take turns doing the same to you from behind");
            }
            else
            {
                outputText(" as her imp spawn gather around, stroking themselves and occasionally squirting their hot, tainted cum on her body or yours as orgasm takes them");
            }
            outputText(".  ");
            if (player.lib < 40)
            {
            ...
            if (!player.isTaur())
            {
                outputText("You kiss [sheilaname] on the lips, pushing your tongue through to play with hers, as your arms push your spent body into position to thrust again.");
            }
            else
            {
                outputText("You reach down with a hand and place it alongside [sheilaname]'s face; her ear twitches as she looks at you brightly, and for a moment you recapture your earlier impression of her as a sweet, devoted lover.  Your legs struggle to push your equine bulk upwards as you return to a thrusting position, dragging her groin into the air by your erect cock.");
            }
            ...
            if (player.cumQ() >= 500)
            {
                outputText("; you nearly manage to fit it all into her this time, with your production still flagging from the last session");
            }
            ...
            {
                dynStats("cor", 10);
                if (flags[kFLAGS.SHEILA_CORRUPTION] > 100)
                {
                    sheilaCorruption(-10);
                }
            }
    ...
    private function analHateFuck() : Void
    {
        ...
        {
            addButton(1, "Call Jojo", analHateFucksWithJojo);
        }
        else
        {
            sceneHunter.print("It's a shame you don't have the corrupted mouse-monk in your camp...");
        }
    }
    ...
    private function sheilaAnalHateFuckAGoGo() : Void
    {
        ...
        if (!player.isTaur())
        {
            outputText("Slipping one hand around your [cockFit 32]");
        }
        else
        {
            outputText("Straddling her");
        }
        ...
        if (!player.isTaur())
        {
            outputText("grab her thrashing legs, using them for leverage as you ");
        }
        ...
        if (sheilaCorruption() >= 90)
        {
            outputText("; her enormous, cushiony breasts spread under her as your prick forces her weight heavily onto them");
        }
    ...
    private function sheilaAnalHateFuckAGoGoNO() : Void
    {
        ...
        if (player.cumQ() >= 500)
        {
            outputText("  Your overproduction floods her insides, and [sheilaname] groans as it begins to leak back out, drooling over her pussy.  \"<i>Hey, you got your wish,</i>\" you tease.  The wanton woman tries to tilt her ass higher, to tip more of your overflow into her cunt, but you laugh and slap her hard on the hip, sending her to one knee again.");
        }
    ...
    private function analHateFucksWithJojo() : Void
    {
        ...
        if (sheilaCorruption() < 40)
        {
            outputText("\"<i>What's with these tiny titties?  Could any man possibly be aroused by your bug bites?  I've seen bigger breasts on incubi!</i>\"");
        }
        else
        {
            outputText("\"<i>These disgusting sacks of fat on your chest... are you under the impression that men like giant stretch marks and nipples that hang to your knees?  Why don't you get in shape, you messy bitch?</i>\"");
        }
    ...
    private function analHateFucksWithJojoNo(clear : Bool) : Void
    {
        if (clear)
        {
            clearOutput();
    ...
    private function jojoRuinsTheAnalHateFuck(clear : Bool = true) : Void
    {
        if (clear)
        {
            clearOutput();
        ...
        if (sheilaCorruption() >= 90)
        {
            outputText(", mashing her tits together as they thicken");
        }
        ...
        if (sheilaCorruption() >= 40)
        {
            outputText("Her breasts tighten up, shedding cup sizes rapidly; t");
        }
        else
        {
            outputText("T");
    ...
    private function bigDickAndThighs() : Void
    {
        ...
        if (!player.isTaur())
        {
            ...
            if (player.cockTotal() > 2)
            {
                outputText(" one at a time, practically stacking them atop each other");
            }
            outputText(".");
        }
        //(horse)
        ...
        function noF() : Void
        {
            ...
            if (!player.isTaur() && player.tallness >= 60)
            {
                outputText("She unbends and kisses you on the cheek, and your face feels a bit fuzzy where her lips touch.  ");
            }
            ...
            if (player.cockTotal() == 1)
            {
                outputText("your dick with both");
            }
            else
            {
                outputText("a prick in each");
            }
            outputText(" and angling upward so the precum drools downward and lubes her fingers.");
            sharedEnd();
        }  //(else corr >= 40)  ;
        
        var yesF : Void->Void = function() : Void
        {
            ...
            if (player.cockTotal() > 1)
            {
                ...
                if (player.cockTotal() == 2)
                {
                    outputText(", you slide it through her legs between your first cock and her cunt, plowing through her entrance like a furrow; she quakes and you can feel it contracting against your skin.  ");
                }
                else
                {
                    outputText("s, you push them through the woman's legs one by one, smearing warm precum on her thighs as you do; her pussy squeezes against the top of your [cock biggest] as you coat her skin with your fluids.  ");
                }
            }
            else
            {
        ...
        var sharedEnd : Void->Void = function() : Void
        {
            ...
            if (player.cockTotal() > 2)
            {
                outputText("relinquish your other pricks and ");
            }
            if (player.cockTotal() == 2)
            {
                outputText("relinquish your other prick and ");
            }
            ...
            if (sheilaCorruption() < 50)
            {
                outputText("  Though her modest breasts can't even begin to encompass your girth, [sheilaname] rubs her cleavage along your skin with gusto while she works your tip with her tongue.");
            }
            else if (sheilaCorruption() < 150)
            {
                outputText("  Her large " + sheilaCup() + " breasts encompass your girth easily, and she pushes the soft, flushed skin together as she slides it up and down your cock.");
            }
            else
            {
                ...
                if (player.cockTotal() > 1)
                {
                    outputText(" built for " + num2Text(player.cockTotal()) + " pricks");
                }
                outputText(".");
            }
            ...
            if (!player.hasVagina())
            {
                outputText("to grow a nice pussy for me so that ");
            }
    ...
    private function winAgainstDemoNSheilaForVaginas() : Void
    {
        ...
        if (!player.isTaur())
        {
            outputText("mauling her tits roughly with your hands, ");
        }
        outputText("and grind your pussy desperately against her midriff.  She slips two fingers into you obligingly, but you ");
        if (!player.isTaur())
        {
            outputText("grab her face, plunging one thumb into the corner of her mouth in your haste, and");
        }
        else
        {
            outputText("turn your head and");
        }
        ...
        if (!player.isTaur())
        {
            ...
            //[(tail or naga)
            if (player.tailType > Tail.NONE && player.isNaga())
            {
                outputText(" and raise your tail");
            }
        }
        else
        {
            outputText("clamber to your hooves and bend your forelegs");
        }
        ...
        {
            outputText(", squashing her tits into you");
        }
        ...
        if (player.lib < 20)
        {
            outputText("even as proud as you are, ");
        }
        ...
        if (player.cor < 50)
        {
            ...
            if (sheilaCorruption() > 0)
            {
                outputText("; her tits wobble violently as she thrusts, and then still as she arches her back and jams her spade all the way in");
            }
    ...
    public function breakScarredBlade() : Void
    {
        ...
        if (player.str >= 85)
        {
            outputText("You grab the saber by the blade and bend it with all your might. The metal groans and the lethicite cracks. You repeatedly twist the blade until one segment of the saber finally snaps off.");
        }
        else
        {
            ...
            {
                outputText("You call Kiha over. \"<i>Yes?</i>\"");
            }
            else if (marbleFollower())
            {
                outputText("You call Marble over. \"<i>What is it you need, sweetie?</i>\" she asks. You ask her if she can break the scarred blade. \"<i>Yes, sweetie. I'll get my hammer.</i>\"");
            }
    ...
    public function rebellingScarredBlade(wieldAttempt : Bool = false) : Void
    {
        if (!wieldAttempt)
        {
            outputText("<b>The scratched sword you carry jerks wildly like a bucking horse, and, tilting hilt-downward, slides itself right out of its scabbard.  Before you can pick it up and re-sheathe it, it lashes out at your hand, cutting you and landing with the point out.  Even when you try to circle it and grab the handle, the uncanny saber spins its edge around to fend you off.  Sighing with irritation, you abandon it for now.</b>\n\n");
        }
        else
        {
            outputText("As soon as you try to wield the sword, it jerks wildly like a bucking horse. You quickly put it back into your pouches before it can do harm to you.");
        }
        if (!wieldAttempt)
        {
            var dmg : Int = 20;
