...
class Ember extends Monster
{
    ...
    //Basic attack, average damage, average accuracy
    {
        ...
        {
            outputText("You dodge aside at the last second and Ember's claws whistle past you.");
        }
        else
        {
            var damage : Float = 0;
            ...
            {
                damage += ((str + weaponAttack) - rand(player.tou) - player.armorDef);
            }
            if (flags[kFLAGS.EMBER_LVL_UP] >= 1)
            {
                damage += (1 + (flags[kFLAGS.EMBER_LVL_UP] * 0.1));
            }
            if (flags[kFLAGS.EMBER_LVL_UP] >= 8)
            {
                damage += (1 + (flags[kFLAGS.EMBER_LVL_UP] * 0.2));
            }
            ...
            if (damage <= 0)
            {
                outputText("Ember's claws scrape noisily but harmlessly off your [armor].");
            }
            else
            {
    ...
    private function embersSupahSpecialDragonBreath() : Void
    {
        ...
        else
        {
            ...
            {
                outputText("  It's a narrow thing, but you manage to throw yourself aside at the last moment.  Fortunately, the energy whirling around and tearing up the soil blinds Ember to your escape until you have recovered and are ready to keep fighting.");
            }
            else
            {
                var damage2 : Float = 0;
                ...
                if (player.hasPerk(PerkLib.FromTheFrozenWaste) || player.hasPerk(PerkLib.ColdAffinity) || player.hasPerk(PerkLib.FireAffinity) || player.hasPerk(PerkLib.FireShadowAffinity) || player.hasPerk(PerkLib.AffinityIgnis))
                {
                    damage2 *= 1.6;
                }
                if (flags[kFLAGS.EMBER_LVL_UP] >= 1)
                {
                    damage2 *= (1 + (flags[kFLAGS.EMBER_LVL_UP] * 0.1));
                }
                if (flags[kFLAGS.EMBER_LVL_UP] >= 8)
                {
                    damage2 *= (1 + (flags[kFLAGS.EMBER_LVL_UP] * 0.2));
                }
                if (flags[kFLAGS.EMBER_LVL_UP] >= 13)
                {
                    damage2 *= (1 + (flags[kFLAGS.EMBER_LVL_UP] * 0.2));
    ...
    //Blind dodge change
    {
        ...
        if (player.getEvasionRoll() || rand(2) == 0)
        {
            ...
            if (rand(2) == 0)
            {
                outputText("duck under");
            }
            else
            {
                outputText("leap over");
            }
        ...
        else
        {
            ...
            if (flags[kFLAGS.EMBER_LVL_UP] >= 1)
            {
                damage3 *= (1 + (flags[kFLAGS.EMBER_LVL_UP] * 0.1));
            }
            if (flags[kFLAGS.EMBER_LVL_UP] >= 8)
            {
                damage3 *= (1 + (flags[kFLAGS.EMBER_LVL_UP] * 0.2));
            }
            if (flags[kFLAGS.EMBER_LVL_UP] >= 13)
            {
                damage3 *= (1 + (flags[kFLAGS.EMBER_LVL_UP] * 0.2));
    ...
    //Effect: Stuns the PC for one turn and deals some damage, not much though. (Note: PC's version of this does something different and Ember has no cooldown to use this again. Obviously do not spam or peeps will rage.)
    {
        ...
        else
        {
            ...
            var damage4 : Float = 10 + rand(10);
            if (flags[kFLAGS.EMBER_LVL_UP] >= 1)
            {
                damage4 *= (1 + (flags[kFLAGS.EMBER_LVL_UP] * 0.1));
            }
            if (flags[kFLAGS.EMBER_LVL_UP] >= 8)
            {
                damage4 *= (1 + (flags[kFLAGS.EMBER_LVL_UP] * 0.2));
            }
            if (flags[kFLAGS.EMBER_LVL_UP] >= 13)
            {
                damage4 *= (1 + (flags[kFLAGS.EMBER_LVL_UP] * 0.2));
