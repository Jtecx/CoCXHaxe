...
class TifaFollower extends NPCAwareContent
{
    
    public function tifaAffection(changes : Float = 0) : Float
    {
        flags[kFLAGS.TIFA_AFFECTION] += changes;
        if (flags[kFLAGS.TIFA_AFFECTION] > 100)
        {
            flags[kFLAGS.TIFA_AFFECTION] = 100;
        }
    ...
    public function tifaMainMenuAppearance() : Void
    {
        ...
        {
            outputText("Currently, she is laying down on a small bed of resin the workers made for her, she does not get to move much anymore");
        }
        else
        {
        ...
        if (player.hasPerk(PerkLib.TransformationImmunityBeeHandmaiden))
        {
            outputText("She’s got one primary pussy on the front and many other slits lining the sides of her abdomen which her drone consorts proceed to regularly fill as to keep the egg production going");
        }
        else
        {
    ...
    public function tifaMainMenuTalk() : Void
    {
        ...
        if (player.gender > 1 && flags[kFLAGS.TIFA_AFFECTION] >= 100 && flags[kFLAGS.TIFA_FOLLOWER] == 9 && player.isRace(Races.BEE, 1, false))
        {
            addButton(2, "Handmaiden", tifaMainMenuTalkRelations).hint("Become her handmaiden.");
        }
        else if (flags[kFLAGS.TIFA_FOLLOWER] > 9)
        {
            addButtonDisabled(3, "Handmaiden", "You're already her handmaiden.");
        }
        else
        {
            addButtonDisabled(3, "Handmaiden", "Become her handmaiden. (Req. female or herm bee morph and discussed all conversation at least once, also have an affection rating with Tifa of 100)");
        }
    ...
    public function tifaMainMenuTalkReturn() : Void
    {
        ...
        if (player.gender > 1 && flags[kFLAGS.TIFA_AFFECTION] >= 100 && flags[kFLAGS.TIFA_FOLLOWER] == 9 && player.isRace(Races.BEE, 1, false))
        {
            addButton(2, "Handmaiden", tifaMainMenuTalkBecomeHerHandmaiden).hint("Become her handmaiden.");
        }
        else if (flags[kFLAGS.TIFA_FOLLOWER] > 9)
        {
            addButtonDisabled(3, "Handmaiden", "You're already her handmaiden.");
    ...
    public function tifaMainMenuTalkHer() : Void
    {
        ...
        if (flags[kFLAGS.TIFA_FOLLOWER] == 6)
        {
    ...
    public function tifaMainMenuTalkBeeLife() : Void
    {
        ...
        if (flags[kFLAGS.TIFA_FOLLOWER] == 7)
        {
            flags[kFLAGS.TIFA_FOLLOWER]++;
        }
        tifaAffection(10);
    ...
    public function tifaMainMenuTalkRelations() : Void
    {
        ...
        if (flags[kFLAGS.TIFA_FOLLOWER] == 8)
        {
            flags[kFLAGS.TIFA_FOLLOWER]++;
        }
        tifaAffection(10);
    ...
    public function tifaMainMenuTalkBecomeHerHandmaidenYes() : Void
    {
        ...
        {
            player.createPerk(PerkLib.BeeOvipositor, player.maxEggs, 0, 0, 0);
        }
        else
        {
            player.setPerkValue(PerkLib.BeeOvipositor, 1, player.maxEggs);
        }
    ...
    public function tifaMainMenuSex() : Void
    {
        ...
        menu();
        if (player.hasVagina() && player.biggestTitSize() > 0)
        {
            addButton(0, "Repeat after me", tifaMainMenuSexRepeatAfterMe);
        }
        else
        {
            addButtonDisabled(0, "Repeat after me", "Req. breast and pussy.");
        }
        if (player.gender > 0)
        {
        ...
        addButton(3, "Sex with the queen", tifaMainMenuSexWithTheQueen).disableIf(!player.hasPerk(PerkLib.TransformationImmunityBeeHandmaiden), "Req. <b>bee</b>ing a little more like bees.");
        addButton(4, "Back", tifaMainMenu);
    }
    ...
    public function tifaMainMenuSexSixty9() : Void
    {
        ...
        if (player.gender == 1)
        {
            outputText("Tifa is swift to grab your juicy member, sliding its length in her mouth. You can feel her long bee tongue wrapping around your cock inside and this makes you all the more happy to lick her cunt.\n\n");
        }
        else if (player.gender == 2)
        {
            outputText("Tifa is swift to also get to the job of her tongue going inside your pussy like a small tentacle. " + ((player.clitLength >= 3) ? "She swiftly finds your clitoris and wraps her appendage around it, jerking it off from the inside of your pussy just like a cock. " : "") + "You moan in delight your own juices starting to flow out just as well as hers.\n\n");
        }
        else
        {
            outputText("Tifa is swift to grab your juicy member, sliding its length in her mouth. You can feel her long bee tongue wrapping around your cock inside and this makes you all the more happy to lick her cunt. Your own pussy isn’t left hanging either as she takes it upon herself to jam a few fingers inside.\n\n");
        }
    ...
    public function tifaMainMenuSexBreastPlay() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText("your cunt divulging its lusty payload all over Tifa’s abdomen" + ((player.vaginas[0].vaginalWetness >= VaginaClass.WETNESS_SLAVERING) ? " your cunt squirting its lusty payload all over Tifa’s chest" : "") + "");
        }
        if (player.gender == 3)
        {
            outputText(" and ");
        ...
        if (player.hasCock())
        {
            ...
            if (player.cumQ() < 100)
            {
                outputText("few strings of semen");
            }
            else if (player.cumQ() < 500)
            {
                outputText("large amount of semen,  her upper body now a mix of brown and white");
            }
            else if (player.cumQ() < 2000)
            {
                outputText("hefty amount of cum, her upper body mostly coated in your blessing");
            }
            else
            {
                outputText("massive amount of your baby batter, her upper body no longer recognizable as a result of the copious amount of spooge covering her");
            }
        }
    ...
    private function pcGotTentaclesForFun() : Bool
    {
