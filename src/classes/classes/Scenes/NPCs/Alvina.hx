...
class Alvina extends Monster
{
    ...
    {
        var lustDmg : Float = player.lustPercent() * (this.inte / 3 + rand(this.lib - this.inte * 2 + 100) / 3);
        if (hasStatusEffect(StatusEffects.CreepingDoom))
        {
            lustDmg *= 2;
        }
    ...
    private function alvinaPolarMidnight() : Void
    {
        ...
        else if (PolarMidnightSequance == 1)
        {
            var damage : Float = 0;
    ...
    private function alvinaInfernalFlare() : Void
    {
        ...
        if (!player.immuneToBurn())
        {
            ...
            {
                player.createStatusEffect(StatusEffects.BurnDoT, SceneLib.combat.debuffsOrDoTDuration(5), 0.05, 0, 0);
            }
        }
        statScreenRefresh();
        outputText("\n");
    ...
    private function alvinaIceStorm() : Void
    {
        ...
        if (!player.immuneToBleed())
        {
            ...
            {
                player.createStatusEffect(StatusEffects.IzmaBleed, SceneLib.combat.debuffsOrDoTDuration(5), 0, 0, 0);
            }
        }
        statScreenRefresh();
        outputText("\n");
    ...
    private function alvinaTimeStop() : Void
    {
        ...
        if (player.hasPerk(PerkLib.Mage))
        {
            outputText("Your magic might be strong but this is on a whole new level, she is actually playing with the very fabric of reality and time right before your eyes. ");
        }
        ...
        else if (player.isRace(Races.FAIRY, 1, false))
        {
            outputText("You avoid the attack thanks to your Faerie magic, flickering out of reality just as the blades collides.\n\n");
        }
        else if (player.isRace(Races.DISPLACERBEAST, 1, false))
        {
            outputText("Little does she know that you have the ability to displace yourself out of such deadly attacks, You are barely conscious of the blades colliding together at your previous position with a deafening crash as you appear a few yards away!\n\n");
        }
        else
        {
            var damage : Float = player.maxHP() * 1.1;
            if (player.minHP() < 0)
            {
                damage += player.minHP();
            }
            if (combatParry2() > 0)
            {
                damage -= 0.01 * player.maxHP() * combatParry2();
            }
            if (player.shieldBlock > 0)
            {
                damage -= 0.01 * player.maxHP() * combatBlock2();
            }
            if (damage < 1)
            {
                damage = 0;
            ...
            {
                outputText("You dodge, deflect, parry and block as best as you can to take as little damage as possible yet are still impaled a few hundred times!");
            }
            //This seems rather boring of a death message.
            else
            {
    ...
    private function alvinaBlackfireDance() : Void
    {
        ...
        var i : Int = 0;
        while (i < 5 && soulforce >= maxSoulforce() * 0.2)
        {
            if (!player.getEvasionRoll())
            {
                var damage : Int = as3hx.Compat.parseInt(eBaseIntelligenceDamage() * 2);
                player.takeFireDamage(damage);
    ...
    public var timeStopUsed : Bool = false;
    public var PolarMidnightSequance : Int = 0;
    public var SoulTearInitiated : Bool = false;
    ...
    override public function combatRoundUpdate() : Void
    {
        ...
        if (VerdictCooldown > 0)
        {
            VerdictCooldown--;
        }
        if (MeteorStormCooldown > 0)
        {
            MeteorStormCooldown--;
        }
    }
    ...
    {
        var choice : Float = rand(((flags[kFLAGS.PRIMARY_DIFFICULTY] >= 2)) ? 11 : 9);
        switch (choice)
        {
            case 0:
                alvinaInfernalFlare();
            ...
            case 2:
                alvinaWaveOfEcstasy();
            case 3:
                if ((this.HP < this.maxHP() * 9) && !hasStatusEffect(StatusEffects.AbilityCooldown1))
                {
                    alvinaNosferatu();
                }
                else
