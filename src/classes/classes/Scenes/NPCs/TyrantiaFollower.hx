...
class TyrantiaFollower extends NPCAwareContent implements SaveableState
{
    public static var TyraniaIsRemovedFromThewGame : Bool;
    public static var TyraniaPostFinalKissScene : Bool;
    public static var TyraniaSeenFlitzy : Bool;
    public static var TyraniaAndIzumi : Bool;
    public static var TyrantiaAffectionMeter : Float;
    public static var TyrantiaFollowerStage : Float;
    public static var TyrantiaTrainingSessions : Float;
    public static var TyraniaThePhalluspear : Bool;
    public static var TyraniaDriderJuggernautPlate : Bool;
    public static var TyraniaCorrupteedLegendaries : Float;
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            TyraniaIsRemovedFromThewGame = Reflect.field(o, "TyraniaIsRemovedFromThewGame");
            TyraniaPostFinalKissScene = Reflect.field(o, "TyraniaPostFinalKissScene");
            TyraniaSeenFlitzy = Reflect.field(o, "TyraniaSeenFlitzy");
            TyraniaAndIzumi = Reflect.field(o, "TyraniaAndIzumi");
            TyrantiaAffectionMeter = Reflect.field(o, "TyrantiaAffectionMeter");
            TyrantiaFollowerStage = Reflect.field(o, "TyrantiaFollowerStage");
            TyrantiaTrainingSessions = Reflect.field(o, "TyrantiaTrainingSessions");
            TyraniaThePhalluspear = Reflect.field(o, "TyraniaThePhalluspear");
            TyraniaDriderJuggernautPlate = Reflect.field(o, "TyraniaDriderJuggernautPlate");
            TyraniaCorrupteedLegendaries = valueOr(Reflect.field(o, "TyraniaCorrupteedLegendaries"), 0);
        }
    ...
    public function tyraniaAffection(changes : Float = 0) : Float
    {
        TyrantiaAffectionMeter += changes;
        if (TyrantiaAffectionMeter > 100)
        {
            TyrantiaAffectionMeter = 100;
        }
        else if (TyrantiaAffectionMeter < 0)
        {
            TyrantiaAffectionMeter = 0;
        }
    ...
    public function postFightOptions(hpVictory : Bool) : Void
    {
        clearOutput();
        if (hpVictory)
        {
    ...
    public function repeatEncounterBattlefield() : Void
    {
        ...
        if (TyrantiaFollowerStage > 2 && !TyraniaPostFinalKissScene)
        {
            TyraniaPostFinalKissScene = true;
        }
        if (flags[kFLAGS.IZUMI_MET] > 0 && !TyraniaAndIzumi)
        {
            TyraniaAndIzumi = true;
        }
        menu();
        if (TyrantiaAffectionMeter > 40)
        {
            ...
            {
                addButtonDisabled(0, "KISS", "Maybe time to think on making your base a bit more crowded? ;)");
            }
            else
            {
                addButton(0, "KISS", postFightOptionsKiss);
            }
        }
        else
        {
        ...
        if (TyrantiaTrainingSessions >= 40)
        {
            addButtonDisabled(3, "Training", "You finished all training session with her.");
        }
        else
        {
        ...
        {
            addButtonDisabled(4, "Sex", "Req. special scene after reaching 40%+ affection.");
        }
        if (TyraniaPostFinalKissScene && TyrantiaFollowerStage < 4)
        {
            addButton(9, "LiveWithMe", TyrantiaLiveWithMe).hint("Take the Spooder home. Do it NOW ^^");
        }
        else if (isLover())
        {
            addButtonDisabled(9, "LiveWithMe", "She's already in your camp!");
        }
        else
        {
            addButtonDisabled(9, "???", "Req. special scene after reaching 40%+ affection.");
        }
    ...
    public function repeatEncounterBattlefieldRe() : Void
    {
        ...
        {
            addButton(9, "LiveWithMe", TyrantiaLiveWithMe).hint("Take the Spooder home. Do it NOW ^^").disableIf(!TyraniaPostFinalKissScene, "Req. special scene after reaching 40%+ affection.");
        }
        addButton(14, "Leave", explorer.done);
    }
    public function repeatEncounterBattlefieldTalk() : Void
    {
        ...
        {
            addButtonDisabled(2, "FightStyle", "You already can train with her.");
        }
        else
        {
            addButton(2, "FightStyle", repeatEncounterBattlefieldTalkFightingStyle);
        }
        if (isLover())
        {
            if (flags[kFLAGS.TIMES_MET_KIHA] > 0)
            {
                addButton(3, "Kiha", repeatEncounterBattlefieldTalkKiha);
            }
            else
            {
                addButtonDisabled(3, "???", "Perhaps if you look around the swamps, you might find someone she might also know...");
            }
            if (DivaScene.instance.status != 0)
            {
                addButton(4, "Diva", repeatEncounterBattlefieldTalkDiva);
            }
            else
            {
                addButtonDisabled(4, "???", "Perhaps if you look around the mountains, you might find someone she might also know...");
            }
        }
        addButton(5, "Her", repeatEncounterBattlefieldTalkHer);
        if (isLover() && !TyraniaThePhalluspear && flags[kFLAGS.KONSTANTIN_FOLLOWER] >= 2 && !player.hasStatusEffect(StatusEffects.KonstantinOff))
        {
            addButton(10, "ThePhalluspear", talkThePhalluspear);
        }
        if (isLover())
        {
            if (flags[kFLAGS.SLEEP_WITH] != "Tyrantia")
            {
                addButton(12, "Sleep With", TyrantiaSleepToggle);
            }
            else
            {
                addButton(12, "Sleep Alone", TyrantiaSleepToggle);
            }
        }
        if (isLover())
        {
    ...
    public function repeatEncounterBattlefieldTalkHer() : Void
    {
        ...
        menu();
        if (isLover() && (LilyFollower.LilyFollowerState || BelisaFollower.BelisaInCamp))
        {
            addButton(0, "H U D?", repeatEncounterBattlefieldTalkHerHowUDoing).hint("How U doing?");
        }
        else
        {
        ...
        {
            addButton(3, "Goblin", repeatEncounterBattlefieldTalkHerGoblin);
        }
        if (TyraniaAndIzumi)
        {
            addButton(4, "Izumi", repeatEncounterBattlefieldTalkHerIzumi);
        }
        if (TyrantiaFollowerStage == 4 && DriderTown.DriderTownComplete && TyrantiaAffectionMeter >= 50)
        {
            addButton(5, "Kids", talkHerKids);
        }
        if (TyrantiaFollowerStage < 4)
        {
            addButton(6, "LotB", repeatEncounterBattlefieldTalkHerLifeOnTheBattlefield).hint("Life on the Battlefield");
        }
    ...
    public function repeatEncounterBattlefieldTalkHerHowUDoing() : Void
    {
        ...
        if (LilyFollower.LilyFollowerState)
        {
            ...
            if (DriderTown.LilyKidsPC > 0)
            {
                ...
                var _sw32_ = (choice[rand(choice.length)]);                
        ...
        if (BelisaFollower.BelisaInCamp)
        {
            ...
            {
                outputText("\"<i>Belisa’s kids are small, but nimble. I’ve been trying to teach some of them how to fight, but most of them are more interested in learning their mother’s craft.</i>\" She smiles. \"<i>She’s rebuilding the tribe, thanks to you.</i>\"\n\n");
            }
        }
    ...
    public function repeatEncounterBattlefieldTalkHerLifeOnTheBattlefield() : Void
    {
        ...
        if (TyraniaPostFinalKissScene)
        {
            outputText("\"<i>Things have slowed down a lot lately…Apparently keeping me busy was their goal.</i>\" She shudders at the thought. \"<i>Now they’re not constantly coming at me, I have…too much time to think.</i>\"\n\n");
        }
        else
        {
            outputText("Tyrantia rolls her shoulders. \"<i>I don’t even know why the demons come here. There’s stuff to salvage, sure, but nothing worth sending people to die.</i>\" She taps her back legs against the ground. \"<i>At least it keeps me busy, eh?</i>\"\n\n");
        }
        //affection gains
        endEncounter();
    }
    ...
    public function TyraniaAndFlitzyScene() : Void
    {
        ...
        if (TyraniaSeenFlitzy)
        {
            ...
            if (TyraniaPostFinalKissScene)
            {
                ...
                {
                    addButtonDisabled(2, "Sex", "Not for genderless ones.");
                }
            }
            else
            {
                addButtonDisabled(2, "Sex", "Req. special scene after reaching 40%+ affection.");
            }
            if (TyrantiaAffectionMeter >= 40)
            {
                addButton(3, "Flirt", TyraniaAndFlitzySceneFlirt);
            }
            else
            {
                addButtonDisabled(3, "Flirt", "Req. 40%+ affection.");
            }
    ...
    public function TyrantiaLostSparring(hpVictory : Bool) : Void
    {
        clearOutput();
        if (hpVictory)
        {
            outputText("Tyrantia is driven back by your last attack, and she holds up a single hand in a classic ‘stop’ gesture. Her armor battered, she sighs heavily. <i>“That’s enough.”</i> Her black aura flickers, and Tyrantia shudders. <i>“I…Don’t want to continue.”</i> She takes her head in both hands, letting her spear fall to the ground.");
        }
        else
        {
    ...
    private function LevelingHerself() : Void
    {
        ...
        {
            if (flags[kFLAGS.TYRANTIA_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.TYRANTIA_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.TYRANTIA_DEFEATS_COUNTER] = 1;
            }
            if (flags[kFLAGS.TYRANTIA_LVL_UP] < 4 && flags[kFLAGS.TYRANTIA_DEFEATS_COUNTER] >= flags[kFLAGS.TYRANTIA_LVL_UP] + 9)
            {
                var addToV1 : Float = player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * flags[kFLAGS.TYRANTIA_DEFEATS_COUNTER];
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers5))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers5, 1, addToV1);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers5, addToV1, 0, 0, 0);
                }
                flags[kFLAGS.TYRANTIA_DEFEATS_COUNTER] = 0;
    ...
    public function TyrantiaTraining() : Void
    {
        ...
        {
            TyrantiaTrainingDick();
        }
        else
        {
            TyrantiaTrainingPussy();
        }
    }
    ...
    public function TyrantiaTraining2() : Void
    {
        if (TyrantiaTrainingSessions == 4 || TyrantiaTrainingSessions == 9 || TyrantiaTrainingSessions == 14 || TyrantiaTrainingSessions == 19 || TyrantiaTrainingSessions == 24 || TyrantiaTrainingSessions == 29 || TyrantiaTrainingSessions == 34 || TyrantiaTrainingSessions == 39)
        {
            ...
            if (TyrantiaTrainingSessions == 9)
            {
                outputText("<b>Tyrant State bonus to melee damage increased by 20%.</b>\n\n");
            }
            if (TyrantiaTrainingSessions == 14)
            {
                outputText("<b>You have gained new magic special: False Weapon - costs 10% of your max Lust and 100 Fatigue and lasts until the end of combat.</b>\n\n");
            }
            if (TyrantiaTrainingSessions == 19)
            {
                outputText("<b>You can now use toggle to turn on/off auto-cast of Tyrant State at the combat start.</b>\n\n");
            }
            if (TyrantiaTrainingSessions == 24)
            {
                outputText("<b>You have gained ability to take less physical and lust damage the closer you're to the maximum lust. (20% at 50% Max Lust, up to 70% at max)</b>\n\n");
            }
            if (TyrantiaTrainingSessions == 29)
            {
                outputText("<b>You can delay your own defeat by Lust for two turns. Reducing your own Lust below the max will reset the timer.</b>\n\n");
            }
            if (TyrantiaTrainingSessions == 34)
            {
                outputText("<b>Constant Buildup - If you are in Rut or Heat, halve the lust DoT you take from it, and deal an additional 20% physical damage while under the effects.</b>\n\n");
            }
        ...
        if (TyrantiaTrainingSessions == 0.5)
        {
            TyrantiaTrainingSessions = 1;
        }
        else
        {
            TyrantiaTrainingSessions += 1;
    ...
    public function TyrantiaSaveFight() : Void
    {
        ...
        var strTyrantia : Float = 295;
        var meleeAtkTyrantia : Float = 150;
        var intTyrantia : Float = 150;
        ...
        meleeAtkTyrantia += (1 + as3hx.Compat.parseInt(meleeAtkTyrantia / 5)) * player.newGamePlusMod();
    ...
    public function TyrantiaLovinCave() : Void
    {
        ...
        {
            addButtonDisabled(3, "Sex", "Not for genderless ones.");
        }
    }
    ...
    {
        clearOutput();
        if (flags[kFLAGS.KIHA_FOLLOWER] == 1 && rand(10) == 0)
        {
            kihaFollower.tyrantiaInteraction(true);
        }
        else
        {
        ...
        addButton(1, "Talk", repeatEncounterBattlefieldTalk);
        addButton(2, "Spar", TyrantiaSpar).disableIf(flags[kFLAGS.PLAYER_COMPANION_1] == "Tyrantia" || flags[kFLAGS.PLAYER_COMPANION_2] == "Tyrantia", "You can't fight against her as long she's in your team.").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, "You need a good sparring ring for that.");
        if (TyrantiaTrainingSessions >= 40)
        {
            addButtonDisabled(3, "Training", "You finished all training session with her.");
        }
        ...
        addButton(4, "Sex", TyrantiaSexMenu);
        if (player.hasPerk(PerkLib.BasicLeadership) && TyrantiaAffectionMeter >= 60)
        {
            addButton(5, "JoinMe", TyrantiaHenchmanOption);
        }
        else
        {
            addButtonDisabled(5, "JoinMe", "You need to have at least Basic Leadership and 60% affection to form a team.");
        }
        if ((DriderTown.TyrantiaMaleKids + DriderTown.TyrantiaFemaleKids) > 0)
        {
            addButton(6, "Kidplay", TyrantiaKidPlayTime).hint("Play with your Unirider kids");
        }
        if (TyraniaCorrupteedLegendaries >= 1)
        {
            addButton(10, "Items", itemImproveMenuCorrupt);
        }
    ...
    public function TyrantiaKidPlayTime() : Void
    {
        ...
        if (rand(3) == 2)
        {
            ...
            if (DriderTown.BelisaKids > 0)
            {
                outputText("\"<i>“I think, if any of us fits that bill, it’s me, dear sister,”</i> teases Belisa, laying down on the other side of you.\n\n");
            }
            if (DriderTown.LilyKidsPC > 0)
            {
                outputText("\"<i>“No comment,”</i> Lily adds from behind you. Your Drider kids are heading in, and Belisa and Lily laugh, leaving Tyrantia and you alone for a few moments.\n\n");
            }
    ...
    public function TyrantiaHenchmanOption() : Void
    {
        ...
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "Tyrantia" || flags[kFLAGS.PLAYER_COMPANION_3] == "Tyrantia")
            {
                addButtonDisabled(0, "JoinMe (1)", "You already have Tyrantia accompany you.");
            }
            else
            {
                addButton(0, "JoinMe (1)", TyrantiaHenchmanOption2, 1).hint("Ask Tyrantia to join you in adventures outside camp.");
            }
        }
        else if (flags[kFLAGS.PLAYER_COMPANION_1] == "Tyrantia")
        {
            addButton(5, "JoinMe (1)", TyrantiaHenchmanOption2, 21).hint("Ask Tyrantia to stay in camp.");
        }
        else
        {
            addButtonDisabled(5, "JoinMe (1)", "You already have other henchman accompany you as first party member. Ask him/her to stay at camp before you talk with Tyrantia about accompaning you as first party member.");
        }
        if (player.hasPerk(PerkLib.IntermediateLeadership))
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "")
            {
                if (flags[kFLAGS.PLAYER_COMPANION_1] == "Tyrantia" || flags[kFLAGS.PLAYER_COMPANION_3] == "Tyrantia")
                {
                    addButtonDisabled(1, "JoinMe (2)", "You already have Tyrantia accompany you.");
                }
                else
                {
                    addButton(1, "JoinMe (2)", TyrantiaHenchmanOption2, 2).hint("Ask Tyrantia to join you in adventures outside camp.");
                }
            }
            else if (flags[kFLAGS.PLAYER_COMPANION_2] == "Tyrantia")
            {
                addButton(6, "JoinMe (2)", TyrantiaHenchmanOption2, 22).hint("Ask Tyrantia to stay in camp.");
            }
            else
            {
                addButtonDisabled(6, "JoinMe (2)", "You already have other henchman accompany you as second party member. Ask him/her to stay at camp before you talk with Tyrantia about accompaning you as second party member.");
            }
        }
        else
        {
    ...
    public function TyrantiaHenchmanOption2(slot : Float = 1) : Void
    {
        clearOutput();
        if (slot < 21)
        {
            ...
            if (!player.isDrider() && !player.isTaur() && !player.isInNonGoblinMech() && !player.isInGoblinMech())
            {
                outputText("She kisses you on the cheek, picking you up and putting you on her Drider back. \"<i>Just tell me where to go, my sweet [race], and I’ll make sure we get there safely.</i>\"\n\n");
            }
            ...
            if (slot == 2)
            {
                flags[kFLAGS.PLAYER_COMPANION_2] = "Tyrantia";
            }
            if (slot == 1)
            {
                flags[kFLAGS.PLAYER_COMPANION_1] = "Tyrantia";
            }
        }
        else
        {
    ...
    {
        var improvableItems : Array<Dynamic> = [
        ...
        for (i in 0...improvableItems.length)
        {
            ...
            else
            {
                ...
                selectMenu.add(item.id, curry(improveItem, item, from)).disableIf(!player.hasItem(from), "You need " + from.longName + " as a base to create this item").disableIf(player.keyItemvX("Radiant shard", 1) < 3, "You need at least three radiant shards in order to create this item.").disableIf(player.gems < 20000, "You need at least 20 000 gems in order to create this item");
            }
        }
        submenu(selectMenu, TyrantiaAtCamp);
        
        var improveItem : ItemType->ItemType->Void = function(item : ItemType, from : ItemType) : Void
        {
            clearOutput();
            ...
            if (player.keyItemvX("Radiant shard", 1) == 3)
            {
                player.removeKeyItem("Radiant shard");
            }
            else
            {
                player.addKeyValue("Radiant shard", 1, -3);
            }
            player.gems -= 20000;
