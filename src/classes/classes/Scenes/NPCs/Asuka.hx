...
class Asuka extends Monster
{
    private function asukaAttack1() : Void
    {
        wrath -= 30;
        var damage : Float = eBaseStrengthDamage() * 2;
    ...
    {
        var damage : Float = ((str * 2) - rand(player.tou));
    ...
    {
        var fireDMG : Float = Math.round(this.lust * 0.9);
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (flags[kFLAGS.ASUKA_LVL_UP] < 1)
        {
            ...
            if (choice1 == 2)
            {
                if (!hasStatusEffect(StatusEffects.AbilityCooldown1) && lust >= 50)
                {
                    asukaFlamesOfLove();
                }
                else
