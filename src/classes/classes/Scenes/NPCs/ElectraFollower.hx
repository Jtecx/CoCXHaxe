...
class ElectraFollower extends NPCAwareContent implements SaveableState implements TimeAwareInterface
{
    public static var ElectraSonsNum : Float;
    public static var ElectraDaughtersNum : Float;
    public static var ElectraHermKidsNum : Float;
    public static var ElectraTotalKidsNum : Float;
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            ElectraSonsNum = Reflect.field(o, "ElectraSonsNum");
            ElectraDaughtersNum = Reflect.field(o, "ElectraDaughtersNum");
            ElectraHermKidsNum = Reflect.field(o, "ElectraHermKidsNum");
            ElectraTotalKidsNum = Reflect.field(o, "ElectraTotalKidsNum");
        }
    ...
    public function electraAffection(changes : Float = 0) : Float
    {
        flags[kFLAGS.ELECTRA_AFFECTION] += changes;
        if (flags[kFLAGS.ELECTRA_AFFECTION] < 0)
        {
            flags[kFLAGS.ELECTRA_AFFECTION] = 0;
        }
        if (flags[kFLAGS.ELECTRA_AFFECTION] > 100)
        {
            flags[kFLAGS.ELECTRA_AFFECTION] = 100;
        }
    ...
    public function repeatElectraEnc() : Void
    {
        ...
        {
            outputText("the Raiju girl");
        }
        outputText(" again. She's panting on the ground with a hand under her dress and masturbating like there is no tomorrow.\n\n");
        outputText("\"<i>Ahhhh someone... someone help me get rid of this maddening desire... rape me... fuck me I don’t care how you do it. Gah, why are they all fleeing!...come back! Come back so I can get rid of my itches... Come back so we can FUCK!</i>\"\n\n");
        if (player.isRace(Races.RAIJU, 1, false) || player.isRace(Races.THUNDERBIRD, 1, false) || player.isRace(Races.KIRIN, 1, false))
        {
            repeatEncAsRaijuPC();
        }
        else
    ...
    public function repeatEncAsRaijuPC() : Void
    {
        ...
        if (player.isRace(Races.RAIJU, 1, false))
        {
            outputText(" another Raiju.");
        }
        if (player.isRace(Races.THUNDERBIRD, 1, false))
        {
            outputText(" a Thunderbird.");
        }
        if (player.isRace(Races.KIRIN, 1, false))
        {
            outputText(" a Kirin.");
        }
    ...
    public function ElectraRecruiting() : Void
    {
        ...
        if (player.isRace(Races.RAIJU, 1, false))
        {
            outputText("" + player.mf("kinsman", "kinswoman") + "");
        }
    ...
    public function ElectraCampMainMenu() : Void
    {
        ...
        if (flags[kFLAGS.ELECTRA_DAILY_STORM_JEWEL] > 0)
        {
            addButtonDisabled(3, "Req. Jewel", "You already asked her for a jewel today.");
        }
        else
        {
            addButton(3, "Req. jewel", electraStromJewel).hint("Ask Electra for a storm jewel.");
        }
        addButton(4, "Spar", electraSpar).hint("Ask Electra for a mock battle with sex for the winner.").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, "You need a good sparring ring for that.");
    ...
    public function electraSex() : Void
    {
        ...
        menu();
        addButton(0, "Charge up", ElectraSeXChargeUp).disableIf(!player.isRace(Races.RAIJU, 1, false) && !player.isRace(Races.THUNDERBIRD, 1, false), "Only available to Raijus and Thunderbirds.");
        addButton(1, "Volt Transfer", ElectraVoltTransfer).disableIf(!player.isRace(Races.RAIJU, 1, false) && !player.isRace(Races.THUNDERBIRD, 1, false), "Only available to Raijus and Thunderbirds.");
    ...
    public function ElectraSexPlayer() : Void
    {
        ...
        {
            outputText(" and milk gushing nipples");
        }
    ...
    public function PlayerSexElectraPostSpar() : Void
    {
        ...
        {
            if (flags[kFLAGS.ELECTRA_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.ELECTRA_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.ELECTRA_DEFEATS_COUNTER] = 1;
            }
            if (flags[kFLAGS.ELECTRA_LVL_UP] < 12)
            {
                if (flags[kFLAGS.ELECTRA_DEFEATS_COUNTER] == 4 + flags[kFLAGS.ELECTRA_LVL_UP])
                {
                    ...
                    {
                        player.addStatusValue(StatusEffects.CampSparingNpcsTimers4, 3, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 4 + flags[kFLAGS.ELECTRA_LVL_UP]);
                    }
                    else
                    {
                        player.createStatusEffect(StatusEffects.CampSparingNpcsTimers4, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 4 + flags[kFLAGS.ELECTRA_LVL_UP], 0);
                    }
                    flags[kFLAGS.ELECTRA_DEFEATS_COUNTER] = 0;
    ...
    public function PlayerSexElectra() : Void
    {
        ...
        menu();
        addButton(0, "Yea Sure", ElectraSeXYes).disableIf(player.lust < 33, "Not horny enough!").disableIf(player.isGenderless(), "Not for genderless!");
    ...
    public function ElectraSeXYesM() : Void
    {
        ...
        if (flags[kFLAGS.ELECTRA_TALKED_ABOUT_HER] == 2 || flags[kFLAGS.ELECTRA_TALKED_ABOUT_HER] >= 4)
        {
            outputText("Well here you go again... You brace yourself for the incoming jolt as");
        }
    ...
    public function afterYes() : Void
    {
        if (flags[kFLAGS.ELECTRA_FOLLOWER] < 2)
        {
            ...
            if (player.isRace(Races.RAIJU, 1, false))
            {
                outputText("Well your own energy management isn't getting any better, seems you will have to expel that on someone else.\n\n");
            }
            else
            {
    ...
    public function afterSex() : Void
    {
        ...
        else
        {
            ...
            inventory.takeItem(useables.RPLASMA, cleanupAfterCombat);
        }
    }
    ...
    public function ElectraSeXChargeUp() : Void
    {
        ...
        if (flags[kFLAGS.ELECTRA_FOLLOWER] < 2)
        {
            outputText("You don't wait for her to tell you twice and run off back to camp.\n\n");
        }
        electraAffection(10);
        if (player.lust100 < 90)
        {
            player.lust = player.maxLust() * 0.9;
        }
        if (flags[kFLAGS.ELECTRA_FOLLOWER] < 2)
        {
            endEncounter();
        ...
        {
            inventory.takeItem(useables.RPLASMA, explorer.done);
        }
    }
    ...
    public function ElectraSeXHeadpat() : Void
    {
        ...
        if (player.hasCock())
        {
            ...
            if (player.hasVagina())
            {
                outputText(", as your pussy gushes with tingling pleasure and anticipation");
            }
        }
        if (player.hasVagina())
        {
            outputText(" pussy gushing with tingling pleasure and anticipation");
        }
    ...
    public function ElectraPregProgression1() : Void
    {
        ...
        //first time{
            ...
            outputText("\"<i>Well…I have [Player] for that.</i>\" Electra says, smiling slightly. Ralthazul seems unamused at this, but continues.  \n\n");
