...
class Marble extends Monster
{
    ...
    //Special1: Heavy overhead swing, high chance of being avoided with evasion, does heavy damage if it hits.
    {
        ...
        damage = as3hx.Compat.parseInt((str + 20 + weaponAttack) - player.armorDef);
        if (damage <= 0)
        {
            damage = 0;
    ...
    //Special2: Wide sweep; very high hit chance, does low damage.
    {
        ...
        //Determine damage - str modified by enemy toughness!
        damage = as3hx.Compat.parseInt((str + 40 + weaponAttack) - player.armorDef);
        damage /= 2;
        if (damage <= 0)
        {
            damage = 0;
