...
class Siegweird extends Monster
{
    ...
    private function siegweirdSmash() : Void
    {
        ...
        if (hasStatusEffect(StatusEffects.BerzerkingSiegweird))
        {
            damage *= 2;
        }
    ...
    override private function performCombatAction() : Void
    {
        ...
        {
            if (statusEffectv1(StatusEffects.HealingSiegweird) > 1)
            {
                addStatusValue(StatusEffects.HealingSiegweird, 1, -1);
            }
            else
            {
                removeStatusEffect(StatusEffects.HealingSiegweird);
            }
        }
        if ((this.lust > this.maxLust() * 0.5) || (this.HP < this.maxHP() * 0.5) && !hasStatusEffect(StatusEffects.BerzerkingSiegweird))
        {
            siegweirdBerserk();
        }
        switch (choice)
        {
            case 0:
                siegweirdSmash();
            case 1:
                if ((this.HP < this.maxHP() * 0.75) && !hasStatusEffect(StatusEffects.HealingSiegweird))
                {
                    siegweirdHeal();
                }
                else
