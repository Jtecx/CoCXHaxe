...
class DivaScene extends XXCNPC
{
    public static var instance(get, never) : DivaScene;

    private static var _instance : DivaScene;
    
    public var status : Int;
    private var firstLoss : Bool = true;
    public var tookVialToday : Bool = false;  // true if took her vial today  
    ...
    private static function get_instance() : DivaScene
    {
        if (_instance == null)
        {
            new DivaScene();
    ...
    {
        if (loadfrom == null || loadfrom.diva == null)
        {
    ...
    {
        if ((type == LOVER || type == COMPANION) && !player.hasStatusEffect(StatusEffects.DivaOff))
        {
            return status == -1;
        }
        return false;
    ...
    override public function campDescription(buttons : ButtonDataList, menuType : Int = -1) : Void
    {
        if (isCompanion(menuType))
        {
            if (time.hours >= 20)
            {
                outputText("Diva is quietly doing things next to her tent, readying for the nighttime.");
            }
            else if (time.hours < 7)
            {
                outputText("Diva is likely hunting a fair the distance away from the camp, though you could call her back at any time.");
            }
            else
            {
                outputText("Since it's daytime, Diva is likely sleeping in her coffin.");
            }
    ...
    private function vampirism() : Void
    {
        ...
        {
            if (!player.hasStatusEffect(StatusEffects.Familiar))
            {
                player.createStatusEffect(StatusEffects.Familiar, 0, 0, 0, 0);
            }
            if (!player.hasPerk(PerkLib.Familiar))
            {
    ...
    public function moonlightSonata(fromCombat : Bool = false) : Void
    {
        if (status == 0)
        {
            status = 1;
        ...
        {
            var x1 : Int;
            if (player.cockThatFits(24, "length") < 0)
            
            //check if ANY is fitting, otherwise this is trash{
                
                x1 = as3hx.Compat.parseInt(player.smallestCockIndex() + 1);
                ...
                if (timesReduced >= 5)
                {
                    outputText("\"<i>It's the fifth time I have to fix thou, are thou seriously enjoying this?</i>\"\n");
                }
                else
                {
                    outputText("\"<i>And now for the main course...</i>\"\n");
                }
                outputText("\n");
                player.cocks[x1 - 1].cockLength = 18;
            }
            else
            {
                x1 = as3hx.Compat.parseInt(player.cockThatFits(24, "length") + 1);
            }
            ...
            {
                sceneHunter.print("Check failed: vampire/bat race");
            }
            ...
            if (fromCombat)
            {
                outputText("\"<i>Thou get to taste my second mouth, so thou should feel honored. It's more than you deserve.</i>\"\n");
            }
            else
            {
                outputText("\"<i>Your neck tastes good, [name], but I will need more than that to be satisfied.</i>\"\n");
            }
            outputText("\n"
                    + "Diva slowly slides down on your slick [cock "
                    + x1
                    + "], giving you a fanged smile of bliss as she slowly takes your length in. She feels similar to what you think a succubus would and her pussy clearly has something inhuman about it. She's now riding you, wings closed around your torso, arms and around your shoulders, as she milks your [cock "
                    + x1
                    + "] thoroughly. You moan in pleasure as the vampire bites your neck in the middle of the act only making you more erect and ready, your [cock "
                    + x1
                    + "] harder than ever. She unplugs her mouth only for a moment, just to whisper in your ear.\n"
                    + "\n"
    ...
    private function bloodyRose() : Void
    {
        ...
        function dickF() : Void
        {
            outputText("Your [cock] begins to harden from the overstimulation of your sensible vampire neck and the excitement of blood. Diva's pussy seems to agree as she's practically wetting the ground below the pair of you. Giving in to your instinct, you slowly insert yourself in her like your fang in her neck as your increasingly hardening [cock] begins to pulse synchronously with your heartbeat. Her pussy reacts similarly to the penetration, she's more than ready to go.\n"
                    + "\n"
                    + "You both moan in each other's necks as you begin to hugfuck her. Diva's pussy is like a living creature, clenching strongly on your [cock] and pulsing as if to milk you dry. You, however, aren't going to simply lay back and let the devilish pussy do all the work as you begin to piston in and out of her. Diva's wings are flapping wildly behind her"
                    + ((player.wings.type == Wings.VAMPIRE) ? ", yours too, and you try not to take flight as the winds created by your combined flapping start shaking the tent" : "")
                    + ".\n"
                    + "Both of you scream in ecstasy, breaking out of the bite, blood still dripping from your chin as you shoot your load in her greedy vagina"
                    + ((player.cumQ() >= 200) ? ", the excess dripping to the ground" : "")
                    + ". Y");
            player.sexReward("vaginalFluids", "Dick");
            sharedEnd();
        };
        ...
        switch (_sw11_)
        {
            case 0:
                outputText("You are taking a walk through the mountain area when, much to your annoyance, a thunderstorm begins. However, unlike the ones you hear of back home, this thunderstorm has no rain.. peculiar. Ever since you shut down the factory the weather has been erratically switching from clear to a full-fledged thunderstorm randomly. Deciding not to head back to camp yet, more out of spite than anything else, you seek shelter instead.\n"
                        ...
                        + "\n"
                        + "\"<i>Fufufufu... foolish mortal! Thou hast walked into mine trap! Abandon all hope of escaping mine castle unscathed, for I am Diva, the grand Countess of Dusk! "
                        + ((silly()) ? "You were expecting Dio, but it was I, Diva! " : "")
                        + "Prepare to forfeit thine blood and body to me! <b>TREMBLE BEFORE MINE POWER!</b></i>\"\n"
                        + "\n"
            ...
            case 1:
                ...
                if (silly())
                {
                    outputText("  Huh, you might be onto something. Is this that 'chuunibyou' thing the mechanic at the factory kept raving on about? He also said something about 'Senpais' noticing things and some creature called 'tsundere'.\n");
                }
                outputText("\n"
                        + "Just as you get inside, a triggering mechanism makes itself known, as the doors lock themselves with a click. Huh, she got up to some tinkering between visits, it seems. Neat. You might ask her about how she pulled off that automatic lock trick after you finish the fight"
                        + ((flags[kFLAGS.CAMP_CABIN_PROGRESS] >= 10) ? ", would be nice to have in your cabin" : "")
                        + ". Diva, unwitting of your musings, appears in a cloud of smoke at the center of the room. She prepared some special effects, too. You would give a polite clap if you didn't know she would bitch at you for breaking the mood.\n"
                        + "\n"
            ...
            case 2:
                outputText("This time you don't even wait for a thunderstorm, you just head straight to Diva's castle for another match. Truth be told, you are beginning to get endeared to her, somewhat. Also, you don't begrudge having a good sparring partner, it's nice having someone strong to practice against without any real risks should you lose.\n"
                        ...
                        + "\n"
                        + "She throws a glass cup filled with what's probably blood to the ground, in what you assume she thinks is an elegant way, as she takes flight, in as much as she can while indoors, high ceilings or not. Aaaaaaand she has gotten herself a battle aura of darkness, of course she has. "
                        + ((silly()) ? "Such is life in the zon- Mareth. " : "")
                        + "But seriously, she is clearly stronger now. Well, it seems she is not holding back this time around.");
    ...
    public function DivaPregProgression2() : Void
    {
        ...
        addButton(2, "Feed", DivaPregCravingsAccept);
    }
