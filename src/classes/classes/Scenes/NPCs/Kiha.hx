...
class Kiha extends Monster
{
    ...
    private function finalizeDamage(damage : Int) : Int
    {
        ...
        {
            damage *= as3hx.Compat.parseInt(1 + (flags[kFLAGS.KIHA_LVL_UP] * 0.1));
        }
        if (flags[kFLAGS.KIHA_LVL_UP] >= 8)
        {
            damage *= as3hx.Compat.parseInt(1 + (flags[kFLAGS.KIHA_LVL_UP] * 0.2));
        }
        if (flags[kFLAGS.KIHA_LVL_UP] >= 13)
        {
            damage *= as3hx.Compat.parseInt(1 + (flags[kFLAGS.KIHA_LVL_UP] * 0.3));
        }
        return damage;
    }
    ...
    private function sillyModeKihaAttack() : Void
    {
        ...
        else
        {
            ...
            var damage : Int = as3hx.Compat.parseInt((str + weaponAttack) - rand(player.tou) - player.armorDef);
            damage += 5;
            ...
            if (player.HP >= 1)
            {
                outputText("You follow the shrill cry of \"<i>B-BAKA!</i>\" in the distance until you reach the exact location you were in a few seconds earlier, prepared to fight again.");
            }
        }
    }
    ...
    private function kihaFirePunch() : Void
    {
        ...
        {
            var damage : Int = as3hx.Compat.parseInt((str) - (player.armorDef));
            damage = finalizeDamage(damage);
    ...
    private function kihaFireBreath() : Void
    {
        ...
        {
            var damage : Float = Math.round(90 + rand(10) + (player.newGamePlusMod() * 30));
            damage = finalizeDamage(damage);
    ...
    {
        if (Std.is(ability, BlindSpell) && hasStatusEffect(StatusEffects.Blind) && display)
        {
    ...
    override private function postAttack(damage : Int) : Void
    {
        ...
        var flame : Int = as3hx.Compat.parseInt(level + rand(6));
    ...
    public function new()
    {
        ...
        if (flags[kFLAGS.SILLY_MODE_ENABLE_FLAG] == 1)
        {
            ...
            this.abilities = [
                    ...
                    {
                        call : sillyModeKihaAttack,
                        type : ABILITY_PHYSICAL,
                        range : RANGE_MELEE,
                        tags : [TAG_BODY, TAG_FIRE],
