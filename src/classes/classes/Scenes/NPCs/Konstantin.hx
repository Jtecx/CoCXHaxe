...
class Konstantin extends NPCAwareContent
{
    /*
		Konstantin flags:
		...
		*/
    private function anyUsableMaterials() : Bool
    {
    ...
    public function meetKonstantinAtForest() : Void
    {
        ...
        else
        {
            ...
            {
                outputText(" The only ones that you know are in Tel’Adre, and albeit decent, their products aren’t exactly outstanding.");
            }
        ...
        menu();
        if (player.armorName == "nothing" || player.armorName == "gear")
        {
            addButtonDisabled(0, "Armor", "You need to wear any armor to pick this option.");
        }
        else
        {
        ...
        {
            addButtonDisabled(1, "Weapon", "You need to use any melee weapon to pick this option.");
        }
        else
        {
    ...
    public function meetKonstantinAtForestArmor() : Void
    {
        ...
        if (player.gems >= 25)
        {
            ...
            {
                flags[kFLAGS.KONSTANTIN_SERVICES]++;
            }
            ...
            else
            {
                ...
                if (flags[kFLAGS.KONSTANTIN_FOLLOWER] >= 2)
                {
                    outputText(", and return to your daily tasks");
                }
                outputText(".\n\n");
            }
        }
        ...
        {
            outputText("\"<i>It’s okay, " + player.mf("man", "girl") + ". Gems can be tough to get sometimes. If you feel like using my services, you can find me in this clearing.</i>\"\n\n");
        }
        endEncounter();
    }
    public function meetKonstantinAtForestWeapon() : Void
    {
        ...
        if (player.gems >= 25)
        {
            ...
            if (flags[kFLAGS.KONSTANTIN_FOLLOWER] >= 2)
            {
                outputText("one of his couches");
            }
            else
            {
                outputText("the tree stump");
            }
    ...
    public function meetKonstantinAtForestNothing() : Void
    {
        ...
        if (flags[kFLAGS.KONSTANTIN_FOLLOWER] >= 2)
        {
            outputText("Stop by again soon if you happen to need help with your stuff");
        }
        else
        {
            outputText("Was a pleasure to meet you, still, stop by again soon if you happen to need a good smith");
        }
        outputText(".</i>\"\n\n");
    ...
    public function meetKonstantinAtForest2() : Void
    {
        ...
        {
            outputText(" at start, you have a camp where you sleep and prepare for your daily tasks");
        }
    ...
    public function KonstantinMainCampMenu() : Void
    {
        ...
        {
            addButton(6, "Smelting", KonstantinSmeltingMenu);
        }
    ...
    public function KonstantinTalkMenu() : Void
    {
        ...
        if (flags[kFLAGS.KONSTANTIN_FOLLOWER] == 2)
        {
            addButton(13, "Better Smiting", KonstantinTalkBetterSmiting);
        }
    ...
    public function KonstantinTalkHisWork() : Void
    {
        ...
        if (player.weaponRangePerk == "Bow" || player.weaponRangePerk == "Crossbow" || player.weaponRangePerk == "Pistol" || player.weaponRangePerk == "Rifle")
        {
            outputText("\"<i>Recently, I’ve come in contact with some old fools from...how in hell’s name that place called, JaChinDau? that told me how to make strong materials for their ranged weapons. Those are all the rage there, since they allow a weaker fighter to defeat a stronger one, as long as he has some focus on his hands. I’ve learned about how sharpen arrows and javelins, and make this hollow metal balls that those ‘gun’ things makes use of.</i>\"\n\n");
        }
        if (flags[kFLAGS.KONSTANTIN_SERVICES] >= 2)
        {
            outputText("\"<i>Also, since you’ve brought me some quite weird stuff to work with, I had to remove the dust from some old manuals about how handle magical and non-metal materials, and how turn them into solid plates and pieces of armor.</i>\"\n\n");
        }
        ...
        if (player.hasPerk(PerkLib.HistorySmith) || player.hasPerk(PerkLib.PastLifeSmith))
        {
            ...
            {
                outputText("Well years of working in your past life. ");
            }
        }
    ...
    public function KonstantinTalkTheCamp() : Void
    {
        ...
        else
        {
            ...
            {
                outputText("\"<i>That cabin of yours makes a good reference point for the rest of the camp. Honestly, I’m tempted to make one for myself. Still, my tent is nice enough, but if we’re having to stay here, a more permanent settlement wouldn’t hurt.</i>\"\n\n");
            }
            if (flags[kFLAGS.CAMP_WALL_PROGRESS] >= 100)
            {
                ...
                if (camp.companionsCount() >= 2)
                {
                    outputText(" and the others safe");
                }
                outputText(".</i>\"\n\n");
            }
            else if (flags[kFLAGS.CAMP_WALL_PROGRESS] >= 100 && flags[kFLAGS.CAMP_WALL_SKULLS] >= 15)
            {
                outputText("\"<i>Only a fool of an imp would get near our camp, given the skull-themed decoration on the wall. I bet than even the incubus and succubus are cowed of coming near. Everything feels safer with them sheltering us.</i>\"\n\n");
            }
            if (flags[kFLAGS.MATERIALS_STORAGE_UPGRADES] >= 4)
            {
                outputText("\"<i>Given the amount of stuff that is found outside, it was a rather wise choice to get a place for holding all of it. If we’re lucky and, in a couple of years, rain returns as normal, this barren land will become a lush prairie, and we could make our own crops around the camp, and obviously, a place to store our harvest will come in handy.</i>\"\n\n");
            }
            if (flags[kFLAGS.CAMP_UPGRADES_HOT_SPRINGS] >= 4)
            {
                ...
                {
                    outputText(" Surely beats that weird milk-bath.");
                }
                if (player.hasCock())
                {
                    outputText(" Though I’ve seen you getting a peek or two at me, not that I mind, perv " + player.mf("guy", "girl") + ". If you’re so eager, we could meet alone...");
                }
                outputText("</i>\"\n\n");
            }
            if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] >= 2)
            {
                outputText("\"<i>And there is this place where we can spend our excess energy. Quite good, since it also keep us in shape in case the demons get too cocky someday and intend to storm our camp.</i>\"\n\n");
            }
    ...
    public function KonstantinSmithingMenu() : Void
    {
        ...
        if (anyUsableMaterials())
        {
            ...
            if (player.hasItem(useables.D_SCALE))
            {
                ...
                {
                    outputText(", well, except for the one with us, so the chances of finding one of those are pretty slim");
                }
        ...
        if (player.hasItem(useables.GREENGL))
        {
            addButton(0, "G.Gel", KonstantinCraftingGreenGelArmor);
        }
        if (player.hasItem(useables.B_CHITN))
        {
            addButton(1, "Chitin", KonstantinCraftingChitinItems);
        }
        if (player.hasItem(useables.T_SSILK) && flags[kFLAGS.RATHAZUL_ARMOR_TYPE] < 1)
        {
            addButton(2, "SpiderSilk", KonstantinCraftingSpiderSilkItems);
        }
        if (player.hasItem(useables.D_SCALE) && flags[kFLAGS.RATHAZUL_ARMOR_TYPE] < 1)
        {
            addButton(3, "Dragonscale", KonstantinCraftingDragonscaleItems);
        }
        if (player.hasItem(useables.EBONBLO) && flags[kFLAGS.RATHAZUL_ARMOR_TYPE] < 1)
        {
            addButton(4, "Ebonbloom", KonstantinCraftingEbonbloomItems);
        }
        if (player.hasItem(useables.WT_BRAN))
        {
            addButton(5, "W.T.Branch", KonstantinCraftingYggdrasilItems);
        }
        if (flags[kFLAGS.KONSTANTIN_FOLLOWER] >= 3)
        {
            if (player.hasItem(useables.IRONORE) || Crafting.BagSlot04 > 0)
            {
                addButton(6, "Iron", KonstantinCraftingIronItems);
            }
            if (player.hasItem(useables.MOONSTO) || Crafting.BagSlot07 > 0)
            {
                addButton(7, "Moonstone", KonstantinCraftingMoonstoneItems);
            }
            if (player.hasItem(useables.SKYMETA) || Crafting.BagSlot08 > 0)
            {
                addButton(8, "Skymetal", KonstantinCraftingSkymetalItems);
            }
            if (player.hasItem(useables.EBONING) || Crafting.BagSlot06 > 0)
            {
                addButton(9, "EbonbIng", KonstantinCraftingEbonbIngItems);
            }
        }
        if (player.hasItem(useables.RED_GEL))
        {
            addButton(10, "R.Gel", KonstantinCraftingRedGelArmor);
        }
    ...
    {
        menu();
        if (TyrantiaFollower.TyraniaThePhalluspear && player.hasItem(weapons.SPEAR) && player.hasItem(consumables.L_DRAFT, 5) && player.hasItem(useables.T_SSILK) && CampStatsAndResources.MetalPieces >= 3)
        {
            addButton(6, "ThePhalluspear", KonstantinCraftingThePhalluspear);
        }
        if (player.hasItem(useables.DBAPLAT))
        {
            addButton(8, "D.Bark Armor", KonstantinCraftingDivineBarkArmor);
        }
        if (player.hasItem(useables.TBAPLAT))
        {
            addButton(9, "T.Bark Armor", KonstantinCraftingTentacledBarkArmor);
        }
        if (player.hasItem(useables.DBAPLAT) && player.hasItem(weapons.W_STAFF))
        {
            addButton(10, "Puritas", KonstantinCraftingPuritas);
        }
        if (player.hasItem(useables.TBAPLAT) && player.hasItem(weapons.W_STAFF))
        {
            addButton(11, "Depravatio", KonstantinCraftingDepravito);
        }
        if (player.hasItem(useables.TBAPLAT) && player.hasItem(weapons.PURITAS))
        {
            addButton(12, "Ascensus", KonstantinCraftingPuritasAscensus);
        }
        if (player.hasItem(useables.DBAPLAT) && player.hasItem(weapons.DEPRAVA))
        {
            addButton(12, "Ascensus", KonstantinCraftingDepravitoAscensus);
        }
    ...
    private function KonstantinCraftingRedGelArmor() : Void
    {
        ...
        {
            clearOutput();
            cast((1), KonstantinCraftingGelArmor);
    ...
    private function KonstantinCraftingGelArmor(type : Float = 0) : Void
    {
    ...
    {
        if (spidersilkType == 1 || spidersilkType == 2 || spidersilkType == 3)
        ...
        else
        {
            ...
            {
                KonstantinCraftingNotEnoughMaterials2();
            }
        }
        if (flags[kFLAGS.RATHAZUL_ARMOR_TYPE] > 0)
        {
            ...
            statScreenRefresh();
            var itype : ItemType;
            var _sw22_ = (flags[kFLAGS.RATHAZUL_ARMOR_TYPE]);            
    ...
    {
        if (dragonscaleType == 1 || dragonscaleType == 2 || dragonscaleType == 3)
        ...
        if (flags[kFLAGS.RATHAZUL_ARMOR_TYPE] > 0)
        {
            statScreenRefresh();
            var itype : ItemType;
            var _sw23_ = (flags[kFLAGS.RATHAZUL_ARMOR_TYPE]);            
    ...
    private function KonstantinCraftingEbonbloomItems() : Void
    {
        ...
        if (player.hasItem(useables.EBONBLO, 8) && player.hasItem(armors.H_GARB_, 1) && player.hasKeyItem("Dark Mage’s Grimoire") >= 0)
        {
            addButton(4, "H. Garb", KonstantinCraftingEbonbloomItems2, 5, null, null, armors.EHGARB_.description);
        }
    ...
    {
        if (ebonbloomType == 1 || ebonbloomType == 2 || ebonbloomType == 3 || ebonbloomType == 4)
        ...
        //Armor or robes{
            ...
            {
                KonstantinCraftingNotEnoughMaterials3();
            }
        }
        else if (ebonbloomType == 5)
        {
            ...
            {
                KonstantinCraftingNotEnoughMaterials5();
            }
        }
        //Undergarments
        else
        {
            ...
            {
                KonstantinCraftingNotEnoughMaterials4();
            }
        }
        if (flags[kFLAGS.RATHAZUL_ARMOR_TYPE] > 0)
        {
            statScreenRefresh();
            var itype : ItemType;
            var _sw24_ = (flags[kFLAGS.RATHAZUL_ARMOR_TYPE]);            
    ...
    private function KonstantinCraftingYggdrasilItems2(yggdrasilType : Int) : Void
    {
        ...
        var itype : ItemType;
        var _sw25_ = (flags[kFLAGS.RATHAZUL_ARMOR_TYPE]);        
    ...
    private function KonstantinCraftinArrowItems(arrowsType : Int) : Void
    {
        ...
        switch (arrowsType)
        {
            ...
            case 2:  //Sword  
                ...
                atype = "moonstone";
            case 3:  //Bow  
                if (Crafting.BagSlot08 > 0)
                {
                    Crafting.BagSlot08 -= 1;
                }
                else
                {
                    player.destroyItems(useables.SKYMETA, 1);
                }
    ...
    private function KonstantinCraftingDivineBarkArmor() : Void
    {
        ...
        {
            outputText(" Your " + biggestBreastSizeDescript() + " barely fit into the breastplate, leaving you displaying a large amount of jiggling cleavage.");
        }
    ...
    public function KonstantinSmeltingMenu() : Void
    {
        ...
        menu();
        if ((player.hasItem(useables.COP_ORE) && player.hasItem(useables.TIN_ORE)) || (Crafting.BagSlot01 > 0 && Crafting.BagSlot02 > 0))
        {
            addButton(0, "Copper/Tin", KonstantinSmeltingMenuCopperAndTin);
        }
        if (player.hasItem(useables.EBONBLO) || Crafting.BagSlot05 > 0)
        {
            addButton(1, "Ebonbloom", KonstantinSmeltingMenuEbonbloom);
        }
    ...
    private function KonstantinSmeltingMenuCopperAndTin() : Void
    {
        ...
        if ((player.hasItem(useables.COP_ORE) && player.hasItem(useables.TIN_ORE)) || (Crafting.BagSlot01 > 0 && Crafting.BagSlot02 > 0))
        {
            addButton(0, "One", KonstantinSmeltingMenuProcess, "copper and tin", 1);
        }
        if (((player.hasItem(useables.COP_ORE, 5) && player.hasItem(useables.TIN_ORE, 5)) || (Crafting.BagSlot01 > 4 && Crafting.BagSlot02 > 4)) && Crafting.BagSlot03 <= (Crafting.BagSlot03Cap - 5))
        {
            addButton(1, "Five", KonstantinSmeltingMenuProcess, "copper and tin", 2, true);
        }
    ...
    private function KonstantinSmeltingMenuEbonbloom() : Void
    {
        ...
        if (player.hasItem(useables.EBONBLO) || Crafting.BagSlot05 > 0)
        {
            addButton(0, "One", KonstantinSmeltingMenuProcess, "ebonbloom", 3);
        }
        if ((player.hasItem(useables.EBONBLO, 5) || Crafting.BagSlot05 > 4) && Crafting.BagSlot06 <= (Crafting.BagSlot06Cap - 5))
        {
            addButton(1, "Five", KonstantinSmeltingMenuProcess, "ebonbloom", 4, true);
        }
        addButton(14, "Back", KonstantinSmeltingMenu);
    }
    ...
    private function KonstantinSmeltingMenuProcess(material : String = "", craft : Float = 1, amount : Bool = false) : Void
    {
        ...
        if (amount)
        {
            outputText(" Of course, I would need more of it for armor or weapons but for now it'd do a great job for smaller items. I could still make arrowheads, or…</i>\" He scratches his chin in thought.\n\n");
        }
        else
        {
            outputText("</i>\"\n\n");
        ...
        switch (craft)
        {
            ...
            case 3:  //1 Ebonbloom  
                if (Crafting.BagSlot05 > 0)
                {
                    Crafting.BagSlot05 -= 1;
                }
                else
                {
            ...
            case 4:  //5 Ebonblooms  
                if (Crafting.BagSlot05 > 4)
                {
                    Crafting.BagSlot05 -= 5;
                }
                else
                {
    ...
    public function KonstantinSexMenu() : Void
    {
        ...
        addButton(4, "Hot Spring Fuck", KonstantinSexMenuHotSpringFuck).disableIf(flags[kFLAGS.CAMP_UPGRADES_HOT_SPRINGS] < 4, "You don't have a hot spring!");
        addButton(14, "Back", KonstantinMainCampMenu);
    }
    public function KonstantinSexMenuGiveBJ() : Void
    {
        ...
        {
            outputText(" At your backside, your [tail] twitches happily at the meal that you’re getting, and your [asshole] feels needy and eager to have a big cock to fill it.");
        }
    ...
    public function KonstantinSexMenuReciveBJ() : Void
    {
        ...
        {
            outputText("it’s hard to look at this monster without having the desire of choke on it");
        }
        else if (player.cocks[x].cockLength >= 6)
        {
            outputText("a good slab of meat like this is deliciously tempting");
        }
        else
        {
            outputText("no matter its size, this thing seems juicy and tasty");
        }
        ...
        if (player.hasBalls())
        {
            outputText(" From time to time, his hands grasp and caress your nuts, enticing the cum sloshing inside them.");
        }
        ...
        if (player.cocks[x].cockLength >= 12)
        {
            outputText("most of your cock in his mouth");
        }
        else
        {
            outputText("your whole cock in his mouth");
        }
        outputText(", ");
        if (player.hasBalls())
        {
            outputText("the tip of his tongue playing with your balls");
        }
        else
        {
            outputText("the tip of his tongue teasing your nethers");
        }
    ...
    public function KonstantinSexMenu69() : Void
    {
        ...
        if (player.hasMuzzle())
        {
            outputText("your snout");
        }
        else
        {
            outputText("your lips");
        }
        ...
        {
            outputText("much larger");
        }
        else
        {
            outputText("mammoth of a");
        }
        outputText(" cock. ");
        if (player.hasBalls())
        {
            outputText("You feel his huge set of fuzzy nuts rubbing against your own");
        }
        else
        {
            outputText("You feel his huge set of fuzzy nuts rubbing against your taint");
        }
        ...
        {
            outputText(". Your [wings] flap softly, as you’re being subjected to the bear rough oral affections.");
        }
        ...
        {
            outputText(", albeit with some difficulty due the sheer girth of your combined erections");
        }
        ...
        {
            outputText("tousling your [hair]");
        }
        else
        {
            outputText("caressing your shoulder");
        }
        ...
        if (player.hasBalls())
        {
            outputText("grope your testicles");
        }
        else
        {
            outputText("tickle your taint [skin]");
        }
        ...
        if (player.cumQ() > 1500)
        {
            outputText("He gulps down you seed with a bit of difficulty as your torrent of cums never seems to subside. When it finally does, the bear has gotten a little rounder from all the cum that he has drinked.");
        }
        else
        {
            outputText("He gulps down your seed with ease, not letting any drop to waste. Once you’re done, he gives your cockhead some playful slurps.");
        }
    ...
    public function KonstantinSexMenuReciveAnal() : Void
    {
        ...
        if (player.hasCock())
        {
            ...
            else if (player.biggestCockLength() >= 12 && player.biggestCockLength() < 20)
            {
                outputText("surpassing");
            }
            else
            {
                outputText("dwarfing yours with ease");
            }
            outputText(", while his copious pre soaks your nethers");
        }
        if (player.hasVagina() && !player.hasCock())
        {
            outputText("his gigantic member is teasingly rubbing your aching lips, as Konstantin’s copious pre soaks your nethers");
        }
        if (player.gender == 0)
        {
            outputText(" you can feel his erection rubbing your nethers, Konstantin’s copious pre soaking your [skin]");
        }
        ...
        {
            outputText("Your experienced backside has nothing but pleasure at feeling that familiar sensation of fullness, and Konstantin has no trouble in slipping a second one, earning a lusty moan from you, as the bear fingers and plays with your pucker.");
        }
        else
        {
            outputText("Your tight asshole has a bit of trouble accommodating the large finger, so Konstantin goes easy at first, loosening it carefully, until you feel comfortable. Once he thinks that your backdoor is roomy enough, he slips a second finger, earning a lusty moan from you, as the bear fingers and plays with your pucker.");
        }
        outputText(".\n\n\"<i>Please</i>\" you say, ");
        if (player.hasVagina() && player.hasCock())
        {
            outputText("your [cock] leaking pre in excitement and your cunt aching from the arousal");
        }
        else if (player.hasVagina())
        {
            outputText("your cunt aching fom the arousal ");
        }
        else
        {
            outputText("your [cock] leaking pre in excitement ");
        }
        ...
        if (player.tailType > 0)
        {
            outputText(" Your [tail] goes limp at the sudden intrusion.");
        }
        ...
        {
            outputText("neglected pussy, having endured enough, wets itself, girlcum soaking your thighs");
        }
        else
        {
            outputText("poor prostate is smashed by the gigantic penis violating your backside, and your own [cock], having endured enough, erupts into a messy orgasm, covering your [skin] ");
    ...
    public function KonstantinSexMenuHotSpringFuck() : Void
    {
        ...
        if (camp.getCampPopulation() >= 3)
        {
            outputText("We’re among friends, and I’m pretty sure that they’ll take no mind if we stroll naked. Is not if we haven’t seen each other naked before");
        }
        else
        {
            outputText("There is no one here besides us");
        }
        outputText(".</i>\"\n\n");
        ...
        {
            outputText("Albeit a bit reluctantly");
        }
        ...
        if (camp.getCampPopulation() >= 3)
        {
            outputText(" quite tranquil and pleasant. The few of your companions that you meet out either give you a mischievous wink or simply give you a friendly wave. Having the soft, fresh winds of Mareth hitting your bodies only adds to the relaxing walk");
        }
        else
        {
            outputText(", chatting with Konstantin all the while, the soft winds of Mareth caress  your naked bodies pleasantly");
        }
        ...
        if (player.hasVagina() && player.hasCock())
        {
            outputText("Your own arousal gets a spike from this, your [cock] exploding in a cummy mess that covers your nethers and chest and your [vagina] soaking itself in girlcum");
        }
        else if (player.hasVagina())
        {
            outputText("Your own arousal gets a spike from this, your [vagina] soaking itself in girlcum");
        }
        else
        {
            outputText("Your own arousal gets a spike from this, your [cock] exploding in a cummy mess that covers your nethers and chest");
        }
