...
class ZenjiScenes extends NPCAwareContent implements SaveableState
{
    public static var Z1stKid : String;
    public static var Z2ndKid : String;
    public static var ZenjiNightWatch : Float;
    public static var ZenjiFood : Bool;
    public static var ZenjiMarae : Bool;
    public static var ZenjiHolli : Bool;
    public static var ZenjiMarried : Bool;
    public static var ZenjiFigMulti : Float;
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            Z1stKid = Reflect.field(o, "Z1stKid");
            Z2ndKid = Reflect.field(o, "Z2ndKid");
            ZenjiNightWatch = Reflect.field(o, "ZenjiNightWatch");
            ZenjiFood = Reflect.field(o, "ZenjiFood");
            ZenjiMarae = Reflect.field(o, "ZenjiMarae");
            ZenjiHolli = Reflect.field(o, "ZenjiHolli");
            ZenjiMarried = Reflect.field(o, "ZenjiMarried");
            ZenjiFigMulti = valueOr(Reflect.field(o, "ZenjiFigMulti"), 0);
            ZenjiSleepCount = valueOr(Reflect.field(o, "ZenjiSleepCount"), 0);
            ZenjiLoverDays = valueOr(Reflect.field(o, "ZenjiLoverDays"), 0);
            ZenjiLoverDaysTracker = valueOr(Reflect.field(o, "ZenjiLoverDaysTracker"), 0);
            ZenjiTalkCount = valueOr(Reflect.field(o, "ZenjiTalkCount"), 0);
        }
    ...
    public function zenjiPerspectiveOnPlayer(changes : Float = 0) : Float
    {
        if (flags[kFLAGS.ZENJI_PROGRESS] < 7)
        {
            flags[kFLAGS.ZENJI_PERSPECTIVE_ON_PLAYER] += changes;
        }
        if (flags[kFLAGS.ZENJI_PERSPECTIVE_ON_PLAYER] > 100)
        {
            flags[kFLAGS.ZENJI_PERSPECTIVE_ON_PLAYER] = 100;
        }
        if (flags[kFLAGS.ZENJI_PERSPECTIVE_ON_PLAYER] < 0)
        {
    ...
    public function part1TrollEncounter() : Void
    {
        ...
        {
            addButtonDisabled(0, "Pay w Gems", "You don't have enough gems!");
        }
    ...
    public function part1TrollEncounterFightZenjiDefeated(hpVictory : Bool) : Void
    {
        clearOutput();
        if (hpVictory)
        {
            ...
            {
                addButton(1, "Hunt him", part1TrollEncounterFightZenjiDefeatedHuntHim).hint("He’s not getting away that easily!");
            }
            else
            {
                addButtonDisabled(1, "???", "Need 50+ corruption.");
            }
    ...
    public function part1TrollEncounterFightTOTHEDEATHZenjiDefeated(hpVictory : Bool) : Void
    {
        clearOutput();
    ...
    {
        var event : Float = rand(10);
        var itype : ItemType;
        if (event < 2)
        {
            itype = consumables.REPTLUM;
        }
        else if (event < 4)
        {
            itype = consumables.REDUCTO;
        }
        else if (event < 6)
        {
        ...
        {
            itype = consumables.COAL___;
        }
        else if (event < 10)
        {
            itype = useables.T_SSILK;
        }
    ...
    public function part2TrollEncounterFirst() : Void
    {
        ...
        if (player.cor >= 50 + player.corruptionTolerance)
        {
            outputText("enji gives you a sideways glance, almost as if he doesn't trust you. \"<i>If dat is de truth, it's nice ta meet you, [name]</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Ahah, [name], so dat is your name, a pleasure ta get to really know ya.</i>\" He replies.\n\n");
        }
    ...
    public function part2TrollEncounterFirstFightZenjiDefeated(hpVictory : Bool) : Void
    {
        spriteSelect(SpriteDb.s_zenji);
        clearOutput();
        if (hpVictory)
    ...
    public function part2TrollEncounterRepeat() : Void
    {
        ...
        {
            outputText("\"<i>Ah, [name],</i>\" he welcomes, \"<i>What brings ya here? Didja want to train or perhaps go for anoda sparring match?</i>\"\n\nYou consider the few options, it appears he doesn’t want to talk about anything else.\n\n");
        }
        menu();
        ...
        if (flags[kFLAGS.ZENJI_PERSPECTIVE_ON_PLAYER] <= 30)
        {
            addButton(3, "Talk", part2TrollEncounterTalk);
        }
    ...
    public function part2TrollEncounterRepeatFightPCDefeated() : Void
    {
        ...
        if (player.HP <= player.minHP())
        {
            if (flags[kFLAGS.ZENJI_PERSPECTIVE_ON_PLAYER] < 30)
            {
                outputText("\"<i>[name]... Are you okay..? Would it help if I went easier? You don’ have ta fight me if you aren’t ready… Please, get some rest.</i>\"\n\n");
            }
            else
            {
                outputText("\"<i>Das all ya got?</i>\" Zenji shakes his head, \"<i>Ya gotta try harder dan dat next time. Ya be safe out dere now.</i>\"\n\n");
            }
            zenjiPerspectiveOnPlayer(-4);
        }
        else
        {
            ...
            {
                outputText("\"<i>Uh… Geez, I’ve been told I am handsome by de chameleon girl, but...</i>\" Zenji shakes his head, \"<i>Yeah, Im gonna act like dis didn’t happen.</i>\"\n\n");
            }
    ...
    public function part2TrollEncounterRepeatFightZenjiDefeated(hpVictory : Bool) : Void
    {
        spriteSelect(SpriteDb.s_zenji);
        clearOutput();
        if (hpVictory)
    ...
    public function part2TrollEncounterTrainStrength() : Void
    {
        ...
        if (player.fatigue > player.maxOverFatigue() * 0.5)
        {
            ...
            if (flags[kFLAGS.ZENJI_PERSPECTIVE_ON_PLAYER] < 30)
            {
                outputText("\"<i>Are... are sure you’re okay? Dere's... You don' have ta train wit me if ya don' wanna. Dere's oda ways to...</i>\" Zenji shakes his head, \"<i>Ya should go home and get some rest, I'll tink of sometin lata.</i>\"\n\n");
            }
            else
            {
                outputText("\"<i>Das all ya got? Maybe dere's an easier ting I got next time, ya should go home and rest for a moment so I can prepare for next time.</i>\"\n\n");
            }
    ...
    public function part2TrollEncounterTrainToughness() : Void
    {
        ...
        if (player.fatigue > player.maxOverFatigue() * 0.5)
        {
            ...
            {
                outputText("\"<i>Dat’s all ya got? Maybe dere's an easier ting I got next time, ya should go home and rest for a moment so I can prepare for next time.</i>\"\n\n");
            }
            player.fatigue += Math.round(player.maxOverFatigue() * 0.2);
            zenjiPerspectiveOnPlayer(-3);
        }
    ...
    public function followerZenjiMainCampMenu() : Void
    {
        ...
        if (player.lust > 33)
        {
            addButton(2, "Sex", followerZenjiSex).hint("Perhaps the hunk could be open to share an intimate moment with you.");
        }
        if (CorruptedGlade.canBeDestroyed())
        {
            addButton(3, "Glades", followerZenjiGlades).hint("Ask Zenji for help in destroying the corrupted glades.");
        }
        ...
        {
            addButton(5, "Spar", followerZenjiSpar).hint("Spar with Zenji.");
        }
        else
        {
            addButtonDisabled(5, "Spar", "You can spar with him, but you need to build a sparring ring first!");
        }
    ...
    public function followerZenjiMainCampMenuTrainingStrength() : Void
    {
        ...
        if (player.statusEffectv1(StatusEffects.ZenjiTrainingsCounters1) < 3)
        {
            player.addStatusValue(StatusEffects.ZenjiTrainingsCounters1, 1, 1);
        }
    ...
    public function followerZenjiMainCampMenuTrainingToughness() : Void
    {
        ...
        if (player.statusEffectv2(StatusEffects.ZenjiTrainingsCounters1) < 3)
        {
            player.addStatusValue(StatusEffects.ZenjiTrainingsCounters1, 2, 1);
        }
    ...
    public function followerZenjiMainCampMenuTrainingSpeed() : Void
    {
        ...
        outputText("Once you’re done, you feel that the exercise was worth the time. You thank Zenji before dismissing yourself.\n\n");
        if (player.statusEffectv3(StatusEffects.ZenjiTrainingsCounters1) < 3)
        {
            player.addStatusValue(StatusEffects.ZenjiTrainingsCounters1, 3, 1);
        }
    ...
    public function followerZenjiMainCampMenuTrainingIntelligence() : Void
    {
        ...
        if (player.statusEffectv1(StatusEffects.ZenjiTrainingsCounters2) < 3)
        {
            player.addStatusValue(StatusEffects.ZenjiTrainingsCounters2, 1, 1);
        }
    ...
    public function followerZenjiMainCampMenuTrainingWisdom() : Void
    {
        ...
        if (player.statusEffectv2(StatusEffects.ZenjiTrainingsCounters2) < 3)
        {
            player.addStatusValue(StatusEffects.ZenjiTrainingsCounters2, 2, 1);
        }
    ...
    public function followerZenjiMainCampMenuTrainingPerks() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.ZenjiTrainingsCounters2, 3, 1);
        }
        if (player.statusEffectv3(StatusEffects.ZenjiTrainingsCounters2) == 5 || player.statusEffectv3(StatusEffects.ZenjiTrainingsCounters2) == 15 || player.statusEffectv3(StatusEffects.ZenjiTrainingsCounters2) == 30)
        {
            outputText("After you're done training, you feel different, you can't explain it, but something has surely changed, almost as if the training you've done with Zenji is really speaking to you.\n\n");
        }
    ...
    public function followerZenjiSparZenjiDefeated(hpVictory : Bool) : Void
    {
        spriteSelect(SpriteDb.s_zenji);
        clearOutput();
        if (hpVictory)
    ...
    public function followerZenjiTalks() : Void
    {
        ...
        {
            if (TrollVillage.ZenjiVillageStage < 4)
            {
                addButton(3, "His Problems", zenjiVillageProblems);
            }
            else
            {
                addButton(3, "His Problems", zenjiVillageProblems2);
            }
        }
    ...
    public function loverZenjiOfferYes() : Void
    {
        ...
        {
            outputText(", picking you up and wrapping you in his warm, fuzzy embrace");
        }
        else if (player.tallness < 68)
        {
            outputText(" to you and wraps you within his warm, fuzzy embrace, resting his chin atop your head");
        }
        else
        {
            outputText(" to you and wraps you within his warm fuzzy embrace");
        }
    ...
    public function loverZenjiMainCampMenu() : Void
    {
        ...
        if (player.statusEffectv1(StatusEffects.ZenjiModificationsList) > 23)
        {
            ...
            var reset : Float = player.statusEffectv1(StatusEffects.ZenjiModificationsList);
            player.addStatusValue(StatusEffects.ZenjiModificationsList, 1, -reset);
        }
        else if (player.lust > player.maxLust() * 0.75)
        {
        ...
        else if (player.pregnancyType == PregnancyStore.PREGNANCY_ZENJI || player.pregnancy2Type == PregnancyStore.PREGNANCY_ZENJI)
        {
        ...
        {
            outputText("You approach Zenji and he looks down at you, raising an eyebrow, \"<i>¿Si, " + ((player.thickness <= 50) ? "" + player.mf("flaco", "flaca") + "" : "gordita") + "?</i>\"\n\n");
        }
        menu();
        ...
        if (CorruptedGlade.canBeDestroyed())
        {
            addButton(3, "Glades", loverZenjiGlades).hint("Have Zenji help you in destroying the corrupted glades.");
        }
        ...
        {
            addButton(5, "Assist Me", zenjiHenchmanOption);
        }
        else
        {
            addButtonDisabled(5, "Assist Me", "You need to have at least Basic Leadership to form a team.");
        }
        addButton(6, "Give Item", loverZenjiGiveItem);  //.hint("Talk to Zenji.")  
        if (flags[kFLAGS.SLEEP_WITH] != "Zenji")
        {
            addButton(7, "Sleep With", zenjiSleepToggle).hint("Spend your nights with Zenji.");
        }
        else
        {
            addButton(7, "Sleep Alone", zenjiSleepToggle).hint("Stop sleeping with Zenji.");
        }
    ...
    public function loverZenjiMainCampMenuAppearance() : Void
    {
        ...
        {
            outputText("7.5 inches long and 2.2");
        }
        if (player.statusEffectv3(StatusEffects.ZenjiModificationsList) == 24)
        {
            outputText("12 inches long and 2.4");
        }
        if (player.statusEffectv3(StatusEffects.ZenjiModificationsList) == 32)
        {
            outputText("16 inches long and 2.6");
        }
        ...
        {
            outputText(" He produces so much cum now that he is permanently leaking a steady stream of cum from his tip. Somehow you know a part of him is loving every moment of the lewdness.");
        }
    ...
    public function loverZenjiTalks() : Void
    {
        ...
        {
            addButton(3, "Children", loverZenjiTalksChildren).hint("Talk to Zenji about your children.");
        }
    ...
    public function loverZenjiTalksHimself() : Void
    {
        ...
        outputText("Zenji spends the rest of the hour talking about new experiences in the camp and other things about himself that he can recall.\n\n");
        if (player.statusEffectv1(StatusEffects.ZenjiPreparationsList) < 15)
        {
            player.addStatusValue(StatusEffects.ZenjiPreparationsList, 1, 1);
        }
        ZenjiTalkCount++;
        endEncounter();
    }
    ...
    public function loverZenjiGiveItem() : Void
    {
        ...
        menu();
        if (player.hasItem(consumables.CLEARLN) || player.hasItem(consumables.ROUGHLN) || player.hasItem(consumables.SEXY_LN) || player.hasItem(consumables.SMTH_LN))
        {
            addButton(0, "Lotion", loverZenjiGiveItemLotion).hint("Give him some lotion to smooth out his cozy fur.");
        }
        else
        {
            addButtonDisabled(0, "Lotion", "Need 1 of any Lotion bought at Rathazul shop. (wip tooltip)");
        }
        if (player.hasItem(consumables.GROPLUS))
        {
            addButton(1, "GroPlus", loverZenjiGiveItemGroPlus).hint("Increase the size of certain parts.");
        }
        else
        {
            addButtonDisabled(1, "GroPlus", "You do not have this item to offer.");
        }
        if (player.hasItem(consumables.REDUCTO))
        {
            addButton(2, "Reducto", loverZenjiGiveItemReducto);
        }
        else
        {
            addButtonDisabled(2, "Reducto", "You do not have this item to offer.");
        }
        if (player.hasItem(consumables.TROLFIG))
        {
            addButton(3, "TrollFig", loverZenjiGiveItemTrollFig).hint("Give Zenji a troll fig, he must like them, right?");
        }
        else
        {
            addButtonDisabled(3, "TrollFig", "You do not have this item to offer");
        }
    ...
    public function loverZenjiGiveItemLotion() : Void
    {
        spriteSelect(SpriteDb.s_zenji);
        clearOutput();
        if (player.hasItem(consumables.CLEARLN))
        {
            player.consumeItem(consumables.CLEARLN);
        }
        else if (player.hasItem(consumables.ROUGHLN))
        {
            player.consumeItem(consumables.ROUGHLN);
        }
        else if (player.hasItem(consumables.SEXY_LN))
        {
            player.consumeItem(consumables.SEXY_LN);
        }
        else if (player.hasItem(consumables.SMTH_LN))
        {
            player.consumeItem(consumables.SMTH_LN);
        }
    ...
    public function loverZenjiGiveItemGroPlusPenis() : Void
    {
        ...
        {
            outputText("Zenji gives a small chuckle, \"<i>Of course dat's what you'd want, Alright, so be it.</i>\"\n\n");
        }
        if (player.statusEffectv3(StatusEffects.ZenjiModificationsList) == 32)
        {
        ...
        else
        {
            ...
            if (player.statusEffectv3(StatusEffects.ZenjiModificationsList) == 15)
            {
                outputText("\"<i>Happy?</i>\" He asks, looking softly into your eyes, \"<i>Dis be some weird stuff, but I don't care, I just wanna be there fa ya.</i>\" He pulls back his loincloth over his length, \"<i>I suppose you'll find out how much ya like it next time we get closer... But for now, I just want ta be left alone, dis doesn’t feel right, but I know it'll be fine in a bit.</i>\"\n\n");
            }
            else
            {
                outputText("Zenji’s brow furrows as he bites his lower lip in slight disgust, \"<i>I hope you’re happy with da choices you’ve made.</i>\" He shakes his head softly, scolding himself slightly. \"<i>Please, leave me alone, I need… I need some time ta myself… Sorry for de outburst.</i>\"\n\n");
            }
            ...
            if (player.statusEffectv3(StatusEffects.ZenjiModificationsList) == 15)
            {
                player.addStatusValue(StatusEffects.ZenjiModificationsList, 3, 9);
            }
            else
            {
                player.addStatusValue(StatusEffects.ZenjiModificationsList, 3, 8);
            }
    ...
    public function loverZenjiGiveItemGroPlusTusks() : Void
    {
        ...
        {
            outputText("Zenji shakes his head, \"<i>Now, now [name], I know deep down dat ma tusks are fine de way dey are, any bigger and it’ll hurt ma neck keeping it up all de time.</i>\"\n\n");
        }
        doNext(loverZenjiGiveItem);
    }
    ...
    public function loverZenjiGiveItemReductoTusks() : Void
    {
        ...
        {
            outputText("Zenji gives you an inquisitive look, \"<i>Now then, [name], I like my tusks. They’re pretty small, I’m not just gonna get rid of them.</i>\"\n\n");
        }
        doNext(loverZenjiGiveItem);
    }
    ...
    public function loverZenjiGiveItemTrollFig() : Void
    {
        ...
        {
            outputText("As he pulls away, Zenji looks down at his groin, \"<i>Aha, I didn’t expect someting like dis from you, [name]. Dat’s one of de weird, wild figs…</i>\" He chuckles softly before looking back at you, \"<i>Let’s just say you’ll be in for a little surprise later…</i>\"\n\n");
        }
        doNext(loverZenjiGiveItem);
    }
    ...
    public function loverZenjiSex() : Void
    {
        ...
        {
            outputText("\"<i>You’d like dat, wouldn’tcha, you want to be dominated by me. You need a big strong troll with you, don’t deny it. Gotta say, I’ve been resisting dese feelings for another man for some time now, nothing better than claiming a nice guy like you…</i>\"\n\n");
        }
        ...
        if ((player.isBiped() || player.isNaga()) && player.lib >= 85)
        {
            addButton(11, "Tease", loverZenjiSexTease).hint("Subdue Zenji beneath your heel (or tail) and make a mockery out of his lust ridden state.");
        }
        else if (player.lib < 85)
        {
            addButtonDisabled(11, "Tease", "You aren't lewd enough to consider mocking him right now.");
        }
        else
        {
            addButtonDisabled(11, "Tease", "You do not have the correct lower body for this scene.");
        }
        ...
        if (player.lust > 33)
        {
            ...
            if (player.hasVagina())
            {
                addButton(1, "Take Vaginal", loverZenjiTakeVaginal).hint("Have Zenji penetrate you vaginally.");
            }
            else
            {
                addButtonDisabled(1, "Take Vaginal", "You need a vagina for this scene.");
            }
            if (player.smallestCockArea() < 46)
            {
                addButton(2, "Pitch Anal", loverZenjiPitchAnal).hint("Penetrate Zenji anally");
            }
            else
            {
                addButtonDisabled(10, "Get Blown", "Your dick is too big for him.\n\nZenji is a big guy, but even he has his limits!");
            }
            if (player.isPregnant())
            {
                addButton(5, "Pregnant Sex", loverZenjiPregnantSex).hint("Zenji seems to eye your pregnant form with interest.");
            }
            else
            {
                addButtonDisabled(5, "Pregnant Sex", "You need to be pregnant for this.");
            }
            if (flags[kFLAGS.CAMP_UPGRADES_HOT_SPRINGS] >= 4)
            {
                addButton(6, "Hot Spring", loverZenjiHotSpring).hint("Enter the hot spring with Zenji.");
            }
            else
            {
                addButtonDisabled(6, "Hot Spring", "You have not yet built a Hot Spring.");
            }
            if (player.hasVagina())
            {
                addButton(7, "Get Fingered", loverZenjiSexGetFingered).hint("Have Zenji put his thick fingers to good use.");
            }
            else
            {
                addButtonDisabled(7, "Get Fingered", "You need a vagina for this scene.");
            }
            if (player.hasCock())
            {
                addButton(8, "Get Handjob", loverZenjiSexGetHandjob).hint("His hands are so luxuriously soft, put them to good use.");
            }
            else
            {
                addButtonDisabled(8, "Get Handjob", "You need a penis for this scene.");
            }
            if (player.statusEffectv4(StatusEffects.ZenjiModificationsList) == 1)
            {
                if (player.hasVagina())
                {
                    addButton(9, "Get Licked", loverZenjiSexGetLicked).hint("Have Zenji put that long tongue to good use.");
                }
                else
                {
                    addButtonDisabled(9, "Get Licked", "You need a vagina for this scene.");
                }
                if (player.hasCock())
                {
                    if (player.smallestCockArea() < 40)
                    {
                        addButton(10, "Get Blown", loverZenjiSexGetBlown).hint("Have Zenji put his mouth to good use.");
                    }
                    else
                    {
                        addButtonDisabled(10, "Get Blown", "Your cock is too big.");
                    }
                }
                else
                {
                    addButtonDisabled(10, "Get Blown", "You need a penis for this scene.");
                }
            }
            else
            {
    ...
    public function loverZenjiTakeAnal() : Void
    {
        ...
        if (player.ass.analLooseness == 0)
        {
            outputText("\"<i>We will go nice and slow, don’t worry…</i>\"\n\n");
        }
        else
        {
            outputText("\"<i>Don' worry, I will make dis special.</i>\"" + ((player.statusEffectv3(StatusEffects.ZenjiModificationsList) == 32) ? " Zenji’s brow furrows at you, \"<i>I can’t believe you’re dat eager to take me when I’m dis huge... I guess since you want me big like dis so bad I’ll just have ta give ya exactly what you want.</i>\"" : "") + "\n\n");
        }
        ...
        {
            outputText(" Despite the fact that Zenji is smaller than you, he still acts like he’s the one with complete control.");
        }
        if (player.tallness < 60)
        {
            outputText(" His massive stature completely encompasses you.");
        }
        ...
        {
            outputText("His lengthy shaft presses its way deep inside of your tight backdoor, you feel yourself stretch over his considerable manhood." + ((player.statusEffectv3(StatusEffects.ZenjiModificationsList) == 32) ? " Maybe making him so huge wasn’t the best idea." : "") + "\n\n");
        }
        else if (player.analCapacity() < 7)
        {
            outputText("His lengthy shaft presses into you, you can feel him accommodating your [ass] as he pushes further and further with each thrust.");
        }
        else
        {
            outputText("His lengthy shaft presses into you, you can feel him push into your roomy backside as he goes deeper into you with each thrust.");
        }
        ...
        if (player.hasCock())
        {
            outputText("bring your grasp down as you begin masturbating yourself");
        }
        else
        {
            outputText("rub your fingers against your clit and begin fingering yourself in tandem with his thrusts");
        }
        ...
        if (player.hasCock())
        {
            outputText("shooting ropes of cum all over the ground beneath you");
        }
        else
        {
            outputText(" girl cum leaking from your lips, sliding down your body");
        }
        ...
        if (player.statusEffectv2(StatusEffects.ZenjiModificationsList) >= 18700)
        {
            outputText("You can feel yourself inflate from the warm seed flowing into you. Zenj holds onto you tighter, forcing you to take as much as possible. His growls grow louder as you’re filled well beyond your limit, excess cum pouring out onto the bedroll beneath you.");
        }
        else if (player.statusEffectv2(StatusEffects.ZenjiModificationsList) >= 4700)
        {
            outputText("He is pumping so much cum into you, it leaks out as you feel your stomach distend slightly from the copious load he has to offer as the rest stills out onto the bedroll beneath you.");
        }
        else
        {
            outputText("His load is dense, you feel yourself being filled by the sheer amount he has to offer.\n\n");
        }
    ...
    public function loverZenjiTakeVaginal() : Void
    {
        ...
        if (player.tallness < 60)
        {
            outputText("He wraps his toned arms around your form, cradling you within his massive presence as he pulls you up to his face for a gentle kiss.");
        }
        else if (player.tallness < 108)
        {
            outputText("He wraps his toned arms around your [skin] and pulls you in for a kiss, his tusks barely touch your cheeks as his tongue explores your mouth.");
        }
        else
        {
            outputText("He attempts to wrap his toned arms around your massive body as you lean down to kiss him, allowing his tongue to explore your mouth.");
        }
        ...
        else if (player.vaginalCapacity() < 80)
        {
        ...
        {
            outputText("His manhood presses into your moist snatch, gently digging his way into you. It doesn't take long before he starts bucking into you.\n\n");
        }
        ...
        if (player.hasCock())
        {
            outputText(" You finally cum, spewing jizz all over his muscular chest.");
        }
        else
        {
            outputText("You finally climax as your girlcum slowly leaks out of your vagina, further lubricating him.");
        }
        ...
        {
            outputText("His load is tremendous, you couldn’t hope to contain all the seed he’s unleashing as your stomach distends slightly as the rest spills out onto the bedroll beneath you.");
        }
        else
        {
            outputText("His load is dense, you feel yourself being filled by all the cum he’s pumping into you.");
        }
        ...
        {
            player.knockUp(PregnancyStore.PREGNANCY_ZENJI, PregnancyStore.INCUBATION_ZENJI);
        }
        endEncounter();
    }
    ...
    public function loverZenjiPitchAnal() : Void
    {
        ...
        {
            outputText("Zenji glances around slightly nervously, \"<i>Sure… I won’t say dat I don’t like it.</i>\"\n\n");
        }
        else
        {
        ...
        if (player.tallness < 60)
        {
            outputText("You follow behind him, mounting his massive body to the best of your abilities while getting a good grasp of his frame. Your erection rises to full mast as you align it to his rear.");
        }
        else if (player.tallness < 108)
        {
            outputText("You follow behind him, your manhood already reaching full length as you align yourself with his rear. You ask him if he’s ready, he gives a quiet nod. You take a firm grasp on his muscular rear.");
        }
        else
        {
            outputText("Even kneeling, your sheer size towers over him. You coax your erection to full mast before aligning it to his rear.");
        }
    ...
    public function loverZenjiSexBlowHim() : Void
    {
        ...
        if (player.statusEffectv2(StatusEffects.ZenjiModificationsList) >= 3700)
        {
            ...
            if (silly())
            {
                outputText("You have created a monster, a ravishingly handsome, virile monster! ");
            }
        ...
        else
        {
            ...
            if (player.lib < 100)
            {
                outputText("Unfortunately his load is too much for you to take in. It escapes from your mouth onto the ground beneath you.");
            }
            else
            {
                outputText("You happily swallow every last drop of seed he pours into you.");
            }
        }
    ...
    public function loverZenjiPregnantSex() : Void
    {
        ...
        if (player.vaginalCapacity() < 10)
        {
            outputText("His manhood presses into your tight snatch, gently stretching out your walls. Despite being pregnant, he still feels huge inside of you." + ((player.statusEffectv3(StatusEffects.ZenjiModificationsList) == 32) ? " He is massive, you grow concerned for your unborn child." : "") + "\n\n");
        }
        else if (player.vaginalCapacity() < 80)
        {
            outputText("His manhood presses into your moist snatch, gently pressing his way into you. He feels so good inside you as he presses deeper, pushing against your walls, his pace slow and gentle.\n\n");
        }
        else
        {
    ...
    public function loverZenjiHotSpring() : Void
    {
        ...
        if (player.isNaked())
        {
            outputText("You slide into the water beside him.\n\n");
        }
        else
        {
        ...
        if (player.hasVagina())
        {
            addButton(3, "Ride Vaginal", loverZenjiHotSpringRideVaginal);
        }
        else
        {
            addButtonDisabled(3, "Ride Vaginal", "You need a vagina for this scene.");
        }
    }
    ...
    public function loverZenjiHotSpringRideAnal() : Void
    {
        ...
        if (player.analCapacity() < 7)
        {
            outputText("His lengthy shaft presses its way deep inside of your tight anus, you feel yourself stretch over his considerable manhood." + ((player.statusEffectv3(StatusEffects.ZenjiModificationsList) == 32) ? " The water makes it a bit easier to adjust to his intense size." : "") + "\n\n");
        }
        else if (player.analCapacity() < 56)
        {
        ...
        {
            outputText("His lengthy shaft presses into you, you can feel him push into your roomy backside as he presses deeper into you with each thrust.\n\n");
        }
        ...
        if (player.gender == 0)
        {
            outputText("tenderly allows his hand to roam along your barren taint as his soft fur sends tingles up your spine to which you reflexively press against him");
        }
        else
        {
            outputText("reaches his hand down to your loins, " + ((player.hasCock()) ? "gently stroking your shaft" : "gently reaching his finger down to your netherlips, teasing your clit and probing your vagina") + "");
        }
        ...
        if (player.gender == 0)
        {
            outputText("You press yourself tightly against him, trying to feel as much of his body against yours");
        }
        else if (player.hasCock())
        {
            outputText("Girlcum drips out past your lips, mixing into the bubbling water");
        }
        else
        {
            outputText("Girlcum drips out past your lips, mixing into the bubbling water");
    ...
    public function loverZenjiHotSpringRideVaginal() : Void
    {
        ...
        {
            outputText("His manhood presses into your tight snatch, gently stretching out your walls. He is so big, but you need him now even more." + ((player.statusEffectv3(StatusEffects.ZenjiModificationsList) == 32) ? "The water makes it easier to adjust to his intense size." : "") + "\n\n");
        }
        else if (player.vaginalCapacity() < 79)
        {
            outputText("His manhood presses into your moist snatch, gently pressing his way into you. He feels so good inside you as he presses deeper, his pace slow and gentle.\n\n");
        }
        else
        {
            outputText("His manhood presses into your moist snatch, gently pressing his way into you. While he may not be the biggest you've had, he is skilled and you couldn't wish for anyone better.\n\n");
    ...
    public function loverZenjiSexGetFingered() : Void
    {
        ...
        if (!player.isNaked())
        {
            outputText("You pull down your [armor], giving Zenji room to finger you.\n\n");
        }
        ...
        if (!player.isNaked())
        {
            outputText("Zenji helps pull your clothes back up once you’ve recovered from your high.\n\n");
        }
    ...
    public function loverZenjiSexGetHandjob() : Void
    {
        ...
        if (!player.isNaked())
        {
            outputText("You pull down your [armor], giving Zenji room to give you a handjob.\n\n");
        }
    ...
    public function loverZenjiSexGetBlown() : Void
    {
        ...
        {
            outputText("His eyebrows furrow for a brief moment before he swallows what you have to offer.\n\n");
        }
        outputText("Zenji stands up, ");
        if (player.tallness > 82)
        {
            outputText("reaching eye level with you");
        }
        else if (player.tallness > 60)
        {
            outputText("his height towering over you");
        }
        else
        {
            outputText("picking you up with him");
        }
    ...
    public function loverZenjiSleepWithGetPenetrated(timeQ : Float) : Void
    {
        spriteSelect(SpriteDb.s_zenji);
        clearOutput();
        ...
        if (player.vaginalCapacity() <= 9)
        {
            outputText("tight snatch, gently stretching out your walls. He feels so big inside you as he makes his way into you." + ((player.statusEffectv3(StatusEffects.ZenjiModificationsList) == 32) ? " Maybe making him so huge wasn’t the best idea." : "") + "");
        }
        else if (player.vaginalCapacity() < 79)
        {
            outputText("moist snatch, gently pressing his way into you. He feels so good inside you as he pushes deeper, gently bucking into you.");
        }
        else
        {
            outputText("moist snatch, gently pressing his way into you. While he may not be the biggest you've had, he is skilled and you couldn't wish for anyone better.\n\n");
        }
        ...
        if (player.gender == 0)
        {
            outputText("slowly fondle your body as you blow Zenji");
        }
        else
        {
            outputText("" + ((player.hasCock()) ? "stroke at your length as you blow him, trying to get the most out of the situation" : "gently finger yourself, playing with your clit and probing your box") + "");
        }
        ...
        if (player.statusEffectv2(StatusEffects.ZenjiModificationsList) >= 18700)
        {
            outputText("can feel yourself inflate from the warm seed flowing into you. Zenj holds onto you tighter, forcing you to take as much as possible. His growls grow louder as you’re filled well beyond your limit, excess cum pouring out onto the bed beneath you. Somehow the mess doesn’t concern you too much");
        }
        else if (player.statusEffectv2(StatusEffects.ZenjiModificationsList) >= 4700)
        {
            outputText("can feel his load surge through your womb, tremendous as ever, filling you up past your limits as your stomach bulges outward, spilling out onto the bed beneath you as well");
        }
        else
        {
            outputText("feel the warmth of his seed surge inside you, being filled by his warm, gooey love");
        }
    ...
    public function loverZenjiSleepWithCatchAnal(timeQ : Float) : Void
    {
        spriteSelect(SpriteDb.s_zenji);
        clearOutput();
        outputText("You tell Zenji that you want to spend the night closer to him" + ((player.isNaked()) ? "" : " you say as you take off your underwear") + ".\n\n");
        ...
        if (player.analCapacity() < 7)
        {
            outputText("His lengthy shaft presses its way deep inside of your tight anus, you feel yourself stretch over his considerable manhood" + ((player.statusEffectv3(StatusEffects.ZenjiModificationsList) == 32) ? " Maybe making him so huge wasn’t the best idea" : "") + ".\n\n");
        }
        else if (player.analCapacity() < 56)
        {
            outputText("His lengthy shaft presses into you, you can feel him accommodating your [ass] as he presses further and further with each thrust.\n\n");
        ...
        if (player.gender == 0)
        {
            outputText(" gently fondling the underside of your taint");
        }
        else
        {
            outputText("" + ((player.hasCock()) ? "gently grasping your throbbing shaft as he strokes it for" : "") + (((player.hasVagina() && !player.hasCock())) ? "gently teasing your clit as he slowly fingers" : "") + " you, his tail joins in the fun as well");
        }
        ...
        if (player.statusEffectv2(StatusEffects.ZenjiModificationsList) >= 18700)
        {
            outputText("You can feel yourself inflate from the warm seed flowing into you. Zenj holds onto you tighter, forcing you to take as much as possible. His growls grow louder as you’re filled well beyond your limit, excess cum pouring out onto the bed beneath you. Somehow the mess doesn’t concern you too much");
        }
        else if (player.statusEffectv2(StatusEffects.ZenjiModificationsList) >= 4700)
        {
            outputText("So much cum is flooding into your back door, to the point where your stomach distends slightly as the rest pours out onto the bed beneath you");
        }
        else
        {
            outputText("You feel the warmth of his seed race inside you, being filled by the sheer amount he has to offer");
        }
    ...
    public function loverZenjiSleepWithNoSex(timeQ : Float) : Void
    {
        spriteSelect(SpriteDb.s_zenji);
        clearOutput();
        ...
        {
            outputText(" You feel his tail coil around you, you reflexively bring your [tail] to tangle with his, the two of you locking tails with each other.");
        }
        else
        {
            outputText(" You can feel his tail fondly stroke your body with tender caresses.");
        }
    ...
    public function zenjiHenchmanOption() : Void
    {
        ...
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "Zenji" || flags[kFLAGS.PLAYER_COMPANION_3] == "Zenji")
            {
                addButtonDisabled(0, "Assist Me (1)", "You already have Zenji accompany you.");
            }
            else
            {
                addButton(0, "Assist Me (1)", zenjiHenchmanOption2, false, 1).hint("Ask Zenji to join you in adventures outside camp.");
            }
        }
        else if (flags[kFLAGS.PLAYER_COMPANION_1] == "Zenji")
        {
            addButton(5, "Assist Me (1)", zenjiHenchmanOption2, false, 21).hint("Ask Zenji to stay in camp.");
        }
        else
        {
            addButtonDisabled(5, "Assist Me (1)", "You already have other henchman accompany you as first party member. Ask him/her to stay at camp before you talk with Zenji about accompaning you as first party member.");
        }
        if (player.hasPerk(PerkLib.IntermediateLeadership))
        {
            if (flags[kFLAGS.PLAYER_COMPANION_2] == "")
            {
                if (flags[kFLAGS.PLAYER_COMPANION_1] == "Zenji" || flags[kFLAGS.PLAYER_COMPANION_3] == "Zenji")
                {
                    addButtonDisabled(1, "Assist Me (2)", "You already have Zenji accompany you.");
                }
                else
                {
                    addButton(1, "Assist Me (2)", zenjiHenchmanOption2, false, 2).hint("Ask Zenji to join you in adventures outside camp.");
                }
            }
            else if (flags[kFLAGS.PLAYER_COMPANION_2] == "Zenji")
            {
                addButton(6, "Assist Me (2)", zenjiHenchmanOption2, false, 22).hint("Ask Zenji to stay in camp.");
            }
            else
            {
                addButtonDisabled(6, "Assist Me (2)", "You already have other henchman accompany you as second party member. Ask him/her to stay at camp before you talk with Zenji about accompaning you as second party member.");
            }
        }
        else
        {
    ...
    public function zenjiHenchmanOption2(textBypass : Bool = false, slot : Float = 1) : Void
    {
        if (!textBypass)
        {
            clearOutput();
        ...
        if (slot < 21)
        {
            ...
            var strZenji : Float = 50;
            var meleeAtkZenji : Float = 145;
            ...
            meleeAtkZenji += (1 + as3hx.Compat.parseInt(meleeAtkZenji / 5)) * player.newGamePlusMod();
            player.createStatusEffect(StatusEffects.CombatFollowerZenji, strZenji, meleeAtkZenji, 0, 0);
            if (slot == 2)
            {
                flags[kFLAGS.PLAYER_COMPANION_2] = "Zenji";
            }
            if (slot == 1)
            {
    ...
    private function applyZenjikidName() : Void
    {
        ...
        else if (mainView.nameBox.text == "" || mainView.nameBox.text == "0" || mainView.nameBox.text == "Zenji")
        {
            ...
            else if (mainView.nameBox.text == "Zenji")
            {
                outputText("\"<i>I'm honored, but dey can’t have my name! Pick another.</i>\"");
            }
            mainView.nameBox.text = "";
        ...
        clearOutput();
        outputText("\"<i>" + ((Z2ndKid != null) ? "" + Z2ndKid + "" : "" + Z1stKid + "") + "... I like dat name…'</i>\" he states with a smile.\n\n");
    ...
    public function mishapsLunaZenji() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.CampLunaMishaps3, 1, 1);
        }
        else
        {
            player.createStatusEffect(StatusEffects.CampLunaMishaps3, 1, 0, 0, 0);
        }
    ...
    //ZENJI MARRIAGE
    public function ZenjiMarriagePreCheck() : Bool
    {
    ...
    private function ZenjiMarriageSceneTres() : Void
    {
        ...
        if (flags[kFLAGS.FACTORY_SHUTDOWN] == 1 && flags[kFLAGS.MARAE_QUEST_COMPLETE] >= 1)
    ...
    private function ZenjiMarriageSexTime(options : Int) : Void
    {
        ...
        else if (options == 3)
        {
            ...
            if (player.statusEffectv2(StatusEffects.ZenjiModificationsList) >= 6000)
            
            //This was supposed to be volume based (6L), but due to Orm's black magic, I can only guess... unless that number is the actual volume. ¯\_(ツ)_/¯ //Haha, good luck everyone -Pyromania{
    ...
    private function ZenjiMarriageSexTime3(options : Int) : Void
    {
        ...
        if (!recalling)
        {
            var timeShift : Int = as3hx.Compat.parseInt((24 - time.hours) + 8);
    ...
    public static function isLover() : Bool
    {
