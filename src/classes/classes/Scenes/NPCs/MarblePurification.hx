...
class MarblePurification extends NPCAwareContent
{
    ...
    public function BLUHBLUH() : Void
    {
        ...
        {
            outputText("  You then recall that bottle of honey the bee-girl gave you, and ask if maybe it might work?");
        }
        if (player.hasStatusEffect(StatusEffects.PureCampJojo) && flags[kFLAGS.JOJO_BIMBO_STATE] != 3)
        {
            outputText("  You remember how Jojo helped you become pure, and suggest maybe his meditations could assist Marble?");
        }
    ...
    public function giveMarblePureQuestLabova() : Void
    {
        ...
        {
            //Play this scene automatically when PC gives Marble the first Labova after reaching her "quota"if (player.statusEffectv4(StatusEffects.MarbleSpecials) <= 30)
            {
    ...
    public function rathazulsMurbelReport() : Void
    {
        ...
        {
            outputText("and your other followers ");
        }
    ...
    public function claraShowsUpInCampBECAUSESHESACUNT() : Void
    {
        ...
        if (flags[kFLAGS.MARBLE_BOVA_LEVEL] <= 1)
        {
            outputText("your mate has");
        }
        else
        {
            outputText("what your mate use to have");
        }
        ...
        if (player.inte >= 60)
        {
            outputText(" with an odd smile on her face");
        }
        ...
        if (player.inte >= 60)
        {
            outputText(" somewhat unconvincingly");
        }
        outputText(".");
    ...
    public function partTwoOfClaraShowingUpAndBeingACunt() : Void
    {
        ...
        if (SceneLib.camp.companionsCount() >= 3)
        {
            ...
            if (player.hasStatusEffect(StatusEffects.PureCampJojo) && flags[kFLAGS.JOJO_BIMBO_STATE != 3] != null)
            {
    ...
    public function dumbshitCowCuntsTryToDrugYouBecauseTheyreCunts(teaDrinkers : Int) : Void
    {
        ...
        if (player.inte >= 60)
        {
            addButton(0, "Next", highIntelligenceOutcomesVsCowCuntDrugs);
        }
        else if (player.inte >= 30)
        {
            addButton(0, "Next", mediumIntelligenceReactsToDrugging);
        }
        else
        {
            addButton(0, "Next", lowIntelligenceGetsDruggedByCowSluts);
        }
    }
    ...
    //set PC’s health to max, fatigue to 40, and lust to 50.  Advance time by 6 hours.
    {
        ...
        {
            outputText("your [weapon] is nearby, and ");
        }
    ...
    public function fightCowCuntAfterBeingTotesMcGoatsDrugged() : Void
    {
        ...
        if (player.weaponName == "fists")
        {
            outputText("raise your fists");
        }
        else
        {
            outputText("grab your " + player.weaponName);
        }
    ...
    public function startCombatWithMarbleSister(inCamp : Bool = true) : Void
    {
        startCombat(new Clara());
        player.createStatusEffect(StatusEffects.NoFlee, 0, 0, 0, 0);
        if (inCamp && !player.hasStatusEffect(StatusEffects.ClaraFoughtInCamp))
        {
            player.createStatusEffect(StatusEffects.ClaraFoughtInCamp, 0, 0, 0, 0);
        }
    }
    //Chose submit
    ...
    public function mediumIntelligenceDoesntDrinkCowCuntsTea() : Void
    {
        ...
        if (SceneLib.camp.companionsCount())
        {
    ...
    public function loseToClara(combat : Bool = true) : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_clara);
        //PC loses via health
        if (combat)
        {
            if (player.HP < 1)
            {
                outputText("Your body gives out from the strain, and you fall to the ground.  The last thing you hear before you pass out, is Clara’s triumphant laughter.");
            }
            //proceed to loss scene
            else
            {
                //PC loses via lustoutputText("You just can’t fight it anymore, and desperately start to masturbate in front of Clara.  Laughing triumphantly, Clara walks up to you and forces a pill down your throat.  A moment later, you pass out.");
            }
            outputText("\n\n");
        }
        //proceed to loss scene
        ...
        {
            outputText("much too big for you.");
        }
        else if (player.tallness <= 82)
        {
            outputText("just your size.");
        }
        else
        {
            outputText("way too small for your stature.");
        }
        ...
        {
            outputText("You’re completely naked.");
        }
    ...
    public function cowCuntBadEndPartIII() : Void
    {
        ...
        {
            outputText("through your " + hairDescript());
        }
        else
        {
            outputText("across your scalp");
        }
    ...
    private function badEndRouter() : Void
    {
        ...
        if (player.hasCock())
        {
            sceneHunter.selectFitNofit(badEndDickThatFitsClara, cockTooBigForCowCuntageBadEndage, 56);
        }
        else
        {
    ...
    public function defeatClaraCuntInAFight(cheated : Bool = false) : Void
    {
        ...
        else
        {
            ...
            {
                //PC drags Clara back to camp, was originally planning on putting more rapes here, but 7 with Marble are probably enough.if (!player.hasStatusEffect(StatusEffects.ClaraFoughtInCamp))
                {
        ...
        if (SceneLib.camp.loversCount() > 1)
        {
            outputText("I'm not even [name]'s only mate.  " + player.mf("He", "She") + " can satisfy their needs, but can't satisfy mine?  ");
        }
        ...
        if ((player.hasCock() && player.cockThatFits(56) >= 0) || player.hasVagina())
        {
            outputText("use Marble's unstable state of mind to get her into a forceful threesome with Clara, ");
        }
        //(if PC either has a cock that will fit Marble, or has a vagina)
        if ((player.hasCock() && player.cockThatFits(marbleScene.marbleCuntCapacity()) >= 0) || player.hasVagina())
        {
            outputText("have sex in front of Clara so she can see what your love really is, ");
        }
        //(if Marble is a futa)
        if (flags[kFLAGS.MARBLE_DICK_TYPE] > 0)
        {
            outputText("egg Marble into violating Clara with her cock, ");
        }
        ...
        if ((player.hasCock() && player.cockThatFits(56) >= 0) || player.hasVagina())
        {
            addButton(1, "Threesome", pcAndMurbleDoubleTeamCowButt);
        }
        if ((player.hasCock() && player.cockThatFits(marbleScene.marbleCuntCapacity()) >= 0) || player.hasVagina())
        {
        ...
        if (flags[kFLAGS.MARBLE_DICK_TYPE] > 0)
        {
            addButton(3, "Marble Cock", futaMarbleIsAHugeCowToCowCuntAndStuffsCowCuntsCuntFullOfCowCock);
        }
        if (!recalling)
        {
    ...
    public function purificationQuestFinal() : Void
    {
        clearOutput();
        marbleScene.marbleSprite();
        var expReward : Float = 500;
        if (player.level > 4)
        {
            expReward += (player.level - 4) * 100;
        }
    ...
    */
    ...
    public function claraCampAddition() : Void
    {
        ...
        if (flags[kFLAGS.CLARA_PURIFIED] > 0)
        {
            if (rand(10) == 0)
            {
                outputText("You catch Clara staring at you for a moment, then her gaze once again returns to some distant mountain top.  She has not taken her purification well.");
            }
            else
            {
                outputText("As always, Clara sits within, staring off into the distance.  She has not taken her purification well.");
            }
        }
        ...
        else
        {
            ...
            if (CoC.instance.time.hours <= 7 || CoC.instance.time.hours >= 20)
            {
                outputText("Clara is laying sprawled out on the ground of her cage, snoring loudly.");
            }
            else
            {
                switch (rand(4))
