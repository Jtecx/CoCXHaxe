...
class DinahFollower extends NPCAwareContent
{
    public function new()
    {
        super();
...
*/
    ...
    private var _roulette1 : Float = 0;
    private var _roulette2 : Float = 0;
    private var _roulette3 : Float = 0;
    
    public function dinahAffection(changes : Float = 0) : Float
    {
        flags[kFLAGS.DINAH_AFFECTION] += changes;
        if (flags[kFLAGS.DINAH_AFFECTION] > 100)
        {
            flags[kFLAGS.DINAH_AFFECTION] = 100;
        }
    ...
    public function DinahIntro2() : Void
    {
        ...
        // && !flags[kFLAGS.DINAH_ATTACKED_TODAY]{
    ...
    {
        var atCamp : Bool = (flags[kFLAGS.DINAH_LVL_UP] > 0.5);
        var weather_choice : Array<Dynamic> = ["sunny", "rainy", "snowy", "windy", "dankest", "reeking with undistilled lust"];
        clearOutput();
        if (atCamp)
        {
            outputText("\"<i>What can I do for you, [name], this " + randomChoice(weather_choice) + " day?</i>\" Dinha asks you with a disturbingly wide and briliant smile on her face.");
        }
        else
        {
            outputText("\"<i>Will you gaze at me the whole day or will you buy something?</i>\" They grumble to themselves.");
        }
        menu();
        ...
        if (atCamp)
        {
            ...
            addButton(1, "Spar", DinahSparring).disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2, "You need a good sparring ring for that.");
            //addButtonDisabled(3, "Talk", "NYI");
            //addButtonDisabled(4, "Sex", "NYI");
            addButton(5, "Gift", recieveGIFTfromDinah).disableIf(player.hasStatusEffect(StatusEffects.DinahGift), "She doesn't have anything for you right now... Come back later!");
        ...
        else
        {
            ...
            {
                addButton(13, "Inv2Camp", DinahMoveToCamp);
            }
    ...
    public function DinahLostSparring() : Void
    {
        ...
        {
            if (flags[kFLAGS.DINAH_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.DINAH_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.DINAH_DEFEATS_COUNTER] = 1;
            }
            if (flags[kFLAGS.DINAH_DEFEATS_COUNTER] == 1 && flags[kFLAGS.DINAH_LVL_UP] == 1)
            {
                ...
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers3, 3, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction));
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers3, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction), 0);
                }
                flags[kFLAGS.DINAH_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.DINAH_DEFEATS_COUNTER] == 2 && flags[kFLAGS.DINAH_LVL_UP] == 2)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers3))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers3, 3, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 2);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers3, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 2, 0);
                }
                flags[kFLAGS.DINAH_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.DINAH_DEFEATS_COUNTER] == 3 && flags[kFLAGS.DINAH_LVL_UP] == 3)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers3))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers3, 3, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 3);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers3, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 3, 0);
                }
                flags[kFLAGS.DINAH_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.DINAH_DEFEATS_COUNTER] == 4 && flags[kFLAGS.DINAH_LVL_UP] == 4)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers3))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers3, 3, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 4);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers3, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 4, 0);
                }
                flags[kFLAGS.DINAH_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.DINAH_DEFEATS_COUNTER] == 5 && flags[kFLAGS.DINAH_LVL_UP] == 5)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers3))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers3, 3, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 5);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers3, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 5, 0);
                }
                flags[kFLAGS.DINAH_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.DINAH_DEFEATS_COUNTER] == 6 && flags[kFLAGS.DINAH_LVL_UP] == 6)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers3))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers3, 3, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 6);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers3, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 6, 0);
                }
                flags[kFLAGS.DINAH_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.DINAH_DEFEATS_COUNTER] == 7 && flags[kFLAGS.DINAH_LVL_UP] == 7)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers3))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers3, 3, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 7);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers3, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 7, 0);
                }
                flags[kFLAGS.DINAH_DEFEATS_COUNTER] = 0;
            ...
            if (flags[kFLAGS.DINAH_DEFEATS_COUNTER] == 8 && flags[kFLAGS.DINAH_LVL_UP] == 8)
            {
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers3))
                {
                    player.addStatusValue(StatusEffects.CampSparingNpcsTimers3, 3, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 8);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers3, 0, 0, player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * 8, 0);
                }
                flags[kFLAGS.DINAH_DEFEATS_COUNTER] = 0;
    ...
    public function DinahShopMainMenu() : Void
    {
        ...
        {
            addButton(11, "HB M&U", buyHowlingBansheeMechAndUpgrades);
        }
        else
        {
            addButtonDisabled(11, "???", "Offers only for those that are wearing HB Armor & HB Helmet.");
        }
        if (flags[kFLAGS.DINAH_LVL_UP] > 0.5)
        {
    ...
    public function DinahShopMainMenu1() : Void
    {
        ...
        if (player.hasItem(weaponsrange.HARKON1, 2))
        {
            addButton(12, "Harkonnen II", getHarkonnenIIMade);
        }
        else
        {
            addButtonDisabled(12, "???", "Req. to have 2x Harkonnen and 4,500 gems.");
        }
        if (player.hasPerk(PerkLib.PrestigeJobNecromancer))
        {
            addButton(13, consumables.PODBONE.shortName, buyItem1, 10).hint("Buy a pack of demon bones.");
        }
        else
        {
            addButtonDisabled(13, "???", "Offers only for Necromancers.");
        }
    ...
    public function DinahShopMainMenu2() : Void
    {
        ...
        {
            addButton(0, consumables.JABBERS.shortName, buyItem4, 23).hint("Buy a Jabberwocky Scale.");
        }
        else
        {
            addButtonDisabled(0, "???", "Req. to beat one of bosses in Wonderland to have access to this TF item.");
        }
        if (flags[kFLAGS.DINAH_LVL_UP] >= 1)
        {
            if (player.statusEffectv2(StatusEffects.TFDealer1) > 0)
            {
                addButton(1, consumables.HYDRASC.shortName, buyItem4, 21).hint("Buy a Hydra Scale.");
            }
            else
            {
                addButtonDisabled(1, "???", "Req. to beat one of bosses in Ebon Labyrinth to have access to this TF item.");
            }
            if (player.statusEffectv3(StatusEffects.TFDealer1) > 0)
            {
                addButton(2, consumables.FSNAILS.shortName, buyItem4, 22).hint("Buy a Fire Snail Saliva.");
            }
            else
            {
                addButtonDisabled(2, "???", "Req. to beat one of bosses in Ebon Labyrinth to have access to this TF item.");
            }
            if (player.statusEffectv1(StatusEffects.TFDealer1) > 0)
            {
                addButton(3, consumables.DSLIMEJ.shortName, buyItem4, 20).hint("Buy a Dark Slime Jelly.");
            }
            else
            {
                addButtonDisabled(3, "???", "Req. to beat one of bosses in Ebon Labyrinth to have access to this TF item.");
            }
            if (player.statusEffectv1(StatusEffects.TFDealer2) > 0)
            {
                addButton(4, consumables.ME_DROP.shortName, buyItem4, 24).hint("Buy a Magic Eye Drop.");
            }
            else
            {
                addButtonDisabled(3, "???", "Req. to beat one of bosses in Ebon Labyrinth to have access to this TF item.");
            }
            if (player.statusEffectv2(StatusEffects.TFDealer2) > 0)
            {
                addButton(5, consumables.M_GOSSR.shortName, buyItem4, 25).hint("Buy a Midnight Black Glossamer.");
            }
            else
            {
                addButtonDisabled(3, "???", "Req. to beat one of bosses in Ebon Labyrinth to have access to this TF item.");
    ...
    public function DinahShopMainMenu3() : Void
    {
        ...
        if (_roulette1 == 3)
        {
            addButton(1, "UnDefKingDest", buyItem5, 42).hint("Undefeated King's Destroyer - Massive mace weapon that will increase PC parry chance by 20%. Have 20% base chance for stun (3 rounds).");
        }
        if (_roulette2 == 0)
        {
            addButtonDisabled(2, "???", "Dud. Shame, shame.");
        }
        if (_roulette2 == 1)
        {
            addButton(2, "FlameLizR", buyItem5, 45).hint("Flame Lizard ring - Increases maximum Wrath by 75. Generate 2/1 wrath per turn/hour. Allow to use Lustzerker.");
        }
        if (_roulette2 == 2)
        {
            addButton(2, "InferMouseR", buyItem5, 46).hint("Infernal Mouse ring - Increases maximum Wrath by 75. Generate 2/1 wrath per turn/hour. Allow to use Blazing battle spirit.");
        }
        if (_roulette2 == 3)
        {
            addButton(2, "BestBlaAr", buyItem5, 47).hint("Bestial Blademaster armor - Increases fire damage based on percent of wrath bar fullness. (+1% per 1% of current wrath");
        }
        if (_roulette2 == 4)
        {
            addButton(2, "BestBlaAc", buyItem5, 48).hint("Bestial Blademaster accoutrements - Increased chance (+10%) to block/parry/evade whilst in a berserker/lustzerker state. These actions would also grant 1% wrath when successful.");
        }
        if (_roulette3 == 0)
        {
            addButtonDisabled(3, "???", "Dud. Shame, shame.");
        }
        if (_roulette3 == 1)
        {
            addButton(3, "HBHelmet", buyItem5, 50).hint("HB helmet - Increase armor by 5 and magic resistance by 4.");
        }
        if (_roulette3 == 2)
        {
            addButton(3, "HBArmor", buyItem5, 51).hint("HB armor - Increasing it armor/resistance when power up by soulforce.");
        }
        if (_roulette3 == 3)
        {
            addButton(3, "HBShirt", buyItem5, 52).hint("HB Shirt - Increase armor by 4, magic resistance by 3, fire/ice/electric resistance by 10%.");
        }
        if (_roulette3 == 4)
        {
            addButton(3, "HBShorts", buyItem5, 53).hint("HB Shorts - Increase armor by 4, magic resistance by 3, fire/ice/electric resistance by 10%.");
        }
    ...
    private function buyItem1(item : Float = 0) : Void
    {
        if (item == 0)
        {
            catChimeraBuy1(consumables.AGILI_E);
        }
        if (item == 1)
        {
            catChimeraBuy1(consumables.W_FRUIT);
        }
        if (item == 10)
        {
            catChimeraBuy1(consumables.PODBONE);
        }
    }
    private function buyItem2(item : Float = 0) : Void
    {
        if (item == 2)
        {
            catChimeraBuy2(consumables.WOFRUIT);
        }
        if (item == 3)
        {
            catChimeraBuy2(consumables.G_FRUIT);
        }
        if (item == 4)
        {
            catChimeraBuy2(consumables.D_FRUIT);
        }
        if (item == 8)
        {
            catChimeraBuy2(consumables.STRASCA);
        }
        if (item == 9)
        {
            catChimeraBuy2(consumables.BCHCAKE);
        }
    }
    private function buyItem3(item : Float = 0) : Void
    {
        if (item == 5)
        {
            catChimeraBuy3(consumables.MANTICV);
        }
        if (item == 6)
        {
            catChimeraBuy3(consumables.VOLTTOP);
        }
        if (item == 7)
        {
            catChimeraBuy3(consumables.REDVIAL);
        }
        if (item == 11)
        {
            catChimeraBuy1(consumables.MADMENK);
        }
    }
    private function buyItem4(item : Float = 0) : Void
    {
        if (item == 20)
        {
            catChimeraBuy4(consumables.DSLIMEJ);
        }
        if (item == 21)
        {
            catChimeraBuy4(consumables.HYDRASC);
        }
        if (item == 22)
        {
            catChimeraBuy4(consumables.FSNAILS);
        }
        if (item == 23)
        {
            catChimeraBuy4(consumables.JABBERS);
        }
        if (item == 24)
        {
            catChimeraBuy4(consumables.ME_DROP);
        }
        if (item == 25)
        {
            catChimeraBuy4(consumables.M_GOSSR);
        }
    }
    private function buyItem5(item : Float = 0) : Void
    {
        if (item == 40)
        {
            catChimeraBuy5(jewelries.UNDKINS);
        }
        if (item == 41)
        {
            catChimeraBuy5(headjewelries.CUNDKIN);
        }
        if (item == 42)
        {
            catChimeraBuy5(weapons.UDKDEST);
        }
        if (item == 45)
        {
            catChimeraBuy5(jewelries.FLLIRNG);
        }
        if (item == 46)
        {
            catChimeraBuy5(jewelries.INMORNG);
        }
        if (item == 47)
        {
            catChimeraBuy5(armors.BESTBLA);
        }
        if (item == 48)
        {
            catChimeraBuy5(headjewelries.BESTBLAA);
        }
        if (item == 50)
        {
            catChimeraBuy5(headjewelries.HBHELM);
        }
        if (item == 51)
        {
            catChimeraBuy5(armors.HBARMOR);
        }
        if (item == 52)
        {
            catChimeraBuy5(undergarments.HBSHIRT);
        }
        if (item == 53)
        {
            catChimeraBuy5(undergarments.HBSHORT);
        }
    }
    ...
    public function buyHowlingBansheeMechAndUpgrades() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.BuyedHowlingBansheMech))
        {
            ...
            {
                if (player.keyItemvX("HB Armor Plating", 1) == 1)
                {
                    addButton(0, "Armor Plating v2", buyHowlingBansheeMechUpgrade, "Armor Plating v2", 2500, 2).hint("Increase armor by 25.");
                }
                if (player.keyItemvX("HB Armor Plating", 1) == 2)
                {
                    addButton(0, "Armor Plating v3", buyHowlingBansheeMechUpgrade, "Armor Plating v3", 3500, 3).hint("Increase armor by 35.");
                }
                if (player.keyItemvX("HB Armor Plating", 1) == 3)
                {
                    addButton(0, "Armor Plating v4", buyHowlingBansheeMechUpgrade, "Armor Plating v4", 4500, 4).hint("Increase armor by 45.");
                }
                if (player.keyItemvX("HB Armor Plating", 1) == 4)
                {
                    addButton(0, "Armor Plating v5", buyHowlingBansheeMechUpgrade, "Armor Plating v5", 5500, 5).hint("Increase armor by 55.");
                }
                if (player.keyItemvX("HB Armor Plating", 1) == 5)
                {
                    addButton(0, "Armor Plating v6", buyHowlingBansheeMechUpgrade, "Armor Plating v6", 6500, 6).hint("Increase armor by 65.");
                }
                if (player.keyItemvX("HB Armor Plating", 1) == 6)
                {
                    addButton(0, "Armor Plating v7", buyHowlingBansheeMechUpgrade, "Armor Plating v7", 7500, 7).hint("Increase armor by 75.");
                }
                if (player.keyItemvX("HB Armor Plating", 1) == 7)
                {
                    addButtonDisabled(0, "Armor Plating v7", "Your HB Mech already have this upgrade.");
                }
            }
            else
            {
                addButton(0, "Armor Plating v1", buyHowlingBansheeMechUpgrade, "Armor Plating v1", 1500, 1).hint("Increase armor by 15.");
            }
            if (player.hasKeyItem("HB Leather Insulation") >= 0)
            {
                if (player.keyItemvX("HB Leather Insulation", 1) == 1)
                {
                    addButton(1, "Leather Insulation v2", buyHowlingBansheeMechUpgrade, "Leather Insulation v2", 2500, 9).hint("Increase magic resistance by 25.");
                }
                if (player.keyItemvX("HB Leather Insulation", 1) == 2)
                {
                    addButton(1, "Leather Insulation v3", buyHowlingBansheeMechUpgrade, "Leather Insulation v3", 3500, 10).hint("Increase magic resistance by 35.");
                }
                if (player.keyItemvX("HB Leather Insulation", 1) == 3)
                {
                    addButton(1, "Leather Insulation v4", buyHowlingBansheeMechUpgrade, "Leather Insulation v4", 4500, 11).hint("Increase magic resistance by 45.");
                }
                if (player.keyItemvX("HB Leather Insulation", 1) == 4)
                {
                    addButton(1, "Leather Insulation v5", buyHowlingBansheeMechUpgrade, "Leather Insulation v5", 5500, 12).hint("Increase magic resistance by 55.");
                }
                if (player.keyItemvX("HB Leather Insulation", 1) == 5)
                {
                    addButton(1, "Leather Insulation v6", buyHowlingBansheeMechUpgrade, "Leather Insulation v6", 6500, 13).hint("Increase magic resistance by 65.");
                }
                if (player.keyItemvX("HB Leather Insulation", 1) == 6)
                {
                    addButton(1, "Leather Insulation v7", buyHowlingBansheeMechUpgrade, "Leather Insulation v7", 7500, 14).hint("Increase magic resistance by 75.");
                }
                if (player.keyItemvX("HB Leather Insulation", 1) == 7)
                {
                    addButtonDisabled(1, "Leather Insulation v7", "Your HB Mech already have this upgrade.");
                }
            }
            else
            {
                addButton(1, "Leather Insulation v1", buyHowlingBansheeMechUpgrade, "Leather Insulation v1", 1500, 8).hint("Increase magic resistance by 15.");
            }
            if (player.hasKeyItem("HB Agility") >= 0)
            {
                if (player.keyItemvX("HB Agility", 1) == 1)
                {
                    addButtonDisabled(2, "Agility v2", "Your HB Mech already have this upgrade.");
                }
                else
                {
                    addButton(2, "Agility v2", buyHowlingBansheeMechUpgrade, "Agility v2", 1000, 16).hint("Adding speed scaling similar to Quick Strike perk to melee mech attacks.");
                }
            }
            else
            {
                addButton(2, "Agility v1", buyHowlingBansheeMechUpgrade, "Agility v1", 500, 15).hint("Adding speed scaling similar to Speed Demon perk to melee mech attacks.");
            }  /*
            ...
            if (player.hasKeyItem("HB Dragon's Breath Flamer") >= 0)
            {
                ...
                {
                    addButton(6, "DB Flamer v2", buyHowlingBansheeMechUpgrade, "Dragon's Breath Flamer v2", 3000, 21).hint("Instal second Dragon's Breath Flamer weapon - adds second fire attack when using this special and cost of use increase 2x.");
                }
                if (player.keyItemvX("HB Dragon's Breath Flamer", 1) == 2)
                {
                    addButton(6, "DB Flamer v3", buyHowlingBansheeMechUpgrade, "Dragon's Breath Flamer v3", 4500, 22).hint("Instal third Dragon's Breath Flamer weapon - adds third fire attack when using this special and cost of use increase 3x.");
                }
                if (player.keyItemvX("HB Dragon's Breath Flamer", 1) == 3)
                {
                    addButton(6, "DB Flamer v4", buyHowlingBansheeMechUpgrade, "Dragon's Breath Flamer v4", 5000, 23).hint("Remodeling Dragon's Breath Flamers into one with shared Nozzle - adds aoe effect to slight stronger fire attack compared to previous version when using this special and cost of use increase 4x.");
                }
                if (player.keyItemvX("HB Dragon's Breath Flamer", 1) == 4)
                {
                    addButton(6, "DB Flamer v5", buyHowlingBansheeMechUpgrade, "Dragon's Breath Flamer v5", 5500, 24).hint("Instal improved Remodeled Dragon's Breath Flamers - adds greater aoe effect to slight stronger fire attack compared to previous version when using this special and cost of use increase 5x.");
                }
                if (player.keyItemvX("HB Dragon's Breath Flamer", 1) == 5)
                {
                    addButtonDisabled(6, "DB Flamer v5", "Your HB Mech already have this upgrade.");
                }
            }
            else
            {
                addButton(6, "DB Flamer v1", buyHowlingBansheeMechUpgrade, "Dragon's Breath Flamer v1", 1500, 20).hint("Add Dragon's Breath Flamer weapon - Allow to enter use special dealing fire damage.");
            }
            if (player.hasKeyItem("HB Scatter Laser") >= 0)
            {
                if (player.keyItemvX("HB Scatter Laser", 1) == 1)
                {
                    addButton(7, "Scatter Laser v2", buyHowlingBansheeMechUpgrade, "Scatter Laser v2", 2500, 18).hint("Adds 2 more units that allow either double shot at lone targets or using all three against groups.");
                }
                if (player.keyItemvX("HB Scatter Laser", 1) == 2)
                {
                    addButton(7, "Scatter Laser v3", buyHowlingBansheeMechUpgrade, "Scatter Laser v3", 3500, 19).hint("Adding 3 more units that allow either four shots at lone targets or using all six against groups.");
                }
                if (player.keyItemvX("HB Scatter Laser", 1) == 3)
                {
                    addButtonDisabled(7, "Scatter Laser v3", "Your HB Mech already have this upgrade.");
                }
            }
            else
            {
                addButton(7, "Scatter Laser v1", buyHowlingBansheeMechUpgrade, "Scatter Laser v1", 1500, 17).hint("Add Scatter Laser weapon - Allow to enter use special dealing lightning damage.");
            }
        }
        else
        {
    ...
    public function buyHowlingBansheeMechUpgrade(upgrade : String, cost : Float, type : Float) : Void
    {
        clearOutput();
    ...
    public function buyHowlingBansheeMechUpgrade1(upgrade : String, cost : Float, type : Float) : Void
    {
        clearOutput();
        ...
        statScreenRefresh();
        if (upgrade == "Armor Plating v1")
        {
            player.createKeyItem("HB Armor Plating", 1, 0, 0, 0);
        }
        if (upgrade == "Armor Plating v2")
        {
            player.addKeyValue("HB Armor Plating", 1, 1);
        }
        if (upgrade == "Armor Plating v3")
        {
            player.addKeyValue("HB Armor Plating", 1, 1);
        }
        if (upgrade == "Armor Plating v4")
        {
            player.addKeyValue("HB Armor Plating", 1, 1);
        }
        if (upgrade == "Armor Plating v5")
        {
            player.addKeyValue("HB Armor Plating", 1, 1);
        }
        if (upgrade == "Armor Plating v6")
        {
            player.addKeyValue("HB Armor Plating", 1, 1);
        }
        if (upgrade == "Armor Plating v7")
        {
            player.addKeyValue("HB Armor Plating", 1, 1);
        }
        if (upgrade == "Leather Insulation v1")
        {
            player.createKeyItem("HB Leather Insulation", 1, 0, 0, 0);
        }
        if (upgrade == "Leather Insulation v2")
        {
            player.addKeyValue("HB Leather Insulation", 1, 1);
        }
        if (upgrade == "Leather Insulation v3")
        {
            player.addKeyValue("HB Leather Insulation", 1, 1);
        }
        if (upgrade == "Leather Insulation v4")
        {
            player.addKeyValue("HB Leather Insulation", 1, 1);
        }
        if (upgrade == "Leather Insulation v5")
        {
            player.addKeyValue("HB Leather Insulation", 1, 1);
        }
        if (upgrade == "Leather Insulation v6")
        {
            player.addKeyValue("HB Leather Insulation", 1, 1);
        }
        if (upgrade == "Leather Insulation v7")
        {
            player.addKeyValue("HB Leather Insulation", 1, 1);
        }
        if (upgrade == "Agility v1")
        {
            player.createKeyItem("HB Agility", 0, 0, 0, 0);
        }
        if (upgrade == "Agility v2")
        {
            player.addKeyValue("HB Agility", 1, 1);
        }
        if (upgrade == "Rapid Reload v1")
        {
            player.createKeyItem("HB Rapid Reload", 0, 0, 0, 0);
        }
        if (upgrade == "Rapid Reload v2")
        {
            player.addKeyValue("HB Rapid Reload", 1, 1);
        }
        if (upgrade == "Internal Systems v1")
        {
            player.createKeyItem("HB Internal Systems", 1, 0, 0, 0);
        }
        if (upgrade == "Internal Systems v2")
        {
            player.addKeyValue("HB Internal Systems", 1, 1);
        }
        if (upgrade == "Dragon's Breath Flamer v1")
        {
            player.createKeyItem("HB Dragon's Breath Flamer", 1, 0, 0, 0);
        }
        if (upgrade == "Dragon's Breath Flamer v2")
        {
            player.addKeyValue("HB Dragon's Breath Flamer", 1, 1);
        }
        if (upgrade == "Dragon's Breath Flamer v3")
        {
            player.addKeyValue("HB Dragon's Breath Flamer", 1, 1);
        }
        if (upgrade == "Dragon's Breath Flamer v4")
        {
            player.addKeyValue("HB Dragon's Breath Flamer", 1, 1);
        }
        if (upgrade == "Dragon's Breath Flamer v5")
        {
            player.addKeyValue("HB Dragon's Breath Flamer", 1, 1);
        }
        if (upgrade == "Scatter Laser v1")
        {
            player.createKeyItem("HB Scatter Laser", 1, 0, 0, 0);
        }
        if (upgrade == "Scatter Laser v2")
        {
            player.addKeyValue("HB Scatter Laser", 1, 1);
        }
        if (upgrade == "Scatter Laser v3")
        {
            player.addKeyValue("HB Scatter Laser", 1, 1);
        }
        if (upgrade == "Camouflage Mode")
        {
            player.createKeyItem("HB Stealth System", 0, 0, 0, 0);
        }
        if (upgrade == "Invisibility Mode v1")
        {
            player.addKeyValue("HB Stealth System", 1, 1);
        }
        if (upgrade == "Invisibility Mode v2")
        {
            player.addKeyValue("HB Stealth System", 1, 1);
        }
        //if (upgrade == "") ;
    ...
    private function giveDinahBrownEgg() : Void
    {
        ...
        if (flags[kFLAGS.DINAH_ASS_HIPS_SIZE] < 35)
        {
            ...
            if (flags[kFLAGS.DINAH_ASS_HIPS_SIZE] > 35)
            {
                flags[kFLAGS.DINAH_ASS_HIPS_SIZE] = 35;
            }
        }
        else
        {
    ...
    private function giveDinahPurpleEgg() : Void
    {
        ...
        if (flags[kFLAGS.DINAH_HIPS_ASS_SIZE] < 20)
        {
            ...
            if (flags[kFLAGS.DINAH_HIPS_ASS_SIZE] > 20)
            {
                flags[kFLAGS.DINAH_HIPS_ASS_SIZE] = 20;
            }
        }
        else
        {
    ...
    private function giveDinahReducto() : Void
    {
        ...
        menu();
        if (flags[kFLAGS.DINAH_CUP_SIZE] > 1)
        {
            addButton(0, "Breasts", dinahReducto, 0);
        }
        if (flags[kFLAGS.DINAH_HIPS_ASS_SIZE] > 6)
        {
            addButton(2, "Hips", dinahReducto, 1);
        }
        if (flags[kFLAGS.DINAH_ASS_HIPS_SIZE] > 6)
        {
            addButton(3, "Butt", dinahReducto, 2);
        }
    ...
    private function dinahReducto(part : Int) : Void
    {
        ...
        //Breasts{
            ...
            chosenPart = "breasts";
            if (flags[kFLAGS.DINAH_CUP_SIZE] > 6)
            {
                flags[kFLAGS.DINAH_CUP_SIZE]--;
        ...
        //Hips{
            ...
            chosenPart = "hips";
            if (flags[kFLAGS.DINAH_HIPS_ASS_SIZE] > 12)
            {
            ...
            if (flags[kFLAGS.DINAH_HIPS_ASS_SIZE] < 6)
            {
                flags[kFLAGS.DINAH_HIPS_ASS_SIZE] = 6;
            }
        }
        else if (part == 2)
        ...
        //Butt{
            ...
            chosenPart = "butt";
            if (flags[kFLAGS.DINAH_ASS_HIPS_SIZE] > 10)
            {
            ...
            if (flags[kFLAGS.DINAH_ASS_HIPS_SIZE] < 6)
            {
                flags[kFLAGS.DINAH_ASS_HIPS_SIZE] = 6;
            }
        }
    ...
    private function dinahTits() : String
    {
        ...
        if (rand(2) == 0)
        {
            ...
            if (flags[kFLAGS.DINAH_CUP_SIZE] > 2 && flags[kFLAGS.DINAH_CUP_SIZE] <= 5)
            {
                descript += "ample ";
            }
            ...
            if (flags[kFLAGS.DINAH_CUP_SIZE] >= 16 && flags[kFLAGS.DINAH_CUP_SIZE] < 21)
            {
    ...
    private function dinahNippleSize() : Float
    {
        var dinahNS : Float = 0.3;
        if (flags[kFLAGS.DINAH_CUP_SIZE] >= 5)
        {
            dinahNS += 0.2;
        }
        if (flags[kFLAGS.DINAH_CUP_SIZE] >= 10)
        {
            dinahNS += 0.3;
        }
        ...
        if (flags[kFLAGS.DINAH_CUP_SIZE] >= 23)
        {
            dinahNS += 0.4;
        }
        if (flags[kFLAGS.DINAH_CUP_SIZE] >= 28)
    ...
    private function dinahHips() : String
    {
        ...
        if (rando == 0)
        {
            if (rand(2) == 0 && flags[kFLAGS.DINAH_HIPS_ASS_SIZE] >= 15)
            {
                desc += "flanks";
            }
            else
