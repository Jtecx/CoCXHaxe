...
/**
	 ...
	 * @author Kitteh6660
	 */
class JoyScene extends NPCAwareContent
{
    ...
    {
        var temp : Int = as3hx.Compat.parseInt(30 + (10 * flags[kFLAGS.JOY_VAGINAL_WETNESS]));
        temp += as3hx.Compat.parseInt(2 * flags[kFLAGS.TIMES_PENETRATED_JOY_VAGINALLY]);
        if (temp > 120)
        {
            temp = 120;
        }
    ...
    {
        var temp : Int = 30;
        temp += as3hx.Compat.parseInt(5 * flags[kFLAGS.JOJO_ANAL_XP]);
        if (temp > 80)
        {
            temp = 80;
        }
    ...
    public function joyCockLength() : Float
    {
        return flags[kFLAGS.JOY_COCK_SIZE];
    }
    public function joyCockGirth() : Float
    {
        return 1 + (Math.floor(flags[kFLAGS.JOY_COCK_SIZE]) / 10);
    }
    
    public function joyBallSize() : Float
    {
        if (joyHasCock())
        {
            if (flags[kFLAGS.JOY_COCK_SIZE] < 7)
            {
                return 1;
            }
            else if (flags[kFLAGS.JOY_COCK_SIZE] < 8)
            {
                return 2;
            }
            else
    ...
    public function joyHasCock() : Bool
    {
    ...
    {
        var text : String = "";
        if (flags[kFLAGS.TIMES_PENETRATED_JOY_VAGINALLY] <= 0 && rand(2) == 0)
        {
            text += "virgin ";
        }
        switch (rand(3))
    ...
    public function joyAssDescript() : String
    {
        ...
        else if (flags[kFLAGS.JOJO_ANAL_XP] < 5)
        {
            text += "tight ";
        }
        else
    ...
    {
        return as3hx.Compat.parseInt(flags[kFLAGS.JOJO_LITTERS] + flags[kFLAGS.JOY_TWINS_BIRTHED]);
    }
    ...
    private function incrementJoysCockFondness(amount : Int) : Void
    {
        ...
        {
            flags[kFLAGS.JOY_COCK_FONDNESS] = 10;
        }
    }
    ...
    */
    ...
    // FOLLOWER
    //------------
    private function genericMenu(approach : Bool = false) : Void
    {
        if (!approach)
        {
            clearOutput();
    ...
    public function approachCampJoy() : Void
    {
            ...
            switch (rand(2))
            {
                case 0:
                    ...
                    if (joyHasCock())
                    {
                        ...
                        {
                            outputText("rubbing our funsticks");
                        }
                        else
                        {
                            outputText("playing with my funstick");
                        }
                    }
                    outputText("?</i>\"");
                case 1:
        ...
        //No Joy sex for you if you're corrupt!
        else
        {
    ...
    private function joyTalkMenu() : Void
    {
        ...
        if (joyTalkCounter() > 0)
        {
            addButton(1, "Old Life", askJoyAboutOldLife);
        }
        addButton(2, "Demons", askJoyAboutDemons);
        if (flags[kFLAGS.JOY_TWINS_BIRTHED] > 0 || player.isPregnant() || jojoScene.pregnancy.isPregnant)
        {
            addButton(3, "Babies", askJoyAboutBabies);
        }
    ...
    private function askJoyAboutYourself() : Void
    {
        ...
        switch (player.raceObject())
        {
            ...
            case Races.GOBLIN:
                ...
                if (player.hasCock())
                {
                    outputText(" Never knew a goblin with, like, a cock of her own, either.");
                }
                if (player.gender == Gender.GENDER_FEMALE && !joyHasCock())
                {
                    outputText(" But I've never, like, heard of a lesbian goblin before.");
                }
                outputText("</i>\"");
            case Races.KITSUNE:
        ...
        else if (player.gender == Gender.GENDER_FEMALE)
        {
            if (joyHasCock())
            {
                outputText("girl, with a yummy baby-hole for me to stick my funstick into and fill you full of mousey-cream.");
            }
            else
            {
                outputText("girl... And that's not a bad thing, but I kinda, like, wish one of us had a funstick; it'd be more fun that way! Oooh! And imagine the fun we could have if we both had funsticks!");
            }
        }
        else if (player.gender == Gender.GENDER_HERM)
        {
            if (joyHasCock())
            {
                outputText("herm and I'm sooo happy about it; we can have so much fun!");
            }
            else
            {
                outputText("herm, with yummy [breasts] and a cute [vagina] for me to lick and a nice [cock] for me to suck and rub and fill my fun-holes with; I just love it when you put cream in my hungry-achey little belly.");
            }
        }
        else
        {
        ...
        if (player.hasPerk(PerkLib.MaraesGiftFertility))
        {
            if (joyHasCock())
            {
                outputText("\"<i>There's this funny green light sitting in your belly... It makes my funstick feel all hard and stiff. I just wanna stuff you full of cream and make lots and lots of little mousies with you!</i>\"");
            }
            else
            {
                outputText("\"<i>There's this funny green light sitting in your belly... It makes me think of babies. Like you're gonna have lots of babies.</i>\"");
            }
        }
        ...
        if (player.pregnancyIncubation > 0)
        {
            ...
            var _sw21_ = (player.pregnancyType);            
    ...
    private function askJoyAboutOldLife() : Void
    {
        ...
        if (!joyHasCock())
        {
            outputText(" and then losing my cock and balls was another thing to get used to");
        }
        ...
        if (flags[kFLAGS.JOY_INTELLIGENCE] < 40)
        {
            flags[kFLAGS.JOY_INTELLIGENCE]++;
        }
    ...
    private function askJoyAboutBabies() : Void
    {
        ...
        if (jojoScene.pregnancy.isPregnant)
        {
            outputText(" I just, like, never once dreamed I'd be the one carrying 'em, y'know?");
        }
        outputText("</i>\"");
        ...
        if (player.isPregnant() || jojoScene.pregnancy.isPregnant)
        {
            outputText("She pats " + ((jojoScene.pregnancy.isPregnant) ? "her" : "your") + " pregnant belly for emphasis.");
        }
    ...
    private function joyMeditationFull(clearText : Bool = true) : Void
    {
        if (clearText)
        {
            clearOutput();
        ...
        {
            var dick : Bool = SceneLib.exgartuan.dickAwake() && (!SceneLib.exgartuan.boobsAwake() || rand(2) == 0);
            ...
            {
                SceneLib.exgartuan.boobsSleep(36 + rand(18));
            }
        }
        ...
        if (rand(100) < flags[kFLAGS.JOY_INTELLIGENCE] + 20)
        
        //Passed, meditation time!{
            ...
            else if (player.gender > 0)
            {
                ...
                if (player.hasCock())
                {
                    outputText(player.cockDescript() + " is already hard ");
                }
                ...
                if (player.hasVagina())
                {
                    outputText("[vagina] is already moist");
                }
    ...
    private function trainToughness() : Void
    {
        ...
        if (player.tou <= 33)
        {
            ...
            if (player.cor > 66)
            {
                outputText("Though you would like to do something a lot more fun than kissing, personally.");
            }
        ...
        else if (player.tou <= 66)
        {
            ...
            if (player.cor > 66)
            {
                outputText(", and privately feeling somewhat ripped off");
            }
    ...
    private function giveItemsToJoy() : Void
    {
        ...
        if (player.hasItem(consumables.INCUBID) || player.hasItem(consumables.P_DRAFT))
        {
            ...
            {
                addButton(buttonPos++, consumables.P_DRAFT.shortName, giveJoyAnIncubiDraft, true);
            }
            else
            {
                addButton(buttonPos++, consumables.INCUBID.shortName, giveJoyAnIncubiDraft, false);
            }
            hasValidItems = true;
        }
        //Succubi Milk
        if (player.hasItem(consumables.SUCMILK) || player.hasItem(consumables.P_S_MLK))
        {
            ...
            {
                addButton(buttonPos++, consumables.P_S_MLK.shortName, giveJoyASuccubiMilk, true);
            }
            else
            {
                addButton(buttonPos++, consumables.SUCMILK.shortName, giveJoyASuccubiMilk, false);
            }
            hasValidItems = true;
        }
        //Pink Egg
        ...
        //DeBimbo
        if (flags[kFLAGS.JOY_TALKED_ABOUT_OLD_LIFE] != null)
        {
            if (flags[kFLAGS.RATHAZUL_DEBIMBO_OFFERED] != null)
            
            //corrupts don't give a fuck{
    ...
    private function giveJoyAScholarsTea() : Void
    {
        ...
        if (flags[kFLAGS.JOY_INTELLIGENCE] < 40)
        {
            flags[kFLAGS.JOY_INTELLIGENCE] += 2;
        }
        else
        {
            flags[kFLAGS.JOY_INTELLIGENCE]++;
    ...
    private function giveJoyAnIncubiDraftForReal() : Void
    {
        ...
        //Joy already has cock.{
            
            if (flags[kFLAGS.JOY_COCK_SIZE] >= 8.5)
    ...
    //Succubi Milk
    private function giveJoyASuccubiMilk(purified : Bool = false) : Void
    {
        clearOutput();
    ...
    private function giveJoyASuccubiMilkForReal() : Void
    {
        ...
        if (flags[kFLAGS.JOY_VAGINAL_WETNESS] >= 5)
        
        //Maxed vaginal wetness{
        ...
        if (flags[kFLAGS.JOY_BREAST_SIZE] >= BreastCup.G)
        
        //Jojo has maxed breast size.{
    ...
    //Pink Egg
    private function giveJoyAPinkEgg(large : Bool = false) : Void
    {
        clearOutput();
    ...
    private function giveJoyAPinkEggForRealYouMonster(large : Bool = false) : Void
    {
        clearOutput();
        if (large)
        {
            player.consumeItem(consumables.L_PNKEG, 1);
        }
        else
        {
            player.consumeItem(consumables.PINKEGG, 1);
        }
    ...
    //Blue Egg
    private function giveJoyABlueEgg(large : Bool = false) : Void
    {
        clearOutput();
        if (large)
        {
            player.consumeItem(consumables.L_BLUEG, 1);
        }
        else
        {
            player.consumeItem(consumables.BLUEEGG, 1);
        }
    ...
    private function giveJoyALactaidYummyMilkTime() : Void
    {
        ...
        {
            outputText("\"<i>I've never seen something like this before. Still, if you're sure.</i>\"");
        }
        else
        {
            outputText("\"<i>You want to drink my milk again? Okay!</i>\" She smiles, absently fondling one breast in anticipation.");
        }
        ...
        if (flags[kFLAGS.JOY_LACTAID_MILKED_COUNTER] == 0)
        {
            outputText("she furrows her brow in confusion, worry crossing her features. \"<i>[name]? Like, my boobies feel really funny... all tingly.</i>\"");
        }
        else
        {
            outputText("she grins widely. \"<i>Oooh, here it comes, all that yummy milk is starting to bubble up inside my little boobies!</i>\"");
        }
        ...
        if (flags[kFLAGS.JOY_LACTAID_MILKED_COUNTER] == 0)
        {
            ...
            if (flags[kFLAGS.JOY_BREAST_SIZE] < 7)
            {
                outputText("a little");
            }
            else if (flags[kFLAGS.JOY_BREAST_SIZE] < 12)
            {
                outputText("pretty");
            }
            else
        ...
        {
            outputText("\"<i>Never better, [name]. Don't you think my boobies look so nice all fat and full of milk?</i>\" She laughs;");
        }
        ...
        var refillAmount : Int = as3hx.Compat.parseInt((flags[kFLAGS.JOY_BREAST_SIZE] + 6) * 5);
    ...
    private function initiateSexWithJoy() : Void
    {
        ...
        if (player.inRut && player.hasCock())
        {
            ...
            if (player.clothedOrNakedLower("clothed", "naked") == "clothed" && (player.armor != armors.LTHCARM && !player.lowerGarment.isNothing))
            {
                outputText("You feel your " + player.armorDescript() + " strain against your [cock]; your bulge completely visible to Joy.");
            }
    ...
    private function joySexMenu() : Void
    {
        menu();
        outputText("\n");
        addButton(0, "Vaginal Fuck", penetrateJoysPussy).hint("Penetrate Joy vaginally with your cock.").disableIf(player.cockThatFits(joyVagCapacity()) < 0, "Req. a cock fitting " + joyVagCapacity() + " area!");
        addButton(1, "Anal Fuck", fuckJoyInTheAss).hint("Take Joy from behind and make sure she gets it good!").disableIf(player.cockThatFits(joyAnalCapacity()) < 0, "Req. a cock fitting " + joyAnalCapacity() + " area!");
        addButton(1, "Anal Fuck", fuckJoyInTheAss).hint("Take Joy from behind and make sure she gets it good!").disableIf(player.cockThatFits(joyAnalCapacity()) < 0, "Req. a cock fitting " + joyAnalCapacity() + " area!");
        addButton(4, "Frottage", frotWithJoy).hint("Do some cock play with Joy.").disableIf(!player.hasCock(), "Req. a cock!").disableIf(!joyHasCock(), "She doesn't have a cock for that...");
        addButton(5, "Get Blown", haveJoySuckYouOff).hint("Have Joy suck your off and give her a taste of your cum.").disableIf(!player.hasCock(), "Req. a cock!");
        addButton(6, "Get Licked", haveJoyLickYourGinas).hint("Have Joy lick your pussy.").disableIf(!player.hasVagina(), "Req. a vagina!");
        addButton(7, "Get Penetrated", haveJoyStuffYourPussy).hint("Have Joy penetrate you vaginally with her cock.").disableIf(!player.hasVagina(), "Req. a vagina!").disableIf(!joyHasCock(), "She doesn't have a cock for that...");
        ...
        addButton(3, "Blow Joy", suckJoysCock).hint("Suck Joy's cock and get some taste of her cum!").disableIf(!joyHasCock(), "She doesn't have a cock for that...");
        addButton(8, "Get Anal", haveJoyStuffYourButthole).hint("Have Joy take you from behind and put her cock to a good use.").disableIf(!joyHasCock(), "She doesn't have a cock for that...");
        addButton(14, "Back", genericMenu);
    }
    ...
    private function penetrateJoysPussy() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_PENETRATED_JOY_VAGINALLY] == 0)
        {
            outputText(" <b>You have taken Joy's virginity!</b>");
        }
        ...
        if (rand(100) < chance)
        {
            jojoScene.pregnancy.knockUp(PregnancyStore.PREGNANCY_PLAYER, PregnancyStore.INCUBATION_MOUSE);
        }
    ...
    private function fuckJoyInTheAss() : Void
    {
        ...
        if (flags[kFLAGS.JOJO_ANAL_XP] == 0)
        {
            outputText("<b>Joy has lost her anal virginity.</b> ");
        }
    ...
    private function frotWithJoy() : Void
    {
        ...
        {
            outputText("serpentine tail");
        }
        else if (player.isGoo())
        {
            outputText("the mass of goo that makes up your legs");
        }
    ...
    private function haveJoyStuffYourPussy() : Void
    {
        ...
        {
            player.knockUp(PregnancyStore.PREGNANCY_JOJO, PregnancyStore.INCUBATION_MOUSE);
        }  //Chance of player getting pregnant!  
    ...
    {
        var isVirgin : Bool = (player.ass.analLooseness == 0);
        clearOutput();
        ...
        if (player.hasCock())
        {
            outputText("You moan again as she happens to poke your prostate.");
        }
        ...
        if (player.hasCock())
        {
            outputText("Your [cock] throbs in pleasure as she brushes against your prostate. ");
        }
        if (player.hasVagina())
        {
            outputText("Your [vagina] nearly juices itself when Joy's balls slap against your moist lips. ");
        }
        if (player.ass.analLooseness <= 2)
        {
            outputText("\"Ooh! [name] you're, like, so tight back here!" + ((isVirgin) ? " Like, you must be a virgin!" : "") + "\" Joy squeaks in excitement. ");
        }
    ...
    //------------
    //Joy
    public function joyPregnancyUpdate() : Bool
    {
        switch (jojoScene.pregnancy.eventTriggered())
        {
            ...
            case 4:
                ...
                if (joyHasCock())
                {
                    outputText(" Her " + joyCockDescript() + " starts to idly creep erect, but you don't know if she really wants to fuck or not; it looks more like just an involuntary response.");
                }
                return true;
            case 5:
    ...
    private function joyGivesBirthPart2() : Void
    {
        ...
        //Gender of babies
        if (babyGender1 == babyGender2)
        
        //Are the twins same gender?{
        ...
        if (flags[kFLAGS.JOY_TWINS_BIRTHED] >= 3 && flags[kFLAGS.JOY_TAKES_BABIES_AWAY_COUNTER] == 0)
        {
            flags[kFLAGS.JOY_TAKES_BABIES_AWAY_COUNTER] = 72;
        }
    ...
    public function wakeUpWithJoyPostFuck() : Void
    {
        ...
        if (player.cumQ() < 100)
        {
            outputText("She succeeds without a slightest hitch");
        }
        else if (player.cumQ() < 1000)
        {
            outputText("Although her already distended belly swells out even further, she manages to drink it all");
        }
        else
        {
            outputText("Amazingly, she manages to keep swallowing and swallowing, even though she ends up wallowing on a belly that could pass for a waterbed as the cum distorts her already swollen midriff");
        }
        ...
        if (player.cumQ() < 100)
        {
            outputText("getting up.");
        }
        else if (player.cumQ() < 1000)
        {
            outputText("struggling to get to her feet with her newly enlarged belly.");
        }
        else
        {
            outputText("wobbling back and forth on her personal waterbed.");
        }
        ...
        if (player.cumQ() < 100)
        {
            outputText("walks away");
        }
        else if (player.cumQ() < 1000)
        {
            outputText("totters away");
        }
        else
        {
            outputText("manages to scrabble along on all fours");
        }
    ...
    public function joyTakesTheBabiesAway() : Void
    {
        ...
        if (camp.amilyFollower())
        
        //Bonus points if Amily is in camp!{
