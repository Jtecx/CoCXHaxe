...
class Electra extends Monster
{
    ...
    public function moveStaticDischarge() : Void
    {
        ...
        var discharge : Float = 4 + as3hx.Compat.parseInt(player.effectiveSensitivity()) / 8;
        if (flags[kFLAGS.ELECTRA_LVL_UP] >= 4)
        {
            discharge += Math.round(player.effectiveSensitivity() / 16);
        }
        if (flags[kFLAGS.ELECTRA_LVL_UP] >= 7)
        {
            discharge += Math.round(player.effectiveSensitivity() / 16);
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        ...
        else if (flags[kFLAGS.ELECTRA_FOLLOWER] >= 2)
        {
            electraScene.PlayerSexElectraPostSpar();
        }
        else
        {
            electraScene.PlayerSexElectra();
        }
    }
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (player.hasStatusEffect(StatusEffects.RiverDungeonA))
        {
            SceneLib.dungeons.riverdungeon.defeatedByRaiju();
        }
        else
        {
    ...
    public function new()
    {
        ...
        else if (flags[kFLAGS.ELECTRA_LVL_UP] < 2)
        {
        ...
        else
        {
            ...
            var electraLvl : Float = flags[kFLAGS.ELECTRA_LVL_UP] - 1;
        ...
        this.lustVuln = .8;
        this.gems = 500;
        this.drop = new ChainedDrop().add(armors.INDESSR, 1 / 10).add(consumables.L_DRAFT, 1 / 4).add(consumables.VOLTTOP, 0.7);
        ...
        if (flags[kFLAGS.ELECTRA_LVL_UP] > 1 || flags[kFLAGS.ELECTRA_TALKED_ABOUT_HER] > 1)
        {
