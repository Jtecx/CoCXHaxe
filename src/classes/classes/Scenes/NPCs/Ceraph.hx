...
class Ceraph extends Monster
{
    ...
    private function ceraphSpecial1() : Void
    {
        ...
        else
        {
            ...
            if (statusEffectv1(StatusEffects.Uber) == 0)
            {
                ...
                if (rand(2) == 0)
                {
                    outputText("The demonic hermaphrodite begins forging demonic symbols in the air before her, each glowing brilliant pink before they blur away in a haze.");
                }
                else
                {
                    outputText("The demonette makes obscene motions with her hands, as if masturbating an imaginary cock or vagina while her hands are wreathed in pink flames.");
                }
    ...
    //[Mini-cum] – takes place of double-attack if very horny
    {
        ...
        {
            damage = as3hx.Compat.parseInt((eBaseDamage()) - player.armorDef);
            if (damage > 0)
            {
            ...
            if (damage <= 0)
            {
                ...
                if (player.armorDef < player.armorDef)
                {
