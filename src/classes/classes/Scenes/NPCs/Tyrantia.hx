...
class Tyrantia extends Monster
{
    private function lustFromHits() : Float
    {
        return (25 + as3hx.Compat.parseInt(player.effectiveLibido() / 5 + player.effectiveSensitivity() / 5 + player.cor / 5));
    }
    private function lustzerkerBoost() : Float
    {
    ...
    private function tyrantiaBasicAttack() : Void
    {
        ...
        {
            dmg0 *= (1 + (0.1 * (flags[kFLAGS.TYRANTIA_LVL_UP] - 1)));
        }
    ...
    private function tyrantiaWebbing() : Void
    {
        ...
        else
        {
            ...
            if (!player.immuneToAcid())
            {
                ...
                {
                    player.createStatusEffect(StatusEffects.AcidDoT, 5, 10, 0, 0);
                }
            }
            player.buff("Goop Web").addStats({
                                        spe : -25
                                    }).withText("Goop Web").combatPermanent();
        }
    }
    ...
    private function tyrantiaCharge() : Void
    {
        if ((rand(player.spe) > this.spe / 2))
        {
            outputText("As the giantess backs up, you brace yourself. She charges, and you throw yourself to one side, narrowly avoiding her charge.");
        }
        else
        {
            ...
            if (hasPerk(PerkLib.GrandTactician) && this.inte >= 150)
            {
                if (this.inte <= 300)
                {
                    critChance += as3hx.Compat.parseInt((this.inte - 150) / 5);
                }
                if (this.inte > 300)
                {
                    critChance += 30;
            ...
            if (flags[kFLAGS.TYRANTIA_LVL_UP] >= 2)
            {
                dmg2 *= (1 + (0.2 * (flags[kFLAGS.TYRANTIA_LVL_UP] - 1)));
            }
            ...
            {
                if (hasPerk(PerkLib.NaturaljousterMastergrade))
                {
                    dmg2 *= 5;
                }
                else
    ...
    private function tyrantiaPounceFail() : Void
    {
        ...
        if (flags[kFLAGS.TYRANTIA_LVL_UP] >= 2)
        {
            dmg3 *= (1 + (0.1 * (flags[kFLAGS.TYRANTIA_LVL_UP] - 1)));
        }
    ...
    override private function outputDefaultTeaseReaction(lustDelta : Float) : Void
    {
        if (lustDelta == 0)
        {
    ...
    override private function performCombatAction() : Void
    {
        ...
        {
            if (statusEffectv1(StatusEffects.Lustzerking) > 1)
            {
                addStatusValue(StatusEffects.Lustzerking, 1, -1);
            }
            else
            {
                this.weaponAttack -= lustzerkerBoost();
        ...
        if (TyrantiaFollower.TyrantiaFollowerStage < 2 || (player.hasStatusEffect(StatusEffects.SparingTyrantia) && player.statusEffectv1(StatusEffects.SparingTyrantia) > 0))
        {
            tyrantiaLustAura();
        }
        //if ()
        //else {
        var choice0 : Float = rand(6);
        switch (choice0)
        {
            case 0:
                tyrantiaBasicAttack();
            ...
            case 3:
                tyrantiaPounce();
            case 4:
                if (!hasStatusEffect(StatusEffects.Lustzerking) && wrath >= 50)
                {
                    tyrantiaLustzerk();
                }
                else
    ...
    {
        if (player.hasStatusEffect(StatusEffects.SparingTyrantia))
        {
            SceneLib.tyrantia.TyrantiaLostSparring(hpVictory);
        }
        else
        {
            SceneLib.tyrantia.postFightOptions(hpVictory);
        }
    }
    ...
    public function new()
    {
        ...
        {
            var mod : Int = as3hx.Compat.parseInt(flags[kFLAGS.TYRANTIA_LVL_UP] - 1);
        ...
        this.drop = new WeightedDrop().add(consumables.B_GOSSR, 1).add(consumables.UNICORN, 1).add(useables.T_SSILK, 3).add(null, 5);
