...
class Holidays extends BaseContent
{
    //Checker functions
    public static function checkDays() : Bool
    {
    ...
    public static function isHeliaBirthday() : Bool
    {
        return flags[kFLAGS.ITS_EVERY_DAY] || date.month == 7;
    }
    public static function isAlvinaBirthday() : Bool
    {
        return flags[kFLAGS.ITS_EVERY_DAY] || date.month == 7;
    }
    public static function isEvangelineBirthday() : Bool
    {
    ...
    public static function gaussEaster(Y : Int) : Array<Dynamic>
    {
        var A : Float;
        var B : Float;
        var C : Float;
        ...
        D = (19 * A + M) % 30;
        E = (2 * B + 4 * C + 6 * D + N) % 7;
        var days : Int = as3hx.Compat.parseInt(22 + D + E);
    ...
    public static function isEaster() : Bool
    {
        ...
        var easter : Array<Dynamic> = gaussEaster(date.fullYear);
        --easter[0];  //decrement month to fit Date class  
        var maxDate : Array<Dynamic> = [];
        ...
        (date.month == maxDate[0] && (!checkDays() || date.date <= maxDate[1]) || date.month < maxDate[0]);
    }
    
    public static function isHalloween() : Bool
    {
        return flags[kFLAGS.ITS_EVERY_DAY] || date.month == 9 && (!checkDays() || date.date >= 28) || date.month == 10 && date.date < 2;
    }
    
    public static function isLunarNewYear() : Bool
    {
    ...
    public function pumpkinFuckEncounter() : Void
    {
        ...
        {
            outputText("  An old, haunting memory surfaces, and you realize this seems... familiar.  Did you find this last year?  Yes... it could have been this very same pumpkin...");
        }
        ...
        function pumpkinFuck() : Void
        {
            ...
            if (player.hasBalls())
            {
                outputText("balls deep ");
            }
            else
            {
                outputText("hilt deep ");
            }
            ...
            if (player.cumQ() < 200)
            {
                outputText("Your weak load disappears into the plant's sucking gullet, wicked away as fast as you fire.");
            }
            else if (player.cumQ() < 1000)
            {
                outputText("Your thick load disappears into the plant's sucking gullet almost as fast as you're firing it, though you can see a bit of white creampie forming around the edges of your cock.");
            }
            else
            {
            ...
            else
            {
                ...
                {
                    outputText("brandishing your fists");
                }
                else
                {
                    outputText("picking up your [weapon]");
                }
        ...
        var pumpkinFuckPartII : Void->Void = function() : Void
        {
            clearOutput();
            ...
            {
                outputText("I know what you were, virile suitor.  ");
            }
        ...
        var pumpkinFuckPartIII : Void->Void = function() : Void
        {
            clearOutput();
            ...
            if (player.hasBalls())
            {
                outputText("pats your sloshing sack");
            }
            else
            {
                outputText("caresses a sensitive spot beneath your dick");
            }
        ...
        function mountPumpkin() : Void
        {
            ...
            if (player.isPregnant() || (player.wetness() < 2 && player.totalFertility() < 30))
            {
                ...
                (try cast(player, Player) catch(e:Dynamic) null).cuntChange(50, true, false, false);
        ...
        var pumpkinMountingLevelII : Void->Void = function() : Void
        {
            clearOutput();
            ...
            {
                outputText(", her free hand working your [fullChest] with glee");
            }
        ...
        var mountPumpkinsIII : Void->Void = function() : Void
        {
            clearOutput();
    ...
    public function feraSceneTwoIntroduction() : Void
    {
        ...
        function exploreFerasWonderland() : Void
        {
            ...
            if (player.gender > 0)
            {
                outputText("  Your heart is hammering");
            }
            if (player.hasCock() && !player.hasVagina())
            {
                outputText(", and there's a telltale tightness building in your groin, expanding and pressing oh-so-hard on your [armor]. You look up at those sweet lips and ponder what they would feel like wrapped around your [cocks], a thought that only makes your gear that much harder to bear as it constrains you.");
            }
            else if (player.hasVagina() && !player.hasCock())
            {
                outputText(", and there's a telltale heat building in your groin, practically dripping out to stain your [armor] with your leaking girl-honey. You look up at those leaky stamen and idly wonder what it would feel like to let one or two inside you, stirring your insides with their sticky-sweet lovejuices.");
            }
            else if (player.hasVagina() && player.hasCock())
            {
                outputText(", and there's a telltale sticky tightness in your [armor], keeping your [cocks] tightly compressed while your [vagina] practically juices itself all over everything, so hot, wet, and unmistakably eager to feel something inside of it. You look up at those leaky stamen and the plump lips between them and wonder what it would feel like to stick yourself in between the lips and let the stamen go to town on your exposed holes.");
            }
            else
            {
            ...
            if (player.inte >= 40)
            {
                ...
                if (player.hasCock())
                {
                    addButton(0, "Continue", fuckDemCorruptFlowerLips);
                }
                else
                {
                    addButton(0, "Continue", stickAStamenInIt);
                }
            ...
            else
            {
                ...
                if (player.hasCock())
                {
                    addButton(0, "Next", fuckDemCorruptFlowerLips);
                }
                else
                {
                    addButton(0, "Next", stickAStamenInIt);
                }
            }
            if (player.hasCock() && player.hasVagina())
            {
        ...
        function fuckDemCorruptFlowerLips() : Void
        {
            ...
            if (player.hasBalls())
            {
                outputText("[balls] against the cushy lower lip, wreathing them in syrupy sap or nectar.  You're not really sure what the appropriate name for such a substance is, and you're well past caring.");
            }
            else
            {
                outputText("loins against the cushy, sap-slicked lips, compressing their hot, smooth surfaces against your [skin].  You're not really sure if sap is the appropriate name for such a substance, but you're far beyond caring at this point.");
            }
            ...
            outputText(" buried in, so hot, wet, and soft. With each passing moment, you become more relaxed, more open and exposed, and more aroused than before. You would probably be afraid if you were capable of any kind of reason."
                    + "\n\nA perfectly smooth, perfectly dry hand traces down your back in a comforting yet sensual gesture, followed a moment later by another upon your [butt]. It squeezes you, kneading your cheek in its supple, dextrous fingers, letting long, sharp nails just barely dig into your "
                    + player.skinFurScales()
                    + ". Then the hands move down to your [legs] to continue their massage, dodging around the tentacle-like stamen so that they can reach your [feet]. There, the hands stop and separate from your body, vanishing until a familiar face leans in front of you, interrupting the slew of mind-tranquilizing colors briefly."
        ...
        var dickBoostingFuntimes : Void->Void = function() : Void
        {
            clearOutput();
            ...
            if (player.hasBalls())
            {
                outputText("cups your [sack] tenderly");
            }
            else
            {
                outputText("tickles your taint");
            }
            ...
            for (x in 0...player.cocks.length)
            {
        ...
        function stickAStamenInIt() : Void
        {
            ...
            if (!player.hasCock())
            {
                outputText(", or a libidinous dick-girl, sowing your seed everywhere you can and saving your cunt for only the strongest of mates");
            }
            outputText("?</i>\""
            ...
            if (!player.hasCock())
            {
                addButton(2, "Dick", growADickForFera);
            }
        }  //Gape  ;
        
        
        
        var gapeMePleaseFera : Void->Void = function() : Void
        {
            clearOutput();
            ...
            if (player.vaginas[0].vaginalLooseness >= VaginaClass.LOOSENESS_LEVEL_CLOWN_CAR)
            {
                outputText(", spreading its well-widened lips as wide as they've ever been");
            }
            else if (player.vaginas[0].vaginalLooseness >= VaginaClass.LOOSENESS_GAPING)
            {
                outputText(", spreading your lips well beyond the widest they've ever been and still farther.");
            }
            else if (player.vaginas[0].vaginalLooseness >= VaginaClass.LOOSENESS_LOOSE)
            {
                outputText(", meeting a bit of token resistance at your unprepared entryway and slithering stamen after stamen inside, the forerunners widening you until the entirety of the column can pass inside, gaping you almost inhumanly wide.");
            }
            else
            {
                outputText(", meeting more than a little resistant at your unstretched entry and finding a way in anyhow. First one tentacle slithers in, then a second, then a fourth, and so on. One after another, they spread you wider and wider, opening you until the entire wiggling column is buried deep, pulling your lips ridiculously wide.");
            }
            ...
            if (player.wetness() < 4)
            {
                outputText(" even though you're not a squirter");
            }
        ...
        function gapeByFeraEpilogue() : Void
        {
            ...
            {
                outputText(", but you'd rather fuck something that'll get you good and pregnant with a well-hung fuck-monster, like a minotaur!");
            }
            else
            {
                outputText(".");
        ...
        var tightLikeAToigar : Void->Void = function() : Void
        {
            clearOutput();
            ...
            if (player.vaginas[0].vaginalLooseness >= VaginaClass.LOOSENESS_LEVEL_CLOWN_CAR)
            {
                outputText(", slipping past your capacitive entrance with ease");
            }
            else if (player.vaginas[0].vaginalLooseness >= VaginaClass.LOOSENESS_GAPING)
            {
                outputText(", sliding into your welcoming entrance fairly easily");
            }
            else if (player.vaginas[0].vaginalLooseness >= VaginaClass.LOOSENESS_LOOSE)
            {
                outputText(", slowly working its way into your decently unstretched entrance");
            }
            else
            {
                outputText(", slowly burrowing past your tight, restrictive entrance");
            }
        ...
        var growADickForFera : Void->Void = function() : Void
        {
            clearOutput();
            ...
            {
                outputText(" and press your tail to the ground");
            }
            outputText(".  You feel the wetness of the hollow tube as it presses against you along with a lance of pure desire and heat in the very center, undoubtedly caused by the tainted needle as it slips through your unprotected flesh.  The thing is compressed so tightly against you that it looks like the entire injector was able to bury itself in your body."
                    ...
                    + "[pg]Pulling back, the tentacle's cup-like tip stretches away from your body a few inches, its base still sealed tight to your "
                    + player.skinFurScales()
            ...
            if (as3hx.Compat.parseInt(Math.random() * 2) == 0)
            {
        ...
        var afterFeraGivesYouACawk : Void->Void = function() : Void
        {
            clearOutput();
    ...
    // Encounter Chance 1 out of 40 and only if you're a centaur
    {
        ...
        var approachPonies : Void->Void = function() : Void
        {
            clearOutput();
            ...
            if (player.weaponName != "fists")
            {
                outputText(", especially armed strangers");
            }
            outputText(".\n\n"
        ...
        var derpPolitely : Void->Void = function() : Void
        {
            clearOutput();
        ...
        var derpCreepy : Void->Void = function() : Void
        {
            clearOutput();
        ...
        var derpyParty : Void->Void = function() : Void
        {
            clearOutput();
    ...
    //DLC April Fools
    {
        ...
        if (nextFunc != null)
        {
            addButton(1, "No", nextFunc);
        }
        else
        {
            addButton(1, "No", EventParser.playerMenu);
        }
        
        var buyDLCPrompt : String->String->Function->Void = function(dlcName : String, dlcPrice : String, nextFunc : Function = null) : Void
        {
            clearOutput();
        ...
        var proceedToCheckout : String->Function->Void = function(method : String, nextFunc : Function = null) : Void
        {
            clearOutput();
        ...
        var reallyCheckout : Void->Void = function() : Void
    ...
    //Introduction: -McGirt
    {
        ...
        var letZeTurkeyApproach : Void->Void = function() : Void
        {
            sceneHunter.selectGender(maleLetZeTurkeyApproach, femaleLetZeTurkeyGalApproach, null, null, 0);
        }
        
        var maleLetZeTurkeyApproach : Void->Void = function() : Void
        {
            clearOutput();
        ...
        }  //(Needs friendly/lover Hel)    //SPITROAST HER! GangHel Style  ;
        
        
        
        
        
        var spitRoastThatTurkey : Void->Void = function() : Void
        {
            clearOutput();
        ...
        }  //{Thanksgiving Day event, nat. Cockmeisters only.}    //Turkey Girl II: Return of the Cockgobbler (Cockwielders)  ;
        
        
        
        
        
        var turkeyGirlTwoTheTurkeningBySavinWhatADickInAButt : Void->Void = function() : Void
        {
            clearOutput();
        ...
        function helloAgain() : Void
        {
            ...
            {
                outputText("and down to the base");
            }
            else
            {
                outputText("and swallowing your shaft until her throat is bulging with your massive manhood");
            }
        ...
        function gravyBoatDatTurkturk() : Void
        {
            ...
            {
                outputText(", her stomach bulging obscenely with your inhuman girth");
            }
            outputText("."
        ...
        function seasonHerDatTurkeyBitch() : Void
        {
            ...
            {
                outputText("Tamani's dildo");
            }
            else if (player.hasItem(consumables.L_DRAFT))
            {
                outputText("a bottle of Lust draft");
            }
            else if (player.hasItem(consumables.F_DRAFT))
            {
                outputText("a bottle of Fuck draft");
            }
            else
            {
                outputText("A FUCKIN ERROR");
            }
        ...
        var turkeyDesertBitches : Void->Void = function() : Void
        {
            clearOutput();
            ...
            if (player.cockTotal() == 2)
            {
                outputText(" with both cocks, one ramming into her abused cunny and the other right up her vulnerable little bumhole");
            }
            else if (player.cockTotal() > 2)
            {
                outputText(" with every cock you've got, half in the pink and half in the stink, spreading her as wide as you are in both barrels");
            }
            ...
            {
                outputText("your hips are slapping into hers, making her butt bounce with the impact");
            }
            ...
            if (player.cockTotal() > 1)
            {
                outputText(" and quivering behind");
            }
            ...
            if (player.cockTotal() > 1)
            {
                outputText("s deposit the last of their");
            }
            else
            {
                outputText(" deposits the last of its");
            }
    ...
    //Ham Roast: Getting All Up In A Honey Pot (Female and Genderless only)
    {
        ...
        outputText("As you enter The Wet Bitch you’re accosted by an incredibly busty waitress. You can’t really discern what exactly she is as your eyes are solely focused on the massive melons jutting out from her chest and jiggling proudly with mile deep cleavage to boot."
                + "\n\n\"<i>Hey there "
                + player.mf("handsome", "cutie")
                + "! We've got quite a unique special going on right now!</i>\" she exclaims in a cheery voice."
        ...
        if (player.gems >= 1)
        {
            addButton(0, "Throw gem", getARoastPiggueOinkOinkOinkMotherfucker);
        }
        ...
        }  //Ham Roast -> In For A Gem    //{Return to The Wet Bitch menu}  ;
        
        
        
        
        
        var getARoastPiggueOinkOinkOinkMotherfucker : Void->Void = function() : Void
        {
            clearOutput();
            player.gems -= 1;
        ...
        }  //Ham Roast -> Tenderize Her (Female and Genderless versions)    //[ Tenderize Her ]  ;
        ...
        function tenderizeDatPigSloot() : Void
        {
            ...
            if (player.tailType > Tail.NONE)
            {
                outputText("You even bring your tail from behind your back to join in, the unfamiliar texture causing the sausage slut to let out surprised, high-pitched squeaks. ");
            }
        ...
        var hamRoastTenderizeHerSoftcore : Void->Void = function() : Void
        {
            clearOutput();
        ...
        var spankEpilogueColonYoureAHorriblePersonForAbusingADumbPigGirl : Void->Void = function() : Void
        {
            clearOutput();
        ...
        var spankEpilogueForRizzles : Void->Void = function() : Void
        {
            clearOutput();
        ...
        var getDoneEatingOutPigSlut : Void->Void = function() : Void
        {
            clearOutput();
            ...
            if (player.biggestTitSize() < 20)
            {
                ...
                {
                    player.growTits(3, player.bRows(), false, 1);
                }
                if (player.biggestTitSize() < 6)
                {
                    player.growTits(2, player.bRows(), false, 1);
                }
            ...
            if (changed)
            {
                outputText(", feeling your body expand as you waddle out back towards camp with belly full of sweet syrup");
            }
    ...
    public function crazyVDayShenanigansByVenithil() : Void
    {
        ...
        var helpValentinesDayII : Void->Void = function() : Void
        {
            clearOutput();
            ...
            {
                outputText("  The bearer of it seems to just come here to look uncomfortably but, perhaps, longingly at the stall for a few moments, before practically leaping back into the Wet Bitch.");
            }
            else
            {
                outputText("  You see a flash of said fur as the person with said characteristic moves away from the stall, clearly carrying a confection or two and some other pink, red, or heart-shaped things into the Wet Bitch.");
            }
        ...
        function helpValentinesDayIII() : Void
        {
            ...
            if (player.cockThatFits(28) >= 0 || player.hasVagina())
            {
                addButton(2, "Pastie", goVisitPastyVDay);
            }
        ...
        var goVisitScyllaVdayII : Void->Void = function() : Void
        {
            clearOutput();
        ...
        }  //{This option doesn't require any sexual endowments whatsoever}    //Cuddle]  ;
        ...
        function cuddleWithScyllaVDay() : Void
        {
            ...
            if (player.hasVagina())
            {
                outputText("  Your [vagina] is starting to get moist, making a small mess in your underclothes.");
            }
            //[if (hasCock = true)]
            if (player.hasCock())
            {
                outputText("  You realize it'd probably be a done deal if you didn't resist the pull of your hardening cock towards one of Scylla's orifices, but you decide to spare her any temptations and endure yourself.");
            }
        ...
        var makeOutWithScyllaVDay : Void->Void = function() : Void
        {
            clearOutput();
            ...
            if (player.cockTotal() == 1)
            {
                outputText("your [cock] is completely hard and stiff, forming a bulge in your clothing");
            }
            else if (player.cockTotal() > 1)
            {
                outputText("[eachCock] is by now completely hard, eager to be touched and wanting to be caressed by the nun you're kissing");
            }
            outputText("."
        ...
        function goVisitAbby() : Void
        {
            ...
            {
                outputText("\"<i>Well, that's unexpected. I have certain doubts about you showing me such a good time,</i>\" she says.  You tell her you don't actually require a cock at all to do this, and she seems surprised.");
            }
        ...
        function fuckAbbyVDay() : Void
        {
            ...
            endEncounter(120);
        }  //{This option doesn't require any sexual endowments whatsoever}    //{PLEASURE HER}  ;
        ...
        function pleasureAbbyVDay() : Void
        {
            ...
            {
                outputText("  Your fingers make it into her pussy alongside your tongue, and the prehensile thing, longer and more flexible than a human's, ravages the insides of her pussy, tasting everywhere as Abby shakes in her orgasm.  Her pussy eagerly tries to milk both your fingers and your tongue, clenching and gripping at them as it drips her pussy juices all over your face.  Abby screams out in a loud, uninhibited orgasm at last, expressing her ecstasy.");
            }
            //([Demon/Dragon Tongue]
            else if (player.hasLongTongue())
            {
                outputText("  Abylon shakes in excitement, pleasure, and fear as you extend your inhuman tongue to reach deep into the recesses of her pussy.  She cannot hold out a moment longer, her cunt gripping your inhuman muscle tightly, the hole and muscles inside quivering as you can taster her orgasmic juices with every single point of your tongue.  She screams, shudders and moans, tongue lolling out from the intense pleasure as you force your own tongue to roll around inside her and stretch her in multiple ways. Only you holding her stops Abylon from slumping forward, it seems...");
            }
            //([Human tongue]
            else
            {
                outputText("  Hooking your fingers at the front and back of her cunny, you plunge your tongue in and wiggle it eagerly.  Abby clamps down on all three invaders and loudly moans out, screaming to her content at last, as your face is flooded with pussy juice.");
            }
        ...
        var goVisitPastyVDay : Void->Void = function() : Void
        {
            clearOutput();
            ...
            {
                menu();
                if (player.hasItem(consumables.BIMBOCH))
                {
                    addButton(0, "BimboCham", pastieValentineIntro, consumables.BIMBOCH.id);
                }
                if (player.hasItem(consumables.BC_BEER))
                {
                    addButton(1, "B.Cat Beer ", pastieValentineIntro, consumables.BC_BEER.id);
                }
                if (player.hasItem(consumables.GOB_ALE))
                {
                    addButton(2, "Gob. Ale", pastieValentineIntro, consumables.GOB_ALE.id);
                }
                if (player.hasVagina())
                {
                    addButton(3, "Pussy", pastieValentineIntro, "vag");
                }
            }
            else
            {
        ...
        var valentineDrinkPastie : ItemType->Void = function(itype : ItemType) : Void
        {
            clearOutput();
            ...
            if (player.cor < 33)
            {
                outputText("  It probably isn't.");
            }
            //[Corruption 21-75]
            else if (player.cor < 75)
            {
                outputText("  You guess as long as she's fine afterwards, it should be okay.");
            }
            //[Corruption 76-100]
            else
            {
                outputText("  As long as she's in the mood to drink more than just the alcohol afterwards, you don't really care.");
            }
        ...
        //{Black Cat Beer}
        {
            ...
            menu();
            if (player.cockThatFits(8) >= 0)
            {
                addButton(0, "Fuck", fuckPastieForVDay);
            }
            if (player.hasCock())
            {
                addButton(1, "RubOnDick", rubPastieOnYourWangDawg);
            }
            if (player.hasVagina())
            {
                addButton(2, "Go In Pussy", goForAPushayDivePasty);
            }
        }  //[Fuck]  ;
        ...
        function rubPastieOnYourWangDawg() : Void
        {
            ...
            if (player.hasBalls())
            {
                outputText("  Your [balls] lurch, [skin] contracting as liquid passion rises within you.");
            }
            ...
            if (player.cumQ() < 250)
            {
                outputText(" easily drenching the tiny fairy, in plump pearls up and down her naked flesh.");
            }
            else if (player.cumQ() < 750)
            {
                outputText(" leaving her and the arm you're holding her in dripping with a sticky alabaster lacquer.");
            }
            //high cum production:
            else
            {
                outputText(" almost blasting her right out of your hand in the torrent of white that spills forth from your mighty member.");
            }
        ...
        var goForAPushayDivePasty : Void->Void = function() : Void
        {
            clearOutput();
            ...
            if (player.hasCock())
            {
                outputText("your other hand moving downwards to tug on [oneCock], ");
            }
            ...
            if (player.hasCock())
            {
                outputText("  All the while, you continue to jerk off [eachCock], preparing yourself for an orgasm on both sides.");
            }
            ...
            if (player.hasCock())
            {
                outputText("  At the same time, your cock deposits its own load onto the ground as your hermaphrodite genitals achieve a simultaneous orgasm.");
            }
            //[if (isSquirter = true)
            if (player.wetness() >= 4)
            {
                outputText("  She doesn't have much to say in the matter when your female ejaculation truly shoots in, though, sending her rolling out of your [vagina] and onto the ground.");
            }
    ...
    public function xmasBitchEncounter() : Void
    {
        ...
        if (flags[kFLAGS.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE] > 0)
        {
            ...
            {
                outputText("Kelt, though he's getting coal AGAIN");
            }
            else
            {
                outputText("Kelly, though I think she's getting another big fat dildo");
            }
        ...
        else
        {
            ...
            if (flags[kFLAGS.KELT_BREAK_LEVEL] < 4)
            {
                outputText("Kelt, though he's getting coal this year");
            }
            else
            {
                outputText("Kelly, though I think she's getting a big fat dildo this year");
            }
            outputText(".</i>\"\n\n");
        }
        ...
        function openXmasPresent() : Void
        {
            ...
            //Great present!{
                ...
                {
                    SceneLib.inventory.takeItem(CoC.instance.armors.SSARMOR, EventParser.playerMenu);
                }
                flags[kFLAGS.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE] = date.fullYear;
            }
        ...
        var unwrapElfyPresent : Void->Void = function() : Void
        {
            CoC.instance.spriteSelect(SpriteDb.s_christmas_elf);
            clearOutput();
            ...
            {
                outputText("reaches up and presses herself against you seductively, caressing your body.");
            }
            else
            {
                outputText("cuddles up alongside, happy to have someone of similar stature to seduce.");
            }
        ...
        var futaDickXmasElfClr : Void->Void = function() : Void
        {
            clearOutput();
            dickXmasElfGo();
        }
        
        var futaVagXmasElfClr : Void->Void = function() : Void
        {
            clearOutput();
            vagFuckXmasElf();
        }
        ...
        var dickXmasElfGo : Void->Void = function() : Void
        //(Dickfuck)
        {
        ...
        var dickXmasElfGo_fits : Void->Void = function() : Void
        {
            ...
            if (player.cumQ() >= 250)
            {
                outputText("  The spunk quickly fills her womb and flows back out her entrance, frothing and bubbling as it drips over your " + Appearance.hipDescription(player) + ".  ");
            }
            if (player.cumQ() >= 1000)
            {
                outputText("  It begins to splatter everywhere as you pour out more and more, the elf looking on with amusement as her cunt turns into a cum-spraying fountain.  ");
            }
            outputText("\n\n"
            ...
            if (player.cumQ() >= 250)
            {
                outputText("dumping out more and more waves of jizm until you're lying in a deep puddle of it, ");
            }
            outputText("and somehow the orgasm lasts on even after you've emptied.   Your eyes roll back for a moment as you nearly lose consciousness.\n\n"
                    + "You snap out of it and the elf is smirking down at you, stationary.  You feel totally numb – your [cock] has all of your sensation, all of your sense of touch.   On their own, your "
                    + Appearance.hipDescription(player)
        ...
        //(TOO BIG)
        {
            ...
            if (player.biggestTitSize() >= 2)
            {
                outputText("making them jiggle");
            }
            else
            {
                outputText("making her tight butt give a tiny jiggle");
            }
            ...
            if (player.hasBalls())
            {
                outputText("r " + Appearance.ballsDescription(true, true, player));
            }
            ...
            if (player.cumQ() < 500)
            {
                outputText("You recoil from shooting out so much – it's far and away more than you're normally capable of.  ");
            }
        ...
        var xmasFuckFollowupFems : Void->Void = function() : Void
        {
            hideUpDown();
            clearOutput();
        ...
        var xmasPerkM : Bool->Void = function(cumQ : Bool) : Void
        {
            if (!player.hasPerk(PerkLib.ElvenBounty))
            {
                if (cumQ)
                {
                    player.createPerk(PerkLib.ElvenBounty, 250, 0, 0, 0);
                }
                else
                {
        ...
        var xmasSmart : Void->Void = function() : Void
        {
            hideUpDown();
            clearOutput();
    ...
    {
        if (flags[kFLAGS.GATS_ANGEL_QUEST_BEGAN] == 0)
        {
            christmasEncounterAngelJunk();
        }
        else if (player.hasKeyItem("North Star Key") < 0)
        {
            findTheWindUpKey();
        }
        else
        ...
        var christmasEncounterAngelJunk : Void->Void = function() : Void
        {
            clearOutput();
            menu();
            //[Not Christmas]
            ...
            {
                //[Must Be Christmas Time]  //[Explore > High Mountains]  {
        ...
        function findTheWindUpKey() : Void
        {
            ...
            if (player.tallness > 60)
            {
                outputText("  You can't even stand straight up below the low-hanging ceiling, forced to carefully tread along on your knees over the slick, hard rock.");
            }
            ...
            doNext(explorer.done);
        }  //Activates upon returning to the High Mountains after the previous scene, before X amount of hours or X amount of time past Dec 25th (presumably)    //ii. Bringing the Key Back  ;
        ...
        function theWorstEndingEverDotJPG() : Void
        {
            ...
            if (player.cor > 49)
            {
                outputText("  It seems like the best response to you for now.  Based on the giantess's sobbing soliloquy, you figure that standing in for the love of the soldier's life will get you straight to the action.");
            }
            else
            {
                outputText("  Wait, what?  It appears that in your desperation to contain the situation, your lingering doubt was enough to get you blurt out a bold faced lie.  You figure that you may as well roll with it for now.");
            }
            ...
            if (player.hasCock())
            {
                outputText("You slide the key playfully down your wrist, rubbing your [cock] with your free hand.  You aren't entirely sure if he'll even feel a sensation from your penetration, but perhaps it'll at least get him in the mood.  With your manhood at full mast, you gently skirt along the edges of the keyhole.");
            }
            else
            {
                outputText("A mischievous tongue traces along the smooth, perfect surface of the windup key.  You promptly lower the makeshift toy precariously in front of your [cunt]; if there was ever a time to make a wind up pun or remark as some sort of goofy foreplay, now would be the time.");
            }
            ...
            if (player.cor > 49)
            {
                outputText("  ...Well, that was a waste of time.");
            }
            ...
            if (player.cor > 49)
            {
                outputText("  More importantly, you have some unsatisfied, pent up lust that you'd like to expend.");
            }
            player.dynStats("lus", 2 + player.lib / 10 + player.cor / 10, "cor", 10, "scale", false);
            doNext(explorer.done);
            flags[kFLAGS.GATS_ANGEL_DISABLED] = 1;
            player.removeKeyItem("North Star Key");
        }  //Requirements: Choosing [Fuck the Angel] at the prompt    //7. Corrupt End #2  ;
        ...
        var fuckTheAngelBadEnd : Void->Void = function() : Void
        {
            clearOutput();
            ...
            {
                outputText(Appearance.vaginaDescript(player, 0));
            }
            ...
            if (player.hasCock())
            {
                outputText("hot, white seed");
            }
            else
            {
                outputText("feminine juices");
            }
            ...
            {
                outputText(Appearance.vaginaDescript(player, 0) + " throbs");
            }
        ...
        var goodEndForChristmasMiracleICant : Void->Void = function() : Void
        {
            clearOutput();
        ...
        function stayForPretendNTRGoodWinjobSex() : Void
        {
            ...
            if (player.lust >= player.maxOverLust())
            {
                ...
                if (player.wetness() >= 4 || player.cumQ() >= 1000)
                {
                    outputText(" - figuratively, and literally, as well, as your fluids drip off the edge to add to the rain of sexual fluids the giantess is laying in the midst of");
                }
                outputText(".");
            }
        ...
        var conclusionOfGiantessGoodEnd : Void->Void = function() : Void
        {
            clearOutput();
    ...
    //Takes place when exploring the mountain.
    {
        ...
        var jizzSnowOnMyCampPlease : Void->Void = function() : Void
        {
            clearOutput();
    ...
    //Back in camp
    {
        ...
        //Pure{
            ...
            else
            {
                ...
                if (player.hasCock())
                {
                    addButton(0, "Pen Ass", penetradeCorruptJojoAss);
                }
                if (player.hasVagina())
                {
                    addButton(1, "Ride Cock", rideCorruptJojoCock);
                }
        ...
        {
            //less than 5!if (SceneLib.amilyScene.amilyFollower() && flags[kFLAGS.JACK_FROST_PROGRESS] <= 5)
            {
                ...
                if (!SceneLib.amilyScene.amilyCorrupt())
                {
                    ...
                    if (player.hasCock())
                    {
                        addButton(0, "Penetrate", penetradePureMouseWaifu);
                    }
                ...
                else
                {
                    ...
                    if (player.gender > 0)
                    {
                        ...
                        if (player.hasCock())
                        {
                            ...
                            if (player.hasBalls())
                            {
                                outputText(", even tickling your [balls]");
                            }
                            else if (player.hasVagina())
                            {
                                outputText(", even tickling your [cunt] for added tantalization");
                            }
                            outputText("."
            ...
            else if (flags[kFLAGS.JACK_FROST_PROGRESS] <= 7 && player.hasStatusEffect(StatusEffects.CampMarble))
            {
                ...
                if (flags[kFLAGS.MARBLE_KIDS] > 0 && flags[kFLAGS.MARBLE_NURSERY_CONSTRUCTION] >= 100)
                {
                    ...
                    if (player.str >= 85)
                    {
                        ...
                        if (flags[kFLAGS.MARBLE_KIDS] > 1)
                        {
                            outputText("the girls");
                        }
                        else
                        {
                            outputText("your daughter");
                        }
            ...
            {
                //if she ain't a follower and is a lover, just have her visit.if (flags[kFLAGS.JACK_FROST_PROGRESS] <= 9 && (SceneLib.helScene.fuckBuddyHel() || (SceneLib.helFollower.followerHel() && !player.hasStatusEffect(StatusEffects.HeliaOff))))
                {
                    ...
                    if (player.spe / 20 + Utils.rand(20) > 11)
                    {
                        outputText("\n\nYou're surprised by the sudden lunge, but thankfully your well trained reflexes means you quickly duck under the pouncing salamander and watch as she falls face-first into the snow behind you with an audible \"<i>Oof!</i>\""
                                + "\n\nShe laughs as she surfaces amidst a spray of slush, whipping her long hair around.  \"<i>That's the way to do it, lover-"
                                + player.mf("boy", "girl")
                                + "!</i>\" she crows.");
                    }
                    else
                    ...
                    if (player.hasCock())
                    {
                        outputText("  A torrent of seed splashes against Helia's taut belly, as [eachCock] throbs and spews its seed while sandwiched between the two of you.");
                    }
                    //(if PC has a pussy)
                    if (player.hasVagina())
                    {
                        outputText("  Your [vagina] quivers and spill your juices with the force of a wave.  Moisture falls on the ground to join in with Helia's own discharge.");
                    }
                ...
                else if (SceneLib.kihaFollower.followerKiha() && flags[kFLAGS.JACK_FROST_PROGRESS] <= 11)
                {
                    ...
                    menu();
                    if (player.hasCock() && player.cockThatFits(67) >= 0)
                    {
                        addButton(0, "Fuck Her", kihaXmasFuck);
                    }
        ...
        function kihaXmasFuck() : Void
        {
            ...
            menu();
            addButton(0, "Next", processJackFrostEvent);
        }  //All sexes!    //[=Lick Boobs and Finger Pussy=]  ;
        ...
        function kihaXmasFingering() : Void
        {
            ...
            menu();
            addButton(0, "Next", processJackFrostEvent);
        }  //Needs PC with cock that fits.    //[=Fuck Her=]  ;
        ...
        function penetradePureMouseWaifu() : Void
        {
            ...
            {
                outputText(", not to mention her dick's pretty hard, too");
            }
            outputText("."
                    + "\n\nSitting down on your bedroll, you open your arms, inviting Amily over.  The mouse promptly approaches");
            if (flags[kFLAGS.AMILY_WANG_LENGTH] > 0)
            {
                outputText(", her own " + SceneLib.amilyScene.amilyCock() + " jutting out in anticipation");
            }
            ...
            if (flags[kFLAGS.AMILY_WANG_LENGTH] > 0)
            {
                outputText(" and painting your belly white with futa-spooge");
            }
            outputText("."
        ...
        function makeOutWithPureMouseWaifu() : Void
        {
            ...
            if (flags[kFLAGS.AMILY_WANG_LENGTH] > 0)
            {
                outputText("Smiling sheepishly you  tell her to mind the erection... though you're not actually looking for sex, you can't help with how cute she looks...  \"<i>Flirt.</i>\"  Amily giggles, but otherwise doesn't comment on it.  ");
            }
        ...
        function penetradeCorruptJojoAss() : Void
        {
            ...
            if (player.cumQ() >= 700)
            {
                outputText(" all over his own cum-engorged gut and ");
            }
        ...
        function rideCorruptJojoCock() : Void
        {
            ...
            if (player.hasCock())
            {
                outputText(", your own futa-cock spewing its load, which you deliberately spray in his face for amusement");
            }
        ...
        }  //Available to all.    //[=Spank and Finger Ass=]  ;
        
        
        
        
        
        var spankNFingerCorruptJojoAss : Void->Void = function() : Void
        {
            clearOutput();
    ...
    public function candyCaneTrapDiscovery() : Void
    {
        ...
        if (player.cor < 50)
        {
            outputText("The memory brings a nostalgic smile to your [face].");
        }
        else
        {
            outputText("The memory reminds you of how bland your world truly was in comparison to Mareth.");
        }
        ...
        function leaveBeforeMeetingCandyCaneBun() : Void
        {
            ...
            {
                outputText("  At least not right now.  You turn back, navigating your way back to your camp.");
            }
            doNext(explorer.done);
        }  //INVESTIGATE  ;
        ...
        var investigateCandyCaneBun : Void->Void = function() : Void
        {
            clearOutput();
            ...
            if (player.cor < 50 && player.lib < 50)
            {
                outputText("  Your eyes are quickly drawn to the rabbit morph's stark nudity.");
            }
            //(If corruption or libido>50)
            else
            {
                outputText("  Your eyes are quickly drawn to the rabbit morph's finely curved ass.");
            }
            ...
            if (player.cor < 50)
            {
                outputText("You calmly inform him that you didn't have the intention of violating him. ");
            }
            //(If corruption>50)
            else
            {
                outputText("Your train of thought is derailed, but you agree to let the bunny believe whatever he wants to.");
            }
            //(If silly mode on)
            if (silly())
            {
                outputText("  You're willing to say anything to calm him down.  This kid's voice is even more shrill than that asshole from Glee!");
            }
            ...
            var helpWithTheCandyCane : Void->Void = function() : Void
            {
                clearOutput();
                ...
                if (player.gender == 1)
                {
                    outputText("[eachCock] rises up, rocking against you.");
                }
                //(If female)
                else if (player.gender == 2)
                {
                    outputText("your [vagina] and [clit] become coated in liberal amounts of juices.");
                }
                //(If herm)
                else if (player.gender == 3)
                {
                    outputText("both of your sexes flare with arousal.");
                }
                //(If genderless)
                else
                {
                    outputText("sweat drips off of you.");
                }
                ...
                if (player.cor < 50)
                {
                    outputText("  Not wanting to disturb him, you make your way back to camp and attempt to ignore the burning fire in your loins.");
                }
                //(If corruption>50)
                else
                {
                    outputText("  Despite your rising urge to take advantage of the unknowing bunny, you decide to retire and return to camp.");
                }
    ...
    //2. Abraxas's Christmas Chicken
    {
        ...
        var talkToXmasChicken : Void->Void = function() : Void
        {
            clearOutput();
            ...
            if (player.gender == 1)
            {
                outputText("How's a blowjob sound to you?");
            }
            //(if female)
            else if (player.gender == 2)
            {
                outputText("How about I eat you out?");
            }
            //(if herm)
            else
            {
                outputText("How about I give you a few minutes to do whatever you want with my mouth?");
            }
            ...
            var acceptChristmasBJs : Void->Void = function() : Void
            //(male)
            {
            ...
            var acceptChristmasManBJ : Void->Void = function() : Void
            {
                clearOutput();
                ...
                else
                {
                    ...
                    if (player.cockTotal() == 1)
                    {
                        outputText("She grabs onto your hips, enabling herself to move all the quicker.");
                    }
                    //(if two cocks)
                    else if (player.cockTotal() == 2)
                    {
                        outputText("With her other hands she starts to work the shaft of the second cock, pumping in time with the motions of her head.");
                    }
                    //(if 3+ cocks)
                    else
                    {
                        outputText("She grabs onto two of your other erect cocks with her hands and begins pumping away at them in time with the motions of her head.");
                    }
                    ...
                    if (player.cumQ() < 250)
                    {
                        outputText("  She closes her eyes and looks quite contented while she swallows your spunk.");
                    }
                    //(else if cum output is high)
                    else if (player.cumQ() < 1000)
                    {
                        outputText("  Semen gushes out of you for what seems like forever, but the harpy doesn't seem to mind.  She closes her eyes and looks quite contented while she swallows your still-torrenting spunk.");
                    }
                    //(else if cum output is ridonkulous)
                    else
                    {
                        outputText("  Though at first she looks like she'll be able to handle it, the river of semen exploding from the rip of your cock doesn't stop.  Her eyes widen as the almost unending torrent of spunk fills her, rushing down her throat and into her belly.  You can almost see her bloating with the stuff before it starts to spill back out, dripping white from her lips around your shaft.");
                    }
                    //(if multicock)
                    if (player.cockTotal() > 1)
                    {
                        outputText("  Your cum sprays over her, coating her skin and plumage in a white to rival their own.  Strands of your semen dripping and running over her hardly even stand out on her bright body.");
                    }
            ...
            var acceptChristmasWomanBJ : Void->Void = function() : Void
            {
                clearOutput();
    ...
    //Short Explanation:
    {
        ...
        //[Player chooses 'Kami']{
            ...
            {
                addButtonDisabled(0, "Give Anal", "Req. cock with area smaller than 70");
            }
            addButton(1, "Receive Anal", takeItRooButtStyle);
            addButton(2, "Give BJ", KamiBlowJob);
            if (player.cockTotal() > 1 && player.cockThatFits(70) >= 0 && player.cockThatFits2(70) >= 0)
            {
                addButton(3, "Doube-Fuck", KamiDoubleDickFuck);
            }
            else
            {
                addButtonDisabled(3, "Doube-Fuck", "Req. 2 cocks with area smaller than 70");
            }
        }
        else
        {
            ...
            if (player.femininity >= 75)
            {
                outputText("feminine ");
            }
            else if (player.femininity <= 25)
            {
                outputText("masculine ");
            }
            ...
            if (silly())
            {
                outputText(" and found space to scrawl a little penis in one corner. Clever");
            }
            outputText("."
        ...
        function chooseLetsGoKami() : Void
        {
            ...
            menu();
            addButton(0, "Give Anal", giveKamiTheChristmasRooAnal).disableIf(!player.cockThatFits(70) < 0, "Req. a cock fitting 70 area.");
            addButton(1, "Receive Anal", takeItRooButtStyle);
            addButton(2, "Give BJ", KamiBlowJob);
            addButton(3, "DoubleFuck", KamiDoubleDickFuck).disableIf(!player.countCocks(-1, 70) < 2, "Req. 2 cocks fitting 70 area.");
        ...
        function giveKamiTheChristmasRooAnal() : Void
        {
            ...
            if (player.hasBalls())
            {
                outputText(", your balls slapping against her with every push");
            }
            ...
            if (player.cumQ() < 1000)
            {
                outputText("  \"<i>Mmm... just like that, babe,</i>\" Kami moans submissively, her asshole clenching one last time before you pull out and climb off your satiated conquest, your " + player.cockDescript(x) + " still dripping creamy white spunk.");
            }
        ...
        function takeItRooButtStyle() : Void
        {
            ...
            if (player.hasVagina())
            {
                outputText("Your cunt oozes in return, envious of the tenderized hole above it.  ");
            }
            ...
            if (player.hasCock())
            {
                outputText("  Your pent-up cock ejaculates there and then, shamelessly gushing sloppy semen against the wall in front of you.");
            }
            //[If female]
            if (player.gender == 3)
            {
                outputText("  Meanwhile, y");
            }
            else if (player.hasVagina())
            {
                outputText("Y");
            ...
            if (player.hasVagina())
            {
                outputText("our eager pussy buckles in arousal, gushing fem-juices on the floor below you.");
            }
        ...
        var KamiBlowJob : Void->Void = function() : Void
        {
            clearOutput();
            //[If player is none-Naga]
            if (!player.isNaga())
            {
                outputText("You get down on your knees");
            }
            //[Else if is Naga]
            else
            {
                outputText("You steer your serpentine half into position, leaning your upper body within arms reach of Kami's wanting member.  You reach out");
            }
            ...
            if (player.hasCock())
            {
                outputText("  You think you might still need to find an outlet for that itch of yours, with [eachCock] still erect and needing release.");
            }
        ...
        var KamiDoubleDickFuck : Void->Void = function() : Void
    ...
    //4. Donto's Polar Pete
    {
        ...
        function unwrapPolarPete() : Void
        {
            ...
            if (player.hasCock())
            {
                outputText("  [EachCock] grows stiff and soaks your [armor] with precum.");
            }
            //(if vagina)
            if (player.hasVagina())
            {
                outputText("  Your [vagina] grows moist, dripping eager juices and soaking your [armor].");
            }
            ...
            if (player.hasCock())
            {
                outputText("  His continued ramming of your prostate mixed with the addictive candy crammed in your mouth sends you over the edge, [eachCock] shooting out ropes of snow-white all over your chest.  Pete eyes you over with a lurid gaze, your face covered in his cum and your body covered in your own.");
            }
            else if (player.hasVagina())
            {
                outputText("  Your [vagina] quivers, the rough thrusting into your [asshole] mixed with the addictive candy in your mouth sending you over the edge.  With a low muffled moan your mound spills your lust over your groin, dripping down to lube Pete's thrusts as well as your [legs].  Pete gives you a lurid gaze.");
            }
        ...
        var leaveDisFukkinPolarBear : Void->Void = function() : Void
        {
            clearOutput();
    ...
    public static function nieveHoliday() : Bool
    {
    ...
    public function nieveBuilding() : Void
    {
        ...
        {
            //Alters Nieve's eyes in scenesif (flags[kFLAGS.NIEVE_STAGE] == 2)
            {
                ...
                menu();
                if (player.hasItem(consumables.COAL___))
                {
                    addButton(0, "Coal", nieveCoalEyes);
                }
        ...
        function nieveCoalEyes() : Void
        {
            ...
            doNext(explorer.done);
        }  //Add to previous text if possible?    //Gems  ;
        ...
        }  //Add to existing text if possible, rather than a new window?    //Snowman  ;
        
        
        
        
        
        var nieveSnowMan : Void->Void = function() : Void
        {
            clearOutput();
    ...
    {
        if (flags[kFLAGS.NIEVE_GENDER] == 1)
        {
            return Man;
        }
        else
    ...
    public function nieveComesToLife() : Void
    {
        ...
        outputText("You awaken with a shiver.  A chill is in the air again, and in the distance you can make out a jolly laugh and jingling bells.  You bolt upright, looking in the direction of the snow pile, expecting a new flurry.  Sadly, there doesn't seem to be any new snow, nor is any coming down.  Frowning, you stand and approach the snow drift."
                + "\n\nThere's nothing to indicate any visitors through the night...  Except, wait...  Your snow"
                + nieveMF("", "wo")
                + "man is gone!  Did someone destroy it while you slept?"
        ...
        if (flags[kFLAGS.NIEVE_GENDER] == 1)
        {
            outputText("  It appears to be male, judging by the modestly sized cock sprouting from its loins.");
        }
        else
        {
            outputText("  It appears to be female, judging by the modestly sized tits on its chest, and the lack of a penis, which is quite a feat in Mareth.");
        }
        outputText("  Pure white hair spills down from its head, almost blending in with the surrounding snow."
                + "\n\nYou ready your [weapon] and give the naked body a poke with your [foot].  "
                + nieveMF("He", "She")
                + " opens one eye with confusion, then the other."
                + "\n\nA soft, \"<i>Whaaa?</i>\" escapes "
                + nieveMF("his", "her")
                + " lips.  \"<i>Where am I?</i>\" "
                + nieveMF("He", "She")
                + " brings a pale blue hand to "
                + nieveMF("his", "her")
                + " head as "
                + nieveMF("he", "she")
                + " surveys the red, blighted landscape surrounding you.  \"<i>This isn't home.</i>\""
                + "\n\nResponding cautiously, in the event this is all an elaborate trick set up by some demon, you ask who "
                + nieveMF("he", "she")
                + " is, and where "
                + nieveMF("he", "she")
                + "'s from."
                ...
                + nieveMF("His", "Her")
                + " eyes, ");
        if (flags[kFLAGS.NIEVE_MOUTH] == "gems")
        {
            outputText("glittering purple");
        }
        else
        {
            outputText("coal black");
        }
        ...
        function yesKeepNieve() : Void
        {
            ...
            if (silly())
            {
                outputText("  A free follower, no worries about buying worthless dye or raising their affection?  Fuck yeah, you'll take " + nieveMF("him", "her") + "!");
            }
        ...
        var noNoKeepNieve : Void->Void = function() : Void
        {
            clearOutput();
    ...
    //6:00
    {
        ...
        {
            outputText("Nieve is sitting cross-legged in the snowdrift, munching on what looks to be icicles.  As you watch, " + nieveMF("he", "she") + " reaches down into the glittering powder surrounding " + nieveMF("him", "her") + " and produces another one.");
        }
        //7:00
        else if (CoC.instance.model.time.hours == 7)
        {
            outputText("Nieve is giggling and laughing as a fresh flurry flutters down on " + nieveMF("him", "her") + ".  You wonder idly if " + nieveMF("he", "she") + "'s causing it to happen.");
        }
        //8:00
        else if (CoC.instance.model.time.hours == 8)
        {
            outputText("Nieve is sitting in the white wintery wonderland, carefully constructing a snowman.  It strikes you as strange, and almost masturbatory in a way.  " + nieveMF("He", "She") + " spots you and gives you a wave and a smile.  For a moment, you think the snowman has too, but you eventually conclude it's your imagination.");
        }
        //9:00
        else if (CoC.instance.model.time.hours == 9)
        {
            outputText("Nieve is sprawled out in the icy field, creating a series of snow angels without a care in the world.  When " + nieveMF("he", "she") + " catches sight of you, she gives a friendly wave, then dives back into the snow.");
        }
        //10:00
        else if (CoC.instance.model.time.hours == 10)
        {
            outputText("Nieve, the ice spirit, sits quietly at the edge of the snowdrift, looking out at the landscape beyond.  When " + nieveMF("he", "she") + " sees you looking, " + nieveMF("he", "she") + " gives a somber wave.");
        }
        //11:00
        else if (CoC.instance.model.time.hours == 11)
        {
            outputText("Nieve is in your camp, poking around your supplies and water barrels. You notice " + nieveMF("he", "she") + " seems a little uncomfortable to be away from " + nieveMF("his", "her") + " snowdrift.");
        }
        //12:00
        else if (CoC.instance.model.time.hours == 12)
        {
            outputText("Nieve is carefully compressing snowballs in " + nieveMF("his", "her") + " little camp.  To your surprise, " + nieveMF("he", "she") + " picks one up and begins eating it like you would an apple.  When " + nieveMF("he", "she") + " catches you looking mid-bite, " + nieveMF("he", "she") + " gives you a grin with her mouth full, " + nieveMF("his", "her") + " cheeks puffed out.");
        }
        //13:00
        else if (CoC.instance.model.time.hours == 13)
        {
            outputText("Nieve is sitting cross-legged, in the middle of what appears to be an argument with a snowman " + nieveMF("he", "she") + "'s just made.");
        }
        //14:00
        else if (CoC.instance.model.time.hours == 14)
        {
            outputText("Nieve is in " + nieveMF("his", "her") + " usual spot, seemingly deep in concentration.  Around " + nieveMF("him", "her") + " snow begins to fall slowly, then quicker and quicker, whipping " + nieveMF("his", "her") + " hair around wildly.  Although strangely, the air is still where you are.");
        }
        //15:00
        else if (CoC.instance.model.time.hours == 15)
        {
        ...
        //16:00
        else if (CoC.instance.model.time.hours == 16)
        {
        ...
        //17:00
        else if (CoC.instance.model.time.hours == 17)
        {
        ...
        //18:00
        else if (CoC.instance.model.time.hours == 18)
        {
        ...
        //19:00
        else if (CoC.instance.model.time.hours == 19)
        {
        ...
        //20:00
        else if (CoC.instance.model.time.hours == 20)
        {
    ...
    public function approachNieve() : Void
    {
        ...
        if (silly())
        {
            outputText("  And you presume a butthole nestled between " + nieveMF("his", "her") + " cheeks, right where it belongs.");
        }
        ...
        if (flags[kFLAGS.NIEVE_MOUTH] == "coal")
        {
            outputText("dusky black");
        }
        else
        {
            outputText("vibrant purple");
        }
        ...
        if (flags[kFLAGS.NIEVE_GENDER] == 1)
        {
            addButton(2, "Turn Female", nieveToFemale).disableIf(!player.hasItem(consumables.P_S_MLK), "Requires a bottle of purified Succubus Milk.");
        }
        else
        {
            addButton(2, "Turn Male", nieveToMale).disableIf(!player.hasItem(consumables.P_DRAFT), "Requires a bottle of purified Incubi Draft.");
        }
        addButton(4, "Back", SceneLib.camp.campLoversMenu);
        
        var nieveToFemale : Void->Void = function() : Void
        {
            clearOutput();
            ...
            if (player.cor < 33)
            {
                outputText("You warn him that Succubi Milk can turn him into a girl and ask if he's willing to try it for your pleasure. The changes are permanent, but reversible, after all... To your surprise, the ice spirit nods enthusiastically and quickly empties the bottle.");
            }
            else if (player.cor < 66)
            {
                outputText("With a devious smirk, you offer him the bottle and say that it's a good drink that even he will probably like. You barely hold your giggle when you tell him how popular it is in Mareth.");
            }
            else
            {
                outputText("Opening the bottle, you press it ho his lips, giving Nieve a <i>hint</i> of what you want him to do. To your surprise, he doesn't resist, drinking it to the last drop.");
            }
        ...
        function nieveToMale() : Void
        {
            ...
            if (player.cor < 33)
            {
                outputText("You explain your idea to her and wait for her reaction. To your surprise, the ice spirit nods enthusiastically and quickly empties the bottle.");
            }
            else if (player.cor < 66)
            {
                outputText("With a devious smirk, you offer the girl a present - a cool white drink that she will certainly like. What could possibly go wrong... She swallows the bait and quickly drinks it without realising what 'the drink' is.");
            }
            else
            {
                outputText("Pushing the girl to her back on the snow, you straddle her and look down at her face. While she tries to smile and tease you, you quickly open the bottle and dump the contents into her willing mouth. She doesn't have any other choice but to swallow everything to the last drop.");
            }
            if (player.cor < 66)
            {
        ...
        var nieveSexMenu : Void->Void = function() : Void
        {
            clearOutput();
            outputText("What will you do with your oh-so-cool lover?");
            if (player.lust < 33)
            {
                outputText("  You aren't quite turned on enough for normal fucking.");
            }
            menu();
            ...
            //Fuck Her{
                ...
                if (flags[kFLAGS.NIEVE_GENDER] == 1)
                {
                    addButton(1, "Get Fucked", nieveFucksYou);
                }
            }
            ...
            function lickNieve() : Void
            {
                ...
                if (!silly())
                {
                    ...
                    {
                        outputText("It doesn't get far, but elicits squeals of surprise and pleasure from your elemental slave who's already grasping at the snow surrounding her for support.");
                    }
                    //Demon Tongue:
                    else
                    {
                        outputText("Her eyes widen as you snake your impressive tongue into her. She grunts and squeals with pleasure, hands grasping piles of snow, wishing they were bedsheets.");
                    }
                    ...
                    if (player.isTaur())
                    {
                        outputText(", or as close to it with your body,");
                    }
                ...
                else
                {
                    ...
                    //(no Kid A:
                    if (flags[kFLAGS.ANEMONE_KID] != 1)
                    {
                        outputText(", Nieve takes a ladle full of water and pours it onto her snatch");
                    }
                    //Kid A:
                    else
                    {
                        outputText(", your little anemone spawn giggles and laughs at your predicament before finally ladling some water onto your lover's snatch");
                    }
                ...
                doNext(explorer.done);
            }  //Any gender    //For male Nieve    //Suck Him by Kinathis  ;
            ...
            var suckNieveOff : Void->Void = function() : Void
            {
                clearOutput();
            ...
            }  //x - dick num or -1 for clitfuck    //Note for fen/gats: In the interest of fairness, I just wanna say that this scene was unfinished at the time the contest closed.    //Nieve's capacity is about 130.    //Must have a penis or at least a 3.5 inch clit    //Female Nieve    //Fuck Her  ;
            ...
            function fuckNieve(x : Int) : Void
            {
                ...
                if (x >= 0)
                {
                    if (player.cocks[x].cockLength < 6)
                    {
                        outputText("  \"<i>It's so cute, [Master],</i>\" Nieve says while catching a breath, \"<i>Like a little toy cock.</i>\"");
                    }
                    //If overly large (>130):
                    else if (player.cocks[x].cockLength >= 24)
                    {
                        outputText("  She has to take a breath almost immediately, saying, \"<i>By the fat man's beard this thing is huge.  You must make the rest of this world jealous.</i>\"");
                    }
                    else
                    {
                        outputText("  \"<i>Mmm,</i>\" she smacks her lips and takes a quick breath, \"<i>Such a nice example of a candycane right here.  Let's go in for another taste.</i>\"");
                    }
                }
                ...
                //(Human){
                    ...
                    {
                        outputText("  Your normal, unaltered cock seems to take on a slight blue tint, although you can confirm it's not going numb.  Quite the contrary, it seems to be getting more sensitive as Nieve's lips roll over it.");
                    }
                    //(Equine)
                    ...
                    {
                        outputText("  Nieve does not seem put off by your tentacle cock in the least, in fact she confesses, \"<i>It reminds me of a comic I read before coming here.</i>\"  As she fawns over your cock, you're certain you hear whispers of \"<i>Oh, you sexy tentacle beast...</i>\" coming from her mouth when she gets her breath.");
                    }
                    //(Feline)
                    else if (player.cocks[x].cockType == CockTypesEnum.CAT)
                    {
                        outputText("  Though the barbs on your cock are soft and pliable, they reflexively harden up after Nieve's tongue washes over them.  She doesn't pay particular attention to the barbs, but she does seem rather careful not to get one hooked on her lip.");
                    }
                    //(Lizard)
                    else if (player.cocks[x].cockType == CockTypesEnum.LIZARD)
                    {
                        outputText("  Nieve fawns over your scaled and ridged reptilian cock, slurping and smooching up and down its length. It seems a bit more resistant to the biting cold of Nieve's mouth, which is lucky when she backs off, exposing your entire wet, frigid length to the wintery air surrounding you.");
                    }
                    //(Anemone)
                    else if (player.cocks[x].cockType == CockTypesEnum.ANEMONE)
                    {
                    ...
                    //(Kangaroo)
                    else if (player.cocks[x].cockType == CockTypesEnum.KANGAROO)
                    {
                        outputText("  It takes Nieve a little while to catch on that your cock isn't quite human, and by the time she figures it out, she mutters, \"<i>Huh, that's strange... I thought kangaroos had balls above the cock,</i>\" before shrugging and getting back to work.");
                    }
                    //(Canine)
                    else if (player.hasKnot(x))
                    {
                        ...
                        {
                            outputText("  Eventually, though, she admits defeat and goes back to working your shaft, though she eyes that knot from time to time with no small amount of anger.");
                        }
                        //(Cocksize <40)
                        else
                        {
                            outputText("  It takes her a few good minutes, but eventually she works her way down and takes the entire knot into her mouth.  She raises her hands in triumph and lets out a \"<i>Mmmmrrrrrpphhh!</i>\" before pulling back for a breath, inadvertently exposing your wet hard cock to the cold, wintery air.");
                        }
                    }
                    //(If you add dicks later, placeholder text:)
                    else
                    {
                        outputText("  Nieve works on your mutant cock, spending time working every unique little facet of it.");
                    }
                }
                //(Clitoris)
                else
                {
                    ...
                    {
                        outputText("It's just like a little cock");
                    }
                    else if (player.clitLength < 9)
                    {
                        outputText("It's just like a cock");
                    }
                    else
                    {
                        outputText("I bet even men get jealous of this bad girl");
                    }
                    outputText(".</i>\"");
                }
                ...
                if (x >= 0)
                {
                    outputText("  Your " + cockClit(x) + " quivers and spurts cum directly into Nieve's eager mouth, who gulps it down as if she were drinking direct from the tap.");
                }
                outputText("\n\nAs you come down from your little orgasmic high, you glance down at the ice woman, ");
                if (x < 0 || player.cumQ() <= 250)
                {
                    outputText("who has a bit of cum dribbling down her chin");
                }
                else if (player.cumQ() <= 400)
                {
                    outputText("who has a steady stream of cum pouring from her mouth");
                }
                else if (player.cumQ() < 600)
                {
                    outputText("who is patting her stomach as though she'd just had a large meal");
                }
                else if (player.cumQ() < 1000)
                {
                    outputText("whose stomach appears slightly distended from the amount of cum you've poured into her");
                }
                else
                {
                    outputText("whose stomach is rather large, like a pregnant woman's, from the sheer amount of cum you've pumped into her gullet");
                }
                ...
                if (x >= 0)
                {
                    outputText("with your " + player.cockDescript(x) + "");
                }
                else
                {
                    outputText("with yours");
                }
                ...
                if (x >= 0)
                {
                    if (player.cockArea(x) >= 130)
                    {
                        outputText("at least as far as it will go, ");
                    }
                }
                ...
                //((cocksize 3/Clit 3){
                    ...
                    if (player.cockArea(x) <= 7)
                    {
                        outputText("Your meager penis isn't much, but at this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes staring up at you, urging you on faster and faster.");
                    }
                    //(Cocksize 30/Clit 12)
                    else if (player.cockArea(x) <= 30)
                    {
                        outputText("At this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes tightly shut as she whispers dirty words you can't even make out.");
                    }
                    //(Cocksize 70/Clit 24)
                    else if (player.cockArea(x) <= 70)
                    {
                        outputText("Nieve's cunt feels like an icy vice on your oversized cock, but even so, you manage to hit all the right places.  Soon she is wriggling around, eyes tightly shut as she pants for breath.");
                    }
                    //(Cocksize 130+/Clit 48+)
                    else
                    {
                        outputText("Though you can't fit your entire " + cockClit(x) + " inside, you are amazed at how much she takes.  Her pale blue stomach bulges with every thrust.  Soon Nieve is wriggling, her eyes slightly open as she bites her lip in ecstasy.");
                    }
                }
                else if (player.clitLength <= 4)
                {
                    outputText("Your meager clit isn't much, but at this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes staring up at you, urging you on faster and faster.");
                }
                ...
                {
                    outputText("Nieve's cunt feels like an icy vice on your oversized clitty, but even so, you manage to hit all the right places.  Soon she is wriggling around, eyes tightly shut as she pants for breath.");
                }
                //(Cocksize 130+/Clit 48+)
                else
                {
                ...
                if (x < 0)
                {
                    outputText("  Your [clit] suddenly feels warm, like every nerve ending inside were on fire. It aches and pulses with the rest of your body, as though it were a little cock longing to cum.");
                }
                else
                {
                    ...
                    if (player.cumQ() < 100)
                    {
                        outputText("spilling its meager offering into the ice spirit's womb");
                    }
                    else if (player.cumQ() < 250)
                    {
                        outputText("ejecting streams of white hot jizz into the ice spirit's hungry snatch");
                    }
                    else if (player.cumQ() < 400)
                    {
                        outputText("painting the ice spirit's insides with your cream until some begins to pour out");
                    }
                    else if (player.cumQ() < 700)
                    {
                        outputText("arcs of pearly white cum shoot forth, filling the ice spirit's womb so much her belly visibly inflates");
                    }
                    else
                    {
                        outputText("erupting inside, filling the ice spirit up with so much of your hot cum so much that she looks visibly pregnant");
                    }
                    outputText(".");
                }
                ...
                player.dynStats("sen", -2);
                doNext(explorer.done);
            }  //Nieve's cock is 9x1.5    //Any Gender    //Male Nieve    //Get Fucked by Gurumash  ;
            ...
            var nieveFucksYou : Void->Void = function() : Void
            {
                clearOutput();
                ...
                if (player.cor < 50)
                {
                    outputText("telling him that you find his form... appeasing to say the least, and would like to know how his 'parts' are working properly.");
                }
                else
                {
                    outputText("leting your creation know that you are interested in 'how' it can please you, giving him no room to imagine what you're insinuating you then tell him bluntly that you want him to fuck you like the plaything he is.");
                }
                ...
                if (silly())
                {
                    outputText("   He did say that his kind were effectively love slaves, and boy does this prove it!");
                }
                //[if (hasVagina = true)
                if (player.hasVagina())
                {
                    outputText("  Your [vag] is a sodden box so soon after he started, and you start to want more, much more!");
                }
                //[if (hasCock = true)
                if (player.hasCock())
                {
                    outputText("  Your [cock biggest] is also rock hard in a short while and you quickly desire some more stimulation from your icy love slave.");
                }
                ...
                function takeNieveAnal() : Void
                {
                    ...
                    if (player.hasCock())
                    {
                        outputText("  Nieve then bends forward while keeping his pace to give you a reach-around.  Grabbing your own [cock biggest] in one hand, he starts to pump it to the speed of his thrusts, torturing you with the extra-stimulation in such a good way.");
                    }
                    //[if (hasVagina = true)
                    else if (player.hasVagina())
                    {
                        outputText("  As Nieve continues to pound away, he places one hand on your sopping wet cunt and begins to finger you in time with his assault, making you lust-drunk with the stimulation as he fingerbangs you.");
                    }
                    ...
                    //[if (hasCock = true){
                        ...
                        if (player.hasCock())
                        {
                            ...
                            if (player.cumQ() < 200)
                            {
                                outputText(", as you cum, you leave a small puddle on the floor below you");
                            }
                            else if (player.cumQ() < 500)
                            {
                                outputText(", as you cum you start to dress your arms and hands with your own cream as you shoot your stuff out onto the floor");
                            }
                        }
                        //[if (hasVagina = true)
                        if (player.hasVagina())
                        {
                            outputText(".  Your soaking vagina makes your body quake and shudder as you orgasm, splashing your femcum all over Nieve and the ground");
                        }
                    }
                    outputText("."
                ...
                var takeNieveVaginal : Void->Void = function() : Void
                {
                    clearOutput();
                    ...
                    if (player.hasBalls())
                    {
                        outputText(", gently moving your [balls] out of the way in the process");
                    }
                    ...
                    if (player.hasCock())
                    {
                        outputText("  During the pounding Nieve grabs your dangling [cock biggest] and starts to pump it in time with his own thrusts, quite eager to please his [master].  Every time your feminine half cums, so does your male half, splashing its happy seed into the valley that your combined bodies have made.");
                    }
                    //[if (isLactating = true)
                    if (player.lactationQ() >= 100)
                    {
                        outputText("  Your ice man reaches out with a free hand to caress your [chest], twiddling his fingers around your delicate [nipples]. After some rough pulling and flicking, pounding away all the while, he feels milk dribble onto his hand.  He catches a quick glance of the liquid before diving head-first to suckle on your milk faucets, drinking heavily of your cream as he's about to give you his.");
                    }
            ...
            var cockClit : Int->String = function(number : Int = 0) : String
            {
                if (player.hasCock() && number >= 0 && number < player.cockTotal())
                {
                    return player.cockDescript(number);
                }
                else
                {
    ...
    public function nieveIsOver() : Void
    {
        ...
        else
        {
            outputText("You wake up with a yawn and a stretch.  It feels just like any other day, though a bit warmer.  Quite a bit warmer in fact.  It feels like winter's grasp on Mareth has slipped, and it's entering Spring.  You consider the fact absently at first, but then you stop dead in your tracks.  What about Nieve?!"
                    + "\n\nYou turn to "
                    + nieveMF("his", "her")
                    + " winter wonderland, relief washing over you as you see it's still there, but note that it is diminishing.  You rush over, watching the parched landscape claiming the slushy terrain bit by bit.  Nieve sits in the middle, looking up at the sky with a worried look on "
                    + nieveMF("his", "her")
                    + " face.  "
                    + nieveMF("His", "Her")
                    + " normally glittering skin now shines with sweat, not frost. Beads of perspiration roll down "
                    + nieveMF("his", "her")
                    + " body and into a worryingly large puddle at "
                    + nieveMF("his", "her")
                    + " feet."
                    + "\n\n\"<i>Ah, [Master], you're here. I was concerned we might not have been able to talk before... before I go home,</i>\" "
                    + nieveMF("he", "she")
                    + " says sadly.  You question "
                    + nieveMF("him", "her")
                    + ", why does "
                    + nieveMF("he", "she")
                    + " have to go home? You've seen "
                    + nieveMF("him", "her")
                    + " make snow before, why can't "
                    + nieveMF("he", "she")
                    + " just sustain "
                    + nieveMF("him", "her")
                    + "self like that?  Nieve justs shakes "
                    + nieveMF("his", "her")
                    + " head, \"<i>I'm afraid it doesn't work like that.  My kind can only exist outside our homeland during the winter season.  Any further, and we simply melt away.</i>\"  "
                    + nieveMF("He", "She")
                    + " sighs sorrowfully, and you take "
                    + nieveMF("his", "her")
                    + " hand.  It's strangely warm, like the body temperature of a human, not at all "
                    + nieveMF("his", "her")
                    + " usual frigid touch."
                    + "\n\nA familiar jingling fills the air, and you both look down to see Nieve's legs replaced by a large ball of melting snow and ice.  Clearly not wanting to think about it, "
                    + nieveMF("he", "she")
                    + " throws "
                    + nieveMF("his", "her")
                    + " arms around you and pulls you into a hug.  \"<i>I just want to say,</i>\" "
                    + nieveMF("he", "she")
                    + " whispers into your ear.  \"<i>You were the best master I could have hoped for.  And...</i>\" "
                    + nieveMF("he", "she")
                    + " pauses, tears forming in "
                    + nieveMF("his", "her")
                    + " eyes, \"<i>And I'll mi--...</i>\"  "
                    + nieveMF("His", "Her")
                    + " voice drifts off.  When you break the hug to look at "
                    + nieveMF("him", "her")
                    + ", to urge "
                    + nieveMF("him", "her")
                    + " to continue, only the lifeless eyes of the snowman you first created stare back at you."
            ...
            if (player.cor < 50)
            {
                outputText("  You even tear up yourself, sad to see such a good friend leave, and so abruptly.");
            }
            //Corruption >50:
            else
            {
                outputText("  You even feel a little sad yourself, it's such a pity to see a good, obedient fucktoy leave.");
            }
    ...
    public function returnOfNieve() : Void
    {
        ...
        outputText("As you awake you find yourself shivering slightly.  A cool breeze sweeps over your camp, while in the distance jingling bells can be heard.  How odd.  You haven't heard bells like that since..."
                ...
                + "\n\nThere really is nothing left of "
                + nieveMF("him", "her")
                + ".  Nothing but this lost fragment.  An echo of a friend.");
        //[Next]
        menu();
        addButton(0, "Next", fixNieve);
        
        var fixNieve : Void->Void = function() : Void
        {
            ...
            var fixNieveMouth : Int->Void = function(arg : Int = 1) : Void
            {
                if (arg == 0)
                {
                    flags[kFLAGS.NIEVE_MOUTH] = "gems";
            ...
            var fixNieveGender : Int->Void = function(arg : Int = 1) : Void
            {
                flags[kFLAGS.NIEVE_GENDER] = arg;
                fixNieve();
            }
            
            var nieveReturnsPartII : Void->Void = function() : Void
            {
                clearOutput();
                outputText("You sigh, resigning yourself to your companion's fate.  However, as you rise from the snow to return to camp, you hear a soft, muffled voice.  Perplexed, you crawl forward in the snow, frantically seeking out the source."
                        ...
                        + "\n\n\"<i>Oh master!</i>\" "
                        + nieveMF("he", "she")
                        + " cries, breaking the hug to look you in the eyes.  "
                        + nieveMF("His", "Her")
                        + " ");
                if (flags[kFLAGS.NIEVE_MOUTH] == "coal")
                {
