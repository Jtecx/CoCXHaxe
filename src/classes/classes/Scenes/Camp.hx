...
class Camp extends NPCAwareContent
{
    private var timeQ(never, set) : Float;

    
    private function set_timeQ(value : Float) : Float
    {
        CoC.instance.timeQ = value;
        return value;
    }
    
    private var campQ : Bool = false;
    private var waitingORresting : Int = 1;
    
    private function hasItemInStorage(itype : ItemType) : Bool
    {
    ...
    private static var _campFollowers : Array<XXCNPC> = new Array<XXCNPC>();
    ...
    {
        var i : Int = Lambda.indexOf(_campFollowers, toRemove);
        if (i >= 0)
        {
    ...
    public function returnToCamp(timeUsed : Int) : Void
    {
        ...
        {
            outputText(cast((timeUsed), Num2Text) + " hours pass...\n");
        }
    ...
    private var izmaJoinsStream : Bool;
    private var marbleJoinsStream : Bool;
    private var heliaJoinsStream : Bool;
    private var amilyJoinsStream : Bool;
    
    public var IsSleeping : Bool = false;
    public var CanDream : Bool = false;
    public var IsWaitingResting : Bool = false;
    ...
    //Only called by playerMenu
    {
        ...
        if ((player.isRaceCached(Races.FMINDBREAKER, 1) || player.isRaceCached(Races.MMINDBREAKER, 1) || player.isRaceCached(Races.ATLACH_NACHA, 3)) && player.cor < 100)
        {
            player.cor = 100;
        }
        ...
        {
            TrollVillage.ZenjiMoneyHelp -= 1;
        }
        if (!marbleScene.marbleFollower() && flags[kFLAGS.MARBLE_LEFT_OVER_CORRUPTION] ==1  /* && player.cor <= 40*/  )
        
        //Removed the check to make her return to everyone{
        ...
        if (flags[kFLAGS.JOJO_BIMBO_STATE] == 4 && flags[kFLAGS.TALKED_TO_JOJO_ABOUT_JOY] == 0)
        
        //post-debimbo conversation{
        ...
        if (flags[kFLAGS.FUCK_FLOWER_KILLED] == 0 && flags[kFLAGS.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE] > 0 && flags[kFLAGS.IN_INGNAM] == null)
        {
        ...
        if (flags[kFLAGS.FUCK_FLOWER_KILLED] == 0 && flags[kFLAGS.MARAE_QUEST_COMPLETE] == 1 && flags[kFLAGS.IN_INGNAM] == null)
        {
        ...
        if (flags[kFLAGS.CHRISTMAS_TREE_LEVEL] > 1 && flags[kFLAGS.CHRISTMAS_TREE_LEVEL] < 5 && flags[kFLAGS.IN_INGNAM] == null)
        {
        ...
        */
        if (player.hasStatusEffect(StatusEffects.CampRathazul) && isAprilFools() && (!player.hasStatusEffect(StatusEffects.RathazulAprilFool) ||
            player.statusEffectv3(StatusEffects.RathazulAprilFool) == 2 && player.statusEffectv2(StatusEffects.RathazulAprilFool) < 5 && date.fullYear > player.statusEffectv1(StatusEffects.RathazulAprilFool)))
        {
        ...
        if (flags[kFLAGS.THE_TRENCH_ENTERED] > 14 && flags[kFLAGS.THE_TRENCH_ENTERED] < 16 && rand(5) == 0 && !player.hasStatusEffect(StatusEffects.GraydaRandomnCampEvents) && (flags[kFLAGS.SIEGWEIRD_FOLLOWER] > 3 || player.hasStatusEffect(StatusEffects.CampRathazul) || (player.hasStatusEffect(StatusEffects.PureCampJojo) && flags[kFLAGS.JOJO_BIMBO_STATE] != 3) || followerKiha()))
        ...
        campQ = false;
        //Clear stuff
        if (player.hasStatusEffect(StatusEffects.SlimeCravingOutput))
        {
        ...
        var canExploreAtNight : Bool = (player.isNightCreature());
        var isAWerebeast : Bool = (player.isWerebeast());
        clearOutput();
        ...
        {
            if (model.time.days < 10)
            {
                outputText("Your campsite is fairly simple at the moment.  Your tent and bedroll are set in front of the rocks that lead to the portal.  You have a small fire pit as well.  ");
            }
            if (model.time.days >= 10 && model.time.days < 20)
            {
                outputText("Your campsite is starting to get a very 'lived-in' look.  The fire-pit is well defined with some rocks you've arranged around it, and your bedroll and tent have been set up in the area most sheltered by rocks.  ");
            }
            if (model.time.days >= 20)
            {
                if (!isabellaFollower())
                {
                    outputText("Your new home is as comfy as a camp site can be.  ");
                }
        ...
        if (model.time.days >= 20)
        {
            outputText("You've even managed to carve some artwork into the rocks around the [camp] perimeter.\n\n");
        }
        if (flags[kFLAGS.CAMP_CABIN_PROGRESS] == 7)
        {
            outputText("There's an unfinished wooden structure. As of right now, it's little more than wooden frames nailed together.\n\n");
        }
        if (flags[kFLAGS.CAMP_CABIN_PROGRESS] == 8)
        {
            outputText("There's an unfinished cabin. It's currently missing both windows and a door.\n\n");
        }
        if (flags[kFLAGS.CAMP_CABIN_PROGRESS] == 9)
        {
            outputText("There's a nearly-finished cabin. It looks complete from the outside, but inside, the floors are still bare earth.\n\n");
        }
        if (flags[kFLAGS.CAMP_CABIN_PROGRESS] >= 10)
        {
            outputText("Your cabin is situated near the edge of [camp].\n\n");
        }
        if (flags[kFLAGS.MATERIALS_STORAGE_UPGRADES] == 0 || flags[kFLAGS.MATERIALS_STORAGE_UPGRADES] == 1)
        {
            outputText("Halfway between the portal and the edge of your [camp] is a place where you will store piles of wood or stones for construction. ");
        }
        if (flags[kFLAGS.MATERIALS_STORAGE_UPGRADES] == 3)
        {
            outputText("Halfway between the portal and the edge of your [camp]  rests a medium sized wood platform, which you use to store wood and next to it is place for storing stones. ");
        }
        if (flags[kFLAGS.MATERIALS_STORAGE_UPGRADES] >= 4)
        {
            outputText("Halfway between the portal and the edge of your [camp]  rests a long and wide wood platform with protective barriers at the edges. Inside it, you can safely store large amounts of wood and stone. ");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 1)
        {
            outputText("There's a half finished warehouse construction near the east edge of your campsite.\n\n");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 2)
        {
            outputText("There's a warehouse located in the east section of your campsite.\n\n");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 3)
        {
            outputText("There's a warehouse and connected to it half finished granary construction located in the east section of your campsite.\n\n");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 4)
        {
            outputText("There's a warehouse and connected to it granary located in the east section of your campsite.\n\n");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 5)
        {
            outputText("There's a warehouse and second one warehouse half finished construction connected by granary located in the east section of your campsite.\n\n");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 6)
        {
            outputText("There are two warehouses and granary connecting them located in the east section of your campsite.\n\n");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] >= 2)
        {
            outputText("Some of your friends are currently sparring at the ");
            if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] >= 4 && flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 7)
            {
                outputText("massive");
            }
            ...
            {
                outputText(" with wooden floor");
            }
            outputText(" toward side of your [camp].");
            if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] >= 3)
            {
                outputText(" Given how large the sparring ring, maybe it's a little excessive for even the largest of people around.");
            }
            if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] >= 4)
            {
                outputText(" A small stand rests adjacent, allowing spectators to cheer on any duels taking place.");
            }
            outputText("\n\n");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_ARCANE_CIRCLE] >= 1)
        {
            ...
            {
                outputText("Eight large arcane circles are");
            }
            else if (flags[kFLAGS.CAMP_UPGRADES_ARCANE_CIRCLE] == 7)
            {
                outputText("Seven large arcane circles are");
            }
            else if (flags[kFLAGS.CAMP_UPGRADES_ARCANE_CIRCLE] == 6)
            {
                outputText("Six large arcane circles are");
            }
            else if (flags[kFLAGS.CAMP_UPGRADES_ARCANE_CIRCLE] == 5)
            {
                outputText("Five large arcane circles are");
            }
            else if (flags[kFLAGS.CAMP_UPGRADES_ARCANE_CIRCLE] == 4)
            {
                outputText("Four large arcane circles are");
            }
            else if (flags[kFLAGS.CAMP_UPGRADES_ARCANE_CIRCLE] == 3)
            {
                outputText("Three large arcane circles are");
            }
            else if (flags[kFLAGS.CAMP_UPGRADES_ARCANE_CIRCLE] == 2)
            {
                outputText("Two large arcane circles are");
            }
            else
            {
                outputText("Large arcane circle is");
            }
        ...
        {
            if (flags[kFLAGS.CAMP_UPGRADES_DAM] == 3)
            {
                outputText("Big, wooden dam noticably increases the width of the nearby stream, slowing the water to a near still. It's created a small bay next to camp.");
            }
            else if (flags[kFLAGS.CAMP_UPGRADES_DAM] == 2)
            {
                outputText("Wooden dam noticably increases the width of the nearby stream, slowing the passage of water");
            }
            else
            {
                outputText("Small wooden dam drapes across the stream, slowing the passage of water");
            }
            outputText(".\n\n");
            if (flags[kFLAGS.CAMP_UPGRADES_FISHERY] >= 1)
            {
                ...
                if (flags[kFLAGS.CAMP_UPGRADES_FISHERY] == 2)
                {
                    outputText("medium-sized");
                }
                if (flags[kFLAGS.CAMP_UPGRADES_FISHERY] == 3)
                {
                    outputText("big");
                }
                if (flags[kFLAGS.CAMP_UPGRADES_FISHERY] == 4)
                {
                    outputText("large");
                }
        ...
        if (flags[kFLAGS.CHRISTMAS_TREE_LEVEL] >= 2 && flags[kFLAGS.CHRISTMAS_TREE_LEVEL] < 8)
        {
            ...
            {
                outputText("At the corner of camp where you planted a seed, a sapling has grown. It has dozens of branches and bright green leaves.\n\n");
            }
        ...
        {
            if (flags[kFLAGS.CAMP_WALL_PROGRESS] / 10 == 1)
            {
                outputText("A thick wooden wall has been erected. Incomplete as it is, it provides only a small amount of defense.  ");
            }
            else
            {
                outputText("Thick wooden walls have been erected around your camp.  ");
            }
        }
        else if (flags[kFLAGS.CAMP_WALL_PROGRESS] >= 100)
        {
            ...
            {
                outputText("A gate has been constructed in the middle of the walls; it gets closed at night to keep any invaders out.  ");
            }
        }
        //Imp Skulls
        if (flags[kFLAGS.CAMP_WALL_PROGRESS] >= 10 && flags[kFLAGS.CAMP_WALL_SKULLS] > 0)
        {
            ...
            if (flags[kFLAGS.CAMP_WALL_SKULLS] == 1)
            {
                outputText("There is currently one skull.  ");
            }
            else
            {
                outputText("There are currently " + num2Text(flags[kFLAGS.CAMP_WALL_SKULLS]) + " skulls.  ");
            }
        }
        outputText("\n\n");
        //Magic Ward
        if (flags[kFLAGS.CAMP_UPGRADES_MAGIC_WARD] >= 2)
        {
            ...
            {
                outputText("Just within the wall are the");
            }
            else
            {
                outputText("Right before the ring of traps are your");
            }
            outputText(" warding stones. They are ");
            if (flags[kFLAGS.CAMP_UPGRADES_MAGIC_WARD] == 2)
            {
                outputText("currently inactive.");
            }
            if (flags[kFLAGS.CAMP_UPGRADES_MAGIC_WARD] == 3)
            {
                outputText("glowing with power, protecting your [camp] from intruders.");
            }
            outputText("\n\n");
        }
        else
        ...
        if (model.time.hours < 6 || model.time.hours > 20)
        {
            ...
            {
                outputText("It is dark out, made worse by the lack of stars in the sky.  A blood-red moon hangs in the sky, seeming to watch you, but providing little light.  It's far too dark to leave the [camp].\n\n");
            }
            else
            {
                outputText("It is dark out. Stars dot the night sky. A blood-red moon hangs in the sky, seeming to watch you, but providing little light.  It's far too dark to leave the [camp].\n\n");
            }
        ...
        {
            if (flags[kFLAGS.CAMP_UPGRADES_HOT_SPRINGS] < 1)
            {
        ...
        if (!player.hasPerk(PerkLib.Rigidity) && ((flags[kFLAGS.PLAYER_DISARMED_WEAPON_ID] != 0) || (flags[kFLAGS.PLAYER_DISARMED_WEAPON_R_ID] != 0)))
        {
            ...
            else if (flags[kFLAGS.PLAYER_DISARMED_WEAPON_ID] != 0)
            {
                player.setWeapon(try cast(ItemType.lookupItem(flags[kFLAGS.PLAYER_DISARMED_WEAPON_ID]), Weapon) catch(e:Dynamic) null);
            }
            else
            {
                player.setWeaponRange(try cast(ItemType.lookupItem(flags[kFLAGS.PLAYER_DISARMED_WEAPON_R_ID]), WeaponRange) catch(e:Dynamic) null);
            }
        ...
        if (inventory.showStash())
        {
            addButton(3, "Stash", inventory.stash).hint("The stash allows you to store your items safely until you need them later.");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] >= 2)
        {
            addButton(4, "Warehouse", inventory.warehouse).hint("The warehouse and granary allow you to store your items in a more organized manner.");
        }
        if (followersCount() > 0)
        {
            addButton(5, "Followers", campFollowers).hint("Check up on any followers or companions who are joining you in or around your [camp].  You'll probably just end up sleeping with them.");
        }
        if (loversCount() > 0)
        {
            addButton(6, "Lovers", campLoversMenu).hint("Check up on any lovers you have invited to your [camp] so far and interact with them.");
        }
        if (slavesCount() > 0)
        {
            addButton(7, "Slaves", campSlavesMenu).hint("Check up on any slaves you have received and interact with them.");
        }
        ...
        {
            addButton(9, "Enter Cabin", cabinProgress.initiateCabin).hint("Enter your cabin.");
        }  //Enter cabin for furnish.  
        if (player.hasPerk(PerkLib.JobSoulCultivator) && !player.hasPerk(PerkLib.Soulless))
        {
            addButton(10, "Soulforce", soulforce.accessSoulforceMenu).hint("Spend some time on the cultivation, or spend some of the soulforce.");
        }
        else if (!player.hasPerk(PerkLib.JobSoulCultivator) && !player.hasPerk(PerkLib.Soulless) && player.hasPerk(PerkLib.Metamorph))
        {
            if (player.blockingBodyTransformations())
            {
                addButtonDisabled(10, "Metamorph", "Your current body state prevents you from using Metamorph. (Either cure it or ascend to gain access to metamorph menu again)");
            }
            else
            {
                addButton(10, "Metamorph", SceneLib.metamorph.openMetamorph).hint("Use your soulforce to mold your body.");
            }
        }
        else if (player.hasPerk(PerkLib.Soulless))
        {
            addButton(10, "Demonic Energy", soulforce.accessDemonicEnergyMenu).hint("You can use harvested souls and lethicite to improve your magic and body.");
        }
        ...
        {
            addButton(12, "Rest", restMenu).hint("Rest for one to twelve hours. Or until fully healed / night comes.");
        }
        ...
        if (CoC_Settings.debugBuild && !player.hasStatusEffect(StatusEffects.ILLYProtocol))
        {
            addButton(14, "Cheats", testmenu.SoulforceCheats).hint("This should be obvious. ^^");
        }
        ...
        if (isNightTime)
        {
            ...
            {
                if (nightTimeActiveFollowers() == 0)
                {
                    removeButton(5);
                }  //Followers  
                if (nightTimeActiveLovers() == 0)
                {
                    removeButton(6);
                }  //Lovers  
                if (nightTimeActiveSlaves() == 0)
                {
                    removeButton(7);
                }
            }
    ...
    public function companionsCount() : Float
    {
        return followersCount() + slavesCount() + loversCount();
    }
    
    public function followersCount() : Float
    {
        var counter : Float = 0;
        ...
        if (flags[kFLAGS.VALERIA_AT_CAMP] == 1)
        {
            counter++;
        }
        if (flags[kFLAGS.AETHER_DEXTER_TWIN_AT_CAMP] > 0)
        {
            counter++;
        }
        if (flags[kFLAGS.AETHER_SINISTER_TWIN_AT_CAMP] > 0)
        {
            counter++;
        }
    ...
    public function slavesCount() : Float
    {
        var counter : Float = 0;
        ...
        if (amilyScene.amilyFollower() && amilyScene.amilyCorrupt() && flags[kFLAGS.FOLLOWER_AT_FARM_AMILY] == 0)
        {
            counter++;
        }
    ...
    public function slavesOptionalCount() : Float
    {
        var counter : Float = 0;
        if (campCorruptJojo() && flags[kFLAGS.FOLLOWER_AT_FARM_JOJO] == 0)
        {
    ...
    public function loversCount() : Float
    {
        var counter : Float = 0;
        if (flags[kFLAGS.ALVINA_FOLLOWER] > 19 || SceneLib.alvinaFollower.AlvinaPurified)
        {
            counter++;
        }
    ...
    public function submissivesAtCampCount() : Float
    {
        var counter : Float = 0;
        counter += slavesCount();
        counter += LunaFollower.WerewolfPackMember;
        if (player.hasPerk(IMutationsLib.HellhoundFireBallsIM))
        {
            counter += LunaFollower.HellhoundPackMember;
        }
        if (arianScene.arianFollower())
        {
            counter++;
        ...
        if (CelessScene.instance.isCompanion() && CelessScene.instance.isCorrupt)
        {
            counter++;
        }
        if (LilyFollower.LilyFollowerState)
    ...
    public function loversHotBathCount() : Float
    {
        var counter : Float = 0;
        ...
        if (flags[kFLAGS.ZENJI_PROGRESS] == 8 || flags[kFLAGS.ZENJI_PROGRESS] == 9 || ZenjiScenes.isLover())
        {
            counter++;
        }
        if (flags[kFLAGS.EXCELLIA_RECRUITED] >= 33)
        ...
        if (flags[kFLAGS.JOJO_BIMBO_STATE] == 3 && flags[kFLAGS.JOY_COCK_SIZE] < 1)
        {
            counter++;
        }
        if (flags[kFLAGS.SIDONIE_FOLLOWER] >= 1)
    ...
    public function maleNpcsHotBathCount() : Float
    {
        var counter : Float = 0;
        ...
        if (arianScene.arianFollower() && flags[kFLAGS.ARIAN_VAGINA] < 1 && flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
        {
            counter++;
        }
        ...
        if (emberScene.followerEmber() && flags[kFLAGS.EMBER_GENDER] == 1)
        {
            counter++;
        }
        return counter;
    ...
    public function sparableCampMembersCount() : Float
    {
        var counter : Float = 0;
        if (emberScene.followerEmber())
        {
        ...
        if (flags[kFLAGS.LUNA_FOLLOWER] > 10 && !player.hasStatusEffect(StatusEffects.LunaOff))
        {
            counter++;
        }
        if (followerHel() && !player.hasStatusEffect(StatusEffects.HeliaOff))
    ...
    public function nightTimeActiveFollowers() : Float
    {
        var counter : Float = 0;
        if (followerShouldra() && !player.hasStatusEffect(StatusEffects.ShouldraOff))
        {
    ...
    public function nightTimeActiveLovers() : Float
    {
        var counter : Float = 0;
    ...
    public function nightTimeActiveSlaves() : Float
    {
        var counter : Float = 0;
        return counter;
    }
    ...
    public function campLoversMenu(descOnly : Bool = false) : Void
    {
        ...
        if (!(model.time.hours <= 5 || model.time.hours >= 23))
        {
            ...
            if (amilyScene.amilyFollower() && flags[kFLAGS.AMILY_FOLLOWER] == 1 && flags[kFLAGS.AMILY_BLOCK_COUNTDOWN_BECAUSE_CORRUPTED_JOJO] == 0 && !descOnly)
            {
                ...
                switch (rand(6))
                {
                    case 0:
                        ...
                        if (player.hasStatusEffect(StatusEffects.CampRathazul))
                        {
                            outputText(".  Rathazul glances over and immediately gets a nosebleed");
                        }
            ...
            if (flags[kFLAGS.CHI_CHI_FOLLOWER] > 2 && flags[kFLAGS.CHI_CHI_FOLLOWER] != 5 && !player.hasStatusEffect(StatusEffects.ChiChiOff))
            {
                ...
                if (player.statusEffectv4(StatusEffects.CampLunaMishaps2) > 0)
                {
                    buttons.add("Chi Chi", SceneLib.chichiScene.ChiChiCampMainMenu2).disableIf(player.statusEffectv4(StatusEffects.CampLunaMishaps2) > 0, "Wet.");
                }
                else
                {
                    buttons.add("Chi Chi", SceneLib.chichiScene.ChiChiCampMainMenu2).disableIf(player.statusEffectv2(StatusEffects.CampSparingNpcsTimers2) > 0, "Training.");
                }
            }
            //Electra
            if (flags[kFLAGS.ELECTRA_FOLLOWER] > 1 && !player.hasStatusEffect(StatusEffects.ElectraOff))
            ...
            if (flags[kFLAGS.ETNA_FOLLOWER] > 0 && flags[kFLAGS.ETNA_TALKED_ABOUT_HER] != 2 && !player.hasStatusEffect(StatusEffects.EtnaOff))
            {
                ...
                buttons.add("Etna", SceneLib.etnaScene.etnaCampMenu2).disableIf(player.statusEffectv1(StatusEffects.CampLunaMishaps2) > 0 && player.statusEffectv1(StatusEffects.CampLunaMishaps2) > 0, "Sleeping.").disableIf(player.statusEffectv4(StatusEffects.CampSparingNpcsTimers1) > 0, "Training.").disableIf(EtnaFollower.EtnaHunting && time.hours >= 8 && time.hours <= 17, "Hunting");
            }
            //Excellia Lover
            if (flags[kFLAGS.EXCELLIA_RECRUITED] >= 33)
            {
            ...
            if (isabellaFollower() && flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 0)
            {
                ...
                {
                    outputText("Isabella is sound asleep in her bunk and quietly snoring.");
                }
                else if (model.time.hours == 6)
                {
                    outputText("Isabella is busy eating some kind of grain-based snack for breakfast.  The curly-haired cow-girl gives you a smile when she sees you look her way.");
                }
                else if (model.time.hours == 7)
                {
                    outputText("Isabella, the red-headed cow-girl, is busy with a needle and thread, fixing up some of her clothes.");
                }
                else if (model.time.hours == 8)
                {
                    outputText("Isabella is busy cleaning up the [camp], but when she notices you looking her way, she stretches up and arches her back, pressing eight bullet-hard nipples into the sheer silk top she prefers to wear.");
                }
                else if (model.time.hours == 9)
                {
                    outputText("Isabella is out near the fringes of your campsite.  She has her massive shield in one hand and appears to be keeping a sharp eye out for intruders or demons.  When she sees you looking her way, she gives you a wave.");
                }
                else if (model.time.hours == 10)
                {
                    outputText("The cow-girl warrioress, Isabella, is sitting down on a chair and counting out gems from a strange pouch.  She must have defeated someone or something recently.");
                }
                else if (model.time.hours == 11)
                {
                    outputText("Isabella is sipping from a bottle labelled 'Lactaid' in a shaded corner.  When she sees you looking she blushes, though dark spots appear on her top and in her skirt's middle.");
                }
                else if (model.time.hours == 12)
                {
                    outputText("Isabella is cooking a slab of meat over the fire.  From the smell that's wafting this way, you think it's beef.  Idly, you wonder if she realizes just how much like her chosen food animal she has become.");
                }
                else if (model.time.hours == 13)
                {
                    outputText("Isabella ");
                    var izzyCreeps : Array<Dynamic> = [];
                    ...
                    {
                        outputText("is sitting down with Rathazul, chatting amiably about the weather.");
                    }
                    else if (izzyCreeps[choice] == 1)
                    {
                        outputText("is sitting down with Jojo, smiling knowingly as the mouse struggles to keep his eyes on her face.");
                    }
                    else if (izzyCreeps[choice] == 2)
                    {
                        outputText("is talking with Amily, sharing stories of the fights she's been in and the enemies she's faced down.  Amily seems interested but unimpressed.");
                    }
                    else if (izzyCreeps[choice] == 3)
                    {
                        outputText("is sitting down chatting with Amily, but the corrupt mousette is just staring at Isabella's boobs and masturbating.  The cow-girl is pretending not to notice.");
                    }
                    else if (izzyCreeps[choice] == 4)
                    {
                        outputText("is sitting down with Izma and recounting some stories, somewhat nervously.  Izma keeps flashing her teeth in a predatory smile.");
                    }
                    else
                    {
                        outputText("is sitting down and thumbing through a book.");
                    }
                }
                else if (model.time.hours == 14)
                {
                    outputText("Isabella is working a grindstone and sharpening her tools.  She even hones the bottom edge of her shield into a razor-sharp cutting edge.  The cow-girl is sweating heavily, but it only makes the diaphanous silk of her top cling more alluringly to her weighty chest.");
                }
                else if (model.time.hours == 15)
                {
                    outputText("The warrior-woman, Isabella is busy constructing dummies of wood and straw, then destroying them with vicious blows from her shield.  Most of the time she finishes by decapitating them with the sharp, bottom edge of her weapon.  She flashes a smile your way when she sees you.");
                }
                else if (model.time.hours == 16)
                {
                    outputText("Isabella is sitting down with a knife, the blade flashing in the sun as wood shavings fall to the ground.  Her hands move with mechanical, practiced rhythm as she carves a few hunks of shapeless old wood into tools or art.");
                }
                else if (model.time.hours == 17)
                {
                    outputText("Isabella is sitting against one of the large rocks near the outskirts of your [camp], staring across the wasteland while idly munching on what you assume to be a leg of lamb.  She seems lost in thought, though that doesn't stop her from throwing a wink and a goofy food-filled grin toward you.");
                }
                else if (model.time.hours == 18)
                {
                    outputText("The dark-skinned cow-girl, Isabella, is sprawled out on a carpet and stretching.  She seems surprisingly flexible for someone with hooves and oddly-jointed lower legs.");
                }
                else if (model.time.hours == 19)
            ...
            if (izmaFollower() && flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 0)
            {
                ...
                {
                    if (rand(6) == 0 && camp.vapulaSlave() && flags[kFLAGS.VAPULA_HAREM_FUCK] > 0)
                    {
                        outputText("Izmael is standing a short distance away with an expression of unadulterated joy on his face, Vapula knelt in front of him and fellating him with noisy enthusiasm.  The shark morph dreamily opens his eyes to catch you staring, and proceeds to give you a huge grin and two solid thumbs up.");
                    }
                    else if (model.time.hours >= 6 && model.time.hours <= 12)
                    {
                        outputText("You keep hearing the sound of objects hitting water followed by peals of male laughter coming from the stream. It sounds as if Izmael is throwing large rocks into the stream and finding immense gratification from the results. In fact, you’re pretty sure that’s exactly what he’s doing.");
                    }
                    else if (model.time.hours <= 16)
                    {
                        outputText("Izmael is a short distance away doing squat thrusts, his body working like a piston, gleaming with sweat. He keeps bobbing his head up to see if anybody is watching him.");
                    }
                    else if (model.time.hours <= 19)
                    {
                        outputText("Izmael is sat against his book chest, masturbating furiously without a care in the world. Eyes closed, both hands pumping his immense shaft, there is an expression of pure, childish joy on his face.");
                    }
                    else if (model.time.hours <= 22)
                    {
                        outputText("Izmael has built a fire and is flopped down next to it. You can’t help but notice that he’s used several of his books for kindling. His eyes are locked on the flames, mesmerized by the dancing light and heat.");
                    }
                    else
                    {
                        outputText("Izmael is currently on his bedroll, sleeping for the night.");
                    }
                    outputText("\n\n");
            ...
            if (followerKiha())
            {
                ...
                {
                    if (kihaFollower.totalKihaChildren() > 0 && flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] > 160 && (flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] % 3 == 0 || model.time.hours == 17))
                    {
                        outputText("Kiha is breastfeeding her offspring right now.\n\n");
                    }
                    else if (kihaFollower.totalKihaChildren() > 0 && flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] > 80 && flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] <= 160 && (flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] % 7 == 0 || model.time.hours == 17))
                    {
                        outputText("Kiha is telling stories to her draconic child" + ((kihaFollower.totalKihaChildren() == 1) ? "" : "ren") + " right now.\n\n");
                    }
                    else
                    {
                        outputText("Kiha's out right now, likely patrolling for demons to exterminate.  You're sure a loud call could get her attention.\n\n");
                    }
                }
                else if (kihaFollower.totalKihaChildren() > 0 && flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] > 160 && (flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] % 3 == 0 || model.time.hours == 20))
                {
                    outputText("Kiha is breastfeeding her offspring right now.\n\n");
                }
                else if (kihaFollower.totalKihaChildren() > 0 && flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] > 80 && flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] <= 160 && (flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] % 7 == 0 || model.time.hours == 20))
                {
                    outputText("Kiha is telling stories to her draconic child" + ((kihaFollower.totalKihaChildren() == 1) ? "" : "ren") + " right now.\n\n");
                }
                else if (kihaFollower.totalKihaChildren() > 0 && flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] <= 80 && (flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] % 3 == 0 || model.time.hours == 20))
                {
                    ...
                    if (rand(2) == 0)
                    {
                        outputText("Right now, she's teaching various techniques.");
                    }
                    else
                    {
                        outputText("Right now, she's teaching her child" + ((kihaFollower.totalKihaChildren() == 1) ? "" : "ren") + " how to make use of axes.\n\n");
                    }
                }
                else
                {
                    ...
                    if (kihaFollower.totalKihaChildren() > 0 && (kihaFollower.totalKihaChildren() >= 2 || flags[kFLAGS.KIHA_CHILD_MATURITY_COUNTER] <= 60))
                    {
                        outputText("Some of them are saved for her child" + ((kihaFollower.totalKihaChildren() == 1) ? "" : "ren") + " to train on. ");
                    }
                    outputText("Most of them are on fire.\n\n");
                }
                if (KihaFollower.DergKidnapped == 1)
                {
                    buttons.add("Kiha", kihaFollower.KihaKidnapped);
                }
                else if (player.statusEffectv3(StatusEffects.CampLunaMishaps1) > 0)
                {
                    buttons.add("Kiha", kihaScene.encounterKiha2).disableIf(player.statusEffectv3(StatusEffects.CampLunaMishaps1) > 0, "Cleaning burnt meat.");
                }
                else
                {
                    buttons.add("Kiha", kihaScene.encounterKiha2).disableIf(player.statusEffectv3(StatusEffects.CampSparingNpcsTimers1) > 0, "Training.");
                }
            }
            //Lily
            ...
            if (player.hasStatusEffect(StatusEffects.CampMarble) && flags[kFLAGS.FOLLOWER_AT_FARM_MARBLE] == 0)
            {
                ...
                if (flags[kFLAGS.MARBLE_PURIFICATION_STAGE] != 4)
                {
                    buttons.add("Marble", marbleScene.interactWithMarbleAtCamp).hint("Go to Marble the cowgirl for talk and companionship.");
                }
            }
            //Nadia
            if (flags[kFLAGS.NADIA_FOLLOWER] >= 6 && !player.hasStatusEffect(StatusEffects.NadiaOff))
            ...
            if (flags[kFLAGS.ANT_WAIFU] > 0)
            {
                ...
                if (flags[kFLAGS.ANT_KIDS] > 0 && flags[kFLAGS.ANT_KIDS] <= 250)
                {
                    outputText(" one of your children exit the anthill to unload some dirt before continuing back down into the colony.  It makes you feel good knowing your offspring are so productive.");
                }
                else if (flags[kFLAGS.ANT_KIDS] > 250 && flags[kFLAGS.ANT_KIDS] <= 1000)
                {
                    outputText(" few of your many children exit the anthill to unload some dirt before vanishing back inside.  It makes you feel good knowing your offspring are so productive.");
                }
                else if (flags[kFLAGS.ANT_KIDS] > 1000)
                {
                    outputText(" some of your children exit the anthill using main or one of the additionally entrances to unload some dirt. Some of them instead of unloading dirt coming out to fulfill some other task that their mother gave them.  You feel a little nostalgic seeing how this former small colony grown to such a magnificent size.");
                }
                else
                {
                    outputText(" Phylla appear out of the anthill to unload some dirt.  She looks over to your campsite and gives you an excited wave before heading back into the colony.  It makes you feel good to know she's so close.");
                }
                outputText("\n\n");
            ...
            if (ZenjiScenes.isLover() && TrollVillage.ZenjiVillageStage != 2)
            {
                ...
                {
                    if (slavesCount() > 0 && rand(5) == 0)
                    {
                        outputText("Zenji is keeping a close eye on some of your more corrupt camp members, ensuring that they don’t cause any harm.");
                    }
                    else if (player.statusEffectv2(StatusEffects.ZenjiModificationsList) >= 998700 && rand(5) == 0)
                    {
                        outputText("Zenji is around your [camp], it’s impossible to miss him as he strokes his length as cascades of cum leak from his erection.");
                    }
                    else if (ZenjiScenes.Z1stKid != "" && rand(5) == 0)
                    {
                        if (rand(2) == 0)
                        {
                            outputText("Zenji is at his bedroll, playing around with " + ((flags[kFLAGS.ZENJI_KIDS] == 1) ? "" + ZenjiScenes.Z1stKid + "" : "some of the children you've made") + ".");
                        }
                        else
                        {
                ...
                {
                    outputText("Zenji is currently lying down on his bedroll, with " + ((flags[kFLAGS.ZENJI_KIDS] == 1) ? "" + ZenjiScenes.Z1stKid + "" : "one of the children you've made") + " sleeping atop his torso, as Zenji gently strokes their head.");
                }
                else if (rand(4) == 0)
                {
                    outputText("Zenji is around your [camp], he gives a small yawn before shaking himself awake.");
                }
                else if (rand(3) == 0)
                {
                    outputText("Zenji is around your [camp], it looks like he’s polishing his tusks and brushing his teeth with something.");
                }
                else if (rand(2) == 0)
                {
                    outputText("Zenji is around your [camp], he is coating his fur in some ashes as he grooms himself.");
                }
                else
                {
                    outputText("With how dark it is, Zenji is keeping much closer to you, making sure that you’re safe. His eyes never seem to stray from you.");
                }
                outputText("\n\n");
    ...
    public function campSlavesMenu(descOnly : Bool = false) : Void
    {
        ...
        if (!(model.time.hours <= 5 || model.time.hours >= 23))
        {
            ...
            if (flags[kFLAGS.PATCHOULI_FOLLOWER] >= PatchouliScene.TIEDINCAMP)
            {
                if (flags[kFLAGS.PATCHOULI_FOLLOWER] == PatchouliScene.TIEDINCAMP)
                {
                    outputText("Patchouli is still tied to a tree. Even incapacitated in this way, he keeps grinning at you, as if taunting you.\n\n");
                }
                else
                {
                    outputText("Patchoulie is lazily resting on a branch in the nearby tree. When she looks at you, she always has that unsettling smile of hers, as if taunting you.\n\n");
                }
    ...
    public function campFollowers(descOnly : Bool = false) : Void
    {
        ...
        //Aether Twins{
            ...
            if (emberScene.followerEmber())
            {
                ...
                if (player.statusEffectv2(StatusEffects.CampLunaMishaps2) > 0)
                {
                    buttons.add("Ember", emberScene.emberCampMenu2).hint("Check up on Ember the dragon-" + ((flags[kFLAGS.EMBER_ROUNDFACE] == 0) ? "morph" : (flags[kFLAGS.EMBER_GENDER] == 1) ? "boy" : "girl") + "").disableIf(player.statusEffectv2(StatusEffects.CampLunaMishaps2) > 0, "Busy searching.");
                }
                else
                {
                    buttons.add("Ember", emberScene.emberCampMenu2).hint("Check up on Ember the dragon-" + ((flags[kFLAGS.EMBER_ROUNDFACE] == 0) ? "morph" : (flags[kFLAGS.EMBER_GENDER] == 1) ? "boy" : "girl") + "").disableIf(player.statusEffectv1(StatusEffects.CampSparingNpcsTimers1) > 0, "Training.");
                }
            }
            //Sophie
            if (sophieFollower() && flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 0 && !player.hasStatusEffect(StatusEffects.SophieOff))
            {
                if (rand(5) == 0)
                {
                    outputText("Sophie is sitting by herself, applying yet another layer of glittering lip gloss to her full lips.\n\n");
                }
                else if (rand(4) == 0)
                {
                    outputText("Sophie is sitting in her nest, idly brushing out her feathers.  Occasionally, she looks up from her work to give you a sultry wink and a come-hither gaze.\n\n");
                }
                else if (rand(3) == 0)
                {
                    outputText("Sophie is fussing around in her nest, straightening bits of straw and grass, trying to make it more comfortable.  After a few minutes, she flops down in the middle and reclines, apparently satisfied for the moment.\n\n");
                }
                else if (rand(2) == 0 || flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] == 0)
                {
                    if (flags[kFLAGS.SOPHIE_BIMBO_ACCEPTED] > 0)
                    {
                        outputText("Your platinum-blonde harpy, Sophie, is currently reading a book - a marked change from her bimbo-era behavior.  Occasionally, though, she glances up from the page and gives you a lusty look.  Some things never change....\n\n");
                    }
                    else
                    {
                        outputText("Your pink harpy, Sophie, is currently reading a book.  She seems utterly absorbed in it, though you question how she obtained it.  Occasionally, though, she'll glance up from the pages to shoot you a lusty look.\n\n");
                    }
                }
                else
                {
                    ...
                    {
                        outputText("surrounded by your daughters");
                    }
            ...
            if (player.hasStatusEffect(StatusEffects.PureCampJojo))
            {
                if (flags[kFLAGS.JOJO_BIMBO_STATE] == 3)
                {
                    ...
                    if (flags[kFLAGS.JOJO_LITTERS] > 0 && model.time.hours >= 16 && model.time.hours < 19)
                    {
                        outputText("You spot the little mice you had with Joy playing about close to her tent.");
                    }
                    else
                    {
                        outputText("Joy herself is nowhere to be found, she's probably out frolicking about or sitting atop the boulder.");
                    }
                    outputText("\n\n");
                ...
                else
                {
                    ...
                    {
                        outputText(" and the mouse is sleeping on it right now.\n\n");
                    }
                    else
                    {
                        outputText(", though the mouse is probably hanging around the [camp]'s perimeter.\n\n");
                    }
            ...
            if (EvangelineFollower.EvangelineFollowerStage >= 1 && flags[kFLAGS.EVANGELINE_WENT_OUT_FOR_THE_ITEMS] <= 0)
            {
                ...
                if (!(model.time.hours > 4 && model.time.hours < 23))
                {
                    outputText(" and she's sleeping on it right now.");
                }
                else
                {
                    outputText(". Though, she probably wandered off not far from [camp], looking for more ingredients to make her potions.");
                }
            ...
            if ((flags[kFLAGS.ZENJI_PROGRESS] == 8 || flags[kFLAGS.ZENJI_PROGRESS] == 9) && TrollVillage.ZenjiVillageStage != 2)
            {
                if (model.time.hours >= 7 && model.time.hours <= 18)
                {
                    if (rand(3) == 0)
                    {
                        outputText("Zenji is around your [camp], you see him currently relaxing atop a tree.");
                    }
                    else if (rand(2) == 0)
                    {
                        outputText("Zenji is around your [camp], you see him doing a routine of stretches.");
                    }
                    else
                    {
                        outputText("Zenji is around your [camp], you see him doing some push-ups.");
                    }
                }
                else if (rand(3) == 0)
                {
                    outputText("Zenji is around your [camp], you see him resting on his bedroll.");
                }
                else if (rand(2) == 0)
                {
                    outputText("Zenji is around your [camp], he is coating his fur in some ashes as he grooms himself.");
            ...
            if (player.hasStatusEffect(StatusEffects.CampRathazul))
            {
                if (flags[kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN] <= 1)
                {
                    ...
                    if (!(model.time.hours > 4 && model.time.hours < 23))
                    {
                        outputText("The alchemist is absent from his usual work location. He must be sleeping right now.");
                    }
                    else
                    {
                        outputText("The alchemist Rathazul looks to be hard at work with his chemicals, working on who knows what.");
                    }
                    if (flags[kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN] == 1)
                    {
                        if (flags[kFLAGS.RATHAZUL_ARMOR_TYPE] < 10)
                        {
                            outputText("  Some kind of spider-silk-based equipment is hanging from a nearby rack.");
                        }
                ...
                {
                    outputText("Mitzi is laying on a cot in Rathazul's lab. A deep blush is present her face and she pants heavily, constantly fading in and out of consciousness.\n\n");
                }
            ...
            else if (flags[kFLAGS.RATHAZUL_SILK_ARMOR_COUNTDOWN] == 1)
            {
                ...
                var _sw0_ = (flags[kFLAGS.RATHAZUL_ARMOR_TYPE]);                
            ...
            if (flags[kFLAGS.KONSTANTIN_FOLLOWER] >= 2 && !player.hasStatusEffect(StatusEffects.KonstantinOff))
            {
                if (model.time.hours >= 6 && model.time.hours <= 8)
                {
                    outputText("Konstantin has dragged out of his [camp] a mat, and is doing some flexing postures under the early morning light. He’s not particularly good at it, so most of times he ends up in awkward positions.");
                }
                else if (model.time.hours <= 12)
                {
                    outputText("You ursine smith is currently at work, sharpening and polishing blades.");
                }
                else if (model.time.hours <= 15)
                {
                    outputText("Konstantin has stopped his work to have a meal, and quite an abundant one. From where you are, you can smell the cooked meat and spice from his plate.");
                }
                else if (model.time.hours <= 18)
                {
                    outputText("The sound of metal on metal is heard, and you easily find the source. The bear smith is currently shaping some plates, polishing them each now and then and putting them aside once he satisfied with how they look.");
                }
                else if (model.time.hours <= 20)
                {
                    outputText("Your local smith, Konstantin isn’t working right now. He has gone to his tent to rest and relax for awhile.");
                }
                else
                {
                    ...
                    {
                        outputText("the starry night above you.");
                    }
                }
                outputText("\n\n");
            ...
            if (flags[kFLAGS.SIDONIE_FOLLOWER] == 1)
            {
                if (model.time.hours >= 6 && model.time.hours <= 8)
                {
                    outputText("Sidonie has taken a table out to have breakfast outside. From what you can see, she’s eating from a bowl filled with oats, milk and strawberries.");
                }
                else if (model.time.hours <= 9)
                {
                    outputText("On a far part of the [camp], you can distinguish Sidonie’s figure. Seems like she’s using the early morning to cut some long planks into smaller ones, as the unmistakable sound of saw on wood makes evident.");
                }
                else if (model.time.hours <= 10)
                {
                    outputText("The equine carpenter is looking at a book containing some furniture designs. She’s probably looking for ideas for her next piece.");
                }
                else if (model.time.hours <= 12)
                {
                    outputText("Near her tent, your equine friend is busy at work, currently sanding some pieces.");
                }
                else if (model.time.hours <= 13)
                {
                    outputText("Sidonie probably went to take her meal, given the hour. The recognizable smell of home-made food coming from her tent confirms quickly your suspicions.");
                }
                else if (model.time.hours <= 15)
                {
                    outputText("Some furniture pieces lie scattered near the equine carpenter’s workspace, who is varnishing them. A few finished ones are a bit away.");
                }
                else if (model.time.hours <= 16)
                {
                    outputText("Looks like Sidonie went to some place to sell her furniture. Se may return in an hour or so.");
                }
                else if (model.time.hours <= 18)
                {
                    outputText("Your equine friend is currently relaxing inside her tent, if you’d like to come in and spend some time with her.");
                }
                else if (model.time.hours <= 20)
                {
                    outputText("Oddly enough, Sidonie is picking some sandalwood sawdust and putting it on boiling water. Not matter the reason, the smell of the resulting liquid is certainly wonderful.");
                }
                else
                {
                    outputText("The horse-girl is having her dinner, as you manage to spot from outside. Seems like she’s having a hot drink with some bread.");
                }
                outputText("\n\n");
            ...
            if (flags[kFLAGS.PC_GOBLIN_DAUGHTERS] > 0)
            {
                buttons.add("Goblin kids", campScenes.PCGoblinDaughters).hint("Check up on your goblin daughters.");
            }
            //Tifa
            if (flags[kFLAGS.TIFA_FOLLOWER] > 5)
            {
        ...
        if (flags[kFLAGS.LUNA_FOLLOWER] >= 4 && !player.hasStatusEffect(StatusEffects.LunaOff))
        {
            ...
            if (flags[kFLAGS.LUNA_JEALOUSY] >= 100)
            {
                outputText(" She looks at you from time to time, as if expecting you to notice her.");
            }
            outputText("\n\n");
    ...
    public function campActions() : Void
    {
        ...
        if (SceneLib.garden.canAccessGarden())
        {
            addButton(6, "Garden", SceneLib.garden.accessGarden).hint("Manage your garden of medicinal plants.");
        }
        else
        {
            addButtonDisabled(6, "Garden", "Req. to have Herb bag of any sort.");
        }
        ...
        if (SceneLib.exploration.counters.explore >= 1)
        {
            addButton(12, "Dummy", dummyTraining).hint("Train your mastery level on this dummy.").disableIf(isNightTime && !player.isNightCreature(), "It's too dark for that!");
        }
        if (player.hasPerk(PerkLib.Soulless))
        {
            addButton(13, "Dark Ascension", promptDarkAscend).hint("Perform an ascension? This will restart your adventures. The game depending on your choice would also get harder.").disableIf(flags[kFLAGS.LETHICE_DEFEATED] <= 0, "Don't you have a job to finish first? Like... to defeat someone, maybe Lethice?");
        }
        else
        {
    ...
    private function campSpendTimeActions() : Void
    {
        ...
        if (model.time.hours == 19)
        {
            addButton(2, "Watch Sunset", watchSunset).hint("Watch the sunset and relax.");
        }
        else if ((model.time.hours >= 20 || model.time.hours <= 5) && flags[kFLAGS.LETHICE_DEFEATED] > 0)
        {
            addButton(2, "Stargaze", watchStars).hint("Look at the starry night sky.");
        }
        ...
        {
            addButton(9, "Gryphon", useGryphonStatuette);
        }
        if (player.hasKeyItem("Peacock Statuette") >= 0)
        {
            addButton(9, "Peacock", usePeacockStatuette);
        }
        if (player.hasKeyItem("Gryphon Statuette") == 0 && player.hasKeyItem("Peacock Statuette") == 0)
        {
            addButtonDisabled(9, "???", "Perhaps if you had a magical statuette...");
        }
    ...
    public function campBuildingSim() : Void
    {
        ...
        if (player.hasKeyItem("Carpenter's Toolbox") >= 0)
        {
            if (flags[kFLAGS.CAMP_WALL_PROGRESS] < 100)
            {
                if (getCampPopulation() >= 4)
                {
                    addButton(0, "Build Wall", buildCampWallPrompt).hint("Build a wall around your [camp] to defend from the imps." + ((flags[kFLAGS.CAMP_WALL_PROGRESS] >= 10) ? "\n\nProgress: " + (flags[kFLAGS.CAMP_WALL_PROGRESS] / 10) + "/10 complete" : "") + "");
                }
                else
                {
                    addButtonDisabled(0, "Build Wall", "Req. 4+ camp population.");
                }
            ...
            if (flags[kFLAGS.CAMP_WALL_PROGRESS] >= 100)
            {
                ...
                if (flags[kFLAGS.CAMP_WALL_GATE] > 0)
                {
                    addButtonDisabled(1, "Build Gate", "Already built.");
                }
                else
                {
                    addButton(1, "Build Gate", buildCampGatePrompt).hint("Build a gate to complete your [camp] defense.");
                }
            }
        ...
        if (flags[kFLAGS.CAMP_CABIN_PROGRESS] > 0 && flags[kFLAGS.CAMP_CABIN_PROGRESS] < 10)
        {
            addButton(2, "Build Cabin", cabinProgress.initiateCabin).hint("Work on your cabin.");
        }
        //Work on cabin.
        else if (flags[kFLAGS.CAMP_CABIN_PROGRESS] == 0)
        {
            addButtonDisabled(2, "Build Cabin", "You need to wait until 7th day.");
        }
        else
        {
            addButtonDisabled(2, "Build Cabin", "Looking at your cabin, it's evidently already built.");
        }
        ...
        {
            addButtonDisabled(10, "AddImpSkull", "Req. built at least one wall section.");
        }
        addButton(14, "Back", campActions);
    }
    ...
    private function campMiscActions() : Void
    {
        ...
        if (flags[kFLAGS.CAMP_UPGRADES_FISHERY] >= 1)
        {
            addButton(0, "Fishery", VisitFishery).hint("Visit Fishery.");
        }
        else
        {
            addButtonDisabled(0, "Fishery", "Would you kindly build it first?");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_MAGIC_WARD] >= 2)
        {
            addButton(1, "Ward", MagicWardMenu).hint("Activate or Deactivate Magic Ward around [camp].");
        }
        else
        {
            addButtonDisabled(1, "Ward", "Would you kindly install it first?");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_KITSUNE_SHRINE] >= 4)
        {
            addButton(2, "Kitsune Shrine", campScenes.KitsuneShrine).hint("Meditate at [camp] Kitsune Shrine.");
        }
        else
        {
            addButtonDisabled(2, "Kitsune Shrine", "Would you kindly build it first?");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_HOT_SPRINGS] >= 4)
        {
            addButton(3, "Hot Spring", campScenes.HotSpring).hint("Visit Hot Spring.").disableIf(isNightTime, "It's not safe to take a dip in the hotsprings at night.");
        }
        else
        {
            addButtonDisabled(3, "Hot Spring", "Would you kindly build it first?");
        }
        if (player.hasPerk(PerkLib.CursedTag))
        {
            addButton(4, "AlterBindScroll", AlterBindScroll).hint("Alter Bind Scroll - DIY aka modify your cursed tag").disableIf(isNightTime, "It's too dark to modify your scroll.");
        }
        else
        {
            addButtonDisabled(4, "Alter Bind Scroll", "Req. to be Jiangshi and having Cursed Tag perk.");
        }
        var bottles : Array<Dynamic> = [
        ...
        for (i in 0...bottles.length)
        {
            addButton(5 + i, "FillBottle(" + bottles[i][2].charAt(0).toUpperCase() + ")", fillUpPillBottle.apply, this, bottles[i]).hint("Fill up one of your pill bottles with " + bottles[i][2] + "-grade soulforce recovery pills.").disableIf(!player.hasItem(bottles[i][0], 10) || !player.hasItem(useables.E_P_BOT, 1), 
        ...
        {
            addButton(10, "Clone", CloneMenu).hint("Check on your clone(s).");
        }
        else
        {
            addButtonDisabled(10, "Clone", "Would you kindly go face F class Heaven Tribulation first?");
        }
        addButtonIfTrue(11, "Pocket Watch", mainPagePocketWatch, "Req. having Pocket Watch key item.", player.hasKeyItem("Pocket Watch") >= 0);
        if (player.hasItem(useables.ENECORE, 1) && CampStatsAndResources.EnergyCoreResc < 200)
        {
            addButton(12, "E.Core", convertingEnergyCoreIntoFlagValue).hint("Convert Energy Core item into flag value.");
        }
        if (player.hasItem(useables.MECHANI, 1) && CampStatsAndResources.MechanismResc < 200)
        {
            addButton(12, "C.Mechan", convertingMechanismIntoFlagValue).hint("Convert Mechanism item into flag value.");
        }
    ...
    public function menuForCombiningAndSeperating() : Void
    {
        ...
        menu();
        var weaponList : Array<Dynamic> = weapons.SingleDualPairList.concat(weaponsrange.SingleDualPairList);
    ...
    public function menuCombineStaging(weaponList : Array<Dynamic>) : Void
    {
        menu();
        var bd : ButtonDataList = new ButtonDataList();
        for (weapongroup in weaponList)
        {
            bd.add(Reflect.field(weapongroup, Std.string(0)).name, curry(menuCombining, Reflect.field(weapongroup, Std.string(0)), Reflect.field(weapongroup, Std.string(1)))).disableIf(player.itemCount(Reflect.field(weapongroup, Std.string(0))) < 2, "You need more than one " + Reflect.field(weapongroup, Std.string(0)).name + "to make a " + Reflect.field(weapongroup, Std.string(1)).name + "!").hint("Combine 2 " + Reflect.field(weapongroup, Std.string(0)).name + " into a " + Reflect.field(weapongroup, Std.string(1)).name);
        }
        submenu(bd, menuForCombiningAndSeperating, 0, false);
    }
    
    public function menuSeperateStaging(weaponList : Array<Dynamic>) : Void
    {
        menu();
        var bd : ButtonDataList = new ButtonDataList();
        for (weapongroup in weaponList)
        {
            bd.add(Reflect.field(weapongroup, Std.string(1)).name, curry(menuSeparating, Reflect.field(weapongroup, Std.string(1)), Reflect.field(weapongroup, Std.string(0)))).disableIf(player.itemCount(Reflect.field(weapongroup, Std.string(1))) == 0, "You need at least one " + Reflect.field(weapongroup, Std.string(1)).name + "to break back down to two " + Reflect.field(weapongroup, Std.string(0)).name + "!").hint("Disassemble " + Reflect.field(weapongroup, Std.string(1)).name + " to get 2 " + Reflect.field(weapongroup, Std.string(0)).name);
        }
        submenu(bd, menuForCombiningAndSeperating, 0, false);
    }
    
    public function menuCombining(weapon1 : Dynamic, weapon2 : Dynamic) : Void
    {
        clearOutput();
    ...
    public function menuSeparating(weapon1 : Dynamic, weapon2 : Dynamic) : Void
    {
        clearOutput();
    ...
    public function menuSeparating1a(weapon2 : Dynamic) : Void
    {
        outputText("\n\n");
        inventory.takeItem(weapon2, menuForCombiningAndSeperating);
    }
    ...
    public function mainPagePocketWatch(page : Int = 1) : Void
    {
        ...
        if (page == 2)
        {
            ...
            if (player.hasPerk(PerkLib.DaoOfTheElements))
            {
                if (player.hasPerk(PerkLib.ElementalContractRank9) && player.hasPerk(PerkLib.ElementsOfMarethBasics) && player.perkv1(PerkLib.DaoOfTheElements) < 2)
                {
                    addButton(1, "DotE (layer 2)", mainPagePocketWatchDaoOfTheElementsPerkLayer2);
                }
                else if (player.perkv1(PerkLib.DaoOfTheElements) >= 2)
                {
                    addButtonDisabled(1, "DotE (layer 2)", "You already have this merged perk.");
                }
                else
                {
            ...
            {
                if (player.hasPerk(PerkLib.ElementalContractRank13) && player.hasPerk(PerkLib.ElementsOfMarethAdvanced) && player.perkv1(PerkLib.DaoOfTheElements) < 3)
                {
                    addButton(2, "DotE (layer 3)", mainPagePocketWatchDaoOfTheElementsPerkLayer3);
                }
                else if (player.perkv1(PerkLib.DaoOfTheElements) >= 3)
                {
                    addButtonDisabled(2, "DotE (layer 3)", "You already have this merged perk.");
                }
                else
                {
                    addButtonDisabled(2, "DotE (layer 3)", "Req. Elemental Contract Rank 13 & Elements of Mareth: Advanced & Dao of the Elements (layer 2) perks.");
                }
            }
            else
            {
        ...
        if (page == 3)
        {
            addButton(0, "E C M & B R (Ex)", mainPagePocketWatchElementalConjurerMindAndBodyResolveEx).disableIf(!player.hasPerk(PerkLib.ElementalConjurerMindAndBodyResolve), 
            ...
            addButton(1, "E C M & B D (Ex)", mainPagePocketWatchElementalConjurerMindAndBodyDedicationEx).disableIf(!player.hasPerk(PerkLib.ElementalConjurerMindAndBodyDedication) || !player.hasPerk(PerkLib.ElementalConjurerMindAndBodyResolveEx), 
            ...
            addButton(2, "E C M & B S (Ex)", mainPagePocketWatchElementalConjurerMindAndBodySacrificeEx).disableIf(!player.hasPerk(PerkLib.ElementalConjurerMindAndBodySacrifice) || !player.hasPerk(PerkLib.ElementalConjurerMindAndBodyDedicationEx), 
        ...
        if (page == 4)
        {
            addButton(0, "ChBS-I (Ex)", mainPagePocketWatchChimericalBodySemiImprovedStageEx).disableIf(!player.hasPerk(PerkLib.ChimericalBodySemiImprovedStage), 
            ...
            addButton(1, "ChBS-S (Ex)", mainPagePocketWatchChimericalBodySemiSuperiorStageEx).disableIf(!player.hasPerk(PerkLib.ChimericalBodySemiSuperiorStage) || !player.hasPerk(PerkLib.ChimericalBodySemiImprovedStageEx), 
            ...
            addButton(2, "ChBS-E (Ex)", mainPagePocketWatchChimericalBodySemiEpicStageEx).disableIf(!player.hasPerk(PerkLib.ChimericalBodySemiEpicStage) || !player.hasPerk(PerkLib.ChimericalBodySemiSuperiorStageEx), 
        ...
        if (page == 5)
        {
            addButton(0, "Raging Inferno (Mst)", mainPagePocketWatchRagingInfernoMastered).disableIf(!player.hasPerk(PerkLib.RagingInfernoSu), "Req. Raging Inferno (Su) perk").disableIf(player.hasPerk(PerkLib.RagingInfernoMastered), "You already got this merged perk");
            
            addButton(1, "Glacial Storm (Mst)", mainPagePocketWatchGlacialStormMastered).disableIf(!player.hasPerk(PerkLib.GlacialStormSu), "Req. Glacial Storm (Su) perk").disableIf(player.hasPerk(PerkLib.GlacialStormMastered), "You already got this merged perk");
            
            addButton(2, "High Voltage (Mst)", mainPagePocketWatchHighVoltageMastered).disableIf(!player.hasPerk(PerkLib.HighVoltageSu), "Req. High Voltage (Su) perk").disableIf(player.hasPerk(PerkLib.HighVoltageMastered), "You already got this merged perk");
            
            addButton(3, "Eclipsing Shadow (Mst)", mainPagePocketWatchEclipsingShadowMastered).disableIf(!player.hasPerk(PerkLib.EclipsingShadowSu), "Req. Eclipsing Shadow (Su) perk").disableIf(player.hasPerk(PerkLib.EclipsingShadowMastered), "You already got this merged perk");
            
            addButton(4, "Archmage (Ex)", mainPagePocketWatchArchmageEx).disableIf(!player.hasPerk(PerkLib.Archmage), "Req. Archmage perk").disableIf(player.hasPerk(PerkLib.ArchmageEx), "You already got this merged perk");
            
            addButton(5, "High Tide (Mst)", mainPagePocketWatchHighTideMastered).disableIf(!player.hasPerk(PerkLib.HighTideSu), "Req. High Tide (Su) perk").disableIf(player.hasPerk(PerkLib.HighTideMastered), "You already got this merged perk");
            
            addButton(6, "Howling Gale (Mst)", mainPagePocketWatchHowlingGaleMastered).disableIf(!player.hasPerk(PerkLib.HowlingGaleSu), "Req. Howling Gale (Su) perk").disableIf(player.hasPerk(PerkLib.HowlingGaleMastered), "You already got this merged perk");
            
            addButton(7, "Rumbling Quake (Mst)", mainPagePocketWatchRumblingQuakeMastered).disableIf(!player.hasPerk(PerkLib.RumblingQuakeSu), "Req. Rumbling Quake (Su) perk").disableIf(player.hasPerk(PerkLib.RumblingQuakeMastered), "You already got this merged perk");
            
            addButton(8, "Corrosive Meltdown (Mst)", mainPagePocketWatchCorrosiveMeltdownMastered).disableIf(!player.hasPerk(PerkLib.CorrosiveMeltdownSu), "Req. Corrosive Meltdown (Su) perk").disableIf(player.hasPerk(PerkLib.CorrosiveMeltdownMastered), "You already got this merged perk");
        ...
        if (page == 6)
        {
            addButton(0, "G.Diehard (Ex)", mainPagePocketWatchGreaterDiehardEx).disableIf(!player.hasPerk(PerkLib.GreaterDiehard), "Req. Greater Diehard perk").disableIf(player.hasPerk(PerkLib.GreaterDiehardEx), "You already got this merged perk");
    ...
    public function campWinionsArmySim() : Void
    {
        ...
        {
            addButton(0, "Make", SceneLib.campMakeWinions.accessMakeWinionsMainMenu).hint("Check your options for making some golems.");
        }
        else
        {
            addButtonDisabled(0, "Make", "You need to learn Golemancer job to use this option.");
        }player.hasPerk(PerkLib.JobElementalConjurer);
        if (flags[kFLAGS.CAMP_UPGRADES_ARCANE_CIRCLE] > 0)
        {
            addButton(1, "Summon", SceneLib.campMakeWinions.accessSummonElementalsMainMenu).hint("Check your options for managing your elemental summons.");
        }
        else
        {
            addButtonDisabled(1, "Summon", "You should first build Arcane Circle. Without some tools from the carpenter's toolbox it would be near impossible to do this OR you not yet learned Elemental Conjurer job.");
        }
        if (player.hasPerk(PerkLib.JobTamer))
        {
            addButton(2, "Tame", SceneLib.campMakeWinions.accessTamedWinionsMainMenu).hint("Check your options for tamed minions.");
        }
        else
        {
            addButtonDisabled(2, "Tame", "You need to learn Tamer job to use this option.");
        }
        if (player.hasPerk(PerkLib.PrestigeJobNecromancer))
        {
            addButton(5, "Skeletons", SceneLib.campMakeWinions.accessMakeSkeletonWinionsMainMenu).hint("Check your options for making some skeletons.");
        }
        else
        {
            addButtonDisabled(5, "Skeletons", "You need to learn Necromancer job to use this option.");
        }
        if (player.hasPerk(PerkLib.PrestigeJobDruid))
        {
            addButton(6, "Fusions", druidMenu);
        }
        else
        {
            addButtonDisabled(6, "Fusions", "You need to learn Druid job to use this option.");
        }
        addButton(14, "Back", campActions);
    }
    private function druidMenu() : Void
    {
        ...
        if (player.hasPerk(PerkLib.SharedPower) && player.perkv1(PerkLib.SharedPower) > 0)
        {
            outputText("\n\n+" + player.perkv1(PerkLib.SharedPower) + "0% multi bonus to health and damage when in an infused state");
        }  //temporaly to check if perk working as intended  
        menu();
        ...
        else
        {
            ...
            {
                if (player.hasPerk(PerkLib.EclipticInfusion))
                {
                    addButton(0, "Air", druidMenuAir);
                }
                else
                {
                    addButton(0, "Air", druidMenuFuseScene, "air", 1, 0);
                }
            }
            else
            {
                addButtonDisabled(0, "Air", "You need to summon Epic Air Elemental first before trying to use this fusion option.");
            }
            if (player.hasStatusEffect(StatusEffects.SummonedElementalsEarthE))
            {
                if (player.hasPerk(PerkLib.EclipticInfusion))
                {
                    addButton(1, "Earth", druidMenuEarth);
                }
                else
                {
                    addButton(1, "Earth", druidMenuFuseScene, "earth", 2, 0);
                }
            }
            else
            {
                addButtonDisabled(1, "Earth", "You need to summon Epic Earth Elemental first before trying to use this fusion option.");
            }
            if (player.hasStatusEffect(StatusEffects.SummonedElementalsFireE))
            {
                if (player.hasPerk(PerkLib.EclipticInfusion))
                {
                    addButton(2, "Fire", druidMenuFire);
                }
                else
                {
                    addButton(2, "Fire", druidMenuFuseScene, "fire", 3, 0);
                }
            }
            else
            {
                addButtonDisabled(2, "Fire", "You need to summon Epic Fire Elemental first before trying to use this fusion option.");
            }
            if (player.hasStatusEffect(StatusEffects.SummonedElementalsWaterE))
            {
                if (player.hasPerk(PerkLib.EclipticInfusion))
                {
                    addButton(3, "Water", druidMenuWater);
                }
                else
                {
                    addButton(3, "Water", druidMenuFuseScene, "water", 4, 0);
                }
            }
            else
            {
                addButtonDisabled(3, "Water", "You need to summon Epic Water Elemental first before trying to use this fusion option.");
            }
    ...
    private function druidMenuAir() : Void
    {
        ...
        {
            addButton(1, "Purity", druidMenuFuseScene, "air", 1, 2);
        }
        else
        {
            addButtonDisabled(1, "Purity", "You need to summon Normal Purity Elemental first before trying to use this fusion option.");
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsCorruption))
        {
            addButton(2, "Corruption", druidMenuFuseScene, "air", 1, 1);
        }
        else
        {
            addButtonDisabled(2, "Corruption", "You need to summon Normal Corruption Elemental first before trying to use this fusion option.");
        }
    ...
    private function druidMenuEarth() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsPurity))
        {
            addButton(1, "Purity", druidMenuFuseScene, "earth", 2, 2);
        }
        else
        {
            addButtonDisabled(1, "Purity", "You need to summon Normal Purity Elemental first before trying to use this fusion option.");
        ...
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsCorruption))
        {
            addButton(2, "Corruption", druidMenuFuseScene, "earth", 2, 1);
        }
        else
        {
            addButtonDisabled(2, "Corruption", "You need to summon Normal Corruption Elemental first before trying to use this fusion option.");
    ...
    private function druidMenuFire() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsPurity))
        {
            addButton(1, "Purity", druidMenuFuseScene, "fire", 3, 2);
        }
        else
        {
            addButtonDisabled(1, "Purity", "You need to summon Normal Purity Elemental first before trying to use this fusion option.");
        ...
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsCorruption))
        {
            addButton(2, "Corruption", druidMenuFuseScene, "fire", 3, 1);
        }
        else
        {
            addButtonDisabled(2, "Corruption", "You need to summon Normal Corruption Elemental first before trying to use this fusion option.");
    ...
    private function druidMenuWater() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsPurity))
        {
            addButton(1, "Purity", druidMenuFuseScene, "water", 4, 2);
        }
        else
        {
            addButtonDisabled(1, "Purity", "You need to summon Normal Purity Elemental first before trying to use this fusion option.");
        ...
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsCorruption))
        {
            addButton(2, "Corruption", druidMenuFuseScene, "water", 4, 1);
        }
        else
        {
            addButtonDisabled(2, "Corruption", "You need to summon Normal Corruption Elemental first before trying to use this fusion option.");
    ...
    private function druidMenuFuseScene(element : String, type : Float, type2 : Float) : Void
    {
        clearOutput();
        ...
        if (type == 1)
        {
            outputText("light green");
        }
        if (type == 2)
        {
            outputText("dark brown");
        }
        if (type == 3)
        {
            outputText("tan");
        ...
        if (type == 1)
        {
            outputText("orange like the twilight sky");
        }
        if (type == 2)
        {
            outputText("green like a leaf");
        }
        if (type == 3)
        {
            outputText("orange like glittering embers");
        }
        if (type == 4)
        {
            outputText("turquoise like the sea");
        }
        outputText(".\n\n");
        if (type == 1)
        {
            outputText("A Constant gust of wind seems to constantly lift the ethereal skirt that formed beneath your chest like a bell or a parachute. The four winds answer your masterful call like an old friend, always happy to please whether with a smooth breeze or with a violent gust. You have acquired true control over the winds.\n\n");
        }
        if (type == 2)
        {
            outputText("In your hair grows a small vine decorated with leaves, and your skin has achieved the durability of hard rock. Your legs and hands reshape, craggy and golem-like; large and crude but very powerful. Your feet are embedded in stone and heavily but not for you, the weight of the world seems no longer to affect you, rather you have acquired true control over not only rock and ground but also soil and plants.\n\n");
        }
        if (type == 3)
        {
            outputText("Your hair, wrist, and knees burn with the excess of your fiery power, the fire ready to do your bidding. Some say you shouldn’t play with fire, but who’s going to stop you now? You have achieved true control over the fire element.\n\n");
        }
        if (type == 4)
        {
            outputText("Your hairs now regularly drip fluid on the ground beneath you if you forget them. Not only that but you seem to be able to control and generate what appears to be an endless quantity of water. You have acquired true control over water.\n\n");
        }
        if (player.hasPerk(PerkLib.SharedPower))
        {
            player.addPerkValue(PerkLib.SharedPower, 1, druidMenuSharedPowerPerkCounting());
        }
        var tier : Float = 1;
        if (type == 1 && player.statusEffectv2(StatusEffects.SummonedElementalsAirE) > 3)
        {
            if (player.statusEffectv2(StatusEffects.SummonedElementalsAirE) > 9)
            {
                tier += 3;
            }
            else if (player.statusEffectv2(StatusEffects.SummonedElementalsAirE) > 6)
            {
                tier += 2;
            }
            else
        ...
        {
            if (player.statusEffectv2(StatusEffects.SummonedElementalsEarthE) > 9)
            {
                tier += 3;
            }
            else if (player.statusEffectv2(StatusEffects.SummonedElementalsEarthE) > 6)
            {
                tier += 2;
            }
            else
        ...
        {
            if (player.statusEffectv2(StatusEffects.SummonedElementalsFireE) > 9)
            {
                tier += 3;
            }
            else if (player.statusEffectv2(StatusEffects.SummonedElementalsFireE) > 6)
            {
                tier += 2;
            }
            else
        ...
        {
            if (player.statusEffectv2(StatusEffects.SummonedElementalsWaterE) > 9)
            {
                tier += 3;
            }
            else if (player.statusEffectv2(StatusEffects.SummonedElementalsWaterE) > 6)
            {
                tier += 2;
            }
            else
        ...
        if (type == 1)
        {
            player.createPerk(PerkLib.AffinitySylph, 0, 0, 0, 0);
        }
        if (type == 2)
        {
            player.createPerk(PerkLib.AffinityGnome, 0, 0, 0, 0);
        }
        if (type == 3)
        {
            player.createPerk(PerkLib.AffinityIgnis, 0, 0, 0, 0);
        }
        if (type == 4)
        {
            player.createPerk(PerkLib.AffinityUndine, 0, 0, 0, 0);
        }
    ...
    private function druidMenuUnfuseScene() : Void
    {
        ...
        if (player.hasPerk(PerkLib.SharedPower))
        {
            player.setPerkValue(PerkLib.SharedPower, 1, 0);
        }
        player.removePerk(PerkLib.ElementalBody);
        if (player.hasPerk(PerkLib.AffinitySylph))
        {
            player.removePerk(PerkLib.AffinitySylph);
        }
        if (player.hasPerk(PerkLib.AffinityGnome))
        {
            player.removePerk(PerkLib.AffinityGnome);
        }
        if (player.hasPerk(PerkLib.AffinityIgnis))
        {
            player.removePerk(PerkLib.AffinityIgnis);
        }
        if (player.hasPerk(PerkLib.AffinityUndine))
        {
            player.removePerk(PerkLib.AffinityUndine);
        }
        CoC.instance.mainViewManager.updateCharviewIfNeeded();
        player.HP = oldHPratio * player.maxHP();
        statScreenRefresh();
    ...
    private function druidMenuSharedPowerPerkCounting() : Float
    {
        var dmSPPC : Float = 12;
        ...
        if (player.hasPerk(PerkLib.ElementalBondFlesh))
        {
            dmSPPC += 1;
        }
        ...
        if (player.hasPerk(PerkLib.ElementalBondUrges))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.StrongElementalBond))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalConjurerKnowledge))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.StrongElementalBondEx))
        {
            dmSPPC += 1;
        }
        ...
        if (player.hasPerk(PerkLib.StrongElementalBondSu))
        {
            dmSPPC += 1;
        }
        ...
        if (player.hasPerk(PerkLib.ElementalContractRank9))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalConjurerSacrifice))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalContractRank10))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalContractRank11))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalTouch))
        {
            dmSPPC += 1;
        }
        ...
        if (player.hasPerk(PerkLib.GreaterSharedPower))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalContractRank13))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.StrongerElementalBondEx))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.EclipticInfusion))
        ...
        if (player.hasPerk(PerkLib.ElementalContractRank14))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalContractRank15))
        {
            dmSPPC += 1;
        }
        ...
        if (player.hasPerk(PerkLib.StrongerElementalBondSu))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalContractRank17))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalContractRank18))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalContractRank19))
        {
            dmSPPC += 1;
        }
        ...
        if (player.hasPerk(PerkLib.ElementalContractRank21))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalContractRank22))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalContractRank23))
        {
            dmSPPC += 1;
        }
        ...
        if (player.hasPerk(PerkLib.ElementalContractRank25))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalContractRank26))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalContractRank27))
        {
            dmSPPC += 1;
        }
        ...
        if (player.hasPerk(PerkLib.ElementalContractRank29))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalContractRank30))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalContractRank31))
        {
            dmSPPC += 1;
        }
        if (player.hasPerk(PerkLib.ElementalConjurerMindAndBodySacrificeEx))
        {
            dmSPPC += 4;
        }
        if (player.hasPerk(PerkLib.DaoOfTheElements))
        {
            dmSPPC += 5;
            if (player.perkv1(PerkLib.DaoOfTheElements) > 1)
            {
                dmSPPC += (5 * (player.perkv1(PerkLib.DaoOfTheElements) - 1));
            }
            if (player.perkv1(PerkLib.DaoOfTheElements) == 7)
            {
                dmSPPC -= 1;
            }
        }
    ...
    private function VisitFishery() : Void
    {
        ...
        if (flags[kFLAGS.FISHES_STORED_AT_FISHERY] > 0)
        {
            outputText(" There is currently " + flags[kFLAGS.FISHES_STORED_AT_FISHERY] + " fish in the barrel.");
        }
        menu();
        if (flags[kFLAGS.FISHES_STORED_AT_FISHERY] > 0)
        {
            addButton(0, "Retrieve", Retrieve);
        }
        if (flags[kFLAGS.FISHES_STORED_AT_FISHERY] > 0)
        {
            addButton(1, "Retrieve Stack", RetrieveStack);
        }
        addButton(14, "Back", campMiscActions);
    }
    ...
    public function FisheryDailyProduction() : Float
    {
        var fishproduction : Float = 0;
    ...
    public function FisheryWorkersCount() : Float
    {
        var fisheryworkers : Float = 0;
        if (flags[kFLAGS.FOLLOWER_AT_FISHERY_1] != "")
        {
            fisheryworkers += 1;
        }
    ...
    public function FisheryMaxWorkersCount() : Float
    {
        var fisherymaxworkers : Float = 1;
        if (flags[kFLAGS.CAMP_UPGRADES_FISHERY] >= 2)
        {
            fisherymaxworkers += 1;
        }
    ...
    private function AlterBindScroll() : Void
    {
        ...
        for (i in 0...statusNames.length)
        {
            if (player.hasStatusEffect(statusNames[i][0]))
            {
                outputText("<b>- " + statusNames[i][1] + "</b>\n");
            }
        }
        ...
        for (i in 0...statusNames.length)
        {
            ...
            else if (currentAltering < limitOnAltering)
            {
                button(i).hint("You don't have this power active. Do you want to activate it?");
            }
            else
            {
                button(i).disable("You already have the maximum amount of powers active as you can maintain without breaking yourself.");
            }
        }
        addButton(14, "Back", campMiscActions);
    }
    ...
    private function alterBindScrollToggle(status : StatusEffectType) : Void
    {
        ...
        {
            player.removeStatusEffect(status);
        }
        else
        {
            player.createStatusEffect(status, 0, 0, 0, 0);
        }
        AlterBindScroll();
    }
	 ...
	 */public function gcc(buffexist : Bool = false) : Float
    {
        if (buffexist)
        {
            return (player.hasStatusEffect(StatusEffects.PCClone)) ? 1 : 0;
        }
        if (player.hasStatusEffect(StatusEffects.PCClone))
        {
            return player.statusEffectv3(StatusEffects.PCClone);
        }
        return 0;
    }
    public function maximumClonesCount(absolutemax : Bool = false) : Float
    // Don't forget to extend Soulforce.clones[] and related buffs if increasing
    {
        
        if (absolutemax)
        {
            return Soulforce.clones.length;
        }
        var mCC : Float = Soulforce.clones.length;
    ...
    private function CloneMenu() : Void
    {
        ...
        if (gcc() > 0)
        {
            ...
            for (clone in 0...Soulforce.clones.length)
            {
                ...
                if (player.statusEffectv1(Soulforce.clones[clone]) > 10 && player.statusEffectv1(Soulforce.clones[clone]) < 21)
                {
                    ...
                    for (d in 0...Soulforce.daos.length)
                    {
        ...
        {
            outputText("You do not have a clone right now, whether you've never made one or one was sacrificed. You would need to make a new one, first.");
        }
        outputText("\n\n");
        menu();
        addButton(13, "Create", FormClone).disableIf(player.isGargoyle(), "You can't clone your stone body!").disableIf(player.negativeLevel > 0, "You need to regain your power before you can create another clone. Unrecovered levels: " + player.negativeLevel).disableIf(gcc() >= maximumClonesCount(), "You cannot have more than " + maximumClonesCount() + " clone" + ((maximumClonesCount() > 1) ? "s" : "") + " right now.").disableIf(gcc() >= maximumClonesCount(true), "You cannot have more than " + maximumClonesCount() + " clones.").disableIf(player.HP <= player.maxHP() * 0.9, "Your HP is too low.").disableIf(player.soulforce < player.maxSoulforce() * 0.9, "Your Soulforce is too low.");
        ...
        for (clone in 0...Soulforce.clones.length)
        {
            ...
            {
                continue;
            }  // short circuit for too many, maybe add pagination if clone cap gets upped for some reason  
            addButton(clone, "Contempl. (" + (clone + 1) + ")", cloneContemplateDao, clone).hint("Task your clone (" + (clone + 1) + ") with contemplating one of the Daos you know.").disableIf(!player.hasStatusEffect(Soulforce.clones[clone]), "Req. fully formed clone (" + (clone + 1) + ").");
        }
        addButton(14, "Back", campMiscActions);
    }
    ...
    private function FormClone() : Void
    {
        ...
        if (gcc(true))
        {
            for (i in gcc()...maximumClonesCount())
            {
                if (player.hasStatusEffect(Soulforce.clones[i - 1]))
                {
                    newClone = i;
                }
            }
	...
	*/
    private function cloneContemplateDao(clone : Int) : Void
    {
        ...
        for (d in 0...Soulforce.daos.length)
        {
            addButton(btn++, Soulforce.daos[d][0], cloneContemplateDaoSet, clone, Soulforce.daos[d][2]).disableIf(player.statusEffectv1(Soulforce.clones[clone]) == Soulforce.daos[d][2], "Your clone (" + clone + ") is currently contemplating this Dao.");
        }
        addButton(13, "None", cloneContemplateDaoSet, clone, 10).disableIf(player.statusEffectv1(Soulforce.clones[clone]) == 10, "Your clone (" + clone + ") is currently not contemplating any Dao.");
        addButton(14, "Back", CloneMenu);
    }
    
    private function cloneContemplateDaoSet(clone : Int, newdao : Float) : Void
    {
        player.changeStatusValue(Soulforce.clones[clone], 1, newdao);
        cloneContemplateDao(clone);
    }
    
    private function dummyTraining(lvl : Int = 0, lustImmune : Bool = true, isGroup : Bool = false) : Void
    {
        clearOutput();
        ...
        menu();
        addButton(0, "Lvl0", dummyTraining, 0, lustImmune, isGroup).disableIf(lvl == 0, "Already selected");
        addButton(1, "Lvl30", dummyTraining, 1, lustImmune, isGroup).disableIf(lvl == 1, "Already selected").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 3, "You need to improve the sparring ring more first!");
        addButton(2, "Lvl60", dummyTraining, 2, lustImmune, isGroup).disableIf(lvl == 2, "Already selected").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 4, "You need to improve the sparring ring more first!");
        addButton(3, "Lvl90", dummyTraining, 3, lustImmune, isGroup).disableIf(lvl == 3, "Already selected").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 5, "You need to improve the sparring ring more first!");
        addButton(4, "Lvl150", dummyTraining, 4, lustImmune, isGroup).disableIf(lvl == 4, "Already selected").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 5, "You need to improve the sparring ring more first!");
        addButton(5, "Normal", dummyTraining, lvl, true, isGroup).disableIf(lustImmune, "Already selected");
        addButton(6, "Sim. Lust", dummyTraining, lvl, false, isGroup).disableIf(!lustImmune, "Already selected").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 3, "You need to improve the sparring ring more first!");
        addButton(7, "Solo", dummyTraining, lvl, lustImmune, false).disableIf(!isGroup, "Already selected");
        addButton(8, "Group", dummyTraining, lvl, lustImmune, true).disableIf(isGroup, "Already selected").disableIf(flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 5, "You need to improve the sparring ring more first!");
        addButton(13, "Fight", dummyTrainingStart, lvl, lustImmune, isGroup);
        addButton(14, "Back", campActions);
    }
    
    private function dummyTrainingStart(lvl : Int = 0, lustImmune : Bool = true, isGroup : Bool = false) : Void
    {
        clearOutput();
    ...
    private function SparrableNPCsMenuText() : Void
    {
        ...
        if (player.hasPerk(PerkLib.BasicLeadership))
        {
            ...
            {
                outputText(", " + flags[kFLAGS.PLAYER_COMPANION_1]);
            }
            else
            {
                outputText(", (no combat companion)");
            }
        }
        if (player.hasPerk(PerkLib.IntermediateLeadership))
        {
            ...
            {
                outputText(", " + flags[kFLAGS.PLAYER_COMPANION_2]);
            }
            else
            {
                outputText(", (no combat companion)");
        ...
        if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] >= 2)
        {
            ...
            {
                outputText("Training Mode\n");
            }
            if (flags[kFLAGS.SPARRABLE_NPCS_TRAINING] < 2)
            {
                outputText("Relax Mode\n");
            }
        }
        if (player.hasStatusEffect(StatusEffects.ChiChiOff))
        {
            outputText("\nChi Chi: [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.DivaOff))
        {
            outputText("\nDiva: [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.ElectraOff))
        {
            outputText("\nElectra: [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.EtnaOff))
        {
            outputText("\nEtna: [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.LunaOff))
        {
            outputText("\nLuna: [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.NadiaOff))
        {
            outputText("\nNadia: [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.TedOff))
        {
            outputText("\nDragon Boi: [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.SophieOff))
        {
            outputText("\nSophie: [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.HeliaOff))
        {
            outputText("\nHelia: [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.KonstantinOff))
        {
            outputText("\nKonstantin: [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.SpoodersOff))
        {
            outputText("\nSpooders: [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.ShouldraOff))
        {
            outputText("\nShouldra: [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.CeraphOff))
        {
            outputText("\nCeraph: [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.CalluOff))
        {
            outputText("\nCallu (Otter girl): [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.TamaniOff))
        {
            outputText("\nTamani (Goblin): [font-dred]<b>Disabled</b>[/font]");
        }
        if (player.hasStatusEffect(StatusEffects.VenusOff))
        {
    ...
    private function SparrableNPCsMenu() : Void
    {
        ...
        if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] >= 2)
        {
            if (flags[kFLAGS.SPARRABLE_NPCS_TRAINING] < 2)
            {
                addButton(10, "Train", NPCsTrain);
            }
            if (flags[kFLAGS.SPARRABLE_NPCS_TRAINING] == 2)
            {
                addButton(10, "Relax", NPCsRelax);
            }
        }
        //since this section is WIP anyway, let her be here too, lol
        if (flags[kFLAGS.GOTTA_CAMP_THEM_ALL_MODE] < 2)
        {
            addButton(11, "Activate", GottaCampThemALLOn).hint("Turn on 'Gotta Camp them ALL' Mode.");
        }
        if (flags[kFLAGS.GOTTA_CAMP_THEM_ALL_MODE] == 2)
        {
            addButton(11, "Deactivate", GottaCampThemALLOff).hint("Turn off 'Gotta Camp them ALL' Mode.");
        }
        addButton(12, "Members", SparrableNPCsMenuCampNPCs).descendants("hint")("Enable or Disable Camp Followers from appearing.");
    ...
    {
        if (flags[kFLAGS.SHOULDRA_SLEEP_TIMER] != 0)
        {
            flags[kFLAGS.SHOULDRA_SLEEP_TIMER] = 0;
        }  //reset neglation tracker  
        toggleNPCStatus(StatusEffects.ShouldraOff);
    }
    
    private function swimInStream() : Void
    {
        var prankChooser : Float = rand(3);
        clearOutput();
        outputText("You ponder over the nearby stream that's flowing. Deciding you'd like a dip, ");
        if (player.armorName == "slutty swimwear")
        {
            outputText("you are going to swim while wearing just your swimwear. ");
        }
        else
        {
            outputText("you strip off your [armor] until you are completely naked. ");
        }
        outputText("You step into the flowing waters. You shiver at first but you step in deeper. Incredibly, it's not too deep. ");
        if (player.basetallness < 60)
        {
            outputText("Your feet aren't even touching the riverbed. ");
        }
        if (player.basetallness >= 60 && player.basetallness < 72)
        {
            outputText("Your feet are touching the riverbed and your head is barely above the water. ");
        }
        if (player.basetallness >= 72)
        {
            outputText("Your feet are touching touching the riverbed and your head is above water. You bend down a bit so you're at the right height. ");
        }
        ...
        //Helia!
        if (rand(2) == 0 && camp.followerHel() && !player.hasStatusEffect(StatusEffects.HeliaOff) && flags[kFLAGS.HEL_CAN_SWIM] != null)
        {
        ...
        if (rand(2) == 0 && camp.amilyFollower() && flags[kFLAGS.AMILY_FOLLOWER] == 1 && flags[kFLAGS.AMILY_OWNS_BIKINI] > 0)
        {
            ...
            {
                outputText(", especially when her penis is exposed");
            }
        ...
        //Pranks!
        if (prankChooser == 0 && (camp.izmaFollower() || (camp.followerHel() && !player.hasStatusEffect(StatusEffects.HeliaOff) && flags[kFLAGS.HEL_CAN_SWIM] != null) || camp.marbleFollower() || (camp.amilyFollower() && flags[kFLAGS.AMILY_FOLLOWER] == 1 && flags[kFLAGS.AMILY_OWNS_BIKINI] > 0)))
        {
    ...
    {
        var pranked : Bool = false;
        var prankRoll : Float = 1;
        //How many people joined!
        if (izmaJoinsStream)
        {
            prankRoll++;
        }
        if (marbleJoinsStream)
        {
            prankRoll++;
        }
        if (heliaJoinsStream)
        ...
        if (rand(prankRoll) == 0 && (camp.followerHel() && !player.hasStatusEffect(StatusEffects.HeliaOff) && flags[kFLAGS.HEL_CAN_SWIM] != null) && !pranked && heliaJoinsStream)
        {
        ...
        if (!pranked)
        {
            outputText("  No one managed to swim past where you left the warm spot before it dissipated. You feel a bit disappointed and just go back to swimming.");
        }
        else
        {
            outputText("  You feel accomplished from the prank and resume swimming. ");
        }
    ...
    private function swimInStreamFinish() : Void
    {
        ...
        if (flags[kFLAGS.FACTORY_SHUTDOWN] == 2)
        {
            ...
            if (player.cor < 25)
            {
                dynStats("lust", 30, "scale", true);
            }
            if (player.cor >= 25 && player.cor < 50)
            {
                dynStats("lust", 20, "scale", true);
            }
            if (player.cor >= 50)
            {
                dynStats("lust", 10, "scale", true);
            }
        }
        ...
        else
        {
            ...
            if (player.cor >= 66)
            {
                dynStats("lust", -5, "scale", false);
            }
        }
        outputText("Eventually, you swim back to the riverbank and dry yourself off");
        if (player.armorName != "slutty swimwear")
        {
            outputText(" before you re-dress yourself in your " + player.armorName);
        }
        outputText(".");
    ...
    public function restMenu() : Void
    {
        ...
        {
            addButton(8, "Till Dawn", restTillDawn).hint("Rest until the dawn comes.");
        }
        else
        {
            addButton(8, "Till Dusk", restTillDusk).hint("Rest until the night comes.");
        }
        addButton(14, "Back", playerMenu);
    }
    ...
    public function acceleratingRecoveryFactor(hours : Float, isSleeping : Bool) : Float
    {
        var perHour : Float = 1;
        if (isSleeping)
        {
            if (player.hasPerk(PerkLib.RecuperationSleep))
            {
                perHour += 1;
            }
            if (player.hasPerk(PerkLib.RejuvenationSleep))
            {
                perHour += 2;
            }
        }
        var bonus : Float = perHour * hours;
    ...
    public function rest() : Void
    {
        ...
        var fatRecovery : Float = 4;
        var hpRecovery : Float = 10;
        var hpTime : Float = acceleratingRecoveryFactor(waitingORresting, false);
        if (player.level >= 24)
        {
            fatRecovery += 2;
            hpRecovery += 5;
        ...
        if (player.hasPerk(PerkLib.Medicine))
        {
            hpRecovery *= 1.5;
        }
        if (player.hasPerk(PerkLib.SpeedyRecovery))
        {
            fatRecovery += 2;
        }
        if (player.hasPerk(PerkLib.SpeedyRecuperation))
        {
            fatRecovery += 4;
        }
        if (player.hasPerk(PerkLib.SpeedyRejuvenation))
        {
            fatRecovery += 8;
        }
        ...
        if (player.hasStatusEffect(StatusEffects.BathedInHotSpring))
        {
            fatRecovery *= 1.2;
        }
        if (flags[kFLAGS.AYANE_FOLLOWER] >= 2)
        ...
        if (timeQ == 0)
        {
            ...
            if (flags[kFLAGS.CAMP_BUILT_CABIN] > 0 && flags[kFLAGS.CAMP_CABIN_FURNITURE_BED] > 0 && !ingnam.inIngnam)
            {
                if (timeQ != 1)
                {
                    outputText("You head into your cabin to rest. You lie down on your bed to rest for " + num2Text(timeQ) + " hours.\n");
                }
                else
                {
                    outputText("You head into your cabin to rest. You lie down on your bed to rest for an hour.\n");
                }
            }
            else if (player.lowerBody == LowerBody.PLANT_FLOWER)
            {
                if (timeQ != 1)
                {
                    outputText("You lie down in your pitcher, closing off your petals as you get comfortable for " + num2Text(timeQ) + " hours.\n");
                }
                else
                {
                    outputText("You lie down in your pitcher, closing off your petals as you get comfortable for an hour.\n");
                }
            }
            else if (player.lowerBody == LowerBody.FLOWER_LILIRAUNE)
            {
                if (timeQ != 1)
                {
                    outputText("You and your twin sister lie down in your pitcher, closing off your petals as you get comfortable for " + num2Text(timeQ) + " hours.\n");
                }
                else
                {
                    outputText("You and your twin sister lie down in your pitcher, closing off your petals as you get comfortable for an hour.\n");
                }
            }
            else if (player.isGargoyle())
            {
                if (timeQ != 1)
                {
                    outputText("You sit on your pedestal, your body petrifying like stone as you briefly slumber for " + num2Text(timeQ) + " hours.\n");
                }
                else
                {
                    outputText("You sit on your pedestal, your body petrifying like stone as you briefly slumber for an hour.\n");
                }
            }
            else if (timeQ != 1)
            {
            ...
            {
                outputText("You lie down to rest for an hour.\n");
            }
            //Ayane
            if (flags[kFLAGS.AYANE_FOLLOWER] >= 2)
            {
            ...
            if (SceneLib.alvinaFollower.AlvinaPurified)
            {
                ...
                else
                {
                    ...
                    if (player.cor >= mincor + 10)
                    {
                        dynStats("cor", -10);
                    }
                    else
                    {
                        dynStats("cor", -(player.cor - mincor));
                    }
                }
            }
        ...
        else
        {
            ...
            if (timeQ != 1)
            {
                outputText("You continue to rest for " + num2Text(timeQ) + " more hours.\n");
            }
            else
            {
                outputText("You continue to rest for another hour.\n");
            }
        }
        goNext(true);
    }
    ...
    public function doWaitMenu() : Void
    {
        ...
        if (player.isNightCreature())
        {
            addButton(8, "Till Dawn", doWaitTillDawn).hint("Wait until the dawn comes.");
        }
        else
        {
            addButton(8, "Till Dusk", doWaitTillDusk).hint("Wait until the night comes.");
        }
        addButton(14, "Back", playerMenu);
    }
    ...
    public function doWait() : Void
    {
        ...
        if (timeQ == 0)
        {
            ...
            {
                outputText("You lie down in your pitcher, closing off your petals as you get comfortable for " + num2Text(timeQ) + " hour" + ((timeQ % 10 == 1) ? "" : "s") + "...\n");
            }
            else
            {
                outputText("You wait " + num2Text(timeQ) + " hour" + ((timeQ % 10 == 1) ? "" : "s") + "...\n");
            }
            //Marble withdrawl
            if (player.hasStatusEffect(StatusEffects.MarbleWithdrawl))
            {
        ...
        {
            outputText("You continue to wait for " + num2Text(timeQ) + " more hours.\n");
        }
        else
        {
            outputText("You continue to wait for another hour.\n");
        }
        goNext(true);
    }
    ...
    //-- SLEEP
    //-----------------
    public function sleepInCabin() : Bool
    {
        return flags[kFLAGS.CAMP_BUILT_CABIN] > 0 && flags[kFLAGS.CAMP_CABIN_FURNITURE_BED] > 0 && Lambda.indexOf(["", "Marble", "Zenji", "Excellia", "Luna", "Samirah", "Sophie", "Hel"], flags[kFLAGS.SLEEP_WITH]) != -1;
    }
    ...
    public function doSleep(clrScreen : Bool = true) : Void
    {
        ...
        if (CoC.instance.timeQ == 0)
        {
            ...
            else if (flags[kFLAGS.SLEEP_WITH] == "Zenji" && ZenjiScenes.isLover())
            {
                ...
                if (flags[kFLAGS.CAMP_CABIN_FURNITURE_BED] > 0)
                {
                    ...
                    if (player.hasVagina())
                    {
                        addButton(1, "Get Penetrated", curry(SceneLib.zenjiScene.loverZenjiSleepWithGetPenetrated, timeQ));
                    }
                    else
                    {
                        addButtonDisabled(1, "Get Penetrated", "You need a vagina for this scene.");
                    }
                ...
                else
                {
                    ...
                    {
                        outputText(" As you lie beside him, you feel his tail coil around you, you reflexively bring your [tail] to tangle with his, the two of you locking tails each other.");
                    }
                    else
                    {
                        outputText(" As you lie beside him, you can feel his tail coil around you, protectively stroking your [legs].");
                    }
                    if (player.tailType == Tail.FOX)
                    {
                        outputText(" You hate to admit it, but Zenji’s tail just might almost kind of rival yours in terms of how soft and cuddly it is.");
                    }
            ...
            else if (flags[kFLAGS.SLEEP_WITH] == "Luna" && flags[kFLAGS.LUNA_FOLLOWER] >= 4)
            {
                ...
                if (flags[kFLAGS.LUNA_JEALOUSY] < -timeQ)
                {
                    flags[kFLAGS.LUNA_JEALOUSY] = -timeQ;
                }
                ...
                {
                    outputText("Luna hugs you tightly, almost possessively so as you both doze off to sleep.");
                }
            }
            else if (flags[kFLAGS.SLEEP_WITH] == "Samirah" && flags[kFLAGS.SAMIRAH_FOLLOWER] > 9)
            {
                ...
                if (player.isNaga())
                {
                    outputText(" She’s not alone either. It indeed took you a while to realise that you are also cold-blooded now. The cold night air sure puts you in a similar state as of late.");
                }
            ...
            else
            {
                ...
                else if (player.isRaceCached(Races.IMP, 3))
                {
                    ...
                    {
                        outputText("Nalcanthet quarters include a luxurious bed for your convenience and you are keen on using it.");
                    }
                    else
                    {
                        outputText("You order your subjects to prepare a throne for you by stacking their tiny bodies on top of one another for you to rest on. They are quick to comply and soon the imp throne is readied. Satisfied with the result you sit down on the impromptu throne.[pg]");
                    }
                    if (player.perkv1(PerkLib.ImpNobility) >= 5)
                    {
                        if (player.hasVagina())
                        {
                            outputText(" You tap the imp in charge of the back area then whisper to his ears to harden up and fuck your pussy gently. If for any reason including him thrusting too fast or roughly you have to wake up early, he will suffer punishment. Without need to be told twice the lesser imp's cock hardens on the spot and slides seamlessly into your royal snatch, his corrupt pre tingling your passage most comfortably.[pg]");
                        }
                        outputText((player.hasCock()) ? ((player.hasVagina()) ? "With your wet passage taken good care of you" : "You") : "");
                        if (player.hasCock())
                        {
                            outputText(" then call out to the imp underneath you ordering him to offer his ass to sleeve your hardening cock and move on his own so that you can relax. Soon his cheeks wraps around your [cock], massaging you at a slow and steady rhythm.[pg]");
                        }
                    }
        ...
        else
        {
            ...
            if (timeQ != 1)
            {
                outputText("You lie down to resume sleeping for the remaining " + num2Text(timeQ) + " hours.\n");
            }
            else
            {
                outputText("You lie down to resume sleeping for the remaining hour.\n");
            }
        }
    ...
    public function sleepWrapper(multiplier : Float = 1.0) : Void
    {
    ...
    public function cheatSleepUntilMorning(multiplier : Float = 1.0) : Void
    {
        var timeToSleep : Int = as3hx.Compat.parseInt(((model.time.hours < 6) ? 6 : 24 + 6) - model.time.hours);
    ...
    public function sleepRecovery(display : Bool = false, multiplier : Float = 1.0) : Void
    {
        var fatRecovery : Float = 20;
        var hpRecovery : Float = 20;
        if (player.level >= 24)
        {
            fatRecovery += 10;
            hpRecovery += 10;
    ...
    private function callRathazulAndEscapeBadEnd() : Void
    {
        ...
        player.ballSize -= (4 + rand(6));
        if (player.ballSize > 18 + (player.str / 2) + (player.basetallness / 4))
        {
            player.ballSize = 16 + (player.str / 2) + (player.basetallness / 4);
        }
        if (player.ballSize < 1)
        {
            player.ballSize = 1;
    ...
    private function dungeonFound() : Bool
    //Returns true as soon as any known dungeon is found
    {
        
        return flags[kFLAGS.FACTORY_FOUND] > 0 || flags[kFLAGS.DISCOVERED_DUNGEON_2_ZETAZ] > 0 || flags[kFLAGS.DEMON_LABORATORY_DISCOVERED] > 0 || flags[kFLAGS.D3_DISCOVERED] > 0 || flags[kFLAGS.DISCOVERED_WITCH_DUNGEON] > 0 || SceneLib.dungeons.checkPhoenixTowerClear() || flags[kFLAGS.EBON_LABYRINTH] > 0 || flags[kFLAGS.HIDDEN_CAVE_FOUND] > 0 || flags[kFLAGS.DEN_OF_DESIRE_BOSSES] > 0 || flags[kFLAGS.DISCOVERED_BEE_HIVE_DUNGEON] > 0 || flags[kFLAGS.DISCOVERED_TWILIGHT_GROVE_DUNGEON] > 0 || flags[kFLAGS.LUMI_MET] > 0 || flags[kFLAGS.ANZU_PALACE_UNLOCKED] > 0;
    }
    ...
    //Returns true as soon as any known dungeon is found
    {
        ...
        {
            if (flags[kFLAGS.FARM_DISABLED] == 0)
            {
                return true;
            }
            if (player.cor >= 70 && player.level >= 12 && SceneLib.farm.farmCorruption.corruptFollowers() >= 2 && flags[kFLAGS.FARM_CORRUPTION_DISABLED] == 0)
            {
                return true;
            }
        }
    ...
    //-- PLACES MENU
    //-----------------
    private function placesKnown() : Bool
    ...
    public function placesCount() : Int
    {
        ...
        if (flags[kFLAGS.OWCA_UNLOCKED] > 0)
        {
            places++;
        }
        ...
        if (flags[kFLAGS.HEXINDAO_UNLOCKED] > 0)
        {
            places++;
        }
        if (flags[kFLAGS.DILAPIDATED_SHRINE_UNLOCKED] > 1)
        {
            places++;
        }
        if (flags[kFLAGS.FOUND_TEMPLE_OF_THE_DIVINE] > 0)
        {
            places++;
        }
        ...
        if (flags[kFLAGS.THE_TRENCH_ENTERED] > 0)
        {
            places++;
        }
    ...
    public function places() : Void
    {
        ...
        var bd : ButtonDataList = new ButtonDataList();
        // Row 1 - towns 1-5
        bd.add("He'Xin'Dao", SceneLib.hexindao.riverislandVillageStuff0).hint("Visit the village of He'Xin'Dao, a place where all greenhorn soul cultivators come together.").disableIf(flags[kFLAGS.HEXINDAO_UNLOCKED] < 1, "Explore the realm.", null, "???");
        bd.add("Tel'Adre", SceneLib.telAdre.visitTelAdre).hint("Visit the city of Tel'Adre in desert, easily recognized by the massive tower.").disableIf(player.statusEffectv1(StatusEffects.TelAdre) < 1, "Explore the realm.", null, "???");
        bd.add("Bazaar", SceneLib.bazaar.enterTheBazaar).hint("Visit the Bizarre Bazaar where the demons and corrupted beings hang out.").disableIf(flags[kFLAGS.BAZAAR_ENTERED] <= 0, "Explore the realm.", null, "???");
        bd.add("Owca", SceneLib.owca.gangbangVillageStuff).hint("Visit the sheep village of Owca, known for its pit where a person is hung on the pole weekly to be gang-raped by the demons.").disableIf(flags[kFLAGS.OWCA_UNLOCKED] != 1, "Search the plains (at late afternoon to ealry evening hours).", null, "???");
        bd.add("Troll Village", SceneLib.trollVillage.EnterTheVillage).hint("Visit the Troll Village.").disableIf(TrollVillage.ZenjiVillageStage <= 0, "You have not visited this place yet.", null, "???");
        // Row 2 - towns/places/NPCs 1-5
        bd.add("The Trench", SceneLib.theTrench.theTrenchEntrance).hint("Visit the Trench where the Arigeans hang out.").disableIf(flags[kFLAGS.THE_TRENCH_ENTERED] <= 0, "Only for Arigeans.", null, "???");
        bd.add("Lumi's Lab", SceneLib.lumi.lumiEncounter).hint("Visit Lumi's laboratory.").disableIf(flags[kFLAGS.LUMI_MET] <= 0, "Explore the realm.", null, "???");
        bd.add("Town Ruins", SceneLib.amilyScene.exploreVillageRuin).hint("Visit the village ruins. \n\nRecommended level: 17").disableIf(!flags[kFLAGS.AMILY_VILLAGE_ACCESSIBLE], "Search the lake.", null, "???");
        bd.add("Farm", SceneLib.farm.farmExploreEncounter).hint("Visit Whitney's farm.").disableIf(!farmFound(), "Search the lake.", null, "???");
        bd.add("Marae", maraeIsland).hint("Visit Marae's Island in middle of the Lake.").disableIf(flags[kFLAGS.MARAE_ISLAND] <= 0, "Search the lake on the boat.", null, "???")  // Row 3 - places/NPCs 6-10  ;
        
        bd.add("Salon", SceneLib.mountain.salon.salonGreeting).hint("Visit the salon for hair services.").disableIf(!player.hasStatusEffect(StatusEffects.HairdresserMeeting), "Search the mountains.", null, "???");
        bd.add("Eldritch Caves", SceneLib.mindbreaker.CaveLayout).hint("Visit the mindbreaker lair.").disableIf(Mindbreaker.MindBreakerQuest != Mindbreaker.QUEST_STAGE_ISMB, "Gotta be outta ya MIND to go there.", null, "???");
        bd.add("Temple", SceneLib.templeofdivine.repeatvisitintro).hint("Visit the temple in the high mountains where Sapphire resides.").disableIf(!flags[kFLAGS.FOUND_TEMPLE_OF_THE_DIVINE], "Search the high mountains.", null, "???");
        bd.add("Chicken Harpy", SceneLib.mountain.chickenHarpy).hint("Visit Chicken Harpy in the Low Mountains.").disableIf(!player.hasItem(consumables.OVIELIX), "You need to have at least 1-2 ovi elixirs to have reason to look for her.").disableIf(flags[kFLAGS.TIMES_MET_CHICKEN_HARPY] <= 1, "Search the low mountains with ovi elixir.", null, "???");
        bd.add("Oasis Tower", SceneLib.mountain.minervaScene.encounterMinerva).hint("Visit the ruined tower in the mountains where Minerva resides.").disableIf(flags[kFLAGS.MET_MINERVA] < 4, "Search the mountains.", null, "???");
        // Row 4 - places/NPCs 11-15
        bd.add("Elven grove", SceneLib.woodElves.GroveLayout).hint("Visit the elven grove where the wood elves spend their somewhat idylic lives.").disableIf(WoodElves.WoodElvesQuest < 5, "Search the forest.", null, "???");
        bd.add("Shrine", SceneLib.kitsuneScene.kitsuneShrine).hint("Visit the kitsune shrine in the deepwoods.").disableIf(!flags[kFLAGS.KITSUNE_SHRINE_UNLOCKED], "Search the deepwoods.", null, "???");
        bd.add("Great Tree", SceneLib.aikoScene.encounterAiko).hint("Visit the Great Tree in the Deep Woods where Aiko lives.").disableIf(flags[kFLAGS.AIKO_TIMES_MET] <= 3, "Great Tree in big big wood ^^", null, "???");
        bd.add("Dilapid.Shrine", SceneLib.dilapidatedShrine.repeatvisitshrineintro).hint("Visit the dilapidated shrine where the echoses of the golden age of gods still lingers.").disableIf(flags[kFLAGS.DILAPIDATED_SHRINE_UNLOCKED] <= 1, "Search the battlefield. (After hearing an npc mention this place)", null, "???");
        bd.add("Winter Gear", SceneLib.glacialYuShop.YuIntro).hint("Visit the Winter gear shop.").disableIf(flags[kFLAGS.YU_SHOP] != 2, "Search the (outer) glacial rift.", null, "???");
        // Row 5 - places/NPCs 16-20
        bd.add("Anzu's Palace", SceneLib.dungeons.anzupalace.enterDungeon).hint("Visit the palace in the Glacial Rift where Anzu the avian deity resides.").disableIf(flags[kFLAGS.ANZU_PALACE_UNLOCKED] <= 0, "Definitely not Elza winter palace ;)", null, "???");
        bd.add("Cathedral", SceneLib.gargoyle.gargoyleRouter).hint((flags[kFLAGS.GAR_NAME] == 0) ? "Visit the ruined cathedral you've recently discovered." : "Visit the ruined cathedral where " + flags[kFLAGS.GAR_NAME] + " resides.").disableIf(!flags[kFLAGS.FOUND_CATHEDRAL], "Explore the realm.", null, "???");  //player.hasStatusEffect(StatusEffects.MeetXuviel)  
        bd.add("Demon lair", SceneLib.demonicLair.demonLairMainMenu).hint("Visit the Xuviel’s lair.").disableIf(player.statusEffectv1(StatusEffects.MeetXuviel) < 4, "Only for Xuviel’s 9th Wife.", null, "???");
        bd.add("Woodcutting", camp.cabinProgress.gatherWoods).hint("You need to explore Forest more to unlock this place.").disableIf(player.statusEffectv1(StatusEffects.ResourceNode1) < 5, "You need to explore Forest more to unlock this place.", null, "???").disableIf(!player.hasStatusEffect(StatusEffects.ResourceNode1), "Search the forest.", null, "???");
        bd.add("Quarry", camp.cabinProgress.quarrySite).hint("You can mine here to get stones, gems and maybe even some ores. <b>(Daylight)</b>").disableIf(player.statusEffectv2(StatusEffects.ResourceNode1) < 5, "You need to explore Mountains more to unlock this place.", null, "???").disableIf(!player.hasStatusEffect(StatusEffects.ResourceNode1), "Search the mountains.", null, "???");
        // Row 6 - separator between places and dungeons
        if (debug)
        {
            bd.add("Ingnam", SceneLib.ingnam.returnToIngnam).hint("Return to Ingnam for debugging purposes. Night-time event weirdness might occur. You have been warned!");
        }
        else
        {
            bd.add("");
        ...
        bd.add("");
        // Row 7 - dungeons 1-5
        bd.add("Factory", SceneLib.dungeons.factory.enterDungeon).hint("Visit the demonic factory in the mountains."
                        ...
                        + ((SceneLib.dungeons.checkFactoryClear()) ? "\n\nCLEARED!" : "")).disableIf(flags[kFLAGS.FACTORY_FOUND] <= 0, "???", null, "???");
        bd.add("Deep Cave", SceneLib.dungeons.deepcave.enterDungeon).hint("Visit the cave you've found in the Deepwoods."
                        + ((flags[kFLAGS.DEFEATED_ZETAZ] > 0) ? "\n\nYou've defeated Zetaz, your old rival." : "")
                        + ((SceneLib.dungeons.checkDeepCaveClear()) ? "\n\nCLEARED!" : "")).disableIf(flags[kFLAGS.DISCOVERED_DUNGEON_2_ZETAZ] <= 0, "???", null, "???");
        bd.add("Demon Laboratory", SceneLib.dungeons.demonLab.EnteringDungeon).hint("Visit the demon laboratory in the mountains."
                        + ((SceneLib.dungeons.checkDemonLaboratoryClear()) ? "\n\nYou have destroyed that laboratory and put an end to the demonic experiments. You've one step closer to defeating Lethice!\n\nCLEARED!" : "")).disableIf(flags[kFLAGS.DEMON_LABORATORY_DISCOVERED] <= 0, "???", null, "???");
        bd.add("Stronghold", SceneLib.d3.enterD3).hint("Visit the stronghold in the high mountains that belongs to Lethice, the demon queen."
                        ...
                        + ((SceneLib.dungeons.checkLethiceStrongholdClear()) ? "\n\nCLEARED!" : "")).disableIf(flags[kFLAGS.D3_DISCOVERED] <= 0, "???", null, "???");
        bd.add("Desert Cave", SceneLib.dungeons.desertcave.enterDungeon).hint("Visit the cave you've found in the desert."
                                ...
                                + ((SceneLib.dungeons.checkSandCaveClear()) ? "\n\nCLEARED!" : "")).disableIf(flags[kFLAGS.DESERT_CAVE_DISABLED], "You can't find the entrance. Maybe it's hidden. Or locked forever. Who knows?").disableIf(flags[kFLAGS.DISCOVERED_WITCH_DUNGEON] <= 0, "???", null, "???");
        bd.add("Phoenix Tower", SceneLib.dungeons.heltower.returnToHeliaDungeon).hint("Re-visit the tower you went there as part of Helia's quest."
                        + ((SceneLib.dungeons.checkPhoenixTowerClear()) ? "\n\nYou've helped Helia in the quest and resolved the problems. \n\nCLEARED!" : "")).disableIf(!SceneLib.dungeons.checkPhoenixTowerClear(), "???", null, "???");
        // Row 8 - dungeons 6-10
        bd.add("Hidden Cave", SceneLib.dungeons.hiddencave.enterDungeon).hint("Visit the hidden cave in the hills."
                        + ((SceneLib.dungeons.checkHiddenCaveClear()) ? "\n\nCLEARED!" : "")).disableIf(flags[kFLAGS.HIDDEN_CAVE_FOUND] <= 0, "???", null, "???");
        bd.add("Bee Hive", SceneLib.dungeons.beehive.enterDungeon).hint("Visit the bee hive you've found in the forest."
                        + ((flags[kFLAGS.TIFA_FOLLOWER] > 5) ? "\n\nYou've defeated all corrupted bees." : "")
                        + ((SceneLib.dungeons.checkBeeHiveClear()) ? "\n\nCLEARED!" : "")).disableIf(flags[kFLAGS.DISCOVERED_BEE_HIVE_DUNGEON] <= 0, "???", null, "???");
        bd.add("Twilight Grove", SceneLib.dungeons.twilightgrove.enterDungeon).hint("Visit the twilight grove you've found in the deepwoods."
                        + ((flags[kFLAGS.TWILIGHT_GROVE_PURIFICATION] > 3) ? "\n\nYou've defeated all corrupted alraunes." : "")
                        + ((SceneLib.dungeons.checkTwilightGroveClear()) ? "\n\nCLEARED!" : "")).disableIf(flags[kFLAGS.DISCOVERED_TWILIGHT_GROVE_DUNGEON] <= 0, "???", null, "???");
        bd.add("EbonLabyrinth", SceneLib.dungeons.ebonlabyrinth.enterDungeon).hint("Visit Ebon Labyrinth."
                        + ((SceneLib.dungeons.checkEbonLabyrinthClear()) ? "\n\nSEMI-CLEARED!" : "")).disableIf(flags[kFLAGS.EBON_LABYRINTH] <= 0, "???", null, "???");
        bd.add("Den of Desire", SceneLib.dungeons.denofdesire.enterDungeon).hint("Visit the den in blight ridge."
                        + ((SceneLib.dungeons.checkDenOfDesireClear()) ? "\n\nCLEARED!" : "")).disableIf(flags[kFLAGS.DEN_OF_DESIRE_BOSSES] <= 0, "???", null, "???");
        bd.add("");
        // Row 9 - spare
        // Row 10 - spare
        // Row 11 - spare
    ...
    public function oldPlacesMenu() : Bool
    {
        hideMenus();
        clearOutput();
        outputText("Which place would you like to visit?");
        ...
        if (dungeonFound())
        {
            addButton(0, "Dungeons", dungeons).hint("Delve into dungeons.");
        }
        else
        {
            addButtonDisabled(0, "???", "Find at least one dungeon.");
        }
        //1 - ???
        if (flags[kFLAGS.MARAE_ISLAND] > 0)
        {
            addButton(2, "Marae", maraeIsland).hint("Visit Marae's Island in middle of the Lake.");
        }
        else
        {
            addButtonDisabled(2, "???", "Search the lake on the boat.");
        }
        addButton(4, "Next", placesPage2);
        
        if (player.statusEffectv1(StatusEffects.TelAdre) >= 1)
        {
            addButton(5, "Tel'Adre", SceneLib.telAdre.visitTelAdre).hint("Visit the city of Tel'Adre in desert, easily recognized by the massive tower.");
        }
        else
        {
            addButtonDisabled(5, "???", "Search the desert.");
        }
        if (flags[kFLAGS.BAZAAR_ENTERED] > 0)
        {
            addButton(6, "Bazaar", SceneLib.bazaar.enterTheBazaar).hint("Visit the Bizarre Bazaar where the demons and corrupted beings hang out.");
        }
        else
        {
            addButtonDisabled(6, "???", "Search the plains.");
        }
        if (flags[kFLAGS.OWCA_UNLOCKED] == 1)
        {
            addButton(7, "Owca", SceneLib.owca.gangbangVillageStuff).hint("Visit the sheep village of Owca, known for its pit where a person is hung on the pole weekly to be gang-raped by the demons.");
        }
        else
        {
            addButtonDisabled(7, "???", "Search the plains.");
        }
        if (TrollVillage.ZenjiVillageStage > 0)
        {
            addButton(8, "Troll Village", SceneLib.trollVillage.EnterTheVillage).hint("Visit the Troll Village.");
        }
        else
        {
            addButtonDisabled(8, "???", "Clear the factory first.");
        }
        if (flags[kFLAGS.THE_TRENCH_ENTERED] > 0)
        {
            addButton(9, "The Trench", SceneLib.theTrench.theTrenchEntrance).hint("Visit the Trench where the Arigeans hang out.");
        }
        else
        {
            addButtonDisabled(9, "???", "Only for Arigeans.");
        }
        
        if (flags[kFLAGS.HEXINDAO_UNLOCKED] >= 1)
        {
            addButton(10, "He'Xin'Dao", SceneLib.hexindao.riverislandVillageStuff0).hint("Visit the village of He'Xin'Dao, a place where all greenhorn soul cultivators come together.");
        }
        else
        {
            addButtonDisabled(10, "???", "Explore the realm.");
        }
        if (flags[kFLAGS.TIMES_MET_CHICKEN_HARPY] > 1)
        {
            if (player.hasItem(consumables.OVIELIX))
            {
                addButton(11, "Chicken Harpy", SceneLib.mountain.chickenHarpy).hint("Visit Chicken Harpy in the High Mountains.");
            }
            else
            {
                addButtonDisabled(11, "Chicken Harpy", "You need to have at least 1-2 ovi elixirs to have reason to look for her.");
            }
        }
        else
        {
            addButtonDisabled(11, "???", "Search the high mountains with ovi elixir.");
        }
        if (flags[kFLAGS.DILAPIDATED_SHRINE_UNLOCKED] > 1)
        {
            addButton(12, "Dilapidated Shrine", SceneLib.dilapidatedShrine.repeatvisitshrineintro).hint("Visit the dilapidated shrine where the echoses of the golden age of gods still lingers.");
        }
        else
        {
            addButtonDisabled(12, "???", "Search the battlefield. (After hearing an npc mention this place)");
        }
        if (WoodElves.WoodElvesQuest >= 5)
        {
            addButton(13, "Elven grove", SceneLib.woodElves.GroveLayout).hint("Visit the elven grove where the wood elves spend their somewhat idylic lives.");
        }
        else
        {
            addButtonDisabled(13, "???", "Search the forest.");
        }
        addButton(14, "Back", playerMenu);
        return true;
    }
    ...
    private function placesPage2() : Void
    {
        ...
        if (flags[kFLAGS.FOUND_CATHEDRAL] > 0)
        {
            ...
            {
                addButton(0, "Cathedral", SceneLib.gargoyle.gargoylesTheShowNowOnWBNetwork).hint("Visit the ruined cathedral you've recently discovered.");
            }
            else
            {
                addButton(0, "Cathedral", SceneLib.gargoyle.returnToCathedral).hint("Visit the ruined cathedral where " + flags[kFLAGS.GAR_NAME] + " resides.");
            }
        }
        else
        {
            addButtonDisabled(0, "???", "Explore the realm.");
        }
        if (farmFound())
        {
            addButton(1, "Farm", SceneLib.farm.farmExploreEncounter).hint("Visit Whitney's farm.");
        }
        else
        {
            addButtonDisabled(1, "???", "Search the lake.");
        }
        if (flags[kFLAGS.AMILY_VILLAGE_ACCESSIBLE] > 0)
        {
            addButton(2, "Town Ruins", SceneLib.amilyScene.exploreVillageRuin).hint("Visit the village ruins. \n\nRecommended level: 12");
        }
        else
        {
            addButtonDisabled(2, "???", "Search the lake.");
        }
        if (player.hasStatusEffect(StatusEffects.HairdresserMeeting))
        {
            addButton(3, "Salon", SceneLib.mountain.salon.salonGreeting).hint("Visit the salon for hair services.");
        }
        else
        {
            addButtonDisabled(3, "???", "Search the mountains.");
        }
        addButton(4, "Next", placesPage3);
        
        if (flags[kFLAGS.KITSUNE_SHRINE_UNLOCKED] > 0)
        {
            addButton(5, "Shrine", SceneLib.kitsuneScene.kitsuneShrine).hint("Visit the kitsune shrine in the deepwoods.");
        }
        else
        {
            addButtonDisabled(5, "???", "Search the deepwoods.");
        }
        if (flags[kFLAGS.MET_MINERVA] >= 4)
        {
            addButton(6, "Oasis Tower", SceneLib.mountain.minervaScene.encounterMinerva).hint("Visit the ruined tower in the mountains where Minerva resides.");
        }
        else
        {
            addButtonDisabled(6, "???", "Search the mountains.");
        }
        if (flags[kFLAGS.FOUND_TEMPLE_OF_THE_DIVINE] > 0)
        {
            addButton(7, "Temple", SceneLib.templeofdivine.repeatvisitintro).hint("Visit the temple in the high mountains where Sapphire resides.");
        }
        else
        {
            addButtonDisabled(7, "???", "Search the high mountains.");
        }
        if (flags[kFLAGS.YU_SHOP] == 2)
        {
            addButton(8, "Winter Gear", SceneLib.glacialYuShop.YuIntro).hint("Visit the Winter gear shop.");
        }
        else
        {
            addButtonDisabled(8, "???", "Search the (outer) glacial rift.");
        }
        addButton(9, "Previous", placesToPage1);
        
        if (flags[kFLAGS.AIKO_TIMES_MET] > 3)
        {
            addButton(10, "Great Tree", SceneLib.aikoScene.encounterAiko).hint("Visit the Great Tree in the Deep Woods where Aiko lives.");
        }
        else
        {
            addButtonDisabled(10, "???", "???");
        }
        if (Mindbreaker.MindBreakerQuest == Mindbreaker.QUEST_STAGE_ISMB)
        {
            addButton(11, "Eldritch Caves", SceneLib.mindbreaker.CaveLayout).hint("Visit the mindbreaker lair.");
        }
        if (debug)
        {
            addButton(13, "Ingnam", SceneLib.ingnam.returnToIngnam).hint("Return to Ingnam for debugging purposes. Night-time event weirdness might occur. You have been warned!");
        }
        addButton(14, "Back", playerMenu);
    }
    ...
    private function placesPage3() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.ResourceNode1))
        {
            if (player.statusEffectv1(StatusEffects.ResourceNode1) < 5)
            {
                addButtonDisabled(0, "???", "You need to explore Forest more to unlock this place.");
            }
            else
            {
                addButton(0, "Woodcutting", camp.cabinProgress.gatherWoods).hint("You can cut some trees here to get wood.");
            }
            if (player.statusEffectv2(StatusEffects.ResourceNode1) < 5)
            {
                addButtonDisabled(1, "???", "You need to explore Mountains more to unlock this place.");
            }
            else
            {
                addButton(1, "Quarry", camp.cabinProgress.quarrySite).hint("You can mine here to get stones, gems and maybe even some ores.");
            }
        }
        else
        {
        ...
        if (player.hasStatusEffect(StatusEffects.MeetXuviel) && player.statusEffectv1(StatusEffects.MeetXuviel) >= 4)
        {
            addButton(2, "Demon lair", SceneLib.demonicLair.demonLairMainMenu).hint("Visit the Xuviel’s lair.");
        }
        else
        {
            addButtonDisabled(2, "???", "Only for Xuviel’s 9th Wife.");
        }
    ...
    private function dungeons() : Void
    {
        menu();
        //Main story dungeons
        if (flags[kFLAGS.FACTORY_FOUND] > 0)
        {
            addButton(0, "Factory", SceneLib.dungeons.factory.enterDungeon).hint("Visit the demonic factory in the mountains." + ((flags[kFLAGS.FACTORY_SHUTDOWN] > 0) ? "\n\nYou've managed to shut down the factory." : "The factory is still running. Marae wants you to shut down the factory!") + ((SceneLib.dungeons.checkFactoryClear()) ? "\n\nCLEARED!" : ""));
        }
        else
        {
            addButtonDisabled(0, "???", "???");
        }
        if (flags[kFLAGS.DISCOVERED_DUNGEON_2_ZETAZ] > 0)
        {
            addButton(1, "Deep Cave", SceneLib.dungeons.deepcave.enterDungeon).hint("Visit the cave you've found in the Deepwoods." + ((flags[kFLAGS.DEFEATED_ZETAZ] > 0) ? "\n\nYou've defeated Zetaz, your old rival." : "") + ((SceneLib.dungeons.checkDeepCaveClear()) ? "\n\nCLEARED!" : ""));
        }
        else
        {
            addButtonDisabled(1, "???", "???");
        }
        if (flags[kFLAGS.DEMON_LABORATORY_DISCOVERED] > 0)
        {
            addButton(2, "Demon Laboratory", SceneLib.dungeons.demonLab.EnteringDungeon).hint("Visit the demon laboratory in the mountains." + ((SceneLib.dungeons.checkDemonLaboratoryClear()) ? "\n\nYou have destroyed that laboratory and put an end to the demonic experiments. You've one step closer to defeating Lethice!\n\nCLEARED!" : ""));
        }
        else
        {
            addButtonDisabled(2, "???", "???");
        ...
        {
            addButton(3, "Stronghold", SceneLib.d3.enterD3).hint("Visit the stronghold in the high mountains that belongs to Lethice, the demon queen." + (((flags[kFLAGS.LETHICE_DEFEATED] > 0)) ? "\n\nYou have slain Lethice and put an end to the demonic threats. Congratulations, you've beaten the main story!" : "") + ((SceneLib.dungeons.checkLethiceStrongholdClear()) ? "\n\nCLEARED!" : ""));
        }
        else
        {
            addButtonDisabled(3, "???", "???");
        ...
        //Side dungeons
        if (flags[kFLAGS.DISCOVERED_WITCH_DUNGEON] > 0 && flags[kFLAGS.DESERT_CAVE_DISABLED] == null)
        {
            addButton(5, "Desert Cave", SceneLib.dungeons.desertcave.enterDungeon).hint("Visit the cave you've found in the desert." + ((flags[kFLAGS.SAND_WITCHES_COWED] + flags[kFLAGS.SAND_WITCHES_FRIENDLY] > 0) ? "\n\nFrom what you've known, this is the source of the Sand Witches." : "") + ((SceneLib.dungeons.checkSandCaveClear()) ? "\n\nCLEARED!" : ""));
        }
        else if (flags[kFLAGS.DESERT_CAVE_DISABLED] != null)
        {
            addButtonDisabled(5, "Desert Cave", "You can't find the entrance. Maybe it's hidden. Or locked forever. Who knows?");
        }
        else
        {
            addButtonDisabled(5, "???", "???");
        }
        if (flags[kFLAGS.DISCOVERED_BEE_HIVE_DUNGEON] > 0)
        {
            addButton(6, "Bee Hive", SceneLib.dungeons.beehive.enterDungeon).hint("Visit the bee hive you've found in the forest." + ((flags[kFLAGS.TIFA_FOLLOWER] > 5) ? "\n\nYou've defeated all corrupted bees." : "") + ((SceneLib.dungeons.checkBeeHiveClear()) ? "\n\nCLEARED!" : ""));
        }
        else
        {
            addButtonDisabled(6, "???", "???");
        ...
        {
            addButton(7, "Phoenix Tower", SceneLib.dungeons.heltower.returnToHeliaDungeon).hint("Re-visit the tower you went there as part of Helia's quest." + ((SceneLib.dungeons.checkPhoenixTowerClear()) ? "\n\nYou've helped Helia in the quest and resolved the problems. \n\nCLEARED!" : ""));
        }
        else
        {
            addButtonDisabled(7, "???", "???");
        }
        if (flags[kFLAGS.DISCOVERED_TWILIGHT_GROVE_DUNGEON] > 0)
        {
            addButton(8, "Twilight Grove", SceneLib.dungeons.twilightgrove.enterDungeon).hint("Visit the twilight grove you've found in the deepwoods." + ((flags[kFLAGS.TWILIGHT_GROVE_PURIFICATION] > 1) ? "\n\nYou've defeated all corrupted alraunes." : "") + ((SceneLib.dungeons.checkTwilightGroveClear()) ? "\n\nCLEARED!" : ""));
        }
        else
        {
            addButtonDisabled(8, "???", "???");
        }
        if (flags[kFLAGS.EBON_LABYRINTH] > 0)
        {
            addButton(9, "EbonLabyrinth", SceneLib.dungeons.ebonlabyrinth.enterDungeon).hint("Visit Ebon Labyrinth." + ((SceneLib.dungeons.checkEbonLabyrinthClear()) ? "\n\nSEMI-CLEARED!" : ""));
        }
        else
        {
            addButtonDisabled(9, "???", "???");
        }
        if (flags[kFLAGS.HIDDEN_CAVE_FOUND] > 0)
        {
            addButton(10, "Hidden Cave", SceneLib.dungeons.hiddencave.enterDungeon).hint("Visit the hidden cave in the hills." + ((SceneLib.dungeons.checkHiddenCaveClear()) ? "\n\nCLEARED!" : ""));
        }
        else
        {
            addButtonDisabled(10, "???", "???");
        }
        if (flags[kFLAGS.DEN_OF_DESIRE_BOSSES] > 0)
        {
            addButton(11, "Den of Desire", SceneLib.dungeons.denofdesire.enterDungeon).hint("Visit the den in blight ridge." + ((SceneLib.dungeons.checkDenOfDesireClear()) ? "\n\nCLEARED!" : ""));
        }
        else
        {
            addButtonDisabled(11, "???", "???");
        }
        if (flags[kFLAGS.LUMI_MET] > 0)
        {
            addButton(12, "Lumi's Lab", SceneLib.lumi.lumiEncounter).hint("Visit Lumi's laboratory.");
        }
        else
        {
            addButtonDisabled(12, "???", "???");
        }
        if (flags[kFLAGS.ANZU_PALACE_UNLOCKED] > 0)
        {
            addButton(13, "Anzu's Palace", SceneLib.dungeons.anzupalace.enterDungeon).hint("Visit the palace in the Glacial Rift where Anzu the avian deity resides.");
        }
        else
        {
            addButtonDisabled(13, "???", "???");
        }
    ...
    public function wakeFromBadEnd() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_BAD_ENDED] >= 2)
        ...
        //Skip time forward
        var timeskip : Float = 24;
        if (flags[kFLAGS.BENOIT_CLOCK_BOUGHT] > 0)
        {
            timeskip += flags[kFLAGS.BENOIT_CLOCK_ALARM];
        }
        else
        {
            timeskip += 6;
        ...
        if (player.hasStatusEffect(StatusEffects.RivereDungeonIB))
        {
            player.removeStatusEffect(StatusEffects.RivereDungeonIB);
        }
        if (player.hasStatusEffect(StatusEffects.ThereCouldBeOnlyOne))
        {
            player.removeStatusEffect(StatusEffects.ThereCouldBeOnlyOne);
        }
        if (player.hasStatusEffect(StatusEffects.LoliBatGolems))
        {
        ...
        if (flags[kFLAGS.HARDCORE_MODE] == 1)
        {
            ...
            {
                player.strStat.core.value = Math.round(player.strStat.core.value * 0.8);
            }
            else
            {
                player.strStat.core.value -= 10;
            }
            if (player.strStat.core.value < 1)
            {
                player.strStat.core.value = 1;
            }
            if (player.touStat.core.value >= 50)
            {
                player.touStat.core.value = Math.round(player.touStat.core.value * 0.8);
            }
            else
            {
                player.touStat.core.value -= 10;
            }
            if (player.touStat.core.value < 1)
            {
                player.touStat.core.value = 1;
            }
            if (player.speStat.core.value >= 50)
            {
                player.speStat.core.value = Math.round(player.speStat.core.value * 0.8);
            }
            else
            {
                player.speStat.core.value -= 10;
            }
            if (player.speStat.core.value < 1)
            {
                player.speStat.core.value = 1;
            }
            if (player.intStat.core.value >= 50)
            {
                player.intStat.core.value = Math.round(player.intStat.core.value * 0.8);
            }
            else
            {
                player.intStat.core.value -= 10;
            }
            if (player.intStat.core.value < 1)
            {
                player.intStat.core.value = 1;
            }
            if (player.wisStat.core.value >= 50)
            {
                player.wisStat.core.value = Math.round(player.wisStat.core.value * 0.8);
            }
            else
            {
                player.wisStat.core.value -= 10;
            }
            if (player.wisStat.core.value < 1)
            {
                player.wisStat.core.value = 1;
            }
            if (player.libStat.core.value >= 50)
            {
                player.libStat.core.value = Math.round(player.libStat.core.value * 0.8);
            }
            else
            {
                player.libStat.core.value -= 10;
            }
            if (player.libStat.core.value < 1)
            {
        ...
        penaltyMultiplier += as3hx.Compat.parseInt(flags[kFLAGS.PRIMARY_DIFFICULTY] * 0.5);
        //Deduct XP and gems.
        if (flags[kFLAGS.HARDCORE_MODE] == 1)
        {
            player.XP = 0;
        ...
        {
            player.XP -= as3hx.Compat.parseInt((player.level * 10) * penaltyMultiplier);
            if (player.XP < 0)
            {
                player.XP = 0;
            }
        }
        player.gems -= as3hx.Compat.parseInt((player.gems / 10) * penaltyMultiplier);
        if (player.gems < 0)
        {
            player.gems = 0;
        }
        menu();
        addButton(0, "Next", doCamp);
    }
    ...
    public function rebirthFromBadEnd() : Void
    {
        ...
        if (silly())
        {
            outputText(" With another chance, you know your journey is far from over. You still have so much to do.");
        }
        if (flags[kFLAGS.HUNGER_ENABLED] > 0)
        {
            player.hunger = 40;
        ...
        if (flags[kFLAGS.BENOIT_CLOCK_BOUGHT] > 0)
        {
            model.time.hours = flags[kFLAGS.BENOIT_CLOCK_ALARM];
        }
        else
        {
            model.time.hours = 6;
        ...
        statScreenRefresh();
        player.addStatusValue(StatusEffects.PCClone, 3, -1);
        if (player.statusEffectv3(StatusEffects.PCClone) <= 0)
        {
        ...
        else if (player.hasStatusEffect(StatusEffects.PCClone2nd))
        {
    ...
    private function buildCampWall() : Void
    {
        ...
        fatigueAmount -= as3hx.Compat.parseInt(player.str / 5);
        fatigueAmount -= as3hx.Compat.parseInt(player.tou / 10);
        fatigueAmount -= as3hx.Compat.parseInt(player.spe / 10);
        if (player.hasPerk(PerkLib.IronMan))
        {
            fatigueAmount -= 20;
        }
        ...
        fatigueAmount /= as3hx.Compat.parseInt(helpers + 1);
        if (fatigueAmount < 15)
        {
            fatigueAmount = 15;
        }
    ...
    private function promptAscend() : Void
    {
        ...
        {
            outputText("<b>New Game+</b>");
        }
        else if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] == 1)
        {
            outputText("<b>New Game++</b>");
        }
        else if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] == 2)
        {
            outputText("<b>New Game+++</b>");
        }
        else
        {
            outputText("<b>New Game+" + (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] + 1) + "</b>");
        }
    ...
    public function possibleToGainAscensionPoints() : Float
    {
        var performancePointsPrediction : Float = 0;
        //Companions
        performancePointsPrediction += companionsCount();
        if (flags[kFLAGS.PC_GOBLIN_DAUGHTERS] > 0)
        {
            performancePointsPrediction--;
        }
        if (flags[kFLAGS.ALVINA_FOLLOWER] == 12)
        {
            performancePointsPrediction++;
        ...
        if (flags[kFLAGS.PATCHOULI_FOLLOWER] == 3)
        {
            performancePointsPrediction++;
        }
        if (player.hasStatusEffect(StatusEffects.DivaOff))
        ...
        if (player.level >= 42)
        {
            performancePointsPrediction += (player.level - 41);
        }
        if (player.level >= 102)
        {
            performancePointsPrediction += (player.level - 101);
        }
        if (player.level >= 180)
        {
            performancePointsPrediction += (player.level - 179);
        }
        ...
        {
            performancePointsPrediction += player.teaseLevel;
        }
        performancePointsPrediction = Math.round(performancePointsPrediction);
        return performancePointsPrediction;
    }
    public function possibleToGainAscensionPointsDungeons() : Float
    {
        var performancePointsPredictionDungeons : Float = 0;
        if (SceneLib.dungeons.checkFactoryClear())
        {
            performancePointsPredictionDungeons++;
        }
        if (SceneLib.dungeons.checkDeepCaveClear())
        {
            performancePointsPredictionDungeons += 2;
        }
        if (SceneLib.dungeons.checkDemonLaboratoryClear())
        {
            performancePointsPredictionDungeons += 3;
        }
        if (SceneLib.dungeons.checkLethiceStrongholdClear())
        {
            performancePointsPredictionDungeons += 4;
        }
    ...
    public function possibleToGainAscensionPointsQuests() : Float
    {
        var performancePointsPredictionQuests : Float = 0;
        if (flags[kFLAGS.MARBLE_PURIFIED] > 0)
        {
            performancePointsPredictionQuests += 2;
        }
        ...
        {
            performancePointsPredictionQuests += 1;
        }
        if (flags[kFLAGS.CORRUPTED_MARAE_KILLED] > 0 || flags[kFLAGS.PURE_MARAE_ENDGAME] >= 2)
        {
            performancePointsPredictionQuests += 3;
        }
        if (player.statusEffectv1(StatusEffects.AdventureGuildQuests1) >= 4)
        {
            performancePointsPredictionQuests += 2;
        }
        if (player.statusEffectv2(StatusEffects.AdventureGuildQuests1) >= 4)
        {
            performancePointsPredictionQuests += 2;
        }
        if (player.statusEffectv3(StatusEffects.AdventureGuildQuests1) >= 4)
        {
            performancePointsPredictionQuests += 2;
        }
        if (player.statusEffectv1(StatusEffects.AdventureGuildQuests2) >= 4)
        {
            performancePointsPredictionQuests += 2;
        }
        if (player.statusEffectv2(StatusEffects.AdventureGuildQuests2) >= 4)
        {
            performancePointsPredictionQuests += 2;
        }
        if (player.statusEffectv1(StatusEffects.AdventureGuildQuests4) >= 2)
        {
            performancePointsPredictionQuests++;
        }
        if (player.statusEffectv2(StatusEffects.AdventureGuildQuests4) >= 2)
        {
            performancePointsPredictionQuests++;
        }
        if (flags[kFLAGS.GALIA_LVL_UP] >= 0.5)
        {
            performancePointsPredictionQuests += 5;
        }
    ...
    public function possibleToGainAscensionPointsCampStructures() : Float
    {
        var performancePointsPredictionCampStructures : Float = 0;
        if (flags[kFLAGS.CAMP_BUILT_CABIN] > 0)
        {
            performancePointsPredictionCampStructures += 10;
        }
        if (flags[kFLAGS.CAMP_WALL_GATE] > 0)
        {
            performancePointsPredictionCampStructures += 11;
        }
        if (flags[kFLAGS.MATERIALS_STORAGE_UPGRADES] > 2)
        {
            performancePointsPredictionCampStructures += 2;
        }
        if (flags[kFLAGS.MATERIALS_STORAGE_UPGRADES] > 3)
        {
            performancePointsPredictionCampStructures += 2;
        ...
        if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] > 1)
        {
            performancePointsPredictionCampStructures += ((flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] - 1) * 2);
        }  //obecnie +4*2  
        if (flags[kFLAGS.CAMP_UPGRADES_ARCANE_CIRCLE] > 0)
        {
            performancePointsPredictionCampStructures += flags[kFLAGS.CAMP_UPGRADES_ARCANE_CIRCLE];
        }  //obecnie +8  
        if (flags[kFLAGS.CAMP_UPGRADES_MAGIC_WARD] > 1)
        {
            performancePointsPredictionCampStructures += 2;
        }
        if (flags[kFLAGS.CAMP_UPGRADES_DAM] > 0)
        {
            performancePointsPredictionCampStructures += (flags[kFLAGS.CAMP_UPGRADES_DAM] * 2);
        }  //obecnie +3*2  
        if (flags[kFLAGS.CAMP_UPGRADES_FISHERY] > 0)
        {
            performancePointsPredictionCampStructures += (flags[kFLAGS.CAMP_UPGRADES_FISHERY] * 2);
        }  //obecnie +2*2  
    ...
    public function possibleToGainAscensionPointsChildren() : Float
    {
        ...
        childPerformance += as3hx.Compat.parseInt((flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS] / 4) + (flags[kFLAGS.LYNNETTE_BABY_COUNT] / 4) + (flags[kFLAGS.ANT_KIDS] / 100) + (flags[kFLAGS.PHYLLA_DRIDER_BABIES_COUNT] / 4) + (flags[kFLAGS.PC_GOBLIN_DAUGHTERS] / 4) + (flags[kFLAGS.MITZI_DAUGHTERS] / 4) + (etnaScene().etnaTotalKids()));
        childPerformance += as3hx.Compat.parseInt((DriderTown.BelisaKids / 4) + (DriderTown.LilyKidsPC / 4) + ((DriderTown.TyrantiaFemaleKids + DriderTown.TyrantiaMaleKids) / 4) + SceneLib.ayaneFollower.ayaneChildren() + flags[kFLAGS.LOPPE_KIDS]);
        performancePointsPredictionChildren += Math.sqrt(childPerformance);
        return performancePointsPredictionChildren;
    }
    
    public function possibleToGainAscensionPointsDaoOfElements() : Float
    {
        var pTGAPDoE : Float = 0;
        if (player.hasStatusEffect(StatusEffects.DaoOfFire))
        {
            pTGAPDoE += player.statusEffectv2(StatusEffects.DaoOfFire);
        }
        if (player.hasStatusEffect(StatusEffects.DaoOfIce))
        {
            pTGAPDoE += player.statusEffectv2(StatusEffects.DaoOfIce);
        }
        if (player.hasStatusEffect(StatusEffects.DaoOfLightning))
        {
            pTGAPDoE += player.statusEffectv2(StatusEffects.DaoOfLightning);
        }
        if (player.hasStatusEffect(StatusEffects.DaoOfDarkness))
        {
            pTGAPDoE += player.statusEffectv2(StatusEffects.DaoOfDarkness);
        }
        if (player.hasStatusEffect(StatusEffects.DaoOfPoison))
        {
            pTGAPDoE += player.statusEffectv2(StatusEffects.DaoOfPoison);
        }
        if (player.hasStatusEffect(StatusEffects.DaoOfWind))
        {
            pTGAPDoE += player.statusEffectv2(StatusEffects.DaoOfWind);
        }
        if (player.hasStatusEffect(StatusEffects.DaoOfBlood))
        {
            pTGAPDoE += player.statusEffectv2(StatusEffects.DaoOfBlood);
        }
        if (player.hasStatusEffect(StatusEffects.DaoOfWater))
        {
            pTGAPDoE += player.statusEffectv2(StatusEffects.DaoOfWater);
        }
        if (player.hasStatusEffect(StatusEffects.DaoOfEarth))
        {
            pTGAPDoE += player.statusEffectv2(StatusEffects.DaoOfEarth);
        }
        if (player.hasStatusEffect(StatusEffects.DaoOfAcid))
        {
            pTGAPDoE += player.statusEffectv2(StatusEffects.DaoOfAcid);
        }
        return pTGAPDoE;
    }
    public function possibleToGainAscensionPointsMasteries() : Float
    {
        var pTGAPM : Float = 0;
    ...
    public function setLevelButton(allowAutoLevelTransition : Bool) : Bool
    {
        var levelup : Bool = player.XP >= player.requiredXP() && (player.level < CoC.instance.levelCap || player.negativeLevel > 0);
    ...
    // This is all just display for the button text and affects nothing directly
    {
        ...
        {
            statpoints += as3hx.Compat.parseInt(player.perkv1(PerkLib.AscensionAdvTrainingX) * 4);
        }
        if (player.level < 1)
        {
            statpoints *= 3;
    ...
    public function getCampPopulation() : Int
    {
        ...
        if (flags[kFLAGS.IN_INGNAM] == null)
        {
            pop++;
        }  //You count toward the population!  
        pop += companionsCount();
        if (flags[kFLAGS.PC_GOBLIN_DAUGHTERS] > 0)
        {
            pop--;
        ...
        {
            pop--;
        }  //Ceraph doesn't stay in your camp.  
        if (player.armor == armors.GOOARMR)
        {
            pop++;
        }  //Include Valeria if you're wearing her.  
        if (player.weapon == weapons.AETHERD)
        {
            pop++;
        }  //Include Aether D twin if you're wearing her.  
        if (player.shield == shields.AETHERS)
        {
            pop++;
        }  //Include Aether S twin if you're wearing her.  
        if (flags[kFLAGS.CLARA_IMPRISONED] > 0)
        {
            pop++;
        }
        ...
        {
            pop += LunaFollower.WerewolfPackMember;
        }
        ...
        {
            pop += LunaFollower.HellhoundPackMember;
        }
        if (player.hasPerk(PerkLib.MummyLord))
        {
            pop += player.perkv1(PerkLib.MummyLord);
        }
        //------------
        //Children check!
        //Followers
        if (followerEmber() && emberScene.emberChildren() > 0)
        {
            pop += emberScene.emberChildren();
        }
        if (flags[kFLAGS.MITZI_DAUGHTERS] > 0)
        {
            pop += flags[kFLAGS.MITZI_DAUGHTERS];
        }
        if (SceneLib.ayaneFollower.ayaneChildren() > 0)
        {
            pop += SceneLib.ayaneFollower.ayaneChildren();
        }
        ...
        if (sophieFollower())
        {
            ...
            if (flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] != null)
            {
                pop += flags[kFLAGS.SOPHIE_ADULT_KID_COUNT];
            }
        }
        //Lovers
        ...
        if (isabellaFollower() && isabellaScene.totalIsabellaChildren() > 0)
        {
            pop += isabellaScene.totalIsabellaChildren();
        }
        if (izmaFollower() && izmaScene.totalIzmaChildren() > 0)
        {
            pop += izmaScene.totalIzmaChildren();
        }
        if (followerKiha() && kihaFollower.totalKihaChildren() > 0)
        {
            pop += kihaFollower.totalKihaChildren();
        }
        if (marbleFollower() && flags[kFLAGS.MARBLE_KIDS] > 0)
        {
            pop += flags[kFLAGS.MARBLE_KIDS];
        }
        if (flags[kFLAGS.ANT_WAIFU] > 0 && (flags[kFLAGS.ANT_KIDS] > 0 || flags[kFLAGS.PHYLLA_DRIDER_BABIES_COUNT] > 0))
        {
            pop += as3hx.Compat.parseInt(flags[kFLAGS.ANT_KIDS] + flags[kFLAGS.PHYLLA_DRIDER_BABIES_COUNT]);
        }
        if (flags[kFLAGS.ZENJI_KIDS] > 0)
        {
            pop += flags[kFLAGS.ZENJI_KIDS];
        }
        if (DriderTown.BelisaKids > 0)
        {
            pop += DriderTown.BelisaKids;
        }
        if (DriderTown.LilyKidsPC > 0)
        {
            pop += DriderTown.LilyKidsPC;
        }
        if (DriderTown.TyrantiaFemaleKids > 0)
        {
            pop += DriderTown.TyrantiaFemaleKids;
        }
        if (DriderTown.TyrantiaMaleKids > 0)
        {
            pop += DriderTown.TyrantiaMaleKids;
        }
        if (flags[kFLAGS.PC_GOBLIN_DAUGHTERS] > 0)
        {
            pop += flags[kFLAGS.PC_GOBLIN_DAUGHTERS];
        }
        //From npcs that can be in more than one tab
        if (SceneLib.excelliaFollower.totalExcelliaChildren() > 0)
        {
            pop += SceneLib.excelliaFollower.totalExcelliaChildren();
        }
        //------------
        //Return number!
        return pop;
    ...
    public function getCampUndergroundPopulation() : Int
    {
        ...
        {
            undpop += as3hx.Compat.parseInt(flags[kFLAGS.ANT_KIDS] + flags[kFLAGS.PHYLLA_DRIDER_BABIES_COUNT]);
        }
        return undpop;
    }
}
