...
class UniqueSexScenes extends BaseContent
{
    public var sceneMenu(get, never) : ButtonDataList;
    ...
    public function AlrauneDungeonBadEnd() : Void
    {
        ...
        if (player.isLiliraune())
        {
            outputText(" When you’re not busy raping intruders or using the dungeons many denizens as your seedbed or cum dumps you spend the days of your quasi-infinite lifespan having sex with your twin in the middle of your pitcher, ever ready to invite whoever enters the room next for a threesome.");
        }
        ...
        if (player.isLiliraune())
        {
            outputText(" you and your twin");
        }
        outputText(" licking your lips in delight. It has been many years since one made it this far.\n\n");
        if (player.isLiliraune())
        {
            outputText("\"<i>Oh my! Isn’t she cute? Came all the way down here just to play with us</i>\"\n\n" +
    ...
    private function get_sceneMenu() : ButtonDataList
    {
        var menuItems : Array<Dynamic> = [];
        var sceneList : Array<Dynamic> = [USSTailpeg(), 
        USSTailRape(), 
        USSSnRape(), 
        USSVoltTsf(), 
        USSHeatTsf(), 
        ...
        //menuItems.push();
        sceneList = cast((sceneList), ScenePostProc);
        if (flags[kFLAGS.USSDISPLAY_STYLE] == 0)
        
        //Available options first, then the unavailable ones.{
            
            var temphold : Array<Dynamic> = [];
            for (USSScenesRez in sceneList)
            {
                if (Std.is(Reflect.field(USSScenesRez, Std.string(1)), Function))
                {
                    menuItems.push(USSScenesRez);
                }
                else
                {
                    temphold.push(USSScenesRez);
                }
            }
            for (postproc in temphold)
            {
        ...
        for (i in menuItems)
        {
            if (Std.is(Reflect.field(i, Std.string(1)), Function))
            {
                bd.add(Reflect.field(i, Std.string(0)), Reflect.field(i, Std.string(1)), Reflect.field(i, Std.string(2)));
                activeBtns++;
            }
            else
            {
                bd.add(Reflect.field(i, Std.string(0))).disable(Reflect.field(i, Std.string(2)));
            }
        }
        return bd;
    }
    ...
    public function pcUSSPreChecksV2(backFunc : Function, btnPos : Int = 13) : Void
    {
        ...
        if (returnscene == camp.returnToCampUseOneHour)
        {
            trace("Missing return scene for USSMenu. Will continue, but will go back to camp after.");
        }
        if (RaijuOverLust())
        {
        ...
        {
            addButton(btnPos, "U.Sex Scenes", openUSSmenu).disableIf(player.hasPerk(PerkLib.ElementalBody), "You can't use unique sex scenes while being an elemental.");
        }
    }
    ...
    public function ScenePostProc(sList : Array<Dynamic>) : Array<Dynamic>
    {
        var temp : Array<Dynamic> = [];
        var temp2 : Array<Dynamic> = [];
        for (x in sList)
        {
            if (x.length == 3)
            {
            ...
            else
            {
                for (i in 0...x.length + 1)
                {
                    ...
                    temp2.push(Reflect.field(x, Std.string(i)));
                }
            }
        }
        return temp;
    ...
    //checking functions ===========================================================================
    private function RaijuOverLust() : Bool
    {
    ...
    private function USSTailRape() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["Tail Rape"];
        if (player.tailType == Tail.MANTICORE_PUSSYTAIL && monster.hasCock())
        {
            btnSet.push(manticoreTailRapeScene);
            btnSet.push("");
        ...
        {
            btnSet.push(false);
            btnSet.push("Req. to have Pussytail and enemy with cock.");
            
        }
        return btnSet;
    }
    private function USSTailpeg() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["TailPeg"];
        if (player.tailType == Tail.HINEZUMI || player.tailType == Tail.SALAMANDER)
        {
            if (Std.is(monster, Anemone) || Std.is(monster, SeaAnemone) || Std.is(monster, HellHound) || Std.is(monster, InfestedHellhound))
            {
                btnSet.push(false);
                btnSet.push("Req. enemy to have anus.");
                
            }
            else
            {
                btnSet.push(hinezumiTailpegScene);
                btnSet.push("");
        ...
        {
            btnSet.push(false);
            btnSet.push("Req. to have Hinezumi Tail.");
            
        }
        return btnSet;
    }
    private function USSSnRape() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["Strangle and Rape"];
        if (player.gender != 0 && (player.tailType == Tail.HINEZUMI || player.tailType == Tail.MOUSE || player.tailType == Tail.DEMONIC))
        {
            btnSet.push(strangleAndRapeScene);
            btnSet.push("");
        ...
        {
            btnSet.push(false);
            btnSet.push("Req. to be non-genderless with hinezumi, mouse or demon tail.");
            
        }
        return btnSet;
    }
    private function USSVoltTsf() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["Volt Transfer"];
        if ((player.isRace(Races.RAIJU, 1, false) || player.isRace(Races.THUNDERBIRD, 1, false) || player.isRace(Races.KIRIN, 1, false)) && !monster.hasPerk(PerkLib.EnemyHugeType) && !monster.hasPerk(PerkLib.EnemyGigantType) && !monster.hasPerk(PerkLib.EnemyColossalType) && !monster.isAlraune() && !monster.isDrider() && !monster.isGoo() && !monster.isNaga() && !monster.isScylla() && !monster.isTaur())
        {
            btnSet.push(raijuVoltTransfer);
            btnSet.push("");
        ...
        {
            btnSet.push(false);
            btnSet.push("Req. species with electrified desire and enemy must be non-gigant humanoid.");
            
        }
        return btnSet;
    }
    //OverHeat Species (?)
    private function USSHeatTsf() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["Heat Transfer"];
        if (player.hasStatusEffect(StatusEffects.Overheat) && !monster.hasPerk(PerkLib.EnemyHugeType) && !monster.hasPerk(PerkLib.EnemyGigantType) && !monster.hasPerk(PerkLib.EnemyColossalType))
        {
            btnSet.push(hinezumiHeatTransferScene);
            btnSet.push("");
        ...
        {
            btnSet.push(false);
            btnSet.push("Req. to have Hinezumi Coat, over 50% of max lust, enemy must be humanoid and not giant.");
            
        }
        return btnSet;
    }
    private function USSCooldown() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["Cooldown"];
        if (player.hasStatusEffect(StatusEffects.Overheat) && monster.hasPerk(PerkLib.IceNature) && player.hasVagina() && monster.hasCock())
        {
            btnSet.push(Cooldown);
            btnSet.push("");
        ...
        {
            btnSet.push(false);
            btnSet.push("Req. a cold type creatures with a cock, the overheat perk and a vagina.");
            
        }
        return btnSet;
    }
    //YukiOnna
    private function USSStlWmth() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["Steal Warmth"];
        if (player.isRace(Races.YUKIONNA, 1, false) && monster.hasCock() && !monster.hasPerk(PerkLib.UniqueNPC) && !monster.hasPerk(PerkLib.EnemyHugeType) && !monster.hasPerk(PerkLib.EnemyGigantType) && !monster.hasPerk(PerkLib.EnemyColossalType) && !monster.isAlraune() && !monster.isDrider() && !monster.isGoo() && !monster.isNaga() && !monster.isScylla() && !monster.isTaur())
        {
            btnSet.push(yukionnaStealWarmthScene);
            btnSet.push("");
        ...
        {
            btnSet.push(false);
            btnSet.push("Req. to be Yuki Onna and enemy must be: non-gigant, humanoid, with cock, non-unique npc.");
            
        }
        return btnSet;
    }
    private function USSGobMech() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = [(player.hasKeyItem("Cum Reservoir") >= 0 && player.keyItemvX("Cum Reservoir", 1)) ? "Cum Reservoir" : "Fill the reservoir"];
        if (player.isInGoblinMech() && player.hasKeyItem("Cum Reservoir") >= 0 && monster.hasCock())
        {
            if (player.keyItemvX("Cum Reservoir", 1) == 4)
            {
                btnSet.push(false);
                btnSet.push("Reservoir is already full. You need to use all this stored cum before you harvest new one.");
                
            }
            else
            {
                btnSet.push(gobomechFillTheReservoir);
                btnSet.push("");
        ...
        {
            btnSet.push(false);
            btnSet.push("Req. to be in goblin mech, having Cum Reservoir upgrade instaled on it and enemy with cock.");
            
        }
        return btnSet;
    }
    private function USSBrainMlt() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["Brain Melt"];
        if (Mindbreaker.MindBreakerQuest == Mindbreaker.QUEST_STAGE_ISMB)
        {
            btnSet.push(brainMelt);
            btnSet.push("");
        ...
        {
            btnSet.push(false);
            btnSet.push("Req. to be a mindbreaker.");
            
        }
        return btnSet;
    }
    private function USSAlrauneSS() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = [];
        if (player.isAlraune())
        {
            if (player.isLiliraune())
            {
                ...
                else
                {
                    btnSet.push("Tag Team");
                    btnSet.push(false);
                    btnSet.push("You need to be two person against a male to use this scene.");
                    
                }
                if (monster.hasVagina())
                {
                    btnSet.push("Triple Girl Fun");
                    btnSet.push(TreeWayRapeLiliraune);
                    btnSet.push("");
                ...
                else
                {
                    btnSet.push("Triple Girl Fun");
                    btnSet.push(false);
                    btnSet.push("You need to be two person against a female to use this scene.");
                    
                }
            }
            else
            {
                if (player.lowerBody == LowerBody.PLANT_FLOWER && monster.hasCock())
                {
                    btnSet.push("Get Pollinated");
                    btnSet.push(alrauneGetPollinatedScene);
                    btnSet.push("");
                    
                }
                //Isn't Alraune supposed to have LowerBody.PLANT_FLOWER to even pass isAlraune?
                else
                {
                    btnSet.push("Get Pollinated");
                    btnSet.push(false);
                    btnSet.push("Req. to have Alraune lower body and enemy with cock.");
                    
                }
                if (player.lowerBody == LowerBody.PLANT_FLOWER && monster.hasVagina())
                {
                    btnSet.push("Seeding");
                    btnSet.push(alrauneSeedingScene);
                    btnSet.push("");
                ...
                else
                {
                    btnSet.push("Seeding");
                    btnSet.push(false);
                    btnSet.push("Req. to have Alraune lower body and enemy with vagina.");
                    
                }
            }
            if (player.isAlraune() && player.cor >= 50 && monster.hasVagina() && !monster.hasPerk(PerkLib.UniqueNPC))
            {
                btnSet.push("Convert");
                btnSet.push(Convert);
                btnSet.push("");
            ...
            else
            {
                btnSet.push("Convert");
                btnSet.push(false);
                btnSet.push("You need to be a very corrupted alraune against a non unique female character to use this scene.");
        ...
        else
        {
            btnSet.push("Alraune");
            btnSet.push(false);
            btnSet.push("You need to be an Alraune.");
            
        }
        return btnSet;
    }
    //wood elf
    private function USSWoodElf() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["Convert Elf"];
        if (player.isWoodElf() && monster.hasVagina() && !monster.hasPerk(PerkLib.UniqueNPC))
        {
            btnSet.push(SceneLib.woodElves.CaptureANewSister);
            btnSet.push("");
            
        }
        else if (player.isWoodElf() && monster.hasCock() && monster.cocks[0].cockType == CockTypesEnum.HORSE && !monster.hasPerk(PerkLib.UniqueNPC))
        {
            btnSet.push(SceneLib.woodElves.CaptureMinotaur);
            btnSet.push("");
    ...
    //easter bunneh
    private function USSEastrBny() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["EGGS!!"];
        if (player.hasPerk(PerkLib.EasterBunnyBalls) && player.hasCock() && player.ballSize > 3)
        {
            btnSet.push(EasterBunnyLayEggsRape);
            btnSet.push("");
        ...
        {
            btnSet.push(false);
            btnSet.push("Req. to be a Easter Bunny.");
            
        }
        return btnSet;
    }
    private function USSTentRape() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["Tentacle Rape!"];
        if (monster.hasVagina() && (player.isKraken() || player.isScylla()))
        {
            btnSet.push(TentacleRape);
            btnSet.push("");
        ...
        {
            btnSet.push(false);
            btnSet.push("You need scylla or kraken tentacles to do this. Only works on female foes.");
            
        }
        return btnSet;
    }
    private function USSLiveDildo() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["Living Dildo"];
        if (player.hasVagina() && player.isKraken() && player.tallness > monster.tallness * 2)
        {
            btnSet.push(HumanDildo);
            btnSet.push("");
        ...
        {
            btnSet.push(false);
            btnSet.push("You need to be a kraken and way taller than your opponent in order to even use this scene.");
            
        }
        return btnSet;
    }
    private function USSJiangshiDrn() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = [];
        if (player.isRace(Races.JIANGSHI, 1, false))
        {
            if (monster.hasPerk(PerkLib.EnemyTrueDemon))
            {
                if (monster.hasCock())
                {
                    btnSet.push("Drain him");
                    btnSet.push(jiangshiDrainHimTrueDemons);
                    btnSet.push("");
                ...
                else
                {
                    btnSet.push("Drain him");
                    btnSet.push(false);
                    btnSet.push("Only male/herm true demon enemies.");
                    
                }
                if (monster.hasVagina())
                {
                    btnSet.push("Drain her");
                    btnSet.push(jiangshiDrainHerTrueDemons);
                    btnSet.push("");
                ...
                else
                {
                    btnSet.push("Drain her");
                    btnSet.push(false);
                    btnSet.push("Only female/herm true demon enemies.");
                    
                }
            }
            if (Std.is(monster, Minotaur) || Std.is(monster, MinotaurLord))
            {
                btnSet.push("Drain him");
                btnSet.push(jiangshiDrainHimMinotaurs);
                btnSet.push("");
            ...
            else
            {
                btnSet.push("Drain him");
                btnSet.push(false);
                btnSet.push("Only minotaur enemies.");
                
            }
        }
        else
        {
            btnSet.push("Jiangshi");
            btnSet.push(false);
            btnSet.push("You need to be a Jiangshi.");
            
        }
        return btnSet;
    }
    private function USSAnubiMummyCurse() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["Mummy Curse!!"];
        if (player.hasPerk(PerkLib.MummyCurse) && AnubiMummyCurseNotWrongEnemyType() && player.hasItem(useables.BANDAGE, 1) && player.perkv1(PerkLib.MummyLord) < player.mummyControlLimit())
        {
            btnSet.push(anubisMummyCurse);
            btnSet.push("");
            
        }
        else if (!player.hasPerk(PerkLib.MummyCurse))
        {
            btnSet.push(false);
            btnSet.push("Req. to have Mummy Curse perk.");
            
        }
        else if (!AnubiMummyCurseNotWrongEnemyType())
        {
            btnSet.push(false);
            btnSet.push("Req. enemy to be bipedal with soul. (No constructs/elementals)");
            
        }
        else if (!player.hasItem(useables.BANDAGE, 1))
        {
            btnSet.push(false);
            btnSet.push("Req. to have at least 1 bandage.");
            
        }
        else
        {
            btnSet.push(false);
            btnSet.push("Req. to be able to control more mummies.");
            
        }
        return btnSet;
    }
    private function AnubiMummyCurseNotWrongEnemyType() : Bool
    {
    ...
    private function USSTrueDemonSuccubusFeast() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["Succubus Feast"];
        if (player.hasVagina() && player.vaginaType() == VaginaClass.DEMONIC && monster.hasCock() && SuccubusIncubusFeastNotWrongEnemyType())
        {
            btnSet.push(trueDemonSuccubusFeast);
            btnSet.push("");
            
        }
        else if (!SuccubusIncubusFeastNotWrongEnemyType())
        {
            btnSet.push(false);
            btnSet.push("Req. enemy to have soul. (No constructs/elementals)");
            
        }
        else if (!monster.hasCock())
        {
            btnSet.push(false);
            btnSet.push("Req. enemy to have cock.");
            
        }
        else
        {
            btnSet.push(false);
            btnSet.push("Req. to have demonic vagina.");
            
        }
        return btnSet;
    }
    private function USSTrueDemonIncubusFeast() : Array<Dynamic>
    {
        var btnSet : Array<Dynamic> = ["Incubus Feast"];
        if (player.hasCock() && player.demonCocks() > 0 && monster.hasVagina() && SuccubusIncubusFeastNotWrongEnemyType())
        {
            btnSet.push(trueDemonIncubusFeast);
            btnSet.push("");
        ...
        {
            btnSet.push(false);
            btnSet.push("Req. enemy to have pussy.");
            
        }
        else
        {
            btnSet.push(false);
            btnSet.push("Req. to have demonic cock.");
            
        }
        return btnSet;
    }
    private function SuccubusIncubusFeastNotWrongEnemyType() : Bool
    {
    ...
    public function RaijuRapeSupercharged() : Void
    {
        ...
        function dickF() : Void
        {
            ...
            if (player.isTaur())
            {
                ...
                if ((player.horseCocks() >= 1) || (player.kirinCocks() >= 1))
                {
                    outputText(" bestial equine");
                }
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            if (monster.hasCock())
            {
                ...
                {
                    outputText(" Damn, [monster his] plug is massive. You barely manage to force [monster him] in, but there's no helping it. It's that or going about a few extra hours as a lust crazed lunatic, and you'd prefer the former.");
                }
    ...
    {
        clearOutput();
        var cock : Bool = player.hasCock() && (!player.hasVagina() || rand(2) == 0);
        ...
        if (monster.hasCock())
        {
            outputText(" [monster cock] hardening and twitching");
        }
        if (monster.hasVagina())
        {
            if (monster.hasCock())
            {
                outputText(" and " + monster.pronoun3);
            }
    ...
    public function brainMelt() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(player.cockDescriptShort());
        }
        ...
        if (player.hasVagina())
        {
            ...
            if (monster.hasVagina())
            {
                outputText("on her now drenched snatch allowing you to bring yourself to new heights of pleasant delirium.");
            }
            else if (monster.hasCock())
            {
                outputText("on his drooling erect cock allowing you to bring yourself to new heights of pleasant delirium.");
            }
            else
            {
                outputText("on your victim naked belly attempting to draw additionnal pleasure in vain. With your victim devoid of sexual organs thats as good as you will get.");
            }
        }
        if (player.hasCock())
        {
            outputText("While you mentaly rape your victim of everything that used to make its sense of self you push your needy cock through the now welcoming hole that is your half broken toy butt thrusting to bring yourself to new heights of pleasant delirium.");
        }
    ...
    public function raijuVoltTransfer() : Void
    {
        ...
        if (monster.hasCock())
        {
            outputText(" wrapping and sheathing around [monster his] penis,");
        }
        if (monster.biggestTitSize() >= 1)
        {
            outputText(" massaging the shape of [monster his] [monster breasts],");
        }
        if (monster.hasVagina())
        {
            outputText(" licking the entrance of [monster his] pussy");
        }
    ...
    public function hinezumiHeatTransferScene() : Void
    {
        ...
        if (monster.hasCock())
        {
            outputText(" wrap and sheath around [monster his] " + monster.cockDescriptShort() + ",");
        }
        if (monster.biggestTitSize() >= 1)
        {
            outputText("massage the shape of [monster his] " + monster.breastDescript(0) + ",");
        }
        if (monster.hasVagina())
        {
            outputText("lick the entrance of [monster his] " + monster.vaginaDescript() + "");
        }
        ...
        if (monster.biggestTitSize() >= 1)
        {
            outputText(" You notice [monster his] breasts begin to dribble milk as well. ");
        }
    ...
    public function gobomechFillTheReservoir() : Void
    {
        ...
        if (player.keyItemvX("Cum Reservoir", 1) == 1)
        {
            outputText("1 quarters");
        }
        if (player.keyItemvX("Cum Reservoir", 1) == 2)
        {
            outputText("2 quarters");
        }
        if (player.keyItemvX("Cum Reservoir", 1) == 3)
        {
            outputText("3 quarters");
        }
        if (player.keyItemvX("Cum Reservoir", 1) == 4)
        {
            outputText("full");
    ...
    public function Cooldown() : Void
    {
        ...
        if (monster.wolfCocks() > 0)
        {
            outputText(", your warm body slowly forcing his puppy pecker out of its sheath");
        }
        ...
        if (monster.wolfCocks() > 0)
        {
            outputText(", enjoying yourself as his knot grows and locks itself in your [pussy]");
        }
        outputText(". You begin to slowly work his tool with your vagina, moaning as the shaft ever so gently scrapes your walls. You want the cool cream inside to fill you all over and soon the beast growls as it reaches its impending climax, its dick unloading thick ropes of cooling cum into your furnace of a pussy.\n\n" +
                "Thoroughly satisfied you pull from [themonster] " + monster.cockDescriptShort() + " and walk off. Gosh, this one felt hella good you should come back more often when you’re in need of some cooling.")  //We give both sex reward so to end player rut or heat in the event pc has either  ;
    ...
    public function jiangshiDrainHerTrueDemons() : Void
    {
        ...
        var sharedEnd : Void->Void = function() : Void
        {
            ...
            {
                outputText("You moan delighted as you rub your [breasts] against the demon prodigious chest savoring the electrifying feeling of smaller input of energies circulating directly from her nipples to yours.\n\n");
            }
    ...
    public function jiangshiDrainHimMinotaurs() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.AlterBindScroll2))
        {
            player.soulforce += player.maxSoulforce() * 0.4;
        }
        else
        {
    ...
    public function trueDemonSuccubusFeast() : Void
    {
        ...
        if (isntATentacleBeast())
        {
            outputText("You caress [themonster] cheek gently with your hand and consider the option giggling as you begin already feeling pleasure again from the uncontrollable throbbing erection in your cunt.\n\n");
        }
        menu();
    ...
    public function trueDemonSuccubusFeastCum() : Void
    {
        ...
        if (SuccubusIncubusFeastNotWrongEnemyTypeForLethicite() && player.hasPerk(PerkLib.Soulless))
        {
            ...
            {
                inventory.takeItem(consumables.LETH1TE, cleanupAfterCombat);
            }
            else if (monster.level < 75)
            {
                inventory.takeItem(consumables.LETH2TE, cleanupAfterCombat);
            }
            else
            {
                inventory.takeItem(consumables.LETH3TE, cleanupAfterCombat);
            }
        }
        else
        {
    ...
    private function isntATentacleBeast() : Bool
    {
        return !(Std.is(monster, TentacleBeast) || Std.is(monster, TentacleBeastRaging));
    }
    ...
    public function trueDemonIncubusFeast() : Void
    {
        ...
        if (isntATentacleBeast())
        {
            outputText("You stroke [themonster] chin with your hand and consider while her body, shivering and twitching, sends jolts of pleasure into you, your throbbing cock absorbing all of her frustrated delight.\n\n");
        }
        menu();
    ...
    public function trueDemonIncubusFeastCum() : Void
    {
        ...
        if (SuccubusIncubusFeastNotWrongEnemyTypeForLethicite() && player.hasPerk(PerkLib.Soulless))
        {
            outputText(" Letting her body warp into a new succubus, you take the small purple shard of lethicite that she left between her legs, pocketing it for later.\n\n");
        }
    ...
    private function SuccubusIncubusFeastNotWrongEnemyTypeForLethicite() : Bool
    {
