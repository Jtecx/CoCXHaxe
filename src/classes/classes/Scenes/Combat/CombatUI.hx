...
class CombatUI extends BaseCombatContent
{
    ...
    public function mainMenu() : Void
    {
        ...
        if (flags[kFLAGS.ELEMENTAL_CONJUER_SUMMONS] == 2 || flags[kFLAGS.ELEMENTAL_CONJUER_SUMMONS] == 4)
        {
            ...
            {
                btnMelee.disable("You cannot use command your elemental to attack an opponent you cannot see or target.");
            }
        }
        /*
        ...
        else
        {
            ...
            if (player.isInGoblinMech() || player.isInNonGoblinMech())
            {
                ...
                {
                    weapon = "twin power blades";
                }
                if (monster.isFlying())
                {
                    if (player.isFlying())
                    {
                        btnMelee.show("Attack", combat.basemechmeleeattacks, "Attempt to attack the enemy with your mech " + weapon + ".  Damage done is determined by your strength and weapon.").icon("A_Melee");
                    }
                    else
                    {
                        btnMelee.disable("No way you could reach enemy in air with melee attacks.");
                    ...
                    if (combat.isEnemyInvisible)
                    {
                        btnRanged.disable("You cannot use shoot an opponent you cannot see or target.");
                    }
                }
                else
                {
            ...
            else if ((player.isStaffTypeWeapon() || player.weapon.isWandType() || player.weaponOff.isWandType() || player.isPartiallyStaffTypeWeapon()) && player.weapon != weapons.TIDAR && player.weaponOff != weapons.TIDAR && player.hasPerk(PerkLib.StaffChanneling) && flags[kFLAGS.STAFF_CHANNELING_MODE] != null)
            
            //No shooting fuckbolts from Tidarion!!!{
                
                if (!Wings.Types[player.wings.type].canFly && Arms.Types[player.arms.type].canFly && player.isFlying && player.statusEffectv2(StatusEffects.Flying) == 0)
                {
                    btnMelee.disable("No way you could use your melee weapon with those arms while flying.");
                }
                else if (player.hasPerk(PerkLib.ElementalBolt))
                {
                    btnMelee.show("E.Bolt", combat.magic.spellElementalBolt, "Attempt to attack the enemy with elemental bolt from your [weapon].  Damage done is determined by your intelligence and weapon.").icon("A_Melee");
                }
                else
                {
                    btnMelee.show("M.Bolt", combat.magic.spellMagicBolt, "Attempt to attack the enemy with magic bolt from your [weapon].  Damage done is determined by your intelligence and weapon.").icon("A_Melee");
                }
            }
            else if (player.hasStatusEffect(StatusEffects.Gallop))
            {
                btnMelee.disable("No way you could hit enemy with melee attacks while galloping. Unless you stop for a moment or two.");
            }
            else if (monster.isFlying())
            {
                if (player.isFlying() || monster.hasStatusEffect(StatusEffects.RootOfTheIssue) || player.haveThrowableMeleeWeapon() || player.weapon.isWhipType() || player.weaponOff.isWhipType() || player.weapon.isRibbonType() || player.weaponOff.isRibbonType() ||
                    ((player.isStaffTypeWeapon() || player.weapon.isWandType() || player.weaponOff.isWandType() || player.isPartiallyStaffTypeWeapon()) && player.hasPerk(PerkLib.StaffChanneling) && flags[kFLAGS.STAFF_CHANNELING_MODE] != null))
                {
                    if (player.isFlying() || monster.hasStatusEffect(StatusEffects.RootOfTheIssue))
                    {
                        ...
                        {
                            if (!Wings.Types[player.wings.type].canFly && Arms.Types[player.arms.type].canFly)
                            {
                                btnMelee.disable("No way you could use your melee weapon with those arms while flying.");
                            }
                            else
                        ...
                        {
                            btnMelee.disable("No way you could hit enemy with melee attacks while flying. Req. Aerial Combat perk or having melee weapon that could be used for range attack too.");
                        }
                    }
                    else if ((player.isStaffTypeWeapon() || player.weapon.isWandType() || player.weaponOff.isWandType() || player.isPartiallyStaffTypeWeapon()) && player.hasPerk(PerkLib.StaffChanneling) && flags[kFLAGS.STAFF_CHANNELING_MODE] != null)
                    {
                ...
                {
                    btnMelee.disable("No way you could reach enemy in air with melee attacks. Unless you have melee weapon that could be used for range attack too.");
                }
            }
            else
            {
        ...
        // Ranged
        var _sw17_ = (player.weaponRangePerk);        
        ...
        switch (_sw17_)
        {
            ...
            case "Throwing":
                ...
                if (player.ammo <= 0 && player.weaponRange != weaponsrange.SHUNHAR)
                {
                    btnRanged.disable("You have used all your throwing weapons in this fight.");
                }
                if (combat.isEnemyInvisible)
                {
                    btnRanged.disable("You cannot use shoot an opponent you cannot see or target.");
                }
            case "Pistol", "Rifle", "2H Firearm", "Dual Firearms", "Dual 2H Firearms":
                if (player.ammo <= 0)
                {
                ...
                {
                    btnRanged.show("Shoot", combat.fireBow, "Fire a round at your opponent with your " + player.weaponRangeName + "!  Damage done is determined only by your weapon. <b>AMMO LEFT: " + player.ammo + "</b>").icon("A_Ranged");
                }
                if (combat.isEnemyInvisible)
                {
                    btnRanged.disable("You cannot use shoot an opponent you cannot see or target.").icon("GenericWeapon_Bow");
                }
            default:
        ...
        {
            btnRanged.disable("It would be rather difficult to aim while flapping your arms.");
        }
        if (player.hasStatusEffect(StatusEffects.Gallop) && !player.hasPerk(PerkLib.CentaurHunterStyleMovingShot))
        {
            btnRanged.disable("It would be rather difficult to aim while galloping.");
        }
        if (player.isInGoblinMech())
        {
            if (player.hasKeyItem("Repeater Gun") >= 0 || player.hasKeyItem("Machine Gun MK1") >= 0 || player.hasKeyItem("Machine Gun MK2") >= 0 || player.hasKeyItem("Machine Gun MK3") >= 0 || player.hasKeyItem("Machine Gun MK4") >= 0 || player.hasKeyItem("Machine Gun MK5") >= 0 || player.hasKeyItem("Machine Gun MK6") >= 0)
            {
                if (player.isFirearmTypeWeapon())
                {
                    ...
                    {
                        btnRanged.disable("Your firearms are not compatibile with the current piloted mech.");
                    }
                }
                else
                {
                    btnRanged.disable("You could use your ranged weapon while piloting the goblin mech if you have firearms.");
                }
            }
            else
            {
                btnRanged.disable("There is no way you could use your ranged weapon while piloting the goblin mech.");
            }
            if (combat.isEnemyInvisible)
            {
                btnRanged.disable("You cannot use shoot an opponent you cannot see or target.");
        ...
        if (player.vehicles == vehicles.HB_MECH)
        {
            ...
            {
                btnRanged.show("Shoot", combat.fireBow, "Attempt to attack the enemy with your mech's inbuilt ranged weapons.  Damage done is determined by your speed and weapon.").icon("A_Ranged");
            }
            else
            {
                btnRanged.disable("Your range weapon is not compatibile to be used with current piloted mech.");
            }
        }
        if (player.hasPerk(PerkLib.ElementalBody) && (player.weaponRangePerk == "" || player.weaponRangePerk == "Tome"))
        {
            btnRanged.show("Throw", combat.throwElementalAttack, "Attack enemy with range elemental attack.  Damage done is determined by your strength.").icon("A_Ranged");
        }
        if (player.weapon == weapons.MGSWORD && (player.weaponRangePerk == "" || player.weaponRangePerk == "Tome"))
        {
            btnRanged.show("MoonWave", combat.throwElementalAttack, "Attack enemy with wave of moonlight.  Damage done is determined by your intelligence and weapon.").icon("A_Ranged");
        }
        if (player.weapon == weapons.MCLAWS && (player.weaponRangePerk == "" || player.weaponRangePerk == "Tome"))
        {
            btnRanged.show("MoonWave", combat.throwElementalAttack, "Attack enemy with wave of moonlight.  Damage done is determined by your intelligence and weapon.").icon("A_Ranged");
        }
        if (Std.is(player.weapon, Tidarion) && (player.weaponRangePerk == "" || player.weaponRangePerk == "Tome"))
        {
            btnRanged.show("FireBeam", combat.throwElementalAttack, "Attack enemy with a beam of fire.  Damage done is determined by your intelligence and weapon.").icon("A_Ranged");
        }
        btnItems.show("Items", inventory.inventoryMenu, "The inventory allows you to use an item.  Be careful, as this leaves you open to a counterattack when in combat.").icon("I_red potion icon id")  // Submenus  ;
        
        
        
        var vampireBiteDuringGrapple : Int->Void = function(Position : Int) : Void
        {
        ...
        var vampireBiteDuringGrappleBreakHypnosis : Int->Void = function(Position : Int) : Void
        {
            if (player.perkv1(IMutationsLib.HollowFangsIM) >= 2)
            {
        ...
        function vampireBiteDuringGrappleV(Position : Int) : Void
        {
            ...
            {
                addButtonDisabled(Position, "Bite", "If only you had fangs.");
            }
        ...
        if (player.isFlying())
        {
            combat.pspecials.buildMenuForFlying(physpButtons);
        }
        else
        {
        ...
        if (physpButtons.length > 0)
        {
            if (player.isInGoblinMech() || player.isInNonGoblinMech())
            {
                btnPSpecials.show("Mech", submenuPhySpecials, "Mech special attacks menu.", "Mech Specials");
            }
            else
            {
                btnPSpecials.show("P. Specials", submenuPhySpecials, "Physical special attack menu.", "Physical Specials");
            }
        }
        ...
        if (magspButtons.length > 0)
        {
            btnMSpecials.show("M. Specials", submenuMagSpecials, "Mental and supernatural special attack menu.", "Magical Specials");
        }
        ...
        // Submenu - Spells
        cast((spellBookButtons), BuildSpellBookMenu);
        if (spellBookButtons.length > 0)
        {
            btnMagic.show("Spells", submenuSpells, "Opens your spells menu, where you can cast any spells you have learned.", "Spells").icon("A_Magic");
        }
        ...
        // Submenu - Soulskills{
            btnMagic.disable().icon("A_Magic")  //combat.soulskills.buildMenu(soulforceButtons);  ;
        }
        
        
        
        buildAbilityMenu(CombatAbilities.ALL_SOULSKILLS, soulforceButtons);
        if (soulforceButtons.length > 0)
        {
            btnSoulskills.show("Soulforce", submenuSoulforce, "Soulforce attacks menu.", "Soulforce Specials");
        }
        // Submenu - Other
        ...
        {
            btnOther.show("Other", submenuOther, "Combat options and uncategorized actions");
        }
        ...
        {
            btnTease.disable("No way you could make an enemy more aroused by striking a seductive pose and exposing parts of your body while piloting elf mech.");
        }
        else if (monster.lustVuln != 0 && monster.hasStatusEffect(StatusEffects.Stunned) && player.hasPerk(PerkLib.Straddle) && !combat.isEnemyInvisible)
        {
            btnTease.show("Straddle", combat.Straddle, "Go to town on your opponent with devastating teases.").icon("A_Tease");
        }
        else
        {
        ...
        {
            btnRun.show("Struggle", combat.struggleCreepingDoom, "Shake away the pests.");
        }
        else
        {
        ...
        var playerBusy : Bool = true;  //changed to true if 'stupid' allies can help.  
        ...
        else if (monster.hasStatusEffect(StatusEffects.Swallowed))
        {
            ...
            if (monster.lustVuln != 0 && player.hasPerk(PerkLib.Straddle) && monster.hasStatusEffect(StatusEffects.Stunned))
            {
                addButton(1, "Straddle", combat.Straddle).hint("Change position and initiate a straddling stance").icon("A_Tease");
            }
        ...
        else if (monster.hasStatusEffect(StatusEffects.Dig))
        {
            ...
            if (monster.statusEffectv1(StatusEffects.Dig) > 0)
            {
                ...
                {
                    addButton(0, "Grab", combat.CancerGrab).hint("Dig underneath your opponent and attempt to grab it in your pincers");
                }
                if (player.arms.type == Arms.FROSTWYRM)
                {
                    if (!player.hasStatusEffect(StatusEffects.CooldownTremor))
                    {
                        addButton(1, "Tremor", combat.Tremor).hint("Cause seismic activity beneath your foes in an attempt to stun them");
                    }
                    if (player.lowerBody == LowerBody.FROSTWYRM)
                    {
                        addButton(0, "Grab", SceneLib.desert.nagaScene.nagaPlayerConstrict).hint("Surge out of the ground and coil around your opponent!");
                    }
                }
        ...
        else if (monster.hasStatusEffect(StatusEffects.Pounce))
        {
            ...
            if (monster.lustVuln != 0 && !monster.plural && player.hasPerk(PerkLib.Straddle))
            {
            ...
            {
                if (monster.hasStatusEffect(StatusEffects.DisplacerPlug))
                {
                    addButton(1, "Feed", combat.displacerFeedContinue).hint("Feast on your foe breast milk.");
                }
                else
                {
                    addButton(1, "Attach", combat.displacerCombatFeed).hint("Attach you tentacles to start feast on your foe breast milk. \n\nFatigue Cost: " + physicalCost(50) + "");
                }
            }
            vampireBiteDuringGrapple(3);
        ...
        if (!playerBusy)
        {
            ...
            {
                //Do not display if enemy is already deadif (!combat.combatIsOver(false))
                ...
                // Migrate Sand Trap/Alruine/Drider Incubus/Minotaur King/AngelLR/TwinBosses/Lethice/WoodElvesHuntingParty{
                    ...
                    if ((player.weaponRange == weaponsrange.GTHRSPE && player.ammo <= 15) || ((player.weaponRange == weaponsrange.ATKNIFE || player.weaponRange == weaponsrange.RTKNIFE || player.weaponRange == weaponsrange.STKNIFE || player.weaponRange == weaponsrange.TTKNIFE) && player.ammo <= 10) || ((player.weaponRange == weaponsrange.GTHRAXE || player.weaponRange == weaponsrange.O_JAVEL || player.weaponRange == weaponsrange.TRJAVEL) && player.ammo <= 5))
                    {
    ...
    public function isWispTurn() : Bool
    {
    ...
    {
        var doWillOfTheWispAttack : Void->Void = function() : Void
        {
    ...
    public function isFlyingSwordTurn() : Bool
    {
    ...
    public function isMummyTurn() : Bool
    {
    ...
    public function isMechAITurn() : Bool
    {
        return player.isInGoblinMech() && (player.hasKeyItem("Improved Artificial Intelligence") >= 0 || player.hasKeyItem("Improved Artificial Intelligence MK2") >= 0 || player.hasKeyItem("Improved Artificial Intelligence MK3") >= 0 || player.hasKeyItem("Improved Artificial Intelligence MK4") >= 0) && (player.hasKeyItem("Auto turret") >= 0 || player.hasKeyItem("Auto turret MK2") >= 0 || player.hasKeyItem("Auto turret MK3") >= 0 || player.hasKeyItem("Auto turret MK4") >= 0 || player.hasKeyItem("Auto turret MK5") >= 0 || player.hasKeyItem("Auto turret MK6") >= 0) && flags[kFLAGS.IN_COMBAT_PLAYER_GOBLIN_MECH_AI_ATTACKED] != 1;
    }
    
    public function doMechAITurn() : Void
    {
        combat.shootMechWeaponByAI();
        if (player.hasKeyItem("Improved Artificial Intelligence MK2") >= 0 || player.hasKeyItem("Improved Artificial Intelligence MK3") >= 0 || player.hasKeyItem("Improved Artificial Intelligence MK4") >= 0)
        {
            combat.pspecials.goblinMechAIUseRadomlyItWeaponFunctions();
        }
        if (player.hasKeyItem("Improved Artificial Intelligence MK4") >= 0)
        {
            combat.pspecials.goblinMechAIUseRadomlyItWeaponFunctions();
        }
    ...
    public function isGolemTurn() : Bool
    {
    ...
    public function doGolemTurn() : Void
    {
        ...
        if (monster.plural)
        {
            ...
            {
                if (player.mana >= combat.pspecials.permanentgolemsendcost() * 3)
                {
                    addButton(6, "Send P.Gol/3", combat.pspecials.sendPermanentGolem, 3).hint("Send 3 golems (deals 5x damage).\n<b>Mana cost of sending 3 permanent golems: " + (combat.pspecials.permanentgolemsendcost() * 3) + "</b>");
                }
                else
                {
                    addButtonDisabled(6, "Send P.Gol/3", "Not enough mana.");
                }
            }
            if (flags[kFLAGS.PERMANENT_GOLEMS_BAG] >= 5)
            {
                if (player.mana >= combat.pspecials.permanentgolemsendcost() * 5)
                {
                    addButton(11, "Send P.Gol/5", combat.pspecials.sendPermanentGolem, 5).hint("Send 3 golems (deals 5x damage).\n<b>Mana cost of sending 5 permanent golems: " + (combat.pspecials.permanentgolemsendcost() * 5));
                }
                else
                {
                    addButtonDisabled(11, "Send P.Gol/5", "Not enough mana.");
                }
            }
        }
        if (flags[kFLAGS.IMPROVED_PERMANENT_GOLEMS_BAG] > 0)
        {
            if (player.mana >= combat.pspecials.permanentimprovedgolemsendcost())
            {
                addButton(2, "Send I.P.Gol/1", combat.pspecials.sendPermanentImprovedGolem, 1).hint("Send 1 improved golem (deals 5x damage).\n<b>Mana cost of sending 1 improved permanent golem: " + combat.pspecials.permanentimprovedgolemsendcost() + "</b>");
            }
            else
            {
                addButtonDisabled(2, "Send I.P.Gol/1", "Not enough mana.");
            }
            if (monster.plural)
            {
                if (flags[kFLAGS.IMPROVED_PERMANENT_GOLEMS_BAG] >= 3)
                {
                    if (player.mana >= combat.pspecials.permanentimprovedgolemsendcost() * 3)
                    {
                        addButton(7, "Send I.P.Gol/3", combat.pspecials.sendPermanentImprovedGolem, 3).hint("Send 3 improved golems (deals 5x damage).\n<b>Mana cost of sending 3 improved permanent golems: " + (combat.pspecials.permanentimprovedgolemsendcost() * 3) + "</b>");
                    }
                    else
                    {
                        addButtonDisabled(7, "Send I.P.Gol/3", "Not enough mana.");
                    }
                }
                if (flags[kFLAGS.IMPROVED_PERMANENT_GOLEMS_BAG] >= 5)
                {
                    if (player.mana >= combat.pspecials.permanentimprovedgolemsendcost() * 5)
                    {
                        addButton(12, "Send I.P.Gol/5", combat.pspecials.sendPermanentImprovedGolem, 5).hint("Send 3 improved golems (deals 5x damage).\n<b>Mana cost of sending 5 improved permanent golems: " + (combat.pspecials.permanentimprovedgolemsendcost() * 5) + "</b>");
                    }
                    else
                    {
                        addButtonDisabled(12, "Send I.P.Gol/5", "Not enough mana.");
                    }
                }
            }
        }
        ...
        {
            if (player.mana >= combat.pspecials.permanentsteelgolemsendcost())
            {
                addButton(3, "Send S.Gol/1", combat.pspecials.sendPermanentSteelGolem, 1).hint("Mana cost of sending 1 permanent steel golem: " + combat.pspecials.permanentsteelgolemsendcost());
            }
            else
            {
                addButtonDisabled(3, "Send S.Gol/1", "Not enough mana.");
            }
            if (monster.plural)
            {
                if (flags[kFLAGS.IMPROVED_PERMANENT_GOLEMS_BAG] >= 3)
                {
                    if (player.mana >= combat.pspecials.permanentimprovedgolemsendcost() * 3)
                    {
                        addButton(8, "Send I.P.Gol/3", combat.pspecials.sendPermanentImprovedGolem, 3).hint("Send 3 improved golems (deals 5x damage).\n<b>Mana cost of sending 3 improved permanent golems: " + (combat.pspecials.permanentimprovedgolemsendcost() * 3) + "</b>");
                    }
                    else
                    {
                        addButtonDisabled(8, "Send I.P.Gol/3", "Not enough mana.");
                    }
                }
                if (flags[kFLAGS.IMPROVED_PERMANENT_GOLEMS_BAG] >= 5)
                {
                    if (player.mana >= combat.pspecials.permanentimprovedgolemsendcost() * 5)
                    {
                        addButton(13, "Send I.P.Gol/5", combat.pspecials.sendPermanentImprovedGolem, 5).hint("Send 3 improved golems (deals 5x damage).\n<b>Mana cost of sending 5 improved permanent golems: " + (combat.pspecials.permanentimprovedgolemsendcost() * 5) + "</b>");
                    }
                    else
                    {
                        addButtonDisabled(13, "Send I.P.Gol/5", "Not enough mana.");
                    }
                }
            }
        }
        if (flags[kFLAGS.IMPROVED_PERMANENT_STEEL_GOLEMS_BAG] > 0)
        {
            if (player.mana >= combat.pspecials.permanentsteelgolemsendcost())
            {
                addButton(4, "Send I.S.Gol/1", combat.pspecials.sendPermanentImprovedSteelGolem, 1).hint("Mana cost of sending 1 permanent improved steel golem: " + combat.pspecials.permanentimprovedsteelgolemsendcost());
            }
            else
            {
                addButtonDisabled(4, "Send I.S.Gol/1", "Not enough mana.");
            }
            if (monster.plural)
            {
                if (flags[kFLAGS.IMPROVED_PERMANENT_STEEL_GOLEMS_BAG] >= 3)
                {
                    if (player.mana >= combat.pspecials.permanentimprovedgolemsendcost() * 3)
                    {
                        addButton(9, "Send I.P.Gol/3", combat.pspecials.sendPermanentImprovedGolem, 3).hint("Send 3 improved golems (deals 5x damage).\n<b>Mana cost of sending 3 improved permanent golems: " + (combat.pspecials.permanentimprovedgolemsendcost() * 3) + "</b>");
                    }
                    else
                    {
                        addButtonDisabled(9, "Send I.P.Gol/3", "Not enough mana.");
                    }
                }
                if (flags[kFLAGS.IMPROVED_PERMANENT_STEEL_GOLEMS_BAG] >= 5)
                {
                    if (player.mana >= combat.pspecials.permanentimprovedgolemsendcost() * 5)
                    {
                        addButton(14, "Send I.P.Gol/5", combat.pspecials.sendPermanentImprovedGolem, 5).hint("Send 3 improved golems (deals 5x damage).\n<b>Mana cost of sending 5 improved permanent golems: " + (combat.pspecials.permanentimprovedgolemsendcost() * 5) + "</b>");
                    }
                    else
                    {
                        addButtonDisabled(14, "Send I.P.Gol/5", "Not enough mana.");
                    }
                }
            }
        }
    ...
    public function isSkeletonsTurn() : Bool
    {
    ...
    public function isEpicElementalTurn() : Bool
    {
    ...
    public function doEpicElementalTurn() : Void
    {
        ...
        {
            if (player.hasPerk(PerkLib.ElementalBody) && player.perkv1(PerkLib.ElementalBody) == 1)
            {
                addButtonDisabled(1, "Air(E)", "You can't command this elemental to attack (independently of you) when you're currently fused with it.");
            }
            else
            {
                addButton(1, "Air(E)", combat.baseelementalattacks, Combat.AIR_E, true);
            }
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsEarthE))
        {
            if (player.hasPerk(PerkLib.ElementalBody) && player.perkv1(PerkLib.ElementalBody) == 2)
            {
                addButtonDisabled(2, "Earth(E)", "You can't command this elemental to attack (independently of you) when you're currently fused with it.");
            }
            else
            {
                addButton(2, "Earth(E)", combat.baseelementalattacks, Combat.EARTH_E, true);
            }
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsFireE))
        {
            if (player.hasPerk(PerkLib.ElementalBody) && player.perkv1(PerkLib.ElementalBody) == 3)
            {
                addButtonDisabled(3, "Fire(E)", "You can't command this elemental to attack (independently of you) when you're currently fused with it.");
            }
            else
            {
                addButton(3, "Fire(E)", combat.baseelementalattacks, Combat.FIRE_E, true);
            }
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsWaterE))
        {
            if (player.hasPerk(PerkLib.ElementalBody) && player.perkv1(PerkLib.ElementalBody) == 4)
            {
                addButtonDisabled(4, "Water(E)", "You can't command this elemental to attack (independently of you) when you're currently fused with it.");
            }
            else
            {
                addButton(4, "Water(E)", combat.baseelementalattacks, Combat.WATER_E, true);
            }
        }
    }
    
    public function isElementalTurn() : Bool
    {
    ...
    public function doElementalTurn() : Void
    {
        ...
        {
            addButton(1, "Air", combat.baseelementalattacks, Combat.AIR);
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsEarth))
        {
            addButton(2, "Earth", combat.baseelementalattacks, Combat.EARTH);
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsFire))
        {
            addButton(3, "Fire", combat.baseelementalattacks, Combat.FIRE);
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsWater))
        {
            addButton(4, "Water", combat.baseelementalattacks, Combat.WATER);
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsEther))
        {
            addButton(5, "Ether", combat.baseelementalattacks, Combat.ETHER);
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsWood))
        {
            addButton(6, "Wood", combat.baseelementalattacks, Combat.WOOD);
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsMetal))
        {
            addButton(7, "Metal", combat.baseelementalattacks, Combat.METAL);
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsIce))
        {
            addButton(8, "Ice", combat.baseelementalattacks, Combat.ICE);
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsLightning))
        {
            addButton(9, "Lightning", combat.baseelementalattacks, Combat.LIGHTNING);
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsDarkness))
        {
            addButton(10, "Darkness", combat.baseelementalattacks, Combat.DARKNESS);
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsPoison))
        {
            addButton(11, "Poison", combat.baseelementalattacks, Combat.POISON);
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsPurity))
        {
            addButton(12, "Purity", combat.baseelementalattacks, Combat.PURITY);
        }
        if (player.hasStatusEffect(StatusEffects.SummonedElementalsCorruption))
        {
            addButton(13, "Corruption", combat.baseelementalattacks, Combat.CORRUPTION);
        }
    }
    
    public function isBloodPuppiesTurn() : Bool
    {
    ...
    public function doBloodPuppiesTurn() : Void
    {
        ...
        addButton(1, "Blood Swipe", confirmFunction, CombatAbilities.BPBloodSwipe.perform).disableIf(!CombatAbilities.BPBloodSwipe.isKnownAndUsable);
        addButton(2, "Blood Dewdrops", confirmFunction, CombatAbilities.BPBloodDewdrops.perform).disableIf(!CombatAbilities.BPBloodDewdrops.isKnownAndUsable);
        addButton(3, "Heart Seeker", confirmFunction, CombatAbilities.BPHeartSeeker.perform).disableIf(!CombatAbilities.BPHeartSeeker.isKnownAndUsable);
    }
    
    public function isCompanionTurn(num : Int) : Bool
    {
        var present : Bool;
        var acted : Bool;
        var occupied : Bool = ((player.hasStatusEffect(StatusEffects.MinoKing) && player.statusEffectv1(StatusEffects.MinoKing) == num) || monster.hasStatusEffect(StatusEffects.PastPresent));
        switch (num)
        {
            case 0:
    ...
    public function doCompanionTurn(num : Int, clearAndNext : Bool = true) : Void
    {
        var companionName : String = (num == 0) ? flags[kFLAGS.PLAYER_COMPANION_0] : (num == 1) ? flags[kFLAGS.PLAYER_COMPANION_1] : (num == 2) ? flags[kFLAGS.PLAYER_COMPANION_2] : flags[kFLAGS.PLAYER_COMPANION_3];
        if (!monster.preCompanionSeal(companionName))
        {
            return;
        }
        var actFunction : Function = (companionName == "Alvina") ? combat.comfoll.alvinaCombatActions : (companionName == "Amily") ? combat.comfoll.amilyCombatActions : (companionName == "Aurora") ? combat.comfoll.auroraCombatActions : (companionName == "Ayane") ? combat.comfoll.ayaneCombatActions : (companionName == "Etna") ? combat.comfoll.etnaCombatActions : (companionName == "Excellia") ? combat.comfoll.excelliaCombatActions : (companionName == "" + flags[kFLAGS.GHOULISH_VAMPIRE_SERVANT_NAME] + "") ? combat.comfoll.ghoulishVampServCombatActions : (companionName == "Kiha") ? combat.comfoll.kihaCombatActions : (companionName == "Midoka") ? combat.comfoll.midokaCombatActions : (companionName == "Mitzi") ? combat.comfoll.mitziCombatActions : (companionName == "Neisa") ? combat.comfoll.neisaCombatActions : (companionName == "Tyrantia") ? combat.comfoll.tyrantiaCombatActions : (companionName == "Zenji") ? combat.comfoll.zenjiCombatActions : null;
        //do action
        if (clearAndNext)
        {
            clearOutput();
        ...
        if (actFunction == null)
        {
            CoC_Settings.error("Illegal companion " + num + ": " + companionName);
        }
        actFunction();
        ...
        if (player.hasPerk(PerkLib.MotivationSu) && player.hasPerk(PerkLib.VocalTactician))
        {
            actFunction();
        }
        if (player.hasPerk(PerkLib.DrillSergeant))
        {
            actFunction();
        }
        if (clearAndNext)
    ...
    private function BuildSpellBookMenu(buttons : ButtonDataList) : Void
    {
        ...
        if (player.hasPerk(PerkLib.JobSorcerer))
        {
            ...
            if (player.hasPerk(PerkLib.StaffChanneling) && (player.weapon.isWandType() || player.weaponOff.isWandType() || player.weapon.isStaffType() || player.weaponOff.isStaffType()))
            {
            ...
            if (player.hasPerk(PerkLib.MagesWrath))
            {
                ...
                if (player.hasPerk(PerkLib.StaffChanneling) && (player.weapon.isWandType() || player.weaponOff.isWandType() || player.weapon.isStaffType() || player.weaponOff.isStaffType()))
                {
                    bd.hint("Attempt to attack the enemy with wrath-empowered magic bolt from your [weapon].  Damage done is determined by your intelligence and weapon.", "Wrath-Empowered Magic Bolt");
                }
                else
                {
                    bd.hint("Attempt to attack the enemy with wrath-empowered magic bolt.  Damage done is determined by your intelligence.", "Wrath-Empowered Magic Bolt");
                }
                if (player.mana < spellCost(40))
                {
                    bd.disable("Your mana is too low to cast this spell.");
        ...
        if (player.hasPerk(PerkLib.ElementalBolt))
        {
            ...
            if (player.hasPerk(PerkLib.StaffChanneling) && (player.weapon.isWandType() || player.weaponOff.isWandType() || player.weapon.isStaffType() || player.weaponOff.isStaffType()))
            {
                bd.hint("Attempt to attack the enemy with elemental bolt from your [weapon].  Damage done is determined by your intelligence and weapon.", "Elemental Bolt");
            }
            else
            {
                bd.hint("Attempt to attack the enemy with elemental bolt.  Damage done is determined by your intelligence.", "Elemental Bolt");
            }
            ...
            if (player.hasPerk(PerkLib.MagesWrath))
            {
                ...
                if (player.hasPerk(PerkLib.StaffChanneling) && (player.weapon.isWandType() || player.weaponOff.isWandType() || player.weapon.isStaffType() || player.weaponOff.isStaffType()))
                {
                    bd.hint("Attempt to attack the enemy with wrath-empowered elemental bolt from your [weapon].  Damage done is determined by your intelligence and weapon.", "Wrath-Empowered Elemental Bolt");
                }
                else
                {
                    bd.hint("Attempt to attack the enemy with wrath-empowered elemental bolt.  Damage done is determined by your intelligence.", "Wrath-Empowered Elemental Bolt");
                }
                if (player.mana < spellCost(80))
                {
                    bd.disable("Your mana is too low to cast this spell.");
        ...
        {
            buttons.add("White Spells", curry(submenu, whiteSpellButtons, submenuSpells, 0, false)).hint("Open your White magic book");
        }
        if (blackSpellButtons.length > 0)
        {
            buttons.add("Black Spells", curry(submenu, blackSpellButtons, submenuSpells, 0, false)).hint("Open your Black magic book");
        }
        if ((player.hasPerk(PerkLib.GreyMagic) || player.hasPerk(PerkLib.PrestigeJobGreySage)) && greySpellButtons.length > 0)
        {
            buttons.add("Grey Spells", curry(submenu, greySpellButtons, submenuSpells, 0, false)).hint("Open your Grey magic book");
        }
        if ((player.hasPerk(PerkLib.HexKnowledge) || player.hasPerk(PerkLib.PrestigeJobGreySage)) && hexSpellButtons.length > 0)
        {
            buttons.add("Hexes", curry(submenu, hexSpellButtons, submenuSpells, 0, false)).hint("Open your Hex grimoire");
        }
        if ((player.hasPerk(PerkLib.DivineKnowledge) || player.hasPerk(PerkLib.PrestigeJobGreySage)) && divineSpellButtons.length > 0)
        {
            buttons.add("Divine", curry(submenu, divineSpellButtons, submenuSpells, 0, false)).hint("Open your Divine tome");
        }
        if ((player.hasPerk(PerkLib.PrestigeJobNecromancer) || player.hasPerk(PerkLib.PrestigeJobGreySage)) && necroSpellButtons.length > 0)
        {
            buttons.add("Necro Spells", curry(submenu, necroSpellButtons, submenuSpells, 0, false)).hint("Open your Necromicon");
        }
        if ((player.hasPerk(PerkLib.HiddenJobBloodDemon) || player.hasPerk(PerkLib.PrestigeJobGreySage)) && bloodSpellButtons.length > 0)
        {
            buttons.add("Blood Spells", curry(submenu, bloodSpellButtons, submenuSpells, 0, false)).hint("Open your Blood grimoire");
        }
        if (greenSpellButtons.length > 0)
        {
            buttons.add("Green Spells", curry(submenu, greenSpellButtons, submenuSpells, 0, false)).hint("Open your Green magic book");
        }
    }
    
    private function buildAbilityMenu(abilities : Array<Dynamic>, buttons : ButtonDataList) : Void
    {
    ...
    private function mainMenuWhenBound() : Void
    {
        menu();
        ...
        if (player.hasPerk(PerkLib.Spectre) && player.hasPerk(PerkLib.Incorporeality))
        {
            ...
            {
                addButtonDisabled(3, "Possess", "<b>You need more time before you can use Possess again.</b>");
            }
            else
            {
                addButton(3, "Possess", combat.mspecials.possess2);
            }
        }
        ...
        if (player.hasStatusEffect(StatusEffects.LethicesRapeTentacles) && player.statusEffectv3(StatusEffects.LethicesRapeTentacles) == 1)
        {
            ...
            addButton(0, "Struggle", (try cast(monster, Lethice) catch(e:Dynamic) null).grappleStruggle);
            addButton(1, "Wait", (try cast(monster, Lethice) catch(e:Dynamic) null).grappleWait);
            
            var whitefireLustCap : Int = as3hx.Compat.parseInt(player.maxLust() * 0.75);
            if (player.hasPerk(PerkLib.Enlightened) && player.cor < (10 + player.corruptionTolerance))
            {
                whitefireLustCap += as3hx.Compat.parseInt(player.maxLust() * 0.1);
            }
            ...
            {
                whitefireLustCap = as3hx.Compat.parseInt(player.maxLust() - 45);
            }
            if (player.hasPerk(PerkLib.GreyMage) && player.hasPerk(PerkLib.Enlightened) && player.cor < (10 + player.corruptionTolerance))
            {
                whitefireLustCap = as3hx.Compat.parseInt(player.maxLust() - 15);
            }
            var gotEnergy : Bool = !player.hasPerk(PerkLib.BloodMage) && player.mana >= 30;
            if (player.lust < whitefireLustCap && player.hasStatusEffect(StatusEffects.KnowsWhitefire) && gotEnergy)
            {
                addButton(2, "Dispell", (try cast(monster, Lethice) catch(e:Dynamic) null).dispellRapetacles);
            }
        }
    }
    
    @:allow(classes.scenes.combat)
    private function mainMenuWhenChanneling() : Void
    {
        menu();
    ...
    @:allow(classes.scenes.combat)
    private function submenuMagSpecials() : Void
    {
        ...
        {
            clearOutput();
            if (player.statusEffectv2(StatusEffects.Sealed) == 6)
            {
            ...
            {
                outputText("You try to use a magical ability but you are currently silenced by the alraune vines!\n\n");
            }
            enemyAI();
            return;
        }
    ...
    @:allow(classes.scenes.combat)
    private function submenuPhySpecials() : Void
    {
    ...
    @:allow(classes.scenes.combat)
    private function submenuSpells() : Void
    {
        ...
        {
            clearOutput();
            if (player.statusEffectv2(StatusEffects.Sealed) == 2)
            {
            ...
            if (player.statusEffectv2(StatusEffects.Sealed) == 10)
            {
                outputText("You try to use magic but you are currently silenced by the alraune vines!\n\n");
            }
            enemyAI();
            return;
        }
    ...
    @:allow(classes.scenes.combat)
    private function submenuSoulforce() : Void
    //if (inCombat && player.hasStatusEffect(StatusEffects.Sealed) && player.statusEffectv2(StatusEffects.Sealed) == 5) {
    {
    ...
    @:allow(classes.scenes.combat)
    private function submenuOther() : Void
    {
