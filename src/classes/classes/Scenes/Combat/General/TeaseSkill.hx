...
class TeaseSkill extends AbstractGeneral
{
    ...
    public function calcBaseHitChance() : Float
    {
        var chance : Float = 90;
    ...
    {
        var lustDmg : Float = combat.teases.teaseBaseLustDamage();
        if (player.hasPerk(PerkLib.BroadSelection) && player.differentTypesOfCocks() > 1)
        {
            lustDmg *= (1 + (0.25 * player.differentTypesOfCocks()));
        }
    ...
    override public function revenge(display : Bool = true) : Void
    {
        ...
        var baseChance : Float = calcBaseHitChance();
        
        var teaseChoiceArray : Array<Dynamic> = combat.teases.genTeaseChoiceArray();
        ...
        if (rand(100) <= baseChance + rand(teaseOpt.bonusChance))
        {
            ...
            var damageMod : Float = teaseOpt.damage + rand(teaseOpt.bonusDamage);
            if (player.perkv1(IMutationsLib.FiendishBallsIM) >= 4 && (selectedTease == 3 || selectedTease == 10 || selectedTease == 11 || selectedTease == 23 || selectedTease == 33 || selectedTease == 35))
            {
                lustDmg *= 4;
            }
            ...
            if (!SceneLib.urtaQuest.isUrta())
            {
                player.SexXP(1 + combat.teases.bonusExpAfterSuccesfullTease());
            }
        }
        else
        {
