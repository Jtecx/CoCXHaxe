...
class MummyAttackSkill extends AbstractGeneral
{
    ...
    public function calcDamage(monster : Monster) : Float
    {
        ...
        {
            mummyamplification *= combat.historyTacticianBonus();
        }
        var mummyamplification : Float = 1;
        if (player.weapon == weapons.SCECOMM)
        {
            mummyamplification += 0.5;
        }
        ...
        if (player.perkv2(PerkLib.MummyLord) > 0)
        {
            mummyamplification *= 2;
        }
