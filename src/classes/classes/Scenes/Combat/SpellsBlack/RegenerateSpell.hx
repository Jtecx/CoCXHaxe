...
class RegenerateSpell extends AbstractBlackSpell
{
    public function new()
    {
        super(
    ...
    public function calcHeal() : Float
    {
        var hpChange2 : Float = player.inte;
        if (player.hasPerk(PerkLib.WisenedHealer))
        {
            hpChange2 += player.wis;
        }
        hpChange2 *= healModBlack();
        if (player.isRace(Races.UNICORN, 2))
        {
            hpChange2 *= 3;
        }
        if (player.isRace(Races.ALICORN))
        {
            hpChange2 *= 4;
        }
