...
class CombatTeases extends BaseCombatContent
{
    public function new()
    {
        super();
    ...
    public function teaseBaseLustDamage(tBLD : Float = 0) : Float
    {
        tBLD += player.lib;
        ...
        if (player.hasPerk(PerkLib.GracefulBeauty))
        {
            tBLD += scalingBonusSpeed() * 0.1;
        }
        if (player.isElf() && player.hasPerk(PerkLib.ELFElvenSpearDancingTechnique) && player.isSpearTypeWeapon())
        {
            tBLD += scalingBonusSpeed() * 0.1;
        }
        ...
        {
            damagemultiplier += player.lust100 * 0.01;
        }
        if (player.hasPerk(PerkLib.HistoryWhore) || player.hasPerk(PerkLib.PastLifeWhore))
        {
            damagemultiplier += combat.historyWhoreBonus();
        }
        if (player.hasPerk(PerkLib.DazzlingDisplay))
        {
        ...
        if (player.armorName == "desert naga pink and black silk dress")
        {
        ...
        if (player.hasPerk(PerkLib.Nightshade))
        {
            damagemultiplier += 0.5;
        }
        if (player.hasPerk(PerkLib.SluttySimplicity) && player.armor.hasTag(ItemConstants.A_REVEALING))
        {
            tBLD *= (1 + ((10 + rand(11)) / 100));
        }
        if (player.weapon == weapons.HELLCAL)
        {
            damagemultiplier *= (1 + (0.01 * player.cor));
        }
        if (player.weapon == weapons.ELYSIUM)
        {
            damagemultiplier *= (1 + (0.01 * (100 - player.cor)));
        }
        if (player.hasStatusEffect(StatusEffects.DemonEnergyThirstFeed))
        {
            var inotcareanymore : Float = 0.05;
        ...
        {
            if (player.perkv1(IMutationsLib.FiendishOvariesIM) >= 4)
            {
                damagemultiplier += 0.5;
            }
            else
        ...
        {
            tBLD *= UmasShop.NEEDLEWORK_LUST_TEASE_DAMAGE_MULTI;
        }
        ...
        for (f/* AS3HX WARNING could not determine type for var: f exp: ECall(EField(EIdent(player),allItemEffects),[EField(EIdent(IELib),Tease_RaceX2)]) type: null */ in player.allItemEffects(IELib.Tease_RaceX2))
        {
            tBLD *= 1 + f.power * ((player.isRaceCached(try cast(f.value1, Race) catch(e:Dynamic) null)) ? 2 : 1) / 100;
        }
    ...
    public function fueledByDesireDamageBonus(damage : Float) : Float
    {
    ...
    public function fueledByDesireHeal(display : Bool = true) : Bool
    {
        if (player.hasPerk(PerkLib.FueledByDesire) && player.lust100 >= 50 && flags[kFLAGS.COMBAT_TEASE_HEALING] == 0)
        {
            if (display)
    ...
    public function masteryBonusDamageTease() : Float
    {
        var lustMult : Float = 1;
        var multiplier : Float = 0.02;
    ...
    public function combatTeaseCritical() : Float
    {
        var critTChance : Float = 0;
        if (player.hasPerk(PerkLib.ElvenSense) && player.inte >= 50)
        {
            critTChance += 5;
        }
        if (player.armor == armors.R_CHANG || player.armor == armors.R_QIPAO || player.armor == armors.G_CHANG || player.armor == armors.G_QIPAO || player.armor == armors.B_CHANG || player.armor == armors.B_QIPAO || player.armor == armors.P_CHANG || player.armor == armors.P_QIPAO)
        {
            critTChance += 5;
        }
        
        if (player.headJewelry == headjewelries.SCANGOG)
        {
            critTChance += 5;
        }
        if (player.headJewelry == headjewelries.SATGOG)
        {
            critTChance += 10;
        }
        ...
        if (player.eyesOfTheHunterAdeptBoost())
        {
            ...
            if (player.hasPerk(PerkLib.EyesOfTheHunterSu) && player.sens >= 30)
            {
                ...
                {
                    critTChance += 2 * Math.round((player.sens - 25) / 5);
                }
            }
        }
        ...
        {
            if (player.hasPerk(PerkLib.EyesOfTheHunterMaster) && player.sens >= 100)
            {
                critTChance += 5;
            }
            if (player.hasPerk(PerkLib.EyesOfTheHunterSu) && player.sens >= 30)
        ...
        {
            if (player.hasPerk(PerkLib.EyesOfTheHunterGrandMaster) && player.sens >= 125)
            {
                critTChance += 5;
            }
            if (player.hasPerk(PerkLib.EyesOfTheHunterSu) && player.sens >= 30)
    ...
    public function teaseAuraLustDamageBonus(monster : Monster, lustDmg : Float) : Float
    {
        if (player.hasPerk(PerkLib.SensualLover))
        {
            lustDmg *= 1.1;
        ...
        if (player.hasPerk(PerkLib.JobCourtesan) && monster != null && monster.hasPerk(PerkLib.EnemyBossType))
        {
            lustDmg *= 1.2;
        }
        if (player.hasPerk(PerkLib.SluttySimplicity) && player.armor.hasTag(ItemConstants.A_REVEALING))
        {
            lustDmg *= (1 + ((10 + rand(11)) / 100));
        }
        if (player.hasPerk(PerkLib.ElectrifiedDesire))
        {
            lustDmg *= (1 + (player.lust100 * 0.01));
        }
        if (player.hasPerk(PerkLib.HistoryWhore) || player.hasPerk(PerkLib.PastLifeWhore))
        {
            lustDmg *= (1 + combat.historyWhoreBonus());
        }
        if (player.hasPerk(PerkLib.ArouseTheAudience) && monster != null && (monster.hasPerk(PerkLib.EnemyGroupType) || monster.hasPerk(PerkLib.EnemyLargeGroupType)))
        {
            lustDmg *= 1.2;
        }
        if (player.hasPerk(PerkLib.ChiReflowLust))
        {
            lustDmg *= UmasShop.NEEDLEWORK_LUST_TEASE_DAMAGE_MULTI;
        }
        if (player.armor == armors.ELFDRES && player.isElf())
        {
            lustDmg *= 2;
    ...
    {
        var teaseChoiceArray : Array<Dynamic> = genTeaseChoiceArray();
    ...
    //==============================
    public function genTeaseChoiceArray() : Array<Dynamic>
    {
        var choices : Array<Dynamic> = [];
        var futa : Bool = player.hasPerk(PerkLib.FutaForm);
        var bimbo : Bool = player.hasPerk(PerkLib.BimboBody);
        var bro : Bool = player.hasPerk(PerkLib.BroBody);
        ...
        //Once chance of butt.{
            ...
            if (player.biggestTitSize() >= 75)
            {
                choices[choices.length] = 5;
            }
            if (player.biggestTitSize() >= 100)
            {
                choices[choices.length] = 5;
            }
            //Pussy Flash!
        ...
        if (player.hasVisiblePregnancy())
        {
            ...
            if ((player.pregnancyIncubation <= sceneHunter.adjustPregEventTimer(180, player.pregnancyType)) || (player.pregnancy2Incubation <= sceneHunter.adjustPregEventTimer(180, player.pregnancy2Type)))
            {
                choices[choices.length] = 13;
            }
            if ((player.pregnancyIncubation <= sceneHunter.adjustPregEventTimer(120, player.pregnancyType)) || (player.pregnancy2Incubation <= sceneHunter.adjustPregEventTimer(120, player.pregnancy2Type)))
            {
                choices[choices.length] = 13;
            }
        ...
        if (player.hasPerk(PerkLib.Feeder) && player.biggestTitSize() >= 4)
        {
            ...
            if (player.biggestTitSize() >= 40)
            {
                choices[choices.length] = 27;
            }
            if (player.biggestTitSize() >= 60)
            {
                choices[choices.length] = 27;
            }
            if (player.biggestTitSize() >= 80)
            {
                choices[choices.length] = 27;
            }
        }
    ...
    public function applySelectedTease(select : Int, display : Bool = true) : TeaseOptions
    {
        ...
        switch (select)
        {
            ...
            case 0:
                ...
                //Display{
                    ...
                    if (player.butt.type >= 10 && player.tone < 60)
                    {
            ...
            case 1:
                ...
                if (player.breastRows.length == 1)
                {
                    ...
                    //50+ breastsize% success rate{
                        ...
                        {
                            outputText("  Your " + nippleDescript(0) + "s seem to demand [monster his] attention.");
                        }
                    }
                }
                //Multirow
                if (player.breastRows.length > 1)
                {
                    ...
                    //50 + 10% per breastRow + breastSize%{
                        ...
                        if (player.lust >= (player.maxLust() * 0.5))
                        {
                            outputText(", your " + nippleDescript(0) + "s painfully visible.");
                        }
                        else
                        {
                            outputText(".");
            ...
            case 2:
                ...
                else
                {
                    ...
                    if (player.cocks.length > 0)
                    {
                        options.chance += 3;
                        options.damage += 3;
                        if (display && player.cocks.length == 1)
                        {
            ...
            case 5:
                ...
                if (player.lust >= (player.maxLust() * 0.5))
                {
                    ...
                    {
                        outputText("  Your hard nipples seem to demand [monster his] attention.");
                    }
                    options.chance += 3;
                    options.damage += 6;
                }
            ...
            case 6:
                ...
                else
                {
                    ...
                    if (display && player.hasPerk(PerkLib.FutaForm))
                    {
                        outputText("futanari ");
                    }
                    else if (player.hasPerk(PerkLib.BimboBody) && display)
                    {
                        outputText("bimbo ");
                    }
                    else if (display)
                ...
                if (display)
                {
                    if (monster.plural)
                    {
                        outputText(monster.capitalA + monster.short + "' gazes are riveted on your groin as you run your fingers up and down your folds.");
                    }
                    else
                    {
                        outputText(monster.capitalA + monster.short + "'s gaze is riveted on your groin as you run your fingers up and down your folds.");
                    }
                    if (player.clitLength > 3)
                    {
                        outputText("  You smile as your " + clitDescript() + " swells out from the folds and stands proudly, begging to be touched.");
                    }
                    else
                    {
                        outputText("  You smile and pull apart your lower-lips to expose your " + clitDescript() + ", giving the perfect view.");
                    }
                    if (player.cockTotal() > 0)
                    {
                        outputText("  Meanwhile, [eachcock] bobs back and forth with your gyrating hips, adding to the display.");
                    }
                }
                //BONUSES!
                if (player.hasCock())
            ...
            case 8:
                ...
                if (player.hasPerk(PerkLib.BroBrains) && display)
                {
                    outputText("Damn, [themonster] has got to love this!");
                }
                else if (display)
                {
                    outputText(monster.capitalA + monster.short + " will probably enjoy the show, but you feel a bit silly doing this.");
                }
            ...
            case 9:
                ...
                if (player.hasPerk(PerkLib.BroBrains) && display)
                {
                    outputText(monster.capitalA + monster.short + " can't resist such a heroic pose!");
                }
                else if (display)
                {
                    outputText("At least the physical changes to your body are proving useful!");
                }
                options.chance += (player.tone - 75) / 2;
                options.damage += (player.tone - 70) / 2;
                options.auto = false;
            ...
            case 10:
                if (display)
                {
                    ...
                    {
                        outputText("No way could [monster he] resist your huge cock!");
                    }
                    else
                    {
                        outputText("This is so crude, but at the same time, you know it'll likely be effective.");
                    }
            ...
            case 11:
                if (display)
                {
                    if (silly() && rand(2) == 0)
                    {
                        outputText("You strike a herculean pose and flex, whispering, \"<i>Do you even lift?</i>\" to [themonster].");
                    }
                    else
                    {
                        ...
                        if (player.hasPerk(PerkLib.BroBrains))
                        {
                            outputText("Bitches love a cum-leaking cock.");
                        }
                        else
                        {
                            outputText("You've got to admit, you look pretty good down there.");
                        }
                    }
                }
                if (player.hasPerk(PerkLib.BulgeArmor))
            ...
            case 15:
                ...
                {
                    outputText("You remove the front of your [armor] exposing your " + chestDesc() + ".  Using both of your hands, you thrust two fingers into your nipple cunts, milky girl cum soaking your hands and fingers.  \"<i>Wouldn't you like to try out these holes too?</i>\"");
                }
                options.breasts = true;
            //16 Anal gape
            case 16:
            ...
            case 18:
                if (display)
                {
                    ...
                    if (player.hasVagina())
                    {
                        outputText("parted cunt-lips");
                    }
                    else
                    {
                        outputText("puckered asshole, hanging, erect maleness,");
                    }
            ...
            case 40:
                if (display)
                {
                    ...
                    if (player.tailCount == 1)
                    {
                        outputText("  Your tail twists and whips about, sliding around your " + hipDescript() + " in a slow arc and framing your rear nicely as you slowly lift your [armor].");
                    }
                    else
                    {
                        outputText("  Your tails fan out, twisting and whipping sensually, sliding up and down your [legs] and framing your rear nicely as you slowly lift your [armor].");
                    }
            ...
            case 44:
                ...
                else if (maiden == 2)
                {
                    if (display)
                    {
                        ...
                        {
                            outputText("Think you can handle a virgin's infinite lust?");
                        }
                        else
                        {
                            outputText("Think you have what it takes to satisfy this perfect pussy?");
                        }
                        outputText("</i>\"");
                    }
                    options.vagina = true;
            ...
            case 45:
                var partChooser : Array<Dynamic> = [];  //Array for choosing.  
                //Choose part. Must not be a centaur for cock and vagina teases!
                partChooser[partChooser.length] = 0;
                if (player.gender == 1 && !player.isTaur())
                {
                    partChooser[partChooser.length] = 1;
                }
                if (player.gender == 2 && !player.isTaur())
                {
                    partChooser[partChooser.length] = 2;
                }
                if (player.gender == 3 && player.hasVagina() && !player.isTaur())
                {
                    partChooser[partChooser.length] = 3;
                }
                //Let's do this!
                var _sw16_ = (partChooser[rand(partChooser.length)]);                
    ...
    public function calcBonusTeaseMod(options : TeaseOptions, display : Bool = true) : TeaseOptions
    {
        ...
        var bonusChance : Float = options.bonusChance;
        var bonusDamage : Float = options.bonusDamage;
    ...
    public function bonusExpAfterSuccesfullTease() : Float
    {
        var sucessBonusTeaseExp : Float = 1;
        if (player.hasPerk(PerkLib.SuperSensual))
        {
            sucessBonusTeaseExp += 2;
        }
        if (player.hasPerk(PerkLib.Sensual))
        {
            sucessBonusTeaseExp += 1;
        }
