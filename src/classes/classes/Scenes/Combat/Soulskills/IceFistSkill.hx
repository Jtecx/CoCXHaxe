...
class IceFistSkill extends AbstractSoulSkill
{
    public function new()
    {
        super(
    ...
    public function calcDamage(monster : Monster) : Float
    {
        ...
        //other bonuses
        if (player.hasPerk(PerkLib.PerfectStrike) && monster != null && monster.monsterIsStunned())
        {
            damage *= 1.5;
        }
    ...
    override public function doEffect(display : Bool = true) : Void
    {
        ...
        monster.buff("FrozenSolid").addStats({
                                    spe : -20
                                }).withText("Frozen Solid").combatTemporary(1);
        if (display)
        {
        ...
        else if (monster.hasStatusEffect(StatusEffects.FrozenSolid))
        {
            monster.addStatusValue(StatusEffects.FrozenSolid, 1, 2);
        }
        else
        {
            monster.createStatusEffect(StatusEffects.FrozenSolid, 2, 0, 0, 0);
        }
        //speed debuff
        if (monster.buff("FrozenSolid").isPresent())
        {
            monster.buff("FrozenSolid").addStats({
                                spe : -20
                            }).addDuration(2);
        ...
        else
        {
            monster.buff("FrozenSolid").addStats({
                                        spe : -20
                                    }).withText("Frozen Solid").combatTemporary(2);
        }
