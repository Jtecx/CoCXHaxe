...
class BladeHailSkill extends AbstractSoulSkill
{
    ...
    private var hailArray : Array<Dynamic> = [
    ...
    public function new(count : Int)
    {
        ...
        hailSelection = as3hx.Compat.parseInt(count - 1);
    ...
    override public function calcCooldown() : Int
    {
        ...
        switch (hailSelection)
        {
            ...
            case 0:return soulskillCooldown(baseCooldown, false);
            default:return soulskillCooldown(baseCooldown, false);
        }
    }
    
    private function calcHailDamage() : Float
    {
        var damage : Float = player.wis;
        ...
        //other bonuses
        if (player.hasPerk(PerkLib.Heroism) && (monster && (monster.hasPerk(PerkLib.EnemyBossType) || monster.hasPerk(PerkLib.EnemyHugeType))))
        {
            damage *= 2;
        }
    ...
    private function fireHail(hits : Int = 1, display : Bool = true) : Void
    {
        var damage : Float = calcHailDamage();
    ...
    override public function doEffect(display : Bool = true) : Void
    {
        if (display)
        {
            outputText("Letting soulforce leak out around you, you form " + hailArray[hailSelection][1] + " ethereal two meter long weapons in four rows. You thrust your hand outwards and in the blink of an eye, weapons shoot forwards [themonster].  ");
        }
        if (monsterDodgeSkill("weapons", display))
        {
            return;
        }
        if (display)
        ...
        var rounds : Float = hailArray[hailSelection][5];
        while (rounds-- > 0)
        {
            fireHail(hailArray[hailSelection][6], display);
        }
        if (display)
        {
            outputText(" damage!\n\n");
