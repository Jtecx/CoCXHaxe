...
class TranceSkill extends AbstractSoulSkill
{
    private static var TranceABC : Dynamic = FnHelpers.FN.buildLogScaleABC(10, 100, 1000, 10, 100);
    public function new()
    {
        super(
                "Trance", 
    ...
    override public function doEffect(display : Bool = true) : Void
    {
        ...
        if (player.hasPerk(PerkLib.FocusedMind) && player.inte >= 50)
        {
            TranceBoost += 10;
        }
        ...
        if (player.hasPerk(PerkLib.Archmage) && player.inte >= 75)
        {
            TranceBoost += 15;
        }
        if (player.hasPerk(PerkLib.GrandArchmage) && player.inte >= 100)
        {
            TranceBoost += 20;
        }
        ...
        if (player.hasPerk(PerkLib.JobEnchanter) && player.inte >= 50)
        {
            TranceBoost += 5;
        }
        if (player.hasPerk(PerkLib.Battleflash) && player.inte >= 50)
        {
            TranceBoost += 15;
        }
        if (player.hasPerk(PerkLib.JobSwordsman))
        {
            TranceBoost -= 10;
        }
        ...
        if (player.hasPerk(PerkLib.JobDervish))
        {
            TranceBoost -= 10;
        }
        ...
        if (player.hasPerk(PerkLib.HeavyArmorProficiency))
        {
            TranceBoost -= 15;
        }
        if (player.hasPerk(PerkLib.AyoArmorProficiency))
        {
            TranceBoost -= 20;
        }
        if (player.hasPerk(PerkLib.Agility))
        {
            TranceBoost -= 10;
        }
        if (player.hasPerk(PerkLib.LightningStrikes))
        {
            TranceBoost -= 10;
        }
        if (player.hasPerk(PerkLib.StarlightStrikes))
        {
            TranceBoost -= 10;
        }
        ...
        player.buff("TranceTransformation").addStats({
                                    str : TranceBoost,
                                    tou : TranceBoost
                                }).withText("Trance Transformation").combatPermanent();
    }
}
