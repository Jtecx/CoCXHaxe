...
class ManyBirdsSkill extends AbstractSoulSkill
{
    public function new()
    {
        super(
    ...
    override public function doEffect(display : Bool = true) : Void
    {
        if (silly())
        {
            if (display)
            {
                outputText("You focus your soulforce, projecting it as an aura around you.  As you concentrate, dozens, hundreds, thousands of tiny, ethereal birds shimmer into existence.  As you raise your hand up, more and more appear, until the area around you and [themonster]  is drowned in spectral flappy shapes.  ");
            }
            else
            {
                ...
                if (monsterDodgeSkill("crystal storm", display))
                {
                    return;
                }
            }
        ...
        else
        {
            if (display)
            {
                outputText("The crystals hit [themonster], dealing ");
            }
            doMagicDamage(damage, true, display);
            if (display)
            {
