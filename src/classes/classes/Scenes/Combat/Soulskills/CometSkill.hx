...
class CometSkill extends AbstractSoulSkill
{
    public function new()
    {
        super(
    ...
    {
        var damage : Float = scalingBonusWisdom() * 6;
        if (damage < 10)
        {
            damage = 10;
        }
    ...
    override public function doEffect(display : Bool = true) : Void
    {
        if (display)
        {
            ...
            if (monster.plural)
            {
                outputText("Shattering into thousands of fragments that shower anything and everything around you.  ");
            }
        }
        if (monsterDodgeSkill("comet fragments", display))
        {
            return;
        }
