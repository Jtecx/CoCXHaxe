...
class BloodReqiuemSkill extends AbstractBloodSoulSkill
{
    public function new(sfInfusion : Bool = false)
    {
        super(
                "Blood Requiem", 
                (sfInfusion) ? "(Soulforce infused) Blood Requiem, infused by a small amount of soulforce to enhance its power, will create pillars of blood that would deal damage and reduce the recovery rate of enemies for a short time.  " : "Blood Requiem will create pillars of blood, dealing damage and reducing the recovery rate of enemies for a short time.  ", 
                TARGET_ENEMY, 
                TIMING_INSTANT, 
    ...
    override public function doEffect(display : Bool = true) : Void
    {
        ...
        if (display)
        {
            outputText("You concentrate, focusing on the power of your blood before starting making gestures with your hands, precise movements sending power blazing through your body. " +
                    additionalSFLine +
        ...
        if (monster.hasStatusEffect(StatusEffects.BloodRequiem))
        {
            monster.addStatusValue(StatusEffects.BloodRequiem, 1, 4);
        }
        else
        {
            monster.createStatusEffect(StatusEffects.BloodRequiem, 4, 0, 0, 0);
        }
