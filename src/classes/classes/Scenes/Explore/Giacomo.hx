...
class Giacomo extends BaseContent implements TimeAwareInterface
{
    ...
    public function timeChange() : Bool
    {
        ...
        {
            if (flags[kFLAGS.CERULEAN_POTION_BAD_END_FUTA_COUNTER] > 0)
            {
                flags[kFLAGS.CERULEAN_POTION_BAD_END_FUTA_COUNTER] -= 0.5;
            }
        }
        return false;
    }
    ...
    {
        if (checkedSuccubi++ == 0 && camp.IsSleeping && player.hasStatusEffect(StatusEffects.SuccubiNight) && (player.hasCock() || player.gender == 0))
        
        //Call secksins!{
            
            if (player.hasStatusEffect(StatusEffects.RepeatSuccubi))
            {
                if (SceneLib.vapula.vapulaSlave() && player.hasCock() && flags[kFLAGS.VAPULA_THREESOMES] > 0 && flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 0)
    ...
    public function giacomoEncounter() : Void
    {
        ...
        else if (!player.hasStatusEffect(StatusEffects.WormOffer) && player.hasStatusEffect(StatusEffects.Infested))
        ...
        if (player.hasStatusEffect(StatusEffects.WormOffer) && player.hasStatusEffect(StatusEffects.Infested))
        {
            addButton(5, "Worm Cure", wormRemovalOffer);
        }
    ...
    private function potionMenu() : Void
    {
        ...
        {
            addButton(3, "Cerulean P.", pitchCeruleanPotion);
        }
        addButton(11, consumables.SAPILL_.shortName, itemBuy2, consumables.SAPILL_);
        if (player.level >= 24)
        {
            addButton(12, consumables.MAPILL_.shortName, itemBuy2, consumables.MAPILL_);
        }
        else
        {
            addButtonDisabled(12, "???", "Req. lvl 24+");
        }
        if (player.level >= 42)
        {
            addButton(13, consumables.BAPILL_.shortName, itemBuy2, consumables.BAPILL_);
        }
        else
        {
            addButtonDisabled(13, "???", "Req. lvl 42+");
        }
    ...
    private function eroticaMenu() : Void
    {
        ...
        if (player.hasVagina())
        {
            addButton(1, "Stim-Belt", pitchSelfStimulationBelt);
        }
        if (player.hasVagina())
        {
            addButton(2, "AN Stim-Belt", pitchAllNaturalSelfStimulationBelt);
        }
        if (player.hasCock())
        {
            addButton(3, "Onahole", pitchOnahole);
        }
        if (player.hasCock())
        {
            addButton(4, "D Onahole", pitchDeluxeOnahole);
        }
        if (player.hasCock() && player.hasVagina())
        {
            addButton(5, "Dual Belt", pitchDualStimulationBelt);
        }
        if (player.hasCock() && player.hasVagina())
        {
            addButton(6, "AN Onahole", pitchAllNaturalOnahole);
        }
    ...
    private function miscMenu() : Void
    {
        ...
        clearOutput();
        menu();
        if (Garden.IngrediantBagSlot01Cap == 0)
        {
            addButton(6, "Herb Bag (LLowG)", pitchLLHerbsBag).hint("Herbs Bag (Lowest Grade)");
        }
        if (Garden.IngrediantBagSlot07Cap == 0)
        {
            if (player.farmingLevel >= 5)
            {
                addButton(7, "Herb Bag (LowG)", pitchLHerbsBag).hint("Herbs Bag (Low Grade)");
            }
            else
            {
                addButtonDisabled(7, "Herb Bag (LowG)", "Herbs Bag (Low Grade) Req. lvl 5 in Farming.");
            }
        }
        if (Garden.PotionsBagSlot01Cap == 0)
        {
            addButton(8, "Pot Bag (LowG)", pitchPotionsBag).hint("Potions Bag (Lowest Grade)");
        }
        if (player.hasKeyItem("Tarnished Ore Bag (Lowest grade)") >= 0)
        {
            addButton(10, "Ore Bag (LowG)", pitchOreBag).hint("Ore Bag (Lowest Grade)");
        }
        if (Holidays.nieveHoliday())
        {
            if (flags[kFLAGS.CHRISTMAS_TREE_LEVEL] == 0)
            {
                addButton(11, "Mysterious Seed", pitchMysteriousSeed);
            }
            if (flags[kFLAGS.CHRISTMAS_TREE_LEVEL] == 5)
            {
                addButton(12, "Decorations", pitchDecorations);
            }
        }
    ...
    private function buyManUpBeer() : Void
    {
        ...
        {
            clearOutput();
            outputText("\n\nGiacomo sighs, indicating you need " + Std.string(15 - player.gems) + " more gems to purchase this item.");
            doNext(potionMenu);
        }
        else
        {
    ...
    private function buyCeruleanPotion() : Void
    {
        ...
        {
            clearOutput();
            outputText("\n\nGiacomo sighs, indicating you need " + Std.string(75 - player.gems) + " more gems to purchase this item.");
            doNext(potionMenu);
        }
        else
        {
    ...
    public function buyCondom() : Void
    {
        ...
        {
            clearOutput();
            outputText("\n\nGiacomo sighs, indicating you need " + Std.string(10 - player.gems) + " more gems to purchase this item.");
            doNext(eroticaMenu);
        }
        else
        {
    ...
    public function buySilverCrossNecklace() : Void
    {
        ...
        {
            clearOutput();
            outputText("\n\nGiacomo sighs, indicating you need " + Std.string(400 - player.gems) + " more gems to purchase this item.");
            doNext(eroticaMenu);
        }
        else
        {
    ...
    private function buyDangerousPlantsBook() : Void
    {
        ...
        if (player.gems < 10)
        {
            outputText("Giacomo sighs, indicating you need " + Std.string(10 - player.gems) + " more gems to purchase this item.");
            doNext(bookMenu);
        }
        else
        {
    ...
    private function buyTelAdreMagazineIssue2() : Void
    {
        ...
        if (player.gems < 100)
        {
            outputText("Giacomo sighs, indicating you need " + Std.string(100 - player.gems) + " more gems to purchase this item.");
            doNext(bookMenu);
        }
        else
        {
    ...
    private function buyDildo() : Void
    {
        ...
        if (player.gems < 20)
        {
            outputText("\n\nGiacomo sighs, indicating you need " + Std.string(20 - player.gems) + " more gems to purchase this item.");
            doNext(eroticaMenu);
        }
        else
        {
    ...
    private function buySelfStimulationBelt() : Void
    {
        ...
        if (player.gems < 30)
        {
            outputText("\n\nGiacomo sighs, indicating you need " + Std.string(30 - player.gems) + " more gems to purchase this item.");
            doNext(eroticaMenu);
            return;
        }
    ...
    private function buyAllNaturalSelfStimulationBelt() : Void
    {
        ...
        if (player.gems < 40)
        {
            outputText("\n\nGiacomo sighs, indicating you need " + Std.string(40 - player.gems) + " more gems to purchase this item.");
            doNext(eroticaMenu);
            return;
        }
    ...
    private function buyDeluxeOnahole() : Void
    {
        ...
        if (player.gems < 50)
        {
            outputText("\n\nGiacomo sighs, indicating you need " + Std.string(50 - player.gems) + " more gems to purchase this item.");
            doNext(eroticaMenu);
            return;
        }
    ...
    private function buyAllNaturalOnahole() : Void
    {
        ...
        if (player.gems < 150)
        {
            outputText("\n\nGiacomo sighs, indicating you need " + Std.string(150 - player.gems) + " more gems to purchase this item.");
            doNext(eroticaMenu);
            return;
        }
    ...
    private function wormRemoval() : Void
    {
        ...
        if (player.HP > as3hx.Compat.parseInt(player.maxHP() * .15))
        {
            player.HP = as3hx.Compat.parseInt(player.maxHP() * .15);
        }
    ...
    private function nightSuccubiFirstTime() : Void
    {
        ...
        var sharedEnd : Bool->Void = function(herm : Bool) : Void
        {
    ...
    private function ceruleanSuccubusEncounterPart2(herm : Bool) : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_cerulean_succubus);
        if (!herm)
    ...
    private function ceruleanSuccubusEncounterPart3(herm : Bool) : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_cerulean_succubus);
        if (!herm)
    ...
    private function ceruleanSuccubusEncounterPart4(herm : Bool) : Void
    {
        spriteSelect(SpriteDb.s_cerulean_succubus);
        if (!herm)
        {
    ...
    private function nightSuccubiRepeat() : Void
    {
        ...
        var dickF : Void->Void = function() : Void
        {
            ...
            function pureF() : Void
            {
                ...
                if (player.isTaur())
                {
                    outputText(" crouch between your legs and impale herself on your [cock] with a wet sound caused by her well-lubricated vulva. Y");
                }
                else
                {
                    outputText(" open her womb and quickly consume your [cock]. She embraces you, entrapping your head in her cleavage as y");
                }
                ...
                if (player.isTaur())
                {
                    outputText("Pushing on your forelegs, she engulfs even more of your " + cockDescript(0));
                }
                else
                {
                    outputText("Almost crushing your pelvis, she wraps her legs around your body");
                }
                ...
                if (player.isTaur())
                {
                    outputText("arches under your belly as you feel your [cock] bending pleasurably inside her, and");
                }
                else
                {
                    outputText("releases you from her grip, allowing you to finally breathe deeply, and leans back, arching high to reveal your joined genitals in the moonlight. You visibly see");
                }
            ...
            var corF : Void->Void = function() : Void
            {
                ...
                if (player.isTaur())
                {
                    outputText("pushes on your forelegs, impaling herself even deeper on your " + cockDescript(0));
                }
                else
                {
                    outputText("wraps her legs around your hips and bears down");
                }
                ...
                if (player.isTaur())
                {
                    outputText("She moans inhumanly, and reflexively digs her claws into your forelegs. Searing with lust, the pain means little to you as you only feel the sensation of your body forcing your fluids out of your body and into hers. You press your [cock] into her");
                }
                else
                {
                    outputText("She embraces you, moaning inhumanly, and reflexively digs her claws into your back. Searing with lust, the pain means little to you as you only feel the sensation of your body forcing your fluids out of your body and into hers. You slam your pelvis into hers");
                }
        ...
        //Bad End-Cerulean Succubus Futa/herm
        {
            ...
            if (flags[kFLAGS.CERULEAN_POTION_BAD_END_FUTA_COUNTER] > 10 && player.cor > 50 && !player.hasPerk(PerkLib.Soulless))
            {
                ...
                if (player.hasBalls())
                {
                    outputText("  Even your nuts are unbearably sore.");
                }
                ...
                if (player.racialScore(Races.HORSE, false) + player.racialScore(Races.DOG, false) + player.racialScore(Races.NAGA, false) + player.racialScore(Races.GOBLIN, false) + player.racialScore(Races.SHARK, false) + player.racialScore(Races.MINOTAUR, false) + player.racialScore(Races.COW, false) > 5)
                {
                    outputText("  All of the other corruptions and changes to your body have faded away as your new form has taken shape.");
                }
