...
class ElementalGolems extends AbstractGargoyle
{
    private function concussiveBlow() : Void
    {
        ...
        var damage : Float = (150 * ((inte / player.inte) / 4));
    ...
    //Knocks you away and forces you to spend a turn running back to do melee attacks.
    {
        ...
        var damage : Float = as3hx.Compat.parseInt((str + weaponAttack) - rand(player.touStat.core.value) - player.armorDef);
        //Dodge
        ...
        else
        {
            ...
            if (player.hasPerk(PerkLib.ShieldWard) && rand(2) == 0)
            {
                ...
                this.createStatusEffect(StatusEffects.KnockedBack, 0, 0, 0, 0);
            }
    ...
    //High damage, lowish accuracy.
    {
        ...
        var damage : Float = 175 + as3hx.Compat.parseInt((str + weaponAttack) - rand(player.touStat.core.value) - player.armorDef);
    ...
    private function disarm() : Void
    {
        ...
        //Avoid
        if (player.getEvasionRoll() || (player.hasPerk(PerkLib.ShieldWard) && rand(2) == 0))
        {
    ...
    //Difficult to avoid, moderate damage.
    {
        ...
        var damage : Float = (200 + as3hx.Compat.parseInt((str + weaponAttack) - rand(player.touStat.core.value) - player.armorDef)) / (rand(2) + 2);
        //Avoid
        if (damage <= 0 || player.getEvasionRoll())
        {
    ...
    override private function performCombatAction() : Void
    {
        if (player.statusEffectv1(StatusEffects.ElementalPearlGolems) == 3)
        {
            if (hasStatusEffect(StatusEffects.Provoke))
            {
                if (!this.hasStatusEffect(StatusEffects.KnockedBack))
                {
                    this.backhand();
                }
                else if (!this.hasStatusEffect(StatusEffects.Disarmed) && player.weaponName != "fists")
                {
                    this.disarm();
                }
                else
                {
                    var opts1 : Array<Dynamic> = [];
            ...
            else if (this.HPRatio() < 0.7 && !this.hasStatusEffect(StatusEffects.KnockedBack))
            {
                this.backhand();
            }
        ...
        else if (player.statusEffectv1(StatusEffects.ElementalPearlGolems) == 2)
        {
            ...
            else
            {
                ...
                if (choice1 == 1)
                {
                    ...
                    {
                        GargoyleStoneClawAttack();
                    }
                    else
                    {
                        moveGargoyleTailSlamAttack();
                    }
                }
                if (choice1 == 2)
                {
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        if (player.statusEffectv1(StatusEffects.ElementalPearlGolems) == 3)
        {
            SceneLib.exploration.elementalGolemBeaten3();
        }
        else if (player.statusEffectv1(StatusEffects.ElementalPearlGolems) == 2)
        {
            SceneLib.exploration.elementalGolemBeaten2();
        }
        else
        {
