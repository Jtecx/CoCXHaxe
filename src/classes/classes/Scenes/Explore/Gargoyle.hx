...
class Gargoyle extends BaseContent
{
    ...
    private function gargoyleConfidence(arg : Float = 0) : Float
    {
        if (arg != 0)
        {
            flags[kFLAGS.GAR_CONFIDENCE] += arg;
            if (flags[kFLAGS.GAR_CONFIDENCE] > 100)
            {
                flags[kFLAGS.GAR_CONFIDENCE] = 100;
            }
            else if (flags[kFLAGS.GAR_CONFIDENCE] < 0)
            {
                flags[kFLAGS.GAR_CONFIDENCE] = 0;
            }
        }
    ...
    public function gargoylesTheShowNowOnWBNetwork() : Void
    {
        ...
        if (SceneLib.desert.isDiscoveredOuter() > 0)
        {
            outputText(", making even the desert seem healthy and full of life");
        }
        ...
        {
            outputText("or the ruins of one of Tel'Adre's more distant neighbors, ");
        }
    ...
    private function gargoyleMeeting2() : Void
    {
        ...
        if (player.lib >= 40)
        {
            outputText("  Maybe you could take this with you as a life-sized sex toy?");
        }
    ...
    private function breakZeChains() : Void
    {
        ...
        {
            outputText("  \"<i>OW! Dammit, that hurt!</i>\"  What the hell were you thinking!?  Still, looks like you got results...");
        }
    ...
    public function returnToCathedral(woken : Bool = false) : Void
    {
        spriteSelect(SpriteDb.s_gargoyle);
        flags[kFLAGS.FOUND_CATHEDRAL] = 1;
        if (!woken)
        {
            ...
            if (rand(3) == 0)
            {
                outputText("You make your way back to the ruined Cathedral in the wastelands.  You notice some of the debris has been cleared away from the courtyard, and some of the windows have been boarded up.  " + flags[kFLAGS.GAR_NAME] + "'s obviously been working since you last left, trying to make the building more habitable.  With your path less obstructed than before, you enter.");
            }
            //[b]Player Returns to the Cathedral B[/b]
            else if (rand(2) == 0)
            {
                outputText("You spot the Cathedral in the distance, and see plumes of smoke rising. Alarmed, you ready your [weaponName] and rush over.  To your relief, however, you discover the burning objects are just a few dead imps that were looking for easy pillage, but found the Cathedral's newly-awoken protector instead.  Stepping around their charred corpses, you enter.");
            }
            //[b]Player Returns to the Cathedral C[/b]
            else
            {
                outputText("You wander into the Cathedral grounds, and are rather alarmed to see a pair of little green goblin sluts poking around outside, trying to cart off some of the stone tombstones for their nefarious schemes.  They see you approaching, however, and quickly flee – more interested in salvage than fighting or fucking today.  Chuckling to yourself, you enter.");
            }
        }
        menu();
        ...
        if (model.time.hours <= 9 || model.time.hours >= 18 || woken)
        {
            if (!woken)
            {
                ...
                {
                    outputText("early morning");
                }
                else
                {
                    outputText("late evening");
                }
                ...
                {
                    outputText("kneels before you, head bowed low.");
                }
                else if (gargoyleConfidence() < 99)
                {
                    outputText("bows low before you, a small smile on her stone lips.");
                }
                else
                {
                    outputText("with a sexy sway of her hips, approaches you and lays a small but tender kiss upon your cheek before bowing her head to you.");
                }
        ...
        else
        {
            if (!woken)
            {
                ...
                if (player.lust >= 33)
                {
                    outputText(", or you could use her to satisfy your own urges, not unlike a life-size sex toy");
                }
                outputText(".");
            }
            ...
            if (player.lust >= 33)
            {
                if (player.hasCock())
                {
                    addButton(1, "Use Her", useGargoyleMaleHerm);
                }
                else if (player.hasVagina())
                {
                    addButton(1, "Use Her", useHerGargoyleFemale);
                }
            }
            addButton(3, "Appearance", gargoyleAppearance);
        }
    ...
    private function gargoyleSexMenu() : Void
    {
        ...
        {
            outputText("  The gargoyle bows low before you, her face expressionless.  \"<i>By your command, Master.  How would you have me?</i>\"");
        }
        else
        {
            outputText("  Her ruby-red eyes lock with yours and a sultry grin plays across her gray marble features.  She slides an arm around your waist and presses her sizable, surprisingly soft breasts against your chest.  \"<i>How would Master have me?</i>\"");
        }
        menu();
    ...
    private function gargoyleAnal() : Void
    {
        ...
        else
        {
            ...
            if (player.hasBalls())
            {
                outputText(", your balls slapping against her butt");
            }
    ...
    private function gargoyleCoochiiGetsPlowed() : Void
    {
        ...
        {
            outputText("as far as she can take you.");
        }
        else
        {
            outputText("up to the hilt inside her.");
        }
        ...
        if (gargoyleConfidence() >= 70)
        {
            outputText("  Before you can stand, though, " + flags[kFLAGS.GAR_NAME] + " hugs her arms tightly around you pulling you down on top of her and mashing your face between her soft tits.");
        }
    ...
    private function titFuckTheGargoyle() : Void
    {
        ...
        if (gargoyleConfidence() >= 70)
        {
            outputText(", and to your surprise, so does " + flags[kFLAGS.GAR_NAME] + "");
        }
        outputText(".");
    ...
    private function strapOnGargoyle() : Void
    {
        ...
        if (!player.isTaur() && !player.isGoo() && !player.isDrider() && !player.isNaga())
        {
            outputText("you reveal your leg first, moving slowly and sensually, giving her a tantalizing look at your nethers before hiding them again.  With a wide grin, you creep your foot up to her thighs and gently run your heel along her smooth, tender flesh.  You come to the base of her rod, standing stark straight in the air like a flag-pole.  As you strip, you grasp her cock between your big and second toe, sliding them up the smooth shaft to the tip.  " + flags[kFLAGS.GAR_NAME] + " moans, rolling her head back as you start to foot-fuck her cock, slowly putting more and more pressure on the steel member until it's pressed against her belly.  Your heel takes over, running quickly along the underside of her cock as you finally rid yourself of your [armor].");
        }
        //Else, continued from above paragraph:
        else
        {
            outputText("you giver her a good show, putting on a brief striptease for the gargoyle.  You strip your [armor] slowly, sensually, showing only the barest hints of your privates until the last moment, when the last piece of your clothing falls to the floor.");
        }
    ...
    private function tailFuckGargoyleScene() : Void
    {
        ...
        if (gargoyleConfidence() > 70)
        {
            outputText(" with just a hint of a smile");
        }
        outputText(".");
    ...
    private function ritualGargoyle() : Void
    {
        ...
        if (flags[kFLAGS.RITUAL_INTRO] == 0)
        {
            ...
            if (gargoyleConfidence() < 50)
            {
                outputText(" Forgive my impertinence, but");
            }
        ...
        if (player.hasStatusEffect(StatusEffects.Infested) || SceneLib.exgartuan.anyPresent())
        {
            addButton(2, "Banish", banishmentGargoyleRitual);
        }
        addButton(4, "Leave", returnToCathedral);
    }
    ...
    private function bodyRitual() : Void
    {
        ...
        {
            for (stat2 in ["str", "spe", "tou", "int", "wis", "lib", "sens"])
            {
    ...
    private function banishmentGargoyleRitual() : Void
    {
        ...
        if (SceneLib.exgartuan.anyPresent())
        {
            outputText("a hitchhiker");
        }
        else
        {
            outputText("some hitchhikerss");
        }
        ...
        if (SceneLib.exgartuan.anyPresent())
        {
            outputText("this bastard");
        }
        else
        {
            outputText("these bastards");
        }
        ...
        if (gargoyleConfidence() >= 50)
        {
            outputText(" soothingly, using her free hand to stroke your thighs");
        }
        ...
        if (SceneLib.exgartuan.anyPresent())
        {
            outputText("  Booming from within you, you hear the demon's voice: \"<i>Hey!  What the shit is this!?  What're you doing? Fucking stop, you ingrate!</i>\"");
        }
        //with Worms:
        else
        {
            outputText("  Gratifyingly, however, you can feel the infestation within you squirming and writhing, desperately seeking escape.");
        }
        ...
        if (SceneLib.exgartuan.anyPresent())
        {
            outputText("creature within you has");
        }
        else
        {
            outputText("creatures within you have");
        }
    ...
    private function gargoyleKinkyRituals() : Void
    {
        ...
        if (SceneLib.exgartuan.anyPresent() || player.hasStatusEffect(StatusEffects.Infested))
        {
            addButton(2, "Banish", banishPervRitual);
        }
        addButton(4, "Leave", returnToCathedral);
    }
    ...
    private function kinkyBodyRitual() : Void
    {
        ...
        {
            outputText(", cupping your [chest] in her cold stone hands");
        }
    ...
    private function banishPervRitual() : Void
    {
        ...
        {
            outputText("  She slowly but surely coaxes the holy rod into your asshole, making long and slow thrusts into you until she finally bottoms out");
        }
        outputText(".");
        ...
        if (!SceneLib.exgartuan.anyPresent())
        {
            outputText("  You can feel her cock ram against your prostate, and shudder as the writhing mass of worms begin to panic as her cock pushes against them, obviously causing the creatures great pain.");
        }
        ...
        if (SceneLib.exgartuan.dickPresent())
        {
            outputText("  You can hear the demonic voice of Exgartuan inside your mind, yelling and cursing as he is rapidly drained from your body and sealed into the holy rod within you.");
        }
        if (SceneLib.exgartuan.boobsPresent())
        {
            outputText("  You can hear the demonic voice of Xenora inside your mind, yelling and cursing as she is rapidly drained from your body and sealed into the holy rod within you.");
        }
        ...
        else
        {
            ...
            if (SceneLib.exgartuan.anyPresent())
            {
                outputText("Exgartuan is");
            }
            else
            {
                outputText("the worms are");
            }
    ...
    //[b]Talk[/b]
    private function talkToGargoyle(output : Bool = true) : Void
    {
        if (output)
        {
            clearOutput();
            ...
            if (gargoyleConfidence() < 50)
            {
                outputText("  She meekly approaches her Master and kneels before you, head bowed submissively.");
            }
            //if Confidence = 51-69:
            else if (gargoyleConfidence() < 70)
            {
                outputText("  She nearly skips to follow you, assuming her low crouch at your feet and looking up at your expectantly.");
            }
            else
            {
                outputText("  Happily, she takes a seat beside you and slips an arm around your shoulders, locking you in companionable closeness.");
            }
    ...
    private function historyOfGargoylesB() : Void
    {
        ...
        if (gargoyleConfidence() > 70)
        {
            outputText(", and I despise them with every fiber of my being");
        }
    ...
    private function cathedralC() : Void
    {
        ...
        if (gargoyleConfidence() > 69)
        {
            outputText(" unless it were to be with you, that is");
        }
    ...
    private function garNameA() : Void
    {
        ...
        if (gargoyleConfidence() > 69)
        {
            outputText("  She continues, \"<i>though even were I not compelled to, I would still serve you,</i>\" she says with a playful wink.");
        }
        ...
        {
            outputText("After what she did to me, I never thought I'd enjoy sex again.</i>\"  With a loving smile, " + flags[kFLAGS.GAR_NAME] + " takes your hand in hers and brushes it against her cheek.  \"<i>But for you, Master, I think I could make an exception.</i>\"");
        }
        ...
        if (gargoyleConfidence() <= 69)
        {
            outputText("  \"<i>I'm sorry, Master, I can't... it hurts too much to think about it.</i>\"");
        }
        else
        {
            outputText("  \"<i>Sorry, Master. I just....Don't want to talk about it right now.  Ask me again some time.</i>\"");
        }
    ...
    private function gargoyleAppearance() : Void
    {
        ...
        if (gargoyleConfidence() < 70)
        {
            outputText("glow timidly, reflecting a submissive and servile nature in the gargoyle.");
        }
        else
        {
            outputText("have a cool fire in them, showing her steadily-growing confidence and warmth toward you.");
        }
