...
class FollowerInteractions extends NPCAwareContent
{
    ...
    private function tellAmilyToFuckOffRuinedJojo() : Void
    {
        ...
        if (player.inte >= 45)
        {
            outputText("  Goblin ale and lust drafts might do the trick.");
        }
    ...
    private function agreeToHelpAmilyFixJojo() : Void
    {
        ...
        if (player.cor < 33)
        {
            outputText("feeling genuinely sorry for what you've done.");
        }
        else if (player.cor < 66)
        {
            outputText("unsure if you're making the right choice.");
        }
        else
        {
            outputText("wondering if this will wind up as an empty promise to placate the angry mouse-cunt.");
        }
    ...
    public function fixJojoOOOOHYEEEEAHSNAPINTOASLIMJIM() : Void
    {
        ...
        if (player.cor > 66)
        {
            outputText("already regretting your choice");
        }
        else if (player.cor > 33)
        {
            outputText("a little confused by the whole situation");
        }
        else
        {
            outputText("glad the damage you did to Jojo wasn't permanent");
        }
        outputText(".");
    ...
    @:allow(classes.scenes)
    private function findJojosNote() : Void
    {
    ...
    public function amilyIngredientDelivery() : Void
    {
        ...
        {
            outputText("big-eared blur whizzes past you");
        }
        else
        {
            outputText("blur of fur whizzes past you");
        }
    ...
    @:allow(classes.scenes)
    private function rathazulFreaksOverJojo() : Void
    {
        clearOutput();
    ...
    @:allow(classes.scenes)
    private function zenjiFreaksOverJojo() : Void
    {
        clearOutput();
    ...
    @:allow(classes.scenes)
    private function zenjiFreaksOverCorruptCeless() : Void
    {
        clearOutput();
    ...
    @:allow(classes.scenes)
    private function marbleVsAmilyFreakout() : Void
    {
        clearOutput();
        flags[kFLAGS.AMILY_NOT_FREAKED_OUT]++;
        //Marble is in camp first
        if (flags[kFLAGS.MARBLE_OR_AMILY_FIRST_FOR_FREAKOUT] <= 1)
        {
            outputText("As you bring Amily into your camp, you see Marble look towards you with a smile for a moment, before that smile turns into a frown.  You realize that this might not have been such a good idea...\n\n");
        }
        ////Amily is in camp first
        else
        {
            outputText("As you bring Marble back to your camp, you see Amily sitting there as Marble asks while frowning, \"<i>Sweetie, who is that?</i>\"  Uh-oh, this could get bad.\n\n");
        }
    ...
    private function srslyPimpinGuyz() : Void
    {
        ...
        {
            outputText("All that is left is a suit of comfortable clothes that you put on.  ");
        }
        else
        {
            outputText("\"<i>Are you all right?</i>\" the armor-goo asks.  You insist her that you have a terrible headache.  ");
        }
    ...
    @:allow(classes.scenes)
    private function amilyUrtaReaction() : Void
    {
        clearOutput();
    ...
    public function askAboutAmily() : Void
    {
        ...
        else
        {
            ...
            {
                outputText(" her " + amilyScene.amilyCock() + " half-erect and drooling pre and");
            }
            outputText(" her stomach swollen from the fox-morph's prodigious cum output to the point she looks ");
            if (amilyScene.pregnancy.event >= 3)
            {
                outputText("almost twice as ");
            }
    ...
    public function amilyUrtaSex() : Void
    {
        ...
        if (player.cor > 50 && flags[kFLAGS.AMILY_WANG_LENGTH] > 0)
        {
            outputText("  Privately, you note that it's bullshit; she's nothing <b>but</b> self-pity.");
        }
        outputText("\n\n");
        ...
        if (!urtaLove())
        {
            outputText("blushes softly at the clunk of her erection against the table");
        }
        else
        {
            outputText("turns sideways to let her dress tent without banging into the table");
        }
        outputText(".\n\n");
        ...
        if (player.hasVagina())
        {
            vag = amilyScene.urtaXAmilyCuntPussyVagSQUICK;
        }
        if (player.hasCock())
        {
            dick = amilyScene.threesomeAmilUrtaCAWKS;
        }
    ...
    private function amilyUrtaSexWatch() : Void
    {
        ...
        if (flags[kFLAGS.AMILY_WANG_LENGTH] > 0)
        {
            outputText(", up the underside of her penis,");
        }
        ...
        if (flags[kFLAGS.AMILY_WANG_LENGTH] > 0)
        {
            outputText(", a bead of pre-cum stretching into a line connecting their two members");
        }
