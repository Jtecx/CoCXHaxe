 ...
 * @author:
 */
class TelAdre extends BaseContent
{
    ...
    public var nails : Float = 0;
    public var wood : Float = 0;
    public var stone : Float = 0;
    ...
    private function telAdreCrystal() : Void
    {
        if (!player.hasStatusEffect(StatusEffects.TelAdre))
        {
            player.createStatusEffect(StatusEffects.TelAdre, 0, 0, 0, 0);
        }
    ...
    public function telAdreMenu() : Void
    {
        ...
        var _sw0_ = (flags[kFLAGS.KATHERINE_UNLOCKED]);        

        switch (_sw0_)
        {
            case -1, 0:  //Still potentially recruitable  
            ...
            default:  //Has given you a spare key to her apartment  
                if (model.time.hours < 10 && rand(12) == 0)
    ...
    //Just displays the normal Tel'Adre menu options, no special events, no description. Useful if a special event has already played
    {
        ...
        if (homes)
        {
            addButton(5, "Homes", houses);
        }
        if (flags[kFLAGS.ARIAN_PARK] > 0 && flags[kFLAGS.ARIAN_PARK] < 4)
        {
            addButton(6, "Park", SceneLib.arianScene.visitThePark);
        }
    ...
    private function armorShops() : Void
    {
        ...
        {
            addButtonDisabled(7, "Gunshop", "Tripixi is busy. Come back later.");
        }
        else
        {
    ...
    public function houses() : Void
    {
        ...
        menu();
        if (flags[kFLAGS.KATHERINE_UNLOCKED] >= 5)
        {
            addButton(0, "Kath's Apt", katherine.visitAtHome);
        }
        ...
        if (flags[kFLAGS.ARIAN_PARK] >= 4 && !SceneLib.arianScene.arianFollower())
        {
            addButton(2, "Arian's", SceneLib.arianScene.visitAriansHouse);
        }
    ...
    public function oswaldPawn() : Void
    {
        ...
        if (!player.hasStatusEffect(StatusEffects.Oswald))
        {
            ...
            if (player.cor < 75)
            {
                outputText("Who would've thought that seeing someone NOT aroused would ever shock you?");
            }
            else
            {
                outputText("What a shame, but maybe you can give him a reason to stand up straight?");
            }
    ...
    private function oswaldPawnMenu(page : Int = 1, refresh : Bool = true) : Void
    //Moved here from Inventory.as
    {
        ...
        if (page == 1)
        {
            for (slot in 0...10)
            {
                ...
                {
                    outputText("\n" + as3hx.Compat.parseInt(player.itemSlots[slot].itype.value / 2) + " gems for " + player.itemSlots[slot].itype.longName + ".");
            ...
            {
                addButton(13, "Next", oswaldPawnMenu, page + 1, refresh = true);
            }
        }
        if (page == 2)
        {
            ...
            {
                addButton(13, "Next", oswaldPawnMenu, page + 1, true);
            }
        }
        if (page == 3)
        {
        ...
        if (totalItems > 1)
        {
            addButton(11, "Sell All", oswaldPawnSellAll);
        }
        addButton(14, "Back", telAdreMenu);
    }
    private function oswaldPawnMenu2() : Void
    {
        ...
        var _sw1_ = (flags[kFLAGS.KATHERINE_UNLOCKED]);        

        switch (_sw1_)
        {
            case 1, 2:addButton(10, "Kath's Alley", katherine.visitKatherine);
            case 3:addButton(10, "Safehouse", katherineEmployment.katherineTrainingWithUrta);
            case 4:addButton(10, "Kath's Alley", katherineEmployment.postTrainingAlleyDescription);
        }
        addButton(14, "Back", oswaldPawn);
    }
    
    private function oswaldPawnSell(slot : Int) : Void
    //Moved here from Inventory.as
    {
        
        spriteSelect(SpriteDb.s_oswald);
        var itemValueOrgin : Float = player.itemSlots[slot].itype.value;
        var itemValue : Int = as3hx.Compat.parseInt(player.itemSlots[slot].itype.value / 2);
        clearOutput();
        ...
        if (itemValue > itemValueOrgin)
        {
            itemValue = as3hx.Compat.parseInt(itemValueOrgin);
        }
        if (itemValue != 0 && player.hasPerk(PerkLib.Greedy) || player.hasPerk(PerkLib.TravelingMerchantOutfit))
        {
            outputText("Thanks to a little magic and a lot of hard bargaining you managed to sell your items for more than normal. ");
        }
        if (shiftKeyDown == 1)
        {
            ...
            {
                outputText("You hand over " + num2Text(player.itemSlots[slot].quantity) + " " + player.itemSlots[slot].itype.shortName + " to Oswald.  He nervously pulls out " + num2Text(itemValue * player.itemSlots[slot].quantity) + " gems and drops them into your waiting hand.");
            }
        ...
        else
        {
            ...
            {
                outputText("You hand over " + player.itemSlots[slot].itype.longName + " to Oswald.  He nervously pulls out " + num2Text(itemValue) + " gems and drops them into your waiting hand.");
            }
            player.itemSlots[slot].removeOneItem();
            player.gems += itemValue;
        }
    ...
    private function oswaldPawnSellAll() : Void
    {
        ...
        for (slot in 0...50)
        {
            if (player.itemSlots[slot].quantity > 0 && player.itemSlots[slot].itype.value >= 1)
            {
                itemValue += as3hx.Compat.parseInt(player.itemSlots[slot].quantity * as3hx.Compat.parseInt(player.itemSlots[slot].itype.value / 2));
        ...
        if (player.hasPerk(PerkLib.Greedy) || player.hasPerk(PerkLib.TravelingMerchantOutfit))
        {
            outputText(" Thanks to a little magic and a lot of hard bargaining you managed to sell your item for more than normal.");
        }
        player.gems += itemValue;
        statScreenRefresh();
        doNext(oswaldPawn);
    ...
    private function anotherButton(button : Int, nam : String, func : Function, arg : Dynamic = -9000) : Int
    {
        if (button > 8)
        {
            return 9;
    ...
    {
        if (isThanksgiving() && flags[kFLAGS.PIG_SLUT_DISABLED] == 0)
        {
            SceneLib.holidays.pigSlutRoastingGreet();
        }
        else
        {
            barTelAdre();
    ...
    // Dominka & Edryn both persist their sprites if you back out of doing anything with them -- I
    {
        ...
        {
            outputText("despite your altered appearance, ");
        }
        ...
        if (model.time.hours >= 7 && model.time.hours <= 17 && SceneLib.alvinaFollower.alvinaCanMeetAtBar())
        {
            ...
            {
                button = anotherButton(button, "Alvina", SceneLib.alvinaFollower.alvinaMeetAtBar);
            }
        }
        ...
        //EDRYN!
        if (edryn.pregnancy.type != PregnancyStore.PREGNANCY_TAOTH)
        ...
        if (auntNancy.auntNancy(false))
        {
            ...
            {
                button = anotherButton(button, "Nancy", auntNancy.interactWithAuntNancy);
            }
            else
            {
                button = anotherButton(button, "Barkeep", auntNancy.interactWithAuntNancy);
            }
        }
        else
        {
        ...
        if (model.time.hours >= 8 && model.time.hours <= 16 && flags[kFLAGS.NIAMH_STATUS] == 0)
        {
            ...
            {
                button = anotherButton(button, "Beer Cat", niamh.approachNiamh);
            }
            else
            {
                button = anotherButton(button, "Niamh", niamh.approachNiamh);
            }
        }
        //ROGAR #1
        ...
        //VALA
        if (SceneLib.vala.purifiedFaerieBitchBar())
        {
            button = anotherButton(button, "Vala", SceneLib.vala.chooseValaInBar);
        }
    ...
    public function carpentryShopInside() : Void
    {
        ...
        {
            addButton(11, "Nails Box", carpentryShopBuySet2);
        }
    ...
    public function carpentryShopSellNails() : Void
    {
        ...
        outputText("Nails: " + CampStatsAndResources.NailsResc + "/" + SceneLib.campUpgrades.checkMaterialsCapNails() + "");
        menu();
        if (CampStatsAndResources.NailsResc >= 1)
        {
            addButton(0, "Sell 1", carpentryShopSellNailsAmount, 1);
        }
        if (CampStatsAndResources.NailsResc >= 5)
        {
            addButton(1, "Sell 5", carpentryShopSellNailsAmount, 5);
        }
        if (CampStatsAndResources.NailsResc >= 10)
        {
            addButton(2, "Sell 10", carpentryShopSellNailsAmount, 10);
        }
        if (CampStatsAndResources.NailsResc >= 25)
        {
            addButton(3, "Sell 25", carpentryShopSellNailsAmount, 25);
        }
        if (CampStatsAndResources.NailsResc >= 50)
        {
            addButton(4, "Sell 50", carpentryShopSellNailsAmount, 50);
        }
        if (CampStatsAndResources.NailsResc >= 100)
        {
            addButton(5, "Sell 100", carpentryShopSellNailsAmount, 100);
        }
        if (CampStatsAndResources.NailsResc >= 200)
        {
            addButton(6, "Sell 200", carpentryShopSellNailsAmount, 200);
        }
        if (CampStatsAndResources.NailsResc >= 300)
        {
            addButton(7, "Sell 300", carpentryShopSellNailsAmount, 300);
        }
        if (CampStatsAndResources.NailsResc > 0)
        {
            addButton(8, "Sell All", carpentryShopSellNailsAmount, CampStatsAndResources.NailsResc);
        }
        addButton(14, "Back", carpentryShopInside);
    }
    ...
    private function carpentryShopSellNailsYes() : Void
    {
        ...
        if (CampStatsAndResources.NailsResc >= nails)
        {
            ...
            if (player.hasPerk(PerkLib.Greedy) || player.hasPerk(PerkLib.TravelingMerchantOutfit))
            {
                outputText("Thanks to a little magic and a lot of hard bargaining you managed to sell your nail" + ((nails > 1) ? "s" : "") + " for more than normal.\n\n");
            }
            outputText("Nails: " + CampStatsAndResources.NailsResc + "/" + SceneLib.campUpgrades.checkMaterialsCapNails() + "");
        }
        else
    ...
    public function carpentryShopSellWood() : Void
    {
        ...
        outputText("Wood: " + CampStatsAndResources.WoodResc + "/" + SceneLib.campUpgrades.checkMaterialsCapWood() + "");
        menu();
        if (CampStatsAndResources.WoodResc >= 1)
        {
            addButton(0, "Sell 1", carpentryShopSellWoodAmount, 1);
        }
        if (CampStatsAndResources.WoodResc >= 5)
        {
            addButton(1, "Sell 5", carpentryShopSellWoodAmount, 5);
        }
        if (CampStatsAndResources.WoodResc >= 10)
        {
            addButton(2, "Sell 10", carpentryShopSellWoodAmount, 10);
        }
        if (CampStatsAndResources.WoodResc >= 25)
        {
            addButton(3, "Sell 25", carpentryShopSellWoodAmount, 25);
        }
        if (CampStatsAndResources.WoodResc >= 50)
        {
            addButton(4, "Sell 50", carpentryShopSellWoodAmount, 50);
        }
        if (CampStatsAndResources.WoodResc >= 100)
        {
            addButton(5, "Sell 100", carpentryShopSellWoodAmount, 100);
        }
        if (CampStatsAndResources.WoodResc >= 200)
        {
            addButton(6, "Sell 200", carpentryShopSellWoodAmount, 200);
        }
        if (CampStatsAndResources.WoodResc >= 300)
        {
            addButton(7, "Sell 300", carpentryShopSellWoodAmount, 300);
        }
        if (CampStatsAndResources.WoodResc > 0)
        {
            addButton(8, "Sell All", carpentryShopSellWoodAmount, CampStatsAndResources.WoodResc);
        }
        addButton(14, "Back", carpentryShopInside);
    }
    ...
    private function carpentryShopSellWoodYes() : Void
    {
        ...
        if (CampStatsAndResources.WoodResc >= wood)
        {
            ...
            if (player.hasPerk(PerkLib.Greedy) || player.hasPerk(PerkLib.TravelingMerchantOutfit))
            {
                outputText("Thanks to a little magic and a lot of hard bargaining you managed to sell your wood for more than normal.\n\n");
            }
            outputText("Wood: " + CampStatsAndResources.WoodResc + "/" + SceneLib.campUpgrades.checkMaterialsCapWood() + "");
        }
        else
    ...
    public function carpentryShopSellStone() : Void
    {
        ...
        outputText("Stone: " + CampStatsAndResources.StonesResc + "/" + SceneLib.campUpgrades.checkMaterialsCapStones() + "");
        menu();
        if (CampStatsAndResources.StonesResc >= 1)
        {
            addButton(0, "Sell 1", carpentryShopSellStoneAmount, 1);
        }
        if (CampStatsAndResources.StonesResc >= 5)
        {
            addButton(1, "Sell 5", carpentryShopSellStoneAmount, 5);
        }
        if (CampStatsAndResources.StonesResc >= 10)
        {
            addButton(2, "Sell 10", carpentryShopSellStoneAmount, 10);
        }
        if (CampStatsAndResources.StonesResc >= 25)
        {
            addButton(3, "Sell 25", carpentryShopSellStoneAmount, 25);
        }
        if (CampStatsAndResources.StonesResc >= 50)
        {
            addButton(4, "Sell 50", carpentryShopSellStoneAmount, 50);
        }
        if (CampStatsAndResources.StonesResc >= 100)
        {
            addButton(5, "Sell 100", carpentryShopSellStoneAmount, 100);
        }
        if (CampStatsAndResources.StonesResc >= 200)
        {
            addButton(6, "Sell 200", carpentryShopSellStoneAmount, 200);
        }
        if (CampStatsAndResources.StonesResc >= 300)
        {
            addButton(7, "Sell 300", carpentryShopSellStoneAmount, 300);
        }
        if (CampStatsAndResources.StonesResc > 0)
        {
            addButton(8, "Sell All", carpentryShopSellStoneAmount, CampStatsAndResources.StonesResc);
        }
        addButton(14, "Back", carpentryShopInside);
    }
    ...
    private function carpentryShopSellStoneYes() : Void
    {
        ...
        if (CampStatsAndResources.StonesResc >= stone)
        {
            ...
            if (player.hasPerk(PerkLib.Greedy) || player.hasPerk(PerkLib.TravelingMerchantOutfit))
            {
                outputText("Thanks to a little magic and a lot of hard bargaining you managed to sell your stone" + ((stone > 1) ? "s" : "") + " for more than normal.\n\n");
            }
            outputText("Stone: " + CampStatsAndResources.StonesResc + "/" + SceneLib.campUpgrades.checkMaterialsCapStones() + "");
        }
        else
    ...
    private function buyItem(odd : ItemType) : Void
    {
        ...
        {
            doYesNo(curry(buyItemYes, odd), buyItemNo);
        }
    }
    ...
    public function tripxiShopMainMenu() : Void
    {
        ...
        if (player.statusEffectv2(StatusEffects.TelAdreTripxi) > 0)
        {
            ...
            {
                player.addStatusValue(StatusEffects.TelAdreTripxi, 3, 16);
            }
            else
            {
                player.addStatusValue(StatusEffects.TelAdreTripxi, 3, 8);
            }
            player.changeStatusValue(StatusEffects.TelAdreTripxi, 2, 0);
            var guns : Array<Dynamic> = [
            ...
            for (gun in guns)
            {
                if (player.hasKeyItem(Reflect.field(gun, Std.string(0))) >= 0)
                {
                    player.removeKeyItem(Reflect.field(gun, Std.string(0)));
                    player.addStatusValue(Reflect.field(gun, Std.string(1)), Reflect.field(gun, Std.string(2)), 1);
                }
            }
            doNext(telAdreMenu);
        }
    ...
    public function tripxiShopMainMenu2a() : Void
    {
        ...
        //3
        if (player.statusEffectv1(StatusEffects.TelAdreTripxiGuns1) > 0)
        {
            addButton(4, weaponsrange.DESEAGL.shortName, buyItemT1, weaponsrange.DESEAGL);
        }
        else if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns1))
        {
            addButtonDisabled(4, "???", "Search the Desert(O). (lvl 6+)");
        }
        if (player.statusEffectv1(StatusEffects.TelAdreTripxiGuns2) > 0)
        {
            addButton(5, weaponsrange.M1CERBE.shortName, buyItemT1, weaponsrange.M1CERBE);
        }
        else if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns2))
        {
            addButtonDisabled(5, "???", "Search the Swamp.");
        }
        if (player.statusEffectv1(StatusEffects.TelAdreTripxiGuns3) > 0)
        {
            addButton(6, weaponsrange.TRFATBI.shortName, buyItemT1, weaponsrange.TRFATBI);
        }
        else if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns3))
        {
            addButtonDisabled(6, "???", "Search the Volcanic Crag.");
        }
        if (player.statusEffectv1(StatusEffects.TelAdreTripxiGuns4) > 0)
        {
            addButton(7, weaponsrange.SNIPPLE.shortName, buyItemT1, weaponsrange.SNIPPLE);
        }
        else if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns4))
        {
            addButtonDisabled(7, "???", "Search the Plains.");
        }
        if (player.statusEffectv1(StatusEffects.TelAdreTripxiGuns5) > 0)
        {
            addButton(8, weaponsrange.TOUHOM3.shortName, buyItemT1, weaponsrange.TOUHOM3);
        }
        else if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns5))
        {
            addButtonDisabled(8, "???", "Search the Caves.");
        }
        if (player.statusEffectv1(StatusEffects.TelAdreTripxiGuns6) > 0)
        {
            addButton(9, weaponsrange.TWINGRA.shortName, buyItemT1, weaponsrange.TWINGRA);
        }
        else if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns6))
        {
            addButtonDisabled(9, "???", "Search the Battlefield(B).");
        }
    ...
    public function tripxiShopMainMenu2b() : Void
    {
        ...
        //3
        if (player.statusEffectv2(StatusEffects.TelAdreTripxiGuns1) > 0)
        {
            addButton(4, weaponsrange.DPISTOL.shortName, buyItemT2, weaponsrange.DPISTOL, false);
        }
        else if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns1))
        {
            addButtonDisabled(4, "???", "Search the Blight Ridge.");
        }
        if (player.statusEffectv2(StatusEffects.TelAdreTripxiGuns2) > 0)
        {
            addButton(5, weaponsrange.TDPISTO.shortName, buyItemT2, weaponsrange.TDPISTO, false);
        }
        else if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns2))
        {
            addButtonDisabled(5, "???", "Search the Defiled Ravine.");
        }
        if (player.statusEffectv2(StatusEffects.TelAdreTripxiGuns3) > 0)
        {
            addButton(6, weaponsrange.HARPGUN.shortName, buyItemT2, weaponsrange.HARPGUN, false);
        }
        else if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns3))
        {
            addButtonDisabled(6, "???", "Search the Beach.");
        }
        //7 - Sakuno M2
        if (player.statusEffectv2(StatusEffects.TelAdreTripxiGuns5) > 0)
        {
            addButton(8, weaponsrange.DERPLAU.shortName, buyItemT2, weaponsrange.DERPLAU, false);
        }
        else if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns5))
        {
            addButtonDisabled(8, "???", "Search the Low Mountains.");
        }
        //9 - Tediore pistol
    ...
    public function tripxiShopMainMenu2c() : Void
    {
        ...
        //2
        //3
        if (player.statusEffectv3(StatusEffects.TelAdreTripxiGuns1) > 0)
        {
            addButton(4, weaponsrange.DBDRAGG.shortName, buyItemT3, weaponsrange.DBDRAGG);
        }
        else if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns1))
        {
            addButtonDisabled(4, "???", "Search the Volcanic Crag.");
        }
        if (player.statusEffectv3(StatusEffects.TelAdreTripxiGuns2) > 0)
        {
            addButton(5, weaponsrange.LBLASTR.shortName, buyItemT3, weaponsrange.LBLASTR);
        }
        else if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns2))
        {
            addButtonDisabled(5, "???", "Search the Hills.");
        }
        //6 - Harkonnen II - BP req. from some dark place/area?
        if (player.statusEffectv3(StatusEffects.TelAdreTripxiGuns4) > 0)
        {
            addButton(7, weaponsrange.ALAKABL.shortName, buyItemT3, weaponsrange.ALAKABL);
        }
        else if (player.hasStatusEffect(StatusEffects.TelAdreTripxiGuns3))
        {
            addButtonDisabled(7, "???", "Search the Forest(I).");
        }
    ...
    private function buyItemT1(odd : ItemType, page : Int = 1) : Void
    {
        ...
        {
            doYesNo(curry(buyItemT1Yes, odd), buyItemT1No);
        }
    }
    ...
    private function buyItemT2(odd : ItemType, nonweapon : Bool) : Void
    {
        clearOutput();
        var cost : Int = odd.value;
        outputText("You point at the " + odd.longName + " and Tripxi put it on the counter.\n\n\"<i>Yea sure this thing will only be " + odd.value + " gems. Hope you got the money on you.</i>\"\n\n");
        ...
        {
            doYesNo(curry(buyItemT2Yes, odd, nonweapon), buyItemT2No);
        }
    }
    private function buyItemT2Yes(odd : ItemType, nonweapon : Bool) : Void
    {
        player.gems -= odd.value;
        statScreenRefresh();
    ...
    private function buyItemT3(odd : ItemType) : Void
    {
        ...
        {
            doYesNo(curry(buyItemT3Yes, odd), buyItemT3No);
        }
    }
    ...
    private function tripxiShopTalk(talkOver : Int = 1) : Void
    {
        ...
        if (player.statusEffectv1(StatusEffects.TelAdreTripxi) > 0)
        {
            addButtonDisabled(5, "Small Selection", "You have already talked about this subject.");
        }
        else
        {
            addButton(5, "SmallSelection", tripxiShopTalkSmallSelection);
        }
    ...
    //PREGGO ALERT!
    {
        ...
        if (player.cor < 35)
        {
            outputText("thankfully ");
        }
        else if (flags[kFLAGS.PC_FETISH] > 0 || player.cor > 80)
        {
            outputText("unfortunately ");
        }
        ...
        {
            outputText("10 gems an hour to use the facilities here, or 500 for a life-time membership.</i>\"  She has her hands on her hips, and it looks you'll have to pay ten gems to actually get to use any of this stuff.");
        }
        else
        {
            outputText("Oh, welcome back [name].  Have a nice workout!</i>\"");
        }
        ...
        if (flags[kFLAGS.LOPPE_MET] > 0 && flags[kFLAGS.LOPPE_DISABLED] == 0)
        {
            ...
            {
                SceneLib.loppe.loppePregAppearance();
            }
            else
            {
        ...
        if (model.time.hours > 9 && model.time.hours < 14)
        {
            heckel.heckelAppearance();
        }
    ...
    private function gymMenu() : Void
    {
        ...
        //NPCs
        if (flags[kFLAGS.PC_IS_A_DEADBEAT_COTTON_DAD] == 0 && cotton.cottonsIntro())
        {
            addButton(5, (flags[kFLAGS.COTTON_MET_FUCKED] > 0) ? "Cotton" : "Horsegirl", cotton.cottonGreeting);
        }
        if (model.time.hours > 9 && model.time.hours <= 15)
        {
            addButton(6, (flags[kFLAGS.MET_HECKEL] > 0) ? "Heckel" : "Hyena", heckel.greetHeckel);
        }
        if (ifris.ifrisIntro())
        {
            addButton(7, (flags[kFLAGS.MET_IFRIS] > 0) ? "Ifris" : "Demon-Girl", ifris.approachIfris);
        }
        addButton(8, (flags[kFLAGS.LOTTIE_TIMES_ENCOUNTERED] > 0) ? "Lottie" : "Pig-Girl", lottie.lottieAppearance(false));
        if (flags[kFLAGS.LOPPE_MET] > 0 && flags[kFLAGS.LOPPE_DISABLED] == 0 && SceneLib.loppe.pregnancy.isPregnant)
        {
            addButton(9, "Loppe", SceneLib.loppe.loppePreggoApproached);
        }
        else if (flags[kFLAGS.LOPPE_MET] > 0 && flags[kFLAGS.LOPPE_DISABLED] == 0)
        {
            addButton(9, "Loppe", SceneLib.loppe.loppeGenericMeetings);
        }
        if (pablo.pabloIntro() && flags[kFLAGS.PABLO_FREAKED_OUT_OVER_WORMS] != 1)
        {
            addButton(10, (flags[kFLAGS.PABLO_MET] > 0) ? "Pablo" : "Imp?", pablo.approachPablo);
        }
        addButton(14, "Leave", telAdreMenu);
    }
    ...
    private function buyGymLifeTimeMembership() : Void
    {
        ...
        if (silly())
        {
            outputText("\"<i>Shut up and take my gems!</i>\" you shout as you pull out your gem-pouch. \n\n");
    ...
    private function weightLifting() : Void
    {
        ...
        if (player.fatigue > player.maxOverFatigue() - 25)
        {
            ...
            if (flags[kFLAGS.LIFETIME_GYM_MEMBER] == 0)
            {
                outputText("It'd be better to save your money and come back after you've rested.");
            }
            doNext(telAdreMenu);
            return;
        }
        ...
        if (player.str < 25)
        {
            outputText("You have to start out on the smaller weights to the left side of the rack due to your strength, but even so, you manage to work up a good burn and a modest sweat.");
        }
        //(< 40 str)
        else if (player.str < 40)
        {
            outputText("You heft a few of the weights and select some of the ones just to the left of the middle.  It doesn't take you long to work up a sweat, but you push on through a variety of exercises that leave your body feeling sore and exhausted.");
        }
        //(< 60 str)
        else if (player.str < 60)
        {
            outputText("You smile when you grip a few of the heavier weights on the rack and start to do some lifts.  With a start, you realize you're probably stronger now than Ingnam's master blacksmith, Ben.  Wow!  This realization fuels you to push yourself even harder, and you spend nearly an hour doing various strength-building exercises with the weights.");
        }
        //(<80 str)
        else if (player.str < 80)
        {
            outputText("You confidently grab the heaviest dumbbells in the place and heft them.  It doesn't take long for you to work up a lather of sweat and feel the burn thrumming through your slowly tiring form.  The workout takes about an hour, but you feel you made some good progress today.");
        }
        //(<90)
        else if (player.str < 90)
        {
            outputText("You grab the heaviest weights they have and launch into an exercise routine that leaves you panting from exertion.  Setting the weights aside, you flex and marvel at yourself – you could probably arm wrestle a minotaur or two and come out victorious!");
        }
        //(else)
        else
        {
            outputText("This place barely has anything left to challenge you, but you take the heaviest weights you can get your mitts on and get to it.  By the time an hour has passed, you've worked up a good sweat, but without heavier weights you probably won't get any stronger.");
        }
        //Stat changes HERE!
        dynStats("str", 5);
        ...
        if (flags[kFLAGS.BROOKE_MET] == 1)
        {
            ...
            {
                doYesNo(brooke.repeatChooseShower, stopGoingBackEveryHourGymCheck);
            }
        }
        else
        {
            doYesNo(sexMachine.exploreShowers, stopGoingBackEveryHourGymCheck);
        }
    }
    ...
    private function goJogging() : Void
    {
        ...
        if (player.tou < 25)
        {
            outputText("but you get so winded you have to stop after a few minutes.  Determined to improve, you force yourself to stay at a fast walk until you can run again.");
        }
        //(<40 tou)
        ...
        {
            outputText("and you do quite well.  You jog around for nearly an hour, working up a healthy lather of sweat.  Even your [legs] tingle and burn with exhaustion.");
        }
        //(<80 tou)
        else if (player.tou < 80)
        {
            outputText("and it doesn't faze you in the slightest.  You run lap after lap at a decent clip, working yourself until you're soaked with sweat and fairly tired.");
        }
        //(<90 tou)
        else if (player.tou < 90)
        {
            outputText("and you have a terrific time.  You can keep yourself just below your sprinting speed for the entire time, though you work up a huge amount of sweat in the process.");
        }
        //else)
        else
        {
            outputText("and it barely challenges you.  You run at a sprint half the time and still don't feel like you're improving in the slightest.  Still, you do manage to burn a lot of calories.");
        }
        //Stat changes HERE!
        dynStats("spe", 5);
        ...
        else if (player.butt.type >= 10 && rand(3) == 0)
        {
        ...
        else if (player.hips.type >= 10 && rand(3) == 0)
        {
            outputText("\n\nIt feels like your " + hipDescript() + " have shed some pounds and narrowed.");
            player.hips.type--;
        }
    ...
    private var justRejectedLuna : Bool = false;
    ...
    public function meetingLunaFirstTime() : Void
    {
        ...
        if (player.gems >= 100)
        {
            addButton(0, "Work4Me", meetingLunaFirstTimeHelp);
        }
        else
        {
            addButtonDisabled(0, "Work4Me", "You're not quite slimy enough to try to take advantage of a desperate girl when you're too broke to pay her. Try later with at least 100 gems.");
        }
    ...
    public function meetingLunaRepated() : Void
    {
        ...
        if (player.gems >= 100)
        {
            addButton(0, "Yes", meetingLunaRepeatedYes).hint("Having a maid sounds like a great addition to your camp.");
        }
        else
        {
            addButtonDisabled(0, "Yes", "You still don't have enough money to feel comfortable offering her work. Try again with at least 100 gems.");
        }
    ...
    public function meetingLunaCamp() : Void
    {
        ...
        {
            outputText(", or on your other companions milling about");
        }
