...
class KeltScene extends AbstractFarmContent
{
...
"KeltOff" - Turns off Kelt */
    ...
    private function bowSkill(diff : Float) : Float
    {
    ...
    private function keltRequiresNakedness() : Void
    {
        ...
        {
            simpleChoices("Reluctantly", keltReluctantlyGetNaked, "Eagerly", keltEagerlyGetNaked, "", null, "", null, "Never", keltRefuseNakedness);
        }
    ...
    private function keltEagerlyGetNaked() : Void
    {
        ...
        if (player.gender == 1 && player.biggestTitSize() < 1)
        {
            outputText("\"<i>What are you, a girl?  Get on with it so I can see what I'm dealing with.</i>\"\r\r");
        }
        else if (player.biggestTitSize() == 0)
        {
            outputText("\"<i>Oops!  Could have sworn I'd find a pair of tits on you.  That's okay... I'm sure you'll hit puberty some day!</i>\"\r\r");
        }
        //(Too small, A to DD:
        ...
        //(Medium, E to HHH:
        else if (player.biggestTitSize() <= 11)
        {
        ...
        else
        {
            ...
            {
                outputText("\"<i>Heh.  Just sporting the normal model, huh?  You know there's a phrase around here... hung like a human!  Trust me... it's not a compliment.</i>\"\r\r");
            }
            //(Dog Cock:
            if (player.cocks[0].cockType == CockTypesEnum.DOG)
            {
                outputText("\"<i>Oh, lookie here!  We got ourselves a little doggie dong!  Well, cock or not, you'll always be a bitch to me, mutt.</i>\"\r\r");
            }
            //(Horse Cock:
            if (player.cocks[0].cockType == CockTypesEnum.HORSE)
            {
                outputText("\"<i>Now that's just sad.  Getting a little envious of me, huh?  Had to go out and get a nice horsecock all your own?  Should have asked.  I'd have given you a taste of mine!</i>\"\r\r");
            }
            //(Tentacle Cock or other weirdness
            if (player.cocks[0].cockType.Index >= 3)
            {
                outputText("\"<i>Hah!  I'll bite... what the fuck are you supposed to be?  That a cock, or just an ugly, misplaced tail?  HA!</i>\"\r\r");
            }
            //(Small Size, ?-10 inches:
            if (player.cocks[0].cockLength <= 10)
            {
                outputText("\"<i>So, when you poke a woman with that, does she notice?  Or do you have to tell her when to start faking her orgasm?</i>\"\r\r");
            }
            //(Medium Size, 11-20 inches:
            else if (player.cocks[0].cockLength <= 20)
            {
                outputText("\"<i>Still, at least you've got a decent sized cock.  You know, for a colt.  Maybe you'll get lucky, and the real women will take pity on you!</i>\"\r\r");
            }
            //(Big Size, 21 inches and beyond:
            else
            {
                outputText("\"<i>Now if only they could find a way to get you to stop stepping on your own cock every other step, maybe you'd be a real man!  Can you even get it up, at this point?  Or would that tip you over?  Ha!</i>\"\r\r");
            }
        }
        ...
        dynStats("lus", as3hx.Compat.parseInt(player.lib / 10) + 5, "scale", false);
        keltMainEncounter2();
    }
    ...
    private function keltBlowjobRequirementNever(newl : Bool = true) : Void
    {
        spriteSelect(SpriteDb.s_kelt);
        if (newl)
        {
    ...
    private function keltBlowjobRequirementShamefully(newl : Bool = true) : Void
    {
        spriteSelect(SpriteDb.s_kelt);
        if (newl)
        {
    ...
    private function keltBlowjobRequirementEagerly(newl : Bool = true) : Void
    {
        spriteSelect(SpriteDb.s_kelt);
    ...
    private function keltMainEncounter() : Void
    {
        ...
        //(Submissive 0-30:
        if (player.statusEffectv2(StatusEffects.Kelt) <= 30)
        {
            outputText("You grind your teeth in irritation, but swallow your pride enough to ask him for help.  ");
        }
        //(Submissive 30-70:
        else if (player.statusEffectv2(StatusEffects.Kelt) <= 70)
        {
    ...
    //After naked requirement
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    outputText("You ignore his barbed comments as good as you can, and soon enough, he leaves the matter alone, instead critiquing your archery skills.\r\r");
                }
            }
        }
        keltMainEncounter2();
    ...
    private function keltMainEncounter3() : Void
    {
        ...
        else
        {
            ...
            if (player.inHeat && player.gender > 1)
            {
                ...
                if (player.statusEffectv2(StatusEffects.Kelt) <= 30)
                {
                    outputText("You struggle as good as you can, but Kelt weighs a good deal more than you do.  As his thrusting hips anxiously press his cock to your nether-lips, you realize this is going to happen, whether you want it to or not.  The thought fills you with an undeniable shiver of pleasure.\r\r");
                }
                //(Submissive, 30-70:
                else if (player.statusEffectv2(StatusEffects.Kelt) <= 70)
                {
                ...
                //(Vagina, Small:
                if (player.vaginalCapacity() <= 16)
                {
                    outputText("The fit seems impossible, but Kelt couldn't care less about your comfort, and shoves the flared head into you without hesitation.  You let out a scream, feeling like you are being torn apart by his cock as it rudely presses against your insides, spreading you wide open.  Kelt thrusts with growing frustration, but can fit no more than half his cock inside of you, despite his best efforts.  Every twitch makes you cry out as your tight pussy squeezes and milks that massive organ.\r\r");
                }
                //(Vagina, Medium:
                ...
                if (player.pregnancyType == PregnancyStore.PREGNANCY_KELT || player.pregnancy2Type == PregnancyStore.PREGNANCY_KELT)
                {
            ...
            if (player.biggestTitSize() == 0 && temporary == 3)
            {
                temporary = 4;
            }
            //(Naked, 20% Chance)
    ...
    //(Blowjob Requirement On)
    {
        ...
        //(Submissiveness 75+, Lust 60+){
            ...
            if (player.lust >= 75 || player.statusEffectv2(StatusEffects.Kelt) >= 90 && rand(2) == 0)
            {
                ...
                if (player.cockTotal() > 0)
                {
                    outputText("  Your [cock] is rock hard, but he ignores it almost contemptuously.  You almost feel ashamed of it, compared to the slowly-growing manhood between his legs.");
                }
                //(Vagina:
                if (player.hasVagina())
                {
                    outputText("  For a moment, his fingers trace the line of your exposed " + vaginaDescript(0) + ", giving you the slightest of warnings before he crudely jams two fingers deep inside, as though scouting out your depths.  You whimper urgently at the treatment, and more so as Kelt removes his fingers, licking with obvious pleasure.");
                }
                ...
                if (player.cockTotal() > 0)
                {
                    outputText("  Beneath you, pressed firmly into the harsh wood of the mounting board, your [cock] erupts, splattering your stomach with your own cum.  As the thick semen slides down towards your face, you begin slipping on your own warm seed, rocking back and forth with each harsh pounding Kelt delivers to your backside.");
                }
                //(Vagina:
                if (player.hasVagina())
                {
                ...
                if (player.looseness(false) < 3)
                {
                    outputText("\"<i>Fuck!  About time!  Sure got a tight little ass back here, but don't worry, slut.  We'll stretch it out in no time. I promise you this... you'll be getting a lot more of my cock from now on, so you'd better be ready!</i>\"\r\r");
                }
                else if (player.looseness(false) < 5)
                {
                    outputText("\"<i>Fuck!  About time!  Sure got a nice ass back here, but don't worry, slut.  We'll turn it into a real gaper soon.  It's a good thing you've been practicing... you'll be getting a lot more of my cock from now on, so you'd better be ready!</i>\"\n\n");
                }
                else
                {
                    outputText("\"<i>Fuck!  About time!  Sure got a nice, stretched out little pucker back here, but don't worry, slut.  I'll train into to squeeze down just right in no time.  I promise you this... you'll be getting a lot more of my cock from now on, so you'd better be ready!</i>\"\n\n");
                }
    ...
    private function keltTrainingPerksCheck() : Bool
    {
    ...
    private function keltTrainingPerks() : Void
    {
        ...
        {
            outputText("Had you been a centaur yourself you might have been able to learn something more useful from him then just aiming accuracy as Kelt technique seems to imitate something akin to mounted archery.");
        }
    }
    ...
    private function keltCentaurBadEnd() : Void
    {
        ...
        function assF() : Void
        {
            ...
            if (player.gender == 0)
            {
                outputText("Ha!  What the fuck are you supposed to be?  No cock, no cunt?  ");
            }
    ...
    private function keltResistance() : Void
    {
        ...
        if (player.faceType == Face.HORSE || player.faceType == Face.DOG)
        {
            outputText("a low growl building in the back of your throat");
        }
        else
        {
            outputText("fighting anger-fueled muscle-spasms");
        }
    ...
    public function fuckKeltsShitUp() : Void
    {
        ...
        if (player.faceType == Face.SNAKE_FANGS && player.tongue.type == Tongue.SNAKE && player.isNaga())
        {
            ...
            function nagaDickF(x : Int) : Void
            {
                ...
                if (player.cocks[x].cockLength < 10)
                {
                    if (player.hasBalls())
                    {
                        outputText("Your [balls] bump into his chin as your cock finds the back of his throat. ");
                    }
                    else
                    {
                        outputText("Your genital slit bumps into his chin as your cock finds the back of his throat. ");
                    }
        ...
        else
        {
            ...
            if (player.tallness < 50 || player.cockTotal() == 0)
            {
                ...
                {
                    outputText("Realizing you're too short to dominate him properly, you come up with another plan.  ");
                }
            ...
            function normalVagF() : Void
            {
                ...
                {
                    outputText("tasting them experimentally.  ");
                }
                else
                {
                    outputText("immediately becoming slick with your fuck-me-juices.  ");
                }
                ...
                if (player.cockTotal() > 0)
                {
                    outputText("Your [cock] splatters cock-cream into his hair, further humiliating him.");
                }
                outputText("\r\r");
                if (!recalling)
                {
            ...
            function normalDickF() : Void
            {
                ...
                if (player.cocks[0].cockLength < 10)
                {
                    outputText("taking the entire thing easily as his tongue licks you to ");
                }
                else if (player.cocks[0].cockLength < 25)
                {
                    outputText("taking the entire thing deep into his throat.  Apparently centaurs lack a gag-reflex, and you can see his throat stretched around you as you're brought to ");
                }
                else
                {
                    outputText("taking as much as he can deep into his throat.  Even though his entire neck is distorted by your massive member, his throat simply isn't long enough to take any more.  You writhe as the throat-fucking brings you to ");
                }
        ...
        var bothRaceEnd : Void->Void = function() : Void
        {
            if (!recalling)
            {
