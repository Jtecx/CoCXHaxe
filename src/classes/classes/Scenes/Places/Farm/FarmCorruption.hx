...
/**
	 ...
	 * @author Gedan
	 */
class FarmCorruption extends AbstractFarmContent
{
    ...
    public function corruptFollowers() : Int
    {
        ...
        if (SceneLib.ceraphScene.ceraphIsFollower() && !player.hasStatusEffect(StatusEffects.CeraphOff))
        {
            count++;
        }
        ...
        if (SceneLib.milkWaifu.milkSlave())
        {
            count++;
        }
    ...
    public function whitneyCorruption(mod : Float = 0) : Int
    {
        if (mod != 0)
        {
            ...
            if (flags[kFLAGS.WHITNEY_CORRUPTION] > flags[kFLAGS.WHITNEY_CORRUPTION_HIGHEST])
            {
                flags[kFLAGS.WHITNEY_CORRUPTION_HIGHEST] = flags[kFLAGS.WHITNEY_CORRUPTION];
            }
        }
        ...
        if (flags[kFLAGS.WHITNEY_CORRUPTION] < 0)
        {
            flags[kFLAGS.WHITNEY_CORRUPTION] = 0;
        }
        if (flags[kFLAGS.WHITNEY_CORRUPTION] >= 120)
        {
            flags[kFLAGS.WHITNEY_CORRUPTION] = 120;
        }
        trace("Whitney corruption changed by " + Std.string(mod));
        trace("Whitney corruption now at " + Std.string(flags[kFLAGS.WHITNEY_CORRUPTION]));
        return flags[kFLAGS.WHITNEY_CORRUPTION];
    }
    
    public function whitneyCorrupt() : Bool
    {
        return flags[kFLAGS.WHITNEY_CORRUPTION_COMPLETE] > 0;
    }
    
    public function whitneyDom() : Bool
    {
        return flags[kFLAGS.WHITNEY_DOM] == 1;
    }
    
    public function whitneyDefurred() : Bool
    {
        return flags[kFLAGS.WHITNEY_DEFURRED] == 1;
    }
    
    public function whitneyHasTattoo() : Bool
    {
        return flags[kFLAGS.WHITNEY_TATTOO_COLLARBONE] != 0 || flags[kFLAGS.WHITNEY_TATTOO_SHOULDERS] != 0 || flags[kFLAGS.WHITNEY_TATTOO_LOWERBACK] != 0 || flags[kFLAGS.WHITNEY_TATTOO_BUTT] != 0;
    }
    
    public function whitneyVagCapacity() : Float
    ...
    public function whitneyMaxedOralTraining() : Bool
    {
    ...
    // Early exit if we've not actually started the corruption process
    {
        ...
        {
            if (flags[kFLAGS.FOLLOWER_AT_FARM_AMILY_GIBS_MILK] == 0)
            {
                flags[kFLAGS.FOLLOWER_AT_FARM_AMILY_GIBS_MILK] = 1;
            }
        ...
        {
            if (flags[kFLAGS.FOLLOWER_AT_FARM_JOJO_GIBS_DRAFT] == 0)
            {
                flags[kFLAGS.FOLLOWER_AT_FARM_JOJO_GIBS_DRAFT] = 1;
            }
        ...
        if (flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 1 && flags[kFLAGS.FOLLOWER_PRODUCTION_VAPULA] == 1)
        {
            ...
            if (flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA_GIBS_MILK] == 0)
            {
                flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA_GIBS_MILK] = 1;
            }
        }
        ...
        if (flags[kFLAGS.FOLLOWER_AT_FARM_CERAPH] > 0)
        {
            ...
            if (flags[kFLAGS.FOLLOWER_AT_FARM_CERAPH] > 7)
            {
        ...
        if (flags[kFLAGS.FOLLOWER_AT_FARM_HOLLI] > 0)
        {
            ...
            if (flags[kFLAGS.FOLLOWER_AT_FARM_HOLLI] > 20)
            {
        ...
        if (flags[kFLAGS.FARM_UPGRADES_CONTRACEPTIVE] == 1)
        {
            ...
            {
                flags[kFLAGS.FARM_CONTRACEPTIVE_STORED] = 1;
            }
        }
    ...
    public function collectTheGoodies() : Void
    {
        ...
        if (flags[kFLAGS.FARM_CORRUPTION_DAYS_SINCE_LAST_PAYOUT] >= 7)
        {
            ...
            if (farmValue() < 25)
            {
                outputText(" You frown; it seems like a feeble amount for such a large operation. Perhaps you could talk to Whitney about that.\n\n");
            }
        ...
        if (flags[kFLAGS.FARM_SUCCUMILK_STORED] > 0)
        {
            ...
            if (flags[kFLAGS.FARM_SUCCUMILK_STORED] == 1)
            {
                outputText("vial of Succubi milk.");
            }
            else
            {
                outputText("vials of Succubi milk.");
            }
            outputText("</b>\n");
        ...
        if (flags[kFLAGS.FARM_INCUDRAFT_STORED] > 0)
        {
            ...
            if (flags[kFLAGS.FARM_INCUDRAFT_STORED] == 1)
            {
                outputText("vial of Incubi draft.");
            }
            else
            {
                outputText("vials of Incubi draft.");
            }
            outputText("</b>\n");
        ...
        if (flags[kFLAGS.FARM_EGG_STORED] > 0)
        {
            ...
            if (flags[kFLAGS.FARM_EGG_STORED] > 1)
            {
                outputText("s");
            }
            outputText(".");
    ...
    private function getItemObj(flag : Int) : Consumable
    {
        ...
        {
            return consumables.SUCMILK;
        }
        if (flag == kFLAGS.FARM_INCUDRAFT_STORED)
        {
            return consumables.INCUBID;
        }
        if (flag == kFLAGS.FARM_EGG_STORED)
        {
            return SceneLib.sophieBimbo.eggTypes[SceneLib.sophieBimbo.eggColors.indexOf(flags[kFLAGS.FOLLOWER_PRODUCTION_SOPHIE_COLORCHOICE])];
        }
        if (flag == kFLAGS.FARM_CONTRACEPTIVE_STORED)
        {
            return consumables.HRBCNT;
        }
    ...
    public function farmValue() : Int
    {
        ...
        if (flags[kFLAGS.WHITNEY_CORRUPTION_COMPLETE] == 1)
        {
            fValue *= 1.25;
        }
        return fValue;
    ...
    public function takeoverPrompt() : Bool
    {
        if (flags[kFLAGS.FARM_CORRUPTION_DISABLED] == 1)
        {
            return false;
        }
    ...
    private function takeoverPromptKelly() : Void
    {
    ...
    private function takeoverPromptMarbleRape() : Void
    {
        outputText("You stand at the top of a small rise overlooking the farm. From here you can just about pick out a beige-furred figure in the pepper field, hard at work. You shake your head almost in disbelief at the pastoral tableau. You remember when you found the farm when you were taking your first faltering steps in this land, with barely anything but the clothes you stood up in. You remember the relief you felt when you found this place, a pocket of peace in this disturbed land, how grateful you were to its owner to grub in the dirt with her and work for a pittance.");
    ...
    private function takeoverPromptGeneric() : Void
    {
    ...
    private function takeoverPromptMerge(firstTime : Bool = false) : Void
    {
    ...
    public function takeoverPromptNow() : Void
    {
        ...
        {
            outputText(", and to demonstrate your peacefulness, you theatrically hold up your [weapon] and then discard it with a careless swing of your arm.");
        }
        else
        {
            outputText(".");
    ...
    private function takeoverPromptLater() : Void
    {
        clearOutput();
    ...
    private function takeoverPromptNever() : Void
    {
        clearOutput();
    ...
    public function rootScene() : Void
    {
        ...
        if (!whitneyCorrupt())
        {
            ...
            if (flags[kFLAGS.FARM_UPGRADES_REFINERY] == 1)
            {
                outputText(" A large machine, bulky and rotund with a conical top, has been built into the milking barn. Fat pipes crawl up onto the roof from it like metal ivy, and white smoke billows busily out of its whistle chimney.");
            }
            
            if (flags[kFLAGS.FARM_UPGRADES_CONTRACEPTIVE] == 1)
            {
                outputText(" Next to the pepper patch another crop has been planted, a field of verdant green shrubs. Their thin stems bob idly in the breeze.");
            }
    ...
    public function farmMenu() : Void
    {
        ...
        if (flags[kFLAGS.WHITNEY_DISABLED_FOR_DAY] != 1)
        {
            if (!whitneyCorrupt())
            {
                addButton(0, "Whitney", dogeNotCorruptYet);
            }
            else
            {
                addButton(0, "Whitney", dogeCorruptedMissionComplete);
            }
        }
        ...
        {
            addButton(1, "Marble", farm.meetMarble);
        }
        ...
        if (player.hasStatusEffect(StatusEffects.Kelt) && !player.hasStatusEffect(StatusEffects.KeltOff))
        {
            if (flags[kFLAGS.KELT_BREAK_LEVEL] >= 4)
            {
                addButton(2, "Kelly", farm.kelly.breakingKeltOptions);
            }
            else if (flags[kFLAGS.KELT_BREAK_LEVEL] == 0 && flags[kFLAGS.KELT_TALKED_FARM_MANAGEMENT] == 0)
            {
                addButton(2, "Kelt", keltAChangeInManagement);
            }
            else
            {
                addButton(2, "Kelt", farm.kelly.breakingKeltOptions);
            }
        }
        //Breast Milker
        ...
        if (slavesAtFarm())
        {
            addButton(6, "Slaves", slavesAtFarmMenu);
        }
        if (loversAtFarm())
        {
            addButton(7, "Lovers", loversAtFarmMenu);
        }
        if (followersAtFarm())
        {
            addButton(8, "Followers", followersAtFarmMenu);
        }
    ...
    private function corruptingTheFarmExplore() : Void
    {
        ...
        if (flags[kFLAGS.FARM_CORRUPTION_DAYS_SINCE_LAST_PAYOUT] >= 7 || flags[kFLAGS.FARM_SUCCUMILK_STORED] > 0 || flags[kFLAGS.FARM_INCUDRAFT_STORED] > 0 || flags[kFLAGS.FARM_EGG_STORED] > 0 || flags[kFLAGS.FARM_CONTRACEPTIVE_STORED] > 0)
        {
            addButton(2, "Collect", collectTheGoodies);
        }
        
        addButton(14, "Back", farmMenu);
    }
    
    public function collectionAvailable() : Bool
    {
        return flags[kFLAGS.FARM_CORRUPTION_DAYS_SINCE_LAST_PAYOUT] >= 7 || flags[kFLAGS.FARM_SUCCUMILK_STORED] > 0 || flags[kFLAGS.FARM_INCUDRAFT_STORED] > 0 || flags[kFLAGS.FARM_EGG_STORED] > 0 || flags[kFLAGS.FARM_CONTRACEPTIVE_STORED] > 0;
    }
    ...
    private function slavesAtFarm() : Bool
    {
    ...
    private function slavesAtFarmMenu() : Void
    {
        ...
        if (flags[kFLAGS.FOLLOWER_AT_FARM_AMILY] == 1)
        {
            addButton(0, "Amily", SceneLib.amilyScene.amilyFollowerEncounter);
        }
        
        if (flags[kFLAGS.FOLLOWER_AT_FARM_JOJO] == 1)
        {
            addButton(1, "Jojo", SceneLib.jojoScene.corruptCampJojo);
        }
        
        if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 1 && SceneLib.sophieBimbo.bimboSophie())
        {
            addButton(2, "Sophie", SceneLib.sophieBimbo.approachBimboSophieInCamp);
        }
        
        if (flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 1)
        {
            addButton(3, "Vapula", SceneLib.vapula.callSlaveVapula);
        }
        
        if (flags[kFLAGS.FOLLOWER_AT_FARM_LATEXY] == 1)
        {
            addButton(4, flags[kFLAGS.GOO_NAME], SceneLib.latexGirl.approachLatexy);
        }
        
        if (flags[kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL] == 1)
        {
            addButton(5, flags[kFLAGS.MILK_NAME], SceneLib.milkWaifu.milkyMenu);
        }
    ...
    private function followersAtFarm() : Bool
    {
    ...
    private function followersAtFarmMenu() : Void
    {
        ...
        if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 1 && !SceneLib.sophieBimbo.bimboSophie())
        {
            addButton(0, "Sophie", SceneLib.sophieFollowerScene.followerSophieMainScreen);
        }
    ...
    private function loversAtFarm() : Bool
    {
    ...
    private function loversAtFarmMenu() : Void
    {
        ...
        if (flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 1)
        {
            addButton(0, "Izma", SceneLib.izmaScene.izmaFollowerMenu);
        }
        if (flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 2)
        {
            addButton(0, "Izmael", EventParser.gameOver);
        }
        
        if (flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 1)
        {
            addButton(1, "Isabella", SceneLib.isabellaFollowerScene.callForFollowerIsabella);
        }
    ...
    private function dogeNotCorruptYet() : Void
    {
        ...
        if (whitneyCorruption() < 30 && flags[kFLAGS.WHITNEY_CORRUPTION_HIGHEST] > 30 && flags[kFLAGS.WHITNEY_CORRUPTION_0_30_DROP_MESSAGE] == 0)
        {
            ...
            if (flags[kFLAGS.WHITNEY_LEAVE_0_60] == 0)
            {
    ...
    private function dogeNotCorruptYetMenu() : Void
    {
        ...
        if (whitneyCorruption() <= 60 && flags[kFLAGS.WHITNEY_LEAVE_0_60] == 0)
        {
            addButton(14, "Back", dogeNotCorruptLeaveFirstTime);
        }
        else if (whitneyCorruption() <= 90 && flags[kFLAGS.WHITNEY_LEAVE_61_90] == 0)
        {
            addButton(14, "Back", dogeNotCorruptLeave6190);
        }
    }
    
    public function availableInvestments() : Bool
    {
        return player.hasKeyItem("Breast Milker - Installed At Whitney's Farm") < 0 && flags[kFLAGS.QUEUE_BREASTMILKER_UPGRADE] == 0 || player.hasKeyItem("Cock Milker - Installed At Whitney's Farm") < 0 && flags[kFLAGS.QUEUE_COCKMILKER_UPGRADE] == 0 || flags[kFLAGS.FARM_UPGRADES_REFINERY] == 0 && flags[kFLAGS.QUEUE_REFINERY_UPGRADE] == 0 || flags[kFLAGS.FARM_UPGRADES_CONTRACEPTIVE] == 0 && flags[kFLAGS.QUEUE_CONTRACEPTIVE_UPGRADE] == 0 || flags[kFLAGS.FARM_UPGRADES_MILKTANK] == 0 && SceneLib.milkWaifu.milkSlave() && flags[kFLAGS.QUEUE_MILKTANK_UPGRADE] == 0;
    }
    ...
    private function whitneyAppearanceCorrupt() : Void
    {
        ...
        if (whitneyDefurred())
        {
            outputText("-girl. Her human transformation has rendered her pretty in a delicate, diffident kind of way.");
        }
        else
        {
            outputText("-morph. Her muzzle is suggestive of a golden retriever but really she could be any breed.");
        }
        if (whitneyMaxedOralTraining())
        {
            outputText(" In sharp contrast to her otherwise fairly thin features, her lips are plump and a depthless black, shining like wet tar. She constantly moves her tongue over them unconsciously.");
        }
        ...
        if (whitneyHasTattoo())
        {
            ...
            if (flags[kFLAGS.WHITNEY_TATTOO_COLLARBONE] != 0)
            {
                outputText(flags[kFLAGS.WHITNEY_TATTOO_COLLARBONE] + "\n");
            }
            if (flags[kFLAGS.WHITNEY_TATTOO_SHOULDERS] != 0)
            {
                outputText(flags[kFLAGS.WHITNEY_TATTOO_SHOULDERS] + "\n");
            }
            if (flags[kFLAGS.WHITNEY_TATTOO_LOWERBACK] != 0)
            {
                outputText(flags[kFLAGS.WHITNEY_TATTOO_LOWERBACK] + "\n");
            }
            if (flags[kFLAGS.WHITNEY_TATTOO_BUTT] != 0)
            {
                outputText(flags[kFLAGS.WHITNEY_TATTOO_BUTT] + "\n");
            }
        }
        ...
        if (!whitneyDefurred())
        {
            outputText("Her fur is sandy, dusking to black at her extremities");
        }
        else
        {
            outputText("Her skin is a sandy color, and she wears black nail varnish");
        }
    ...
    private function prosperityGoNotCorrupt() : Void
    {
        ...
        if (lowValue && lowProtection)
        {
            ...
            {
                outputText(" Send me any kind of decent body - someone, anyone - and I can start trying to make more-a this place.");
            }
            else
            {
                outputText(" I need more hands to make the best of the place. More suitable hands at least.");
            }
            outputText("</i>\"");
        }
    ...
    {
        menu();
        if (player.hasKeyItem("Breast Milker - Installed At Whitney's Farm") < 0 && flags[kFLAGS.QUEUE_BREASTMILKER_UPGRADE] == 0)
        {
            addButton(0, "Breast Milker", investmentBreastMilker);
        }
        if (player.hasKeyItem("Cock Milker - Installed At Whitney's Farm") < 0 && flags[kFLAGS.QUEUE_COCKMILKER_UPGRADE] == 0)
        {
            addButton(1, "Cock Milker", investmentCockMilker);
        }
        if (flags[kFLAGS.FARM_UPGRADES_REFINERY] == 0 && flags[kFLAGS.QUEUE_REFINERY_UPGRADE] == 0)
        {
            addButton(2, "Refinery", investmentRefinery);
        }
        if (flags[kFLAGS.FARM_UPGRADES_CONTRACEPTIVE] == 0 && flags[kFLAGS.QUEUE_CONTRACEPTIVE_UPGRADE] == 0)
        {
            addButton(3, "Contraceptive", investmentContraceptive);
        }
        if (flags[kFLAGS.FARM_UPGRADES_MILKTANK] == 0 && SceneLib.milkWaifu.milkSlave() && flags[kFLAGS.QUEUE_MILKTANK_UPGRADE] == 0)
        {
            addButton(4, "MilkTank", investmentMilktank);
        }
        
        if (!whitneyCorrupt())
        {
            addButton(14, "Back", dogeNotCorruptYetMenu);
        }
        else
        {
    ...
    private function investmentMilktank() : Void
    {
        ...
        else if (flags[kFLAGS.MILK_SIZE] > 0 && flags[kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL] == 1)
        {
            outputText("You tell Whitney you want her to give [bathgirlName] an intensive course of Gro-plus and Lactaid, and to build a small swimming tank to house her.");
        ...
        if (player.gems >= 400)
        {
            addButton(0, "Do it", doMilktankInvestment);
        }
        else
        {
            addButton(0, "Do it", turnDownInvestment, true);
    ...
    private function doMilktankInvestment() : Void
    {
        ...
        {
            outputText("You press the gems into your taskmaster’s hand, turn and leave without another word. Your thoughts turn to huge, delightfully plush, brown boobs, of luxurious milky baths; it will be well worth the cost. ");
        }
    ...
    private function turnDownInvestment(money : Bool = false) : Void
    {
        clearOutput();
        whitneySprite();
    ...
    private function dontDeFurDoge() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(" crotch into");
        }
        else
        {
            outputText(" hips against");
        }
    ...
    private function dogeCorruptedMissionComplete(output : Bool = true) : Void
    {
        ...
        if (flags[kFLAGS.FARM_CORRUPTION_BRANDING_MENU_UNLOCKED] == 1 || flags[kFLAGS.QUEUE_BRANDING_UPGRADE] < 1)
        {
            addButton(2, "Branding", brandingMenu);
        }
        if (whitneyDom())
        {
        ...
        {
            addButton(3, "Pleasure", whitneySubPleasure);
        }
        //Orgy room
        var massage : Function = null;
        if (flags[kFLAGS.FARM_UPGRADES_ORGYROOM] == 0 && flags[kFLAGS.QUEUE_ORGYROOM_UPGRADE] == 0 && !whitneyDom())
        {
            massage = wantOrgyRoom;
        }
        if (flags[kFLAGS.FARM_UPGRADES_ORGYROOM] == 1 && !whitneyDom() && !player.isGoo() && !player.isTaur() && !player.isGenderless())
        {
            massage = orgyRoomSubMassage;
        }
        if (massage != null)
        {
            addButton(4, "Massage", massage);
        }
        if (sceneHunter.other)
        {
            addButton(5, "Control", cheatSheet);
        }
        addButton(14, "Back", rootScene);
    }
    ...
    {
        clearOutput();
        if (flags[kFLAGS.WHITNEY_ORAL_TRAINING_COCK] == 0)
        {
            firstOralTraining();
        }
    ...
    private function cockOralTraining(herm : Bool = false) : Void
    {
        if (flags[kFLAGS.WHITNEY_ORAL_TRAINING_COCK] == 0)
        {
            firstCockOralTraining();
        }
        else if (flags[kFLAGS.WHITNEY_ORAL_TRAINING_COCK] <= 3)
        {
            cockOralTrainingStageOne();
        }
        else if (flags[kFLAGS.WHITNEY_ORAL_TRAINING_COCK] == 4)
        {
            firstCockOralTrainingStageTwo(herm);
        }
        else if (flags[kFLAGS.WHITNEY_ORAL_TRAINING_COCK] <= 9)
        {
            cockOralTrainingStageTwo(herm);
        }
        else if (flags[kFLAGS.WHITNEY_ORAL_TRAINING_COCK] == 10)
        {
            firstCockOralTrainingStageThree(herm);
        }
        else if (flags[kFLAGS.WHITNEY_ORAL_TRAINING_COCK] == 11)
        {
            cockOralTrainingStageThree(herm);
        }
        else
        {
            cockOralTrainingMaxed(herm);
        }
    ...
    {
        if (flags[kFLAGS.WHITNEY_ORAL_TRAINING_VAG] == 0)
        {
            firstVaginaOralTraining();
        }
        else if (flags[kFLAGS.WHITNEY_ORAL_TRAINING_VAG] <= 3)
        {
            vaginaOralTrainingStageOne();
        }
        else if (flags[kFLAGS.WHITNEY_ORAL_TRAINING_VAG] == 4)
        {
            firstVaginaOralTrainingStageTwo();
        }
        else if (flags[kFLAGS.WHITNEY_ORAL_TRAINING_VAG] <= 9)
        {
            vaginaOralTrainingStageTwo();
        }
        else if (flags[kFLAGS.WHITNEY_ORAL_TRAINING_VAG] == 10)
        {
            firstVaginaOralTrainingStageThree();
        }
        else if (flags[kFLAGS.WHITNEY_ORAL_TRAINING_VAG] == 11)
        {
    ...
    private function firstOralTraining() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(" cocksucker");
        }
        else
        {
            outputText(" pussy eater");
        }
        ...
        if (silly())
        {
            outputText(" \"<i>Do. Or do not. There is no try.</i>\"");
        }
        else
        {
            outputText(" \"<i>You will.</i>\"");
        }
    ...
    private function firstCockOralTraining() : Void
    {
        ...
        {
            sceneHunter.print("Failed check: need bigger dick!");
        }
    ...
    private function cockOralTrainingStageOne() : Void
    {
        ...
        if (player.biggestCockLength() > 12)
        {
            outputText(" She still struggles a bit with her gag reflex but it only takes her a few tries to swallow your thick prick, allowing you to pump into the delightful tightness of her throat without issue.");
        }
    ...
    private function firstCockOralTrainingStageTwo(herm : Bool) : Void
    {
        clearOutput();
        whitneySprite();
        ...
        if (herm && player.hasBalls())
        {
            ...
            if (player.cumQ() >= 1500)
            {
                outputText(" which is no mean feat, given it feels like you’ve fucked what feels like a gallon of it down her gullet. Once you’re almost done, as ever you spurt the last few creamy ropes across her face and clothes");
            }
            outputText(".");
        ...
        else if (player.hasBalls())
        {
            ...
            if (player.cocks.length > 1)
            {
                outputText(" The second cock trapped in her grasp flexes out cum just as eagerly as the first, sweat standing out on your forehead as you are clenched by your twin high; it paints her skirt with its generous discharge.");
            }
        }
        //Male without balls:
        else
    ...
    private function cockOralTrainingStageTwo(herm : Bool) : Void
    {
        clearOutput();
        whitneySprite();
        ...
        if (!herm)
        {
            ...
            if (player.cocks.length > 1)
            {
                outputText("She wraps her hand around your [cock biggest2] when she’s ready, giving it an experimental pump before beginning to softly and slowly work it, remembering exactly how to grip and bend its girth as she prepares for the main course.");
            }
            ...
            if (player.cocks.length > 1)
            {
                outputText("she occasionally forgets the length bulging in her hand so eager is she to push her lips further down your [cock biggest] and work at her own clit, and you need to murmur to get her to concentrate.");
            }
            else
            {
                outputText("she still occasionally forgets the length buried in her mouth, so eager is she to work keenly at her own clit, and you need to murmur to get her to concentrate.");
            }
            ...
            if (player.cocks.length == 1)
            {
                outputText("Whitney cleans your [cock biggest] with her flat tongue, humming as she laps your oozing head clean, deliciously soothing your aching cock.");
            }
            else
            {
                outputText("Whitney cleans first your [cock biggest2] and then your [cock biggest] with her flat tongue, humming as she laps each of your oozing heads clean, deliciously soothing your aching cock.");
            }
    ...
    private function firstCockOralTrainingStageThree(herm : Bool) : Void
    {
        clearOutput();
        whitneySprite();
        ...
        if (!herm)
        {
            ...
            else
            {
                ...
                if (player.cocks.length > 1)
                {
                    outputText(" jerking both your cocks with her smooth, exacting grasp in tandem");
                }
                outputText(".");
            }
            ...
            if (player.cocks.length > 1 || player.cumQ() >= 1500)
            {
                outputText("\n\n");
            }
            if (player.cocks.length > 1)
            {
                ...
                if (player.cocks.length > 2)
                {
                    outputText(" they flex");
                }
                else
                {
                    outputText(" it flexes");
                }
                ...
                {
                    outputText(" transcendent");
                }
                outputText(" high;");
                if (player.cocks.length > 2)
                {
                    outputText(" they paint");
                }
                else
                {
                    outputText(" it paints");
                }
        ...
        else if (player.hasVagina())
        {
            ...
            {
                outputText("the back of her mouth");
            }
            else
            {
                outputText("her gullet");
            }
            ...
            if (player.wetness() > 2)
            {
                outputText("soaking her arm as it gutters your juices, ");
            }
            outputText("heightening the ecstasy you feel to mind-blowing heights. ");
            if (player.cocks.length > 1)
            {
                outputText(" She doesn’t forget the cock trapped in her grasp for a moment either; she milks it as hard as you’re fucking her mouth, and it flexes out cum just as eagerly as the first, delirium seizing you as you are clenched by your triple high as it paints her skirt with its generous discharge.");
            }
            if (player.cumQ() >= 1500)
            {
                outputText(" Once you’re almost done, as ever you pull out and spurt the last few creamy ropes across her face and clothes. Red with her own huge high, the dog woman does her level best to catch them in her mouth.");
            ...
            if (player.cocks.length == 1)
            {
                outputText("Whitney first cleans your [vagina] and then your [cock biggest] with her flat tongue, polishing your dripping pussy and thighs to a shine before lapping your oozing head clean, deliciously soothing your aching cock.");
            }
            else
            {
                outputText("Whitney uses her flat tongue to clean first your [vagina], polishing your dripping pussy and thighs to a shine, and then each of your pricks, lapping the oozing heads clean, deliciously soothing your aching cocks.");
            }
    ...
    private function cockOralTrainingStageThree(herm : Bool) : Void
    {
        clearOutput();
        whitneySprite();
        ...
        if (player.cocks.length >= 2)
        {
            outputText(", allowing your other cock");
        }
        if (player.cocks.length > 2)
        {
            outputText("s");
        ...
        if (player.cocks.length >= 2)
        {
            outputText(" to unfurl and bob behind it");
        }
        if (player.hasVagina() && player.cocks.length >= 2)
        {
            outputText(" and");
        }
        if (player.hasVagina())
        {
            outputText(", leisurely exposing your moist [vagina]");
        }
        if (player.cocks.length >= 2 && player.hasVagina())
        {
        ...
        if (!herm)
        {
            ...
            if (player.cocks.length > 2)
            {
                outputText("Finally, she snakes her other hand around your [cock biggest3], rolling her wrist as she gently wrings it with the same sliding thoroughness.} You open your mouth to the shifting delectation inundating your overgrowth of cocks.");
            }
            ...
            if (player.hasBalls())
            {
                outputText(", your saliva-glossed [balls] beating against her chin");
            }
        ...
        else
        {
            ...
            if (player.cocks.length > 1)
            {
                outputText("As she bathes your [cock biggest] in delicious sensation she wraps her hand around your [cock biggest2], softening and then tightening her grip, sliding up and down its stem until it is every bit as rock-hard as the one she is lapping at. ");
            }
    ...
    private function cockOralTrainingMaxed(herm : Bool) : Void
    {
        clearOutput();
        whitneySprite();
        ...
        if (!herm)
        {
            ...
            if (player.cocks.length > 2)
            {
                outputText(" Finally, she snakes her other hand around your [cock biggest3], rolling her wrist as she gently wrings it with the same sliding thoroughness.");
            }
            ...
            if (player.cocks.length > 1)
            {
                outputText("In the long, delicious daze that follows you move her around");
            }
            if (player.cocks.length == 2)
            {
                outputText(", switching her attention to your [cock biggest2] for a while");
            }
            else if (player.cocks.length >= 3)
            {
                outputText(" switching her attention first to one cock and then the other");
            }
            if (player.cocks.length > 1)
            {
                outputText(" so that each of your fiercely erect pricks has time to enjoy the tight, sucking embrace of her mouth.");
            }
            ...
            if (player.hasBalls())
            {
                outputText(" and her tongue once again sliding across your [balls], ");
            }
            ...
            if (player.cocks.length > 1)
            {
                outputText(", her hands wrapping back around your other cock");
            }
            if (player.cocks.length > 2)
            {
                outputText("s");
            ...
            if (player.cocks.length > 1)
            {
                outputText(" as if they never left them");
            }
        ...
        else
        {
            ...
            {
                outputText(", switching her first to one cock then another");
            }
            if (player.cocks.length > 1)
            {
                outputText(" so that each of your fiercely erect pricks has time to enjoy the tight, sucking embrace of her mouth.");
            ...
            if (player.cocks.length > 1)
            {
                outputText(" and her other hand wrapping back around your [cock biggest2]");
            }
            ...
            if (player.cocks.length > 1)
            {
                outputText(" She doesn’t forget the [cock biggest2] trapped in her grasp for a moment; she milks it as hard as you’re fucking her mouth, and it flexes out cum just as eagerly as the first, delirium seizing you as you are clenched by your triple high; it paints her skirt with its generous discharge.");
            }
    ...
    private function firstVaginaOralTraining() : Void
    {
        ...
        {
            outputText(" despite the fact it must be practically poking her in the eye");
        }
    ...
    private function vaginaOralTrainingStageOne() : Void
    {
        ...
        {
            outputText(" The dog woman coughs and splutters around the lubricant you dribble into her, some of it dripping out of her nose even as you continue to push yourself into her. This doesn’t stop her from furiously moving her hand in her knickers, flicking her clit for all she is worth as she attempts to cope with the pressing demand of your snatch. On the edge already it doesn’t take her long, and her eyes roll as she tenses and shudders.");
        }
    ...
    private function firstVaginaOralTrainingStageTwo() : Void
    {
        ...
        {
            outputText(" dripping slightly");
        }
        else
        {
            outputText(" semi-erect");
        }
        ...
        {
            outputText(" obscene folds");
        }
        else
        {
            outputText(" sensitive points");
        }
        ...
        if (player.hasFuckableNipples())
        {
            outputText(" sends her tongue questing into its sensitive inside.");
        }
        else
        {
            outputText("bites it ever so gently.");
        }
        if (player.lactationQ() > 0)
        {
        ...
        if (player.wetness() <= 2)
        {
            outputText(" wetting both your slave’s hand and mouth");
        }
        else
        {
            outputText(" absolutely soaking your slave’s face and arm");
        }
        ...
        if (player.wetness() >= 5)
        {
            outputText(" which is no mean feat, given it feels like you gutter what feels like a gallon down her gullet. Once you’re almost done, as ever you get up and spurt the last of it on her hair and clothes");
        }
        outputText(".");
    ...
    private function vaginaOralTrainingStageTwo() : Void
    {
        ...
        if (player.lactationQ() > 0)
        {
            outputText(" and it quickly makes the liquid warmth in your breasts rise to a point, beading milk freely from your teats. Whitney laps and sucks it down eagerly, but gratifyingly she remembers your order and stops reluctantly after only taking a single pull");
        }
        outputText(", leaving your nipples");
        if (!player.hasFuckableNipples())
        {
            outputText(" rock-hard and");
        }
        ...
        if (player.wetness() >= 5)
        {
            outputText(", fountaining fem juice down her throat");
        }
        ...
        if (player.wetness() >= 5)
        {
            outputText(", soaking her again and again with warm, female approval");
        }
        outputText(".");
    ...
    private function firstVaginaOralTrainingStageThree() : Void
    {
        ...
        if (!player.hasFuckableNipples())
        {
            outputText(" she takes each of your [nipples] into her mouth to bathe them in close, sucking attention");
        }
        else
        {
            outputText(" she sends her tongue lapping into each of your cunt nipples, curling at their sensitive pink until they are wet with need");
        }
        ...
        if (player.lactationQ() > 0)
        {
            outputText(" It quickly makes the liquid warmth in your breasts rise to a point, beading milk freely from your teats. Whitney laps and sucks it down eagerly, but gratifyingly she remembers your order and stops reluctantly after only taking a single pull.");
        }
        ...
        {
            outputText(" She uses her new anchorage to take your bulging fem-dick entirely into her mouth, sucking on it avidly; she hums as she does it, sending ecstatic pleasure thrumming through you.");
        }
        ...
        if (player.wetness() >= 5)
        {
            outputText(", soaking her mouth and face again and again with warm, female approval");
        }
        ...
        if (player.wetness() >= 5)
        {
            outputText(" It is in a state of ecstatic momentum that you rise away from her mouth and spurt the last of it onto her hair and clothes. Red with her own huge high, the dog woman does her level best to catch your spotting fluids in her mouth.");
        }
    ...
    private function vaginaOralTrainingStageThree() : Void
    {
        ...
        if (player.lactationQ() > 0)
        {
            outputText(" It quickly makes the liquid warmth in your breasts rise to a point, beading milk freely from your teats. Whitney laps and sucks it down eagerly, whimpering as she withdraws after taking a single pull. You smile softly, knowing that the small, teasing taste of your sweetness only serves to make her thirstier.");
        }
        outputText(" She sinks downwards leaving your nipples");
        if (player.hasFuckableNipples())
        {
    ...
    private function vaginaOralTrainingMaxed() : Void
    {
        ...
        if (!player.hasFuckableNipples())
        {
            outputText(" she envelopes each of your [nipples] with her lips, kissing them with their sumptuous sponginess before bathing them in close, sucking attention");
        }
        else
        {
            outputText(" she pushes her lips against your cunt nipples before sending her tongue gently into each of them, curling at their sensitive pink until they are oozing wetness in delight");
        }
        ...
        {
            outputText("firm butt");
        }
        if (player.butt.type < 8)
        {
            outputText("supple rump");
        }
        else
        {
            outputText("soft ass");
    ...
    private function whitneyDomPleasure() : Void
    {
        ...
        else
        {
            ...
            if (!player.isTaur())
            {
                scenes.push(whitneyDomBondageOral);
            }
            if (player.hasCock() || player.hasVagina())
            {
    ...
    private function whitneyDomBondageOral() : Void
    {
        ...
        if (player.hasCock() && !player.hasVagina())
        {
            ...
            {
                outputText(" \"<i>So many things to play with tucked away in here!</i>\" comes her merry voice again, as you feel her grasp your [cock smallest] at the same time; you groan into the confines of her flesh as she gives it a soft, teasing pump. It’s obvious your main member has really caught her attention though, and both her hands soon return to it.");
            }
    ...
    private function whitneyDomStraponDoggy() : Void
    {
        ...
        {
            outputText(" Next, she pushes your ankles apart, threads a leather strap around each and then pulls them tight with a sharp yank; you carefully test them and find with a cold thrill that there is a metal bar attached between them, forcing you to stay in this position.");
        }
        else if (player.isDrider())
        {
            outputText(" You don’t struggle as she then forces your spindly legs beneath the bed, looping a leather strap attached to the frame around them all, trapping them there.");
        }
        else if (player.isNaga())
        {
            outputText(" She then ties your reptilian coils around the bedposts, securing them with leather straps fitted for the purpose.");
        }
        else
        {
            trace("Couldn't determine body type, welp.");
        }
        ...
        // wet and/or loose
        var capacity : Float = ((player.hasVagina())) ? player.vaginalCapacity() : player.analCapacity();
        if (capacity >= 36)
        {
            outputText("\n\nThe dog woman huffs in surprise as she slides her prosthetic length in with ease, your ");
            if (player.hasVagina())
            {
                outputText("deep, eager vagina");
            }
            else
            {
                outputText("oily, well-used passage");
            }
            ...
            if (player.hasVagina())
            {
                outputText("sit and spread ‘em for anything going, don’t you?");
            }
            else
            {
                outputText("I bet you get on your knees for anything with a nice, juicy cock, right?");
            }
        ...
        else
        {
            ...
            if (player.hasVagina())
            {
                outputText("take of your tight cunt");
            }
            else
            {
                outputText("give of your tight ass");
            }
            ...
            if (player.hasVagina())
            {
                outputText("servants, you tight, bossy little bitch");
            }
            else
            {
                outputText("cattle, you tight, bossy little boy-bitch");
            }
            outputText(".</i>\" ");
        ...
        if (player.hasVagina() && !player.hasCock() && SceneLib.vapula.vapulaSlave())
        {
            outputText(" You feel the end of the dildo tense deep within you before releasing, filling you full of warm demon spunk until it pools out onto the bed. The feeling is horribly, disgustingly satisfying, and for a moment you can see exactly how Vapula views the world.");
        }
    ...
    public function whitneyDomRide() : Void
    {
        ...
        if (player.isBiped())
        {
            outputText(" She does the same with your ankles, leaving you helplessly spread-eagled across the bed.");
        }
        else if (player.isDrider())
        {
            outputText(" She has long, leather straps attached to the underside of the bed; these are wrapped around the joining of your spindly legs to your heavy abdomen, leaving your chitinous feet pointing helplessly at the ceiling.");
        }
        else if (player.isGoo())
        {
            outputText(" \"<i>I can’t secure you properly,</i>\" she breathes, squeezing your gooey form, sinking her fingers into it. \"<i>Owing to this nonsense you call a body. So I will jus’ say it again. Lie. Still.</i>\"");
        }
        else if (player.isNaga())
        {
            outputText(" Your long, heavy tail is wrapped around the bedsteads at the foot of the bed and secured with more leather straps, leaving you helplessly spread-eagled across the bed.");
        }
        ...
        {
            var cockAuto : Int = as3hx.Compat.parseInt(player.cockThatFits(whitneyVagCapacity() * 1.33) + 1);
            if (cockAuto < 0)
            {
                cockAuto = player.smallestCockIndex();
            }
            selection(cockAuto);
        }
        ...
        {
            var x1 : Int = as3hx.Compat.parseInt(x + 1);  //it was here, so I don't want to move this shit.  
            var tooBig : Bool = player.cockArea(x) >= whitneyVagCapacity() * 1.33;
            if (tooBig)
            {
    ...
    private function brandingMenu() : Void
    {
        ...
        if (flags[kFLAGS.FARM_CORRUPTION_BRANDING_MENU_UNLOCKED] == 0)
        {
            ...
            if (player.gems >= 500)
            {
                addButton(0, "Do it", getBrandingStuff);
            }
            else
            {
                addButton(0, "Do it", dontGetBrandingStuff);
            }
        ...
        else
        {
            ...
            if (player.gems >= 50)
            {
                ...
                {
                    addButton(0, "Whitney", brandWhitney);
                }
                if (flags[kFLAGS.FOLLOWER_AT_FARM_AMILY] == 1 && hasFreeTattooSlot("amily"))
                {
                    addButton(1, "Amily", brandAmily);
                }
                if (flags[kFLAGS.FOLLOWER_AT_FARM_JOJO] == 1 && hasFreeTattooSlot("jojo"))
                {
                    addButton(2, "Jojo", brandJojo);
                }
                if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 1 && SceneLib.sophieBimbo.bimboSophie() && hasFreeTattooSlot("sophie"))
                {
                    addButton(3, "Sophie", brandBimboSophie);
                }
                if (flags[kFLAGS.FOLLOWER_AT_FARM_VAPULA] == 1 && hasFreeTattooSlot("vapula"))
                {
                    addButton(4, "Vapula", brandVapula);
                }
                if (flags[kFLAGS.KELT_BREAK_LEVEL] >= 4 && hasFreeTattooSlot("kelly"))
                {
                    addButton(5, "Kelly", brandKelly);
                }
                if (flags[kFLAGS.FOLLOWER_AT_FARM_BATH_GIRL] == 1 && hasFreeTattooSlot("milky"))
                {
                    addButton(6, flags[kFLAGS.MILK_NAME], brandMilky);
                }
            }
    ...
    private var slotNames : Array<Dynamic> = [
        "collarbone", 
        "shoulders", 
        "lower back", 
        "butt"
    ...
    private var tatooFlags : Dynamic = {
    ...
    {
        if (!tatooFlags.exists(name))
        {
            throw new Error("Unable to determine correct NPC flags for tatoo checks.");
        }
        var cnt : Int = 0;
        for (flag/* AS3HX WARNING could not determine type for var: flag exp: EArray(EIdent(tatooFlags),EIdent(name)) type: Dynamic */ in Reflect.field(tatooFlags, name))
        {
            if (flags[flag] != 0)
            {
                ++cnt;
    ...
    public function hasFreeTattooSlot(name : String) : Bool
    {
        return numTattoos(name) < Reflect.field(tatooFlags, name).length;
    }
    
    public function hasTattoo(name : String) : Bool
    {
    ...
    public function numTats(name : String, text : String) : Int
    {
        ...
        for (flag/* AS3HX WARNING could not determine type for var: flag exp: EArray(EIdent(tatooFlags),EIdent(name)) type: Dynamic */ in Reflect.field(tatooFlags, name))
        {
            if (Std.is(flags[flag], String) && flags[flag].indexOf(text) >= 0)
            {
                ++cnt;
            }
        }
    ...
    public function fullTats(name : String, text : String) : Bool
    {
        return numTats(name, text) == Reflect.field(tatooFlags, name).length;
    }
    
    public function fullTribalTats(name : String) : Bool
    {
    ...
    private function dontGetBrandingStuff() : Void
    {
        ...
        if (whitneyDom())
        {
            outputText(" \"<i>You’ll pay for that later, I swear!</i>\"");
        }
    ...
    public function brandSlotSelect(name : String, selectFun : Function) : Void
    {
        ...
        for (slot in Reflect.fields(Reflect.field(tatooFlags, name)))
        {
            if (Reflect.field(flags, Std.string(Reflect.field(Reflect.field(tatooFlags, name), slot))) == 0)
            {
                addButton(slot, capitalizeFirstLetter(Reflect.field(slotNames, slot)), selectFun, slot);
            }
        }
    }
    ...
    public function brandSelect(slot : Int) : Void
    {
        ...
        if (player.hasCock() && whitneyMaxedOralTraining())
        {
            addButton(4, "Cocksucker", champCocksuckerTattoo, slot);
        }
        if (player.hasVagina() && whitneyMaxedOralTraining())
        {
    ...
    public function amilyBrandSelect(slot : Int) : Void
    {
        ...
        if (player.hasCock() && slot == 2)
        {
            addButton(4, "Cock Here", amilyCockGoesHereTattoo, slot);
        }
        if (player.hasVagina())
        {
            addButton(5, "Mommy's Girl", amilyMommysGirlTattoo, slot);
        }
    }
    ...
    public function jojoBrandSelect(slot : Int) : Void
    {
        ...
        if (player.hasCock() && slot == 2)
        {
            addButton(4, "Cock Here", jojoCockGoesHereTattoo, slot);
        }
        if (player.hasVagina())
        {
            addButton(5, "Mommy's Boy", jojoMommysBoyTattoo, slot);
        }
    }
    ...
    public function bimboSophieBrandSelect(slot : Int) : Void
    {
        ...
        if (slot == 3)
        {
            addButton(5, "Wide Load", bimboSophieWideLoadTattoo, slot);
        }
        if (player.hasCock() && slot == 2)
        {
            addButton(6, "Cock Goes Here", bimboSophieCockGoesHereTattoo, slot);
        }
    }
    ...
    public function vapulaBrandSelect(slot : Int) : Void
    {
        ...
        if (slot == 2 || slot == 3)
        {
            addButton(4, "Buttslut", vapulaButtslutTattoo, slot);
        }
        if (!player.hasCock())
        {
            addButton(5, "Dildo Polisher", vapulaDildoPolisherTattoo, slot);
        }
    }
    ...
    public function kellyBrandSelect(slot : Int) : Void
    {
        ...
        if (silly())
        {
            addButton(4, "Dick Won", kellyDickWonTattoo, slot);
        }
        if (slot == 1)
        {
            addButton(5, "Horseshoe", kellyHorseshoeTattoo, slot);
        }
    }
    ...
    public function smallMilkyBrandSelect(slot : Int) : Void
    {
        ...
        if (slot == 0)
        {
            addButton(5, "Mega Milk", smallMilkyMegaMilkTattoo, slot);
        }
        if (slot == 0 && player.hasCock())
        {
            addButton(6, "Cock Cozy", smallMilkyCockCozyTattoo, slot);
        }
    }
    ...
    public function bigMilkyBrandSelect(slot : Int) : Void
    {
        ...
        if (slot == 0)
        {
            addButton(4, "Mega Milk", bigMilkyMegaMilkTattoo, slot);
        }
        if (slot == 0 && player.hasCock())
        {
            addButton(5, "Cock Cozy", bigMilkyCockCozyTattoo, slot);
        }
    }
    ...
    private function orgyRoomTalk() : Void
    {
        ...
        if (whitneyDom())
        {
            outputText("The sight sends a shuddering thrill down your spine. \"<i>You didn’t think I’d forget those, did you [master]? We both know how important they are for properly relaxing you,</i>\" Whitney murmurs, pushing her hot suppleness into your side. ");
        }
        else
        {
            outputText("You raise an eyebrow at these and Whitney giggles and blushes. \"<i>Just in case you’re feeling adventurous, [master]. Or, um, want help.</i>\"");
        }
        ...
        if (!whitneyDom())
        {
            outputText("\"<i>It’s exactly what a harem owner needs, [master] - with this you could take care of me and your other sluts all night, every night!</i>\"");
        }
        else
        {
            outputText("\"<i>I could do so many things to you [master], if you were on this stuff. And for so long. Just imagine it!</i>\"");
        }
    ...
    private function orgyRoomSubMassage() : Void
    {
        ...
        if (player.isBiped())
        {
            outputText(" you clamber onto the heated slab, placing your chin on your hands on the cushion at the end");
        }
        else if (player.isDrider())
        {
            outputText(" you tick-tack over and sink your spider underbelly onto the heated marble, before resting your human half down as well. Whitney was foresighted enough to make the slab quite long, and you complete the action in comfort");
        }
        else if (player.isNaga())
        {
            outputText(" you slither over and slide onto the heated slab, placing your chin on your hands on the cushion at the end. You allow your long, heavy tail to pool onto the floor below");
        }
        outputText(".");
        ...
        if (player.hasCock())
        {
            outputText(" grips your [cock]");
        }
        else if (player.hasVagina())
        {
            outputText(" traces the entrance of your [vagina]");
        }
        ...
        if (player.isBiped())
        {
            ...
            if (!(player.lowerBody == LowerBody.HOOFED || player.lowerBody == LowerBody.NAGA || player.lowerBody == LowerBody.GOO || player.lowerBody == LowerBody.PONY))
            {
                ...
                if (player.lowerBody == LowerBody.DEMONIC_CLAWS || player.lowerBody == LowerBody.LIZARD || player.lowerBody == LowerBody.HARPY || player.lowerBody == LowerBody.DRAGON)
                {
                    outputText(" claws");
                }
                else
        ...
        else if (player.isNaga())
        {
            ...
            {
                outputText(" You grin, flicking your forked tongue out to smell her arousal.");
            }
        ...
        if (player.lust >= 33)
        {
            outputText(" The hot tub bubbles quietly behind her and it is not just oil which is making your skin glow. This would be the moment to pursue a happy ending if you so wished.");
        }
        else
        {
            outputText(" You play with the idea of ‘rewarding’ her but... damn. She’s actually managed to relax you <i>too</i> much. You thank her regally, redress, and head out the door, enjoying the lightness of step and self-control which thrums through you.");
        }
    ...
    private function orgyRoomSubMassageHappyEnding() : Void
    {
        ...
        //[Male][Female]/[Next]
        menu();
        if (player.cockThatFits(whitneyVagCapacity() * 1.33) != -1)
        {
            addButton(0, "Male", subHappyEndingMale);
        }
        if (player.hasVagina())
        {
            addButton(1, "Female", subHappyEndingFemale);
        }
    }
    ...
    {
        clearOutput();
        var cockThatFits : Int = as3hx.Compat.parseInt(player.cockThatFits(whitneyVagCapacity() * 1.33) + 1);
        var hasBiggerCock : Bool = false;
        
        if (player.cockArea(cockThatFits - 1) != player.biggestCockArea())
        {
            hasBiggerCock = true;
        }
        ...
        {
            outputText(" against your [hips]");
        }
        else
        {
            outputText(" around your back");
        }
    ...
    private function subHappyEndingFemale() : Void
    {
        ...
        if (player.wetness() >= 5)
        {
            outputText(" Your pussy spurts excitement gleefully with each thrust, utterly drowning her own in your juices, polluting the water around you with your musk.");
        }
        if (player.hasCock())
        {
            outputText(" Whitney has stroked and tantalized your [cock biggest] with her hot hands and smooth belly intermittently this whole time, and as your orgasm builds and overwhelms you she gets her just reward for it; you jet cum from [eachCock], pasting first her face and then her tight breasts with your thick cream as you ride her hard.");
