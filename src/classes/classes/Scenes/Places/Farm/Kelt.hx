...
class Kelt extends Monster
{
    ...
    private function keltShootBow() : Void
    {
        ...
        var damage : Float = 0;
        damage = as3hx.Compat.parseInt((20 + str / 3 + 100) + spe / 3 - rand(player.tou) - player.armorDef);
        if (damage < 0)
        {
            damage = 0;
        }
    ...
    private function KellyuraAttack() : Void
    {
        ...
        if (select == 0)
        {
            outputText("Kelt flashes his cockiest smile and gestures downward.  \"<i>Did you forget why you're here, slut?  Taking me by surprise once doesn't make you any less of a whore.</i>\"");
        }
        //(2)
        else if (select == 2)
        {
            outputText("Grinning, Kelt runs by, trailing a cloud of his musk and pheremones behind you.  You have to admit, they get you a little hot under the collar...");
        }
        //(3)
        else
        {
    ...
    override private function performCombatAction() : Void
    {
        if (statusEffectv1(StatusEffects.BowCooldown) > 0)
        {
            ...
            if (statusEffectv1(StatusEffects.BowCooldown) <= 0)
            {
                removeStatusEffect(StatusEffects.BowCooldown);
            }
        }
        else if (rand(2) == 0 && flags[kFLAGS.KELT_BREAK_LEVEL] >= 2)
        {
            dayDreamKelly();
        }
        else
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        ...
        else if (game.flags[kFLAGS.KELT_BREAK_LEVEL] == 1)
        {
            SceneLib.farm.kelly.defeatKellyNDBREAKHIM();
        }
        else
        {
            SceneLib.farm.kelly.breakingKeltNumeroThree();
        }
    }
    ...
    {
        super();
        var breakLevel2 : Bool = game.flags[kFLAGS.KELT_BREAK_LEVEL] == 2;
        this.a = "";
        this.short = "Kelt";
        this.imageName = "kelt";
