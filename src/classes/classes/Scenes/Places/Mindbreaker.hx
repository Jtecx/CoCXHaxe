...
class Mindbreaker extends BaseContent implements SaveableState
{
    
    public static var MindBreakerQuest : Int;
    public static var QUEST_STAGE_MBOFF : Int = -1;
    public static inline var QUEST_STAGE_NOT_STARTED : Int = 0;
    public static inline var QUEST_STAGE_METMB : Int = 1;
    public static inline var QUEST_STAGE_ISMB : Int = 2;
    ...
    public static var PlayerEggIsFertile : Bool;
    
    public static var MindbreakerPrisoner : Bool;
    public static var MindbreakerPrisonerCooldown : Int;
    public static var MindbreakerEncounterChance : Float;
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            MindBreakerQuest = Reflect.field(o, "stage");
            
            MindBreakerConvert = Reflect.field(o, "numberOfConvert");
            MindBreakerConvertGoal = Reflect.field(o, "numberOfConvertGoal");
            MindBreakerGoblinConvert = Reflect.field(o, "numberOfGoblinConvert");
            MindBreakerFetishFemaleConvert = Reflect.field(o, "numberOfFetishFemaleConvert");
            MindBreakerFetishMaleConvert = Reflect.field(o, "numberOfFetishMaleConvert");
            
            MindBreakerFullConvert = Reflect.field(o, "numberOfFullConvert");
            MindBreakerFemaleFullConvert = Reflect.field(o, "numberOfFullFemaleConvert");
            MindBreakerMaleFullConvert = Reflect.field(o, "numberOfMaleFullConvert");
            MindBreakerMiniFullConvert = Reflect.field(o, "numberOfMiniFullConvert");
            
            PlayerEggIsFertile = Reflect.field(o, "playerEggIsFertile");
            MindbreakerPrisoner = Reflect.field(o, "mindbreakerPrisoner");
            if (Lambda.has(o, "MindbreakerPrisonerCooldown"))
            
            // new save, can load{
                
                MindbreakerPrisonerCooldown = Reflect.field(o, "mindbreakerPrisonerCooldown");
            }
            ...
            MindbreakerEncounterChance = valueOr(Reflect.field(o, "MindbreakerEncounterChance"), 1);
        }
    ...
    public function YesTentacle() : Void
    {
        ...
        if (!player.hasDarkVision())
        {
            outputText(", leaving you with nothing but your keen senses");
        }
    ...
    public function YesTentacleStage2() : Void
    {
        ...
        {
            outputText(" laying cross your bosom");
        }
        outputText("." +
    ...
    public function YesTentacleStage3() : Void
    {
        ...
        if (player.hasCock() && !player.hasVagina())
        {
            ...
            {
                player.killCocks(player.cocks.length - 1);
            }
    ...
    public function CaveLayout(back : Bool = false) : Void
    {
        clearOutput();
        if (back)
        {
        ...
        if (MindBreakerConvert == 1)
        {
            outputText("It seems a new friend already made it to the cave and is going through the re-education process.");
        }
        else if (MindBreakerConvert >= 2)
        {
            outputText(" In a corner you can see that your many new friends are going through the re-education process.");
        }
        if (MindBreakerGoblinConvert == 1)
        {
            outputText(" You see the goblin addition masturbating with a newly designed tentacle earplug toy in the corner.");
        }
        else if (MindBreakerGoblinConvert >= 2)
        {
            outputText(" You see a few goblins masturbating with some newly designed tentacle earplug toys in the corner, their combined juices making a lovely puddle on the floor.");
        }
        if (MindBreakerFetishFemaleConvert >= 1)
        {
            ...
            else if (MindBreakerFetishFemaleConvert >= 2)
            {
                outputText(" The female cultists you converted sit quietly in a corner of the cave practicing your teachings by themselves. It occurs to you some of them could use a good earfucking. Maybe you could even make proper mindbreakers out of some of them?");
            }
            else
            {
                outputText(" The female cultist you converted sits quietly in a corner of the cave, practicing your teachings by herself. It occurs to you that she could use a good earfucking. Maybe you could even make a proper mindbreaker out of her?");
            }
        }
        if (MindBreakerFetishMaleConvert >= 1)
        {
            ...
            else if (MindBreakerFetishMaleConvert >= 2)
            {
                outputText(" The male cultists you converted sit quietly in a corner of the cave practicing your teachings by himself. Of course witheout the help of a mindbreaker there is absolutely no way for them to cum so they just end up masturbating pointlessly. Maybe they hope that their next true orgasm blows their mind out?");
            }
            else
            {
                outputText(" The male cultist you converted sit quietly in a corner of the cave practicing your teachings by himself. Of course witheout the help of a mindbreaker there is absolutely no way for him to cum so they just end up masturbating pointlessly. You suspect that his next true orgasm will literally blow his mind out.");
            }
        }
        ...
        if (back)
        {
            outputText("Kaerb Dnim isn't too far in case you want to ask something of her.");
        }
        else
        {
            ...
            if (MindBreakerConvert >= 1)
            {
                outputText(" As you can see, your followers have been awaiting your return eagerly.");
            }
            outputText("</i>\"");
        }
        menu();
    ...
    //if (player.hairType != Hair.MINDBREAKER) addButton(1, "Fix Me", Regaintentacles);
    {
        ...
        if (MindBreakerGoblinConvert < 1)
        {
            addButtonDisabled(2, "Use gob", "You need at least one goblin follower to do this.");
        }
        ...
        if (MindBreakerFetishMaleConvert < 1)
        {
            addButtonDisabled(4, "Use boy", "You need at least one male follower to do this.");
        }
        if ((MindBreakerGoblinConvert >= 1 || MindBreakerFetishFemaleConvert >= 1) && (PlayerEggIsFertile || player.hasCock()))
        {
            addButton(5, "Convert", CreateMB).hint("Make one of your female pets into a new Mindbreaker.");
        }
        else
        {
            addButtonDisabled(5, "Convert", "You need at least one follower in order to get create a new mindbreaker. Furthermore you need your parasitic eggs to be properly fertilised by a male follower's cum...or preferably, that of a male mindbreaker.");
        }
        ...
        if (MindBreakerConvert >= 10)
        {
            addButton(8, "Prisoner", BreakAPrisonner).hint("It appears your followers brought in a new toy for you to break how kind. Time to fix the mind of this defiant new plaything.");
        }
        else if (MindBreakerConvert >= 10 && !MindbreakerPrisoner)
        {
            addButtonDisabled(8, "Prisoner").hint("There is no prisoners in the cave for you to play with at the time.");
        }
        addButton(10, "More?", totalSubservience).hint("Ask if you can do even more for the Mindbreakers.");
    ...
    public function SexCaerbDnim() : Void
    {
        ...
        var sharedEnd : Void->Void = function() : Void
        {
            ...
            {
                outputText("The two of you slowly disentangle from each other after briefly relishing the afterglow together.");
            }
    ...
    public function GoblinTentacle() : Void
    {
        ...
        if (MindBreakerGoblinConvert == 1)
        {
            outputText("your favorite goblin slave");
        }
        else
        {
            outputText("one of your favorite of many goblin slaves");
        }
    ...
    public function FuckBoy() : Void
    {
        ...
        if (MindBreakerFullConvert >= 2)
        {
            outputText(" and the group of mindbreakers");
        }
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            if (MindBreakerFullConvert >= 2)
            {
                outputText(" letting another mindflayer have her turn");
            }
            else
            {
                outputText(" letting Kaerb Dnim have her turn");
            }
            if (MindBreakerFetishFemaleConvert >= 1)
            {
                outputText(", as well as any possible female slave in need");
            }
            outputText("." +
        ...
        var dickF : Void->Void = function() : Void
        {
            ...
            if (MindBreakerFullConvert >= 2)
            {
                outputText(" Several eager mindflayers take turn");
            }
            else
            {
                outputText(" Kaerb Dnim takes her time");
            }
    ...
    public function CreateMB() : Void
    {
        clearOutput();
        PlayerEggIsFertile = false;
        if (MindBreakerFetishMaleConvert == 0)
        {
            convertMale();
        }
        else if (MindBreakerFetishFemaleConvert == 0)
        {
            convertFemale();
        }
        else
        ...
        //Female Scene
        {
            ...
            if (MindBreakerFetishFemaleConvert == 1)
            {
                outputText("your female slave.");
            }
            else
            {
                outputText("one of the many female slaves in the lair.");
            }
        ...
        //Male Scene
        {
            ...
            if (MindBreakerFetishFemaleConvert == 1)
            {
                outputText("your male slave.");
            }
            else
            {
                outputText("one of the many male slaves in the lair.");
            }
    ...
    public function BreakAPrisonner() : Void
    {
        ...
        MindBreakerFetishFemaleConvert++;
        MindBreakerConvert++;
        if (MindBreakerConvert == MindBreakerConvertGoal)
        {
            cast((false), MindbreakerBrainEvolution);
        }
    ...
    public function MindbreakerBrainEvolution(isOutOfCave : Bool = true) : Void
    {
        ...
        if (isOutOfCave)
        {
            outputText(", ordering your dazed toy to head to the cave where Kaerb Dnim will provide it with further training.");
        }
        else
        {
            outputText(".");
        ...
        else
        {
            ...
            var newRank : Int = as3hx.Compat.parseInt(player.perkv1(PerkLib.MindbreakerBrain1toX) + 1);
            player.setPerkValue(PerkLib.MindbreakerBrain1toX, 1, newRank);
        }
        MindBreakerConvertGoal = as3hx.Compat.parseInt(MindBreakerConvert + 20);
        if (isOutOfCave)
        {
    ...
    private function totalSubservience() : Void
    {
        ...
        var refuseDeepOne : Void->Void = function() : Void
        {
            clearOutput();
            outputText("Perhaps another time.");
            cast((true), CaveLayout);
        }
    }
}
