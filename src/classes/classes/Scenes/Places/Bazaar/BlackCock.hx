...
/**
	 ...
	 * @author Kitteh6660
	 */
class BlackCock extends BazaarAbstractContent
{
    ...
    public function enterTheBlackCock() : Void
    {
        ...
        if (isHalloween())
        {
            outputText("The interior is lit by several carved Jack O'Lanterns on each table, emitting a warm glow from flickering flames that cast dancing shapes along the walls.");
        }
        else if (isChristmas())
        {
            outputText("The interior is lit by a combination of black rooster-shaped candles on each table and a large globe sitting in the center of the tent. It looks like they just dumped the glowing creamy fluid from all the small globes into a larger one. You also see a big festive tree tucked into the corner decorated with glowing frosting… oh... wait, that's not frosting.");
        }
        else
        {
            outputText("The interior is lit by what looks like round globes on each table, each of which is filled with a thick creamy liquid that emits a pale glow.");
        }
        //Staff descriptor
    ...
    private function checkFoodMenu() : Void
    {
        ...
        {
            outputText("You sit on one of the vacant chairs surrounding a table.");
        }
        else
        {
            outputText("You " + ((player.isNaga()) ? "slither" : "walk") + " to a vacant spot not occupied by any chair.");
        }
    ...
    private function eatFoodNomNom(foodName : String, foodEatDesc : String) : Void
    {
        ...
        switch (foodName)
        {
            ...
            case "Giant Burger":
                ...
                {
                    player.modThickness(player.maxThicknessCap(), 4);
                }
                if (rand(3) == 0 && player.hairLength < 16)
                {
                    player.hairLength += 0.5;
                }
            case "P. Potatoes":
                player.refillHunger(70);
                if (flags[kFLAGS.HUNGER_ENABLED] == 0)
                {
                    player.modThickness(player.maxThicknessCap(), 2);
                }
                if (flags[kFLAGS.HUNGER_ENABLED] == 0 || (rand(2) == 0 && player.hunger >= (player.maxHunger() * 0.8)))
                {
                    player.butt.type++;
                }
            case "Spicy Chilli":
            ...
            case "G.Ham Dinner":
                ...
                if (flags[kFLAGS.HUNGER_ENABLED] == 0)
                {
                    player.modThickness(player.maxThicknessCap(), 5);
                }
            case "MysteryMeat":
                player.refillHunger(50);
                mysteryMeatTFs();
        ...
        {
            player.createStatusEffect(StatusEffects.Fullness, 4, 0, 0, 0);
        }
        else
        {
            player.changeStatusValue(StatusEffects.Fullness, 1, 4);
        }
    ...
    private function meetAnitaRoswell() : Void
    {
        ...
        if (player.lust >= 33)
        {
            addButton(1, "Sex", rompUpAnitaOffer);
        }
        else
        {
            addButtonDisabled(1, "Sex", "You aren't aroused enough to do that.");
        }
    ...
    private function eagleFuckAnita() : Void
    //Eagle fuck, must have wings
    {
        ...
        var dickF : Void->Void = function() : Void
        {
            ...
            var scene : Int->Void = function(x : Int) : Void
            {
                ...
                if (player.cumQ() >= 2500)
                {
                    outputText("Your chest swells with pride as you watch Anita fly, her snatch trailing a sticky trail of your glistening baby-batter as her swollen stomach and loosened lady parts empty.");
                }
    ...
    private function vaginalFuckAnita() : Void
    //Vaginal fuck, must have penis
    {
    ...
    private function sex69Anita() : Void
    //69 with Anita, must not be a taur.
    {
        ...
        {
            if (player.cocks[x].cockLength >= 25)
            {
                dickF(x);
            }
            //nofit
            ...
            {
                sceneHunter.callBigSmall(dickF, 13, 7, "length", 25);
            }
        };
        ...
        //Males and hermaphrodites
        {
            ...
            else if (player.cocks[x].cockLength >= 7 && player.cocks[x].cockLength < 13)
            {
            ...
            else if (player.cocks[x].cockLength >= 13 && player.cocks[x].cockLength < 25)
            {
            ...
            if (player.hasVagina())
            {
                outputText("She even begins to finger your [vagina], The added sensation pushes you over the edge and your [cock " + x1 + "] swells as you reach climax. ");
            }
            else
            {
                outputText("She begins to suck harder, using her hands to massage your [legs] and pull you harder against her. Your [cock " + x1 + "] slides, free of her mouth as she kisses its rigid underside. The added sensation pushes you over the edge and your [cock " + x1 + "] swells as you reach climax. ");
            }
            if (player.cumQ() >= 250 && player.cumQ() < 1000)
            {
                outputText("You feel the hot evidence of your climax hit your jaw as you unload, painting hot streaks of cum from your chin to her perky tits.");
            }
            else if (player.cumQ() >= 1000 && player.cumQ() < 2500)
            {
                outputText("You feel the hot evidence of your climax slam into your chin, a huge glob of your cum painting your jaw white. You groan as you orgasm rages on, painting both your bodies in a liberal coating of jism as Anita continues to lick the belly of your [cock " + x1 + "].");
            }
            else if (player.cumQ() >= 2500)
            {
                outputText("A tidal wave of baby-batter fills your vision with white, your [cock " + x1 + "] expelling gigantic torrents of seed with every contraction that rains back down to the earth. By the time your orgasm has subsided you're quietly convulsing with you and Anita covered in spunk and surrounded by a pool of hot seed.");
            }
    ...
    private function smokeSomeHookah() : Void
    {
        ...
        switch (rand(2))
        {
            ...
            case 1:
                ...
                if (flags[kFLAGS.TIMES_TRANSFORMED] < 3 || player.isRace(Races.HUMAN, 1, false))
                {
                    outputText("You nod, telling him that you look the same as when you walked through the portal.");
                }
                else
                {
                    outputText("You shake your head and tell the curious satyr you've changed since coming to Mareth.");
                }
        ...
        player.gems -= 5;
        statScreenRefresh();
        player.buff("AndySmoke").forHours(7).addStats({
                            int.mult : 0.05,
                            spe.mult : -0.05
                        }).withText("Andy Smoke");
    ...
    private function grabAndysDongAfterSmoking() : Void
    {
        ...
        var assF : Void->Void = function() : Void
        {
            ...
            if (player.isBiped())
            {
                outputText("You spread your legs as you settle down and take a deep hit, enjoying the wet teasing fingers and the lung full of warm joy.");
            }
            else if (player.isNaga())
            {
                outputText("You wrap your coils around his waist, enjoying the wet teasing fingers and the lung full of warm joy.");
            }
    ...
    private function dontSmokeItsBad() : Void
    ...
    private function rompUpWithAndyTheSatyr() : Void
    {
        ...
        if (player.hasCock())
        {
            addButton(0, "Fuck Him", fuckAndyTheSatyr_selector).hint("Take Andy from behind and put your cock to good use.");
        }
        else
        {
            addButtonDisabled(0, "Fuck Him", "You will need a penis for that.");
        }
    ...
    private function fuckAndyTheSatyr(x : Int) : Void
    {
        ...
        if (player.isTaur())
        {
            ...
            if (player.hasBalls())
            {
                outputText("Your [balls] churn with your need for release as hot tingles shoot up your spine. ");
            }
            if (player.hasVagina())
            {
                outputText("Even your [vagina] feels fit to explode as you mercilessly pound the sexy little satyr’s furry rump.");
            }
            ...
            if (player.cockTotal() > 1)
            {
                ...
                if (player.cumQ() >= 2500)
                {
                    outputText("His belly swells dangerously as each of your dicks spits, creating a puddle of mixed man cream as Andy passes out beneath you. ");
                }
            }
            else
            {
                ...
                if (player.cumQ() >= 2500)
                {
                    outputText("From your vantage point above the two of you you can see Andy’s belly swelling dangerously. Baby batter leaks from his wrecked hole as well, creating an impressive puddle while making him look pregnant at the same time. ");
                }
            }
        }
        else
        {
            ...
            if (player.cocks[x].cockLength < 7)
            {
                ...
                {
                    outputText("He takes your remaining dick into his hand and fists it with a sure and practiced hand. ");
                }
                else if (player.cockTotal() > 2)
                {
                    outputText("He takes two of your remaining dicks and fists them with two sure and practiced hands. ");
                }
            ...
            else if (player.cocks[x].cockLength >= 7 && player.cocks[x].cockLength < 12)
            {
                ...
                if (player.cockTotal() == 2)
                {
                    outputText("He takes your remaining [cock 2] into his hand and fists it with a sure and practiced hand. ");
                }
                else if (player.cockTotal() > 2)
                {
                    outputText("He takes two of your remaining dicks and fists them with two sure and practiced hands. ");
            ...
            if (player.hasKnot())
            {
                ...
                if (player.cumQ() >= 2500)
                {
                    outputText("You watch with relish as the little guy’s stomach swells, your baby batter filling him up from the back door and pushing his still bent legs forward, forcing him to let them go. ");
                }
            ...
            else
            {
                ...
                if (player.cumQ() >= 2500)
                {
                    outputText("Your body trembles as you soak his innards with blast after blast of ejaculent, some of which you can feel streaming down your [legs] as the little guy’s stomach swells with however much of your seed doesn’t fall into the puddle you’re creating below. ");
                }
    ...
    private function haveAndyFuckYou(isVaginal : Bool) : Void
    {
        var vagOrAss : String = (isVaginal) ? "[vagina]" : "[ass]";
        clearOutput();
        if (isVaginal)
        {
            outputText(images.showImage("andy-sex-vaginalcatch"));
        }
        else
        {
            outputText(images.showImage("andy-sex-analcatch"));
        }
        var ANDY_COCK_AREA : Int = as3hx.Compat.parseInt(12 * 1.5);  //18  
        ...
        //Tauric lower body{
            ...
            if (player.looseness(isVaginal) < 2)
            {
                ...
                if (isVaginal)
                {
                    player.cuntChange(ANDY_COCK_AREA, true);
                }
                else
                {
                    player.buttChange(ANDY_COCK_AREA, true);
                }
            ...
            //Genitals check
            if (player.hasCock() && player.hasVagina() && isVaginal)
            ...
            //Hermaphrodite{
                ...
                if (player.cumQ() >= 1000 && player.cumQ() < 2500)
                {
                    outputText("As you unload your cum pools around your [feet], spreading out in a giant puddle beneath you.");
                }
                else if (player.cumQ() >= 2500)
                {
                    outputText("As you unload you feel [eachCock] spasm and coat your [legs] and [feet] in hot pearly goo. Your orgasm rages on for what seems like an eternity, [eachCock] creating an ocean of melted pearl that stretches out and covers the ground around you.");
                }
            ...
            //Male{
                ...
                if (player.cumQ() >= 1000 && player.cumQ() < 2500)
                {
                    outputText("As you fire blast after blast of baby batter across the floor the cum spewing from your body pools around your [feet], leaving a pearly puddle below you.");
                }
                else if (player.cumQ() >= 2500)
                {
                    outputText("Your cum explodes out of [eachCock] like fireworks. Groaning like a dying man your shiver in ecstasy as your cum soaks the earth beneath your [feet], spreading out like a small lake made of moonlight.");
                }
                outputText("\n\nThere you are breathing like you just ran a marathon. You feel an incredible amount of satyr spunk sloshing around in your horse half. Your body feels heavy but at the same time you’re blissfully aware of the glorious post orgasmic ache spreading throughout your body. You can feel Andy lying against your horse half, catching his breath before he stands and tugs his dog cock. You canter back, his swollen knot lodged inside your [ass] and holding the two of you together.");
            }
            else if (player.hasVagina() && isVaginal)
        ...
        else
        {
            ...
            //Hermaphrodite{
                ...
                if (player.cumQ() >= 250 && player.cumQ() < 1000)
                {
                    outputText("As you unload Andy opens his mouth to catch your seed as it paints his chest and chin in streaks of pearly white baby batter.");
                }
                else if (player.cumQ() >= 1000 && player.cumQ() < 2500)
                {
                    outputText("As you unload the Andy opens his mouth to catch your seed as it paints his chest, chin and abs in a thick coating of white goo.");
                }
                else if (player.cumQ() >= 2500)
                {
                    outputText("As you unload the satyr practically drowns in your baby batter, coated from horns to abs in a heavy coating of white spooge that pools around the two of you like a lake of melted pearls.");
                }
            ...
            //Male or herm choosing anal{
                ...
                {
                    outputText("As you fire blast after blast of baby batter across Andy’s body you manage to liberally coat his entire form from his chin to his abs, [eachCock] spent as you drool out your last bit of spunk.");
                }
                else if (player.cumQ() >= 2500)
                {
                    outputText("Your cum explodes out of [eachCock] like fireworks and as you cry out so does Andy, sputtering as your titanic orgasm leaves him covered head to abs in your copious load.");
                }
            ...
            //Genderless or female choosing anal
            else
            {
        ...
        if (isVaginal)
        {
            ...
            {
                player.knockUp(PregnancyStore.PREGNANCY_SATYR, PregnancyStore.INCUBATION_SATYR, 50);
            }
        }
        else
        {
            player.buttKnockUp(PregnancyStore.PREGNANCY_SATYR, PregnancyStore.INCUBATION_SATYR, 50);
        }
    ...
    private function meetHarryRoswell() : Void
    {
        ...
        if (player.lust >= 33)
        {
            addButton(0, "Sex", rompUpWithHarry);
        }
        else
        {
            addButtonDisabled(0, "Sex", "You aren't horny enough to consider this.");
        }
    ...
    private function haveHarryFuckYou() : Void
    {
        ...
        var HARRY_COCK_AREA : Int = as3hx.Compat.parseInt(18 * 3);  //54  
        ...
        if (player.analCapacity() < 40)
        {
            ...
            //Hermaphrodite{
                ...
                if (player.hasBalls())
                {
                    outputText("His powerful grip begins to slide down your [cock]. They continue down to your [balls] and give them a firm but gentle squeeze, one that sends you bucking back into his giant cock as it spills gobs of precum onto your [ass]. ");
                }
                else
                {
                    outputText("His powerful grip slides down your [cock] with firm squeezes, his thumb rubbing the top of your dick before it travels south. ");
                }
            ...
            //Male{
                ...
                if (player.hasBalls())
                {
                    outputText("His powerful grip begins to slide down your [cock]. They continue down to your [balls] and give them a firm but gentle squeeze, one that sends you bucking back into his greasy cock as it spills gobs of precum onto your [ass]. ");
                }
                else
                {
                    outputText("His powerful grip slides down your [cock] with firm squeezes, his thumb rubbing the top of your dick as it travels south probably searching for a pair of gonads you don't have; once his fruitless search is over he returns to tugging your [cock]. ");
                }
            ...
            //Female{
                ...
                if (player.wetness() >= 4)
                {
                    outputText("Every time you gush fluid onto his fingers he vibrates his palm, sending uncontrollable shuddering screams into the air as the sensations intensify. ");
                }
    ...
    private function meetFrida() : Void
    {
        ...
        else
        {
            ...
            if (flags[kFLAGS.BLACK_COCK_FRIDAS_CAKE_EATEN_COUNTER] > 0)
            {
    ...
    private function orderFridasCake() : Void
    {
        ...
        if (!player.hasStatusEffect(StatusEffects.Fullness))
        {
            player.createStatusEffect(StatusEffects.Fullness, 2, 0, 0, 0);
        }
        else
        {
            player.changeStatusValue(StatusEffects.Fullness, 1, 2);
        }
    ...
    private function noThanksToCake() : Void
    {
        ...
        if (flags[kFLAGS.BLACK_COCK_FRIDAS_CAKE_EATEN_COUNTER] == 0)
        {
            outputText("You politely decline, not feeling very hungry at the moment.");
        }
        else
        {
    ...
    private function checkPatrons() : Void
    {
        ...
        if (patronCount == 0)
        {
            outputText("There appears to be nobody interesting here.");
        }
        addButton(14, "Back", enterTheBlackCock);
    }
    ...
    private function satyrGangIntro() : Void
    {
        ...
        if (player.lust >= 33)
        {
            if (!player.isTaur())
            {
                addButton(0, "Let Them", letTheSatyrsHaveTheirWayWithYou).hint("Let the satyrs have their way with you. This is going to be rough and intense. " + ((player.hasVagina()) ? "\n\nNOTE: Contains rimjob! You have been warned." : ""));
            }
            else
            {
                addButtonDisabled(0, "Let Them", "You doubt you'll be able to participate into the orgy with your tauric body.");
            }
        }
        else
        {
            addButtonDisabled(0, "Let Them", "You aren't horny enough to consider that.");
        }
    ...
    private function letTheSatyrsHaveTheirWayWithYou() : Void
    {
        ...
        if (player.hasVagina())
        {
            ...
            if (player.isBiped())
            {
                outputText("The satyr presses your [legs] forward. ");
            }
            else if (player.isNaga())
            {
                outputText("The satyr embraces your coils as they slide around his body. ");
            }
            ...
            else if (player.vaginas[0].vaginalLooseness <= 3)
            {
        ...
        {
            player.knockUp(PregnancyStore.PREGNANCY_SATYR, PregnancyStore.INCUBATION_SATYR);
        }
    ...
    {
        var changes : Int = 0;
        var changeLimit : Int = as3hx.Compat.parseInt(3 + player.additionalTransformationChances);
        ...
        if (rand(3) == 0 && changes < changeLimit && player.hasVagina() && player.hasStatusEffect(StatusEffects.BonusVCapacity))
        {
            ...
            if (player.hasStatusEffect(StatusEffects.BonusVCapacity))
            {
                ...
                {
                    player.removeStatusEffect(StatusEffects.BonusVCapacity);
                }
            }
            changes++;
        }
    ...
    public function rhinoTFs() : Void
    {
        ...
        if (rand(4) == 0 && changes < changeLimit && player.lowerBody != LowerBody.GARGOYLE && (player.hasCoat() || player.skinColor != "gray" || player.skinAdj != "tough"))
        {
            ...
            {
                var _sw1_ = (player.skin.base.type);                
        ...
        if (rand(2) == 0 && player.tone < 80)
        {
            if (player.tone < 50)
            {
                player.modTone(80, 2 + rand(2));
            }
            else
            {
                player.modTone(80, 1 + rand(2));
            }
        }
        //Gain thickness
        if (rand(2) == 0 && player.thickness < 80)
        {
            ...
            {
                player.modThickness(80, 2 + rand(2));
            }
            else
            {
                player.modThickness(80, 1 + rand(2));
            }
        }
    ...
    public function echidnaTFs() : Void
    {
        ...
        if (rand(3) == 0 && changes < changeLimit && player.lowerBody != LowerBody.GARGOYLE && !player.isFurCovered())
        {
            ...
            CoC.instance.transformations.SkinFur(Skin.COVERAGE_LOW, {
                                color : "brown"
                            }).applyEffect(false);
            changes++;
        }
        ...
        if (rand(3) == 0 && changes < changeLimit && player.ears.type == Ears.ECHIDNA && player.tailType != Tail.ECHIDNA)
        {
            ...
            switch (_sw4_)
            {
                ...
                case Tail.LIZARD, Tail.DRACONIC:
                ...
                case Tail.HORSE:
                    if (player.isTaur())
                    {
                        outputText("Your shiny horse tail begins to feel chilly as all of its fur falls out. It becomes tight as the majority of the tail’s length recedes into your body, leaving you with just a little stump for a tail. This new, mismatched tail looks a bit odd on your horse lower body.");
                    }
                    else
                    {
                        outputText("Your shiny horse tail begins to feel chilly as all of its fur falls out. It becomes tight as the majority of the tail’s length recedes into your body, leaving you with just a little stump for a tail.");
                    }
                case Tail.MOUSE:
        ...
        if (rand(3) == 0 && changes < changeLimit && player.ears.type == Ears.ECHIDNA && player.tailType && Tail.ECHIDNA && player.lowerBody != LowerBody.ECHIDNA)
        {
            ...
            switch (_sw5_)
            {
                ...
                case LowerBody.HOOFED, LowerBody.CLOVEN_HOOFED:
                ...
                case LowerBody.HARPY, LowerBody.DEMONIC_CLAWS, LowerBody.DEMONIC_HIGH_HEELS, LowerBody.LIZARD, LowerBody.DRAGON:
                ...
                case LowerBody.CAT, LowerBody.DOG, LowerBody.FOX, LowerBody.WOLF, LowerBody.KANGAROO, LowerBody.BUNNY, LowerBody.RACCOON, LowerBody.FERRET:
    ...
    public function mysteryMeatTFs() : Void
    {
        ...
        if (rand(3) == 0 && changes < changeLimit && player.arms.type == Arms.ONI)
        {
            ...
            {
                transformations.HornsOniDual.applyEffect();
            }
            else
            {
                transformations.HornsOni.applyEffect();
            }
            changes++;
        }
        ...
        if (!cast((Arms.SPIDER), InCollection) && changes < changeLimit && rand(3) == 0)
        {
            outputText("[pg]");
