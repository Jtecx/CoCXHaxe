...
class Lilium extends BazaarAbstractContent
{
    ...
    //#########AREA TEXT#########
    public function LiliumText(output : Bool = false) : Function
    {
        if (output)
    ...
    private function approachLilium() : Void
    {
        ...
        if (player.gems < 200)
        {
            outputText("<b>You remember that you haven't got the 200 gems for her services right now.  Maybe next time.</b>");
        }
        else
        {
            pay = payForLilium;
    ...
    private function payForLilium() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_FUCKED_LILIUM] == 0)
        {
            ...
            if (player.short == "Lilium")
            {
                outputText("\"<i>No way!</i>\" she cries upon learning of your identical name.  \"<i>Well... I hope you're not going to ask me to scream your name during sex.  That might be a bit awkward.</i>\"\n\nHow do you want to have the whore?\n\n");
            }
            else
            {
    ...
    private function leaveLilium() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_FUCKED_LILIUM] == 0)
        {
            outputText("\"<i>I'm deathly allergic to fun, unfortunately,</i>\" you reply before moving on to examine more of the bazaar.");
        }
        else
        {
            outputText("\"<i>Just passing through, and I thought I'd see how you were doing,</i>\" you reply, and leave Lilium alone at her post.");
        }
        doNext(bazaar.enterTheBazaar);
    }
    ...
    private function buttFuckTongueJeorb() : Void
    {
        ...
        {
            outputText("  You can feel Lilium's smaller cock resting atop your " + cockDescript(x) + ".");
        }
        ...
        if (player.biggestTitSize() >= 1)
        {
            ...
            {
                outputText("  The stream of milk that leaves your breasts each time you press your body against Lilium's adds further lubrication between you.");
            }
        }
        outputText("\n\n");
        ...
        if (player.isNaga())
        {
            outputText("As you both stand there, reeling in the wake of your ecstasy, she leans back against you causing you to flop onto your [butt] with the demon girl on top of you; you haven't the strength to remain standing.");
        }
        //(else)
        else
        {
            outputText("As you both stand there, reeling in the wake of your ecstasy, she leans back against you causing you to flop onto your [butt] with the demon girl on top of you; your wobbling legs have temporarily lost the strength to keep you both upright.");
        }
    ...
    private function liliumDickRidah() : Void
    {
        ...
        if (player.hairLength >= 15)
        {
            outputText("You can feel Lilium shudder slightly beneath you as your hair tickles her nipples.  ");
        }
        ...
        if (player.biggestTitSize() >= 1)
        {
            ...
            {
                outputText("  Letting your long tongue extend out from between your lips, you wrap it around one " + nippleDescript(0) + " and gently squeeze, sending an electric thrill that flows to your core and back out to the tips of your fingers.");
            }
            //(else if no demonic tongue)
            else
            {
                outputText("  You begin to squeeze your " + nippleDescript(0) + " between two fingers, sending an electric thrill that flows to your core and back out to the tips of your fingers.");
            }
            //(if lactating a little)
            if (player.biggestLactation() >= 1 && player.biggestLactation() <= 2)
            {
                outputText("  A small stream of milk leaks out and dribbles over your tongue, giving you a taste of your own fluids.");
            }
            //(else if lactating a lot)
            else if (player.biggestLactation() > 2)
            {
                outputText("  A powerful stream of milk fires out of your " + nippleDescript(0) + ", coating Lilium's flat stomach.");
            }
            outputText("\n\n");
        }
        ...
        {
            outputText("With one finger you begin to knead your " + clitDescript() + ", which instantly increases the pleasure emanating from your lower body.");
        }
        //(else if big clit)
        else
        {
            outputText("Tenderly grasping it between your fingers you begin to rub your " + clitDescript() + " from base to tip, each stroke adding a wave of pleasure to that of the tongue treatment your " + vaginaDescript(0) + " is receiving.");
        }
        ...
        {
            player.knockUp(PregnancyStore.PREGNANCY_IMP, PregnancyStore.INCUBATION_IMP - 14, 61);
        }  //Lilium causes faster pregnancies  
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    private function liliumNippleFuck() : Void
    {
        ...
        //(If cock is pierced)
        if (player.cocks[x].pierced > 0)
        {
            outputText("You can feel a slight bump as her tongue passes over each piercing and she gives the head of your cock a flick with the tip of her tongue as she reaches the end.  ");
        }
        //(else)
        else
        {
            outputText("As her tongue reaches the end of its journey she gives the head of your " + cockDescript(x) + " a little flick with the tip of her tongue.  ");
        }
        ...
        {
            outputText("your " + player.feet());
        }
        outputText(" as you cum");
        if (player.cockTotal() > 2)
        {
            outputText(", your unused cocks drenching her with seed");
        }
        outputText(".  The tightness of her ");
        if (doubleNipFuck)
        {
            outputText("nipple-cunts around your twin cocks causes some of your cum to squirt back out of her nipples");
        }
        else
        {
            outputText("nipple-cunt around your cock causes some of your cum to squirt back out of her nipple");
        }
        outputText(" each time you fire another load into her, the balance leaking down on Lilium's stomach and thighs.");
        if (player.cumQ() >= 700 && player.cockTotal() > 1)
        {
            outputText("Rivers and rivers of cum pour out of your cocks, distending her nipple-holes as the backflow gushes from around the heads.");
        }
        outputText("\n\n");
