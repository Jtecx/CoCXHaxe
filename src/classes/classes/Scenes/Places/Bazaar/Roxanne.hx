...
class Roxanne extends BazaarAbstractContent implements TimeAwareInterface
{
    //Roxanne Poisontail
    //-no hair,
    ...
    private var autoLose : Bool = false;  // -Is PC losing the Roxanne's drinking contest intentionally?  
    ...
    //Increase Roxanne's growing dick size...
    {
        ...
        if (flags[kFLAGS.ROXANNE_DICK_SIZE] >= 300 && model.time.hours == 1 && rand(5) == 0)
        {
            flags[kFLAGS.ROXANNE_DICK_SIZE] = 1;
        }
        if (model.time.hours > 23 && flags[kFLAGS.ROXANNE_BONUS_SCORE] > 0)
        {
            flags[kFLAGS.ROXANNE_BONUS_SCORE]--;
        }  //Reduce drinking contest bonus  
        return false;
    }
    ...
    public function RoxanneChooseApproachOrRepeat() : Void
    {
        ...
        if (flags[kFLAGS.ROXANNE_MET] == 0)
        {
            ...
            if (player.cor > 66)
            {
                outputText("Yeah, you've got some corruption in you, but I don't think you're contagious like the demons and their ilk yet.  ");
            }
            ...
            if (player.cor < 33)
            {
                outputText(", but would a champion really do something like that?");
            }
            else
            {
                outputText(".");
        ...
        else if (flags[kFLAGS.ROXANNE_LAST_RESULT] == 2 && flags[kFLAGS.ROXANNE_DICK_SIZE] >= 200)
        {
            ...
            {
                outputText("she's going to keep you stretched and gaping");
            }
            else
            {
                outputText("she's probably going to stretch your ass beyond its normal limits");
            }
        ...
        {
            outputText("If you're reading this, something broke.");
        }
    ...
    private function roxanneDrinkingContest() : Void
    {
        ...
        clearOutput();
        outputText("Roxanne ");
        if (flags[kFLAGS.ROXANNE_DICK_SIZE] >= 200)
        {
        ...
        if (flags[kFLAGS.ROXANNE_TIMES_WON] + flags[kFLAGS.ROXANNE_TIMES_LOST] <= 0 || flags[kFLAGS.ROXANNE_LAST_RESULT] == 2)
        {
            ...
            if (flags[kFLAGS.ROXANNE_LAST_RESULT] == 2)
            {
        ...
        outputText("  You ");
        if (flags[kFLAGS.ROXANNE_TIMES_WON] + flags[kFLAGS.ROXANNE_TIMES_LOST] < 3)
        {
        ...
        //(FIRST TIME)
        if (flags[kFLAGS.ROXANNE_TIMES_WON] + flags[kFLAGS.ROXANNE_TIMES_LOST] <= 0)
        {
        ...
        else if (flags[kFLAGS.ROXANNE_TIMES_WON] == 0)
        {
        ...
        else if (flags[kFLAGS.ROXANNE_TIMES_WON] > 0 && flags[kFLAGS.ROXANNE_TIMES_LOST] <= 0)
        {
        ...
        else if (flags[kFLAGS.ROXANNE_TIMES_LOST] > 0 && flags[kFLAGS.ROXANNE_LAST_RESULT] == 1)
        {
        ...
        if (!autoLose)
        {
            score = (player.tallness * ((player.thickness + 100) / 200) * (player.tou / 100)) + flags[kFLAGS.ROXANNE_BONUS_SCORE];
        }
        ...
        //Increment loss count!{
            ...
            var canFatLose : Bool = player.butt.type > 12 && player.tone <= 50 && flags[kFLAGS.ROXANNE_TIMES_LOST] > 1;
            if (canFatLose && rand(2) == 0)
            {
            ...
            else
            {
                if (!canFatLose)
                {
                    sceneHunter.print("Check failed: fat ass, low muscle tone, at least one loss.");
                }
                if (flags[kFLAGS.ROXANNE_DICK_SIZE] >= 200)
                {
    ...
    private function roxanneGivesABlowjob() : Void
    {
        ...
        if (player.lust < 70)
        {
            outputText("it stiffens to a full, erect state.");
        }
        else
        {
            outputText("the already-hard cock-flesh stands proud and erect.");
        }
        ...
        if (player.hasBalls())
        {
            outputText("A set of warm, long-nailed fingers squeeze your [balls], dragging sharp nail-tips along the underside of your sack to tease the poor, cum-packed orbs.  ");
        }
        ...
        {
            outputText("the entire length of your " + cockDescript(x));
        }
        else
        {
            outputText("as much of your over-sized member as she can encompass");
        }
        ...
        if (player.hasVagina())
        {
            outputText(", stopping just short of your female entrance");
        }
        outputText(".\n\n");
        ...
        if (player.cumQ() >= 1500)
        {
            outputText("huge torrent utterly drenches her face, horns, neck, and tight top");
        }
        else if (player.cumQ() >= 500)
        {
            outputText("a huge spurt soaks her face and horns");
        }
        else
        {
            outputText("big spurt splatters her face");
        }
    ...
    private function roxanneCunnilingus() : Void
    {
        ...
        if (player.hasFuckableNipples())
        {
            ...
            {
                outputText("expertly as the one below");
            }
            else
            {
                outputText("they begin to squirt milk around her");
            }
            outputText(".  ");
        }
        ...
        if (player.hasCock())
        {
            outputText(SMultiCockDesc() + " bounces on her head, between her horns, spilling a trail of sticky seed down her back.  ");
        }
    ...
    private function roxanneRimjob() : Void
    {
        ...
        {
            outputText(", your breasts cushioning you from the hard wood below");
        }
        ...
        if (player.hasCock())
        {
            ...
            {
                outputText("pulses with need");
            }
            outputText(", a bead of pre-cum already hanging from the tip.  ");
        }
        if (player.hasVagina())
        {
            outputText("Meanwhile, your neglected box is getting wetter and wetter, with no sign of stopping.  The lizan's massive, questing tongue ignores it.  ");
        }
        ...
        if (player.hasCock())
        {
            ...
            {
                outputText("  It forms a nicely-sized pool before the ground wicks it up.");
            }
            else if (player.cumQ() < 1500)
            {
                outputText("  It forms a large pool before the ground soaks it up.");
            }
            else
            {
                outputText("  It forms a massive pool before the ground soaks it up.");
            }
        }
        else if (player.hasVagina())
        {
            ...
            if (player.wetness() >= 4)
            {
                outputText("soaking them with your gushing orgasms ");
            }
            else
            {
                outputText("sliming them with copious girl-cum ");
            }
    ...
    private function roxanneReamsYouNormal() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText("against your own, the " + num2Text(player.balls + 2) + " orbs rolling around each other in their sweaty sacks");
        }
        else if (player.hasVagina())
        {
            outputText("against your " + vaginaDescript() + ", the sweaty orbs growing slick with the moisture of your growing need");
        }
        else
        {
            outputText("against your taint, the sweaty skin pressing hard between your bodies");
        }
        outputText(".\n\n");
        ...
        {
            outputText("until you're voluntarily working your abdominal muscles to avoid the pain.");
        }
        else
        {
            outputText("until you're moaning with delight from every butt-reddening strike.");
        }
        ...
        if (player.hasCock())
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("It bounces against your belly with your heartbeats, loving the forced anal pressure.  ");
            }
            else
            {
                outputText("They bounce against your belly with your heartbeats, loving the anal pressure.  ");
            }
            outputText("Sticky droplets of pre-cum leak from you, and you can't stifle the pleasured gasps that burst from your maw.");
            if (player.hasVagina())
            {
                outputText("  To your delight and shame, your pussy is equally aroused by the action, musky and wet from the pirate's lewd anal battering.");
            }
            outputText("\n\n");
        }
        //(JUST CUNTS:
        else if (player.hasVagina())
        {
            ...
            if (player.wetness() < 2)
            {
                outputText("sliming your ready vulva.");
            }
            else if (player.wetness() <= 3)
            {
                outputText("soaking your ready vulva.");
            }
            else if (player.wetness() <= 4)
            {
                outputText("which drip from your ready vulva.");
            }
            else
            {
                outputText("which leak in a steady stream from your ready vulva.");
            }
            outputText("\n\n");
        }
        ...
        {
            outputText("Roxanne shifts forward, placing her tits around your head, letting the smooth scales of her mammoth breasts envelop you in their soft embrace.");
        }
        else
        {
            outputText("Roxanne shifts forward slightly, heaving the smooth scales of her mammoth breasts a little further up your back.");
        }
        ...
        if (player.hasCock())
        {
            ...
            {
                outputText("An outpouring of spunk drools from " + sMultiCockDesc() + ", spurting submissively under the table.");
            }
        }
        if (player.biggestLactation() >= 2)
        {
            outputText("  Bursts of milk erupt from your pinned " + nippleDescript(0) + "s, blasting out while you cum to stain the table white.");
        }
    ...
    private function roxanneFucksYourAssOHGODITSHUGE() : Void
    {
        ...
        if (player.tailType > Tail.NONE)
        {
            outputText("She lifts your tail ");
        }
        else
        {
            outputText("She spreads your cheeks ");
        }
        ...
        if (player.analCapacity() < 150)
        {
            outputText("Thanks to the ointment, there's little discomfort from the gut-distending penetration.");
        }
        else
        {
            outputText("Thanks to your experience with that hole, there's little discomfort from the gut-distending penetration.");
        }
        ...
        {
            outputText(", leaving red lines in your unblemished skin");
        }
        ...
        {
            outputText("a roughly-scratched 'X' on your scales");
        }
        else
        {
            outputText("an irritated, red 'X' on your skin");
        }
        ...
        {
            outputText(", drooling spit into the huge puddle of cum you made on her bed");
        }
        else if (player.hasVagina() && player.wetness() >= 4)
        {
            outputText(", drooling spit into the sloppy puddy of fem-cum you splattered on the mattress");
        }
        outputText(".");
    ...
    private function bigBootyRoxanneContestLoss() : Void
    {
        ...
        if (!player.hasVagina())
        {
            outputText(" Oh, I know you don't have a pussy, but tonight, you're my big-bootied, cum-dumpster bitch!");
        }
        else
        {
            outputText("Oh, you'll make such a great anal cum-dumpster for me.  Your poor pussy will be so cum-starved, but you'll be so stuffed with jizz that you'll look pregnant anyhow!");
        }
