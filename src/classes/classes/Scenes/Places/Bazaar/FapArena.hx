...
class FapArena extends BazaarAbstractContent
{
    ...
    public function fapAppearance(display : Bool = true) : Void
    {
        if (display)
        
        //[Paragraph: Bazaar description, 1st time]{
            
            if (flags[kFLAGS.FAP_ARENA_RULES_EXPLAINED] == 0)
            {
    ...
    public function fapArenaGOOOO() : Void
    {
        clearOutput();
        //(set X = longest cock)
        var x : Float = player.longestCock();
        //[1st time]
        if (flags[kFLAGS.FAP_ARENA_RULES_EXPLAINED] == 0)
        {
        ...
        if (player.longestCockLength() < 8)
        {
            ...
            if (!player.hasCock())
            {
                outputText("you have a cock.");
            }
            else
            {
                outputText("your cock is sufficiently long.");
            }
            outputText("</i>\"");
        ...
        else if (flags[kFLAGS.FAP_ARENA_RULES_EXPLAINED] == 0)
        {
            ...
            if (player.cor > 66)
            {
                outputText("Sounds fun!");
            }
            //[else]
            else
            {
                outputText("By gods, what are you doing here?");
            }
        ...
        else
        {
            ...
            if (player.cor > 66)
            {
                outputText("  You look forward to this new masturbating session.");
            }
    ...
    //Increment 'times fap-arena'ed
    {
        ...
        if (player.cor > 66)
        {
            outputText("You eagerly seize the rod on your right side; squeezing it a bit in impatience.  A  little tingle alerts you to your left neighbor grabbing " + oMultiCockDesc() + " with an expert hand.  Everyone around you quickly grabs hold of the nearest cock on their right side.  Some of them are trembling because of the induced stress and arousal.\n\n");
        }
        else if (player.cor > 33)
        {
            outputText("You nervously grab hold of the rod on your right side.  In your stress, you can't help but give it a little squeeze.  You tremble as your left neighbor grabs " + oMultiCockDesc() + " with an expert hand.  Most people are as nervous as you, their whole naked body trembling in stress.\n\n");
        }
        else
        {
        ...
        //5% chance of losing no matter what.{
            ...
            var d : Float = player.hoursSinceCum;
            var c : Float = 0;
            ...
            else if (player.cocks[x].cockType == CockTypesEnum.LIZARD || player.cocks[x].cockType == CockTypesEnum.CAVE_WYRM)
            {
                c = 3;
            }
            ...
            if (st <= 0)
            {
                doNext(createCallBackFunction(fapResults, 3));
            }
            //[else if the player doesn't lose, but doesn't win either - he cums neither first nor last]
            else if (st < 29.5)
            {
                doNext(createCallBackFunction(fapResults, 2));
            }
            //[else, the player wins]
            else
            {
                doNext(createCallBackFunction(fapResults, 1));
            }
        }
        else
        {
            var cval : Int = as3hx.Compat.parseInt(rand(2) +2  //Either 2/3  );
            doNext(createCallBackFunction(fapResults, cval));
        }
    }
    
    private function fapResults(place : Float = 3) : Void
    {
        clearOutput();
        var x : Float = player.longestCock();
        var num : Float = rand(50) + 5;
        var tent : Bool = false;
        //Loses
        if (place == 3)
        {
            ...
            {
                outputText("appreciable");
            }
            //(depending on cum production)
            outputText(" rope of the creamiest, whitest liquid you've ever produced.");
            if (player.cockTotal() > 1)
            {
                outputText("  Several more gouts follow suit from the unstimulated dickflesh dangling from your groin.");
            }
            ...
            {
                outputText("  Your [allbreasts] also release a milky flood of their own, the soft feeling of cream teasing your nipple only prolonging your orgasm.");
            }
            outputText("\n\n");
        ...
        //[if first time]
        if (flags[kFLAGS.FAP_ARENA_RULES_EXPLAINED] == 1)
        {
            outputText("You stand here, a little lost, wondering what will happen next.");
        }
        else
        {
            outputText("You join them, congratulating some on the strength of their stroke, and commenting on the various aspects of the fapping game you've just had.\n\n");
        }
        ...
        if (player.cor < 50)
        {
            outputText("You find yourself yelling with them, astounded by everyone's enthusiasm and eagerness to be fucked in the ass, even after this rough masturbating session.  You wish you weren't so blindly following others, but they are so hypnotizing, their hard glistening cocks pointing at the air as they keep shouting over and over again, \"<i>BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!</i>\"  ");
        }
        //[else, corr >50]
        else
        {
            outputText("Your body tenses and " + sMultiCockDesc() + " hardens at the perspective of a collective anal orgy.  You yell in lust like a minotaur in rut, \"<i>BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!</i>\" along with the others.  ");
        }
        //end of global condition about PC's corr, display the following
        ...
        if (place <= 1)
        {
            outputText("Since you're the winner, you walk to the tail of the butt-fuck train, enjoying your prize.  ");
        }
        //[if you didn't win, didn't lose]
        else if (place <= 2)
        {
            outputText("You place yourself between two horny bodies.  The lust and confusion have obscured your senses and all you see is a rock-hard dick and a gaping anus, both waiting for you to play with.  ");
        }
        //[if you lost]
        else
        {
            ...
            if (player.cor < 50)
            {
                outputText("a whorish smile");
            }
            else
            {
                outputText("apprehension");
            }
        ...
        if (place <= 1)
        {
            ...
            {
                if (player.cockTotal() > 3 || (!tent && player.cockTotal() > 2))
                {
                    outputText("  Your other cocks also spill ");
                }
                if ((tent && player.cockTotal() == 3) || (player.cockTotal() == 2 && !tent))
                {
                    outputText("  Your other cock also spills ");
                }
                outputText("a ");
                if (player.cumQ() > 1000)
                {
        ...
        else
        {
            ...
            if (player.lib > 50)
            {
                outputText("  Maybe losing wasn't that bad? You feel like you could lose a few more times...");
            }
            outputText("\n\n");
        }
    ...
    private function tentacleFapSpecial(place : Float) : Void
    {
        ...
        if (player.cocks[x].cockLength >= 40 && player.cocks[x].cockType == CockTypesEnum.TENTACLE)
        {
            ...
            if (place <= 2)
            {
                outputText("fucking your partner's anus");
            }
            if (place == 2)
            {
                outputText(" and");
            ...
            if (place >= 2)
            {
                outputText(" being fucked from behind");
            }
            outputText(".\n\n");
            ...
            if (place <= 2)
            {
                ...
                if (place == 1)
                {
                    outputText("  Such raw power, fucking two people at the same time, and one from so far away...  You're basically fucking a wagon of people twice.  The enormity of the act makes you laugh, adding to your pleasure-induced confusion.");
                }
                else
                {
                    outputText("  Your resistance is quickly fading because of the triple fuck and the mix of feelings combining to provide the richest, dirtiest orgasm; but you feel consoled in knowing the herm girl you're fucking is completely lost.");
                }
    ...
    private function tentacleFapCum() : Bool
    {
        var index : Int = player.cocks.length;
        var x : Float = player.longestCock();
        while (index > 0)
