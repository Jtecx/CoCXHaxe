...
class Benoit extends BazaarAbstractContent
{
    public static var itemShop(get, never) : Array<Dynamic>;
    public static var armorShop(get, never) : Array<Dynamic>;
    ...
    {
        if (flags[kFLAGS.BENOIT_STATUS] == 1 || flags[kFLAGS.BENOIT_STATUS] == 2 || flags[kFLAGS.BENOIT_STATUS] == 3)
        {
            return stringF;
        }
        return stringM;
    }
    private function benoitLover() : Bool
    {
        return flags[kFLAGS.BENOIT_TIMES_SEXED_FEMPCS] >= 2;
    }public function benoitAffection(changes : Float = 0) : Float
    {
    ...
    //Convert old basi's to real basi's!
    {
        ...
        if (player.pregnancyType == PregnancyStore.PREGNANCY_BASILISK && player.hasPerk(PerkLib.BasiliskWomb))
        {
            player.knockUpForce(PregnancyStore.PREGNANCY_BENOIT, player.pregnancyIncubation);
        }
        if (player.pregnancy2Type == PregnancyStore.PREGNANCY_BASILISK && player.hasPerk(PerkLib.BasiliskWomb))
        {
            player.knockUpForce(PregnancyStore.PREGNANCY_BENOIT, player.pregnancy2Incubation, 1);
        }
        //Knock up chances:
        if ((player.inHeat || player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS || player.pregnancy2Type == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS || player.hasPerk(PerkLib.HarpyWomb) || player.hasPerk(PerkLib.Oviposition) || player.hasPerk(PerkLib.BasiliskWomb)) && player.canGetPregnant() || player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS || player.pregnancy2Type == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS)
        {
            if (player.hasPerk(PerkLib.BasiliskWomb) && flags[kFLAGS.BENOIT_TESTED_BASILISK_WOMB] == 1)
            {
                ...
                {
                    player.knockUpForce(PregnancyStore.PREGNANCY_BENOIT, player.pregnancyIncubation, 1);
                }
            }
...
*/
    ...
    public function benoitInClutch() : Bool
    {
        ...
        var diffDays : Int = as3hx.Compat.parseInt((currDay - startDay) % 28);
...
*/
    public function numEggs() : Int
    {
        ...
        var numEggs : Int = as3hx.Compat.parseInt(player.cumQ() / 200);
        var minEggs : Int = as3hx.Compat.parseInt(1 + eggMod);
        var maxEggs : Int = as3hx.Compat.parseInt(12 + eggMod);
        if (numEggs < minEggs)
        {
            numEggs = minEggs;
        }
    ...
    public function benoitKnockUp() : Bool
    {
        if (benoitPreggers() || !benoitInClutch())
        {
            return false;
        }
    ...
    public function clearBenoitPreggers() : Void
    {
        if (flags[kFLAGS.FEMOIT_EGGS] != 0)
        {
            ...
            flags[kFLAGS.FEMOIT_NEXTDAY_EVENT] = CoC.instance.model.time.days;
        }
    }
    
    public function setBenoitShop(setButtonOnly : Bool = false) : Void
    {
        if (model.time.hours >= 9 && model.time.hours <= 17 && (flags[kFLAGS.FEMOIT_NEXTDAY_EVENT_DONE] != 1 || CoC.instance.model.time.days >= flags[kFLAGS.FEMOIT_NEXTDAY_EVENT]))
        {
            if (flags[kFLAGS.TIMES_IN_BENOITS] == 0)
            {
                ...
                {
                    addButton(0, "Market Stall", benoitIntro);
                }
            }
            ...
            {
                addButton(0, "[benoit name]", benoitIntro);
            }
        }
        else
        {
            addButtonDisabled(0, (flags[kFLAGS.TIMES_IN_BENOITS] == 0) ? "???" : "[benoit name]", "Currently closed.");
        }
    }
    ...
    public function benoitIntro() : Void
    {
        ...
        // Missed it by a week{
            ...
            {
                outputText("When you enter the stall, you notice that Benoite's stomach is flat again. She gives you a toothy grin when you enter her stall. \"<i>I have good newz [name]! You are a fazer! " + Std.string(flags[kFLAGS.FEMOIT_EGGS]) + " timez over in fact! Oh I wish zat you could have helped; but I was ztrong, ztrong enough get through it on my own.</i>\" she gushes, speaking at a mile-a-minute. She's clearly pleased with herself; and your virility.");
        ...
        else
        {
            ...
            if (benoitAffection() <= 10)
            {
                outputText("Once again, you carefully enter the gloom of Benoit's salvage shop.  The proprietor sniffs the air as you enter, and then looks at you sourly.  \"<i>Well?</i>\" [benoit ey] rasps.");
            }
            ...
            {
                outputText("Once again, you carefully enter the gloom of Benoit's salvage shop.  The proprietor sniffs the air as you enter, and then relaxes in [benoit eir] seat.  \"<i>Allo again, [name].  What brings you 'ere?</i>\"");
            }
        ...
        //Feminize & Herminize
        if (flags[kFLAGS.FEMOIT_UNLOCKED] == 0)
        {
            addButton(3, "HelpRace-Him", femoitInitialTalk).disableIf(flags[kFLAGS.BENOIT_TIMES_SEXED_FEMPCS] > 0 && !sceneHunter.other, 
                                    "The problem is <i>already</i> solved.\n\n"
                                    + "<b>You can enable SH:Other to try this option too.</b>", "???"
                    ).disableIf(benoitAffection() < 40, "He doesn't trust you enough.", "???").disableIf(flags[kFLAGS.BENOIT_TALKED_TO_PROPERLY] == 0, "You don't know him good enough.", "???");
        }
        else if (flags[kFLAGS.FEMOIT_UNLOCKED] == 1 && flags[kFLAGS.BENOIT_STATUS] == 0)
        {
            addButton(3, "Feminize", benoitFeminise);
        }
        else if (flags[kFLAGS.BENOIT_STATUS] > 0 && flags[kFLAGS.BENOIT_STATUS] < 3)
        {
            addButton(3, "Herminize", benoitHerminise);
        }
        //Basilisk Womb
        if (flags[kFLAGS.BENOIT_WOMB_TALK_UNLOCKED] == 1 && (!(player.hasPerk(PerkLib.BasiliskWomb))) && flags[kFLAGS.BENOIT_TESTED_BASILISK_WOMB] == 0 && (flags[kFLAGS.BENOIT_STATUS] == 0 || flags[kFLAGS.BENOIT_STATUS] == 3))
        {
            addButton(4, "Basil. Womb", tryToConvertToBassyWomb);
        }
        else if (flags[kFLAGS.BENOIT_WOMB_TALK_UNLOCKED] == 0)
        {
            addButton(4, "HelpRace-You", talkHelp).disableIf(!player.hasVagina(), "Req. a pussy.").disableIf(flags[kFLAGS.BENOIT_TIMES_SEXED_FEMPCS] < 2, "Requires meeting Benoit a little <i>closer</i>.", "???");
        }
        //Suggest & sex
        ...
        if (flags[kFLAGS.BENOIT_STATUS] > 0)
        {
            if (flags[kFLAGS.FEMOIT_TALKED_TO] == 0)
            {
                addButton(6, "What's next?", femoit1stTalk);
            }
            else
            {
                addButton(6, "Sex", femoitSexIntro).disableIf(player.lust < 33, "Not aroused enough.");
            }
        }
    }
    ...
    public function benoitsBuyMenu() : Void
    {
        ...
        var shop : Array<Dynamic> = [];
        var item : ItemType;
        var btn : Int = 0;
        for (flag in [kFLAGS.BENOIT_1, kFLAGS.BENOIT_2, kFLAGS.BENOIT_3, kFLAGS.BENOIT_4, kFLAGS.BENOIT_5])
        {
            shop.push(ItemType.lookupItem(Reflect.field(flags, Std.string(flag))));
        }
        menu();
        for (item in shop)
        {
            var cost : Int = as3hx.Compat.parseInt(buyMod * item.value);
            outputText("\n" + item.longName + ": " + cost);
            addButton(btn++, item.shortName, benoitTransactBuy, item).disableIf(player.gems < cost, "Not enough gems.");
        }
        ...
        if (player.keyItemvX("Backpack", 1) < 14)
        {
            addButton(5, "Backpack", buyBackpack).hint("This backpack will allow you to carry more items.");
        }
        if (flags[kFLAGS.BENOIT_CLOCK_BOUGHT] <= 0 && flags[kFLAGS.CAMP_CABIN_FURNITURE_NIGHTSTAND] > 0)
        {
            addButton(6, "Alarm Clock", buyAlarmClock).hint("This mechanical clock looks like it was originally constructed by the Goblins before the corruption spreaded throughout Mareth.");
        }
        if (flags[kFLAGS.BENOIT_PISTOL_BOUGHT] < 2 && flags[kFLAGS.BENOIT_AFFECTION] == 100)
        {
            addButton(7, "Francisca t a", buyFranciscaThrowingAxe).hint("Francisca throwing axe");
        }
    ...
    private function benoitSellMenu(page : Int = 1) : Void
    {
        ...
        if (flags[kFLAGS.BENOIT_EGGS] > 0 || flags[kFLAGS.BENOIT_STATUS] != 0)
        {
            sellMod = 2;
        }
        ...
        if (page == 1)
        {
            for (slot in 0...10)
            {
                if (player.itemSlots[slot].quantity > 0 && as3hx.Compat.parseInt(player.itemSlots[slot].itype.value / sellMod) >= 1)
                {
                    outputText("\n" + as3hx.Compat.parseInt(player.itemSlots[slot].itype.value / sellMod) + " gems for " + player.itemSlots[slot].itype.longName + ".");
            ...
            if (inventory.getMaxSlots() > 10)
            {
                addButton(13, "Next", benoitSellMenu, page + 1);
            }
        }
        if (page == 2)
        {
        ...
        if (totalItems > 1)
        {
            addButton(10, "Sell All", createCallBackFunction2(benoitSellAllTransact, totalItems, sellMod));
        }
        addButton(14, "Back", benoitIntro);
    }
    ...
    private function benoitTransactBuy(itype : ItemType) : Void
    {
        ...
        if (benoitLover())
        {
            outputText("After examining what you've picked out with [benoit eir] fingers, [benoit name] hands it over and accepts your gems with a grin.");
        }
        else
        {
            outputText("After examining what you've picked out with [benoit eir] fingers, [benoit name] hands it over, names the price and accepts your gems with a curt nod.\n\n");
        }
        //(+3 Affection)
        benoitAffection(3);
        
        player.gems -= as3hx.Compat.parseInt(buyMod * itype.value);
        statScreenRefresh();
    ...
    private function benoitSellTransact(slot : Int, sellMod : Int) : Void
    {
        ...
        if (benoitLover())
        {
            outputText("[benoit name] gives your object the briefest of goings-over with [benoit eir] fingers before stowing it away and handing over your gem reward with a trusting smile.");
        }
        else
        {
            outputText("Following a painstaking examination of what you've given [benoit em] with his hands and nose, Benoit grudgingly accepts it and carefully counts out your reward.");
        }
        if (shiftKeyDown)
        {
            while (player.itemSlots[slot].quantity > 0)
            {
                player.gems += as3hx.Compat.parseInt(player.itemSlots[slot].itype.value / sellMod);
    ...
    private function benoitSellAllTransact(totalItems : Int, sellMod : Int) : Void
    {
        ...
        for (slot in 0...50)
        {
            if (player.itemSlots[slot].quantity > 0 && as3hx.Compat.parseInt(player.itemSlots[slot].itype.value / sellMod) >= 1)
            {
                itemValue += as3hx.Compat.parseInt(player.itemSlots[slot].quantity * as3hx.Compat.parseInt(player.itemSlots[slot].itype.value / sellMod));
        ...
        {
            outputText("Following a painstaking examination of the items you've given him with his hands and nose, Benoit grudgingly accepts them and carefully counts out your " + num2Text(itemValue) + " gem reward.");
        }
    ...
    // items by slots
    private static function get_itemShop() : Array<Dynamic>
    {
        return [
        consumables.INCUBID, 
        consumables.MINOBLO, 
    ...
    private static function get_armorShop() : Array<Dynamic>
    {
        return [
        armors.W_ROBES, 
        armors.S_SWMWR, 
    ...
    {
        var pickedItems : Array<Dynamic> = randomChoices(true, 4, itemShop);
    ...
    private function buyBackpack() : Void
    {
        ...
        menu();
        if (player.keyItemvX("Backpack", 1) < 2)
        {
            addButton(0, "Small", buyBackpackConfirmation, 2, "Small", 100, "Grants additional two slot. \n\nCost: 100 gems");
        }
        if (player.keyItemvX("Backpack", 1) < 4)
        {
            addButton(1, "Medium", buyBackpackConfirmation, 4, "Medium", 200, "Grants additional four slots. \n\nCost: 200 gems");
        }
        if (player.keyItemvX("Backpack", 1) < 6)
        {
            addButton(2, "Large", buyBackpackConfirmation, 6, "Large", 300, "Grants additional six slots. \n\nCost: 300 gems");
        }
        if (player.keyItemvX("Backpack", 1) < 8)
        {
            addButton(3, "X Large", buyBackpackConfirmation, 8, "X Large", 400, "Grants additional eight slots. \n\nCost: 400 gems");
        }
        if (player.keyItemvX("Backpack", 1) < 10)
        {
            addButton(4, "XX Large", buyBackpackConfirmation, 10, "XX Large", 500, "Grants additional ten slots. \n\nCost: 500 gems");
        }
        if (player.keyItemvX("Backpack", 1) < 12)
        {
            addButton(5, "XXX Large", buyBackpackConfirmation, 12, "XXX Large", 600, "Grants additional twelve slots. \n\nCost: 600 gems");
        }
        if (player.keyItemvX("Backpack", 1) < 14)
        {
            addButton(6, "XXXX Large", buyBackpackConfirmation, 14, "XXXX Large", 700, "Grants additional fourteen slots. \n\nCost: 700 gems");
        }
    ...
    private function buyBackpackConfirmation(size : Int = 2, sizeDesc : String = "Small", price : Int = 100) : Void
    {
        ...
        {
            price = as3hx.Compat.parseInt((size - player.keyItemvX("Backpack", 1)) * 50);
    ...
    private function talkToBenoit() : Void
    {
        ...
        else
        {
            var choice : Int;
            /* BUILD ZE CHOICES!*/
            var choices : Array<Dynamic> = [0, 1, 2, 3, 4, 5, 6, 7, 8];
            ...
            switch (choice)
            {
                ...
                case 9:
                    ...
                    {
                        outputText("  There's a hint of longing jealousy in the basilisk's voice and when [benoit ey] lapses into silence you decide not to push the subject.");
                    }
                ...
                case 14:
                    ...
                    if (benoitLover() && player.hasCock() && player.hasVagina())
                    {
    ...
    public function eggySuggest() : Void
    {
        ...
        //Suggest:{
            ...
            if (player.gender == 3)
            {
                ...
                {
                    outputText("  Although this is evidently an uncanny experience for [benoit em], [benoit ey] does manage a cocky smile as [benoit eir] hand moves around your male sex. \"Mine is bigger,\" [benoit ey] teases.  Unable to reward [benoit eir] cheek from where [benoit ey] is, you simply snort and shuffle your hooves, impatient now with need.");
                }
                //[cock  >10 inches:
                else
                {
                    outputText("  This is evidently an uncanny experience for [benoit em], the alien nature of it deepening as [benoit eir] hands moves around your male sex. \"Oly Gods [name], you are a monster,\" [benoit ey] says thickly.  You smile and stamp your hooves, impatient now with need.");
                }
            }
            ...
            {
                outputText("  The slow, sensual sex is enough for you to begin to bead milk from your sensitive [nipples]; you moan as the flow intensifies until you are instinctively kneading your [chest], spurting your sweet, warm fluids onto the floor.  [benoit name] starts in genuine amazement at the sound.  \"Mammals are so damn weird,\" [benoit ey] mutters, making you giggle.");
            }
            ...
            if (player.gender == 3)
            {
                outputText("  [EachCock] thickens and spurts in sympathy to your female high, spattering the floor with white paint.");
            }
        }
        else
        {
            ...
            //[Demon:
            if (player.horns.count > 0 && player.horns.type == Horns.DEMON && player.tailType == Tail.DEMONIC && (player.isRace(Races.DEMON, 1, false) || player.isRace(Races.IMP, 1, false)))
            {
            ...
            else if (player.ears.type == Ears.DOG && player.tailType == Tail.DOG && player.racialScore(Races.DOG, false) >= 3)
            {
            ...
            else if (player.racialScore(Races.CAT, false) >= 3 && player.tailType == Tail.CAT && player.ears.type == Ears.CAT)
            {
            ...
            else if (player.lowerBody == LowerBody.HARPY && player.wings.type == Wings.FEATHERED_LARGE)
            {
            ...
            //[Reptile/Naga:
            else if (player.isScaleCovered() && (player.racialScore(Races.LIZARD, false) >= 3 || player.racialScore(Races.NAGA, false) >= 3) || player.racialScore(Races.DRAGON) >= 3)
            {
            ...
            //[Bee:
            else if ((player.wings.type == Wings.BEE_SMALL || player.wings.type == Wings.BEE_LARGE) && player.lowerBody == LowerBody.BEE)
            {
            ...
            if (player.hasCock())
            {
                ...
                if (player.cocks[player.biggestCockIndex()].cockLength < 10)
                {
            ...
            if (player.inHeat)
            {
                outputText(" By now your vagina is practically gushing, your body's own deep-seated pheromone need stoked to blazing heights by the basilisk's gentle, painstaking exploration.  You cannot stop yourself thrusting against [benoit eir] soaked hand, announcing how badly you want this with heavy moans.");
            }
            ...
            if (player.lactationQ() > 100)
            {
                ...
                if (!player.isTaur())
                {
                    outputText("hit [benoit em] in the face");
                }
                else
                {
                    outputText("trickle over [benoit eir] fingers");
                }
            ...
            if (player.hasCock())
            {
                if (!player.isTaur())
                {
                    outputText("  Stuck between your two burning bodies, y");
                }
                else
                {
                    outputText("Y");
                ...
                if (!player.isTaur())
                {
                    outputText("both of you");
                }
                else
                {
                    outputText("the dry dirt");
                }
    ...
    private function repeatBenoitFuckTakeCharge() : Void
    {
        ...
        if (player.isTaur())
        {
            ...
            if (player.vaginalCapacity() < 30)
            {
                outputText("You slide back down onto [benoit em], cooing this time as you feel [benoit em] bottom out with several inches of [benoit em] still outside of you.  [benoit Eir] thin, long length could not be more perfect for your tight sex; [benoit ey] rubs your walls up and down as you dreamily thrust in and out of [benoit eir] body, sending waves of pleasure flowing through you.");
            }
            //Large capacity:
            else
            {
    ...
    public function tryToConvertToBassyWomb() : Void
    {
        ...
        else
        {
            ...
            //[No oviposition:
            if (player.hasPerk(PerkLib.Oviposition) < 0)
            {
                outputText("  You feel slightly bloated, but otherwise fine; you sense that you can lay eggs of your own volition now.");
            }
            else
            {
                outputText("You feel slightly bloated, but otherwise fine; you sense that were you to get impregnated by basilisk seed, the eggs you produce would be pure basilisk.");
            }
            ...
            if (player.lust >= 33 || recalling)
            {
                ...
                {
                    outputText("  The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and hoists you over [benoit eir] shoulder.  You squeal in mock terror as [benoit ey] hauls you as fast as [benoit ey] can into the back room, knocking over half [benoit eir] stock as [benoit ey] does.");
                }
                else
                {
                    outputText("  The basilisk is still for a moment, and then with a sudden surge of movement, grabs you by the waist and frenetically attempts to hoist you over [benoit eir] shoulder.  You are far too big for [benoit em] though; after several valiant attempts, [benoit ey] collapses against a shelf.  Laughing, you pick the stricken, panting reptile up, hoist [benoit em] over your own shoulder, and navigate a path into the back room.");
                }
            ...
            {
                outputText("  You grin and say you're not even sure it worked... but you'll be back at some point to try it out, and [benoit ey]'d better be ready for when you do.  You gently pry yourself out of [benoit eir] grip and leave as deliberately as you can, aware of the beguiling, invisible scent you are leaving for the stunned, silent basilisk to simmer in.");
            }
            doNext(camp.returnToCampUseOneHour);
        }
    }
    ...
    private function suggestSexAfterBasiWombed(later : Bool = true) : Void
    {
        clearOutput();
        ...
        if (!player.isTaur())
        {
            ...
            if (player.hasCock())
            {
                outputText("  [benoit Ey] deliberately rubs himself up and down your body, and the [cock] trapped between your warm bodies quickly hardens against the warm, smooth friction.");
            }
        ...
        else
        {
            ...
            if (player.hasCock())
            {
                outputText("  [benoit Ey] deliberately moves [benoit eir] hand over [oneCock] before trapping it in [benoit eir] warm grasp.   It quickly hardens against [benoit eir] warm, smooth friction.");
            }
    ...
    public function popOutBenoitEggs() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("and [eachCock] starting to leak pre-cum ");
        }
        ...
        if (player.hasCock())
        {
            outputText(" and a downpour of semen");
        }
        outputText(".");
    ...
    public function femoitFirstTimeYes() : Void
    {
        ...
        else if (player.isRace(Races.DOG, 1, false) && player.ears.type == 2 && player.tailType == 2)
        {
        ...
        else if ((player.racialScore(Races.BUNNY, false) >= 4 && player.ears.type == 7 && player.tailType == 10) || (player.racialScore(Races.CAT, false) >= 4 && player.ears.type == 5 && player.tailType == 8))
        {
        ...
        else if (player.isRace(Races.HARPY, 1, false) && player.wings.type != 0 && player.arms.type == 1)
        {
        ...
        else if (player.isRace(Races.BEE, 1, false) && player.wings.type != 0 && player.lowerBody == 7)
        {
        ...
        else if (player.racialScore(Races.SLIME, false) >= 4 && player.isGooSkin())
        {
        ...
        {
            outputText("  Although this is evidently an uncanny experience for her, she does manage a cocky smile as her hand moves around your sex.  \"<i>Mine was bigger,</i>\" she teases.  You reward her cheek by doing some feeling yourself, grasping her large, supple behind, making her squeak as you move into her.");
        }
        else
        {
        ...
        if (player.biggestCockLength() < 15 && player.balls == 0)
        {
            outputText("  Eventually you manage to hilt yourself entirely in her depths, your stomach pressing against her own tight belly.");
        }
        else if (player.biggestCockLength() < 15 && player.hasBalls())
        {
            outputText("  Eventually you manage to hilt yourself in her depths, your [balls] pressing into her sex as your stomach bumps into her own tight belly.");
        }
        else if (player.biggestCockLength() >= 15 && player.balls == 0)
        {
            outputText("  Eventually you manage to bottom out, your dick pressed against her cervix.");
        }
        else if (player.biggestCockLength() >= 15 && player.hasBalls())
        {
            outputText("  Eventually you manage to bottom out, your dick pressed against her cervix, your [balls] swinging heavily below your shaft.");
        }
        ...
        {
            outputText(" The pressure and arousal makes your [nipples] dribble milk, spattering fluids across both of your chests.");
        }
    ...
    public function femoitSexIntro() : Void
    {
        ...
        if (!benoitPreggers())
        {
            ...
            if (benoitInClutch())
            {
                outputText("  Her stomach bulges with clutch and the air is thick with female musk; the sight and smell of overwhelming ripeness speaks to your body in a way which bypasses your higher cognition entirely, sending you growling on top her, barely even hearing her giggle.");
            }
            ...
            {
                outputText("Soon you are hilting yourself in her depths, making her gasp as you slap into her.");
            }
            else
            {
                outputText("Soon you are bottoming out in her, making her gasp as your hulking length spreads her wide.");
            }
            ...
            if (!player.isTaur())
            {
                ...
                if (!benoitInClutch())
                {
                    outputText(" your stomach beating out a slapping rhythm against her own flat abdomen.  ");
                }
                else
                {
                    outputText(" your stomach beating out a slapping rhythm against her bulging, gravid abdomen.  ");
                }
            }
            ...
            if (player.hasBalls())
            {
                outputText(" and [balls].  You tumble over your peak as her cunt suddenly tightens around yours, sending surge after surge of cum into her fertile depths, your body seized in a rictus of pleasure.");
            }
            if (player.cumQ() >= 2500)
            {
                outputText("  The quantity of it is such that it quickly dribbles back out around your cock and pools on the floor.");
            }
        ...
        else if (benoitRegularPreggers() && (!player.isTaur() || (player.isTaur() && (player.tallness * (5 / 6) < player.cocks[player.longestCock()].cockLength))))
        {
            ...
            if (player.cumQ() >= 2500)
            {
                outputText("  Because her womb is already so jam-packed with eggs, the bulk of your deposit simply oozes messily back out of her, leaving her well and truly creampied.");
            }
        ...
        else if (benoitVeryHeavyPreggers() || benoitExtremePreggers())
        {
            ...
            else
            {
                ...
                if (player.balls == 0)
                {
                    outputText(" with a smile, knowing damn well you don't, literally");
                }
                outputText(".");
    ...
    public function femoitBirths() : Void
    {
        ...
        else
        {
            ...
            {
                outputText("... which is good, because you can tell there're a lot of them coming");
            }
        ...
        if (silly())
        {
            outputText(".  It's shaped like a complex rhomboidal polygon with 15 sides.");
        }
        else
        {
            outputText(", the smoothly curved peak of a jade egg beginning to crest.");
        }
        ...
        if (benoitRegularPreggers())
        {
            outputText(" She sighs softly.  \"<i>A small clutch, this is... normal for my people, yes, but not very good for my mission, is it?  Still, a small step to freedom is still a step.</i>\"  She manages to smile.  \"<i>My children will be free, and that is something to celebrate.</i>\"");
        }
        else if (benoitHeavyPreggers())
        {
            outputText(" \"<i>My, a nice big clutch we had together, didn't we?</i>\" she smiles, proudly.  \"<i>Yes, these will be strong children, I zink.</i>\"");
        }
        else if (benoitVeryHeavyPreggers())
        {
            outputText(" \"<i>...Wow, you're quite ze stud, aren't you?</i>\" Benoite says, giving a throaty growl of lust.  \"<i>I chose well when I decided to let you fertilize my eggs... so many eggs, too.</i>\"");
        }
        else if (benoitExtremePreggers())
        {
