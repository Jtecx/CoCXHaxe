...
class Cinnabar extends BazaarAbstractContent
{
    
    //Appearnace
    public function cinnabarAppearance(output : Bool = true) : Function
    ...
    {
        var big : Bool = player.hasCock() && player.biggestCockArea() >= 100;
        var multi : Bool = big && player.cockTotal() >= 3 && player.biggestCockArea2() >= 75;
    ...
    {
        var x1 : Int = as3hx.Compat.parseInt(player.findCock(1, -1, 100) + 1);
        ...
        if (!player.hasVagina())
        {
            outputText(" and your " + assholeDescript());
        }
        else
        {
            outputText(", your " + vaginaDescript() + ", and your " + assholeDescript());
        }
        ...
        if (player.hasVagina())
        {
            outputText("grab your " + clitDescript() + ", rolling it between her thumb-pads with a hair too much pressure");
        }
        else
        {
            outputText("slip two digits into your " + assholeDescript() + ", pulling it apart far enough to cause you discomfort");
        }
        ...
        if (player.hasVagina())
        {
            outputText("release their squeeze");
        }
        else
        {
            outputText("release the butt-stretching pressure");
        }
        ...
        {
            outputText("  Unbidden, " + sMultiCockDesc() + " begins to harden, stiffening with every brush against your prostate.");
        }
        else if (player.hasVagina())
        {
            outputText("  Femcum begins to soak the rat's practiced fingers, drenching her finger-pads with liquid lust.");
        }
        outputText("  You rock back, slamming your [butt] against the wall and moaning with need.  She's quite skilled, and ");
        if (player.hasVagina())
        {
            outputText("caresses your passage in ways that ");
        }
        else
        {
            outputText("works your body in ways that ");
        }
        ...
        if (player.hasCock() && !player.hasVagina())
        {
            outputText("You gasp and moan when you feel her other hand wrap around your [cock " + x1 + "], pulling and tugging on it until it starts to leak a trail of squeezed-out man-slime on the mattress.  ");
        }
        ...
        if (player.hasVagina())
        {
            if (player.vaginalCapacity() < 30 && player.analCapacity() < 30)
            {
                outputText("fingers");
            }
            else if (player.vaginalCapacity() >= 30 && player.analCapacity() >= 30)
            {
                outputText("fists");
            }
            else
            {
                outputText("fingers and fist");
            }
        }
        ...
        {
            outputText("tightly-clenched fingers");
        }
        ...
        if (player.hasCock() && !player.hasVagina())
        {
            ...
            if (player.cumQ() >= 250)
            {
                ...
                if (player.cumQ() >= 1000)
                {
                    outputText(" and puddles on the floor");
                }
                outputText(".");
            }
        }
        ...
        if (player.gender == 0)
        {
            outputText("  Her soft pads rub your " + assholeDescript() + " just right, setting off your climax with professional skill.  You grunt and squeeze, wringing her fingers inside your hungry hole as if you could milk cum from them.");
        }
        ...
        {
            outputText("  Her arm seems quite soaked with your leavings.");
        }
        else
        {
            outputText("  Her arm and chest are left splattered from your explosive, squirting orgasm.");
        }
        outputText("\n\n");
        ...
        if (player.cumQ() >= 1000)
        {
            outputText(" and the floor mopped");
        }
    ...
    private function cinnabarBigRouter() : Void
    {
        ...
        var pure : Bool = player.cor <= 75 + player.corruptionTolerance;
        var corr : Bool = player.cor >= 75 - player.corruptionTolerance;
        ...
        if (sceneHunter.dickSelect)
        {
            if (big >= 0 && huge < 0)
            {
                outputText("While your cock should be big enough for her anyway, you're thinking... is it possible to push it even further? Sadly, you still lack the size for that.");
            }
            else if (big < 0 && huge >= 0)
            {
                outputText("You wonder what would happen if your cock wasn't <b>that</b> big...");
            }
            else
            {
                outputText("While your " + cockDescript(player.findCock(1, 100, 200)) + " should be big enough for her anyway, you're thinking... maybe you should try using even bigger one?");
            }
            if (huge >= 0 && pure && corr)
            {
        ...
        {
            cinnabarGetsFUKKKKED((huge >= 0) ? huge : big, corr);
        }
    }
    
    //[FUCK] – tracks if last fuck was huger + corrupteded version
    private function cinnabarGetsFUKKKKED(x : Int, corr : Bool = false) : Void
    {
        clearOutput();
        ...
        if (flags[kFLAGS.CINNABAR_FUCKED_CORRUPT_PREVIOUSLY] > 0)
        {
            outputText("  Just be gentler than last time, okay " + player.mf("stud", "baby") + "?");
        }
        ...
        if (player.str >= 60 || player.tallness >= 72)
        {
            outputText("a little");
        }
        else
        {
            outputText("on your heels");
        }
        ...
        {
            outputText("centaur mare wince");
        }
        else
        {
            outputText("a dragoness cry");
        }
        ...
        if (player.cor > 66)
        {
            outputText("enjoying the raunchiness of the act");
        }
        else if (player.cor > 33)
        {
            outputText("a little worried by how much you're enjoying this");
        }
        else
        {
            outputText("a little amazed that you would ever do such a thing");
        }
    ...
    private function fuckCinnabarHugerAndCorruptedEr() : Void
    {
        ...
        {
            outputText(", and only now do you realize that you're long enough that you should've gone the whole way through her.  Just what kinds of magics is she capable of!?");
        }
        else
        {
            outputText(", and only now do you realize you blew past her cervix long ago, the tight ring hugging your " + cockDescript(x) + " in a clutching embrace.");
        }
        outputText("\n\n");
        ...
        if (player.inte >= 50)
        {
            outputText("You dimly register it as an arousal spell, but you were already on edge, and your poor prostate is practically wringing itself dry in an effort to cum.");
        }
        else
        {
            outputText("It's too much for your utterly encapsulated member and violated anus, and your poor prostate practically wrings itself dry as it tries to unload.");
        }
        ...
        if (player.cumQ() < 250)
        {
            outputText("makes your body churn well after you're empty.");
        }
        else if (player.cumQ() < 500)
        {
            outputText("causes her hugely-stretched lips to glaze white.");
        }
        else if (player.cumQ() < 1000)
        {
            outputText("causes her lips to glaze and her belly to bloat.");
        }
        else if (player.cumQ() < 1500)
        {
            outputText("makes her look pregnant and vents a puddle of syrupy-spooge on top of you.");
        }
        else
        {
            outputText("makes her look like a hugely-pregnant fuck-beast and dumps cum everywhere.");
        }
        outputText("\n\n");
        ...
        if (player.cumQ() >= 500)
        {
            outputText(", save for her pregnant belly and ");
        }
        else
        {
            outputText(", save for her ");
        }
    ...
    private function cinnabarHuger() : Void
    {
        ...
        {
            outputText(num2Text(Math.round(player.cocks[x].cockLength)) + " inches");
        }
        else
        {
            outputText(num2Text(Math.round(player.cocks[x].cockLength / 12)) + " feet");
        }
        outputText(" all over again.");
        if (player.cocks[x].cockLength >= 60)
        {
            outputText("  There's no way this should be possible, but it's happening and you couldn't be happier.");
        }
        outputText("\n\n");
        ...
        if (player.cumQ() >= 500)
        {
            ...
            if (player.cumQ() < 1000)
            {
                outputText("spooge-paunch from all the cum.");
            }
            else if (player.cumQ() < 1500)
            {
                outputText("spooge-packed belly from all the cum.");
            }
            else
            {
                outputText("spooge-pregnancy from all the cum.  It swings underneath her, making her look ready to birth.");
            }
            outputText("  ");
        }
        ...
        if (player.cocks[x].cockLength >= 60)
        {
            outputText("that it should be sticking up past her head, yet somehow isn't.");
        }
        else
        {
            outputText("to know that she must be some kind of magical to take a fucking like that without damage.");
        }
        ...
        if (player.cumQ() >= 1000)
        {
            outputText(", the sloshing in her belly still quite audible");
        }
        outputText(".");
        player.sexReward("vaginalFluids", "Dick");
        dynStats("sen", -4);
    ...
    {
        flags[kFLAGS.CINNABAR_FUCKED_CORRUPT_PREVIOUSLY] = 0;
        var x : Float = player.findCock(1, 100, 200);
        ...
        if (!player.hasSheath())
        {
            outputText("the base of your twitching cock");
        }
        else
        {
            outputText("your twitching cock-sheath");
        }
        ...
        if (player.balls == 0)
        {
            outputText("prostate has been squeezed dry.");
        }
        else
        {
            outputText(ballsDescriptLight() + " feel sore and empty, completely drained.");
        }
        if (player.cumQ() > 500)
        {
            outputText("  The rat-girl has gained a nice, ");
            if (player.cumQ() < 1000)
            {
                outputText("jiggling spooge-paunch from all the cum.");
            }
            else if (player.cumQ() < 1500)
            {
                outputText("spooge-packed belly from all the cum.");
    ...
    private function cinnabarMultiCockPortalFuckFest() : Void
    {
        ...
        var x : Float = player.biggestCockIndex();
        var y : Float = player.biggestCockIndex2();
        var z : Float = player.biggestCockIndex3();
        if (player.cockTotal() < 4)
        {
            sceneHunter.print("Failed check: up to 4 cocks usable!");
        }
        outputText("Cinnabar's hands fly through your [armor], diving through your undergarments to squeeze " + sMultiCockDesc() + ".  ");
        if (player.lust >= 70)
        {
            outputText("She groans when she feels the full, turgid masses in her fingers, shivering as her padded fingertips become slippery with your dripping pre-cum.");
        }
        else
        {
            outputText("She sighs blissfully when she feels the size and number of your half-hard members, squeezing them with gentle pressure until they start to expand, engulfing her hand.");
        }
        ...
        {
            outputText("under your arm");
        }
        else
        {
            outputText("up against you");
        }
        ...
        if (flags[kFLAGS.CINNABAR_HOUSE_VISITED] == 0)
        {
            outputText("She leans, guiding you towards her home even as she lets herself be led.");
        }
        else
        {
            outputText("She blushes hard, her thighs darkening from the fluid racing down them as you pull her towards her house.");
        }
        outputText("\n\n");
        ...
        if (player.cockTotal() == 4)
        {
            outputText("  The extra dick dives into the pillowy mounds, smearing a streamer of pre-cum over the tit's surface before it slides into that warm, welcoming cleavage.");
        }
        else if (player.cockTotal() > 4)
        {
            outputText("  The extra dicks dive into the pillowy mounds, smearing streamers of pre-cum all over the tits' surfaces before they slide home into that warm, welcoming cleavage.");
        }
        outputText("\n\n");
        ...
        if (player.cockTotal() >= 4)
        {
            outputText(" while you tit-fuck her swollen breasts");
        }
        ...
        if (player.cockTotal() >= 4)
        {
            ...
            if (player.cockTotal() == 4)
            {
                outputText("extra dick");
            }
            else
            {
                outputText("remaining dicks");
            }
            outputText(".");
        }
        ...
        if (player.cockTotal() >= 4)
        {
            outputText(", her tits shining white from all the spooge you dump on to them");
        }
        outputText(".\n\n");
        ...
        if (player.cumQ() >= 500 && player.cumQ() < 1000)
        {
            ...
            if (player.cockTotal() >= 4)
            {
                outputText("  The rat's tits are completely white, spooge leaking off her nipples like some kind of thick, sticky milk.");
            }
        ...
        else if (player.cumQ() >= 1000)
        {
            ...
            if (player.cumQ() >= 1500)
            {
                outputText("  Best of all, her bed is completely soaked in the stuff, and she wallows in it like a pig in mud, her cleavage absolutely drenched with cum.");
            }
            outputText("\n\n");
        }
        ...
        if (player.cumQ() >= 1000)
        {
            outputText(" while her huge, cum-filled belly bulges out to the sides");
        }
