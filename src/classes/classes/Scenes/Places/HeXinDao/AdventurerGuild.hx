...
class AdventurerGuild extends HeXinDaoAbstractContent implements SaveableState
{
    public static var playerGuildLevel(get, never) : Int;
    public static var playerInGuild(get, never) : Bool;

    // Array of pairs [item type, guild rank]
    private static var slotDefs : Array<Dynamic> = [
        //Copper plate 01-02
        ...
        [ItemType.NOTHING, RANK_BRONZE]  //Adamantite plate 26-30    //Orichalcum plate 21-25    //Mithril plate 17-20    //Platinum plate 13-16    //Gold plate 10-12    //Silver plate 08-09    // 07  
    ];
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            ...
            else if (Lambda.has(o, "Slot01"))
            {
                ...
                for (i in 0...lootBag.SlotCount)
                {
                    var s : String = padStart(Std.string(i + 1), 2, "0");
                    lootBag.Slots[i] = numberOr(Reflect.field(o, Std.string("Slot" + s)), 0);
                    lootBag.SlotCaps[i] = numberOr(Reflect.field(o, Std.string("Slot" + s + "Cap")), 0);
                }
            }
            else
            {
    ...
    public static var GuildRanks : Array<Dynamic> = [];
    public static inline var RANK_NONE : Int = 0;
    
    public static inline var RANK_COPPER : Int = 1;
    
    public static inline var RANK_IRON : Int = 2;
    
    public static inline var RANK_BRONZE : Int = 3;
    ...
    {
        for (slot in 0...slotDefs.length)
        {
            if (slotDefs[slot][1] == rank)
            {
                lootBag.SlotCaps[slot] = cap;
            }
        }
    }
    private static function get_playerGuildLevel() : Int
    {
        var i : Int = as3hx.Compat.parseInt(GuildRanks.length - 1);
        while (i > 0)
        {
            if (GuildRanks[i] != null && GuildRanks[i].keyItem && player.hasKeyItem(GuildRanks[i].keyItem) >= 0)
            {
                return i;
            }
            i--;
    ...
    private static function get_playerInGuild() : Bool
    {
    ...
    public function BoardkeeperYangMain() : Void
    {
        ...
        if (playerInGuild)
        {
            ...
            if (player.keyItemvX("Adventurer Guild: Copper plate", 1) > 1 || player.keyItemvX("Adventurer Guild: Iron plate", 1) > 1)
            {
                addButton(2, "Promotion", BoardkeeperYangPromotion).hint("Ask Yang for a promotion once you have achieved enough jobs.");
            }
            else
            {
                addButtonDisabled(2, "Promotion", "Not meet req.");
            }
            addButton(3, "Sex", BoardkeeperYangSex);
            addButton(4, "Back", curry(SceneLib.journeyToTheEast.enteringInn, false));
            if (flags[kFLAGS.AURORA_LVL] == 0.3 || (flags[kFLAGS.AURORA_LVL] >= 0.6 && flags[kFLAGS.AURORA_LVL] < 0.75))
            {
                addButton(5, "GRJ", BoardkeeperYangGolemRetrievalJob).hint("Golem retrieval job");
            }
        }
        else
        {
    ...
    public function BoardkeeperYangQuest() : Void
    {
        ...
        if (flags[kFLAGS.GALIA_LVL_UP] == 0.53 || (flags[kFLAGS.GALIA_LVL_UP] >= 0.5 && flags[kFLAGS.GALIA_TALKS] > 0))
        {
            addButtonDisabled(10, "Ferals (C)", "You already finished this quest.");
        }
        else if (player.statusEffectv2(StatusEffects.AdventureGuildQuests2) >= 2)
        {
            if (flags[kFLAGS.GALIA_LVL_UP] >= 0.44 && flags[kFLAGS.GALIA_AFFECTION] == 10)
            {
                addButton(10, "Ferals (C)", BoardkeeperYangQuestEzekiel1a);
            }
            else
            {
                addButton(10, "Ferals (C)", BoardkeeperYangQuestEzekiel1);
            }
        }
        else
        {
            addButtonDisabled(10, "Ferals (C)", "Req. finishing Ferals(2) quest first.");
        }
    ...
    public function BoardkeeperYangQuestImps1() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.AdventureGuildQuests1))
        {
            ...
            else if (player.hasItem(useables.IMPSKLL, 3, true))
            {
                outputText("Yang examines the skulls to make sure they are imps and then nods.\n\n");
    ...
    public function BoardkeeperYangQuestFerals1() : Void
    {
        ...
        if (player.statusEffectv1(StatusEffects.AdventureGuildQuests2) > 0)
        {
            ...
            else if (player.statusEffectv1(StatusEffects.AdventureGuildQuests2) == 6)
            {
                ...
                {
                    outputText("You try to turn in the quest, but Yang tells you you don’t have enough tentacles yet.\n\n");
                }
            }
            ...
            else if (player.hasItem(useables.SEVTENT, 1, true))
            {
                outputText("You turn in the quest and Yang nods in appreciation.\n\n");
    ...
    public function BoardkeeperYangQuestDemons1() : Void
    {
        ...
        if (player.statusEffectv2(StatusEffects.AdventureGuildQuests1) > 0)
        {
            ...
            else if (player.hasItem(useables.DEMSKLL, 1, true))
            {
        ...
        else
        {
            ...
            {
                player.createStatusEffect(StatusEffects.AdventureGuildQuests1, 0, 1, 0, 0);
            }
        }
        doNext(BoardkeeperYangQuest);
    }
    public function BoardkeeperYangQuestFerals2() : Void
    {
        ...
        if (player.statusEffectv2(StatusEffects.AdventureGuildQuests2) > 0)
        {
            ...
            else if (player.statusEffectv2(StatusEffects.AdventureGuildQuests2) == 6)
            {
                ...
                {
                    outputText("You try to turn in the quest, but Yang tells you you don’t have enough feral imp skulls yet.\n\n");
                }
            }
            ...
            else if (player.hasItem(useables.FIMPSKL, 3, true))
            {
                outputText("You turn in the quest and Yang nods in appreciation.\n\n");
                outputText("\"<i>My my, I wasn’t sure I would ever see you back.</i>\"\n\n");
                outputText("Seems she misjudged you then?\n\n");
        ...
        else
        {
            ...
            {
                player.createStatusEffect(StatusEffects.AdventureGuildQuests2, 0, 1, 0, 0);
            }
        }
        doNext(BoardkeeperYangQuest);
    }
    public function BoardkeeperYangQuestMinotaurs() : Void
    {
        ...
        if (player.statusEffectv3(StatusEffects.AdventureGuildQuests1) > 0)
        {
            ...
            else if (player.statusEffectv3(StatusEffects.AdventureGuildQuests1) == 6)
            {
                ...
                {
                    outputText("\"<i>Hey [name], I counted those horns and clearly you forgot a few. Get out there and bring me the remaining ones.</i>\"\n\n");
                }
            }
            ...
            else if (player.hasItem(useables.MINOHOR, 2, true))
            {
                outputText("Yang counts the horns, then smiles.\n\n");
        ...
        else
        {
            ...
            {
                player.createStatusEffect(StatusEffects.AdventureGuildQuests1, 0, 0, 1, 0);
            }
        }
        doNext(BoardkeeperYangQuest);
    }
    public function BoardkeeperYangQuestGel() : Void
    {
        ...
        if (player.statusEffectv2(StatusEffects.AdventureGuildQuests4) > 0)
        {
            ...
            else if (player.statusEffectv2(StatusEffects.AdventureGuildQuests4) == 4)
            {
                ...
                {
                    outputText("You try to turn in the quest but Yang tells you you don’t have enough gel yet.\n\n");
                }
            }
            ...
            else if (player.hasItem(useables.GREENGL, 5))
            {
                outputText("You turn in the quest and Yang nods in appreciation.\n\n");
        ...
        else
        {
            ...
            {
                player.createStatusEffect(StatusEffects.AdventureGuildQuests4, 0, 1, 0, 0);
            }
        }
        doNext(BoardkeeperYangQuest);
    }
    public function BoardkeeperYangQuestChitin() : Void
    {
        ...
        if (player.statusEffectv1(StatusEffects.AdventureGuildQuests4) > 0)
        {
            ...
            else if (player.statusEffectv1(StatusEffects.AdventureGuildQuests4) == 4)
            {
                ...
                {
                    outputText("You try to turn in the quest but Yang tells you you don’t have enough chitin yet.\n\n");
                }
            }
            ...
            else if (player.hasItem(useables.B_CHITN, 5))
            {
                outputText("You turn in the quest and Yang nods in appreciation.\n\n");
                outputText("\"<i>My my, nice job. I can only hope you kept that weapon clean of innocent blood. Regardless, here is your payment. The client told me to leave you this scroll as a reward. You know what to do with it, I suppose?</i>\"\n\n");
                outputText("(<b>Gained 1 Perk Point</b>)");
        ...
        else
        {
            ...
            {
                player.createStatusEffect(StatusEffects.AdventureGuildQuests4, 1, 0, 0, 0);
            }
        }
        doNext(BoardkeeperYangQuest);
    }
    public function BoardkeeperYangQuestEzekiel1() : Void
    {
        ...
        if (flags[kFLAGS.GALIA_LVL_UP] > 0)
        {
            if (flags[kFLAGS.GALIA_AFFECTION] == 10)
            {
                ...
                if (flags[kFLAGS.GALIA_LVL_UP] == 0.01)
                {
                    flags[kFLAGS.GALIA_LVL_UP] = 0.05;
                }
                else
                {
                    flags[kFLAGS.GALIA_LVL_UP] += 0.05;
            ...
            {
                outputText("You try to turn in the quest but looking inside your bag Yang tells you you cant turn an empty bag in and need to capture an imp first.\n\n");
            }
        }
        else
        {
    ...
    public function BoardkeeperYangGolemRetrievalJob() : Void
    {
        ...
        {
            SceneLib.auroraFollower.putInTheJadeTalismanEpilogue();
        }
    }
    ...
    public function BoardkeeperYangTalkOthers() : Void
    {
        ...
        {
            outputText(" If you want to get in on that action you may want to enlist in the arena gauntlet challenges to draw her attention.");
        }
        outputText("</i>\"\n\n");
        doNext(BoardkeeperYangTalk);
    }
    ...
    public function questItemsBag(page : Int = 0) : Void
    {
        ...
        for (i in 0...lootBag.SlotCount)
        {
            if (lootBag.SlotCaps[i] > 0)
            {
                ...
                bdl.add("Store " + itemType.shortName, curry(storeItem, i, itemType)).disableIf(!player.hasItem(itemType) || lootBag.Slots[i] >= lootBag.SlotCaps[i]);
                bdl.add("Take " + itemType.shortName, curry(takeItem, i, itemType)).disableIf(lootBag.Slots[i] == 0);
            }
        }
        menu();
    ...
    private static var AdventurerGuild_static_initializer = {
