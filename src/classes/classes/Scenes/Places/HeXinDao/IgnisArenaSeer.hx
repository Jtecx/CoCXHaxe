...
class IgnisArenaSeer extends Monster
{
    public var ignisfight : IgnisArenaSeerScene = new IgnisArenaSeerScene();
    
    public function IgnisCastsNuke() : Void
    {
        var defended : Bool = player.hasPerk(PerkLib.SoulSprite) > 0 || player.hasPerk(PerkLib.Archmage) > 0;
        ...
        if (defended)
        {
            outputText("You try and cast makeshift defenses around yourself, with limited success. ");
        }
        else
        {
    ...
    public function IgnisCastsFoxFire() : Void
    {
        ...
        if (!hasStatusEffect(StatusEffects.IgnisCounter))
        {
            createStatusEffect(StatusEffects.IgnisCounter, 1, 0, 0, 0);
        }
        else
        {
            addStatusValue(StatusEffects.IgnisCounter, 1, 1);
        }
    }
    ...
    {
        if (!this.hasStatusEffect(StatusEffects.IgnisCastedNuke))
        {
            IgnisCastsNuke();
        }
        else if (this.statusEffectv1(StatusEffects.IgnisCounter) > 2)
        {
            IgnisCastsComet();
        }
        else
    ...
    {
        var ignisBelow50 : Bool = (HP < maxHP() * 0.5);
    ...
    public function new()
    {
        ...
        {
            this.drop = new ChainedDrop().add(headjewelries.SEERPIN, 1);
        }
        if (flags[kFLAGS.IGNIS_ARENA_SEER] > 2)
        {
            this.drop = new WeightedDrop().add(consumables.MG_SFRP, 1).add(consumables.FOXJEWL, 4);
        }
