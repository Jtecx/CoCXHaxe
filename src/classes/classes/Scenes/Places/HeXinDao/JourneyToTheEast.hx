...
class JourneyToTheEast extends HeXinDaoAbstractContent implements SaveableState
{
    public var riverdungeon : RiverDungeon = new RiverDungeon();
    
    public static var AhriStatsToPerksConvertCounter : Float;
    public static var AhriTavernTalks : Bool;
    public static var AhriStatsToSuperPerksConvertCounter : Float;
    public static var EvelynnPerksToStatsConvertCounter : Float;
    public static var EvelynnTavernTalks : Bool;
    public static var EvelynnCoreLimitBreakerCounter : Float;
    public static var DianaTavernTalks1 : Bool;
    public static var DianaTavernTalks2 : Bool;
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            AhriStatsToPerksConvertCounter = Reflect.field(o, "AhriStatsToPerksConvertCounter");
            AhriTavernTalks = Reflect.field(o, "AhriTavernTalks");
            AhriStatsToSuperPerksConvertCounter = valueOr(Reflect.field(o, "AhriStatsToSuperPerksConvertCounter"), 0);
            EvelynnPerksToStatsConvertCounter = valueOr(Reflect.field(o, "EvelynnPerksToStatsConvertCounter"), 0);
            EvelynnTavernTalks = valueOr(Reflect.field(o, "EvelynnTavernTalks"), false);
            EvelynnCoreLimitBreakerCounter = valueOr(Reflect.field(o, "EvelynnCoreLimitBreakerCounter"), 0);
            DianaTavernTalks1 = valueOr(Reflect.field(o, "DianaTavernTalks1"), false);
            DianaTavernTalks2 = valueOr(Reflect.field(o, "DianaTavernTalks2"), false);
        }
    ...
    public function enteringInn(first : Bool = true) : Void
    {
        clearOutput();
        if (first)
        {
        ...
        if (workHoursMadam() || workHoursTemptress())
        {
            addButton(6, "5/0/0", tableNo5);
        }
        else
        {
            addButtonDisabled(6, "5/0/0", "Table No. 5 is curently empty.");
        }
        if (flags[kFLAGS.MICHIKO_FOLLOWER] < 1)
        {
            addButton(8, "???", SceneLib.michikoFollower.firstMeetingMichiko).hint("You see a suspicious looking squirrel in one corner, nursing a drink and small bowl of peanuts.");
        }
        if (flags[kFLAGS.CURSE_OF_THE_JIANGSHI] < 2 && player.isPureHuman())
        {
            if (flags[kFLAGS.CURSE_OF_THE_JIANGSHI] < 1)
            {
                addButton(9, "???", firstTimeMeetingNekomataBoy).hint("A strange cat morph with two tails is sitting at one of the tables muttering to himself.");
            }
            if (flags[kFLAGS.CURSE_OF_THE_JIANGSHI] == 1)
            {
                addButton(9, "???", firstTimeMeetingNekomataBoy).hint("A strange cat morph with two tails is sitting at one of the tables muttering to himself.");
        ...
        {
            addButton(10, "ShieldMaiden", firstTimeMeetingNeisa);
        }
        if (flags[kFLAGS.NEISA_FOLLOWER] == 2)
        {
            addButton(10, "Neisa", meetingNeisaAfterDecline);
        }
        if (flags[kFLAGS.NEISA_FOLLOWER] == 3)
        {
            addButton(10, "Neisa", NeisabutPCgotKOd);
        }
        if (flags[kFLAGS.NEISA_FOLLOWER] == 4 || flags[kFLAGS.NEISA_FOLLOWER] == 5)
        {
            addButton(10, "Neisa", meetingNeisaPostDungeonExploration).hint("Neisa is sitting at a table enjoying one of the local drinks.");
        }
        if (flags[kFLAGS.NEISA_FOLLOWER] == 6)
        {
            addButton(10, "Neisa", meetingNeisaPostDungeonExploration2).hint("Neisa is sitting at a table enjoying one of the local drinks.");
        }
        if (DianaTavernTalks1)
        {
            addButton(11, "Healer", dianaAtJttEMain).hint("You see a horse woman healer sitting at a table on your left.");
        }
        else
        {
            addButton(11, "???", dianaAtJttEMain).hint("You see a horse woman sitting at a table on your left.");
        }
    ...
    private function confirmBuyDrink(itype : ItemType, price : Int) : Void
    {
        ...
        var priceRate : Float = itype.value / price;
    ...
    private function shadyPerson(second : Bool = true) : Void
    {
    ...
    private function tableNo5() : Void
    {
        ...
        if (workHoursMadam())
        {
            if (AhriTavernTalks)
            {
                addButton(0, "Madam", visitMadam).hint("You see the 'Madam' sitting at the table.");
            }
            else
            {
                addButton(0, "???", visitMadam).hint("You see mysterious looking animal-morph sitting at the table.");
            }
        }
        else if (AhriTavernTalks)
        {
            addButtonDisabled(0, "Madam", "'Madam' isn't currently at her usual spot at table in the inn.");
        }
        if (workHoursTemptress())
        {
            if (EvelynnTavernTalks)
            {
                addButton(1, "Temptress", visitTemptress).hint("You see 'Temptress' sitting at the table.");
            }
            else
            {
                addButton(1, "???", visitTemptress).hint("You see mysterious looking devil sitting at the table.");
            }
        }
        else if (EvelynnTavernTalks)
        {
            addButtonDisabled(1, "Temptress", "'Temptress' isn't currently at her usual spot at table in the inn.");
        }
        addButton(14, "Back", curry(enteringInn, false));
    }
    private function workHoursMadam() : Bool
    {
    ...
    private function visitMadam() : Void
    {
        ...
        if (AhriTavernTalks)
        {
            outputText("\"<i>You came back? What do you seek from this Madam?</i>\" You can swear to see her eyes glow for a moment under the hood as she looks at you. \"<i>Another session to exchange your grown potential to increased ability to develop mystical abilities?</i>\"\n\n");
        }
        else
        {
    ...
    private function visitMadamConvert() : Void
    {
        ...
        else
        {
            ...
            if (AhriStatsToPerksConvertCounter > 0)
            {
                AhriStatsToPerksConvertCounter += 1;
            }
            else
            {
                AhriStatsToPerksConvertCounter = 1;
    ...
    private function workHoursTemptress() : Bool
    {
    ...
    private function visitTemptress() : Void
    {
        ...
        if (EvelynnTavernTalks)
        {
            outputText("\"<i>Did you came back longing for more?</i>\" Her eyes glow for a moment under the hood, her gaze piercing through you. \"<i>Another touch? You wish to exchange your experiences and skills for additional potential?</i>\"\n\n");
        }
        else
        {
    ...
    private function visitTemptressConvert() : Void
    {
        ...
        else
        {
            ...
            if (EvelynnPerksToStatsConvertCounter > 0)
            {
                EvelynnPerksToStatsConvertCounter += 1;
            }
            else
            {
                EvelynnPerksToStatsConvertCounter = 1;
            }
            if (EvelynnCoreLimitBreakerCounter > 0)
            {
                EvelynnCoreLimitBreakerCounter += 1;
            }
            else
            {
                EvelynnCoreLimitBreakerCounter = 1;
    ...
    public function dianaAtJttEMain() : Void
    {
        ...
        if (DianaTavernTalks1)
        {
            outputText("\"<i>You came again.</i>\" Her eyes alight with anticipation. \"<i>Do you need any healing or curing?</i>\"");
        }
        else
        {
        ...
        {
            addButtonIfTrue(0, "Healing", dianaAtJttEMainHeal, "You need to have 50 gems.", player.gems >= 50);
        }
        else
        {
            addButtonDisabled(0, "Healing", "You're fully healed already.");
        }
        ...
        addButton(3, "Uncurse", SceneLib.dianaScene.dianaAtJttECursedItemsRemoval1).hint("Ask horse healer to remove your cursed item. Costs 500 gems. ").disableIf(player.gems < 500, "Ask horse healer to remove your cursed item. Costs 500 gems (Can't afford).").disableIf(player.equippedKnownCursedItems().length == 0 && player.carriedKnownCursedItems().length == 0, "You don't have any cursed items");
        if (player.weaponRange == weaponsrange.SAGITTB)
        {
            addButton(4, "Uncurse", SceneLib.dianaScene.dianaAtJttECursedItemsRemoval2);
        }
        addButton(14, "Back", curry(enteringInn, false));
    }
    public function dianaAtJttEMainHeal() : Void
    {
        ...
        {
            if (player.statusEffectv1(StatusEffects.CombatWounds) > 0.2)
            {
                player.addStatusValue(StatusEffects.CombatWounds, 1, -0.2);
            }
            else
            {
    ...
    {
        var strNeisa : Float = 50;
        strNeisa *= (1 + (0.2 * player.newGamePlusMod()));
        strNeisa = Math.round(strNeisa);
        var meleeAtkNeisa : Float = 12;
        meleeAtkNeisa += (1 + as3hx.Compat.parseInt(meleeAtkNeisa / 5)) * player.newGamePlusMod();
    ...
    public function meetingNeisaPostDungeonExploration2() : Void
    {
        ...
        if (flags[kFLAGS.SPIRIT_STONES] >= 10)
        {
            addButton(1, "Pay", meetingNeisaPostDungeonExplorationPay);
        }
        else
        {
            addButtonDisabled(1, "Pay", "You still not have 10 spirit stones to pay back.");
        }
    ...
    public function meetingNeisaPostDungeonExplorationHireHer() : Void
    {
        ...
        if (flags[kFLAGS.NEISA_FOLLOWER] < 6)
        {
            ...
            if (flags[kFLAGS.NEISA_AFFECTION] < 10)
            {
    ...
    public function meetingNeisaPostDungeonExplorationNo() : Void
    {
        ...
        if (flags[kFLAGS.NEISA_FOLLOWER] == 4)
        {
            flags[kFLAGS.NEISA_FOLLOWER] = 5;
        }
        doNext(enteringInn, false);
    }
    ...
    public function firstTimeMeetingNekomataBoyYesPart2() : Void
    {
        ...
        if (player.gender > 1 && player.biggestTitSize() > 0)
        {
            ...
            if (player.hasCock())
            {
                outputText(" This said the transformation doesn't end there as heat begins to move down to your crotch.");
            }
        }
        if (player.gender == 1)
        {
        ...
        if (!player.weapon.isNothing)
        {
            ...
            {
                flags[kFLAGS.PLAYER_DISARMED_WEAPON_ID] = item.id;
            }
        }
        if (!player.weaponRange.isNothing)
        {
            ...
            if (item != null && !item.isNothing)
            {
                flags[kFLAGS.PLAYER_DISARMED_WEAPON_R_ID] = item.id;
            }
        }
        if (!player.shield.isNothing)
        {
            ...
            if (item != null && !item.isNothing)
            {
                flags[kFLAGS.PLAYER_DISARMED_SHIELD_ID] = item.id;
            }
        }
        if (!player.armor.isNothing)
        {
            ...
            if (item != null && !item.isNothing)
            {
                flags[kFLAGS.PLAYER_DISARMED_ARMOR_ID] = item.id;
            }
        }
        ...
        if (!player.lowerGarment.isNothing)
        {
            ...
            if (item != null && !item.isNothing)
            {
                flags[kFLAGS.PLAYER_DISARMED_UNDERWEAR_BOTTOM_ID] = item.id;
            }
        }
        if (!player.upperGarment.isNothing)
        {
            ...
            if (item != null && !item.isNothing)
            {
                flags[kFLAGS.PLAYER_DISARMED_UNDERWEAR_UPPER_ID] = item.id;
            }
        }
        if (!player.headJewelry.isNothing)
        {
            ...
            if (item != null && !item.isNothing)
            {
                flags[kFLAGS.PLAYER_DISARMED_HEAD_ACCESORY_ID] = item.id;
            }
        }
