...
class ElderStoreHut extends TrollVillageAbstractContent
{
    public function new()
    {
        super();
    ...
    private function TheElderSnacks() : Void
    {
        ...
        var foodStoreTroll : ItemType->Void = function(item : ItemType) : Void
        {
            clearOutput();
            if (player.gems >= 6)
            {
    ...
    public function itemsConsumed(item : String) : Void
    {
        if (item == "AjidAji")
        {
            ...
            player.statStore.addBuff("inte", +0, "AjidAji", {
        ...
        if (item == "D.Morada")
        {
            ...
            if (silly())
            {
                outputText("It’s DEFINITELY not grape. You consider whether or not that was supposed to be a scam or not.");
            }
            ...
            player.statStore.addBuff("inte", +0, "DMorada", {
                        rate : Buff.RATE_HOURS,
                        tick : 1
    ...
    private function TheElderWeapons() : Void
    {
        ...
        var weaponStoreTroll : String->Void = function(item : String) : Void
        {
            var cost : Int = 0;
            var item2 : Weapon;
            switch (item)
        ...
        var nope : Void->Void = function() : Void
        {
        ...
        var buyIt : Weapon->Int->Void = function(item2 : Weapon, cost : Int) : Void
        {
            if (player.gems >= cost)
            {
                player.gems -= cost;
    ...
    private function TheElderArmours() : Void
    {
        ...
        var armourStoreTroll : String->Void = function(item : String) : Void
        {
            var cost : Int = 0;
            var item2 : Dynamic;
            switch (item)
        ...
        var buyIt : Dynamic->Int->Void = function(item2 : Dynamic, cost : Int) : Void
        {
            if (player.gems >= cost)
            {
                player.gems -= cost;
