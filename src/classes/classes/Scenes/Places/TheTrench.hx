...
class TheTrench extends BaseContent implements TimeAwareInterface
{
    ...
    public function timeChangeLarge() : Bool
    {
        ...
        {
            var arigeanProgress : Float = 0;
            var arigeanProgressOncePerNight : Float = 0;
            if (player.statusEffectv1(StatusEffects.ArigeanInfected) == 0)
            {
                ...
                if (player.hasVagina() && player.vaginaType() != VaginaClass.HUMAN)
                {
                    player.vaginaType(0);
                }
            ...
            if (player.statusEffectv1(StatusEffects.ArigeanInfected) == 2 && arigeanProgress == 0 && arigeanProgressOncePerNight == 0)
            {
                ...
                if (EvangelineFollower.EvangelineCuringArigeanInfection == 0)
                {
                    outputText(" Perhaps it would be best to try and find a way to remove this thing from your body before your situation gets any worse.\n");
                }
                if (EvangelineFollower.EvangelineCuringArigeanInfection == 1)
                {
                    outputText(" You can’t help but feel relieved at the reappearance of your tail-like partner, would it hurt to let it stay for a bit?\n");
                }
                if (EvangelineFollower.EvangelineCuringArigeanInfection == 2)
                {
                    outputText(" You can’t help but tightly embrace one of the heads into a hug as a few tears fall from your eyes, oh how you missed them.\n");
                }
            ...
            {
                if (!InCollection(player.hairColor, ArigeanRace.ArigeanHairColors))
                {
                    player.hairColor = randomChoice(ArigeanRace.ArigeanHairColors);
                }
                ...
                if (player.hasVagina())
                {
                    transformations.VaginaArigean().applyEffect(false);
                }
                if (player.hasCock())
                {
                    transformations.CockArigean().applyEffect(false);
                }
            ...
            if (player.statusEffectv1(StatusEffects.ArigeanInfected) == 5 && arigeanProgress == 0 && arigeanProgressOncePerNight == 0)
            {
                ...
                if (player.gender == 2)
                {
                    outputText("eventually stops, however you think it might be best to stay up to see if any other monstrous transformation is going to be brought upon you. Nothing happens and you eventually drift back off into a deep rest.\n");
                }
                else
                {
            ...
            if (player.statusEffectv1(StatusEffects.ArigeanInfected) == 6)
            {
                player.addStatusValue(StatusEffects.ArigeanInfected, 2, 1);
            }
        }
        return false;
    }
    ...
    private function nightSixChoiceEnding() : Void
    {
        ...
        if (!player.hasVagina())
        {
            transformations.VaginaArigean().applyEffect();
        }
    ...
    private function theTrenchQuarters() : Void
    {
        ...
        else
        {
            ...
            {
                outputText("large extra mouth hanging off the bed due to its massive size");
            }
            else if (player.tailType == Tail.ARIGEAN_RED || player.tailType == Tail.ARIGEAN_YELLOW)
            {
                outputText("symbiotic friend wrapping itself around your waist to stay close to the warmth your body offers");
            }
            else
            {
                outputText("parasitic partner partner wrapping itself around your waist to stay close to the warmth your body offers");
            }
    ...
    private function theTrenchGraydaQuestNPCTalkColoring() : Void
    {
        ...
        {
            outputText("were to ask you to give me a massage, you would probably feel compelled to fulfill that order.");
        }
        if (player.tailType == Tail.ARIGEAN_RED)
        {
            outputText("were to order you to strip and patrol the trench nude, you would most likely fulfill that order without hesitation.");
        }
        if (player.tailType == Tail.ARIGEAN_YELLOW)
        {
            outputText("could order you to do something, but since we’re the same rank, it would be entirely up to your choice.");
        }
        outputText("</i>\"\n\n");
    ...
    private function theTrenchGraydaQuestNPCTalkHer() : Void
    {
        ...
        if (player.tailType == Tail.ARIGEAN_GREEN)
        {
            outputText("\"<i>...Recruit, I am not like the beings of the surface, down here it is considered highly rude to indulge oneself in other people’s lives only after the first meeting, I forgive you as you most likely have not known, but do not do this again until we know each other better, recruit.</i>\"\n\n");
        }
        if (player.tailType == Tail.ARIGEAN_RED)
        {
            outputText("\"<i>mmm…</i>\" she shuffles a bit as she puts a hand under her chin. This is clearly a subject she doesn’t enjoy remembering, or talking about. \"<i>I… Was simply someone who was at the wrong place, at the wrong time. Nothing more.</i>\"\n\n");
        }
        if (player.tailType == Tail.ARIGEAN_YELLOW)
        {
    ...
    private function theTrenchGraydaQuestNPCMissions() : Void
    {
        ...
        {
            if (flags[kFLAGS.THE_TRENCH_ENTERED] >= 3)
            {
                addButtonDisabled(0, "Getting Stronger", "You already finished Training Quest 1.");
            }
            else if (player.hasItem(useables.IMPSKLL, 3, true) && player.hasItem(useables.GOBOEAR, 4, true))
            {
                addButton(0, "Getting Stronger", theTrenchGraydaQuestNPCMissionsGettingStronger).hint("Training Quest 1");
            }
            else
            {
                addButtonDisabled(0, "Getting Stronger", "You still need to gather 3 imp skulls and 4 goblin ears.");
            }
        }
        else
        {
        ...
        if (player.level >= 6 && flags[kFLAGS.THE_TRENCH_ENTERED] >= 3)
        {
            ...
            {
                if (flags[kFLAGS.THE_TRENCH_ENTERED] >= 5)
                {
                    addButtonDisabled(1, "Proving Yourself", "You already finished Promotion Quest 1.");
                }
                else if (player.hasItem(useables.SEVTENT, 2, true))
                {
                    addButton(1, "Proving Yourself", theTrenchGraydaQuestNPCMissionsProvingYourself).hint("Promotion Quest 1");
                }
                else
                {
                    addButtonDisabled(1, "Proving Yourself", "You still need to gather 2 severed tentacles.");
                }
            }
            else
            {
        ...
        {
            addButtonDisabled(1, "???", "Req. to finish previous mission and have lvl 6+.");
        }
        if (player.level >= 12 && flags[kFLAGS.THE_TRENCH_ENTERED] >= 5)
        {
            if (flags[kFLAGS.THE_TRENCH_ENTERED] >= 6)
            {
                if (flags[kFLAGS.THE_TRENCH_ENTERED] >= 7)
                {
                    addButtonDisabled(2, "Continued Improvement", "You already finished Training Quest 2.");
                }
                else if (player.hasItem(useables.THHTONG, 4, true) && player.hasItem(useables.MINOHOR, 3, true))
                {
                    addButton(2, "Continued Improvement", theTrenchGraydaQuestNPCMissionsContinuedImprovement).hint("Training Quest 2");
                }
                else
                {
                    addButtonDisabled(2, "Continued Improvement", "You still need to gather 4 hellhound tongues and 3 minotaur horns.");
                }
            }
            else
            {
        ...
        {
            addButtonDisabled(2, "???", "Req. to finish previous mission and have lvl 12+.");
        }
        if (player.level >= 18 && flags[kFLAGS.THE_TRENCH_ENTERED] >= 7)
        {
            if (flags[kFLAGS.THE_TRENCH_ENTERED] >= 8)
            {
                addButtonDisabled(3, "Final Appraisal", "You already finished Training Quest 3.");
            }
            else
            {
                addButton(3, "Final Appraisal", theTrenchGraydaQuestNPCMissionsFinalAppraisal).hint("Training Quest 3");
            }
        }
        else
        {
            addButtonDisabled(3, "???", "Req. to finish previous mission and have lvl 18+.");
        }
        if (player.level >= 24 && flags[kFLAGS.THE_TRENCH_ENTERED] >= 8)
        {
            if (flags[kFLAGS.THE_TRENCH_ENTERED] >= 11)
            {
                addButtonDisabled(4, "Tracking the Monstrosity", "You already finished Promotion Quest 2.");
            }
            else if (flags[kFLAGS.THE_TRENCH_ENTERED] == 9)
            {
                addButtonDisabled(4, "Tracking the Monstrosity", "You still tracking enemy.");
            }
            else
            {
                addButton(4, "Tracking the Monstrosity", theTrenchGraydaQuestNPCMissionsTrackingTheMonstrosity).hint("Promotion Quest 2");
            }
        }
        else
        {
            addButtonDisabled(4, "???", "Req. to finish previous mission and have lvl 24+.");
        }
        if (player.level >= 30 && flags[kFLAGS.THE_TRENCH_ENTERED] >= 12)
        {
            if (player.hasItem(useables.EBONBLO, 4) && player.hasItem(useables.S_INGOT, 1) && player.hasKeyItem("Liquid Diamond"))
            {
                addButton(10, "Coronation", theTrenchGraydaQuestNPCMissionsCoronationFinalPart1).hint("IF YOU CLICK THIS BUTTON CHARACTER WILL BECOME PERNAMENTLY ARIGEAN PRINCESS!!!");
            }
            else if (flags[kFLAGS.THE_TRENCH_ENTERED] >= 13 && flags[kFLAGS.THE_TRENCH_ENTERED] < 14)
            {
                addButtonDisabled(10, "Coronation", "You still need to find: Four Ebonblooms, a Silver ingot and ‘Liquid Diamond’.");
            }
            else
            {
                addButton(10, "Coronation", theTrenchGraydaQuestNPCMissionsCoronation);
            }
        }
        else
        {
            addButtonDisabled(10, "???", "Req. to have lvl 30+ and special event triggering at the camp.");
        }
        addButton(14, "Back", theTrenchGraydaQuestNPC);
    }
    ...
    private function theTrenchGraydaQuestNPCMissionsCoronationFinalPart1() : Void
    {
        ...
        {
            flags[kFLAGS.APEX_SELECTED_RACE] = Races.ARIGEAN;
        }
