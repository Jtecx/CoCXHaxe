...
class BakeryScene extends TelAdreAbstractContent
{
    ...
    public function bakeryuuuuuu() : Void
    {
        ...
        else
        {
            ...
            if (Holidays.nieveHoliday())
            {
                ...
                {
                    addButton(3, "Pudding", SceneLib.holidays.getWinterPudding);
                }
            }
            //Normal repeats!
            else
            {
                outputText("You step into the bakery's domed interior and inhale, treated to a symphony of pleasant smells and the cozy warmth that radiates from the baking ovens.  There are plenty of tables and chairs around for one to eat at, and you find yourself stepping into line while you glance at the menu.\n\n");
            }
        }
        //Times visited!
    ...
    private function talkToBaker() : Void
    {
        ...
        if (flags[kFLAGS.MADDIE_QUEST_STATE] >= 4)
        {
            outputText("  Or you want special order?");
        }
        outputText("</i>\"");
    ...
    private function buyRoot() : Void
    {
        ...
        {
            inventory.takeItem(consumables.RDRROOT, ingredientsMenu);
        }
    }
    ...
    //choices("Brownies",createCallBackFunction2(nomnomnom, "brownies", 3),"Cookies",2831,"Cupcakes",2833,"Doughnuts",createCallBackFunction2(nomnomnom, "doughnuts", 5),"Pound Cake",createCallBackFunction2(nomnomnom, "pound cake", 4),"Fox Berry",buyFoxBerry,"SpecialEclair",minoCum,"GiantCupcake",gcupcake,rubiT,rubiB,"Leave",telAdreMenu);
    {
        ...
        {
            addButton(3, "Frosty", SceneLib.telAdre.frosty.approachFrosty);
        }
        else
        {
            addButton(3, "PinkGirl", SceneLib.telAdre.frosty.approachFrosty);
        }
        addButton(14, "Leave", bakeryuuuuuu);
    }
    
    public function nomnomnom(name : String, price : Float) : Void
    {
        ...
        if (flags[kFLAGS.TEMP_STORAGE_PASTRY_NAME] == "eclair")
        {
            ...
            {
                outputText("males fill their pants");
            }
            else if (player.femininity <= 25)
            {
                outputText("females squirm");
            }
            else
            {
                outputText("other patrons squirm and fill out their pants");
            }
...
[mino cum eclair] – helps your cravings and – tone!, LUST!*/
    ...
    private function easterBakeSale() : Void
    {
        ...
        if (telAdre.rubi.rubiAffection() >= 40)
        {
            outputText("An extremely busy Rubi can only manage a wave in your direction before going back to attending customers.  ");
        }
    ...
    private function easterBakeSaleHelp() : Void
    {
        ...
        {
            outputText("Your [fullChest] bounce at the vigor of your movement.  ");
        }
        ...
        if (player.gender == 1)
        {
            outputText("[eachCock] jumps to full hardness.");
        }
        //(if the pc is female)
        else if (player.gender == 2)
        {
            outputText("your nipples stiffening noticeably, while your [vagina] prepares for what's to come.");
        }
        //(if the pc is a herm)
        else
        {
            outputText("[eachCock] jumping to full hardness, your nipples and [vagina] not far behind in getting ready for your encounter.");
        }
    ...
    private function malesHelpOutWithEaster() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText(", sticky drops of chocolate trailing down your [balls]");
        }
        ...
        if (silly())
        {
            outputText("You regret not bringing your umbrella for this Chocolate Rain, so that you could be like those that stay dry rather than those who feel the pain.  ");
        }
        ...
        {
            outputText("Your cock spews out a significant amount of seed, filling your partner's deepness quickly while a small volume shoots out with some force.  You are happy to see that she seems to have gained a little weight from your baby-batter.");
        }
        //(very large cum vol)
        else if (player.cumQ() < 5000)
        {
            outputText("Your cock spews into your partner's deepness, filling it almost instantly while a significant volume splatters out.  You are happy to see she seems to have gained a little weight from your baby-batter.");
        }
        else
        {
            outputText("Your cock opens like a river, streaming into your partner with such force that her belly distends.  A spew begins to erupt from her vagina, empting the significant amount she could not take on to the floor.  You are happy to see she has gained some weight from your baby-batter.");
        }
