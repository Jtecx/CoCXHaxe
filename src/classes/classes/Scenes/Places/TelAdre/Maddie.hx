...
class Maddie extends TelAdreAbstractContent
{
    //VARS
    ...
    private function procMaddieOneIntro() : Void
    {
        clearOutput();
        if (flags[kFLAGS.MINOCHEF_FIRST_MEETING] == 0)
        {
            ...
            {
                outputText("looking down at");
            }
            else if (player.tallness < 100)
            {
                outputText("staring levelly at");
            }
            else
            {
                outputText("glaring up at");
            }
    ...
    private function followMinotaurIntoBackroom() : Void
    {
        ...
        if (flags[kFLAGS.MADDIE_QUEST_STARTED] == 0)
        {
            ...
            {
                outputText("You lick your lips when you realize you're meeting the source of the 'special' éclairs.");
            }
            else
            {
                outputText("You blush when you realize what he must be using for cream filling.");
            }
            //[Give Them] [Leave]
            ...
            {
                simpleChoices("", null, "", null, "", null, "", null, "Leave", camp.returnToCampUseOneHour);
            }
    ...
    public function nopeAintGotNoneODemSpeculIngredimathings() : Void
    {
        ...
        if (player.cor > 50)
        {
            outputText("starting a fight inside Tel'Adre");
        }
        else
        {
            outputText("overstaying your welcome");
        }
    ...
    public function handOverIngredientsItBeBakingTimeYo() : Void
    {
        ...
        if (player.hasPerk(PerkLib.MinotaurCumAddict))
        {
            outputText("  You lurch forward involuntarily, craving the rest of his jism, but he pushes you into the wall and grunts, \"<i>No,</i>\" in a tone that brooks no disagreement.  It actually shocks you out of your addicted haze.");
        }
        outputText("\n\n");
        ...
        if (player.hasPerk(PerkLib.MinotaurCumAddict))
        {
            outputText("  Your mouth salivates at the thought.");
        }
        else
        {
            outputText("You aren't sure you want to.");
        }
        outputText("\n\n");
        if (player.hasPerk(PerkLib.MinotaurCumAddict))
        {
            doNext(waitForSlutCake);
        }
        //[Wait] [Sneak Out]
        else
        {
            simpleChoices("Wait", waitForSlutCake, "Sneak Out", sneakAwayFromMaddie, "", null, "", null, "", null);
        }
    }
    ...
    private function waitForSlutCake() : Void
    {
        ...
        {
            outputText("nervous about this whole thing");
        }
        else
        {
            outputText("intrigued by this whole thing");
        }
        outputText(".  One of the waitresses brings you a glass of milk, and ");
        if (player.cor < 50)
        {
            outputText("it smells normal enough, so you go ahead and sip on it");
        }
        else
        {
            outputText("you sip on it while you wait");
        }
    ...
    public function talkToMaddie() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("is like, all out of icing mix!  So I'm going to borrow a few cups from you, 'kay?");
        }
        else
        {
            outputText("gave me so much icing mix, and you like, would look soooo much better with some vanil- van... yummy frosting!");
        }
        ...
        function dickF() : Void
        {
            ...
            if (player.cor < 33)
            {
                outputText("You muster as much authority as you can in such a compromising position and explain to Maddie that what comes out of there is NOT icing.");
            }
            else if (player.cor < 66)
            {
                outputText("You offhandedly mention that you don't actually make icing.");
            }
            else
            {
                outputText("You smirk and mention that what you squirt isn't quite icing.");
            }
            ...
            if (!player.hasSheath())
            {
                outputText("encircles the base");
            }
            else
            {
                outputText("pokes and prods into your sheath");
            }
            ...
            if (player.hasBalls())
            {
                outputText(", the cum-heavy spheres bouncing in your twitching sack as they get ready to explode");
            }
            ...
            if (player.cockTotal() == 1)
            {
                outputText(", your " + cockDescript(x) + " emptying every drop of jizz into the baked cum-tank.");
            }
            else
            {
            ...
            if (player.cockTotal() > 1)
            {
                if (player.cockTotal() == 2)
                {
                    outputText(" along with your other penis");
                }
                else
                {
                    outputText(" along with your other dicks");
                }
            }
        ...
        var drinkF : Void->Void = function() : Void
        {
            ...
            if (player.thickness < 60)
            {
                outputText(", you're looking awful thin");
            }
            else if (player.tone >= 70)
            {
                outputText(", you look like you're carved from stone.  A little softness would do you good");
            }
            else
            {
                outputText(", you look like you'd better eat to keep up your gorgeous figure");
            }
            ...
            if (player.thickness < 60 || player.tone >= 50)
            {
                outputText("forward");
            }
            else
            {
                outputText("more forward than normal");
            }
    ...
    @:allow(classes.scenes.places.telAdre)
    private function bakeryEpilogue() : Void
    {
        clearOutput();
