...
class Library extends TelAdreAbstractContent implements TimeAwareInterface
{
    public function timeChange() : Bool
    {
        if (flags[kFLAGS.MALI_BLADE_STATUS] > 1)
        {
            --flags[kFLAGS.MALI_BLADE_STATUS];
        }
        return false;
    }
    ...
    private static function quinnHere() : Bool
    {
    ...
    {
        if (flags[kFLAGS.TIMES_BEEN_TO_LIBRARY] == 0)
        {
            firstTowerVisit();
        }
        else
        ...
        if (quinnHere())
        {
            ...
            if (flags[kFLAGS.DOMINIKA_COVENANT] > 0)
            
            //nothing but the quest, sadly{
            ...
            if (player.gems >= 200)
            {
                addButton(11, "R.G.Tome", buyRestrainedTome, 200, weaponsrange.RG_TOME).hint("Buy Restrained Grey Tome.");
            }
        }
    ...
    private function buyRestrainedTome(cost : Float, type : ItemType) : Void
    {
        clearOutput();
        player.gems -= cost;
        statScreenRefresh();
    ...
    private function firstTowerVisit() : Void
    {
        ...
        else
        {
            ...
            flags[kFLAGS.TIMES_BEEN_TO_LIBRARY] = -2;
        }
    }
    ...
    private function studyInTA() : Void
    {
        ...
        else
        {
            ...
            //magic){
                ...
                for (spell in spells)
                {
                    if (player.inte >= Reflect.field(spell, Std.string(0)) && !player.hasStatusEffect(Reflect.field(spell, Std.string(1))))
                    {
                        player.createStatusEffect(Reflect.field(spell, Std.string(1)), 0, 0, 0, 0);
                        outputText("\n\nYou blink in surprise, assaulted by the knowledge of a <b>new spell: " + (try cast(Reflect.field(spell, Std.string(1)), StatusEffectType) catch(e:Dynamic) null).id.substr(6) + "</b>");  //substring to remove "Knows "  
                        break;
                    }
                }
            }
            //OR (player is bimbo/bimbro/whatever)
            else if ((player.lib > 75 || player.cor > 75 || player.hasPerk(PerkLib.BimboBrains) || player.hasPerk(PerkLib.FutaFaculties) || player.hasPerk(PerkLib.BroBrains)) && rand(2) == 0)
            {
