...
class Frosty extends TelAdreAbstractContent
{
    ...
    //Original prices, lol
    {
        ...
        if (flags[kFLAGS.SIGNED_FROSTYS_CONTRACT] == 0)
        {
            addButton(8, "Contract", getAFuckingFuckContractFromFucks);
        }
        else
        {
            addButton(8, "Specials", frostysLimitedSpecialMenu);
        }
        addButton(14, "Leave", SceneLib.telAdre.telAdreMenu);
    }
    
    public function frostySpecialsMenu(free : Bool = false) : Void
    {
        if (free)
        {
        ...
        addButton(3, "Spanking", frostySpanking).disableIf(flags[kFLAGS.FROSTY_TIMES_SPANKED] == 0 && flags[kFLAGS.FROSTY_POINTS] < 100, "Not enough points (100)!").disableIf(flags[kFLAGS.FROSTY_TIMES_SPANKED] > 0 && flags[kFLAGS.FROSTY_POINTS] < 200, "Not enough points (200)!");
    ...
    public function cupcakeBuy(arg : Int = 1) : Void
    {
        ...
        if (flags[kFLAGS.SIGNED_FROSTYS_CONTRACT] != null)
        {
            frostyPoints(arg);
        }
    ...
    private function frostyPoints(arg : Int = 0) : Int
    {
        ...
        if (flags[kFLAGS.FROSTY_POINTS] < 0)
        {
            flags[kFLAGS.FROSTY_POINTS] = 0;
        }
        return flags[kFLAGS.FROSTY_POINTS];
    }
    ...
    private function analPitcher() : Void
    {
        ...
        {
            outputText(", just like  you. Knowing this centauress is watching ass fuck this pink slut makes your cock even harder, like you can feel her eyes on your cock. You could blow your load into her colon right now, but you hold back. You want this curious horse-cunt to look on longer.");
        }
        ...
        if (player.cumQ() < 500)
        {
            outputText("small trickle of cum that just drips out of her gaping hole.");
        }
        else if (player.cumQ() < 1000)
        {
            outputText("decent stream of cum that flows for a second and then just starts to drip.");
        }
        else if (player.cumQ() < 2000)
        {
            outputText("flood of cum following your cock out of the gaping hole. ");
        }
        else
        {
            outputText("huge flood of cum, you have to hold Frosty’s legs up for a few more minutes before all the jizz comes out of her, almost sounding like a water faucet when it hits the ground. The ground and your feet are soaked in the stuff now.");
        }
    ...
    private function analCatcher() : Void
    {
        ...
        {
            outputText("Her entire hand feels up your [cocks] while her fingers rub your [vagina].");
        }
        else if (player.hasCock())
        {
            outputText("She traces the outline of your [cockplural] on your [armor] with her fingertip.");
        }
        else if (player.hasVagina())
        {
            outputText("She rubs a finger along the length of your [cunt], damping the crotch of your [armor].");
        }
        else
        {
            outputText("Her hand is looking for something that isn’t there, and you smirk a little at her efforts.");
        }
        outputText("\n\n");
        ...
        if (player.hasCock())
        {
            outputText("[cock] to get hard and beads of pre form on " + ((player.cockTotal() > 1) ? "their throbbing tips" : "its throbbing  tip"));
        }
        if (player.hasVagina())
        {
            if (player.hasCock())
            {
                outputText("before dripping onto your already sodden [vagina].");
            }
            else
            {
                outputText("cunt to quiver in excitement");
            }
        }
        if (!player.hasCock() && !player.hasVagina())
        {
            outputText("heart to beat in anticipation");
        }
        outputText(".\n\n"
        ...
        if (player.isNaga())
        {
            outputText("snake-tail, the tip of it");
        }
        else
        {
            outputText("[legs], one");
        ...
        if (player.hasCock() && player.hasVagina())
        {
            outputText("your sexes release a huge mix of male and female cum, making a huge mess by your feet.");
        }
        else if (player.hasCock())
        {
            outputText("your [cock] shoots out streams of white baby batter.");
        }
        else if (player.hasVagina())
        {
            outputText("your [vagina] gives one last quiver and twitches, releasing a gusher of girl-cum down your thighs.");
        }
        else
        {
            outputText("you feel an orgasm building up in your groin and then releasing a glow of ecstasy across your whole body.");
        }
        outputText("\n\n"
    ...
    private function spankingYes() : Void
    {
        ...
        frostyPoints(-100 * ((flags[kFLAGS.FROSTY_TIMES_SPANKED] != null) ? 2 : 1));
        menu();
    ...
    private function getSpanked() : Void
    {
        ...
        outputText(" with every inch she raises it. Higher and higher it goes, above her head and as far as her arm can strech. She's holding it high in place, watching you squirm and writhing, awaiting this pain you so rightfully deserve."
                + "\n\n"
                + "<b>WHACK!</b> The paddle hits in pleasurable pain. You moan out as you throw your head back. Soon there are three quick slaps to your rear, every slap feels like its own orgasmic wave of pleasure; like a tsunami, it spreads throughout your body. From the top of your head to the tips of your "
                + ((player.isNaga()) ? "tail" : "toes")
                + ", you shake with pleasure and sweat in pain. Your sexy bits are brimming with arousal in the wake of these waves. Three more quick whacks give you a burst of lust that you could just overdose on. You start panting with your tongue out as your eyes begin to form tears in reaction to the pain, trying to betray your true feelings."
                + "\n\n"
        ...
        if (player.hasCock() || player.hasVagina())
        {
            outputText(", getting closer to" + ((player.hasCock() && player.hasVagina()) ? "their" : "its") + "climax, aching with pain-fueled lust and pleasure");
        }
        outputText("."
                + "\n\n"
                + "Frosty becomes a spanking machine cranked up to its maximum! Your "
                + ((player.isNaga()) ? "tail" : "toes")
                + " curl up as your bottom is brutally and mercilessly tortured by this baker’s studded paddle. The moans of gleeful pleasure that roar through your throat are overcomed the hard, smacking sound of wood on the glowing-red flesh. Even as "
                + ((player.isGenderless()) ? "a phantom orgasm rips through your body" : "you cum like a gusher")
                + ", Frosty is chastising your ass like a nanny. Your euphoric state has you sprawled out over Frosty’s pink thigh as she starts to gently paddle your rear, every second she gets slower with her punishing thwacks. Then she stops with her paddle high in the air."
                + "\n\n"
