...
class Heckel extends TelAdreAbstractContent
{
    public function new()
    {
        super();
...
*/
    ...
    public function greetHeckel() : Void
    {
        ...
        {
            // Affection = 70, after first-time sex, talk to Heckel between 13:00 and 15:00, must not be a first-time encounter with Heckel, requires a genderif (telAdre.brooke.brookeAffection() >= 70 && flags[kFLAGS.BROOKE_MEDIUM_SCENE] > 0 && flags[kFLAGS.MET_HECKEL] > 0 && model.time.hours >= 13 && model.time.hours <= 15 && (player.hasVagina() || player.cockThatFits(telAdre.brooke.brookeCapacity()) >= 0))
            {
    ...
    private function heckelTraining() : Void
    {
        ...
        if (flags[kFLAGS.LOW_STANDARDS_FOR_ALL] == null && player.tone < 60)
        {
    ...
    //anal
    private function heckelLovesDaAnal(loss : Bool = false) : Void
    {
        clearOutput();
        ...
        if (player.tone >= 80)
        {
            outputText("  \"<i>Damn [name], you've got one hell of an ass. I bet it's gonna feel even better than it looks.</i>\"");
        }
        //[<IF TONE BETWEEN 60-79>
        else if (player.tone >= 60)
        {
            outputText("  \"<i>Mmm, not bad [name], not perfect, but not bad.</i>\"");
        }
        outputText("\n\n");
    ...
    private function heckelLovesAnal2() : Void
    {
        ...
        if (player.hasCock() && player.hasVagina())
        {
            outputText("Although " + sMultiCockDesc() + " is trapped beneath you, Heckel's balls slap into your cunt with every one of her quickening thrusts.  Although neither your [cocks] or " + vaginaDescript(0) + " are getting much stimulation, your prostate is being milked better than it ever has. One thrust in particular makes you cry out, and Heckel laughs above you as she slams her furry thighs into yours again. \"<i>You moan like a whore, [name], a perfect bitch for me to ride.</i>\"\n\n");
        }
        //[<IF PC HAS PENIS AND BALLS>
        else if (player.hasCock() && player.hasBalls())
        {
            outputText(SMultiCockDesc() + " pulses beneath you with every thrust, her duo of fist-sized balls slapping into your [balls]. You gasp in surprise as one of her thrusts rubs against your prostate, and she barks out a single laugh above you. \"<i>You moan like a whore, [name], a perfect bitch for me to ride.</i>\"\n\n");
        }
        //[<IF PC HAS VAGOO>
        else if (player.hasVagina())
        {
            outputText("Heckel's fist-sized balls slap against your ass and grind your snatch with every thrust, driving you crazy with the desperate need to be filled. You moan in anguish, pushing yourself towards her with every thrust, desperate to milk as much stimulation out of her as possible. She laughs above you, but rewards you with harder and faster thrusts. \"<i>You moan like a whore, [name], a perfect bitch for me to ride.</i>\"\n\n");
        }
        ...
        if (player.gender == 2)
        {
            outputText("  \"<i>I'm going to fuck your ass like you wish I'd fuck your cunt,</i>\"");
        }
        //[<IF PC IS MALE OR HERM >
        else
        {
            outputText("  \"<i>I'm going to ride your ass like you wish you could ride me,</i>\"");
        }
    ...
    public function dominateHeckel() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_DOMMED_HECKEL] == 0)
        {
            outputText("there's a new top dog just waiting to claim the town bitch");
        }
        else
        {
            outputText("you're back for a taste of your favorite bitch");
        }
        outputText(".  Heckel snorts, \"<i>Nice try, fuck-meat, but ");
        if (flags[kFLAGS.TIMES_DOMMED_HECKEL] > 0)
        {
            outputText("last time was a fluke");
        }
        else
        {
            outputText("I'm always on top");
        }
        ...
        if (flags[kFLAGS.TIMES_LOST_HECKEL_DOM_CHALLENGE] > 0)
        {
            outputText(" again");
        }
        ...
        if (player.tou < 60)
        {
            outputText(", pushing you back a bit");
        }
        outputText(".");
    ...
    private function dominateHeckelConclusion() : Void
    {
        ...
        {
            outputText("You grin and blush hotly, imagining the whole of the gym watching you fuck this hyena, their watchful eyes exciting you further.");
        }
        else if (player.cor < 33)
        {
            outputText("You blush a bit at that, uncomfortable at your sex being so public but unwilling to stop.");
        }
        else if (player.cor < 66)
        {
            outputText("You smirk a little at that while barely acknowledge the worry of being caught nagging at your consciousness.");
        }
        else
        {
            outputText("You grin widely at that and secretly hope that the whole gym will come in to see their 'alpha' laid low.");
        }
        ...
        {
            outputText(", fanning the others across her neck and forehead, burying her in penis.");
        }
        else if (player.cockTotal() == 2)
        {
            outputText(", dropping your [cock smallest] alongside it over her eyes, burying her in penis.");
        }
        else
        {
            outputText(", draping her in hard penis.");
        }
        ...
        {
            outputText("s throb and thicken");
        }
        else
        {
            outputText(" throbs and thickens");
        }
        ...
        if (player.cumQ() < 250)
        {
            outputText("single drop");
        }
        else if (player.cumQ() < 1000)
        {
            outputText("steady drip");
        }
        else
        {
            outputText("never-ending flow");
        }
        ...
        {
            outputText("as much of it as possible");
        }
        else
        {
            outputText("it");
        ...
        if (player.hasBalls())
        {
            outputText(", basting your [balls] with her steaming warmth");
        }
        else if (player.hasVagina())
        {
            outputText(", basting your [vagina] with her steaming warmth");
        }
        else
        {
            outputText(", basting your [skin] with her steaming warmth");
        }
        ...
        if (player.cumQ() > 500)
        {
            ...
            if (player.cumQ() >= 1000)
            {
                outputText("  Still, you keep blasting wad after wad of spunk inside her, and her belly rounds nicely, puffed up with virile sperm.");
            }
            if (player.cumQ() >= 2000)
            {
                outputText("  Your spooge is seemingly without end, pouring out endlessly long after she's spunk-gravid and nearly immobile.  The rivers that run from her abused cunt-lips quickly turn the table into a puddle.");
            }
            if (player.cumQ() >= 3000)
            {
                ...
                if (player.cumQ() >= 4000)
                {
                    ...
                    {
                        outputText("a foot deep");
                    }
                    else if (player.cumQ() < 15000)
                    {
                        outputText("knee high by most measurements");
                    }
                    else if (player.cumQ() < 20000)
                    {
                        outputText("waist high");
                    }
                    else
                    {
                        outputText("as high as the table");
                    }
                }
        ...
        if (silly())
        {
            ...
            {
                outputText(", then give Lottie a high five");
            }
