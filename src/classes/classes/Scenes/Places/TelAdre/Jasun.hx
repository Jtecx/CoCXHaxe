...
class Jasun extends TelAdreAbstractContent
{
    ...
    //Charge for gym if no lifetime member!
    {
        ...
        {
            simpleChoices("Jasun", meetJasun, "", null, "", null, "", null, "Leave", telAdre.gymDesc);
        }
    }
    ...
    private function meetJasun() : Void
    {
        ...
        else if (flags[kFLAGS.JASUN_MET] > 0 && flags[kFLAGS.JASUN_TIMES_FUCKED] == 0)
        {
            ...
            if (player.tone < 75 || player.armorName != "slutty swimwear")
            {
                ...
                if (player.tone < 75)
                {
                    outputText("fit enough");
                }
                else if (player.armorName != "slutty swimwear")
                {
                    outputText("properly attired");
                }
                else
                {
                    outputText("a little more compatible with me");
                }
    ...
    private function meetJasun2() : Void
    {
        ...
        if (player.str < 50)
        {
            outputText("that you're pretty sure you couldn't have moved on your own at all ");
        }
