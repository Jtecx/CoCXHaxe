...
class Loppe extends NPCAwareContent implements TimeAwareInterface implements SaveableState
{
    private var canGetPreg(get, never) : Bool;

    
    //public var loppeTrainingOnYou:int;	//0 for not started, 1~4 for started, 5 for finished
    public var loppeCured : Bool;
    public var loppeTalkedAboutKids : Bool;
    public var usagiHasReturned : Bool;
    public var usagiReacted : Bool;
    public var pcFirstLoppeKits : Bool;
    ...
    public function loadFromObject(o : Dynamic, ignoreErrors : Bool) : Void
    {
        if (o != null)
        {
            loppeCured = (Lambda.has(o, "loppeCured")) ? Reflect.field(o, "loppeCured") : false;
            loppeTalkedAboutKids = (Lambda.has(o, "loppeTalkedAboutKids")) ? Reflect.field(o, "loppeTalkedAboutKids") : false;
            usagiHasReturned = (Lambda.has(o, "usagiHasReturned")) ? Reflect.field(o, "usagiHasReturned") : false;
            usagiReacted = (Lambda.has(o, "usagiReacted")) ? Reflect.field(o, "usagiReacted") : false;
            pcFirstLoppeKits = (Lambda.has(o, "pcFirstLoppeKits")) ? Reflect.field(o, "pcFirstLoppeKits") : true;
        }
        else
        {
            resetState();
...
*/
    ...
    private function yesToLoppeMeetingTwo() : Void
    {
        ...
        if (player.cor < 40)
        {
            outputText(" as politely as possible. It felt nice to see a display like that here; makes it feel a little less like a wasteland of sex-mad monsters and a little more like home");
        }
        outputText(".");
    ...
    private function sureBakeryWithLoppe() : Void
    {
        ...
        if (player.gems >= 30)
        {
            ...
            if (player.cor > 40)
            {
                outputText("  A small voice inside wonders if she'd eat so extravagantly were she footing her own bill.");
            }
            //remove 30 gems
            player.gems -= 30;
            statScreenRefresh();
    ...
    public function loppeGenericMeetings() : Void
    {
        spriteSelect(SpriteDb.s_Loppe_16bit);
        clearOutput();
        if (flags[kFLAGS.LOPPE_TRAINING] >= 5 && !loppeTalkedAboutKids)
        {
            talkAboutKids();
        }
        else if (usagiHasReturned && !usagiReacted)
        {
            usagiReturns();
        }
        else if (flags[kFLAGS.LOPPE_KIDS] >= flags[kFLAGS.LOPPE_KIDS_LIMIT] && flags[kFLAGS.LOPPE_FERTILE] == 1)
        {
            noMoreBabies();
        }
        else if (flags[kFLAGS.LOPPE_KIDS] >= flags[kFLAGS.LOPPE_KIDS_LIMIT] && flags[kFLAGS.LOPPE_FERTILE] == 0 && flags[kFLAGS.LOPPE_KIDS_LIMIT] == 8 && usagiHasReturned)
        {
            letsHaveMoreKiddies();
        }
    ...
    private function talkWithLoppe() : Void
    {
        ...
        {
            addButton(1, "Children", askLoppeAboutChildren);
        }
        //Gossip (after sexin' once)
        if (flags[kFLAGS.LOPPE_TIMES_SEXED] > 0)
        {
            addButton(2, "Gossip", gossipWithLoppe);
        }
        //Working (after [Loppe]
        if (flags[kFLAGS.TIMES_ASKED_LOPPE_ABOUT_LOPPE] > 0)
        {
            addButton(3, "Working", talkWithLoppeAboutWorking);
        }
        //Her Mom (available after choosing \"<i>Loppe</i>\" at least once)
        if (flags[kFLAGS.TIMES_ASKED_LOPPE_ABOUT_LOPPE] > 0)
        {
            addButton(4, "Her Mom", chatWithLoppeAboutHerMom);
        }
        //Her Village (after [Loppe]
        if (flags[kFLAGS.TIMES_ASKED_LOPPE_ABOUT_LOPPE] > 0)
        {
            addButton(5, "Her Village", chatWithLoppeAboutLoppesVillage);
        }
        //The curse (unfinished; available after choosing \"<i>Loppe</i>\" at least once)
        //Fondle (after "Loppe"; see appropriate section)
        if (flags[kFLAGS.TIMES_ASKED_LOPPE_ABOUT_LOPPE] > 0 && flags[kFLAGS.LOPPE_TIMES_SEXED] > 0)
        {
            addButton(6, "Tease Her", fondleAndTease);
        }
    ...
    private function askLoppeAboutChildren() : Void
    {
        ...
        if (flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] + flags[kFLAGS.COTTON_KID_COUNT] > 0)
        {
    ...
    private function gossipWithLoppe() : Void
    {
        ...
        {
            addButton(1, "Scylla", gossipWithLoppeAboutScylla);
        }
        //Jasun
        if (flags[kFLAGS.JASUN_MET] > 0)
        {
            addButton(2, "Jasun", gossipWithLoppeAboutJasun);
        }
        //Heckel
        if (flags[kFLAGS.MET_HECKEL] > 0)
        {
            addButton(3, "Heckel", gossipWithLoppeAboutHeckel);
        }
        //Edryn
        ...
        {
            addButton(5, "Lottie", gossipWithLoppeAboutLottie);
        }
        //Cotton
        if (flags[kFLAGS.COTTON_MET_FUCKED] > 0)
        {
            addButton(6, "Cotton", gossipWithLoppeAboutCotton);
        }
    ...
    private function chatWithLoppeAboutHerMom() : Void
    {
        ...
        if (player.gender == 1)
        {
            outputText("  Well, from what Loppe's told you, Uma wouldn't be attracted to you anyway.");
        }
    ...
    //Sex
    private function loppeSexChoice(bakery : Bool = false) : Void
    {
        clearOutput();
        ...
        //first-time only output, if not coming from bakery date{
            ...
            if (!bakery)
            {
                ...
                else
                {
                    ...
                    if (player.lib >= 40)
                    {
                        outputText(".. it almost sounds exciting.");
                    }
                }
            ...
            if (player.cor < 40)
            {
                outputText("  You tell her they're quite nice... though privately you're curious what the interior looks like.");
            }
            else
            {
                outputText("  They don't seem to work very well - after all, <i>you're</i> here.");
            }
            ...
            if (player.hasCock())
            {
                if (player.smallestCockArea() <= loppeCapacity())
                {
                    outputText("Pulling her onto your dick would result in some cowgirl fun, though you'd have to deal with her cock pointed right at you when she came.  ");
                }
                else
                {
                    outputText("As good as it would feel to shove your [cock smallest] into her cunt, the nervous looks she's giving it tell you what her response would probably be.  ");
                }
            }
            //[(any cock)
            if (player.hasCock())
            {
                outputText("You could just whip it out and ask what she thinks of it, compared to hers - it might lead to some cross words and crossed swords, though.  ");
            }
            if (player.hasVagina())
            {
                outputText("You could take her monster length vaginally, but if her libido is what she claims, you'll probably wind up quite stretched.  ");
            }
            //[(boobs >= boobjob req)
            if (player.biggestTitSize() >= 5)
            {
                outputText("Her hardening horsecock looks like it would fit between your [chest], an act likely to net you a messy demonstration of her enthusiasm.  ");
            }
        ...
        menu();
        //if(flags[kFLAGS.LOPPE_TIMES_SEXED] > 0)
        addButton(0, "Cow-girl", loppeRidesCocks).disableIf(!player.hasCock(), "Must have a cock").disableIf(player.lust < 33, "Not horny enough").disableIf(player.cockThatFits(loppeCapacity()) < 0, "Your need a cock that fits");
        addButton(1, "Get BJ", loppeWorshipsDicks).disableIf(!player.hasCock(), "Must have a cock").disableIf(player.lust < 33, "Not horny enough");
        addButton(2, "TakeVaginal", getFuckedInYerTwatYaCunt).disableIf(!player.hasVagina(), "Must have a cock").disableIf(player.lust < 33, "Not horny enough");
        addButton(3, "Boob-job", boobjobLoppe).disableIf(player.biggestTitSize() < 4, "You need bigger breasts");
        addButton(4, "SqueezeJob", loppeSqueezedickWhateverThatIs).disableIf(flags[kFLAGS.LOPPE_TIMES_SEXED] <= 0, "Must have experience her first");
        if (player.isTaur())
        {
            addButton(5, "TakeAnal", getAssFuckedByLoppeAsACentaur).disableIf(player.lust < 33, "Not horny enough");
        }
        else
        {
            addButton(6, "TakeAnal", getButtFuckedNonHoarseByLoppe).disableIf(player.lust < 33, "Not horny enough");
        }
    ...
    private function loppeRidesCocks() : Void
    {
        ...
        if (!player.isTaur())
        {
            outputText("on her bed, making yourself comfortable, and start suggestively stroking yourself");
        }
        //(centaur)
        else
        {
            outputText("cumbersomely on the floor, rolling over and spreading your hindlegs to expose yourself");
        }
        ...
        {
            outputText("  \"<i>In fact... I could just eat both of those up.</i>\"");
        }
        //(3+ fit cocks)
        else if (y >= 0 && player.cockTotal() == 3)
        {
            outputText("  \"<i>But I'm afraid I'll only be able to handle two of them.</i>\"");
        }
        ...
        if (y < 0)
        {
            outputText("slide your shaft against her");
        }
        else
        {
            outputText("pinion her shaft between your");
        }
        ...
        if (player.hasBalls())
        {
            outputText("  Your [balls] gently brush and rub against her own swollen cum-factories, and you can't wait to empty your overfilled sac into her waiting womb.");
        }
        ...
        if (y >= 0)
        {
            outputText("; its neighbor is aimed for her tight rosebud");
        }
        ...
        if (flags[kFLAGS.LOPPE_FURRY] == 0)
        {
            outputText("silky-smooth");
        }
        else
        {
            outputText("softly furred");
        }
    ...
    private function loppeRidesYouNoFaceJizz() : Void
    {
        ...
        if (player.isTaur())
        {
            outputText("foreleg tucks under her cock, angling it away from your body.");
        }
        else
        {
            outputText("hand lunges forward and places itself underneath, fingers curling around as you push it further up, aiming it over your shoulder.  You can feel it throbbing like mad against your fingers, can feel the impressive bulge of semen as it distends her urethra and surges up towards her flared tip.");
        }
        ...
        if (y >= 0)
        {
            outputText(" while her ass suckles on your second cock with just as much eagerness");
        }
        outputText(".");
    ...
    private function loppeRidesYouSpunksInYourEye() : Void
    {
        ...
        if (y >= 0)
        {
            outputText(", while her anal ring constricts your " + cockDescript(y) + " tightly, intent on holding you in place");
        }
        outputText(".");
    ...
    private function loppeRidesPCCockFinal() : Void
    {
        ...
        if (y >= 0 || player.hasVagina())
        {
            ...
            if (y >= 0)
            {
                outputText("your second " + cockDescript(y) + " just as easily pumps her perverted ass full of baby juice");
            }
            if (y >= 0 && player.hasVagina())
            {
                outputText(" and ");
        ...
        if (y < 0)
        {
            outputText("it almost smacks");
        }
        else
        {
            outputText("they almost smack");
        }
        ...
        if (player.minLust() <= 30 && player.lib <= 50)
        {
            outputText("; if I have to stop to tend to you after every coupling, I'll never get these empty");
        }
        ...
        if (player.cumQ() >= 500)
        {
            outputText(" despite your usually messy orgasms");
        }
        outputText(".");
        ...
        {
            outputText("; it gives you something of a thrill, though nowhere near as much as taking them off for her in the first place did");
        }
    ...
    private function loppeWorshipsDicks() : Void
    {
        ...
        else
        {
            ...
            if (player.biggestCockArea() > loppeCapacity())
            {
                outputText("even if it does look too big to fit.");
            }
            else
            {
                outputText("or maybe I should get a taste first?");
            }
        ...
        if (player.lib >= 40)
        {
            outputText(" - nor would you want to; she looks just right covered in your fluids");
        }
        outputText(".");
        ...
        //Low Libido:
        if (player.lib < 33 && player.minLust() < 30)
        {
            outputText("\n\nYou groan from the depths of your throat; Loppe and her libido...");
        }
        //{Moderate Libido:
        else if (player.lib < 66 && player.minLust() < 50)
        {
    ...
    private function getFuckedInYerTwatYaCunt() : Void
    {
        ...
        if (!player.isTaur())
        {
            outputText("spreading your labia in an invitation that needs no words.");
        }
        else
        {
            outputText("exposing your rump and spreading your hind legs in your best effort to make her feel welcome.");
        }
        ...
        {
            outputText("princess.  A sexy, smoking hot princess");
        }
        else
        {
            outputText("prince.  A dashing, dandy prince");
        }
        ...
        if (player.vaginas[0].virgin)
        {
            ...
            if (player.cor < 50)
            {
                outputText("  You look coy, for her benefit, and cast your eyes down shyly.");
            }
            else
            {
                outputText("  You do your best to warn her away from the topic with your eyes, eager to get on with the fuck.");
            }
            //(not for horsez)
            if (!player.isTaur())
            {
            ...
            if (player.cor < 50)
            {
                outputText("and tap her playfully on the nose");
            }
            else
            {
                outputText("in your irritation and squeeze her face imploringly");
            }
        ...
        if (player.vaginalCapacity() >= 35)
        {
            outputText(" bottoms out, completely hilted inside you");
        }
        //[else:
        else
        {
            outputText(" bumps against your cervix");
        }
        outputText(". You moan in delight as she sinks into you");
        if (player.vaginas[0].virgin)
        {
            outputText(", repressing a shiver of pain as she tears through your hymen, taking your virginity");
        }
        else if (player.vaginalCapacity() < 35)
        {
            outputText(", forcing your entrance wide to accommodate herself");
        }
        outputText(".");
        ...
        if (!player.isTaur())
        {
            outputText("  You wrap your [legs] around her hips as best you can, thrusting in your efforts to meet her strong rhythm, feeling your cunt rippling around her wonderfully long, thick horsecock.");
        }
        ...
        if (player.hasCock())
        {
            outputText(", precum begins to drool from your " + multiCockDescriptLight());
        }
        outputText(".");
        ...
        if (!player.isTaur())
        {
            ...
            if (player.gender >= 3)
            {
                outputText(" and smearing both your bellies with herm-pre");
            }
            outputText(".");
        }
        //Centaur:
        ...
        if (player.hasCock())
        {
            ...
            if (player.cumQ() < 500)
            {
                outputText("between you and painting you both");
            }
            else
            {
                outputText("gushing onto the bedsheets and smearing your belly");
            }
        ...
        {
            outputText(", giving you a quick peck on the cheek and licking a small gob of cum that seems to have stuck to your chin");
        }
        outputText(".");
        ...
        if (player.cor < 50)
        {
            outputText("a pleading look... and if there's maybe a trace of horror dancing in the depths of your eyes, well, that's understandable, isn't it?");
        }
        else
        {
            outputText("an incendiary glare, letting her know just what you think of her posturing.");
        }
        ...
        if (silly())
        {
            outputText("  \"<i>Da! Sandvich is kredit to team!</i>\"");
        }
        ...
        //[Libido >=50:
        if (player.lib >= 50 || player.minLust() >= 30)
        {
            outputText("  Not that you didn't just titillate yourself thinking of it...");
        }
    ...
    private function getButtFuckedNonHoarseByLoppe() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText(" pussy oozing femlube");
        }
        ...
        if (player.hasCock())
        {
            outputText("One of her hands reaches around to grasp your pre-slickned [cock biggest], stroking you in time with her thrusts.  ");
        }
        if (player.gender == 2)
        {
            outputText("One of her hands reaches around to tease your dripping [vagina] and erect [clit], dipping a finger in every time she bottoms out.  ");
        }
        ...
        if (flags[kFLAGS.SATYR_KIDS] > 0)
        {
            outputText("You laugh softly; hopefully she'll never have to meet the creature that dispelled that same doubt in you.  ");
        }
    ...
    private function getAssFuckedByLoppeAsACentaur() : Void
    {
        ...
        if (player.ass.analWetness > 0)
        {
            outputText("  She doesn't even have to bother with lube; your ass is wet enough to accommodate her on its own.");
        }
        ...
        if (player.hasCock())
        {
            outputText(", feeling your erect [cock] press against your underbelly, mashed between your centaur half and the soft carpet of Loppe's room.  You hope she doesn't mind a few stains...  ");
        }
        //(else vag)
        else if (player.hasVagina())
        {
            outputText("; your cunt is sopping, making a small pool of juices one the floor.  ");
        }
        else
        {
            outputText(".  ");
        ...
        if (player.gender <= 1)
        {
            outputText("  Even if you are the wrong sex entirely for that.");
        }
        ...
        if (player.hasCock())
        {
            outputText("  The bumps against your prostate send waves of pleasure running through [eachCock] and spreading along your body, until they reach your throat and come out as a ragged groan.  ");
        }
        ...
        if (player.hasCock())
        {
            outputText("  [EachCock] spasms and unloads beneath you, smearing you and the carpet below and pooling thickly with fluid forced out by the prostate stimulation.");
        }
        if (player.hasVagina())
        {
            outputText("  Your vagina contracts, trying to grip at a phantom member as you spill your fluids on the carpet beneath you.");
        }
        ...
        {
            outputText(" have goosebumps all over your body");
        }
        else
        {
            outputText("r scales are practically standing on end");
        }
        outputText(".");
        ...
        {
            outputText("Though I hope you won't mind riding me, sometime,</i>\" she laughs.  \"<i>");
        }
    ...
    private function superLoppeOrgasmDenialGo() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("  Maybe next time I'll be the one to give you a bit of a holdout!");
        }
        outputText("</i>\"");
    ...
    private function boobjobLoppe() : Void
    {
        ...
        if (player.hasLongTongue())
        {
            outputText("retract your tongue into your mouth and ");
        }
        ...
        function weakF() : Void
        {
            ...
            {
                outputText("looping your inhuman tongue around it and stroking for extra stimulus even while ");
            }
            ...
            if (silly())
            {
                outputText(".  You can't help but think of Family Matters");
            }
        ...
        var strongF : Void->Void = function() : Void
        {
            ...
            if (player.cor >= 60)
            {
                outputText("You don't really feel like it, but you figure you had better humor the greedy herm if you want to keep fucking her whenever you like.  ");
            }
        ...
        var sharedEnd : Void->Void = function() : Void
        {
            ...
            {
                outputText("- while your stomach is still heavily swollen and gurgles in protest, it's not bad enough to impair you - ");
            }
    ...
    //req LoppeChat > 1
    private function fondleAndTease(output : Bool = true) : Void
    {
        if (output)
        {
            clearOutput();
    ...
    private function teaseLoppeNSuck() : Void
    {
        ...
        if (player.tongue.type > Tongue.HUMAN)
        {
            outputText(", inhumanly long tongue looping out to coil wetly around the length");
        }
    ...
    public function canEncounterUsagi() : Bool
    {
        return !usagiHasReturned;
    }
    ...
    private function usagiCamp() : Void
    {
        ...
        {
            outputText("You wonder whether or not you should tell Usagi that her lover and daughter are still alive and safe in Tel’Adre...[pg]");
        }
        ...
        {
            addButton(0, "Comfort", tellUsagiAboutLoppeNo);
        }
    }
    ...
    public function loppeKnockupAttempt(isplayer : Bool = false) : Void
    {
        if (isplayer)
        {
            if (player.isPregnant() || !canGetPreg)
            {
                return;
            }
            if (flags[kFLAGS.LOPPE_FERTILE] > 0)
            {
                ...
                {
                    player.knockUp(PregnancyStore.PREGNANCY_LOPPE, PregnancyStore.INCUBATION_LOPPE, 100);
                }
            }
        }
        else
        {
            if (pregnancy.isPregnant || !canGetPreg)
            {
                return;
            }
            if (flags[kFLAGS.LOPPE_FERTILE] > 0)
    ...
    private function get_canGetPreg() : Bool
    {
        ...
        if (player.isPregnant() && player.pregnancyType == PregnancyStore.PREGNANCY_LOPPE)
        {
            kids += 2;
        }
        if (player.isPregnant() && player.pregnancy2Type == PregnancyStore.PREGNANCY_LOPPE)
        {
            kids += 2;
        }
        return kids < flags[kFLAGS.LOPPE_KIDS_LIMIT];
    }
    private function toggleContraception() : Void
    //TODO
    {
        ...
        if (flags[kFLAGS.LOPPE_FERTILE] == 0 && flags[kFLAGS.LOPPE_KIDS] >= flags[kFLAGS.LOPPE_KIDS_LIMIT])
        {
            contraceptionNope();
        }
        if (flags[kFLAGS.LOPPE_FERTILE] == 1)
        {
            contraceptionOn();
        }
        else
    ...
    {
        if (flags[kFLAGS.LOPPE_KIDS] >= 12)
        
        //usagi is in town && loppeKids >= 12){
    ...
    //Appearance
    {
        ...
        addButton(0, "Appearance", appearanceOfLoppe);
        addButton(1, "Talk", talkWithLoppe);
        addButton(2, "Sex", loppePreggoSexChoice).disable("Scenes not yet written but placeholders to give ideas. Please contribute if you can");
        addButton(3, "Contraception", toggleContraception).hint("Currently: " + ((flags[kFLAGS.LOPPE_FERTILE] == 1) ? "Off" : "On"), "Contraceptives");
        addButton(4, "Leave", telAdre.telAdreMenu);  //Leave (Return to Tel'Adre menu)  
        addButton(5, "Snuggle", loppePreggoSnuggle);
        addButton(6, "Massage", loppePreggoMassage).disable("Scenes not yet written but placeholders to give ideas. Please contribute if you can");
    }
    ...
    private function loppeGivesBirth() : Void
    //TODO
    {
    ...
    {
        var _sw16_ = (pregnancy.event);        

        switch (_sw16_)
        {
            case 1, 2:outputText("You spot Loppe the laquine wandering around, towel slung over her shoulder. She looks extremely happy, and she gives you a thrilled look when she sees you, smiling blissfully and patting her belly in a meaningful gesture.");
            case 3, 4, 5, 6:outputText("You spot Loppe sitting on a bench, apparently for the comfort of its padded seats.  Her bulging belly makes it obvious she's pregnant - and the wide grin that seems fixed to her face makes it obvious she's ecstatic about it. When she sees you, she smiles and waves to you and you wave back.");
            case 7, 8, 9:outputText("Loppe's pregnancy has caused her to seek out a bench, apparently for the comfort of its padded seats..  Looking at the sheer size of her, she's doubtlessly going to pop any day now.");
        }
    }
    ...
    private function loppePreggoSnuggle() : Void
    {
        ...
        addButton(1, "Lewd it up", snuggleLewds).disable("Scenes written but not yet added");
    }
    ...
    public function pcGivesBirthToLoppeKits(womb : Int = 0) : Void
    {
        ...
        {
            outputText("It hurts a little as your cervix starts to stretch wide");
        }
        else
        {
            outputText("It actually feels kind of nice as your cervix is stretched wide");
        }
