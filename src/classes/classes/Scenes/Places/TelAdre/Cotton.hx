...
class Cotton extends TelAdreAbstractContent implements TimeAwareInterface
{
    ...
    public function timeChange() : Bool
    {
        ...
        if (flags[kFLAGS.COTTON_KID_COUNT] > 0 && model.time.hours == 23)
        {
            flags[kFLAGS.COTTON_OLDEST_KID_AGE]++;
        }
        return false;
    }
    ...
    private function pregCottonChance(bonusMult : Float = 1) : Void
    ...
    //No kids yet - lucky!
    {
        ...
        else if (flags[kFLAGS.COTTON_HERBS_OFF] == 0)
        {
            if (player.hasUniquePregnancy())
            {
                player.impregnationRacialCheck();
            ...
            {
                player.knockUp(PregnancyStore.PREGNANCY_COTTON, PregnancyStore.INCUBATION_COTTON, 1000);
            }
        }
        else if (player.hasUniquePregnancy())
        {
            player.impregnationRacialCheck();
        }
        else
        {
            player.knockUp(PregnancyStore.PREGNANCY_COTTON, PregnancyStore.INCUBATION_COTTON, 100);
        }
    }
    
    public function cottonsIntro() : Bool
    {
        ...
        //Gym intro scene (haven't met):{
            ...
            else if (flags[kFLAGS.COTTON_MET_FUCKED] == 1)
            {
    ...
    public function cottonGreeting() : Void
    {
        ...
        {
            outputText("The centauress sees you starting for the horse-girl and says, \"<i>Go ahead and talk, but if you want to work out with her I'll have to charge you.</i>\"\n\n");
        }
        //Greeting (first time):
        if (flags[kFLAGS.COTTON_MET_FUCKED] == 0)
        {
            ...
            {
                outputText("Despite her impressive stature, you still look down on her.");
            }
            //(If PC height is in between:
            else if (player.tallness >= 76)
            {
                outputText("She's roughly your height, which is rather impressive in itself.");
            }
            //(If PC height is under 6'4</i>\":
            else
            {
                outputText("She is truly tall, and you have to look up to her.");
            }
        ...
        else
        {
            ...
            else if (player.isTaur() || player.isNaga())
            {
                centaurNagaBodyBookStuff();
            }
            else
    ...
    private function cottonMenu() : Void
    {
        ...
        if (flags[kFLAGS.COTTON_KID_COUNT] > 0)
        {
            addButton(1, "Visit Kids", visitCottonKids);
        }
        if (pregnancy.isPregnant || flags[kFLAGS.COTTON_KID_COUNT] >= 1)
        {
            addButton(2, "Herbs", cottonContraceptionToggle);
        }
    ...
    private function acceptYoga() : Void
    {
        ...
        else if (flags[kFLAGS.TIMES_HAD_YOGA] == 0)
        {
            ...
            if (player.gender == 3)
            {
                outputText("  She spies your crotch and smiles, \"<i>Oh, best of both worlds, are we? Well you're in good company then.</i>\"");
            }
            //(If PC is male:
            else if (player.hasCock())
            {
                outputText("  She cradles your [cock] and smiles at you, \"<i>Well, we might find a use for that later.</i>\"");
            }
            //(If PC is female:
            else if (player.hasVagina())
            {
                outputText("  She pats your groin and says, \"<i>Y'know, I know some good exercises that really work the vaginal muscles... perhaps we'll talk about that later.</i>\"");
            }
            outputText("\n\n");
        ...
        {
            //Done yoga > 0 && met type = 1if (flags[kFLAGS.COTTON_MET_FUCKED] == 1 && flags[kFLAGS.TIMES_HAD_YOGA] > 0)
            {
    ...
    private function sexMenu(disableTantric : Bool = false) : Void
    {
        menu();
        ...
        if (!disableTantric)
        {
            addButton(2, "Tantric Sex", cottonTantricSex).disableIf(player.spe < 50 || player.isTaur(), 
    ...
    private function cottonChat() : Void
    {
        ...
        var choice : Float = chats[rand(chats.length)];
        ...
        //(If you've rejected Urta's love or left her){
            ...
            if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] < 0 || flags[kFLAGS.URTA_PC_LOVE_COUNTER] < 0)
            {
        ...
        //(If Edryn has been knocked up, and PC rejected it){
            ...
            if (flags[kFLAGS.EDRYN_NEVER_SEE_AGAIN] > 0)
            {
        ...
        //(if Scylla hasn't formed support group){
            ...
            if (flags[kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA] < 5 && flags[kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA] > 0)
            {
                outputText("While you're doing your stretches, you find yourself chatting about the folks of Tel'Adre. \"<i>Scylla?</i>\" Cotton says, \"<i>She's the oddly dressed woman at the Wet Bitch, right? Can't say I know much about her. She's so secretive.</i>\"\n\n");
            }
            //(if Scylla has formed support group)
            else if (flags[kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA] >= 5)
            {
                outputText("While you're doing your stretches, you find yourself chatting about the folks of Tel'Adre. \"<i>Scylla?</i>\" Cotton says, \"<i>I hear she formed an addiction support group. Good on her, there's a lot of people in and around town who need help. I'm glad she's stepping up.</i>\"\n\n");
            }
        }
        //(Vala chat)
        else if (choice == 4)
        ...
        //(Only if Vala has been freed){
            ...
            if (flags[kFLAGS.FREED_VALA] != 0)
            {
                outputText("While you're doing your stretches, you find yourself chatting about the folks of Tel'Adre. \"<i>Vala?</i>\" Cotton says, \"<i>That's the new waitress at the Wet Bitch, right? She's cute. A shame what she's gone through. Sometimes I wish we could wipe out every last imp.</i>\"\n\n");
            }
        }
        //(Jojo chat)
        else if (choice == 5)
    ...
    private function cottonFucksYou() : Void
    {
        ...
        if (player.gender == 1)
        {
            ...
            if (player.cumQ() >= 1000)
            {
                outputText("  You nearly clog the drain with all the spooge leaking from your flaccid shaft.");
            }
            outputText("\n\n");
    ...
    private function fuckCottonInShowerRepeat() : Void
    {
        ...
        if (player.cockArea(x) < 70)
        {
            sceneHunter.print("You may go bigger~");
        }
        ...
        if (player.isTaur())
        {
            ...
            {
                outputText("pulling you gently towards her bulging belly, which you can't help but put your hands against as you kiss her. The bulge is firm and solid, almost a drum of solid muscle, and you caress it as you and the mare-morph make out, eliciting soft murmurs of appreciation for your efforts.\n\n");
            }
            else
            {
                outputText("pulling you closer.\n\n");
            }
            ...
            if (player.cockArea(x) >= 60)
            {
                ...
                {
                    outputText("You feel your " + cockDescript(x) + " stirring beneath your large body, while Cotton's remains curiously limp.  While her cock dangles, yours strains for attention.");
                }
                //(If PC has multiple cocks)
                else if (player.cockTotal() > 1)
                {
                    outputText("You feel your [cocks] stirring beneath your large body, while Cotton's remains curiously limp.  While her cock dangles, your group of cocks strains for attention.");
                }
            }
            else if (player.cockTotal() == 1)
            {
                outputText("You feel your " + cockDescript(x) + " stirring beneath your large body, while you watch Cotton's do the same. Her cock rubs against your stomach while yours strains for attention.");
            }
            //(If PC has multiple cocks)
            else if (player.cockTotal() > 1)
            {
                outputText("You feel your [cocks] stirring beneath your large body, while you watch Cotton's do the same. Her cock rubs against your stomach while your group of cocks strains for attention.");
            }
            //(If PC has a pussy, add the following)
            if (player.hasVagina())
            {
                outputText("  Meanwhile, your " + vaginaDescript(0) + " behind you moistens both from the steam and from arousal, and your " + clitDescript() + " aches, craving attention.");
            }
            outputText("\n\n");
            ...
            if (player.biggestTitSize() >= 2)
            {
                ...
                {
                    outputText("  Cotton soon switches to the other breast, teasing your " + nippleDescript(0) + " with her talented tongue.");
                }
                outputText("\n\n");
            }
            ...
            else if (player.cocks[x].cockLength < 4)
            {
                ...
                if (pregnancy.event > 1)
                {
                    outputText("Don't worry pet, it's plenty adequate for me.  And you've got to admit,</i>\" she smirks, rubbing her round belly, \"<i>It gets the job done, doesn't it?");
                }
                else
                {
                    outputText("Don't worry pet, it's plenty adequate for me.");
                }
            ...
            {
                outputText("  You piston back and forth, and though Cotton's cunt is sizable, it's able to grip you tightly. Your equine lover moans and grasps at your chest, eventually seizing your leg for support, though she leans backwards.");
            }
            outputText("\n\n");
            ...
            {
                outputText("Oblivious to you, her own limp cock leaks cum down onto the bench as an orgasm rocks her body.");
            }
            else if (pregnancy.event > 1)
            {
                outputText("Oblivious to you, her own cock twitches and spasms, letting loose a spray of cum across her swollen, pregnant belly, breasts and the bench as an orgasm rocks her body.");
            }
            else
            {
                outputText("Oblivious to you, her own cock twitches and spasms, letting loose a spray of cum across her belly, breasts and the bench as an orgasm rocks her body.");
            }
            //(If player is too big, add)
            if (player.cockArea(x) > 70)
            {
                outputText("  Even with all the pounding you've been doing, you can't fit your entire girth into her, but she doesn't care. Her eyes seem to roll back and forth like it's hard to focus, and her tongue drips water, sweat and saliva onto her body. She's panting for breath and you are able to make out faint, \"<i>Oh gods, oh gods</i>\" over the running water. Were you able to see her toned stomach, you're sure you could see the outline of your " + cockDescript(x) + " in it. Her back arches under the shower, her red hair messily draped around her face and bench, and she bites her lip as another orgasm rocks her body.");
            }
            outputText("\n\n");
            ...
            if (player.hasVagina())
            {
                outputText("  Your " + cockDescript(x) + " erupts inside Cotton's warm cunt which clenches you tightly, while your own femme sex twinges and clenches on air, drooling juices down your legs.");
            }
            //(else)
            else
            {
                outputText("   Your " + cockDescript(x) + " erupts inside Cotton's warm cunt which clenches you tightly.");
            }
            //(regardless or above, add)
            outputText("  You give a couple more token thrusts and pull out. As you do, ");
            if (player.cumQ() < 200)
            {
                outputText("cum begins to seep from her folds");
            }
            else if (player.cumQ() < 700)
            {
                outputText("a gush of cum escapes from Cotton's folds");
            }
            else if (player.cumQ() < 1200)
            {
                outputText("a small torrent of cum escapes from Cotton's folds");
            }
            else if (player.cumQ() < 2000)
            {
                outputText("cum surges from her abused hole");
            }
            else
            {
                outputText("a massive flood of cum escapes from her cunt");
            }
        ...
        else if (player.isNaga())
        {
            ...
            //(If PC has one cock){
                ...
                if (player.cockTotal() == 1)
                {
                    outputText("  You feel your " + cockDescript(x) + " stirring beneath you, slowly coming to attention, though Cotton's remains curiously limp.  Your " + player.cockHead(x) + " rubs against her belly, sending ripples of pleasure up your spine.");
                }
                //(If PC has multiple cocks)
                else if (player.cockTotal() > 1)
                {
                    outputText("  You feel your [cocks] stirring beneath you, slowly coming to attention, though Cotton's remains curiously limp.  The group of cocks rubs together as you make out, sending ripples of pleasure up your spine.");
                }
            }
            //(If PC has one cock)
            else
            {
                ...
                if (player.cockTotal() == 1)
                {
                    outputText("You feel your " + cockDescript(x) + " stirring beneath you, slowly coming to attention alongside Cotton's equine member.  The two rub together as you make out, sending ripples of pleasure up your spine.");
                }
                //(If PC has multiple cocks)
                else if (player.cockTotal() > 1)
                {
                    outputText("You feel your [cocks] stirring beneath you, slowly coming to attention alongside Cotton's equine member. The group of cocks rubs together as you make out, sending ripples of pleasure up your spine.");
                }
            }
            //(If PC has a pussy, add the following)
            if (player.hasVagina())
            {
                outputText("Your " + vaginaDescript(0) + " meanwhile moistens both from the water and from arousal, and your " + clitDescript() + " aches, craving attention.");
            }
            outputText("\n\n");
            ...
            if (player.cockTotal() == 2)
            {
                outputText("  With a slight grin, you position your extra dick at Cotton's other hole. She quirks her eyebrow and a look of panic momentarily crosses her face, but you push forward anyway. Cotton's eyes roll up briefly and her lip quivers.");
            }
            //(If the player has more cocks, add)
            else if (player.cockTotal() > 2)
            {
                outputText("  The rest of your cocks strain, aching for holes to fill. With none available, they throb as they rub against Cotton's smooth skin.");
            }
            //(If player is too big, add)
            if (player.cockArea(x) > 70)
            {
            ...
            {
                outputText("  You piston back and forth, and though Cotton's cunt is sizeable, it's able to grip you tightly. Your equine lover moans and grasps at your chest, eventually seizing your shoulder for support, though she leans backwards.");
            }
            outputText("\n\n");
            ...
            if (player.cockArea(x) >= 100)
            {
                outputText("Her own cock, still limp, visibly twitches and begins leaking cum onto her belly as an orgasm rocks her body.");
            }
            else if (pregnancy.event > 1)
            {
                outputText("Her own cock twitches and spasms, letting loose a spray of cum across her swollen, pregnant belly, breasts and face as an orgasm rocks her body.");
            }
            else
            {
                outputText("Her own cock twitches and spasms, letting loose a spray of cum across her belly, breasts and face as an orgasm rocks her body.");
            }
            //(If player is too big, add)
            if (player.cockArea(x) > 70)
            {
                outputText("  Even in this pseudo-missionary position you can't stuff your entire enormous girth into her, but she doesn't care. Her eyes seem to roll back and forth like it's hard to focus, and her tongue drips water, sweat and saliva onto her body. She's panting for breath and you are able to make out faint, \"<i>Oh gods, oh gods</i>\" over the running water. You can even make out the shape of your cock in Cotton's toned stomach, pumping back and forth, which brings a smile to your face. Her back arches under the shower, her hair messily spread out beneath her and she bites her lip as another orgasm rocks her body.");
            }
            outputText("\n\n");
        ...
        else
        {
            ...
            if (player.hasVagina())
            {
                outputText("  Your " + vaginaDescript(0) + " moistens both from the water and from arousal, and your " + clitDescript() + " aches, craving attention.");
            }
            outputText("\n\n");
            ...
            if (player.cockTotal() == 2)
            {
                outputText("  With a slight grin, you position your other dick at Cotton's other hole. She quirks her eyebrow and a look of panic momentarily crosses her face, but you push forward anyway. Cotton's eyes roll up briefly and her lip quivers.");
            }
            //(If the player has more cocks, add)
            if (player.cockTotal() > 2)
            {
            ...
            {
                outputText("Her own cock twitches and spasms, loosing a spray of cum across her swollen, pregnant belly, breasts and face as an orgasm rocks her body.");
            }
            else
            {
                outputText("Her own cock twitches and spasms, letting loose a spray of cum across her belly, breasts and face as an orgasm rocks her body.");
            ...
            if (player.cockArea(x) > 70)
            {
                outputText("  Even in the missionary position you can't stuff your entire enormous girth into her, but she doesn't care. Her eyes seem to roll back and forth like it's hard to focus, and her tongue drips water, sweat and saliva onto her body. She's panting for breath and you are able to make out faint, \"<i>Oh gods, oh gods</i>\" over the running water. You can even make out the shape of your cock in Cotton's toned stomach, pumping back and forth, which brings a smile to your face. Her back arches under the shower, her hair messily spread out beneath her and she bites her lip as another orgasm rocks her body.");
            }
            outputText("\n\n");
    ...
    private function cottonFucksYouInShowerRepeat() : Void
    {
        ...
        if (player.isTaur())
        {
            ...
            if (player.gender == 1)
            {
                outputText("  Your [cock] tingles betwixt your legs, but remains limp in the presence of Cotton's impressive member.");
            }
            //(If PC has a penis and vagina)
            else if (player.gender == 3)
            {
                outputText("  Your [cock] tingles betwixt your legs, but remains limp in the presence of Cotton's impressive member, while your " + vaginaDescript(0) + " moistens almost immediately from the steam and arousal.");
            }
            //(If PC has a vagina and no penis)
            else if (player.gender == 2)
            {
                outputText("  Your " + vaginaDescript(0) + " moistens almost immediately as you make out, both from the steam and your increasing arousal.");
            }
            //(If PC is genderless)
            else
            {
                outputText("  A deep aching burns within you, a need your body is ill-equipped to process, but still your nipples harden and you find your " + assholeDescript() + " puckering in anticipation.");
            }
            outputText("\n\n");
            ...
            if (player.hasCock())
            {
                ...
                {
                    outputText("  \"<i>Oh my, what's this?</i>\" She puts a hand under your enormous, yet embarrassingly limp cock and lifts it slightly. \"<i>My little pet has such a big dick... Just how I like it. Perhaps next time I'll get to try it out... but not today, hm? This is all about you right now.</i>\"");
                }
                //(If PC has a penis under 4</i>\", add)
                else if (player.longestCockLength() < 4)
                {
                    outputText("  \"<i>Awww, what's this?</i>\" She puts a hand under your embarrassingly small and limp cock and lifts it slightly. \"<i>It's so cute and tiny. And it certainly knows its place. Only room for one cock right now, not that this is much of a cock.</i>\" She giggles and plants a kiss on the tip, \"<i>It is cute though. I love it.</i>\"");
                }
                //(If PC has a penis neither large or small, add)
                else
                {
                    outputText("Cotton puts a hand under your embarrassingly limp cock and smiles, \"<i>You know how to show a girl you like her... There's only room for one cock right now.</i>\"");
                }
            }
            //(If PC has a vagina)
            if (player.hasVagina())
            {
                outputText("  Spying your " + vaginaDescript(0) + ", Cotton smiles and flicks your " + clitDescript() + " teasingly before slipping two fingers inside your folds and bringing them to her mouth, licking them clean. \"<i>Mmm... I love the taste of your juices, pet...</i>\"");
            }
            //(if PC is genderless)
            if (player.gender == 0)
            {
                outputText("  She places a hand on your bare crotch and quirks an eyebrow. \"<i>Well this is new... Certainly not bad though.</i>\" She runs a hand along your bare mound, which somehow manages to send ripples of pleasure up your spine.");
            }
            outputText("\n\n");
        ...
        else
        {
            ...
            if (player.gender == 1)
            {
                outputText("  Your [cock] tingles beneath you, but remains limp in the presence of Cotton's impressive member.");
            }
            //(If PC has a penis and vagina)
            else if (player.gender == 3)
            {
                outputText("  Your [cock] tingles beneath you, but remains limp in the presence of Cotton's impressive member while your " + vaginaDescript(0) + " moistens almost immediately from the steam and arousal.");
            }
            //(If PC has a vagina and no penis)
            else if (player.gender == 2)
            {
            ...
            if (player.isNaga())
            {
                ...
                if (pregnancy.event > 1)
                {
                    outputText("Moving quickly, you wrap your arms around Cotton's neck and coil your body around her waist, just below her round, pregnant belly, and down one leg.  You can feel her swollen midriff rubbing against the scales on your serpentine half with each motion you make.");
                }
                else
                {
                    outputText("Moving quickly, you wrap your arms around Cotton's neck and coil your body around her waist and down one leg.");
                }
            }
            //(If Humanoid body)
            else
            {
                ...
                if (pregnancy.event > 1)
                {
                    outputText("Moving quickly, you wrap your arms and legs around Cotton so you don't fall over, carefully avoiding her pregnant belly, which is no mean feat. Her belly is pressed into yours, letting you feel the solid weight of it, gentle kicks from your unborn child within.");
                }
                else
                {
                    outputText("Moving quickly, you wrap your arms and legs around Cotton so you don't fall over.");
                }
            }
    ...
    public function cottonTantricSex() : Void
    {
        ...
        if (player.isNaga())
        {
            outputText("  You impale yourself fully, settling down and sitting on her lap, stretching your snake-like body out behind and to one side of her, with your arms propping you up as you lean back.");
        }
        //(for Humanoid Body, add)
        else
        {
            outputText("  You impale yourself fully, settling down and sitting on her lap, mimicking her pose with your legs stretched out behind her and your arms propping you up as you lean back.");
        }
        outputText("\n\n");
        ...
        if (player.hasCock())
        {
            outputText("  Meanwhile, your own [cock] hangs limply in front of you, tingling with arousal, but unable to harden. Cotton gives it a pat and says, \"<i>Worry not, pet, there's only room for one cock right now, but I'll want it some other time.</i>\"");
        }
        outputText("\n\n");
        ...
        if (player.isNaga())
        {
            outputText("  You do the same, curling your arms around your partner while your lower body coils around her midsection. Cotton pulls her legs in as well, and it almost looks like you're both sitting normally, if it weren't for you being on her lap with her dick up inside you.");
        }
        //(Humanoid body, add)
        else
        {
            outputText("  You do the same and curl your arms and legs around your partner. Cotton pulls her legs in as well, and it almost looks like you're both sitting cross-legged if it weren't for you being on her lap with her dick up inside you.");
        }
        ...
        if (player.biggestTitSize() >= 2)
        {
            ...
            {
                outputText("  She quickly switches to the other nipple, her tongue expertly teasing and tantalizing it before pulling away once more.");
            }
            outputText("\n\n");
        }
        ...
        if (player.hasCock())
        {
            outputText("  Your [cock] twitches and tingles, releasing its pent-up seed in a dribbling torrent down your belly and chest, right into your face and pooling around your head.");
        }
        //(If PC has a vagina)
        if (player.hasVagina())
        {
            outputText("  Your " + vaginaDescript(0) + " shivers and clamps down hard on the invading cock, pulsing and milking it as it unloads directly into your womb.");
        }
        //(If PC doesn't have a vagina)
        else
        {
            outputText("  Your " + assholeDescript() + " shivers and clamps down hard on the invading cock, pulsing and milking it as it unloads directly into your intestines.");
        }
...
Eldest Child Age: Interactions with your children depend on the age of your eldest kid.*/
    ...
    private function cottonPregnantAlertII() : Void
    {
        ...
        if (flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] > 0 || (telAdre.edryn.pregnancy.isPregnant && flags[kFLAGS.EDRYN_PREGNANT_AND_NOT_TOLD_PC_YET] != 0))
        {
            outputText("  Should have figured as much after you got Edryn pregnant.");
        }
        ...
        if (flags[kFLAGS.EDRYN_NEVER_SEE_AGAIN] == 0)
        {
            outputText("you'll stick with me through this, right?");
        }
        else
        {
            outputText("you won't abandon me like you did Edryn, will you?");
        }
        outputText("</i>\"");
        //[Leave Her] [Stay]
        menu();
    ...
    private function beABadCottonDad() : Void
    {
        ...
        if (flags[kFLAGS.EDRYN_NEVER_SEE_AGAIN] == 0)
        {
            outputText("and slaps you as hard as she can");
        }
        else
        {
            outputText("and punches you as hard as she can");
        }
        outputText(". \"<i>Fuck you 'pet'.");
        if (flags[kFLAGS.EDRYN_NEVER_SEE_AGAIN] > 0)
        {
            outputText("  That was for Edryn too.");
        }
    ...
    {
        var kid : Int = as3hx.Compat.parseInt(rand(3) + 1);  //1,2,3  
        function kidSel(male : String, fem : String, herm : String = "") : String
        {
            return (kid == 1) ? male : kid == 2 || !(herm != null) ? fem : herm;
        };
        ...
        if (flags[kFLAGS.COTTON_KID_COUNT] == 0)
        {
            ...
            if (silly())
            {
                outputText("As long as the cube shark isn't there.  ");
            }
            outputText("Let's go.</i>\"");
            //(Depending on PC height:
            if (player.tallness < 60)
            {
                outputText("She wraps an arm around your neck");
            }
            else if (player.tallness < 70)
            {
                outputText("She places a hand on your shoulder");
            }
            else if (player.tallness < 86)
            {
                outputText("She wraps an arm around your midsection");
            }
            else
            {
                outputText("She wraps an arm around your waist");
            }
            ...
            if (kid == 3)
            {
                outputText("  I guess it runs in the family.");
            }
            outputText("</i>\"");
            ...
            {
                flags[kFLAGS.COTTON_OLDEST_KID_AGE] = 1;
            }
    ...
    private function cottonContraceptionToggle() : Void
    {
        ...
        //(Cottonpreg:
        if (player.gender != 3 || pregnancy.isPregnant)
        {
            outputText("virile");
        }
        else
        ...
        if (flags[kFLAGS.COTTON_KID_COUNT] < 3)
        {
            outputText("Well that's true,</i>\" she admits.  \"<i>But I leave it up to you, pet.");
        }
        else
        {
            outputText("Well, considering the brood we have, they've already proven pretty useless, haven't they?   So, while I'll keep taking them if you want me to, I don't see much point in it.");
        }
        outputText("</i>\"");
    ...
    private function tellCottonToKeepFiringBlanksAsshole() : Void
    {
        ...
        {
            outputText("overrun by a herd of little horsefolk");
        }
        outputText(", right?</i>\"");
    ...
    public function birthingCottonsKids() : Void
    {
        ...
        var kidSel : String->String->String->String = function(male : String, fem : String, herm : String = "") : String
        {
            return (kid == 1) ? male : kid == 2 || !(herm != null) ? fem : herm;
        }
    ...
    {
        return (flags[kFLAGS.COTTON_OLDEST_KID_GENDER] == 1) ? male : flags[kFLAGS.COTTON_OLDEST_KID_GENDER] == 2 || !(herm != null) ? fem : herm;
    }
    ...
    private function visitCottonKids() : Void
    {
        ...
        //If player lactates, display \"<i>Stay Quiet</i>\" or \"<i>Feed</i>\" options; otherwise, skip straight to Stay Quiet option{
            ...
            if (flags[kFLAGS.COTTON_KID_COUNT] > 1)
            {
                outputText("  Fortunately, your other kids don't seem to be hungry, so it's just the one who needs feeding.");
            }
