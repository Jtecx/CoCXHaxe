 ...
 */
class PabloScene extends TelAdreAbstractContent
{
    ...
    public function pabloIntro(textOutput : Bool = true) : Bool
    {
    ...
    public function pabloAffection(mod : Int = 0) : Int
    {
        ...
        if (flags[kFLAGS.PABLO_AFFECTION] < 0)
        {
            flags[kFLAGS.PABLO_AFFECTION] = 0;
        }
        if (flags[kFLAGS.PABLO_AFFECTION] > 100)
        {
            flags[kFLAGS.PABLO_AFFECTION] = 100;
        }
    ...
    private function workoutWithPablo() : Void
    {
        ...
        if (flags[kFLAGS.PABLO_AFFECTION] < 60)
        {
            pabloAffection(5);
        }
        pabloAffection(5);
    ...
    public function pabloSexMenu() : Void
    {
        ...
        //Ass
        addButton(2, "T.Charge (A)", pabloButtSecks, true).hint("You'll take charge and take his dick anally.", "Take Charge (Ass)");
        addButton(7, "P.Leads  (A)", pabloButtSecks, false).hint("You'll let Pablo lead and he'll fuck your ass.", "Pablo Leads (Ass)");
    }
    
    private function pabloDickingTime(inCharge : Bool = false) : Void
    {
        clearOutput();
        if (inCharge)
        {
    ...
    private function pabloPussPuss(inCharge : Bool = false) : Void
    {
        clearOutput();
        if (inCharge)
        {
    ...
    private function pabloButtSecks(inCharge : Bool) : Void
    {
        clearOutput();
        if (inCharge)
        {
