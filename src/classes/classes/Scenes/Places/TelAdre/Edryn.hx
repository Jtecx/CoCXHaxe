...
class Edryn extends TelAdreAbstractContent implements TimeAwareInterface
{
    //VARS
    ...
    public function timeChange() : Bool
    {
        ...
        {
            flags[kFLAGS.EDRYN_PREGNANCY_INCUBATION]++;
        }
        return false;
    }
    ...
    public function edrynBarTalk() : Void
    {
        ...
        {
            player.createStatusEffect(StatusEffects.Edryn, 0, 0, 0, 0);
        }
        clearOutput();
        outputText(images.showImage("edryn-bar-chat"));
        
        //Used for finding what cock to use!
        var x : Float = player.cockThatFits(300);
        ...
        if (telAdre.katherineEmployment.canTalkToEdryn())
        ...
        if (flags[kFLAGS.EDRYN_NEEDS_TO_TALK_ABOUT_KID] == 1)
        {
            ...
            else
            {
                ...
                if (flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] < 3)
                {
                    outputText("I miss the way you fill me");
                }
                else
                {
                    outputText("I want you to make me pregnant again");
                }
            }
            outputText(".</i>\"\n\n");
        ...
        if (player.isTaur())
        {
            outputText("offering you a spot at the table across from her.  You realize your companion isn't on any kind of seat at all, and is instead 'sitting' on her lower half.  You do the same, settling down across from her, thankful this bar was made to accommodate centaurs.");
        }
        else
        {
            outputText("gesturing to a nearby stool.  You grab the seat and realize that your centaur companion isn't sitting still at all, but is instead 'sitting' on her lower half, which currently lies on the floor.");
        }
        ...
        if (player.isTaur() && player.cockTotal() > 0)
        {
            ...
            {
                outputText("pours out of its sheath");
            }
            else
            {
                outputText("fills near instantaneously");
            }
        ...
        else if (player.cockTotal() > 0)
        {
            if (player.horseCocks() > 0)
            {
                ...
                if (player.cocks[0].cockLength > 16)
                {
                    outputText("You barely register the thump of your hardening " + Appearance.cockNoun(CockTypesEnum.HORSE) + " as it smacks into the underside of the table.\n\n");
                }
                else
                {
                    outputText("You squirm uncomfortably, feeling constrained by your [armor] as you surge to erectness.\n\n");
                }
            ...
            else
            {
                ...
                if (player.cor > 50 || player.statusEffectv1(StatusEffects.Edryn) > 0 || flags[kFLAGS.LOW_STANDARDS_FOR_ALL] != null)
                {
                    ...
                    {
                        outputText("  She smiles coyly when your [cock] wriggles, wrapping around her arm.");
                    }
                    outputText("\n\n");
    ...
    {
        if (player.statusEffectv1(StatusEffects.Edryn) >= 4)
        {
            return 0;
        }
        else
        {
            return as3hx.Compat.parseInt(200 / Math.pow(2, player.statusEffectv1(StatusEffects.Edryn)));
        }
    }
    
    private function edrynOffer(x : Float) : Void
    {
        edrynSprite();
        ...
        //Too small{
            ...
            if (cost > 0)
            {
                outputText(num2Text(cost) + " gems?");
            }
            else
            {
                outputText("an hour of your time?");
            }
            outputText("</i>\"\n\n");
            ...
            if (cost > 0)
            {
                outputText("(Do you pay " + num2Text(cost) + " gems to fuck her?)");
            }
            else
            {
                outputText("(Do you fuck her?)");
            }
        ...
        //(HORSE CONT){
            ...
            //Too bigsies{
                ...
                if (player.cockArea(x) >= 24)
                {
                    ...
                    if (player.hasBalls())
                    {
                        outputText("balls tight and your ");
                    }
            ...
            else
            {
                ...
                else if (player.cockArea(x) >= 24)
                {
                    ...
                    if (cost > 0)
                    {
                        outputText(cast((cost), Num2Text) + " gems ");
                    }
                    else
                    {
                        outputText("An hour of your time ");
    ...
    private function fuckEdrynTaur() : Void
    {
        ...
        if (player.cor < 75)
        {
            outputText("You recoil for a second, scolding yourself for the strange thoughts, but you don't slow down the fucking in the slightest.");
        }
        else
        {
            outputText("You smile, realizing you hadn't meant to think of her as a mare and you as her stud, but that's exactly what you are right now.  Your thrusts speed up as you renew your determination to fill this cunt full of fillies.");
        }
        ...
        {
            outputText("strokes your chest");
        }
        else
        {
            outputText("caresses a breast");
        }
        ...
        if (player.cumQ() < 100)
        {
            outputText(" even after you've finished.");
        }
        else
        {
            ...
            if (player.cumQ() > 250)
            {
                outputText("Her body gurgles noisily from the massive fluid injection it's just taken, and her belly expands noticeably.  ");
            }
            if (player.cumQ() > 500)
            {
                outputText("It keeps going as your body spends the last of its massive reserves, bloating her to the point that she'll have trouble walking.  ");
            }
            outputText("Her cunt keeps milking you even after.");
        }
        
        if (player.statusEffectv1(StatusEffects.Edryn) <= 1)
        {
        ...
        else if (player.statusEffectv1(StatusEffects.Edryn) == 4)
        {
    ...
    private function fuckEdrynNonTaur() : Void
    {
        ...
        {
            outputText(" and drop down to your knees to better level yourself with her");
        }
        ...
        if (player.tallness < 60)
        {
            outputText("You grab a stool so you'll be tall enough to fuck her properly and climb onto it.  ");
        }
        outputText("Supporting your");
        if (player.cockArea(x) > 200)
        {
            outputText(" hefty package with both hands");
        }
        else
        {
            outputText("self with your hand");
        }
        ...
        {
            outputText("feeling your ring of prepuce slipping between her lips.  ");
        }
        else
        {
            outputText("squirting a little bit of fluid on the floor.  ");
        }
        ...
        if (player.hasKnot(x))
        {
            outputText("tries to rise, lifting up enough for you to get a few good breaths in, but your knot keeps her locked to you, and the two of you are tied together for another minute or two while it deflates, emptying the very last dregs of your spunk into her.  You pop free, ");
        }
        else
        {
            outputText("slowly rises up off of you, ");
        }
        ...
        if (player.statusEffectv1(StatusEffects.Edryn) <= 1)
        {
            outputText("She sighs happily and looks down at you, \"<i>Ohhhh that was nice.  Most of my customers can't get me off like that.  I hope you're ok!  Don't worry about me getting pregnant either, I take some herbs to prevent that sort of thing until I'm ready for it.</i>\"\n\n");
        }
        else if (player.statusEffectv1(StatusEffects.Edryn) <= 3)
        {
            outputText("She sighs happily and looks down at you, \"<i>Ooh [name], you're the best.  We should do this again soon.  Oh goddess, I can feel my juices starting just thinking about it.</i>\"\n\n");
        }
        else if (player.statusEffectv1(StatusEffects.Edryn) == 4)
        {
            outputText("She sighs happily and says, \"<i>Oh baby, you know what?  I'm never charging you again.  Just keep getting me off like that and we can do it anytime.</i>\"\n\n");
        }
        else
        {
            outputText("She sighs contentedly and says, \"<i>Mmm, never stop visiting ok?</i>\"\n\n");
        }
...
*/
    ...
    public function edrynHeliaThreesomePossible() : Bool
    {
        if (model.time.totalTime == edrynHeliaLastThreesomeCheck || model.time.totalTime == -edrynHeliaLastThreesomeCheck)
        ...
        if (player.gender == 0 || model.time.hours < 14 || model.time.hours >= 20 || rand(2) == 0 || (flags[kFLAGS.HEL_FUCKBUDDY] == 0 && !SceneLib.helFollower.followerHel()) || (flags[kFLAGS.HEL_FOLLOWER_LEVEL] == 1 && flags[kFLAGS.HEL_HARPY_QUEEN_DEFEATED] == 0))
        {
    ...
    private function threesomeEdrynAndHel() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText(", your sack slapping wetly against the bottom of her cunny");
        }
        outputText(".\n\n");
    ...
    //Pregger Offer
    private function pregdrynOffer(cs : Bool = true) : Void
    {
        edrynSprite();
        if (cs)
        {
        ...
        if (pregnancy.incubation < sceneHunter.adjustPregEventTimerNum(250, PregnancyStore.INCUBATION_CENTAUR + 80))
        {
            ...
            {
                outputText("All of your [cocks] fill in seconds, growing rock hard and actually aching with their need.  ");
            }
            else if (player.cockTotal() == 1)
            {
                outputText("Your [cock] fills in seconds, growing rock hard and actually aching with need.  ");
            }
            outputText("You're totally dazed by the massive spike in arousal, and ");
            if (player.cor + player.lib < 100 || player.cockTotal() < 1)
            {
                outputText("you struggle not to reach into your [armor] to touch yourself.");
            }
            else
            {
        ...
        if (player.cockArea(x) >= 24 && player.cockArea(x) < 300)
        {
            ...
            {
                outputText("Does my scent have an even stronger effect on you now");
            }
            else
            {
                outputText("Are you going to cum just from sniffing at my cunt");
            }
    ...
    public function fuckPregEdryn() : Void
    {
        ...
        if (!player.isTaur())
        {
            ...
            if (player.tallness < 60)
            {
                outputText("bouncing against the top of your head");
            }
            else if (player.tallness < 84)
            {
                outputText("battering your face");
            }
            else
            {
                outputText("crushed against your chest");
            }
        ...
        else
        {
            ...
            {
                outputText("Your prodigious jism output soaks her sloppy tunnel and drips from the entrance with the centauress' girlcum.");
            }
            else
            {
                ...
                if (player.cumQ() >= 4000)
                {
                    outputText("  By the time you calm down you've soaked all the pillows and spooge is puddling underneath.");
                }
            }
            outputText("\n\n");
    ...
    private function jizzFromEatingPregdrynOut() : Void
    {
        ...
        if (player.cockArea(x) < 24)
        {
            outputText("small dick begins to twitch and leak.");
        }
        else
        {
            outputText("mammoth cock drags the ground, leaking pre-cum.");
        }
        ...
        if (player.cumQ() < 100)
        {
            outputText("drips pre-cum onto the pillows.");
        }
        else if (player.cumQ() < 500)
        {
            outputText("leaks pre-cum in a steady stream, quickly soaking a few pillows.");
        }
        else
        {
            outputText("pours out enough pre-cum to soak a pillow, and in no time your overenthusiastic dick is doing its damnedest to dribble over everything.");
        }
        ...
        if (player.cumQ() < 1000)
        {
            ...
            if (player.cumQ() < 3000)
            {
                outputText("out the last of your liquid pleasure.");
            }
            else
            {
                outputText("ever greater amounts of liquid peasure.  Jism drips through the floorboards as you create a puddle a few inches deep.");
            }
        }
        outputText("\n\n");
    ...
    private function edrynFucktroduction() : Void
    {
        ...
        if (!player.isTaur())
        {
            if (cost > 0)
            {
                outputText("Coins exchange hands and her arm finds its way back inside your [armor], pulling you towards a back room.  A round of chuckles chases you through the barroom until they're silenced by the heavy thud of a door closing behind you.  You're pushed against the wall hard enough to make your teeth click.  The stars gradually clear from your view, revealing a massive gash, drooling with clear feminine slime.  The scent assails you, ");
            }
            else if (flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] == 0)
            {
                outputText("Her arm finds its way back inside your [armor], pulling you towards a back room.  A round of chuckles chases you through the barroom until they're silenced by the heavy thud of a door closing behind you.  You're pushed against the wall hard enough to make your teeth click.  The stars gradually clear from your view, revealing a massive gash, drooling with clear feminine slime.  The scent assails you, ");
            }
            else
            {
                outputText("A round of chuckles chases you through the barroom until they're silenced by the heavy thud of a door closing behind you.  You're pushed against the wall hard enough to make your teeth click.  The stars gradually clear from your view, revealing a massive gash, drooling with clear feminine slime.  The scent assails you, ");
            }
            if (player.cocks[0].cockType == CockTypesEnum.HORSE)
            {
                outputText("making your " + cockDescript(x) + " twitch and ache.  It floods your body with animalistic desires, overpowering any sense of propriety you might have once had.");
            }
            else
            {
                outputText("overpowering you with strange lust.  As turned on as you are, it's difficult not to enjoy this.");
            }
        }
        //TAUR
        else
    ...
    public function eatEdrynPussyLikeABawss() : Void
    {
        ...
        if (player.cockTotal() == 1)
        {
            outputText("its underside");
        }
        else
        {
            outputText("their underside");
        }
        ...
        {
            outputText("  You note that the smell seems more powerful in an enclosed space, but that's little more than a dim note on a forgotten chalkboard to your brain.  It's nigh-impossible to think about anything but plunging yourself [sheath] deep inside her when exposed to her scent this strongly.");
        }
        ...
        if (flags[kFLAGS.TIMES_EATEN_EDRYN_PUSSY_RUT] == 0)
        {
            outputText("smile tugging at the corners of her mouth at your bravado");
        }
        else
        {
            outputText("lecherous grin spreading across her face as she remembers the last time you did this");
        }
        outputText(".");
        ...
        {
            outputText("She's not even begun to taste the fruits of your talents, and you let your long, long tongue spool out inside her, pressing hard on her walls and tenderly flicking across each sensitive fold inside her.  ");
        }
        ...
        if (player.gender != 1)
        {
            outputText(", or close enough to it");
        }
        ...
        if (y >= 0)
        {
            outputText(" and your " + cockDescript(y) + " into her asshole");
        }
        outputText(".  Her ");
        if (pregnancy.isPregnant)
        {
            outputText("milky tits, swollen from her pregnancy,");
        }
        else
        {
            outputText("big tits");
        ...
        if (pregnancy.isPregnant)
        {
            outputText(" as they drip milk everywhere");
        }
        ...
        if (y >= 0)
        {
            outputText(" and clenching tightness is wreathing your other erection");
        }
        ...
        if (!pregnancy.isPregnant)
        {
            ...
            if (y >= 0)
            {
                outputText("  Her ass is in a similar state.  It's so pressurized with jizz that it squirts out of her with each contraction.");
            }
        }
        else
        {
            ...
            if (y >= 0)
            {
                outputText("  Her butthole gets a similar treatment, filled enough that it's squirting your jizz back out with each contraction the centauress makes.");
            }
