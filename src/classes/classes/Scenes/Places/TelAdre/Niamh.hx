...
class Niamh extends TelAdreAbstractContent implements TimeAwareInterface
{
    ...
    public function timeChange() : Bool
    {
        ...
        {
            flags[kFLAGS.NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER]--;
        }
        if (flags[kFLAGS.NIAMH_MOVED_OUT_COUNTER] > 1)
        {
            flags[kFLAGS.NIAMH_MOVED_OUT_COUNTER]--;
        }
    ...
    public function telAdreNiamh() : Void
    {
        if (flags[kFLAGS.MET_NIAMH] == 0)
        {
            ...
            {
                outputText("massive N-cup breasts, spilling over the table");
            }
            else if (model.time.hours <= 9)
            {
                outputText("huge M-cup breasts, taking up much of the table");
            }
            else if (model.time.hours <= 10)
            {
                outputText("great L-cup breasts, advancing far onto the table");
            }
            else if (model.time.hours <= 11)
            {
                outputText("heavy K-cup breasts, lying on the table for support");
            }
            else if (model.time.hours <= 12)
            {
                outputText("hefty J-cup breasts, resting comfortably on the table");
            }
            else if (model.time.hours <= 13)
            {
                outputText("weighty I-cup breasts, dangling to lie on the table");
            }
            else if (model.time.hours <= 14)
            {
                outputText("squeezable H-cup breasts, settling burdensomely on her chest");
            }
            else
            {
                outputText("moderately manageable G-cups, trembling with every breath she takes");
            }
        ...
        else
        {
            ...
            if (model.time.hours <= 8)
            {
                outputText("her titanic N-cup 'kegs' resting against the thankfully sturdy table in front of her.  Her teats are engorged with the beer she's cursed to carry, and the stuff dribbles out whenever she shifts.");
            }
            else if (model.time.hours <= 14)
            {
                outputText("her ");
                if (model.time.hours <= 9)
                {
                    outputText("M-cup");
                }
                else if (model.time.hours <= 10)
            ...
            {
                outputText("her G-cup beer-filled boobs far smaller than their morning volume, though still impressive in stature.  You can follow the progress she's made throughout the day by the beer stains running along the wooden surface, trailing from all the way off the table and ending where her boobs now lie.");
            }
        }
    }
    ...
    public function getANiamhBeer() : Void
    {
        ...
        {
            outputText("  You understand exactly what she's going through; the weight of a huge pair of breasts, the sensitivity as they engorge with fluids, the near-maddening sensation of the contents sloshing around underneath your straining skin...");
        }
    ...
    private function drinkNiamhsBeerInTelAdre() : Void
    {
        ...
        if (player.lactationQ() >= 300 && player.biggestTitSize() >= 5 && rand(2) == 0 && flags[kFLAGS.MET_NIAMH] > 0)
        {
            ...
            if (isChristmas())
            {
                ...
                if (player.hasCock())
                {
                    outputText("[eachCock] to stiffen");
                }
    ...
    private function talkToNiamh() : Void
    {
        ...
        if (flags[kFLAGS.TALKED_NIAMH] == 0)
        {
            ...
            if (model.time.hours < 15)
            {
                outputText("who usually sits over yonder ");
            }
    ...
    public function blackCatBeerEffects(player : Player, clearScreen : Bool = true, newLine : Bool = false) : Void
    {
        if (clearScreen)
        {
            clearOutput();
        ...
        {
            if (player.libStat.max - player.lib >= 10)
            {
                lib = 10;
            }
            else
            {
                lib = player.libStat.max - player.lib;
            }
	...
	*/
    ...
    public function bimboChampagne(player : Player, clearScreen : Bool, intro : Bool) : Void
    {
        if (clearScreen)
        {
            clearOutput();
    ...
    {
        var y : Float = player.biggestCockIndex() + 1;
        clearOutput();
        //if (flags[kFLAGS.NIAMH_STATUS] == 2) {
    ...
    private function sellYourBooze() : Void
    {
        clearOutput();
        player.gems += 2;
        var gems : Float = Math.round(player.lactationQ() / 100);
        if (gems > 40)
        {
            gems = 40 + rand(3);
        }
        ...
        if (player.bRows() == 1)
        {
            outputText("With a chipper giggle Niamh pulls you closer to the bar.  With both hands she helps you heft your tits up and on to the counter.");
        }
        //If player has four or six breasts.
        else
        {
            outputText("Niamh giggles as you try to move your breasts closer to the bar to be milked by the patrons, but it's obvious only the top pair of your tits can easily rest on the counter top.  \"<i>Lassie, it might be unconventional, but givin' the circumstance mayhaps it'd be best if ye laid down on the bar.</i>\"");
        }
        ...
        if (player.hasVagina())
        {
            outputText("  You feel your excitement dripping down the inside of your thighs as your pussy wettens.");
        }
        dynStats("lus", 10 + player.sens / 5, "scale", false);
        //[If player has cocks]
        if (player.hasCock())
        {
            outputText("  You start to feel sympathetic sensations in [eachCock] as the hands and paws of the bar goers milk your nipples.  With their fingers sliding over your sensitive nubs you can easily imagine them sliding up and down your cock");
        }
        ...
        else
        {
            ...
            if (player.hasVagina())
            {
                outputText("You can feel your labia dripping with need.  ");
            }
            if (player.hasCock())
            {
    ...
    private function barBeerOrgyTits() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText("The drooling of your [vagina] gets worse as the constant \"tapping of your kegs\" drives your sense of self-restraint to its limits.  The hands of various drunks start moving more aggressively over your breasts, eventually winding their way down between your thighs.");
        }
        ...
        {
            outputText("  She slaps your ass to emphasize her meaning, and the action sends jiggling waves through each immense cheek.");
        }
        ...
        {
            outputText("Your giant boobs are large enough to temporarily hang off the sides of your body as you're carried.");
        }
        ...
        if ((!player.hasVeryVisiblePregnancy()) && player.bRows() > 1)
        {
            outputText("  You are essentially a giant pregnant belly surrounded by your [chest] on the table, helpless to the sexual advances of the bar goers and you can't get enough of it.");
        }
        // [IF player has four or more giant tits, but not hugely pregnant]
        else if (player.bRows() > 1)
        {
            outputText("  You are essentially a pile of " + boozeBoobsType() + " filled tits.");
        }
        ...
        if (model.time.hours < 15 && flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 0 && player.hasVagina())
        {
            ...
            {
                outputText("  Some folks give yelps and cheers when they see the bulge that repeatedly forms in your abdomen each time Urta thrusts herself inside you.");
            }
        ...
        else if (player.hasVagina())
        {
            ...
            {
                outputText("  One is sporting a cock that is obviously too massive for your pussy, and eventually backs down so that the other one, despite still being on the large size, can take the honor of fucking your cunt silly.");
            }
            //ELSE IF [player has a gaping pussy or bigger]
            else
            {
                outputText("  They're both sporting large canine cocks, but one is particularly massive while the other would likely be a loose fit for a pussy as naturally stretched out as yours.  The largest one wins out and positions himself to give your overly wide fuck hole a much needed stretching.");
            }
	...
	================*/
        ...
        {
            outputText("  \"<i>Oi!  Careful folks, this babe could blow at any time!</i>\"  Niamh pats your belly with chiding affection.");
        }
        ...
        {
            outputText("  Eventually some of the randier drunks start whipping out their cocks, and begin pressing the heads against the openings of your nipples.  Within moments you feel your booze bloated breasts being fucked silly by " + num2Text(player.totalNipples()) + " hard cocks.  Each thrust causes " + boozeBoobsType() + " to spill out around their shafts.");
        }
        // ELSE[Character doesn't have fuckable nipples]
        else
        {
            outputText("  Some of the hornier customers start shoving cocks of all shapes and sizes into the moist wet cleavages formed by yours and Niamh's breasts.");
        }
        ...
        if (player.cockTotal() > 0)
        {
            ...
            {
                outputText("  Globs of cum splatter across your gravid belly.");
            }
        }
        ...
        if (player.statusEffectv1(StatusEffects.Edryn) >= 5 && player.gender == 3 && model.time.hours == 14 && flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 0)
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("  Edryn moans out loud in ecstasy as your [cock] fills her up.");
            }
            // If [player has multiple cocks]
            else if (player.cockTotal() > 1)
            {
                outputText("  The heads of your cocks press in between Edryn's labia causing her to cry out, \"<i>Oh gawd!  How many dicks are y-you f-f-fucking meeee with???</i>\"  Edryn's orgasm cuts her voice off and every muscle in her cunt contracts around your [cocks].");
            }
            ...
            if (player.hasVisiblePregnancy())
            {
                outputText("  Your pregnant belly presses up against Edryn's ass as both Urta's cock and Edryn's pussy keeps you pinned in place between them.");
            }
            // IF[player is not pregnant]
            else
            {
                outputText("  Urta's cock and Edryn's pussy pin you in place between them, and as Urta fucks you deeper her cock head forces your belly to bulge outwards until it presses against Edryn's ass.");
            }
            ...
            if (player.bRows() == 2)
            {
                outputText("  Your second row of tits get thrust out to either side every time Urta's torso presses your own against Edryn.");
            }
            //IF [player has six giant breasts]
            else if (player.bRows() == 3)
            {
                outputText("  Your lowest row of tits slap against all three of your thighs as the love making becomes more brutal.");
            }
            //IF [player has large balls]
            if (player.hasBalls() && player.ballSize > 14)
            {
                outputText("  Your hanging testicles bounce around between everyone's legs.");
            }
            //IF [player has giant ass]
            if (player.butt.type >= 22)
            {
                outputText("  The fat of your ass cheeks jiggles and quakes even as Urta practically stuffs her hips into your crack.  \"<i>Damn, you've got a bigger ass than Edryn,</i>\" she chides as she spanks you.  \"<i>Hey!</i>\"  Edryn calls out with a false tone of hurt pride.");
            }
        ...
        else if (player.hasVagina() && model.time.hours < 15 && flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 0)
        {
            ...
            if (player.bRows() > 1)
            {
                outputText("  Your rows of breasts shift around as your posture changes, making squelching sounds as they slide around.");
            }
            //If [player not pregnant]
            if (player.canGetPregnant())
            {
                outputText("  Urta's small ocean of sperm streams out from your pussy like a river down your legs as you try to stand.");
            }
        ...
        else
        {
            //Generic ending if the first two don't trigger{
                ...
                if (player.hasVagina())
                {
                    outputText("  The giant knot of the dog morph finally breaks its way inside causing your pussy to become overstretched.  The pleasurable torture only gets worse as he begins unloading a river of his cum inside you.  It begins squirting out around his cock and soaks both of your thighs in the process.");
                }
                player.cuntChange(60, true, true, false);
                ...
                if (player.hasVagina())
                {
                    outputText("  You gag on it at the same time a new cock fills your cunt, ready to renew the thrusting that's been causing your tits to bounce all over the place.");
                }
                
                //IF [player has single cock small-med cock]
                if (player.cockTotal() == 1 && player.biggestCockArea() < 75)
                {
                ...
                if (player.cockTotal() == 1)
                {
                    outputText("  The six titted girl smiles at you, patting her belly while your cum runs down the inside of her thighs.  You only now wonder about how fertile she might be.");
                }
                // IF [player has multiple cocks]
                else if (player.cockTotal() > 1)
                {
                    outputText("  Various girls around the room are patting their bellies and giving you sultry looks while your cum continues to drip from their pussies.  You passively wonder if any of them might indeed be hauling around some larger pregnant bellies in a few days, but you try to push those thoughts from your head.  At the moment they seem happy.");
                }
                // IF [player has pussy]
                if (player.hasVagina())
                {
                    ...
                    if (player.canGetPregnant())
                    {
                        outputText("  You can't help but wonder how virile those dog morphs might have been as their cum and the cum of other customers sloshes around inside your uterus.");
                    }
                    switch (rand(6))
                    {
                        case 0:
