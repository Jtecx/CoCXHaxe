...
class KatherineEmployment extends TelAdreAbstractContent
{
    public var edryn(get, never) : Edryn;
    public var katherine(get, never) : Katherine;
    public var threesome(get, never) : KatherineThreesome;
    ...
    public static inline var KBIT_TRAINING_TALK_EDRYN : Int = 4;
    public static inline var KBIT_TRAINING_TALK_URTA : Int = 8;
    public static inline var KBIT_TRAINING_TALK_KATH : Int = 16;
    public static inline var KBIT_TRAINING_URTA_HELP : Int = 32;
    ...
    private function get_threesome() : KatherineThreesome
    {
        return telAdre.katherineThreesome;
    }
    ...
    private function tryTheWatch() : Void
    {
        ...
        var pissedOffUrta : Bool = (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] == -1);
        var pissedOffEdryn : Bool = (flags[kFLAGS.EDRYN_NEVER_SEE_AGAIN] == 1);
        ...
        //Well aren't you a giant dick{
            ...
            flags[kFLAGS.KATHERINE_TRAINING] = 3;
        }
        else
        {
    ...
    //Triggers when you go to talk to Edryn in the Wet Bitch and
    {
        ...
        if (edryn.pregnancy.isPregnant)
        {
            outputText(" stop her pregnancy-induced feasting, stifle a belch and");
        }
        ...
        var edrynKidsPlus : Int = as3hx.Compat.parseInt(flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] + ((edryn.pregnancy.isPregnant) ? 1 : 0));
        if (edrynKidsPlus > 0)
        {
            outputText("  She then widens her eyes in surprise");
            if (edryn.pregnancy.isPregnant)
            {
                outputText(", one hand brushing against her swollen equine belly as good it can");
            }
        ...
        flags[kFLAGS.KATHERINE_TRAINING] = flags[kFLAGS.KATHERINE_TRAINING] | KBIT_TRAINING_TALK_EDRYN;  //Using a mask so it doesn’t matter what order you talk to Edryn and Urta in  
    ...
    public function canTalkToUrta() : Bool
    {
    ...
    //Triggers when you go to talk to Urta in the Wet Bitch
    {
        ...
        flags[kFLAGS.KATHERINE_TRAINING] = flags[kFLAGS.KATHERINE_TRAINING] | KBIT_TRAINING_TALK_URTA;  //Using a mask so it doesn’t matter what order you talk to Edryn and Urta in  
        if (urta.urtaLove())
    ...
    //Only appears if you’ve talked to Edryn, Urta or the desk sargeant and haven’t yet had this talk with Kath
    {
        ...
        flags[kFLAGS.KATHERINE_TRAINING] = flags[kFLAGS.KATHERINE_TRAINING] | KBIT_TRAINING_TALK_KATH;  //Using a mask so we still know if you’ve talked to Urta. This is important if she’s a lover.  
        doNext(camp.returnToCampUseOneHour);
    }
    
    public function initiateTraining() : Bool
    {
        if ((flags[kFLAGS.KATHERINE_TRAINING] & KBIT_TRAINING_TALK_KATH) == 0)
        {
            return false;
        }  //Can't start training yet  
        var urtaAvailable : Bool = urta.urtaAtBar() && !urta.urtaDrunk();
        ...
        if (!urta.urtaLove() || (flags[kFLAGS.KATHERINE_TRAINING] & KBIT_TRAINING_TALK_URTA) == 0)
        {
            withUrta = null;
        }  //Urta must be a lover and you must have talked about Kath  
    ...
    public function katherineTrainingStage1(clearOut : Bool = true) : Void
    {
        if (clearOut)
        {
            clearOutput();
        ...
        switch (rand(3))
        {
            case 0:doNext(katherineTrainingStage1Success);
            case 1:doNext(katherineTrainingStage1Failure);
    ...
    public function katherineTrainingStage2() : Void
    {
        ...
        switch (rand(3))
        {
            case 0:doNext(katherineTrainingStage2Success);
            case 1:doNext(katherineTrainingStage2Failure);
    ...
    private function katherineTrainingStage2Horny() : Void
    {
        ...
        if (player.cockThatFits(70) > -1)
        {
            ...
            {
                penBoth = katherine.pcPenetratesKatDoubly;
            }
        }
        if (player.hasVagina() && player.vaginalCapacity() >= 100)
        ...
        //Don't want to get knotted together out here at the lake{
            ...
            {
                takeVagAndAss = katherine.getDoublePennedByKat;
            }
        }
        if (katherine.knotSize < 4)
        {
            takeAnal = katherine.getPenetrated;
        }
    ...
    public function katherineTrainingStage3() : Void
    {
        ...
        else
        {
            switch (rand(3))
            {
                case 0:doNext(katherineTrainingStage3Success);
                case 1:doNext(katherineTrainingStage3Failure);
    ...
    private function katherineTrainingStage3Failure() : Void
    {
        ...
        if (flags[kFLAGS.KATHERINE_TRAINING] > 99)
        {
            flags[kFLAGS.KATHERINE_TRAINING] = 99;
        }  //Only a successful training session should lead to her completing her training next time.  
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    private function alleywaySexOptions() : Void
    {
        ...
        if (player.cockThatFits(70) > -1)
        {
            ...
            if (katherine.knotSize <= 4)
            {
                suckNFucks = katherine.suckNFuck;
            }
        }
        if (player.hasCock() && flags[kFLAGS.KATHERINE_TIMES_SEXED] > 0)
        {
            suckNFucked = katherine.suckedNFuckedByKat;
        }
        if (player.hasVagina())
        {
            takeVag = katherine.letKatKnotYourCuntPussyFuck;
    ...
    //Triggers when you visit Kath with her training score at or over 100
    {
        ...
        if (urta.urtaDrunk() || model.time.hours >= 15)
        {
            ...
            {
                outputText("Although Kath is ready for testing you don’t think the captain of Tel’Adre’s guard is going to be in any condition to test her.  You’re better off waiting until she’s sober.");
            }
        ...
        if (flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > -1)
        {
            ...
            //Urta is either your lover or a fuckbuddy{
                ...
                if (urta.urtaLove() && (flags[kFLAGS.AMILY_VISITING_URTA] == 4 || flags[kFLAGS.URTA_KNOWS_PC_HAS_MARBLE_FOLLOWER] > 0))
                {
                    ...
                    if (flags[kFLAGS.URTA_OPEN_ABOUT_EDRYN] > 0)
                    {
                        outputText(" and [he] doesn’t get [his] back up when I sleep around");
                    }
        ...
        else if (flags[kFLAGS.TIMES_FUCKED_URTA] == 0)
    ...
    private function trainKathWithUrta() : Void
    {
        ...
        flags[kFLAGS.KATHERINE_TRAINING] = flags[kFLAGS.KATHERINE_TRAINING] | KBIT_TRAINING_URTA_HELP;  //This indicates that next time we see Kath it’s time for her to train with Urta  
    ...
    private function katherineTrainingWithUrtaStage1() : Void
    {
        ...
        switch (rand(3))
        {
            case 0:doNext(katherineTrainingWithUrtaStage1Success);
            case 1:doNext(katherineTrainingWithUrtaStage1Failure);
    ...
    private function katherineTrainingWithUrtaStage2() : Void
    {
        ...
        switch (rand(3))
        {
            case 0:doNext(katherineTrainingWithUrtaStage2Success);
            case 1:doNext(katherineTrainingWithUrtaStage2Failure);
    ...
    private function katherineTrainingWithUrtaStage3() : Void
    {
        ...
        else
        {
            switch (rand(3))
            {
                case 0:doNext(katherineTrainingWithUrtaStage3Success);
                case 1:doNext(katherineTrainingWithUrtaStage3Failure);
    ...
    private function katherineTrainingWithUrtaCompleteContinued() : Void
    {
        ...
        if (flags[kFLAGS.KATHERINE_URTA_AFFECTION] > 28)
        {
            flags[kFLAGS.KATHERINE_URTA_AFFECTION] = 28;
        }  //Make sure they don't like each other too much  
        if (player.gender == 0)
        {
    ...
    //This scene plays automatically the first time that the player goes to Tel’Adre after Kath’s training is done
    {
        ...
        {
            flags[kFLAGS.KATHERINE_TRAINING] = 1;
        }
    ...
    public function postTrainingAlleyDescription() : Void
    {
        ...
        if (!katherine.isAt(Katherine.KLOC_KATHS_APT))
