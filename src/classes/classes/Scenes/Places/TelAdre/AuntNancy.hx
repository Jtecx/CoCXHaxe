...
//265 - Talk Nancy
//266 - Times boned
class AuntNancy extends TelAdreAbstractContent
{
    //[Introduction Blurb:
    //(appears in the Wet Bitch between 6:00 and 14:00)]
    public function auntNancy(display : Bool = true) : Bool
    {
        if (model.time.hours >= 6 && model.time.hours <= 14)
        {
            if (display)
            {
                ...
                {
                    outputText("  Aunt Nancy");
                }
                outputText(" glances up at you and smiles as you enter");
                //[(if unmet)
                if (flags[kFLAGS.NANCY_MET] == 0)
                {
                    outputText(", and you see that she has additional red eyes spread across her forehead");
                }
                outputText(".");
            }
            return true;
    ...
    public function interactWithAuntNancy() : Void
    {
        ...
        else
        {
            //If Relationship with Nancy < 30{
                ...
                if (flags[kFLAGS.NANCY_MET] < 1)
                {
                    flags[kFLAGS.NANCY_MET] = 1;
                }
                //[Choice: Strong, Light]
    ...
    private function lightStuff() : Void
    {
        ...
        if (player.inte >= 20)
        {
            outputText("  You realize that what look like almost-shoulder-length gloves over her hands must actually be shell-like portions of this exoskeleton.");
        }
        outputText("\n\n");
        ...
        if (flags[kFLAGS.NANCY_TALKS] == 0)
        {
            ...
            {
                outputText("like poor little Yara at the earring shop,");
            }
    ...
    private function timeForAuntNancySpiderCooch() : Void
    {
        ...
        {
            outputText(" and taking the opportunity to grope your [allbreasts] right through your " + player.armorName);
        }
        outputText(".\n\n");
    ...
    private function auntNancyPoonPartII() : Void
    {
        ...
        {
            outputText("\"<i>Are you ready for me, friend?</i>\" murmurs Aunt Nancy, her throaty voice washing over you, as two of her hands rest on your " + chestDesc() + " and the other two wrap carefully around your neck and shoulders, gently rubbing you in the promised massage. She's crawling along the web, her many feet easily avoiding the sticky parts, and her face is mere inches from yours. You can smell her sweet, slightly alcohol-scented breath as her many eyes look down at you.  \"<i>I certainly am, little fly.</i>\"\n\n");
        }
        ...
        if (player.inte > 40)
        {
            outputText("realize that the spit must contain some kind of chemical originally meant to still prey");
        }
        else
        {
            outputText("question whether or not the spider-lady's kiss is responsible");
        }
        outputText(".\n\n");
        ...
        {
            outputText(" to the top of your tail");
        }
        outputText(".\n\n");
        ...
        if (player.isNaga())
        {
            outputText("Gently, she peels away the fold of skin that disguises your genitals, now moist-and-dripping with anticipation.  ");
        }
        ...
        if (player.hasBalls())
        {
            ...
            {
                if (player.ballSize > 5 && player.ballSize < 15)
                {
                    outputText("taking one");
                }
                else
                {
                    outputText("both of them");
                }
            ...
            {
                outputText("kissing and licking at them with those moist, relaxing lips.\n\n");
            }
        }
        ...
        if (player.hasVagina())
        {
            ...
            {
                outputText("lifts away your other \"<i>equipment</i>\" and ");
            }
        ...
        if (player.gender == 0)
        {
            outputText("\"<i>Well, well, well,</i>\" murmurs Aunt Nancy, intrigued by what she sees.  \"<i>Very unorthodox, but we'll see what we can do.</i>\"  She pauses to lick the fleshy expanse where your genitals used to be.\n\n");
        }
        ...
        function dickF() : Void
        {
            ...
            if (player.cockTotal() >= 2)
            {
                if (player.cocks[y].cockLength >= 22)
                {
                    ...
                    if (player.cockTotal() > 2)
                    {
                        outputText("another one of ");
                    }
                    outputText("your ");
                    if (player.cockTotal() == 2)
                    {
                        outputText("other cock ");
                    }
                    else
                    {
                        outputText("cocks ");
                    ...
                    if (player.cocks[y].cockLength >= 30)
                    {
                        outputText("letting its head slip into her mouth, where she sucks and licks at it hungrily, ");
                    }
                    else
                    {
                        outputText("letting her long, dexterous tongue dangle down from her mouth to wrap around the head of her dick, occasionally licking at her own impressive bosom as well, ");
                    }
                ...
                if (player.cockTotal() > 2)
                {
                    ...
                    {
                        outputText("Another of her hands wraps itself around your other cock");
                    }
                    else
                    {
                        outputText("Her other hands wrap themselves around more of your cocks");
                    }
            ...
            if (!titFucked || (titFucked && player.cockTotal() <= 2))
            {
                ...
                if (player.hasBalls())
                {
                    outputText("gently kneading and caressing your [balls] as the cum inside churns and boils and ");
                }
                //[If the player has a vagina]
                else if (player.hasVagina())
                {
                    outputText("exploring the folds and creases of your " + vaginaDescript(0) + " and ");
                }
        ...
        function vagF() : Void
        {
            ...
            if (player.tailType > Tail.NONE)
            {
                outputText("tail before rising to your ");
            }
    ...
    {
        var titfuck : Bool = y >= 0 && player.cocks[y].cockLength >= 22;
        var freeDicks : Int = as3hx.Compat.parseInt(player.cockTotal() - ((titfuck) ? 2 : 1));
        if (y >= 0 && !titfuck)
        {
            sceneHunter.print("Check failed - second cock not long enough!");
        }
        clearOutput();
        //[If PC has a cock]
        if (x >= 0)
        {
            ...
            if (player.hasBalls())
            {
                outputText(ballsDescriptLight() + " tense, churning and tightening.");
            }
            //Else:]
            else
            {
                outputText("body tensing in preparation for letting out your seed.");
            }
            outputText("\n\n");
            ...
            if (player.cumQ() >= 750)
            {
                outputText("  Sooner or later though, even her marvelous body can simply contain no more of your jizz, and she slips her swollen cunt up off of your organ, letting it continue squirting your thick, plentiful cum all over her as it swings free.");
            }
            outputText("\n\n");
            ...
            if (titfuck)
            {
                ...
                if (player.cumQ() >= 750)
                {
                    outputText(", some even reaching to the ceiling to fall into her short, pretty white hair and splattering over her trailing abdomen");
                }
                outputText(".\n\n");
            }
            if (freeDicks == 1)
            ...
            if (player.hasVagina())
            {
                ...
                {
                    outputText("making you long for something inside of it");
                }
            ...
            {
                outputText("  Her lower body's a mess of smeary dollops of cum, right up to the bottom halves of her impressive boobs, and her face and upper body haven't fared much better. Behind her, you can see where stray shots that sprayed over her have painted streaks of her abdomen white.");
            }
            outputText("\n\n");
        ...
        else
        {
            ...
            if (player.hasVagina())
            {
                outputText(vaginaDescript(0) + " and ");
            }
    ...
    private function lunaStory() : Void
    {
        ...
        if (flags[kFLAGS.LUNA_FOLLOWER] == 0 || flags[kFLAGS.LUNA_FOLLOWER] == 1)
        {
            outputText("Mentally you make a note not to get handsy with the strange maid");
        }
        if (flags[kFLAGS.LUNA_FOLLOWER] == 0)
        {
            outputText(".");
        }
        if (flags[kFLAGS.LUNA_FOLLOWER] == 1)
        {
            outputText(", especially with her staying at your camp now. Still, she's shown no sign of aggression so far...");
        }
        if (flags[kFLAGS.LUNA_FOLLOWER] == 2 || flags[kFLAGS.LUNA_FOLLOWER] == 3)
        {
            outputText("You find yourself speechless, and decide it will be best if Aunt Nancy doesn't know.");
        }
        if (flags[kFLAGS.LUNA_FOLLOWER] >= 4 && flags[kFLAGS.LUNA_FOLLOWER] <= 6)
        {
            outputText("You tell Aunt Nancy that she's actually been working for you at your camp, and that she's been a marvelous help with no sign of any such episode.\n\nShe looks surprised, and says \"<i>Well, I'm glad to hear she's doing well. I wouldn't try anything saucy though, if I were you; not unless you get a pretty clear invitation.</i>\" She laughs wryly, and you agree.");
        }
        if (flags[kFLAGS.LUNA_FOLLOWER] > 6)
        {
            outputText("Knowing what you know about Luna now, it's hardly a surprise something like this happened. You feel badly about sending her here in retrospect, though there's no way you could've known.");
        }
