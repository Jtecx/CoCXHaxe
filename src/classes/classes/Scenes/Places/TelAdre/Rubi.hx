...
class Rubi extends TelAdreAbstractContent
{
    private function rubiSprite() : Void
    {
        if (flags[kFLAGS.RUBI_HORNTYPE] > 0)
        {
            spriteSelect(SpriteDb.s_rubi_horns);
        }
        else
        {
            spriteSelect(SpriteDb.s_rubi_hornless);
        }
    }
    ...
    private function rubiBimbo() : Bool
    {
        return (flags[kFLAGS.RUBI_BIMBO] > 0);
    }
    private function rubiIncubus() : Bool
    {
        return (flags[kFLAGS.RUBI_INCUBUS_PROGRESS] == 3);
    }
    public function rubiAffection(delt : Float = 0) : Float
    {
        if (delt == 0)
        {
        ...
        else if (flags[kFLAGS.RUBI_AFFECTION] < 0)
        {
            flags[kFLAGS.RUBI_AFFECTION] = 0;
        }
        return flags[kFLAGS.RUBI_AFFECTION];
    }
    ...
    public function rubiCapacity() : Float
    {
        var bonus : Int = as3hx.Compat.parseInt(flags[kFLAGS.RUBI_TIMES_ANALLY_TRAINED] * 10);
        if (bonus > 76)
        {
            bonus = 76;
        }
    ...
    public function rubiMF(man : String, woman : String) : String
    {
        ...
        else if (flags[kFLAGS.RUBI_SHE] == 1)
        {
            return woman;
        }
        else
    ...
    private function rubiIntros() : Function
    {
        if (flags[kFLAGS.RUBI_SETUP] == 0)
        {
            initializeRubi();
        }
        if (flags[kFLAGS.RUBI_DISABLED] > 0 || flags[kFLAGS.RUBI_PROSTITUTION] == -2)
        {
            return null;
        }
        outputText("\n");
        ...
        {
            outputText("A cute human girl moves from table to table, bussing the empty ones and serving drinks to the occupied.  She's pretty flat-chested, and wears a pink and white frilly waitress uniform with an apron.  When she turns her back to you, you can tell she's not <i>entirely</i> human, what with the long red spaded tail coming out from under her dress.");
        }
        //(Have met, relationship 0-19)
        else if (rubiAffection() < 30)
        {
            outputText("Rubi is here, moving from table to table efficiently clearing the empty ones and serving new customers.  Her long red tail swishes back and forth, sometimes lifting her dress enough to get a peek at her panties.");
        }
        ...
        {
            outputText("Rubi is here, moving from table to table, not quite as efficiently as [rubi ey] used to, but with a lot more exuberance.  [rubi Eir] long red tail swishes around, seemingly drawn towards the crotch of patrons and co-workers alike.");
        }
        else
        {
            outputText("Rubi is here, moving from table to table efficiently clearing the empty ones and serving new customers.  [rubi Eir] long red tail swishes back and forth excitedly.");
        }
        outputText("\n");
    ...
    public function approachRubiScenes() : Void
    {
        ...
        else
        {
            ...
            //(Waitress Outfit (Bimbo)){
                ...
                if (flags[kFLAGS.RUBI_BREAST_SIZE] >= 4)
                {
                    outputText("  Rubi's breasts strain against the tight blouse, threatening to pop a button at any moment.");
                }
                if (flags[kFLAGS.RUBI_COCK_SIZE] >= 9)
                {
                    outputText("  You notice Rubi holds [rubi emself] a little awkwardly, and it's only when [rubi ey] sits down that you spy the slight bulge in [rubi eir] skirt.");
                }
            }
            //(Suit Clothes (Normal))
            else if (select == 2)
            ...
            //(Suitclothes){
                ...
                if (flags[kFLAGS.RUBI_HORNTYPE] > 0)
                {
                    outputText("just behind the horns");
                }
                outputText(".  In a sexy sort of way, [rubi ey] looks like a girl dressed in her boyfriend's clothes");
                if (flags[kFLAGS.RUBI_BREAST_SIZE] >= 4)
                {
                    outputText(", though with [rubi eir] large bosom, [rubi ey] can't quite button up all the way, giving the appearance of a sexy secretary");
                }
                outputText(".");
            }
            else if (select == 3)
            ...
            //(Rubber Fetish Outfit (Bimbo)){
                ...
                if (flags[kFLAGS.RUBI_NIPPLE_TYPE] > 0)
                {
                    outputText("black ");
                }
                ...
                if (flags[kFLAGS.RUBI_BREAST_SIZE] >= 4)
                {
                    outputText(" impressive bosom, and sort of makes [rubi em] look like the kind of rubber doll one might buy for those lonely nights.");
                }
                else
                {
                    outputText(" relatively flat chest, though with the rest of [rubi eir] figure, it's hard to imagine [rubi em] as anything but a woman.");
                }
            }
            else if (select == 5)
            ...
            //(Tube Top){
                ...
                if (flags[kFLAGS.RUBI_BREAST_SIZE] >= 4)
                {
                    outputText("  [rubi Eir] breasts strain the fabric of the tube top, any wrong move threatening to have them burst forth.");
                }
                //(dick 9" or higher)
                if (flags[kFLAGS.RUBI_COCK_SIZE] >= 4)
                {
                    outputText("  [rubi Eir] rather large bulge can't be concealed, however, and you sometimes catch Rubi holding [rubi eir] notepad down across [rubi eir] crotch when [rubi ey] moves from table to table.");
                }
                else
                {
                    outputText("[rubi Eir] bulge is well concealed at least.  You're sure it would take a skilled eye, or someone who knew what to look for, to notice Rubi's little package.");
                }
            }
            else if (select == 6)
            ...
            //(Bodysuit){
                ...
                if (flags[kFLAGS.RUBI_COCK_SIZE] >= 9)
                {
                    outputText("  However, with an outfit this tight, it's almost impossible for [rubi em] to hide [rubi eir] impressive package.  Rubi's cock pokes straight upwards, creating a bulge from [rubi eir] crotch up towards [rubi eir] belly button.");
                }
                else if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
                {
                    outputText("  However, with an outfit this tight you can see a slight outline where [rubi eir] small package is tucked away.  You wonder how many other patrons have noticed the same thing.");
                }
            }
            else if (select == 7)
            ...
            //(Long Dress (Bimbo)){
                ...
                if (rubiBimbo())
                {
                    outputText("  [rubi Ey]'s even modified the dress with false gems and a slit up the side to show off [rubi eir] leg.");
                }
                ...
                if (flags[kFLAGS.RUBI_BREAST_SIZE] <= 1)
                {
                    outputText("  The dress must have come with a padded bra, because you're surprised to see Rubi with  breasts.  Though small, they help to accentuate [rubi eir] feminine figure.");
                }
            }
            else if (select == 8)
            ...
            //(Tight Leather Pants (Bimbo)){
                ...
                if (flags[kFLAGS.RUBI_BREAST_SIZE] >= 4)
                {
                    outputText("  With [rubi eir] large rack, [rubi ey] looks much like a busty piratess.  From the way [rubi eir] breasts strain at the top, you're certain that one wrong breath could send a button flying across the room.");
                }
                else
                {
                    outputText("  With [rubi eir] relatively flat chest, however, you wonder if perhaps it makes [rubi em] look a bit too much like a man.  An effeminate man, but a man nonetheless.");
                }
                //Cock variants:
                //(9" or higher)
                if (flags[kFLAGS.RUBI_COCK_SIZE] >= 9)
                {
                    outputText("  [rubi Eir] generous cock is on display, much to [rubi eir] chagrin, as a bulge running down the length of one pant leg.");
                }
                else
                {
                    outputText("Luckily, given [rubi eir] relatively small package, [rubi eir] bulge is kept nice and secret even with the tightness of the clothing.");
                }
            }
            else if (select == 9)
            ...
            //(Nurse Clothes (Bimbo)){
                ...
                if (flags[kFLAGS.RUBI_BREAST_SIZE] >= 4)
                {
                    outputText("  Rubi's large breasts strain the fabric, drawing the dress further up [rubi eir] body and showing off more of [rubi eir] legs and ass.");
                }
            }
            //(Slutty Swimwear (Normal))
            else if (select == 10)
            ...
            //(Slutty Swimwear){
                ...
                if (flags[kFLAGS.RUBI_BREAST_SIZE] >= 4)
                {
                    outputText("  Rubi's tremendous cleavage threatens to spill out of the skimpy bikini top with every movement, and from the red faces around the bakery, you wonder if that's already happened.");
                }
                else if (flags[kFLAGS.RUBI_BREAST_SIZE] <= 1)
                {
                    outputText("  The top only emphasizes Rubi's relatively flat chest, though with the rest of [rubi eir] figure, [rubi ey] still looks extremely feminine.");
                }
            }
            ...
            else if (select == 11)
            {
                ...
                {
                    outputText("  [rubi Eir] skirt rides so high you catch glimpses of [rubi eir] unsurprisingly unpantied crotch, and the enticing package held within.  You wonder how many people have gotten a surprise with their drinks today!");
                }
            }
            //(Bondage Straps)
            else if (select == 12)
            ...
            else if (select == 14)
            {
                ...
                if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
                {
                    outputText(rubiCock(33) + ", clad in a cute little frilly pink cocksock");
                }
                if (flags[kFLAGS.RUBI_NO_CUNT] == 0 && flags[kFLAGS.RUBI_COCK_SIZE] > 0)
                {
                    outputText(" and ");
                }
                if (flags[kFLAGS.RUBI_NO_CUNT] == 0)
                {
                    ...
                    {
                        outputText(" trimmed feminine mons, free and open to the air");
                    }
                }
                outputText(".");
                //(D breasts or higher)
                if (flags[kFLAGS.RUBI_BREAST_SIZE] >= 4)
                {
                    outputText("  Rubi's breasts, large as they are, show off a decent amount of sideboob and threaten to spill out at any moment.  Judging by the flushed faces of some customers, this may have already happened.");
                }
            ...
            if (select == 4)
            {
                ...
                if (flags[kFLAGS.RUBI_BREAST_SIZE] >= 4)
                {
                    outputText("  In an odd sort of way, [rubi ey] looks like an adventurer who met the wrong end of a witch's curse.");
                }
            }
            ...
            if (select == 1)
            {
                ...
                if (rubiBimbo() && flags[kFLAGS.RUBI_DEBIMBO] == 0)
                {
                    ...
                    if (flags[kFLAGS.RUBI_NO_CUNT] == 0 && flags[kFLAGS.RUBI_COCK_SIZE] == 0)
                    {
                        outputText(" and wet cunt");
                    }
                    else if (flags[kFLAGS.RUBI_NO_CUNT] > 0 && flags[kFLAGS.RUBI_COCK_SIZE] > 0)
                    {
                        outputText(" and limp cock");
                    }
                    else
                    {
                        outputText(", wet cunt, and limp cock");
                    }
                    outputText(" to anyone behind [rubi em]");
                }
                else
                {
                    outputText("excited to see Rubi's choice of frilly pink panties clinging to [rubi eir] supple ass");
                }
                outputText(".");
            }
            //(Dashing Outfit)
            ...
            else if (select == 15)
            {
                ...
                if (rubiBimbo())
                {
                    outputText("  You even spy a wet patch in the crotch, with rivulets of moisture running down the legs, which [rubi ey] must have trouble hiding... if [rubi ey] even bothers to try.");
                }
            }
            //(Long Dress)
            else if (select == 7)
            {
                ...
                if (rubiBimbo() && flags[kFLAGS.RUBI_DEBIMBO] == 0)
                {
                    outputText("And then [rubi ey] begins to shake [rubi eir] ass, making spanking motions and effectively ruining the air of sophistication.");
                }
            }
            //(Nurse Uniform)
            else if (select == 9)
            {
                ...
                {
                    outputText("excited to see Rubi's choice of frilly white panties clinging to [rubi eir] supple ass");
                }
                outputText(".");
            }
            ...
            else if (select == 3)
            {
                ...
                {
                    outputText("excited to see a pair of translucent rubber panties clinging to [rubi eir] supple ass");
                }
                outputText(".");
            }
            //(Suitclothes)
            ...
            else if (select == 11)
            {
                ...
                {
                    outputText("excited to see a cute, little thong wedged into [rubi eir] supple ass");
                }
                outputText(".");
            }
            //(Bodysuit)
            else if (select == 6)
            {
                ...
                if (rubiBimbo())
                {
                    outputText("  You even spy a moist patch in the crotch, which must be impossible to hide... if [rubi ey] even bothers to try.");
                }
            }
            //(Bondage Straps)
            else if (select == 12)
            ...
            else if (select == 14)
            {
                ...
                {
                    outputText("excited to see a pair of pink frilly panties clinging to [rubi eir] supple ass");
                }
                outputText(".");
            }
        }
        ...
        if (flags[kFLAGS.RUBI_ADMITTED_GENDER] >= 1)
        {
            place = rubisFuckingHouseYouPervert;
        }
    ...
    public function buyRubiMilk() : Void
    {
        ...
        if (rubiAffection() >= 30 && flags[kFLAGS.RUBI_ADMITTED_GENDER] == 0)
        {
    ...
    public function rubisFuckingHouseYouPervert() : Void
    {
        ...
        //First Time:{
            ...
            if (flags[kFLAGS.TIMES_RUBI_MASSAGED] == 0)
            {
    ...
    //[Fuck Rubi (if player has cock, OR at least a 4</i>\" clit)] [Dildo Fuck (If player has Deluxe Dildo)] [Tease] [Release (Only if Normal or Incubus Rubi who has been teased)] [Titfuck (Bimbo Rubi only)] [Give Item]
    {
        ...
        addButton(4, "AnalTrain", anallyTrainYourBitchBoySlutHowToBeAnalForDCL).disableIf(player.cockThatFits(rubiCapacity()) < 0, "Req. a cock fitting " + rubiCapacity() + " capacity.").disableIf(flags[kFLAGS.RUBI_AFFECTION] < 75, "You don't know [rubi em] well enough!");
        if (flags[kFLAGS.RUBI_AFFECTION] >= 75 && flags[kFLAGS.RUBI_COCK_SIZE] >= 5)
        {
            addButton(5, "Get Fucked", getFuckedByRubi).disableIf(flags[kFLAGS.RUBI_COCK_SIZE] < 5, "Rubi doesn't have big enough cock!").disableIf(flags[kFLAGS.RUBI_AFFECTION] < 75, "You don't know [rubi em] good enough!");
        }
    ...
    public function fuckRubiRouter() : Void
    {
        ...
        {
            sceneHunter.callFitNofit(fuckRubi, rubiCapacity());
        }
    }
    ...
    public function fuckRubi(x : Int = -1) : Void
    {
        ...
        if (player.hasCock())
        {
            ...
            else if (player.cockArea(x) < rubiCapacity())
            {
                size = 1;
            }
            else
        ...
        else if (player.clitLength <= 12 || flags[kFLAGS.HYPER_HAPPY] != null)
        {
            size = 1;
        }
        else
        ...
        if (size == 0)
        {
            ...
            if (flags[kFLAGS.RUBI_COCK_SIZE] <= 5)
            {
                outputText("I guess we're in the same boat.</i>\"");
            }
            else if (rubiBimbo() && flags[kFLAGS.RUBI_DEBIMBO] == 0)
            {
                outputText("That's too bad, I was soooo looking forward to a real fucking.</i>\"");
            }
            else
            {
                outputText("That's pretty sad, getting shown up by a dainty little thing like me...</i>\"");
            }
        }
        else if (size == 1)
        {
        ...
        if (player.hasCock())
        {
            outputText("swollen member");
        }
        else
        {
            outputText("impressive love button");
        }
        ...
        if (player.hasCock())
        {
            outputText("your cockhead");
        }
        else
        {
            outputText("the tip of your clit");
        }
        outputText(", sending shivers of pleasure up your spine.");
        //(If Cock)
        if (player.hasCock())
        {
            outputText("  Amazingly, with the expert ministrations of your lover's mouth, you can't hold yourself in much longer, and begin to tense up, but at the last minute you pull yourself away from Rubi's grip and lips, halting yourself before you can cum.");
        }
        ...
        //(Normal/Incubus Rubi){
            ...
            if (rubiBimbo())
            {
                outputText(", not that [rubi ey] needs the extra lubrication, as [rubi ey]'s practically dripping like a faucet");
            }
            outputText(".");
        ...
        else
        {
            ...
            if (rubiBimbo())
            {
                outputText(", not that [rubi ey] needs the extra lubrication, as she's practically dripping like a faucet, which dribbles down [rubi eir] legs suitably enough");
            }
            outputText(".");
            ...
            if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
            {
                outputText("hard cock");
            }
            else
            {
                outputText("stiff clit");
            }
        ...
        if (flags[kFLAGS.RUBI_ORGASM_DENIAL] > 0 && !rubiBimbo())
        {
            outputText("  You give Rubi's ass a hard smack and remind [rubi em] that no matter what happens, [rubi ey] still can't cum yet.  [rubi Eir] eyes flutter open and [rubi ey] actually whimpers a little, but can't form the words to object.");
        }
        //(If Orgasm Denial is NOT activated; male/herm Rubi)
        else if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
        {
            outputText("  Rubi’s body shivers and trembles, and almost collapses.  [rubi Ey] grasps at the blankets with wild abandon as ropes of pearly white cum erupt from [rubi eir] cock, splattering against [rubi eir] chest and the blankets.");
        }
        //(Female Rubi)
        else
        {
            outputText("  Rubi's body shivers and trembles, and [rubi ey] almost collapses.  [rubi Ey] grasps at the blankets with wild abandon and gives and orgasmic scream as [rubi eir] cunt clenches and unclenches fiercely.  A spray of femjuices erupts outwards from [rubi em], coating your crotch and dripping down both yours and [rubi eir] legs.");
        }
        ...
        if (player.hasCock() && player.cockArea(x) <= rubiCapacity())
        {
            ...
            if (player.cumQ() < 250)
            {
                outputText("Cum dribbles out from [rubi eir] hole as you give a couple extra thrusts.");
            }
            else if (player.cumQ() < 1000)
            {
                outputText("A generous amount of cum trickles from [rubi eir] depths as you give a couple extra thrusts.");
            }
        ...
        else if (player.hasCock())
        {
            ...
            if (player.cumQ() < 250)
            {
                outputText("with a fair amount of your seed.");
            }
            else if (player.cumQ() < 1000)
            {
                outputText("with a generous amount of your spunk.");
            }
            else
            {
                outputText("with a veritable torrent of potent pearlescent spooge.");
            }
        }
        //(Player has clit?)
        else
        {
            outputText("  Your " + clitDescript() + " throbs and engorges briefly as your " + vaginaDescript(0) + " reflexively contracts and a warm feeling rocks through your entire body.  Femjuices run down your [legs] and relief overtakes you like a powerful wave upon a beach.");
        }
        ...
        if (rubiBimbo() && flags[kFLAGS.RUBI_DEBIMBO] == 0)
        {
            outputText("like, totally ");
        }
        else
        {
            outputText("absolutely ");
        }
        ...
        doNext(camp.returnToCampUseOneHour);
        rubiAffection(1);
        if (flags[kFLAGS.RUBI_ORGASM_DENIAL] > 0)
        {
    ...
    public function dildoFuckRubi() : Void
    {
        ...
        if (!player.hasCock())
        {
            outputText("You don't have the equipment to give Rubi a good fucking, but there is one thing you do have.");
        }
        //(If PC has cock)
        else
        {
            outputText("While you do have the requisite equipment to give Rubi a good fucking, you'd much rather use something else.");
        }
        //(Add)
        ...
        if (player.hasVagina())
        {
            outputText("You press the tip into your cunt quickly, giving it a taste of what is to come.  The brief contact is enough to make you shudder slightly with pleasure.  ");
        }
        //(If PC has fuckable nipples)
        if (player.hasFuckableNipples())
        {
            outputText("You let the dildo meander up your body until it reaches your breasts, and with a bit of a grin you insert the tip right into one nipple, enjoying the feeling of your breasts swell up with arousal.  You do the same with the other nipple, and take a minute going back and forth.  ");
        }
        ...
        if (!rubiBimbo())
        {
            outputText("They part shakily and [rubi eir] tongue reaches out to taste the dildo.  ");
        }
        //(Bimbo Rubi)
        else
        {
            outputText("They easily part, and [rubi eir] tongue takes a long slurp, just to taste the dildo.  ");
        }
        ...
        if (flags[kFLAGS.RUBI_NO_CUNT] == 1)
        {
            outputText("against [rubi eir] tight ass");
        }
        else
        {
            outputText("against [rubi eir] tight pussy");
        }
        ...
        if (player.hasVagina())
        {
            outputText("  Your " + clitDescript() + " stiffens, aching for attention.  It peeks from your folds like a shy little lewd faerie.");
        }
        //(If player has a cock)
        if (player.hasCock())
        {
            outputText("  Meanwhile, [eachCock] stiffens, throbbing as the drug overtakes it.");
        }
        if (player.lactationQ() > 0)
        {
            outputText("  As the warmth spreads up your body, not even your breasts are left alone.  Your nipples harden and begin leaking milk, which pours down the sides of your body and is soaked up into the cushions beneath you.");
        }
        ...
        else if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
        {
            ...
            if (!rubiIncubus())
            {
                outputText("little cock");
            }
            else
            {
                outputText("demonic cock");
            }
        ...
        if (player.hasCock())
        {
            outputText("[eachCock], stroking it fervently, in desperate need of release.");
        }
        //(PC doesn't have cock, but has pussy?)
        else if (player.hasVagina())
        {
            outputText("your " + clitDescript() + ", rubbing it frantically, in desperate need of release.");
        }
        //(PC doesn't have either, but breasts above A-cup?)
        else
        {
            outputText("your [chest], rubbing your nipples vigorously, in desperate need of release.");
        }
        //(Bonus, fuckable nipples?)
        if (player.hasFuckableNipples())
        {
            outputText("  Your fingers easily slip inside each nipple, pumping in and out like little cocks.");
        }
        //(No cock, no pussy, no breasts?)
        if (player.gender == 0 && player.biggestTitSize() == 0)
        {
            outputText("  Your hands roam all along your body, from your bare crotch to your nipples, even through your hair, all in search of something to give you the release you so desperately crave.");
        }
        ...
        if (player.hasCock())
        {
            ...
            if (player.cumQ() < 250)
            {
                outputText("splattering messily");
            }
            else if (player.cumQ() < 500)
            {
                outputText("coating you lightly");
            }
            else if (player.cumQ() < 1000)
            {
                outputText("covering you in a copious layer of cum");
            }
            else
            {
                outputText("covering you in a thick layer of your potent seed");
            }
            outputText(".");
        }
        //(PC has pussy?)
        if (player.hasVagina())
        {
            outputText("  Your " + vaginaDescript(0) + " clenches down on the enormous invader, which at this point feels like an enormous steel rod lodged firmly inside you, and a spray of fem-juices erupts from your nethers, coating the already slick dildo and Rubi.");
        }
        //(PC lactating?)
        if (player.lactationQ() > 0)
        {
            outputText("  Your breasts shudder and quake as the orgasmic tsunami washes over them, spraying your sweet smelling milk all over yourself, Rubi and the couch.");
        }
    ...
    public function teaseRubi() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
        {
            ...
            if (flags[kFLAGS.RUBI_SKIN] == 0)
            {
                outputText("pale");
            }
            else if (flags[kFLAGS.RUBI_SKIN] == 2)
            {
                outputText("fur-covered");
            }
            else
            {
                outputText("red");
        ...
        else
        {
            ...
            else if (flags[kFLAGS.RUBI_SKIN] == 1)
            {
                outputText("crimson");
            }
            else if (flags[kFLAGS.RUBI_SKIN] == 2)
    ...
    public function releaseRubi() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
        {
            outputText("[rubi eir] [rubi cock] straining at the seams, trying to break free.");
        }
        else
        {
            outputText("clinging tight to [rubi eir] flat little mound.");
        }
        ...
        if (flags[kFLAGS.RUBI_BLU_BALLS] <= 3)
        {
            ...
            if (flags[kFLAGS.RUBI_BALLS_TYPE] == 1)
            {
                outputText(", trapped in their little sissy sack");
            }
        ...
        {
            outputText("  Not to mention [rubi eir] sizeable balls, which seem to have swelled up to twice their normal size, thanks to your relentless teasing.  They even have a slight bluish tinge to them, although it's not immediately visible.");
        }
        else
        {
            outputText("  Not to mention [rubi eir] enormous balls, which, thanks to your endless teasing, are seemingly the size of baseballs.  They even have a noticeable blue tinge to them, obviously aching for relief.");
        }
        ...
        {
            outputText("slightly blue");
        }
        else
        {
            outputText("incredibly swollen blue");
        }
        ...
        if (flags[kFLAGS.RUBI_BLU_BALLS] <= 3)
        {
            outputText("  It's quickly followed by another, less forceful jet of pearly white cum, which lands on [rubi eir] face, and several even smaller jets which splatter the area around [rubi em].  [rubi Eir] cock twitches and spasms, then slowly softens.");
        }
        //(Blueballs 4-5)
        else if (flags[kFLAGS.RUBI_BLU_BALLS] <= 5)
        {
            outputText("  It's quickly followed by another, less forceful jet of pearly white cum, then another, and another, each landing closer to [rubi eir] face.  Finally, it gives a few more token jerks, spurting out tiny globules of cum onto the surrounding area.  [rubi Eir] cock twitches and spasms, squirting nothing but air, as if [rubi eir] body is still craving release.");
        }
        //(Blueballs 6-7)
        else if (flags[kFLAGS.RUBI_BLU_BALLS] <= 7)
        {
            outputText("  It's quickly followed by another, more forceful jet of pearly white cum which splatters against the ceiling, dripping down onto both of you.  Another spurt of cum bursts forth, then another, and another, each one liberally covering Rubi's stomach, chest and face.  [rubi Eir] cock finally gives a few last twitches and spasms, squirting nothing but air, as if [rubi eir] body is still craving release.");
        }
        //(Blueballs 8-9)
        else if (flags[kFLAGS.RUBI_BLU_BALLS] <= 9)
        {
            outputText("  It's quickly followed by another, more forceful jet of pearly white cum, which splatters against the ceiling, dripping down onto both of you, and then another, and another.  It isn't long before the ceiling has a huge white cum-covered blotch right above you, drizzling down on you like dew dripping from trees.  Rubi's cock doesn't stop there, however, it continues twitching and spurting [rubi eir] seed until [rubi ey] is completely covered in the stuff.  Finally, mercifully, it ends, though [rubi eir] [rubi cock] continues to spasm, squirting nothing but air, as if [rubi eir] body is still craving release.");
        }
        //(Blueballs 10+)
        else
        {
            outputText("  It's quickly followed by another, more powerful jet of pearly white cum, which splatters against the ceiling, dripping down onto both of you, and then another, and another.  It isn't long before the ceiling has a huge white cum-covered blotch right above you, drizzling down on you like dew dripping from the trees.  Rubi's cock doesn't stop there, however, it continues twitching and spurting [rubi eir] seed, liberally covering [rubi em]self, you and the surrounding area until everything appears to be in a white haze.");
        }
        ...
        //(Blueballs 1-5)
        if (flags[kFLAGS.RUBI_BLU_BALLS] <= 5)
        {
            outputText("You clean yourself up, leave Rubi a little \"<i>See you next time</i>\" note, and head out the door, back to camp.");
        }
        //(Blueballs 6-9)
        else if (flags[kFLAGS.RUBI_BLU_BALLS] <= 9)
        {
            outputText("You head into the bathroom and wash the cum off you, taking a short shower.  Afterwards, you dry up, leave Rubi a little \"<i>See you next time</i>\" note, and head out the door, back to camp.");
        }
        //(Blueballs 10+)
        else
        {
            outputText("You head into the bathroom and wash the cum off you, no easy feat to be sure.  Afterwards, you dry up, leave Rubi a little \"<i>See you next time</i>\" note, and head out the door, back to camp.  You certainly don't want to be there when [rubi ey] wakes up, lest you have to clean the entire house with [rubi em].");
        }
    ...
    private function waitAndGetNTRedLikeTheBoyBitchYouAre() : Void
    {
        ...
        if (silly())
        {
            outputText("They're even doing positions that aren't available in the game!  ");
        }
    ...
    private function chocoRubiMassage() : Void
    {
        ...
        if (player.cor >= 66)
        {
            outputText("start sensually undressing, turning the simple act into a seductive show that causes Rubi's petite boner to jerk unpredictably beneath [rubi eir] clothes.  A small, damp stain appears at the tip by the time you finish your salacious tease.");
        }
        else if (player.cor >= 33)
        {
            outputText("get undressed without much pause or fanfare.  Rubi makes a show of turning [rubi eir] head, but you can see [rubi em] peeping glimpses out of the corners of [rubi eir] eyes, and [rubi eir] boner seems to be bigger and harder than it was moments before.");
        }
        else
        {
            ...
            if (player.biggestCockArea() >= 40 || player.biggestTitSize() >= 8)
            {
                outputText(", not that it does much good");
            }
            outputText(".");
        }
        ...
        else if (flags[kFLAGS.RUBI_BLU_BALLS] < 8)
        {
            outputText("big");
        }
        else
        {
            outputText("huge, stretched");
        }
    ...
    private function releaseRubiMassage() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_BLU_BALLS] <= 7)
        {
            sceneHunter.print("You can do better!");
        }
        flags[kFLAGS.RUBI_ORGASM_DENIAL] = 0;
        flags[kFLAGS.RUBI_BLU_BALLS] = 0;
        doNext(camp.returnToCampUseTwoHours);
    ...
    private function rubiAppearance() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_SKIN] == 0)
        {
            outputText("pale, almost like porcelain");
        }
        else if (flags[kFLAGS.RUBI_SKIN] == 1)
        {
            outputText("an exotic cherry red");
        }
        else if (flags[kFLAGS.RUBI_SKIN] == 2)
        {
            outputText("covered with a fine layer of silky-smooth pink fur");
        }
        else
        {
            outputText("an exotic cherry red which contrasts well with a grey belly and stripes that run up and down [rubi eir] legs and arms");
        }
        
        if (flags[kFLAGS.RUBI_SKIN] != 2)
        {
            outputText(" and completely hairless, except for [rubi eir]");
        }
        else
        {
            outputText(", and [rubi ey] sports");
        }
        
        if (flags[kFLAGS.RUBI_HAIR_LENGTH] == 0)
        {
            outputText(" short");
        }
        else if (flags[kFLAGS.RUBI_HAIR_LENGTH] == 1)
        {
        ...
        if (flags[kFLAGS.RUBI_WHISKERS] > 0)
        {
            outputText(", with a cute set of whiskers sprouting from [rubi eir] cheeks");
        }
        outputText(", [rubi eir] lips painted a ");
        //(normal/furry skin)
        if (flags[kFLAGS.RUBI_SKIN] == 0 || flags[kFLAGS.RUBI_SKIN] == 2)
        {
            outputText("crimson red");
        }
        //(red/siren skin)
        else
        {
        ...
        if (flags[kFLAGS.RUBI_EYE_TYPE] == 0 || rubiBimbo())
        {
            outputText("green");
        }
        ...
        if (flags[kFLAGS.RUBI_HORNTYPE] > 0)
        {
            outputText("Two tiny horns extend upwards from [rubi eir] hairline, solid black in color.  ");
        }
        outputText("A pair of ");
        if (flags[kFLAGS.RUBI_EAR_TYPE] == 0)
        {
            outputText("cute, normal ears protrude from the sides of [rubi eir] head, each one pierced with a small dangling earring");
        }
        else if (flags[kFLAGS.RUBI_EAR_TYPE] == 1)
        {
            outputText("cute, cat-like ears pop up from the top of [rubi eir] head, each one seemingly able to move independently");
        }
        else if (flags[kFLAGS.RUBI_EAR_TYPE] == 2)
        {
            outputText("feathered ears protrude from the sides of [rubi eir] head, each one fluttering cutely from time to time, and each pierced with a small dangling earring");
        }
        outputText(".");
        ...
        if (flags[kFLAGS.RUBI_NIPPLE_TYPE] == 1)
        {
            outputText("black ");
        }
        outputText("nipples.");
        ...
        if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
        {
            ...
            if (flags[kFLAGS.RUBI_BLU_BALLS] >= 4 && flags[kFLAGS.RUBI_BLU_BALLS] < 9)
            {
                outputText("cum-swollen ");
            }
            else if (flags[kFLAGS.RUBI_BLU_BALLS] >= 9)
            {
                outputText("engorged, blue-tinged ");
            }
            outputText("testicles");
            if (flags[kFLAGS.RUBI_BALLS_TYPE] == 1)
            {
                outputText(", which cling tight to [rubi eir] body in a cute spherical pouch");
            }
        }
        if (flags[kFLAGS.RUBI_COCK_SIZE] > 0 && flags[kFLAGS.RUBI_NO_CUNT] == 0)
        {
            outputText(" and a ");
        }
        if (flags[kFLAGS.RUBI_NO_CUNT] == 0)
        {
            outputText("cute ");
            if (flags[kFLAGS.RUBI_CUNTTYPE] > 0)
            {
                outputText("black ");
            }
        ...
        if (flags[kFLAGS.RUBI_FEET] == 1)
        {
            outputText("Finally, Rubi stands on a long black spike that makes [rubi em] walk as though [rubi ey] were wearing high heels.");
        }
    ...
    private function getCloset() : Array<Dynamic>
    {
        return [
        ...
        ["A Risque Waitress's Uniform", "RisqueUniform", flags[kFLAGS.RUBI_AFFECTION] >= 100, putOnAWaitressUniformYouWhore, null]
    ];
    }
    ...
    private function playDressUp() : Void
    {
        ...
        var closet : Array<Dynamic> = getCloset();
        ...
        for (item in closet)
        {
            if (Reflect.field(item, Std.string(2)) != null)
            {
                outputText(Reflect.field(item, Std.string(0)) + "\n");
                addButton(btn++, Reflect.field(item, Std.string(1)), Reflect.field(item, Std.string(3)));
            }
        }
    ...
    private function putRubiInAWaitressUniform() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_BREAST_SIZE] < 1)
        {
            outputText(", though it's obvious [rubi ey] doesn't need it with [rubi eir] flat-chest");
        }
        ...
        if (flags[kFLAGS.RUBI_BREAST_SIZE] < 1)
        {
            outputText(", showing off some of [rubi eir] flat chest");
        }
        else
        {
            outputText(", showing off a generous portion of cleavage");
        }
        outputText(", and tucks the hem of the blouse into the skirt.  Finally, Rubi selects a pair of ");
        if (flags[kFLAGS.RUBI_FEET] == 1)
        {
            outputText("what appears to be flats, with a hole in the heel for [rubi eir] demonic heels");
        }
        else
        {
            outputText("short heels");
        }
        ...
        if (!rubiBimbo() || flags[kFLAGS.RUBI_DEBIMBO] > 0)
        {
            outputText("girly panties");
        }
        else
        {
            outputText("naked nethers");
        }
        outputText(".");
    ...
    private function putOnADashingOutfitYouWhore() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
        {
            outputText("tucking [rubi eir] cock firmly into the triangular strip of cloth covering [rubi eir] crotch, while ");
        }
        ...
        if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
        {
            outputText(", not to mention the sizeable package in [rubi eir] crotch");
        }
        outputText(".");
        ...
        if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
        {
            outputText("  Combined with the sizeable bulge in the crotch, Rubi looks like quite the dashing, if slutty, piratess.");
        }
    ...
    private function goOnAnAnalAdventureRubiNotReallyJustAnAdventurersOutfit() : Void
    {
        ...
        if (silly())
        {
            outputText("collector of waifus!");
        }
        else
        {
            outputText("layer of ladies");
        }
        ...
        {
            outputText(", covering [rubi eir] feline ears");
        }
        outputText(".  The boots that go with the outfit are mid-calf length and made of supple, fine leather");
        if (flags[kFLAGS.RUBI_FEET] == 1)
        {
            outputText(" with a hole cut out in the heel, apparently for Rubi's demonic heels to fit through");
        }
    ...
    private function giveRubiALongDressToHideHerHideousBody() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_BREAST_SIZE] < 1)
        {
            outputText(", despite the fact that [rubi ey] clearly doesn't need one");
        }
        ...
        if (flags[kFLAGS.RUBI_SKIN] == 1 || flags[kFLAGS.RUBI_SKIN] == 3)
        {
            outputText(" (or darkens at least)");
        }
        ...
        if (flags[kFLAGS.RUBI_FEET] == 1)
        {
            outputText(" with the heels cut off and a hole for Rubi's demonic heel to fit through");
        }
    ...
    private function rubiPutsOnNursesClothesSoSheCanCheckYourTemperature() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_FEET] > 0)
        {
            outputText(" with the heels removed and a hole for [rubi eir] demonic heels");
        }
        outputText(".");
    ...
    private function rubiHasARubberFetish() : Void
    {
        ...
        if (!rubiBimbo() || flags[kFLAGS.RUBI_DEBIMBO] > 0)
        {
            outputText("[rubi Ey] first takes a pair of transparent rubber panties and slides them up [rubi eir] legs until they lovingly cling around [rubi eir] ass.  ");
        }
        ...
        if (flags[kFLAGS.RUBI_FEET] == 1)
        {
            outputText(", the actual heel cut off to make room for [rubi eir] demonic heel,");
        }
    ...
    private function putOnASuitYouSlut() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_FEET] == 1)
        {
            outputText(", with the actual heel cut off to make room for [rubi eir] demonic heel,");
        }
    ...
    private function putOnATubeTopYouWhore() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_BREAST_SIZE] > 2)
        {
            outputText(", which strain the flimsy piece of material");
        }
        outputText(".  Completing the look is a simple pair of sandals");
        if (flags[kFLAGS.RUBI_FEET] == 1)
        {
            outputText(", with a hole cut in the sole to make room for Rubi's demonic heels");
        }
        outputText(".");
        outputText("\n\nNow that Rubi is dressed up, what do you want to do?");
    ...
    private function putOnSluttySwimwearYouSkank() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_BREAST_SIZE] < 3)
        {
            outputText(" sit loosely over Rubi's small chest");
        }
        else
        {
            outputText("strain thanks to Rubi's ample busom");
        }
        outputText(".");
        ...
        if (flags[kFLAGS.RUBI_FEET] > 0)
        {
            outputText(", with a hole in the sole to make room for [rubi eir] demonic heels");
        }
        outputText(".");
    ...
    private function putOnADressYouBimbo() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_FEET] > 0)
        {
            outputText(", with the heels removed to make room for [rubi eir] own demonic heels");
        }
        outputText(".");
    ...
    private function putOnMyBodysuitYouWhore() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
        {
            outputText("[rubi eir] package is neatly tucked away");
        }
        else
        {
            outputText("a red rose is neatly covering up [rubi eir] pubic mound");
        }
    ...
    private function putOnAWaitressUniformYouWhore() : Void
    {
        ...
        if (!rubiBimbo() || flags[kFLAGS.RUBI_DEBIMBO] > 0)
        {
            ...
            if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
            {
                outputText("Oddly enough, these panties only seem to cover Rubi's balls, leaving [rubi eir] cock right out in the open.  ");
            }
        }
        ...
        if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
        {
            outputText("  To top it all off, Rubi takes a pink lacy cocksock and slips it down [rubi eir] [rubi cock], fastening it into place with a pink ribbon that [rubi ey] ties into a bow around the base of [rubi eir] cock.");
        }
    ...
    private function fancyDate() : Void
    {
        ...
        if (player.gems >= 10)
        {
            ...
            {
                outputText(" though not as good as Callu's, the ottergirl fisherman you met at the lake.");
            }
            else
            {
                outputText(", which makes you wonder who Rubi is thinking about.  Perhaps one of [rubi eir] parents?");
            }
    ...
    private function exhibitionistDate() : Void
    {
        ...
        {
            outputText("glistening, wet pussy");
        }
        outputText(".");
    ...
    {
        var x1 : Int = as3hx.Compat.parseInt(player.cockThatFits(rubiCapacity()) + 1);
        clearOutput();
        rubiSprite();
        ...
        if (flags[kFLAGS.RUBI_TIMES_ANALLY_TRAINED] == 1)
        {
            ...
            if (flags[kFLAGS.RUBI_SKIN] == 1 || flags[kFLAGS.RUBI_SKIN] == 3)
            {
                outputText(", or redder at least");
            }
            else
            {
                outputText(", what little of it you can see");
            }
            ...
            if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
            {
                outputText(", [rubi eir] dick wobbling enticingly");
            }
            outputText(".");
            ...
            {
                outputText("quickly dampening pussy");
            }
            outputText(".");
            ...
            if (flags[kFLAGS.RUBI_COCK_SIZE] == 0)
            {
                outputText("slick vagina");
            }
            else
            {
                outputText("hungry asshole");
            }
            outputText(".");
            ...
            if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
            {
                outputText(rubiCock() + " is standing at full attention, bobbing up and down as Rubi fucks [rubi emself] on the dildo");
            }
            if (flags[kFLAGS.RUBI_COCK_SIZE] > 0 && flags[kFLAGS.RUBI_NO_CUNT] == 0)
            {
                outputText(", and [rubi eir] ");
            }
            if (flags[kFLAGS.RUBI_NO_CUNT] == 0)
            {
                outputText("cunt lips are glistening red with arousal, just begging for attention");
            }
            ...
            {
                outputText("feline ear");
            }
            else
            {
                outputText("feathered ear");
            }
        ...
        {
            //Must have a cock greater than Rubi's capacity.  //Rubi's capacity won't go larger than 100.  {
                ...
                if (flags[kFLAGS.RUBI_SKIN] == 0)
                {
                    outputText("go bright red");
                }
                else if (flags[kFLAGS.RUBI_SKIN] == 1 || flags[kFLAGS.RUBI_SKIN] == 3)
                {
                    outputText("go a vivid purple");
                }
                else
                {
                    outputText("would be blushing bright red now, were it not for the fine layer of fur");
                }
                ...
                if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
                {
                    outputText("You can feel your lover's cock straining against your leg, aching for some release.  You idly press your foot against it, feeling a slick layer of precum coating it from top to bottom already.  ");
                }
                ...
                if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
                {
                    outputText("  Even, no, <i>especially</i> [rubi eir] cock twitches and spasms jerking upwards and splattering globules of sticky white goo against the inside of Rubi's babydoll before dripping down onto the floor between [rubi eir] legs, creating a rapidly growing puddle.");
                }
    ...
    private function getFuckedByRubi() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_EYE_TYPE] > 0)
        {
            outputText(" slitted");
        }
        ...
        if (player.hasCock())
        {
            outputText(", ignoring your [cocks] completely,");
        }
        ...
        {
            outputText("pink, mushroom-like tip");
        }
        else if (this.rubiGetCockType() == CockTypesEnum.DEMON)
        {
            outputText("purple, nodule-laden tip");
        }
        else if (this.rubiGetCockType() == CockTypesEnum.HORSE)
        {
            outputText("blunted, musky tip");
        }
        else if (this.rubiGetCockType() == CockTypesEnum.CAT)
        {
            outputText("spiked tip");
        }
        else if (this.rubiGetCockType() == CockTypesEnum.ANEMONE)
        {
            outputText("tentacle-ringed crown");
        }
        else
        {
            outputText("<b>ERROR: Rubi Cock Type set invalid. Currently: " + this.rubiGetCockType() + "</b>  ");
        }
        ...
        if (!player.hasVagina())
        {
            player.buttChange(flags[kFLAGS.RUBI_COCK_SIZE] * flags[kFLAGS.RUBI_COCK_SIZE] / 6, true, true, false);
        }
        else
        {
            player.cuntChange(flags[kFLAGS.RUBI_COCK_SIZE] * flags[kFLAGS.RUBI_COCK_SIZE] / 6, true, true, false);
        }
        ...
        {
            outputText("Your insides rumble dangerously and your stomach actually swells up, gaining a noticeable paunch as Rubi's lavish load coats your insides, filling every available nook and cranny and even creating new ones.  What doesn't fit slips out of your abused hole in a flood, coating the bed and floor beneath you in a slick, white glaze.  ");
        }
    ...
    private function rubiHotdogging() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_SKIN] == 0)
        {
            outputText("pale skinned");
        }
        else if (flags[kFLAGS.RUBI_SKIN] == 1 || flags[kFLAGS.RUBI_SKIN] == 3)
        {
            outputText("red skinned");
        }
        else
        {
            outputText("fur-covered");
        ...
        if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
        {
            outputText("taking hold of [rubi eir] cock");
        }
        else
        {
            outputText("rubbing a finger along [rubi eir] clit");
        }
        ...
        if (flags[kFLAGS.RUBI_SKIN] != 2)
        {
            outputText("tender skin");
        }
        else
        {
            outputText("silky fur");
        }
        ...
        if (flags[kFLAGS.RUBI_SKIN] == 0)
        {
            outputText("pale skin");
        }
        else if (flags[kFLAGS.RUBI_SKIN] == 1 || flags[kFLAGS.RUBI_SKIN] == 3)
        {
            outputText("crimson skin");
        }
        else
        {
            outputText("fur-covered flesh");
        }
    ...
    private function teaseButtjobs() : Void
    {
        ...
        if (player.cumQ() <= 50)
        {
            outputText("in one go, with a small, proud look on [rubi eir] face");
        }
        else if (player.cumQ() <= 150)
        {
            outputText("with one, throat-straining swallow, a small, proud smile taking [rubi eir] face when [rubi ey] stops panting for breath");
        }
        else
        {
            outputText("as it comes, mouthful after mouthful of cum practically injected straight into [rubi eir] stomach. There's so much of it, [rubi ey] can't help but smile mid-way through, proud to take such a huge load");
        }
        if (player.cumQ() >= 1000)
        {
            outputText(".  After guzzling down so much, you're certain Rubi's actually sporting a bit of a belly now, almost as if [rubi ey] were several months pregnant");
        }
        outputText(".");
    ...
    private function popButtjobs() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_NO_CUNT] == 0)
        {
            outputText("cunt clamps down on your thumb and gushes, coating your hand in [rubi eir] slick juices");
        }
        else
        {
            outputText("asshole quivers and attempts to suck in more of your thumb before clamping down, leaking slick fluids all over your hand");
        }
        ...
        if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
        {
            outputText("  Beneath [rubi em], [rubi eir] cock jerks and twitches, loosing a spray of pearly white cum which splatters against the couch below");
        }
        if (flags[kFLAGS.RUBI_BLU_BALLS] < 4)
        ...
        {
            outputText(" which is quickly followed by another, more powerful stream of cum, which splashes against [rubi eir] stomach, coating [rubi eir] [rubi breasts] in semen");
        }
        else if (flags[kFLAGS.RUBI_BLU_BALLS] <= 7)
        {
            outputText(" which is quickly followed by another, more powerful stream of cum, and a third, and a fourth.  Before you know it, Rubi's entire torso, including [rubi eir] [rubi breasts] are practically glazed with cum");
        }
        else if (flags[kFLAGS.RUBI_BLU_BALLS] <= 9)
        {
            outputText(" which is followed up by several more, each seemingly more poweful than the one previous.  Rubi's body twitches and shudders with every jet, until [rubi eir] entire torso, and the couch, is covered in the musky jizz");
        }
        else
        {
            outputText(" which is followed by a dozen more, each one thicker and more powerful than the last.  Rubi's cock twitches uncontrollably, just like [rubi eir] body, as it coats the entire couch, [rubi eir] entire upper body, and even some of the wall five feet away in the musky stuff");
        }
        outputText(".");
    ...
    private function giveRubiATFItem(itype : ItemType) : Void
    {
        ...
        if (itype == consumables.SUCMILK || itype == consumables.P_S_MLK)
        {
            ...
            if (flags[kFLAGS.RUBI_BREAST_SIZE] < 7 || (flags[kFLAGS.RUBI_BREAST_SIZE] < 30 && flags[kFLAGS.HYPER_HAPPY] != null))
            {
            ...
            if ((flags[kFLAGS.RUBI_COCK_SIZE] > 4) && (flags[kFLAGS.HYPER_HAPPY] == null))
            {
            ...
            if (rand(3) == 0 && flags[kFLAGS.RUBI_SKIN] != 1 && itype == consumables.SUCMILK)
            {
                ...
                {
                    outputText("coat of fur");
                }
                outputText(".  The blotches soon spread, rapidly overtaking [rubi eir] body.  ");
                if (flags[kFLAGS.RUBI_SKIN] == 2)
                {
                    outputText("Meanwhile the silky soft fur that covered [rubi eir] body seems to be falling out as this red menace takes over.  ");
                }
        ...
        //Incubus Draft{
            ...
            if (itype == consumables.INCUBID)
            {
                outputText("You hand over the bottle of milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the label before pulling out the cork and downing the whole bottle in one gulp.");
            }
            //Purified Incubus Draft
            else
            {
                //Decrease bust size by 1. (See Incubus Draft)  //Increase penis size by 1 (will grow a penis if none exists). (See Incubus Draft)    //Random chance to remove vagina. (See Incubus Draft)  outputText("You hand over the bottle of purified milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the pink label before pulling out the cork and downing the whole bottle in one gulp.");
            }
            
            //(If Bust size decreases)
            if ((flags[kFLAGS.RUBI_BREAST_SIZE] > 0) && (flags[kFLAGS.HYPER_HAPPY] == null))
            {
            ...
            if (flags[kFLAGS.RUBI_COCK_SIZE] > 0 && ((flags[kFLAGS.RUBI_COCK_SIZE] < 12 || (this.rubiGetCockType() == CockTypesEnum.HORSE && flags[kFLAGS.RUBI_COCK_SIZE] < 20)) ||
                (flags[kFLAGS.RUBI_COCK_SIZE] < 30 && flags[kFLAGS.HYPER_HAPPY] != null)))
            {
        ...
        {
            //Can choose Boobs or Penis. Grow that body part by 2.if (itype == consumables.GROPLUS)
            {
            ...
            else if (itype == consumables.BIMBOLQ)
            {
                ...
                if (flags[kFLAGS.RUBI_COCK_SIZE] > 0)
                {
                    ...
                    if (flags[kFLAGS.RUBI_NO_CUNT] == 0)
                    {
                        outputText("[rubi eir] vagina");
                    }
                    else
                    {
                        outputText("a pristine new vagina");
                    }
                ...
                if (flags[kFLAGS.RUBI_BREAST_SIZE] < 7)
                {
                    ...
                    if (flags[kFLAGS.RUBI_BREAST_SIZE] == 0)
                    {
                        outputText("  Within seconds, Rubi has the chest of a pubescent girl, within a minute they're at least a B-cup.");
                    }
            ...
            {
                //Gives horsecock.  //Grows cock. (As Incubus Draft above)  if (itype == consumables.EQUINUM)
                {
                ...
                //Decrease bust size by 1 if above 1. (See Incubus Draft){
                    ...
                    {
                        outputText("  Rubi's cunt glistens, soaked through entirely thanks to your ministrations");
                    }
                    if (flags[kFLAGS.RUBI_BREAST_SIZE] > 0)
                    {
                        outputText(", while [rubi eir] [rubi breasts] gently rock up and down with the motions of [rubi eir] breathing");
                    }
                    outputText(".");
                    ...
                    //(If Bust size decreases)
                    if ((flags[kFLAGS.RUBI_BREAST_SIZE] > 1) && (flags[kFLAGS.HYPER_HAPPY] == null))
                    {
                        outputText("\n\nRubi gasps a little and [rubi eir] hands immediately dart to [rubi eir] chest.  Before your eyes, Rubi's breasts begin to dwindle, shrinking down an entire cup size.  Rubi cups [rubi eir] breasts experimentally, getting used to their new, reduced, weight.");
                        flags[kFLAGS.RUBI_BREAST_SIZE]--;
                        if (flags[kFLAGS.RUBI_BREAST_SIZE] < 1)
                        {
                            flags[kFLAGS.RUBI_BREAST_SIZE] = 1;
                        }
                    }
                    //(If Bust size increases)
                    else if (flags[kFLAGS.RUBI_BREAST_SIZE] < 1)
                    ...
                    //(If Penis size decreases)
                    else if ((flags[kFLAGS.RUBI_COCK_SIZE] > 3) && (flags[kFLAGS.HYPER_HAPPY] == null))
                    {
                        outputText("\n\nRubi's [rubi cock] swells up suddenly, growing painfully hard.  [rubi Ey] grasps it and moans suddenly as it begins to dwindle in size, shrinking down by at least an inch, leaving [rubi em] with a ");
                        flags[kFLAGS.RUBI_COCK_SIZE]--;
                        if (flags[kFLAGS.RUBI_COCK_SIZE] < 3)
                        {
                            flags[kFLAGS.RUBI_COCK_SIZE] = 3;
                        }
                        outputText(Math.round(flags[kFLAGS.RUBI_COCK_SIZE] * 10) / 10 + "-inch penis.");
                    }
                    //(Trappy ballsack)
                ...
                {
                    //Progression of furriness: cat ears, then whiskers and paws, then cat penis, then furry body.if (itype == consumables.W_FRUIT)
                    {
        ...
        //Requires 5 Purity Peaches{
            ...
            {
                outputText("furred skin");
            }
            else
            {
                outputText("striped skin");
            }
            ...
            if (flags[kFLAGS.RUBI_EAR_TYPE] != 2 || flags[kFLAGS.RUBI_SKIN] != 3 || (flags[kFLAGS.RUBI_COCK_SIZE] > 0 && this.rubiGetCockType() != CockTypesEnum.ANEMONE))
            {
                ...
                else if (flags[kFLAGS.RUBI_SKIN] != 3)
                {
                    ...
                    if (flags[kFLAGS.RUBI_SKIN] == 0)
                    {
                        outputText("its normal paleness replaced with a vibrant cherry red");
                    }
                    else if (flags[kFLAGS.RUBI_SKIN] == 1)
                    {
                        outputText("which is barely noticeable on [rubi eir] already cherry red flesh");
                    }
                    else if (flags[kFLAGS.RUBI_SKIN] == 2)
                    {
                        outputText("the fur rapidly retreating and replaced with bright red flesh underneath");
                    }
                ...
                else if (flags[kFLAGS.RUBI_COCK_SIZE] > 0 && this.rubiGetCockType() != CockTypesEnum.ANEMONE)
                {
                    ...
                    if (this.rubiGetCockType() != CockTypesEnum.HUMAN)
                    {
                        outputText(", first shifting towards a more human-like appearance");
                    }
    ...
    private function rubiGrowPlusBreasts() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_BREAST_SIZE] < 7 || (flags[kFLAGS.RUBI_BREAST_SIZE] < 30 && flags[kFLAGS.HYPER_HAPPY] != null))
        {
            ...
            // trace("Hyper happy grow");{
                ...
                if (flags[kFLAGS.RUBI_BREAST_SIZE] > 30)
                {
                    flags[kFLAGS.RUBI_BREAST_SIZE] = 30;
                }
            }
            // trace("Normal grow");
            else
    ...
    private function rubiPenisGroPlus() : Void
    {
        ...
        //(If Penis increase)
        if (flags[kFLAGS.RUBI_COCK_SIZE] < 12 || (this.rubiGetCockType() == CockTypesEnum.HORSE && flags[kFLAGS.RUBI_COCK_SIZE] < 20) || (flags[kFLAGS.RUBI_COCK_SIZE] < 30 && flags[kFLAGS.HYPER_HAPPY] != null))
        {
            ...
            {
                if (flags[kFLAGS.RUBI_COCK_SIZE] > 12)
                {
                    flags[kFLAGS.RUBI_COCK_SIZE] = 12;
                }
            }
            else if (flags[kFLAGS.RUBI_COCK_SIZE] > 20)
            {
                flags[kFLAGS.RUBI_COCK_SIZE] = 20;
            }
        }
        else
        {
            ...
            if (this.rubiGetCockType() == CockTypesEnum.HORSE)
            {
                outputText("sheath-stuffing horse-dick");
            }
            else
            {
                outputText("giant dick.");
            }
        }
        player.consumeItem(consumables.GROPLUS);
        menu();
    ...
    private function pickADressToGiftRubi() : Void
    {
        ...
        for (item in closet)
        {
            ...
            //Present
            else if (Reflect.field(item, Std.string(4)) != null)
            {
                addButton(btn++, Reflect.field(item, Std.string(1)), giveRubiClothes, Reflect.field(item, Std.string(4))).disableIf(!player.hasItem(Reflect.field(item, Std.string(4))));
            }
        }
        if (btn == 0)
    ...
    private function pickAnItemToFeedRubi() : Void
    {
        ...
        if (!rubiBimbo())
        {
            addButton(button++, "Bimbo Liq", giveRubiATFItem, consumables.BIMBOLQ).disableIf(!player.hasItem(consumables.BIMBOLQ));
        }
        addButton(button++, "Incubi Draft", giveRubiATFItem, consumables.INCUBID).disableIf(!player.hasItem(consumables.INCUBID));if (player.hasItem(consumables.P_DRAFT))
        {
            addButton(button++, "Pure Draft", giveRubiATFItem, consumables.P_DRAFT).disableIf(!player.hasItem(consumables.P_DRAFT));
        }
    ...
    public function hypnoBimboficationForRubiSloots() : Void
    {
        ...
        if (flags[kFLAGS.RUBI_BREAST_SIZE] < 1)
        {
            outputText("femmy boy");
        }
        else if (flags[kFLAGS.RUBI_NO_CUNT] > 0)
        {
            outputText("dick-girl");
        }
        else
        {
            outputText("little slut");
        }
        ...
        {
            outputText("bend down");
        }
        else
        {
            outputText("lean forward");
        }
