...
class Brooke extends TelAdreAbstractContent
{
    //Introduction
    ...
    public function brookeAffection(arg : Float = 0) : Float
    {
        flags[kFLAGS.BROOKE_AFFECTION] += arg;
        if (flags[kFLAGS.BROOKE_AFFECTION] > 100)
        {
            flags[kFLAGS.BROOKE_AFFECTION] = 100;
        }
        else if (flags[kFLAGS.BROOKE_AFFECTION] < 0)
        {
            flags[kFLAGS.BROOKE_AFFECTION] = 0;
        }
        if ((player.isTaur() || flags[kFLAGS.BROOKE_SHOWERED_WITH] == 0) && flags[kFLAGS.BROOKE_AFFECTION] > 20)
        {
            flags[kFLAGS.BROOKE_AFFECTION] = 20;
        }
        return flags[kFLAGS.BROOKE_AFFECTION];
    }
    public function brookeCapacity() : Float
    {
        return 60;
    }
    ...
    public function meetBrookeFirstTime() : Void
    {
        ...
        if (player.isNaga())
        {
            outputText("coil your tail nervously under yourself");
        }
        else
        {
            outputText("shuffle your [feet]");
        }
    ...
    public function helpBrookeOut() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(" and being so close to such a woman, you find it difficult to keep [eachCock] from getting too excited");
        }
        outputText(".");
        ...
        if (player.gender == 1)
        {
            outputText("not caring about the stirring in [eachCock] in the privacy of the stall");
        }
        //[if {PC is female}]
        if (player.gender == 2)
        {
            outputText("feeling a fire stoke in your [vagina] even under the constant water");
        }
        if (player.gender == 3)
        {
            outputText("your loins beginning to stir, each and every part of you enticed");
        }
    ...
    public function mediumLowBrookeAffection() : Void
    {
        ...
        if (brookeAffection() >= 40 && brookeAffection() <= 70 && flags[kFLAGS.BROOKE_MEDIUM_SCENE] > 0)
        {
            addButton(3, "Grope", gropeDatBrooke);
        }
    }
    ...
    public function mediumBrookeAffectionMeetingAfterSex() : Void
    {
        ...
        addButton(2, "Her Bod", brookesRockinBod);
        if (brookeAffection() >= 40 && flags[kFLAGS.BROOKE_MEDIUM_SCENE] > 0)
        {
            addButton(3, "Grope", gropeDatBrooke);
        }
    }
    ...
    public function talkToBrookeAboutGymFolks() : Void
    {
        ...
        if (flags[kFLAGS.BROOKE_AND_HELIA_3SOME] > 0 && flags[kFLAGS.BROOKE_AND_HELIA_3SOME] < 4)
        {
        ...
        else if (flags[kFLAGS.BROOKE_AND_HELIA_3SOME] > 3 && flags[kFLAGS.BROOKE_AND_HELIA_3SOME] < 7)
        {
        ...
        else if (flags[kFLAGS.BROOKE_AND_HELIA_3SOME] > 6)
        {
    ...
    public function gropeDatBrooke() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("  You can only imagine what it could feel like to drive your [cock] between those cheeks, just inches away from you.  The thought makes [eachCock] rouse.");
        }
        ...
        else
        {
            ...
            if (player.hasVagina())
            {
                outputText(" and dragging her wet tail enticingly through the vulva of your [vagina]");
            }
        }
    ...
    private function sexMenu() : Void
    {
        ...
        addButton(4, "Get Laid", curry(sceneHunter.selectGender, //SPAGHETTIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII
                                ...
                                null, null, 1, 
                                player.cockThatFits(brookeCapacity()) >= 0
                ), nofitMsg).disableIf(cockFits == 0 && !player.hasVagina(), nofitMsg + " or a vagina");
    ...
    public function penetrateBrooke() : Void
    {
        ...
        if (y >= 2)
        {
            outputText(".  Your " + cockDescript(y) + " rubs up and down the crack of her ass lamely, though the way she clenches against you with every inward thrust makes every sensation worth the effort");
        }
        //[if (isHerm = true)
        if (player.gender == 3)
        {
            outputText(".  Your [vagina] pulses longingly, jealous of the attention you're giving to Brooke's own, and hungers for something to suffocate the fire built inside it");
        }
        ...
        if (player.hasBalls())
        {
            outputText(" across your [sack] and");
        }
        ...
        if (y >= 0)
        {
            outputText("  Your " + cockDescript(y) + " shoots its own load up and onto her lower back and the base of her tail, snug between the cheeks of her ass");
        }
        //[if (cocks >= 3)
        if (player.cockTotal() > 1 && y >= 0)
        {
            outputText(" while [eachCock] sprays your jizz all over the place: the floor and the walls of the small stall in particular, but also on her ass, onto her tail, and down her legs.  Most of it washes into the drain beneath you, but it'll definitely take a bit of scrubbing for her to get really clean after this.");
        }
        //[if (cocks = 2)
        if (player.cockTotal() == 2 && y >= 0)
        {
            outputText(".");
        }
        if (player.gender == 3)
        {
            outputText("  Your [vagina] cums along with the rest of you, spurting your femcum down your legs, washing the inside of your thighs in your juices.  The combined pleasure between your genitals makes your knees buckle – if only you had something, or someone, fucking you too!");
        }
    ...
    public function brookeAnal() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_IN_BROOKES_BUTT] == 0)
        {
            outputText("  You wonder – is this the first time she's ever had anal before?");
        }
        else
        {
            outputText("  She's tight as a vice, but you know she can take you.  She's just getting herself ready.");
        }
        
        outputText("\n\nThe " + player.cockHead(x) + " of your " + cockDescript(x) + " rubs against her rosebud in anticipation");
        if (player.cockTotal() > 1)
        {
            outputText(" while your other member entertains itself between her thighs, bumping and sliding pleasurably across her vulva");
        }
        ...
        if (player.cor >= 45)
        {
            outputText("  With any luck, that someone will come to investigate – you're sure it wouldn't take much convincing to start a party.");
        }
        
        outputText("\n\n<i>\"Harder!\"</i> she demands, thrusting her ass back onto you.  You're not sure how much harder you can go");
        if (player.cockTotal() >= 2)
        {
            outputText(".  Any one of your dicks that's not inside her is battering painfully around her ass as she continues to hit you with it, making it harder to focus on fucking her this way");
        }
        ...
        if (player.hasBalls())
        {
            outputText(" in your [balls]");
        }
        outputText("; your cock stiffens");
        if (player.gender == 3)
        {
            outputText("; your lonely pussy clenches at nothing");
        }
        outputText("; and you cum into her, blasting rope after rope of your jism inside her.");
        if (player.cockTotal() > 1)
        {
            outputText("  Your second dick flails and erupts with your " + cockDescript(x) + ", spraying onto the wall in front of Brooke and across the lips of her sex.");
        }
        //[if (cocks >= 3)]
        if (player.cockTotal() > 2)
        {
            outputText("  [EachCock] blasts in time with your " + cockDescript(x) + ", each of your dicks pointing in a different direction.  Your cum gets on everything: Brooke, yourself, the stall – some even reaches as high as the shower nozzle.");
        }
        ...
        if (flags[kFLAGS.TIMES_IN_BROOKES_BUTT] == 0)
        {
            outputText("<i>\"I never knew anal could be such a thrill!  We'll have to do that again sometime!\"</i>  You chuckle politely but deep down you're almost dreading your next anal session with her.  You'll have to build up your stamina some more to keep up with her.");
        }
        else
        {
            outputText("<i>\"Thanks for putting up with me being such a bossy bitch, [name].  But it's your fault, you know.  You're the one who awakened the anal-hungry monster within me.\"</i>  You're aware of the beast you've created, you tell her, but truth be told, since your first session, it's been a bit easier to keep up with her.  She's just that good of a workout partner.");
        }
    ...
    public function tribadism() : Void
    {
        ...
        if (player.gender == 3)
        {
            outputText(" your [cock], impishly wishing it were put to use.  You have some other plans this time, though.");
        }
        else
        {
            outputText(" your " + player.skinFurScales() + ".  The tone of her ass almost makes you wish you had a dick to fuck her with, but you're sure she'll give you a hell of a ride all the same.");
        }
        ...
        if (player.gender == 3)
        {
            outputText(" and all the way to the base of your now-straining, pre-leaking cock");
        }
        outputText(".");
        ...
        if (player.tone >= 50)
        {
            outputText(", and she coos sexily, gripping and pulling at each of your toned, chiseled asscheeks, even giving one as hard a slap as her hand can manage");
        }
        ...
        if (!player.hasCock())
        {
            outputText(".  Brooke moves her hands up to your tits, her hands groping onto your [chest], her palms pressing onto your [nipples] mindlessly, and her tongue lolls and flags with each buck up against her you make");
        }
        //[if {is not a herm and is lactating}
        if (!player.hasCock() && player.lactationQ() >= 200)
        {
            outputText(".  Your milk streams from your [chest] as Brooke puts more of her weak pressure on them: it sprays from between her fingers and drips down the back of her hands on across her wrists");
        }
        if (!player.hasCock() && player.hasFuckableNipples())
        {
            outputText(".  Feeling the texture of your [nipples] under her hands snaps her out of her reverie enough to think to stimulate you further by pressing into each with a thumb, fucking into and against you with each of your excited thrusts");
        }
        outputText(".");
        if (player.hasCock())
        {
            outputText("  Brooke moves one hand to your [chest] and wraps her other around your stiff [cock], pumping it in time with your thrusts against her.");
        }
        ...
        if (player.gender == 3)
        {
            outputText(".  Your [cock] explodes along with its sister, spurting your cum high between your pressed bodies, coating the underside of your breasts and your stomachs, but mostly onto Brooke, who keeps your flow pointed towards her as she pumps you");
        }
        else
        {
            outputText(".  Brooke continues to drive against you as she mauls your breasts, using whatever strength is in her fingers to grope what she can, eager to reach her own peak not far off");
        }
        ...
        if (player.gender == 3)
        {
            outputText(" and [eachCock] goes limp");
        }
        ...
        if (player.gender == 3)
        {
            outputText(".  <i>\"Lucky for me, we won't always have to choose.\"</i>  She gently strokes your waning erection some more, releasing you when the [cockHead] touches down on her skin limply");
        }
        outputText(".");
    ...
    public function goDownOnBrooke() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("  [EachCock] begins to fill with your desire, desperate to replace your fingers, and it takes no small amount of willpower to deny yourself.");
        }
        ...
        if (player.hasCock())
        {
            outputText("and [eachCock] pulls forward, pointing towards her like a compass to magnetic north, leaking your pre at her scent and ready to fuck her, or something, anything.");
        }
        else
        {
            outputText("and you find yourself salivating – such a viable, powerful specimen like herself has you wishing you had a dick to fuck her with.");
        }
        ...
        if (player.hasCock())
        {
            outputText("  [EachCock] has hardly gone unnoticed this whole while, still standing at attention and begging, absolutely pleading for some action.");
        }
        else
        {
            outputText("  Your own [vagina] stands out, puffy and needy against the cold air, and it's certainly not gone unnoticed.");
        }
    ...
    public function getLaidByBrooke(x : Int = -1) : Void
    {
        ...
        if (x >= 0)
        {
            outputText("spearing your " + cockDescript(x) + " between her cheeks wickedly, feeling it rise effortlessly against her soft fur and the smooth, strong muscles of her glutes");
        }
        //[if (hasCock = false)]
        else
        {
            outputText("and with some clever maneuvering, you manage to wiggle her tail between your legs, and you feel the soft fur and the smooth, powerful muscles of her tail brush against your [vagina], stimulating you easily");
        }
        ...
        if (x >= 0)
        {
            outputText("your " + cockDescript(x) + " continues to thrust and spear between her luscious ass, demanding attention be paid to it.  Brooke notices, and rocks her hips against you, squeezing your meat teasingly with each downstroke her ass makes on your " + cockDescript(x));
        }
        else
        {
        ...
        if (x >= 0)
        {
            outputText("stroking along [eachCock], hissing inwardly at what she finds, loving the way her fingers, hurt as they are, just manage to glide across your skin");
        }
        else
        {
            outputText("stroking along your [vagina], grinning at how your lower lips just manage to wrap around her fingers like they were made for each other.  Her thumb drags across your [clit] intentionally, several times, and she smiles wider at every feminine grunt you make");
        }
        ...
        if (x >= 0)
        {
            outputText(" [eachCock], now fully erect and at attention, just for her");
        }
        else
        {
            outputText(" your [vagina], loving how you can hardly resist fucking your hips at nothing in anticipation");
        }
        ...
        if (x >= 0)
        {
            ...
            if (player.cockTotal() >= 3)
            {
                outputText(" and all over the rest of her");
            }
            outputText(".");
            ...
            if (player.cockTotal() <= 2)
            {
                outputText("enjoying the lips of your canine lover as they slide over [eachCock] and how she seems to enjoy herself so thoroughly");
            }
            else if (player.cockTotal() >= 3)
            {
                outputText("enjoying how she tries her damndest to make sure every little bit of you has enough attention and is pleased to the best of her ability, no matter how much effort she requires");
            }
            outputText(".");
            if (player.gender != 3)
            {
            ...
            else
            {
                ...
                if (player.hasBalls())
                {
                    outputText("burying her muzzle underneath your [sack] to push your [balls] out of her way, ");
                }
            ...
            if (player.cockTotal() >= 3)
            {
                outputText(".  Her hands are practically a blur, up and down on [eachCock] – she's pulled them each down and towards her, aiming them towards their proper target");
            }
            //[if (isHerm = true)]
            if (player.hasVagina())
            {
                outputText(".  Her fingers are straight as can be, pistoning into you roughly while her thumb frigs against your [clit], and the feel of her enthusiastic fingers inside your [vagina], massaging and caressing your walls, has you seeing stars");
            }
        ...
        else
        {
            ...
            if (player.cor <= 49)
            {
                outputText("  That would almost be embarrassing enough to turn you off.");
            }
            else
            {
                outputText("  You want this sexy bitch all for yourself, and you'll be damned if you're sharing.  At least not until you're done.");
            }
        ...
        if (player.cockTotal() >= 3)
        {
            outputText(", and letting the excess wash from her fur half-heartedly");
        }
        ...
        if (x >= 0)
        {
            outputText(", snowballing what she hasn't swallowed quite yet");
        }
    ...
    public function doubleDicked() : Void
    {
        ...
        if (player.isNaga())
        {
            outputText("Twice the dick means twice the fun... but twice the work, too.  I'm up to the challenge, though, if you are.");
        }
        else
        {
            outputText("Haven't actually seen anyone that's not half-snake sporting more than one.  Mind if I put 'em to the test?");
        }
        ...
        if (flags[kFLAGS.TIMES_IN_BROOKES_BUTT] > 0)
        {
            outputText("  <i>\"Let's just hope this doesn't turn me any more into a buttfucking freak, huh?\"</i>");
        }
        else
        {
            outputText("  Brooke hesitates just a bit as she feels the heat of your dick against her asshole.  <i>\"Well, first time for everything,\"</i> she says to herself.");
        }
        ...
        if (player.cockTotal() >= 3)
        {
            outputText("  Your other dicks rise as well, though not as quickly as the first two, jealous that they were chosen instead.");
        }
        ...
        if (flags[kFLAGS.TIMES_IN_BROOKES_BUTT] == 0)
        {
            outputText("it's certainly not a workout she's accustomed to,");
        }
        else
        {
            outputText("no matter how much she loves getting fucked in the butt, it's still not a usual thing for her,");
        }
        ...
        if (player.cockTotal() >= 3)
        {
            outputText("The rest of you lamentably stands between you, leaking their pre uselessly between your bodies, pleading for some attention themselves.  ");
        }
        if (player.gender == 3)
        {
            outputText("Your [vagina] clenches in time with its neighbor – you can easily imagine what it might feel like to have two dicks in you at once.  You almost wish for it while you're waiting for Brooke's go-ahead.  ");
        }
        ...
        if (flags[kFLAGS.TIMES_IN_BROOKES_BUTT] == 0)
        {
            outputText("Brooke's breathing slowly starts to even out, and she starts to moan enjoyably.  Maybe she's starting to like it?");
        }
        else
        {
            outputText("Brooke rocks her hips onto both of your cocks, alternating from taking one deeper than the other.  As much as she just loves sex in general, it's the alien feeling of being so full that keeps her from going any harder.");
        }
        outputText("  She gasps and jerks with your every thrust, and you grunt, shoving yourself");
        if (player.hasBalls())
        {
            outputText(" down to your [balls]");
        }
        ...
        if (player.cockTotal() >= 3)
        {
            outputText("  Your extra equipment, painfully erect, squashes between your bodies, pointing right at the underside of her boobs and your own [chest].");
        }
        ...
        {
            outputText(" – she's probably going to have trouble doing much of anything for a day or two until she 'digests' her load");
        }
    ...
    public function mediumAffectionOneTimeEvent() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("  You feel [eachCock] beginning to wake from her proximity, and in just a few seconds, she'll know it.");
        }
        ...
        if (player.hasCock())
        {
            outputText("  Her thighs brush against your quickly rising [cock], and she's not even pretending it was an accident.");
        }
    ...
    public function brookeSpecialMediumSceneContinued() : Void
    {
        ...
        function dickF() : Void
        {
            ...
            if (player.cockTotal() == 2)
            {
                outputText(" while your second phallus brushes higher, rubbing itself through the wet fur on her stomach between the gorge of her abs");
            }
            //[if (cocks >= 3)
            if (y >= 0 && player.cockTotal() >= 3)
            {
                outputText(" while [eachCock] finds themselves either driving themselves higher, through the wet fur on her stomach and across each of her tensed abs, or joining the first in between her legs, pressing and pulling with her needy humping");
            }
            ...
            if (player.hasBalls())
            {
                outputText("[sack] resting gently against her crotch");
            }
            else
            {
                outputText("crotch resting gently against hers");
            }
            ...
            if (player.hasBalls())
            {
                outputText("  Your [balls] swing away with you, slapping onto her pussy with every inward thrust.  You feel no pain, focused only on fucking this girl raw, knowing she's strong enough to take you.");
            }
            if (player.cockTotal() == 2)
            {
                outputText("  Your other dick swings freely, cold and alone in the shower between you, occasionally spurting pre into the air and onto Brooke's fur.  It's quickly washed away with the water.");
            }
            if (player.cockTotal() >= 3)
            {
                outputText("  Your other dicks swing freely, cold and alone in the shower between you, occasionally spurting pre into the air and onto Brooke's fur.  They're quickly washed away with the water.");
            }
            ...
            {
                outputText(", running her fingers through your " + hairDescript());
            }
            ...
            if (player.hasBalls())
            {
                outputText("  Your [balls] begin to churn.");
            }
            ...
            if (player.cockTotal() == 2)
            {
                outputText("  Your other prick tenses along with the first, pointing up and along the tight abs of the Shepherd girl, jizz erupting from the tip and blasting her on the underside of her perky tits.  If she even notices, she certainly doesn't seem to care.");
            }
            if (player.cockTotal() >= 3)
            {
                outputText("  [EachCock] spurts and flails along with the first, painting the stall white with your warm sperm.  It lands on the walls and the floor mostly, but more than a good deal lands on Brooke and into her fur.  She's so out of it from her own orgasm that she barely even notices.");
            }
        ...
        var hermF : Void->Void = function() : Void
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("  You drill into that canine pussy, giving Brooke everything you have.  Your body burns and your cum churns the longer you go.  Her voice ascends from pitiful puppy whining to the jubilant cries of a woman well fucked, and the rise of her yells bring with them the rise of your cum.");
            }
            if (player.cockTotal() == 2)
            {
                outputText("  Your second penis swings, cold and alone, between your body and Brooke's as you continue to saw into her with your " + cockDescript(x) + ".  The tip points up to the cleft of her swinging, pounding breasts, ready to erupt and coat the underside of her tits with your offering.  She knows of it, and she does what she can to pleasure it, but even her well-defined six-pack can only do so much.");
            }
            if (player.cockTotal() >= 3)
            {
                outputText("  Unfortunately, as much as both you and Brooke want to pleasure as much of you as possible, she can only do so much.  [EachCock] thrusts into nothing between you, each of them stiff and pointed right up at her face – if they can't cum inside her, then they'll jealously paint her white on the outside.");
            }
            ...
            if (player.cumQ() <= 250)
            {
                outputText("  Your cum erupts into the Shepherd girl, pumping into her hard, and she continues to coax as much as she can with each ripple her orgasm gives her.  It's the grandest orgasm you've had in some time, and it takes a good few moments before you finally calm down.");
            }
            //[if (cumMedium = true)]
            if (player.cumQ() <= 500)
            {
                outputText("  Your fluid pumps into her hungry cooch with abandon: each of your heartbeats begets a pulse through your " + cockDescript(x) + " and into her womb.  She continues to shout with each of her convulsions, her voice slowly and steadily descending as her own orgasm subsides.  It takes you a moment to calm down, yourself.");
            }
            //[if (cumHigh = true)]
            else if (player.cumQ() <= 1000)
            {
                outputText("  Urgently, your cum floods into her, pumping into her with each heartbeat.  She feels it all and squirms with each new squirt she manages to pull out of you – she continues to rock her hips, her howling from her orgasm keeping up with your own.  She holds you tight with her powerful legs, intent on not letting a drop go.");
            }
            //[if (cumVeryHigh = true)]
            else if (player.cumQ() <= 3000)
            {
                outputText("  You orgasm powerfully, the cum ripping through your " + cockDescript(x) + " and into her with a force she's never experienced before.  Her howling is cut short as she doubles over, pressing her tits into your face, her hips desperately rocking into yours every time you deposit another spurt.  Her stomach begins to lose some definition from the sheer amount of cum of pumping into her, but from how she continues to convulse even long after her initial orgasm, she seems to like it.");
            }
            //[if (cumExtreme = true)]
            else
            {
                outputText("  Your orgasm is undeniable, the initial blast into her womb barely even a hint of the torrent following behind it.  Her howl is cut short as she feels her womb fill so full, so immediately, and then she howls again, even louder, as you continue to relentlessly pump your semen into her.  Her stomach not only loses its six-pack definition, but it actually begins to balloon slightly, nearly suggesting that she's not all muscle.  Her orgasm either never stops, or she has more than a few – you're not positive.");
            }
            //[if (cocks = 2)]
            if (player.cockTotal() == 2)
            {
                outputText("  Your second phallus erupts in time with its brother, spraying into the air uselessly, but coating the underside of Brooke's breasts just as it promised it would.  Your cum erupts in equal parts from both dicks, soaking into her fine fur, blending in with the white strip of fur running down her torso.  She'll definitely need a second rinsing after this.");
            }
            //[if (cocks >= 3)]
            if (player.cockTotal() >= 3)
            {
                outputText("  [EachCock] not inside her sprays out in time with their lucky brother, aimed right at her hunched, humping body and coating it all with your cum.  Her tight thighs, her taut belly, her ribs, her round tits, and even as high as her collarbone and her face each get a hefty blast of your cum, fulfilling their unspoken promise.");
            }
            //if (isSquirter = false)
            if (player.wetness() >= 4)
            {
    ...
    public function submitHeckelXBrookeThreesome() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("engulfing your [cock] into her warm, busy mouth");
        }
        if (player.cockTotal() == 2)
        {
            outputText(" while she strokes your second dick with a free hand, wobbly balancing herself on her other, tired arm");
        }
        if (player.cockTotal() >= 3)
        {
            outputText(" while she does her best to please your [eachCock], massaging one with her fingers and shoving the others in her mouth, sometimes at once");
        }
        if (player.hasCock())
        {
            outputText(".  Brooke and Heckel move at the same pace: the Shepherd girl, eager to please, does all she can with her mouth, sucking, bathing, and worshiping your tool, ");
        }
        if (!player.hasCock() && player.hasVagina())
        {
            outputText("burying her muzzle into your [vagina] and immediately gets to work digging and searching your tunnel with her warm, moist, energetic tongue.  She licks, kisses, and worships everything from your vulva to your [clit] and when she gets bored of that, she thrusts her whole face into you once more, finding some new depth neither of you thought she could reach.  All the ");
        }
        ...
        if (player.hasCock())
        {
            outputText("  You might have taken offense to the idea you're not as good a fuck if you weren't too focused on the amazing blowjob Brooke's giving you.");
        }
        ...
        if (player.hasCock())
        {
            outputText("you erupt, spraying your cum directly into Brooke's thirsty throat.  Despite her own ecstasy, she knows to swallow to avoid choking, taking hurried breaths through her nostrils when she can while you coat her mouth white several times over.  Her mouth and her tongue are mostly still, barely conscious enough to move much less pleasure you farther.  You don't mind, and you cum and cum into her mouth regardless.");
        }
        if (player.gender == 2)
        {
            outputText("you cum into her mouth, spraying your feminine juices onto her tongue and wherever else Brooke didn't have the sense to cover.  It gets into her mouth; onto her muzzle; and down her face.  You coat her from your end as your pleased cunny quivers and cums again and again.  Brooke, for her part, swallows whatever happens to land into her maw, but she barely even seems to notice whenever your happen to get her anywhere else.");
        }
        ...
        if (player.hasCock())
        {
            outputText("Based on how you know Brooke to be such a cum-hungry bitch when she's really into it, you doubt she'd rather have it any other way.  ");
        }
    ...
    public function brookeVHeckelBrookeWins2() : Void
    {
        ...
        function dickF() : Void
        {
            ...
            if (player.gender == 3)
            {
                outputText("  With every thrust into Heckel, you're reminded of your own empty pussy and you idly wish there was a spare cock around to fill you up too.");
            }
            ...
            {
                var bj : Bool = flags[kFLAGS.TIMES_FUCKED_HECKEL_BLOWJOB] > 0;
                var anal : Bool = flags[kFLAGS.TIMES_FUCKED_HECKEL_ANAL] > 0;
                
                if (bj && anal)
                {
                    outputText("  Every time you met Heckel, you either went down on her, or had to take her dick up your ass – now, with the rare opportunity to return the favor, y");
                }
                else if (bj && !anal)
                {
                    outputText("  Every time you met Heckel, you went down on her - now, with the rare opportunity to return the favour, y");
                }
                else
                {
                    outputText("  Every time you met Heckel, you had to take her dick up your ass - now, with the rare opportunity to return the favour, y");
                }
            }
            else
            {
            ...
            if (player.cockTotal() >= 2)
            {
                outputText("  Your extra dick flops lamely up the crack of her ass, hotdogging between her cheeks with every thrust.  You consider the possibility of maybe double-dicking the cocky hyena, but you opt against it, not willing to break the good flow you've got between the three of you.");
            }
            ...
            if (player.hasKnot(x))
            {
                outputText("  Likewise with Brooke, you pound your knot against Heckel's quim again and again, refusing to be denied entry, and with your orgasm, her pussy relents, taking in every inch of your " + cockDescript(x) + " with you.");
            }
            ...
            if (player.gender == 3)
            {
                outputText(".  Your [vagina] pulses in time with your " + cockDescript(x) + ", spraying your feminine juices down the inside of your tired legs uselessly, lonely and longing for a cock of its own to take");
            }
            if (player.cockTotal() == 2)
            {
                outputText(".  Your second dick flails and pulses, ejaculating in time with its lucky brother, shooting up and coating Heckel's back with your jism");
            }
            if (player.cockTotal() >= 3)
            {
                outputText(" while [eachCock] pumps yet more of your virile jism every which way, staining the tile floor and tagging the lockers surrounding you with your cum");
            }
            outputText(".");
    ...
    public function brookeVHeckelBrookeWins3() : Void
    {
        ...
        if (!player.hasCock())
        {
            outputText(", muffled from your [vagina] still plugging her lips");
        }
        ...
        if (player.hasCock())
        {
            outputText(", and gently lay her down on her back, with Brooke awkwardly laying herself down with her");
        }
