...
class Dominika extends TelAdreAbstractContent
{
    public function fellatrixSucked() : Bool
    {
        return timesFellatrixSucked() > 0;
    }
    private function timesFellatrixSucked() : Float
    {
    ...
    private function acceptDominikasKnowledge() : Void
    {
        ...
        else
        {
            ...
            if (!fellatrixSucked())
            {
                outputText("learning more arcane lore.");
            }
            else
            {
                outputText("getting to feel those black lips on your skin once again.");
            }
            ...
            if (player.inte >= 80)
            {
                outputText("Not due to lack of intellect, but due to lack of time.  You're all too aware of your mental skill.  ");
            }
        ...
        if (flags[kFLAGS.DOMINIKA_MAGIC_COOLDOWN] == 0)
        {
            ...
            else
            {
                ...
                for (spell in spells)
                {
                    if (!player.hasStatusEffect(Reflect.field(spell, Std.string(0))))
                    {
                        player.createStatusEffect(Reflect.field(spell, Std.string(0)), 0, 0, 0, 0);
                        outputText("\n\n<b>New " + ((Reflect.field(spell, Std.string(1)) != null) ? "white" : "black") + " magic spell Learned: " + (try cast(Reflect.field(spell, Std.string(0)), StatusEffectType) catch(e:Dynamic) null).id.substr(6) + "</b>");  //substring to remove "Knows "  
                        break;
                    }
                }
                dynStats("int", 2);
    ...
    private function fellatrixSexWarmup() : Void
    {
        ...
        {
            outputText("They briefly run down to the top of your " + breastDescript(0) + " before returning to your collar.  ");
        }
        //First time
        if (flags[kFLAGS.DOMINIKA_MET] == 2)
        {
            outputText("\"<i>I have not been unaffected by this land. I have certain... dietary needs.</i>\" She coughs slightly, glancing aside and looking as though she's doing her very best not to blush.\n\n");
        }
        //Repeat
        else
        {
            outputText("\"<i>You know what I need,</i>\" She coughs slightly, glancing aside and looking as though she's doing her very best not to blush.\n\n");
        }
    ...
    private function acceptFellatrixOral() : Void
    {
        ...
        while (c-- > 0)
        {
            if (player.cockArea(c) < 36)
            {
                ...
                else if (player.cocks[c].cockType == CockTypesEnum.HORSE)
                {
                    horse = c;
                }
                else
        ...
        function vagF() : Void
        {
            ...
            else
            {
                ...
                {
                    outputText("A part of you even wishes that you had a cock so you could shove through them!\n\n");
                }
                else
                {
                    outputText("A part of you even wishes that she had chosen your cock so you could shove it through them!\n\n");
                }
        ...
        // - first time wangsuck
        {
            ...
            if (flags[kFLAGS.DOMINIKA_SUCKED_SMALL] == 0)
            {
                ...
                if (player.hasBalls())
                {
                    outputText("Meanwhile her mouth slips down your dick, planting a few kisses along the way before reaching your " + sackDescript() + ". Her dark lips press against each of your [balls] in turn, granting each her moist smooch. She exhales warmly over them, teasing the cumfactories with a grin. Her mouth slips partially over one, her tongue slipping around the soft flesh. She is gentle and delicate in her administrations, and you feel comfortable about your " + sackDescript() + " sitting in her warm mouth – though you would prefer she wrap those plump lips around something else. Your head continues to be stimulated by her soft palm, and now adds pre-cum to the saliva she slowly spreads. A grunt makes her smirk, lifting from your " + sackDescript() + " and sliding her lips slowly up your dick.\n\n");
                }
        ...
        var bigF : Int->Void = function(x : Int) : Void
        // - first time wangsuck
        {
        ...
        {
            dynStats("lib", -1, "sen", 1);
            if (timesFellatrixSucked() >= 4 && flags[kFLAGS.DOMINIKA_COVENANT] == 0)
            {
                doNext(dominidrama);
            }
            else
    ...
    private function agreeWithDominika() : Void
    {
        ...
        else
        {
            ...
            {
                outputText("leans down");
            }
            else
            {
                outputText("pushes herself up");
            }
    ...
    private function dominikaBlowjobs() : Void
    {
        ...
        if (player.cor < 50)
        {
            outputText("Though you still feel ill at ease restrained like this you have to admit that Dominika has never done you wrong, and you should be safe in her home.");
        }
        //(OR Corruption greater than 50)
        else
        {
            outputText("The restraints are somewhat erotic and exciting.  You suppose that laying here is not so bad, particularly with the sexy woman before you.");
        }
        ...
        if (player.canFly())
        {
            outputText("Your wings barely have room to avoid being crushed.  ");
        }
    ...
    private function dominikaBlowjobs2() : Void
    {
        ...
        if (flags[kFLAGS.DOMINIKA_FOLLOWUP] == 1 || flags[kFLAGS.DOMINIKA_FOLLOWUP] == 4)
        {
            outputText("At the same time, you get the sense that perhaps it's for the best you did so.");
        }
        //[Disagreed/Neutral with Dominika]
        else
        {
            outputText("It deeply bothers you, leaving a sense of anxiety behind that you don't know how to deal with. You'll have to just hope something comes along that reminds you.");
        }
