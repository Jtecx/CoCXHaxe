...
class Farm extends BaseContent
{
    ...
    public function farmExploreEncounter() : Void
    {
        ...
        if (player.statusEffectv1(StatusEffects.MetWhitney) < 2)
        {
            ...
            else
            {
                ...
                if (player.statusEffectv1(StatusEffects.MetWhitney) == 2)
                {
                    outputText("<b>You've been to the farm enough to easily find it.  You can return by selecting it from the places menu (and will no longer encounter it during random lake exploration)</b>.\n\n");
                }
            }
        ...
        else
        {
            ...
            if (player.hasStatusEffect(StatusEffects.Kelt) && !player.hasStatusEffect(StatusEffects.KeltOff) && flags[kFLAGS.KELT_KILLED] <= 0)
            {
                ...
                {
                    addButton(1, "Kelly", kelly.breakingKeltOptions);
                }
                else
                {
            ...
            if (player.hasKeyItem("Breast Milker - Installed At Whitney's Farm") >= 0)
            {
                ...
                else if (flags[kFLAGS.WHITNEY_FLIPPED_OUT_OVER_KELLY] == 0)
                {
                    addButton(2, "Get Milked", getMilked);
                }
            }
    ...
    //[YES]
    private function whitneyMilkerHookup(breast : Bool = true) : Void
    {
        spriteSelect(SpriteDb.s_whitney);
        clearOutput();
    ...
    private function talkWhitney() : Void
    {
        ...
        if (rand(4) == 0 && player.hasKeyItem("Breast Milker - Installed At Whitney's Farm") < 0)
        {
            ...
            else if (player.biggestLactation() >= 2)
            {
                ...
                if (player.cor < 33)
                {
                    outputText("You blush with shame");
                }
                else if (player.cor <= 66)
                {
                    outputText("You flush with a touch of exhibitionism");
                }
                else
                {
                    outputText("You flush hotly and arch your back, openly displaying your milk to the dog-girl");
                }
        ...
        if (player.cor > 50)
        {
            outputText("or at least the parts you think she would want to hear ");
        }
        ...
        {
            outputText("how well the farm has been going");
        }
        else
        {
            outputText("how poorly the farm has been going since the lake became tainted.  She has to work three times as hard to keep her livestock and crops from succumbing to the taint, and the demons and monsters of the forest are many times more bold");
        }
        ...
        if (player.inte100 < 15)
        {
            dynStats("int", 1);
        }
        if (player.inte100 < 20)
        {
            dynStats("int", 1);
        }
    ...
    public function workFarm() : Void
    {
        ...
        if (!player.hasStatusEffect(StatusEffects.NoMoreMarble) && player.hasStatusEffect(StatusEffects.MarbleWithdrawl))
        {
            ...
            {
                marbleScene.addictedEncounterHappy();
            }
            else
            {
                marbleScene.encounterMarbleAshamedAddiction();
            }
            return;
        }
            ...
            //Pre-addiction events(explore events take 1 hour, working ones take 3){
                ...
                else
                {
                    ...
                    if (player.statusEffectv3(StatusEffects.Marble) == 1)
                    {
                        ...
                        {
                            marbleScene.marbleEncounterAddictedNonWithdrawl();
                        }
                        return;
                    }
                    else
                    {
                        ...
                        {
                            marbleScene.marbleEncounterAddictedNonWithdrawlAshamed();
                        }
                        return;
                    }
                }
    ...
    public function meetMarble() : Void
    {
        ...
        //Meet Marble while exploring version 1 (can occur anytime before the player becomes addicted):{
            ...
            {
                marbleScene.encounterMarbleExploring2();
            }
        }
        else
        {
    ...
    public function exploreFarm() : Void
    {
        ...
        if (explore == 2)
        {
            ...
            if (player.horseCocks() > 0)
            {
                ...
                if (player.cor > 60)
                {
                    outputText("It makes you smile.");
                }
                if (player.cor < 30)
                {
                    outputText("It disgusts you.");
                }
                //[else/mid-corruption]
                else if (player.cor <= 60)
                {
                    outputText("You aren't sure how you feel about that.");
                }
	...
	(HUGE – 2.4 - Size 12 + 5 Multi + 4 tits)*/
    {
        ...
        if (!player.hasStatusEffect(StatusEffects.BreastsMilked))
        {
            if (player.cor < 50)
            {
                outputText("You shiver nervously when you step inside.");
            }
            else
            {
                outputText("You smile eagerly as you walk inside.");
            }
        ...
        else
        {
            ...
            {
                outputText("  Your " + nippleDescript(0) + "s are engorged and ready to be taken care of.");
            }
            //If cocks
        ...
        var application : Float = rand(3);
        //Super huge nips scene
        if (player.nippleLength == 3 && rand(2) == 0)
        {
            application = 3;
        }
        //Apply
        if (!player.hasStatusEffect(StatusEffects.BreastsMilked) || application == 0)
        {
            if (!player.hasStatusEffect(StatusEffects.BreastsMilked))
            {
                player.createStatusEffect(StatusEffects.BreastsMilked, 0, 0, 0, 0);
            }
            ...
            if (player.nippleLength <= 1.5)
            {
                outputText("pulling each of your nipples entirely into the suction-tubes.  ");
            }
            else
            {
                outputText("struggling to fit around each of your nipples as they slide into the suction-tubes.  ");
            }
            outputText("There is a mechanical lurching noise as the suction builds rapidly.  Your nipple swells out to " + as3hx.Compat.parseInt(player.nippleLength * 1.5 * 10) / 10 + " inches of length, turning purplish from the strain.   You can feel something welling up inside your [allbreasts], building as it moves towards your " + nippleDescript(0) + "s.\n\n");
        }
        ...
        else if (application == 1)
        {
            ...
            if (player.nippleLength >= 3)
            {
                outputText("They nearly burst the tubes designed to milk them by virtue of their sheer size.  ");
            }
        ...
        else
        {
            ...
            if (milksplosion == 0)
            {
                ...
                if (player.hasVagina())
                {
                    outputText("cream yourself");
                }
                else if (player.cockTotal())
                {
                    outputText("jizz yourself");
                }
                else
                {
                    outputText("orgasm");
        ...
        liters = as3hx.Compat.parseInt(player.lactationQ() * (rand(10) + 90) / 100) / 1000;
        if (liters < 0)
        {
            liters = 1337;
        }
        //Pay 4 gem for every half-liter.
        payout = as3hx.Compat.parseInt(liters * 2 * 4);
        ...
        //If over cap reduce payout to the difference{
            ...
            if (payout + flags[kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK] > cap)
            {
            ...
            if (flags[kFLAGS.WHITNEY_GEMS_PAID_THIS_WEEK] >= cap)
            {
                outputText("  <b>The machinery warns you that Whitney can't afford any more this week!</b>");
            }
            player.gems += payout;
        }
        else
        ...
        if (liters > 2)
        {
            ...
            else
            {
                if (liters / 2 > 10)
                {
                    player.addCurse("int", 10, 2);
                }
                else
                {
                    player.addCurse("int", liters / 2, 2);
                }
                if (liters > 30)
                {
            ...
            else if (player.inte < 15)
            {
                outputText("  You stretch and let out a contented moo, long and loud.  How silly!");
            }
            else if (player.inte < 25)
            {
                outputText("  You quietly moo, then giggle to yourself at how strange you're acting.");
            }
        }
        ...
        else
        {
            ...
            {
                outputText(" It isn't until you get back that you remember the disapproving look Whitney gave you, but if anything, it only makes you hornier.");
            }
        ...
        if (player.statusEffectv1(StatusEffects.LactationEndurance) < 1.5)
        {
    ...
    {
        var cumQ : Float = player.cumQ() * (rand(10) + 90) / 100;
        clearOutput();
        //First time barn entrance
        if (!player.hasStatusEffect(StatusEffects.CockPumped))
        {
        ...
        if (!player.hasStatusEffect(StatusEffects.CockPumped))
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("single hose wriggles out from the hole, squirming across the floor.  It stops and lifts up, slowly homing in your horny shaft.\n\n");
            }
            else if (player.cockTotal() == 2)
            {
                outputText("pair of hoses wriggle out from the hole, squirming across the floor.  They stop and lift up, slowly homing in your horny shafts.\n\n");
            }
            else
            {
                outputText("group of hoses wriggle out from the hole, squirming across the floor.  They stop and lift up, slowly homing in your horny shafts.\n\n");
            }
        ...
        else
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("hose wriggles out from the wall, curving up towards your crotch. ");
            }
            else
            {
                outputText("hoses wriggle out from the wall, curving up towards your crotch. ");
            }
        ...
        //(small/medium helperless skeet){
            ...
            if (cumQ < 1000)
            {
                ...
                if (cumQ > 100)
                {
                    outputText("and a clatter of gems drop into the plate ");
                }
        ...
        else if (cumQ < 333)
        {
            if (player.cockTotal() == 1)
            {
                ...
                if (player.hasBalls())
                {
                    outputText("bloated [balls] are");
                }
                else
                {
                    outputText("sensitive [cock] is");
                }
            ...
            else
            {
                ...
                {
                    outputText("sensitive [cocks] are");
                }
        ...
        cumQ = as3hx.Compat.parseInt(cumQ);
        if (cumQ > 100)
        {
        ...
        if (payout > 0)
        {
            if (player.cumQ() < 1000)
            {
                player.modCumMultiplier(1);
            }
            if (payout == 1)
            {
                outputText(cast((payout), Num2Text) + " gem rolls ");
            }
            else
            {
                outputText(cast((payout), Num2Text) + " gems roll");
            }
        ...
        {
            var gems : Int = as3hx.Compat.parseInt(rand(2) + player.countCockSocks("gilded"));
    ...
    private function milkerBadEnd1() : Void
    {
        ...
        if (cumQ > 100)
        {
            outputText(" and another clatter of gems");
        }
        ...
        if (player.hasVagina())
        {
            ...
            {
                outputText("A smaller suction tube fastens over your " + clitDescript() + " as it rouses from the vaginal stimulation.");
            }
        }
        ...
        if (player.hasBalls())
        {
            outputText(" and your " + sackDescript() + " stretches as your [balls] nearly double in size");
        }
        ...
        if (player.hasBalls())
        {
            outputText(" and the new volume of your monstrous testicles");
        }
        //[(big skeet)
        if (cumQ > 1000)
        {
            outputText(", spraying out of the overflow valve and coating your entire lower body");
        }
        outputText(".");
        //[(if vag)
        if (player.hasVagina())
        {
            ...
            if (player.clitLength > 3)
            {
                outputText(", and your clitoris twitches under the cup, threatening to black you out from the pleasure");
            }
            outputText(".");
        }
        //[(lactation)
        if (player.biggestTitSize() > 1)
        {
            ...
            if (player.hasFuckableNipples())
            {
                outputText(" and rings raise from the bases of the shafts inside your nipples, forming a seal,");
            }
        ...
        {
            outputText(", the drug now tasting of the milk mixed in");
        }
        ...
        else
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("a gigantic, erect penis bobbing in front of you and throwing off your balance");
            }
            //(multi)
            else
            {
                outputText("multiple gigantic, erect penises bobbing in front of you and dragging between your legs, head pinned against the floor");
            }
    ...
    private function centaurToysHoooooo() : Void
    {
        ...
        if (player.cor < 50)
        {
            outputText("You shift awkwardly and ask her what's wrong.");
        }
        else
        {
            outputText("You strut a bit, showing yourself off in a subtly lewd manner.  When you're finished, you ask the dog-girl if she likes what she saw.");
        }
        outputText("\n\n");
