...
class TempleOfTheDivine extends BaseContent
{
    ...
    {
        return flags[kFLAGS.WEDDINGS_UNLOCKED];
    }
    ...
    public function repeatvisitintro() : Void
    {
        ...
        {
            outputText(" Or perhaps you're here for something more... mutually agreeable, instead?");
        }
        outputText("</i>\"");
        if (flags[kFLAGS.SAPPHIRE_AFFECTION] == 100)
        {
            outputText(" she says, winking with a lusty smile.");
        }
        mainmenu();
    }
    ...
    //outputText("\n\nGargoyle Quest progress: " + flags[kFLAGS.GARGOYLE_QUEST] + "");
    {
        ...
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_MARAE] == 1 && flags[kFLAGS.FACTORY_SHUTDOWN] == 1)
        
        //req. PURE Marae{
            
            if (player.hasItem(consumables.P_PEARL, 1))
            {
                addButton(2, "Pearl", puttingPurePearlOnAltar);
            }
            if (player.statusEffectv3(StatusEffects.TempleOfTheDivineTracker) == 2)
            {
                addButton(2, "Pearl", takingPurePearlFromAltar);
            }
            if (havingOrUsingBSwordOrExcalibur())
            {
                addButton(3, "Put Sword", puttingBSwordOrExcaliburOnAltar);
            }
            if (player.statusEffectv2(StatusEffects.TempleOfTheDivineTracker) == 2 || player.statusEffectv2(StatusEffects.TempleOfTheDivineTracker) == 3)
            {
                addButton(3, "Take Sword", takingExcaliburFromAltar);
            }
            if (havingOrUsingBStaffOrParacelsus())
            {
                addButton(4, "Put Staff", puttingBStaffOrParacelsusOnAltar);
            }
            if (player.statusEffectv1(StatusEffects.TempleOfTheDivineTracker2) == 2 || player.statusEffectv2(StatusEffects.TempleOfTheDivineTracker) == 3)
            {
                addButton(4, "Take Staff", takingParacelsusFromAltar);
            }
        }
        addButton(10, "Sapphire", sapphire.sapphiremenu).hint("Have a chat with the gargoyle.");
        if (flags[kFLAGS.ONYX_PATH] > 0)
        {
            addButton(11, "[onyx name]", onyx.krystalonyxmenu).hint("Have a sex with [onyx name].");
        }
        else
        {
            addButtonDisabled(11, "???", "Sapphire is a little lonely out there. Maybe you could make her a friend...?");
        }
        addButton(12, "Basement", templeBasement).hint("Visit the temple basement.");
        if (flags[kFLAGS.FORGEFATHER_MOVED_TO_TEMPLE] == 1)
        {
            addButton(13, "Workshop", SceneLib.forgefatherScene.workshopMainMenu);
        }
    ...
    public function PlayerPrayAtTemple() : Void
    {
        ...
        if (anyOfAltarsRepaired())
        {
            ...
            menu();
            if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_MARAE] == 1 && flags[kFLAGS.FACTORY_SHUTDOWN] == 1 && !player.hasStatusEffect(StatusEffects.BlessingOfDivineMarae))
            ...
            {
                addButtonDisabled(0, "Marae", "You haven't restored this altar yet.");
            }
            if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_TAOTH] == 1 && !player.statStore.hasBuff("TaothBlessing"))
            {
                addButton(1, "Taoth", PlayerPrayAtTempleTaothAltar).hint("Pray the trickster god for an increase to your Agility, (if kitsune)kitsune powers (end of cut) and guile.");
            }
            else
            {
                addButtonDisabled(1, "Taoth", "You haven't restored this altar yet.");
            }
            if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_FERA] == 1 && !player.hasStatusEffect(StatusEffects.BlessingOfDivineFera))
            {
                addButton(2, "Fera", PlayerPrayAtTempleFeraAltar).hint("Pray the fallen goddess Fera for an increase to your innuendo and resilience to desire.");
            }
            else
            {
                addButtonDisabled(2, "Fera", "You haven't restored this altar yet.");
            }
            //addButtonDisabled(3, "E.e.ie.", "You haven't restored this altar yet.");
            if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_FENRIR] == 1 && !player.statStore.hasBuff("FenrirBlessing"))
            {
                addButton(4, "Fenrir", PlayerPrayAtTempleFenrirAltar).hint("Pray to the god sharing your body for an increase to your might.");
            }
            else
            {
                addButtonDisabled(4, "Fenrir", "You haven't restored this altar yet.");
            ...
            //Remove curses
            if (anyOfAltarsRepaired() && player.gems >= 5000)
            {
                addButton(13, "Remove Curses", PlayerRemoveCurses).hint("Make a donation to a divinity in order to be freed of all curses or hexes.");
            }
            else if (!anyOfAltarsRepaired())
            {
                addButtonDisabled(13, "Remove Curses", "Without a functionning altar you cannot call upon divine power for deliverence.");
            }
            else if (!player.statStore.hasBuff("Weakened") && !player.statStore.hasBuff("Drained") && !player.statStore.hasBuff("Damaged"))
            {
                addButtonDisabled(13, "Remove Curses", "You are not currently under the affliction of a curse or hex.");
            }
            else if (player.gems < 5000)
            {
                addButtonDisabled(13, "Remove Curses", "You need at least 5000 gem in order to request deliverance from your maledictions and other status ailments.");
            }
    ...
    private function anyOfAltarsRepaired() : Bool
    {
    ...
    public function PlayerRemoveCurses() : Void
    {
        ...
        menu();
        if (player.statStore.hasBuff("Weakened"))
        {
            addButtonIfTrue(0, "\"Weakened\"", curry(PlayerRemoveCursesYes, "Weakened", 1250), 
                    "Not enough gems!", player.gems >= 1250
            );
        }
        if (player.statStore.hasBuff("Drained"))
        {
            addButtonIfTrue(1, "\"Drained\"", curry(PlayerRemoveCursesYes, "Drained", 2500), 
                    "Not enough gems!", player.gems >= 2500
            );
        }
        if (player.statStore.hasBuff("Damaged"))
        {
            addButtonIfTrue(2, "\"Damaged\"", curry(PlayerRemoveCursesYes, "Damaged", 5000), 
    ...
    public function PlayerRemoveCursesYes(tierPower : String, cost : Float) : Void
    {
        clearOutput();
    ...
    public function PlayerPrayAtTempleMaraeAltar() : Void
    {
        ...
        var blessingPower : Float = 0;
        blessingPower += 0.1;
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS] >= 6)
        {
            blessingPower += 0.1;
        }
        ...
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS] >= 18)
        {
            blessingPower += 0.05;
        }
        if (player.statusEffectv2(StatusEffects.TempleOfTheDivineTracker) == 2 || player.statusEffectv2(StatusEffects.TempleOfTheDivineTracker) == 3)
    ...
    public function TempleAltarsRebuildMenu() : Void
    {
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS] >= 2)
        {
            ...
            {
                if (CampStatsAndResources.StonesResc >= 150)
                {
                    addButton(1, "Statue of Marae", rebuildStatueOfMarae).hint("Repair the statue.");
                }
                else
                {
                    addButtonDisabled(1, "Statue of Marae", "You don't have enough stones. Required: 150");
                }
            }
            if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS] >= 5 && flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS] < 7)
            {
                ...
                {
                    addButton(2, "Gargoyles", repairGargoylesOnTheWalls).hint("Repair some of the decorative gargoyles.");
                }
                else
                {
                    addButtonDisabled(2, "Gargoyles", "You don't have enough stones. Required: 500");
                }
            }
            if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS] >= 7 && flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS] < 17)
            {
                if (CampStatsAndResources.WoodResc >= 50 && CampStatsAndResources.NailsResc >= 10)
                {
                    addButton(3, "Prayer Bench", makeNewPrayerBenches).hint("Repair some of the temple banches.");
                }
                else
                {
                    addButtonDisabled(3, "Prayer Bench", "You don't have enough wood (50) or/and nails (10).");
                }
            }
    ...
    private function addRebuildButton(btn : Int, name : String, fun : Function, notBuilt : Bool, canBuild : Bool, disabledMsg : String) : Void
    {
        if (!notBuilt)
        {
            return;
    ...
    public function rebuildMaraeAltar() : Void
    {
        ...
        {
            outputText(" By the time you're done you can feel divine power amass around it anew.");
        }
        CampStatsAndResources.StonesResc -= 50;
        flags[kFLAGS.TEMPLE_OF_THE_DIVINE_MARAE] = 1;
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS] < 3)
        {
            flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS]++;
        }
    ...
    public function rebuildStatueOfMarae() : Void
    {
        ...
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS] < 4)
        {
            outputText("It looks slightly better, but it is far from finished.");
        }
        else if (flags[kFLAGS.FACTORY_SHUTDOWN] == 1)
        {
            outputText("By the time you're done you can feel divine power radiating from it.");
        }
        else
        {
            outputText("Even though the altar is dysfunctional, the repaired statue looks like a nice addition to the temple.");
        }
    ...
    public function makeNewPrayerBenches() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.TempleOfTheDivineTracker))
        {
            player.addStatusValue(StatusEffects.TempleOfTheDivineTracker, 1, 2);
        }
        else
        {
            player.createStatusEffect(StatusEffects.TempleOfTheDivineTracker, 2, 0, 0, 0);
        }
    ...
    public function currentStateOfTemple() : Void
    {
        ...
        {
            outputText("is broken, a large fissure running along its center. Crude graffiti litters the once pure stone with obscenities.");
        }
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_MARAE] == 1)
        {
            if (flags[kFLAGS.FACTORY_SHUTDOWN] == 1)
            ...
            else if (flags[kFLAGS.CORRUPTED_MARAE_KILLED] != null)
            {
            ...
            else
            {
                ...
                if (flags[kFLAGS.MET_MARAE] != null)
                {
                    outputText("You're not sure if Marae knows about the restored altar at all - she spends whatever power she has left to protect herself from lake's corruption.");
                }
                else
                {
                    outputText("Maybe something is still broken.");
                }
            }
        }
        outputText("\n\nTo the left of Marae's Altar, the Altar of Taoth");
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_TAOTH] < 1)
        {
            outputText(" lies shattered into pieces. The trickster god cannot even be visualised from the rubble that once made up his effigy.");
        }
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_TAOTH] == 1)
        {
            outputText(", the trickster god, has a mesmerising outlook, his effigy making a mocking smile. You swear you can hear faint laughter coming from its direction.");
        }
        outputText("\n\nTo your right is the ");
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_FENRIR] < 1)
        {
            outputText("damaged Altar of Fenrir. Even in this state its aura is ominous, promising eventual demise to everyone.You almost dare not approach, lest your journey ends here.");
        }
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_FENRIR] == 1)
        {
            outputText("Altar of Fenrir, an ominous aura radiates from it as it sits dark and foreboding.");
        }
        outputText("\n\nOn the side is the ");
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_FERA] < 1)
        {
            outputText("broken altar of Fera. You believe it was damaged way before the demons came in, likely by the priesthood tending the temple.");
        }
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_FERA] == 1)
        {
            outputText("Altar of Fera. An aura of depraved lust rise from it, inviting you to unknown pleasure. This altar radiate a clearly demonic aura and dims the sanctity of the temple like an idol.");
        }
        outputText("\n\nMarae's statue ");
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS] < 5)
        {
            outputText("lies on the ground, its head shattered to pieces");
        }
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS] >= 5)
        {
            outputText("in the background awaits the worshippers with its serene and compassionate expression");
        }
        outputText(". Lining both walls");
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS] < 7)
        {
            outputText(" are the destroyed remains of the many gargoyles who fought to defend this place. Sadly, of all the guardians, only Sapphire remains now.");
        }
        if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS] >= 7)
        {
            ...
            {
                outputText(" While Sapphire is the last living stony guardian of the temple, this still gives off the illusion of a platoon of guardians watching faithfully over this divine place.");
            }
            if (player.isGargoyle())
            {
                outputText(" Your own pedestal stands right next to Sapphire's, allowing you to keep close to her when you need to rest (using your pedestal allows you to sleep with Sapphire if your relation is high enough).");
            }
            if (flags[kFLAGS.ONYX_PATH] > 0)
            {
    ...
    public function puttingBSwordOrExcaliburOnAltar() : Void
    {
        ...
        if (player.weapon == weapons.B_SWORD || player.hasItem(weapons.B_SWORD, 1))
        {
            ...
            if (player.hasStatusEffect(StatusEffects.TempleOfTheDivineTracker))
            {
                player.addStatusValue(StatusEffects.TempleOfTheDivineTracker, 2, 2);
            }
            else
            {
                player.createStatusEffect(StatusEffects.TempleOfTheDivineTracker, 0, 2, 0, 0);
            }
        }
        else
        {
    ...
    public function takingExcaliburFromAltar() : Void
    {
        ...
        if (player.statusEffectv2(StatusEffects.TempleOfTheDivineTracker) == 3)
        {
            player.addStatusValue(StatusEffects.TempleOfTheDivineTracker, 2, -2);
        }
        else
    ...
    private function havingOrUsingBSwordOrExcalibur() : Bool
    {
    ...
    public function puttingBStaffOrParacelsusOnAltar() : Void
    {
        ...
        if (player.weapon == weapons.B_STAFF || player.hasItem(weapons.B_STAFF, 1))
        {
            ...
            if (player.hasStatusEffect(StatusEffects.TempleOfTheDivineTracker2))
            {
                player.addStatusValue(StatusEffects.TempleOfTheDivineTracker2, 1, 2);
            }
            else
            {
                player.createStatusEffect(StatusEffects.TempleOfTheDivineTracker2, 2, 0, 0, 0);
            }
        }
        else
        {
            outputText("The altar radiates with increased potency as the staff is put back on display.");
            if (player.weapon == weapons.PARACEL)
            {
                player.setWeapon(WeaponLib.FISTS);
            }
            else
    ...
    public function takingParacelsusFromAltar() : Void
    {
        clearOutput();
        outputText("You feel the power of the altar diminishing, however the weapon is stronger than ever and likely ready for its primary use, demon slaying.\n\n");
        if (player.statusEffectv1(StatusEffects.TempleOfTheDivineTracker2) == 3)
        {
            player.addStatusValue(StatusEffects.TempleOfTheDivineTracker2, 1, -2);
        }
        else
    ...
    private function havingOrUsingBStaffOrParacelsus() : Bool
    {
    ...
    public function puttingPurePearlOnAltarYes() : Void
    {
        ...
        {
            player.createStatusEffect(StatusEffects.TempleOfTheDivineTracker, 0, 0, 2, 0);
        }
        doNext(templeMainMenu);
    }
    ...
    public function templeBasement() : Void
    {
        ...
        if (flags[kFLAGS.GARGOYLE_QUEST] > 0)
        {
            addButton(1, "Statue", playerBuilder.currentStateOfStatue).hint("Check on the statue.");
        }
        if (flags[kFLAGS.GARGOYLE_QUEST] > 0 && flags[kFLAGS.ONYX_PATH] < 1)
        {
            addButtonIfTrue(2, "Spare Statue", onyx.makingNewGargoyle, "You can't do anything with it... unless you manage to find a filled soul gem somewhere?", player.hasKeyItem("Black Soul Gem") >= 0, "Check on the spare statue.");
        }  //DON'T RESET THE QUEST VALUE!!!  
