...
class Onyx extends BaseContent
{
    ...
    public function krystalonyxmenu() : Void
    {
        ...
        {
            outputText(" There’s even a water source nearby to use as lube if necessary.");
        }
        outputText("</i>\"\n\n");
    ...
    public function KrystalOnyxTailFuck() : Void
    {
        ...
        if (flags[kFLAGS.ONYX_GENDER] == 1 || flags[kFLAGS.ONYX_GENDER] == 2)
        {
            outputText("her stone breasts mashing against your chest as she");
        }
        else
        {
            outputText("His rock hard cock across your chest as he");
        }
    ...
    public function KrystalTribadism() : Void
    {
        ...
        if (player.isGargoyle())
        {
            outputText("and petrify along with Krystal, both of your bodies frozen in place for some time. You regain consciousness a while later and give her still frozen form a parting kiss as you head back to camp.\n\n");
        }
        else
        {
            outputText("and cover Krystal's stone pussy with your girl cum. Krystal petrifies from the orgasm, frozen with an expression of extreme pleasure as you doze off on your statuesque lover. When you wake up, you give her still frozen form a parting kiss as you head back to camp.\n\n");
        }
        player.sexReward("no");
        doNext(camp.returnToCampUseOneHour);
    }
    ...
    public function KrystalOnyx69() : Void
    {
        ...
        if (flags[kFLAGS.ONYX_GENDER] == 1 || flags[kFLAGS.ONYX_GENDER] == 2)
        {
            outputText("Krystal's smoothly shaped pussy");
        }
        else
        {
            outputText("Onyx's perfectly carved cock");
        }
        ...
        if (flags[kFLAGS.ONYX_GENDER] == 1 || flags[kFLAGS.ONYX_GENDER] == 2)
        {
            outputText("her back. She");
        }
        else
        {
            outputText("his back. He");
        }
        outputText(" is quick to comply, and you take your time to position your genitals right above [onyx eir] face and ");
        if (flags[kFLAGS.ONYX_GENDER] == 1 || flags[kFLAGS.ONYX_GENDER] == 2)
        {
            outputText("give her pussy a quick lick. She");
        }
        else
        {
            outputText("give his glans a quick lick. He");
        }
        ...
        if (player.hasCock())
        {
            outputText("and suckling on your cockhead.");
        }
        outputText(" You moan in delight and proceed to reward [onyx eir] effort with");
        if (player.hasVagina())
        {
            outputText(" pussy juices.");
        }
        if (player.hasCock())
        {
            outputText(" precum.[onyx Ey] strangely doesn't have a bad taste, albeit very different from a creature of flesh and blood.\n\n");
        }
        if (player.isGargoyle())
        {
        ...
        else
        {
            ...
            if (player.hasCock())
            {
                outputText("blowing your load in [onyx name]'s waiting mouth");
            }
            if (player.hasVagina())
            {
                outputText("[onyx name] drinking the flood of your juices and licking your pussy clean");
            }
        }
    ...
    public function KrystalOnyxGetFucked() : Void
    {
        ...
        if (!player.isGargoyle())
        {
            outputText(". Well not so ready yet... there's still the need for lubrication");
        }
        outputText(". You lower your head and proceed to suck on [onyx name]'s perfect cock");
        if (!player.isGargoyle())
        {
            outputText(", liberally coating the entire length with your saliva");
        }
        ...
        {
            outputText("gasping as the cold hard stone enters you");
        }
    ...
    public function KrystalFuckHer() : Void
    {
        ...
        {
            outputText("making sure to lube your [cock] liberally to compensate for the lack of lubricant.");
        }
    ...
    public function makingNewGargoyle() : Void
    {
        if (flags[kFLAGS.ONYX_GENDER] >= 1)
        {
            makingNewGargoylePart4();
        }
        else
        {
            makingNewGargoylePart1();
        }
    }
    ...
    public function makingNewGargoylePart4() : Void
    {
        ...
        if (flags[kFLAGS.ONYX_GENDER] == 1 || flags[kFLAGS.ONYX_GENDER] == 2)
        {
            outputText("She gives you an affectionate kiss on the cheek, surprising you, then flies to perch on a nearby pedestal and begins her watch.");
        }
        else
        {
            outputText("He warmly hugs you to show his appreciation, then flies to perch on a nearby pedestal and begins his watch.");
        }
        outputText("\n\n");
