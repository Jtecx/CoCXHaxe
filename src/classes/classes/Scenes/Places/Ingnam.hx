...
class Ingnam extends BaseContent
{
    public var inIngnam(get, never) : Bool;
    ...
    private function get_inIngnam() : Bool
    {
        return flags[kFLAGS.IN_INGNAM];
    }
    ...
    //Force autosave
    {
        ...
        if (flags[kFLAGS.INGNAM_PROLOGUE_COMPLETE] > 0)
        {
            addButton(5, "Return2Camp", returnToMareth);
        }
        if (flags[kFLAGS.NEW_GAME_PLUS_LEVEL] > 0 && inventory.showStash() && flags[kFLAGS.INGNAM_PROLOGUE_COMPLETE] <= 0)
        {
            addButton(6, "Stash", inventory.stash);
        }
        ...
        {
            addButton(9, "Rest", SceneLib.camp.rest);
        }
        if (model.time.hours >= 21 || model.time.hours < 6)
        {
            removeButton(0);
    ...
    public function shopBlacksmith() : Void
    {
        ...
        //20% discount for History: Fighter{
        ...
        //20% discount for History: Smith perk{
    ...
    public function shopAlchemist() : Void
    {
        ...
        //20% discount for History: Alchemist perk{
    ...
    private function sellAtTradingPost(page : Int = 1) : Void
    {
        ...
        if (page == 1)
        {
            for (slot in 0...10)
            {
                ...
                {
                    outputText("\n" + as3hx.Compat.parseInt(player.itemSlots[slot].itype.value / 3) + " gems for " + player.itemSlots[slot].itype.longName + ".");
            ...
            {
                addButton(13, "Next", sellAtTradingPost, page + 1);
            }
        }
        if (page == 2)
        {
        ...
        if (totalItems > 1)
        {
            addButton(11, "Sell All", shopTradingPostSellAll);
        }
    ...
    private function shopTradingPostSell(slot : Int) : Void
    {
        ...
        var itemValue : Int = as3hx.Compat.parseInt(player.itemSlots[slot].itype.value / 3);
        clearOutput();
        if (shiftKeyDown)
        {
            if (itemValue == 0)
            ...
            {
                outputText("You hand over " + num2Text(player.itemSlots[slot].quantity) + " " + player.itemSlots[slot].itype.shortName + " to trader.  He nervously pulls out " + num2Text(itemValue * player.itemSlots[slot].quantity) + " gems and drops them into your waiting hand.");
            }
        ...
        else
        {
            ...
            {
                outputText("You hand over " + player.itemSlots[slot].itype.longName + " to trader.  He nervously pulls out " + num2Text(itemValue) + " gems and drops them into your waiting hand.");
            }
            player.itemSlots[slot].removeOneItem();
    ...
    //Determine shop
    {
        ...
        {
            shopToGo = shopAlchemist;
        }
        else if (shop == 5)
        {
            shopToGo = shopTradingPost;
        }
        else
        {
            shopToGo = shopBlackMarket;
        }
        //Process
        clearOutput();
        if (player.gems >= price)
