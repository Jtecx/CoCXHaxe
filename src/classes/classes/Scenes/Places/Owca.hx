...
class Owca extends BaseContent
{
    ...
    private function gangbangVillageFirstGoRound() : Void
    {
        ...
        else
        {
            ...
            if (player.cor > 50)
            {
                outputText("  You can't help but smile internally.  Let her think that, if she wants.");
            }
    ...
    private function agreeToFollowRebecFirstTime() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_REFUSED_REBECCS_OFFER] == 0)
        {
            ...
            if (silly())
            {
                outputText("I-I don't think there's a single grown man or woman in our village that still has their anal virginity.");
            }
            //[if silly mode off]
            else
            {
                outputText("Th-they abused us.  They mistreated us.  They would gather in circles and... use us in every possible way.");
            }
            outputText("</i>\"");
            //[if Corr < 20]
            if (player.cor < 20)
            {
                ...
                if (silly())
                {
                    outputText("she'll never have to worry about her asshole again.");
                }
                //[if silly mode off]
                else
                {
                    outputText("she will never be abused again.");
                }
            ...
            if (player.cor < 50 && player.inte >= 50)
            {
                ...
                if (player.lib > 50)
                {
                    outputText("... and considering what Rebecc meant by 'gratitude'");
                }
        ...
        //Yes/No
        if (flags[kFLAGS.TIMES_IN_DEMON_PIT] == 0)
        {
            doYesNo(createCallBackFunction2(acceptRebeccsPlea, true, true), declineRebeccsPlea);
        }
        else
        {
    ...
    //Accept plea (Z)
    private function acceptRebeccsPlea(firstTime : Bool = false, sacrificed : Bool = false) : Void
    {
        clearOutput();
        flags[kFLAGS.OWCAS_ATTITUDE] += 10;
        if (flags[kFLAGS.OWCAS_ATTITUDE] > 100)
        {
    ...
    private function intoTheDemonPit(sacrifice : Bool = true) : Void
    {
        clearOutput();
        ...
        {
            var passed : Int = as3hx.Compat.parseInt(21 - model.time.hours);
            outputText("<b>" + cast((passed), Num2Text) + " " + ((passed > 1) ? "hours pass" : "hour passes") + "...</b>\n");
            model.time.hours = 21;
            statScreenRefresh();
        }
        ...
        if (sacrifice)
        {
            outputText(", tangled in your restraints");
        }
        outputText(".  She leans down and coos, ");
        //[if first time]
        if (flags[kFLAGS.TIMES_IN_DEMON_PIT] == 0)
        {
            outputText("\"<i>Awww, look at the poor little thing!  So this is what they gave us this time... Look at [him], everyone!  Do you know you'd be a cute pet?  People here call me Vapula, and I don't think you'll forget that name any time soon.</i>\"");
        }
        //[else if Vapula Submissiveness >50]
        else if (flags[kFLAGS.VAPULA_SUBMISSIVENESS] > 50)
        {
            outputText("\"<i>You again?  By Lethice, you must really enjoy it!  You love being a bitch for Vapula, don't you?  Yes you do!</i>\"");
        }
        //[else]
        else
        {
            outputText("\"<i>Fuck, you again?  Tell me it's so!  I've been looking forward to putting you in your place as my newest slut!</i>\"");
        }
        //[(if sacrificed)
        if (sacrifice)
    ...
    //Submit/Fight
    private function fightZeDemons(sacrifice : Bool = true) : Void
    {
        clearOutput();
    ...
    public function loseOrSubmitToVapula() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText(", followed by your " + vaginaDescript(0));
        }
        if (player.hasCock())
        {
            outputText(", before finally wrapping around your [cock] and stroking it; the friction uncontrollably arouses you, and you find yourself reaching full erectness");
        }
        ...
        //[(no Buttslut)
        if (!player.hasPerk(PerkLib.Buttslut))
        {
            outputText("No!  You aren't supposed to enjoy it...  ");
        }
        ...
        if (player.hasCock())
        {
            outputText("  It wraps around [eachCock] for a while, jerking it for a bit and rubbing its tip against yours.");
        }
        //[if antennae.type]
        ...
        {
            outputText("  You feel your horns being used as leverage to slap your head with even more rock-hard dick.  The impacts stun you until your whole forehead feels numb and coated with a mixture of ballsweat and pre-cum.");
        }
        //[if vagina
        if (player.hasVagina())
        {
            ...
            if (player.statusEffectv1(StatusEffects.BonusVCapacity) < 200)
            {
                player.addStatusValue(StatusEffects.BonusVCapacity, 1, 15);
            }
        ...
        if (player.hasCock())
        {
            ...
            {
                outputText("  She is soon joined by other horny girls, too eager to wait for a piece of your crotch to play with.  They all comfortably position themselves, positioning their pussy or anus atop each of your cocks before riding you like there's no tomorrow.");
            }
            //[if 4 to 7 cocks]
            if (player.cockTotal() >= 4)
            {
                outputText("  Nevertheless, they aren't satisfied with a single insertion and most grab hold of one of your extra cocks before forcefully stuffing it in their other hole, howling and thrashing all the more as they are penetrated a second time.");
            }
            //[if 7 cocks or more]
            if (player.cockTotal() >= 7)
            {
                ...
                {
                    outputText(" as well as your remaining pricks");
                }
        ...
        if (player.hasCock())
        {
            ...
            if (player.cumQ() > 1500)
            {
                outputText("  The effort appears to be vain, as the absurdly high volume you've ejected is enough to completely pack every hole; the remaining jism spills on the tainted ground, soaking it further.");
            }
        }
        ...
        if (player.hasVagina())
        {
            outputText(" and filling your womb to its maximum capacity");
        }
        ...
        if (!recalling)
        {
            ...
            if (flags[kFLAGS.VAPULA_SUBMISSIVENESS] >= 90 && flags[kFLAGS.VAPULA_SUBMISSIVENESS] < 100)
            {
            ...
            if (flags[kFLAGS.VAPULA_SUBMISSIVENESS] >= 100)
            {
                doNext(slaveToVapulaBadEnd);
            }
            else if (player.hasStatusEffect(StatusEffects.LostVillagerSpecial))
            {
    ...
    public function defeetVapulasHorde() : Void
    {
        ...
        if (monster.hasStatusEffect(StatusEffects.AttackDisabled))
        {
            ...
            {
                outputText("You grin wickedly as the last demons fall, defeated.  Some of the errant blows have broken and mangled the links of your chains, and you find you can get free with a bit of additional effort.");
            }
            //[if won by Lust]
            else
            {
                outputText("You grin wickedly as the demons give up the fight, too turned on to care about you.  One even begins unfastening your bindings, hopeful desperation glinting in her eyes as she attempts to entice you with her long, thick nipples and enormous, dripping gash.");
            }
        }
        //Not
        else
        {
            if (monster.HP <= monster.minHP())
            {
                outputText("You grin wickedly as the last demons fall, defeated.");
            }
            //[if won by Lust]
            else
            {
                outputText("You grin wickedly as the demons give up the fight, too turned on to care about you.  One even has hopeful desperation glinting in her eyes as she attempts to entice you with her long, thick nipples and enormous, dripping gash.");
            }
        }
    ...
    public function rapeZeVapula() : Void
    {
        ...
        function dickF() : Void
        {
            ...
            if (player.hasVagina())
            {
                outputText("You stick a finger in your own " + vaginaDescript() + " in order to lubricate it.  ");
            }
            ...
            if (monster.HP <= monster.minHP())
            {
                outputText("the many bruises she's received");
            }
            else
            {
                outputText("the constant flushing due to intense arousal");
            }
            ...
            if (player.hasVagina())
            {
                outputText("bitch in heat");
            }
            else
            {
                outputText("centaur in rut, abusing and filling her teasing cunt and ass with every protuberance you have");
            }
            ...
            if (player.cockTotal() > 1)
            {
                outputText("  Seeing that she has extra room left, you grab another of your dongs and put it at the entrance of her dripping cunt before forcing it in.");
            }
            ...
            if (player.cockTotal() > 1)
            {
                outputText(" and her love-tunnel");
            }
            outputText(".");
            //[if massive cum production]
            if (player.cumQ() > 1000)
            {
                outputText("  Some of it spurts by little jets outside of her, even as she instinctively contracts her muscles to keep as much spooge as possible and her belly accomodates by bloating out absurdly.");
            }
            ...
            if (player.hasBalls())
            {
                outputText("fondling your [balls], ");
            }
            //[if multicocks]
            if (player.cockTotal() > 1)
            {
                outputText("alternatively deepthroating or jerking every cock of yours, ");
            }
            ...
            if (player.cockTotal() > 1)
            {
                outputText("s harden and thicken ");
            }
            else
            {
                outputText(" hardens and thickens ");
            }
            ...
            if (player.cockTotal() > 1)
            {
                outputText("In an unholy sex-frenzy, you want more; you start grabbing creatures by groups and alternatively pump in and out of their asses with your [cock], always hungry for more anal.");
            }
            ...
            if (silly())
            {
                outputText("  As you look back, you snicker, \"<i>Ass for ass, assholes.</i>\"");
            }
            if (!recalling)
            {
                player.sexReward("vaginalFluids", "Dick");
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            {
                outputText("His cock is lost in your vast cunt and you work him as hard as you can to feel him against your walls, his body convulsing and dazed eyes rolling against the wet, savage rape.  ");
            }
            ...
            if (player.wetness() >= 5)
            {
                outputText("The sound of spattering fluid fills the air as your cunt spurts its juices around the incubus's cock, coating him in your fragrant slime.  ");
            }
            ...
            {
                outputText(" and your " + breastDescript(0) + " bounce gently");
            }
            ...
            {
                outputText("  Your freakish nipples moisten with excitement to the smell of man juice, and you feel your libido ratcheting up even higher.  Bending forward, you grab two other imps with smaller dicks, quickly guiding their lengths into your nipplecunts; they are small enough that you can mash their bodies against your soft flesh, doing all the work for them as your sensitive nubs are penetrated.");
            }
    ...
    private function owcaMainScreenOn() : Void
    {
        ...
        //[if attitude > 80]
        if (flags[kFLAGS.OWCAS_ATTITUDE] > 80)
        {
            outputText("Villagers greet you as you arrive, praising your courage and your heroism.  People shake your hands or ask you to kiss them, as if they hoped your nobility were somehow contagious.  Once they're done complimenting and flattering you in every way possible, they leave you to your business.");
        }
        //[if 60<=attitude<80]
        else if (flags[kFLAGS.OWCAS_ATTITUDE] > 60)
        {
            outputText("Villagers greet you with a friendly smile as you arrive.  Some approach you and thank you for what you've done, before leaving awkwardly.  They're obviously not used to strangers.");
        }
        //[if 40<=attitude<60]
        else if (flags[kFLAGS.OWCAS_ATTITUDE] > 40)
        {
            outputText("Villagers seem to accept your presence without comment now; though none go out of their way to greet you, neither do they avoid doing so when you pass by.");
        }
        //[if 20<attitude <= 40]
        else if (flags[kFLAGS.OWCAS_ATTITUDE] > 20)
        {
            outputText("Villagers keep looking at you uncomfortably; no one seems to be willing to talk to you apart from Rebecc, your friend.");
        }
        //[if attitude <= 20]
        else
        {
            outputText("Villagers here appear to be openly hostile.  You keep hearing muffled whispers as you pass by, and people look at you rancorously.  They don't bother you, though.");
        }
        ...
        if (flags[kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS] == 7 && flags[kFLAGS.OWCA_SACRIFICE_DISABLED] == 0)
        {
            outputText("  A week");
        }
        else if (flags[kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS] > 7 && flags[kFLAGS.OWCA_SACRIFICE_DISABLED] == 0)
        {
            outputText("  More than a week");
        }
        if (flags[kFLAGS.DAYS_SINCE_LAST_DEMON_DEALINGS] >= 7 && flags[kFLAGS.OWCA_SACRIFICE_DISABLED] == 0)
        {
            outputText(" has passed since the last offering to the demons; guarding the pit would certainly help improve your relations with the little town's denizens.");
        }
        ...
        {
            if (flags[kFLAGS.OWCA_SACRIFICE_DISABLED] == 0)
            {
                addButton(0, "Pit", zePit).hint("Guard the Pit tonight.");
            }
            else
            {
                addButtonDisabled(0, "Pit", "Horde disbanded.");
            }
        }
        if (flags[kFLAGS.OWCAS_ATTITUDE] >= 50)
        {
        ...
        {
            addButtonDisabled(1, "Herds", "Req. 50%+ attitude.");
        }
        ...
        {
            addButtonDisabled(3, "Tavern", "Req. 10%+ attitude.");
        }
        if (flags[kFLAGS.KINDRA_FOLLOWER] < 1)
        {
            addButton(10, "LookAround", meetKindra).hint("Wander around the village.");
        }
    ...
    public function meetKindra() : Void
    {
        ...
        else if (flags[kFLAGS.KINDRA_LVL_UP] >= 2 && flags[kFLAGS.KINDRA_LVL_UP] < 6)
        {
            outputText("As you wander the plains near Owca borders you find a warning sign again. It's obviously the feral sheep girl territory. You can either enter and fight her or leave.");
        }
        else
        {
            outputText("As you explore the plains near Owca borders you come upon a peculiar signpost with an area delimited by a line of burnt grass. The message is crudely written but somewhat clear ‘DO NOT ENTER’. You can either ignore the warning or leave what will you do?");
        }
        menu();
    ...
    public function returnToOwcaVillage() : Void
    {
        ...
        if (flags[kFLAGS.KINDRA_LVL_UP] == 6)
        {
            outputText("You don’t see a reason to annoy the sheep in front of her own house and decide to simply leave before she indeed start shooting arrows at you.");
        }
        else if (flags[kFLAGS.KINDRA_LVL_UP] >= 2 && flags[kFLAGS.KINDRA_LVL_UP] < 6)
        {
            outputText("You have no desire to engage the sheep archer right now and decide to simply head back to village.\n\n");
        }
        else
        {
    ...
    {
        if (item == consumables.SHEEPMK)
        {
            buyOwcaShit(item, 180 - flags[kFLAGS.OWCAS_ATTITUDE]);
        }
        else if (item == consumables.GOB_ALE)
        {
            buyOwcaShit(item, 60 - Math.round(flags[kFLAGS.OWCAS_ATTITUDE] / 2));
        }
        else if (item == consumables.BROBREW)
        {
            buyOwcaShit(item, 2000);
        }
        else
        {
            buyOwcaShit(item, 300 - flags[kFLAGS.OWCAS_ATTITUDE]);
        }
    }
    
    private function buyOwcaShit(bleh : ItemType, price : Float = 0) : Void
    {
        clearOutput();
        player.gems -= price;
        statScreenRefresh();
    ...
    private function rebeccBathScene() : Void
    {
        ...
        else
        {
            ...
            if (player.hasCock())
            {
                outputText("; your [cock] grows to full erect size");
            }
            if (player.hasVagina() && player.hasCock())
            {
                outputText(" and ");
            ...
            if (player.hasVagina())
            {
                outputText("your " + vaginaDescript() + " leaks a little trickle of girl-juice");
            }
    ...
    private function rapeRebecc(outside : Bool = false) : Void
    {
        clearOutput();
        sceneHunter.selectGender(dickF, vagF);
        //==========================
        function dickF() : Void
        {
            ...
            {
                outputText("  You keep burying more of your [cock] deep inside her rectum until you reach her limit and can't fit more cock in.");
            }
            else
            {
                outputText("  Eventually the entirety of your prick is lodged inside her lush asshole.");
            }
            ...
            if (player.hasBalls())
            {
                outputText(", and your [balls] almost feel sore from colliding with her dribbling cunt");
            }
            ...
            if (outside)
            {
                outputText("Some of the villagers are apathetically watching you, too stunned and bruised to intervene.  ");
            }
            ...
            if (outside)
            {
                outputText(" to these miserable people, and even moreso");
            }
    ...
    public function beatUpOwca() : Void
    {
        ...
        {
            rape = createCallBackFunction(rapeRebecc, true);
        }
    ...
    private function torchOwcaMotherFuckers() : Void
    {
        ...
        {
            doNext(torchUpVillagersAndLeave);
        }
    }
    ...
    public function loseToOwca() : Void
    {
        ...
        {
            outputText("your [weapon] is taken away and ");
        }
    ...
    private function forgiveOwca() : Void
    {
        ...
        if (player.cor >= 50)
        {
            outputText(", lying through your teeth,");
        }
    ...
    private function subdueVapula() : Void
    {
        ...
        if (monster.HP <= monster.minHP())
        {
            outputText("overwhelmed by your might");
        }
        else
        {
            outputText("rendered insensate by his own lust");
        }
    ...
    private function enslaveVapulaWithYourWang() : Void
    {
        ...
        if (player.lust < 0.8 * player.maxLust())
        {
            dynStats("lus", 0.8 * player.maxLust() - player.lust, "scale", false);
        }
        enslaveVapulaShared();
    }
