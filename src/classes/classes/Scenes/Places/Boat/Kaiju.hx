...
class Kaiju extends AbstractLakeContent
{
    ...
    {
        if (flags[kFLAGS.KAIJU_COCK] == 0)
        {
            spriteSelect(SpriteDb.s_venus);
        }
        else
        {
            spriteSelect(SpriteDb.s_venus_herm);
        }
    }
    ...
    public function kaijuMeeting() : Void
    {
        ...
        else if (flags[kFLAGS.FACTORY_SHUTDOWN] == 2 && !kaijuCock())
        {
            kaijuGrowsWangus();
        }
        else if (flags[kFLAGS.KAIJU_BAD_END_COUNTER] >= 5)
        {
            kaijuBadEndToyBOOSH();
        }
        else
    ...
    private function kaijuCock() : Bool
    {
    ...
    private function meetDatKaijuYo() : Void
    {
        ...
        if (player.gender > 0)
        {
            addButton(1, "Flirt", flirtWithKaiju);
        }
    ...
    private function corruptKaijuInsertion() : Void
    {
        ...
        outputText("\n\nYou come to your senses almost an hour later, unsure of how you survived such an aerial trip.  Perhaps she snatched you out of the air?  You find yourself in your boat back on shore, hair matted down and sticky with cum.");
    ...
    private function complimentKaiju() : Void
    {
        ...
        //[Let her/Stop it]
        menu();
        addButton(0, "Let Her", hugBoobs, false);
    ...
    //[If let her]
    private function hugBoobs(repeat : Bool) : Void
    {
        clearOutput();
        kaijuSprite();
        if (repeat)
        ...
        if (player.hasCock())
        {
            outputText(", [eachCock] stiffening in your [armor] from the stimulation as your whole body is used in what seems more and more like a massive boob job");
        }
        outputText(".");
        if (player.hasVagina())
        {
            outputText("  Your [vagina] moistens from your body's growing arousal.");
        }
        ...
        if (player.gender == 0)
        {
            outputText("bare groin");
        }
        outputText(".  \"<i>Let's have some fun!</i>\"");
        sceneHunter.selectGender(dickF, vagF, ["Mouth", glessF]);
        ...
        function dickF() : Void
        {
            ...
            if (player.hasBalls())
            {
                outputText("  Your [balls] begin to ache as you slowly empty your load onto green flesh.");
            }
    ...
    private function flirtWithKaiju() : Void
    {
        ...
        if (player.sens > 50)
        {
            ...
            if (player.hasCock())
            {
                outputText("  [EachCock] spasms as you release your load onto her pussy walls, making them just a bit more slick with your contribution.");
            }
            if (player.hasVagina())
            {
                outputText("  Your own pussy releases its own feminine juices, soaking your thighs even further in wetness.");
            }
        ...
        if (kaijuCock())
        {
            ...
            if (player.gender > 0)
            {
                ...
                if (player.biggestTitSize() >= 1)
                {
                    outputText("  You press your [chest] against her cock flesh as well, giving her penis as much a boob job as such a behemoth prick can be given.");
                }
                if (player.lactationQ() >= 200)
                {
                    outputText("  Milk spurts and flows from your [nipples], making your breast slick and shiny and helping you slide more easily along the throbbing meat-pole.");
                }
            }
    ...
    private function fuckThisGiantYouDumbCunt() : Void
    {
        ...
        outputText("\n\nJust when you begin to think you can see her womb her hand drags you back down her slippery passage, before pushing you back up again.  Her fingers push you up and down, in and out of her trembling pussy, her wet and skillful muscles helping to speed you along as she steadily picks up her pace.  The closer she seems to get to climax, the more her pussy walls tremble and push, the more her juices gather, and the faster she sends you on your dizzying roller coaster ride through her female sex.");
        //[If PC's sensitivity is 50 or over]
        if (player.sens >= 50 && player.lib < 50)
        
        //added a second check so it won't be one-way lock{
        ...
        else
        {
            ...
            if (kaijuCock())
            {
                ...
                if (player.biggestTitSize() >= 1)
                {
                    outputText("You press your [fullChest] against her cock flesh as well, giving her penis as much a boob job as such a behemoth prick can be given.");
                }
                if (player.lactationQ() >= 200)
                {
                    outputText("  Milk spurts and flows from your [nipples], making your breast slick and shiny and helping you slide more easily along the throbbing meat-pole.");
    ...
    //[If Urethra Fuck]
    private function urethraFuckDatGiantCock(fromTF : Bool = false) : Void
    {
        if (!fromTF)
        {
            clearOutput();
        ...
        if (player.hasCock())
        {
            outputText("  So much jizz fills your ass it can’t help but to press in on your prostate, making [eachCock] stiffen with a seriously growing urge.");
        }
        if (player.hasVagina())
        {
            outputText("  Your ass isn’t alone in the feeling of being flooded as your [vagina] and womb are packed with the hermaphrodite’s sperm.");
        }
    ...
    private function talkToKaiju() : Void
    {
        ...
        else if (flags[kFLAGS.KAIJU_TALK_CYCLE] == 2)
        {
            ...
            {
                outputText("  But the flow of yummy fluids seems to have been stopped.");
            }
            //[If the Factory was shut down and Marae corrupted]
            else if (kaijuCock())
            {
                outputText("  But the pouring of liquids has been stopped for some reason, but not before there was one final, huge burst of tainted fluids. It was so strong and changed me even further, making me grow this,</i>\" she says as she rises further out of the lake's waters, revealing a large green cock.");
            }
    ...
    private function peekAtSomePhatAssKaijuButt() : Void
    {
        ...
        if (!kaijuCock())
        {
            outputText(" while her thumb begins to rub her clit");
        }
        else
        {
            outputText(" while her thumb begins rubbing up and down her slowly stiffening cock");
        }
    ...
    private function mockDatTurtleGirl() : Void
    {
        ...
        if (player.hasBalls() && player.ballSize >= 6)
        {
            outputText("Your [balls] rest on the hermaphrodite's hand, bouncing loudly with each thunderous stroke from the speed and force she is employing to get herself off.  ");
        }
        //[end balls] [If player has cock]
        if (player.hasCock())
        {
            outputText("You can feel your [armor] getting tighter as [eachCock] begins to stiffen under your clothing from all the constant pressure of the cock squeezing down on your member.  ");
        }
        //[If player has vagina]
        if (player.hasVagina())
        {
            outputText("You begin to feel a wetness between your legs as your cunt begins leaking as your arousal spikes from the intense rubbing the herm's penis is giving your whole body.  ");
        }
        //[If genderless]
        if (player.gender == 0)
        {
            outputText("Your cheeks blush in arousal from all this heavy rubbing.  ");
        }
        if (player.cor < 33)
        {
            outputText("You can't believe that you're being aroused by this.  Aren't you supposed to be a champion?");
        }
        else if (player.cor < 66)
        {
            outputText("Wow, this is arousing you.  You wonder if you really are a champion.");
        }
        else
        {
            outputText("You just want to get off now.");
        }
        ...
        if (player.hasCock())
        {
            outputText(" as you cum in your [armor]");
        }
    ...
    private function helpNewFutaKaijuGetOff() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("  [EachCock] quickly hardens now as it is rubbed up against the giant's massive shaft.");
        }
        if (player.hasVagina())
        {
            outputText("  The warm heat that radiates off of the titanic member seems to seep into your nether region, making your [vagina] leak out moisture onto the massive prick.");
        }
        ...
        if (player.cor < 50 && player.lib < 75)
        {
            outputText("  You knead the cock head with your hands, massaging it quickly and roughly as you grind up against it, rock back and forth on the giant member.  Some of the pre dribbles down, and you begin to use it as sticky massage oil, working it into the cock flesh.  The cooing and lustful moaning coming from the giantess increases in intensity with your every movement.");
        }
        else
        {
        ...
        if (player.lactationQ() >= 200)
        {
            outputText("  Milk begins to dribble from your [nipples], making the cock head slick and allowing you to wriggle your body against it more smoothly.");
        }
    ...
    private function yesTurnKaijuFuta() : Void
    {
        ...
        for (i in 0...10)
        {
            ...
            else if (player.hasItem(consumables.INCUBID))
            {
    ...
    private function badEndPartTwo() : Void
    {
        ...
        {
            outputText("  Your snaky forked tongue begins lashing about, flicking at the nipple tip in your mouth while rubbing at the underside of it, urging the tender spot to give up a drop of the heavenly white liquid.");
        }
        ...
        if (kaijuCock())
        {
            ...
            if (player.hasCock())
            {
                outputText("  Your prostate is pressed down like a button, and there’s just no letting up!");
            }
            if (player.hasVagina())
            {
                outputText("  Even if your cunt is empty, there is no free space left there, your cunt walls forced together like glue.");
            }
        ...
        if (player.hasCock())
        {
            outputText("[EachCock] stiffens as the tree tendril pushes against your prostate, forcing a thick glob of pre up your shaft.  ");
        }
        if (player.hasVagina())
        {
        ...
        if (player.biggestTitSize() >= 1)
        {
            outputText("  Another tendril snakes its way up your abdomen to nestle between your [chest], slicking your mounds with pre as it slides between your tits, giving itself a titty-fuck with your titties!");
        }
        ...
        if (player.hasCock())
        {
            outputText("  More tentacles arrive and begin to wrap around your [cocks], slowly jacking you off to the tempo set by the cocks in your ass and mouth, wracking you with further pleasure.");
        }
