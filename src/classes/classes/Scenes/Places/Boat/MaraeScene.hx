...
class MaraeScene extends AbstractBoatContent implements TimeAwareInterface
{
    ...
    public function timeChange() : Bool
    {
        if (model.time.hours > 23)
        {
            ...
            if (flags[kFLAGS.MARAE_QUEST_COMPLETE] == 1)
            ...
            //Pure Holli version{
                ...
                //If flower hasn't been burned down yet{
                    ...
                    if (flags[kFLAGS.FLOWER_LEVEL] < 4 && flags[kFLAGS.FUCK_FLOWER_GROWTH_COUNTER] < 1000)
            ...
            if (flags[kFLAGS.HOLLI_FUCKED_TODAY] == 1)
            {
                flags[kFLAGS.HOLLI_FUCKED_TODAY] = 0;
            }
        }
        return false;
    }
    ...
    public function canVisitCor() : Bool
    {
    ...
    public function visitSelector() : Void
    {
        ...
        //until you steal Lethicite
        else if (canVisitPure())
        {
            encounterPureMaraeEndgame();
        }
        else if (canVisitCor())
        {
            if (flags[kFLAGS.CORRUPT_MARAE_FOLLOWUP_ENCOUNTER_STATE] > 0 && enoughPerks())
            {
                level3MaraeEncounter();
            }
            //only when all perks are received.
            else
            {
    ...
    public function enoughPerks() : Bool
    {
        ...
        //First meeting{
            ...
            if (flags[kFLAGS.MET_MARAE] <= 0 && flags[kFLAGS.MARAE_QUEST_START] <= 0)
            {
                ...
                if (player.cor > 66 + player.corruptionTolerance)
                {
                    ...
                    if (player.hasPerk(PerkLib.BlessingOfTheAncestorTree) || player.hasPerk(PerkLib.Soulless) || (player.hasPerk(PerkLib.Phylactery) && !player.hasPerk(PerkLib.InnerPhylactery)) || player.isRaceCached(Races.FMINDBREAKER) || player.isRaceCached(Races.MMINDBREAKER, 2) || player.isRaceCached(Races.ATLACH_NACHA, 3))
                    {
                ...
                else
                {
                    ...
                    {
                        outputText("Your eyes can't help but take note of the tree-woman's shapely breasts, and wonder if they feel like tits or wood.  ");
                    }
            ...
            else
            {
                ...
                if (player.cor > 66 + player.corruptionTolerance)
                {
                    ...
                    if (player.level >= 130)
                    {
                        outputText(" Of course, you could probably try to overthrow her.");
                    }
                    endEncounter();
                }
    ...
    private function alraunezeMeYes() : Void
    {
        ...
        if (flags[kFLAGS.FACTORY_SHUTDOWN] == 2)
        {
            ...
            {
                outputText("smooth flesh of the goddess");
            }
            outputText(". Marae glances down knowingly ");
            if (!player.isNaked())
            {
                outputText("and begins to undo your gear, tossing it aside");
            }
            else
            {
                outputText("regarding you");
            }
            ...
            {
                outputText("The rounded tip nuzzles against you through your -lower armor descript- like a familiar friend, but quickly angles itself to slip inside causing you to jump a little in shock at the way it now presses against your bare flesh. ");
            }
    ...
    public function loseAgainstMarae() : Void
    {
        ...
        if (flags[kFLAGS.FACTORY_SHUTDOWN] == 2)
        {
            if (player.HP <= 0)
            {
                outputText("You collapse, too weak to continue fighting. You know that your journey will be over. ");
            }
            else
            {
                outputText("Your desire to keep fighting has been slain by your overwhelming lust and you collapse. You know that your journey will be over. ");
            }
            doNext(maraeBadEnd);
        }
        else
        {
            if (player.HP <= 0)
            {
                outputText("You collapse, too weak to continue fighting. \"<i>Get some rest, champion,</i>\" Marae says. You finally black out. \n\nBy the time you wake up, you find yourself in your camp.");
            }
            else
            {
                outputText("Your desire to keep fighting has been slain by your overwhelming lust and you collapse. \"<i>Control your urges and get some rest, champion,</i>\" Marae says. You finally black out from your lust. \n\nBy the time you wake up, you find yourself in your camp.");
            }
            cleanupAfterCombat();
        }
    }
    ...
    public function winAgainstMarae() : Void
    {
        ...
        if (flags[kFLAGS.FACTORY_SHUTDOWN] == 2)
        {
            ...
            if (player.hasStatusEffect(StatusEffects.KnowsWhitefire) || player.hasPerk(PerkLib.FireLord) || player.hasPerk(PerkLib.Hellfire) || player.hasPerk(PerkLib.DragonFireBreath))
            {
            ...
            {
                outputText("You raise your [weapon] and finish off Marae at last. The corrupted goddess is no more. All the tentacles shrivel up. Afterwards, you turn your attention elsewhere.");
            }
            if (player.str < 40)
            {
                outputText("You scream as your hand hits the chitin. Maybe it's because you're not strong enough? ");
            }
            if (player.str >= 40 && player.str < 70)
            {
                outputText("In spite of your decent strength, the bark refuses to bend or break. ");
            }
            if (player.str >= 70)
            {
                outputText("Despite your incredible strength, the bark refuses to bend or break. ");
            }
        ...
        else
        {
            if (monster.HP <= 0)
            {
                outputText("Marae reels back from the incredible amount of damage you've dealt to her.");
            }
            else
            {
                outputText("Marae clearly shows signs of her overwhelming arousal and reels back.");
            }
    ...
    private function maraeBadEnd() : Void
    {
        ...
        {
            outputText("The goddess flows out of the tree, stepping away from it as a living woman, curvy and nude.\n\n");
        }
        ...
        if (player.gender == 0)
        {
            outputText(" where it pauses in momentary confusion");
        }
        ...
        {
            outputText("Any thought or need to drink Marble's milk vanishes from your mind and body.  ");
        }
        ...
        if (player.hasCock())
        {
            ...
            {
                var i : Int = as3hx.Compat.parseInt(player.cocks.length - 1);
                while (i >= 0)
                {
    ...
    private function maraeStealLethicite(deliberate : Bool = false) : Void
    {
        spriteSelect(SpriteDb.s_marae);
        clearOutput();
        //(SUCCESS)
        ...
        else
        {
            ...
            var studCheck : Bool = player.hasCock() && (!player.hasPerk(PerkLib.MaraesGiftStud) || !player.hasVagina() || recalling);
            var fertCheck : Bool = (player.hasVagina() || player.gender == 0) && (!player.hasPerk(PerkLib.MaraesGiftFertility) || !player.hasCock() || recalling);
            if (sceneHunter.lossSelect)
            {
                ...
                if (!player.hasCock())
                {
                    addButtonDisabled(0, "Stud", "You lack the required parts for this one.");
                }
                else
                {
                    addButtonIfTrue(0, "Stud", stud, "You already have it.", studCheck);
                }
                if (!player.hasVagina())
                {
                    addButtonDisabled(1, "Fertility", "You lack the required parts for this one.");
                }
                else
                {
                    addButtonIfTrue(1, "Fertility", fertility, "You already have it.", fertCheck);
                }
            }
            else if (studCheck && (!fertCheck || rand(2) == 0))
            {
                stud();
            }
            else
            ...
            function stud() : Void
            {
                ...
                if (!recalling)
                {
                    ...
                    if (!WoodElves.WoodElfMagicTranerGetLaid)
                    {
                        WoodElves.WoodElfMagicTranerGetLaid = true;
                    }
                    endEncounter(120);
                }
                else
    ...
    public function level2MaraeEncounter() : Void
    {
        ...
        if (player.cor < 33)
        {
            outputText("Knowing all too well what kinds of horrors await");
        }
        else if (player.cor < 66)
        {
            outputText("Knowing all too well what kinds of sexual escapades lurk ahead");
        }
        else
        {
            outputText("Worried about enjoying the pleasures of the island overmuch");
        }
        ...
        if (player.spe < 50)
        {
            outputText("dropping you flat on your ass");
        }
        else
        {
            outputText("nearly dropping you flat on your ass before you catch yourself and sit down");
        }
    ...
    private function MaraeIIStageII() : Void
    {
        ...
        function vagF() : Void
        {
            ...
            {
                outputText("at your crotch");
            }
            else
            {
                outputText("against your belly");
            }
        ...
        }  //Marae uses a tree-tentacle to 'milk' male PC's.  Oral or Vajayjay? Not sure.    //[Dudezillaz]  ;
        ...
        function dickF() : Void
        {
            ...
            {
                outputText(" while your other dangles against her thigh");
            }
            else if (player.cockTotal() > 2)
            {
                outputText(" while your others dangle against her thighs");
            }
            ...
            if (player.cockTotal() == 2)
            {
                outputText("  Another vine that may as well be the first's twin snakes from between the goddess' legs and jumps onto your " + cockDescript(1) + ".");
            }
            else if (player.cockTotal() > 2)
            {
                outputText("  More 'open' vines shimmy forth from between Marae's legs and jump up onto your " + Appearance.cockNoun(CockTypesEnum.HUMAN) + "s.");
            }
        ...
        var hermF : Void->Void = function() : Void
        {
            ...
            {
                outputText(", slipping inside your cunt-like nipples with ease.  You moan into her ear at the unexpected penetration and shiver from the tingling shocks of pleasure in your " + breastDescript(0) + ".");
            }
            else
            {
                outputText(".");
            ...
            if (player.cockTotal() == 2)
            {
                outputText(" while your other cock rubs over her surface");
            }
            else if (player.cockTotal() > 2)
            {
                outputText(" while your other cocks rubs over her surface");
            }
            outputText(".\n\n");
            ...
            if (player.cocks[0].cockThickness > 5 || player.biggestCockArea() > 100)
            {
                outputText("With how big you are, there's no way it should be able to fit, but her body isn't even distorted by your girth.  Perhaps she changed you to fit her?  You pull back and your thickness seems unchanged.  You shake your head to clear the unwelcome thoughts and ram yourself back into her.  Fucking is what's important.  ");
            }
            else
            {
                outputText("She feels perfect.  A velvet vice of hot, slippery wetness clutches tightly around your [cock].  It almost feels like it's actually gripping you, cradling your cock in her ambrosia-slicked box.");
            }
            if (player.cockTotal() > 1)
            {
                if (player.cockTotal() > 2)
            ...
            {
                outputText("Sticky wetness glistens between your thighs");
            }
            else if (player.vaginas[0].vaginalWetness < VaginaClass.WETNESS_DROOLING)
            {
                outputText("Drops of feminine arousal run down your thighs");
            }
            else
            {
                outputText("Trails of viscous feminine fluid leak from your " + vaginaDescript(0));
            }
            ...
            if (player.cockTotal() > 1)
            {
                outputText("Her asshole is squelching and dripping from your " + cockDescript(1) + ", actually squirting more lubricant than her pussy from the violent fucking!  ");
            }
            ...
            if (player.hasBalls())
            {
                outputText("rapidly contracting balls");
            }
            else
            {
                outputText("tentacle-squeezed prostate");
            }
            ...
            if (player.cockTotal() > 1)
            {
                outputText("The " + cockDescript(1) + " in her ass spasms and explodes with its brother, glazing her slippery colon with a coating of syrupy spunk.  ");
            }
            if (player.cockTotal() > 2)
            {
            ...
            if (player.cumQ() < 500)
            {
                outputText("The orgasm is so much more massive than normal, and you wonder if her magic enhanced it.  ");
            }
            if (player.cumQ() > 1000)
            {
                outputText("She's actually surprised when you keep fountaining more seed into her.  Her nipples start to squirt out the excess seed, but you keep cumming until she's squirted enough to soak herself with your jism.  ");
            }
