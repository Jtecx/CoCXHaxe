...
class ThiefScene extends BaseContent
{
    ...
    private function giveGems() : Void
    {
        ...
        if (player.gems <= 0)
        {
            outputText(", only to find it empty and tell the thief that you're poor. \"<i>Damn! Looks like I've picked the wrong person,</i>\" the thief says with a grumbled look. He lets go of you and saunters off.");
        }
    ...
    public function winAgainstThief() : Void
    {
        ...
        if (flags[kFLAGS.THIEF_GEMS] > 0)
        {
            outputText(", happily retrieving the gems the thief has taken from you");
        }
        outputText(".");
        ...
        if (player.lust >= 33)
        {
            ...
            if (player.hasCock())
            {
                ...
                {
                    addButton(0, "Anal Fuck", rapeThiefAnally).hint("The thief definitely needs to learn a lesson not to mess with you. Buttfuck the thief.");
                }
                else
                {
                    outputText(" <b>Your cock is too big to fit!</b>");
                }
            }
            if (player.hasVagina())
            {
                addButton(1, "Get Licked", getLicked).hint("Punish the thief by having him lick your [vagina].");
            }
        }
    ...
    private function getLicked() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(" while your [cock] fires ropes of jism");
        }
