...
class Masturbation extends BaseContent
{
    public static function canMeditate() : Bool
    {
        var religious : Bool = (player.hasPerk(PerkLib.HistoryReligious) || player.hasPerk(PerkLib.PastLifeReligious))
        && player.cor <= 66 + player.corruptionTolerance;
        var enlightened : Bool = player.hasPerk(PerkLib.Enlightened)
        && player.cor <= 10 + player.corruptionTolerance;
        return (religious || enlightened)  /*&& !SceneLib.exgartuan.anyAwake()*/  ;
    }
    
    public function masturButton(pos : Int) : CoCButton
    {
        if (inDungeon && DungeonAbstractContent.dungeonLoc != DungeonAbstractContent.DUNGEON_CABIN || inRoomedDungeon || explorer.isActive)
        {
            return addButton(pos, "Masturbate", masturbateGo).hint("Attempt to masturbate in order to relieve your lust buildup.").disableIf(player.lust < 33, "You're not horny enough to masturbate.");
        }
        else if (canMeditate() && player.lust < 33)
        {
            return addButton(pos, "Meditate", meditate).hint("Meditate in order to reduce lust and corruption.").disableIf(SceneLib.exgartuan.anyAwake(), "Your inner demon won't let you concentrate right now.");
        }
        else
        {
            return addButton(pos, "Masturbate", masturbateMenu).hint("Attempt to manually masturbate in order to relieve your lust buildup."
                            + ((canMeditate()) ? "  You can also try to meditate instead of masturbating." : "")).disableIf(player.lust < 33, "You're not horny enough to masturbate.");
        }
    }
    
    private function masturGasm(args : Array<Dynamic> = null) : Void
    {
        if (args.length == 0)
        {
            if (player.hasCock())
        ...
        if (player.hasPerk(PerkLib.ElectrifiedDesire) || player.hasStatusEffect(StatusEffects.RaijuLightningStatus))
        {
            ...
            for (i in 0...args.length)
            {
                ...
                {
                    player.orgasmRaijuStyle(args[i]);
                }
            }
            dynStats("sen", 0.5);
        }
    ...
    public function masturbateMenu() : Void
    {
        ...
        if (player.hasCock() && (player.cocks[0].cockType == CockTypesEnum.BEE))
        {
            ...
            if (canMeditate())
            {
                ...
                addButton(10, "Meditate", meditate).disableIf(SceneLib.exgartuan.anyAwake(), "Your inner demon won't let you concentrate right now.");
            }
            return;
        }
        else
        {
            sceneHunter.print("Check failed: bee-cock.");
        }
        //normal menu
        menu();
        if (canMeditate())
    ...
    private function specialOptions(page : Int = 1) : Void
    {
        ...
        if (page == 1)
        {
            addButton(0, "Lick Cock", catAutoLick).disableIf(!player.hasPerk(PerkLib.Flexibility) && flags[kFLAGS.TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY] == 0, "<b>You're not flexible enough to try this.</b>" +
                            "\n\nRequires 'Flexibility' (cat-morph) perk.").disableIf(!player.hasCock(), "You can't suck lick cock without having one.");
            addButton(1, "Lick Pussy", lickYerGirlParts).disableIf(!player.hasPerk(PerkLib.Flexibility) && flags[kFLAGS.TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY] == 0, "<b>You're not flexible enough to try this.</b>" +
                            "\n\nRequires 'Flexibility' (cat-morph) perk.").disableIf(!player.hasVagina(), "You can't lick your vagina without having one.");
            addButton(2, "EggsInCock", getHugeEggsInCawk).disableIf(player.findCock(1, 100, -1) < 0, 
                            "Req. a cock with area larger than 100"
                ).disableIf(!player.canOvipositBee(), "Req. a bee ovipositor.");
            addButton(3, "EggsInTits", layEggsInYerTits).disableIf(player.biggestTitSize() < 21, "You would need bigger tits for this.").disableIf(!player.canOviposit(), "Req. an ovipositor.");
            addButton(4, "TentacleFun", tentacleSelfFuck2).hint("Fuck yourself with your tentacle limbs!").disableIf(!player.hasVagina(), "You need a vagina to 'have fun' with.").disableIf(!player.isScylla() && !player.isKraken(), "Requires to have Scylla or Kraken lower body.");
            //TODO: make sure stamens also work!
            //TODO: stamens are copypasted... right?
            addButton(5, "TentInPussy", tentacleSelfFuck).hint("Fuck your pussy with your tentacle cocks!").disableIf(!player.hasVagina(), "You would need a pussy first.").disableIf(player.countCocksWithType(CockTypesEnum.TENTACLE) == 0, "Req. tentacle cocks");
            addButton(6, "TentInButt", tentacleGoesUpYerPooperNewsAtEleven).hint("Fuck your ass with your tentacle dicks!").disableIf(player.countCocksWithType(CockTypesEnum.TENTACLE) == 0, "Req. tentacle cocks");
            addButton(13, "-2-", specialOptions, page + 1);
            addButton(14, "Back", masturbateMenu);
        }
        if (page == 2)
        {
            addButton(0, "LayBunnyEggs", EasterBunnyLayEggs).disableIf(player.ballSize <= 3, "Your balls are too small for this.").disableIf(player.balls < 2, "You would need at least two balls.").disableIf(!player.hasCock(), "You also need a cock.").disableIf(!player.hasPerk(PerkLib.EasterBunnyBalls), "Requires to have Easter Bunny balls.");
            addButton(1, "StamenPussy", stamenSelfFuck).hint("Fuck your pussy with your stamens!").disableIf(!player.hasVagina(), "You would need a vagina first.").disableIf(player.countCocksWithType(CockTypesEnum.STAMEN) == 0, "Req. stamen cocks").disableIf(!player.isAlraune(), "Req. to be an Alraune.");
            addButton(2, "StamenButt", stamenGoesUpYerPooperNewsAtEleven).hint("Fuck your ass with your stamens!").disableIf(player.countCocksWithType(CockTypesEnum.STAMEN) == 0, "Req. stamen cocks").disableIf(!player.isAlraune(), "Req. to be an Alraune.");
            addButton(3, "TwinYuri", twinYuri).hint("Fuck your vagina with your stamens!").disableIf(!player.hasVagina(), "You would need a vagina first.").disableIf(!player.isLiliraune(), "Req. to be a Liliraune.");
            addButton(4, "Impregnator", gobomechImpregnator1).disableIf(player.keyItemvX("Cum Reservoir", 1) != 4, "Your cum reservoir is not full yet!").disableIf(!player.isInGoblinMech(), "You need to be inside a goblin mech for this!").disableIf(!player.hasVagina(), "First of all, you need a vagina.");
            addButton(5, "Fuck Machine", gobomechFuckingMachine).disableIf(player.keyItemvX("Cum Reservoir", 1) != 4, "Your cum reservoir is not full yet!").disableIf(!player.isInGoblinMech(), "You need to be inside a goblin mech for this!").disableIf(!player.hasVagina(), "First of all, you need a vagina.");
            addButton(6, "Goo Sat", gooeySatisfaction).hint("Play with your malleable body in a new way.", "Gooey Satisfaction").disableIf(!player.isGoo(), "Req. goo body.");
            addButton(7, "Mummy Sex", mummySex).disableIf(player.gender == 0, "Req. to not be genderless.").disableIf(player.soulforce < Math.round(player.maxSoulforce() * 0.5), "Req. 50% of max soulforce.").disableIf(player.racialScore(Races.ANUBIS) < 15, "Req. to be Anubi race.").disableIf(player.perkv2(PerkLib.MummyLord) > 0, "Last mummy sex effect not yet wear off.").disableIf(player.hasPerk(PerkLib.MummyLord) && player.perkv1(PerkLib.MummyLord) < 3, "Req. to have 3+ mummies.");
    ...
    private function fappingItems() : Void
    {
        ...
        //for dildos{
            button(0 + b * 5).disableIf(player.isTaur() && player.longestCockLength() < player.tallness * 5 / 6, 
        ...
        //dildos
        addKAbutton(1, "Dildo (Vag)", "Dildo", stickADildoInYourVagooSlut).disableIf(!player.hasVagina(), "Req. a vagina.");
        addKAbutton(6, "Dildo (Ass)", "Dildo", dildoButts);
        addKAbutton(11, "D. Dildo", "Deluxe Dildo", deluxeDildo).disableIf(player.isTaur(), "").disableIf(!player.hasVagina(), "Req. a vagina.");
        for (b in 0...3)
        
        //for belts{
            button(1 + b * 5).disableIf(player.isTaur(), "You can't reach your lower bits with your current body!");
        }
        //belts
        ...
        addKAbutton(12, "Dual Belt", "Dual Belt", dualBeltMasturbation).disableIf(!player.hasCock(), "Req. a cock too!");  //only for herms  
        for (b in 0...3)
        
        //for belts{
            button(2 + b * 5).disableIf(!player.hasVagina(), "Req. a vagina.").disableIf(player.isTaur(), "You can't put a belt on your taur-like body!");
        }
        //taur tools
        addKAbutton(3, "Fake Mare", "Fake Mare", centaurDudesGetHorseAids).disableIf(!player.hasCock(), "Req. a cock.");
        addKAbutton(8, "C. Pole", "Centaur Pole", centaurGirlsGetHorseAids).disableIf(!player.hasVagina(), "Req. a vagina.");
        for (b in 0...2)
        
        //for belts{
            button(3 + b * 5).disableIf(!player.isTaur(), "You need to be a taur to use it!");
        }
        //'no item' disables
        for (b in 0...15)
        {
    ...
    public function doStripCheck() : Void
    {
        ...
        else if (flags[kFLAGS.IN_INGNAM] != null || flags[kFLAGS.CAMP_BUILT_CABIN] <= 0)
        {
            if (player.cor < 15)
            {
                ...
                if (player.armor == armors.GOOARMR)
                {
                    ...
                    {
                        outputText("you remove your goo-soaked [lowerGarment] then ");
                    }
                    outputText("you reach your hand into your goo-covered groin.");
                }
                else if (player.armor == armors.LTHCARM && player.lowerGarment.name == "nothing")
                {
                    outputText("you reach for the intricately-decorated opening in your lethicite armor to access your groin.");
                }
                else
                {
                    outputText("you remove your [armor]");
                }
                outputText("\n\n");
            }
            ...
            if (player.cor >= 30 && player.cor < 60)
            {
                ...
                if (player.armor == armors.GOOARMR)
                {
                    if (!player.lowerGarment.isNothing)
                    {
                        outputText("remove your goo-soaked [lowerGarment] and ");
                    }
                    outputText("reach your hand into your goo-covered groin");
                }
                else if (player.armor == armors.LTHCARM && player.lowerGarment.name == "nothing")
                {
                    outputText("reach for the intricately-decorated opening in your lethicite armor to access your groin.");
                }
                else
                {
                    outputText("remove your [armor]");
                }
            ...
            if (player.cor >= 60 && player.cor < 80)
            {
                ...
                if (player.armor == armors.GOOARMR)
                {
                    if (!player.lowerGarment.isNothing)
                    {
                        outputText("remove your goo-soaked [lowerGarment] and you ");
                    }
                    outputText("reach your hand into your goo-covered groin");
                }
                else if (player.armor == armors.LTHCARM && player.lowerGarment.name == "nothing")
                ...
                {
                    outputText("strip naked in an exaggerated fashion");
                }
            ...
            if (player.cor >= 80)
            {
                ...
                {
                    outputText("fondling your naughty bits as you do so and ");
                }
        ...
        else
        {
            if (player.cor < 15)
            {
                ...
                {
                    outputText("You then proceed to remove your [armor].");
                }
                outputText("\n\n");
            }
            if (player.cor >= 15 && player.cor < 30)
            ...
            if (player.cor >= 30 && player.cor < 60)
            {
                ...
                {
                    outputText("You happily remove your [armor].");
                }
                outputText("\n\n");
            }
            if (player.cor >= 60 && player.cor < 80)
            {
                ...
                {
                    outputText("You eagerly remove your [armor].");
                }
                outputText("\n\n");
            }
            if (player.cor >= 80)
            {
                ...
                {
                    outputText("You eagerly remove your [armor] in an exaggerated fashion.");
                }
                outputText("\n\n");
            }
        }
    ...
    public function masturbateGo() : Void
    {
        ...
        if (SceneLib.exgartuan.dickAwake() && (!SceneLib.exgartuan.boobsAwake() || rand(2) == 0))
        {
            ...
            if (!player.isNaga())
            {
                sceneHunter.print("Check failed: Naga.");
            }
            if (player.isNaga() && rand(2) == 0)
            {
                SceneLib.exgartuan.exgartuanNagaStoleMyMasturbation();
            }
            else
            {
                SceneLib.exgartuan.exgartuanMasturbation_dick();
            }
            return;
        }
        else
        {
            sceneHunter.print("Check failed: Exgartuan.");
        }
        ...
        {
            sceneHunter.print("Check failed: Xenora.");
        }
        ...
        {
            sceneHunter.print("Check failed: Gilded cock socks");
        }
        if (inDungeon && DungeonAbstractContent.dungeonLoc != DungeonAbstractContent.DUNGEON_CABIN && player.companionsInPCParty())
        {
            ...
            if (flags[kFLAGS.PLAYER_COMPANION_3] != "" && rand(2) == 0)
            {
                outputText("" + flags[kFLAGS.PLAYER_COMPANION_3] + "");
            }
            else if (flags[kFLAGS.PLAYER_COMPANION_2] != "" && rand(2) == 0)
            {
        ...
        var autofellatio : Bool = false;
        var nippleFuck : Bool = false;
        //Early prep
        doStripCheck();
        //Tit foreplay
        titForeplay();
        ...
        if (player.cocks.length > 0)
        {
            if (player.cocks[0].cockType == CockTypesEnum.BEE && (player.hasItem(consumables.BEEHONY) || player.hasItem(consumables.PURHONY) || player.hasItem(consumables.SPHONEY)))
            {
                ...
                {
                    player.consumeItem(consumables.BEEHONY, 1);
                }
                else if (player.hasItem(consumables.PURHONY))
                {
                    player.consumeItem(consumables.PURHONY, 1);
                }
                else if (player.hasItem(consumables.SPHONEY))
                {
                    player.consumeItem(consumables.SPHONEY, 1);
                }
            ...
            if (player.cocks.length == 1)
            {
                ...
                {
                    outputText(" teasingly, pre-cum running down your length from your constant state of arousal.  ");
                }
            }
            else
            {
                ...
                {
                    outputText(" teasingly, pre-cum running down your cocks from your constant state of arousal, pooling around you.  ");
                }
            }
        }
        if (player.vaginas.length > 0)
        {
            ...
            //0 = dry, 1 = wet, 2 = extra wet, 3 = always slick, 4 = drools constantly, 5 = female ejaculator{
                ...
                {
                    outputText("You touch your enflamed and aroused [vagina], ");
                }
                var _sw4_ = (player.vaginas[0].vaginalWetness);                

                switch (_sw4_)
                {
                    case VaginaClass.WETNESS_DRY:
                        outputText("expertly arousing your female parts.  ");
                    case VaginaClass.WETNESS_NORMAL:
                        outputText("sighing as it quickly becomes moist.  ");
                    case VaginaClass.WETNESS_WET:
                        outputText("giggling as your fingers get a little wet.  ");
                    case VaginaClass.WETNESS_SLICK:
                        outputText("smiling as your fingers become coated in your slick fluids.  ");
                    case VaginaClass.WETNESS_DROOLING:
                        outputText("slicking your fingers in the juices that constantly dribble from [vagina]  ");
                    default:
            ...
            if (player.vaginas.length > 1)
            {
                ...
                {
                    outputText("Touch your enflamed and aroused [vagina]s, ");
                }
                var _sw5_ = (player.vaginas[0].vaginalWetness);                
                ...
                switch (_sw5_)
                {
                    case VaginaClass.WETNESS_DRY:
                        outputText("expertly arousing your female parts.  ");
                    case VaginaClass.WETNESS_NORMAL:
                        outputText("sighing as they quickly becomes moist.  ");
                    case VaginaClass.WETNESS_WET:
                        outputText("giggling as your fingers get a little wet.  ");
                    case VaginaClass.WETNESS_SLICK:
                        outputText("smiling as your fingers become coated in your slick fluids.  ");
                    case VaginaClass.WETNESS_DROOLING:
                        outputText("slicking your fingers in the juices that constantly dribble from [vagina]s  ");
                    default:
        ...
        if (player.hasCock())
        {
            sceneHunter.print("For dicks, there WILL be checks for nipplecunts.");
        }
        ...
        //New lines for masturbation!
        {
            ...
            {
                outputText("You grasp onto your [cock] with both hands, but fail to encircle it fully as you begin to masturbate.  ");
            }
            //If length > 12 proud
            ...
            else if (player.cocks[0].cockLength < 26)
            {
                ...
                if (player.canTitFuck() && player.biggestTitSize() > 3 && !nippleFuck)
                {
                    ...
                    {
                        outputText("Jets of milk squirt out of your nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ");
                    }
                }
            }
            else
        ...
        var twoF : Void->Void = function() : Void
        //Pair of cocks
        {
            ...
            {
                outputText("squeezing as many of your tools as your fingers can encircle.  The feeling of squeezing your thick twin cocks is amazing and overwhelming at the same time.  ");
            }
            //Get into it
            //Play with it if <= 10"
            ...
            //autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs{
                ...
                //check for dogcocks{
                    ...
                    //make sure that your cocks will fit in your tits if dogcocks{
                        ...
                        else if (player.dogCocks() != player.cocks.length)
                ...
                if (player.canTitFuck() && player.mostBreastsPerRow() > 2 && !nippleFuck)
                {
                    ...
                    if (player.biggestLactation() > 0)
                    {
                        outputText("Jets of milk squirt out of your many nipples with each thrust of your hips, adding to the already copious amounts of fluids coating your body. ");
                    }
                }
                ...
                //check for dogcocks{
                    ...
                    //make sure that your cocks will fit in your tits if dogcocks{
                        ...
                        //if you have something that will fit{
                            ...
                            nippleFuck = true;
                            autofellatio = true;
                            if (player.normalCocks() > 0 || player.horseCocks() > 0)
                            {
                                titFuckSingle();
                            }
                        }
        ...
        //Three or more cocks
        {
            ...
            {
                outputText("squeezing as many of your tools as your fingers can encircle.  The feeling of rubbing your bundle of cocks together is indescribable.  Your eyes roll back in uncontrollable pleasure.  ");
            }
            //Get into it
            //Play with it if <= 10"
            if (player.averageCockLength() <= 10)
                ...
                //check for dogcocks{
                    ...
                    //make sure that your cocks will fit in your tits if dogcocks{
                        ...
                        //if you have something that will fit{
                            ...
                            nippleFuck = true;
                            autofellatio = true;
                            if ((player.normalCocks() > 0 || player.horseCocks() > 0) && player.cocks.length - player.dogCocks() == 1)
                            {
                                titFuckSingle();
                            }
                            if (player.cocks.length - player.dogCocks() > 1)
                            {
                                multiTitFuck();
                            }
                        }
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            if (player.vaginas.length == 1)
            {
                ...
                var _sw6_ = (player.vaginas[0].vaginalWetness);                

                switch (_sw6_)
                {
                    case VaginaClass.WETNESS_DRY:
                        outputText("You have some difficulty with your relatively dry pussy, but you manage to gently and pleasurably masturbate by taking it slowly.  ");
                    case VaginaClass.WETNESS_NORMAL:
                        outputText("Your horny puss is aching for attention and you oblige it, dipping your fingers into the moist honeypot, and jilling yourself vigorously.  ");
                    case VaginaClass.WETNESS_WET:
                        outputText("The moistened cleft on your groin demands your full attention, drawing your fingers deep inside to explore the wet passage.  ");
                    case VaginaClass.WETNESS_SLICK:
                        outputText("The sweltering heat of your slick cunt aches for something to fill it, and you oblige, dipping your fingers into your slippery cunt.  ");
                    case VaginaClass.WETNESS_DROOLING:
                        outputText("Warm wetness runs down your legs in thick streams, pouring from your [vagina].  You smile sheepishly and stroke up your now slickened legs to your pussy-lips, parting them and letting your fingers dive inside the wet channel.  ");
                    default:
    ...
    //ORGASM GOES HERE
    {
        ...
        if (!player.hasPerk(PerkLib.ElectrifiedDesire) || !player.hasStatusEffect(StatusEffects.RaijuLightningStatus))
        {
            ...
            //Single Cock{
                ...
                if (player.cocks.length == 1)
                {
                    ...
                    else if (player.lib <= 80)
                    {
                        ...
                        {
                            outputText("you dream in a depraved kinky fantasia.");
                        }
                    }
                    else
                    {
                        outputText("You groan and drift to sleep, your rigid [cock] pulsing and throbbing with continual lust.");
                    }
                }
                //Multi Cock
                else
                {
                    ...
                    {
                        outputText("You groan and drift into a brief catnap, your rigid erections pulsing and throbbing with continual lust.");
                    }
                }
                if (player.perkv1(IMutationsLib.NukiNutsIM) >= 2)
                {
                    var cumAmmount : Float = player.cumQ();
                    var payout : Float = 1;
            ...
            else
            {
                if (player.vaginas.length > 0)
                {
                    outputText("You sigh softly and drift off into a quick nap, smelling of sex.");
                }
                //Girl ending
                else
                {
                    outputText("You sigh and drift off to sleep.");
                }
            }
        }
        if (inDungeon && DungeonAbstractContent.dungeonLoc != DungeonAbstractContent.DUNGEON_CABIN && player.companionsInPCParty())
    ...
    private function genderlessMasturbate() : Void
    {
        ...
        //Tit foreplay
        titForeplay();
        if (!player.hasStatusEffect(StatusEffects.FappedGenderless))
        ...
        else
        {
            ...
            {
                outputText("your four fingers in deep");
            }
        ...
        else
        {
            ...
            {
                outputText("  Satisfied, you roll over and drift off to sleep.");
            }
        }
    }
    ...
    //Ok lets touch our boobies if we haz them and they are big enough
    {
        ...
        if (player.breastRows.length == 1 && player.biggestTitSize() > 3)
        {
            ...
            {
                outputText("You squeeze your [breasts] brutally with both hands");
            }
        }
        else if (player.breastRows.length > 1 && player.biggestTitSize() > 3)
        {
            ...
            {
                outputText("You aggressively run your hands across your front, groping each of your " + num2Text(player.totalBreasts()) + " breasts in turn. ");
            }
            ...
            {
                outputText("The tactile reminder of your abnormal endowments gives you a delicious thrill.  You sigh and mash your tits together");
            }
        }
        ...
        //more than one row?{
            ...
            {
                outputText(" as you move your attention to your nipples");
            }
            ...
            {
                outputText(". Eagerly, you begin to roughly stroke the ");
            }
            ...
            {
                outputText("swollen, cunt-like lips that top each breast. ");
            }
            ...
            {
                outputText("You squeal with pleasure as ");
            }
            ...
            {
                outputText("Your freakish mammaries constantly drool a steady stream of milky cunt-juice, drenching your [legs] and crotch in milky goo.");
            }
        }
        ...
        else if (player.breastRows.length > 0 && player.biggestTitSize() > 3)
        {
            ...
            {
                outputText("squeals of pleasure each time you flick one of your ");
            }
            ...
            {
                outputText("A constant stream of milk drizzles from each teat, soaking your [legs] and crotch.  ");
            }
        }
    }
    ...
    //different based on player.libido
    {
        ...
        else if (player.lib < 70)
        {
            ...
            var _sw7_ = (player.cocks[0].cockType);            

            switch (_sw7_)
            {
                case CockTypesEnum.ANEMONE, CockTypesEnum.DISPLACER:
                    outputText("wriggling ");
                case CockTypesEnum.DEMON:
                    outputText("bump-encircled ");
                case CockTypesEnum.DOG, CockTypesEnum.FOX:
                    outputText("pointed ");
        ...
        else
        {
            ...
            var _sw8_ = (player.cocks[0].cockType);            

            switch (_sw8_)
            {  //Different way of choosing cock to use, results in consistent description of cock[0]  
        ...
        {
            outputText("huge, bloated cunt-nipples");
        }
        if (player.breastRows.length > 1)
        {
        ...
        if (player.averageLactation() == 0)
        {
            ...
            else if (player.averageVaginalWetness() < 4)
            {
                ...
                var _sw9_ = (player.cocks[0].cockType);                
                ...
                {  //Different way of choosing cock to use, results in consistent description of cock[0]  
                    ...
                    case CockTypesEnum.LIZARD, CockTypesEnum.CAVE_WYRM:
            ...
            {
                outputText("Slick juices squirt out from around your [cock] and y");
            }
        }
        else if (player.averageLactation() < 2)
        {
            outputText("Rivulets of milky girlcum drizzle ");
            var _sw10_ = (player.cocks[0].cockType);            
        ...
        else
        {
            ...
            var _sw11_ = (player.cocks[0].cockType);            
        ...
        //Different noises based on sensitivity{
            ...
            {
                outputText("ou scream with a combination of pain and pleasure as you attempt to stuff your [cock] into ");
            }
            ...
            {
                outputText("the gaping fuck-mouth of your inhuman nipple-cunt. ");
            }
            ...
            if (Math.round(player.biggestCockArea()) == player.vaginalCapacity())
            {
                ...
                {
                    var _sw12_ = (player.cocks[0].cockType);                    
            ...
            if (Math.round(player.biggestCockArea()) > player.vaginalCapacity())
            {
                ...
                {
                    var _sw13_ = (player.cocks[0].cockType);                    
            ...
            else if (player.averageVaginalWetness() < 4)
            {
                ...
                var _sw14_ = (player.cocks[0].cockType);                
                ...
                {  //Different way of choosing cock to use, results in consistent description of cock[0]  
                    ...
                    case CockTypesEnum.DISPLACER, CockTypesEnum.DOG, CockTypesEnum.FOX:
            ...
            {
                outputText("girl-lube spurts out of your tortured nipple with each thrust of your [cock], spattering your arms and face with your secretions. ");
            }
        }
        if (Math.round(player.biggestCockArea()) < player.vaginalCapacity())
        ...
        //Different noises based on sensitivity{
            ...
            {
                outputText("ou scream with delight");
            }
            ...
            else
            {
                ...
                var _sw15_ = (player.cocks[0].cockType);                
    ...
    private function multiTitFuck() : Bool
    {
        ...
        if (fittableCocks > 2 && holeTotal < fittableCocks)
        {
            ...
            if (player.vaginalCapacity() >= 2 * player.cockArea(randomCock))
            {
                ...
                {
                    outputText("With a flash of irritation, you realize that even if you stick two cocks in each hole, you still won't be able to fit all " + num2Text(player.cocks.length) + " of your dicks into your abused tits. Deciding to make the best of it, you prepare to stuff in as many as you can. ");
                }
            }
            else
            {
                outputText("Accepting that you can't do anything about it, you start pushing. ");
            }
        }
        //How wet/milky is this procedure?
        if (player.averageLactation() == 0)
        {
            ...
            {
                outputText("Slick juices squirt out from around your shafts and y");
            }
        }
        else if (player.averageLactation() < 2)
        {
        ...
        {
            outputText("Milky girlcum squirts out from around your shafts, staining them white. Y");
        }
        //Round and compare cock thickness to vag looseness
        ...
        //Different noises based on sensitivity{
            ...
            {
                outputText("ou scream with a combination of pain and pleasure as you attempt to stuff your cocks into ");
            }
            //Different based on looseness
            if (player.averageVaginalLooseness() < 2)
            {
            ...
            {
                outputText("the gaping fuck-mouths of your inhuman nipple-cunts. ");
            }
            //Compare cockthickness and vaglooseness more specifically
            //if it barely fits
            if (Math.round(player.cockArea(randomCock)) == player.vaginalCapacity())
            {
                ...
                {
                    outputText("The swollen lips of your bloated nipples stretch around your throbbing cocks, swallowing them whole. ");
                }
            ...
            if (Math.round(player.cockArea(randomCock)) > player.vaginalCapacity())
            {
                ...
                {
                    outputText("The swollen lips of your bloated nipples gape wide, but the enormous girth of your throbbing members spread them even wider. ");
                }
            ...
            {
                outputText("girl-lube spurts out of your tortured nipples with each thrust of your hips, spattering your arms and face with your secretions. ");
            }
        }
        if (Math.round(player.cockArea(randomCock)) < player.vaginalCapacity())
        {
            ...
            //Different noises based on sensitivity{
                ...
                {
                    outputText("ou scream with delight as your stiff cocks slide into ");
                }
                //Different based on looseness
                if (player.averageVaginalLooseness() < 2)
                {
                ...
                {
                    outputText("the gaping fuck-mouths of your inhuman nipple-cunts. The swollen lips of your bloated nipples engulf your hard rods and begin to slide down their length, swallowing them completely. ");
                }
                ...
                {
                    outputText("girl-lube pours out of your swollen nipples in a constant deluge of sticky fluid, creating a large puddle beneath you. ");
                }
            }
            //Different noises based on sensitivity
            else
            {
                ...
                if (4 * Math.round(player.averageCockThickness()) < player.averageVaginalLooseness())
                {
                    ...
                    {
                        outputText("the gaping fuck-mouths of your inhuman nipple-cunts. The swollen lips of your bloated nipples squish open as they swallow your fat dicks with little difficulty. ");
                    }
                ...
                else
                {
                    ...
                    {
                        outputText("the gaping fuck-mouths of your inhuman nipple-cunts. The swollen lips of your bloated nipples are stretched to their limits as you push two of your fat dicks into one. ");
                    }
    ...
    private function multiNippleFuckPrep(randomCock : Float) : Void
    //Start with some detail on one random cock
    {
        ...
        if (player.cocks[randomCock].cockType == CockTypesEnum.HUMAN)
        {
            ...
            {
                outputText("your " + player.cockDescript(randomCock) + " ");
            }
        }
        ...
        {
            outputText("huge, bloated cunt-nipple");
        }
        if (player.breastRows.length > 1)
        {
            outputText(" on one of your lower breasts");
    ...
    //ORGASM COOOOAD
    private function orgazmo(selfSucking : Bool, nippleFuck : Bool) : Void
    {
        outputText("\n\n");
        if (player.cocks.length > 0)
        {
            if (player.cocks.length == 1)
            {
                ...
                if (selfSucking)
                {
                    outputText("into your eager mouth.  ");
                }
                if (nippleFuck)
                {
                ...
                if (selfSucking)
                {
                    ...
                    {
                        outputText("Your orgasm never seems to end, and your world dissolves into the feelings from your [cock] as it erupts jet after jet of cum into your mouth.  You nearly gag, cum overflowing to spray out in a river, pooling around you.  ");
                    }
                    //Refill hunger!
                    if (player.cumQ() < 1000)
                    {
                        player.refillHunger(player.cumQ() / 15);
                    }
                    else if (player.cumQ() < 3000)
                    {
                        player.refillHunger(50 + ((player.cumQ() - 1000) / 30));
                    }
                    else
                    {
                        player.refillHunger(100 + ((player.cumQ() - 3000) / 45));
                    }
                }
                else if (nippleFuck)
                {
                    ...
                    {
                        outputText("Your orgasm never seems to end, and your world dissolves into the feelings from your [cock] as it erupts jet after jet of cum into your abused tit.  With a *pop*, your cock is forced free by the pressure. It continues to squirt semen everywhere, while a river of cum pours out of your dialated nipple, pooling around you.  ");
                    }
                }
                else if (player.cumQ() < 25)
                {
                ...
                {
                    outputText("The orgasm never seems to end, and your world dissolves to little more than the feeling of each jet of cum erupting from your cock.  Your mind dimly processes the feeling of each burst splattering over you, but it only enhances the feeling.  ");
                }
                if (player.armor == armors.GOOARMR)
                {
                    outputText("Your seed gets absorbed into your gooey covering.  ");
                }
            }
            //MULTICOCK
            if (player.cocks.length > 1)
            {
                ...
                if ((multiTypedIndex = player.findCockWithType(CockTypesEnum.HORSE)) >= 0)
                ...
                else if ((multiTypedIndex = player.findCockWithType(CockTypesEnum.DOG, 1, -1, -1, "area", 1)) >= 0)
                ...
                if (selfSucking)
                {
                    ...
                    {
                        outputText("Your orgasm never seems to end, and your world dissolves into the feelings from your [cock] as it erupts jet after jet of cum into your mouth.  You nearly gag, cum overflowing to spray out in a river, pooling around you.  Your other 'equipment' rains jizz upon you the whole while, soaking you in a cum-puddle.");
                    }
                    //Refill hunger!
                    if (player.cumQ() < 1000)
                    {
                ...
                {
                    outputText("The orgasm never seems to end, and your world dissolves to little more than the feeling of multiple cum eruptions spurting from your pricks.  Your mind dimly processes the feeling of each burst splattering over you, but it only enhances the feeling.  ");
                }
            }
        }
        //Vaginal CUMMING
        ...
        //Single vagina!{
            ...
            if (player.vaginas.length == 1)
            {
                ...
                {
                    outputText("A veritable gush of fluid explodes from your nethers, pulsing in time with the ripples of your [vagina].  ");
                }
            }
        }
        ...
        if (player.armor == armors.GOOARMR)
        {
            ...
            if (player.hasVagina())
            {
                ...
                {
                    valeriaFluids += as3hx.Compat.parseInt(player.vaginas[0].vaginalWetness * 3);
                }
            }
            if (player.hasCock())
            {
                if (player.cumQ() < 100)
                {
                    valeriaFluids += as3hx.Compat.parseInt(player.cumQ() / 5);
                }
                else if (player.cumQ() >= 100 && player.cumQ() < 500)
                {
                    valeriaFluids += as3hx.Compat.parseInt(20 + (player.cumQ() / 20));
                }
                else if (player.cumQ() >= 500 && player.cumQ() < 3500)
                {
                    valeriaFluids += as3hx.Compat.parseInt(40 + (player.cumQ() / 50));
                }
                else if (player.cumQ() >= 3500)
                {
                    valeriaFluids += 100;
                }
            }
            ...
            if (valeriaFluids > 0)
            {
                outputText("\"<i>Thanks for the fluids!</i>\" Valeria says.  ");
            }
        }
        //DONE!
        flags[kFLAGS.TIMES_MASTURBATED]++;
    ...
    private function titCum(cumQuantity : Float = 3) : Void
    //var tempSize:Number = Math.round((nippleLength + baseCockLength/2)*100)/100;
    {
        ...
        if (player.averageLactation() >= 2 && !player.hasFuckableNipples())
        {
            ...
            if (player.averageLactation() <= 2.6)
            {
                outputText("thin streams of milk spray out of your ");
            }
            if (player.averageLactation() > 2.6 && player.averageLactation() < 3)
            {
                outputText("thick gouts of milk spurt out of your ");
            }
            if (player.averageLactation() >= 3)
            {
                outputText("huge torrents of milk blast out of your ");
            }
            //different description based on size of nips
            if (player.averageNippleLength() <= 1)
            {
                outputText("erect nipples, ");
            }
            if (player.averageNippleLength() > 1 && player.averageNippleLength() < 4)
            {
                outputText("fat, puckered nipples, ");
            }
            if (player.averageNippleLength() >= 4)
            {
                outputText("huge, swollen teats, ");
            }
            if (player.armor == armors.GOOARMR)
            {
                if (player.averageLactation() < 2.6)
                {
                    outputText("spattering milk into the goo.  ");
                }
                if (player.averageLactation() >= 2.6)
                {
                    outputText("stuffing the goo with your milk.  ");
                }
            }
            else
            {
                if (player.averageLactation() <= 2.6)
                {
                    outputText("spattering milk everywhere.  ");
                }
                if (player.averageLactation() > 2.6 && player.averageLactation() < 3)
                {
                    outputText("covering everything nearby.  ");
                }
                if (player.averageLactation() >= 3)
                {
                    outputText("drenching the entire area.  ");
                }
            }
        }
        ...
        if (player.hasFuckableNipples())
        {
            ...
            if (player.sens < 70)
            {
                outputText("moan with ecstacy as your freakishly swollen nipples ");
            }
            if (player.sens >= 70)
            {
                outputText("scream with ecstacy as your freakishly swollen nipples ");
            }
            //Different description based on wetness
            if (player.averageVaginalWetness() < 2)
            {
            ...
            if (player.armor == armors.GOOARMR)
            {
                outputText("into the blue goo covering your body.  ");
            }
            else
            {
    ...
    private function deluxeDildo() : Void
    {
        ...
        //(highcor){
            ...
            {
                outputText("You blush feverishly as you grab your pink dildo.  It flops about lewdly as you run off behind some rocks and strip down to use it.  You feel like such a pervert.");
            }
            outputText("\n\n");
            //(low cor)
            if (player.cor < 50)
            ...
            {
                outputText("You hold the fake dong, squeezing it and giggling at the realistic texture.  You can't wait to try it out.");
            }
            ...
            {
                outputText(" fills with blood, twitching and pulsating like a man's cock");
            }
            outputText(".  You ");
            if (player.cor > 50)
            {
            ...
            {
                outputText("ramming it deep inside you, moaning as it rubs your now over-sensitive walls.");
            }
            ...
            if (JojoScene.monk >= 5 && flags[kFLAGS.JOJO_DEAD_OR_GONE] == 0)
            {
                ...
                {
                    outputText("splattering himself with mouse-spunk as he finishes enjoying your inadvertent show.  He runs off before you have a chance to react.");
                }
            }
            masturGasm("Vaginal");
            flags[kFLAGS.TIMES_MASTURBATED]++;
    ...
    //Clear text for new stuff
    {
        ...
        if (!player.hasStatusEffect(StatusEffects.PlainOnaholeUsed))
        {
            ...
            {
                outputText("With your free hand, you slap your cock to 'attention' and ease the onahole over your cock.");
            }
    ...
    private function onaholeMulticockContinuation() : Void
    {
        ...
        {
            outputText("you pass out with [cocks] sore from exertion.");
        }
        masturGasm("Dick");
        endEncounter();
    }
    ...
    private function onaholeFutaContinuation() : Void
    {
        ...
        outputText(Std.string(as3hx.Compat.parseInt(((Math.random() * player.str / 12) + player.str / 6) * 10) / 10));
    ...
    private function lickYerGirlParts() : Void
    ...
    private function EasterBunnyLayEggs() : Void
    //Female cat masturbation
    {
    ...
    private function catAutoLick() : Void
    ...
    public function meditate(description : String = "rock") : Void
    {
        ...
        if (player.hasPerk(PerkLib.Enlightened) && player.cor < 10)
        {
            HPChange(50, true);
        }
        fatigue(-10);
    ...
    private function dualBeltMasturbation() : Void
    {
        ...
        {
            outputText("covering up your tail, ");
        }
        ...
        if (player.cumQ() >= 1000)
        {
            outputText("It grows larger and larger until you are sure it will pop, but it doesn't.  It just sloshes around - a huge bubble, nearly waist high. ");
        }
        ...
        if (player.biggestLactation() >= 2)
        {
            outputText("Milk gushes out from your [breasts] as you orgasm, filling the inside of the suit with a slick layer of milk and forming milk bubbles that hang lewdly off your chest. ");
        }
        ...
        if (player.biggestLactation() >= 2)
        {
            outputText("Thin streams of creamy milk flow from your [allbreasts], your torso and midsection dripping wet from the stuff. ");
        }
    ...
    private function centaurMasturbation() : Bool
    {
        clearOutput();
        var canMasturbateHugeCock : Bool = player.hasCock() && (player.tallness * (5 / 6) < player.cocks[player.longestCock()].cockLength);
        if (player.hasFuckableNipples())
        {
            if (canMasturbateHugeCock && rand(2) == 0)
    ...
    private function centaurNippleCunt() : Void
    {
        ...
        {
            outputText("smiling to yourself as you blatantly oogle your [nipple]s.");
        }
        ...
        {
            outputText("  They're shallow enough that you don't get much more than a fingertip inside them, but the flesh is extraordinarily sensitive, shooting arcs of pleasure deep into your body.");
        }
        ...
        {
            outputText("Slick lubricant squishes around your fingers as you finger-fuck your chest-pussies.");
        }
        ...
        if (player.hasVagina() || player.hasCock())
        {
            ...
            if (player.hasCock())
            {
                ...
                {
                    outputText(player.cockDescript());
                }
                //Set as plural if multi dick or dick and vag.
                if (player.hasVagina() || player.cockTotal() > 1)
                {
                    plural = true;
                }
            }
            ...
            {
                outputText(" leaks its own fluids, quivering with ache and need that you can't reach to satisfy.");
            }
            if (player.cockTotal() > 1)
            {
            ...
            if (player.hasBalls())
            {
                outputText("  Rocking slightly inside your sack, your [balls] repeatedly clench up against your body, ready to release.");
            }
            outputText("\n\n");
        }
        //(ORGAZMO)
        ...
        if (player.hasCock())
        {
            if (player.cumQ() < 50)
            {
                outputText("A small puddle of semen has formed under you, ");
            }
            else if (player.cumQ() < 200)
            {
                outputText("A decent sized puddle of white seed has formed under you, ");
            }
            else if (player.cumQ() < 1000)
            {
                outputText("A large puddle of white seed has formed underneath you and even splattered your underside, ");
            }
            else if (player.cumQ() < 2000)
            {
                outputText("A frothing puddle of thick cum has formed below you, soaking your legs and underbelly, ");
            }
            else
            {
                outputText("A small lake of semen surrounds you, ");
            }
        ...
        if (player.hasVagina())
        {
            ...
            {
                outputText("The air is filled with the thick musk of your bestial horse-pussy.  If the rivulets of moisture dripping down to your hooves are any indication, you might need to get used to the smell.");
            }
        }
        if (player.hasCock() || player.hasVagina())
        {
        ...
        {
            outputText("You're horrified at what you're doing to deal with your inner perversions.");
        }
        //DONE!
        masturGasm("Vaginal");
    }
    ...
    //Set plurality and primary cock.
    {
        ...
        var plural : Bool = (player.cocks.length > 1);
        ...
        {
            outputText("Smiling with excitement, ");
        }
        ...
        if (!plural)
        {
            outputText("it grows full and hard.");
        }
        else
        {
            outputText("they grow full and hard.");
        }
        if (plural)
        {
        ...
        else
        {
            ...
            if (player.cocks[primary].cockType == CockTypesEnum.HORSE)
            {
                outputText("flared tip");
            }
            else if (player.cocks[primary].cockType == CockTypesEnum.DEMON)
            {
                outputText("nodule-ringed head");
            }
            else if (player.cocks[primary].cockType == CockTypesEnum.DOG)
            {
                outputText("pointed erection");
            }
            else if (player.cocks[primary].cockType == CockTypesEnum.TENTACLE)
            {
                outputText("wriggling, mushroom-like tip");
            }
            else if (player.cocks[primary].cockType == CockTypesEnum.CAT)
            {
                outputText("spiny head");
            }
            else if (player.cocks[primary].cockType == CockTypesEnum.LIZARD)
            {
                outputText("pointed, purple tip");
            }
            else if (player.cocks[primary].cockType == CockTypesEnum.ANEMONE)
            {
                outputText("tentacle-ringed head");
            }
            else
            {
                outputText("dome-like head");
            }
            outputText(".");
        }
        ...
        if (player.tallness * 1.5 >= player.cocks[player.longestCock()].cockLength)
        {
            ...
            {
                outputText("A tiny dollop of pre-cum slowly builds on your tip as you get into the act.");
            }
            if (player.hasVagina())
            {
                outputText("  Your neglected [vagina] aches for penetration, but there's no way you'll be able to reach it in your current state.");
            }
            outputText("  You sigh, delighted that you're able to caress at least one of your sexual organs with this body");
            if (player.cor < 40)
            {
                outputText(", but you're also worried that you're falling into the perversion that lurks in this strange world");
            }
        ...
        else
        {
            ...
            if (player.cumQ() > 3000)
            {
                outputText("  As you finish cumming, the puddle deepens.  It's inches thick and splattering everywhere whenever you take a step.");
            }
        }
        ...
        if (player.hasSheath())
        {
            ...
            {
                outputText(" slowly growing limp and sliding back inside your sheath.");
            }
        }
        else if (player.minLust() > 40 || player.lib > 50)
        {
            ...
            {
                outputText(" growing slightly less hard.  As much as you seem to need sex, that doesn't surprise you.");
            }
        }
        else if (plural)
        {
        ...
        {
            outputText(" slowly growing limp, shrinking.");
        }
        outputText("\n\n");
        //Cunt cums
        if (player.hasVagina())
        {
            ...
            {
                outputText("Spatters of fluids gush out from between your thighs, coating your upper legs in a thick coat of clear feminine lubricant as you orgasm.  The air is filled from the scent of your liquid centaur lust as more and more drips into a rapidly expanding puddle between your rear legs.");
            }
            if (player.hasFuckableNipples())
            {
                if (player.totalNipples() > 2)
    ...
    private function centaurCantMasturbate() : Void
    {
        if (flags[kFLAGS.UNABLE_TO_MASTURBATE_BECAUSE_CENTAUR] == 0)
        {
            ...
            {
                outputText("anything remotely sexual");
            }
        ...
        {
            outputText("It looks like you'll have to find a partner to relieve your pent up need, but in your current state you'll probably be on the receiving end of whatever you can get!");
        }
    }
    ...
    private function centaurDudesGetHorseAids() : Void
    {
        ...
        if (player.keyItemvX("Fake Mare", 1) == 0)
        {
            ...
            {
                outputText("You decide to play with the mare-shaped cocksleeve Whitney gave you.  You pull it out of your stash and spend a few minutes setting it up in the heart of camp.  Once done, you stand behind a wood-and-iron replica of a mare, adjusted to the perfect height for you.  Looking \"<i>her</i>\" over, your eyes are drawn to the slick black lips of the Onahole between her legs, craftily shaped like a horsecunt, and what looks like a second, smaller one above it simulating an anus.\n\n");
            }
        }
        ...
        {
            outputText("Your fist slips in easily, and you give the toy a few preparatory thrusts to make sure it's nice and ready for your hefty cock.");
        }
        outputText("\n\n");
        ...
        {
            outputText("lining up with the toy's gaping cunt");
        }
    ...
    private function centaurGirlsGetHorseAids() : Void
    {
        ...
        //[If low Corruption:]{
            ...
            {
                outputText("Unable to sate your own lusts due to your centaur configuration, you decide to put one of Whitney's centaur toys to use.  You dig the Centaur Pole out of your stash and drag it to the middle of camp.\n\n");
            }
        }
    ...
    private function getHugeEggsInCawk() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_EGGED_IN_EXGARTUAN] == 0)
        {
            ...
            {
                sceneHunter.print("Check failed: Exgartuan in dick and awake.");
            }
        }
        eggDickNormal();
    }
    ...
    public function eggExgartuan() : Void
    {
        ...
        if (player.cor >= 66)
        {
            ...
            {
                outputText(", hoping one of your companions sees");
            }
        }
        else if (player.cor >= 33)
        {
        ...
        {
            outputText("in the wastes just outside of camp to be out of sight and mind");
        }
        ...
        {
            outputText(", with Shouldra's assistance,");
        }
        ...
        if (player.hasBalls())
        {
            ...
            {
                outputText(", and you know beyond a doubt that they'll be fertilized before much longer and you'll be able to lay your own eggs");
            }
            outputText(".");
        }
        //[If no balls:]
    ...
    private function eggDickNormal() : Void
    {
        ...
        {
            outputText("into the trees outside of camp to be out of sight and mind");
        }
        ...
        if (player.gender == 3)
        {
            ...
            if (player.str > 50)
            {
                outputText("even with your considerable strength, ");
            }
        ...
        if (player.hasBalls())
        {
            ...
            if (!player.isTaur())
            {
                outputText("More begin to follow, and you massage your quickly-bloating balls as they fill with egg after egg after egg.  ");
            }
    ...
    private function layEggsInYerTits() : Void
    {
        ...
        //Exgartuan; breasts should be HH or larger, fuckable nipples, only if Exgartuan is awake{
            ...
            if (player.hasVagina())
            {
                outputText("  Your [vagina] lets sticky fluids out slowly around your thighs; you are only able to tell by the damp feel of your [legs].  It feels fantastic!");
            }
        ...
        else
        {
            ...
            if (player.eggs() < 20)
            {
                ...
                if (player.hasCock())
                {
                    outputText("  Your [chest] and ovipositor meanwhile are slowly being coated in white from your languid pleasure.");
                }
    ...
    private function tentacleSelfFuck() : Void
    {
        ...
        if (y >= 0)
        {
            outputText("  It loops around your " + cockDescript(y) + " while you fight with it, strangling the other phallus in tight coils of squeezing, floral friction.");
        }
        ...
        {
            outputText(", kneading the soft mammary");
        }
        outputText(" and twisting your [nipple] as your body goes white-hot with pleasure");
        if (player.hasFuckableNipples())
        {
            outputText(", even stuffing a finger inside a sloppy, dripping nipplecunt");
        }
        outputText(".");
        if (player.lactationQ() >= 250)
        {
            outputText("  Milk squirts from your engorged teat almost immediately to fall in a moist, creamy rain across your writhing form.");
        }
        ...
        if (player.tentacleCocks() >= 3 || player.stamenCocks() > 2)
        {
            ...
            if ((player.tentacleCocks() || player.stamenCocks()) < player.cocks.length)
            {
                outputText(" and the open air");
            }
            outputText(".");
            if (player.biggestTitSize() >= 4)
            {
                outputText("  Laying right between your boobs, it doesn't take long for the shaft to extend far enough to loop around each tit, sliding slowly encircling each curvy mound as it presses them together around itself, getting a titfuck while it plows your unresisting mouth.");
            }
        }
        //No bonus cawks!
        else
        ...
        if (player.tentacleCocks() >= 6 || player.stamenCocks() > 5)
        {
            ...
            if (player.tentacleCocks() >= 7 || player.stamenCocks() > 6)
            {
                outputText("  They pile in there, really squirming against each other as much as you, frotting against your sweat-slick flesh.  ");
            }
        ...
        if (player.tentacleCocks() >= 3 || player.stamenCocks() > 2)
        {
            ...
            if (player.cumQ() >= 500)
            {
                ...
                {
                    outputText("and the cum is soon squirting out the corners of your mouth while your cheeks bulge cartoonishly");
                }
            }
            outputText(".");
        }
        outputText("  Your birth canal is quickly flooded with white spooge");
        if (player.tentacleCocks() >= 2 || player.stamenCocks() > 1)
        {
            outputText(", while your [ass]'s interior is painted bright white");
        }
        outputText(".");
        if (player.tentacleCocks() == 2 || player.stamenCocks() == 2)
        {
            ...
            {
                outputText("like a virile sprinkler");
            }
            outputText(".");
            if (player.cocks.length > 2)
            {
        ...
        else if (player.tentacleCocks() >= 4 || player.stamenCocks() > 3)
        {
            ...
            if (player.tentacleCocks() >= 5 || player.stamenCocks() > 4)
            {
                outputText("s");
            }
    ...
    private function stamenSelfFuck() : Void
    {
        ...
        if (y >= 0)
        {
            outputText("  It loops around your distended, wriggling plant-shaft while you fight with it, strangling the other stamens in tight coils of squeezing, floral friction.");
        }
        outputText("\n\nYou arch your back as you try to contain the unexpected waves of desire that flood your groin, but it’s no use. In the span of a few seconds, you decide to accept that you need to fuck something now. Foreplay is no longer an option.");
        ...
        if (player.stamenCocks() < player.cocks.length)
        {
            outputText(" and the open air");
        }
        outputText(".");
    ...
    private function tentacleGoesUpYerPooperNewsAtEleven() : Void
    {
        ...
        if (player.cumQ() >= 1000)
        {
            outputText(", making [eachCock] bulge.  The extra feeling sends you over the edge and you quickly reach your climax as you cum and cum");
        }
        outputText(".");
        //[if cum production is moderate]
        if (player.cumQ() >= 500)
        {
            outputText("  Your belly swells a bit from all the semen being packed inside you.");
        }
        //[if cum production is massive]
        if (player.cumQ() >= 1500)
        {
            outputText("  Your poor insides cannot handle the enormous cumshot being unloaded in your [asshole] and a significant volume of spunk dribbles outside, carelessly polluting the floor.");
        }
        ...
        if (player.perkv1(IMutationsLib.NukiNutsIM) >= 2)
        {
            var cumAmmount1 : Float = player.cumQ();
            var payout1 : Float = 1;
            //Get rid of extra digits
            //cumAmmount = int(cumAmmount);
            //Calculate payout
            if (cumAmmount1 > 10)
            {
                payout1 += 2 + as3hx.Compat.parseInt(cumAmmount1 / 100) * 2;
            }
            //Reduce payout if it would push past
            if (player.perkv1(IMutationsLib.NukiNutsIM) >= 3)
            {
    ...
    private function stamenGoesUpYerPooperNewsAtEleven() : Void
    {
        ...
        if (player.isLiliraune())
        {
            outputText(" and your sister both");
        }
    ...
    private function gildedCockTurbate() : Void
    {
        clearOutput();
        var gildedCock : Int;
        for (gildedCock in 0...player.cocks.length)
        {
            if (player.cocks[gildedCock].sock == "gilded")
            {
                break;
            }
        }
        for (x in gildedCock + 1...player.cocks.length)
        
        //Find the biggest gilded cock you've got{
            
            if (player.cocks[x].sock == "gilded" && player.cocks[x].cArea() > player.cocks[gildedCock].cArea())
            {
                gildedCock = x;
            }
        }
        ...
        {
            outputText("  Clamped around your dick, the metal casing looks more like a gigantic cock ring. The golden sheath actually allows you a better grip on your massive member, a ring of coolness around your hot meat.");
        }
    ...
    private function stickADildoInYourVagooSlut() : Void
    {
        ...
        //LOW CORRUPTION DEFLOWER{
            ...
            if (player.cor <= 50)
            {
                ...
                {
                    outputText("The toy slides effortlessly thanks to your copious, almost inhuman lubrication.");
                }
    ...
    private function dildoButts() : Void
    {
        ...
        {
            outputText("Your experienced hole readily accepts your saliva-coated finger.");
        }
        ...
        if (silly())
        {
            outputText(", the alliteration of the experience further arousing you");
        }
    ...
    private function midasCockJackingGemsRoll() : Int
    {
        ...
        var gems : Int = as3hx.Compat.parseInt(10 + rand(20));
        
        if (player.cumQ() < 1000 * cockSocks)
        {
            gems += as3hx.Compat.parseInt(player.cumQ() / 10);
        }
        else if (player.cumQ() < 2500 * cockSocks)
        {
            gems += as3hx.Compat.parseInt(player.cumQ() / 50);
        }
        else if (player.cumQ() < 5000 * cockSocks)
        {
            gems += as3hx.Compat.parseInt(player.cumQ() / 150);
        }
        else if (player.cumQ() < 10000 * cockSocks)
        {
            gems += as3hx.Compat.parseInt(player.cumQ() / 450);
        }
        else
        {
            gems += as3hx.Compat.parseInt(player.cumQ() / 1350);
        }
        
        if (gems > 200 * cockSocks)
        {
            gems = as3hx.Compat.parseInt(200 * cockSocks + rand(20));
        }
        
        return gems;
    }
    
    private function onaholeRepeatUse(corrupted : Bool) : Void
    {
    ...
    private function gooeySatisfaction() : Void
    {
        ...
        outputText("You are way too horny to hunt anything right now, and while you could just let the imp next door use you to empty his seed, you would rather be the one on top.\n"
                + "\n"
                + "With those facts clear, you"
                + player.clothedOrNaked(" ooze out of your equipment and")
                + " get ready to blow off some steam. That said, now that you think about it, you are literally made of goo. You could shape your body however you like in a thousand possible form and giggle at the idea of what your about to inflict yourself.\n"
                + "\n"
