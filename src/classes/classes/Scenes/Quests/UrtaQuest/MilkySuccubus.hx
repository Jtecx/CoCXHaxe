...
class MilkySuccubus extends AbstractSuccubus
{
    ...
    {
        if (!hasStatusEffect(StatusEffects.MilkyUrta) && rand(3) == 0)
        {
    ...
    private function drinkMinoCum() : Void
    {
        ...
        if (!hasStatusEffect(StatusEffects.DrankMinoCum) || !hasStatusEffect(StatusEffects.DrankMinoCum2))
        {
            ...
            addHP(400);
            lust += 25;
            if (!hasStatusEffect(StatusEffects.DrankMinoCum))
            {
                createStatusEffect(StatusEffects.DrankMinoCum, 0, 0, 0, 0);
            }
            else
            {
                createStatusEffect(StatusEffects.DrankMinoCum2, 0, 0, 0, 0);
            }
        }
        //Failure:
        else
    ...
    private function succubusTease() : Void
    {
        ...
        {
            outputText("The succubus turns slightly and slowly bends over, sliding her hands down the sides of her milk laden jugs. \"<i>Mmm, would you help a poor girl relax? These things need some attention,</i>\" she says with a lust filled moan as her hands reach her multitude of nipples.");
        }
        else
        {
            outputText("The succubus leans forwards holding her tits, while wrapping her fingers around her nipples.  \"<i>My boobs are soo full.  Would you like to help me drain them?</i>\" she says with a husky voice.");
        }
