...
class ThePansLabyrinthSatyr extends Monster
{
    public function MaddeningTuneGo() : Void
    {
        var turn : Float = statusEffectv1(StatusEffects.MaddeningTune);
        switch (turn)
        {
            case 6:
            ...
            case 3:
                ...
                var lustDmg : Int = as3hx.Compat.parseInt(rand(player.lib / 10) + rand(this.lib / 5) + 5);
        ...
        if (statusEffectv1(StatusEffects.MaddeningTune) == 1)
        {
            SceneLib.dungeons.twilightgrove.defeatedByThePansLabyrinthSatyr();
        }
        else
        {
            addStatusValue(StatusEffects.MaddeningTune, 1, -1);
        }
    }
    ...
    public function new()
    {
        ...
        this.drop = new ChainedDrop().add(consumables.INCUBID, 1 / 2).add(weapons.SFLUTTE, 1 / 4);
