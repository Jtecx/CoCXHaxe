...
class LadyRafflesia extends Monster
{
    ...
    public function ladyRafflesiaVineStrike() : Void
    {
        ...
        {
            outputText(" ");
            var damage : Int = as3hx.Compat.parseInt(eBaseStrengthDamage() + rand(str));
    ...
    public function ladyRafflesiaEnergyDrain() : Void
    {
        ...
        if (!player.immuneToBleed())
        {
            ...
            {
                player.createStatusEffect(StatusEffects.Hemorrhage, SceneLib.combat.debuffsOrDoTDuration(2 + rand(2)), 0.05, 0, 0);
            }
        }
    }
    ...
    public function ladyRafflesiaDevourservants() : Void
    {
        ...
        if (hasStatusEffect(StatusEffects.LadyRafflesiaDevSer))
        {
            addStatusValue(StatusEffects.LadyRafflesiaDevSer, 1, 1);
        }
        else
        {
            createStatusEffect(StatusEffects.LadyRafflesiaDevSer, 1, 0, 0, 0);
        }
    ...
    override private function performCombatAction() : Void
    {
        if (hasStatusEffect(StatusEffects.Level))
        {
            ...
            {
                ladyRafflesiaStrangulate();
            }
            else if (player.hasStatusEffect(StatusEffects.LadyRafflesiaGrab))
            {
                ladyRafflesiaEnergyDrain();
            }
            else if (HPRatio() < .5 && statusEffectv1(StatusEffects.LadyRafflesiaDevSer) < 5)
            {
                ladyRafflesiaDevourservants();
            }
            else
            {
                ...
                if (choice == 0)
                {
                    ladyRafflesiaTeaseAttack();
                }
                if (choice == 1)
                {
                    if (!hasStatusEffect(StatusEffects.LustAura))
                    {
                        ladyRafflesiaPollenCloud();
                    }
                    else
                    {
                        ladyRafflesiaTeaseAttack();
