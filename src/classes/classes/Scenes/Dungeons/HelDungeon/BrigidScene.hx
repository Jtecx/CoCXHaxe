...
class BrigidScene extends BaseContent
{
    public function new()
    {
        super();
    ...
    private function brigitPostQueenDeathBadEndII() : Void
    {
        ...
        {
            if (sceneHunter.lossSelect && player.hasVagina())
            ...
            else if (player.gender <= 1 || !player.canGetPregnant())
            {
                dickF();
            }
            else
            ...
            function dickF() : Void
            {
                ...
                if (!player.hasCock())
                {
                    ...
                    {
                        outputText("  Once this disgusting thing you're carrying falls out of you, we'll use you more traditionally, but...</i>\"  She pauses to press uncomfortably on your pregnant belly, then resumes speaking.  \"<i>I have no intention of letting you sit idle for however long that takes.");
                    }
                ...
                if (player.cockTotal() > 1 || player.cumQ() > 1000)
                {
                    outputText("  " + SMultiCockDesc() + " holds forth for a while, until Kiri's ass, pussy, and tailfeathers are soaked with oozing, white cum.");
                }
                ...
                {
                    addButton(1, "Retry", SceneLib.dungeons.heltower.retryDungeonFromBadEndPrompt);
                }
            ...
            //hymen check]
            {
                ...
                if (player.hasCock())
                {
                    outputText(sMultiCockDesc() + " bobbing up and down, drooling pre-cum, as ");
                }
                ...
                if (player.hasCock())
                {
                    outputText(" as the phoenix's cunt slides onto your [cock smallest]");
                }
