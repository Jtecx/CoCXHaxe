...
class TwinBosses extends Monster
{
    public function onisunetwinspellMod() : Float
    {
        var kmods : Float = 1;
        if (hasPerk(PerkLib.Archmage) && inte >= 75)
        {
            kmods += .3;
        }
        ...
        if (hasPerk(PerkLib.GrandArchmage) && inte >= 100)
        {
            kmods += .4;
        }
        if (hasPerk(PerkLib.JobSorcerer) && inte >= 25)
        {
            kmods += .1;
        }
    ...
    public function onisunetwinsoulskillMod() : Float
    {
        var kmodss : Float = 1;
    ...
    public function onisunetwinCastsFoxFire() : Void
    {
        ...
        outputText(" ");
        var lustDmg : Float = player.lustVuln * ((this.inte / 10) + rand(player.lib + player.cor) / 5);
        lustDmg *= 0.5;
    ...
    public function onisunetwinCastsComet() : Void
    {
        ...
        {
            outputText("Thankfully, your armor manages to absorb most of the impact. ");
        }
        var damage : Float = 0;
        ...
        if (player.armorPerk != "Heavy" && player.armorPerk != "Light Ayo" && player.armorPerk != "Heavy Ayo" && player.armorPerk != "Ultra Heavy Ayo")
        {
            damage *= 2;
        }
        damage = Math.round(damage);
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (choice > 2)
        {
            ...
            else if (mana >= 200 && soulforce >= 108)
            {
                onisunetwinCastsFoxFire();
            }
            else
            {
                eAttack();
    ...
    override public function postPlayerBusyBtnSpecial(btnSpecial1 : CoCButton, btnSpecial2 : CoCButton) : Void
    {
        ...
        {
            btnSpecial1.show("Dish Helper", SceneLib.dungeons.riverdungeon.dishHelperTB);
        }
        else
        {
            btnSpecial1.showDisabled("Dish Helper", "You don't have anyone to take care of other twin!");
        }
    }
    ...
    {
        var twinBelow40 : Bool = (HP < maxHP() * 0.4);
    ...
    public function new()
    {
        ...
        this.drop = new WeightedDrop().add(consumables.FOXJEWL, 3).add(consumables.ONISAKE, 2).add(useables.EL_CORE, 1);
