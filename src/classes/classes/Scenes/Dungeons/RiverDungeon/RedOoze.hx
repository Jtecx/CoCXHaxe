...
class RedOoze extends Monster
{
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        ...
        {
            floors.defeatedByRedMistOoze();
        }
        else
        {
            floors.defeatedByRedOoze();
        }
    }
    ...
    private function lustAttack() : Void
    {
        ...
        var baseLA : Float = player.lib / 10 + 8;
        if (flags[kFLAGS.RED_OOZE_SUBTYPE] == 1)
        {
            baseLA += player.lib / 20 + 4;
        }
    ...
    private function lustReduction() : Void
    {
        ...
        var lustRed : Float = 20;
        var healthR : Float = 50;
        if (flags[kFLAGS.RED_OOZE_SUBTYPE] == 1)
        {
            lustRed *= 5;
            healthR *= 20;
    ...
    public function new()
    {
        ...
        if (flags[kFLAGS.RED_OOZE_SUBTYPE] == 1)
        {
            this.drop = new ChainedDrop().add(useables.PCSHARD, 1 / 2).elseDrop(useables.RED_GEL);
        }
        else
        {
            this.drop = new ChainedDrop().add(weapons.PIPE, 1 / 10).add(consumables.WETCLTH, 1 / 2).elseDrop(useables.RED_GEL);
        }
