...
class Demetrial extends AbstractAngel
{
    private function dmgForAlmostAll() : Float
    {
        var dFAA : Float = 0;
        dFAA += this.str;
    ...
    private function moveSwarmbringer() : Void
    {
        ...
        player.buff("Swarmbringer").addStats({
                                    str : -20,
                                    spe : -20
                                }).withText("Swarmbringer!").combatPermanent();
    }
    private function moveRootOfTheIssue() : Void
    {
        ...
        {
            outputText("You manage to duck and dodge the roots, narrowly avoiding their clutches. ");
        }
        else
        {
            ...
            var dmg1 : Float = dmgForAlmostAll() * 0.5;
            ...
            if (player.canFly())
            {
                outputText(" You stabilise yourself mid-flight, now looking down upon the angel and your ally. You’re outside the reach of the angel’s tendrils…and yet, you feel slightly nervous. ");
            }
    ...
    private function moveDivineJudgemen() : Void
    {
        ...
        {
            outputText("You flail as the sky darkens, clouds forming around you. You can see sparks around you as you try to orient yourself. You know what’s happening, but you can’t do anything to stop it. A wall of lightning strikes you, sending you into convulsions as the strike slams into your back. ");
        }
    ...
    override private function performCombatAction() : Void
    {
        ...
        {
            if (!hasStatusEffect(StatusEffects.ATranscendentSoulField) && soulforce >= 150)
            {
                createStatusEffect(StatusEffects.ATranscendentSoulField, 100, 100, 0, 0);
            }
        ...
        {
            if (!hasStatusEffect(StatusEffects.ATranscendentSoulField) && soulforce >= 75)
            {
                createStatusEffect(StatusEffects.ATranscendentSoulField, 50, 50, 0, 0);
            }
            if (hasStatusEffect(StatusEffects.ATranscendentSoulField))
            {
                if (soulforce >= 75)
        ...
        if (!hasStatusEffect(StatusEffects.AbilityCooldown1) && !hasStatusEffect(StatusEffects.Swarmbringer))
        {
            moveSwarmbringer();
        }
        else if (!hasStatusEffect(StatusEffects.RootOfTheIssue))
        {
            moveRootOfTheIssue();
        }
        else
    ...
    {
        if (hasStatusEffect(StatusEffects.Swarmbringer) && !hasStatusEffect(StatusEffects.RootOfTheIssue))
        {
            secondFarmerAction();
        }
        else if (rand(2) == 0)
    ...
    public function new()
    {
        ...
        this.drop = new WeightedDrop().add(consumables.SNOWFLO, 3).add(consumables.EMBER_F, 3).add(consumables.MARAFRU, 3).add(consumables.BEEHONY, 3).add(useables.PCRYSTA, 2).add(useables.SRESIDUE, 1).add(useables.EL_CORE, 1);
