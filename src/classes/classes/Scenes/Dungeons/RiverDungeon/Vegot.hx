...
class Vegot extends Monster
{
    private function vegotBasicAttack() : Void
    {
        ...
        player.buff("Frostlight Slow").addStats({
                                    spe : -10
                                }).withText("Frostlight Slow").combatPermanent();
        player.takeLightningDamage(dmg0, true);
        if (HP < maxOverHP())
        {
            var temp1 : Float = 0;
            temp1 += Math.round(dmg0 * 0.1);
            if (HP + temp1 > maxOverHP())
            {
                temp1 -= (this.maxOverHP() - HP);
            }
            HP += temp1;
    ...
    private function vegotW1() : Float
    {
        var dmg2 : Float = 0;
        dmg2 += Math.round(eBaseIntelligenceDamage() * 1.2);
        return dmg2;
    }
    private function vegotW2() : Float
    {
        var lust0 : Float = 0;
    ...
    private function vegotE() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.Stunned, 1, 4);
        }
        else
        {
    ...
    public function new()
    {
        ...
        this.drop = new WeightedDrop().add(consumables.VOLTTOP, 3).add(consumables.ALCTHUN, 2).add(useables.EL_CORE, 1);
