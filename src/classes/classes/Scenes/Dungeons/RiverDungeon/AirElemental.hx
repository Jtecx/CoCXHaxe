...
class AirElemental extends Monster
{
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        if (flags[kFLAGS.RIVER_DUNGEON_ELEMENTAL_MIXER] == 4)
        {
            SceneLib.dungeons.riverdungeon.defeatAirElementalSubBoss();
        }
        else
        {
            cleanupAfterCombat();
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (flags[kFLAGS.RIVER_DUNGEON_ELEMENTAL_MIXER] == 4)
        {
            SceneLib.dungeons.riverdungeon.defeatedByAirElementalSubBoss();
        }
        else
        {
