...
class WaterElemental extends Monster
{
    ...
    override private function performCombatAction() : Void
    {
        ...
        else if (hasStatusEffect(StatusEffects.Provoke))
        {
            var choiceP : Float = rand(5);
            if (choiceP < 4)
            {
                fluffyOfPunches();
            }
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        if (flags[kFLAGS.RIVER_DUNGEON_ELEMENTAL_MIXER] == 4)
        {
            SceneLib.dungeons.riverdungeon.defeatWaterElementalSubBoss();
        }
        else
        {
            cleanupAfterCombat();
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (flags[kFLAGS.RIVER_DUNGEON_ELEMENTAL_MIXER] == 4)
        {
            SceneLib.dungeons.riverdungeon.defeatedByWaterElementalSubBoss();
        }
        else
        {
    ...
    public function new()
    {
        ...
        if (flags[kFLAGS.RIVER_DUNGEON_ELEMENTAL_MIXER] == 4)
        {
            ...
            this.drop = new WeightedDrop().add(useables.LELSHARD, 3).add(useables.ELCRYST, 1);
        }
        else
        {
            this.createPerk(PerkLib.EnemyEliteType, 0, 0, 0, 0);
            this.drop = new WeightedDrop().add(useables.ELSHARD, 3).add(useables.LELSHARD, 1);
        }
        checkMonster();
    }
}
