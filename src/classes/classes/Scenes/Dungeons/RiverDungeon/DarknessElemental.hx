...
class DarknessElemental extends Monster
{
    public function baseElementalAttack() : Void
    {
        ...
        if (player.hasPerk(PerkLib.DarknessAffinity) || player.hasPerk(PerkLib.FireShadowAffinity))
        {
            damage *= 0.3;
        }
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        if (flags[kFLAGS.RIVER_DUNGEON_ELEMENTAL_MIXER] == 4)
        {
            SceneLib.dungeons.riverdungeon.defeatDarknessElementalSubBoss();
        }
        else
        {
            cleanupAfterCombat();
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (flags[kFLAGS.RIVER_DUNGEON_ELEMENTAL_MIXER] == 4)
        {
            SceneLib.dungeons.riverdungeon.defeatedByDarknessElementalSubBoss();
        }
        else
        {
            SceneLib.dungeons.riverdungeon.defeatedByDarknessElemental();
        }
    }
    ...
    public function new()
    {
        ...
        if (flags[kFLAGS.RIVER_DUNGEON_ELEMENTAL_MIXER] == 4)
        {
            ...
            this.drop = new WeightedDrop().add(useables.ELCRYST, 3).add(useables.LELSHARD, 1);
        }
        else
        {
