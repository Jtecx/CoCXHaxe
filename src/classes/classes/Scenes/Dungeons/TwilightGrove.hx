...
class TwilightGrove extends DungeonAbstractContent
{
    ...
    {
        inDungeon = true;
        if ((player.cor >= 51 || player.isRaceCached(Races.ALRAUNE)) && flags[kFLAGS.TWILIGHT_GROVE_PURIFICATION] < 1)
        {
            room1TGCorrupt();
        }
        else if (flags[kFLAGS.TWILIGHT_GROVE_PURIFICATION] > 1)
        {
            room1TG();
        }
        else
    ...
    private function room1TGCorrupt() : Void
    {
        ...
        {
            if (flags[kFLAGS.PC_CANT_DECIDE_ON_BEEN_ALRAUNE_OR_NOT] == 1 && !playerIsAlraune())
            {
                flags[kFLAGS.PC_CANT_DECIDE_ON_BEEN_ALRAUNE_OR_NOT] = 0;
            }
            menu();
            addButton(3, "Leave", room1TGCorruptLeave);
            if (playerIsAlraune())
            {
                if (flags[kFLAGS.PC_CANT_DECIDE_ON_BEEN_ALRAUNE_OR_NOT] == 0)
                {
                    room1TGCorruptEnter2();
                }
                else
                {
            ...
            else
            {
                ...
                if (flags[kFLAGS.DISCOVERED_TWILIGHT_GROVE_DUNGEON] > 2)
                {
                ...
                else
                {
                    ...
                    if (flags[kFLAGS.DISCOVERED_TWILIGHT_GROVE_DUNGEON] == 2)
                    {
                        addButtonIfTrue(7, "VIP", corruptTGHaveARestVIPBuyItBuyItNowBeforeItPriceRise, "Req. 500+ gems", player.gems >= 500);
                    }
                }
            }
        }
        else
        {
            ...
            if (flags[kFLAGS.DISCOVERED_TWILIGHT_GROVE_DUNGEON] < 1)
            {
                flags[kFLAGS.DISCOVERED_TWILIGHT_GROVE_DUNGEON] = 1;
            }
            menu();
    ...
    private function room1TGCorruptEnter() : Void
    {
        ...
        {
            outputText("\"<i>Oh, a fleshling… I guess I should've gotten used to them by now. I'm afraid the staff is full, but if you are looking for something other than quality time, perhaps an extra job, I could use you. Perhaps in return for my gracious hospitality, you could gather a few items for me, namely Succubi’s Delight. As for the presentation, my name is Rafflesia, daughter of Marae.</i>\"\n\n");
        }
    ...
    private function corruptTGHaveARestPay() : Void
    {
        ...
        if (flags[kFLAGS.DISCOVERED_TWILIGHT_GROVE_DUNGEON] == 2 && !playerIsAlraune())
        {
            ...
            else if (player.hasItem(consumables.INCUBID, 1))
            {
    ...
    private function corruptTGHaveARest2() : Void
    {
        ...
        else
        {
            ...
            if (player.gender == 1)
            {
                outputText("plug your throbbing cock");
            }
            if (player.gender == 2)
            {
                outputText("seek out your needy pussy");
            }
            if (player.gender == 3)
            {
                outputText("plug both your pussy and cock");
            }
            outputText(".\n\n");
        }
    ...
    private function playerIsAlraune() : Bool
    {
    ...
    public function meetTwilightGroveRandomnMob() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.TGRandomnMob, 1, 20);
        }
    }
    ...
    public function room3TG() : Void
    {
        ...
        {
            outputText("This is the place where you met the satyr musician. The satyr’s body is still on the ground, a testimony of your victory over his mind games.");
        }
        if (CoC.instance.inCombat)
        {
            return;
