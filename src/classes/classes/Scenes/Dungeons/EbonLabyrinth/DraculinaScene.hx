...
class DraculinaScene extends BaseContent
{
    public function new()
    {
        super();
    ...
    private function allTheTricksForDarkness() : Bool
    {
        if (player.hasStatusEffect(StatusEffects.KnowsBlind) || player.hasStatusEffect(StatusEffects.KnowsSunrise) || (player.tailType == Tail.KITSHOO && player.tailCount >= 6))
        {
            return true;
        }
        else
    ...
    {
        if (player.wis >= 150 && allTheTricksForDarkness())
        {
            encounterYes();
        }
        else
    ...
    public function encounterYes() : Void
    {
        ...
        if (player.racialScore(Races.DRACULA) >= 22)
        {
            ...
            if (player.gender > 0 || player.breastRows[0].breastRating >= 1)
            {
                ...
                if (player.hasCock())
                {
                    outputText("[cock] hardens and throbs ");
                }
                if (player.gender == 3)
                {
                ...
                if (player.hasVagina())
                {
                    outputText("pussy gushes a cascade of girlcum, falling down your body with a wet plopping sound to the ground below ");
                }
    ...
    private function becomeDracula() : Void
    {
        ...
        player.skin.setBaseOnly({
                    type : Skin.PLAIN,
                    color1 : "pale",
                    pattern : Skin.PATTERN_BLOOD_MAGIC_TATTOO
                });
        ...
        {
            flags[kFLAGS.APEX_SELECTED_RACE] = Races.DRACULA;
        }
