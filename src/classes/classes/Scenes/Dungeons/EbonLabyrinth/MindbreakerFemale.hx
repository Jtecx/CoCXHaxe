...
class MindbreakerFemale extends Monster
{
    ...
    private function mindbreakerMindTrust() : Void
    {
        ...
        var damage : Float = eBaseIntelligenceDamage() * 1.5;
    ...
    private function mindbreakerMindBreak() : Void
    {
        ...
        {
            player.buff("Mind Break").addStats({
                                        int : -25
                                    }).withText("Mind Break").combatPermanent();
            if (player.inte < 50)
            {
                if (player.hasVagina())
                {
                    if (player.intStat.train.value >= 50)
                    {
                        player.intStat.train.value -= 50;
                    }
                    else if (player.intStat.core.value >= 50)
                    {
                        player.intStat.core.value -= 50;
                    }
                    else
                    {
                        player.intStat.train.value = 0;
                    }
                }
    ...
    private function mindbreakerGrapple() : Void
    {
        ...
        {
            outputText("You barely manage to evade her embrace!");
        }
        else
        {
