...
class AtlachNachaScene extends BaseContent
{
    public function new()
    {
        super();
    ...
    private function becomeAtlach() : Void
    {
        ...
        if (!player.hasVagina())
        {
            outputText(" Such a strange accommodation, one you know you'll get pleasure out of.");
        }
    ...
    public function defeat() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.TFDealer2) && player.statusEffectv2(StatusEffects.TFDealer2) < 1)
        {
            player.addStatusValue(StatusEffects.TFDealer2, 2, 1);
        }
        else
        {
            player.createStatusEffect(StatusEffects.TFDealer2, 0, 1, 0, 0);
        }
        cleanupAfterCombat();
    }
}
