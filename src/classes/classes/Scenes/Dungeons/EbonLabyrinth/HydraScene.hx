...
class HydraScene extends BaseContent
{
    public function new()
    {
        super();
    ...
    public function encounter() : Void
    {
        ...
        if (player.isNaga())
        {
            ...
            if (player.hasStatusEffect(StatusEffects.TFDealer1) && player.statusEffectv2(StatusEffects.TFDealer1) < 1)
            {
                player.addStatusValue(StatusEffects.TFDealer1, 2, 1);
            }
            else
            {
                player.createStatusEffect(StatusEffects.TFDealer1, 0, 1, 0, 0);
            }
            menu();
            addButton(1, "Sex", sex);
