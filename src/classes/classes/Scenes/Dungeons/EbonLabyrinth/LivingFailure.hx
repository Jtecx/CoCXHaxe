...
class LivingFailure extends Monster
{
    ...
    private function livingFailureCorruptEruption() : Void
    {
        ...
        outputText("\n\n");
        player.takeLustDamage(10 + as3hx.Compat.parseInt(player.lib / 10 + player.effectiveSensitivity() / 10 + player.cor / 10), true);
    }
    ...
    private function livingFailureDefileBody() : Void
    {
        ...
        if (player.hasCock())
        {
            if (player.hasVagina())
            {
                outputText(", [cock] and clit");
            }
            else
            {
                outputText(" and penis");
            }
        }
        if (player.hasVagina())
        {
    ...
    {
        if (!hasStatusEffect(StatusEffects.LustTransferance) && rand(4) == 0)
        {
            livingFailureLustTransferance();
        }
        else
        {
            ...
            if (choice == 0)
            {
                ...
                {
                    livingFailureMadness();
                }
                else
                {
                    livingFailureIdInsinuation();
                }
            }
            if (choice == 1)
            {
            ...
            if (choice == 2)
            {
                livingFailurePrimalDemonfire();
            }
            if (choice == 3)
            {
                if (player.hasStatusEffect(StatusEffects.Sealed))
                {
                    livingFailureDefileBody();
                }
                else
                {
                    livingFailureEclipse();
                }
            }
            if (choice == 4)
            {
                livingFailureCorruptEruption();
            }
            if (choice == 5)
            {
                livingFailureDefileBody();
