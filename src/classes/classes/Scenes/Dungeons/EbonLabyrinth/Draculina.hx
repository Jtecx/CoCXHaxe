...
class Draculina extends Monster
{
    ...
    public function dispellDarkness(choice : Float) : Void
    {
        ...
        if (choice == 2)
        {
            if (game.player.hasStatusEffect(StatusEffects.BloodCultivator))
            {
                game.player.HP -= 400;
            }
            else
            {
                game.player.soulforce -= 400;
            }
        }
    ...
    public function draculinaBite() : Void
    {
        ...
        {
            addHP(maxHP() * .05);
            var dam : Int = as3hx.Compat.parseInt(this.str * 5);
            for (i in 0..._biteCounter)
            {
                dam += as3hx.Compat.parseInt(dam * .10);
            }
            _biteCounter++;
            ...
            {
                var drain : Float = Math.round(player.touStat.max * 0.05);
                player.saveHPRatio();
                player.buff("Bat bites").addStats({
                                            tou : -drain
                                        }).withText("Bat bites!").combatPermanent();
    ...
    override private function performCombatAction() : Void
    {
        ...
        else
        {
            ...
            if (choice == 1)
            {
                if (!player.hasStatusEffect(StatusEffects.MonsterInvisible) && !hasStatusEffect(StatusEffects.AbilityCooldown1))
                {
                    draculinaPerfectDark();
                }
                else
