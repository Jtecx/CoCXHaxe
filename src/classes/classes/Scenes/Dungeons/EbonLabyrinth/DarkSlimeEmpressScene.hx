...
class DarkSlimeEmpressScene extends BaseContent
{
    public function new()
    {
        super();
    ...
    public function defeat() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.TFDealer1) && player.statusEffectv1(StatusEffects.TFDealer1) < 1)
        {
            player.addStatusValue(StatusEffects.TFDealer1, 1, 1);
        }
        else
        {
            player.createStatusEffect(StatusEffects.TFDealer1, 1, 0, 0, 0);
        }
        cleanupAfterCombat();
    }
}
