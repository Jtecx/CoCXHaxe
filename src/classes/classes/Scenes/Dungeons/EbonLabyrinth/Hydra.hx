...
class Hydra extends Monster
{
    ...
    {
        var damage : Float = this.inte * 0.2;
        ...
        if (!player.immuneToAcid())
        {
            ...
            {
                player.addStatusValue(StatusEffects.AcidDoT, 2, 10);
            }
            //More heads will produce more potent acid
            else
            {
                player.createStatusEffect(StatusEffects.AcidDoT, 6, 10, 0, 0);
            }
        }
    }
    ...
    {
        var damage : Float = this.str * 0.3;
        damage += this.spe * 0.3;
        var damage2 : Float = 0.9;
        ...
        {
            var multiplier : Float = this.statusEffectv1(StatusEffects.HydraTails);  //more heads -> more venom  
    ...
    {
        if (HPRatio() < .75 && this.statusEffectv1(StatusEffects.HydraTails) < 12 && !hasStatusEffect(StatusEffects.AbilityCooldown2))
        {
            hydraGrowHead();
        }
        else if (!hasStatusEffect(StatusEffects.AbilityCooldown1) && rand(3) == 0)
        {
