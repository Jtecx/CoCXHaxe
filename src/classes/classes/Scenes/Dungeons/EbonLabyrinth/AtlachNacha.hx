...
class AtlachNacha extends Monster
{
    ...
    private function atlachNachaBite() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.ArcaneWeb))
        {
            outputText("sinks its fangs in you and the wound begins to heat");
        }
        else
        {
            outputText("rushes towards you face first and you back away feeling for two new pinpricks that start heating");
        }
    ...
    override private function performCombatAction() : Void
    {
        ...
        else if (player.hasStatusEffect(StatusEffects.ArcaneWeb) && rand(4) > 0)
        {
            atlachNachaBite();
        }
        else
