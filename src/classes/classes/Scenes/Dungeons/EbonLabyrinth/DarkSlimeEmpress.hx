...
class DarkSlimeEmpress extends Monster
{
    private function gooHaremStrike() : Void
    {
        ...
        if (player.gender == 0)
        {
            outputText("empty nether");
        }
        outputText(" religiously in an all out effort to make you cum the fluids they all so crave");
        if (player.gender == 0)
        {
            outputText(", though you doubt they’ll get any due to your absence of endowments");
        }
        outputText(". ");
        if (player.isLactating())
        {
            outputText("One of them even made suction cup tentacles in order to milk your breasts. ");
        }
        ...
        {
            createStatusEffect(StatusEffects.LingeringSlime, 0, 0, 0, 0);
        }
    }
    ...
    private function gooSlimeBarrage() : Void
    {
        ...
        while (r <= 85)
        {
            if (player.spe < ((2 * r * (player.newGamePlusMod() + 1)) + rand(r)))
            {
                gooSlimeBarrageD();
            }
            r += 5;
    ...
    private function gooGroupGrapple() : Void
    {
        ...
        {
            outputText("You barely manage to break out of their clingy bodies!");
        }
        else
        {
    ...
    override private function performCombatAction() : Void
    {
        if (hasStatusEffect(StatusEffects.LingeringSlime))
        {
            ...
            player.takeLustDamage(10 + as3hx.Compat.parseInt(player.lib / 12 + player.cor / 14), true);
    ...
    public function new()
    {
        ...
        this.abilities = [
                ...
                }, 
                {
                    ...
                    {
                        return player.hasStatusEffect(StatusEffects.GooBind);
                    },
                    weight : Infinity
                }, 
                {
                    call : gooSlimeBarrage,
                    type : ABILITY_PHYSICAL,
                    range : RANGE_RANGED,
                    tags : [TAG_FLUID]
                }
