...
class HellfireSnailScene extends BaseContent
{
    public function new()
    {
        super();
    ...
    public function encounter() : Void
    {
        ...
        else if (flags[kFLAGS.HELLFIRE_SNAIL_ENC] == 1)
        {
            sceneHunter.print("Check failed: Fire Affinity perk!");
        }
        ...
        if (flags[kFLAGS.HELLFIRE_SNAIL_ENC] < 1)
        {
            flags[kFLAGS.HELLFIRE_SNAIL_ENC] = 1;
        }
    ...
    public function defeat() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.TFDealer1) && player.statusEffectv3(StatusEffects.TFDealer1) < 1)
        {
            player.addStatusValue(StatusEffects.TFDealer1, 3, 1);
        }
        else
        {
            player.createStatusEffect(StatusEffects.TFDealer1, 0, 0, 1, 0);
        }
        cleanupAfterCombat();
    }
    ...
    public function hellfireSnailSex() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(" as you fill her smoldering lovehole with [race] seeds");
        }
        if (!player.hasCock())
        {
            outputText(" as your girl juice mix with hers");
        }
