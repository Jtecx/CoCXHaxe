...
class RiverDungeon extends DungeonAbstractContent
{
    public function new()
    {
        super();
    ...
    public function enterDungeon() : Void
    {
        ...
        {
            player.createStatusEffect(StatusEffects.RivereDungeonIB, 0, 0, 0, 0);
        }
        playerMenu();
    }
    public function exitDungeon() : Void
    {
        ...
        {
            outputText("You leave the river dungeon behind and take off, heading through He'Xin'Dao and back to camp.");
        }
    ...
    private function encountersRulette() : Bool
    {
    ...
    public function takeAetherSister1() : Void
    {
        ...
        {
            outputText(" You even have my twin sister.");
        }
    ...
    public function takeAetherSister1b() : Void
    {
        ...
        if (flags[kFLAGS.AETHER_SINISTER_TWIN_AT_CAMP] == 2)
        {
            outputText("</i>\" After this you hear other voice in your head. \"<i>The Aether twins united. Go go go twins!!!");
        }
        outputText("</i>\"\n\n");
        if (item == null)
        {
            inventory.takeItem(weapons.AETHERD, playerMenu);
        }
        // failed to equip
    ...
    public function takeAetherSister2b() : Void
    {
        ...
        if (flags[kFLAGS.AETHER_DEXTER_TWIN_AT_CAMP] == 2)
        {
            outputText("</i>\" After this you hear other voice in your head. \"<i>The Aether twins are united!");
        }
        outputText("</i>\"\n\n");
        if (item == null)
        {
            inventory.takeItem(shields.AETHERS, playerMenu);
        }
        // failed to equip
        else if (item.isNothing)
        {
    ...
    private function encountersRuletteBFire() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.RiverDungeonA, 1, 20);
        }
    }
    ...
    public function defeatedByQuatroElementalBoss() : Void
    {
        ...
        if (player.statusEffectv1(StatusEffects.RivereDungeonIB) == 3)
        {
            outputText("Flames");
        }
        if (player.statusEffectv4(StatusEffects.RivereDungeonIB) == 3)
        {
            outputText("Magma");
        }
        if (player.statusEffectv2(StatusEffects.RivereDungeonIB) == 3)
        {
            outputText("Magma and steam");
        }
        if (player.statusEffectv3(StatusEffects.RivereDungeonIB) == 3)
        {
            outputText("Magma, steam and razor sharp air currents");
        }
    ...
    private function encountersRuletteCDarkness() : Void
    {
        if (encountersRulette())
        {
            ...
            player.createStatusEffect(StatusEffects.ThereCouldBeOnlyOne, 0, 0, 0, 0);
            //spriteSelect(SpriteDb.s_green_slime);
            if (player.hasKeyItem("Key Of Darkness") >= 0 || flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] > 7)
            {
                flags[kFLAGS.RIVER_DUNGEON_ELEMENTAL_MIXER] = rand(4);
            }
            else
            {
                var rdem : Float = rand(5);
                if (rand(3) > 0)
                {
                    rdem = 4;
                }
            ...
            {
                outputText("A soft shuffling sound catches your attention and you turn around, spotting a small ball of darkness rushing towards you!  Realizing it's been spotted, the elemental's body shoots up from the ground, shadowy tendrils expanding from its inky mass. Arms and legs grown, it attacks!");
            }
            startCombat(new DarknessElemental(), true);
        }
        else
    ...
    {
        if (!player.hasPerk(PerkLib.FireAffinity) && !player.hasPerk(PerkLib.FireShadowAffinity) && !player.hasPerk(PerkLib.AffinityIgnis))
        {
            player.createStatusEffect(StatusEffects.ConstantHeatConditions, 1, 0, 0, 0);
        }
    }
    ...
    private function SubZeroConditions() : Void
    {
        ...
        {
            player.createStatusEffect(StatusEffects.SubZeroConditions, 1, 0, 0, 0);
        }
    }
    ...
    private function ConstantHeatAndSubZeroConditions() : Void
    {
        if (!player.hasPerk(PerkLib.FireAffinity) && !player.hasPerk(PerkLib.FireShadowAffinity) && !player.hasPerk(PerkLib.AffinityIgnis))
        {
            player.createStatusEffect(StatusEffects.ConstantHeatConditions, 2, 0, 0, 0);
        }
        if (!player.hasPerk(PerkLib.ColdAffinity))
        {
            player.createStatusEffect(StatusEffects.SubZeroConditions, 2, 0, 0, 0);
        }
    }
    ...
    public function roomA01() : Void
    {
        ...
        if (flags[kFLAGS.NEISA_AFFECTION] < 10 && flags[kFLAGS.NEISA_FOLLOWER] < 4)
        {
            addButtonDisabled(10, "Up", "You would leave now but you have yet to find enough to pay Neisa.");
        }
        else
        {
            addButton(10, "Up", exitDungeon);
        }
    }
    ...
    public function roomA03() : Void
    {
        ...
        if (flags[kFLAGS.NEISA_AFFECTION] < 5 && flags[kFLAGS.NEISA_FOLLOWER] < 4)
        {
            tresureRoom1A();
        }
    }
    ...
    public function roomA06() : Void
    {
        ...
        if (flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] > 1)
        {
            if (flags[kFLAGS.NEISA_FOLLOWER] >= 3)
            {
                addButton(11, "Down", roomB01);
            }
            else
            {
                addButtonDisabled(11, "Down", "You still need to find enough (two piles) of treasure to pay back shield maiden for help to get into this dungeon. Maybe search again whole floor carefully?");
            }
        }
        else
        {
            addButtonDisabled(11, "Down", "You still need to beat the guardians of this floor to descend into lower strata of the dungeon.");
        }
    }
    ...
    public function roomA15() : Void
    {
        ...
        else if (!player.hasStatusEffect(StatusEffects.ThereCouldBeOnlyOne))
        {
            encountersRuletteA();
        }
        if (CoC.instance.inCombat)
    ...
    public function roomB08() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.RivereDungeonIB) && (player.statusEffectv1(StatusEffects.RivereDungeonIB) == 1 || player.statusEffectv1(StatusEffects.RivereDungeonIB) == 3))
        {
            outputText("burning with a red flame");
        }
        else
        {
            outputText("is extinguished");
        }
        outputText(".\nNortheast one ");
        if (player.hasStatusEffect(StatusEffects.RivereDungeonIB) && (player.statusEffectv2(StatusEffects.RivereDungeonIB) == 1 || player.statusEffectv2(StatusEffects.RivereDungeonIB) == 3))
        {
            outputText("burning with a blue flame");
        }
        else
        {
            outputText("is extinguished");
        ...
        outputText(".\nSoutheast one ");
        if (player.hasStatusEffect(StatusEffects.RivereDungeonIB) && (player.statusEffectv3(StatusEffects.RivereDungeonIB) == 1 || player.statusEffectv3(StatusEffects.RivereDungeonIB) == 3))
        {
            outputText("burning with a white flame");
        }
        else
        {
            outputText("is extinguished");
        ...
        outputText(".\nSouthwest one ");
        if (player.hasStatusEffect(StatusEffects.RivereDungeonIB) && (player.statusEffectv4(StatusEffects.RivereDungeonIB) == 1 || player.statusEffectv4(StatusEffects.RivereDungeonIB) == 3))
        {
            outputText("burning with a brown flame");
        }
        else
        {
            outputText("is extinguished");
    ...
    public function roomB15() : Void
    {
        ...
        if (flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] > 6)
        {
            addButton(6, "Down", roomC01);
        }
        else
        {
            addButtonDisabled(6, "Down", "You still need to beat the guardian of this floor to descend further into the dungeon");
        }
    }
    ...
    public function roomC20() : Void
    {
        ...
        {
            addButton(11, "Down", roomD07);
        }
        else
        {
            addButtonDisabled(11, "Down", "You still need to beat guardian of this floor to descend into lower strata of the dungeon.");
        }
    }
    ...
    public function roomC32() : Void
    {
        ...
        outputText("You stand in a massive hall decorated with lavishly ornate trinkets. The baubles glimmer in the light, many of which resemble ice, snowflakes or bolts of lightning.\n\n");
        outputText("The walls to the manor have paintings depicting a battle between a man and his foes, the monsters range in size from humans to giants. It's difficult to distinguish what exactly the paintings were, time has long since faded the colors. Holes in the canvas trail strands of material.");
        if (flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] > 7)
        {
            dungeons.setDungeonButtonsRD(roomC33, roomC31, null, roomC28);
        }
        else
        {
            ...
            if (player.hasKeyItem("Key Of Darkness") >= 0)
            {
                addButton(0, "Insert Key", insertTheKey).hint("Now you have the key. Do you open the double door?");
            }
            else
            {
                addButtonDisabled(0, "Insert Key", "Would you kindly find the key first?");
            }
        }
    }
    ...
    public function roomD03() : Void
    {
        ...
        if (flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] > 11)
        {
        ...
        else
        {
            ...
            if (player.hasKeyItem("Black Crystal") >= 0)
            {
                addButton(10, "West", useBlackCrystalRe).hint("Would you like to use Black Crystal here?");
            }
            else
            {
                addButtonDisabled(10, "West", "There are massive rumble that even with any previous visitors attempts seems to be not clearable. Looking like whole celling just caved in and blocked passage to room behind it. So how could you go to check what is on the other end of passage? Maybe somewhere on this floor you can find the solution? You also feel lingering aura of floor boss here suggesting even with correct solution boss itself could interfere with attempts to clear this rumble...");
            }
        }
    }
    ...
    public function roomD05() : Void
    {
        ...
        if (flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] > 9)
        {
            dungeons.setDungeonButtonsRD(roomD01, roomD08, null, null);
        }
        else
        {
            ...
            if (player.hasKeyItem("Black Crystal") >= 0)
            {
                addButton(6, "North", useBlackCrystal).hint("Would you like to use Black Crystal here?");
            }
            else
            {
    ...
    public function roomD12() : Void
    {
        ...
        if (anvilMaterialsCheck())
        {
            addButton(0, "Anvil", anvilUncrafting).hint("Now you have needed materials. Do you uncraft them?");
        }
        else
        {
            addButtonDisabled(0, "Anvil", "Would you kindly find the materials first?");
        }
    ...
    public function roomE01() : Void
    {
        ...
        {
            addButton(10, "Down", roomF01);
        }
        else
        {
            addButtonDisabled(10, "Down", "Staircase is blocked by impenetrable thorn wall. Maybe go check floor boss first to do something about this wall?");
        }
    }
    ...
    private function insertTheKey() : Void
    {
        ...
        if (flags[kFLAGS.PLAYER_COMPANION_1] != "")
        {
            compname = "" + flags[kFLAGS.PLAYER_COMPANION_1] + "";
        }
        else if (flags[kFLAGS.PLAYER_COMPANION_2] != "")
        {
            compname = "" + flags[kFLAGS.PLAYER_COMPANION_2] + "";
        }
        else if (flags[kFLAGS.PLAYER_COMPANION_3] != "")
        {
            compname = "" + flags[kFLAGS.PLAYER_COMPANION_3] + "";
        }
        ...
        if (player.hasKeyItem("Key Of Darkness") >= 0)
        {
            player.removeKeyItem("Key Of Darkness");
        }
        player.createStatusEffect(StatusEffects.ThereCouldBeOnlyOne, 0, 0, 0, 0);
        flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS]++;
        doNext(roomC32);
    }
    private function anvilMaterialsCheck() : Bool
    {
    ...
    private function anvilUncrafting() : Void
    {
        clearOutput();
        menu();
        if (player.hasItem(useables.PCSHARD, 6))
        {
            addButton(0, "???", anvilUncrafting2, 2).hint("Purple Crystal");
        }
        ...
        if (player.hasItem(useables.RED_GEL, 1) && player.hasItem(consumables.CHOCBOX, 1) && player.hasItem(consumables.LETHITE, 1) && player.hasItem(consumables.SALAMFW, 1) && player.hasItem(useables.SRESIDUE, 1) && player.hasItem(consumables.ONISAKE, 1))
        {
            if (player.hasKeyItem("Black Crystal") >= 0)
            {
                addButtonDisabled(7, "Black Crystal", "You can't craft 2nd one until you use the one you have.");
            }
            else
            {
                addButton(7, "Black Crystal", anvilUncrafting2, 1);
            }
        }
        addButton(14, "Back", roomD12);
    }
    private function anvilUncrafting2(type : Float = 0) : Void
    {
        clearOutput();
        ...
        if (mainitem == "Black Crystal")
        {
            outputText("<b>You have gained Key Item: Black Crystal</b>");
        }
        if (mainitem == "Black Crystal")
        {
            doNext(anvilUncrafting);
        }
        else
        {
            inventory.takeItem(item, anvilUncrafting);
        }
    }
    private function useBlackCrystal() : Void
    {
        ...
        if (player.hasKeyItem("Black Crystal") >= 0)
        {
            player.removeKeyItem("Black Crystal");
        }
        flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS]++;
        doNext(roomD05);
    }
    ...
    public function dishHelperTB() : Void
    {
        ...
        if (flags[kFLAGS.PLAYER_COMPANION_1] != "")
        {
            addButton(0, "1st", dishHelperTB2, 1).hint("Ask your first henchman to deal with it.");
        }
        if (flags[kFLAGS.PLAYER_COMPANION_2] != "")
        {
            addButton(1, "2nd", dishHelperTB2, 2).hint("Ask your second henchman to deal with it.");
        }
    }
    public function dishHelperTB2(henchman : Float = 0) : Void
    {
        clearOutput();
        ...
        if ((henchman == 1 && flags[kFLAGS.PLAYER_COMPANION_1] == "Neisa") || (henchman == 2 && flags[kFLAGS.PLAYER_COMPANION_2] == "Neisa"))
        {
            ...
            if (silly())
            {
                outputText("She speaks through her exertion, \"<i>Stop resisting! I said stop resisting! You onisunes think you’re so high and mighty, just above the law don’t you?!</i>\"\n\n");
            }
            else
            {
                outputText("She speaks through her exertion, \"<i>I need you to stay down!</i>\"\n\n");
            }
        }
    ...
    private function teleportCircleFloor1() : Void
    {
        ...
        if (flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] > 1 && flags[kFLAGS.NEISA_FOLLOWER] >= 3)
        {
            addButton(1, "Floor 2", teleportToFloor2);
        }
        else
        {
            addButtonDisabled(1, "Floor 2", "You still need to beat guardians of floor 1 to use this teleport option. Also need to be able to leave dungeon at any time (paying back shield maiden for her help).");
        }
        if (flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] > 6)
        {
        ...
        {
            addButtonDisabled(2, "Floor 3", "You still need to beat guardian of floor 2 to use this teleport option.");
        }
        if (flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] > 8)
        {
        ...
        {
            addButtonDisabled(3, "Floor 4", "You still need to beat guardian of floor 3 to use this teleport option.");
        }
        if (flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] > 11)
        {
        ...
        {
            addButtonDisabled(4, "Floor 5", "You still need to beat guardians of floor 4 to use this teleport option.");
        }
        if (flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] > 12)
        {
            addButton(5, "Floor 6", teleportToFloor6);
        }
        else
        {
            addButtonDisabled(5, "Floor 6", "You still need to beat guardian of floor 5 to use this teleport option.");
        }
        //7
    ...
    private function teleportCircleFloor2() : Void
    {
        ...
        {
            addButtonDisabled(2, "Floor 3", "You need to beat the guardian of floor 2 to use this teleport.");
        }
        if (flags[kFLAGS.RIVER_DUNGEON_FLOORS_PROGRESS] > 8)
        {
            addButton(3, "Floor 4", teleportToFloor4);
