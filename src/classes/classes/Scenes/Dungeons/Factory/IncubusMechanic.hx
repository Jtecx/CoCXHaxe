...
class IncubusMechanic extends Monster
{
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        ...
        {
            SceneLib.dungeons.factory.incubusMechanicDefeated(hpVictory);
        }
        else
        {
    ...
    private function wonInDungeon1(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (pcCameWorms)
        {
    ...
    private function wonInDungeon3(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        ...
        {
            SceneLib.d3.incubusMechanic.mechanicFuckedYouUp(hpVictory, pcCameWorms);
        }
    }
    ...
    private function spoogeAttack() : Void
    {
        ...
        switch (rand(3))
        {
            case 0:  //Face  
                ...
                {
                    player.addStatusValue(StatusEffects.DemonSeed, 1, 7);
                }
                player.slimeFeed();
            case 1:  //Chest  
            ...
            {
                outputText(allBreastsDescript() + ".  Thankfully it doesn't seem to have much effect.");
            }
            default:  //Crotch  
                ...
                {
                    outputText("crotch.  Thankfully, it doesn't seem to have much effect.");
                }
        }
        lust -= 10;
        if (lust < 0)
    ...
    public function new()
    {
        ...
        if (flags[kFLAGS.D3_MECHANIC_LAST_GREET] > 0)
        {
            this.createPerk(PerkLib.OverMaxHP, 68, 0, 0, 0);
        }
        else
        {
