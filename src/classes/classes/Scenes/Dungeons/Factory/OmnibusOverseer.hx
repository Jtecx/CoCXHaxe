...
class OmnibusOverseer extends Monster
{
    ...
    private function lustAura() : Void
    {
        ...
        if (hasStatusEffect(StatusEffects.LustAura))
        {
            ...
            player.takeLustDamage(eBaseLibidoDamage() / 20 + as3hx.Compat.parseInt(player.lib / 20 + player.cor / 25), true);
        }
        else
        {
    ...
    private function milkAttack() : Void
    {
        ...
        else
        {
            ...
            {
                outputText("Milk dribbles from your [allbreasts] in sympathy.");
            }
