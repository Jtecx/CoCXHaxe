...
class EbonLabyrinth extends DungeonAbstractContent
{
    public var enemyLevelMod(get, never) : Int;
    ...
    private function get_enemyLevelMod() : Int
    {
        return as3hx.Compat.parseInt(as3hx.Compat.parseInt(room / 50) + depth);
    }
    ...
    public function resetEncChance() : Void
    {
        encChance = 10;
    }
    public function incEncChance() : Void
    {
        encChance += 10;
    }
    
    //Direction constants
    public static inline var DIR_NORTH : Int = 0;
    public static inline var DIR_WEST : Int = 1;
    public static inline var DIR_SOUTH : Int = 2;
    public static inline var DIR_EAST : Int = 3;
    public static inline var DIR_UP : Int = 4;
    public static inline var DIR_DOWN : Int = 5;
    ...
    public var fountainRoom : Bool = false;
    public var fountainCorrupt : Bool = false;
    ...
    */
    ...
    private var bossPool : Array<Dynamic>;
    ...
    public function setDungeonButtonsEL() : Void
    {
        ...
        if (room == 1)
        {
            addButtonIfTrue(7, "Cat", shortcuts, "You not even beaten ANY boss yet.", flags[kFLAGS.EBON_LABYRINTH_RECORD] >= 50, "Talk to the cat only if you plan skip some rooms.");
        }
    ...
    public function roomStatic(move : Bool = false, newDir : Int = DIR_DOWN) : Void
    {
        clearOutput();
        ...
        else
        {
            ...
            else if (direction == DIR_UP || direction == DIR_DOWN || newDir == direction)
        ...
        if (fountainRoom)
        {
            addButton(10, "Fountain", encountersUpgradeFountain, true);
        }
    }
    ...
    //clear room-specific
    {
        ...
        //minLevel must be checked already{
    ...
    private function navigateToXRoom(rooms : Float = 50) : Void
    //move
    {
        
        room += as3hx.Compat.parseInt(rooms);
        resetEncChance();
        bossSelector(0);
    }
    ...
    public function highScore() : Void
    {
        if (flags[kFLAGS.EBON_LABYRINTH_RECORD] < room)
        {
            ...
            var _sw0_ = (flags[kFLAGS.EBON_LABYRINTH_RECORD]);            
    ...
    public function selectEncounter() : Bool
    //Every 50 levels - boss
    {
        ...
        {
            //Room 49 - Alvina's quest chimeraif (room >= 49 && flags[kFLAGS.ALVINA_FOLLOWER] == 17)
            {
    ...
    private function encountersUpgradeFountain(repeat : Bool = false) : Void
    {
        clearOutput();
        if (!repeat)
        {
    ...
    private function encountersLootChest() : Void
    {
        ...
        var exp : Int = as3hx.Compat.parseInt(2000 * Math.exp(0.2 * enemyLevelMod));
        var gems : Int = as3hx.Compat.parseInt(500 * Math.exp(0.2 * enemyLevelMod));
        player.XP += exp;
        player.gems += gems;
    ...
    //Make the pool of encounters
    {
        ...
        var boss : Int;
        for (boss in 0...bossPool[tier].length)
        
        //[bit_num, function]{
            
            if (!getBit(bossTracker, bossPool[tier][boss][0]))
        ...
        bossPool[tier][boss][1]();
    }
    
    //selects an enemy and starts the encounter
    //'print' argument disables printing (used for night ambushes)
    private function enemySelector(print : Bool = true) : Void
    {
        var choices : Array<Dynamic> = [displacerEL, darkSlimeEL, succubusEL, incubusEL, amogusEL, tentabeastEL, minotaurEL, mindbreakerEL];
    ...
    private function displacerEL(print : Bool = true) : Void
    {
        if (print)
        {
            clearOutput();
    ...
    private function darkSlimeEL(print : Bool = true) : Void
    {
    ...
    private function succubusEL(print : Bool = true) : Void
    {
        if (print)
        {
            clearOutput();
    ...
    private function incubusEL(print : Bool = true) : Void
    {
        if (print)
        {
            clearOutput();
    ...
    private function amogusEL(print : Bool = true) : Void
    {
        if (print)
        {
            clearOutput();
    ...
    private function tentabeastEL(print : Bool = true) : Void
    {
        if (print)
        {
            clearOutput();
    ...
    private function minotaurEL(print : Bool = true) : Void
    {
        if (print)
        {
            clearOutput();
    ...
    private function mindbreakerEL(print : Bool = true) : Void
    {
        if (print)
        {
            clearOutput();
