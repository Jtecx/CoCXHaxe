...
class Zetaz extends Monster
{
    ...
    //Chucks faux-heat draft ala goblins. -
    {
        ...
        else
        {
            ...
            {
                player.createStatusEffect(StatusEffects.TemporaryHeat, 0, 10, 0, 0);
            }
        }
    }
    ...
    public function gigaArouse() : Void
    {
        ...
        var lustDmg : Float = inteligencescalingbonus() / 3;
        ...
        if (player.cocks.length > 0)
        {
            ...
            {
                outputText("You feel [eachcock] dribble pre-cum, bouncing with each beat of your heart and aching to be touched.  ");
            }
        ...
        {
            if (player.lust100 >= 60 && player.vaginas[0].vaginalWetness == VaginaClass.WETNESS_NORMAL && player.vaginas.length == 1)
            {
                outputText("Your [vagina] dampens perceptibly, feeling very empty.  ");
            }
            if (player.lust100 >= 60 && player.vaginas[0].vaginalWetness == VaginaClass.WETNESS_WET && player.vaginas.length > 0)
            {
            ...
            if (player.lust100 >= 60 && player.vaginas[0].vaginalWetness == VaginaClass.WETNESS_SLICK && player.vaginas.length == 1)
            {
                outputText("Your [vagina] becomes sloppy and wet, dribbling with desire to be mounted and fucked.  ");
            }
            if (player.lust100 >= 60 && player.vaginas[0].vaginalWetness == VaginaClass.WETNESS_DROOLING && player.vaginas.length > 0)
            {
            ...
            if (player.lust100 >= 60 && player.vaginas[0].vaginalWetness == VaginaClass.WETNESS_SLAVERING && player.vaginas.length == 1)
            {
    ...
    public function gigaWhitefire() : Void
    {
        ...
        var damage : Float = eBaseIntelligenceDamage() * 5;
    ...
    public function zetazTaunt() : Void
    {
        ...
        else
        {
            ...
            {
                outputText("\"<i>I lost my post!  And when you screwed up the factory?  I barely escaped with my life!  You ruined EVERYTHING!</i>\" screams Zetaz.");
            }
            else if (statusEffectv1(StatusEffects.round) == 3)
            {
                outputText("Zetaz snarls, \"<i>Do you know how hard it is to hide from Lethice?  DO YOU HAVE ANY IDEA!?  I've had to live in this fetid excuse for a jungle, and just when I found some friends and made it livable, you show up and DESTROY EVERYTHING!</i>\"");
            }
            else if (statusEffectv1(StatusEffects.round) == 4)
            {
                outputText("Zetaz explains, \"<i>I won't let you go.  I'm going to break you.</i>\"");
            }
            else if (statusEffectv1(StatusEffects.round) == 5)
            {
                outputText("\"<i>Would it have been that bad to go along with me?  You've seen the factory.  We would've kept you fed, warm, and provided you with limitless pleasure.  You would've tasted heaven and served a greater purpose.  It's not too late.  If you come willingly I can make sure they find a good machine to milk you with,</i>\" offers the imp lord.");
            }
            else if (statusEffectv1(StatusEffects.round) == 6)
            {
                outputText("\"<i>Why won't you fall?</i>\" questions Zetaz incredulously.");
            }
            else if (statusEffectv1(StatusEffects.round) == 7)
            {
                outputText("The imp lord suggests, \"<i>If you give up and let me fuck your ass maybe I'll let you go.</i>\"");
            }
            else if (statusEffectv1(StatusEffects.round) == 8)
            {
                outputText("Zetaz pants, \"<i>Just give up!  I'm nothing like the weakling you met so long ago!  I've been through hell to get here and I'm not giving it up just because you've shown up to destroy my hard work!</i>\"");
            }
            else
            {
    ...
    override public function displaySpecialStatuses() : Array<Dynamic>
    {
        var statusArray : Array<Dynamic> = super.displaySpecialStatuses();
        if (statStore.hasBuff("ImpMight"))
        {
            statusArray.push("Imp Might");
        }
        return statusArray;
    }
    ...
    private function buildAbilities() : Array<Dynamic>
    {
    ...
    private var mightCasted : Bool = false;
}
