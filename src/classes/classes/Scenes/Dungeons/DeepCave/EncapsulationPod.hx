...
class EncapsulationPod extends Monster
{
    ...
    //[Round 1 Action]
    {
        ...
        else if (statusEffectv1(StatusEffects.Round) == 2)
        {
            ...
            if (player.cockTotal() > 1)
            {
                ...
                {
                    outputText(player.multiCockDescriptLight() + " get the same treatment and soon feel quite happy to be here.  ");
                }
            }
            if (player.hasVagina())
            {
            ...
            {
                outputText("the underside of your [allbreasts]");
            }
        ...
        else
        {
            ...
            {
                outputText("Your body twitches weakly, too relaxed to move while it gets off from being double-penetrated.");
            }
            if (player.hasFuckableNipples())
            {
            ...
            if (player.cockTotal() > 0)
            {
                ...
                if (player.cumQ() > 800)
                {
                    outputText("  Of course with all the semen you produce, the flesh-pod's ooze clouds over quite quickly, blocking your vision with a purple-white haze.");
                }
            }
            if (player.biggestLactation() > 1)
            {
                ...
                {
                    outputText("though the slight dribble is barely noticeable to you.");
                }
                else if (player.biggestLactation() < 3)
                {
                    outputText("coloring things a little more white.");
                }
                else
                {
                    outputText("thickening your fluid-filled prison with nutrients.");
                }
            }
    ...
    //[Round 1 Description]
    {
        ...
        else if (statusEffectv1(StatusEffects.Round) == 2)
        {
            ...
            if (player.hasFullCoatOfType(Skin.SCALES))
            {
                _long += "scaley ";
            }
        ...
        else
        {
            ...
            if (player.biggestTitSize() >= 1)
            {
                _long += "the underside of your [allbreasts]";
            }
            else
            {
                _long += "your chest";
