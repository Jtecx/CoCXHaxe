...
class BeeHive extends DungeonAbstractContent
{
    public function new()
    {
        super();
    ...
    {
        inDungeon = true;
        if (flags[kFLAGS.DISCOVERED_BEE_HIVE_DUNGEON] < 2.75 && flags[kFLAGS.TIFA_FOLLOWER] <= 5)
        {
            room1Pass();
        }
        else
    ...
    private function room3() : Void
    {
        ...
        {
            if (flags[kFLAGS.TIFA_FOLLOWER] == 3)
            {
                outputText("You go back to the blocked path trying to find a way to open it.");
            }
            else
            {
    ...
    private function room4() : Void
    {
        ...
        if (flags[kFLAGS.TIFA_FOLLOWER] == 1)
        {
            flags[kFLAGS.TIFA_FOLLOWER] = 3;
        }
        else
        {
            flags[kFLAGS.TIFA_FOLLOWER] = 2;
        }
    ...
    public function room1South() : Void
    {
        ...
        else if (flags[kFLAGS.DISCOVERED_BEE_HIVE_DUNGEON] > 2.5)
        {
            outputText("No one's there to block your path to the Hive, it seems you can proceed.");
        }
        else
        {
            outputText("The two guards fall to the ground, " + ((flags[kFLAGS.DISCOVERED_BEE_HIVE_DUNGEON] == 2.5) ? "too busy masturbating" : "their wounds too great") + " to keep up on fighting. You get past them and enter the Hive.");
        }
    ...
    public function room3West() : Void
    {
        ...
        if (flags[kFLAGS.TIFA_FOLLOWER] > 3)
        {
            outputText("The library is empty right now.\n");
        }
    ...
    public function room5North(first : Bool = true) : Void
    {
        ...
        if (flags[kFLAGS.TIFA_FOLLOWER] > 4)
        {
            outputText("The way to the throne room is open.");
        }
        ...
        {
            outputText("You walk into " + ((first) ? "a" : "the") + " wall of caramel colored substance. Though you can see through it like glass, you have no way to get through it for now.");
        }
        dungeons.setDungeonButtons(null, room2Center, null, null);
        if (flags[kFLAGS.TIFA_FOLLOWER] == 5)
        {
            addButton(6, "Get through", room6);
        }
    }
}
