...
class AnzuScene extends DungeonAbstractContent
{
    public function new()
    {
        super();
    ...
    public function anzuAffection(amount : Int = 0) : Int
    {
        ...
        if (flags[kFLAGS.ANZU_AFFECTION] < 0)
        {
            flags[kFLAGS.ANZU_AFFECTION] = 0;
        }
        if (flags[kFLAGS.ANZU_AFFECTION] > 100)
        {
            flags[kFLAGS.ANZU_AFFECTION] = 100;
        }
        return flags[kFLAGS.ANZU_AFFECTION];
    }
    public function anzuRelationshipLevel() : Int
    {
        if (flags[kFLAGS.ANZU_RELATIONSHIP_LEVEL] < 1 && flags[kFLAGS.ANZU_PALACE_UNLOCKED] > 0)
        {
            flags[kFLAGS.ANZU_RELATIONSHIP_LEVEL] = 1;
        }
        return flags[kFLAGS.ANZU_RELATIONSHIP_LEVEL];
    }
    public function anzuSexCounter() : Int
    {
        return as3hx.Compat.parseInt(flags[kFLAGS.ANZU_ANAL_CATCH_COUNTER] + flags[kFLAGS.ANZU_VAGINAL_CATCH_COUNTER] + flags[kFLAGS.ANZU_BLOWN_YOU_COUNTER] + flags[kFLAGS.ANZU_SUCKED_OFF_COUNTER] + flags[kFLAGS.ANZU_ANAL_PITCH_COUNTER] + flags[kFLAGS.ANZU_FED_COUNTER]);
    }
    ...
    private function apporachInitialPalace() : Void
    {
        ...
        if (flags[kFLAGS.LETHICE_DEFEATED] > 0 || flags[kFLAGS.CORRUPTED_MARAE_KILLED] > 0 || flags[kFLAGS.PURE_MARAE_ENDGAME] > 0)
        {
            outputText("Even the so-called gods and lords of this land couldn’t stand against your might, so whoever, or whatever who resides there surely won’t be a threat.");
        }
        else
        {
            outputText("Besides, after the snowstorm, you’ve been frozen to the bones. Hopefully inside will be warmer.");
        }
        ...
        if (player.inte < 60)
        {
            ...
            else
            {
                ...
                {
                    outputText("nearly twice");
                }
                else if (player.tallness >= 96 && player.tallness < 120)
                {
                    outputText("twice");
                }
                else if (player.tallness >= 60 && player.tallness < 96)
                {
                    outputText("thrice");
                }
                else if (player.tallness >= 48 && player.tallness < 60)
                {
                    outputText("four times");
                }
                else if (player.tallness >= 40 && player.tallness < 48)
                {
                    outputText("five times");
                }
                else if (player.tallness < 40)
                {
                    outputText("six times");
                }
            ...
            {
                outputText("another god besides Marae ");
            }
            if (flags[kFLAGS.URTA_QUEST_STATUS] >= 1)
            {
                outputText("and Taoth surprises you");
            }
            else
            {
                outputText("a god living in this place on Mareth surprises you. Intrigued you ask him more and he explains how, before arriving in Mareth, he was the god of fire and storms in another realm. Anzu talks with you for a while, and before you can realize, its almost night.");
            }
    ...
    public function anzuTransition() : Void
    {
        if (anzuRelationshipLevel() == 1)
        {
            ...
            {
                outputText(("You tell him about the friends that you’ve made in Tel’Adre." + camp.hasCompanions()) ? "You think about all the people you've met and the thought of the friends you've made in your camp come to mind as you talk about them." : "");
            }
            else if (camp.hasCompanions())
            {
                outputText("You tell him about the companions in your camp.");
            }
            ...
            if (!player.hasStatusEffect(StatusEffects.TelAdre) && !camp.hasCompanions())
            {
                outputText("You tell him about the friends that you had back on Ingnam, and  how you used to spend the afternoons playing and having fun together.");
            }
    ...
    public function anzuMenus() : Void
    {
        ...
        if (flags[kFLAGS.ANZU_RELATIONSHIP_LEVEL] >= 3)
        {
            addButton(2, "Sex", anzuSexMenu).hint("Have some sexy times with the sexy avian deity.");
        }
        else
        {
            addButtonDisabled(2, "Sex", "Maybe if Anzu likes you better, he will let you do that.");
        }
        if (dungeonLoc == DUNGEON_ANZU_ROOF)
        {
            if (player.canFly())
            {
                ...
                {
                    addButton(3, "Race Him", null).hint("Challenge Anzu to a race and have some fun!");
                }
                else
                {
                    addButtonDisabled(3, "Race Him", "Maybe if Anzu likes you better, he'll consider letting you race against him.");
                }
            }
            else
            {
                addButtonDisabled(3, "Race Him", "You'll need a pair of flight-enabling wings to do that.");
            }
        }
    ...
    //Sex
    private function anzuSexMenu(showText : Bool = true) : Void
    {
        if (showText)
        {
            clearOutput();
        ...
        if (player.hasVagina())
        {
            addButton(1, "Vaginal Catch", catchVaginal).hint("Have Anzu fill you vaginally to the brim.");
        }
        else
        {
            addButtonDisabled(1, "Vaginal Catch", "You need a cooter for that.");
        }
        if (player.hasCock())
        {
            addButton(2, "Get Sucked", getBlown).hint("Have Anzu suck you off? Though his beak can prove to be a challenge.");
        }
        else
        {
            addButtonDisabled(2, "Get Sucked", "You need to have a cock for that.");
        }
        ...
        if (player.hasCock())
        {
            addButton(4, "Fuck Him", fuckGodlyBirdButt).hint("Put that cock of yours to a good use and fuck that divine ass!");
        }
        else
        {
            addButtonDisabled(4, "Fuck Him", "You need to have a cock to fuck that divine butt.");
        }
        if (player.lactationQ() >= 100 || player.hasPerk(PerkLib.Feeder))
        {
            addButton(5, "Feed Him", feedAnzu).hint("Anzu is certainly thirsty. Put those breasts of yours to a good use.");
        }
        else
        {
            addButtonDisabled(5, "Feed Him", "This option is only available if you have lactating breasts.");
        }
        addButton(14, "Back", anzuMenus);
    }
    ...
    private function catchAnalPart1() : Void
    {
        ...
        if (flags[kFLAGS.ANZU_ANAL_CATCH_COUNTER] == 0)
        {
            ...
            {
                outputText("That certainly could be a difficult task, but");
            }
            else if (player.analCapacity() < 150)
            {
                outputText("A big one, certainly, but, since you already had some experience with those things, taking it shouldn’t be a problem, and");
            }
            else
            {
                outputText("Really? Well, you had plenty of experience on Mareth, and despite how big he can be, you could easily take it on a single thrust, besides, you can’t see how his avian manhood is left unattended day after day. Even with the occasional release");
            }
            if (flags[kFLAGS.ANZU_SUCKED_OFF_COUNTER] > 0)
            {
                outputText(", and blowjob");
            }
    ...
    {
        clearOutput();
        var wasVirgin : Bool = player.ass.analLooseness < 1;
    ...
    private function feedAnzu() : Void
    {
        ...
        if (flags[kFLAGS.ANZU_FED_COUNTER] == 0)
        {
            ...
            {
                outputText("His eyes dart to your crotch, focusing on your [cock] but, you dispel his misguided idea, jiggling your tits again to entice him. ");
            }
            else
            {
        ...
        else
        {
            ...
            if (player.cockTotal() > 0)
            {
                outputText("Darting his sight to your crotch, he tries to slip a hand inside the lower part of your [armor]. Taking his misguided hand and putting in on your breasts, you make clear what you want to do.");
            }
    ...
    private function eatFoodWithAnzu() : Void
    {
        ...
        if (hungerRefillAmount < 20)
        {
            hungerRefillAmount = 20;
        }
        if (model.time.hours == 8)
        {
    ...
    private function acceptSleepingWithAnzu() : Void
    {
        ...
        //Not wearing clothes/armour?{
            
            if (!player.lowerGarment.isNothing || !player.upperGarment.isNothing)
    ...
    private function wakeUpWithAnzuGoodMorningBirdie() : Void
    {
        ...
        {
            player.refillHunger(player.maxHunger() + 10 - player.hunger);
        }
        inDungeon = false;
        doNext(playerMenu);
    }
