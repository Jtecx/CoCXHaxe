...
class SandWitchMob extends Monster
{
    public function sandWitchMobAI() : Void
    {
        ...
        else if (hasStatusEffect(StatusEffects.Sandstorm) && rand(2) == 0 && !player.hasStatusEffect(StatusEffects.LustStones))
        {
            sandstonesAreCool();
        }
        else if (rand(3) == 0)
        ...
        //[IF PC has solid body]{
            ...
            if (!player.isGoo())
            {
                ...
                if (player.hasVagina())
                {
                    ...
                    {
                        outputText(", robbing you of your virginity as a trickle of blood runs down your [leg].");
                    }
                    bonus++;
                }
                //[If PC has balls:
