 ...
 */
class LabGuard extends Monster
{
    public function new()
    {
        super();
    ...
    public var shieldWall : Bool = false;
    ...
    private function CorVolley() : Void
    {
        ...
        else
        {
            ...
            if (player.shield.isNothing)
            {
                ...
                player.takePhysDamage(as3hx.Compat.parseInt(spe * 25) - rand(player.touStat.core.value) - player.armorDef);
            }
            else
            {
                outputText("You cover yourself with [shield], but some of the bullets find their mark.");
                player.takePhysDamage(as3hx.Compat.parseInt(spe * 20) - rand(player.touStat.core.value) - player.armorDef);
            }
            outputText("\n\n");
        }
    }
    ...
    private function SeductionAttack() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(" You can all but feel your [cock] jamming into the tight passage of a succubus, her walls pressing around you, milking your rod for all the slut’s worth. You can feel your hands around her neck, forcing the bitch to wail as you ravage her cunt, forcing her lips wide as you spurt your load inside…");
        }
        if (player.hasVagina())
        {
            outputText(" Your labia quivers at the sight, your knees shake as you can all but feel the throbbing rods inside you, veins pulsing. Warmth spreads to your womb as you can feel the twitch, as the demon’s sperm erupts into…");
        }
    ...
    private function ThrownWeapons() : Void
    {
        ...
        for (i in 0...10)
        {
            ...
            {
                player.takeLustDamage(10 + (player.lib / 20) + (player.cor / 20) + rand(10), true);
            }
        }
    }
