 ...
 */
class ProjectNightwalker extends Monster
{
    private var _biteCounter : Int = 0;
    private var _inDungeon : Bool;
    private var plantBite : Bool;
    private var slimeBite : Bool;
    private var stoneBite : Bool;
    ...
    private function courtesansCall() : Void
    {
        ...
        {
            lustMult += player.statusEffectv1(StatusEffects.IzmaBleed);
        }
    ...
    private function neckbite() : Void
    {
        ...
        {
            addHP(maxHP() * .1);
            var dam : Int = as3hx.Compat.parseInt(this.str * 3);
    ...
    public function struggleFaceSitting(wait : Bool = false) : Void
    {
        if (wait)
        {
    ...
    private function randomiseAction() : Void
    {
        ...
        {
            case 0, 1, 2:
                if (plantBite || stoneBite || slimeBite)
                {
                    randomiseAction();
                }
                else
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        ...
        else
        {
            if (plantBite || stoneBite || slimeBite)
            {
                outputText("The vampire strolls up to you, giving you one last harsh kick in the face as you black out.\n");
            }
            else
            {
                outputText("The Vampire strolls up to you, grabbing you by your shoulders and sinking her fangs into you, draining you until you pass out.\n");
            }
            cleanupAfterCombat();
        }
    }
