 ...
 */
class DemonDragonGroup extends Monster
{
    ...
    // when you use melee, chance is reduced to 50% if you’re flying
    {
        ...
        if (!player.isFlying() || rand(2) == 0)
        {
            ...
            {
                outputText("You laugh, the dragon’s breath feeling like a warm breeze. You barrel through the flame.");
            }
            else
            {
                player.takeFireDamage(((inte + wis) * 1.2) + rand(80), true);
            }
            outputText("\n\n");
        }
    }
    ...
    {
        var damage : Float = (inte + wis) * 3 * player.newGamePlusMod() + 900 + rand(100);
