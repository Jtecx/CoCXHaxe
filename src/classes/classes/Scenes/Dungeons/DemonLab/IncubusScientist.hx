...
class IncubusScientist extends Monster
{
    ...
    //Miss:
    {
        outputText("The demon scientist lines you up in his sights, pulling the trigger several times.")  //Determine if evaded  ;
    ...
    private var ShieldCooldown : Float;
    ...
    {
        if (DungeonAbstractContent.dungeonLoc == DungeonAbstractContent.DUNGEON_LAB_ENTRANCE)
        {
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (ShieldCooldown == 0 && ShieldHits == 0)
        {
            ShieldsUp();
        }
        if (HP < maxHP() * 0.6)
