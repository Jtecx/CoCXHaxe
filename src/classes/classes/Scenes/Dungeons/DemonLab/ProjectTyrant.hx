 ...
 */
class ProjectTyrant extends Monster
{
    ...
    private function TackleGrapple() : Void
    {
        ...
        var damage : Float = ((str + tou) * 1.2) + rand(50);
    ...
    {
        if (rand(3) == 0 || (rand(player.str) > this.str / 2) || player.hasPerk(PerkLib.FluidBody))
        {
            TackleGrappleSuccess();
        }
        else
        {
            TackleGrapple();
    ...
    public function BlackMortar() : Void
    {
        ...
        else
        {
            ...
            if (!player.immuneToAcid())
            {
                ...
                {
                    player.createStatusEffect(StatusEffects.AcidDoT, 3, 10, 0, 0);
                }
            }
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (choice0 == 1 && player.hasStatusEffect(StatusEffects.Pounced))
        {
            SlamTrample();
        }
        switch (choice0)
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        ...
        else
        {
            ...
            if (player.cor <= 10)
            {
                outputText("You reluctantly step in, bringing your [weapon] down into the creature’s bare neck. As it dies, you can feel your heart sink. It’s...No, <i>they’re</i> dead. They died a free, sentient creature.[pg]");
            }
            else if (player.cor <= 40)
            {
                outputText("You hesitate only slightly before sheathing your [weapon]. You pull a dagger from your pack, and you slit the Drider creature’s throat.[pg]");
            }
            else
            {
                outputText("Without hesitation, you take one of the creature’s fallen weapons, and bring it down onto the creature’s mangled head.[pg]");
            }
            cleanupAfterCombat();
        }
    }
