 ...
 */
class UltimisFlamespreader extends Monster
{
    private var gunner1Alive : Bool;
    private var gunner2Alive : Bool;
    ...
    public function struggleNom(wait : Bool) : Void
    {
        ...
        else
        {
            if (!wait)
            {
                outputText("You struggle, slamming your [weapon] into the creature’s snout, but it doesn’t let go. ");
            }
    ...
    private function gunnersAttack() : Void
    {
        ...
        while (attacks-- > 0)
        {
            if (rand(5) == 0)
            {
                player.takePhysDamage(str / 4 + rand(50), true);
            }
        }
    }
    ...
    private function randomiseAction() : Void
    {
        ...
        switch (choice)
        {
            case 0, 1, 2:flameAttack();
            case 3:swipe();
            case 4:if (!player.hasStatusEffect(StatusEffects.OnFire))
                {
                    randomiseAction();
                }
                nom();
    ...
    public function desc() : String
    {
        ...
        if (HP > maxHP() * .5)
        {
            desc += "Two demon soldiers sit on the massive Dragon’s shoulders, each with a large gun of some sort, and a LOT of bullets.";
        }
        else if (HP > maxHP() * .4)
        {
            desc += "One demon soldier sits on one of the massive Dragon's shoulder with a large gun of some sort, the other gun having exploded.";
        }
        else
        {
            desc += "Both large guns on the massive Dragon's shoulders have exploded, sending the demons who manned them flying to their doom. ";
        }
        return desc;
    }
