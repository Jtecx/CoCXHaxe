...
class Incels extends Monster
{
    ...
    private var damageMultBase : Float = 1;
    private var damageMult : Float = 1;
    
    override public function eBaseDamage() : Float
    {
    ...
    private var grappleCooldown : Float;
    ...
    public function DraftSupportCheck() : Void
    {
        ...
        else if (dsTurnCounter > 4)
        {
            DraftSupportContinue();
        }
    }
    ...
    {
        if (rand(player.str) > (this.str / 10) * (1 + player.getStatusValue(StatusEffects.Pounced, 1)) || (player.hasStatusEffect(StatusEffects.Pounced) && player.statusEffectv1(StatusEffects.Pounced) < 1) || player.hasPerk(PerkLib.FluidBody))
        {
            RipBreakOut();
        }
        else
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (grappleCooldown == 0 && !player.hasStatusEffect(StatusEffects.Pounced) && rand(2) == 0)
        {
