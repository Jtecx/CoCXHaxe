...
class HiddenCave extends DungeonAbstractContent
{
    public function new()
    {
        super();
    ...
    public function meetLoliBatGolem() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.LoliBatGolems, 1, 10);
        }
    }
    ...
    {
        var beatmore : Int = as3hx.Compat.parseInt(8 - flags[kFLAGS.HIDDEN_CAVE_LOLI_BAT_GOLEMS] - flags[kFLAGS.HIDDEN_CAVE_GOLEM_GROUPS]);
        ...
        if (flags[kFLAGS.HIDDEN_CAVE_LOLI_BAT_GOLEMS] > 4 && flags[kFLAGS.HIDDEN_CAVE_GOLEM_GROUPS] > 2)
        {
            addButton(10, "West", roomSStaircase);
        }
        else
        {
            addButtonDisabled(10, "West", "You still need to beat " + beatmore + " enemies to deactivate restriction formation from this hidden passage.");
        }
    ...
    {
        var beatmore : Int = as3hx.Compat.parseInt(5 - flags[kFLAGS.HIDDEN_CAVE_LOLI_BAT_GOLEMS] - flags[kFLAGS.HIDDEN_CAVE_GOLEM_GROUPS]);
        ...
        if (flags[kFLAGS.HIDDEN_CAVE_LOLI_BAT_GOLEMS] > 3 && flags[kFLAGS.HIDDEN_CAVE_GOLEM_GROUPS] > 0)
        {
            addButton(10, "West", roomNTE);
        }
        else
        {
            addButtonDisabled(10, "West", "You still need to beat " + beatmore + " enemies to deactivate restriction formation from this passage.");
        }
    }
    public function roomSWUP() : Void
    {
        var beatmore : Int = as3hx.Compat.parseInt(4 - flags[kFLAGS.HIDDEN_CAVE_LOLI_BAT_GOLEMS]);
        ...
        {
            addButton(11, "South", roomSStorage);
        }
        else
        {
            addButtonDisabled(11, "South", "You still need to beat " + beatmore + " enemies to deactivate restriction formation from this passage.");
        }
    }
    ...
    public function roomTunnel01() : Void
    {
        ...
        if (flags[kFLAGS.HIDDEN_CAVE_2ND_FLOOR] > 0)
        {
            addButton(6, "North", roomMediumCave);
        }
    }
    ...
    public function roomTedsLair() : Void
    {
        ...
        if (player.hasPerk(PerkLib.SoulSense))
        {
            ...
            if (flags[kFLAGS.HIDDEN_CAVE_2ND_FLOOR] > 0)
            {
                outputText("And you already pulled it. ");
            }
        }
    ...
    {
        clearOutput();
        if (flags[kFLAGS.AURORA_LVL] == 0.2)
        {
            outputText("When you return to the chamber again, it lights up like last time. And the chained golem in the middle of it just looks at you for a long moment before asking. \"<i>Master?</i>\"\n\n");
        }
        else
        {
            ...
            {
                outputText("towering above you");
            }
            else if (player.tallness >= 72 && player.tallness < 85)
            {
                outputText("closely to your own height");
            }
            else
            {
                outputText("shorter than you");
            }
        ...
        if (silly())
        {
            addButton(1, "FTSIO", putInTheJadeTalisman1).hint("Fuck This Shit I'm out");
        }
        else
        {
            addButton(1, "No", putInTheJadeTalisman1);
        }
        if (player.hasKeyItem("Adventurer Guild: Copper plate") >= 0 || player.hasKeyItem("Adventurer Guild: Iron plate") >= 0)
        {
            addButton(3, "Yes", putInTheJadeTalisman2);
        }
        else
        {
            addButtonDisabled(3, "Yes", "Req. to have Adventurer Guild plate.");
        }
    }
