...
class Lethice extends Monster
{
    public var fightPhase(get, never) : Int;
    ...
    override private function get_long() : String
    {
        ...
        if (player.hasStatusEffect(StatusEffects.LethicesRapeTentacles))
        {
            ...
            {
                outputText("<b> Unfortunately, they’ve grabbed you. You need to break free to do anything! </b>");
            }
    ...
    private var dictOrder : Array<Dynamic> = [];
    private var dictValue : Array<Dynamic> = [];
    private var dictHistory : Array<Dynamic> = [];
    private var dictAttacked : Array<Dynamic> = [];
    private var roundCheck : Int = 0;
    private var decayCheck : Bool = false;
    private var companionCheck : Bool = false;
    private var hpLimit : Float = 0;
    private var lustLimit : Float = 0;
    
    private function furubeYuraYuraYatsukaNoTsurugiIkaishinshoMakora() : Void
    {
        var _statusEffects : Array<Dynamic> = statusEffects;
        ...
        for (__i in 0...dictAttacked.length)
        {
            var ___index : Int = Lambda.indexOf(dictOrder, dictAttacked[__i]);
            if (___index > -1)
            {
                dictOrder.splice(___index, 1)[0];
                dictValue.splice(___index, 1)[0];
            }
        ...
        // outputText( statusEffects[i].stype.id + " "+ statusEffects[i].value1 + " \n");{
            ...
            {
                var _index : Int = Lambda.indexOf(dictOrder, _statusEffects[i].stype.id);
                if (_index > -1)
                {
                    _statusEffects[i].value1 = 1;
                    dictOrder.splice(_index, 1)[0];
                    dictValue.splice(_index, 1)[0];
                    
                    // Re add Immunity if you trigger deflect lul
                    dictOrder.push(_statusEffects[i].stype.id);
                    dictValue.push(115);
                }
                else if (_statusEffects[i].value1 > 2)
        ...
        // To prevent modifying looping array{
            ...
            var tmpOrder : Array<Dynamic> = [];
            var tmpValue : Array<Dynamic> = [];
            
            roundCheck++;
            
            for (_i in 0...dictOrder.length)
    ...
    // Limit immunity down to 9, remove the first added immunity
    {
        ...
        {
            dictOrder.splice(0, 1)[0];
            dictValue.splice(0, 1)[0];
        }
    }
    private function wheelHandleDesc() : String
    {
        ...
        var _sw1_ = (3 + flags[kFLAGS.PRIMARY_DIFFICULTY]);        
    ...
    private function wheelHandleString() : String
    {
        ...
        var wheelTurned : Float = 0;
        var tmp : Array<Dynamic> = [];
        
        for (___i in 0...dictOrder.length)
        {
            if (Lambda.indexOf(dictHistory, dictOrder[___i]) < 0)
            {
                wheelTurned += 1;
            }
    ...
    private function statusHandler(string : String = "") : Bool
    // Append status effect immmunity and make Lethice immune to adapted debuff
    {
        
        if (Lambda.indexOf(dictOrder, string) < 0)
    ...
    override private function handleConstricted() : Bool
    {
        return statusHandler("constrict");
    }
    override private function handleStun() : Bool
    {
        return statusHandler("stun");
    }
    override private function handleFear() : Bool
    {
        return statusHandler("fear");
    }
    override private function handleConfusion() : Bool
    {
        return statusHandler("confuse");
    }
    override public function handleStatusEffects(statusEffect : StatusEffectType) : Bool
    {
        var string : String = "";
    ...
    private function adaptionDeflect(damage : Float, font : String, dict : String = "physical", damageType : Float = 0) : Float
    {
        if (_fightPhase != 2)
        {
            if (companionCheck)
            ...
            var index : Int = Lambda.indexOf(dictOrder, dict);
            if (index > -1)
            {
                var numberformat : NumberFormatter = new NumberFormatter();
                var deflectDamage : Float = Math.round(damage * (dictValue[index] / 100));
    ...
    override public function doDamageBefore(damage : Float) : Float
    {
        return adaptionDeflect(damage, "damage");
    }
    override public function doMagicDamageBefore(damage : Float) : Float
    {
        return adaptionDeflect(damage, "damage", "magic", 4);
    }
    override public function doFireDamageBefore(damage : Float) : Float
    {
        return adaptionDeflect(damage, "fire", "fire", 5);
    }
    override public function doIceDamageBefore(damage : Float) : Float
    {
        return adaptionDeflect(damage, "cold", "cold", 6);
    }
    
    override public function doLightningDamageBefore(damage : Float) : Float
    {
        return adaptionDeflect(damage, "lightning", "lightning", 7);
    }
    
    override public function doDarknessDamageBefore(damage : Float) : Float
    {
        return adaptionDeflect(damage, "dark", "dark", 8);
    }
    
    override public function doPoisonDamageBefore(damage : Float) : Float
    {
        return adaptionDeflect(damage, "poison", "poison", 9);
    }
    
    override public function doWindDamageBefore(damage : Float) : Float
    {
        return adaptionDeflect(damage, "wind", "wind", 10);
    }
    
    override public function doWaterDamageBefore(damage : Float) : Float
    {
        return adaptionDeflect(damage, "water", "water", 11);
    }
    
    override public function doAcidDamageBefore(damage : Float) : Float
    {
        return adaptionDeflect(damage, "acid", "acid", 13);
    }
    
    override public function doEarthDamageBefore(damage : Float) : Float
    {
        return adaptionDeflect(damage, "earth", "earth", 12);
    }
    
    override public function doTrueDamageBefore(damage : Float) : Float
    {
        return adaptionDeflect(damage, "damage", "true");
    }
    
    override private function applyTease(lustDelta : Float, display : Bool = true, aura : Bool = false) : Void
    {
    ...
    override public function preCompanionSeal(companionName : String) : Bool
    {
        if (_fightPhase != 2)
        {
            if (Lambda.indexOf(dictOrder, "companion") > -1)
            {
    ...
    override public function postPlayerBusyBtnSpecial(btnSpecial1 : CoCButton, btnSpecial2 : CoCButton) : Void
    {
        if (player.hasStatusEffect(StatusEffects.LethicesRapeTentacles))
        {
            if (player.lust < SceneLib.combat.magic.getWhiteMagicLustCap() && player.hasStatusEffect(StatusEffects.KnowsWhitefire) && ((!player.hasPerk(PerkLib.BloodMage) && player.mana >= 30) || (player.hasStatusEffect(StatusEffects.BloodMage) && ((player.HP + 30) > (player.minHP() + 30)))))
            {
    ...
    {
        if (fightPhase == 3 && Std.is(ability, AbstractSpell) && ability.hasTag(CombatAbility.TAG_DAMAGING))
        {
            if (display)
            {
    ...
    private function get_fightPhase() : Int
    {
        return _fightPhase;
    }
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (_fightPhase != 2)
        {
            ...
            {
                if (Lambda.indexOf(dictOrder, "hplossimmune") > -1)
                {
            ...
            {
                if (Lambda.indexOf(dictOrder, "lustlossimmune") > -1)
                {
    ...
    {
        var atks : Array<Dynamic> = [demonicArouse, demonfire];
        if (_roundCount % 10 == 3)
        {
    ...
    private function demonicArouse() : Void
    {
        ...
        else if (player.lust <= (player.lust100 * 0.9))
        {
        ...
        else if (player.lust <= player.lust100)
        {
    ...
    public function dispellRapetacles() : Void
    {
        ...
        if (game.player.hasStatusEffect(StatusEffects.BloodMage))
        {
            game.player.HP -= 30;
        }
        else
    ...
    //I can’t believe it’s not whitefire. Cannot be avoided/blocked. Medium damage.
    {
        ...
        var damage : Float = (this.inte + 100 + rand(25)) * 10;
        if (player.hasPerk(PerkLib.FromTheFrozenWaste) || player.hasPerk(PerkLib.ColdAffinity))
        {
            damage *= 3;
        }
    ...
    private function phase1Ends(hpVictory : Bool) : Void
    {
        clearOutput();
        if (hpVictory)
        {
    ...
    {
        var atks : Array<Dynamic> = [demonLustMagic, dirtyDancing, hornyPoke, crushingBodies];
    ...
    private function dirtyDancing() : Void
    {
        ...
        {
            if (player.lust <= (player.lust100 * 0.33))
            {
            ...
            var l : Float = player.lib + player.cor + 100;
            if (player.hasStatusEffect(StatusEffects.MinotaurKingsTouch))
            {
                l *= 1.25;
            }
    ...
    private function hornyPoke() : Void
    {
        ...
        {
            var damage : Float = 7000 - rand(player.touStat.core.value);
    ...
    private function phase2Ends(hpVictory : Bool) : Void
    {
        clearOutput();
        if (hpVictory)
        {
        ...
        {
            EngineCore.addButton(0, "DemonFuck", p2DemonFuck, hpVictory);
        }
        if (player.hasStatusEffect(StatusEffects.KnowsHeal))
        {
            EngineCore.addButton(1, "Heal", p2Heal);
        }
        EngineCore.addButton(2, "Next", p2Next);
    }
    
    private function p2DemonFuck(hpVictory : Bool) : Void
    {
        clearOutput();
        ...
        if (player.hasCock())
        {
            EngineCore.addButton(1, "FuckDemon", fuckDemon);
        }
    ...
    private function p2Next() : Void
    {
        ...
        if (dictOrder.length > 0)
        {
            if (roundCheck == 1)
            {
                ...
                if (Lambda.indexOf(dictOrder, "physical") > -1)
                {
                    str += "Quickly rushing towards her in hope to finish the fight, your suspicion became clear as you closing in, reveals a large <b>" + wheelHandleDesc() + "</b>-handled wheel as it turned briefly, a loud clank follows before it stopped and fade back into nothingness. And the next thing you knew you realised that Lethice intercept your attack, completely still as if the force you put into your strike was never there in the first place, then you felt the sensation being pushed back,";
                }
                else
    ...
    private function beginPhase3(doLethNext : Bool) : Void
    // 9999 configure phase 3
    {
        ...
        if (doLethNext)
        {
            EngineCore.addButton(0, "Next", p2Next);
        }
        else
        {
            SceneLib.combat.combatRoundOver();
    ...
    //Every turn she boosts her defense against lust or HP depending on how the PC damaged her.
    {
        ...
        {
            var atks : Array<Dynamic> = [parasiteThrowingStars, whiptrip, sonicwhip];
    ...
    private function parasiteThrowingStars() : Void
    {
        ...
        else
        {
            ...
            var damage : Float = str + weaponAttack - rand(player.touStat.core.value);
    ...
    private function sonicwhip() : Void
    {
        ...
        var damage : Float = (75 + weaponAttack + str) * 10;
        player.takeMagicDamage(damage, true);
    }
    ...
    private function whipchoke() : Void
    {
        ...
        else
        {
            ...
            var damage : Float = weaponAttack + 25 - rand(player.touStat.core.value);
            player.takePhysDamage(damage, true);
