...
/**
	 ...
	 * @author Gedan
	 */
class SuccubusGardenerScenes extends BaseContent
{
    ...
    private var GARDENER_LEFT(default, never) : Int = 1;
    private var GARDENER_FUCKED(default, never) : Int = 2;
    private var GARDENER_KILLED(default, never) : Int = 3;
    
    public function fuckUpTheGardener(hpVictory : Bool) : Void
    {
        clearOutput();
    ...
    private function tentaFail(cock : Int, createdVag : Bool = false) : Void
    {
        if (!recalling)
        {
        ...
        if (cock < 0)
        {
            outputText(" cunt-kissers");
        }
        else
        {
            outputText(" cock-suckers");
        }
        outputText(" like those are one in a million.");
        
        if (flags[kFLAGS.NUMBER_OF_TIMES_MET_SCYLLA] > 0)
        {
            outputText(" Perhaps Scylla's ruby mouth could give her a run for her money, but there's only one way to be sure.");
        }
        else if (SceneLib.telAdre.dominika.fellatrixSucked() > 0)
        {
            outputText(" Perhaps Dominika's all-consuming mouth could give her a run for her money, but there's only one way to be sure.");
        }
        ...
        {
            outputText(" the sensitive skin of your groin");
        }
        ...
        if (cock >= 0)
        {
            outputText(" blow a wad down this slut's throat");
        }
        else if (!createdVag)
        {
            outputText(" cream yourself on this slut's face");
        }
        else
        {
            outputText(" teach this bitch how to pleasure an asshole");
        }
        ...
        if (cock < 0)
        {
            outputText(" Its outline is clearly phallic, but how could such a huge thing ever fit inside you, let alone anyone?");
        }
        else
        {
            outputText(" Its tip oozes lubricants, but the gaping pussy at its tip is big enough to hold six minotaurs' monster-cocks.");
            if (player.cocks[cock].cockLength < 36)
            {
                outputText(" How could you ever hope to fill it?");
            }
            else
            {
                outputText(" How long were they preparing this for you?");
            }
        }
        if (createdVag)
        ...
        else
        {
            ...
            if (player.cocks[cock].cockLength < 12)
            {
                outputText(" The ease with which you slide inside is no surprise, given the disparity in size.");
            }
            else if (player.cocks[cock].cockLength < 30)
            {
                outputText(" The ease with which you enter is no real surprise, given that even your bloated length" + (((player.cocks.length > 1)) ? "s are" : " is") + " small by comparison.");
            }
            else if (player.cocks[cock].cockLength < 48)
            {
                outputText(" You slide in with ease, perfectly matched to the hungry fuck-tunnel in size.");
            }
            else
            {
                outputText(" Sliding in must take quite the effort. The orange-hued twat is drooling around you, slowly working down an inch at a time. It takes all of you, even if it has to stretch beyond all reason to do so.");
            }
            outputText(" It feels good, better");
            if (player.cocks[cock].cockLength < 12)
            {
                outputText(" than a giant that looks that loose should. You'd swear it was tight little twat from how firmly it's squeezing you!");
            }
            else if (player.cocks[cock].cockLength < 30)
            {
                outputText(" than you had thought at a glance. Not only is it the perfect size, but it seems to hug and touch every part of your maleness" + (((player.cocks.length > 1)) ? "es" : "") + " just right.");
            }
            else if (player.cocks[cock].cockLength >= 48)
            {
                outputText(" than you would expect given how tightly-stretched it looks. You'd think it'd be pressing down painfully, but it feels tailor made to take you instead.");
            }
    ...
    public function surrenderToTheGardener(hpVictory : Bool = false) : Void
    {
        if (player.isAlraune())
        {
    ...
    private function femGenderlessLoss(hpVictory : Bool) : Void
    {
        clearOutput();
        ...
        if (hpVictory)
        {
            outputText(", not even able to push your bruised body up onto its [feet].");
        }
        else
        {
            outputText(", too turned on to try and stop your hands from diving into your needy crotch.");
        }
        ...
        if (!hpVictory)
        {
            ...
            if (player.hasVagina())
            {
                outputText(" Your underthings were already swampy with need, but the sight of watching those eager vines squirm elicits fresh trickles of lube to course down your [legs].");
            }
            else
            {
                outputText(" You were already horny, but you find yourself wishing you had a pussy, just so you could take even more of them inside you.");
            }
        }
        else
        {
            outputText(" You cast a worried look towards the writhing vines. They seem so eager, yet they hold back for now, perhaps waiting for their mistress to finish. No... you can't go down like this!");
        }
        ...
        if (player.hasVagina())
        {
            outputText(" Against your better judgment, you're starting to get wet from the promises of her words. You know you're being given to a neverending hell of sexual stimulation, but you can't stop your body's libido from responding to the idea.");
        }
        ...
        if (player.hasBalls())
        {
            outputText(" across your [sack],");
        }
        else if (player.hasCock())
        {
            outputText(" across the underside of your [cock],");
        }
        else
        {
    ...
    private function femGenderlessLossII() : Void
    {
        ...
        if (player.isBiped())
        {
            outputText(" Your [legs] are eased open, but who are you to stop them? You're as wet as anything, and if spreading wide is the next step towards scratching your itch, so be it.");
        }
        outputText(" Your [clit] must look like a");
        if (player.clitLength > 6)
        {
            outputText(" glistening, cum-soaked shaft");
        }
        else if (player.clitLength > 3)
        {
            outputText(" tiny, cum-soaked cock");
        }
        else if (player.clitLength > 1)
        {
            outputText(" swollen, lube-lacquered gumball");
        }
        else
        {
            outputText(" glistening, pink target");
        }
        ...
        if (player.clitLength > 6)
        {
            outputText(" It stretches some around the cock-sized button");
        }
        else if (player.clitLength > 3)
        {
            outputText(" It has no trouble handling your plus-sized button");
        }
        else
        {
            outputText(" Your tiny button seems dwarfed by the tube enveloping it");
        }
        ...
        {
            outputText(" and jiggle your tits");
        }
        ...
        {
            outputText(" to splatter against your occupied womb.");
        }
        else
        {
            outputText(" to fill your womb, to make it round with life so that others can feel what you're feeling.");
        }
        
        outputText("\n\nYour girlhoney squirts, splattering over the ground.");
        if (player.wetness() < 4)
        {
            outputText(" You may not normally be capable of such soaking orgasms, but that seems of little concern to you.");
        }
    ...
    private function maleLoss(hpVictory : Bool) : Void
    {
        clearOutput();
        outputText("You drop to the ground, defeated and degraded");
        if (hpVictory)
        ...
        {
            outputText(", too turned on to try and stop your hands from rushing to your crotch. Why bother?");
        }
    ...
    private function maleLossPet() : Void
    {
        ...
        {
            outputText(" <i>\"Oh, it's quite the big bone, isn't it? You're going to have an awful hard time dragging that around on all fours, [puppy]. Why, I bet the moment you see a bitch, you're going to be humping the ground, practically immobilized by it.");
        }
        else if (player.biggestCockLength() >= 8)
        {
            ...
            if (player.cocks[player.biggestCockIndex()].cockType != CockTypesEnum.DOG)
            {
                outputText(", once you have an appropriately-shaped cock");
            }
            outputText(".");
        }
        else
        ...
        if (player.hasBalls())
        {
            outputText(", just above your [balls],");
        }
        ...
        if (!player.hasMuzzle())
        {
            outputText(", heedless of the fact that you seem to be able to reach a little farther with every one you consume");
        }
        ...
        if (!player.isTaur())
        {
            outputText(" You scrabble over, unusued to having to move in such a way,");
        }
        else
        {
            outputText(" You gallop over and drop down on your forelegs");
        }
        ...
        if (player.tailType != 0)
        {
            ...
            if (player.tailType == 13 && player.tailCount > 1)
            {
                outputText("s");
            }
        }
        ...
        {
            outputText(" your stubby, new tail");
        }
        outputText(" wagging.");
        if (player.tailType != 2)
        {
            outputText(" Never mind that it gets longer and fuzzier with every gulp or that your");
        }
        else
        {
            outputText(" Never mind that your");
        }
        ...
        if (player.hasVagina())
        {
            outputText(", lubricated with a sheen vaginal juices from your forgotten sex organ");
        }
        ...
        if (player.isTaur() || player.isDrider())
        {
            ...
            if (!player.isDrider())
            {
                outputText(" clip-clop of your hooves");
            }
            else
            {
                outputText(" sharp clacks of your spidery claws");
            }
        ...
        else
        {
            ...
            if (player.isGoo())
            {
                outputText(" Your gooey undercarriage easily divides into two hind legs, slapping wetly with regularity as you plod along.");
            }
            else if (player.isNaga())
            {
                outputText(" Your slithering tail twitches, stopping you for a second, but then it splits into two limbs, each firmer and more canine than the snake-like appendage you once had. They wobble unsteadily for a second before jumping into action.");
            }
            else
            {
                outputText(" Your [legs] shorten as you plod along, slowing you momentarily as they change. You lift your knees up so that you can stand on your [feet] once more, only they're soft, padded paws now, tipped with short nails.");
            }
    ...
    private function maleLossPetII() : Void
    {
        ...
        {
            outputText(" Was that there before?");
        }
        ...
        if (player.cumQ() >= 2500)
        {
            outputText(" A few thick ropes shoot past your head to cream the wall, eliciting a pleased giggle from your owner as she squeezes yet more from your length.");
        }
    ...
    private function maleLossFeeder() : Void
    {
        ...
        {
            outputText(" the underside of your breasts");
        }
        else
        {
            outputText(" your chin");
        }
        ...
        else
        {
            ...
            {
                outputText(" Thankfully, a few tentacles loop under the gigantic orbs to help support them.");
            }
        }
        ...
        if (player.biggestTitSize() >= 3)
        {
            outputText(" all over your body");
        }
        else
        {
            outputText(" directly onto the soil below");
        }
    ...
    private function maleLossFeederII() : Void
    {
        ...
        if (player.tailType != 0)
        {
            outputText(" past your tail to slither");
        }
        outputText(" between your cheeks.");
        if (player.ass.analLooseness == 0)
        {
            outputText(" The virginal tightness of your pucker holds the tentacle at bay at first. Still, it's no match for the combination of slippery plant fluid and constant pressure.");
        }
        else if (player.ass.analLooseness <= 3)
        {
            outputText(" The tentacle has little problem slipping through your tight backdoor, slick as it is with fluid and backed by constant pressure.");
        }
        else
        {
            outputText(" The tentacle has little difficulty squeezing into your well-loosened ass. Even without its constant, firm pressure and slippery fluid, it could enter with ease.");
        }
        ...
        if (player.hasVagina())
        {
            outputText(", and once a thick, cock-shaped plant slides inside your [vagina], you truly are");
        }
        outputText(".");
