...
class LivingStatue extends Monster
{
    ...
    //Maybe replace this with passive stun? TERRIBLE IDEA
    {
        ...
        var damage : Float = (200 * ((inte / player.inte) / 3));
    ...
    //High damage, lowish accuracy.
    {
        ...
        var damage : Float = 225 + as3hx.Compat.parseInt((str + weaponAttack) - rand(player.touStat.core.value) - player.armorDef);
    ...
    //Difficult to avoid, moderate damage.
    {
        ...
        var damage : Float = (300 + as3hx.Compat.parseInt((str + weaponAttack) - rand(player.touStat.core.value) - player.armorDef)) / (rand(2) + 1);
        //Avoid
        if (damage <= 0 || player.getEvasionRoll())
        {
