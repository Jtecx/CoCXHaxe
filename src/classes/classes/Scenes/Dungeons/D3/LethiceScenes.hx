...
class LethiceScenes extends BaseContent
{
    public static inline var GAME_END_REDEMPTION : Int = 1;
    public static inline var GAME_END_CONSORT : Int = 2;
    public static inline var GAME_END_PURGE_0_50 : Int = 3;
    public static inline var GAME_END_PURGE_50_100 : Int = 4;
    public static inline var GAME_END_PURGE_100 : Int = 5;
    public static inline var GAME_END_CONQUER_LOW : Int = 6;
    public static inline var GAME_END_CONQUER_MED : Int = 7;
    public static inline var GAME_END_CONQUER_HIGH : Int = 8;
    public static inline var GAME_END_MINDBREAKER : Int = 9;
    ...
    public function defeated(hpVictory : Bool = false) : Void
    {
        clearOutput();
        if (!recalling)
        {
        ...
        addButton(6, "Takeover", sorotityTakeover).hint("Take over her post with the followers of your mindbreaker cult.").disableIf(Mindbreaker.MindBreakerConvert < 200, 
    ...
    private function boobPlay(hpVictory : Bool) : Void
    {
        clearOutput();
        ...
        var vagAssF : Void->Void = function() : Void
        {
            ...
            if (!recalling)
            {
                player.sexReward("milk", (player.hasVagina()) ? "Vaginal" : "Anal");
            }
            theChoiceDunDunDun();
        }
    }
    
    private function redemption(hpVictory : Bool) : Void
    {
        clearOutput();
    ...
    private function joinHerII() : Void
    {
        ...
        {
            outputText(" newwgrown cock. Just another side benefit of demonhood: molding your body on the fly to suit whatever takes your fancy moment to moment.");
        }
    ...
    public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        clearOutput();
        ...
        if (monster.lust < 50 && monster.HP > (monster.maxHP() * 0.5))
        {
            outputText(" Almost.");
        }
    ...
    private function woofwoof() : Void
    {
        ...
        {
            outputText("Your predecessor destroyed our welcoming facility. Otherwise I’m sure a certain imp would have revealed the truth to you long ago, before joining you with our breeding machines.\"</i>");
        }
        else
        {
    ...
    private function goHomeJustice() : Void
    {
        ...
        {
            outputText(" to say nothing of [his] bizarre friends");
        }
    ...
    private function HowFuckedIsThePlayerOMeter() : Float
    {
        var levelOfFuckedness : Float = 0;
        ...
        //Jojo corrupt +10
        if (SceneLib.camp.campCorruptJojo())
        {
            levelOfFuckedness += 10;
        }
        ...
        //Ceraph tamed -5
        if (SceneLib.camp.ceraphIsFollower())
        {
            levelOfFuckedness -= 5;
        }
        //Owca saved -10
    ...
    {
        return player.statusEffectv1(StatusEffects.Birthed) > 0 || flags[kFLAGS.AMILY_BIRTH_TOTAL] > 0 || flags[kFLAGS.PC_TIMES_BIRTHED_AMILYKIDS] > 0 || flags[kFLAGS.BENOIT_EGGS] > 0 || flags[kFLAGS.COTTON_KID_COUNT] > 0 || flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] > 0 || flags[kFLAGS.EMBER_CHILDREN_MALES] > 0 || flags[kFLAGS.EMBER_CHILDREN_FEMALES] > 0 || flags[kFLAGS.EMBER_CHILDREN_HERMS] > 0 || flags[kFLAGS.EMBER_EGGS] > 0 || flags[kFLAGS.IZMA_CHILDREN_SHARKGIRLS] > 0 || flags[kFLAGS.IZMA_CHILDREN_TIGERSHARKS] > 0 || flags[kFLAGS.KELLY_KIDS_MALE] > 0 || flags[kFLAGS.KELLY_KIDS] > 0 || flags[kFLAGS.MARBLE_KIDS] > 0 || flags[kFLAGS.ANT_KIDS] > 0 || flags[kFLAGS.PHYLLA_DRIDER_BABIES_COUNT] > 0 || flags[kFLAGS.SHEILA_JOEYS] > 0 || flags[kFLAGS.SHEILA_IMPS] > 0 || flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] > 0 || flags[kFLAGS.SOPHIE_DAUGHTER_MATURITY_COUNTER] > 0 || flags[kFLAGS.SOPHIE_EGGS_LAID] > 0 || flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS] > 0 || SceneLib.urtaPregs.urtaKids() > 0;
    }
    ...
    private function purgeShit() : Void
    {
        ...
        var levelOfFuckedness : Float = HowFuckedIsThePlayerOMeter();
