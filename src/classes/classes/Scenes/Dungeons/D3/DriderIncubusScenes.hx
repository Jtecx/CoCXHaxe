...
class DriderIncubusScenes extends BaseContent
{
    public function new()
    {
        super();
    ...
    public function spooderbuttGetsANewCockSleeve(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        clearOutput();
        
        var m : DriderIncubus = try cast(monster, DriderIncubus) catch(e:Dynamic) null;
    ...
    public function beatTheSpooderbutt(hpVictory : Bool = false) : Void
    {
        clearOutput();
        if (!recalling)
        {
    ...
    {
        var doneDriderbus : Bool = false;
        var doneGoblin : Bool = false;
        menu();
    ...
    public function titfuckMitzi() : Void
    {
        ...
        if (player.hasBalls() && player.lust >= 75)
        {
    ...
    public function actuallyRideDriderDick() : Void
    {
        ...
        if (!player.hasPerk(PerkLib.HistorySlut) && !player.hasPerk(PerkLib.HistoryWhore))
        {
            outputText(" chaste");
        }
        else
    ...
    public function buttfuckDrider() : Void
    {
        ...
        if ((try cast(monster, DriderIncubus) catch(e:Dynamic) null).goblinFree)
        {
