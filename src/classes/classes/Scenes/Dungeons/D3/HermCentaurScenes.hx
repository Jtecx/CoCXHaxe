...
/**
	 ...
	 * @author Gedan
	 */
class HermCentaurScenes extends BaseContent
{
    ...
    public function encounterThePony() : Void
    {
        ...
        if (player.vaginalCapacity() < 100 || player.analCapacity() < 100)
        {
    ...
    private static inline var CENTAUR_KILLED : Int = 1;
    private static inline var CENTAUR_RELEASED : Int = 2;
    private static inline var CENTAUR_FUCKED : Int = 3;
    
    public function beatThePony(hpVictory : Bool = false) : Void
    {
        clearOutput();
        if (!recalling)
        {
        ...
        {
            outputText(" as she tries to stretch for her pulsating, cum-dripping member");
        }
        ...
        if (hpVictory)
        {
            outputText(" resigned glare");
        }
        else
        {
            outputText(" lustful glare");
        }
    ...
    private function maleFuckHer() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText(" Slapping wetly against hers, your [balls] ring in an increasing torrent of fleshy slaps.");
        }
        ...
        if (player.cumQ() >= 1000)
        {
            outputText(" You quake at the sensation of dumping so much of your creamy batter into her stomach, nearly closing your eyes in bliss.  Luckily, you hold onto your wits.");
        }
        if (player.cumQ() >= 5000)
        {
            outputText(" It doesn't make much difference to the demon beneath you, as her belly slowly rounds from the obscene deposit you're plugging her with.");
        }
        if (player.cumQ() >= 15000)
        {
            outputText(" Bigger and bigger, she bloats until she looks fat or pregnant, jiggling with your thick spooge.");
        }
        ...
        if (player.cumQ() >= 15000)
        {
            outputText(", practically dragging her gut on the stones as she runs");
        }
    ...
    private static inline var HORZGOG : Int = 0;
    private static inline var DOGGECOCK : Int = 1;
    private static inline var TENTACOCK : Int = 2;
    ...
    private function femFuckHerII(cockType : Int) : Void
    {
        ...
        if (cockType == TENTACOCK)
        {
            ...
            {
                outputText(" A coil of cock loops around your gigantic buzzer, effectively frotting it like another cock and chance at protest from your lips.");
            }
            else
            {
                outputText(" A bend in the cock grinds against your buzzer with particular intensity, effectively robbing any chance at protest from your lips.");
            }
            ...
            if (player.clitLength >= 6)
            {
                outputText(" - you swear it spends as much time caressing your [clit] as squeezing it off");
            }
        ...
        else
        {
            ...
            if (player.clitLength >= 6)
            {
                outputText(" The hard, feminine pole twitches nicely at your touch, a far more sensitive rod than any cock.");
            }
    ...
    public function inSovietCoCPonyRidesYou(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
    ...
    private function femLoss(hpVictory : Bool) : Void
    {
        clearOutput();
        ...
        if (player.tallness >= 7 * 12)
        {
            outputText(", even though your [feet] still touch the ground");
        }
        ...
        if (player.hasCock())
        {
            outputText(" [EachCock] fares little better, a turgid mess of pre-cum leaking lust that seems to grow harder and more sensitive by the moment.");
        }
    ...
    private function maleLoss(hpVictory : Bool) : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText(" Your [vagina] fares no better, puffing up and drooling like its big brother while your [clit] grows taut and sensitive.");
        }
        ...
        if (player.hasBalls())
        {
            outputText(" [balls] feel fit to burst!");
        }
        else
        {
            outputText(" manhood feels swollen to bursting!");
        }
        outputText("  Realizing the only way out will be with your tongue, you open wide");
        if (player.tongue.type != 0)
        {
            outputText(" and uncoil your inhuman length to spear");
        }
        else
        {
            outputText(" and begin to lap");
        }
        ...
        {
            outputText(", and a familiar one at that");
        }
        ...
        if (flags[kFLAGS.D1_OMNIBUS_KILLED] != 1)
        {
            outputText(" overseer asks, \"<i>Remember me?  I thought it only proper that I thank you for letting me live.  Now, be a good boy and suck my dick - show all everyone just what a horny, helplessly fuckable toy you are.</i>\"");
        }
        else
        {
            outputText(" asks, \"<i>I hope you don't mind slurping down a little cock, because you're going to anyway.</i>\"");
        }
        
        outputText("\n\nThe second herm grabs you by the");
        if (player.horns.type != 0)
        {
            outputText(" horns");
        }
        else
