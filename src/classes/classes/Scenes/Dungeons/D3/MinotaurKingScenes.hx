...
class MinotaurKingScenes extends BaseContent
{
    public function new()
    {
        super();
    ...
    public function encounterMinotaurKing() : Void
    {
        ...
        else if (SceneLib.helFollower.followerHel() && !player.hasStatusEffect(StatusEffects.HeliaOff))
        {
    ...
    public function theKingIsDeadLongLiveTheKing() : Void
    {
        ...
        if (flags[kFLAGS.EXCELLIA_RECRUITED] != null && !recalling)
        {
        ...
        menu();
        if (flags[kFLAGS.MINOTAURKING_FUCKED] == null || recalling)
        {
        ...
        if (flags[kFLAGS.EXCELLIA_RECRUITED] == null && flags[kFLAGS.MINOTAURKINGSSLUT_FUCKED] == null || recalling)
        {
        ...
        if (!recalling)
        {
            if (flags[kFLAGS.EXCELLIA_RECRUITED] == null)
            {
                addButton(10, "RecruitHer", recruitExcellia).hint("Keep Excellia after you deal with the Lethice.");
            }
            addButton(13, (flags[kFLAGS.EXCELLIA_RECRUITED] != null) ? "Kill Him" : "Kill Them", murderhobo);
            addButton(14, (flags[kFLAGS.EXCELLIA_RECRUITED] != null) ? "Leave Him" : "Leave Them", leavethem);
        }
        else
        {
    ...
    {
        var x1 : Int = as3hx.Compat.parseInt(cockIdx + 1);
        clearOutput();
    ...
    private function buttufucku() : Void
    {
        ...
        {
            outputText(" until his belly bulges with the clear outline of your [cock biggest], displaying just how fully he has been taken to the entire assemblage. You’ve claimed this once noble beast, turned his body into a fuzzy, pleasant-smelling condom for your excessively-large cock.");
        }
        ...
        {
            outputText(" until you’ve buried the entirety of your [cock biggest] inside his snugly squeezing intestines, repurposing his digestive system into your own personal, phallus playground.");
        }
    ...
    private function mechanicalbullhue() : Void
    {
        ...
        if (!recalling)
        {
            ...
            {
                player.refillGargoyleHunger(60);
            }
    ...
    public function hailToTheKingBaby(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        clearOutput();
        
        var mk : MinotaurKing = try cast(monster, MinotaurKing) catch(e:Dynamic) null;
        ...
        if (player.hasPerk(PerkLib.BimboBrains) || player.hasPerk(PerkLib.FutaFaculties) || player.cor >= 66)
        {
