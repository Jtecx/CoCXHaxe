...
/**
	 ...
	 * @author Gedan
	 */
class Doppleganger extends Monster
{
    private var _roundCount : Int = 0;
    
    public function mirrorAttack(damage : Float) : Void
    {
    ...
    override public function handleTease(lustDelta : Float, successful : Bool, display : Bool = true) : Bool
    {
        clearOutput();
        if (display)
        {
    ...
    private function addTalkShit() : Void
    {
        ...
        switch (_roundCount)
        {
            ...
            case 4, 5:
                ...
                if (player.hasCock())
                {
                    outputText(" strokes [his] [cock]");
                }
                else if (player.hasVagina())
                {
                    outputText(" slides two fingers into [his] [vagina] and gently frigs [himself]");
                }
                else
                {
                    outputText(" slips a hand ");
                }
    ...
    override public function preMeleeDmg(damage : Float) : Float
    {
        if (!monsterIsStunned())
        {
            if (damage > 0)
        ...
        if (hasStatusEffect(StatusEffects.MirroredAttack))
    ...
    {
        if (Std.is(ability, WhitefireSpell))
        {
    ...
    public function handleSpellResistance(spell : String) : Void
    {
        ...
        if (spell == "fireball")
        {
            var damage : Float = player.level * 10 + 45 + rand(10);
            damage = Math.round(damage);
            player.takeFireDamage(damage, true);
        }
        else if (spell == "whitefire")
        {
            var damage2 : Float = 10 + (player.inte / 3 + rand(player.inte / 2));
    ...
    override private function get_long() : String
    {
        ...
        str += Std.string(Math.floor(player.tallness / 12) + " foot " + player.tallness % 12 + " inch tall ");
        ...
        {
            str += " " + player.mf("His", "Her") + " tail flicks this way and that.";
        }
        str += " [He] wields the exact same [weapon] you do, and is dressed in the mirror image of your [armor]. ";
        if (player.biggestTitSize() >= 2)
        {
            str += "It’s difficult not to notice the way the mirror image of your " + player.breastDescript(player.biggestTitRow()) + " ebbs and heaves within it.";
        }
