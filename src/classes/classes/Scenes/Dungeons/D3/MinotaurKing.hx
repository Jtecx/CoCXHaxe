...
class MinotaurKing extends Monster
{
    public var orgasms(get, never) : Int;
    ...
    private function get_orgasms() : Int
    {
        return _orgasms;
    }
    private var _lastRoundStun : Bool = false;
    private var _lastSpellCastCount : Int;
    
    private var atks : Array<Dynamic> = [backhand, battleaxe, minoPheromones];
    ...
    private function backhand() : Void
    {
        ...
        var damage : Float = (str + weaponAttack) - rand(player.tou);
    ...
    private function headbutt() : Void
    {
        ...
        var damage : Float = ((str + weaponAttack) / 2) - rand(player.tou);
        if (damage <= 0 || player.getEvasionRoll())
        {
            outputText(" Luckily, you dodge aside.");
        }
    ...
    private function hpRestore() : Void
    {
        ...
        {
            player.addStatusValue(StatusEffects.MinotaurKingMusk, 1, 1);
        }
    }
    ...
    override public function postPlayerBusyBtnSpecial(btnSpecial1 : CoCButton, btnSpecial2 : CoCButton) : Void
    {
        ...
        {
            btnSpecial1.show("Dish Helper", SceneLib.hexindao.dishHelper);
        }
        else
        {
            btnSpecial1.showDisabled("Dish Helper", "You don't have anyone to take care of " + ((player.hasStatusEffect(StatusEffects.SoulArena)) ? "cow maid" : "Excellia") + "!");
        }
    }
}
