...
/**
	 ...
	 * @author Gedan
	 */
class JeanClaude extends Monster
{
    ...
    override public function interceptPlayerAbility(ability : CombatAbility) : Bool
    {
        if (Std.is(ability, BlindSpell))
        {
            ...
            if (rand(player.spe) >= 50 || rand(player.inte) >= 50)
            {
                ...
                HP -= as3hx.Compat.parseInt(10 + (player.inte / 3 + rand(player.inte / 2)) * SceneLib.combat.spellModWhite());
            }
            else
            {
