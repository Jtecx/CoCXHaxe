...
/**
	 ...
	 * @author Gedan
	 */
class SuccubusGardener extends Monster
{
    ...
    // The succubus gardener is a multistage fight. She starts off all but immune to lust damage. She has enough HP not to be one-shot and a heal move that takes priority over any stun. Once she is reduced to 60% HP, she either drinks from her tentacles or is force-fed by them (if stunned). This fully heals her but makes her 15% more vulnerable to lust.
    {
        ...
        {
            var opts : Array<Dynamic> = [sicem, corruptiveShower, lustAuraCast];
    ...
    {
        if (HPRatio() <= 0.6 && !hasStatusEffect(StatusEffects.MonsterAttacksDisabled))
        {
            return true;
        }
        else
    ...
    private function vineHeal() : Void
    {
        ...
        if (!hasStatusEffect(StatusEffects.Stunned))
        {
            ...
            if (this.lustVuln <= 0.1)
            {
                outputText(" Whatever is in that healing nectar must be weakening her self-control.");
            }
        }
        else
        {
    ...
    private function showerDotEffect() : Void
    {
        ...
        {
            outputText("The tentacles' sex-juices are still covering you - still slowly arousing you. You've got a good handle on it for now.\n\n");
        }
        else if (player.lust < .6 * player.maxLust())
        {
            ...
            if (player.cor < 50)
            {
                outputText(" You're ashamed to admit");
            }
            else
            {
                outputText(" You're a little irritated to admit");
            }
    ...
    private function motorboat() : Void
    {
        ...
        if (player.tallness <= this.tallness - 6)
        {
            outputText(", somehow short enough in spite of the height differences.");
        }
        else if (player.tallness >= this.tallness + 6)
        {
            outputText(", somehow tall enough in spite of the height differences.");
        }
    ...
    //Used once after first orgasm
    {
        ...
        // 20%?
        var effect : GardenerSapSpeedDebuff = try cast(player.createOrFindStatusEffect(StatusEffects.GardenerSapSpeed), GardenerSapSpeedDebuff) catch(e:Dynamic) null;
        effect.increase();
    }
    ...
    private function lustAuraCast() : Void
    {
        ...
        if (this.hasStatusEffect(StatusEffects.LustAura))
        {
            ...
            player.takeLustDamage(8 + as3hx.Compat.parseInt(player.lib / 20 + player.cor / 25), true);
        }
        else
        {
    ...
    //Strength check based lust damage, used when aroused only.
    {
        ...
        if (rand(player.str - 30) + 30 > this.str)
        {
            ...
            player.dynStats("lus", 8 + as3hx.Compat.parseInt(player.lib / 20 + player.cor / 25), "cor+", 5);
        }
