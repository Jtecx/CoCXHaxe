...
class D3 extends BaseContent
{
    ...
    private function configureRooms() : Void
    {
        ...
        Reflect.setField(rooms, Std.string(tRoom.RoomName), tRoom);
    ...
    public function exitD3() : Void
    {
        ...
        {
            clearOutput();
            if (flags[kFLAGS.EXCELLIA_RECRUITED] == 1 && (!camp.followerShouldra() || player.hasStatusEffect(StatusEffects.ShouldraOff)))
            {
                outputText("Before you go, you look for where you left the former cow queen. You find her laying in an imp nest covered in cum. She appears to have fallen asleep sometime during the battle. Shaking your head, you wipe most of the mess from her body then drag her back to camp, leaving the stronghold behind.\n\n");
            }
    ...
    public function move(roomName : String, timeToPass : Float = 0) : Void
    {
        trace("Entering room", roomName);
        cheatTime(timeToPass);
        clearOutput();
        
        if (Reflect.field(rooms, roomName) == null)
        {
            clearOutput();
        ...
        var tRoom : Room = Reflect.field(rooms, roomName);
    ...
    private function entranceRoomFunc() : Bool
    {
    ...
    private function tunnel1RoomFunc() : Bool
    {
    ...
    private function antechamberRoomFunc() : Bool
    {
    ...
    private function roomofmirrorsRoomFunc() : Bool
    {
        ...
        if (flags[kFLAGS.D3_MIRRORS_SHATTERED] == 1)
        {
            outputText(" now shattered,");
        }
        ...
        else
        {
            ...
            if (flags[kFLAGS.D3_ENTERED_MAGPIEHALL] == 1)
            {
                outputText("  Your spirits rise. They look like they may very well be made of the same material as the screen in the basilisk hall.");
            }
            if (player.inte >= 70 || player.sens >= 70)
            {
                outputText("  Disquiet edges down your spine. Something about this place doesn’t feel right. The room seems faded at the corners, as if it’s not quite there.");
            }
    ...
    private function magpiehallsRoomFunc() : Bool
    {
        ...
        if (flags[kFLAGS.D3_JEAN_CLAUDE_DEFEATED] == 0)
        {
            if (flags[kFLAGS.D3_ENTERED_MAGPIEHALL] == 0)
            {
                ...
                if (flags[kFLAGS.BENOIT_AFFECTION] == 100)
                {
                    outputText("  This can only be the hall that " + SceneLib.bazaar.benoit.benoitMF("Benoit", "Benoite") + " once worked in.");
                }
                ...
                if (player.canFly())
                {
                    outputText(", even if you tried to fly it");
                }
                outputText(". Wouldn’t you? You can’t outrun the gaze of a thousand basilisks... could you?");
                if (player.hasKeyItem("Laybans") >= 0)
                {
                    outputText("  You take the Laybans out of your pouch and hold them up against the glass. It’s exactly as you hoped - they are made of the same material, and are almost certainly what the demons wear when they themselves interact with the basilisks. They would surely help you get across the hall, if you were crazy enough to try.");
                }
            }
            else
            {
                ...
                if (player.hasKeyItem("Laybans") < 0)
                {
                    outputText("  Are you going to try this?");
                }
                else
                {
                    outputText("  You take the Laybans out of your pocket, turning them around in your hands as you consider. Are you going to try this?");
                }
            }
        ...
        if (eggsAvailable() >= 0 || flags[kFLAGS.D3_DEMONIC_SCYTHE] == 0 || flags[kFLAGS.D3_GOBLIN_MECH_PRIME] == 0)
        {
    ...
    private var BLACK(default, never) : Int = 1 << 0;
    private var BLUE(default, never) : Int = 1 << 1;
    private var WHITE(default, never) : Int = 1 << 2;
    private var PINK(default, never) : Int = 1 << 3;
    private var BROWN(default, never) : Int = 1 << 4;
    private var PURPLE(default, never) : Int = 1 << 5;
    ...
    private function goToEggPile() : Void
    {
        ...
        {
            outputText(" When searching you even finds something that looks like scythe. Where does it come frome among all those eggs?");
        }
        menu();
        ...
        if ((flagNum & BLACK) == 0)
        {
            addButton(0, "Black", takeEgg, BLACK);
        }
        if ((flagNum & BLUE) == 0)
        {
            addButton(1, "Blue", takeEgg, BLUE);
        }
        if ((flagNum & WHITE) == 0)
        {
            addButton(2, "White", takeEgg, WHITE);
        }
        if ((flagNum & PINK) == 0)
        {
            addButton(3, "Pink", takeEgg, PINK);
        }
        if ((flagNum & BROWN) == 0)
        {
            addButton(4, "Brown", takeEgg, BROWN);
        }
        if ((flagNum & PURPLE) == 0)
        {
            addButton(5, "Purple", takeEgg, PURPLE);
        }
        if (flags[kFLAGS.D3_DEMONIC_SCYTHE] == 0)
        {
            addButton(6, "Scythe", takeScythe);
        }
        if (flags[kFLAGS.D3_GOBLIN_MECH_PRIME] == 0)
        {
            addButton(7, "Mech", takeMech);
        }
    ...
    private function fallbackFromMagpieHallS() : Void
    {
        ...
        if (player.hasKeyItem("Laybans") < 0 && player.inte >= 50)
        {
            outputText("  Surely the demons themselves are not immune to the basilisks’ glares - the darkened screen is proof of that. How do they interact with the creatures, then? Maybe if you keep poking around, you might find an answer.");
        }
    ...
    private function magpiehallnRoomFunc() : Bool
    {
        outputText("<b><u>Magpie Hall</u></b>\n");
        if (flags[kFLAGS.D3_JEAN_CLAUDE_DEFEATED] == 0)
        {
    ...
    private function tunnel2RoomFunc() : Bool
    {
    ...
    private function edgeofkeepRoomFunc() : Bool
    {
    ...
    private function northentryRoomFunc() : Bool
    {
    ...
    private function southcourtyardRoomFunc() : Bool
    {
    ...
    private function southwestcourtyardRoomFunc() : Bool
    {
    ...
    private function southwestwalkRoomFunc() : Bool
    {
    ...
    private function westwalkRoomFunc() : Bool
    {
        ...
        {
            outputText(" see a massive statue with an immense hammer");
        }
        else
        {
            outputText(" a mound of rubble, the scattered remains of the animated statue that you slew");
        }
    ...
    private function northwestwalkRoomFunc() : Bool
    {
    ...
    private function northwestcourtyardRoomFunc() : Bool
    {
    ...
    private function northcourtyardRoomFunc() : Bool
    {
    ...
    private function northeastcourtyardRoomFunc() : Bool
    {
    ...
    private function northeastwalkRoomFunc() : Bool
    {
    ...
    private function eastwalkRoomFunc() : Bool
    {
        ...
        if (flags[kFLAGS.D3_STATUE_DEFEATED] == 0)
        {
            outputText(" a massive statue with an immense hammer.");
        }
        else
        {
            outputText(" a mound of rubble, the scattered remains of the animated statue that you slew.");
    ...
    private function southeastwalkRoomFunc() : Bool
    {
    ...
    private function southeastcourtyardRoomFunc() : Bool
    {
    ...
    private function courtyardsquareRoomFunc() : Bool
    {
    ...
    private function greatliftRoomFunc() : Bool
    {
    ...
    private function throneRoom() : Bool
    {
    ...
    public function unlockedThroneRoom() : Bool
    {
