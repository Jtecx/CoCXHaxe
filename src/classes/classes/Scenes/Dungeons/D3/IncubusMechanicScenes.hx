...
/**
	 ...
	 * @author Gedan
	 */
class IncubusMechanicScenes extends BaseContent
{
    ...
    private function metIncubusMechanicInD1() : Bool
    {
        return flags[kFLAGS.FACTORY_INCUBUS_DEFEATED] > 0 || flags[kFLAGS.FACTORY_INCUBUS_BRIBED] > 0;
    }
    
    public static inline var MECHNIC_NO_SELECTION : Int = 0;
    public static inline var MECHANIC_PAID : Int = 1;
    public static inline var MECHANIC_SUCKED : Int = 2;
    public static inline var MECHANIC_FOUGHT : Int = 3;
    
    public static inline var MECHANIC_KILLED : Int = 1;
    public static inline var MECHANIC_RELEASED : Int = 2;
    public static inline var MECHANIC_FUCKED : Int = 3;
    public static inline var MECHANIC_FUCKED_YOU : Int = 4;
    ...
    public function meetAtElevator() : Void
    {
        if (flags[kFLAGS.D3_MECHANIC_LAST_GREET] == MECHNIC_NO_SELECTION)
        {
            ...
            {
                outputText(", a rolled-up hentai magazine tucked neatly into the chest-pocket of his overalls");
            }
            if (metIncubusMechanicInD1())
            {
            ...
            if (metIncubusMechanicInD1())
            {
                outputText("\"<i>Once more, you bumble");
            }
            else
            {
                outputText("\"<i>You've bumbled");
            }
    ...
    public function suckIncubusDick() : Void
    {
        ...
        if (player.gems < 500)
        {
            outputText("You spare a glance to your depleted pouches and admit, \"<i>I guess I'll suck your dick then.</i>\"");
        }
        else
        {
            outputText("You shrug and smile. \"<i>I guess now is as good a time as any to suck some dick, huh?</i>\"");
        }
        ...
        if (player.hasVagina())
        {
            ...
            if (player.wetness() <= 2)
            {
                outputText(" even more soaked than normal");
            }
            else if (player.wetness() <= 4)
            {
                outputText(" wet with fresh lust");
            }
            else
            {
                outputText(" sticky with lust");
            }
    ...
    public function beatDaMechanic(hpVictory : Bool = false) : Void
    {
        if (!recalling)
        {
        ...
        if (hpVictory)
        {
            outputText(" falling off his prone from, shredded.");
        }
        else
        {
            outputText(" hanging open as he reaches down for his exposed, dangling cock to wildly masturbate.");
        }
        ...
        if (recalling)
        {
            addButton(4, "Suck Dick", suckIncubusDick).hint("Maybe you should have just sucked his dick instead of fighting?");
        }
    }
    ...
    private function letMechanicGo(hpVictory : Bool) : Void
    {
        if (!recalling)
        {
        ...
        if (hpVictory)
        {
            outputText(" slowly struggles to his feet");
        }
        else
        {
            outputText(" reluctantly stops masturbating");
        }
    ...
    private function buttfuckTheMechanic(hpVictory : Bool) : Void
    {
        ...
        if (!hpVictory)
        {
            outputText(" throbbing hard");
        }
        else
        {
            outputText(" gently bobbing");
        }
        ...
        if (silly())
        {
            outputText(" tab C with slot buttfuck");
        }
        else
        {
            outputText(" your eager tool with the corrupted anus");
        }
    ...
    private static inline var MECHANIC_DEMON_COCK : Int = 0;
    private static inline var MECHANIC_HORZ_GOG : Int = 1;
    private static inline var MECHANIC_DOG_COCK : Int = 2;
    ...
    private function rideMechanicsCock() : Void
    {
        ...
        {
            outputText(", tail draped across his face");
        }
    ...
    private function rideMechanicsCockII(cType : Int) : Void
    {
        ...
        if (player.wetness() <= 2)
        {
            outputText(", letting your free-flowing pussyjuice wash over it, lubricating it with your glorious girlcum");
        }
        else if (player.wetness() <= 3)
        {
            outputText(", letting your oh-so-juicy vagina drip and dribble, soaking it with your copious girlcum");
        }
        else
        {
            outputText(", letting your lubricant drip over it");
        }
        ...
        var mCockArea : Float = monster.cockArea(0);
        var pCuntArea : Float = player.vaginalCapacity();
        var tightFit : Bool = false;
        ...
        if (pCuntArea >= mCockArea * 1.5)
        {
            ...
            {
                outputText(", your belly bulging with his equine shape");
            }
            else if (cType == MECHANIC_DOG_COCK)
            {
                outputText(", your lips slurping noisily about his knot");
            }
            outputText(".");
        }
        ...
        if (flags[kFLAGS.FACTORY_INCUBUS_BRIBED] > 0)
        {
            outputText(" Nothing but hentai mags for you to drain your balls with....");
        }
        outputText("</i>\"");
        ...
        {
            outputText(" You thumb at it, rubbing in slow circles around it. Sometimes you push on its hood. Other times you oh-so-gently squeeze at it. It feels so good that the touches drive you crazy with ecstasy, and your walls flutter excitedly around your demonic lover.");
        }
        ...
        if (cType == MECHANIC_HORZ_GOG)
        {
            outputText(", the head flaring to seal itself against your cervix");
        }
        else if (cType == MECHANIC_DOG_COCK)
        {
            outputText(", the knot popping in just in time to start bloating, locking you in place");
        }
        ...
        if (!player.isPregnant())
        {
            ...
            if (cType == MECHANIC_HORZ_GOG)
            {
                outputText(" After all, that bestial horse-cock doesn't really leave it anywhere else to go. The demonic animal-cunt just continues spurting straight on in to have its way with your eggs, and you're sure there isn't a contraceptive this side of Mareth that could stop you from having his imps.");
            }
            else if (cType == MECHANIC_DOG_COCK)
            {
                outputText(" After all, that immense knot doesn't really leave it anywhere else to go. Once every fold is bathed in alabaster doggie-cream, the continued ejaculations start pouring in after your eggs. His knot is so big and stretching you so wide; you don't think he would pop out even if he filled your belly to look nine months pregnant. Worse, or perhaps better, you're sure with how thick and virile his seed feels that you'll be getting pregnant with his imps from this.");
            }
            if (!player.isGoblinoid() && !recalling)
            {
        ...
        else
        {
            ...
            {
                outputText(" and drip out over his balls, much to his chagrin.");
            }
            else
            {
                outputText(" and stretch you further.");
            }
        }
        ...
        if (player.wetness() < 4)
        {
            outputText(", even though you aren't normally a squirter");
        }
    ...
    public function mechanicFuckedYouUp(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
    ...
    private function maleLossToMechanic(hpVictory : Bool) : Void
    {
        clearOutput();
        ...
        if (player.cocks.length > 1)
        {
            ...
            if (!allDemon)
            {
                outputText(" The skin of your transforming length is darkening to a purplish hue, and in the wake of the magical cock-ring, nodules are slowly emerging, growing into an imitation of the cock you're still suckling.");
            }
    ...
    private function errybodyelseLossToMechanic(hpVictory : Bool) : Void
    {
        clearOutput();
        if (player.isAlraune())
        {
        ...
        else
        {
            ...
            else if (!player.hasVagina() && hpVictory)
            {
                ...
                {
                    outputText(" slipping inside a [nipple] to thrust just like your other hand. You work your upper and lower pussies like a woman possessed.");
                }
                else
                {
                    outputText(" pinching and tugging on");
                    if (player.totalNipples() > player.totalBreasts())
                    {
                        outputText(" nipple after nipple, your myriad breastly endowments providing you with oh so many buttons to tend to");
                    }
                    else
                    {
                        outputText("your [nipples] one after another");
                    }
                }
            ...
            if (player.isNaga())
            {
                outputText("r coils don't get sore");
            }
            else if (!player.isBiped())
            {
                outputText(" don't even need to support yourself anymore");
            }
            else
            {
                outputText(" knees don't get sore");
            }
            ...
            if (player.clitLength > 5)
            {
                outputText(", even if that makes it truly monstrous indeed");
            }
