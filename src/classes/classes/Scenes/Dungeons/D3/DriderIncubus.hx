...
class DriderIncubus extends AbstractSpiderMorph
{
    public var goblinFree(get, set) : Bool;
    public var goblinOiled(get, never) : Bool;
    ...
    private static inline var VENOM_SPEED_DRAIN_FLAT : Int = 30;
    private static inline var VENOM_SPEED_DRAIN_MULT : Int = 10;
    ...
    private var _goblinFree : Bool = false;
    private function get_goblinFree() : Bool
    {
        return _goblinFree;
    }
    private function set_goblinFree(v : Bool) : Bool
    {
        if (v != _goblinFree)
        {
            this.long = "The drider incubus is a marvel of demonic perversions applied to inhuman flesh. His glittering dark skin is like a solid sheet of coal. Twisted obsidian horns spiral from his forehead like a pair of misshapen spears. Even his eyes are an eerie marvel, no longer windows to the soul, only pits of endless hunger. His evil visage pales in comparison to the monstrous form below his waist - that of a gigantic spider. Eight clattering legs skitter across the floor with his every move. A bulbous abdomen hangs behind them, covered with twitching spinnerets so full of silk that half-formed blobs of it dangle behind him.";
    ...
    override public function postPlayerBusyBtnSpecial(btnSpecial1 : CoCButton, btnSpecial2 : CoCButton) : Void
    {
        if (!goblinFree)
        {
            btnSpecial1.show("Free Goblin", freeGoblin);
        }
    }
    
    private function performPhysicalAttack() : Void
    {
        if (_combatRound >= 3 && (_combatRound % 6 == 0 || _combatRound == 3))
        {
            stunningSpear();
        }
        else
        {
            var opts : Array<Dynamic> = [bite, spiderMorphWebAttack, kick, kick, doubleStrike, doubleStrike];
    ...
    private function performLustAttack() : Void
    {
        ...
        {
            var opts : Array<Dynamic> = [arouseSpell, arouseSpell];
            if (!player.hasStatusEffect(StatusEffects.TaintedMind) && !_seenResolute)
            {
                opts.push(taintedMind);
            }
            if (!_seenResolute)
            {
                opts.push(purpleHaze);
            }
            opts[rand(opts.length)]();
        }
    }
    ...
    {
        var opts : Array<Dynamic> = [goblinHandjob, goblinTongueGrapple, bootyTwerking, webNipplechain];
        if (!_goblinOiled)
        {
            opts.push(babyOilMeUp);
        }
    ...
    //Always does this once plus another physical attack when physically attacking
    {
        ...
        var damage : Float = (str + weaponAttack) * 0.40;
    ...
    private function bite() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.Stunned) || player.spe <= 1)
        {
            ...
            player.statStore.addBuff("str", -amount, "Poison", {
                        text : "Poisoned!",
                        time : Buff.RATE_HOURS,
                        tick : 4
                    });
            showStatDown("str");
    ...
    private function kick() : Void
    {
        ...
        {
            var damage : Float = (str + weaponAttack + 25) - rand(player.tou);
    ...
    //Uses another strike like his normal one with a different intro
    {
        ...
        var damage : Float = str + weaponAttack + 10 - rand(player.tou);
    ...
    private var _seenResolute : Bool = false;
    ...
    private var _goblinWebChain : Bool = false;
    ...
    private var _goblinOiled : Bool = false;
    private function get_goblinOiled() : Bool
    {
        return _goblinOiled;
    }
