...
/**
	 ...
	 * @author Gedan
	 */
class HermCentaur extends Monster
{
    ...
    private var _usedGottaCum : Bool = false;
    private var _usedHeal : Bool = false;
    
    private var _chargingArouse : Bool = false;
    private var _lustAtChargeStart : Float = -1;
    ...
    override private function performCombatAction() : Void
    {
        ...
        else if ((this.lust + 60) / this.maxLust() >= .9 && !_usedGottaCum)
        {
            gottaCum();
        }
        else if (this.HPRatio() <= 0.5 && !_usedHeal)
        {
            healUp();
        }
        else if (rand(lust) / this.maxLust() >= 0.4)
        {
            this.hypnoCock();
        }
        // Selections
        else
        {
            ...
            var opts : Array<Dynamic> = [feminineMusk, aphrodisiacSquirt];
            if (!_chargingArouse && _arouseCooldown <= 0)
            {
                opts.push(arouseSpellCharge);
            }
    ...
    private function feminineMusk() : Void
    {
        ...
        if (player.lust <= .5 * player.maxLust())
        {
            outputText(" your [skin] grows feverishly hot.");
        }
        else
        {
            ...
            if (player.hasCock())
            {
                outputText(" [eachCock] with the desire to plunge inside that juicy-hot hole.");
            }
            else if (player.hasVagina())
            {
                outputText(" your [vagina] with the desire to submit while juices pool between your [legs].");
            }
            else
            {
                outputText(" your anus until it puckers, craving something to fill it - anything.");
            }
        }
    ...
    private function aphrodisiacSquirt() : Void
    {
        ...
        if (player.lust < .3 * player.maxLust())
        {
        ...
        else if (player.lust < .4 * player.maxLust())
        {
            outputText("\n\nYour heart beats faster.");
        }
        else if (player.lust < .5 * player.maxLust())
        {
        ...
        else if (player.lust < .7 * player.maxLust())
        {
            ...
            if (player.hasCock())
            {
                outputText(" [eachCock] awakens, filling itself towards full erection.");
            }
            else if (player.hasVagina())
            {
                outputText(" a wet heat answers in your loins, growing more insistent from moment to moment.");
            }
            else
            {
                outputText(" your [asshole] tingles, craving something to fill it.");
            }
        }
        else if (player.lust < .8 * player.maxLust())
        {
            ...
            if (player.hasCock())
            {
                outputText(" ignoring the stiffening pressure of [eachCock]");
            }
            else
            {
                outputText(" ignoring the increasing wetness of your inner [armor]");
            }
        ...
        else if (player.lust < .9 * player.maxLust())
        {
            ...
            if (player.hasCock())
            {
                outputText("  Pre begins to dribble from your hot-and-ready cum-slit, eager to pour out in a deluge of sperm.");
            }
            else if (player.hasVagina())
            {
                outputText("  A flood of sopping moisture dribbles down your [legs], the proof of your unholy desire to breed.");
            }
        ...
        {
            outputText(" Sinking to your knees, you start trying to tear your way out of your [armor].  You don't care about winning anymore - fucking this woman... this beast... that's all that matters.  You don't care how she takes you, as long as you get to cum all over her!");
        }
    }
    ...
    // one turn charge-up, can be interrupted by tease
    {
        ...
        if (player.inte > 80)
        {
            outputText("  A tease attack would likely be the most effective method of attack.");
        }
    ...
    private function hypnoCock() : Void
    {
        ...
        if (player.inte * (2 / _hypnoCockUses) > rand((player.lib / 3) + (player.effectiveSensitivity() / 3) + (player.cor / 3)))
        {
            ...
            if (player.lust <= .33 * player.maxLust())
            {
            ...
            else if (player.lust <= .66 * player.maxLust())
            {
