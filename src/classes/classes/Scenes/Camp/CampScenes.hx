...
class CampScenes extends NPCAwareContent
{
    ...
    public function HotSpring() : Void
    {
        ...
        switch (_sw0_)
        {
            ...
            case 1:
                ...
                if (flags[kFLAGS.CAMP_UPGRADES_HOT_SPRINGS] >= 5)
                {
                    addButton(2, "Peep", PeepingTom2).hint("Peep at bathing girls.");
                }
            case 3:
                //girls
                if (player.femininity >= 60 || player.femininity >= 30 && player.breastRows[0].breastRating > 1 || player.breastRows[0].breastRating >= Appearance.breastCupInverse("D"))
                {
                    addButton(0, "Bath (F)", HaveAGirlBath).hint("Have a bath with girls.");
                }
                else
                {
                    addButtonDisabled(0, "Bath (F)", "You're too masculine to count as a female. Perhaps, if you have big enough rack, they'll reconsider?");
                }
                //boys
                ...
                {
                    addButtonDisabled(1, "Bath (M)", "You're too feminine. Maybe having a big enough dick to shadow them all will make them reconsider?");
                }
                if (flags[kFLAGS.CAMP_UPGRADES_HOT_SPRINGS] >= 5)
                {
                    addButton(2, "Peep", PeepingTom2).hint("Peep at bathing girls.");
    ...
    public function HaveABoysBath() : Void
    {
        ...
        {
            outputText("Jojo isn’t exactly what you would call muscular, but his size and lean frame fits his mouse appearance.\n\n");
        }
        if (player.hasStatusEffect(StatusEffects.CampRathazul))
        {
            outputText("You're almost glad you don’t see any part of Rathazul’s body below the hips. Though, you’re not particularly interested anyway.\n\n");
        }
        if (arianScene.arianFollower() && flags[kFLAGS.ARIAN_VAGINA] < 1 && flags[kFLAGS.ARIAN_COCK_SIZE] > 0)
        {
            outputText("Arian doesn’t seem to hate the water. He's likely cold blooded and is probably enjoying it.\n\n");
        }
        if (flags[kFLAGS.IZMA_BROFIED] == 1)
        {
            outputText("Since Izmael is a true guy now, he was allowed to bathe with the boys.\n\n");
        }
        if (flags[kFLAGS.KONSTANTIN_FOLLOWER] >= 2 && !player.hasStatusEffect(StatusEffects.KonstantinOff))
        {
            outputText("Sitting near you, Konstantin relaxes and stretches his limbs. His large and thick build dwarf those from the other guys. While chit-chatting, he friendly puts one arm around you, and with the other he shamelessly washes his gigantic manhood, earning no few stares and some blushes from the other boys.\n\n");
        }
        if (flags[kFLAGS.SIEGWEIRD_FOLLOWER] > 3)
        {
            ...
            if (camp.maleNpcsHotBathCount() > 1)
            {
                outputText(", he seems extremely nervous around all the other people");
            }
            outputText(". Maybe he's not one for the heat.\n\n");
        }
        if (flags[kFLAGS.ZENJI_PROGRESS] == 8 || flags[kFLAGS.ZENJI_PROGRESS] == 9)
        {
            outputText("Zenji rests in the hot spring, his short fur carried by the bubbling water.\n\n");
        }
        ...
        {
            outputText("Ember’s body and maleness could easily give all the other boys an inferiority complex.\n\n");
        }
    ...
    public function PeepingTom3() : Void
    {
        ...
        {
            outputText("Amily is sitting in the left corner. Naturally, she’s a little shy about her modest breasts.\n\n");
        }
        if (flags[kFLAGS.JOJO_BIMBO_STATE] == 3 && flags[kFLAGS.JOY_COCK_SIZE] < 1)
        {
            outputText("Joy is quite silent for once, you expected her to constantly blabber lewd comments about everyone's nudity.\n\n");
        }
        if (followerKiha())
        {
            outputText("Kiha definitely doesn’t have a size complex, with her huge breasts leaving most of the other girls in the shade.\n\n");
        }
        ...
        if (sophieFollower() && flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 0 && !player.hasStatusEffect(StatusEffects.SophieOff))
        {
            ...
            {
                outputText(", her daughter sitting at her side");
            }
            outputText(".\n\n");
        }
        if (followerHel() && !player.hasStatusEffect(StatusEffects.HeliaOff))
        {
            outputText("Helia is lazily sitting in the bath, not saying a word.\n\n");
        }
        if (CelessScene.instance.isCompanion() && CelessScene.instance.isAdult)
        {
            outputText(CelessScene.instance.Name + " is relaxing letting her equine half soak in the deeper watery area.\n\n");
        }
        if (flags[kFLAGS.KINDRA_FOLLOWER] >= 1)
        {
            outputText("Kindra’s wool doesn't seem to prevent her from enjoying the bath.\n\n");
        }
        if (flags[kFLAGS.ETNA_FOLLOWER] > 0 && !player.hasStatusEffect(StatusEffects.EtnaOff))
        {
            outputText("Etna isn’t far from your hiding spot, her tail tip covered with a towel for everyone else's safety.\n\n");
        }
        if (flags[kFLAGS.AYANE_FOLLOWER] >= 2)
        {
            outputText("Ayane is enjoying some time off duty in the other corner, tending to her fur.\n\n");
        }
        if (flags[kFLAGS.IZMA_FOLLOWER_STATUS] == 1 && flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 0)
        {
            if (flags[kFLAGS.IZMA_NO_COCK] == 0)
            {
                outputText("While not entirely a girl, Izma is womanly enough that she was still accepted in the bath, her crotch well hidden behind a cloth.\n\n");
            }
            else
            {
                outputText("While she was not always a girl, Izma is womanly enough that she was still accepted in the bath.\n\n");
            }
        }
        if (flags[kFLAGS.SAMIRAH_FOLLOWER] > 9)
        {
            outputText("As a naga, Samirah takes a lot of space in the bath. You're glad you made it big enough to hold as many people.\n\n");
        }
        if (flags[kFLAGS.NADIA_FOLLOWER] >= 6 && !player.hasStatusEffect(StatusEffects.NadiaOff))
        {
            outputText("Nadia is resting next to a set of medicinal and aromatic oils she regularly spray in the bath.\n\n");
        }
        //if (flags[kFLAGS.MICHIKO_FOLLOWER] >= 1) Michiko
        if (flags[kFLAGS.MITZI_RECRUITED] >= 4)
        {
            outputText("Your resident goblin Mitzi saunters over, looking excited. She strips what little clothing she wears then climbs into the warm waters. Her large tits help her stay afloat as she lays back and relaxes, letting out a sigh of relief.\n\n");
        }
        if (flags[kFLAGS.THE_TRENCH_ENTERED] > 14)
        {
            outputText("You can see Grayda staying close to you. However, she seems to be fighting the urge to fall asleep in the comfort of the warm water.\n\n");
        }
        if (flags[kFLAGS.EXCELLIA_RECRUITED] >= 33)
        {
            outputText("Excellia comes over to enjoy the soothing waters. She climbs in, letting out a content [exc moo] as she lays back letting the warm waters wash over her.\n\n");
        }
        if (flags[kFLAGS.LUNA_FOLLOWER] >= 4 && !player.hasStatusEffect(StatusEffects.LunaOff))
        {
            outputText("Luna is enjoying a break relaxing for once as she is not on duty" + ((flags[kFLAGS.LUNA_FOLLOWER] > 6) ? ", though for a few split second you imagined her doing doggy paddle in the water" : "") + ".\n\n");
        }
        if (arianScene.arianFollower() && flags[kFLAGS.ARIAN_VAGINA] > 0 && flags[kFLAGS.ARIAN_COCK_SIZE] == 0)
        {
            outputText("Arian while formerly a male seems to get along with the other girls.\n\n");
        }
    ...
    public function HaveAGirlBath() : Void
    {
        ...
        if (flags[kFLAGS.ALVINA_FOLLOWER] > 12)
        {
            outputText("Alvina does not seem to care about her breast size. Her dark charms and unholy attraction aura does everything for her. Naturally, nobody but you can see she’s there.\n\n");
        }
        if (player.hasStatusEffect(StatusEffects.CampMarble) && flags[kFLAGS.FOLLOWER_AT_FARM_MARBLE] == 0)
        {
            outputText("Marble easily has the largest pair here, beating all of the other girls without contest, They’re probably the milkiest too.\n\n");
        }
        if (isabellaFollower() && flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 0 && !player.hasStatusEffect(StatusEffects.SophieOff))
        {
            outputText("Isabella sings as she bathes, which is not surprising. You assume they’re folk songs from her home.\n\n");
        }
        if (sophieFollower() && flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE] == 0)
        {
            ...
            if (flags[kFLAGS.SOPHIE_DAUGHTER_MATURITY_COUNTER] > 0)
            {
                outputText(", her daughter sittings at her side");
            }
            outputText(".\n\n");
        }
        if (followerHel() && !player.hasStatusEffect(StatusEffects.HeliaOff))
        ...
        if (flags[kFLAGS.ETNA_FOLLOWER] > 0 && !player.hasStatusEffect(StatusEffects.EtnaOff))
        {
            outputText("Etna isn’t far from you, her tail tip covered with a towel for everyone else's safety.\n\n");
        }
        if (flags[kFLAGS.AYANE_FOLLOWER] >= 2)
        {
            ...
            {
                outputText(" and yours");
            }
            outputText(".\n\n");
        }
        if (flags[kFLAGS.SAMIRAH_FOLLOWER] > 9)
        {
            outputText("As a naga Samirah takes a lot of space in the bath. You're glad you made your pool as large as you could.\n\n");
        }
        if (flags[kFLAGS.NADIA_FOLLOWER] >= 6 && !player.hasStatusEffect(StatusEffects.NadiaOff))
        {
            outputText("Nadia is resting next to a set of medicinal and aromatic oils she regularly spray in the bath. Can’t say this is not welcome.\n\n");
        }
        //if (flags[kFLAGS.MICHIKO_FOLLOWER] >= 1) Michiko
        if (flags[kFLAGS.MITZI_RECRUITED] >= 4)
        {
        ...
        if (emberScene.followerEmber() && flags[kFLAGS.EMBER_GENDER] == 2)
        {
            ...
            if (player.hasStatusEffect(StatusEffects.CampMarble) && flags[kFLAGS.FOLLOWER_AT_FARM_MARBLE] == 0)
            {
                outputText(". You still can’t help but wonder how Marble can sit right next to her and act like everything is ok");
            }
            outputText(".\n\n");
        }
        if (amilyScene.amilyFollower() && !amilyScene.amilyCorrupt())
        {
            ...
            {
                outputText("she is off course the smallest");
            }
            else
            {
                outputText("you are of course the smallest");
            }
        ...
        if (flags[kFLAGS.FUCK_FLOWER_LEVEL] >= 4 || flags[kFLAGS.FLOWER_LEVEL] >= 4)
        {
            outputText("At some point during the orgy, you hear Holli yelling that it’s unfair she’s rooted in place at the other edge of the camp while everyone is enjoying themselves.\n\n");
        }
        if (player.hasStatusEffect(StatusEffects.CampRathazul))
        {
            outputText("You can hear a half muffled chuckle from behind the wall. You could swear it was Rathazul, but you’re sure the old rat wouldn’t stoop so low as to peep on the lot of you, right?\n\n");
        }
    ...
    public function HclassHTbeaten() : Void
    {
        ...
        {
            var BTB : Float = 1;
            if (player.statusEffectv1(StatusEffects.HeavenTribulationCR) > 1)
            {
                BTB += 2;
            }
            if (player.statusEffectv1(StatusEffects.HeavenTribulationCR) > 2)
            {
                BTB += 3;
            }
    ...
    public function FFclassHTbeaten() : Void
    {
        ...
        if (player.statusEffectv1(StatusEffects.HeavenTribulationCR) > 0)
        {
            ...
            if (player.statusEffectv1(StatusEffects.HeavenTribulationCR) > 5)
            {
                BTBa += 6;
            }
            if (player.hasPerk(PerkLib.BodyTempering))
    ...
    public function goblinsBirthScene(womb : Int = 0) : Void
    {
        ...
        if (daughtersCount > 1)
        {
            outputText("Each new baby you pop is a new orgasm and by the end of it your tongue is panting out from pleasure. ");
        }
    ...
    private function goblinsBirthScene2(womb : Int = 0) : Void
    {
        ...
        if (player.vaginas[womb].vaginalWetness == VaginaClass.WETNESS_DRY)
        {
    ...
    public function PCGoblinDaughters() : Void
    {
        ...
        if (flags[kFLAGS.PC_GOBLIN_DAUGHTERS] > 9 && player.hasStatusEffect(StatusEffects.PCDaughtersWorkshop))
        {
            ...
            {
                addButton(2, "Engineering", SceneLib.lumi.lumiWorkshop);
            }
            else
            {
                addButtonDisabled(2, "Engineering", "You need to be goblin for this.");
            }
            if (player.hasStatusEffect(StatusEffects.PCDaughtersWorkshopSpareParts))
            {
                addButton(3, "Spare parts", PCGoblinDaughtersBuilingWorkshopSpareParts);
            }
            else
            {
                addButtonDisabled(3, "Spare parts", "You need to wait till tomorrow for new spare parts.");
            }
        }
    ...
    public function PCGoblinDaughtersBuilingWorkshopSpareParts() : Void
    {
        ...
        if (player.statusEffectv2(StatusEffects.PCDaughtersWorkshopSpareParts) > 0)
        {
            outputText(", " + player.statusEffectv2(StatusEffects.PCDaughtersWorkshopSpareParts) + " metal pieces");
        }
        if (player.statusEffectv3(StatusEffects.PCDaughtersWorkshopSpareParts) > 0)
        {
            outputText(", " + player.statusEffectv3(StatusEffects.PCDaughtersWorkshopSpareParts) + " mechanism");
        }
        if (player.statusEffectv4(StatusEffects.PCDaughtersWorkshopSpareParts) > 0)
        {
            outputText(", " + player.statusEffectv4(StatusEffects.PCDaughtersWorkshopSpareParts) + " energy core");
        }
        outputText("</b>");
