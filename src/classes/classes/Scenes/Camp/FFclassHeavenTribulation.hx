...
class FFclassHeavenTribulation extends GclassHeavenTribulation
{
    override private function performCombatAction() : Void
    {
        ...
        if (player.statusEffectv1(StatusEffects.HeavenTribulationCR) >= 7)
        {
            HP = 0;
        }
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        if (player.statusEffectv1(StatusEffects.HeavenTribulationCR) >= 7)
        {
            campscene.FFclassHTsurvived();
        }
        else
        {
            campscene.FFclassHTbeaten();
        }
    }
    ...
    public function new()
    {
        ...
        this.long = "You're currently facing the FF class Heavenly Tribulation: A gathering of jet black clouds that sends down a wrathful storm to test any soul cultivator dreaming to break to Soul King stage.";
        this.plural = false;
