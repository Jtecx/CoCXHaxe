...
/**
	 ...
	 * @author Kitteh6660
	 */
class CabinProgress extends BaseContent
{
    ...
    public function initiateCabin() : Void
    {
        ...
        if (player.fatigue <= player.maxOverFatigue() - gatherWoodsORquarrySiteMineCost())
        {
            ...
            else if (flags[kFLAGS.CAMP_CABIN_PROGRESS] == 2)
            {
                startLayout();
            }
            else if (flags[kFLAGS.CAMP_CABIN_PROGRESS] == 3)
            {
                startThinkingOfMaterials();
            }
            else if (flags[kFLAGS.CAMP_CABIN_PROGRESS] == 4)
            {
                checkToolbox();
            }
            //For stage 4, explore forest.
            else if (flags[kFLAGS.CAMP_CABIN_PROGRESS] == 5)
            {
                startCabinPart2();
            }
            //Build cabin!
            else if (flags[kFLAGS.CAMP_CABIN_PROGRESS] == 6)
            {
                buildCabinPart1();
            }
            else if (flags[kFLAGS.CAMP_CABIN_PROGRESS] == 7)
            {
                buildCabinPart2();
            }
            else if (flags[kFLAGS.CAMP_CABIN_PROGRESS] == 8)
            {
                buildCabinPart3();
            }
            else if (flags[kFLAGS.CAMP_CABIN_PROGRESS] == 9)
            {
                buildCabinPart4();
            }
            //Furnish your cabin!
            else if (flags[kFLAGS.CAMP_CABIN_PROGRESS] == 10)
            {
                enterCabinFirstTime();
            }
        }
    ...
    private function startThinkingOfMaterials() : Void
    {
        ...
        else if (player.hasKeyItem("Carpenter's Toolbox") >= 0)
        {
    ...
    public function canGatherWoods() : Bool
    {
        return (player.weapon == weapons.L__AXE || player.weapon == weapons.DL_AXE_ || player.weapon == weapons.MACGRSW || player.weapon == weapons.TMACGRSW || player.weapon == weapons.RIPPER1 || player.weapon == weapons.TRIPPER1 || player.weapon == weapons.RIPPER2 || player.weapon == weapons.TRIPPER2 || player.hasKeyItem("Carpenter's Toolbox") >= 0 || player.isInGoblinMech()) && CampStatsAndResources.WoodResc < SceneLib.campUpgrades.checkMaterialsCapWood() && player.statusEffectv1(StatusEffects.ResourceNode1) < 5;
    }
    //STAGE 4 - Gather woods, explore forest to encounter.
    public function gatherWoods() : Void
    {
        clearOutput();
        if (player.hasStatusEffect(StatusEffects.ResourceNode1) && player.statusEffectv1(StatusEffects.ResourceNode1) >= 5)
        {
            outputText("You return to the forest where you found a good thick trees ready to be cut down.");
        }
        else
        {
        ...
        {
            if (player.statusEffectv1(StatusEffects.ResourceNode1) == 4)
            {
                outputText("You have found this type of logging area enough times to be able to find them in the future without trouble. ('Woodcutting' option has been unlocked in Places menu)\n\n");
            }
    ...
    private function cutTreeTIMBER() : Void
    {
        ...
        {
            outputText("You ready your oversized axe. ");
        }
        else if (player.weapon == weapons.DL_AXE_)
        {
            outputText("You ready your oversized axes. ");
        }
        else
        {
            outputText("You ready your axe. ");
        }
        ...
        var cTT : Float = (10 + ((player.strStat.core.value + player.strStat.train.value) * 4));
        if (player.weapon == weapons.DL_AXE_)
        {
            cTT = Math.round(cTT * 1.5);
        }
    ...
    private function cutTreeMechTIMBER() : Void
    {
        ...
        else if (player.weapon == weapons.RIPPER2)
        {
            outputText("Ripper 2.0");
        }
        else if (player.weapon == weapons.TRIPPER2)
        {
            outputText("Twin Ripper 2.0");
        }
        else if (player.weapon == weapons.RIPPER1)
        {
            outputText("Ripper 1.0");
        }
        else if (player.weapon == weapons.TRIPPER1)
        {
            outputText("Twin Ripper 1.0");
        }
        else if (player.weapon == weapons.TMACGRSW)
        {
            outputText("chainsaw swords");
        }
        else
        {
            outputText("chainsaw sword");
        }
        outputText(" as the metal teeth begin to spin. ");
        if (player.isInGoblinMech())
        {
            outputText("Pressing the joystick on your command board you move ");
        }
        else
        {
            outputText("Grabbing a hold of the handle, you press ");
        }
        ...
        {
            var cTMT : Float = (13 + ((player.strStat.core.value + player.strStat.train.value) * 4));
            if (player.weapon == weapons.TMACGRSW || player.weapon == weapons.TRIPPER1 || player.weapon == weapons.TRIPPER2)
            {
                cTMT = Math.round(cTMT * 1.5);
            }
    ...
    {
        clearOutput();
        if (player.hasStatusEffect(StatusEffects.ResourceNode1) && player.statusEffectv2(StatusEffects.ResourceNode1) >= 5)
        {
            outputText("You return to the mountain area where you found before a very good mineral formation.");
        }
        else
        {
        ...
        {
            outputText(" Next to it is an old pickaxe and leather bag with runic engraving left by a native who likely met an unfortunate end.");
        }
        if (!player.hasStatusEffect(StatusEffects.ResourceNode1))
        {
            player.createStatusEffect(StatusEffects.ResourceNode1, 0, 0, 0, 0);
        ...
        {
            if (player.statusEffectv2(StatusEffects.ResourceNode1) == 4)
            {
                outputText("You have found this quarry enough times to be able to find it in the future without trouble. ('Quarry' option has been unlocked in Places menu)\n\n");
            }
        ...
        else if (model.time.hours <= 5 || model.time.hours >= 21)
        {
            addButton(0, "Mine (N)", quarrySiteMine, true);
        }
        else
        {
            addButton(0, "Mine (D)", quarrySiteMine).hint("Some ores might be available only at night.");
        }
    ...
    private function gatherWoodsORquarrySiteMineCost() : Float
    {
        var fatigueAmount : Float = 50;
        if (player.hasPerk(PerkLib.MummyLord) && player.perkv1(PerkLib.MummyLord) > 0)
        {
            ...
            {
                fatigueAmount = Math.round(50 * (1 / player.perkv1(PerkLib.MummyLord)));
            }
        }
        return fatigueAmount;
    }
    private function quarrySiteMine(nightExploration : Bool = false) : Void
    {
        if (player.fatigue > player.maxOverFatigue() - gatherWoodsORquarrySiteMineCost())
        {
        ...
        var minedStones : Float = 13 + ((player.strStat.core.value + player.strStat.train.value) * 4);
        fatigue(gatherWoodsORquarrySiteMineCost(), USEFATG_PHYSICAL);
        if (minedStones > (40 + (2 * player.miningLevel) + (20 * player.newGamePlusMod())))
        {
            minedStones = (40 + (2 * player.miningLevel) + (20 * player.newGamePlusMod()));
        }
        flags[kFLAGS.ACHIEVEMENT_PROGRESS_YABBA_DABBA_DOO] += minedStones;
        incrementStoneSupply(minedStones);
        var mineEXP : Float = player.MiningMulti();
        player.mineXP(mineEXP);
        if (rand(10) == 0)
        {
            var gemsMined : Float = 1 + rand(1 + player.miningLevel);
    ...
    private function findOre(nightExploration : Bool = false) : Void
    {
        if (player.miningLevel > 0)
        {
            ...
            {
                var itype : ItemType;
                var ore : Float = 2;  //0 = copper, 1 = tin, 2 = iron, 3 = moonstone  
                if (player.miningLevel > 1)
                {
                    ore += 1;
                }
                if (player.miningLevel > 2 && nightExploration)
                {
                    ore += 1;
                }
                var choice : Float = rand(ore);
                switch (choice)
                {
                    case 0:
    ...
    private function getHelpFromKiha() : Void
    {
        ...
        if (player.str < 33)
        {
            outputText("It's a daunting task as you can only carry few of the wood at a time. Even Kiha is far superior to your carrying capacity as she can carry a lot of wood.");
        }
        if (player.str >= 33 && player.str < 66)
        {
            outputText("It's quite the chore. Though you can carry several pieces of wood at a time, Kiha is still superior to you when it comes to carrying wood.");
        }
        if (player.str >= 66)
        {
            outputText("You easily tackle the task of carrying wood. You even manage to carry five pieces of wood at a time!");
        }
    ...
    private function noMoreSrt() : Float
    {
        var nMS : Float = player.str;
        if (nMS > 10)
        {
            nMS = 10;
        }
    ...
    private function doCabinWork1() : Void
    {
        ...
        {
            outputText("\"<i>My, my. What are you building?</i>\" Rathazul asks. \n\n");
        }
        if (player.hasStatusEffect(StatusEffects.PureCampJojo))
        {
            outputText("\"<i>You're building something?</i>\" Jo" + ((flags[kFLAGS.JOJO_BIMBO_STATE] == 3) ? "y" : "jo") + " asks. \n\n");
        }
        if (camp.marbleFollower())
        {
            outputText("\"<i>Sweetie, you're building a cabin? That's nice,</i>\" Marble says. \n\n");
        }
        if (camp.companionsCount() > 0)
        {
            outputText("You announce that yes, you're building a cabin.\n\n");
        }
        ...
        if (player.canFly() && player.str >= 80)
        {
            outputText("You use your wings, lift the roof frame and carefully place it on the frame. ");
        }
        else
        {
            outputText("You construct a temporary ramp to push the roof frame into place. ");
        }
        ...
        {
            outputText("Your lone camp follower comes to see what you've been working on. They nod in approval, impressed by your handiwork.");
        }
        else if (camp.companionsCount() > 1)
        {
            outputText("Your camp followers come to see what you've built so far. Most of them are even impressed.");
        }
