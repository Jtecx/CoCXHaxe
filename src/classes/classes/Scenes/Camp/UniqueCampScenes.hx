...
class UniqueCampScenes extends BaseContent
{
    
    public function new()
    {
        super();  //flags[kFLAGS.ALRAUNE_GROWING] == 15 - 2 weeks old children  
    ...
    public function manageyourgarden() : Void
    {
        ...
        {
            addButton(0, "Play", playwithyourplantdaughters);
        }
        if (flags[kFLAGS.ALRAUNE_DAILY_WATERING] < 1)
        {
            addButton(9, "Water", wateryourgarden);
        }
    ...
    public function wateryourgarden() : Void
    {
        ...
        if (flags[kFLAGS.ALRAUNE_GROWING] < 22)
        {
    ...
    private function impTomeGiveIn() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(" wraps around your [cocks] jerking you off vigorously");
        }
        outputText((player.hasCock() && player.hasVagina()) ? " while another" : "");
        if (player.hasVagina())
        {
            outputText(" inserts itself into your welcoming pussy, rushing past your cervix as it causes your stomach to bulge with the shape of the foreign flesh ravaging your insides");
        }
        ...
        if (player.hasCock())
        {
            outputText(" servicing your cock as you order them to bring the next bitch to break");
        }
        outputText((player.hasCock() && player.hasVagina()) ? ", while a tied up manslut serves as your personnal throne" : "");
        if (player.hasVagina())
        {
            outputText(" fucking your fertile imp cunt to birth the new legion of servants obediant to your royal will");
        }
