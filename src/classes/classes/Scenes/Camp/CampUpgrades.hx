...
class CampUpgrades extends BaseContent
{
    
    public static function builtAnything() : Bool
    {
        return flags[kFLAGS.CAMP_CABIN_PROGRESS] >= 10 || flags[kFLAGS.MATERIALS_STORAGE_UPGRADES] >= 3 || flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] >= 2 || flags[kFLAGS.CAMP_UPGRADES_FISHERY] >= 1 || flags[kFLAGS.CAMP_WALL_PROGRESS] >= 100 || flags[kFLAGS.CAMP_UPGRADES_HOT_SPRINGS] >= 4 || flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] >= 2;
    }
    ...
    */
    ...
    {
        menu();
        if (flags[kFLAGS.MATERIALS_STORAGE_UPGRADES] == 1 || flags[kFLAGS.MATERIALS_STORAGE_UPGRADES] == 2)
        {
            addButton(0, "Wood Storage", materialgatheringstorageupgrade).hint("Build up storage to gather more wood at the camp. (Req. " + usedFatigue(150, true) + " fatigue)");
        }
        if (flags[kFLAGS.MATERIALS_STORAGE_UPGRADES] == 3)
        {
            addButton(0, "Stone Storage", materialgatheringstorageupgrade).hint("Build up storage to gather more stones at the camp. (Req. " + usedFatigue(150, true) + " fatigue)");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 0 || flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 1)
        {
            addButton(1, "1st Warehouse", warehousegranary).hint("Build 1st part of the Warehouse to expand your storage space. (Req. " + usedFatigue(250, true) + " fatigue)");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 2 || flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 3)
        {
            addButton(1, "Granary", warehousegranary).hint("Build Granary to expand your food space. (Req. " + usedFatigue(250, true) + " fatigue)");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 4 || flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 5)
        {
            addButton(1, "2nd Warehouse", warehousegranary).hint("Build 2nd part of the Warehouse to expand your storage space. (Req. " + usedFatigue(250, true) + " fatigue)");
        }
        if ((player.isRace(Races.KITSUNE, 1, false) || player.isRace(Races.KITSHOO, 1, false)) && (flags[kFLAGS.CAMP_UPGRADES_KITSUNE_SHRINE] < 1 || flags[kFLAGS.CAMP_UPGRADES_KITSUNE_SHRINE] == 1 || flags[kFLAGS.CAMP_UPGRADES_KITSUNE_SHRINE] == 2))
        {
            addButton(2, "Shrine", kitsuneshrine).hint("Build up kitsune shrine at the camp. (Req. " + usedFatigue(200) + "~" + usedFatigue(300) + " fatigue)");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_KITSUNE_SHRINE] == 3)
        {
            if (!(player.hasItem(useables.GLDSTAT)))
            {
                addButtonDisabled(2, "Shrine", "You need to have Kitsune Statue and your own Star Sphere to finish the shrine!");
            }
            if (!player.hasPerk(PerkLib.StarSphereMastery))
            {
                addButtonDisabled(2, "Shrine", "You need to have Kitsune Statue and your own Star Sphere to finish the shrine!");
            }
            if (player.hasPerk(PerkLib.StarSphereMastery) && player.hasItem(useables.GLDSTAT))
            {
                addButton(2, "Shrine", kitsuneshrine2).hint("Finish up kitsune shrine at the camp.");
            }
        }
        if (flags[kFLAGS.CAMP_UPGRADES_HOT_SPRINGS] == 2 || flags[kFLAGS.CAMP_UPGRADES_HOT_SPRINGS] == 3)
        {
            addButton(3, "Hot Spring", hotspring).hint("Build up hot spring at the camp. (Req. " + usedFatigue(100, true) + " fatigue)");
        }
        ...
        //if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 7) addButton(4, "Sparring Ring", sparringRing).hint("Expand your stone sparing ring to collosal size. (Decrease npc's training time by 1/2 and increase exp from using training dummy by 500% more (1500% of base amount))(Req. "+usedFatigue(12150, true)+" fatigue)");{
            ...
            {
                addButton(4, "Sparring Ring", sparringRing).hint("Add wood floor to sparring ring for better training experience. (Decrease npc's training time by 1/4 and increase exp from using training dummy by 250% more (700% of base amount))(Req. " + usedFatigue(1350, true) + " fatigue)");
            }
            if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 4)
            {
                addButton(4, "Sparring Ring", sparringRing).hint("Expand sparring ring to massive size. (Decrease npc's training time by 1/5 and increase exp from using training dummy by another 200% (450% of base amount))(Req. " + usedFatigue(450, true) + " fatigue)");
            }
            if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 3)
            {
                addButton(4, "Sparring Ring", sparringRing).hint("Expand sparring ring to large size. (Decrease npc's training time by 1/6 and increase exp from using training dummy by 150% (250% of base amount))(Req. " + usedFatigue(150, true) + " fatigue)");
            }
            if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] < 2)
            {
                addButton(4, "Sparring Ring", sparringRing).hint("Build up sparring ring at the camp. (Unlock sparring option for all camp members that have this option)(Req. " + usedFatigue(50, true) + " fatigue)");
            }
        }
        ...
        {
            addButton(6, "Elemental E. C.", arcaneCircleUpgrade).hint("Add Elemental Energy Conduits to your arcane circle to store in them elemental energy stored in elementals shards for more easy use. (Allowing to replace mana and reduce fatigue usage when summoning/ranking up normal tier elementals. Allowing to rank up elementals of tiers above normal tier.)(Req. 50 fatigue, enough stones, mana and elemental shards)");
        }
        if (player.inte >= 50 && flags[kFLAGS.CAMP_UPGRADES_MAGIC_WARD] == 1)
        {
            addButton(7, "Magic Ward", magicWard).hint("Set up a Magic Ward around the camp. (Req. 200 fatigue)");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_DAM] < 3)
        {
            addButton(8, "Dam", dam).hint("Build up a dam on the stream next to the camp. (Req. " + usedFatigue(200, true) + " fatigue * tier of built dam)");
        }
        if (flags[kFLAGS.CAMP_UPGRADES_DAM] >= 1 && flags[kFLAGS.CAMP_UPGRADES_FISHERY] < 2)
        {
            addButton(9, "Fishery", fishery).hint("Build up a fishery on the stream next to the camp. (Req. " + usedFatigue(200, true) + " fatigue)");
        }
    ...
    public function checkMaterialsCapNails() : Float
    {
        var cMC1 : Float = 250;
    ...
    public function checkMaterialsCapWood() : Float
    {
        var cMC1 : Float = 400;
        if (flags[kFLAGS.MATERIALS_STORAGE_UPGRADES] >= 3)
        {
    ...
    public function checkMaterialsCapStones() : Float
    {
        var cMC1 : Float = 400;
    ...
    private function getHelpers() : Dynamic
    {
        var helpers : Dynamic = {
            count : 0,
            names : ""
    ...
    private function buildWithHelpers(baseFatigue : Int, mummies : Bool = false, timeWithout : Int = 4, timeWithOne : Int = 2, timeWithMore : Int = 1) : Void
    {
        var formatTime : Int->String = function(hrs : Int) : String
        {
            return (hrs > 1) ? NUMBER_WORDS_NORMAL[hrs] + " hours" : "one hour";
        }
        
        var helpers : Dynamic = getHelpers();
    ...
    private function useFatigue(base : Int, helpersCount : Int = 0, mummies : Bool = false) : Void
    {
        ...
        fatigueAmount /= as3hx.Compat.parseInt(helpersCount + 1);
        ...
        {
            if (player.perkv1(PerkLib.MummyLord) > 19)
            {
                fatigueAmount = 0;
            }
            else
            {
                fatigueAmount = Math.round(fatigueAmount * (1 / player.perkv1(PerkLib.MummyLord)));
            }
        }
        fatigue(fatigueAmount);
    }
    private function usedFatigue(base : Int, mummies : Bool = false) : Float
    {
        var baseFatigue : Float = base;
        var helpers : Dynamic = getHelpers();
        ...
        if (mummies && player.hasPerk(PerkLib.MummyLord) && player.perkv1(PerkLib.MummyLord) > 0)
        {
            ...
            {
                baseFatigue = Math.round(baseFatigue * (1 / player.perkv1(PerkLib.MummyLord)));
            }
        }
        return baseFatigue;
    }
    ...
    public function warehousegranary() : Void
    {
        ...
        if (player.fatigue <= player.maxOverFatigue() - usedFatigue(250, true))
        {
            ...
            else if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 1)
            {
                startWarehouse2();
            }
            else if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 2)
            {
                startGranary1();
            }
            else if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 3)
            {
                startGranary2();
            }
            else if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 4)
            {
                start2ndWarehouse1();
            }
            else if (flags[kFLAGS.CAMP_UPGRADES_WAREHOUSE_GRANARY] == 5)
            {
                startWarehouse2();
            }
        }
    ...
    public function kitsuneshrine() : Void
    {
        ...
        else if (player.fatigue <= player.maxOverFatigue() - usedFatigue(300) && flags[kFLAGS.CAMP_UPGRADES_KITSUNE_SHRINE] == 1)
        {
            buildStructure();
        }
        else if (player.fatigue <= player.maxOverFatigue() - usedFatigue(200) && flags[kFLAGS.CAMP_UPGRADES_KITSUNE_SHRINE] == 2)
        {
            buildAltar();
        }
        else
    ...
    {
        clearOutput();
        if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] == 1 && player.fatigue <= player.maxOverFatigue() - usedFatigue(50, true))
        {
            buildSmallRing();
        }
        else if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] == 2 && player.fatigue <= player.maxOverFatigue() - usedFatigue(150, true))
        {
            buildLargeRing();
        }
        else if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] == 3 && player.fatigue <= player.maxOverFatigue() - usedFatigue(450, true))
        {
            buildMassiveRing();
        }
        else if (flags[kFLAGS.CAMP_UPGRADES_SPARING_RING] == 4 && player.fatigue <= player.maxOverFatigue() - usedFatigue(1350, true))
        {
            buildRingWoodFloor();
        }
        else
    ...
    {
        var stoneCost : Int = as3hx.Compat.parseInt(newTier * 4);
        var hpCost : Int = as3hx.Compat.parseInt(75 * newTier);
        var manaCost : Int = as3hx.Compat.parseInt(100 * newTier);
    ...
    public function arcaneCircleUpgrade() : Void
    {
        ...
        if (player.fatigue <= player.maxOverFatigue() - 50)
        {
            ...
            {
                var currentTier : Int = !(player.hasStatusEffect(StatusEffects.ElementalEnergyConduits)) ? 0 : player.statusEffectv3(StatusEffects.ElementalEnergyConduits) / 2;
                var upgrade : Bool = player.hasStatusEffect(StatusEffects.ElementalEnergyConduits)
                ...
                {
                    buildOrUpgradeElementalEnergyConduit(currentTier, upgrade);
                }
            }
            else
            {
    ...
    public function buildOrUpgradeElementalEnergyConduit(currentTier : Int, upgrade : Bool) : Void
    {
        var stoneShardCost : Int = as3hx.Compat.parseInt(3 + currentTier);
        var manaCost : Int = as3hx.Compat.parseInt(150 + currentTier * 50);
    ...
    {
        var stoneShardCost : Int = as3hx.Compat.parseInt(3 + currentTier);
        var manaCost : Int = as3hx.Compat.parseInt(300 + currentTier * 100);
        var capacityIncrease : Int = as3hx.Compat.parseInt(900 + currentTier * 300);
        clearOutput();
        if (currentTier == 0)
        {
    ...
    {
        clearOutput();
        if (player.fatigue <= player.maxOverFatigue() - 200 && flags[kFLAGS.CAMP_UPGRADES_MAGIC_WARD] == 1)
        {
            setUpMagicWard();
        }
        else
    ...
    {
        clearOutput();
        if (flags[kFLAGS.CAMP_UPGRADES_DAM] < 1 && player.fatigue <= player.maxOverFatigue() - usedFatigue(200, true))
        {
            buildUpMinorWoodDam();
        }
        else if (flags[kFLAGS.CAMP_UPGRADES_DAM] == 1 && player.fatigue <= player.maxOverFatigue() - usedFatigue(400, true))
        {
            buildUpWoodDam();
        }
        else if (flags[kFLAGS.CAMP_UPGRADES_DAM] == 2 && player.fatigue <= player.maxOverFatigue() - usedFatigue(600, true))
        {
            buildUpMajorWoodDam();
        }
