...
class Achievements extends BaseContent
{
    ...
    public function addAchievement(title : String, achievement : Dynamic, descLocked : String, descUnlocked : String = "", isShadow : Bool = false) : Void
    //If not specified, default to the locked description.
    {
        ...
        if (isShadow && Reflect.field(achievements, Std.string(achievement)) <= 0)
        {
            return;
        }
        //Set text.
        ...
        if (Reflect.field(achievements, Std.string(achievement)) > 0)
        {
            if (!silent)
            {
                outputText("[font-green]Unlocked[/font] - " + descUnlocked);
            }
        ...
        {
            outputText("[font-dred]Locked[/font] - " + descLocked);
        }
        if (isShadow)
        {
            if (!silent)
            {
                outputText(" - [font-dark]Shadow Achievement[/font]");
            }
            achievementsTotalCurrentlyUnlocked++;
    ...
    public function checkAll(silent : Bool = false) : Void
    {
        this.silent = silent;
