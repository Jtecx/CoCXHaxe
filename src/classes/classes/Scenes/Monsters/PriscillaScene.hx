...
class PriscillaScene extends BaseContent
{
    public function goblinElderEncounter() : Void
    {
        var canFuck : Bool = player.cor < 60 + player.corruptionTolerance;
        ...
        if (flags[kFLAGS.TIMES_ENCOUNTERED_PRISCILLA] == 0)
        {
            ...
            if (player.hasCock())
            {
                if (canFuck)
                {
                    outputText("\"<i>Hey, my name is Priscilla. Would you like to fuck me?  I'll promise to teach my newborn goblins the values of pre-corruption lifestyle.</i>\"");
                }
                else
                {
                    outputText("\"<i>Please don't fuck me!  You're too corrupt!</i>\"");
                }
            }
            else
            {
                outputText("\"<i>You don't have anything to knock me up but I'm always up for the challenge.</i>\"");
            }
        }
        ...
        else
        {
            ...
            if (player.hasCock())
            {
                if (canFuck)
                {
                    outputText("\"<i>Hello again" + ((player.hasCock()) ? ", " + player.mf("stud", "slut") + ".  Ready to knock me up with your cum?  Or are you here to duel?" : ",") + "</i>\"");
                }
                else
                {
                    outputText("\"<i>Please don't fuck me!  You're too corrupt!</i>\"");
        ...
        if (flags[kFLAGS.PRISCILLA_TALK_COUNTER] < 2)
        {
            addButton(1, "Talk", talkToGoblinElder);
        }
        if (player.hasVagina() && player.lust >= 33 && canFuck)
        {
            addButton(2, "Sex Anyway", startffaction);
            if (flags[kFLAGS.PRISCILLA_AFFECTION] >= 3 || player.cockThatFits(monster.vaginalCapacity()) >= 0)
            {
                addButton(2, "Sex", startforeplay);
            }
        }
    ...
    public function priscillaDefeated() : Void
    {
        ...
        {
            if (flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.PRISCILLA_DEFEATS_COUNTER] = 1;
            }
    ...
    {
        var canHelp : Bool = player.hasItem(consumables.PURHONY, 1) || player.hasItem(consumables.PPHILTR, 1);
        clearOutput();
        ...
        if (canHelp)
        {
            outputText(" and you just happen to have the right item.  Should you give it to her?");
        }
        menu();
    ...
    private function scissorscene() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("Your " + player.cockDescript(0) + " flops noisily on you and Priscilla, smearing and shooting pre everywhere.  ");
        }
        ...
        if (player.hasCock())
        {
            outputText("and your " + player.cockDescript(0) + " jerking you as hard as she could");
        }
    ...
    private function startforeplay() : Void
    {
        ...
        if (flags[kFLAGS.PRISCILLA_FOREPLAY_COUNTER] > 0 || flags[kFLAGS.PRISCILLA_AFFECTION] >= 3)
        {
            ...
            if (player.hasVagina())
            {
                ...
                {
                    addButton(2, "Lesbi?", hermF_noFP).hint("Suggest having fun without using your dick.");
                }
            }
        }
        ...
        var hermF : Void->Void = function() : Void
        {
            ...
            {
                outputText("kneel down and scoop Priscilla into your arms for a kiss.  ");
            }
            else
            {
                outputText("pull Priscilla into a kiss.  ");
            }
        ...
        var dickF_sub : Void->Void = function() : Void
        {
            ...
            if (player.tallness >= 60)
            {
                outputText("You kneel down and scoop Priscilla into your arms for a kiss");
            }
            else
            {
                outputText("You pull Priscilla into a kiss");
            }
        ...
        var dickF_dom : Void->Void = function() : Void
        {
        ...
        {
            if (flags[kFLAGS.PRISCILLA_AFFECTION] < 4)
            {
                vagF_first();
            }
            else
        ...
        var vagF_first : Void->Void = function() : Void
        {
        ...
        var vagF_nice : Void->Void = function() : Void
        {
        ...
        var sexMenu_dick : Void->Void = function() : Void
        {
            menu();
        ...
        var sexMenu_vag : Void->Void = function() : Void
        {
            menu();
    ...
    private function unleashthekraken() : Void
    {
        ...
        {
            outputText("You ask Priscilla how she learned to call the beasts as you feel a tentacle going through your hands and arms, binding.  \"<i>Goblin sex hunters.</i>\" Priscilla said as the beast bound her as well.  \"<i>4 of them at a time would gather around, blow a beast whistle, and rape whatever comes.</i>\"  ");
        }
        ...
        {
            outputText("\"<i>Yes!  Here it comes!</i>\"  Priscilla yells, a look of mania in her eyes.  You close your eyes, anticipating the tentacles, coming for you and for Priscilla.  ");
        }
        ...
        if (player.hasCock())
        {
            outputText("your " + player.cockDescript(0) + " instantly goes erect, the pain was excruciating, but exciting.  ");
        }
        ...
        if (player.hasCock())
        {
            outputText("your " + player.cockDescript(0) + " starts cumming everywhere, shooting your nut on to the tentacles and all over Priscilla’s face, ");
        }
    ...
    private function dompriscilla() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("you start pulling on " + player.cockDescript(0) + " slowly, getting it hard.");
        }
        ...
        if (player.hasCock())
        {
            outputText("kneeling down you slowly push your " + player.cockDescript(0) + " in front of Priscilla’s face, who immediately opens her mouth, ready for meat any time-however with a chuckle, you pull away.  ");
        }
    ...
    private function priscilladom() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("Your " + player.cockDescript(0) + " twirls around, aching for contact, but whether Priscilla didn’t care, or didn’t want you to enjoy this (too late), she didn’t touch you.  ");
        }
        ...
        if (player.hasCock())
        {
            outputText("At this, your cock becomes fully erect, the clean but lewd taste of her asshole exciting you.  ");
        }
    ...
    private function pussyfuck() : Void
    {
        ...
        if (player.cumQ() >= 750)
        {
            outputText("Amidst your nipple suckling, you watched as Priscilla’s stomach started to slowly inflate as if she already was pregnant, and you felt a large gush of your flow squeeze itself out of Priscilla’s pussy onto your balls.");
        }
        ...
        if (player.cumQ() >= 750)
        {
            outputText("\"<i>You’re such an amazing breeder baby!  Maybe so good, I shouldn’t let you go...</i>\"");
        }
        else
        {
            outputText("\"<i>Stay safe out there, " + player.mf("stud", "slut") + ".  I’m going to need more of that sticky cream later.  Find me again okay?</i>\"");
        }
    ...
    private function assfuck() : Void
    {
        ...
        {
            outputText("Slamming her into your meat, you took delight not only in her tightness, but at how her stomach bulged out when forced to take your cock.  ");
        }
        ...
        if (player.cumQ() >= 750)
        {
            outputText("a pure white flow of your baby jism shamelessly torrents out of Priscilla’s gaping brown star, a hole which doesn’t look like it’s going to fully close any time soon.");
        }
        else
        {
            outputText("a thin trail of your man milk leaks subtly out of Priscilla’s quivering but closing asshole.  ");
        }
    ...
    private function mouthfuck() : Void
    {
        ...
        if (player.cocks[x].cockLength > 14)
        {
            outputText("\"<i>So much meat...And it’s all mine!</i>\"  ");
        }
        else
        {
            outputText("\"<i>Oh, this is going to be a treat.</i>\"  ");
        }
