...
class AngelLR extends AbstractAngel
{
    ...
    private function angelSwitchWithOtherOne() : Void
    {
        ...
        if (hasStatusEffect(StatusEffects.StunnedTornado))
        {
            removeStatusEffect(StatusEffects.StunnedTornado);
        }
    ...
    private function AngelBaseAttack() : Void
    {
        ...
        var damage : Float = eBaseWisdomDamage();
    ...
    override private function performCombatAction() : Void
    {
        if (hasStatusEffect(StatusEffects.TrueFormAngel))
        {
            if (!hasStatusEffect(StatusEffects.ATranscendentSoulField) && soulforce >= (soulfieldsustaincost() * 2))
            {
                createStatusEffect(StatusEffects.ATranscendentSoulField, soulfieldsustaincost() * 2, soulfieldsustaincost() * 2, 0, 0);
            }
            if (hasStatusEffect(StatusEffects.ATranscendentSoulField))
            {
                if (soulforce >= (soulfieldsustaincost() * 2))
                {
                    soulforce -= (soulfieldsustaincost() * 2);
                }
                else
                {
                    removeStatusEffect(StatusEffects.ATranscendentSoulField);
                }
            }
        }
        else
        {
            if (!hasStatusEffect(StatusEffects.ATranscendentSoulField) && soulforce >= soulfieldsustaincost())
            {
                createStatusEffect(StatusEffects.ATranscendentSoulField, soulfieldsustaincost(), soulfieldsustaincost(), 0, 0);
            }
            if (hasStatusEffect(StatusEffects.ATranscendentSoulField))
            {
                if (soulforce >= soulfieldsustaincost())
                {
                    soulforce -= soulfieldsustaincost();
                }
                else
                {
                    removeStatusEffect(StatusEffects.ATranscendentSoulField);
    ...
    override public function postPlayerBusyBtnSpecial(btnSpecial1 : CoCButton, btnSpecial2 : CoCButton) : Void
    {
        ...
        {
            if (!player.hasStatusEffect(StatusEffects.MinoKing) && player.companionsInPCParty())
            {
                btnSpecial1.show("Dish Helper", SceneLib.hexindao.dishHelperIL);
            }
            else
            {
                btnSpecial1.showDisabled("Dish Helper", "You don't have anyone to take care of second angel!");
            }
        }
    }
    private function soulfieldsustaincost() : Float
    {
        var sfsc : Float = 10;
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (player.hasStatusEffect(StatusEffects.RiverDungeonA))
        {
            floor4.defeatedByLowRankMistAngel();
        }
        else
        {
            cleanupAfterCombat();
    ...
    override private function get_long() : String
    {
        ...
        if (hasStatusEffect(StatusEffects.TrueFormAngel))
        {
            str += " purple irises each. Around each of their bodies are two circles each with pair of eyes looking the same as the main eye and above angel head hoover halo.";
        }
        else
        {
            str += " irises each, one red and the other blue. Angel eyes also mirrors this with right one been red and left blue one.";
        }
        if (player.hasStatusEffect(StatusEffects.RiverDungeonA))
        {
            str += " All three of them have their bodies covered in nearly not visible purple lines that glows ocassionaly.";
        }
    ...
    public function new()
    {
        ...
        if (player.hasStatusEffect(StatusEffects.RiverDungeonA))
        {
            ...
            this.drop = new ChainedDrop().add(useables.SRESIDUE, 0.5).add(useables.PCSHARD, 1);
            this.level = 42;
            this.bonusHP = 3000;
            this.bonusLust = 166;
            this.bonusWrath = 700;
        ...
        else if (player.hasStatusEffect(StatusEffects.SoulArena))
        {
            ...
            this.drop = new ChainedDrop().add(useables.SRESIDUE, 1);
            this.level = 9;
            this.bonusHP = 400;
            this.bonusLust = 36;
            this.bonusWrath = 100;
