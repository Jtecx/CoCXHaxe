...
class IvorySuccubusScene extends BaseContent
{
    ...
    public function winAgainstSuccubusRape() : Void
    {
        ...
        {
            outputText("s. When you feel at least one of them sink in a bit, you thrust in her hard, the others splaying out in all directions. The succubus screams at you, obviously violated. But that only serves to drive you on further, fucking her faster and harder. One of your cocks slips out, slathered in her own juices and someone else's cum, and another takes its place.\n\n");
        }
        else
        {
            outputText(" When you feel yourself sink in a bit, you thrust in her hard. The succubus screams at you, obviously violated. But that only serves to drive you on further, fucking her faster and harder.\n\n");
        }
        ...
        if (player.cockTotal() > 1)
        {
            outputText("She only wears herself out trying to wriggle away from you, and you use the chance to carefully cram another of your cocks into her. You can feel her getting stretched out. ");
        }
    ...
    public function loseToSuccubus(fromBattle : Bool = true) : Void
    {
        clearOutput();
        if (fromBattle)
        {
            outputText("Too badly " + ((player.HP <= 0) ? "beaten" : "aroused") + " by the succubus, you give in and let the succubus do whatever she wants to you.\n\n");
        }
        dynStats("cor", 3);
        player.addCurse("lib", 2, 2);
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            explorer.stopExploring();
            inventory.takeItem(consumables.INCUBID, cleanupAfterCombat);
        }
    }
