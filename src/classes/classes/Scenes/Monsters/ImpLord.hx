...
class ImpLord extends Imp
{
    ...
    //Special Attack 1
    private function impFire() : Void
    //[-HP // +Lust(minor)]
    {
        
        var damage : Int = as3hx.Compat.parseInt(40 + rand(10));
    ...
    //Heavy Attack
    private function impLordHeavyEncounter() : Void
    {
        var damage : Int = as3hx.Compat.parseInt((str + weaponAttack + 20) - rand(player.tou) - player.armorDef);
        outputText("The demonic creature slashes a clawed hand towards your stomach,");
        if (damage <= 0)
        {
            outputText(" but the attack proves ineffectual.");
        }
        else
        {
    ...
    //Lust Attack
    private function impLordLustAttack() : Void
    {
    ...
    private function impLordLustAttack2() : Void
    {
        ...
        var damage : Int = as3hx.Compat.parseInt(3 + rand(10));
        player.takePhysDamage(damage, true);
        //[-HP(minor) // +Lust]
    ...
    {
        var choices : Array<Dynamic> = [impFire, impLordLustAttack2, impLordLustAttack, impLordHeavyEncounter, eAttack];
    ...
    public function new()
    {
        ...
        if (player.hasStatusEffect(StatusEffects.SiegweirdImp))
        {
            this.short = "imp with a huge greatsword";
        }
        else
        {
            this.short = "imp lord";
        ...
        if (player.hasStatusEffect(StatusEffects.SiegweirdImp))
        {
            this.long = "You are under attack by an imp wielding a huge greatsword! The imp’s arms are so muscular it would be funny if not for the fact he’s trying to split you in half with a weapon trice his size!";
        }
        else
        {
        ...
        if (flags[kFLAGS.IMP_LORD_MALEHERM_PROGRESS] >= 10 && !player.hasStatusEffect(StatusEffects.SiegweirdImp))
        {
            this.createVagina();
        }
        createBreastRow(0);
        ...
        this.drop = new WeightedDrop().add(consumables.MINOBLO, 1).add(consumables.LABOVA_, 1).add(consumables.INCUBID, 6).add(consumables.SUCMILK, 6);
