...
class Imp extends Monster
{
    override public function defeated(hpVictory : Bool) : Void
    {
        ...
        else if (rand(20) == 0 && !player.hasStatusEffect(StatusEffects.TookImpTome))
        {
    ...
    private function lustMagicAttack() : Void
    {
        ...
        {
            outputText("You feel strangely warm.  ");
        }
        if (player.lust >= (player.maxLust() * 0.3) && player.lust < (player.maxLust() * 0.6))
        {
            outputText("Blood rushes to your groin as a surge of arousal hits you, making your knees weak.  ");
        }
        if (player.lust >= (player.maxLust() * 0.6))
        {
            outputText("Images of yourself fellating and fucking the imp assault your mind, unnaturally arousing you.  ");
        }
        ...
        {
            var _sw0_ = (player.vaginas[0].vaginalWetness);            
    ...
    private function lustMagicAttack1() : Void
    {
        outputText("You see " + a + short + " make sudden arcane gestures at you!\n\n");
        ...
        if (player.lust >= (player.maxLust() * 0.6) && player.hasVagina())
        {
            var _sw1_ = (player.vaginas[0].vaginalWetness);            
    ...
    public function allVaginaDescript() : String
    {
        ...
        {
            return Appearance.vaginaDescript(player, rand(player.vaginas.length - 1));
        }
        if (player.vaginas.length > 1)
        {
            return (Appearance.vaginaDescript(player, rand(player.vaginas.length - 1)) + "s");
        }
