...
class ImpScene extends BaseContent
{
    public function new()
    {
        super();
    ...
    public function impVictory() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.Feeder))
        {
            ...
            {
                outputText("  You're not really turned on enough to rape it, but it might be fun to force it to guzzle your breast-milk.\n\nDo you breastfeed it?");
            }
        }
    ...
    public function impVictory2() : Void
    {
        ...
        {
            if (flags[kFLAGS.GALIA_AFFECTION] > 0)
            {
                addButtonDisabled(3, "Capture", "You need to turn in already captured imp before you can capture another one.");
            }
            else
            {
                addButton(3, "Capture", captureFeralImp);
            }
        }
        addButton(4, "Leave", cleanupAfterCombat);
    }
    private function rapeImpWithDick(condomed : Bool = false) : Void
    {
        var x : Float = player.cockThatFits(monster.analCapacity());
        if (x < 0)
        {
            x = 0;
        }
        ...
        if (player.cocks.length == 1)
        {
            ...
            if (!player.isTaur())
            {
                ...
                if (player.breastRows.length > 0 && player.breastRows[0].breastRating > 2)
                {
                    outputText("smashing him against your [breasts] while you jerk hard on your " + cockDescript(x) + ", bringing it to a full, throbbing erection.");
                }
                else
                {
                    outputText("stroking it to full hardness languidly.");
                }
            }
            ...
            {
                outputText("  The tiny creature's claws dig into your sides at the feeling of soft, hooked barbs stroking his sensitive insides.");
            }
            if (player.cocks[0].cockLength >= 7 && player.cocks[0].cockLength <= 12)
            {
                outputText("  Each thrust obviously distorts the imp's abdomen.  It amazes you that it doesn't seem to be hurting him.");
            }
            if (player.cocks[0].cockLength > 12)
            {
                outputText("  Each plunge into the imp's tight asshole seems to distort its entire body, bulging obscenely from its belly and chest.  Amazingly he doesn't seem to mind, his efforts focused solely on his sorely throbbing demon-dick.");
            }
            ...
            {
                outputText("  The cum seems to boil out from inside you as your [vag] soaks itself.  With delicious slowness you fire rope after rope of cum " + ((condomed) ? "inside your condom." : "deep into the imp's rectum.") + "");
            }
            if (player.balls == 0 && player.vaginas.length == 0)
            {
                outputText("  The cum seems to boil out from inside you, flowing up your " + cockDescript(x) + ".  With delicious slowness, you fire rope after rope of cum " + ((condomed) ? "inside your condom." : "deep into the imp's rectum.") + "");
            }
            if (player.cumQ() >= 14 && player.cumQ() <= 30)
            {
                outputText("  Your orgasm drags on and on, until your slick jism is dripping out around your " + cockDescript(x) + ".");
            }
            if (player.cumQ() > 30 && player.cumQ() <= 100)
            {
                outputText("  Your orgasm seems to last forever, jizz dripping out of " + ((condomed) ? "your condom" : "the imp's asshole") + " around your " + cockDescript(x) + " as you plunder him relentlessly.");
            }
            if (player.cumQ() > 100)
            {
                outputText("  Your orgasm only seems to grow more and more intense as it goes on, each spurt more powerful and copious than the last.  " + ((condomed) ? "Your condom swells nearly to the point of bursting, and tiny jets of cum squirt out around your " + cockDescript(x) + " with each thrust." : "The imp begins to look slightly pregnant as you fill him, and tiny jets of cum squirt out around your " + cockDescript(x) + " with each thrust.") + "");
            }
        ...
        if (player.cocks.length >= 2)
        {
            ...
            if (!player.isTaur())
            {
                ...
                if (player.breastRows.length > 0 && player.breastRows[0].breastRating > 2)
                {
                    outputText("smashing him against your [breasts] while you jerk hard on one of your " + cockDescript(x) + "s, bringing it to a full, throbbing erection.");
                }
                else
                {
                    outputText("stroking one of your members to full hardness languidly.");
                }
            }
            ...
            if (player.cocks[0].cockLength > 12 && player.cocks[0].cockLength <= 18)
            {
                outputText("  Each plunge into the imp's tight asshole seems to distort its entire body, bulging obscenely from its belly and chest.  Amazingly he doesn't seem to mind, his efforts focused solely on his sorely throbbing demon-dick.");
            }
            outputText("\n\nThe tight confines of the imp's ass prove too much for you, and you feel your orgasm build.");
            if (player.hasBalls())
            {
                outputText("The cum seems to boil in your balls, sending heat spreading through your " + cockDescript(x) + " as your muscles clench reflexively, propelling hot spurts of jism deep into the imp's rectum.  Your other equipment pulses and dripples steady streams of its own cum.");
            }
            if (player.balls == 0 && player.vaginas.length > 0)
            {
                outputText("The cum seems to boil out from inside you as your [vag] soaks itself.  With delicious slowness you fire rope after rope of cum " + ((condomed) ? "inside your condom." : "deep into the imp's rectum.") + "  Your other equipment drizzles small streams of jizz in sympathy.");
            }
            if (player.balls == 0 && player.vaginas.length == 0)
            {
                outputText("The cum seems to boil out from inside you, flowing up your " + cockDescript(x) + ".  With delicious slowness, you fire rope after rope of cum " + ((condomed) ? "inside your condom." : "deep into the imp's rectum.") + "  Your other equipment drizzles small streams of jizz in sympathy.");
            }
            if (player.cumQ() >= 14 && player.cumQ() <= 30)
            {
                outputText("  Your orgasm drags on and on, until your slick jism is dripping out around your " + cockDescript(x) + ".");
    ...
    private function sprocketImp() : Void
    {
        ...
        {
            outputText("His claws dig into your hair ");
        }
        else
        {
            outputText("His claws dig into your wrists ");
        }
        ...
        {
            outputText("your [cock], rubbing the tip of his prick against your flared head, ");
        }
        else if (player.cocks[0].cockType == CockTypesEnum.TENTACLE)
        {
            outputText("your huge green dick, rubbing the tip of his prick against your purplish cock-head, ");
        }
        else if (player.hasKnot(0))
        {
            outputText("your [cock], rubbing the tip of his prick against your point, ");
        }
        else
        {
            outputText("your [cock], rubbing the tip of his prick against your own, ");
        }
        ...
        if (player.cocks[0].cockType == CockTypesEnum.HORSE)
        {
            outputText("seats his dick in the flared head of your prick, and then pushes farther. His shaft plunges into yours, filling your cock more than any cum load ever could.  ");
        }
        else if (player.cocks[0].cockType == CockTypesEnum.TENTACLE)
        {
            outputText("shoves his dick deeply into the slit in the head of your vine-like cock.  ");
        }
        else if (player.hasKnot(0))
        {
            outputText("finds the hole in the pointed head of your cock and plunges his shaft deeply into it, literally fucking your urethra.  ");
        }
        else
        {
            outputText("shoves his shaft deeply into the slit in the head of your dick.  ");
        }
        //New PG
        outputText("\n\n");
        ...
        {
            outputText("furry cock-sleeve");
        }
        else if (player.bodyColor == "purple" || player.bodyColor == "blue" || player.bodyColor == "shiny black")
        {
            outputText("demonic cock-sleeve");
        }
        else
        {
            outputText("human cock-sleeve");
        }
        //Bonus boob shake or period if no boobs.
        if (player.breastRows.length > 0 && player.biggestTitSize() > 2)
        {
            outputText(", fucking you so hard that your [allbreasts] bounce with each thrust.  ");
        }
        else
        {
            outputText(".  ");
        ...
        if (player.cocks.length == 2)
        {
            outputText("your other cock finding pleasure in rubbing against his body ");
        }
        if (player.cocks.length > 2)
        {
            outputText("your other cocks finding pleasure in rubbing against his body ");
        }
        //Cum
        ...
        if (player.cocks.length == 2)
        {
            outputText("Your other cock cums at the same time, liberally splattering your spunk up his back.  ");
        }
        if (player.cocks.length > 2)
        {
            outputText("The rest of your [cocks] twitch and release their seed at the same time, creating a shower of spunk that rains down on both you and the imp, coating both of your bodies.  ");
        }
        if (player.biggestLactation() >= 1)
        {
            outputText("At the same time, milk bursts from your [nipple]s, splattering him in the face.  You feel a sick sort of triumph as you get him back for cumming inside you.  ");
        }
        //Vagoooz
        if (player.vaginas.length > 0)
        {
            outputText("Your pussy quivers, contracting furiously as your orgasm hits you - like it's trying to milk a phantom dick dry.  ");
        }
        //new PG
    ...
    private function centaurGirlOnImps() : Void
    {
        ...
        if (player.hasVagina())
        {
            ...
            {
                outputText("  Your hands reflexively grab your " + chestDesc() + " and mash them about.");
            }
            outputText("\n\n");
            ...
            if (player.biggestTitSize() > 1)
            {
                ...
                {
                    outputText("  His fingers slide inside your [nipple]s and start spreading and squishing them. Your femcum leaks out over his hands and soon your front is slick and shiny.");
                }
                //All other nipples:
                else
                {
                    outputText("  His fingers grope and grab at your nipples, stretching them uncomfortably. Before you can complain he seems to realize his mistake and releases them.");
                }
                //[Is lactating normally:
                if (player.biggestLactation() >= 1 && player.lactationQ() < 50)
                {
                    outputText("  Milk dribbles and squirts from you as his desperate squishing continues, forming small puddles on the ground.");
                }
                else if (player.biggestLactation() >= 1)
                {
                    outputText("  Milk sprays from you as his desperate squishing continues, creating massive puddles of milk that you splash through as you continue moving.");
                }
                outputText("\n\n");
            }
    ...
    private function centaurOnImpStart() : Void
    {
        ...
        if (monster.HP == 0)
        {
            outputText("panting in exhaustion");
        }
        else
        {
            outputText("masturbating furiously");
        }
        ...
        if (player.cor < 50)
        {
            outputText("You lick your lips slightly as you begin to approach the small figure.");
        }
        else
        {
            outputText("You lick your lips obscenely as you approach the small figure.\n\n");
        }
        if (player.gender == 1)
        {
            centaurOnImpMale();
    ...
    private function centaurOnImpMale(vape : Bool = false) : Void
    {
        if (vape)
        {
            clearOutput();
    ...
    private function cOIM_tenta() : Void
    {
        ...
        {
            addButtonDisabled(1, "Two", "Req. 2 tentacles");
        }
        ...
        {
            addButtonDisabled(2, "Three", "Req. 3 tentacles");
        }
    }
    ...
    private function cOIM_tenta_2() : Void
    {
        ...
        else
        {
            ...
            if (player.hasFuckableNipples())
            {
                outputText("Shoving your fingers deep into your [nipple]s");
            }
            else
            {
                outputText("With a fierce tug on your [nipple]s");
            }
            outputText("you begin to cum deep and directly into the imp's stomach and " + eAssholeDescript() + ".  ");
            //<<cum multiplier: lots>>
            if (player.cumQ() > 250)
            {
                outputText("Beneath you the creature's belly is distending more and more, and you can feel some of the overflowing cum filling back out until it is pouring out of the creature's unconscious mouth and overstretched ass, forming a spermy pool beneath it.");
            }
    ...
    private function cOIM_tenta_3() : Void
    {
        ...
        if (player.biggestTitSize() >= 0)
        {
            ...
            if (player.hasFuckableNipples())
            {
                outputText(" and fingering your [nipple]s");
            }
    ...
    //CUNTS
    private function centaurOnImpFemale(vape : Bool = false) : Void
    {
        if (vape)
        {
            clearOutput();
        ...
        //Penetration for non-gape cases{
            ...
            else
            {
                if (player.vaginalCapacity() < monster.cockArea(0))
                {
                    outputText("It groans in delight at your incredible tightness and shoves itself forward even harder.  ");
                }
                //[Increase size as needed]
                //<<At Dicksize>>
                if (player.vaginalCapacity() >= monster.cockArea(0) && player.vaginalCapacity() <= monster.cockArea(0) * 1.25)
                {
                    outputText("It makes a pleased sound as it slides deeply into your [vag].  ");
                }
                //<<Bigger than dicksize>>
                if (player.vaginalCapacity() >= monster.cockArea(0) * 1.25)
                {
                    outputText("Its dick slides easily and slopping noises start sounding from your backside.  Part of you wishes that its large member was larger still, as your mind drifts to some of the monstrous cocks that have penetrated you in the past.  ");
                }
            }
        ...
        else
        {
            ...
            if (player.cor >= 50)
            {
                outputText("You take comfort in knowing that at least there is a cock inside of you, and that soon it will be filling you with its seed.  Perhaps it might even impregnate you!  ");
            }
            ...
            if (player.cor < 80)
            {
                outputText("Tears pour out of your eyes, and you are sure you must be bleeding slightly, ");
            }
            //<<Cor <50>>
            if (player.cor < 50)
            {
                outputText("and you hang on to the tree, afraid of the pain from even the slightest movement.  ");
            }
            //<<Cor 50+>>
            else
            {
                outputText("and you hang on to the tree, grunting like a rutting animal as you delight in the intense pain.  ");
            }
            //<<Cor 80+>>
            if (player.cor >= 80)
            {
                outputText("You howl out in pain and pleasure, bucking and hoping to intensify the sensation, hurling enticements and insults at the imp like a slut.  ");
            }
            //<<Cor 50+, Breasts>>
            if (player.cor >= 50 && player.biggestTitSize() >= 2)
            {
                ...
                if (player.hasFuckableNipples())
                {
                    outputText(" and shoving your fingers into your [nipple].  ");
                }
                else
                {
                    outputText(".  ");
            ...
            if (player.analCapacity() >= 35)
            {
                ...
                if (player.ass.analLooseness < 4 && player.cor < 80)
                {
                    outputText("Your howl of pain leaves your throat raw.  ");
                }
                else
                {
                    outputText("Your howl of perverse pleasure leaves your throat raw.  ");
                }
            }
            ...
            if (player.cor < 80)
            {
                outputText("promise to yourself that you will not do that again.");
            }
            else
            {
                outputText("find your cunt juices already dripping down your legs in anticipation of doing this again.");
            }
            player.sexReward("cum", "Vaginal");
            cleanupAfterCombat();
        }
    ...
    private function centaurOnImpResults(iNum : Float) : Void
    {
        var x : Float = player.cockThatFits(monster.analCapacity());
        if (x < 0)
        {
        ...
        switch (iNum)
        {
            ...
            //<<no knot>>{
                ...
                {
                    outputText("Soon the amount is overflowing from the abused " + eAssholeDescript() + ", dripping between you with no sign of stopping as you continue thrusting yourself into the imp.  ");
                }
                //<<knot>>
                else
                {
                    outputText("Soon the abused " + eAssholeDescript() + " is full to the brim, though your knot keeps any from escaping while more and more pumps in.  Soon the creature's belly is distending, and the imp is gasping wordlessly. ");
                }
                ...
                {
                    outputText("Your other cock drenches the imp's back with its own secretions that immediately start dripping down its sides.  ");
                }
                //<<3+ cocks>>
                if (player.cockTotal() > 2)
                {
                    outputText("Your other cocks release their cum all over the creature's back and sides, leaving it a glazed mess.  ");
                }
                //<</I1_1>>
            ...
            case 2, 3:
                ...
                {case 2:
                        ...
                        if (player.cumQ() >= 250)
                        {
                            ...
                            if (player.cumQ() >= 500)
                            {
                                outputText("No matter how fast it is swallowing it does not seem to be enough, and soon its belly is distended and its skin is covered in a thick coating of cum.  ");
                            }
                        }
                ...
                if (player.biggestTitSize() >= 2)
                {
                    ...
                    //<<nip-pussies and milk>>{
                        ...
                        if (player.biggestLactation() >= 1)
                        {
                            outputText("Milk streams out from your [nipple]s as if they had been recently filled with dripping cum.  ");
                        }
                        else
                        {
                            outputText("Your fingers slide faster and faster into your [nipple]s even as the imp begins to stroke itself under you.  ");
                        }
                    }
                    //No pussies
                    else
                    {
                        ...
                        //<<little milk>>{
                            ...
                            if (player.biggestLactation() <= 1)
                            {
                                outputText("Beads of milk begin to drip down your chest and occasionally spurt outward.  ");
                            }
                            //<<else>>
                            else
                            {
                                outputText("Milk pours out of your [breasts] and streams down your body.  ");
                            }
                        }
                    }
                }  //<</Breasts>>  
    ...
    public function impGangabangaEXPLOSIONS(loss : Bool = false) : Void
    {
        player.slimeFeed();
        ...
        if (!loss)
        {
            outputText("<b>You sleep uneasily. A small sound near the edge of your camp breaks into your rest, and you awaken suddenly to find yourself surrounded by [themonster]</b>!\n\n");
        }
        ...
        var getPinned : Void->Void = function() : Void
        //(First encounter)
        {
            ...
            if (!player.hasStatusEffect(StatusEffects.ImpGangBang))
            {
                ...
                {
                    outputText(" while another kicks your weapon out of reach");
                }
            ...
            if (player.biggestTitSize() > 1)
            {
                outputText("Having your arms tied behind your back forces your chest out, making your [allbreasts] stand out. They bounce as you struggle.  ");
            }
            ...
            if (player.cor < 50)
            {
                outputText("Your face flushes with humiliation. Your imp rider twists your [hair] hard and you whimper in pain. Imps rub their cocks along your [hips] while others stroke themselves and jeer at your helplessness.  ");
            }
            //(High Corruption)
            else
            {
                outputText(monster.capitalA + " swarms over your body, some stroking themselves as they watch you squirm while others rub their cocks over your flanks. Your imp rider twists your hair, pulling your head back, and you moan in pleasure at the rough handling. Your [skin.type] tingles as you start to flush with desire.  ");
            }
            ...
            if (player.hasBalls())
            {
                outputText("The tongue slides over your [sack], coating it with warm drool.  ");
            }
            //(Low Corruption)
            if (player.cor < 50)
            {
                outputText("You shake your hips, trying to escape the demonic tongue. The imp grips your [hips] and pulls his face further into your cunt, sliding his nimble tongue over your lips. You grit your teeth, trying to ignore the warmth spreading from your [vag].");
            }
            //(High Corruption)
            else
            {
                outputText("You let out a shuddering sigh as the heat from your cunt spreads into the rest of your body. Your [hips] tremble as the tongue slides over the folds of your [vag]. The imp grips your flanks harder and dives his nimble tongue into your fuck-hole.");
            }
            outputText("\n\n");
            ...
            if (player.biggestTitSize() > 1)
            {
                ...
                if (player.cor < 50)
                {
                    outputText("You can't escape the tongues lapping and pulling at your [nipple], matching the one in your cunt. You shake your head to deny the pleasure, but your breathing comes faster and faster as lust invades your body.");
                }
                //(High Corruption)
                else
                {
                    outputText("The tongues squeezing and tugging your nipples match the tongue working your [vag], flooding your body with lust. You moan and arch your back, offering your tits to the imps. You can hear your pulse pounding in your ears as you pant with desire.");
                }
            ...
            if (player.cor < 50)
            {
                outputText("You yelp and twist your head to escape the heat.  ");
            }
            //(End low corruption)
            ...
            if (player.biggestTitSize() > 2)
            {
                outputText("The big imp grabs one of your painfully distended breasts in each hand, mauling and bouncing the flesh as if weighing them. You gasp in pain as your [allbreasts] swell further at his touch. ");
            }
            ...
            if (player.cor < 50)
            {
                outputText("It's either swallow or have that demon-tongue forced all the way down your throat. Against your will you gulp back the glob.");
            }
            //(High Corruption)
            else
            {
                outputText("You swallow the glob of pre-cum eagerly, trying to suck the demon's tongue into your throat.");
            }
            outputText("\n\n");
            ...
            if (player.biggestTitSize() > 2)
            {
                outputText("The other imps reclaim your aching breasts, sucking your [nipple] and mauling your [allbreasts] so hard their fingers disappear into your swelling flesh. ");
            }
            ...
            if (player.cor < 50)
            {
                outputText("You scream for help");
            }
            //(High corruption)
            else
            {
                outputText("You moan with lust");
            }
            ...
            if (player.biggestTitSize() > 1)
            {
                outputText("The imps working your breasts suck harder, kneading your tit-flesh as though trying to milk you. ");
            }
            ...
            if (player.cor < 50)
            {
                outputText(" in helpless terror as you feel the bulge work up from the base of your groin towards your stomach. You let out a shuddering moan of pain as inch after inch of monstrous " + Appearance.cockNoun(CockTypesEnum.HORSE) + " stretches your belly");
            }
            //(High corruption)
            else
            {
                outputText(", panting in lust as the monstrous " + Appearance.cockNoun(CockTypesEnum.HORSE) + " pushes your flesh aside to make room for itself");
            }
            outputText(". ");
            ...
            if (player.cor < 50)
            {
                outputText("whimper, spasming as the hot shaft presses against new areas");
            }
            //(High corruption)
            else
            {
                outputText("moan in pleasure, rotating your hips around this incredible cock");
            }
            outputText(" in your stuffed [vag]. The big imp sneers and flexes his cock again, watching ");
            //(If the character has breasts)
            if (player.biggestTitSize() >= 2)
            {
                outputText("your [allbreasts] roll on your chest as you squirm");
            }
            //(If the character doesn't have breasts)
            else
            {
                outputText("your eyes roll back as you squirm");
            }
            outputText(".\n\n");
            ...
            if (player.cockTotal() > 0)
            {
                outputText("Your [cocks] bounces as the bulge passes over it.  ");
            }
            ...
            if (player.biggestTitSize() >= 2)
            {
                outputText("The slow fucking shakes your breasts, and the imps sucking at your nipples cling tightly to your monstrously swollen [allbreasts]. Your " + biggestBreastSizeDescript() + " have grown three cup sizes since the imps pumped their venom into you. An ache starts deep in the base of your tits and works its way to your sore [nipple]. Your already bloated nipples swell as the imps suckle, and you gasp as the first rush of milk spills into their mouths. Your rider reaches around and starts to milk your udders, moving his hands between your [allbreasts] and forcing out more milk for his gangmates.\n\n");
            }
            ...
            if (player.biggestTitSize() > 1)
            {
                outputText("Imps gulp milk from your bloated " + biggestBreastSizeDescript() + ". As one imp drinks his fill and staggers away with a sloshing belly, another steps up to pump your milk-spewing udders.  ");
            }
            //(If the character has a dick)
            if (player.cockTotal() > 0)
            {
                outputText("Your [cocks] swell painfully as the rough fucking pumps blood into your groin.  ");
            }
            ...
            if (player.cockTotal() > 0)
            {
                outputText("He draws his tongue back and wraps it around your [cock], sliding its length along your shaft and flicking his tongue over your cock-head.  ");
            }
            outputText("You gasp in time to the big imp's thrusts, whimpering when his cock or tongue hit a sensitive point. ");
            //(Low Corruption)
            if (player.cor < 50)
            {
                outputText("You're being raped by a demon, milked like a cow, and you're about to cum hard. This corrupted land has left its mark on you.");
            }
            //(High corruption)
            else
            {
                outputText("This corrupted land has left its mark on you. You could never have taken a cock this big before you arrived here.");
            }
            ...
            {
                outputText("Your udders shake back and forth under your chest in time to the rough fucking. You arch your back to press your [nipple] into eager mouths, moaning as your rider milks your distended [allbreasts]. ");
            }
            //(Low Corruption).
            if (player.cor < 50)
            {
                outputText("Some part of you can still feel shame, and you whine and clench your teeth as the urge to <i>moo</i> rises in you.");
            }
            //(High corruption)
            else
            {
                outputText("You moan shamelessly as you're fucked and milked, and the moans turn to long <i>mooos</i> of ecstasy.");
            }
            outputText("\n\n");
            ...
            if (player.cor >= 50)
            {
                outputText("You lap eagerly at the salty cum, licking up and drinking as much as you can.");
            }
            outputText("\n\n");
            ...
            if (player.biggestTitSize() > 2)
            {
                outputText(", and bouncing [allbreasts]");
            }
            ...
            if (player.biggestTitSize() > 2)
            {
                outputText("milk spurts from your [nipple] and ");
            }
            ...
            if (player.cor < 50)
            {
                outputText("Your last coherent thought is to find a way to better hide your camp, so this never happens again.");
            }
            //(High corruption)
            else
            {
                outputText("Your last coherent thought is to find a way to make your own mutated master imp, maybe even a stable full of them...");
            }
            player.sexReward("cum", "Vaginal");
            dynStats("lib", 2, "cor", 3);
            if (!player.isGoblinoid())
            {
                player.knockUp(PregnancyStore.PREGNANCY_IMP, PregnancyStore.INCUBATION_IMP - 14);
        ...
        var tiedToTheTree : Void->Void = function() : Void
        {
            clearOutput();
            if (player.hasStatusEffect(StatusEffects.ImpGangBang))
            ...
            //(Subsequent encounters - Low Corruption){
                ...
                if (player.cor < 50)
                {
                    outputText("You can't tell if this is the same [monster name] as last time or not. You're not racist, but all imps look alike to you. The [monster name] surges forward, grabbing at your legs and arms and running their hands over your body. You struggle, but there are just too many to fight. The result is the same as last time...\n\n");
                }
                //(Subsequent encounters - High Corruption)
                else
                {
                    outputText("It's about time they showed up. It's not like there's a lot to do in these rocks, and you were getting bored. You grab an imp dick in either hand and spread your legs as other imps grope your thighs...\n\n");
                }
            }
            ...
            if (player.cor < 50)
            {
                outputText("swing your [weapon] wildly, determined not to let them take you");
            }
            //(High Corruption)
            else
            {
                outputText("twist and struggle in their grips, determined to make them work for their fun");
            }
            ...
            if (player.biggestTitSize() > 0)
            {
                outputText("Your unbound " + biggestBreastSizeDescript() + " bounce out over the weathered log. ");
            }
            ...
            if (player.cor < 50)
            {
                outputText("Your face flushes with humiliation as they turn their attentions on each other, each working their hands and tongue over the other's dick. How dare these demons use you as a bed to sate their lusts?!");
            }
            //(High Corruption)
            else
            {
                outputText("Your face flushes with anger as they turn their attentions on each other, each working their hands and tongue over the other's dick. You worked hard for this magnificent body, and now they're not using it?!");
            }
            outputText("\n\n");
            ...
            if (player.biggestTitSize() > 2)
            {
                outputText("An imp mounts the log and slaps his [monster cockshort] between your [allbreasts], squeezing them tight over his cock as he rubs back and forth. He mauls your breasts cruelly, squeezing his fingers deep into your soft flesh.  ");
            }
            //(If the player has a SINGLE cock)
            if (player.cockTotal() == 1)
            {
                outputText("An imp ducks under your body and grabs your [cock]. His nimble tongue flicks over your cock-head while he pricks the shaft with his tiny claws.  ");
            }
            //(If the player has a MULTI cock)
            if (player.cockTotal() > 1)
            {
                outputText("Two imps duck under your body and seize your [cocks], licking the tips with their inhumanly flexible tongues while they stroke the shafts.  ");
            }
            //(Low Corruption)
            if (player.cor < 50)
            {
                outputText("You fight to free your hind legs and buck the imps off your back, while sweaty hands slide over your crotch. You whine through clenched teeth as sharp claws jab at your sensitive flesh.\n\n");
            }
            //(High Corruption)
            else
            {
                outputText("You writhe in the grasp of the imps, reveling in the sensations as tiny claws and teeth nip at your sensitive crotch. You lick salty musk off the swollen balls dangling above your mouth.\n\n");
            }
            outputText("\n\n");
            ...
            if (player.biggestTitSize() > 2)
            {
                outputText("The imp fucking your " + biggestBreastSizeDescript() + " handles your soft flesh roughly, pressing and pulling your tits into a fuck-canal for his demon cock. Other imps slap your [allbreasts] and laugh as you cry out.  ");
            }
            //(Low Corruption)
            if (player.cor < 50)
            {
                outputText("You whimper as your mistreated flesh stings with dozens of pin-prick scratches and bites, and [themonster] slaps your chest and flanks. The abuse falls on you from all sides, leaving you with no escape. The imp on your shoulders pries your jaws open, and you gag on his " + monster.ballsDescriptLight() + ".");
            }
            //(High Corruption)
            else
            {
                outputText("You suckle eagerly at the musky balls in your mouth. Abuse falls on you from all sides, imps leaving tiny marks on your skin as they nip and scratch at you. You whimper in delight as tiny hands slap your chest and flanks.");
            }
            outputText("\n\n");
            ...
            if (player.biggestTitSize() > 2)
            {
                outputText("one of your sore " + biggestBreastSizeDescript() + " in his calloused hand, brutally pressing his fingers into your flesh");
            }
            //(If the player doesn't have breasts)
            else
            {
                outputText("your tail and yanks, brutally pulling on it");
            }
            ...
            if (player.cockTotal() > 0)
            {
                outputText("Pre-cum drips from the broad tip of it, dripping down to the base of your [cocks].  ");
            }
            ...
            if (player.cor < 50)
            {
                outputText("You try to pull away from the hot cock-head rubbing against your hole, but [themonster] holds you tight.");
            }
            //(High Corruption)
            else
            {
                outputText("The scent of musk steaming off the" + Appearance.cockNoun(CockTypesEnum.DOG) + " drives you wild, and you push back to try and capture the cock-tip.");
            }
            outputText("\n\n");
            ...
            if (player.biggestTitSize() > 2)
            {
                outputText("  The imp fucking your aching " + biggestBreastSizeDescript() + " paints your tits with a massive load of cum. He falls off the log and another imp jumps up to take his place.");
            }
            outputText("\n\n");
            ...
            if (player.hasVagina())
            {
                outputText("the entrance of your womb");
            }
            //(If the player doesn't have a vagina)
            else
            {
                outputText("depths of your bowels");
            }
            ...
            if (player.biggestTitSize() > 2)
            {
                outputText("The rough fucking shakes your cum-stained breasts, and the imp fucking your [allbreasts] clings tightly to your red and swollen tit flesh. Your " + biggestBreastSizeDescript() + " burn with agony as [themonster] slaps your tits like drums.  ");
            }
            //(Low Corruption)
            if (player.cor < 50)
            {
                outputText("You're being raped again by demons, impaled on cocks like a roast pig on a spit, and you can feel your lust rising. This corrupted land has left its mark on you.");
            }
            //(High corruption)
            else
            {
            ...
            if (player.cor < 50)
            {
                outputText("You gurgle helplessly as the cock raping your throat pours thick wads of");
            }
            //(High Corruption)
            else
            {
                outputText("You eagerly chug thick wads of cum from the cock stretching your throat, working your throat to force more");
            }
            ...
            if (player.cockTotal() > 0)
            {
                outputText("  Your [cocks] shoots cum across the ground and into the waiting mouths of the imps licking your crotch.");
            }
            outputText("\n\n");
            ...
            if (player.cor < 50)
            {
                outputText("You howl around the imp-cock stretching your throat. The bloated knot opens your hole far beyond anything you've endured before. Your violent thrashing throws the imps off your back, and you buck uselessly, thrashing as the swollen " + Appearance.cockNoun(CockTypesEnum.DOG) + " plunges deeper into you.");
            }
            //(High corruption)
            else
            {
                outputText("The master imp's bloated knot stretches your entrance and plunges into your hole with a loud <i>pop</i>. Another orgasm hits you as the " + Appearance.cockNoun(CockTypesEnum.DOG) + " rams even deeper into you. You howl around the imp-cock stretching your throat, bucking as your orgasm shakes you. Your violent thrashing throws the imps off your back and slams your hips against the big imp, pushing him further into your hole.");
            }
            ...
            if (player.biggestTitSize() > 2)
            {
                outputText("The imp riding your " + biggestBreastSizeDescript() + " cums, his load lost in the flood of jism dripping off your abused fuck-udders. ");
            }
            ...
            if (player.biggestTitSize() > 0)
            {
                outputText("your bruised and throbbing tits pressed against the rough wood, ");
            }
            ...
            if (player.cor < 50)
            {
                outputText("How long can you last in this corrupted land, when your body can be so horribly twisted by the sick pleasures of its denizens?");
            }
            //(High corruption)
            else
            {
                outputText("Why bother with your silly quest, when you've only scratched the surface of the pleasures this land offers you?\n");
            }
            player.sexReward("cum");
            dynStats("lib", 2, "cor", 3);
            if (!player.isGoblinoid())
            ...
            if (player.hasVagina())
            {
                if (player.cuntChange(monster.cockArea(2), true))
                {
                    outputText("\n");
                }
            }
        ...
        var getSwarmed : Void->Void = function() : Void
        {
            clearOutput();
            //(First encounter)
            if (!player.hasStatusEffect(StatusEffects.ImpGangBang))
            ...
            if (player.biggestTitSize() > 0)
            {
                outputText("Imps surround your chest, slapping their [monster cockshort]s on your [allbreasts] and rubbing their slippery pre-cum into your [nipple].  ");
            }
            ...
            if (player.cockTotal() > 0)
            {
                outputText("An imp rubs his hand across his cock-head, smearing it with his pre-cum. He rubs his hand over your [cocks], making your cock-skin tingle as his fluid soaks into you.");
            }
            outputText("\n\n");
            ...
            if (player.cor < 50)
            {
                outputText("You renew your struggles, trying to break free of your captors. They only laugh and bear down harder on you.  ");
            }
            //(High corruption)
            else
            {
                outputText("You buck your hips, trying to capture his [monster cockshort] with your [vag].  ");
            }
            ...
            if (player.cor < 33)
            {
                outputText("You yelp and buck your hips to escape the heat.  ");
            }
            ...
            if (player.cor < 50)
            {
                outputText("in helpless terror as the bulge inches up from the base of your groin towards your stomach. You let out a shuddering moan of pain as inch after inch of monstrous " + Appearance.cockNoun(CockTypesEnum.HORSE) + " stretches your belly");
            }
            //(High corruption)
            else
            {
            ...
            if (player.cockTotal() > 0)
            {
                outputText("Your [cocks] slaps against your distended belly as you shake.");
            }
            outputText("\n\n");
            ...
            if (player.cor < 50)
            {
                outputText("The pain as this huge cock stretches you is overwhelming, but every thrust rubs more corrupted pre-cum into your pussy walls. You start to pant as the imp rapes you, using your body for his own pleasure. Your nipples swell as the heat of his pre-cum soaks through your body. The huge shaft forces your [clit] out, and the steaming fluids splashing on it make it tingle almost painfully. Your whimpers and moans of pain start to take on a different tone, and the master imp starts to fuck you faster.");
            }
            //(High corruption)
            else
            {
                outputText("Pain and pleasure blend into one as the huge " + Appearance.cockNoun(CockTypesEnum.HORSE) + " stretches you, rubbing pre-cum into steaming pussy. You moan as the big imp fucks you, turning you into a mindless fuck-puppet. Your [clit] swells painfully as hot juices splash over it. Your [nipple] tingle almost painfully as the heat of his pre-cum spreads through your body.");
            }
            outputText("\n\n");
            ...
            if (player.biggestTitSize() >= 3)
            {
                outputText("Your [allbreasts] bounce and jiggle back and forth as the master imp roughly fucks you.  ");
            }
            //(If the character has a dick)
            if (player.cockTotal() > 0)
            {
            ...
            {
                outputText("His tongue flicks down to your [clit], the split ends of it teasing your clit.");
            }
            outputText("  You gasp in time to the big imp's thrusts, whimpering when his cock or tongue hit a sensitive point.  ");
            //(Low Corruption)
            if (player.cor < 50)
            {
                outputText("You're being raped by a demon, forced to take an inhuman cock, and you're about to cum hard. This corrupted land has left its mark on you.");
            }
            //(High corruption)
            else
            {
            ...
            if (player.cockTotal() > 0)
            {
                outputText(", and cum-dripping [cock]");
            }
            //(If the character has breasts)
            if (player.biggestTitSize() >= 3)
            {
            ...
            //(If the character has breasts)
            if (player.biggestTitSize() >= 3 && player.biggestLactation() > 1)
            {
                outputText("milk spurts from your [nipple] and ");
            }
            ...
            {
                outputText("Your last coherent thought is to find a way to make your own mutated master imp, one you can keep as a fuck-toy...");
            }
            player.sexReward("cum");
            dynStats("lib", 2, "cor", 3);
            if (!player.isGoblinoid())
        ...
        var tagTeam : Void->Void = function() : Void
        //Imp Scene 2 (Bipedal, vaginal)
        {
            ...
            //(Subsequent encounters - Low Corruption){
                ...
                if (player.cor < 50)
                {
                    outputText("You can't tell if this is the same [monster name] as last time or not - all imps look alike to you.  The [monster name] surges forward, grabbing at your [legs] and arms and running their hands over your body. You struggle, but there are just too many to fight. The result is the same as last time...\n\n");
                }
                //(Subsequent encounters - High Corruption)
                else
                {
            ...
            if (player.cor < 50)
            {
                outputText("you punch and kick wildly, determined not to let them take you");
            }
            //(High Corruption)
            else
            {
                outputText("you twist and struggle in their grips, determined to make them work for their fun");
            }
            ...
            if (player.isNaga())
            {
                outputText("coils out, twisting them around a log to hold you still.\n\n");
            }
            else
            {
                outputText(player.legs() + " wide apart, holding them against the log.\n\n");
            }
            ...
            if (player.biggestTitSize() > 0)
            {
                outputText("An imp mounts your chest and slaps his [monster cockshort] between your [allbreasts], squeezing them tight over his cock as he rubs back and forth.  ");
            }
            //(If the player has a SINGLE cock)
            if (player.cockTotal() == 1)
            {
                outputText("Your [cock] is seized by an imp, who licks the tip with his inhumanly nimble tongue while he strokes the shaft.  ");
            }
            //(If the player has a MULTI cock)
            if (player.cockTotal() > 1)
            {
                outputText("Two imps seize your [cocks], licking the tips with their inhumanly nimble tongues while they stroke the shafts.  ");
            }
            //(Low Corruption)
            if (player.cor < 50)
            {
                outputText("You fight to free your arms and shake the imp off your chest while tiny hands slide over your face. They tug at your lips and try to pry your jaws open");
            }
            //High Corruption)
            else
            {
                outputText("You writhe in the grasp of the imps, reveling in the sensations of being spread open and completely at the mercy of these demons. Tiny hands slide over your face, and you lick and suck at the fingers");
            }
            outputText(".\n\n");
            ...
            if (player.biggestTitSize() > 0)
            {
                ...
                if (player.cor < 50)
                {
                    outputText("You can't escape the tongue lapping and pulling at your [nipple]. You shake your head to deny the pleasure, but your breathing comes faster and faster as lust invades your body.");
                }
                //(High Corruption)
                else
                {
                    outputText("The tongue squeezing and tugging your nipple floods your body with lust. You moan and arch your back, offering your tits to the imp riding your chest. You can hear your pulse pounding in your ears as you pant in desire.");
                }
            ...
            if (player.isNaga())
            {
                outputText("alongside your taut tail");
            }
            else
            {
            ...
            if (player.cor < 50)
            {
                outputText("You screech in agony as the big imp forces his mutated wolf-cock into your hole, brutally shoving thick inch after inch of painfully hot " + Appearance.cockNoun(CockTypesEnum.DOG) + " deeper into you than anything should ever go.  ");
            }
            //(High Corruption)
            else
            {
                outputText("The master imp's painfully hot " + Appearance.cockNoun(CockTypesEnum.DOG) + " stretches your hole wider than it ever should be, and you moan in perverse ecstasy.  ");
            }
            ...
            if (player.biggestTitSize() > 0)
            {
                outputText("The big imp reaches past your tit-rider and grabs one of your painfully distended breasts in each hand, mauling and bouncing the flesh as if weighing them. You gasp in pain as your [allbreasts] swell further at his touch.  ");
            }
            ...
            if (player.biggestTitSize() > 1)
            {
                outputText("The rough fucking shakes your breasts, and the imp sucking your nipples clings tightly to your monstrously swollen [allbreasts]. Your " + biggestBreastSizeDescript() + " have grown three cup sizes since the imp pumped his venom into you.  ");
            }
            ...
            if (player.biggestTitSize() > 0)
            {
                outputText("An ache starts deep in the base of your tits and works its way to your sore [nipple]. Your already bloated nipples swell as your rider suckles, and you gasp as the first rush of milk spills into his mouth. Your rider milks your udders, moving his hands between your [allbreasts] and forcing out more milk than he could ever drink. Other imps lick the milk from the shiny skin of your swollen breasts.\n\n");
            }
            ...
            if (player.cockTotal() > 0)
            {
                outputText("  Your [cocks] shoots cum across your belly and into the waiting mouths of the imps licking your crotch.");
            }
            outputText("\n\n");
            //(If the character has breasts)
            if (player.biggestTitSize() > 0)
            {
                ...
                if (player.hasFuckableNipples())
                {
                    ...
                    {
                        outputText("You feel the bulbous head of his cock squeeze further and deeper until it pushes up against your ribs.");
                    }
                    //[ELSE IF breastSize > DD]
                    else
                    {
                        outputText("You feel the unnaturally large erection spear the fat filled depths of your " + biggestBreastSizeDescript() + " until at last the imp has shoved himself in to his hilt. He smiles at the sensation of having his manhood completely engulfed in your " + biggestBreastSizeDescript() + ".");
                    }
            ...
            if (player.biggestTitSize() > 0)
            {
                outputText("The imp riding your " + biggestBreastSizeDescript() + " finally cums, painting your distended fuck-udders with his massive load.  ");
            }
            ...
            if (player.cor < 50)
            {
                outputText("How long can you last in this corrupted land, when your body can be so horribly twisted for the sick pleasures of its denizens?\n\n");
            }
            //(High corruption)
            else
            {
    ...
    public function impRapesAlraune() : Void
    {
        ...
        {
            outputText(" Your twin sister can’t help but rub her pussy in anticipation the moment she sees this.\n\n" +
        ...
        if (player.isLiliraune())
        {
            outputText("juice themselves ");
        }
        else
        {
            outputText("juices itself ");
        }
        ...
        if (player.isLiliraune())
        {
            outputText("At first, he’s unsure who to fuck first then ");
        }
        else
        {
            outputText("He ");
        ...
        if (player.isLiliraune())
        {
            outputText("Your frustrated twin, unable to join in on the action, sticks to masturbating to the show until it’s her turn.");
        }
        outputText("\n\n" +
                "The rough fucking becomes more and more pleasurable as time passes and ");
        if (player.isLiliraune())
        {
            outputText("your twin finally gets an idea as she begins");
        }
        else
        {
            outputText("you begin");
        }
        ...
        if (player.isLiliraune())
        {
            outputText("Your twin seeing a signal there, grabs him and takes her turn.");
        }
        outputText("\n\n");
        if (player.isLiliraune())
        {
            outputText("Several minutes of fucking later, he");
        }
        else
        {
            outputText("He");
        ...
        if (player.isLiliraune())
        {
            outputText(" You’ve been masturbating your vine all the while and cum has splashed everywhere.");
        }
    ...
    public function impRapesYou() : Void
    {
        ...
        function vaginal() : Void
        {
            ...
            {
                outputText("You find yourself hoping you're pregnant as you swiftly lose consciousness.");
            }
            else if (player.canGetPregnant())
            {
                if (player.cor > 75)
                {
                    outputText("With an appreciative moan, you bury your fingers in its slimy warmth, hoping you are pregnant with some fiendish offspring, and lose consciousness.");
                }
                else
                {
                    outputText("You hope you don't become pregnant, but promptly lose consciousness before you can contemplate the prospect any further.");
                }
            }
            if (!player.isGoblinoid())
            {
        ...
        var suckHimMale : Void->Void = function() : Void
        {
            ...
            if (player.isTaur())
            {
                outputText("hocks and knees, your [cock] hurting from the massive blood pressure caused by your unbridled lust. He approaches you and stops about two feet in front of you, watching with delight your helpless state");
            }
            else
            {
                outputText("knees, pull out your [cock] and begin mindlessly stroking yourself as the imp approaches you, a wicked grin on his face. Your mind races with thoughts and images of sucking the imp's cock. He approaches you and stops about two feet in front of you, watching with delight as you succumb to your own lust");
            }
            ...
            if (player.isTaur())
            {
                outputText("Dropping down to the ground, your [cock] trembles against your body to the rhythm of the imp's thrusts, leaving your underbelly smeared with its own pre-cum.\n\n");
            }
            else
            {
                outputText("On all fours now, your [cock] bounces up and down against you to the rhythm of the imp's thrusts, leaving your belly smeared in your own pre-cum.\n\n");
            }
            if (player.ballSize >= 5)
            {
                outputText("Your huge [balls] swing heavily against you as well, responding to the force of the imp's thrusts, slapping your own ass and driving your [cock] even stiffer with lust, the pre-cum pulsing out of your cock in time with the slapping.\n\n");
            }
        ...
        var assFuck : Void->Void = function() : Void
        {
            ...
            if (!player.isTaur())
            {
                outputText("as you are disrobed");
            }
            ...
            if (player.sens > 40)
            {
                ...
                if (player.hasCock())
                {
                    outputText("  You jizz all over the ground in front of you, spraying cum in huge squirts in time with the demon's thrusts.");
                }
                dynStats("cor", 1);
            }
            player.sexReward("cum", "Anal");
    ...
    private function noogaisNippleRape() : Void
    {
        ...
        {
            outputText("unleashing a torrent of thick, creamy milk and ");
        }
        //(if regular milky;
        else if (player.lactationQ() >= 50 && player.biggestLactation() >= 1)
        {
            outputText("releasing a steady trickle of warm milk and ");
        }
        ...
        if (silly())
        {
            outputText("  The imp is sickened, but curious.");
        }
        outputText("\n\n");
        ...
        {
            outputText("bottoming out halfway on his immense dick.");
        }
        else
        {
            outputText("only stopping when the flesh of your immense mammary bumps into his quaking ballsack.");
        }
        outputText("\n\n");
        ...
        if (player.biggestTitSize() <= 4)
        {
            outputText("you feel his swollen cock bottom out, your petite breast unable to swallow any more of his throbbing maleness");
        }
        //((over D)
        else
        {
            outputText("his swollen cock and desperately filled balls are entirely engulfed in tit-flesh");
        }
        ...
        if (player.hasCock())
        {
            outputText(" pumping your " + cockDescript(0));
        }
        //(if female)
        else if (player.hasVagina())
        {
            outputText(" fingering your hungry baby tunnel");
        }
        else
        {
            outputText(" fingering your tingling anus");
        }
        outputText(".\n\n");
        ...
        if (player.gender == 0)
        {
            outputText("your asshole clenches tight on your finger");
        }
        outputText(".\n\n");
        ...
        //(if lactating)
        if (player.biggestLactation() >= 1 && player.lactationQ() < 40)
        {
            outputText("milk, ");
        }
        ...
        if (player.cor > 60)
        {
            outputText("You thrust your digits into your [nipple] once more, scooping out as much imp jizz as you can reach.  You happily drink up the thick goo, savoring the cloying taste before quickly getting dressed and leaving the imp to slumber.");
        }
        //(continue to non-corrupt text)
        else
        {
            //(if not)outputText("You quickly get dressed and leave the imp to his slumbering, his hands still tied together by his loincloth.");
        }
    ...
    private function sexAnImpLord() : Void
    {
        ...
        if (!player.isTaur())
        {
            outputText("  You grab his hands, removing them from his [monster cockshort]. This gets his attention immediately, and you grin widely, pinning him to the ground.");
        }
        else
        {
            outputText("  You place one of your front hooves on his chest, knocking him onto his back.  He attempts to get back up, but you apply more pressure to his thick, manly chest, until he gasps.  The imp gets the idea quickly and stops masturbating, all of his focus now on you.");
        }
        menu();
        ...
        if (player.lust >= 33)
        {
            ...
            {
                addButton(3, "Breastfeed", feederBreastfeedRape).disableIf(!player.hasPerk(PerkLib.Feeder), "Req. Feeder perk");
            }
            LustyMaidensArmor.addTitfuckButton(4);
            if (sceneHunter.other)
            {
                addButton(5, "...", impRapeMenu, sexAnImpLord).hint("Regular imp rape menu.");
            }
        }
        addButton(14, "Leave", cleanupAfterCombat);
    }
    ...
    private function getBlownByAnImpLord() : Void
    {
        ...
        if (player.cocks[player.biggestCockIndex()].cockThickness < 7)
        {
            ...
            if (player.hasBalls())
            {
                outputText("  His hands move down towards your [balls].");
            }
            outputText("\n\n");
            //if(balls > 0)
            if (player.hasBalls())
            ...
            {
                outputText("The demon seems to be searching for your testicles, but when he doesn't find anything, he moves his hand a bit further.\n\n");
            }
            
            //if(hasVagina)
            if (player.hasVagina())
            {
                outputText("A pair of fat red fingers slip into your [vagina] making you gasp in surprise, and clench your walls around the intruders.  After a moment, you relax as those clawed fingers scratch and rub your walls in a way you didn't know possible.  You groan loudly as you draw closer and closer to orgasm.");
            }
            else
            {
                outputText("Two fat red fingers force their way into your [asshole] making you yelp in surprise.  The surprise turns quickly to pleasure as those fingers dance along your insides, massaging places you didn't know you had.  You might have been annoyed at the imp for the advancements if it hadn't felt so good.  You can't help but pant in ecstasy while those clawed fingers gently scratch at your prostate, drawing you closer and closer to orgasm.");
            }
        }
    ...
    private function putBeeEggsInAnImpYouMonster() : Void
    {
        ...
        if (player.isGoo())
        {
            outputText("rippled goo edges");
        }
        else if (player.isNaga())
        {
            outputText("trembling coils");
        }
        else
        {
            outputText("straddling knees");
        }
        ...
        if (player.biggestTitSize() >= 1)
        {
            ...
            if (player.bRows() > 1)
            {
                outputText("first row of ");
            }
            outputText(breastDescript(0) + ", squeezing your tits as you fuck him full.");
            if (player.lactationQ() >= 500)
            {
                outputText("  Rivulets of your milk run down his forearms as he inexpertly milks you.");
            }
        }
        //[If cock:
        else if (player.hasCock())
        {
            outputText("the rise of his swollen belly soon presses against [oneCock] and the rhythm of your thrusts strokes his shiny red stomach against your sensitive organ.");
        }
        else if (player.hasVagina())
        {
            outputText("the imp’s tiny, clawed feet scrabble against you as he flails in pleasure.  By mistake, one slips between the lips of your pussy, small toes wriggling against your inner walls, and you instinctively push down against the small limb, fucking yourself with his foot.");
        }
        else
        {
            outputText("you feel a firm pressure at your [asshole] as the tip of the imp’s lashing tail prods frantically against you, manically shoving in and out of your [asshole].");
        }
    ...
    public function dominateF() : Void
    {
        ...
        if (player.hasKeyItem("Dildo") < 0 && player.hasKeyItem("Deluxe Dildo") < 0)
        {
            outputText(" Annoyed, you fetch your backpack grab a dildo and cut short his pitiful cry by forcing the toy into his mouth. With the moron now properly gaged, you");
        }
        else
        {
            outputText(" You");
        ...
        if (player.hasKeyItem("Dildo") < 0 && player.hasKeyItem("Deluxe Dildo") < 0)
        {
            outputText(" You remove the toy from his mouth only when your sure he won't protest further. ");
        }
    ...
    public function defeatImpPack() : Void
    {
        ...
        if (monster.HP <= monster.minHP())
        {
            outputText("The last of the imps collapses into the pile of his defeated comrades.  You're not sure how you managed to win a lopsided fight, but it's a testament to your new-found prowess that you succeeded at all.");
        }
        else
        {
            outputText("The last of the imps collapses, pulling its demon-prick free from the confines of its loincloth.  Surrounded by masturbating imps, you sigh as you realize how enslaved by their libidos the foul creatures are.");
        }
    ...
    public function impPackGetsRapedByMale() : Void
    {
        ...
        if (player.balls == 0)
        {
            outputText(" and plunging deep inside");
        }
        outputText(".  The imp to the right wraps his tongue around the base ");
        if (player.hasSheath())
        {
            outputText("just above your sheath ");
        }
        ...
        if (player.hasBalls())
        {
            outputText("Cum boils in your balls, ready to paint your foe white.  ");
        }
        ...
        if (player.cumQ() > 1000)
        {
            outputText("  You keep cumming while the other two imps keep licking and servicing you.   By the time you finish, they're glazed in spooge and masturbating as well.");
        }
        outputText("\n\n");
    ...
    public function impPackGetsRapedByFemale() : Void
    {
        ...
        {
            outputText("and coating it with feminine drool ");
        }
        ...
        {
            outputText("You stroke the cock-like appendage in your hand, trembling with delight.  ");
        }
        ...
        {
            outputText("Cunt juices splatter him as you squirt explosively, leaving a puddle underneath him.  ");
        }
        else
        {
            outputText("Cunt juices drip down his shaft, oozing off his balls to puddle underneath him.  ");
        }
        ...
        {
            outputText("  You'll probably get pregnant.");
        }
        player.sexReward("cum", "Vaginal");
        if (!player.isGoblinoid())
        {
    ...
    public function loseToAnImpPack() : Void
    {
        ...
        if (player.HP < 1)
        {
            outputText("Unable to handle your myriad wounds, you collapse with your strength exhausted.\n\n");
        }
        //(LUST)
        else
        {
            outputText("Unable to handle the lust coursing through your body, you give up and collapse, hoping the mob will get you off.\n\n");
        }
        ...
        if (player.biggestTitSize() > 1)
        {
            ...
            if (player.biggestLactation() > 1)
            {
                outputText("  Milk starts to squirt from the pressure being applied to your [breasts], which only encourages the imp to squeeze even harder.");
            }
            outputText("\n\n");
        }
        //(NIPPLECUNTS!)
        if (player.hasFuckableNipples())
        {
            ...
            {
                outputText("  He can only get partway in, but it doesn't seem to deter him.");
            }
            else
            {
                outputText("  Thanks to your massive bust, he is able to fit his entire throbbing prick inside you.");
            }
        ...
        if (player.hasVagina())
        {
            outputText("A thick load bastes your pussy with whiteness, and you can feel it seeping deeper inside your fertile womb.  ");
        }
        ...
        if (player.cockTotal() == 0)
        {
            outputText("and you clench tightly around the uncomfortable intrusion.");
        }
        else
        {
    ...
    private function killImp() : Void
    {
        ...
        {
            player.purifyDemonBonus();
        }
        if (player.hasPerk(PerkLib.QueenOfTheFairies))
        {
            player.QueenOfTheForestDemonBonus();
        }
        if (player.cor < 25)
        {
            dynStats("cor", -0.5);
        ...
        {
            addButton(3, "Harvest", harvestBones);
        }
        else
        {
            addButtonDisabled(3, "???", "Req. Prestige Job: Necromancer.");
        }
    }
    ...
    {
        var harv : Float = 1 + rand(5);
        if (player.hasPerk(PerkLib.GreaterHarvest))
        {
            harv += 4 + rand(12);
        }
        if (harv + player.perkv1(PerkLib.PrestigeJobNecromancer) > SceneLib.campMakeWinions.maxDemonBonesStored())
        {
            harv = SceneLib.campMakeWinions.maxDemonBonesStored() - player.perkv1(PerkLib.PrestigeJobNecromancer);
        }
