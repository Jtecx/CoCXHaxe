...
class Magnar extends Monster
{
    public var maxDominationLevel : Int = 0;
    public var halfBlind : Bool = false;
    public var cantFly : Bool = false;
    ...
    public function heatWave() : Void
    {
        ...
        else if (player.hasStatusEffect(StatusEffects.BurnDoT))
        {
            player.addStatusValue(StatusEffects.BurnDoT, 1, 1);
        }
        else
        {
            player.createStatusEffect(StatusEffects.BurnDoT, SceneLib.combat.debuffsOrDoTDuration(3), 0.03, 0, 0);
        }
        outputText("\n\n");
    }
    ...
    public function magnarPinStruggle(wait : Bool = false) : Void
    {
        clearOutput();
        //If fail:
        if (wait)
    ...
    //(Only option if PC is male or maleherm)
    {
    ...
    //(If this is chosen, it will be the default for the rest of the fight, only if the PC is 8ft or less and female or female herm)
    {
        ...
        if (player.statusEffectv1(StatusEffects.MagnarDominated) > maxDominationLevel)
        {
            maxDominationLevel++;
        }
        ...
        if (player.statusEffectv1(StatusEffects.MagnarDominated) == 1)
        {
            if (!player.armor.isNothing || !player.lowerGarment.isNothing)
            {
                ...
                {
                    outputText("The massive dragon-demon ignores your struggles and cries, ripping off the lower half of your [armor]. You look up into his eyes, spots flashing on your vision as he bears down, tapered dragon dong rubbing your inner thigh. ");
                }
                if (!player.lowerGarment.isNothing)
                {
                    outputText("  Your [lowergarment] is slid to one side by his draconid erection, exposing your now flushing lips to the world.");
                }
                //(DoT, Lust damage, lose your armor and lower garment for the rest of the fight)
                player.takeLustDamage(player.effectiveSensitivity() / 10 + 2, true);
            }
            //play next scene direcly if no armor equipped
            else
            {
        ...
        player.addStatusValue(StatusEffects.MagnarDominated, 1, 1);
        var damage : Int = as3hx.Compat.parseInt((str) - rand(player.tou) - player.armorDef);
        player.takePhysDamage(damage, true);
    }
    ...
    public function searingSpurn() : Void
    {
        ...
        if (!player.immuneToBurn())
        {
            ...
            {
                player.createStatusEffect(StatusEffects.BurnDoT, SceneLib.combat.debuffsOrDoTDuration(3), 0.02, 0, 0);
            }
        }
    }
