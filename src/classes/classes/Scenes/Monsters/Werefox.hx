...
class Werefox extends Monster
{
    ...
    public function createElement() : Void
    {
        ...
        else if (player.hasPerk(PerkLib.FireAffinity) || player.hasPerk(PerkLib.FireShadowAffinity) || player.hasPerk(PerkLib.AffinityIgnis))
        {
            type = "ice";
        }
        else if (player.hasPerk(PerkLib.AquaticAffinity) || player.hasPerk(PerkLib.AffinityUndine))
        {
            type = "lightning";
        }
        else if (rand(2) == 0)
        ...
        var ElementDmg : Float = eBaseWisdomDamage() * 1.5;
        ElementDmg *= SoulskillMod();
        ElementDmg = Math.round(ElementDmg);
        if (type == "fire")
        {
            player.takeFireDamage(ElementDmg, true);
        }
        else if (type == "ice")
        {
            player.takeIceDamage(ElementDmg, true);
        }
        else if (type == "lightning")
        {
            player.takeLightningDamage(ElementDmg, true);
        }
        else if (type == "darkness")
        {
            player.takeDarknessDamage(ElementDmg, true);
        }
        else if (type == "water")
        {
            player.takeWaterDamage(ElementDmg, true);
        }
        else if (type == "earth")
        {
            player.takeEarthDamage(ElementDmg, true);
        }
        else
        {
            player.takeWindDamage(ElementDmg, true);
        }
    }
    ...
    public function new()
    {
        ...
        if (flags[kFLAGS.WEREFOX_EXTRAS] == 1)
        {
            ...
            this.drop = new WeightedDrop().add(consumables.BAGOCA2, 1).add(consumables.BAGOCA1, 3).add(consumables.DESERTB, 6);
        }
        if (flags[kFLAGS.WEREFOX_EXTRAS] == 2)
        {
            ...
            this.drop = new WeightedDrop().add(consumables.BAGOCA3, 1).add(consumables.BAGOCA2, 3).add(consumables.DESERTB, 6);
        }
        this.a = "the ";
        ...
        if (flags[kFLAGS.WEREFOX_EXTRAS] == 2)
        {
            IMutationsLib.WhiteFacedOneBirthrightIM.acquireMutation(this, "none", 2);
        }
        checkMonster();
    }
}
