...
class AngeloidGroup extends AbstractAngel
{
    ...
    {
        var damage : Float = eBaseWisdomDamage() * 3;
    ...
    private function AngelBaseAttack() : Void
    {
        ...
        var damage : Float = eBaseWisdomDamage() * 2;
    ...
    {
        clearOutput();
        var TB : Float = Math.round(touStat.core.value * 0.4);
        var SB : Float = Math.round(speStat.core.value * 0.2);
        var WB : Float = Math.round(wisStat.core.value * 0.6);
    ...
    override private function get_long() : String
    {
        ...
        if (hasStatusEffect(StatusEffects.TrueFormAngel))
        {
            str += " purple irises. Around their body are two to four circles each with two to four eyes looking the same as the main eye and above their heads are halo.";
        }
        else
        {
