...
class WeresharkScene extends BaseContent
{
    ...
    private function wsG() : Bool
    {
        return flags[kFLAGS.WERESHARKS_EXTRAS] < 2;
    }
    ...
    public function lostToWereshark() : Void
    {
        ...
        if (player.isRaceCached(Races.WERESHARK))
        {
            ...
            {
                outputText("Your injuries are mounting, your fins slowly give up support as you’re caught by the ocean. The current surrounds you as you’re unable to withstand his assault any longer. ");
            }
            else
            {
                outputText("Your body stiffens, heat growing in your loins. The cool water around you is like a gentle caress from a tender lover as you find yourself caught by the current. ");
            }
            ...
            if (player.hasVagina())
            {
                ...
                if (player.vaginalCapacity() < 79)
                {
                    outputText("He slowly rubs his member against your labia, testing your resistance before he finally pulls you in tighter, his erection pushing into you as you adjust to his presence.");
                }
                else
                {
                    outputText("He slowly rubs his member against your labia, testing your resistance only to bring himself closer to you, allowing his twin erections to push into you as you adjust to his presence.");
                }
            ...
            else
            {
                ...
                if (player.hasCock())
                {
                    outputText(" Your [cock] twitches and throbs as you cum, unloading everything you have to offer onto his chest." + ((player.cumQ() > 1999) ? " The water around you is starting to turn white just as the ocean relents to your virility." : ""));
                }
        ...
        else
        {
            if (player.HP <= player.minHP())
            {
                outputText("Your injuries are mounting, your cuts growing too deep as the saltwater pours into your wounds. You're caught by the current around you, unable to withstand his assault any longer. ");
            }
            else
            {
                outputText("Your body stiffens, heat growing in your loins. The cool water around you is like a gentle caress from a tender lover as you find yourself caught by the current. ");
    ...
    private function tfIntoWereshark() : Void
    {
        ...
        if (player.hasCoat())
        {
            ...
            {
                outputText("scales(end of cut)");
            }
            outputText("falls out replaced with");
        }
        else
        {
            outputText("you grow a full coat of");
        }
        ...
        if (player.hasCock())
        {
            outputText("Your " + ((player.normalCocks() == 0) ? "now fully human " : "") + "cock fattens and grows in size and length" + ((player.balls <= 2) ? ", a sloshing quartet of gonads forming beneath your twitching shaft" : ", a second pair of orbs falling into your ballsack and sloshing with fresh cum") + " as you harden to full mast shooting your semen into the moonlit water. ");
        }
        ...
        if (player.hasVagina() && player.vaginaType() != VaginaClass.SHARK)
        {
        ...
        {
            player.createPerk(PerkLib.Selachimorphanthropy, bonusStats, 0, 0, 0);
        }
        if (player.hasPerk(PerkLib.SelachimorphanthropyDormant))
        {
        ...
        player.statStore.replaceBuffObject({
                    ...
                    tou.mult : bonusStats * 0.05 * ngM,
                    spe.mult : bonusStats * 0.05 * ngM,
