...
class AbstractSuccubus extends Monster
{
    private function whipAttack() : Void
    {
        ...
        //Blind dodge change{
            ...
            if (this.short == "omnibus")
            {
                outputText("omnibus");
            }
            else if (this.short == "cow succubus")
            {
            ...
            if (temp == 0)
            {
                outputText("ass");
                damage = 4;
                player.takeLustDamage(6 + as3hx.Compat.parseInt(player.effectiveSensitivity() / 20), true);
            }
            //Whip yo tits!
            if (temp == 1)
            {
                ...
                damage = as3hx.Compat.parseInt(9 - player.armor.def);
                if (damage <= 0)
                {
                    outputText(" but you feel no pain thanks to your protection");
                    damage = 0;
                }
                player.takeLustDamage(4 + as3hx.Compat.parseInt(player.effectiveSensitivity() / 15), true);
            }
            //Whip yo groin
            if (temp == 2)
            {
                ...
                {
                    if (player.armor.perk == "Light")
                    {
                        groinProtection = 1;
                    }
                    else if (player.armor.perk == "Medium")
                    {
                        groinProtection = 2;
                    }
                    else
                    ...
                    if (player.armor == armors.BEEARMR || player.armor == armors.SSARMOR || player.armor == armors.DBARMOR || player.armor == armors.TBARMOR)
                    {
                        groinProtection = 2;
                    }  //The only thing protecting your groin is a loincloth.  
                    if (player.armor == armors.GOOARMR)
                    {
                        groinProtection = 2;
                    }  //Goo armor is soft and gooey so not complete protection.  
                ...
                {
                    if (player.lowerGarment == undergarments.SS_LOIN || player.lowerGarment == undergarments.SSPANTY)
                    {
                        groinProtection += 1;
                    }
                    else if (player.lowerGarment == undergarments.DS_LOIN || player.lowerGarment == undergarments.DSTHONG)
                    {
                        groinProtection += 2;
                    }
                }
                ...
                else
                {
                    if (player.gender == 0)
                    {
                        outputText("groin");
                        damage = 5;
                        damage = as3hx.Compat.parseInt(damage / (groinProtection + 1));
                    }
                    if (player.gender == 1)
                    {
                        outputText("groin, dealing painful damage to your [cocks], doubling you over in agony");
                        damage = as3hx.Compat.parseInt((player.maxHP()) / 4);
                    ...
                    if (player.gender == 3)
                    {
                        ...
                        damage = as3hx.Compat.parseInt((player.maxHP()) / 3);
                        if (groinProtection > 0)
                        {
                            outputText("! Of course, it would have been worse if you didn't wear groin protection");
                            damage = as3hx.Compat.parseInt(damage / (groinProtection + 1));
            ...
            outputText("! ");
            damage *= 0.1;
            var damage2 : Float = damage * this.weaponAttack;
    ...
    //[Kiss of Death Text]
    {
        ...
        if (player.getEvasionRoll())
        {
            ...
            if (player.lust >= maxLust() * 0.7)
            {
                outputText("  Maybe you shouldn't have bothered to move, it might have been fun.");
            }
        }
        //get hit
        else
        {
            ...
            {
                player.createStatusEffect(StatusEffects.KissOfDeath, 0, 0, 0, 0);
            }
        }
    }
    ...
    private function seduceAttack() : Void
    {
        ...
        //rand(breastRating) + breastRows*BreastperRow{
            ...
            //50+ breastsize% success rate{
                ...
                if (lust >= maxLust() * 0.5)
                {
                    outputText("  " + pronoun2 + " hard nipples seem to demand your attention.");
                }
