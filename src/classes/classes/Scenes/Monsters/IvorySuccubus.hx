...
class IvorySuccubus extends AbstractSuccubus
{
    ...
    public function succubusBarrage() : Void
    {
        ...
        else
        {
            ...
            if (rand((player.str / 3) + 10) < ((str / 3) + 10))
            {
                ...
                damage = as3hx.Compat.parseInt(30 + str + weaponAttack);
            }
            else
            {
                outputText("You bring your strength to bear and throw her off with only minor injuries. ");
                damage = as3hx.Compat.parseInt(Math.floor((15 + str) / 2) + weaponAttack);
            }
    ...
    public function new()
    {
        ...
        this.drop = new WeightedDrop().add(consumables.INCUBID, 1).add(consumables.SUCMILK, 1).add(consumables.SDELITE, 1);
