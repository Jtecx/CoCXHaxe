...
class Priscilla extends Goblin
{
    ...
    public function castSpell() : Void
    {
        ...
        if (lust > 75)
        {
            spellChooser = as3hx.Compat.parseInt(rand(3) + 3);
        }
        if (spellChooser == 0 && hasStatusEffect(StatusEffects.ChargeWeapon))
        {
            spellChooser = as3hx.Compat.parseInt(rand(5) + 1);
        }
        ...
        else if (spellChooser == 2 && mana >= spellCostWhitefire)
        {
            ...
            var damage : Int = as3hx.Compat.parseInt(inte + rand(50) * spellMultiplier());
        ...
        else if (spellChooser == 3 && mana >= spellCostArouse)
        {
            ...
            var lustDamage : Int = as3hx.Compat.parseInt((inte / 10) + (player.lib / 10) + rand(10) * spellMultiplier());
        ...
        else if (spellChooser == 4 && mana >= spellCostHeal)
        {
            ...
            var temp : Int = as3hx.Compat.parseInt(as3hx.Compat.parseInt(10 + (inte / 2) + rand(inte / 3)) * spellMultiplier());
        ...
        {
            outputText("[Themonster] tries to cast something, but fails miserably. Seems like she's out of mana!");
        }
    }
    ...
    public function slash() : Void
    {
        ...
        else
        {
            ...
            //Get hit
            var damage : Int = as3hx.Compat.parseInt(str + weaponAttack + rand(40));
            if (flags[kFLAGS.PRISCILLA_LVL_UP] >= 1)
            {
                damage += as3hx.Compat.parseInt((flags[kFLAGS.PRISCILLA_LVL_UP] + rand(15)) * 4);
            }
            if (damage < 40)
            {
                damage = 40;
    ...
    public function shieldBash() : Void
    {
        ...
        else
        {
            ...
            var damage : Int = as3hx.Compat.parseInt(str + rand(10));
            if (flags[kFLAGS.PRISCILLA_LVL_UP] >= 1)
            {
                damage += as3hx.Compat.parseInt(flags[kFLAGS.PRISCILLA_LVL_UP] + rand(15));
            }
            if (damage < 10)
            {
                damage = 10;
    ...
    private function spellMultiplier() : Float
    {
        var mult : Float = 1;
    ...
    public function new()
    {
        ...
        else
        {
            ...
            var lvlMulti : Float = flags[kFLAGS.PRISCILLA_LVL_UP];
        ...
        this.drop = new WeightedDrop().add(consumables.GOB_ALE, 5).add(consumables.REDUCTO, 2).add(jewelries.POWRRNG, 1).addMany(1, consumables.L_DRAFT, 
                        consumables.PINKDYE, 
                        consumables.BLUEDYE, 
                        consumables.ORANGDY, 
                        consumables.GREEN_D, 
        ...
        {
            this.createPerk(PerkLib.PrestigeJobSpellKnight, 0, 0, 0, 0);
        }
