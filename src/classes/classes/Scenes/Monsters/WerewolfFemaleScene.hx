...
class WerewolfFemaleScene extends BaseContent
{
    ...
    public function introWerewolfFemale() : Void
    {
        ...
        {
            outputText("Your night-adapted eyes allow you to quickly notice a silver-furred female figure skulking towards you, the shadows doing a decent job at concealing her...But not from you. She pounces at you, but you saw her coming. You easily dodge her attack, readying for battle.");
        }
        else
        {
            outputText("Out of nowhere a fur-covered figure hits you from the side, bringing you down to the ground. You quickly shove it off, you manage to shove her back, and you see your assailant clearly for the first time. She appears to be a wolf girl of some sort.");
        }
    ...
    public function introWerewolfHuntress() : Void
    {
        ...
        {
            outputText("Out of nowhere a shadowy pounce on you and it’s barely if you manage to shove her back after overcoming the surprise attack. You focus your vision on your opponent and determine it to be a wolf girl of some sort.");
        }
        outputText("She growls at you as she circles your position, claws and fangs out in a menacing display as she looks for an opening. This She-wolf seems intent on fighting you.");
    ...
    public function rapeMenu() : Void
    {
        ...
        if (player.hasCock())
        {
            addButton(0, "Dominate", domWithCock).hint("Show that pup who's the boss.");
        }
        else
        {
        ...
        if (player.hasVagina())
        {
            addButton(1, "Femdom", femDom).hint("You're top girl here. It's time to show it!");
        }
        else
        {
    ...
    public function domWithCock() : Void
    {
        ...
        if (player.hasKnot())
        {
            outputText(", your knot expanding, trapping your shaft inside them");
        }
        outputText(".\n\n");
        ...
        if (player.hasKnot())
        {
            outputText(" or [monster his] cunt");
        }
        ...
        if (player.isAnyRaceCached(Races.WEREWOLF, Races.WOLF, Races.CERBERUS) && player.hasMutation(IMutationsLib.AlphaHowlIM) && (LunaFollower.WerewolfPackMember < 5 * player.perkv1(IMutationsLib.AlphaHowlIM)))
        {
            if (rand(100) >= 80 || flags[kFLAGS.WEREWOLF_PACK_PITY_SYSTEM] > 3)
            {
                ...
                if (flags[kFLAGS.WEREWOLF_PACK_PITY_SYSTEM] > 3)
                {
                    flags[kFLAGS.WEREWOLF_PACK_PITY_SYSTEM] = 0;
                }
                LunaFollower.WerewolfPackMember += 1;
            }
            else
            {
                flags[kFLAGS.WEREWOLF_PACK_PITY_SYSTEM]++;
            }
        }
        else
        {
            ...
            {
                outputText(", preferring escape over subservience. Well it looks like you didn't get that one, maybe next time.\n\n");
            }
            outputText(".\n\n");
        }
    ...
    public function femDom() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("<i> and get to pumping my tool</i>");
        }
        outputText("!\"\n\n");
        ...
        if (player.hasCock())
        {
            outputText(" and [monster his] paws jerks your " + cockDescript(x) + "");
        }
        ...
        if (player.hasCock())
        {
            outputText(" as your " + cockDescript(x) + " unloads, covering [monster his] chest up to [monster his] belly in semen");
        }
        if (player.isRaceCached(Races.WEREWOLF))
        {
            outputText(" while you howl to the moon, savoring your mind blowing orgasm");
        }
        outputText(".\n\n");
