...
/**
	 ...
	 * @author ...
	 */
class CorruptedDrider extends AbstractSpiderMorph
{
    ...
    public function driderKiss() : Void
    {
        ...
        if (player.getEvasionRoll())
        {
            ...
            if (temp == 0)
            {
                outputText("I just wanted to give my delicious morsel a kiss...</i>\"\n");
            }
            else if (temp == 1)
            {
                outputText("Why won't you let me kiss you?</i>\"\n");
            }
            else if (temp == 2)
            {
                outputText("Mmm, do you have to squirm so much, prey?</i>\"\n");
            }
            else
            {
                outputText("Just look at my glossy, dripping lips.  Imagine how great it would feel to have them locked against you.  Why resist?</i>\"\n");
            }
        }
        ...
        //(HIT? + 10 lust){
            ...
            if (player.lust <= (0.9 * player.maxLust()))
            {
                outputText("  Somehow, you work up the willpower to back away, but your body slowly begins to burn hotter and harder, afflicted with a slowly-building lust.");
            }
        ...
        else
        {
            ...
            //(HIT? + 15 lust){
                ...
                if (player.hasCock())
                {
                    outputText("  " + SMultiCockDesc() + " spews a rope of pre-cum into your [armor], desperate to get out and fuck.");
                }
                if (player.hasVagina())
                {
                    outputText("  Fem-cum dribbles down your [legs] while your [clit] gets so hard you think it'll explode.");
                }
                outputText("  This time, the drider is the one to break the kiss.  She asks, \"<i>Are you ready, my horny little morsel?</i>\"\n");
                if (player.lust <= (0.9 * player.maxLust()))
                {
                    outputText("You shake your head 'no' and stand your ground!\n");
                }
            }
            //(Get hit 3rd+ time)
            else
            {
                ...
                if (player.hasCock() || player.hasVagina())
                {
                    ...
                    if (player.wetness() < 3 && player.cumQ() < 200)
                    {
                        outputText("small");
                    }
                    else if (player.wetness() < 5 && player.cumQ() < 500)
                    {
                        outputText("large");
                    }
                    else
                    ...
                    if (player.lust <= (0.9 * player.maxLust()))
                    {
                        outputText("  Amazingly, you resist her and pull back, panting for breath.");
                    }
                }
            }
        }
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        ...
        {
            SceneLib.hexindao.gaunletchallange2fight4();
        }
        else
        {
