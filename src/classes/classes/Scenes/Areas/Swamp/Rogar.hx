...
class Rogar extends BaseContent
{
...
((Scenario 4, [Frot]))*/
    ...
    public function encounterRogarSwamp() : Void
    {
        ...
        if (flags[kFLAGS.ROGAR_PHASE] == 0)
        {
            ...
            else
            {
                ...
                if (player.isNaga() || player.tailType > Tail.NONE)
                {
                    outputText("  Your tail flicks about happily.");
                }
        ...
        else if (flags[kFLAGS.ROGAR_PHASE] == 1)
        {
            ...
            else
            {
                ...
                if (player.lib >= 50)
                {
                    outputText("  Your loins burn");
                }
                else
                {
                    outputText("  Your nose crinkles");
                }
            ...
            if (player.biggestTitSize() >= 2 && player.lib >= 50)
            {
                ...
                if (player.inte <= 30)
                {
                    ...
                    if (player.hasVagina())
                    {
                        outputText("  When you witnessed him make that other beast submit with strength alone it created a demanding need in your " + vaginaDescript() + " during the match, and the memory is doing much the same now.");
                    }
                    //((if player has cock(s))
                    if (player.hasCock())
                    {
                        outputText("  You feel " + sMultiCockDesc() + " shift as you remember how hard you got from behind the tree, finding yourself absent mindedly stroking yourself with an open palm as pre-cum soaks through your clothing.");
                    }
            ...
            else if (player.biggestTitSize() >= 2)
            {
                ...
                if (player.inte < 30)
                {
                    outputText("Ro'gar apologizes for wrestling for his territory in your sight.");
                }
                //(int>=30)
                else
                {
                    outputText("Ro'gar apologizes for how messy his hut is.");
                }
            ...
            else if (player.lib < 50)
            {
                ...
                if (player.inte < 30)
                {
                    outputText("leftovers from that fight in the swamp still linger on his body.");
                }
                else
                {
                    outputText("it makes you wonder what Ro'gar was up to.");
                }
        ...
        //((high femininity or breasts >=B-cup)){
            ...
            else
            {
                ...
                //(Dirt Mc Girt = 2)
                if (flags[kFLAGS.ROGAR_DIRT] == 2)
                {
                    outputText("\"<i>[name], y'all took off on me real quick like,</i>\" he says through a snicker.  The joke falls flat and the orc seems to blush lime green as you stare at him uncomprehendingly.  He rubs the back of his neck with a hand before continuing.  \"<i>Sorry about that.  I was plumb tuckered out afterwards.</i>\"\n\n");
                }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            outputText("  Hands hook under your armpits to lift you up gently.");
                        }
                        //(if player height is taller than 7 feet)
                        if (player.tallness >= 84)
                        {
                            outputText("  A hand on your shoulder tugs you to lower your head.");
                        }
    ...
    private function waitForChunkyOrcLoe() : Void
    {
        ...
        if (flags[kFLAGS.ROGAR_PHASE] == 0)
        {
            ...
            if (player.lib < 50)
            {
                outputText("The hulking orc is surprisingly gentle as he cleans you like a young child.  The warm waters and careful touch soothe your tired muscles.  Your skin becomes textured with goose bumps as the tickles of droplets roll down your body and the filth is washed down to the waters below.  You find yourself sighing in relaxation from the treatment.  A smile curls your lips as you look down at Ro'gar, and he returns the pleased look with a toothy smirk.\n\n");
            }
            ...
            else
            {
                ...
                {
                    outputText("  " + SMultiCockDesc() + " stirs to life, making it obvious how much you are enjoying the rub down, then begins to drool with lust.  Through lidded eyes you faintly make out Ro'gar's toothy smirk becoming a hungry grin.");
                }
    ...
    private function okayBroLetsHaveAGayCarwash() : Void
    {
        ...
        //[(if player has tail)
        if (player.hasLongTail() > 0 || player.isNaga())
        {
            outputText("  Your tail brushes along Ro'gar's legs eagerly, tickling his thighs as you service him.");
        }
        ...
        {
            outputText("your " + assholeDescript());
        }
        ...
        if (player.inte < 30)
        {
            outputText("  That fight between him and his opponent went on for quite a while, so it makes some sense.");
        }
    ...
    public function rogarThirdPhase() : Void
    {
        ...
        else
        {
            ...
            if (player.bodyColor == "green")
            {
                outputText("  \"<i>Hey... Have y'ever had somebody randomly run up an' ask ya ta 'Waaagh!'?</i>\" he mutters quietly.  You give him a perplexed look.  The orc laughs off your confusion dismissively.  \"<i>Nothin', nothin'.</i>\"");
            }
            outputText("\n\n");
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        outputText("fantasize about those strong hands touching every part of you");
                    }
                    ...
                    if (player.hasLongTail() > 0 && player.isNaga())
                    {
                        outputText(", and you feel your tail flicking rapidly in excitement");
                    }
                    outputText(".\n\n");
    ...
    public function rogarPhaseFour() : Void
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (flags[kFLAGS.ROGAR_DIRT] == 1)
                {
                    outputText("</i>\"I know ya probably ain't ever imagined hangin' out with a feller like me, but... I could really use the company tonight.</i>\"  ");
                }
    ...
    private function rogarFuckMenu() : Void
    {
        ...
        if (player.hasItem(consumables.BROBREW))
        {
            brodown = brobrobrobro;
        }
        if (player.hasCock())
        {
    ...
    private function loseButtGinity() : Void
    {
        ...
        if (player.ass.analLooseness == 0)
        {
            ...
            if (player.hasBalls())
            {
                outputText("  Ro'gar's dark green cockhead rubs over your [balls] and coats the plump sack in his musky goo, like an alpha male marking his territory.");
            }
            outputText("\n\n");
            ...
            if (player.hasLongTail() || player.isNaga())
            {
                outputText("  Your tail wraps around Ro'gar's arm for some sort of stability, the tip flicking lazily as it coils and squeezes around a bicep.");
            }
            outputText("\n\n");
        ...
        else
        {
            ...
            if (player.tallness >= 84)
            {
                outputText("  You lean down as Ro'gar tilts up, your lips connecting with his, both mouths open to let tongues wrestle.");
            }
            //(If player height is less than 7 feet)
            if (player.tallness < 84)
            {
                outputText("  You lean up into Ro'gar's mouth, his hand caressing your cheek.");
            }
            ...
            if ((player.thickness >= 75 && player.tone < 33) || player.tone >= 75)
            {
                ...
                if (player.hasBalls())
                {
                    outputText(" and your [balls] clench tight as you feel your limit reached");
                }
                outputText(".");
                //[(if player has toes)
            ...
            else
            {
                ...
                if (player.hasLongTail() > 0 || player.isNaga())
                {
                    outputText("  Your tail flicks helplessly below you, urgently wrapping around Ro'gar's thigh and trying to pull him deeper into you.");
                }
                ...
                if (player.hasBalls())
                {
                    outputText("  You feel your [balls] pull up tight in your sack as your labored breath comes out in pants.");
                }
    ...
    private function fuckRogarsButtPussyBoyCuntManMoundSissySlitQueerQuim() : Void
    {
        ...
        {
            outputText("You reach a hand up to cup his chin, yanking it roughly towards your face as you plant a kiss on his lips.");
        }
        //(If player is taller than 7 feet)
        else if (player.tallness > 84)
        {
            outputText("You reach down and cup Ro'gar's chin, roughly yanking it up to crash his lips against yours.  ");
        }
        ...
        else if (player.shortestCockLength() > 18)
        {
            ...
            if (player.cockTotal() > 1)
            {
                outputText(SMultiCockDesc() + " grinds over every inch of Ro'gar's front, and his dark olive skin soaks in your glistening fluids.  Ro'gar's length is lost in the forest of your meat.  With his green monster assaulted by multiple angles and dwarfed in size he whimpers softly.  \"<i>Ain't never felt nothin' like this.  Almost like fuckin' a dick.</i>\"");
            }
            //(else If single cock)
            else
            {
                outputText("Your [cocks] bucks against Ro'gar's orcish pride.  The pair of hard cocks press tightly together as you dwarf Ro'gar in size.  You hump his orc sex with your [cocks] and grunt in strain; your hands reach down to grip the pair of manhoods tightly, jerking them fast with squeezed pumps.");
            }
            //(If player has balls)
            if (player.hasBalls())
            {
                outputText("  Ro'gar squirms under you, his chest heaving with labored breath while his musky scrotum presses against your own.");
            }
        ...
        else
        {
            ...
            {
                outputText("  You grind around in his green pucker, the barbs working over Ro'gar's inner walls.  \"<i>W-what in tarnation?</i>\" Ro'gar grunts as you watch him squirm under you.  You buck your hips, digging deeper into orcflesh with each thrust, his muscled ass tensing with each slam of your hips.");
            }
            //(If horse cock)
            else if (player.cocks[0].cockType == CockTypesEnum.HORSE)
            {
                outputText("  The flared head of your [cock] nestles in Ro'gar's hole.  His breath quickens as you push in, the girthy, flared head stretching Ro'gar's insides impossibly.  You simply thrust and buck you way deeper within the green pucker to force more of your lengthy shaft past that olive rosebud each time.\n\n");
            }
            //(If dog cock)
            else if (player.hasKnot(0))
            {
                outputText("  Your pointed tip slips in easily and you begin to plunge into Ro'gar's depths, and the burly orc gives satisfied grunts as his strong hand reaches down to touch himself.  You begin to fill his orc hole, the pre-slicked cock working as a lubing tool as you begin to build a nice rhythm within Ro'gar.  Your knot bulges as your climax approaches and begins to ram into Ro'gar's green pucker, the tight ring unable to allow entry as you swell bigger and bigger.");
            }
            //(If other cock)
            else
            {
                outputText("  Breaking Ro'gar's orc hole open, you thrust deep within his hot, tight depths.  The burly orc pants with heated breaths, humid air fluttering out of his tusked mouth.  His musclegut heaves, each breath punctuating the thrusts of your hips.  You plow his ass roughly, pulling out until only the head remains inside him before plunging back in further than before.  Ro'gar grunts low in his chest with each bit of progress made, his strong, calloused hand stroking his drooling cock.  \"<i>Mmmf, [name],</i>\" he groans as you finally hilt yourself within him.");
            }
            ...
            {
                outputText("  Your other cocks grind over that firm orc rump, drooling over his skin and making his rear glisten with your sex.");
            }
            else if (player.cockTotal() > 1)
            {
                outputText("  Your other cock grinds over that firm orc rump, drooling over his skin and making his rear glisten with your sex.");
            }
            ...
            if (player.hasKnot(0))
            {
                outputText("  Your knot forces past Ro'gar's tight, anal ring to pulse inside him, joining the pair of you together.");
            }
            //[(if player has balls)
            if (player.hasBalls())
            {
                outputText("  With short jerks of your hips, you begin to unload your pent up orbs into the orc's innards.");
            }
            ...
            if (player.cockTotal() > 1)
            {
                outputText("  The other meat rubbing over his olive skin outside adds to the coating of spunk, making a giant mess of Ro'gar's body.  In seconds he resembles a well-paid whore beneath your [cocks].");
            }
            outputText("\n\n");
        }
        //end right fit path
    ...
    private function takeDatBroBrewFromDaBigMeanOlOrc() : Void
    {
        ...
        if (changed)
        {
            ...
            if (cocked)
            {
                outputText(", and you smile to yourself as your now-larger endowment" + ((player.armorDescript() != "gear") ? " rubs against garments not suited to it" : " flops freely in the air") + "");
            }
    ...
    private function frotWithRogar() : Void
    {
        ...
        else
        {
            ...
            if (player.hasBalls())
            {
                outputText("Your [balls] slap audibly against Ro'gar's heavy green sack.  ");
            }
    ...
    private function kinkyWithDaOrc() : Void
    {
        ...
        if (player.lib < 33)
        {
            outputText("feigned confusion ");
        }
        else if (player.lib > 66)
        {
            outputText("clear lust ");
        }
        ...
        {
            outputText("\"<i>Tarnation, [name].  You're so tight...</i>\"\n\n");
        }
        else if (player.ass.analLooseness < 5)
        {
            outputText("\"<i>My favorite boy ready for the ride of his life?</i>\"\n\n");
        }
        ...
        if (player.ass.analLooseness < 3)
        {
            outputText("the tight hole.  Your teeth clench as he stretches you wide around his length, though he continues without hesitation, and finally you cry out with the sting of the bulbous head squeezing into your tiny hole, though it falls on deaf ears as Ro'gar merely groans at your tightness.");
        }
        else if (player.ass.analLooseness < 5)
        {
            outputText("your well-used hole. You moan with need as he stretches you wide around his length, continuing without hesitation until he's inside.");
        }
        player.buttChange(25, true, true, false);
