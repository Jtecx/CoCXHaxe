...
class SpiderMorphMob extends Monster
{
    //==============================
    ...
    //SPIDER HORDE ATTACK - Miss (guaranteed if turns 1-3 and PC lost to Kiha)
    {
        ...
        else
        {
            ...
            var damage : Int = as3hx.Compat.parseInt(as3hx.Compat.parseInt((str + weaponAttack) - rand(player.tou) - player.armorDef) + 20);
            if (damage > 0)
            {
    ...
    {
        if (rand(2) == 0 || player.hasStatusEffect(StatusEffects.UBERWEB))
        {
