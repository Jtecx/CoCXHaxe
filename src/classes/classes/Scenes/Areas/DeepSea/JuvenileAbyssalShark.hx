...
class JuvenileAbyssalShark extends Monster
{
    ...
    public function createElement() : Void
    {
        ...
        var ElementDmg : Float = eBaseWisdomDamage();
        ElementDmg *= 2;
    ...
    private function abyssalSharkBiteAttack() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.CombatWounds) && player.statusEffectv1(StatusEffects.CombatWounds) < 0.99)
        {
        ...
        if (!player.immuneToBleed())
        {
            ...
            {
                player.createStatusEffect(StatusEffects.Hemorrhage, SceneLib.combat.debuffsOrDoTDuration(3), 0.1, 0, 0);
            }
        }
    }
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (chooser < 2)
        {
            if (this.soulforce >= 60)
            {
                createElement();
            }
            else
            {
                abyssalSharkBiteAttack();
            }
        }
        if (chooser == 2)
        {
            if (this.soulforce >= 90)
            {
                castHailOfSwordfishesSoulskill();
            }
            else
            {
                abyssalSharkBiteAttack();
    ...
    public function new()
    {
        ...
        this.level = 73;
        this.gems = 0;
        this.drop = new WeightedDrop().add(consumables.BAGOCA1, 1).add(consumables.SHARK_T, 1).add(consumables.ASTOOTH, 3);
