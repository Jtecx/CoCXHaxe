...
class SandWitchScene extends BaseContent implements TimeAwareInterface
{
    ...
    public static var rapedBefore : Bool = false;
    ...
    private function sandwitchRape() : Void
    {
        spriteSelect(SpriteDb.s_sandwich);
    ...
    private function centaurStart() : Void
    {
        ...
        {
            outputText("and torn in many places. ");
        }
        //[win via lust]
        else
        {
            outputText("and her fingers squeezing her nipples as lust overwhelms her. ");
        }
        //[has breasts]
        if (player.biggestTitSize() > 3)
        {
            ...
            if (monster.HP <= monster.minHP())
            {
                outputText("is unable to stop herself from slowly licking her lips. ");
            }
            //[win via lust]
            else
            {
                outputText("sensually licks her parted lips. ");
            }
        }
        //[lactating]
    ...
    private function centaurRide() : Void
    {
        ...
        {
            outputText("; your " + nippleDescript(0) + "s  become stiff");
        }
        //[cock only]
        if (player.cockTotal() > 0 && !player.hasVagina())
        {
            outputText(" and your [cock] hardens.");
        }
        //[cunt only]
        if (player.hasVagina() && player.cockTotal() == 0)
        {
            outputText(" and your " + vaginaDescript(0) + " begins to drip with moisture.");
        }
        //[cock and cunt]
        if (player.hasVagina() && player.cockTotal() > 0)
        {
            outputText(" and your [cock] hardens as your " + vaginaDescript(0) + " begins to drip with moisture.");
        }
        if (player.gender == 0)
        {
            outputText(" her.");
        ...
        if (player.biggestTitSize() > 0)
        {
            outputText(", eyes fixed on your " + player.allBreastsDescript());
        }
        outputText(".\n\n");
        ...
        if (player.str > 50)
        {
            outputText("pick her up and toss her onto your back. ");
        }
        //[STR < 50]
        else
        {
            outputText("offer her your hand and help her onto your back. ");
        }
        ...
        if (player.biggestTitSize() > 0)
        {
            outputText("fondle your [allbreasts] and ");
        }
        ...
        //[lactating]{
            ...
            if (player.biggestLactation() >= 1)
            {
                ...
                if (player.biggestLactation() < 3)
                {
                    outputText("small streams running out ");
                }
                //[heavy lactation]
                else
                {
                    outputText("torrents gushing out ");
                }
    ...
    private function centaurRape() : Void
    {
        ...
        if (player.str >= 50)
        {
            outputText("easily tear them off of her. ");
        }
        //[STR < 50]
        else
        {
            outputText("drag her a bit until they come off. ");
        }
    ...
    private function centaurDick() : Void
    {
        ...
        var tenta : Bool = (x >= 0 && cocks != 2);  //twin cunts = no assfuck  
        if (!tenta)
        {
        ...
        {
            outputText("After aligning your [cocks] to her twin cunts, you push yourself deeply into her, ");
        }
        //[3+ cocks]
        else
        {
        ...
        if (player.cocks[x].cockLength >= 12)
        {
            if (cocks == 1)
            {
                outputText("Her body surrenders more and more as your " + player.cockDescript(x) + " presses against her cervix. ");
            }
            else
            {
                outputText("Her body surrenders more and more as your [cocks]  press against her cervixes. ");
            }
        ...
        {
            outputText(", and her cries have long since turned into groans of pleasure. ");
        }
        outputText("You cannot hold off your orgasm any longer. Your [cocks] explode" + ((cocks == 1) ? "" : "s") + ", ");
        //[large cum production]
        if (player.cumQ() >= 250)
        {
            outputText("gushing massive amounts of your cum ");
        }
        //[regular cum production]
        else
        {
            outputText("pushing your sperm ");
        }
    ...
    private function centaurVA_start() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText("and your " + vaginaDescript(0) + " dripping with desire");
        }
    ...
    private function centaurVag() : Void
    {
        ...
        {
            outputText("one of her fingers ");
        }
        //[cunt size is normal]
        else if (player.vaginalCapacity() < 20)
        {
            outputText("several of her fingers ");
        }
        //[cunt size is large]
        else
        {
        ...
        {
            outputText("her tongue ");
        }
        //[anus size is normal]
        else if (player.analCapacity() < 20)
        {
            outputText("two of her fingers ");
        }
        //[anus size is large]
        else
        {
    ...
    private function knotSandwitch(x : Int) : Void
    {
        ...
        if (player.cumQ() < 25)
        {
            outputText("The cum slicks your dick as you continue trying to hump, easing the over-sensation of your dick while you're glued together by the bulbous knot locked in her snatch.");
        }
        if (player.cumQ() >= 25 && player.cumQ() < 100)
        {
            outputText("Your cum expands her pussy, stretching her out and leaving your " + player.cockDescript(x) + " swimming in your spunk, trapped within her tight-stretched walls by your massive knot.");
        }
        if (player.cumQ() >= 100 && player.cumQ() < 500)
        {
            outputText("Your cum begins to pump into her corked body, filling her hole to the brim.  Surely the pressure is piling the jizz deep into her womb, and after what feels like forever, the overfull spunk shoves back on your knot, a little dribbling out around the edges of her taut feminine folds as her pussy reaches its limit.");
        }
        if (player.cumQ() >= 500)
        {
            outputText("Your cum spurts into her in rapid jets, coating the inside of her walls briefly white as more continues to pour into her. Her cunt begins to fill, and as she strugles to relieve the pressure that continues to pump into her, she only succeeds in spurring your orgasm to a new height.  You continue to blast within her, your knot holding her fast as she squirms, each shot of cum from your prick managing to squirt a tiny bit of jizz from your locked genitals.");
        }
        player.sexReward("vaginalFluids", "Dick");
        cleanupAfterCombat();
    }
    ...
    @:allow(classes.scenes.areas.desert)
    private function beatSandwitch() : Void
    {
        spriteSelect(SpriteDb.s_sandwich);
        clearOutput();
        ...
        {
            outputText("You smile in satisfaction as the [monster name] drops down on all fours and struggles to rise.  ");
        }
        //lust check
        if (player.lust < 33)
        {
        ...
        if (silly())
        {
            addButton(2, "Use 3i@-", missingoSex);
        }
    ...
    public function sexMenu() : Void
    {
        ...
        for (i in 0...player.cocks.length)
        {
            ...
            else if (normalDick < 0 || player.cockArea(i) > player.cockArea(normalDick))
            {
                normalDick = i;
            }
        }
        ...
        {
            addButtonDisabled(6, "???", "Req. to be posessed by some ghost");
        }
    ...
    private function sandwitchGetsDildoed() : Void
    {
        ...
        if (player.cor > 50)
        {
            EngineCore.outputText("lick it clean, ");
        }
    ...
    private function ovipositSandWitches() : Void
    {
        ...
        if (monster.HP <= monster.minHP())
        {
            outputText("bruised and battered");
        }
        else
        {
            outputText("lusty");
        ...
        if (monster.HP <= monster.minHP())
        {
            outputText("fright as she clamors to get away.");
        }
        else
        {
            outputText("an uneasy curiosity to see what exactly you have planned.");
        }
        ...
        if (player.hasCock())
        {
            outputText("hardened  " + multiCockDescriptLight());
        }
        if (player.gender == 3)
        {
            outputText(" and ");
        ...
        {
            outputText(num2Text(player.cockTotal() + 1));
        }
        else
        {
            outputText("two");
    ...
    private function laySomeEggsInThatWitchFinally() : Void
    {
        ...
        if (cunt)
        {
            outputText("folds of her vulva");
        }
        else
        {
            outputText("ring of her anus");
        }
        ...
        if (cunt)
        {
            outputText("ribbed contours of her pussy");
        }
        else
        {
            outputText("tight fit of her innards");
        }
        ...
        if (cunt)
        {
            outputText("opening of your lover's cervix");
        }
        else
        {
            outputText("deepest recesses of her sphincter");
        }
        ...
        if (cunt)
        {
            outputText("at the waiting lips of the sand witch's vagina");
        }
        else
        {
            outputText(" through her rectal walls");
        }
        ...
        if (cunt)
        {
            outputText("into her waiting womb");
        }
        else
        {
            outputText("as far as you can safely tuck it");
        }
        ...
        //(Bee, non-horse:
        if (player.canOvipositBee() && !player.isTaur())
        {
            outputText("torso");
        }
        else
        ...
        if (player.cockTotal() > 1 || player.gender > 2)
        {
            outputText("es");
        }
        ...
        if (player.hasCock())
        {
            outputText("take hold of your [cocks] and bring it");
        }
        else
        {
            outputText("bring your " + vaginaDescript(0));
        }
        ...
        if (facial == 0)
        {
            outputText("her facial features with proof of your climax");
        }
        else if (facial == 1)
        {
            outputText("the interior of her mouth with sweltering streams of your cum");
        }
        ...
        if (player.fertilizedEggs() > 0)
        {
            ...
            {
                outputText("A little ghost pops up. \"<i>Looks like something went wrong determining what kind of ovipositor you have. You should report this to the coders, along with what race you are.</i>\"");
            }
        }
        player.dumpEggs();
        player.sexReward("vaginalFluids", "Dick");
    ...
    private function sandwitchBirthsYourMonstrosities(reluctantly : Int) : Void
    {
        ...
        if (reluctantly == 0)
        {
            outputText(", with a quick kiss on the lips,");
        }
    ...
    public function witchBirfsSomeBees() : Void
    {
        ...
        if (silly())
        {
            outputText(", echoing tiny little cries of \"<i>@</i>\"");
        }
    ...
    private function sandwitchSpanking() : Void
    {
        ...
        if (monster.HP <= monster.minHP())
        {
            outputText("  You can't even stand up!");
        }
        else
        {
            outputText("  You can't even stop masturbating, can you?");
        }
        outputText("</i>\"  You kick her hand away, and she tumbles down into a distraught heap, crying softly");
        if (monster.lust >= monster.maxOverLust())
        {
            outputText(" but still masturbating");
        }
        ...
        if (player.cor < 33)
        {
            outputText("and though you feel kind of bad about using her this way, she likely would have done worse to you.");
        }
        else if (player.cor < 66)
        {
            outputText("and though you aren't sure you ought to be using her this way, she likely would have done worse to you.");
        }
        else
        {
            outputText("and though you used to find things like this distasteful, you don't really give a shit anymore.");
        }
        ...
        if (player.hasCock())
        {
            outputText("Worse still, [eachCock] is now unprotected and pressed against the rough stone by your body-weight.  It's less than pleasant.  ");
        }
        else if (player.hasVagina())
        {
            outputText("Worse still, your [vagina] is pressed up against the rough stone.  It's less than pleasant.  ");
        }
        outputText("You bristle at your helplessness");
        if (player.hasPerk(PerkLib.FireLord) || player.hasPerk(PerkLib.Hellfire) || player.hasPerk(PerkLib.DragonFireBreath))
        {
            outputText(", breathing out a gout of flame in rage, but you can't even direct the blast properly like this.  All it does is melt some of the sand below into glass");
        }
        else
        {
            outputText(", but there's nothing you can do");
        }
        ...
        if (player.cor < 50)
        {
            outputText("She sounds almost fanatical in her dedication to her order.");
        }
        else
        {
            outputText("What the hell is she on about?  Does she actually think her little torture session is going change your attitude?");
        }
