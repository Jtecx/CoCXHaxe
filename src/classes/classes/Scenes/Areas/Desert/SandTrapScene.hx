...
class SandTrapScene extends BaseContent
{
    private var sandTrap(get, never) : SandTrap;

    private function get_sandTrap() : SandTrap
    {
        return try cast(monster, SandTrap) catch(e:Dynamic) null;
    }
    ...
    public function sandtrapmentLoss(clear : Bool = false) : Void
    {
        if (flags[kFLAGS.SANDTRAP_LOSS_REPEATS] >= 2 && player.eyes.type == Eyes.BLACK_EYES_SAND_TRAP && player.wings.type == Wings.GIANT_DRAGONFLY)
        {
        ...
        {
            doNext((player.hasCock()) ? dickwieldersLoseToSandTarps : (player.hasVagina()) ? chicksLoseToSandTarp : genderlessLoss);
        }
    }
    
    @:allow(classes.scenes.areas.desert)
    private function pcBeatsATrap() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_sandtrap);
        ...
        {
            outputText("The sandtrap groans and collapses forwards into the dirt.  With the creature no longer controlling it, the sand underneath you feels a great deal more stable... climbing out won't be too difficult.");
        }
        //PC lust victory:
        else
        {
            outputText("The sand around you stops sinking.  Overpowered with lust, the sandtrap moans and gives up control over itself and its pit to desperately run one set of hands over its flat chest whilst the other pushes into the sand to masturbate... whatever it has down there.  With your foe in this state, climbing out won't be so difficult now.");
        }
        ...
        if (player.lust >= 33)
        {
            ...
            {
                addButtonDisabled(0, "???", "Requires naga lower body, naga fangs, and to have a certain Naga lover. (not genderless)");
            }
    ...
    private function dickwieldersLoseToSandTarps() : Void
    {
        ...
        //6 inches to 15:
        else if (player.cocks[0].cockLength < 15)
        {
        ...
        if (player.cumQ() > 1000)
        {
            outputText("  The sandtrap gasps and coos as you ejaculate over and over again, and beneath you feel your cum wet the sands as it beads around the clenching grip it has you in.");
        }
        if (player.hasVagina())
        {
            outputText("  Ignored entirely, your " + player.vaginaDescript(0) + " quivers and orgasms in tandem, flexing and wetting fruitlessly against the dry water pressing against it.");
        }
        ...
        {
            outputText("  Although the oil that coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature's ovipositor and stretches your tight hole wide.");
        }
        else
        {
            outputText("  Your accommodating ass feels like it was made for this purpose, and with the creature's oil soothing your thoughts and reflexes you savor the creature taking you like this as if it were your quim.  You moan as something round travels down the creature's ovipositor and into your wet ass, the sensation making [eachCock] hard again.");
        }
    ...
    private function chicksLoseToSandTarp() : Void
    {
        ...
        {
            outputText("  \"<i>A cute little wwworker hasz fallen into my home.  Wwwell, that isz ok.  Fortunately for hwyou, I already have some eggsz ready.  Hhyou will get to be a queen! Iszn't that exciting?</i>\"");
        }
        else
        {
            outputText("  \"<i>A cute little wwworker hasz fallen into my home, but I have no fertile eggsz to givve it.  Wwwell, that isz ok.  Hwwe can still have fun, can't we?</i>\"");
        }
        ...
        {
            outputText("Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature's ovipositor and stretches your tight ass wide.");
        }
        else
        {
    ...
    private function genderlessLoss() : Void
    {
        ...
        if (player.gender == 0)
        {
            ...
            if (monster.hasStatusEffect(StatusEffects.Fertilized))
            {
                outputText("\"<i>Not a worker OR a drone! Wwwell, that isz ok.  Even ants who hhhave nothing can be someone for somebody.  And hhyou can be mommy for my childrrhen!</i>\"");
            }
            else
            {
                outputText("\"<i>Not a worker OR a drone! Wwwell, that isz ok.  Hwwe can still have fun, can't we?</i>\"");
            }
        }
        ...
        if (player.analCapacity() < 30)
        {
            outputText("Although the oil that coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature's ovipositor and stretches your tight ass wide.");
        }
        else
        {
            outputText("Your accommodating ass feels like it was made for this purpose, and with the creature's oil soothing your thoughts and reflexes you savor the creature taking your [asshole] like this as if it were a quim.  You moan as something round travels down the creature's ovipositor and into your wet ass.");
        }
    ...
    public function birfSandTarps() : Void
    {
        ...
        if (player.gender == 1)
        {
            ...
            {
                outputText("  Your oily hands descend upon your cock, and you massage your shaft as you feel the pressure in your bowels intensify again.");
            }
        }
        //Female:
        else if (player.gender == 2)
        {
            ...
            if (!player.isTaur())
            {
                outputText("  Your oily hands push softly into your cleft, fingering your needy " + player.clitDescript() + " as you feel the pressure in your bowels intensify again.");
            }
        }
        //Herm:
        else if (player.gender == 3)
        {
            ...
            if (!player.isTaur())
            {
                outputText("  Your oily hands descend upon your genitals and you begin to slowly pump your shaft and finger your needy " + clitDescript() + " as you feel the pressure in your bowels intensify again.");
            }
        }
    ...
    private function useSandTarpsHands() : Void
    {
        ...
        //find longest cocks, prepare for tags
        {
            ...
            switch (player.cockTotal())
            {
                ...
                case 3:
                    ...
                    if (player.hasBalls())
                    {
                        outputText("  It grins up at you shyly and, with its final hand, cups your [balls].  ]You laugh with an almost hysterical edge to the sky at how mindboggling getting a triple handjob feels, and the trap giggles slightly as well despite itself.");
                    }
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            if (player.tone < 60)
            {
                outputText("  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands sinking deep into your soft, pillowy behind as it clutches onto you.");
            }
            else
            {
                outputText("  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands clutching onto your tight, muscled behind so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
            }
        ...
        {
            var x1 : Array<Dynamic> = [];  //longest cocks  
            for (i in 0...3)
            {
                x1[i] = player.findCock(i + 1, -1, -1, "length") + 1;
            }
    ...
    private function rideDatSandTarpLikeIts1999() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("  [EachCock] flexes in delicious concert with your quim, spurting jizz onto the trap's chest and face.");
        }
    ...
    private function stickWangInSandgina() : Void
    {
        ...
        var scene : Int->Void = function(x : Int) : Void
        {
            ...
            if (player.hasBalls())
            {
                outputText(", makes your [balls] swell in readiness,");
            }
            ...
            //8 inches to 35:
            else if (player.cocks[x].cockLength < 35)
            {
            ...
            else
            {
                ...
                if (player.hasBalls())
                {
                    outputText("  Your [balls] slap against the creature's wet opening.  ");
                }
            ...
            if (silly())
            {
                outputText(", though with fewer pouting blue lips and doe eyes");
            }
            ...
            if (player.hasBalls())
            {
                outputText("balls ache and your ");
            }
    ...
    private function nagaThreesomeWithSandTrap() : Void
    {
        ...
        function dickF() : Void
        {
            ...
            if (player.cocks[0].cockLength < 14)
            {
                ...
                if (player.hasBalls())
                {
                    outputText("slapping your [balls] into its nose, forcing your cock all the way to the top of its tight throat.");
                }
            }
            //14 inches or more:
            else
            {
                ...
                {
                    outputText("beading sweat, ");
                }
                outputText("pistoning into its submissive opening with fervor, ");
                if (player.hasBalls())
                {
                    outputText("your [balls] swinging heavily in front of the sandtrap's nose, ");
                }
            ...
            if (player.cocks[0].cockLength >= 14)
            {
                outputText("  You're not sure, which is the more pleasing sight; her, or the rhythmic bulging in the sandtrap's neck.");
            }
        ...
        function vagF() : Void
        {
            ...
            if (player.wetness() < 4)
            {
                outputText("  Its gentle worship of your " + player.vaginaDescript(0) + " soon has you streaming rivulets of girlcum from your oversexed hole, dribbling down the sandtrap's face as you eagerly press yourself down for more.");
            }
            ...
            if (player.cumQ() > 1000)
            {
                outputText("  The long, sensuous tongue bath has pushed your body into overdrive and the waterfall you unleash is even more uncontrollable than the last; you wonder dizzily if you haven't just lost control of your bladder.  The hypnotised sandtrap gladly drinks what you give it, but it's far too much and soon it is coughing your girlcum back up, through its nose and back onto your still contracting " + player.vaginaDescript(0) + ".");
            }
    ...
    private function sandTrapBadEnd() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("[EachCock] hardens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap's will.  ");
        }
        //Female:
        else if (player.hasVagina())
        {
            outputText("Your [vagina] moistens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap's will.  ");
        }
    ...
    private function desztiny() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("Your cock strains upwards and you moan; feeling as sensitive as you do, you are both desperate to be touched and desperate not to be.  ");
        }
        //Female:
        else if (player.hasVagina())
        {
            outputText("You feel but barely hear the drip of moisture on the sand, both your [asshole] and [vagina] beading with oily need.  ");
        }
    ...
    private function sandTrapBadEndFinale() : Void
    {
        ...
        if (player.gender > 0)
        {
            ...
            if (player.hasCock())
            {
                outputText("cock harden");
            }
            else
            {
                outputText("vagina dribble");
            }
        }
        ...
        if (player.hasCock())
        {
            outputText(" and masturbating [oneCock] desperately");
        }
        else if (player.hasVagina())
        {
            outputText(" and fingering your [vagina] desperately");
        }
