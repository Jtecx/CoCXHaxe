...
class AnubisScene extends BaseContent
{
    ...
    {
        clearOutput();
        var genderA : Float = 0;  //male  
        if (rand(2) == 0)
        {
            genderA = 1;
        }
        var skinA : Float = 0;  //dark-skinned  
        if (rand(2) == 0)
        {
            skinA = 1;
        }
        ...
        if (genderA == 1)
        {
            outputText(", her " + ((skinA == 1) ? "obsidian" : "dark-skinned") + " breast exposed to the wind. ");
        }
        else
        {
            outputText(". His " + ((skinA == 1) ? "obsidian" : "dark-skinned") + " chest glistening with sweat, exposed to the wind. ");
        }
    ...
    public function anubisDefeated(genderA : Float = 0) : Void
    {
        clearOutput();
        if (monster.HP < 1)
        {
    ...
    public function anubisWon(genderA : Float = 0) : Void
    {
        clearOutput();
