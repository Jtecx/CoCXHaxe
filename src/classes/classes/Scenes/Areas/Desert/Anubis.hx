...
class Anubis extends Monster
{
    ...
    public var anubisGender : Float = 0;
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (choice == 2)
        {
            if (player.hasStatusEffect(StatusEffects.LockingCurse) && player.statusEffectv1(StatusEffects.LockingCurse) == 1)
            {
                anubisSoulDrain();
            }
            else
    ...
    public function anubisFingerOfDeath() : Void
    {
        ...
        var dmgW2 : Float = this.weaponAttack * 3;
    ...
    public function new(genderA : Float, skinA : Float)
    {
        super();
        this.a = "the ";
        this.short = "anubi";
        ...
        if (skinA == 1)
        {
            this.bodyColor = "obsidian";
        }
        else
        {
            this.bodyColor = "dark-skinned";
        }
        ...
        this.drop = new WeightedDrop().add(useables.BANDAGE, 1).add(consumables.JACKALD, 1).add(weapons.JUDGE_S, 2).add(headjewelries.DEATHPR, 2).add(armors.DEATHPO, 2);
