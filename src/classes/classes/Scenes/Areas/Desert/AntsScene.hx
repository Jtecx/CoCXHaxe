...
class AntsScene extends BaseContent
{
    public var phyllaCapacity(default, never) : Float = 50;
    ...
    public function phyllaSprite(nude : Bool = false) : Void
    {
        if (nude)
        {
            spriteSelect(SpriteDb.s_phylla_nude);
        }
        else
        {
    ...
    private function firstAntColonyEncounter() : Void
    {
        ...
        if (player.cor > 75 && player.lust > 50)
        {
            outputText(" looking for something to slake your unquenchable lusts, ");
        }
        outputText("you come across an abandoned merchant's cart.  The cart looks eerily disheveled, with a majority of its contents strewn about the sand.  Whoever or whatever deserted this thing did so in a hurry, and for seemingly no reason.")  //add button to avoid savescumming  ;
        ...
        var approach : Void->Void = function() : Void
        {
    ...
    public function demonsFuckAntgirl() : Void
    {
        ...
        function dickF() : Void
        {
            ...
            {
                outputText("  Licking the head of your [cock " + (x + 1) + "] in anticipation of what's to come only adds to your growing lust.");
            }
            //If player cock size tiny:
            else if (player.cockArea(x) < 6)
            {
                outputText("  Though your cock isn't the most impressive, it's 'got it where it counts' and you intend to make the most of it.  You lovingly tease yourself until you're fully erect; running your fingers down the ridge causes a bead of pre-cum to form on your tip.");
            }
            //If player cock size normal:
            else if (player.cockArea(x) < 12)
            {
                outputText("  You hold your cock like the hilt of a sword and start to pump your shaft slowly.  Your breathing becomes heavier and labored, as you stroke yourself with mounting passion.");
            }
            //If player cock size large:
            else if (player.cockArea(x) < 30)
            {
                outputText("  The fear of having your cock give away your position quickly passes through your mind, but it's too late now.  The only way out of this is to cum, so you might as well enjoy it.  You firmly grasp your cock and start jacking off, hoping that you'll finish sooner rather than later.");
            }
            //If player cock size MASSIVE:
            else
            {
                outputText("  The towering meat log jutting from your crotch gives away your 'hiding' spot, but the demons don't seem to mind, as they're too busy with their new toy.  You doubt even if they did care that any of them would be able to satisfy you. You wrap your arms around your fully erect cock and start to pump, hoping to release your load soon.");
            }
        ...
        var vagF : Void->Void = function() : Void
        {
            ...
            {
                outputText("  You feel your warm milk flow out of your hardened arola, as your body begs to release its sweet nectar.");
            }
            //If PC has dick(s):
            if (player.hasCock())
            {
                outputText("  Feeling [eachCock] throb between your legs, you run one of your hands down your chest and start to stroke lovingly.");
            }
    ...
    @:allow(classes.scenes.areas.desert)
    private function consolePhylla() : Void
    {
        clearOutput();
        ...
        if (player.lib >= 50)
        {
            outputText("After taking an eyeful of her smooth, enticing pussy, revealed in her current position by the inadequacies of her loincloth, y");
        }
        else
        {
            outputText("Y");
    ...
    private function enterTheColony() : Void
    {
        ...
        if (player.cor < 40)
        {
            outputText("\"<i>'Champion', you've been sent to this land to resist the demons and your efforts thus far, insignificant though they be, have not escaped my gaze.  ");
        }
        else
        {
            outputText("\"<i>Despite your obvious taint, you seem to have retained your valor as the 'Champion' you proclaim yourself to be, and not as a rutting beast.  Something to be admired, for sure.  ");
        }
    ...
    private function antColiseumFight() : Void
    {
        ...
        {
            //(Gnoll - Intro)
            if (flags[kFLAGS.MET_ANT_ARENA_GNOLL] == 0)
            {
                outputText("With due ceremony, the manager leads you down to one of the staging areas for the arena.  You watch through the bars as the stadium fills far past capacity.  There must be a thousand ants here... maybe even the whole colony.  Gazing out into the crowd, you pick out the royal seating and in it, Princess Phylla.  The princess is in much finer attire than you're used to seeing - her chest is actually covered by a fine red and blue dress and her hair is combed, framing her face nicely.  Her mother is nowhere to be seen, and you can tell that Phylla is taking full advantage.  She waves and cheers for you just like the crowd is and, perhaps by her orchestration, you hear the crowd pick up a chant in your name.  Seeing Phylla chant with them makes you feel invigorated.  As the gates raise, you charge out into the center of the arena, roaring your finest battle cry; your opponent is taken somewhat aback but finds resolve and braces for your charge.");
            }
            //►(Gnoll - Intro Repeat - for people that lost once)
            else
            {
                outputText("You move through the network of tunnels and caves and come to the colosseum once more.  You are greeted by the event manager and he ushers you down to one of the staging areas for the arena.  The noise intensifies as the colosseum fills to the brim with spectators; Phylla herself is up in her box waving to you.  You look across the arena floor and the gnoll at the other side of the arena seems just as excited to fight as you are.  The gates open and you charge each other!");
            }
    ...
    private function phyllaFirstTimePenis() : Void
    {
        ...
        else
        {
            ...
            {
                outputText("[cock Biggest] swings downwards at her, causing her to unleash a surprised scream as she scrambles out of the way, narrowly avoiding being smashed by the brutish, fleshy cudgel as it swings downward in what arguably could have been a debilitating injury for Phylla.");
            }
            else
            {
                outputText("[cock Biggest] bursts outward and slams into her shoulder blade. \"<i>Ahh~tk!</i>\" She clicks, more in surprise than pain.  She rubs at her bruised shoulder, somewhat shocked that someone could possess THAT kind of ability.  ");
            }
            //merge
    ...
    //original check - corruption > 75
    {
        ...
        //SCENE
        //==========================================================================================================
        function scene(rough : Bool) : Void
        {
            ...
            if (player.isGoo())
            {
                outputText(", leaving traces of your slimy lust along her body for her to wear, allowing your detached bits of mass to ignite her sexual fires");
            }
            outputText(".");
            ...
            if (!rough)
            {
                outputText("a gentle amount of force, ");
            }
            else
            {
                outputText("an uncaring, brutish amount of force, ");
            }
            ...
            if (!rough)
            {
                doNext(phyllaFirstTimePureBabiesFuckEnding);
            }
            //(Jump to Corruption more than 75 - Corrupt Ending)
            else
            {
                doNext(phyllaCorruptMascEnding);
            }
        };
    }
    ...
    private function phyllaFirstTimePureBabiesFuckEnding() : Void
    {
        ...
        if (player.cumQ() >= 100 && player.cumQ() < 1000)
        {
            outputText("  Truly amazing considering how much your loads usually contain.  You think you see a little bulge in her stomach that wasn't there before.");
        }
        //If cum milliliters over 1000:
        else if (player.cumQ() >= 1000)
        {
            outputText("  As you look down at her nethers your mind struggles to understand just how she could... then looking up her body, you see.  Her belly is bloated with your sperm, almost to the point of already looking pregnant.");
        }
        ...
        if (player.canOvipositSpider())
        {
            ...
            {
                SceneLib.phyllaScene.ovipositionScene();
            }
        }
    ...
    private function phyllaCorruptMascEnding() : Void
    {
        ...
        if (player.isGoo())
        {
            outputText("  Perhaps the poor dear could use some tender attention, you ponder to yourself.  Reducing the solidity of your body, you work her abdomen deep into your sticky, goopy body, re-solidifying once she's firmly in there and lowering your hands down to her nipples, pinching them hard as you continue your pumping of her rump.");
        }
        ...
        if (y >= 0)
        {
            ...
            {
                outputText("one of your free cocks");
            }
            else
            {
                outputText("your free cock");
            }
            ...
            {
                sceneHunter.print("Failed check: have one non-fitting dick");
            }
        }
        else
        {
            sceneHunter.print("Failed check: fitting second cock");
        }
        ...
        if (y >= 0)
        {
            outputText(" and her ass.");
        }
        else
        {
            outputText(", and onto the bedspread below.");
        }
    ...
    private function cuntmuffinLingusPhyllaDickBig() : Void
    {
        ...
        if (player.longestCockLength() < 69)
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("it points");
            }
            else
            {
                outputText("they point");
            }
        ...
        if (player.cockTotal() == 1)
        {
            outputText(" switches and bobs");
        }
        else
        {
            outputText("s switch and bob");
        }
        ...
        {
            outputText("wall in the hallway");
        }
        outputText(".");
        ...
        {
            outputText("  You run the split of your forked mouth muscle from the tip of her clit to the base.  As your textured feeler makes its way down, you wrap around the rest of her long clitorus, like a boa constrictor.  Once your split reaches the hood of her clit you roll and flick the tip of your tongue rapidly.  Pulling at your hair she gasps for breath in between teeth grinding moans.");
        }
        ...
        if (player.isGoo())
        {
            outputText(", tumbling out of your mass as she tears free");
        }
        ...
        if (player.cockTotal() == 1)
        {
            outputText("this monster");
        }
        else
        {
            outputText("these monsters");
        }
    ...
    private function femalePhyllaFirstTimePlusCock() : Void
    {
        ...
        {
            outputText("your [cock] leaving the lucky member to bask in affectionate glory as the rest of your manhood laments being left out.  \"<i>Don't worry, I have four hands...</i>\" Phylla coos as she works your shaft.");
        }
        ...
        //Dicks too Big:
        {
            ...
            if (player.isTaur())
            {
                outputText("Seeing your impending predicament, Phylla quickly scrambles with all four of her arms to collect a bunch of cushions.  She stacks them in a makeshift 'resting place' and helps you into a comfortable position as your cocks precede to grow uncontrollably.");
            }
            //Drider body:
            else if (player.isDrider())
            {
                outputText("Phylla immediately realizes this, and flips you over so that you are resting on your head and neck, ducking in past your [cocks] and coming to rest on the border between your spider carapace and soft clit.");
            }
            //Goo body:
            else if (player.isGoo())
            {
                outputText("Phylla immediately realizes this, and flips you over so that you are resting on your head and neck, and she works her way through your goopy mass, connecting her clit with yours in little time or effort. \"<i>This is a little weird... but I like it...</i>\" she flirts. Hardening your mass slightly, you snugly secure Phylla against your body, telling her that things \"might\" get rough.");
            }
            //Human/Harpy/Naga legs:
            else
            {
                outputText("Phylla immediately realizes this, and flips you over so that you are resting on your head and neck, and curls your [legs] towards her.");
            }
        ...
        var nofitF : Void->Void = function() : Void
        {
            ...
            //PC has (a) Tentacle dick(s):{
                ...
                if (tenta == 1)
                {
                    outputText("your cock into her mouth as she continues to rock her hips into yours.");
                }
                //two:
                else if (tenta == 2)
                {
                    outputText("your cocks into her mouth and between her breasts as she continues to rock her hips into yours.");
                }
                else
                {
                    outputText("your cocks into her mouth, between her breasts, and along her body.  The ones on her body tightly wrap around her insectile joints, using her muscular curves to get you off.");
                }
            }
            ...
            else
            {
                ...
                else if (player.countCocks(-1, 4, "width") > 0)
                {
                    outputText("your " + cockDescript(player.smallestCockIndex()) + ", stroking it apologetically as she scissors you.  You feel her almost insatiable appetite for all parts of your body.  You feel Phylla's mind slowly becoming lost in pleasure.");
                }
                //PC only has a dick(s) that('s) (are) more than 4 inches in width:
                else
                {
                    outputText("your " + cockDescript(player.smallestCockIndex()) + ", but she simply can't fit any number of her hands around the entirety of your glans; instead, she takes to caressing the head of your " + cockDescript(player.biggestCockIndex()) + ".");
                }
            }
            scissorContinue(false);
        }
    ...
    private function scissorContinue(clear : Bool) : Void
    {
        if (clear)
        {
            clearOutput();
