...
class NagaScene extends BaseContent
{
    private var gorgonOrNaga : String = "naga";
    private var isGorgon : Bool = false;
    
    public function new(gorgon : Bool = false)
    {
        super();
        if (gorgon)
        {
    ...
    public function nagaEncounter() : Void
    {
        ...
        if (!player.hasStatusEffect(StatusEffects.Naga))
        {
            player.createStatusEffect(StatusEffects.Naga, 0, 0, 0, 0);
        }
        clearOutput();
    ...
    private function nagaNaga() : Void
    {
        ...
        {
            if (flags[kFLAGS.SAMIRAH_FOLLOWER] < 1)
            {
                flags[kFLAGS.SAMIRAH_FOLLOWER] = 1;
            }
            else
            {
                flags[kFLAGS.SAMIRAH_FOLLOWER]++;
            }
        }
        ...
        //[If player has fangs]
        if (player.faceType == Face.SNAKE_FANGS || player.faceType == Face.DRAGON_FANGS || player.faceType == Face.SALAMANDER_FANGS || player.faceType == Face.SPIDER_FANGS || player.faceType == Face.YETI_FANGS || player.faceType == Face.DEVIL_FANGS || player.faceType == Face.ORC_FANGS || player.faceType == Face.WOLF_FANGS)
        {
            outputText("your own.");
        }
        //[player has no fangs]
    ...
    private function gooNagaRape() : Void
    {
        ...
        //(if corrupt male){
            ...
            {
                outputText("you feel a tingle at your groin from the thought.");
            }
        }
        //(if not corrupt)
        else
        ...
        if (player.cor > 50)
        {
            outputText("but immediately after, you decide that this is going to be too much fun to not push on.");
        }
        //(if not corrupt)
        else
        {
            outputText("but a nagging voice at the back of your head urges you to push further.");
        }
        ...
        if (player.cockTotal() > 0)
        {
            ...
            {
                outputText("your " + cockDescript(0));
            }
            else
            {
                outputText("one of your " + multiCockDescriptLight());
            }
        ...
        if (player.gender == 3)
        {
            ...
            if (player.cumQ() < 250)
            {
                outputText(" spurting onto her stomach as well.");
            }
            //(lots of jizz)
            else if (player.cumQ() < 1000)
            {
                outputText(" spurting onto her stomach and face.");
            }
            else
            {
    ...
    private function victoryMale_taur() : Void
    {
        ...
        if (player.cumQ() < 250)
        {
            outputText("you cum inside of her, ");
        }
        //[extra cum]
        else if (player.cumQ() < 1000)
        {
            outputText("you fill her insides with huge spurts of cum, ");
        }
        //[SUPER CUM]
        else
        {
            outputText("you explode with huge fountains of cum spurting from your [cock], gushing out from where the two of you are joined to rain on the sands below, ");
        }
    ...
    private function victoryMale() : Void
    {
        ...
        function scene(x : Int) : Void
        {
            ...
            if (player.cockArea(x) <= monster.vaginalCapacity())
            {
                ...
                if (player.cumQ() < 250)
                {
                    outputText("shooting thick ropes of sperm deep inside her as both of you wail with excitement.");
                }
                //[lots of cum]
                else if (player.cumQ() < 500)
                {
                    outputText("pumping large amounts of cum deep inside her as both your voices rise in screams of pleasure.");
                }
                //[Spermageddon]
                else
                {
                    outputText("your " + player.cockDescript(x) + " erupting, blasting a violent torrent of semen deep inside her petite frame, filling her up completely as both your voices rise in screams of pleasure.");
                }
            }
            //[If cock too large]
            else
            {
                ...
                if (player.cumQ() < 250)
                {
                    outputText("filling her mouth with your warm seed.");
                }
                //[lots of cum]
                else if (player.cumQ() < 500)
                {
                    outputText("pumping her mouth full of your sticky seed, some of which she is forced to swallow to avoid choking.");
                }
                //[Oh God, it just keeps coming out, help me!]
                else
                {
                    outputText("easily flooding her mouth with your seed. Your seed gushes from her nose, with still more rushing down her throat. She gags and chokes a bit, a large amount of your seed leaking out onto her chest.");
                }
                ...
                if (player.cumQ() > 500)
                {
                    outputText(", the last sticky blasts of your semen splashing across her face,");
                }
                outputText(" and looks to you for instruction. You simply nod, and she understands, ");
                //[normal cum load]
                if (player.cumQ() < 250)
                {
                    outputText(" gulping down the warm sperm in her mouth.");
                }
                //[lots of cum OR more]
                else
                {
                    outputText(" doing the best she can to swallow her mouthful of cum.");
                }
            }
    ...
    //Male or 50% herms
    {
        ...
        if (player.cumQ() > 250)
        {
            outputText(" with traces of semen in it");
        }
        //[if OMGWTFBBQGALLONSOFCUM]
        else if (player.cumQ() > 2000)
        {
            outputText(" that's filled with your thick seed");
        }
        outputText(".\n\n");
        ...
        {
            player.createStatusEffect(StatusEffects.MeanToNaga, 0, 0, 0, 0);
        }
        player.sexReward("vaginalFluids", "Dick");
        cleanupAfterCombat();
    }
    ...
    private function victoryFemale_taur() : Void
    {
        ...
        if (monster.lust >= monster.maxOverLust())
        {
            ...
            if (player.gender == 3)
            {
                outputText("  As her tail gently squeezes your body, it draws your own [cocks] up against your body, her soft underbelly stimulating the sensitive skin.");
            }
            ...
            if (player.gender == 3)
            {
                outputText(" and your [cocks] constantly stimulated by the tail wrapped around your body");
            }
    ...
    private function victoryFemale_rough() : Void
    {
        ...
        if (player.str > 50)
        {
            ...
            if (player.cor > 80)
            {
                outputText("  As a parting gift, you kick sand in her face, and walk away without a care in the world.");
            }
            //(If low corr )
            else
            {
                outputText("  You think to yourself, \"<i>Was all that really necessary?</i>\"  But, in the end you decide that yes, it was.");
            }
        }
        else
        {
            sceneHunter.print("Failed str check");
        }
    ...
    public function nagaFUCKSJOOOOOO() : Void
    {
        ...
        function nagaRapesBimbo() : Void
        {
            ...
            //[If Lactating]
            if (player.lactationQ() >= 50 && player.biggestLactation() >= 1)
            {
                outputText("Your warm milk is leaking from your " + nippleDescript(0) + "s the entire time, coating your skin and soaking her hands. At one point, she withdraws a hand and licks the milk from it. You can only assume that she liked it, as she begins to focus even more on your breasts.\n\n");
            }
            //-------
            outputText("The " + gorgonOrNaga);
            ...
            if (player.lactationQ() >= 50 && player.biggestLactation() >= 1)
            {
                outputText("As her mouth sucks and plays with your nipple, your warm, sweet milk begins to flow forth once again, filling her mouth with your delectable cream. Her hand is also absolutely covered by it, and the excess drips down to the dry sand below, absorbed in seconds by the thirsty earth.\n\n");
            }
            //------
            ...
            if (player.lactationQ() >= 50 && player.biggestLactation() >= 1)
            {
                outputText("She shares a mouthful of your delicious milk with you.\n\n");
            }
            //------
        ...
        var nagaRapesMale : Void->Void = function() : Void
        //select cock first
        {
            
            var x : Float = player.cockThatFits(monster.vaginalCapacity());  //fit  
            if (sceneHunter.dickSelect && rand(2))
            {
                x = player.findCock(1, monster.vaginalCapacity(), -1);
            }  //select the biggest one (non-fitting)  
            if (x < 0)
            {
                x = player.biggestCockIndex();
            }  //select overall biggest anyway  
            //(If defeated by lust)
            ...
            if (player.lust >= player.maxOverLust())
            {
                outputText("all your resistance seems to do is make your " + player.cockDescript(x) + " harder.");
            }
            //if defeated by KO)
            else
            {
                outputText("you are too weak to do anything to stop her squeezing.");
            }
            ...
            if (player.cor > 75)
            {
                outputText("  The thought somehow makes your " + player.cockDescript(x) + " throb even more.");
            }
            outputText("\n\n");
            ...
            if (player.cockArea(x) <= monster.vaginalCapacity())
            {
                ...
                if (player.cumQ() < 250)
                {
                    outputText("moans and pants");
                }
                //(Lots of cum)
                else if (player.cumQ() < 1000)
                {
                    outputText("squeals with joy");
                }
                //(It's gonna take weeks to clean this up!)
                else
                {
                    outputText("screams in delight");
                }
                outputText(" as your hot, gooey sperm ");
                //(Normal)
                if (player.cumQ() < 250)
                {
                    outputText("coats her womb.");
                }
                //(Lots of cum)
                else if (player.cumQ() < 1000)
                {
                    outputText("completely fills her.");
                }
                //(Indonesia called, they're flooded again)
                else
                {
                    outputText("absolutely engorges her, with the excess spurting out in torrents from her pussy.");
                }
                player.sexReward("vaginalFluids", "Dick");
            }
            //(Cock too big)
        ...
        var nagaRapesFemale : Void->Void = function() : Void
        {
            ...
            {
                outputText("and while you know you probably won't like it, your " + vaginaDescript(0) + " says otherwise.  ");
            }
        ...
        var nagaRapesGless : Void->Void = function() : Void
        //(If player loses because of lust)
        {
            ...
            if (player.cor < 50)
            {
                outputText("You try to turn your head away, but she forcefully shoves it down into her soft folds.  ");
            }
            //if player is corrupted)
            else
            {
                outputText("You eagerly bring your mouth to her glistening pussy. ");
            }
    ...
    public function nagaRapeChoice() : Void
    {
        ...
        {
            outputText("The " + gorgonOrNaga + " writhes in the sand, masturbating feverishly!  She's completely forgotten about fighting you.  ");
        }
        if (player.lust < 33)
        {
        ...
        //Overhauled menu
        var lowCor : Bool = player.cor < 66 + player.corruptionTolerance;
        var hiCor : Bool = player.cor >= 66 - player.corruptionTolerance;
        menu();
        //male
    ...
    public function nagaRapeChoice2() : Void
    {
        if (monster.HP < 1)
        {
            outputText("You've defeated the naga!  ");
        }
        else
        {
            outputText("The naga writhes in the sand, masturbating feverishly!  She's completely forgotten about fighting you.  ");
        }
    ...
    public function nagaPlayerConstrict() : Void
    {
        ...
        if (combat.checkConcentration("", true))
        {
            return;
        }  //Amily concentration... why(true) SceneLib.combat.enemyAIImpl(); and not enemyAI();? no idea  
        //WRAP IT UPPP
        if (monster.hasStatusEffect(StatusEffects.Dig))
        {
            if (rand(player.spe + 80) > monster.spe)
            {
                ...
                {
                    monster.createStatusEffect(StatusEffects.Constricted, 2 + rand(4), 0, 0, 0);
                }
                else
                {
        ...
        else if (rand(player.spe + 40) > monster.spe)
        {
            if (monster.short == "demons")
            {
    ...
    public function nagaSqueeze() : Void
    {
        ...
        var damage : Int = as3hx.Compat.parseInt(monster.maxHP() * (.10 + rand(15) / 100));
        if (player.perkv1(IMutationsLib.MightyLowerHalfIM) >= 2)
        {
            damage += as3hx.Compat.parseInt(combat.scalingBonusStrength() * 0.5 * (player.perkv1(IMutationsLib.MightyLowerHalfIM) - 1));
        }
        ...
        if (player.lowerBody == LowerBody.FROSTWYRM)
        {
            ...
            damageBonus *= as3hx.Compat.parseInt(combat.scalingBonusStrength() * 2);
        }
        ...
        {
            damage *= player.statusEffectv1(StatusEffects.ControlFreak);
        }
        if (player.hasPerk(PerkLib.Sadomasochism))
        {
            damage *= player.sadomasochismBoost();
        }
        if (player.perkv1(IMutationsLib.MightyLowerHalfIM) >= 1)
        {
            damage *= as3hx.Compat.parseInt(1 + (0.25 * player.perkv1(IMutationsLib.MightyLowerHalfIM)));
        }
        damage = as3hx.Compat.parseInt(damage + damageBonus);
    ...
    public function nagaTease() : Void
    {
        ...
        if (monster.hasStatusEffect(StatusEffects.NagaVenom) || monster.gender == 0 || monster.lustVuln == 0)
        {
            ...
            {
                outputText("You attempt to stimulate [themonster] by rubbing [monster his] nether regions, but [monster he] seems too affected by your poison to react.\n\n");
            }
            if (monster.gender == 0)
            {
            ...
            {
                outputText("You attempt to stimulate [themonster] by rubbing [monster his] nether regions, but it has no effect!  Your foe clearly does not experience lust in the same way as you.\n\n");
            }
            enemyAI();
            return;
        }
        ...
        if (player.teaseDmgStat.value > 0)
        {
            chance += 10;
        }
        //10% for bimbo shits
    ...
    private function beePositANagaPlease() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("  [EachCock] erupts to splatter her with white");
        }
        ...
        if (player.eggs() > 30)
        {
            outputText(" from the outside at least.  You stuffed her so full that she looks positively pregnant, bulging to a picture of utter fertility");
        }
