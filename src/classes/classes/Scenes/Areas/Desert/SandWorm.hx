...
class SandWorm extends Monster
{
    ...
    private function Bite() : Void
    {
        ...
        else
        {
            ...
            if (!player.immuneToAcid())
            {
                if (player.hasStatusEffect(StatusEffects.AcidDoT))
                {
                    player.addStatusValue(StatusEffects.AcidDoT, 2, 1);
                }
                else
                {
                    player.createStatusEffect(StatusEffects.AcidDoT, 5, 10, 0, 0);
    ...
    override private function performCombatAction() : Void
    {
        ...
        {
            var actionChoices : WeightedAction = new WeightedAction().add(Dig, 40).add(Bite, 40).add(Sting, 30).add(eAttack, 20);
