...
class SandWormScene extends BaseContent
{
    ...
    private function loseAsMale() : Void
    {
        ...
        {
            player.lowerGarment.name == "nothing";
        }
    ...
    private function loseAsFemale() : Void
    {
        ...
        if (!player.hasVagina())
        {
            transformations.VaginaHuman(0).applyEffect(false);
        }
        if (player.hasCock())
        {
        ...
        if (player.bRows() > 1)
        {
            transformations.BreastRowsOne.applyEffect(false);
        }
        if (player.biggestTitSize() < 2)
        {
            player.growTits(2, 1, false, 3);
        }
        ...
        {
            flags[kFLAGS.APEX_SELECTED_RACE] = Races.SANDWORM;
        }
