...
class Naga extends Monster
{
    ...
    private function nagaPoisonBiteAttack() : Void
    ...
    private function nagaConstrict() : Void
    {
    ...
    private function nagaTailWhip() : Void
    {
        ...
        else
        {
            ...
            if (player.armorDef < 20)
            {
                damage += 20 - player.armorDef;
            }
            damage += rand(10);
            damage *= 5;
    ...
    {
        if (flags[kFLAGS.SAMIRAH_FOLLOWER] > 9 && !sceneHunter.other)
        {
            SceneLib.desert.nagaScene.nagaRapeChoice2();
        }
        else
        {
    ...
    public function new(noInit : Bool = false)
    {
        ...
        if (flags[kFLAGS.SAMIRAH_FOLLOWER] > 9)
        {
            ...
            this.drop = new WeightedDrop().add(null, 1).add(useables.PCSHARD, 10).add(consumables.SNAKOIL, 7).add(headjewelries.GNHAIR, 4).add(consumables.REPTLUM, 1);
        }
        else
        {
            ...
            this.drop = new WeightedDrop().add(null, 1).add(consumables.SNAKOIL, 10).add(headjewelries.GNHAIR, 6).add(consumables.REPTLUM, 3);
        }
        // this.plural = false;
