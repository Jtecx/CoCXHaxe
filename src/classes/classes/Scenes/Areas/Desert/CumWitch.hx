...
class CumWitch extends Monster
{
    ...
    //Hurt!
    {
        ...
        if (player.hasCock())
        {
            choices[choices.length] = cumMagicAttack;
        }
        ...
        if (player.gender == 3)
        {
            choices[choices.length] = genderConfusionAttack;
        }
        //VAGOOZLES
        if (player.hasVagina())
        {
            choices[choices.length] = cumHungerAttack;
        }
    ...
    //*Cum Witch hikes up her dress and bukkake's at PC.  Large # of chance for 'hits' for low individual damage.  Small reduction to sand witch lust.  Used more at high lust.
    {
        ...
        var hits : Int = as3hx.Compat.parseInt(8 + rand(11));
        var bonus : Int = 0;
        var damage : Int = 0;
        while (hits > 0)
        {
            ...
            //Dragon-shell shield
            else if (player.shield == game.shields.DRGNSHL && rand(2) == 0)
            {
