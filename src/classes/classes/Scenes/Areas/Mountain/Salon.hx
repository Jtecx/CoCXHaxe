...
class Salon extends BaseContent implements TimeAwareInterface
{
    ...
    {
        menu();
        if (flags[kFLAGS.LYNNETTE_PREGNANCY_CYCLE] >= 4 && player.hasCock())
        {
            addButton(5, "Fuck Goblin", fuckLynnette);
        }
        if (player.hasCock())
        {
            addButton(0, "Goblin Blow", goblinHairDresserFacefuck);
        }
        ...
        {
            addButton(8, "Buy MinoCum", buyMinoCum);
        }
    ...
    private function salonPaymentMenu() : Void
    {
        ...
        if (player.hasCock())
        {
            addButton(0, "Goblin Blow", goblinHairDresserFacefuck).hint("Let Lynnette suck you off.");
        }
        ...
        if (flags[kFLAGS.CAN_BUY_MINOCUM] > 0)
        {
            addButton(8, "Buy MinoCum", buyMinoCum).hint("Buy a bottle of minotaur cum for 60 gems?");
        }
    ...
    public function salonPurchaseMenu() : Void
    {
        ...
        var mudFacialEnabled : Bool = false;
        var sandFacialEnabled : Bool = false;
        //Enable mud facial
        if (player.femininity < 100 && player.gender == 2)
        {
            mudFacialEnabled = true;
        }
        else if (player.femininity < 85 && (player.gender == 0 || player.gender == 3))
        {
            mudFacialEnabled = true;
        }
        else if (player.femininity < 70 && player.gender == 1)
        {
            mudFacialEnabled = true;
        }
        else if (player.femininity < 100 && player.hasPerk(PerkLib.Androgyny))
        {
            mudFacialEnabled = true;
        }
        //Enable sand facial
        if (player.femininity > 0 && player.gender == 1)
        {
            sandFacialEnabled = true;
        }
        else if (player.femininity > 30 && player.gender == 2)
        {
            sandFacialEnabled = true;
        }
        else if (player.femininity > 20 && (player.gender == 0 || player.gender == 3))
        {
            sandFacialEnabled = true;
        }
        else if (player.femininity > 0 && player.hasPerk(PerkLib.Androgyny))
        {
            sandFacialEnabled = true;
        }
        ...
        {
            addButton(0, "Cut Short", cut, 1);
        }
        if (player.hairLength > 13)
        {
            addButton(1, "Cut Medium", cut, 10);
        }
        if (player.hairLength >= 26)
        {
            addButton(2, "Cut Long", cut, 25);
        }
        if (player.hairLength < player.tallness)
        {
            addButton(3, "Lengthen", hairGrow);
        }
        if (player.hairLength > 0)
        {
            addButton(4, "Remove Hair", removeHair);
        }
        addButton(5, "Buy Products", dyeMenu);
        if (flags[kFLAGS.CAN_BUY_MINOCUM] > 0)
        {
            addButton(6, "Buy MinoCum", buyMinoCum).hint("Buy a bottle of minotaur cum for 60 gems?");
        }
        addButton(7, "Beard Options", beardMenu);
        if (mudFacialEnabled)
        {
            addButton(8, "Mud Facial", mudFacial).hint("This facial is supposed to enhance the softness of your face and enhance its femininity greatly.");
        }
        if (sandFacialEnabled)
        {
            addButton(9, "Sand Facial", sandFacial).hint("The goblins promise this facial will give you a rough, handsome look thanks to their special, timeless sands.");
        }
        if (player.hairLength > 2 && !Hair.Types[player.hairType].ignoresStyle)
        {
            addButton(10, "Haircut", changeHairStyle);
        }
        else if (Hair.Types[player.hairType].ignoresStyle)
        {
            addButtonDisabled(10, "Haircut", "Your current hair can't have its style changed!");
        }
        else if (player.hairLength <= 2)
        {
            addButtonDisabled(10, "Haircut", "Your current hair length is too short!");
        }
        addButton(14, "Leave", explorer.done);
    }
    ...
    private function hairDresserRepeatGreeting() : Void
    {
        ...
        if (flags[kFLAGS.LYNNETTE_PREGNANCY_CYCLE] == 4 && flags[kFLAGS.LYNNETTE_CARRYING_COUNT] > 0)
        {
            ...
            if (flags[kFLAGS.LYNNETTE_BABY_COUNT] > 0)
            {
    ...
    private function gloryholeMinotaur() : Void
    {
        ...
        {
            outputText("arch your back to expose ");
        }
    ...
    private function goblinHairDresserFacefuck() : Void
    {
        ...
        {
            outputText("around as much of you as she can");
        }
        else
        {
            outputText("around your first few inches");
        }
        ...
        if (player.cumQ() >= 250)
        {
            outputText(" She can't hold it all and suddenly starts swallowing as runnels of it escape her nose, and a wave of it rolls down her lips.");
        }
    ...
    private function beardMenu() : Void
    {
        ...
        menu();
        if (player.hasBeard() && player.beardLength > 0.5)
        {
            addButton(0, "Cut Beard", cutBeard);
        }
        if (player.hasBeard() && player.beardLength < 6)
        {
            addButton(1, "Lengthen Beard", growBeard, 0);
        }
        if (!player.hasBeard())
        {
            addButton(1, "Grow Beard", growBeard, 1);
        }
        if (player.hasBeard())
        {
            addButton(2, "Beard Style", changeBeardStyle);
        }
        if (player.hasBeard())
        {
            addButton(3, "Remove Beard", removeBeard);
        }
        addButton(14, "Back", hairDressingMainMenu);
    }
    ...
    private function minotaurCumBukkakeInSalon() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText(" and puffing up your twat with liquid arousal.\n\n");
        }
        else if (player.hasCock())
        {
            outputText(" and turning " + sMultiCockDesc() + " into a turgid, pulsating mass.\n\n");
        }
        else
        {
            outputText("\n\n");
        ...
        if (player.hasVagina() || player.hasCock())
        {
            ...
            if (player.hasCock())
            {
                ...
                {
                    outputText("pre-cum a few inches wide ");
                }
                else
                {
                    outputText("pre-cum over a foot wide ");
                }
            }
        ...
        if (player.gender > 0)
        {
            outputText(" and increasing the size of the puddle you're leaving on the floor");
        }
        ...
        if (player.biggestTitSize() >= 2)
        {
            ...
            {
                outputText("Even so, you can't quite get your breasts the entire way around him, so you make up for it by pressing it harder into you with your busy hands.  ");
            }
        ...
        if (player.biggestTitSize() >= 2)
        {
            outputText("tit-fucking ");
        }
        else
        {
            outputText("full-body handjob ");
        }
        ...
        if (player.biggestTitSize() >= 2)
        {
            outputText("The sudden girth change sends an enticing ripple through your [allbreasts] that's pleasurable enough to make you moan into his steadily-widening urethra.  ");
        }
        ...
        if (player.hasVagina())
        {
            outputText(" while the animal part of your brain hopes it makes you pregnant with an equally girthy son");
        }
        outputText(".\n\n");
        //ADD PREG CHECK
        //Preggers chance!
        ...
        else if (player.analCapacity() < 140)
        {
            outputText("delighting in the feeling of perfect fullness.");
        }
        else
        {
            outputText("delighting in realizing that you could take far larger than even this virile specimen!");
        }
        //(buttchange here: 90)
        ...
        if (!player.hasCock())
        {
            outputText("drags through your body, touching sensitive nerves you didn't even know you had until you cum, shuddering and shaking like a wanton whore.");
        }
        else
        {
            outputText("presses on your prostate as it squeezes by, making " + sMultiCockDesc() + " drip and spurt freely until you can bear it no longer and cum, shaking and shuddering like a wanton whore.  Jizz drips and pours from " + sMultiCockDesc() + " in a steady stream that pools on the floor, slowly rolling towards a drain that doubtless empties into a tank or greedy goblin cunt.");
        }
        outputText("\n\n");
...
*/
    ...
    private function fuckLynnette() : Void
    {
        ...
        if (flags[kFLAGS.LYNNETTE_FUCK_COUNTER] == 0)
        {
            ...
            if (player.balls == 0)
            {
                outputText("pressing down near the [sheath]");
            }
            else
            {
                outputText("curling her petite fingers around your [sack] to heft it");
            }
            outputText(".");
            //IF FAIL!
        ...
        else
        {
            ...
            if (player.lust <= 75)
            {
                outputText("it fills to full size almost immediately");
            }
            else
            {
                outputText("it's fully-erect state makes itself known");
            }
            outputText(". Lynnette strokes a few times to make sure you're suitably engorged and then shifts lower, squeezing ");
            if (player.balls == 0)
            {
                outputText("around your [sheath] to get a feel for how pent-up you are");
            }
            else
            {
                outputText("gently at your [balls], weighing them in her palm to get a feel for just how pent-up you are");
            }
            outputText(".");
            //NOT ENOUGH!
            if (player.hoursSinceCum < 24)
        ...
        if (player.cockArea(x) > 50)
        {
            ...
            {
                outputText("into her ribcage, penetrating farther than any lesser race could ever hope to handle.");
            }
        }
        ...
        if (player.cockArea(x) <= 90)
        {
            ...
            if (player.hasBalls())
            {
                outputText(", [balls] resting on her soaked, juicy ass-cheeks.");
            }
        }
        else
        {
            ...
            if (player.hasBalls())
            {
                outputText("  Her feet wrap around your [sack] to squeeze your [balls] affectionately, compensating quite nicely for her own shortcomings.");
            }
        }
        ...
        if (player.hasBalls())
        {
            outputText(", making your sack feel tight, and your testes bloated and tender.");
        }
        else
        {
            outputText(", making your gut clench and spasm with near-orgasmic contractions.");
        }
        ...
        {
            outputText("up against her slightly-yielding cervix");
        }
        else
        {
            outputText("in as close to the cervix as you possibly can");
        }
        ...
        if (player.cockArea(x) <= 90)
        {
            outputText("completely inside once more");
        }
        else
        {
            outputText("as deeply inside as you can");
        }
        ...
        if (player.cockTotal() > 1)
        {
            ...
            {
                outputText("on the greedy green slut's belly");
            }
            else
            {
                outputText("in the milk-stained valley between her tits, absolutely buried in squishy-soft breast");
            }
        ...
        if (player.hasKnot(x))
        {
            outputText(" with a wet-sounding 'pop'.  Your knot does a terrific job of gaping her lips during its exit, and you cannot help but marvel at how well you've broken this bitch in.  A shame she'll probably tighten up again in short order");
        }
        else
        {
            outputText(" with a gush of girlspunk to chase you");
        }
        ...
        else if (player.cumQ() <= 10000)
        {
            if (flags[kFLAGS.LYNNETTE_ANNOUNCED_APPROVAL] == 1)
            {
                outputText("\"<i>Wow, [name].  You cum like a minotaur... and without drugging me out of my mind to boot!  Gods, I can feel the little swimmers hunting down my eggs already.</i>\"  She giggles and retrieves a vial, downing it in a single gulp.  \"<i>I better make sure there's enough eggs for them all, huh?</i>\"\n\nYou leave with a happy smile. It seems Lynnette approves of you as a mate.");
            }
            //"GOOD + LYNNETTE FAVORITE"
            else
            {
                outputText("\"<i>Mmmm, [name], I love the way you pack me full.</i>\"  She tosses back a vial of fertility-boosting chemicals and sighs.  \"<i>Do me a favor and come back soon.  I'd rather be pouring out swarms of your babies than a slurry of weakling sluts.</i>\"");
            }
            lynnetteApproval(10);
        }
        //"UBUR"
        ...
        if (player.cumQ() >= 1000)
        {
            flags[kFLAGS.LYNNETTE_CARRYING_COUNT] += 1 + rand(3);
        }
        if (player.cumQ() >= 2000)
        {
            flags[kFLAGS.LYNNETTE_CARRYING_COUNT] += 1 + rand(3);
        ...
        if (player.cumQ() >= 3000)
        {
            flags[kFLAGS.LYNNETTE_CARRYING_COUNT] += 2 + rand(3);
        }
        if (player.cumQ() >= 4000)
        {
            flags[kFLAGS.LYNNETTE_CARRYING_COUNT] += 2 + rand(3);
    ...
    public function lynnetteApproval(arg : Int = 0) : Float
    {
        if (arg != 0)
        {
            flags[kFLAGS.LYNNETTE_APPROVAL] += arg;
        }
        if (flags[kFLAGS.LYNNETTE_APPROVAL] < -100)
        {
            flags[kFLAGS.LYNNETTE_APPROVAL] = -100;
        }
        else if (flags[kFLAGS.LYNNETTE_APPROVAL] > 100)
        {
            flags[kFLAGS.LYNNETTE_APPROVAL] = 100;
        }
