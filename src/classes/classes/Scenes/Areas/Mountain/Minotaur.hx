...
{
    public var hasAxe : Bool;
    
    public var hasTwoAttacks : Bool = false;
    ...
    override private function get_long() : String
    {
        if (inDungeon)
        {
            return "This minotaur, attired with a steel full plate and a pair of deadly battle axes, is easily eight feet tall. True to his species nature, his huge equine cock is already dripping with precum in anticipation.";
        }
        else
        {
    ...
    public function new(axe : Bool = false)
    {
        ...
        if (inDungeon)
        {
            ...
            this.level = 64 + 5 * mod;
            this.drop = new ChainedDrop().add(necklaces.COWBELL, 1 / 12).add(weapons.DL_AXE_, 1 / 8).add(consumables.MINOCUM, 1 / 5).add(consumables.MINOBLO, 1 / 2).elseDrop(null);
        ...
        this.abilities = [
                ...
                {
                    call : SceneLib.mountain.minotaurScene.minoPheromones,
                    type : ABILITY_TEASE,
                    range : RANGE_RANGED,
                    tags : []
                }
