...
class HellCatScene extends BaseContent
{
    ...
    public function DefeatedHellCat() : Void
    {
        ...
        if (player.hasCock())
        {
            addButton(1, "Pussycat", DefeatedHellCatPussycat);
        }
        if (player.hasVagina())
        {
            addButton(2, "Catcock", DefeatedHellCatCatcock);
        }
    ...
    public function DefeatedHellCatPussycat() : Void
    {
        if (player.gender == 3)
        {
            outputText("You ain’t gonna get any close to that feline penis of hers. " + ((flags[kFLAGS.CODEX_ENTRY_HELLHOUNDS] > 0) ? "If your hunch is good it’s likely even worse than that of the hellhounds. " : "") + "");
        }
    ...
    public function DefeatedHellCatCatcock() : Void
    {
        if (silly())
        {
            outputText("Oh my gawsh, a true catcock for once! You’ve seen horse, dog, demon, lizard, dragon and even plant dongs so why is it you could never find a cat one to bone yourself with in the entirety of this porn adventure" + ((flags[kFLAGS.AKBAL_SUBMISSION_STATE] != 0) ? " save for Akbal" : "") + "?! ");
        }
    ...
    public function WitchesSabbath() : Void
    {
        ...
        if (flags[kFLAGS.WITCHES_SABBATH] > 3)
        {
            if (player.gender == 3)
            {
                ...
                if (flags[kFLAGS.WITCHES_SABBATH] == 4)
                {
                    outputText("Not what you expected actually, some magic must be at work, how else could her cunt taste so sweet? ");
                }
    ...
    public function WitchesSabbathThirdVisitYes() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText(" Done with the licking, the leading Hellcat goes one step further and grabs the small idol on the altar, aligning it with your asshole before she gets to sliding inch after inch of the thing in. Your pleasure doubles up as the cursed idol bottoms out, fully plugging your ass.");
        }  //Furrin: You left no else fields after this, leading to it being nonsense for males and nulls.  
        ...
        if (!player.hasVagina())
        {
            outputText("With her unholy rod up your butt, you realize you can suddenly understand the chanting, the magic creating a translated echo within you. ");
        }
        ...
        if (player.hasCock() && player.catCocks() < player.cockTotal())
        {
            ...
            if (player.hasSheath())
            {
                outputText("The surface of your " + ((player.cockTotal() > 1) ? "many lenghts" : "entire length") + "");
            }
            else
            {
                outputText(" Pressure builds around " + ((player.cockTotal() > 1) ? "their" : "its") + " base" + ((player.cockTotal() > 1) ? "s" : "") + " as a newly formed sheath pulls " + ((player.cockTotal() > 1) ? "them" : "it") + " into its fold. Only for " + ((player.cockTotal() > 1) ? "them" : "it") + " to surge back as the shape of your " + ((player.cockTotal() > 1) ? "many lenghts" : "entire length") + "");
            }
    ...
    public function WitchesSabbathFourthVisitYeahSure2() : Void
    {
        ...
        var playerWasAlreadyHerm : Bool = false;
        if (player.hasCock() && player.catCocks() >= 1)
        {
            playerWasAlreadyHerm = true;
        }
        ...
        player.modFem(100, 100);
        if (playerWasAlreadyHerm)
        {
            outputText(" your body begins to change.");
        }
    ...
    {
        var i : Float = 0;
        while (i < (player.cockTotal()) && player.cocks[i].cockType == CockTypesEnum.CAT)
        {
            i++;
        }
    ...
    //outputText("You moan, feeling overwhelming heat build in your nethers as ");
    {
        ...
        else
        {
            ...
            if (!player.hasSheath())
            {
                outputText(", slowly being pulled inside a forming sheath between your legs, only to surge back out");
            }
