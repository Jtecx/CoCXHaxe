...
class MinoIncubus extends Monster
{
    ...
    public function minoincubusHeadbutt() : Void
    {
        ...
        var damage : Float = ((str + weaponAttack) * 0.5) - rand(player.tou * 0.75);
    ...
    public function minoincubusBattleaxes() : Void
    {
        ...
        var damage : Float = ((str + weaponAttack) * 1.25) - rand(player.tou);
        if (damage <= 0 || player.getEvasionRoll())
        {
    ...
    public function new()
    {
        ...
        this.drop = new WeightedDrop().add(consumables.BROBREW, 1).add(weapons.DL_AXE_, 2).add(consumables.MINOCUM, 6).add(consumables.MINOBLO, 8).add(consumables.INCUBID, 10);
