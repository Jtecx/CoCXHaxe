...
class DemonSoldier extends Monster
{
    private static inline var SEED_COCK_OFFSET : Int = 13579;
    private static inline var SEED_TITS_OFFSET : Int = 73591;
    private static inline var SEED_SKIN_OFFSET : Int = 28640;
    
    private static var SKIN_COLORS_LIST : Array<Dynamic> = ["red", "crimson", "blue", "pale blue", "purple", "pink", "pale", "fair", "dark"];
    
    private var demonGender : Int = 0;
    
    private static inline var DEMON_GENDER_GENDERLESS : Int = 0;  //Only a failsafe.  
    private static inline var DEMON_GENDER_MALE : Int = 1;
    private static inline var DEMON_GENDER_FEMALE : Int = 2;
    private static inline var DEMON_GENDER_HERM : Int = 3;
    private static inline var DEMON_GENDER_CUNTBOY : Int = 4;
    private static inline var DEMON_GENDER_SHEMALE : Int = 5;
    private static inline var DEMON_GENDER_MALEHERM : Int = 6;
    ...
    //Variant 0 = no prefix. Variant 1 = 'a'. Variant 2 = 'the',
    {
        ...
        if (variant == 1)
        {
            txt += (((this.demonGender == 1 || this.demonGender == 3)) ? "an" : "a") + " ";
        }
        if (variant == 2)
        {
            txt += "the ";
        }
        var _sw0_ = (this.demonGender);        
    ...
    public function generateAppearance() : Void
    {
        ...
        //Feminine with tits{
            ...
            {
                this.demonGender = DEMON_GENDER_HERM;
            }
            else if (this.hasCock())
            {
                this.demonGender = DEMON_GENDER_SHEMALE;
            }
            else if (this.hasVagina())
            {
                this.demonGender = DEMON_GENDER_FEMALE;
            }
        }
        //Masculine without tits
        else
        {
            
            if (this.hasCock() && this.hasVagina())
            {
                this.demonGender = DEMON_GENDER_MALEHERM;
            }
            else if (this.hasCock())
            {
                this.demonGender = DEMON_GENDER_MALE;
            }
            else if (this.hasVagina())
            {
                this.demonGender = DEMON_GENDER_CUNTBOY;
            }
        }
        //Set pronouns.
        var _sw2_ = (this.demonGender);        

        switch (_sw2_)
        {
            case DEMON_GENDER_MALE, DEMON_GENDER_MALEHERM, DEMON_GENDER_CUNTBOY:
                this.pronoun1 = "he";
                this.pronoun2 = "him";
                this.pronoun3 = "his";
            case DEMON_GENDER_FEMALE, DEMON_GENDER_HERM, DEMON_GENDER_SHEMALE:
                this.pronoun1 = "she";
                this.pronoun2 = "her";
                this.pronoun3 = "her";
    ...
    private function demonPhysicalAttack() : Void
    {
        ...
        if (player.hasStatusEffect(StatusEffects.Blind) || this.fatigueLeft() < 10)
        {
            attackChooser = rand(4);
        }  //Don't re-apply blind.  
    ...
    //Pick your poison
    {
        ...
        if (teaseChooser == 0 && (hasStatusEffect(StatusEffects.TensionReleased) || this.fatigueLeft() < 15))
        {
            teaseChooser = as3hx.Compat.parseInt(1 + rand(3));
        }
        if (teaseChooser == 1 && this.fatigueLeft() < 10)
        {
            teaseChooser = as3hx.Compat.parseInt(2 + rand(2));
        }
        //Genital tease
        ...
        //Dick tease{
            ...
            this.fatigue += 15;
            this.lust -= 30;
            this.createStatusEffect(StatusEffects.TensionReleased, 3, 0, 0, 0);
        }
        //Lust strike
    ...
    override public function doAI() : Void
    {
        ...
        if (this.hasStatusEffect(StatusEffects.TensionReleased))
        
        //Cooldown on genital tease.{
            
            this.changeStatusValue(StatusEffects.TensionReleased, 1, statusEffectv1(StatusEffects.TensionReleased) - 1);
            if (statusEffectv1(StatusEffects.TensionReleased) <= 0)
            {
                removeStatusEffect(StatusEffects.TensionReleased);
            }
        }
    ...
    public function new()
    {
        ...
        else
        {
            ...
            if (this.hasCock() && this.hasVagina())
            {
                this.long += "both " + this.mf("his", "her") + " sets of infernal genitailia";
            }
            else if (this.hasCock())
            {
                this.long += this.mf("his", "her") + " pulsating cock";
            }
            else if (this.hasVagina())
            {
                this.long += this.mf("his", "her") + "slavering pussy";
            }
            else
            {
                this.long += this.mf("his", "her") + "oddly blank crotch";
            }  //Failsafe  
        ...
        {
            (try cast(this.drop, WeightedDrop) catch(e:Dynamic) null).add(consumables.INCUBID, 6).add(consumables.BROBREW, 2);
        }
        if (this.demonGender == DEMON_GENDER_SHEMALE || this.demonGender == DEMON_GENDER_FEMALE)
        {
            (try cast(this.drop, WeightedDrop) catch(e:Dynamic) null).add(consumables.SUCMILK, 6).add(consumables.BIMBOLQ, 2);
        }
