...
class DemonSoldierScene extends BaseContent
{
    public function encounterTheSoldierz() : Void
    {
        ...
        if (flags[kFLAGS.DEMON_SOLDIERS_ENCOUNTERED] < 1)
        {
            ...
            {
                outputText("<i>\"Well well, it looks like I've found some sport to liven up a dull patrol!\"</i>");
            }
            else
            {
                outputText("\"<i>Hmm... you may look like one of us, but I can smell your soul from here; I shall enjoy fucking it out of you!\"</i>");
            }
            outputText(" The " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle(0) + " purrs. \n\nThe demon draws [monster his] wickedly serrated scimitar and adopts an aggressive combat stance. It's a fight! ");
        ...
        else
        {
            ...
            outputText("\n\nThis time, the demon is " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle(1) + " and [monster he] draws [monster his] scimitar. It's a fight! ");
            flags[kFLAGS.DEMON_SOLDIERS_ENCOUNTERED]++;
        }
        if (monster.looksFemale())
        {
    ...
    public function victoryOverSoldier(hpVictory : Bool) : Void
    {
        clearOutput();
        if (hpVictory)
        {
            outputText("The " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + " sprawls on ground before you, weakened and dazed, you're quite sure [monster he] couldn’t put up even the slightest resistance to anything you might want to do...");
        }
        else
        {
            outputText("The " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + " can take no more of your relentless sexual stimulation, and gasps as [monster he] drops to [monster his] knees, frantically masturbating with both of [monster his] hands <i>and</i> [monster his] tail. <i>\"Unngh... Holy Mother of Fuck, Ahh... what manner of mortal <b>are</b> you?\"</i>");
        }
        menu();
        ...
        {
            addButtonDisabled(3, "Kill " + monster.mf("Him", "Her"), "You must have beaten the demon by reducing [monster his] HP to zero.");
        }
    ...
    {
        clearOutput();
        outputText("You approach the vanquished " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + " " + ((player.cor < 40) ? "hesitantly" : ((player.cor < 75) ? "with a wicked grin on your face" : "positively salivating at the prospect of raping [monster him] senseless")) + ", and quickly set about stripping " + player.clothedOrNakedLower("both yourself and ", "") + "your victim naked.");
        ...
        outputText("\n\nThe " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + " squirms and moans on the ground as you squat down beside [monster him] and slowly run your fingertip around [monster his] " + ((monster.hasCock()) ? "cock head" : "pussy lips") + " before leisurely drawing [monster him] up the infernal beast's midriff, over [monster his] [chest] and up to [monster his] lips, where the Demon begins to suckle enthusiastically at the intruding digit.");
        ...
        var dickFits : Void->Void = function() : Void
        {
            ...
            outputText("\n\nGrabbing hold of the Demon's horns you ram your [cockFit 65] deep into [monster his] gullet, the beast easily takes your meaty dick down [monster his] experienced, cock-milking throat. You rock your " + ((player.isTaur()) ? "equine flanks" : "hips") + " as you roughly face-fuck the helpless " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + " and use your grip on [monster his] horns to slide the infernal monster's head back and forth, forcefully dragging the dick-sucking circle of [monster his] lips up and down the length of your prick.");
            ...
            outputText("\n\nFinally, you pull your cock from [monster his] silken sheath" + ((player.cockTotal() > 1) ? ", as your other cock" + ((player.cockTotal() == 2) ? " splatters" : "s splatter") + " cum onto the ground" : "") + " and finish the last of your orgasm over the " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + "'s face, as [monster he] gasps for long-denied air. ");
            player.sexReward("saliva", "Dick");
            sharedEnd();
        }
        
        var dickTooBig : Void->Void = function() : Void
        {
        ...
        //First Body type check
        {
            ...
            outputText("demonic saliva. At your urging the " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + " lets out more and more of [monster his] great, slick rope of a tongue, first doubling back on [monster him]self within the silken innards of your " + player.assholeOrPussy() + ", then wriggling [monster his] tip " + ((player.hasVagina()) ? "out of your lower lips to begin tracing circles around your [clit]" : "far inside your colon, stimulating and filling the deepest reaches your hole") + ".");
        ...
        //The final part! Gender variations converge here.
        {
            ...
            outputText("\n\nThe frustrated " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + ", " + ((player.hasCock()) ? "[monster his] face still covered with pearlescent spunk and " : ((player.hasVagina()) ? "face still smeared with your cunt slime and " : "")) + "[monster his] hands still bound behind [monster his] back, is desperately trying to masturbate as best [monster he] can.\n\n");
    ...
    private function killTheSoldier() : Void
    {
        ...
        {
            addButton(3, "Harvest", killTheSoldierHarvest);
        }
        else
        {
    ...
    public function defeatedBySoldier(hpVictory : Bool) : Void
    {
        clearOutput();
        outputText("Lying prostrate at the Demon Soldier's feet, you are barely even aware of the " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + player.clothedOrNakedLower("stripping off your " + player.armorDescript(), "looming even closer") + ". [monster He] takes the opportunity to run [monster his] hands across your helpless body, murmuring to [monster him]self, \"<i>Very nice, I can certainly work with this...</i>\"");
        if (monster.hasCock() && (rand(2) == 0 || !monster.hasVagina()))
        
        //If the demon is a hermaphrodite, 50/50 split.{
    ...
    private function demonLossSceneDicked() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText(((player.isNaga()) ? "groin" : "thighs") + ", the " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + " presses [monster his] mouth into your [vagina]");
        }
        else
        {
            outputText(((player.isNaga()) ? "serpentine groin" : "butt-cheeks") + ", the " + capitalizeFirstLetter((try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle(2)) + " seals [monster his] lips over your [butthole]");
        }
        ...
        outputText("\n\nThe " + capitalizeFirstLetter((try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle()) + " pulls [monster his] now semi-flaccid dick from your thoroughly-fucked " + ((player.hasVagina()) ? "cunt" : "asshole") + " with a loud, obscene slurp and makes an attempt to wipe [monster him]self off on your " + player.clothedOrNaked(player.armorDescript(), "body") + ".");
    ...
    //The sarcastic remark begins!
    {
        ...
        if (gender == 1)
        {
            ...
            outputText("The stimulation causes you to sharply exhale involuntarily, and your " + player.multiCockDescriptLight() + " to begin to swell. The " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + " withdraws, releasing your cock from suction, a glistening spider-web of saliva linking the " + player.cockHead(player.biggestCockIndex()) + " of your " + player.cockDescript(player.biggestCockIndex()) + " with the infernal monster's lips.");
        }
        else if (gender == 2)
        {
        ...
        //Phew. Let's proceed!
        outputText("\n\n<i>\"Mmmmm, delicious.\"</i> [monster He] hisses <i>\"Now, let's put that pretty little mouth of yours to good use.\"</i> " + ((player.isTaur() || player.tallness >= 90) ? "On all fours, the " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + " slinks up to your head, and, with" : "With") + " a smooth swivel, the " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + " straddles your face.");
        ...
        outputText("\n\nAs demonic fem-juices begin to dribble across your [face]" + ((monster.hasCock()) ? " and hot pre-cum starts spitting onto your [chest] from the end of [monster his] pulsating cock," : ",") + " the " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + " grinds [monster his] hips down in response to your efforts and groans in pleasure.");
        outputText("\n\n<i>\"Hhhhnnnnyyyyyeeeessssss... Good, nnngooood, hhhaaaahhh... Now... I want to... unf... taste you...\"</i>");
        if (player.isTaur() || player.tallness >= 90)
        
        //Taur or large stature.{
        ...
        else
        {
            ...
            if (player.cockTotal() > 1 && gender == 1)
            {
                ...
                {
                    outputText("with both [monster his] hands to grasp your " + player.cockDescript(player.smallestCockIndex()));
                }
                else if (player.cockTotal() == 3)
                {
                    outputText("and grab hold of both of your other cocks");
                }
                else
                {
                    outputText("and grab hold of two of your other cocks");
                }
        ...
        //The final part!
        outputText("\n\nYou lie beneath the Demon Soldier, sucking, licking, bucking and writhing. Tongue whirling and twirling as the " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + " gropes and suckles at your flesh. The overwhelming musk of the abomination's sloppy fuck-hole and the mounting pleasure drive you rapidly towards your imminent orgasm. Judging by the way the Demon's soaking cunt is splashing infernal sex-fluids across your face the " + (try cast(monster, DemonSoldier) catch(e:Dynamic) null).demonTitle() + " must be nearing [monster his] own climax.");
        ...
        else
        {
            if (gender == 1)
            {
                if (player.thickestCockThickness() < 4.5)
                {
                    outputText("[monster his] throat vibrating around your phallic meat, ");
                }
                else
                {
                    outputText("[monster his] infernal mouth vibrating around your phallic meat, ");
                }
            }
            else if (gender == 2)
            {
