...
/**
	 ...
	 * @author ...
	 */
class CalluScene extends AbstractLakeContent
{
    ...
    public function ottahGirl() : Void
    {
        ...
        if (flags[kFLAGS.MET_OTTERGIRL] == 1)
        {
            ...
            {
                outputText("crystal clear, giving you a stunning view of the lakebed");
            }
            else
            {
                outputText("only slightly clouded, giving you an obscured view of the lakebed");
            }
            ...
            if (silly())
            {
                outputText("  No tragic backstory to go through? No annoying combat encounter? Just meet and fuck?  My god, what has this world come to?");
            }
        ...
        else
        {
            ...
            if (player.hasCock())
            {
                ...
                {
                    addButton(0, "Fuck Her", ottergirlLikesDongs);
                }
            }
            if (player.hasVagina() || !player.hasCock())
            {
                addButton(1, "Facesitting", ottersForGals);
            }
        }
        if (flags[kFLAGS.MET_OTTERGIRL] > 1)
        {
        ...
        if (player.hasKeyItem("Fishing Pole") < 0)
        {
            addButton(3, "FishingPole", getDatFishingPole);
        }
    ...
    private function ottergirlLikesDongs() : Void
    {
        ...
        while (i < player.cockTotal())
        {
            if (player.cocks[i].cockLength < 48)
            {
                ...
                else if (player.cocks[i].cockLength > player.cocks[y].cockLength)
                {
                    y = i;
                }
            }
        ...
        if (player.cocks[x].cockLength < 6)
        {
            outputText("little, wanna-be cock");
        }
        else
        {
            outputText("bad boy");
        ...
        {
            outputText("to your flare, sucking, teasing and ");
        }
        //[demonic]
        else if (player.cocks[x].cockType == CockTypesEnum.DEMON)
        {
            outputText("to the demonic nodules ringing your cock, ");
        }
        //[anemone]
        else if (player.cocks[x].cockType == CockTypesEnum.ANEMONE)
        {
            outputText("to the little wriggling tentacles ringing the head and base of your cock, ");
        }
        //[canine]
        else if (player.hasKnot(x))
        {
            outputText("to the base of your cock, planting sloppy kisses on your knot, ");
        }
        else
        {
            outputText("to the sensitive little spot on the underside of the head, ");
        }
        ...
        if (player.cumQ() >= 250)
        {
            ...
            if (player.cumQ() < 500)
            {
                outputText("some of it begins to dribble down your cock, forming a puddle right under your ass cheeks");
            }
            else if (player.cumQ() < 1000)
            {
                outputText("it begins to spray out of the edges of your cock, like water coming out of a blocked faucet");
            }
            else if (player.cumQ() < 2000)
            {
                outputText("Callu's stomach begins to visibly inflate, even from your point of view");
            }
            else
            {
                outputText("Callu's stomach inflates to a huge degree. She suddenly looks to be about eight months pregnant, though she doesn't seem bothered by this in the least");
            }
            outputText(".");
        }
        ...
        if (player.cumQ() >= 2000)
        {
            outputText(", despite the huge belly she now sports");
        }
        ...
        if (player.hasKnot(x))
        {
            outputText("fails, held fast by the knot tying the two of you together. She looks at you in surprise, but eventually smiles and leans back down. The two of you cuddle for half an hour, until your knot deflates enough to let her *pop* off of it. She stands and ");
        }
    ...
    private function ottersForGals() : Void
    {
        ...
        {
            outputText("unnatural bare crotch");
        }
        ...
        {
            outputText("You even slip a finger or two inside, stretching your nipple-cunts out with deliciously pleasurable results.  ");
        }
        outputText("Combined with Callu's tender tongue ");
        if (player.hasVagina())
        {
            outputText("paying lip service to your wet cunt");
        }
        else
        {
            outputText("doing a cave dive in your rear");
        }
        ...
        if (player.hasVagina())
        {
            outputText("Your snatch clenches and clamps down on thin air, flooding Callu's face with your feminine juices in a tasty, refreshing spray.");
        }
        else
        {
            outputText("Your asshole clenches and spasms randomly, aching to be filled by something, anything in your quest for release.");
        }
    ...
    private function getDatFishingPole() : Void
    {
        ...
        if (player.gems >= 50)
        {
            addButton(0, "Buy it", yesMissIHaveGems);
        }
        else
        {
            addButtonDisabled(0, "Buy it", "You don't have enough gems.");
        }
