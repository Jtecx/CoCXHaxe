...
class GooGirl extends Monster
{
    ...
    private function gooGalAttack() : Void
    {
        ...
        {
            outputText("The slime holds its hands up and they morph into a replica of your [weapon].  Happily, she swings at you");
        }
        //Determine if dodged!
        ...
        if (hasPerk(PerkLib.Acid))
        {
            damage = as3hx.Compat.parseInt((str + 10 + weaponAttack) - rand(player.tou) - player.armorDef);
        }
        else
        {
            damage = as3hx.Compat.parseInt((str + weaponAttack) - rand(player.tou) - player.armorDef);
        }
    ...
    override public function teased(lustDelta : Float, isNotSilent : Bool = true, display : Bool = true, aura : Bool = false) : Void
    {
        if (lust <= maxLust() * 0.99)
        {
            ...
            else if (lustDelta < maxLust() * 0.13)
            {
    ...
    public function new(noInit : Bool = false)
    {
        ...
        var playerHasBigBoobs : Bool = player.biggestTitSize() >= 3;
        this.a = "the ";
        this.short = "goo-girl";
        ...
        this.level = 7;
        this.gems = rand(6) + 4;
        this.drop = new ChainedDrop().add(weapons.PIPE, 1 / 10).add(consumables.WETCLTH, 1 / 2).elseDrop(useables.GREENGL);
