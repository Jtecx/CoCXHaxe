...
class FetishZealot extends Monster
{
    
    private static inline var RELIGIOUS_CLOTHES : String = "religious clothes";
    private static inline var PIRATE_CLOTHES : String = "pirate clothes";
    private static inline var MILITARY_CLOTHES : String = "military clothes";
    private static inline var LEATHER_CLOTHES : String = "leather clothes";
    private static inline var STUDENTS_CLOTHES : String = "student's clothes";
    ...
    override public function combatRoundUpdate() : Void
    {
        ...
        if (changed)
        {
            outputText("The " + this.short + "'s clothing shifts and twists, until he is wearing " + armorName + ".\n\n");
        }
        lust += lustVuln * 5;
    }
    ...
    //Costumes
    {
        ...
        //In front of you is someone wearing a green military uniform.  They obviously aren't in any military you've ever heard of, as on his shoulder he has emblazoned <i>FF Army Sex Instructor</i>.  It seems you are his latest Recruit...{
            ...
            if (player.cor > 50)
            {
                outputText("You eagerly cry out \"<i>Yes, sir!</i>\" and show yourself off to the best of your ability.  The whole act is extremely arousing.");
            }
            //[player is not corrupt]
            else
            {
                outputText("You have no idea why, but you promptly display yourself in the most provocative way possible.  After a moment you realize what you're doing and quickly stop, flushed with embarrassment and arousal.");
            }
        }
        //Gimp gear;
        if (armorName == LEATHER_CLOTHES)
    ...
    override private function postAttack(damage : Int) : Void
    {
        if (damage > 0)
        {
    ...
    public function new()
    {
        ...
        this.lust = 25;
        this.lustVuln = 0.75;
        this.drop = new WeightedDrop().add(armors.C_CLOTH, 1).add(consumables.L_DRAFT, 2).add(weapons.L_DAGGR, 1);
