...
class Kirin extends Monster
{
    private var superCharged : Bool = false;
    private var runAway : Bool = false;
    ...
    private function kirinThunderCharge(attacking : Bool = false) : Void
    {
        runAway = false;
        var string : String = "";
    ...
    override public function isLustDefeatable() : Bool
    {
        return false;
    }
    ...
    override public function combatRoundUpdate() : Void
    {
        ...
        if (!superCharged && lust100 > 75)
        {
            ...
            statStore.replaceBuffObject({
                        spe.mult : Math.round(speStat.mult.value)
                    }, "Supercharged", {
                        text : "Supercharged!"
                    });
    ...
    public function new()
    {
        ...
        this.lustVuln = .8;
        this.gems = 500;
        this.drop = new ChainedDrop().add(consumables.EQUINUM, 0.3).add(consumables.ASKIRIN, 0.7);
