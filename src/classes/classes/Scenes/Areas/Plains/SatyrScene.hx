...
class SatyrScene extends BaseContent
{
...
*/
    ...
    public function satyrEncounter(location : Int = 0) : Void
    {
        ...
        if (rand(2) == 0 || player.isPregnant() || player.isButtPregnant() || player.gender == 0)
        {
            ...
            if (location == 0)
            {
                outputText("grassy plains");
            }
            else
            {
                outputText("sodden expanse of the swamp");
            }
        ...
        {
            //Happens in either the plains or swamp  //Pregnant PCs can't get this encounter.  {
                ...
                if (location == 0)
                {
                    doYesNo(createCallBackFunction(consensualSatyrFuck, 0), explorer.done);
                }
                else
                {
                    doYesNo(createCallBackFunction(consensualSatyrFuck, 0), explorer.done);
    ...
    private function consensualSatyrFuck(loc : Int = 0) : Void
    {
        ...
        if (loc == 0)
        {
            outputText("trampled-down grass");
        }
        else
        {
            outputText("relatively dry and solid ground");
        }
    ...
    @:allow(classes.scenes.areas.plains)
    private function loseToSatyr() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_satyr);
        //[Lust loss
        ...
        {
            outputText("You pant as you begin masturbating furiously, too horny to care about anything the grinning satyr before you has in mind.");
        }
        else
        {
            outputText("You try to steady yourself, clutching your body in pain as the satyr grins at you.");
        }
        ...
        if (player.isNaga())
        {
            outputText("pins down your serpentine body");
        }
        else if (player.isBiped())
        {
            outputText("spreads your [legs]");
        }
        else
        {
            outputText("pins you down");
        }
        outputText(" and reaches forward, probing for a vagina.  ");
        if (player.hasVagina())
        {
            outputText("Upon finding one, he grins lustfully, his already impressively erect cock rising to full flag.");
        }
        else
        {
            outputText("Finding you lacking that trait, he grunts disdainfully and probes roughly for an asshole, which he locates right between your buttcheeks where it belongs, then jerks himself with several strokes to help coax his masculinity to full erection.");
        }
        ...
        if ((player.hasVagina() && player.vaginalCapacity() < monster.cockArea(0)) || (!player.hasVagina() && player.analCapacity() < monster.cockArea(0)))
        {
            outputText(", not even bothered by the fact that his cock doesn't even fit inside you");
        }
        outputText(".");
        //[cunt/buttchange]
        if (player.hasVagina())
        ...
        if (player.hasCock())
        {
            outputText("  [EachCock] throbs as it send strings of cum flying straight into the satyr's chest, painting it white.");
        }
        if (player.hasVagina())
        {
            outputText("  A veritable flood of girlcum escapes the small space between your vaginal walls and the satyr's massive shaft, covering his balls and legs with your female fluids.");
        }
    ...
    //Victory Rapes
    @:allow(classes.scenes.areas.plains)
    private function defeatASatyr() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_satyr);
        //Lust Victory
        ...
        {
            outputText("The satyr collapses to its caprine knees, bleating in dismay as it paws frantically at its huge cock, oblivious to everything in its need to get off.  Already, pre-cum is fountaining from the goat-man's shaft, his jerking motions smearing the pungent sexual fluid across the crown.");
        }
        //HP Victory
        else
        {
            outputText("Beaten and dazed, the satyr collapses to its caprine knees, shaking his head in a futile attempt to recover himself from the brutal trouncing you've just given him.  The combination of the blows and his previous drunken state mean he's quite incapable of getting back, however.");
        }
        if (player.lust < 33)
        {
    ...
    private function femaleTakesAdvantageOfSatyr() : Void
    {
        ...
        if (player.hasCock())
        {
            ...
            {
                outputText("the base of your " + multiCockDescriptLight());
            }
        }
        ...
        if (player.hasCock())
        {
            outputText(", your own cock belching its usual load across his stomach and the surroundings");
        }
        outputText(".");
    ...
    private function malesTakeAdvantageOfSatyrs() : Void
    {
        ...
        if (player.cumQ() < 250)
        {
            outputText("  He tries to milk you for far more than you can give him, and at one point you feel like his anal muscles are going to tear your dick off, until finally they go lax and lets you slip out.");
        }
        //(Medium Cum Amount)
        else if (player.cumQ() < 1000)
        {
            outputText("  You dump far more than you usually do; the satyr's clenching asshole makes sure to drain you of every little drop of cum you have until finally it goes lax and lets you slip out, leaving the goat-man with a pudgy belly.");
        }
        //(High Cum Amount)
        else
        {
            outputText("  You shoot jet after jet of cum, far faster than the satyr's ass can milk you for it, and still he doesn't seem to stop trying.  His rippling muscles draw your cum inside his bowels in big bubbles.  When his ass finally goes lax, you're propelled halfway out of him by the force of your own backflowing seed; the satyr's ass looks like a wreck, and his belly is overflowing with so much spunk you're surprised it didn't come out of his mouth.");
        }
    ...
    private function willinglyBoneSatyr() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("hardening maleness, giving it a few strokes to bring it to full mast, then he moves on to your ");
        }
        if (player.hasBalls())
        {
            outputText("[balls], rolling each of your orbs around with reverence at the life-giving load they produce; finally he addresses your ");
        }
        //((if PC has a pussy)
        if (player.hasVagina())
        {
            outputText("[vagina], spreading your labia wide to see the interior of your rapidly moistening walls.");
        }
        else
        {
            outputText("[ass], spreading your cheeks apart to gaze at your winking rosebud.");
        }
        ...
        if (player.hasVagina())
        {
            outputText("[vagina], licking all over your netherlips and tasting your juices.");
        }
        else
        {
            outputText("[butt], licking all around your hole to prepare it for what's coming.");
        }
        ...
        if (!player.hasVagina())
        {
            outputText("clenching butt");
        }
        else
        {
            outputText("spasming vagina");
        }
        ...
        if (player.hasVagina())
        {
            outputText("pussy gushes fluids down his massive manhood");
        }
        if (player.gender == 3)
        {
            outputText(" and ");
        ...
        if (player.hasCock())
        {
            outputText("cock spurts cream across your intertwined forms");
        }
    ...
    //Birthing (Z)
    //Baby is always male.
    public function satyrBirth(vag : Bool, womb : Int = 0) : Void
    {
        spriteSelect(SpriteDb.s_satyr);
        outputText("\nSudden sharp, intense pangs rip through your gut, seeming to emanate from your ");
        if (vag)
        {
            outputText(player.vaginaDescript(womb));
        }
        else
        {
            outputText("[butt]");
        ...
        {
            player.buttChange(40, true, false, true);
        }
