...
class KirinScene extends BaseContent
{
    ...
    //spriteSelect(SpriteDb.s_naga);
    {
        ...
        var string : String = "";
        var pRaiju_Check : Bool = player.isRace(Races.RAIJU, 1, false);
        var pThunderbird_Check : Bool = player.isRace(Races.THUNDERBIRD, 1, false);
        var pKirin_Check : Bool = player.isRace(Races.KIRIN, 1, false);
    ...
    {
        var meetElectra : Bool = (flags[kFLAGS.ELECTRA_TALKED_ABOUT_HER] >= 1);
        ...
        {
            var playerVagina : Bool = player.hasVagina();
    ...
    private function kirin_FriendlyDischarge() : Void
    {
        clearOutput();
        var string : String = "";
        var pGear : Bool = player.armorDescript() == "gear";
        
        var cumVolume : Float = player.cumQ();
        var milkVolume : Float = player.lactationQ();
        
        var pCannon : Bool = player.perkv1(IMutationsLib.MinotaurTesticlesIM) >= 1 && cumVolume > 0;
        var pMilk : Bool = player.perkv1(IMutationsLib.LactaBovinaOvariesIM) >= 1 && milkVolume > 0;
    ...
    public function kirinWon() : Void
    {
        ...
        + " pass right through your "
        + ((playerVagina) ? "cunt lips as it seeks out your pussy" : "butt cheeks as it seeks out your ass hole")
        + ". The kirin lightning rod finds purchase and rushes through, delivering a discharge that reaches all the way to your brain, you almost blacked out as pleasure overloads your senses and sending your body to spasm with each thrust.\n\nShock after shock, and just when you thought that she is never going to stop fucking you, she finally unloads. A cock that is almost as thick as a ball of fist, somehow felt even thicker inside you as what must be an enormous volume of gunk rushing through her cock, and finally, reaches your "
        + ((playerVagina) ? "womb" : "bowel")
        + " in a nigh constant stream, bloating your belly until you look like 3 or 4 months into pregnancy as her girth utterly plugged your hole for any reasonable leakage.\n\nFinally finished unloading, kirin leaves you paralysed on the ground and gallops away with a sigh of relief. It isn't long before gnoll and other less morally clean denizen walked by and abuses your body for sexual relief, and many hours later you finally managed to stand up and walk back to your camp.";
