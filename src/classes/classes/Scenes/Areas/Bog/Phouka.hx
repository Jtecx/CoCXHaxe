...
class Phouka extends Monster
{
    private function phoukaFightAttack() : Void
    {
    ...
    private function phoukaFightLustAttack() : Void
    //Only the faerie, bunny and horse forms make lust attacks
    {
        ...
        {
            outputText("The stallion rears up on his hind legs, waving his massive cock at you.  ");
        }
        ...
        if (player.getEvasionRoll())
        {
            ...
            {
                outputText("You manage to look away in time and the " + this.short + "'s lewd display has no real effect on you.\n");
            }
        }
        else
        {
            ...
            {
                outputText("You are hypnotized by the equine cock jabbing at the air.  Then the " + this.short + " charges past you and you can taste the musk in the air.");
            }
            player.takeLustDamage(30 + player.lib / 5 + player.cor / 2 + rand(25), true);
        }
    }
    
    private function phoukaFightSilence() : Void
    ...
    {
        var blinded : Bool = hasStatusEffect(StatusEffects.Blind);
        ...
        else
        {
            ...
            if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_FAERIE)
            {
                if (blinded)
                {
                    transformChance = rand(3);
                }
                //100% chance of change from blind phouka if not doing silence attack
                else
                {
                    transformChance = rand(4);
                }
            }
            switch (transformChance)
            {
                case 0:phoukaTransformToBunny();
                case 1:phoukaTransformToGoat();
    ...
    override public function teased(lustDelta : Float, isNotSilent : Bool = true, display : Bool = true, aura : Bool = false) : Void
    {
        if (lustDelta >= 10)
        {
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (player.hasStatusEffect(StatusEffects.RiverDungeonA))
        {
            SceneLib.dungeons.riverdungeon.defeatedByMistPhouka();
        }
        else if (pcCameWorms)
        {
            ...
            {
                outputText("lad and spread yer asscheeks for me.\"</i>\n\n");
            }
            doNext(SceneLib.combat.endLustLoss);
        }
        else if (player.hasVagina() && (!sceneHunter.uniHerms || rand(3) > 0))
        
        //Phouka prefer vaginal if they can get it{
            
            if (player.isTaur() || rand(2) == 0)
            {
                SceneLib.bog.phoukaScene.phoukaSexHorse(true, !hpVictory);
            }
            //And they love mating with female or herm centaurs in their horse form
            else
            {
                SceneLib.bog.phoukaScene.phoukaSexBunny(true, !hpVictory);
            }
        }
        else
        {
            SceneLib.bog.phoukaScene.phoukaSexGoat(true, !hpVictory);
        }
    }
    
    private function phoukaTransformToBunny() : Void
    {
        if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_BUNNY)
        {
            return;
    ...
    private function phoukaTransformToGoat() : Void
    {
    ...
    private function phoukaTransformToHorse() : Void
    {
    ...
    private function phoukaTransformToPhouka() : Void
    {
        if (PhoukaScene.phoukaForm == PhoukaScene.PHOUKA_FORM_FAERIE)
        {
            return;
    ...
    public function new(phoukaName : String)
    {
        super();
        if (player.hasStatusEffect(StatusEffects.RiverDungeonA))
        {
        ...
        {
            this.drop = new WeightedDrop().add(consumables.BLACK_D, 2).add(consumables.RIZZART, 2).add(consumables.GROPLUS, 2).add(consumables.SDELITE, 2).add(consumables.P_WHSKY, 2).add(consumables.CHOCBOX, 8).add(useables.S_SHARD, 40);
        }
        else if (player.hasStatusEffect(StatusEffects.RiverDungeonA))
        {
            this.drop = new WeightedDrop().add(consumables.P_WHSKY, 10).add(consumables.CHOCBOX, 20).add(useables.PCSHARD, 20);
        }
        else
        {
            this.drop = new WeightedDrop().add(consumables.BLACK_D, 15).add(consumables.RIZZART, 10).add(consumables.GROPLUS, 2).add(consumables.SDELITE, 13).add(consumables.P_WHSKY, 10).add(consumables.CHOCBOX, 15);
        }
