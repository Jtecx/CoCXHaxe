...
class LizanRogueScene extends BaseContent
{
    ...
    public function lizanIntro() : Void
    {
        ...
        else
        {
            ...
            if (flags[kFLAGS.MET_LIZAN_FIRST_TIME] <= 0)
            {
                ...
                {
                    outputText("He is shorter than you and wearing nothing but a loincloth with a pack belted onto it.");
                }
                else if (player.tallness >= 60)
                {
                    outputText("He is about the same height as you and wearing nothing but a loincloth with a pack belted onto it.");
                }
                else
                {
                    outputText("He is taller than you and wearing nothing but a loincloth with a pack belted onto it.");
                }
    ...
    public function fightLizan(pure : Bool = false) : Void
    {
        clearOutput();
        startCombat(new LizanRogue());
        if (!pure)
        {
            outputText("You ready your [weapon], prepared to let the little punk know just who it is he is dealing with.");
        }
        else
        {
    ...
    public function leaveLizan(pure : Bool = false) : Void
    {
        clearOutput();
        if (!pure)
        {
    ...
    public function sexMenuRough() : Void
    {
        ...
        if (player.lust >= 33)
        {
            ...
            if (player.hasVagina())
            {
                addButton(1, "Use Pussy", rapeLizanWithPussy).hint("Get on top of the lizan and stuff his cock into your [pussy].");
            }
    ...
    public function haveSexWithLizan(continuation : Bool = false) : Void
    {
        if (!continuation)
        {
            clearOutput();
        ...
        if (player.hasCock())
        {
            addButton(0, "Use Dick", consensualButtfuck).hint("Anally penetrate him with your penis.");
        }
        if (player.hasVagina())
        {
            addButton(1, "Use Vagina", consensualGetFucked).hint("Have him take you vaginally.");
        }
        addButton(2, "Use Ass", consensualGetButtFucked).hint("Have him take you anally.");
        if (player.hasVagina())
        {
            addButton(3, "Dbl.Penetration", consensualDoublePenetration).hint("Have him stuff both your holes with his dual cocks.", "Double Penetration");
        }
        if (flags[kFLAGS.WATERSPORTS_ENABLED] > 0 && flags[kFLAGS.LIZAN_ROGUE_SEX_COUNTER] >= 3 && !continuation)
        {
            addButton(4, "Watersports", consensualWatersports).hint("Participate into urine activity with him. \n\nNOTE: Contains watersports!");
        }
    ...
    //start
    {
        ...
        var consButt_multiSel : Int->Void = function(cock : Int) : Void
        {
            sceneHunter.selectSingleMulti(curry(scene, cock, false), curry(scene, cock, true));
        }
        
        var scene : Int->Bool->Void = function(x : Int, multi : Bool) : Void
        {
            if (!multi)
            ...
            else
            {
                ...
                if (player.hasBalls())
                {
                    outputText("You groan as you feel his spit drip down your [balls] and [legs].");
                }
                if (player.isBiped())
                {
            ...
            if (player.cumQ() < 350)
            {
                outputText("Your [face] twists around a strained groan, and you spray your own impressive load deep into the lizan's bowels.");
            }
            else if (player.cumQ() < 1000)
            {
                outputText("Your [face] twists around a deep growl as you pour your load deep into the lizan's bowels. As you fuck your seed deeper into his clenching bowls you can feel some of it gushing out and dripping down your [legs].");
            }
            else if (player.cumQ() < 2500)
            {
                outputText("Your [face] twists into a chilling howl as you explode deep within the lizan's bowels. As you fuck your seed deep into his bowels his stomach becomes a little less defined, a testament to how much seed you've poured into him. You can even feel it dripping down your [legs].");
            }
            else
            {
                outputText("Your [face] twists into a titanic roar as [eachCock] explodes with enough force to blast out of the lizan's rectal embrace. Torrents of creamy white fluid blast from [eachCock]. Soon the lizan's upturned ass and the base of his tail are covered, but you're still going. By the time you're finished the lizan is grinning from ear to ear amidst a small lake of cum.");
            }
    ...
    public function consensualGetFucked() : Void
    {
        ...
        if (player.isLactating() && (!player.hasFuckableNipples() || rand(2) == 0))
        {
            outputText("When he reaches your [fullChest] he takes one look at your [nipples] and cocks his head to the side in confusion. His tongue slips out to taste a single bead of creamy white, a taste you see repeated on the next breast. He seems to really like your lactating nipples. He opens his mouth wide and latches on to one of your [nipples], his hand playing with another. The tingling sensation he causes is quite pleasant. He quite happily slurps milk from your [nipples] and massages the soft flesh in a way that is both firm and erotic.");
        }
        else if (player.hasFuckableNipples())
        {
            outputText("When he reaches your [fullChest] he takes one look at your [nipples] and does a double take. For a moment he is at a loss for words but when you clear your throat he quickly collects his wits and sticks his tongue into your fuckable nipples. The sensation is similar to getting your pussy licked but much higher up. You groan as he suckles each nipple in turn while massaging your [chest]");
        }
        else
        {
            outputText("When he reaches your [fullChest] he takes one look at your [nipples] and dives right in. He makes a show of pulling one into his mouth. All you can see is his " + monster.bodyColor + " lips but inside you can feel his tongue swirling around your sensitive nipples. He sucks off with a pop that sends a tingle down to your groin before he latches on to the next one and repeats this torturous practice.");
        }
        ...
        else if (player.averageVaginalLooseness() <= 3)
        ...
        //Not gaping{
            ...
            {
                outputText("His other lizan cock explodes as well, painting your raised [tail] and [ass] with a generous coating of funky lizan spunk.");
            }
            else
            {
                outputText("His other lizan dick explodes as well, painting your [ass] and lower back with a generous coating of lizan baby batter.");
            }
        }
        //Gaping
        else
    ...
    public function consensualDoublePenetration() : Void
    {
        ...
        if (player.hasCock())
        {
            ...
            {
                outputText("Your [cock] erupts as your [pussy] floods around the lizan's still thrusting cock.");
            }
            if (player.cumQ() >= 1000)
            {
                outputText("The lizan squeezes your still spurting [cock] as beneath the two of you your pearly white dick snot forms a large puddle.");
            }
        }
        else
        {
    ...
    public function consensualWatersports() : Void
    //Yay watersports!
    {
        ...
        else if (player.hasCock())
        {
            ...
            {
                outputText("You stand in front of the lizan and aim your [cock] at him and release your pressure, a stream of warm urine flows from your penis");
            }
            else
            {
                outputText("You stand in front of the lizan and aim your [cock] at him and release your pressure, streams of warm urine flow from your multiple cocks");
            }
        }
        //Just because you're a genderless doesn't mean you can't pee.
        else
        {
