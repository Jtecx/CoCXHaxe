...
class PhoukaScene extends BaseContent implements TimeAwareInterface
{
    
    @:allow(classes.scenes.areas.bog)
    private static var phoukaForm : Int = 0;  //This keeps track of the form of the phouka across different scenes and through combat  
    @:allow(classes.scenes.areas.bog)
    private static inline var PHOUKA_FORM_FAERIE : Int = 0;
    @:allow(classes.scenes.areas.bog)
    private static inline var PHOUKA_FORM_BUNNY : Int = 1;
    @:allow(classes.scenes.areas.bog)
    private static inline var PHOUKA_FORM_GOAT : Int = 2;
    @:allow(classes.scenes.areas.bog)
    private static inline var PHOUKA_FORM_HORSE : Int = 3;
    ...
    private function phoukaFaerieFire() : Void
    //In old Irish legends the Phouka are responsible for Will-o’-the-wisps. In the deep woods it’s Kitsune, but in the bog it’s a Phouka
    {
        ...
        menu();
        if (flags[kFLAGS.PHOUKA_ENCOUNTER_STATUS] != 0 || player.hasKeyItem("Traveler's Guide") >= 0)
        {
            addButton(4, "Leave", phoukaFaerieFireLeave);
        }
        addButton(0, "Follow", phoukaStuck);
    }
    
    private function phoukaFaerieFireLeave() : Void
    {
        if (flags[kFLAGS.PHOUKA_ENCOUNTER_STATUS] == 0)
        {
    ...
    private function phoukaStuck() : Void
    {
        clearOutput();
        var hasAlcohol : Bool = player.hasItem(consumables.P_WHSKY, 1);
        ...
        if (flags[kFLAGS.PHOUKA_ENCOUNTER_STATUS] == 0)
        {
            flags[kFLAGS.PHOUKA_ENCOUNTER_STATUS] = 1;
        }
        
        menu();
        if (hasAlcohol)
        {
            addButton(0, "Offer Drink", phoukaStuckOfferWhiskey);
        }
        addButton(1, "Talk", phoukaTalk);
        addButton(2, "Fight", phoukaStartFight);
    }
    
    private function phoukaStuckOfferWhiskey() : Void
    {
        clearOutput();
    ...
    private function phoukaMasturbating() : Void
    ...
    private function phoukaStartFight() : Void
    {
    ...
    private function phoukaStartFightSilenced() : Void
    {
        startCombat(new Phouka(phoukaName()));
        player.createStatusEffect(StatusEffects.WebSilence, 0, 0, 0, 0);
    }
    
    private function phoukaTalk() : Void
    {
        clearOutput();
        ...
        {
            outputText(" if he really wants a taste of what you're going to give him.");
        }
        
        if (player.cor + rand(125) <= 110)
        ...
        //Players with 0% corruption have a 12% chance that the Phouka is willing to talk, players with 100% corruption have a 92% chance{
            ...
            {
                outputText("fuckable little " + phoukaName());
            }
        ...
        else
        {
            ...
            outputText("\n\nThe " + phoukaName());
            var _sw0_ = ((player.level < 10) ? rand(8) : rand(12));            

            switch (_sw0_)
            {
                case 0, 1, 2, 3, 4, 5, 6:  //Offers you a chance to submit, which leads to the various defeat sex scenes  
                ...
                case 7, 8:  //Decides he doesn't want you  
                    ...
                    break;  //(9, 10 or 11) Offers to share some whiskey  
    ...
    private function phoukaDrinkWhilePregnant(playerOfferedTheBooze : Bool) : Void
    {
        if ((player.hasVeryVisiblePregnancy()) || (player.buttPregnancyIncubation <= sceneHunter.adjustPregEventTimer(100, player.buttPregnancyType)))
        ...
        if (acceptable < 0)
        {
            ...
            if (acceptable == -4)
            {
                outputText("  One of them seems to crave the whiskey, but the other is a more normal child.");
            }
    ...
    private function phoukaDrinkAccept() : Void
    //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
    {
        ...
        else if (player.touStat.core.value < 70)
        {
    ...
    private function phoukaDrinkRefuse() : Void
    //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
    {
        
        clearOutput();
        if ((player.buttPregnancyIncubation == 0) && (player.hasVisiblePregnancy()))
    ...
    private function phoukaDrinkRefuseLeave() : Void
    //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
    {
    ...
    private function phoukaDrinkRefusePregnantLeave() : Void
    //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
    {
    ...
    private function phoukaDrinkRefuseAfterOffer() : Void
    {
        clearOutput();
    ...
    private function phoukaDrinkTalk(playerNotDrinking : Bool) : Void
    {
        ...
        var playerIsInebriated : Bool = (playerNotDrinking == false) && (player.tou < 70);
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    outputText("and you feel your sphincter twitch as your brain thinks about what you could do with this phouka now.");
                }
            }
            ...
            if (player.lust >= 33)
            {
                ...
                if (player.hasCock())
                {
                    addButton(2, "Fuck Faerie", phoukaSexFaerieFemaleNonCombat);
                }
            }
        }
    }
    
    private function phoukaDrinkLeave() : Void
    {
        if (player.lust < 33)
        {
    ...
    public function phoukaHalloween() : Void
    {
        ...
        if (player.inte > 80)
        {
            addButton(4, "Leave", phoukaHalloweenLeave);
        }
    }
    
    private function phoukaHalloweenLeave() : Void
    {
    ...
    private function phoukaHalloweenTreacleEater() : Void
    {
        clearOutput();
    ...
    private function phoukaLeaveOnLustWin(newScreen : Bool = true) : Void
    {
        if (newScreen)
        {
            clearOutput();
    ...
    @:allow(classes.scenes.areas.bog)
    private function phoukaPlayerWins(hpVictory : Bool) : Void
    {
        clearOutput();
        if (hpVictory)
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    outputText(".  It would be so easy to rape this perverted faerie, how can you resist?");
                }
                menu();
                ...
                if (player.hasCock())
                {
                    addButton(2, "Fuck Faerie", phoukaSexFaerieFemalePostCombat);
                }  //Male or Herm Only  
    ...
    public function phoukaPregUpdate(womb : Dynamic) : Bool
    //Belly size doesn't change, instead you get updates on what's going on
    {
        ...
        if (Reflect.field(womb, "incubation") == sceneHunter.adjustPregEventTimer(36, Reflect.field(womb, "type")))
        ...
        //Stage 5:{
            ...
            {
                outputText("Despite being pregnant you still don't feel restricted or slowed in any way.  It's as if the tiny life inside is living in harmony with you.\n");
            }
            return true;
        }
        return false;
    ...
    private function phoukaSexAddStandardMenuChoices() : Void
    ...
    @:allow(classes.scenes.areas.bog)
    private function phoukaSexBunny(postCombat : Bool, lustLoss : Bool) : Void
    //Player must have a vagina for bunny-morph sex
    {
        ...
        if (postCombat)
        {
            ...
            {
                outputText("body too bruised to");
            }
        ...
        else if (player.cor <= 50)
        {
            ...
            if (phoukaForm != PHOUKA_FORM_BUNNY)
            {
                outputText(", and morphs into a rabbit");
            }
        ...
        else
        {
            ...
            if (phoukaForm != PHOUKA_FORM_BUNNY)
            {
                outputText("  He goes through a short series of warm up exercises and then transforms himself into the rabbit you desire.");
            }
        }
        phoukaForm = PHOUKA_FORM_BUNNY;
    ...
    private function phoukaSexBunnyChoice() : Void
    {
        phoukaSexBunny(false, false);
    }
    
    @:allow(classes.scenes.areas.bog)
    private function phoukaSexGoat(postCombat : Bool, lustLoss : Bool) : Void
    //Player does not have a vagina or has chosen anal
    {
        ...
        if (postCombat)
        {
            ...
            if (phoukaForm != PHOUKA_FORM_GOAT)
            {
                outputText("  As you lay there, the " + phoukaName() + "'s body stretches and morphs until it takes the form of a large, black furred goat-morph.");
            }
        }
        else
        {
            ...
            {
                outputText("Then you put on a show, taking your clothes off piece by piece.  You strip off your final undergarment, bend over and wink your asshole at the goat.");
            }
        }
        ...
        if (postCombat)
        {
            outputText("  While you're recovering the goat-morph reaches into your gem pouch and takes a handful.");
        }
    ...
    private function phoukaSexGoatChoice() : Void
    {
        phoukaSexGoat(false, false);
    }
    
    @:allow(classes.scenes.areas.bog)
    private function phoukaSexHorse(postCombat : Bool, lustLoss : Bool) : Void
    //Player has a vagina
    {
    ...
    private function phoukaSexHorseChoice() : Void
    {
        phoukaSexHorse(false, false);
    }
    
    private function phoukaSexFaerie(postCombat : Bool) : Void
    {
        clearOutput();
        ...
        else
        {
            ...
            if (player.hasCock())
            {
                outputText("  [EachCock] fires several blasts of cum into the bog [if (isTaur = true)beneath you][if (isTaur = false)and all over your faerie partner].");
            }
    ...
    private function phoukaSexFaerieNonCombat() : Void
    {
        phoukaSexFaerie(false);
    }
    
    private function phoukaSexFaeriePostCombat() : Void
    {
        phoukaSexFaerie(true);
    }
    
    private function phoukaSexFaerieFemale(postCombat : Bool) : Void
    {
        ...
        if ((flags[kFLAGS.FREED_VALA] == 1) || (flags[kFLAGS.TIMES_PC_DEFEATED_VALA] > 0) || (flags[kFLAGS.TIMES_FUCKED_VALA_IN_DUNGEON] > 0) || (flags[kFLAGS.INVESTIGATED_VALA_AFTER_ZETAZ_DEFEATED] > 0))
        ...
        {
            outputText("You push the last inch of your " + as3hx.Compat.parseInt(player.biggestCockLength()) + " prick into the moaning slut.");
        }
        else if (player.biggestCockLength() < 15)
        {
            outputText("You feed the last inch of your " + as3hx.Compat.parseInt(player.biggestCockLength()) + " cock into her.  Your cockhead bumps against her cervix and the slut beneath you begs for more.");
        }
        else
        {
    ...
    private function phoukaSexFaerieFemaleNonCombat() : Void
    {
        phoukaSexFaerieFemale(false);
    }
    
    private function phoukaSexFaerieFemalePostCombat() : Void
    {
        phoukaSexFaerieFemale(true);
    }
    
    private function phoukaSexPregnate(postCombat : Bool) : Void
    //Whether by horse, bunny or (male) faerie sex it all ends up here if the PC has a vagina
    {
        ...
        if ((!postCombat) || (player.cor > 50))
        {
            ...
            else
            {
                ...
                {
                    outputText("Perhaps he's being a considerate lover or perhaps he just wants to better his chances of knocking you up.");
                }
            }
        }
        else
        ...
        if (postCombat)
        {
            outputText(" rummages around in your gem pouch and then");
        }
        outputText(" takes to the air and hovers nearby as if waiting for something.\n\n");
        if (player.isPregnant())
        {
            var isFirstTime : Bool = (flags[kFLAGS.BIRTHS_PHOUKA] == 0) && (flags[kFLAGS.BIRTHS_FAERIE] == 0);
            ...
            if (!postCombat || (player.cor > 50))
            {
                outputText(", yer a real prize slut");
            }
            ...
            {
                outputText("At least you know what to expect from this form of pregnancy now.");
            }
        }
        else
        {
            outputText("When you pull yourself up your belly begins to deflate and leak its gooey grey payload into the mud.  The " + phoukaName() + " says <i>\"Ah well, there's always next time,\"</i> and buzzes away to some other part of the bog.");
        }
        phoukaSexPregnateEnd(postCombat);
    }
    
    private function phoukaSexPregnateEnd(postCombat : Bool) : Void
    //Everything after the sex. Handles awards, gem loss and text for player leaving the bog
    {
        ...
        //In this case postCombat means you need an award because you must have won to get faerie sex{
            ...
            if (player.cor > 50)
            {
                outputText("  But either way you plan to return and give all of them that lesson.");
            }
            if (postCombat)
            {
                clearStatuses(false);
        ...
        {
            if (postCombat && player.gems > 0)
            {
                outputText("  While you're recovering the " + phoukaName() + " reaches into your gem pouch and takes a handful.");
            }
            ...
            outputText("\n\nYou ");
            if (phoukaForm == PHOUKA_FORM_HORSE && !player.isTaur())
            {
                outputText("wait for the throbbing pain in your pelvis to subside.  Then you ");
            }
            if (postCombat)
            {
            ...
            {
                outputText("Not what you expected, but at least you got off.");
            }
        }
    }
}
