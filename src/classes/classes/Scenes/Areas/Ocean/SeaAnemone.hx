...
class SeaAnemone extends Monster
{
    private static inline var STAT_DOWN_FLAT : Int = 4;
    private static inline var STAT_DOWN_MULT : Int = 4;
    private var cuteCheck : Bool = true;
    ...
    override public function eOneAttack(display : Bool = false) : Int
    {
    ...
    override public function postMeleeDmgSkip(attackInstance : Int) : Bool
    {
        if (attackInstance == 1)
        {
            return cuteCheck;
    ...
    public function applyVenom(amt : Float = 1) : Void
    {
        var ave : AnemoneVenomDebuff = try cast(player.createOrFindStatusEffect(StatusEffects.AnemoneVenom), AnemoneVenomDebuff) catch(e:Dynamic) null;
        ave.applyEffect(amt * 3);
    }
