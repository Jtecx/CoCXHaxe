...
class UnderwaterSharkGirlsPack extends Monster
{
    ...
    private function sharkBiteAttack() : Void
    {
        ...
        if (!player.immuneToBleed())
        {
            ...
            {
                player.createStatusEffect(StatusEffects.Hemorrhage, SceneLib.combat.debuffsOrDoTDuration(3), 0.2, 0, 0);
            }
        }
    }
    ...
    public function new()
    {
        ...
        this.drop = new WeightedDrop().add(consumables.L_DRAFT, 3).add(armors.S_SWMWR, 1).add(consumables.TSTOOTH, 5).add(null, 1);
