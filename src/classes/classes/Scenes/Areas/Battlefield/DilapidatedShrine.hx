...
class DilapidatedShrine extends BaseContent
{
    ...
    public function repeatvisitshrineintro() : Void
    {
        ...
        if (shinshoku())
        {
            outputText("As you reach the yard, you hear a voice again, \"<i>Oh, is that you again? I apologize for the dust...This place isn't the easiest to care for.</i>\" As you notice them, the old person that greeted you returns to sweeping the shrine like you aren't even here.\n\n");
        }
    ...
    private function shinshoku() : Bool
    {
    ...
    private function westSection() : Void
    {
        ...
        //outputText("\n\n");
        menu();
        if (player.hasItem(consumables.CHILLYP, 3))
        {
            addButton(0, "Offering", westSectionOffering);
        }
        else
        {
            addButtonDisabled(0, "???", "Perhaps if you came back with 3 Chilly peppers, like the voice said...");
        }
        addButton(14, "Back", shrinemainmenu);
    }
    ...
    public function weaponsXPtrader() : Void
    {
        ...
        var weaponClasses : Array<Dynamic> = [ItemConstants.WT_GAUNTLET, ItemConstants.WT_SWORD, ItemConstants.WT_AXE, ItemConstants.WT_MACE_HAMMER, ItemConstants.WT_DUELING, ItemConstants.WT_POLEARM, ItemConstants.WT_SPEAR, ItemConstants.WT_DAGGER, ItemConstants.WT_WHIP, ItemConstants.WT_EXOTIC, ItemConstants.WT_THROWING, ItemConstants.WT_BOW, ItemConstants.WT_CROSSBOW, ItemConstants.WT_2H_FIREARM, ItemConstants.WT_RIFLE, ItemConstants.WT_PISTOL, ItemConstants.WT_DUAL_FIREARMS, ItemConstants.WT_EXOTIC, ItemConstants.WT_RIBBON];  // "Scythe", "Staff", "Tetsubo", "Ribbon", "Tome"  
        var btnInt : Int = 0;
        var tempAry : Array<Dynamic> = [];
        ...
        for (i in 0...inventory.getMaxSlots())
        {
            var weaponType : String = "";
            var itemClassReplace : Dynamic;
            try
            {
                if (Std.is(player.itemSlot(i).itype, Weapon))
                {
                    itemClassReplace = try cast(player.itemSlot(i).itype, Weapon) catch(e:Dynamic) null;
                    weaponType = itemClassReplace.type;
                }
                else if (Std.is(player.itemSlot(i).itype, WeaponRange))
                {
                    itemClassReplace = try cast(player.itemSlot(i).itype, WeaponRange) catch(e:Dynamic) null;
                    weaponType = Reflect.field(itemClassReplace, "perk").split(",")[0];
                }
                //I don't know why I decided to go with German with this. But it's funny.
                else
                {
                    
                    trace("ACHTUNG, ARTIKEL IM STECKPLATZ " + i + " IST NICHT KOMPATIBEL!\n");
                }
                if (Lambda.indexOf(weaponClasses, weaponType) >= 0)
                {
        ...
        for (arr1 in tempAry)
        {
            addButton(btnInt, Reflect.field(arr1, Std.string(0)).shortName, conv2mastery, arr1);
            btnInt++;
        }
        addButton(14, "Back", southSection);
        
        
        var conv2mastery : Array<Dynamic>->Bool->Void = function(wType : Array<Dynamic>, conf : Bool = false) : Void
        {
            clearOutput();
            if (!conf)
            {
