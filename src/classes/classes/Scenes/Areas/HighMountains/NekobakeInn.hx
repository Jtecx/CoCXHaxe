...
class NekobakeInn extends BaseContent implements TimeAwareInterface
{
    ...
    private function transform() : Void
    {
        ...
        {
            transformations.SkinFur(Skin.COVERAGE_LOW, {
                                colors : ["brown", "chocolate", "auburn", "caramel", "orange", "sandy brown", "golden", "black", "midnight black", "dark gray", "gray", "light gray", "silver", "white", "orange and white", "brown and white", "black and white", "gray and white", "lilac and white"]
                            }).applyEffect(false);
        }
        if (player.hasGills())
        {
    ...
    private function male2() : Void
    {
        ...
        if (!player.isAnyRace(Races.NEKOMATA, Races.CAT, Races.SPHINX, Races.MANTICORE, Races.HELLCAT, Races.CHESHIRE))
        {
            transform();
        }
        var x : Int = 20;
    ...
    private function female2(fromSpring : Bool = false) : Void
    {
        ...
        {
            transformations.CreateBreastRow(BreastCup.D).applyEffect(false);
        }
        if (!player.isAnyRace(Races.NEKOMATA, Races.CAT, Races.SPHINX, Races.MANTICORE, Races.HELLCAT, Races.CHESHIRE))
        {
            transform();
