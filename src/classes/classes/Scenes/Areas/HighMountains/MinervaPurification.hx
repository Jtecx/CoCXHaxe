...
class MinervaPurification extends BaseContent
{
    ...
    private function playerParentDesc(caps : Bool = false) : String
    {
        ...
        switch (_sw0_)
        {
            case Gender.GENDER_NONE, Gender.GENDER_MALE, Gender.GENDER_FEMALE, Gender.GENDER_HERM:
                ...
                }  //A bit wonky. I should consider the maleherms.  
                if (caps)
                {
                    return "Mommy-Daddy";
                }
    ...
    public function purificationByMarae() : Void
    {
        ...
        if (flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] != 0)
        {
            outputText(" Her posture shifts slightly, thanks to her newly widened hips, and you think, but aren't sure, that the part of her bulge denoting her hefty pair of testicles grows slightly smaller.");
        }
        else
        {
            outputText(" Her posture shifts slightly, thanks to her newly widened hips, and you think, but aren't sure, that the part of her bulge denoting her hefty pair of testicles swells till they are the size of cantaloupes.");
        }
    ...
    public function purificationByJojoPart2() : Void
    {
        ...
        if (flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] > 0)
        {
            outputText(", trying to avoid looking too hard at either the harpy sluts or Minerva's still partially erect and dribbling cock");
        }
        ...
        if (flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] != 0)
        {
            outputText(" You think, but you can't be certain, that her balls may have shrunk, if only a little.");
        }
        else
        {
            outputText(" Looking at her, you can clearly see her balls have swollen up to the size of cantaloupes) Finally, and most startlingly, she has sprouted a second pair of wings, maybe half the size of the original ones, further down her back.");
        }
    ...
    public function purificationByRathazul() : Void
    {
        ...
        if (flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] > 0)
        {
            outputText("\"<i>I don't think I have very long before this... thing... inside me finishes what it's supposed to do. It's worth a shot,</i>\"");
        }
        else
        {
            outputText("\"<i>It can't possibly be any worse than what I'm already going through, trapped here forever,</i>\"");
        }
    ...
    public function declineToMakeBaby() : Void
    {
        ...
        if (player.gender == 1)
        {
            outputText("her warm hand naughtily reaches down and grabs the bulge in your [armor], before gently squeezing");
        }
        else if (player.gender == 2)
        {
            outputText("her warm hand reaches down and slips into your [armor] teasingly, her fingers exploring until they find your womanhood and rub it gently, the golden siren giving you a naughty sharky grin. Clearly she can't wait to start her family with you.");
        }
        else if (player.gender == 3)
        {
            outputText("her warm hand reaches down to grab the bulge in your [armor], squeezing your maleness teasingly. At the same time she guides your hand between her legs where you can clearly feel her surprisingly hot nethers pressing against your fingers.");
        }
        else
        {
            outputText("her warm hand reaches down and slips into your [armor] only to feel nothing. \"<i>Wait, you don't have anything to help either of us with having a child,</i>\" she says. She adds \"<i>Come back when you've got a gender, okay?</i>\"");
        }
    ...
    //0: Singing
    {
        ...
        var chooserArray : Array<Dynamic> = [];
        ...
        {
            chooserArray[chooserArray.length] = 3;
        }
        if (player.isPregnant() || minervaScene.pregnancy.isPregnant)
        {
            chooserArray[chooserArray.length] = 4;
        }
        if (player.cor >= 50 && flags[kFLAGS.MINERVA_PURE_TALKED_CORRUPTION] == null)
        {
            chooserArray = [5];
        }  //FORCE the corruption talk  
        //Select a scene!
        ...
        {
            outputText("Once again, you arrive at the tower and push open the door and enter the interior.");
        }
    ...
    private function minervaSinging() : Void
    {
        ...
        if (flags[kFLAGS.MINERVA_TOWER_TREE] > 0)
        {
            ...
            if (flags[kFLAGS.MINERVA_CHILDREN] >= 2)
            {
                outputText("As you look around you catch sight of " + num2Text(flags[kFLAGS.MINERVA_CHILDREN]) + " sirens ");
                var sceneChooser : Int = rand(3);
                if (sceneChooser == 0)
                {
                    outputText("perched in the tree.");
                }
                else if (sceneChooser == 1)
                {
                    outputText("splashing playfully in the pool.");
                }
                else
                {
                    outputText("curled up against the roots of the tree.");
                }
            }
    ...
    private function minervaBathing() : Void
    {
        ...
        {
            outputText(" before slipping down and running over her swollen pregnant tummy");
        }
        ...
        if (player.hasCock())
        {
            outputText("what a willing and fertile female she is and that you must do your duty as a seed bearing ");
            if (!player.hasVagina())
            {
                outputText("male");
            }
        ...
        if (player.hasVagina())
        {
            outputText("what a virile and pleasurable breeder this gorgeous herm is and that she would bring you otherworldly pleasures as she impregnates you so thoroughly.");
        }
    ...
    private function minervaRelaxing() : Void
    {
        ...
        if (flags[kFLAGS.MINERVA_CHILDREN] <= 0)
        {
            outputText("having some babies");
        }
        else
        {
            outputText("making more adorable daughters");
        }
    ...
    private function minervaPregnancyMeet() : Void
    {
        ...
        //Minerva is pregnant.{
            ...
            //Pregnant with sirens.{
                ...
                if (flags[kFLAGS.MINERVA_TOWER_TREE] > 0)
                {
                    outputText("against the huge beautiful oak tree that sits in the center of the spring, the beautiful hermaphrodite gently singing to herself and the tree as she lays against its shaded trunk.");
                }
                else
                {
                    outputText("against one of her sweet smelling fruit trees that surround the purifying spring. The beautiful hermaphrodite is gently singing to herself as she lies against its shaded trunk.");
                }
                ...
                if (flags[kFLAGS.MINERVA_TOWER_TREE] > 0)
                {
                    outputText("Stepping into the water you wade over to the huge oak tree and the childbearing broodmother, as you approach you see her face light up, her eyes being drawn to the obvious sign of your own pregnancy.");
                }
                else
                {
                    outputText("Without another thought you stride toward the childbearing broodmother, as you approach you see her face light up, her eyes being drawn to the obvious sign of your own pregnancy.");
                }
                ...
                if (flags[kFLAGS.MINERVA_CHILDREN] > 0)
                {
                    outputText("My virile lover, always so good at filling my belly with babies... ");
                }
                ...
                if (flags[kFLAGS.MINERVA_CHILDREN] > 0)
                {
                    outputText(", your daughters curled up with you and Minerva; they seem very happy, as if they all know that they are going to be getting more sisters soon");
                }
            ...
            //Not pregnant or pregnant with something else.
            else
            {
    ...
    private function minervaBabiesMeet() : Void
    {
        ...
        switch (temp)
        {
            ...
            case 1:  //Presents  
                ...
                {
                    outputText("Minerva holds her two daughters up on her shoulders, letting them use her like a ladder.");
                }
                else
                {
                    outputText("Minerva holds one of her daughters up on her shoulder, letting her use her parent like a ladder. The other daughters all work together, forming a \"<i>human</i>\" ladder of young sirens, each standing on another's shoulders as they pick the tasty sweet fruit from the trees.");
                }
    ...
    public function pureMinervaMenu() : Void
    {
        ...
        if (player.lust >= 33)
        {
            addButton(2, "Sex", pureMinervaSexMenu);
        }
        ...
        if (minervaScene.minervaRomanced() && model.time.hours >= 20)
        {
            addButton(6, "Sleep With", sleepWithMinerva);
        }
        //if (flags[kFLAGS.MINERVA_CHILDREN] > 0) addButton(7, "Children", checkUpOnMinervaChildren);
        if (minervaScene.pregnancy.isPregnant || player.pregnancyType == PregnancyStore.PREGNANCY_MINERVA || player.pregnancy2Type == PregnancyStore.PREGNANCY_MINERVA)
        {
            addButton(7, "Pregnancy", checkPregnancy);
        }
        if (player.hasKeyItem("Marae's Seed") >= 0)
        {
            addButton(8, "Plant Seed", growTreePostPurification);
        }
        if (debug)
        {
            addButton(10, "ROLLBACK", fixHer).hint("Rollback Minerva to pre-purified state.");
        }
    ...
    private function pureMinervaAppearance() : Void
    {
        ...
        {
            outputText("\n\n");
            if (minervaScene.pregnancy.incubation >= sceneHunter.adjustPregEventTimerNum(216, PregnancyStore.INCUBATION_MINERVA))
            {
                outputText("<b>Her belly appears to be a bit swollen.</b>");
            }
            else if (minervaScene.pregnancy.incubation >= sceneHunter.adjustPregEventTimerNum(144, PregnancyStore.INCUBATION_MINERVA) && minervaScene.pregnancy.incubation < sceneHunter.adjustPregEventTimerNum(216, PregnancyStore.INCUBATION_MINERVA))
            {
                outputText("<b>Her belly shows obvious signs of pregnancy.</b>");
            }
            else if (minervaScene.pregnancy.incubation >= sceneHunter.adjustPregEventTimerNum(72, PregnancyStore.INCUBATION_MINERVA) && minervaScene.pregnancy.incubation < sceneHunter.adjustPregEventTimerNum(144, PregnancyStore.INCUBATION_MINERVA))
            {
                outputText("<b>Her gravid belly has gotten bigger.</b>");
            }
            else if (minervaScene.pregnancy.incubation < sceneHunter.adjustPregEventTimerNum(72, PregnancyStore.INCUBATION_MINERVA))
            {
                outputText("<b>It's impossible to not notice her pregnancy. She is about to give birth soon.</b>");
            }
        }
    ...
    public function talkingToMinervaAboutBackstory() : Void
    {
        ...
        if (flags[kFLAGS.MINERVA_BACKSTORY] == 0)
        {
            ...
            outputText("</i>\" \n\nSaying goodbye to the beautiful golden siren you kiss her goodbye before heading back to camp.")  // PC returns to camp.  ;
        }
        //PART 2 - DONE
        ...
        else
        {
            ...
            if (flags[kFLAGS.MINERVA_CHILDREN] > 0)
            {
                outputText(". Well... our babies are just as important you understand");
            }
    ...
    public function talkToMinervaAboutSpring() : Void
    {
        ...
        if (flags[kFLAGS.MINERVA_TOWER_TREE] > 0)
        {
            outputText("Marae told me that its some kind of old nexus of pure energies, like a bright lantern glowing in a pitch black room. ");
        }
        else
        {
            outputText("Jojo told me what he knew about it, He said it's some kind of wellspring of holy power, one of the few untainted places left in this world. ");
        }
    ...
    public function talkToMinervaAboutSharkgirl() : Void
    {
        ...
        if (flags[kFLAGS.MINERVA_CHILDREN] > 0)
        {
            outputText("by how our own sweet little daughters grow up");
        }
    ...
    public function talkToMinervaAboutMotherhood() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(" And so you begin weaving a story of how you fought six minotaurs at the same time, each of them twelve feet tall and carrying huge axes. It was a hard fight, most of your clothes were shredded from the fierce battle, your body glistening with sweat as you stood over your fallen beastial foes.");
        ...
        if (flags[kFLAGS.MINERVA_CHILDREN] <= 0)
        {
            outputText("I do wish that one day I could have kids like that, Have a great big loving family to call my own.</i>\" The siren says before sighing wistfully.");
        }
        else
        {
            outputText("But I no longer envy them now that I have a great big loving family to call my own, thanks to you.</i>\" she says, sweetly.");
        }
        outputText("\n\nShaking your head you grin and shuffle over to Minerva, sitting down with her, you ");
        if (minervaScene.minervaRomanced())
        {
            outputText("hug her tightly and kiss her cheek, telling her that she will have children one day, that you will make sure of it.");
        }
        else
        {
            outputText("You place a hand on her shoulder and give her a comforting smile before telling her that she will one day, you're sure of it.");
        }
    ...
    public function talkToMinervaAboutTree() : Void
    {
        ...
        if (flags[kFLAGS.MINERVA_CHILDREN] > 0)
        {
            outputText("as well as tend to her your daughters");
        }
    ...
    private function sleepWithMinerva() : Void
    {
        ...
        if (flags[kFLAGS.MINERVA_TOWER_TREE] > 0)
        {
            outputText("Minerva guides you to the most comfortable spot under the huge oak tree. Being under the tree gives you a great sense of shelter. She takes off her tube top, exposing her breasts completely.\n\n");
        }
        else
        {
        ...
        {
            outputText("You take your time to get your [armor] off until you're completely naked. ");
        }
        outputText("You lay next to Minerva while you rest your head on her soft breasts. ");
        if (player.armor == armors.GOOARMR)
        {
            outputText("Valeria, your goo-girl companion, envelops you and Minerva, helping to keep you and Minerva warm. ");
        }
    ...
    private function sleepWithMinervaII() : Void
    {
        ...
        {
            outputText("You get re-dressed in your [armor] ");
        }
    ...
    private function minervaACapacity() : Float
    {
        return minervaScene.minervaACapacity();
    }
    
    private function minervaVCapacity() : Float
    {
    ...
    //  SEX SCENES!
    //------------------
    private function pureMinervaSexMenu(clear : Bool = true) : Void
    {
        if (clear)
        {
            clearOutput();
        ...
        if (player.hasVagina())
        {
            outputText("\"<i>Want me to stuff that hot little cunt of yours? Or maybe I should prod your delightful rump?</i>\"");
        }
    ...
    private function fuckMinervasAsshole() : Void
    {
        ...
        if (player.cumQ() >= 1000)
        {
            outputText(" Your unnatural orgasm keeps pouring into her guts, flooding her interior with sticky cum. By the time you give out with a gasp of effort, her stomach is visibly bulging, swollen from the amount of cum you have filled her with. \"<i>Ooohhh... fuck... so much... It looks like I have a baby in there,</i>\" Minerva says with a long, drawn out groan, unused to the feeling of having her guts filled up by such hot, thick stuff.");
        }
        //Very High cum:
        if (player.cumQ() >= 5000)
        {
    ...
    private function minervaCowgirlSex() : Void
    {
        ...
        {
            outputText(" with some trepidation, given the difference in your sizes,");
        }
        ...
        {
            outputText("Minerva looks unsure, the siren biting her lip and looking a little away from you, a burning blush staining her creamy cheeks. \"<i>Well... We waited so long already, you have done so much for me, saved my life and gave me a real future. Yes, lets do it, I need this, I need you!</i>\"");
        }
        else
        {
            outputText("Minerva smiles brightly and reaches around you, grabbing your own [ass] and pulling you against her body. \"<i>Oh how could I ever say no to you my love. Brace yourself darling, I am going to ride you into the ground!</i>\" she proclaims excitedly before giving your crotch a playful squeeze.");
        }
        ...
        if (minervaScene.pregnancy.type == PregnancyStore.PREGNANCY_PLAYER)
        {
            outputText(" I want to feel you fill me up so bad, show our little daughters how much you love their mommy!");
        }
        ...
        if (minervaScene.pregnancy.type == PregnancyStore.PREGNANCY_PLAYER)
        {
            outputText(" You really are trying to get me pregnant all over again, aren't you? My little babies are all swimming in their daddy's sperm; how very naughty!</i>\"");
        }
        //she's ALWAYS ready
        ...
        if (player.hasVagina())
        {
            outputText("  A few even slither down into your [vagina] to sting you there as well, making sure you are, at least temporarily, turned into a sex-hungry beast.");
        }
        ...
        if (minervaScene.pregnancy.type == PregnancyStore.PREGNANCY_PLAYER)
        {
            outputText("Her massive childbearing tummy pressing against your body gently as you snuggle.");
        }
        ...
        if (flags[kFLAGS.MINERVA_CHILDREN] > 0)
        {
            outputText("I know the girls would love to see you again. ");
        }
    ...
    private function fuckMinervaWithHerHandsBehindHerBack() : Void
    {
        ...
        if (player.hasBalls())
        {
            outputText(" As you continue to pound the siren, your [balls] swing between your legs, the sperm-filled orbs churning with your fertility, preparing to fill the needy siren with every drop you can muster when you let loose your eventual orgasm.");
        }
        ...
        else
        {
            ...
            if (player.hasBalls())
            {
                outputText(" Minerva groans and lets out mindless gurgles as you unload your overproductive balls into her.");
            }
    ...
    private function minervaLapSex() : Void
    {
        ...
        if (player.gender == 3)
        {
            outputText(" One of her hands trail down to your hard herm " + cockDescript(x) + ", stroking you softly.");
        }
        ...
        {
            outputText(" The thick siren prick plunges into your soaked cunt and fills your sweltering hole with her oddly cool cock, both of you groaning in delightful pleasure.");
        }
        //Virgin:
        else
        {
            outputText(" The thick siren prick plunges into your soaked cunt, filling your sweltering pussy with her oddly cool cock, in the process tearing right through your hymen, claiming your first fuck for herself as you bite back a pained yelp, though it is quickly displaced by the pleasure from the stimulation and venom.");
        }
        ...
        if (player.hasCock())
        {
            ...
            if (player.cumQ() < 500)
            {
                outputText(" gushing your cum all over both of your bellies");
            }
            //High cum volume:
            else if (player.cumQ() < 4000)
            {
                outputText(" splattering thick splooge all over you and Minerva, caking your torsos in fertile jizz");
            }
            //Very high cum:
            else
            {
                outputText(" exploding out onto your unsuspecting bodies, thick ropes of starkly white sperm gushing over your bodies, quickly dousing both of you in buckets of cum and pooling on the ground");
            }
        }
        outputText(".");
    ...
    private function getButtFuckedYouSlut() : Void
    {
        ...
        if (player.gender == 3)
        {
            outputText(" One of her hands trail down to your hard herm " + cockDescript(x) + " and " + vaginaDescript() + ", stroking and fingering you softly.");
        }
        ...
        if (player.hasVagina())
        {
            ...
            {
                outputText("leaking femspunk");
            }
            else
            {
                outputText("shooting femspunk");
            }
        }
    ...
    private function goDownOnAHermAndLoveItYouDirtySlutYou() : Void
    {
        ...
        {
            outputText("your fingers not even able to fit around the herm's hefty girth");
        }
        else
        {
            outputText("your fingers barely wrapping around the meaty member, the fat thing throbbing gently");
        }
        ...
        {
            outputText("Your deliciously long tongue slithers deeper and deeper, reaching sensitive walls and nerves that only the deepest penetrations would reach. The deep licking makes Minerva shudder and let out a long drawn out moan, \"<i>O-oh! Your tongue is so... licking me so deep...</i>\"");
        }
    ...
    private function noBallsMinervaAndContinuation(clear : Bool = true) : Void
    {
        if (clear)
        {
            clearOutput();
        ...
        if (!clear)
        {
            outputText(" Retreating from her throbbing balls, you");
        }
        else
        {
            outputText(" You");
        ...
        {
            outputText(", your perversely prehensile tongue slowly retracting from her quivering quim, rubbing and teasing as it comes out and nearly sending the overly sensitive herm into another climax");
        }
        outputText(".");
    ...
    private function letMinervaSuckYouOff() : Void
    {
        ...
        if (player.isNaga())
        {
            outputText(" lounge against the rock, spreading open your protective slit and letting your " + cockDescript(x) + " hang free.");
        }
        else
        {
            outputText(" sit down, spreading your [legs] and opening up the crotch of your [armor] to the air, letting your " + cockDescript(x) + " hang free.");
        }
        ...
        if (player.hasBalls())
        {
            outputText(" Her hand on your hip moves down between your legs, sliding inwards to gently caress and massage your cum-filled [balls] to increase your pleasure even more, her hands trying their best to get your nuts worked up and prepare a good load for her at the end.");
        }
        ...
        if (player.hasBalls())
        {
            outputText(" before licking and teasing your [balls]");
        }
        outputText(".");
        ...
        {
            outputText(", your huge cock deep inside her mouth, and soon far down her throat. With some difficulty, her slightly angular nose presses down against your crotch, every inch of your " + cockDescript(x) + " trapped inside her cool, wet mouth and throat");
        }
        outputText(".");
        ...
        if (player.hasBalls())
        {
            outputText(", while your [balls] lurch and swell, rising up as you reach your climactic explosion");
        }
        outputText(".");
    ...
    public function checkPregnancy() : Void
    {
        ...
        {
            if (minervaScene.pregnancy.incubation >= sceneHunter.adjustPregEventTimerNum(168, PregnancyStore.INCUBATION_MINERVA))
            {
                pregnancyStage1(flags[kFLAGS.MINERVA_CHILDREN] > 0);
            }
            else if (minervaScene.pregnancy.incubation >= sceneHunter.adjustPregEventTimerNum(144, PregnancyStore.INCUBATION_MINERVA))
            {
                pregnancyStage2(flags[kFLAGS.MINERVA_CHILDREN] > 0);
            }
            else if (minervaScene.pregnancy.incubation >= sceneHunter.adjustPregEventTimerNum(72, PregnancyStore.INCUBATION_MINERVA))
            {
                pregnancyStage3(flags[kFLAGS.MINERVA_CHILDREN] > 0);
            }
            else
            {
                pregnancyStage4(flags[kFLAGS.MINERVA_CHILDREN] > 0);
            }
        }
        else if (player.pregnancyIncubation >= sceneHunter.adjustPregEventTimer(108, player.pregnancyType))
        {
            pregnancyPlayerStage1();
        }
        else
        {
            pregnancyPlayerStage2();
        }
    }
    
    //If you've impregnated Minerva.
    public function pregnancyStage1(repeat : Bool = false) : Void
    {
        clearOutput();
    ...
    public function pregnancyStage2(repeat : Bool = false) : Void
    {
        clearOutput();
        if (!repeat)
        {
    ...
    public function pregnancyStage3(repeat : Bool = false) : Void
    {
        clearOutput();
        if (!repeat || !player.hasCock())
        
        //because another scene depends on it...{
            ...
            if (flags[kFLAGS.MINERVA_TOWER_TREE] > 0)
            {
                outputText("the huge oak tree that dominates the spring, sitting her down with her back against the trunk of the tree.");
            }
            else
            {
                outputText("one of the ever-growing fruit trees, laying her down against the trunk of the fruit bearing trees.");
            }
        ...
        else
        {
            ...
            if (player.hasBalls())
            {
                outputText(" Thankfully your aching swollen nuts shrink with each geyser like gush you let loose from your massively swollen prick, though if anything it only seems to mitigate the effects of all the siren venom inside you not stop it and soon your balls will merely refill.");
            }
    ...
    private function pregnancyStage4(repeat : Bool = false) : Void
    {
        clearOutput();
    ...
    public function minervaGivesBirth() : Void
    {
        ...
        if (flags[kFLAGS.MINERVA_CHILDREN] <= 0)
        {
            outputText("\"<i>I feel like a mother again, and I want to thank you for helping me. You've fulfilled my wishes,</i>\" she says gratefully with a smile. ");
        }
    ...
    public function playerGivesBirth(womb : Int = 0) : Void
    {
        ...
        {
            outputText("<b>No scene for corrupted Minerva yet, so just take this one for now.</b>");
        }
        ...
        if (flags[kFLAGS.MINERVA_CHILDREN] > 0)
        {
            outputText(". My daughters are already excited to see their new addition to our family");
        }
        ...
        {
            flags[kFLAGS.MINERVA_CHILDREN] += 4;
        }
        else
        {
            flags[kFLAGS.MINERVA_CHILDREN] += 2;
