...
class Harpy extends Monster
{
    ...
    private function harpyUberCharge() : Void
    //(Harpy special attack 1, part one)
    {
        ...
        else
        {
            if (!Combat.playerWaitsOrDefends())
            {
                ...
                var damage : Float = (str + weaponAttack + 320 + rand(20)) * (1 + (player.newGamePlusMod() / 2));
    ...
    private function harpyTease() : Void
    {
    ...
    public function new(noInit : Bool = false)
    {
        ...
        this.drop = new ChainedDrop().add(armors.W_ROBES, 1 / 10).add(consumables.SKYSEED, 1 / 5).elseDrop(consumables.GLDSEED);
