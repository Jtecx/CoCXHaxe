...
/**
	 ...
	 * @programmer Gedan
	 */
class IzumiScene extends BaseContent
{
    ...
    public function heightDesc(doName : Bool = true) : String
    {
        if (rand(4) != 0)
        {
            return "kid";
    ...
    private function abortabortleavethefukkencave() : Void
    {
        clearOutput();
    ...
    private function nopeLeavePlz() : Void
    {
        clearOutput();
    ...
    // Long: Untested
    // Short: Tested
    private function enterFirstTime() : Void
    {
        clearOutput();
        ...
        // High strength variant{
            ...
            if (player.horns.type == Horns.COW_MINOTAUR)
            {
                ...
                {
                    outputText(" . Or at least, ones without the temperament to match the... physique, if you follow?</i>\"");
                }
            }
            else
            {
        ...
        if (player.tou <= 75)
        {
            outputText(" and of course, that obnoxiously endurance-sapping heat");
        }
        outputText(".\n\n");
        ...
        if (player.tallness < 108)
        {
            ...
            if (player.racialScore(Races.MINOTAUR, false) > 3)
            {
                outputText("plus it looks like you're thinking with the head on top of your neck, rather than the head between your legs, unlike the stupid brutes who usually wander around these parts.  ");
            }
            else if (player.isRace(Races.DOG, 1, false))
            {
                outputText("plus it looks like you'd actually be able to resist chasing down a stick, if I threw one outta' the cave.  ");
            }
            else if (player.isGoblinoid())
            {
                outputText("plus you don't seem to be as insufferable as the rest of those little green whores trolloping around these parts, hunting their next lay.  ");
            }
            else if (player.isRace(Races.DEMON, 1, false) || player.isRace(Races.IMP, 1, false))
            {
                outputText("plus you're not forever going on about corruption this, enslavement that, demonic taint the other, unlike the insufferable clods who usually wander around these parts.  ");
            }
            else if (player.isRace(Races.HARPY, 1, false))
            {
                outputText("plus you're not screeching incessantly, like the feathery sluts who usually flap their way around these parts.  They're lucky they </i>can<i> fly, 'cause I'd give them something to really squawk about if I could get my hands on them.  ");
            }
            else if (player.isRace(Races.LIZARD, 1, false))
            {
                outputText("plus, you don't have the same creepy eyes that the scaley idiots from around these parts keep trying to use on me.  ");
            }
            else
            {
                outputText("plus you don’t look like the troglodytes who usually wander around these parts.  ");
            }
        ...
        if (player.tou < 60)
        {
            outputText(" causes you to cough and splutter.\n\n");
        }
        else
        {
            outputText(" momentarily steals your voice, having been ill-prepared for the sudden burn.\n\n");
        }
    ...
    private function enterFirstTimePartII() : Void
    {
        clearOutput();
        ...
        if (player.tou < 60)
        {
            outputText(" struggling with");
        }
        else
        {
            outputText(" distracted by");
        }
        outputText(" the smoke, you turn to see her, only barely through the gloom, at the boulder blocking entry to the cave.");
        if (player.str <= 75)
        {
            outputText("  To your amazement, the boulder begins to move, and as more light - along with a much-needed rush of fresh air - floods into the smoky cavern, you see she’s actually rolling it aside with her <i> bare hands </i>.  Even given her great size, this woman’s strength must be immense!\n\n");
        }
        else
        {
            outputText("  The boulder begins to move, and as more light - along with a welcome rush of fresh air - floods into the smoky cavern, you see she’s rolling it aside with nothing more than her bare hands with graceful, if unnerving, ease.\n\n");
        }
        ...
        if (player.cor <= 33)
        {
            outputText(" exposing an uncomfortable amount of creamy-white thigh flesh.");
        }
        else if (player.cor > 33 && player.cor < 66)
        {
            outputText(" exposing a tantalising view of creamy-white thigh flesh.");
        }
        else
        {
            outputText(" exposing a delicious view of her thigh, the creamy-white flesh almost screaming out for your caresses.");
        }
        ...
        if (player.tallness <= 96)
        {
            outputText(" - if a little... imposing -");
        }
        else
        {
            outputText(",");
        ...
        if (player.str <= 75 || player.tallness < 96)
        {
            outputText(" You can’t help but feel like a sheep yourself - one who’s just noticed the wolf is staring it down.\n\n");
        }
    ...
    private function smokeEffect(smokedPipe : Bool) : Void
    {
        var SMOKE_SPEED_REDUCE : Float = 1.1;
        var SMOKE_SENS_BOOST : Float = 1.1;
        var SMOKE_LIBIDO_BOOST : Float = 1.1;
        var deltaSpd : Int = as3hx.Compat.parseInt(player.spe - (player.spe * SMOKE_SPEED_REDUCE));
        var deltaSns : Int = as3hx.Compat.parseInt((player.sens * SMOKE_SENS_BOOST) - player.sens);
        var deltaLib : Int = as3hx.Compat.parseInt((player.lib * SMOKE_LIBIDO_BOOST) - player.lib);
        var lustMod : Int = 7;
    ...
    private function smokeThePipe() : Void
    {
        clearOutput();
        ...
        if (player.tou <= 60)
        {
            outputText(" you cough and splutter");
        }
        else if (player.tou < 75)
        {
            outputText(" you flinch, holding back a cough");
        }
        else
        {
            outputText(" your eyes water");
        }
        ...
        if (player.tou <= 80)
        {
            outputText("  After a while, you find it hard to even raise your arms to reach for the pipe again.\n\n");
        }
        else
        {
            outputText("\n\n");
    ...
    private function dontSmokeThePipe() : Void
    {
        clearOutput();
    ...
    private function smokeJoins() : Void
    {
        ...
        if (player.str <= 75 || player.tou <= 75 || player.tallness <= 96)
        {
            outputText("  You swallow, hard, suddenly feeling oddly nervous.");
        }
    ...
    private function fuckinChegIt() : Void
    {
        hideUpDown();
    ...
    /**
		 * FIGHT SHIT
		 */
    private function fightTheFuckhugeOni() : Void
    {
        hideUpDown();
        ...
        if (flags[kFLAGS.IZUMI_LAST_ENCOUNTER] == 0)
        {
            ...
            if (player.tone <= 75)
            {
                outputText("\"<i>Alright!  I’ll tell you what I’ll do... I’ll give myself a handicap.</i>\"  Izumi turns to face you, cracks her neck, then extends a hand, folding the other arm behind her back.  \"<i>I’ll only use one arm.  I wouldn’t want to put you in your place </i>too<i> quickly.  Where would be the fun in that?</i>\"  She leers.\n\n");
            }
            ...
            if (player.tone <= 75)
            {
                outputText("The idea that she thinks she can beat you with one hand behind her back is insulting to say the least, but on the other hand, her overconfidence could be her downfall.\n\n");
            }
    ...
    /**
		 * SURRENDER SCENES
		 */
    private function izumiSurrender() : Void
    {
        ...
        {
            sceneHunter.selectGender(izumiSurrenderMale, izumiSurrenderFemale);
        }
    }
    
    private function izumiSurrenderMale() : Void
    {
        ...
        if (player.isTaur())
        {
            outputText(" flank and forcing you sideways onto the ground.");
        }
        else
        {
            outputText(" chest and forcing you backwards onto the ground.");
        }
    ...
    private function surrenderSmallCock() : Void
    {
        var x1 : Int = as3hx.Compat.parseInt(player.findCock(-1, -1, 6, "length") + 1);  //smallest lol  
        ...
        if (player.isTaur())
        {
            outputText(" perches her muscular ass on your flank, your body making for a suitable seat.");
        }
        else
        {
            outputText(" drops to the floor, laying next to you on her side.");
        }
        ...
        if (player.biggestTitSize() >= 1 && player.biggestTitSize() < 3)
        {
            outputText(", your [chest] dwarfed by the larger womans own pair.");
        }
        else if (player.biggestTitSize() >= 3 && player.biggestTitSize() < 5)
        {
            outputText(", your [chest] moulding themselves against the underside of the Oni’s larger pair.");
        }
        else if (player.biggestTitSize() >= 5 && player.biggestTitSize() <= 8)
        {
            outputText(", your [chest] compressing against the Oni’s larger pair, forming a delicious valley of flesh between your bodies.");
        }
        else
        {
            outputText(", your [chest] dwarfing the larger womans own pair.");
        }
        ...
        if (player.str <= 75)
        {
            outputText(" and you would be powerless to resist...");
        }
        else
        {
            outputText(" and you would struggle to deny her will...");
        }
        ...
        if (player.hasVagina())
        {
            outputText(", sticky moisture dribbling across your thigh, your [vagina] clenching in lust - fueled harmony with your cock");
        }
        ...
        if (player.hasBalls())
        {
            outputText(" and her free fingers reach out to gently stroke against your [balls].");
        }
        else if (player.hasVagina())
        {
            outputText(" and her free fingers reach out to gently stroke against your [vag].");
        }
        else
        {
            outputText(".");
        ...
        else
        {
            if (!player.isTaur())
            {
                outputText("As you squirm in her arms, she releases your head from her grip, instead pushing you up into a seated position.");
            }
            ...
            if (!player.isTaur())
            {
                outputText(" as one arm reaches around to continue the handjob from the other side");
            }
            outputText(".  She stares intently at your cock, an expectant smirk plastered over her face, her free hand working");
            if (player.isTaur())
            {
                outputText(" over your flank");
            }
            else
            {
                outputText(" you over with increased vigour");
            }
            ...
            if (player.cumQ() <= 100)
            {
                outputText(" dribbles a hot, sticky mess onto the cold cavern floor.");
            }
            else if (player.cumQ() > 100 && player.cumQ() <= 250)
            {
                outputText(" spurts and splutters a hot, sticky mess onto the cold cavern floor.");
            }
            else if (player.cumQ() > 250)
            {
                ...
                if (player.cumQ() > 500)
                {
                    ...
                    if (player.cumQ() > 1000)
                    {
                        outputText(", slowly expanding into a small lake....\n\n");
                    }
                }
                else
                {
    ...
    private function surrenderMediumCock() : Void
    {
        var x1 : Int = as3hx.Compat.parseInt(player.findCock(1, 6, 14, "length") + 1);  //medium  
        ...
        if (player.hasVagina())
        {
            ...
            if (player.wetness() == 0)
            {
                outputText(" moistening");
            }
            else if (player.wetness() == 1)
            {
                outputText(" dribbling runnels of sticky fem-juice");
            }
            else if (player.wetness() == 2)
            {
                outputText(" constantly oozing feminine arousal");
            }
            else
            {
                outputText(" a soppy mess, gushing a veritable lake of fem-slime");
            }
        ...
        if (player.isTaur())
        {
            outputText(" perching her muscular ass on your flank, your lower body offering a comfortable seat for Izumi.");
        }
        else
        {
            outputText(" laying on her side next to you.");
        }
    ...
    private function surrenderMediumCockObey(x1 : Int) : Void
    {
        ...
        {
            outputText("  Lowering your shaking hips, y");
        }
        ...
        if (player.hasVagina())
        {
            outputText(" mixed puddle of your own fem-slime and");
        }
        else
        {
            outputText(" puddle of your own");
        }
    ...
    private function surrenderMediumCockRefuse(x1 : Int) : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText(", [vagina] spasming in sympathy");
        }
        ...
        if (player.cumQ() <= 250)
        {
            outputText(" spurting free, spattering down randomly over you as Izumi watches, happily observing the effects of your ruined orgasm.");
        }
        else if (player.cumQ() <= 500)
        {
            outputText(" sputtering freely from your [cock " + x1 + "], glob after glob of thick jizz showering down randomly over you as Izumi watches, mildly amused with the results of your ruined orgasm.");
        }
        else if (player.cumQ() <= 1000)
        {
            outputText(" a torrent of fertile spunk rushes forth from your [cock " + x1 + "], glob after glob of thick jizz showering down randomly over you, covering you from head to toe.  Izumi simply watches, amused with the results of your ruined orgasm.");
        }
        else
        {
            outputText("a torrent of fertile spunk rushes forth from your [cock " + x1 + "], rope after rope of the thick cum launching into the air and splattering down over you.  Your orgasm continues unabated as you slowly paint yourself in a creamy white layer, Izumi giggling quietly, evidently quite proud of herself.");
        }
    ...
    // Urtadicks itt
    private function surrenderLargeCock() : Void
    {
        var x1 : Int = as3hx.Compat.parseInt(player.findCock(1, 14, -1, "length") + 1);
        ...
        if (player.isTaur())
        {
            outputText(" back on your haunches, front end raised");
        }
        ...
        {
            outputText(", occasionally sliding down to massage your [balls] before returning to your cock.");
        }
        else if (player.balls == 0 && player.hasVagina())
        {
            outputText(", occasionally sliding down to tease at the lips of your [vagina].");
        }
        else if (player.hasBalls() && player.hasVagina())
        {
            outputText(", occasionally sliding down, alternating between a gentle massage of your [balls] and teasing caresses of your [vagina].");
        }
        outputText("\n\n");
        ...
        if (player.hasBalls())
        {
            outputText("  You groan, your aching balls coming to the same conclusion as you have, twitching under a passing stroke from one of Izumi’s soles and tensing up in readiness.");
        }
        ...
        if (player.cumQ() > 500)
        {
            outputText("You remain sat there for a crippling eternity, unable to do anything but moan as you inexplicably continue to cum, your aching erection firing out shot after shot of hot, dripping goo as Izumi watches you from the safety of the shadows, grinning happily like the cat that got the cream.  ");
        }
    ...
    private function izumiSurrenderFemale() : Void
    {
        ...
        if (flags[kFLAGS.IZUMI_TIMES_SUBMITTED] == 1)
        {
            ...
            if (player.cor <= 33)
            {
                outputText("  You can’t help but feel a crippling sense of embarrassment as the strange woman undresses you, and you look away, embarrassed, while Izumi’s hands explore your body.");
            }
            else if (player.cor >= 66)
            {
                outputText("  As Izumi slowly reveals your naked body to the world, you begin to feel an oh-so-familiar heat building up inside of you.  You look down at Izumi with pride as she inspects your body, running a skillful hand over your chest with a thoughtful look on her face.");
            }
            outputText("\n\n");
        ...
        if (flags[kFLAGS.PC_FETISH] >= 1)
        {
            ...
            if (player.hasCock())
            {
                outputText(", your [cocks] slowly thickening under her scrutiny");
            }
            outputText(".  You shift in place,");
            if (player.isNaga())
            {
                outputText(" wriggling atop your [legs]");
            }
            else
            {
                outputText(" subtly parting your [legs] and even ");
            }
            if (player.isTaur())
            {
                outputText(" rolling your butt upward");
            }
            else
            {
                outputText(" tilting your hips forwards");
            }
            ...
            if (!player.isNaga())
            {
                outputText("You rub your thighs together in delight");
            }
            else
            {
                outputText("You wriggle and writhe in delight");
            }
            ...
            if (player.isNaga())
            {
                outputText(" deep within your [vagina].  You slither around in place, your [leg] shifting and weaving around, moving and undulating... b");
            }
            else
            {
                outputText(" between your [legs] as well.  As you move your weight repeatedly from one [foot] to the other, you shiver in enjoyment.  B");
            }
        ...
        {
            sceneHunter.print("Check failed: Exhibitionist fetish.");
        }
        ...
        if (!player.isTaur())
        {
            ...
            if (!player.isNaga())
            {
                outputText("  Before you know it, she has you down on your knees, the cold stone of the floor digging into your [skinfurscales] a little as she circles behind you, smirking cruelly.");
            }
            else
            {
                outputText("  She takes hold of your shoulders and forces you lower to the ground, forced to slither more and more of your tail out from under yourself, until Izumi towers over you.  She circles behind you, a cruel smirk the only hint of her intentions.");
            }
            outputText("\n\n");
    ...
    private function surrenderFemaleNonExhibitionVariant() : Void
    {
        clearOutput();
        ...
        if (player.isTaur())
        {
            outputText(" sitting upright on your back and");
        }
        ...
        if (player.biggestTitSize() <= 2)
        {
            ...
            if (!player.isTaur())
            {
                outputText(", guiding your hand down to your own nethers, which you’re shocked - in a foggy, distant kind of way - to discover are sopping wet");
            }
            outputText(".\n\n");
        ...
        else
        {
            
            {
                ...
                if (player.isNaga())
                {
                    outputText(" over your [hip].");
                }
                else if (player.isDrider())
                {
                    outputText(" between your spindly legs, slapping them lightly to force you to spread them wider.");
                }
                else
                {
                    outputText(" between your thighs, slapping them lightly to force you to open them wider.");
                }
    ...
    private function surrenderFemaleExhibitionVariant() : Void
    {
        clearOutput();
        ...
        if (player.isBiped())
        {
            outputText(", you spread your legs a little wider,");
        }
        outputText(" you");
        ...
        addButton(1, "No", surrenderFemaleNonExhibitionVariant);
    }
    
    // Female surrender, Exhibitionist variant scene body
    private function surrenderFemaleLookitYouTheCenterOfAttentionYouSlut() : Void
    {
    ...
    /**
		 * LOSS SCENES
		 */
    public function fuckedUpByAFuckhugeOni(titLoss : Bool = false) : Void
    {
        flags[kFLAGS.IZUMI_LAST_ENCOUNTER] = 1;
        flags[kFLAGS.IZUMI_SEEN_PC_GENDER] = player.gender;
        ...
        {
            outputText("  Despite how much the fight with the enormous Oni has turned you on, you’re still hesitant to capitulate to her will entirely.");
        }
        ...
        if (player.lust >= 75)
        {
            outputText(" the idea of sex with Izumi <i>is</i> quite a tantalizing prospect.  F");
        }
        else
        {
            outputText(" f");
        ...
        if (player.lust >= 75)
        {
            outputText("  Despite yourself, you have to wonder exactly what it would feel like to slide between those giant sweat-slicked thighs, bury your face into Izumi’s mountainous bust, and just let your libido run wild.");
        }
        outputText("\n\n");
    ...
    private function lossSceneLeave() : Void
    {
        clearOutput();
    ...
    private function lossSceneStay() : Void
    {
        clearOutput();
        ...
        if (player.hasCock())
        {
            outputText("\"<i>Or maybe,</i>\"  She whispers seductively into your ear, \"<i>You wanna bury that little Oni of yours in </i>me<i>.  Hmmmm?</i>\"  You shudder as you feel her hand snake into your underwear and run sinuously across your crotch, readying you for the ordeal ahead...\n\n");
        }
    }
    ...
    public function deathBySnuSnuIMeanGiantOniTits() : Void
    {
        ...
        {
            sceneHunter.print("Check failed: short PC with small (5\") dicks... Yes, you guessed right. Billy Bones died from cringe.");
        }
    ...
    private function cowedIntoPuttingItInIntoHerGiantOniButt() : Void
    {
        lossSceneStay();
    ...
    private function fuckhugeOniWantsYourBabiesOrSomeShit() : Void
    {
        lossSceneStay();
    ...
    private function noDickNoVagNoService() : Void
    {
        lossSceneStay();
        ...
        if (!player.isNaga())
        {
            outputText("before grabbing onto your other ankle, forcibly spreading your legs apart and leaving you creating a very undignified Y-shape");
        }
        outputText(".\n\n");
    ...
    private function littleChampLittleChampFuckhugeOniIsCominTaEatcha() : Void
    {
        lossSceneStay();
        ...
        if (!player.isNaga())
        {
            outputText(" grabbing onto your other ankle, forcibly spreading your legs apart and leaving you creating a very undignified Y-shape");
        }
        outputText(".\n\n");
    ...
    public function touchThatFluffyHorn() : Void
    {
        ...
        {
            if (flags[kFLAGS.IZUMI_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.IZUMI_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.IZUMI_DEFEATS_COUNTER] = 1;
            }
        ...
        {
            outputText(" the brief reprieve giving both of you a moment to recover.\n\n");
        }
        else if (monster.lust >= monster.maxOverLust())
        {
            outputText(" the Oni’s cheeks tinged a rosy peach in contrast to her alabaster skin.\n\n");
        }
        else
        {
            outputText(" momentarily staggered by your furious onslaught.\n\n");
        }
        ...
        else if (player.lib >= 40 && (player.biggestTitSize() >= 2 || player.biggestCockLength() >= 5))
        {
            ...
            if (player.biggestTitSize() > 2)
            {
                outputText("maybe giving her face a little personal time nestled between your [chest] will finally distract the Oni enough.  You reach out for the bright red curve of Izumi’s horns, ");
            }
            else
            {
                outputText("thinking quickly, you slip open your [armor] with one hand, bearing your [cock] to the world, your other hand reaching for the bright red curve of Izumi’s horns, ");
            }
        ...
        {
            outputText("\"<i>C-Could you please let go of that?</i>\"");
        }
    ...
    // Pfft, nofux
    private function letGoAndLeaveYouSillyFuck() : Void
    {
        clearOutput();
    ...
    // Kinda friendly fux
    private function letGoAndFuck() : Void
    {
        clearOutput();
        ...
        if (player.biggestTitSize() >= 1 && player.biggestTitSize() < 3)
        {
            outputText(" smothering your [chest] with her own, much larger, pair.\n\n");
        }
        else if (player.biggestTitSize() >= 3 && player.biggestTitSize() < 5)
        {
            outputText(" your [chest] moulding themselves against the Oni’s larger pair.\n\n");
        }
        else if (player.biggestTitSize() >= 5 && player.biggestTitSize() <= 8)
        {
            outputText(" your [chest] compressing against the Oni’s larger pair, forming a delicious valley of flesh.\n\n");
        }
        else if (player.biggestTitSize() > 8)
        {
            outputText(" your [chest] dominating the Oni’s own in the battle for space between the two of you.\n\n");
        }
        else
        {
            outputText(" pressing you into her chest.\n\n");
        }
    ...
    private function gonnaGetMeSomeRevengeButtsexin() : Void
    {
        clearOutput();
        ...
        {
            var x1 : Int = as3hx.Compat.parseInt(player.findCock(-1, -1, 6, "length") + 1);
            ...
            if (player.cor >= 50)
            {
                outputText("  You hate to admit it, but in a messed up way, her apathy is beginning to turn you on a bit by itself.  You knew you weren’t the most well-endowed adventurer in the first place, and to someone physically huge like Izumi the problem must be magnified.  For some reason, you find that idea hot as hell....\n\n");
            }
        ...
        {
            var x1 : Int = as3hx.Compat.parseInt(player.findCock(1, 6, 14, "length") + 1);
        ...
        var bigF : Void->Void = function() : Void
        {
            var x1 : Int = as3hx.Compat.parseInt(player.findCock(1, 14, -1, "length") + 1);
    ...
    // Huehue, Tentacruel!
    private function tentacruelRevengeFux() : Void
    {
        clearOutput();
        ...
        if (player.tentacleCocks() >= 10 || player.stamenCocks() >= 10)
        {
            outputText("  You decide to test the theory, snaking another prehensile appendage into each of her open, outstretched palms.  Sure enough, after a few fumbling attempts, Izumi manages to grasp the concept of a handjob and awkwardly starts to jerk them off.");
        }
    ...
    // Facefuck
    private function sayThatsAPrettyFaceBeAShameIfSomebodyJizzedAllOverIt() : Void
    {
        clearOutput();
        ...
        if (player.cocks[cockIndex].cockType == CockTypesEnum.HORSE)
        {
            outputText("  Visibly unsure of where to put herself, Izumi finally settles for brushing her lips against the exact center of the flared mass that makes up the head of your stallion-prick.");
        }
        else if (player.cocks[cockIndex].cockType == CockTypesEnum.DOG)
        {
            outputText("  Her eyes darting nervously from side to side, Izumi leans forwards and takes the very tip of your dogcock into her mouth, pressing her lips against the main bulk of what passes for your head.");
        }
        else
        {
            outputText("  You feel the soft, warm sensation of Izumi’s lips brushing against the sensitive head of your cock for a moment, then the process repeats itself as Izumi actually darts in again, unbidden, for a second smooch.");
        }
        outputText("\n\n");
        ...
        if (player.cumQ() > 500)
        {
            outputText("  Her surprise only deepens when you simply keep cumming, your prolonged orgasm flooding her mouth faster than she can swallow, forcing her cheeks to bulge out and tiny rivulets of cum to occasionally slip free of the tight seal her lips have formed around your cock.\n\n");
        }
    ...
    private function tribThatRockyHornGirl() : Void
    {
        clearOutput();
        ...
        if (player.cor > 80)
        {
            outputText("\"<i>Whatever.  Just try not to cum too hard while I’m making you my bitch, you little slut.</i>\" You shoot back, returning");
        }
        else
        {
            outputText("Ignoring her protests entirely, you return");
        }
