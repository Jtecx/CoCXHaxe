...
class MinotaurMobScene extends BaseContent implements TimeAwareInterface
{
    ...
    //326 Number of sons grown
    {
        ...
        if (flags[kFLAGS.MINOTAUR_SONS_GROW_COUNTDOWN] > 30)
        {
            flags[kFLAGS.MINOTAUR_SONS_GROW_COUNTDOWN] = 30;
        }
        if (flags[kFLAGS.MINOTAUR_SONS_GROW_COUNTDOWN] < 0)
        {
            flags[kFLAGS.MINOTAUR_SONS_GROW_COUNTDOWN] = 0;
        }
        ...
        {
            if (flags[kFLAGS.MINO_SONS_LVL_UP] > 0)
            {
    ...
    public function meetMinotaurSons() : Void
    {
        ...
        //(Non-Addicted){
            ...
            if (!player.minotaurAddicted() && !player.hasPerk(PerkLib.LactaBovineImmunity))
            {
                ...
                if (player.cor < 33)
                {
                    outputText("enjoyed their affections more than you ought to");
                }
                else if (player.cor < 66)
                {
                    outputText("indulged yourself in their glorious cum more than a few times");
                }
                else
                {
                    outputText("never felt like turning down a fuck from one of these glorious cum-dispensing fuck-beasts");
                }
            ...
            else
            {
                ...
                if (player.cor < 33)
                {
                    outputText("You wouldn't fuck your own offspring just for another hit of cum... would you?");
                }
                else if (player.cor < 66)
                {
                    outputText("You're not sure how to feel about this morally, but you can't keep from licking your lips, fantasizing about the fix these wonderful kids of yours are so willing to give you.");
                }
                else
                {
                    outputText("Who cares if they're your offspring?  They're all grown up, and they smell so... appetizing.");
                }
                //[Fight] [Submit] [Run]
                simpleChoices("Fight", fightOTaurs, "Submit", submitToMinotaurMob, "Negotiate", negotiate, "", null, "Leave", runFromMinotaurs);
            }
        ...
        else
        {
            ...
            //[Non Addicted Repeat]{
                ...
                if (!player.minotaurAddicted() && !player.hasPerk(PerkLib.LactaBovineImmunity))
                {
                    ...
                    if (player.wetness() <= 2)
                    {
                        outputText("A trickle of wetness runs down your [leg] almost immediately.");
                    }
                    else if (player.wetness() <= 4)
                    {
                        outputText("A steady flow of your juices soaks your [legs] almost immediately, just from the scent invading your nostrils.");
                    }
                    else
                    {
                        outputText("A flood of wetness soaks your [armor] and [legs] with feminine drool almost immediately as your body reflects the obscene lust coursing through your veins. This could be a bit difficult.");
                    }
                    outputText("\n\nThe largest folds his arms across his broad, well-defined chest and smirks before grunting, \"<i>");
                    if (player.hasVisiblePregnancy() && (player.pregnancyType == PregnancyStore.PREGNANCY_MINOTAUR || player.pregnancy2Type == PregnancyStore.PREGNANCY_MINOTAUR))
                    {
                        outputText("Pregnant with another brother for us, and already you come back for more?");
                    }
                    else if (player.hasVisiblePregnancy())
                    {
                        outputText("Huh... Looks like Mommy couldn't keep her legs closed, and she got knocked up by some vermin.  You're such a disappointment to your beloved family.  Don't worry, though, mom, we'll still fuck you.");
                    }
                    else
                    {
                        outputText("Hey mom, come back, so we could all have another whack at you?  We could use another brother.");
                    }
                    outputText("</i>\"\n\n");
                ...
                else
                {
                    ...
                    {
                        outputText("A flood of wetness soaks your [armor] and [legs] with feminine drool almost immediately as your body reflects the obscene lust coursing through your veins.");
                    }
                    ...
                    if (player.hasVisiblePregnancy() && (player.pregnancyType == PregnancyStore.PREGNANCY_MINOTAUR || player.pregnancy2Type == PregnancyStore.PREGNANCY_MINOTAUR))
                    {
                        outputText("Pregnant with another brother for us, and you're already back for another fix?  Why even leave in the first place?");
                    }
                    else if (player.hasVisiblePregnancy())
                    {
                        outputText("Huh... you couldn't keep your legs closed, eh, mom?  Looks like you're all knocked up with vermin, too!  You're a disappointment to our family... don't worry, though, we'll still give you a fix.");
                    }
                    else
                    {
                        outputText("Hey, mom!  You came back, so we could have another go at knocking you up?  Heh.  Knowing you, you're probably just here for another cum fix.</i>\"\n\n");
                    }
            ...
            else
            {
                ...
                if (!player.minotaurAddicted() && !player.hasPerk(PerkLib.LactaBovineImmunity))
                {
                    ...
                    if (!player.isPregnant() || player.hasNonVisiblePregnancy())
                    {
                        outputText("Come back to add to the tribe?");
                    }
                    else if (player.pregnancyType == PregnancyStore.PREGNANCY_MINOTAUR || player.pregnancy2Type == PregnancyStore.PREGNANCY_MINOTAUR)
                    {
                        outputText("Come back to show off the lil' bro yer growin'?");
                    }
                    else
                    {
                        outputText("Come back to see some real studs after letting a weakling knock you up?");
                    }
                    ...
                    if (player.wetness() >= 5)
                    {
                        outputText("You can feel your juices running unimpeded from under your [armor]; a veritable flood of need, fed by your body's primal response to the pheromones in the air.");
                    }
                    else if (player.wetness() >= 3)
                    {
                        outputText("You can feel your juices dribbling out from under your [armor], fed by your body's primal response to the pheromones in the air.");
                    }
                    else
                    {
                        outputText("You can feel your [armor] clinging to your sopping nethers, the liquid need only growing from the pheromones in the air.");
                    }
                    ...
                    if (player.cor < 33)
                    {
                        outputText("You can't think that way! Sure, you've birthed minotaurs enough to create this huge tribe of horny monsters, but you're a champion!");
                    }
                    else if (player.cor < 66)
                    {
                        outputText("You shouldn't think that way! Sure, you've had enough minotaurs inside you to spawn this tribe of muscular beast-men, but you've got a mission to focus on.");
                    }
                    else
                    {
                        outputText("You shouldn't think that way! There's a whole bevy of other creatures out there waiting to be fucked over, and if your boys want a piece of you, they're the ones who should be kneeling down and getting ready to be fucked!");
                    }
                    outputText("  What do you do?");
                }
                ...
                else
                {
                    ...
                    if (!player.isPregnant() || player.hasNonVisiblePregnancy())
                    {
                        outputText("Come for a baby? Or do you just want to be spunk-stuffed?");
                    }
                    else if (player.pregnancyType == PregnancyStore.PREGNANCY_MINOTAUR || player.pregnancy2Type == PregnancyStore.PREGNANCY_MINOTAUR)
                    {
                        outputText("Come back for some more spunk? I bet your next son comes out as a spooge-slurping minitaur.  We could use another cock-sleeve around here.");
                    }
                    else
                    {
                        outputText("Come back to see some real studs after letting a weakling knock you up?  Don't fret, we'll make you forget all about it.");
                    }
    ...
    private function runFromMinotaurs() : Void
    {
        ...
        if (player.canFly())
        {
            outputText("leaping from the small cliffs to take flight");
        }
        else
        {
            outputText("fleeing down the mountainside");
        }
        outputText(".  ");
        //ESCAPE!
    ...
    @:allow(classes.scenes.areas.highMountains)
    private function minotaurDeFeet() : Void
    {
        clearOutput();
        if (!player.hasVagina())
        {
        ...
        else
        {
            ...
            if (player.wetness() <= 2)
            {
                outputText("begins to drip from your ");
            }
            else if (player.wetness() <= 4)
            {
                outputText("leaks from your ");
            }
            else
            {
                outputText("rolls out in a wave from your ");
            }
            outputText(vaginaDescript(0) + ".");
            if (player.hasCock())
            {
                outputText("  " + SMultiCockDesc() + " hardens and dangles above the ground, leaking in sympathy with your hungry quim.  Of course, it will likely be ignored - rutting minotaurs have little use for someone else's dick.");
            }
        ...
        if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] >= 25 && player.minotaurNeed())
        {
            minotaurGangBadEnd();
        }
        else if (player.hasVisiblePregnancy())
        {
            ...
            if (player.hasVeryVisiblePregnancy())
            {
    ...
    private function nonAddictMinotaurGangBang() : Void
    {
        ...
        {
            outputText("A sudden, forceful yank stretches out your tail, and a larger member presses between your [butt] to prod at your clutching asshole.  ");
        }
        ...
        {
            outputText("The assembled beasts snicker at the noise, stroking themselves slowly while awaiting a chance at your hungry holes.");
        }
        else
        {
            outputText("The minitaur giggles at the noise, stroking himself and mewling while he waits permission to join.");
        }
        ...
        if (player.wetness() >= 5)
        {
            outputText("gushes everywhere with each thrust, puddling your lubricants on the ground");
        }
        else if (player.wetness() >= 3)
        {
            outputText("drools a thick stream of lubricants to puddle on the ground with each thrust.");
        }
        else
        {
            outputText("drips lubricant with every thrust, leaving tiny blotches of wetness on the ground.");
        }
        ...
        if (player.cor >= 66)
        {
            outputText(".  You couldn't be happier about it");
        }
        ...
        {
            outputText("his chest");
        }
        else
        {
            outputText("his shoulder");
        }
        ...
        {
            outputText("I hope you don't mind me making another brother.");
        }
        else if (player.pregnancyType == PregnancyStore.PREGNANCY_MINOTAUR || player.pregnancy2Type == PregnancyStore.PREGNANCY_MINOTAUR)
        {
            outputText("I hope you don't mind me saying hi to my little brother.");
        }
        else
        {
            outputText("I hope you come back after you pop out this vermin, so we can give you a real baby!");
        }
        ...
        if (player.hasCock())
        {
            ...
            if (player.cumQ() < 25)
            {
                outputText("a few spots of cum into his sweat-matted fur.");
            }
            else if (player.cumQ() < 50)
            {
                outputText("a steady trickle of cum into his fur.");
            }
            else if (player.cumQ() < 100)
            {
                outputText("a steady stream of cum into his matted fur.");
            }
            else
            {
                outputText("a constant flow of cum that utterly soaks the minotaur's fur and balls with your wasted, inferior seed.");
            }
        }
        outputText("\n\n");
    ...
    private function loseToMinoMobVeryPregnant() : Void
    {
        ...
        {
            outputText("She doesn't know how to keep her hungry twat closed.  You can see the vermin squirming in her belly.  Disgusting.");
        }
        else
        {
            outputText("I can feel our little brother.  It won't be long now.");
        }
        ...
        if (player.cor < 33)
        {
            outputText("irritation at being addressed so insultingly");
        }
        else if (player.cor < 66)
        {
            outputText("annoyance at their familiarity");
        }
        else
        {
            outputText("irritation at losing control of the situation");
        }
        ...
        if (player.hasCock())
        {
            outputText("  The pre-cum leaking from your [cocks] reminds you of your poor, ignored maleness, but you'll have to settle for servicing your sons' delectable organs.\n\n");
        }
        ...
        if (player.isNaga() || player.tailType == Tail.DEMONIC || player.tailType == Tail.COW || player.tailType == Tail.SHARK || player.tailType == Tail.LIZARD || player.tailType == Tail.KANGAROO)
        {
            outputText("  Even your tail reaches up to curl around a third, stroking the thick meat.");
        }
        outputText("\n\n");
        ...
        if (player.pregnancyType == PregnancyStore.PREGNANCY_MINOTAUR)
        {
            outputText("That baby is probably mine too.  You let your son father his brother.");
        }
        else
        {
            outputText("That baby isn't even the right species.  You probably get knocked up by every male you meet, but you always come back to suck off your sons.");
        }
        ...
        if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] < 8 || rand(3) == 0)
        {
            ...
            if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] > 3)
            {
                if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] == 4)
                {
                    outputText("Sorry bro, it looks like Mom's done.  You'll have to snag a shot at her later - she's pretty fucking pregnant, and I don't want her broken.");
                }
                else
                {
                    outputText("Sorry bros, it looks like Mom's done.  You'll have to snag a shot at her later - she's pretty fucking pregnant, and I don't want her broken.");
                }
            }
            else
            {
                outputText("Damn, that was nice.  Who would've guessed a slut as pregnant as Mom would be such a great cock-guzzler?");
            }
            outputText("</i>\"");
        }
        //(Long)
        else
        {
            ...
            if (player.pregnancyType == PregnancyStore.PREGNANCY_MINOTAUR)
            {
                outputText("we wouldn't want the next member of our tribe injured.");
            }
            else
            {
                outputText("we wouldn't want to ruin our favorite toy, would we?");
            }
    ...
    private function analSpearSemiPregMinotaurGangbang() : Void
    {
        ...
        {
            outputText(", you're soon expressing drops of milk into his squeezing mitts.");
        }
        else
        {
            outputText(", you're soon moaning and squirming from his crude caresses.");
        }
        ...
        if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] >= 20)
        {
            outputText("to make the tribe even bigger");
        }
        else
        {
            outputText("more brothers");
        }
        ...
        if (player.pregnancyType == PregnancyStore.PREGNANCY_MINOTAUR || player.pregnancy2Type == PregnancyStore.PREGNANCY_MINOTAUR)
        {
            outputText("our little brother");
        }
        else
        {
            outputText("her baby-maker");
        }
        outputText(".</i>\"\n\n");
        ...
        if (!player.minotaurAddicted() && !player.hasPerk(PerkLib.LactaBovineImmunity))
        {
            outputText("We've got to get mom so high on us that she'll float back to her camp and come crawling back to us tomorrow.");
        }
        else
        {
            outputText("We've gotta get mom her fix.");
        }
        ...
        if (player.hasFuckableNipples() && (player.biggestLactation() < 1 || rand(2) == 0))
        {
            ...
            if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] < 7)
            {
                outputText("shakes his head.  \"<i>You're only saving up trouble for later, little cunt.  Your hole after Mom's, remember.</i>\"  The minitaur wistfully withdraws his cock and crawls a few paces off, where he sits in a sulk.");
            }
            else
            {
                outputText("nods his head toward one of the idle members of the tribe, who pulls the runt out of your nipplehole with a 'pop' and carries him off, the line of horny minotaurs parting and then forming up again after he passes.  From behind them you can hear squeals of agony and then ecstasy as your smallest son joins you in your state of overfucked, drugged euphoria.");
            }
            outputText("\n\n");
        }
        ...
        {
            outputText(", your pregnant, milky tits swaying underneath you while they're suckled");
        }
        else
        {
            outputText(", your hard, aching nipples sucked and licked as they sway beneath you");
        }
        ...
        if (player.wetness() <= 3)
        {
            outputText(", even though you're not usually that wet");
        }
        ...
        {
            outputText(", and the minitaur's lips disentangle from your " + nippleDescript(0));
        }
        outputText(".  Hands gently lower you to the ground");
        if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] >= 5)
        {
            outputText(", but you're positioned with your ass in the air.  A voice calls out, \"<i>Okay, Mom's pretty out of it.  One at a time and don't damage her, okay?</i>\"\n\nGiggling dreamily, you're fucked repeatedly, spunk squirting from your anus with every thrust.  You pass out like that, feeling quite satisfied.");
        }
        else
        {
            outputText(".  They step back and admire their handiwork while you float through your narcotic-infused haze.");
        }
        player.sexReward("cum", "Anal");
        player.sexReward("cum", "Vaginal");
        dynStats("spe", -.5, "int", -.5, "lib", .5, "sen", .5, "cor", 1);
    ...
    @:allow(classes.scenes.areas.highMountains)
    private function victoryMinotaurGang() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_minotaurSons);
        //(HP)
        ...
        {
            if (flags[kFLAGS.MINO_SONS_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.MINO_SONS_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.MINO_SONS_DEFEATS_COUNTER] = 1;
            }
            //level up
    ...
    private function victoryMinotaurGangTitFuck() : Void
    {
        ...
        if (player.minotaurAddicted() || player.hasPerk(PerkLib.LactaBovineImmunity))
        {
            outputText("Of course it helps that pleasing them will give you exactly what you want - more of that delicious, wondrous cock-cream.  ");
        }
        ...
        {
            outputText("on your forelegs");
        }
        else if (player.isGoo())
        {
            outputText("into your gooey undercarriage");
        }
        else if (player.isNaga())
        {
            outputText("into the coils of your tail");
        }
        else
        {
            outputText("INTO YOUR ERROR: UNCLASSIFIED LOWER BODY");
        }
        ...
        if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] >= 10)
        {
            ...
            if (player.minotaurAddicted() || player.hasPerk(PerkLib.LactaBovineImmunity))
            {
                outputText("I'd hate to see you go mad with need around a lesser beast.");
            }
            else
            {
                outputText("This will be so much better once you learn to need it.");
            }
            outputText("</i>\"\n\n");
        ...
        else
        {
            ...
            if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] == 3)
            {
                outputText("one minotaur");
            }
            else
            {
                outputText(num2Text(flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] - 2) + " minotaurs");
            }
            ...
            if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] == 3)
            {
                outputText("You grab the minitaur's cock and drag the shy beast-man over your cum-slicked tits, wasting no time in burying the shaft in mommy's mammaries, where it belongs.");
            }
            else
            {
                ...
                if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] == 4)
                {
                    outputText("the remaining brother");
                }
                else
                {
                    outputText("one of the remaining brothers");
                }
            ...
            if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] > 3)
            {
                outputText("His glaze spurts up along the opening created by his larger brother's shaft, and soon he too is cumming, obediently orgasming for you.  ");
            }
            ...
            if (!player.minotaurAddicted() && !player.hasPerk(PerkLib.LactaBovineImmunity))
            {
                outputText("Maybe we can get Mom completely hooked.  Then we won't have to fight her?");
            }
            else
            {
                outputText("That'll be a nice snack for her later.  We wouldn't want her to lose control around lesser species.");
            }
            outputText("</i>\"\n\n");
            ...
            if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] < 5)
            {
                outputText("The minotaurs let you get dressed while they sigh happily, clearly pleased with the outcome.");
            }
            else
            {
                ...
                if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] > 5)
                {
                    ...
                    if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] >= 6)
                    {
                        outputText("s");
                    }
                }
                ...
                if (!player.isGoo())
                {
                    outputText(" look like a white goo-girl");
                }
                else
                {
                    outputText("r gelatinous body is filled with long slashes of white");
                }
    ...
    private function victoryAllThePenetrationsMinotaurGangBang() : Void
    {
        ...
        {
            outputText(" nearly instantly, too aroused to turn down the idea of their mother doing ANYTHING with their hard shafts.");
        }
        else
        {
            outputText(" somewhat sluggishly as they work to get nude for you, despite their wounds.  As they finish, they start to move a bit faster, recovering from the battle quicker than you would have thought.");
        }
        ...
        {
            outputText("It slips into your " + vaginaDescript(0) + " with ease, nestling into your accommodating opening as if it were a second home.");
        }
        else if (player.vaginalCapacity() >= 40)
        {
            outputText("It slips into your " + vaginaDescript(0) + " ever so slowly, gradually widening your quim until it's able to handle its size.");
        }
        else
        {
            outputText("It stretches your " + vaginaDescript(0) + " with painful force as it penetrates you, forcibly widening your quim until it's able to nestle its sheath against your vulva.");
        }
        ...
        {
            outputText("Your " + assholeDescript() + " is so tight that he's forced to push and release over and over.  The painful penetration drags on and on as he widens your hole to accept his monstrous cock.");
        }
        else if (player.analCapacity() < 60)
        {
            outputText("Your " + assholeDescript() + " slowly widens to accept the cock that's being deposited into its depths.  You find yourself somewhat grateful that you've been stretched enough to prevent this from being painful.");
        }
        else
        {
            outputText("Your " + assholeDescript() + " devours the cock easily.  The only thing even close to a tight fit is the feeling of your pucker being stretched at his very base.");
        }
        ...
        {
            outputText("  There's still more you could be doing.");
        }
        outputText("\n\n");
        ...
        if (player.hasFuckableNipples() && flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] > 4)
        {
            ...
            {
                outputText("Milky lube");
            }
            else
            {
                outputText("Lubricant");
            }
            ...
            if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] == 4)
            {
                outputText("One stud assembles next to ");
            }
            else if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] >= 5 && player.totalBreasts() == 2)
            {
                outputText("Two studs assemble around ");
            }
            else
            {
                var dicks : Float = player.totalBreasts();
                if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] - 3 < dicks)
                {
                    dicks = flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] - 3;
                }
            ...
            if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] == 4)
            {
                outputText("He excitedly fingers");
            }
            else
            {
                outputText("They excitedly finger");
            }
            outputText(" your tits, slipping a few digits in to explore the depths of your chest-pussies.  ");
            if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] == 4)
            {
                outputText("He licks his fingers clean, and a moment later, a cock is spearing one of your nipples, disappearing deep into the tingling passages concealed within your chest.\n\n");
            }
            else
            {
                outputText("They lick their fingers clean, and a moment later, cocks are spearing your nipples, disappearing deep into the tingling passages concealed within your chest.\n\n");
            }
        }
        //Dicks)
        if (player.hasCock())
        {
            outputText(SMultiCockDesc() + " hangs limply on one of your boys' bellies, dribbling weakly at your need, but there's not any way for it to get involved in the crowd of orgiastic bodies.\n\n");
        }
        ...
        if (player.cor < 33)
        {
            outputText(" that you cringe a little when you realize you're having an incestuous orgy, but the rising tide of minotaur-drug in your bloodstream washes the errant thought away.");
        }
        else
        {
            outputText(" that you can barely hang on to your sanity, but it feels like all the minotaur-drug leaking into your bloodstream might wash away even that.");
        }
        outputText("\n\n");
        ...
        if (player.hasFuckableNipples() && flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] > 4)
        {
            outputText("your nipples squirt, ");
        }
        ...
        if (player.hasFuckableNipples() && flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] > 4)
        {
            if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] > 4)
            {
                outputText("Meanwhile, your " + nippleDescript(0) + "s get a similar feeling from the studs arranged around them");
            }
            else
            {
                outputText("Meanwhile, your " + nippleDescript(0) + " gets a similar feeling from the stud pumping away at it");
            }
    ...
    private function forceMinitaurToGiveOral(choice : Float = 0) : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_minotaurSons);
        ...
        if (choice == 0)
        {
            ...
            else
            {
                if (player.hasBalls())
                {
                    outputText("lift your balls and ");
                }
            ...
            {
                outputText(", until the huge size of it is revealed.");
            }
            else if (player.clitLength >= 3)
            {
                outputText(", until the cock-like length of your feminine organ is revealed.");
            }
            else if (player.clitLength >= 1)
            {
                outputText(", until the large, womanly organ is revealed.");
            }
            else
            {
                outputText(", revealing its cute button self.");
            }
            outputText("\n\n");
            ...
            if (player.isTaur())
            {
                outputText("disappearing under your hind-legs as he attends to his newly commanded duty.");
            }
            else
            {
                outputText("looking up at you longingly as he leans in towards your entrance.");
            }
            ...
            {
                outputText(", even sucking your clit into his mouth and running his tongue along its length.");
            }
            else
            {
                outputText(", even stroking your clit in his hand in time with his licks.");
            }
            ...
            {
                outputText("He jacks your huge clit like a cock, and the incredible stimulation is too much for you to take.");
            }
            else
            {
                outputText("He sucks and licks your pussy and clit simultaneously, displaying hidden talent with his bovine tongue.  It's just too much for you to take!");
            }
            outputText("  You cum - hard, into the cow-boy's muzzle.  He laps at your juices enthusiastically - a natural-born cunt-licker.");
            if (player.wetness() >= 5)
            {
                outputText("  The wave of squirting fluids drips from his shaggy chin and runs down his chest.");
            }
            else if (player.wetness() >= 4)
            {
                outputText("  The stream of dribbling fluids drips from his shaggy chin to run down his chest.");
            }
            else if (player.wetness() >= 3)
            {
                outputText("  The dribbling fluid runs down his shaggy chin and drips towards the ground.");
            }
            else if (player.wetness() >= 2)
            {
                outputText("  Some of the fluids escape his mouth to run down his shaggy chin.");
            }
            else
            {
                outputText("  All of your moistness is collected on his tongue and sucked into his mouth.");
            }
            ...
            if (player.hasCock())
            {
                outputText("  Salty cum dribbles down his back from your spurting [cocks], the male part of you orgasming along in a perverse symphony with your girl-parts.\n\n");
            }
        }
        //(PENISCOCKSUCKERFUCK)
        else
        {
            ...
            if (player.isTaur())
            {
                outputText("flex your [cocks] to slap against your belly.");
            }
            else
            {
                outputText("stroke your [cocks] with one hand.");
            }
            outputText("  " + SMultiCockDesc() + " ");
            if (player.lust < 50)
            {
                outputText("grows hard");
            }
            else
            {
                outputText("gets even harder");
            }
            ...
            {
                outputText("looking up at you longingly as he leans in towards " + oMultiCockDesc() + ".");
            }
            ...
            if (player.hasBalls())
            {
                outputText("around your " + sackDescript() + " before curling ");
            }
            ...
            if (!player.isTaur())
            {
                outputText("break eye-contact or ");
            }
            outputText("slow down in the slightest.\n\n");
            
            if (player.hasVagina())
            {
                outputText("Humming happily, the minitaur slips a digit inside your " + vaginaDescript() + ", and while you didn't ask for it, the added stimulation only heightens your need.  ");
            }
            ...
            if (player.cockTotal() > 1)
            {
                ...
                {
                    outputText("one of your ignored penises");
                }
                else
                {
                    outputText("on your ignored penis");
                }
            }
            outputText(".");
            if (player.hasVagina())
            {
                outputText("  You begin to cum into your son's mouth, your pussy hugging his finger tightly inside its depths.");
            }
            outputText("\n\n");
            ...
            if (player.cumQ() < 100)
            {
                outputText("squirts into his mouth repeatedly, puddling on his tongue before he noisily swallows it.");
            }
            else if (player.cumQ() <= 250)
            {
                outputText("squirts into his mouth over and over, puddling on his tongue and forcing his cheeks to bulge slightly before he swallows.");
            }
            else if (player.cumQ() <= 1000)
            {
                outputText("spurts into his mouth, filling it with the first burst alone.  He swallows it just in time for the next, and he gulps noisily to keep up with your orgasm.");
            }
            else
            {
                ...
                if (player.cumQ() >= 2000)
                {
                    outputText("  Spunk runs from his nostrils as he utterly fails.");
                }
            }
            ...
            if (player.hasVagina())
            {
                if (player.wetness() >= 5)
                {
                    outputText("  Your cunny squirts onto his chest throughout the ordeal.");
                }
                else if (player.wetness() >= 3)
                {
                    outputText("  Your cunny drips everywhere, making a mess of your [legs].");
                }
            }
    ...
    private function victoryBJNippleFuckMinotaurGang() : Void
    {
        ...
        if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] == 3)
        {
            outputText("the minitaur, much to the smaller monster's chagrin.");
        }
        else
        {
            outputText("the rest of his kin, much to their chagrin.");
        }
        ...
        {
            outputText("most of a foot");
        }
        else if (player.biggestTitSize() <= 14)
        {
            outputText("over a foot");
        }
        else if (player.biggestTitSize() <= 30)
        {
            outputText("nearly two feet");
        }
        else
        {
            outputText("the whole length");
        }
        ...
        if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] >= 7)
        {
            outputText(", chased by jeers from the remainder of your brood");
        }
        ...
        if (player.hasCock())
        {
            ...
            if (player.hasVagina())
            {
                outputText(" and " + vaginaDescript() + " drool");
            }
            else
            {
                outputText(" drools");
        ...
        {
            outputText(" while you hose sprays of milk around the twin rods");
        }
        else if (player.biggestLactation() >= 1.5)
        {
            outputText(" while you dribble milk around the twin rods");
        }
        outputText(".\n\n");
        ...
        if (flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] < 5)
        {
            outputText("relax and fan themselves while giving you lusty looks");
        }
        else
        {
            outputText("stare gloomily at the lucky three before turning to regard your form again with unrepentant lust");
        }
    ...
    private function minotaurGangBadEnd() : Void
    {
        ...
        {
            outputText("slide inside your nipple-cunts and finger your chest pussies in a small approximation of what's happening below.");
        }
        else if (player.biggestLactation() >= 1)
        {
            outputText("tug and pull at your drippy nipples, releasing thick flows of creamy milk.");
        }
        else
        {
            outputText("tug at your achy nipples.");
        }
        ...
        {
            outputText("inside your empty womb");
        }
        else
        {
            outputText("against the plugged entrance of your pregnant womb");
        }
    ...
    private function minotaurGangBadEnd2() : Void
    {
        ...
        if (player.wetness() >= 5)
        {
            outputText("Sure, you were a squirter before, but now, you'll squirt a half-dozen times even before you orgasm.");
        }
        else
        {
            outputText("Sure, you could get plenty wet before, but now, you'll be squirting and drizzling before you even orgasm.");
        }
