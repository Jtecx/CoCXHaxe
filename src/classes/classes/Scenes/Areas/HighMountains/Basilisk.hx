...
class Basilisk extends Monster
{
    ...
    private function compulsion() : Void
    {
        ...
        //Immune to Basilisk?{
            ...
            else
            {
                ...
                if (player.perkv1(IMutationsLib.GorgonEyesIM) >= 1)
                {
                    flags[kFLAGS.BASILISK_RESISTANCE_TRACKER] += 2;
                }
                if (player.perkv1(IMutationsLib.GorgonEyesIM) >= 2)
                {
                    flags[kFLAGS.BASILISK_RESISTANCE_TRACKER] += 2;
                }
    ...
    private function basiliskTailSwipe() : Void
    {
        ...
        if (player.takePhysDamage(as3hx.Compat.parseInt((str + 20) - Math.random() * (player.tou + player.armorDef)), true) == 0)
        {
    ...
    override public function midAttackSkip() : Bool
    {
        if (!player.hasPerk(PerkLib.BasiliskResistance))
        {
    ...
    {
        if (!player.hasStatusEffect(StatusEffects.BasiliskCompulsion) && rand(3) == 0 && !hasStatusEffect(StatusEffects.Blind))
        {
            compulsion();
        }
        else if (rand(3) == 0)
    ...
    public function new()
    {
        ...
        this.drop = new ChainedDrop().add(useables.EBONBLO, 1 / 20).elseDrop(consumables.REPTLUM);
