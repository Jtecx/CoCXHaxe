...
class Minerva extends Monster
{
    ...
    private function minervaBite() : Void
    {
        ...
        var damage : Int = as3hx.Compat.parseInt((str + 5) - rand(player.tou) - player.armorDef);
        if (flags[kFLAGS.MINERVA_LVL_UP] >= 10)
        {
            damage + 340;
        }
        else if (flags[kFLAGS.MINERVA_LVL_UP] >= 6)
        {
            damage + 255;
        }
        else if (flags[kFLAGS.MINERVA_LVL_UP] >= 2)
        {
            damage + 170;
        }
        else
        ...
        {
            outputText("  Your hands lash out, knocking her head to the side as she tries to bite you.  With the bite deflected, Minerva makes a loud chomping sound as she only bites the air.");
        }
        //[if attack lands]
        else
        {
    ...
    private function minervaKnowsKungfu() : Void
    {
        ...
        var damage : Int = as3hx.Compat.parseInt((str + weaponAttack + 100) - rand(player.tou) - player.armorDef);
        if (flags[kFLAGS.MINERVA_LVL_UP] >= 10)
        {
            damage += 400;
        }
        ...
        {
            outputText("  Steadying yourself, you reach up, grabbing hold of Minerva as she attempts to land a heavy blow on you.  Grunting hard, you pull against her and toss the siren aside completely, halting her attack.");
        }
        //[if attack lands]
        else
        {
    ...
    {
        var damage : Int = as3hx.Compat.parseInt((str + 35) - rand(player.tou) - player.armorDef);
        if (flags[kFLAGS.MINERVA_LVL_UP] >= 10)
        {
            damage += 140;
        }
        ...
        if (damage <= 0)
        {
            outputText("  Lashing out with a fierce kick you intercept the tail-whip, your [foot] impacting against her strong appendage and totally neutralizing its momentum.");
        }
        //[if attack lands]
        else
        {
            ...
            {
                outputText(", but your defense has been reduced");
            }
            outputText("! ");
            player.takePhysDamage(damage, true);
    ...
    private function minervaUsesHalberdStab() : Void
    {
        ...
        var damage : Int = as3hx.Compat.parseInt((str + weaponAttack) - rand(player.tou));
        if (flags[kFLAGS.MINERVA_LVL_UP] >= 10)
        {
            damage += 100;
        }
        ...
        if (player.getEvasionRoll())
        {
            outputText("  You sidestep the attack just as she thrusts the point past your face.");
        }
        //[else block]
        else if (damage < 0)
        {
            outputText("  With all your strength, you swing your [weapon], the blow landing on the side of Minerva's halberd and deflecting the goring strike away from you.");
        }
        //[if attack lands]
        else
        {
    ...
    private function minervaUsesHalberdCHOP() : Void
    {
        ...
        var damage : Int = as3hx.Compat.parseInt((str + 100) - rand(player.tou) - player.armorDef);
        if (flags[kFLAGS.MINERVA_LVL_UP] >= 10)
        {
            damage += 400;
        }
        ...
        if (player.getEvasionRoll())
        {
            outputText("  You get out of the way quickly, her attack chopping deeply into the earth. ");
        }
        //[else block]
        else if (damage < 0)
        {
            outputText("  In a mad show of pure skill, you lift your hands, clamping them down on the cheeks of the halberd blade and stop Minerva's attack cold, bewildering the siren in the process.");
        }
        //[if attack lands]
        else
        {
    ...
    private function kiteFire() : Void
    {
        ...
        var damage : Int = as3hx.Compat.parseInt(10 + (inteused / 3 + rand(inteused / 2)) * 1.5);
    ...
    private function bootyShortInYoFaceSon() : Void
    {
        ...
        if (flags[kFLAGS.MINERVA_LVL_UP] >= 10)
        {
            player.takeLustDamage(50 + player.lib / 10 + rand(8), true);
        }
        else if (flags[kFLAGS.MINERVA_LVL_UP] >= 6)
        {
            player.takeLustDamage(40 + player.lib / 10 + rand(7), true);
        }
        else if (flags[kFLAGS.MINERVA_LVL_UP] >= 2)
        {
            player.takeLustDamage(30 + player.lib / 10 + rand(6), true);
        }
        else
        {
    ...
    //The Siren's Song (2-part attack) (Rarely used or when she's desperate aka: Less than 10% hp)
    {
        ...
        else
        {
            ...
            {
                outputText("  You cover your ears before she even opens her lips, wary of its power.  Judging by the dim feeling of pleasure simmering through you with the little sound you're picking up regardless, it probably was for the better.");
            }
            //No wait - insta loss:
            else
            {
    ...
    override private function performCombatAction() : Void
    {
        ...
        else if (rand(25) == 0 || (HP < 100 && rand(2) == 0))
        {
            sirensSong();
        }
        ...
        {
            var choices : Array<Dynamic> = [lickDatPole, 
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        ...
        {
            SceneLib.mountain.minervaScene.beatUpDatSharpie();
        }
    }
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        ...
        {
            SceneLib.mountain.minervaScene.loseToMinerva();
        }
    }
    ...
    public function new()
    {
        ...
        //leave min and max levels to easily balance npc combat
        else
        {
            
            var lvlMulti : Float = flags[kFLAGS.MINERVA_LVL_UP];
        ...
        this.balls = 2;
        this.ballSize = 3;
        if (flags[kFLAGS.MINERVA_PURIFICATION_PROGRESS] >= 10 || flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] >= 10)
        {
            ballSize += 2;
        }
        ...
        if (flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] < 10)
        {
            (try cast(this.drop, WeightedDrop) catch(e:Dynamic) null).add(consumables.PURPEAC, 1);
        }  //corrupted still non-existent  
        ...
        {
            this.createPerk(PerkLib.PrestigeJobTempest, 0, 0, 0, 0);
        }
