...
class Izumi extends Monster
{
    // Set trace outout for this classes' content.
    private var combatDebug : Bool = true;
    
    public function new()
    {
        super();
        var lvlMulti : Float = flags[kFLAGS.IZUMI_LVL_UP];
        ...
        if (player.hasStatusEffect(StatusEffects.RiverDungeonA))
        {
            ...
            this.drop = new ChainedDrop().add(armors.SPKIMO, 0.1).add(necklaces.OBNECK, 0.1).add(consumables.ONISAKE, 0.5).add(useables.PCSHARD, 1);
        ...
        else
        {
            ...
            this.drop = new ChainedDrop().add(armors.SPKIMO, 1 / 10).add(necklaces.OBNECK, 1 / 5).elseDrop(consumables.ONISAKE);
        }
        ...
        {
            this.createPerk(PerkLib.PrestigeJobBerserker, 0, 0, 0, 0);
        }
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (player.hasStatusEffect(StatusEffects.RiverDungeonA))
        {
            SceneLib.dungeons.riverdungeon.defeatedByMistOni();
        }
        else
        {
    ...
    // Handle chokeslam mechanics
    {
        ...
        if (player.hasStatusEffect(StatusEffects.Chokeslam))
        {
            if (combatDebug)
            {
                trace("ChokeSlam Rounds to Damage: " + player.statusEffectv1(StatusEffects.Chokeslam));
            }
        ...
        {
            var actions : Array<Dynamic> = [straightJab, straightJab, straightJab, roundhouseKick, roundhouseKick, roundhouseKick, chokeSlam];
    ...
    public function cleanup() : Void
    {
        if (combatDebug)
        {
            trace("Cleaning up lingering effects...");
        }
        cleanupChokeslam();
        cleanupTitsmother();
    }
    ...
    public function straightJab() : Void
    {
        ...
        var damage : Int = as3hx.Compat.parseInt((str + 175) - rand(player.tou) - player.armorDef);
        if (flags[kFLAGS.IZUMI_LVL_UP] >= 2)
        {
            damage += 50;
        }
        ...
        else
        {
            ...
            if (player.isNaga())
            {
                outputText(" raise back up onto your [legs]");
            }
            else
            {
                outputText(" stand");
    ...
    public function roundhouseKick() : Void
    {
        ...
        if (player.cor >= 50 || player.lib >= 50 || player.sens >= 50)
        {
            ...
            else if (flags[kFLAGS.IZUMI_LVL_UP] >= 2)
            {
    ...
    public function chokeSlam() : Void
    {
        ...
        else
        {
            ...
            if (combatDebug)
            {
                trace("Applied Chokeslam effect");
            }
        }
    }
    ...
    public function chokeSlamStruggle() : Void
    {
        ...
        if (brokeFree)
        {
            if (combatDebug)
            {
                trace("Escaped from Chokeslam grapple");
            }
            chokeSlamEscape();
        }
        else
    ...
    public function chokeSlamDamage() : Void
    {
        ...
        var damage : Int = as3hx.Compat.parseInt((str + 225) - rand(player.tou) - player.armorDef);
        if (flags[kFLAGS.IZUMI_LVL_UP] >= 2)
        {
            damage += 100;
        }
    ...
    {
        var damage : Float = 50 + rand(player.str);
        if (combatDebug)
        {
            trace("Escaping from Chokeslam!");
        }
        
        outputText("Scrabbling desperately against her wrist, you narrow your eyes at the Oni woman’s superior expression,");
        if (player.isBiped())
        {
            outputText(" raise a [leg] and kick her roundly");
        }
        else if (player.isNaga())
        {
            outputText(" raise your tail and slap her solidly");
        }
        else
        {
            outputText(" and slap her square");
        }
    ...
    public function groundPound() : Void
    {
        ...
        else
        {
            ...
            var spdReducedBy : Int = as3hx.Compat.parseInt(player.spe * 0.25);
            player.buff("Groundpound").addStats({
                                        spe : -spdReducedBy
                                    }).withText("Groundpound!").combatPermanent();
            
            
            if (combatDebug)
            {
                trace("Applying Groundslam slow");
            }
        }
    }
    ...
    public function titSmother() : Void
    {
        ...
        if (flags[kFLAGS.IZUMI_LVL_UP] >= 6)
        {
            player.takeLustDamage((player.lib / 15) + 15 + rand(15), true);
        }
        else if (flags[kFLAGS.IZUMI_LVL_UP] >= 2)
        {
            player.takeLustDamage((player.lib / 15) + 10 + rand(10), true);
        }
        else
        {
    ...
    public function titSmotherStruggle() : Void
    {
        ...
        if (brokeFree)
        {
            if (combatDebug)
            {
                trace("Broke free of Titsmother!");
            }
            titSmotherEscape();
        }
        else
        {
            ...
            else
            {
                ...
                if (player.hasCock())
                {
                    ...
                    if (player.cockTotal() > 1)
                    {
                        outputText(" erections throb");
                    }
                    else
                    {
                        outputText(" erection throbs");
                    }
    ...
    public function titSmotherEscape() : Void
    {
        ...
        cleanupTitsmother();
        var damage : Float = (15 + rand(player.str));
        this.HP -= damage;
        outputText("<b>([font-damage]" + damage + "[/font])</b>");
    }
    ...
    public function titSmotherWait() : Void
    {
        ...
        if (combatDebug)
        {
            trace("Waiting during TitSmother");
        }
