...
class MinervaScene extends BaseContent implements TimeAwareInterface
{
    public var minervaPurified(get, never) : Bool;
    ...
    public function minervaVCapacity() : Float
    {
        return 90;
    }
    public function minervaACapacity() : Float
    {
        return 100;
    }
    private function get_minervaPurified() : Bool
    {
    ...
    public function minervaSprite() : Void
    {
        if (minervaPurified)
        {
            spriteSelect(SpriteDb.s_minerva_pure);
        }
        else if (flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] >= 10)
        {
            spriteSelect(SpriteDb.s_minerva_corrupt);
        }
        else
        {
            spriteSelect(SpriteDb.s_minerva);
        }
    }
    
    public function minervaRomanced() : Bool
    {
    ...
    public function timeChange() : Bool
    {
        ...
        if (flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] >= 10 && flags[kFLAGS.MINERVA_CHILDREN] > 0)
        {
            ...
            {
                flags[kFLAGS.MINERVA_CORRRUPTED_HARPY_COUNTER] *= 2;
            }
        }
        if (model.time.hours > 23)
        {
            if (flags[kFLAGS.MINERVA_CORRUPTION_TIMER] > 0)
            {
                --flags[kFLAGS.MINERVA_CORRUPTION_TIMER];
            }
        ...
        if (pregnancy.incubation == 0 && pregnancy.type == PregnancyStore.PREGNANCY_PLAYER)
        {
            ...
            {
                minervaCorruption.corruptMinervaBirth();
            }
            else
            {
    ...
    public function tryToImpregnateMinerva() : Void
    {
        ...
        //No limits for corrupted one!{
            ...
            if (flags[kFLAGS.MINERVA_CHILDREN] >= 100)
            {
                return;
            }  //Maxed out!  
            if (flags[kFLAGS.MINERVA_CHILDREN] > 0)
            {
                chance -= (flags[kFLAGS.MINERVA_CHILDREN] * 2);
            }
        }
    ...
    public function encounterMinerva() : Void
    {
        if (flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] > 0)
        {
            if (flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] == 10)
            {
                minervaCorruption.corruptEncounter();
            }
            else if ((flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] == 5 || flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] == 6)
                && flags[kFLAGS.MINERVA_CORRUPTION_TIMER] == 0)
            {
                minervaCorruption.corruptTransformation();
            }
            else
            {
                minervaCorruption.lustfulEncounter();
            }
        }
        else if (flags[kFLAGS.MINERVA_PURIFICATION_PROGRESS] == 10)
        {
            minervaPurification.visitTowerPostPurification();
        }
        else if (flags[kFLAGS.MET_MINERVA] == 0)
        {
        ...
        else if (flags[kFLAGS.MINERVA_BACKSTORY_LEARNED] > 0 && player.gender > 0 && flags[kFLAGS.MINERVA_LOVE] == 0 && rand(2) == 0)
        {
            minervaAfterFiveTalksRomancing();
        }
        else if (flags[kFLAGS.MINERVA_LOVE] == 1)
        {
    ...
    private function firstMinervaEncounter() : Void
    {
        ...
        //If Sophie Bimbo=0:
        if (flags[kFLAGS.MET_SOPHIE_COUNTER] > 0 && flags[kFLAGS.SOPHIE_BIMBO_ACCEPTED] == 0 && flags[kFLAGS.SOPHIE_DISABLED] == 0 && !SceneLib.sophieFollowerScene.sophieFollower() && !player.hasStatusEffect(StatusEffects.SophieOff))
        {
            outputText(" that matronly harpy, Sophie");
        }
        else
        {
            outputText(" something interesting");
        }
        ...
        {
            outputText(".  With any luck, you might even find a hunky minotaur to scratch that burning itch in your mind");
        }
    ...
    private function takeMinervasPath() : Void
    {
        ...
        if (player.cor < 50)
        {
            outputText("  \"<i>All right, you!  Who are you and what are you doing here?!  I don't get any visitors besides those horny harpies, so what are you after?  Hm?");
        }
        //High corruption:
        else
        {
            outputText("  \"<i>All right, you corrupted beast!  Who are you and what are you doing here?  Your presence is not welcome in this sanctuary... answer quickly, or I'll drop you to your death!");
        }
        //Demon Score=3>:
        if (player.isRace(Races.DEMON, 1, false) || player.isRace(Races.IMP, 1, false))
        {
            outputText("  Filthy demon!  Come to finish your work, have you?!  You won't take me without a fight!  Maybe if you beg for your life I'll give you a chance. Speak!  Convince me to not simply kill you here and now!");
        }
        ...
        if (player.isGoo())
        {
            ...
            if (player.cor >= 75)
            {
                outputText("... \"<i>yet</i>\", you think to yourself.");
            }
        ...
        if (player.cor > 66)
        {
            outputText("  She is wise not to fully trust you.");
        }
    ...
    public function repeatEncounterMinerva() : Void
    {
        ...
        if (!minervaRomanced())
        {
            outputText("  \"<i>Ah, [name], you have returned!  What can I do for you?</i>\"");
        }
        else
        {
            outputText("  \"<i>Mmm... my lover has come to see me!  Oh, how I have missed you, my sweet!  Won't you join me for a bath?  Or... is there anything else I can do for you?</i>\"");
        }
    ...
    private function minervaThirdPlusEncounter() : Void
    {
        ...
        if (!minervaRomanced())
        {
            outputText("  \"<i>Welcome back, [name]!  It's good to see you again.  Have you come to give me some company, or would you like to partake in some of the fruit?  Or are you maybe here to sate your thirst?</i>\"");
        }
        else
        {
            outputText("  \"<i>Darling!  Coming to visit me all the way out here!  How sweet of you!</i>\"");
        }
    ...
    private function goForASkyRideWithMinervaHeartBaring() : Void
    {
        ...
        if (player.isGoo())
        {
            outputText(" while you're barely able to maintain your slimy cohesion");
        }
    ...
    private function yeahSureMinervaILikeYouAndShit() : Void
    {
        ...
        if (player.gender == 1)
        {
            if (player.cockThatFits(minervaVCapacity()) >= 0)
            {
                doNext(minervaCowgirlSex);
            }
            else
            {
                doNext(letMinervaSuckYouOff);
            }
        }
        else
        {
            doNext(minervaLapSex);
        }
    }
    ...
    private function rejectMinervasLove() : Void
    {
        ...
        if (player.cor < 45)
        {
            outputText("The look in the siren's eyes only makes this harder for you as you look into them; you can't help but feel bad that you really don't feel anything special for her.");
        }
        else
        {
            outputText("You can tell she really wants this - the siren is almost desperate for your attention.  Unfortunately for the love starved herm, she won't find the love she craves with you.");
        }
    ...
    public function genericMenu(display : Bool = false) : Void
    {
        menu();
        if (display)
        {
        ...
        if (minervaRomanced() && model.time.hours >= 19)
        {
            addButton(6, "Sleep With", sleepWithMinerva);
        }
        if (player.hasKeyItem("Rathazul's Purity Potion") >= 0)
        {
            addButton(7, "Purify", minervaPurification.purificationByRathazul);
        }
        if (player.hasKeyItem("Marae's Seed") >= 0)
        {
            addButton(8, "Plant Seed", minervaPurification.purificationByMarae);
        }
        if (flags[kFLAGS.MINERVA_PURIFICATION_PROGRESS] == 0)
        {
            addButton(10, "Taint", minervaCorruption.introToCorruptMinerva).hint("Taint her spring with a bottle of Fuck Draft.\n\n"
                                                    + "<b>Corrupt encounters are not finished scene-wise, but 100% functional. There WILL be a button to revert her back to normal state (requires Debug Mode). Enjoy the scenes!</b>").disableIf(!player.hasItem(consumables.F_DRAFT), "Requires a bottle of Fuck Draft").disableIf(player.cor < 50 - player.corruptionTolerance, "You're not corrupted enough.", "???").disableIf(flags[kFLAGS.MET_MINERVA] < 4, "You need to visit her a bit more often for this", "???").disableIf(!debug, "Not released yet. Requires to enable debug mode (settings) to view. Sorry, the text is not good enough yet.");
        }
    ...
    public function minervaTalkSelect(bath : Bool = true) : Void
    {
        ...
        else if (flags[kFLAGS.MINERVA_TOWER_TREE] != null)
        {
    ...
    private function talkingToMinervaAboutBackstory() : Void
    {
        ...
        else
        {
            ...
            if (!minervaRomanced())
            {
                outputText("  \"<i>You're a good friend to have, and you know that, [name].</i>\"");
            }
            //Romance:
            else
            {
                outputText("  \"<i>You're a wonderful lover, you know.  I couldn't be happier that you found me.</i>\"");
            }
            // PC returns to camp.
            if (flags[kFLAGS.MINERVA_BACKSTORY_LEARNED] == 0)
            {
    ...
    private function minervaMotherhood() : Void
    {
        ...
        if (player.gender == 1)
        {
            outputText(" and those goblins");
        }
        //PC vagina:
        else if (player.gender == 2)
        {
            outputText(" and that bee-girl");
        }
        //PC herm:
        if (player.gender == 3)
        {
            outputText(", goblins and that bee-girl");
        }
        ...
        if (minervaRomanced())
        {
            outputText(" hug her tightly and kiss her cheek, telling her that she will have children one day, and perhaps you will have to see about helping her with that 'problem.'");
        }
        //No romance:
        else
        {
            outputText(" place a hand on her shoulder and give her a comforting smile before telling her that she will have that one day - you're sure of it.");
        }
    ...
    public function minervaSexMenu(display : Bool = true) : Void
    {
        if (display)
        {
            clearOutput();
            ...
            if (flags[kFLAGS.TIMES_MINERVA_SEXED] == 0)
            {
                ...
                if (player.hasVagina())
                {
                    outputText("  Would you mind how we do it?  You're the one asking, so... would you want me to fuck your ass?  Or... do you want me to try to fit between those sexy hips of yours?");
                }
                outputText("</i>\"");
            }
            ...
            else if (!minervaRomanced())
            {
                ...
                if (player.hasVagina())
                {
                    outputText("  Since you don't seem to mind... would you like me to fuck your ass or to squeeze into your cute cunny?");
                }
                outputText("</i>\"");
            }
            //Romanced:
            else
            {
                
                {
                    ...
                    if (player.hasCock())
                    {
                        outputText("  Since we're together, I'll even let you have my pussy, I bet you like the sound of that!");
                    }
                    //PC female, add:
                    if (player.hasVagina())
                    {
                        outputText("  I'd be happy to pound that cute butt of yours and stuff your hot pussy - which would you rather have, my dear?");
                    }
                    outputText("</i>\"");
                }
            }
    ...
    private function minervaCowgirlSex() : Void
    {
        ...
        {
            outputText(", with some trepidation, given the difference in size,");
        }
        ...
        {
            outputText("  Minerva runs a hand through her hair and smiles, \"<i>Well... the last time was quite nice, I think it could be fun.  Sure, let's do it.</i>\"");
        }
        ...
        if (player.tallness < 96)
        {
            outputText(", as well as her superior size,");
        }
        ...
        if (player.cumQ() < 500)
        {
            outputText(", Minerva's belly visibly bloated by the time you are finished");
        }
        outputText(".");
        if (player.cumQ() < 500)
        {
        ...
        if (player.cumQ() >= 4000)
        {
            ...
            if (!minervaRomanced())
            {
                outputText("  \"<i>Oohhh...I'm so... so big... It's almost like you've gotten me pregnant.</i>\"");
            }
            else
            {
                outputText("  \"<i>Oohhh... I'm so... so big... cumming so much, I didn't know you were so desperate to get me pregnant.</i>\"");
            }
        }
        //Low-to-moderate:
        if (player.cumQ() >= 500)
    ...
    private function goDownOnAHermAndLoveItYouDirtySlutYou() : Void
    {
        ...
        //Dragon/Demon tongue
        if (player.tongue.type == Tongue.DEMONIC || player.tongue.type == Tongue.DRACONIC)
        {
    ...
    public function letMinervaSuckYouOff() : Void
    {
        ...
        if (flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] != null)
        {
    ...
    public function pcGetsEatenOutByMinerva() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText(", and, perhaps, your more male parts while she's down there");
        }
        outputText(".");
        ...
        if (player.hasBalls())
        {
            outputText(", one of her hands lifting your [balls] up before giving each of the fertile nuts a kiss");
        }
        ...
        if (player.hasVagina())
        {
            outputText("Thanks to her handy stimulation, your musky maleness throbs and dilates along your femininity, the thick cockvein feeding your bloating member as your seed rushes out, splattering up and onto Minerva's back, defiling her smooth skin with your fertility.  ");
        }
        ...
        //First time ending:{
            ...
            if (flags[kFLAGS.MINERVA_LEZZES_OUT] == 0)
            {
    ...
    private function loseToMinerva() : Void
    {
        clearOutput();
        //PC loss by physical
        if (player.HP < 1)
    ...
    @:allow(classes.scenes.areas.highMountains)
    private function beatUpDatSharpie() : Void
    {
        clearOutput();
        if (flags[kFLAGS.SPARRABLE_NPCS_TRAINING] == 2)
        {
            if (flags[kFLAGS.MINERVA_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.MINERVA_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.MINERVA_DEFEATS_COUNTER] = 1;
            }
