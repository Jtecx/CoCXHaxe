...
/**
	 ...
	 * @author ...
	 */
class MinotaurMob extends Monster
{
    public var wastedTurn : Bool = false;
    
    private function precumTease() : Void
    {
        var teased : Bool = false;
        var damage : Float = 0;
        var oldLust : Float = player.lust;
        ...
        if (rand(2) == 0)
        {
            ...
            if (rand(5) > 0)
            {
                ...
                if (flags[kFLAGS.MINO_SONS_LVL_UP] > 8)
                {
                    damage = 19 + player.lib;
                }
                else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 6)
                {
                    damage = 16 + player.lib / 5;
                }
                else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 4)
                {
                    damage = 13 + player.lib / 10;
                }
                else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 2)
                {
                    damage = 10 + player.lib / 15;
                }
                else
                {
            ...
            else
            {
                ...
                if (flags[kFLAGS.MINO_SONS_LVL_UP] > 8)
                {
                    damage = 7 + player.lib / 10;
                }
                else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 6)
                {
                    damage = 6 + player.lib / 15;
                }
                else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 4)
                {
                    damage = 5 + player.lib / 20;
                }
                else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 2)
                {
                    damage = 4 + player.lib / 25;
                }
                else
                {
        ...
        if (rand(2) == 0)
        {
            ...
            else
            {
                ...
                if (player.spe / 5 + rand(20) > 20)
                {
                    ...
                    if (flags[kFLAGS.MINO_SONS_LVL_UP] > 8)
                    {
                        damage = 18 + player.lib;
                    }
                    else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 6)
                    {
                        damage = 15 + player.lib / 5;
                    }
                    else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 4)
                    {
                        damage = 12 + player.lib / 10;
                    }
                    else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 2)
                    {
                        damage = 9 + player.lib / 15;
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (flags[kFLAGS.MINO_SONS_LVL_UP] > 8)
                    {
                        damage = 35 + player.lib;
                    }
                    else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 6)
                    {
                        damage = 30 + player.lib / 5;
                    }
                    else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 4)
                    {
                        damage = 25 + player.lib / 10;
                    }
                    else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 2)
                    {
                        damage = 20 + player.lib / 15;
                    }
                    else
                    {
        ...
        if (!teased || rand(3) == 0)
        {
            ...
            if (flags[kFLAGS.MINO_SONS_LVL_UP] > 8)
            {
                damage = 21 + player.lib;
            }
            else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 6)
            {
                damage = 18 + player.lib / 5;
            }
            else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 4)
            {
                damage = 15 + player.lib / 10;
            }
            else if (flags[kFLAGS.MINO_SONS_LVL_UP] > 2)
            {
                damage = 12 + player.lib / 15;
            }
            else
            {
        ...
        if (damage <= 8)
        {
            ...
            {
                outputText("  Still, you're too horny to fight any longer.");
            }
        }
        //(Medium damage taken)
        else if (damage <= 14)
        {
            ...
            if (player.lust > 70)
            {
                outputText(", though you didn't think such a thing was possible");
            }
            else
            {
                outputText(", feeling like two bullets scraping along the inside of your " + player.armorName);
            }
            ...
            if (player.lust >= player.maxOverLust())
            {
                outputText("  Fuck it, they smell so good.  You want, no, NEED more.");
            }
            else
            {
                outputText("  A growing part of you wants to experience that.");
            }
        }
        //(high damage taken)
        else
        {
            ...
            if (player.hasCock())
            {
                outputText("  " + SMultiCockDesc() + " twitches and dribbles its own pre-seed, but it doesn't smell anywhere near as good!");
            }
            ...
            if (player.lust < player.maxOverLust())
            {
                outputText("You pull back from the brink with a start.  It'll take more than a little drugged pre-cum to bring you down!");
            }
            else
            {
                outputText("You sigh and let your tongue loll out.  It wouldn't so bad, would it?");
            }
        }
    }
    ...
    private function minotaurGangGropeAttack() : Void
    {
        ...
        if (player.lust >= Math.round(player.maxLust() * 0.8))
        {
            outputText(", and your pussy is absolutely soaking wet");
        }
        outputText(".");
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (flags[kFLAGS.ETNA_FOLLOWER] == 3)
        {
            ...
            {
                minotaurThisIsSparta();
            }
        }
        else
        {
            wastedTurn = false;
            var select : Float = rand(7);
            if (select <= 2)
            {
                precumTease();
            }
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        if (flags[kFLAGS.ETNA_FOLLOWER] == 3)
        {
            SceneLib.etnaScene.etnaMarriageYes4();
        }
        else
        {
            SceneLib.mountain.minotaurMobScene.victoryMinotaurGang();
        }
    }
    ...
    public function new()
    {
        ...
        var mod1 : Int = as3hx.Compat.parseInt(flags[kFLAGS.MINO_SONS_LVL_UP] + flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] - 3);
        var mod2 : Int = as3hx.Compat.parseInt((flags[kFLAGS.MINO_SONS_LVL_UP] * 6) + flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] - 3);
        this.a = "the ";
        if (flags[kFLAGS.ETNA_FOLLOWER] == 3)
        {
            this.short = "300 jealous minotaurs Ex-es";
        }
        else if (game.flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] < 20)
        {
        ...
        {
            this.long = cast((game.flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE]), Num2Text) + " shaggy beastmen stand around you in a loose circle.  Their postures aren't exactly threatening.  If anything, they seem to be standing protectively around you, as if their presence would somehow shelter you from the rest of the mountain.  All of their features share a brotherly similarity, though there's still a fair bit of differences between your minotaur sons.  One of them is a head above the rest, a massive hulk of muscle so big he seems to dwarf the rest.  In stark contrast, a feminine minitaur keeps his distance in the rear." + ((game.flags[kFLAGS.MINOTAUR_SONS_TRIBE_SIZE] >= 20) ? "  The tribe constantly makes hoots and cat-calls, fully expecting to be fucking you soon." : "");
        }
        this.plural = true;
        this.pronoun1 = "they";
        this.pronoun2 = "them";
        ...
        var bonusHP : Float = 600;
        var lustVuln : Float = 0.45;
        if (flags[kFLAGS.ETNA_FOLLOWER] == 3)
        {
        ...
        this.drop = NO_DROP;
        if (flags[kFLAGS.MINOTAUR_SONS_GROW_COUNTDOWN] > 20 || flags[kFLAGS.ETNA_FOLLOWER] == 3)
        {
        ...
        if (flags[kFLAGS.MINO_SONS_LVL_UP] >= 2)
        {
            this.createPerk(PerkLib.JobBeastWarrior, 0, 0, 0, 0);
        }
        if (flags[kFLAGS.MINO_SONS_LVL_UP] >= 4)
        {
            this.createPerk(PerkLib.JobWarrior, 0, 0, 0, 0);
        }
        if (flags[kFLAGS.MINO_SONS_LVL_UP] >= 6)
        {
        ...
        if (flags[kFLAGS.MINO_SONS_LVL_UP] >= 10)
        {
