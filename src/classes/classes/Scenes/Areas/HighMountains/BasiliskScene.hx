...
class BasiliskScene extends BaseContent
{
    public function new()
    {
        super();
    ...
    public function basiliskGreeting() : Void
    {
        ...
        if (flags[kFLAGS.TIMES_ENCOUNTERED_BASILISK] == 0)
        {
            ...
            //(spd loss)
            player.buff("Basilisk Slow").addStats({
                                        spe : -5
                                    }).withText("Basilisk Slow").combatPermanent();
    ...
    public function defeatBasilisk() : Void
    {
        ...
        {
            outputText("Unable to stand anymore, the basilisk shakily sinks down on one knee, drops its head and looks at the ground, evidently demonstrating submission.");
        }
        //Player Lust victory:
        else
        {
            outputText("No longer able to control its raging erection, the basilisk closes its eyes and sinks to one knee.  It would probably be attempting to signal its submission to you if it weren't furiously masturbating its long, purple cock, which has emerged straining from the creature's genital slit.");
        }
    ...
    private function tongueBasiliskSmex() : Void
    {
        ...
        function dickF() : Void
        {
            ...
            var dickEnd : Void->Void = function() : Void
            {
    ...
    private function defeatBasiliskAndAnal() : Void
    {
        ...
        if (player.cocks[x].cockLength <= 13)
        {
            ...
            if (player.hasBalls())
            {
                outputText(" your [balls] slapping against it,");
            }
        ...
        else
        {
            ...
            if (player.hasBalls())
            {
                outputText("Your [balls] swing heavily underneath your shaft, swelling as your rhythm picks up.  ");
            }
        ...
        if (player.cocks[x].cockLength <= 13)
        {
            outputText("gripping its hips and pushing every inch of yourself in and reaching a glorious, skin tingling peak.");
        }
        //(More than 13 inches:)
        else
        {
            outputText("gripping its hips and pushing as much of your huge, bulging cock into it as you can, its stomach bulging as you reach a glorious, skin tingling peak.");
        }
    ...
    private function defaultBasiliskRape() : Void
    {
        ...
        if (player.hasCock() && (!player.hasVagina() || rand(2) == 0))
        {
            ...
            if (player.hasVagina())
            {
                outputText(" and sticks some of the smaller fingers of its other hand in your " + vaginaDescript(0));
            }
    ...
    private function basiliskHasVagEggStuff() : Void
    {
        ...
        //(egg preg:
        if (player.isPregnant() && (player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS || player.pregnancy2Type == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS))
        {
            outputText("  The basilisk licks your bulging belly hungrily, pushing against and testing for the eggs you are carrying.  Your sensitive cargo shifts around under its hungry attention; you'd squirm, but that is, of course, impossible.");
        }
        //(heat or perk:
        if (player.inHeat || player.hasPerk(PerkLib.Oviposition) || player.hasPerk(PerkLib.BasiliskWomb))
        {
            outputText("  The basilisk licks your belly hungrily, its sticky tongue crawling like a warm tentacle across your sensitive underside.  You'd squirm, but that is impossible.  The creature is making you feel everything it is forcing upon you.");
        }
        outputText("\n\n");
        ...
        {
            outputText("Though it is not girthy, it is a perfect fit for your tight hole, and its long length coupled with your drawn up position has you panting as it touches your deepest, most sensitive depths.");
        }
        //(Loose:
        else
        {
            outputText("At first the sensation is not great, its thin penis quickly lost in your vast, accommodating twat, but then, in between breaths, the basilisk barks out more harsh words. Once again you feel that helpless, red flex in your mind, and suddenly your vagina tightens around it, beginning to eagerly milk the reptile.  You desperately wish it didn't and at the same time are hopelessly glad it did; the sensation of your walls pushing and pulling the long, smooth prick in loving synchronisation is unbearably pleasurable.");
        }
        ...
        if (player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS)
        {
            ...
            player.knockUpForce(PregnancyStore.PREGNANCY_BASILISK, PregnancyStore.INCUBATION_BASILISK - 150);
        }
        if (player.pregnancy2Type == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS)
        {
            outputText("\n\nYour womb gurgles, and you instinctively put a hand on your belly. It seems larger than it usually is, and you feel oddly more tender and motherly than normal.  You shake your head at the thought.  Damn hormones.");
            player.knockUpForce(PregnancyStore.PREGNANCY_BASILISK, PregnancyStore.INCUBATION_BASILISK - 150, 1);
        }
    ...
    public function basiliskBirth(womb : Int = 0) : Void
    {
        ...
        if (player.hasPerk(PerkLib.BasiliskWomb))
        {
            ...
            if (player.hasCock())
            {
                outputText(" and a downpour of hermcum");
            }
            outputText(".");
            ...
            {
                flags[kFLAGS.BENOIT_GENERIC_EGGS] += Math.floor(player.totalFertility() / 5);
            }
            else
            {
        ...
        else
        {
            ...
            if (player.cor < 33)
            {
                outputText("You find yourself shaken by the experience, and deeply disquieted by the thought of the clutch of monsters you have unleashed on this world.  You pick yourself up, rub yourself down and leave, promising yourself fervently you'll be more careful around basilisks in the future.\n\n");
            }
            else if (player.cor < 66)
            {
                outputText("You pick yourself up, rub yourself down and leave.  You feel conflicted about what just happened; on the one hand you feel disquieted about the dozen monsters you just unleashed on this world, on the other you cannot help feel oddly proud of them and yourself.\n\n");
            }
            else
            {
                outputText("With a soft smile, you get up and leave, enjoying the sensation of green slime trickling down your legs.  You cannot wait to get pregnant again, for your stomach to bulge with eggs, to release more delightful creatures into this world which can grow up to fuck you and everyone else in turn, so everyone can enjoy life as much as you do.");
            }
        }
        outputText("\n");
    }
    ...
    private function basiliskBadEnd() : Void
    {
        ...
        if (player.gender == 3)
        {
            outputText("  You moan through your still throat as you feel blood rushing to your groin, your [cocks] stiffening and your " + vaginaDescript(0) + " beginning to drip.");
        }
        if (player.gender == 1)
        {
            outputText("  You moan through your still throat as you feel blood rushing to your groin, your [cocks] stiffening.");
        }
        if (player.gender == 2)
        {
            outputText("  You moan through your still throat as you feel blood rushing to your groin, your " + vaginaDescript(0) + " beginning to drip.");
        }
    ...
    private function basiliskAdvantageHarpy() : Void
    {
        ...
        if (player.hasCock() && (!sceneHunter.uniHerms || rand(3) > 0))
        {
            ...
            if (player.isBiped())
            {
                outputText("You teeter horribly on your frozen heels for a moment and then fall onto your back like a collapsing statue.  ");
            }
            else
            {
                outputText("You feel yourself slowly and horribly lose your balance before flopping onto your side like a collapsing statue.  ");
            }
            ...
            function multiF() : Void
            {
                ...
                {
                    outputText(", crowded out by the bodies of the others despite her transparent desire for the unoccupied man meat flopping against them,");
                }
    ...
    private function basiliskAdvantageGoblin() : Void
    {
        ...
        else
        {
            ...
            {
                outputText("  You didn't get the wet end, and you groan as you feel the thing begin to puff up as it reacts with your eager juices until it is pushing almost painfully against your inner walls.");
            }
            else
            {
                outputText("  You didn't get the wet end, and you groan as you feel the thing begin to puff up as it reacts with your eager juices until it fills your well-used passage almost perfectly.");
            }
    ...
    private function basiliskAdvantageMinotaur_part2() : Void
    {
        ...
        if (player.gender == 1)
        {
            outputText("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark, and you moan as you spontaneously ejaculate, ropes of your own cum spattering on the ground and your own nerveless [feet].");
        }
        else if (player.gender == 2)
        {
            outputText("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark, and you moan as you spontaneously orgasm, your juices spattering your thighs and calves.");
        }
        else if (player.gender == 3)
        {
            outputText("The sensual high it has already instilled in you amplifies by tenfold; the warm, oozing fluid finds its mark, and you moan as you spontaneously orgasm, ropes of your own jizz spattering on the ground and girl cum dripping down your thighs.");
        }
        else
        {
            outputText("The sensual high it has already instilled in you amplifies by tenfold as the warm, oozing fluid finds its mark, and you moan as you spontaneously orgasm, your anus helplessly milking the creature for all it can get.");
        }
        outputText("\n\n");
        ...
        //(Addict:
        if (flags[kFLAGS.MINOTAUR_CUM_ADDICTION_STATE] > 0 || player.hasPerk(PerkLib.MinotaurCumAddict) || player.hasPerk(PerkLib.LactaBovineImmunity))
        {
            outputText("  You're incredibly frustrated that you can't clench yourself and hold the magical substance deep inside you, so you can savor the wonderful, soft elation it blossoms inside of you for as long as you can.");
        }
        outputText("\n\n");
    ...
    private function driderPCEggLaysBasilisk() : Void
    {
        ...
        if (silly())
        {
            outputText("Pinning him between a rock and a hardening place, ");
        }
        ...
        if (player.hasVagina())
        {
            outputText("already-dripping pussy");
        }
        if (player.gender == 3)
        {
            outputText(" and ");
        ...
        {
            outputText("  You watch with idle amusement as the scent of your excitement perks up the basilisk's reptilian member, rising until it reaches its impressive full length.  <i>Maybe some other time</i>, you think to yourself, as you set about the business of unloading your overfull abdomen.");
        }
        ...
        if (player.hasCock())
        {
            outputText("wrap tightly around your waiting prick");
        }
        else
        {
            outputText("burrow deep into your needy slit");
        }
        ...
        if (player.cor >= 50)
        {
            outputText(" before heading off towards your camp. The harpies will probably free him from your webs... eventually.");
        }
        else
        {
            outputText(".  You reach up and slice him free from the webbing, carefully placing him on the ground below.  The blindfold stays, though.  You're kind, not stupid.  With your good deed for the day complete, you gather yourself up and head back to camp.");
        }
        player.dumpEggs();
        player.sexReward("Default", "Default", true, false);
        cleanupAfterCombat();
    ...
    private function layBeeEggsInABasilisk() : Void
    {
        ...
        {
            outputText("buzz lightly over to the basilisk's writhing body");
        }
        else
        {
            outputText("saunter arrogantly over to the basilisk's crumpled up figure");
        }
        ...
        if (player.fertilizedEggs())
        {
            outputText("fertilized offspring");
        }
        else
        {
            outputText("burdensome load");
        }
        outputText(".");
        ...
        if (player.isTaur() || player.gender == 0)
        {
            ...
            if (player.gender == 0)
            {
                outputText("no needy cock or cunt demanding a caress - except the basilisk's, and you care not a bit about that one.");
            }
            else if (player.isTaur())
            {
                outputText("even if you wanted to reach your genitals, your arms resolutely refuse to bend that way, so you focus on the slippery, silky feelings travelling along your ovipositor.");
            }
            sharedEnd();
        }
        else
        ...
        function dickF() : Void
        {
            ...
            if (player.hasBalls())
            {
                outputText("and inside your balls ");
            }
