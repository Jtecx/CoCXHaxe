 ...
 */
class Cockatrice extends Monster
{
    ...
    private function sandAttack() : Void
    {
        ...
        if (player.spe / 5 + rand(20) < 28 + player.newGamePlusMod() * 5)
    ...
    {
        var actionChoices : WeightedAction = new WeightedAction().add(sandAttack, 40).add(cockaTease, 40).add(tailSwipe, 30).add(eAttack, 20);
    ...
    public function new()
    {
        ...
        this.drop =new WeightedDrop().add(consumables.REPTLUM, 35).add(consumables.GLDSEED, 35).add(null, 10)  //.add(consumables.TOTRICE, 20) //same.  ;
