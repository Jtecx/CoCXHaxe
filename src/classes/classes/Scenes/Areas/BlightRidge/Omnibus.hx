...
class Omnibus extends AbstractSuccubus
{
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (player.hasStatusEffect(StatusEffects.RiverDungeonA))
        {
            SceneLib.dungeons.riverdungeon.defeatedByMistOmnibus();
        }
        else if (inDungeon)
        {
    ...
    public function new()
    {
        ...
        if (player.hasStatusEffect(StatusEffects.RiverDungeonA))
        {
            this.drop = new WeightedDrop().add(weapons.WHIP, 2).add(consumables.LETHITE, 5).add(useables.PCSHARD, 5);
        }
        else
        {
            this.drop = new WeightedDrop().add(consumables.BIMBOLQ, 1).add(consumables.BROBREW, 1).add(weapons.WHIP, 2).add(consumables.SUCMILK, 12).add(consumables.INCUBID, 12);
        }
