...
class Incubus extends Monster
{
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (inDungeon)
        {
            SceneLib.dungeons.ebonlabyrinth.defeatedByStrayDemon();
        }
        else
        {
            SceneLib.dungeons.factory.doLossIncubus(false);
        }
    }
    ...
    private function spoogeAttack2() : Void
    {
        ...
        switch (rand(3))
        {
            case 0:  //Face  
                ...
                {
                    player.addStatusValue(StatusEffects.DemonSeed, 1, 6);
                }
                player.slimeFeed();
            case 1:  //Chest  
    ...
    public function new()
    {
        ...
        this.drop = new WeightedDrop().add(consumables.BROBREW, 1).add(consumables.INCUBID, 12);
