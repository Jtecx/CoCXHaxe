...
class WaspGirl extends Monster
{
    ...
    public function waspBarrageOfDarts() : Void
    {
        ...
        if (player.buff("wasp paralyze venom").isPresent())
        {
            ...
            player.buff("wasp paralyze venom").addStats({
                                        str : -2,
                                        spe : -2,
                                        tou : -4
                                    }).withText("wasp paralyze venom").combatPermanent();
        }
        else
        {
        ...
        var damage : Int = 0;
        damage += as3hx.Compat.parseInt(str + rand(20));
    ...
    //Blind dodge change
    {
        ...
        //Determine if dodged!
        if (player.spe - spe > 0 && as3hx.Compat.parseInt(Math.random() * (((player.spe - spe) / 4) + 160)) > 160)
        {
            if (player.spe - spe < 16)
            {
                outputText("You narrowly avoid " + a + short + "'s stinger!");
            }
            if (player.spe - spe >= 16 && player.spe - spe < 40)
            {
                outputText("You dodge " + a + short + "'s stinger with superior quickness!");
            }
            if (player.spe - spe >= 40)
            {
