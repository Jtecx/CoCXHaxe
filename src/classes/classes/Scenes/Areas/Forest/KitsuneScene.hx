...
class KitsuneScene extends BaseContent
{
    public function new()
    {
        super();
    ...
    private function kitsuneSprite() : Void
    {
        ...
        {
            spriteSelect(SpriteDb.s_kitsune_blonde);
        }
        else if (monster.hairColor == "black")
        {
            spriteSelect(SpriteDb.s_kitsune_black);
        }
        else
        {
            spriteSelect(SpriteDb.s_kitsune_red);
        }
    }
    ...
    public function winKitsuneImpFight() : Void
    {
        ...
        if ((player.lust < 70) || (player.inte >= 40))
        {
            ...
            if (monster.hairColor == "blonde")
            {
                outputText("back-length, flaxen");
            }
            else if (monster.hairColor == "black")
            {
                outputText("ass-length, raven");
            }
            else
            {
                outputText("shoulder-length, crimson");
            }
        ...
        {
            //With Religious BG:if (player.hasPerk(PerkLib.HistoryReligious) || player.hasPerk(PerkLib.PastLifeReligious))
            {
    ...
    //[Follow] (C)
    private function followTheWillOWisp(firstTime : Bool = false) : Void
    {
        clearOutput();
        if (!firstTime)
        {
        ...
        else
        {
            ...
            if (!CoC.instance.inCombat)
            {
                simpleChoices("Fight", fightSomeKitsunes, "Talk", talkAfterResistingKitsunellusion, "", null, "", null, "", null);
            }
            else
            {
                simpleChoices("Fight", fightSomeKitsunes, "Talk", talkAfterResistingKitsunellusion, "", null, "", null, "", null);
    ...
    //[Leave] (C)
    private function leaveKitsune(talked : Bool = false) : Void
    {
        clearOutput();
        //if PC was Talking to the kitsune
        if (talked)
        {
            outputText("Nervously, you thank her for her generous offer, but decline, turning to leave.");
        }
        //if PC defeated her in combat
        else
        {
            outputText("You turn to leave, ready to put some distance between yourself and the fallen trickster.");
        }
        //MERGE:
    ...
    //Illusory Mansion (C)
    private function mansion(willing : Bool, firstTime : Bool = false) : Void
    {
        clearOutput();
        kitsuneSprite();
        if (willing)
        {
            if (!firstTime)
            {
                outputText("Curiosity and lust get the better of you, and you decide to throw caution to the wind and take her up on her offer.  If her sisters are any bit as stunning as she is, then you know you're in for a real treat.  She seems pleased with your decision, teasing the underside of your chin with one of her tails and beckoning you to follow her.\n\n");
            }
            //next
        ...
        if (flags[kFLAGS.MANSION_VISITED] > 0)
        {
            outputText("the trio of familiar kitsune sisters.");
        }
        else
        {
        ...
        else if (flags[kFLAGS.MANSION_VISITED] > 0)
        {
    ...
    private function mansionSceneRouter(willing : Bool) : Void
    {
        clearOutput();
        ...
        else
        {
            ...
            if (player.hasVagina())
            {
                sceneHunter.selectGender(dickF, vagOrAssF);
            }
            else
            {
                sceneHunter.selectGender(dickF, null, vagOrAssF);
            }
        }
    }
    
    //Formerly maleOrHerm()
    private function kitsuneMaleOrHermMansion(willing : Bool) : Void
    {
        clearOutput();
    ...
    // end function
    //fomerly letHer()
    private function kitSuneLetHerMansion(willing : Bool = true) : Void
    {
        clearOutput();
        ...
        if (player.gender == 1)
        {
            outputText("hindquarters");
        }
        else
        {
            outputText("warm folds");
        }
        ...
        {
            outputText("circling your navel with her index finger and hugging her perky breasts into your back");
        }
        ...
        {
            outputText("Numerous streams spurt into her, sloppily smearing her tunnel with your thick load.  Her inner muscles squeeze you greedily, massaging your shaft to draw your seed " + (((player.biggestCockArea() > 80)) ? "toward her convulsing womb." : "deeper into her convulsing bowels.") + "  A deep crimson blush spreads across her cheeks as a thick spray of girlcum erupts from her pussy, splattering over the black-haired girl's face, and she opens her mouth to catch it all, pressing her plump lips against her sister's quivering cunt and drinking deeply as she hums a satisfied melody into her clitoris.");
        }
    ...
    //Formerly shoveHer()
    private function kitsuneShoveHerMansion(willing : Bool = true) : Void
    {
        clearOutput();
    ...
    private function kitsuneFemaleOrGenderless(willing : Bool) : Void
    {
        clearOutput();
    ...
    private function kitsunesGenderlessLetHer(willing : Bool) : Void
    {
        clearOutput();
    ...
    //fomerly shoveHer()
    private function kitsunesGenderlessShoveHer(willing : Bool) : Void
    {
        clearOutput();
    ...
    private function genderlessKitsuneStillHungry(willing : Bool) : Void
    {
        clearOutput();
    ...
    {
        var a : Bool = player.isAlraune();  //used in text selection.. still dumb check but better than before IMO  
        clearOutput();
    ...
    private function outro(tentacles : Bool = false, dick : Bool = true) : Void
    {
        clearOutput();
        fatigue(15);
        ...
        if (tentacles)
        {
            outputText(" - at the very least, your groin seems to be back to its usual configuration");
        }
        ...
        if (player.cumQ() < 15000 || (!player.hasCock() && player.lactationQ() < 15000))
        {
            ...
            {
                outputText("milk ");
                if (player.lactationQ() < 750)
                {
                    player.boostLactation(3);
                }
                if (player.lactationQ() < 3000)
                {
                    player.boostLactation(2);
                }
                if (player.lactationQ() < 15000)
                {
    ...
    private function fightSomeKitsunes() : Void
    {
        ...
        if (monster.hairColor == "blonde")
        {
            outputText("\"<i>Ah, so we're going to play like that, are we?</i>\"  she says with a grin, making a small clawing motion in the air.  \"<i>Very well my darling, if that's how you want it, but I won't be taking it easy on you just because you're cute~.</i>\"");
        }
        //Black Hair
        else if (monster.hairColor == "black")
        {
            outputText("\"<i>Oh, dear...  are you sure you can't just spare the tiniest morsel?  I'm so hungry...</i>\"\n\nShe licks her lips to accentuate this, rubbing her stomach.  You however, have no intention of being made a meal, or whatever it is she intends to do with you.\n\n\"<i>Well, I guess it can't be helped,</i>\" she says, pursing her lips impishly.");
        }
        //Redhead
        else
        {
    ...
    private function fluffCount() : String
    {
        ...
        else if (flags[kFLAGS.MET_KITSUNES] >= 2)
        {
            str += "five";
        }
        else
    ...
    public function loseToKitsunes() : Void
    {
        ...
        //old behavior{
            ...
            {
                scene.push(loseToBlackHairLatexWhileMilky);
                scene.push(loseToBlackHairLatexWhileMilky);
                scene.push(loseToBlackHairLatexWhileMilky);
                scene.push(loseToBlackHairLatexWhileMilky);
            ...
            {
                scene.push(loseToKitsunesWithBallsAndHighCumQ);
                scene.push(loseToKitsunesWithBallsAndHighCumQ);
    ...
    private function getRapedByRedHeadFutaKitsune() : Void
    {
        ...
        if (player.hasVagina())
        {
            ...
            {
                outputText("a combination of pain and ");
            }
    ...
    //[WIN FIGHT]
    //Damage victory
    public function defeatTheKitsunes(display : Bool = true) : Void
    {
        if (display)
        {
            clearOutput();
        ...
        {
            if (flags[kFLAGS.KITSUNES_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.KITSUNES_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.KITSUNES_DEFEATS_COUNTER] = 1;
            }
        ...
        {
            addButtonDisabled(1, "FuckAss", "Special alternative scene available.");
        }
        ...
        if (monster.hairColor == "red")
        {
            if (flags[kFLAGS.REDHEAD_IS_FUTA] != null)
            {
    ...
    private function fuckDraftBlond() : Void
    {
        ...
        if (player.inRut)
        {
            outputText("The sweet musk of her voluminous fem-jizz fills your mind with a lustful haze, higher-order thought processes gradually shutting down as animalistic mating instinct begins to take over.  ");
        }
        ...
        {
            outputText(player.legs());
        }
        if (silly())
        {
            outputText(" with a heat you could roast marshmallows over.");
        }
        else
        {
            outputText(" with a heat that sends blood rushing to your groin.");
        }
        ...
        if (player.inRut)
        {
            ...
            {
                outputText(" heedless of whether she can handle it or not.  Thankfully, you needn't worry - her cunt eagerly swallows your cock whole with room to spare, somehow");
            }
        }
        else
        {
        ...
        if (player.inRut)
        {
            outputText("  Vivid fantasies of stuffing her pussy full of your virile semen race through your mind, unable to think of anything else.  Come whatever may, you know you MUST impregnate her - there's simply no other option.");
        }
        ...
        else
        {
            ...
            if (player.hasKnot(x))
            {
                outputText("Her belly balloons to a ludicrous size, swelling more by the second as you continue to pump her full.  Your knot blocks the only escape, forcing her to endure as her abdomen swells with your seed, sloshing and jiggling heavily.");
            }
            else
            {
                outputText("The sheer volume is more than she can handle, and when her now heavily pregnant-looking belly begins to press against the ground, pressurized streams start to pour out of her tight hole, splattering against your [hips] and [fullChest].");
            }
    ...
    private function lactaidDoseAKitSune() : Void
    {
        ...
        else
        {
            ...
            if (player.cumQ() >= 2500)
            {
                outputText("  It seems as though your climax goes on forever, fresh streamers of cum flying everywhere until the milky white puddle surrounding her is equal parts milk and seed.");
            }
        }
    ...
    private function feederTheKitsunes() : Void
    {
        ...
        if (monster.hairColor == "black")
        {
            outputText("lustrous black ");
        }
        else
        {
            outputText(monster.hairColor);
        }
        outputText("hair.\n\n");
    ...
    private function getABJFromAFoxGirl() : Void
    {
        ...
        {
            outputText("her lips squeezing sensually around the " + (((player.hasSheath())) ? "sheath" : "base") + " each time she goes down you.  Moaning intensely, she closes her eyes and rolls your " + cockDescript(x) + " back and forth in her mouth, swirling her tongue sensually along it.\n\n");
        }
    ...
    public function kitsuneShrine() : Void
    {
        ...
        {
            outputText("You find your way to the abandoned kitsune shrine again.  The place is full of rotten timber, but it has a bookcase stuffed with well-maintained tomes.  The remains of a camp are in here as well, though the owner is curiously absent.  Judging by the layer of dust on everything, whoever lived here hasn't been around in quite some time.  You're sure they wouldn't mind if you helped yourself to some of those books - you might just learn a thing or two.\n\n");
            if (flags[kFLAGS.KITSUNE_SHRINE_UNLOCKED] > 0 && flags[kFLAGS.AYANE_FOLLOWER] < 2)
            {
                outputText("Ayane is meditating on a rock next to the shrine.\n\n");
            }
            if (rand(3) == 0)
            {
                outputText("A few kitsunes seem to be here to meditate today.\n\n");
            }
            outputText("");
        }
        ...
        {
            addButton(1, "Meditate", meditateLikeAKitsuneEhQuestionMark);
        }
        if ((player.hasItem(useables.GLDSTAT) || flags[kFLAGS.TOOK_KITSUNE_STATUE] == 0) && flags[kFLAGS.KITSUNE_SHRINE_UNLOCKED] < 1)
        {
            addButton(2, "Statue", stealAStatue);
        }
        if (player.hasPerk(PerkLib.StarSphereMastery) && player.perkv1(PerkLib.StarSphereMastery) < SphereMastery && player.gems >= 1000)
        {
            addButton(3, "Offering", offeringToTaoth);
        }
        if (flags[kFLAGS.KITSUNE_SHRINE_UNLOCKED] > 0 && flags[kFLAGS.AYANE_FOLLOWER] < 2)
        {
            addButton(5, "Ayane", SceneLib.ayaneFollower.ayaneShop);
        }
        if (flags[kFLAGS.AYANE_FOLLOWER] == 1)
        {
            addButton(6, "Servant", AyaneServant);
        }
        if (player.hasPerk(PerkLib.CorruptedNinetails) && player.inte >= 100 && player.wis >= 100 && player.cor >= 50 && flags[kFLAGS.KITSUNE_SHRINE_UNLOCKED] > 0 && flags[kFLAGS.AYANE_FOLLOWER] < 2)
        {
            addButton(7, "Slave", AyaneSlave);
        }
    ...
    public function meditateAtKitsuneShrine() : Int
    {
        ...
        {
            var notANineTail : Bool = !player.hasPerk(PerkLib.CorruptedNinetails)
            ...
            if (flags[kFLAGS.KITSUNE_SHRINE_UNLOCKED] > 0 && player.tailCount >= 2 && !player.hasPerk(PerkLib.StarSphereMastery))
            {
                ...
                player.statStore.replaceBuffObject({
                            ...
                            text : "Kitsune shrine Meditation",
                            rate : Buff.RATE_DAYS,
                            tick : 7
                        });
            ...
            else if (player.tailCount == 8 && player.level >= 42 && player.inte >= 120 && player.wis >= 120 && notANineTail)
            {
                ...
                else
                {
                    ...
                    if (fur != null && !InCollection(player.furColor, KitsuneRace.ElderKitsuneColors))
    ...
    //clearOutput();
    {
        ...
        if (player.racialScore(Races.KITSUNE) >= 8 && (player.tailType == Tail.FOX && player.tailCount == 9) && !player.hasPerk(PerkLib.StarSphereMastery))
        
        //Should check for if player is kitsune, has 9 tails, and without starsphere{
