...
class TamaniScene extends BaseContent implements TimeAwareInterface
{
...
*/
    ...
    private function tamaniChest() : String
    {
        var descript : String;
        var _sw4_ = (flags[kFLAGS.TAMANI_TIMES_IMPREGNATED]);        

        switch (_sw4_)
        {
            case -1, 0:descript = ((rand(2) == 0) ? "ample " : "handful of ");
            case 1, 2, 3:descript = ((rand(2) == 0) ? "large " : "jiggling ");
            case 4, 5, 6:descript = ((rand(2) == 0) ? "wobbling " : "impressive ");
            case 7, 8, 9, 10:descript = ((rand(2) == 0) ? "over-proportioned " : "super-sized ");
            default:descript = ((rand(2) == 0) ? "massive " : "gigantic ");
        }
        switch (rand(11))
        {
            case 0, 1, 2:return descript + "tits";
    ...
    public function tamaniDefeated(hpVictory : Bool) : Void
    {
        clearOutput();
        if (hpVictory)
        {
        ...
        {
            if (flags[kFLAGS.TAMANI_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.TAMANI_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.TAMANI_DEFEATS_COUNTER] = 1;
            }
            //level up
    ...
    private function tamaniFemaleYes() : Void
    {
        ...
        {
            outputText("Getting cock-slapped would've been kind of hot...\n\n");
        }
        ...
        if (player.hasCock())
        {
            outputText("Oh, and don't forget to come back and use your other sex next time when you're done playing with my toy.</i>\"\n\n");
        }
        else
        {
            outputText("Oh, and remember — Tamani owns all the cocks around here, so if you ever grow one, come pay your dues!</i>\"\n\n");
        }
    ...
    private function tamaniFemaleNo() : Void
    {
        ...
        {
            outputText("You hope she misses a few.");
        }
        endEncounter();
    }
    //[Encounter Tamani – HAZ COCK]
    ...
    public function tamaniFirstTimeConsentual() : Void
    {
        ...
        function scene(x : Int) : Void
        {
            ...
            if (player.hasBalls())
            {
                outputText("Your [balls] swell with seed, spurring your desire to new heights.");
            }
            else
            {
                outputText("Something inside you swells up with seed, spurring your desire to new heights.");
            }
            ...
            if (player.cockArea(x) <= 90)
            {
                ...
                if (player.hasBalls())
                {
                    outputText(" and drips from your " + ballsDescriptLight());
                }
                ...
                {
                    outputText(" and watching her belly bulge from your size");
                }
                ...
                if (player.cor > 66)
                {
                    outputText("  Your [cock " + x1 + "] twitches at the thought, ready and wanting more.");
                }
            }
            //(TOO BIG)
            else
            {
                ...
                {
                    outputText("licks all over your balls");
                }
                else if (player.hasVagina())
                {
                    outputText("sneaks between your folds to tease your now-hardening clit");
                }
                else
                {
                    outputText("licks the sensitive [color] [skin.type] of your inner thighs");
                }
                outputText("\n\n");
                ...
                if (player.hasBalls() && player.hoursSinceCum > 200)
                {
                    outputText(", visibly draining your [balls] down to their normal size");
                }
    ...
    private function tamaniFirstTimeRefusal() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText("  Seeing her suspicion, you also point out that you're not her rival in any way, and you don't need more cocks than your own.");
        }
    ...
    private function tamaniMaleRepeatEncounter() : Void
    {
        ...
        if (pregnancy.isPregnant || flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS] > 0)
        {
            outputText("While exploring, you're startled by the feeling of tiny hands stroking the insides of your thighs.  You look down and find Tamani there, grinning wolfishly,  \"<i>Ready for another fuck, big " + player.mf("boy", "girl") + "?</i>\"\n\n");
        }
        //(ELSE)
        else
        {
            outputText("While exploring, you're startled by the feeling of tiny hands stroking the insides of your thighs.  You look down and find Tamani the goblin there, grinning with desire, \"<i>Ready to stuff me with cum?  I'm not taking no for an answer this time.</i>\"\n\n");
        }
    ...
    //[let her]
    @:allow(classes.scenes.areas.forest)
    private function tamaniSexLetHer() : Void
    {
        spriteSelect(SpriteDb.s_tamani);
        tamaniKnockUp();
        clearOutput();
        if (!player.isTaur())
        {
            sceneHunter.print("Failed check/fork: taur lower body.");
        }
        //[lost combat]
        ...
        //Taurs{
            ...
            if (player.isTaur())
            {
                if (player.HP < 1)
                {
                    outputText("You stumble, legs weaving drunkenly as you try to keep your feet.  ");
                }
                else
                {
                    outputText("You stumble, legs weaving drunkenly as " + sMultiCockDesc() + " ache and drool with need.  ");
                }
            ...
            else
            {
                if (player.HP < 1)
                {
                    outputText("You collapse, unable to fight back any longer.  For her part, Tamani seems disappointed with the outcome, but approaches your prone form in spite of your sorry appearance.");
                }
                else
                {
                    outputText("You tear off your clothes in a frenzy of lust and stagger towards Tamani, openly fondling your groin.   She smirks and pushes you hard, throwing you off balance and onto your back.");
                }
                outputText("  She wiggles her " + tamaniChest() + ", and despite your efforts to control your body, ");
                if (player.cockTotal() == 1)
                {
                    outputText("an erection emerges");
                }
                else
                {
                    outputText("erections sprout up like weeds");
                }
        ...
        if (player.isTaur())
        {
            sceneHunter.callFitNofit(taur, 18, "length");
        }
        else
        {
            sceneHunter.callFitNofit(others, 90, "area");
        }
        ...
        var taurFit : Int->Void = function(x : Int) : Void
        {
            var x1 : Int = as3hx.Compat.parseInt(x + 1);
            ...
            if (player.balls == 0)
            {
                outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics.\n\n");
            }
            else
            {
                outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics, before the goblin in question drives all thoughts out of your head by mashing her soft soles into your [balls].\n\n");
            }
        ...
        var taurNofit : Int->Void = function(x : Int) : Void
        {
            var x1 : Int = as3hx.Compat.parseInt(x + 1);
            ...
            {
                outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics, before the goblin in question drives all thoughts out of your head by beginning to mash her soft soles into your [balls].\n\n");
            }
            ...
            else
            {
                ...
                if (player.hasBalls())
                {
                    outputText("Your [balls] ache and you feel very thirsty indeed.");
                }
                else
                {
                    outputText("You feel very thirsty indeed.");
                }
        ...
        var taurEnd : Void->Void = function() : Void
        {
        ...
        var others : Int->Void = function(x : Int) : Void
        {
        ...
        var othersFit : Int->Void = function(x : Int) : Void
        {
            var x1 : Int = as3hx.Compat.parseInt(x + 1);
            ...
            {
                outputText("Her body visibly stretches around you, and you silently thank whatever gods or demons adapted goblins to be able to fulfill this role.  ");
            }
            ...
            {
                outputText("Your [allbreasts] bounce and jiggle obscenely as the goblin twists and pulls, abusing your nipples.");
            }
            else
            {
                outputText("Your " + nippleDescript(0) + "s burn with pain as the goblin twists and pulls on them, abusing them.  Pleasure and pain mix together, overwhelming you, reducing your world to little more than the slap of genital against genital and the painful torture happening to your chest.\n\n");
            }
            ...
            if (player.hasBalls())
            {
                outputText("Your [balls] twitch powerfully, venting their contents, releasing a jet of hot cum into the goblin.");
            }
            else
            {
                outputText("Your body's internal muscles twitch, pushing a jet of hot cum into the goblin.");
            }
            ...
            if (player.cumQ() >= 250)
            {
                outputText("  After a while you can feel the pressure building inside her, starting to give her a bit of a belly.  Tamani rubs it and begs, \"<i>More!</i>\"");
            }
            if (player.cumQ() >= 500)
            {
                outputText("  Your body fulfills her requests and more, stuffing her until she's positively pregnant and your spunk is squirting from her entrance with each wave of sticky spooge.");
            }
            else
            {
                outputText("  Sadly, you only manage a few more spurts before your orgasm ends.");
            }
            ...
            if (player.cumQ() < 250)
            {
                outputText(".  I really enjoy our fucks " + player.mf("stud", "hun") + ", and I can't wait to 'bump' into you again.");
            }
            else
            {
                ...
                if (player.hasBalls())
                {
                    outputText(ballsDescriptLight() + " fill back up with cum, ok?");
                }
                else
                {
                    outputText(cockDescript(0) + " fills back up with cum, ok?");
                }
            }
            outputText("</i>\"\n\n");
        ...
        var othersNofit : Int->Void = function(x : Int) : Void
        {
            var x1 : Int = as3hx.Compat.parseInt(x + 1);
            ...
            if (player.hasBalls())
            {
                outputText("lifts up your " + ballsDescriptLight());
            }
            else
            {
                outputText("leans forwards, looking between your legs");
            }
            ...
            if (player.cumQ() >= 250)
            {
                ...
                if (player.cumQ() >= 500)
                {
                    outputText(" as you keep cumming, splattering her tits and face, soaking the little goblin slut");
                }
                outputText(".  ");
            }
        ...
        var othersEnd : Void->Void = function() : Void
        {
            if (CoC.instance.inCombat)
            {
    ...
    @:allow(classes.scenes.areas.forest)
    private function tamaniSexLost() : Void
    {
        spriteSelect(SpriteDb.s_tamani);
        tamaniKnockUp();
        clearOutput();
        ...
        if (player.HP < 1)
        {
            outputText("lie there, unable to fight back.  ");
        }
        else
        {
            outputText("touch yourself, giving in to the tiny goblin's desire to breed.  ");
        }
        ...
        {
            outputText("Impossibly, despite your titanic girth, the ring somehow expands to squeeze onto you, though it's painfully tight.\n\n");
        }
        else
        {
            outputText("Somehow the ring fits onto you perfectly, as if it was made exactly for your [cock], though it remains painfully tight.\n\n");
        }
        ...
        {
            outputText("but can't manage to see your bloated prick past the wall of titflesh.\n\n");
        }
        else
        {
            outputText("at your bloated prick, looking almost overinflated from all the blood that tight ring has forced into it.\n\n");
        }
        ...
        if (player.hasBalls())
        {
            outputText("cups your [balls] tightly");
        }
        else
        {
            outputText("caresses you around the ring");
        }
        ...
        if (player.hasBalls())
        {
            outputText("Your balls are getting VERY swollen and tender.");
        }
        else
        {
            outputText("Your body aches inside, feeling bloated and ready to pop.");
        }
        if (player.cor < 50)
        {
            outputText("  You have no choice, you'll say what she wants you to say.\n\n");
        }
        else
        {
            outputText("  Being denied like this was kind of fun, but you REALLY need to get off now, so you'll say what she wants you to say.\n\n");
        }
        ...
        if (player.cor > 50)
        {
            outputText("blushing hotly and doing your best to look like a submissive toy.\n\n");
        }
        else
        {
            outputText("blushing in shame and embarrassment, but meaning every word.\n\n");
        }
        ...
        if (player.cumQ() >= 250)
        {
            outputText("  The bottle quickly overflows, but Tamani happily sets it aside and slides her own cunt under you, letting you pump the rest of your seed into her fertile womb.");
        }
        if (player.cumQ() >= 500)
        {
            outputText("  She coos happily when you still manage to pump enough jism into her to bloat her belly, but even with your impressive fertility, the orgasm has to end.");
        }
        outputText("\n\n");
    ...
    @:allow(classes.scenes.areas.forest)
    private function tamaniSexWon() : Void
    {
        tamaniKnockUp();
        clearOutput();
        ...
        function fitF() : Void
        {
            ...
            if (player.cockTotal() == 1)
            {
                outputText("pussy squeezing tightly as she cums hard.\n\n");
            }
            else
            {
                outputText("holes squeezing tightly as she cums hard.\n\n");
            }
            ...
            if (player.cockTotal() > 1)
            {
                ...
                if (player.cumQ() >= 250)
                {
                    outputText("  She pants, turning her head in the mud to watch as her belly visibly inflates, stuffed totally full of cum. ");
                }
                if (player.cumQ() >= 500)
                {
                    outputText("  In no time at all a river of jism pours from her, pooling below as you overwhelm her body's capacity to store spunk.");
                }
            }
            else
            {
            ...
            if (player.hasBalls())
            {
                outputText("sexy balls have ");
            }
            else
            {
                outputText("sweet cock has ");
            }
        ...
        var nofitF : Void->Void = function() : Void
        {
            ...
            if (player.cockTotal() > 1)
            {
                outputText("any of your massive members");
            }
            else
            {
                outputText("your massive member");
            }
            ...
            if (player.cumQ() >= 250)
            {
                outputText("  You continue working your dick with her supple feet, watching your dripping spooge froth and bubble as you continue to pump more onto the sassy wench.");
            }
            if (player.cumQ() >= 500)
            {
                outputText("  She sputters, blowing some of the caked up semen off her face so she can breathe.  The slut gathers up the goopy mess as you continue to paint her, alternatively devouring it with her mouth and shoveling drippy handfuls into her eager cunt.");
            }
            outputText("\n\n");
    ...
    private function tamaniPregnantFuck() : Void
    {
        ...
        //(Very large:
        if (flags[kFLAGS.TAMANI_TIMES_IMPREGNATED] > 6)
        {
            outputText("  They're almost too big for you to handle; they probably make it hard for the poor girl to walk.");
        }
        outputText("\n\n");
        ...
        function scene(x : Int) : Void
        {
            ...
            else
            {
                ...
                if (player.hasBalls())
                {
                    outputText("massaging your balls with her feet");
                }
                else
                {
                    outputText("jacking the base of your [cock " + x1 + "] with her feet");
                }
    ...
    private function tamaniPoopsOutBabies() : Void
    {
        ...
        if (flags[kFLAGS.TAMANI_PREGNANCY_COUNT] > 2)
        {
            ...
            if (flags[kFLAGS.TAMANI_PREGNANCY_COUNT] > 3)
            {
                ...
                {
                    outputText(", and more than you can count");
                }
            }
            outputText(".");
        }
    ...
    @:allow(classes.scenes.areas.forest)
    private function tamaniKnockUp() : Void
    {
        ...
        {
            flags[kFLAGS.TAMANI_PREGNANCY_COUNT] += 3;
        }
        if (cum >= 50 && rand(2) == 0)
        {
            flags[kFLAGS.TAMANI_PREGNANCY_COUNT]++;
        }
        if (cum >= 100 && rand(2) == 0)
        {
            flags[kFLAGS.TAMANI_PREGNANCY_COUNT]++;
        ...
        switch (_sw5_)
        {
            case 1:
                ...
                else
                {
                    ...
                    switch (_sw6_)
                    {
                        case 2:tamaniPregnantEncounter();  //She's moderately pregnant  
                        case 3:tamaniPoopsOutBabies();  //She's close to giving birth so do it now  
                        default:tamaniMaleRepeatEncounter();
                    }
                }
            case 2:
    ...
    @:allow(classes.scenes.areas.forest)
    private function getRapedByTamaniYouHypnoSlut() : Void
    {
        spriteSelect(SpriteDb.s_tamani);
        clearOutput();
        ...
        function scene(x : Int) : Void
        {
            ...
            var y1 : Int = as3hx.Compat.parseInt(y + 1);
            if (y < 0)
            {
                sceneHunter.print("Check failed: two dicks.");
            }
            //[FITS]
            if (player.cockArea(x) <= 65)
            {
                ...
                {
                    outputText("She moans and pants, babbling about how good you are to her the whole time.");
                }
                ...
                if (y >= 0)
                {
                    outputText(" around your [cock " + y1 + "]");
                }
                outputText(". Her tiny form jiggles, ");
                if (player.tallness > 55)
                {
                    outputText("lifting off the ground as you get into it, giving yourself over to her sex.");
                }
                else
                {
                    outputText("nearly falling over as you give your body over to her sex.");
                }
                ...
                if (y < 0)
                {
                    outputText("her tongue hanging out as she pants nonsensical pleasure-noises.");
                }
                else
                {
                    outputText("and her mouth is too full of [cock " + y1 + "] to do anything but gurgle out nonsensical pleasure-noises.");
                }
                ...
                {
                    outputText("Your flare widens, actually holding itself inside Tamani's womb");
                }
                else if (player.cocks[x].cockType == CockTypesEnum.DEMON)
                {
                    outputText("The corrupted nodules along your [cock " + x1 + "] pulsate in rippling, wave-like motions, massaging the interior of Tamani's twat");
                }
                else if (player.cocks[x].cockType == CockTypesEnum.TENTACLE)
                {
                    outputText("Your tentacle-cock's tip widens, becoming mushroom-like inside Tamani's womb");
                }
                else if (player.hasKnot(x))
                {
                    outputText("Your knot balloons inside her, locking her in place");
                }
                else
                {
                    outputText("Your sensitive head swells slightly in time with the contractions of your orgasm");
                }
                outputText(" as you begin to fill your wife's womb with seed.   The orgasm is intense");
                if (player.cumQ() < 50)
                {
                    outputText(", but brief, and after a few moments of spasming pleasure, the flood of cum slows to a trickle.");
                }
                else if (player.cumQ() < 300)
                {
                    outputText(" and voluminous, forcing more and more seed into Tamani's uterus until she sloshes with each new squirt.  After a few long moments of spasming pleasure, the flood of cum slows to a trickle.");
                }
                else
                {
                    ...
                    if (player.cumQ() < 1000)
                    {
                        outputText("  Though your orgasm tapers off, the additional seed squirts from between her thighs, trickling down to a slow drip of viscous white goo.");
                    }
                    else
                    {
                        outputText("  Trapped inside her and cumming as hard as ever, you raise the pressure inside the goblin until your seed is washing out of her cunt around you, puddling on the ground.");
                    }
                    if (player.cumQ() >= 1000 && player.cumQ() < 4000)
                    {
                        outputText("  It does eventually end, but not until a sizable puddle has formed between the pair of you.");
                    }
                    else
                    {
                        outputText("  It lasts seemingly forever, and by the time you're done, the two of you are standing in a puddle inches deep.");
                    }
                }
                if (y >= 0)
                {
                    ...
                    if (player.cumQ() > 300)
                    {
                        ...
                        if (player.cumQ() > 600)
                        {
                            outputText("  A few seconds later she pulls back coughing, and the perky goblin takes the rest of your load in her face.");
                        }
                    }
                    else
                    {
                ...
                if (player.tallness > 55)
                {
                    outputText(" in spite of the height differential");
                }
            ...
            else
            {
                ...
                {
                    outputText("between your " + breastDescript(0) + " ");
                }
                ...
                if (player.cumQ() > 250)
                {
                    outputText("  In no time, you've filled her to capacity, and white mixes in with the clear sexual fluids that are dripping from the goblin's cunt.");
                }
                if (player.cumQ() > 700)
                {
                    outputText("  A few more blasts visibly bloat her belly, and Tamani pulls her hands away from her pussy to stroke the slowly rounding dome on her abdomen.");
                }
                if (player.cumQ() > 1500)
                {
                    ...
                    if (player.cumQ() > 3000)
                    {
                        outputText(", and by the time you finish, both of you are in a thick puddle of spunk");
                    }
                    outputText(".");
                }
                if (cocks > 1)
                {
                    if (cocks == 2)
                    {
                        outputText("  Your other [cock " + y1 + "]");
                    }
                    else
                    {
                        outputText("  Each of your other " + multiCockDescriptLight());
                    }
    ...
    @:allow(classes.scenes.areas.forest)
    private function tamaniAnalShits() : Void
    {
        spriteSelect(SpriteDb.s_tamani);
    ...
    @:allow(classes.scenes.areas.forest)
    private function tamaniBeaten() : Void
    {
        clearOutput();
        ...
        if (player.hasCock())
        {
            outputText("licks her lips in anticipation, eyes locked on [eachCock]");
        }
        else
        {
            outputText("frowns in displeasure, noting your lack of a penis");
        }
        ...
        if (flags[kFLAGS.TIMES_OVIPOSITED_TAMANI] == 0)
        {
            outputText("my baby-" + player.mf("daddy", "momma"));
        }
        else
        {
            outputText("my egg-obsessed arachnophile");
        }
        ...
        if (flags[kFLAGS.TIMES_OVIPOSITED_TAMANI] > 0)
        {
            outputText(", or in your case even more eggs");
        }
        ...
        if (player.hasCock())
        {
            outputText("  White coats the goblin from her shoulders up, the only evidence of your male satisfaction.  You were so focused on the act of impregnation and the unique pleasure that it provides that you barely noticed [eachCock] erupting in sympathetic pleasure.");
        }
    ...
    public function preferTamaniFemdom() : Void
    {
        ...
        if (player.hasCock())
        {
            outputText("\"<i>You prefer girl-on-girl relationships, huh?");
        }
        else
        {
            outputText("\"<i>You're into girls, huh?");
        }
