...
class Kitsune extends Monster
{
    ...
    public function kitsuneWait() : Void
    {
        ...
        if (EngineCore.silly())
        {
            outputText("  You're so glad you got to touch fluffy tail.");
        }
        outputText("\n\nShe licks her lips, running her hands along you wherever she can find exposed flesh.  Her fingertips leave small trails of dazzling blue that make you flush with lust - you must escape her grasp soon or else you will be like putty in her hands!");
        player.takeLustDamage(5 + player.effectiveSensitivity() / 10, true);
    }
    ...
    private function foxFireAttack() : Void
    {
        ...
        var damage : Int = as3hx.Compat.parseInt(this.inte + this.wis + rand(20));
    ...
    private function illusionKitsuneAttack() : Void
    {
        ...
        if ((player.hasPerk(PerkLib.HistoryReligious) || player.hasPerk(PerkLib.PastLifeReligious)) && player.cor < 20)
        {
            resist += as3hx.Compat.parseInt(20 - player.cor);
        }
        ...
        else
        {
            ...
            statStore.addBuff("spe", 20, "KitsuneIllusion", {
    ...
    private function kitSuneTeases() : Void
    {
        ...
        if (hairColor == "red" && rand(2) == 0)
        {
            select = 3;
        }
        if (select == 0)
        {
            outputText("You rub your eyes, suddenly seeing triple as you find yourself in the midst of a crowd of kitsune doppelgangers.  They run their hands all over you, teasing and doting on you as their tails caress every inch of your body.  Taken by surprise, you forget to fight back until they have already dispersed, blending back into a single fox-woman.");
        }
        else if (select == 1)
        {
            outputText("Bending forward, the kitsune runs her hands down over her breasts, jiggling them enticingly and squeezing them together.  Hooking a finger in her robes, she slides it down, tugging them aside until her nipples are just barely covered, and with a teasing smirk, pulls them back up, leaving you wanting.");
        }
        else if (select == 2)
        {
            outputText("Turning her back to you, the kitsune fans out her tails, peering back as she lifts the hem of her robe to expose her plump hindquarters.  Her tails continually shift and twist, blocking your view, but it only serves to make you want it even <i>more</i>, licking your lips in anticipation.");
        }
        //Redhead only:
        else
        {
            outputText("The kitsune sways her hips enticingly as she appears in front of you abruptly, rubbing up against your side.  Her teasing caresses make you shiver with arousal, and you can feel something thick and warm pressing against your [hips].  She gives you a wry grin as she breaks away from you, sporting an obvious tent in her robes.  \"<i>Just you wait...</i>\"");
        }
        player.takeLustDamage(5 + player.effectiveSensitivity() / 7, true);
    }
    
    override public function preAttackSeal() : Bool
    {
    ...
    {
        var moves : Array<Dynamic> = [foxFireAttack, foxFireAttack, kitSuneTeases, kitSuneTeases];
    ...
    public function new(hairColor : String)
    {
        super();
        var str : String = "";
        ...
        if (rand(3) != 2)
        {
            game.flags[kFLAGS.REDHEAD_IS_FUTA] = 1;
        }
        this.a = "a ";
        this.short = "kitsune";
        ...
        this.drop = new WeightedDrop().add(armors.ARCBANG, 1).add(consumables.FOXJEWL, 4);
