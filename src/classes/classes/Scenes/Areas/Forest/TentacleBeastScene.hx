...
class TentacleBeastScene extends BaseContent
{
...
*/
    ...
    private function alrauneLoss() : Void
    {
        ...
        if (player.isLiliraune())
        {
            outputText(" A quick glance to the side reveals your twin is subjected to the same delight which explains why you feel your female organ’s pleasure twice as much.");
        }
    ...
    {
        if (player.cor < 75 && player.lust < player.maxOverLust())
        {
            outputText("It grabs you before you can get away!\n\nWhile you attempt to resist the abomination, its raw muscle mass is too much. ");
        }
        ...
        if (player.hasBalls())
        {
            outputText(", and you feel your " + ballsDescript() + " rise and drop in unison to the muscular contractions pumping freshly made cum into position for release");
        }
        outputText(".\n\n");
        ...
        if (player.hasBalls())
        {
            outputText("Your balls have overfilled the creature!\n\n");
        }
        else
        {
            outputText("Your cum has overfilled the creature!\n\n");
        }
    ...
    private function centaurLoss() : Void
    {
        ...
        function dickNofit() : Void
        {
            ...
            if (player.hasBalls())
            {
                outputText("The sucking reaches all the way to your [balls], a spectacularly strange sensation that nevertheless feels wonderful.");
            }
            outputText("\n\n");
            scenePart2Selector();
        }  //=========================================================  ;
        
        var scenePart2Selector : Void->Void = function() : Void
        {
        ...
        var scenePart2Milk : Void->Void = function() : Void
        // Breasts > Manly, non-lactating:
        {
            ...
            if (player.lactationQ() <= 0)
            {
                ...
                {
                    outputText("  The anus-like tips affix to them.");
                }
                else
                {
                    outputText("  The anus-like tips attach to one pair as more appear in order to take the others.");
                }
            ...
            else if (player.lactationQ() >= 1000)
            {
                ...
                if (player.hasCock())
                {
                    ...
                    if (player.cumQ() <= 500)
                    {
                        outputText("pushing the creature to the edge of its fluid-containing abilities.");
                    }
                    else
                    {
                        outputText("quickly overfilling the tentacle attached to it; it explodes off of you, freeing your spunk to spray from both you and the retreating beast.  ");
                    }
                }
        ...
        var scenePart2Cum : Void->Void = function() : Void
        //normal start
        {
            ...
            if (player.hasBalls())
            {
                outputText("Your [balls] tighten up against you in preparation for their inevitable release, ready to spray their boiling load into the beast.  ");
            }
            ...
            if (player.cumQ() >= 1500)
            {
                ...
                if (player.hasBalls())
                {
                    outputText("r " + ballsDescriptLight());
                }
        ...
        var scenePart2WinShared : Void->Void = function() : Void
        {
            player.sexReward("cum", "Anal");
            player.sexReward("cum", "Lips");
        ...
        var scenePart2Fail : Bool->Void = function(describeCock : Bool = true) : Void
        // has vagina:
        {
            ...
            if (player.hasVagina())
            {
                ...
                if (player.hasCock())
                {
                    ...
                    {
                        outputText("  Your humping appears to not affect the creatures continuing efforts, despite the force of your body.");
                    }
                }
            ...
            if (player.ass.analLooseness >= 4)
            {
                ...
                if (player.hasCock())
                {
                    outputText("  A series of undulating lumps pass over your prostate, pushing out a splash of pre-cum.");
                }
                outputText("  You moan into the tentacle in your mouth appreciatevely at the beast's spectacular skill.\n\n");
            }
            else
            {
                sceneHunter.print("Check failed: gaping ass");
            }
            //has cock:
    ...
    private function milkLoss() : Void
    {
        ...
        function vagF() : Void
        {
            if (player.gender >= 2)
            {
                outputText("Your [allbreasts] ache, but you can tell immediately they are not depleted.  More milk dribbles as the tentacles try to retreat, and you grin, hardly satisfied with the beast's attack.  You reach between your thighs, seizing the nearest knobbly tentacle.  The beast is so sated it offers no resistance as you begin to pound your " + vaginaDescript(0) + " with the living dildo.  The idea of turning the tables on the raping beast spurs you on to new heights, and you cum quickly around the knobby shaft, your cunt spasming and milking the bumpy tentacle hard.  As you finish with the tentacle the beast gives a final gurgle and retreats into the forest.");
            }
            sharedEnd();
        }  //[Male]  ;
    ...
    private function tentacleLossRape() : Void
    {
        clearOutput();
        if (player.isAlraune())
        {
        ...
        {
            sceneHunter.print("Check failed: Liliraune/Alraune");
        }
        //Genderless madness
        if (player.gender == 0)
        {
        ...
        {
            sceneHunter.print("Check failed... genderless. Come on, try it, it's funny.");
        }
        //Bad-end counter
        if (player.isHerm())
        {
        ...
        //Centaur madness!
        var horsecockFound : Bool = player.findCockWithType(CockTypesEnum.HORSE, 1, 15, -1, "length") >= 0;
        if (player.isTaur())
        {
        ...
        {
            addButton(4, "Surrender", futaTentacleBadEnd).hint("<i>You can't resist it anymore... You want to suck, fuck, and be fucked by these tentacles!</i>");
        }
        //horsecock
        ...
        function dickScene() : Void
        {
            ...
            if (player.cockTotal() > 1)
            {
                outputText("at the base of your [cocks].");
            }
            ...
            if (player.cockTotal() == 1)
            {
                outputText("your member immediately becomes fully erect and pre-cum leaks liberally from your tip.");
            }
            else
            {
                outputText("your members immediately become fully erect, pre-cum drizzling from the tips.");
            }
            ...
            if (player.cockTotal() == 1)
            {
                outputText("In your daze you see a monstrous, hollow tentacle poised over your furious cock. You scream in shock and protest, but your cries fall upon deaf ears. The tentacle descends upon your penis, now begging for release, and clamps down upon your pubic mound, fully encapsulating your member.");
            }
            else
            {
                outputText("In your daze you see " + player.cockTotal() + " monstrous, hollow tentacles poised over your furious cocks.  You scream in shock and protest, but your cries fall upon deaf ears.  The tentacles descend upon your [cocks], all begging for release, and clamps down upon your pubic mound, fully encapsulating your dicks.");
            }
            doNext(dickScene2);
        };
        var vagScene : Void->Void = function() : Void
        {
            ...
            if (player.cor < 75)
            {
                outputText("You struggle to break free, but the creature only constricts you further, ensuring your immobility. ");
            }
            outputText("A quick flex of the tentacles securing your legs leaves you spreadeagled before the maw of the horror.  ");
            if (player.cor < 75)
            {
                outputText("Fearing for your life, you scream and struggle for help, but only the apathetic sounds of nature respond.");
            }
        ...
        function hermScene() : Void
        {
            ...
            if (player.cockTotal() > 1)
            {
                outputText(multiCockDescriptLight() + " immediately become fully erect and leak pre-cum liberally from their tips.  ");
            }
            else
            {
                outputText("member immediately becomes fully erect and pre-cum leaks liberally from your tip.  ");
            }
            outputText("  " + cast((player.totalNipples()), Num2Text) + " thorny tentacles pierce your nipples, and you feel as if someone shot acid into your tits, which immediately begin to swell.");
            ...
            if (player.cockTotal() > 1)
            {
                outputText("furious cocks.  ");
            }
            else
            {
                outputText("furious cock.  ");
            }
            outputText("You scream in shock and protest, but your cries fall upon deaf ears. The tentacle descends upon your ");
            if (player.cockTotal() > 1)
            {
                outputText(multiCockDescriptLight() + ", now begging for release, and clamps down around your pubic mound, fully encapsulating your members.  ");
            }
            else
            {
                outputText(cockDescript(0) + ", now begging for release, and clamps down upon your pubic mound, fully encapsulating your member.");
            }
            doNext(hermScene2);
        }  //=========== PART 2  ;
        
        var dickScene2 : Void->Void = function() : Void
        {
            clearOutput();
        ...
        var vagScene2 : Void->Void = function() : Void
        {
            clearOutput();
        ...
        var hermScene2 : Void->Void = function() : Void
        {
            clearOutput();
    ...
    private function tentacleRapeContinuationForFemales() : Void
    {
        ...
        {
            outputText("too intoxicated with lust to continue the pleasure. ");
        }
        ...
        if (player.clitLength < 7 && player.clitLength >= 3.5 && player.hasPerk(PerkLib.BigClit))
        {
            ...
            outputText("Your massive clitty eventually diminishes, retaining a fair portion of its former glory.  It is now " + as3hx.Compat.parseInt(player.clitLength * 10) / 10 + " inches long when aroused, ");
        ...
        {
            outputText("In time, it returns to its normal size, losing all the extra volume.");
        }
    ...
    private function centaurGenderlessRetardation() : Void
    {
        ...
        if (flags[kFLAGS.TENTABEAST_CENT_GLESS_MET] == 0 || player.balls == 0)
        {
            ...
            if (player.cor < 50 && player.lust < 70)
            {
                outputText("dread");
            }
            else
            {
                outputText("anticipation");
            }
    ...
    public function defeatedByAncientTentacleBeast() : Void
    {
        ...
        if (!player.hasCock())
        {
            ...
            if (player.hasVagina())
            {
                outputText(" Oh, no, you're a full herm now!");
            }
        }
        if (player.balls == 0)
        {
            outputText(" Underneath your [cock] a large sack has formed. You feel nauseous as balls drops in churching with fresh cum.");
        }
        if (!player.hasVagina())
        {
