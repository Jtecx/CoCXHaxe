...
class AkbalScene extends BaseContent
{
    public function new()
    {
        super();
    ...
    /**
		 ...
		 */
    public function akbalDefeated(hpVictory : Bool, count : Bool = true) : Void
    {
        flags[kFLAGS.AKBAL_SUBMISSION_STATE] = 1;
        if (flags[kFLAGS.SPARRABLE_NPCS_TRAINING] == 2)
        {
            if (flags[kFLAGS.AKBAL_DEFEATS_COUNTER] >= 1)
            {
                flags[kFLAGS.AKBAL_DEFEATS_COUNTER]++;
            }
            else
            {
                flags[kFLAGS.AKBAL_DEFEATS_COUNTER] = 1;
            }
            //level up
    ...
    public function akbalWon(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        flags[kFLAGS.AKBAL_SUBMISSION_STATE] = -1;
    ...
    public function akbalSubmissionFollowup() : Void
    {
        ...
        {
            //(fighting Akbal disables this scene, but you retain the ability if you rape him after)if (flags[kFLAGS.AKBAL_SUBMISSION_COUNTER] >= 8 && player.cor > 80 - player.corruptionTolerance)
            {
    ...
    private function akbitchNoThnx(clear : Bool = true) : Void
    {
        if (clear)
        {
            clearOutput();
    ...
    private function basicAkbitch() : Void
    {
        ...
        {
            var x1 : Int = as3hx.Compat.parseInt(x + 1);  //for output  
        ...
        var dickF2 : Int->Void = function(x : Int) : Void
        {
        ...
        var vagF2 : Void->Void = function() : Void
        {
            clearOutput();
            ...
            player.sexReward("cum", "Vaginal");
            dynStats("cor", 3);
            if (player.hasVagina() && !player.isGoblinoid())
            {
                player.knockUp(PregnancyStore.PREGNANCY_IMP, PregnancyStore.INCUBATION_IMP, 101);
            }
            endEncounter();
