...
class Nightmare extends Monster
{
    ...
    public function usingTease() : Void
    {
        switch (rand(3))
        {
            ...
            case 2:
                ...
                if (player.hasCock())
                {
                    outputText(" I could even let you slide that lovely [cock] inside and make you cum all over my face.");
                }
                outputText("</i>\"\n\n");
        }
        var teaseD : Float = 0;
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (this.lust >= this.maxLust() * 0.8)
        {
            usingCorruptionBlast();
        }
        if (hasStatusEffect(StatusEffects.Uber))
        {
            usingHeal();
