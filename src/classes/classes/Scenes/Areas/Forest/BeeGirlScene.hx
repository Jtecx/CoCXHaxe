...
class BeeGirlScene extends BaseContent
{
    private var attitude(get, set) : Int;
    private var badEndWarning(get, set) : Bool;
    private var conversation(get, set) : Int;

    private static inline var BEE_GIRL_CONVERSATION : Int = 0x7FFF0000;  //Used to track conversations when the player is a female bee morph (Highest bit is for the bad end warning)  
    private static inline var BEE_GIRL_ATTITUDE : Int = 0x0000FFFF;
    private static inline var BEE_GIRL_TALKED : Int = 1;  //Replaces the old bee progress flag  
    private static inline var BEE_GIRL_TALKED_AND_LEFT : Int = 2;  //Refusing to take her eggs leads to conversation  
    private static inline var BEE_GIRL_TALKED_AND_LEFT_TWICE : Int = 3;
    private static inline var BEE_GIRL_PLAYER_AFRAID : Int = 4;
    private static inline var BEE_GIRL_PLAYER_VOLUNTARY_EGGING : Int = 5;  //End of the afraid chain, from now on player gets egged when they meet her  
    private static inline var BEE_GIRL_PLAYER_DISGUSTED : Int = 6;
    private static inline var BEE_GIRL_PLAYER_DUTY : Int = 7;
    ...
    {
        flags[kFLAGS.BEE_GIRL_STATUS] = BEE_GIRL_TALKED;
    }
    
    private function get_attitude() : Int
    {
        return as3hx.Compat.parseInt(flags[kFLAGS.BEE_GIRL_STATUS] & BEE_GIRL_ATTITUDE);
    }
    
    private function set_attitude(value : Int) : Int
    {
        flags[kFLAGS.BEE_GIRL_STATUS] = (flags[kFLAGS.BEE_GIRL_STATUS] & BEE_GIRL_CONVERSATION) + value;
        return value;
    }
    
    private function get_badEndWarning() : Bool
    {
        return (flags[kFLAGS.BEE_GIRL_STATUS] & 0x80000000) != 0;
    }
    
    private function set_badEndWarning(value : Bool) : Bool
    {
        flags[kFLAGS.BEE_GIRL_STATUS] = (flags[kFLAGS.BEE_GIRL_STATUS] & as3hx.Compat.parseInt(BEE_GIRL_ATTITUDE | BEE_GIRL_CONVERSATION)) + ((value) ? 0x80000000 : 0);
        return value;
    }
    
    private function get_conversation() : Int
    {
        return as3hx.Compat.parseInt((flags[kFLAGS.BEE_GIRL_STATUS] & BEE_GIRL_CONVERSATION) >> 16);
    }
    
    private function set_conversation(value : Int) : Int
    {
        flags[kFLAGS.BEE_GIRL_STATUS] = (flags[kFLAGS.BEE_GIRL_STATUS] & BEE_GIRL_ATTITUDE) + (value << 16);
        return value;
    }
    ...
    private function beeEncounterSelect(clearScreen : Bool = true) : Void
    {
        if (clearScreen)
        {
            clearOutput();
        ...
        {
            sceneHunter.print("Check failed: Alraune body");
        }
        //Exgartuan
        ...
        {
            sceneHunter.print("Check failed: Exgartuan. He should be NOT sleeping too.");
        }
        //Worms
        ...
        //Bee-morph
        var isBeeMorph : Bool = player.isRace(Races.BEE, 1, false);
        var femBee : Bool = isBeeMorph && player.hasVagina();
        if (player.hasCock() && (player.findCockWithType(CockTypesEnum.BEE, 1, 50, -1) >= 0 || isBeeMorph && player.biggestCockArea() >= 50))
        {
            if (!femBee || rand(2) == 0)
            
            //randomize the girl thingy{
        ...
        {
            sceneHunter.print("Check failed: bee race and cock area greater than 50 (or just a bee cock of this size)");
        }
        if (femBee)
        
        // if cock thing failed,{
        ...
        {
            sceneHunter.print("Check failed: bee race and a pussy.");
        }
        //Fighs counter
        ...
        {
            sceneHunter.print("Check failed: consecutive losses/wins/rapes");
        }
        //Attitudes
        switch (attitude)
        {
    ...
    private function beeEncounterDefault() : Void
    {
        ...
        if (player.lust < player.maxLust() * 0.75)
        {
            ...
            if (player.gender == 3 && player.lib > 50)
            {
                addButton(2, "Seduce", seduceBeeGirl);
            }
            else
            {
                addButtonDisabled(2, "Seduce", "You need to be a herm and more libido to overpower her pheromones.");
            }
    ...
    private function beeEncounterAsBeeMorphMaleOrGiantCock(isBeeMorph : Bool) : Void
    {
        ...
        {
            outputText("Come here little one, I’ll help you become a real bee boy");
        }
    ...
    private function beeEncounterAsBeeMorphFemale() : Void
    {
        ...
        {
            outputText("You grin at the thought, it would certainly be fun fucking her, but maybe it would be even more fun to force yourself on her?");
        }
    ...
    private function beeEncounterAfraidFirstTimeSex(firstTime : Bool = true) : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_bee_girl);
        ...
        {
            outputText("<b>Psst... enable SceneHunter to get up to 4 scenes here...</b>");
        }
    ...
    private function beeEncounterAfraidRepeatSex() : Void
    {
        ...
        {
            outputText("slipping her fingere inside your " + assholeDescript() + "till you’re ready");
        }
        ...
        if (player.analCapacity() < 25)
        {
            ...
            {
                outputText("<b>Your " + assholeDescript() + " has become looser thanks to the knotted appendage penetrating you.</b>");
            }
        ...
        {
            outputText("her small body against yours.  She rubs her head against your chest, and you vaguely hear her whisper something about a strong carrier for strong eggs.");
        }
    ...
    private function beeEncounterSheBeatsYouRegularly() : Void
    {
        ...
        if (player.lust < player.maxLust() * 0.50 || rand(5) == 0)
    ...
    private function beeEncounterSheBeatsYouRegularlyAndYouLetHerLaysEggs(clearScreen : Bool = true) : Void
    {
        if (clearScreen)
        {
            clearOutput();
    ...
    private function beeEncounterWithWorms() : Void
    ...
    public function afterfightoptionswithBeeGirl(hpVictory : Bool) : Void
    {
    ...
    private function leaveAfterDefeating(fromRape : Bool = false) : Void
    {
        if (fromRape)
        {
            flags[kFLAGS.BEE_GIRL_COMBAT_WINS_WITHOUT_RAPE]--;
        }  //undo 'rape' changes  
        //work with counters
        if (monster.HP < 1)
        {
        ...
        //This only happens if you beat her up and then don't rape her
        else
        {
    ...
    private function beeEncounterClassic(clearScreen : Bool = true) : Void
    {
        if (clearScreen)
        {
            clearOutput();
        ...
        }  //Reset your friendly conversation path if autorape or accepted  
    ...
    private function beeEncounterClassicSex() : Void
    {
        ...
        {
            sceneHunter.print("Failed check: Taur lower body");
        }
        //NON TAURS
        ...
        function stingerF() : Void
        {
            ...
            if (player.vaginas.length == 0)
            {
                ...
                {
                    outputText(", your nipples swollen and red with desire.  ");
                }
                else
                {
                    outputText(".");
            ...
            else
            {
                ...
                {
                    outputText("centering around your now puffy vulva.  ");
                }
                if (player.vaginas[0].vaginalWetness >= VaginaClass.WETNESS_WET && player.vaginas[0].vaginalWetness < VaginaClass.WETNESS_DROOLING)
                {
                    outputText("growing more potent around your " + vaginaDescript(0) + " as it begins to leak with the honey of your desire.  ");
                }
                if (player.vaginas[0].vaginalWetness >= VaginaClass.WETNESS_DROOLING)
                {
                    outputText("inflaming your " + vaginaDescript(0) + " with need, and allowing a river of your wetness to gush from between your puffy vulva.  ");
                }
            }
            //Boobies!
            if (player.biggestTitSize() > 1)
            {
                ...
                {
                    outputText(", leaning around to lick them with her curving sinuous tongue");
                }
                //if lactating
                if (player.biggestLactation() > 1)
                {
                    outputText(", humming in surprise as a squirt of milk splatters across the flower's petals");
                }
                outputText(".  ");
            }
            //Continue as normal!
            ...
            if (player.vaginas.length > 0)
            {
                outputText(" as your hand is guided to your " + vaginaDescript(0));
            }
            outputText(".\n\n");
            //Cum text
            if (player.vaginas.length > 0)
            {
                outputText("Your fingers dance over your clit as you lap up her honey");
            }
            else
            {
                outputText("Your fingers explore every inch of both your bodies as you lap up her honey");
            }
            ...
            if (player.vaginas.length > 0)
            {
                outputText(", the constant aphrodisiac honey you're mindlessly devouring, and your fingers' instinctual ravaging of your " + vaginaDescript(0) + ", ");
            }
            else
            {
                outputText(" and the constant aphrodisiac honey you're mindlessly devouring, ");
            }
        ...
        function taurF() : Void
        {
            ...
            {
                sceneHunter.selectBigSmall(bigOrGless, 100, smallOrVag, -1, null, "length");
            }
            ...
            //[Male (bypasses the bucking scene):
            {
                ...
                if (player.gender == 1)
                {
                    ...
                    if (player.balls == 0)
                    {
                        outputText("With your cock quivering and spasming you continue to lap at the honey covered tree until the vibrating bee-girl finishes.  ");
                    }
                    //[With testicles:
                    else
                    {
                        outputText("With your cock quivering and spasming semen erupts from the tip.  ");
                    }
                    //[Little-normal cum amount:
                    if (player.cumQ() < 100)
                    {
                        outputText("You spurt out more cum than you thought your [balls] could hold. White speckles the base of the tree and tiny puddles form underneath you as you groan in satisfaction.\n\n");
                    }
                    //[Huge cum amount:
                    else
                    {
                        outputText("Your [balls] goes off like a firehouse, coating the base of the tree along with a number of the ones behind it. Soon you're standing in a massive puddle of your own splooge, the scent threatening to overpower that of the honey. The bee-girl compensates by splashing more onto your back, causing you to release yet another burst of thick white cum.\n\n");
                    }
                }
                ...
                else if (player.gender == 3)
                {
                    ...
                    if (player.balls == 0)
                    {
                        outputText("With your cock quivering and spasming and your untended to " + vaginaDescript(0) + " squirting repeatedly, you continue to lap at the honey covered tree until the vibrating bee-girl finishes.  ");
                    }
                    //[With testicles:
                    else
                    {
                    ...
                    {
                        outputText(SMultiCockDesc() + " goes off like a firehouse, coating the base of the tree along with a number of the ones behind it. Soon you're standing in a massive puddle of your own splooge, the scent threatening to overpower that of the honey. The bee-girl compensates by splashing more onto your back, causing you to release yet another burst of thick white cum.\n\n");
                    }
                }
            ...
            var smallOrVag : Void->Void = function() : Void
            //[Male:
            {
                
                if (player.gender == 1)
                {
                    outputText("Your inability to reach your engorged [cocks] quickly becomes a problem as the pleasure from the bee-maiden's scent courses through you. You ache to be tended to, gushing out pre.");
                }
                //[Female:
                else if (player.gender == 2)
                {
                    outputText("Your inability to reach your " + vaginaDescript(0) + " quickly becomes a problem as the pleasure from the bee maiden's scent courses through you, it starts reflexively rippling, trying to milk the cock it so desperately wants while your " + clitDescript() + " quivers.");
                }
                //[Herm:
                else if (player.gender == 3)
                {
                    outputText("Your inability to reach your engorged [cocks] quickly becomes a problem as the pleasure from the bee maiden's scent courses through you. It aches to be tended to, gushing out pre. Your " + clitDescript() + " quivers, desperate for contact as your vagina starts reflexively rippling, trying to milk the cock it isn't going to get.");
                }
                //[With breasts:
                if (player.biggestTitSize() > 1)
                {
                    ...
                    {
                        outputText("  Your fingers slip in and out of them, sending femcum flying everywhere. But the effects of the bee's honey appear to prevent you from getting off.");
                    }
                    //[Other:
                    else
                    {
                        outputText("  No amount of stimulation seems to get you off though.");
                    }
                }
                outputText("\n\n");
                ...
                switch (_sw0_)
                {
                    ...
                    case 1:
                        ...
                        if (player.balls == 0)
                        {
                            outputText("You cum violently, your [cocks] quivering and spasming.  ");
                        }
                        //[With testicles:
                        else
                        {
                            outputText("You cum violently, your [cocks] quivering and spasming while semen erupts from the tip.  ");
                        }
                        //[Little-normal cum amount:
                        if (player.cumQ() < 100)
                        {
                            outputText("You spurt out more cum than you thought your [balls] could hold. White speckles your underside and tiny puddles form underneath you as you groan in satisfaction.\n\n");
                        }
                        //[Huge cum amount:
                        else
                        {
                            outputText("Your [balls] goes off like a firehouse, coating your underside with white and dripping down your front legs. Soon you're standing in a massive puddle of your own splooge, the scent threatening to overpower that of the honey. The bee-girl compensates by splashing more onto your back, causing you to release yet another burst of thick white cum.\n\n");
                        }
                    case 2:
                        ...
                        {
                            outputText("Her awkward ministrations to your " + clitDescript() + " cause a gush of fluids from your " + vaginaDescript(0) + " and you let out a lewd moan of satisfaction.  ");
                        }
                        //[Cock-like clit:
                        else
                        {
                            outputText("She quickly finds your " + clitDescript() + " and begins to jerk it off like a cock.  The sensation is incredible, and you moan lewdly as your " + vaginaDescript(0) + " gushes fluid.  ");
                        }
    ...
    public function beeSexForCocks(clearScreen : Bool = true, disableBadEnd : Bool = false) : Void
    {
        if (clearScreen)
        {
            clearOutput();
    ...
    private function beeTalk() : Void
    {
        ...
        //Replaced beeProgress{
            ...
            if (player.cor < 33)
            {
                outputText("You are sure no good can come of this, but your body is ready to say yes.");
            }
            if (player.cor >= 33 && player.cor <= 66)
            {
                outputText("Her offer intrigues you, and the arousing sweetness of her scent makes it difficult to resist.");
            }
            if (player.cor > 66)
            {
                outputText("Looking at her through lust-tinted eyes, you're sure she can deliver on her offer.  Getting closer to her scent alone would be worth bearing a few eggs...");
            }
        ...
        else
        {
            ...
            else
            {
                ...
                if (player.cor < 33)
                {
                    outputText("You are sure no good can come of this, but your body is urging you to agree.");
                }
                if (player.cor >= 33 && player.cor <= 66)
                {
                    outputText("Her offer intrigues you, and the arousing sweetness of her scent makes it difficult to resist.");
    ...
    private function beeEncounterRefusedHerEggs() : Void
    {
        ...
        switch (attitude)
        {
            ...
            case BEE_GIRL_TALKED_AND_LEFT:
                ...
                break;  //This handled the first refusal - only BEE_GIRL_TALKED should go to the default  
    ...
    private function beeMaidenPlay() : Void
    {
        ...
        {
            outputText("You barely keep yourself from jumping her in your excitement, but you manage to stay where you are, so she can make the first move.");
        }
        sceneHunter.selectGender(null, femF, glessF);
        
        var femF : Void->Void = function() : Void
        {
    ...
    public function canBeeMaiden() : Bool
    {
        return player.cor < 66 + player.corruptionTolerance;
    }
    private function beeMaidenConversation() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_bee_girl);
        if (conversation > 2 && !canBeeMaiden())
        {
            conversation = 2;
        }
        switch (conversation)
    ...
    private function seduceBeeGirl() : Void
    {
        ...
        if (player.isTaur())
        {
            outputText(" leaning between your knees to reach your bulging member.\n\nShe drops to her own knees, gazing at your [cock] before");
        }
        else
        {
            outputText(" continuing , sliding to the front of your pants to feel your bulging member.\n\nShe falls to her knees, sliding your cock free before");
        }
        ...
        if (player.hasBalls())
        {
            outputText(", sliding down and over your balls,");
        }
    ...
    public function beeRapesYou() : Void
    {
        ...
        if (!player.isTaur() && player.hasCock())
        {
            ...
            {
                sceneHunter.print("Check failed: Need to be under poison or paralysis effect.");
            }
        }
        ...
        if (!player.isTaur() && player.hasCock() && (!player.hasVagina() || rand(2) == 0))
        {
            ...
            {
                outputText("Overcome by lust, you throw yourself into her waiting arms.  ");
            }
            else
            {
                outputText("Overcome by your wounds, you are unable to resist as she lifts you into her arms and embraces you.  ");
            }
        ...
        if (player.hasVagina())
        {
            if (player.lust < player.maxOverLust())
            {
                outputText("Barely conscious, you look up as the bee approaches.  ");
            }
            else
            {
                outputText("Overcome with desire, you look up as the bee approaches.  ");
            }
            ...
            if (player.isTaur())
            {
                outputText("After a while, her hot and sweet pussy swiftly disappears from your sight, and you feel the bee-girl's weight on your butt, right before her stinger,");
            }
            else
            {
                outputText("You feel the bee-girl's stinger");
            }
    ...
    private function beeRapesTaurs() : Void
    {
        ...
        if (cockIndex < 0)
        {
            cockIndex = player.smallestCockIndex();
        }
        sceneHunter.print("Fork: HP and lust loses...")  //Damage:  ;
        ...
        else
        {
            ...
            if (player.gender == 1 && player.hasBalls())
            {
                ...
                if (player.cockTotal() == 1)
                {
                    outputText("it as it's ");
                }
                else
                {
                    outputText("them as they're ");
                }
                ...
                if (player.cumQ() >= 1500)
                {
                    outputText("Your massive cum amount soon causes you to be lying in a deep puddle of splooge, the scent so strong that it's almost enough to overwhelm that of the honey, despite it coating your face.\n\n");
                }
            ...
            else if (player.gender == 2)
            {
                ...
                if (player.wetness() == 5)
                {
                    outputText("Your massive cum amount soon causes you to be lying in a deep puddle of femcum, the scent so strong that it's almost enough to overwhelm that of the honey, despite it coating your face.\n\n");
                }
            ...
            else if (player.gender == 3)
            {
                ...
                if (player.cumQ() >= 1500)
                {
                    outputText("Your massive cum amount soon causes you to be lying in a deep puddle of splooge and femcum, the scent so strong that it's almost enough to overwhelm that of the honey, despite it coating your face.\n\n");
                }
                outputText("She continues pumping into you as you pass out, your entire rear aching, and the combined scent of cum and honey overwhelming you.");
            }
            //[Genderless:
    ...
    {
        var x : Float = player.findCock(1, -1, monster.vaginalCapacity());
        var y : Float = player.findCock(2, -1, monster.vaginalCapacity());
        var z : Float = player.findCock(3, -1, monster.vaginalCapacity());
        var zz : Float = player.findCockNotIn([x, y, z]);  //any is good  
        spriteSelect(SpriteDb.s_bee_girl);
        clearOutput();
        //Doubledick special
        ...
        var onHerBack : Void->Void = function() : Void
        {
        ...
        var onHerKnees : Void->Void = function() : Void
        {
        ...
        var lustOverload : Void->Void = function() : Void
        {
            ...
            {
                outputText("  She stretches against you amazingly, squeezing down on you like a vice.  You theorize it's only because of her strange body and the copious amounts of honey she's leaking against you that you aren't ripping her apart.");
            }
            //New PG
            ...
            if (player.hasBalls())
            {
                outputText(" with your [balls] hitting her abdomen hard enough to make smacking sounds ring out in the forest around you.");
            }
            else
            {
                outputText(".");
            ...
            {
                sceneHunter.print("Check failed: at least 3 fitting cocks");
            }
            //Orgasm PG
            ...
            if (z >= 0)
            {
                ...
                {
                    outputText("  The " + cockDescript(zz) + " not inside her shakes wildly as you cum; jets of your semen let loose over her abused frame, coating her in white.");
                }
                if (player.cocks.length > 4)
                {
                    outputText("  The [cocks]s not inside her shake wildly as you cum; jets of your semen let loose over her abused frame, coating her in white.");
                }
            }
            //New PG
            outputText("\n\n");
            ...
            if (!player.isTaur())
            {
                outputText("stuffing your [cocks] inside your " + player.armorName);
            }
            else
            {
                outputText("your glorious but spent [cocks] dangling under your belly,");
            }
    ...
    private function taurRapesBeeWithCock() : Void
    {
        ...
        //Cock short enough to penetrate her:
        {
            ...
            if (player.cockArea(x) <= monster.vaginalCapacity())
            {
                ...
                if (player.hasBalls())
                {
                    outputText("  Her other hand slides down and begins to massage your [balls], squeezing and tugging gently at your scrotum. Her tongue joins them, sliding in and out of your ass before wrapping around your sack. You feel incredible and your " + cockDescript(x) + " drips pre, forming a puddle on the ground.");
                }
                //No testicles:\
                else
                {
                    outputText("Her other hand starts pulling it open and soon her tongue joins them, sliding in and out of your ass delicately. Your " + cockDescript(x) + " quivers with anticipation, and you swish your tail happily.");
                }
                outputText("\n\n");
                ...
                if (player.hasBalls())
                {
                    outputText("  Your [balls] really feel spectacular, the vibrations causing the hot spunk inside to roil about.");
                }
                ...
                if (player.hasBalls())
                {
                    outputText(", and your [balls] constrict");
                }
                outputText(".\n\n");
                ...
                if (player.balls == 0)
                {
                    outputText("  It dribbles into the bee-girl as her honey squirts out of her once again. Her body trembles as the extreme exertions take their toll, and she slides off of your member with a damp plop, splashing into a puddle of her own fluids.");
                }
                //Normal cum amount:
                else
                {
                    ...
                    if (player.cumQ() < 500)
                    {
                        outputText("  It sprays inside the bee-girl as her honey squirts out of her once again. Her body trembles as the extreme exertions take their toll, and she slides off of your member with a damp plop, splashing into a puddle of her own fluids as your spunk dribbles onto her.");
                    }
                    //Huge cum amount:
                    else
                    {
                        outputText("  It sprays inside the bee-girl, quickly overfilling her and causing her belly to expand. Her sudden increased girth and extreme exertion cause her to lose her grip on you, and she is propelled off of you into a puddle of spunk and honey. You can't control yourself as your [balls] continue to pump out more and more fluid, spraying down the exhausted bee and coating her flower with enough gunk to cause it to wilt slightly. Eventually it stops, with only periodic gushes spraying out.");
                    }
                }
                outputText("\n\n");
                ...
                if (player.cumQ() >= 500)
                {
                    outputText(" and watching as her lower body sinks into your spunk like a thick white bath");
                }
            ...
            else
            {
                ...
                if (player.hasBalls())
                {
                    outputText("  The vibrations do a number on your [balls] too, as the semen stored within starts to shake and dance about inside.");
                }
                ...
                {
                    outputText("  Her tongue wraps around your " + cockDescript(x) + " and slides up and down the small area it can reach before flicking about at the tip of your urethra. Her expert licking feels amazing and it's not long before your feel the familiar sensation of orgasm creeping up on you.");
                }
                outputText("\n\n");
                ...
                if (player.hasBalls() && player.tallness * (5 / 6) < player.cocks[x].cockLength)
                {
                    ...
                    if (player.cumQ() >= 750)
                    {
                        outputText("  You don't stop there though, as your firehose of a cock sprays it down until it's sagging with the weight of your seed.");
                    }
                }
                ...
                else if (player.hasBalls())
                {
                    ...
                    if (player.cumQ() >= 750)
                    {
                        outputText("  You don't stop there though, as your firehose of a cock sprays wildly, coating her head and her flower, hosing it down until it's sagging with the weight of your seed.");
                    }
                }
                //Without testicles:
                else
                {
                    outputText("Your " + assholeDescript() + " tightens uncontrollably around its intruder, making the vibrations feel even stronger there. Your member jerks as you orgasm and a few droplets of semen land on the bee maiden.");
                }
    ...
    private function rapeTheBeeGirlWithADick() : Void
    {
        ...
        function honeyFuck() : Void
        {
            ...
            if (player.isTaur())
            {
                outputText("as your " + cockDescript(x) + " hardens under your belly.");
            }
            else
            {
                outputText("as you disrobe, revealing your " + cockDescript(x) + " to her.");
            }
            outputText("  Wasting no time, you get down to the business of ");
            if (player.cockTotal() == 1)
            {
                outputText("penetrating her slick snatch, forcing in inch after inch.  ");
            }
            if (player.cockTotal() == 2)
            {
                outputText("cramming your " + multiCockDescript() + " into her, double-stuffing the poor bee-slut with your double dongs.  ");
            }
            if (player.cockTotal() >= 3)
            {
                outputText("cramming two out of your " + num2Text(player.cockTotal()) + " " + multiCockDescript() + "s into her, double-stuffing the poor bee-slut with double the dicks.  ");
            }
            ...
            {
                outputText("  You reach down and finger your " + vaginaDescript(0) + " roughly, lost in the throes of your orgasm.");
            }
            outputText("\n\n");
            //wind down
            ...
            {
                outputText("You smile proudly at the steady stream of your cum that pours from her abused cunt, pooling underneath her.   Her abdomen even looks bloated with your seed.");
            }
            if (player.cumQ() >= 500 && player.cumQ() < 1000)
            {
                outputText("You smile proudly at your handy-work, noting how bloated and distended the bee's abdomen is.  It bulges obscenely, nearly double its previous size.  A sizable river of your spunk drools from between your legs, but amazingly she seems able to keep most of your over-sized orgasm in.");
            }
            if (player.cumQ() >= 1000)
            {
                outputText("You giggle at your poor victim's state.  She really is a mess.  Her abdomen and belly are both swollen, making her look pregnant in both her bee AND human halves.   A practical river of spunk drools from her glossy pussy-lips, pooling below her.  Even her ovipositor dangles down, dripping with your seed, forced out from her abdomen by the sheer amount of spunk she's carrying.  It twitches, bulges moving along its length, eventually dropping egg after egg on the ground, unable to keep in its cargo.");
            }
            player.sexReward("vaginalFluids", "Dick");
            cleanupAfterCombat();
        }  //Male 2  ;
        
        var roughFuck : Void->Void = function() : Void
        {
            var x : Float = player.cockThatFits(monster.vaginalCapacity());
            clearOutput();
    ...
    private function horseVagoo() : Void
    {
        ...
        if (player.hasFuckableNipples())
        {
            outputText("  Not even repeated penetration of your " + nippleDescript(0) + "s seems to bring the relief you need.");
        }
        outputText("\n\n");
        ...
        {
            outputText("Her legs release your head and she falls back, knocking into your [allbreasts] and causing them to jiggle lewdly. You grab her before she hits the ground, surprised at how light she is.");
        }
        //[Breasts <=C:
        else
        {
            outputText("Her legs release and she falls back, but you catch her before she hits the ground and are surprised by how light she is.");
        }
        ...
        {
            outputText(" and ooze fluids in a constant stream down your front");
        }
        ...
        {
            outputText(" and your flailing tail is held firmly in place by her hand");
        }
        ...
        {
            outputText("  Not wanting this, she attempts to brace her other hand between your [ass]'s cheeks, but your " + assholeDescript() + " opens and she slides inside. With an immense effort she pulls out of your ass, nearly pushing you over the edge in the process, but the arm stuck on your vagina is stuck tight.");
        }
        ...
        if (player.wetness() >= 5)
        {
            outputText("Your vagina ripples and squeezes with enough force to hurt most normal cocks. Her exoskeleton protects her from the harmful effects though, and with a massive gush of fluids the bee-girl's arm is propelled out of you.");
        }
        //[All other cum amounts:
        else
        {
            outputText("Your vagina ripples and squeezes with enough force to hurt most normal cocks. Her exoskeleton protects her from the harmful effects though, and she slides her arm out of you as you cum.");
        }
    ...
    private function rapeABeeGirlWithYourVagina() : Void
    {
        ...
        if (player.breastRows.length > 1)
        {
            outputText(" and " + player.allBreastsDescript());
        }
        ...
        {
            outputText("Thankfully her tongue keeps up its assault, curling 'round your clit and probing your depths in equal measure, keeping you slick and writhing in pleasure.  ");
        }
        ...
        {
            outputText("Your hips wiggle and writhe on the length of her tongue as it dives into your slippery depths and curls tightly around your clit, jacking it up and down like a cock.  Your girl-cum soaks her chin, drooling happily as she pleasures you.  ");
        }
        if (player.vaginas[0].vaginalWetness >= VaginaClass.WETNESS_SLAVERING)
        {
            outputText("Your hips quiver and grind against her face as her unusually long tongue simultaneously probes your depths and works your clit, wrapping around it and jerking it like a cock.   You squirt and drool constantly from her skilled assault, soaking her face and hair with your girl-cum.  ");
        }
        if (player.clitLength > 3 && player.clitLength < 5)
        {
            outputText("Your hips piston your huge clit into her mouth with no warning, forcing her to give your clit a blowjob.  Her slick black lips wrap around it immediately, sucking and licking, filling your body with lust and pleasure.  ");
        }
        ...
        if (player.isTaur())
        {
            outputText("You orgasm in no time, coating her face with girl-cum the process.  You return the favor and dive into her muff, planting kisses and licks down as a reward for your victim's rather skilled efforts.  The taste is sweet and sexy all at once, and you quickly lose track of your reward scheme, simply licking and slurping to get as much of her nectar as possible until she is satisfied as well.");
        }
        else
        {
            outputText("In no time you both orgasm, sweet girl-cum coating both your faces as you work each other's cunt with desperation born of desire.  ");
        }
        player.sexReward("saliva", "Vaginal");
        player.sexReward("vaginalFluids", "Lips");
        cleanupAfterCombat();
    ...
    private function futaRapesBeeGirl() : Void
    {
        ...
        if (player.isTaur())
        {
            outputText("as you idly fondle your [allbreasts] as you feel your blood-gorged [cocks] swaying under your belly.  ");
        }
        else
        {
            outputText("as you disrobe, revealing your [allbreasts], " + vaginaDescript(0) + ", and [cocks].  ");
        }
        ...
        {
            outputText("Your " + vaginaDescript(0) + " becomes puffy and moist with excitement, ready for what you have planned.");
        }
        if (player.vaginas[0].vaginalWetness > VaginaClass.WETNESS_NORMAL && player.vaginas[0].vaginalWetness < VaginaClass.WETNESS_DROOLING)
        {
            outputText("Your " + vaginaDescript(0) + " squicks wetly from your gentle ministrations as your vulva become sensitive and engorged.");
        }
        if (player.vaginas[0].vaginalWetness >= VaginaClass.WETNESS_DROOLING)
        {
            outputText("Your " + vaginaDescript(0) + " gushes in response, dripping steadily over your victim's face, covering it with a spattering of girl-cum.");
        }
        //New PG
        ...
        if (player.balls >= 2)
        {
            ...
            {
                outputText("Your [balls] droop into her eyes, blinding and humiliating her in equal measure.  ");
            }
            else
            {
                outputText("Your [balls] obscure most of her face, making it difficult to see her reactions, but her mouth gives you all the feedback you need.  It doesn't take more than a few moments for her struggles to cease, and soon, her long insectile tongue is worming its way into your " + vaginaDescript(0) + ".  ");
            }
        }
        ...
        if (player.cumQ() < 75)
        {
            outputText(", enjoying the chance to cut loose and explore your male half.  ");
        }
        if (player.cumQ() >= 75 && player.cumQ() < 500)
        {
            outputText(", smearing your hardness in copious pre-cum and reveling at the marvelous sensation it gives you.  ");
        }
        if (player.cumQ() >= 500)
        {
            outputText(".  A few drops of your pre manage to drip onto her face, mixing with the fluids from your cunt.  ");
        }
        ...
        if (player.cumQ() >= 100)
        {
            ...
            if (player.balls >= 2)
            {
                outputText(ballsDescriptLight() + " and ");
            }
        ...
        if (player.breastRows.length > 0)
        {
            ...
            {
                outputText("You free a hand to tend to your [allbreasts] and aching nipples, alternating between squeezing your female flesh and tugging on your erect nipples.  ");
            }
        }
        outputText("\n\n");
        //New PG
        ...
        if (player.breastRows.length > 0)
        {
            if (player.biggestTitSize() >= 2)
            {
                ...
                {
                    outputText(", squirting milk all over the bee-girl's midsection.  ");
                }
                if (player.biggestLactation() > 2 && player.biggestLactation() <= 3)
                {
                    outputText(", spraying milk all over your bee-bitch, soaking her completely with your milky-white goodness.  ");
                }
                if (player.biggestLactation() > 3)
                {
                    outputText(", hosing milk everywhere, soaking the bee-girl completely.  The force of your orgasm seems to echo through your breasts, making them produce far more milk than normal.  ");
                }
                if (player.biggestLactation() <= 1)
                {
                    outputText(".  ");
        ...
        if (player.balls >= 2)
        {
            outputText("The warmth and pleasurable pressure of your male orgasm builds in your [balls], growing stronger as it migrates up your groin to the base of your pulsing member.  You squeeze your [cock] tightly, trying to hold on a bit longer, but the squirming tongue inside you is too much.  ");
        }
        //Mmmmmmangasm!
        if (player.isTaur())
        {
            outputText("As you're going to explode, you smile evilly as an idea crosses your mind. You turn around swiftly, leaving her surprised and still in the position of licking your pussy, her slender and squirming tongue out, and her face covered in girl-cum.  ");
        }
        if (player.cumQ() < 25)
        {
            ...
            if (!player.isTaur())
            {
                outputText("Spent at last, you rise up off of her, shivering as her tongue retracts from your woman-hood.");
            }
        }
        if (player.cumQ() >= 25 && player.cumQ() < 250)
        {
            ...
            if (player.balls >= 2)
            {
                outputText("with empty balls, ");
            }
            if (!player.isTaur())
            {
                outputText("you rise up off her, shivering as her slender and squirming tongue retracts from your " + vaginaDescript(0) + ".");
            }
            else
            {
                outputText("you walk away from her smiling, and sated.");
            }
        }
        if (player.cumQ() >= 250 && player.cumQ() < 500)
        {
            ...
            if (!player.isTaur())
            {
                outputText("You rise up off of her in mid-orgasm, aiming lower and painting her with each blast of seed.  ");
            }
            else
            {
                outputText("You aim right at the oval of her face and paint her with each blast of seed.  ");
            }
        ...
        if (player.cumQ() >= 500)
        {
            ...
            if (!player.isTaur())
            {
                outputText("You stand up off your victim, letting her tongue slip free from your soaked folds, and");
            }
            else
            {
                outputText("You");
            ...
            if (player.balls >= 2)
            {
                outputText("Your balls eventually empty");
            }
            else
            {
                outputText("Your body's cum supply eventually empties");
            }
    ...
    private function beeGirlRapeForTheDistinguishedGentleman() : Void
    {
        ...
        if (monster.HP < 1)
        {
            outputText("The bee-maiden staggers backward against her perch and wheezes.  Quickly, you launch yourself forward and pin her bodily against the giant flora, grabbing her feebly thrashing arms.  In a flash of inspiration, you pull a long supple leaf from the underside of the blossom, twisting it lengthwise and managing to bind her hands to the stem above her head with your makeshift cord before she can focus on what's happening.\n\n");
        }
        //(if lust win)
        else
        {
            outputText("The bee-girl's knees buckle together, and she starts fingering her pussy desperately, too preoccupied to take any offensive posture.  You suavely stroll up to her with a toothy smile and, laying one arm around her waist, gently push her backwards against the giant flower she sat on before.  As she looks at you with interest and confusion displayed equally across her face, you pull a long supple leaf down from the underside of the flower and twist it lengthwise to make a sort of grass rope.  You guide her hands together above her head and tie them to the stem there, quite pleased with your own ingenuity.\n\n");
        }
        //---------------------------------
        ...
        function tongueF() : Void
        {
            ...
            if (player.hasCock())
            {
                outputText(", spraying the fuzzy side of the bee's abdomen with your sticky come,");
            }
        ...
        function dickF() : Void
        {
            ...
            {
                outputText("  The bee-girl barely winces as the head of your shaft bumps up painfully against her cervix, quickly returning to a rictus of ecstasy as you continue jerking her ovipositor off with your now-sticky hand.");
            }
        ...
        var boobsF : Void->Void = function() : Void
        {
            clearOutput();
        ...
        var sharedEnd : Void->Void = function() : Void
        {
            ...
            if (player.hasCock())
            {
                outputText("stroke your " + Appearance.cockNoun(CockTypesEnum.HUMAN));
            }
            else if (player.hasVagina())
            {
                outputText("rub your " + vaginaDescript(0));
            }
            else
            {
                outputText("tweak your " + nippleDescript(0) + "s");
            }
            ...
            {
                outputText(" and spray your sticky fluids all over her tortured groin");
            }
            outputText(".\n\n");
    ...
    private function corruptNagaBitchesRapeABee() : Void
    {
        ...
        function dickF() : Void
        {
            ...
            if (player.cockTotal() > 1)
            {
                outputText("they free themselves from ");
            }
            else
            {
                outputText("it frees itself from ");
            }
            ...
            if (player.hasBalls())
            {
                outputText("Not while her hands cup your " + sackDescript() + " and knead tenderly, infusing you with the craving for release; ");
            }
            else
            {
                outputText("not ");
            }
            outputText("even when she takes your [cock] to the hilt, her eyes straining up in their sockets. You watch " + as3hx.Compat.parseInt(player.cocks[player.biggestCockIndex()].cockLength) + " inches of [cock] disappear inside her mouth, and the vibrations in her throat tell you that you'd hear her cooing if it wasn't packed tight.\n\n");
            ...
            if (player.hasBalls())
            {
                outputText("your " + sackDescript() + " clenches as ");
            }
            outputText("you explode inside her chest, ");
            //[normal cum load]
            if (player.cumQ() < 50)
            {
                outputText("a burst of cum settling itself in her belly.");
            }
            //[medium cum load]
            else if (player.cumQ() < 1000)
            {
                outputText("rope after rope of cum shooting into her belly.");
            }
            //[large cum load]
            else if (player.cumQ() < 2500)
            {
                outputText("waves of cum pumping into her belly and visibly bulging it out.");
            }
            //[huge cum load]
            else
            {
                outputText("an unending stream of cum gushing into her belly and stretching it out significantly.");
            }
        ...
        function vagF() : Void
        {
            ...
            {
                outputText("  Emergent from its sheath, she uses the base of her tongue as a bottom lip and begins to suck you off like a cock, unceasing in her lingual assault on the inside of your canal.");
            }
            outputText("\n\n");
        ...
        function hermF() : Void
        {
            ...
            if (player.cockTotal() > 1)
            {
                outputText("each of your shafts");
            }
            else
            {
            ...
            //[if Player cock => 12\"]
            if (player.cocks[player.biggestCockIndex()].cockLength > 12)
            {
                outputText(", surprising you with its elasticity");
            }
            outputText(".");
            ...
            if (player.cockTotal() > 1)
            {
                ...
                if (player.cockTotal() > 2)
                {
                    outputText("s, giving each a few pumps before moving on to the next");
                }
        ...
        var glessF : Void->Void = function() : Void
        {
            ...
            if (player.gender == 0)
            {
                outputText("Your lack of genitals requires you to come up with some creative solutions, and you decide to put your right brain to the test. ");
            }
    ...
    public function beeGirlsGetsDildoed() : Void
    {
        ...
        if (player.cor < 50)
        {
            outputText("You feel kind of bad for her, but y");
        }
        else
        {
            outputText("Y");
    ...
    private function layEggsInABeeSpiderLike() : Void
    {
        ...
        if (player.faceType == Face.SNAKE_FANGS || player.faceType == Face.SPIDER_FANGS)
        {
            outputText(", fanged");
        }
        ...
        {
            outputText("unfertilized");
        }
        else
        {
            outputText("fertilized");
        }
