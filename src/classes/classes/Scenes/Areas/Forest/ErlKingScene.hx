...
class ErlKingScene extends BaseContent
{
    public function new()
    {
        super();
    ...
    public function playerHuntScore() : Int
    {
        ...
        trace("Int + Spd = " + Std.string(player.inte + player.spe));
        trace("Base = " + Std.string((player.inte + player.spe) - (player.fatigue * 2)));
        var baseVal : Int = as3hx.Compat.parseInt((player.inte + player.spe) - (player.fatigue * 2));
        ...
        {
            baseVal -= as3hx.Compat.parseInt(player.racialScore(Races.KITSUNE, false) * 20);
            trace("-20 for each Kitsune part (-" + Std.string(player.racialScore(Races.KITSUNE, false) * 20) + ")");
        }
        if (player.racialScore(Races.BUNNY, false) > 0)
        {
            baseVal -= as3hx.Compat.parseInt(player.racialScore(Races.BUNNY, false) * 20);
            trace("-20 for each Bunny part (-" + Std.string(player.racialScore(Races.BUNNY, false) * 20) + ")");
        }
        if (player.racialScore(Races.HARPY, false) > 0)
        {
            baseVal -= as3hx.Compat.parseInt(player.racialScore(Races.HARPY, false) * 20);
            trace("-20 for each Harpy part (-" + Std.string(player.racialScore(Races.HARPY, false) * 20) + ")");
        }
        if (player.racialScore(Races.SLIME, false) > 0)
        {
            baseVal -= as3hx.Compat.parseInt(player.racialScore(Races.SLIME, false) * 10);
            trace("-10 for each Goo part (-" + Std.string(player.racialScore(Races.SLIME, false) * 10) + ")");
        }
    ...
    public function firstWildHuntEncounter() : Void
    {
        ...
        if (model.time.hours >= 0 && model.time.hours <= 10)
        {
            outputText(" chill morning");
        }
        else if (model.time.hours >= 11 && model.time.hours <= 13)
        {
            outputText(" unusually cold daytime");
        }
        else if (model.time.hours >= 14 && model.time.hours <= 17)
        {
            outputText(" brisk afternoon");
        }
        else if (model.time.hours >= 18 && model.time.hours <= 24)
        {
            outputText(" freezing night");
        }
    ...
    private function firstWildHuntChase(waited : Bool = false) : Void
    {
        clearOutput();
        ...
        else
        {
            ...
            {
                outputText(", and for a moment all you can think of are Whitney’s canine peppers");
            }
            outputText(".\n\n");
        ...
        {
            inventory.takeItem(consumables.FOXBERY, explorer.done);
        }
    }
    ...
    public function repeatWildHuntEncounter() : Void
    {
        ...
        {
            var canRunFast : Bool = player.isAnyRace(Races.BUNNY, Races.KITSUNE, Races.HARPY) || playerHuntScore() > 100;
    ...
    private function repeatWildHuntWait() : Void
    {
        clearOutput();
    ...
    private function repeatWildHuntChase() : Void
    {
    ...
    private function repeatWildHuntEscaped() : Void
    {
        clearOutput();
        ...
        if (player.isGoo())
        {
            outputText("You move like quicksilver over the forest floor, your slimy bottom half flowing over all obstacles, oozing you faster and faster, ever onward.");
        }
        else if (player.isBiped())
        {
            outputText("  Your [legs] pound against the mossy ground, deftly moving across the forest floor.");
        }
        else if (player.isNaga())
        {
            outputText("  You move like the wind across the mossy ground, your coils propelling you through the forest.");
        }
        else if (player.isDrider())
        {
            outputText("  Your multitude of legs skitter over the forest floor, propelling you between the trees at great speed.");
        }
        else if (player.isTaur())
        {
            outputText("  Your hooves send you rocketing through the forest, dodging between the trees and ducking below branches.");
        }
    ...
    private function mockSlow() : Void
    {
        clearOutput();
    ...
    private function mockFast() : Void
    {
        clearOutput();
    ...
    private function repeatWildHuntCaught(pScore : Int) : Void
    {
        clearOutput();
        ...
        if (player.isAnyRace(Races.BUNNY, Races.KITSUNE, Races.HARPY) || pScore > 100)
        {
            repeatWildHuntAWinnerIsYou();
        }
        else
    ...
    public function repeatWildHuntGivenToTheHounds() : Void
    {
        ...
        if (player.hasVagina())
        {
            outputText(" long tongue touching the bottom edge of your [vagina]");
        }
        else if (player.hasBalls())
        {
            outputText(" long tongue lapping at the base of your balls");
        }
        else if (player.hasCock())
        {
            outputText(" long tongue lapping at the base of your cock");
        }
        else
        {
            outputText(" long tongue slapping warmly against your taint");
        }
        ...
        if (player.isTaur())
        {
            outputText(" and realizing that you’re too tall, you fold your legs beneath you, dropping all the way to the ground,");
        }
        outputText(" when the other hound roughly grabs your");
        if (player.femininity < 30)
        {
            outputText(" strong jaw");
        }
        else if (player.femininity > 70)
        {
            outputText(" delicate chin");
        }
        else
        {
            outputText(" chin");
        ...
        {
            outputText("  The Hound behind grabs you by [onetail], using it as a handhold as he thrusts over and over into your [asshole].");
        }
        else
        {
            outputText("  The Hound behind grabs you by your [ass], thrusting into you again and again.");
        }
        ...
        var gemLoss : Int = as3hx.Compat.parseInt(10 + rand(15));
        if (player.gems < gemLoss)
        {
            gemLoss = player.gems;
        }
        player.gems -= gemLoss;
    ...
    private function repeatWildHuntAWinnerIsYou() : Void
    {
        spriteSelect(SpriteDb.s_erlking_clothed);
    ...
    private function whatsMyPrize() : Void
    {
        clearOutput();
        ...
        var gemFind : Int = as3hx.Compat.parseInt(10 + rand(15));
    ...
    private function stopTheMadness() : Void
    {
    ...
    private function surrenderToTheHounds() : Void
    //[Bad End]
    {
        ...
        {
            outputText("Your fur turns jet black.");
        }
        else
        {
            outputText("Black fur runs down your body like a tide coming in.");
        }
        outputText("  Your muscles bulge and swell beneath the midnight coat.");
        if (player.hasBreasts())
        {
            outputText("  Your chest first flattens out, then swells, as");
        }
        else
        {
            outputText("  T");
        ...
        {
            outputText("  Your doggie paws tingle as muscles build there, rebuilding them as stocky, athletic hound legs.");
        }
        else
        {
            outputText("  Your [legs] bend and crack, making you howl in pain as they rebuild themselves as onyx-clawed canine paws.");
        }
        outputText("\n\n");
        ...
        {
            outputText("  You whine, rolling on your back and with a start, realize that your wings must have fallen off while you were distracted with your cock.");
        }
        ...
        {
            outputText("  The world bends alarmingly as your nose pushes out, creating a black-furred muzzle where your mouth once was.");
        }
    ...
    private function predatoryPrey() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_erlking_nude);
        ...
        function vagF() : Void
        {
            ...
            if (player.isBiped() || player.isDrider() || player.isGoo())
            {
                outputText(" [legs] up, letting them wrap around his back.");
            }
            else if (player.isNaga())
            {
                outputText(" tail up, letting your coils wrap around his back.");
            }
        ...
        var taurVagF : Void->Void = function() : Void
        {
            ...
            {
                outputText("  The friction begins milking you, making you ooze milk down the trunk.");
            }
        ...
        var taurDickF : Void->Void = function() : Void
        {
    ...
    private function encounterPrincessGwynn() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_erlking_clothed);
        ...
        if (selector == 0)
        {
            outputText("The Princess’s pink fur is slathered in smears of saps and juices, and angry red stings pepper her body.  It’s very obvious that she’s fallen prey to the roaming tentacle monsters that haunt the forests.\n\n");
        }
        else if (selector == 1)
        {
            outputText("The Princess winces as low branches drag across her ass, and you can see angry red claw marks criss-crossing her pert ass.  It seems she’s run afoul of Akbal, the feline lord of the Deepwoods.\n\n");
        }
        else if (selector == 2)
        {
            outputText("The Princess coughs, a trickle of cum running down her lips and chin.  You see bruises on her throat and chin, and tiny claw marks across her body.  It looks like she was caught by a roving gang of imps.\n\n");
        }
        else if (selector == 3)
        {
            outputText("The Princess’ pink fur is stained with multicolored blotches, and she sways a little bit, apparently woozy.  Goblin attacks can leave a person that way for quite some time.\n\n");
        }
        else if (selector == 4)
        {
            outputText("The Princess’ neck and wrists bear bruises, as if someone had bound her up roughly and repeatedly.\n\n");
        }
        else if (selector == 5)
        {
            outputText("As she turns to squeeze between two trees, you see that the Princess has numerous claw marks up and down her back.  It looks like, since her transformation, her Hounds have turned on her.\n\n");
        }
        ...
        if (player.hasCock())
        {
            outputText("\"<i>I could suck your dick, or you could fuck my princess pussy, or ");
        }
        else
        {
            outputText("\"<i>");
        ...
        if (player.hasVagina())
        {
            outputText("I could eat your pussy, ");
        }
    ...
    private function gwynnSucksDicks() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_erlking_nude);
    ...
    private function gwynnGetsButtfuxed() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_erlking_nude);
    ...
    private function gwynnNomsDaCunts() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_erlking_nude);
    ...
    private function gwynnGetsDickmilked() : Void
    {
        clearOutput();
        spriteSelect(SpriteDb.s_erlking_nude);
    ...
    private function gwynnGibsGifts() : Void
    {
        clearOutput();
        ...
        if (rand(4) > 0)
        {
            inventory.takeItem(consumables.PRNPKR, explorer.done);
        }
        else
        {
            inventory.takeItem(consumables.PRNPKR, goldenRindBonus);
        }
    }
