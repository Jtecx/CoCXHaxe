...
class TentacleBeast extends Monster
{
    ...
    private function tentaclePhysicalAttack() : Void
    {
        ...
        var temp : Int = as3hx.Compat.parseInt((str + weaponAttack) - Math.random() * (player.tou) - player.armorDef);
        if (inDungeon)
        {
            temp += as3hx.Compat.parseInt(str + weaponAttack);
        }
        if (temp < 0)
        {
            temp = 0;
    ...
    private function tentacleEntwine() : Void
    {
        ...
        //Success{
            ...
            else
            {
                ...
                {
                    outputText("The creature quickly positions a long tentacle with a single sucker over your clitoris. You feel the power of the suction on you, and your body quickly heats up.  Your clit engorges, prompting the beast to latch the sucker onto your [clit].\n");
                }
                //Genderless
                else
                {
                    outputText("The creature quickly positions a long tentacle against your " + Appearance.assholeDescript(player) + ". It circles your pucker with slow, delicate strokes that bring unexpected warmth to your body.\n");
                }
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        ...
        //EL check
        else if (flags[kFLAGS.DISCOVERED_BEE_HIVE_DUNGEON] < 1 && rand(4) == 0)
        
        // && rand(10) == 0-na razie zamiast 10% szansa podniesiona do 25%		move to diff stronger variant of tentacle beast later on?{
        ...
        else if (!hpVictory && player.gender > 0)
        {
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (inDungeon)
        {
            SceneLib.forest.tentacleBeastScene.defeatedByAncientTentacleBeast();
        }
        //EL check
        else
        {
    ...
    public function new()
    {
        ...
        this.abilities = [
                {
                    call : tentaclePhysicalAttack,
                    type : ABILITY_PHYSICAL,
                    range : RANGE_MELEE,
                    tags : [],
                    condition : function() : Bool
                    {
                        return hasStatusEffect(StatusEffects.TentacleCoolDown);
                    }
                }, 
                {
                    call : tentacleEntwine,
                    type : ABILITY_PHYSICAL,
                    range : RANGE_MELEE,
                    tags : []
                }
