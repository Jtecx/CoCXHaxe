...
class NightmareScene extends BaseContent
{
    ...
    public function nightmareDefeat() : Void
    {
        ...
        if (silly())
        {
            outputText("You step over her with clear intent to finish your work as all the demons turn to watch, and in a single voice, they cry out <b><i>FINISH HER!!!</i></b> Smirking at the vocal support, you declare sternly:");
        }
        else
        {
            outputText("You step over her with clear intent to finish your work, but not without a little beration.\n\n");
        }
        ...
        {
            outputText("You end her life swiftly and mercilessly, putting a definitive end to her influence in the forest. You can already feel the area breathe easier as the thick fog of corruption surrounding this creature vanishes. The demons pay you no heed as you leave the area holding your opponent two horns as a trophy.\n\n");
        }
    ...
    public function nightmareVictory() : Void
    {
        ...
        if (!player.blockingBodyTransformations())
        {
            if (!player.hasVagina())
            {
                ...
                if (player.biggestTitSize() < 7)
                {
                    ...
                    if (!recalling)
                    {
                        ...
                        {
                            player.breastRows[player.biggestTitRow()].breastRating = 7;
                        }
                    }
                }
                ...
                if (player.hasBalls())
                {
                    outputText(" at the base of your balls");
                }
            ...
            if (player.isTaur())
            {
                ...
                {
                    outputText(" You have felt this attraction before while around Kelt, however her aura is infinitely stronger than that of the misogynic macho centaur.");
                }
                outputText(" Her cock is spreading for you a scent that hypnotises and lures you like a moth to a flame.");
                if (!player.inHeat)
                {
                    outputText(" The more you focus on her, the more your mind clouds as your vagina begins to abundantly drool. You realize her presence alone was enough to send you into deep heat.");
                }
                ...
                if (player.isFurCovered() && !InCollection(player.furColor, "black", "midnight black", "midnight"))
                {
                    if (!recalling)
                    {
                        player.furColor = randomChoice("black", "midnight black", "midnight");
                    }
            ...
            else
            {
                ...
                if (player.wings.type > Wings.NONE)
                {
                    outputText(" replacing your old ones");
                }
        ...
        if (!recalling)
        {
            throw new Error("Bruh, SH messed up the returns. Report this pls.");
        }
        doNext(recallWakeUp);
    }
