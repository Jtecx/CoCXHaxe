...
class WorldTree extends BaseContent
{
    ...
    public function YggdrasilDiscovery() : Void
    {
        ...
        else if (flags[kFLAGS.YGGDRASIL_GENERAL] < 1)
        {
            ...
            {
                flags[kFLAGS.YGGDRASIL_GENERAL]++;
            }
            YggdrasilMenu();
        }
        else
    ...
    public function YggdrasilMenu() : Void
    {
        ...
        {
            addButton(2, "Aid", AidTakeBranch).hint("Can Yggdrasil provide any assistance for your quest?");
        }
        else
        {
            addButton(2, "Take Branch", AidTakeBranch).hint("Take a fallen branch of the world tree");
        }
        if (flags[kFLAGS.YGGDRASIL_TF] < 1)
        {
            addButton(3, "Accept", AcceptTransform);
        }
        else
        {
            addButton(3, "Transform", AcceptTransform);
        }
        if (flags[kFLAGS.YGGDRASIL_TF] < 1)
        {
            addButton(14, "Decline", DeclineWolrdTreeOffer);
        }
        else
        {
            addButton(14, "Leave", LeaveWorldTree);
        }
    }
    ...
    public function AidTakeBranch() : Void
    {
        ...
        if (flags[kFLAGS.YGGDRASIL_BRANCH] < 1)
        {
            ...
            if (!player.isRace(Races.YGGDRASIL, 1, false))
            {
                outputText("What power we can grant you, we offer freely, you need only accept.");
            }
            else
            {
                outputText("We have already offered you what power we could.");
            }
            ...
            {
                outputText("you already know someone who can turn it into a weapon befitting a champion.</i>\"\n\n");
            }
            else
            {
                outputText("we have faith you will find a way to use it in our defense.</i>\"\n\n");
            }
            if (flags[kFLAGS.YGGDRASIL_BRANCH] < 1)
            {
                flags[kFLAGS.YGGDRASIL_BRANCH]++;
            }
        }
        else
        {
            ...
            if (flags[kFLAGS.YGGDRASIL_BRANCH] < 2)
            {
                flags[kFLAGS.YGGDRASIL_BRANCH]++;
            }
        }
    ...
    private function yggdrasilTF() : Void
    {
        ...
        if (player.gender <= 1 || (player.gender == 3 && player.mf("m", "f") == "m"))
        {
            ...
            if (player.cor >= 33 && player.cor < 66)
            {
                if (rand(2) == 0)
                {
                    if (player.cockTotal() > player.tentacleCocks())
                    {
                        var choices2 : Array<Dynamic> = [];
                        i = 0;
                        while (i < player.cockTotal())
                        {
                            if (player.cocks[i].cockType != CockTypesEnum.TENTACLE)
                            {
                                choices2[choices2.length] = i;
                            }
                            i++;
                        }
                ...
                else if (player.cockTotal() > player.stamenCocks())
                {
                    var choices3 : Array<Dynamic> = [];
                    i = 0;
                    while (i < player.cockTotal())
                    {
                        if (player.cocks[i].cockType != CockTypesEnum.STAMEN)
                        {
                            choices3[choices3.length] = i;
                        }
                        i++;
                    }
            ...
            if (player.cor < 33)
            {
                ...
                {
                    var choices4 : Array<Dynamic> = [];
                    i = 0;
                    while (i < player.cockTotal())
                    {
                        if (player.cocks[i].cockType != CockTypesEnum.STAMEN)
                        {
                            choices4[choices4.length] = i;
                        }
                        i++;
                    }
        ...
        if (player.cor >= 33 && player.cor < 66)
        {
            ...
            else if (player.arms.type != Arms.PLANT && changes < changeLimit)
            {
                outputText("\n\n");
        ...
        if (player.ears.type != Ears.LIZARD && player.tailType == Tail.YGGDRASIL && player.lowerBody == LowerBody.YGG_ROOT_CLAWS && changes < changeLimit)
        {
            ...
            if (player.isFurCovered() || player.hairLength > 0)
            {
                outputText("discreet ");
            }
