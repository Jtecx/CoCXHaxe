...
class TamanisDaughters extends Goblin
{
    private function midRoundMadness() : Void
    {
        var selector : Float = rand(4);
        if (selector == 0)
        {
        ...
        {
            outputText("A daughter lays down in front of you and starts jilling herself on the spot.  It's impossible to not glance down and see her or hear her pleasured moans.  You step away to remove the distraction but it definitely causes some discomfort in your [armor].\n\n");
        }
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (choice >= 2)
        {
            createStatusEffect(StatusEffects.Attacks, as3hx.Compat.parseInt((flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS] / 20) + (flags[kFLAGS.TAMANI_DAUGHTERS_LVL_UP] / 2)), 0, 0, 0);  //Tamani's Daughters get multiattacks!  
            eAttack();
        }
    }
    ...
    {
        super(true);
        var mod1 : Int = as3hx.Compat.parseInt(flags[kFLAGS.TAMANI_DAUGHTERS_LVL_UP] + flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS]);
        var mod2 : Int = as3hx.Compat.parseInt((flags[kFLAGS.TAMANI_DAUGHTERS_LVL_UP] * 6) + flags[kFLAGS.TAMANI_NUMBER_OF_DAUGHTERS]);
        this.a = "the group of ";
        ...
        this.level = 8 + mod2;
        this.gems = rand(15) + 5;
        this.drop = new WeightedDrop().add(consumables.GOB_ALE, 5).addMany(1, consumables.L_DRAFT, 
                        consumables.PINKDYE, 
                        consumables.BLUEDYE, 
                        consumables.ORANGDY, 
                        consumables.PURPDYE
        ...
        if (flags[kFLAGS.TAMANI_DAUGHTERS_LVL_UP] >= 2)
        {
            this.createPerk(PerkLib.JobSeducer, 0, 0, 0, 0);
        }
        if (flags[kFLAGS.TAMANI_DAUGHTERS_LVL_UP] >= 4)
        {
            this.createPerk(PerkLib.JobCourtesan, 0, 0, 0, 0);
        }
        if (flags[kFLAGS.TAMANI_DAUGHTERS_LVL_UP] >= 6)
        {
            this.createPerk(PerkLib.JobEromancer, 0, 0, 0, 0);
        }
        if (flags[kFLAGS.TAMANI_DAUGHTERS_LVL_UP] >= 8)
        {
            this.createPerk(PerkLib.EpicLibido, 0, 0, 0, 0);
        }
        if (flags[kFLAGS.TAMANI_DAUGHTERS_LVL_UP] >= 10)
        {
