...
class WoodElvesHuntingParty extends Monster
{
    ...
    public function WoodElfSpearStab() : Void
    {
        ...
        if (this.weaponAttack < 51)
        {
            damage *= (1 + (this.weaponAttack * 0.03));
        }
        else if (this.weaponAttack >= 51 && this.weaponAttack < 101)
        {
            damage *= (2.5 + ((this.weaponAttack - 50) * 0.025));
        }
        else if (this.weaponAttack >= 101 && this.weaponAttack < 151)
        {
            damage *= (3.75 + ((this.weaponAttack - 100) * 0.02));
        }
        else if (this.weaponAttack >= 151 && this.weaponAttack < 201)
        {
            damage *= (4.75 + ((this.weaponAttack - 150) * 0.015));
        }
        else
        {
            damage *= (5.5 + ((this.weaponAttack - 200) * 0.01));
        }
    ...
    override public function postPlayerBusyBtnSpecial(btnSpecial1 : CoCButton, btnSpecial2 : CoCButton) : Void
    {
        if (flags[kFLAGS.PLAYER_DISARMED_WEAPON_ID] != 0)
        {
            btnSpecial1.show("Pick (M)", pickUpMelee, "Pick up your melee weapon.");
        }
        if (flags[kFLAGS.PLAYER_DISARMED_WEAPON_R_ID] != 0)
        {
            btnSpecial2.show("Pick (R)", pickUpRange, "Pick up your range weapon.");
        }
    }
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (choice == 1)
        {
            if (player.weapon.canUnequip(false) || player.weaponRange.canUnequip(false))
            {
                Disarm();
            }
            else
    ...
    public function new()
    {
        ...
        this.drop = new WeightedDrop().add(weaponsrange.BOWLIGH, 1).add(weapons.SPEAR, 1).add(consumables.ELFEARS, 4);
