...
class HornetGirl extends Monster
{
    ...
    public function hornetJavelinAttack() : Void
    {
        ...
        else
        {
            ...
            if (player.buff("wasp paralyze venom").isPresent())
            {
                ...
                player.buff("hornet paralyze venom").addStats({
                                            str : -2,
                                            spe : -4,
                                            tou : -2
                                        }).withText("hornet paralyze venom").combatPermanent();
            }
            else
            {
            ...
            var damage : Int = 0;
            damage += as3hx.Compat.parseInt((str * 2.5) + rand(100));
    ...
    //Blind dodge change
    {
        ...
        //Determine if dodged!
        if (player.spe - spe > 0 && as3hx.Compat.parseInt(Math.random() * (((player.spe - spe) / 4) + 320)) > 320)
        {
            if (player.spe - spe < 32)
            {
                outputText("You narrowly avoid " + a + short + "'s stinger!");
            }
            if (player.spe - spe >= 32 && player.spe - spe < 80)
            {
                outputText("You dodge " + a + short + "'s stinger with superior quickness!");
            }
            if (player.spe - spe >= 80)
            {
                outputText("You deftly avoid " + a + short + "'s slow attempts to sting you.");
            }
            return;
        }
        ...
        if (rand(2) == 0)
        {
            ...
            if (player.gender == 1)
            {
                outputText("or dripping honey-slicked cunts beckoning you. ");
            }
            if (player.gender == 2)
            {
                outputText("planting your aching sex over her face while you lick her sweet honeypot. ");
            }
            if (player.gender == 3)
            {
                outputText("or cocks, tits, and puffy nipples. ");
            }
            ...
            if (player.lust > player.lust100 * 0.6)
            {
                ...
                {
                    outputText(" but it's difficult with the sensitive bulge in your groin.");
                }
                if (player.gender == 2)
                {
                    outputText(" but can't ignore the soaking wetness in your groin.");
                }
                if (player.sens > 50)
                {
                    outputText("  The sensitive nubs of your nipples rub tightly under your [armor].");
                }
            }
            else
            {
                outputText(" You shake your head and clear the thoughts from your head, focusing on the task at hand.");
            }
    ...
    public function new()
    {
        ...
        initWisLibSensCor(20, 60, 55, -100);
        ...
        this.level = 6;
        this.gems = rand(20) + 5;
        this.drop = new ChainedDrop().add(consumables.OVIELIX, 1 / 6).add(consumables.W__BOOK, 1 / 4).add(consumables.BEEHONY, 1 / 2).elseDrop(useables.B_CHITN);
