...
class Akbal extends Monster
{
    ...
    override private function handleBlind() : Bool
    {
        return true;
    }
    ...
    //Chances to miss:
    {
        ...
        //(medium HP damage){
            ...
            {
                damage = as3hx.Compat.parseInt((str + 40 + weaponAttack) - Math.random() * (player.tou) - player.armorDef);
            }
            else if (flags[kFLAGS.AKBAL_LVL_UP] >= 8)
            {
                damage = as3hx.Compat.parseInt((str + 20 + weaponAttack) - Math.random() * (player.tou) - player.armorDef);
            }
            else if (flags[kFLAGS.AKBAL_LVL_UP] >= 4)
            {
                damage = as3hx.Compat.parseInt((str + 10 + weaponAttack) - Math.random() * (player.tou) - player.armorDef);
            }
            else
            {
                damage = as3hx.Compat.parseInt((str + 5 + weaponAttack) - Math.random() * (player.tou) - player.armorDef);
            }
            if (damage <= 0)
            {
        ...
        else
        {
            ...
            if (flags[kFLAGS.AKBAL_LVL_UP] >= 12)
            {
                damage = as3hx.Compat.parseInt((str + 240 + weaponAttack) - Math.random() * (player.tou) - player.armorDef);
            }
            else if (flags[kFLAGS.AKBAL_LVL_UP] >= 8)
            {
                damage = as3hx.Compat.parseInt((str + 120 + weaponAttack) - Math.random() * (player.tou) - player.armorDef);
            }
            else if (flags[kFLAGS.AKBAL_LVL_UP] >= 4)
            {
                damage = as3hx.Compat.parseInt((str + 60 + weaponAttack) - Math.random() * (player.tou) - player.armorDef);
            }
            else
            {
                damage = as3hx.Compat.parseInt((str + 30 + weaponAttack) - Math.random() * (player.tou) - player.armorDef);
            }
            if (damage == 0)
            {
    ...
    //*Lust Attack -
    {
        ...
        if (!player.hasStatusEffect(StatusEffects.Whispered))
        {
            ...
            var lustattack1 : Float = 9 + rand(9);
            if (flags[kFLAGS.AKBAL_LVL_UP] >= 1)
            {
                lustattack1 += flags[kFLAGS.AKBAL_LVL_UP] + rand(flags[kFLAGS.AKBAL_LVL_UP]);
            }
        ...
        else
        {
            
            {
                ...
                var lustattack2 : Float = 12 + rand(12);
                if (flags[kFLAGS.AKBAL_LVL_UP] >= 1)
                {
                    lustattack2 += 3 + flags[kFLAGS.AKBAL_LVL_UP] + rand(flags[kFLAGS.AKBAL_LVL_UP]);
                }
        ...
        else
        {
            
            {
                ...
                if (player.hasStatusEffect(StatusEffects.Blizzard))
                {
                    ...
                    var damage2 : Int = as3hx.Compat.parseInt(inte / 4);
                    if (flags[kFLAGS.AKBAL_LVL_UP] >= 1)
                    {
                        damage2 *= as3hx.Compat.parseInt(1 + (flags[kFLAGS.AKBAL_LVL_UP] * 0.1));
                    }
    ...
    public function akbalHeal() : Void
    {
        ...
        var addedHP : Float = 30;
        if (flags[kFLAGS.AKBAL_LVL_UP] >= 1)
        {
            addedHP += 5 * flags[kFLAGS.AKBAL_LVL_UP];
        }
        addedHP *= (1 + rand(4));
        addHP(addedHP);
        lust += 10;
    ...
    public function new()
    {
        ...
        {
            IMutationsLib.CatLikeNimblenessIM.acquireMutation(this, "none", 3);
        }
        checkMonster();
    }
}
