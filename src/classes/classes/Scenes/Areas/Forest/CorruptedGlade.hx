...
class CorruptedGlade extends BaseContent implements TimeAwareInterface
{
    ...
    public function timeChange() : Bool
    {
        ...
        //Extinct if you destroyed 100 Corrupted Glades.{
            
            if (flags[kFLAGS.AMILY_DESTROYING_CORRUPTED_GLADES] > 0 && rand(6) == 0)
            {
                flags[kFLAGS.CORRUPTED_GLADES_DESTROYED]++;
            }
            if (flags[kFLAGS.KIHA_DESTROYING_CORRUPTED_GLADES] > 0 && rand(4) == 0)
            {
                flags[kFLAGS.CORRUPTED_GLADES_DESTROYED]++;
            }
            if (flags[kFLAGS.ZENJI_DESTROYING_CORRUPTED_GLADES] > 0 && rand(2) == 0)
            {
                flags[kFLAGS.CORRUPTED_GLADES_DESTROYED]++;
            }
            if (model.time.days % 3 == 0 && model.time.hours > 23)
            {
                flags[kFLAGS.CORRUPTED_GLADES_DESTROYED]--;
            }
        }
    ...
    private function scene(dryad : Bool) : Void
    {
        clearOutput();
    ...
    private function flowerFun() : Void
    {
        ...
        var cockEnd : Void->Void = function() : Void
        {
            ...
            if (player.hasPerk(PerkLib.MessyOrgasms) && player.cumMultiplier < 3)
            {
                booster += 2;
            }
        ...
        var oralF : Void->Void = function() : Void
    ...
    private function tentacleFun() : Void
    {
        ...
        var vagScene : Int->Void = function(tentacle : Int) : Void
        {
            ...
            //Small{
                ...
                var _sw1_ = (player.vaginas[0].vaginalLooseness);                
            ...
            //Normal{
                ...
                var _sw2_ = (player.vaginas[0].vaginalLooseness);                
            ...
            //Large{
                ...
                var _sw3_ = (player.vaginas[0].vaginalLooseness);                
            ...
            if (player.cuntChange(tentacleSize, true))
            {
                outputText("\n\n");
            ...
            if (rand(4) == 0 && player.hips.type <= 10)
        ...
        var suckF : Void->Void = function() : Void
        {
            ...
            if (rand(3) == 0 && player.cocks.length > 0 && player.hairColor == "green")
            {
                if (player.tentacleCocks() < player.cockTotal())
                {
                    ...
                    //multi{
                        ...
                        {
                            transformations.CockTentacle(x).applyEffect(false);
                        }
                    }
                }
            }
    ...
    private function treeBoobFun() : Void
    {
        ...
        if (!player.hasStatusEffect(StatusEffects.LustyTongue))
        {
            ...
            {
                outputText("  Thankfully, the feeling goes away after a few minutes, leaving you feeling just a bit more sensitive.");
            }
        }
        if (rand(2) == 0)
    ...
    private function destroyTheCorruptedGladesChoice() : Void
    {
        ...
        if (player.hasPerk(PerkLib.DragonFireBreath) || player.hasPerk(PerkLib.FireLord) || player.hasPerk(PerkLib.Hellfire))
        {
            ...
            {
                addButtonDisabled(button++, "Fire Breath", "You are too tired to destroy the foul glade this way.");
            }
            else
            {
        ...
        if ((player.hasPerk(PerkLib.EnlightenedNinetails) || player.hasPerk(PerkLib.CorruptedNinetails)) && player.tailType == Tail.FOX && player.tailCount >= 7)
        {
            ...
            {
                addButtonDisabled(button++, "Fox Fire", "You are too tired to destroy the foul glade this way.");
            }
            else
            {
        ...
        if (player.hasStatusEffect(StatusEffects.KnowsWhitefire))
        {
            if (player.mana < 40)
            {
                addButtonDisabled(button++, "Whitefire", "You are too low on mana to destroy the foul glade this way.");
            }
            else
            {
        ...
        if (player.hasKeyItem("Carpenter's Toolbox") >= 0 || player.weapon == weapons.L__AXE)
        {
            ...
            {
                addButtonDisabled(button++, "Axe", "You are too tired to destroy the foul glade this way.");
            }
            else
            {
        ...
        if (player.weaponVerb == "stab" || player.weaponVerb == "slash" || player.weaponVerb == "cleave" || player.weaponVerb == "keen cut" || player.isPartiallyStaffTypeWeapon())
        {
            ...
            {
                addButtonDisabled(button++, "Weapon", "You are too tired to destroy the foul glade this way.");
            }
            else
            {
        ...
        if (player.fatigue > player.maxOverFatigue() - 50)
        {
            addButtonDisabled(button++, "Your Hands", "You are too tired to destroy the foul glade this way.");
        }
        else
        {
    ...
    private function destroyTheCorruptedGlades(choice : Int) : Void
    {
        ...
        switch (choice)
        {
            ...
            case 2, 3:
                ...
                var minlimit1 : Float = 40 - (player.str / 10);
                if (minlimit1 < 5)
                {
                    minlimit1 = 5;
                }
            ...
            case 4:  //Weapon  
                ...
                var minlimit2 : Float = 30 - (player.str / 5);
                if (minlimit2 < 5)
                {
                    minlimit2 = 5;
                }
            ...
            else if (player.str >= 30 && player.str < 70)
        ...
        {
            outputText("That's one glade eliminated! With effort and dedication, you will be able to cleanse the forest of foul glades.");
        }
        else
        {
            outputText("Once again, the forest is cleansed of a tainted glade.");
        }
        dynStats("cor", -1);
        destroyAmount++;
        flags[kFLAGS.CORRUPTED_GLADES_DESTROYED] += destroyAmount;
        //Milestone events
        if (flags[kFLAGS.CORRUPTED_GLADES_DESTROYED] == 25)
        {
            outputText("\n\nYou have a feeling you'll see the glades somewhat less often.");
        }
        if (flags[kFLAGS.CORRUPTED_GLADES_DESTROYED] == 50)
        {
            outputText("\n\nYou have a feeling you'll see the glades less often.");
        }
        if (flags[kFLAGS.CORRUPTED_GLADES_DESTROYED] == 75)
        {
        ...
        {
            player.QueenOfTheForestGladeBonus();
        }
        endEncounter();
    }
}
