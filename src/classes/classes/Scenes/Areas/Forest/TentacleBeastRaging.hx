...
class TentacleBeastRaging extends Monster
{
    ...
    private function tentaclePhysicalAttackWrath() : Void
    {
        ...
        var temp : Int = as3hx.Compat.parseInt(((str + weaponAttack) * 2) - Math.random() * (player.tou) - player.armorDef);
        if (temp < 0)
        {
            temp = 0;
        }
    ...
    override public function defeated(hpVictory : Bool) : Void
    {
        ...
        if (hpVictory)
        {
            SceneLib.forest.tentacleBeastScene.choiceofaction();
        }
        else
        {
