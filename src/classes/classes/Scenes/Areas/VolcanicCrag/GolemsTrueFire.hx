...
class GolemsTrueFire extends Monster
{
    public function backhand() : Void
    {
        ...
        var damage : Float = as3hx.Compat.parseInt(((str + weaponAttack) * 5) - rand(player.tou) - player.armorDef);
    ...
    public function overhandSmash() : Void
    {
        ...
        var damage : Float = 100 + as3hx.Compat.parseInt(((str + weaponAttack) * 5) - rand(player.tou) - player.armorDef);
        if (player.hasPerk(PerkLib.FromTheFrozenWaste) || player.hasPerk(PerkLib.ColdAffinity))
        {
            damage *= 3;
        }
    ...
    {
        var damage : Float = (eBaseIntelligenceDamage() + eBaseWisdomDamage()) * 2;
        damage = Math.round(damage);
        if (hasStatusEffect(StatusEffects.Provoke))
        {
            damage = Math.round(damage * statusEffectv2(StatusEffects.Provoke));
