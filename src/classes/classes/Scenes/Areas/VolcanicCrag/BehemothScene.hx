...
class BehemothScene extends BaseContent
{
    ...
    {
        return as3hx.Compat.parseInt(flags[kFLAGS.BEHEMOTH_ANAL_PITCH] + flags[kFLAGS.BEHEMOTH_ANAL_CATCH] + flags[kFLAGS.BEHEMOTH_VAGINAL_CATCH] + flags[kFLAGS.BEHEMOTH_COCK_SUCKED] + flags[kFLAGS.BEHEMOTH_CUM_BATH] + flags[kFLAGS.BEHEMOTH_WATERSPORTS]);
    }
    ...
    private function behemothMenu() : Void
    {
        ...
        if (player.lust >= 33)
        {
            addButton(2, "Sex", behemothSexMenu, false, null, null, "Initiate sexy time with the Behemoth.");
        }
        else
        {
            addButtonDisabled(2, "Sex", "You are not aroused enough to initiate sex with the Behemoth.");
        }
        if (flags[kFLAGS.BEHEMOTH_TALKED_ABOUT_CUM] > 0)
        {
            addButton(3, "Get Cum", getCum).hint("Get some of Behemoth's cum!");
        }
        else
        {
            addButtonDisabled(3, "???", "You need to have talked to the Behemoth about his cum.");
        }
    ...
    public function talkToBehemothMenu() : Void
    {
        ...
        if (flags[kFLAGS.BEHEMOTH_TALK_LEVEL] >= 1)
        {
            addButton(2, "His Past", talkAboutSadStory).hint("Talk to the behemoth about his past.");
        }
        else
        {
            addButtonDisabled(2, "His Past", "Talk to the behemoth at least once to unlock this.");
        }
        if (timesSexed() >= 2)
        {
            addButton(3, "His Cum", talkAboutHisCum).hint("Talk to the behemoth about the special properties of his cum and how he managed to cum that much despite his ball size.");
        }
        else
        {
            addButtonDisabled(3, "His Cum", "Have sex with the behemoth at least twice to unlock this.");
        }
        if (flags[kFLAGS.BEHEMOTH_CHILDREN] > 0)
        {
            addButton(4, "Children", talkAboutHisChildren).hint("Talk to the behemoth about his son" + ((flags[kFLAGS.BEHEMOTH_CHILDREN] == 1) ? "" : "s") + ".");
        }
        else
        {
            addButtonDisabled(4, "???", "Have at least 1 child with him to unlock this.");
        }
    ...
    private function talkAboutYourself() : Void
    {
        ...
        if (flags[kFLAGS.DEFEATED_ZETAZ] > 0)
        {
            ...
            if (silly())
            {
                outputText("Did he just break the fourth wall?");
            }
        }
        if (flags[kFLAGS.LETHICE_DEFEATED] > 0)
        {
            ...
            if (player.armor == armors.LTHCARM)
            {
                ...
                if (player.lowerGarment.name == "nothing")
                {
                    outputText("He blushes when he looks at your " + ((player.hasCock()) ? "[cock] hanging freely from the opening" : "groin") + ".");
                }
            }
        }
    ...
    private function talkAboutBehemoth() : Void
    {
        ...
        if (flags[kFLAGS.BEHEMOTH_TALK_LEVEL] <= 1)
        {
            flags[kFLAGS.BEHEMOTH_TALK_LEVEL]++;
        }
        endEncounter();
    }
    private function talkAboutSadStory() : Void
    {
        ...
        {
            outputText("You walk over to the vacant chair and sit on it so you're facing the behemoth.");
        }
        else
        {
            outputText("Due to your irregular lower half, you move the chair aside and adjust your position so you're facing the behemoth.");
        }
        //Early life
        ...
        if (flags[kFLAGS.BEHEMOTH_TALK_LEVEL] <= 2)
        {
            flags[kFLAGS.BEHEMOTH_TALK_LEVEL]++;
        }
        endEncounter();
    ...
    public function winAgainstBehemoth() : Void
    {
        ...
        {
            outputText("The behemoth falls down on his knees, too badly beaten to continue fighting.");
        }
        else
        {
            outputText("The behemoth falls down on his knees, too turned on to continue fighting.");
        }
        outputText("\n\n\"<i>You have managed to beat me. Nothing beats a good workout,</i>\" he says.");
        //Get some gems back.
        if (flags[kFLAGS.BEHEMOTH_GEMS] >= 5)
        {
            var gemsBonus : Int = as3hx.Compat.parseInt(flags[kFLAGS.BEHEMOTH_GEMS] / 2);
            if (gemsBonus < 5)
            {
                gemsBonus = 5;
            }
        ...
        if (player.lust >= 33)
        {
            addButton(0, "Sex", behemothSexMenu, true, null, null, "Initiate sexy time with the Behemoth.");
        }
    ...
    {
        clearOutput();
        var gemsLost : Int = as3hx.Compat.parseInt(player.gems / 20);
        if (gemsLost < 20)
        {
            gemsLost = 20;
        }
        ...
        if (gemsLost > player.gems)
        {
            gemsLost = player.gems;
        }  //Keeps gems from going into negatives.  
        ...
        if (player.HP <= 0)
        {
            outputText("You collapse from your injuries.");
        }
        else
        {
            outputText("You collapse from your overwhelming desires.");
        }
    ...
    public function giveBirthToBehemoth(womb : Int = 0) : Void
    {
        ...
        {
            flags[kFLAGS.BEHEMOTH_CHILDREN] += 2;
        }
        else
        {
    ...
    //Sex menu
    private function behemothSexMenu(victory : Bool = false) : Void
    {
        clearOutput();
        if (victory)
        {
    ...
    private function analPitchBehemoth() : Void
    {
        ...
        else if (player.cumQ() >= 250 && player.cumQ() < 1000)
        {
            outputText(", filling it completely");
        }
        else if (player.cumQ() >= 1000)
        {
            outputText(", filling it completely and stuffing his stomach");
        }
        outputText(".");
    ...
    private function analCatchBehemoth() : Void
    {
        ...
        if (isVirgin)
        {
            outputText("\"<i>Gods, that's tight! It's a good thing you've been saving your virginity for me,</i>\" The behemoth says with a smile.");
        }
        if (player.looseness(false) < 3)
        {
            outputText(" It takes a bit of effort for him to slide his cock inside due to your tightness.");
        }
        else
        {
            outputText(" He easily slides his entire length into your [ass].");
        }
        ...
        if (player.hasCock())
        {
            outputText("Your " + ((player.cocks.length == 1) ? "cock" : "cocks") + " springs to full erectness and precum leaks from your " + player.multiCockDescript() + ". ");
        }
        ...
        if (player.hasVagina())
        {
            outputText("" + ((player.hasCock()) ? "femspunk" : "Femspunk") + " leaks from your [vagina].");
        }
        ...
        if (flags[kFLAGS.BEHEMOTH_ANAL_CATCH] <= 0)
        {
            outputText("He says, \"<i>I'm sure you've never known about how much a behemoth can cum. In fact, I'm capable of producing a lot of cum, far beyond what a minotaur can produce. Or even that fox-herm in Tel'Adre! Don't let the size of my balls deceive you,</i>\" he says with a chuckle. You recall his balls being three inches in length.");
        }
        ...
        if (player.hasCock())
        {
            ...
            if (player.cumQ() < 50)
            {
                outputText("shooting small ropes of jism all over the ground.");
            }
            else if (player.cumQ() >= 50 && player.cumQ() < 250)
            {
                outputText("shooting ropes of jism all over the ground.");
            }
            else if (player.cumQ() >= 250 && player.cumQ() < 500)
            {
                outputText("shooting jets of cum all over the ground, forming a small puddle.");
            }
            else if (player.cumQ() >= 500 && player.cumQ() < 2500)
            {
                outputText("shooting torrents of cum all over the ground, forming a medium puddle of your seed.");
            }
            else
            {
                outputText("shooting waves after waves of your seed that never seem to end. A large puddle of cum forms after you eventually stop cumming.");
            }
        }
        if (player.hasCock() && player.hasVagina())
        {
        ...
        if (player.hasVagina())
        {
            ...
            {
                outputText("" + ((player.hasCock()) ? "femspunk" : "Femspunk") + " bursts from your [vagina] and seeps down your [legs].");
            }
            else
            {
                outputText("" + ((player.hasCock()) ? "femspunk" : "Femspunk") + " gushes from your [vagina] and splashes onto the ground.");
            }
        }
        if (player.gender == 0)
        {
            outputText("Your entire body rocks from the overwhelming pleasure.");
        }
    ...
    {
        var isVirgin : Bool = player.hasVirginVagina();
        clearOutput();
        ...
        if (flags[kFLAGS.BEHEMOTH_CHILDREN] <= 0)
        {
            outputText("\"<i>I want to be a father someday. If I could start a family, that would be great,</i>\" the Behemoth says. ");
        }
        ...
        if (player.hasCock())
        {
            outputText("Your [cock] shoots ropes of jism at the Behemoth.");
        }
        ...
        if (flags[kFLAGS.BEHEMOTH_CHILDREN] < 3 && player.cor < 25 + player.corruptionTolerance)
        {
            ...
            {
                player.knockUp(PregnancyStore.PREGNANCY_BEHEMOTH, PregnancyStore.INCUBATION_BEHEMOTH, 50 + (flags[kFLAGS.BEHEMOTH_CHILDREN] * 15) + player.cor);
            }
        }
    ...
    {
        clearOutput();
        var hungerRefill : Int = as3hx.Compat.parseInt(100 - player.hunger);
        if (hungerRefill < 40)
        {
            hungerRefill = 40;
        }
    ...
    private function haveACumBathLiterally() : Void
    {
        ...
        if (flags[kFLAGS.BEHEMOTH_CUM_BATH] <= 0)
        {
            ...
            if (silly())
            {
                outputText("There has never been a literal cum bath in this game! ");
            }
        ...
        if (flags[kFLAGS.BEHEMOTH_CUM_BATH] <= 0)
        {
            outputText("You never knew having a literal cum bath felt so good!");
        }
        ...
        {
            outputText("Valeria says, \"<i>Thanks for the fluids!</i>\"");
        }
        ...
        if (player.armor == armors.GOOARMR)
        {
            SceneLib.valeria.feedValeria(100);
        }
        dynStats("str", 0.5, "tou", 0.5, "lus", 30);
        player.slimeFeed();
