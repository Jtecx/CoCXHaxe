...
class YoungFrostGiant extends Monster
{
    ...
    public function youngGiantAttackPunch() : Void
    {
        ...
        else if (rand(player.spe + 40) < spe)
        {
            ...
            damage = as3hx.Compat.parseInt((str + weaponAttack + 150) + rand(100));
            if (damage < 40)
            {
                damage = 40;
            }
        ...
        else
        {
            ...
            damage = as3hx.Compat.parseInt(50 + rand(str + weaponAttack));
            player.takePhysDamage(damage, true);
        }
    }
    ...
    public function youngGiantGrabFail(struggle : Bool = true) : Void
    {
        var damage : Int = 0;
        if (struggle)
        {
            ...
            else if (player.str >= 20 && player.str < 40)
            {
                ...
                {
                    damage = as3hx.Compat.parseInt(10 + rand((str + weaponAttack) * 0.5));
                    player.takePhysDamage(damage, true);
                }
            }
            else if (player.str < 20)
            {
                ...
                if (!player.hasPerk(PerkLib.Juggernaut) && armorPerk != "Heavy")
                {
                    damage = as3hx.Compat.parseInt(20 + rand((str + weaponAttack) * 0.75));
                    player.takePhysDamage(damage, true);
                }
            }
        ...
        switch (rand(6))
        {
            case 0, 1, 2:  //Taunt  
                ...
                damage = as3hx.Compat.parseInt(50 + rand((str + weaponAttack) * 0.4));
                player.takePhysDamage(damage, true);
            case 3, 4:  //Ground Pound  
                ...
                player.removeStatusEffect(StatusEffects.GiantGrabbed);
                damage = as3hx.Compat.parseInt(180 + rand((str + weaponAttack) * 1.2));
                player.takePhysDamage(damage, true);
            case 5:  //Throw  
                ...
                damage = as3hx.Compat.parseInt(50 + rand((str + weaponAttack) * 0.8));
                if (damage < 50)
                {
                    damage = 50;
                }
    ...
    public function youngGiantGrabSuccess() : Void
    {
        ...
        if (player.str >= 200)
        {
            outputText("You roar and force young giant's hand open. He gasps in surprise, using his other hand to close you in, but it's too late by then. You jump to the ground and roll away, readying for your next attack. ");
        }
        if (player.str >= 160 && player.str < 200)
        {
            outputText("You push, pull, squeeze, squirm and finally you escape young giant's grasp. You drop and roll and make distance before readying your weapon. ");
        }
        if (player.str >= 120 && player.str < 160)
        {
            outputText("With no small effort, you  pop out from young giant's clench. You run up his arm and jump down his back, making distance. ");
        }
        if (player.str >= 80 && player.str < 120)
        {
            outputText("Despite the cold, hard confines of young giant's hand, you manage to slip out of his hand and fall to the ground before scrambling up and running from the angry giant. ");
        }
        if (player.str >= 40 && player.str < 80)
        {
    ...
    public function youngGiantBoulderHit(mode : Int = 0) : Void
    {
        ...
        {
            outputText(", but you do look up just in time to nearly avoid the large boulder he chucked your way. Scrambling to react, you jump to the side, only to realize you chose the wrong side. The boulder hits you in the back, propelling you.  Battered, beaten, bruised, you struggle to stand, when young giant picks you up, laughs in his deep, mighty bellow, and punts you over a tree. You land several feet deep in a dirt, and see something flying toward you before passing out. ");
        }
        else
        {
            outputText("You begin to cast, focusing intently on summoning your magic. Too focused, though, as young giant propels the boulder in an arc to you. You notice the boulder just in time to not be crushed by it, though it still hits you and you fly several dozen yards before hitting a nice, jagged rock face. ");
        }
        if (player.hasStatusEffect(StatusEffects.GiantBoulder))
        {
            player.removeStatusEffect(StatusEffects.GiantBoulder);
        }
        var damage : Int = as3hx.Compat.parseInt((str * 2) + 200 + rand(100));
        if (damage < 200)
        {
            damage = 200;
        }
    ...
    public function youngGiantBoulderMiss() : Void
    {
        ...
        var damage : Int = as3hx.Compat.parseInt(10 + rand(str / 2));
        player.takePhysDamage(damage, true);
        outputText("\n\n");
    }
    ...
    override private function performCombatAction() : Void
    {
        ...
        if (chooser < 6)
        {
            youngGiantAttackPunch();
        }  //60% chance  
        if (chooser >= 6 && chooser < 9)
        {
            youngGiantGrab();
        }  //30% chance  
        if (chooser >= 9)
        {
            youngGiantBoulderThrow();
        }
    }
