...
class CancerAttackScene extends BaseContent
{
    ...
    public function rape() : Void
    {
        ...
        function maleRape() : Void
        {
            ...
            if (!player.hasVagina() || rand(2) == 0 && sceneHunter.uniHerms)
            {
                ...
                if (!player.isNaga() && !player.lowerBody != LowerBody.MELKIE && !player.lowerBody != LowerBody.GOO)
                {
                    outputText(" and proceeds to pull one of your leg away with his free pincer");
                }
                ...
                {
                    outputText(" Thankfully his rod isn’t too small or you wouldn’t be getting off at all from this, to be honest you have begun to enjoy yourself as well despite your predicament.");
                }
                outputText("\n\n<i>\"Ahhh… cumming…\"</i>\n\n" +
        ...
        var femaleRape : Void->Void = function() : Void
        {
            clearOutput();
            if (!player.hasVagina() && !player.hasCock())
            {
                ...
                if (silly())
                {
                    outputText(" while the cancer discusses with a man in a black suit how [monster he]'s not paid enough by producer Liadri to perform a proper raping job");
                }
                outputText(".");
            }
            else
    ...
    public function defeat() : Void
    {
        ...
        if (silly())
        {
            outputText("You ought to tell this stupid show producer miss Liadri about how her crabs dig their escapes instead of calmly letting themselves be raped!");
        }
        cleanupAfterCombat();
    }
}
