...
class PinchousWaterwearAndTools extends BaseContent
{
    public function new()
    {
        super();
    ...
    private function confirmBuy(itype : ItemType) : Void
    {
        ...
        switch (itype)
        {
            case consumables.ORCASUN:descString = "\"<i>A bottle of sunscreen lotion to protect against those annoying sunburns. That said its an item made for orca morphs who favor sunbathing. You might want to take precautions.</i>\"";
            case consumables.BUBBLEG:descString = "\"<i>Just a very ordinary candy, I eat these all the time but I’ve never seen an outsider take these. You might want to take precautions.</i>\"";
            case weapons.TRIDENT:descString = "\"<i>That is a trident… useful for catching fish or fighting underwater.</i>\"";
            case consumables.BLACKIN:descString = "\"<i>This is standard calligraphy equipment, you know to write stuff. However I heard that if consumed weird things can happens. You might want to take precautions.</i>\"";
            case consumables.AIRWEED:descString = "\"<i>Don’t like the breather, huh? Well these Airweeds will allow you to breath underwater for a time.</i>\"";
            case necklaces.MCPNECK:descString = "\"<i>You want to go underwater or improve your ability to swim? This necklace can allow you to do it. However it does not come cheap.</i>\"";
            case shields.BATTNET:descString = "\"<i>This net can be used for fishing or entangling foes.</i>\"";
            case headjewelries.AQBREATH:descString = "\"<i>Don’t like the breather? Well you will have to deal with it because it's still the cheapest diving item I got aside the Airweeds.</i>\"";
            case armors.SAQBIKNI, armors.SBABIKNI, armors.SBUBIKNI, armors.SGRBIKNI, armors.SPIBIKNI, armors.SREBIKNI, armors.SWHBIKNI, armors.SYEBIKNI:descString = "\"<i>What now? This is just a bikini… you know... for swimming. It’ll make any groundwalker less of a bad swimmer anyway that’s for sure.</i>\"";
            case armors.KBDRESS:descString = "\"<i>That’s for people with tentacle legs… I can’t understand that fashion style really, but apparently it would improve those tentacle people’s abilities.</i>\"";
        }
