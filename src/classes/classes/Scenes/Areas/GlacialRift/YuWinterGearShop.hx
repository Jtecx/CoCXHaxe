...
class YuWinterGearShop extends BaseContent
{
    ...
    public function YuMenuMain() : Void
    {
        ...
        if (flags[kFLAGS.YU_TALKS] == 1)
        {
            if (player.gender == 0)
            {
                addButtonDisabled(2, "Sex", "You need to have any gender other than genderless.");
            }
            else
            {
                addButton(2, "Sex", YuShopSex);
            }
        }
        else
        {
            addButtonDisabled(2, "Sex", "You need to talk with her first.");
        }
    ...
    public function YuShopTalkHerself() : Void
    {
        ...
        if (flags[kFLAGS.YU_TALKS] == 1)
        {
            outputText("Yu though you knew but Yu doesn’t mind repeating. ");
        }
    ...
    public function YuShopSex() : Void
    {
        ...
        function dickF() : Void
        {
            ...
            if (flags[kFLAGS.YU_SEX] == 1)
            {
                outputText("\"<i>[name], Yu just can’t wait anymore, so please go in Yu.</i>\"\n\n");
            }
            else
            {
                outputText("\"<i>[name] be Yu first so please  be gentle with Yu.</i>\"\n\n");
            }
