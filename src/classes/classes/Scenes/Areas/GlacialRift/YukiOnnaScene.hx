...
class YukiOnnaScene extends BaseContent
{
    ...
    public function encounterYukiOnnaYes() : Void
    {
        ...
        else
        {
            ...
            if (player.statusEffectv1(StatusEffects.MetYukiOnna) == 1 || player.statusEffectv1(StatusEffects.MetYukiOnna) == 3)
            {
                outputText("You wake up in a very different place from where you remember laying down. The bed is still there partially covered with a snowy blanket, but the hearth looks like it hasn't been used in years. There is a hole in the roof explaining where the snow came from and what isn't buried by snow is covered with a thin layer of dust like the place has been abandoned for years.\n\n");
            }
            else
            {
                outputText("You wake up on the same snowcovered bed you slept in last night. Truth be told you are somewhat confused because despite the delicious experience, you fully expected to die. It's a mystery how you didn't die all things considered since your form has literally been buried under a thin blanket of snow for several hours now.\n\n");
            }
    ...
    public function winAgainstYukiOnna() : Void
    {
        ...
        if (player.statusEffectv1(StatusEffects.MetYukiOnna) == 2)
        {
            player.addStatusValue(StatusEffects.MetYukiOnna, 1, 1);
        }
