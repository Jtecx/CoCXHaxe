...
class FrostGiantScene extends BaseContent
{
    public function winAgainstGiant() : Void
    {
        ...
        {
            if (Std.is(monster, FrostGiant))
            {
                awardAchievement("Like Chuck Norris", kACHIEVEMENTS.GENERAL_LIKE_CHUCK_NORRIS);
            }
            else if (Std.is(monster, YoungFrostGiant))
            {
                awardAchievement("Texas Ranger", kACHIEVEMENTS.GENERAL_TEXAS_RANGER);
            }
        }
    ...
    private function noseJob() : Void
    {
        ...
        if (player.cocks.length == 1)
        {
            outputText("His contracting nostril massages your prick enough to get you going.");
        }
        else
        {
            outputText("His contracting nostrils massage your pricks enough to get you going.");
        }
    ...
    private function rideVaginally() : Void
    {
        ...
        {
            outputText("dwarves your height.");
        }
        else if (player.tallness >= 36 && player.tallness < 48)
        {
            outputText("matches your height.");
        }
        else
        {
            outputText("is only slightly shorter than you are tall.");
        }
        outputText("\n\n");
        if (player.cocks.length <= 0)
        {
            outputText("Seeing no other option");
        }
        else
        {
            outputText("Deciding to give your vagina the attention it so craves");
        }
        ...
        if (player.hasCock())
        {
            outputText("You shoot ropes of jism from your [cocks] and cum all over his chest.  ");
        }
        if (player.hasVagina())
        {
            outputText("Feminine juice splatters all over from your [vagina] and onto his cock.  ");
        }
