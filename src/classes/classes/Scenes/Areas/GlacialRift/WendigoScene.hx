...
class WendigoScene extends BaseContent
{
    ...
    public function encounterWendigo2() : Void
    {
        ...
        else if (player.isRace(Races.WENDIGO, 1, false))
        {
    ...
    public function becomeWendigo() : Void
    {
        ...
        if (player.faceType == Face.ANIMAL_TOOTHS)
        {
            ...
            if (!player.hasPlainSkinOnly() && !player.hasPartialCoat(Skin.FUR))
            {
        ...
        if (player.tailType != Tail.WENDIGO)
        {
            CoC.instance.transformations.TailWendigo.applyEffect(false);
        }
        if (player.lowerBody != LowerBody.WENDIGO)
        {
            transformations.LowerBodyWendigo.applyEffect(false);
        }
        if (player.arms.type != Arms.WENDIGO)
        {
            CoC.instance.transformations.ArmsWendigo.applyEffect(false);
        }
        ...
        if (player.tongue.type != Tongue.RAVENOUS_TONGUE)
        {
            CoC.instance.transformations.TongueRavenous.applyEffect(false);
        }
        if (player.horns.type != Horns.ANTLERS)
        {
            CoC.instance.transformations.HornsAntlersWendigo.applyEffect(false);
        }
        if (player.ears.type != Ears.DEER)
        {
            CoC.instance.transformations.EarsDeer.applyEffect(false);
        }
        if (player.wings.type != Wings.LEVITATION)
        {
            CoC.instance.transformations.WingsLevitation.applyEffect(false);
        }
        if (player.rearBody.type != RearBody.FUR_COAT)
        {
            CoC.instance.transformations.RearBodyFurCoat.applyEffect(false);
        }
        if (player.hasVagina() && player.vaginaType() != VaginaClass.EQUINE)
        {
            player.vaginaType(VaginaClass.EQUINE);
        }
        if (player.hasVagina() && player.biggestTitSize() < 8)
        {
            player.breastRows[0].breastRating += 8;
        }
        if (player.hasCock() && player.horseCocks() < player.cocks.length)
        {
            player.addHorseCock();
        }
