...
class FrostGiant extends Monster
{
    ...
    override public function midAttackSeal() : Bool
    {
    ...
    override public function interceptPlayerAbility(ability : CombatAbility) : Bool
    {
        if (player.hasStatusEffect(StatusEffects.GiantBoulder))
        {
            if (Std.is(ability, AbstractSpell))
            {
                giantBoulderHit(2);
                return true;
            }
    ...
    public function giantAttackPunch() : Void
    {
        ...
        else if (rand(player.spe + 120) < spe)
        {
            ...
            damage = as3hx.Compat.parseInt((((str + weaponAttack) * 3) + 500) + rand(200));
            if (damage < 120)
            {
                damage = 120;
            }
        ...
        else
        {
            ...
            damage = as3hx.Compat.parseInt(150 + rand((str + weaponAttack) * 3));
            player.takePhysDamage(damage, true);
        }
    }
    ...
    public function giantGrab() : Void
    {
        ...
        if (player.getEvasionRoll())
        {
            ...
            var giantDmg : Float = 100 + rand(80);
            HP -= giantDmg;
    ...
    {
        if (rand(200) >= player.str || rand(10) == 0)
        {
            giantGrabFail();
        }
    ...
    public function giantGrabFail(struggle : Bool = true) : Void
    {
        var damage : Int = 0;
        if (struggle)
        {
            ...
            else if (player.str >= 50 && player.str < 100)
            {
                ...
                {
                    damage = as3hx.Compat.parseInt(30 + rand((str + weaponAttack) * 1.5));
                    player.takePhysDamage(damage, true);
                }
            }
            else if (player.str < 50)
            {
                ...
                if (!player.hasPerk(PerkLib.Juggernaut) && armorPerk != "Heavy")
                {
                    damage = as3hx.Compat.parseInt(60 + rand((str + weaponAttack) * 2.25));
                    player.takePhysDamage(damage, true);
                }
            }
        ...
        switch (rand(6))
        {
            case 0, 1, 2:  //Taunt  
                ...
                damage = as3hx.Compat.parseInt(150 + rand((str + weaponAttack) * 1.2));
                player.takePhysDamage(damage, true);
            case 3, 4:  //Ground Pound  
                ...
                player.removeStatusEffect(StatusEffects.GiantGrabbed);
                damage = as3hx.Compat.parseInt(540 + rand((str + weaponAttack) * 3.6));
                player.takePhysDamage(damage, true);
            case 5:  //Throw  
                ...
                damage = as3hx.Compat.parseInt(150 + rand((str + weaponAttack) * 2.4));
                if (damage < 150)
                {
                    damage = 150;
                }
    ...
    public function giantGrabSuccess() : Void
    {
        ...
        if (player.str >= 500)
        {
            outputText("You roar and force the giant's hand open. He gasps in surprise, using his other hand to close you in, but it's too late by then. You jump to the ground and roll away, readying for your next attack. ");
        }
        if (player.str >= 400 && player.str < 500)
        {
            outputText("You push, pull, squeeze, squirm and finally you escape the giant's grasp. You drop and roll and make distance before readying your weapon. ");
        }
        if (player.str >= 300 && player.str < 400)
        {
            outputText("With no small effort, you  pop out from the giant's clench. You run up his arm and jump down his back, making distance. ");
        }
        if (player.str >= 200 && player.str < 300)
        {
            outputText("Despite the cold, hard confines of the giant's hand, you manage to slip out of his hand and fall to the ground before scrambling up and running from the angry giant. ");
        }
        if (player.str >= 100 && player.str < 200)
        {
            outputText("Body aching, you exploit a light grip for a moment and drop to the ground with a hard thud. Forcing yourself to get up and MOVE, you run as quickly as your throbbing [ass] can handle. ");
        }
        if (player.str < 100)
        {
    ...
    public function giantBoulderThrow() : Void
    {
        ...
        if (!player.hasStatusEffect(StatusEffects.GiantBoulder))
        {
            player.createStatusEffect(StatusEffects.GiantBoulder, 0, 0, 0, 0);
        }
    }
    public function giantBoulderFantasize() : Void
    {
        ...
        if (player.cocks.length > 0)
        {
            if (player.biggestCockArea() >= 24)
            {
                ...
                {
                    outputText("collection of dicks. ");
                }
                else
                {
                    outputText(" one really big cock. ");
                }
    ...
    public function giantBoulderHit(mode : Int = 0) : Void
    {
        ...
        {
            outputText(", but you do look up just in time to nearly avoid the large boulder he chucked your way. Scrambling to react, you jump to the side, only to realize you chose the wrong side. The boulder hits you in the back, propelling you.  Battered, beaten, bruised, you struggle to stand, when the giant picks you up, laughs in his deep, mighty bellow, and punts you over a mountain. You land several feet deep in a snowbank, and see something flying toward you before passing out. ");
        }
        else
        {
            outputText("You begin to cast, focusing intently on summoning your magic. Too focused, though, as the giant propels the boulder in an arc to you. You notice the boulder just in time to not be crushed by it, though it still hits you and you fly several dozen yards before hitting a nice, jagged rock face. ");
        }
        if (player.hasStatusEffect(StatusEffects.GiantBoulder))
        {
            player.removeStatusEffect(StatusEffects.GiantBoulder);
        }
        var damage : Int = as3hx.Compat.parseInt((str * 5) + 1000 + rand(100));
        if (damage < 1000)
        {
            damage = 1000;
        }
    ...
    public function giantBoulderMiss() : Void
    {
        ...
        var damage : Int = as3hx.Compat.parseInt(25 + rand(str));
        player.takePhysDamage(damage, true);
        outputText("\n\n");
    }
    ...
    public function new()
    {
        ...
        this.drop = new WeightedDrop().add(consumables.ICICLE_, 1).add(null, 3);
