...
class DarkSlime extends Monster
{
    private function gooGalAttack() : Void
    {
        ...
        {
            outputText("Her body quivering from your flames, the dark slime ");
        }
        else
        {
            outputText("The dark slime holds its hands up and they morph into a replica of your [weapon].  Happily, she swings at you");
        }
        //Determine if dodged!
        ...
        if (hasPerk(PerkLib.Acid))
        {
            damage = as3hx.Compat.parseInt(((str + 10 + weaponAttack) * 1.5) - rand(player.tou) - player.armorDef);
        }
        else
        {
            damage = as3hx.Compat.parseInt(((str + weaponAttack) * 1.5) - rand(player.tou) - player.armorDef);
        }
        ...
        if (damage <= 0)
        {
            ...
            if (hasPerk(PerkLib.Acid))
            {
                ...
                {
                    outputText("tries to slap you, but the acid-bearing slap spatters weakly off your [armor]. ");
                }
                else
                {
                    outputText("tries to slap you with an acid-loaded hand, but it splatters off you ineffectually. ");
                }
            }
            ...
            else
            {
                ...
                if (rand(player.armorDef + player.tou) < player.armorDef)
                {
                    outputText(", her attack slapping fruitlessly against your [armor]. ");
                }
                else
                {
                    outputText(", her attack splattering ineffectually against you. ");
                }
            }
        }
        //everyone else
        else
        {
            if (hasPerk(PerkLib.Acid))
            {
                ...
                {
                    player.createStatusEffect(StatusEffects.AcidSlap, 0, 0, 0, 0);
                }
            }
            else
            {
                outputText(", painfully smacking her gooey limbs against your head.  You shake your [hair], clearing your head of the dazing slap. ");
            }
        }
        if (damage > 0)
        {
    ...
    private function gooEngulph() : Void
    {
        ...
        {
            player.createStatusEffect(StatusEffects.GooBind, 0, 0, 0, 0);
        }
    }
    ...
    public function new()
    {
        ...
        this.lust = 45;
        this.lustVuln = .75;
        this.drop = new ChainedDrop().add(weapons.PIPE, 1 / 10).add(consumables.DSLIMEJ, 1 / 2).elseDrop(useables.GREENGL);
