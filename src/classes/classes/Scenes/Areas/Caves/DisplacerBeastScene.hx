...
class DisplacerBeastScene extends BaseContent
{
    ...
    public function displacerBeastDefeat() : Void
    {
        ...
        if (!player.blockingBodyTransformations())
        {
            ...
            if (player.breastRows[0].lactationMultiplier < 1)
            {
        ...
        if (player.hasCock())
        {
            if (player.hasVagina())
            {
                outputText("r cock and pussy leak fluids");
            }
            else
            {
                outputText(" cum");
