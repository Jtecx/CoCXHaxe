...
class CaveWyrm extends Monster
{
    ...
    public function moveBlueFlamethrower() : Void
    {
        ...
        firedamage += as3hx.Compat.parseInt(eBaseIntelligenceDamage() * 1.2);
    ...
    public function cavewyrmLustzerk() : Void
    {
        ...
        {
            outputText(" as her dick begin to dribble precum");
        }
        outputText(".\n\n");
        var lustzerkBonus : Float = (40 + (40 * (1 + player.newGamePlusMod())));
    ...
    override public function won(hpVictory : Bool, pcCameWorms : Bool) : Void
    {
        if (flags[kFLAGS.HERM_CAVE_WYRM] == 1)
        {
            if (player.hasCock() && rand(3) == 0)
            {
                cavewyrmScene.losingToCaveWyrmD();
            }
            else
            {
                cavewyrmScene.losingToHermCaveWyrm();
            }
        }
        if (player.hasCock() && rand(3) == 0)
        {
        ...
        {
            cavewyrmScene.losingToCaveWyrm();
        }
    }
    ...
    override private function get_long() : String
    {
        ...
        if (game.flags[kFLAGS.HERM_CAVE_WYRM] == 1)
        {
            str += " Between her leg a somewhat evident bulging lizard cock that is pulsing with both a neon blue light and luminescent precum.";
        }
        return str;
    }
    ...
    public function new()
    {
        ...
        this.drop = new ChainedDrop().add(useables.EBONBLO, 1 / 20).add(consumables.METHIRC, 0.7);
