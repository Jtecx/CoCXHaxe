...
class BlightRidge extends BaseContent
{
    public var blightRidgeEncounter(get, never) : GroupEncounter;
    ...
    private function get_blightRidgeEncounter() : GroupEncounter
    {
    ...
    private function init() : Void
    {
        _blightRidgeEncounter = Encounters.group("blightridge", {
                            ...
                            when : function() : Bool
                            {
                                var v1 : Float = flags[kFLAGS.SIEGWEIRD_FOLLOWER];
                                var proc1a : Bool = flags[kFLAGS.SIEGWEIRD_FOLLOWER] < 2;
                                var proc1b : Bool = !(player.statusEffectv1(StatusEffects.AlvinaTraining2) == 3 && !SceneLib.alvinaFollower.AlvinaPurified);
                                var v2 : Float = player.statusEffectv1(StatusEffects.AlvinaTraining2);
                                var proc2a : Bool = flags[kFLAGS.SIEGWEIRD_FOLLOWER] == 2.5;
                                var proc2b : Bool = (player.statusEffectv1(StatusEffects.AlvinaTraining2) == 3 && flags[kFLAGS.SIEGWEIRD_FOLLOWER] >= 3);
                                var proc2c : Bool = (flags[kFLAGS.SIEGWEIRD_FOLLOWER] > 2 && flags[kFLAGS.SIEGWEIRD_FOLLOWER] < 4 && SceneLib.alvinaFollower.AlvinaPurified);
                            ...
                            when : function() : Bool
                            {
                                return flags[kFLAGS.SIEGWEIRD_FOLLOWER] == 2.5 || (player.statusEffectv1(StatusEffects.AlvinaTraining2) == 3 && flags[kFLAGS.SIEGWEIRD_FOLLOWER] >= 3) || (flags[kFLAGS.SIEGWEIRD_FOLLOWER] > 2 && flags[kFLAGS.SIEGWEIRD_FOLLOWER] < 4 && SceneLib.alvinaFollower.AlvinaPurified);
                            },
                            chance : blightRidgeChance,
                            night : false,
    ...
    public var areaLevel(default, never) : Int = 21;
    ...
    public function discover() : Void
    {
        ...
        {
            outputText("  Then again you have been to that Bizarre Bazaar, they seem to exist there to an extent without any trouble...");
        }
        ...
        if (player.cor < 66)
        {
            ...
            {
                outputText("  Gods... If Marae could see this.");
            }
            outputText("\n\n");
        }
        else if (player.cor >= 66)
        {
            ...
            if (flags[kFLAGS.MET_MARAE] >= 1)
            {
                outputText("  Heh, if Marae could see this she'd flip her shit.");
            }
            outputText("\n\n");
        }
    ...
    public function blightRidgeChance() : Float
    {
        var temp : Float = 0.5;
    ...
    private function findDenOfDesire() : Void
    {
        ...
        if (flags[kFLAGS.DEN_OF_DESIRE_QUEST] > 0)
        {
            outputText("This looks to be the lair of the golemancer Rathazul mentioned. ");
        }
